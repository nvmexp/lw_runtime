...
// __half data is colwerted to float when copying to host.
template <>
...
Status CopyToHost(T* dst, const T* src, size_t num_elements) {
  return GetStatus(lwdaMemcpyAsync(dst, src, num_elements * sizeof(T),
                                   lwdaMemcpyDeviceToHost));
}

Status CopyToHost(float* dst, const __half* src, size_t num_elements) {
  float* dev_dst = nullptr;
  RETURN_IF_ERROR_STATUS(GetStatus(lwdaHostGetDevicePointer(&dev_dst, dst, 0)));
  ColwertDeviceData(1.0, dev_dst, src, num_elements);
  return OkStatus();
}
...
Status DeviceDataEqual(const DeviceType* dev_first,
  ...
  // AllocateHostMemory, which ultimately calls lwdaMallocHost, is extremely
