...
Texture2D g_tx0 : register(t20, space2);
Texture2D g_tx1[2]: register(t20, space3);
...
ConstantBuffer<Transforms> cb0 : register(b12, space4);
...
ConstantBuffer<Constants> cb1[2] : register(b12, space5);

RWBuffer<uint> uav0   : register(u11, space6);
RWBuffer<uint> uav1[2] : register(u11, space7);
...
VS_OUTPUT_GEO GeometryVS2( VS_INPUT_GEO input )
{
	...
	float scale = cb1[cbIdx0].scaleFactor / length(transformedNormal);

    output.Tex1 = input.Tex1;

    output.Tex0.x = cb1[cbIdx1].half + scale * transformedNormal.x;
	output.Tex0.y = cb1[1].half - scale * transformedNormal.y;

    return output;
}

float4 GeometryPS2( VS_OUTPUT_GEO input ) : SV_Target
{
    ...
    BumpOffset.x = Bump.x * cb1[cbIdx0].half;
    BumpOffset.y = Bump.y * cb1[cbIdx1].half;
    ...
    uint Sampled0 = uav0[cbIdx0];
    uint Sampled1 = uav1[0][cbIdx1];
    uint Sampled2 = uav1[1][cbIdx1];
    ...
    return Diffuse + cb1[1].quarter * Elw;
}

VS_OUTPUT_ELW ElwVS2( VS_INPUT_ELW input )
{
...
float4 ElwPS2( VS_OUTPUT_ELW input ) : SV_Target
{
