...
static UINT32 RotateRight(UINT32 val, int shift)
{
    shift &= 0x1f;
    return (val << (0x20 - shift)) | (val >> shift);
}

static UINT32 RotateLeft(UINT32 val, int shift)
{
    shift &= 0x1f;
    return (val >> (0x20 - shift)) | (val << shift);
}

CTR64Encryptor::CTR64Encryptor()
{
    memset(m_initializatiolwector, 0, sizeof(m_initializatiolwector));
    memset(m_contentKey, 0, sizeof(m_initializatiolwector));

    SetContentKey(m_contentKey);
}

void CTR64Encryptor::Encrypt(const UINT08* in, UINT08* out, const UINT32 byteCount)
{
    UINT32 temp[AES::DW];
    UINT08 *ctr = reinterpret_cast<UINT08*>(&m_lwrCounterBlock[2]);

    for (size_t i = 0; byteCount / AES::B > i; ++i)
    {
        AES::RijndaelEncrypt(m_roundKey, m_lwrCounterBlock, temp);
        XOR128(out, in, temp);
        for (int j = 7; 0 <= j; --j)
        {
            ++ctr[j];
    ...
    if (0 != extra)
    {
        ...
        PadBlock(in, extra, (UINT08*)block);
        XOR128(block, block, temp);
        memcpy(out, block, AES::B);
    }
}
...
void AES::CalcRoundKey(const UINT32 key[AES::DW], UINT32 rk[AES::RK])
{
    rk[0] = key[0];
    rk[1] = key[1];
    rk[2] = key[2];
...
void CTR64Encryptor::SetContentKey(const UINT32 key[AES::DW])
{
    memcpy(m_contentKey, key, sizeof(m_contentKey));
    AES::CalcRoundKey(key, m_roundKey);
}

void CTR64Encryptor::SetInitializatiolwector(const UINT32 iv[AES::DW])
{
    memcpy(m_initializatiolwector, iv, sizeof(m_initializatiolwector));
    StartOver();
}

void CTR64Encryptor::StartOver()
{
    memcpy(m_lwrCounterBlock, m_initializatiolwector, sizeof(m_lwrCounterBlock));
}
...
#define ENCRYPT_ROUND(a, b, c, d, e, f, g, h, r)                                                                           \
    e = te0[a & 0xff] ^ te1[b & 0xff] ^ te2[c & 0xff] ^ te3[d & 0xff] ^ rk[r];                      ROTL_BYTE(a, b, c, d); \
    f = te0[c & 0xff] ^ te1[d & 0xff] ^ te2[a & 0xff] ^ te3[b & 0xff] ^ RotateRight(rk[r + 1], 8);  ROTL_BYTE(a, b, c, d); \
    g = te0[a & 0xff] ^ te1[b & 0xff] ^ te2[c & 0xff] ^ te3[d & 0xff] ^ RotateRight(rk[r + 2], 16); ROTL_BYTE(a, b, c, d); \
    h = te0[c & 0xff] ^ te1[d & 0xff] ^ te2[a & 0xff] ^ te3[b & 0xff] ^ RotateRight(rk[r + 3], 24)

void AES::RijndaelEncrypt(const UINT32 rk[AES::RK], const UINT32 pt[AES::DW], UINT32 ct[AES::DW])
{
    UINT32 s0, s1, s2, s3, t0, t1, t2, t3;

    // round 0
    s0 =            pt[0] ^ rk[0];
    s1 = RotateLeft(pt[1] ^ rk[1], 24);
    s2 = RotateLeft(pt[2] ^ rk[2], 16);
    s3 = RotateLeft(pt[3] ^ rk[3], 8);

    // round 1 - 8
    for (size_t i = 0; 4 > i; ++i, rk += 8)
    {
    ...
    // round 10
    ct[0] = tes[t0 & 0xff] ^ (tes[t1 & 0xff] << 8) ^ (tes[t2 & 0xff] << 16) ^ (tes[t3 & 0xff] << 24) ^ rk[8];  ROTL_BYTE(t0, t1, t2, t3);
    ct[1] = tes[t1 & 0xff] ^ (tes[t2 & 0xff] << 8) ^ (tes[t3 & 0xff] << 16) ^ (tes[t0 & 0xff] << 24) ^ rk[9];  ROTL_BYTE(t0, t1, t2, t3);
    ct[2] = tes[t2 & 0xff] ^ (tes[t3 & 0xff] << 8) ^ (tes[t0 & 0xff] << 16) ^ (tes[t1 & 0xff] << 24) ^ rk[10]; ROTL_BYTE(t0, t1, t2, t3);
    ct[3] = tes[t3 & 0xff] ^ (tes[t0 & 0xff] << 8) ^ (tes[t1 & 0xff] << 16) ^ (tes[t2 & 0xff] << 24) ^ rk[11];
}

void CTR64Encryptor::PadBlock(const UINT08* pt, const UINT32 bytes, UINT08 block[AES::B])
{
    memcpy(block, pt, bytes);
    block[bytes] = 0x80;
    if (bytes < AES::B - 1)
    {
        memset(block + bytes + 1, 0, AES::B - bytes - 1);
    }
}
