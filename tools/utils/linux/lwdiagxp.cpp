 ...
 *
 * Copyright 2010, 2013-2018 by LWPU Corporation. All rights reserved. All
...
string LwDiagXp::GetElw(string Variable)
{
    const char* elwVar = getelw(Variable.c_str());
    if(elwVar)
        return string(elwVar);
    else
        return string("");
}
...
{
   bool bResult = (access(strFilename.c_str(), F_OK) == 0);
   return bResult;
}
...
{
    return static_cast<Socket *>(new SocketLinux);
}
