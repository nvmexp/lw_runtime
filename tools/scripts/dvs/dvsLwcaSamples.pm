...
$VERSION     = 1.00;
@ISA         = qw(Exporter);
@EXPORT      = qw(print_test_preambule print_test_summary
                  ...
                  copy_header_files
                  set_commands set_elwironment set_paths
                  copy_msvc_build_extensions copy_installation
                  build_samples package_samples build_libraries
...
use constant BLACK_LIST_FILE => 'blacklist.txt';
...
{ 
  my ($mode_ref, $action_ref, $elwironment_ref, $keywords_include, $keywords_exclude) = @_;
  ...
  $samples_rootdir = "$toolkit_rootdir/samples/github-lwca-samples/Samples" if $samples_rootdir eq "";
  ...
  {
    $msvc_version = "VC10"; # Setting default MSVC Version to VC10 as no specific msvc specified.
  }
...
sub build_keyword_samples
{
  ...
  my $samples_list_file = "$samples_rootdir/../scripts/dvs/".SAMPLES_LIST_FILE;
...
sub copy_header_files
{
  ...
  my $lwgraph  = ($all || exists ${$keywords_include}{"LWGRAPH"})   && !exists ${$keywords_exclude}{"LWGRAPH"};
  my $lwjpeg   = ($all || exists ${$keywords_include}{"LWJPEG"})   && !exists ${$keywords_exclude}{"LWJPEG"};
  my $misc     = ($all || (!$lwrand && !$lwblas && !$lwfft && !$lwsparse && !$npp && !$lwsolver && !$lwjpeg && !$lwgraph));
  ...
  install_files ("$toolkit_rootdir/lwca/import/*.h", $bin_path);
  install_files ("$toolkit_rootdir/lwca/tools/lwdart/*.h", $bin_path);
  install_files ("$toolkit_rootdir/lwca/tools/lwdart/*.hpp", $bin_path);
  install_files ("$toolkit_rootdir/lwca/tools/cnprt/*.h", $bin_path);
  ...
  install_files ("$toolkit_rootdir/lwsparse/src/*.h", $bin_path) if $lwsparse || $lwsolver;
  ...
  install_files ("$toolkit_rootdir/lwgraph/include/lwgraph.h", $bin_path) if $lwgraph;
  ...
  if ($os eq "Linux" || $os eq "Darwin")
    {
      ...
      $$make .= "TARGET_ARCH=armv7l "  if $target_arch eq "ARMv7";
...
{
  $unzip = "${p4_build_rootdir}\\sw\\tools\\win32\\infozip\\unzip.exe";

  if ($os eq "win32")
  {
      $pkg_cmd .= "cd $toolkit_rootdir && ";
      $pkg_cmd .= "${p4_build_rootdir}\\sw\\tools\\win32\\infozip\\zip.exe ";
      ...
      $delete_pkg_cmd .= "$toolkit_rootdir\\build\\msys\\1.0\\bin\\rm.exe -f $package";
  }
  ...
  if ($os eq "Linux" or $os eq "Darwin")
    {
      ...
      if (-e "${package_dir}/LWCA-lwgraph-package.tar.bz2") {
        start_timer("LWGRAPH");
        dvsLwdaSamples::exec "tar --extract --bzip2 --directory $tgt_dir --file ${package_dir}/LWCA-lwgraph-package.tar.bz2";
        stop_timer("LWGRAPH");
      }
  ...
  if ($os eq "win32")
    {
      ...
      if (-e "${package_dir}\\LWCA-lwgraph-package.zip") { 
        start_timer("LWGRAPH");
        dvsLwdaSamples::exec "$unzip -o -d $tgt_dir ${package_dir}\\LWCA-lwgraph-package.zip";
        stop_timer("LWGRAPH");
      }
...
sub package_samples
{
  ...
  if ($os eq "win32") {
      my $samples_bin = "samples\\github-lwca-samples\\bin";
      ...
      foreach my $sample (sort (@$samples_list))
        {
          push @files, "samples\\github-lwca-samples\\Samples\\$sample";
        }
      dvsLwdaSamples::exec "$pkg_cmd ".join(" ", @files);
      dvsLwdaSamples::exec "$pkg_cmd $samples_bin\\*";
      dvsLwdaSamples::exec "$pkg_cmd samples\\github-lwca-samples\\Common\\*";
      dvsLwdaSamples::exec "$pkg_cmd samples\\github-lwca-samples\\scripts\\*";
      ...
      my $samples_bin = "samples/github-lwca-samples/bin/$target_arch/linux";
      $samples_bin .= "/debug" if ($build eq "debug");
      $samples_bin .= "/release" if ($build eq "release");
