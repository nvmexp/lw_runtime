...
my @supported_vs_years = ("2012", "2013", "2015", "2017", "2019");
my @os_list = ("Linux", "Windows", "MacOSX", "QNX", "Android");
...
for my $lwr_info (@info_list)
{
    ...
    if ($solution)
    {
        for (@build_vs_years)
        {
            # Skipping generate VS 2012 files for Vulkan sample,
            # due to build issues of GLM and other C++11 features used in the sample
            if ($_ eq "2012" && $vulkan)
            {
                next;
            }
            # Skipping generate VS 2012, 2013 files for DX12 sample as it is supported from VS2015 onwards
            if (($_ eq "2012" || $_ eq "2013") && $directX eq "12")
            {
                next;
            }
            GenerateSolution($_);
...
#    $sample_supported_platforms{"arm"}     = 0;
    ...
    for (@supported_platforms)
    {
        ...
        my $black_list_file = dirname(__FILE__) . "/../../scripts/blacklist.txt";
...
sub GenerateMasterReadmeMD
{
    ...
    $readme_md_text = Tag($readme_md_text, "MACOSX_SAMPLES", $samples_list_tmp);
...
sub GenerateReadmeMD
{
    ...
    if (@driver_api or @runtime_api)
    {
        ...
        if (@runtime_api)
        {
            $lwda_apis .= "### [LWCA Runtime API](http://docs.lwpu.com/lwca/lwca-runtime-api/index.html)\n";
            my $lwda_apis_tmp = "";
            for (@runtime_api)
            {
                $lwda_apis_tmp .= "$_ ";
...
END
    ...
    if ($os_exist{MacOSX})
    {
        push (@{$samples_by_os{MacOSX}}, $name);
        $build_details .= <<END
...
END
    ...
    if ($vs_year eq "2012")
    {
        $project_text = Tag($project_text, "ADDITIONAL_GLOBAL_PROPERTIES", "    <PlatformToolset>v110</PlatformToolset>\n<<ADDITIONAL_GLOBAL_PROPERTIES>>");
    }
    
    if ($vs_year eq "2013")
    {
        $project_text = Tag($project_text, "ADDITIONAL_GLOBAL_PROPERTIES", "    <PlatformToolset>v120</PlatformToolset>\n<<ADDITIONAL_GLOBAL_PROPERTIES>>");
    }
    
    if ($vs_year eq "2015")
    {
        $project_text = Tag($project_text, "ADDITIONAL_GLOBAL_PROPERTIES", "    <PlatformToolset>v140</PlatformToolset>\n<<ADDITIONAL_GLOBAL_PROPERTIES>>");
    }
...
END
    ...
    if ($cpp11)
    {
        $makefile_text = AddCPP11($makefile_text);
    }
...
sub AddDirectX
{
    ...
    else
    {
        ...
        $text = Tag($text, "PLATFORM_SPECIFIC_LINKER_32", "<AdditionalLibraryDirectories>\$(DXSDK_DIR)/Lib/x86;%(AdditionalLibraryDirectories)</AdditionalLibraryDirectories>\n<<PLATFORM_SPECIFIC_LINKER_32>>");
        $text = Tag($text, "PLATFORM_SPECIFIC_LINKER_64", "<AdditionalLibraryDirectories>\$(DXSDK_DIR)/Lib/\$(PlatformName);%(AdditionalLibraryDirectories)</AdditionalLibraryDirectories>\n<<PLATFORM_SPECIFIC_LINKER_64>>");
        $text = Tag($text, "ADDITIONAL_INCLUDE_DIRS", "\$(DXSDK_DIR)/include;<<ADDITIONAL_INCLUDE_DIRS>>");
        $text = Tag($text, "ADDITIONAL_LIBS", "d3d$directX.lib;d3dx$directX.lib;<<ADDITIONAL_LIBS>>");
    }
...
sub AddOpenGLES
{
    ...
    copy ("$samples_root/common/findgleslib.mk", "$output_dir") or die "findgleslib.mk copy failed";
...
sub AddOpenGL
{
    ...
    {
        copy ("$samples_root/common/findgllib.mk", "$output_dir") or die "findgllib.mk copy failed";
...
ifeq ($(TARGET_OS),darwin)
 ...
 LIBRARIES += -lGL -lGLU -lglut
    ...
    elsif ($builder eq "vcxproj")
    {
        ...
        $text = Tag($text, "PLATFORM_SPECIFIC_LINKER_32", "<AdditionalDependencies>glew32.lib;%(AdditionalDependencies)</AdditionalDependencies>\n<<PLATFORM_SPECIFIC_LINKER_32>>");
...
\$(shell echo "#include \\"FreeImage.h\\"" > test.c; echo "int main() { return 0; }" >> test.c ; \$(LWCC) \$(ALL_CCFLAGS) \$(INCLUDES) \$(LIBRARIES) -l freeimage test.c)
...
sub AddCPP11
{
    ...
    my $cpp11_text = '#Detect if installed version of GCC supports C++11
ifeq ($(TARGET_OS),linux)
    empty :=
    space := $(empty) $(empty)
...
$object->{TARGET}:$object->{SRC}";
        ...
        if (!$cmd)
        {
            ...
            else
            {
                if ($libLWRTC)
                {
                    $cmd = "\$($compiler) \$(INCLUDES) \$(CCFLAGS) \$(EXTRA_CCFLAGS) -o \$@ -c \$<";
                }
                else
                {
...
sub AddIncludePaths
{
    ...
    if ($builder eq "makefile")
    {
        my $path_text = "INCLUDES +=";
        for my $path (@include_paths)
        {
            $path_text .= " -I$path";
        }

        $text = Tag($text, "INCLUDES_PATHS", $path_text);
    }
    elsif ($builder eq "vcxproj")
    {
        for (@include_paths)
        {
            $text = Tag($text, "ADDITIONAL_INCLUDE_DIRS", "$_;<<ADDITIONAL_INCLUDE_DIRS>>");
        }
    }
...
END
        ...
        if ($fallback_min_ptx)
        {
            my ($desktop_sms_ref, $tegra_sms_ref) = GetSMs();
