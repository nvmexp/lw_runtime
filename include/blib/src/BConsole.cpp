...
)
{
    ...
    {
        IlwalidateRect (m_hWindow,NULL,TRUE);
    }
...
)
{
        ...
        (GetFolws() == m_hWindow))
    {
        if (m_dwFlags & flagCaretEnabled)
        {
            ...
            int x = m_ptLwrsor.x * m_sizeCell.cx - m_ptTopLeft.x;
            int y = m_ptLwrsor.y * m_sizeCell.cy - m_ptTopLeft.y;
            SetCaretPos (x,y);
            ShowCaret (m_hWindow);
            m_dwFlags |= flagCaretOn;
            return;
    ...
    if (bForceRecreate)
    {
        ...
        IlwalidateRect (m_hWindow,NULL,FALSE); // for some reason the caret needs a paint job when recreated - win32 bug
    }
}
...
{
    assert (m_ptLwrsor.x >= 0);
    assert (m_ptLwrsor.x < m_sizeBuffer.cx);
    assert (m_ptLwrsor.y >= 0);
    assert (m_ptLwrsor.y < m_sizeBuffer.cy);

    BOOL b = FALSE;
    int  x[2] = { m_ptLwrsor.x * m_sizeCell.cx - m_ptTopLeft.x, x[0] + m_sizeCell.cx };
    int  y[2] = { m_ptLwrsor.y * m_sizeCell.cy - m_ptTopLeft.y, y[0] + m_sizeCell.cy };
    RECT r;
...
)
{
    ...
    fillWord (m_pawScreen + m_sizeBuffer.cx * (m_sizeBuffer.cy - 1),(m_dwLwrrentColor << 8) | 0x20,m_sizeBuffer.cx);
}
...
)
{
    ...
    DWORD dwColor = (m_dwLwrrentColor << 8) & 0xff00;
    for (const char *psz = str; psz[0]; psz++)
    {
        switch (psz[0])
        {
            case '\n':   m_ptLwrsor.y++;                         break;
            case '\r':   m_ptLwrsor.x = 0;                       break;
            case '\t':   m_ptLwrsor.x = (m_ptLwrsor.x + 4) & ~3; break;
            case '\x01': clear2EOL(); break;
            default:
            {
                m_pawScreen[m_ptLwrsor.y * m_sizeBuffer.cx + m_ptLwrsor.x] = WORD(dwColor) | WORD(psz[0]);
                m_ptLwrsor.x++;
                break;
            }
        }

        if (m_ptLwrsor.x >= m_sizeBuffer.cx)
        {
            m_ptLwrsor.x = 0;
            m_ptLwrsor.y++;
        }
        assert (m_ptLwrsor.x < m_sizeBuffer.cx);
        if (m_ptLwrsor.y >= m_sizeBuffer.cy)
        {
            m_ptLwrsor.y--;
            scrollBuffer();
        }
    ...
    scrollToCaret(); 
    releaseMutex();
    IlwalidateRect (m_hWindow,NULL,FALSE);
}
...
{
    for (int x = m_ptLwrsor.x; x < m_sizeBuffer.cx; x++)
    {
        m_pawScreen[m_ptLwrsor.y * m_sizeBuffer.cx + x] = WORD(m_dwLwrrentColor << 8) | WORD(0x20);
    }
}
