...
public:
    ...
    inline void addAt   (R data, BPosition pos);
...
template<class T,class R,BOOL ts> void BLinkedList<T,R,ts>::addAt   (R data, BPosition pos)
{
    if (ts) EnterCriticalSection (&m_csThread);
    assert (pos);
    ENTRY *oEntry   = (ENTRY*)pos;  // old entry

    ENTRY *pEntry   = new ENTRY;
    if(!pEntry) throw BException(BException::outOfMemory);

    pEntry->m_pNext = oEntry;
    pEntry->m_pPrev = oEntry->m_pPrev;
    pEntry->m_Data  = data;
    if (oEntry->m_pPrev) oEntry->m_pPrev->m_pNext = pEntry;
                    else m_pHead                  = pEntry;
    oEntry->m_pPrev = pEntry;
