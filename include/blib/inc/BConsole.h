...
protected:
    LRESULT windowProc (HWND hWnd,UINT uMsg,WPARAM wParam,LPARAM lParam)
    {
        switch (uMsg)
        {
            ...
            MAP_MESSAGE (WM_SETFOLWS,       setFolws)
            MAP_MESSAGE (WM_KILLFOLWS,      killFolws)
    ...
    DECL_MESSAGE (setFolws);
    DECL_MESSAGE (killFolws);
...
public:
    ...
    // buffer
    void setCell   (int iX,int iY,WORD wValue)       { getMutex(); m_pawScreen[iY * m_sizeBuffer.cx + iX] = wValue; IlwalidateRect (m_hWindow,NULL,FALSE); releaseMutex(); }
    ...
    // palettes
    void setPalette (const COLORREF *pacolPalette) { getMutex(); memcpy (m_acolPalette,pacolPalette,sizeof(m_acolPalette)); IlwalidateRect (m_hWindow,NULL,TRUE); releaseMutex(); }
    void getPalette (COLORREF *pacolPalette) const { memcpy (pacolPalette,m_acolPalette,sizeof(m_acolPalette)); }

    // print
    void  setColor   (DWORD dwIndex)                { m_dwLwrrentColor = dwIndex; }
    DWORD getColor   (void)                         { return m_dwLwrrentColor; }
    ...
    void getXY     (POINT *pPt)        const { *pPt = m_ptLwrsor; }
    void getXY     (int *piX,int *piY) const { *piX = m_ptLwrsor.x; *piY = m_ptLwrsor.y; }
    void showCaret (void)                    
    { 
        getMutex(); 
...
public:
    ...
    void       clearScreen (void) { m_Screen.fillCells (0,0,m_sizeBuffer.cx,m_sizeBuffer.cy,WORD(m_Screen.getColor()<<8)|WORD(0x20));m_Screen.gotoXY(0,0);}
