...
typedef struct _lwdec_hevc_pic_s
{
    ...
    // general
    unsigned char tileformat;                                  // tileformat 0:TBL,1:KBL
    ...
    // sps
    unsigned short pic_width_in_luma_samples;                      // :15, 48(?)..16384, multiple of 8 (48 is smallest width supported by LWDEC for CTU size 16x16)
    ...
    unsigned int lw_qp_delta_enabled_flag                     : 1; //
    unsigned int diff_lw_qp_delta_depth                       : 2; //  0..3
    ...
    unsigned char  lwrr_pic_idx;                                // current  picture store buffer index,used to derive the store addess of frame buffer and MV
...
#define LWDEC_VP9HWPAD(x, y) unsigned char x[y]
