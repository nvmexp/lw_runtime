...
// 1  -  INTRODUCTION
// 
// The LWENC is a video encoder engine. Lwrrently only H.264 baseline, main 
// and high profile are supported. For baseline, only 1 slicegroup is allowed.
// LWENC has built-in motion estimation that enables it to do encode on its own
...
// LWENC motion search is divided into a simple integer search module and a high
// perf subpel refinement module. The integer search capability of LWENC is 
...
// Search and encoding involve forward dct and quantization, ilwerse dct and 
...
// As SVC is not supported in lwenc, so svc_extension_flag should be always false. Then, in the prefix NAL unit,
...
// ------------------------
// 
// (1) Reference pictures to be accessed by LWENC must be in tiled 16x16 format
...
//     'pic_struct' (either fields or frames). LWENC cannot encode an interlaced 
...
// within this range are used to encode the macroblocks. LWENC does not clear,
...
// of the pictures, and from this the lwenc app will derive the cropping 
...
} lwenc_pred_weight_table_s;   //  452 bytes
...
typedef struct
{
  ...
  unsigned int   lw21_enable                        : 1;    // the surface format is yuv or yvu
  unsigned int   input_bl_mode                      : 2;    // the input block linear mode: 0~gpu bl; 1~cheetah bl; 2~naive bl this is only for cmod and cmod/plugin
  unsigned int   reserved                           : 19;   // pad to LwU32
} lwenc_h264_surface_cfg_s;                                 // 32 bytes
...
typedef struct 
{
  ...
  unsigned int   num_ref_idx_l0_active_minus1       : 5;    // number of lwrrently active reference pictures in list 0 minus 1, range 0..30
  unsigned int   num_ref_idx_l1_active_minus1       : 5;    // number of lwrrently active reference pictures in list 1 minus 1, range 0..30
...
} lwenc_h264_me_hint_cfg_s;                     //16 bytes
...
} lwenc_h264_me_const_vec_s;                                // 4 bytes
...
} lwenc_h264_me_stamp_s;                                    // 12 bytes
...
} lwenc_h264_me_limit_mv_s;                                 // 8 bytes
...
typedef struct 
{
  ...
  lwenc_h264_me_limit_mv_s      limit_mv; 
  lwenc_h264_me_hint_cfg_s      predsrc;                    // predictor sources
  lwenc_h264_me_const_vec_s     l0_hint;                    // constant MV used as L0 hint
  lwenc_h264_me_const_vec_s     l1_hint;                    // constant MV used as L1 hint
  lwenc_h264_me_const_vec_s     l0_pred;                    // constant MV used as L0 predictor
  lwenc_h264_me_const_vec_s     l1_pred;                    // constant MV used as L1 predictor
  lwenc_h264_me_stamp_s         shape0;                     // priority 0 stamping pattern
  lwenc_h264_me_stamp_s         shape1;                     // priority 1 stamping pattern
  lwenc_h264_me_stamp_s         shape2;                     // priority 2 stamping pattern
  lwenc_h264_me_stamp_s         shape3;                     // priority 3 stamping pattern
  lwenc_h264_me_stamp_s         shape4;                     // priority 4 stamping pattern
  lwenc_h264_me_stamp_s         shape5;                     // priority 5 stamping pattern
  lwenc_h264_me_stamp_s         shape6;                     // priority 6 stamping pattern
  lwenc_h264_me_stamp_s         shape7;                     // priority 7 stamping pattern
  lwenc_me_tebif_s              teb_para;                   // TEB parameter 
  ...
  unsigned int partDecisionMadeByFPP              : 1;    // enable LW32 FPP;
  unsigned int penalty_factor_FPP                 : 6;    // MV with cost no larger than min_cost + ((min_cost * factor) >> 8) will go to SPS, not used from lwenc7
  unsigned int spatial_hint_pattern               : 4;
  unsigned int temporal_hint_pattern              : 6;
  unsigned int Lw16partDecisionMadeByFPP          : 1;    // enable LW16 making part decision
  ...
  unsigned int fbm_select_best_lw16_parttype_num    : 3;    // #winner partitions selected for lw16x16. valid range is 0~7;
  unsigned int fbm_op_winner_num_p_frame            : 3;    // FBM output number of integer winners for each PU. when fbm_op_winner_num_p_frame_lw8 has non-zero value, this field means the PU winners for LW64~LW16, i.e. w/o LW8
  ...
  unsigned int fbm_select_best_lw32_parttype_num    : 3;    // lw32 select best parttype num
  ...
  short        ct_threshold                         ;    // for OFS only, threshold in Census Transform callwlation
  ...
  unsigned char  lw8partDecisionMadeByFPP          : 1; //enable LW8x8 partition selection in FBM
  unsigned char  fbm_select_best_lw8_parttype_num  : 3; //#winner partitions selected for lw8x8. valid range is 0~7; 
  unsigned char  fbm_op_winner_num_p_frame_lw8     : 3; // FBM output number of integer winners for each PU of LW size 8. 
  unsigned char  reserved4                         : 1;
  
  unsigned char  lw64partDecisionMadeByFPP         : 1; //enable LW64x64 partition selection in FBM
  unsigned char  fbm_select_best_lw64_parttype_num : 3; //#winner partitions selected for lw64x64. valid range is 0~7; 
...
} lwenc_h264_me_control_s;                                  // 192 bytes
...
typedef struct 
{
  ...
  unsigned short mv_cost_bias;                              // bias applied to internal motion vector cost callwlation
  
  unsigned short intra_nxn_bias_multiplier;                 // 0..255 range; used for internal bias callwlation
  unsigned short intra_most_prob_bias_multiplier;           // 0..31 range; used for internal bias callwlation
  
           short pskip_bias_multiplier;                     // S16 range; used for internal bias callwlation
           short bdirect_bias_multiplier;                   // S16 range; used for internal bias callwlation
  ...
  unsigned int   mv_cost_enable                     : 1;    // enable mv cost callwlations in mode decision
  ...
  unsigned int   early_termination_ip1              : 1;    // enable IP1 early termination
           short bias_favor_intra_16x16_early;              // S16 range; used for internal bias callwlation
  ...
  unsigned int   calc_ssim                          : 1;    // Callwlate SSIM distortion: 0 disable, 1: enable. ssim_rdo = 1 => calc_ssim = 1
...
} lwenc_h264_md_control_s;                                  // 128 bytes
...
typedef struct 
{
  ...
  unsigned   int reserved2[11];                             // pad to 192 bytes to allow 3 reads of 64 bytes in ucode
                                                            //     this is optimal size as 160=5 reads of 32 will exceed dma queue
} lwenc_h264_quant_control_s;                               // 192 bytes    
...
} lwenc_h264_refpic_cmd_s;                                     // 4 bytes
...
} lwenc_h264_ref_pic_reorder_s;                                // 4 bytes 
...
typedef struct 
{
  ...
  unsigned int   me_control_idx                     : 4;    // index in array of lwenc_h264_me_control_s to use for this slice
  unsigned int   md_control_idx                     : 4;    // index in array of lwenc_h264_md_control_s to use for this slice
  unsigned int   q_control_idx                      : 4;    // index in array of lwenc_h264_quant_control_s to use for this slice
  ...
  unsigned int   wp_control_idx                     : 4;    // index in array of lwenc_pred_weight_table_s to use for this slice
  ...
  lwenc_h264_refpic_cmd_s        ref_pic_cmd_cfg;

  // int 7-22: RefPicListReorder/Modification: Defined in Section 7.4.3.1
  lwenc_h264_ref_pic_reorder_s   ref_pic_list_reorder_cmd[2][8];     // size shall not exceed num_ref_idx_lx_active_minus1 + 1 (8+8=16)

  //int 23-30: MMCO: Defined in Section: 7.4.3.3
  lwenc_h264_mmco_s              mmco_cmd[8] ; 

  // int 31
  unsigned int   reserved3;
  
} lwenc_h264_slice_control_s;                               // 128 bytes


// Quarter-resolution modifier flags for first_pass_source_half_scaled
#define LWENC_QRES_FLAG_ENABLE          0x1     // Set if quarter-resolution is enabled
#define LWENC_QRES_FLAG_DECIMATED       0x10    // Set to indicate that subsampling was performed with decimation rather than low-pass
#define LWENC_QRES_FLAG_PASS2REFS       0x20    // Set to indicate that 1st-pass uses subsampled 2nd pass references instead of self-generated references

// Lookahead macros for ab_beta(INFO1) and prev_act(INFO2)
#define LWENC_LOOKAHEAD_INFO1(Wp2Wi)        ((unsigned char)(Wp2Wi))    // clamp((int)(Wp*256.0/Wi), 0, 255) -> ratio of inter over intra complexity, 0=no lookahead
...
#define LWENC_LOOKAHEAD_INFO2(nextI,depth)  (((unsigned short)(nextI)) | (((unsigned short)(depth))<<8))

// HRD conformance and misc RC flags
#define LWENC_RC_HRD_VCL                0x1 // vcl_cpb_size and vcl_bitrate
#define LWENC_RC_HRD_NAL                0x2 // nal_cpb_size and nal_bitrate
#define LWENC_RC_HRD_STRICTGOPTARGET    0x4 // Strict GOP target (minimizes GOP-to-GOP rate fluctuations)
#define LWENC_RC_FIXED_PBRATIO          0x8U // Disable dynamic B-frame rhopbi adjustments
...
typedef struct
{
  ...
  unsigned char  maxQPD;                                    // max QP delta between two conselwtive QP updates
  unsigned char  baseQPD;                                   // initial QP delta between two conselwtive QP updates  
  ...
  unsigned char  first_pass_source_half_scaled;             // 0 : first pass on half resolution 1: first pass on full resolution (along with LWENC_QRES_FLAG_XXX)
...
} lwenc_aq_stat_s;                                         // 16 bytes

typedef struct 
{
           char  l0[8];                                     // reference picture list 0
           char  l1[8];                                     // reference picture list 1
           char  temp_dist_l0[8];                           // temporal distance of each ref pic in list 0 DiffPicOrderCnt( lwrrPicOrField, pic0) )
           char  temp_dist_l1[8];                           // temporal distance of each ref pic in list 1 DiffPicOrderCnt( lwrrPicOrField, pic1) )
  ...
  unsigned char  num_me_controls_minus1;                    // number of lwenc_h264_me_control_s entries in array minus1
  unsigned char  num_md_controls_minus1;                    // number of lwenc_h264_md_control_s entries in array minus1
  unsigned char  num_q_controls_minus1;                     // number of lwenc_h264_quant_control_s entries in array minus1
  unsigned int   slice_control_offset;                      // offset from start of top level "lwenc_h264_drv_pic_setup_s" structure
                                                            // to start of array of "lwenc_h264_slice_control_s" structures
  unsigned int   me_control_offset;                         // offset from start of top level "lwenc_h264_drv_pic_setup_s" structure
                                                            // to start of array of "lwenc_h264_me_control_s" structures
  unsigned int   md_control_offset;                         // offset from start of top level "lwenc_h264_drv_pic_setup_s" structure
                                                            // to start of array of "lwenc_h264_md_control_s" structures
  unsigned int   q_control_offset;                          // offset from start of top level "lwenc_h264_drv_pic_setup_s" structure
                                                            // to start of array of "lwenc_h264_quant_control_s" structures
  ...
  char           lwr_interview_ref_pic;                     //current interview ref pic
  ...
  unsigned int   slice_stat_offset;                         // offset from start of top level "lwenc_stat_data_s" structure
                                                            // to start of array of "lwenc_slice_stat_s" structure
  unsigned int   mpec_stat_offset;                          // offset from start of top level "lwenc_stat_data_s" structure
                                                            // to start of array of "lwenc_mpec_stat_s" structure
  unsigned int   wp_control_offset;                         // offset from start of top level "lwenc_h264_drv_pic_setup_s" structure
                                                            // to start of array of "lwenc_pred_weight_table_s" structure
                                                            // to start of array of "lwenc_mpec_stat_s" structure

  unsigned char  num_wp_controls_minus1;                    // number of lwenc_pred_weight_table_s entries in array minus 1
  ...
  unsigned char  aclwrate_skip_mv                   : 1;
  ...
  unsigned int   aq_stat_offset;                            // start of array of "lwenc_aq_stat_s" structure
  unsigned int   act_stat_offset;                           // offset from start of "lwenc_rc_pic_stat_s" structure,
                                                            // in the RC stats buffer. This field points to the start
                                                            // of data for external (delta-)QP map.
  unsigned int   stats_fifo_offset;                         // offset from start of top level "lwenc_stat_data_s" structure
} lwenc_h264_pic_control_s;                                 // 276 bytes


// This is the top level config structure passed by driver to lwenc. It must be aligned
...
// terminate with error code "LwC9B7EncErrorH264BadMagic" if there is a mismatch. The value
// for the current class is defined as:

#ifdef LW_LWENC_8_2
#define LW_LWENC_DRV_MAGIC_VALUE  0xC9B70006
#elif LW_LWENC_8_0
#define LW_LWENC_DRV_MAGIC_VALUE  0xC8B70006
#elif LW_LWENC_7_3
#define LW_LWENC_DRV_MAGIC_VALUE  0xC7B70006
#elif LW_LWENC_7_2
#define LW_LWENC_DRV_MAGIC_VALUE  0xC4B70006
#elif LW_LWENC_7_0
#define LW_LWENC_DRV_MAGIC_VALUE  0xC5B70006
#elif LW_LWENC_6_8
#define LW_LWENC_DRV_MAGIC_VALUE  0xB6B70006
#elif LW_LWENC_6_6
#define LW_LWENC_DRV_MAGIC_VALUE  0xB4B70006
#elif LW_LWENC_6_4
#define LW_LWENC_DRV_MAGIC_VALUE  0xC3B70006
#elif LW_LWENC_6_2
#define LW_LWENC_DRV_MAGIC_VALUE  0xC2B70006
#elif LW_LWENC_6_0
#define LW_LWENC_DRV_MAGIC_VALUE  0xc1b70006
#elif LW_LWENC_5_0
#define LW_LWENC_DRV_MAGIC_VALUE  0xd0b70006
#elif LW_LWENC_1_0
#define LW_LWENC_DRV_MAGIC_VALUE  0xc0b70006
#elif LW_MSENC_2_0
#define LW_MSENC_DRV_MAGIC_VALUE  0xa0b70006
#else
#define LW_MSENC_DRV_MAGIC_VALUE  0x90b70006
#endif
...
typedef struct 
{
  ...
  lwenc_h264_surface_cfg_s          refpic_cfg;             // defines layout of reference pictures     32 bytes
  lwenc_h264_surface_cfg_s          input_cfg;              // defines layout of input pictures         32 bytes
  lwenc_h264_surface_cfg_s          outputpic_cfg;          // defines layout of reconstructed pictures 32 bytes
  lwenc_h264_sps_data_s             sps_data;               //                                           4 bytes
  lwenc_h264_pps_data_s             pps_data;               //                                           8 bytes
  lwenc_h264_rc_s                   rate_control;           // Rate Control information                 88 bytes
  lwenc_h264_pic_control_s          pic_control;            //                                         276 bytes
  lwenc_h264_surface_cfg_s          half_scaled_outputpic_cfg; // defines layout of 2x2 subsampled reconstructed picture 32 bytes
  unsigned int                      gpTimer_timeout_val;    // GPTimer cycle count set from driver       4 bytes 
  
} lwenc_h264_drv_pic_setup_s;                               // 512 bytes
...
typedef struct
{
  ...
  unsigned int   input_bl_mode                      : 2;    // the input block linear mode: 0~gpu bl; 1~cheetah bl; 2~naive bl this is only for cmod and cmod/plugin
  unsigned int   reserved                           : 19;   // pad to LwU32
} lwenc_h265_surface_cfg_s;                                 // 32 bytes
...
typedef struct
{
  ...
  unsigned int   separate_colour_plane_flag         : 1;    //if this bit is enabled, the chroma_format_idc should be 3
  unsigned int   log2_max_pic_order_cnt_lsb_minus4  : 4;
  unsigned int   min_lw_size                        : 2;    //0-8x8, 1-16x16, 2-32x32, 3-64x64
  unsigned int   max_lw_size                        : 2;    //0-8x8, 1-16x16, 2-32x32, 3-64x64
  ...
  unsigned int   pcm_loop_filter_disable_flag       : 1;    // Disable loop filter and SAO for LW with pcm_flag to 1
...
typedef struct 
{
  ...
  unsigned int   lw_qp_delta_enabled_flag                    : 1;    // if set , it enables lw_qp_delta_abs is encoded at coding unit layer.
...
{  
  ...
  unsigned char  ab_alpha;                                  // VBR/CBR: min/target quality level (1..51); rcmode4: weight of prev frame activity compared to part of the current picture
  unsigned char  ab_beta;                                   // lookahead info1: ratio of inter over intra complexity (fix8); rcmode4: weight of current activity compared to neighbors
  unsigned short prev_act;                                  // lookahead info2: LWENC_LOOKAHEAD_INFO2; rcmode4: average activity level of the first reference (L0)
...
typedef struct 
{
           ...
           int   reserved                           : 8;
  unsigned short used_by_lwrr_pic_flag;           
  unsigned short use_delta_flag;                  
           short num_negative_pics;               
           short num_positive_pics;               
           short delta_poc_s0_minus1[16];         
  unsigned short used_by_lwrr_pic_s0_flag;        
           short delta_poc_s1_minus1[16];         
  unsigned short used_by_lwrr_pic_s1_flag;        
}lwenc_h265_strps_s;                   // 80bytes multiple of 4
...
typedef struct
{
  ...
  unsigned char  slh_inter_layer_pred_layer_idc[8];             //NumPicTotalLwrr <= 8
}lwenc_mv_hevc_cfg_s;                   //12bytes multiple of 4
...
typedef struct 
{
  ...
  unsigned short lw_early_terminatiom_enable        : 1;    // 0: disable LW early termination, 1: enable LW early termination
  ...
  unsigned int   mpec_threshold;                            // collect mpec stats after threshold mbs
                                                            // when mpec_stat_on is enabled and mpec_threshold is not equal to picWidthInMbs,
                                                            // rc mode 0 will be used to collect stats
  unsigned int   slice_stat_offset;                         // offset from start of top level "lwenc_h265_stat_data_s" structure
                                                            // to start of array of "lwenc_h265_slice_stat_s" structure
  unsigned int   mpec_stat_offset;                          // offset from start of top level "lwenc_h265_stat_data_s" structure
                                                            // to start of array of "lwenc_h265_mpec_stat_s" structure
  unsigned int   aq_stat_offset;                            // to start of array of "lwenc_h265_aq_stat_s" structure
  ...
  lwenc_h265_strps_s    short_term_rps;                     // if short_term_ref_pic_set_sps_flag is set to 0, the lwenc_h265_strps_s  structure is signalled in the slice  segment segment  header.
  lwenc_h265_ltrps_s    long_term_rps;                      // if long_term_ref_pics_present_flag is set to 1 in SPS , lwenc_h265_ltrps_s structure is signalled in the slice segment segment header
  lwenc_h265_ref_pic_list_modification_s    ref_pic_list_modification; // if lists_modification_present_flag is set to 1, then lwenc_h265_ref_pic_list_modification_s  structure is encoded in slice segment header.  
  unsigned int   rc_pass2_offset;
  
  unsigned int   wp_control_offset;                         // offset from start of top level "lwenc_h265_drv_pic_setup_s" structure
                                                            // to start of array of "lwenc_pred_weight_table_s" structure
  ...
  lwenc_mv_hevc_cfg_s mv_hevc_cfg;
  unsigned int   chroma_skip_threshold_4x4          : 16;   // low 3 bits is the factional, to check if chroma residual is big in skip mb, can't simply set chroma residual to 0 even though luma has been decided to skip
  ...
  unsigned int   same_poc[2];                               // set to 1 when the poc of lwr ref pic and neighbor ref pic are same, otherwise set it to 0.  
  unsigned int   act_stat_offset;                            // offset from start of "lwenc_rc_pic_stat_s" structure, points to data for external QP map
  unsigned int   stats_fifo_offset;                         // offset from start of top level "lwenc_stat_data_s" structure
} lwenc_h265_pic_control_s;                                 // 536 bytes

typedef struct 
{
  // 256 aligned
  unsigned int                      magic;                  // magic number, see text above             //  4 bytes
  lwenc_h265_surface_cfg_s          refpic_cfg;             // defines layout of reference pictures     // 32 bytes
  lwenc_h265_surface_cfg_s          input_cfg;              // defines layout of input pictures         // 32 bytes
  lwenc_h265_surface_cfg_s          outputpic_cfg;          // defines layout of reconstructed pictures // 32 bytes
  lwenc_h265_sps_data_s             sps_data;                                                           //  8 bytes
  lwenc_h265_pps_data_s             pps_data;                                                           //  8 bytes
  lwenc_h265_rc_s                   rate_control;           // Rate Control information                 // 96 bytes
  lwenc_h265_pic_control_s          pic_control;                                                        //536 bytes
  lwenc_h265_surface_cfg_s          half_scaled_outputpic_cfg; // defines layout of 2x2 subsampled reconstructed picture // 32 bytes
  unsigned int                      gpTimer_timeout_val;    // GPTimer cycle count set from driver       4 bytes 
  unsigned char                     reserved[240];          // Pad to 1024 bytes
  
} lwenc_h265_drv_pic_setup_s;
...
typedef struct 
{
  ...
  unsigned char  lw16x16_part_2Nx2N_enable       : 1;    // enable prediction for 2Nx2N
  unsigned char  lw16x16_part_2NxN_enable        : 1;    // enable prediction for 2NxN
  unsigned char  lw16x16_part_Nx2N_enable        : 1;    // enable prediction for Nx2N
  unsigned char  lw16x16_part_NxN_enable         : 1;    // enable prediction for NxN
  unsigned char  lw16x16_part_2NxnU_enable       : 1;    // enable prediction for 2NxnU
  unsigned char  lw16x16_part_2NxnD_enable       : 1;    // enable prediction for 2NxnD
  unsigned char  lw16x16_part_nLx2N_enable       : 1;    // enable prediction for nLx2N
  unsigned char  lw16x16_part_nRx2N_enable       : 1;    // enable prediction for nRx2N
    
  unsigned char  lw32x32_part_2Nx2N_enable       : 1;    // enable prediction for 2Nx2N
  unsigned char  lw32x32_part_2NxN_enable        : 1;    // enable prediction for 2NxN
  unsigned char  lw32x32_part_Nx2N_enable        : 1;    // enable prediction for Nx2N
  unsigned char  lw32x32_part_NxN_enable         : 1;    // enable prediction for NxN
  unsigned char  lw32x32_part_2NxnU_enable       : 1;    // enable prediction for 2NxnU
  unsigned char  lw32x32_part_2NxnD_enable       : 1;    // enable prediction for 2NxnD
  unsigned char  lw32x32_part_nLx2N_enable       : 1;    // enable prediction for nLx2N
  unsigned char  lw32x32_part_nRx2N_enable       : 1;    // enable prediction for nRx2N

  unsigned char  lw64x64_part_2Nx2N_enable       : 1;    // enable prediction for 2Nx2N
  unsigned char  lw64x64_part_2NxN_enable        : 1;    // enable prediction for 2NxN
  unsigned char  lw64x64_part_Nx2N_enable        : 1;    // enable prediction for Nx2N
  unsigned char  lw64x64_part_NxN_enable         : 1;    // enable prediction for NxN
  unsigned char  lw64x64_part_2NxnU_enable       : 1;    // enable prediction for 2NxnU
  unsigned char  lw64x64_part_2NxnD_enable       : 1;    // enable prediction for 2NxnD
  unsigned char  lw64x64_part_nLx2N_enable       : 1;    // enable prediction for nLx2N
  unsigned char  lw64x64_part_nRx2N_enable       : 1;    // enable prediction for nRx2N
  ...
  unsigned int   reserved1                          : 14;   // reserved for future special prediction extensions
           ...
           short bias_lwsize_8x8;                           //bias for 8x8 LW
           short bias_lwsize_16x16;                         //bias for 16x16 LW
           short bias_lwsize_32x32;                         //bias for 32x32 LW
           short bias_lwsize_64x64;                         //bias for 64x64 LW
  ...
  unsigned int   force_ipcm                         : 1;    // 0=normal encode; 1=force mpeb/mpec encode as IPCM
  unsigned int   early_termination_ip1              : 1;    // enable IP1 early termination
  unsigned int   tu_search_num_lw16                 : 3;    // tu search part num in lw16
  unsigned int   tu_search_num_lw32                 : 3;    // tu search part num in lw32
  ...
  short skip_evaluate_enable_lw8                    : 1;    // 0: not check skip ssd cost; 1, evaluate skip ssd cost
  short skip_evaluate_enable_lw16                   : 1;    // 0: not check skip ssd cost; 1, evaluate skip ssd cost
  short skip_evaluate_enable_lw32                   : 1;    // 0: not check skip ssd cost; 1, evaluate skip ssd cost
  short skip_evaluate_enable_lw64                   : 1;    // 0: not check skip ssd cost; 1, evaluate skip ssd cost
  short reserved2                                   : 1;    
  ...
  short rdo_level                                   : 3;    // 0:lw size decided by satd in P frame  1:lw size decided by ssd
...
} lwenc_h265_md_control_s;                                  // 128 bytes
...
} lwenc_h265_slice_control_s;                              // 64 bytes
...
} lwenc_h265_quant_control_s;                               // 16 bytes 
...
}lwenc_vp9_filter_cfg_s;  //16 bytes
...
}lwenc_vp9_quant_cfg_s;  //44 bytes
...
typedef struct
{
    ...
    //filter param
    lwenc_vp9_filter_cfg_s filter_cfg;              // filter config
    //quant param
    lwenc_vp9_quant_cfg_s quant_cfg;                // quant & round config
    
    lwenc_vp9_smalle_config_s smalle_cfg;           // small-e config
    unsigned int   num_tile_controls_minus1;        // number of lwenc_vp9_tile_control_s entries in array minus1
    unsigned int   tile_controls_offset;            // offset from start of top level "lwenc_vp9_pic_control_s" structure
                                                    // to start of array of "lwenc_vp9_tile_control_s" structures
...
}lwenc_vp9_pic_control_s;  //112 bytes
...
typedef struct
{
    ...
    unsigned int reserved     : 2;
    lwenc_vp9_modedma_cfg_s mode_dma_cfg;           // mode-dma config for this tile
    unsigned char tile_idx;                         // tile index in the picture
    unsigned char reserved2[55];                
}lwenc_vp9_tile_control_s;  //64bytes
...
} lwenc_vp9_rc_s;                                           // 56 bytes, lwrrently keep the same as vp8
...
typedef struct
{
  ...
  unsigned int   intra_lw32x32_count;                       // number of intra 32x32 LWs.    
  unsigned int   inter_lw32x32_count;                       // number of inter 32x32 LWs.
  unsigned int   intra_lw16x16_count;                       // number of intra 16x16 LWs.    
  unsigned int   inter_lw16x16_count;                       // number of inter 16x16 LWs.
  unsigned int   intra_lw8x8_count;                         // number of intra 8x8   LWs.    
  unsigned int   inter_lw8x8_count;                         // number of inter 8x8   LWs.
  unsigned int   lwmulative_intra_cost;                     // sum of all intra SA(T)Ds
  unsigned int   lwmulative_inter_cost;                     // sum of all inter SA(T)Ds
...
} lwenc_vp8_surface_cfg_s;                                  // 28 bytes
...
} lwenc_vp8_sequence_data_s;                                // 4 bytes

typedef struct 
{
  unsigned int   frame_type                         : 1;    // 0=key frame, 1=interframe
  unsigned int   version                            : 3;    // VP8 version, also affects luma/chroma mocomp: 0=bilwbic/bilwbic, 1,2=bilinear/bilinear, 3..7=bilinear/fullpel (SE: perhaps separate flags for bilinear/fullpel?)
  ...
  unsigned int   clamp_type                         : 1;    // 0=clamping of reconstructed samples required, 1=no clamping necessary (SE: not supported by LWENC?)
  ...
  unsigned int   ref_frame_sign_bias_gf             : 1;    // golden frame is future frame (ilwert mv sign)
  unsigned int   ref_frame_sign_bias_arf            : 1;    // alt_ref frame is future frame (ilwert mv sign)
...
} lwenc_vp8_picture_data_s;                                 // 32 bytes
...
typedef struct 
{
  ...
  unsigned short early_intra_disable_mpeb_threshold;        // if early intra cost is below this, disable full search & select intra
  //unsigned short lambda_multiplier_mvcost;                  // used for mv cost callwlation
           short bias_favor_intra_16x16_early;              // bias for early intra (subtracts from early intra 16x16 cost)

  unsigned short lambda_multiplier_intra[4];                // used for intra mode cost callwlation  [segment_id]
  ...
  unsigned int   mdpLambdaCoefMv                    :16;
           ...
           short mv_cost_bias;                              // bias applied to internal motion vector cost callwlation
           short bias_intra_over_inter;                     // bias for any intra   (subtracts from intra cost)
...
} lwenc_vp8_md_control_s;                                   // 64 bytes
...
} lwenc_vp8_me_hint_cfg_s;                                  // 8 bytes
...
} lwenc_vp8_me_limit_mv_s;                                 // 8 bytes
...
} lwenc_vp8_me_const_vec_s;                                // 4 bytes
...
typedef struct 
{
  ...
  lwenc_vp8_me_limit_mv_s      limit_mv; 
  lwenc_vp8_me_hint_cfg_s      predsrc;                     // predictor sources
  lwenc_vp8_me_const_vec_s     l0_hint;                     // constant MV used as L0 hint
  lwenc_vp8_me_const_vec_s     l1_hint;                     // constant MV used as L1 hint
  lwenc_vp8_me_const_vec_s     l0_pred;                     // constant MV used as L0 predictor
  // should this be made an array?
  lwenc_vp8_me_stamp_s         shape0;                      // priority 0 stamping pattern
  lwenc_vp8_me_stamp_s         shape1;                      // priority 1 stamping pattern
  lwenc_vp8_me_stamp_s         shape2;                      // priority 2 stamping pattern
  lwenc_vp8_me_stamp_s         shape3;                      // priority 3 stamping pattern
  lwenc_vp8_me_stamp_s         shape4;                      // priority 4 stamping pattern
  lwenc_vp8_me_stamp_s         shape5;                      // priority 5 stamping pattern
  lwenc_vp8_me_stamp_s         shape6;                      // priority 6 stamping pattern
  lwenc_vp8_me_stamp_s         shape7;                      // priority 7 stamping pattern

  lwenc_me_tebif_s             teb_para;                    // teb parameters
  unsigned char                mbc_mb_size;                 // MBC size in MB
  unsigned char                tempmv_scale;                // temporal mv scale
  unsigned char                reserved1[2];

} lwenc_vp8_me_control_s;                                   // 136 bytes
...
} lwenc_vp8_quant_control_s;                                // 72 bytes
...
} lwenc_vp8_pic_control_s;                                 // 48 bytes
...
} lwenc_vp8_rc_s;                                          // 56 bytes

typedef struct 
{
  // 256 aligned
  unsigned int                      magic;                  // magic number
  lwenc_vp8_surface_cfg_s           refpic_cfg;             // 28 bytes
  lwenc_vp8_surface_cfg_s           input_cfg;              // 28 bytes
  lwenc_vp8_surface_cfg_s           outputpic_cfg;          // 28 bytes
  lwenc_vp8_sequence_data_s         sequence_data;          // 4 bytes
  lwenc_vp8_picture_data_s          picture_data;           // 32 bytes
  lwenc_vp8_pic_control_s           pic_control;            // 48 bytes
  lwenc_vp8_me_control_s            me_control;             // 136 bytes
  lwenc_vp8_md_control_s            md_control;             // 64 bytes
  lwenc_vp8_quant_control_s         quant_control;          // 72 bytes
  lwenc_vp8_rc_s                    rate_control;           // 56 bytes
  unsigned char                     reserved[16];           // pad to 512 bytes 

} lwenc_vp8_drv_pic_setup_s;                                // 512 bytes
...
// data in this buffer will be needed for future pictures encoded by both CEA and LWENC.
...
// of each (sub)partition. Note also that since LWENC does not lwrrently support sub-8x8
...
} lwenc_h264_coloc_entry_s;

typedef struct                                              // 64 byte aligned block
{
  lwenc_h264_coloc_entry_s    coldata[16];                  // col data for 1 macroblock = array of 16 entries
} lwenc_h264_coloc_mb_s;
...
// subject to change. The data is produced and consumed by LWENC only and should never be
...
// 
// doc/lwenc/arch/hist_layout.vsd
// 
// 2.4. Reference and Input Picture Buffer
// ---------------------------------------
// 
// Reference Picture Buffer contains reference pictures produced by LWENC during 
...
// 
// Pictures are by default stored in LW24 field or LW12 frame format. LWENC will 
...
// ---------------------------
// 
// CEA (a LWCA based integer motion search kernel) can provide hints to LWENC.
// These hints can either be taken as explicit vectors to evaluate or as guides
// for LWENC's built-in integer search.
// In this mode of operation, CEA and LWENC typically work on the same frame, 
// with CEA slightly leading (by a few macroblock rows). LWENC would then read
...
// already completed and written out into this buffer, so that LWENC can read
...
// It is written by CEA and read by LWENC. 
// 
...
// Please refer to the document named "lwenc_cea_mvinfo_format.doc" for more information. 
// This document can be found in the lwenc HW development tree under doc/lwenc/arch
...
typedef struct 
{
  ...
  int              part_type   : 2;                         // 29:28 0=16x16 1=16x8 2=8x16 3=8x8 (blocks in partition must be conselwtive)
...
typedef struct 
{
  ...
  short            last_of_lw     : 1;                      // 13:13 last mv of current LW
  short            last_of_sb     : 1;                      // 14:14 last mv of current SB
  short            reserved0      : 1;                      

  short            mvx            : 14;                     // 13:0  mvx, in S12.2 format
  short            lw_size        : 2;                      // 15:14 LW size: 0: 8x8, 1: 16x16, 2:32x32, 3:64x64
...
typedef struct
{
  ...
  unsigned short pic_type;                                  // copied from pic_type in lwenc_h264_pic_control_s   
...
typedef struct
{
  ...
  unsigned char  reserved[4];
} lwenc_slice_stat_s;                                       // 16 bytes per status
...
typedef struct
{
  ...
  unsigned int   lwmulative_intra_cost;                     // sum of all intra SA(T)Ds
  unsigned int   lwmulative_inter_cost;                     // sum of all inter SA(T)Ds
  unsigned int   lwmulative_ipcm_cost;                      // sum of all inter SA(T)Ds for the MBs that caused IPCM rewind
  unsigned int   lwmulative_skip_cost;                      // sum of all P and B skip MB SA(T)Ds
...
} lwenc_mpec_stat_s;                                       // 64 bytes per status
...
} lwenc_stats_fifo_s;                                       // 8 bytes per stat

typedef struct 
{
  lwenc_pic_stat_s   pic_stat;                              // 64Bytes
  lwenc_slice_stat_s slice_stat[((4096/16)*(4096/16)) + 3]; // To account for 1mb/slice and 4kx4k, plus 3 for alignment
  lwenc_mpec_stat_s  mpec_stat[((4096/16)*(4096/16)) + 3];  // To account for stat/1mb and 4kx4k, plus 3 for alignment
  lwenc_aq_stat_s    aq_stats[((4096/16)*(4096/16)) + 3];   // AQ stats from PDMA
  lwenc_stats_fifo_s stats_fifo[((4096/16)*(4096/16)) + 3];
} lwenc_stat_data_s;
...
} lwenc_h265_pic_stat_s;                                    // 128 bytes
...
}lwenc_h265_slice_stat_s;                                   // 16 bytes per status
...
typedef struct
{
  ...
  unsigned int   intra_lw32x32_count;                       // number of intra 32x32 LWs.    
  unsigned int   inter_lw32x32_count;                       // number of inter 32x32 LWs.
  unsigned int   skip_lw32x32_count;                        // number of skip 32x32 LWs.
  unsigned int   ipcm_lw32x32_count;                        // number of ipcm 32x32 LWs.
  unsigned int   intra_lw16x16_count;                       // number of intra 16x16 LWs.    
  unsigned int   inter_lw16x16_count;                       // number of inter 16x16 LWs.
  unsigned int   skip_lw16x16_count;                        // number of skip 16x16 LWs.
  unsigned int   ipcm_lw16x16_count;                        // number of ipcm 16x16 LWs.
  unsigned int   intra_lw8x8_count;                         // number of intra 8x8 LWs.    
  unsigned int   inter_lw8x8_count;                         // number of inter 8x8 LWs.
  unsigned int   skip_lw8x8_count;                          // number of skip 8x8 LWs.
  unsigned int   ipcm_lw8x8_count;                          // number of ipcm 8x8 LWs.

  unsigned int   total_emulated_bit_count;                  // total number of emulated bits in total_bit_count  
  unsigned char  reserved[20];
} lwenc_h265_mpec_stat_s;                                   // 128 bytes per status
...
typedef struct
{
  ...
  unsigned int   res_cost                           : 24 ;  // satd cost of residual error
  unsigned int   flags                              : 8  ;  // flags 
} lwenc_h265_stats_fifo_s;                                  // 8 bytes per stat

typedef struct 
{
  lwenc_h265_pic_stat_s   pic_stat;                              // 64Bytes
  lwenc_h265_slice_stat_s slice_stat[((4096/16)*(4096/16)) + 3]; // To account for 1mb/slice and 4kx4k, plus 3 for alignment. support upto 65535 slices
  lwenc_h265_mpec_stat_s  mpec_stat[((4096/16)*(4096/16)) + 3];  // To account for stat/1mb and 4kx4k, plus 3 for alignment. support upto 65535 slices
  lwenc_h265_aq_stat_s    aq_stats [((4096/16)*(4096/16)) + 3];  // AQ stats from pdma. support upto 65535 slices
  lwenc_h265_stats_fifo_s stats_fifo[((4096/16)*(4096/16)) + 3];
} lwenc_h265_stat_data_s;
...
typedef struct
{
    ...
    unsigned int   pic_type                           : 14;   // copied from pic_type in lwenc_h264_pic_control_s   
    ...
    unsigned int   bitstream_start_pos;                       // byte_offset where mpec will start writing slice header
    unsigned int   intra_lw64x64_count;                       // number of intra 64x64 LWs.    
    unsigned int   inter_lw64x64_count;                       // number of inter 64x64 LWs.
    unsigned int   intra_lw32x32_count;                       // number of intra 32x32 LWs.    
    unsigned int   inter_lw32x32_count;                       // number of inter 32x32 LWs.
    unsigned int   intra_lw16x16_count;                       // number of intra 16x16 LWs.    
    unsigned int   inter_lw16x16_count;                       // number of inter 16x16 LWs.
...
} lwenc_av1_pic_stat_s;                                   // 128 bytes
...
}lwenc_av1_tile_stat_s;                                       // 16 bytes per status
...
} lwenc_av1_mpec_stat_s;                            // 128 bytes per status
...
typedef struct
{
  ...
  unsigned int   res_cost                           : 24 ;  // satd cost of residual error
  unsigned int   flags                              : 8  ;  // flags 
} lwenc_av1_stats_fifo_s;                                   // 8 bytes per stat

typedef struct
{
    lwenc_av1_pic_stat_s   pic_stat;                                 // 64Bytes
    lwenc_av1_tile_stat_s  tile_stat[128];                           // maximum tile number is 128 per AV1 spec
    lwenc_av1_mpec_stat_s  mpec_stat[((8192 / 64)*(8192 / 64)) ];    // To account for stat/1SB and 8kx8k, plus 3 for alignment?
    lwenc_av1_aq_stat_s    aq_stats[((8192 / 64)*(8192 / 64)) ];     // AQ stats from pdma 
    lwenc_av1_stats_fifo_s stats_fifo[((8192 / 64)*(8192 / 64)) ];
} lwenc_av1_stat_data_s;
...
typedef struct
{
  ...
  unsigned int   lwmulative_intra_cost;                     // sum of all intra SA(T)Ds
  unsigned int   lwmulative_inter_cost;                     // sum of all inter SA(T)Ds
  unsigned int   lwmulative_skip_cost;                      // sum of all P skip MB SA(T)Ds
  unsigned short intra_mb_count;                            // number of intra MBs.    
  unsigned short inter_mb_count;                            // number of inter MBs.
  unsigned short skip_mb_count;                             // number of skip MBs.
  unsigned short reserved0;                                 // align to a word boundary
} lwenc_vp8_segment_stat_s;                                 // 44 Bytes
...
typedef struct
{
  ...
  lwenc_vp8_segment_stat_s segment_stat[4];
  unsigned int   total_intra_cost;                          // sum of all intra best mode's SATDs, each mb has one intra SATD.
  unsigned int   total_inter_cost;                          // sum of all inter best mode's SATDs, each mb has one inter SATD.
  unsigned int reserved1[3];
} lwenc_vp8_stat_data_s;                                    // 256 Bytes
...
typedef struct
{
    ...
    unsigned int    reserved                 : 2;
}lwenc_av1_loop_filter_cfg_s;  //16 bytes
...
}lwenc_av1_lr_filter_cfg_s;  // 4 bytes
...
typedef struct
{
    ...
    unsigned short   sfc_pitch_chroma;                          // pitch of chroma plane
    unsigned short   sfc_trans_mode;                            // least 3 significient bits are used to stand for 8 modes, normal, xflip, yflip etc.
    unsigned short   reserved2;                                 // pad to LwS32
    ...
    unsigned int   lw21_enable      : 1;                        // (deprecated) the surface format is yuv or yvu
    unsigned int   input_bl_mode    : 2;                        // the input block linear mode: 0~gpu bl; 1~cheetah bl; 2~naive bl this is only for cmod and cmod/plugin. TBL only for Orin. 
    unsigned int   src_in_lsb       : 1;                        // pixel is LSB aligned in 16bit container, only valid for high-bitdepth
    unsigned int   reserved         : 18;                       // pad to LwU32
} lwenc_av1_surface_cfg_s;                                      // 32 bytes

typedef struct
{
    //sequence header 
    unsigned int   min_lw_size                         : 2;            //0-8x8, 1-16x16, 2-32x32, 3-64x64. Orin AV1 supports 1~3
    unsigned int   max_lw_size                         : 2;            //0-8x8, 1-16x16, 2-32x32, 3-64x64  Orin AV1 supports 1~3
    ...
    unsigned int   input_bit_depth_minus8              : 4;            // Internal high-bitdepth support in HW
                                                                       // the field is to specify the bit-depth of source yuv. the decoded yuv bitdepth is specified lwenc_av1_seq_hdr_data_s::bit_depth_minus_8; HW will do the bitdepth colwersion from source yuv-> recon yuv
                                                                       //
    ...
    unsigned int   reserved0                           : 2;            // reserved bits
} lwenc_av1_seq_hdr_data_s;                                            // 4 bytes
...
typedef struct
{
    ...
    lwenc_av1_loop_filter_cfg_s       lf_cfg;                    // loop filter configuration
    lwenc_av1_cdef_cfg_s              cdef_cfg;                  // CDEF configuration
    lwenc_av1_quant_cfg_s             quant_cfg;                 // quantizer configuration
    lwenc_av1_lr_filter_cfg_s         lrf_cfg;                   // loop restoration filter configuration
} lwenc_av1_frame_hdr_data_s;                                    // 64 bytes
...
typedef struct
{
    ...
    unsigned char    minQP[3];                                  // min QP for 0:P picture, ,1:B picture 2; I picture 
    unsigned char    maxQP[3];                                  // max QP for 0:P picture, ,1:B picture 2; I picture 
    char    maxQPD;                                    // max QP delta between two conselwtive QP updates
    char    baseQPD;                                   // initial QP delta between two conselwtive QP updates  
...
typedef struct 
{
  ...
  char  temp_dist_l0[4];                                    // temporal distance of each ref pic in list 0 DiffPicOrderCnt( lwrrPicOrField, pic0) )
  char  temp_dist_l1[3];                                    // temporal distance of each ref pic in list 1 DiffPicOrderCnt( lwrrPicOrField, pic1) )
  ...
  unsigned int   lw_early_terminatiom_enable  : 1;          // 0: disable LW early termination, 1: enable LW early termination. Not supported.
  ...
  unsigned int   tile_control_offset;                       // offset from start of top level "lwenc_av1_drv_pic_setup_s" structure
  unsigned int   hist_buf_size;                             // size in bytes of the buffer allocated for history
  unsigned int   bitstream_buf_size;                        // size in bytes of the buffer allocated for bitstream slice/mb data
  unsigned int   bitstream_start_pos;                       //start position in bitstream buffer where data should be written (byte offset)
  unsigned int   pic_type                      :  2;        // 0 = P, 1 = B, 2 = I, 3 = IDR
  ...
  unsigned int   mpec_threshold;                            // collect mpec stats after threshold mbs
                                                                // when mpec_stat_on is enabled and mpec_threshold is not equal to picWidthInMbs,
  unsigned int   tile_stat_offset;                          // offset from start of top level "lwenc_av1_stat_data_s" structure
                                                                // rc mode 0 will be used to collect stats
  unsigned int   mpec_stat_offset;                          // offset from start of top level "lwenc_stat_data_s" structure
                                                                // to start of array of "lwenc_mpec_stat_s" structure
  unsigned int   aq_stat_offset;                            // to start of array of "lwenc_av1_aq_stat_s" structure
...
} lwenc_av1_pic_control_s;                                  // 112 bytes

typedef struct
{
    unsigned int lw8x8_part_2Nx2N_enable       : 1;            // enable prediction for Inter LW8x8 2Nx2N                                                  
    unsigned int lw16x16_part_2Nx2N_enable     : 1;            // enable prediction for Inter LW16x16 2Nx2N
    unsigned int lw16x16_part_2NxN_enable      : 1;            // enable prediction for Inter LW16x16 2NxN
    unsigned int lw16x16_part_Nx2N_enable      : 1;            // enable prediction for Inter LW16x16 Nx2N                                                   
    unsigned int lw32x32_part_2Nx2N_enable     : 1;            // enable prediction for Inter LW32x32 2Nx2N
    unsigned int lw32x32_part_2NxN_enable      : 1;            // enable prediction for Inter LW32x32 2NxN
    unsigned int lw32x32_part_Nx2N_enable      : 1;            // enable prediction for Inter LW32x32 Nx2N                                                 
    unsigned int lw64x64_part_2Nx2N_enable     : 1;            // enable prediction for Inter LW64x64 2Nx2N
    unsigned int lw64x64_part_2NxN_enable      : 1;            // enable prediction for Inter LW64x64 2NxN
    unsigned int lw64x64_part_Nx2N_enable      : 1;            // enable prediction for Inter LW64x64 Nx2N
    unsigned int tusize_4x4_enable             : 1;            //enable 4x4 transform
    ...
    unsigned int multiply_bias_with_lambda     : 1;            // setting this to 1 will result in bias being multiplied by lambda
    unsigned int skip_evaluate_enable_lw8      : 1;            // 0: not check skip ssd cost; 1, evaluate skip ssd cost. Not supported
    unsigned int skip_evaluate_enable_lw16     : 1;            // 0: not check skip ssd cost; 1, evaluate skip ssd cost. Not supported
    unsigned int skip_evaluate_enable_lw32     : 1;            // 0: not check skip ssd cost; 1, evaluate skip ssd cost. Not supported
    unsigned int skip_evaluate_enable_lw64     : 1;            // 0: not check skip ssd cost; 1, evaluate skip ssd cost. Not supported
    unsigned int inter_rdo_cnt_8x8             : 3;            // #RD cost evaluation for inter top N SATD winner partitions of LW8x8. support 0~4 
    unsigned int inter_rdo_cnt_16x16           : 3;            // #RD cost evaluation for inter top N SATD winner partitions of LW16x16. support 0~4 
    unsigned int inter_rdo_cnt_32x32           : 3;            // #RD cost evaluation for inter top N SATD winner partitions of LW32x32. support 0~4 

    unsigned int inter_rdo_cnt_64x64           : 3;            // #RD cost evaluation for inter top N SATD winner partitions of Lw64x64. support 0~4 
...
} lwenc_av1_md_control_s;                                     // 160 bytes

typedef lwenc_h264_me_control_s lwenc_me_control_s;

typedef struct
{
    // 256 aligned
    unsigned int                magic;                         // magic number, see text above   //  4 bytes
    lwenc_av1_surface_cfg_s     refpic_cfg;                    // defines layout of reference pictures     // 32 bytes
    lwenc_av1_surface_cfg_s     input_cfg;                     // defines layout of input pictures     // 32 bytes
    lwenc_av1_surface_cfg_s     outputpic_cfg;                 // defines layout of reconstructed pictures  // 32 bytes

    lwenc_av1_seq_hdr_data_s    seqhdr_data;                   // 4 bytes
    lwenc_av1_frame_hdr_data_s  framehdr_data;                 // 64 bytes
    lwenc_av1_rc_s              rate_control;                  // Rate Control information   // 96 bytes
    lwenc_av1_pic_control_s     pic_control;                   // 112 bytes
    lwenc_av1_surface_cfg_s     half_scaled_outputpic_cfg;     // defines layout of 2x2 subsampled reconstructed picture // 32 bytes
    lwenc_av1_md_control_s      md_control;                    // 160 bytes
    lwenc_me_control_s          me_control;                    // 192 bytes
    unsigned int                gpTimer_timeout_val;           // GPTimer cycle count set from driver    
    unsigned char               reserved0[4];             
} lwenc_av1_drv_pic_setup_s; //768 bytes   

typedef lwenc_av1_drv_pic_setup_s lwenc_vp9_drv_pic_setup_s;  //TODO: Orin VP9 will share the same class as AV1. Define new one for Hopper VP9
...
typedef struct
{
    ...
    unsigned int   tile_idx                            : 8;    // tile index at picture level (potentially different from index in array of lwenc_av1_tile_control_s when doing strip encoding)
...
} lwenc_av1_tile_control_s;                                    // 32 bytes
...
typedef struct
{
    ...
    unsigned int   reserved1;                                  // padding to 16 bytes
} lwenc_av1_quant_control_s;                            // 16 bytes 
...
// 2. program the golden crc to LW_CLWENC_SAFETY_CRC_GOLDEN_PART_[X]_INTERFACE_NAME, and config the SAFETY_CRC_CFG register (with values from dbg_crc_init_enable/comp_enable/addr_comp/mem_wr_dis), set SAFETY_CRC_CTRL_EN = 1 (TZ)
// 3. kick off HW and trigger HW to compare width golden by writing SAFETY_CRC_CFG_COMP_EN when finish processing the frame; compare result could read from LW_CLWENC_SAFETY_CRC_STATUS_PART_[X] 
// 4. raw HW CRC values could be read from LW_CLWENC_SAFETY_CRC_HW_PART_[X]_INTERFACE_NAME
typedef struct
{
    unsigned int dbg_crc_enable     : 1;     //Global enable flag for enable/disable interface crc callwlation in LWENC HW
    ...
    unsigned int dbg_crc_addr_comp  : 1;     //Enable crc callwlation for fbif address
...
} lwenc_debug_status_s; //2304 bytes
...
// buffer will be shared in all exelwtes of a particular stream.
// 
// This buffer is used only if picture level rate control is enabled ((rc_mode & 3) != 0)

// Macro to initialize Bavg in VBR mode
#define LWENC_VBR_INIT_BAVG(framerate, ravg)                (0)                             // zero-centered vbr fullness
// Macro to initialize Whist and Wpbi
#define LWENC_RC_INIT_COMPLEXITY(bpp_at_qs1)                ((int)((bpp_at_qs1)*16))        // use this macro to decouple from implementation-dependent precision
...
typedef struct
{
  ...
  unsigned char  avgBf;                                     // 3.5 fixed point, average number of conselwtive B-frames
  unsigned char  reserved;
  unsigned int   printf_offset;                             // Using for ucode print
  unsigned char  reserved2[4];
  
} lwenc_persistent_state_s;                                 // 256Bytes
...
// the stats from the same buffer for QP callwlation. The layout of buffer is
...
typedef struct
{
  ...
  unsigned int   mbrow_lwmulative_intra_cost;               // sum of all intra SA(T)Ds
  unsigned int   mbrow_lwmulative_inter_cost;               // sum of all inter SA(T)Ds
...
typedef lwenc_generic_map_s lwenc_qp_map_s;
typedef lwenc_generic_map_s lwenc_lambda_map_s;
...
// Lwrrently used only for MSENC 2.0 for T114
...
// 
// The LWENC application method registers are accessed directly by the driver. 
...
// SetControlParams()
// ------------------
// This method sets the control parameters that are used in this exelwtion
...
//     submodules in LWENC will be reset, the status will be informed in flowControl
...
//     mpec statistics will be dumped to lwenc_mpec_stat_s. 
//
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SET_CTRL_PARAM                    0x00000700  /* -W-4R */
// #define LW_C9B7_LWENC_SET_CTRL_PARAM_CODEC_TYPE                3:0  /* -W-UF */
// #define LW_C9B7_LWENC_SET_CTRL_PARAM_CODEC_TYPE_H264    0x00000003 
// #define LW_C9B7_LWENC_SET_CTRL_PARAM_FORCE_OUTPIC              8:8  /* -W-UF */
// #define LW_C9B7_LWENC_SET_CTRL_PARAM_FORCE_OUTCOL              9:9  /* -W-UF */
// #define LW_C9B7_LWENC_SET_CTRL_PARAM_ME_ONLY                 10:10  /* -W-UF */
// #define LW_C9B7_LWENC_SET_CTRL_PARAM_SLICE_STAT_ON           11:11  /* -W-UF */
// #define LW_C9B7_LWENC_SET_CTRL_PARAM_GPTIMER_ON              12:12  /* -W-UF */  
// #define LW_C9B7_LWENC_SET_CTRL_PARAM_MPEC_STAT_ON            15:15  /* -W-UF */
// #define LW_C9B7_LWENC_SET_CTRL_PARAM_DEBUG_MODE              16:16  /* -W-UF */  
// #define LW_C9B7_LWENC_SET_CTRL_PARAM_SUBFRAME_MODE           18:17  /* -W-UF */
// #define LW_C9B7_LWENC_SET_CTRL_PARAM_RC_MODE                 31:24  /* -W-UF */  
// 
// 
// SetPictureIndex()
// -----------------
// 
// This method sets the picture index to LWENC. For the same picture, CEA and LWENC 
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SET_PICTURE_INDEX                 0x00000704  /* -W-4R */
// #define LW_C9B7_LWENC_SET_PICTURE_INDEX_INDEX                 31:0  /* -W-UF */
// 
// 
// SetInRcData()
// ------------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_RC_DATA                      0x0000070c  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_RC_DATA_OFFSET                     31:0  /* -W-UF */
// 
// SetInDrvPicSetup()
// ------------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_DRV_PIC_SETUP               0x00000710  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_DRV_PIC_SETUP_OFFSET              31:0  /* -W-UF */
// 
// 
// SetInCEAHintsData()
// -------------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_CEA_HINTS_DATA              0x00000714  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_CEA_HINTS_DATA_OFFSET             31:0  /* -W-UF */
// 
// 
// SetOutEncStatus()
// -----------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETOUT_ENC_STATUS                 0x00000718  /* -W-4R */
// #define LW_C9B7_LWENC_SETOUT_ENC_STATUS_OFFSET                31:0  /* -W-UF */
// 
// 
// SetOutBitstream()
// -----------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETOUT_BITSTREAM                  0x0000071c  /* -W-4R */
// #define LW_C9B7_LWENC_SETOUT_BITSTREAM_OFFSET                 31:0  /* -W-UF */
// 
// 
// SetIOHistory()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIO_HISTORY                     0x00000720  /* -W-4R */
// #define LW_C9B7_LWENC_SETIO_HISTORY_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetIORCProcess()
// ----------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIO_RC_PROCESS                 0x00000724  /* -W-4R */
// #define LW_C9B7_LWENC_SETIO_PROCESS_OFFSET                   31:0  /* -W-UF */
// 
// 
// SetInColocData()
// ----------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_COLOC_DATA                  0x00000728  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_COLOC_DATA_OFFSET                 31:0  /* -W-UF */
// 
// 
// SetOutColocData()
// -----------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETOUT_COLOC_DATA                 0x0000072c  /* -W-4R */
// #define LW_C9B7_LWENC_SETOUT_COLOC_DATA_OFFSET                31:0  /* -W-UF */
// 
// 
// SetOutRefPic()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETOUT_REFPIC                     0x00000730  /* -W-4R */
// #define LW_C9B7_LWENC_SETOUT_REFPIC_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetInLwrPic()
// -------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_LWRPIC                      0x00000734  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_LWRPIC_OFFSET                     31:0  /* -W-UF */
// 
// 
// SetInMEPredData()
// -----------------
// This method sets the offset for motion estimation predictor input data buffer. 
// This is the buffer containing vector hints produced by an earlier execute of LWENC.
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_MEPRED_DATA                 0x00000738  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_MEPRED_DATA_OFFSET                31:0  /* -W-UF */
// 
// 
// SetOutMEPredData()
// ------------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETOUT_MEPRED_DATA                0x0000073c  /* -W-4R */
// #define LW_C9B7_LWENC_SETOUT_MEPRED_DATA_OFFSET               31:0  /* -W-UF */
// 
// 
// SetInRefPic0()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC0                     0x00000400  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC0_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetInRefPic1()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC1                     0x00000404  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC1_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetInRefPic2()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC2                     0x00000408  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC2_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetInRefPic3()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC3                     0x0000040c  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC3_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetInRefPic4()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC4                     0x00000410  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC4_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetInRefPic5()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC5                     0x00000414  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC5_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetInRefPic6()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC6                     0x00000418  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC6_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetInRefPic7()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC7                     0x0000041c  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC7_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetInRefPic8()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC8                     0x00000420  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC8_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetInRefPic9()
// --------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC9                     0x00000424  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC9_OFFSET                    31:0  /* -W-UF */
// 
// 
// SetInRefPic10()
// ---------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC10                    0x00000428  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC10_OFFSET                   31:0  /* -W-UF */
// 
// 
// SetInRefPic11()
// ---------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC11                    0x0000042c  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC11_OFFSET                   31:0  /* -W-UF */
// 
// 
// SetInRefPic12()
// ---------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC12                    0x00000430  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC12_OFFSET                   31:0  /* -W-UF */
// 
// 
// SetInRefPic13()
// ---------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC13                    0x00000434  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC13_OFFSET                   31:0  /* -W-UF */
// 
// 
// SetInRefPic14()
// ---------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC14                    0x00000438  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC14_OFFSET                   31:0  /* -W-UF */
// 
// 
// SetInRefPic15()
// ---------------
...
// `-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-'
// 
// #define LW_C9B7_LWENC_SETIN_REFPIC15                    0x0000043c  /* -W-4R */
// #define LW_C9B7_LWENC_SETIN_REFPIC15_OFFSET                   31:0  /* -W-UF */
// 
// 
// 
// 
...
// Errors
#define LwC9B7EncErrorH264ApptimerExpired                   0x30000001
#define LwC9B7EncErrorH264IlwalidInput                      0x30000002
#define LwC9B7EncErrorH264HwerrInterrupt                    0x30000003
#define LwC9B7EncErrorH264BadMagic                          0x30000004
