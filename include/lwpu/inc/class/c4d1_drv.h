...
// CLASS LW_C4D1_LWJPG
//
// LWJPG is the combination of JPEG decoder and encoder, it will support baseline sequential profile.
...
#ifdef LW_LWJPG_SUPPORT_ENC
typedef struct
{
    unsigned int    chroma_format       :  2;   // 0: 444; 1: 422H; 2:422V; 3:420
    unsigned int    mlw_width           : 12;   // picture width  in mlw
    unsigned int    mlw_height          : 12;   // picture height in mlw
...
typedef struct 
{
    unsigned int mlw_width      : 12;
    unsigned int rsvd0          :  4;
    unsigned int mlw_height     : 12;
...
}lwjpg_drv_pic_param_s;

typedef struct
{
    unsigned int bytes_offset;      //bytes consumed by HW
    unsigned int mlw_x;             //decoded mlw_x 
    unsigned int mlw_y;             //decoded mlw_y
    unsigned int cycle_count;       //total cycles taked for execute. read from LW_CLWJPG_PDMA_CYCLE register
...
typedef struct
{
    ...
    unsigned int mlw_x;             //encoded mlw_x 
    unsigned int mlw_y;             //encoded mlw_y
    unsigned int cycle_count;       //total cycles taked for execute. read from LW_CLWJPG_PDMA_CYCLE register
    unsigned int error_status;      //report error if any
    unsigned char reserved[12];  
}lwjpg_enc_status;
...
{
    int     jpeg_core_num;
    lwjpg_drv_pic_param_s   params[32];
}lwjpg_drv_batch_setup_s;
