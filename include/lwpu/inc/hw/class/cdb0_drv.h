...
//LWDEC5.0 low latency decoding (partial stream kickoff without context switch), method will reuse HevcSetSliceInfoBufferOffset.
typedef struct _lwdec_substream_entry_s {
...
} lwdec_substream_entry_s;/*low latency without context switch substream entry map,12bytes*/
...
#define HEVC_DBLK_TOP_BUF_SIZE(w)           LWDEC_ALIGN( (ALIGN_UP(w,16)/16 + 2) * HEVC_DBLK_TOP_SIZE_IN_SB16) // 8K: 1285*256

#define HEVC_DBLK_LEFT_SIZE_IN_SB16         ALIGN_UP(506, 128) // ctb16 + 444
#define HEVC_DBLK_LEFT_BUF_SIZE(h)          LWDEC_ALIGN( (ALIGN_UP(h,16)/16 + 2) * HEVC_DBLK_LEFT_SIZE_IN_SB16) // 8K: 1028*256

#define HEVC_SAO_LEFT_SIZE_IN_SB16          ALIGN_UP(713, 128) // ctb16 + 444
#define HEVC_SAO_LEFT_BUF_SIZE(h)           LWDEC_ALIGN( (ALIGN_UP(h,16)/16 + 2) * HEVC_SAO_LEFT_SIZE_IN_SB16) // 8K: 1542*256

// VP9 Filter FG buffer
#define VP9_DBLK_TOP_SIZE_IN_SB64           ALIGN_UP(2000, 128) // 420
#define VP9_DBLK_TOP_BUF_SIZE(w)            LWDEC_ALIGN( (ALIGN_UP(w,64)/64 + 2) * VP9_DBLK_TOP_SIZE_IN_SB64) // 8K: 1040*256

#define VP9_DBLK_LEFT_SIZE_IN_SB64          ALIGN_UP(1600, 128) // 420
#define VP9_DBLK_LEFT_BUF_SIZE(h)           LWDEC_ALIGN( (ALIGN_UP(h,64)/64 + 2) * VP9_DBLK_LEFT_SIZE_IN_SB64) // 8K: 845*256

// VP9 Hint Dump Buffer
#define VP9_HINT_DUMP_SIZE_IN_SB64          ((64*64)/(4*4)*8)           // 8 bytes per LW, 256 LWs(2048 bytes) per SB64
#define VP9_HINT_DUMP_SIZE(w, h)            LWDEC_ALIGN(VP9_HINT_DUMP_SIZE_IN_SB64*((w+63)/64)*((h+63)/64))
...
typedef struct _lwdec_hevc_main10_444_ext_s
{
    ...
    unsigned int diffLwChromaQpAdjustmentDepth:2;        //pps diff_lw_chroma_qp_adjustment_depth, 0...3
...
typedef struct _lwdec_hevc_pic_v3_s
{
...
typedef struct _lwdec_hevc_pic_s
{
    ...
    unsigned char gob_height                 : 3 ;   // Set GOB height, 0: GOB_2, 1: GOB_4, 2: GOB_8, 3: GOB_16, 4: GOB_32 (LWDEC3 onwards)
...
typedef struct _lwdec_dpb_entry_s {  // 16 bytes
  ...
  unsigned int index : 7;                 // DPB index, lwrrently 16 + 1, normally 5bit should be enough
  unsigned int col_idx : 5;               // index of associated colmv data buffer
  unsigned int state : 2;                 // 0: empty, 1: only top, 2: only btm, 3: top+btm
  unsigned int is_long_term : 1;          // 0: short-term, 1: long-term
  unsigned int not_existing : 1;          // set to 1 when marked as non-existing
  unsigned int is_field : 1;              // set to 1 when unpaired field or complementary field pair
  unsigned int top_field_marking : 4;     // 0: not used for reference, 1: used for short-term, 2: used for long-term
  unsigned int bottom_field_marking : 4;  // 0: not used for reference, 1: used for short-term, 2: used for long-term
  unsigned int output_memory_layout : 1;  // 0: LW12; 1:LW24, should match !frame_mbs_only_flag
  unsigned int reserved : 6;              //
  unsigned int FieldOrderCnt[2];          // 32bit, 0-top, 1-btm
  int FrameIdx;                           // short-term: FrameNum (16 bits), long-term: LongTermFrameIdx (4 bits)
...
typedef struct _lwdec_h264_pic_s
{
    ...
    unsigned char hint_dump_en; //enable COLOMV surface dump for all frames, which includes hints of "MV/REFIDX/QP/CBP/MBPART/MBTYPE", lwbug: 200212874
    ...
    unsigned int gob_height                 : 3 ;   // Set GOB height, 0: GOB_2, 1: GOB_4, 2: GOB_8, 3: GOB_16, 4: GOB_32 (LWDEC3 onwards)
...
typedef struct _ref_frame_struct_s{
    frame_info_t    info;
    scale_factors_reference_s sf;                              // scalefactor for reference frame and current frame size, driver can callwlate it
    unsigned char   sign_bias                    : 1;          // callwate based on frame_offset and current frame offset 
...
typedef struct _lwdec_av1_pic_s
{
    lwdec_pass2_otf_s encryption_params;
    ...
    unsigned int    lwr_frame_force_integer_mv   : 1;          // AMVR enable
    ...
    unsigned char   errorConcealOn               : 1;          // this field is not used, use ctrl_param.error_conceal_on to enable error concealment in ucode, 
                                                               // always set LW_CLWDEC_GIP_ERR_CONCEAL_CTRL_ON = 1 to enable error detect in hw
...
#define AV1_TILEINFO_BUF_SIZE_OLD           LWDEC_ALIGN(AV1_MAX_TILES * sizeof(AV1TileInfo_OLD))
#define AV1_TILEINFO_BUF_SIZE               LWDEC_ALIGN(AV1_MAX_TILES * sizeof(AV1TileInfo))

// AV1 TileStreamInfo buffer
#define AV1_TILESTREAMINFO_BUF_SIZE         LWDEC_ALIGN(AV1_MAX_TILES * sizeof(AV1TileStreamInfo))

// AV1 SubStreamEntry buffer
#define MAX_SUBSTREAM_ENTRY_SIZE            32
#define AV1_SUBSTREAM_ENTRY_BUF_SIZE        LWDEC_ALIGN(MAX_SUBSTREAM_ENTRY_SIZE * sizeof(lwdec_substream_entry_s))

// AV1 FilmGrain Parameter buffer
#define AV1_FGS_BUF_SIZE                    LWDEC_ALIGN(sizeof(AV1FilmGrainMemory))
...
#define AV1_GLOBAL_MOTION_BUF_SIZE          LWDEC_ALIGN(7*32)

// AV1 Intra Top buffer
#define AV1_INTRA_TOP_BUF_SIZE              LWDEC_ALIGN(8*8192)

// AV1 Histogram buffer
#define AV1_HISTOGRAM_BUF_SIZE              LWDEC_ALIGN(1024)

// AV1 Filter FG buffer
#define AV1_DBLK_TOP_SIZE_IN_SB64           ALIGN_UP(1920, 128)                
#define AV1_DBLK_TOP_BUF_SIZE(w)            LWDEC_ALIGN( (ALIGN_UP(w,64)/64 + 2) * AV1_DBLK_TOP_SIZE_IN_SB64)

#define AV1_DBLK_LEFT_SIZE_IN_SB64          ALIGN_UP(1536, 128)                
#define AV1_DBLK_LEFT_BUF_SIZE(h)           LWDEC_ALIGN( (ALIGN_UP(h,64)/64 + 2) * AV1_DBLK_LEFT_SIZE_IN_SB64)

#define AV1_CDEF_LEFT_SIZE_IN_SB64          ALIGN_UP(1792, 128)                
#define AV1_CDEF_LEFT_BUF_SIZE(h)           LWDEC_ALIGN( (ALIGN_UP(h,64)/64 + 2) * AV1_CDEF_LEFT_SIZE_IN_SB64)

#define AV1_HUS_LEFT_SIZE_IN_SB64           ALIGN_UP(12544, 128) 
#define AV1_ASIC_HUS_LEFT_BUFFER_SIZE(h)    LWDEC_ALIGN( (ALIGN_UP(h,64)/64 + 2) * AV1_HUS_LEFT_SIZE_IN_SB64)
...
#define AV1_LR_LEFT_BUF_SIZE(h)             LWDEC_ALIGN( (ALIGN_UP(h,64)/64 + 2) * AV1_LR_LEFT_SIZE_IN_SB64)

#define AV1_FGS_LEFT_SIZE_IN_SB64           ALIGN_UP(320, 128)                
#define AV1_FGS_LEFT_BUF_SIZE(h)            LWDEC_ALIGN( (ALIGN_UP(h,64)/64 + 2) * AV1_FGS_LEFT_SIZE_IN_SB64)

// AV1 Hint Dump Buffer
#define AV1_HINT_DUMP_SIZE_IN_SB64          ((64*64)/(4*4)*8)           // 8 bytes per LW, 256 LWs(2048 bytes) per SB64
#define AV1_HINT_DUMP_SIZE_IN_SB128         ((128*128)/(4*4)*8)         // 8 bytes per LW,1024 LWs(8192 bytes) per SB128
#define AV1_HINT_DUMP_SIZE(w, h)            LWDEC_ALIGN(AV1_HINT_DUMP_SIZE_IN_SB128*((w+127)/128)*((h+127)/128))  // always use SB128 for allocation


/*******************************************************************
                New  H264 
********************************************************************/
typedef struct _lwdec_new_h264_pic_s {  // 500 bytes

  // DRM, 64 bytes
  lwdec_pass2_otf_s encryption_params;  // 52 bytes
  lwdec_pass2_otf_ext_s ssm;            // 12 bytes

  // DPB, 256 bytes
  lwdec_dpb_entry_s dpb[16];  // 16 bytes x 16 of reference frames

  // Reference picture list (frame initialization), 96 bytes
  unsigned char ref_pic_list_p[32];     // P-SLICE, 6bit: btm_field(1b) + dpb_idx(5b), frame: 16, field: 32, L0, dpb_idx=0x1F means invalid
  unsigned char ref_pic_list_b[2][32];  // B-SLICE, 6bit: btm_field(1b) + dpb_idx(5b), frame: 16, field: 32, L0/L1, dpb_idx=0x1F means invalid

  // MVC, 32 bytes
  unsigned char inter_view_refidx_list[2][16];  // 5bit, DPB index (must also be marked as long-term), dpb_idx=0x1F means invalid

  // GLOBAL, 24 bytes
  unsigned int gptimer_timeout_value;      // Suggested setting: 3 * (cycles required for one frame)
  unsigned int stream_len;                 // bitstream length bytes
  unsigned int sw_skip_start_length : 14;  // bitstream start skip bytes
  unsigned int glb_reserved_1 : 18;        //

  unsigned int lwr_pic_idx : 5;                // current frame DPB index
  unsigned int lwr_col_idx : 5;                // current colmv buffer index
  unsigned int tileFormat : 2;                 // only support 1: KBL
  unsigned int gob_height : 3;                 // Set GOB height, 0: GOB_2, 1: GOB_4, 2: GOB_8, 3: GOB_16, 4: GOB_32
  unsigned int output_memory_layout : 1;       // 0: LW12; 1:LW24, HW support only LW12
  unsigned int histogram_en : 1;               // enable histogram info collection
  unsigned int hint_dump_en : 1;               // enable colmv surface dump for current frame
  unsigned int scaling_list_present_flag : 1;  // set to 1 when (seq_scaling_matrix_present_flag || pic_scaling_matrix_present_flag)
  unsigned int glb_reserved_2 : 13;            //

  unsigned int luma_stride : 10;  // unit of 64 pixels
  unsigned int glb_reserved_3 : 22;

  unsigned int chroma_stride : 10;  // unit of 64 pixels
  unsigned int glb_reserved_4 : 22;

  // SPS, 8 bytes
  unsigned int bit_depth_luma : 4;     // (8..10)
  unsigned int bit_depth_chroma : 4;   // (8..10)
  unsigned int PicWidthInMbs : 10;     // unit of 16 pixels, (3..512)
  unsigned int FrameHeightInMbs : 10;  // unit of 16 pixels, histogram disabled: (1..512), histogram enabled: (3..512)
  unsigned int chroma_format_idc : 2;  // (0..2), 0: 400, 1: 420, 2: 422
  unsigned int sps_reserved_1 : 2;     //

  unsigned int profile_idc : 8;                        // for error check, set to 0 if driver can not provide
  unsigned int level_idc : 8;                          // for error check, set to 0 if driver can not provide
  unsigned int log2_max_frame_num_minus4 : 4;          // (0..12)
  unsigned int log2_max_pic_order_cnt_lsb_minus4 : 4;  // (0..12)
  unsigned int pic_order_cnt_type : 2;                 // (0..2)
  unsigned int delta_pic_order_always_zero_flag : 1;   //
  unsigned int max_num_ref_frames : 5;                 // (0..16)

  // PPS, 8 bytes
  unsigned int num_ref_idx_l0_active_minus1 : 5;                  // frame: (0..15), field: (0..31)
  unsigned int num_ref_idx_l1_active_minus1 : 5;                  // frame: (0..15), field: (0..31)
  int chroma_qp_index_offset : 5;                                 // (-12..+12)
  int second_chroma_qp_index_offset : 5;                          // (-12..+12)
  int pic_init_qp_minus26 : 7;                                    // (-38..+25)
  unsigned int bottom_field_pic_order_in_frame_present_flag : 1;  //
  unsigned int deblocking_filter_control_present_flag : 1;        //
  unsigned int redundant_pic_cnt_present_flag : 1;                //
  unsigned int transform_8x8_mode_flag : 1;                       // 0: no 8x8, 1: may use 8x8
  unsigned int constrained_intra_pred_flag : 1;                   //

  unsigned int entropy_coding_mode_flag : 1;   // 0: CAVLC, 1: CABAC
  unsigned int weighted_pred_flag : 1;         // 0: default, 1: explicit
  unsigned int weighted_bipred_idc : 2;        // (0..2), 0: default, 1: explicit, 2: implicit
  unsigned int num_inter_view_refs_l0 : 5;     // for MVC, number of inter-view references for L0, must be 0 when not MVC
  unsigned int num_inter_view_refs_l1 : 5;     // for MVC, number of inter-view references for L1, must be 0 when not MVC
  unsigned int direct_8x8_inference_flag : 1;  // from SPS, for B_Skip, B_Direct_16x16 and B_Direct_8x8
  unsigned int frame_mbs_only_flag : 1;        // from SPS, 0: can be either field or frame; 1: always frame
  unsigned int colmv_surface_stride : 10;      // unit of 256, should be set to NEW_H264_COLMV_STRIDE/256
  unsigned int colmv_16mv : 1;                 // for HW colmv surface save, 0: 4MV, 1: 16MV, should be set to !(direct_8x8_inference_flag && frame_mbs_only_flag)
  unsigned int pps_reserved_1 : 5;             //

  // SLICE, 12 bytes
  unsigned int frame_num : 16;         // slice header
  unsigned int field_pic_flag : 1;     // slice header
  unsigned int bottom_field_flag : 1;  // slice header
  unsigned int second_field : 1;       // second field of complementary reference field
  unsigned int ref_pic_flag : 1;       // NAL, reference picture (nal_ref_idc != 0)
  unsigned int intra_pic_flag : 1;     // indicate current decoding picture has only I-slices, set to 0 should work under all cases, when set to 1 will benefit HW perf/bandwidth
  unsigned int slice_reserved_1 : 11;  //

  int lwr_field_order_cnt[2];  // slice header, signed 32bit, 0-TopFieldOrderCnt, 1-BottomFieldOrderCnt

} lwdec_new_h264_pic_s;

// NEW_H264 scaling list structure
typedef struct _lwdec_new_h264_scaling_list_s {  // 224 bytes

  unsigned char WeightScale[6][4][4];     // 8bit, [intra/inter][y/cb/cr], for 4x4, in raster scan order
  unsigned char WeightScale8x8[2][8][8];  // 8bit, [intra/inter][y], for 8x8, in raster scan order

} lwdec_new_h264_scaling_list_s;

// NEW_H264 colmv unit structure
typedef struct _lwdec_new_h264_colmv_unit_s {  // 8 bytes, 4MV: 32 bytes/MB, 16MV: 128 bytes/MB

  unsigned int mvx : 16;  // PU
  unsigned int mvy : 16;  // PU

  unsigned int mb_type : 2;         // MB, 0: skip, 1: P, 2: B, 3: I
  unsigned int mb_part : 2;         // MB, 0: 16x16, 1: 16x8, 2: 8x16, 3: 8x8
  unsigned int sub_mb_type : 2;     // SUB_MB, 0: 8x8, 1: 8x4, 2: 4x8, 3: 4x4
  unsigned int mb_field : 1;        // PIC, 0: frame, 1: field
  unsigned int col_zero : 1;        // PU
  unsigned int qp : 7;              // MB, (0..64)
  unsigned int ref_pic : 5;         // MB/SUB_MB, dpb_idx(4b) + btm_field(1b)
  unsigned int cbp : 4;             // MB, CodedBlockPatternLuma, one bit for each luma 8x8
  unsigned int col_16mv : 1;        // col_save_mode indicator, 0: 4mv, 1: 16mv
  unsigned int col_reserved_1 : 7;  //

} lwdec_new_h264_colmv_unit_s;

#define NEW_H264_INTRA_TOP_SIZE_IN_MB (64)  // unit of byte
#define NEW_H264_INTRA_TOP_SIZE(mbw) LWDEC_ALIGN(NEW_H264_INTRA_TOP_SIZE_IN_MB *(mbw))

#define NEW_H264_DBLK_TOP_SIZE_IN_MB (32 * 4)  // unit of pxl
#define NEW_H264_DBLK_TOP_SIZE(mbw, is_8bit) LWDEC_ALIGN(NEW_H264_DBLK_TOP_SIZE_IN_MB *(is_8bit ? 1 : 2) * (mbw))

#define NEW_H264_SCALING_LIST_SIZE LWDEC_ALIGN(sizeof(lwdec_new_h264_scaling_list_s))

// NEW_H264 colmv surface size for one MB line
#define NEW_H264_COLMV_STRIDE(mbw, is_4mv) LWDEC_ALIGN((mbw) * sizeof(lwdec_new_h264_colmv_unit_s) * ((is_4mv) ? 4 : 16))  // unit of byte
// NEW_H264 colmv surface size for one frame
#define NEW_H264_COLMV_SIZE(mbw, mbh, is_4mv) LWDEC_ALIGN(NEW_H264_COLMV_STRIDE((mbw), (is_4mv)) * ALIGN_UP((mbh), 2))

// golden crc struct dumped into surface
// for each part, if golden crc compare is enabled, one interface is selected to do crc callwlation in vmod.
// vmod's crc is compared with cmod's golden crc (4*32 bits), and compare reuslt is written into surface.
typedef struct
{
    // input
    unsigned int    dbg_crc_enable_partb    : 1;    // Eable flag for enable/disable interface crc callwlation in LWDEC HW's part b
    unsigned int    dbg_crc_enable_partc    : 1;    // Eable flag for enable/disable interface crc callwlation in LWDEC HW's part c
    unsigned int    dbg_crc_enable_partd    : 1;    // Eable flag for enable/disable interface crc callwlation in LWDEC HW's part d
    unsigned int    dbg_crc_enable_parte    : 1;    // Eable flag for enable/disable interface crc callwlation in LWDEC HW's part e
