...
#include <sys/stat.h>
#include <fcntl.h>
#include "test_lwsciipc_write_events.h"
#include "test_lwsciipc_write_log.h"
#include "test_lwsciipc_write_eventlib.h"
#include "test_lwsciipc_write_events_json.h"
...
{
    struct lwsciip_write_test_log_ctx *ptr_ctx = &g_el_handle.el_ctx;
...
void lwsciipc_eventlib_close(void)
{
    ...
    if ((g_el_handle.init_status & SHARD_MEMORY_REGISTER_DONE) != 0) {
        (void)eventlib_unregister_shmem(LWSCIIP_WRITE_TEST_EVENT_PROVIDER_NAME,
...
int32_t lwsciipc_eventlib_init(void)
{
    ...
    for (i = 0; i < LWSCIIP_WRITE_TEST_EVENT_BITMASK_SIZE; i++) {
        ptr_ctx->bitmask[i] = ~0U;
    }

    ptr_ctx->ctx.w2r_shm = eventlib_register_shmem(LWSCIIP_WRITE_TEST_EVENT_PROVIDER_NAME,
                                                   EVENT_LOG_SIZE,
                                                   (char *)test_lwsciipc_write_events_json,
                                                   test_lwsciipc_write_events_json_len);
    ...
    if (ret == 0) {
        ...
        fprintf(stderr, "eventlib_init() failed for write\n");
