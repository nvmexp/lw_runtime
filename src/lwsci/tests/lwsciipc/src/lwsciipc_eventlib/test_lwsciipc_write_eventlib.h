...
#ifndef __TEST_LWSCIIPC_WRITE_EVENTLIB_H__
#define __TEST_LWSCIIPC_WRITE_EVENTLIB_H__
...
#define LWSCIIPC_WRITE_TEST_DUMP_FILENAME "/tmp/dump_test_lwsciipc_write.bin"

typedef struct {
    struct lwsciip_write_test_log_ctx el_ctx;
...
#define LWSCIIPC_EVENTLIB_LOG(_type, ...) do { \
    if ((g_el_handle.init_status & EVENTLIB_INIT_DONE) != 0) { \
        if ((g_el_handle.el_ctx.ctx.priv != NULL) && \
            (lwsciip_write_test_##_type##_check(&g_el_handle.el_ctx) > 0)) { \
            lwsciip_write_test_##_type##_write(&g_el_handle.el_ctx, 0, __VA_ARGS__); } \
    } \
} while(false)
#else
