 ...
 *
 * LwSciIpc does not provide checksum for data to be transferred or check for
...
typedef struct LwSciIpcEndpointInfo LwSciIpcEndpointInfo;
 ...
 * closing channel), it must be called at most once and LwSciIpcGetEvent() must
 * NOT be called after LwSciIpcResetEndpoint() is called.
 *
