...
//!   - Calls the constructor of the Block base class with BlockType::RETURNSYNC.
//!
//! \implements{}
LWCOV_ALLOWLIST_LINE(LWCOV_AUTOSAR(A3_1_1), "Bug 2758535")
ReturnSync::ReturnSync(LwSciSyncModule const syncModule) noexcept :
LWCOV_ALLOWLIST_LINE(LWCOV_CERTC(EXP39_C), "Bug 200662929")
    Block(BlockType::RETURNSYNC),
    ...
    teardown(false)
{
    ...
    if (!dispatchThread.joinable()) {
        ...
        dispatchThread = std::thread(&ReturnSync::waitForFenceThreadFunc, this);
...
//! - Call Packet::fenceConsWait() to CPU wait for all fences in the payload.
//! - Updates the location of the packet to Packet::Location::Upstream
//!   by calling Packet::locationUpdate().
//! - Forwards the payload upstream by calling the dstRecvPayload()
//!   interface of the source block through the source
//!   SafeConnection.
//! - Proceeds with next iteration if packets available in queue.
//!
//! \return void, Triggers the following error events:
//! - Any error returned by Packet::fenceConsWait().
//!
//! \implements{}
void
ReturnSync::waitForFenceThreadFunc(void) noexcept
{
...
//! \implements{}
LWCOV_ALLOWLIST_BEGIN(LWCOV_CERTC(EXP39_C), "Bug 200662929")
ReturnSync::~ReturnSync(void) noexcept
{
...
//!
//! \implements{}
LwSciError ReturnSync::disconnect(void) noexcept
{
    LWCOV_ALLOWLIST_BEGIN(LWCOV_CERTC(EXP39_C), "Bug 200662929")
    disconnectSrc();
    disconnectDst();
...
//! - Discards the producer waiter requirements.
...
//!   and Waiters::doneSet().
//! - Sends the waiter info downstream by calling srcRecvSyncWaiter()
//!   interface of the destination block through the SafeConnection.
//!
//! \implements{}
void ReturnSync::srcRecvSyncWaiter(
    ...
    Waiters const& syncWaiter) noexcept
{
    ...
    // Discard the producer waiter info
    ...
    // Send the waiter info downstream
...
//! - Call dstRecvSyncSignal() interface of source connection to send
//!   the new object upstream.
//! - Call phaseConsSyncDoneSet() to advance setup phase.
void ReturnSync::dstRecvSyncSignal(
    ...
    Signals const& syncSignal) noexcept
{
    ...
    LwSciError err
        { tmpSignal.sizeInit(consumerCountGet(), elementCountGet()) };
    if (LwSciError_Success == err) {
    ...
    // Send the signal info upstream
    getSrc().dstRecvSyncSignal(tmpSignal);
...
//!   queued.
//! - Call Packet::fenceConsCopy() to save a copy of the consumer fences.
//! - Call Packet::fenceConsWait() to check wheter all fences are expired.
//!   If so call dstRecvPayload() interface of source block to send the
//!   payload upstream and return.
...
//!
//! \implements{}
void ReturnSync::dstRecvPayload(
    uint32_t const dstIndex,
    Packet const& consPayload) noexcept
{
    ...
    // Check if all fences all fences are expired, and if so return
    //   upstream immediately
    if (LwSciError_Success == pkt->fenceConsWait(waitContext, 0U)) {
