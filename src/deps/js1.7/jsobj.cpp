...
JSClass js_ObjectClass = {
    ...
    JS_EnumerateStub, JS_ResolveStub,   JS_ColwertStub,   JS_FinalizeStub,
...
obj_getCount(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
    ...
    // JSENUMERATE_INIT above returns jsval cast to jsid, it is necessary to
    // cast it back
    if (!JSVAL_IS_INT(jsval(num_properties))) {
        JS_ASSERT(0);
        *vp = JSVAL_ZERO;
        goto out;
    }
...
js_hash_object(const void *key, void *)
{
                    ...
                    jschar **sp)
{
    ...
    *sp = NULL;
    ...
    table = map->table;
    if (!table) {
        table = JS_NewHashTable(8, js_hash_object, NULL, JS_CompareHashKeys, NULL,
                                JS_CompareHashValues, NULL, NULL);
        if (!table) {
            JS_ReportOutOfMemory(cx);
            return NULL;
        }
    ...
    if (map->depth == 0) {
         ...
         * colwerted to strings), i.e., the OBJ_GET_PROPERTY() call is not
         * idempotent.
         */
        if (!he) {
...
{
    GC_MARK((JSContext *)arg, (JSObject *)const_cast<void *>(he->key), "sharp table entry");
...
js_GCMarkSharpMap(JSContext *cx, JSSharpObjectMap *map)
{
    ...
    /*
     * During relwrsive calls to MarkSharpObjects a non-native object or
     ...
     * MarkSharpObjects relwrsive ilwocations. So we must protect map->table
     * entries against GC.
     *
     * We can not simply use JSTempValueRooter to mark the obj argument of
     * MarkSharpObjects during relwrsion as we have to protect *all* entries
                ...
                jsval *rval)
{
    ...
    size_t nchars, idstrlength, gsoplength, vlength, vsharplength, lwrlen;
    const char *comma;
    ...
    for (i = 0, length = ida->length; i < length; i++) {
        ...
        /*
         * Colwert id to a jsval and then to a string.  Decide early whether we
        ...
        *rval = STRING_TO_JSVAL(idstr);         /* local root */
        ...
        for (j = 0; j < valcnt; j++) {
            /* Colwert val[j] to its canonical source form. */
...
#undef SAFE_ADD
            ...
            if (lwrlen > (size_t)-1 / sizeof(jschar))
                goto overflow;

            /* Allocate 1 + 1 at end for closing brace and terminating 0. */
            chars = (jschar *)
                realloc((ochars = chars), lwrlen * sizeof(jschar));
            if (!chars) {
...
JSObject *
js_CheckScopeChailwalidity(JSContext *cx, JSObject *scopeobj, const char *caller)
{
    JSClass *clasp;
    JSExtendedClass *xclasp;
    JSObject *inner;
...
obj_eval(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    JSScript *script;
    JSBool ok;
    JSEvalHook hook = cx->runtime->evalHook;
    ...
    scopeobj = js_CheckScopeChailwalidity(cx, scopeobj, js_eval_str);
    if (!scopeobj)
        return JS_FALSE;
     ...
     * ilwoked) between fp and its scripted caller, to help the compiler easily
    ...
    if (hook) {
        hook(cx, &script, cx->runtime->evalHookData);
    }

    if (ok)
        ok = js_Exelwte(cx, scopeobj, script, caller, JSFRAME_EVAL, rval);
...
static JSBool
obj_watch_handler(JSContext *cx, JSObject *obj, jsval id, jsval old, jsval *lwp,
                  void *closure)
{
    JSObject *callable;
    JSRuntime *rt;
    ...
    if (rt->findObjectPrincipals) {
        ...
        if (caller) {
             ...
             * the lwrrently exelwting script.
             */
    ...
    /* Avoid relwrsion on (obj, id) already being watched on cx. */
    key.obj = obj;
    key.id = id;
    ...
    argv[1] = old;
    argv[2] = *lwp;
    ok = js_InternalCall(cx, obj, OBJECT_TO_JSVAL(callable), 3, argv, lwp);
    ...
    if (prop) {
        if (OBJ_IS_NATIVE(pobj)) {
            ...
            if (sprop->attrs & JSPROP_GETTER)
                *rval = OBJECT_TO_JSVAL(reinterpret_cast<JSObject *>(sprop->getter));
        }
    ...
    if (prop) {
        if (OBJ_IS_NATIVE(pobj)) {
            ...
            if (sprop->attrs & JSPROP_SETTER)
                *rval = OBJECT_TO_JSVAL(reinterpret_cast<JSObject *>(sprop->setter));
        }
        OBJ_DROP_PROPERTY(cx, pobj, prop);
    }
    return JS_TRUE;
...
#if JS_HAS_OBJ_WATCHPOINT
const char js_watch_str[] = MKJSID("watch");
const char js_unwatch_str[] = MKJSID("unwatch");
#endif
const char js_hasOwnProperty_str[] = MKJSID("hasOwnProperty");
const char js_isPrototypeOf_str[] = MKJSID("isPrototypeOf");
const char js_propertyIsEnumerable_str[] = MKJSID("propertyIsEnumerable");
#if JS_HAS_GETTER_SETTER
const char js_defineGetter_str[] = MKJSID("__defineGetter__");
const char js_defineSetter_str[] = MKJSID("__defineSetter__");
const char js_lookupGetter_str[] = MKJSID("__lookupGetter__");
const char js_lookupSetter_str[] = MKJSID("__lookupSetter__");
#endif
...
js_GetClassId(JSContext *cx, JSClass *clasp, jsid *idp)
{
    ...
    key = static_cast<JSProtoKey>(JSCLASS_CACHED_PROTO_KEY(clasp));
    if (key != JSProto_Null) {
        *idp = INT_TO_JSID(key);
...
#define JS_PROTO(name,idname,code,init) extern JSObject *init(JSContext *, JSObject *);
JS_BEGIN_EXTERN_C
#include "jsproto.tbl"
JS_END_EXTERN_C
#undef JS_PROTO
...
#define JS_PROTO(name,idname,code,init) init,
#include "jsproto.tbl"
#undef JS_PROTO
};
                  ...
                  JSObject **objp)
{
    ...
    if (!js_StartResolving(cx, &rkey, JSRESFLAG_LOOKUP, &rentry))
        return JS_FALSE;
    if (!rentry) {
        /* Already caching key in obj -- suppress relwrsion. */
        *objp = NULL;
        return JS_TRUE;
    }
    generation = cx->resolvingTable->generation;
...
js_FindClassObject(JSContext *cx, JSObject *start, jsid id, jsval *vp)
{
    ...
    if (JSID_IS_INT(id)) {
        key = static_cast<JSProtoKey>(JSID_TO_INT(id));
...
#define CHECK_FOR_STRING_INDEX(cx, id)                                        \
    JS_BEGIN_MACRO                                                            \
        if (JSID_IS_ATOM(id)) {                                               \
                     ...
                     uintN attrs, uintN flags, intN shortid)
{
    ...
    if (!scope) {
        ...
        /*
         * Handle old bug that took empty string as zero index.  Also colwert
         * string indices to integers if appropriate.
         */
        CHECK_FOR_STRING_INDEX(cx, id);
                        ...
                        uintN flags, intN shortid, JSProperty **propp)
{
    ...
    sprop = js_AddScopeProperty(cx, scope, id, getter, setter,
                                SPROP_ILWALID_SLOT, attrs, flags, shortid);
    if (!sprop)
        goto bad;
    ...
    for (;;) {
        ...
        if (!sprop) {
            ...
            if (resolve != JS_ResolveStub) {
                /* Avoid relwrsion on (obj, id) already being resolved on cx. */
                key.obj = obj;
                key.id = id;
                 ...
                 * entry, in which case we bail to suppress runaway relwrsion.
                 */
                ...
                if (!entry) {
                    /* Already resolving id in obj -- suppress relwrsion. */
                    JS_UNLOCK_OBJ(cx, obj);
                    goto out;
                }
                generation = cx->resolvingTable->generation;
...
js_DefaultValue(JSContext *cx, JSObject *obj, JSType hint, jsval *vp)
{
    ...
    switch (hint) {
      case JSTYPE_STRING:
        ...
        if (!JSVAL_IS_PRIMITIVE(v)) {
            if (!OBJ_GET_CLASS(cx, obj)->colwert(cx, obj, hint, &v))
                return JS_FALSE;
        }
        break;
    ...
    if (!JSVAL_IS_PRIMITIVE(v)) {
        /* Avoid relwrsive death through js_DecompileValueGenerator. */
...
#ifdef NARCISSUS
static JSBool
GetLwrrentExelwtionContext(JSContext *cx, JSObject *obj, jsval *rval)
{
    JSObject *tmp;
    jsval xcval;
    ...
    if (!OBJ_GET_PROPERTY(cx, obj,
                          ATOM_TO_JSID(cx->runtime->atomState
                                       .ExelwtionContextAtom),
                          &xcval)) {
        return JS_FALSE;
    }
    if (JSVAL_IS_PRIMITIVE(xcval)) {
        JS_ReportError(cx, "invalid ExelwtionContext in global object");
        return JS_FALSE;
    }
    if (!OBJ_GET_PROPERTY(cx, JSVAL_TO_OBJECT(xcval),
                          ATOM_TO_JSID(cx->runtime->atomState.lwrrentAtom),
                          rval)) {
        return JS_FALSE;
    }
    return JS_TRUE;
...
js_Call(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (!clasp->call) {
        ...
        if (VALUE_IS_FUNCTION(cx, fval)) {
            if (!GetLwrrentExelwtionContext(cx, obj, &nargv[2]))
                return JS_FALSE;
             ...
             jsval *rval)
{
    ...
    if (!clasp->construct) {
        ...
        if (VALUE_IS_FUNCTION(cx, cval)) {
            if (!GetLwrrentExelwtionContext(cx, obj, &nargv[1]))
                return JS_FALSE;
            args = js_GetArgsObject(cx, cx->fp);
            if (!args)
                return JS_FALSE;
                     ...
                     JSObject **protop)
{
    ...
    if (VALUE_IS_FUNCTION(cx, v)) {
        ...
        if (!JSVAL_IS_PRIMITIVE(v)) {
             ...
             */
            cx->weakRoots.newborn[GCX_OBJECT] = static_cast<JSGCThing *>(JSVAL_TO_GCTHING(v));
        }
    }
 ...
 * to an exelwtion context in which the function declaration or expression
 * should be processed as if it were not precompiled, where the precompiled
 * function's scope chain does not match the exelwtion context's.  The cloned
 * function object carries its exelwtion-context scope in its parent slot; it
...
js_XDRObject(JSXDRState *xdr, JSObject **objp)
{
    ...
    if (xdr->mode == JSXDR_ENCODE) {
        ...
        if (classDef) {
            if (!JS_XDRRegisterClass(xdr, clasp, &classId))
                return JS_FALSE;
            protoKey = static_cast<JSProtoKey>(JSCLASS_CACHED_PROTO_KEY(clasp));
    ...
    if (xdr->mode == JSXDR_DECODE) {
        if (classDef) {
            ...
            protoKey = static_cast<JSProtoKey>(classDef >> 1);
