...
js_CompareAndSwap(jsword *w, jsword ov, jsword lw)
{
    __asm {
        mov eax, ov
        mov ecx, lw
...
1:"
    ...
    JS_ASSERT(ov != lw);
    return compare_and_swap(w, ov, lw);
#endif
}
...
js_CompareAndSwap(jsword *w, jsword ov, jsword lw)
{
    return !_check_lock((atomic_p)w, ov, lw);
}
 ...
 * subroutine of JS_EndRequest too.  The bulk of the work here ilwolves making
...
js_GetSlotThreadSafe(JSContext *cx, JSObject *obj, uint32 slot)
{
    ...
    JS_ASSERT(LWRRENT_THREAD_IS_ME(me));
 ...
 *
 * If another thread B tries to lock the object lwrrently locked by A, B is
...
js_LockRuntime(JSRuntime *rt)
{
    PR_Lock(rt->rtLock);
#ifdef DEBUG
    rt->rtLockOwner = js_LwrrentThreadId();
#endif
}
...
{
    return js_LwrrentThreadId() == rt->rtLockOwner;
}
...
js_IsScopeLocked(JSContext *cx, JSScope *scope)
{
     ...
     * a thin or fat lock to cope with shared (conlwrrent) ownership.
     */
    if (scope->ownercx) {
        JS_ASSERT(scope->ownercx == cx || scope->ownercx->thread == cx->thread);
        return JS_TRUE;
    }
    return js_LwrrentThreadId() ==
