...
typedef enum REOp {
    ...
    REOP_STAR          = 26, /* zero or more oclwrrences of kid */
    REOP_PLUS          = 27, /* one or more oclwrrences of kid */
    ...
    REOP_REPEAT        = 52, /* directs exelwtion of greedy quantifier */
    REOP_MINIMALREPEAT = 53, /* directs exelwtion of non-greedy quantifier */
...
typedef struct REGlobalData {
    JSContext *cx;
    JSRegExp *regexp;               /* the RE in exelwtion */
    ...
    size_t lwrsz;                   /* size of current stack entry */
 ...
 * 2. Let u be ch colwerted to upper case as if by calling
 *    String.prototype.toUpperCase on the one-character string ch.
 * 3. If u does not consist of a single character, return ch.
 * 4. Let lw be u's character.
 * 5. If ch's code point value is greater than or equal to decimal 128 and lw's
...
/*
 * Validates and colwerts hex ascii value.
 */
static JSBool
 ...
 * state->flags to discover whether an error oclwrred under findMax.
 */
static uintN
...
/*
 * Callwlate the total size of the bitmap required for a class expression.
 */
static JSBool
CallwlateBitmapSize(CompilerState *state, RENode *target, const jschar *src,
                    const jschar *end)
{
    uintN max = 0;
...
lexHex:
        ...
        if (state->flags & JSREG_FOLD) {
            ...
            if (static_cast<uintN>(c) > localMax)
                localMax = c;
        }
        if (inRange) {
            if (static_cast<uintN>(rangeStart) > localMax) {
                JS_ReportErrorNumber(state->context,
                                     js_GetErrorMessage, NULL,
                                     JSMSG_BAD_CLASS_RANGE);
                return JS_FALSE;
...
ParseTerm(CompilerState *state)
{
    ...
    doFlat:
        ...
        case '9':
            ...
            num = GetDecimalValue(c, (uintN)state->parenCount, FindParenCount, state);
...
doSimple:
        ...
        default:
            ...
            state->result->u.flat.chr = c;
            state->result->u.flat.length = 1;
            state->result->kid = const_cast<jschar *>(state->cp - 1);
            state->progLength += 3;
            break;
        }
        break;
    ...
    claim:
        /*
         * Call CallwlateBitmapSize now as we want any errors it finds
         * to be reported during the parse phase, not at exelwtion.
         */
        if (!CallwlateBitmapSize(state, state->result, termStart, state->cp++))
            return JS_FALSE;
        /*
    ...
    for (;;) {
        *pc++ = op;
        ...
        case REOP_ENDALT:
             ...
             * REOP_ENDALT is exelwted only on successful match of the last
        ...
        if (t) {
            ...
            op = static_cast<REOp>(emitStateSP->continueOp);
        }
    }
             ...
             JSString *str, uintN flags, JSBool flat)
{
    ...
    if (len != 0 && flat) {
        ...
        state.result->kid = const_cast<jschar *>(state.cpbegin);
    ...
    if (re->classCount) {
        ...
        for (i = 0; i < re->classCount; i++)
            re->classList[i].colwerted = JS_FALSE;
    } else {
        re->classList = NULL;
    }
                   ...
                   size_t parenIndex, size_t parenCount)
{
    ...
    REBackTrackData *result =
        (REBackTrackData *) ((char *)gData->backTrackSP + gData->lwrsz);
...
/*
 *   Conselwtive literal characters.
 */
#if 0
static REMatchState *
...
static REMatchState *
ExelwteREBytecode(REGlobalData *gData, REMatchState *x)
{
    REMatchState *result = NULL;
...
#define CHECK_BRANCH()                                                         \
    ...
    for (;;) {
        if (REOP_IS_SIMPLE(op)) {
            ...
            lwrState = &gData->stateStack[gData->stateStackTop];
            switch (op) {
            case REOP_EMPTY:
                result = x;
                break;
            ...
            case REOP_ALTPREREQ2:
                ...
                if (x->cp != gData->cpend) {
                    ...
                    if (!charSet->colwerted && !ProcessCharSet(gData, charSet))
                        return NULL;
                    matchCh1 = *x->cp;
                    k = matchCh1 >> 3;
                    if ((charSet->length == 0 ||
            ...
            doAlt:
                ...
                lwrState->parenSoFar = parenSoFar;
            ...
            /*
             * Oclwrs at (successful) end of REOP_ALT,
             */
            case REOP_JUMP:
                --gData->stateStackTop;
                pc += GET_OFFSET(pc);
            ...
            /*
             * Oclwrs at last (successful) end of REOP_ALT,
             */
            case REOP_ENDALT:
                --gData->stateStackTop;
                op = (REOp) *pc++;
            ...
            case REOP_ASSERT:
                ...
                lwrState->u.assertion.top =
                    (char *)gData->backTrackSP - (char *)gData->backTrackStack;
                lwrState->u.assertion.sz = gData->lwrsz;
                lwrState->index = x->cp - gData->cpbegin;
                lwrState->parenSoFar = parenSoFar;
                PUSH_STATE_STACK(gData);
            ...
            case REOP_ASSERTTEST:
                --gData->stateStackTop;
                --lwrState;
                x->cp = gData->cpbegin + lwrState->index;
                gData->backTrackSP =
                    (REBackTrackData *) ((char *)gData->backTrackStack +
                                         lwrState->u.assertion.top);
                gData->lwrsz = lwrState->u.assertion.sz;
                if (result)
                    result = x;
                break;
            ...
            case REOP_STAR:
                lwrState->u.quantifier.min = 0;
                lwrState->u.quantifier.max = (uintN)-1;
                goto quantcommon;
            case REOP_PLUS:
                lwrState->u.quantifier.min = 1;
                lwrState->u.quantifier.max = (uintN)-1;
                goto quantcommon;
            case REOP_OPT:
                lwrState->u.quantifier.min = 0;
                lwrState->u.quantifier.max = 1;
                goto quantcommon;
            case REOP_QUANT:
                pc = ReadCompactIndex(pc, &k);
                lwrState->u.quantifier.min = (uintn)k;
                pc = ReadCompactIndex(pc, &k);
                /* max is k - 1 to use one byte for (uintN)-1 sentinel. */
                lwrState->u.quantifier.max = (uintN)(k - 1);
                JS_ASSERT(lwrState->u.quantifier.min
                          <= lwrState->u.quantifier.max);
            quantcommon:
                if (lwrState->u.quantifier.max == 0) {
                    pc = pc + GET_OFFSET(pc);
                    op = (REOp) *pc++;
                    result = x;
                    continue;
                ...
                if (REOP_IS_SIMPLE(op)) {
                    if (!SimpleMatch(gData, x, op, &nextpc, JS_TRUE)) {
                        if (lwrState->u.quantifier.min == 0)
                            result = x;
                        else
                            result = NULL;
                        pc = pc + GET_OFFSET(pc);
                ...
                lwrState->index = startcp - gData->cpbegin;
                lwrState->continue_op = REOP_REPEAT;
                lwrState->continue_pc = pc;
                lwrState->parenSoFar = parenSoFar;
                PUSH_STATE_STACK(gData);
                if (lwrState->u.quantifier.min == 0 &&
            ...
            case REOP_ENDCHILD: /* marks the end of a quantifier child */
                pc = lwrState[-1].continue_pc;
                op = static_cast<REOp>(lwrState[-1].continue_op);
                continue;
            ...
            case REOP_REPEAT:
                ...
                do {
                    ...
                    if (lwrState->u.quantifier.min != 0)
                        lwrState->u.quantifier.min--;
                    if (lwrState->u.quantifier.max != (uintN) -1)
                        lwrState->u.quantifier.max--;
                    if (lwrState->u.quantifier.max == 0)
                        goto repeatDone;
                    nextpc = pc + ARG_LEN;
                ...
                pc = nextpc;
                op = (REOp) *pc++;
                parenSoFar = lwrState->parenSoFar;
                continue;
            ...
            minimalquantcommon:
                ...
                PUSH_STATE_STACK(gData);
                if (lwrState->u.quantifier.min != 0) {
                    lwrState->continue_op = REOP_MINIMALREPEAT;
                    lwrState->continue_pc = pc;
                    /* step over <next> */
                    pc += OFFSET_LEN;
                    op = (REOp) *pc++;
            ...
            case REOP_MINIMALREPEAT:
                ...
                if (!result) {
                     ...
                     */
                    if (lwrState->u.quantifier.max == (uintN) -1 ||
                        lwrState->u.quantifier.max > 0) {
                        lwrState->index = x->cp - gData->cpbegin;
                        lwrState->continue_op = REOP_MINIMALREPEAT;
                        lwrState->continue_pc = pc;
                        pc += ARG_LEN;
                        for (k = lwrState->parenSoFar; k < parenSoFar; k++)
                            x->parens[k].index = -1;
                        PUSH_STATE_STACK(gData);
                        op = (REOp) *pc++;
                        continue;
        ...
        if (!result) {
            ...
            lwrState = &gData->stateStack[gData->stateStackTop - 1];
...
MatchRegExp(REGlobalData *gData, REMatchState *x)
{
    ...
    for (cp2 = cp; cp2 <= gData->cpend; cp2++) {
        ...
        for (j = 0; j < gData->regexp->parenCount; j++)
            x->parens[j].index = -1;
        result = ExelwteREBytecode(gData, x);
        if (!gData->ok || result)
            return result;
...
JSBool
js_ExelwteRegExp(JSContext *cx, JSRegExp *re, JSString *str, size_t *indexp,
                 JSBool test, jsval *rval)
{
    ...
    /*
     * Call the relwrsive matcher to do the real work.  Return null on mismatch
...
static JSPropertySpec regexp_props[] = {
    {MKJSID("source"),     REGEXP_SOURCE,      REGEXP_PROP_ATTRS | JSPROP_READONLY,0,0},
    {MKJSID("global"),     REGEXP_GLOBAL,      REGEXP_PROP_ATTRS | JSPROP_READONLY,0,0},
    {MKJSID("ignoreCase"), REGEXP_IGNORE_CASE, REGEXP_PROP_ATTRS | JSPROP_READONLY,0,0},
    {MKJSID("lastIndex"),  REGEXP_LAST_INDEX,  REGEXP_PROP_ATTRS,0,0},
    {MKJSID("multiline"),  REGEXP_MULTILINE,   REGEXP_PROP_ATTRS | JSPROP_READONLY,0,0},
    {0,0,0,0,0}
};
...
static JSPropertySpec regexp_static_props[] = {
    ...
    {MKJSID("$1"), 0, JSPROP_ENUMERATE|JSPROP_READONLY|JSPROP_SHARED,
     regexp_static_getProperty,    regexp_static_getProperty},
    {MKJSID("$2"), 1, JSPROP_ENUMERATE|JSPROP_READONLY|JSPROP_SHARED,
     regexp_static_getProperty,    regexp_static_getProperty},
    {MKJSID("$3"), 2, JSPROP_ENUMERATE|JSPROP_READONLY|JSPROP_SHARED,
     regexp_static_getProperty,    regexp_static_getProperty},
    {MKJSID("$4"), 3, JSPROP_ENUMERATE|JSPROP_READONLY|JSPROP_SHARED,
     regexp_static_getProperty,    regexp_static_getProperty},
    {MKJSID("$5"), 4, JSPROP_ENUMERATE|JSPROP_READONLY|JSPROP_SHARED,
     regexp_static_getProperty,    regexp_static_getProperty},
    {MKJSID("$6"), 5, JSPROP_ENUMERATE|JSPROP_READONLY|JSPROP_SHARED,
     regexp_static_getProperty,    regexp_static_getProperty},
    {MKJSID("$7"), 6, JSPROP_ENUMERATE|JSPROP_READONLY|JSPROP_SHARED,
     regexp_static_getProperty,    regexp_static_getProperty},
    {MKJSID("$8"), 7, JSPROP_ENUMERATE|JSPROP_READONLY|JSPROP_SHARED,
     regexp_static_getProperty,    regexp_static_getProperty},
    {MKJSID("$9"), 8, JSPROP_ENUMERATE|JSPROP_READONLY|JSPROP_SHARED,
     regexp_static_getProperty,    regexp_static_getProperty},
...
JSClass js_RegExpClass = {
    ...
    JS_ColwertStub,     regexp_finalize,
    NULL,               NULL,
    regexp_call,        NULL,
                ...
                JSBool test, jsval *rval)
{
    ...
    if (lastIndex < 0 || JSSTRING_LENGTH(str) < lastIndex) {
        ...
        *rval = JSVAL_NULL;
        ...
        i = (size_t) lastIndex;
        ok = js_ExelwteRegExp(cx, re, str, &i, test, rval);
        if (ok && (re->flags & JSREG_GLOB))
            ok = js_SetLastIndex(cx, obj, (*rval == JSVAL_NULL) ? 0 : (jsdouble)i);
    }
...
static JSFunctionSpec regexp_methods[] = {
    ...
    {MKJSID("compile"),         regexp_compile,         1,0,0},
    {MKJSID("exec"),            regexp_exec,            0,0,0},
    {MKJSID("test"),            regexp_test,            0,0,0},
    {0,0,0,0,0}
};
...
js_InitRegExpClass(JSContext *cx, JSObject *obj)
{
    ...
    if (!proto || !(ctor = JS_GetConstructor(cx, proto)))
        return NULL;
    if (!JS_AliasProperty(cx, ctor, MKJSID("input"),        MKJSID("$_")) ||
        !JS_AliasProperty(cx, ctor, MKJSID("multiline"),    MKJSID("$*")) ||
        !JS_AliasProperty(cx, ctor, MKJSID("lastMatch"),    MKJSID("$&")) ||
        !JS_AliasProperty(cx, ctor, MKJSID("lastParen"),    MKJSID("$+")) ||
        !JS_AliasProperty(cx, ctor, MKJSID("leftContext"),  MKJSID("$`")) ||
        !JS_AliasProperty(cx, ctor, MKJSID("rightContext"), MKJSID("$'"))) {
        goto bad;
    }
...
js_CloneRegExpObject(JSContext *cx, JSObject *obj, JSObject *parent)
{
    ...
    if (!clone)
        return NULL;
    re = static_cast<JSRegExp *>(JS_GetPrivate(cx, obj));
