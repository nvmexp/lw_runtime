...
#define JS_PROTO(name,idname,code,init) const char js_##name##_str[] = idname;
#include "jsproto.tbl"
#undef JS_PROTO

const char *js_proto_strs[JSProto_LIMIT] = {
#define JS_PROTO(name,idname,code,init) js_##name##_str,
#include "jsproto.tbl"
#undef JS_PROTO
};

const char js_anonymous_str[]       = MKJSID("anonymous");
const char js_arguments_str[]       = MKJSID("arguments");
const char js_arity_str[]           = MKJSID("arity");
const char js_callee_str[]          = MKJSID("callee");
const char js_caller_str[]          = MKJSID("caller");
const char js_class_prototype_str[] = MKJSID("prototype");
const char js_constructor_str[]     = MKJSID("constructor");
const char js_count_str[]           = MKJSID("__count__");
const char js_each_str[]            = MKJSID("each");
const char js_eval_str[]            = MKJSID("eval");
const char js_fileName_str[]        = MKJSID("fileName");
const char js_get_str[]             = MKJSID("get");
const char js_getter_str[]          = MKJSID("getter");
const char js_index_str[]           = MKJSID("index");
const char js_input_str[]           = MKJSID("input");
const char js_iterator_str[]        = MKJSID("__iterator__");
const char js_length_str[]          = MKJSID("length");
const char js_lineNumber_str[]      = MKJSID("lineNumber");
const char js_message_str[]         = MKJSID("message");
const char js_name_str[]            = MKJSID("name");
const char js_next_str[]            = MKJSID("next");
const char js_noSuchMethod_str[]    = MKJSID("__noSuchMethod__");
const char js_object_str[]          = MKJSID("object");
const char js_parent_str[]          = MKJSID("__parent__");
const char js_proto_str[]           = MKJSID("__proto__");
const char js_setter_str[]          = MKJSID("setter");
const char js_set_str[]             = MKJSID("set");
const char js_stack_str[]           = MKJSID("stack");
const char js_toSource_str[]        = MKJSID("toSource");
const char js_toString_str[]        = MKJSID("toString");
const char js_toLocaleString_str[]  = MKJSID("toLocaleString");
const char js_valueOf_str[]         = MKJSID("valueOf");

#if JS_HAS_XML_SUPPORT
const char js_etago_str[]           = MKJSID("</");
const char js_namespace_str[]       = MKJSID("namespace");
const char js_ptagc_str[]           = MKJSID("/>");
const char js_qualifier_str[]       = MKJSID("::");
const char js_space_str[]           = MKJSID(" ");
const char js_stago_str[]           = MKJSID("<");
const char js_star_str[]            = MKJSID("*");
const char js_starQualifier_str[]   = MKJSID("*::");
const char js_tagc_str[]            = MKJSID(">");
const char js_xml_str[]             = MKJSID("xml");
#endif

#if JS_HAS_GENERATORS
const char js_close_str[]           = MKJSID("close");
const char js_send_str[]            = MKJSID("send");
#endif

#ifdef NARCISSUS
const char js_call_str[]             = MKJSID("__call__");
const char js_construct_str[]        = MKJSID("__construct__");
const char js_hasInstance_str[]      = MKJSID("__hasInstance__");
const char js_ExelwtionContext_str[] = MKJSID("ExelwtionContext");
const char js_lwrrent_str[]          = MKJSID("current");
#endif
...
js_hash_atom_key(const void *key, void *)
{
    jsval v;
    jsdouble *dp;
    ...
    if (JSVAL_IS_STRING(v))
        return js_HashString(JSVAL_TO_STRING(v), 0);
    if (JSVAL_IS_INT(v))
...
js_compare_atom_keys(const void *k1, const void *k2, void *)
{
    jsval v1, v2;
...
JSBool
js_InitAtomState(JSContext *cx, JSAtomState *state, JSHashFunction hashFun, void *hashFunData,
                 JSHashKeyComparator compareKeys, void *compareKeysData)
{
    // Original code passes js_hash_atom_key as the hash function, but later
    // ignores it and uses js_HashString in js_AtomizeString and specific hash
    // functions depending on the type of the atom. In fact js_hash_atom_key
    // is never called at all. So in js_AtomizeString we use js_HashString if
    // JSHashTable::keyHash is equal to js_hash_atom_key or user supplied
    // hashFun otherwise.
    state->table = JS_NewHashTable(JS_ATOM_HASH_SIZE, 0 == hashFun ? js_hash_atom_key : hashFun,
                                   hashFunData,
                                   0 == compareKeys ? js_compare_atom_keys : compareKeys,
                                   compareKeysData, js_compare_stub, &atom_alloc_ops, state);
    if (!state->table) {
...
#define FROB(lval,str)                                                      \
    ...
    FROB(ExelwtionContextAtom,    js_ExelwtionContext_str);
    FROB(lwrrentAtom,             js_lwrrent_str);
#endif
...
js_AtomizeString(JSContext *cx, JSString *str, uintN flags)
{
    ...
    // js_hash_atom_key is the default value for JSHashTable::keyHash. If it is
    // set to default, use js_HashString as the original code does, or use user
    // supplied JSHashTable::keyHash.
    keyHash = (js_hash_atom_key == table->keyHash ? js_HashString(str, NULL) :
               table->keyHash(str, table->keyHashData));
...
js_AtomizeChars(JSContext *cx, const jschar *chars, size_t length, uintN flags)
{
    ...
    str = ALIGN(buf, JSString);
    str->chars = const_cast<jschar *>(chars);
    str->SetFlags(0);
...
js_GetExistingStringAtom(JSContext *cx, const jschar *chars, size_t length)
{
    ...
    str->SetFlags(0);
    str->SetLength(length);
    keyHash = js_HashString(str, NULL);
    key = STRING_TO_JSVAL(str);
    state = &cx->runtime->atomState;
    JS_LOCK(&state->lock, cx);
    table = state->table;
...
JS_STATIC_DLL_CALLBACK(JSHashNumber)
js_hash_atom_ptr(const void *key, void *)
{
    const JSAtom *atom = static_cast<const JSAtom *>(key);
    return atom->number;
}
...
{
    JSContext *cx = static_cast<JSContext *>(priv);
    void *space;
    ...
    if (!ale) {
        if (al->count < 10) {
            ...
            if (!al->table) {
                ...
                JS_ASSERT(!hep);
                al->table = JS_NewHashTable(al->count + 1, js_hash_atom_ptr, NULL,
                                            JS_CompareHashKeys, 0, JS_CompareHashValues,
                                            &temp_alloc_ops, cx);
                if (!al->table)
                    return NULL;
...
js_map_atom(JSHashEntry *he, intN i, void *arg)
{
    ...
    const JSAtom **vector = static_cast<const JSAtom **>(arg);
...
js_InitAtomMap(JSContext *cx, JSAtomMap *map, JSAtomList *al)
{
    ...
    vector = (const JSAtom **) JS_malloc(cx, (size_t) count * sizeof *vector);
    if (!vector)
        return JS_FALSE;
...
#endif
        JS_HashTableEnumerateEntries(al->table, js_map_atom, (void *)vector);
    } else {
        do {
            vector[ALE_INDEX(ale)] = ALE_ATOM(ale);
    ...
    map->vector = const_cast<JSAtom **>(vector);
