 ...
 * - JSCLASS_DOLWMENT_OBSERVER support -- live two-way binding to Gecko's DOM!
 ...
 */
const char js_isXMLName_str[]     = MKJSID("isXMLName");
const char js_XMLList_str[]       = MKJSID("XMLList");
const char js_localName_str[]     = MKJSID("localName");
const char js_xml_parent_str[]    = MKJSID("parent");
const char js_prefix_str[]        = MKJSID("prefix");
const char js_toXMLString_str[]   = MKJSID("toXMLString");
const char js_uri_str[]           = MKJSID("uri");
...
JS_FRIEND_DATA(JSExtendedClass) js_NamespaceClass = {
  { MKJSID("Namespace"),
    ...
    JS_EnumerateStub,  JS_ResolveStub,    JS_ColwertStub,    namespace_finalize,
    NULL,              NULL,              NULL,              NULL,
...
JS_FRIEND_DATA(JSExtendedClass) js_QNameClass = {
  { MKJSID("QName"),
    ...
    JS_EnumerateStub,  JS_ResolveStub,    JS_ColwertStub,    qname_finalize,
    NULL,              NULL,              NULL,              NULL,
...
JS_FRIEND_DATA(JSClass) js_AnyNameClass = {
    ...
    JS_PropertyStub,   JS_PropertyStub,   JS_PropertyStub,   JS_PropertyStub,
    JS_EnumerateStub,  JS_ResolveStub,    JS_ColwertStub,    anyname_finalize,
    NULL,              NULL,              NULL,              NULL,
    NULL,              NULL,              qname_mark,        NULL
};
...
JSObject *
js_ConstructXMLQNameObject(JSContext *cx, jsval nsval, jsval llwal)
{
    jsval argv[2];
...
static void
XMLArrayLwrsorInit(JSXMLArrayLwrsor *cursor, JSXMLArray *array)
{
    JSXMLArrayLwrsor *next;

    cursor->array = array;
    cursor->index = 0;
    next = cursor->next = array->lwrsors;
    if (next)
        next->prevp = &cursor->next;
    cursor->prevp = &array->lwrsors;
    array->lwrsors = cursor;
    cursor->root = NULL;
}

static void
XMLArrayLwrsorFinish(JSXMLArrayLwrsor *cursor)
{
    JSXMLArrayLwrsor *next;
...
static void *
XMLArrayLwrsorNext(JSXMLArrayLwrsor *cursor)
{
    JSXMLArray *array;
...
static void *
XMLArrayLwrsorItem(JSXMLArrayLwrsor *cursor)
{
    JSXMLArray *array;
...
static void
XMLArrayLwrsorMark(JSContext *cx, JSXMLArrayLwrsor *cursor)
{
    while (cursor) {
...
{
    JSXMLArrayLwrsor *cursor;

    JS_free(cx, array->vector);

    while ((cursor = array->lwrsors) != NULL)
        XMLArrayLwrsorFinish(cursor);
...
XMLArrayInsert(JSContext *cx, JSXMLArray *array, uint32 i, uint32 n)
{
    ...
    for (cursor = array->lwrsors; cursor; cursor = cursor->next) {
        if (cursor->index > i)
            cursor->index += n;
    }
    return JS_TRUE;
...
XMLArrayTruncate(JSContext *cx, JSXMLArray *array, uint32 length)
{
    ...
    if (length == 0) {
        ...
        vector = static_cast<void **>(realloc(array->vector, length * sizeof(void *)));
        if (!vector)
            return;
    }
 ...
 */
static const char js_ignoreComments_str[]   = MKJSID("ignoreComments");
static const char js_ignoreProcessingInstructions_str[]
                                            = MKJSID("ignoreProcessingInstructions");
static const char js_ignoreWhitespace_str[] = MKJSID("ignoreWhitespace");
static const char js_prettyPrinting_str[]   = MKJSID("prettyPrinting");
static const char js_prettyIndent_str[]     = MKJSID("prettyIndent");
    ...
    switch (pn->pn_type) {
      case TOK_XMLELEM:
        ...
        while ((pn2 = pn2->pn_next) != NULL) {
            ...
            /* XXX where is this dolwmented in an XML spec, or in E4X? */
            if ((flags & XSF_IGNORE_WHITESPACE) &&
      ...
      case TOK_XMLPTAGC:
        ...
        while ((pn2 = *pnp) != NULL) {
            ...
            if (length >= 5 &&
                 ...
                 * we enforce the ilwariant, noted many times in ECMA-357, that
...
ToXML(JSContext *cx, jsval v)
{
    ...
    if (JSVAL_IS_PRIMITIVE(v)) {
        ...
        clasp = OBJ_GET_CLASS(cx, obj);
        if (clasp->flags & JSCLASS_DOLWMENT_OBSERVER) {
            JS_ASSERT(0);
        }
...
GeneratePrefix(JSContext *cx, JSString *uri, JSXMLArray *decls)
{
     ...
     * of 'a's of size length (allowing dp-callwlating code to work with or
     * without this branch exelwting) plus the space for storing a hyphen and
     * the serial number (avoiding reallocation if a collision happens).
     */
    bp = const_cast<jschar *>(cp);
    newlength = length;
    if (STARTS_WITH_XML(cp, length) || !IsXMLName(cp, length)) {
        newlength = length + 2 + (size_t) log10(static_cast<double>(decls->length));
        bp = (jschar *)
    ...
    do {
        ...
        for (i = 0, n = decls->length; i < n; i++) {
            ...
            if (ns && ns->prefix &&
                ...
                if (bp == cp) {
                    newlength = length + 2 + (size_t) log10(static_cast<double>(n));
                    bp = (jschar *)
                         JS_malloc(cx, (newlength + 1) * sizeof(jschar));
                    if (!bp)
                        return NULL;
                ...
                ++serial;
                JS_ASSERT(serial <= n);
                dp = bp + length + 2 + (size_t) log10(static_cast<double>(serial));
                *dp = 0;
               ...
               uintN indentLevel)
{
    ...
    switch (xml->xml_class) {
      ...
      case JSXML_CLASS_LIST:
        /* ECMA-357 10.2.2. */
        XMLArrayLwrsorInit(&cursor, &xml->xml_kids);
        i = 0;
        while ((kid = (JSXML *) XMLArrayLwrsorNext(&cursor)) != NULL) {
            if (pretty && i != 0)
                js_AppendChar(&sb, '\n');
    ...
    XMLArrayLwrsorInit(&cursor, &xml->xml_namespaces);
    while ((ns = (JSXMLNamespace *) XMLArrayLwrsorNext(&cursor)) != NULL) {
        if (!ns->declared)
            continue;
    ...
    XMLArrayLwrsorInit(&cursor, &xml->xml_attrs);
    while ((attr = (JSXML *) XMLArrayLwrsorNext(&cursor)) != NULL) {
        js_AppendChar(&sb, ' ');
    ...
    XMLArrayLwrsorInit(&cursor, &decls);
    while ((ns2 = (JSXMLNamespace *) XMLArrayLwrsorNext(&cursor)) != NULL) {
                 ...
                 uintN flags)
{
    ...
    XMLArrayLwrsorInit(&cursor, from);
    j = 0;
    ok = JS_TRUE;
    while ((kid = (JSXML *) XMLArrayLwrsorNext(&cursor)) != NULL) {
        if ((flags & XSF_IGNORE_COMMENTS) &&
...
DeepCopyInLRS(JSContext *cx, JSXML *xml, uintN flags)
{
    ...
    copy = js_NewXML(cx, static_cast<JSXMLClass>(xml->xml_class));
    if (!copy)
        return NULL;
    qn = xml->name;
    if (qn) {
...
Descendants(JSContext *cx, JSXML *xml, jsval id)
{
     ...
     * which protects list.  Any other object allocations oclwring beneath
...
/* Relwrsive (JSXML *) parameterized version of Equals. */
static JSBool
XMLEquals(JSContext *cx, JSXML *xml, JSXML *vxml, JSBool *bp)
{
    JSXMLQName *qn, *vqn;
    uint32 i, j, n;
    JSXMLArrayLwrsor cursor, vlwrsor;
...
retry:
    ...
    if (JSXML_HAS_VALUE(xml)) {
        ...
        *bp = JS_FALSE;
        ...
        XMLArrayLwrsorInit(&cursor, &xml->xml_kids);
        XMLArrayLwrsorInit(&vlwrsor, &vxml->xml_kids);
        for (;;) {
            kid = (JSXML *) XMLArrayLwrsorNext(&cursor);
            vkid = (JSXML *) XMLArrayLwrsorNext(&vlwrsor);
            if (!kid || !vkid) {
                *bp = !kid && !vkid;
                ok = JS_TRUE;
                break;
                 ...
                 JSBool attributes, JSXML *list)
{
    ...
    XMLArrayLwrsorInit(&cursor, array);
    while ((kid = (JSXML *) XMLArrayLwrsorNext(&cursor)) != NULL) {
    ...
    if (xml->xml_class == JSXML_CLASS_LIST) {
        ...
        if (js_IdIsIndex(id, &index)) {
            ...
            if (!vxml ||
                ...
                ok = JS_ColwertValue(cx, *vp, JSTYPE_STRING, vp);
                if (!ok)
                    goto out;
                roots[VAL_ROOT] = *vp;
            }
        ...
        if (primitiveAssign) {
            ...
            XMLArrayLwrsorInit(&cursor, &xml->xml_kids);
            cursor.index = index;
            kid = (JSXML *) XMLArrayLwrsorItem(&cursor);
            ...
            if (ok) {
                ...
                if (ok && !IS_EMPTY(JSVAL_TO_STRING(*vp))) {
                    roots[VAL_ROOT] = *vp;
                    if ((JSXML *) XMLArrayLwrsorItem(&cursor) == kid)
                   ...
                   JSProperty **propp)
{
    ...
    if (*propp == FOUND_XML_PROPERTY) {
        sprop = js_AddNativeProperty(cx, obj, id, GetProperty, PutProperty,
                                     SPROP_ILWALID_SLOT, JSPROP_ENUMERATE,
                                     0, 0);
        if (!sprop)
            return JS_FALSE;
    ...
    switch (enum_op) {
      case JSENUMERATE_INIT:
        if (length == 0) {
            ...
            cursor = (JSXMLArrayLwrsor *) JS_malloc(cx, sizeof *cursor);
            if (!cursor)
                return JS_FALSE;
            XMLArrayLwrsorInit(cursor, &xml->xml_kids);
        }
      ...
      case JSENUMERATE_NEXT:
        cursor = static_cast<JSXMLArrayLwrsor *>(JSVAL_TO_PRIVATE(*statep));
...
JS_FRIEND_DATA(JSClass) js_XMLClass = {
    ...
    JS_PropertyStub,   JS_PropertyStub,   JS_PropertyStub,   JS_PropertyStub,
    JS_EnumerateStub,  JS_ResolveStub,    JS_ColwertStub,    xml_finalize,
...
mark_temp_ns_array(JSContext *cx, JSTempValueRooter *tvr)
{
    ...
    namespace_mark_vector(cx,
                          ...
                          tmp->array.length);
    XMLArrayLwrsorMark(cx, tmp->array.lwrsors);
...
static const char js_attribute_str[] = MKJSID("attribute");
static const char js_text_str[]      = MKJSID("text");
...
xml_replace(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    *rval = OBJECT_TO_JSVAL(obj);
    ...
    vxml = VALUE_IS_XML(cx, value)
           ...
           : NULL;
    if (!vxml) {
        if (!JS_ColwertValue(cx, value, JSTYPE_STRING, &argv[1]))
            return JS_FALSE;
        value = argv[1];
    } else {
                 ...
                 jsval *rval)
{
    ...
    if (!JSVAL_IS_PRIMITIVE(name) &&
        ...
        if (!JS_ColwertValue(cx, name, JSTYPE_STRING, &argv[0]))
            return JS_FALSE;
        name = argv[0];
...
xml_setName(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (nameqn->prefix) {
         ...
         * does not need to be colwerted from null to empty by QName).
         */
...
static JSFunctionSpec xml_methods[] = {
    {MKJSID("addNamespace"),          xml_addNamespace,          1,0,0},
    {MKJSID("appendChild"),           xml_appendChild,           1,0,0},
    {js_attribute_str,                xml_attribute,             1,0,0},
    {MKJSID("attributes"),            xml_attributes,            0,0,0},
    {MKJSID("child"),                 xml_child,                 1,0,0},
    {MKJSID("childIndex"),            xml_childIndex,            0,0,0},
    {MKJSID("children"),              xml_children,              0,0,0},
    {MKJSID("comments"),              xml_comments,              0,0,0},
    {MKJSID("contains"),              xml_contains,              1,0,0},
    {MKJSID("copy"),                  xml_copy,                  0,0,0},
    {MKJSID("descendants"),           xml_descendants,           1,0,0},
    {MKJSID("elements"),              xml_elements,              1,0,0},
    {MKJSID("hasOwnProperty"),        xml_hasOwnProperty,        1,0,0},
    {MKJSID("hasComplexContent"),     xml_hasComplexContent,     1,0,0},
    {MKJSID("hasSimpleContent"),      xml_hasSimpleContent,      1,0,0},
    {MKJSID("inScopeNamespaces"),     xml_inScopeNamespaces,     0,0,0},
    {MKJSID("insertChildAfter"),      xml_insertChildAfter,      2,0,0},
    {MKJSID("insertChildBefore"),     xml_insertChildBefore,     2,0,0},
    ...
    {MKJSID("namespaceDeclarations"), xml_namespaceDeclarations, 0,0,0},
    {MKJSID("nodeKind"),              xml_nodeKind,              0,0,0},
    {MKJSID("normalize"),             xml_normalize,             0,0,0},
    {js_xml_parent_str,               xml_parent,                0,0,0},
    {MKJSID("processingInstructions"),xml_processingInstructions,1,0,0},
    {MKJSID("prependChild"),          xml_prependChild,          1,0,0},
    {MKJSID("propertyIsEnumerable"),  xml_propertyIsEnumerable,  1,0,0},
    {MKJSID("removeNamespace"),       xml_removeNamespace,       1,0,0},
    {MKJSID("replace"),               xml_replace,               2,0,0},
    {MKJSID("setChildren"),           xml_setChildren,           1,0,0},
    {MKJSID("setLocalName"),          xml_setLocalName,          1,0,0},
    {MKJSID("setName"),               xml_setName,               1,0,0},
    {MKJSID("setNamespace"),          xml_setNamespace,          1,0,0},
...
static JSFunctionSpec xml_static_methods[] = {
    {MKJSID("settings"),         xml_settings,          0,0,0},
    {MKJSID("setSettings"),      xml_setSettings,       1,0,0},
    {MKJSID("defaultSettings"),  xml_defaultSettings,   0,0,0},
    {0,0,0,0,0}
};
    ...
    *rval = OBJECT_TO_JSVAL(xobj);
    ...
    if ((cx->fp->flags & JSFRAME_CONSTRUCTING) && !JSVAL_IS_PRIMITIVE(v)) {
        ...
        if (clasp == &js_XMLClass ||
            (clasp->flags & JSCLASS_DOLWMENT_OBSERVER)) {
...
js_MarkXML(JSContext *cx, JSXML *xml)
{
    ...
    xml_mark_vector(cx,
                    ...
                    xml->xml_kids.length);
    XMLArrayLwrsorMark(cx, xml->xml_kids.lwrsors);
    XMLArrayTrim(&xml->xml_kids);
    ...
    if (xml->xml_class == JSXML_CLASS_LIST) {
        ...
        XMLArrayLwrsorMark(cx, xml->xml_namespaces.lwrsors);
        XMLArrayTrim(&xml->xml_namespaces);

        xml_mark_vector(cx,
                        (JSXML **) xml->xml_attrs.vector,
                        xml->xml_attrs.length);
        XMLArrayLwrsorMark(cx, xml->xml_attrs.lwrsors);
    ...
    if (!obj) {
        ...
        if (!obj) {
            ...
            do {
                 ...
                 * but no big deal: we want to lwstomize toString anyway for
                 * clearer diagnostics.
                 */
...
js_FilterXMLList(JSContext *cx, JSObject *obj, jsbytecode *pc, jsval *vp)
{
    ...
    *vp = JSVAL_NULL;
    ...
    XMLArrayLwrsorInit(&cursor, &list->xml_kids);
    while ((kid = (JSXML *) XMLArrayLwrsorNext(&cursor)) != NULL) {
        kidobj = js_GetXMLObject(cx, kid);
        if (!kidobj)
            break;
