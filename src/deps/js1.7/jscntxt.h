...
struct JSThread {
    ...
    /* Opaque thread-id, from NSPR's PR_GetLwrrentThread(). */
    jsword              id;
    ...
    /* Flag indicating that the current thread is exelwting close hooks. */
...
extern JSThread *
js_GetLwrrentThread(JSRuntime *rt);
...
struct JSRuntime {
    ...
    uint32              gcMallocBytes;
    JSGCArena           *glwnscannedArenaStackTop;
#ifdef DEBUG
    size_t              glwnscannedBagSize;
#endif
    ...
    JSInterpreterHook   exelwteHook;
    void               *exelwteHookData;
    ...
    JSConcatHook        concatHook;
    void               *concatHookData;
    JSFinalizeStrHook   strFinalizeHook;
    void               *strFinalizeHookData;
    JSEvalHook          evalHook;
    void               *evalHookData;
    JSAddPropHook       addPropHook;
    void               *addPropHookData;
    JSArrayJoinHook     arrayJoinHook;
    void               *arrayJoinHookData;
    JSCheckIndexHook    checkIndexHook;
    void               *checkIndexHookData;
    JSDefaultNumHook    defaultNumHook;
    void               *defaultNumHookData;
    JSExpandErrArgHook  expandErrArgHook;
    void               *expandErrArgHookData;
    JSValDecompileHook  valDecompileHook;
    void               *valDecompileHookData;
...
/*
 * Linked list mapping format strings for JS_{Colwert,Push}Arguments{,VA} to
...
struct JSContext {
    ...
    /* Limit pointer for checking stack consumption during relwrsion. */
    jsuword             stackLimit;
    ...
    /* State for object and array toSource colwersion. */
    JSSharpObjectMap    sharpObjectMap;

    /* Argument formatter support for JS_{Colwert,Push}Arguments{,VA}. */
...
#define CX_FROM_THREAD_LINKS(tl) \
     ...
     * jsval by calling JS_SetCallReturlwalue2(cx, idval).
     */
    jsval               rval2;
    ...
    /*
     * True if creating an exception object, to prevent runaway relwrsion.
    ...
    /* Locale specific callbacks for string colwersion. */
    JSLocaleCallbacks   *localeCallbacks;

    /*
     * cx->resolvingTable is non-null and non-empty if we are initializing
     * standard classes lazily, or if we are otherwise relwrsing indirectly
     * from js_LookupProperty through a JSClass.resolve hook.  It is used to
     * limit runaway relwrsion (see jsapi.c and jsobj.c).
     */
 ...
 *
 * But when exelwting a script or scripted function, the interpreter changes
 ...
 */
extern void
js_OlwersionChange(JSContext *cx);
...
/*
 * JSClass.resolve and watchpoint relwrsion damping machinery.
 */
extern JSBool
...
/*
 * Report an exception, which is lwrrently realized as a printf-style format
...
#   include "Tracy.hpp"
#   define JS_PROFILE_ZONE_COLOR                0xA1A823
#   define JS_PROFILE_ZONE                      ZoneScopedC(JS_PROFILE_ZONE_COLOR)
#   define JS_PROFILE_ZONE_SET_NAME(name, size) ZoneName(name, size)
#   define JS_PROFILE_FRAME_START(name)         FrameMarkStart(name)
#   define JS_PROFILE_FRAME_END(name)           FrameMarkEnd(name)
#else
#   define JS_PROFILE_ZONE
#   define JS_PROFILE_ZONE_SET_NAME(name, size)
#   define JS_PROFILE_FRAME_START(name)
#   define JS_PROFILE_FRAME_END(name)
#endif
