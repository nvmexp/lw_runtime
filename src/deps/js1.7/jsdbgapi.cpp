...
js_MarkWatchPoints(JSContext *cx)
{
    ...
    for (wp = (JSWatchPoint *)rt->watchPointList.next;
        ...
        if (wp->sprop->attrs & JSPROP_SETTER)
            JS_MarkGCThing(cx, reinterpret_cast<void*>(wp->setter), "wp->setter", NULL);
    }
}
        ...
        if (wp->object == obj && SPROP_USERID(sprop) == id &&
            ...
            if (ok) {
                ...
                if (nslots <= JS_ARRAY_LENGTH(smallv)) {
                    ...
                    argv = static_cast<jsval *>(JS_malloc(cx, nslots * sizeof(jsval)));
                    if (!argv) {
                ...
                argv[1] = JSVAL_NULL;
                // Surpress 0 length parameter warning
                if (nslots > 2) {
                ...
                ok = !wp->setter ||
                     ((sprop->attrs & JSPROP_SETTER)
                      ? js_InternalCall(cx, obj, OBJECT_TO_JSVAL(reinterpret_cast<JSObject *>(wp->setter)),
                                        1, vp, vp)
...
{
    return static_cast<JSVersion>(script->version & JSVERSION_MASK);
}
                          ...
                          jsval *rval)
{
    ...
    ok = js_Exelwte(cx, scobj, script, fp, JSFRAME_DEBUGGER | JSFRAME_EVAL,
                    rval);
...
{
    rt->strFinalizeHook = hook;
    rt->strFinalizeHookData = closure;
    return JS_TRUE;
}
...
{
    rt->arrayJoinHook = hook;
    rt->arrayJoinHookData = closure;
    return JS_TRUE;
}
...
{
    rt->checkIndexHook = hook;
    rt->checkIndexHookData = closure;
    return JS_TRUE;
}
...
{
    rt->defaultNumHook = hook;
    rt->defaultNumHookData = closure;
    return JS_TRUE;
}
...
{
    rt->expandErrArgHook = hook;
    rt->expandErrArgHookData = closure;
    return JS_TRUE;
}
...
{
    rt->valDecompileHook = hook;
    rt->valDecompileHookData = closure;
    return JS_TRUE;
}
...
JS_GetObjectTotalSize(JSContext *cx, JSObject *obj)
{
    ...
    if (OBJ_IS_NATIVE(obj)) {
        ...
        if (scope->object == obj) {
            nbytes += sizeof *scope;
            nbytes += (size_t)SCOPE_CAPACITY(scope) * sizeof(JSScopeProperty *);
        }
    }
    return nbytes;
}
...
GetAtomTotalSize(JSContext *cx, JSAtom *atom)
{
    ...
    if (ATOM_IS_STRING(atom)) {
        ...
        nbytes += (ATOM_TO_STRING(atom)->GetLength() + 1) * sizeof(jschar);
