...
typedef struct JSGCCloseState {
    ...
    /*
     * Flag indicating that the current thread is exlwting a close hook for
     * single thread case.
     */
 ...
 */
typedef enum JSGCIlwocationKind {
    /* Normal invocation. */
    GC_NORMAL,
...
typedef struct JSGCStats {
    ...
    uint32  depth;      /* mark tail relwrsion depth */
    uint32  maxdepth;   /* maximum mark tail relwrsion depth */
    uint32  cdepth;     /* mark relwrsion depth of C functions */
    uint32  maxcdepth;  /* maximum mark relwrsion depth of C functions */
...
#endif
    uint32  maxlevel;   /* maximum GC nesting (indirect relwrsion) level */
...
typedef struct JSWeakRoots {
    ...
    /* Root for the result of the most recent js_InternalIlwoke call. */
    jsval               lastInternalResult;
} JSWeakRoots;

#if defined(JS_USE_STRONGLY_TYPED_JSVAL_AND_JSID)
// because of weaknesses of MSVC JSVAL_NULL cannot be constexpr and jsval::Null
// cannot be jsval
JS_STATIC_ASSERT(jsval::Null == 0);
#else
