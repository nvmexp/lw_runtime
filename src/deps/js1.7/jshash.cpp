...
JS_NewHashTable(uint32 n, JSHashFunction keyHash, void *keyHashData, JSHashKeyComparator keyCompare,
                void *keyCompareData, JSHashValueComparator valueCompare, JSHashAllocOps *allocOps,
                void *allocPriv)
{
    JSHashTable *ht;
    size_t nb;
    ...
    ht->keyHash = keyHash;
    ht->keyHashData = keyHashData;
    ht->keyCompare = keyCompare;
    ht->keyCompareData = keyCompareData;
...
JS_HashTableRawLookup(JSHashTable *ht, JSHashNumber keyHash, const void *key)
{
    ...
    while ((he = *hep) != NULL) {
        if (he->keyHash == keyHash && ht->keyCompare(key, he->key, ht->keyCompareData)) {
...
#ifdef DEBUG
    size_t nold = (size_t)NBUCKETS(ht);
#endif
...
JS_HashTableAdd(JSHashTable *ht, const void *key, void *value)
{
    ...
    keyHash = ht->keyHash(key, ht->keyHashData);
    hep = JS_HashTableRawLookup(ht, keyHash, key);
    if ((he = *hep) != NULL) {
