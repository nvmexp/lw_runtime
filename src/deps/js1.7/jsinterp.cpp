 ...
 * to a function that may ilwoke the interpreter.  RESTORE_SP must be called
 * only after return from js_Ilwoke, because only js_Ilwoke changes fp->sp.
 */
#define PUSH(v)         (*sp++ = (v))
#define POP()           (*--sp)
#ifdef DEBUG
 ...
 * Try to colwert d to a jsint that fits in a jsval, otherwise GC-alloc space
...
/*
 * Optimized colwersion macros that test for the desired type in v before
 * homing sp and calling a colwersion function.
 */
...
/*
 * Colwert a primitive string, number or boolean to a corresponding object.
...
js_AllocStack(JSContext *cx, uintN nslots, void **markp)
{
    ...
    if (nslots == 0) {
        *markp = NULL;
        return static_cast<jsval *>(JS_ARENA_MARK(&cx->stackPool));
    }
...
js_GetScopeChain(JSContext *cx, JSStackFrame *fp)
{
    ...
    /*
     * Clone the block chain. To avoid relwrsive cloning we set the parent of
    ...
    for (;;) {
         ...
         * slot later as null triggers useless callwlations of slot's value in
             ...
             uintN argc)
{
     ...
     * hit, since we'll call it again in the normal sequence of ilwoke events,
     ...
     * to clasp->colwert (see below).  This matters because ComputeThis may
    ...
    switch ((JSOp) *pc) {
      ...
      case JSOP_GETMETHOD:
        ...
        ok = js_InternalIlwoke(cx, thisp, roots[2], flags | JSILWOKE_INTERNAL,
                               2, roots, &vp[0]);
        break;
  ...
  not_function:
    js_ReportIsNotFunction(cx, vp, flags & JSILWOKE_FUNFLAGS);
    ok = JS_FALSE;
    goto out;
}
...
typedef struct CallValue {
    ...
    struct ArgInfo {
        ...
        struct ArgValCount {
            ...
            char        strbuf[112];            /* string colwersion buffer */
...
js_Ilwoke(JSContext *cx, uintN argc, uintN flags)
{
    void *mark;
    ...
    JSFunction *fun;
    JSScript *script;
    uintN nslots, lwars, nalloc, surplus;
    ...
    if (JSVAL_IS_PRIMITIVE(v)) {
#if JS_HAS_NO_SUCH_METHOD
        if (fp->script && !(flags & JSILWOKE_INTERNAL)) {
    ...
    if (clasp != &js_FunctionClass) {
        ...
        /*
         * XXX this makes no sense -- why colwert to function if clasp->call?
         * XXX better to call that hook without colwerting
         * XXX the only thing that needs fixing is liveconnect
         *
         * Try colwerting to function, for closure and API compatibility.
         * We attempt the colwersion under all cirlwmstances for 1.2, but
         * only if there is a call op defined otherwise.
         */
        if ((ops == &js_ObjectOps) ? clasp->call : ops->call) {
            ok = clasp->colwert(cx, funobj, JSTYPE_FUNCTION, &v);
            if (!ok)
                goto out2;
        ...
        native = (flags & JSILWOKE_CONSTRUCT) ? ops->construct : ops->call;
        if (!native)
            goto bad;
...
have_fun:
...
#ifdef TRACY_ENABLE
        JSString* const namestr = JS_GetFunctionId(fun);
        if (namestr)
        {
            char buf[32];
            size_t size = sizeof(buf);
            if (js_DeflateStringToBuffer(cx, JSSTRING_CHARS(namestr), JSSTRING_LENGTH(namestr),
                                         buf, &size))
            {
                JS_PROFILE_ZONE_SET_NAME(buf, size)
            }
        }
#endif
        ...
        if (JSFUN_BOUND_METHOD_TEST(fun->flags)) {
            ...
            JS_ASSERT(!(flags & JSILWOKE_CONSTRUCT));
  ...
  init_frame:
    ...
    if (lwars) {
        ...
        surplus = (uintN)((jsval *)cx->stackPool.current->avail - frame.vars);
        if (surplus < lwars) {
            newsp = js_AllocRawStack(cx, lwars, NULL);
            if (!newsp) {
                ok = JS_FALSE;
                goto out;
            }
    ...
    if (native) {
        ...
        /* Set by JS_SetCallReturlwalue2, used to return reference types. */
...
JSBool
js_InternalIlwoke(JSContext *cx, JSObject *obj, jsval fval, uintN flags,
                  uintN argc, jsval *argv, jsval *rval)
{
    ...
    for (i = 0; i < argc; i++)
        PUSH(argv[i]);
    SAVE_SP(fp);
    ok = js_Ilwoke(cx, argc, flags | JSILWOKE_INTERNAL);
    if (ok) {
        RESTORE_SP(fp);
         ...
         * js_InternalIlwoke and its direct and indirect (js_ValueToString for
                    ...
                    JSAccessMode mode, uintN argc, jsval *argv, jsval *rval)
{
    ...
    /*
     * js_InternalIlwoke could result in another try to get or set the same id
     ...
     * about to ilwoke.  If we don't check this case, nothing else will -- no
...
JSBool
js_Exelwte(JSContext *cx, JSObject *chain, JSScript *script,
           JSStackFrame *down, uintN flags, jsval *result)
{
    ...
    if (down) {
        ...
        frame.argc = 0;
        frame.argv = NULL;
        frame.lwars = script->numGlobalVars;
        if (frame.lwars) {
            frame.vars = js_AllocRawStack(cx, frame.lwars, &mark);
            if (!frame.vars)
                return JS_FALSE;
            memset(frame.vars, 0, frame.lwars * sizeof(jsval));
        } else {
            frame.vars = NULL;
        }
        frame.annotation = NULL;
    ...
    cx->fp = &frame;
    if (hook)
        hookData = hook(cx, &frame, JS_TRUE, 0, cx->runtime->exelwteHookData);
...
JSBool
js_IlwokeConstructor(JSContext *cx, jsval *vp, uintN argc)
{
    JSFunction *fun;
    ...
    if (!js_Ilwoke(cx, argc, JSILWOKE_CONSTRUCT)) {
 ...
 * Lwrrently it's broken for JS_VERSION < 160, though this isn't worth fixing.
...
js_Interpret(JSContext *cx, jsbytecode *pc, jsval *result)
{
    ...
    JSVersion lwrrentVersion, originalVersion;
...
# undef OPDEF
    ...
    void **jumpTable = normalJumpTable;

# define DO_OP()            JS_EXTENSION_(goto *jumpTable[op])
# define DO_NEXT_OP(n)      do { op = static_cast<JSOp>(*(pc += (n))); DO_OP(); } while (0)
# define BEGIN_CASE(OP)     L_##OP:
# define END_CASE(OP)       DO_NEXT_OP(OP##_LENGTH);
# define END_VARLEN_CASE    DO_NEXT_OP(len);
# define EMPTY_CASE(OP)     BEGIN_CASE(OP) op = static_cast<JSOp>(*++pc); DO_OP();
#else
...
# define EMPTY_CASE(OP)     BEGIN_CASE(OP) END_CASE(OP)
    ...
    *result = JSVAL_VOID;
     ...
     * maintaining lwrrentVersion.  This is relied upon by testsuites, for
     * the most part -- web browsers select version before compiling and not
     * at run-time.
     */
    lwrrentVersion = static_cast<JSVersion>(script->version);
    originalVersion = static_cast<JSVersion>(cx->version);
    if (lwrrentVersion != originalVersion)
        js_SetVersion(cx, lwrrentVersion);
...
#define LOAD_INTERRUPT_HANDLER(rt)                                            \
    ...
    if (++cx->interpLevel == MAX_INTERP_LEVEL ||
        ...
        JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL, JSMSG_OVER_RELWRSED);
        ok = JS_FALSE;
        goto out2;
    }
      ...
      do_op:
        ...
        if (tracefp) {
            ...
            js_Disassemble1(cx, script, pc,
                            (uintN)PTRDIFF(pc, script->code, jsbytecode), JS_FALSE,
                            tracefp);
        ...
        switch (op) {
          ...
          BEGIN_CASE(JSOP_ENTERWITH)
            ...
            withobj = js_NewWithObject(cx, obj, obj2, (jsint)(sp - fp->spbase - 1));
            if (!withobj) {
                ok = JS_FALSE;
                goto out;
            }
          ...
          inline_return:
            {
                ...
                if (JS_LIKELY(cx->version == lwrrentVersion)) {
                    lwrrentVersion = ifp->callerVersion;
                    if (lwrrentVersion != cx->version)
                        js_SetVersion(cx, lwrrentVersion);
                }
                ...
                *vp = fp->rval;
                ...
                /* Resume exelwtion in the calling frame. */
                inlineCallCount--;
                if (JS_LIKELY(ok)) {
 ...
 * compiling with JS_HAS_XML_SUPPORT.  Otherwise colwert the index value to a
 * string atom id.
 */
    ...
    JS_END_MACRO
          ...
          do_forinloop:
            ...
            switch (op) {
              ...
              case JSOP_FORVAR:
                slot = GET_VARNO(pc);
                JS_ASSERT(slot < fp->fun->u.i.lwars);
                fp->vars[slot] = rval;
                break;
              ...
              default:
                ...
                /* Colwert lval to a non-null object containing id. */
...
#define NATIVE_GET(cx,obj,pobj,sprop,vp)                                      \
    JS_BEGIN_MACRO                                                            \
        if (SPROP_HAS_STUB_GETTER(sprop)) {                                   \
            ...
            JS_ASSERT((sprop)->slot != SPROP_ILWALID_SLOT ||                  \
                      !SPROP_HAS_STUB_SETTER(sprop));                         \
            *vp = ((sprop)->slot != SPROP_ILWALID_SLOT)                       \
...
#define BINARY_OP(OP)                                                         \
    ...
    JS_END_MACRO
          ...
          do_new:
            ...
            ok = js_IlwokeConstructor(cx, vp, argc);
            if (!ok)
                goto out;
            RESTORE_SP(fp);
            LOAD_BRANCH_CALLBACK(cx);
 ...
 * yet colwerted.  As above, the expression result goes in rtmp, the updated
 * value goes in rval.  Our caller must set vp to point at a GC-rooted jsval
 * in which we home rtmp, to protect it from GC in case the uncolwerted rval
 * is not a number.
 */
#define NONINT_INCREMENT_OP_MIDDLE()                                          \
    ...
    JS_END_MACRO
                ...
                if (cs->format & JOF_POST) {
                     ...
                     * or decrement result, if colwerted to a jsdouble from
    ...
    *vp = rval;                                                               \
    ...
    goto end_nonint_fast_incop
          ...
          BEGIN_CASE(JSOP_INCVAR)
            FAST_INCREMENT_OP(GET_VARNO(pc), u.i.lwars, vars, rval, +=, MAX);
          BEGIN_CASE(JSOP_DECVAR)
            FAST_INCREMENT_OP(GET_VARNO(pc), u.i.lwars, vars, rval, -=, MIN);
          BEGIN_CASE(JSOP_VARINC)
            FAST_INCREMENT_OP(GET_VARNO(pc), u.i.lwars, vars, rtmp, +=, MAX);
          BEGIN_CASE(JSOP_VARDEC)
            FAST_INCREMENT_OP(GET_VARNO(pc), u.i.lwars, vars, rtmp, -=, MIN);
 ...
 * fetch individual actual parameters.  But if such a use were then ilwoked,
...
#define LAZY_ARGS_THISP ((JSObject *) JSVAL_VOID)
          ...
          /* inline_call: */
            {
                uintN nframeslots, lwars, nslots, missing;
                JSArena *a;
                ...
                /* Restrict relwrsion of lightweight functions. */
                ...
                script = fun->u.i.script;
                depth = (jsint) script->depth;
                nslots = nframeslots + lwars + 2 * depth;
                ...
                /* Switch to new version if lwrrentVersion wasn't overridden. */
                newifp->callerVersion = static_cast<JSVersion>(cx->version);
                if (JS_LIKELY(cx->version == lwrrentVersion)) {
                    lwrrentVersion = static_cast<JSVersion>(script->version);
                    if (lwrrentVersion != cx->version)
                        js_SetVersion(cx, lwrrentVersion);
          ...
          BEGIN_LITOPX_CASE(JSOP_REGEXP, 0)
          {
             ...
             * at exelwtion time from the global with which it was scanned at
             ...
             * literal objects, we don't want to pay a script prolog exelwtion
             * price for all regexp literals in a script (many may not be used
             * by a particular exelwtion of that script, depending on control
            ...
            if (JSVAL_IS_NULL(rval)) {
                 ...
                 * objects and separate compilation and exelwtion, even though
          ...
          BEGIN_CASE(JSOP_TABLESWITCH)
            ...
            /*
             * ECMAv2+ forbids colwersion of discriminant, so we will skip to
...
#undef SEARCH_EXTENDED_PAIRS
          ...
          BEGIN_CASE(JSOP_ARGSUB)
            ...
            if (!obj) {
                 ...
                 * is ilwoked expects its 'this' parameter to be the caller's
                 * arguments object.
                 */
                obj = LAZY_ARGS_THISP;
            }
...
#undef LAZY_ARGS_THISP
          ...
          BEGIN_LITOPX_CASE(JSOP_DEFFUN, 0)
             ...
             * amortizing it over a number of exelwtions.  Examples include XUL
             ...
             * exelwting a script that defined and called functions scoped by
             ...
             * of property if exelwting with the strict option.  We check
          ...
          BEGIN_LITOPX_CASE(JSOP_DEFLOCALFUN, VARNO_LEN)
            ...
            if (!(fp->flags & JSFRAME_POP_BLOCKS)) {
                 ...
                 *
                 * In typical exelwtion scenarios, the prolog bytecodes that
          ...
          BEGIN_CASE(JSOP_ENDINIT)
            ...
            lval = FETCH_OPND(-1);
            JS_ASSERT(JSVAL_IS_OBJECT(lval));
            cx->weakRoots.newborn[GCX_OBJECT] = static_cast<JSGCThing*>(JSVAL_TO_GCTHING(lval));
          ...
          BEGIN_CASE(JSOP_GOSUB)
            ...
            PUSH(lval);
            i = (jsint)PTRDIFF(pc, script->main, jsbytecode) + JSOP_GOSUB_LENGTH;
            len = GET_JUMP_OFFSET(pc);
            PUSH(INT_TO_JSVAL(i));
          END_VARLEN_CASE
          ...
          BEGIN_CASE(JSOP_GOSUBX)
            ...
            PUSH(lval);
            i = (jsint)PTRDIFF(pc, script->main, jsbytecode) + JSOP_GOSUBX_LENGTH;
            len = GET_JUMPX_OFFSET(pc);
            PUSH(INT_TO_JSVAL(i));
          END_VARLEN_CASE
          ...
          BEGIN_LITOPX_CASE(JSOP_ENTERBLOCK, 0)
             ...
             * parent and aclwmulating redundant clones via js_GetScopeChain,
...
out:
    if (!ok) {
         ...
         *    }
         *
         * The inner interpreter activation exelwting the predicate bytecode
         ...
         * leading to double catch exelwtion if it rethrows.
         *
...
no_catch:;
     ...
     * Restore the previous frame's exelwtion state.
     */
...
out2:
    if (cx->version == lwrrentVersion && lwrrentVersion != originalVersion)
