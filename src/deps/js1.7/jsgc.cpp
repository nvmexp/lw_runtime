 ...
 * allocation for things+flags there are always 8 conselwtive 1K-pages each
 ...
 * and callwlate the address for the flag byte as
...
/* Callwlate table capacity based on the current value of JSPtrTable.count. */
static size_t
...
#define ULSTAT(x)   UL(rt->gcStats.x)
    ...
    fprintf(fp, "       mark relwrsion depth: %lu\n", ULSTAT(depth));
    fprintf(fp, "     maximum mark relwrsion: %lu\n", ULSTAT(maxdepth));
    fprintf(fp, "     mark C relwrsion depth: %lu\n", ULSTAT(cdepth));
    fprintf(fp, "   maximum mark C relwrsion: %lu\n", ULSTAT(maxcdepth));
...
js_AddRootRT(JSRuntime *rt, void *rp, const char *name)
{
    ...
    if (rt->gcRunning && rt->gcThread->id != js_LwrrentThreadId()) {
        do {
            JS_AWAIT_GC_DONE(rt);
        } while (rt->gcLevel > 0);
    }
...
js_root_printer(JSDHashTable *table, JSDHashEntryHdr *hdr, uint32 i, void *arg)
{
    ...
    fprintf(stderr,
            ...
            rhe->name ? (const char *)rhe->name : "", rhe->root);
                 ...
                 void *arg)
{
    ...
    return static_cast<JSDHashOperator>(op);
}
...
/*
 * Check if we should delay exelwtion of the close hook.
 *
 ...
 * must not close any generators that came from a page that is lwrrently in
 ...
 */
static void
FindAndMarkObjectsToClose(JSContext *cx, JSGCIlwocationKind gckind,
                          JSGenerator **todoQueueTail)
{
    ...
    while ((gen = *genp) != NULL) {
        if (*js_GetGCThingFlags(gen->obj) & GCF_MARK) {
            ...
            /* Generator must not be exelwting when it becomes unreachable. */
            JS_ASSERT(gen->state == JSGEN_NEWBORN ||
    ...
    if (gckind == GC_LAST_CONTEXT) {
         ...
         * we do not allow exelwtion of arbitrary scripts at this point.
         */
        rt->gcCloseState.todoQueue = NULL;
    } else {
        /*
...
js_RunCloseHooks(JSContext *cx)
{
    ...
    JSGenerator **genp, *gen;
    JSBool ok, defer;
#if defined(JS_GCMETER) && JS_GCMETER
    uint32 deferCount;
#endif
     ...
     * worst case just delays the exlwtion of close hooks.
     */
     ...
     * close hooks and then triggers GC we ignore relwrsive ilwocations of
    ...
    *GC_RUNNING_CLOSE_HOOKS_PTR(cx) = JS_TRUE;
    ...
    while ((gen = *genp) != NULL) {
        ...
        *genp = gen->next;
        ...
        if (cx->throwing) {
             ...
             * other quit ASAP condition. Break exelwtion until the next
    ...
    if (tempList.head) {
        /*
         * Some close hooks were not yet exelwted, put them back into the
         * scheduled list.
         */
        while ((gen = *genp) != NULL) {
            genp = &gen->next;
...
js_NewGCThing(JSContext *cx, uintN flags, size_t nbytes)
{
    ...
    flindex = (uintN)GC_FREELIST_INDEX(nbytes);
    ...
    /* Force a GC cycle if the number of bytes from malloc has hit the max. */
    if (doGC)
        rt->gcPoke = JS_TRUE;
#endif
...
js_LockGCThingRT(JSRuntime *rt, void *thing)
{
    ...
    if (lock || deep) {
        ...
        if (!lhe->thing) {
            lhe->thing = static_cast<JSGCThing *>(thing);
            lhe->count = deep ? 1 : 2;
        } else {
...
#define JS_PROTO(name,idname,code,init) \
...
#undef JS_PROTO
        ...
        lwal = ID_TO_VALUE(sprop->id);
        if (JSVAL_IS_INT(lwal)) {
            JS_snprintf(buf, bufsize, "%ld", (long)JSVAL_TO_INT(lwal));
        } else if (JSVAL_IS_STRING(lwal)) {
            JS_snprintf(buf, bufsize, "%s",
                        JS_GetStringBytes(JSVAL_TO_STRING(lwal)));
        } else {
...
{
    GCMarkNode *prev = (GCMarkNode *)cx->gcLwrrentMarkNode;
...
js_MarkNamedGCThing(JSContext *cx, void *thing, const char *name)
{
    ...
    markNode.name  = name;
    markNode.next  = NULL;
    markNode.prev  = (GCMarkNode *)cx->gcLwrrentMarkNode;
    if (markNode.prev)
        markNode.prev->next = &markNode;
    cx->gcLwrrentMarkNode = &markNode;
    ...
    cx->gcLwrrentMarkNode = markNode.prev;
}
...
js_MarkAtom(JSContext *cx, JSAtom *atom)
{
    ...
    if (atom->flags & ATOM_HIDDEN)
        js_MarkAtom(cx, static_cast<JSAtom *>(atom->entry.value));
}
                    ...
                    JSBool shouldCheckRelwrsion)
{
     ...
     * uses the non-relwrsive code that otherwise would be called only on
     * a low C stack condition.
     */
#ifdef JS_GC_ASSUME_LOW_C_STACK
# define RELWRSION_TOO_DEEP() shouldCheckRelwrsion
#else
    int stackDummy;
# define RELWRSION_TOO_DEEP() (shouldCheckRelwrsion &&                        \
    ...
    switch (*flagp & GCF_TYPEMASK) {
      case GCX_OBJECT:
        ...
        for (; vp != end; ++vp) {
            ...
            if (thing) {
                ...
                *flagp |= GCF_MARK;
                ...
                if (*next_flagp & GCF_MARK) {
                    /*
                     * This happens when relwrsive MarkGCThingChildren marks
        ...
        if (thing) {
             ...
             * can optimize tail relwrsion.
             *
             * Since we already know that there is enough C stack space,
             * we clear shouldCheckRelwrsion to avoid extra checking in
             * RELWRSION_TOO_DEEP.
             */
            shouldCheckRelwrsion = JS_FALSE;
            goto on_tail_relwrsion;
        }
        break;
      ...
      case GCX_MUTABLE_STRING:
        ...
        strcpy(name, "base");
#endif
        /* Fallthrough to code to deal with the tail relwrsion. */

      on_tail_relwrsion:
#ifdef GC_MARK_DEBUG
        /*
         * Do not eliminate C relwrsion when debugging to allow
        ...
        break;
#else
        /* Eliminate tail relwrsion for the last unmarked child. */
        JS_ASSERT(*flagp != GCF_FINAL);
 ...
 * thingsPerUnscannedChunk callwlation when thingSize is a power of two.
 */
...
AddThingToUnscannedBag(JSRuntime *rt, void *thing, uint8 *flagp)
{
    ...
    *flagp |= GCF_FINAL;
...
#ifdef DEBUG
    ++rt->glwnscannedBagSize;
    METER(if (rt->glwnscannedBagSize > rt->gcStats.maxunscanned)
              rt->gcStats.maxunscanned = rt->glwnscannedBagSize);
#endif
    ...
    if (pi->unscannedBitmap != 0) {
        JS_ASSERT(rt->glwnscannedArenaStackTop);
        ...
        if (arena->unscannedPages != 0) {
            ...
            if (!arena->prevUnscanned) {
                if (!rt->glwnscannedArenaStackTop) {
                    /* Stack was empty, mark the arena as bottom element. */
                    arena->prevUnscanned = arena;
                } else {
                    JS_ASSERT(rt->glwnscannedArenaStackTop->prevUnscanned);
                    arena->prevUnscanned = rt->glwnscannedArenaStackTop;
                }
                rt->glwnscannedArenaStackTop = arena;
            }
         }
     }
    JS_ASSERT(rt->glwnscannedArenaStackTop);
...
ScanDelayedChildren(JSContext *cx)
{
    ...
    rt = cx->runtime;
    arena = rt->glwnscannedArenaStackTop;
    if (!arena) {
        JS_ASSERT(rt->glwnscannedBagSize == 0);
        return;
    }
    ...
    for (;;) {
        ...
        while (arena->unscannedPages != 0) {
            ...
            for (; thingOffset != thingLimit; thingOffset += thingSize) {
                 ...
                 * pi.
                 */
                thing = reinterpret_cast<JSGCThing *>(((jsuword)pi + thingOffset));
                flagp = js_GetGCThingFlags(thing);
                if (thingsPerUnscannedChunk != 1) {
                    /*
...
# endif
         ...
         */
        if (arena == rt->glwnscannedArenaStackTop) {
            ...
            rt->glwnscannedArenaStackTop = arena = prevArena;
        } else {
            arena = rt->glwnscannedArenaStackTop;
        }
    ...
    JS_ASSERT(rt->glwnscannedArenaStackTop);
    JS_ASSERT(!rt->glwnscannedArenaStackTop->prevUnscanned);
    rt->glwnscannedArenaStackTop = NULL;
    JS_ASSERT(rt->glwnscannedBagSize == 0);
}
...
gc_root_marker(JSDHashTable *table, JSDHashEntryHdr *hdr, uint32 num, void *arg)
{
    ...
    if (!JSVAL_IS_NULL(v) && JSVAL_IS_GCTHING(v)) {
        ...
        if (!root_points_to_gcArenaList && rhe->name) {
            fprintf(stderr,
"JS API usage error: the address passed to JS_AddNamedRoot lwrrently holds an\n"
...
gc_lock_marker(JSDHashTable *table, JSDHashEntryHdr *hdr, uint32 num, void *arg)
{
    ...
    void *thing = static_cast<void *>(const_cast<JSGCThing *>(lhe->thing));
    JSContext *cx = (JSContext *)arg;
...
js_MarkStackFrame(JSContext *cx, JSStackFrame *fp)
{
    ...
    if (fp->vars)
        GC_MARK_JSVALS(cx, fp->lwars, fp->vars, "var");
 ...
 */
void
js_GC(JSContext *cx, JSGCIlwocationKind gckind)
{
    JSRuntime *rt;
    JSBool keepAtoms;
    uintN i, type;
    ...
    JS_PROFILE_ZONE
    JS_PROFILE_FRAME_START("GC")
#if defined(TRACY_ENABLE) && defined(__cplusplus)
    struct EndGCFrame
    {
        ~EndGCFrame()
        {
            JS_PROFILE_FRAME_END("GC")
        }
    };
    EndGCFrame markFrameEndOnReturn;
#endif

  restart_after_callback:
    /*
     * Let the API user decide to defer a GC if it wants to (unless this
     * is the last context).  Ilwoke the callback regardless.
     */
    if (rt->gcCallback &&
    ...
    /* Lock out other GC allocator and collector ilwocations. */
    if (gckind != GC_LAST_DITCH)
        JS_LOCK_GC(rt);

    /* Do nothing if no mutator has exelwted since the last GC. */
    if (!rt->gcPoke) {
...
#ifdef JS_THREADSAFE
    JS_ASSERT(cx->thread->id == js_LwrrentThreadId());
...
restart:
    ...
    while ((acx = js_ContextIterator(rt, JS_TRUE, &iter)) != NULL) {
         ...
         *
         * (NB: see comment on this whole "dormant" thing in js_Exelwte.)
         */
        chain = acx->fp;
        if (chain) {
    ...
    /*
     * Mark children of things that caused too deep relwrsion during above
     * marking phase.
     */
    ScanDelayedChildren(cx);
    ...
    /*
     * Mark children of things that caused too deep relwrsion during the
  ...
  { extern void DumpSrcNoteSizeHist();
    ...
    /*
     * We want to restart GC if js_GC was called relwrsively or if any of the
...
#ifdef JS_THREADSAFE
    /* If we were ilwoked during a request, pay back the temporary debit. */
    if (requestDebit)
        rt->requestCount += requestDebit;
    rt->gcThread = NULL;
...
#endif
    bytes = *pbytes;
    *pbytes = ((uint32)-1 - bytes <= nbytes) ? (uint32)-1 : bytes + (uint32)nbytes;
}
