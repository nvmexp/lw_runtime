...
Load(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (!script) {
            ...
            ok = !compileOnly
                 ? JS_ExelwteScript(cx, obj, script, &result)
                 : JS_TRUE;
...
Quit(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    JS_ColwertArguments(cx, argc, argv,"/ i", &gExitCode);
...
static JSBool
ColwertArgs(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    JSBool b = JS_FALSE;
    jschar c = 0;
    int32 i = 0, j = 0;
    ...
    if (!JS_AddArgumentFormatter(cx, "ZZ", ZZ_formatter))
        return JS_FALSE;;
    ok = JS_ColwertArguments(cx, argc, argv, "b/ciujdIsSWofvZZ*",
...
Seal(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (!JS_ColwertArguments(cx, argc, argv, "o/b", &target, &deep))
        return JS_FALSE;
    if (!target)
        return JS_TRUE;
              ...
              jsval *rval)
{
    ...
    sobj = NULL;
    if (!JS_ColwertArguments(cx, argc, argv, "S / o", &str, &sobj))
        return JS_FALSE;

    scx = JS_NewContext(JS_GetRuntime(cx), gStackChunkSize, NULL, NULL, NULL, NULL);
    if (!scx) {
        JS_ReportOutOfMemory(cx);
        return JS_FALSE;
    }
...
static char *shell_help_messages[] = {
    ...
    "trap([fun, [pc,]] exp) Trap bytecode exelwtion",
...
#endif
#ifdef TEST_CVTARGS
    "cvtargs(b, c, ...)     Test JS_ColwertArguments",
#endif
...
its_item(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    *rval = OBJECT_TO_JSVAL(obj);
    if (argc != 0)
        JS_SetCallReturlwalue2(cx, argv[0]);
    return JS_TRUE;
}
               ...
               jsval *rval)
{
    ...
    if (!JS_ColwertArguments(cx, argc, argv, "so", &name, &method))
        return JS_FALSE;
...
SendSourceToJSDebugger(const char *filename, uintN lineno,
                       ...
                       void **listenerTSData, void* pvJsdc)
{
    JSDSourceText *jsdsrc = (JSDSourceText *) *listenerTSData;
    JSDContext    *jsdc = (JSDContext *) pvJsdc;
...
static JSBool
its_colwert(JSContext *cx, JSObject *obj, JSType type, jsval *vp)
{
    if (its_noisy)
        fprintf(gOutFile, "colwerting it to %s type\n", JS_GetTypeName(cx, type));
    return JS_TRUE;
}
               ...
               jsval *rval)
{
    ...
    if (!JS_ColwertArguments(cx, argc, argv, "Sv/bbb",
...
Evaluate(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (!JS_ColwertArguments(cx, argc, argv, "S/su",
...
snarf(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    /* Get the lwrrently exelwting script's name. */
    fp = JS_GetScriptedCaller(cx, NULL);
...
JSBool JS_StartModsDebugger(JSRuntime *rt, JSContext *cx, JSObject *glob)
{
    JSObject *it;
