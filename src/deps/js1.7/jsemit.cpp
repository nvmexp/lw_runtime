                     ...
                     JSPrincipals *principals)
{
    ...
    cg->firstLine = cg->prolog.lwrrentLine = cg->main.lwrrentLine = lineno;
...
EmitCheck(JSContext *cx, JSCodeGenerator *cg, JSOp op, ptrdiff_t delta)
{
    ...
    if (next + delta > limit) {
        ...
        length = (length <= BYTECODE_CHUNK)
                 ? BYTECODE_CHUNK
                 : JS_BIT(JS_CeilingLog2((JSUint32)length));
...
/**
  ...
  relwrsion, but with some tricky optimizations to its height-balancing code
...
OptimizeSpanDeps(JSContext *cx, JSCodeGenerator *cg)
{
    ...
    if (growth) {
         ...
         * even though lwrrently it must be JUMPX_OFFSET_LEN - JUMP_OFFSET_LEN
        ...
        for (sn = cg->main.notes, snlimit = sn + cg->main.noteCount;
            ...
            if (spec->isSpanDep) {
                ...
                for (i = 0; i < n; i++) {
                    ...
                    span *= spec->isSpanDep;
                    noteIndex = (uintN)(sn - cg->main.notes);
        ...
        for (tn = cg->tryBase, tnlimit = cg->tryNext; tn < tnlimit; tn++) {
             ...
             */
            length = tn->length;
            sd2 = FindNearestSpanDep(cg, offset + length, (int)(sd - sdbase), &guard);
            if (sd2 != sd)
            ...
            if (offset != 0) {
                sd = FindNearestSpanDep(cg, offset, (int)(sd2 - sdbase), &guard);
...
#ifdef DEBUG_brendan
  {
    ...
    for (sd = sdbase; sd < sdlimit; sd++) {
        ...
        /* NB: sd->top lwrsors into the original, unextended bytecode vector. */
        if (sd->top != top) {
            JS_ASSERT(top == -1 ||
     ...
     */
    size = SPANDEPS_SIZE((size_t)JS_BIT(JS_CeilingLog2(cg->numSpanDeps)));
                      ...
                      JSOp *returnop)
{
     ...
     * when exelwting the finally clause.
     *
        ...
        if (obj == fp->scopeChain) {
            ...
            if (ok) {
                if (pobj == obj &&
                    ...
                    /*
                     * We're compiling code that will be exelwted immediately,
                     * not re-exelwted against a different scope chain and/or
...
/*
 * Allocate an index ilwariant for all activations of the code being compiled
 ...
 * and exelwtion happen indivisibly, but in this implementation and many of
 * its embeddings, code is precompiled early and re-exelwted in multiple
 ...
 * XXXbe hey, who checks for fun->lwars and fun->nargs overflow?!
 */
 ...
 * whether optimization oclwrred, in which case BindNameToSlot also updated
 ...
 *
 * The caller should initialize *answer to false and ilwoke this function on
 ...
 * the value left on the stack after the code exelwtes, will be discarded by a
 * pop bytecode.
 */
static JSBool
    ...
    switch (pn->pn_arity) {
      ...
      case PN_LIST:
        if (pn->pn_type == TOK_NEW ||
             ...
             * to avoid too much relwrsion.  All such lists must be presumed
             * to be useful because each index operation could ilwoke a getter
             * (the JSOP_ARGUMENTS special case below, in the PN_BINARY case,
             * does not apply here: arguments[i][j] might ilwoke a getter).
             *
      ...
      case PN_BINARY:
        if (pn->pn_type == TOK_ASSIGN) {
             ...
             * declared in the function lwrrently being compiled.
             */
            pn2 = pn->pn_left;
      ...
      case PN_NAME:
        ...
        if (pn->pn_type == TOK_NAME && pn->pn_op != JSOP_NOP) {
            ...
            if (pn->pn_slot < 0 && pn->pn_op != JSOP_ARGUMENTS) {
                 ...
                 * could ilwoke a getter that has side effects.
                 */
                *answer = JS_TRUE;
            }
        }
...
EmitPropOp(JSContext *cx, JSParseNode *pn, JSOp op, JSCodeGenerator *cg)
{
     ...
     * bottom up (reversing again as we go), to avoid excessive relwrsion.
     */
    if (pn2->pn_type == TOK_DOT) {
        ...
        do {
            ...
            if (js_NewSrcNote2(cx, cg, static_cast<JSSrcNoteType>(SrcNoteForPropOp(pndot, static_cast<JSOp>(pndot->pn_op))),
                               CG_OFFSET(cg) - pndown->pn_offset) < 0) {
                return JS_FALSE;
            }
            if (!EmitAtomOp(cx, pndot, static_cast<JSOp>(pndot->pn_op), cg))
                return JS_FALSE;
    ...
    if (js_NewSrcNote2(cx, cg, static_cast<JSSrcNoteType>(SrcNoteForPropOp(pn, op)),
...
EmitElemOp(JSContext *cx, JSParseNode *pn, JSOp op, JSCodeGenerator *cg)
{
    ...
    if (pn->pn_arity == PN_LIST) {
        /* Left-associative operator chain to avoid too much relwrsion. */
...
#define INTMAP_LENGTH   256
     ...
     * so they can be colwerted to span dependencies with null targets to
    ...
    if (switchOp == JSOP_CONDSWITCH) {
        ...
        /*
         * After this point, all control flow ilwolving JSOP_TABLESWITCH
...
#define UPDATE_LINE_NUMBER_NOTES(cx, cg, pn)                                  \
    JS_BEGIN_MACRO                                                            \
        ...
        uintN delta_ = line_ - CG_LWRRENT_LINE(cg);                           \
        if (delta_ != 0) {                                                    \
            /*                                                                \
                     ...
                     JSBool wantpop)
{
    ...
    if (pn->pn_type == TOK_RB || pn->pn_type == TOK_RC) {
        ...
        switch (pn->pn_op) {
          ...
          case JSOP_SETGVAR:
            slot = (jsuint) pn->pn_slot;
            EMIT_UINT16_IMM_OP(static_cast<JSOp>(pn->pn_op), slot);
            if (wantpop && js_Emit1(cx, cg, JSOP_POP) < 0)
                return JS_FALSE;
            break;
...
/*
 * Relwrsive helper for EmitDestructuringOps.
 *
...
EmitDestructuringOpsHelper(JSContext *cx, JSCodeGenerator *cg, JSParseNode *pn)
{
    ...
    for (pn2 = pn->pn_head; pn2; pn2 = pn2->pn_next) {
        ...
        /*
         * Now push the property name lwrrently being matched, which is either
                     ...
                     JSParseNode *pn)
{
    ...
    /*
     * Call our relwrsive helper to emit the destructuring assignments and
              ...
              JSBool inLetHead, ptrdiff_t *headNoteIndex)
{
    ...
    *headNoteIndex = -1;
    ...
    forIlwar = (pn->pn_extra & PNX_FORILWAR) != 0;
#if JS_HAS_BLOCK_SCOPE
    forInLet = let && forIlwar;
    ...
    for (pn2 = pn->pn_head; ; pn2 = pn2->pn_next) {
        ...
        if (pn2->pn_type != TOK_NAME) {
            if (pn2->pn_type == TOK_RB || pn2->pn_type == TOK_RC) {
                ...
                if (!EmitDestructuringDecls(cx, cg, static_cast<JSOp>(pn->pn_op), pn2))
                    return JS_FALSE;
                break;
            }
            ...
            if (pn->pn_count == 1 && !forInLet) {
                ...
                if (!MaybeEmitGroupAssignment(cx, cg,
                                              inLetHead ? JSOP_POP : static_cast<JSOp>(pn->pn_op),
                                              pn2, &op)) {
                    return JS_FALSE;
                }
                if (op == JSOP_NOP) {
            ...
            pn3 = pn2->pn_left;
            if (!EmitDestructuringDecls(cx, cg, static_cast<JSOp>(pn->pn_op), pn3))
                return JS_FALSE;
        ...
        if (op == JSOP_ARGUMENTS) {
            ...
            if (!MaybeEmitVarDecl(cx, cg, static_cast<JSOp>(pn->pn_op), pn2, &atomIndex))
                return JS_FALSE;
    ...
    switch (pn->pn_type) {
      case TOK_FUNCTION:
      {
        ...
        if (pn->pn_op != JSOP_NOP) {
            EMIT_ATOM_INDEX_OP(static_cast<JSOp>(pn->pn_op), atomIndex);
            break;
        }
      ...
      case TOK_IF:
        /* Initialize so we can detect else-if chains and avoid relwrsion. */
      ...
      if_again:
        ...
        if (stmtInfo.type == STMT_IF) {
            ...
            if (!js_SetSrcNoteOffset(cx, cg, (uintN)noteIndex, 0, jmp - beq))
                return JS_FALSE;
            if (!js_SetSrcNoteOffset(cx, cg, (uintN)noteIndex, 1, top - jmp))
                return JS_FALSE;
        }
      ...
      case TOK_DO:
         ...
         * offset, and fetching that delta in order to decompile relwrsively.
         */
      ...
      case TOK_FOR:
        ...
        if (pn2->pn_type == TOK_IN) {
             ...
             */
            pn3 = pn2->pn_left;
            type = static_cast<JSTokenType>(pn3->pn_type);
            cg->treeContext.flags |= TCF_IN_FOR_INIT;
            ...
            /*
             * Emit a bytecode to colwert top of stack value to the iterator
...
#endif
            if (js_Emit1(cx, cg, static_cast<JSOp>(pn->pn_op)) < 0)
                return JS_FALSE;
            ...
            switch (type) {
              ...
              case TOK_VAR:
                ...
                if (pn3->pn_type == TOK_RB || pn3->pn_type == TOK_RC) {
                    op = static_cast<JSOp>(pn2->pn_left->pn_op);
        ...
        if (pn2->pn_type != TOK_IN) {
            ...
            if (pn3) {
                ...
                if (CG_LWRRENT_LINE(cg) != (uintN) off) {
                    if (js_NewSrcNote2(cx, cg, SRC_SETLINE, off) < 0)
                        return JS_FALSE;
                    CG_LWRRENT_LINE(cg) = (uintN) off;
                }
            }
      ...
      case TOK_TRY:
      {
        ...
        if (lastCatch && lastCatch->pn_kid2) {
            ...
            /*
             * Rethrow the exception, delegating exelwting of finally if any
      ...
      case TOK_CATCH:
      {
        ...
        if (pn->pn_kid2) {
            if (!js_EmitTree(cx, cg, pn->pn_kid2))
                return JS_FALSE;
            if (!js_SetSrcNoteOffset(cx, cg, (uintN)CATCHNOTE(*stmt), 0,
      ...
      case TOK_SEMI:
        ...
        if (pn2) {
            /*
             * Top-level or called-from-a-native JS_Exelwte/EvaluateScript,
            ...
            if (!useful &&
                ...
                CG_LWRRENT_LINE(cg) = pn2->pn_pos.begin.lineno;
      ...
      case TOK_ASSIGN:
        ...
        if (pn2->pn_type != TOK_NAME &&
            ...
            pn2->pn_type != TOK_RC &&
#endif
            js_NewSrcNote2(cx, cg, static_cast<JSSrcNoteType>(SrcNoteForPropOp(pn2, static_cast<JSOp>(pn2->pn_op))),
                           CG_OFFSET(cg) - top) < 0) {
            return JS_FALSE;
        }
        ...
        switch (pn2->pn_type) {
          case TOK_NAME:
            if (pn2->pn_slot < 0 || !(pn2->pn_attrs & JSPROP_READONLY)) {
                if (pn2->pn_slot >= 0) {
                    EMIT_UINT16_IMM_OP(static_cast<JSOp>(pn2->pn_op), atomIndex);
                } else {
          case TOK_DOT:
                    EMIT_ATOM_INDEX_OP(static_cast<JSOp>(pn2->pn_op), atomIndex);
                }
            }
            break;
          case TOK_LB:
      ...
      case TOK_HOOK:
         ...
         * ignore the value pushed by the first branch.  Exelwtion will follow
      ...
      case TOK_AND:
        /*
         * JSOP_OR colwerts the operand on the stack to boolean, and if true,
         ...
         *
         * JSOP_AND colwerts the operand on the stack to boolean, and if false,
         * leaves the original operand value on the stack and jumps; otherwise
         * it pops and falls into the right operand's bytecode.
         *
         * Avoid tail relwrsion for long ||...|| expressions and long &&...&&
      ...
      case TOK_DIVOP:
        if (pn->pn_arity == PN_LIST) {
            /* Left-associative operator chain: avoid too much relwrsion. */
            pn2 = pn->pn_head;
            if (!js_EmitTree(cx, cg, pn2))
                return JS_FALSE;
            op = static_cast<JSOp>(pn->pn_op);
      ...
      case TOK_DBLCOLON:
            if (pn->pn_arity == PN_NAME) {
                if (!js_EmitTree(cx, cg, pn->pn_expr))
                    return JS_FALSE;
                if (!EmitAtomOp(cx, pn, static_cast<JSOp>(pn->pn_op), cg))
                    return JS_FALSE;
                break;
            }
      ...
      case TOK_DEC:
      {
         ...
         * colwerts to a jsdouble.  In the TOK_NAME cases, op has 0 operand
      ...
      case TOK_DELETE:
        ...
        switch (pn2->pn_type) {
          ...
          default:
            /*
             * If useless, just emit JSOP_TRUE; otherwise colwert delete foo()
      ...
      case TOK_DOT:
        /*
         * Pop a stack operand, colwert it to object, get a property named by
         ...
         * "obj" register to the left operand's ToObject colwersion result,
         * for use by JSOP_PUSHOBJ.
         */
        ok = EmitPropOp(cx, pn, static_cast<JSOp>(pn->pn_op), cg);
        break;
      ...
      case TOK_DBLDOT:
#endif
        /*
         * Pop two operands, colwert the left one to object and the right one
         ...
         * on the left operand.
         */
        ok = EmitElemOp(cx, pn, static_cast<JSOp>(pn->pn_op), cg);
        break;
      ...
      case TOK_LP:
      {
        ...
        argc = pn->pn_count - 1;
        if (js_Emit3(cx, cg, static_cast<JSOp>(pn->pn_op), ARGC_HI(argc), ARGC_LO(argc)) < 0)
            return JS_FALSE;
        break;
      }
      ...
      case TOK_LEXICALSCOPE:
      {
         ...
         */
        noteIndex = -1;
        type = static_cast<JSTokenType>(pn->pn_expr->pn_type);
      ...
      case TOK_LET:
       ...
       case TOK_ARRAYPUSH:
        ...
        if (!js_EmitTree(cx, cg, pn->pn_kid))
            return JS_FALSE;
        EMIT_UINT16_IMM_OP(static_cast<JSOp>(pn->pn_op), cg->arrayCompSlot);
        break;
#endif
      ...
      case TOK_OBJECT:
         ...
         */
        ok = EmitAtomOp(cx, pn, static_cast<JSOp>(pn->pn_op), cg);
        break;
      ...
      case TOK_XMLETAGO:
      {
        ...
        if ((pn->pn_extra & PNX_XMLROOT) && js_Emit1(cx, cg, static_cast<JSOp>(pn->pn_op)) < 0)
            return JS_FALSE;
        break;
      }
                    ...
                    uintN which, ptrdiff_t offset)
{
    ...
    if (offset > (ptrdiff_t)SN_3BYTE_OFFSET_MASK) {
        ...
        if (!(*sn & SN_3BYTE_OFFSET_FLAG)) {
            ...
            index = (uintN)PTRDIFF(sn, CG_NOTES(cg), jssrcnote);
...
js_FinishTakingSrcNotes(JSContext *cx, JSCodeGenerator *cg, jssrcnote *notes)
{
    ...
    if (prologCount && cg->prolog.lwrrentLine != cg->firstLine) {
        CG_SWITCH_TO_PROLOG(cg);
...
js_FinishTakingTryNotes(JSContext *cx, JSCodeGenerator *cg, JSTryNote *notes)
{
    ...
    count = (uintN)PTRDIFF(cg->tryNext, cg->tryBase, JSTryNote);
    if (!count)
        return;
