...
CreateScopeTable(JSContext *cx, JSScope *scope, JSBool report)
{
    ...
    scope->table = (JSScopeProperty **)
        calloc((size_t)JS_BIT(sizeLog2), sizeof(JSScopeProperty *));
    if (!scope->table) {
        if (report)
            JS_ReportOutOfMemory(cx);
        return JS_FALSE;
    }
    js_UpdateMallocCounter(cx, (size_t)JS_BIT(sizeLog2) * sizeof(JSScopeProperty *));
...
js_HashScopeProperty(JSDHashTable *table, const void *key)
{
    ...
    hash = (JSDHashNumber)((hash >> (JS_DHASH_BITS - 4)) ^ (hash << 4)
    ...
    hash = (JSDHashNumber)((hash >> (JS_DHASH_BITS - 4)) ^ (hash << 4) ^ sprop->id);
    return hash;
}
...
NewPropTreeKidsChunk(JSRuntime *rt)
{
    ...
    chunk = static_cast<PropTreeKidsChunk *>(calloc(1, sizeof *chunk));
    if (!chunk)
        return NULL;
                    ...
                    uintN attrs, uintN flags, intN shortid)
{
    ...
    JSAddPropHook hook = cx->runtime->addPropHook;
    if (hook) {
        hook(cx, scope, id, getter, setter, slot, attrs, flags, shortid,
             cx->runtime->addPropHookData);
    }
    ...
    if (!sprop) {
        ...
        if (!(flags & SPROP_IS_ALIAS)) {
            if (attrs & JSPROP_SHARED) {
                 ...
                 * Callers should therefore pass SPROP_ILWALID_SLOT for all
                            ...
                            JSPropertyOp getter, JSPropertyOp setter)
{
    ...
    if (SCOPE_LAST_PROP(scope) == sprop) {
        ...
        if ((sprop->attrs & JSPROP_SHARED) && !(attrs & JSPROP_SHARED)) {
            JS_ASSERT(child.slot == SPROP_ILWALID_SLOT);
...
js_RemoveScopeProperty(JSContext *cx, JSScope *scope, jsid id)
{
    ...
    /* Colwert from a list to a hash so we can handle "middle deletes". */
...
js_SweepScopeProperties(JSRuntime *rt)
{
    ...
    while ((a = *ap) != NULL) {
        ...
        for (sprop = (JSScopeProperty *) a->base; sprop < limit; sprop++) {
             ...
             * RemovePropertyTreeChild never colwerts a single-entry chunky
