...
#define JS_KEYWORD(keyword, idname, type, op, version) \
    const char js_##keyword##_str[] = idname;
...
GrowTokenBuf(JSStringBuffer *sb, size_t newlength)
{
    ...
    cx = static_cast<JSContext *>(sb->data);
    base = sb->base;
...
js_NewBufferTokenStream(JSContext *cx, const jschar *base, size_t length)
{
    ...
    ts->userbuf.base = const_cast<jschar *>(base);
    ts->userbuf.limit = const_cast<jschar *>(base + length);
    ts->userbuf.ptr = const_cast<jschar *>(base);
...
js_CloseTokenStream(JSContext *cx, JSTokenStream *ts)
{
    if (ts->flags & TSF_OWNFILENAME)
        JS_free(cx, const_cast<char *>(ts->filename));
    if (ts->principals)
    ...
    if (ts->ungetpos != 0) {
        ...
        do {
            if (ts->linebuf.ptr == ts->linebuf.limit) {
                ...
                if (len <= 0) {
                    if (!ts->file) {
                        // Send the last line to the debugger (no need to
                        // decrement the line number since it never gets
                        // incremented past the last line)
                        if (ts->listener && ts->linelen) 
                        {
                            ts->listener(ts->filename, ts->lineno, 
                                         ts->linebuf.ptr - ts->linelen, 
                                         ts->linelen,
                                         &ts->listenerTSData, 
                                         ts->listenerData);
                        }
                ...
                if (ts->listener) {
                    ...
                    {
                        // Lines are sent after the fact to the debugger so it is
                        // necessary to decrement the line number when sending data
                        // to the debugger
                        ts->listener(ts->filename, ts->lineno - 1, 
                                     ts->linebuf.ptr - ts->linelen, ts->linelen,
                                     &ts->listenerTSData, 
                                     ts->listenerData);
                    }
                }
                         ...
                         JSBool charArgs, va_list ap)
{
    ...
    switch (flags & JSREPORT_HANDLE) {
      case JSREPORT_TS:
        ts = static_cast<JSTokenStream *>(handle);
        break;
      case JSREPORT_CG:
        cg = static_cast<JSCodeGenerator *>(handle);
        break;
      case JSREPORT_PN:
        pn = static_cast<JSParseNode *>(handle);
        ts = pn->pn_ts;
        break;
    }
    ...
    do {
        if (ts) {
            ...
            if (tp->begin.lineno == tp->end.lineno) {
                ...
                index = (uintN)(tp->begin.index - ts->linepos);
            }
        ...
        if (cg) {
            ...
            report->lineno = CG_LWRRENT_LINE(cg);
            break;
        }
     ...
     * number, set that as the pending exception.  For errors oclwring at
...
GrowStringBuffer(JSStringBuffer *sb, size_t newlength)
{
    ...
    if ((size_t)offset < newlength && newlength < ~(size_t)0 / sizeof(jschar))
        bp = static_cast<jschar *>(realloc(sb->base, newlength * sizeof(jschar)));
    else
        bp = NULL;
    if (!bp) {
        free(sb->base);
...
js_PeekTokenSameLine(JSContext *cx, JSTokenStream *ts)
{
    ...
    if (!ON_LWRRENT_LINE(ts, LWRRENT_TOKEN(ts).pos))
        return TOK_EOL;
...
NewToken(JSTokenStream *ts, ptrdiff_t adjust)
{
    ...
    tp->pos.begin.index = (uint16)(ts->linepos +
...
retry:
    ...
    switch (c) {
      ...
      case '<':
         ...
         * you send") ilwites a natural-law reperlwssion for JS as "protocol":
         *
      ...
      case '>':
        if (MatchChar(ts, c)) {
            tp->t_op = MatchChar(ts, c) ? JSOP_URSH : JSOP_URSH;
            tt = MatchChar(ts, '=') ? TOK_ASSIGN : TOK_SHOP;
        } else {
...
skipline:
        ...
        if (ts->flags & TSF_OPERAND) {
             ...
             * sharing among threads and sequentially across re-exelwtion,
             * select JSOP_REGEXP.
             */
...
eol_out:
    ...
    tp->pos.end.index = (uint16)(ts->linepos +
