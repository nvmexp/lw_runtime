 ...
 * that calling a standard colwersion routine might allow strings such as
        ...
        if (oldlen - newlen < (1 << 24)) {
            ...
            for (;;) {
                ...
                if (id2 == JSVAL_VOID)
                    break;
                jsval idJsval;
                JS_IdToValue(cx, id2, &idJsval);
                if (js_IdIsIndex(idJsval, &index) && index - newlen < gap) {
...
static JSBool
array_colwert(JSContext *cx, JSObject *obj, JSType type, jsval *vp)
{
...
JSClass js_ArrayClass = {
    ...
    JS_EnumerateStub,  JS_ResolveStub,    array_colwert,     JS_FinalizeStub,
               ...
               JSString *sep, jsval *rval)
{
    ...
    JSAtom *atom;
    int stackDummy;
    JSArrayJoinHook hook;

    hook = cx->runtime->arrayJoinHook;
    ...
    if (op == TO_SOURCE) {
        ...
        if (sep) {
            if (hook) {
                hook(cx, sep, ARRAY_JOIN_SEPARATOR, cx->runtime->arrayJoinHookData);
            }
    ...
    /* Use rval to locally root each element value as we loop and colwert. */
#define v (*rval)
    ...
    for (index = 0; index < length; index++) {
        ...
        if (hook) {
            hook(cx, str, ARRAY_JOIN_APPEND_STR, cx->runtime->arrayJoinHookData);
        }
        js_strncpy(&chars[nchars], JSSTRING_CHARS(str), tmplen);
        nchars += tmplen;
        if (hook) {
            hook(cx, str, ARRAY_JOIN_FINISH_STR, cx->runtime->arrayJoinHookData);
        }
    ...
    if (hook) {
        hook(cx, sep, ARRAY_JOIN_FINISH_SEP, cx->runtime->arrayJoinHookData);
    }
...
#undef v
  ...
  make_string:
    ...
    if (hook) {
        hook(cx, str, ARRAY_JOIN_RESULT, cx->runtime->arrayJoinHookData);
    }
...
array_sort(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        all_strings &= (JSBool)JSVAL_IS_STRING(vec[newlen]);
...
array_splice(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    /* Colwert the first argument into a starting index. */
    ...
    /* Colwert the second argument from a count into a fencepost index. */
    delta = length - begin;
    if (argc == 0) {
        count = delta;
        end = length;
            ...
            ArrayExtraMode mode)
{
    ...
    for (i = 0; i < length; i++) {
         ...
         * iteration around the loop since js_Ilwoke uses origsp[0] for rval
        ...
        *sp++ = OBJECT_TO_JSVAL(obj);
        ...
        /* Do the call. */
        fp->sp = sp;
        ok = js_Ilwoke(cx, 3, JSILWOKE_INTERNAL);
        vp[1] = fp->sp[-1];
        fp->sp = oldsp;
        if (!ok)
            break;
...
static JSFunctionSpec array_methods[] = {
    ...
    /* Perl-ish methods. */
    {MKJSID("join"),                array_join,             1,JSFUN_GENERIC_NATIVE,0},
    {MKJSID("reverse"),             array_reverse,          0,JSFUN_GENERIC_NATIVE,2},
    {MKJSID("sort"),                array_sort,             1,JSFUN_GENERIC_NATIVE,2},
    {MKJSID("push"),                array_push,             1,JSFUN_GENERIC_NATIVE,0},
    {MKJSID("pop"),                 array_pop,              0,JSFUN_GENERIC_NATIVE,0},
    {MKJSID("shift"),               array_shift,            0,JSFUN_GENERIC_NATIVE,1},
    {MKJSID("unshift"),             array_unshift,          1,JSFUN_GENERIC_NATIVE,1},
    {MKJSID("splice"),              array_splice,           2,JSFUN_GENERIC_NATIVE,1},

    /* Python-esque sequence methods. */
    {MKJSID("concat"),              array_concat,           1,JSFUN_GENERIC_NATIVE,1},
    {MKJSID("slice"),               array_slice,            2,JSFUN_GENERIC_NATIVE,1},

#if JS_HAS_ARRAY_EXTRAS
    {MKJSID("indexOf"),             array_indexOf,          1,JSFUN_GENERIC_NATIVE,0},
    {MKJSID("lastIndexOf"),         array_lastIndexOf,      1,JSFUN_GENERIC_NATIVE,0},
    {MKJSID("forEach"),             array_forEach,          1,JSFUN_GENERIC_NATIVE,2},
    {MKJSID("map"),                 array_map,              1,JSFUN_GENERIC_NATIVE,2},
    {MKJSID("filter"),              array_filter,           1,JSFUN_GENERIC_NATIVE,2},
    {MKJSID("some"),                array_some,             1,JSFUN_GENERIC_NATIVE,2},
    {MKJSID("every"),               array_every,            1,JSFUN_GENERIC_NATIVE,2},
#endif
