...
/*
** Fill a number. The order is: optional-sign zero-filling colwersion-digits
*/
...
/*
** Colwert a long into its printable form
*/
                 ...
                 int type, int flags, const char *hexp)
{
    ...
    /*
    ** Colwerting decimal is a little tricky. In the unsigned case we
    ...
    */
    ...
    /*
    ** Now that we have the number colwerted without its sign, deal with
...
/*
** Colwert a 64-bit integer into its printable form
*/
...
/*
** Colwert a double precision floating point number into its printable
** form.
**
** XXX stop using sprintf to colwert floating point
*/
static int cvt_f(SprintfState *ss, double d, const char *fmt0, const char *fmt1)
{
    char fin[20];
    char fout[300];
    int amount = (int)(fmt1 - fmt0);
    ...
    /* Colwert floating point using the native sprintf code */
#ifdef DEBUG
    {
        const char *p = fin;
        while (*p) {
    ...
    */
    ...
    return (*ss->stuff)(ss, fout, (JSUint32)strlen(fout));
}

/*
** Colwert a string into its printable form.  "width" is the output
        ...
        */
        ...
        switch (c) {
...
#ifdef JS_C_STRINGS_ARE_UTF8
                u.wch = va_arg(ap, int);
                utf8len = js_OneUcs4ToUtf8Char(reinterpret_cast<uint8 *>(utf8buf), u.wch);
          ...
          case 's':
            if(type == TYPE_INT16) {
                /*
                 * This would do a simple string/byte colwersion
...
static int GrowStuff(SprintfState *ss, const char *sp, JSUint32 len)
{
    ...
    JS_ASSERT((JSUint32)(ss->lwr - ss->base) <= ss->maxlen);
    return 0;
}
...
{
    JSUint32 limit = (JSUint32)(ss->maxlen - (ss->lwr - ss->base));
                                  ...
                                  va_list ap)
{
    ...
    if( (ss.lwr != ss.base) && (ss.lwr[-1] != '\0') )
        ss.lwr[-1] = '\0';

    n = (JSUint32)(ss.lwr - ss.base);
    return n ? n - 1 : n;
}
