...
JS_InitArenaPool(JSArenaPool *pool, const char *name, size_t size, size_t align)
{
    ...
    pool->mask = JS_BITMASK(JS_CeilingLog2((JSUint32)align));
...
#if defined(__arm__)
    // WAR a bug with GCC 4.6.1 where using realloc causes crashes
    // in the JS engine.
    // Lwrrently the cause is unknown, it is either a GCC bug, glibc bug
    // or JS engine bug. If I was to bet, I would bet on a glibc bug.
    b = (JSArena *) malloc(gross);
    if (!b)
        return NULL;
    memcpy(b, a, (gross > a->limit-(jsuword)a) ? a->limit-(jsuword)a : gross);
    free(a);
    a = b;
#else
...
JS_ArenaFreeAllocation(JSArenaPool *pool, void *p, size_t size)
{
     ...
     * at it.  We must preserve LIFO order of mark/release lwrsors, so we use
