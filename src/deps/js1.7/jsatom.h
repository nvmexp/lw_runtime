...
struct JSAtom {
    ...
    bool IsObject() const { return GetKey().IsObject(); }
    bool IsInt() const { return GetKey().IsInt(); }
    bool IsDouble() const { return GetKey().IsDouble(); }
    bool IsString() const { return GetKey().IsString(); }
    bool IsBoolean() const { return GetKey().IsBoolean(); }

    JSObject * ToObject() const { return GetKey().ToObject(); }
    jsint ToInt() const { return GetKey().ToInt(); }
    jsdouble * ToDouble() const { return GetKey().ToDouble(); }
    JSString * ToString() const { return GetKey().ToString(); }
    JSBool ToBoolean() const { return GetKey().ToBoolean(); }
};

#define ATOM_KEY(atom)        (atom)->GetKey()
#define ATOM_IS_OBJECT(atom)  (atom)->IsObject()
#define ATOM_TO_OBJECT(atom)  (atom)->ToObject()
#define ATOM_IS_INT(atom)     (atom)->IsInt()
#define ATOM_TO_INT(atom)     (atom)->ToInt()
#define ATOM_IS_DOUBLE(atom)  (atom)->IsDouble()
#define ATOM_TO_DOUBLE(atom)  (atom)->ToDouble()
#define ATOM_IS_STRING(atom)  (atom)->IsString()
#define ATOM_TO_STRING(atom)  (atom)->ToString()
#define ATOM_IS_BOOLEAN(atom) (atom)->IsBoolean()
#define ATOM_TO_BOOLEAN(atom) (atom)->ToBoolean()
...
#define ALE_ATOM(ale)   ((const JSAtom *) (ale)->entry.key)
#define ALE_INDEX(ale)  ((jsatomid) JS_PTR_TO_UINT32((ale)->entry.value))
#define ALE_JSOP(ale)   (static_cast<JSOp>(reinterpret_cast<uintptr_t>((ale)->entry.value)))
...
// Giving an atom _atom and an atom list _al, this macro assigns _ale a
// pointer to the atom list element that contains this atom. Also it assigns
// _hep a pointer to *JSHashEntry.
...
#define JS_PROTO(name,idname,code,init) extern const char js_##name##_str[];
#include "jsproto.tbl"
#undef JS_PROTO
...
extern const char   js_ExelwtionContext_str[];
extern const char   js_lwrrent_str[];
#endif
 ...
 * sequence is lwrrently not atomized.
 */
extern JSAtom *
...
/*
 * Colwert v to an atomized string.
 */
extern JSAtom *
