...
struct JSCodeGenerator {
    ...
    struct {
        ...
        uintN       lwrrentLine;    /* line number for tree-based srcnote gen */
    } prolog, main, *current;
    ...
    uintN           emitLevel;      /* js_EmitTree relwrsion level */
...
#define CG_LWRRENT_LINE(cg)     ((cg)->current->lwrrentLine)
 ...
 * time.  It returns true unless an error oclwrred.
 *
 ...
 */
#define CG_COUNT_FINAL_SRCNOTES(cg, cnt, cnttype)                                    \
    JS_BEGIN_MACRO                                                                   \
        ptrdiff_t diff_ = CG_PROLOG_OFFSET(cg) - (cg)->prolog.lastNoteOffset;        \
        cnt = (cnttype)((cg)->prolog.noteCount + (cg)->main.noteCount + 1);          \
        if ((cg)->prolog.noteCount &&                                                \
            (cg)->prolog.lwrrentLine != (cg)->firstLine) {                           \
            if (diff_ > SN_DELTA_MASK)                                               \
                cnt += (cnttype)(JS_HOWMANY(diff_ - SN_DELTA_MASK, SN_XDELTA_MASK)); \
            cnt += (cnttype)(2 + (((cg)->firstLine > SN_3BYTE_OFFSET_MASK) << 1));   \
        } else if (diff_ > 0) {                                                      \
            ...
            if (diff_ > 0)                                                           \
                cnt += (cnttype)(JS_HOWMANY(diff_, SN_XDELTA_MASK));                 \
        }                                                                            \
    JS_END_MACRO
 ...
 */
#define CG_COUNT_FINAL_TRYNOTES(cg, cnt, cnttype)                \
    JS_BEGIN_MACRO                                               \
        cnt = (cnttype)(((cg)->tryNext > (cg)->tryBase)          \
