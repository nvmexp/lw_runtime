                    ...
                    ...)
{
    ...
    va_start(ap, format);
    ok = JS_ColwertArgumentsVA(cx, argc, argv, format, ap);
    va_end(ap);
    return ok;
}
...
JS_BeginRequest(JSContext *cx)
{
    ...
    if (!cx->requestDepth) {
        ...
        /* NB: we use cx->thread here, not js_GetLwrrentThread(). */
...
{
    return js_NewContext(rt, stackChunkSize, hashFun, hashFunData, compareAtomKeys, compareAtomKeysData);
}
...
{
    return static_cast<JSVersion>(cx->version & JSVERSION_MASK);
}
...
JS_SetVersion(JSContext *cx, JSVersion version)
{
    ...
    oldVersion = static_cast<JSVersion>(cx->version & JSVERSION_MASK);
                        ...
                        JSBool *resolved)
{
    ...
    *resolved = JS_FALSE;
    ...
    if (stdnm) {
         ...
         *
         * If ilwersely, either id does not name a class, or id does not name
...
#ifdef JS_THREADSAFE
    JS_ASSERT(cx->runtime->gcThread->id == js_LwrrentThreadId());
#endif
...
JS_GC(JSContext *cx)
{
    ...
    /* Don't nuke active arenas if exelwting or compiling. */
...
JS_MaybeGC(JSContext *cx)
{
     ...
     * cell density lwrrently and Sl and Fl are the size and the density
    ...
    if ((bytes > 8192 && bytes > lastBytes + lastBytes / 5) ||
        ...
        /* Run scheduled but not yet exelwted close hooks. */
        js_RunCloseHooks(cx);
    }
#endif
#endif
...
JS_Enumerate(JSContext *cx, JSObject *obj)
{
    ...
    n = JSVAL_TO_INT(jsval(num_properties));
    if (n <= 0)
        n = 8;
...
static JSClass prop_iter_class = {
    ...
    JS_EnumerateStub, JS_ResolveStub,  JS_ColwertStub,  prop_iter_finalize,
    NULL,             NULL,            NULL,            NULL,
...
JS_ExelwteScript(JSContext *cx, JSObject *obj, JSScript *script, jsval *rval)
{
    ...
    CHECK_REQUEST(cx);
    ok = js_Exelwte(cx, obj, script, NULL, 0, rval);
                     ...
                     JSExecPart part, jsval *rval)
{
    ...
    if (part == JSEXEC_PROLOG) {
        tmp.length = (uint32)PTRDIFF(tmp.main, tmp.code, jsbytecode);
    } else {
        tmp.length -= (uint32)PTRDIFF(tmp.main, tmp.code, jsbytecode);
        tmp.code = tmp.main;
    }
    ...
    ok = JS_ExelwteScript(cx, obj, &tmp, rval);
                  ...
                  jsval *rval)
{
    ...
    if (!chars)
        return JS_FALSE;
    ok = JS_EvaluateUCScript(cx, obj, chars, (uintN)length, filename, lineno, rval);
    JS_free(cx, chars);
    return ok;
}
                               ...
                               jsval *rval)
{
    ...
    if (!chars)
        return JS_FALSE;
    ok = JS_EvaluateUCScriptForPrincipals(cx, obj, principals, chars, (uintN)length,
...
JS_GetStringChars(JSString *str)
{
     ...
     * If it fails to colwert a dependent string into an independent one, our
     ...
     *
     * The gain of dependent strings, which lwre quadratic and cubic growth
...
JS_ReportPendingException(JSContext *cx)
{
     ...
     * colwersion done by all {js,JS}_Report* functions except for OOM.  The
     * cx->creatingException flag was added to suppress relwrsive divergence
