...
js_Disassemble(JSContext *cx, JSScript *script, JSBool lines, FILE *fp)
{
    ...
    while (pc < end) {
        ...
        len = js_Disassemble1(cx, script, pc,
                              (uintN)PTRDIFF(pc, script->code, jsbytecode),
                              lines, fp);
        if (!len)
            return JS_FALSE;
        pc += len;
                ...
                JSBool lines, FILE *fp)
{
    ...
    switch (type) {
      ...
      case JOF_JUMPX:
        ...
        fprintf(fp, " %u (%d)", (unsigned int)(loc + off), (int)off);
        break;
      ...
      case JOF_TABLESWITCHX:
      {
        ...
        pc2 += JUMP_OFFSET_LEN;
        fprintf(fp, " defaultOffset %d low %d high %d", (int)off, (int)low, (int)high);
        for (i = low; i <= high; i++) {
            off = GetJumpOffset(pc, pc2);
            fprintf(fp, "\n\t%d: %d", (int)i, (int)off);
            pc2 += jmplen;
      ...
      case JOF_LOOKUPSWITCHX:
      {
        ...
        pc2 += ATOM_INDEX_LEN;
        fprintf(fp, " offset %d npairs %u", (int)off, (uintN) npairs);
        while (npairs) {
...
typedef struct Sprinter {
    JSContext       *context;       /* context exelwting the decompiler */
...
js_printf(JSPrinter *jp, const char *format, ...)
{
    ...
    fp = NULL;
    if (!jp->pretty && format[cc = (int)strlen(format) - 1] == '\n') {
    ...
    /* Allocate temp space, colwert format, and put. */
    bp = JS_vsmprintf(format, ap);      /* XXX vsaprintf */
    if (fp) {
...
GetOff(SprintStack *ss, uintN i)
{
    ...
    if (off < 0) {
        ...
        str = js_DecompileValueGenerator(ss->sprinter.context, (intN)off,
                                         JSVAL_NULL, NULL);
        if (!str)
            return 0;
...
PushOff(SprintStack *ss, ptrdiff_t off, JSOp op)
{
    ...
    ss->offsets[top] = off;
    ss->opcodes[top] = ((int)op == JSOP_GETPROP2) ? JSOP_GETPROP
                     : ((int)op == JSOP_GETELEM2) ? JSOP_GETELEM
                     : (jsbytecode) op;
    ss->top = ++top;
                ...
                ptrdiff_t defaultOffset, JSBool isCondSwitch)
{
    ...
    if (tableLength) {
        ...
        if (diff > 0) {
            ...
            js_printf(jp, "\t%s:\n", js_default_str);
            jp->indent += 2;
            if (!Decompile(ss, pc + defaultOffset, (intN)diff))
                return JS_FALSE;
            jp->indent -= 4;
        }
        ...
        for (i = 0; i < tableLength; i++) {
            ...
            if (isCondSwitch) {
                ...
                if (!Decompile(ss, pc + caseExprOff,
                               (intN)(nextCaseExprOff - caseExprOff))) {
                    return JS_FALSE;
                }
            ...
            if (off <= defaultOffset && defaultOffset < off2) {
                ...
                if (diff != 0) {
                    if (!Decompile(ss, pc + off, (intN)diff))
                        return JS_FALSE;
                    off = defaultOffset;
                }
                jp->indent -= 2;
            ...
            if (!Decompile(ss, pc + off, (intN)(off2 - off)))
                return JS_FALSE;
            jp->indent -= 4;
...
#define LOCAL_ASSERT_RV_PTR(expr, rv, type) \
    JS_BEGIN_MACRO                          \
        JS_ASSERT(expr);                    \
        if (!(expr)) return (type *)(rv);   \
    JS_END_MACRO
...
#define LOCAL_ASSERT(expr)  LOCAL_ASSERT_RV(expr, (jsbytecode *)NULL)
#define LOAD_OP_DATA(pc)    (oplen = (cs = &js_CodeSpec[op = static_cast<JSOp>(*pc)])->length)
...
DecompileDestructuring(SprintStack *ss, jsbytecode *pc, jsbytecode *endpc)
{
     ...
     * chars so the destructuring decompilation aclwmulates contiguously in
...
#define LOCAL_ASSERT_NULL_PTR(expr, type) LOCAL_ASSERT_RV_PTR(expr, 0, type)
        ...
        if (pc + oplen == jp->dvgfence) {
            ...
            if (((fp && pc == fp->pc) ||
                ...
                if (mode == JOF_NAME) {
                     ...
                     */
                    if (mode == JOF_PROP) {
                        op = static_cast<JSOp>((format & JOF_SET) ? JSOP_GETPROP2 : JSOP_GETPROP);
                    } else if (mode == JOF_ELEM) {
                        op = static_cast<JSOp>((format & JOF_SET) ? JSOP_GETELEM2 : JSOP_GETELEM);
                    } else {
                        /*
...
#endif
                          default:
                            LOCAL_ASSERT_NULL_PTR(0, jsbytecode);
                        }
                    }
                }
            }
            ...
            LOCAL_ASSERT_NULL_PTR(js_CodeSpec[saveop].length == oplen, jsbytecode);
            ...
            switch (cs->nuses) {
              case 2:
                ...
                if (sn && SN_TYPE(sn) == SRC_ASSIGNOP) {
                     ...
                     * problem).
                     */
                    op = static_cast<JSOp>(pc[oplen]);
                    LOCAL_ASSERT_NULL_PTR(op != saveop, jsbytecode);
                }
                rval = POP_STR();
                lval = POP_STR();
                if (op != saveop) {
                    ...
                    LOCAL_ASSERT_NULL_PTR(op == JSOP_ADD, jsbytecode);
...
#define END_LITOPX_CASE                                                       \
              ...
              case JSOP_NOP:
                ...
                switch (sn ? SN_TYPE(sn) : SRC_NULL) {
                  ...
                  do_forloop:
                    ...
                    LOCAL_ASSERT_NULL_PTR(tail + GetJumpOffset(pc+tail, pc+tail) == 0, jsbytecode);
                    ...
                    if (pc[next] != JSOP_GOTO && pc[next] != JSOP_GOTOX) {
                        ...
                        DECOMPILE_CODE(pc + next, (intN)(tail - next - 1));
                        js_printf(jp, " %s", POP_STR());
                    }
                    ...
                    DECOMPILE_CODE(pc + cond + oplen, (intN)(next - cond - oplen));
                    jp->indent -= 4;
                    js_printf(jp, "\t}\n");
                  ...
                  case SRC_FUNCDEF:
                    atom = js_GetAtom(cx, &jp->script->atomMap,
                                      (jsatomid) js_GetSrcNoteOffset(sn, 0));
                    LOCAL_ASSERT_NULL_PTR(ATOM_IS_OBJECT(atom), jsbytecode);
                  do_function:
                    obj = ATOM_TO_OBJECT(atom);
                  ...
                  case SRC_BRACE:
                    ...
                    DECOMPILE_CODE(pc + oplen, (intN)(len - oplen));
                    jp->indent -= 4;
                    js_printf(jp, "\t}\n");
                    break;
              ...
              case JSOP_GROUP:
                ...
                if ((cs->prec != 0 &&
                     ...
                     * its thing.
                     */
                    LOCAL_ASSERT_NULL_PTR(ss->top != 0, jsbytecode);
              ...
              case JSOP_PUSH:
                ...
                if (sn && SN_TYPE(sn) == SRC_GROUPASSIGN) {
                    ...
                    if (!pc)
                        return NULL;
                    LOCAL_ASSERT_NULL_PTR(*pc == JSOP_SETSP, jsbytecode);
              ...
              case JSOP_RETSUB:
                rval = POP_STR();
                LOCAL_ASSERT_NULL_PTR(strcmp(rval, retsub_pc_cookie) == 0, jsbytecode);
                lval = POP_STR();
                LOCAL_ASSERT_NULL_PTR(strcmp(lval, exception_cookie) == 0, jsbytecode);
                todo = -2;
                break;

              case JSOP_SWAP:
                /*
                 * We don't generate this opcode lwrrently, and previously we
              ...
              case JSOP_GOSUBX:
                 ...
                 * the stack balanced by a JSOP_RETSUB exelwted elsewhere.
                 */
                todo = -2;
                break;
              ...
              case JSOP_SETSP:
              {
                ...
                oldtop = ss->top;
                LOCAL_ASSERT_NULL_PTR(newtop <= oldtop, jsbytecode);
                todo = -2;
                        ...
                        if (sn) {
                            ...
                            if (SN_TYPE(sn) == SRC_DECL) {
                                if (ss->top == jp->script->depth) {
                                    ...
                                    pc = pc2 + 1;
                                    len = js_GetSrcNoteOffset(sn, 0);
                                    LOCAL_ASSERT_NULL_PTR(pc[len] == JSOP_LEAVEBLOCK, jsbytecode);
                                    js_printf(jp, "\tlet (%s) {\n", rval);
                                    js_printf(jp, "\t}\n");
                                    goto end_setsp;
                                }
              ...
              case JSOP_EXCEPTION:
                ...
                LOCAL_ASSERT_NULL_PTR(JS_FALSE, jsbytecode);
                break;
              ...
              case JSOP_POPV:
                ...
                switch (sn ? SN_TYPE(sn) : SRC_NULL) {
                  ...
                  case SRC_PCDELTA:
                    ...
                    if (!Decompile(ss, done, (intN)(pc - done))) {
                        JS_free(cx, const_cast<char *>(lval));
                        return NULL;
                    }
                  ...
                  do_letheadbody:
                    ...
                    if (pc[len] == JSOP_LEAVEBLOCK) {
                        js_printf(CLEAR_MAYBE_BRACE(jp), "\tlet (%s) {\n",
                                  POP_STR());
                        jp->indent += 4;
                        DECOMPILE_CODE(pc, (intN)len);
                        jp->indent -= 4;
                        js_printf(jp, "\t}\n");
                        todo = -2;
                    } else {
                        LOCAL_ASSERT_NULL_PTR(pc[len] == JSOP_LEAVEBLOCKEXPR, jsbytecode);

                        lval = JS_strdup(cx, POP_STR());
                        if (!lval)
                            return NULL;

                        if (!Decompile(ss, pc, (intN)len)) {
                            JS_free(cx, const_cast<char *>(lval));
                            return NULL;
                        }
                        rval = POP_STR();
              ...
              case JSOP_ENTERWITH:
                LOCAL_ASSERT_NULL_PTR(!js_GetSrcNote(jp->script, pc), jsbytecode);
                rval = POP_STR();
              ...
              case JSOP_LEAVEWITH:
                ...
                if (sn && SN_TYPE(sn) == SRC_HIDDEN)
                    break;
                rval = POP_STR();
                LOCAL_ASSERT_NULL_PTR(strcmp(rval, with_cookie) == 0, jsbytecode);
                jp->indent -= 4;
                js_printf(jp, "\t}\n");
                break;
              ...
              BEGIN_LITOPX_CASE(JSOP_ENTERBLOCK)
              {
                ...
                for (sprop = OBJ_SCOPE(obj)->lastProp; sprop;
                    ...
                    if (!(sprop->flags & SPROP_HAS_SHORTID))
                        continue;
                    LOCAL_ASSERT_NULL_PTR(sprop->shortid < argc, jsbytecode);
                ...
                switch (sn ? SN_TYPE(sn) : SRC_NULL) {
                  ...
                  case SRC_BRACE:
                    ...
                    jp->indent += 4;
                    len = js_GetSrcNoteOffset(sn, 0);
                    ok = Decompile(ss, pc + oplen, (intN)(len - oplen)) != NULL;
                    if (!ok)
                        goto enterblock_out;
                    jp->indent -= 4;
                    js_printf(jp, "\t}\n");
                  ...
                  case SRC_CATCH:
                    ...
                    pc2 = pc;
                    pc += oplen;
                    LOCAL_ASSERT_NULL_PTR(*pc == JSOP_EXCEPTION, jsbytecode);
                    ...
                    if (*pc == JSOP_DUP) {
                        ...
                        if (sn2 && SN_TYPE(sn2) == SRC_HIDDEN) {
                             ...
                             * an exception guard.
                             */
                            LOCAL_ASSERT_NULL_PTR(js_GetSrcNoteOffset(sn, 0) != 0, jsbytecode);
                            pc += JSOP_DUP_LENGTH;
                        }
                    }
#if JS_HAS_DESTRUCTURING
                    if (*pc == JSOP_DUP) {
                        ...
                        LOCAL_ASSERT_NULL_PTR(*pc == JSOP_POP, jsbytecode);
                        pc += JSOP_POP_LENGTH;
                        lval = PopStr(ss, JSOP_NOP);
                        js_puts(jp, lval);
                    } else {
#endif
                        LOCAL_ASSERT_NULL_PTR(*pc == JSOP_SETLOCALPOP, jsbytecode);
                        i = GET_UINT16(pc);
                    ...
                    if (len) {
                        ...
                        LOCAL_ASSERT_NULL_PTR(len > 0, jsbytecode);
                        js_printf(jp, " if ");
                        ok = Decompile(ss, pc, (intN)len) != NULL;
                        if (!ok)
                            goto enterblock_out;
                        js_printf(jp, "%s", POP_STR());
                        pc += len;
                        LOCAL_ASSERT_NULL_PTR(*pc == JSOP_IFEQ || *pc == JSOP_IFEQX, jsbytecode);
              ...
              case JSOP_LEAVEBLOCKEXPR:
              {
                ...
                if (op == JSOP_LEAVEBLOCKEXPR) {
                    LOCAL_ASSERT_NULL_PTR(SN_TYPE(sn) == SRC_PCBASE, jsbytecode);
                    rval = POP_STR();
                } else if (sn) {
                    LOCAL_ASSERT_NULL_PTR(op == JSOP_LEAVEBLOCK, jsbytecode);
                    if (SN_TYPE(sn) == SRC_HIDDEN)
                        break;
                    LOCAL_ASSERT_NULL_PTR(SN_TYPE(sn) == SRC_CATCH, jsbytecode);
                    LOCAL_ASSERT_NULL_PTR((uintN)js_GetSrcNoteOffset(sn, 0) == ss->top, jsbytecode);
                }
                top = ss->top;
                depth = GET_UINT16(pc);
                LOCAL_ASSERT_NULL_PTR(top >= depth, jsbytecode);
                top -= depth;
                ss->top = top;
              ...
              case JSOP_GETLOCAL:
                i = GET_UINT16(pc);
                sn = js_GetSrcNote(jp->script, pc);
                LOCAL_ASSERT_NULL_PTR((uintN)i < ss->top, jsbytecode);
                rval = GetLocal(ss, i);
              ...
              case JSOP_ARRAYPUSH:
              {
                ...
                rval = POP_STR();
                pos = ss->top;
                while ((op = static_cast<JSOp>(ss->opcodes[--pos])) != JSOP_ENTERBLOCK &&
                       op != JSOP_NEWINIT) {
                    LOCAL_ASSERT_NULL_PTR(pos != 0, jsbytecode);
                }
                blockpos = pos;
                ...
                LOCAL_ASSERT_NULL_PTR(ss->opcodes[pos] == JSOP_NEWINIT, jsbytecode);
                startpos = pos;
                start = ss->offsets[pos];
                LOCAL_ASSERT_NULL_PTR(ss->sprinter.base[start] == '[' ||
                                      ss->sprinter.base[start] == '#', jsbytecode);
                pos = blockpos;
                while (ss->opcodes[++pos] == JSOP_STARTITER)
                    LOCAL_ASSERT_NULL_PTR(pos < ss->top, jsbytecode);
                LOCAL_ASSERT_NULL_PTR(pos < ss->top, jsbytecode);
              ...
              if_again:
                ...
                switch (sn ? SN_TYPE(sn) : SRC_NULL) {
                  ...
                  case SRC_IF_ELSE:
                    ...
                    rval = POP_STR();
                    if (ss->inArrayInit) {
                        LOCAL_ASSERT_NULL_PTR(SN_TYPE(sn) == SRC_IF, jsbytecode);
                    ...
                    if (SN_TYPE(sn) == SRC_IF) {
                        ...
                        LOCAL_ASSERT_NULL_PTR(!ss->inArrayInit, jsbytecode);
                        tail = js_GetSrcNoteOffset(sn, 0);
                        DECOMPILE_CODE(pc + oplen, (intN)(tail - oplen));
                        jp->indent -= 4;
                        pc += tail;
                        LOCAL_ASSERT_NULL_PTR(*pc == JSOP_GOTO || *pc == JSOP_GOTOX, jsbytecode);
                        ...
                        if (cond != 0) {
                            DECOMPILE_CODE(pc + oplen, (intN)(cond - oplen));
                            pc += cond;
                            elseif = JS_TRUE;
                            goto if_again;
                        }
                  ...
                  case SRC_COND:
                    ...
                    lval = JS_strdup(cx, POP_STR());
                    if (!lval) {
                        JS_free(cx, const_cast<char *>(xval));
                        return NULL;
                    }
                    pc += len;
                    LOCAL_ASSERT_NULL_PTR(*pc == JSOP_GOTO || *pc == JSOP_GOTOX, jsbytecode);
              ...
              case JSOP_IFNE:
              case JSOP_IFNEX:
                /* Lwrrently, this must be a do-while loop's upward branch. */
                jp->indent -= 4;
                ...
                if (jp->pretty &&
                    ...
                    if (!rval) {
                        ...
                        tail = Sprint(&ss->sprinter, "%*s%s",
                                      jp->indent + 4, "", rval);
                        JS_free(cx, const_cast<char *>(rval));
                    }
                    if (tail < 0)
                        todo = -1;
                } else {
              ...
              case JSOP_FORARG:
                ...
                LOCAL_ASSERT_NULL_PTR(atom, jsbytecode);
              ...
              do_forinhead:
                ...
                if (atom) {
                    ...
                    LOCAL_ASSERT_NULL_PTR(*xval != '\0', jsbytecode);
              ...
              case JSOP_FORELEM:
                 ...
                 * bound the relwrsively decompiled loop body.
                 */
                sn = js_GetSrcNote(jp->script, pc);
                LOCAL_ASSERT_NULL_PTR(!forelem_tail, jsbytecode);
                 ...
                 */
                LOCAL_ASSERT_NULL_PTR(!forelem_done, jsbytecode);
              ...
              case JSOP_ENUMCONSTELEM:
                ...
                op = saveop;
                rval = POP_STR();
                LOCAL_ASSERT_NULL_PTR(strcmp(rval, forelem_cookie) == 0, jsbytecode);
                LOCAL_ASSERT_NULL_PTR(forelem_tail > pc, jsbytecode);
                tail = forelem_tail - pc;
                forelem_tail = NULL;
                LOCAL_ASSERT_NULL_PTR(forelem_done > pc, jsbytecode);
                len = forelem_done - pc;
                forelem_done = NULL;
                goto do_forinhead;
              ...
              case JSOP_DUP2:
                ...
                todo = SprintCString(&ss->sprinter, rval);
                if (todo < 0 || !PushOff(ss, todo, static_cast<JSOp>(ss->opcodes[ss->top-2])))
                    return NULL;
                /* FALL THROUGH */
              ...
              case JSOP_DUP:
                ...
                sn = js_GetSrcNote(jp->script, pc);
                if (sn) {
                    LOCAL_ASSERT_NULL_PTR(SN_TYPE(sn) == SRC_DESTRUCT, jsbytecode);
                    pc = DecompileDestructuring(ss, pc, endpc);
                    if (!pc)
                        return NULL;
                    len = 0;
                ...
                rval = GetStr(ss, ss->top-1);
                saveop = static_cast<JSOp>(ss->opcodes[ss->top-1]);
                todo = SprintCString(&ss->sprinter, rval);
                break;
              ...
              do_setlval:
                ...
                if (op == JSOP_SETLOCALPOP) {
                    if (!PushOff(ss, todo, saveop))
                        return NULL;
                    rval = POP_STR();
                    LOCAL_ASSERT_NULL_PTR(*rval != '\0', jsbytecode);
                    js_printf(jp, "\t%s;\n", rval);
                    todo = -2;
                }
                break;
              ...
              case JSOP_SETCALL:
                ...
                LOCAL_ASSERT_NULL_PTR(ss->top >= 2, jsbytecode);
                (void) PopOff(ss, op);
              ...
              case JSOP_LITOPX:
                atomIndex = GET_LITERAL_INDEX(pc);
                pc2 = pc + 1 + LITERAL_INDEX_LEN;
                op = saveop = static_cast<JSOp>(*pc2);
                ...
                switch (op) {
                  ...
                  default:                LOCAL_ASSERT_NULL_PTR(0, jsbytecode);
                }
                /* NOTREACHED */
                break;
              ...
              case JSOP_TABLESWITCHX:
              {
                ...
                sn = js_GetSrcNote(jp->script, pc);
                LOCAL_ASSERT_NULL_PTR(sn && SN_TYPE(sn) == SRC_SWITCH, jsbytecode);
                len = js_GetSrcNoteOffset(sn, 0);
                ...
                if (n == 0) {
                    ...
                    for (i = j = 0; i < n; i++) {
                        ...
                        if (off2) {
                            sn = js_GetSrcNote(jp->script, pc2);
                            if (sn) {
                                LOCAL_ASSERT_NULL_PTR(SN_TYPE(sn) == SRC_LABEL, jsbytecode);
                                table[j].label =
              ...
              case JSOP_CONDSWITCH:
              {
                ...
                for (ncases = 0; off2 != 0; ncases++) {
                    pc2 += off2;
                    LOCAL_ASSERT_NULL_PTR(*pc2 == JSOP_CASE || *pc2 == JSOP_DEFAULT ||
                                          *pc2 == JSOP_CASEX || *pc2 == JSOP_DEFAULTX, jsbytecode);
                    if (*pc2 == JSOP_DEFAULT || *pc2 == JSOP_DEFAULTX) {
                        /* End of cases, but count default as a case. */
                        off2 = 0;
                    } else {
                        sn = js_GetSrcNote(jp->script, pc2);
                        LOCAL_ASSERT_NULL_PTR(sn && SN_TYPE(sn) == SRC_PCDELTA, jsbytecode);
              ...
              do_initprop:
                ...
                if (lastop == JSOP_GETTER || lastop == JSOP_SETTER) {
                    if (!atom || !ATOM_IS_STRING(atom) ||
                        ...
                        rval += 8 + 1;
                        LOCAL_ASSERT_NULL_PTR(rval[strlen(rval)-1] == '}', jsbytecode);
              ...
              case JSOP_XMLNAME:
              case JSOP_FILTER:
                /* Colwersion and prefix ops do nothing in the decompiler. */
                todo = -2;
                break;
                 ...
                 uintN pcdepth)
{
    ...
    cx = jp->sprinter.context;
    JSValDecompileHook hook = cx->runtime->valDecompileHook;
    void *hookData = cx->runtime->valDecompileHookData;
    ...
    /* Call relwrsive subroutine to do the hard work. */
    oldscript = jp->script;
    jp->script = script;
    if (hook) {
        hook(jp->sprinter.context, script, jp->scope, JS_TRUE, hookData);
    }
    ok = Decompile(&ss, pc, len) != NULL;
    if (hook) {
        hook(jp->sprinter.context, script, jp->scope, JS_FALSE, hookData);
    }
    jp->script = oldscript;
                           ...
                           JSString *fallback)
{
    ...
    if (spindex == JSDVG_SEARCH_STACK) {
         ...
         *
         * We search from limit to base to find the most recently callwlated
    ...
    /*
     * |this| could colwert to a very long object initialiser, so cite it by
    ...
    if ((cs->format & JOF_MODEMASK) != JOF_NAME) {
        ...
        if (!sn)
            goto do_fallback;
        noteType = static_cast<JSSrcNoteType>(SN_TYPE(sn));
