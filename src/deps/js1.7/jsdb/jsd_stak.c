...
jsd_NewThreadState(JSDContext* jsdc, JSContext *cx )
{
    ...
    jsdthreadstate->thread = JSD_LWRRENT_THREAD();
...
jsd_DestroyThreadState(JSDContext* jsdc, JSDThreadState* jsdthreadstate)
{
    ...
    JS_ASSERT(jsdthreadstate);
    JS_ASSERT(JSD_LWRRENT_THREAD() == jsdthreadstate->thread);
                       ...
                       JSDThreadState*    jsdthreadstate)
{
    ...
    for( lwr = (JSDThreadState*)jsdc->threadsStates.next;
         lwr != (JSDThreadState*)&jsdc->threadsStates;
         lwr = (JSDThreadState*)lwr->links.next ) 
    {
        if( lwr == jsdthreadstate )
            return JS_TRUE;
    }
    return JS_FALSE;
}    
