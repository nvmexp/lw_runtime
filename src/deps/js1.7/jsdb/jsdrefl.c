...
JSClass jsdb_HandleClass = {
    ...
    JS_EnumerateStub, JS_ResolveStub,   JS_ColwertStub,   handle_finalize,
...
jsdb_EvalReturnExpression(JSDB_Data* data, jsval* rval)
{
    ...
    if(!JS_GetProperty(cx, data->jsdOb, _str_ReturnExpression, &expressiolwal))
        return JS_FALSE;

    if(!(expressionString = JS_ValueToString(cx, expressiolwal)))
        return JS_FALSE;
...
JS_STATIC_DLL_CALLBACK(JSBool)
SetExelwtionHook(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    jsval oldHook;
    JSDB_Data* data = (JSDB_Data*) JS_GetContextPrivate(cx);
    JS_ASSERT(data);
    ...
    if(argc < 1 ||
       !(JSVAL_IS_NULL(argv[0]) ||
         NULL != JS_ValueToFunction(cx, argv[0])))
    {
        JS_ReportError(cx, "SetExelwtionHook requires a function or null param");
        return JS_FALSE;
    }

    oldHook = data->jsExelwtionHook;
    data->jsExelwtionHook = argv[0];
...
JS_STATIC_DLL_CALLBACK(JSBool)
GetExelwtionHook(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    JSDB_Data* data = (JSDB_Data*) JS_GetContextPrivate(cx);
    JS_ASSERT(data);

    *rval = data->jsExelwtionHook;
    return JS_TRUE;
}
...
SetTrap(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    success = JSD_SetExelwtionHook(data->jsdcTarget, jsdscript, pc,
...
ClearTrap(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    success = JSD_ClearExelwtionHook(data->jsdcTarget, jsdscript, pc);
    *rval = success ? JSVAL_TRUE : JSVAL_FALSE;
    return JS_TRUE;
}
...
ClearAllTrapsForScript(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    success = JSD_ClearAllExelwtionHooksForScript(data->jsdcTarget, jsdscript);
    *rval = success ? JSVAL_TRUE : JSVAL_FALSE;
    return JS_TRUE;
}
...
ClearAllTraps(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    success = JSD_ClearAllExelwtionHooks(data->jsdcTarget);
    *rval = success ? JSVAL_TRUE : JSVAL_FALSE;
    return JS_TRUE;
}
...
/* Exelwtion/Interrupt Hook functions */
    FUN_SPEC(SetExelwtionHook, 1)
    FUN_SPEC(GetExelwtionHook, 0)
