...
class jsdService : public jsdIDebuggerService
{
  ...
  private:
    ...
    nsCOMPtr<jsdIExelwtionHook> mBreakpointHook;
    nsCOMPtr<jsdIExelwtionHook> mDebugHook;
    nsCOMPtr<jsdIExelwtionHook> mDebuggerHook;
    nsCOMPtr<jsdIExelwtionHook> mInterruptHook;
    nsCOMPtr<jsdIScriptHook>    mScriptHook;
    nsCOMPtr<jsdIExelwtionHook> mThrowHook;
...
class jsdThreadState : public jsdIThreadState
{
  public:
     ...
     * instance of this we've created, and "ilwalidate" them before we let the
     * engine continue.  The next time we need a threadstate, we can search the
     * list to find an ilwalidated one, and just reuse it.
     */
