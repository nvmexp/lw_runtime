...
static JHandle*
_constructInteger(ExecElw *ee, long i)
{
    return (JHandle*) 
        exelwte_java_constructor(ee, "java/lang/Integer", 0, "(I)", i);
}

static JHandle*
_putHash(ExecElw *ee, JHandle* tbl, JHandle* key, JHandle* ob)
{
    return (JHandle*) 
        exelwte_java_dynamic_method( 
            ee, tbl, "put",
...
static JHandle*
_getHash(ExecElw *ee, JHandle* tbl, JHandle* key)
{
    return (JHandle*) 
        exelwte_java_dynamic_method( 
            ee, tbl, "get",
...
static JHandle*
_removeHash(ExecElw *ee, JHandle* tbl, JHandle* key)
{
    return (JHandle*) 
        exelwte_java_dynamic_method( 
            ee, tbl, "remove",
...
_constructJSStackFrameInfo( ExecElw* ee, JSDStackFrameInfo* jsdframe, 
                            struct Hnetscape_jsdebug_JSThreadState* threadState )
{
    ...
    frame = (struct Hnetscape_jsdebug_JSStackFrameInfo*)
        exelwte_java_constructor( ee, "netscape/jsdebug/JSStackFrameInfo", 0, 
...
_constructJSPC( ExecElw* ee, struct Hnetscape_jsdebug_Script* script, long pc )
{
    struct Hnetscape_jsdebug_JSPC * pcOb;

    pcOb = (struct Hnetscape_jsdebug_JSPC *)
        exelwte_java_constructor( ee, "netscape/jsdebug/JSPC", 0, 
...
_scriptObFromJSDScriptPtr( ExecElw* ee, JSDScript* jsdscript )
{
             ...
             void*       callerdata )
{
    ...
    if( creating )
    {
        ...
        script = (Hnetscape_jsdebug_Script*)
                exelwte_java_constructor(ee, "netscape/jsdebug/Script", 0, "()");
        ...
        {
            exelwte_java_dynamic_method( ee,(JHandle*)unhand(controller)->scriptHook,
                                         "justLoadedScript",
    ...
    else
    {
        ...
        exelwte_java_dynamic_method( ee,(JHandle*)script,
                                     "_setIlwalid",
                                     "()V" );
    }
}
...
/***************************************************************************/
PRUintn PR_CALLBACK
_exelwtionHook( JSDContext*     jsdc, 
                ...
                void*           callerdata )
{
    ...
    threadState = (struct Hnetscape_jsdebug_JSThreadState*)
            exelwte_java_constructor( ee, "netscape/jsdebug/JSThreadState",0,"()");
    if( ! threadState )
        return JSD_HOOK_RETURN_HOOK_ERROR;
    ...
    unhand(threadState)->lwrrentFramePtr = (long) jsdframe;
...
void netscape_jsdebug_DebugController__setController(struct Hnetscape_jsdebug_DebugController * self,/*boolean*/ long on)
{
    if(on)
    {
        ...
        JSD_SetDebugBreakHook(context, _exelwtionHook, (void*)1 );
    }
    else
    {
        /* XXX stop somehow... */
...
void netscape_jsdebug_DebugController_setInstructionHook0(struct Hnetscape_jsdebug_DebugController * self,struct Hnetscape_jsdebug_PC * pcOb)
{
    ...
    script = (Hnetscape_jsdebug_Script*)
        exelwte_java_dynamic_method(ee, (JHandle*)pcOb, "getScript","()Lnetscape/jsdebug/Script;");
    ...
    pc = (PRUintn)
        exelwte_java_dynamic_method(ee, (JHandle*)pcOb, "getPC","()I");

    JSD_SetExelwtionHook(context, jsdscript, pc, _exelwtionHook, 0);
}
...
void netscape_jsdebug_DebugController_sendInterrupt0(struct Hnetscape_jsdebug_DebugController * self)
{
    if( ! context || ! controller )
        return;
    JSD_SetInterruptHook(context, _exelwtionHook, 0);
}

struct Hjava_lang_String *netscape_jsdebug_DebugController_exelwteScriptInStackFrame0(struct Hnetscape_jsdebug_DebugController *self,struct Hnetscape_jsdebug_JSStackFrameInfo *frame,struct Hjava_lang_String *src,struct Hjava_lang_String *filename,long lineno)
{
...
struct Hnetscape_jsdebug_StackFrameInfo *netscape_jsdebug_JSThreadState_getLwrrentFrame(struct Hnetscape_jsdebug_JSThreadState * self)
{
    JSDThreadState* jsdstate;
    JSDStackFrameInfo* jsdframe;
...
struct Hnetscape_jsdebug_StackFrameInfo *netscape_jsdebug_JSStackFrameInfo_getCaller0(struct Hnetscape_jsdebug_JSStackFrameInfo * self)
{
    ...
    jsdframeLwr = (JSDStackFrameInfo*) unhand(self)->_nativePtr;
    if( ! jsdframeLwr )
        return NULL;
    ...
    jsdframeCaller = JSD_GetCallingStackFrame(context, jsdthreadstate, jsdframeLwr);
    if( ! jsdframeCaller )
        return NULL;
...
struct Hnetscape_jsdebug_SourceLocation *netscape_jsdebug_JSPC_getSourceLocation(struct Hnetscape_jsdebug_JSPC * self)
{
    ...
    return (struct Hnetscape_jsdebug_SourceLocation *)
        exelwte_java_constructor( ee, "netscape/jsdebug/JSSourceLocation", 0, 
...
void netscape_jsdebug_JSSourceTextProvider_refreshSourceTextVector(struct Hnetscape_jsdebug_JSSourceTextProvider * self)
{
    ...
    /* create new vector */
    vec = (JHandle*) exelwte_java_constructor(ee, "netscape/util/Vector", 0, "()");
    if( ! vec )
        return;
    ...
    while( 0 != (item = JSD_IterateSources(context, &iterp)) )
    {
        ...
        itemOb = (JHandle*)
            exelwte_java_dynamic_method( ee, (JHandle*)self, "findSourceTextItem0",
        ...
        if( ! itemOb )
        {
            ...
            itemOb = (JHandle*)
                exelwte_java_constructor(ee, "netscape/jsdebug/SourceTextItem",0,  
                 ...
                 JSD_SOURCE_CLEARED != status )
        {
            ...
            textOb = makeJavaString((char*)str, length);
            exelwte_java_dynamic_method(ee, itemOb, "setText", 
                                        "(Ljava/lang/String;)V", textOb);
            exelwte_java_dynamic_method(ee, itemOb, "setStatus", 
                                        "(I)V", status );
            exelwte_java_dynamic_method(ee, itemOb, "setDirty", "(Z)V", 1 );
        }
        ...
        if( itemOb )
            exelwte_java_dynamic_method(ee, vec, "addElement", 
