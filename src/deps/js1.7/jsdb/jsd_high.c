               ...
               void*              user)
{
    ...
    jsdc->dumbContext = JS_NewContext(jsdc->jsrt, 256, NULL, NULL, NULL, NULL);
    if( ! jsdc->dumbContext )
        goto label_newJSDContext_failure;
                      ...
                      void*              user)
{
    ...
    JS_SetExelwteHook(jsdc->jsrt, jsd_TopLevelCallHook, jsdc);
...
jsd_DebuggerOff(JSDContext* jsdc)
{
    ...
    JS_SetExelwteHook(jsdc->jsrt, NULL, NULL);
...
#ifdef JSD_HAS_DANGEROUS_THREAD
    _dangerousThread = JSD_LWRRENT_THREAD();
#endif
                   ...
                   JSErrorReport *report, void *closure)
{
    ...
    switch(errorReporter(jsdc, cx, message, report, errorReporterData))
    {
        ...
        case JSD_ERROR_REPORTER_DEBUG:
        {
            ...
            jsd_CallExelwtionHook(jsdc, cx, JSD_HOOK_DEBUG_REQUESTED,
                                  hook, hookData, &rval);
            /* XXX Should make this dependent on ExelwtionHook retval */
            return JS_TRUE;
        }
