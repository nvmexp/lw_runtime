                     ...
                     jsval*          rval)
{
    ...
    if(data->jsExelwtionHook &&
       NULL != (fun = JS_ValueToFunction(data->cxDebugger, data->jsExelwtionHook)))
    {
...
Load(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (!script)
            continue;
        ok = JS_ExelwteScript(cx, obj, script, &result);
...
Write(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (!str)
        return JS_FALSE;
    pStr = pNewStr = JS_GetStringBytes(str);
    ...
    if (numPercent) 
    {
        pNewStr = malloc(strlen(pStr) + numPercent + 1);
        pLwrPtr = pNewStr;
        while (*pStr) 
        {
            *pLwrPtr = *pStr++;
...
JSDB_InitDebugger(JSRuntime* rt, JSDContext* jsdc, int depth)
{
    ...
    printf("NewRuntime: %p\n", data->rtDebugger);
    if (!(data->cxDebugger = cx = JS_NewContext(data->rtDebugger, 8192, NULL, NULL, NULL, NULL))) {
        JS_DestroyRuntime(data->rtDebugger);
        free(data);
