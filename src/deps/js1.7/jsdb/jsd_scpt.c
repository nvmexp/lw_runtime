              ...
              JSFunction*  function)
{
...
#else
    jsdscript->app = LWDBG_GetLwrrentApp();    
    if( jsdscript->app && raw_filename )
    {
        ...
        {
            jsdscript->lwscript = LWDBG_GetLwrrentTopLevelScript();
        }
    }
#endif
                  ...
                  JSDScript*   jsdscript)
{
    ...
    /* destroy all hooks */
    jsd_ClearAllExelwtionHooksForScript(jsdc, jsdscript);
...
jsd_hash_script(const void *key, void *closure)
{
...
jsd_InitScriptManager(JSDContext* jsdc)
{
    ...
    jsdc->scriptsTable = JS_NewHashTable(JSD_SCRIPT_HASH_SIZE, jsd_hash_script, NULL,
...
{
    if (script->profileData)
        return script->profileData->maxRelwrseDepth;
...
{
    if (script->profileData)
        return script->profileData->minExelwtionTime;
...
{
    if (script->profileData)
        return script->profileData->maxExelwtionTime;
...
{
    if (script->profileData)
        return script->profileData->totalExelwtionTime;
...
{
    if (script->profileData)
        return script->profileData->minOwnExelwtionTime;
...
{
    if (script->profileData)
        return script->profileData->maxOwnExelwtionTime;
...
{
    if (script->profileData)
        return script->profileData->totalOwnExelwtionTime;
...
#ifdef LIVEWIRE
    if( 1 == lineno )
        jsdlw_PreLoadSource(jsdc, LWDBG_GetLwrrentApp(), filename, JS_TRUE );
#endif
                ...
                void *closure)
{
    ...
    return jsd_CallExelwtionHook(jsdc, cx, JSD_HOOK_BREAKPOINT,
                                 hook, hookData, rval);
}
                     ...
                     void*                 callerdata)
{
    ...
    if( ! hook )
    {
        jsd_ClearExelwtionHook(jsdc, jsdscript, pc);
        JSD_UNLOCK();
        return JS_TRUE;
    }
