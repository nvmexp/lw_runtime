...
* code. Also, note that these stubs include a jsd_LwrrentThread()            
 ...
 * the call to PR_GetLwrrentThread.
 *
 * XXX We ignore the memory leak issue - i.e. we never call PR_DetachThread.
 *
 */
#undef _LWRRENT_THREAD
#ifdef JSD_ATTACH_THREAD_HACK
#define _LWRRENT_THREAD(out)                                                  \
JS_BEGIN_MACRO                                                                \
    out = (void*) PR_GetLwrrentThread();                                      \
    if(!out)                                                                  \
...
extern void* __stdcall GetLwrrentThreadId(void);
#endif
