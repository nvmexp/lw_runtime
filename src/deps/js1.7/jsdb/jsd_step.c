                ...
                if (pdata)
                {
                    if (before)
                    {
                        if (JSLL_IS_ZERO(pdata->lastCallStart))
                        {
                            ...
                            if (++pdata->relwrseDepth > pdata->maxRelwrseDepth)
                                pdata->maxRelwrseDepth = pdata->relwrseDepth;
                        }
                        /* make sure we're called for the return too. */
                        hookresult = JS_TRUE;
                    } else if (!pdata->relwrseDepth &&
                        ...
                        delta /= 1000.0;
                        pdata->totalExelwtionTime += delta;
                        /* minExelwtionTime starts as 0, so we need to overwrite
                         * it on the first call always. */
                        if ((0 == pdata->callCount) ||
                            delta < pdata->minExelwtionTime)
                        {
                            pdata->minExelwtionTime = delta;
                        }
                        if (delta > pdata->maxExelwtionTime)
                            pdata->maxExelwtionTime = delta;
                        ...
                        pdata->totalOwnExelwtionTime += delta;
                        /* See minExelwtionTime comment above. */
                        if ((0 == pdata->callCount) ||
                            delta < pdata->minOwnExelwtionTime)
                        {
                            pdata->minOwnExelwtionTime = delta;
                        }
                        if (delta > pdata->maxOwnExelwtionTime)
                            pdata->maxOwnExelwtionTime = delta;
