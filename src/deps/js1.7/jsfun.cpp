...
js_PutArgsObject(JSContext *cx, JSStackFrame *fp)
{
    ...
    /*
     * Clear the private pointer to fp, which is about to go away (js_Ilwoke).
...
args_or_call_mark(JSContext *cx, JSObject *obj, void *arg)
{
    ...
    fp = static_cast<JSStackFrame *>(JS_GetPrivate(cx, obj));
...
JSClass js_ArgumentsClass = {
    ...
    JS_ColwertStub,     JS_FinalizeStub,
    NULL,               NULL,
    NULL,               NULL,
    NULL,               NULL,
...
static JSPropertySpec call_props[] = {
    ...
    {MKJSID("__callee__"),      CALL_CALLEE,    0,0,0},
    {0,0,0,0,0}
};
...
js_GetCallVariable(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
    ...
    if (fp) {
        ...
        if ((uintN)JSVAL_TO_INT(id) < fp->lwars)
...
call_enumerate(JSContext *cx, JSObject *obj)
{
    ...
    for (sprop = SCOPE_LAST_PROP(scope); sprop; sprop = sprop->parent) {
        ...
        atom = static_cast<JSAtom *>(atom->entry.value);
...
static JSBool
call_colwert(JSContext *cx, JSObject *obj, JSType type, jsval *vp)
{
    JSStackFrame *fp;
    ...
    if (str == ATOM_TO_STRING(prototypeAtom)) {
        ...
        if (fun->object != obj && fun->object) {
            ...
            if (!JSVAL_IS_PRIMITIVE(pval)) {
                 ...
                 */
                cx->weakRoots.newborn[GCX_OBJECT] = static_cast<JSGCThing *>(JSVAL_TO_GCTHING(pval));
...
static JSBool
fun_colwert(JSContext *cx, JSObject *obj, JSType type, jsval *vp)
{
    switch (type) {
...
fun_xdrObject(JSXDRState *xdr, JSObject **objp)
{
    ...
    if (!JS_XDRUint16(xdr, &fun->nargs) ||
        ...
        !JS_XDRUint16(xdr, &fun->u.i.lwars) ||
        ...
        if (xdr->mode == JSXDR_ENCODE) {
            ...
            for (i = 0; i < n; i++) {
                ...
                type = (i < fun->nargs)
                       ...
                       ? JSXDR_FUNCONST
                       : JSXDR_FULWAR;
                userid = static_cast<uint32>(INT_TO_JSVAL(sprop->shortid));
            ...
            for (i = n; i != 0; i--) {
                ...
                JS_ASSERT(type == JSXDR_FUNARG || type == JSXDR_FULWAR ||
                          type == JSXDR_FUNCONST);
                if (type == JSXDR_FUNARG) {
                    getter = js_GetArgument;
                    setter = js_SetArgument;
                    JS_ASSERT(nargs++ <= fun->nargs);
                } else if (type == JSXDR_FULWAR || type == JSXDR_FUNCONST) {
                ...
                if (!js_AddHiddenProperty(cx, fun->object,
                                          ATOM_TO_JSID(propAtom),
                                          getter, setter, SPROP_ILWALID_SLOT,
                                          attrs | JSPROP_SHARED,
                                          dupflag | SPROP_HAS_SHORTID,
                                          JSVAL_TO_INT((jsval)userid))) {
                    goto bad;
                }
            }
        }
                ...
                uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (!argv) {
        ...
        if (!VALUE_IS_FUNCTION(cx, fval)) {
            /*
             * If we don't have a function to start off with, try colwerting
             * the object to a function.  If that doesn't work, complain.
             */
            if (JSVAL_IS_OBJECT(fval)) {
                obj = JSVAL_TO_OBJECT(fval);
                if (!OBJ_GET_CLASS(cx, obj)->colwert(cx, obj, JSTYPE_FUNCTION,
                                                     &fval)) {
                    return JS_FALSE;
                }
                argv[-1] = fval;
...
static const char call_str[] = MKJSID("call");
...
fun_call(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    if (argc == 0) {
        ...
        /* Otherwise colwert the first arg to 'this' and skip over it. */
    ...
    *sp++ = OBJECT_TO_JSVAL(obj);
    ...
    oldsp = fp->sp;
    fp->sp = sp;
    ok = js_Ilwoke(cx, argc, JSILWOKE_INTERNAL | JSILWOKE_SKIP_CALLER);
...
fun_apply(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    /* Colwert the first arg to 'this' and skip over it. */
    if (!js_ValueToObject(cx, argv[0], &obj))
        return JS_FALSE;

    /* Allocate stack space for fval, obj, and the args. */
    argc = (uintN)JS_MIN(length, static_cast<decltype(length)>(ARRAY_INIT_LIMIT - 1));
    sp = js_AllocStack(cx, 2 + argc, &mark);
    if (!sp)
        return JS_FALSE;
                     ...
                     jsval *rval)
{
    ...
    *sp++ = JSVAL_NULL; /* This is filled automagically. */
    ...
    oldsp = fp->sp;
    fp->sp = sp;
    ok = js_IlwokeConstructor(cx, newsp, length);
...
static JSFunctionSpec function_methods[] = {
    ...
    {MKJSID("apply"),           fun_apply,      2,0,0},
    {call_str,                  fun_call,       1,0,0},
#ifdef NARCISSUS
    {MKJSID("__applyConstructor__"), fun_applyConstructor, 1,0,0},
#endif
    {0,0,0,0,0}
};
...
Function(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
     ...
     * file, therefore it is callable only as a native function by js_Ilwoke.
     * Find the scripted caller, possibly skipping other native frames such as
     * are built for Function.prototype.call or .apply activations that ilwoke
...
js_InitFunctionClass(JSContext *cx, JSObject *obj)
{
    ...
    if (!proto)
        return NULL;
    atom = js_Atomize(cx, js_FunctionClass.name, strlen(js_FunctionClass.name), 0);
    if (!atom)
        goto bad;
...
js_InitCallClass(JSContext *cx, JSObject *obj)
{
     ...
     */
    OBJ_SET_PROTO(cx, proto, nullptr);
    return proto;
}
...
# error "JSILWOKE_CONSTRUCT and JSV2F_SEARCH_STACK are not disjoint!"
#endif
