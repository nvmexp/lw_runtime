...
struct JSStackFrame {
    ...
    uintN           lwars;          /* local variable count */
...
#define JSFRAME_INTERNAL      0x02  /* internal call, not ilwoked by a script */
...
#define JSFRAME_ASSIGNING     0x08  /* a complex (not simplex JOF_ASSIGNING) op
                                       is lwrrently assigning to a property */
...
#if defined(JS_PROPERTY_CACHE_METERING)
...
/*
 * NB: js_Ilwoke requires that cx is lwrrently running JS (i.e., that cx->fp
...
/*
 * Consolidated js_Ilwoke flags simply rename certain JSFRAME_* flags, so that
 ...
 */
#define JSILWOKE_CONSTRUCT      JSFRAME_CONSTRUCTING
#define JSILWOKE_INTERNAL       JSFRAME_INTERNAL
#define JSILWOKE_SKIP_CALLER    JSFRAME_SKIP_CALLER
#define JSILWOKE_ITERATOR       JSFRAME_ITERATOR

/*
 * Mask to isolate construct and iterator flags for use with jsfun.h functions.
 */
#define JSILWOKE_FUNFLAGS       (JSILWOKE_CONSTRUCT | JSILWOKE_ITERATOR)
...
#define js_InternalCall(cx,obj,fval,argc,argv,rval)                           \
    js_InternalIlwoke(cx, obj, fval, 0, argc, argv, rval)

#define js_InternalConstruct(cx,obj,fval,argc,argv,rval)                      \
    js_InternalIlwoke(cx, obj, fval, JSILWOKE_CONSTRUCT, argc, argv, rval)
