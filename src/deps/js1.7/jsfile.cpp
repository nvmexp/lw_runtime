...
#define LWRRENTDIR_PROPERTY     MKJSID("lwrrentDir")
#define SEPARATOR_PROPERTY      MKJSID("separator")
#define FILE_CONSTRUCTOR        MKJSID("File")
#define PIPE_SYMBOL             '|'
...
{
// DJGPP uses XP_UNIX but needs to use windows method for
// determining the absolute path
#if defined(XP_WIN) || defined(XP_OS2) || defined(DJGPP)
    return *name && name[1] == ':';
#else
    return (name[0]
...
{
    int len = static_cast<int>(strlen(base));
    char* result = static_cast<char *>(JS_malloc(cx, len + strlen(name) + 2));
...
js_fileBaseName(JSContext *cx, const char *pathname)
{
    ...
    index = static_cast<jsint>(strlen(pathname) - 1);
    ...
    /* Allocate and copy. */
    result = static_cast<char *>(JS_malloc(cx, aux - index + 1));
    if (!result)
        return NULL;
...
js_fileDirectoryName(JSContext *cx, const char *pathname)
{
    ...
    if (cp < pathname && end != pathname) {
        ...
        result = static_cast<char *>(JS_malloc(cx, 1 + 1)); /* The separator + trailing NUL. */
    ...
    if (cp < pathname) {
        ...
        result = static_cast<char *>(JS_malloc(cx, pathsize + 1));
        if (!result)
            return NULL;
...
js_absolutePath(JSContext *cx, const char * path)
{
    ...
    if (js_isAbsolute(path)) {
        ...
        if (!JS_GetProperty(cx, obj, LWRRENTDIR_PROPERTY, &prop)) {
            JS_ReportErrorNumber(cx, JSFile_GetErrorMessage, NULL,
                                 JSFILEMSG_FILE_LWRRENTDIR_UNDEFINED_ERROR);
            return JS_strdup(cx, path);
        }
        ...
        /* should we have an array of lwrr dirs indexed by drive for windows? */
...
js_canonicalPath(JSContext *cx, char *oldpath)
{
    ...
    tmp = static_cast<char *>(JS_malloc(cx, j-i+2));
    if (!tmp)
        return NULL;
    ...
    while (strcmp(dir, current)) {
        if (!strcmp(base, "..")) {
            ...
            if (back > 0) {
                ...
                tmp = result;
                result = static_cast<char *>(JS_malloc(cx, strlen(base) + 1 + strlen(tmp) + 1));
                if (!result)
                    goto out;

                strcpy(result, base);
                c = static_cast<jsint>(strlen(result));
                if (*tmp) {
    ...
    tmp = result;
    result = static_cast<char *>(JS_malloc(cx, strlen(dir)+1+strlen(tmp)+1));
    if (!result)
        goto out;
                                   ...
                                   uint16 onechar)
{
    ...
    if (onechar == LINE_SEPARATOR || onechar == PARAGRAPH_SEPARATOR) {
        strcpy((char*)tobufp, "\n");
        return static_cast<int16>(strlen((char*)tobufp));
    }
 ...
 * utf8_to_ucs2_char
 *
 * Colwert a utf8 multibyte character to ucs2
 *
 ...
 */
static int16
utf8_to_ucs2_char(const unsigned char *utf8p, int16 buflen, jschar *ucs2p)
{
...
// JS 1.7 broke backwards compatibility for the open command
// by making the mode the first parameter and the type of file an optional
// second parameter.  We still need backward compatibility so swap the
// parameters here
...
js_BufferedRead(JSFile *f, unsigned char *buf, int32 len)
{
    ...
    if (len > 0) {
        count += (!f->isNative)
                 ...
                 : (int32)fread(buf, 1, len, f->nativehandle);
    }
    return count;
}
...
js_FileSeek(JSContext *cx, JSFile *file, int32 len, int32 mode)
{
    ...
    switch (mode) {
      case ASCII:
        count = PR_Seek(file->handle, len, PR_SEEK_LWR);
        break;
      ...
      case UCS2:
        count = PR_Seek(file->handle, len*2, PR_SEEK_LWR)/2;
        break;
    ...
    switch (mode) {
      case ASCII:
        ...
        count = (!file->isNative)
                ...
                : (int32)fwrite(aux, 1, len, file->nativehandle);
      ...
      case UTF8:
        ...
        j = (!file->isNative)
            ...
            : (int32)fwrite(utfbuf, 1, i, file->nativehandle);
      ...
      case UCS2:
        count = (!file->isNative)
                ...
                : (int32)fwrite(buf, 1, len*2, file->nativehandle) >> 1;
...
{
    JSFile      *file = static_cast<JSFile *>(JS_GetInstancePrivate(cx, obj, &js_FileClass, NULL));
    ...
    SELWRITY_CHECK(cx, NULL, "open", file);
    ...
    len = static_cast<int>(strlen(file->path));

// JS 1.7 broke backwards compatibility for the open command
// by making the mode the first parameter and the type of file an optional
// second parameter.  We still need backward compatibility so make type an
// optional FIRST parameter (i.e. with two parameters the arguments are
// (<type>, <mode>) with one parameter it is just (<mode>))
    strtype = 0;
    strmode = 0;

    // Get type/mode in a backward compatible manner for two parameters
    if (argc>=2) {
    ...
    /* Type */
    if (argc > 1) {
        // strtype was retrieved above as the first argument instead
        // of the second
        if (!strtype) {
            JS_ReportErrorNumber(cx, JSFile_GetErrorMessage, NULL,
                                JSFILEMSG_FIRST_ARGUMENT_OPEN_NOT_STRING_ERROR,
    ...
    if (file->path[0]==PIPE_SYMBOL || file->path[len-1]==PIPE_SYMBOL) {
        if (file->path[0] == PIPE_SYMBOL && file->path[len-1] == PIPE_SYMBOL) {
            ...
            int i = 0;
            char pipemode[3];
            SELWRITY_CHECK(cx, NULL, "pipe_open", file);
            ...
            if(file->path[0] == PIPE_SYMBOL){
                ...
                char *command = static_cast<char *>(JS_malloc(cx, len));
		...
		// To maintain backwards compatibility non-ASCII files should be opened
		// in binary mode
        if (type != ASCII)
        {
            mask |= PR_BINARY;
...
file_close(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "close", file);
...
file_remove(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "remove", file);
...
file_copyTo(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "copyTo", file);   /* may need a second argument!*/
    ...
    buffer = static_cast<char *>(JS_malloc(cx, size));

    count = INT_TO_JSVAL(PR_Read(file->handle, buffer, static_cast<int32>(size)));
...
file_renameTo(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "renameTo", file); /* may need a second argument!*/
...
file_flush(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "flush", file);
...
file_write(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "write", file);
    JSFILE_CHECK_WRITE;

    for (i = 0; i<argc; i++) {
        str = JS_ValueToString(cx, argv[i]);
        count = js_FileWrite(cx, file, JS_GetStringChars(str),
            static_cast<int32>(JS_GetStringLength(str)), file->type);
        if (count==-1){
          *rval = JSVAL_FALSE;
          return JS_FALSE;
        }
...
file_writeln(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "writeln", file);
    JSFILE_CHECK_WRITE;
    ...
    if (js_FileWrite(cx, file, JS_GetStringChars(str), static_cast<int32>(JS_GetStringLength(str)),
            file->type)==-1){
        *rval = JSVAL_FALSE;
        return JS_FALSE;
    }
...
file_writeAll(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "writeAll", file);
...
file_read(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "read", file);
    ...
    buf = static_cast<jschar *>(JS_malloc(cx, want * sizeof buf[0]));
    if (!buf)  goto out;
...
file_readln(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "readln", file);
    JSFILE_CHECK_READ;

    buf = static_cast<jschar *>(JS_malloc(cx, MAX_LINE_LENGTH * sizeof data));
    if (!buf)
        return JS_FALSE;
    ...
    for (;;) {
        ...
        switch (data) {
          ...
          default:
            if (--room < 0) {
                tmp = static_cast<jschar *>(JS_realloc(cx, buf,
...
eof:
    ...
    buf[offset] = 0;
    tmp = static_cast<jschar *>(JS_realloc(cx, buf, (offset + 1) * sizeof data));
    if (!tmp)
        goto out;
...
file_readAll(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "readAll", file);
    JSFILE_CHECK_READ;
...
file_seek(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "seek", file);
...
file_list(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "list", file);
    JSFILE_CHECK_NATIVE("list");

    if (argc==1) {
        if (JSVAL_IS_REGEXP(cx, argv[0])) {
            re = static_cast<JSRegExp *>(JS_GetPrivate(cx, JSVAL_TO_OBJECT(argv[0])));
        }else
        if (VALUE_IS_FUNCTION(cx, argv[0])) {
            func = static_cast<JSFunction *>(JS_GetPrivate(cx, JSVAL_TO_OBJECT(argv[0])));
        }else{
            JS_ReportErrorNumber(cx, JSFile_GetErrorMessage, NULL,
    ...
    while ((entry = PR_ReadDir(dir, PR_SKIP_BOTH))!=NULL) {
        ...
        if (re!=NULL) {
            ...
            if(!js_ExelwteRegExp(cx, re, str, &index, JS_TRUE, &v)){
...
file_mkdir(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "mkdir", file);
...
file_init(JSContext *cx, JSObject *obj, char *bytes)
{
    ...
    file = static_cast<JSFile *>(JS_malloc(cx, sizeof *file));
    if (!file)
        return NULL;
...
js_NewFileObject(JSContext *cx, const char *filename)
{
    ...
    file = file_init(cx, obj, const_cast<char *>(filename));
    if(!file) return NULL;
    return obj;
}
...
JSObject*
js_NewFileObjectFromFILE(JSContext *cx, FILE *nativehandle, const char *filename,
                 ...
                 jsval *rval)
{
    ...
    SELWRITY_CHECK(cx, NULL, "constructor", file);
...
static JSFunctionSpec file_functions[] = {
    {MKJSID("open"),           file_open, 0},
    {MKJSID("close"),          file_close, 0},
    {MKJSID("remove"),         file_remove, 0},
    {MKJSID("copyTo"),         file_copyTo, 0},
    {MKJSID("renameTo"),       file_renameTo, 0},
    {MKJSID("flush"),          file_flush, 0},
    {MKJSID("seek"),           file_seek, 0},
    {MKJSID("read"),           file_read, 0},
    {MKJSID("readln"),         file_readln, 0},
    {MKJSID("readAll"),        file_readAll, 0},
    {MKJSID("write"),          file_write, 0},
    {MKJSID("writeln"),        file_writeln, 0},
    {MKJSID("writeAll"),       file_writeAll, 0},
    {MKJSID("list"),           file_list, 0},
    {MKJSID("mkdir"),          file_mkdir, 0},
    {MKJSID("toString"),       file_toString, 0},
    {MKJSID("toURL"),          file_toURL, 0},
    {0}
};
...
static JSPropertySpec file_props[] = {
   {MKJSID("length"),          FILE_LENGTH,        JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("parent"),          FILE_PARENT,        JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("path"),            FILE_PATH,          JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("name"),            FILE_NAME,          JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("isDirectory"),     FILE_ISDIR,         JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("isFile"),          FILE_ISFILE,        JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("exists"),          FILE_EXISTS,        JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("canRead"),         FILE_CANREAD,       JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("canWrite"),        FILE_CANWRITE,      JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("canAppend"),       FILE_APPEND,        JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("canReplace"),      FILE_REPLACE,       JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("isOpen"),          FILE_OPEN,          JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("type"),            FILE_TYPE,          JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("mode"),            FILE_MODE,          JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("creationTime"),    FILE_CREATED,       JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("lastModified"),    FILE_MODIFIED,      JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("size"),            FILE_SIZE,          JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("hasRandomAccess"), FILE_RANDOMACCESS,  JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("hasAutoFlush"),    FILE_AUTOFLUSH,     JSPROP_ENUMERATE | JSPROP_READONLY },
   {MKJSID("position"),        FILE_POSITION,      JSPROP_ENUMERATE },
   {MKJSID("isNative"),        FILE_ISNATIVE,      JSPROP_ENUMERATE | JSPROP_READONLY },
   {0}
};
...
file_getProperty(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
    ...
    switch (tiny) {
    case FILE_PARENT:
        SELWRITY_CHECK(cx, NULL, "parent", file);
    ...
    case FILE_TYPE:
        ...
        break;
    case FILE_MODE:
        SELWRITY_CHECK(cx, NULL, "mode", file);
        JSFILE_CHECK_OPEN("mode");
        bytes = static_cast<char *>(JS_malloc(cx, MODE_SIZE));
        bytes[0] = '\0';
        flag = JS_FALSE;
        ...
        *vp = STRING_TO_JSVAL(JS_NewStringCopyZ(cx, bytes));
        ...
        break;
    case FILE_CREATED:
        SELWRITY_CHECK(cx, NULL, "creationTime", file);
        ...
        *vp = OBJECT_TO_JSVAL(js_NewDateObject(cx,  expandedTime.tm_year,
        ...
        break;
    case FILE_MODIFIED:
        SELWRITY_CHECK(cx, NULL, "lastModified", file);
        ...
        *vp = OBJECT_TO_JSVAL(js_NewDateObject(cx, expandedTime.tm_year,
        ...
        break;
    case FILE_SIZE:
        SELWRITY_CHECK(cx, NULL, "size", file);
        *vp = js_size(cx, file);
        break;
    case FILE_LENGTH:
        SELWRITY_CHECK(cx, NULL, "length", file);
    ...
    case FILE_RANDOMACCESS:
            SELWRITY_CHECK(cx, NULL, "hasRandomAccess", file);
            JSFILE_CHECK_OPEN("hasRandomAccess");
            *vp = BOOLEAN_TO_JSVAL(file->hasRandomAccess);
        break;
    case FILE_POSITION:
        SELWRITY_CHECK(cx, NULL, "position", file);
        ...
        if (file->isOpen && js_isFile(cx, file)) {
            int pos = PR_Seek(file->handle, 0, PR_SEEK_LWR);
            if(pos!=-1){
                *vp = INT_TO_JSVAL(pos);
            }else{
                JS_ReportErrorNumber(cx, JSFile_GetErrorMessage, NULL,
        ...
        break;
    default:
        SELWRITY_CHECK(cx, NULL, "file_access", file);
...
file_setProperty(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
    ...
    /* File.position  = 10 */
    case FILE_POSITION:
        SELWRITY_CHECK(cx, NULL, "set_position", file);
...
/*
    File.lwrrentDir = new File("D:\") or File.lwrrentDir = "D:\"
*/
static JSBool
file_lwrrentDirSetter(JSContext *cx, JSObject *obj, jsval id, jsval *vp)
{
    JSFile   *file;

    file = static_cast<JSFile *>(JS_GetInstancePrivate(cx, obj, &js_FileClass, NULL));
    ...
    if (JSVAL_IS_OBJECT(*vp)) {
        if (JS_InstanceOf(cx, obj, &js_FileClass, NULL)) {
            ...
            if (file && (!js_exists(cx, file) || !js_isDirectory(cx, file))) {
                JS_GetProperty(cx, obj, LWRRENTDIR_PROPERTY, vp);
                return JS_FALSE;
            } else {
                chdir(file->path);
                return JS_TRUE;
...
JSClass js_FileClass = {
    MKJSID("File"), JSCLASS_HAS_PRIVATE | JSCLASS_HAS_CACHED_PROTO(JSProto_File),
    JS_PropertyStub,  JS_PropertyStub,  file_getProperty,  file_setProperty,
    JS_EnumerateStub, JS_ResolveStub,   JS_ColwertStub,    file_finalize
};
...
js_InitFileClass(JSContext *cx, JSObject* obj)
{
    ...
    /* Define LWRRENTDIR property. We are doing this to get a
    slash at the end of the current dir */
    afile = js_NewFileObject(cx, LWRRENT_DIR);
    lwrrentdir =  static_cast<char *>(JS_malloc(cx, MAX_PATH_LENGTH));
    lwrrentdir =  getcwd(lwrrentdir, MAX_PATH_LENGTH);
    afile = js_NewFileObject(cx, lwrrentdir);
    JS_free(cx, lwrrentdir);
    vp = OBJECT_TO_JSVAL(afile);
    JS_DefinePropertyWithTinyId(cx, ctor, LWRRENTDIR_PROPERTY, 0, vp,
                JS_PropertyStub, file_lwrrentDirSetter,
                JSPROP_ENUMERATE | JSPROP_READONLY );

    /* Define input */
    vp = OBJECT_TO_JSVAL(js_NewFileObjectFromFILE(cx, stdin,
            STDINPUT_NAME, PR_RDONLY, JS_TRUE, JS_FALSE));
    JS_SetProperty(cx, ctor, MKJSID("input"), &vp);

    /* Define output */
    vp = OBJECT_TO_JSVAL(js_NewFileObjectFromFILE(cx, stdout,
            STDOUTPUT_NAME, PR_WRONLY, JS_TRUE, JS_FALSE));
    JS_SetProperty(cx, ctor, MKJSID("output"), &vp);

    /* Define error */
    vp = OBJECT_TO_JSVAL(js_NewFileObjectFromFILE(cx, stderr,
            STDERROR_NAME, PR_WRONLY, JS_TRUE, JS_FALSE));
    JS_SetProperty(cx, ctor, MKJSID("error"), &vp);
