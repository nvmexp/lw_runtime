 ...
 * JS parser.
 *
 * This is a relwrsive-descent parser for the JavaScript language specified by
 * "The JavaScript 1.5 Language Specification".  It uses lexical and semantic
 * feedback to disambiguate non-LL(1) structures.  It generates trees of nodes
 * induced by the relwrsive parsing (not precise syntax trees, see jsparse.h).
          ...
          JSTreeContext *tc)
{
     ...
     * a list, to reduce js_FoldConstants and js_EmitTree relwrsion.
     */
    if (left->pn_type == tt &&
        left->pn_op == op &&
...
CheckGetterOrSetter(JSContext *cx, JSTokenStream *ts, JSTokenType tt)
{
    ...
    JS_ASSERT(LWRRENT_TOKEN(ts).type == TOK_NAME);
    atom = LWRRENT_TOKEN(ts).t_atom;
    rt = cx->runtime;
    ...
    if (LWRRENT_TOKEN(ts).t_op != JSOP_NOP) {
        js_ReportCompileErrorNumber(cx, ts, JSREPORT_TS | JSREPORT_ERROR,
                ...
                JSStackFrame *newfp)
{
    ...
    if (oldfp) {
        ...
        // For new special frames, require that the varobj 
        // pointer be also be not null (fixes issues with inspecting variable
        // in the JS debugger at the top stack frame)
        while ((oldfp->flags & JSFRAME_SPECIAL) ||
               ((newfp->flags & JSFRAME_SPECIAL) && !oldfp->varobj)) {
            oldfp = oldfp->down;
            if (!oldfp)
                break;
        }
...
BindLocalVariable(JSContext *cx, BindData *data, JSAtom *atom)
{
    ...
    /*
     * Can't increase fun->lwars in an active frame, so insist that getter is
    ...
    if (!js_AddHiddenProperty(cx, data->obj, ATOM_TO_JSID(atom),
                              ...
                              SPROP_HAS_SHORTID, fun->u.i.lwars)) {
        return JS_FALSE;
    }
    if (fun->u.i.lwars == JS_BITMASK(16)) {
        JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL,
            ...
            JSBool lambda)
{
    ...
    ts->flags &= ~TSF_KEYWORD_IS_NAME;
    if (tt == TOK_NAME) {
        funAtom = LWRRENT_TOKEN(ts).t_atom;
    } else {
        funAtom = NULL;
        js_UngetToken(ts);
    }
        ...
        if (AT_TOP_LEVEL(tc) && (tc->flags & TCF_IN_FUNCTION)) {
            ...
            if (!prop ||
                ...
                if (!js_AddHiddenProperty(cx, varobj, ATOM_TO_JSID(funAtom),
                                          ...
                                          SPROP_ILWALID_SLOT,
                                          JSPROP_PERMANENT | JSPROP_SHARED,
                                          sflags, fp->fun->u.i.lwars)) {
                    return NULL;
                }
                if (fp->fun->u.i.lwars == JS_BITMASK(16)) {
                    JS_ReportErrorNumber(cx, js_GetErrorMessage, NULL,
                                         JSMSG_TOO_MANY_FUN_VARS);
                    return NULL;
                }
    ...
    if (!js_MatchToken(cx, ts, TOK_RP)) {
        ...
        do {
            ...
            switch (tt) {
              ...
              case TOK_NAME:
                if (!data.binder(cx, &data, LWRRENT_TOKEN(ts).t_atom, tc))
                    return NULL;
                break;
    ...
    MUST_MATCH_TOKEN(TOK_LC, JSMSG_LWRLY_BEFORE_BODY);
    pn->pn_pos.begin = LWRRENT_TOKEN(ts).pos.begin;
    ...
    MUST_MATCH_TOKEN(TOK_RC, JSMSG_LWRLY_AFTER_BODY);
    pn->pn_pos.end = LWRRENT_TOKEN(ts).pos.end;
...
Condition(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
     ...
     * XXX not ECMA, but dolwmented in several books -- now a strict warning.
     */
...
ImportExpr(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
    ...
    if (!pn)
        return NULL;
    pn->pn_op = JSOP_NAME;
    pn->pn_atom = LWRRENT_TOKEN(ts).t_atom;
    pn->pn_expr = NULL;
    pn->pn_slot = -1;
    pn->pn_attrs = 0;
    ...
    while ((tt = js_GetToken(cx, ts)) == TOK_DOT || tt == TOK_LB) {
        ...
        if (tt == TOK_DOT) {
            ...
            if (js_MatchToken(cx, ts, TOK_STAR)) {
                ...
                pn2->pn_op = JSOP_GETPROP;
                pn2->pn_atom = LWRRENT_TOKEN(ts).t_atom;
                pn2->pn_slot = -1;
                pn2->pn_attrs = 0;
            }
            ts->flags &= ~TSF_KEYWORD_IS_NAME;
            ...
            pn2->pn_pos.end = LWRRENT_TOKEN(ts).pos.end;
        } else {
        ...
        if (sprop->getter == js_GetArgument) {
            ...
            if (fun) {
                ...
                if (data->u.var.clasp == &js_FunctionClass) {
                    ...
                    JS_ASSERT((sprop->flags & SPROP_HAS_SHORTID) &&
                              (uint16) sprop->shortid < fun->u.i.lwars);
...
FindPropertyValue(JSParseNode *pn, JSParseNode *pnid, FindPropValData *data)
{
    ...
    if (data->table.ops) {
        ...
        return JS_DHASH_ENTRY_IS_BUSY(&entry->hdr) ? entry->plwal : NULL;
    }
...
ContainsStmt(JSParseNode *pn, JSTokenType tt)
{
    ...
    switch (pn->pn_arity) {
      ...
      case PN_BINARY:
        /*
         * Limit relwrsion if pn is a binary expression, which can't contain a
         * var statement.
         */
...
LetBlock(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc, JSBool statement)
{
    ...
    JS_ASSERT(LWRRENT_TOKEN(ts).type == TOK_LET);
    ...
    if (statement) {
        ...
        if (!pnlet->pn_right)
            return NULL;
        MUST_MATCH_TOKEN(TOK_RC, JSMSG_LWRLY_AFTER_LET);
    } else {
        /*
...
Statement(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
    ...
    switch (tt) {
      ...
      case TOK_SWITCH:
      {
        ...
        MUST_MATCH_TOKEN(TOK_LC, JSMSG_LWRLY_BEFORE_SWITCH);
        ...
        pn->pn_pos.end = pn2->pn_pos.end = LWRRENT_TOKEN(ts).pos.end;
        pn->pn_left = pn1;
        pn->pn_right = pn2;
        return pn;
      }
      ...
      case TOK_DO:
        ...
        if ((cx->version & JSVERSION_MASK) != JSVERSION_ECMA_3) {
             ...
             * insertion after do-while.  See the testcase and dislwssion in
      ...
      case TOK_FOR:
      {
        ...
        if (js_MatchToken(cx, ts, TOK_NAME)) {
            if (LWRRENT_TOKEN(ts).t_atom == cx->runtime->atomState.eachAtom)
        ...
        if (tt == TOK_SEMI) {
             ...
             * expressions ilwolving an 'in' operator are illegal in the init
      ...
      case TOK_TRY: {
        ...
        MUST_MATCH_TOKEN(TOK_LC, JSMSG_LWRLY_BEFORE_TRY);
        js_PushStatement(tc, &stmtInfo, STMT_TRY, -1);
        pn->pn_kid1 = Statements(cx, ts, tc);
        if (!pn->pn_kid1)
            return NULL;
        MUST_MATCH_TOKEN(TOK_RC, JSMSG_LWRLY_AFTER_TRY);
        js_PopStatement(tc);
        ...
        if (tt == TOK_CATCH) {
            ...
            do {
                ...
                MUST_MATCH_TOKEN(TOK_LC, JSMSG_LWRLY_BEFORE_CATCH);
                pn2->pn_kid3 = Statements(cx, ts, tc);
                if (!pn2->pn_kid3)
                    return NULL;
                MUST_MATCH_TOKEN(TOK_RC, JSMSG_LWRLY_AFTER_CATCH);
                js_PopStatement(tc);
        ...
        if (tt == TOK_FINALLY) {
            tc->tryCount++;
            MUST_MATCH_TOKEN(TOK_LC, JSMSG_LWRLY_BEFORE_FINALLY);
            js_PushStatement(tc, &stmtInfo, STMT_FINALLY, -1);
            pn->pn_kid3 = Statements(cx, ts, tc);
            if (!pn->pn_kid3)
                return NULL;
            MUST_MATCH_TOKEN(TOK_RC, JSMSG_LWRLY_AFTER_FINALLY);
            js_PopStatement(tc);
        } else {
            js_UngetToken(ts);
            pn->pn_kid3 = NULL;
      ...
      case TOK_LET:
      {
        ...
        /*
         * This is a let declaration. We must colwert the nearest JSStmtInfo
        ...
        if (stmt && (stmt->flags & SIF_SCOPE)) {
            ...
            if (!stmt) {
                 ...
                 */
                LWRRENT_TOKEN(ts).type = TOK_VAR;
                LWRRENT_TOKEN(ts).t_op = JSOP_DEFVAR;
            ...
            /* Colwert the block statement into a scope statement. */
            obj = js_NewBlockObject(cx);
            if (!obj)
                return NULL;
            atom = js_AtomizeObject(cx, obj, 0);
      ...
      case TOK_LC:
      {
        ...
        MUST_MATCH_TOKEN(TOK_RC, JSMSG_LWRLY_IN_COMPOUND);
        js_PopStatement(tc);
      ...
      case TOK_DEFAULT:
        ...
        if (!pn)
            return NULL;
        if (!js_MatchToken(cx, ts, TOK_NAME) ||
            LWRRENT_TOKEN(ts).t_atom != cx->runtime->atomState.xmlAtom ||
            !js_MatchToken(cx, ts, TOK_NAME) ||
            LWRRENT_TOKEN(ts).t_atom != cx->runtime->atomState.namespaceAtom ||
            !js_MatchToken(cx, ts, TOK_ASSIGN) ||
            LWRRENT_TOKEN(ts).t_op != JSOP_NOP) {
            js_ReportCompileErrorNumber(cx, ts, JSREPORT_TS | JSREPORT_ERROR,
    ...
    if (ON_LWRRENT_LINE(ts, pn->pn_pos)) {
        ts->flags |= TSF_OPERAND;
        tt = js_PeekTokenSameLine(cx, ts);
        ts->flags &= ~TSF_OPERAND;
        if (tt == TOK_ERROR)
...
Variables(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
    ...
    data.pn = NULL;
    data.ts = ts;
    data.op = let ? JSOP_NOP : LWRRENT_TOKEN(ts).t_op;
...
RelExpr(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
    ...
    while (pn &&
           (js_MatchToken(cx, ts, TOK_RELOP) ||
             ...
             * lwrrently in the init expr of a for loop.
             */
...
UnaryExpr(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
    ...
    switch (tt) {
      ...
      case TOK_MINUS:
        ...
        pn->pn_op = LWRRENT_TOKEN(ts).t_op;
           ...
           JSBool allowCallSyntax)
{
    ...
    while ((tt = js_GetToken(cx, ts)) > TOK_EOF) {
        if (tt == TOK_DOT) {
            ...
            if (!pn3)
                return NULL;
            tt = static_cast<JSTokenType>(pn3->pn_type);
            if (tt == TOK_NAME ||
                (tt == TOK_DBLCOLON &&
                ...
                JSTreeContext *tc)
{
    ...
    JS_ASSERT(LWRRENT_TOKEN(ts).type == TOK_DBLCOLON);
    pn2 = NewParseNode(cx, ts, PN_NAME, tc);
    if (!pn2)
        return NULL;
...
AttributeIdentifier(JSContext *cx, JSTokenStream *ts, JSTreeContext *tc)
{
    ...
    JS_ASSERT(LWRRENT_TOKEN(ts).type == TOK_AT);
    pn = NewParseNode(cx, ts, PN_UNARY, tc);
    if (!pn)
        return NULL;
...
XMLExpr(JSContext *cx, JSTokenStream *ts, JSBool inTag, JSTreeContext *tc)
{
    ...
    JS_ASSERT(LWRRENT_TOKEN(ts).type == TOK_LC);
    pn = NewParseNode(cx, ts, PN_UNARY, tc);
    if (!pn)
        return NULL;
    ...
    MUST_MATCH_TOKEN(TOK_RC, JSMSG_LWRLY_IN_XML_EXPR);
    ts->flags = oldflags;
    pn->pn_kid = pn2;
 ...
 * TOK_XMLTEXT, TOK_XMLCDATA, TOK_XMLCOMMENT, or TOK_XMLPI.  When colwerting
                  ...
                  JSTreeContext *tc)
{
    ...
    for (;;) {
        ...
        textAtom = LWRRENT_TOKEN(ts).t_atom;
        if (textAtom) {
    ...
    JS_ASSERT(LWRRENT_TOKEN(ts).type == TOK_XMLETAGO);
    ts->flags |= TSF_XMLTAGMODE;
    return JS_TRUE;
}
                 ...
                 JSBool allowList)
{
    ...
    JS_ASSERT(LWRRENT_TOKEN(ts).type == TOK_XMLSTAGO);
    pn = NewParseNode(cx, ts, PN_LIST, tc);
    if (!pn)
        return NULL;
    ...
    switch (tt) {
      ...
      case TOK_LB:
      {
        ...
        if (!matched) {
            for (index = 0; ; index++) {
                ...
                if (tt == TOK_COMMA) {
                    /* So LWRRENT_TOKEN gets TOK_COMMA and not TOK_LB. */
            ...
            if (index == 0 &&
                ...
                *pn->pn_tail = NULL;
                ...
                do {
                    ...
                    if (js_MatchToken(cx, ts, TOK_NAME)) {
                        if (LWRRENT_TOKEN(ts).t_atom == rt->atomState.eachAtom)
        ...
        for (;;) {
            ...
            switch (tt) {
              case TOK_NUMBER:
                ...
                if (pn3)
                    pn3->pn_dval = LWRRENT_TOKEN(ts).t_dval;
                break;
              case TOK_NAME:
#if JS_HAS_GETTER_SETTER
                {
                    ...
                    if (atom == rt->atomState.getAtom ||
                        ...
                        if (js_MatchToken(cx, ts, TOK_NAME)) {
                            ...
                            if (!pn3)
                                return NULL;
                            pn3->pn_atom = LWRRENT_TOKEN(ts).t_atom;
                            pn3->pn_expr = NULL;
                            pn3->pn_slot = -1;
                            pn3->pn_attrs = 0;
                            ...
                            LWRRENT_TOKEN(ts).t_op = JSOP_NOP;
                            LWRRENT_TOKEN(ts).type = TOK_FUNCTION;
     ...
     end_obj_init:
      ...
      case TOK_DEFSHARP:
        ...
        defsharp->pn_num = (jsint) LWRRENT_TOKEN(ts).t_dval;
        ts->flags |= TSF_OPERAND;
        tt = js_GetToken(cx, ts);
        ts->flags &= ~TSF_OPERAND;
        goto again;
      ...
      case TOK_USESHARP:
        ...
        if (!pn)
            return NULL;
        pn->pn_num = (jsint) LWRRENT_TOKEN(ts).t_dval;
        notsharp = JS_TRUE;
        break;
...
#if JS_HAS_XML_SUPPORT
        if (tt == TOK_XMLPI)
            pn->pn_atom2 = LWRRENT_TOKEN(ts).t_atom2;
        else
#endif
            pn->pn_op = LWRRENT_TOKEN(ts).t_op;
        if (tt == TOK_NAME) {
      ...
      case TOK_NUMBER:
        ...
        if (!pn)
            return NULL;
        pn->pn_dval = LWRRENT_TOKEN(ts).t_dval;
#if JS_HAS_SHARP_VARS
        notsharp = JS_TRUE;
#endif
        break;
...
FoldXMLConstants(JSContext *cx, JSParseNode *pn, JSTreeContext *tc)
{
    ...
    tt = static_cast<JSTokenType>(pn->pn_type);
    pnp = &pn->pn_head;
    pn1 = *pnp;
    aclwm = NULL;
    if ((pn->pn_extra & PNX_CANTFOLD) == 0) {
        if (tt == TOK_XMLETAGO)
            aclwm = ATOM_TO_STRING(cx->runtime->atomState.etagoAtom);
        else if (tt == TOK_XMLSTAGO || tt == TOK_XMLPTAGC)
            aclwm = ATOM_TO_STRING(cx->runtime->atomState.stagoAtom);
    }
    ...
    for (pn2 = pn1, i = j = 0; pn2; pn2 = pn2->pn_next, i++) {
        ...
        switch (pn2->pn_type) {
          ...
          default:
            ...
            if ((tt == TOK_XMLSTAGO || tt == TOK_XMLPTAGC) &&
                ...
                printf("1: %d, %d => %s\n",
                       i, j, aclwm ? JS_GetStringBytes(aclwm) : "NULL");
#endif
            } else if (aclwm && pn1 != pn2) {
                ...
                pn1->pn_atom = js_AtomizeString(cx, aclwm, 0);
                if (!pn1->pn_atom)
                    return JS_FALSE;
        ...
        if (aclwm) {
            str = ((tt == TOK_XMLSTAGO || tt == TOK_XMLPTAGC) && i != 0)
                  ? js_AddAttributePart(cx, i & 1, aclwm, str)
                  : js_ConcatStrings(cx, aclwm, str);
            if (!str)
                return JS_FALSE;
#ifdef DEBUG_brendanXXX
            printf("2: %d, %d => %s (%u)\n",
    ...
    if (aclwm) {
        ...
        if (str) {
            aclwm = js_ConcatStrings(cx, aclwm, str);
            if (!aclwm)
                return JS_FALSE;
        }
...
{
#define TAIL_RELWRSE(pn2) JS_BEGIN_MACRO pn = (pn2); goto recur; JS_END_MACRO
...
recur:
    ...
    switch (pn->pn_arity) {
      ...
      case PN_LIST:
        if (pn->pn_head)
            TAIL_RELWRSE(pn->pn_head);
        break;
      case PN_TERNARY:
        if (pn->pn_kid1)
            TAIL_RELWRSE(pn->pn_kid1);
        break;
      case PN_BINARY:
        if (pn->pn_left)
            TAIL_RELWRSE(pn->pn_left);
        break;
      case PN_UNARY:
        ...
        if (pn->pn_type == TOK_RP)
            return tt == TOK_LP;
        if (pn->pn_kid)
            TAIL_RELWRSE(pn->pn_kid);
        break;
      case PN_NAME:
        if (pn->pn_type == TOK_DOT || pn->pn_type == TOK_DBLDOT)
            TAIL_RELWRSE(pn->pn_expr);
        /* FALL THROUGH */
    }
    return JS_FALSE;
#undef TAIL_RELWRSE
...
js_FoldConstants(JSContext *cx, JSParseNode *pn, JSTreeContext *tc)
{
    ...
    switch (pn->pn_arity) {
      ...
      case PN_LIST:
        ...
        switch (pn->pn_type) {
          ...
          case TOK_XMLPTAGC:
             ...
             * its super-tree, and we would have to colwert each XML object
      ...
      case PN_NAME:
         ...
         * excessive relwrsion.  Our only goal here is to fold constants (if
    ...
    switch (pn->pn_type) {
      ...
      case TOK_PLUS:
        if (pn->pn_arity == PN_LIST) {
            ...
            /* Ok, we're concatenating: colwert non-string constant operands. */
            length = 0;
            for (pn2 = pn1; pn2; pn2 = pn2->pn_next) {
                if (!FoldType(cx, pn2, TOK_STRING))
                    return JS_FALSE;
                /* XXX fold only if all operands colwert to string */
                if (pn2->pn_type != TOK_STRING)
                    return JS_TRUE;
      ...
      do_binary_op:
        if (pn->pn_arity == PN_LIST) {
            ...
            for (pn2 = pn1; pn2; pn2 = pn2->pn_next) {
                /* XXX fold only if all operands colwert to number */
                if (pn2->pn_type != TOK_NUMBER)
                    break;
            }
            if (!pn2) {
                JSOp op = static_cast<JSOp>(pn->pn_op);
            ...
            if (pn1->pn_type == TOK_NUMBER && pn2->pn_type == TOK_NUMBER) {
                if (!FoldBinaryNumeric(cx, static_cast<JSOp>(pn->pn_op), pn1, pn2, pn, tc))
                    return JS_FALSE;
            }
        }
        break;
