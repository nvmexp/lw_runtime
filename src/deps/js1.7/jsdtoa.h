...
/*
 * Modes for colwerting floating-point numbers to strings.
 *
 * Some of the modes can round-trip; this means that if the number is colwerted to
 * a string using one of these mode and then colwerted back to a number, the result
 * will be identical to the original number (except that, due to ECMA, -0 will get colwerted
 ...
 * colwersion can produce.  This maximum is reached for a number like -0.0000012345678901234567. */
#define DTOSTR_STANDARD_BUFFER_SIZE 26

/* Maximum number of characters (including trailing null) that one of the other colwersions
...
/*
 * Colwert dval according to the given mode and return a pointer to the resulting ASCII string.
...
/*
 * Colwert d to a string in the given base.  The integral part of d will be printed exactly
 ...
 * equal d (except for -0.0, which colwerts to "0", and NaN, which is not equal to itself).
 *
...
/*
 * Clean up any persistent RAM allocated during the exelwtion of DtoA
