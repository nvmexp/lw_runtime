               ...
               jsval *rval)
{
    ...
    /* Compile using the caller's scope chain, which js_Ilwoke passes to fp. */
    fp = cx->fp;
    ...
    scopeobj = js_CheckScopeChailwalidity(cx, scopeobj, js_script_compile);
    if (!scopeobj)
        return JS_FALSE;
     ...
     * flags, because compilation is here separated from exelwtion, and the
...
script_exec(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval)
{
     ...
     * all propagated by js_Exelwte via a non-null fourth (down) argument to
     * js_Exelwte.  If there is no scripted caller, js_Exelwte uses its second
    ...
    scopeobj = js_CheckScopeChailwalidity(cx, scopeobj, js_script_exec);
    if (!scopeobj)
        return JS_FALSE;
...
XDRAtomMap(JSXDRState *xdr, JSAtomMap *map)
{
    ...
    for (i = 0; i != natoms; ++i) {
         ...
         * an unoclwpied element of atoms array.
         */
...
js_XDRScript(JSXDRState *xdr, JSScript **scriptp, JSBool *hasMagic)
{
     ...
     */
    if (xdr->mode == JSXDR_ENCODE)
        magic = JSXDR_MAGIC_SCRIPT_LWRRENT;
    ...
    if (!JS_XDRBytes(xdr, (char *)notes, nsrcnotes * sizeof(jssrcnote)) ||
        !JS_XDRCStringOrNull(xdr, const_cast<char **>(&script->filename)) ||
    ...
    if (xdr->mode == JSXDR_DECODE) {
        ...
        if (filename) {
            ...
            if (!filename)
                goto error;
            JS_free(cx, const_cast<char *>(script->filename));
...
static JSFunctionSpec script_methods[] = {
    ...
    {MKJSID("compile"),         script_compile,         2,0,0},
    {MKJSID("exec"),            script_exec,            1,0,0},
#if JS_HAS_XDR_FREEZE_THAW
    {MKJSID("freeze"),          script_freeze,          0,0,0},
...
JS_FRIEND_DATA(JSClass) js_ScriptClass = {
    ...
    JS_EnumerateStub, JS_ResolveStub,   JS_ColwertStub,   script_finalize,
...
js_compare_strings(const void *k1, const void *k2, void *)
{
...
{
    size_t nbytes = offsetof(ScriptFilenameEntry, filename) + strlen(static_cast<const char*>(key)) + 1;
...
js_InitRuntimeScriptState(JSRuntime *rt)
{
    ...
    rt->scriptFilenameTable =
        JS_NewHashTable(16, JS_HashString, NULL, js_compare_strings, NULL, NULL,
...
typedef struct ScriptFilenamePrefix {
    JSCList     links;      /* cirlwlar list linkage for easy deletion */
...
SaveScriptFilename(JSRuntime *rt, const char *filename, uint32 flags)
{
    ...
    hash = JS_HashString(filename, NULL);
...
#define FILENAME_TO_SFE(fn) \
    ((ScriptFilenameEntry *) (const_cast<char*>(fn) - offsetof(ScriptFilenameEntry, filename)))
...
js_NewScriptFromCG(JSContext *cx, JSCodeGenerator *cg, JSFunction *fun)
{
    ...
    CG_COUNT_FINAL_SRCNOTES(cg, nsrcnotes, uint32);
    CG_COUNT_FINAL_TRYNOTES(cg, ntrynotes, uint32);
     ...
     */
    if (fun) {
        JS_ASSERT(FUN_INTERPRETED(fun) && 0 == FUN_SCRIPT(fun));
...
js_PCToLineNumber(JSContext *cx, JSScript *script, jsbytecode *pc)
{
    ...
    /*
     * General case: walk through source notes aclwmulating their deltas,
...
js_FindFinallyHandler(JSScript *script, jsbytecode *pc)
{
    ...
    do {
        if ((jsuword)(off - tn->start) < (jsuword)tn->length) {
            ...
            op2 = static_cast<JSOp>(pc[JSOP_SETSP_LENGTH]);
