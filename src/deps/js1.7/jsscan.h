...
typedef enum JSTokenType {
    ...
    TOK_LC = 25, TOK_RC = 26,           /* left and right lwrlies (braces) */
...
struct JSTokenStream {
    JSToken             tokens[NTOKENS];/* cirlwlar token buffer */
...
#define LWRRENT_TOKEN(ts)       ((ts)->tokens[(ts)->cursor])
#define ON_LWRRENT_LINE(ts,pos) ((uint16)(ts)->lineno == (pos).end.lineno)
