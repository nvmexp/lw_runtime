...
#define JS_KEYWORD(keyword, idname, type, op, version) #keyword,
...
{
    const char *str1 = keyword_list[*(const unsigned *)a];
    const char *str2 = keyword_list[*(const unsigned *)b];
...
{
    unsigned nlength, lwrrent_length, i, l;
...
generate_letter_switch(struct gen_opt *opt,
                       unsigned indexes[], unsigned nelem,
                       unsigned lwrrent_length)
{
    unsigned *columns;
    unsigned i;

    columns = malloc(sizeof(columns[0]) * lwrrent_length);
    if (!columns) {
        perror("malloc");
        exit(EXIT_FAILURE);
    }
    for (i = 0; i != lwrrent_length; ++i) {
        columns[i] = i;
    }
    generate_letter_switch_r(opt, indexes, nelem, columns, lwrrent_length);
    free(columns);
}
