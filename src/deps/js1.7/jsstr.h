...
struct JSString {
    ...
    typedef jschar value_type;

    // one can use these getter and setter when this is not a dependent string
    void SetLength(size_t l);
    size_t GetLength() const;

    void SetFlags(uintN flags);
    uintN GetFlags() const;
...
#define JSSTRING_CHARS(str)         (JSSTRING_IS_DEPENDENT(str)               \
                                     ...
                                     : (str)->chars)
#if defined __cplusplus
#define JSSTRING_CONSTCHARS(str)    (JSSTRING_IS_DEPENDENT(str)               \
                                     ? JSSTRCONSTDEP_CHARS(str)               \
                                     : (str)->chars)
#endif
...
#if defined __cplusplus
#define JSSTRCONSTDEP(str)          ((const JSDependentString *)(str))
#endif

#if defined __cplusplus
#define JSSTRDEP_START(str)         (JSSTRING_IS_PREFIX(str) ? 0              \
                                     : ((JSSTRCONSTDEP(str)->length           \
...
#if defined __cplusplus
#define JSSTRDEP_LENGTH(str)        (JSSTRCONSTDEP(str)->length               \
...
#if defined __cplusplus
#define JSSTRCONSTDEP_BASE(str)     (JSSTRCONSTDEP(str)->base)
#endif
...
#if defined __cplusplus
#define JSSTRCONSTDEP_CHARS(str)                                              \
    (JSSTRING_IS_DEPENDENT(JSSTRCONSTDEP_BASE(str))                           \
     ? js_GetDependentStringChars(str)                                        \
     : JSSTRCONSTDEP_BASE(str)->chars + JSSTRDEP_START(str))
#endif
...
/*
 * Shorthands for ASCII (7-bit) decimal and hex colwersion.
...
/*
 * Colwert a value to a printable C string.
 */
...
/*
 * Colwert a value to a string, returning null after reporting an error,
...
/*
 * Colwert a value to its source expression, returning null after reporting
...
inline
void JSString::SetLength(size_t l) {
    length = l;
}

inline
size_t JSString::GetLength() const {
    return length;
}

inline
void JSString::SetFlags(uintN flags) {
    length = (length & JSSTRING_LENGTH_MASK) | (static_cast<size_t>(flags) << JSSTRING_LENGTH_BITS);
}

inline
uintN JSString::GetFlags() const {
    return static_cast<uintN>(length >> JSSTRING_LENGTH_BITS);
}

inline
void JSString::SetDependent() {
    length |= JSSTRFLAG_DEPENDENT;
}

inline
void JSString::ClearDependent() {
    length &= ~JSSTRFLAG_DEPENDENT;
}

inline
bool JSString::IsDependent() const {
    return 0 != (length & JSSTRFLAG_DEPENDENT);
}

inline
void JSString::SetPrefix() {
    length |= JSSTRFLAG_PREFIX;
}

inline
void JSString::ClearPrefix() {
    length &= ~JSSTRFLAG_PREFIX;
}

inline
bool JSString::IsPrefix() const {
    return 0 != (length & JSSTRFLAG_PREFIX);
}
#endif
