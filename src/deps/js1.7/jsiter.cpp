...
extern "C" const char js_throw_str[]; /* from jsscan.h */
...
js_ValueToIterator(JSContext *cx, uintN flags, jsval *vp)
{
    ...
    if (!JSVAL_IS_PRIMITIVE(*vp)) {
         ...
         * This is contrary to ECMA-262 9.9 ToObject, ilwoked from step 3 of
    ...
    if (JSVAL_IS_VOID(*vp)) {
      default_iter:
        ...
        *vp = OBJECT_TO_JSVAL(iterobj);
        ...
        if (!js_InternalIlwoke(cx, obj, *vp, JSILWOKE_ITERATOR, 1, &arg, vp))
            goto bad;
        if (JSVAL_IS_PRIMITIVE(*vp)) {
...
JSClass js_GeneratorClass = {
    ...
    JS_PropertyStub,  JS_PropertyStub, JS_PropertyStub, JS_PropertyStub,
    JS_EnumerateStub, JS_ResolveStub,  JS_ColwertStub,  generator_finalize,
    NULL,             NULL,            NULL,            NULL,
...
{
    JSObject *obj;
    uintN argc, nargs, lwars, depth, nslots;
    JSGenerator *gen;
    jsval *newsp;
    ...
    /* Copy call-ilwariant script and function references. */
...
#define COPY_STACK_ARRAY(vec,cnt,num)                                         \
    ...
    *newsp++ = fp->argv[-1];
    ...
    COPY_STACK_ARRAY(vars, lwars, lwars);
