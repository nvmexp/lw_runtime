 ...
 *
 * This header is included only in other .h files, for colwenience and for
...
#define JSID_ATOM                   jsid::Tags::Atom
#define JSID_INT                    jsid::Tags::Integer
#define JSID_OBJECT                 jsid::Tags::Object
#define JSID_TAG(id)                (id).GetTag()
#define JSID_CLRTAG(id)             ((id) & ~jsid::tagMask)

#define JSID_IS_ATOM(id)            (id).IsAtom()
#define JSID_TO_ATOM(id)            (id).ToAtom()
#define ATOM_TO_JSID(atom)          jsid::FromAtom(atom)
#define ATOM_JSID_TO_JSVAL(id)      (id).ToAtom()->GetKey()

#define JSID_IS_INT(id)             (id).IsInt()
#define JSID_TO_INT(id)             (id).ToInt()
#define INT_TO_JSID(i)              jsid::FromInt(i)
#define INT_JSID_TO_JSVAL(id)       (id)
#define INT_JSVAL_TO_JSID(v)        (v)

#define JSID_IS_OBJECT(id)          (id).IsObject()
#define JSID_TO_OBJECT(id)          (id).ToObject()
#define OBJECT_TO_JSID(obj)         jsid::FromObject(obj)
#define OBJECT_JSID_TO_JSVAL(id)    jsval::FromObject((id).ToObject())
#define OBJECT_JSVAL_TO_JSID(v)     jsid::FromObject((v).ToObject())
#else
...
#define INT_TO_JSID(i)              (((jsid)(i) << 1) | JSID_INT)
#define INT_JSID_TO_JSVAL(id)       (id)
#define INT_JSVAL_TO_JSID(v)        (v)
...
typedef struct JSXMLArrayLwrsor     JSXMLArrayLwrsor;
...
/*
 * This hook captures high level script exelwtion and function calls (JS or
 * native).  It is used by JS_SetExelwteHook to hook top level scripts and by
 * JS_SetCallHook to hook function calls.  It will get called twice per script
 * or function call: just before exelwtion begins and just after it finishes.
 * In both cases the 'current' frame is that of the exelwting code.
 *
 * The 'before' param is JS_TRUE for the hook invocation before the exelwtion
 * and JS_FALSE for the invocation after the code has run.
 *
 * The 'ok' param is significant only on the post exelwtion invocation to
 * signify whether or not the code completed 'normally'.
 *
 * The 'closure' param is as passed to JS_SetExelwteHook or JS_SetCallHook
...
typedef void
(*JS_DLL_CALLBACK JSConcatHook)(JSContext *cx, JSString *left, JSString *right,
                                JSString *result, JSBool before, void *closure);

typedef void
(*JS_DLL_CALLBACK JSFinalizeStrHook)(JSContext *cx, JSString *str, void *closure);

typedef void
(*JS_DLL_CALLBACK JSEvalHook)(JSContext *cx, JSScript **script, void *closure);

typedef void
(*JS_DLL_CALLBACK JSAddPropHook)(JSContext *cx, JSScope *scope, jsid id, JSPropertyOp getter,
                                 JSPropertyOp setter, uint32 slot, uintN attrs, uintN flags,
                                 intN shortid, void *closure);
typedef void
(*JS_DLL_CALLBACK JSPropLookupHook)(JSContext *cx, JSScope *scope, jsid id, JSPropertyOp getter,
                                    JSPropertyOp setter, uint32 slot, uintN attrs, uintN flags,
                                    intN shortid, void *closure);

typedef void
(*JS_DLL_CALLBACK JSCheckIndexHook)(JSContext *cx, jsid *id, void *closure);

typedef void
(*JS_DLL_CALLBACK JSDefaultNumHook)(JSContext *cx, jsval v, JSBool before, void *closure);

enum ArrayJoinOp {
    ARRAY_JOIN_APPEND_STR,
    ARRAY_JOIN_FINISH_STR,
    ARRAY_JOIN_SEPARATOR,
    ARRAY_JOIN_FINISH_SEP,
    ARRAY_JOIN_RESULT
};

typedef void
(*JS_DLL_CALLBACK JSArrayJoinHook)(JSContext *cx, JSString *str, enum ArrayJoinOp op, void *closure);

typedef void
(*JS_DLL_CALLBACK JSExpandErrArgHook)(
    JSContext *cx, const jschar **messageArgs, JSBool before, void *closure
);

typedef void
(*JS_DLL_CALLBACK JSValDecompileHook)(JSContext *cx, JSScript *script, JSScope *scope,
                                      JSBool before, void *closure);
