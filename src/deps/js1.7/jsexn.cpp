...
JSClass js_ErrorClass = {
    ...
    exn_enumerate,    (JSResolveOp)exn_resolve, JS_ColwertStub, exn_finalize,
...
#define JS_CHARS_SIZE(jschars) ((js_strlen(jschars) + 1) * sizeof(jschar))
    ...
    if (argsArraySize != 0) {
        ...
        JS_ASSERT(cursor == (const uint8 *)copy->messageArgs[0] + argsCopySize);
    }
...
#define APPEND_CHAR_TO_STACK(c)                                               \
    JS_BEGIN_MACRO                                                            \
        if (stacklen == stackmax) {                                           \
            ...
            if (!ptr_)                                                        \
                goto bad;                                                     \
            stackbuf = static_cast<jschar*>(ptr_);                            \
        }                                                                     \
        stackbuf[stacklen++] = (c);                                           \
    JS_END_MACRO
...
#define APPEND_STRING_TO_STACK(str)                                                    \
    JS_BEGIN_MACRO                                                                     \
        ...
        if (length_ > stackmax - stacklen) {                                           \
            ...
            stackmax = (size_t)JS_BIT(JS_CeilingLog2((JSUint32)(stacklen + length_))); \
            ptr_ = JS_realloc(cx, stackbuf, (stackmax+1) * sizeof(jschar));            \
            if (!ptr_)                                                                 \
                goto bad;                                                              \
            stackbuf = static_cast<jschar*>(ptr_);                                     \
...
#undef STACK_LENGTH_LIMIT
    ...
    if (stacklen < stackmax) {
        ...
        if (shrunk)
            stackbuf = static_cast<jschar*>(shrunk);
    }
...
js_InitExceptionClasses(JSContext *cx, JSObject *obj)
{
    ...
    /*
     * If lazy class initialization oclwrs for any Error subclass, then all
...
js_ErrorToException(JSContext *cx, const char *message, JSErrorReport *reportp)
{
    ...
    exn = static_cast<JSExnType>(errorString ? errorString->exnType : JSEXN_NONE);
    ...
    /*
     * Prevent runaway relwrsion, just as the Exception native constructor
     * must do, via cx->creatingException.  If an out-of-memory error oclwrs,
...
js_ReportUncaughtException(JSContext *cx)
{
    ...
    if (!str) {
        bytes = "unknown (can't colwert to string)";
    } else {
        if (exnObject)
