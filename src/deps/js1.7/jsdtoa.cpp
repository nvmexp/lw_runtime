...
/*
 * Portable double to alphanumeric string and back colwerters.
 */
#include "jsstddef.h"
#include "jslibmath.h"
#include "jstypes.h"
 ...
 * before ilwoking strtod or dtoa.  If the machine uses (the equivalent
 ...
 * Point Numbers Aclwrately" [Proc. ACM SIGPLAN '90, pp. 92-101].
 *
 * Modifications:
 *
 ...
 * #define Inaclwrate_Divide for IEEE-format with correctly rounded
 *  products but inaclwrate quotients, e.g., for Intel i860.
 ...
 *  appropriate.  If MALLOC is undefined, malloc will be ilwoked
 ...
 *  such as decimal-to-binary colwersion of a very long string of
 *  digits.
 ...
 *  You must also ilwoke freedtoa(s) to free the value s returned by
...
/* Private memory lwrrently doesn't work with JS_THREADSAFE */
...
/* Colwert d into the form b*2^e, where b is an odd integer.  b is the returned
...
/* dtoa for IEEE arithmetic (dmg): colwert double to ASCII string.
 *
 * Inspired by "How to Print Floating-Point Numbers Aclwrately" by
 ...
 *     quantities.
 *  5. When colwerting floating-point integers less than 1e16,
 *     we use floating-point arithmetic rather than resorting
 *     to multiple-precision integers.
 *  6. When asked to produce fewer than 15 digits, we first try
 *     to get by with floating-point arithmetic; we resort to
 *     multiple-precision integer arithmetic only if we cannot
 *     guarantee that the floating-point callwlation has given
    ...
    int *decpt, int *sign, char **rve, char *buf, size_t bufsize)
{
    ...
    */
    ...
    if ((word0(d) & Exp_mask) == Exp_mask) {
        /* Infinity or NaN */
        *decpt = 9999;
        s = !word1(d) && !(word0(d) & Frac_mask) ? const_cast<char *>("Infinity") : const_cast<char *>("NaN");
...
/*          JS_SetError(JS_BUFFER_OVERFLOW_ERROR, 0); */
    ...
    if ((i = (int32)(word0(d) >> Exp_shift1 & (Exp_mask>>Exp_shift1))) != 0) {
         ...
         * (We could get a more accurate k by ilwoking log10,
    ...
    /* Arrange for colwenient computation of quotients:
...
JS_dtostr(char *buffer, size_t bufferSize, JSDToStrMode mode, int precision, double d)
{
    ...
    if (decPt != 9999) {
        ...
        if (exponentialNotation) {
            /* Insert a decimal point if more than one significant digit */
            if (nDigits != 1) {
                numBegin--;
                numBegin[0] = numBegin[1];
                numBegin[1] = '.';
            }
            JS_snprintf(numEnd, (JSUint32)(bufferSize - (numEnd - buffer)), "e%+d", decPt-1);
 ...
 * which oclwrs when printing -5e-324 in binary.  We could compute a better estimate of the size of
