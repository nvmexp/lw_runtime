 ...
 * can mutate the table.  The relwrsion level is stored in additional
...
#define RELWRSION_LEVEL(table_) (*(uint32*)(table_->entryStore + \
                                            JS_DHASH_TABLE_SIZE(table_) * \
                                            table_->entrySize))

#define ENTRY_STORE_EXTRA                   sizeof(uint32)
#define INCREMENT_RELWRSION_LEVEL(table_)   (++RELWRSION_LEVEL(table_))
#define DECREMENT_RELWRSION_LEVEL(table_)   (--RELWRSION_LEVEL(table_))

#else

#define ENTRY_STORE_EXTRA 0
#define INCREMENT_RELWRSION_LEVEL(table_)   ((void)1)
#define DECREMENT_RELWRSION_LEVEL(table_)   ((void)0)
...
JS_DHashStringKey(JSDHashTable *table, const void *key)
{
    ...
    h = 0;
    for (s = static_cast<const unsigned char *>(key); *s != '\0'; s++)
                       ...
                       const void *key)
{
    ...
    return stub->key == key ||
           (stub->key && key && strcmp(static_cast<const char *>(stub->key), static_cast<const char *>(key)) == 0);
}
                  ...
                  uint32 entrySize, uint32 capacity)
{
    ...
    table->entryStore = static_cast<char *>(ops->allocTable(table, nbytes + ENTRY_STORE_EXTRA));
    if (!table->entryStore)
        return JS_FALSE;
...
JS_DHashTableFinish(JSDHashTable *table)
{
    ...
    INCREMENT_RELWRSION_LEVEL(table);
    ...
    DECREMENT_RELWRSION_LEVEL(table);
    JS_ASSERT(RELWRSION_LEVEL(table) == 0);
...
ChangeTable(JSDHashTable *table, int deltaLog2)
{
    ...
    newEntryStore = static_cast<char *>(table->ops->allocTable(table, nbytes + ENTRY_STORE_EXTRA));
    if (!newEntryStore)
        return JS_FALSE;

    /* We can't fail from here on, so update table parameters. */
#ifdef DEBUG
    relwrsionLevel = RELWRSION_LEVEL(table);
#endif
...
#ifdef DEBUG
    RELWRSION_LEVEL(table) = relwrsionLevel;
#endif
...
JS_DHashTableOperate(JSDHashTable *table, const void *key, JSDHashOperator op)
{
    ...
    JS_ASSERT(op == JS_DHASH_LOOKUP || RELWRSION_LEVEL(table) == 0);
    INCREMENT_RELWRSION_LEVEL(table);
...
JS_DHashTableEnumerate(JSDHashTable *table, JSDHashEnumerator etor, void *arg)
{
    ...
    JS_ASSERT(!didRemove || RELWRSION_LEVEL(table) == 1);
