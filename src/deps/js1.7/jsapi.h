...
#define JSVAL_OBJECT            jsval::Tags::Object  /* untagged reference to object */
#define JSVAL_INT               jsval::Tags::Integer /* tagged 31-bit integer value */
#define JSVAL_DOUBLE            jsval::Tags::Double  /* tagged reference to double */
#define JSVAL_STRING            jsval::Tags::String  /* tagged reference to string */
#define JSVAL_BOOLEAN           jsval::Tags::Boolean /* tagged boolean value */

/* Type tag bitfield length and derived macros. */
#define JSVAL_TAGBITS           jsval::tagBits
#define JSVAL_TAG(v)            (v).GetTag()
#define JSVAL_ALIGN             jsval::align

/* Predicates for type testing. */
#define JSVAL_IS_OBJECT(v)      (v).IsObject()
#define JSVAL_IS_NUMBER(v)      (v).IsNumber()
#define JSVAL_IS_INT(v)         (v).IsInt()
...
#define JSVAL_IS_VOID(v)        (v).IsVoid()
#define JSVAL_IS_PRIMITIVE(v)   (v).IsPrimitive()

/* Objects, strings, and doubles are GC'ed. */
#define JSVAL_IS_GCTHING(v)     (v).IsGcThing()
#define JSVAL_TO_GCTHING(v)     (v).ToGcThing()
#define JSVAL_TO_OBJECT(v)      (v).ToObject()
#define JSVAL_TO_DOUBLE(v)      (v).ToDouble()
#define JSVAL_TO_STRING(v)      (v).ToString()
#define OBJECT_TO_JSVAL(obj)    jsval::FromObject(obj)
#define DOUBLE_TO_JSVAL(dp)     jsval::FromDouble(dp)
#define STRING_TO_JSVAL(str)    jsval::FromString(str)

/* Lock and unlock the GC thing held by a jsval. */
#define JSVAL_LOCK(cx,v)        (v).Lock(cx)
#define JSVAL_UNLOCK(cx,v)      (v).UnLock(cx)

#define JSVAL_INT_BITS          jsval::intBits
#define JSVAL_INT_MIN           jsval::intMin
#define JSVAL_INT_MAX           jsval::intMax
#define INT_FITS_IN_JSVAL(i)    jsval::IntFits(i)
#define JSVAL_TO_INT(v)         (v).ToInt()
// we need jsval::value_type(i) colwersion because MSVC has issues with
// is_colwrtible in jsval constructor
#define INT_TO_JSVAL(i)         jsval::FromInt(jsval::value_type(i))

/* Colwert between boolean and jsval. */
#define JSVAL_TO_BOOLEAN(v)     (v).ToBoolean()
#define BOOLEAN_TO_JSVAL(b)     jsval::FromBoolean(b)

/* A private data pointer (2-byte-aligned) can be stored as an int jsval. */
#define JSVAL_TO_PRIVATE(v)     (v).ToPrivate()
#define PRIVATE_TO_JSVAL(p)     jsval::FromPrivate(p)
#else
/*
 ...
 */
#if defined(JS_USE_STRONGLY_TYPED_JSVAL_AND_JSID)
#define JSVAL_VOID              jsval(jsval::Void)
#define JSVAL_NULL              jsval(jsval::Null)
#else
 ...
 * specifying the tabulated type colwersions:
 *
 *   b      JSBool          Boolean
 ...
 *   v      jsval           Argument value (no colwersion)
 ...
 * uncolwerted arguments.
 */
extern JS_PUBLIC_API(JSBool)
JS_ColwertArguments(JSContext *cx, uintN argc, jsval *argv, const char *format,
                    ...);

#ifdef va_start
extern JS_PUBLIC_API(JSBool)
JS_ColwertArgumentsVA(JSContext *cx, uintN argc, jsval *argv,
                      const char *format, va_list ap);
#endif

/*
 * Ilwerse of JS_ColwertArguments: scan format and colwert trailing arguments
...
/*
 * Add and remove a format string handler for JS_{Colwert,Push}Arguments{,VA}.
 ...
 * the formatter to colwert zero or more jsvals at sp to zero or more C values
 * accessed via pointers-to-values at ap, updating both sp (via *vpp) and ap
 * (via *app) to point past the colwerted arguments and their result pointers
 * on the C stack.
 *
 * When called from JS_PushArgumentsVA, the formatter is ilwoked thus:
 *
 ...
 * JS_{Colwert,Push}Arguments{,VA}.  The actual format trailing substring used
 * in each Colwert or PushArguments call is passed to the formatter, so that
...
extern JS_PUBLIC_API(JSBool)
JS_ColwertValue(JSContext *cx, jsval v, JSType type, jsval *vp);
...
/*
 * Colwert a value to a number, then to an int32, according to the ECMA rules
 * for ToInt32.
 */
extern JS_PUBLIC_API(JSBool)
...
/*
 * Colwert a value to a number, then to a uint32, according to the ECMA rules
 * for ToUint32.
 */
extern JS_PUBLIC_API(JSBool)
...
/*
 * Colwert a value to a number, then to an int32 if it fits by rounding to
...
JS_NewContext(JSRuntime *rt, size_t stackChunkSize, JSHashFunction hashFun, void *hashFunData,
              JSHashKeyComparator compareAtomKeys, void *compareAtomKeysData);
 ...
 * prototypes for JS_ExelwteScript, JS_EvaluateScript, etc.
 */
#define JSOPTION_STRICT         JS_BIT(0)       /* warn on dubious practice */
#define JSOPTION_WERROR         JS_BIT(1)       /* colwert warning to error */
...
#define JSOPTION_DONT_REPORT_UNCAUGHT \
                                JS_BIT(8)       /* When returning from the
                                                   ...
                                                   being colwerted to error
                                                   reports */

#define JSOPTION_NO_EXN_ON_REPORT_ERR \
                                JS_BIT(9)       /* Do not generate exceptions for
                                                   reported errors */
...
extern JS_PUBLIC_API(const char *)
JS_GetImplementatiolwersion(void);
 ...
 * roots help native code protect newly-created GC-things from GC ilwocations
 ...
 *
 * The map function should return JS_MAP_GCROOT_REMOVE to cause the lwrrently
...
#if defined(JS_USE_STRONGLY_TYPED_JSVAL_AND_JSID)
inline JSBool jsval::Lock(JSContext *cx)
{
    return  IsGcThing() ? JS_LockGCThing(cx, ToGcThing()) : JS_TRUE;
}

inline JSBool jsval::UnLock(JSContext *cx)
{
    return  IsGcThing() ? JS_UnlockGCThing(cx, ToGcThing()) : JS_TRUE;
}
#endif
 ...
 * be ilwoked on the primordial thread only, at startup -- or else the entire
...
#define JSCLASS_CONSTRUCT_PROTOTYPE     (1<<6)  /* call constructor on class
                                                   prototype */
#define JSCLASS_DOLWMENT_OBSERVER       (1<<7)  /* DOM document observer */
 ...
 * colwert from and to a jsval using JS_ValueToId and JS_IdToValue.
 */
struct JSProperty {
    jsid id;
};
 ...
 * should handle this id specially before colwerting id via JSVAL_TO_INT.
 */
...
extern JS_PUBLIC_API(JSBool)
JS_ColwertStub(JSContext *cx, JSObject *obj, JSType type, jsval *vp);
 ...
 * creating the script via JS_Compile* and before a JS_ExelwteScript* call.
 ...
 *    do {
 *        jsval result;
 *        JS_ExelwteScript(cx, global, script, &result);
 *        JS_GC();
...
/*
 * NB: JS_ExelwteScript, JS_ExelwteScriptPart, and the JS_Evaluate*Script*
 ...
 * 'var' and 'function' bind names) of the exelwtion context for script.
 *
 ...
 * exelwted with the variables object equal to this global object.  But these
 ...
 */
extern JS_PUBLIC_API(JSBool)
JS_ExelwteScript(JSContext *cx, JSObject *obj, JSScript *script, jsval *rval);
...
extern JS_PUBLIC_API(JSBool)
JS_ExelwteScriptPart(JSContext *cx, JSObject *obj, JSScript *script,
...
extern JS_PUBLIC_API(JSBool)
JS_CallFunctiolwalue(JSContext *cx, JSObject *obj, jsval fval, uintN argc,
                     jsval *argv, jsval *rval);
...
/*
 * Returns true if a script is exelwting and its current bytecode is a set
 ...
 */
extern JS_PUBLIC_API(void)
JS_SetCallReturlwalue2(JSContext *cx, jsval v);
 ...
 * You may call JS_MakeStringImmutable to colwert the string from a mutable
 * (growable or dependent) string to an immutable (and therefore thread-safe)
 * string.  The engine takes care of colwerting growable and dependent strings
...
/*
 * Colwert a dependent string into an independent one.  This function does not
...
/*
 * Colwert a mutable string (either growable or dependent) into an immutable,
 * thread-safe one.
 */
extern JS_PUBLIC_API(JSBool)
...
/*
 * Locale specific string colwersion and error message callbacks.
 */
...
/*
 * Report an exception represented by the sprintf-like colwersion of format
 ...
 * warning was not colwerted into an error due to the JSOPTION_WERROR option
 ...
 * JS_ExelwteScript returns failure, and signal or propagate the exception, as
 * appropriate.
 */
