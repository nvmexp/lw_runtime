    ...
    JS_BEGIN_MACRO                                                            \
        if ((xdr)->mode == JSXDR_ENCODE) {                                    \
            if (MEM_LIMIT(xdr) &&                                             \
                ...
                if (!data_)                                                   \
                    return 0;                                                 \
                MEM_BASE(xdr) = static_cast<char*>(data_);                    \
...
JS_XDRNewMem(JSContext *cx, JSXDRMode mode)
{
    ...
    if (mode == JSXDR_ENCODE) {
        if (!(MEM_BASE(xdr) = static_cast<char *>(JS_malloc(cx, MEM_BLOCK)))) {
            JS_free(cx, xdr);
            return NULL;
        }
    } else {
...
JS_XDRMemSetData(JSXDRState *xdr, void *data, uint32 len)
{
    if (xdr->ops != &xdrmem_ops)
        return;
    MEM_LIMIT(xdr) = len;
    MEM_BASE(xdr) = static_cast<char*>(data);
    MEM_COUNT(xdr) = 0;
}
...
JS_XDRDestroy(JSXDRState *xdr)
{
    ...
    if (xdr->registry) {
        ...
        if (xdr->reghash)
            JS_DHashTableDestroy(static_cast<JSDHashTable*>(xdr->reghash));
    }
    JS_free(cx, xdr);
}
...
JS_XDRBytes(JSXDRState *xdr, char *bytes, uint32 len)
{
    ...
    if (len % JSXDR_ALIGN) {
        ...
        if (xdr->mode == JSXDR_ENCODE) {
            ...
            if (!xdr->ops->seek(xdr, padlen, JSXDR_SEEK_LWR))
                return JS_FALSE;
        }
    }
    return JS_TRUE;
...
/**
 * Colwert between a C string and the XDR representation:
...
/*
 * Colwert between a JS (Unicode) string and the XDR representation.
 */
JS_PUBLIC_API(JSBool)
JS_XDRString(JSXDRState *xdr, JSString **strp)
{
    ...
    if (xdr->mode == JSXDR_ENCODE)
        nchars = (uint32)JSSTRING_LENGTH(*strp);
...
/*
 * FIXME: This performs lossy colwersion and we need to switch to
...
JS_XDRRegisterClass(JSXDRState *xdr, JSClass *clasp, uint32 *idp)
{
    ...
    if (xdr->reghash) {
        JSRegHashEntry *entry = (JSRegHashEntry *)
            JS_DHashTableOperate(static_cast<JSDHashTable*>(xdr->reghash), clasp->name, JS_DHASH_ADD);
        if (!entry) {
...
JS_XDRFindClassIdByName(JSXDRState *xdr, const char *name)
{
    ...
    if (numclasses >= 10) {
            ...
            if (xdr->reghash) {
                for (i = 0; i < numclasses; i++) {
                    ...
                    entry = (JSRegHashEntry *)
                        JS_DHashTableOperate(static_cast<JSDHashTable*>(xdr->reghash), clasp->name,
                                             JS_DHASH_ADD);
                    entry->name = clasp->name;
                    entry->index = i;
                }
        ...
        if (xdr->reghash) {
            entry = (JSRegHashEntry *)
                JS_DHashTableOperate(static_cast<JSDHashTable*>(xdr->reghash), name, JS_DHASH_LOOKUP);
