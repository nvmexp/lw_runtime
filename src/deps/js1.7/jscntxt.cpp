...
{
    JSThread *thread = js_GetLwrrentThread(cx->runtime);
...
{
#ifdef DEBUG
    JSVersion version = static_cast<JSVersion>(JSVERSION_NUMBER(cx));
              ...
              JSHashKeyComparator compareAtomKeys, void *compareAtomKeysData)
{
    ...
    if (first) {
        ...
        ok = (rt->atomState.liveAtoms == 0)
             ? js_InitAtomState(cx, &rt->atomState, hashFun, hashFunData, compareAtomKeys, compareAtomKeysData)
...
js_DestroyContext(JSContext *cx, JSDestroyContextMode mode)
{
    ...
    /* Destroy the resolve relwrsion damper. */
...
resolving_HashKey(JSDHashTable *table, const void *ptr)
{
    ...
    return (JSDHashNumber)(((JSDHashNumber)JS_PTR_TO_UINT32(key->obj) >> JSVAL_TAGBITS) ^ key->id);
}
                  ...
                  JSResolvingEntry **entryp)
{
    ...
    if (entry->flags & flag) {
        /* An entry for (key, flag) exists already -- dampen relwrsion. */
        entry = NULL;
    } else {
...
ReportError(JSContext *cx, const char *message, JSErrorReport *reportp)
{
     ...
     * with the old scheme.
     */
    if ((cx->options & JSOPTION_NO_EXN_ON_REPORT_ERR) ||
 ...
 * Instead we just ilwoke the errorReporter with an "Out Of Memory"
 ...
 * message is placed into reportp->ucmessage colwerted to a JSString.
 *
                        ...
                        JSBool *warningp, JSBool charArgs, va_list ap)
{
    ...
    *messagep = NULL;
    ...
    JSExpandErrArgHook hook = cx->runtime->expandErrArgHook;
    void *hookData = cx->runtime->expandErrArgHookData;
    ...
    if (efs) {
        ...
        if (hook) {
            hook(cx, reportp->messageArgs, JS_TRUE, hookData);
        }
        ...
        if (argCount > 0) {
            if (efs->format) {
                ...
                /*
                * Note - the above callwlation assumes that each argument
    ...
    if (*messagep == NULL) {
        ...
        *messagep = (char *)JS_malloc(cx, nbytes);
        if (!*messagep)
            goto error;
        JS_snprintf(*messagep, (JSUint32)nbytes, defaultErrorMessage, errorNumber);
    }
    return JS_TRUE;

error:
    if (hook) {
        hook(cx, reportp->messageArgs, JS_FALSE, hookData);
    }
    if (reportp->messageArgs) {
        ...
        if (charArgs) {
            ...
            while (reportp->messageArgs[i])
                JS_free(cx, const_cast<jschar*>((reportp->messageArgs[i++])));
        }
        JS_free(cx, (void *)reportp->messageArgs);
        reportp->messageArgs = NULL;
    }
    if (reportp->ucmessage) {
        JS_free(cx, const_cast<jschar*>(reportp->ucmessage));
                       ...
                       JSBool charArgs, va_list ap)
{
    ...
    void *hookData = cx->runtime->expandErrArgHookData;
    if (hook) {
        hook(cx, report.messageArgs, JS_FALSE, hookData);
    }
    ...
    if (report.messageArgs) {
        ...
        if (charArgs) {
            ...
            while (report.messageArgs[i])
                JS_free(cx, const_cast<jschar*>(report.messageArgs[i++]));
        }
        JS_free(cx, (void *)report.messageArgs);
    }
    if (report.ucmessage)
        JS_free(cx, const_cast<jschar*>(report.ucmessage));
