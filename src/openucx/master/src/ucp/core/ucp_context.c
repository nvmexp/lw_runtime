...
static ucs_config_field_t ucp_config_table[] = {
  ...
  {"TLS", UCP_RSC_CONFIG_ALL,
   ...
   " - lwca    : LWCA (LWPU GPU) memory support.\n"
  ...
  {"WARN_ILWALID_CONFIG", "y",
   "Issue a warning in case of invalid device and/or transport configuration.",
   ucs_offsetof(ucp_config_t, warn_ilwalid_config), UCS_CONFIG_TYPE_BOOL},
  ...
  {"RNDV_THRESH_FALLBACK", "inf",
   "Message size to start using the rendezvous protocol in case the callwlated threshold "
   "is zero or negative",
  ...
  {"MAX_EAGER_RAILS", "1",
   "Maximal number of devices on which an eager operation may be exelwted in parallel",
   ucs_offsetof(ucp_config_t, ctx.max_eager_lanes), UCS_CONFIG_TYPE_UINT},
  ...
  {"MAX_RNDV_RAILS", "2",
   "Maximal number of devices on which a rendezvous operation may be exelwted in parallel",
   ucs_offsetof(ucp_config_t, ctx.max_rndv_lanes), UCS_CONFIG_TYPE_UINT},
  ...
  {"TM_THRESH", "1024", /* TODO: callwlate automatically */
  ...
  {"TM_MAX_BB_SIZE", "1024", /* TODO: callwlate automatically */
  ...
  {"TM_FORCE_THRESH", "8192", /* TODO: callwlate automatically */
  ...
  {"MEMTYPE_CACHE", "y",
   "Enable memory type (lwca/rocm) cache \n",
...
static ucp_tl_alias_t ucp_tl_aliases[] = {
  ...
  { "lwca",  { "lwda_copy", "lwda_ipc", "gdr_copy", NULL } },
...
{
    unsigned full_prefix_len = sizeof(UCS_DEFAULT_ELW_PREFIX) + 1;
    unsigned elw_prefix_len  = 0;
    ucp_config_t *config;
    ucs_status_t status;
    ...
    if (elw_prefix != NULL) {
        elw_prefix_len   = strlen(elw_prefix);
        full_prefix_len += elw_prefix_len;
    }

    config->elw_prefix = ucs_malloc(full_prefix_len, "ucp config");
    if (config->elw_prefix == NULL) {
        status = UCS_ERR_NO_MEMORY;
        goto err_free_config;
    }

    if (elw_prefix_len != 0) {
        ucs_snprintf_zero(config->elw_prefix, full_prefix_len, "%s_%s",
                          elw_prefix, UCS_DEFAULT_ELW_PREFIX);
    } else {
        ucs_snprintf_zero(config->elw_prefix, full_prefix_len, "%s",
                          UCS_DEFAULT_ELW_PREFIX);
    }

    status = ucs_config_parser_fill_opts(config, ucp_config_table,
                                         config->elw_prefix, NULL, 0);
    if (status != UCS_OK) {
                      ...
                      const char *title, ucs_config_print_flags_t print_flags)
{
    ucs_config_parser_print_opts(stream, title, config, ucp_config_table,
                                 NULL, UCS_DEFAULT_ELW_PREFIX, print_flags);
}
                                    ...
                                    const ucp_config_t *config)
{
    ...
    if (context->config.ext.estimated_num_eps != UCS_ULUNITS_AUTO) {
        /* num_eps was set via the elw variable. Override current value */
    ...
    if (context->config.ext.estimated_num_ppn != UCS_ULUNITS_AUTO) {
        /* num_ppn was set via the elw variable. Override current value */
    ...
    if (context->config.ext.bcopy_bw == UCS_BANDWIDTH_AUTO) {
        /* bcopy_bw wasn't set via the elw variable. Callwlate the value */
    ...
    context->config.elw_prefix = ucs_strdup(config->elw_prefix, "ucp config");
    if (context->config.elw_prefix == NULL) {
        status = UCS_ERR_NO_MEMORY;
        goto err;
    }
...
err_free_elw_prefix:
    ucs_free(context->config.elw_prefix);
err:
