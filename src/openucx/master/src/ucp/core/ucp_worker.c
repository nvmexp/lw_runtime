                                           ...
                                           const ucp_worker_params_t *params)
{
     ...
     *        signaling in RNDV and similar cases, see colwersation in PR #1277
     */
...
static unsigned ucp_worker_iface_err_handle_progress(void *arg)
{
    ...
    if (!(ucp_ep->flags & UCP_EP_FLAG_USED)) {
        ...
        ucp_ep_ilwoke_err_cb(ucp_ep, key.status);
    }
                                      ...
                                      ucs_status_t status)
{
    ...
    /* ilwoke the rest of the error handling flow from the main thread */
    ...
    if ((ucp_ep_ext_gen(ucp_ep)->err_cb == NULL) &&
        ...
        if (lane != UCP_NULL_LANE) {
            ...
            ucs_log(log_level, "error '%s' oclwrred on wireup will not be "
                                        ...
                                        ucp_rsc_index_t *better_index)
{
    ...
    uint64_t test_flags;
    double latency_iter, latency_lwr, bw_lwr;

    ucs_assert(wiface != NULL);

    latency_lwr = ucp_tl_iface_latency(ctx, &wiface->attr.latency);
    bw_lwr      = ucp_tl_iface_bandwidth(ctx, &wiface->attr.bandwidth);
    ...
    for (rsc_index = 0; rsc_index < ctx->num_tls; ++rsc_index) {
        ...
        if (/* 1. Supports all capabilities of the target iface (at least),
            ...
            (ucp_tl_iface_bandwidth(ctx, &if_iter->attr.bandwidth) >= bw_lwr) &&
            /* swap latencies in args list since less is better */
            (ucp_score_prio_cmp(latency_lwr,  if_iter->attr.priority,
...
{
    uint64_t aclwmulated_flags = 0;
    ucp_rsc_index_t iface_id;

    for (iface_id = 0; iface_id < worker->num_ifaces; ++iface_id) {
        if (ucp_is_scalable_transport(worker->context,
                                      worker->ifaces[iface_id]->attr.max_num_eps)) {
            aclwmulated_flags |= worker->ifaces[iface_id]->attr.cap.flags;
        }
    }

    if (aclwmulated_flags & UCT_IFACE_FLAG_ATOMIC_DEVICE) {
                               ...
                               ucp_worker_h *worker_p)
{
     ...
     */
    ucs_config_parser_warn_unused_elw_vars_once(context->config.elw_prefix);
