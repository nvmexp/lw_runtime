 ...
 * Lwrrently only UCP_AM_FLAG_WHOLE_MSG is supported,
 ...
 * lwrrently the only flag tells UCP to pass in
...
typedef struct ucp_params {
     ...
     * delayed and only ilwoked at @ref ucp_worker_destroy.
     *
...
typedef struct ucp_listener_params {
     ...
     * In order for the callback inside this handler to be ilwoked, the
     ...
     * flow. In order for the callback inside this handler to be ilwoked, the
 ...
 *          ilwoke callback function at operation completion. If the
 ...
 *         .request                    = request,
 *         .cb.ucp_send_nbx_callback_t = lwstom_send_callback_f,
...
typedef struct {
    ...
    /**
     * Callback function that is ilwoked whenever the
 ...
 *
 * @param [in]  elw_prefix    If non-NULL, the routine searches for the
 *                            environment variables that start with
 *                            @e \<elw_prefix\>_UCX_ prefix.
 ...
 */
ucs_status_t ucp_config_read(const char *elw_prefix, const char *filename,
 ...
 * multiple @ref ucp_worker_h "workers" in order to enable conlwrrent access to
 ...
 * The file descriptor will get signaled when an event oclwrs, as part of the
 ...
 * calling @ref ucp_worker_progress (which is not ilwoked in that duration).
 *
 * @note UCP @ref ucp_feature "features" have to be triggered
 *   with @ref UCP_FEATURE_WAKEUP to select proper transport
 *
 ...
 * updated ("write") or an event oclwrs in the system.
 *
 * This function is guaranteed to return only if new communication events occur
 ...
 * @ref ucp_worker_progress repeatedly until it returns 0.
 *
 * @note This routine can be used by an application that exelwtes busy-waiting
 ...
 * exelwtion until the memory is updated. The goal of the routine is to provide
 ...
 * The user's call-back will be ilwoked once the endpoint is created.
 *
 * @param [in]  worker           Worker object that is associated with the
 *                               params object.
 ...
 * it will be ilwoked with status @ref UCS_ERR_REJECTED.
 *
 ...
 *                          remote endpoint. Lwrrently only
 *                          UCP_AM_FLAG_WHOLE_MSG is supported, which
 *                          indicates the callback will not be ilwoked
 ...
 * @param [in]  cb          Callback that is ilwoked upon completion of the 
 ...
 * call-back function @a cb is @b not ilwoked. If the operation is
 ...
 * @param [in]  count       Number of elements to send.
 * @param [in]  datatype    Datatype descriptor for the elements in the buffer.
 * @param [in]  cb          Callback function that is ilwoked whenever the
 *                          send operation is completed. It is important to note
 *                          that the call-back is only ilwoked in a case when
 ...
 * @a cb is @b not ilwoked. If the operation is @b not completed immediately
 * and no error reported then the UCP library will schedule to ilwoke the
 ...
 *
 * This routine provides a colwenient and efficient way to implement a
 ...
 * completed when it is safe to reuse the source @e buffer. If the send
 * operation is completed immediately the routine returns UCS_OK and the
 * call-back function is @b not ilwoked. If the operation is @b not completed
 * immediately and no error reported then the UCP library will schedule to
 * ilwoke the call-back whenever the send operation is completed. In other
 * words, the completion of a message can be signaled by the return code or
 * the call-back.
 ...
 * completion of a scheduled receive operation, the UCP library will ilwoke
 ...
 * operation the UCP library will ilwoke the call-back @a cb when the received
 ...
 * completion of the receive operation the UCP library will ilwoke the
 ...
 * ilwoked. If the operation is @b not completed immediately and no error is
 ...
 * @param [in]  rkey         Remote memory key associated with the
 *                           remote memory address.
 * @param [in]  cb           Call-back function that is ilwoked whenever the
 ...
 * call-back routine @a cb is @b not ilwoked. If the operation is @b not
 ...
 * signaled by the return code or exelwtion of the call-back.
 *
 * @note A user can use @ref ucp_worker_flush_nb "ucp_worker_flush_nb()"
 * in order to guarantee re-usability of the source address @e buffer.
 *
 ...
 * @a cb is @b not ilwoked. If the operation is @b not completed immediately and no
 ...
 * of an atomic operation can be signaled by the return code or exelwtion of the call-back.
 *
 ...
 * @param [in] cb          Call-back function that is ilwoked whenever the
 *                         send operation is completed. It is important to note
 *                         that the call-back function is only ilwoked in a case when
 *                         the operation cannot be completed in place.
 *
 * @return NULL                 - The operation was completed immediately.
 * @return UCS_PTR_IS_ERR(_ptr) - The operation failed.
...
/**
 * @ingroup UCP_COMM
 * @brief Check the status and lwrrently available state of non-blocking request
 ...
 * will be ilwoked for this request.
 */
