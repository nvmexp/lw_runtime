                               ...
                               ucp_wireup_select_context_t *select_ctx)
{
    ...
    /* Add to the list of lanes and remove all oclwrrences of the remote md
    ...
    for (;;) {
        ...
        /* Add lane description and remove all oclwrrences of the remote md. */
        ucp_wireup_add_lane(select_params, &select_info, usage, select_ctx);
        ucp_wireup_unset_tl_by_md(select_params, &select_info, &tl_bitmap,
                                  &remote_md_map);
    }
                       ...
                       ucp_wireup_select_context_t *select_ctx)
{
    ...
    select_info.priority   = 0;  /**< Lwrrently we have only 1 CM
                                      implementation */
                        ...
                        ucp_wireup_select_context_t *select_ctx)
{
    ...
    while ((num_lanes < bw_info->max_lanes) &&
        ...
        if (local_dev_count[dev_index] >= iface_attr->dev_num_paths) {
            /* exclude local device if reached max conlwrrency level */
        ...
        if (remote_dev_count[ae->dev_index] >= ae->dev_num_paths) {
            /* exclude remote device if reached max conlwrrency level */
