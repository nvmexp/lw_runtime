                           ...
                           const ucp_unpacked_address_t *remote_address)
{
    ...
    if (msg->dest_ep_ptr != 0) {
        ...
        if (!(ep->flags & UCP_EP_FLAG_LISTENER)) {
             ...
             * flush state) should be valid until user's callback ilwoking */
            ucp_ep_flush_state_reset(ep);
        }
    ...
    if (send_reply) {
        ...
        /* if in client-server flow, schedule ilwoking the user's callback
...
void ucp_wireup_process_ack(ucp_worker_h worker, const ucp_wireup_msg_t *msg)
{
     ...
     * a large worker address from the client, ilwoke the cached user callback
