...
ucs_status_t ucs_timerq_init(ucs_timer_queue_t *timerq)
{
    ...
    ucs_relwrsive_spinlock_init(&timerq->lock, 0);
    timerq->timers       = NULL;
    timerq->num_timers   = 0;
...
void ucs_timerq_cleanup(ucs_timer_queue_t *timerq)
{
    ...
    status = ucs_relwrsive_spinlock_destroy(&timerq->lock);
    if (status != UCS_OK) {
                            ...
                            ucs_time_t interval)
{
    ...
    ucs_relwrsive_spin_lock(&timerq->lock);
...
out_unlock:
    ucs_relwrsive_spin_unlock(&timerq->lock);
    return status;
}
