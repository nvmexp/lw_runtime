 ...
 * @param lwrrent_time  Current time to initialize the timer with.
 */
 ...
 * @param lwrrent_time  Current time to dispatch the timers for.
 *
 ...
 */
void __ucs_twheel_sweep(ucs_twheel_t *t, ucs_time_t lwrrent_time);
static inline void ucs_twheel_sweep(ucs_twheel_t *t, ucs_time_t lwrrent_time)
{
    if (ucs_unlikely(lwrrent_time - t->now >= t->res)) {
        __ucs_twheel_sweep(t, lwrrent_time);
    }
}
 ...
 * @param timer      Timer callback to ilwoke every time.
