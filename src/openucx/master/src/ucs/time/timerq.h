 ...
 * @param _lwrrent_time Current time to dispatch the timers for.
 *
 ...
 */
#define ucs_timerq_for_each_expired(_timer, _timerq, _lwrrent_time, _code) \
    { \
        ucs_time_t __lwrrent_time = _lwrrent_time; \
        ucs_relwrsive_spin_lock(&(_timerq)->lock); /* Grab lock */ \
        for (_timer = (_timerq)->timers; \
             _timer != (_timerq)->timers + (_timerq)->num_timers; \
             ++_timer) \
        { \
            if (__lwrrent_time >= (_timer)->expiration) { \
                /* Update expiration time */ \
                (_timer)->expiration = __lwrrent_time + (_timer)->interval; \
                _code; \
            } \
        } \
        ucs_relwrsive_spin_unlock(&(_timerq)->lock); /* Release lock  */ \
    }
