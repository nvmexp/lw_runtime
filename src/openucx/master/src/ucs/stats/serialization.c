...
static unsigned ucs_stats_get_all_classes_relwrs(ucs_stats_node_t *node,
                                                 ...
                                                 ucs_stats_clsid_t **cls_hash)
{
    ...
    ucs_list_for_each(child, &node->children[sel], list) {
        count += ucs_stats_get_all_classes_relwrs(child, sel, cls_hash);
    }
                                     ...
                                     FILE *stream)
{
     ...
     * Then, an array of all counters, each one oclwpying the size listed before.
     */
    for (i = 0; i < num_counters; ++i) {
        value = counters[i];
        if (value == 0) {
...
static void
ucs_stats_serialize_binary_relwrs(FILE *stream, ucs_stats_node_t *node,
                                  ucs_stats_children_sel_t sel,
                                  ucs_stats_clsid_t **cls_hash)
{
    ...
    /* Children */
    ucs_list_for_each(child, &node->children[sel], list) {
        ucs_stats_serialize_binary_relwrs(stream, child, sel, cls_hash);
    }
                           ...
                           ucs_stats_children_sel_t sel)
{
    ...
    hdr.reserved    = 0;
    hdr.num_classes = ucs_stats_get_all_classes_relwrs(root, sel, cls_hash);
    ...
    /* Write stats nodes */
    ucs_stats_serialize_binary_relwrs(stream, root, sel, cls_hash);
...
static ucs_status_t
ucs_stats_serialize_text_relwrs_filtered(FILE *stream,
                                         ...
                                         unsigned indent)
{
    ...
    ucs_list_for_each(filter_child, &filter_node->children, list) {
        ucs_stats_serialize_text_relwrs_filtered(stream, filter_child,
                                                 indent + 1);
    }
...
ucs_status_t ucs_stats_serialize(FILE *stream, ucs_stats_node_t *root, int options)
{
    ...
    if (options & UCS_STATS_SERIALIZE_BINARY) {
        ...
        return ucs_stats_serialize_text_relwrs_filtered(stream,
                                                        root->filter_node,
                                                        0);
    }
}
...
static ucs_status_t
ucs_stats_deserialize_relwrs(FILE *stream, ucs_stats_class_t **classes,
                             ...
                             ucs_stats_node_t **p_root)
{
    ...
    /* Read children */
    do {
        status = ucs_stats_deserialize_relwrs(stream, classes, num_classes, 0,
                                              &child);
        if (status == UCS_OK) {
            ...
            ucs_error("ucs_stats_deserialize_relwrs returned %s", ucs_status_string(status));
...
ucs_status_t ucs_stats_deserialize(FILE *stream, ucs_stats_node_t **p_root)
{
    ...
    /* Read nodes */
    status = ucs_stats_deserialize_relwrs(stream, classes, hdr.num_classes,
