...
typedef struct ucs_callbackq_priv {
    ucs_relwrsive_spinlock_t lock;           /**< Protects adding / removing */
...
{
    ucs_relwrsive_spin_lock(&ucs_callbackq_priv(cbq)->lock);
}

static void ucs_callbackq_leave(ucs_callbackq_t *cbq)
{
    ucs_relwrsive_spin_unlock(&ucs_callbackq_priv(cbq)->lock);
}
...
ucs_status_t ucs_callbackq_init(ucs_callbackq_t *cbq)
{
    ...
    ucs_relwrsive_spinlock_init(&priv->lock, 0);
