...
static void ucs_pgt_entry_dump_relwrs(const ucs_pgtable_t *pgtable, unsigned indent,
                                      ...
                                      unsigned shift, ucs_log_level_t log_level)
{
    ...
    if (ucs_pgt_entry_test(pte, UCS_PGT_ENTRY_FLAG_REGION)) {
        ...
        for (i = 0; i < UCS_PGT_ENTRIES_PER_DIR; ++i) {
            ucs_pgt_entry_dump_relwrs(pgtable, indent + 2, &pgd->entries[i], i,
...
void ucs_pgtable_dump(const ucs_pgtable_t *pgtable, ucs_log_level_t log_level)
{
    ...
    ucs_pgt_entry_dump_relwrs(pgtable, 0, &pgtable->root, 0, pgtable->base,
 ...
 */
static ucs_status_t
ucs_pgtable_remove_page_relwrs(ucs_pgtable_t *pgtable, ucs_pgt_addr_t address,
                               ...
                               ucs_pgt_region_t *region)
{
    ...
    if (ucs_pgt_entry_test(pte, UCS_PGT_ENTRY_FLAG_REGION)) {
        ...
        status = ucs_pgtable_remove_page_relwrs(pgtable, address, order, next_dir,
                        ...
                        unsigned order, ucs_pgt_region_t *region)
{
    ...
    status = ucs_pgtable_remove_page_relwrs(pgtable, address, order, &dummy_pgd,
...
static void ucs_pgtable_search_relwrs(const ucs_pgtable_t *pgtable,
    ...
    if (ucs_pgt_entry_test(pte, UCS_PGT_ENTRY_FLAG_REGION)) {
        ...
        *last_p = region;
        ...
        if (order < shift) {
            ...
            ucs_pgtable_search_relwrs(pgtable, address, order, next_pte,
                              ...
                              ucs_pgt_search_callback_t cb, void *arg)
{
    ...
    while (address <= to) {
        order = ucs_pgtable_get_next_page_order(address, end);
        if ((address & pgtable->mask) == pgtable->base) {
            ucs_pgtable_search_relwrs(pgtable, address, order, &pgtable->root,
