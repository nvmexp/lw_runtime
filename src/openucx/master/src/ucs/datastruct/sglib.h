...
#define SGLIB_ARRAY_QUICK_SORT(type, a, max, comparator, elem_exchanger) {\
  ...
  while (_stacki_ > 0) {\
    ...
    while (_end_ - _start_ > 2) {\
      ...
      /* handle relwrsive calls without relwrsion */\
      if (_i_-_start_ > 1 && _end_-_j_ > 1) {\
        /* two relwrsive calls, use array-stack */\
...
/* !!!!!!! This data structure is NOT dolwmented, do not use it !!!!!!!!!! */
...
/*
  ...
  table oclwrs on exactly one index.  Once an object is stored in the
...
#define SGLIB_LIST_SORT(type, list, comparator, next) {\
  /* a non-relwrsive merge sort on lists */\
  type *_r_;\
...
#define SGLIB___BIN_TREE_MAP_ON_ELEMENTS(type, tree, iteratedVariable, order, left, right, command) {\
  /* this is non-relwrsive implementation of tree traversal */\
  /* it maintains the path to the current node in the array '_path_' */\
  /* the _path_[0] contains the root of the tree; */\
  /* the _path_[_pathi_] contains the _lwrrent_element_ */\
  /* the macro does not use the _lwrrent_element_ after exelwtion of command */\
...
#define SGLIB_BIN_TREE_MAP_ON_ELEMENTS(type, tree, _lwrrent_element_, left, right, command) {\
  SGLIB___BIN_TREE_MAP_ON_ELEMENTS(type, tree, _lwrrent_element_, 1, left, right, command);\
}

#define SGLIB_BIN_TREE_MAP_ON_ELEMENTS_PREORDER(type, tree, _lwrrent_element_, left, right, command) {\
  SGLIB___BIN_TREE_MAP_ON_ELEMENTS(type, tree, _lwrrent_element_, 0, left, right, command);\
}

#define SGLIB_BIN_TREE_MAP_ON_ELEMENTS_POSTORDER(type, tree, _lwrrent_element_, left, right, command) {\
  SGLIB___BIN_TREE_MAP_ON_ELEMENTS(type, tree, _lwrrent_element_, 2, left, right, command);\
}
...
#define SGLIB_DEFINE_HASHED_TABLE_PROTOTYPES(type, dim, hash_function, comparator) \
  ...
  extern type *sglib_hashed_##type##_it_lwrrent(struct sglib_hashed_##type##_iterator *it); \
...
#define SGLIB_DEFINE_HASHED_TABLE_FUNCTIONS(type, dim, hash_function, comparator) \
  ...
  type *sglib_hashed_##type##_it_lwrrent(struct sglib_hashed_##type##_iterator *it) {\
    return(table[it->lwrrentIndex]);\
  }\
...
#define SGLIB_DEFINE_HASHED_CONTAINER_FUNCTIONS(type, dim, hash_function) \
  ...
  type *sglib_hashed_##type##_it_init_on_equal(struct sglib_hashed_##type##_iterator *it, type *table[dim], int (*subcomparator)(type *, type *), type *equalto) {\
    ...
    it->subcomparator = subcomparator;\
    it->equalto = equalto;\
    e = sglib_##type##_it_init_on_equal(&it->containerIt, table[it->lwrrentIndex], it->subcomparator, it->equalto);\
  ...
  type *sglib_hashed_##type##_it_lwrrent(struct sglib_hashed_##type##_iterator *it) {\
    return(sglib_##type##_it_lwrrent(&it->containerIt));\
  }\
  type *sglib_hashed_##type##_it_next(struct sglib_hashed_##type##_iterator *it) {\
    type *e;\
    e = sglib_##type##_it_next(&it->containerIt);\
    while (e==NULL && (++(it->lwrrentIndex))<(dim)) {\
      e = sglib_##type##_it_init_on_equal(&it->containerIt, it->table[it->lwrrentIndex], it->subcomparator, it->equalto);\
    }\
    return(e);\
  }
...
#define SGLIB_DEFINE_LIST_PROTOTYPES(type, comparator, next) \
 ...
 extern type *sglib_##type##_it_lwrrent(struct sglib_##type##_iterator *it); \
...
#define SGLIB_DEFINE_LIST_FUNCTIONS(type, comparator, next) \
 ...
 type *sglib_##type##_it_lwrrent(struct sglib_##type##_iterator *it) {\
   return(it->lwrrentelem);\
 }\
...
\
static void sglib___##type##_add_relwrsive(type **tree, type *elem) {\
  int cmp;\
  type *t;\
  t = *tree;\
  if (t == NULL) {\
...
\
int sglib___##type##_delete_relwrsive(type **tree, type *elem) {\
  type  *t, *theLeaf;\
  ...
  if (t==NULL) {\
    ...
    cmp = comparator(elem, t);\
    if (cmp < 0 || (cmp==0 && elem<t)) {\
      deepDecreased = sglib___##type##_delete_relwrsive(&t->left, elem);\
      if (deepDecreased) {\
        res = sglib___##type##_fix_left_deletion_discrepancy(tree);\
      }\
    } else if (cmp > 0  || (cmp==0 && elem>t)) {\
      deepDecreased = sglib___##type##_delete_relwrsive(&t->right, elem);\
      if (deepDecreased) {\
...
void sglib_##type##_add(type **tree, type *elem) {\
  ...
  sglib___##type##_add_relwrsive(tree, elem);\
  SGLIB___SET_VALUE((*tree)->bits,BLACK);\
}\
\
void sglib_##type##_delete(type **tree, type *elem) {\
  sglib___##type##_delete_relwrsive(tree, elem);\
...
\
void sglib__##type##_it_compute_lwrrent_elem(struct sglib_##type##_iterator *it) {\
    int   i,j,cmp;\
    type  *s, *eqt;\
    int   (*subcomparator)(type *, type *);\
    eqt = it->equalto;\
    ...
    it->lwrrentelem = NULL;\
    while(it->pathi > 0 && it->lwrrentelem==NULL) {\
        i = it->pathi-1;\
        if (i >= 0) {\
            if (it->pass[i] >= 2) {\
                /* goto up */\
        ...
        if (it->pathi>0 && it->order == it->pass[it->pathi-1]) {\
            it->lwrrentelem = it->path[it->pathi-1];\
        }\
    }\
}\
type *sglib__##type##_it_init(struct sglib_##type##_iterator *it, type *tree, int order, int (*subcomparator)(type *, type *), type *equalto) {\
    ...
    if (t == NULL) {\
        ...
        if (order == 0) {\
            ...
            sglib__##type##_it_compute_lwrrent_elem(it);\
        }\
    }\
    return(it->lwrrentelem);\
}\
...
\
static void sglib___##type##_consistency_check_relwrsive(type *t, int *pathdeep, int cdeep) {\
  if (t==NULL) {\
    ...
    if (SGLIB___GET_VALUE(t->bits) == RED) {\
      ...
      sglib___##type##_consistency_check_relwrsive(t->left, pathdeep, cdeep);\
      sglib___##type##_consistency_check_relwrsive(t->right, pathdeep, cdeep);\
    } else {\
      sglib___##type##_consistency_check_relwrsive(t->left, pathdeep, cdeep+1);\
      sglib___##type##_consistency_check_relwrsive(t->right, pathdeep, cdeep+1);\
    }\
  }\
}\
\
void sglib___##type##_consistency_check(type *t) {\
  ...
  pathDeep = -1;\
  sglib___##type##_consistency_check_relwrsive(t, &pathDeep, 0);\
}
