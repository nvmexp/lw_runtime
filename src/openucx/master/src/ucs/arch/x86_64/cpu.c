...
#define X86_CPUID_ILWARIANT_TSC   0x80000007u
...
static int ucs_x86_ilwariant_tsc()
{
    uint32_t _eax, _ebx, _ecx, _edx;

    ucs_x86_cpuid(X86_CPUID_GET_MAX_VALUE, &_eax, &_ebx, &_ecx, &_edx);
    if (_eax <= X86_CPUID_ILWARIANT_TSC) {
        goto warn;
    }

    ucs_x86_cpuid(X86_CPUID_ILWARIANT_TSC, &_eax, &_ebx, &_ecx, &_edx);
    if (!(_edx & UCS_BIT(8))) {
        goto warn;
    }
...
warn:
    ucs_debug("CPU does not support ilwariant TSC, using fallback timer");
