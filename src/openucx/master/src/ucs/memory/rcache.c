...
typedef struct ucs_rcache_ilw_entry {
    ucs_queue_elem_t         queue;
    ucs_pgt_addr_t           start;
    ucs_pgt_addr_t           end;
} ucs_rcache_ilw_entry_t;
...
static ucs_stats_class_t ucs_rcache_stats_class = {
    ...
    .counter_names = {
        ...
        [UCS_RCACHE_UNMAP_ILWALIDATES]  = "regions_ilw_unmap",
...
/* Lock must be held in write mode */
static void ucs_rcache_region_ilwalidate(ucs_rcache_t *rcache,
                                         ucs_rcache_region_t *region,
                                         int must_be_in_pgt,
                                         int must_be_destroyed)
{
    ...
    ucs_rcache_region_trace(rcache, region, "ilwalidate");
...
/* Lock must be held in write mode */
static void ucs_rcache_ilwalidate_range(ucs_rcache_t *rcache, ucs_pgt_addr_t start,
                                        ucs_pgt_addr_t end)
{
    ...
    ucs_list_for_each_safe(region, tmp, &region_list, list) {
        ...
        ucs_rcache_region_ilwalidate(rcache, region, 1, 0);
        UCS_STATS_UPDATE_COUNTER(rcache->stats, UCS_RCACHE_UNMAP_ILWALIDATES, 1);
    }
}

/* Lock must be held in write mode */
static void ucs_rcache_check_ilw_queue(ucs_rcache_t *rcache)
{
    ucs_rcache_ilw_entry_t *entry;

    ucs_trace_func("rcache=%s", rcache->name);

    ucs_relwrsive_spin_lock(&rcache->ilw_lock);
    while (!ucs_queue_is_empty(&rcache->ilw_q)) {
        entry = ucs_queue_pull_elem_non_empty(&rcache->ilw_q,
                                              ucs_rcache_ilw_entry_t, queue);
         ...
         * this lock.
         */
        ucs_relwrsive_spin_unlock(&rcache->ilw_lock);

        ucs_rcache_ilwalidate_range(rcache, entry->start, entry->end);
                                         ...
                                         ucm_event_t *event, void *arg)
{
    ...
    ucs_relwrsive_spin_lock(&rcache->ilw_lock);
    entry = ucs_mpool_get(&rcache->ilw_mp);
    if (entry != NULL) {
        /* Add region to ilwalidation list */
        entry->start = start;
        entry->end   = end;
        ucs_queue_push(&rcache->ilw_q, &entry->queue);
        UCS_STATS_UPDATE_COUNTER(rcache->stats, UCS_RCACHE_UNMAPS, 1);
    } else {
        ucs_error("Failed to allocate ilwalidation entry for 0x%lx..0x%lx, "
                         ...
                         ucs_rcache_region_t **region_p)
{
    ...
    ucs_rcache_check_ilw_queue(rcache);
    ...
    ucs_list_for_each_safe(region, tmp, &region_list, list) {
        ...
        if (!ucs_test_all_flags(*prot, region->prot)) {
             ...
             *
             * TODO: lwrrently rcache is optimized for the case where most of
            ...
            if (!ucs_test_all_flags(mem_prot, *prot)) {
                 ...
                 * Do the safest thing: ilwalidate cached region
                 */
                ucs_rcache_region_ilwalidate(rcache, region, 1, 0);
                continue;
...
retry:
    ...
    if (status != UCS_OK) {
        if (merged) {
             ...
             * all merged regions has been ilwalidated and registration will
             * succeed.
             */
            ucs_debug("failed to register merged region " UCS_PGT_REGION_FMT ": %s, retrying",
                      UCS_PGT_REGION_ARG(&region->super), ucs_status_string(status));
            ucs_rcache_region_ilwalidate(rcache, region, 1, 1);
            goto retry;
        } else {
                            ...
                            int prot, void *arg, ucs_rcache_region_t **region_p)
{
    ...
    if (ucs_queue_is_empty(&rcache->ilw_q)) {
                           ...
                           const char *name, ucs_stats_node_t *stats_parent)
{
    ...
    status = ucs_relwrsive_spinlock_init(&self->ilw_lock, 0);
    if (status != UCS_OK) {
    ...
    if (status != UCS_OK) {
        goto err_destroy_ilw_q_lock;
    }

    status = ucs_mpool_init(&self->ilw_mp, 0, sizeof(ucs_rcache_ilw_entry_t), 0,
                            1, 1024, UINT_MAX, &ucs_rcache_mp_ops, "rcache_ilw_mp");
    if (status != UCS_OK) {
        goto err_cleanup_pgtable;
    }

    ucs_queue_head_init(&self->ilw_q);
...
err_destroy_mp:
    ucs_mpool_cleanup(&self->ilw_mp, 1);
err_cleanup_pgtable:
    ucs_pgtable_cleanup(&self->pgtable);
err_destroy_ilw_q_lock:
    spinlock_status = ucs_relwrsive_spinlock_destroy(&self->ilw_lock);
    if (spinlock_status != UCS_OK) {
        ucs_warn("ucs_relwrsive_spinlock_destroy() failed (%d)", spinlock_status);
    }
err_destroy_rwlock:
...
static UCS_CLASS_CLEANUP_FUNC(ucs_rcache_t)
{
    ...
    ucm_unset_event_handler(self->params.ucm_events, ucs_rcache_unmapped_callback,
                            self);
    ucs_rcache_check_ilw_queue(self);
    ucs_rcache_purge(self);

    ucs_mpool_cleanup(&self->ilw_mp, 1);
    ucs_pgtable_cleanup(&self->pgtable);
    status = ucs_relwrsive_spinlock_destroy(&self->ilw_lock);
    if (status != UCS_OK) {
