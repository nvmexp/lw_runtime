 ...
 *
 * If an error oclwrs sysconf modified errno and ucs_sysconf aborts.
 *
                                         ...
                                         char *buf, size_t max)
{
    ...
    if (ret >= 0) {
        if ((sys_errno == EILWAL) && (alloc_size > ipc_info.shmmax)) {
            snprintf(p, endp - p,
                            ...
                            int flags, const char *alloc_name, int *shmid)
{
    ...
    * behavior */
    ...
    if (ptr == (void*)-1) {
        if (errno == ENOMEM) {
            return UCS_ERR_NO_MEMORY;
        } else if (RUNNING_ON_VALGRIND && (errno == EILWAL)) {
            return UCS_ERR_NO_MEMORY;
        } else {
...
void *ucs_sys_realloc(void *old_ptr, size_t old_length, size_t new_length)
{
    ...
    if (old_ptr == NULL) {
         ...
         * partially undefined when colwerted to syscall arguments */
