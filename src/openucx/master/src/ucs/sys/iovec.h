...
/* An iterator that should be used by IOV colwertor in order to save
 ...
 * @param [in]     iov            A pointer to an array of iovec elements.
 * @param [in]     iov_cnt        A number of elements in a iov array.
 * @param [in/out] lwr_iov_idx    A pointer to an index in a iov array from
...
void ucs_iov_advance(struct iovec *iov, size_t iov_cnt,
                     size_t *lwr_iov_idx, size_t consumed);
