...
typedef struct ucs_profile_thread_context {
    ...
    struct {
        ucs_profile_record_t          *start;        /**< Cirlwlar log buffer start */
        ucs_profile_record_t          *end;          /**< Cirlwlar log buffer end */
                              ...
                              ucs_time_t default_end_time)
{
    ...
    if (ucs_global_opts.profile_mode & UCS_BIT(UCS_PROFILE_MODE_ACLWM)) {
        num_locations = ctx->aclwm.num_locations;
    } else {
        num_locations = 0;
    }
    ...
    ucs_profile_file_write_data(fd, ctx->aclwm.locations,
                                num_locations * sizeof(*ctx->aclwm.locations));
...
ucs_profile_thread_context_t* ucs_profile_thread_init()
{
    ...
    if (ucs_global_opts.profile_mode & UCS_BIT(UCS_PROFILE_MODE_ACLWM)) {
        ctx->aclwm.num_locations = 0;
        ctx->aclwm.locations     = NULL;
        ctx->aclwm.stack_top     = -1;
    }
...
static void ucs_profile_thread_expand_locations(int loc_id)
{
    ...
    new_num_locations = ucs_max(loc_id, ctx->aclwm.num_locations);
    ctx->aclwm.locations = ucs_realloc(ctx->aclwm.locations,
                                       sizeof(*ctx->aclwm.locations) *
                                       new_num_locations,
                                       "profile_thread_locations");
    if (ctx->aclwm.locations == NULL) {
        ucs_fatal("failed to allocate profiling per-thread locations");
    }

    for (i = ctx->aclwm.num_locations; i < new_num_locations; ++i) {
        ctx->aclwm.locations[i].count      = 0;
        ctx->aclwm.locations[i].total_time = 0;
    }

    ctx->aclwm.num_locations = new_num_locations;
}
                        ...
                        int line, const char *function, volatile int *loc_id_p)
{
    ...
    if (ucs_global_opts.profile_mode & UCS_BIT(UCS_PROFILE_MODE_ACLWM)) {
        if (ucs_unlikely(loc_id > ctx->aclwm.num_locations)) {
            /* expand the locations array of the current thread */
            ucs_profile_thread_expand_locations(loc_id);
        }
        ucs_assert(loc_id - 1 < ctx->aclwm.num_locations);

        loc = &ctx->aclwm.locations[loc_id - 1];
        switch (type) {
        case UCS_PROFILE_TYPE_SCOPE_BEGIN:
            ctx->aclwm.stack[++ctx->aclwm.stack_top] = lwrrent_time;
            break;
        case UCS_PROFILE_TYPE_SCOPE_END:
            loc->total_time += lwrrent_time - ctx->aclwm.stack[ctx->aclwm.stack_top];
            --ctx->aclwm.stack_top;
            break;
        default:
            break;
