...
static ucs_relwrsive_spinlock_t ucs_kh_lock;
...
static const char *ucs_signal_cause_fpe(int si_code)
{
    ...
    case FPE_FLTILW   : return "floating-point invalid operation";
...
void ucs_debug_init()
{
    ucs_relwrsive_spinlock_init(&ucs_kh_lock, 0);
...
void ucs_debug_cleanup(int on_error)
{
    ...
    status = ucs_relwrsive_spinlock_destroy(&ucs_kh_lock);
    if (status != UCS_OK) {
