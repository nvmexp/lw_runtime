...
static void *ucs_async_thread_func(void *arg)
{
    ...
    ucs_time_t last_time, lwrr_time, timer_interval, time_spent;
    ...
    while (!thread->stop) {
        ...
        if (timer_interval == UCS_TIME_INFINITY) {
            ...
            time_spent = lwrr_time - last_time;
        ...
        /* Check timers */
        lwrr_time = ucs_get_time();
        if (lwrr_time - last_time > timer_interval) {
            status = ucs_async_dispatch_timerq(&thread->timerq, lwrr_time);
            if (status == UCS_ERR_NO_PROGRESS) {
                 is_missed = 1;
            }
...
{
    return ucs_relwrsive_spinlock_init(&async->thread.spinlock, 0);
}
...
static void ucs_async_thread_spinlock_cleanup(ucs_async_context_t *async)
{
    ...
    status = ucs_relwrsive_spinlock_destroy(&async->thread.spinlock);
    if (status != UCS_OK) {
...
{
    return ucs_relwrsive_spin_trylock(&async->thread.spinlock);
}

static void ucs_async_thread_spinlock_unblock(ucs_async_context_t *async)
{
    ucs_relwrsive_spin_unlock(&async->thread.spinlock);
}
