...
/**
 * Initialize an asynchronous exelwtion context. The context is not allocated.
...
/**
 * Block the async handler (if its lwrrently running, wait until it exits and
 * block it then). Used to serialize accesses with the async handler.
 *
 * @param event Event context to block events for.
 * @note This function might wait until a lwrrently running callback returns.
 */
#define UCS_ASYNC_BLOCK(_async) \
    do { \
        if ((_async)->mode == UCS_ASYNC_MODE_THREAD_SPINLOCK) { \
            ucs_relwrsive_spin_lock(&(_async)->thread.spinlock); \
...
/**
 * Unblock asynchronous event delivery, and ilwoke pending callbacks.
 *
...
#define UCS_ASYNC_UNBLOCK(_async) \
    do { \
        if ((_async)->mode == UCS_ASYNC_MODE_THREAD_SPINLOCK) { \
            ucs_relwrsive_spin_unlock(&(_async)->thread.spinlock); \
        } else if ((_async)->mode == UCS_ASYNC_MODE_THREAD_MUTEX) { \
