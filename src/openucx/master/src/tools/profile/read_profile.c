...
static int show_profile_data_aclwm(profile_data_t *data, options_t *opts)
{
    typedef struct {
...
static int redirect_output(const profile_data_t *data, options_t *opts)
{
    ...
    if (data->header->mode & UCS_BIT(UCS_PROFILE_MODE_ACLWM)) {
...
static int show_profile_data(profile_data_t *data, options_t *opts)
{
    ...
    if (data->header->mode & UCS_BIT(UCS_PROFILE_MODE_ACLWM)) {
        show_profile_data_aclwm(data, opts);
        printf("\n");
    }
