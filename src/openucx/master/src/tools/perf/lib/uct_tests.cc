...
public:
                                  ...
                                  unsigned flags)
    {
        ...
        ucs_assert(UCS_CIRLWLAR_COMPARE8(*(psn_t*)arg, <=, *(psn_t*)data));
        ...
        if (my_index == 1) {
            ...
            UCX_PERF_TEST_FOREACH(&m_perf) {
                if (flow_control) {
                    ...
                    sn = get_recv_sn(recv_sn, m_perf.uct.recv_mem.mem_type);
                    ucs_assertv(UCS_CIRLWLAR_COMPARE8(send_sn - 1, >=, sn),
                                "recv_sn=%d iters=%ld", sn, m_perf.current.iters);

                    while (UCS_CIRLWLAR_COMPARE8(send_sn, >, sn + fc_window)) {
                        progress_responder();
                        sn = get_recv_sn(recv_sn, m_perf.uct.recv_mem.mem_type);
                    }
                }
            ...
            if (!flow_control) {
                ...
                do {
                    progress_responder();
                    sn = get_recv_sn(recv_sn, m_perf.uct.recv_mem.mem_type);
                } while (UCS_CIRLWLAR_COMPARE8((psn_t)(send_sn - 1), >, sn));
            }
        } else if (my_index == 0) {
            if (flow_control) {
                ...
                UCX_PERF_TEST_FOREACH(&m_perf) {
                    ...
                    if (UCS_CIRLWLAR_COMPARE8(sn, >, (psn_t)(send_sn + (fc_window / 2)))) {
                ...
                sn = get_recv_sn(recv_sn, m_perf.uct.recv_mem.mem_type);
                if (UCS_CIRLWLAR_COMPARE8(sn, >, send_sn)) {
                    wait_for_window(send_window);
                    sn = get_recv_sn(recv_sn, m_perf.uct.recv_mem.mem_type);
                    send_b(ep, sn, send_sn, buffer, length, remote_addr,
                           rkey, &m_completion);
