...
struct ucx_perf_context {
    ...
    ucs_time_t                   end_time;        /* inaclwrate end time (upper bound) */
    ...
    struct {
        ...
        ucs_time_t               time;    /* inaclwrate time (for median and report interval) */
...
{
    perf->current.time_acc = ucs_get_aclwrate_time();
}
