...
static void usage(const struct perftest_context *ctx, const char *program)
{
    ...
    printf("     -i <offset>    distance between conselwtive scatter-gather entries (%zu)\n",
...
static ucs_status_t run_test_relwrs(struct perftest_context *ctx,
                                    ...
                                    unsigned depth)
{
    ...
    while ((status = read_batch_file(batch_file, ctx->batch_files[depth],
        ...
        run_test_relwrs(ctx, &params, depth + 1);
        free(params.msg_size_list);
...
static ucs_status_t run_test(struct perftest_context *ctx)
{
    ...
    status = run_test_relwrs(ctx, &ctx->params, 0);
    if (status != UCS_OK) {
