...
static ucs_pgt_dir_t *uct_lwda_ipc_cache_pgt_dir_alloc(const ucs_pgtable_t *pgtable)
{
    void *ptr;
    int ret;
    ...
    ret = ucs_posix_memalign(&ptr,
                             ...
                             sizeof(ucs_pgt_dir_t), "lwda_ipc_cache_pgdir");
    return (ret == 0) ? ptr : NULL;
}

static void uct_lwda_ipc_cache_pgt_dir_release(const ucs_pgtable_t *pgtable,
                                               ucs_pgt_dir_t *dir)
{
    ucs_free(dir);
}
...
static void
uct_lwda_ipc_cache_region_collect_callback(const ucs_pgtable_t *pgtable,
                                           ucs_pgt_region_t *pgt_region,
                                           void *arg)
{
    ucs_list_link_t *list = arg;
    uct_lwda_ipc_cache_region_t *region;

    region = ucs_derived_of(pgt_region, uct_lwda_ipc_cache_region_t);
    ucs_list_add_tail(list, &region->list);
}

static void uct_lwda_ipc_cache_purge(uct_lwda_ipc_cache_t *cache)
{
    uct_lwda_ipc_cache_region_t *region, *tmp;
    ucs_list_link_t region_list;

    ucs_list_head_init(&region_list);
    ucs_pgtable_purge(&cache->pgtable, uct_lwda_ipc_cache_region_collect_callback,
                      &region_list);
    ucs_list_for_each_safe(region, tmp, &region_list, list) {
        UCT_LWDADRV_FUNC(lwIpcCloseMemHandle((LWdeviceptr)region->mapped_addr));
        ucs_free(region);
    }
    ucs_trace("%s: lwca ipc cache purged", cache->name);
}

static ucs_status_t uct_lwda_ipc_open_memhandle(LWipcMemHandle memh,
                                                LWdeviceptr *mapped_addr)
{
    const char *lw_err_str;
    LWresult lwerr;

    lwerr = lwIpcOpenMemHandle(mapped_addr, memh,
                               LW_IPC_MEM_LAZY_ENABLE_PEER_ACCESS);
    if (lwerr != LWDA_SUCCESS) {
        if (lwerr == LWDA_ERROR_ALREADY_MAPPED) {
            return UCS_ERR_ALREADY_EXISTS;
        }

        lwGetErrorString(lwerr, &lw_err_str);
        ucs_error("lwIpcOpenMemHandle() failed: %s", lw_err_str);
...
static void uct_lwda_ipc_cache_ilwalidate_regions(uct_lwda_ipc_cache_t *cache,
                                                  void *from, void *to)
{
    ucs_list_link_t region_list;
    ucs_status_t status;
    ...
    ucs_pgtable_search_range(&cache->pgtable, (ucs_pgt_addr_t)from,
                             (ucs_pgt_addr_t)to,
                             uct_lwda_ipc_cache_region_collect_callback,
                             &region_list);
    ucs_list_for_each_safe(region, tmp, &region_list, list) {
...
{
    uct_lwda_ipc_cache_t *cache = (uct_lwda_ipc_cache_t *) rem_cache;
    ...
    if (!region->refcount && !cache_enabled) {
        ...
        status = UCT_LWDADRV_FUNC(lwIpcCloseMemHandle((LWdeviceptr)region->mapped_addr));
        ucs_free(region);
    }
...
UCS_PROFILE_FUNC(ucs_status_t, uct_lwda_ipc_map_memhandle,
                 (arg, key, mapped_addr),
                 void *arg, uct_lwda_ipc_key_t *key, void **mapped_addr)
{
    uct_lwda_ipc_cache_t *cache = (uct_lwda_ipc_cache_t *)arg;
    ucs_status_t status;
    ucs_pgt_region_t *pgt_region;
    uct_lwda_ipc_cache_region_t *region;
    int ret;
    ...
    if (ucs_likely(pgt_region != NULL)) {
        ...
        if (memcmp((const void *)&key->ph, (const void *)&region->key.ph,
                   sizeof(key->ph)) == 0) {
            /*cache hit */
            ucs_trace("%s: lwda_ipc cache hit addr:%p size:%lu region:"
            ...
            *mapped_addr = region->mapped_addr;
            ...
            ucs_trace("%s: lwda_ipc cache remove stale region:"
            ...
            /* close memhandle */
            UCT_LWDADRV_FUNC(lwIpcCloseMemHandle((LWdeviceptr)
                                                 region->mapped_addr));
            key->d_mapped = 0;
            ucs_free(region);
        }
    ...
    status = (key->d_mapped == 0) /* potentially already opened in rkey_unpack */
           ? uct_lwda_ipc_open_memhandle(key->ph, &key->d_mapped)
           : UCS_OK;
    ...
    *mapped_addr = (void *)key->d_mapped;
    ...
    if (ucs_unlikely(status != UCS_OK)) {
        if (ucs_likely(status == UCS_ERR_ALREADY_EXISTS)) {
            ...
            uct_lwda_ipc_cache_ilwalidate_regions(cache, (void *)key->d_bptr,
                                                  UCS_PTR_BYTE_OFFSET(key->d_bptr,
                                                                      key->b_len));
            status = uct_lwda_ipc_open_memhandle(key->ph, (LWdeviceptr *)mapped_addr);
            if (ucs_unlikely(status != UCS_OK)) {
                if (ucs_likely(status == UCS_ERR_ALREADY_EXISTS)) {
                    /* unmap all cache entries and retry */
                    uct_lwda_ipc_cache_purge(cache);
                    status = uct_lwda_ipc_open_memhandle(key->ph, (LWdeviceptr *)mapped_addr);
                    if (status != UCS_OK) {
    ...
    ret = ucs_posix_memalign((void **)&region,
                             ucs_max(sizeof(void *), UCS_PGT_ENTRY_MIN_ALIGN),
                             sizeof(uct_lwda_ipc_cache_region_t),
                             "uct_lwda_ipc_cache_region");
    if (ret != 0) {
        ucs_warn("failed to allocate uct_lwda_ipc_cache region");
    ...
    if (status == UCS_ERR_ALREADY_EXISTS) {
        ...
        uct_lwda_ipc_cache_ilwalidate_regions(cache,
    ...
    ucs_trace("%s: lwda_ipc cache new region:"UCS_PGT_REGION_FMT" size:%lu",
              cache->name, UCS_PGT_REGION_ARG(&region->super), key->b_len);
...
{
    ucs_status_t status;
    uct_lwda_ipc_cache_t *cache_desc;
    int ret;

    cache_desc = ucs_malloc(sizeof(uct_lwda_ipc_cache_t), "uct_lwda_ipc_cache_t");
    if (cache_desc == NULL) {
        ucs_error("failed to allocate memory for lwda_ipc cache");
    ...
    status = ucs_pgtable_init(&cache_desc->pgtable,
                              uct_lwda_ipc_cache_pgt_dir_alloc,
                              uct_lwda_ipc_cache_pgt_dir_release);
    if (status != UCS_OK) {
