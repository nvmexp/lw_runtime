...
uct_sockcm_ep_conn_state_str(uct_sockcm_ep_conn_state_t state)
{
    ...
    default:
        ucs_fatal("ilwaild sockcm endpoint state %d", state);
    }
}
                                    ...
                                    ucs_status_t status)
{
    ...
    uct_sockcm_ep_ilwoke_completions(ep, status);
    pthread_mutex_unlock(&ep->ops_mutex);
}
...
void uct_sockcm_ep_ilwoke_completions(uct_sockcm_ep_t *ep, ucs_status_t status)
{
