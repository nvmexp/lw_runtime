                                                ...
                                                ucs_status_t status)
{
    ...
    /* TODO handle disconnect if the ep already ilwoked the connect_cb */
}
...
static ucs_status_t uct_tcp_sockcm_ep_server_ilwoke_conn_req_cb(uct_tcp_sockcm_ep_t *cep)
{
     ...
     * to uct_ep_create() which will be ilwoked by the user and therefore moving
...
ucs_status_t uct_tcp_sockcm_ep_handle_data_received(uct_tcp_sockcm_ep_t *cep)
{
    ...
    status = uct_tcp_sockcm_ep_server_ilwoke_conn_req_cb(cep);
    if (status != UCS_OK) {
        goto out;
    }
