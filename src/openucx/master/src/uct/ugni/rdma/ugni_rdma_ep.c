...
/* Endpoint operations */
static inline void uct_ugni_ilwoke_orig_comp(uct_ugni_rdma_fetch_desc_t *fma_desc, ucs_status_t status)
{
    if (ucs_likely(NULL != fma_desc->orig_comp_cb)) {
        uct_ilwoke_completion(fma_desc->orig_comp_cb, status);
    }
}
...
static void uct_ugni_amo_unpack64(uct_completion_t *self, ucs_status_t status)
{
    ...
    *(uint64_t *)fma_desc->user_buffer = *(uint64_t *)(fma_desc + 1);
    uct_ugni_ilwoke_orig_comp(fma_desc, status);
}
...
static void assemble_composed_unaligned(uct_completion_t *self, ucs_status_t status)
{
    ...
    uct_ugni_ilwoke_orig_comp(fma_head, status);
}
