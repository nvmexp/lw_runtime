...
static ucs_status_t get_cookie(uint32_t *cookie)
{
    ...
    cookie_str = getelw("PMI_GNI_COOKIE");
    if (NULL == cookie_str) {
        ucs_error("getelw PMI_GNI_COOKIE failed");
...
static ucs_status_t get_ptag(uint8_t *ptag)
{
    ...
    ptag_str = getelw("PMI_GNI_PTAG");
    if (NULL == ptag_str) {
        ucs_error("getelw PMI_GNI_PTAG failed");
        return UCS_ERR_IO_ERROR;
    }
...
static ucs_status_t get_nic_address(uct_ugni_device_t *dev_p)
{
    ...
    if (NULL == pmi_elw) {
        ...
        while ((token = strtok(pmi_elw, ":")) != NULL) {
        ...
        pmi_elw = getelw("PMI_GNI_LOC_ADDR");
        ucs_assert(NULL != pmi_elw);
        i = 0;
        while ((token = strtok(pmi_elw, ":")) != NULL) {
            if (i == alps_dev_id) {
