...
#define uct_ugni_cdm_init_lock(_cdm) ucs_relwrsive_spinlock_init(&(_cdm)->lock, 0)
#define uct_ugni_cdm_destroy_lock(_cdm) \
    do { \
        ucs_status_t status; \
        \
        status = ucs_relwrsive_spinlock_destroy(&(_cdm)->lock); \
        if (status != UCS_OK) {\
...
if (uct_ugni_check_lock_needed(_cdm)) {  \
    ...
    ucs_relwrsive_spin_lock(&(_cdm)->lock);   \
}
#define uct_ugni_cdm_unlock(_cdm) \
if (uct_ugni_check_lock_needed(_cdm)) {    \
    ucs_trace_async("Releasing lock");        \
    ucs_relwrsive_spin_unlock(&(_cdm)->lock);   \
}
#else
