...
static void process_mbox(uct_ugni_smsg_iface_t *iface, uct_ugni_smsg_ep_t *ep){
    ...
    if (!ucs_relwrsive_spin_trylock(&iface->mbox_lock)) {
        return;
    }
    while(1){
        ...
        uct_iface_ilwoke_am(&iface->super.super, tag, user_data,
                            header->length, 0);
        uct_ugni_cdm_lock(&iface->super.cdm);
    ...
    ucs_relwrsive_spin_unlock(&iface->mbox_lock);
}
...
static void uct_ugni_smsg_handle_remote_overflow(uct_ugni_smsg_iface_t *iface){
    ...
    uct_ugni_cdm_unlock(&iface->super.cdm);
    lwrrent_ep = sglib_hashed_uct_ugni_ep_t_it_init(&ep_iterator, iface->super.eps);

    while(NULL != lwrrent_ep){
        ep = ucs_derived_of(lwrrent_ep, uct_ugni_smsg_ep_t);
        process_mbox(iface, ep);
        lwrrent_ep = sglib_hashed_uct_ugni_ep_t_it_next(&ep_iterator);
    }
}
...
static UCS_CLASS_CLEANUP_FUNC(uct_ugni_smsg_iface_t)
{
    ...
    status = ucs_relwrsive_spinlock_destroy(&self->mbox_lock);
    if (status != UCS_OK) {
                           ...
                           const uct_iface_config_t *tl_config)
{
    ...
    status = ucs_relwrsive_spinlock_init(&self->mbox_lock, 0);
    if (UCS_OK != status) {
            goto exit;
    }
    ...
    if (ugni_rc != GNI_RC_SUCCESS) {
        ucs_error("Smsg buffer size callwlation failed");
 ...
 clean_cq:
    uct_ugni_destroy_cq(self->remote_cq, &self->super.cdm);
 clean_lock:
    ucs_relwrsive_spinlock_destroy(&self->mbox_lock);
 exit:
