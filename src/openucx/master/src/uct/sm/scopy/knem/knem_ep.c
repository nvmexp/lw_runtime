                            ...
                            uct_scopy_tx_op_t tx_op)
{
    ...
    total_iov_length = ucs_iov_colwerter(local_iov, &local_iov_cnt,
    ...
    if (ucs_unlikely((ret < 0) ||
                     (icopy.lwrrent_status != KNEM_STATUS_SUCCESS))) {
        ucs_error("KNEM inline copy \"%s\" failed, ioctl() return value - %d, "
                  "copy status - %d: %m",
                  uct_knem_ep_tx_op_str[tx_op], ret, icopy.lwrrent_status);
