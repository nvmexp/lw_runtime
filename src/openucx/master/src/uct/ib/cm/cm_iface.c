                                         ...
                                         struct ib_cm_event *event)
{
    ...
    uct_iface_ilwoke_am(&iface->super.super, hdr->am_id, hdr + 1, hdr->length, 0);
}
...
static void uct_cm_iface_outstanding_purge(uct_cm_iface_t *iface)
{
    ...
    ucs_queue_for_each_extract(op, &iface->outstanding_q, queue, 1) {
        if (op->is_id) {
            ...
            uct_ilwoke_completion(op->comp, UCS_ERR_CANCELED);
        }
        ucs_free(op);
    }
