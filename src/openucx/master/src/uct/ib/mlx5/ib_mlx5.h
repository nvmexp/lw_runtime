...
typedef struct uct_ib_mlx5_md {
    ...
    ucs_relwrsive_spinlock_t dbrec_lock;
...
/**
 * Callwlate unique id for atomic
 */
static inline uint8_t uct_ib_mlx5_md_get_atomic_mr_id(uct_ib_mlx5_md_t *md)
{
     ...
     * job will have conselwtive PIDs. For example MPI ranks, slurm spawned tasks...
     */
    return getpid() % 256;
#else
    return 0;
...
static inline uct_ib_mlx5_dbrec_t *uct_ib_mlx5_get_dbrec(uct_ib_mlx5_md_t *md)
{
    ...
    ucs_relwrsive_spin_lock(&md->dbrec_lock);
    dbrec = (uct_ib_mlx5_dbrec_t *)ucs_mpool_get_inline(&md->dbrec_pool);
    ucs_relwrsive_spin_unlock(&md->dbrec_lock);
    if (dbrec != NULL) {
        dbrec->md = md;
    }
