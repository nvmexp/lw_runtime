...
uct_ib_mlx5_poll_cq(uct_ib_iface_t *iface, uct_ib_mlx5_cq_t *cq)
{
    ...
    if (ucs_unlikely(uct_ib_mlx5_cqe_is_hw_owned(op_own, cqe_index, cq->cq_length))) {
        ...
        UCS_STATIC_ASSERT(MLX5_CQE_ILWALID & (UCT_IB_MLX5_CQE_OP_OWN_ERR_MASK >> 4));
        ucs_assert((op_own >> 4) != MLX5_CQE_ILWALID);
...
uct_ib_mlx5_txwq_validate(uct_ib_mlx5_txwq_t *wq, uint16_t num_bb)
{
    ...
    wqe_s = UCS_PTR_BYTE_DIFF(wq->qstart, wq->lwrr) / MLX5_SEND_WQE_BB;
    ...
    if (!is_ok) {
        ucs_fatal("tx wq overrun: hw_ci: %u sw_pi: %u lwr: %u-%u num_bb: %u wqe_cnt: %u",
