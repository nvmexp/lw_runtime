...
enum {
    ...
    UCT_IB_MLX5_HCA_CAP_OPMOD_GET_LWR	= 1
};
...
struct uct_ib_mlx5_cmd_hca_cap_bits {
    ...
    uint8_t    vnic_elw_queue_counters[0x1];
    uint8_t    ets[0x1];
    ...
    uint8_t    log_max_lwrrent_mc_list[0x5];
    uint8_t    reserved_at_3f8[0x3];
    uint8_t    log_max_lwrrent_uc_list[0x5];
...
struct uct_ib_mlx5_qpc_bits {
    ...
    uint8_t         fre[0x1];
    uint8_t         lwr_rnr_retry[0x3];
    uint8_t         lwr_retry_count[0x3];
