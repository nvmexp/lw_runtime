                                             ...
                                             ucs_status_t status)
{
    ...
    if (ep != NULL) {
        pthread_mutex_lock(&ep->ops_mutex);
        uct_rdmacm_ep_set_failed(&iface->super.super, &ep->super.super, status);
        uct_rdmacm_ep_ilwoke_completions(ep, status);
        pthread_mutex_unlock(&ep->ops_mutex);
    }
}
                                              ...
                                              struct sockaddr *remote_addr)
{
    ...
    /* TODO check the iface's cb_flags to determine when to ilwoke this callback.
     * lwrrently only UCT_CB_FLAG_ASYNC is supported so the cb is ilwoked from here */
                               ...
                               struct rdma_cm_event *event)
{
    ...
    case RDMA_CM_EVENT_ADDR_RESOLVED:
        ...
        if (ep == NULL) {
            ...
            ret_flags |= UCT_RDMACM_PROCESS_EVENT_DESTROY_CM_ID_FLAG;
            uct_rdmacm_client_handle_failure(iface, ep, UCS_ERR_ILWALID_ADDR);
        }
        break;
    ...
    case RDMA_CM_EVENT_ROUTE_RESOLVED:
        ...
        if (ep == NULL) {
            ...
            /* TODO check the ep's cb_flags to determine when to ilwoke this callback.
             * lwrrently only UCT_CB_FLAG_ASYNC is supported so the cb is ilwoked from here */
    ...
    case RDMA_CM_EVENT_ESTABLISHED:
        ...
        if (ep != NULL) {
            pthread_mutex_lock(&ep->ops_mutex);
            ep->status = UCS_OK;
            uct_rdmacm_ep_ilwoke_completions(ep, UCS_OK);
            pthread_mutex_unlock(&ep->ops_mutex);
        }
        break;
