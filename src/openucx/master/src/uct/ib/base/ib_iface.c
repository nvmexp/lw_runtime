...
ucs_config_field_t uct_ib_iface_config_table[] = {
  ...
  {"NUM_PATHS", "auto",
   ...
   "              by "UCS_DEFAULT_ELW_PREFIX UCT_IB_CONFIG_PREFIX"LID_PATH_BITS configuration.",
   ucs_offsetof(uct_ib_iface_config_t, num_paths), UCS_CONFIG_TYPE_ULUNITS},

  {"ROCE_PATH_FACTOR", "1",
   "Multiplier for RoCE LAG UDP source port callwlation. The UDP source port\n"
                                           ...
                                           int flags, struct ibv_cq **cq_p)
{
    uct_ib_device_t *dev = uct_ib_iface_device(iface);
    ucs_status_t status;
#if HAVE_DECL_IBV_EXP_SETELW && !HAVE_DECL_MLX5DV_CQ_INIT_ATTR_MASK_CQE_SIZE
    static const char *cqe_size_elw_var = "MLX5_CQE_SIZE";
    const char *cqe_size_elw_value;
    size_t cqe_size = 64;
    size_t cqe_size_min;
    ...
    cqe_size_elw_value = getelw(cqe_size_elw_var);

    if (cqe_size_elw_value != NULL) {
        cqe_size = atol(cqe_size_elw_value);
        if (cqe_size < cqe_size_min) {
            ucs_error("%s is set to %zu, but at least %zu is required (inl: %zu)",
                      cqe_size_elw_var, cqe_size, cqe_size_min, *inl);
            return UCS_ERR_ILWALID_PARAM;
        }
    } else {
        cqe_size = uct_ib_get_cqe_size(cqe_size_min);
        snprintf(cqe_size_buf, sizeof(cqe_size_buf),"%zu", cqe_size);
        ucs_debug("%s: setting %s=%s", uct_ib_device_name(dev), cqe_size_elw_var,
                  cqe_size_buf);
        ret = ibv_exp_setelw(dev->ibv_context, cqe_size_elw_var, cqe_size_buf, 1);
        if (ret) {
            ucs_error("ibv_exp_setelw(%s=%s) failed: %m", cqe_size_elw_var,
                      cqe_size_buf);
            return UCS_ERR_ILWALID_PARAM;
        }
...
out_unsetelw:
    ...
    *inl = cqe_size / 2;
    if (elw_var_added) {
        ...
        ret = ibv_exp_unsetelw(dev->ibv_context, cqe_size_elw_var);
        if (ret) {
            ucs_warn("unsetelw(%s) failed: %m", cqe_size_elw_var);
        }
    }
#endif
    return status;
                                ...
                                uct_iface_attr_t *iface_attr)
{
    ...
    /* Wire speed callwlation: Width * SignalRate * Encoding */
    width                 = ib_port_widths[width_idx];
    wire_speed            = (width * signal_rate * encoding) / 8.0;

    /* Callwlate packet overhead  */
