                                ...
                                UCS_STATS_ARG(ucs_stats_node_t *stats_parent))
{
    ...
    ucs_relwrsive_spinlock_init(&dev->ah_lock, 0);
...
void uct_ib_device_cleanup(uct_ib_device_t *dev)
{
    ...
    status = ucs_relwrsive_spinlock_destroy(&dev->ah_lock);
    if (status != UCS_OK) {
...
const uct_ib_device_spec_t* uct_ib_device_spec(uct_ib_device_t *dev)
{
    ...
    for (spec = md->lwstom_devices.specs;
         spec < md->lwstom_devices.specs + md->lwstom_devices.count; ++spec) {
                                            ...
                                            struct ibv_ah **ah_p)
{
    ...
    ucs_relwrsive_spin_lock(&dev->ah_lock);
...
unlock:
    ucs_relwrsive_spin_unlock(&dev->ah_lock);
    return status;
}
