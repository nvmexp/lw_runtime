...
struct uct_ib_iface_config {
    ...
    /* Multiplier for RoCE LAG UDP source port callwlation */
 ...
 * and transport-specific data/header. When the active message callback is ilwoked,
...
uct_ib_iface_ilwoke_am_desc(uct_ib_iface_t *iface, uint8_t am_id, void *data,
                            unsigned length, uct_ib_iface_recv_desc_t *ib_desc)
{
    void *desc = (char*)ib_desc + iface->config.rx_headroom_offset;
    ucs_status_t status;

    status = uct_iface_ilwoke_am(&iface->super, am_id, data, length,
...
/**
 * Colwert IB address to a human-readable string.
 */
