...
static ucs_config_field_t uct_ib_md_config_table[] = {
    ...
    {"DEVICE_SPECS", "",
     ...
     ucs_offsetof(uct_ib_md_config_t, lwstom_devices), UCS_CONFIG_TYPE_STRING_ARRAY},
...
static ucs_status_t uct_ib_md_query(uct_md_h uct_md, uct_md_attr_t *md_attr)
{
    ...
    if (md->config.enable_gpudirect_rdma != UCS_NO) {
        ...
        uct_ib_check_gpudirect_driver(md, md_attr,
                                      "/sys/kernel/mm/memory_peers/lw_mem/version",
        ...
        if (!(md_attr->cap.reg_mem_types & ~UCS_BIT(UCS_MEMORY_TYPE_HOST)) &&
            (md->config.enable_gpudirect_rdma == UCS_YES)) {
                ucs_error("%s: Couldn't enable GPUDirect RDMA. Please make sure"
                          " lw_peer_mem or amdgpu plugin installed correctly.",
                                       ...
                                       size_t chunk, struct ibv_mr **mrs)
{
    ...
    uct_ib_md_mem_reg_thread_t *ctxs, *lwr_ctx;
    pthread_attr_t attr;
    ...
    for (thread_idx = 0; thread_idx < thread_num; thread_idx++) {
        /* callwlate number of mrs for each thread so each one will
         * get proportional amount */
        thread_num_mrs  = ucs_div_round_up(mr_num - mr_idx, thread_num - thread_idx);

        lwr_ctx         = &ctxs[thread_idx];
        lwr_ctx->pd     = md->pd;
        lwr_ctx->addr   = UCS_PTR_BYTE_OFFSET(address, mr_idx * chunk);
        lwr_ctx->len    = ucs_min(thread_num_mrs * chunk, length - (mr_idx * chunk));
        lwr_ctx->access = access_flags;
        lwr_ctx->mr     = &mrs[mr_idx];
        lwr_ctx->chunk  = chunk;
        ...
        ret = pthread_create(&lwr_ctx->thread, &attr,
                             uct_ib_md_mem_handle_thread_func, lwr_ctx);
        if (ret) {
    ...
    for (thread_idx = 0; thread_idx < thread_num; thread_idx++) {
        lwr_ctx = &ctxs[thread_idx];
        pthread_join(lwr_ctx->thread, &thread_status);
                                     ...
                                     void *rkey_buffer)
{
    ...
    if (memh->flags & UCT_IB_MEM_FLAG_ATOMIC_MR) {
        ...
        atomic_rkey = UCT_IB_ILWALID_RKEY;
    }
...
static void uct_ib_md_release_device_config(uct_ib_md_t *md)
{
    ...
    for (i = 0; i < md->lwstom_devices.count; ++i) {
        free((char*)md->lwstom_devices.specs[i].name);
    }
    ucs_free(md->lwstom_devices.specs);
}
...
uct_ib_md_parse_device_config(uct_ib_md_t *md, const uct_ib_md_config_t *md_config)
{
    ...
    count = md->lwstom_devices.count = md_config->lwstom_devices.count;
    if (count == 0) {
        md->lwstom_devices.specs = NULL;
        md->lwstom_devices.count = 0;
        return UCS_OK;
    }

    md->lwstom_devices.specs = ucs_calloc(count, sizeof(*md->lwstom_devices.specs),
                                          "ib_lwstom_devices");
    if (md->lwstom_devices.specs == NULL) {
        status = UCS_ERR_NO_MEMORY;
        goto err;
    }

    for (i = 0; i < count; ++i) {
        spec = &md->lwstom_devices.specs[i];
        nfields = sscanf(md_config->lwstom_devices.spec[i],
                         "%hi:%hi:%m[^:]:%m[^:]:%hhu",
        ...
        if (nfields < 2) {
            ucs_error("failed to parse device config '%s' (parsed: %d/%d)",
                      md_config->lwstom_devices.spec[i], nfields, 5);
            status = UCS_ERR_ILWALID_PARAM;
            goto err_free;
        }
...
{
    const char *pci_width_file_name = "lwrrent_link_width";
    const char *pci_speed_file_name = "lwrrent_link_speed";
...
#if HAVE_DECL_IBV_EXP_SETELW
    ibv_exp_setelw(md->dev.ibv_context, "MLX_QP_ALLOC_TYPE", "ANON", 0);
    ibv_exp_setelw(md->dev.ibv_context, "MLX_CQ_ALLOC_TYPE", "ANON", 0);
#endif
