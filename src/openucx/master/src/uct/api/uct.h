 ...
 *      This callback is ilwoked by the UCT transport to handle an incoming connection
 ...
 *      This callback is ilwoked by the UCT transport to fill auxiliary data in
 ...
 *      This callback is ilwoked by the UCT transport to handle the connection
 ...
 *      This callback is ilwoked by the UCT transport when the client side calls
 ...
 *      @ref uct_ep_disconnect_cb_t will not be ilwoked.
 *
 ...
 * @ref uct_cm_ep_priv_data_pack_callback_t
 *      This callback is ilwoked by the UCT transport to fill the user's private data
 ...
 *      This callback is ilwoked by the UCT transport to handle a connection response
 *      from the server.
 *      After ilwoking this callback, the UCT transport will finalize the client's
 ...
 *      notification from the remote side.
 * @ref uct_ep_disconnect_cb_t
 *      This callback is ilwoked by the UCT transport when the server side calls
 *      uct_ep_disconnect as well.
 * @ref uct_ep_destroy
 *      Destroy the endpoint connected to the remote peer.
 *
...
#define UCT_IFACE_FLAG_CB_SYNC        UCS_BIT(44) /**< Interface supports setting a callback
                                                       which is ilwoked only from the calling context of
                                                       uct_worker_progress() */
#define UCT_IFACE_FLAG_CB_ASYNC       UCS_BIT(45) /**< Interface supports setting a callback
                                                       which will be ilwoked within a reasonable amount of
                                                       time if uct_worker_progress() is not being called.
                                                       The callback can be ilwoked from any progress context
                                                       and it may also be ilwoked when uct_worker_progress()
                                                       is called. */
...
enum uct_cb_flags {
    ...
    UCT_CB_FLAG_ASYNC    = UCS_BIT(2)  /**< Callback is allowed to be called
                                            ...
                                            the callback will be ilwoked only
...
enum uct_md_mem_flags {
    ...
    UCT_MD_MEM_FLAG_LOCK        = UCS_BIT(2), /**< Registered memory should be
                                                   locked. May inlwr extra cost for
...
struct uct_iface_params {
    ...
    union {
        ...
        struct {
            ...
            /** Callback flags to indicate where the callback can be ilwoked from.
             * @ref uct_cb_flags */
            uint32_t                             cb_flags;
        } sockaddr;
    } mode;
...
struct uct_ep_params {
     ...
     * request, the callback will not be ilwoked.
     */
    ...
    union {
        /**
         * Callback that will be ilwoked when the endpoint on the client side
         * is being connected to the server by a connection manager @ref uct_cm_h .
         */
        uct_cm_ep_client_connect_callback_t      client;

        /**
         * Callback that will be ilwoked when the endpoint on the server side
    ...
    /**
     * Callback that will be ilwoked when the endpoint is disconnected.
     */
 ...
 * which will be ilwoked every time progress is made on the worker. *id_p will
 ...
 *
 * @param [in]    worker        Handle to the worker whose progress should ilwoke
 *                              the callback.
 ...
 * @param [in]  elw_prefix    If non-NULL, search for environment variables
...
ucs_status_t uct_md_iface_config_read(uct_md_h md, const char *tl_name,
                                      const char *elw_prefix, const char *filename,
 ...
 *
 * @return UCS_OK if found, otherwise UCS_ERR_ILWALID_PARAM or UCS_ERR_NO_ELEM
 *         if error.
 */
 ...
 *                      (lwrrently unsupported - set to 0).
...
/**
 * @ingroup UCT_CLIENT_SERVER
 * @brief Reject connection request. Will ilwoke an error handler @ref
 ...
 * @ref uct_ep_destroy should be called on this endpoint after ilwoking this
 ...
 *                               been ilwoked to handle the request.
 ...
 *         UCS_ERR_ILWALID_ADDR If failed to detect memory type
 */
 ...
 *       UCS_ERR_ILWALID_PARAM will be returned.
 *
 * @return Error code.
 */
 ...
 *
 * @note Lwrrently only one completion type is supported. It guarantees that
 ...
 * @param [in]    flags  Flags that control completion semantic (lwrrently only
 ...
 * @param [in]    flags  Flags that control ordering semantic (lwrrently
 ...
 *                            buffer can be reused or ilwalidated.
 *
 ...
 * ilwoked. I.e. it should be a handle of the real operation which is not
 * completed yet.
 *
 * @param [in] ep Destination endpoint handle.
 ...
 *
 * This routine can be ilwoked on the server side. It rejects a connection request
 * from the client.
 *
