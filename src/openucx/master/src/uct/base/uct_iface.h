...
typedef struct uct_base_iface {
    ...
    unsigned                progress_flags;   /* Which progress is lwrrently enabled */
...
/*
 * Ilwoke active message handler.
 *
 * @param iface    Interface to ilwoke the handler for.
...
uct_iface_ilwoke_am(uct_base_iface_t *iface, uint8_t id, void *data,
...
/**
 * Ilwoke send completion.
 *
 * @param comp   Completion to ilwoke.
 * @param data   Optional completion data (operation reply).
 */
static UCS_F_ALWAYS_INLINE
void uct_ilwoke_completion(uct_completion_t *comp, ucs_status_t status)
{
