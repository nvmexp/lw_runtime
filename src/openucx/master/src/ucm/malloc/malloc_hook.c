...
typedef struct ucm_malloc_hook_state {
     ...
     */
    ucs_relwrsive_spinlock_t lock; /* Protect heap counters.
     ...
     */
    pthread_mutex_t         elw_lock;
    char                    **elw_strs;
    unsigned                num_elw_strs;
...
static ucm_malloc_hook_state_t ucm_malloc_hook_state = {
    ...
    .heap_end         = (void*)-1,
    .ptrs             = {0},
    .elw_lock         = PTHREAD_MUTEX_INITIALIZER,
    .elw_strs         = NULL,
    .num_elw_strs     = 0
};
...
static void ucm_malloc_mmaped_ptr_add(void *ptr)
{
    ...
    ucs_relwrsive_spin_lock(&ucm_malloc_hook_state.lock);
    ...
    ucs_relwrsive_spin_unlock(&ucm_malloc_hook_state.lock);
}
...
/*
 * We remember the string we pass to putelw() so we would be able to release
 * them if they are overwritten (and thus avoid leaks).
 */
static int ucm_add_to_elwiron(char *elw_str)
{
    char *saved_elw_str;
    unsigned idx;
    size_t len;
    ...
    p = strchr(elw_str, '=');
    if (p == NULL) {
        len = strlen(elw_str); /* Compare whole string */
    } else {
        len = p + 1 - elw_str; /* Compare up to and including the '=' character */
    }

    /* Check if we already have variable with same name */
    idx = 0;
    while (idx < ucm_malloc_hook_state.num_elw_strs) {
        saved_elw_str = ucm_malloc_hook_state.elw_strs[idx];
        if ((strlen(saved_elw_str) >= len) && !strncmp(elw_str, saved_elw_str, len)) {
            ucm_trace("replace `%s' with `%s'", saved_elw_str, elw_str);
            ucm_free(saved_elw_str, NULL);
            goto out_insert;
        }
        ++idx;
    }
    ...
    idx = ucm_malloc_hook_state.num_elw_strs;
    ++ucm_malloc_hook_state.num_elw_strs;
    ucm_malloc_hook_state.elw_strs =
                    ucm_realloc(ucm_malloc_hook_state.elw_strs,
                                sizeof(char*) * ucm_malloc_hook_state.num_elw_strs,
                                NULL);

out_insert:
    ucm_malloc_hook_state.elw_strs[idx] = elw_str;
    return 0;
}

/*
 * We need to replace setelw() because glibc keeps a search tree of environment
 * strings and releases it with *original* free() (in __tdestroy).
 * If we always use putelw() instead of setelw() this search tree will not be used.
 */
static int ucm_setelw(const char *name, const char *value, int overwrite)
{
    char *lwrr_value;
    char *elw_str;
    int ret;

    pthread_mutex_lock(&ucm_malloc_hook_state.elw_lock);
    lwrr_value = getelw(name);
    if ((lwrr_value != NULL) && !overwrite) {
        ret = 0;
        goto out;
    }

    elw_str = ucm_malloc(strlen(name) + 1 + strlen(value) + 1, NULL);
    if (elw_str == NULL) {
        errno = ENOMEM;
        ret = -1;
        goto out;
    ...
    sprintf(elw_str, "%s=%s", name, value);
    ret = putelw(elw_str);
    if (ret != 0) {
        goto err_free;
    }

    ucm_add_to_elwiron(elw_str);
    ret = 0;
    goto out;

err_free:
    ucm_free(elw_str, NULL);
out:
    pthread_mutex_unlock(&ucm_malloc_hook_state.elw_lock);
    return ret;
}
...
static ucm_reloc_patch_t ucm_malloc_symbol_patches[] = {
    ...
    { .symbol = "setelw", .value = ucm_setelw, .blacklist = ucm_malloc_blacklist },
...
static void ucm_malloc_set_elw_mallopt()
{
...
UCS_STATIC_INIT {
    ucs_relwrsive_spinlock_init(&ucm_malloc_hook_state.lock, 0);
