...
static ucs_relwrsive_spinlock_t ucm_kh_lock;
...
UCS_STATIC_INIT {
    ucs_relwrsive_spinlock_init(&ucm_kh_lock, 0);
...
UCS_STATIC_CLEANUP {
    ...
    status = ucs_relwrsive_spinlock_destroy(&ucm_kh_lock);
    if (status != UCS_OK) {
        ucm_warn("ucs_relwrsive_spinlock_destroy() failed (%d)", status);
    }
}
