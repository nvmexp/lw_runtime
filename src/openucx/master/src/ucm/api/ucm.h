...
typedef enum ucm_event_type {
     ...
     * Lwrrently implemented only for @ref UCM_EVENT_MEM_TYPE_ALLOC.
     */
...
typedef struct ucm_global_config {
    ...
    int                  enable_lwda_reloc;           /* Enable installing LWCA relocations */
 ...
 * result, but **only if it's lwrrently invalid**. A valid result indicates that
 ...
 *       may lead to deadlock or infinite relwrsion.
 *
 ...
 * case the corresponding UCM function needs to be ilwoked to trigger event
 * handlers.
