...
#  - EXELWTOR_NUMBER   : number of exelwtor within the test machine
...
#  - nworkers : number of parallel exelwtors
#  - worker   : number of current parallel exelwtor
#  - COV_OPT  : command line options for Coverity static checker
#

WORKSPACE=${WORKSPACE:=$PWD}
ucx_inst=${WORKSPACE}/install
LWDA_MODULE="dev/lwda10.1"
GDRCOPY_MODULE="dev/gdrcopy1.3_lwda10.1"
...
#
# Set affinity to 2 cores according to Jenkins exelwtor number.
# Affinity is inherited from agent in Azure CI.
# TODO: remove or rename after CI migration.
#
if [ -n "$EXELWTOR_NUMBER" ] && [ -n "$JENKINS_RUN_TESTS" ]
then
	AFFINITY="taskset -c $(( 2 * EXELWTOR_NUMBER ))","$(( 2 * EXELWTOR_NUMBER + 1))"
else
	AFFINITY=""
fi
...
#
# Set up parallel test exelwtion - "worker" and "nworkers" should be set by jenkins
#
...
#
# try load lwca modules if lwpu driver is installed
#
try_load_lwda_elw() {
	num_gpus=0
	have_lwda=no
	have_gdrcopy=no
	if [ -f "/proc/driver/lwpu/version" ]; then
		have_lwda=yes
		have_gdrcopy=yes
		module_load $LWDA_MODULE    || have_lwda=no
		module_load $GDRCOPY_MODULE || have_gdrcopy=no
		num_gpus=$(lwpu-smi -L | wc -l)
	fi
}

unload_lwda_elw() {
	module unload $LWDA_MODULE
	module unload $GDRCOPY_MODULE
}

#
# Check whether this test should do a task with given index,
# according to the parallel test exelwtion parameters.
#
should_do_task() {
	set +x
	task=$1
...
build_release_pkg() {
	...
	cd ${WORKSPACE}
	# extract version from configure.ac and colwert to MAJOR.MINOR.PATCH representation
...
#
build_lwda() {
	echo 1..1 > build_lwda.tap
	if module_load $LWDA_MODULE
	then
		if module_load $GDRCOPY_MODULE
		then
			echo "==== Build with enable lwca, gdr_copy ===="
			../contrib/configure-devel --prefix=$ucx_inst --with-lwca --with-gdrcopy
			$MAKEP clean
			$MAKEP
			$MAKEP distclean

			../contrib/configure-release --prefix=$ucx_inst --with-lwca --with-gdrcopy
			$MAKEP clean
			$MAKEP
			$MAKEP distclean
			module unload $GDRCOPY_MODULE
		...
		echo "==== Build with enable lwca, w/o gdr_copy ===="
		../contrib/configure-devel --prefix=$ucx_inst --with-lwca --without-gdrcopy
		$MAKEP clean
		$MAKEP

		module unload $LWDA_MODULE

		echo "==== Running test_link_map with lwca build but no lwca module ===="
		elw UCX_HANDLE_ERRORS=bt ./test/apps/test_link_map

		$MAKEP distclean
		echo "ok 1 - build successful " >> build_lwda.tap
	else
		echo "==== Not building with lwca flags ===="
		echo "ok 1 - # SKIP because lwca not installed" >> build_lwda.tap
	fi
	unload_lwda_elw
}
...
run_client_server_app() {
	...
	server_port=$((10000 + EXELWTOR_NUMBER))
...
run_ucp_hello() {
	...
	if [ "X$have_lwda" == "Xyes" ]
	then
		mem_types_list+="lwca lwca-managed "
	fi
...
run_uct_hello() {
	...
	if [ "X$have_lwda" == "Xyes" ] && [ -f "/sys/kernel/mm/memory_peers/lw_mem/version" ]
	then
		mem_types_list+="lwca-managed "
		if [ -f "/sys/kernel/mm/memory_peers/lw_mem/version" ]
		then
			# test RDMA GPUDirect
			mem_types_list+="lwca "
		fi
...
run_ucx_perftest() {
	...
	cat $ucx_inst_ptest/test_types_ucp | grep -v lwca | sort -R > $ucx_inst_ptest/test_types_short_ucp
	...
	# run lwca tests if lwca module was loaded and GPU is found
	if [ "X$have_lwda" == "Xyes" ]
	then
		tls_list="all "
		gdr_options="n "
		if (lsmod | grep -q "lw_peer_mem")
		then
			echo "GPUDirectRDMA module (lw_peer_mem) is present.."
			tls_list+="rc,lwda_copy "
			gdr_options+="y "
		fi
		...
		then
			...
			tls_list+="rc,lwda_copy,gdr_copy "
		fi

		if [ $num_gpus -gt 1 ]; then
			export LWDA_VISIBLE_DEVICES=$(($worker%$num_gpus)),$(($(($worker+1))%$num_gpus))
		fi

		cat $ucx_inst_ptest/test_types_ucp | grep lwca | sort -R > $ucx_inst_ptest/test_types_short_ucp
		sed -s 's,-n [0-9]*,-n 10 -w 1,g' $ucx_inst_ptest/msg_pow2 | sort -R > $ucx_inst_ptest/msg_pow2_short

		echo "==== Running ucx_perf with lwca memory===="
		...
		if [ $with_mpi -eq 1 ]
		then
			$MPIRUN -np 2 -x UCX_TLS=self,shm,cma,lwda_copy $AFFINITY $ucx_perftest $ucp_test_args
			$MPIRUN -np 2 -x UCX_TLS=self,sm,lwda_ipc,lwda_copy $AFFINITY $ucx_perftest $ucp_test_args
			$MPIRUN -np 2 $AFFINITY $ucx_perftest $ucp_test_args
		else
			export UCX_TLS=self,shm,cma,lwda_copy
			run_client_server_app "$ucx_perftest" "$ucp_test_args" "$(hostname)" 0 0
			unset UCX_TLS
		...
		# Specifically test lwda_ipc for large message sizes
	        cat $ucx_inst_ptest/test_types_ucp | grep -v lwca | sort -R > $ucx_inst_ptest/test_types_lwda_ucp
		ucp_test_args_large="-b $ucx_inst_ptest/test_types_lwda_ucp \
			             -b $ucx_inst_ptest/msg_pow2_large -w 1"
		if [ $with_mpi -eq 1 ]
		then
			for ipc_cache in y n
			do
				$MPIRUN -np 2 -x UCX_TLS=self,sm,lwda_copy,lwda_ipc \
					-x UCX_LWDA_IPC_CACHE=$ipc_cache $AFFINITY $ucx_perftest $ucp_test_args_large
			done
		else
			for ipc_cache in y n
			do
				export UCX_TLS=self,sm,lwda_copy,lwda_ipc
				export UCX_LWDA_IPC_CACHE=$ipc_cache
				run_client_server_app "$ucx_perftest" "$ucp_test_args_large" "$(hostname)" 0 0
				unset UCX_TLS
				unset UCX_LWDA_IPC_CACHE
			done
		fi

		unset LWDA_VISIBLE_DEVICES
	fi
}
...
test_unused_elw_var() {
	...
	echo "==== Running ucx_info elw vars test ===="
	UCX_IB_PORTS=mlx5_0:1 ./src/tools/info/ucx_info -epw -u t | grep "unused" | grep -q "UCX_IB_PORTS"
}

test_elw_var_aliases() {
	echo "==== Running MLX5 elw var aliases test ===="
	...
	then
		...
		do
			...
			do
				...
				# To check that changing elw var takes an effect,
...
test_jucx() {
	...
	then
		jucx_port=$((20000 + EXELWTOR_NUMBER))
...
run_coverity() {
	...
	then
		...
		cov-analyze --jobs $parallel_jobs $COV_OPT --security --conlwrrency --dir $cov_build
...
run_gtest_watchdog_test() {
	...
	elw WATCHDOG_GTEST_TIMEOUT_=$watchdog_timeout \
...
run_gtest() {
	...
	if [ $num_gpus -gt 0 ]; then
		export LWDA_VISIBLE_DEVICES=$(($worker%$num_gpus))
	fi
	...
	$AFFINITY $TIMEOUT \
		elw MALLOC_MMAP_THRESHOLD_=16384 \
		GTEST_SHARD_INDEX=0 \
		GTEST_TOTAL_SHARDS=1 \
...
run_gtest_release() {
	...
	elw GTEST_FILTER=\*test_obj_size\*:\*test_ucp_tag_match.rndv_rts_unexp\* \
		$AFFINITY $TIMEOUT make -C test/gtest test
...
run_tests() {
	...
	# test lwca build if lwca modules available
	do_distributed_task 2 4 build_lwda

	# load lwca elw only if GPU available for remaining tests
	try_load_lwda_elw
	...
	do_distributed_task 0 4 test_unused_elw_var
	do_distributed_task 2 4 test_elw_var_aliases
