...
#define LWDA_FUNC(_func)                                   \
    do {                                                   \
        lwdaError_t _result = (_func);                     \
        if (lwdaSuccess != _result) {                      \
            fprintf(stderr, "%s failed: %s\n",             \
                    #_func, lwdaGetErrorString(_result));  \
        }                                                  \
    } while(0)
...
void *mem_type_malloc(size_t length)
{
    ...
    case UCS_MEMORY_TYPE_LWDA:
        LWDA_FUNC(lwdaMalloc(&ptr, length));
        break;
    case UCS_MEMORY_TYPE_LWDA_MANAGED:
        LWDA_FUNC(lwdaMallocManaged(&ptr, length, lwdaMemAttachGlobal));
        break;
#endif
    default:
...
void *mem_type_memcpy(void *dst, const void *src, size_t count)
{
    ...
    case UCS_MEMORY_TYPE_LWDA:
    case UCS_MEMORY_TYPE_LWDA_MANAGED:
        LWDA_FUNC(lwdaMemcpy(dst, src, count, lwdaMemcpyDefault));
        break;
#endif
    default:
        fprintf(stderr, "Unsupported memory type: %d\n", test_mem_type);
...
void *mem_type_memset(void *dst, int value, size_t count)
{
    ...
    case UCS_MEMORY_TYPE_LWDA:
    case UCS_MEMORY_TYPE_LWDA_MANAGED:
        LWDA_FUNC(lwdaMemset(dst, value, count));
        break;
#endif
    default:
...
ucs_memory_type_t parse_mem_type(const char *opt_arg)
{
    ...
    } else if (!strcmp(opt_arg, "lwca") &&
               check_mem_type_support(UCS_MEMORY_TYPE_LWDA)) {
        return UCS_MEMORY_TYPE_LWDA;
    } else if (!strcmp(opt_arg, "lwca-managed") &&
               check_mem_type_support(UCS_MEMORY_TYPE_LWDA_MANAGED)) {
        return UCS_MEMORY_TYPE_LWDA_MANAGED;
    } else {
...
void print_common_help()
{
    ...
    if (check_mem_type_support(UCS_MEMORY_TYPE_LWDA)) {
        fprintf(stderr, "                 lwca - LWPU GPU memory\n");
    }
    if (check_mem_type_support(UCS_MEMORY_TYPE_LWDA_MANAGED)) {
        fprintf(stderr, "                 lwca-managed - LWPU GPU managed/unified memory\n");
    }
}
