 ...
 *    - Lwrrently, the passed IP needs to be an IPoIB or a RoCE address.
...
/**
 * The callback on the receiving side, which is ilwoked upon receiving the
 * stream message.
 */
...
/**
 * The callback on the sending side, which is ilwoked after finishing sending
 * the message.
 */
...
{
    printf("error handling callback was ilwoked with status %d (%s)\n",
 ...
 * side.
 */
static void print_result(int is_server, char *recv_message, int lwrrent_iter)
{
    if (is_server) {
...
static int request_finalize(ucp_worker_h ucp_worker, test_req_t *request,
                            int is_server, char *recv_message, int lwrrent_iter)
{
    ucs_status_t status;
    int ret = 0;
    ...
    if ((lwrrent_iter == 0) || (lwrrent_iter == (num_iterations - 1)) ||
        !((lwrrent_iter + 1) % (PRINT_INTERVAL))) {
        print_result(is_server, recv_message, lwrrent_iter);
    }
 ...
 * Lwrrently closing the endpoint with UCP_EP_CLOSE_MODE_FORCE since we lwrrently
...
/**
 * A callback to be ilwoked by UCX in order to initialize the user's request.
 */
...
static int client_server_communication(ucp_worker_h worker, ucp_ep_h ep,
                                       ...
                                       int is_server, int lwrrent_iter)
{
    int ret;
    ...
    case CLIENT_SERVER_SEND_RECV_STREAM:
        ...
        ret = send_recv_stream(worker, ep, is_server, lwrrent_iter);
        break;
    case CLIENT_SERVER_SEND_RECV_TAG:
        /* Client-Server communication via Tag-Matching API */
        ret = send_recv_tag(worker, ep, is_server, lwrrent_iter);
        break;
    default:
...
/**
 * The callback on the server side which is ilwoked upon receiving a connection
                      ...
                      char *listen_addr, send_recv_type_t send_recv_type)
{
    ...
    while (1) {
         ...
         * callback is ilwoked, i.e. several clients are trying to connect in
