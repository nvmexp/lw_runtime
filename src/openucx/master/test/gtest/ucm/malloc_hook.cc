...
void test_thread::test() {
    ...
    setelw("TEST", "VALUE", 1);
    test_str = getelw("TEST");
    if (test_str != NULL) {
        EXPECT_EQ(std::string("VALUE"), test_str);
    } else {
        UCS_TEST_ABORT("getelw(\"TEST\") returned NULL");
    }
...
UCS_TEST_SKIP_COND_F(malloc_hook_cplusplus, mallopt,
     ...
     * that the malloc hooks are installed after the setelw()
     */
    p = getelw("UCX_IB_RCACHE");
    if ((p == NULL) || (p[0] != 'n')) {
...
UCS_TEST_SKIP_COND_F(malloc_hook, bistro_patch, RUNNING_ON_VALGRIND) {
    ...
    EXPECT_EQ(res, 0);
    /* due to cache coherency issues on ARM systems could be exelwted
