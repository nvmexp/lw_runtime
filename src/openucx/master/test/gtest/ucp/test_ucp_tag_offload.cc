...
{
    m_elw.push_back(new ucs::scoped_setelw("UCX_RC_TM_ENABLE", "n"));
...
public:
    test_ucp_tag_offload_selection() {
        m_elw.push_back(new ucs::scoped_setelw("UCX_RC_TM_ENABLE", "y"));
    }
...
class test_ucp_tag_offload_lwda : public test_ucp_tag_offload {
public:
    test_ucp_tag_offload_lwda() {
...
UCS_TEST_P(test_ucp_tag_offload_lwda, sw_rndv_to_lwda_mem, "TM_SW_RNDV=y")
{
    activate_offload(sender());

    size_t size   = 2048;
    ucp_tag_t tag = 0xCAFEBABEul;
    // Test will be skipped here if LWCA mem is not supported
    mem_buffer rbuf(size, UCS_MEMORY_TYPE_LWDA);
...
UCP_INSTANTIATE_TEST_CASE_TLS(test_ucp_tag_offload_lwda, rc_dc_lwda,
                              "dc_x,rc_x,lwda_copy")
...
class test_ucp_tag_offload_stats_lwda : public test_ucp_tag_offload_stats {
public:
    test_ucp_tag_offload_stats_lwda() {
        m_elw.push_back(new ucs::scoped_setelw("UCX_IB_GPU_DIRECT_RDMA", "n"));
    }
};

UCS_TEST_P(test_ucp_tag_offload_stats_lwda, block_lwda_no_gpu_direct,
           "TM_THRESH=1")
{
    activate_offload(sender());
...
UCP_INSTANTIATE_TEST_CASE_TLS(test_ucp_tag_offload_stats_lwda, rc_dc_lwda,
                              "dc_x,rc_x,lwda_copy")
