...
UCS_TEST_F(test_sys, spinlock) {
    ...
    ucs_relwrsive_spinlock_init(&lock, 0);

    ucs_relwrsive_spin_lock(&lock);
    EXPECT_TRUE(ucs_relwrsive_spin_is_owner(&lock, self));

    /* coverity[double_lock] */
    ucs_relwrsive_spin_lock(&lock);
    EXPECT_TRUE(ucs_relwrsive_spin_is_owner(&lock, self));

    ucs_relwrsive_spin_unlock(&lock);
    EXPECT_TRUE(ucs_relwrsive_spin_is_owner(&lock, self));

    /* coverity[double_unlock] */
    ucs_relwrsive_spin_unlock(&lock);
    EXPECT_FALSE(ucs_relwrsive_spin_is_owner(&lock, self));
}
