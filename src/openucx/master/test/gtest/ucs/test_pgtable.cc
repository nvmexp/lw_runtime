...
UCS_TEST_SKIP_COND_F(test_pgtable, ilwalid_param,
                     (UCS_PGT_ADDR_ALIGN == 1)) {
    ucs_pgt_region_t region1 = {0x4000, 0x4001};
    insert(&region1, UCS_ERR_ILWALID_PARAM);

    ucs_pgt_region_t region2 = {0x4001, 0x5000};
    insert(&region2, UCS_ERR_ILWALID_PARAM);

    ucs_pgt_region_t region3 = {0x5000, 0x4000};
    insert(&region3, UCS_ERR_ILWALID_PARAM);
}
...
protected:
    ...
    void measure_workload(ucs_pgt_addr_t max_addr,
                          ...
                          unsigned blocks_per_superblock, /* Number of conselwtive basic blocks per big block */
