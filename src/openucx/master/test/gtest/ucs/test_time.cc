...
UCS_TEST_SKIP_COND_F(test_time, get_time,
    ...
    if (ucs::perf_retry_count) {
        ...
        double nsec = (ucs_time_to_nsec(lwrrent_time - start_time)) / count;
...
UCS_TEST_F(test_time, timerq) {
    ...
    for (unsigned test_count = 0; test_count < 500; ++test_count) {
        ...
        ucs_time_t lwrrent_time = time_base;
        ...
        /*
         * Check that both timers are ilwoked
         */
        counter1 = 0;
        counter2 = 0;
        for (unsigned count = 0; count < test_time; ++count) {
            ++lwrrent_time;
            ucs_timerq_for_each_expired(timer, &timerq, lwrrent_time, {
        ...
        /*
         * Check that after canceling, only one timer is ilwoked
         */
        counter1 = 0;
        counter2 = 0;
        ...
        /*
         * Check that after rescheduling, both timers are ilwoked again
         */
        ucs_timerq_add(&timerq, TIMER_ID_1, interval1);
