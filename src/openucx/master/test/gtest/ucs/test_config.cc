...
protected:
    ...
    class car_opts {
    public:
        car_opts(const char *elw_prefix, const char *table_prefix) :
            m_opts(parse(elw_prefix, table_prefix)), m_max(1024), m_value(NULL)
        {
    ...
    private:
        ...
        static car_opts_t parse(const char *elw_prefix,
                                       ...
                                       const char *prefix = NULL)
    {
        ...
        char line_buf[1024];
        char alias[128];
        car_opts opts(UCS_DEFAULT_ELW_PREFIX, NULL);
...
UCS_TEST_F(test_config, parse_default) {
    car_opts opts(UCS_DEFAULT_ELW_PREFIX, "TEST");
...
UCS_TEST_F(test_config, clone) {
...

    {
        ...
        ucs::scoped_setelw elw1("UCX_COLOR", "white");
        /* coverity[tainted_string_argument] */
        ucs::scoped_setelw elw2("UCX_PRICE_ALIAS", "0");
        ...
        /* coverity[tainted_string_argument] */
        ucs::scoped_setelw elw3("UCX_COLOR", "black");
...
UCS_TEST_F(test_config, set_get_with_table_prefix) {
    /* coverity[tainted_string_argument] */
    ucs::scoped_setelw elw1("UCX_COLOR", "black");
    /* coverity[tainted_string_argument] */
    ucs::scoped_setelw elw2("UCX_CARS_COLOR", "white");

    car_opts opts(UCS_DEFAULT_ELW_PREFIX, "CARS_");
    EXPECT_EQ(COLOR_WHITE, opts->color);
    EXPECT_EQ(std::string(color_names[COLOR_WHITE]),
              std::string(opts.get("COLOR")));
}

UCS_TEST_F(test_config, set_get_with_elw_prefix) {
    /* coverity[tainted_string_argument] */
    ucs::scoped_setelw elw1("UCX_COLOR", "black");
    /* coverity[tainted_string_argument] */
    ucs::scoped_setelw elw2("TEST_UCX_COLOR", "white");

    car_opts opts("TEST_" UCS_DEFAULT_ELW_PREFIX, NULL);
    EXPECT_EQ(COLOR_WHITE, opts->color);
    EXPECT_EQ(std::string(color_names[COLOR_WHITE]),
              std::string(opts.get("COLOR")));
}
...
UCS_TEST_F(test_config, performance) {
    ...
    /* Add stuff to elw to presumably make getelw() slower */
    ucs::ptr_vector<ucs::scoped_setelw> elw;
    for (unsigned i = 0; i < 300; ++i) {
        elw.push_back(new ucs::scoped_setelw(
...
UCS_TEST_F(test_config, unused) {
    ucs::ucx_elw_cleanup elw_cleanup;

    /* set to warn about unused elw vars */
    ucs_global_opts.warn_unused_elw_vars = 1;

    const std::string warn_str    = "unused elw variable";
    const std::string unused_var1 = "UCX_UNUSED_VAR1";
    /* coverity[tainted_string_argument] */
    ucs::scoped_setelw elw1(unused_var1.c_str(), "unused");

    {
        ...
        ucs_config_parser_warn_unused_elw_vars_once(UCS_DEFAULT_ELW_PREFIX);
...

    {
        ...
        /* coverity[tainted_string_argument] */
        ucs::scoped_setelw elw2(unused_var2.c_str(), "unused");

        config_err_exp_str.push_back(warn_str + ": " + unused_var2);
        scoped_log_handler log_handler(config_error_handler);
        car_opts opts("TEST_", NULL);

        ucs_config_parser_warn_unused_elw_vars_once("TEST_");

        config_err_exp_str.pop_back();
    }

    /* reset to not warn about unused elw vars */
    ucs_global_opts.warn_unused_elw_vars = 0;
}
...
UCS_TEST_F(test_config, deprecated) {
    ...
    /* coverity[tainted_string_argument] */
    ucs::scoped_setelw elw1(deprecated_var1.c_str(), "Taxi driver");
...

    {
        ...
        /* coverity[tainted_string_argument] */
        ucs::scoped_setelw elw2(deprecated_var2.c_str(), "58");
