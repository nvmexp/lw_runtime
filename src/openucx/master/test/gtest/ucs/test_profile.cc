...
{
    const int ITER           = 5;
    uint64_t exp_count       = (int_mode & UCS_BIT(UCS_PROFILE_MODE_ACLWM)) ?
                               ITER : 0;
...
UCS_TEST_P(test_profile, aclwm) {
    do_test(UCS_BIT(UCS_PROFILE_MODE_ACLWM), "aclwm");
}
...
UCS_TEST_P(test_profile, log_aclwm) {
    do_test(UCS_BIT(UCS_PROFILE_MODE_LOG) | UCS_BIT(UCS_PROFILE_MODE_ACLWM),
            "log,aclwm");
}
...
UCS_TEST_SKIP_COND_P(test_profile_perf, overhead, RUNNING_ON_VALGRIND) {
    ...
    scoped_profile p(*this, PROFILE_FILENAME, "aclwm");
