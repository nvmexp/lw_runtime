...
UCS_TEST_F(test_math, colwert_flag) {
    volatile uint32_t value = FLAG1 | FLAG3;
    volatile uint32_t tmp = ucs_colwert_flag(value, FLAG1, 0x1);

    EXPECT_EQ(0x1u, tmp);
    EXPECT_EQ(0x0u, ucs_colwert_flag(value, FLAG2, 0x2u));
    EXPECT_EQ(0x4u, ucs_colwert_flag(value, FLAG3, 0x4u));

    EXPECT_EQ(0x10000u, ucs_colwert_flag(value, FLAG1, 0x10000u));
    EXPECT_EQ(0x00000u, ucs_colwert_flag(value, FLAG2, 0x20000u));
    EXPECT_EQ(0x40000u, ucs_colwert_flag(value, FLAG3, 0x40000u));
}
...
UCS_TEST_F(test_math, cirlwlar_compare) {
    EXPECT_TRUE(  UCS_CIRLWLAR_COMPARE32(0x000000001, <,  0x000000002) );
    EXPECT_TRUE(  UCS_CIRLWLAR_COMPARE32(0x000000001, ==, 0x000000001) );
    EXPECT_TRUE(  UCS_CIRLWLAR_COMPARE32(0xffffffffU, >,  0xfffffffeU) );
    EXPECT_TRUE(  UCS_CIRLWLAR_COMPARE32(0xffffffffU, <,  0x00000000U) );
    EXPECT_TRUE(  UCS_CIRLWLAR_COMPARE32(0xffffffffU, <,  0x00000001U) );
    EXPECT_TRUE(  UCS_CIRLWLAR_COMPARE32(0xffffffffU, <,  0x00000001U) );
    EXPECT_TRUE(  UCS_CIRLWLAR_COMPARE32(0x80000000U, >,  0x7fffffffU) );
    EXPECT_TRUE(  UCS_CIRLWLAR_COMPARE32(0xffffffffU, <,  0x7fffffffU) );
}
