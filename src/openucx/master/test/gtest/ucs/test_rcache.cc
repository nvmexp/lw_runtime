...
UCS_MT_TEST_F(test_rcache, merge_ilw, 6) {
    /*
     * Merge with another region which causes immediate ilwalidation of the
     * other region.
     * +---------+
     * | region1 |
     * +---+-----+----------+
 ...
 * +-----+---------------+
 * |     |    region2    |  4. region2 is created. region1 must be ilwalidated and
 * +-----+---------------+     kicked out of pagetable.
 */
UCS_MT_TEST_F(test_rcache, merge_ilwalid_prot, 6)
{
    static const size_t size1 = 10 * ucs_get_page_size();
    static const size_t size2 =  8 * ucs_get_page_size();
    int ret;
 ...
 * +-----+---------------+
 * |     |    region2(w) |  4. region2 is created. region1 must be ilwalidated and
 * +-----+---------------+     kicked out of pagetable. Creation of region2
 *                             must fail.
 */
UCS_MT_TEST_F(test_rcache_no_register, merge_ilwalid_prot_slow, 5)
{
    static const size_t size1 = 10 * ucs_get_page_size();
    static const size_t size2 =  8 * ucs_get_page_size();
    int ret;
...
protected:
    ...
    void dump_stats() {
        printf("gets %d hf %d hs %d misses %d merges %d unmaps %d"
               " unmaps_ilw %d puts %d regs %d deregs %d\n",
               ...
               get_counter(UCS_RCACHE_UNMAP_ILWALIDATES),
...
UCS_TEST_F(test_rcache_stats, unmap_dereg) {
    ...
    /* Should generate umap event but no dereg or unmap ilwalidation.
    ...
    EXPECT_EQ(0, get_counter(UCS_RCACHE_UNMAP_ILWALIDATES));
    EXPECT_EQ(0, get_counter(UCS_RCACHE_DEREGS));

    mem = alloc_pages(size1, PROT_READ|PROT_WRITE);

    /*
     * Adding a new region shall force a processing of ilwalidation queue and dereg
     */
    r1 = get(mem, size1);
    EXPECT_GE(get_counter(UCS_RCACHE_UNMAPS), 1);
    EXPECT_EQ(1, get_counter(UCS_RCACHE_UNMAP_ILWALIDATES));
