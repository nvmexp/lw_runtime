...
{
    static bool tcp_fastest_dev = (getelw("GTEST_UCT_TCP_FASTEST_DEV") != NULL);
...
bool uct_test::check_caps(uint64_t required_flags, uint64_t ilwalid_flags) {
    FOR_EACH_ENTITY(iter) {
        if (!(*iter)->check_caps(required_flags, ilwalid_flags)) {
            return false;
        }
    }
    return true;
...
void uct_test::check_caps_skip(uint64_t required_flags, uint64_t ilwalid_flags) {
    if (!check_caps(required_flags, ilwalid_flags)) {
...
bool uct_test::entity::check_caps(uint64_t required_flags,
                                  uint64_t ilwalid_flags)
{
    uint64_t iface_flags = iface_attr().cap.flags;
    return (ucs_test_all_flags(iface_flags, required_flags) &&
            !(iface_flags & ilwalid_flags));
}
