...
public:
                             ...
                             bool take_uct_desc = false)
    {
        ...
        if (static_cast<send_func>(&test_tag::tag_rndv_zcopy) == sfunc) {
            // Need to cancel origin RNDV operation, bealwse no RNDV_COMP
...
private:
    ...
    void set_elw_var_or_skip(void *config, const char *var, const char *val);
    size_t           m_max_hdr;
                                     ...
                                     m_last_received(false)
{
    ...
    m_uct_comp.count = 512; // We do not need completion func to be ilwoked
...
void test_tag_mp_xrq::set_elw_var_or_skip(void *config, const char *var,
                                          const char *val)
{
...
{
    set_elw_var_or_skip(m_iface_config, "RC_TM_ENABLE", "y");
    set_elw_var_or_skip(m_iface_config, "RC_TM_MP_SRQ_ENABLE", "try");
    set_elw_var_or_skip(m_iface_config, "RC_TM_MP_NUM_STRIDES", "8");
    set_elw_var_or_skip(m_md_config, "MLX5_DEVX_OBJECTS", "dct,dcsrq,rcsrq,rcqp");
