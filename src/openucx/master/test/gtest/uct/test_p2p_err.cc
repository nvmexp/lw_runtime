...
public:
                        ...
                        const std::string& error_pattern)
    {
        ...
        EXPECT_GT(num_matched, 0ul) <<
                        ...
                        "' has oclwrred during the test";
        EXPECT_EQ(0ul, num_unmatched) <<
                        "Unexpected error(s) oclwrred during the test";
    }
...
UCS_TEST_SKIP_COND_P(uct_p2p_err_test, ilwalid_put_short_length,
...
UCS_TEST_SKIP_COND_P(uct_p2p_err_test, ilwalid_put_bcopy_length,
...
UCS_TEST_SKIP_COND_P(uct_p2p_err_test, ilwalid_am_short_length,
...
UCS_TEST_SKIP_COND_P(uct_p2p_err_test, ilwalid_am_bcopy_length,
...
UCS_TEST_SKIP_COND_P(uct_p2p_err_test, ilwalid_am_zcopy_hdr_length,
...
UCS_TEST_SKIP_COND_P(uct_p2p_err_test, short_ilwalid_am_id,
                     !check_caps(UCT_IFACE_FLAG_AM_SHORT)) {
    mapped_buffer sendbuf(4, 2, sender());

    test_error_run(OP_AM_SHORT, UCT_AM_ID_MAX, sendbuf.ptr(), sendbuf.length(),
                   UCT_MEM_HANDLE_NULL, 0, UCT_ILWALID_RKEY,
                   "active message id");
}

UCS_TEST_SKIP_COND_P(uct_p2p_err_test, bcopy_ilwalid_am_id,
                     !check_caps(UCT_IFACE_FLAG_AM_BCOPY)) {
    mapped_buffer sendbuf(4, 2, sender());
...
UCS_TEST_SKIP_COND_P(uct_p2p_err_test, zcopy_ilwalid_am_id,
                     !check_caps(UCT_IFACE_FLAG_AM_ZCOPY)) {
    mapped_buffer sendbuf(4, 2, sender());
