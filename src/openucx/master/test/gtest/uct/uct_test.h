...
protected:
    ...
    public:
        ...
        bool check_caps(uint64_t required_flags, uint64_t ilwalid_flags = 0);
    ...
    private:
        ...
        void lwda_mem_alloc(size_t length, uct_allocated_memory_t *mem) const;
        void lwda_mem_free(const uct_allocated_memory_t *mem) const;
    ...
    bool is_caps_supported(uint64_t required_flags);
    bool check_caps(uint64_t required_flags, uint64_t ilwalid_flags = 0);
    void check_caps_skip(uint64_t required_flags, uint64_t ilwalid_flags = 0);
    bool check_atomics(uint64_t required_ops, atomic_mode mode);
...
#define UCT_TEST_LWDA_MEM_TYPE_TLS \
    lwda_copy,              \
    gdr_copy
