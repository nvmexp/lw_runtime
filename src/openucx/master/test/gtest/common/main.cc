...
static void modify_config_for_valgrind(const char *name, const char *value)
{
    ...
    snprintf(full_name, sizeof(full_name), "%s%s", UCS_DEFAULT_ELW_PREFIX, name);

    if (getelw(full_name) == NULL) {
        UCS_TEST_MESSAGE << " Setting for valgrind: " << full_name << "=" << value;
        setelw(full_name, value, 1);
    }
}
...
int main(int argc, char **argv) {
    ...
    ucs_global_opts.warn_unused_elw_vars = 0; /* Avoid warnings if not all
