...
class scoped_setelw {
public:
    scoped_setelw(const char *name, const char *value);
    ~scoped_setelw();
private:
    scoped_setelw(const scoped_setelw&);
    const std::string m_name;
    std::string       m_old_value;
};

class ucx_elw_cleanup {
public:
    ucx_elw_cleanup();
    ~ucx_elw_cleanup();
private:
    std::vector<std::string> ucx_elw_storage;
};
...
public:
    ...
    void reset(const T& value, dtor_t dtor) {
        reset();
        if (value == NULL) {
            throw std::ilwalid_argument("value cannot be NULL");
        }
        m_value = value;
        m_dtor  = dtor;
