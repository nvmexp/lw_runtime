...
static int show_result(const struct timeval *tv_prev,
                       const struct timeval *tv_lwrr,
                       long iters, size_t msg_size, int force)
{
    double elapsed;

    elapsed = (tv_lwrr->tv_sec + tv_lwrr->tv_usec * 1e-6) -
...
{
    struct timeval tv_prev, tv_lwrr;
    int my_pe, dst_pe;
    volatile int *rsn;
    char *msg;
    int *ssn;
    ...
    for (i = 0; i < num_iters; ++i) {
        ...
        *ssn = sn;
        ...
        if ((i % 1000) == 0) {
            gettimeofday(&tv_lwrr, NULL);
            if (show_result(&tv_prev, &tv_lwrr, i - prev_i, msg_size, 0)) {
                prev_i = i;
                tv_prev = tv_lwrr;
            }
        }
    ...
    gettimeofday(&tv_lwrr, NULL);
    show_result(&tv_prev, &tv_lwrr, num_iters - prev_i, msg_size, 1);
    free(msg);
}
