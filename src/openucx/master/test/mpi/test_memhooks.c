...
{
    setelw("UCX_MEM_MALLOC_HOOKS", "n", 1);
    setelw("UCX_MEM_MMAP_RELOC",   "n", 1);
    return UCS_OK;
}
...
int malloc_hooks_run_flags(void *dl, ucm_event_type_t events)
{
    ...
    const char *lwst_mmap_name  = "memhook_test_lib_call_mmap";
    void * (*lwst_mmap)(size_t size);
    ...
    DL_FIND_FUNC(dl_test, lwst_mmap_name, lwst_mmap, goto fail_close_all);
    total_mapped = 0;
    ptr_direct_mmap = lwst_mmap(size);
...
int ext_event_run(void *dl)
{
    ...
    CHKERR_JUMP(total_mapped != 0,
                "Callback for mmap ilwoked, while hooks were not set", fail);
    ...
    CHKERR_JUMP(total_unmapped != 0,
                "Callback for munmap ilwoked, while hooks were not set\n", fail);
