...
sub process_subdir {
    ...
    # Ensure that we got a good configure exelwtable.
    my_die "Did not generate a \"configure\" exelwtable in $dir.\n"
        if (! -x "configure");
...
##############################################################################
# Find a specific exelwtable and ensure that it is a recent enough
# version.
...
if (!$ok || $help_arg) {
  ...
  --force | -f                  Run even if ilwoked from the source tree of an expanded
...
my_die "autogen.pl has been ilwoked in the source tree of an Open MPI distribution tarball; aborting...
You likely do not need to ilwoke \"autogen.pl\" -- you can probably run \"configure\" directly.
...
foreach my $s (@scripts) {
    verbose "=== $s\n";
    if (! -x $s) {
        print "Cannot find exelwtable $s!\nAborting.\n";
        my_exit(1);
    }
    if (system($s) != 0) {
