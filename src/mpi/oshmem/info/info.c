 ...
 * Global variables
 */
oshmem_info_t oshmem_shmem_info_elw = {
    false,
    false,
    false,
    256 * 1024 * 1024
...
int oshmem_info_init(void)
{
    ...
    if (NULL != (cptr = getelw(OSHMEM_ELW_VERSION))) {
        ret = oshmem_info_value_to_bool(cptr, &oshmem_shmem_info_elw.print_version);
        if (OSHMEM_SUCCESS != ret) {
            goto out;
        }
    }
    if (oshmem_shmem_info_elw.print_version && 0 == ORTE_PROC_MY_NAME->vpid) {
    ...
    if (NULL != (cptr = getelw(OSHMEM_ELW_INFO))) {
        ret = oshmem_info_value_to_bool(cptr, &oshmem_shmem_info_elw.print_info);
        if (OSHMEM_SUCCESS != ret) {
            goto out;
        }
    }
    if (oshmem_shmem_info_elw.print_info && 0 == ORTE_PROC_MY_NAME->vpid) {
        orte_show_help("help-shmem-runtime.txt",
                       "oshmem_init:print-info",
                       true,
                       OSHMEM_ELW_VERSION,
                       OSHMEM_ELW_INFO,
                       OSHMEM_ELW_SYMMETRIC_SIZE,
                       OSHMEM_ELW_DEBUG);
    }

    if (NULL != (cptr = getelw(OSHMEM_ELW_DEBUG))) {
        ret = oshmem_info_value_to_bool(cptr, &oshmem_shmem_info_elw.debug);
        if (OSHMEM_SUCCESS != ret) {
            goto out;
        }
    }

    if (NULL != (cptr = getelw(OSHMEM_ELW_SYMMETRIC_SIZE))) {
        char *p1 = getelw(SHMEM_HEAP_SIZE);
        if (p1 && strcmp(cptr, p1)) {
            SHMEM_API_ERROR("Found conflict between elw '%s' and '%s'.\n",
                          OSHMEM_ELW_SYMMETRIC_SIZE, SHMEM_HEAP_SIZE);
            ret = OSHMEM_ERR_BAD_PARAM;
            goto out;
        }
        ret = oshmem_info_get_heap_size(cptr, &oshmem_shmem_info_elw.symmetric_heap_size);
        if (OSHMEM_SUCCESS != ret) {
            goto out;
        }
    } else if (NULL != (cptr = getelw(SHMEM_HEAP_SIZE))) {
        ret = oshmem_info_get_heap_size(cptr, &oshmem_shmem_info_elw.symmetric_heap_size);
        if (OSHMEM_SUCCESS != ret) {
            goto out;
        }
...
/**
 * Colwert value string to boolean
 *
 * Colwert value string \c value into a boolean.  The
 * strings "true", "false", and integer numbers can be colwerted
...
/**
 * Colwert value string to integer
 *
 * Colwert value string \c value into a integer.
...
static int oshmem_info_value_to_int(char *value, int *interp)
{
    ...
    /* underflow */
    if (tmp == 0 && errno == EILWAL) return OSHMEM_ERR_BAD_PARAM;
...
static int oshmem_info_get_heap_size(char *value, size_t *interp)
{
    ...
    *interp = 0;
    ...
    if (size <= 0) {
        ...
        opal_setelw(OSHMEM_ELW_SYMMETRIC_SIZE, p, true, &elwiron);
        opal_setelw(SHMEM_HEAP_SIZE, p, true, &elwiron);
