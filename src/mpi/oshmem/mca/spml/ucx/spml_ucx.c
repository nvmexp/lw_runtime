...
static int mca_spml_ucx_ctx_create_common(long options, mca_spml_ucx_ctx_t **ucx_ctx_p)
{
    ...
    int j;
    unsigned int lwr_ucp_worker = mca_spml_ucx.ucp_worker_cnt++ % mca_spml_ucx.ucp_workers;
    ucs_status_t err;
    spml_ucx_mkey_t *ucx_mkey;
    sshmem_mkey_t *mkey;
    ...
    for (i = 0; i < nprocs; i++) {
        ep_params.field_mask = UCP_EP_PARAM_FIELD_REMOTE_ADDRESS;
        ep_params.address    = (ucp_address_t *)(mca_spml_ucx.remote_addrs_tbl[lwr_ucp_worker][i]);
...
int mca_spml_ucx_quiet(shmem_ctx_t ctx)
{
    ...
    /* If put_all_nb op/s is/are being exelwted asynchronously, need to wait its
     * completion as well. */
...
static int mca_spml_ucx_create_aux_ctx(void)
{
    ...
    if (rand_dci_supp) {
        pthread_mutex_lock(&mca_spml_ucx.ctx_create_mutex);
        opal_setelw("UCX_DC_MLX5_TX_POLICY", "rand", 0, &elwiron);
    }

    rc = mca_spml_ucx_ctx_create_common(SHMEM_CTX_PRIVATE, &mca_spml_ucx.aux_ctx);

    if (rand_dci_supp) {
        opal_unsetelw("UCX_DC_MLX5_TX_POLICY", &elwiron);
        pthread_mutex_unlock(&mca_spml_ucx.ctx_create_mutex);
    }
