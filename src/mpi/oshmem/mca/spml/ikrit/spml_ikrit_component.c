...
static inline int set_mxm_tls(void)
{
     ...
     */
    opal_setelw("MXM_OSHMEM_DC_QP_LIMIT",      "2", 0, &elwiron);
    opal_setelw("MXM_OSHMEM_DC_RNDV_QP_LIMIT", "2", 0, &elwiron);
    opal_setelw("MXM_OSHMEM_DC_MSS",          "8196", 0, &elwiron);

    tls = getelw("MXM_OSHMEM_TLS");
    if (NULL != tls) {
    ...
    tls = getelw("MXM_TLS");
    if (NULL == tls) {
        opal_setelw("MXM_OSHMEM_TLS", mca_spml_ikrit.mxm_tls, 1, &elwiron);
        return check_mxm_tls("MXM_OSHMEM_TLS");
    }
    if (OSHMEM_SUCCESS == check_mxm_tls("MXM_TLS")) {
        opal_setelw("MXM_OSHMEM_TLS", tls, 1, &elwiron);
        return OSHMEM_SUCCESS;
    }
    return OSHMEM_ERROR;
}
...
static inline int set_mxm_hw_rdma_tls(void)
{
    if (!mca_spml_ikrit.hw_rdma_channel) {
        return check_mxm_hw_tls("MXM_OSHMEM_TLS", getelw("MXM_OSHMEM_TLS"));
    }
    opal_setelw("MXM_OSHMEM_HW_RDMA_RC_QP_LIMIT", "-1", 0, &elwiron);
    opal_setelw("MXM_OSHMEM_HW_RDMA_TLS", "rc", 0, &elwiron);
    SPML_VERBOSE(5, "Additional communication channel is enabled. Transports are: %s",
                 getelw("MXM_OSHMEM_HW_RDMA_TLS"));
