    ...
    for (i = 0; i < n; i++) {
        ...
        if (0 == memheap_oob.mkeys[tr_id].va_base) {
            ...
            if (0 < memheap_oob.mkeys[tr_id].len) {
                ...
                memheap_oob.mkeys[tr_id].u.key = MAP_SEGMENT_SHM_ILWALID;
            }
...
static int oshmem_mkey_recv_cb(void)
{
    ...
    while(r != (oob_comm_request_t *)opal_list_get_end(&memheap_oob.req_list)) {
        ...
        if (NULL == tmp_buf) {
            ...
            /*
             * send reply before posting the receive request again to limit the relwrsion size to
...
static int memheap_oob_get_mkeys(shmem_ctx_t ctx, int pe, uint32_t seg, sshmem_mkey_t *mkeys)
{
    ...
    if (OSHMEM_SUCCESS == MCA_SPML_CALL(oob_get_mkeys(ctx, pe, seg, mkeys))) {
        for (i = 0; i < memheap_map->num_transports; i++) {
            MEMHEAP_VERBOSE(5,
                            "MKEY CALLWLATED BY LOCAL SPML: pe: %d tr_id: %d %s",
                            pe,
                            i,
...
void mca_memheap_modex_recv_all(void)
{
    ...
    OPAL_TIMING_ELW_INIT(recv_all);

    if (!mca_memheap_base_key_exchange) {
        oshmem_shmem_barrier();
        return;
    }
    OPAL_TIMING_ELW_NEXT(recv_all, "barrier");
    nprocs = oshmem_num_procs();
    my_pe = oshmem_my_proc_id();
    OPAL_TIMING_ELW_NEXT(recv_all, "proc position");
    ...
    OPAL_TIMING_ELW_NEXT(recv_all, "alloc bufs");
    ...
    OPAL_TIMING_ELW_NEXT(recv_all, "serialize data");
    ...
    OPAL_TIMING_ELW_NEXT(recv_all, "allgather: transport cnt");
    ...
    OPAL_TIMING_ELW_NEXT(recv_all, "allgather: size info");

    /* callwlating offsets (displacements) for allgatherv */
    ...
    OPAL_TIMING_ELW_NEXT(recv_all, "alloc data buf");
    ...
    OPAL_TIMING_ELW_NEXT(recv_all, "Perform mkey exchange");
    ...
    OPAL_TIMING_ELW_NEXT(recv_all, "Unpack data");
...
exit_fatal:
    ...
    OPAL_TIMING_ELW_NEXT(recv_all, "Cleanup");
