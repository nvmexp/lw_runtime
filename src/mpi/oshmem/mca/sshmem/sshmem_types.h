 ...
 * @file
 *
 * shmem (shared memory backing facility) framework types, colwenience macros,
 * etc.
 */
 ...
 * invalid id value
 */
#define MAP_SEGMENT_SHM_ILWALID         (-1)
 ...
 */
#define MAP_SEGMENT_ILWALIDATE(ds_buf)                                       \
do {                                                                         \
    (ds_buf)->flags &= ~MAP_SEGMENT_FLAGS_VALID;                             \
} while (0)
 ...
 *   such keys use va_base = 0, len = 0 and key != MAP_SEGMENT_SHM_ILWALID
 *   va_base will be set once segment is attached.
 * 2. empty key: len = 0, key == MAP_SEGMENT_SHM_ILWALID
