...
mca_scoll_mpi_comm_query(oshmem_group_t *osh_group, int *priority)
{
    ...
    *priority = 0;
    ...
    OPAL_TIMING_ELW_INIT(comm_query);
    ...
    if (NULL == oshmem_group_all) {
        ...
        OPAL_TIMING_ELW_NEXT(comm_query, "ompi_mpi_comm_world");
    } else {
        err = ompi_comm_group(&(ompi_mpi_comm_world.comm), &world_group);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != err)) {
            return NULL;
        }
        OPAL_TIMING_ELW_NEXT(comm_query, "ompi_comm_group");
        ...
        OPAL_TIMING_ELW_NEXT(comm_query, "malloc");
        ...
        OPAL_TIMING_ELW_NEXT(comm_query, "build_ranks");
        ...
        OPAL_TIMING_ELW_NEXT(comm_query, "ompi_group_incl");
        ...
        OPAL_TIMING_ELW_NEXT(comm_query, "ompi_comm_create_group");
        ...
        OPAL_TIMING_ELW_NEXT(comm_query, "ompi_group_free");

        free(ranks);
        osh_group->ompi_comm = newcomm;
        OPAL_TIMING_ELW_NEXT(comm_query, "set_group_comm");
    }
