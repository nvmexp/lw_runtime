...
int mca_scoll_enable(void)
{
    ...
    OPAL_TIMING_ELW_INIT(mca_scoll_enable);
    ...
    OPAL_TIMING_ELW_NEXT(mca_scoll_enable, "memheap");
    ...
    OPAL_TIMING_ELW_NEXT(mca_scoll_enable, "group_all");

    if (OSHMEM_SUCCESS != (ret = mca_scoll_base_select(oshmem_group_self))) {
        return ret;
    }

    OPAL_TIMING_ELW_NEXT(mca_scoll_enable, "group_self");
