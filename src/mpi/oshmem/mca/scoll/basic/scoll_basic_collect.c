...
static int _algorithm_f_tournament(struct oshmem_group_t *group,
                                    ...
                                    size_t nlong,
                                    long *pSync);
static int _algorithm_f_relwrsive_doubling(struct oshmem_group_t *group,
                                            void *target,
                                            const void *source,
                                            size_t nlong,
                                            long *pSync);
                            ...
                            int alg)
{
    ...
    if ((rc == OSHMEM_SUCCESS) && oshmem_proc_group_is_member(group)) {
        ...
        if (nlong_type) {
            ...
            case SCOLL_ALG_COLLECT_RELWRSIVE_DOUBLING:
                {
                    rc = _algorithm_f_relwrsive_doubling(group,
                                                          target,
                                                          source,
                                                          nlong,
                                                          pSync);
                                         ...
                                         long *pSync)
{
    ...
    if (PE_root == group->my_pe) {
        ...
        for (i = 0; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) {
            ...
            pe_lwr = oshmem_proc_pe(group->proc_array[i]);

            if (pe_lwr == group->my_pe)
                continue;

            SCOLL_VERBOSE(14,
                          "[#%d] Gather data (%d bytes) from #%d",
                          group->my_pe, (int)nlong, pe_lwr);

            /* Get data from the current peer */
            rc = MCA_SPML_CALL(get(oshmem_ctx_default, (void *)source, nlong, (void*)((unsigned char*)target + i * nlong), pe_lwr));
        }
    }
                                            ...
                                            long *pSync)
{
    ...
    SCOLL_VERBOSE(12,
                  "[#%d] Collect algorithm: Relwrsive Doubling (identical size)",
                  group->my_pe);
    SCOLL_VERBOSE(15,
    ...
    if (my_id >= floor2_proc) {
        ...
        for (i = 0; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) {
            ...
            SCOLL_VERBOSE(14,
                          ...
                          group->my_pe, pe_lwr);
            rc = MCA_SPML_CALL(put(oshmem_ctx_default, (void*)((unsigned char*)target + data_index * nlong), nlong, (void *)source, pe_lwr));
        }
    ...
    if (PE_root == group->my_pe) {
        ...
        if (wait_pe_array) {
            ...
            while (wait_pe_count) {
                ...
                for (i = 1; (i < group->proc_count) && (rc == OSHMEM_SUCCESS);
                    ...
                    if (wait_pe_array[i] == 0) {
                        pe_lwr = oshmem_proc_pe(group->proc_array[i]);
                        value = 0;
                        rc = MCA_SPML_CALL(get(oshmem_ctx_default, (void*)pSync, sizeof(value), (void*)&value, pe_lwr));
                        if ((rc == OSHMEM_SUCCESS)
                                && (value != _SHMEM_SYNC_VALUE)) {
                            ...
                            SCOLL_VERBOSE(14,
                                          ...
                                          (int)value, pe_lwr, (int)wait_pe_count);
                        }
                    }
                }
            }
            ...
            for (i = 1; (i < group->proc_count) && (rc == OSHMEM_SUCCESS);
                ...
                /* Get data from the current peer */
                rc = MCA_SPML_CALL(get(oshmem_ctx_default, (void *)source, (size_t)wait_pe_array[i], (void*)((unsigned char*)target + offset), pe_lwr));

                SCOLL_VERBOSE(14,
                              "Got %d bytes of data from #%d (offset: %d)",
                              (int)wait_pe_array[i], pe_lwr, (int)offset);
