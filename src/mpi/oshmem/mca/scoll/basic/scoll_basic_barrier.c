...
int mca_scoll_basic_barrier(struct oshmem_group_t *group, long *pSync, int alg)
{
    ...
    if ((rc == OSHMEM_SUCCESS) && oshmem_proc_group_is_member(group)) {
        if (pSync) {
            ...
            case SCOLL_ALG_BARRIER_RELWRSIVE_DOUBLING:
                {
                    rc = _algorithm_relwrsive_doubling(group, pSync);
                    break;
                }
    ...
    if (PE_root == group->my_pe) {
        ...
        if (wait_pe_array) {
            ...
            for (i = 0; (i < group->proc_count) && (rc == OSHMEM_SUCCESS);
                ...
                if (pe_lwr != PE_root) {
                    rc = MCA_SPML_CALL(put(oshmem_ctx_default, (void*)pSync, sizeof(value), (void*)&value, pe_lwr));
                }
            }
...
/*
 Pairwise Exchange With Relwrsive Doubling.
                                          ...
                                          long *pSync)
{
    ...
    SCOLL_VERBOSE(12,
                  "[#%d] Barrier algorithm: Relwrsive Doubling",
                  group->my_pe);
    SCOLL_VERBOSE(15,
...
static int _algorithm_basic(struct oshmem_group_t *group, long *pSync)
{
    ...
    /* The root collects and broadcasts the messages. */
        ...
        for (i = 0; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) {
            pe_lwr = oshmem_proc_pe(group->proc_array[i]);
            if (pe_lwr != PE_root) {
                rc = MCA_SPML_CALL(recv(NULL, 0, pe_lwr)); 
            }
            if (OSHMEM_SUCCESS != rc) {
                return rc;
            }
        ...
        for (i = 0; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) {
            pe_lwr = oshmem_proc_pe(group->proc_array[i]);
            if (pe_lwr != PE_root) {
                rc = MCA_SPML_CALL(send(NULL, 0, pe_lwr, MCA_SPML_BASE_PUT_STANDARD));
            }
            if (OSHMEM_SUCCESS != rc) {
                return rc;
            }
