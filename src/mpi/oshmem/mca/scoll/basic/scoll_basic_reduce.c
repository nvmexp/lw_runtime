...
static int _algorithm_tournament(struct oshmem_group_t *group,
                                  ...
                                  long *pSync,
                                  void *pWrk);
static int _algorithm_relwrsive_doubling(struct oshmem_group_t *group,
                                          struct oshmem_op_t *op,
                                          void *target,
                                          const void *source,
                                          size_t nlong,
                           ...
                           int alg)
{
    ...
    if ((rc == OSHMEM_SUCCESS) && oshmem_proc_group_is_member(group)) {
        ...
        if (pSync) {
            ...
            case SCOLL_ALG_REDUCE_RELWRSIVE_DOUBLING:
                {
                    rc = _algorithm_relwrsive_doubling(group,
                                                        op,
                                                        target,
                                                        source,
                                                        nlong,
                                       ...
                                       void *pWrk)
{
    ...
    if (PE_root == group->my_pe) {
        int pe_lwr = 0;
        void *target_lwr = NULL;

        target_lwr = malloc(nlong);
        if (target_lwr) {
            ...
            for (i = 0; (i < group->proc_count) && (rc == OSHMEM_SUCCESS);
                ...
                memset(target_lwr, 0, nlong);

                /* Get data from the current peer */
                rc = MCA_SPML_CALL(get(oshmem_ctx_default, (void *)source, nlong, target_lwr, pe_lwr));

                /* Do reduction operation */
                if (rc == OSHMEM_SUCCESS) {
                    op->o_func.c_fn(target_lwr, target, nlong / op->dt_size);
                }
            }
                                  ...
                                  void *pWrk)
{
    ...
    target_lwr = malloc(nlong);
    if (target_lwr) {
        memcpy(target_lwr, (void *) source, nlong);
    } else {
    ...
    while (exit_flag && (rc == OSHMEM_SUCCESS)) {
        ...
        if (my_id < peer_id) {
            ...
            /* Do reduction operation */
            if (rc == OSHMEM_SUCCESS) {
                op->o_func.c_fn(target, target_lwr, nlong / op->dt_size);
            }
        } else {
       ...
       */
            ...
            rc = MCA_SPML_CALL(put(oshmem_ctx_default, target, nlong, target_lwr, peer_pe));
    ...
    if ((my_id == 0) && (rc == OSHMEM_SUCCESS)) {
        ...
        memcpy(target, target_lwr, nlong);
                                          ...
                                          void *pWrk)
{
    ...
    SCOLL_VERBOSE(12,
                  "[#%d] Reduce algorithm: Relwrsive Doubling",
                  group->my_pe);
    SCOLL_VERBOSE(15,
                           ...
                           void *pWrk)
{
    ...
    rank = group->my_pe;
    size = group->proc_count;
    dim = opal_lwbe_dim(group->proc_count);
    ...
    /* Loop over lwbe dimensions. High processes send to low ones in the
     * dimension. */
