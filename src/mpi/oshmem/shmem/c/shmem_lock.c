...
{
    oshmem_lock_counter_t *lwrrent_counter = lock_counter_head;
    oshmem_lock_prev_pe_container_t *lwrrent_pe_container =
    ...
    while (0 != lwrrent_counter) {
        oshmem_lock_counter_t *counter_to_free = lwrrent_counter;
        lwrrent_counter = lwrrent_counter->next;
        free(counter_to_free);
    }
    lock_counter_head = 0;

    while (0 != lwrrent_pe_container) {
        oshmem_lock_prev_pe_container_t *container_to_free =
                lwrrent_pe_container;
        lwrrent_pe_container = lwrrent_pe_container->next;
...
static oshmem_lock_counter_t *lock_find_counter(void *lock)
{
    ...
    while (0 != lwrrent_counter) {
        if (lwrrent_counter->lock == lock) {
            return lwrrent_counter;
        }
...
static int shmem_lock_decrement_counter(void *lock, int lock_size)
{
    int my_pe = shmem_my_pe();
    int server_pe = shmem_lock_get_server(lock);
    int lwrrent_lock_counter = -1;

    if (my_pe == server_pe) {
        oshmem_lock_counter_t *counter = lock_find_counter(lock);

        if (counter) {
            if (oshmem_shmem_lock_relwrsive) {
                counter->counter -= 1;
            } else {
                counter->counter = 0;
            }

            if ((lwrrent_lock_counter = counter->counter) <= 0) {
                shmem_lock_remove_counter(lock);
                lwrrent_lock_counter = 0;
            }
        }
    } else {
        ...
        if (counter > 0) {
            if (oshmem_shmem_lock_relwrsive) {
                lwrrent_lock_counter = counter - 1;
            } else {
                lwrrent_lock_counter = 0;
            }

            lock_pack_counter(lock, lock_size, &lwrrent_lock_counter, 1);
        }
    }

    return lwrrent_lock_counter;
}
...
{
    int lwrrent_lock_counter = shmem_lock_decrement_counter(lock, lock_size);

    if (0 == lwrrent_lock_counter) {
        int next_informed = 0;
        int pe_next = 0;
