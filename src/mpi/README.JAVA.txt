...
For colwenience, the "mpijavac" wrapper compiler has been provided for
...
For colwenience, mpirun has been updated to detect the "java" command
and ensure that the required MPI libraries and class paths are defined
to support exelwtion. You therefore do NOT need to specify the Java
...
either on the command line or via the CLASSPATH elwironmental
variable. Note that the local directory will be added to the class
path if nothing is specified.

As always, the "java" exelwtable, all required libraries, and your
...
under a colwenience class MPI. Examples are the predefined communicator
...
be ilwoked by all MPI Java applications. The following example illustrates
these concepts:
...
classes available in the Java SDK such as ByteBuffer. For colwenience
...
cases arrays will be a better choice. You can easily colwert a
