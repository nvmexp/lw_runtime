...
options_data_init(struct options_data_t *data)
{
    ...
    data->version = NULL;
    data->compiler_elw = NULL;
    data->compiler_flags_elw = NULL;
...
options_data_free(struct options_data_t *data)
{
    ...
    if (NULL != data->compiler_elw) free(data->compiler_elw);
    if (NULL != data->compiler_flags_elw) free(data->compiler_flags_elw);
...
data_callback(const char *key, const char *value)
{
    ...
    } else if (0 == strcmp(key, "compiler_elw")) {
        if (NULL != value) options_data[parse_options_idx].compiler_elw = strdup(value);
    } else if (0 == strcmp(key, "compiler_flags_elw")) {
        if (NULL != value) options_data[parse_options_idx].compiler_flags_elw = strdup(value);
...
static void
load_elw_data(const char *project, const char *flag, char **data)
{
    char *elwname;
    char *elwvalue;
    ...
    asprintf(&elwname, "%s_MPI%s", project, flag);
    if (NULL == (elwvalue = getelw(elwname))) {
        free(elwname);
        asprintf(&elwname, "%s_%s", project, flag);
        if (NULL == (elwvalue = getelw(elwname))) {
            free(elwname);
            return;
        }
    ...
    *data = strdup(elwvalue);
}


static void
load_elw_data_argv(const char *project, const char *flag, char ***data)
{
    char *elwname;
    char *elwvalue;
...
main(int argc, char *argv[])
{
    ...
    if( 0 != strlen(EXEEXT) ) {
        ...
        /* Only if there was a match of .exe, erase the last oclwrence of .exe */
        if ( NULL != old_match ) {
            *old_match = '\0';
        }
    }
    ...
    /* compiler */
    load_elw_data(options_data[user_data_idx].project_short, options_data[user_data_idx].compiler_elw, &options_data[user_data_idx].compiler);

    /* preprocessor flags */
    load_elw_data_argv(options_data[user_data_idx].project_short, "CPPFLAGS", &options_data[user_data_idx].preproc_flags);

    /* compiler flags */
    load_elw_data_argv(options_data[user_data_idx].project_short, options_data[user_data_idx].compiler_flags_elw,
                       &options_data[user_data_idx].comp_flags);

    /* linker flags */
    load_elw_data_argv(options_data[user_data_idx].project_short, "LDFLAGS", &options_data[user_data_idx].link_flags);

    /* libs */
    load_elw_data_argv(options_data[user_data_idx].project_short, "LIBS", &options_data[user_data_idx].libs);
    ...
    for (i = 0 ; i < user_argc ; ++i) {
        if (0 == strncmp(user_argv[i], "-showme", strlen("-showme")) ||
            ...
            /* check for specific things we want to see.  First three
               still ilwoke all the building routines.  Last set want
            ...
            /* This is an intentionally undolwmmented wrapper compiler
               ...
               this flag for test MPI applications that also ilwoke
    ...
    */
    ...
    if (flags & COMP_DRY_RUN) {
        ...
        tmp = opal_path_findv(exec_argv[0], 0, elwiron, NULL);
        if (NULL == tmp) {
