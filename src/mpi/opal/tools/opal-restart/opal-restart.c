...
static int post_elw_vars(int prev_pid, opal_crs_base_snapshot_t *snapshot);
...
main(int argc, char *argv[])
{
    ...
    if( OPAL_SUCCESS != (ret = check_file() )) {
        opal_show_help("help-opal-restart.txt", "ilwalid_filename", true,
    ...
    if(NULL == expected_crs_comp) {
        ...
        if( NULL == (metadata = fopen(full_metadata_path, "r")) ) {
            opal_show_help("help-opal-restart.txt", "ilwalid_metadata", true,
                           opal_restart_globals.snapshot_metadata,
                           full_metadata_path);
            exit_status = OPAL_ERROR;
            goto cleanup;
    ...
    /* Since some checkpoint/restart systems don't pass along elw vars to the
     ...
     */
    if(OPAL_SUCCESS != (ret = post_elw_vars(prev_pid, snapshot) ) ) {
        exit_status = ret;
        goto cleanup;
    }
...
static int initialize(int argc, char *argv[])
{
     ...
     */
    (void) mca_base_var_elw_name("crs_base_do_not_select", &tmp_elw_var);
    opal_setelw(tmp_elw_var,
    ...
    if( NULL != opal_restart_globals.snapshot_compress ) {
        (void) mca_base_var_elw_name("compress", &tmp_elw_var);
        opal_setelw(tmp_elw_var,
    ...
    if( NULL != opal_restart_globals.snapshot_compress ) {
         ...
         */
        (void) mca_base_var_elw_name("compress", &tmp_elw_var);
        opal_unsetelw(tmp_elw_var, &elwiron);
        free(tmp_elw_var);
        tmp_elw_var = NULL;
...
static int finalize(void)
{
     ...
     * Lwrrently BLCR is calling opal_even_fini() in the restart
...
static int post_elw_vars(int prev_pid, opal_crs_base_snapshot_t *snapshot)
{
     ...
     * application process.
     */
    asprintf(&proc_file, "%s/%s-%d", opal_tmp_directory(), OPAL_CR_BASE_ELW_NAME, prev_pid);
    asprintf(&command, "elw | grep OMPI_ > %s", proc_file);

    opal_output_verbose(5, opal_restart_globals.output,
                        "post_elw_vars: Execute: <%s>", command);
