...
static int parse_args(int argc, char *argv[]) {
    ...
    char **app_elw = NULL, **global_elw = NULL;
    char * tmp_elw_var = NULL;
    char *argv0 = NULL;
     ...
     */
    mca_base_cmd_line_process_args(&cmd_line, &app_elw, &global_elw);

    len = opal_argv_count(app_elw);
    for(i = 0; i < len; ++i) {
        putelw(app_elw[i]);
    }

    len = opal_argv_count(global_elw);
    for(i = 0; i < len; ++i) {
        putelw(global_elw[i]);
    }
    ...
    if ( 0 >= opal_checkpoint_globals.pid ) {
        opal_show_help("help-opal-checkpoint.txt", "ilwalid_pid", true,
                       opal_checkpoint_globals.pid);
        return OPAL_ERROR;
    }
