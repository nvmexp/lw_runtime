...
int opal_dss_compare_byte_object(opal_byte_object_t *value1, opal_byte_object_t *value2, opal_data_type_t type)
{
     ...
     * callwlation to determine "biggest"
     */
    checksum = 0;
...
int opal_dss_compare_value(opal_value_t *value1, opal_value_t *value2, opal_data_type_t type)
{
    ...
    case OPAL_ELWAR:
        return opal_dss_compare_elwar(&value1->data.elwar, &value2->data.elwar, type);
    default:
...
int opal_dss_compare_elwar(opal_elwar_t *value1, opal_elwar_t *value2, opal_data_type_t type)
{
    ...
    if (NULL != value1->elwar) {
        if (NULL == value2->elwar) {
            return OPAL_VALUE1_GREATER;
        }
        rc = strcmp(value1->elwar, value2->elwar);
        if (rc < 0) {
            return OPAL_VALUE2_GREATER;
        } else if (0 < rc) {
            return OPAL_VALUE1_GREATER;
        }
    } else if (NULL != value2->elwar) {
        /* we know value1->elwar had to be NULL */
        return OPAL_VALUE2_GREATER;
    }
