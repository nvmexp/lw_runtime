                        ...
                        opal_data_type_t type)
{
    ...
    *dest = OBJ_NEW(opal_value_t);
    ...
    case OPAL_NAME:
        ...
        break;
    case OPAL_ELWAR:
        OBJ_CONSTRUCT(&p->data.elwar, opal_elwar_t);
        if (NULL != src->data.elwar.elwar) {
            p->data.elwar.elwar = strdup(src->data.elwar.elwar);
        }
        if (NULL != src->data.elwar.value) {
            p->data.elwar.value = strdup(src->data.elwar.value);
        }
        p->data.elwar.separator = src->data.elwar.separator;
        break;
    default:
