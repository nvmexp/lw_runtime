...
static void opal_elwar_construct(opal_elwar_t *obj)
{
    obj->elwar = NULL;
    obj->value = NULL;
    obj->separator = '\0';
}
static void opal_elwar_destruct(opal_elwar_t *obj)
{
    if (NULL != obj->elwar) {
        free(obj->elwar);
    }
...
OBJ_CLASS_INSTANCE(opal_elwar_t,
                   opal_list_item_t,
                   opal_elwar_construct,
                   opal_elwar_destruct);
...
int opal_dss_register_vars (void)
{
    ...
    elwiro_val = getelw("OPAL_dss_debug");
    if (NULL != elwiro_val) {  /* debug requested */
        opal_dss_verbose = 0;
    }
...
int opal_dss_open(void)
{
    ...
    tmp = OPAL_ELWAR;
    if (OPAL_SUCCESS != (rc = opal_dss.register_type(opal_dss_pack_elwar,
                                          opal_dss_unpack_elwar,
                                          (opal_dss_copy_fn_t)opal_dss_copy_elwar,
                                          (opal_dss_compare_fn_t)opal_dss_compare_elwar,
                                          (opal_dss_print_fn_t)opal_dss_print_elwar,
                                          OPAL_DSS_UNSTRUCTURED,
                                          "OPAL_ELWAR", &tmp))) {
        return rc;
    }
