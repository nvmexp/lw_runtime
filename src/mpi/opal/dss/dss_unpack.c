                          ...
                          int32_t *num_vals, opal_data_type_t type)
{
   ...
   OPAL_OUTPUT( ( opal_dss_verbose, "opal_dss_unpack_float * %d\n", (int)*num_vals ) );
    ...
    for (i = 0; i < (*num_vals); ++i) {
        n=1;
        if (OPAL_SUCCESS != (ret = opal_dss_unpack_string(buffer, &colwert, &n, OPAL_STRING))) {
            return ret;
        }
        if (NULL == colwert) {
            return OPAL_ERR_UNPACK_FAILURE;
        }
        tmp = strtof(colwert, NULL);
        memcpy(&desttmp[i], &tmp, sizeof(tmp));
        free(colwert);
        colwert = NULL;
    }
                          ...
                          int32_t *num_vals, opal_data_type_t type)
{
    ...
    ptr = (opal_elwar_t *) dest;
    n = *num_vals;

    for (i = 0; i < n; ++i) {
        m=1;
        if (OPAL_SUCCESS != (ret = opal_dss_unpack_string(buffer, &ptr[i].elwar, &m, OPAL_STRING))) {
            OPAL_ERROR_LOG(ret);
            return ret;
        }
        m=1;
