...
int opal_dss_print_value(char **output, char *prefix, opal_value_t *src, opal_data_type_t type)
{
    ...
    case OPAL_PTR:
        ...
        break;
    case OPAL_ELWAR:
        asprintf(output, "%sOPAL_VALUE: Data type: OPAL_ELWAR\tKey: %s\tName: %s\tValue: %s\tSeparator: %c",
                 prefx, src->key,
                 (NULL == src->data.elwar.elwar) ? "NULL" : src->data.elwar.elwar,
                 (NULL == src->data.elwar.value) ? "NULL" : src->data.elwar.value,
                 ('\0' == src->data.elwar.separator) ? ' ' : src->data.elwar.separator);
        break;
    default:
...
int opal_dss_print_elwar(char **output, char *prefix,
                         opal_elwar_t *src, opal_data_type_t type)
{
    char *prefx = " ";
    ...
    /* if src is NULL, just print data type and return */
    if (NULL == src) {
        asprintf(output, "%sData type: OPAL_ELWAR\tValue: NULL pointer", prefx);
        return OPAL_SUCCESS;
    }

    asprintf(output, "%sOPAL_VALUE: Data type: OPAL_ELWAR\tName: %s\tValue: %s\tSeparator: %c",
             prefx, (NULL == src->elwar) ? "NULL" : src->elwar,
