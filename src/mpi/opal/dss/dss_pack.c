                        ...
                        int32_t num_vals, opal_data_type_t type)
{
    ...
    for (i = 0; i < num_vals; ++i) {
        asprintf(&colwert, "%f", ssrc[i]);
        if (OPAL_SUCCESS != (ret = opal_dss_pack_string(buffer, &colwert, 1, OPAL_STRING))) {
            free(colwert);
            return ret;
        }
        free(colwert);
                        ...
                        int32_t num_vals, opal_data_type_t type)
{
    ...
    for (i = 0; i < num_vals; ++i) {
        ...
        case OPAL_STATUS:
            ...
            break;
        case OPAL_ELWAR:
            if (OPAL_SUCCESS != (ret = opal_dss_pack_buffer(buffer, &ptr[i]->data.elwar, 1, OPAL_ELWAR))) {
                return ret;
            }
            break;
        default:
                        ...
                        int32_t num_vals, opal_data_type_t type)
{
    int ret;
    int32_t n;
    opal_elwar_t *ptr = (opal_elwar_t*)src;

    for (n=0; n < num_vals; n++) {
        if (OPAL_SUCCESS != (ret = opal_dss_pack_string(buffer, &ptr[n].elwar, 1, OPAL_STRING))) {
            return ret;
        }
