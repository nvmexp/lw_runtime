...
#define OPAL_CR_BASE_ELW_NAME ("opal_cr_restart-elw")
...
enum opal_cr_ckpt_cmd_state_t {
    ...
    OPAL_CHECKPOINT_CMD_IN_PROGRESS, /* Checkpoint is lwrrently running */
    OPAL_CHECKPOINT_CMD_NULL,        /* Checkpoint cannot be started because it is not supported */
    OPAL_CHECKPOINT_CMD_ERROR,       /* An error oclwrred such that the checkpoint cannot be completed */
    ...
    OPAL_CR_STATUS_RUNNING,    /* Checkpoint is lwrrently running */
...
typedef enum opal_cr_ckpt_cmd_state_t opal_cr_ckpt_cmd_state_t;
     ...
     */
    OPAL_DECLSPEC int opal_cr_debug_set_lwrrent_ckpt_thread_self(void);
    OPAL_DECLSPEC int opal_cr_debug_clear_lwrrent_ckpt_thread(void);

    /*
     * This MPI Debugger function needs to be accessed here and have a specific
     * name. Thus we are breaking the traditional naming colwentions to provide this functionality.
     */
     ...
     */
    OPAL_DECLSPEC int opal_cr_refresh_elwiron(int prev_pid);
    ...
    OPAL_DECLSPEC extern bool opal_cr_lwrrently_stalled;
