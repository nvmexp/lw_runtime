                   ...
                   opal_cmd_line_t *opal_info_cmd_line)
{
    ...
    mca_base_cmd_line_process_args(opal_info_cmd_line, &app_elw, &global_elw);
...
static void opal_info_show_mca_group_params(const mca_base_var_group_t *group, mca_base_var_info_lvl_t max_level, bool want_internal)
{
    ...
    const mca_base_var_group_t *lwrr_group = NULL;
    ...
    for (i = 0 ; i < count ; ++i) {
        ...
        if (opal_info_pretty && lwrr_group != group) {
...
static char *escape_quotes(const char *value)
{
    ...
    // clear to read; it's not partilwlarly efficient (performance is
