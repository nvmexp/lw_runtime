...
bool opal_built_with_lwda_support = OPAL_INT_TO_BOOL(OPAL_LWDA_SUPPORT);
bool opal_lwda_support = false;
bool opal_warn_on_missing_liblwda = true;
...
int opal_register_params(void)
{
    ...
    */
    ...
    ret = mca_base_var_register("opal", "opal", NULL, "built_with_lwda_support",
                                "Whether LWCA GPU buffer support is built into library or not",
                                MCA_BASE_VAR_TYPE_BOOL, NULL, 0, MCA_BASE_VAR_FLAG_DEFAULT_ONLY,
                                OPAL_INFO_LVL_4, MCA_BASE_VAR_SCOPE_CONSTANT,
                                &opal_built_with_lwda_support);
    if (0 > ret) {
        return ret;
    }

    /* Current default is to enable LWCA support if it is built into library */
    opal_lwda_support = opal_built_with_lwda_support;
    ret = mca_base_var_register ("opal", "opal", NULL, "lwda_support",
                                 "Whether LWCA GPU buffer support is enabled or not",
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, MCA_BASE_VAR_FLAG_SETTABLE,
                                 OPAL_INFO_LVL_3, MCA_BASE_VAR_SCOPE_ALL_EQ,
                                 &opal_lwda_support);
    if (0 > ret) {
    ...
    opal_warn_on_missing_liblwda = true;
    ret = mca_base_var_register ("opal", "opal", NULL, "warn_on_missing_liblwda",
                                 "Whether to print a message when LWCA support is enabled but liblwda is not found",
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, MCA_BASE_VAR_FLAG_SETTABLE,
                                 OPAL_INFO_LVL_3, MCA_BASE_VAR_SCOPE_ALL_EQ,
                                 &opal_warn_on_missing_liblwda);
    if (0 > ret) {
        return ret;
    }
    ...
    ret = mca_base_var_register("opal", "opal", NULL, "abort_delay",
                                "If nonzero, print out an identifying message when abort operation is ilwoked (hostname, PID of the process that called abort) and delay for that many seconds before exiting (a negative delay value means to never abort).  This allows attaching of a debugger before quitting the job.",
    ...
    ret = mca_base_var_register("opal", "opal", NULL, "abort_print_stack",
                                 "If nonzero, print out a stack trace when abort is ilwoked",
    ...
    /* register the elwar-forwarding params */
    (void)mca_base_var_register ("opal", "mca", "base", "elw_list",
                                 "Set SHELL elw variables",
                                 MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0, OPAL_INFO_LVL_3,
                                 MCA_BASE_VAR_SCOPE_READONLY, &mca_base_elw_list);

    mca_base_elw_list_sep = MCA_BASE_ELW_LIST_SEP_DEFAULT;
    (void)mca_base_var_register ("opal", "mca", "base", "elw_list_delimiter",
                                 "Set SHELL elw variables delimiter. Default: semicolon ';'",
                                 MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0, OPAL_INFO_LVL_3,
                                 MCA_BASE_VAR_SCOPE_READONLY, &mca_base_elw_list_sep);

    /* Set OMPI_MCA_mca_base_elw_list variable, it might not be set before
     * if mca variable was taken from amca conf file. Need to set it
     * here because mca_base_var_process_elw_list is called from schizo_ompi.c
     * only when this elw variable was set.
     */
    if (NULL != mca_base_elw_list) {
        char *name = NULL;
        (void) mca_base_var_elw_name ("mca_base_elw_list", &name);
        if (NULL != name) {
            opal_setelw(name, mca_base_elw_list, false, &elwiron);
            free(name);
        }
    }

    /* Register internal MCA variable mca_base_elw_list_internal. It can be set only during
     * parsing of amca conf file and contains SHELL elw variables specified via -x there.
     * Its format is the same as for mca_base_elw_list.
     */
    (void)mca_base_var_register ("opal", "mca", "base", "elw_list_internal",
            "Store SHELL elw variables from amca conf file",
            MCA_BASE_VAR_TYPE_STRING, NULL, 0, MCA_BASE_VAR_FLAG_INTERNAL, OPAL_INFO_LVL_3,
            MCA_BASE_VAR_SCOPE_READONLY, &mca_base_elw_list_internal);
