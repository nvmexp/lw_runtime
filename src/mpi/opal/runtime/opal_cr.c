...
bool opal_cr_lwrrently_stalled = false;
 ...
 ******************/
static int extract_elw_vars(int prev_pid, char * file_name);

static void opal_cr_sigpipe_debug_signal_handler (int signo);

static opal_cr_user_inc_callback_fn_t lwr_user_coord_callback[OPAL_CR_INC_MAX] = {NULL};
static opal_cr_coord_callback_fn_t  lwr_coord_callback = NULL;
static opal_cr_notify_callback_fn_t lwr_notify_callback = NULL;
...
int opal_cr_init(void )
{
    ...
    if( !opal_cr_is_tool && opal_cr_thread_use_if_avail) {
         ...
         *      actual locks for threads. However lwrrently the
...
void opal_cr_test_if_checkpoint_ready(void)
{
    ...
    /*
     * If we are lwrrently checkpointing:
     *  - If a request is pending then cancel it
     *  - o.w., skip it.
     */
    if(OPAL_CR_STATUS_RUNNING == opal_cr_checkpointing_state ) {
        if( OPAL_SUCCESS != (ret = lwr_notify_callback(OPAL_CHECKPOINT_CMD_IN_PROGRESS) ) ) {
            opal_output(opal_cr_output,
    ...
    if (NULL == opal_crs.crs_checkpoint ) {
         if( OPAL_SUCCESS != (ret = lwr_notify_callback(OPAL_CHECKPOINT_CMD_NULL) ) ) {
             opal_output(opal_cr_output,
 ...
 STAGE_1:
    if( OPAL_SUCCESS != (ret = lwr_notify_callback(OPAL_CHECKPOINT_CMD_START) ) ) {
        opal_output(opal_cr_output,
...
int opal_cr_inc_core_prep(void)
{
     ...
     */
    if(OPAL_SUCCESS != (ret = lwr_coord_callback(OPAL_CRS_CHECKPOINT)) ) {
        if ( OPAL_EXISTS != ret ) {
            opal_output(opal_cr_output,
                        "opal_cr: inc_core: Error: lwr_coord_callback(%d) failed! %d\n",
                          ...
                          int *state)
{
    ...
    if(*state == OPAL_CRS_RESTART) {
        opal_cr_refresh_elwiron(core_prev_pid);
...
int opal_cr_inc_core_recover(int state)
{
     ...
     * Use the registered coordination routine
     */
    if(OPAL_SUCCESS != (ret = lwr_coord_callback(state)) ) {
        if ( OPAL_EXISTS != ret ) {
            opal_output(opal_cr_output,
                        "opal_cr: inc_core: Error: lwr_coord_callback(%d) failed! %d\n",
                        state, ret);
...
#if OPAL_ENABLE_CRDEBUG == 1
    opal_cr_debug_clear_lwrrent_ckpt_thread();
#endif
                                ...
                                opal_cr_notify_callback_fn_t *prev_func)
{
     ...
     */
    if( NULL != lwr_notify_callback) {
        *prev_func = lwr_notify_callback;
    }
    else {
        *prev_func = NULL;
    }
                                       ...
                                       opal_cr_user_inc_callback_fn_t  *prev_function)
{
    ...
    if( NULL != lwr_user_coord_callback[event] ) {
        *prev_function = lwr_user_coord_callback[event];
    } else {
        *prev_function = NULL;
    }

    lwr_user_coord_callback[event] = function;
                              ...
                              opal_cr_user_inc_callback_state_t state)
{
    ...
    return ((lwr_user_coord_callback[event])(event, state));
}
                               ...
                               opal_cr_coord_callback_fn_t *prev_func)
{
     ...
     * Preserve the previous callback
     */
    if( NULL != lwr_coord_callback) {
        *prev_func = lwr_coord_callback;
    }
    else {
        *prev_func = NULL;
    }
     ...
     * Update the callbacks
     */
    lwr_coord_callback     = new_func;

    return OPAL_SUCCESS;
}

int opal_cr_refresh_elwiron(int prev_pid) {
    char *file_name;
#if OPAL_ENABLE_CRDEBUG == 1
    char *tmp;
#endif
     ...
     *  1) We have already exelwted this function, and
     *  2) The file has been deleted on the previous round.
     */
    asprintf(&file_name, "%s/%s-%d", opal_tmp_directory(), OPAL_CR_BASE_ELW_NAME, prev_pid);
    if (NULL == file_name) {
...
#if OPAL_ENABLE_CRDEBUG == 1
    mca_base_var_elw_name ("opal_cr_enable_crdebug", &tmp);
    opal_unsetelw(tmp, &elwiron);
    free (tmp);
#endif

    extract_elw_vars(prev_pid, file_name);
...
static int extract_elw_vars(int prev_pid, char * file_name)
{
    ...
    if( 0 >= prev_pid ) {
        opal_output(opal_cr_output,
                    "opal_cr: extract_elw_vars: Invalid PID (%d)\n",
                    prev_pid);
        exit_status = OPAL_ERROR;
        goto cleanup;
    }

    if (NULL == (elw_data = fopen(file_name, "r")) ) {
        exit_status = OPAL_ERROR;
        goto cleanup;
    }
    ...
    while(!feof(elw_data) ) {
        ...
        if( NULL == fgets(tmp_str, OPAL_PATH_MAX, elw_data) ) {
            exit_status = OPAL_ERROR;
            goto cleanup;
        }
        len = strlen(tmp_str);
        if(tmp_str[len - 1] == '\n') {
            ...
            opal_output(opal_cr_output,
                        "opal_cr: extract_elw_vars: Error: Parameter too long (%s)\n",
                        tmp_str);
            continue;
        }
        ...
        opal_setelw(t_set[0], t_set[1], true, &elwiron);
...
static void* opal_cr_thread_fn(opal_object_t *obj)
{
    ...
    while( opal_cr_thread_is_active && !opal_cr_thread_is_done) {
        ...
        while ( !opal_cr_thread_in_library ) {
            ...
            /* Sanity check */
            if( OPAL_UNLIKELY(opal_cr_lwrrently_stalled) ) {
                OPAL_CR_TEST_CHECKPOINT_READY();
            }
        }
...
#if OPAL_ENABLE_CRDEBUG == 1
int opal_cr_debug_set_lwrrent_ckpt_thread_self(void)
{
    int t;
...
int opal_cr_debug_clear_lwrrent_ckpt_thread(void)
{
