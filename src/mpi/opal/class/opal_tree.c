...
static int count_descendants(opal_tree_item_t* item)
{
    ...
    while (item) {
        lwrrent_count += count_descendants(opal_tree_get_first_child(item));
        lwrrent_count++; /* count ourselves */
        item = opal_tree_get_next_sibling(item);
    }
    return(lwrrent_count);
}
 ...
 * item passed in on down.  We exit out of colwerting tree items once we've
                                     ...
                                     int depth)
{
    ...
    int level = 0; /* 0 - one up 1 - lwrr, 2 - one down */

    if (!*lwrr_delim) {
        if (OPAL_SUCCESS !=
            (rc = opal_dss.unpack(data, lwrr_delim, &idx, OPAL_STRING))) {
            return(rc);
        }
    }
    while(*lwrr_delim[0] != end_stream[0]) {
        if (*lwrr_delim[0] == start_lvl[0]) {
            level++;
        } else {
            level--;
        }
        ...
        case 2:
            ...
            deserialize_add_tree_item(data, new_item, deserialize, lwrr_delim,
                                      depth+1);
            level--;
            break;
        }
                                      ...
                                      opal_tree_item_t *left, opal_tree_item_t *right)
{
    ...
    /* Relwrsively compare all children */
...
{
    opal_tree_item_t *lwrr_item = item, *result = NULL;
    ...
    if (!opal_tree_is_empty(item->opal_tree_container)) {
        ...
        if (!result) {
            ...
            if (NULL != (lwrr_item = opal_tree_get_next_sibling(lwrr_item))) {
                result = find_in_descendants(lwrr_item, key);
            }
        }

        /* check my ancestors (uncles) for match */
        lwrr_item = item;
        while (!result && lwrr_item && lwrr_item->opal_tree_num_ancestors > 0){
            lwrr_item = opal_tree_get_next_sibling(item->opal_tree_parent);
            while (NULL == lwrr_item &&
