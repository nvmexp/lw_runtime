...
static void opal_free_list_construct(opal_free_list_t* fl)
{
    ...
    fl->fl_rcache_reg_flags = MCA_RCACHE_FLAGS_CACHE_BYPASS |
        MCA_RCACHE_FLAGS_LWDA_REGISTER_MEM;
    fl->ctx = NULL;
...
int opal_free_list_grow_st (opal_free_list_t* flist, size_t num_elements, opal_free_list_item_t **item_out)
{
    ...
    /* NTH: callwlate allocation alignment first as it might change the number of elements */
    if (0 != flist->fl_payload_buffer_size) {
        ...
        if (MCA_RCACHE_FLAGS_LWDA_REGISTER_MEM & flist->fl_rcache_reg_flags) {
            size_t pagesize = opal_getpagesize ();
            /* LWCA cannot handle registering overlapping regions, so make
    ...
    /* callwlate head allocation size */
