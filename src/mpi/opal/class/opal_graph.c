...
static int compare_vertex_distance(const void *item1, const void *item2)
{
    ...
    /* colwert the void pointers to vertex distance pointers. */
...
{
    int graph_order;
    vertex_distance_from_t *Q, *q_start, *lwrrent_vertex;
    opal_adjacency_list_t *adj_list;
    ...
    while (number_of_items_in_q > 0) {
        ...
        for (i = 0; i < number_of_items_in_q; i++) {
            weight = opal_graph_adjacent(graph, lwrrent_vertex->vertex, q_start[i].vertex);
            /**
             ...
             */
            if (lwrrent_vertex->weight + weight < q_start[i].weight) {
                q_start[i].weight = weight + lwrrent_vertex->weight;
            }
        }
