 ...
 * threads may [erroneously] modify an opal_list_item_t conlwrrently.
 *
 ...
 * the caller; no calls to OBJ_RETAIN() or OBJ_RELEASE() are ilwoked.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
                                        ...
                                        opal_list_item_t *item)
{
  ...
  OPAL_DECLSPEC bool opal_list_insert(opal_list_t *list, opal_list_item_t *item,
     ...
     * opal_list_sort() does before ilwoking qsort, so go read that
     * documentation first.
     *
