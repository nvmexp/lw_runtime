 ...
 * the eviction callback is ilwoked (in microseconds)
 ...
 *
 * @return OPAL_SUCCESS if the oclwpant is successfully checked in,
                                     ...
                                     int *room_num)
{
    ...
    if (OPAL_UNLIKELY(hotel->last_unoclwpied_room < 0)) {
...
static inline void opal_hotel_checkout(opal_hotel_t *hotel, int room_num)
{
    ...
    room = &(hotel->rooms[room_num]);
    if (OPAL_LIKELY(NULL != room->oclwpant)) {
        /* Do not change this logic without also changing the same
           logic in opal_hotel_checkout_and_return_oclwpant() and
 ...
 */
static inline void opal_hotel_checkout_and_return_oclwpant(opal_hotel_t *hotel, int room_num, void **oclwpant)
{
    opal_hotel_room_t *room;
    ...
    room = &(hotel->rooms[room_num]);
    if (OPAL_LIKELY(NULL != room->oclwpant)) {
        opal_output (10, "checking out oclwpant %p from room num %d", room->oclwpant, room_num);
        /* Do not change this logic without also changing the same
 ...
 */
static inline void opal_hotel_knock(opal_hotel_t *hotel, int room_num, void **oclwpant)
{
    opal_hotel_room_t *room;
    ...
    *oclwpant = NULL;
    ...
    room = &(hotel->rooms[room_num]);
    if (OPAL_LIKELY(NULL != room->oclwpant)) {
        opal_output (10, "oclwpant %p in room num %d responded to knock", room->oclwpant, room_num);
        *oclwpant = room->oclwpant;
    }
}
