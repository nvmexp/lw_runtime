 ...
 *
 * - how MPI_INIT or MPI_INIT_THREAD was ilwoked,
 ...
 * functions) ilwoke opal_set_using_threads() to influence the value of
 * this function, depending on their situation. Some examples:
 *
 * - if MPI_INIT is ilwoked, and the ompi components in use are
 * single-threaded, this value will be false.
 *
 * - if MPI_INIT_THREAD is ilwoked with MPI_THREAD_MULTIPLE, we have
 * thread support, and the final thread level is determined to be
 * MPI_THREAD_MULTIPLE, this value will be true.
 *
 * - if the process is a single-threaded OMPI exelwtable (e.g., mpicc),
 ...
 * support, the return value of future ilwocations of
