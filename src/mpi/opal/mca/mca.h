 ...
 * elwisioned) changing the MCA base component struct itself.  Oops.
 *
 ...
 *   that the struct oclwpies; if we make future changes in the struct
 ...
 *   ulwersioned versions of struct names (both in the MCA base and in
 *   individual framework bases) are intended for components who want
 *   to be forward source-compatible.  That is, the ulwersioned struct
 ...
 *   let us know!***
 *
 * - We are lwrrently only aware of one external developer making Open
 ...
 * - Per the dislwssion above, we decided to have MCA v2.0 no longer
 ...
 *
 * All MCA components can have an "open" function that is ilwoked once
 ...
 *
 * @retval any_other_value Some error oclwrred, but is likely to be
 * ignored.
 *
 * This function is ilwoked on a component after all of its modules
 ...
 * will be ilwoked to register all MCA parameters associated with the
 * component.  This function is ilwoked *before* the component "open"
 * function is ilwoked.
 *
 ...
 * function ilwoked on a component; component authors should take care
 ...
 * exelwtion and print out all existing registered parameters from the
...
enum {
     ...
     * so as to limit its use. See dislwssion from the Open MPI
...
/**
 * Macro for framework author colwenience.
 *
