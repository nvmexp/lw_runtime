...
int mca_rcache_udreg_module_init (mca_rcache_udreg_module_t *rcache)
{
    ...
    rcache->super.rcache_ilwalidate_range = NULL;
...
static uint32_t mca_rcache_udreg_dereg_func (void *device_data, void *dreg_context)
{
    ...
    /* might be worth printing out a warning if an error oclwrs here */
        ...
        do {
            ...
            if (old_reg) {
                ...
                if (!old_reg->ref_count) {
                    ...
                    old_reg->flags |= MCA_RCACHE_FLAGS_CACHE_BYPASS | MCA_RCACHE_FLAGS_ILWALID;
                }
            }
