 ...
 * cached by the lwda_common code.
 */
...
{
    mca_common_lwda_construct_event_and_handle(&item->event,
                                               (void *)&item->evtHandle);
}
...
{
    mca_common_lwda_destruct_event(item->event);
}
...
void mca_rcache_gpusm_module_init(mca_rcache_gpusm_module_t* rcache)
{
    ...
    /* Start with 0 entries in the free list since LWCA may not have
     * been initialized when this free list is created and there is
     * some LWCA specific activities that need to be done. */
                             ...
                             mca_rcache_base_registration_t **reg)
{
    ...
    *reg = NULL;
    ...
    rc = lwda_getmemhandle (base, size, gpusm_reg, NULL);
                               ...
                               mca_rcache_base_registration_t *reg)
{
    ...
    rc = lwda_ungetmemhandle (NULL, reg);
