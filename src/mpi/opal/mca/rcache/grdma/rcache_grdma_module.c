...
static int mca_rcache_grdma_ilwalidate_range (mca_rcache_base_module_t *rcache, void *base,
                                              size_t size);
...
#if OPAL_LWDA_GDR_SUPPORT
static int check_for_lwda_freed_memory(mca_rcache_base_module_t *rcache, void *addr, size_t size);
#endif /* OPAL_LWDA_GDR_SUPPORT */
...
void mca_rcache_grdma_module_init(mca_rcache_grdma_module_t* rcache, mca_rcache_grdma_cache_t *cache)
{
    ...
    rcache->super.rcache_ilwalidate_range = mca_rcache_grdma_ilwalidate_range;
...
static int mca_rcache_grdma_check_cached (mca_rcache_base_registration_t *grdma_reg, void *ctx)
{
    ...
    if ((grdma_reg->flags & MCA_RCACHE_FLAGS_ILWALID) || &rcache_grdma->super != grdma_reg->rcache ||
...
#if OPAL_LWDA_GDR_SUPPORT
    if (flags & MCA_RCACHE_FLAGS_LWDA_GPU_MEM) {
        size_t psize;
        mca_common_lwda_get_address_range(&base, &psize, addr);
        bound = base + psize - 1;
        /* Check to see if this memory is in the cache and if it has been freed. If so,
         * this call will boot it out of the cache. */
        check_for_lwda_freed_memory(rcache, base, psize);
    }
#endif /* OPAL_LWDA_GDR_SUPPORT */
...
#if OPAL_LWDA_GDR_SUPPORT
    if (flags & MCA_RCACHE_FLAGS_LWDA_GPU_MEM) {
        mca_common_lwda_get_buffer_id(grdma_reg);
    }
#endif /* OPAL_LWDA_GDR_SUPPORT */
...
static int mca_rcache_grdma_add_to_gc (mca_rcache_base_registration_t *grdma_reg)
{
    ...
    uint32_t flags = opal_atomic_fetch_or_32 ((volatile int32_t *) &grdma_reg->flags, MCA_RCACHE_FLAGS_ILWALID);

    if ((flags & MCA_RCACHE_FLAGS_ILWALID) || 0 != grdma_reg->ref_count) {
        /* nothing to do */
        return OPAL_SUCCESS;
    }

    /* This may be called from free() so avoid relwrsively calling into free by just
...
static int gc_add (mca_rcache_base_registration_t *grdma_reg, void *ctx)
{
    ...
    if (grdma_reg->flags & MCA_RCACHE_FLAGS_ILWALID) {
        /* nothing more to do */
        return OPAL_SUCCESS;
    }
...
static int mca_rcache_grdma_ilwalidate_range (mca_rcache_base_module_t *rcache,
 ...
 * that we do not have a lwMemAlloc, lwMemFree, lwMemAlloc state.  If we do
...
static int check_for_lwda_freed_memory (mca_rcache_base_module_t *rcache, void *addr, size_t size)
{
    ...
    if (!(mca_common_lwda_previously_freed_memory(reg))) {
        return OPAL_SUCCESS;
    }
     ...
     * relwrsive and this is only called from register. */
