 ...
 * GPU memory when using LWCA.  Hence, the name is "rgpusm" for "remote
 * LWCA" GPU memory.  There is a cache that can be used to store the
 ...
 * an error back from the LWCA library saying the memory is in use.
 ...
 * The LWCA library returns an error saying it is already mapped.  To
...
static inline bool mca_rcache_rgpusm_deregister_lru (mca_rcache_base_module_t *rcache) {
    ...
    rc = lwda_closememhandle (NULL, old_reg);
...
void mca_rcache_rgpusm_module_init(mca_rcache_rgpusm_module_t* rcache)
{
    ...
    opal_free_list_init (&rcache->reg_list, sizeof(struct mca_rcache_common_lwda_reg_t),
    ...
    rcache->stat_cache_valid = rcache->stat_cache_ilwalid = 0;
...
{
    mca_rcache_rgpusm_module_t *rcache_rgpusm = (mca_rcache_rgpusm_module_t*)rcache;
    mca_rcache_common_lwda_reg_t *rgpusm_reg;
    ...
    rget_reg = (mca_rcache_common_lwda_reg_t *)*reg;
     ...
     * number of registrations oclwring at the same time.  Since we
    ...
    if(!mca_rcache_rgpusm_component.leave_pinned && 0 == mca_rcache_rgpusm_component.rcache_size_limit) {
        ...
        rgpusm_reg = (mca_rcache_common_lwda_reg_t*)item;
        ...
        rc = lwda_openmemhandle (addr, size, (mca_rcache_base_registration_t *)rgpusm_reg,
    ...
    if (*reg != NULL) {
        ...
        if (mca_common_lwda_memhandle_matches((mca_rcache_common_lwda_reg_t *)*reg, rget_reg)) {
            ...
            (*reg)->ref_count++;
            /* Ilwalidate the registration so it will get booted out. */
            (*reg)->flags |= MCA_RCACHE_FLAGS_ILWALID;
            mca_rcache_rgpusm_deregister_no_lock(rcache, *reg);
            *reg = NULL;
            rcache_rgpusm->stat_cache_ilwalid++;
        }
    } else {
     ...
     * LWCA codes attempt to register the memory.  The case that this
     * can happen is as follows.  A block of memory is registered.
     * Then the sending side frees the memory.  The sending side then
     * lwMemAllocs memory again and gets the same base
     * address. However, it lwMemAllocs a block that is larger than
     * the one in the cache.  The cache will return that memory is not
     * registered and call into LWCA to register it.  However, that
     * will fail with LWDA_ERROR_ALREADY_MAPPED.  Therefore we need to
    ...
    if (OPAL_ERR_WOULD_BLOCK == rc) {
        ...
        if (NULL != oldreg) {
            ...
            oldreg->ref_count++;
            /* Ilwalidate the registration so it will get booted out. */
            oldreg->flags |= MCA_RCACHE_FLAGS_ILWALID;
                            ...
                            mca_rcache_base_registration_t *reg)
{
    ...
    if(mca_rcache_rgpusm_component.leave_pinned && registration_is_cachebale(reg))
    {
        ...
        {
             assert(reg->ref_count == 0);
             rc = lwda_closememhandle (NULL, reg);
         }
...
void mca_rcache_rgpusm_finalize(struct mca_rcache_base_module_t *rcache)
{
    ...
    if(true == mca_rcache_rgpusm_component.print_stats) {
        opal_output(0, "%s rgpusm: stats "
                ...
                rcache_rgpusm->stat_cache_ilwalid, rcache_rgpusm->stat_cache_miss,
                rcache_rgpusm->stat_evicted);
    }
