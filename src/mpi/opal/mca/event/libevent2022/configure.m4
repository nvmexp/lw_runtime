...
dnl
dnl This macro is ilwoked when event:external is going to be used (and
dnl this component is *not* going to be used).
dnl
dnl $1: action if this component can compile
dnl (we still ilwoke $1 so that "make distclean" and friends will work)
dnl
...
dnl ilwoked when this component is going to be used (i.e., when
dnl event:external is *not* going to be used).  If this macro is ilwoked
...
AC_DEFUN([MCA_opal_event_libevent2022_REAL_CONFIG],[
    ...
    # Ilwoke the embedded configure script.
