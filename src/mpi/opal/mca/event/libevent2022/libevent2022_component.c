...
static int libevent2022_register (void)
{
     ...
     * won't be used with libevent.  For example, we lwrrently have
    ...
    if (NULL != (*_eventop)) {
        ...
        int lwr_len = snprintf (available_eventops, len, "%s", (*(_eventop++))->name);

        for (int i = 1 ; ompi_eventops[i] && lwr_len < len ; ++i) {
            lwr_len += snprintf (available_eventops + lwr_len, len - lwr_len, ", %s",
