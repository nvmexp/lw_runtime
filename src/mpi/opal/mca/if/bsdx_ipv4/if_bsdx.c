...
/* colwert a netmask (in network byte order) to CIDR notation */
...
static int if_bsdx_open(void)
{
    ...
    for (lwr_ifaddrs = *ifadd_list; NULL != lwr_ifaddrs;
        ...
        if (AF_INET != lwr_ifaddrs->ifa_addr->sa_family) {
            continue;
        }
        ...
        sin_addr = (struct sockaddr_in *) lwr_ifaddrs->ifa_addr;
        ...
        ((struct sockaddr_in*) &intf->if_addr)->sin_len =  lwr_ifaddrs->ifa_addr->sa_len;
