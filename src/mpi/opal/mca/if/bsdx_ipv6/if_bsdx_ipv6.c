...
static int if_bsdx_ipv6_open(void)
{
    ...
    for (lwr_ifaddrs = *ifadd_list; NULL != lwr_ifaddrs;
         lwr_ifaddrs = lwr_ifaddrs->ifa_next) {
        opal_if_t *intf;
        struct in6_addr a6;

        /* skip non-ipv6 interface addresses */
        if (AF_INET6 != lwr_ifaddrs->ifa_addr->sa_family) {
            opal_output_verbose(1, opal_if_base_framework.framework_output,
                                "skipping non-ipv6 interface %s[%d].\n",
                                lwr_ifaddrs->ifa_name, (int)lwr_ifaddrs->ifa_addr->sa_family);
            continue;
        }

        /* skip interface if it is down (IFF_UP not set) */
        if (0 == (lwr_ifaddrs->ifa_flags & IFF_UP)) {
            opal_output_verbose(1, opal_if_base_framework.framework_output,
                                "skipping non-up interface %s.\n", lwr_ifaddrs->ifa_name);
            continue;
        }

        /* skip interface if it is a loopback device (IFF_LOOPBACK set) */
        if (!opal_if_retain_loopback && 0 != (lwr_ifaddrs->ifa_flags & IFF_LOOPBACK)) {
            opal_output_verbose(1, opal_if_base_framework.framework_output,
        ...
        if (0!= (lwr_ifaddrs->ifa_flags & IFF_POINTOPOINT)) {
            opal_output_verbose(1, opal_if_base_framework.framework_output,
                                "skipping p2p interface %s.\n", lwr_ifaddrs->ifa_name);
            continue;
        }

        sin_addr = (struct sockaddr_in6 *) lwr_ifaddrs->ifa_addr;
        ...
        if ((IN6_IS_ADDR_LINKLOCAL (&sin_addr->sin6_addr))) {
            opal_output_verbose(1, opal_if_base_framework.framework_output,
                                ...
                                "%s with scope %d.\n",
                                lwr_ifaddrs->ifa_name, sin_addr->sin6_scope_id);
            continue;
        }
        ...
        if (0 < opal_output_get_verbosity(opal_if_base_framework.framework_output)) {
            ...
            opal_output(0, "ipv6 capable interface %s discovered, address %s.\n",
                        lwr_ifaddrs->ifa_name, addr_name);
            free(addr_name);
        }
        ...
        strncpy(intf->if_name, lwr_ifaddrs->ifa_name, IF_NAMESIZE);
         ...
         */
        intf->if_mask = 64;
        intf->if_flags = lwr_ifaddrs->ifa_flags;
        ...
        intf->if_kernel_index =
            (uint16_t) if_nametoindex(lwr_ifaddrs->ifa_name);
