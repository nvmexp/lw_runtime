...
static int if_posix_open(void)
{
     ...
     *
     * - Solaris returns -1, errno EILWAL if there is not enough
     *   space
    ...
    do {
        ...
        if (ioctl(sd, SIOCGIFCONF, &ifconf) < 0) {
            /* if we got an eilwal, we probably don't have enough
               space.  so we'll fall down and try to expand our
               space */
            if (errno != EILWAL && lastlen != 0) {
        ...
        /* Yes, we overflowed (or had an EILWAL on the ioctl).
