                ...
                mca_btl_base_endpoint_t         **endpoint)
{
    ...
    for (uint32_t i = 0 ; i < nprocs ; i++) {
        struct opal_proc_t *lwrr_proc;

        lwrr_proc = procs[i];

        /* portals doesn't support heterogeneous yet... */
        if (opal_proc_local_get()->proc_arch != lwrr_proc->proc_arch) {
            opal_output_verbose(1, opal_btl_base_framework.framework_output,
                                "Portals 4 BTL does not support heterogeneous operations.");
            opal_output_verbose(1, opal_btl_base_framework.framework_output,
                "Proc %s architecture %x, mine %x.",
                OPAL_NAME_PRINT(lwrr_proc->proc_name),
                lwrr_proc->proc_arch, opal_proc_local_get()->proc_arch);
                          ...
                          opal_bitmap_t* reachable)
{
     ...
     */
    for (i = 0 ; i < nprocs ; ++i) {
        struct opal_proc_t *lwrr_proc = procs[i];
                      ...
                      mca_btl_base_descriptor_t* des)
{
    ...
    if (BTL_PORTALS4_FRAG_TYPE_EAGER == frag->type) {
        ...
        if (frag->me_h != PTL_ILWALID_HANDLE) {
            frag->me_h = PTL_ILWALID_HANDLE;
        }
                            ...
                            uint32_t flags)
{
    ...
    if (0 != reserve || 0 != opal_colwertor_need_buffers(colwertor)) {
        ...
        ret = opal_colwertor_pack(colwertor, &iov, &iov_count, &max_data );
                                ...
                                mca_btl_base_registration_handle_t *handle)
{
    ...
    if (!PtlHandleIsEqual(handle->me_h, PTL_ILWALID_HANDLE)) {
        ...
        handle->me_h = PTL_ILWALID_HANDLE;
    }
...
void mca_btl_portals4_free_module(mca_btl_portals4_module_t *portals4_btl)
{
    ...
    if (!PtlHandleIsEqual(portals4_btl->send_md_h, PTL_ILWALID_HANDLE)) {
        PtlMDRelease(portals4_btl->send_md_h);
        portals4_btl->send_md_h = PTL_ILWALID_HANDLE;
    }
    if (!PtlHandleIsEqual(portals4_btl->zero_md_h, PTL_ILWALID_HANDLE)) {
        PtlMDRelease(portals4_btl->zero_md_h);
        portals4_btl->zero_md_h = PTL_ILWALID_HANDLE;
    }

    if (!PtlHandleIsEqual(portals4_btl->long_overflow_me_h, PTL_ILWALID_HANDLE)) {
        PtlMEUnlink(portals4_btl->long_overflow_me_h);
        portals4_btl->long_overflow_me_h = PTL_ILWALID_HANDLE;
    }
    ...
    if (!PtlHandleIsEqual(portals4_btl->portals_ni_h, PTL_ILWALID_HANDLE)) {
        ...
        portals4_btl->portals_ni_h = PTL_ILWALID_HANDLE;
    }
