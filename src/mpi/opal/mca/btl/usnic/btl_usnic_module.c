                            ...
                            char *str, size_t len_param)
{
    ...
    if (len > len_param) {
        opal_show_help("help-mpi-btl-usnic.txt",
                       ...
                       FI_ENODATA,
                       "Failed to colwert address to string: buffer too short");
                                      ...
                                      struct mca_btl_base_endpoint_t** endpoints)
{
    /* We need to ensure that we don't overrun the libfabric AV EQ.
       ...
       sliding window style of AV inserts to get better conlwrrency of
       AV resolution). */
    ...
    /* Per above, the blocks are expressed in terms of number of AV
       inserts.  Colwert them to be expressed in terms of number of
       procs. */
        ...
        if (NULL != proc) {
            ...
            for (j = 0; j < proc->proc_endpoint_count; ++j) {
                ...
                if (NULL != endpoint && endpoint->endpoint_module == module) {
                     ...
                     * ***because it is only ever ilwoked after an
 ...
 * the colwertor contained in the frag.
 *
    ...
    opal_btl_usnic_large_send_frag_t *lfrag)
{
     ...
     * we aren't using a colwertor (SG[1] will be NULL if we have a colwertor).
     */
    while (seg_space > 0 &&
           lfrag->lsf_pack_bytes_left > 0 &&
           NULL != lfrag->lsf_lwr_ptr) {
        ...
        memcpy(copyptr, lfrag->lsf_lwr_ptr, copylen);
        seg_space -= copylen;
        copyptr += copylen;
        lfrag->lsf_bytes_left_in_sge -= copylen;
        lfrag->lsf_pack_bytes_left -= copylen;
        if (lfrag->lsf_bytes_left_in_sge > 0) {
            lfrag->lsf_lwr_ptr += copylen;
        } else {
            ++lfrag->lsf_lwr_sge;
            lfrag->lsf_lwr_ptr =
                lfrag->lsf_des_src[lfrag->lsf_lwr_sge].seg_addr.pval;
            lfrag->lsf_bytes_left_in_sge =
                lfrag->lsf_des_src[lfrag->lsf_lwr_sge].seg_len;
        }
    }

    if (seg_space > 0 && lfrag->lsf_pack_bytes_left > 0) {
        /* the remaining bytes come from a colwertor; pack using it */
        assert(NULL == lfrag->lsf_lwr_ptr);
        assert(1 == lfrag->lsf_lwr_sge);

        copylen = lfrag->lsf_pack_bytes_left;
        if (copylen > seg_space) {
            copylen = seg_space;
        }
        usnic_colwertor_pack_simple(&lfrag->lsf_base.sf_colwertor, copyptr,
                                    copylen, &max_data);
        seg_space -= max_data;
    ...
    opal_btl_usnic_send_frag_t *frag)
{
    ...
    chp->ch_frag_offset = lfrag->lsf_lwr_offset;
    ...
    lfrag->lsf_lwr_offset += payload_len;
...
/*
 * Called back by libevent if an async event oclwrs on the device
 */
static void module_async_event_callback(int fd, short flags, void *arg)
{
    ...
    if (-FI_EAGAIN == ret) {
        ...
        default:
            ...
            /* For the moment, these are the only other cases libfabric
               ...
               for completeness.  We lwrrently abort if any async
               event other than LINK_UP oclwrs. */
    ...
    /* If this is fatal, ilwoke the upper layer error handler to abort
       the job */
    if (fatal) {
        opal_btl_usnic_exit(module);
        /* Does not return */
...
/*************************************************************************
 The following routines are all short utility / colwenience functions
 for module_init().
*************************************************************************/
 ...
 * callwlate some payload lengths (and indirectly, some queue
 * lengths).
 */
...
{
#if OPAL_HAVE_HWLOC
    /* If this process is bound to a single NUMA locality, callwlate
