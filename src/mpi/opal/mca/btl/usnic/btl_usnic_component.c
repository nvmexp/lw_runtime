...
opal_relwrsive_mutex_t btl_usnic_lock =  OPAL_RELWRSIVE_MUTEX_STATIC_INIT;
...
static int usnic_component_close(void)
{
    /* Note that this list should already be empty, because:
       - module.finalize() is ilwoked before component.close()
...
static int check_reg_mem_basics(void)
{
    ...
    if (0 == ret) {
        if ((long) limit.rlim_lwr > (64 * 1024) ||
            limit.rlim_lwr == RLIM_INFINITY) {
            return OPAL_SUCCESS;
        } else {
                                                    ...
                                                    bool want_mpi_threads)
{
    ...
    *num_btl_modules = 0;
    ...
    OBJ_CONSTRUCT(&btl_usnic_lock, opal_relwrsive_mutex_t);
    ...
    /* There are multiple dimensions to consider when requesting an
       ...
       is lwrrently no way to know a) what package version of
        ...
        switch (frag->uf_type) {
            case OPAL_BTL_USNIC_FRAG_LARGE_SEND:
                ...
                snprintf(tmp, sizeof(tmp), " tag=%"PRIu8" id=%"PRIu32" offset=%llu/%llu post_cnt=%"PRIu32" ack_bytes_left=%llu\n",
                        lsfrag->lsf_tag,
                        lsfrag->lsf_frag_id,
                        (unsigned long long)lsfrag->lsf_lwr_offset,
