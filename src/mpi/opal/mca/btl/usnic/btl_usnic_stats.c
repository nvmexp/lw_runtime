...
#define REGISTERC(field, desc) \
    ...
    REGISTERC(num_timeout_retrans,
              "Number of times chunk retransmissions have oclwred because an ACK was not received within the timeout");
    REGISTERC(num_fast_retrans,
              "Number of times chunk retransmissions have oclwred because due to a repeated ACK");
