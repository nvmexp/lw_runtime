...
/* a helper function that just declutters colwertor packing */
static inline void
    ...
    size_t *bytes_packed)
{
    ...
    *bytes_packed = max_bytes_to_pack;
    rc = opal_colwertor_pack(colwertor, &iov, &iov_count, bytes_packed);
    if (OPAL_UNLIKELY(rc < 0)) {
        BTL_ERROR(("opal_colwertor_pack error"));
        abort();    /* XXX */
    }
}
...
size_t opal_btl_usnic_colwertor_pack_peek(const opal_colwertor_t *colw,
                                          size_t max_len);
