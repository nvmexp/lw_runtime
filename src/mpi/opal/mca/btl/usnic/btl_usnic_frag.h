 ...
 * cookie).
 *
 * Lwrrently, this struct is meaningless (but it must be defined /
...
typedef struct opal_btl_usnic_send_frag_t {
    ...
    struct opal_colwertor_t sf_colwertor; /* copy of original message data if
                                             colwertor required */

    uint32_t sf_seg_post_cnt;   /* total segs lwrrently posted for this frag */
...
typedef struct opal_btl_usnic_large_send_frag_t {
    ...
    size_t lsf_lwr_offset;      /* next byte offset to be enqueued on the
                                   endpoint (incl. any colwertor payload) */
    ...
    size_t lsf_pack_bytes_left; /* bytes remaining to be packed into chunk
                                   segments (incl. any colwertor payload) */
    uint8_t *lsf_lwr_ptr;       /* current packing pointer */
    int lsf_lwr_sge;
    size_t lsf_bytes_left_in_sge;

    uint8_t *lsf_buffer;        /* attached storage for usnic_alloc() */

    opal_list_t lsf_seg_chain;  /* chain of segments for colwerted data */

    bool lsf_pack_on_the_fly;   /* true if we are packing on the fly */
} opal_btl_usnic_large_send_frag_t;

/* Shortlwt member macros.  Access uf_src_seg array instead of the descriptor's
 ...
 * an inline send, but will colwert to a single SG entry is inline cannot
 ...
 * and colwert to a single segment.
 */
    ...
    opal_btl_usnic_frag_t *frag)
{
    ...
    if (frag->uf_type == OPAL_BTL_USNIC_FRAG_LARGE_SEND) {
        ...
        if (2 == lfrag->lsf_base.sf_base.uf_base.USNIC_SEND_LOCAL_COUNT &&
            ...
            opal_colwertor_cleanup(&lfrag->lsf_base.sf_colwertor);
        }
    }
