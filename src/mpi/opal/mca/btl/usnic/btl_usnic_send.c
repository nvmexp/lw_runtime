    ...
    mca_btl_base_tag_t tag)
{
    ...
    if (frag->sf_base.uf_type == OPAL_BTL_USNIC_FRAG_SMALL_SEND) {
         ...
         * We already packed via the colwertor if necessary, so we only need to
        ...
        /* Save info about the frag so that future ilwocations of
         * usnic_handle_large_send can generate segments to put on the wire. */
        lfrag = (opal_btl_usnic_large_send_frag_t *)frag;
        lfrag->lsf_tag = tag;
        lfrag->lsf_lwr_offset = 0;
        lfrag->lsf_lwr_ptr = lfrag->lsf_des_src[0].seg_addr.pval;
        lfrag->lsf_lwr_sge = 0;
        ...
        if (lfrag->lsf_pack_on_the_fly) {
            ...
            /* we pre-packed the colwertor into a chain in prepare_src */
