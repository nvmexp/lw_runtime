...
hijack it for sending very small fragments (<= tiny_mtu, lwrrently 768 bytes)
...
desc = usnic_prepare_src(colwertor, size)
usnic_send(desc)
...
usnic_alloc() lwrrently asserts the length is "small", allocates and
...
place where PML should construct its header.  If the data colwertor says
data is contiguous, 2nd SG entry points to user buffer, else it is null and
sf_colwertor is filled in with address of colwertor.
...
usnic_put()
Do a fast version of what happens in prepare_src() (can take shortlwts
because we know it will always be a contiguous buffer / no colwertor
...
each endpoint has a "send window" , lwrrently 4096 entries.
...
will then ilwoke PUT on the source's BTL module.  With GET, the target
...
out the use of the descriptors.  Have not ilwestigated how easy/hard
that would be.
...
- VF/PF sanity checks in component.c:check_usnic_config() uses
  ...
  as provider-specific info is still being dislwssed upstream.
...
The said lock is btl_usnic_lock. It is a RELWRSIVE lock, meaning that
