...
static int btl_openib_modex_send(void)
{
    ...
    OPAL_MODEX_SEND(rc, OPAL_PMIX_GLOBAL,
                    ...
                    message, msg_size);
    free(message);
    opal_output(-1, "Modex sent!  %d callwlated, %d actual\n", (int) msg_size, (int) (offset - message));
...
#if OPAL_LWDA_SUPPORT
    if (reg->flags & MCA_RCACHE_FLAGS_LWDA_REGISTER_MEM) {
        mca_common_lwda_register (base, size,
...
#if OPAL_LWDA_SUPPORT
        if (reg->flags & MCA_RCACHE_FLAGS_LWDA_REGISTER_MEM) {
            mca_common_lwda_unregister(openib_reg->base.base,
                openib_reg->base.rcache->rcache_component->rcache_version.mca_component_name);
        }
#endif
...
#define P(N) (((N) > count) ? NULL : params[(N)])
    while (queues[qp] != NULL) {
        ...
        if ('P' == params[0][0]) {
            ...
            /* Callwlate the smallest freelist size that can be allowed */
            ...
            /* rd_init is initial value for rd_lwrr_num of all SRQs, 1/4 of rd_num by default */
            rd_init = atoi_param(P(5), rd_num / 4);
            /* by default set srq_limit to be 3/16 of rd_init (it's 1/4 of rd_low_local,
               the value of rd_low_local we callwlate in create_srq function) */
            srq_limit = atoi_param(P(6), (rd_init - (rd_init / 4)) / 4);

            /* If we set srq_limit less or greater than rd_init
               (init value for rd_lwrr_num) => we receive the IBV_EVENT_SRQ_LIMIT_REACHED
               event immediately and the value of rd_lwrr_num will be increased */
...
/* callwlate memory registation limits */
static uint64_t callwlate_total_mem (void)
{
    hwloc_obj_t machine;
    int rc;
    uint64_t mem, *mptr;
...
static uint64_t callwlate_max_reg (const char *device_name)
{
...
static int init_one_device(opal_list_t *btl_list, struct ibv_device* ib_dev)
{
    ...
    device->mem_reg_max_total = callwlate_max_reg(ibv_get_device_name(ib_dev));
    ...
    /* Eager RDMA is not lwrrently supported with progress threads */
        ...
        /* Check to ensure that all devices used in this process have
           ...
           --> Jeff thinks that it would be great to use
               ...
               1. The check for local receive_queue compatibility is
                  ...
                  leaving this as explicitly dolwmented for some
                  future implementer...
               ...
               2. Conside a scenario with server 1 having HCA A/subnet
                  ...
                  But note that modex [lwrrently] only sends around
                  ...
                  now, so we're leaving this as explicitly dolwmented
...
static float get_ib_dev_distance(struct ibv_device *dev)
{
    ...
    case HWLOC_OBJ_PU:
        ...
        if (NULL != my_obj) {
            ...
            /* Distance may be asymetrical, so callwlate both of them
               and take the max */
            #if HWLOC_API_VERSION < 0x20000
...
#if OPAL_LWDA_SUPPORT
   if (mca_btl_openib_component.lwda_want_gdr && (0 == opal_leave_pinned)) {
        opal_show_help("help-mpi-btl-openib.txt",
                       "LWDA_gdr_and_nopinned", true,
                       opal_process_info.nodename);
        goto no_btls;
    }
#endif /* OPAL_LWDA_SUPPORT */
            ...
            */
            for (len = opal_list_get_size(&ep->qps[qp].no_credits_pending_frags[pri]);
                ...
                /* If _endpoint_post_send() fails because of
                   ...
                   oclwrred, starting at the send frag out-of-credits
                                         ...
                                         size_t byte_len)
{
    ...
    if(OPAL_LIKELY(!(is_credit_msg = is_credit_message(frag)))) {
        ...
        assert(0 == (des->des_flags & MCA_BTL_DES_FLAGS_LWDA_COPY_ASYNC));
#endif /* OPAL_LWDA_SUPPORT */
        reg = mca_btl_base_active_message_trigger + hdr->tag;
        reg->cbfunc( &openib_btl->super, hdr->tag, des, reg->cbdata );
#if OPAL_LWDA_SUPPORT /* LWDA_ASYNC_RECV */
        if (des->des_flags & MCA_BTL_DES_FLAGS_LWDA_COPY_ASYNC) {
...
static int btl_openib_component_progress(void)
{
...
#if OPAL_LWDA_SUPPORT /* LWDA_ASYNC_SEND */
    /* Check to see if there are any outstanding dtoh LWCA events that
     ...
     */
    {
        ...
        while (local_count < 10 && (1 == progress_one_lwda_dtoh_event(&frag))) {
...
int mca_btl_openib_post_srr(mca_btl_openib_module_t* openib_btl, const int qp)
{
    ...
    int rd_lwrr_num = openib_btl->qps[qp].u.srq_qp.rd_lwrr_num;
    int num_post, i, rc;
    ...
    num_post = rd_lwrr_num - openib_btl->qps[qp].u.srq_qp.rd_posted;
    ...
    if(OPAL_LIKELY(0 == rc)) {
        ...
        if(true == openib_btl->qps[qp].u.srq_qp.srq_limit_event_flag) {
            srq_attr.max_wr = openib_btl->qps[qp].u.srq_qp.rd_lwrr_num;
            srq_attr.max_sge = 1;
