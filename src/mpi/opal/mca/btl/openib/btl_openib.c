                                    ...
                                    const char *func, const char *dev)
{
    if (ENOMEM == errno) {
        ...
        } else if (limit.rlim_lwr == RLIM_INFINITY) {
            asprintf(&str_limit, "unlimited");
        } else {
            asprintf(&str_limit, "%ld", (long)limit.rlim_lwr);
        }
...
static int create_srq(mca_btl_openib_module_t *openib_btl)
{
    ...
    for(qp = 0; qp < mca_btl_openib_component.num_qps; qp++) {
        ...
        if(!BTL_OPENIB_QP_TYPE_PP(qp)) {
            ...
            rd_lwrr_num = openib_btl->qps[qp].u.srq_qp.rd_lwrr_num = mca_btl_openib_component.qp_infos[qp].u.srq_qp.rd_init;

            if(true == mca_btl_openib_component.enable_srq_resize &&
                                    true == device_support_modify_srq) {
                if(0 == rd_lwrr_num) {
                    openib_btl->qps[qp].u.srq_qp.rd_lwrr_num = 1;
                }

                openib_btl->qps[qp].u.srq_qp.rd_low_local = rd_lwrr_num - (rd_lwrr_num >> 2);
                openib_btl->qps[qp].u.srq_qp.srq_limit_event_flag = true;
            } else {
                openib_btl->qps[qp].u.srq_qp.rd_lwrr_num = rd_num;
    ...
    uint32_t flags)
{
    ...
    /* If the colwertor is copying the data asynchronously, then record an event
    ...
    if ((colwertor->flags & COLWERTOR_LWDA_ASYNC) && (0 != max_data)) {
        mca_common_lwda_record_dtoh_event("btl_openib", (mca_btl_base_descriptor_t *)frag);
        to_base_frag(frag)->base.des_flags = flags | MCA_BTL_DES_FLAGS_LWDA_COPY_ASYNC;
    }
#endif /* OPAL_LWDA_SUPPORT */
...
#if OPAL_LWDA_GDR_SUPPORT
    if (flags & MCA_BTL_REG_FLAG_LWDA_GPU_MEM) {
        mflags |= MCA_RCACHE_FLAGS_LWDA_GPU_MEM;
    }
#endif /* OPAL_LWDA_GDR_SUPPORT */
