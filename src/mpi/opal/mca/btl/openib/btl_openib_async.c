...
/* Function colwerts event to string (name)
...
/* The main idea of resizing SRQ algorithm -
   ...
   the number of WQEs that we post = rd_lwrr_num < rd_num and this value is
...
static int btl_openib_async_srq_limit_event(struct ibv_srq* srq)
{
    ...
    if(openib_btl->qps[qp].u.srq_qp.rd_lwrr_num >=
                         mca_btl_openib_component.qp_infos[qp].rd_num) {
        openib_btl->qps[qp].u.srq_qp.rd_lwrr_num = mca_btl_openib_component.qp_infos[qp].rd_num;
