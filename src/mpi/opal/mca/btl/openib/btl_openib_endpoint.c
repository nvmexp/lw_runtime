...
void mca_btl_openib_endpoint_send_cts(mca_btl_openib_endpoint_t *endpoint)
{
    ...
    if (OPAL_UNLIKELY(NULL == sc_frag)) {
        ...
        mca_btl_openib_endpoint_ilwoke_error(endpoint);
        return;
    }
...
/*
 * Ilwoke an error on the btl associated with an endpoint.  If we
 * don't have an endpoint, then just use the first one on the
 * component list of BTLs.
 */
void *mca_btl_openib_endpoint_ilwoke_error(void *context)
{
    ...
    /* Ilwoke the callback to the upper layer */
