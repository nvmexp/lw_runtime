 ...
 * excluded, none of its other interface functions will be ilwoked for
 * the duration of the process.
 *
 * - component_init(): The openib BTL's component_init() function
 * calls connect_base_init(), which will ilwoke this query function on
 ...
 * is connected to us yet), this function is ilwoked to post buffers
 * on the QP, setup credits for the endpoint, etc.  This function is
 * *only* ilwoked if the CPC's cbm_uses_cts is false.
 *
 * - opal_btl_openib_cpc_complete(endpoint): once that a CPC knows
 * that a QP is connected on *both* sides, this function is ilwoked to
 * tell the main openib BTL "ok, you can use this connection now."
 * (e.g., the main openib BTL will either ilwoke the CTS protocol or
...
/**
 * This function is ilwoked once by the openib BTL component during
 ...
 * returned OPAL_SUCCESS, this function is ilwoked once per BTL module
 ...
 *
 * - Other OPAL_ERR_* code: an error oclwrred.
 */
...
typedef struct opal_btl_openib_connect_base_module_data_t {
    /** Pointer back to the component.  Used by the base and openib
        btl to callwlate this module's index for the modex. */
