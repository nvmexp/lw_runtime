...
/* Callwlate the *real* length of the message (not aligned/rounded
   up) */
...
/*
 * Ilwoked by both main and service threads
 */
                                     ...
                                     int num)
{
    ...
    /* This is odd an worth explaining: when we place the context on
       ...
       1. Normal termination: client side endpoint_finalize removes
          ...
          will occur on both sides; the client DISCONNECT will ilwoke
          ...
          will ilwoke rdma_destroy_id() -- we obviously can't have
          ...
          the server side will only ilwoke one RELEASE.
       ...
       2. Abnormal termination: if the server side terminates
          ...
          oclwrs *and* the DISCONNECT event arrives.
                                       ...
                                       mca_btl_base_endpoint_t *endpoint)
{
    ...
    /* Don't use the CPC to get the message, because this function is
       ilwoked from the event_handler (to intitiate connections in the
...
static void *show_help_cant_find_endpoint(void *context)
{
    ...
    /* Now kill it */
    mca_btl_openib_endpoint_ilwoke_error(NULL);
    return NULL;
}
#endif
 ...
 * is zero chance that the DISCONNECT event is delivered and exelwted
 ...
 * progress thread because this function may be ilwoked to kill a
    ...
    OPAL_LIST_FOREACH(item, &client_list, rdmacm_contents_t) {
        if (endpoint == item->endpoint) {
            ...
            /* Fun race condition: we cannot call
               ...
               DISCONNECT event will be delivered and get exelwted
               ...
               ilwoke rdma_disconnect() in the rdmacm event thread
...
static inline void rdmamcm_event_error(struct rdma_cm_event *event)
{
    ...
    mca_btl_openib_run_in_main (mca_btl_openib_endpoint_ilwoke_error,
                                endpoint);
}
...
static void *rdmacm_event_dispatch(int fd, int flags, void *context)
{
     ...
     * included data in the event.  In certain cirlwmstances, the time
