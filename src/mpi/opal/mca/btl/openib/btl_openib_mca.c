...
#define CHECK(expr) do {\
    ...
    tmp = mca_base_component_var_register(&mca_btl_openib_component.super.btl_version,
                                          "have_fork_support",
                                          "Whether the OpenFabrics stack supports applications that ilwoke the \"fork()\" system call or not (0 = no, 1 = yes). "
...
#if OPAL_LWDA_SUPPORT /* LWDA_ASYNC_RECV */
    /* Default is enabling LWCA asynchronous send copies */
    CHECK(reg_bool("lwda_async_send", NULL,
                   "Enable or disable LWCA async send copies "
                   "(true = async; false = sync)",
                   true, &mca_btl_openib_component.lwda_async_send));

    /* Default is enabling LWCA asynchronous receive copies */
    CHECK(reg_bool("lwda_async_recv", NULL,
                   "Enable or disable LWCA async recv copies "
                   "(true = async; false = sync)",
                   false, &mca_btl_openib_component.lwda_async_recv));
    ...
    mca_btl_openib_component.lwda_have_gdr = OPAL_INT_TO_BOOL(OPAL_LWDA_GDR_SUPPORT);
    (void) mca_base_component_var_register(&mca_btl_openib_component.super.btl_version, "have_lwda_gdr",
                                           "Whether LWCA GPU Direct RDMA support is built into library or not",
                                           MCA_BASE_VAR_TYPE_BOOL, NULL, 0,
                                           MCA_BASE_VAR_FLAG_DEFAULT_ONLY,
                                           OPAL_INFO_LVL_5,
                                           MCA_BASE_VAR_SCOPE_CONSTANT,
                                           &mca_btl_openib_component.lwda_have_gdr);

    /* Indicates if driver has GPU Direct RDMA support.  Not changeable.  */
    if (OPAL_SUCCESS == opal_os_dirpath_access("/sys/kernel/mm/memory_peers/lw_mem/version", S_IRUSR)) {
    ...
    CHECK(reg_bool("want_lwda_gdr", NULL,
                   "Enable or disable LWCA GPU Direct RDMA support "
                   "(true = enabled; false = disabled)",
                   false, &mca_btl_openib_component.lwda_want_gdr));

    if (mca_btl_openib_component.lwda_want_gdr && !mca_btl_openib_component.lwda_have_gdr) {
        opal_show_help("help-mpi-btl-openib.txt",
                       "LWDA_no_gdr_support", true,
                       opal_process_info.nodename);
        return OPAL_ERROR;
    }
    if (mca_btl_openib_component.lwda_want_gdr && !mca_btl_openib_component.driver_have_gdr) {
        opal_show_help("help-mpi-btl-openib.txt",
    ...
    if (mca_btl_openib_component.lwda_want_gdr) {
        mca_btl_openib_module.super.btl_flags |= MCA_BTL_FLAGS_LWDA_GET;
        mca_btl_openib_module.super.btl_lwda_eager_limit = SIZE_MAX; /* magic number - indicates set it to minimum */
        mca_btl_openib_module.super.btl_lwda_rdma_limit = 30000;  /* default switchover is 30,000 to pipeline */
    } else {
        mca_btl_openib_module.super.btl_lwda_eager_limit = 0; /* Turns off any of the GPU Direct RDMA code */
        mca_btl_openib_module.super.btl_lwda_rdma_limit = 0;  /* Unused */
    }
...
#if OPAL_LWDA_SUPPORT /* LWDA_ASYNC_RECV */
    if (mca_btl_openib_component.lwda_async_send) {
        mca_btl_openib_module.super.btl_flags |= MCA_BTL_FLAGS_LWDA_COPY_ASYNC_SEND;
    } else {
        mca_btl_openib_module.super.btl_flags &= ~MCA_BTL_FLAGS_LWDA_COPY_ASYNC_SEND;
    }

    if (mca_btl_openib_component.lwda_async_recv) {
        mca_btl_openib_module.super.btl_flags |= MCA_BTL_FLAGS_LWDA_COPY_ASYNC_RECV;
    } else {
        mca_btl_openib_module.super.btl_flags &= ~MCA_BTL_FLAGS_LWDA_COPY_ASYNC_RECV;
    }
...
#endif /* Workaround */
    if (0 != mca_btl_openib_module.super.btl_lwda_max_send_size) {
        opal_show_help("help-mpi-btl-openib.txt", "do_not_set_openib_value",
                       true, opal_process_info.nodename);
        mca_btl_openib_module.super.btl_lwda_max_send_size = 0;
    }
#endif
