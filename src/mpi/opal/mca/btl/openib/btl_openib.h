...
struct mca_btl_openib_srq_qp_info_t {
    int32_t sd_max;
    /* The init value for rd_lwrr_num variables of all SRQs */
    int32_t rd_init;
...
struct mca_btl_openib_component_t {
    ...
    bool lwda_async_send;
    bool lwda_async_recv;
    bool lwda_have_gdr;
...
typedef struct mca_btl_openib_modex_message_t {
    ...
    /** Dummy field used to callwlate the real length */
    uint8_t end;
...
struct mca_btl_openib_module_srq_qp_t {
    ...
    /** We post additional WQEs only if a number of WQEs (in specific SRQ) is less of this value.
         The value increased together with rd_lwrr_num. The value is unique for every SRQ. */
    ...
    /**< In difference of the "--mca enable_srq_resize" parameter that says, if we want(or no)
         to start with small num of pre-posted receive buffers (rd_lwrr_num) and to increase this number by needs
         (the max of this value is rd_num * the whole size of SRQ), the "srq_limit_event_flag" says if we want to get limit event
         from device if the defined srq limit was reached (signal to the main thread) and we put off this flag if the rd_lwrr_num
 ...
 * @param colwertor (IN)         Datatypes colwerter
...
/**
 * Show an error during init, partilwlarly when running out of
 * registered memory.
 */
