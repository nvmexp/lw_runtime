...
#include "btl_smlwda_endpoint.h"
#include "btl_smlwda_frag.h"
#include "btl_smlwda_fifo.h"

#if OPAL_LWDA_SUPPORT
static struct mca_btl_base_registration_handle_t *mca_btl_smlwda_register_mem (
    struct mca_btl_base_module_t* btl, struct mca_btl_base_endpoint_t *endpoint, void *base,
    size_t size, uint32_t flags);

static int mca_btl_smlwda_deregister_mem (struct mca_btl_base_module_t* btl,
                                          struct mca_btl_base_registration_handle_t *handle);
#endif

mca_btl_smlwda_t mca_btl_smlwda = {
    .super = {
        .btl_component = &mca_btl_smlwda_component.super,
        .btl_add_procs = mca_btl_smlwda_add_procs,
        .btl_del_procs = mca_btl_smlwda_del_procs,
        .btl_finalize = mca_btl_smlwda_finalize,
        .btl_alloc = mca_btl_smlwda_alloc,
        .btl_free = mca_btl_smlwda_free,
        .btl_prepare_src = mca_btl_smlwda_prepare_src,
#if OPAL_LWDA_SUPPORT
        .btl_register_mem = mca_btl_smlwda_register_mem,
        .btl_deregister_mem = mca_btl_smlwda_deregister_mem,
#endif /* OPAL_LWDA_SUPPORT */
        .btl_send = mca_btl_smlwda_send,
        .btl_sendi = mca_btl_smlwda_sendi,
        .btl_dump = mca_btl_smlwda_dump,
        .btl_register_error = mca_btl_smlwda_register_error_cb,
        .btl_ft_event = mca_btl_smlwda_ft_event
    }
};

#if OPAL_LWDA_SUPPORT
static void mca_btl_smlwda_send_lwda_ipc_request(struct mca_btl_base_module_t* btl,
                                                 struct mca_btl_base_endpoint_t* endpoint);
#endif /* OPAL_LWDA_SUPPORT */
/*
 * callwlate offset of an address from the beginning of a shared memory segment
 */
#define ADDR2OFFSET(ADDR, BASE) ((char*)(ADDR) - (char*)(BASE))

/*
 * callwlate an absolute address in a local address space given an offset and
...
static void *mpool_calloc(size_t nmemb, size_t size)
{
    ...
    mca_mpool_base_module_t *mpool = mca_btl_smlwda_component.sm_mpool;
...
static int
setup_mpool_base_resources(mca_btl_smlwda_component_t *comp_ptr,
...
static int
sm_segment_attach(mca_btl_smlwda_component_t *comp_ptr)
{
    int rc = OPAL_SUCCESS;
    int fd = -1;
    ssize_t bread = 0;
...
static int
smlwda_btl_first_time_init(mca_btl_smlwda_t *smlwda_btl,
                       int32_t my_smp_rank,
                       int n)
{
    ...
    mca_btl_smlwda_component_t* m = &mca_btl_smlwda_component;
    char *loc, *mynuma;
    opal_process_name_t wildcard_rank;

    /* Assume we don't have hwloc support and fill in dummy info */
    mca_btl_smlwda_component.mem_node = my_mem_node = 0;
    mca_btl_smlwda_component.num_mem_nodes = num_mem_nodes = 1;
    ...
    if (OPAL_SUCCESS == rc) {
        ...
        mca_btl_smlwda_component.num_mem_nodes = num_mem_nodes = strtoul(loc, NULL, 10);
        free(loc);
    } else {
        ...
        if (OPAL_SUCCESS == opal_hwloc_base_get_topology()) {
            ...
            /* JMS This tells me how many numa nodes are *available*,
               ...
               used *in this job*. */
            mca_btl_smlwda_component.num_mem_nodes = num_mem_nodes = i;
        }
    }
    ...
    if (OPAL_SUCCESS == rc) {
        if (NULL == loc) {
            mca_btl_smlwda_component.mem_node = my_mem_node = -1;
        } else {
            ...
            if (NULL == mynuma ||
                ...
                mca_btl_smlwda_component.mem_node = my_mem_node;
            }
            if (NULL != mynuma) {
                free(mynuma);
            }
        ...
        if (OPAL_SUCCESS == opal_hwloc_base_get_topology() &&
             ...
             */
            if (1 == n_bound) {
                mca_btl_smlwda_component.mem_node = my_mem_node = numa;
            } else {
                mca_btl_smlwda_component.mem_node = my_mem_node = -1;
            }
        }
    ...
    mca_btl_smlwda_component.sm_mpools =
    ...
    res->mem_node = -1;
    res->allocator = mca_btl_smlwda_component.allocator;
    ...
    mca_btl_smlwda_component.sm_mpools[0] = common_sm_mpool_create (res);
    /* Sanity check to ensure that we found it */
    if (NULL == mca_btl_smlwda_component.sm_mpools[0]) {
        free(res);
        return OPAL_ERR_OUT_OF_RESOURCE;
    }

    mca_btl_smlwda_component.sm_mpool = mca_btl_smlwda_component.sm_mpools[0];

    mca_btl_smlwda_component.sm_mpool_base =
        mca_btl_smlwda_component.sm_mpools[0]->mpool_base(mca_btl_smlwda_component.sm_mpools[0]);

    /* create a list of peers */
    mca_btl_smlwda_component.sm_peers = (struct mca_btl_base_endpoint_t**)
        calloc(n, sizeof(struct mca_btl_base_endpoint_t*));
    if (NULL == mca_btl_smlwda_component.sm_peers) {
        free(res);
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
...
#if OPAL_LWDA_SUPPORT
    /* Register the entire shared memory region with the LWCA library which will
    ...
    opal_output_verbose(10, opal_btl_base_framework.framework_output,
                        "btl:smlwda: LWCA lwMemHostRegister address=%p, size=%d",
                        mca_btl_smlwda_component.sm_mpool_base, (int)res->size);
    mca_common_lwda_register(mca_btl_smlwda_component.sm_mpool_base, res->size, "smlwda");
    ...
    smlwda_btl->rcache = mca_rcache_base_module_create("gpusm", smlwda_btl, &rcache_res);
    if (NULL == smlwda_btl->rcache) {
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
#endif /* OPAL_LWDA_SUPPORT */
     ...
     * specified limits */
    if(mca_btl_smlwda_component.sm_max_procs > 0 &&
       mca_btl_smlwda_component.num_smp_procs + n >
       mca_btl_smlwda_component.sm_max_procs) {
        return OPAL_ERROR;
    }

    mca_btl_smlwda_component.shm_fifo = (volatile sm_fifo_t **)mca_btl_smlwda_component.sm_seg->module_data_addr;
    mca_btl_smlwda_component.shm_bases = (char**)(mca_btl_smlwda_component.shm_fifo + n);
    mca_btl_smlwda_component.shm_mem_nodes = (uint16_t*)(mca_btl_smlwda_component.shm_bases + n);

    /* set the base of the shared memory segment */
    mca_btl_smlwda_component.shm_bases[mca_btl_smlwda_component.my_smp_rank] =
        (char*)mca_btl_smlwda_component.sm_mpool_base;
    mca_btl_smlwda_component.shm_mem_nodes[mca_btl_smlwda_component.my_smp_rank] =
    ...
    mca_btl_smlwda_component.shm_fifo[mca_btl_smlwda_component.my_smp_rank] = my_fifos;

    /* cache the pointer to the 2d fifo array.  These addresses
     * are valid in the current process space */
    mca_btl_smlwda_component.fifo = (sm_fifo_t**)malloc(sizeof(sm_fifo_t*) * n);

    if(NULL == mca_btl_smlwda_component.fifo)
        return OPAL_ERR_OUT_OF_RESOURCE;

    mca_btl_smlwda_component.fifo[mca_btl_smlwda_component.my_smp_rank] = my_fifos;

    mca_btl_smlwda_component.mem_nodes = (uint16_t *) malloc(sizeof(uint16_t) * n);
    if(NULL == mca_btl_smlwda_component.mem_nodes)
        return OPAL_ERR_OUT_OF_RESOURCE;

    /* initialize fragment descriptor free lists */

    /* allocation will be for the fragment descriptor and payload buffer */
    length = sizeof(mca_btl_smlwda_frag1_t);
    length_payload =
        sizeof(mca_btl_smlwda_hdr_t) + mca_btl_smlwda_component.eager_limit;
    i = opal_free_list_init (&mca_btl_smlwda_component.sm_frags_eager, length,
                             opal_cache_line_size, OBJ_CLASS(mca_btl_smlwda_frag1_t),
                             length_payload, opal_cache_line_size,
                             mca_btl_smlwda_component.sm_free_list_num,
                             mca_btl_smlwda_component.sm_free_list_max,
                             mca_btl_smlwda_component.sm_free_list_inc,
                             mca_btl_smlwda_component.sm_mpool, 0, NULL, NULL, NULL);
    if ( OPAL_SUCCESS != i )
        return i;

    length = sizeof(mca_btl_smlwda_frag2_t);
    length_payload =
        sizeof(mca_btl_smlwda_hdr_t) + mca_btl_smlwda_component.max_frag_size;
    i = opal_free_list_init (&mca_btl_smlwda_component.sm_frags_max, length,
                             opal_cache_line_size, OBJ_CLASS(mca_btl_smlwda_frag2_t),
                             length_payload, opal_cache_line_size,
                             mca_btl_smlwda_component.sm_free_list_num,
                             mca_btl_smlwda_component.sm_free_list_max,
                             mca_btl_smlwda_component.sm_free_list_inc,
    ...
    i = opal_free_list_init (&mca_btl_smlwda_component.sm_frags_user,
                             sizeof(mca_btl_smlwda_user_t),
                             opal_cache_line_size, OBJ_CLASS(mca_btl_smlwda_user_t),
                             sizeof(mca_btl_smlwda_hdr_t), opal_cache_line_size,
                             mca_btl_smlwda_component.sm_free_list_num,
                             mca_btl_smlwda_component.sm_free_list_max,
                             mca_btl_smlwda_component.sm_free_list_inc,
                             mca_btl_smlwda_component.sm_mpool, 0, NULL, NULL, NULL);
    ...
    mca_btl_smlwda_component.num_outstanding_frags = 0;

    mca_btl_smlwda_component.num_pending_sends = 0;
    i = opal_free_list_init (&mca_btl_smlwda_component.pending_send_fl,
                             sizeof(btl_smlwda_pending_send_item_t), 8,
...
create_sm_endpoint(int local_proc, struct opal_proc_t *proc)
{
    ...
    if(NULL == ep)
        return NULL;
    ep->peer_smp_rank = local_proc + mca_btl_smlwda_component.num_smp_procs;
    ...
    if(ep->fifo_fd < 0) {
        opal_output(0, "mca_btl_smlwda_add_procs: open(%s) failed with errno=%d\n",
                    path, errno);
        free(ep);
        return NULL;
    }
    ...
    opal_bitmap_t* reachability)
{
    ...
    smlwda_btl = (mca_btl_smlwda_t *)btl;
    ...
    for (proc = 0; proc < (int32_t)nprocs; proc++) {
        ...
        if(my_proc == procs[proc]) {
            my_smp_rank = mca_btl_smlwda_component.my_smp_rank = n_local_procs++;
            continue;
        }
    ...
    if (!smlwda_btl->btl_inited) {
        return_code =
            smlwda_btl_first_time_init(smlwda_btl, my_smp_rank,
                                   mca_btl_smlwda_component.sm_max_procs);
     ...
     * rapid access and callwlate reachability */
    for(proc = 0; proc < (int32_t)nprocs; proc++) {
        if(NULL == peers[proc])
            continue;
        mca_btl_smlwda_component.sm_peers[peers[proc]->peer_smp_rank] = peers[proc];
        peers[proc]->my_smp_rank = my_smp_rank;
    }

    bases = mca_btl_smlwda_component.shm_bases;
    sm_mpool_modp = (mca_common_sm_mpool_module_t *)mca_btl_smlwda_component.sm_mpool;
     ...
     */
    for(j = mca_btl_smlwda_component.num_smp_procs;
        j < mca_btl_smlwda_component.num_smp_procs + FIFO_MAP_NUM(n_local_procs); j++) {

        return_code = sm_fifo_init( mca_btl_smlwda_component.fifo_size,
                                    mca_btl_smlwda_component.sm_mpool,
                                   &mca_btl_smlwda_component.fifo[my_smp_rank][j],
                                    mca_btl_smlwda_component.fifo_lazy_free);
        if(return_code != OPAL_SUCCESS)
            goto CLEANUP;
    }
     ...
     */
    (void)opal_atomic_add_fetch_32(&mca_btl_smlwda_component.sm_seg->module_seg->seg_inited, 1);
    while( n_local_procs >
           mca_btl_smlwda_component.sm_seg->module_seg->seg_inited) {
        opal_progress();
        opal_atomic_rmb();
    }
    ...
    if (0 == my_smp_rank) {
        if (OPAL_SUCCESS !=
            mca_common_sm_module_unlink(mca_btl_smlwda_component.sm_seg)) {
        ...
        if (-1 == unlink(mca_btl_smlwda_component.sm_mpool_rndv_file_name)) {
            opal_output(0, "WARNING: %s unlink failed.\n",
                        mca_btl_smlwda_component.sm_mpool_rndv_file_name);
        }
        if (-1 == unlink(mca_btl_smlwda_component.sm_rndv_file_name)) {
            opal_output(0, "WARNING: %s unlink failed.\n",
                        mca_btl_smlwda_component.sm_rndv_file_name);
        }
    }

    /* free up some space used by the name buffers */
    free(mca_btl_smlwda_component.sm_mpool_ctl_file_name);
    free(mca_btl_smlwda_component.sm_mpool_rndv_file_name);
    free(mca_btl_smlwda_component.sm_ctl_file_name);
    free(mca_btl_smlwda_component.sm_rndv_file_name);

    /* coordinate with other processes */
    for(j = mca_btl_smlwda_component.num_smp_procs;
        j < mca_btl_smlwda_component.num_smp_procs + n_local_procs; j++) {
        ptrdiff_t diff;

        /* spin until this element is allocated */
        /* doesn't really wait for that process... FIFO might be allocated, but not initialized */
        opal_atomic_rmb();
        while(NULL == mca_btl_smlwda_component.shm_fifo[j]) {
            opal_progress();
            opal_atomic_rmb();
        }

        /* Callwlate the difference as (my_base - their_base) */
        diff = ADDR2OFFSET(bases[my_smp_rank], bases[j]);

        /* store local address of remote fifos */
        mca_btl_smlwda_component.fifo[j] =
            (sm_fifo_t*)OFFSET2ADDR(diff, mca_btl_smlwda_component.shm_fifo[j]);

        /* cache local copy of peer memory node number */
        mca_btl_smlwda_component.mem_nodes[j] = mca_btl_smlwda_component.shm_mem_nodes[j];
    }

    /* update the local smp process count */
    mca_btl_smlwda_component.num_smp_procs += n_local_procs;

    /* make sure we have enough eager fragmnents for each process */
    return_code = opal_free_list_resize_mt (&mca_btl_smlwda_component.sm_frags_eager,
                                            mca_btl_smlwda_component.num_smp_procs * 2);
    if (OPAL_SUCCESS != return_code)
        goto CLEANUP;
...
int mca_btl_smlwda_finalize(struct mca_btl_base_module_t* btl)
{
    return OPAL_SUCCESS;
}
 ...
 */
int mca_btl_smlwda_register_error_cb(
        struct mca_btl_base_module_t* btl,
        mca_btl_base_module_error_cb_fn_t cbfunc)
{
    mca_btl_smlwda_t *smlwda_btl = (mca_btl_smlwda_t *)btl;
    smlwda_btl->error_cb = cbfunc;
    return OPAL_SUCCESS;
}
 ...
 */
extern mca_btl_base_descriptor_t* mca_btl_smlwda_alloc(
    struct mca_btl_base_module_t* btl,
    struct mca_btl_base_endpoint_t* endpoint,
    uint8_t order,
    size_t size,
    uint32_t flags)
{
    mca_btl_smlwda_frag_t* frag = NULL;
    if(size <= mca_btl_smlwda_component.eager_limit) {
        MCA_BTL_SMLWDA_FRAG_ALLOC_EAGER(frag);
    } else if (size <= mca_btl_smlwda_component.max_frag_size) {
        MCA_BTL_SMLWDA_FRAG_ALLOC_MAX(frag);
    }
 ...
 */
extern int mca_btl_smlwda_free(
    struct mca_btl_base_module_t* btl,
 ...
 * @param btl (IN)      BTL module
 */
struct mca_btl_base_descriptor_t* mca_btl_smlwda_prepare_src(
    struct mca_btl_base_module_t* btl,
    struct mca_btl_base_endpoint_t* endpoint,
    ...
    uint32_t flags)
{
    ...
    if ( reserve + max_data <= mca_btl_smlwda_component.eager_limit ) {
        MCA_BTL_SMLWDA_FRAG_ALLOC_EAGER(frag);
    } else {
        MCA_BTL_SMLWDA_FRAG_ALLOC_MAX(frag);
    }
    ...
    rc = opal_colwertor_pack(colwertor, &iov, &iov_count, &max_data );
...
#if 0
#define MCA_BTL_SMLWDA_TOUCH_DATA_TILL_CACHELINE_BOUNDARY(sm_frag)          \
    do {                                                                \
 ...
 */
int mca_btl_smlwda_sendi( struct mca_btl_base_module_t* btl,
                      struct mca_btl_base_endpoint_t* endpoint,
                      struct opal_colwertor_t* colwertor,
                      void* header,
                      size_t header_size,
                      ...
                      mca_btl_base_descriptor_t** descriptor )
{
    ...
    if (mca_common_lwda_enabled && (IPC_INIT == endpoint->ipcstate) && mca_btl_smlwda_component.use_lwda_ipc) {
        mca_btl_smlwda_send_lwda_ipc_request(btl, endpoint);
    }
    /* We do not want to use this path when we have LWCA IPC support */
    if ((colwertor->flags & COLWERTOR_LWDA) && (IPC_ACKED == endpoint->ipcstate)) {
        if (NULL != descriptor) {
            *descriptor = mca_btl_smlwda_alloc(btl, endpoint, order, payload_size+header_size, flags);
        }
    ...
    if( length < mca_btl_smlwda_component.eager_limit ) {
        ...
        */
        if( payload_size ) {
            ...
            (void)opal_colwertor_pack( colwertor, &iov, &iov_count, &max_data);

            assert(max_data == payload_size);
        }

        MCA_BTL_SMLWDA_TOUCH_DATA_TILL_CACHELINE_BOUNDARY(frag);
    ...
    /* presumably, this code path will never get exelwted */
    *descriptor = mca_btl_smlwda_alloc( btl, endpoint, order,
 ...
 * @param peer (IN)     BTL peer addressing
 */
int mca_btl_smlwda_send( struct mca_btl_base_module_t* btl,
                     struct mca_btl_base_endpoint_t* endpoint,
                     struct mca_btl_base_descriptor_t* descriptor,
                     mca_btl_base_tag_t tag )
{
    mca_btl_smlwda_frag_t* frag = (mca_btl_smlwda_frag_t*)descriptor;
    int rc;
...
static struct mca_btl_base_registration_handle_t *mca_btl_smlwda_register_mem (
    ...
    size_t size, uint32_t flags)
{
    mca_btl_smlwda_t *smlwda_module = (mca_btl_smlwda_t *) btl;
    mca_rcache_common_lwda_reg_t *reg;
    int access_flags = flags & MCA_BTL_REG_FLAG_ACCESS_ANY;
    int rcache_flags = 0;

    if (MCA_BTL_REG_FLAG_LWDA_GPU_MEM & flags) {
        rcache_flags |= MCA_RCACHE_FLAGS_LWDA_GPU_MEM;
    }

    smlwda_module->rcache->rcache_register (smlwda_module->rcache, base, size, rcache_flags,
...
{
    mca_btl_smlwda_t *smlwda_module = (mca_btl_smlwda_t *) btl;
    mca_rcache_common_lwda_reg_t *reg = (mca_rcache_common_lwda_reg_t *)
        ((intptr_t) handle - offsetof (mca_rcache_common_lwda_reg_t, data));

    smlwda_module->rcache->rcache_deregister (smlwda_module->rcache, &reg->base);

    return OPAL_SUCCESS;
}

int mca_btl_smlwda_get_lwda (struct mca_btl_base_module_t *btl,
...
{
    mca_rcache_common_lwda_reg_t rget_reg;
    mca_rcache_common_lwda_reg_t *reg_ptr = &rget_reg;
    int rc, done;
    ...
    MCA_BTL_SMLWDA_FRAG_ALLOC_USER(frag);
    if(OPAL_UNLIKELY(NULL == frag)) {
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    rc = mca_common_lwda_memcpy(local_address, remote_memory_address, size,
                                "mca_btl_smlwda_get", (mca_btl_base_descriptor_t *)frag,
                                &done);
    if (OPAL_SUCCESS != rc) {
        /* Out of resources can be handled by upper layers. */
        if (OPAL_ERR_OUT_OF_RESOURCE != rc) {
            opal_output(0, "Failed to lwMemcpy GPU memory, rc=%d", rc);
        }
        return rc;
    }
    ...
    if (OPAL_UNLIKELY(1 == done)) {
        ...
        mca_btl_smlwda_free(btl, (mca_btl_base_descriptor_t *)frag);
    }
...
/**
 * Send a LWCA IPC request message to the peer.  This indicates that this rank
 * is interested in establishing LWCA IPC support between this rank and GPU
                                                 ...
                                                 struct mca_btl_base_endpoint_t* endpoint)
{
    ...
    if (0 != (res = mca_common_lwda_get_device(&mydevnum))) {
    ...
    opal_output_verbose(10, mca_btl_smlwda_component.lwda_ipc_output,
                        "Sending LWCA IPC REQ (try=%d): myrank=%d, mydev=%d, peerrank=%d",
                        endpoint->ipctries,
 ...
 *
 */
void mca_btl_smlwda_dump(struct mca_btl_base_module_t* btl,
                     struct mca_btl_base_endpoint_t* endpoint,
                     int verbose)
{
    mca_btl_smlwda_frag_t* frag;
    ...
    if( NULL != endpoint ) {
        OPAL_LIST_FOREACH(frag, &endpoint->pending_sends, mca_btl_smlwda_frag_t) {
...
int mca_btl_smlwda_ft_event(int state) {
    return OPAL_SUCCESS;
}
#else
int mca_btl_smlwda_ft_event(int state) {
    /* Notify mpool */
    if( NULL != mca_btl_smlwda_component.sm_mpool &&
        NULL != mca_btl_smlwda_component.sm_mpool->mpool_ft_event) {
        mca_btl_smlwda_component.sm_mpool->mpool_ft_event(state);
    }

    if(OPAL_CRS_CHECKPOINT == state) {
        if( NULL != mca_btl_smlwda_component.sm_seg ) {
             ...
             * TODO: FIXIT soon
            orte_sstore.set_attr(orte_sstore_handle_lwrrent,
                                 SSTORE_METADATA_LOCAL_TOUCH,
                                 mca_btl_smlwda_component.sm_seg->shmem_ds.seg_name);
             */
        }
    }
        ...
        if (opal_cr_continue_like_restart) {
            if( NULL != mca_btl_smlwda_component.sm_seg ) {
                ...
                opal_crs_base_cleanup_append(mca_btl_smlwda_component.sm_seg->shmem_ds.seg_name, false);
            }
