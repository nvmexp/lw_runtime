 ...
 *                         All rights reserved.
 * Copyright (c) 2012-2013 LWPU Corporation.  All rights reserved.
 * $COPYRIGHT$
 *
 ...
 * @file
 */
#ifndef MCA_BTL_SMLWDA_H
#define MCA_BTL_SMLWDA_H
 ...
 * Shared Memory FIFOs
 *
 * The FIFO is implemented as a cirlwlar queue with head and tail pointers
...
typedef struct mca_btl_smlwda_mem_node_t {
    mca_mpool_base_module_t* sm_mpool; /**< shared memory pool */
} mca_btl_smlwda_mem_node_t;

/**
 * Shared Memory (SM) BTL module.
 */
struct mca_btl_smlwda_component_t {
...
#endif
    struct mca_btl_smlwda_t      **sm_btls;
    struct mca_btl_smlwda_frag_t **table;
    size_t sm_num_btls;
    size_t sm_max_btls;
    ...
    int lwda_ipc_verbose;
    int lwda_ipc_output;
    int use_lwda_ipc;
    int use_lwda_ipc_same_gpu;
#endif /* OPAL_LWDA_SUPPORT */
    unsigned long mpool_min_size;
    char *allocator;
};
typedef struct mca_btl_smlwda_component_t mca_btl_smlwda_component_t;
OPAL_MODULE_DECLSPEC extern mca_btl_smlwda_component_t mca_btl_smlwda_component;

/**
 * SM BTL Interface
 */
struct mca_btl_smlwda_t {
    mca_btl_base_module_t  super;       /**< base BTL interface */
    bool btl_inited;  /**< flag indicating if btl has been inited */
    mca_btl_base_module_error_cb_fn_t error_cb;
    mca_rcache_base_module_t *rcache;
};
typedef struct mca_btl_smlwda_t mca_btl_smlwda_t;
OPAL_MODULE_DECLSPEC extern mca_btl_smlwda_t mca_btl_smlwda;

struct btl_smlwda_pending_send_item_t
{
    opal_free_list_item_t super;
    void *data;
};
typedef struct btl_smlwda_pending_send_item_t btl_smlwda_pending_send_item_t;
 ...
 * virtual addresses.
 */
#define VIRTUAL2RELATIVE(VADDR ) ((long)(VADDR)  - (long)mca_btl_smlwda_component.shm_bases[mca_btl_smlwda_component.my_smp_rank])
#define RELATIVE2VIRTUAL(OFFSET) ((long)(OFFSET) + (long)mca_btl_smlwda_component.shm_bases[mca_btl_smlwda_component.my_smp_rank])
 ...
 */
extern int mca_btl_smlwda_component_progress(void);
...
extern int mca_btl_smlwda_finalize(
    struct mca_btl_base_module_t* btl
);
...
extern int mca_btl_smlwda_add_procs(
    struct mca_btl_base_module_t* btl,
    size_t nprocs,
 ...
 *
 */
extern int mca_btl_smlwda_del_procs(
    struct mca_btl_base_module_t* btl,
    size_t nprocs,
    struct opal_proc_t **procs,
    struct mca_btl_base_endpoint_t **peers
 ...
 * @param peer (IN)     BTL peer addressing
 */
extern int mca_btl_smlwda_sendi( struct mca_btl_base_module_t* btl,
                             struct mca_btl_base_endpoint_t* endpoint,
                             struct opal_colwertor_t* colwertor,
                             void* header,
                             size_t header_size,
 ...
 * @param peer (IN)     BTL peer addressing
 */
extern int mca_btl_smlwda_send(
    struct mca_btl_base_module_t* btl,
    struct mca_btl_base_endpoint_t* endpoint,
...
/* LWCA IPC control message */
typedef struct ctrlhdr_st {
        enum ipcCtrlMsg ctag;
        int lwdev;
} ctrlhdr_t;

/* State of setting up LWCA IPC on an endpoint */
enum ipcState {
    IPC_INIT = 1,
    IPC_SENT,
    IPC_ACKING,
...
extern void mca_btl_smlwda_dump(struct mca_btl_base_module_t* btl,
                            struct mca_btl_base_endpoint_t* endpoint,
                            int verbose);
...
#if OPAL_ENABLE_PROGRESS_THREADS == 1
void mca_btl_smlwda_component_event_thread(opal_object_t*);
#endif

#if OPAL_ENABLE_PROGRESS_THREADS == 1
#define MCA_BTL_SMLWDA_SIGNAL_PEER(peer) \
{ \
    unsigned char cmd = DATA; \
    if(write(peer->fifo_fd, &cmd, sizeof(cmd)) != sizeof(cmd)) { \
        opal_output(0, "mca_btl_smlwda_send: write fifo failed: errno=%d\n", errno); \
    } \
}
#else
#define MCA_BTL_SMLWDA_SIGNAL_PEER(peer)
