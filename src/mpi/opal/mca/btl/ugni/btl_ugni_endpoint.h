...
typedef struct mca_btl_base_endpoint_t {
    ...
    /** may need to lock relwrsively as the modex lookup could call opal_progress
     * and hence our progress function. if this changes modify this mutex to not
     * be relwrsive. also need to update the constructor function. */
    ...
    /** endpoint is lwrrently wait-listed for SMSG progress */
    bool wait_listed;
 ...
 * and multiple ugni modules. Lwrrently there is only one so we just use the
