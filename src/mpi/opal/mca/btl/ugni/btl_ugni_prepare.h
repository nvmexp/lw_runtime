                                        ...
                                        uint32_t flags)
{
    ...
    rc = opal_colwertor_pack (colwertor, &iov, &iov_count, size);
                               ...
                               uint32_t flags)
{
    ...
    send_in_place = (btl->btl_flags & MCA_BTL_FLAGS_SEND_INPLACE) && !(opal_colwertor_need_buffers(colwertor) ||
                                                                       (use_eager_get && ((uintptr_t)data_ptr & 3)));

    if (send_in_place) {
        return mca_btl_ugni_prepare_src_send_inplace (btl, endpoint, colwertor, order,
                                                      reserve, size, flags);
    } else {
        return mca_btl_ugni_prepare_src_send_buffered (btl, endpoint, colwertor, order,
                                                       reserve, size, flags);
    }
}
