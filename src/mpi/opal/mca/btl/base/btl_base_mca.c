                                ...
                                mca_btl_base_module_t *module)
{
    ...
    /* If no LWCA RDMA support, zero them out */
    if (!(MCA_BTL_FLAGS_LWDA_GET & module->btl_flags)) {
        module->btl_lwda_eager_limit = 0;
        module->btl_lwda_rdma_limit = SIZE_MAX;
    }
    (void) mca_base_component_var_register(version, "lwda_eager_limit", "Maximum size (in bytes, including header) of \"GPU short\" messages (must be >= 1).",
                                           MCA_BASE_VAR_TYPE_SIZE_T, NULL, 0, 0,
                                           OPAL_INFO_LVL_5,
                                           MCA_BASE_VAR_SCOPE_READONLY,
                                           &module->btl_lwda_eager_limit);
    (void) mca_base_component_var_register(version, "lwda_rdma_limit", "Size (in bytes, including header) of GPU buffer when switch to rndv protocol and pipeline.",
                                           MCA_BASE_VAR_TYPE_SIZE_T, NULL, 0, 0,
                                           OPAL_INFO_LVL_5,
                                           MCA_BASE_VAR_SCOPE_READONLY,
                                           &module->btl_lwda_rdma_limit);
#endif /* OPAL_LWDA_GDR_SUPPORT */
#if OPAL_LWDA_SUPPORT
    module->btl_lwda_max_send_size = 0;
    (void) mca_base_component_var_register(version, "lwda_max_send_size", "Maximum size (in bytes) of a single GPU \"phase 2\" fragment of a long message when using the pipeline protocol (must be >= 1) (only valid on smlwda btl)",
                                           MCA_BASE_VAR_TYPE_SIZE_T, NULL, 0, 0,
                                           OPAL_INFO_LVL_4,
                                           MCA_BASE_VAR_SCOPE_READONLY,
                                           &module->btl_lwda_max_send_size);
