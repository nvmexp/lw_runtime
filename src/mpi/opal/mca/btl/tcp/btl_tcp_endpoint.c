 ...
 * displayed might be inalwrate, but when we manipulate the pending fragments we
                          ...
                          const char* msg)
{
    ...
    if( NULL != btl_endpoint->endpoint_send_frag )
        used += mca_btl_tcp_frag_dump(btl_endpoint->endpoint_send_frag, "active send (inaclwrate iov)",
                                      &outmsg[used], DEBUG_LENGTH - used);
    if (used >= DEBUG_LENGTH) goto out;
...
static void mca_btl_tcp_endpoint_recv_handler(int sd, short flags, void* user)
{
    ...
    case MCA_BTL_TCP_CLOSED:
         ...
         * threads exelwting the receive callback, when we reach the end
