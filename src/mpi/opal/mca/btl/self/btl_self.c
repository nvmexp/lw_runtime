...
{
    bool inline_send = !opal_colwertor_need_buffers(colwertor);
...
static int mca_btl_self_sendi (struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                               struct opal_colwertor_t *colwertor, void *header, size_t header_size,
                               size_t payload_size, uint8_t order, uint32_t flags, mca_btl_base_tag_t tag,
                               mca_btl_base_descriptor_t **descriptor)
{
    mca_btl_base_descriptor_t *frag;

    if (!payload_size || !opal_colwertor_need_buffers(colwertor)) {
        void *data_ptr = NULL;
        if (payload_size) {
            opal_colwertor_get_lwrrent_pointer (colwertor, &data_ptr);
        }
    ...
    frag = mca_btl_self_prepare_src (btl, endpoint, colwertor, order, header_size, &payload_size,
