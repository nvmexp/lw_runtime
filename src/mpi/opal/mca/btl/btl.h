 ...
 * possible the BTL should support the use of both modes conlwrrently.
 *
 */
...
/**
 * Opaque registration handle for exelwting RDMA and atomic
...
#define MCA_BTL_TAG_SMLWDA            (MCA_BTL_TAG_BTL + 2)
...
/* btl transport reliability flags - lwrrently used only by the DR PML */
...
#define MCA_BTL_FLAGS_LWDA_PUT        0x0400
#define MCA_BTL_FLAGS_LWDA_GET        0x0800
#define MCA_BTL_FLAGS_LWDA_RDMA (MCA_BTL_FLAGS_LWDA_GET|MCA_BTL_FLAGS_LWDA_PUT)
#define MCA_BTL_FLAGS_LWDA_COPY_ASYNC_SEND 0x1000
#define MCA_BTL_FLAGS_LWDA_COPY_ASYNC_RECV 0x2000
...
#define MCA_BTL_ERROR_FLAGS_ADD_LWDA_IPC 0x4
...
enum {
    ...
    MCA_BTL_REG_FLAG_LWDA_GPU_MEM  = 0x00010000,
#endif
};
 ...
 */
#define MCA_BTL_DES_FLAGS_LWDA_COPY_ASYNC   0x0008
 ...
 * indicates whether multiple threads may ilwoke this component
 ...
 * non-threaded polling elwironments.
 *
...
/**
 * Prepare a descriptor for send using the supplied colwertor. If the colwertor
 ...
 * @param colwertor (IN)    Data type colwertor
 * @param order (IN)        The ordering tag (may be MCA_BTL_NO_ORDER)
 ...
 *
 * All BTLs allow multiple conlwrrent asynchronous send operations on a descriptor
 *
 * @param btl (IN)         BTL module
 ...
 *  to send the header and "size" bytes from the datatype using the colwertor.
 ...
 * @param payload_size (IN)    Size of payload (from colwertor).
 * @param order (IN)           The ordering tag (may be MCA_BTL_NO_ORDER)
...
#if OPAL_LWDA_GDR_SUPPORT
    size_t      btl_lwda_eager_limit;  /**< switch from eager to RDMA */
    size_t      btl_lwda_rdma_limit;   /**< switch from RDMA to rndv pipeline */
#endif /* OPAL_LWDA_GDR_SUPPORT */
#if OPAL_LWDA_SUPPORT
    size_t      btl_lwda_max_send_size;   /**< set if LWCA max send_size is different from host max send size */
...
/**
 * Colwinience macro for detecting the BTL interface version.
 */
