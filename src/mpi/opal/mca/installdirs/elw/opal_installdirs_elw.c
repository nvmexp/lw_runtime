...
static int installdirs_elw_open(void);


opal_installdirs_base_component_t mca_installdirs_elw_component = {
...
#define SET_FIELD(field, elwname)                                         \
    do {                                                                  \
        char *tmp = getelw(elwname);                                      \
         if (NULL != tmp && 0 == strlen(tmp)) {                           \
             tmp = NULL;                                                  \
         }                                                                \
         mca_installdirs_elw_component.install_dirs_data.field = tmp;     \
    } while (0)


static int
installdirs_elw_open(void)
{
