...
static int _intercept_brk (void *addr)
{
    ...
    if (!original_brk) {
        /* get the lwrrent_addr */
        new_addr = (void *) (intptr_t) memory_patcher_syscall(SYS_brk, addr);

#ifdef HAVE___LWRBRK
        /*
         * Note: if we were using glibc brk/sbrk, their __lwrbrk would get
         * updated, but since we're going straight to the syscall, we have
         * to update __lwrbrk or else glibc won't see it.
         */
        __lwrbrk = new_addr;
#endif
    } else {
...
static int patcher_open (void)
{
    static int was_exelwted_already = 0;
    int rc;

    if (was_exelwted_already) {
        return OPAL_SUCCESS;
    }

    was_exelwted_already = 1;
