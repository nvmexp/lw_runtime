...
/**
 * Search through a path of directories, ilwoking a callback on each
 * unique regular (non-Libtool) file basename found (e.g., will only
 * be ilwoked once for the files "foo.la" and "foo.so", with the
 * parameter "foo").
 *
 * Arguments:
 *   path   = OPAL_ELW_SEP-delimited list of directories
 *   cb_func= function to ilwoke on each filename found
