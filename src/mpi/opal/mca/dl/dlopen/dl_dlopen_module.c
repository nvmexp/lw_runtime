...
/*
 * Scan all the files in a directory (or path) and ilwoke a callback
 * on each one.
 */
                              ...
                              void *data)
{
    ...
    dirs = opal_argv_split(search_path, OPAL_ELW_SEP);
    ...
    /* Ilwoke the callback on all the found files */
    if (NULL != good_files) {
