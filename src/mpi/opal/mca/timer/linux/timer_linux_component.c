...
static int opal_timer_linux_find_freq(void)
{
        ...
        if (NULL != loc) {
            ...
            if (1 == ret) {
                /* number is in MHz * 2 and has 2 decimal digits
                   colwert to Hz and make an integer */
    ...
    if (0 == opal_timer_linux_freq) {
        ...
        if (NULL != loc) {
            ret = sscanf(loc, "%f", &cpu_f);
            if (1 == ret) {
                /* numer is in MHz - colwert to Hz and make an integer */
    ...
    /* colwert the timer frequency to MHz to avoid an extra operation when
     * colwerting from cycles to usec */
