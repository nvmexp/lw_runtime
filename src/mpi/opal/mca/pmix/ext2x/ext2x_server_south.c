...
int ext2x_server_setup_fork(const opal_process_name_t *proc, char ***elw)
{
    pmix_status_t rc;
    pmix_proc_t p;
                               ...
                               opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == source) {
        ...
        op->p.rank = ext2x_colwert_opalrank(OPAL_VPID_ILWALID);
    } else {
        (void)opal_snprintf_jobid(op->p.nspace, PMIX_MAX_NSLEN, source->jobid);
        op->p.rank = ext2x_colwert_opalrank(source->vpid);
    }
