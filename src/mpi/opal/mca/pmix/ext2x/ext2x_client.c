...
int ext2x_client_init(opal_list_t *ilist)
{
    ...
    if (NULL != getelw(OPAL_MCA_PREFIX"orte_launch")) {
        ...
        opal_colwert_string_to_jobid(&pname.jobid, my_proc.nspace);
    } else {
    ...
    pname.vpid = ext2x_colwert_rank(my_proc.rank);
                 ...
                 opal_list_t *procs)
{
    ...
    if (NULL != procs && 0 < (cnt = opal_list_get_size(procs))) {
        ...
        OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
            if (NULL == (nsptr = ext2x_colwert_jobid(ptr->name.jobid))) {
                PMIX_PROC_FREE(parray, cnt);
                return OPAL_ERR_NOT_FOUND;
            }
            (void)strncpy(parray[n].nspace, nsptr, PMIX_MAX_NSLEN);
            parray[n].rank = ext2x_colwert_opalrank(ptr->name.vpid);
            ++n;
        }
    }
...
int ext2x_store_local(const opal_process_name_t *proc, opal_value_t *val)
{
    ...
    if (NULL != proc) {
        if (NULL == (nsptr = ext2x_colwert_jobid(proc->jobid))) {
            job = OBJ_NEW(opal_ext2x_jobid_trkr_t);
        ...
        p.rank = ext2x_colwert_opalrank(proc->vpid);
    } else {
        /* use our name */
        (void)strncpy(p.nspace, my_proc.nspace, PMIX_MAX_NSLEN);
        p.rank = ext2x_colwert_opalrank(OPAL_PROC_MY_NAME.vpid);
    }
...
{
    pmix_value_t kv;
    pmix_scope_t pmix_scope = ext2x_colwert_opalscope(opal_scope);
    pmix_status_t rc;
               ...
               opal_list_t *info, opal_value_t **val)
{
    ...
    if (NULL == proc && NULL != key) {
        ...
        if (0 == strcmp(key, OPAL_PMIX_RANK)) {
            ...
            (*val)->key = strdup(key);
            (*val)->type = OPAL_INT;
            (*val)->data.integer = ext2x_colwert_rank(my_proc.rank);
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_SUCCESS;
        }
    }
    *val = NULL;
    ...
    if (NULL == proc) {
        (void)strncpy(p.nspace, my_proc.nspace, PMIX_MAX_NSLEN);
        p.rank = ext2x_colwert_rank(PMIX_RANK_WILDCARD);
    } else {
        if (NULL == (nsptr = ext2x_colwert_jobid(proc->jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
                       ...
                       pmix_value_t *kv, void *cbdata)
{
    ...
    rc = ext2x_colwert_opalrc(status);
                 ...
                 opal_pmix_value_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == proc) {
        ...
        op->p.rank = ext2x_colwert_rank(PMIX_RANK_WILDCARD);
    } else {
        if (NULL == (nsptr = ext2x_colwert_jobid(proc->jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
        ...
        op->p.rank = ext2x_colwert_opalrank(proc->vpid);
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
...
int ext2x_lookup(opal_list_t *data, opal_list_t *info)
{
    ...
    if (PMIX_SUCCESS == rc) {
        ...
        OPAL_LIST_FOREACH(d, data, opal_pmix_pdata_t) {
            ...
            d->proc.vpid = ext2x_colwert_rank(pdata[n].proc.rank);
                      ...
                      void *cbdata)
{
    ...
    if (OPAL_SUCCESS == rc) {
        ...
        for (n=0; n < ndata; n++) {
            ...
            d->proc.vpid = ext2x_colwert_rank(data[n].proc.rank);
...
int ext2x_connect(opal_list_t *procs)
{
    ...
    OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
        ...
        p[n].rank = ext2x_colwert_opalrank(ptr->name.vpid);
        ++n;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
                     ...
                     void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
        ...
        op->procs[n].rank = ext2x_colwert_opalrank(ptr->name.vpid);
        ++n;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
                         ...
                         opal_list_t *procs)
{
    ...
    if (OPAL_JOBID_WILDCARD != jobid) {
        if (NULL == (nspace = ext2x_colwert_jobid(jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
        }
    } else {
    ...
    if (NULL != array && 0 < nprocs) {
        ...
        for (n=0; n < nprocs; n++) {
            ...
            /* if we don't already have it, add this to our jobid tracker */
            if (NULL == ext2x_colwert_jobid(nm->name.jobid)) {
                job = OBJ_NEW(opal_ext2x_jobid_trkr_t);
                (void)strncpy(job->nspace, array[n].nspace, PMIX_MAX_NSLEN);
                job->jobid = nm->name.jobid;
                opal_list_append(&mca_pmix_ext2x_component.jobids, &job->super);
            }
            nm->name.vpid = ext2x_colwert_rank(array[n].rank);
        }
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
    }
...
int ext2x_resolve_nodes(opal_jobid_t jobid, char **nodelist)
{
    ...
    if (NULL == (nsptr = ext2x_colwert_jobid(jobid))) {
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
        return OPAL_ERR_NOT_FOUND;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
