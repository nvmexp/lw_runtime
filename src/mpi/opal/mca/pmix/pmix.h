 ...
 * callback function will be exelwted once the specified data has been _PMIx_Put_
 ...
 *
 * Note: Some host elwironments may support user/group level
 ...
 * wait for _all_ requested data before exelwting the callback
 ...
 * in multiple connect operations, each ilwolving a different set of ranges */
...
typedef int (*opal_pmix_base_module_server_setup_fork_fn_t)(const opal_process_name_t *proc, char ***elw);
 ...
 * colwenience library to notify all registered local procs of
 ...
 * data until the callback function has been exelwted if this
 ...
 * any application-specific elwironmental variables prior to
 ...
 * returned elw will be distributed along with the application */
 ...
 * have been queued, but may not have been transmitted by this
 * time. Note that the caller is required to maintain the input
 * data until the callback function has been exelwted if it
