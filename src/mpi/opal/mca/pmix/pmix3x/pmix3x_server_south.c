...
static void lkcbfunc(pmix_status_t status, void *cbdata)
{
    ...
    OPAL_POST_OBJECT(lk);
    lk->status = pmix3x_colwert_rc(status);
    ...
    if (NULL != info && 0 < (sz = opal_list_get_size(info))) {
        ...
        OPAL_LIST_FOREACH(kv, info, opal_value_t) {
            ...
            if (0 == strcmp(kv->key, OPAL_PMIX_PROC_DATA)) {
                ...
                /* the value contains a list of values - colwert
...
int pmix3x_server_setup_fork(const opal_process_name_t *proc, char ***elw)
{
    pmix_status_t rc;
    pmix_proc_t p;
    ...
    rc = PMIx_server_setup_fork(&p, elw);
    return pmix3x_colwert_rc(rc);
}
                               ...
                               opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL != info && 0 < (sz = opal_list_get_size(info))) {
        ...
        OPAL_LIST_FOREACH(kv, info, opal_value_t) {
            ...
            if (0 == strcmp(kv->key, OPAL_PMIX_JOB_TERM_STATUS)) {
                ...
                pinfo[n].value.data.status = pmix3x_colwert_opalrc(kv->data.integer);
            } else {
                pmix3x_value_load(&pinfo[n].value, kv);
            }
            ++n;
    ...
    /* colwert the jobid */
    if (NULL == source) {
        (void)opal_snprintf_jobid(op->p.nspace, PMIX_MAX_NSLEN, OPAL_JOBID_ILWALID);
        op->p.rank = pmix3x_colwert_opalrank(OPAL_VPID_ILWALID);
    } else {
        (void)opal_snprintf_jobid(op->p.nspace, PMIX_MAX_NSLEN, source->jobid);
        op->p.rank = pmix3x_colwert_opalrank(source->vpid);
    }
                         ...
                         pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (OPAL_SUCCESS == rc && NULL != info) {
        /* need to colwert the info array to a list */
        for (n=0; n < ninfo; n++) {
