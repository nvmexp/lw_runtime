...
static void opal_opcbfunc(int status, void *cbdata)
{
    ...
    if (NULL != opalcaddy->opcbfunc) {
        opalcaddy->opcbfunc(pmix3x_colwert_opalrc(status), opalcaddy->cbdata);
    }
                                                ...
                                                pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    proc.vpid = pmix3x_colwert_rank(p->rank);
                                     ...
                                     pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    for (n=0; n < nprocs; n++) {
        ...
        nm->name.vpid = pmix3x_colwert_rank(procs[n].rank);
    }
    ...
    if (NULL != opalcaddy->lkupcbfunc) {
        ...
        if (NULL != data) {
            ...
            OPAL_LIST_FOREACH(p, data, opal_pmix_pdata_t) {
                ...
                d[n].proc.rank = pmix3x_colwert_opalrank(p->proc.vpid);
                                         ...
                                         pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    /* colwert the code */
    status = pmix3x_colwert_rc(code);

    /* colwert the source */
    if (OPAL_SUCCESS != (rc = opal_colwert_string_to_jobid(&src.jobid, source->nspace))) {
        OBJ_RELEASE(opalcaddy);
        return pmix3x_colwert_opalrc(rc);
    }
    src.vpid = pmix3x_colwert_rank(source->rank);
                        ...
                        void *release_cbdata)
{
    ...
    /* colwert the status */
    pcaddy->status = pmix3x_colwert_opalrc(status);

    /* colwert the list to a pmix_info_t array */
                                  ...
                                  void *cbdata)
{
    ...
    /* colwert the requestor */
    if (OPAL_SUCCESS != (rc = opal_colwert_string_to_jobid(&requestor.jobid, proct->nspace))) {
        OBJ_RELEASE(opalcaddy);
        return pmix3x_colwert_opalrc(rc);
    }
    requestor.vpid = pmix3x_colwert_rank(proct->rank);
    ...
    for (n=0; n < nqueries; n++) {
        ...
        for (m=0; m < queries[n].nqual; m++) {
            ...
            if (0 == strcmp(queries[n].qualifiers[m].key, PMIX_NSPACE)) {
                /* must colwert this to jobid */
                oinfo->key = strdup(OPAL_PMIX_PROCID);
                if (OPAL_SUCCESS != (rc = opal_colwert_string_to_jobid(&oinfo->data.name.jobid, queries[n].qualifiers[m].value.data.string))) {
                    OBJ_RELEASE(opalcaddy);
                    return pmix3x_colwert_opalrc(rc);
                }
            } else {
                                   ...
                                   void *cbdata)
{
    ...
    for (n=0; n < ninfo; n++) {
        ...
        if (0 == strncmp(oinfo->key, PMIX_NSPACE, PMIX_MAX_KEYLEN)) {
            ...
            if (!found) {
                if (OPAL_SUCCESS != (rc = opal_colwert_string_to_jobid(&oinfo->data.name.jobid, info[n].value.data.proc->nspace))) {
                    OPAL_ERROR_LOG(rc);
                    OBJ_RELEASE(opalcaddy);
                    err = pmix3x_colwert_opalrc(rc);
                    if (NULL != cbfunc) {
                                     ...
                                     pmix_info_cbfunc_t cbfunc, void *cbdata)
{
    ...
    /* colwert the directive */
    odir = pmix3x_colwert_allocdir(directive);
                                        ...
                                        pmix_info_cbfunc_t cbfunc, void *cbdata)
{
    ...
    for (n=0; n < ntargets; n++) {
        nm = OBJ_NEW(opal_namelist_t);
        opal_list_append(&opalcaddy->procs, &nm->super);
        if (OPAL_SUCCESS != (rc = opal_colwert_string_to_jobid(&nm->name.jobid, targets[n].nspace))) {
            OBJ_RELEASE(opalcaddy);
            return pmix3x_colwert_opalrc(rc);
        }
        nm->name.vpid = pmix3x_colwert_rank(targets[n].rank);
    }
