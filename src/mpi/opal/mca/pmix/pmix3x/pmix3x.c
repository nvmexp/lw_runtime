...
static void opcbfunc(pmix_status_t status, void *cbdata)
{
    ...
    if (NULL != op->opcbfunc) {
        op->opcbfunc(pmix3x_colwert_rc(status), op->cbdata);
    }
    OBJ_RELEASE(op);
}
                                    ...
                                    void *notification_cbdata)
{
    ...
    if (NULL != cd->pmixcbfunc) {
        ...
        /* colwert the status */
        pstatus = pmix3x_colwert_opalrc(status);
                       ...
                       void *cbdata)
{
    ...
    cd->status = pmix3x_colwert_rc(status);
    opal_output_verbose(2, opal_pmix_base_framework.framework_output,
    ...
    if (NULL == source) {
        ...
        cd->pname.vpid = pmix3x_colwert_rank(source->rank);
    }
     ...
     * handler ilwoke a PMIx function */
...
static int pmix3x_register_cleanup(char *path, bool directory, bool ignore, bool jobscope)
{
    ...
    if (ignore) {
        ...
        if (directory) {
            ...
            ++ninfo;
            /* relwrsively cleanup directories */
            PMIX_INFO_LOAD(&pinfo[ninfo], PMIX_CLEANUP_RELWRSIVE, NULL, PMIX_BOOL);
            ++ninfo;
        } else {
...
opal_vpid_t pmix3x_colwert_rank(pmix_rank_t rank)
{
    switch(rank) {
...
pmix_rank_t pmix3x_colwert_opalrank(opal_vpid_t vpid)
{
    switch(vpid) {
...
pmix_status_t pmix3x_colwert_opalrc(int rc)
{
    switch (rc) {
...
int pmix3x_colwert_rc(pmix_status_t rc)
{
    switch (rc) {
...
opal_pmix_scope_t pmix3x_colwert_scope(pmix_scope_t scope)
{
    switch(scope) {
...
pmix_scope_t pmix3x_colwert_opalscope(opal_pmix_scope_t scope) {
    switch(scope) {
...
pmix_data_range_t pmix3x_colwert_opalrange(opal_pmix_data_range_t range) {
    switch(range) {
...
opal_pmix_data_range_t pmix3x_colwert_range(pmix_data_range_t range) {
    switch(range) {
...
opal_pmix_persistence_t pmix3x_colwert_persist(pmix_persistence_t persist)
{
    switch(persist) {
...
pmix_persistence_t pmix3x_colwert_opalpersist(opal_pmix_persistence_t persist)
{
    switch(persist) {
...
char* pmix3x_colwert_jobid(opal_jobid_t jobid)
{
    opal_pmix3x_jobid_trkr_t *jptr;
                       ...
                       opal_value_t *kv)
{
    ...
    switch(kv->type) {
        ...
        case OPAL_STATUS:
            v->type = PMIX_STATUS;
            v->data.status = pmix3x_colwert_opalrc(kv->data.status);
            break;
        case OPAL_VPID:
            v->type = PMIX_PROC_RANK;
            v->data.rank = pmix3x_colwert_opalrank(kv->data.name.vpid);
            break;
        case OPAL_NAME:
            v->type = PMIX_PROC;
            ...
            v->data.proc->rank = pmix3x_colwert_opalrank(kv->data.name.vpid);
            break;
        ...
        case OPAL_PERSIST:
            v->type = PMIX_PERSIST;
            v->data.persist = pmix3x_colwert_opalpersist((opal_pmix_persistence_t)kv->data.uint8);
            break;
        case OPAL_SCOPE:
            v->type = PMIX_SCOPE;
            v->data.scope = pmix3x_colwert_opalscope((opal_pmix_scope_t)kv->data.uint8);
            break;
        case OPAL_DATA_RANGE:
            v->type = PMIX_DATA_RANGE;
            v->data.range = pmix3x_colwert_opalrange((opal_pmix_data_range_t)kv->data.uint8);
            break;
        ...
        case OPAL_PROC_INFO:
            ...
            v->data.pinfo->proc.rank = pmix3x_colwert_opalrank(kv->data.pinfo.name.vpid);
            if (NULL != kv->data.pinfo.hostname) {
                v->data.pinfo->hostname = strdup(kv->data.pinfo.hostname);
            }
            if (NULL != kv->data.pinfo.exelwtable_name) {
                v->data.pinfo->exelwtable_name = strdup(kv->data.pinfo.exelwtable_name);
            }
            v->data.pinfo->pid = kv->data.pinfo.pid;
            v->data.pinfo->exit_code = kv->data.pinfo.exit_code;
            v->data.pinfo->state = pmix3x_colwert_opalstate(kv->data.pinfo.state);
            break;
        case OPAL_ELWAR:
            v->type = PMIX_ELWAR;
            PMIX_ELWAR_CONSTRUCT(&v->data.elwar);
            if (NULL != kv->data.elwar.elwar) {
                v->data.elwar.elwar = strdup(kv->data.elwar.elwar);
            }
            if (NULL != kv->data.elwar.value) {
                v->data.elwar.value = strdup(kv->data.elwar.value);
            }
            v->data.elwar.separator = kv->data.elwar.separator;
            break;
        default:
            /* silence warnings */
            break;
                        ...
                        const pmix_value_t *v)
{
    ...
    case PMIX_STATUS:
        kv->type = OPAL_STATUS;
        kv->data.status = pmix3x_colwert_rc(v->data.status);
        break;
    case PMIX_VALUE:
    ...
    case PMIX_PROC:
        ...
        if (!found) {
            if (OPAL_SUCCESS != (rc = opal_colwert_string_to_jobid(&kv->data.name.jobid, v->data.proc->nspace))) {
                return pmix3x_colwert_opalrc(rc);
            }
        }
        kv->data.name.vpid = pmix3x_colwert_rank(v->data.proc->rank);
        break;
    case PMIX_APP:
        OPAL_ERROR_LOG(OPAL_ERR_NOT_SUPPORTED);
        rc = OPAL_ERR_NOT_SUPPORTED;
    ...
    case PMIX_PERSIST:
        ...
        kv->data.uint8 = pmix3x_colwert_persist(v->data.persist);
        break;
    case PMIX_POINTER:
        kv->type = OPAL_PTR;
    ...
    case PMIX_SCOPE:
        kv->type = OPAL_SCOPE;
        kv->data.uint8 = pmix3x_colwert_scope(v->data.scope);
        break;
    case PMIX_DATA_RANGE:
        kv->type = OPAL_DATA_RANGE;
        kv->data.uint8 = pmix3x_colwert_range(v->data.range);
        break;
    case PMIX_COMMAND:
        OPAL_ERROR_LOG(OPAL_ERR_NOT_SUPPORTED);
        rc = OPAL_ERR_NOT_SUPPORTED;
    ...
    case PMIX_PROC_INFO:
        ...
        if (!found) {
            if (OPAL_SUCCESS != (rc = opal_colwert_string_to_jobid(&kv->data.pinfo.name.jobid, v->data.pinfo->proc.nspace))) {
                return pmix3x_colwert_opalrc(rc);
            }
        }
        kv->data.pinfo.name.vpid = pmix3x_colwert_rank(v->data.pinfo->proc.rank);
        if (NULL != v->data.pinfo->hostname) {
            kv->data.pinfo.hostname = strdup(v->data.pinfo->hostname);
        }
        if (NULL != v->data.pinfo->exelwtable_name) {
            kv->data.pinfo.exelwtable_name = strdup(v->data.pinfo->exelwtable_name);
        }
        kv->data.pinfo.pid = v->data.pinfo->pid;
        kv->data.pinfo.exit_code = v->data.pinfo->exit_code;
        kv->data.pinfo.state = pmix3x_colwert_state(v->data.pinfo->state);
        break;
    ...
    case PMIX_PROC_RANK:
        kv->type = OPAL_VPID;
        kv->data.name.vpid = pmix3x_colwert_rank(v->data.rank);
        break;
    case PMIX_QUERY:
        OPAL_ERROR_LOG(OPAL_ERR_NOT_SUPPORTED);
        rc = OPAL_ERR_NOT_SUPPORTED;
    ...
    case PMIX_ELWAR:
        kv->type = OPAL_ELWAR;
        OBJ_CONSTRUCT(&kv->data.elwar, opal_elwar_t);
        if (NULL != v->data.elwar.elwar) {
            kv->data.elwar.elwar = strdup(v->data.elwar.elwar);
        }
        if (NULL != v->data.elwar.value) {
            kv->data.elwar.value = strdup(v->data.elwar.value);
        }
        kv->data.elwar.separator = v->data.elwar.separator;
        break;
    default:
        /* silence warnings */
        rc = OPAL_ERROR;
                           ...
                           void *cbdata)
{
    ...
    if (NULL != op->evregcbfunc) {
        op->evregcbfunc(pmix3x_colwert_rc(status), errhandler_ref, op->cbdata);
    }
    OBJ_RELEASE(op);
}
                             ...
                             void *cbdata)
{
    ...
    if (NULL != event_codes) {
        ...
        OPAL_LIST_FOREACH(kv, event_codes, opal_value_t) {
            op->pcodes[n] = pmix3x_colwert_opalrc(kv->data.integer);
            ++n;
        }
    }
                        ...
                        opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == source) {
        ...
        if (NULL == (nsptr = pmix3x_colwert_jobid(source->jobid))) {
            OBJ_RELEASE(op);
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
        }
        (void)strncpy(p.nspace, nsptr, PMIX_MAX_NSLEN);
        p.rank = pmix3x_colwert_opalrank(source->vpid);
        pptr = &p;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);

    /* colwert the range */
    prange = pmix3x_colwert_opalrange(range);
    ...
    if (NULL != info && 0 < (op->ninfo = opal_list_get_size(info))) {
        ...
        OPAL_LIST_FOREACH(kv, info, opal_value_t) {
            ...
            if (0 == strcmp(kv->key, OPAL_PMIX_JOB_TERM_STATUS)) {
                ...
                op->info[n].value.data.status = pmix3x_colwert_opalrc(kv->data.integer);
            } else {
                pmix3x_value_load(&op->info[n].value, kv);
            }
            ++n;
    ...
    return pmix3x_colwert_rc(pstatus);
}
...
opal_pmix_alloc_directive_t pmix3x_colwert_allocdir(pmix_alloc_directive_t dir)
{
    switch (dir) {
        case PMIX_ALLOC_NEW:
...
int pmix3x_colwert_state(pmix_proc_state_t state)
{
    switch(state) {
...
pmix_proc_state_t pmix3x_colwert_opalstate(int state)
{
    switch(state) {
        case 0:
