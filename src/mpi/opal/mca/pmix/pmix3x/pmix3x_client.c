...
int pmix3x_client_init(opal_list_t *ilist)
{
    ...
    /* colwert the incoming list to info structs */
    ...
    if (NULL != getelw(OPAL_MCA_PREFIX"orte_launch")) {
        ...
        opal_colwert_string_to_jobid(&pname.jobid, mca_pmix_pmix3x_component.myproc.nspace);
    } else {
    ...
    pname.vpid = pmix3x_colwert_rank(mca_pmix_pmix3x_component.myproc.rank);
...
int pmix3x_tool_init(opal_list_t *info)
{
    ...
    pmix_status_t rc;
    int ret;
    opal_process_name_t pname = {OPAL_JOBID_ILWALID, OPAL_VPID_ILWALID};
    opal_pmix3x_event_t *event;
    ...
    if (NULL != info && 0 < (ninfo = opal_list_get_size(info))) {
        ...
        OPAL_LIST_FOREACH(val, info, opal_value_t) {
            ...
            if (0 == strcmp(val->key, OPAL_PMIX_TOOL_NSPACE)) {
                opal_colwert_string_to_jobid(&pname.jobid, val->data.string);
     ...
     * so mark as native launch so we don't colwert back/forth */
    mca_pmix_pmix3x_component.native_launch = true;
...
int pmix3x_store_local(const opal_process_name_t *proc, opal_value_t *val)
{
    ...
    if (NULL != proc) {
        if (NULL == (nsptr = pmix3x_colwert_jobid(proc->jobid))) {
            job = OBJ_NEW(opal_pmix3x_jobid_trkr_t);
        ...
        p.rank = pmix3x_colwert_opalrank(proc->vpid);
    } else {
        /* use our name */
        (void)strncpy(p.nspace, mca_pmix_pmix3x_component.myproc.nspace, PMIX_MAX_NSLEN);
        p.rank = pmix3x_colwert_opalrank(OPAL_PROC_MY_NAME.vpid);
    }
...
{
    pmix_value_t kv;
    pmix_scope_t pmix_scope = pmix3x_colwert_opalscope(opal_scope);
    pmix_status_t rc;
               ...
               opal_list_t *info, opal_value_t **val)
{
    ...
    if (NULL == proc) {
        ...
        if (0 == strcmp(key, OPAL_PMIX_RANK)) {
            ...
            (*val)->key = strdup(key);
            (*val)->type = OPAL_INT;
            (*val)->data.integer = pmix3x_colwert_rank(mca_pmix_pmix3x_component.myproc.rank);
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_SUCCESS;
        }
    }
    *val = NULL;
    ...
    if (NULL == proc) {
        (void)strncpy(p.nspace, mca_pmix_pmix3x_component.myproc.nspace, PMIX_MAX_NSLEN);
        p.rank = pmix3x_colwert_rank(PMIX_RANK_WILDCARD);
    } else {
        if (NULL == (nsptr = pmix3x_colwert_jobid(proc->jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
                       ...
                       pmix_value_t *kv, void *cbdata)
{
    ...
    rc = pmix3x_colwert_opalrc(status);
                 ...
                 opal_pmix_value_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == proc) {
        ...
        op->p.rank = pmix3x_colwert_rank(PMIX_RANK_WILDCARD);
    } else {
        if (NULL == (nsptr = pmix3x_colwert_jobid(proc->jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
        ...
        op->p.rank = pmix3x_colwert_opalrank(proc->vpid);
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
    ...
    if (PMIX_SUCCESS == rc) {
        ...
        OPAL_LIST_FOREACH(d, data, opal_pmix_pdata_t) {
            if (mca_pmix_pmix3x_component.native_launch) {
                /* if we were launched by the OMPI RTE, then
                 * the jobid is in a special format - so get it */
                opal_colwert_string_to_jobid(&d->proc.jobid, pdata[n].proc.nspace);
            } else {
                /* we were launched by someone else, so make the
                 * jobid just be the hash of the nspace */
            ...
            d->proc.vpid = pmix3x_colwert_rank(pdata[n].proc.rank);
                      ...
                      void *cbdata)
{
    ...
    rc = pmix3x_colwert_rc(op->status);
    if (OPAL_SUCCESS == rc) {
        OPAL_PMIX_ACQUIRE_THREAD(&opal_pmix_base.lock);
        ...
        for (n=0; n < ndata; n++) {
            ...
            if (mca_pmix_pmix3x_component.native_launch) {
                /* if we were launched by the OMPI RTE, then
                 * the jobid is in a special format - so get it */
                opal_colwert_string_to_jobid(&d->proc.jobid, data[n].proc.nspace);
            } else {
                /* we were launched by someone else, so make the
                 * jobid just be the hash of the nspace */
            ...
            d->proc.vpid = pmix3x_colwert_rank(data[n].proc.rank);
...
int pmix3x_spawn(opal_list_t *job_info, opal_list_t *apps, opal_jobid_t *jobid)
{
    ...
    *jobid = OPAL_JOBID_ILWALID;
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (NULL != app->elw) {
            papps[n].elw = opal_argv_copy(app->elw);
        }
        if (NULL != app->cwd) {
    ...
    if (PMIX_SUCCESS == rc) {
        ...
        if (mca_pmix_pmix3x_component.native_launch) {
            /* if we were launched by the OMPI RTE, then
             * the jobid is in a special format - so get it */
            opal_colwert_string_to_jobid(jobid, nspace);
        } else {
            /* we were launched by someone else, so make the
             * jobid just be the hash of the nspace */
                     ...
                     char *nspace, void *cbdata)
{
    pmix3x_opcaddy_t *op = (pmix3x_opcaddy_t*)cbdata;
    opal_pmix3x_jobid_trkr_t *job;
    opal_jobid_t jobid = OPAL_JOBID_ILWALID;
    int rc;

    OPAL_ACQUIRE_OBJECT(op);

    rc = pmix3x_colwert_rc(status);
                   ...
                   opal_pmix_spawn_cbfunc_t cbfunc, void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (NULL != app->elw) {
            op->apps[n].elw = opal_argv_copy(app->elw);
        }
...
int pmix3x_connect(opal_list_t *procs)
{
    ...
    OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
        ...
        p[n].rank = pmix3x_colwert_opalrank(ptr->name.vpid);
        ++n;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
                     ...
                     void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
        ...
        op->procs[n].rank = pmix3x_colwert_opalrank(ptr->name.vpid);
        ++n;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
                         ...
                         opal_list_t *procs)
{
    ...
    if (OPAL_JOBID_WILDCARD != jobid) {
        if (NULL == (nspace = pmix3x_colwert_jobid(jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
        }
    } else {
    ...
    if (NULL != array && 0 < nprocs) {
        ...
        for (n=0; n < nprocs; n++) {
            ...
            if (mca_pmix_pmix3x_component.native_launch) {
                /* if we were launched by the OMPI RTE, then
                 * the jobid is in a special format - so get it */
                opal_colwert_string_to_jobid(&nm->name.jobid, array[n].nspace);
            } else {
                /* we were launched by someone else, so make the
                 * jobid just be the hash of the nspace */
            ...
            /* if we don't already have it, add this to our jobid tracker */
            if (NULL == pmix3x_colwert_jobid(nm->name.jobid)) {
                job = OBJ_NEW(opal_pmix3x_jobid_trkr_t);
                (void)strncpy(job->nspace, array[n].nspace, PMIX_MAX_NSLEN);
                job->jobid = nm->name.jobid;
                opal_list_append(&mca_pmix_pmix3x_component.jobids, &job->super);
            }
            nm->name.vpid = pmix3x_colwert_rank(array[n].rank);
        }
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
    }
...
int pmix3x_resolve_nodes(opal_jobid_t jobid, char **nodelist)
{
    ...
    if (NULL == (nsptr = pmix3x_colwert_jobid(jobid))) {
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
        return OPAL_ERR_NOT_FOUND;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
