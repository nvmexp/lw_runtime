...
static pmix_status_t spawn_debugger(char *appspace)
{
    ...
    PMIX_INFO_LOAD(&dinfo[0], PMIX_MAPBY, "ppr:1:node", PMIX_STRING);  // instruct the RM to launch one copy of the exelwtable on each node
...
static int attach_to_running_job(char *nspace)
{
    ...
    napps = 1;
    PMIX_APP_CREATE(app, napps);
    /* setup the name of the daemon exelwtable to launch */
    ...
    PMIX_INFO_LOAD(&app[0].info[0], PMIX_MAPBY, "ppr:1:node", PMIX_STRING);  // instruct the RM to launch one copy of the exelwtable on each node
