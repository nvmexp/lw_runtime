...
int main(int argc, char **argv)
{
    ...
    if( rank == 0 ){
        double  lwm_get_total_time = 0,
                lwm_get_loc_time = 0,
                lwm_get_rem_time = 0,
                lwm_get_time = 0,
                lwm_put_total_time = 0,
                lwm_put_loc_time = 0,
                lwm_put_rem_time = 0,
                lwm_commit_time = 0,
                lwm_fence_time = 0,
        ...
        for(i = 0; i < nproc; i++){
            ...
            lwm_get_time += pmi_get_double(i, key);

            sprintf(key, "PMIX_PERF_put_loc_time.%d", i);
            lwm_put_loc_time += pmi_get_double(i, key);

            sprintf(key, "PMIX_PERF_put_rem_time.%d", i);
            lwm_put_rem_time += pmi_get_double(i, key);

            sprintf(key, "PMIX_PERF_commit_time.%d", i);
            lwm_commit_time += pmi_get_double(i, key);

            sprintf(key, "PMIX_PERF_fence_time.%d", i);
            lwm_fence_time += pmi_get_double(i, key);
        ...
        if( get_loc_cnt ){
            sprintf(c_get_ltime,"%lf", lwm_get_loc_time / nproc);
        } else {
            sprintf(c_get_ltime,"--------");
        }
        if( get_rem_cnt ){
            sprintf(c_get_rtime,"%lf", lwm_get_rem_time / nproc);
        } else {
            sprintf(c_get_rtime,"--------");
        }

        if( get_loc_cnt + get_rem_cnt ){
            sprintf(c_get_ttime,"%lf", lwm_get_time / nproc);
        } else {
            sprintf(c_get_ttime,"--------");
        }

        if( put_loc_cnt ){
            sprintf(c_put_ltime,"%lf", lwm_put_loc_time / nproc);
            lwm_put_total_time += lwm_put_loc_time;
        } else {
            sprintf(c_put_ltime,"--------");
        }
        if( put_rem_cnt ){
            sprintf(c_put_rtime,"%lf", lwm_put_rem_time / nproc);
            lwm_put_total_time += lwm_put_rem_time;
        } else {
            sprintf(c_put_rtime,"--------");
        }
        ...
        fprintf(stderr,"init: %lf; put: %lf; commit: %lf; fence: %lf; get: %lf; total: %lf\n",
                lwm_init_time / nproc,
                lwm_put_total_time / nproc,
                lwm_commit_time / nproc, lwm_fence_time / nproc,
                lwm_get_total_time / nproc,
                lwm_total_time / nproc);
        ...
        fprintf(stderr,"mem:           loc %0.2lf avg %0.2lf min %0.2lf max %0.2lf total %0.2lf Kb\n",
                mem_pss, lwm_mem_pss / nproc, min_mem_pss, max_mem_pss, lwm_mem_pss);
        if( have_shmem ) {
