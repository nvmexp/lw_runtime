...
#    set +x
#    elw| sed -ne "s/\(\w*\)=\(.*\)\$/export \1='\2'/p" > $WORKSPACE/test_elw.sh
#    chmod 755 $WORKSPACE/test_elw.sh
#    set -x
}
...
if [ "$jenkins_test_src_rpm" = "yes" ]; then
    ...
    if [ -x /usr/bin/dpkg-buildpackage ]; then
        ...
        (cd ./contrib/ && elw rpmbuild_options="$rpm_flags" rpmtopdir=$rpm_dir ./buildrpm.sh $tarball_src)
    fi
fi
