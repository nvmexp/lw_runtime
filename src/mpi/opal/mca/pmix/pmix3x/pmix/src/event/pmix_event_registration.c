...
static pmix_status_t _add_hdlr(pmix_rshift_caddy_t *cd, pmix_list_t *xfer)
{
     ...
     * our host if we want elwironmental events */
    if (PMIX_PEER_IS_SERVER(pmix_globals.mypeer) &&
        !PMIX_PEER_IS_LAUNCHER(pmix_globals.mypeer) && cd->elwiro &&
...
static void reg_event_hdlr(int sd, short args, void *cbdata)
{
    ...
    if (NULL != cd->info) {
        for (n=0; n < cd->ninfo; n++) {
            ...
            } else if (0 == strncmp(cd->info[n].key, PMIX_EVENT_LWSTOM_RANGE, PMIX_MAX_KEYLEN)) {
                                             ...
                                             void *cbdata)
{
     ...
     * registrations AFTER we have exelwted the callback which allows
