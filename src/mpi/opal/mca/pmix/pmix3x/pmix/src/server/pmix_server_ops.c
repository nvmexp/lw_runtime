 ...
 * ilwolved AND has called the collective operation.
 *
                                       ...
                                       size_t nprocs, pmix_cmd_t type)
{
    ...
    /* there is no shortlwt way to search the trackers - all
                                       ...
                                       size_t nprocs, pmix_cmd_t type)
{
    ...
    for (i=0; i < nprocs; i++) {
        ...
        /* check if multiple nspaces are ilwolved in this operation */
        if (0 == strlen(first)) {
                                ...
                                pmix_op_cbfunc_t opcbfunc)
{
    ...
    if (NULL == (trk = get_tracker(NULL, procs, nprocs, PMIX_FENCENB_CMD))) {
       ...
       /* mark if they want the data back */
        ...
        case PMIX_COLLECT_NO:
            if (collect_data) {
                trk->collect_type = PMIX_COLLECT_ILWALID;
            }
            break;
    ...
    if (trk->def_complete &&
        ...
        /* this fence ilwolves non-local procs - check if the
         * host supports it */
        if (NULL == pmix_host_server.fence_nb) {
            rc = PMIX_ERR_NOT_SUPPORTED;
                                          ...
                                          void *cbdata)
{
    ...
    /* if they asked for elwiro events, and our host doesn't support
     * register_events, then we cannot meet the request */
    if (elwiro_events && NULL == pmix_host_server.register_events) {
        elwiro_events = false;
        rc = PMIX_ERR_NOT_SUPPORTED;
        goto cleanup;
    }
    ...
    for (n=0; n < ncodes; n++) {
        ...
        if (found) {
            ...
            prev->elwiro_events = elwiro_events;
            pmix_list_append(&reginfo->peers, &prev->super);
        } else {
    ...
    /* if they asked for elwiro events, call the local server */
    if (elwiro_events) {
        ...
        if (PMIX_SUCCESS == (rc = pmix_host_server.register_events(scd->codes, scd->ncodes, scd->info, scd->ninfo, regevopcbfunc, scd))) {
             ...
             * that oclwrs _after_ the client returns from registering the
                                ...
                                void *cbdata)
{
     ...
     * want to be refreshed to be exelwted locally, and those that
    ...
    for (n=0; n < cd->nqueries; n++) {
        ...
        cb.copy = false;
        /* set the proc */
        if (PMIX_RANK_ILWALID == proc.rank &&
                                   ...
                                   void *cbdata)
{
    ...
    pmix_proc_t proc;
    size_t n;
    bool relwrse = false, leave_topdir = false, duplicate;
    ...
    for (n=0; n < cd->ninfo; n++) {
        ...
        } else if (PMIX_CHECK_KEY(&cd->info[n], PMIX_CLEANUP_RELWRSIVE)) {
            relwrse = PMIX_INFO_TRUE(&cd->info[n]);
            ++cnt;
            ...
            PMIX_LIST_FOREACH(epicd, &epicache, pmix_srvr_epi_caddy_t) {
                ...
                PMIX_LIST_FOREACH(cdir2, &epicd->epi->cleanup_dirs, pmix_cleanup_dir_t) {
                    if (0 == strcmp(cdir2->path, cdir->path)) {
                         ...
                         * precedence rules */
                        if (!cdir->relwrse && relwrse) {
                            cdir->relwrse = relwrse;
                        }
...
static void tcon(pmix_server_trkr_t *t)
{
    ...
    t->collect_type = PMIX_COLLECT_ILWALID;
    t->modexcbfunc = NULL;
    t->op_cbfunc = NULL;
    t->hybrid = false;
}
...
static void ilcon(pmix_ilwentory_rollup_t *p)
{
    PMIX_CONSTRUCT_LOCK(&p->lock);
    p->lock.active = false;
...
static void ildes(pmix_ilwentory_rollup_t *p)
{
    PMIX_DESTRUCT_LOCK(&p->lock);
    PMIX_LIST_DESTRUCT(&p->payload);
}
PMIX_CLASS_INSTANCE(pmix_ilwentory_rollup_t,
                    pmix_object_t,
                    ilcon, ildes);
