...
static void pmix_mutex_construct(pmix_mutex_t *m)
{
    ...
    /* set type to ERRORCHECK so that we catch relwrsive locks */
...
static void pmix_relwrsive_mutex_construct(pmix_relwrsive_mutex_t *m)
{
    pthread_mutexattr_t attr;
    pthread_mutexattr_init(&attr);
...
PMIX_CLASS_INSTANCE(pmix_relwrsive_mutex_t,
                   pmix_object_t,
                   pmix_relwrsive_mutex_construct,
                   pmix_mutex_destruct);
