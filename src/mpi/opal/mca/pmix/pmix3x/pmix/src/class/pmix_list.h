 ...
 * Given the highly conlwrrent nature of this class, these spot checks
 ...
 * threads may [erroneously] modify an pmix_list_item_t conlwrrently.
 *
 ...
 * can run into "unlucky" cases where two threads are conlwrrently
 ...
 * list - so provide two colwenience macros that do so and then
 ...
 * the caller; no calls to PMIX_RETAIN() or PMIX_RELEASE() are ilwoked.
 *
 * This is an inlined function in compilers that support inlining, so
 * it's usually a cheap operation.
 */
...
{
#if PMIX_ENABLE_DEBUG
    /* Spot check: ensure that the item we're insertting is lwrrently
       not on any list */
  ...
  bool pmix_list_insert(pmix_list_t *list, pmix_list_item_t *item,
     ...
     *
     * This function is ilwoked by qsort(3) from within
     * pmix_list_sort().  It is important to understand what
     * pmix_list_sort() does before ilwoking qsort, so go read that
     * documentation first.
     *
