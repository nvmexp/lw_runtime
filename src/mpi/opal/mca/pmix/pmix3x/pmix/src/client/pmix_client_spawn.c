                                        ...
                                        pmix_spawn_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL != job_info) {
        for (n=0; n < ninfo; n++) {
            if (PMIX_CHECK_KEY(&job_info[n], PMIX_SETUP_APP_ELWARS)) {
                PMIX_CONSTRUCT(&ilist, pmix_list_t);
                rc = pmix_pnet_base_harvest_elwars(harvest, NULL, &ilist);
                if (PMIX_SUCCESS != rc) {
                    PMIX_LIST_DESTRUCT(&ilist);
                    return rc;
                }
                PMIX_LIST_FOREACH(kv, &ilist, pmix_kval_t) {
                    /* cycle across all the apps and set this elwar */
                    for (m=0; m < napps; m++) {
                        aptr = (pmix_app_t*)&apps[m];
                        pmix_setelw(kv->value->data.elwar.elwar,
                                    kv->value->data.elwar.value,
                                    true, &aptr->elw);
                    }
                }
                jobelwars = true;
    ...
    for (n=0; n < napps; n++) {
        ...
        if (!jobelwars) {
            for (m=0; m < aptr->ninfo; m++) {
                if (PMIX_CHECK_KEY(&aptr->info[m], PMIX_SETUP_APP_ELWARS)) {
                    PMIX_CONSTRUCT(&ilist, pmix_list_t);
                    rc = pmix_pnet_base_harvest_elwars(harvest, NULL, &ilist);
                    if (PMIX_SUCCESS != rc) {
                        PMIX_LIST_DESTRUCT(&ilist);
