                               ...
                               pmix_info_t info[], size_t ninfo)
{
     ...
     * check to see if we were given one in the elw - this
     * will be the case when we are launched by a PMIx-enabled
     * daemon */
    if (!nspace_given) {
        if (NULL != (evar = getelw("PMIX_NAMESPACE"))) {
            nspace = strdup(evar);
            nspace_in_elwiro = true;
        }
    }
    ...
    if (nspace_given || nspace_in_elwiro) {
    ...
    if (PMIX_PEER_IS_CLIENT(pmix_globals.mypeer)) {
         ...
         * rest of the elwar-based server assignments */
     ...
     * on the corresponding elwars that should have been passed
     * to us at launch */
    evar = getelw("PMIX_SELWRITY_MODE");
     ...
     * on the corresponding elwars that should have been passed
     * to us at launch */
    evar = getelw("PMIX_BFROP_BUFFER_TYPE");
    if (NULL == evar) {
    ...
    if (PMIX_PEER_IS_LAUNCHER(pmix_globals.mypeer)) {
        ...
        /* add it to the end of the list of recvs */
        pmix_list_append(&pmix_ptl_globals.posted_recvs, &rcv->super);
        /* open the pnet framework so we can harvest elwars */
    ...
    if (PMIX_PEER_IS_CLIENT(pmix_globals.mypeer)) {
         ...
         * is a launcher that is being exelwted multiple times
...
pmix_status_t pmix_tool_init_info(void)
{
     ...
     * other processes are exelwting on this node - so
