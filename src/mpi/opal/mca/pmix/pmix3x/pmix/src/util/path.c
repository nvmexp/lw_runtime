...
static void path_elw_load(char *path, int *pargc, char ***pargv);
static char *list_elw_get(char *var, char **list);
 ...
 */
char *pmix_path_find(char *fname, char **pathv, int mode, char **elwv)
{
    char *fullpath;
    char *delimit;
    char *elw;
    ...
    while (pathv[i] && NULL == fullpath) {
        ...
        if ('$' == *pathv[i]) {
            ...
            elw = list_elw_get(pathv[i]+1, elwv);
            if (delimit) {
                *delimit = PMIX_PATH_SEP[0];
            }
            if (NULL != elw) {
                if (!delimit) {
                    fullpath = pmix_path_access(fname, elw, mode);
                } else {
                    pfix = (char*) malloc(strlen(elw) + strlen(delimit) + 1);
                    if (NULL == pfix) {
                        return NULL;
                    }
                    strcpy(pfix, elw);
 ...
 */
char *pmix_path_findv(char *fname, int mode, char **elwv, char *wrkdir)
{
    char **dirv;
    char *fullpath;
    char *path;
    ...
    if (NULL != (path = list_elw_get("PATH", elwv))) {
        path_elw_load(path, &dirc, &dirv);
    }
    ...
    if(NULL == dirv)
        return NULL;
    fullpath = pmix_path_find(fname, dirv, mode, elwv);
    pmix_argv_free(dirv);
    return fullpath;
}
...
char *pmix_path_access(char *fname, char *path, int mode)
{
    ...
    if ((X_OK & mode) && !(S_IXUSR & buf.st_mode)) {
        /* if they asked us to check exelwtable permission,
...
/**
 *
 *  Loads argument array with $PATH elw var.
 *
 *  Accepts
...
static void path_elw_load(char *path, int *pargc, char ***pargv)
{
    ...
    /* Loop through the paths (delimited by PATHELWSEP), adding each
       one to argv. */
 ...
 * the $PATH.
 * In case of error or if exelwtable was not found (as an example if
 ...
 * on any network file system (lwrrently NFS, Lustre, Panasas and GPFS).
 *
