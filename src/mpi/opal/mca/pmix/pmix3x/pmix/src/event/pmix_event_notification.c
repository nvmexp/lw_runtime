...
static pmix_status_t notify_event_cache(pmix_notify_caddy_t *cd)
{
    ...
    if (PMIX_SUCCESS != rc) {
        ...
        if (0 <= idx) {
            /* we found the oldest oclwpant - evict it */
            pmix_hotel_checkout_and_return_oclwpant(&pmix_globals.notifications, idx, (void**)&pk);
            PMIX_RELEASE(pk);
            rc = pmix_hotel_checkin(&pmix_globals.notifications, cd, &cd->room);
        }
    }
                                            ...
                                            pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    pmix_ilwoke_local_event_hdlr(chain);
 ...
 * registered callbacks and ilwoke the matching ones. Note
 * that we will ilwoke the callbacks in order from single
 * to multi-event to default, keeping a log of any returned
 * info and passing it down to the next ilwoked event handler.
 * Thus, each handler is given the opportunity to see what
 * prior handlers did, and decide if anything further needs
 * to be done.
 */
void pmix_ilwoke_local_event_hdlr(pmix_event_chain_t *chain)
{
    ...
    pmix_output_verbose(2, pmix_client_globals.event_output,
                        "%s:%d ilwoke_local_event_hdlr for status %s",
                        pmix_globals.myid.nspace, pmix_globals.myid.rank,
  ...
  ilwk:
    ...
    /* ilwoke the handler */
    pmix_output_verbose(2, pmix_client_globals.event_output,
                        "[%s:%d] ILWOKING EVHDLR %s", __FILE__, __LINE__,
...
static void _notify_client_event(int sd, short args, void *cbdata)
{
    ...
    /* if they provided a PMIX_EVENT_LWSTOM_RANGE info object but
     * specified a range other than PMIX_RANGE_LWSTOM, then this
     * is an error */
    if (PMIX_RANGE_LWSTOM != cd->range && NULL != cd->targets) {
                             ...
                             const pmix_proc_t *proc)
{
    ...
    if (PMIX_RANGE_LWSTOM == rng->range) {
...
void pmix_event_timeout_cb(int fd, short flags, void *arg)
{
    ...
    if (PMIX_PEER_IS_SERVER(pmix_globals.mypeer) &&
        ...
        pmix_ilwoke_local_event_hdlr(ch);
    }
}
    ...
    if (NULL != info && 0 < ninfo) {
       ...
       /* need to copy the info */
        for (n=0; n < ninfo; n++) {
            ...
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_EVENT_LWSTOM_RANGE)) {
