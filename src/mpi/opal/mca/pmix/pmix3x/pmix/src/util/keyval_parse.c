...
static char *elw_str = NULL;
static int elwsize = 1024;
                       ...
                       pmix_keyval_parse_fn_t callback)
{
    ...
    while (!pmix_util_keyval_parse_done) {
        ...
        case PMIX_UTIL_KEYVAL_PARSE_ELWVAR:
        case PMIX_UTIL_KEYVAL_PARSE_ELWEQL:
            parse_line_new(val);
            break;
...
int pmix_util_keyval_save_internal_elwars(pmix_keyval_parse_fn_t callback)
{
    if (NULL != elw_str && 0 < strlen(elw_str)) {
        callback("mca_base_elw_list_internal", elw_str);
        free(elw_str);
        elw_str = NULL;
    }
    return PMIX_SUCCESS;
...
static int add_to_elw_str(char *var, char *val)
{
    int sz, varsz, valsz;
    void *tmp;
    ...
    if (NULL != elw_str) {
        ...
        sz = strlen(elw_str)+varsz+valsz+2;
        if (elwsize <= sz) {
            elwsize *=2;

            tmp = realloc(elw_str, elwsize);
            if (NULL == tmp) {
                return PMIX_ERR_OUT_OF_RESOURCE;
            }
            elw_str = tmp;
...
static int parse_line_new(pmix_keyval_parse_state_t first_val)
{
    ...
    while (PMIX_UTIL_KEYVAL_PARSE_NEWLINE != val && PMIX_UTIL_KEYVAL_PARSE_DONE != val) {
        ...
        } else if (PMIX_UTIL_KEYVAL_PARSE_ELWEQL == val) {
            trim_name (key_buffer, "-x", "=");
            trim_name (key_buffer, "--x", NULL);

            val = pmix_util_keyval_yylex();
            if (PMIX_UTIL_KEYVAL_PARSE_VALUE == val) {
                add_to_elw_str(key_buffer, pmix_util_keyval_yytext);
            } else {
                parse_error(5);
                return PMIX_ERROR;
            }
        } else if (PMIX_UTIL_KEYVAL_PARSE_ELWVAR == val) {
            trim_name (key_buffer, "-x", "=");
            trim_name (key_buffer, "--x", NULL);
            add_to_elw_str(key_buffer, NULL);
        } else {
