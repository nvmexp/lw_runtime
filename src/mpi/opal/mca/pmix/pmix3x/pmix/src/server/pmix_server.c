...
// local variables
static char *selwrity_mode = NULL;
                                           ...
                                           pmix_info_t info[], size_t ninfo)
{
    ...
    if (NULL == pmix_server_globals.tmpdir) {
        if (NULL == (evar = getelw("PMIX_SERVER_TMPDIR"))) {
    ...
    if (NULL == pmix_server_globals.system_tmpdir) {
        if (NULL == (evar = getelw("PMIX_SYSTEM_TMPDIR"))) {
    ...
    selwrity_mode = pmix_psec_base_get_available_modules();
    ...
    if (!nspace_given) {
        ...
        if (NULL == (evar = getelw("PMIX_SERVER_NAMESPACE"))) {
    ...
    if (!rank_given) {
        ...
        mypid = getpid();
        if (NULL == (evar = getelw("PMIX_SERVER_RANK"))) {
            /* use our pid */
...
PMIX_EXPORT pmix_status_t PMIx_server_finalize(void)
{
    ...
    for (i=0; i < pmix_server_globals.clients.size; i++) {
        if (NULL != (peer = (pmix_peer_t*)pmix_pointer_array_get_item(&pmix_server_globals.clients, i))) {
             ...
             * at zero refcount */
            pmix_exelwte_epilog(&peer->epilog);
            PMIX_RELEASE(peer);
        }
    }
    ...
    PMIX_LIST_FOREACH(ns, &pmix_globals.nspaces, pmix_namespace_t) {
         ...
         * at zero refcount */
        pmix_exelwte_epilog(&ns->epilog);
    }
    PMIX_LIST_DESTRUCT(&pmix_server_globals.iof);

    pmix_hwloc_cleanup();

    if (NULL != selwrity_mode) {
        free(selwrity_mode);
    }
...
static void _register_nspace(int sd, short args, void *cbdata)
{
    ...
    PMIX_LIST_FOREACH(trk, &pmix_server_globals.collectives, pmix_server_trkr_t) {
        ...
        if (trk->def_complete && pmix_list_get_size(&trk->local_cbs) == trk->nlocal) {
            ...
            PMIX_EXELWTE_COLLECTIVE(tcd, trk, pmix_server_exelwte_collective);
        }
    }
                              ...
                              pmix_proc_t *proc)
{
    ...
    /* see if this proc is ilwolved in any direct modex requests */
...
static void _deregister_nspace(int sd, short args, void *cbdata)
{
    ...
    PMIX_LIST_FOREACH(tmp, &pmix_globals.nspaces, pmix_namespace_t) {
        if (PMIX_CHECK_NSPACE(tmp->nspace, cd->proc.nspace)) {
            ...
            pmix_exelwte_epilog(&tmp->epilog);
...
void pmix_server_exelwte_collective(int sd, short args, void *cbdata)
{
    ...
    if (PMIX_FENCENB_CMD == trk->type) {
         ...
         * server so they can cirlwlate it - only take data
...
static void _register_client(int sd, short args, void *cbdata)
{
    ...
    if (SIZE_MAX != nptr->nlocalprocs &&
        ...
        PMIX_LIST_FOREACH(trk, &pmix_server_globals.collectives, pmix_server_trkr_t) {
             ...
             * already called the collective. While the collective created the
             * tracker, it would not have updated the number of local participants
             * from this nspace UNLESS the collective ilwolves all procs in the
...
/* setup the elwars for a child process */
PMIX_EXPORT pmix_status_t PMIx_server_setup_fork(const pmix_proc_t *proc, char ***elw)
{
    char rankstr[128];
    pmix_listener_t *lt;
    pmix_status_t rc;
    ...
    /* pass the nspace */
    pmix_setelw("PMIX_NAMESPACE", proc->nspace, true, elw);
    /* pass the rank */
    (void)snprintf(rankstr, 127, "%d", proc->rank);
    pmix_setelw("PMIX_RANK", rankstr, true, elw);
    ...
    PMIX_LIST_FOREACH(lt, &pmix_ptl_globals.listeners, pmix_listener_t) {
        if (NULL != lt->uri && NULL != lt->varname) {
            ...
            for (n=0; NULL != varnames[n]; n++) {
                pmix_setelw(varnames[n], lt->uri, true, elw);
            }
            pmix_argv_free(varnames);
        }
    }
    /* pass our active security modules */
    pmix_setelw("PMIX_SELWRITY_MODE", selwrity_mode, true, elw);
    /* pass our available ptl modules */
    pmix_setelw("PMIX_PTL_MODULE", ptl_mode, true, elw);
    /* pass the type of buffer we are using */
    if (PMIX_BFROP_BUFFER_FULLY_DESC == pmix_globals.mypeer->nptr->compat.type) {
        pmix_setelw("PMIX_BFROP_BUFFER_TYPE", "PMIX_BFROP_BUFFER_FULLY_DESC", true, elw);
    } else {
        pmix_setelw("PMIX_BFROP_BUFFER_TYPE", "PMIX_BFROP_BUFFER_NON_DESC", true, elw);
    }
    /* pass our available gds modules */
    pmix_setelw("PMIX_GDS_MODULE", gds_mode, true, elw);

    /* get any PTL contribution such as tmpdir settings for session files */
    if (PMIX_SUCCESS != (rc = pmix_ptl_base_setup_fork(proc, elw))) {
        PMIX_ERROR_LOG(rc);
        return rc;
    }

    /* get any network contribution */
    if (PMIX_SUCCESS != (rc = pmix_pnet.setup_fork(proc, elw))) {
        PMIX_ERROR_LOG(rc);
        return rc;
    }

    /* get any GDS contributions */
    if (PMIX_SUCCESS != (rc = pmix_gds_base_setup_fork(proc, elw))) {
        PMIX_ERROR_LOG(rc);
        return rc;
    }
    ...
    pmix_setelw("PMIX_HOSTNAME", pmix_globals.hostname, true, elw);

    /* communicate our version */
    pmix_setelw("PMIX_VERSION", PMIX_VERSION, true, elw);
...
{
    pmix_ilwentory_rollup_t *rollup = (pmix_ilwentory_rollup_t*)cbdata;
    if (NULL != rollup->info) {
...
static void clct_complete(pmix_status_t status,
                          ...
                          void *cbdata)
{
    pmix_ilwentory_rollup_t *cd = (pmix_ilwentory_rollup_t*)cbdata;
    pmix_kval_t *kv;
    size_t n;
    pmix_status_t rc;
    ...
    if (cd->replies == cd->requests) {
        ...
        if (NULL != cd->infocbfunc) {
            /* colwert the list to an array of pmix_info_t */
...
static void clct(int sd, short args, void *cbdata)
{
    ...
    pmix_pnet.collect_ilwentory(cd->info, cd->ninfo,
                                clct_complete, cd);

    return;
}

pmix_status_t PMIx_server_collect_ilwentory(pmix_info_t directives[], size_t ndirs,
                                            pmix_info_cbfunc_t cbfunc, void *cbdata)
{
    pmix_ilwentory_rollup_t *cd;
    ...
    /* need to threadshift this request */
    cd = PMIX_NEW(pmix_ilwentory_rollup_t);
    if (NULL == cd) {
        return PMIX_ERR_NOMEM;
    }
    cd->info = directives;
...
static void dlilw_complete(pmix_status_t status, void *cbdata)
{
    pmix_shift_caddy_t *cd = (pmix_shift_caddy_t*)cbdata;
...
static void dlilw(int sd, short args, void *cbdata)
{
    pmix_shift_caddy_t *cd = (pmix_shift_caddy_t*)cbdata;
    ...
    pmix_pnet.deliver_ilwentory(cd->info, cd->ninfo,
...
pmix_status_t PMIx_server_deliver_ilwentory(pmix_info_t info[], size_t ninfo,
 ...
 ****    CONTEXT, OR IN OUR OWN THREAD CONTEXT IF THE CALLBACK OCLWRS    ****
...
static void _mdxcbfunc(int sd, short argc, void *cbdata)
{
    ...
    if (PMIX_COLLECT_ILWALID == tracker->collect_type) {
        rc = PMIX_ERR_ILWALID_ARG;
        goto finish_collective;
    }
 ...
 * sending the command) and can be exelwted while we wait. In these cases,
