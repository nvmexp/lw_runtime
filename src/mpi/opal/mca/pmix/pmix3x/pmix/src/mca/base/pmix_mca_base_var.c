...
#define PMIX_MCA_BASE_ELW_LIST_SEP_DEFAULT ";"
static char *pmix_mca_base_env_list_sep = PMIX_MCA_BASE_ELW_LIST_SEP_DEFAULT;
...
int pmix_mca_base_var_init(void)
{
    ...
    if (!pmix_mca_base_var_initialized) {
        ...
        pmix_mca_base_env_list_sep = PMIX_MCA_BASE_ELW_LIST_SEP_DEFAULT;
...
int pmix_mca_base_var_cache_files(bool rel_path_search)
{
    ...
    ret = asprintf(&pmix_mca_base_param_file_path, "%s" PMIX_PATH_SEP "amca-param-sets%c%s",
                   pmix_pinstall_dirs.pmixdatadir, PMIX_ELW_SEP, cwd);
    if (0 > ret) {
        return PMIX_ERR_OUT_OF_RESOURCE;
    }
    ...
    if (NULL != force_agg_path) {
        if (NULL != pmix_mca_base_param_file_path) {
            ...
            ret = asprintf(&pmix_mca_base_param_file_path, "%s%c%s", force_agg_path, PMIX_ELW_SEP, tmp_str);
            free(tmp_str);
            if (0 > ret) {
                return PMIX_ERR_OUT_OF_RESOURCE;
            }
    ...
    if (NULL != pmix_mca_base_var_file_prefix) {
       resolve_relative_paths(&pmix_mca_base_var_file_prefix, pmix_mca_base_param_file_path, rel_path_search, &pmix_mca_base_var_files, PMIX_ELW_SEP);
    }
    ...
    if (0 == access(pmix_mca_base_var_override_file, F_OK)) {
        read_files (pmix_mca_base_var_override_file, &pmix_mca_base_var_override_values, PMIX_ELW_SEP);
    }
...
static int fixup_files(char **file_list, char * path, bool rel_path_search, char sep) {
    ...
    search_path = pmix_argv_split(path, PMIX_ELW_SEP);
