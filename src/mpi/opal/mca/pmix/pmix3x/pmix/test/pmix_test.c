...
int main(int argc, char **argv)
{
    ...
    TEST_VERBOSE(("Exelwting test: %s", tmp));
    free(tmp);

    /* verify exelwtable */
    if( 0 > ( rc = stat(params.binary, &stat_buf) ) ){
        TEST_ERROR(("Cannot stat() exelwtable \"%s\": %d: %s", params.binary, errno, strerror(errno)));
        FREE_TEST_PARAMS(params);
        return 0;
    } else if( !S_ISREG(stat_buf.st_mode) ){
        TEST_ERROR(("Client exelwtable \"%s\": is not a regular file", params.binary));
        FREE_TEST_PARAMS(params);
        return 0;
    }else if( !(stat_buf.st_mode & S_IXUSR) ){
        TEST_ERROR(("Client exelwtable \"%s\": has no exelwtable flag", params.binary));
        FREE_TEST_PARAMS(params);
        return 0;
    }
