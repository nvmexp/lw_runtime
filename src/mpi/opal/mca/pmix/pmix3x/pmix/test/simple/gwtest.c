...
int main(int argc, char **argv)
{
    ...
    x = PMIX_NEW(myxfer_t);
    rc = PMIx_server_collect_ilwentory(NULL, 0, infocbfunc, (void*)x);
    if (PMIX_SUCCESS != rc) {
    ...
    rc = PMIx_server_deliver_ilwentory(x->info, x->ninfo, NULL, 0, opcbfunc, x);
    if (PMIX_SUCCESS != rc) {
