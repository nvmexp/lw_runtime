...
static int srv_wait_all(double timeout)
{
    ...
    while (!pmix_list_is_empty(server_list) &&
        ...
        // callwlate current timestamp
        gettimeofday(&tv, NULL);
        lwr_time = tv.tv_sec + 1E-6*tv.tv_usec;
    }
...
int server_launch_clients(int local_size, int univ_size, int base_rank,
                   test_params *params, char *** client_elw, char ***base_argv)
{
    int n;
    uid_t myuid;
    gid_t mygid;
    ...
    for (n = 0; n < local_size; n++) {
        ...
        if (PMIX_SUCCESS != (rc = PMIx_server_setup_fork(&proc, client_elw))) {//n
        ...
        if (cli_info[cli_counter].pid == 0) {
            ...
            execve(params->binary, client_argv, *client_elw);
            /* Does not return */
