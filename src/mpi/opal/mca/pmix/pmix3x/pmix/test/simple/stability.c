...
int main(int argc, char **argv)
{
    ...
    char *tmp, **atmp, *exelwtable=NULL, *nspace;
    int rc, nprocs=1, n, k;
    uid_t myuid;
    gid_t mygid;
    pid_t pid;
    ...
    for (n=1; n < argc; n++) {
        if (0 == strcmp("-n", argv[n]) &&
            ...
            if (NULL != strstr(exelwtable, "quietclient")) {
                istimeouttest = true;
            }
    ...
    if (nettest) {
        ...
        putelw("PMIX_MCA_pnet_test_lwerts=nodes:5;plane:d:3;plane:s:2;plane:d:5");
    ...
    if (nettest) {
        ...
        unsetelw("PMIX_MCA_pnet_test_lwerts");
    }
