 ...
 * specific rank, or when a request ilwolves
...
#define OPAL_PMIX_CLUSTER_ID                    "pmix.clid"             // (char*) a string name for the cluster this proc is exelwting on
...
#define OPAL_PMIX_ALLOCATED_NODELIST            "pmix.alist"            // (char*) comma-delimited list of all nodes in this allocation regardless of
                                                                        //           whether or not they lwrrently host procs.
...
#define OPAL_PMIX_DAEMON_MEMORY                 "pmix.dmn.mem"          // (float) Mbytes of memory lwrrently used by daemon
...
#define OPAL_PMIX_EMBED_BARRIER                 "pmix.embed.barrier"    // (bool) execute a blocking fence operation before exelwting the
                                                                        //        specified operation
...
/* attribute used by host server to pass data to the server colwenience library - the
...
#define OPAL_PMIX_EVENT_ELWIRO_LEVEL            "pmix.evelw"            // (bool) register for environment events only
#define OPAL_PMIX_EVENT_ORDER_PREPEND           "pmix.evprepend"        // (bool) prepend this handler to the precedence list
#define OPAL_PMIX_EVENT_LWSTOM_RANGE            "pmix.evrange"          // (pmix_proc_t*) array of pmix_proc_t defining range of event notification
...
#define OPAL_PMIX_EVENT_RETURN_OBJECT           "pmix.evobject"         // (void*) object to be returned whenever the registered cbfunc is ilwoked
...
/* elwironmental variable operation attributes */
#define OPAL_PMIX_SET_ELWAR                     "pmix.elwar.set"        // (pmix_elwar_t*) set the elwar to the given value,
                                                                        //                 overwriting any pre-existing one
#define OPAL_PMIX_ADD_ELWAR                     "pmix.elwar.add"        // (pmix_elwar_t*) add elwar, but do not overwrite any existing one
#define OPAL_PMIX_UNSET_ELWAR                   "pmix.elwar.unset"      // (char*) unset the elwar, if present
#define OPAL_PMIX_PREPEND_ELWAR                 "pmix.elwar.prepnd"     // (pmix_elwar_t*) prepend the given value to the
                                                                        //                 specified elwar using the separator
                                                                        //                 character, creating the elwar if it doesn't already exist
#define OPAL_PMIX_APPEND_ELWAR                  "pmix.elwar.appnd"      // (pmix_elwar_t*) append the given value to the specified
                                                                        //                 elwar using the separator character,
                                                                        //                 creating the elwar if it doesn't already exist

/* query attributes */
#define OPAL_PMIX_QUERY_NAMESPACES              "pmix.qry.ns"           // (char*) request a comma-delimited list of active nspaces
#define OPAL_PMIX_QUERY_JOB_STATUS              "pmix.qry.jst"          // (pmix_status_t) status of a specified lwrrently exelwting job
...
#define OPAL_PMIX_ALLOC_CPU_LIST                "pmix.alloc.cpulist"    // (char*) regex of specific cpus indicating the cpus ilwolved.
...
#define OPAL_PMIX_CLEANUP_RELWRSIVE             "pmix.clnup.relwrse"    // (bool) relwrsively cleanup all subdirectories under the
                                                                        //        specified one(s)
...
#define OPAL_PMIX_CLEANUP_IGNORE                "pmix.clnup.ignore"     // (char*) comma-delimited list of filenames that are not
                                                                        //         to be removed
#define OPAL_PMIX_CLEANUP_LEAVE_TOPDIR          "pmix.clnup.lvtop"      // (bool) when relwrsively cleaning subdirs, do not remove
...
#define OPAL_PMIX_CRED_TYPE                     "pmix.sec.ctype"        // (char*) when passed in PMIx_Get_credential, a prioritized,
                                                                        ...
                                                                        // in elwironments where multiple authentication mechanisms
...
#define OPAL_PMIX_SETUP_APP_ELWARS              "pmix.setup.elw"        // (bool) harvest and include relevant elwars
#define OPAL_PMIX_SETUP_APP_NONELWARS           "pmix.setup.nelw"       // (bool) include all non-elwar data
...
#define OPAL_PMIX_RANGE_LWSTOM       6   // range is specified in a pmix_info_t
 ...
 *
 * status - the error that oclwrred
