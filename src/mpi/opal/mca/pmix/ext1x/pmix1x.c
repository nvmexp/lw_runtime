...
pmix_status_t pmix1_colwert_opalrc(int rc)
{
    switch (rc) {
...
int pmix1_colwert_rc(pmix_status_t rc)
{
    switch (rc) {
...
pmix_scope_t pmix1_colwert_opalscope(opal_pmix_scope_t scope) {
    switch(scope) {
                      ...
                      opal_value_t *kv)
{
    ...
    switch(kv->type) {
        ...
        case OPAL_PERSIST:
            v->type = PMIX_INT;
            /* the value is in the uint8_t field, so colwert it first */
