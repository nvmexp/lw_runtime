...
static void opcbfunc(pmix_status_t status, void *cbdata)
{
    ...
    if (NULL != op->opcbfunc) {
        op->opcbfunc(pmix1_colwert_rc(status), op->cbdata);
    }
    OBJ_RELEASE(op);
}
...
{
    pmix_value_t kv;
    pmix_scope_t pmix_scope = pmix1_colwert_opalscope(opal_scope);
    pmix_status_t rc;
