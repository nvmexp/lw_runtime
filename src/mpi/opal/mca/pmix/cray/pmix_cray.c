...
static void cray_get_more_info(void)
{
    ...
    /* add these elwars to prep MPI-2 info pre-defined key/values */
    snprintf(pbuf, sizeof(pbuf), "%d", layout.numCmds);
    opal_setelw("OMPI_NUM_APP_CTX", pbuf, true, &elwiron);
    opal_setelw("OMPI_FIRST_RANKS", firstrankstring, true, &elwiron);
    opal_setelw("OMPI_APP_CTX_NUM_PROCS", npstring, true, &elwiron);
     ...
     * For SLURM, we have to check the SLURM_WORKING_DIR elw.
     * variable.  If it is set, we can't set wdir since
     * we can't assume PWD is where we started.
     */
    if(getelw("SLURM_WORKING_DIR") == NULL) {
        opal_getcwd(pbuf, OPAL_PATH_MAX);
        opal_setelw("OMPI_MCA_initial_wdir", pbuf, true, &elwiron);
    }
...
static void fencenb(int sd, short args, void *cbdata)
{
    ...
    opal_output_verbose(2, opal_pmix_base_framework.framework_output,
                        "%s pmix:cray exelwting fence cache_global %p cache_local %p",
    ...
    for (i=0; i < pmix_nlranks; i++) {
        ...
        if (OPAL_SUCCESS != (rc = opal_pmix_base_fetch(&id,
            ...
            locality = OPAL_PROC_ON_CLUSTER | OPAL_PROC_ON_LW | OPAL_PROC_ON_NODE;
        } else {
            kp = (opal_value_t*)opal_list_get_first(&vals);
...
static char* pmix_error(int pmix_err)
{
    ...
    switch(pmix_err) {
        ...
        case PMI_ERR_ILWALID_ARG: err_msg = "Invalid argument"; break;
        case PMI_ERR_ILWALID_KEY: err_msg = "Invalid key argument"; break;
        case PMI_ERR_ILWALID_KEY_LENGTH: err_msg = "Invalid key length argument"; break;
        case PMI_ERR_ILWALID_VAL: err_msg = "Invalid value argument"; break;
        case PMI_ERR_ILWALID_VAL_LENGTH: err_msg = "Invalid value length argument"; break;
        case PMI_ERR_ILWALID_LENGTH: err_msg = "Invalid length argument"; break;
        case PMI_ERR_ILWALID_NUM_ARGS: err_msg = "Invalid number of arguments"; break;
        case PMI_ERR_ILWALID_ARGS: err_msg = "Invalid args argument"; break;
        case PMI_ERR_ILWALID_NUM_PARSED: err_msg = "Invalid num_parsed length argument"; break;
        case PMI_ERR_ILWALID_KEYVALP: err_msg = "Invalid keyvalp argument"; break;
        case PMI_ERR_ILWALID_SIZE: err_msg = "Invalid size argument"; break;
#if defined(PMI_ERR_ILWALID_KVS)
        /* pmi.h calls this a valid return code but mpich doesn't define it (slurm does). */
        case PMI_ERR_ILWALID_KVS: err_msg = "Invalid kvs argument"; break;
#endif
