 ...
 * only find PUs (!).  On such platforms, then do the same callwlation
...
void opal_hwloc_base_free_topology(hwloc_topology_t topo)
{
    ...
    if (!topo_in_shmem) {
        ...
        /* now relwrsively descend and release userdata
                                      ...
                                      hwloc_obj_t obj)
{
    ...
    data = (opal_hwloc_obj_data_t*)obj->userdata;
    if (NULL == data || !data->npus_callwlated) {
 ...
 * in an attribute union. So looking for cache objects ilwolves
 * a multi-step test :-(
 */
                                                            ...
                                                            char *cpuset1, char *cpuset2)
{
     ...
     * sense multi-lw, multi-cluster systems grows
     */
    locality = OPAL_PROC_ON_NODE | OPAL_PROC_ON_HOST | OPAL_PROC_ON_LW | OPAL_PROC_ON_CLUSTER;
    ...
    /* colwert the strings to cpusets */
...
{
    /* this support lwrrently is limited to Intel Phi processors
...
opal_hwloc_locality_t opal_hwloc_compute_relative_locality(char *loc1, char *loc2)
{
    ...
    for (n1=0; NULL != set1[n1]; n1++) {
        /* colwert the location into bitmap */
