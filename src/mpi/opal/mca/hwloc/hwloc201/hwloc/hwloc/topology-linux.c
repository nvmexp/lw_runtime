...
hwloc__read_fd(int fd, char **bufferp, size_t *sizep)
{
   ...
   * Only oclwrs on first invocation and if the kernel ever uses multiple page for a single mask.
   */
  do {
    char *tmp;
...
hwloc__read_fd_as_cpumask(int fd, hwloc_bitmap_t set)
{
   ...
   * in case there's ever multiple conlwrrent calls.
   */
  _filesize = filesize;
...
hwloc__read_fd_as_cpulist(int fd, hwloc_bitmap_t set)
{
   ...
   * so don't bother remember the actual read size between ilwocations.
   * We don't have many ilwocations anyway.
   */
				      ...
				      unsigned *max_os_index_p, unsigned long **linuxmaskp)
{
  ...
  if (max_os_index == (unsigned) -1)
    max_os_index = 0;
  /* add 1 to colwert the last os_index into a max_os_index,
...
hwloc_linux_membind_policy_to_hwloc(int linuxpolicy, hwloc_membind_policy_t *policy)
{
  switch (linuxpolicy) {
  case MPOL_DEFAULT:
  case MPOL_LOCAL: /* colwerted from MPOL_PREFERRED + empty nodeset by the caller */
...
static int hwloc_linux_get_allowed_resources_hook(hwloc_topology_t topology)
{
  ...
  fsroot_path = getelw("HWLOC_FSROOT");
  if (!fsroot_path)
    fsroot_path = "/";
			      ...
			      struct hwloc_numanode_attr_s *memory)
{
  ...
  const char *pagesize_elw = getelw("HWLOC_DEBUG_PAGESIZE");
  int types = 2;
  int err;
  ...
  if (topology->is_thissystem || pagesize_elw) {
    /* we cannot report any page_type info unless we have the page size.
     * we'll take it either from the system if local, or from the debug elw variable
     */
  ...
  if (memory->page_types) {
    ...
    if (pagesize_elw) {
      /* We cannot get the pagesize if not thissystem, use the elw-given one to experience the code during make check */
      memory->page_types[0].size = strtoull(pagesize_elw, NULL, 10);
      /* If failed, use 4kB */
      if (!memory->page_types[0].size)
	memory->page_types[0].size = 4096;
    }
    assert(memory->page_types[0].size); /* from sysconf if local or from the elw */
			 ...
			 struct hwloc_linux_backend_data_s *data)
{
  ...
  for (i = 0; i < cpus.n; ++i) {
    ...
    /* Callwlate cache level and CPU mask */
    cpuset = hwloc_bitmap_alloc();
    if (0 == look_powerpc_device_tree_discover_cache(&cpus,
          cpus.p[i].phandle, &level, cpuset)) {
      char cpu[256];
							...
							unsigned long MCDRAM_numa_size)
{
  ...
  char buffer[512] = {0};
  char *data_beg = NULL;
  char * fallback_elw = getelw("HWLOC_KNL_HDH_FALLBACK");
  int fallback = fallback_elw ? atoi(fallback_elw) : -1; /* by default, only fallback if needed */
	       ...
	       const char *path, unsigned *found)
{
  ...
  if (NULL == nodes_cpuset || NULL == nodes || NULL == distances) {
	...
	char *elw = getelw("HWLOC_KNL_NUMA_QUIRK");
	int noquirk = (elw && !atoi(elw)) || !distances || !hwloc_filter_check_keep_object_type(topology, HWLOC_OBJ_GROUP);
	int mscache;
	unsigned j, closest;
...
#define CPU_TOPOLOGY_STR_LEN 128
  ...
  int threadwithcoreid = data->is_amd_with_LW ? -1 : 0; /* -1 means we don't know yet if threads have their own coreids within thread_siblings */
  ...
  hwloc_bitmap_foreach_begin(i, cpuset) {
	...
	if (hwloc_bitmap_first(packageset) == i) {
	  ...
	  if (merge_buggy_core_siblings) {
	    ...
	    hwloc_obj_t lwrpackage = packages;
	    while (lwrpackage) {
	      if (lwrpackage->os_index == mypackageid) {
		...
		if (!reported && !hwloc_hide_errors()) {
		  char *a, *b;
		  hwloc_bitmap_asprintf(&a, lwrpackage->cpuset);
		...
		hwloc_bitmap_or(lwrpackage->cpuset, lwrpackage->cpuset, packageset);
		goto package_done;
	      }
	      lwrpackage = lwrpackage->next_cousin;
	    }
	  }
  ...
  while (fgets(str,len,fd)!=NULL) {
    ...
    if (end)
      *end = 0;
      ...
      noend = 1;
    /* if empty line, skip and reset lwrproc */
    if (!*str) {
      lwrproc = -1;
      continue;
    }
...
#   define getprocnb_end()						\
    ...
    Lprocs[lwrproc].Pproc = Pproc;
    Lprocs[lwrproc].Pcore = -1;
    Lprocs[lwrproc].Ppkg = -1;
    Lprocs[lwrproc].Lcore = -1;
    Lprocs[lwrproc].Lpkg = -1;
    Lprocs[lwrproc].infos = NULL;
    Lprocs[lwrproc].infos_count = 0;
    getprocnb_end() else
    getprocnb_begin(PACKAGEID, Ppkg);
    Lprocs[lwrproc].Ppkg = Ppkg;
    getprocnb_end() else
    getprocnb_begin(COREID, Pcore);
    Lprocs[lwrproc].Pcore = Pcore;
    getprocnb_end() else {
	...
	parse_cpuinfo_func = hwloc_linux_parse_cpuinfo_generic;
      ...
      parse_cpuinfo_func(prefix, value,
			 lwrproc >= 0 ? &Lprocs[lwrproc].infos : global_infos,
			 lwrproc >= 0 ? &Lprocs[lwrproc].infos_count : global_infos_count,
			 lwrproc < 0);
    }
			 ...
			 struct hwloc_linux_backend_data_s *data)
{
  ...
  elw = getelw("HWLOC_DUMP_NOFILE_INFO");
  if (elw && *elw) {
    file = fopen(elw, "w");
    if (file) {
...
hwloc_linux_try_hardwired_cpuinfo(struct hwloc_backend *backend)
{
  ...
  if (getelw("HWLOC_NO_HARDWIRED_TOPOLOGY"))
    return -1;
...
hwloc_look_linuxfs(struct hwloc_backend *backend)
{
  ...
  if (getelw("HWLOC_LINUX_USE_CPUINFO") || !sysfs_cpu_path) {
				  ...
				  const void *_data3 __hwloc_attribute_unused)
{
  ...
  data->dumped_hwdata_dirname = getelw("HWLOC_DUMPED_HWDATA_DIR");
				    ...
				    struct hwloc_obj *obj, const char *osdevpath)
{
  ...
  if (!hwloc_read_path_by_length(path, line, sizeof(line), root_fd)) {
    if (!strncmp(line, "nd_", 3)) {
      strcpy(blocktype, "LWDIMM"); /* Save the blocktype now since udev reports "" so far */
...
#endif
 {
  ...
  if (!strcmp(blocktype, "disk") || !strncmp(obj->name, "lwme", 4))
    obj->subtype = strdup("Disk");
  else if (!strcmp(blocktype, "LWDIMM")) /* FIXME: set by us above, to workaround udev returning "" so far */
    obj->subtype = strdup("LWDIMM");
...
hwloc_linuxfs_lookup_drm_class(struct hwloc_backend *backend, unsigned osdev_flags)
{
  ...
  while ((dirent = readdir(dir)) != NULL) {
     ...
     *
     * All these aren't very useful compared to LWCA/OpenCL/...
     ...
     * to decide whether card%d could be useful (likely not for LWPU).
     */
...
hwloc_linuxfs_pci_look_pcidevices(struct hwloc_backend *backend)
{
  ...
  /* We could lookup /sys/devices/pci.../.../busid1/.../busid2 relwrsively
...
hwloc_look_linuxfs_io(struct hwloc_backend *backend)
{
 ...
 if (bfilter == HWLOC_TYPE_FILTER_KEEP_NONE
  ...
  if (ofilter != HWLOC_TYPE_FILTER_KEEP_NONE) {
    ...
    if (getelw("HWLOC_VIRTUAL_LINUX_OSDEV"))
