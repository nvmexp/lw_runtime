...
int hwloc_bitmap_snprintf(char * __hwloc_restrict buf, size_t buflen, const struct hwloc_bitmap_s * __hwloc_restrict set)
{
  ...
  int needcomma = 0;
  int i;
  unsigned long aclwm = 0;
  int aclwmed = 0;
#if HWLOC_BITS_PER_LONG == HWLOC_BITMAP_SUBSTRING_SIZE
  const unsigned long aclwm_mask = ~0UL;
#else /* HWLOC_BITS_PER_LONG != HWLOC_BITMAP_SUBSTRING_SIZE */
  const unsigned long aclwm_mask = ((1UL << HWLOC_BITMAP_SUBSTRING_SIZE) - 1) << (HWLOC_BITS_PER_LONG - HWLOC_BITMAP_SUBSTRING_SIZE);
  ...
  while (i>=0 || aclwmed) {
    ...
    if (!aclwmed) {
      aclwm = set->ulongs[i--];
      aclwmed = HWLOC_BITS_PER_LONG;
    }

    if (aclwm & aclwm_mask) {
      /* print the whole subset if not empty */
        res = hwloc_snprintf(tmp, size, needcomma ? ",0x" HWLOC_PRIxSUBBITMAP : "0x" HWLOC_PRIxSUBBITMAP,
		     (aclwm & aclwm_mask) >> (HWLOC_BITS_PER_LONG - HWLOC_BITMAP_SUBSTRING_SIZE));
      needcomma = 1;
    } else if (i == -1 && aclwmed == HWLOC_BITMAP_SUBSTRING_SIZE) {
    ...
    aclwmed = 0;
#else
    aclwm <<= HWLOC_BITMAP_SUBSTRING_SIZE;
    aclwmed -= HWLOC_BITMAP_SUBSTRING_SIZE;
#endif
...
int hwloc_bitmap_sscanf(struct hwloc_bitmap_s *set, const char * __hwloc_restrict string)
{
  ...
  while (*current != '\0') {
    ...
    aclwm |= (val << ((count * HWLOC_BITMAP_SUBSTRING_SIZE) % HWLOC_BITS_PER_LONG));
    if (!(count % HWLOC_BITMAP_STRING_PER_LONG)) {
      set->ulongs[count / HWLOC_BITMAP_STRING_PER_LONG] = aclwm;
      aclwm = 0;
    }
