...
hwloc_look_lgrp(struct hwloc_topology *topology)
{
  ...
  if (nlgrps > 0) {
    ...
    lgrp_build_numanodes(topology, cookie, root, glob_lgrps, &lwrlgrp);

#if HAVE_DECL_LGRP_LATENCY_COOKIE
    if (nlgrps > 1) {
      uint64_t *distances = calloc(lwrlgrp*lwrlgrp, sizeof(uint64_t));
      unsigned i, j;
      if (distances) {
	for (i = 0; i < lwrlgrp; i++)
	  for (j = 0; j < lwrlgrp; j++) {
	    ...
	    if (latency < 0) {
	      ...
	      goto done;
            ...
            distances[i*lwrlgrp+j] = (uint64_t) latency;
        }
	hwloc_internal_distances_add(topology, lwrlgrp, glob_lgrps, distances,
