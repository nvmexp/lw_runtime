...
#include <errno.h>
#include <X11/Xlib.h>
#include <LWCtrl/LWCtrl.h>
#include <LWCtrl/LWCtrlLib.h>
...
hwloc_gl_discover(struct hwloc_backend *backend)
{
  ...
  for (i = 0; i < HWLOC_GL_SERVER_MAX; ++i) {
    ...
    /* Check for LW-CONTROL extension (it's per server) */
    if(!XQueryExtension(display, "LW-CONTROL", &opcode, &event, &error)) {
    ...
    for (j = 0; j < (unsigned) ScreenCount(display) && j < HWLOC_GL_SCREEN_MAX; j++) {
      ...
      int gpu_number;
      int lw_ctrl_pci_bus;
      int lw_ctrl_pci_device;
      int lw_ctrl_pci_domain;
      int lw_ctrl_pci_func;
      char *productname;
      char name[64];

      /* the server supports LW-CONTROL but it may contain non-LWPU screen that don't support it */
      if (!XLWCTRLIsLwScreen(display, screen))
        continue;

      /* Gets the GPU number attached to the default screen. */
      /* For further details, see the <LWCtrl/LWCtrlLib.h> */
      err = XLWCTRLQueryTargetBinaryData (display, LW_CTRL_TARGET_TYPE_X_SCREEN, screen, 0,
                                          LW_CTRL_BINARY_DATA_GPUS_USED_BY_XSCREEN,
...
#ifdef LW_CTRL_PCI_DOMAIN
      /* Gets the ID's of the GPU defined by gpu_number
       * For further details, see the <LWCtrl/LWCtrlLib.h> */
      err = XLWCTRLQueryTargetAttribute(display, LW_CTRL_TARGET_TYPE_GPU, gpu_number, 0,
                                        LW_CTRL_PCI_DOMAIN, &lw_ctrl_pci_domain);
      if (!err)
        continue;
#else
      lw_ctrl_pci_domain = 0;
#endif

      err = XLWCTRLQueryTargetAttribute(display, LW_CTRL_TARGET_TYPE_GPU, gpu_number, 0,
                                        LW_CTRL_PCI_BUS, &lw_ctrl_pci_bus);
      if (!err)
        continue;

      err = XLWCTRLQueryTargetAttribute(display, LW_CTRL_TARGET_TYPE_GPU, gpu_number, 0,
                                        LW_CTRL_PCI_DEVICE, &lw_ctrl_pci_device);
      if (!err)
        continue;

      err = XLWCTRLQueryTargetAttribute(display, LW_CTRL_TARGET_TYPE_GPU, gpu_number, 0,
                                        LW_CTRL_PCI_FUNCTION, &lw_ctrl_pci_func);
      if (!err)
        continue;

      productname = NULL;
      err = XLWCTRLQueryTargetStringAttribute(display, LW_CTRL_TARGET_TYPE_GPU, gpu_number, 0,
                                              LW_CTRL_STRING_PRODUCT_NAME, &productname);
	...
	hwloc_obj_add_info(osdev, "GPUModel", productname);
      ...
      parent = hwloc_pcidisc_find_by_busid(topology, (unsigned)lw_ctrl_pci_domain, (unsigned)lw_ctrl_pci_bus, (unsigned)lw_ctrl_pci_device, (unsigned)lw_ctrl_pci_func);
      if (!parent)
	parent = hwloc_pcidisc_find_busid_parent(topology, (unsigned)lw_ctrl_pci_domain, (unsigned)lw_ctrl_pci_bus, (unsigned)lw_ctrl_pci_device, (unsigned)lw_ctrl_pci_func);
      if (!parent)
	parent = hwloc_get_root_obj(topology);
      ...
      hwloc_debug("GL device %s (product %s) on PCI %04x:%02x:%02x.%01x\n",
		  name, productname,
		  (unsigned) lw_ctrl_pci_domain, (unsigned) lw_ctrl_pci_bus, (unsigned) lw_ctrl_pci_device, (unsigned) lw_ctrl_pci_func);
    }
    XCloseDisplay(display);
  }
