...
int hwloc_hide_errors(void)
{
  ...
  if (!checked) {
    const char *elwvar = getelw("HWLOC_HIDE_ERRORS");
    if (elwvar)
      hide = atoi(elwvar);
    checked = 1;
  }
		    ...
		    struct hwloc_tma *tma)
{
  ...
  /* relwrsively duplicate object children */
 ...
 *
 * Some type won't actually ever be ilwolved in such merging.
 */
...
/* Try to insert OBJ in LWR, relwrse if needed.
 * Returns the object if it was inserted,
 * the remaining object it was merged,
 * NULL if failed to insert.
 */
static struct hwloc_obj *
hwloc___insert_object_by_cpuset(struct hwloc_topology *topology, hwloc_obj_t lwr, hwloc_obj_t obj,
			        hwloc_report_error_t report_error)
{
  ...
  hwloc_obj_t *lwr_children = &lwr->first_child;
   ...
   */
  for (child = lwr->first_child, child ? next_child = child->next_sibling : NULL;
       child;
	...
	/* OBJ is strictly contained is some child of LWR, go deeper.  */
	...
	goto putback;
      ...
      case HWLOC_OBJ_DIFFERENT:
        /* OBJ should be a child of LWR before CHILD, mark its position if not found yet. */
	if (!putp && hwloc__object_cpusets_compare_first(obj, child) < 0)
	  /* Don't insert yet, there could be intersect errors later */
	  putp = lwr_children;
	/* Advance lwr_children.  */
	lwr_children = &child->next_sibling;
	break;

      case HWLOC_OBJ_CONTAINS:
	/* OBJ contains CHILD, remove CHILD from LWR */
	*lwr_children = child->next_sibling;
  ...
  /* lwr/obj_children points to last LWR/OBJ child next_sibling pointer, which must be NULL. */
  assert(!*obj_children);
  assert(!*lwr_children);

  /* Put OBJ where it belongs, or in last in LWR's children.  */
  if (!putp)
    putp = lwr_children;
 ...
 putback:
  /* Put-back OBJ children in LWR and return an error. */
  if (putp)
    lwr_children = putp; /* No need to try to insert before where OBJ was supposed to go */
  else
    lwr_children = &lwr->first_child; /* Start from the beginning */
  ...
  while ((child = obj->first_child) != NULL) {
    ...
    obj->parent = lwr;
    /* Find child position in LWR, and insert. */
    while (*lwr_children && hwloc__object_cpusets_compare_first(*lwr_children, child) < 0)
      lwr_children = &(*lwr_children)->next_sibling;
    child->next_sibling = *lwr_children;
    *lwr_children = child;
  }
  return NULL;
}
			    ...
			    hwloc_report_error_t report_error __hwloc_attribute_unused)
{
  ...
  assert(obj->nodeset);
  lwr_children = &parent->memory_first_child;
  while (*lwr_children) {
...
propagate_nodeset(hwloc_obj_t obj)
{
   ...
   * in that branch of the tree as we relwrse down.
   */
  if (!obj->nodeset)
  ...
  for_each_memory_child(child, obj) {
    /* FIXME rather relwrse in the memory hierarchy */
...
hwloc_level_take_objects(hwloc_obj_t top_obj,
			 hwloc_obj_t *lwrrent_objs, unsigned n_lwrrent_objs,
			 ...
			 hwloc_obj_t *remaining_objs, unsigned n_remaining_objs __hwloc_attribute_unused)
{
  ...
  for (i = 0; i < n_lwrrent_objs; i++)
    if (hwloc_type_cmp(top_obj, lwrrent_objs[i]) == HWLOC_OBJ_EQUAL) {
      /* Take it, add main children.  */
      taken_objs[taken_i++] = lwrrent_objs[i];
      for (j = 0; j < lwrrent_objs[i]->arity; j++)
	remaining_objs[new_i++] = lwrrent_objs[i]->children[j];
    } else {
      /* Leave it.  */
      remaining_objs[new_i++] = lwrrent_objs[i];
    }

#ifdef HWLOC_DEBUG
  /* Make sure we didn't mess up.  */
  assert(taken_i == n_taken_objs);
  assert(new_i == n_lwrrent_objs - n_taken_objs + n_remaining_objs);
#endif
...
hwloc_list_special_objects(hwloc_topology_t topology, hwloc_obj_t obj)
{
  ...
  if (obj->type == HWLOC_OBJ_NUMANODE) {
    ...
    /* Relwrse, Misc only have Misc children */
    for_each_misc_child(child, obj)
      hwloc_list_special_objects(topology, child);
    ...
    /* Relwrse, I/O only have I/O and Misc children */
    for_each_io_child(child, obj)
      hwloc_list_special_objects(topology, child);
    for_each_misc_child(child, obj)
      hwloc_list_special_objects(topology, child);
...
next_cpubackend:
  ...
  if (topology->binding_hooks.get_allowed_resources && topology->is_thissystem) {
    const char *elw = getelw("HWLOC_THISSYSTEM_ALLOWED_RESOURCES");
    if ((elw && atoi(elw))
...
hwloc_topology_load (struct hwloc_topology *topology)
{
  ...
  if (!getelw("HWLOC_COMPONENTS")) {
     ...
     */
    if (!topology->backends) {
      const char *fsroot_path_elw = getelw("HWLOC_FSROOT");
      if (fsroot_path_elw)
	hwloc_disc_component_force_enable(topology,
					  1 /* elw force */,
					  HWLOC_DISC_COMPONENT_TYPE_CPU, "linux",
					  NULL /* backend will getelw again */, NULL, NULL);
    }
    if (!topology->backends) {
      const char *cpuid_path_elw = getelw("HWLOC_CPUID_PATH");
      if (cpuid_path_elw)
	hwloc_disc_component_force_enable(topology,
					  1 /* elw force */,
    ...
    if (!topology->backends) {
      const char *synthetic_elw = getelw("HWLOC_SYNTHETIC");
      if (synthetic_elw)
	hwloc_disc_component_force_enable(topology,
					  1 /* elw force */,
					  -1, "synthetic",
					  synthetic_elw, NULL, NULL);
    }
    if (!topology->backends) {
      const char *xmlpath_elw = getelw("HWLOC_XMLFILE");
      if (xmlpath_elw)
	hwloc_disc_component_force_enable(topology,
					  1 /* elw force */,
					  -1, "xml",
					  xmlpath_elw, NULL, NULL);
    }
  }
   ...
   */
  hwloc_internal_distances_ilwalidate_cached_objs(topology);
  /* And refresh distances so that multithreaded conlwrrent distances_get()
   * don't refresh() conlwrrently (disallowed).
   */
...
hwloc_topology_restrict(struct hwloc_topology *topology, hwloc_const_cpuset_t cpuset, unsigned long flags)
{
  ...
  if (!hwloc_bitmap_intersects(cpuset, topology->allowed_cpuset)) {
    errno = EILWAL; /* easy failure, just don't touch the topology */
    return -1;
  }
  ...
  /* now relwrse to filter sets and drop things */
...
hwloc__check_memory_children(hwloc_topology_t topology, hwloc_bitmap_t gp_indexes, hwloc_obj_t parent)
{
  ...
  for(prev = NULL, child = parent->memory_first_child, j = 0;
    ...
    /* only Memory and Misc children, relwrse */
...
hwloc__check_io_children(hwloc_topology_t topology, hwloc_bitmap_t gp_indexes, hwloc_obj_t parent)
{
  ...
  for(prev = NULL, child = parent->io_first_child, j = 0;
    ...
    /* only I/O and Misc children, relwrse */
    assert(!child->first_child);
...
hwloc__check_object(hwloc_topology_t topology, hwloc_bitmap_t gp_indexes, hwloc_obj_t obj)
{
  ...
  /* nodesets are checked during another relwrsion with state below */
}
...
hwloc__check_nodesets(hwloc_topology_t topology, hwloc_obj_t obj, hwloc_bitmap_t parentset)
{
  ...
  if (obj->type == HWLOC_OBJ_NUMANODE) {
    ...
    /* for each children, relwrse to check/get its contribution */
...
hwloc_topology_check(struct hwloc_topology *topology)
{
  ...
  /* relwrse and check the tree of children, and type-specific checks */
  ...
  /* relwrse and check the nodesets of children */
  set = hwloc_bitmap_alloc();
