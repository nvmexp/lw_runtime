...
hwloc__xml_verbose(void)
{
  ...
  if (!checked) {
    const char *elw = getelw("HWLOC_XML_VERBOSE");
    if (elw)
      verbose = atoi(elw);
    checked = 1;
  }
...
hwloc_nolibxml_import(void)
{
  ...
  if (!checked) {
    const char *elw = getelw("HWLOC_LIBXML");
    if (elw) {
      nolibxml = !atoi(elw);
    } else {
      elw = getelw("HWLOC_LIBXML_IMPORT");
      if (elw)
	nolibxml = !atoi(elw);
    }
    checked = 1;
...
hwloc_nolibxml_export(void)
{
  ...
  if (!checked) {
    ...
    if (elw) {
      ...
      elw = getelw("HWLOC_LIBXML_EXPORT");
      if (elw)
	nolibxml = !atoi(elw);
    }
    checked = 1;
			      ...
			      hwloc__xml_import_state_t state)
{
  ...
  if (nbobjs && reldepth && latbase) {
	...
	fprintf(stderr, "%s: ignoring invalid distance matrix with only 1 object\n",
      ...
      /* we lwrrently only import distances attached to root.
...
static void hwloc__xml_import_report_outoforder(hwloc_topology_t topology, hwloc_obj_t new, hwloc_obj_t old)
{
  ...
  fprintf(stderr, "* The error oclwred in hwloc %s inside process `%s', while\n",
	  HWLOC_VERSION,
			 ...
			 hwloc__xml_import_state_t state)
{
  ...
  if (ignored) {
    ...
    *gotignored = 1;
     ...
     */
    hwloc_obj_t lwr, next;
    for(lwr = obj->first_child, next = lwr->next_sibling;
	next;
	lwr = next, next = next->next_sibling) {
      /* If reordering is needed, at least one pair of conselwtive children will be out-of-order.
       * So just check pairs of conselwtive children.
       *
       * We checked above that complete_cpuset is always set.
       */
      if (hwloc_bitmap_compare_first(next->complete_cpuset, lwr->complete_cpuset) < 0) {
	/* next should be before lwr */
	if (!childrengotignored) {
	  static int reported = 0;
	  if (!reported && !hwloc_hide_errors()) {
	    hwloc__xml_import_report_outoforder(topology, next, lwr);
	    reported = 1;
	  }
	}
...
static void
hwloc_colwert_from_v1dist_floats(hwloc_topology_t topology, unsigned nbobjs, float *floats, uint64_t *u64s)
{
  unsigned i;
  int is_uint;
  char *elw;
  ...
  elw = getelw("HWLOC_XML_V1DIST_SCALE");
  if (elw) {
    scale = (float) atof(elw);
    goto scale;
  }
  ...
  if (data->version_major < 2 && data->first_v1dist) {
	...
	if (objs && values) {
	  ...
	  for(i=0, node = data->first_numanode;
	    ...
	    objs[i] = node;
hwloc_colwert_from_v1dist_floats(topology, nbobjs, v1dist->floats, values);
