...
#ifdef HAVE_PROGRAM_ILWOCATION_NAME
#include <errno.h>
...
#elif defined HAVE_PROGRAM_ILWOCATION_NAME
  name = program_ilwocation_name; /* Glibc. BGQ CNK. */
  /* could use program_ilwocation_short_name directly, but we have the code to remove the path below anyway */
#elif defined HAVE___PROGNAME
  name = __progname; /* fallback for most unix, used for OpenBSD */
#else
  /* TODO: _NSGetExelwtablePath(path, &size) on Darwin */
  /* TODO: AIX, HPUX */
  name = NULL;
#endif
  if (!name)
