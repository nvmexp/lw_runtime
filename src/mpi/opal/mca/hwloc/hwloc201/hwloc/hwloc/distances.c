...
void hwloc_internal_distances_prepare(struct hwloc_topology *topology)
{
  ...
  if (topology->grouping) {
    ...
    HWLOC_BUILD_ASSERT(sizeof(topology->grouping_aclwracies)/sizeof(*topology->grouping_aclwracies) == 5);
    topology->grouping_aclwracies[0] = 0.0f;
    topology->grouping_aclwracies[1] = 0.01f;
    topology->grouping_aclwracies[2] = 0.02f;
    topology->grouping_aclwracies[3] = 0.05f;
    topology->grouping_aclwracies[4] = 0.1f;
    topology->grouping_nbaclwracies = 5;

    hwloc_localeswitch_init();
    elw = getelw("HWLOC_GROUPING_ACLWRACY");
    if (!elw) {
      /* only use 0.0 */
      topology->grouping_nbaclwracies = 1;
    } else if (strcmp(elw, "try")) {
      /* use the given value */
      topology->grouping_nbaclwracies = 1;
      topology->grouping_aclwracies[0] = (float) atof(elw);
    } /* otherwise try all values */
    hwloc_localeswitch_fini();

    topology->grouping_verbose = 0;
    elw = getelw("HWLOC_GROUPING_VERBOSE");
    if (elw)
      topology->grouping_verbose = atoi(elw);
  }
}
...
static void
hwloc__groups_by_distances(struct hwloc_topology *topology, unsigned nbobjs, struct hwloc_obj **objs, uint64_t *values, unsigned long kind, unsigned nbaclwracies, float *aclwracies, int needcheck);
				 ...
				 unsigned long kind, unsigned long flags)
{
  ...
  if (topology->grouping && (flags & HWLOC_DISTANCES_ADD_FLAG_GROUP)) {
    float full_aclwracy = 0.f;
    float *aclwracies;
    unsigned nbaclwracies;

    if (flags & HWLOC_DISTANCES_ADD_FLAG_GROUP_INACLWRATE) {
      aclwracies = topology->grouping_aclwracies;
      nbaclwracies = topology->grouping_nbaclwracies;
    } else {
      aclwracies = &full_aclwracy;
      nbaclwracies = 1;
    }
    ...
    hwloc__groups_by_distances(topology, nbobjs, objs, values,
			       kind, nbaclwracies, aclwracies, 1 /* check the first matrice */);
  }
...
#define HWLOC_DISTANCES_ADD_FLAG_ALL (HWLOC_DISTANCES_ADD_FLAG_GROUP|HWLOC_DISTANCES_ADD_FLAG_GROUP_INACLWRATE)
...
void
hwloc_internal_distances_ilwalidate_cached_objs(hwloc_topology_t topology)
{
  struct hwloc_internal_distances_s *dist;
		     ...
		     unsigned long kind, unsigned long flags __hwloc_attribute_unused)
{
   ...
   * changing the topology, so that future conlwrrent get() won't cause
   * conlwrrent refresh().
   */
...
static void hwloc_report_user_distance_error(const char *msg, int line)
{
    ...
    if (!reported && !hwloc_hide_errors()) {
        ...
        fprintf(stderr, "*\n");
        fprintf(stderr, "* %s\n", msg);
        fprintf(stderr, "* Error oclwrred in topology.c line %d\n", line);
        fprintf(stderr, "*\n");
			   ...
			   int needcheck)
{
  ...
  for(i=0; i<nbaclwracies; i++) {
    if (verbose)
      fprintf(stderr, "Trying to group %u %s objects according to physical distances with accuracy %f\n",
	      nbobjs, hwloc_obj_type_string(objs[0]->type), aclwracies[i]);
    if (needcheck && hwloc__check_grouping_matrix(nbobjs, _values, aclwracies[i], verbose) < 0)
      continue;
    nbgroups = hwloc__find_groups_by_min_distance(nbobjs, _values, aclwracies[i], groupids, verbose);
    if (nbgroups)
      break;
  }
  if (!nbgroups)
...
#define GROUP_VALUE(i, j) groupvalues[(i) * nbgroups + (j)]
	...
	hwloc_debug("%s", "\n");
      ...
      hwloc__groups_by_distances(topology, nbgroups, groupobjs, groupvalues, kind, nbaclwracies, aclwracies, 0 /* no need to check generated matrix */);
  }
}
