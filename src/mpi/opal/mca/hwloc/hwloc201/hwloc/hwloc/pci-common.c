...
static void
hwloc_pci_forced_locality_parse(struct hwloc_topology *topology, const char *_elw)
{
  char *elw = strdup(_elw);
...
hwloc_pci_discovery_prepare(struct hwloc_topology *topology)
{
  ...
  elw = getelw("HWLOC_PCI_LOCALITY");
  if (elw) {
    int fd;

    topology->pci_has_forced_locality = 1;

    fd = open(elw, O_RDONLY);
    if (fd >= 0) {
      struct stat st;
      char *buffer;
      int err = fstat(fd, &st);
	...
	if (st.st_size <= 64*1024) { /* random limit large enough to store multiple cpusets for thousands of PUs */
	  ...
	  fprintf(stderr, "Ignoring HWLOC_PCI_LOCALITY file `%s' too large (%lu bytes)\n",
		  elw, (unsigned long) st.st_size);
	}
      }
      close(fd);
    } else
      hwloc_pci_forced_locality_parse(topology, elw);
  }
}
...
{
  struct hwloc_obj **lwrp, **childp;

  lwrp = parent_io_first_child_p;
  while (*lwrp) {
    enum hwloc_pci_busid_comparison_e comp = hwloc_pci_compare_busids(new, *lwrp);
    switch (comp) {
    case HWLOC_PCI_BUSID_HIGHER:
      /* go further */
      lwrp = &(*lwrp)->next_sibling;
      continue;
    case HWLOC_PCI_BUSID_INCLUDED:
      /* insert new below current bridge */
      hwloc_pci_add_object(*lwrp, &(*lwrp)->io_first_child, new);
      return;
    case HWLOC_PCI_BUSID_LOWER:
    case HWLOC_PCI_BUSID_SUPERSET: {
      /* insert new before current */
      new->next_sibling = *lwrp;
      *lwrp = new;
      new->parent = parent;
      if (new->type == HWLOC_OBJ_BRIDGE) {
	/* look at remaining siblings and move some below new */
	childp = &new->io_first_child;
	lwrp = &new->next_sibling;
	while (*lwrp) {
	  hwloc_obj_t lwr = *lwrp;
	  if (hwloc_pci_compare_busids(new, lwr) == HWLOC_PCI_BUSID_LOWER) {
	    /* this sibling remains under root, after new. */
	    if (lwr->attr->pcidev.domain > new->attr->pcidev.domain
		|| lwr->attr->pcidev.bus > new->attr->bridge.downstream.pci.subordinate_bus)
	      /* this sibling is even above new's subordinate bus, no other sibling could go below new */
	      return;
	    lwrp = &lwr->next_sibling;
	  } else {
...
hwloc_pcidisc_tree_attach(struct hwloc_topology *topology, struct hwloc_obj *old_tree)
{
  ...
  while (old_tree) {
    ...
    unsigned short lwrrent_domain = child->attr->pcidev.domain;
    unsigned char lwrrent_bus = child->attr->pcidev.bus;
    unsigned char lwrrent_subordinate = lwrrent_bus;

    hwloc_debug("Starting new PCI hostbridge %04x:%02x\n", lwrrent_domain, lwrrent_bus);
  ...
  next_child:
    ...
    *dstnextp = child;
    ...
    if (child->type == HWLOC_OBJ_BRIDGE
	&& child->attr->bridge.downstream.pci.subordinate_bus > lwrrent_subordinate)
      lwrrent_subordinate = child->attr->bridge.downstream.pci.subordinate_bus;

    /* use next child if it has the same domains/bus */
    child = *srcnextp;
    if (child
	&& child->attr->pcidev.domain == lwrrent_domain
	&& child->attr->pcidev.bus == lwrrent_bus)
      goto next_child;
    ...
    hostbridge->attr->bridge.downstream.pci.domain = lwrrent_domain;
    hostbridge->attr->bridge.downstream.pci.secondary_bus = lwrrent_bus;
    hostbridge->attr->bridge.downstream.pci.subordinate_bus = lwrrent_subordinate;
    hwloc_debug("New PCI hostbridge %04x:[%02x-%02x]\n",
		lwrrent_domain, lwrrent_bus, lwrrent_subordinate);
...
hwloc__pci_find_busid_parent(struct hwloc_topology *topology, struct hwloc_pcidev_attr_s *busid)
{
  ...
  if (!forced) {
    ...
    snprintf(elwname, sizeof(elwname), "HWLOC_PCI_%04x_%02x_LOCALCPUS",
	     busid->domain, busid->bus);
    elw = getelw(elwname);
    if (elw) {
	...
	hwloc_bitmap_sscanf(cpuset, elw);
	forced = 1;
      }
      /* if elw exists, even empty, don't let quirks change what the OS reports */
      noquirks = 1;
    }
  }
