...
{
  const char *verboseelw;
  const char *path = HWLOC_PLUGINS_PATH;
  const char *elw;
  int err;

  verboseelw = getelw("HWLOC_PLUGINS_VERBOSE");
  hwloc_plugins_verbose = verboseelw ? atoi(verboseelw) : 0;

  hwloc_plugins_blacklist = getelw("HWLOC_PLUGINS_BLACKLIST");

  err = lt_dlinit();
  if (err)
    goto out;

  elw = getelw("HWLOC_PLUGINS_PATH");
  if (elw)
    path = elw;
...
hwloc_components_init(void)
{
  ...
  verboseelw = getelw("HWLOC_COMPONENTS_VERBOSE");
  hwloc_components_verbose = verboseelw ? atoi(verboseelw) : 0;
				  ...
				  const void *data1, const void *data2, const void *data3)
{
  ...
  if (backend) {
    backend->elwvar_forced = elwvar_forced;
				...
				int elwvar_forced)
{
  ...
  if (topology->backend_excludes & comp->type) {
    if (hwloc_components_verbose)
      /* do not warn if elwvar_forced since system-wide HWLOC_COMPONENTS must be silently ignored after set_xml() etc.
       */
  ...
  if (!backend) {
    if (hwloc_components_verbose || elwvar_forced)
...
hwloc_disc_components_enable_others(struct hwloc_topology *topology)
{
  ...
  int tryall = 1;
  const char *_elw;
  char *elw; /* we'll to modify the elw value, so duplicate it */

  _elw = getelw("HWLOC_COMPONENTS");
  elw = _elw ? strdup(_elw) : NULL;
  ...
  if (elw) {
    ...
    while (*lwrelw) {
      s = strcspn(lwrelw, HWLOC_COMPONENT_SEPS);
      if (s) {
	char c;

	/* replace linuxpci with linuxio for backward compatibility with pre-v2.0 */
	if (!strncmp(lwrelw, "linuxpci", 8) && s == 8) {
	  lwrelw[5] = 'i';
	  lwrelw[6] = 'o';
	  lwrelw[7] = *HWLOC_COMPONENT_SEPS;
	} else if (lwrelw[0] == HWLOC_COMPONENT_EXCLUDE_CHAR && !strncmp(lwrelw+1, "linuxpci", 8) && s == 9) {
	  lwrelw[6] = 'i';
	  lwrelw[7] = 'o';
	  lwrelw[8] = *HWLOC_COMPONENT_SEPS;
	  /* skip this name, it's a negated one */
	  goto nextname;
	}

	if (lwrelw[0] == HWLOC_COMPONENT_EXCLUDE_CHAR)
	  goto nextname;

	if (!strncmp(lwrelw, HWLOC_COMPONENT_STOP_NAME, s)) {
	  tryall = 0;
	  break;
	}
	...
	comp = hwloc_disc_component_find(-1, lwrelw);
	if (comp) {
	  hwloc_disc_component_try_enable(topology, comp, NULL, 1 /* elwvar forced */);
	} else {
	  fprintf(stderr, "Cannot find discovery component `%s'\n", lwrelw);
	}

	/* restore chars (the second loop below needs elw to be unmodified) */
	lwrelw[s] = c;
      }
...
nextname:
  ...
  /* elw is still the same, the above loop didn't modify it */
  ...
  if (tryall) {
	...
	goto nextcomp;
      /* check if this component was explicitly excluded in elw */
      if (elw) {
	char *lwrelw = elw;
	while (*lwrelw) {
	  size_t s = strcspn(lwrelw, HWLOC_COMPONENT_SEPS);
	  if (lwrelw[0] == HWLOC_COMPONENT_EXCLUDE_CHAR && !strncmp(lwrelw+1, comp->name, s-1) && strlen(comp->name) == s-1) {
	    if (hwloc_components_verbose)
      ...
      hwloc_disc_component_try_enable(topology, comp, NULL, 0 /* defaults, not elwvar forced */);
nextcomp:
      comp = comp->next;
    }
  }
...
{
  struct hwloc_backend *backend;
  const char *local_elw;

  /* Apply is_thissystem topology flag before we enforce elwvar backends.
   ...
   * it may use HWLOC_THISSYSTEM elwvar below as well.
   */

  topology->is_thissystem = 1;

  /* apply thissystem from normally-given backends (elwvar_forced=0, either set_foo() or defaults) */
  backend = topology->backends;
  while (backend != NULL) {
    if (backend->elwvar_forced == 0 && backend->is_thissystem != -1) {
  ...
  /* now apply elwvar-forced backend (elwvar_forced=1) */
  backend = topology->backends;
  while (backend != NULL) {
    if (backend->elwvar_forced == 1 && backend->is_thissystem != -1) {
      assert(backend->is_thissystem == 0);
      topology->is_thissystem = 0;
    }
    backend = backend->next;
  ...
  /* override with elwvar-given flag */
  local_elw = getelw("HWLOC_THISSYSTEM");
  if (local_elw)
    topology->is_thissystem = atoi(local_elw);
}
