...
/* by default, do not cleanup to avoid issues with conlwrrent libxml users */
...
hwloc_libxml2_init_once(void)
{
  ...
  if (!checked) {
    ...
    if (getelw("HWLOC_LIBXML_CLEANUP"))
...
typedef struct hwloc__libxml_export_state_data_s {
  xmlNodePtr lwrrent_node; /* current node to output */
			       ...
			       const char *name)
{
  ...
  ldata->lwrrent_node = xmlNewChild(lpdata->lwrrent_node, NULL, BAD_CAST name, NULL);
}
...
{
  hwloc__libxml_export_state_data_t ldata = (void *) state->data;
  xmlNewProp(ldata->lwrrent_node, BAD_CAST name, BAD_CAST value);
}
...
{
  hwloc__libxml_export_state_data_t ldata = (void *) state->data;
  xmlNodeAddContentLen(ldata->lwrrent_node, BAD_CAST buffer, length);
}
