				...
				int verbose)
{
  ...
  if (i == length) {
...

   {
	...
	loops[lwr_loop].step = step;
	loops[lwr_loop].nb = nb;
	if (step < minstep)
	  minstep = step;
	nbs *= nb;
	lwr_loop++;
	...
	for(i=0; ; i++) {
	  if (!data->level[i].arity) {
	    loops[lwr_loop].level_depth = (unsigned)-1;
	    break;
	  }
	  ...
	  if (type == HWLOC_OBJ_GROUP
	    ...
	    continue;
	  loops[lwr_loop].level_depth = (unsigned)i;
	  break;
	}
	if (loops[lwr_loop].level_depth == (unsigned)-1) {
	  if (verbose)
	...
	lwr_loop++;
      ...
      for(lwr_loop=0; lwr_loop<nr_loops; lwr_loop++) {
	unsigned mydepth = loops[lwr_loop].level_depth;
	unsigned prevdepth = 0;
	unsigned step, nb;
	for(i=0; i<nr_loops; i++) {
	  if (loops[i].level_depth == mydepth && i != lwr_loop) {
	    if (verbose)
			    ...
			    int verbose)
{
  ...
  if (index_string) {
    if (sind->string && verbose)
      fprintf(stderr, "Overwriting duplicate indexes attribute with last oclwrence\n");
...
hwloc_synthetic_free_levels(struct hwloc_synthetic_backend_data_s *data)
{
  ...
  for(i=0; i<HWLOC_SYNTHETIC_MAX_DEPTH; i++) {
    struct hwloc_synthetic_level_data_s *lwrlevel = &data->level[i];
    struct hwloc_synthetic_attached_s **pprev = &lwrlevel->attached;
    while (*pprev) {
      struct hwloc_synthetic_attached_s *lwr = *pprev;
      *pprev = lwr->next;
      free(lwr);
    }
    free(lwrlevel->indexes.array);
    if (!lwrlevel->arity)
      break;
  }
			     ...
			     const char *description)
{
  ...
  unsigned unset;
  int verbose = 0;
  const char *elw = getelw("HWLOC_SYNTHETIC_VERBOSE");
  int err;
  ...
  for (i=0; i<count; i++) {
    struct hwloc_synthetic_level_data_s *lwrlevel = &data->level[i];
    hwloc_obj_type_t type = lwrlevel->attr.type;

    if (type == HWLOC_OBJ_GROUP) {
      if (lwrlevel->attr.depth == (unsigned)-1)
	lwrlevel->attr.depth = type_count[HWLOC_OBJ_GROUP]--;

    } else if (hwloc__obj_type_is_cache(type)) {
      if (!lwrlevel->attr.memorysize) {
	if (1 == lwrlevel->attr.depth)
	  /* 32Kb in L1 */
	  lwrlevel->attr.memorysize = 32*1024;
	else
	  /* *4 at each level, starting from 1MB for L2, unified */
	  lwrlevel->attr.memorysize = 256*1024 << (2*lwrlevel->attr.depth);
      }

    } else if (type == HWLOC_OBJ_NUMANODE && !lwrlevel->attr.memorysize) {
      /* 1GB in memory nodes. */
      lwrlevel->attr.memorysize = 1024*1024*1024;
    }
...
/*
 * Relwrsively build objects whose cpu start at first_cpu
		      ...
		      hwloc_bitmap_t parent_cpuset)
{
  hwloc_obj_t obj;
  unsigned i;
  struct hwloc_synthetic_level_data_s *lwrlevel = &data->level[level];
  hwloc_obj_type_t type = lwrlevel->attr.type;
  hwloc_bitmap_t set;
  unsigned os_index;
  ...
  os_index = hwloc_synthetic_next_index(&lwrlevel->indexes, type);

  set = hwloc_bitmap_alloc();
  if (!lwrlevel->arity) {
    hwloc_bitmap_set(set, os_index);
  } else {
    for (i = 0; i < lwrlevel->arity; i++)
  ...
  if (hwloc_filter_check_keep_object_type(topology, type)) {
    ...
    hwloc_synthetic_set_attr(&lwrlevel->attr, obj);

    hwloc_insert_object_by_cpuset(topology, obj);
  }

  hwloc_synthetic_insert_attached(topology, data, lwrlevel->attached, set);
				      ...
				      const void *_data3 __hwloc_attribute_unused)
{
  ...
  if (!_data1) {
    const char *elw = getelw("HWLOC_SYNTHETIC");
    if (elw) {
				...
				char *buffer, size_t buflen)
{
 ...
 exportall:
  ...
  lwr = level[0];
  while (lwr) {
    res = snprintf(tmp, tmplen, "%u%s", lwr->os_index,
		   lwr->next_cousin ? "," : ")");
    if (hwloc__export_synthetic_update_status(&ret, &tmp, &tmplen, res) < 0)
      return -1;
    lwr = lwr->next_cousin;
				 ...
				 char *buffer, size_t buflen)
{
  ...
  if (!obj->logical_index /* only display indexes once per level (not for non-first NUMA children, etc.) */
    ...
    hwloc_obj_t lwr = obj;
    while (lwr) {
      if (lwr->os_index != lwr->logical_index) {
	needindexes = 1;
	break;
      }
      lwr = lwr->next_cousin;
