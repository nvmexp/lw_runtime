...
Lwrrently, the "core" class is only needed for OPL systems
*****************************************************************************/
...
static void probe_picl(void)
{
   ...
   *
   * Use this elw var to disable the PICL homogeneous-only parsing in the meantime.
   */
  elw = getelw("HWLOC_PICL_HETEROGENEOUS");
  if (elw && atoi(elw))
    return;
