...
# Probably only ever ilwoked by hwloc's configure.ac
...
AC_DEFUN([HWLOC_DEFINE_ARGS],[
    ...
    AC_ARG_ENABLE([io],
                  AS_HELP_STRING([--disable-io],
                                 [Disable I/O discovery entirely (PCI, LinuxIO, LWCA, OpenCL, LWML, GL)]))
    ...
    # LWCA?
    AC_ARG_ENABLE([lwca],
                  AS_HELP_STRING([--disable-lwca],
                                 [Disable the LWCA device discovery using liblwdart]))

    # LWML?
    AC_ARG_ENABLE([lwml],
                  AS_HELP_STRING([--disable-lwml],
                                 [Disable the LWML device discovery]))
...
dnl Distribution tarballs just install pre-built dolwemntation that was
...
EOF
    ...
    if test "$enable_picky" = "yes"; then
        if test "$GCC" = "yes"; then
            ...
            AC_MSG_RESULT([no])
            AC_MSG_WARN([Warning: --enable-picky used, but is lwrrently only defined for the GCC compiler set -- automatically disabled])
            hwloc_want_picky=0
        fi
...
EOF
    ...
    hwloc_old_LIBS="$LIBS"
    chosen_lwrses=""
    for lwrses in nlwrses lwrses
    do
      for lib in "" -ltermcap -l${lwrses}w -l$lwrses
      do
        AC_MSG_CHECKING(termcap support using $lwrses and $lib)
...
#include <term.h>
          ...
          chosen_lwrses=$lwrses
        ], [
          AC_MSG_RESULT(no)
        ])
        test "x$chosen_lwrses" != "x" && break
      done
      test "x$chosen_lwrses" != "x" && break
    done
    if test "$chosen_lwrses" = nlwrses
    then
      AC_DEFINE([HWLOC_USE_NLWRSES], [1], [Define to 1 if nlwrses works, preferred over lwrses])
    fi
    LIBS="$hwloc_old_LIBS"
...
chmod +x ]hwloc_config_prefix[tests/hwloc/linux/test-topology.sh \
	...
	hwloc_config_prefix[tests/hwloc/ports/topology-lwca.c]:hwloc_config_prefix[hwloc/topology-lwca.c]
	hwloc_config_prefix[tests/hwloc/ports/topology-lwml.c]:hwloc_config_prefix[hwloc/topology-lwml.c]
