...
enum hwloc_distances_kind_e {
   ...
   * of the matrix (distance between an object and itself).
   * Such values are lwrrently ignored for distance-based grouping.
   * \hideinitializer
   */
 ...
 *
 * \p flags is lwrrently unused, should be \c 0.
 *
 ...
 *
 * \return -1 if object \p obj is not ilwolved in structure \p distances.
 */
static __hwloc_inline int
 ...
 *
 * \return -1 if object \p obj1 or \p obj2 is not ilwolved in structure \p distances.
 */
static __hwloc_inline int
...
enum hwloc_distances_add_flag_e {
  ...
  /** \brief If grouping, consider the distance values as inaclwrate and relax the
   * comparisons during the grouping algorithms. The actual accuracy may be modified
   * through the HWLOC_GROUPING_ACLWRACY environment variable (see \ref elwvar).
   * \hideinitializer
   */
  HWLOC_DISTANCES_ADD_FLAG_GROUP_INACLWRATE = (1UL<<1)
};
