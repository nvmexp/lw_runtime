 ...
 *
 * This is colwenient for iterating over all largest objects within a CPU set
 ...
 * included in \p set.  The next ilwokation should pass the previous
 ...
 * ilwokation should pass the previous return value in \p prev so as
 ...
 * type covering at least part of CPU set \p set.  The next ilwokation
 ...
 *
 * This function is useful for colwerting a CPU set into the PU
 * objects it contains.
 ...
 *
 * This function is useful for colwerting a nodeset into the NUMA node
 * objects it contains.
 ...
 *
 * Array \p set will be filled with \p n cpusets relwrsively distributed
	      ...
	      int until, unsigned long flags)
{
  ...
  for (i = 0, given = 0, givenweight = 0; i < n_roots; i++) {
	...
	hwloc_bitmap_or(cpusetp[-1], cpusetp[-1], cpuset);
      ...
      /* Still more to distribute, relwrse into children */
...
/** \defgroup hwlocality_helper_nodeset_colwert Colwerting between CPU sets and node sets
 *
 * @{
 */

/** \brief Colwert a CPU set into a NUMA node set and handle non-NUMA cases
 *
...
/** \brief Colwert a NUMA node set into a CPU set and handle non-NUMA cases
 *
 * If the topology contains no NUMA nodes, the machine is considered
 * as a single memory node, and the following behavior is used:
