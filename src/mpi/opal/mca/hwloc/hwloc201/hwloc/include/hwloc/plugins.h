...
struct hwloc_backend {
  ...
  /** \private Reserved for the core. Set to 1 if forced through elwvar, 0 otherwise. */
  int elwvar_forced;
  /** \private Reserved for the core. Used internally to list backends topology->backends. */
  struct hwloc_backend * next;

  /** \brief Backend flags, lwrrently always 0. */
  unsigned long flags;
 ...
 *
 * \p flags is lwrrently unused, must 0.
 */
...
hwloc_plugin_check_namespace(const char *pluginname __hwloc_attribute_unused, const char *symbol __hwloc_attribute_unused)
{
  ...
  lt_dlclose(handle);
  if (!sym) {
    static int verboseelw_checked = 0;
    static int verboseelw_value = 0;
    if (!verboseelw_checked) {
      const char *verboseelw = getelw("HWLOC_PLUGINS_VERBOSE");
      verboseelw_value = verboseelw ? atoi(verboseelw) : 0;
      verboseelw_checked = 1;
    }
    if (verboseelw_value)
