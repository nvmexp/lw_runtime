...
typedef enum {
    ...
    NETLOC_NETWORK_TYPE_ILWALID     = 3  /**< Invalid network */
 ...
 */
typedef enum {
    NETLOC_TOPOLOGY_TYPE_ILWALID = -1, /**< Invalid */
...
typedef enum {
    ...
    NETLOC_NODE_TYPE_ILWALID = 2  /**< Invalid node */
} netloc_node_type_t;
 ...
 * be entered into the data store without cirlwlar references.
...
struct netloc_arch_node_t {
    ...
    int num_lwrrent_slots; /* Number of PUs */
    NETLOC_int *lwrrent_slots; /* indices in the complete tree */
...
struct netloc_arch_t {
    ...
    NETLOC_int num_lwrrent_hosts; /* if has_slots, host is a slot, else host is a node */
    NETLOC_int *lwrrent_hosts; /* indices in the complete topology */
};
...
int netloc_arch_set_lwrrent_resources(netloc_arch_t *arch);
