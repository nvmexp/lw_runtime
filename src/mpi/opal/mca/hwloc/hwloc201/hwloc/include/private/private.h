...
#define HWLOC_SLEVEL_TO_DEPTH(x) (HWLOC_TYPE_DEPTH_NUMANODE-(x))
  ...
  struct hwloc_binding_hooks {
     ...
     * see hwloc_alloc_or_fail which is colwenient for that.  */
  ...
  struct hwloc_internal_distances_s {
    ...
    unsigned nbobjs;
    uint64_t *indexes; /* array of OS or GP indexes before we can colwert them into objs. */
    ...
    /* objects are lwrrently stored in physical_index order */
  ...
  unsigned grouping_nbaclwracies;
  float grouping_aclwracies[5];
...
/* Move PCI objects lwrrently attached to the root object ot their actual location.
...
extern void hwloc_internal_distances_ilwalidate_cached_objs(hwloc_topology_t topology);
...
/* Return the name of the lwrrently running program, if supported.
