...
AC_DEFUN([MCA_opal_hwloc_hwloc201_CONFIG],[
    ...
    # disable lwca support
    enable_lwda=no

    # Open MPI lwrrently does not use hwloc's LWML support
    enable_lwml=no
    ...
    AS_IF([test -n "$opal_datatype_lwda_CPPFLAGS"],
          [CPPFLAGS="$CPPFLAGS $opal_datatype_lwda_CPPFLAGS"])
    ...
    # Restore some elw variables, if necessary
