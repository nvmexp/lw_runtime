 ...
 * provider. However, if an error oclwrs we can no longer guarantee
...
{
    struct fi_info *provider = provider_list, *lwrrent_provider = provider_list;
    ...
    lwrrent_provider = provider;

    /* Cycle through remaining fi_info objects, looking for alike providers */
    while (NULL != lwrrent_provider) {
        if (!check_provider_attr(provider, lwrrent_provider)) {
            cpusets_match = false;
#if OPAL_OFI_PCI_DATA_AVAILABLE
            if (NULL != lwrrent_provider->nic) {
                pci = lwrrent_provider->nic->bus_attr->attr.pci;
             ...
             */
            if (cpusets_match || !provider_found) {
                provider_table[num_provider] = lwrrent_provider;
                num_provider++;
            }
        }
        lwrrent_provider = lwrrent_provider->next;
    }
