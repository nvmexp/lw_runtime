                             ...
                             opal_crs_state_type_t *state)
{
    ...
    if( options->stop ) {
        opal_output(0,
                    "crs:self: checkpoint(): Error: SIGSTOP Not lwrrently supported!");
    }
...
static int crs_self_find_function(char *prefix, char *suffix,
     ...
     * us to not have to dlopen/dlclose the exelwtable. A bit of short hand
     * really.
     */
...
static int opal_crs_self_restart_cmd(opal_crs_self_snapshot_t *snapshot, char **cmd)
{
    ...
    (void) mca_base_var_elw_name("crs", &tmp_elw_var);
    opal_setelw(tmp_elw_var,
                "self",
                true, &elwiron);
    free(tmp_elw_var);
    ...
    (void) mca_base_var_elw_name("crs_self_do_restart", &tmp_elw_var);
    opal_setelw(tmp_elw_var,
                "1",
                true, &elwiron);
    free(tmp_elw_var);
    ...
    (void) mca_base_var_elw_name("crs_self_prefix", &tmp_elw_var);
    opal_setelw(tmp_elw_var,
