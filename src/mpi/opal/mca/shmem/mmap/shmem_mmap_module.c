...
shmem_ds_reset(opal_shmem_ds_t *ds_buf)
{
    ...
    ds_buf->seg_id = OPAL_SHMEM_DS_ID_ILWALID;
    ds_buf->seg_size = 0;
               ...
               size_t size)
{
    ...
    if (OPAL_SUCCESS != (rc = enough_space(real_file_name,
                                           ...
                                           &amount_space_avail,
                                           &space_available))) {
        opal_output(0, "shmem: mmap: an error oclwrred while determining "
...
out:
    ...
    /* an error oclwred, so ilwalidate the shmem object and munmap if needed */
    if (OPAL_SUCCESS != rc) {
...
segment_unlink(opal_shmem_ds_t *ds_buf)
{
    ...
    OPAL_SHMEM_DS_ILWALIDATE(ds_buf);
    return OPAL_SUCCESS;
}
