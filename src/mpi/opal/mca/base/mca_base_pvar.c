...
static int mca_base_pvar_get_internal (int index, mca_base_pvar_t **pvar, bool ilwalidok);
                            ...
                            mca_base_set_value_fn_t set_value, mca_base_notify_fn_t notify, void *ctx)
{
    ...
    flags &= ~MCA_BASE_PVAR_FLAG_ILWALID;
...
int mca_base_component_pvar_register (const mca_base_component_t *component, const char *name,
                                      ...
                                      mca_base_set_value_fn_t set_value, mca_base_notify_fn_t notify, void *ctx)
{
    /* ilwalidate this variable if the component's group is deregistered */
...
static int mca_base_pvar_get_internal (int index, mca_base_pvar_t **pvar, bool ilwalidok)
{
    ...
    *pvar = opal_pointer_array_get_item (&registered_pvars, index);
    ...
    if (((*pvar)->flags & MCA_BASE_PVAR_FLAG_ILWALID) && !ilwalidok) {
        *pvar = NULL;
        return OPAL_ERR_VALUE_OUT_OF_BOUNDS;
    }
...
int mca_base_pvar_mark_ilwalid (int index)
{
    mca_base_pvar_t *pvar;
    int ret;
    ...
    pvar->flags |= MCA_BASE_PVAR_FLAG_ILWALID;
...
{
    if (mca_base_pvar_is_ilwalid (handle->pvar)) {
        ...
        *handle = pvar_handle;
        ...
        if (!mca_base_pvar_is_continuous (pvar) || mca_base_pvar_is_sum (pvar) ||
            ...
            /* if a variable is not continuous we will need to keep track of its last value
               ...
               value to 0. */
            pvar_handle->lwrrent_value = calloc (*count, datatype_size);
            if (NULL == pvar_handle->lwrrent_value) {
                ret = OPAL_ERR_OUT_OF_RESOURCE;
                break;
            }
        }
        ...
        if (mca_base_pvar_is_sum (pvar) || mca_base_pvar_is_watermark (pvar)) {
            ...
            if (mca_base_pvar_is_continuous (pvar)) {
                if (mca_base_pvar_is_sum (pvar)) {
                    ...
                    ret = pvar->get_value (pvar, pvar_handle->lwrrent_value, pvar_handle->obj_handle);
                }
...
int mca_base_pvar_handle_update (mca_base_pvar_handle_t *handle)
{
    ...
    if (mca_base_pvar_is_sum (handle->pvar) || mca_base_pvar_is_watermark (handle->pvar)) {
        ...
        if (mca_base_pvar_is_sum (handle->pvar)) {
            for (i = 0 ; i < handle->count ; ++i) {
                ...
                case MCA_BASE_VAR_TYPE_UNSIGNED_INT:
                    ((unsigned *) handle->lwrrent_value)[i] += ((unsigned *) handle->tmp_value)[i] -
                        ((unsigned *) handle->last_value)[i];
                    break;
                case MCA_BASE_VAR_TYPE_UNSIGNED_LONG:
                    ((unsigned long *) handle->lwrrent_value)[i] += ((unsigned long *) handle->tmp_value)[i] -
                        ((unsigned long *) handle->last_value)[i];
                    break;
                case MCA_BASE_VAR_TYPE_UNSIGNED_LONG_LONG:
                    ((unsigned long long *) handle->lwrrent_value)[i] += ((unsigned long long *) handle->tmp_value)[i] -
                        ((unsigned long long *) handle->last_value)[i];
                    break;
                case MCA_BASE_VAR_TYPE_DOUBLE:
                    ((double *) handle->lwrrent_value)[i] += ((double *) handle->tmp_value)[i] -
            ...
            for (i = 0 ; i < handle->count ; ++i) {
                if (MCA_BASE_PVAR_CLASS_LOWWATERMARK == handle->pvar->var_class) {
                    switch (handle->pvar->type) {
                    case MCA_BASE_VAR_TYPE_UNSIGNED_INT:
                        ((unsigned *) handle->lwrrent_value)[i] = min(((unsigned *) handle->tmp_value)[i],
                                                                      ((unsigned *) handle->lwrrent_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_UNSIGNED_LONG:
                        ((unsigned long *) handle->lwrrent_value)[i] = min(((unsigned long *) handle->tmp_value)[i],
                                                                           ((unsigned long *) handle->lwrrent_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_UNSIGNED_LONG_LONG:
                        ((unsigned long long *) handle->lwrrent_value)[i] = min(((unsigned long long *) handle->tmp_value)[i],
                                                                                ((unsigned long long *) handle->lwrrent_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_DOUBLE:
                        ((double *) handle->lwrrent_value)[i] = min(((double *) handle->tmp_value)[i],
                                                                    ((double *) handle->lwrrent_value)[i]);
                        break;
                    default:
                        /* shouldn't happen */
                        break;
                    ...
                    switch (handle->pvar->type) {
                    case MCA_BASE_VAR_TYPE_UNSIGNED_INT:
                        ((unsigned *) handle->lwrrent_value)[i] = max(((unsigned *) handle->tmp_value)[i],
                                                                      ((unsigned *) handle->lwrrent_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_UNSIGNED_LONG:
                        ((unsigned long *) handle->lwrrent_value)[i] = max(((unsigned long *) handle->tmp_value)[i],
                                                                           ((unsigned long *) handle->lwrrent_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_UNSIGNED_LONG_LONG:
                        ((unsigned long long *) handle->lwrrent_value)[i] = max(((unsigned long long *) handle->tmp_value)[i],
                                                                                ((unsigned long long *) handle->lwrrent_value)[i]);
                        break;
                    case MCA_BASE_VAR_TYPE_DOUBLE:
                        ((double *) handle->lwrrent_value)[i] = max(((double *) handle->tmp_value)[i],
                                                                    ((double *) handle->lwrrent_value)[i]);
                        break;
                    default:
                        /* shouldn't happen */
        ...
        ret = handle->pvar->get_value (handle->pvar, handle->lwrrent_value, handle->obj_handle);
        if (OPAL_SUCCESS != ret) {
            return ret;
        }
    }

    /* XXX -- TODO -- For watermarks this function will have to be ilwoked for each handle whenever the underlying value is updated. */
...
int mca_base_pvar_handle_read_value (mca_base_pvar_handle_t *handle, void *value)
{
    ...
    if (mca_base_pvar_is_sum (handle->pvar) || mca_base_pvar_is_watermark (handle->pvar) ||
        ...
        memmove (value, handle->lwrrent_value, handle->count * ompi_var_type_sizes[handle->pvar->type]);
    } else {
...
int mca_base_pvar_handle_write_value (mca_base_pvar_handle_t *handle, const void *value)
{
    ...
    memmove (handle->lwrrent_value, value, handle->count * ompi_var_type_sizes[handle->pvar->type]);
    /* read the value directly from the variable. */
    ret = handle->pvar->set_value (handle->pvar, value, handle->obj_handle);
...
int mca_base_pvar_handle_reset (mca_base_pvar_handle_t *handle)
{
    ...
    if (mca_base_pvar_is_sum (handle->pvar)) {
        ...
        memset (handle->lwrrent_value, 0, handle->count * ompi_var_type_sizes[handle->pvar->type]);
...
static void mca_base_pvar_handle_destructor (mca_base_pvar_handle_t *handle)
{
    ...
    if (NULL != handle->lwrrent_value) {
        free (handle->lwrrent_value);
    }
