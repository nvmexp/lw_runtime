...
static char *mca_base_elwar_files = NULL;
...
static char *mca_base_elwar_file_prefix = NULL;
static char *mca_base_param_file_path = NULL;
char *mca_base_elw_list = NULL;
char *mca_base_elw_list_sep = MCA_BASE_ELW_LIST_SEP_DEFAULT;
char *mca_base_elw_list_internal = NULL;
static bool mca_base_var_suppress_override_warning = false;
static opal_list_t mca_base_var_file_values;
static opal_list_t mca_base_elwar_file_values;
...
int mca_base_var_init(void)
{
    ...
    if (!mca_base_var_initialized) {
        ...
        OBJ_CONSTRUCT(&mca_base_elwar_file_values, opal_list_t);
...
static void process_elw_list(char *elw_list, char ***argv, char sep)
{
    char** tokens;
    char *ptr, *value;

    tokens = opal_argv_split(elw_list, (int)sep);
    if (NULL == tokens) {
        return;
    }
    ...
    for (int i = 0 ; NULL != tokens[i] ; ++i) {
        if (NULL == (ptr = strchr(tokens[i], '='))) {
            value = getelw(tokens[i]);
            if (NULL == value) {
                opal_show_help("help-mca-var.txt", "incorrect-elw-list-param",
                               true, tokens[i], elw_list);
                break;
            }
            ...
            if (NULL != (ptr = strchr(value, '='))) {
                *ptr = '\0';
                opal_setelw(value, ptr + 1, true, argv);
            } else {
                opal_setelw(tokens[i], value, true, argv);
            }

            free (value);
        } else {
            *ptr = '\0';
            opal_setelw(tokens[i], ptr + 1, true, argv);
...
int mca_base_var_process_elw_list(char *list, char ***argv)
{
    char sep;
    sep = ';';
    if (NULL != mca_base_elw_list_sep) {
        if (1 == strlen(mca_base_elw_list_sep)) {
            sep = mca_base_elw_list_sep[0];
        } else {
            opal_show_help("help-mca-var.txt", "incorrect-elw-list-sep",
                    true, mca_base_elw_list_sep);
            return OPAL_SUCCESS;
        }
    }
    if (NULL != list) {
        process_elw_list(list, argv, sep);
    } else if (NULL != mca_base_elw_list) {
        process_elw_list(mca_base_elw_list, argv, sep);
    }

    return OPAL_SUCCESS;
}

int mca_base_var_process_elw_list_from_file(char ***argv)
{
    if (NULL != mca_base_elw_list_internal) {
        process_elw_list(mca_base_elw_list_internal, argv, ';');
    }
    return OPAL_SUCCESS;
}
...
int mca_base_var_cache_files(bool rel_path_search)
{
    ...
    mca_base_elwar_files = strdup(mca_base_var_files);
    ...
    mca_base_elwar_file_prefix = NULL;
    ret = mca_base_var_register ("opal", "mca", "base", "elwar_file_prefix",
                                 "Aggregate MCA parameter file set for elw variables",
                                 MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0, OPAL_INFO_LVL_3,
                                 MCA_BASE_VAR_SCOPE_READONLY, &mca_base_elwar_file_prefix);
    if (0 > ret) {
        return ret;
    }

    ret = asprintf(&mca_base_param_file_path, "%s" OPAL_PATH_SEP "amca-param-sets%c%s",
                   opal_install_dirs.opaldatadir, OPAL_ELW_SEP, cwd);
    if (0 > ret) {
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    if (NULL != force_agg_path) {
        if (NULL != mca_base_param_file_path) {
            ...
            asprintf(&mca_base_param_file_path, "%s%c%s", force_agg_path, OPAL_ELW_SEP, tmp_str);
            free(tmp_str);
        } else {
    ...
    if (NULL != mca_base_var_file_prefix) {
       resolve_relative_paths(&mca_base_var_file_prefix, mca_base_param_file_path, rel_path_search, &mca_base_var_files, OPAL_ELW_SEP);
    }
    read_files (mca_base_var_files, &mca_base_var_file_values, ',');

    if (NULL != mca_base_elwar_file_prefix) {
       resolve_relative_paths(&mca_base_elwar_file_prefix, mca_base_param_file_path, rel_path_search, &mca_base_elwar_files, ',');
    }
    read_files (mca_base_elwar_files, &mca_base_elwar_file_values, ',');

    if (0 == access(mca_base_var_override_file, F_OK)) {
        read_files (mca_base_var_override_file, &mca_base_var_override_values, OPAL_ELW_SEP);
    }
...
int mca_base_var_elw_name(const char *param_name,
                          char **elw_name)
{
    int ret;

    assert (NULL != elw_name);

    ret = asprintf(elw_name, "%s%s", mca_prefix, param_name);
    if (0 > ret) {
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
 ...
 */
static int var_find_by_name (const char *full_name, int *vari, bool ilwalidok)
{
    ...
    if (ilwalidok || (var && VAR_IS_VALID(var[0]))) {
                     ...
                     bool ilwalidok)
{
    ...
    ret = var_find_by_name(full_name, &vari, ilwalidok);
 ...
 */
int mca_base_var_build_elw(char ***elw, int *num_elw, bool internal)
{
    mca_base_var_t *var;
    size_t i, len;
    int ret;
    ...
    for (i = 0; i < len; ++i) {
        ...
        opal_argv_append(num_elw, elw, str);
        free(str);
...
/*
 * Shut down the MCA parameter system (normally only ilwoked by the
...
int mca_base_var_finalize(void)
{
    ...
    if (mca_base_var_initialized) {
        ...
        while (NULL !=
               (item = opal_list_remove_first(&mca_base_elwar_file_values))) {
            OBJ_RELEASE(item);
        }
        OBJ_DESTRUCT(&mca_base_elwar_file_values);
        ...
        free (mca_base_elwar_files);
        mca_base_elwar_files = NULL;
    }
...
static int fixup_files(char **file_list, char * path, bool rel_path_search, char sep) {
    ...
    search_path = opal_argv_split(path, OPAL_ELW_SEP);
                              ...
                              void *storage)
{
    ...
    if (0 > var_index) {
        ...
        if (0 <= var_find_by_name (var->mbv_full_name, &tmp, 0)) {
            /* XXX --- FIXME: variable overshadows an existing variable. this is diffilwlt to support */
            assert (0);
        }
...
static int var_get_elw (mca_base_var_t *var, const char *name, char **source, char **value)
{
    char *source_elw, *value_elw;
    int ret;

    ret = asprintf (&source_elw, "%sSOURCE_%s", mca_prefix, name);
    if (0 > ret) {
        return OPAL_ERROR;
    }

    ret = asprintf (&value_elw, "%s%s", mca_prefix, name);
    if (0 > ret) {
        free (source_elw);
        return OPAL_ERROR;
    }

    *source = getelw (source_elw);
    *value = getelw (value_elw);
 ...
 */
static int var_set_from_elw (mca_base_var_t *var, mca_base_var_t *original)
{
    ...
    ret = var_get_elw (var, var_long_name, &source_elw, &value_elw);
    if (OPAL_SUCCESS != ret) {
        ret = var_get_elw (var, var_full_name, &source_elw, &value_elw);
    }
    ...
    original->mbv_source = MCA_BASE_VAR_SOURCE_ELW;

    if (NULL != source_elw) {
        if (0 == strncasecmp (source_elw, "file:", 5)) {
            original->mbv_source_file = append_filename_to_list(source_elw + 5);
        ...
        } else if (0 == strcasecmp (source_elw, "command")) {
    ...
    if (deprecated) {
        ...
        switch (var->mbv_source) {
        case MCA_BASE_VAR_SOURCE_ELW:
            opal_show_help("help-mca-var.txt", "deprecated-mca-elw",
    ...
    return var_set_from_string (original, value_elw);
}
...
static int var_set_from_file (mca_base_var_t *var, mca_base_var_t *original, opal_list_t *file_values)
{
    ...
    OPAL_LIST_FOREACH(fv, file_values, mca_base_var_file_value_t) {
        ...
        if (MCA_BASE_VAR_FLAG_ELWIRONMENT_ONLY & original->mbv_flags) {
...
static int var_set_initial (mca_base_var_t *var, mca_base_var_t *original)
{
    ...
    ret = var_set_from_elw (var, original);
    if (OPAL_ERR_NOT_FOUND != ret) {
        return ret;
    }

    ret = var_set_from_file (var, original, &mca_base_elwar_file_values);
    if (OPAL_ERR_NOT_FOUND != ret) {
        return ret;
    }
...
static int var_value_string (mca_base_var_t *var, char **value_string)
{
    ...
    if (NULL == var->mbv_enumerator) {
        ...
        /* we use an enumerator to handle string->bool and bool->string colwersion */
        if (MCA_BASE_VAR_TYPE_BOOL == var->mbv_type) {
