...
static void add_to_elw(char **params, char **values, char ***elw);
...
int mca_base_cmd_line_setup(opal_cmd_line_t *cmd)
{
    ...
    {
        opal_cmd_line_init_t entry =
            {"mca_base_elwar_file_prefix", '\0', "tune", NULL, 1,
             NULL, OPAL_CMD_LINE_TYPE_STRING,
...
int mca_base_cmd_line_process_args(opal_cmd_line_t *cmd,
                                   char ***context_elw, char ***global_elw)
{
    int i, num_insts, rc;
    char **params;
    char **values;
    ...
    if (NULL != params) {
        add_to_elw(params, values, context_elw);
    ...
    if (NULL != params) {
        add_to_elw(params, values, global_elw);
        opal_argv_free(params);
        opal_argv_free(values);
    }
...
static void add_to_elw(char **params, char **values, char ***elw)
{
    ...
    /* Loop through all the args that we've gotten and make elw
       vars of the form OPAL_MCA_PREFIX*=value. */

    for (i = 0; NULL != params && NULL != params[i]; ++i) {
        (void) mca_base_var_elw_name (params[i], &name);
        opal_setelw(name, values[i], true, elw);
        free(name);
    }
}
