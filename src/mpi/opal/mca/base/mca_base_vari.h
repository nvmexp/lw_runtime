 ...
 * @param[in]  ilwalidok   Return group even if it has been deregistered
 */
OPAL_DECLSPEC int mca_base_var_group_get_internal (const int group_index, mca_base_var_group_t **group, bool ilwalidok);
 ...
 * \internal
 *
 * Call save_value callback for generated internal mca parameter storing elw variables
 */
OPAL_DECLSPEC int mca_base_internal_elw_store(void);
