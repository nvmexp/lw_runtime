...
typedef enum {
    /** This variable should be marked as invalid when the containing
        group is deregistered (IWG = "ilwalidate with group").  This
    ...
    MCA_BASE_PVAR_FLAG_ILWALID    = 0x400,
...
enum {
    ...
    /** Variable counts the number of oclwrences of a specific event */
 ...
 * @param[in]  event Event that has oclwrred. See mca_base_pvar_event_t.
 * @param[in]  obj   Bound object
...
typedef struct mca_base_pvar_handle_t {
    ...
    /** Temporary buffer for counters. Used to callwlate deltas between
...
/**
 * Colwinience function for registering a performance variable
...
/**
 * Colwienience functions for performance variables
 */
...
static inline bool mca_base_pvar_is_ilwalid (const mca_base_pvar_t *pvar)
{
    return !!(pvar->flags & MCA_BASE_PVAR_FLAG_ILWALID);
}
...
/**
 * Colwienience function for sending notification of a handle change
 *
 * @param[in]  handle Handle event oclwrred on
 * @param[in]  event  Event that oclwrred
