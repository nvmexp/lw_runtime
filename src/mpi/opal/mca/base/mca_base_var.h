...
typedef enum {
    ...
    MCA_BASE_VAR_FLAG_ELWIRONMENT_ONLY = 0x0020,
 ...
 * ilwoked internally (by mca_base_open()) and is only dolwmented
 ...
 * from the ompi_info exelwtable.  The {description} string is copied
...
/**
 * Colwenience function for registering a variable associated with a
 * component.
 *
...
/**
 * Colwenience function for registering a variable associated with a framework. This
 ...
 *
 * @retval string A string suitable for setelw() or appending to
 * an elwiron-style string array.
 ...
 */
OPAL_DECLSPEC int mca_base_var_elw_name(const char *param_name,
                                        char **elw_name);
 ...
 *
 * It is not always colwenient to widely propagate a variable's index
 ...
 *
 * @param[out] elw A pointer to an argv-style array of key=value
 * strings, suitable for use in an environment
 * @param[out] num_elw A pointer to an int, containing the length
 * of the elw array (not including the final NULL entry).
 ...
 */
OPAL_DECLSPEC int mca_base_var_build_elw(char ***elw, int *num_elw,
                                         bool internal);

/**
 * Shut down the MCA variable system (normally only ilwoked by the
 ...
 * ilwoked after this function.
 *
 * This function is normally only ilwoked by the MCA framework itself
 * when the process is shutting down (e.g., during MPI_FINALIZE).  It
 * is only dolwmented here for completeness.
 */
...
/*
 * Parse a provided list of elwars and add their local value, or
 * their assigned value, to the provided argv
 */
OPAL_DECLSPEC int mca_base_var_process_elw_list(char *list, char ***argv);
OPAL_DECLSPEC int mca_base_var_process_elw_list_from_file(char ***argv);
...
extern char *mca_base_elw_list;
#define MCA_BASE_ELW_LIST_SEP_DEFAULT ";"
extern char *mca_base_elw_list_sep;
extern char *mca_base_elw_list_internal;
