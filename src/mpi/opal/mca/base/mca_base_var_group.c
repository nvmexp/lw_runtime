...
int mca_base_var_group_get_internal (const int group_index, mca_base_var_group_t **group, bool ilwalidok)
{
    if (group_index < 0) {
    ...
    *group = (mca_base_var_group_t *) opal_pointer_array_get_item (&mca_base_var_groups,
                                                                   group_index);
    if (NULL == *group || (!ilwalidok && !(*group)->group_isvalid)) {
        *group = NULL;
        return OPAL_ERR_NOT_FOUND;
    }
...
static int group_find_by_name (const char *full_name, int *index, bool ilwalidok)
{
    ...
    rc = mca_base_var_group_get_internal ((int)(uintptr_t) tmp, &group, ilwalidok);
    if (OPAL_SUCCESS != rc) {
        return rc;
    }

    if (ilwalidok || group->group_isvalid) {
...
static int group_find_linear (const char *project_name, const char *framework_name,
                              const char *component_name, bool ilwalidok)
{
    for (int i = 0 ; i < mca_base_var_group_count ; ++i) {
        mca_base_var_group_t *group;

        int rc = mca_base_var_group_get_internal (i, &group, ilwalidok);
        if (OPAL_SUCCESS != rc) {
            continue;
        }
                       ...
                       const char *component_name, bool ilwalidok)
{
    ...
    if ((project_name && '*' == project_name[0]) || (framework_name && '*' == framework_name[0]) ||
        ...
        return group_find_linear (project_name, framework_name, component_name, ilwalidok);
    }
    ...
    ret = group_find_by_name(full_name, &index, ilwalidok);
    free (full_name);
...
int mca_base_var_group_deregister (int group_index)
{
    ...
    /* ilwalidate all associated mca performance variables */
    ...
    for (int i = 0 ; i < size ; ++i) {
        ...
        (void) mca_base_pvar_mark_ilwalid (params[i]);
    }
