...
int opal_datatype_register_params(void)
{
    ...
    ret = mca_base_var_register ("opal", "opal", NULL, "lwda_verbose",
                                 "Set level of opal lwca verbosity",
                                 MCA_BASE_VAR_TYPE_INT, NULL, 0, MCA_BASE_VAR_FLAG_SETTABLE,
                                 OPAL_INFO_LVL_8, MCA_BASE_VAR_SCOPE_LOCAL,
                                 &opal_lwda_verbose);
    if (0 > ret) {
...
int32_t opal_datatype_finalize( void )
{
    ...
    /* clear all master colwertors */
    opal_colwertor_destroy_masters();
