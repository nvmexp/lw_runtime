...
#include "opal_datatype_lwda.h"

#undef MEM_OP_NAME
#define MEM_OP_NAME non_overlap_lwda
#undef MEM_OP
#define MEM_OP opal_lwda_memcpy_sync
#include "opal_datatype_copy.h"

#undef MEM_OP_NAME
#define MEM_OP_NAME overlap_lwda
#undef MEM_OP
#define MEM_OP opal_lwda_memmove
#include "opal_datatype_copy.h"

#define SET_LWDA_COPY_FCT(lwda_device_bufs, fct, copy_function)     \
    do {                                                            \
        if (true == lwda_device_bufs) {                             \
            fct = copy_function;                                    \
        }                                                           \
    } while(0)
#else
                                             ...
                                             char* destination_base, char* source_base )
{
...
#if OPAL_LWDA_SUPPORT
    bool lwda_device_bufs = opal_lwda_check_bufs(destination_base, source_base);
#endif
    ...
    /**
     * see dislwssion in coll_basic_reduce.c for the computation of extent when
    ...
    SET_LWDA_COPY_FCT(lwda_device_bufs, fct, non_overlap_lwda_copy_content_same_ddt);
    if( destination_base < source_base ) {
        if( (destination_base + extent) > source_base ) {
            /* memmove */
            fct = overlap_copy_content_same_ddt;
            SET_LWDA_COPY_FCT(lwda_device_bufs, fct, overlap_lwda_copy_content_same_ddt);
        }
    } else {
