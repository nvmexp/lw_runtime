...
#if !defined(CHECKSUM) && OPAL_LWDA_SUPPORT
/* Make use of existing macro to do LWCA style memcpy */
#undef MEMCPY_CSUM
#define MEMCPY_CSUM( DST, SRC, BLENGTH, COLWERTOR ) \
 ...
 */
static inline int
unpack_partial_blocklen( opal_colwertor_t* COLWERTOR,
                        ...
                        size_t* SPACE )
{
    ...
    OPAL_DATATYPE_SAFEGUARD_POINTER( _memory, do_now_bytes, (COLWERTOR)->pBaseBuf,
    ...
    MEMCPY_CSUM( _memory, _packed, do_now_bytes, (COLWERTOR) );
...
static inline void
unpack_predefined_data( opal_colwertor_t* COLWERTOR,
                        const dt_elem_desc_t* ELEM,
                        size_t* COUNT,
                        unsigned char** packed,
                        unsigned char** memory,
    ...
    *(COUNT) -= cando_count;
    ...
    if( 1 == _elem->blocklen ) {  /* Do as many full blocklen as possible */
        for(; cando_count > 0; cando_count--) {
            OPAL_DATATYPE_SAFEGUARD_POINTER( _memory, blocklen_bytes, (COLWERTOR)->pBaseBuf,
                                             (COLWERTOR)->pDesc, (COLWERTOR)->count );
            ...
            MEMCPY_CSUM( _memory, _packed, blocklen_bytes, (COLWERTOR) );
...
static inline void unpack_contiguous_loop( opal_colwertor_t* COLWERTOR,
                                           const dt_elem_desc_t* ELEM,
                                           size_t* COUNT,
                                           unsigned char** packed,
                                           unsigned char** memory,
                                           size_t* SPACE )
{
    ...
    for(size_t _i = 0; _i < _copy_loops; _i++ ) {
        OPAL_DATATYPE_SAFEGUARD_POINTER( _memory, _end_loop->size, (COLWERTOR)->pBaseBuf,
                                    (COLWERTOR)->pDesc, (COLWERTOR)->count );
        ...
        MEMCPY_CSUM( _memory, *(packed), _end_loop->size, (COLWERTOR) );
...
#define UNPACK_PARTIAL_BLOCKLEN( COLWERTOR,    /* the colwertor */       \
...
unpack_partial_blocklen( (COLWERTOR), (ELEM), &(COUNT), &(PACKED), &(MEMORY), &(SPACE) )

#define UNPACK_PREDEFINED_DATATYPE( COLWERTOR,    /* the colwertor */    \
                                    ELEM,         /* the basic element to be packed */ \
                                    COUNT,        /* the number of elements */ \
                                    PACKED,       /* the destination pointer (char*) */ \
                                    MEMORY,       /* the source pointer (char*) */ \
                                    SPACE )       /* the space in the destination buffer */ \
unpack_predefined_data( (COLWERTOR), (ELEM), &(COUNT), &(PACKED), &(MEMORY), &(SPACE) )

#define UNPACK_CONTIGUOUS_LOOP( COLWERTOR, ELEM, COUNT, PACKED, MEMORY, SPACE ) \
    unpack_contiguous_loop( (COLWERTOR), (ELEM), &(COUNT), &(PACKED), &(MEMORY), &(SPACE) )
