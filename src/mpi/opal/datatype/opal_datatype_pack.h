 ...
 */
static inline int
pack_partial_blocklen( opal_colwertor_t* COLWERTOR,
                       ...
                       size_t* SPACE )
{
    ...
    MEMCPY_CSUM( _packed, _memory, do_now_bytes, (COLWERTOR) );
 ...
 */
static inline void
pack_predefined_data( opal_colwertor_t* COLWERTOR,
                      const dt_elem_desc_t* ELEM,
                      size_t* COUNT,
                      unsigned char** memory,
                      unsigned char** packed,
    ...
    *(COUNT) -= cando_count;
    ...
    if( 1 == _elem->blocklen ) { /* Do as many full blocklen as possible */
        for(; cando_count > 0; cando_count--) {
            ...
            MEMCPY_CSUM( _packed, _memory, blocklen_bytes, (COLWERTOR) );
...
static inline void pack_contiguous_loop( opal_colwertor_t* COLWERTOR,
                                         const dt_elem_desc_t* ELEM,
                                         size_t* COUNT,
                                         unsigned char** memory,
                                         unsigned char** packed,
                                         size_t* SPACE )
{
    ...
    for(size_t _i = 0; _i < _copy_loops; _i++ ) {
        ...
        MEMCPY_CSUM( *(packed), _memory, _end_loop->size, (COLWERTOR) );
...
#define PACK_PARTIAL_BLOCKLEN( COLWERTOR,    /* the colwertor */                       \
...
pack_partial_blocklen( (COLWERTOR), (ELEM), &(COUNT), &(MEMORY), &(PACKED), &(SPACE) )

#define PACK_PREDEFINED_DATATYPE( COLWERTOR,    /* the colwertor */     \
                                  ELEM,         /* the basic element to be packed */      \
                                  COUNT,        /* the number of elements */              \
                                  MEMORY,       /* the source pointer (char*) */          \
                                  PACKED,       /* the destination pointer (char*) */     \
                                  SPACE )       /* the space in the destination buffer */ \
pack_predefined_data( (COLWERTOR), (ELEM), &(COUNT), &(MEMORY), &(PACKED), &(SPACE) )

#define PACK_CONTIGUOUS_LOOP( COLWERTOR, ELEM, COUNT, MEMORY, PACKED, SPACE ) \
    pack_contiguous_loop( (COLWERTOR), (ELEM), &(COUNT), &(MEMORY), &(PACKED), &(SPACE) )
