 ...
 */
static inline int
opal_colwertor_merge_iov( struct iovec* iov, uint32_t* iov_count,
 ...
 * colwersion (i.e. no heterogeneity) is taken into account, and that all
 * length we're working on are local.
 */
int32_t
opal_colwertor_raw( opal_colwertor_t* pColwertor,
                    ...
                    size_t* length )
{
    ...
    assert( (*iov_count) > 0 );
    if( OPAL_LIKELY(pColwertor->flags & COLWERTOR_COMPLETED) ) {
        iov[0].iov_base = NULL;
        iov[0].iov_len  = 0;
        *iov_count      = 0;
    ...
    if( OPAL_LIKELY(pColwertor->flags & COLWERTOR_NO_OP) ) {
        /* The colwertor contain minimal informations, we only use the bColwerted
         * to manage the colwersion. This function work even after the colwertor
         * was moved to a specific position.
         */
        opal_colwertor_get_lwrrent_pointer( pColwertor, (void**)&iov[0].iov_base );
        iov[0].iov_len = pColwertor->local_size - pColwertor->bColwerted;
        *length = iov[0].iov_len;
    ...
    DO_DEBUG( opal_output( 0, "opal_colwertor_raw( %p, {%p, %" PRIu32 "}, %"PRIsize_t " )\n", (void*)pColwertor,
    ...
    source_base  = pColwertor->pBaseBuf + pStack->disp;
    ...
    DO_DEBUG( opal_output( 0, "raw start pos_desc %d count_desc %" PRIsize_t " disp %ld\n"
                           ...
                           pos_desc, count_desc, (long)(source_base - pColwertor->pBaseBuf),
    ...
    if( pElem->elem.common.flags & OPAL_DATATYPE_FLAG_DATA ) {
        ...
        if( count_desc != ((size_t)current->count * current->blocklen) ) {  /* Not the full element description */
            if( (do_now = count_desc % current->blocklen) ) {
                ...
                OPAL_DATATYPE_SAFEGUARD_POINTER( source_base, blength, pColwertor->pBaseBuf,
                                                 pColwertor->pDesc, pColwertor->count );
                DO_DEBUG( opal_output( 0, "raw 1. iov[%d] = {base %p, length %" PRIsize_t "}\n",
                                       index, (void*)source_base, blength ); );
                opal_colwertor_merge_iov( iov, iov_count,
    ...
    while( 1 ) {
        while( pElem->elem.common.flags & OPAL_DATATYPE_FLAG_DATA ) {
            ...
            for(size_t _i = 0; _i < do_now; _i++ ) {
                ...
                DO_DEBUG( opal_output( 0, "raw 2. iov[%d] = {base %p, length %" PRIsize_t "}\n",
                                       index, (void*)source_base, blength ); );
                if( opal_colwertor_merge_iov( iov, iov_count,
                                              (IOVBASE_TYPE *) source_base, blength, &index ) )
        ...
        if( OPAL_DATATYPE_END_LOOP == pElem->elem.common.type ) { /* end of the current loop */
            ...
            if( --(pStack->count) == 0 ) { /* end of loop */
                if( 0 == pColwertor->stack_pos ) {
                    ...
                    index++;  /* account for the lwrrently updating iovec */
                    goto complete_loop;
                }
                pColwertor->stack_pos--;
                pStack--;
        ...
        if( OPAL_DATATYPE_LOOP == pElem->elem.common.type ) {
            ...
            if( pElem->loop.common.flags & OPAL_DATATYPE_FLAG_CONTIGUOUS ) {
                ...
                source_base += offset;
                for(; count_desc > 0; ) {
                    OPAL_DATATYPE_SAFEGUARD_POINTER( source_base, end_loop->size, pColwertor->pBaseBuf,
                                                     pColwertor->pDesc, pColwertor->count );
                    if( opal_colwertor_merge_iov( iov, iov_count,
 ...
 complete_loop:
    pColwertor->bColwerted += sum_iov_len;  /* update the already colwerted bytes */
    *length = sum_iov_len;
    *iov_count = index;
    ...
    PUSH_STACK( pStack, pColwertor->stack_pos, pos_desc, OPAL_DATATYPE_UINT1, count_desc,
                source_base - pColwertor->pBaseBuf );
