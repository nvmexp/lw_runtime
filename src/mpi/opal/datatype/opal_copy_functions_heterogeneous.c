...
/*
 * Long-Term TODO:
 * We need a better way for the upper layer to colwert
 * multiple, conselwtive struct-types, e.g. float_int.
 ...
 *
 * A better way would be to have a colwersion registration functionality.
 */
...
static int32_t                                                                            \
copy_##TYPENAME##_heterogeneous(opal_colwertor_t *pColwertor, size_t count,               \
...
{                                                                       \
    ...
    datatype_check( #TYPE, sizeof(TYPE), sizeof(TYPE), &count,          \
                   ...
                   to, to_length, to_extent);                           \
                                                                        \
    if ((pColwertor->remoteArch & OPAL_ARCH_ISBIGENDIAN) !=             \
        ...
        if( (to_extent == from_extent) && (to_extent == sizeof(TYPE)) ) { \
            ...
            if (LONG_DOUBLE) {                                          \
                opal_dt_swap_long_double(to, from, sizeof(TYPE), count, pColwertor->remoteArch);\
            }                                                           \
        } else {                                                        \
            for( i = 0; i < count; i++ ) {                              \
                opal_dt_swap_bytes(to, from, sizeof(TYPE), 1);          \
                if (LONG_DOUBLE) {                                      \
                    opal_dt_swap_long_double(to, from, sizeof(TYPE), 1, pColwertor->remoteArch);\
                }                                                       \
                to += to_extent;                                        \
                from += from_extent;                                    \
            }                                                           \
...
{                                                                       \
    ...
    if ((pColwertor->remoteArch & OPAL_ARCH_ISBIGENDIAN) !=             \
        ...
        if( (to_extent == from_extent) && (to_extent == (2 * sizeof(TYPE))) ) { \
            ...
            if (LONG_DOUBLE) {                                          \
                opal_dt_swap_long_double(to, from, sizeof(TYPE), 2*count, pColwertor->remoteArch);\
            }                                                           \
        } else {                                                        \
            for( i = 0; i < count; i++ ) {                              \
                opal_dt_swap_bytes(to, from, sizeof(TYPE), 2);          \
                if (LONG_DOUBLE) {                                      \
                    opal_dt_swap_long_double(to, from, sizeof(TYPE), 2, pColwertor->remoteArch);\
                }                                                       \
                to += to_extent;                                        \
                from += from_extent;                                    \
            }                                                           \
...
static int32_t
copy_cxx_bool_heterogeneous(opal_colwertor_t *pColwertor, size_t count,
                            const char* from, size_t from_len, ptrdiff_t from_extent,
                            char* to, size_t to_length, ptrdiff_t to_extent,
                            ptrdiff_t *advance)
{
    /* fix up the from extent */
    if ((pColwertor->remoteArch & OPAL_ARCH_BOOLISxx) !=
        (opal_local_arch & OPAL_ARCH_BOOLISxx)) {
        switch (pColwertor->remoteArch & OPAL_ARCH_BOOLISxx) {
    ...
    if ((to_extent != sizeof(bool) || from_extent != sizeof(bool)) ||
        ((pColwertor->remoteArch & OPAL_ARCH_BOOLISxx) !=
         (opal_local_arch & OPAL_ARCH_BOOLISxx))) {
        switch (pColwertor->remoteArch & OPAL_ARCH_BOOLISxx) {
        case OPAL_ARCH_BOOLIS8:
...
colwersion_fct_t opal_datatype_heterogeneous_copy_functions[OPAL_DATATYPE_MAX_PREDEFINED] = {
   [OPAL_DATATYPE_LOOP]                = NULL,
   [OPAL_DATATYPE_END_LOOP]            = NULL,
   [OPAL_DATATYPE_LB]                  = NULL,
   [OPAL_DATATYPE_UB]                  = NULL,
   [OPAL_DATATYPE_INT1]                = (colwersion_fct_t) copy_int1_heterogeneous,
   [OPAL_DATATYPE_INT2]                = (colwersion_fct_t) copy_int2_heterogeneous,
   [OPAL_DATATYPE_INT4]                = (colwersion_fct_t) copy_int4_heterogeneous,
   [OPAL_DATATYPE_INT8]                = (colwersion_fct_t) copy_int8_heterogeneous,
   [OPAL_DATATYPE_INT16]               = (colwersion_fct_t) copy_int16_heterogeneous,
   [OPAL_DATATYPE_UINT1]               = (colwersion_fct_t) copy_int1_heterogeneous,
   [OPAL_DATATYPE_UINT2]               = (colwersion_fct_t) copy_int2_heterogeneous,
   [OPAL_DATATYPE_UINT4]               = (colwersion_fct_t) copy_int4_heterogeneous,
   [OPAL_DATATYPE_UINT8]               = (colwersion_fct_t) copy_int8_heterogeneous,
   [OPAL_DATATYPE_UINT16]              = (colwersion_fct_t) copy_int16_heterogeneous,
   [OPAL_DATATYPE_FLOAT2]              = (colwersion_fct_t) copy_float2_heterogeneous,
   [OPAL_DATATYPE_FLOAT4]              = (colwersion_fct_t) copy_float4_heterogeneous,
   [OPAL_DATATYPE_FLOAT8]              = (colwersion_fct_t) copy_float8_heterogeneous,
   [OPAL_DATATYPE_FLOAT12]             = (colwersion_fct_t) copy_float12_heterogeneous,
   [OPAL_DATATYPE_FLOAT16]             = (colwersion_fct_t) copy_float16_heterogeneous,
   [OPAL_DATATYPE_FLOAT_COMPLEX]       = (colwersion_fct_t) copy_float_complex_heterogeneous,
   [OPAL_DATATYPE_DOUBLE_COMPLEX]      = (colwersion_fct_t) copy_double_complex_heterogeneous,
   [OPAL_DATATYPE_LONG_DOUBLE_COMPLEX] = (colwersion_fct_t) copy_long_double_complex_heterogeneous,
   [OPAL_DATATYPE_BOOL]                = (colwersion_fct_t) copy_cxx_bool_heterogeneous,
   [OPAL_DATATYPE_WCHAR]               = (colwersion_fct_t) copy_wchar_heterogeneous,
