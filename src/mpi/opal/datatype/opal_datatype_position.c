...
static inline void
position_single_block(opal_colwertor_t* COLWERTOR,
                      unsigned char** mem, ptrdiff_t mem_update,
                      size_t* space, size_t space_update,
                      size_t* cnt, size_t cnt_update)
{
    OPAL_DATATYPE_SAFEGUARD_POINTER( *mem, mem_update, (COLWERTOR)->pBaseBuf,
...
/**
 * Advance the colwertors' position according. Update the pointer and the remaining space
 * accordingly.
 */
static inline void
position_predefined_data( opal_colwertor_t* COLWERTOR,
                          dt_elem_desc_t* ELEM,
                          size_t* COUNT,
                          ...
                          size_t* SPACE )
{
    ...
    if( 0 != do_now ) {
        ...
        if( 0 != do_now ) {
            ...
            position_single_block( COLWERTOR, &_memory, do_now_bytes,
    ...
    if( 0 != do_now ) {
        ...
        for(size_t _i = 0; _i < do_now; _i++ ) {
            position_single_block( COLWERTOR, &_memory, _elem->extent,
...
int opal_colwertor_generic_simple_position( opal_colwertor_t* pColwertor,
                                            size_t* position )
{
    ...
    dt_elem_desc_t* description = pColwertor->use_desc->desc;
    dt_elem_desc_t* pElem;    /* current position */
    unsigned char *base_pointer = pColwertor->pBaseBuf;
    ptrdiff_t extent = pColwertor->pDesc->ub - pColwertor->pDesc->lb;

    DUMP( "opal_colwertor_generic_simple_position( %p, &%ld )\n", (void*)pColwertor, (long)*position );
    assert(*position > pColwertor->bColwerted);
     ...
     */
    iov_len_local = *position - pColwertor->bColwerted;
    if( iov_len_local > pColwertor->pDesc->size ) {
        pStack = pColwertor->pStack;  /* we're working with the full stack */
        count_desc = iov_len_local / pColwertor->pDesc->size;
        DO_DEBUG( opal_output( 0, "position before %lu asked %lu data size %lu"
                               " iov_len_local %lu count_desc %" PRIsize_t "\n",
                               (unsigned long)pColwertor->bColwerted, (unsigned long)*position, (unsigned long)pColwertor->pDesc->size,
                               (unsigned long)iov_len_local, count_desc ); );
        /* Update all the stack including the last one */
        for( pos_desc = 0; pos_desc <= pColwertor->stack_pos; pos_desc++ )
            pStack[pos_desc].disp += count_desc * extent;
        pColwertor->bColwerted += count_desc * pColwertor->pDesc->size;
        iov_len_local = *position - pColwertor->bColwerted;
        pStack[0].count -= count_desc;
        DO_DEBUG( opal_output( 0, "after bColwerted %lu remaining count %lu iov_len_local %lu\n",
                               (unsigned long)pColwertor->bColwerted, (unsigned long)pStack[0].count, (unsigned long)iov_len_local ); );
    }
    ...
    DO_DEBUG( opal_output( 0, "position start pos_desc %d count_desc %" PRIsize_t " disp %llx\n"
                           ...
                           pos_desc, count_desc, (unsigned long long)(base_pointer - pColwertor->pBaseBuf),
                           pColwertor->stack_pos, pStack->index, pStack->count, (unsigned long long)pStack->disp ); );
    /* Last data has been only partially colwerted. Compute the relative position */
    if( 0 != pColwertor->partial_length ) {
        ...
        if( missing_length >= iov_len_local ) {
            pColwertor->partial_length = (pColwertor->partial_length + iov_len_local) % element_length;
            pColwertor->bColwerted    += iov_len_local;
            assert(pColwertor->partial_length < element_length);
            return 0;
        }
        pColwertor->partial_length = 0;
        pColwertor->bColwerted += missing_length;
    ...
    while( 1 ) {
        if( OPAL_DATATYPE_END_LOOP == pElem->elem.common.type ) { /* end of the the entire datatype */
            ...
            base_pointer = pColwertor->pBaseBuf + pStack->disp;
        ...
        while( pElem->elem.common.flags & OPAL_DATATYPE_FLAG_DATA ) {
            ...
            position_predefined_data( pColwertor, pElem, &count_desc, &base_pointer, &iov_len_local );
            if( 0 != count_desc ) {  /* completed */
 ...
 complete_loop:
    pColwertor->bColwerted = *position;  /* update the already colwerted bytes */

    if( !(pColwertor->flags & COLWERTOR_COMPLETED) ) {
        /* I complete an element, next step I should go to the next one */
        PUSH_STACK( pStack, pColwertor->stack_pos, pos_desc, pElem->elem.common.type, count_desc,
                    base_pointer - pColwertor->pBaseBuf );
