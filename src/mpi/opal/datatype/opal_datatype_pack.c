 ...
 * the status with just the informations from pColwertor->bColwerted.
 */
int32_t
opal_pack_homogeneous_contig_function( opal_colwertor_t* pColw,
                                       struct iovec* iov,
                                       uint32_t* out_size,
                                       size_t* max_data )
{
    dt_stack_t* pStack = pColw->pStack;
    unsigned char *source_base = NULL;
    uint32_t iov_count;
    size_t length = pColw->local_size - pColw->bColwerted, initial_amount = pColw->bColwerted;

    source_base = (pColw->pBaseBuf + pColw->pDesc->true_lb + pStack[0].disp + pStack[1].disp);
    ...
    for( iov_count = 0; iov_count < (*out_size); iov_count++ ) {
        ...
        if( iov[iov_count].iov_base == NULL ) {
            ...
            COMPUTE_CSUM( iov[iov_count].iov_base, iov[iov_count].iov_len, pColw );
        } else {
            ...
            OPAL_DATATYPE_SAFEGUARD_POINTER( source_base, iov[iov_count].iov_len,
                                        pColw->pBaseBuf, pColw->pDesc, pColw->count );
            MEMCPY_CSUM( iov[iov_count].iov_base, source_base, iov[iov_count].iov_len, pColw );
        }
        length -= iov[iov_count].iov_len;
        pColw->bColwerted += iov[iov_count].iov_len;
        pStack[0].disp += iov[iov_count].iov_len;
        source_base += iov[iov_count].iov_len;
    }

    /* update the return value */
    *max_data = pColw->bColwerted - initial_amount;
    *out_size = iov_count;
    if( pColw->bColwerted == pColw->local_size ) {
...
int32_t
opal_pack_homogeneous_contig_with_gaps_function( opal_colwertor_t* pColw,
                                                 struct iovec* iov,
                                                 uint32_t* out_size,
                                                 size_t* max_data )
{
    size_t remaining, length, initial_bytes_colwerted = pColw->bColwerted;
    ...
    /* We can provide directly the pointers in the user buffers (like the colwertor_raw) */
    if( NULL == iov[0].iov_base ) {
        user_memory = pColw->pBaseBuf + pData->true_lb;
        ...
        for( idx = 0; (idx < (*out_size)) && stack[0].count; idx++ ) {
            ...
            COMPUTE_CSUM( iov[idx].iov_base, iov[idx].iov_len, pColw );

            pColw->bColwerted += stack[1].count;
    ...
    for( idx = 0; idx < (*out_size); idx++ ) {
        /* Limit the amount of packed data to the data left over on this colwertor */
        ...
        length = (0 == pColw->stack_pos ? 0 : stack[1].count);  /* left over from the last pack */
        ...
        if( (pData->size != length) && (length <= remaining)) {
            ...
            OPAL_DATATYPE_SAFEGUARD_POINTER( user_memory, length, pColw->pBaseBuf,
                                             pData, pColw->count );
            DO_DEBUG( opal_output( 0, "pack dest %p src %p length %" PRIsize_t " [prologue]\n",
                                   (void*)user_memory, (void*)packed_buffer, length ); );
            MEMCPY_CSUM( packed_buffer, user_memory, length, pColw );
        ...
        for( i = 0; pData->size <= remaining; i++ ) {
            OPAL_DATATYPE_SAFEGUARD_POINTER( user_memory, pData->size, pColw->pBaseBuf,
                                             pData, pColw->count );
            ...
            MEMCPY_CSUM( packed_buffer, user_memory, pData->size, pColw );
        ...
        if( 0 != remaining ) {
            ...
            DO_DEBUG( opal_output( 0, "4. pack dest %p src %p length %" PRIsize_t "\n",
                                   (void*)user_memory, (void*)packed_buffer, remaining ); );
            MEMCPY_CSUM( packed_buffer, user_memory, remaining, pColw );
 ...
 */
int32_t
opal_generic_simple_pack_function( opal_colwertor_t* pColwertor,
                                   ...
                                   size_t* max_data )
{
    ...
    DO_DEBUG( opal_output( 0, "opal_colwertor_generic_simple_pack( %p:%p, {%p, %lu}, %d )\n",
                           (void*)pColwertor, (void*)pColwertor->pBaseBuf,
                           (void*)iov[0].iov_base, (unsigned long)iov[0].iov_len, *out_size ); );

    description = pColwertor->use_desc->desc;

    /* For the first step we have to add both displacement to the source. After in the
     * main while loop we will set back the colw_ptr to the correct value. This is
    ...
    DO_DEBUG( opal_output( 0, "pack start pos_desc %d count_desc %" PRIsize_t " disp %ld\n"
                           ...
                           pColwertor->stack_pos, pStack->index, pStack->count, pStack->disp ); );
    ...
    for( iov_count = 0; iov_count < (*out_size); iov_count++ ) {
        ...
        if( pElem->elem.common.flags & OPAL_DATATYPE_FLAG_DATA ) {
            if( ((size_t)pElem->elem.count * pElem->elem.blocklen) != count_desc ) {
                ...
                int rc = PACK_PARTIAL_BLOCKLEN( pColwertor, pElem, count_desc,
                                                colw_ptr, iov_ptr, iov_len_local );
                if( 0 == rc )  /* not done */
                    goto complete_loop;
                if( 0 == count_desc ) {
                    colw_ptr = pColwertor->pBaseBuf + pStack->disp;
        ...
        while( 1 ) {
            while( pElem->elem.common.flags & OPAL_DATATYPE_FLAG_DATA ) {
                ...
                PACK_PREDEFINED_DATATYPE( pColwertor, pElem, count_desc,
                                          colw_ptr, iov_ptr, iov_len_local );
            ...
            if( OPAL_DATATYPE_END_LOOP == pElem->elem.common.type ) { /* end of the current loop */
                DO_DEBUG( opal_output( 0, "pack end_loop count %" PRIsize_t " stack_pos %d"
                                       ...
                                       pStack->count, pColwertor->stack_pos,
                ...
                if( --(pStack->count) == 0 ) { /* end of loop */
                    ...
                    pColwertor->stack_pos--;  /* go one position up on the stack */
                    pStack--;
            ...
            if( OPAL_DATATYPE_LOOP == pElem->elem.common.type ) {
                ...
                if( pElem->loop.common.flags & OPAL_DATATYPE_FLAG_CONTIGUOUS ) {
                    PACK_CONTIGUOUS_LOOP( pColwertor, pElem, count_desc,
                                          colw_ptr, iov_ptr, iov_len_local );
                    if( 0 == count_desc ) {  /* completed */
    ...
    *max_data = total_packed;
    pColwertor->bColwerted += total_packed;  /* update the already colwerted bytes */
    *out_size = iov_count;
    if( pColwertor->bColwerted == pColwertor->local_size ) {
...
static inline void
pack_predefined_heterogeneous( opal_colwertor_t* COLWERTOR,
                               ...
                               size_t* SPACE )
{
    const opal_colwertor_master_t* master = (COLWERTOR)->master;
    ...
    OPAL_DATATYPE_SAFEGUARD_POINTER( _source, (_count * _elem->extent), (COLWERTOR)->pBaseBuf,
                                     (COLWERTOR)->pDesc, (COLWERTOR)->count );
    DO_DEBUG( opal_output( 0, "pack [l %s r %s] memcpy( %p, %p, %lu ) => space %lu\n",
                           ...
                           (unsigned long)(*(SPACE)) ); );
    master->pFunctions[_elem->common.type]( COLWERTOR, _count,
...
int32_t
opal_pack_general_function( opal_colwertor_t* pColwertor,
                            struct iovec* iov, uint32_t* out_size,
                            size_t* max_data )
{
    dt_stack_t* pStack;       /* pointer to the position on the stack */
    ...
    DO_DEBUG( opal_output( 0, "opal_colwertor_general_pack( %p:%p, {%p, %lu}, %d )\n",
                           (void*)pColwertor, (void*)pColwertor->pBaseBuf,
                           (void*)iov[0].iov_base, (unsigned long)iov[0].iov_len, *out_size ); );
    ...
    for( iov_count = 0; iov_count < (*out_size); iov_count++ ) {
        ...
        while( 1 ) {
            while( pElem->elem.common.flags & OPAL_DATATYPE_FLAG_DATA ) {
                ...
                pack_predefined_heterogeneous( pColwertor, pElem, &count_desc,
                                               &colw_ptr, &iov_ptr, &iov_len_local);
#if 0
                PACK_PREDEFINED_DATATYPE( pColwertor, pElem, count_desc,
                                          colw_ptr, iov_ptr, iov_len_local );
