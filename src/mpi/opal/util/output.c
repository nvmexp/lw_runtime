...
bool opal_output_init(void)
{
    ...
    str = getelw("OPAL_OUTPUT_STDERR_FD");
    if (NULL != str) {
        default_stderr_fd = atoi(str);
    }
    str = getelw("OPAL_OUTPUT_REDIRECT");
    if (NULL != str) {
        if (0 == strcasecmp(str, "syslog")) {
            opal_output_redirected_to_syslog = true;
        }
    }
    str = getelw("OPAL_OUTPUT_SYSLOG_PRI");
#ifdef HAVE_SYSLOG_H
    if (NULL != str) {
...
#endif  /* HAVE_SYSLOG_H */
    str = getelw("OPAL_OUTPUT_SYSLOG_IDENT");
    if (NULL != str) {
    ...
    if (opal_output_redirected_to_syslog) {
        ...
        str = getelw("OPAL_OUTPUT_INTERNAL_TO_STDOUT");
        if (NULL != str && str[0] == '1') {
