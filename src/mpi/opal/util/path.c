 ...
 */
char *opal_path_find(char *fname, char **pathv, int mode, char **elwv)
{
    char *fullpath;
    char *delimit;
    char *elw;
    ...
    while (pathv[i] && NULL == fullpath) {
        ...
        if ('$' == *pathv[i]) {
            ...
            if (NULL != elw) {
                if (!delimit) {
                    fullpath = opal_path_access(fname, elw, mode);
                } else {
 ...
 */
char *opal_path_findv(char *fname, int mode, char **elwv, char *wrkdir)
{
    char **dirv;
    char *fullpath;
    char *path;
    ...
    if(NULL == dirv)
        return NULL;
    fullpath = opal_path_find(fname, dirv, mode, elwv);
    opal_argv_free(dirv);
    return fullpath;
}
