...
#ifndef OPAL_ELWIRON_H
#define OPAL_ELWIRON_H
 ...
 */
OPAL_DECLSPEC char **opal_elwiron_merge(char **minor, char **major) __opal_attribute_warn_unused_result__;
 ...
 * @retval OPAL_EXISTS If the name already exists in \em elw and
 ...
 * putelw(3), if \em value is inserted in \em elw, it is copied.
 ...
 *   opal_setelw("foo", "bar", true, &elwiron);
 * \endcode
 *
 * NOTE: If you use the real elwiron, opal_setelw() will turn
 ...
 * \code
 *   char **my_elw = NULL;
 *   opal_setelw("foo", "bar", true, &my_elw);
 * \endcode
 */
OPAL_DECLSPEC int opal_setelw(const char *name, const char *value,
                              bool overwrite, char ***elw) __opal_attribute_nonnull__(1);
 ...
 * @retval OPAL_ERR_NOT_FOUND If \em name is not found in \em elw.
 ...
 */
OPAL_DECLSPEC int opal_unsetelw(const char *name, char ***elw) __opal_attribute_nonnull__(1);
...
#else
OPAL_DECLSPEC extern char **elwiron;
#endif
