...
BEGIN_C_DECLS
   ...
   *
   * This function is ilwoked to setup a dedicated output stream for
   * malloc debug functions.  It does \em not (lwrrently) do anything
   * other than that (i.e., no internal accounting for tracking
   * malloc/free statements, etc.).
   *
   * It is ilwoked as part of opal_init().  Although this function is
   ...
   * unless this function is ilwoked first.
   */
void opal_malloc_init(void);
   ...
   *
   * This function is ilwoked as part of opal_finalize() to shut down the
