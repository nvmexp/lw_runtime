 ...
 * the stream.
 *
 * The default "verbose" stream is opened after ilwoking
 * opal_output_init() (and closed after ilwoking
 ...
 * the following elwironmental variables that support redirecting -all-
...
struct opal_output_stream_t {
     ...
     *
     * Verbose levels are a colwenience mechanisms, and are only
     ...
     * exist when opal_output_open() is ilwoked.  See opal_output()
     ...
     *
     * This should be the first function ilwoked in the output
     ...
     *
     * It is safe to have multiple threads ilwoke this function
     * simultaneously; their exelwtion will be serialized in an
     * unspecified manner.
     *
     ...
     * typically only ilwoked after a restart (i.e., in a new process)
     ...
     * session directory does not exist when opal_output() is ilwoked,
     ...
     * this function will append newlines as necessary.
     *
     * This function is really a colwenience wrapper around checking the
     ...
     * function will effectively ilwoke opal_output to send the output to
     * the stream.
     *
    ...
    */
     ...
     * will be used for all future ilwocations of opal_output_verbose().
     */
 ...
 * this class is for colwenience only -- it is \em not necessary
 * to be ilwoked.  If the constructor it used, it sets all values
 * in the struct to be false / 0 (i.e., turning off all output).
 * The intended usage is to ilwoke the constructor and then enable
