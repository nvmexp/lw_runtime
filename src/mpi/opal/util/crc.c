    ...
    )
{
	...
	if (*lastPartialLength) {
        ...
        */
		...
		src = (unsigned long *)((char *)src + sizeof(unsigned long) - *lastPartialLength);
		*lastPartialLength = copylen;
		/* reset temp, and callwlate next partial word */
		temp = 0;
		if (copylen) {
...
/* globals for CRC32 bcopy and callwlation routines */
...
{
    register int i,j;
    register unsigned int crc_aclwm;

    for (i = 0; i < 256; i++) {
        crc_aclwm = (i << 24);
        for (j = 0; j < 8; j++) {
            if (crc_aclwm & 0x80000000)
                crc_aclwm = (crc_aclwm << 1) ^ CRC_POLYNOMIAL;
            else
                crc_aclwm = (crc_aclwm << 1);
        }
        _opal_crc_table[i] = crc_aclwm;
    }
    ...
    unsigned int partial_crc)
{
    ...
    if (INTALIGNED(source) && INTALIGNED(destination)) {
        ...
        /* callwlate CRC over remaining bytes... */
        while (crclenresidue--) {
            i = ((partial_crc >> 24) ^ *ts++) & 0xff;
            partial_crc = (partial_crc << 8) ^ _opal_crc_table[i];
        }
