 ...
 * files in it.  If the relwrsive variable is non-zero, then it will
 * try to relwrsively remove all directories.  If provided, the
 * callback function is exelwted prior to the directory or file being
                            ...
                            opal_os_dirpath_destroy_callback_fn_t cbfunc)
{
    ...
    while (NULL != (ep = readdir(dp))) {
        ...
        if (0 > rc) {
             ...
             * other process running on the same node. That typically oclwrs
        ...
        /*
         * If not relwrsively decending, then if we find a directory then fail
        ...
        /* Directories are relwrsively destroyed */
        if (is_dir) {
            rc = opal_os_dirpath_destroy(filenm, relwrsive, cbfunc);
            free(filenm);
            if (OPAL_SUCCESS != rc) {
                exit_status = rc;
                closedir(dp);
