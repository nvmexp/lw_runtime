...
static void cmd_line_constructor(opal_cmd_line_t *cmd)
{
    ...
    OBJ_CONSTRUCT(&cmd->lcl_mutex, opal_relwrsive_mutex_t);
...
static int make_opt(opal_cmd_line_t *cmd, opal_cmd_line_init_t *e)
{
    ...
    if (NULL != e->ocl_mca_param_name) {
        (void) mca_base_var_elw_name (e->ocl_mca_param_name,
...
static int set_dest(ompi_cmd_line_option_t *option, char *sval)
{
    ...
    if (NULL != option->clo_mca_param_elw_var) {
        ...
        case OPAL_CMD_LINE_TYPE_SIZE_T:
            opal_setelw(option->clo_mca_param_elw_var, sval, true, &elwiron);
            break;
        case OPAL_CMD_LINE_TYPE_BOOL:
            opal_setelw(option->clo_mca_param_elw_var, "1", true, &elwiron);
            break;
        default:
            break;
        }
