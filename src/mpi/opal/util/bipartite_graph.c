			   ...
			   void *e_data)
{
    ...
    if (capacity < 0) {
        /* negative cost is fine, but negative capacity is not lwrrently
 ...
 *
 * The graph will be left in an undefined state if an error oclwrs (though
...
int opal_bp_graph_bipartite_to_flow(opal_bp_graph_t *g)
{
     ...
     * visualize this colwentionally with all of the source vertices on the
    ...
    for (u = 0; u < order; ++u) {
        ...
        FOREACH_OUT_EDGE(g, u, e_ptr) {
             ...
             * back any cost already inlwrred. */
    ...
    *flow_out = NULL;
    ...
    while (opal_bp_graph_bellman_ford(gx, gx->source_idx, gx->sink_idx, pred)) {
        ...
        FOREACH_UV_ON_PATH(pred, gx->source_idx, gx->sink_idx, u, v) {
            ...
            assert(f(u,v) == -f(v,u)); /* skew symmetry ilwariant */
					     ...
					     int **match_edges_out)
{
    ...
    *match_edges_out = NULL;
     ...
     * graph.  For colwenience we adjust all residual capacities as we go
