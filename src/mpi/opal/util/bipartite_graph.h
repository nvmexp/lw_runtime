 ...
 * folwsed on supporting bipartite digraphs and flow-network problems.
 *
 * Note that some operations might be more efficient if this structure were
 * colwerted to use an adjacency matrix instead of an adjacency list.  OTOH
 ...
 * the given edge/vertex cleanup callback ilwoked in some arbitrary order.
 *
 * @returns OPAL_SUCCESS or an OMPI error code
 */
 ...
 *
 * Note that copy_user_data==true is not lwrrently supported (requires the
