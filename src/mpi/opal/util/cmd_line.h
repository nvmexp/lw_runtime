 ...
 * example, say the following was used in for an exelwtable named
 * "foo":
 *
 * \code
 ...
 * \endverbatim
 *
 * Note that this interface can also track multiple ilwocations of the
 ...
 * parameters have been registered, the user ilwokes
 ...
 * the special "--" token.
 *
 * After the parse has oclwrred, various accessor functions can be
 ...
 *
 * Note that a shortlwt to creating a large number of options exists
 ...
 * without the need to repeatedly ilwoke opal_cmd_line_make_opt3() (or
...
BEGIN_C_DECLS
    ...
    struct opal_cmd_line_t {
        ...
        /** Thread safety */
        opal_relwrsive_mutex_t lcl_mutex;
     ...
     * threads ilwoke functions on the same handle at the same time --
     ...
     * argv[0] is assumed to be the exelwtable name, and is ignored during
     ...
     *
     * exelwtable --foo a b -- other arguments
     *
     ...
     * with "-".  For example, if a user wants to mpirun an exelwtable
     ...
     *   mpirun ./-my-mpi-program
     *
     * Finally, note that ilwoking this function multiple times on
     ...
     * display (to allow for undolwmented options).
     *
     * This function is typically only ilwoked internally by the
     ...
     * opal_cmd_line_parse(), or opal_cmd_line_parse() was not ilwoked on
     * this handle.
     *
     ...
     * handle, or opal_cmd_line_parse() was not ilwoked on this handle.
     */
     ...
     *
     * exelwtable --foo bar1 bar2 --foo bar3 bar4
     *
