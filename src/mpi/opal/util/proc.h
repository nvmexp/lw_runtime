...
#define OPAL_JOBID_ILWALID  (OPAL_JOBID_MAX + 2)
...
#define OPAL_VPID_ILWALID   (OPAL_VPID_MAX + 2)
...
#define OPAL_NAME_ILWALID       (&opal_name_ilwalid)
OPAL_DECLSPEC extern opal_process_name_t opal_name_ilwalid;


#define OPAL_NAME_ARGS(n) \
    (unsigned long) ((NULL == n) ? (unsigned long)OPAL_JOBID_ILWALID : (unsigned long)(n)->jobid), \
    (unsigned long) ((NULL == n) ? (unsigned long)OPAL_VPID_ILWALID : (unsigned long)(n)->vpid) \
...
typedef struct opal_proc_t {
    ...
    /** Base colwertor for the proc described by this process */
    struct opal_colwertor_t*        proc_colwertor;
...
OPAL_DECLSPEC extern int (*opal_colwert_string_to_process_name)(opal_process_name_t *name,
                                                                const char* name_string);
OPAL_DECLSPEC extern int (*opal_colwert_process_name_to_string)(char** name_string,
...
OPAL_DECLSPEC extern int (*opal_colwert_string_to_jobid)(opal_jobid_t *jobid, const char *jobid_string);
