                       ...
                       opal_keyval_parse_fn_t callback)
{
    ...
    while (!opal_util_keyval_parse_done) {
        ...
        case OPAL_UTIL_KEYVAL_PARSE_ELWVAR:
        case OPAL_UTIL_KEYVAL_PARSE_ELWEQL:
            parse_line_new(val);
            break;
...
int opal_util_keyval_save_internal_elwars(opal_keyval_parse_fn_t callback)
{
...
static int parse_line_new(opal_keyval_parse_state_t first_val)
{
    ...
    while (OPAL_UTIL_KEYVAL_PARSE_NEWLINE != val && OPAL_UTIL_KEYVAL_PARSE_DONE != val) {
        ...
        } else if (OPAL_UTIL_KEYVAL_PARSE_ELWEQL == val) {
            trim_name (key_buffer, "-x", "=");
            trim_name (key_buffer, "--x", NULL);

            val = opal_util_keyval_yylex();
            if (OPAL_UTIL_KEYVAL_PARSE_VALUE == val) {
                add_to_elw_str(key_buffer, opal_util_keyval_yytext);
            } else {
                parse_error(5);
                return OPAL_ERROR;
            }
        } else if (OPAL_UTIL_KEYVAL_PARSE_ELWVAR == val) {
            trim_name (key_buffer, "-x", "=");
            trim_name (key_buffer, "--x", NULL);
