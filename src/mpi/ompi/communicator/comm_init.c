...
/*
** Table for Fortran <-> C communicator handle colwersion
...
/* This is the counter for the number of communicators, which contain
   ...
   shortlwt for finalize and abort. */
...
int ompi_comm_init(void)
{
    ...
    ompi_mpi_comm_world.comm.c_lwbe_dim     = opal_lwbe_dim((int)size);
...
ompi_communicator_t *ompi_comm_allocate ( int local_size, int remote_size )
{
    ...
    /* fill in the inscribing hyper-lwbe dimensions */
    new_comm->c_lwbe_dim = opal_lwbe_dim(local_size);
        ...
        if ( NULL != comm ) {
            ...
            if ( NULL != comm ) {
                ...
                if ( !OMPI_COMM_IS_EXTRA_RETAIN(comm)) {
                     ...
                     * this only oclwrs if the cid of the local_comm is lower than of its
...
static void ompi_comm_destruct(ompi_communicator_t* comm)
{
    ...
    /* Tell the PML that this communicator is done.
       ...
       Also -- do not ilwoke the pml_del_comm if the corresponding
       pml_add_comm was never ilwoked.  This can happen in an error
       ...
       want to make it all the way up the stack to ilwoke the MPI
