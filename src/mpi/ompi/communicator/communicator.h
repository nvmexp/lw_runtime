...
#define OMPI_COMM_ILWALID      0x00000020
...
#define OMPI_COMM_IS_ILWALID(comm) ((comm)->c_flags & OMPI_COMM_ILWALID)
...
#define OMPI_COMM_SET_ILWALID(comm) ((comm)->c_flags |= OMPI_COMM_ILWALID)
...
struct ompi_communicator_t {
    ...
    int c_id_available; /* the lwrrently available Cid for allocation
               to a child*/
 ...
 *
 * - Colwert all predefined handles to run-time-assigned pointers
 ...
 *   It would have been a pain to colwert a bunch of the code to use
 ...
 * ompi_comm_ilwalid() as originally coded -- per the MPI-1
 * definition, where MPI_COMM_NULL is an invalid communicator.
 * The MPI_Comm_c2f() function, therefore, calls
 * ompi_comm_ilwalid() but also explictily checks to see if the
 * handle is MPI_COMM_NULL.
 */
static inline int ompi_comm_ilwalid(ompi_communicator_t* comm)
{
    if ((NULL == comm) || (MPI_COMM_NULL == comm) ||
        (OMPI_COMM_IS_FREED(comm)) || (OMPI_COMM_IS_ILWALID(comm)) )
        return true;
    else
        return false;
}
...
static inline bool ompi_comm_peer_ilwalid(ompi_communicator_t* comm, int peer_id)
{
