                       ...
                       ompi_request_t **req )
{
    ...
    *req = NULL;
    ...
    newcomm->c_lwbe_dim = opal_lwbe_dim(local_size);
...
static int ompi_comm_split_type_get_part (ompi_group_t *group, const int split_type, int **ranks_out, int *rank_size) {
    ...
    for (int i = 0 ; i < size ; ++i) {
        ...
        case OMPI_COMM_TYPE_HOST:
            ...
            break;
        case OMPI_COMM_TYPE_LW:
            include = OPAL_PROC_ON_LOCAL_LW(locality);
            break;
                          ...
                          opal_info_t *info, ompi_communicator_t **newcomm)
{
    ...
    if (MPI_UNDEFINED == global_split_type) {
        /* short-cirlwt. every rank provided MPI_UNDEFINED */
...
int ompi_comm_dump ( ompi_communicator_t *comm )
{
    ...
    opal_output(0,"  f2c index:%d lwbe_dim: %d\n", comm->c_f_to_c_index,
                comm->c_lwbe_dim);
 ...
 * checked by the time this has been ilwoked. This function should do
                               ...
                               ompi_errhandler_t *errh)
{
    ...
    /* determine the lwbe dimensions */
    comm->c_lwbe_dim = opal_lwbe_dim(comm->c_local_group->grp_proc_count);
