...
typedef struct ompi_timing_t {
    ...
    ompi_timing_list_t *lwr_timing;
} ompi_timing_t;

#define OMPI_TIMING_ENABLED \
    (getelw("OMPI_TIMING_ENABLE") ? atoi(getelw("OMPI_TIMING_ENABLE")) : 0)
...
#define OMPI_TIMING_INIT(_size)                                                \
    ...
    {                                                                          \
        char *ptr;                                                             \
        ptr = getelw("OMPI_TIMING_ENABLE");                                    \
        if (NULL != ptr) {                                                     \
            OMPI_TIMING.enabled = atoi(ptr);                                   \
        }                                                                      \
        if (OMPI_TIMING.enabled) {                                             \
            setelw("OPAL_TIMING_ENABLE", "1", 1);                              \
            ...
            OMPI_TIMING.lwr_timing = OMPI_TIMING.timing;                       \
        }                                                                      \
    }
...
#define OMPI_TIMING_ITEM_EXTEND                                                    \
    do {                                                                           \
        if (OMPI_TIMING.enabled) {                                                 \
            OMPI_TIMING.lwr_timing->next = (struct ompi_timing_list_t*)malloc(sizeof(ompi_timing_list_t)); \
            OMPI_TIMING.lwr_timing = (ompi_timing_list_t*)OMPI_TIMING.lwr_timing->next;                    \
            memset(OMPI_TIMING.lwr_timing, 0, sizeof(ompi_timing_list_t));                                 \
            OMPI_TIMING.lwr_timing->val = malloc(sizeof(ompi_timing_val_t) * OMPI_TIMING.size);            \
        }                                                                          \
    } while(0)
...
#define OMPI_TIMING_NEXT(...)                                                      \
    do {                                                                           \
        if (!OMPI_TIMING.error && OMPI_TIMING.enabled) {                           \
            ...
            int len = 0;                                                           \
            if (OMPI_TIMING.lwr_timing->use >= OMPI_TIMING.size){                  \
                OMPI_TIMING_ITEM_EXTEND;                                           \
            }                                                                      \
            len = snprintf(OMPI_TIMING.lwr_timing->val[OMPI_TIMING.lwr_timing->use].desc,        \
                OPAL_TIMING_STR_LEN, ##__VA_ARGS__);                               \
            if (len >= OPAL_TIMING_STR_LEN) {                                      \
                OMPI_TIMING.error = 1;                                             \
            }                                                                      \
            OMPI_TIMING.lwr_timing->val[OMPI_TIMING.lwr_timing->use].file = strdup(f);     \
            OMPI_TIMING.lwr_timing->val[OMPI_TIMING.lwr_timing->use].prefix = strdup(__func__);      \
            OMPI_TIMING.lwr_timing->val[OMPI_TIMING.lwr_timing->use++].ts =        \
...
#define OMPI_TIMING_APPEND_PREFIX(filename, _prefix, func,desc,ts)                                  \
    do {                                                                           \
        ...
        int len = snprintf(OMPI_TIMING.lwr_timing->val[OMPI_TIMING.lwr_timing->use].desc,        \
            OPAL_TIMING_STR_LEN, "%s", desc);                                      \
        if (len >= OPAL_TIMING_STR_LEN) {                                          \
            OMPI_TIMING.error = 1;                                                 \
        }                                                                          \
        if( _prefix[0] ) {                                                         \
            OMPI_TIMING.lwr_timing->val[OMPI_TIMING.lwr_timing->use].prefix =      \
                                                              func "/" _prefix;    \
        } else {                                                                   \
            OMPI_TIMING.lwr_timing->val[OMPI_TIMING.lwr_timing->use].prefix = func;\
        }                                                                          \
        OMPI_TIMING.lwr_timing->val[OMPI_TIMING.lwr_timing->use].file = filename;  \
        OMPI_TIMING.lwr_timing->val[OMPI_TIMING.lwr_timing->use++].ts = ts;        \
        OMPI_TIMING.cnt++;                                                         \
    } while(0)
...
#define OMPI_TIMING_IMPORT_OPAL_PREFIX(_prefix, func)                              \
    do {                                                                           \
        if (!OMPI_TIMING.error && OMPI_TIMING.enabled) {                           \
            ...
            OPAL_TIMING_ELW_CNT_PREFIX(_prefix, func, cnt);                        \
            OPAL_TIMING_ELW_ERROR_PREFIX(_prefix, func, OMPI_TIMING.error);        \
            for(i = 0; i < cnt; i++){                                              \
                char *desc, *filename;                                             \
                OMPI_TIMING.lwr_timing->val[OMPI_TIMING.lwr_timing->use].imported= \
                    OMPI_TIMING.import_cnt;                                        \
                OPAL_TIMING_ELW_GETDESC_PREFIX(_prefix, &filename, func, i, &desc, ts);  \
