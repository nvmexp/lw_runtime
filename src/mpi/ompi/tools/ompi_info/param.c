...
void ompi_info_do_config(bool want_all)
{
    ...
    char *fortran_have_iso_fortran_elw;
...
#define ompi_mpi_param_check 999
    ...
    fortran_have_iso_fortran_elw = OMPI_FORTRAN_HAVE_ISO_FORTRAN_ELW ?
        "yes" : "no";
    ...
    if (want_all) {
        ...
        /* JMS: See above for note about C++ bool size.  We don't have
           the bool alignment the way configure lwrrently runs -- need
    ...
    opal_info_out("Fort ISO_FORTRAN_ELW",
                  "compiler:fortran:iso_fortran_elw",
                  fortran_have_iso_fortran_elw);
