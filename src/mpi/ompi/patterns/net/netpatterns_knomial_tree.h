...
struct netpatterns_pair_exchange_node_t {
    ...
    /* ranks of nodes ilwolved in data exchnge */
    int *rank_exchanges;
...
struct netpatterns_k_exchange_node_t {
    ...
    /* reindex map */
    int *reindex_map;
    /* ilwerse of reindex map, i.e. given a reindexed id find out its actual rank */
    int *ilw_reindex_map;
    /* reindexed node_rank */
    int reindex_myid;
    /* 2-d array that hold payload info for each level of relwrsive k-ing */
    ...
    int *k_level)
{
    ...
    /* Callwlate disctance from source of data */
