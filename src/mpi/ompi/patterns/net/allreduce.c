        ...
        int op_type, opal_list_t *peers)
{
    ...
    char *sbuf_lwrrent,*rbuf_lwrrent;
    ...
    for (item = opal_list_get_first(peers) ;
        ...
        if(ompi_proc_local()==(ompi_proc_t *)item){
            /* this is the pointer to my proc strulwture */
            break;
        }
        my_rank++;
    }
    ...
    ret=ompi_netpatterns_setup_relwrsive_doubling_tree_node(n_peers,my_rank,&my_exchange_node);
    if(OMPI_SUCCESS != ret){
        return ret;
    }
    ...
    for( stripe_number=0 ; stripe_number < n_data_segments ; stripe_number++ ) {
        ...
        sbuf_lwrrent=(char *)sbuf+count_processed*dt_size;
        memcopy(scratch_bufers[send_buffer],sbuf_lwrrent,count_this_stripe*dt_size);
        ...
        rbuf_lwrrent=(char *)rbuf+count_processed*dt_size;
        memcopy(scratch_bufers[recv_buffer],rbuf_lwrrent,count_this_stripe*dt_size);
