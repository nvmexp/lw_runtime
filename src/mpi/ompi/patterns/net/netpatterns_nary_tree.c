        ...
        netpatterns_tree_node_t *my_node)
{
    ...
    int lvl,lwm_cnt, my_rank_in_my_level,n_lvls_in_tree;
    ...
    if( 0 == my_rank ) {
        ...
        for (lvl = 0 ; lvl < my_level_in_tree ; lvl ++ ) {
            /* lwmmulative count up to this level */
            lwm_cnt+=cnt;
            /* number of ranks in this level */
            cnt*=tree_order;
        }
        my_rank_in_my_level=my_rank-lwm_cnt;
        /* tree_order conselwtive ranks have the same parent */
        my_node->parent_rank=lwm_cnt-cnt/tree_order+my_rank_in_my_level/tree_order;
    }
    ...
    if( my_level_in_tree == (n_lvls_in_tree -1 ) ) {
        ...
        start_index=lwm_cnt+my_rank_in_my_level*tree_order;
...
void ompi_netpatterns_cleanup_narray_knomial_tree (netpatterns_narray_knomial_tree_node_t *my_node)
{
	...
	ompi_netpatterns_cleanup_relwrsive_knomial_tree_node (&my_node->k_node);
    }
}
        ...
        netpatterns_narray_knomial_tree_node_t *my_node)
{
    ...
    if( 0 == my_rank ) {
        ...
        rc = ompi_netpatterns_setup_relwrsive_knomial_tree_node(
...
/* callwlate the nearest power of radix that is equal to or greater
