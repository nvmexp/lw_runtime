...
/* setup relwrsive doubleing tree node */

OMPI_DECLSPEC int ompi_netpatterns_setup_relwrsive_knomial_allgather_tree_node(
        ...
        netpatterns_k_exchange_node_t *exchange_node)
{
    ...
    int peer;
    int *prev_data = NULL;
    int *lwrrent_data = NULL;
    int *group_info = NULL;


    NETPATTERNS_VERBOSE(
            ("Enter ompi_netpatterns_setup_relwrsive_knomial_tree_node(num_nodes=%d, node_rank=%d, tree_order=%d)",
    ...
    /* Callwlate the number of levels in the tree for
    ...
    /* this is the actual number of relwsive k-ing steps
    ...
    /* now, start the callwlation to find the first stray rank aka "extra" rank */
    stray = 0;
    ...
    /* this is the ilwerse map */
    exchange_node->ilw_reindex_map = (int *) malloc(num_nodes*sizeof(int));
    /*int reindex_myid;*/
    /* reindex */
    if( stray < num_nodes ) {
    ...
    /* Now, let's compute the ilwerse mapping here */
    for( i = 0; i < num_nodes; i++){
        j = 0;
        while(exchange_node->reindex_map[j] != i ){
            j++;
        }
        exchange_node->ilw_reindex_map[i] = j;
    }
    ...
    lwrrent_data = (int *) malloc( num_nodes*sizeof(int) );
    if( NULL == lwrrent_data ) {
        goto Error;
    }
    ...
    */
    ...
    for(i = 0; i < num_nodes; i++){
        ...
        lwrrent_data[i] = hier_ranks[i];
    }

    /* everyone will need to do this loop over all ranks
     * Phase I callwlate the contribution from the extra ranks
     */
    ...
    /* Phase II callwlate the contribution from each relwrsive k - ing level
     *
     */
    ...
    for( i = 0; i < pow_k; i++) {
        /* get my new rank */
        for( myid = 0; myid < num_nodes; myid++){
            lwrrent_data[myid] = prev_data[myid];
            /*fprintf(stderr,"my current data at level %d is %d\n",i+1,lwrrent_data[myid]);*/
            for( j = 0; j < num_nodes; j++ ){
                if( myid == exchange_node->reindex_map[j] ){
                    reindex_myid = j;
                    break;
            ...
            if( reindex_myid < stray ) {
                ...
                for( j = 0; j < ( tree_order - 1 ); j ++ ) {
                    ...
                    if( peer < stray ) {
                        ...
                        lwrrent_data[myid] += prev_data[exchange_node->reindex_map[peer]];
                    }
                }
            }
       ...
       /* fprintf(stderr,"Level %d current data ",i+1);*/
        for( j = 0; j < num_nodes; j++){
           /* fprintf(stderr,"%d ",lwrrent_data[j]); */
            prev_data[j] = lwrrent_data[j];
        }
       /* fprintf(stderr,"\n");*/
    ...
    /* this is the natural way to do relwrsive k-ing */
     ...
     * in the relwrsive k-ing, and get the result back at the end.
     * set the initial and final data exchanges - those that are not
     * part of the relwrsive k-ing.
     */
        ...
        for( i = 0; i < exchange_node->n_exchanges; i ++) {
            ...
            for( j = 0; j < (tree_order-1); j ++ ) {
                ...
                if ( peer < stray ) {
                    ...
                    /* an actual exchange oclwrs, bump the counter */
...
OMPI_DECLSPEC void ompi_netpatterns_cleanup_relwrsive_knomial_allgather_tree_node(
        netpatterns_k_exchange_node_t *exchange_node)
{
    int i;

    free(exchange_node->reindex_map);
    free(exchange_node->ilw_reindex_map);
    if (exchange_node->n_extra_sources > 0) {
        free(exchange_node->rank_extra_sources_array) ;
        exchange_node->n_extra_sources = 0;
        exchange_node->rank_extra_sources_array = NULL;
...
OMPI_DECLSPEC int ompi_netpatterns_setup_relwrsive_knomial_tree_node(
        ...
        netpatterns_k_exchange_node_t *exchange_node)
{
     ...
     *  in the relwrsive doubling, and get the result back at the end.
     */
    if (node_rank + 1 > cnt) {
        exchange_node->node_type = EXTRA_NODE;
    } else {
    ...
    /* set the initial and final data exchanges - those that are not
     *   part of the relwrsive doubling.
     */
    if (EXCHANGE_NODE == exchange_node->node_type)  {
        exchange_node->n_extra_sources = 0;
...
Error:
    ...
    ompi_netpatterns_cleanup_relwrsive_knomial_tree_node (exchange_node);

    /* error return */
    return OMPI_ERROR;
}

OMPI_DECLSPEC void ompi_netpatterns_cleanup_relwrsive_knomial_tree_node(
        netpatterns_k_exchange_node_t *exchange_node)
{
    int i;
...
#if 1
OMPI_DECLSPEC int ompi_netpatterns_setup_relwrsive_doubling_n_tree_node(int num_nodes, int node_rank, int tree_order,
        netpatterns_pair_exchange_node_t *exchange_node)
{
    ...
    NETPATTERNS_VERBOSE(("Enter ompi_netpatterns_setup_relwrsive_doubling_n_tree_node(num_nodes=%d, node_rank=%d, tree_order=%d)", num_nodes, node_rank, tree_order));
    ...
    /* Ishai: I see no reason for callwlating tmp. Add an assert before deleting it */
    assert(tmp == cnt);
...
OMPI_DECLSPEC void ompi_netpatterns_cleanup_relwrsive_doubling_tree_node(
    netpatterns_pair_exchange_node_t *exchange_node)
{
...
OMPI_DECLSPEC int ompi_netpatterns_setup_relwrsive_doubling_tree_node(int num_nodes, int node_rank,
        netpatterns_pair_exchange_node_t *exchange_node)
{
    return ompi_netpatterns_setup_relwrsive_doubling_n_tree_node(num_nodes, node_rank, 2, exchange_node);
}

#if 0
/*OMPI_DECLSPEC int old_netpatterns_setup_relwrsive_doubling_tree_node(int num_nodes, int node_rank,*/
OMPI_DECLSPEC int ompi_netpatterns_setup_relwrsive_doubling_n_tree_node(int num_nodes, int node_rank,int tree_order,
        netpatterns_pair_exchange_node_t *exchange_node)
{
    /* local variables */
