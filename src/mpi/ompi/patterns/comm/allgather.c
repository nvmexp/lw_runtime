        ...
        int n_peers, int *ranks_in_comm,ompi_communicator_t *comm)
{
    ...
    size_t message_extent,lwrrent_data_extent,lwrrent_data_count;
    size_t dt_size;
    ptrdiff_t dt_extent;
    char *src_buf_lwrrent;
    char *dest_buf_lwrrent;
    ...
    rc = ompi_netpatterns_setup_relwrsive_doubling_tree_node(n_peers,
    ...
    if(0 < my_exchange_node.n_extra_sources)  {
        ...
        if ( EXCHANGE_NODE == my_exchange_node.node_type ) {
             ...
             * messages in the relwrsive doubling phase */
            dest_buf_lwrrent=(char *)dest_buf+message_extent*extra_rank;
            rc=MCA_PML_CALL(recv(dest_buf_lwrrent,
             ...
             */
            extra_rank=my_exchange_node.rank_extra_source;
            src_buf_lwrrent=(char *)src_buf;
            rc=MCA_PML_CALL(send(src_buf_lwrrent,
                    count,dtype,ranks_in_comm[extra_rank],
                    -OMPI_COMMON_TAG_ALLREDUCE,
    ...
    lwrrent_data_extent=message_extent;
    lwrrent_data_count=count;
    src_buf_lwrrent=(char *)dest_buf+my_rank_in_group*message_extent;
    proc_block=1;
    ...
    for(exchange=0 ; exchange < my_exchange_node.n_exchanges ; exchange++) {
        ...
        if(pair_rank > my_rank_in_group ){
            recv_iov[0].iov_base=src_buf_lwrrent+lwrrent_data_extent;
            recv_iov[0].iov_len=lwrrent_data_extent;
            iovec_len=1;
        ...
        rc=MCA_PML_CALL(irecv(recv_iov[0].iov_base,
                    lwrrent_data_count,dtype,ranks_in_comm[pair_rank],
                    -OMPI_COMMON_TAG_ALLREDUCE,
        ...
        send_iov[0].iov_base=src_buf_lwrrent;
        send_iov[0].iov_len=lwrrent_data_extent;
        iovec_len=1;
        /* the data from the non power of 2 ranks */
        ...
        if(pair_rank < my_rank_in_group ){
            src_buf_lwrrent-=lwrrent_data_extent;
            local_data_start_rank-=proc_block;
        }
        proc_block*=2;
        lwrrent_data_extent*=2;
        lwrrent_data_count*=2;
    ...
    ompi_netpatterns_cleanup_relwrsive_doubling_tree_node(&my_exchange_node);
