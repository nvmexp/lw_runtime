        ...
        ompi_communicator_t *comm)
{
    ...
    for( stripe_number=0 ; stripe_number < n_data_segments ; stripe_number++ ) {
        ...
        sbuf_lwrrent=(char *)sbuf+count_processed*dt_extent;
        rc=ompi_datatype_copy_content_same_ddt(dtype,count_this_stripe,
                scratch_bufers[send_buffer], sbuf_lwrrent);
        if( OMPI_SUCCESS != rc ) {
            goto Error;
        }
        ...
        memcpy(rbuf_lwrrent,scratch_bufers[send_buffer], count_this_stripe*dt_size);
