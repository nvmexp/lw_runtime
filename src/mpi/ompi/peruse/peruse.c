 ...
 * should be ended by the tuple {NULL, PERUSE_LWSTOM_EVENT}.
 */
static const peruse_event_associations_t PERUSE_events[] = {
    ...
    { "PERUSE_LWSTOM_EVENT", PERUSE_LWSTOM_EVENT }
};
...
int PERUSE_Query_elwironment (int * elw_size, char *** elw)
{
    /* XXX tbd */
    return PERUSE_SUCCESS;
}
                                ...
                                peruse_event_h *          event_h)
{
    ...
    if (MPI_PARAM_CHECK) {
        ...
        if( (MPI_COMM_NULL == comm) || ompi_comm_ilwalid (comm) )
