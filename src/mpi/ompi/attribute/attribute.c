 ...
 *
 * This is complicated enough that it deserves a lengthy dislwssion of
 ...
 * and no translation oclwrs.  A pointer is written, and that same
 * pointer is returned.
 *
 * Example A: int *ret;
 *            MPI_Attr_get(..., &ret);
 ...
 * triggered and acquired right after, allowing for relwrsive behaviors.
 */
...
static void attribute_value_construct(attribute_value_t *item)
{
    item->av_key = MPI_KEYVAL_ILWALID;
...
ompi_attribute_keyval_construct(ompi_attribute_keyval_t *keyval)
{
    ...
    /* Set the keyval->key value to an invalid value so that we can know
       ...
       Specifically, the destructor may get ilwoked if we weren't able
                          ...
                          bool predefined)
{
    ...
    /* MPI says to set the returned value to MPI_KEYVAL_ILWALID */
    *key = MPI_KEYVAL_ILWALID;
                       ...
                       opal_hash_table_t *newattr_hash)
{
    ...
    while (OMPI_SUCCESS == ret) {
        ...
        /* The COPY_ATTR_CALLBACKS macro will have colwerted the
                                 ...
                                 bool predefined)
{
 ...
 exit:
    /* Decrement the ref count for the keyval.  If ref count goes to
       ...
       here if MPI_*_FREE_KEYVAL was previously ilwoked and we just
                     ...
                     bool predefined)
{
    ...
    /* Note that this function can be ilwoked by ompi_attr_copy_all()
 ...
 *
 * This function does not fail -- it is only ilwoked in "safe"
 * situations.
 */
static void *translate_to_c(attribute_value_t *val)
{
