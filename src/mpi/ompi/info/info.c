...
ompi_predefined_info_t ompi_mpi_info_elw = {{{{{0}}}}};
 ...
 * for the MPI_INFO_GET_ELW object
 */
int ompi_mpiinfo_init(void)
{
    ...
    /* Create MPI_INFO_ELW */
    OBJ_CONSTRUCT(&ompi_mpi_info_elw.info, ompi_info_t);
    assert(ompi_mpi_info_elw.info.i_f_to_c_index == 1);

    /* fill the elw info object */

    /* command for this app_context */
    if (NULL != (cptr = getelw("OMPI_COMMAND"))) {
        opal_info_set(&ompi_mpi_info_elw.info.super, "command", cptr);
    }

    /* space-separated list of argv for this command */
    if (NULL != (cptr = getelw("OMPI_ARGV"))) {
        opal_info_set(&ompi_mpi_info_elw.info.super, "argv", cptr);
    }

    /* max procs for the entire job */
    if (NULL != (cptr = getelw("OMPI_MCA_orte_ess_num_procs"))) {
        opal_info_set(&ompi_mpi_info_elw.info.super, "maxprocs", cptr);
        /* Open MPI does not support the "soft" option, so set it to maxprocs */
        opal_info_set(&ompi_mpi_info_elw.info.super, "soft", cptr);
    }

    /* local host name */
    gethostname(val, sizeof(val));
    opal_info_set(&ompi_mpi_info_elw.info.super, "host", val);

    /* architecture name */
    if (NULL != (cptr = getelw("OMPI_MCA_orte_cpu_type"))) {
        opal_info_set(&ompi_mpi_info_elw.info.super, "arch", cptr);
    }
#ifdef HAVE_SYS_UTSNAME_H
    else {
     ...
     */
    if (NULL != (cptr = getelw("OMPI_MCA_initial_wdir"))) {
        opal_info_set(&ompi_mpi_info_elw.info.super, "wdir", cptr);
    }
    ...
    case MPI_THREAD_SINGLE:
        opal_info_set(&ompi_mpi_info_elw.info.super, "thread_level", "MPI_THREAD_SINGLE");
        break;
    case MPI_THREAD_FUNNELED:
        opal_info_set(&ompi_mpi_info_elw.info.super, "thread_level", "MPI_THREAD_FUNNELED");
        break;
    case MPI_THREAD_SERIALIZED:
        opal_info_set(&ompi_mpi_info_elw.info.super, "thread_level", "MPI_THREAD_SERIALIZED");
        break;
    case MPI_THREAD_MULTIPLE:
        opal_info_set(&ompi_mpi_info_elw.info.super, "thread_level", "MPI_THREAD_MULTIPLE");
        break;
    default:
    ...
    if (NULL != (cptr = getelw("OMPI_NUM_APP_CTX"))) {
        opal_info_set(&ompi_mpi_info_elw.info.super, "ompi_num_apps", cptr);
    }

    /* space-separated list of first MPI rank of each app_context */
    if (NULL != (cptr = getelw("OMPI_FIRST_RANKS"))) {
        opal_info_set(&ompi_mpi_info_elw.info.super, "ompi_first_rank", cptr);
    }

    /* space-separated list of num procs for each app_context */
    if (NULL != (cptr = getelw("OMPI_APP_CTX_NUM_PROCS"))) {
        opal_info_set(&ompi_mpi_info_elw.info.super, "ompi_np", cptr);
    }

    /* location of the directory containing any prepositioned files
     * the user may have requested
     */
    if (NULL != (cptr = getelw("OMPI_FILE_LOCATION"))) {
        opal_info_set(&ompi_mpi_info_elw.info.super, "ompi_positioned_file_dir", cptr);
    }
...
// it's colwenient to have ompi_info_t wrappers for some of the opal_info_t
// related calls:
...
int ompi_mpiinfo_finalize(void)
{
    ...
    OBJ_DESTRUCT(&ompi_mpi_info_elw);
...
/*
 * This function is ilwoked when OBJ_NEW() is called. Here, we add this
