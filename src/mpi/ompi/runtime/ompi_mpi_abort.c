...
static bool have_been_ilwoked = false;
 ...
 * abort scenarios (but we don't, lwrrently):
 *      - MPI_Abort()
               ...
               int errcode)
{
    ...
    /* Protection for relwrsive invocation */
    if (have_been_ilwoked) {
        return OMPI_SUCCESS;
    }
    have_been_ilwoked = true;
