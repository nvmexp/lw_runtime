...
int ompi_mpi_finalize(void)
{
    ...
    /* NOTE: MPI-2.1 requires that MPI_FINALIZE is "collective" across
       *all* connected processes.  This only means that all processes
       ...
       After lengthy dislwssions many times over the course of this
       ...
       - Arguably, if B or C do try to communicate with the now-gone
         ...
         diffilwlt problem -- sure, it's easy for A to tell B that it
      ...
      crash, but that is such a diffilwlt problem that we decided we
    ...
    /* cleanup environment */
    opal_unsetelw("OMPI_COMMAND", &elwiron);
    opal_unsetelw("OMPI_ARGV", &elwiron);
