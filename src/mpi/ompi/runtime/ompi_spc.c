...
/* Array for colwerting from SPC indices to MPI_T indices */
...
static ompi_spc_event_t ompi_spc_events_names[OMPI_SPC_NUM_COUNTERS] = {
    ...
    SET_COUNTER_ARRAY(OMPI_SPC_MATCH_TIME, "The number of microseconds spent matching unexpected messages.  Note: The timer used on the back end is in cycles, which could potentially be problematic on a system where the clock frequency can change.  On such a system, this counter could be inaclwrate since we assume a fixed clock rate."),
    SET_COUNTER_ARRAY(OMPI_SPC_UNEXPECTED_IN_QUEUE, "The number of messages that are lwrrently in the unexpected message queue(s) of an MPI process."),
    SET_COUNTER_ARRAY(OMPI_SPC_OOS_IN_QUEUE, "The number of messages that are lwrrently in the out of sequence message queue(s) of an MPI process."),
...
static int ompi_spc_notify(mca_base_pvar_t *pvar, mca_base_pvar_event_t event, void *obj_handle, int *count)
{
    ...
    index = (int)(uintptr_t)pvar->ctx;  /* Colwert from MPI_T pvar index to SPC index */
 ...
 * so we need to colwert from MPI_T index to SPC index and then set the 'value' argument
...
static int ompi_spc_get_count(const struct mca_base_pvar_t *pvar, void *value, void *obj_handle)
{
    ...
    /* Colwert from MPI_T pvar index to SPC index */
    ...
    *counter_value = (long long)ompi_spc_events[index].value;
    /* If this is a timer-based counter, colwert from cycles to microseconds */
...
static void ompi_spc_dump(void)
{
    ...
    /* Colwert from cycles to usecs before sending */
    for(i = 0; i < OMPI_SPC_NUM_COUNTERS; i++) {
...
/* Stops a cycle-precision timer and callwlates the total elapsed time
...
/* Colwerts a counter value that is in cycles to microseconds.
 */
