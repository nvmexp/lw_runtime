...
#define OMPI_ADD_PROCS_LWTOFF_DEFAULT 0
uint32_t ompi_add_procs_lwtoff = OMPI_ADD_PROCS_LWTOFF_DEFAULT;
...
static bool show_elwiro_mca_params = false;
...
int ompi_mpi_register_params(void)
{
    ...
    (void) mca_base_var_register("ompi", "mpi", NULL, "show_mca_params",
                                 ...
                                 "for debug purposes). Accepted values are all, default, file, api, and elwiro - or a comma "
        ...
        if (NULL == args) {
            ...
            for (i=0; NULL != args[i]; i++) {
                if (0 == strcasecmp(args[i], "all")  || 0 == strcmp(args[i], "1")) {
                    ...
                    show_file_mca_params = true;
                } else if (0 == strncasecmp(args[i], "elw", 3)) {
                    show_elwiro_mca_params = true;
    ...
    value = mca_base_var_find ("opal", "opal", NULL, "lwda_support");
    if (0 <= value) {
        mca_base_var_register_synonym(value, "ompi", "mpi", NULL, "lwda_support",
                                      MCA_BASE_VAR_SYN_FLAG_DEPRECATED);
    }

    value = mca_base_var_find ("opal", "opal", NULL, "built_with_lwda_support");
    if (0 <= value) {
        mca_base_var_register_synonym(value, "ompi", "mpi", NULL, "built_with_lwda_support", 0);
    }

    if (opal_lwda_support && !opal_built_with_lwda_support) {
        opal_show_help("help-mpi-runtime.txt", "no lwca support",
                       true);
        ompi_rte_abort(1, NULL);
    }

    ompi_add_procs_lwtoff = OMPI_ADD_PROCS_LWTOFF_DEFAULT;
    (void) mca_base_var_register ("ompi", "mpi", NULL, "add_procs_lwtoff",
                                  ...
                                  &ompi_add_procs_lwtoff);

    ompi_mpi_dynamics_enabled = true;
    (void) mca_base_var_register("ompi", "mpi", NULL, "dynamics_enabled",
                                 "Is the MPI dynamic process functionality enabled (e.g., MPI_COMM_SPAWN)?  Default is yes, but certain transports and/or elwironments may disable it.",
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0,
                                 OPAL_INFO_LVL_4,
                                 MCA_BASE_VAR_SCOPE_READONLY,
...
int ompi_show_all_mca_params(int32_t rank, int requested, char *nodename) {
    ...
    for (i = 0 ; i < var_count ; ++i) {
        ...
        /* is this an elwiro value and we are not displaying elwiros,
         * ignore it
         */
        if (MCA_BASE_VAR_SOURCE_ELW == var->mbv_source && !show_elwiro_mca_params) {
            continue;
        }
