 ...
 * variables -- you must have a function that is ilwoked from
 ...
 * corresponding dummy function that is ilwoked from this function).
 *
 ...
 * all four Fortran symbol colwentions, it can only support one
                  ...
                  bool reinit_ok)
{
    ...
    if (!opal_atomic_compare_exchange_strong_32(&ompi_mpi_state, &expected,
        ...
        if (expected >= OMPI_MPI_STATE_FINALIZE_STARTED) {
            ...
            opal_show_help("help-mpi-runtime.txt",
                           "mpi_init: ilwoked multiple times", true);
            return MPI_ERR_OTHER;
        }
    }
     ...
     * some elwars so the MPI_INFO_ELW can get the cmd name
     * and argv (but only if the user supplied a non-NULL argv!), and
     * the requested thread level
     */
    if (NULL == getelw("OMPI_COMMAND") && NULL != argv && NULL != argv[0]) {
        opal_setelw("OMPI_COMMAND", argv[0], true, &elwiron);
    }
    if (NULL == getelw("OMPI_ARGV") && 1 < argc) {
        char *tmp;
        tmp = opal_argv_join(&argv[1], ' ');
        opal_setelw("OMPI_ARGV", tmp, true, &elwiron);
        free(tmp);
    }
     ...
     * create the local colwertor and local arch used in the proc
     * init.
     */
     ...
     * if data exchange is required. The modex oclwrs solely across procs
     ...
     * their datatype colwertors, if required
     */
    ...
    /* if we exelwted the above fence in the background, then
