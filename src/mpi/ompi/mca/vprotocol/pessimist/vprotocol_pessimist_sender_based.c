...
{
    char *path;
#ifdef SB_USE_COLWERTOR_METHOD
    mca_pml_base_send_request_t pml_req;
    sb.sb_colw_to_pessimist_offset = (uintptr_t) VPROTOCOL_SEND_REQ(NULL) -
            ((uintptr_t) &pml_req.req_base.req_colwertor -
             (uintptr_t) &pml_req);
    V_OUTPUT_VERBOSE(500, "pessimist: colw_to_pessimist_offset: %p", (void *) sb.sb_colw_to_pessimist_offset);
#endif
    sb.sb_offset = 0;
    sb.sb_length = size;
    sb.sb_pagesize = getpagesize();
    sb.sb_lwrsor = sb.sb_addr = (uintptr_t) NULL;
    sb.sb_available = 0;
...
void vprotocol_pessimist_sender_based_alloc(size_t len)
{
    ...
    sb.sb_offset += sb.sb_lwrsor - sb.sb_addr;
    sb.sb_lwrsor = sb.sb_offset % sb.sb_pagesize;
    sb.sb_offset -= sb.sb_lwrsor;

    /* Adjusting sb_length for the largest application message to fit   */
    len += sb.sb_lwrsor + sizeof(vprotocol_pessimist_sender_based_header_t);
    if(sb.sb_length < len)
        sb.sb_length = len;
    /* How much space left for application data */
    sb.sb_available = sb.sb_length - sb.sb_lwrsor;

    sb_mmap_alloc();

    sb.sb_lwrsor += sb.sb_addr; /* set absolute addr of sender_based buffer */
    V_OUTPUT_VERBOSE(30, "pessimist:\tsb\tgrow\toffset %llu\tlength %llu\tbase %p\tlwrsor %p", (unsigned long long) sb.sb_offset, (unsigned long long) sb.sb_length, (void *) sb.sb_addr, (void *) sb.sb_lwrsor);
}

#undef sb

#ifdef SB_USE_COLWERTOR_METHOD
int32_t vprotocol_pessimist_sender_based_colwertor_advance(opal_colwertor_t* pColwertor,
                                                            struct iovec* iov,
                                                            uint32_t* out_size,
                                                            size_t* max_data) {
    int ret;
    ...
    ftreq = VPESSIMIST_COLW_REQ(pColwertor);
    pColwertor->flags = ftreq->sb.colw_flags;
    pColwertor->fAdvance = ftreq->sb.colw_advance;
    ret = opal_colwertor_pack(pColwertor, iov, out_size, max_data);
    ...
    pColwertor->flags &= ~COLWERTOR_NO_OP;
    pColwertor->fAdvance = &vprotocol_pessimist_sender_based_colwertor_advance;
    return ret;
}
#endif
