...
#define __VPROTOCOL_PESSIMIST_SEND_BUFFER() do {                              \
    ...
    {                                                                         \
        ...
        if(OPAL_UNLIKELY(ompi_comm_ilwalid(mca_vprotocol_pessimist.el_comm))) \
        {                                                                     \
            rc = vprotocol_pessimist_event_logger_connect(0,                  \
                                        &mca_vprotocol_pessimist.el_comm);    \
            if(OMPI_SUCCESS != rc)                                            \
                OMPI_ERRHANDLER_ILWOKE(mca_vprotocol_pessimist.el_comm, rc,   \
...
static inline void vprotocol_pessimist_delivery_log(ompi_request_t *req)
{
    ...
    if(req == NULL)
    {
        ...
        {
            /* conselwtive probes not delivering anything are merged */
