...
/*******************************************************************************
 * Colwertor pack (blocking) method (good latency, bad bandwidth)
 */
...
static inline void __SENDER_BASED_METHOD_COPY(mca_pml_base_send_request_t *pmlreq)
{
    if(0 != pmlreq->req_bytes_packed)
    {
        ...
        opal_colwertor_clone_with_position( &pmlreq->req_base.req_colwertor,
                                            &colw, 0, &zero );
        opal_colwertor_pack(&colw, &iov, &iov_count, &max_data);
    }
}
...
/*******************************************************************************
 * Colwertor replacement (non blocking) method (under testing)
 */
#elif defined(SB_USE_COLWERTOR_METHOD)
int32_t vprotocol_pessimist_sender_based_colwertor_advance(opal_colwertor_t*,
                                                            struct iovec*,
                                                            uint32_t*,
                                                            size_t*);
...
#define __SENDER_BASED_METHOD_COPY(REQ) do {                                  \
    ...
    mca_vprotocol_pessimist_send_request_t *ftreq;                            \
                                                                              \
    pColw = & (REQ)->req_base.req_colwertor;                                  \
    ftreq = VPESSIMIST_SEND_FTREQ(REQ);                                       \
    ftreq->sb.colw_flags = pColw->flags;                                      \
    ftreq->sb.colw_advance = pColw->fAdvance;                                 \
                                                                              \
    pColw->flags &= ~COLWERTOR_NO_OP;                                         \
    pColw->fAdvance = vprotocol_pessimist_sender_based_colwertor_advance;     \
} while(0)

#define __SENDER_BASED_METHOD_FLUSH(REQ)

#define VPESSIMIST_COLW_REQ(COLW) ((mca_vprotocol_pessimist_send_request_t *) \
    (mca_vprotocol_pessimist.sender_based.sb_colw_to_pessimist_offset +       \
     (uintptr_t) ((COLW)->clone_of)))
...
static inline int vprotocol_pessimist_sb_progress_req(mca_pml_base_send_request_t *req)
{
    ...
    if(ftreq->sb.bytes_progressed < req->req_bytes_packed)
    {
        ...
        opal_colwertor_clone_with_position(&req->req_base.req_colwertor,
                                           &colw, 0, &position );
        opal_colwertor_pack(&colw, &iov, &iov_count, &max_data);
...
static inline void vprotocol_pessimist_sender_based_copy_start(ompi_request_t *req)
{
    ...
    ftreq->sb.cursor = mca_vprotocol_pessimist.sender_based.sb_lwrsor;
#if 1
    mca_vprotocol_pessimist.sender_based.sb_lwrsor +=
        sizeof(vprotocol_pessimist_sender_based_header_t) +
        pmlreq->req_bytes_packed;
