                                                             ...
                                                             MPI_Offset **offbuff,struct mca_sharedfp_base_data_t *sh)
{
    int num = 0, i= 0, ctr = 0;
    int ret=OMPI_SUCCESS;
    mca_sharedfp_individual_metadata_node *lwrrnode;
    ...
    while (lwrrnode)  {
        ...
        *(*rec_length + ctr) = lwrrnode->recordlength;
        *(*buff + ctr) = lwrrnode->timestamp;
        *(*offbuff + ctr) = lwrrnode->localposition;

        ctr = ctr + 1;

        headnode->next = lwrrnode->next;
        if ( mca_sharedfp_individual_verbose ) {
            opal_output(ompi_sharedfp_base_framework.framework_output,
