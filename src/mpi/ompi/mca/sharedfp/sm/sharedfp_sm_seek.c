                      ...
                      OMPI_MPI_OFFSET_TYPE off, int whence)
{
    ...
    if( 0 == fh->f_rank ){
        if ( MPI_SEEK_SET == whence){
        ...
        else if( MPI_SEEK_LWR == whence){
            OMPI_MPI_OFFSET_TYPE lwrrent_position;
            ret = mca_sharedfp_sm_get_position ( fh, &lwrrent_position);
            if ( mca_sharedfp_sm_verbose ) {
                opal_output(ompi_sharedfp_base_framework.framework_output,
                            "sharedfp_sm_seek: MPI_SEEK_LWR: lwrr=%lld, offset=%lld, call status=%d\n",
                            lwrrent_position,offset,status);
            }
            offset = lwrrent_position + offset;
            if ( mca_sharedfp_sm_verbose ) {
                opal_output(ompi_sharedfp_base_framework.framework_output,
                            "sharedfp_sm_seek: MPI_SEEK_LWR: new_offset=%lld\n",offset);
            }
            if(offset < 0){
                opal_output(0,"sharedfp_sm_seek - MPI_SEEK_LWRE, offset must be > 0, got offset=%lld.\n",offset);
                ret = -1;
            }
        }
            ...
            if(offset < 0){
                opal_output(0,"sharedfp_sm_seek - MPI_SEEK_LWR, offset must be > 0, got offset=%lld.\n",offset);
                ret = -1;
            }
        }
        else {
