...
static bool lwda_elwvar_set = false;
#endif
...
ompi_mtl_psm2_component_register(void)
{
    ...
    if ((num_local_procs == num_total_procs) && (1 < num_total_procs)) {
        ...
        setelw("PSM2_DEVICES", "self,shm", 0);
    }
...
#if OPAL_LWDA_SUPPORT
    if (lwda_elwvar_set) {
        opal_unsetelw("PSM2_LWDA", &elwiron);
    }
#endif
    return OMPI_SUCCESS;
}
...
get_local_rank(int *out_rank)
{
    ...
    *out_rank = 0;
    ...
    if (OMPI_NODE_RANK_ILWALID == (my_node_rank =
...
#if OPAL_LWDA_SUPPORT
    /*
     * If using LWCA enabled Open MPI, the user likely intends to
     * run with LWCA buffers. So, force-set the elwvar here if user failed
     * to set it.
     */
    ret = glob("/sys/module/lwpu", GLOB_DOOFFS, NULL, &globbuf);
    if (globbuf.gl_pathc > 0) {
        globfree(&globbuf);
    }

    lwda_elw = getelw("PSM2_LWDA");
    if (!lwda_elw && (0 == ret)) {
        opal_show_help("help-mtl-psm2.txt",
                       "no psm2 lwca elw", true,
                       ompi_process_info.nodename);
        opal_setelw("PSM2_LWDA", "1", false, &elwiron);
        lwda_elwvar_set = true;
    }
#endif
