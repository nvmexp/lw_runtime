...
int ompi_mtl_psm2_module_init(int local_rank, int num_local_procs) {
    ...
    generated_key = getelw(OPAL_MCA_PREFIX"orte_precondition_transports");
     ...
     * contexts correctly.
     */
    snprintf(elw_string, sizeof(elw_string), "%d", local_rank);
    setelw("MPI_LOCALRANKID", elw_string, 0);
    snprintf(elw_string, sizeof(elw_string), "%d", num_local_procs);
    setelw("MPI_LOCALNRANKS", elw_string, 0);
    ...
    ompi_mtl_psm2.super.mtl_flags |= MCA_MTL_BASE_FLAG_LWDA_INIT_DISABLE;
#endif
...
ompi_mtl_psm2_connect_error_msg(psm2_error_t err)
{
	...
	case PSM2_EPID_ILWALID_NODE:
	case PSM2_EPID_ILWALID_MTU:
	case PSM2_EPID_ILWALID_UUID_KEY:
	case PSM2_EPID_ILWALID_VERSION:
	case PSM2_EPID_ILWALID_CONNECT:
...
int ompi_mtl_psm2_progress( void ) {
    ...
    do {
        ...
        if (mtl_psm2_request->type == OMPI_mtl_psm2_IRECV) {
            ...
            ompi_mtl_datatype_unpack(mtl_psm2_request->colwertor,
