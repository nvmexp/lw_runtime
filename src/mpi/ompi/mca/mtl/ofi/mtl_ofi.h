...
ompi_mtl_ofi_context_progress(int ctxt_id)
{
    ...
    while (true) {
        ...
        if (ret > 0) {
            ...
            /**
             * An error oclwred and is being reported via the CQ.
                  ...
                  bool ofi_cq_data)
{
    ...
    ompi_ret = ompi_mtl_datatype_pack(colwertor, &start, &length, &free_after);
                           ...
                           ompi_mtl_ofi_request_t *ofi_req)
{
    ...
    if (OPAL_UNLIKELY(ofi_req->buffer)) {
        ompi_ret = ompi_mtl_datatype_unpack(ofi_req->colwertor,
                                            ofi_req->buffer,
                                            wc->len);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ompi_ret)) {
            opal_output_verbose(1, opal_common_ofi.output,
    ...
    */
     ...
     * If a failure oclwrs the provider will notify the error
...
/**
 * Called when an error oclwred on a recv request.
 */
__opal_attribute_always_inline__ static inline int
                   ...
                   bool ofi_cq_data)
{
    ...
    ompi_ret = ompi_mtl_datatype_recv_buf(colwertor,
                                          &start,
                                          &length,
                                          &free_after);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ompi_ret)) {
...
/**
 * Called when an error oclwred on a mrecv request.
 */
__opal_attribute_always_inline__ static inline int
               ...
               mca_mtl_request_t *mtl_request)
{
    return ompi_mtl_ofi_isend_generic(mtl, comm, dest, tag,
                                    colwertor, mode, blocking, mtl_request,
                                    ompi_mtl_ofi.fi_cq_data);
}
