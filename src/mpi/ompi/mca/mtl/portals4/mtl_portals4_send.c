                           ...
                           bool *complete)
{
    ...
    if (PTL_EVENT_GET == ev->type) {
        ...
        if (!PtlHandleIsEqual(ptl_request->me_h, PTL_ILWALID_HANDLE)) {
            ...
            ptl_request->me_h = PTL_ILWALID_HANDLE;
        }
    }
    ...
    if ((PTL_EVENT_ACK == ev->type) &&
        ...
        (!PtlHandleIsEqual(ptl_request->me_h, PTL_ILWALID_HANDLE))) {
                             ...
                             ompi_mtl_portals4_isend_request_t *ptl_request)
{
    ...
    /* We have to wait for some GET events.
       ...
       else we will re-callwlate this number when we received the first ACK event (with remote overflow list)
     */
                             ...
                             ompi_mtl_portals4_isend_request_t* ptl_request)
{
    ...
    ret = ompi_mtl_datatype_pack(colwertor, &start, &length, &free_after);
                       ...
                       mca_pml_base_send_mode_t mode)
{
    ...
    ret = ompi_mtl_portals4_send_start(mtl, comm, dest, tag,
                                       colwertor, mode, &ptl_request.super);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
