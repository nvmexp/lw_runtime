...
portals4_init_interface(void)
{
 ...
 error:
    if (!PtlHandleIsEqual(ompi_mtl_portals4.long_overflow_me_h, PTL_ILWALID_HANDLE)) {
        PtlMEUnlink(ompi_mtl_portals4.long_overflow_me_h);
    }
    if (!PtlHandleIsEqual(ompi_mtl_portals4.zero_md_h, PTL_ILWALID_HANDLE)) {
        PtlMDRelease(ompi_mtl_portals4.zero_md_h);
    }
    if (!PtlHandleIsEqual(ompi_mtl_portals4.send_md_h, PTL_ILWALID_HANDLE)) {
    ...
    if (!PtlHandleIsEqual(ompi_mtl_portals4.send_eq_h, PTL_ILWALID_HANDLE)) {
        PtlEQFree(ompi_mtl_portals4.send_eq_h);
    }
    if (!PtlHandleIsEqual(ompi_mtl_portals4.recv_eq_h, PTL_ILWALID_HANDLE)) {
...
ompi_mtl_portals4_finalize(struct mca_mtl_base_module_t *mtl)
{
    ...
    if (!PtlHandleIsEqual(ompi_mtl_portals4.ni_h, PTL_ILWALID_HANDLE)) {
