    ...
    case PTL_EVENT_PUT:
        ...
        if (!MTL_PORTALS4_IS_SHORT_MSG(ev->match_bits) && msg_length > ev->mlength) {
            ...
            /* If we're either using the eager protocol or were a
               ...
               the message. */
            ret = ompi_mtl_datatype_unpack(ptl_request->colwertor,
                                           ev->start,
                                           ev->mlength);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
                opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
    ...
    case PTL_EVENT_PUT_OVERFLOW:
        ...
        if (MTL_PORTALS4_IS_SHORT_MSG(ev->match_bits)) {
            ...
            if (ev->mlength > 0) {
                ...
                ret = opal_colwertor_unpack(ptl_request->colwertor,
                                            &iov, &iov_count,
                                            &max_data );
                if (NULL != ptl_request->buffer_ptr) free(ptl_request->buffer_ptr);
                if (OPAL_UNLIKELY(ret < 0)) {
                    opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
                                        "%s:%d: opal_colwertor_unpack failed: %d",
                                        __FILE__, __LINE__, ret);
                    goto callback_error;
                }
            }
                        ...
                        mca_mtl_request_t *mtl_request)
{
    ...
    ret = ompi_mtl_datatype_recv_buf(colwertor, &start, &length, &free_after);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    ptl_request->colwertor = colwertor;
