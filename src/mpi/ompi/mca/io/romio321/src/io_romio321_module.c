 ...
 * MPI status.  Colweniently, it maps directly to
                          ...
                          struct ompi_datatype_t *datatype, MPI_Count nbytes)
{
    /* Note that ROMIO is going to give a number of *bytes* here, but
       ...
       rather than try to do a colwersion up here, just set the number
       of bytes with MPI_CHAR as the datatype.  If someone does a
       GET_STATUS later, they have to supply their own datatype, and
       we do the right callwlations there.  This prevents roundoff
