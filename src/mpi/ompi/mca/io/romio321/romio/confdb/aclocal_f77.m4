...
dnl.ve
dnl If an action is specified, it is exelwted instead.
dnl 
dnl Notes:
...
# We could have done the character colwersion with 'tr'
...
dnl Restore FFLAGS before 2nd/3rd argument commands are exelwted,
...
dnl arguments, but produced an exelwtable anyway.
