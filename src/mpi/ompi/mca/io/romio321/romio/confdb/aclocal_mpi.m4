...
dnl
dnl Notes:
dnl Lwrrently, only checks for lib mpi and mpi.h.  Later, we will add
...
dnl when the MPI version of the compiler creates exelwtables that cannot
dnl be run on the local system (for example, the IBM SP, where exelwtables
dnl created with mpcc will not run locally, but exelwtables created
...
dnl it became necessary to define CC etc. before ilwoking AC_PROG_CC (and
...
dnl AC_PROG_CC (or anything that might cause AC_PROG_CC to be ilwoked).
dnl
dnl See also:
...
dnl
dnl To keep autoconf from prematurely ilwoking the compiler check scripts,
...
dnl However, this function is not lwrrently used at all.
