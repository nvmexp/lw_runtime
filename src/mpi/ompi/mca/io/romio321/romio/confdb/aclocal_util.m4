...
for dir in $1 ; do
	...
	tmp_lwrdir="${tmp_lwrdir}$tmp_subdir"
	if test ! -d "$tmp_lwrdir" ; then mkdir "$tmp_lwrdir" ; fi
        tmp_lwrdir="${tmp_lwrdir}/"
    done
    PAC_POP_FLAG([IFS])
done
])
...
dnl
dnl Colwerts the given hexadecimal integer constant to an integer constant and
...
AC_DEFUN([PAC_COLW_HEX_TO_DEC],[AS_VAR_SET([$2],[`printf "%d" $1`])])
