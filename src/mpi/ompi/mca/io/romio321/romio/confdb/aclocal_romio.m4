...
if test -n "$arch_IRIX"; then
   ...
   changequote(,)dnl
   cpucount=`hilw | grep '[0-9]* [0-9]* MHZ IP[0-9]* Proc' | cut -f 1 -d' '`
   if test "$cpucount" = "" ; then
     cpucount=`hilw | grep 'Processor [0-9]*:' | wc -l | sed -e 's/ //g'`
   fi
   changequote([,])dnl
   if test "$cpucount" = "" ; then
   ...
   dnl -comm=shared
   cputype=`hilw -t cpu | tail -1 | cut -f 3 -d' '`
   if test -z "$cputype" ; then
        AC_MSG_RESULT([Could not get cputype from hilw -t cpu command. Please send])
        hilw -t cpu 2>&1
        hilw -t cpu | cut -f 3 -d' ' 2>&1
	AC_MSG_ERROR([to romio-maint@mcs.anl.gov])
   fi
...
main() { 
  ...
  FILE *f=fopen("memaliglwal","w");
  if (fd == -1) exit(1);
  if (!f) exit(1);
  fcntl(fd, F_DIOINFO, &st);
  fprintf( f, "%u\n", st.d_mem);
  exit(0);
}],Pac_CV_NAME=`cat memaliglwal`,Pac_CV_NAME="")
rm -f memaliglwal
rm -f /tmp/romio_tmp.bin
