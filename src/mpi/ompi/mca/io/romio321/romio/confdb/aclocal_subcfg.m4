...
dnl    which are in turns ilwoked by other subconfigure.
          ...
          *)
            ...
            for pac_pvar in $ac_precious_vars ; do
                ...
                if ( echo $pac_arg | grep "^$pac_pvar=" >/dev/null 2>&1 ) ; then
                    # check if current precious variable is set in elw
                    ...
                    if test "$pvar_set" = "set" ; then
                        ...
                        # value_of_prec_var is fetched from the elw.
...
dnl colwerts the path given by MACRO_PATH_SUFFIX (with '/' chars in it) to one
...
dnl colwert path separators into '_', the m4_translit is intentionally unquoted
...
dnl ilwokes the PAC_SUBCFG_BODY_foo macro for the "foo" subsys, when "foo" is
...
dnl ilwokes the PAC_SUBCFG_PREREQ_foo macro for the "foo" subsys, when "foo" is
dnl passed as the only argument to this macro.  The first arg may be the '/'
dnl path version instead of having underscores.
AC_DEFUN([PAC_SUBCFG_DO_PREREQ],[PAC_SUBCFG_EXPAND_PATH_SUFFIX_MACRO([PAC_SUBCFG_PREREQ],[$1])])

dnl takes no arguments, expands to "foo_bar_baz" when ilwoked in a file named
