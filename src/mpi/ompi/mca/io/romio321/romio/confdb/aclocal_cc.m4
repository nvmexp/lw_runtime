...
if test "$pac_result" = "yes" \
    ...
    AC_LINK_IFELSE([
        ...
        dnl which clang (but not GCC) would treat as an *error*, ilwalidating
...
dnl Restore CFLAGS before 2nd/3rd argument commands are exelwted,
...
correctly set error code when a fatal error oclwrs])
fi
])
...
if test "$enable_strict_done" != "yes" ; then
    ...
    #       value requires further ilwestigation; 1 MiB would have at least
    ...
    #	    the aggregate value is colwerted to a scalar.
    ...
    #       one needs to compare the return value with 0 to see whether a colwersion
    #       was performed.
    ...
    #       throwing too many (correct) warnings lwrrently, causing us
    ...
    # However, since Intel compiler lwrrently does not include -Wtype-limits
    ...
    if test "${enable_strict_done}" = "yes" ; then
       ...
       if test "${enable_c99}" = "yes" ; then
          ...
          # for dislwssion on why not to use -xc99
...
dnl
dnl Possible values lwrrently include no and four.
dnl
