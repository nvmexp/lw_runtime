...
dnl
dnl The "action-if-not-given" part of AC_ARG_ENABLE is not exelwted until
dnl after the AC_CACHE_LOAD is exelwted (!).  Thus, the value of 
...
dnl
dnl/*D 
dnl PAC_ARG_CACHING - Enable caching of results from a configure exelwtion
dnl
dnl Synopsis:
dnl PAC_ARG_CACHING
dnl
...
dnl commands are exelwted *before* the subdir configures.
dnl
dnl D*/
...
if test "x$1" = "xalways" -o \( "$cache_file" = "/dev/null" -a "X$real_enable_cache" = "Xnotgiven" \) ; then
    ...
    ac_cv_elw_CC_set=set
    ac_cv_elw_CC_value=$CC
    ac_cv_elw_CFLAGS_set=${CFLAGS+set}
    ac_cv_elw_CFLAGS_value=$CFLAGS
    ac_cv_elw_CPP_set=set
    ac_cv_elw_CPP_value=$CPP
    ac_cv_elw_CPPFLAGS_set=${CPPFLAGS+set}
    ac_cv_elw_CPPFLAGS_value=$CPPFLAGS
    ac_cv_elw_LDFLAGS_set=${LDFLAGS+set}
    ac_cv_elw_LDFLAGS_value=$LDFLAGS
    ac_cv_elw_LIBS_set=${LIBS+set}
    ac_cv_elw_LIBS_value=$LIBS
    ac_cv_elw_FC_set=${FC+set}
    ac_cv_elw_FC_value=$FC
    ac_cv_elw_F77_set=${F77+set}
    ac_cv_elw_F77_value=$F77
    ac_cv_elw_F90_set=${F90+set}
    ac_cv_elw_F90_value=$F90
    ac_cv_elw_FFLAGS_set=${FFLAGS+set}
    ac_cv_elw_FFLAGS_value=$FFLAGS
    ac_cv_elw_CXX_set=${CXX+set}
    ac_cv_elw_CXX_value=$CXX

    ac_elw_CC_set=set
    ac_elw_CC_value=$CC
    ac_elw_CFLAGS_set=${CFLAGS+set}
    ac_elw_CFLAGS_value=$CFLAGS
    ac_elw_CPP_set=set
    ac_elw_CPP_value=$CPP
    ac_elw_CPPFLAGS_set=${CPPFLAGS+set}
    ac_elw_CPPFLAGS_value=$CPPFLAGS
    ac_elw_LDFLAGS_set=${LDFLAGS+set}
    ac_elw_LDFLAGS_value=$LDFLAGS
    ac_elw_LIBS_set=${LIBS+set}
    ac_elw_LIBS_value=$LIBS
    ac_elw_FC_set=${FC+set}
    ac_elw_FC_value=$FC
    ac_elw_F77_set=${F77+set}
    ac_elw_F77_value=$F77
    ac_elw_F90_set=${F90+set}
    ac_elw_F90_value=$F90
    ac_elw_FFLAGS_set=${FFLAGS+set}
    ac_elw_FFLAGS_value=$FFLAGS
    ac_elw_CXX_set=${CXX+set}
    ac_elw_CXX_value=$CXX

    dnl other parameters are
    dnl build_alias, host_alias, target_alias

    # It turns out that A C CACHE_SAVE can't be ilwoked more than once
