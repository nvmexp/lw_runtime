...
int MPIU_write_external32_colwersion_fn (const void *userbuf, MPI_Datatype datatype,
...
int MPIU_read_external32_colwersion_fn(void *userbuf, MPI_Datatype datatype,
        int count, void *filebuf)
{
    int position_i = 0;
    MPI_Aint position = 0;
...
int MPIU_external32_buffer_setup(const void * buf, int count, MPI_Datatype type, void **newbuf)
{
    ...
    *newbuf = ADIOI_Malloc(bytes);
    ...
    error_code = MPIU_write_external32_colwersion_fn(buf, type, count, *newbuf);
    if (error_code != MPI_SUCCESS) {
	ADIOI_Free(*newbuf);
	return error_code;
    }
