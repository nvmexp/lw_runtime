...
int MPI_Register_datarep(const char *datarep, MPI_Datarep_colwersion_function *read_colwersion_fn,
			 MPI_Datarep_colwersion_function *write_colwersion_fn,
...
. read_colwersion_fn - function ilwoked to colwert from file representation to
                 native representation (function)
. write_colwersion_fn - function ilwoked to colwert from native representation to
                  file representation (function)
. dtype_file_extent_fn - function ilwoked to get the exted of a datatype as represented
                  in the file (function)
- extra_state - pointer to extra state that is passed to each of the
                three functions

 Notes:
 This function allows the user to provide routines to colwert data from
...
int MPI_Register_datarep(ROMIO_CONST char *datarep,
			 MPI_Datarep_colwersion_function *read_colwersion_fn,
			 MPI_Datarep_colwersion_function *write_colwersion_fn,
			 ...
			 void *extra_state)
{
    ...
    /* Check Non-NULL Read and Write colwersion function pointer */
    /* Read and Write colwersions are lwrrently not supported.   */
    if ( (read_colwersion_fn != NULL) || (write_colwersion_fn != NULL) )
    {
        error_code = MPIO_Err_create_code(MPI_SUCCESS, MPIR_ERR_RECOVERABLE,
                                          myname, __LINE__,
                                          MPI_ERR_COLWERSION,
                                          "**drcolwnotsupported", 0);
    ...
    adio_datarep->read_colw_fn  = read_colwersion_fn;
    adio_datarep->write_colw_fn = write_colwersion_fn;
