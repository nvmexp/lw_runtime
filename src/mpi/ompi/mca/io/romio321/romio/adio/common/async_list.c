...
#define NUM 100
    ...
    ADIOI_Async_node *lwrr, *ptr;
    int i;

    if (!ADIOI_Async_avail_head) {
	ADIOI_Async_avail_head = (ADIOI_Async_node *)
	              ADIOI_Malloc(NUM*sizeof(ADIOI_Async_node));  
	lwrr = ADIOI_Async_avail_head;
	for (i=1; i<NUM; i++) {
	    lwrr->next = ADIOI_Async_avail_head+i;
	    lwrr = lwrr->next;
	}
	lwrr->next = NULL;
	ADIOI_Async_avail_tail = lwrr;
...
/* add request to list of outstanding requests */
	...
	lwrr = ADIOI_Async_list_tail;
	lwrr->next = ADIOI_Malloc_async_node();
	ADIOI_Async_list_tail = lwrr->next;
	ADIOI_Async_list_tail->request = request;
	ADIOI_Async_list_tail->prev = lwrr;
...
/* Delete a request that has already been completed from the async
   ...
   from within an ADIO_Test/ADIO_Wait. */ 
    ...
    ADIOI_Async_node *lwrr, *prev, *next;

    lwrr = (*request)->ptr_in_async_list;
    prev = lwrr->prev;

    if (prev) prev->next = lwrr->next;
    else ADIOI_Async_list_head = lwrr->next;

    next = lwrr->next;
    if (next) next->prev = prev;
    else ADIOI_Async_list_tail = prev;

    ADIOI_Free_async_node(lwrr);
}
