...
typedef struct open_status_s open_status;
    ...
    /* steps for getting a handle:  (it gets a little colwoluted, but at least
     * it's deterministic) 
     * . lookup the file.  
...
void ADIOI_PVFS2_Open(ADIO_File fd, int *error_code)
{
	...
	*error_code = MPIO_Err_create_code(MPI_SUCCESS,
    ...
    /* lwrrently everyone gets their own credentials */
	...
	ret = PVFS_util_resolve(fd->filename, &lwr_fs, 
	...
	*error_code = MPIO_Err_create_code(MPI_SUCCESS,
					   MPIR_ERR_RECOVERABLE,
					   myname, __LINE__,
					   ADIOI_PVFS2_error_colwert(o_status.error),
					   "Unknown error", 0);
	/* TODO: FIX STRING */
	return;
    }
