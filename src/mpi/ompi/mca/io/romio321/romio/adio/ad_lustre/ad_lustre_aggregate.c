				    ...
				    int mode)
{
    ...
    /* Callwlate the available number of I/O clients */
    if (!mode) {
        /* for collective read,
    ...
    /* Callwlate how many IO clients we need */
...
/* ADIOI_LUSTRE_Calc_my_req() - callwlate what portions of the access requests
			      ...
			      int ***buf_idx_ptr)
{
    ...
    int i, l, proc;
    ADIO_Offset avail_len, rem_len, lwrr_idx, off;
    ADIOI_Access *my_req;
			   ...
			   ADIO_Offset *len_list, int nprocs)
{
    ...
    /* callwlate total_req_size and total_access_count */
    for (i = 0; i < contig_access_count; i++)
        req_size += len_list[i];
