...
void ADIOI_PANFS_SetInfo(ADIO_File fd, MPI_Info users_info, int *error_code)
{
    ...
    *error_code = MPI_SUCCESS;
	...
	/* anticipate conlwrrent writes in an MPI-IO application */
	ADIOI_Info_set (fd->info, "panfs_conlwrrent_write", "1");

        /* has user specified striping parameters 
               and do they have the same value on all processes? */
        if (users_info != MPI_INFO_NULL) {

	    ADIOI_Info_check_and_install_int(fd, users_info, "panfs_conlwrrent_write",
