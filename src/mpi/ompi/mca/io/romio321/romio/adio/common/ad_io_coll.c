 ...
 * It callwlates the 1st and last byte accessed */
			...
			ADIO_Offset *st_offset, ADIO_Offset *end_offset)
{
	...
	*end_offset = -1;
	...
	if (file_ptr_type == ADIO_INDIVIDUAL) {	
	    ...
	    /* callwlate byte starting point of first filetype */
		      ...
		      int rdwr, ADIO_Status *status, int *error_code)
{
...
#endif
    MPI_Type_get_extent(lwstom_ftype, &lb, &f_extent);
    MPI_Type_size_x(lwstom_ftype, &f_size);
    ...
    ADIOI_Datatype_iscontig(lwstom_ftype, &f_is_contig);
    ADIOI_Datatype_iscontig(datatype, &m_is_contig);
    if (!f_is_contig)
	ADIOI_Flatten_datatype (lwstom_ftype);
    ...
    if (!f_is_contig)
	ADIOI_Delete_flattened (lwstom_ftype);
