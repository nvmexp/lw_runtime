	...
	ADIO_Offset* fd_end)
{
    ...
    *error_code = MPI_SUCCESS; /* initialize to success */
	...
	for (i=0;i<numTargetAggs;i++) {
		...
		/* here are the offsets lwrrently being collected by the aggregator during this round */
		ADIO_Offset lwrrentRoundFDStartForMyTargetAgg = (ADIO_Offset)((ADIO_Offset)targetAggsForMyDataFDStart[i] + (ADIO_Offset)((ADIO_Offset)roundIter*(ADIO_Offset)coll_bufsize));
		ADIO_Offset lwrrentRoundFDEndForMyTargetAgg = (ADIO_Offset)((ADIO_Offset)targetAggsForMyDataFDStart[i] + (ADIO_Offset)((ADIO_Offset)(roundIter+1)*(ADIO_Offset)coll_bufsize) - (ADIO_Offset)1);
		if (lwrrentRoundFDEndForMyTargetAgg > targetAggsForMyDataFDEnd[i])
		    lwrrentRoundFDEndForMyTargetAgg = targetAggsForMyDataFDEnd[i];

#ifdef p2pcontigtrace
		printf("roundIter %d target iter %d targetAggsForMyData is %d myOffsetStart is %ld myOffsetEnd is %ld targetAggsForMyDataFDStart is %ld targetAggsForMyDataFDEnd is %ld lwrrentRoundFDStartForMyTargetAgg is %ld lwrrentRoundFDEndForMyTargetAgg is %ld\n",
			roundIter,i,targetAggsForMyData[i],myOffsetStart,myOffsetEnd,
			targetAggsForMyDataFDStart[i],targetAggsForMyDataFDEnd[i],
			lwrrentRoundFDStartForMyTargetAgg,lwrrentRoundFDEndForMyTargetAgg);
#endif

		/* send the portion of my data that is within
		 * lwrrentRoundFDStartForMyTargetAgg to
		 * lwrrentRoundFDEndForMyTargetAgg */
		/* find the offset into the send buffer and the amount
		 * of data to send */
		int sendBufferOffset = 0;
		int bufferAmountToSend = 0;

		if ((myOffsetStart >= lwrrentRoundFDStartForMyTargetAgg) && (myOffsetStart <= lwrrentRoundFDEndForMyTargetAgg)) {
		    if (myOffsetEnd > lwrrentRoundFDEndForMyTargetAgg)
			bufferAmountToSend = (lwrrentRoundFDEndForMyTargetAgg - myOffsetStart) +1;
		    else
			bufferAmountToSend = (myOffsetEnd - myOffsetStart) +1;
		}
		else if ((myOffsetEnd >= lwrrentRoundFDStartForMyTargetAgg) && (myOffsetEnd <= lwrrentRoundFDEndForMyTargetAgg)) {
		    sendBufferOffset = (int) (lwrrentRoundFDStartForMyTargetAgg - myOffsetStart);
		    if (myOffsetEnd > lwrrentRoundFDEndForMyTargetAgg)
			bufferAmountToSend = (lwrrentRoundFDEndForMyTargetAgg - lwrrentRoundFDStartForMyTargetAgg) +1;
		    else
			bufferAmountToSend = (myOffsetEnd - lwrrentRoundFDStartForMyTargetAgg) +1;
		}
		else if ((myOffsetStart <= lwrrentRoundFDStartForMyTargetAgg) && (myOffsetEnd >= lwrrentRoundFDEndForMyTargetAgg)) {
		    sendBufferOffset = (int) (lwrrentRoundFDStartForMyTargetAgg - myOffsetStart);
		    bufferAmountToSend = (lwrrentRoundFDEndForMyTargetAgg - lwrrentRoundFDStartForMyTargetAgg) +1;
		}
	...
	if (iAmUsedAgg) {
		...
		lwrrentRoundFDEnd = lwrrentRoundFDStart + coll_bufsize - 1;
#ifdef p2pcontigtrace
	    printf("lwrrentRoundFDStart is %ld lwrrentRoundFDEnd is %ld within file domeain %ld to %ld\n",lwrrentRoundFDStart,lwrrentRoundFDEnd,fd_start[myAggRank],fd_end[myAggRank]);
#endif
	}
	...
	for (i=0;i<numSourceProcs;i++) {
	    if ((remainingDataOffsetToGetPerProc[i] >= lwrrentRoundFDStart) && (remainingDataOffsetToGetPerProc[i] <= lwrrentRoundFDEnd)) {
		if ((remainingDataOffsetToGetPerProc[i] + remainingDataAmountToGetPerProc[i]) <= lwrrentRoundFDEnd)
		    dataSizeGottenThisRoundPerProc[i] = remainingDataAmountToGetPerProc[i];
		else
		    dataSizeGottenThisRoundPerProc[i] = (lwrrentRoundFDEnd - remainingDataOffsetToGetPerProc[i]) +1;
	    }
	    else if (((remainingDataOffsetToGetPerProc[i]+remainingDataAmountToGetPerProc[i]) >= lwrrentRoundFDStart) && ((remainingDataOffsetToGetPerProc[i]+remainingDataAmountToGetPerProc[i]) <= lwrrentRoundFDEnd)) {
		if ((remainingDataOffsetToGetPerProc[i]) >= lwrrentRoundFDStart)
		    dataSizeGottenThisRoundPerProc[i] = remainingDataAmountToGetPerProc[i];
		else
		    dataSizeGottenThisRoundPerProc[i] = (remainingDataOffsetToGetPerProc[i]-lwrrentRoundFDStart) +1;
	    }
	    else
		dataSizeGottenThisRoundPerProc[i] = 0;

#ifdef p2pcontigtrace
	    printf("dataSizeGottenThisRoundPerProc[%d] set to %d - remainingDataOffsetToGetPerProc is %d remainingDataAmountToGetPerProc is %d lwrrentRoundFDStart is %d lwrrentRoundFDEnd is %d\n",i,dataSizeGottenThisRoundPerProc[i],remainingDataOffsetToGetPerProc[i],remainingDataAmountToGetPerProc[i],lwrrentRoundFDStart,lwrrentRoundFDEnd);
#endif
	...
	for (i=0;i<numSourceProcs;i++) {
	    ...
	    int lwrrentWBOffset = 0;
	    for (j=0;j<i;j++)
		lwrrentWBOffset += dataSizeGottenThisRoundPerProc[j];

	    /* only receive from source procs that will send > 0 count data */
	    if (dataSizeGottenThisRoundPerProc[i] > 0) {
#ifdef p2pcontigtrace
		printf("receiving data from rank %d dataSizeGottenThisRoundPerProc is %d lwrrentWBOffset is %d\n",sourceProcsForMyData[i],dataSizeGottenThisRoundPerProc[i],lwrrentWBOffset);
#endif
		MPI_Irecv(&((char*)write_buf)[lwrrentWBOffset],dataSizeGottenThisRoundPerProc[i],
	...
	if (numDataRecvToWaitFor > 0) {
		...
		ADIO_WriteContig(fd, write_buf, (int)totalDataReceivedThisRound,
			...
			lwrrentRoundFDStart, &status, error_code);
	...
	ADIO_Offset* fd_end)
{
    ...
    *error_code = MPI_SUCCESS;
	...
	for (i=0;i<numSourceAggs;i++) {
		...
		/* here are the offsets lwrrently being sent by the aggregator
		 * during this round */
		ADIO_Offset lwrrentRoundFDStartForMySourceAgg =
		    (ADIO_Offset)((ADIO_Offset)sourceAggsForMyDataFDStart[i] +
			    (ADIO_Offset)((ADIO_Offset)roundIter*(ADIO_Offset)coll_bufsize));
		ADIO_Offset lwrrentRoundFDEndForMySourceAgg =
		    (ADIO_Offset)((ADIO_Offset)sourceAggsForMyDataFDStart[i] +
			    (ADIO_Offset)((ADIO_Offset)(roundIter+1)*(ADIO_Offset)coll_bufsize) - (ADIO_Offset)1);
		if (lwrrentRoundFDEndForMySourceAgg > sourceAggsForMyDataFDEnd[i])
		    lwrrentRoundFDEndForMySourceAgg = sourceAggsForMyDataFDEnd[i];

#ifdef p2pcontigtrace
		printf("roundIter %d source iter %d sourceAggsForMyData is %d myOffsetStart is %ld myOffsetEnd is %ld sourceAggsForMyDataFDStart is %ld sourceAggsForMyDataFDEnd is %ld lwrrentRoundFDStartForMySourceAgg is %ld lwrrentRoundFDEndForMySourceAgg is %ld\n",roundIter,i,sourceAggsForMyData[i],myOffsetStart,myOffsetEnd,sourceAggsForMyDataFDStart[i],sourceAggsForMyDataFDEnd[i],lwrrentRoundFDStartForMySourceAgg,lwrrentRoundFDEndForMySourceAgg);
#endif

		/* get the portion of my data that is within lwrrentRoundFDStartForMySourceAgg to lwrrentRoundFDEndForMySourceAgg */
		/* find the offset into the recv buffer and the amount of data to get */
		int recvBufferOffset = 0;
		int bufferAmountToGet = 0;

		if ((myOffsetStart >= lwrrentRoundFDStartForMySourceAgg) && (myOffsetStart <= lwrrentRoundFDEndForMySourceAgg)) {
		    if (myOffsetEnd > lwrrentRoundFDEndForMySourceAgg)
			bufferAmountToGet = (lwrrentRoundFDEndForMySourceAgg - myOffsetStart) +1;
		    else
			bufferAmountToGet = (myOffsetEnd - myOffsetStart) +1;
		}
		else if ((myOffsetEnd >= lwrrentRoundFDStartForMySourceAgg) && (myOffsetEnd <= lwrrentRoundFDEndForMySourceAgg)) {
		    recvBufferOffset = (int) (lwrrentRoundFDStartForMySourceAgg - myOffsetStart);
		    if (myOffsetEnd > lwrrentRoundFDEndForMySourceAgg)
			bufferAmountToGet = (lwrrentRoundFDEndForMySourceAgg - lwrrentRoundFDStartForMySourceAgg) +1;
		    else
			bufferAmountToGet = (myOffsetEnd - lwrrentRoundFDStartForMySourceAgg) +1;
		}
		else if ((myOffsetStart <= lwrrentRoundFDStartForMySourceAgg) && (myOffsetEnd >= lwrrentRoundFDEndForMySourceAgg)) {
		    recvBufferOffset = (int) (lwrrentRoundFDStartForMySourceAgg - myOffsetStart);
		    bufferAmountToGet = (lwrrentRoundFDEndForMySourceAgg - lwrrentRoundFDStartForMySourceAgg) +1;
		}
	...
	for (i=0;i<numTargetProcs;i++) {
	    if ((remainingDataOffsetToSendPerProc[i] >= lwrrentRoundFDStart) &&
		    (remainingDataOffsetToSendPerProc[i] <= lwrrentRoundFDEnd)) {
		if ((remainingDataOffsetToSendPerProc[i] +
			    remainingDataAmountToSendPerProc[i]) <= lwrrentRoundFDEnd)
		    dataSizeSentThisRoundPerProc[i] = remainingDataAmountToSendPerProc[i];
		else
		    dataSizeSentThisRoundPerProc[i] =
			(lwrrentRoundFDEnd - remainingDataOffsetToSendPerProc[i]) +1;
	    }
		...
		if ((remainingDataOffsetToSendPerProc[i]) >= lwrrentRoundFDStart)
		    ...
		    dataSizeSentThisRoundPerProc[i] =
			(remainingDataOffsetToSendPerProc[i]-lwrrentRoundFDStart) +1;
	    }
	    else
