			       ...
			       int rw_type)
{
    ...
    int num_filetypes = 0, lwr_flat_file_reg_off = 0;
    ...
    *error_code = MPI_SUCCESS;  
     ...
     * colwert this to off in terms of actual data bytes (the offset
     * minus the number of bytes that are not used).  We are allowed
     * to do this since PVFS2 handles offsets with respect to a
     * file_req in bytes, otherwise we would have to colwert into a
     * pure byte offset as is done in other methods.  Explicit offset
     * case is handled by using fd->disp and byte-colwerted off. */
	...
	else 
	{
	    ...
	    off = bytes_into_filetype + lwr_flat_file_reg_off;
	}
    }
    else /* ADIO_EXPLICIT */
    { 
    ...
    /* Colwert the MPI memory and file datatypes into
     * PVFS2 datatypes */
    ret = colwert_mpi_pvfs2_dtype(&datatype, &tmp_mem_req);
    if (ret < 0)
    {
	goto error_state;
    }
    ret = colwert_mpi_pvfs2_dtype(&(fd->filetype), &tmp_file_req);
    if (ret < 0)
    {
	goto error_state;
    }
    ...
    if (ret != 0) /* TODO: colwert this to MPIO error handling */
...
/* colwert_mpi_pvfs2_dtype - Colwert a MPI datatype into
 * a PVFS2 datatype so that we can natively use the PVFS2 
 * datatypes in the PVFS2 I/O calls instead of colwerting 
 * all datatypes to the hindexed method 
 * return 1  - a leaf node
 * return 0  - normal return 
 * return -1 - problems */

int colwert_mpi_pvfs2_dtype(MPI_Datatype *mpi_dtype, 
			    PVFS_Request *pvfs_dtype)
{
    ...
    MPI_Type_get_elwelope(*mpi_dtype,
			  &num_int,
			  &num_addr,
			  &num_dtype,
			  &combiner);
    ...
    {
	colwert_named(mpi_dtype, pvfs_dtype, combiner);
	return 1;
    }
	...
	if ((old_pvfs_dtype = ADIOI_Malloc(sizeof(PVFS_Request))) == NULL)
	    fprintf(stderr, "colwert_mpi_pvfs2_dtype: "
		    "Failed to allocate PVFS_Request\n");
	switch (combiner)
	{
	    case MPI_COMBINER_CONTIGUOUS:
		leaf = colwert_mpi_pvfs2_dtype(&arr_dtype[0], old_pvfs_dtype);
		 ...
		 * colwert the data from MPI_get_contents() into 
		 * a PVFS_size buffer */
		...
		/* No native PVFS2 support for this operation lwrrently */
	...
	/* When colwerting into a PVFS_Request_struct, we no longer
    ...
    fprintf(stderr, "colwert_mpi_pvfs2_dtype:  SERIOUS ERROR\n");
    return -1;
}

int colwert_named(MPI_Datatype *mpi_dtype, 
...
{ 
    ...
    {
	    fprintf(stderr, "colwert_named: predefined type not found");
	    return -1;
    }
    if (ret != 0)
	fprintf(stderr, "colwert_named: Datatype creation failed\n");
    return ret;
}
