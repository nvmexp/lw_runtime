...
int ADIOI_PVFS2_Initialized = MPI_KEYVAL_ILWALID;
...
void ADIOI_PVFS2_Init(int *error_code )
{
    ...
    if (ADIOI_PVFS2_Initialized != MPI_KEYVAL_ILWALID) {
	*error_code = MPI_SUCCESS;
	return;
    }

    /* for consistency, we should disable the pvfs2 ncache.  If the
     * elwirontment variable is already set, assume a  user knows it
     * won't be a problem */
    ncache_timeout = getelw("PVFS2_NCACHE_TIMEOUT");
    if (ncache_timeout == NULL )
	setelw("PVFS2_NCACHE_TIMEOUT", "0", 1);
...
int ADIOI_PVFS2_error_colwert(int pvfs_error)
{
    switch(pvfs_error)
    {
	case PVFS_EPERM:
