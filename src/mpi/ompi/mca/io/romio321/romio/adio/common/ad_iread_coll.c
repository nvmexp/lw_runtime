                                             ...
                                             int *error_code)
{
    ...
    || (!vars->interleave_count && (fd->hints->cb_read == ADIOI_HINT_AUTO)))
    {
        ...
        /* NOTE: This is lwrrently a workaround to avoid weird errors, e.g.,
         * stack fault, oclwrred on Linux. When the host OS is Linux and
...
static void ADIOI_Iread_and_exch(ADIOI_NBC_Request *nbc_req, int *error_code)
{
    ...
    *error_code = MPI_Iallreduce(&vars->ntimes, &vars->max_ntimes, 1, MPI_INT,
    ...
    vars->lwrr_offlen_ptr = (int *)ADIOI_Calloc(nprocs, sizeof(int));
    ...
    vars->done = 0;
    vars->off = st_loc;
    vars->for_lwrr_iter = vars->for_next_iter = 0;
                                          ...
                                          int *error_code)
{
    ...
    char *read_buf;
    int *lwrr_offlen_ptr, *count, *send_size;
    ...
    lwrr_offlen_ptr = vars->lwrr_offlen_ptr;
    count = vars->count;
    ...
    real_off = vars->off - vars->for_lwrr_iter;
    real_size = size + vars->for_lwrr_iter;
...
#if defined(ROMIO_RUN_ON_LINUX) && !defined(HAVE_AIO_LITE_H)
        MPI_Status status;
        ADIO_ReadContig(fd, read_buf+vars->for_lwrr_iter, (int)size,
                        MPI_BYTE, ADIO_EXPLICIT_OFFSET, vars->off,
                        &status, error_code);
#else
        ADIO_IreadContig(fd, read_buf+vars->for_lwrr_iter, (int)size,
                         MPI_BYTE, ADIO_EXPLICIT_OFFSET, vars->off,
                                        ...
                                        int *error_code)
{
    ...
    vars->for_lwrr_iter = for_next_iter;
...
static void ADIOI_Iread_and_exch_fini(ADIOI_NBC_Request *nbc_req, int *error_code)
{
    ...
    ADIOI_Free(vars->lwrr_offlen_ptr);
