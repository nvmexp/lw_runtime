...
int ADIOI_cb_config_list_keyval = MPI_KEYVAL_ILWALID;
static char *yylval;
...
static int match_this_proc(char *name, int lwr_proc, int max_matches,
 ...
 *
 * Parameters:
 * fd - ADIO_File for which update is oclwrring.  cb_nodes and ranklist
...
int ADIOI_cb_config_list_parse(char *config_list, 
			 ...
			 int cb_nodes)
{
    int token, max_procs, lwr_rank = 0, nr_procnames;
    char *lwr_procname, *lwr_procname_p, **procnames;
    char *used_procnames;
     ...
     */
    lwr_procname = ADIOI_Malloc((MPI_MAX_INFO_VAL+1) * sizeof(char));
    if (lwr_procname == NULL) {
	return -1;
    }

    yylval = ADIOI_Malloc((MPI_MAX_INFO_VAL+1) * sizeof(char));
    if (yylval == NULL) {
	ADIOI_Free(lwr_procname);
	return -1;
    }
	...
	for (lwr_rank = 0; lwr_rank < cb_nodes; lwr_rank++) {
	    ranklist[lwr_rank] = lwr_rank;
	}
	ADIOI_Free(lwr_procname);
	ADIOI_Free(yylval);
	yylval = NULL;
	...
	if (token == AGG_WILDCARD) {
	    ...
	    ADIOI_Strncpy(lwr_procname, yylval, MPI_MAX_INFO_VAL+1);
	    lwr_procname_p = lwr_procname;
	}
	...
	if (token == AGG_WILDCARD) {
	    ...
	    FPRINTF(stderr, "looking for %s:%d\n", lwr_procname, max_procs);
	}
#endif

	/* do the matching for this piece of the cb_config_list */
	match_procs(lwr_procname_p, max_procs, procnames, used_procnames,
		    nr_procnames, ranklist, cb_nodes, &lwr_rank);
    }
    ADIOI_Free(lwr_procname);
    ADIOI_Free(yylval);
    yylval = NULL;
...
static int match_procs(char *name, 
		       ...
		       int *nr_ranks_allocated)
{
    int wildcard_proc, lwr_proc, old_nr_allocated, ret;
	...
	if (max_per_proc == 0) {
	    ...
	    for (lwr_proc = 0; lwr_proc < nr_procnames; lwr_proc++) {
		used_procnames[lwr_proc] = 1;
	    }
	    return 0;
	}
 ...
 *                     the "procnames" array, starting with index "lwr_proc"
 ...
 * lwr_proc - index into procnames[] at which to start matching
 * procnames - array of processor names
