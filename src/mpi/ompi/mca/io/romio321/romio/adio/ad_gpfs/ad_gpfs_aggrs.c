...
/* Dislwssion of values available from above:
 *
 ...
 * file domains, and we use the same approach here when callwlating the
 * location of an offset/len in a specific file domain.  Further we assume
 * this same distribution when callwlating the rank_index, which is later
    ...
    *fd_end_ptr          = (ADIO_Offset *) ADIOI_Malloc(nprocs_for_coll * sizeof(ADIO_Offset));
	...
	for (j=0;j<naggs_large;j++) {
		...
		if (tmpbridgelistnum[bridgeiter] > 0) {
		     ...
		     */
		    int lwrrentbridgelistnum =
			(fd->hints->fs_hints.bg.bridgelistnum[bridgeiter]-
			 tmpbridgelistnum[bridgeiter]);
		    int lwrrentfdsizeindex = bridgelistoffset[bridgeiter] +
			lwrrentbridgelistnum;
		    fd_size[lwrrentfdsizeindex] = (nb_cn_small+1) * blksize;
		    tmpbridgelistnum[bridgeiter]--;
		}
 ...
 *
 * ADIOI_Calc_my_req() - callwlate what portions of the access requests
   ...
   They are used as memory buffer indices so it seems like the 2G limit is in effect */
{
    ...
    int i, l, proc;
    ADIO_Offset fd_len, rem_len, lwrr_idx, off;
    ADIOI_Access *my_req;
...
/* buf_idx is relevant only if buftype_is_contig.
   ...
   without extra buffer. This can't be done if buftype is not contig. */
    ...
    /* one pass just to callwlate how much space to allocate for my_req;
     * contig_access_count was callwlated way back in ADIOI_Calc_my_off_len()
     */
...
/* now fill in my_req */
	...
	if (buf_idx[proc] == -1)
  {
    ADIOI_Assert(lwrr_idx == (int) lwrr_idx);
    buf_idx[proc] = (int) lwrr_idx;
  }
...
/*     total_cora2a+=timebase()-cora2a1; */
	...
	if (count_others_req_per_proc[i])
  {
    ...
    /* Now send the callwlated offsets and lengths to respective processes */
