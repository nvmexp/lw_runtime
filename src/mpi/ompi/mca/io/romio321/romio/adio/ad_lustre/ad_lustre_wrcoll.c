				   ...
				   ADIO_Status *status, int *error_code)
{
    ...
    /* based on everyone's my_req, callwlate what requests of other processes
                                        ...
                                        int *error_code)
{
    ...
    int *recv_lwrr_offlen_ptr, *recv_count, *recv_size;
    int *send_lwrr_offlen_ptr, *send_size;
    ...
    *error_code = MPI_SUCCESS;	/* changed below if error */
    ...
    /* callwlate the number of writes of stripe size to be done.
...
/*     max_ntimes = (int)((max_end_loc - min_st_loc) / step_size + 1); */
    ...
    /* callwlate the start offset for each iteration */
    ...
    recv_lwrr_offlen_ptr = (int *) ADIOI_Calloc(nprocs, sizeof(int));
    send_lwrr_offlen_ptr = (int *) ADIOI_Calloc(nprocs, sizeof(int));
    ...
    /* used to store the starting value of recv_lwrr_offlen_ptr[i] in
       this iteration */
    ...
    */
	...
	for (i = 0; i < nprocs; i++) {
            if (my_req[i].count) {
                this_buf_idx[i] = buf_idx[i][send_lwrr_offlen_ptr[i]];
                for (j = send_lwrr_offlen_ptr[i]; j < my_req[i].count; j++) {
                ...
                send_lwrr_offlen_ptr[i] = j;
            }
	    if (others_req[i].count) {
		recv_start_pos[i] = recv_lwrr_offlen_ptr[i];
		for (j = recv_lwrr_offlen_ptr[i]; j < others_req[i].count; j++) {
                    req_off = others_req[i].offsets[j];
...
over:
    ...
    ADIOI_Free(recv_lwrr_offlen_ptr);
    ADIOI_Free(send_lwrr_offlen_ptr);
...
static void ADIOI_LUSTRE_W_Exchange_data(ADIO_File fd, const void *buf,
					 ...
					 ADIOI_Access *others_req,
					 int *send_buf_idx,
					 int *lwrr_to_proc, int *done_to_proc,
                                         int *hole, int iter,
                                         MPI_Aint buftype_extent,
					 int *buf_idx,
                          ADIO_Offset **srt_off, int **srt_len, int *srt_num,
                          int *error_code)
{
    ...
    *hole = 0;
	...
	ADIOI_LUSTRE_Fill_send_buffer(fd, buf, flat_buf, send_buf, offset_list,
                                      ...
                                      sent_to_proc, nprocs, myrank,
                                      contig_access_count, striping_info,
                                      send_buf_idx, lwrr_to_proc, done_to_proc,
                                      iter, buftype_extent);
					  ...
					  MPI_Aint buftype_extent)
{
	...
	while (rem_len != 0) {
		...
		if (lwrr_to_proc[p] + len > done_to_proc[p]) {
			...
			size = (int) ADIOI_MIN(lwrr_to_proc[p] + len -
					       done_to_proc[p],
					       send_size[p] -
					       send_buf_idx[p]);
			...
			ADIOI_BUF_INCR
			    ...
			    buf_incr = (int) (lwrr_to_proc[p] + len -
					      done_to_proc[p]);
