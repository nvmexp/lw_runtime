...
/*   From common code - not implemented for bg. */
    ...
    if ((gpfsmpio_read_aggmethod == 1) || (gpfsmpio_read_aggmethod == 2)) {
      ...
      for (i=0; i<nprocs; i++) {
        if (count_sizes[i] > 0) {
          st_offsets[lwrrentNonZeroDataIndex] = st_offsets[i];
          end_offsets[lwrrentNonZeroDataIndex] = end_offsets[i];
          lwrrentNonZeroDataIndex++;
        }
      }
    }
    if (gpfsmpio_tuneblocking) {
    if ((gpfsmpio_read_aggmethod == 1) || (gpfsmpio_read_aggmethod == 2)) {
    ADIOI_GPFS_Calc_file_domains(fd, st_offsets, end_offsets, lwrrentNonZeroDataIndex,
    ...
    if ((gpfsmpio_read_aggmethod == 1) || (gpfsmpio_read_aggmethod == 2)) {
    ADIOI_Calc_file_domains(st_offsets, end_offsets, lwrrentNonZeroDataIndex,
			    nprocs_for_coll, &min_st_offset,
			    &fd_start, &fd_end,
     ...
     */
      ADIOI_OneSidedReadAggregation(fd, offset_list, len_list, contig_access_count, buf,
		    datatype,error_code, st_offsets, end_offsets, lwrrentNonZeroDataIndex, fd_start, fd_end);
    ...
    /* callwlate where the portions of the access requests of this process 
     ...
     * data callwlated above.  fills in the following:
...
/* callwlate the number of reads of size coll_bufsize
   ...
   coll_bufsize is obtained from the hints object. */
    ...
    /* callwlate ntimes, the number of times this process must perform I/O
    ...
    for (m=0; m<ntimes; m++) {
	  ...
	  (lwrr_offlen_ptr) to 
          ...
          where ! indicates where the current read_size limitation lwts 
	...
	real_off = off - for_lwrr_iter;
	real_size = size + for_lwrr_iter;
	...
	for (i=0; i<nprocs; i++) {
		...
		for (j=lwrr_offlen_ptr[i]; j<others_req[i].count;
		     j++) {
		    if (partial_send[i]) {
	...
	if (flag) {
      ...
      ADIOI_Assert(size == (int)size);
	    ADIO_ReadContig(fd, read_buf+for_lwrr_iter, (int)size, MPI_BYTE,
			    ADIO_EXPLICIT_OFFSET, off, &status, error_code);
#ifdef RDCOLL_DEBUG
	    DBG_FPRINTF(stderr, "\tread_coll: 700, data read [%lld] = ", size );
	    for (iii=0; iii<size && iii<80; iii++) { DBGV_FPRINTF(stderr, "%3d,", *((unsigned char *)read_buf + for_lwrr_iter + iii) ); }
	    DBG_FPRINTF(stderr, "\n" );
#endif
...
/* nothing to send, but check for recv. */
	...
	ADIOI_R_Exchange_data(fd, buf, flat_buf, offset_list, len_list,
        ...
        else    /* strncmp( elw_switch, "alltoall", 8 ) == 0 */
	if (gpfsmpio_comm == 0)
        ADIOI_R_Exchange_data_alltoallv(fd, buf, flat_buf, offset_list, len_list,
                            send_size, recv_size, count, 
                            start_pos, partial_send, recd_from_proc, nprocs,
...
/* this function is only called if buftype is not contig */
    ...
    unsigned *lwrr_from_proc, *done_from_proc, *recv_buf_idx;

    ADIOI_UNREFERENCED_ARG(requests);
    ADIOI_UNREFERENCED_ARG(statuses);

/*  lwrr_from_proc[p] = amount of data recd from proc. p that has already
	...
	while (rem_len > 0) {
		...
		if (lwrr_from_proc[p]+len > done_from_proc[p]) {
		    if (done_from_proc[p] > lwrr_from_proc[p]) {
			size = ADIOI_MIN(lwrr_from_proc[p] + len - 
			      done_from_proc[p], recv_size[p]-recv_buf_idx[p]);
			buf_incr = done_from_proc[p] - lwrr_from_proc[p];
			ADIOI_BUF_INCR
			buf_incr = lwrr_from_proc[p]+len-done_from_proc[p];
      ADIOI_Assert((done_from_proc[p] + size) == (unsigned)((ADIO_Offset)done_from_proc[p] + size));
			lwrr_from_proc[p] = done_from_proc[p] + size;
			ADIOI_BUF_COPY
		    }
		    else {
			size = ADIOI_MIN(len,recv_size[p]-recv_buf_idx[p]);
			buf_incr = len;
      ADIOI_Assert((lwrr_from_proc[p] + size) == (unsigned)((ADIO_Offset)lwrr_from_proc[p] + size));
			lwrr_from_proc[p] += (unsigned) size;
			ADIOI_BUF_COPY
		    }
		}
		else {
        ADIOI_Assert((lwrr_from_proc[p] + len) == (unsigned)((ADIO_Offset)lwrr_from_proc[p] + len));
		    lwrr_from_proc[p] += (unsigned) len;
		    buf_incr = len;
		    ADIOI_BUF_INCR
		}
	    }
    ...
    for (i=0; i < nprocs; i++) 
	if (recv_size[i]) recd_from_proc[i] = lwrr_from_proc[i];

    ADIOI_Free(lwrr_from_proc);
