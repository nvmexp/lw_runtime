...
static void ADIOI_W_Exchange_data(ADIO_File fd, const void *buf, char *write_buf,
                         ...
                         *offset_list, ADIO_Offset *len_list, int *send_size, 
                         ...
                         int *send_buf_idx, int *lwrr_to_proc,
...
static void ADIOI_W_Exchange_data_alltoallv(
		...
		ADIO_Offset *fd_end,
		ADIOI_Access *others_req,
		int *send_buf_idx, int *lwrr_to_proc,			/* 3 */
 ...
 */
	...
	/* For this process's request, callwlate the list of offsets and
...
/* Divide the I/O workload among "nprocs_for_coll" processes. This is
   ...
   process may directly access only its own file domain. */
    ...
    if ((gpfsmpio_write_aggmethod == 1) || (gpfsmpio_write_aggmethod == 2)) {
      ...
      for (i=0; i<nprocs; i++) {
        if (count_sizes[i] > 0) {
          st_offsets[lwrrentValidDataIndex] = st_offsets[i];
          end_offsets[lwrrentValidDataIndex] = end_offsets[i];
          lwrrentValidDataIndex++;
        }
      }
    }
	...
	if ((gpfsmpio_write_aggmethod == 1) || (gpfsmpio_write_aggmethod == 2)) {
	    ADIOI_Calc_file_domains(st_offsets, end_offsets, lwrrentValidDataIndex,
		    nprocs_for_coll, &min_st_offset,
		    &fd_start, &fd_end,
     ...
     */
      ...
      ADIOI_OneSidedWriteAggregation(fd, offset_list, len_list, contig_access_count,
	      ...
	      lwrrentValidDataIndex, fd_start, fd_end, &holeFound);
      int anyHolesFound = 0;
	 ...
	 * and behavior.
        ...
        if (gpfsmpio_onesided_inform_rmw && (myrank ==0))
          ...
          ADIOI_OneSidedWriteAggregation(fd, offset_list, len_list, contig_access_count, buf, datatype, error_code, st_offsets, end_offsets, lwrrentValidDataIndex, fd_start, fd_end, &holeFound);
...
/* callwlate what portions of the access requests of this process are
...
/* based on everyone's my_req, callwlate what requests of other
...
static void gpfs_wr_access_start(int fd, ADIO_Offset offset, ADIO_Offset length)
{
        ...
        take_locks.header.fcntlVersion = GPFS_FCNTL_LWRRENT_VERSION;
...
static void gpfs_wr_access_end(int fd, ADIO_Offset offset, ADIO_Offset length)
{
        ...
        free_locks.header.fcntlVersion = GPFS_FCNTL_LWRRENT_VERSION;
...
/* Send data to appropriate processes and write in sizes of no more
   ...
   at least another 8Mbytes of temp space is unacceptable. */
    ...
    /* Not colwinced end_loc-st_loc couldn't be > int, so make these offsets*/
    ADIO_Offset size=0;
    ...
    int *lwrr_offlen_ptr, *count, *send_size, req_len, *recv_size;
    int *partial_recv, *sent_to_proc, *start_pos, flag;
    int *send_buf_idx, *lwrr_to_proc, *done_to_proc;
    MPI_Status status;
    ...
    *error_code = MPI_SUCCESS;  /* changed below if error */
    /* only I/O errors are lwrrently reported */

/* callwlate the number of writes of size coll_bufsize
...
/* ntimes=ceiling_div(end_loc - st_loc + 1, coll_bufsize)*/
    ...
    if (ntimes > 0) { /* only set the gpfs hint if we have io - ie this rank is
      ...
      if (getelw("ROMIO_GPFS_DECLARE_ACCESS")!=NULL) {
    ...
    if (ntimes > 0) { /* only set the gpfs hint if we have io - ie this rank is
      ...
      if (getelw("ROMIO_GPFS_DECLARE_ION_ACCESS")!=NULL) {
    ...
    lwrr_offlen_ptr = (int *) ADIOI_Calloc(nprocs, sizeof(int)); 
    ...
    lwrr_to_proc = (int *) ADIOI_Malloc(nprocs*sizeof(int));
    ...
    /* used to store the starting value of lwrr_offlen_ptr[i] in 
       this iteration */
...
/* I need to check if there are any outstanding nonblocking writes to
    ...
    */
	...
	/* first callwlate what should be communicated */
	...
	for (i=0; i < nprocs; i++) {
	    if (others_req[i].count) {
		start_pos[i] = lwrr_offlen_ptr[i];
		for (j=lwrr_offlen_ptr[i]; j<others_req[i].count; j++) {
		    if (partial_recv[i]) {
	...
	ADIOI_W_Exchange_data(fd, buf, write_buf, flat_buf, offset_list,
			    ...
			    others_req, send_buf_idx, lwrr_to_proc,
	...
	if (flag) {
	    ...
	    setelw("LIBIOLOG_EXTRA_INFO", round, 1);
	...
	ADIOI_W_Exchange_data(fd, buf, write_buf, flat_buf, offset_list,
			    ...
			    others_req, send_buf_idx, 
                            lwrr_to_proc, done_to_proc, &hole, m, 
    ...
    ADIOI_Free(lwrr_to_proc);
    ADIOI_Free(done_to_proc);

    if (ntimes != 0 && getelw("ROMIO_GPFS_DECLARE_ACCESS")!=NULL) {
...
/* for partial recvs, restore original lengths */
    ...
    *hole = 0;
	...
	if (*hole) {
	    ...
	    setelw("LIBIOLOG_EXTRA_INFO", stuff, 1);
   ...
   user buf at location given by buf_idx. else use send_buf. */
	...
	ADIOI_Fill_send_buffer(fd, buf, flat_buf, send_buf,
			   ...
			   send_req,
                           ...
                           contig_access_count,
                           min_st_offset, fd_size, fd_start, fd_end, 
                           send_buf_idx, lwrr_to_proc, done_to_proc, iter,
                           buftype_extent);
...
/*  lwrr_to_proc[p] = amount of data sent to proc. p that has already
    ...
    for (i=0; i < nprocs; i++) {
	send_buf_idx[i] = lwrr_to_proc[i] = 0;
  ...
  while (rem_len != 0) {
		...
		if (lwrr_to_proc[p]+len > done_to_proc[p]) {
		    if (done_to_proc[p] > lwrr_to_proc[p]) {
			size = ADIOI_MIN(lwrr_to_proc[p] + len - 
                                done_to_proc[p], send_size[p]-send_buf_idx[p]);
			buf_incr = done_to_proc[p] - lwrr_to_proc[p];
			ADIOI_BUF_INCR
      ADIOI_Assert((lwrr_to_proc[p] + len - done_to_proc[p]) == (unsigned)(lwrr_to_proc[p] + len - done_to_proc[p]));
		        buf_incr = lwrr_to_proc[p] + len - done_to_proc[p];
      ADIOI_Assert((done_to_proc[p] + size) == (unsigned)(done_to_proc[p] + size));
			lwrr_to_proc[p] = done_to_proc[p] + size;
		        ADIOI_BUF_COPY
		    }
		    else {
			size = ADIOI_MIN(len,send_size[p]-send_buf_idx[p]);
			buf_incr = len;
      ADIOI_Assert((lwrr_to_proc[p] + size) == (unsigned)((ADIO_Offset)lwrr_to_proc[p] + size));
			lwrr_to_proc[p] += size;
			ADIOI_BUF_COPY
		    }
		    if (send_buf_idx[p] == send_size[p]) {
        ...
        ADIOI_Assert((lwrr_to_proc[p] + len) == (unsigned)((ADIO_Offset)lwrr_to_proc[p] + len));
		    lwrr_to_proc[p] += len;
		    buf_incr = len;
		    ADIOI_BUF_INCR
		}
    ...
    for (i=0; i < nprocs; i++) 
	if (send_size[i]) sent_to_proc[i] = lwrr_to_proc[i];
}
		      ...
		      int nprocs, int nprocs_recv, int total_elements)
{
	...
	/* Heapify(a, i, heapsize); Algorithm from Cormen et al. pg. 143
           ...
           removed the relwrsion so that there are no function calls.
