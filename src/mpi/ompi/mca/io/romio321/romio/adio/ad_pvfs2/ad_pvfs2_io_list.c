			      ...
			      int *error_code, int rw_type)
{
    ...
    int64_t lwr_io_size = 0, io_size = 0;
    int etype_size = -1;
    ...
    int64_t lwr_flat_buf_reg_off = 0;
    int64_t lwr_flat_file_reg_off = 0;
	...
	while (!flag)
	{
		...
		if (flat_file_p->blocklens[i])
		{
		    ...
		    {
			flat_file_index = i;
			lwr_flat_file_reg_off = fd->fp_ind -
			    (fd->disp + flat_file_p->indices[i] +
			     (num_filetypes * filetype_extent));
			flag = 1;
			break;
	...
	for (i=0; i<flat_file_p->count; i++) {
	    ...
	    {
		flat_file_index = i;
		lwr_flat_file_reg_off = flat_file_p->blocklens[i] - 
    ...
    fprintf(stderr, "ADIOI_PVFS2_StridedListIO: (fd->fp_ind=%Ld,fd->disp=%Ld,"
            " offset=%Ld)\n(flat_file_index=%d,lwr_flat_file_reg_off=%Ld,"
	    "bytes_into_filetype=%d)\n",
            fd->fp_ind, fd->disp, offset, flat_file_index, 
	    lwr_flat_file_reg_off, bytes_into_filetype);
#endif
#ifdef DEBUG_LIST2
	...
	gen_listio_arr(flat_buf_p,
		       &flat_buf_index,
		       &lwr_flat_buf_reg_off,
		       buftype_size,
		       buftype_extent,
		       flat_file_p,
		       &flat_file_index,
		       &lwr_flat_file_reg_off,
		       filetype_size,
		       filetype_extent,
		       MAX_OL_COUNT,
		       fd->disp,
...
int gen_listio_arr(ADIOI_Flatlist_node *flat_buf_p,
		   ...
		   int64_t *lwr_flat_buf_reg_off_p,
		   int flat_buf_size,
		   int flat_buf_extent,
		   ADIOI_Flatlist_node *flat_file_p,
		   int *flat_file_index_p,
		   int64_t *lwr_flat_file_reg_off_p,
		   int flat_file_size,
		   ...
		   int32_t *file_ol_count_p)
{
    ...
    /* parameters for flattened memory and file datatypes*/
    int64_t lwr_flat_buf_reg_left = 0;
    int64_t lwr_flat_file_reg_left = 0;
	...
	*lwr_flat_file_reg_off_p;
	...
	lwr_flat_buf_reg_left = flat_buf_p->blocklens[*flat_buf_index_p]
	    - *lwr_flat_buf_reg_off_p;
	lwr_flat_file_reg_left = flat_file_p->blocklens[*flat_file_index_p]
	    - *lwr_flat_file_reg_off_p;

#ifdef DEBUG_LIST2
	fprintf(stderr, 
		"flat_buf_index=%d flat_buf->blocklens[%d]=%d\n"
		"lwr_flat_buf_reg_left=%Ld "
		"*lwr_flat_buf_reg_off_p=%Ld\n" 
		"flat_file_index=%d flat_file->blocklens[%d]=%d\n"
		"lwr_flat_file_reg_left=%Ld "
		"*lwr_flat_file_reg_off_p=%Ld\n" 
		"bytes_completed=%Ld\n"
	...
	if (lwr_flat_buf_reg_left > lwr_flat_file_reg_left)
	    region_size = lwr_flat_file_reg_left;
	else
	    region_size = lwr_flat_buf_reg_left;
	...
	if (lwr_flat_buf_reg_left == region_size)
	{
#ifdef DEBUG_LIST2
	...
	else if (lwr_flat_buf_reg_left > region_size)
	{
#ifdef DEBUG_LIST2
	    fprintf(stderr, "advanced %d in memory block...\n",
		    region_size);
#endif
	    (*lwr_flat_buf_reg_off_p) += region_size;
	    buf_len_arr[*buf_ol_count_p] += region_size;
	}
	else
	{
	...
	/* To callwlate the absolute file offset we need to 
	...
	if (lwr_flat_file_reg_left == region_size)
	{
#ifdef DEBUG_LIST2
	...
	else if (lwr_flat_file_reg_left > region_size)
	{
#ifdef DEBUG_LIST2
	    fprintf(stderr, "advanced %d in file block...\n",
		    region_size);
#endif
	    (*lwr_flat_file_reg_off_p) += region_size;
	    file_len_arr[*file_ol_count_p] += region_size;
	}
	else
	{
	...
	*bytes_completed += region_size;
    ...
    if (*lwr_flat_buf_reg_off_p != 0)
	(*buf_ol_count_p)++;
    if (*lwr_flat_file_reg_off_p != 0)
	(*file_ol_count_p)++;
