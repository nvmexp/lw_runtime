...
int ADIOI_ZOIDFS_Initialized = MPI_KEYVAL_ILWALID;
...
void ADIOI_ZOIDFS_End(int *error_code)
{
	...
	*error_code = MPIO_Err_create_code(MPI_SUCCESS,
					   ...
					   myname, __LINE__,
					   ADIOI_ZOIDFS_error_colwert(ret),
...
void ADIOI_ZOIDFS_Init(int rank, int *error_code )
{
    ...
    if (ADIOI_ZOIDFS_Initialized != MPI_KEYVAL_ILWALID) {
	*error_code = MPI_SUCCESS;
	return;
    }
...
int ADIOI_ZOIDFS_error_colwert(int error)
{
    switch (error)
    {
	case ZFSERR_PERM: /* ??? */
