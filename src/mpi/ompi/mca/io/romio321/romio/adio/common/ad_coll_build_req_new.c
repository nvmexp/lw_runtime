...
static inline ADIO_Offset view_state_get_lwr_sz(view_state *tmp_view_state_p,
                                                int op_type)
{
    ...
    {   
        case TEMP_OFF:
            ...
            break;
        case REAL_OFF:
            tmp_state_p = &(tmp_view_state_p->lwr_state);
            break;
        default:
            fprintf(stderr, "op_type invalid\n");
    }
    return tmp_state_p->lwr_sz;
}
 ...
 * index and lwr_reg_off point to.  The regions should be coalesced if
 * possible later on. */
    ...
    *st_reg_p = tmp_state_p->abs_off;
	...
	if (tmp_flat_type_p->count == 1)
	{
	    ...
	    fprintf(stderr, "view_state_add_region: %s contig type "
		    "(old abs_off=%Ld,abs_off=%Ld,lwr_sz=%Ld,reg size=%Ld)\n", 
		    off_type_name[op_type], tmp_state_p->abs_off - data_sz, 
		    tmp_state_p->abs_off, tmp_state_p->lwr_sz, data_sz);
#endif
	}
	else
	{ 
	    ...
	    fprintf(stderr, "view_state_add_region: %s inner region type "
		    ...
		    (tmp_flat_type_p->indices[tmp_state_p->idx + 1] -
                    (tmp_flat_type_p->indices[tmp_state_p->idx] +
                     tmp_state_p->lwr_reg_off)), tmp_state_p->abs_off, 
		    tmp_state_p->lwr_sz, data_sz);
#endif
	    }
	...
	tmp_state_p->lwr_reg_off = 0;
    }
    else /* Add part of the piece */
    {
	data_sz = max_sz;
	tmp_state_p->lwr_reg_off += data_sz;
	tmp_state_p->abs_off += data_sz;
	tmp_state_p->lwr_sz += data_sz;
#ifdef DEBUG3 
	    fprintf(stderr, "view_state_add_region: %s partial region type "
		    "(lwr_reg_off=%Ld,abs_off=%Ld,lwr_sz=%Ld,reg size=%Ld\n", 
		    off_type_name[op_type], tmp_state_p->lwr_reg_off, 
		    tmp_state_p->abs_off, tmp_state_p->lwr_sz, data_sz);
#endif
    }
...
/* Set up the abs_off, idx, and lwr_reg_off of a view_state for the
 * tmp_state or the lwr_state. */
		    ...
		    int op_type)
{
	...
	switch(op_type)
	{
		...
		break;
	    case REAL_OFF:
		tmp_state_p = &(view_state_arr[i].lwr_state);
		break;
	    default:
		fprintf(stderr, "op_type invalid\n");
	}
	...
	/* Re-initialize the lwr_size so that the abs_off was set to
	 * the proper position while the actual size = 0.*/
	tmp_state_p->lwr_sz = 0;
#ifdef DEBUG1
	fprintf(stderr, "init_view_state: %s (idx=%d,byte_off=%Ld,"
		...
		tmp_state_p->lwr_reg_off, tmp_view_p->sz);
#endif	
				  ...
				  ADIO_Offset *fr_max_len_p) 
{
	...
	*fr_max_len_p = fr_node_p->blocklens[0];
    ...
    /* Callwlate how many times to loop through the fr_type 
...
static inline int find_next_off(ADIO_File fd,
				...
				MPI_Datatype *fr_type_p,
				int op_type,
				ADIO_Offset *lwr_off_p,
				ADIO_Offset *lwr_reg_max_len_p)
{
    ADIOI_Flatlist_node *tmp_flat_type_p = NULL;
	...
	case TEMP_OFF:
	    ...
	    break;
	case REAL_OFF:
	    tmp_state_p = &(view_state_p->lwr_state);
	    break;
	default:
	    fprintf(stderr, "op_type invalid\n");
    }
    ...
    if (tmp_state_p->lwr_sz < view_state_p->sz) {
	tmp_st_off = 0;
	tmp_reg_sz = 0;
	/* If the current region is not within the file realm, advance
	 * the state until it is and callwlate the end of the next file 
	 * realm in fr_max_len. */
	...
	while ((tmp_state_p->abs_off < fr_next_off) &&
	       (tmp_state_p->lwr_sz != view_state_p->sz))
	{
	 ...
	 * first piece. */
		...
		if (skip_type_ct > 0) {
		     ...
		     * finished up already */
		    tmp_state_p->lwr_sz += skip_type_ct *
	...
	if (tmp_state_p->lwr_sz != view_state_p->sz) {
	    ...
	    /* Callwlate how much of the remaining file realm there is from the
	     * current offset */
    ...
    *lwr_off_p = tmp_off;
    *lwr_reg_max_len_p = tmp_fr_max_len;
#ifdef AGGREGATION_PROFILE
			 ...
			 MPI_Datatype *agg_dtype_p)
{
    ...
    ADIO_Offset lwr_off = -1, lwr_reg_max_len = -1;
    ...
    int *client_ol_lwr_ct_arr = NULL;
    int agg_ol_ct = 0, agg_ol_lwr_ct = 0;
    int lwr_off_proc = -1;
    int next_off_idx = -1;
    ...
    if ((client_ol_lwr_ct_arr = 
	 (int *) ADIOI_Calloc(nprocs, sizeof(int))) == NULL)
    {
	fprintf(stderr, "ADIOI_Build_agg_reqs: "
		"malloc client_ol_lwr_ct_arr failed\n");
	return -1;
    }
	...
	for (j=0; j<nprocs; j++) {
	    find_next_off(fd, 
			  ...
			  &lwr_off,
			  &lwr_reg_max_len);
	    if ((lwr_off != -1) && (lwr_reg_max_len > 0)) {
		ADIOI_Heap_insert(&offset_heap, lwr_off, j, lwr_reg_max_len);
#ifdef DEBUG_HEAP
		printf ("initial: inserting offset %lld with "
			"lwr_reg_max_len = %lld for p%d\n",
			lwr_off, lwr_reg_max_len, j);
#endif
	    }
	...
	while (tmp_coll_buf_sz < fd->hints->cb_buffer_size)
	{
	    ...
	    ADIOI_Heap_extract_min(&offset_heap, &lwr_off, &lwr_off_proc,
			     &lwr_reg_max_len);
#ifdef DEBUG_HEAP
	    printf ("extracted lwr_off %lld from proc %d\n",
		    lwr_off, lwr_off_proc);
#endif
	    ...
	    fprintf(stderr, "ADIOI_Build_agg_reqs: %s proc %d start/add to"
		    ...
		    "cb_buffer_size=%d)\n", off_type_name[i], lwr_off_proc,
		    lwr_reg_max_len, tmp_coll_buf_sz, 
		...
		if (ds_fr_end == -1)
		{
		    ds_fr_end = lwr_off + lwr_reg_max_len;
#ifdef DEBUG1
		fprintf(stderr, "ADIOI_Build_agg_reqs: "
			"lwr_off=%Ld, lwr_reg_max_len=%Ld\n"
		...
		else
		{
		     ...
		     * here. */
		    if (ds_fr_end != lwr_off + lwr_reg_max_len)
		    {
#ifdef DEBUG1
			fprintf(stderr, "ADIOI_Build_agg_reqs: "
				"Data sieving file realm end changed from "
				"%Ld to %Ld\n", ds_fr_end, 
				lwr_off + lwr_reg_max_len);
#endif
			break;
		    }
		}
	     ...
	     * buffer. */
	    if (lwr_reg_max_len > (fd->hints->cb_buffer_size - 
				   tmp_coll_buf_sz))
		lwr_reg_max_len = fd->hints->cb_buffer_size - tmp_coll_buf_sz;

	    view_state_add_region(
		lwr_reg_max_len,
		&(client_file_view_state_arr[lwr_off_proc]), 
		&st_reg, &act_reg_sz, i);
		...
		case TEMP_OFF:
		     ...
		     * region. */
		    if (client_comm_next_off_arr[lwr_off_proc] != 
			tmp_coll_buf_sz)
		    {
			(client_ol_ct_arr[lwr_off_proc])++;
		    }
		    client_comm_next_off_arr[lwr_off_proc] = 
		...
		case REAL_OFF:
		     ...
		     * region. */
		    next_off_idx = client_ol_lwr_ct_arr[lwr_off_proc];
		    if (client_comm_next_off_arr[lwr_off_proc] != 
			tmp_coll_buf_sz)
		    {
			client_disp_arr[lwr_off_proc][next_off_idx] =
			    tmp_coll_buf_sz;
			client_blk_arr[lwr_off_proc][next_off_idx] = 
			    act_reg_sz;
			(client_ol_lwr_ct_arr[lwr_off_proc])++;
		    }
		    else
		    {
			client_blk_arr[lwr_off_proc][next_off_idx - 1] 
			    += act_reg_sz;
		    }
		    client_comm_sz_arr[lwr_off_proc] += act_reg_sz;
		    client_comm_next_off_arr[lwr_off_proc] =
			tmp_coll_buf_sz + act_reg_sz;
			...
			agg_disp_arr[agg_ol_lwr_ct] = st_reg -
			    (MPI_Aint) *agg_dtype_offset_p;
			agg_blk_arr[agg_ol_lwr_ct] = act_reg_sz;	
			agg_ol_lwr_ct++;
		    }
		    else
		    {
			agg_blk_arr[agg_ol_lwr_ct - 1] += act_reg_sz;
		    }
		    agg_next_off = st_reg + act_reg_sz;
		...
		ADIOI_Heap_insert(&offset_heap, next_off, lwr_off_proc,
			    next_reg_max_len);
#ifdef DEBUG_HEAP
    ...
    for (i = 0; i < nprocs; i++)
    {
	if ((client_file_view_state_arr[i].lwr_state.lwr_sz !=
    ...
    assert(agg_ol_lwr_ct == agg_ol_ct);
    ...
    ADIOI_Free(client_ol_lwr_ct_arr);
			    ...
			    MPI_Datatype *agg_comm_dtype_arr)
{
    ...
    ADIO_Offset total_agg_comm_sz = 0, lwr_total_agg_comm_sz = 0;
    ADIO_Offset st_reg = 0, act_reg_sz = 0, tmp_reg_sz = 0;
    ADIO_Offset lwr_off = -1, lwr_reg_max_len = -1;
    ADIO_Offset tmp_lwr_off = -1, tmp_lwr_reg_max_len = -1;
    ADIO_Offset agg_mem_st_reg = 0, agg_mem_act_reg_sz = 0;
    ADIO_Offset *fr_st_off_arr = fd->file_realm_st_offs;
    ADIO_Offset *agg_comm_lwr_sz_arr = NULL;
    MPI_Datatype *fr_type_arr = fd->file_realm_types;
    int *agg_ol_ct_arr = NULL;
    int *agg_ol_lwr_ct_arr = NULL;
    ...
    if ((agg_comm_lwr_sz_arr = (ADIO_Offset *) 
	 ADIOI_Malloc(nprocs*sizeof(ADIO_Offset))) == NULL)
    {
	fprintf(stderr, "ADIOI_Build_client_reqs: malloc agg_comm_lwr_sz_arr"
		" failed\n");
	return -1;
    }
    ...
    if ((agg_ol_lwr_ct_arr = (int *) ADIOI_Calloc(nprocs, sizeof(int)))
	== NULL)
    {
	fprintf(stderr, "ADIOI_Build_client_reqs: "
		"malloc agg_ol_lwr_ct_arr failed\n");
	return -1;
    }
    ...
    for (i = 0; i < MAX_OFF_TYPE; i++)
    {
	lwr_total_agg_comm_sz = 0;
	memset(agg_comm_lwr_sz_arr, 0, nprocs*sizeof(ADIO_Offset));
	memset(agg_mem_next_off_arr, -1, nprocs*sizeof(ADIO_Offset));
	while (total_agg_comm_sz > lwr_total_agg_comm_sz)
	{
		 ...
		 * along. */
		if (tmp_agg_fr_idx < 0 || 
		    agg_comm_lwr_sz_arr[j] == agg_comm_sz_arr[j])
		{
		    continue;
		}
		    ...
		    (lwr_off > tmp_lwr_off))
		{
		    lwr_off_proc = j;
		    lwr_off = tmp_lwr_off;
		    lwr_reg_max_len = tmp_lwr_reg_max_len;
		}
	    }

	    assert(lwr_off_proc != -1);
	    
	    /* Add up to the end of the file realm or as many bytes
	     * are left for this particular aggregator in the client's
	     * filetype */
	    if (lwr_reg_max_len > agg_comm_sz_arr[lwr_off_proc] - 
		agg_comm_lwr_sz_arr[lwr_off_proc])
	    {
		lwr_reg_max_len = agg_comm_sz_arr[lwr_off_proc] - 
		    agg_comm_lwr_sz_arr[lwr_off_proc];
	    }
	    assert(lwr_reg_max_len > 0);
	    
	    view_state_add_region(
		lwr_reg_max_len,
		&(agg_file_view_state_arr[lwr_off_proc]),
		&st_reg, &act_reg_sz, i);
	     ...
	     * file_view_state. */
	    tmp_file_state_p = &(agg_file_view_state_arr[lwr_off_proc]);
	    tmp_mem_state_p = &(my_mem_view_state_arr[lwr_off_proc]);
	    assert(view_state_get_lwr_sz(tmp_file_state_p, i) - act_reg_sz >=
		   view_state_get_lwr_sz(tmp_mem_state_p, i));
	    while (view_state_get_lwr_sz(tmp_file_state_p, i) - act_reg_sz != 
		   view_state_get_lwr_sz(tmp_mem_state_p, i))
	    {
		ADIO_Offset fill_st_reg = -1, fill_reg_sz = -1;
		view_state_add_region(
		    view_state_get_lwr_sz(tmp_file_state_p, i) - act_reg_sz -
		    view_state_get_lwr_sz(tmp_mem_state_p, i),
		    tmp_mem_state_p,
		    &fill_st_reg,
		    &fill_reg_sz, i);
		...
		fprintf(stderr, "ADIOI_Build_client_reqs: Mem region %s"
			...
			agg_mem_st_reg, agg_mem_act_reg_sz);
#endif
		agg_comm_lwr_sz_arr[lwr_off_proc] += agg_mem_act_reg_sz;
		lwr_total_agg_comm_sz += agg_mem_act_reg_sz;	    
		switch(i)
		{
		    case TEMP_OFF:
			...
			if (agg_mem_next_off_arr[lwr_off_proc] != 
			    agg_mem_st_reg)
			{
			    agg_ol_ct_arr[lwr_off_proc]++;
			}
			agg_mem_next_off_arr[lwr_off_proc] = 
			    agg_mem_st_reg + agg_mem_act_reg_sz;
			break;
		    case REAL_OFF:
			 ...
			 * possible. */
			agg_next_off_idx = agg_ol_lwr_ct_arr[lwr_off_proc];
			if (agg_mem_next_off_arr[lwr_off_proc] != 
			    agg_mem_st_reg)
			{
			    agg_disp_arr[lwr_off_proc][agg_next_off_idx] = 
				agg_mem_st_reg;
			    agg_blk_arr[lwr_off_proc][agg_next_off_idx] = 
				agg_mem_act_reg_sz;
			    (agg_ol_lwr_ct_arr[lwr_off_proc])++;
			}
			else
			{
			    agg_blk_arr[lwr_off_proc][agg_next_off_idx - 1]
				+= agg_mem_act_reg_sz;
			}
			agg_mem_next_off_arr[lwr_off_proc] = 
			    agg_mem_st_reg + agg_mem_act_reg_sz;
...
#ifdef DEBUG
    fprintf(stderr, "ADIOI_Build_client_reqs:(agg,lwr_ol_count=ol_count)=");
    for (i = 0; i < nprocs; i++)
    {
	int tmp_agg_idx = ADIOI_Agg_idx(i, fd);
	if (tmp_agg_idx >= 0)
	{
	    fprintf(stderr, "(%d,%d=%d)", i, agg_ol_lwr_ct_arr[i],
		    agg_ol_ct_arr[i]);
	    assert(agg_ol_ct_arr[i] == agg_ol_lwr_ct_arr[i]);
    ...
    ADIOI_Free(agg_comm_lwr_sz_arr);
    ADIOI_Free(agg_ol_ct_arr);
    ADIOI_Free(agg_ol_lwr_ct_arr);
#ifdef AGGREGATION_PROFILE
    MPE_Log_event (5019, 0, NULL);
#endif    
    return 0;
}
/* ADIOI_Build_client_pre_req allows a client to callwlate the memtype
			       ...
			       int max_ol_ct)
{
    ...
    ADIO_Offset lwr_sz = 0, max_sz = 0, agg_mem_next_off = -1;
    ADIO_Offset fill_st_reg = -1, fill_reg_sz = -1;
    ADIO_Offset *fr_st_off_arr = fd->file_realm_st_offs;
    MPI_Datatype *fr_type_arr = fd->file_realm_types;
    ...
    if (agg_file_view_state_p->lwr_state.lwr_sz == 
	...
	switch(i)
	{
		...
		break;
	    case REAL_OFF:
		tmp_mem_state_p  = &(my_mem_view_state_p->lwr_state);
		tmp_file_state_p = &(agg_file_view_state_p->lwr_state);
		break;
	    default:
	...
	{
	    lwr_sz = my_mem_view_state_p->pre_sz;
	...
	{
	    lwr_sz = my_mem_view_state_p->pre_sz;
	    agg_ol_lwr_ct = my_mem_view_state_p->pre_ol_ct;
	    ...
	    agg_mem_next_off = 
		my_mem_view_state_p->pre_disp_arr[agg_ol_lwr_ct - 1] +
		my_mem_view_state_p->pre_blk_arr[agg_ol_lwr_ct - 1];
	}
	else
	{
	    lwr_sz = 0;
	 ...
	 * to preprocess */
	if (max_pre_req_sz - lwr_sz > 
	    agg_file_view_state_p->sz - tmp_file_state_p->lwr_sz)
	{
	    max_sz = lwr_sz +
		agg_file_view_state_p->sz - tmp_file_state_p->lwr_sz;
	}
	...
	assert(lwr_sz != max_sz);
#ifdef DEBUG1
	fprintf(stderr, 
		"ADIOI_Build_client_pre_req: (lwr_sz=%Ld,agg_ol_ct=%d,"
		"agg_mem_next_off=%Ld,max_sz=%Ld,max_ol_ct=%d)\n", 
		lwr_sz, agg_ol_ct, agg_mem_next_off, max_sz, max_ol_ct);
#endif
	while (lwr_sz < max_sz)
	{
	     ...
	     * even if lwr_sz != max_sz since find_next_off may
	    ...
	    if (lwr_reg_max_len > max_sz - lwr_sz)
		lwr_reg_max_len = max_sz - lwr_sz;

	    assert(lwr_reg_max_len > 0);

	    /* Add memtype ol pairs while we have not passed
	     * lwr_reg_max_len or the max number of ol pairs
	     * allowed */
	    act_reg_sz = 0;
	    exit_loop = 0;
	    while ((act_reg_sz < lwr_reg_max_len) && 
		   (exit_loop == 0))
	    {
		view_state_add_region(
		    lwr_reg_max_len - act_reg_sz,
		    my_mem_view_state_p,
		    &agg_mem_st_reg, &agg_mem_act_reg_sz, 
		    i);
		...
		switch(i)
		{
			 ...
			 * if possible. */
			agg_next_off_idx = agg_ol_lwr_ct;
			if (agg_mem_next_off != agg_mem_st_reg)
			{
			    my_mem_view_state_p->
    ...
    my_mem_view_state_p->pre_sz = lwr_sz;
    my_mem_view_state_p->pre_ol_ct = agg_ol_ct;

#ifdef DEBUG1
    fprintf(stderr, "ADIOI_Build_client_pre_req:(agg=%d,lwr_ol_count=%d"
	    "=ol_count=%d)\n",
			   ...
			   ADIO_Offset *agg_mem_next_off_p)
{
    ...
    {
	case TEMP_OFF:
	    /* Use only some of the precallwlated data */
		...
		*agg_comm_lwr_sz_p = *agg_comm_pre_sz_p;
		*agg_ol_ct_p = i;
		
	    }
	    else /* Use all the precallwlated data */
	    {
		...
		*agg_mem_next_off_p = 
	    ...
	    fprintf(stderr, "process_pre_req: TEMP_OFF "
		    "agg_comm_pre_sz=%Ld,agg_comm_lwr_sz=%Ld,agg_ol_ct=%d\n",
		    *agg_comm_pre_sz_p, *agg_comm_lwr_sz_p, *agg_ol_ct_p);
#endif
	    assert(*agg_comm_lwr_sz_p <= *agg_comm_sz_p);
	    break;
	case REAL_OFF:
	    ...
	    *agg_comm_lwr_sz_p = *agg_comm_pre_sz_p;
	    ...
	    else /* Used all the precallwlated ol pairs */
	    {
		ADIOI_Free(my_mem_view_state_p->pre_disp_arr);
		ADIOI_Free(my_mem_view_state_p->pre_blk_arr);
			   ...
			   MPI_Datatype *agg_comm_dtype_p)
{
    ...
    ADIO_Offset agg_mem_next_off = 0, agg_comm_lwr_sz = 0, agg_comm_pre_sz = 0;
    ADIO_Offset *fr_st_off_arr = fd->file_realm_st_offs;
    MPI_Datatype *fr_type_arr = fd->file_realm_types;
    flatten_state *tmp_mem_state_p = NULL, *tmp_file_state_p = NULL;
#ifdef DTYPE_SKIP
	...
	while (agg_comm_lwr_sz < agg_comm_sz)
	{	
	    find_next_off(fd, agg_file_view_state_p,
			  fr_st_off_arr[agg_idx],
			  &(fr_type_arr[agg_idx]),
	     ...
	     * are left for this particular aggregator in the client's
	     * filetype */
	    if (lwr_reg_max_len > (agg_comm_sz - agg_comm_lwr_sz))
	    {
		lwr_reg_max_len = agg_comm_sz - agg_comm_lwr_sz;
	    }
	    assert(lwr_reg_max_len > 0);
	    ...
	    fprintf(stderr, "ADIOI_Build_client_req: %s File region"
		    " (proc=%d,off=%Ld,sz=%Ld)\n",
		    off_type_name[i], agg_rank, lwr_off, act_reg_sz);
#endif
	    ...
	    assert(tmp_file_state_p->lwr_sz - act_reg_sz >= 
		   tmp_mem_state_p->lwr_sz);
	    
	    while (tmp_file_state_p->lwr_sz - act_reg_sz != 
		   tmp_mem_state_p->lwr_sz)
	    {
		ADIO_Offset fill_st_reg = -1, fill_reg_sz = -1;
#ifdef DTYPE_SKIP
		if (my_mem_view_state_p->flat_type_p->count > 1) {
		    /* let's see if we can skip whole memory datatypes */
		    skip_type_ct =
			(tmp_file_state_p->lwr_sz - act_reg_sz -
			 tmp_mem_state_p->lwr_sz) /
			my_mem_view_state_p->type_sz;
		    if (skip_type_ct > 0) {
			tmp_mem_state_p->lwr_sz +=
			...
			tmp_mem_state_p->abs_off +=
			    skip_type_ct * my_mem_view_state_p->ext;
			if ((tmp_mem_state_p->lwr_sz - act_reg_sz) ==
			    tmp_file_state_p->lwr_sz)
			    break;
		    }
		}
		...
		view_state_add_region(
		    tmp_file_state_p->lwr_sz - 
		    act_reg_sz - tmp_mem_state_p->lwr_sz,
		    my_mem_view_state_p,
		    &fill_st_reg,
		    &fill_reg_sz, i);
	    }
		...
		fprintf(stderr, "ADIOI_Build_client_req: %s Mem region"
			...
			agg_mem_act_reg_sz);
#endif
		agg_comm_lwr_sz += agg_mem_act_reg_sz;
		switch(i)
		{
		    case TEMP_OFF:
			/* Increment the ol list count if the next
    ...
    assert(agg_ol_ct == agg_ol_lwr_ct);
#ifdef DEBUG1
    fprintf(stderr, 
	    "ADIOI_Build_client_req:(agg=%d,lwr_ol_count=%d=ol_count=%d)\n",
	    agg_rank, agg_ol_lwr_ct, agg_ol_ct);
#endif
