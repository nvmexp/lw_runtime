...
int ADIOI_Type_get_elwelope (MPI_Datatype datatype, int *num_integers,
                             int *num_addresses, int *num_datatypes, int *combiner)
{
  ...
  rc = MPI_Type_get_elwelope (datatype, num_integers, num_addresses, num_datatypes, combiner);
                             ...
                             MPI_Aint array_of_addresses[], MPI_Datatype array_of_datatypes[])
{
  ...
  rc = MPI_Type_get_elwelope (datatype, &dontcare, &dontcare, &dontcare, &combiner);
  if (MPI_SUCCESS != rc) {
    return rc;
  }
...
void ADIOI_Flatten_datatype(MPI_Datatype datatype)
{
  ...
  DBG_FPRINTF(stderr,"ADIOI_Flatten_datatype:: is_contig %#X\n",is_contig);
    ...
    flat->count = ADIOI_Count_contiguous_blocks(datatype, &lwrr_index);
#ifdef FLATTEN_DEBUG 
    DBG_FPRINTF(stderr,"ADIOI_Flatten_datatype:: count %llX, lwr_idx = %#llX\n",flat->count,lwrr_index);
#endif
/*    DBG_FPRINTF(stderr, "%d\n", flat->count);*/
  ...
  #endif
#else
    ADIOI_Flatten(datatype, flat, 0, &lwrr_index);
  #ifdef FLATTEN_DEBUG 
...
void ADIOI_Flatten(MPI_Datatype datatype, ADIOI_Flatlist_node *flat, 
		  ADIO_Offset st_offset, MPI_Count *lwrr_index)
{
    ...
    MPI_Datatype *types;
    ADIOI_Type_get_elwelope(datatype, &nints, &nadds, &ntypes, &combiner);
  ...
  #ifdef FLATTEN_DEBUG 
  DBG_FPRINTF(stderr,"ADIOI_Flatten:: st_offset %#llX, lwrr_index %#llX\n",st_offset,*lwrr_index);
   ...
   * standard:
    ...
    #endif
        ADIOI_Type_get_elwelope(types[0], &old_nints, &old_nadds,
			        &old_ntypes, &old_combiner); 
        ADIOI_Datatype_iscontig(types[0], &old_is_contig);
	if ((old_combiner != MPI_COMBINER_NAMED) && (!old_is_contig))
            ADIOI_Flatten(types[0], flat, st_offset, lwrr_index);
        break;
#endif
    ...
    case MPI_COMBINER_SUBARRAY:
      ...
      DBG_FPRINTF(stderr,"ADIOI_Flatten:: MPI_COMBINER_SUBARRAY\n");
	    ...
	    ADIO_Type_create_subarray(dims,
				      ...
				      types[0],        /* type */
				      &stype);
	    ADIOI_Flatten(stype, flat, st_offset, lwrr_index);
	    MPI_Type_free(&stype);
	}
	break;
#endif
    ...
    case MPI_COMBINER_DARRAY:
	{
      ...
      DBG_FPRINTF(stderr,"ADIOI_Flatten:: MPI_COMBINER_DARRAY\n");
	    ...
	    ADIO_Type_create_darray(ints[0],         /* size */
				    ...
				    &dtype);
      #ifdef FLATTEN_DEBUG 
      DBG_FPRINTF(stderr,"ADIOI_Flatten:: MPI_COMBINER_DARRAY <ADIOI_Flatten(dtype, flat->indices[%#X] %#llX, flat->blocklens[%#X] %#llX, st_offset %#llX, lwrr_index %#llX);\n",
              0, flat->indices[0], 0, flat->blocklens[0], st_offset, *lwrr_index);
      #endif
	    ADIOI_Flatten(dtype, flat, st_offset, lwrr_index);
      #ifdef FLATTEN_DEBUG 
      DBG_FPRINTF(stderr,"ADIOI_Flatten:: MPI_COMBINER_DARRAY >ADIOI_Flatten(dtype, flat->indices[%#X] %#llX, flat->blocklens[%#X] %#llX, st_offset %#llX, lwrr_index %#llX);\n",
              0, flat->indices[0], 0, flat->blocklens[0], st_offset, *lwrr_index);
      #endif
	...
	prev_index = *lwrr_index;
	if ((old_combiner != MPI_COMBINER_NAMED) && (!old_is_contig))
	    ADIOI_Flatten(types[0], flat, st_offset, lwrr_index);

	if (prev_index == *lwrr_index) {
	...
	if ((old_combiner != MPI_COMBINER_NAMED) && (!old_is_contig))
  {
    ...
    ADIO_Offset stride = ints[top_count+1];
        ADIOI_Flatten(types[0], flat,
         st_offset+stride* ADIOI_AINT_CAST_TO_OFFSET old_extent, lwrr_index);
  }
...
/* Now repeat with strides. */
	...
	if ((old_combiner != MPI_COMBINER_NAMED) && (!old_is_contig))
  {
        ADIOI_Flatten(types[0], flat, st_offset+adds[0], lwrr_index); 
  }
...
/* Now repeat with strides. */
	...
	for (n=0; n<top_count; n++) {
	    ADIOI_Type_get_elwelope(types[n], &old_nints, &old_nadds,
				    &old_ntypes, &old_combiner); 
            ADIOI_Datatype_iscontig(types[n], &old_is_contig);

	    prev_index = *lwrr_index;
            if ((old_combiner != MPI_COMBINER_NAMED) && (!old_is_contig))
		ADIOI_Flatten(types[n], flat, st_offset+adds[n], lwrr_index);
...
/* The current type has to be replicated blocklens[n] times */
    ...
    /* handle the Lb */
	j = *lwrr_index;
	/* when we process resized types, we (relwrsively) process the lower
...
/* There is a bug in SGI's impl. of MPI_Type_get_contents. It doesn't
   return new datatypes. Therefore no need to free. */
    for (i=0; i<ntypes; i++) {
 	MPI_Type_get_elwelope(types[i], &old_nints, &old_nadds, &old_ntypes,
 			      &old_combiner);
  ...
  #ifdef FLATTEN_DEBUG 
  DBG_FPRINTF(stderr,"ADIOI_Flatten:: return st_offset %#llX, lwrr_index %#llX\n",st_offset,*lwrr_index);
  #endif
 ...
 * lwrr_index to reflect the space for the additional blocks.
 *
 * ASSUMES THAT TYPE IS NOT A BASIC!!!
 */
MPI_Count ADIOI_Count_contiguous_blocks(MPI_Datatype datatype, MPI_Count *lwrr_index)
{
    int i, n;
	...
	ADIOI_Datatype_iscontig(types[0], &old_is_contig);
	if ((old_combiner != MPI_COMBINER_NAMED) && (!old_is_contig))
	    count = ADIOI_Count_contiguous_blocks(types[0], lwrr_index);
	else {
		count = 1;
		(*lwrr_index)++;
	}
    ...
    case MPI_COMBINER_SUBARRAY:
	    ...
	    ADIO_Type_create_subarray(dims,
				      ...
				      types[0],        /* type */
				      &stype);
	    count = ADIOI_Count_contiguous_blocks(stype, lwrr_index);
	    /* lwrr_index will have already been updated; just pass
	     * count back up.
	     */
	    MPI_Type_free(&stype);
    ...
    case MPI_COMBINER_DARRAY:
	{
	    ...
	    ADIO_Type_create_darray(ints[0],         /* size */
				    ...
				    types[0],
				    &dtype);
	    count = ADIOI_Count_contiguous_blocks(dtype, lwrr_index);
	    /* lwrr_index will have already been updated; just pass
	     * count back up.
	     */
	    MPI_Type_free(&dtype);
...
/* indexed type made up of noncontiguous derived types */
	    basic_num = *lwrr_index - prev_index;

/* The noncontiguous types have to be replicated blocklens[i] times
   and then strided. */
	    *lwrr_index += (ints[1]-1) * basic_num;
	    count *= ints[1];

/* Now repeat with strides. */
	    for (i=1; i<top_count; i++) {
		count += ints[1+i] * basic_num;
		*lwrr_index += ints[1+i] * basic_num;
	    }
	}
	break;
...
/* Now repeat with strides. */
	...
	for (n=0; n<top_count; n++) {
	    ...
	    prev_index = *lwrr_index;
	    if ((old_combiner != MPI_COMBINER_NAMED) && (!old_is_contig))
	    count += ADIOI_Count_contiguous_blocks(types[n], lwrr_index);
