...
void ADIOI_PANFS_Open6(ADIO_File fd, int *error_code)
{
    ...
    if (fd->access_mode & ADIO_CREATE) {
        ...
        *error_code = MPI_SUCCESS;
         ...
         * PAN_FS_CLIENT_LAYOUT_TYPE__ILWALID                 = 0,    - *INVALID
        ...
        if ((layout_type == PAN_FS_CLIENT_LAYOUT_TYPE__RAID1_5_PARITY_STRIPE) ||
             ...
             * be able to handle this condition.  Lwrrently,
    ...
    value = (char *) ADIOI_Malloc((MPI_MAX_INFO_VAL + 1) * sizeof(char));
    ADIOI_Info_get(fd->info, "panfs_conlwrrent_write", MPI_MAX_INFO_VAL, value, &flag);
    if (flag) {
        unsigned long int conlwrrent_write = strtoul(value, NULL, 10);
        if (conlwrrent_write == 1) {
            amode = amode | O_CONLWRRENT_WRITE;
        }
    }
    ADIOI_Free(value);
    ...
    if (fd->fd_sys != -1) {
        ...
        if (rc < 0) {
            ...
            ADIOI_Info_set(fd->info, "panfs_layout_type", "PAN_FS_CLIENT_LAYOUT_TYPE__ILWALID");
        }
        else {
