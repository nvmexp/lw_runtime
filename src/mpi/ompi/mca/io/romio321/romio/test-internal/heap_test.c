...
int main(int argc, char **argv) {
	...
	if (!strcmp("-A", argv[i])) {
	    ...
	    test.offsets[lwrr_add_idx] = atoi(argv[i]);
	    lwrr_add_idx++;
	    i++;
	}
	else {
...
int run_test(test_params_t *test) {
    ...
    int lwrr_insert_idx  = 0;
    int lwrr_extract_idx = 0;
	...
	for (j=0; j<test->action_count_arr[i]; j++) {
		...
		case BUILD:
		    ...
		    break;
		case INSERT:
		    ADIOI_Heap_insert(&myheap, test->offsets[lwrr_insert_idx],
				lwrr_insert_idx, lwrr_insert_idx);
		    lwrr_insert_idx++;
		    break;
		case EXTRACT:
		    heap_extract_min(&myheap, &extracted[lwrr_extract_idx],
				     &stored_proc, &stored_reg_max_len);
		    if (test->verify && (extracted[lwrr_extract_idx] !=
			test->correct_order[lwrr_extract_idx]))
			err_flag++;
		    lwrr_extract_idx++;
		    break;
		case EXTRACT_INSERT:
