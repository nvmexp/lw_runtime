...
#include "osc_base_obj_colwert.h"
                                      ...
                                      uint32_t *prim_count)
{
    ...
    /* We now have the count as a size_t, colwert it to an uint32_t */
                              ...
                              ompi_op_t *op)
{
    ...
    if(MPI_MINLOC == op || MPI_MAXLOC == op) {
        if(MPI_SHORT_INT == datatype ||
           ...
           opal_output(0, "Error: %s datatype is lwrrently "
    ...
    if (ompi_datatype_is_predefined(datatype)) {
        ...
        if (ompi_datatype_is_contiguous_memory_layout (datatype, count) &&
             ...
             * datatype. fast path. do not set up a colwertor to deal with the
             * datatype. */
        ...
        MEMCHECKER(
            memchecker_colwertor_call(&opal_memchecker_base_mem_noaccess,
                                      &colwertor);
        );
                             ...
                             ompi_op_t *op)
{
    ...
    opal_colwertor_copy_and_prepare_for_send (ompi_mpi_local_colwertor, &origin_dt->super,
                                              origin_count, origin, 0, &origin_colwertor);

    OBJ_CONSTRUCT(&target_colwertor, opal_colwertor_t);
    opal_colwertor_copy_and_prepare_for_recv (ompi_mpi_local_colwertor, &target_dt->super,
                                              target_count, target, 0, &target_colwertor);
    ...
    do {
        ...
        done = opal_colwertor_raw (&origin_colwertor, origin_iovec, &origin_iov_count, &origin_size);
