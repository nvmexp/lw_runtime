...
struct ompi_osc_rdma_state_t {
    /** used when rdma is in use to handle exlwsive locks and global shared locks (lock_all) */
    ...
    ompi_osc_rdma_lock_t aclwmulate_lock;
...
#if OPAL_HAVE_ATOMIC_MATH_64
    volatile int64_t lwrr_index;
#else
    volatile int32_t lwrr_index;
#endif
