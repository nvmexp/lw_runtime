 ...
 * continue to submit rdma transfers until the entire region is transferred or a fatal error oclwrs.
 */
                                           ...
                                           const ompi_osc_rdma_fn_t rdma_fn, const bool alloc_reqs)
{
    ...
    opal_colwertor_t local_colwertor, remote_colwertor;
    ...
    OBJ_CONSTRUCT(&remote_colwertor, opal_colwertor_t);
    ret = opal_colwertor_copy_and_prepare_for_send (ompi_mpi_local_colwertor, &remote_datatype->super, remote_count,
                                                    (void *) (intptr_t) remote_address, 0, &remote_colwertor);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }

    OBJ_CONSTRUCT(&local_colwertor, opal_colwertor_t);
    ret = opal_colwertor_copy_and_prepare_for_send (ompi_mpi_local_colwertor, &local_datatype->super, local_count,
                                                    local_address, 0, &local_colwertor);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    do {
        ...
        /* opal_colwertor_raw returns true when it has reached the end of the data */
        done = opal_colwertor_raw (&remote_colwertor, remote_iovec, &remote_iov_count, &remote_size);
        ...
        while (remote_iov_index != remote_iov_count) {
            if (local_iov_index == local_iov_count) {
                ...
                local_iov_index = 0;
                (void) opal_colwertor_raw (&local_colwertor, local_iovec, &local_iov_count, &local_size);
            }
    ...
    /* clean up colwertors */
    opal_colwertor_cleanup (&local_colwertor);
    OBJ_DESTRUCT(&local_colwertor);
    opal_colwertor_cleanup (&remote_colwertor);
    OBJ_DESTRUCT(&remote_colwertor);
