...
ompi_osc_base_module_t ompi_osc_rdma_module_rdma_template = {
    ...
    .osc_aclwmulate = ompi_osc_rdma_aclwmulate,
    .osc_compare_and_swap = ompi_osc_rdma_compare_and_swap,
    .osc_fetch_and_op = ompi_osc_rdma_fetch_and_op,
    .osc_get_aclwmulate = ompi_osc_rdma_get_aclwmulate,

    .osc_rput = ompi_osc_rdma_rput,
    .osc_rget = ompi_osc_rdma_rget,
    .osc_raclwmulate = ompi_osc_rdma_raclwmulate,
    .osc_rget_aclwmulate = ompi_osc_rdma_rget_aclwmulate,
...
static int ompi_osc_rdma_component_register (void)
{
    ...
    asprintf(&description_str, "Enable optimizations for MPI_Fetch_and_op, MPI_Aclwmulate, etc for codes "
                                          ...
                                          int flavor)
{
    ...
    if (MPI_WIN_FLAVOR_CREATE == flavor) {
        if (opal_lwda_check_bufs(*base, NULL)) {
            return -1;
        }
    }
#endif /* OPAL_LWDA_SUPPORT */
...
static int allocate_state_single (ompi_osc_rdma_module_t *module, void **base, size_t size)
{
    ...
    /* allocate anything that will be accessed remotely in the same region. this lwts down on the number of
                                           ...
                                           int flavor, int *model)
{
    ...
    OBJ_CONSTRUCT(&module->lock, opal_relwrsive_mutex_t);
    ...
    /* callwlate and store various structure sizes */
