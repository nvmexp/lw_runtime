...
#include "osc_rdma_aclwmulate.h"
#include "osc_rdma_request.h"
#include "osc_rdma_comm.h"

#include "ompi/mca/osc/base/osc_base_obj_colwert.h"

static inline void ompi_osc_rdma_peer_aclwmulate_cleanup (ompi_osc_rdma_module_t *module, ompi_osc_rdma_peer_t *peer, bool lock_acquired)
{
    if (lock_acquired) {
        (void) ompi_osc_rdma_lock_release_exclusive (module, peer, offsetof (ompi_osc_rdma_state_t, aclwmulate_lock));
    }

    /* clear out the aclwmulation flag */
    ompi_osc_rdma_peer_clear_flag (peer, OMPI_OSC_RDMA_PEER_ACLWMULATING);
}
                                     ...
                                     ompi_osc_rdma_request_t *request, bool lock_acquired)
{
    ...
    ompi_osc_rdma_peer_aclwmulate_cleanup (module, peer, lock_acquired);
    ...
    if (&ompi_mpi_op_replace.op != op || OMPI_OSC_RDMA_TYPE_GET_ACC == request->type) {
        ...
        if (OMPI_OSC_RDMA_TYPE_GET_ACC == request->type) {
            if (NULL == result) {
                ...
                opal_colwertor_unpack (&request->colwertor, &iov, &iov_count, &size);
                opal_colwertor_cleanup (&request->colwertor);
            } else {
...
{
    ompi_osc_rdma_peer_aclwmulate_cleanup (request->module, request->peer, !ompi_osc_rdma_peer_is_exclusive (request->peer));
}
                                             ...
                                             ompi_datatype_t *target_datatype, ompi_op_t *op, ompi_osc_rdma_request_t *request)
{
    ...
    opal_colwertor_t source_colwertor, target_colwertor;
    ...
    /* needed for opal_colwertor_raw but not used */
    ...
    if (OPAL_LIKELY((!source_count || ompi_datatype_is_predefined (source_datatype)) &&
        ...
        if (source_datatype) {
            /* the colwertors will handle the lb */
            (void) ompi_datatype_get_extent (source_datatype, &lb, &extent);
            source_addr = (void *)((intptr_t) source_addr - lb);
        }
    ...
    /* the colwertor will handle lb from here */
    (void) ompi_datatype_get_extent (target_datatype, &lb, &extent);
    target_address -= lb;
    ...
    /* prepare colwertors for the source and target. these colwertors will be used to determine the
     * contiguous segments within the source and target. */
    /* the source may be NULL if using MPI_OP_NO_OP with MPI_Get_aclwmulate */
    if (source_datatype) {
        OBJ_CONSTRUCT(&source_colwertor, opal_colwertor_t);
        ret = opal_colwertor_copy_and_prepare_for_send (ompi_mpi_local_colwertor, &source_datatype->super, source_count, source_addr,
                                                       0, &source_colwertor);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            return ret;
        }
    ...
    OBJ_CONSTRUCT(&target_colwertor, opal_colwertor_t);
    ret = opal_colwertor_copy_and_prepare_for_send (ompi_mpi_local_colwertor, &target_datatype->super, target_count,
                                                    (void *) (intptr_t) target_address, 0, &target_colwertor);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    do {
        ...
        source_iov_index = 0;
        /* opal_colwertor_raw returns done when it has reached the end of the data */
        if (!source_datatype) {
            done = true;
            ...
            done = opal_colwertor_raw (&source_colwertor, source_iovec, &source_iov_count, &source_size);
        }
        ...
        while (source_iov_index != source_iov_count) {
            if (target_iov_index == target_iov_count) {
                ...
                target_iov_index = 0;
                (void) opal_colwertor_raw (&target_colwertor, target_iovec, &target_iov_count, &target_size);
            }
            ...
            if (result_datatype) {
                /* prepare a colwertor for this part of the result */
                opal_colwertor_copy_and_prepare_for_recv (ompi_mpi_local_colwertor, &result_datatype->super, result_count,
                                                          result_addr, 0, &subreq->colwertor);
                opal_colwertor_set_position (&subreq->colwertor, &result_position);
    ...
    if (source_datatype) {
        opal_colwertor_cleanup (&source_colwertor);
        OBJ_DESTRUCT(&source_colwertor);
    }

    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "finished scheduling rdma on non-contiguous datatype(s)");

    opal_colwertor_cleanup (&target_colwertor);
    OBJ_DESTRUCT(&target_colwertor);
 ...
 * be depending on the platform, compiler, etc. This function lwrrently
                                    ...
                                    ompi_win_t *win)
{
    ...
    while (!ompi_osc_rdma_peer_test_set_flag (peer, OMPI_OSC_RDMA_PEER_ACLWMULATING)) {
        ompi_osc_rdma_progress (module);
    }

    /* get an exclusive lock on the peer */
    if (!ompi_osc_rdma_peer_is_exclusive (peer) && !(module->acc_single_intrinsic || win->w_acc_ops <= OMPI_WIN_ACLWMULATE_OPS_SAME_OP)) {
        (void) ompi_osc_rdma_lock_acquire_exclusive (module, peer, offsetof (ompi_osc_rdma_state_t, aclwmulate_lock));
        lock_acquired = true;
    }
...
static inline
int ompi_osc_rdma_rget_aclwmulate_internal (ompi_osc_rdma_sync_t *sync, const void *origin_addr, int origin_count,
...
int ompi_osc_rdma_get_aclwmulate (const void *origin_addr, int origin_count, ompi_datatype_t *origin_datatype,
                                  ...
                                  ompi_op_t *op, ompi_win_t *win)
{
    ...
    return ompi_osc_rdma_rget_aclwmulate_internal (sync, origin_addr, origin_count, origin_datatype,
...
int ompi_osc_rdma_rget_aclwmulate (const void *origin_addr, int origin_count, ompi_datatype_t *origin_datatype,
                                   void *result_addr, int result_count, ompi_datatype_t *result_datatype,
                                   int target_rank, MPI_Aint target_disp, int target_count, ompi_datatype_t *target_datatype,
                                   ompi_op_t *op, ompi_win_t *win, ompi_request_t **request)
{
    ...
    ret = ompi_osc_rdma_rget_aclwmulate_internal (sync, origin_addr, origin_count, origin_datatype, result_addr,
...
int ompi_osc_rdma_raclwmulate (const void *origin_addr, int origin_count, ompi_datatype_t *origin_datatype, int target_rank,
                               ...
                               ompi_win_t *win, ompi_request_t **request)
{
    ...
    ret = ompi_osc_rdma_rget_aclwmulate_internal (sync, origin_addr, origin_count, origin_datatype, NULL, 0,
...
int ompi_osc_rdma_aclwmulate (const void *origin_addr, int origin_count, ompi_datatype_t *origin_datatype, int target_rank,
                              ptrdiff_t target_disp, int target_count, ompi_datatype_t *target_datatype, ompi_op_t *op,
                              ompi_win_t *win)
{
    ompi_osc_rdma_module_t *module = GET_MODULE(win);
    ...
    return ompi_osc_rdma_rget_aclwmulate_internal (sync, origin_addr, origin_count, origin_datatype, NULL, 0,
                                                   NULL, peer, target_rank, target_disp, target_count, target_datatype,
                                                   op, NULL);
}
                                ...
                                ptrdiff_t target_disp, ompi_op_t *op, ompi_win_t *win)
{
    ...
    return ompi_osc_rdma_rget_aclwmulate_internal (sync, origin_addr, 1, dt, result_addr, 1, dt, peer,
