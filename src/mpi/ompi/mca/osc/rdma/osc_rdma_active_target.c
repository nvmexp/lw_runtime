...
static void ompi_osc_rdma_handle_post (ompi_osc_rdma_module_t *module, int rank, ompi_osc_rdma_peer_t **peers, int npeers) {
    ...
    for (int j = 0 ; j < npeers ; ++j) {
        if (rank == peers[j]->rank) {
            ...
            /* an atomic is not really necessary as this function is lwrrently used but it doesn't hurt */
...
static int ompi_osc_rdma_post_peer (ompi_osc_rdma_module_t *module, ompi_osc_rdma_peer_t *peer)
{
    ...
    do {
        ...
        /* try to post. if the value isn't 0 then another rank is oclwpying this index */
        if (!ompi_osc_rdma_peer_local_state (peer)) {
        ...
        /* prevent cirlwlar wait by checking for post messages received */
...
int ompi_osc_rdma_fence_atomic (int assert, ompi_win_t *win)
{
     ...
     * no communication has oclwrred. this flag will be set to true on the next put, get,
