...
struct ompi_osc_pt2pt_module_t {
    ...
    /** origin side list of locks lwrrently outstanding */
    ...
    opal_atomic_lock_t aclwmulate_lock;

    /** accumulate operations pending the aclwmulation lock */
...
int ompi_osc_pt2pt_aclwmulate(const void *origin_addr,
			      int origin_count,
			      struct ompi_datatype_t *origin_dt,
			      int target,
			      ptrdiff_t target_disp,
...
int ompi_osc_pt2pt_get_aclwmulate(const void *origin_addr,
                                 int origin_count,
...
int ompi_osc_pt2pt_raclwmulate(const void *origin_addr,
                              int origin_count,
                              struct ompi_datatype_t *origin_dt,
                              int target,
                              ptrdiff_t target_disp,
...
int ompi_osc_pt2pt_rget_aclwmulate(const void *origin_addr,
                                  int origin_count,
                                  struct ompi_datatype_t *origin_datatype,
                                  void *result_addr,
                                  int result_count,
 ...
 *
 * @short Progress one pending aclwmulation or compare and swap operation.
 *
 * @param[in] module   - OSC PT2PT module
 *
 * @long If the aclwmulation lock can be aquired progress one pending
 ...
 *       buffer. The copy is done with a colwertor generated from proc,
 *       datatype, and count.
 */
                                          ...
                                          int count, ompi_datatype_t *datatype)
{
    ...
    /* initialize colwertor */
    opal_colwertor_copy_and_prepare_for_recv(proc->super.proc_colwertor, &datatype->super, count, target,
                                             0, &colwertor);

    iov.iov_len  = source_len;
    iov.iov_base = (IOVBASE_TYPE *) source;
    max_data     = iov.iov_len;
    MEMCHECKER(memchecker_colwertor_call(&opal_memchecker_base_mem_defined, &colwertor));

    opal_colwertor_unpack (&colwertor, &iov, &iov_count, &max_data);

    MEMCHECKER(memchecker_colwertor_call(&opal_memchecker_base_mem_noaccess, &colwertor));

    OBJ_DESTRUCT(&colwertor);
}
                                           ...
                                           int count, ompi_datatype_t *datatype)
{
    ...
    opal_colwertor_copy_and_prepare_for_send(proc->super.proc_colwertor, &datatype->super,
                                             count, source, 0, &colwertor);

    iov.iov_len = target_len;
    iov.iov_base = (IOVBASE_TYPE *) target;
    opal_colwertor_pack(&colwertor, &iov, &iov_count, &max_data);
...
/**
 * ompi_osc_pt2pt_aclwmulate_lock:
 *
 * @short Internal function that spins until the aclwmulation lock has
 *        been aquired.
 *
 * @param[in] module - OSC PT2PT module
 *
 * @returns 0
 *
 * @long This functions blocks until the aclwmulation lock has been aquired. This
 ...
 *       module. It will be released by ompi_osc_pt2pt_aclwmulate_unlock().
 */
static inline int ompi_osc_pt2pt_aclwmulate_lock (ompi_osc_pt2pt_module_t *module)
{
    while (opal_atomic_trylock (&module->aclwmulate_lock)) {
        opal_progress ();
    }
...
/**
 * ompi_osc_pt2pt_aclwmulate_trylock:
 *
 * @short Try to aquire the aclwmulation lock.
 *
 * @param[in] module - OSC PT2PT module
 *
 * @returns 0 if the aclwmulation lock was aquired
 * @returns 1 if the lock was not available
 *
 * @long This function will try to aquire the aclwmulation lock. This function
 *       is safe to call from a callback.
 */
static inline int ompi_osc_pt2pt_aclwmulate_trylock (ompi_osc_pt2pt_module_t *module)
{
    return opal_atomic_trylock (&module->aclwmulate_lock);
}
...
/**
 * ompi_osc_pt2pt_aclwmulate_unlock:
 *
 * @short Unlock the aclwmulation lock and release a pending aclwmulation operation.
 *
 * @param[in] module - OSC PT2PT module
 *
 * @long This function unlocks the aclwmulation lock and release a single pending
 *       aclwmulation operation if one exists. This function may be called relwrsively.
 */
static inline void ompi_osc_pt2pt_aclwmulate_unlock (ompi_osc_pt2pt_module_t *module)
{
    opal_atomic_unlock (&module->aclwmulate_lock);
