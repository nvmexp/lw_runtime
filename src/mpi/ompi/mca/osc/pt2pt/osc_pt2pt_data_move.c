...
/**
 * struct osc_pt2pt_aclwmulate_data_t:
 *
 * @brief Data associated with an in-progress aclwmulation operation.
 */
struct osc_pt2pt_aclwmulate_data_t {
...
typedef struct osc_pt2pt_aclwmulate_data_t osc_pt2pt_aclwmulate_data_t;

static void osc_pt2pt_aclwmulate_data_constructor (osc_pt2pt_aclwmulate_data_t *acc_data)
{
...
static void osc_pt2pt_aclwmulate_data_destructor (osc_pt2pt_aclwmulate_data_t *acc_data)
{
    if (acc_data->source) {
...
OBJ_CLASS_DECLARATION(osc_pt2pt_aclwmulate_data_t);
OBJ_CLASS_INSTANCE(osc_pt2pt_aclwmulate_data_t, opal_list_item_t, osc_pt2pt_aclwmulate_data_constructor,
                   osc_pt2pt_aclwmulate_data_destructor);
 ...
 */
static inline int osc_pt2pt_aclwmulate_buffer (void *target, void *source, size_t source_len, ompi_proc_t *proc,
 ...
 * @param[in]  target        - Target for the aclwmulation
 ...
 * @param[out] acc_data_out  - New aclwmulation data
 *
 * This function is used to create a copy of the data needed to perform an aclwmulation.
 * This data should be provided to ompi_osc_pt2pt_isend_w_cb or ompi_osc_pt2pt_irecv_w_cb
 * as the ctx parameter with aclwmulate_cb as the cb parameter.
 */
static int osc_pt2pt_aclwmulate_allocate (ompi_osc_pt2pt_module_t *module, int peer, void *target, void *source, size_t source_len,
                                         ompi_proc_t *proc, int count, ompi_datatype_t *datatype, ompi_op_t *op,
                                         int request_count, osc_pt2pt_aclwmulate_data_t **acc_data_out)
{
    osc_pt2pt_aclwmulate_data_t *acc_data;

    acc_data = OBJ_NEW(osc_pt2pt_aclwmulate_data_t);
 ...
 * with ctx allocated by osc_pt2pt_aclwmulate_allocate. This callback will free the accumulate
 * data once the aclwmulation operation is complete.
 */
static int aclwmulate_cb (ompi_request_t *request)
{
    struct osc_pt2pt_aclwmulate_data_t *acc_data = (struct osc_pt2pt_aclwmulate_data_t *) request->req_complete_cb_data;
    ompi_osc_pt2pt_module_t *module = acc_data->module;
    int rank = MPI_PROC_NULL;
    int ret = OMPI_SUCCESS;

    OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                         "aclwmulate_cb, request_count = %d", acc_data->request_count));
    ...
    if (0 == OPAL_THREAD_ADD_FETCH32(&acc_data->request_count, -1)) {
        /* no more requests needed before the buffer can be aclwmulated */
        ...
        ompi_osc_pt2pt_aclwmulate_unlock (module);
 ...
 * @param[in] datatype   - Aclwmulation datatype
 * @param[in] acc_header - Accumulate header
 *
 * The module's aclwmulation lock must be held before calling this
                                    ...
                                    ompi_datatype_t *datatype, ompi_osc_pt2pt_header_acc_t *acc_header)
{
    ...
    ret = osc_pt2pt_aclwmulate_buffer (target, data, data_len, proc, acc_header->count,
                                      datatype, op);
...
static int ompi_osc_pt2pt_acc_long_start (ompi_osc_pt2pt_module_t *module, int source, ompi_datatype_t *datatype,
    ...
    struct osc_pt2pt_aclwmulate_data_t *acc_data;
    size_t buflen;
    void *buffer;
    ompi_proc_t *proc;
    void *target = (unsigned char*) module->baseptr +
    ...
    do {
        ...
        ret = osc_pt2pt_aclwmulate_allocate (module, source, target, buffer, buflen, proc, acc_header->count,
                                     ...
                                     ompi_datatype_t *datatype, ompi_osc_pt2pt_header_acc_t *acc_header)
{
    ...
    do {
        ret = osc_pt2pt_aclwmulate_allocate (module, source, target, data, data_len, proc, acc_header->count,
                                            datatype, op, 1, &acc_data);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            break;
        }
                                     ...
                                     ompi_osc_pt2pt_header_acc_t *acc_header)
{
    ...
    do {
        ...
        ret = ompi_osc_pt2pt_irecv_w_cb (buffer, acc_header->count, datatype,
                                        source, tag_to_target(acc_header->tag), module->comm,
                                        &recv_request, aclwmulate_cb, acc_data);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            OBJ_RELEASE(acc_data);
            break;
        }
 ...
 *
 * @brief Progress one pending aclwmulation or compare and swap operation.
 *
 * @param[in] module   - OSC PT2PT module
 *
 * If the aclwmulation lock can be aquired progress one pending
...
int ompi_osc_pt2pt_progress_pending_acc (ompi_osc_pt2pt_module_t *module)
{
     ...
     * operation completes */
    if (ompi_osc_pt2pt_aclwmulate_trylock (module)) {
        return OMPI_SUCCESS;
    }
    ...
    if (OPAL_UNLIKELY(NULL == pending_acc)) {
        /* called without any pending aclwmulation operations */
        ompi_osc_pt2pt_aclwmulate_unlock (module);
        return OMPI_SUCCESS;
    }
                               ...
                               ompi_osc_pt2pt_header_acc_t *acc_header)
{
    ...
    if (0 == ompi_osc_pt2pt_aclwmulate_trylock (module)) {
...
static int ompi_osc_pt2pt_callback (ompi_request_t *request)
{
    ...
    /* to avoid deep relwrsion from complet -> start -> complete -> ... we simply put this
