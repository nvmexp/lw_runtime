...
static inline ompi_osc_pt2pt_frag_t *ompi_osc_pt2pt_frag_alloc_non_buffered (ompi_osc_pt2pt_module_t *module,
                                                                             ...
                                                                             size_t request_len)
{
    ompi_osc_pt2pt_frag_t *lwrr;

    /* to ensure ordering flush the buffer on the peer */
    lwrr = peer->active_frag;
    if (NULL != lwrr && opal_atomic_compare_exchange_strong_ptr (&peer->active_frag, &lwrr, NULL)) {
        /* If there's something pending, the pending finish will
           start the buffer.  Otherwise, we need to start it now. */
        int ret = ompi_osc_pt2pt_frag_finish (module, lwrr);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            return NULL;
        }
    }

    lwrr = (ompi_osc_pt2pt_frag_t *) opal_free_list_get (&mca_osc_pt2pt_component.frags);
    if (OPAL_UNLIKELY(NULL == lwrr)) {
        return NULL;
    }

    lwrr->target = peer->rank;

    lwrr->header = (ompi_osc_pt2pt_frag_header_t*) lwrr->buffer;
    lwrr->top = (char*) (lwrr->header + 1);
    lwrr->remain_len = mca_osc_pt2pt_component.buffer_size;
    lwrr->module = module;
    lwrr->pending = 1;

    lwrr->header->base.type = OMPI_OSC_PT2PT_HDR_TYPE_FRAG;
    lwrr->header->base.flags = OMPI_OSC_PT2PT_HDR_FLAG_VALID;
    if (module->passive_target_access_epoch) {
        lwrr->header->base.flags |= OMPI_OSC_PT2PT_HDR_FLAG_PASSIVE_TARGET;
    }
    lwrr->header->source = ompi_comm_rank(module->comm);
    lwrr->header->num_ops = 1;
                                             ...
                                             char **ptr, bool long_send, bool buffered)
{
    ...
    if (buffered) {
        lwrr = peer->active_frag;
        if (NULL == lwrr || lwrr->remain_len < request_len || (long_send && lwrr->pending_long_sends == 32)) {
            lwrr = ompi_osc_pt2pt_frag_alloc_non_buffered (module, peer, request_len);
            if (OPAL_UNLIKELY(NULL == lwrr)) {
                OPAL_THREAD_UNLOCK(&module->lock);
                return OMPI_ERR_OUT_OF_RESOURCE;
            }

            lwrr->pending_long_sends = long_send;
            peer->active_frag = lwrr;
        } else {
            OPAL_THREAD_ADD_FETCH32(&lwrr->header->num_ops, 1);
            lwrr->pending_long_sends += long_send;
        }
    ...
    *buffer = lwrr;
    ...
    lwrr->top += request_len;
    lwrr->remain_len -= request_len;
