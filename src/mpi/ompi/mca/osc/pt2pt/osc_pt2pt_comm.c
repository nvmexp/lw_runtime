                                           ...
                                           ompi_datatype_t *datatype, ptrdiff_t target_disp, ompi_osc_pt2pt_module_t *module)
{
    ...
    ompi_osc_pt2pt_aclwmulate_lock (module);
...
static int
ompi_osc_pt2pt_aclwmulate_w_req (const void *origin_addr, int origin_count,
                                struct ompi_datatype_t *origin_dt,
                                int target, ptrdiff_t target_disp,
                                int target_count,
...
int
ompi_osc_pt2pt_aclwmulate(const void *origin_addr, int origin_count,
                         struct ompi_datatype_t *origin_dt,
                         int target, ptrdiff_t target_disp,
                         int target_count,
                         struct ompi_datatype_t *target_dt,
                         struct ompi_op_t *op, ompi_win_t *win)
{
    return ompi_osc_pt2pt_aclwmulate_w_req (origin_addr, origin_count, origin_dt,
...
int ompi_osc_pt2pt_fetch_and_op(const void *origin_addr, void *result_addr,
                               ...
                               struct ompi_win_t *win)
{
    return ompi_osc_pt2pt_get_aclwmulate(origin_addr, 1, dt, result_addr, 1, dt,
...
int ompi_osc_pt2pt_raclwmulate(const void *origin_addr, int origin_count,
                              ...
                              struct ompi_win_t *win, struct ompi_request_t **request)
{
    ...
    OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                         "raclwmulate: 0x%lx, %d, %s, %d, %d, %d, %s, %s, %s",
                         (unsigned long) origin_addr, origin_count,
                         origin_dt->name, target, (int) target_disp,
                         target_count, target_dt->name, op->o_name,
                         win->w_name));
    ...
    ret = ompi_osc_pt2pt_aclwmulate_w_req (origin_addr, origin_count, origin_dt, target,
...
static inline
int ompi_osc_pt2pt_rget_aclwmulate_internal (const void *origin_addr, int origin_count,
                                            ...
                                            bool release_req, struct ompi_request_t **request)
{
    ...
    /* get_aclwmulates are always request based, so that we know where to land the data */
    OMPI_OSC_PT2PT_REQUEST_ALLOC(win, pt2pt_request);
...
int ompi_osc_pt2pt_get_aclwmulate(const void *origin_addr, int origin_count,
                                 struct ompi_datatype_t *origin_dt,
                                 void *result_addr, int result_count,
                                 ...
                                 struct ompi_op_t *op, struct ompi_win_t *win)
{
    ...
    return ompi_osc_pt2pt_rget_aclwmulate_internal (origin_addr, origin_count, origin_dt,
...
int ompi_osc_pt2pt_rget_aclwmulate(const void *origin_addr, int origin_count,
                                  struct ompi_datatype_t *origin_dt,
                                  void *result_addr, int result_count,
                                  struct ompi_datatype_t *result_dt,
                                  int target, MPI_Aint target_disp,
