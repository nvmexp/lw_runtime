...
static int ompi_osc_pt2pt_lock_internal_exelwte (ompi_osc_pt2pt_module_t *module, ompi_osc_pt2pt_sync_t *lock)
{
...
static int ompi_osc_pt2pt_lock_internal (int lock_type, int target, int assert, ompi_win_t *win)
{
    ...
    if (OMPI_OSC_PT2PT_SYNC_TYPE_FENCE == module->all_sync.type) {
        /* if not communication has oclwrred during a fence epoch then we can enter a lock epoch
    ...
    ret = ompi_osc_pt2pt_lock_internal_exelwte (module, lock);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
