...
int ompi_osc_ucx_post(struct ompi_group_t *group, int assert, struct ompi_win_t *win) {
    ...
    if ((assert & MPI_MODE_NOCHECK) == 0) {
        ...
        for (i = 0; i < size; i++) {
            ...
            lwrr_idx = result & (OMPI_OSC_UCX_POST_PEER_MAX - 1);

            remote_addr = (module->state_info_array)[ranks_in_win_grp[i]].addr + OSC_UCX_STATE_POST_STATE_OFFSET + sizeof(uint64_t) * lwrr_idx;
