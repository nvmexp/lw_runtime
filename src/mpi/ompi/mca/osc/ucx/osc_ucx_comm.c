...
static inline int create_iov_list(const void *addr, int count, ompi_datatype_t *datatype,
    ...
    ret = opal_colwertor_copy_and_prepare_for_send(ompi_mpi_local_colwertor,
...
static inline int ddt_put_get(ompi_osc_ucx_module_t *module,
    ...
    if (!is_origin_contig && !is_target_contig) {
        ...
        while (origin_ucx_iov_idx < origin_ucx_iov_count) {
            lwrr_len = MIN(origin_ucx_iov[origin_ucx_iov_idx].len,
                           target_ucx_iov[target_ucx_iov_idx].len);

            if (!is_get) {
                status = ucp_put_nbi(ep, origin_ucx_iov[origin_ucx_iov_idx].addr, lwrr_len,
                ...
                status = ucp_get_nbi(ep, origin_ucx_iov[origin_ucx_iov_idx].addr, lwrr_len,
                                     remote_addr + (uint64_t)(target_ucx_iov[target_ucx_iov_idx].addr), rkey);
                if (status != UCS_OK && status != UCS_INPROGRESS) {
            ...
            origin_ucx_iov[origin_ucx_iov_idx].addr = (void *)((intptr_t)origin_ucx_iov[origin_ucx_iov_idx].addr + lwrr_len);
            target_ucx_iov[target_ucx_iov_idx].addr = (void *)((intptr_t)target_ucx_iov[target_ucx_iov_idx].addr + lwrr_len);

            origin_ucx_iov[origin_ucx_iov_idx].len -= lwrr_len;
            if (origin_ucx_iov[origin_ucx_iov_idx].len == 0) {
                origin_ucx_iov_idx++;
            }
            target_ucx_iov[target_ucx_iov_idx].len -= lwrr_len;
...
int ompi_osc_ucx_aclwmulate(const void *origin_addr, int origin_count,
                            struct ompi_datatype_t *origin_dt,
                            int target, ptrdiff_t target_disp, int target_count,
    ...
    if (op == &ompi_mpi_op_replace.op) {
        ...
        if (ompi_datatype_is_predefined(origin_dt) || is_origin_contig) {
            ...
            if ((op != &ompi_mpi_op_maxloc.op && op != &ompi_mpi_op_minloc.op) ||
                ...
                size_t temp_size;
                char *lwrr_temp_addr = (char *)temp_addr;
                ompi_datatype_type_size(temp_dt, &temp_size);
                while (origin_ucx_iov_idx < origin_ucx_iov_count) {
                    int lwrr_count = origin_ucx_iov[origin_ucx_iov_idx].len / temp_size;
                    ompi_op_reduce(op, origin_ucx_iov[origin_ucx_iov_idx].addr,
                                   lwrr_temp_addr, lwrr_count, temp_dt);
                    lwrr_temp_addr += lwrr_count * temp_size;
                    origin_ucx_iov_idx++;
                }
            } else {
                int i;
                void *lwrr_origin_addr = origin_ucx_iov[origin_ucx_iov_idx].addr;
                for (i = 0; i < (int)temp_count; i++) {
                    ompi_op_reduce(op, lwrr_origin_addr,
                                   (void *)((char *)temp_addr + i * temp_extent),
                                   1, temp_dt);
                    lwrr_origin_addr = (void *)((char *)lwrr_origin_addr + temp_extent);
                    origin_ucx_iov_idx++;
                    if (lwrr_origin_addr >= (void *)((char *)origin_ucx_iov[origin_ucx_iov_idx].addr + origin_ucx_iov[origin_ucx_iov_idx].len)) {
                        origin_ucx_iov_idx++;
                        lwrr_origin_addr = origin_ucx_iov[origin_ucx_iov_idx].addr;
                    }
                }
            }
...
int ompi_osc_ucx_fetch_and_op(const void *origin_addr, void *result_addr,
    ...
    if (op == &ompi_mpi_op_no_op.op || op == &ompi_mpi_op_replace.op ||
        ...
        return ompi_osc_ucx_get_aclwmulate(origin_addr, 1, dt, result_addr, 1, dt,
                                           target, target_disp, 1, dt, op, win);
    }
}

int ompi_osc_ucx_get_aclwmulate(const void *origin_addr, int origin_count,
                                struct ompi_datatype_t *origin_dt,
...
int ompi_osc_ucx_raclwmulate(const void *origin_addr, int origin_count,
                             struct ompi_datatype_t *origin_dt,
                             int target, ptrdiff_t target_disp, int target_count,
    ...
    ret = ompi_osc_ucx_aclwmulate(origin_addr, origin_count, origin_dt, target, target_disp,
...
int ompi_osc_ucx_rget_aclwmulate(const void *origin_addr, int origin_count,
    ...
    ret = ompi_osc_ucx_get_aclwmulate(origin_addr, origin_count, origin_datatype,
