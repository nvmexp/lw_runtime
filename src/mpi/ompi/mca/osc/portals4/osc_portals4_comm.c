                ...
                ptl_size_t       *ptl_iovec_count)
{
    ...
    OBJ_CONSTRUCT(&colwertor, opal_colwertor_t);
    ret = opal_colwertor_copy_and_prepare_for_send (ompi_mpi_local_colwertor, &datatype->super, count,
                                                    address, 0, &colwertor);
    ...
    *ptl_iovec_count = 0;
    ...
    do {
        ...
        done = opal_colwertor_raw (&colwertor, iov, &iov_count, &size);
             ...
             void             *user_ptr)
{
    ...
    if (module->origin_iovec_md_h != PTL_ILWALID_HANDLE) {
        PtlMDRelease(module->origin_iovec_md_h);
        free(module->origin_iovec_list);
        module->origin_iovec_md_h = PTL_ILWALID_HANDLE;
                  ...
                  void             *user_ptr)
{
    ...
    if (OMPI_SUCCESS != ret) {
        opal_output(ompi_osc_base_framework.framework_output,
                "datatype is not lwrrently supported");
        return OMPI_ERR_NOT_SUPPORTED;
    }
    ret = ompi_osc_portals4_get_op(op, &ptl_op);
    if (OMPI_SUCCESS != ret) {
        opal_output(ompi_osc_base_framework.framework_output,
                "operation is not lwrrently supported");
        return OMPI_ERR_NOT_SUPPORTED;
    }
              ...
              void             *user_ptr)
{
    ...
    if (module->result_iovec_md_h != PTL_ILWALID_HANDLE) {
        PtlMDRelease(module->result_iovec_md_h);
        free(module->result_iovec_list);
        module->result_iovec_md_h = PTL_ILWALID_HANDLE;
                 ...
                 void             *user_ptr)
{
    ...
    opal_colwertor_t origin_colwertor, target_colwertor;
    ...
    OBJ_CONSTRUCT(&origin_colwertor, opal_colwertor_t);
    ret = opal_colwertor_copy_and_prepare_for_send (ompi_mpi_local_colwertor, &origin_datatype->super, origin_count,
                                                    (void*)origin_address, 0, &origin_colwertor);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    do {
        ...
        /* opal_colwertor_raw returns done when it has reached the end of the data */
        done = opal_colwertor_raw (&target_colwertor, target_iovec, &target_iov_count, &target_size);
        ...
        while (target_iov_index != target_iov_count) {
            if (origin_iov_index == origin_iov_count) {
                ...
                origin_iov_index = 0;
                (void) opal_colwertor_raw (&origin_colwertor, origin_iovec, &origin_iov_count, &origin_size);
            }
    ...
    /* clean up colwertors */
    opal_colwertor_cleanup (&origin_colwertor);
    OBJ_DESTRUCT(&origin_colwertor);
                    ...
                    void             *user_ptr)
{
    ...
    opal_colwertor_t result_colwertor, origin_colwertor, target_colwertor;
    ...
    /* prepare colwertors for the result, source and target. these colwertors will be used to determine the
     * contiguous segments within the source and target. */
    OBJ_CONSTRUCT(&result_colwertor, opal_colwertor_t);
    ret = opal_colwertor_copy_and_prepare_for_send (ompi_mpi_local_colwertor, &result_datatype->super, result_count,
                                                    (void*)result_address, 0, &result_colwertor);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    do {
        ...
        while (target_iov_index != target_iov_count) {
            if (result_iov_index == result_iov_count) {
                ...
                result_iov_index = 0;
                (void) opal_colwertor_raw (&result_colwertor, result_iovec, &result_iov_count, &result_size);
            }
            if (origin_iov_index == origin_iov_count) {
                /* decode segments of the target buffer */
                origin_iov_count = OSC_PORTALS4_IOVEC_MAX;
...
int
ompi_osc_portals4_raclwmulate(const void *origin_addr,
                              int origin_count,
                              struct ompi_datatype_t *origin_dt,
                              int target,
                              ptrdiff_t target_disp,
                              ...
                              struct ompi_request_t **ompi_req)
{
    ...
    OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                         "raclwmulate: 0x%lx, %d, %s, %d, %lu, %d, %s, %s 0x%lx",
                         (unsigned long) origin_addr, origin_count,
                         origin_dt->name, target, (unsigned long) target_disp,
                         target_count, target_dt->name,
                         op->o_name,
...
int
ompi_osc_portals4_rget_aclwmulate(const void *origin_addr,
                                  int origin_count,
                                  struct ompi_datatype_t *origin_dt,
                                  void *result_addr,
                                  int result_count,
                                  ...
                                  struct ompi_request_t **ompi_req)
{
    ...
    OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                         "rget_aclwmulate: 0x%lx, %d, %s, 0x%lx, %d, %s, %d, %lu, %d, %s, %s, 0x%lx",
                         (unsigned long) origin_addr, origin_count,
    ...
    *ompi_req = &request->super;
    ...
    if (target_count > 0 && !ompi_datatype_is_contiguous_memory_layout(target_dt, target_count)) {
        if (MPI_REPLACE == op) {
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "rget_aclwmulate: MPI_REPLACE  non-contiguous target"));
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "rget_aclwmulate: MPI_NO_OP  non-contiguous target"));
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "rget_aclwmulate: other-op  non-contiguous target"));
        ...
        if (MPI_REPLACE == op) {
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "rget_aclwmulate: MPI_REPLACE  non-contiguous origin/result"));
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "rget_aclwmulate: MPI_NO_OP  non-contiguous origin/result"));
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "rget_aclwmulate: other-op  non-contiguous origin/result"));
        ...
        if (MPI_REPLACE == op) {
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "rget_aclwmulate: MPI_REPLACE  contiguous"));
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "rget_aclwmulate: MPI_NO_OP  contiguous"));
            ptl_size_t md_offset;
            ...
            OPAL_OUTPUT_VERBOSE((90,ompi_osc_base_framework.framework_output,
                                  "%s,%d MPI_Get_aclwmulate", __FUNCTION__, __LINE__));
            ret = segmentedGet(&module->opcount,
                               module->req_md_h,
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "rget_aclwmulate: other-op  contiguous"));
            ptl_size_t result_md_offset, origin_md_offset;
...
int
ompi_osc_portals4_aclwmulate(const void *origin_addr,
                             int origin_count,
                             struct ompi_datatype_t *origin_dt,
                             int target,
                             ptrdiff_t target_disp,
...
int
ompi_osc_portals4_get_aclwmulate(const void *origin_addr,
                                 int origin_count,
                                 struct ompi_datatype_t *origin_dt,
                                 void *result_addr,
                                 int result_count,
                                 ...
                                 struct ompi_win_t *win)
{
    ...
    OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                         "get_aclwmulate: 0x%lx, %d, %s, 0x%lx, %d, %s, %d, %lu, %d, %s, %s, 0x%lx",
                         (unsigned long) origin_addr, origin_count,
                         origin_dt->name, (unsigned long) result_addr,
                         result_count, result_dt->name,
                         target, (unsigned long) target_disp,
    ...
    if (target_count > 0 && !ompi_datatype_is_contiguous_memory_layout(target_dt, target_count)) {
        if (MPI_REPLACE == op) {
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "get_aclwmulate: MPI_REPLACE  non-contiguous target"));
            ret = swap_from_noncontig(module,
                                      module->md_h,
                                      result_addr,
                                      result_count,
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "get_aclwmulate: MPI_NO_OP  non-contiguous target"));
            ret = atomic_get_from_noncontig(module,
                                            module->md_h,
                                            result_addr,
                                            result_count,
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "get_aclwmulate: other-op  non-contiguous target"));
            ret = fetch_atomic_from_noncontig(module,
                                              module->md_h,
                                              result_addr,
                                              result_count,
        ...
        if (MPI_REPLACE == op) {
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "get_aclwmulate: MPI_REPLACE  non-contiguous origin/result"));
            ret = swap_to_iovec(module,
                                result_addr,
                                result_count,
                                result_dt,
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "get_aclwmulate: MPI_NO_OP  non-contiguous origin/result"));
            ret = atomic_get_to_iovec(module,
                                      result_addr,
                                      result_count,
                                      result_dt,
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "get_aclwmulate: other-op  non-contiguous origin/result"));
            ret = fetch_atomic_to_iovec(module,
                                        result_addr,
                                        result_count,
                                        result_dt,
        ...
        if (MPI_REPLACE == op) {
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "get_aclwmulate: MPI_REPLACE  contiguous"));
            ptl_size_t result_md_offset, origin_md_offset;
            ...
            if (OMPI_SUCCESS != ret) {
                opal_output(ompi_osc_base_framework.framework_output,
                        "MPI_Get_aclwmulate: datatype is not lwrrently supported");
                return OMPI_ERR_NOT_SUPPORTED;
            }
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "get_aclwmulate: MPI_NO_OP  contiguous"));
            ptl_size_t md_offset;
            ...
            OPAL_OUTPUT_VERBOSE((50, ompi_osc_base_framework.framework_output,
                                 "get_aclwmulate: other-op  contiguous"));
            ptl_size_t result_md_offset, origin_md_offset;
            ...
            if (OMPI_SUCCESS != ret) {
                opal_output(ompi_osc_base_framework.framework_output,
                        "MPI_Get_aclwmulate: operation is not lwrrently supported");
                return OMPI_ERR_NOT_SUPPORTED;
            }
                                   ...
                                   struct ompi_win_t *win)
{
    ...
    if (OMPI_SUCCESS != ret) {
        opal_output(ompi_osc_base_framework.framework_output,
                "MPI_Compare_and_swap: datatype is not lwrrently supported");
        return OMPI_ERR_NOT_SUPPORTED;
    }
                               ...
                               struct ompi_win_t *win)
{
    ...
    if (OMPI_SUCCESS != ret) {
        opal_output(ompi_osc_base_framework.framework_output,
                "MPI_Fetch_and_op: datatype is not lwrrently supported");
        return OMPI_ERR_NOT_SUPPORTED;
    }
    ...
    if (MPI_REPLACE == op) {
        ...
        if (OMPI_SUCCESS != ret) {
            opal_output(ompi_osc_base_framework.framework_output,
                    "MPI_Fetch_and_op: operation is not lwrrently supported");
            return OMPI_ERR_NOT_SUPPORTED;
        }
