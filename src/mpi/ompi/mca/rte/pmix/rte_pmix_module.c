...
opal_process_name_t pmix_name_ilwalid = {UINT32_MAX, UINT32_MAX};
...
#define OPAL_SCHEMA_ILWALID_CHAR        '$'
#define OPAL_SCHEMA_ILWALID_STRING      "$"
...
int ompi_rte_colwert_string_to_process_name(opal_process_name_t *name,
                                            const char* name_string)
{
    ...
    /* set default */
    name->jobid = pmix_name_ilwalid.jobid;
    name->vpid = pmix_name_ilwalid.vpid;
    ...
    *token = '\0';
    ...
    } else if (0 == strcmp(temp, OPAL_SCHEMA_ILWALID_STRING)) {
        job = pmix_name_ilwalid.jobid;
    } else {
    ...
    } else if (0 == strcmp(token, OPAL_SCHEMA_ILWALID_STRING)) {
        vpid = pmix_name_ilwalid.vpid;
    } else {
...
int ompi_rte_colwert_process_name_to_string(char** name_string,
                                            const opal_process_name_t *name)
{
    ...
    } else if (pmix_name_ilwalid.jobid == name->jobid) {
        asprintf(&tmp, "%s", OPAL_SCHEMA_ILWALID_STRING);
    } else {
    ...
    } else if (pmix_name_ilwalid.vpid == name->vpid) {
        asprintf(&tmp2, "%s%c%s", tmp, OPAL_SCHEMA_DELIMITER_CHAR, OPAL_SCHEMA_ILWALID_STRING);
    } else {
...
static int ompi_pmix_colwert_string_to_jobid(opal_jobid_t *jobid, const char* jobidstring)
{
    if (NULL == jobidstring) {  /* got an error */
        OPAL_ERROR_LOG(OPAL_ERR_BAD_PARAM);
        *jobid = OPAL_JOBID_ILWALID;
        return OPAL_ERR_BAD_PARAM;
    }
    ...
    if (0 == strcmp(OPAL_SCHEMA_ILWALID_STRING, jobidstring)) {
        *jobid = OPAL_JOBID_ILWALID;
        return OPAL_SUCCESS;
    }
...
static int _colwert_string_to_process_name(opal_process_name_t *name,
                                           const char* name_string)
{
    return ompi_rte_colwert_string_to_process_name(name, name_string);
}

static int _colwert_process_name_to_string(char** name_string,
                                          const opal_process_name_t *name)
{
    return ompi_rte_colwert_process_name_to_string(name_string, name);
}

static int
_colwert_string_to_jobid(opal_jobid_t *jobid, const char *jobid_string)
{
    return ompi_pmix_colwert_string_to_jobid(jobid, jobid_string);
}
...
int ompi_rte_init(int *pargc, char ***pargv)
{
    ...
    /* Colwince OPAL to use our naming scheme */
    opal_process_name_print = _process_name_print_for_opal;
    opal_vpid_print = _vpid_print_for_opal;
    opal_jobid_print = _jobid_print_for_opal;
    opal_compare_proc = _process_name_compare;
    opal_colwert_string_to_process_name = _colwert_string_to_process_name;
    opal_colwert_process_name_to_string = _colwert_process_name_to_string;
    opal_snprintf_jobid = ompi_pmix_snprintf_jobid;
    opal_colwert_string_to_jobid = _colwert_string_to_jobid;
     ...
     */
    if (NULL == getelw(OPAL_MCA_PREFIX"opal_ess_num_procs")) {
     ...
     */
    if (NULL == getelw(OPAL_MCA_PREFIX"opal_precondition_transports")) {
        ...
        asprintf(&elwar, OPAL_MCA_PREFIX"opal_precondition_transports=%s", string_key);
        putelw(elwar);
        added_transport_keys = true;
        /* cannot free the elwar as that messes up our elwiron */
        free(string_key);
    }
        ...
        for (i=0; NULL != peers[i]; i++) {
            ...
            if (pname.vpid == pmix_process_info.my_name.vpid) {
                ...
                if (OPAL_SUCCESS == ret && NULL != val) {
                    ...
                    u16 = OPAL_PROC_ON_CLUSTER | OPAL_PROC_ON_LW | OPAL_PROC_ON_NODE;
                }
            }
    ...
    if (NULL != getelw("SLURM_CPU_BIND_TYPE")) {
...
int ompi_rte_finalize(void)
{
    ...
    if (added_transport_keys) {
        unsetelw(OPAL_MCA_PREFIX"opal_precondition_transports");
    }
    if (added_num_procs) {
        unsetelw(OPAL_MCA_PREFIX"opal_ess_num_procs");
    }
    if (added_app_ctx) {
...
void ompi_rte_wait_for_debugger(void)
{
    ...
    if (!debugger && NULL == getelw("PMIX_TEST_DEBUGGER_ATTACH")) {
        /* if not, just return */
        return;
    }
    ...
    if (NULL != (evar = getelw("PMIX_TEST_DEBUGGER_SLEEP"))) {
...
static int _setup_job_session_dir(char **sdir)
{
    ...
    if( NULL == (tmpdir = getelw("TMPDIR")) )
        if( NULL == (tmpdir = getelw("TEMP")) )
            if( NULL == (tmpdir = getelw("TMP")) )
                tmpdir = "/tmp";
