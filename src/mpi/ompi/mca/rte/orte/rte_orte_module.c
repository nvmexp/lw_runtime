...
void ompi_rte_wait_for_debugger(void)
{
    ...
    if (!debugger && NULL == getelw("ORTE_TEST_DEBUGGER_ATTACH")) {
        /* if not, just return */
        return;
    }
    ...
    if (NULL != (evar = getelw("ORTE_TEST_DEBUGGER_SLEEP"))) {
...
bool ompi_rte_connect_accept_support(const char *port)
{
    ...
    *ptr = '\0';
    if (ORTE_SUCCESS != orte_util_colwert_string_to_process_name(&name, tmp)) {
        free(tmp);
        orte_show_help("help-orterun.txt", "orterun:malformedport", true);
        return false;
    }
