...
/* Keep tracks of how many components are lwrrently using the common part */
...
int mca_common_monitoring_lwrrent_state = 0;
...
static char* mca_common_monitoring_lwrrent_filename = NULL;
...
static int mca_common_monitoring_set_flush(struct mca_base_pvar_t *pvar,
                                           const void *value, void *obj)
{
    if( NULL != mca_common_monitoring_lwrrent_filename ) {
        free(mca_common_monitoring_lwrrent_filename);
    }
    if( NULL == *(char**)value || 0 == strlen((char*)value) ) {  /* No more output */
        mca_common_monitoring_lwrrent_filename = NULL;
    } else {
        mca_common_monitoring_lwrrent_filename = strdup((char*)value);
        if( NULL == mca_common_monitoring_lwrrent_filename )
            return OMPI_ERROR;
    }
    return OMPI_SUCCESS;
                                              ...
                                              void *obj, int *count)
{
    ...
    case MCA_BASE_PVAR_HANDLE_BIND:
        ...
        *count = (NULL == mca_common_monitoring_lwrrent_filename
                  ? 0 : strlen(mca_common_monitoring_lwrrent_filename));
    case MCA_BASE_PVAR_HANDLE_UNBIND:
        return OMPI_SUCCESS;
    case MCA_BASE_PVAR_HANDLE_START:
        mca_common_monitoring_lwrrent_state = mca_common_monitoring_enabled;
    ...
    case MCA_BASE_PVAR_HANDLE_STOP:
        return mca_common_monitoring_flush(3, mca_common_monitoring_lwrrent_filename);
    }
    return OMPI_ERROR;
}
                                                  ...
                                                  int *count)
{
    ...
    case MCA_BASE_PVAR_HANDLE_START:
        ...
        return OMPI_SUCCESS;
    case MCA_BASE_PVAR_HANDLE_STOP:
        mca_common_monitoring_lwrrent_state = 0;
        return OMPI_SUCCESS;
    }
...
void mca_common_monitoring_finalize( void )
{
    ...
    mca_common_monitoring_flush(mca_common_monitoring_output_enabled,
                                mca_common_monitoring_lwrrent_filename);
...
void mca_common_monitoring_register(void*pml_monitoring_component)
{
    ...
    if( NULL != mca_common_monitoring_initial_filename )
        mca_common_monitoring_lwrrent_filename = strdup(mca_common_monitoring_initial_filename);
...
{
    if( 0 == mca_common_monitoring_lwrrent_state ) return;  /* right now the monitoring is not started */
...
static int mca_common_monitoring_flush(int fd, char* filename)
{
    ...
    if( 0 == mca_common_monitoring_lwrrent_state || 0 == fd ) /* if disabled do nothing */
        return OMPI_SUCCESS;
