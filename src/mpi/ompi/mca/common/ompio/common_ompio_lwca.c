...
#include "common_ompio_lwda.h"


static opal_mutex_t     mca_common_ompio_lwda_mutex;      /* lock for thread safety */
...
static int32_t  mca_common_ompio_lwda_init = 0;
static int32_t  mca_common_ompio_pagesize=4096;
static void* mca_common_ompio_lwda_alloc_seg ( void *ctx, size_t *size );
static void mca_common_ompio_lwda_free_seg ( void *ctx, void *buf );
				      ...
				      int *is_managed)
{
    ...
    *is_managed=0;
    ...
    colwertor.flags=0;
    if ( opal_lwda_check_one_buf ( (char *)buf, &colwertor ) ) {
        *is_gpu = 1;
        if ( colwertor.flags & COLWERTOR_LWDA_UNIFIED ){
            *is_managed =1;
        }
    } 
...
static void* mca_common_ompio_lwda_alloc_seg ( void*ctx, size_t *size )
{
    ...
    buf = malloc ( realsize);
    if ( NULL != buf ) {
        mca_common_lwda_register ( ( char *)buf, realsize, NULL  );
    }
    *size = realsize;
    return buf;
}
...
{
    if ( NULL != buf ) {
        mca_common_lwda_unregister ( (char *) buf, NULL );
        free ( buf );
    }
    return;
}
...
int mca_common_ompio_lwda_alloc_init ( void )
{
    ...
    if(OPAL_THREAD_ADD_FETCH32(&mca_common_ompio_lwda_init, 1) > 1)
        return OMPI_SUCCESS;

    /* initialize static objects */
    OBJ_CONSTRUCT(&mca_common_ompio_lwda_mutex, opal_mutex_t);

    OPAL_THREAD_LOCK (&mca_common_ompio_lwda_mutex );
    /* lookup name of the allocator to use */
    if(NULL == (mca_common_ompio_allocator_component = mca_allocator_component_lookup("basic"))) {
        OPAL_THREAD_UNLOCK(&mca_common_ompio_lwda_mutex);
    ...
    mca_common_ompio_allocator = mca_common_ompio_allocator_component->allocator_init(thread_safe, 
                                                                                      mca_common_ompio_lwda_alloc_seg, 
                                                                                      mca_common_ompio_lwda_free_seg, 
                                                                                      NULL);
...
int mca_common_ompio_lwda_alloc_fini ( void )
{
    if ( NULL != mca_common_ompio_allocator ) {
        ...
        OPAL_THREAD_UNLOCK (&mca_common_ompio_lwda_mutex);
        OBJ_DESTRUCT (&mca_common_ompio_lwda_mutex);
    }
...
void *mca_common_ompio_alloc_buf ( ompio_file_t *fh, size_t bufsize )
{
    ...
    if ( !mca_common_ompio_lwda_init ){
        mca_common_ompio_lwda_alloc_init ();
    }
