...
//      opal_output(10, "Improper use of FILE Mode, Using WRONLY for Read!\n");
    ...
    if ( is_gpu && !is_managed ) {
        ...
        OMPIO_LWDA_PREPARE_BUF(fh,buf,count,datatype,tbuf,&colwertor,max_data,decoded_iov,iov_count);        
    ...
    if ( is_gpu && !is_managed ) {
        ...
        opal_colwertor_unpack (&colwertor, decoded_iov, &iov_count, &pos );
...
//      opal_output(10, "Improper use of FILE Mode, Using WRONLY for Read!\n");
    ...
    if ( NULL != fh->f_fbtl->fbtl_ipreadv ) {
        ...
        if ( is_gpu && !is_managed ) {
            ...
            OMPIO_LWDA_PREPARE_BUF(fh,buf,count,datatype,tbuf,&ompio_req->req_colwertor,max_data,decoded_iov,iov_count);        
