                                     ...
                                     mca_common_ompio_contg *contg_groups)
{
    ...
    ** file partitioning strategy in the paper. Note, that the formulas lwrrently only model
    ...
    ** value relative to the last number tested. The aggresivnes of lwtting of
    ** the increasie in the number of aggregators is controlled by the new mca
    ** parameter mca_io_ompio_aggregator_lwtoff_threshold.  Lower values for
    ...
    */
    dtime_threshold = (double) OMPIO_MCA_GET(fh, aggregators_lwtoff_threshold) / 100.0;
				          ...
				          mca_common_ompio_contg *contg_groups)
{
    ...
    //Callwlate contg chunk size and contg subgroups
		                   ...
		                   size_t bytes_per_proc)
{
    ...
    //Callwlate final number of aggregators
				      ...
				      int *ompio_grouping_flag)
{
    ...
    *end_offsets = &end_offsets_tmp[0];
    ...
    //Merge only if the groups are conselwtive
    if(*is_aggregator == 1){
    ...
    *aggr_bytes_per_group = &aggr_bytes_per_group_tmp[0];
    //Go through the decision list to see if non conselwtive
...
** unexpected jumps in the exelwtion time. Using float leads to 
