...
static mca_common_ompio_generate_lwrrent_file_view_fn_t generate_lwrrent_file_view_fn;
                                ...
                                ompio_file_t *ompio_fh, bool use_sharedfp)
{
    ...
    ompio_fh->f_colwertor = opal_colwertor_create (remote_arch, 0);
    ...
    ompio_fh->f_generate_lwrrent_file_view=generate_lwrrent_file_view_fn;
    ...
    if ( ompio_fh->f_amode & MPI_MODE_APPEND ) {
        OMPI_MPI_OFFSET_TYPE lwrrent_size;
        mca_sharedfp_base_module_t * shared_fp_base_module;

        ompio_fh->f_fs->fs_file_get_size( ompio_fh,
                                          &lwrrent_size);
        mca_common_ompio_set_explicit_offset (ompio_fh, lwrrent_size);
        if ( true == use_sharedfp ) {
            if ( NULL != ompio_fh->f_sharedfp ) {
                shared_fp_base_module = ompio_fh->f_sharedfp;
                ret = shared_fp_base_module->sharedfp_seek(ompio_fh,lwrrent_size, MPI_SEEK_SET);
                if ( MPI_SUCCESS != ret  ) {
...
int mca_common_ompio_file_close (ompio_file_t *ompio_fh)
{
	...
	*/
    ...
    if (NULL != ompio_fh->f_colwertor) {
        free (ompio_fh->f_colwertor);
        ompio_fh->f_colwertor = NULL;
    }
                                      ...
                                      uint32_t *iovec_count)
{
    ...
    opal_colwertor_clone (fh->f_colwertor, &colwertor, 0);

    if (OMPI_SUCCESS != opal_colwertor_prepare_for_send (&colwertor,
                                                         &(datatype->super),
                                                         count,
                                                         buf)) {
        opal_output (1, "Cannot attach the datatype to a colwertor\n");
        return OMPI_ERROR;
    }
	...
	*iov = NULL;
    ...
    while (0 == opal_colwertor_raw(&colwertor,
				   temp_iov,
                                   &temp_count,
                                   &temp_data)) {
#if 0
...
int mca_common_ompio_set_callbacks(mca_common_ompio_generate_lwrrent_file_view_fn_t generate_lwrrent_file_view,
                                   mca_common_ompio_get_mca_parameter_value_fn_t get_mca_parameter_value)
{
    generate_lwrrent_file_view_fn = generate_lwrrent_file_view;
