...
typedef struct mca_io_ompio_aggregator_data {
    ...
    int lwrrent_index, lwrrent_position;
                                      ...
                                      ompi_status_public_t *status)
{
    ...
    ompi_request_t **lwrr_reqs=NULL,**prev_reqs=NULL;
    ...
    */
     ...
     ********************************************************************/
    ret = fh->f_generate_lwrrent_file_view( (struct ompio_file_t *) fh,
					    max_data,
					    &local_iov_array,
					    &local_count);
    if (ret != OMPI_SUCCESS){
    ...
    if ( cycles > 0 ) {
        for ( i=0; i<dynamic_gen2_num_io_procs; i++ ) {
            ret = shuffle_init ( 0, cycles, aggregators[i], fh->f_rank, aggr_data[i], 
                                 &lwrr_reqs[i*(fh->f_procs_per_group + 1)] );
            if ( OMPI_SUCCESS != ret ) {
                goto exit;
            }
        }
    ...
    for (index = 1; index < cycles; index++) {
        SWAP_REQUESTS(lwrr_reqs,prev_reqs);
                          ...
                          ompi_request_t **reqs )
{
    ...
    /*****************************************************************
     *** 7c. Callwlate how much data will be contributed in this cycle
     ***     by each process
     *****************************************************************/
    
    /* The blocklen and displs callwlation only done at aggregators!*/
    while (data->bytes_to_write_in_cycle) {
        
        /* This next block identifies which process is the holder
        ** of the sorted[lwrrent_index] element;
        */
        blocks = data->fview_count[0];
        for (j=0 ; j<data->procs_per_group ; j++) {
            if (data->sorted[data->lwrrent_index] < blocks) {
                data->n = j;
                break;
            }
            else {
        ...
        if (data->bytes_remaining) {
            ...
            if (data->bytes_remaining <= data->bytes_to_write_in_cycle) {
                ...
                if (aggregator == rank) {
                    ...
                    data->displs_per_process[data->n][data->disp_index[data->n] - 1] =
                        (ptrdiff_t)data->global_iov_array[data->sorted[data->lwrrent_index]].iov_base +
                        (data->global_iov_array[data->sorted[data->lwrrent_index]].iov_len
            ...
            if (data->bytes_to_write_in_cycle <
                (MPI_Aint) data->global_iov_array[data->sorted[data->lwrrent_index]].iov_len) {
                ...
                data->bytes_remaining = data->global_iov_array[data->sorted[data->lwrrent_index]].iov_len -
                    data->bytes_to_write_in_cycle;
                data->bytes_to_write_in_cycle = 0;
                break;
            }
                ...
                if (aggregator == rank) {
                    ...
                    data->displs_per_process[data->n][data->disp_index[data->n] - 1] = (ptrdiff_t)
                        data->global_iov_array[data->sorted[data->lwrrent_index]].iov_base;
                ...
                if (data->procs_in_group[data->n] == rank) {
                    bytes_sent += data->global_iov_array[data->sorted[data->lwrrent_index]].iov_len;
                }
                data->bytes_to_write_in_cycle -=
                    data->global_iov_array[data->sorted[data->lwrrent_index]].iov_len;
                data->lwrrent_index ++;
    ...
    /*************************************************************************
     *** 7d. Callwlate the displacement on where to put the data and allocate
        ...
        while (remaining) {
            ...
            if(0 == block_index) {
                ...
                displs_proc[block_index] = (ptrdiff_t) (data->decoded_iov[data->iov_index].iov_base) +
                                                        data->lwrrent_position - send_mem_address;
            }

            if (remaining >=
                (data->decoded_iov[data->iov_index].iov_len - data->lwrrent_position)) {
                ...
                data->lwrrent_position = 0;
            }
            else {
                blocklength_proc[block_index] = remaining;
                data->lwrrent_position += remaining;
                remaining = 0;
            }
        }
                                        ...
                                        int stripe_count, int stripe_size)
{
    ...
    /* Step 2: recallwlating the total lengths per aggregator */
    for ( i=0; i< stripe_count; i++ ) {
			    ...
			    int *sorted)
{
    ...
    /* num_entries can be a large no. so NO RELWRSION */
