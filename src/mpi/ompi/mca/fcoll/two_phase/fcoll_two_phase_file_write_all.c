...

{
    ...
    int *send_buf_idx=NULL, *lwrr_to_proc=NULL, *done_to_proc=NULL;
    ...
    lwrr_offlen_ptr = (int *) calloc(fh->f_size, sizeof(int));
    ...
    lwrr_to_proc = (int *) malloc(fh->f_size*sizeof(int));

    if ( NULL == lwrr_to_proc ){
        ret = OMPI_ERR_OUT_OF_RESOURCE;
        goto exit;
    }
				      ...
				      int *aggregator_list)
{
	...
	while (rem_len != 0) {
		...
		if (lwrr_to_proc[p]+len > done_to_proc[p]) {
			...
			size = OMPIO_MIN(lwrr_to_proc[p] + len -
