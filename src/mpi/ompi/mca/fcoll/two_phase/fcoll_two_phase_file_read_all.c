...
static int two_phase_read_and_exch(ompio_file_t *fh,
    ...
    int m = 0;
    int *lwrr_offlen_ptr=NULL, *count=NULL, *send_size=NULL, *recv_size=NULL;
    ...
    OMPI_MPI_OFFSET_TYPE for_lwrr_iter=0;
    ...
    lwrr_offlen_ptr = (int *)calloc (fh->f_size,
				     sizeof(int));
    if (NULL == lwrr_offlen_ptr){
	ret = OMPI_ERR_OUT_OF_RESOURCE;
	goto exit;
    }
...
static void two_phase_fill_user_buffer(ompio_file_t *fh,
    ...
    unsigned *lwrr_from_proc=NULL, *done_from_proc=NULL, *recv_buf_idx=NULL;

    lwrr_from_proc = (unsigned *) malloc (fh->f_size * sizeof(unsigned));
	...
	while (rem_len != 0) {
		...
		if (lwrr_from_proc[p]+len > done_from_proc[p]) {
			...
			size = OMPIO_MIN(lwrr_from_proc[p] + len -
