                                  ...
                                  ompi_status_public_t *status)
{
    ...
    int lwrrent_index = 0, temp_index=0;
	 ...
	 ***     by each process
	    ...
	    */
            ...
            for (j=0 ; j<fh->f_procs_per_group ; j++) {
                if (sorted[lwrrent_index] < blocks) {
                    n = j;
                    break;
                }
                else {
            ...
            if (bytes_remaining) {
                ...
                if (bytes_remaining <= bytes_to_write_in_cycle) {
                    ...
                    if (my_aggregator == fh->f_rank) {
                        ...
                        displs_per_process[n][disp_index[n] - 1] =
                            (ptrdiff_t)global_iov_array[sorted[lwrrent_index]].iov_base +
                            (global_iov_array[sorted[lwrrent_index]].iov_len
                             - bytes_remaining);
		       ...
		       bytes_to_write_in_cycle, so we have to segment again */
                ...
                if (bytes_to_write_in_cycle <
                    (MPI_Aint) global_iov_array[sorted[lwrrent_index]].iov_len) {
                    ...
                    bytes_remaining = global_iov_array[sorted[lwrrent_index]].iov_len -
                        bytes_to_write_in_cycle;
                    bytes_to_write_in_cycle = 0;
                    break;
                }
                    ...
                    if (my_aggregator == fh->f_rank) {
                        ...
                        displs_per_process[n][disp_index[n] - 1] = (ptrdiff_t)
                            global_iov_array[sorted[lwrrent_index]].iov_base;
                    ...
                    if (fh->f_procs_in_group[n] == fh->f_rank) {
                        bytes_sent += global_iov_array[sorted[lwrrent_index]].iov_len;
                    }
                    bytes_to_write_in_cycle -=
                        global_iov_array[sorted[lwrrent_index]].iov_len;
                    lwrrent_index ++;
        ...
        if ( sendbuf_is_contiguous ) {
            ...
            while (remaining) {
                mem_address = (ptrdiff_t)
                    (decoded_iov[iov_index].iov_base) + lwrrent_position;

                if (remaining >=
                    (decoded_iov[iov_index].iov_len - lwrrent_position)) {
                    ...
                    memcpy (send_buf+temp_position,
                            (IOVBASE_TYPE *) mem_address,
                            remaining);
                    lwrrent_position = lwrrent_position + remaining;
                    remaining = 0;
                }
            }
	}
