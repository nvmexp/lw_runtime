...
int ompi_coll_tuned_allreduce_intra_check_forced_init (coll_tuned_force_algorithm_mca_param_indices_t *mca_param_indices)
{
    ...
    mca_param_indices->algorithm_param_index =
        mca_base_component_var_register(&mca_coll_tuned_component.super.collm_version,
                                        "allreduce_algorithm",
                                        "Which allreduce algorithm is used. Can be locked down to any of: 0 ignore, 1 basic linear, 2 nonoverlapping (tuned reduce + tuned bcast), 3 relwrsive doubling, 4 ring, 5 segmented ring",
                                            ...
                                            int algorithm, int faninout, int segsize)
{
    ...
    case (3):
        return ompi_coll_base_allreduce_intra_relwrsivedoubling(sbuf, rbuf, count, dtype, op, comm, module);
    case (4):
