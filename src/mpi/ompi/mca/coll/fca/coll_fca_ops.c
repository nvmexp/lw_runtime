...
{
    mca_coll_fca_module_t *fca_module = (mca_coll_fca_module_t*)module;
    MCA_COLL_FCA_DECLARE_COLWERTOR(colw);
    ...
    if (mca_coll_fca_array_size(datatype, count, &gap, &size)) {
        ...
        mca_coll_fca_colwertor_create(&colw, datatype, count, buff,
    ...
    if (spec.size > fca_module->fca_comm_caps.max_payload) {
         FCA_VERBOSE(5, "Unsupported bcast operation size %d, using fallback",
                     spec.size);
         if (spec.buf != buff) {
             mca_coll_fca_colwertor_destroy(&colw);
         }
         goto orig_bcast;
    }
    ...
    if (spec.buf != buff && root == fca_module->rank) {
        mca_coll_fca_colwertor_process(&colw, 0);
    }
    ...
    /* Destroy colwertor if operation failed */
    if (ret < 0) {
        mca_coll_fca_colwertor_destroy(&colw);
        if (ret == -EUSEMPI) {
            goto orig_bcast;
    ...
    /* Unpack data and clean up colwertor */
...
static size_t __setup_gather_sendbuf(void *sbuf, int scount,
                                     ...
                                     struct mca_coll_fca_colwertor *scolw,
                                     void **real_sendbuf)
{
    size_t gap, ssize;
    ...
    if (mca_coll_fca_array_size(sdtype, scount, &gap, &ssize)) {
        *real_sendbuf = (char*) sbuf + gap;
        ...
        mca_coll_fca_colwertor_create(scolw, sdtype, scount, sbuf,
                                      MCA_COLL_FCA_COLW_SEND, real_sendbuf,
                                      &ssize);
        mca_coll_fca_colwertor_process(scolw, 0);
    }
    return ssize;
}
                                             ...
                                             void **real_sendbuf)
{
    ...
    if (mca_coll_fca_array_size(rdtype, rcount, &gap, &ssize)) {
        *real_sendbuf = (char*) inplace_sbuf + gap;
        ...
        FCA_VERBOSE(5, "Packing send buffer");
        mca_coll_fca_colwertor_create(scolw, rdtype, rcount, inplace_sbuf,
                                      MCA_COLL_FCA_COLW_SEND, real_sendbuf,
                                      &ssize);
        mca_coll_fca_colwertor_process(scolw, 0);
    }
                           ...
                           mca_coll_base_module_t *module)
{
    ...
    MCA_COLL_FCA_DECLARE_COLWERTOR(scolw);
    MCA_COLL_FCA_DECLARE_COLWERTOR(rcolw);
    ...
    if(sbuf == MPI_IN_PLACE ) {
        ...
        spec.size = __setup_gather_sendbuf_inplace(
                        ...
                        rcount, rdtype, &scolw, &spec.sbuf);
    } else {
        spec.size = __setup_gather_sendbuf((void *)sbuf, scount, sdtype, &scolw, &spec.sbuf);
    }
    ...
    if (mca_coll_fca_array_size(rdtype, total_rcount, &rgap, &rsize) && rgap == 0) {
        ...
        mca_coll_fca_colwertor_create(&rcolw, rdtype, total_rcount, rbuf,
                                      MCA_COLL_FCA_COLW_RECV, &spec.rbuf, &rsize);
    }
    ...
    /* Destroy colwertors if operation failed */
    if (ret < 0) {
        mca_coll_fca_colwertor_destroy(&scolw);
        mca_coll_fca_colwertor_destroy(&rcolw);
        if (ret == -EUSEMPI) {
            goto orig_allgather;
        }
    ...
    /* Unpack data and clean up colwertor */
    mca_coll_fca_colwertor_destroy(&scolw);
    if (mca_coll_fca_colwertor_valid(&rcolw)) {
        FCA_VERBOSE(5, "Unpacking Allgather receive buffer");
        mca_coll_fca_colwertor_process(&rcolw, 0);
        mca_coll_fca_colwertor_destroy(&rcolw);
    }
    return OMPI_SUCCESS;
                           ...
                           mca_coll_base_module_t *module)
{
    ...
    if(sbuf == MPI_IN_PLACE ) {
        spec.sendsize = __setup_gather_sendbuf_inplace(
                                ...
                                rcounts[fca_module->rank], rdtype, &scolw, &spec.sbuf);
    } else {
        spec.sendsize = __setup_gather_sendbuf((void *)sbuf, scount, sdtype, &scolw, &spec.sbuf);
    }
    ...
    /* Callwlate the size of receive buffer */
    sum_rcounts = 0;
    for (i = 0; i < comm_size; ++i) {
        sum_rcounts += rcounts[i];
    }

    /* colwert MPI counts which depend on dtype) to FCA sizes (which are in bytes) */
    if (mca_coll_fca_array_size(rdtype, sum_rcounts, &rgap, &rsize) && rgap == 0) {
        ...
        mca_coll_fca_colwertor_create(&rcolw, rdtype, sum_rcounts, rbuf,
                                      MCA_COLL_FCA_COLW_RECV, &spec.rbuf, &rsize);
    ...
    if (mca_coll_fca_colwertor_valid(&rcolw)) {
        FCA_VERBOSE(5, "Unpacking AllgatherV receive buffer rdtype_extent=%ld",
                    rdtype_extent);
        for (i = 0; i < comm_size; ++i) {
            mca_coll_fca_colwertor_set(&rcolw, rdtype,
                                       (char*)rbuf + disps[i] * rdtype_extent,
                                       rcounts[i]);
            mca_coll_fca_colwertor_process(&rcolw, spec.displs[i]);
        }
        mca_coll_fca_colwertor_destroy(&rcolw);
    }
    return OMPI_SUCCESS;
