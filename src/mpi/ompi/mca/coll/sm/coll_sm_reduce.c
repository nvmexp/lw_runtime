                             ...
                             mca_coll_base_module_t *module)
{
    ...
    if ((int)size > mca_coll_sm_component.sm_control_size) {
        ...
        /* Lazily enable the module the first time we ilwoke a
           collective on it */
 ...
 * need a receive colwertor and receive each fragment into a temporary
 ...
 *   use the colwertor to copy the fragment out of shmem into a temp
                          ...
                          mca_coll_base_module_t *module)
{
    ...
    /* Figure out how much we should have the colwertor copy.  We need
    ...
    if (root == rank) {
        opal_colwertor_t rtb_colwertor, rbuf_colwertor;
        ...
        /* If the datatype is the same packed as it is unpacked, we
           ...
           receive colwertor and a temporary buffer to receive
           into. */
        ...
        if (ompi_datatype_is_contiguous_memory_layout(dtype, count)) {
             ...
             * two colwertors:
             *
             * rtb_colwertor: unpacking from the shmem to the
             * reduce_temp_buffer (where we can then apply the
             * reduction).
             *
             * rbuf_colwertor: unpacking from the shmem directly to the
             * rbuf (no need to go to the reduce_temp_buffer first and
             * then apply the reduction -- just copy straight to the
             * target buffer).
             */
            OBJ_CONSTRUCT(&rtb_colwertor, opal_colwertor_t);
            OBJ_CONSTRUCT(&rbuf_colwertor, opal_colwertor_t);
            ...
            if (OMPI_SUCCESS !=
                (ret = opal_colwertor_copy_and_prepare_for_recv(
                                       ompi_mpi_local_colwertor,
                                       &(dtype->super),
                                       segment_ddt_count,
                                       reduce_temp_buffer,
                                       0,
            ...
            /* See if we need the rbuf_colwertor */
            if (size - 1 != rank) {
                if (OMPI_SUCCESS !=
                    (ret = opal_colwertor_copy_and_prepare_for_recv(
                                       ompi_mpi_local_colwertor,
            ...
            do {
                ...
                /* Process (size-1) is not the root */
                    ...
                    /* If the datatype is noncontiguous, use the
                       rbuf_colwertor to unpack it straight to the
                       rbuf */
                    else {
                        max_data = segment_ddt_bytes;
                        COPY_FRAGMENT_OUT(rbuf_colwertor, size - 1, index,
                                          iov, max_data);
                    }
                }
                        ...
                        /* Otherwise, unpack the fragment to the temporary
                           buffer and then do the reduction from there */
                            ...
                            /* Unpack the fragment into my temporary
                               buffer */
                            max_data = segment_ddt_bytes;
                            COPY_FRAGMENT_OUT(rtb_colwertor, peer, index,
                                              iov, max_data);
                            opal_colwertor_set_position(&rtb_colwertor, &zero);
        ...
        /* Kill the colwertor, if we had one */

        if (NULL != free_buffer) {
            OBJ_DESTRUCT(&rtb_colwertor);
            OBJ_DESTRUCT(&rbuf_colwertor);
            free(free_buffer);
        }
        if (NULL != inplace_temp) {
            free(inplace_temp);
     ...
     *********************************************************************/
        ...
        /* Here we get a colwertor for the full count that the user
           provided (as opposed to the colwertor that the root got) */

        opal_colwertor_t sbuf_colwertor;
        OBJ_CONSTRUCT(&sbuf_colwertor, opal_colwertor_t);
        if (OMPI_SUCCESS !=
            (ret =
             opal_colwertor_copy_and_prepare_for_send(ompi_mpi_local_colwertor,
                                                      &(dtype->super),
                                                      count,
                                                      sbuf,
                                                      0,
        ...
        do {
            ...
            do {
                ...
                /* Copy from the user's buffer to my shared mem
                   segment */
                max_data = segment_ddt_bytes;
                COPY_FRAGMENT_IN(sbuf_colwertor, index, rank, iov, max_data);
                bytes += max_data;
