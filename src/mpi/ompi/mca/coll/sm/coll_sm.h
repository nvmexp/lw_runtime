  ...
  exit_label:
    ...
    typedef struct mca_coll_sm_component_t {
        ...
        /** MCA parameter: Number of processes to use in the
            callwlation of the "info" MCA parameter */
     ...
     * members: the number of processes that are lwrrently using this
     * set of segments and the operation number of the current
     * operation.
     */
    typedef struct mca_coll_sm_in_use_flag_t {
        /** Number of processes lwrrently using this set of
            segments */
    ...
    typedef struct mca_coll_sm_comm_t {
        ...
        /** Number of barriers that we have exelwted (i.e., which set
    ...
    /* Lazily enable a module (since it ilwolves expensive/slow mmap
       allocation, etc.) */
 ...
 * segment
 */
#define COPY_FRAGMENT_IN(colwertor, index, rank, iov, max_data) \
    (iov).iov_base = \
        (index)->mcbmi_data + \
        ((rank) * mca_coll_sm_component.sm_fragment_size); \
    (iov).iov_len = (max_data); \
    opal_colwertor_pack(&(colwertor), &(iov), &mca_coll_sm_one, \
                        &(max_data) )
 ...
 * buffer
 */
#define COPY_FRAGMENT_OUT(colwertor, src_rank, index, iov, max_data) \
    (iov).iov_base = (((char*) (index)->mcbmi_data) + \
                       ((src_rank) * (mca_coll_sm_component.sm_fragment_size))); \
    (iov).iov_len = (max_data); \
    opal_colwertor_unpack(&(colwertor), &(iov), &mca_coll_sm_one, \
                          &(max_data) )
 ...
 * the child's ID based on the shift used to callwlate the "me" node
