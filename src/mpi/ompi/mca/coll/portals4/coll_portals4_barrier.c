...
static int
barrier_hyperlwbe_top(struct ompi_communicator_t *comm,
        ...
        mca_coll_portals4_module_t *portals4_module)
{
    ...
    /* Build "tree" out of hyperlwbe */
    dim = comm->c_lwbe_dim;
    hibit = opal_hibit(rank, dim);
    --dim;
    /* callwlate number of children to receive from */
...
static int
barrier_hyperlwbe_bottom(ompi_coll_portals4_request_t *request)
{
    int ret;
        ...
        mca_coll_base_module_t *module)
{
    ...
    ret = barrier_hyperlwbe_top(comm, request, portals4_module);
    if (OMPI_SUCCESS != ret) {
        opal_output_verbose(1, ompi_coll_base_framework.framework_output,
                "%s:%d: ompi_coll_portals4_barrier_hyperlwbe_top failed %d\n",
                __FILE__, __LINE__, ret);
        return OMPI_ERROR;
    }

    ret = barrier_hyperlwbe_bottom(request);
    if (OMPI_SUCCESS != ret) {
        opal_output_verbose(1, ompi_coll_base_framework.framework_output,
                "%s:%d: ompi_coll_portals4_barrier_hyperlwbe_bottom failed %d\n",
                __FILE__, __LINE__, ret);
        return OMPI_ERROR;
    }
