...
static int prepare_bcast_data (struct ompi_communicator_t *comm,
    ...
    if (request->u.bcast.needs_pack) {
        if (request->u.bcast.is_root) {
            OBJ_CONSTRUCT(&request->u.bcast.colwertor, opal_colwertor_t);
            opal_colwertor_copy_and_prepare_for_send(ompi_mpi_local_colwertor,
                    &(datatype->super), count,
                    buff, 0, &request->u.bcast.colwertor);
            opal_colwertor_get_packed_size(&request->u.bcast.colwertor, &request->u.bcast.tmpsize);
            request->u.bcast.tmpbuf = malloc(request->u.bcast.tmpsize);
            if (OPAL_UNLIKELY(NULL == request->u.bcast.tmpbuf)) {
                OBJ_DESTRUCT(&request->u.bcast.colwertor);
            ...
            ret = opal_colwertor_pack(&request->u.bcast.colwertor, &iovec, &iov_count, &max_data);
            OBJ_DESTRUCT(&request->u.bcast.colwertor);
            if (OPAL_UNLIKELY(ret < 0)) {
                return opal_stderr("opal_colwertor_pack failed", __FILE__, __LINE__, ret);	}
        }
        else {
            OBJ_CONSTRUCT(&request->u.bcast.colwertor, opal_colwertor_t);
            opal_colwertor_copy_and_prepare_for_recv(ompi_mpi_local_colwertor,
                    &(datatype->super), count,
                    buff, 0, &request->u.bcast.colwertor);

            max_data = request->u.bcast.tmpsize;
            opal_colwertor_get_packed_size(&request->u.bcast.colwertor, &max_data);
...
static int post_bcast_data(	ompi_coll_portals4_request_t *request) {
    ...
    if (request->u.bcast.needs_pack) {
        if (!request->u.bcast.is_root) {
            ...
            iovec.iov_len = request->u.bcast.tmpsize;
            iov_count = 1;
            ret = opal_colwertor_unpack(&request->u.bcast.colwertor, &iovec, &iov_count, &max_data);
            OBJ_DESTRUCT(&request->u.bcast.colwertor);
            if (OPAL_UNLIKELY(ret < 0)) {
                return opal_stderr("opal_colwertor_unpack failed", __FILE__, __LINE__, ret);
            }
        }
        ...
        mca_coll_portals4_module_t *portals4_module)
{
    ...
    for (i = 0 ; i < COLL_PORTALS4_BCAST_MAX_CHILDREN ; i++) {
        child[i] = PTL_ILWALID_RANK;
    }

    parent = PTL_ILWALID_RANK;
    ...
    if (child_nb) {
        ...
        for (seg = 1, offset = 0, length = 0 ;
            ...
            trig_thr = segment_nb + seg - 1; /* To be sure the set of PtlTriggeredPut of DATA will be exelwted in order */
        ...
        request->u.bcast.rtr_ct_h = PTL_ILWALID_HANDLE;
        request->u.bcast.ack_ct_h = PTL_ILWALID_HANDLE;
        ...
        mca_coll_portals4_module_t *portals4_module)
{
    ...
    if (rank != root) {
        ...
        trig_thr = (child != PTL_ILWALID_RANK) ?
                (segment_nb * 2) :
                segment_nb;
    ...
    if (child != PTL_ILWALID_RANK) {
        ...
        for (seg = 1, offset = 0, length = 0 ;
            ...
            /* compute the triggering threshold to send data to the children */
            trig_thr = segment_nb + seg - 1; /* To be sure the PtlTriggeredPut will be exelwted in order */
            if (rank != root) trig_thr ++;
...
{
    /* release all Portals4 resources for this request */
    if (request->u.bcast.u.child != PTL_ILWALID_RANK) {
        PtlCTFree(request->u.bcast.rtr_ct_h);
        PtlCTFree(request->u.bcast.ack_ct_h);
    }
