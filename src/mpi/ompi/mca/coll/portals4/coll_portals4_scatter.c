                             ...
                             mca_coll_portals4_module_t   *portals4_module)
{
    ...
    ompi_coll_portals4_create_send_colwerter (&request->u.scatter.send_colwerter,
    ...
    opal_colwertor_get_packed_size(&request->u.scatter.send_colwerter, &request->u.scatter.packed_size);
    OBJ_DESTRUCT(&request->u.scatter.send_colwerter);
    ...
    if (i_am_root) {
        ...
        /*
         * callwlate the total size of the packed data
         */
        ...
        for (int32_t i=0;i<request->u.scatter.size;i++) {
            ...
            opal_colwertor_pack(&request->u.scatter.send_colwerter, &iov, &iov_count, &max_data);
...
err_hdlr:
    opal_output(ompi_coll_base_framework.framework_output,
                "%s:%4d:%4d\tError oclwrred ret=%d, rank %2d",
...
err_hdlr:
    opal_output(ompi_coll_base_framework.framework_output,
                "%s:%4d:%4d\tError oclwrred (ptl_ret=%d) ret=%d, rank %2d",
                                            ...
                                            mca_coll_base_module_t *module)
{
    ...
    request->u.scatter.scatter_mdh  = PTL_ILWALID_HANDLE;
    request->u.scatter.scatter_cth  = PTL_ILWALID_HANDLE;
    request->u.scatter.scatter_meh  = PTL_ILWALID_HANDLE;
    request->u.scatter.sync_mdh     = PTL_ILWALID_HANDLE;
    request->u.scatter.sync_cth     = PTL_ILWALID_HANDLE;
    request->u.scatter.sync_meh     = PTL_ILWALID_HANDLE;
                                               ...
                                               ompi_coll_portals4_request_t *request)
{
    ...
    if (NULL != request->u.scatter.unpack_dst_buf) {
        ...
        ompi_coll_portals4_create_recv_colwerter (&request->u.scatter.recv_colwerter,
        ...
        opal_colwertor_unpack(&request->u.scatter.recv_colwerter, &iov, &iov_count, &max_data);

        OBJ_DESTRUCT(&request->u.scatter.recv_colwerter);
    }
