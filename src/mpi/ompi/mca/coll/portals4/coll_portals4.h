...
#define PTL_ILWALID_RANK ((ptl_rank_t)-1)
...
static inline int
get_nchildren(int lwbe_dim, int hibit, int rank, int size)
{
    int guess = lwbe_dim - (hibit + 1);

    if ((rank | (1 << (lwbe_dim - 1))) >= size) {
        guess--;
    }
    if (guess < 0) {
        return 0;
        ...
        ptl_rank_t *prev, ptl_rank_t *next)
{
    ...
    *next = (rank == (np - 1)) ?
            ((root == PTL_FIRST_RANK) ? PTL_ILWALID_RANK : PTL_FIRST_RANK):
            ((rank == (root - 1)) ? PTL_ILWALID_RANK : (rank + 1));
    return;
}
...
void get_k_ary_tree(const unsigned int k_ary,
    ...
    *father = (up == PTL_ILWALID_RANK) ? PTL_ILWALID_RANK : ((up + root) % np);
...
static inline void
ompi_coll_portals4_create_recv_colwerter (opal_colwertor_t *colwerter,
                                          void *target,
                                          ompi_proc_t *proc,
                                          int count,
...
{
    /* create colwerter */
    OBJ_CONSTRUCT(colwerter, opal_colwertor_t);

    /* initialize colwerter */
    opal_colwertor_copy_and_prepare_for_recv(proc->super.proc_colwertor,
                                             &datatype->super,
                                             count,
                                             target,
                                             0,
...
static inline void
ompi_coll_portals4_create_send_colwerter (opal_colwertor_t *colwerter,
                                          const void *source,
                                          ompi_proc_t *proc,
                                          int count,
                                          ompi_datatype_t *datatype)
{
    ...
    opal_colwertor_copy_and_prepare_for_send(proc->super.proc_colwertor,
                                             &datatype->super,
                                             count,
                                             source,
                                             0,
