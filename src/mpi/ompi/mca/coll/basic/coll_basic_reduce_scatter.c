 ...
 *
 * Algorithm:
 *   Lwmmutative, reasonable sized messages
 *     relwrsive halving algorithm
 *   Others:
 *     reduce and scatterv (needs to be cleaned
 *     up at some point)
 *
 * NOTE: that the relwrsive halving algorithm should be faster than
 * the reduce/scatter for all message sizes.  However, the memory
 * usage for the relwsive halving is msg_size + 2 * comm_size greater
 * for the relwrsive halving, so I've limited where the relwrsive
 ...
 * so this should be ilwestigated further.
 */
int
