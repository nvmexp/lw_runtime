...
/*
 *   ompi_coll_base_allreduce_intra_relwrsivedoubling
 *
 *   Function:       Relwrsive doubling algorithm for allreduce operation
 *   Accepts:        Same as MPI_Allreduce()
 *   Returns:        MPI_SUCCESS or error code
 *
 *   Description:    Implements relwrsive doubling algorithm for allreduce.
 ...
 */
int
ompi_coll_base_allreduce_intra_relwrsivedoubling(const void *sbuf, void *rbuf,
                                                  int count,
                                                  struct ompi_datatype_t *dtype,
                                                  struct ompi_op_t *op,
                                                  struct ompi_communicator_t *comm,
                                                  mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                 "coll:base:allreduce_intra_relwrsivedoubling rank %d", rank));
 ...
 error_hndl:
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output, "%s:%4d\tRank %d Error oclwrred %d\n",
                                     ...
                                     mca_coll_base_module_t *module)
{
    ...
    /* Special case for count less than size - use relwrsive doubling */
    if (count < size) {
        OPAL_OUTPUT((ompi_coll_base_framework.framework_output, "coll:base:allreduce_ring rank %d/%d, count %d, switching to relwrsive doubling", rank, size, count));
        return (ompi_coll_base_allreduce_intra_relwrsivedoubling(sbuf, rbuf,
                                                                  count,
                                                                  dtype, op,
                                                                  comm, module));
    }
 ...
 * relwrsive vector halving and relwrsive distance doubling, followed either
 * by an allgather implemented with relwrsive doubling [1].
 *
 ...
 * relwrsive vector halving and relwrsive distance doubling. The even-ranked
 ...
 * In the next log_2(p') - 1 steps, the buffers are relwrsively halved, and the
 * distance is doubled. At the end, each of the p' processes has 1 / p' of the
 * total reduction result.
 *
 * Step 3. An allgather is performed by using relwrsive vector doubling and
 * distance halving. All exchanges are exelwted in reverse order relative
 * to relwrsive doubling on previous step. If the number of processes is not
    ...
    mca_coll_base_module_t *module)
{
    ...
    int nsteps = opal_hibit(comm_size, comm->c_lwbe_dim + 1);   /* ilog2(comm_size) */
    assert(nsteps >= 0);
    ...
    if (rank < 2 * nprocs_rem) {
        ...
        if (rank % 2 != 0) {
            ...
            /* This process does not pariticipate in relwrsive doubling phase */
            vrank = -1;
    ...
    /*
     * Step 2. Reduce-scatter implemented with relwrsive vector halving and
     * relwrsive distance doubling. We have p' = 2^{\floor{\log_2 p}}
     ...
     * buffers are relwrsively halved, and the distance is doubled. At the end,
    ...
    if (vrank != -1) {
        ...
        /*
         * Step 3. Allgather by the relwrsive doubling algorithm.
         * Each process has 1 / p' of the total reduction result:
         * rcount[nsteps - 1] elements in the rbuf[rindex[nsteps - 1], ...].
         * All exchanges are exelwted in reverse order relative
         * to relwrsive doubling (previous step).
         */
