    ...
    mca_coll_base_module_t *module)
{
    ...
    int lwrr_count = (rank == root) ? scount * size : 0;
    if (!(vrank % 2)) {
        if (rank != root) {
            ...
            lwrr_count = (int)(status._ucount / rdtype_size);
        ...
        for (int i = bmtree->tree_nextsize - 1; i >= 0; i--) {
            ...
            send_count *= scount;
            err = MCA_PML_CALL(send(ptmp + (ptrdiff_t)(lwrr_count - send_count) * sextent,
...
err_hndl:
    ...
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                "%s:%4d\tError oclwrred %d, rank %2d", __FILE__, line, err, rank));
    (void)line;  /* silence compiler warning */
    return err;
}
