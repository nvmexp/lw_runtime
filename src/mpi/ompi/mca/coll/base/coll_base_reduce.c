 ...
 * for the first block: thus we must copy sendbuf to aclwmbuf on intermediate
                                    ...
                                    int max_outstanding_reqs )
{
    ...
    char *aclwmbuf = NULL, *aclwmbuf_free = NULL;
    ...
    if( tree->tree_nextsize > 0 ) {
        ...
        aclwmbuf = (char*)recvbuf;
        if( (NULL == aclwmbuf) || (root != rank) ) {
            /* Allocate temporary aclwmulator buffer. */
            size = opal_datatype_span(&datatype->super, original_count, &gap);
            aclwmbuf_free = (char*)malloc(size);
            if (aclwmbuf_free == NULL) {
                line = __LINE__; ret = -1; goto error_hndl;
            }
            aclwmbuf = aclwmbuf_free - gap;
        }

        /* If this is a non-commutative operation we must copy
           sendbuf to the aclwmbuf, in order to simplfy the loops */
            ...
            for( i = 0; i < tree->tree_nextsize; i++ ) {
                ...
                if( segindex < num_segments ) {
                    ...
                    if( 0 == i ) {
                        ...
                        if( (ompi_op_is_commute(op)) &&
                            ...
                            local_recvbuf = aclwmbuf + (ptrdiff_t)segindex * (ptrdiff_t)segment_increment;
                        }
                    }
                ...
                if( i > 0 ) {
                    ...
                    ompi_op_reduce(op, local_op_buffer,
                                   aclwmbuf + (ptrdiff_t)segindex * (ptrdiff_t)segment_increment,
                                   recvcount, datatype );
                } else if ( segindex > 0 ) {
                    void* aclwmulator = aclwmbuf + (ptrdiff_t)(segindex-1) * (ptrdiff_t)segment_increment;
                    ...
                    ompi_op_reduce(op, local_op_buffer, aclwmulator, prevcount,
                                   datatype );
                    ...
                    if (rank != tree->tree_root) {
                        /* send combined/aclwmulated data to parent */
                        ret = MCA_PML_CALL( send( aclwmulator, prevcount,
        ...
        if( aclwmbuf_free != NULL ) free(aclwmbuf_free);
    }
 ...
 error_hndl:  /* error handler */
    ...
    if( inbuf_free[0] != NULL ) free(inbuf_free[0]);
    if( inbuf_free[1] != NULL ) free(inbuf_free[1]);
    if( aclwmbuf_free != NULL ) free(aclwmbuf);
 ...
 *
 * Step 3. A binomial tree gather is performed by using relwrsive vector
