...
/*
 * ompi_coll_base_scan_intra_relwrsivedoubling
 *
 * Function:  Relwrsive doubling algorithm for inclusive scan.
 * Accepts:   Same as MPI_Scan
 * Returns:   MPI_SUCCESS or error code
 *
 * Description:  Implements relwrsive doubling algorithm for MPI_Scan.
 ...
 */
int ompi_coll_base_scan_intra_relwrsivedoubling(
    ...
    mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                 "coll:base:scan_intra_relwrsivedoubling: rank %d/%d",
                 rank, comm_size));
    if (count == 0)
        return MPI_SUCCESS;
