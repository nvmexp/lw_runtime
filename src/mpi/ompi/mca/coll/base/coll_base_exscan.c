...
/*
 * ompi_coll_base_exscan_intra_relwrsivedoubling
 *
 * Function:  Relwrsive doubling algorithm for exclusive scan.
 * Accepts:   Same as MPI_Exscan
 * Returns:   MPI_SUCCESS or error code
 *
 * Description:  Implements relwrsive doubling algorithm for MPI_Exscan.
 ...
 */
int ompi_coll_base_exscan_intra_relwrsivedoubling(
    ...
    mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output, "coll:base:exscan_intra_relwrsivedoubling: rank %d/%d",
                 rank, comm_size));
    if (count == 0)
        return MPI_SUCCESS;
    if (comm_size < 2)
