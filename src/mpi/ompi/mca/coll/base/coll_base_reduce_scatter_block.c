...
/*
 * ompi_coll_base_reduce_scatter_block_intra_relwrsivedoubling
 *
 * Function:  Relwrsive doubling algorithm for reduce_scatter_block.
 * Accepts:   Same as MPI_Reduce_scatter_block
 * Returns:   MPI_SUCCESS or error code
 *
 * Description:  Implements relwrsive doubling algorithm for MPI_Reduce_scatter_block.
    ...
    mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                 "coll:base:reduce_scatter_block_intra_relwrsivedoubling: rank %d/%d",
                 rank, comm_size));
    if (rcount == 0)
        return MPI_SUCCESS;
    if (comm_size < 2)
    ...
    /* Relwrsive distance doubling */
    int rdoubling_step = 0;
    for (int mask = 1; mask < comm_size; mask <<= 1) {
        int remote = rank ^ mask;
        int lwr_tree_root = ompi_rounddown(rank, mask);
         ...
         *
         * Send block from tmpbuf: [0..lwr_tree_root - 1], [lwr_tree_root + mask, p - 1]
         * Recv block into tmprecv: [0..remote_tree_root - 1], [remote_tree_root + mask, p - 1]
         */

        /* Send type */
        blocklens[0] = rcount * lwr_tree_root;
        blocklens[1] = (comm_size >= lwr_tree_root + mask) ?
                       rcount * (comm_size - lwr_tree_root - mask) : 0;
        displs[0] = 0;
         ...
         * Relwrsive halving algorithm is used for search of process.
         */
        if (remote_tree_root + mask > comm_size) {
             ...
             */
            int nprocs_alldata = comm_size - lwr_tree_root - mask;
        ...
        if (is_block_received) {
            ...
            if (is_commutative || (remote_tree_root < lwr_tree_root)) {
...
/*
 * ompi_coll_base_reduce_scatter_block_intra_relwrsivehalving
 *
 * Function:  Relwrsive halving algorithm for reduce_scatter_block
 * Accepts:   Same as MPI_Reduce_scatter_block
 * Returns:   MPI_SUCCESS or error code
 *
 * Description:  Implements relwrsive halving algorithm for MPI_Reduce_scatter_block.
    ...
    mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                 "coll:base:reduce_scatter_block_intra_relwrsivehalving: rank %d/%d",
                 rank, comm_size));
    ...
    if (vrank != -1) {
        /*
         * Step 2. Relwrsive vector halving. We have p' = 2^{\floor{\log_2 p}}
         ...
         * buffers are relwrsively halved. At the end, each of the p' processes
        ...
        for (int mask = nprocs_pof2 >> 1; mask > 0; mask >>= 1) {
            ...
            /*
             * Callwlate the recv_count and send_count because the
             * even-numbered processes who no longer participate will
             * have their result callwlated by the process to their
             * right (rank + 1).
             */
    ...
    mca_coll_base_module_t *module)
{
    ...
    nprocs_pof2 >>= 1;
    int nprocs_rem = comm_size - nprocs_pof2;
    int log2_size = opal_lwbe_dim(nprocs_pof2);
    ...
    mca_coll_base_module_t *module)
{
    ...
    int log2_comm_size = opal_lwbe_dim(comm_size);
