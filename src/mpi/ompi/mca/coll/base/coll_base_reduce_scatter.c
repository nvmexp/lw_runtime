...
/*
 * Relwrsive-halving function is (*mostly*) copied from the BASIC coll module.
...
/*
 *  reduce_scatter_intra_basic_relwrsivehalving
 *
 *  Function:   - reduce scatter implementation using relwrsive-halving
 *                algorithm
 ...
 */
int
ompi_coll_base_reduce_scatter_intra_basic_relwrsivehalving( const void *sbuf,
                                                            void *rbuf,
                                                            const int *rcounts,
                                                            struct ompi_datatype_t *dtype,
                                                            struct ompi_op_t *op,
                                                            ...
                                                            mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,"coll:base:reduce_scatter_intra_basic_relwrsivehalving, rank %d", rank));
    ...
    if (tmp_rank >= 0) {
        ...
        /* recallwlate disps and rcounts to account for the
        ...
        /* do the relwrsive halving communication.  Don't use the
           dimension information on the communicator because I
           think the information is ilwalidated by our "shrinking"
           of the communicator */
        mask = tmp_size >> 1;
                                          ...
                                          mca_coll_base_module_t *module)
{
    ...
    char *tmpsend = NULL, *tmprecv = NULL, *aclwmbuf = NULL, *aclwmbuf_free = NULL;
    ...
    */
    ...
    aclwmbuf_free = (char*)malloc(dsize);
    if (NULL == aclwmbuf_free) { ret = -1; line = __LINE__; goto error_hndl; }
    ...
    */
    ...
    ret = MCA_PML_CALL(irecv(inbuf[inbi], max_block_count, dtype, recv_from,
                             ...
                             &reqs[inbi]));
    if (MPI_SUCCESS != ret) { line = __LINE__; goto error_hndl; }
    tmpsend = aclwmbuf + (ptrdiff_t)displs[recv_from] * extent;
    ...
    for (k = 2; k < size; k++) {
        ...
        */
        tmprecv = aclwmbuf + (ptrdiff_t)displs[prevblock] * extent;
    ...
    tmprecv = aclwmbuf + (ptrdiff_t)displs[rank] * extent;
    ...
    if (NULL != aclwmbuf_free) free(aclwmbuf_free);
