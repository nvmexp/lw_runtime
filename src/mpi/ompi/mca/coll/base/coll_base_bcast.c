                                     ...
                                     ompi_coll_tree_t* tree )
{
 ...
 error_hndl:
    ...
    OPAL_OUTPUT( (ompi_coll_base_framework.framework_output,"%s:%4d\tError oclwrred %d, rank %2d",
                                            ...
                                            uint32_t segsize )
{
 ...
 error_hndl:
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,"%s:%4d\tError oclwrred %d, rank %2d", __FILE__,line,err,rank));
 ...
 *
 * Function:  Bcast using a binomial tree scatter followed by a relwrsive
 *            doubling allgather.
 * Accepts:   Same arguments as MPI_Bcast
 * Returns:   MPI_SUCCESS or error code
 *
 ...
 *   Relwrsive doubling allgather: \alpha\log(p) + \beta*m((p-1)/p)
 *
 * Example, p=8, count=8, root=0
 *    Binomial tree scatter      Relwrsive doubling allgather
 * 0: --+  --+  --+  [0*******]  <-+ [01******]  <--+   [0123****] <--+
 * 1:   |   2|  <-+  [*1******]  <-+ [01******]  <--|-+ [0123****] <--+-+
 * 2:  4|  <-+  --+  [**2*****]  <-+ [**23****]  <--+ | [0123****] <--+-+-+
 * 3:   |       <-+  [***3****]  <-+ [**23****]  <----+ [0123****] <--+-+-+-+
    ...
    uint32_t segsize)
{
    ...
    int lwrr_count = (rank == root) ? count : 0;
    ...
    while (mask < comm_size) {
        if (vrank & mask) {
            ...
            if (recv_count <= 0) {
                ...
                /* Get received count */
                lwrr_count = (int)(status._ucount / datatype_size);
            }
            break;
        }
        mask <<= 1;
    ...
    while (mask > 0) {
        if (vrank + mask < comm_size) {
            send_count = lwrr_count - scatter_count * mask;
            if (send_count > 0) {
    ...
    /*
     * Allgather by relwrsive doubling
     * Each process has the lwrr_count elems in the buf[vrank * scatter_count, ...]
     */
    int rem_count = count - vrank * scatter_count;
    lwrr_count = (scatter_count < rem_count) ? scatter_count : rem_count;
    if (lwrr_count < 0)
        lwrr_count = 0;
