...
static int callwlate_level( int fanout, int rank )
{
    int level, num;
    if( rank < 0 ) return -1;
...
static int callwlate_num_nodes_up_to_level( int fanout, int level )
{
                                 ...
                                 int root )
{
     ...
     * shiftedrank should be used in callwlating distances
    ...
    /* callwlate my level */
    level = callwlate_level( fanout, shiftedrank );
    ...
    /* find my parent */
    slimit = callwlate_num_nodes_up_to_level( fanout, level );
    sparent = shiftedrank;
    if( sparent < fanout ) {
        sparent = 0;
    } else {
                                  ...
                                  int root )
{
    ...
    /*
     * Callwlate maximum chain length
     */
