...
static inline int exscan_sched_linear(
    ...
    void *tmpbuf);
static inline int exscan_sched_relwrsivedoubling(
...
static int nbc_exscan_init(const void* sendbuf, void* recvbuf, int count, MPI_Datatype datatype, MPI_Op op,
    ...
    if (alg == NBC_EXSCAN_LINEAR) {
        ...
        res = exscan_sched_relwrsivedoubling(rank, p, sendbuf, recvbuf, count,
...
/*
 * exscan_sched_relwrsivedoubling:
 *
 * Function:  Relwrsive doubling algorithm for exclusive scan.
 * Accepts:   Same as MPI_Iexscan
 * Returns:   MPI_SUCCESS or error code
 *
 * Description:  Implements relwrsive doubling algorithm for MPI_Iexscan.
