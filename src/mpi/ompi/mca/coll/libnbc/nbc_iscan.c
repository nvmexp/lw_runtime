...
static inline int scan_sched_linear(
    ...
    void *tmpbuf);
static inline int scan_sched_relwrsivedoubling(
...
static int nbc_scan_init(const void* sendbuf, void* recvbuf, int count, MPI_Datatype datatype, MPI_Op op,
  ...
  if (NULL == found) {
    ...
    if (alg == NBC_SCAN_LINEAR) {
        ...
        res = scan_sched_relwrsivedoubling(rank, p, sendbuf, recvbuf, count,
...
/*
 * scan_sched_relwrsivedoubling:
 *
 * Function:  Relwrsive doubling algorithm for inclusive scan.
 * Accepts:   Same as MPI_Iscan
 * Returns:   MPI_SUCCESS or error code
 *
 * Description:  Implements relwrsive doubling algorithm for MPI_Iscan.
