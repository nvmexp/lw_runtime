...
static inline int allred_sched_relwrsivedoubling(int rank, int p, const void *sendbuf, void *recvbuf,
    ...
    if (p == 1) {
      ...
      switch(alg) {
        ...
        case NBC_ARED_RING:
          ...
          break;
        case NBC_ARED_RDBL:
          res = allred_sched_relwrsivedoubling(rank, p, sendbuf, recvbuf, count, datatype, gap, op, inplace, schedule, tmpbuf);
          break;
      }
    }
...
/*
 *   allred_sched_relwrsivedoubling
 *
 *   Function:       Relwrsive doubling algorithm for iallreduce operation
 *
 *   Description:    Implements relwrsive doubling algorithm for iallreduce.
 ...
 * by an allgather implemented with relwrsive doubling.
 *
        ...
        int count_rhalf = count - count_lhalf;
         if (rank % 2 != 0) {
            ...
            res = NBC_Sched_send((char *)rbuf + (ptrdiff_t)count_lhalf * extent,
                                 false, count_rhalf, datatype, rank - 1, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) { goto cleanup_and_return; }
             /* This process does not participate in relwrsive doubling phase */
            vrank = -1;
         } else {
            /*
