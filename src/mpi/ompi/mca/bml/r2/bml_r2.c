...
static void mca_bml_r2_callwlate_bandwidth_latency (mca_bml_base_btl_array_t *btl_array, double *total_bandwidth, uint32_t *latency)
{
    ...
    if (btl_flags & MCA_BTL_FLAGS_SEND) {
        ...
        if (!bml_btl || bml_btl->btl->btl_exclusivity <= btl->btl_exclusivity) {
            ...
            if (NULL == bml_btl || (bml_btl->btl->btl_exclusivity <= btl->btl_exclusivity)) {
                ...
                /**
                 * callwlate the bitwise OR of the btl flags
                 */
...
static void mca_bml_r2_compute_endpoint_metrics (mca_bml_base_endpoint_t *bml_endpoint)
{
    ...
    mca_bml_r2_callwlate_bandwidth_latency (&bml_endpoint->btl_send, &total_bandwidth, &latency);
    ...
    mca_bml_r2_callwlate_bandwidth_latency (&bml_endpoint->btl_rdma, &total_bandwidth, &latency);
