...
static inline mca_bml_base_btl_t* mca_bml_base_btl_array_get_next(mca_bml_base_btl_array_t* array)
{
    ...
    if( 1 == array->arr_size ) {
        ...
        size_t lwrrent_position = array->arr_index;  /* force to always start from zero */
        if( (lwrrent_position + 1) == array->arr_size ) {
            array->arr_index = 0;  /* next time serve from the beginning */
        } else {
            array->arr_index = lwrrent_position + 1;  /* continue */
        }
        return &array->bml_btls[lwrrent_position];
    }
}
                                            ...
                                            mca_btl_base_descriptor_t** des)
{
    ...
    *des = btl->btl_prepare_src( btl, bml_btl->btl_endpoint, colw,
 ...
 * processes can not be reached by the lwrrently active BTLs.  This is
 ...
 * be reached by a lwrrently active BTL. This is not a fatal error, and the
