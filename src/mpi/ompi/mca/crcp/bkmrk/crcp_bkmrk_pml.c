...
static uint64_t lwrrent_msg_id = 0;
static ompi_crcp_bkmrk_pml_message_type_t lwrrent_msg_type = 0;
...
void ompi_crcp_bkmrk_pml_peer_ref_construct(ompi_crcp_bkmrk_pml_peer_ref_t *peer_ref) {
    peer_ref->proc_name.jobid  = ORTE_JOBID_ILWALID;
    peer_ref->proc_name.vpid   = ORTE_VPID_ILWALID;
...
void ompi_crcp_bkmrk_pml_traffic_message_ref_construct(ompi_crcp_bkmrk_pml_traffic_message_ref_t *msg_ref) {
    ...
    msg_ref->proc_name.jobid  = ORTE_JOBID_ILWALID;
    msg_ref->proc_name.vpid   = ORTE_VPID_ILWALID;

    msg_ref->matched        = ILWALID_INT;
    msg_ref->done           = ILWALID_INT;
    msg_ref->active         = ILWALID_INT;
    msg_ref->posted         = ILWALID_INT;
    msg_ref->active_drain   = ILWALID_INT;
}
...
void ompi_crcp_bkmrk_pml_drain_message_ref_construct(ompi_crcp_bkmrk_pml_drain_message_ref_t *msg_ref) {
    ...
    msg_ref->done           = ILWALID_INT;
    msg_ref->active         = ILWALID_INT;
    msg_ref->already_posted = ILWALID_INT;
}
...
void ompi_crcp_bkmrk_pml_drain_message_ack_ref_construct(ompi_crcp_bkmrk_pml_drain_message_ack_ref_t *msg_ack_ref) {
    ...
    msg_ack_ref->peer.jobid  = ORTE_JOBID_ILWALID;
    msg_ack_ref->peer.vpid   = ORTE_VPID_ILWALID;
}
...
int ompi_crcp_bkmrk_pml_init(void) {
    message_seq_num = 1;
    lwrrent_msg_id  = 0;
    lwrrent_msg_type = COORD_MSG_TYPE_UNKNOWN;
                                  ...
                                  ompi_crcp_base_pml_state_t* pml_state )
{
    ...
    if( OMPI_CRCP_PML_PRE == pml_state->state ) {
        ...
        /*  Bookkeeping */
        peer_ref->total_msgs_sent += 1;
        lwrrent_msg_id = msg_ref->msg_id;
        lwrrent_msg_type = COORD_MSG_TYPE_B_SEND;
                                  ...
                                  ompi_crcp_base_pml_state_t* pml_state)
{
    ...
    if( OMPI_CRCP_PML_PRE == pml_state->state) {
         ...
         */
            ...
            /*  Bookkeeping */
            lwrrent_msg_id = msg_ref->msg_id;
            lwrrent_msg_type = COORD_MSG_TYPE_B_RECV;
                                  ...
                                  ompi_crcp_base_pml_state_t* pml_state)
{
     ...
     *****************************/
        ...
        for(item  = opal_list_get_first(&ompi_crcp_bkmrk_pml_peer_refs);
            ...
            ompi_crcp_bkmrk_pml_peer_ref_t *lwr_peer_ref;
            lwr_peer_ref = (ompi_crcp_bkmrk_pml_peer_ref_t*)item;

            /* JJH - Assuming only one global jobid at the moment */
            lwr_peer_ref->proc_name.jobid = OMPI_PROC_MY_NAME->jobid;
        }
                                  ...
                                  ompi_crcp_bkmrk_pml_traffic_message_ref_t **msg_ref)
{
    ...
    /*
     * Determine if message is lwrrently in the list
    ...
    if( NULL != *msg_ref ) {
        ...
        if( NULL != peer_ref ) {
            ...
            CREATE_NEW_MSG((*msg_ref), msg_type,
                           count, ddt_size, tag, dest, comm,
                           ORTE_JOBID_ILWALID, ORTE_VPID_ILWALID);
        }
                                 ...
                                 ompi_crcp_bkmrk_pml_message_content_ref_t ** content_ref)
{
    ...
    traffic_message_find_mark_persistent(msg_ref, request,
                                         false,  /* Find lwrrently not active */
                                         true, /* Mark as active */
                                         content_ref);
    return OMPI_SUCCESS;
}
                                                ...
                                                ompi_crcp_bkmrk_pml_message_content_ref_t **c_ref)
{
    ...
    for(item  = opal_list_get_first(&(msg_ref->msg_contents));
        ...
        if( content_ref->active != lwr_active ) {
            continue;
        }
                                                ...
                                                int *num_posted)
{
    ...
    *num_posted = 0;
    ...
    for(m_iter = 0; m_iter < m_total; ++m_iter) {
        ...
        /* Decrementing active oclwrs when we stall in the Blocking Recv, do not do so here. */
        if( NULL != prev_content ) {
    ...
    *found_msg_ref = NULL;
    ...
    for(item  = opal_list_get_last(search_list);
        ...
        if(msg_ref->count == count  &&
           ...
           (peer          == ILWALID_INT || msg_ref->rank == peer)  &&
           msg_ref->ddt_size == ddt_size) {
...
static int drain_message_find_any(size_t count, int tag, int peer,
                                  ...
                                  ompi_crcp_bkmrk_pml_peer_ref_t **peer_ref)
{
    ompi_crcp_bkmrk_pml_peer_ref_t *lwr_peer_ref = NULL;
    opal_list_item_t* item = NULL;
...
static int ft_event_coordinate_peers(void)
{
    ...
    if( stall_for_completion ) {
        OPAL_OUTPUT_VERBOSE((10, mca_crcp_bkmrk_component.super.output_handle,
                             ...
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
                             (lwrrent_msg_type == COORD_MSG_TYPE_B_SEND ? "Send" : "Recv"),
                             getpid() ));
        step_to_return_to = 1;
        exit_status = OMPI_SUCCESS;
        goto DONE;
...
static int ft_event_post_drained(void)
{
    ...
    for(item  = opal_list_get_first(&ompi_crcp_bkmrk_pml_peer_refs);
        ...
        for(d_item  = opal_list_get_first(&(lwr_peer_ref->drained_list));
            d_item != opal_list_get_end(&(lwr_peer_ref->drained_list));
    ...
    for(item  = opal_list_get_first(&ompi_crcp_bkmrk_pml_peer_refs);
        ...
        for(d_item  = opal_list_get_first(&(lwr_peer_ref->drained_list));
            ...
            for(c_item  = opal_list_get_first(&(drain_msg_ref->msg_contents));
                ...
                lwr_peer_ref->ack_required = true;
        ...
        if( peer_total > 0 || stall_for_completion ) {
            OPAL_OUTPUT_VERBOSE((10, mca_crcp_bkmrk_component.super.output_handle,
                                 ...
                                 OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
                                 OMPI_NAME_PRINT(&(lwr_peer_ref->proc_name)),
                                 peer_total,
                                 quiesce_request_count,
...
static int wait_quiesce_drained(void)
{
    ...
    for(item  = opal_list_get_first(&ompi_crcp_bkmrk_pml_peer_refs);
        ...
        if( lwr_peer_ref->ack_required ) {
            ...
            if (ORTE_SUCCESS != (ret = ompi_rte_send_buffer_nb(&(lwr_peer_ref->proc_name),
        ...
        lwr_peer_ref->ack_required = false;
        ...
        for(d_item  = opal_list_get_first(&(lwr_peer_ref->drained_list));
            ...
            for(c_item  = opal_list_get_first(&(drain_msg_ref->msg_contents));
                ...
                if( content_ref->already_posted ) {
                    drain_message_remove(lwr_peer_ref, drain_msg_ref, content_ref);
    ...
    *num_resolved = 0;
    ...
    if( num_still_active > 0 ) {
        ...
        if( NULL != posted_recv_msg_ref ) {
            ...
            if (lwrrent_msg_id         == posted_recv_msg_ref->msg_id  &&
