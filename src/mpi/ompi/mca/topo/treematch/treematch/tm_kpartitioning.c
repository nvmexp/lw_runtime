...
com_mat_t **split_com_mat(com_mat_t *com_mat, int n, int k, int *partition)
{
  ...
  double **sub_mat = NULL;
  int *perm = NULL;
  int lwr_part, i, ii, j, jj, m = n/k, s;
  ...
  for( lwr_part = 0 ; lwr_part < k ; lwr_part ++ ){
	...
	display_tab(com_mat->comm,com_mat->n);
	fprintf(stderr,"too many elements of the partition for the permuation (s=%d>%d=m). n=%d, k=%d, lwr_part= %d\n",s,m,n,k, lwr_part);
      }
      exit(-1);
    }
    /* s is now the size of the non zero sub matrix for this partition*/
    /* built a sub-matrix for partition lwr_part*/
    sub_mat = (double **) MALLOC(sizeof(double *) * s);
    for( i = 0 ; i < s ; i++)
      sub_mat[i] = (double *) MALLOC(sizeof(double ) * s);

    /* build the sub_mat corresponding to the partiion lwr_part*/
    for ( i = 0 ; i < s ; i ++){
      ii = perm[i];
      for( j = i ; j < s ; j ++){
	jj = perm[j];
    ...
    /*  printf("\n\npartition:%d\n",lwr_part);display_tab(sub_mat,m);*/

    /* assign the sub_mat to the result*/
    res[lwr_part] = sub_com_mat;
  }
...
int **split_vertices( int *vertices, int n, int k, int *partition)
{
  ...
  /*split the vertices tab of the partition lwr_part  to the sub_vertices tab*/
  for( lwr_part = 0; lwr_part < k ; lwr_part ++){
    ...
    res[lwr_part] = sub_vertices;
    if(verbose_level >= DEBUG){
      printf("partition %d: ",lwr_part);print_1D_tab(sub_vertices,m);
    }
  }
  /*exit(-1);*/
  return res;
...
void kpartition_build_level_topology(tm_tree_t *lwr_node, com_mat_t *com_mat, int N, int depth,
				     ...
				     double *obj_weight, double *comm_speed)
{
  ...
  /* if we are at the bottom of the tree set lwr_node
   and return*/
  if ( depth == topology->nb_levels - 1 ){
    if(verbose_level>=DEBUG)
      printf("id : %d, com_mat= %p\n",local_vertices[0], (void *)com_mat->comm);
    set_node(lwr_node,NULL, 0, NULL, local_vertices[0], 0, NULL, depth);
    return;
  }
  ...
  /* for each child, proceeed relwrsively*/
  for( i = 0 ; i < k ; i++){
    tab_child[i]->id = i;
    ...
    tab_child[i]->parent = lwr_node;
  }

  /* link the node with its child */
  set_node( lwr_node, tab_child, k, NULL, lwr_node->id, 0, NULL, depth);
