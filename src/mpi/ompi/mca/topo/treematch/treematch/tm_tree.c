...
int relwrs_select_independent_groups(group_list_t **, int, int, int, int,
...
double eval_grouping(tm_affinity_mat_t *aff_mat, tm_tree_t **lwr_group, int arity)
{
  double res = 0;
  int i, j, id, id1, id2;
...
void add_to_list(group_list_t *list, tm_tree_t **lwr_group, int arity, double val)
{
  ...
  for( i = 0 ; i < arity ; i++ ){
    tab[i] = lwr_group[i];
    if(verbose_level>=DEBUG)
      printf("lwr_group[%d]=%d ", i, lwr_group[i]->id);
  }
  if(verbose_level>=DEBUG)
    printf(": %f\n", val);
...
void  list_all_possible_groups(tm_affinity_mat_t *aff_mat, tm_tree_t *tab_node, int id, int arity, int depth,
			       tm_tree_t **lwr_group, group_list_t *list)
{
  double val;
  int i;
  ...
  if(depth == arity){
    val = eval_grouping(aff_mat, lwr_group, arity);
    add_to_list(list, lwr_group, arity, val);
    return;
	...
	continue;
      lwr_group[depth] = &tab_node[i];
      if(verbose_level>=DEBUG)
	printf("%d<-%d\n", depth, i);
      list_all_possible_groups(aff_mat, tab_node, i+1, arity, depth+1, lwr_group, list);
    }
  }
}
...
int relwrs_select_independent_groups(group_list_t **tab, int i, int n, int arity, int d, int M, double val, double *best_val, group_list_t **selection, group_list_t **best_selection)
{
  group_list_t *elem = NULL;
  /*
    if(val>=*best_val)
  ...
  */
  ...
  while( i < n ){
	...
	printf("%d: %d\n", d, i);
      selection[d] = elem;
      val += elem->val;
      return relwrs_select_independent_groups(tab, i+1, n, arity, d+1, M, val, best_val, selection, best_selection);
    }
    i++;
  }
  return 0;
			       ...
			       group_list_t **best_selection, int bound, double max_duration)
{
  ...
  for( i = 0 ; i < MIN(bound, n) ; i++ ){
    ...
    val = tab_group[i]->val;
    relwrs_select_independent_groups(tab_group, i+1, n, arity, 1, M, val, best_val, selection, best_selection);
...
static int thread_derelwrs_exhaustive_search(group_list_t **tab_group, int i, int nb_groups, int arity, int depth, int solution_size,
...
static void  partial_exhaustive_search(int nb_args, void **args, int thread_id){
  ...
  while(work->tab_group){
	...
	continue;
      ...
      /* } */
      if(verbose_level>=INFO){
	fprintf(stdout, "\r%d: %.2f%% of search space explored...", thread_id,(100.0*lwr_work)/total_work);
	fflush(stdout);
      }
      for(i=0;i<work->nb_groups;i++){
	...
	val +=  tab_group[id]->val;
      ...
      thread_derelwrs_exhaustive_search(tab_group, id+1, n, arity, work->nb_groups, solution_size, val, best_val, selection, best_selection, indep_mat, lock, thread_id, tab_i, work->nb_groups);
  next_work:
      work=work->next;
      lwr_work++;
  }
...
static work_unit_t *create_work_unit(work_unit_t *lwr,  int *tab,int size){
...
static work_unit_t *generate_work_units(work_unit_t *lwr,  int i, int id, int *tab_group,int size, int id_max){

  tab_group[i] = id;
  if(i==size-1){
    return create_work_unit(lwr,tab_group,size);
  }
  ...
  id++;
  for(;id < id_max;id++){
    lwr = generate_work_units(lwr,i+1,id,tab_group, size, id_max);
  }
...
static work_unit_t *create_tab_work(int n){
  int work_size = 4;
  int i;
  work_unit_t *lwr,*res = (work_unit_t *) CALLOC(1,sizeof(work_unit_t));
  int *tab_group = MALLOC(work_size*sizeof(int));
  lwr = res;
  lwr = generate_work_units(lwr,0,0,tab_group,3,n);
  lwr = generate_work_units(lwr,0,1,tab_group,2,n);
  lwr = generate_work_units(lwr,0,2,tab_group,2,n);

  for(i=3;i<n;i++)
    lwr = generate_work_units(lwr,0,i,tab_group,1,n);

  for(lwr = res; lwr->tab_group; lwr = lwr-> next)
    res->nb_work++;
...
#if 0
static int old_relwrs_exhaustive_search(group_list_t **tab, int i, int n, int arity, int d, int solution_size, double val, double *best_val, group_list_t **selection, group_list_t **best_selection, int8_t **indep_mat)
{
  group_list_t *elem = NULL;
  ...
  while( i < n ){
	...
	selection[d] = elem;
	val += elem->val;
	old_relwrs_exhaustive_search(tab, i+1, n, arity, d+1, solution_size, val, best_val, selection, best_selection, indep_mat);
      val -= elem->val;
      }
    }
    i++;
...
#if 0
static int relwrs_exhaustive_search(group_list_t **tab, int i, int n, int arity, int d, int solution_size, double val, double *best_val, group_list_t **selection, group_list_t **best_selection, int8_t **indep_mat, int* tab_i)
{
  group_list_t *elem = NULL;
...
/* matrix of indepedency between groups (i.e; 2 groups are independent if they
  ...
  for( i = 0 ; i < n ; i++ ){
    ...
    selection[0] = tab_group[i];
    val = tab_group[i]->val;
    /* relwrs_exhaustive_search(tab_group, i+1, n, arity, 1, solution_size, val, best_val, selection, best_selection, indep_mat, tab_i); */
    old_relwrs_exhaustive_search(tab_group, i+1, n, arity, 1, solution_size, val, best_val, selection, best_selection, indep_mat);
  }
...
void  fast_group(tm_affinity_mat_t *aff_mat, tm_tree_t *tab_node, tm_tree_t *parent, int id, int arity, int n,
		 double *best_val, tm_tree_t **lwr_group, int *nb_groups, int max_groups)
{
  double val;
  int i;
  int mat_order = aff_mat->order;
  ...
  */
  for( i = id+1 ; i < mat_order ; i++ ){
    ...
    lwr_group[n] = &tab_node[i];
    /*
    printf("%d<-%d %d/%d\n", n, i, *nb_groups, max_groups);
    exit(-1);
    relwrsively add the next element to this group
    */
    fast_group(aff_mat, tab_node, parent, i, arity, n+1, best_val, lwr_group, nb_groups, max_groups);
...
{
  tm_tree_t **lwr_group = NULL;
  int l, i, nb_done;
  double best_val, val=0;

  lwr_group = (tm_tree_t**)MALLOC(sizeof(tm_tree_t*)*arity);
  for( l = 0 ; l < solution_size ; l++ ){
    best_val = DBL_MAX;
    nb_done = 0;
    /*printf("nb_groups%d/%d, nb_groups=%ld\n", l, M, nb_groups);*/
    /* select the best greedy grouping among the 10 first one*/
    /*fast_group(tab, tab_node, &new_tab_node[l], -1, arity, 0, &best_val, lwr_group, mat_order, &nb_done, MAX(2, (int)(50-log2(nb_groups))-M/10));*/
    fast_group(aff_mat, tab_node, &new_tab_node[l], -1, arity, 0, &best_val, lwr_group, &nb_done, MAX(10, (int)(50-CmiLog2(nb_groups))-solution_size/10));
    val += best_val;
...
static double k_partition_grouping(tm_affinity_mat_t *aff_mat, tm_tree_t *tab_node, tm_tree_t *new_tab_node, int arity, int solution_size) {
  ...
  /* new_tab_node[i]->child[j] = &tab_node[k] where 0<=i< solution size, 0<=j<arity and partition[k]=i and the jth oclwrence of i in the partition*/
 ...
 */
  ...
  if( nbg > 30000 ){
    ...
    list_all_possible_groups(cost_mat, tab_node, 0, arity, 0, lwr_group, &list);
...
Then it calls relwrsively the function to prefrom the grouping at the above level.
			     ...
			     double *obj_weight, double *comm_speed)
{
  ...
  */
  ...
  /* if we have extended the matrix with zero, free the data here as they are local to this relwrsive step only*/
  if(completed){
  ...
  if(nb_constraints && topology->constraints){
    *constraints = (int*)MALLOC(sizeof(int)*(nb_constraints));
    ...
    for(i = 0 ; i < nb_constraints ; i++){
	 ...
	 0, 0, .., 0, 1, 1, ..., 1, 2, 2, 2, ..., 2, ... where the number of identical conselwtive number is topology->oversub_fact.
