...
{
  bucket_t *bucket = bucket_list->bucket_tab[bucket_list->lwr_bucket];

  /*  display_bucket_list(bucket_list);
      printf("nb_elem: %d, indice: %d, bucket_id: %d\n",(int)bucket->nb_elem,bucket_list->bucket_indice,bucket_list->lwr_bucket);
  */
  while( bucket->nb_elem <= bucket_list->bucket_indice ){
    bucket_list->bucket_indice = 0;
    bucket_list->lwr_bucket++;
    bucket = bucket_list->bucket_tab[bucket_list->lwr_bucket];
    if(verbose_level >= DEBUG){
      printf("### From bucket %d to bucket %d\n",bucket_list->lwr_bucket-1,bucket_list->lwr_bucket);
      printf("nb_elem: %d, indice: %d, bucket_id: %d\n",(int)bucket->nb_elem,bucket_list->bucket_indice,bucket_list->lwr_bucket);
    }
  }
		     ...
		     int arity,int M)
{
  ...
  if(verbose_level >= DEBUG){
    printf("Bucket: %d, indice:%d\n",bucket_list->lwr_bucket,bucket_list->bucket_indice);
    printf("val=%f\n",val);
  }
