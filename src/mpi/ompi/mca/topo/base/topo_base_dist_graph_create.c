...
{
    int i, j, err, count, left_over, pending_reqs, lwrrent_pos, index, csize;
    ...
    for( left_over = count, lwrrent_pos = i = 0; left_over > 0; i++ ) {
        ...
        if (MPI_UNWEIGHTED != weights) {
            for( j = 0; j < ((int)how_much >> 1); j++, lwrrent_pos++ ) {
                topo->in[lwrrent_pos]  = temp[2 * j + 0 + (count - left_over)];
                topo->inw[lwrrent_pos] = temp[2 * j + 1 + (count - left_over)];
            }
        }
        left_over -= how_much;
    }
    ...
    for( left_over = count, lwrrent_pos = i = 0; left_over > 0; i++ ) {
        ...
        if (MPI_UNWEIGHTED != weights) {
            for( j = 0; j < ((int)how_much >> 1); j++, lwrrent_pos++ ) {
                topo->out[lwrrent_pos]  = temp[2 * j + 0 + (count - left_over)];
                topo->outw[lwrrent_pos] = temp[2 * j + 1 + (count - left_over)];
            }
        }
        left_over -= how_much;
    }
