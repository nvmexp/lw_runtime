...
static int mca_pml_monitoring_component_close(void)
{
    ...
    /**
     * If this component is already active, then we are lwrrently monitoring
     * the exelwtion and this call to close if the one from MPI_Finalize.
    ...
    /**
     * We are supposed to monitor the exelwtion. Save the winner PML component and
