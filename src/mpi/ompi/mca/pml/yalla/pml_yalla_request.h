...
#define PML_YALLA_INIT_MXM_RECV_REQ_ELWELOPE(_rreq, _rank, _tag, _comm) \
    { \
...
#define PML_YALLA_INIT_MXM_RECV_REQ(_rreq, _buf, _count, _dtype, _rank, _tag, _comm, _stream_type, ...) \
    { \
        ...
        PML_YALLA_INIT_MXM_RECV_REQ_ELWELOPE(_rreq, _rank, _tag, _comm); \
    }
...
#define PML_YALLA_FREE_BLOCKING_MXM_REQ(_req) \
    { \
        if ((_req)->data_type == MXM_REQ_DATA_STREAM) { \
            mca_pml_yalla_colwertor_free((mca_pml_yalla_colwertor_t*)((_req)->context)); \
        } \
    }
