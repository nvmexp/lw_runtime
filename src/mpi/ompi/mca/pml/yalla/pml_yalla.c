                                         ...
                                         void *cbdata, bool from_alloc)
{
    mxm_mem_unmap(ompi_pml_yalla.mxm_context, buf, length,
                  from_alloc ? MXM_MEM_UNMAP_MARK_ILWALID : 0);
}
...
int mca_pml_yalla_init(void)
{
    ...
    OBJ_CONSTRUCT(&ompi_pml_yalla.colws, mca_pml_yalla_freelist_t);
...
int mca_pml_yalla_cleanup(void)
{
    ...
    OBJ_DESTRUCT(&ompi_pml_yalla.colws);
...
int mca_pml_yalla_start(size_t count, ompi_request_t** requests)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        PML_YALLA_ASSERT(req->ompi.req_state != OMPI_REQUEST_ILWALID);
