...
{                                                                       \
    ...
    MCA_PML_CM_SWITCH_LWDA_COLWERTOR_OFF(flags, datatype, count);       \
    opal_colwertor_copy_and_prepare_for_send(                           \
                                             ompi_proc->super.proc_colwertor, \
                                             &(datatype->super),        \
                                             count,                     \
                                             buf,                       \
                                             flags,                     \
                                             &(req_send)->req_base.req_colwertor ); \
...
{                                                                       \
    ...
    if (opal_datatype_is_contiguous_memory_layout(&datatype->super, count)) { \
        (req_send)->req_base.req_colwertor.remoteArch =                 \
            ompi_mpi_local_colwertor->remoteArch;                       \
        (req_send)->req_base.req_colwertor.flags      =                 \
            ompi_mpi_local_colwertor->flags;                            \
        (req_send)->req_base.req_colwertor.master     =                 \
            ompi_mpi_local_colwertor->master;                           \
        (req_send)->req_base.req_colwertor.local_size =                 \
            count * datatype->super.size;                               \
        (req_send)->req_base.req_colwertor.pBaseBuf   =                 \
            (unsigned char*)buf + datatype->super.true_lb;              \
        (req_send)->req_base.req_colwertor.count      = count;          \
        (req_send)->req_base.req_colwertor.pDesc      = &datatype->super; \
    } else {                                                            \
        MCA_PML_CM_SWITCH_LWDA_COLWERTOR_OFF(flags, datatype, count);   \
        opal_colwertor_copy_and_prepare_for_send(                       \
            ompi_mpi_local_colwertor,                                   \
...
#define MCA_PML_CM_HVY_SEND_REQUEST_INIT( sendreq,                      \
    ...
    do {                                                                \
        ...
        MCA_PML_CM_HVY_SEND_REQUEST_INIT_COMMON( (&sendreq->req_send),  \
                                             ...
                                             count,                     \
                                             flags )                    \
        opal_colwertor_get_packed_size(                                 \
...
do {                                                                    \
    ...
    if(sendreq->req_count > 0) {                                        \
        ...
        if (NULL == sendreq->req_buff) {                                \
            ...
            iov_count = 1;                                              \
            opal_colwertor_pack( &sendreq->req_send.req_base.req_colwertor, \
                                 &iov,                                  \
                                 &iov_count,                            \
                                 &max_data );                           \
...
#define MCA_PML_CM_HVY_SEND_REQUEST_RETURN(sendreq)                     \
    {                                                                   \
        ...
        opal_colwertor_cleanup( &(sendreq->req_send.req_base.req_colwertor) ); \
