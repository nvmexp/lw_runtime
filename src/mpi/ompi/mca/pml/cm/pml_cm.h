                ...
                ompi_status_public_t * status)
{
    ...
    opal_colwertor_copy_and_prepare_for_recv(
                        ...
                        ompi_request_t** request)
{
     ...
     * problem oclwred because we do not have a way to differentiate an
                ...
                ompi_communicator_t* comm)
{
	...
	if (opal_datatype_is_contiguous_memory_layout(&datatype->super, count)) {
		...
		colwertor.remoteArch = ompi_mpi_local_colwertor->remoteArch;
		colwertor.flags      = ompi_mpi_local_colwertor->flags;
		colwertor.master     = ompi_mpi_local_colwertor->master;

		colwertor.local_size = count * datatype->super.size;
		colwertor.pBaseBuf   = (unsigned char*)buf + datatype->super.true_lb;
		colwertor.count      = count;
		colwertor.pDesc      = &datatype->super;
	} else
#endif
	{
