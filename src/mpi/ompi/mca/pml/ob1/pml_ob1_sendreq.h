 ...
 * Copyright (c) 2011-2012 LWPU Corporation.  All rights reserved.
...
int mca_pml_ob1_send_request_start_lwda(
    mca_pml_ob1_send_request_t* sendreq,
                                    ...
                                    mca_bml_base_btl_t* bml_btl )
{
    ...
    if (btl->btl_lwda_eager_limit && (sendreq->req_send.req_base.req_colwertor.flags & COLWERTOR_LWDA)) {
        eager_limit = btl->btl_lwda_eager_limit - sizeof(mca_pml_ob1_hdr_t);
    }
    ...
    if( OPAL_LIKELY(size <= eager_limit) ) {
        ...
        if(sendreq->req_send.req_send_mode == MCA_PML_BASE_SEND_BUFFERED) {
            rc = mca_pml_ob1_send_request_start_buffered(sendreq, bml_btl, size);
                ...
                (opal_colwertor_need_buffers(&sendreq->req_send.req_base.req_colwertor) == false) {
            unsigned char *base;
            ...
            if (sendreq->req_send.req_base.req_colwertor.flags & COLWERTOR_LWDA) {
                return mca_pml_ob1_send_request_start_lwda(sendreq, bml_btl, size);
            }
#endif /* OPAL_LWDA_SUPPORT */
            rc = mca_pml_ob1_send_request_start_rndv(sendreq, bml_btl, size, 0);
        }
