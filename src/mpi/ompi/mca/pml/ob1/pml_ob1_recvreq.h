...
static inline void prepare_recv_req_colwerter(mca_pml_ob1_recv_request_t *req)
{
    if( req->req_recv.req_base.req_datatype->super.size | req->req_recv.req_base.req_count ) {
        opal_colwertor_copy_and_prepare_for_recv(
                req->req_recv.req_base.req_proc->super.proc_colwertor,
                &(req->req_recv.req_base.req_datatype->super),
                req->req_recv.req_base.req_count,
                req->req_recv.req_base.req_addr,
                0,
                &req->req_recv.req_base.req_colwertor);
        opal_colwertor_get_unpacked_size(&req->req_recv.req_base.req_colwertor,
...
do {                                                                              \
    ...
    if(request->req_recv.req_bytes_packed > 0) {                                  \
        ...
        PERUSE_TRACE_COMM_OMPI_EVENT (PERUSE_COMM_REQ_XFER_CONTINUE,              \
                                      ...
                                      PERUSE_RECV);                               \
        opal_colwertor_set_position( &(request->req_recv.req_base.req_colwertor), \
                                     &data_offset );                              \
        opal_colwertor_unpack( &(request)->req_recv.req_base.req_colwertor,       \
                               iov,                                               \
                               &iov_count,                                        \
                               &max_data );                                       \
