...
extern void mca_pml_ob1_lwda_add_ipc_support(struct mca_btl_base_module_t* btl,
...
int mca_pml_ob1_add_procs(ompi_proc_t** procs, size_t nprocs)
{
    ...
    OPAL_LIST_FOREACH(sm, &mca_btl_base_modules_initialized, mca_btl_base_selected_module_t) {
        ...
        if (SIZE_MAX == sm->btl_module->btl_lwda_eager_limit) {
            sm->btl_module->btl_lwda_eager_limit = sizeof(mca_pml_ob1_hdr_t);
        }
        /* If size is 0, then this value is unused.  If it is non-zero then do some
         * extra checking of it. */
        if (0 != sm->btl_module->btl_lwda_eager_limit) {
            if (sm->btl_module->btl_lwda_eager_limit < sizeof(mca_pml_ob1_hdr_t)) {
                opal_show_help("help-mpi-pml-ob1.txt", "lwda_eager_limit_too_small",
                               true,
                               sm->btl_component->btl_version.mca_component_name,
                               ompi_process_info.nodename,
                               sm->btl_component->btl_version.mca_component_name,
                               sm->btl_module->btl_lwda_eager_limit,
                               sm->btl_component->btl_version.mca_component_name,
                               sizeof(mca_pml_ob1_hdr_t),
                               sm->btl_component->btl_version.mca_component_name);
                rc = OMPI_ERR_BAD_PARAM;
        ...
        if (0 == sm->btl_module->btl_lwda_rdma_limit) {
            /* All is fine.  0 means to ignore value so set to SIZE_MAX */
            sm->btl_module->btl_lwda_rdma_limit = SIZE_MAX;
        } else {
            if (sm->btl_module->btl_lwda_rdma_limit < sm->btl_module->btl_lwda_eager_limit) {
                opal_show_help("help-mpi-pml-ob1.txt", "lwda_rdma_limit_too_small",
                               true,
                               sm->btl_component->btl_version.mca_component_name,
                               ompi_process_info.nodename,
                               sm->btl_component->btl_version.mca_component_name,
                               sm->btl_module->btl_lwda_rdma_limit,
                               sm->btl_component->btl_version.mca_component_name,
                               sm->btl_module->btl_lwda_eager_limit,
                               sm->btl_component->btl_version.mca_component_name);
                rc = OMPI_ERR_BAD_PARAM;
...
#if OPAL_LWDA_SUPPORT
    if (flags & MCA_BTL_ERROR_FLAGS_ADD_LWDA_IPC) {
        mca_pml_ob1_lwda_add_ipc_support(btl, flags, (struct ompi_proc_t*)errproc, btlinfo);
        return;
    }
#endif /* OPAL_LWDA_SUPPORT */
...
int mca_pml_ob1_ft_event( int state )
{
    ...
    if(OPAL_CRS_CHECKPOINT == state) {
        ...
        if (opal_cr_continue_like_restart && !first_continue_pass) {
             ...
             *       the code that point to indv. procs in this strulwtre. For our
