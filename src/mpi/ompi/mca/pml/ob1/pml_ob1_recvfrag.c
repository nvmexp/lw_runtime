                                          ...
                                          void* cbdata )
{
    ...
    if(OPAL_LIKELY(match)) {
        ...
        if(match->req_bytes_expected > 0) {
            ...
            opal_colwertor_unpack( &match->req_recv.req_base.req_colwertor,
                                   iov,
                                   &iov_count,
                                   &bytes_received );
                                        ...
                                        void* cbdata )
{
    ...
    if ((sendreq->req_send.req_base.req_colwertor.flags & COLWERTOR_LWDA) &&
        (btl->btl_flags & MCA_BTL_FLAGS_LWDA_COPY_ASYNC_SEND)) {
        /* The user's buffer is GPU and this BTL can support asynchronous copies,
         * so adjust the colwertor accordingly.  All the subsequent fragments will
         * use the asynchronous copy. */
        void *strm = mca_common_lwda_get_dtoh_stream();
        opal_lwda_set_copy_function_async(&sendreq->req_send.req_base.req_colwertor, strm);
    }
#endif /* OPAL_LWDA_SUPPORT */
...
void mca_pml_ob1_recv_frag_callback_frag(mca_btl_base_module_t* btl,
    ...
    /* If data is destined for GPU buffer and colwertor was set up for asynchronous
     * copies, then start the copy and return.  The copy completion will trigger
     * the next phase. */
    if (recvreq->req_recv.req_base.req_colwertor.flags & COLWERTOR_LWDA_ASYNC) {
        assert(btl->btl_flags & MCA_BTL_FLAGS_LWDA_COPY_ASYNC_RECV);

        /* This will trigger the opal_colwertor_pack to start asynchronous copy. */
        ...
        des->des_flags |= MCA_BTL_DES_FLAGS_LWDA_COPY_ASYNC;
