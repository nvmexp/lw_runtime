                                  ...
                                  int status )
{
    ...
    /* Lwrrently, we cannot support a failure in the send.  In the blocking
    ...
    size_t size)
{
    ...
    iov_count = 1;
    max_data = size;
    if((rc = opal_colwertor_pack( &sendreq->req_send.req_base.req_colwertor,
                                  &iov,
                                  &iov_count,
                                  &max_data)) < 0) {
    ...
    if( OPAL_LIKELY(sendreq->req_send.req_addr == sendreq->req_send.req_base.req_addr) ) {
        ...
        /* re-init colwertor for packed data */
        opal_colwertor_prepare_for_send( &sendreq->req_send.req_base.req_colwertor,
                                         ...
                                         size_t size )
{
    ...
    if(NULL != bml_btl->btl->btl_sendi) {
        ...
        rc = mca_bml_base_sendi( bml_btl, &sendreq->req_send.req_base.req_colwertor,
        ...
        /* just in case the btl changed the colwerter, reset it */
        if (size > 0 && NULL != des) {
            MCA_PML_OB1_SEND_REQUEST_RESET(sendreq);
        }
    } else {
    ...
    if(size > 0) {
        ...
        (void)opal_colwertor_pack( &sendreq->req_send.req_base.req_colwertor,
                                            ...
                                            size_t size )
{
    ...
    mca_bml_base_prepare_src( bml_btl,
                              &sendreq->req_send.req_base.req_colwertor,
                              MCA_BTL_NO_ORDER,
                              OMPI_PML_OB1_MATCH_HDR_LEN,
                              &size,
                              MCA_BTL_DES_FLAGS_PRIORITY | MCA_BTL_DES_FLAGS_BTL_OWNERSHIP,
                                         ...
                                         size_t size )
{
    ...
    if (!(bml_btl->btl_flags & (MCA_BTL_FLAGS_GET | MCA_BTL_FLAGS_LWDA_GET))) {
        sendreq->rdma_frag = NULL;
    ...
    /* at this time ob1 does not support non-contiguous gets. the colwertor represents a
     * contiguous block of memory */
    opal_colwertor_get_lwrrent_pointer (&sendreq->req_send.req_base.req_colwertor, &data_ptr);
    ...
    mca_bml_base_alloc(bml_btl, &des, MCA_BTL_NO_ORDER, sizeof (*hdr) + reg_size,
                       ...
                       MCA_BTL_DES_FLAGS_SIGNAL);
    if( OPAL_UNLIKELY(NULL == des) ) {
        /* NTH: no need to reset the colwerter here. it will be reset before it is retried */
        MCA_PML_OB1_RDMA_FRAG_RETURN(frag);
        return OMPI_ERR_OUT_OF_RESOURCE;
    }
...
cannot_pack:
         ...
         * for colwerter to pack then send it through another BTL */
        ...
        if(bml_btl->btl->btl_max_send_size != 0) {
            ...
            if ((sendreq->req_send.req_base.req_colwertor.flags & COLWERTOR_LWDA) && (bml_btl->btl->btl_lwda_max_send_size != 0)) {
                max_send_size = bml_btl->btl->btl_lwda_max_send_size - sizeof(mca_pml_ob1_frag_hdr_t);
            } else {
        ...
        opal_colwertor_set_position(&sendreq->req_send.req_base.req_colwertor,
                                    &offset);
        ...
        mca_bml_base_prepare_src(bml_btl, &sendreq->req_send.req_base.req_colwertor,
        ...
        if( OPAL_UNLIKELY(des == NULL || size == 0) ) {
            if(des) {
                /* Colwerter can't pack this chunk. Append to another chunk
                 * from other BTL */
                mca_bml_base_free(bml_btl, des);
                range->range_btls[btl_idx].length -= data_remaining;
                goto cannot_pack;
                                   ...
                                   mca_pml_ob1_rdma_hdr_t* hdr )
{
    ...
    opal_colwertor_get_offset_pointer (&sendreq->req_send.req_base.req_colwertor,
