...
#if OPAL_LWDA_SUPPORT
int mca_pml_ob1_lwda_need_buffers(mca_pml_ob1_recv_request_t* recvreq,
    ...
    size_t bytes_received)
{
    ...
    if(hdr->hdr_msg_length > bytes_received) {
        ...
        if(opal_colwertor_need_buffers(&recvreq->req_recv.req_base.req_colwertor) == 0 &&
           hdr->hdr_match.hdr_common.hdr_flags & MCA_PML_OB1_HDR_FLAGS_CONTIG &&
           rdma_num != 0) {
            unsigned char *base;
            opal_colwertor_get_lwrrent_pointer( &recvreq->req_recv.req_base.req_colwertor, (void**)&(base) );
            ...
            if (recvreq->req_rdma_cnt != 0) {
                ...
                /* use colwertor to figure out the rdma offset for this request */
                recvreq->req_send_offset = hdr->hdr_msg_length -
                ...
                /* use colwerter to figure out the rdma offset for this
                 * request */
                opal_colwertor_set_position(&recvreq->req_recv.req_base.req_colwertor,
                                               ...
                                               mca_btl_base_descriptor_t* des)
{
     ...
     * from LWCA code about what went wrong. */
    result = mca_common_lwda_record_htod_event("pml", des);
 ...
 * can free the fragment that held that data.  This is lwrrently
 * called directly by the common LWCA code. No memchecker support
                                             ...
                                             size_t num_segments )
{
    ...
    if (opal_colwertor_need_buffers(&recvreq->req_recv.req_base.req_colwertor) == true) {
#if OPAL_LWDA_SUPPORT
        if (mca_pml_ob1_lwda_need_buffers(recvreq, btl))
#endif /* OPAL_LWDA_SUPPORT */
        {
    ...
    if (OPAL_UNLIKELY(NULL == rdma_bml)) {
        if (recvreq->req_recv.req_base.req_colwertor.flags & COLWERTOR_LWDA) {
            mca_bml_base_btl_t *bml_btl;
            bml_btl = mca_bml_base_btl_array_find(&bml_endpoint->btl_send, btl);
            /* Check to see if this is a LWCA get */
            if (bml_btl->btl_flags & MCA_BTL_FLAGS_LWDA_GET) {
                rdma_bml = bml_btl;
            }
        } else {
    ...
    if (rdma_bml->btl->btl_register_mem) {
        ...
        if (recvreq->req_recv.req_base.req_colwertor.flags & COLWERTOR_LWDA) {
            flags |= MCA_BTL_REG_FLAG_LWDA_GPU_MEM;
        }
        ...
        opal_colwertor_set_position( &recvreq->req_recv.req_base.req_colwertor, &offset);
        opal_colwertor_get_lwrrent_pointer (&recvreq->req_recv.req_base.req_colwertor, &data_ptr);
     ...
     * aclwmulates the number of bytes that were sent so far.
     *
    ...
    while (bytes_remaining > 0) {
        ...
        OPAL_THREAD_LOCK(&recvreq->lock);
        opal_colwertor_set_position( &recvreq->req_recv.req_base.req_colwertor, &offset);
        opal_colwertor_get_lwrrent_pointer (&recvreq->req_recv.req_base.req_colwertor, &frag->local_address);
        OPAL_THREAD_UNLOCK(&recvreq->lock);
...
#if OPAL_LWDA_SUPPORT /* LWDA_ASYNC_RECV */
    /* If BTL supports it and this is a LWCA buffer being received into,
     * have all subsequent FRAGS copied in asynchronously. */
    if ((recvreq->req_recv.req_base.req_colwertor.flags & COLWERTOR_LWDA) &&
        (btl->btl_flags & MCA_BTL_FLAGS_LWDA_COPY_ASYNC_RECV)) {
        void *strm = mca_common_lwda_get_htod_stream();
        opal_lwda_set_copy_function_async(&recvreq->req_recv.req_base.req_colwertor, strm);
    }
#endif
                                            ...
                                            mca_bml_base_btl_t *start_bml_btl )
{
    ...
    while(bytes_remaining > 0 &&
        ...
        /* take lock to protect colwertor against conlwrrent access
         * from unpack */
        OPAL_THREAD_LOCK(&recvreq->lock);
        opal_colwertor_set_position (&recvreq->req_recv.req_base.req_colwertor,
                                     &recvreq->req_rdma_offset);
    ...
    for (i = comm->last_probed + 1; i < comm->num_procs; i++) {
        ...
        if((frag = recv_req_match_specific_proc(req, procp[i]))) {
            *p = procp[i];
            ...
            prepare_recv_req_colwerter(req);
...
void mca_pml_ob1_recv_req_start(mca_pml_ob1_recv_request_t *req)
{
    ...
    if(req->req_recv.req_base.req_peer == OMPI_ANY_SOURCE) {
         ...
         * we can safely construct the colwertor based on the proc
