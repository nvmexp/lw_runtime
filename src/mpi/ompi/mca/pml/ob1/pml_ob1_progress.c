 ...
 */
static inline int mca_pml_ob1_process_pending_lwda_async_copies(void)
{
    mca_btl_base_descriptor_t *frag;
    int progress, count = 0;

    do {
        progress = progress_one_lwda_htod_event(&frag);
        if (1 == progress) {
...
#if OPAL_LWDA_SUPPORT
    if (opal_lwda_support)
        completed_requests += mca_pml_ob1_process_pending_lwda_async_copies();
#endif /* OPAL_LWDA_SUPPORT */
