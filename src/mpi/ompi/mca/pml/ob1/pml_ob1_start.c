...
int mca_pml_ob1_start(size_t count, ompi_request_t** requests)
{
    ...
    for (size_t i = 0 ; i < count ; ++i) {
        ...
        /* If the persistent request is lwrrently active - verify the status
        ...
        switch(pml_request->req_type) {
            case MCA_PML_REQUEST_SEND:
            {
                ...
                if (!pml_request->req_pml_complete) {
                    ...
                    size_t offset = 0;
                    /**
                     * Reset the colwertor in case we're dealing with the original
                     * request, which when completed do not reset the colwertor.
                     */
