...
size_t mca_pml_ob1_rdma_lwda_btls(
    mca_bml_base_endpoint_t* bml_endpoint,
    unsigned char* base,
    size_t size,
    mca_pml_ob1_com_btl_t* rdma_btls);

int mca_pml_ob1_lwda_need_buffers(void * rreq,
                                  mca_btl_base_module_t* btl);

void mca_pml_ob1_lwda_add_ipc_support(struct mca_btl_base_module_t* btl, int32_t flags,
                                      ompi_proc_t* errproc, char* btlinfo);

/**
 * Handle the LWCA buffer.
 */
int mca_pml_ob1_send_request_start_lwda(mca_pml_ob1_send_request_t* sendreq,
                                        mca_bml_base_btl_t* bml_btl,
                                        size_t size) {
    int rc;
#if OPAL_LWDA_GDR_SUPPORT
    /* With some BTLs, switch to RNDV from RGET at large messages */
    if ((sendreq->req_send.req_base.req_colwertor.flags & COLWERTOR_LWDA) &&
        (sendreq->req_send.req_bytes_packed > (bml_btl->btl->btl_lwda_rdma_limit - sizeof(mca_pml_ob1_hdr_t)))) {
        return mca_pml_ob1_send_request_start_rndv(sendreq, bml_btl, 0, 0);
    }
#endif /* OPAL_LWDA_GDR_SUPPORT */

    sendreq->req_send.req_base.req_colwertor.flags &= ~COLWERTOR_LWDA;
    if (opal_colwertor_need_buffers(&sendreq->req_send.req_base.req_colwertor) == false) {
        unsigned char *base;
        opal_colwertor_get_lwrrent_pointer( &sendreq->req_send.req_base.req_colwertor, (void**)&base );
        /* Set flag back */
        sendreq->req_send.req_base.req_colwertor.flags |= COLWERTOR_LWDA;
        if( 0 != (sendreq->req_rdma_cnt = (uint32_t)mca_pml_ob1_rdma_lwda_btls(
            ...
            if (bml_btl->btl_flags & MCA_BTL_FLAGS_LWDA_PUT) {
    ...
    mca_pml_ob1_com_btl_t* rdma_btls)
{
    ...
    /* shortlwt when there are no rdma capable btls */
    if(num_btls == 0) {
        return 0;
    }
        ...
        if (bml_btl->btl_flags & MCA_BTL_FLAGS_LWDA_GET) {
            ...
            if( NULL != bml_btl->btl->btl_register_mem ) {
                ...
                handle = bml_btl->btl->btl_register_mem (bml_btl->btl, bml_btl->btl_endpoint,
                                                         base, size, MCA_BTL_REG_FLAG_LWDA_GPU_MEM |
                                  ...
                                  mca_btl_base_module_t* btl)
{
    ...
    if ((recvreq->req_recv.req_base.req_colwertor.flags & COLWERTOR_LWDA) &&
        (bml_btl->btl_flags & MCA_BTL_FLAGS_LWDA_GET)) {
        recvreq->req_recv.req_base.req_colwertor.flags &= ~COLWERTOR_LWDA;
 ...
 * best thing, but this may go away if LWCA IPC is supported everywhere in the
 * future. */
void mca_pml_ob1_lwda_add_ipc_support(struct mca_btl_base_module_t* btl, int32_t flags,
                                      ompi_proc_t* errproc, char* btlinfo)
{
    ...
    /* Find the corresponding bml and adjust the flag to support LWCA get */
    for( i = 0; i < (int)ep->btl_send.arr_size; i++ ) {
        if( ep->btl_send.bml_btls[i].btl == btl ) {
            ep->btl_send.bml_btls[i].btl_flags |= MCA_BTL_FLAGS_LWDA_GET;
            opal_output_verbose(5, btl_verbose_stream,
                        "BTL %s: rank=%d enabling LWCA IPC "
                        "to rank=%d on node=%s \n",
