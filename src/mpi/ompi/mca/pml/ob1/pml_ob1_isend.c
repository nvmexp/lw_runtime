...
/**
 * Single usage request. As we allow relwrsive calls (as an
                                           ...
                                           ompi_communicator_t * comm)
{
    ...
    if (count > 0) {
        /* initialize just enough of the colwertor to avoid a SEGV in opal_colwertor_cleanup */
        OBJ_CONSTRUCT(&colwertor, opal_colwertor_t);

        /* We will create a colwertor specialized for the        */
        /* remote architecture and prepared with the datatype.   */
        opal_colwertor_copy_and_prepare_for_send (dst_proc->super.proc_colwertor,
                                                  (const struct opal_datatype_t *) datatype,
                                                  count, buf, 0, &colwertor);
        opal_colwertor_get_packed_size (&colwertor, &size);
    } else {
        size = 0;
    }
    ...
    rc = mca_bml_base_sendi (bml_btl, &colwertor, &match, OMPI_PML_OB1_MATCH_HDR_LEN,
