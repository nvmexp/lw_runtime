...
static void mca_pml_base_send_request_construct(mca_pml_base_send_request_t* request)
{
    ...
    OBJ_CONSTRUCT(&request->req_base.req_colwertor, opal_colwertor_t);
}

static void mca_pml_base_send_request_destruct(mca_pml_base_send_request_t* req)
{
    /* For each request the colwertor get cleaned after each message
     * (in the base _FINI macro). Therefore, as the colwertor is a static object
