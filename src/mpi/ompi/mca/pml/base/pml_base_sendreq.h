 ...
 * @param colwertor_flags (IN) Flags to pass to colwertor
 *
...
#define MCA_PML_BASE_SEND_REQUEST_INIT( request,                          \
   ...
   {                                                                      \
      ...
      (request)->req_bytes_packed = 0;                                    \
                                                                          \
      /* initialize datatype colwertor for this request */                \
      if( count > 0 ) {                                                   \
          OMPI_DATATYPE_RETAIN(datatype);                                 \
         /* We will create a colwertor specialized for the        */      \
         /* remote architecture and prepared with the datatype.   */      \
         opal_colwertor_copy_and_prepare_for_send(                        \
                            (request)->req_base.req_proc->super.proc_colwertor, \
                            &((request)->req_base.req_datatype->super),   \
                            (request)->req_base.req_count,                \
                            (request)->req_base.req_addr,                 \
                            colwertor_flags,                              \
                            &(request)->req_base.req_colwertor );         \
         opal_colwertor_get_packed_size( &(request)->req_base.req_colwertor, \
...
#define MCA_PML_BASE_SEND_REQUEST_FINI( request )                         \
    do {                                                                  \
        ...
        opal_colwertor_cleanup( &((request)->req_base.req_colwertor) );   \
    } while (0)
