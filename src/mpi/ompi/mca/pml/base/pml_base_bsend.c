...
int mca_pml_base_bsend_request_start(ompi_request_t* request)
{
    ...
    if(sendreq->req_bytes_packed > 0) {
        ...
        /* The colwertor is already initialized in the begining so we just have to
        ...
        if((rc = opal_colwertor_pack( &sendreq->req_base.req_colwertor,
                                      &iov,
                                      &iov_count,
                                      &max_data )) < 0) {
            return OMPI_ERROR;
        ...
        /* setup colwertor to point to packed buffer (at position zero) */
        opal_colwertor_prepare_for_send( &sendreq->req_base.req_colwertor, &(ompi_mpi_packed.dt.super),
