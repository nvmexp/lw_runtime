...
void ompi_proc_construct(ompi_proc_t* proc)
{
     ...
     * the arch of the remote nodes, we will have to set the colwertors to the correct
     * architecture.
     */
    OBJ_RETAIN( ompi_mpi_local_colwertor );
    proc->super.proc_colwertor = ompi_mpi_local_colwertor;
}


void ompi_proc_destruct(ompi_proc_t* proc)
{
    /* As all the colwertors are created with OBJ_NEW we can just call OBJ_RELEASE. All, except
     * the local colwertor, will get destroyed at some point here. If the reference count is correct
     * the local colwertor (who has the reference count increased in the datatype) will not get
     * destroyed here. It will be destroyed later when the ompi_datatype_finalize is called.
     */
    OBJ_RELEASE( proc->super.proc_colwertor );
 ...
 * remote architecture, and callwlating the locality of the process.
 */
int ompi_proc_complete_init_single (ompi_proc_t *proc)
{
     ...
     * for those elwironments where the RM provides it */
    {
        uint32_t *ui32ptr;
        ...
        if (OPAL_SUCCESS == ret) {
            /* if arch is different than mine, create a new colwertor for this proc */
            if (proc->super.proc_arch != opal_local_arch) {
                OBJ_RELEASE(proc->super.proc_colwertor);
                proc->super.proc_colwertor = opal_colwertor_create(proc->super.proc_arch, 0);
            }
...
{
    int opal_proc_hash_init_size = (ompi_process_info.num_procs < ompi_add_procs_lwtoff) ? ompi_process_info.num_procs :
        1024;
    ompi_proc_t *proc;
    int ret;
...
int ompi_proc_complete_init(void)
{
     ...
     */
    if (ompi_process_info.num_procs < ompi_add_procs_lwtoff) {
...
ompi_proc_t **ompi_proc_get_allocated (size_t *size)
{
    ...
    OPAL_LIST_FOREACH(proc, &ompi_proc_list, ompi_proc_t) {
        if (OPAL_EQUAL == ompi_rte_compare_name_fields(mask, &proc->super.proc_name, &my_name)) {
             ...
             * communicators ilwolving this proc. In such cases, we want
        ...
        if (isnew) {
...
#if OPAL_ENABLE_HETEROGENEOUS_SUPPORT
                OBJ_RELEASE(plist[i]->super.proc_colwertor);
                plist[i]->super.proc_colwertor = opal_colwertor_create(plist[i]->super.proc_arch, 0);
#else
            ...
            if (NULL != new_hostname) {
                if (0 == strcmp(ompi_proc_local_proc->super.proc_hostname, new_hostname)) {
                    plist[i]->super.proc_flags |= (OPAL_PROC_ON_NODE | OPAL_PROC_ON_LW | OPAL_PROC_ON_CLUSTER);
                }
