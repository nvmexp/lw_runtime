...
{
    int ret = MPI_SUCCESS;
    MPI_Comm comm = MPI_COMM_WORLD; /* Lwrrently ignore provided comm */
    ...
    if( OMPI_SUCCESS != ret ) {
        OBJ_RELEASE(datum);
        OMPI_ERRHANDLER_ILWOKE(comm, MPI_ERR_OTHER,
                               FUNC_NAME);
    }
     ...
     * (Old) info logic
     */
    /*opal_info_set((opal_info_t*)*info, "target", lwr_datum.target_dir);*/
