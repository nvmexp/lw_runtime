...
static int get_rsrc_lwrrent_binding(char str[OMPI_AFFINITY_STRING_MAX]);
...
static int get_layout_lwrrent_binding(char str[OMPI_AFFINITY_STRING_MAX]);
                      ...
                      char exists[OMPI_AFFINITY_STRING_MAX])
{
    ...
    memset(lwrrent_binding, 0, OMPI_AFFINITY_STRING_MAX);
    ...
    if (OMPI_SUCCESS != (ret = get_rsrc_ompi_bound(ompi_bound)) ||
        OMPI_SUCCESS != (ret = get_rsrc_lwrrent_binding(lwrrent_binding)) ||
        OMPI_SUCCESS != (ret = get_rsrc_exists(exists))) {
        return OMPI_ERRHANDLER_ILWOKE(MPI_COMM_WORLD, ret, FUNC_NAME);
    }
    break;
    case OMPI_AFFINITY_LAYOUT_FMT:
    if (OMPI_SUCCESS != (ret = get_layout_ompi_bound(ompi_bound)) ||
        OMPI_SUCCESS != (ret = get_layout_lwrrent_binding(lwrrent_binding)) ||
...
/*
 * Where is this process lwrrently bound? (prettyprint)
 */
static int get_rsrc_lwrrent_binding(char str[OMPI_AFFINITY_STRING_MAX])
{
    int ret;
...
/*
 * Where is this process lwrrently bound? (layout string)
 */
static int get_layout_lwrrent_binding(char str[OMPI_AFFINITY_STRING_MAX])
{
    int ret;
