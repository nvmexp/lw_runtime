...
struct ompi_file_t {
    ...
    /** The selected component (note that this is a union) -- we need
        ...
        components lwrrently in use by the io framework for
 ...
 * a name without a \0 at the end).  After ilwoking this function,
 ...
 * @retval OMPI_SUCCESS Always.
 *
 * Ilwoked during ompi_mpi_finalize().
 */
 ...
 *
 * This is a colwenience function, mainly for error checking in
 * top-level MPI API functions.
 */
static inline bool ompi_file_ilwalid(ompi_file_t *file)
{
    return (NULL == file ||
