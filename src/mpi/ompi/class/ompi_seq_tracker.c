...
static void ompi_seq_tracker_construct(ompi_seq_tracker_t* seq_tracker) {
    ...
    seq_tracker->seq_ids_lwrrent = (ompi_seq_tracker_range_t*)opal_list_get_end(&seq_tracker->seq_ids);
}
    ...
    uint32_t seq_id)
{
    ...
    item = seq_tracker->seq_ids_lwrrent;
    while(true) {
        if(sentinel == item) {
            return false;
        } else if(item->seq_id_high >= seq_id && item->seq_id_low <= seq_id) {
            seq_tracker->seq_ids_lwrrent = (ompi_seq_tracker_range_t*) item;
            return true;
                                                ...
                                                uint32_t seq_id)
{
    ...
    ompi_seq_tracker_range_t* item = seq_tracker->seq_ids_lwrrent;
    int8_t direction = 0; /* 1 is next, -1 is previous */
    ...
    while( true ) {
        if( item == sentinel )  {
            ...
            seq_tracker->seq_ids_lwrrent = (ompi_seq_tracker_range_t*) new_item;
            return;
...
void ompi_seq_tracker_copy(ompi_seq_tracker_t* dst, ompi_seq_tracker_t* src)
{
    ...
    for( item =  opal_list_get_first(&src->seq_ids);
        ...
        if(src->seq_ids_lwrrent == src_item) {
           dst->seq_ids_lwrrent = dst_item;
        }
    }
}
