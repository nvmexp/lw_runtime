 ...
 * overhead, since this is typically ilwoked in the critical
 ...
 * ilwoked on that request, then this request was added to the f2c
...
do {                                                                    \
    (request)->req_state = OMPI_REQUEST_ILWALID;                        \
...
/**
 * Initialize the MPI_Request subsystem; ilwoked during MPI_INIT.
 */
int ompi_request_init(void);

/**
 * Shut down the MPI_Request subsystem; ilwoked during MPI_FINALIZE.
 */
 ...
 *  the current exelwtion flow created the request, and no synchronized wait
 *  has been set.
        ...
        if( OPAL_LIKELY(with_signal) ) {
            ...
            if(!OPAL_ATOMIC_COMPARE_EXCHANGE_STRONG_PTR(&request->req_complete, &_tmp_ptr, REQUEST_COMPLETED)) {
                ompi_wait_sync_t *tmp_sync = (ompi_wait_sync_t *) OPAL_ATOMIC_SWAP_PTR(&request->req_complete,
                                                                                       REQUEST_COMPLETED);
                /* In the case where another thread conlwrrently changed the request to REQUEST_PENDING */
