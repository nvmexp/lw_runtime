    ...
    ompi_status_public_t * status)
{
    ...
    /* return status.  If it's a generalized request, we *have* to
       ilwoke the query_fn, even if the user procided STATUS_IGNORE.
       MPI-2:8.2. */
    if (OMPI_REQUEST_GEN == req->req_type) {
        ompi_grequest_ilwoke_query(req, &req->req_status);
    }
                                  ...
                                  ompi_status_public_t * status)
{
  ...
  after_sync_wait:
    ...
    if (OMPI_REQUEST_GEN == request->req_type) {
        rc = ompi_grequest_ilwoke_query(request, &request->req_status);
    }
    if (MPI_STATUS_IGNORE != status) {
 ...
 finish:
    ...
    if (MPI_STATUSES_IGNORE != statuses) {
        ...
        for( i = 0; i < count; i++, rptr++ ) {
            ...
            if (OMPI_REQUEST_GEN == request->req_type) {
                ompi_grequest_ilwoke_query(request, &request->req_status);
            }
