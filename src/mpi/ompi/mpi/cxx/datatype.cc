                                ...
                                void* extra_state, int &keyval)
{
    ...
    if (NULL != c_copy_fn && NULL != c_delete_fn) {
        ret = ompi_cxx_attr_create_keyval_type (c_copy_fn, c_delete_fn, &keyval,
                                                extra_state, 0, NULL);
        if (MPI_SUCCESS != ret) {
            return ompi_cxx_errhandler_ilwoke_comm (MPI_COMM_WORLD, ret,
    ...
    if (NULL == cxx_extra_state) {
        return ompi_cxx_errhandler_ilwoke_comm (MPI_COMM_WORLD, MPI_ERR_NO_MEM,
                                                "MPI::Datatype::Create_keyval");
    }
    ...
    if (2 != count) {
        ...
        return ompi_cxx_errhandler_ilwoke_comm (MPI_COMM_WORLD, MPI_ERR_ARG,
                                                "MPI::Datatype::Create_keyval");
    }
    ...
    // that gets ilwoked when the keyval's reference count goes to 0
