...
// This function uses OMPI types, and is ilwoked with C linkage for
// the express purpose of having a C++ entity call back the C++
// function (so that types can be colwerted, etc.).
extern "C"
void ompi_mpi_cxx_comm_errhandler_ilwoke(MPI_Comm *c_comm, int *err,
                                         const char *message, void *comm_fn)
{
    ...
    // casting it down to an (MPI::Comm&) when ilwoking the callback.
...
// function (so that types can be colwerted, etc.).
extern "C"
void ompi_mpi_cxx_file_errhandler_ilwoke(MPI_File *c_file, int *err,
...
// function (so that types can be colwerted, etc.).
extern "C"
void ompi_mpi_cxx_win_errhandler_ilwoke(MPI_Win *c_win, int *err,
...
// ilwolved if someone tries to make a 100% C application link against
...
// cite).
//
// Hence, there are two problems when trying to ilwoke the user's
// callback funcion from an MPI::Op:
//
// 1. The MPI_Datatype that the C library has must be colwerted to an
// (MPI::Datatype)
// 2. The C++ callback function must then be called with a
// (MPI::Datatype&)
//
// Some relevant facts for the dislwssion:
//
// - The main engine for ilwoking Op callback functions is in libmpi
// (i.e., in C code).
//
...
//
// So the problem remains -- how to ilwoke a C++ MPI::Op callback
// function (which only oclwrrs for user-defined datatypes, BTW) from
...
// ompi_mpi_cxx_op_intercept() is ilwoked with these arguments, it has
...
// ompi_mpi_cxx_op_intercept() is ilwoked, simply use that global
...
// ilwoked based on the corresponding datatype.  But when an MPI_Op
...
//
// 5. When the user ilwokes an function that uses the MPI::Op (or,
...
// op->o_func[1].c_fn.
//
// 6. ompi_mpi_cxx_op_intercept() is therefore ilwoked and receives
// both the (MPI_Datatype*) (which is easy to colwert to
...
// pointer to (MPI::User_function*) and ilwokes it.
//
// Wasn't that simple?
//
extern "C" void
ompi_mpi_cxx_op_intercept(void *ilwec, void *outvec, int *len,
                          MPI_Datatype *datatype, MPI_User_function *c_fn)
{
    ...
    cxx_callback(ilwec, outvec, *len, cxx_datatype);
}
...
// generalized request callbacks (colwert arguments to C++ types and
// ilwoke the C++ callback signature).
