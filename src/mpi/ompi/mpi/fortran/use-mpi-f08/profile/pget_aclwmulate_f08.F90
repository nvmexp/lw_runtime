...
subroutine PMPI_Get_aclwmulate_f08(origin_addr,origin_count,origin_datatype,&
                                  result_addr,result_count,result_datatype,&
                                  target_rank,target_disp,target_count,    &
                                  target_datatype,op,win,ierror)
   use :: mpi_f08_types, only : MPI_Datatype, MPI_Op, MPI_Win, MPI_ADDRESS_KIND
   use :: ompi_mpifh_bindings, only : ompi_get_aclwmulate_f
   implicit none
   ...
   call ompi_get_aclwmulate_f(origin_addr,origin_count,origin_datatype%MPI_VAL,&
