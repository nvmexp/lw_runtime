...
OMPI_ABSTRACT INTERFACE
  SUBROUTINE MPI_User_function(ilwec, inoutvec, len, datatype)
    USE mpi_f08_types
    USE, INTRINSIC :: ISO_C_BINDING, ONLY : C_PTR
    IMPLICIT NONE
    TYPE(C_PTR), VALUE :: ilwec, inoutvec
    INTEGER :: len
...
!
!  subroutine my_user_function( ilwec, inoutvec, len, datatype )   bind(c)
!    use, intrinsic :: iso_c_binding, only : c_ptr, c_f_pointer
!    type(c_ptr), value :: ilwec, inoutvec
!    integer, intent(in) :: len
!    type(MPI_Datatype) :: datatype
!    real, pointer :: ilwec_r(:), inoutvec_r(:)
!    if (datatype%MPI_VAL == MPI_REAL%MPI_VAL) then
!       call c_f_pointer(ilwec, ilwec_r, (/ len /) )
!       call c_f_pointer(inoutvec, inoutvec_r, (/ len /) )
!       inoutvec_r = ilwec_r + inoutvec_r
!    end if
...
OMPI_ABSTRACT INTERFACE
SUBROUTINE MPI_Datarep_colwersion_function(userbuf, datatype, count, &
