 ...
 * Fortran's version of MPI_COLWERSION_FN_NULL, and some helpful
...
/**
 * Function typedef for the colwersion function pointer in
 * MPI_REGISTER_DATAREP */
typedef void (ompi_mpi2_fortran_datarep_colwersion_fn_t)
 ...
 * MPI_COLWERSION_FN_NULL.  We need the 4 fortran compiler colwetions
 * and 1 for the "real" back-end function (even though these functions
 * are never ilwoked -- they're only used as sentinel values -- it's
 ...
 */
OMPI_DATAREP_FORTRAN_DECLARE(mpi_colwersion_fn_null, MPI_COLWERSION_FN_NULL, (char *userbuf, MPI_Fint *datatype, MPI_Fint *count, char *filebuf, MPI_Offset *position, MPI_Aint *extra_state, MPI_Fint *ierr))
 ...
 * colwenient way to check whether an argument is the sentinel value
 * MPI_COLWERSION_FN_NULL.
 */
#if OMPI_FORTRAN_CAPS
#define OMPI_IS_FORTRAN_COLWERSION_FN_NULL(addr) \
  (MPI_COLWERSION_FN_NULL == addr)
#elif OMPI_FORTRAN_PLAIN
#define OMPI_IS_FORTRAN_COLWERSION_FN_NULL(addr) \
  (mpi_colwersion_fn_null == addr)
#elif OMPI_FORTRAN_SINGLE_UNDERSCORE
#define OMPI_IS_FORTRAN_COLWERSION_FN_NULL(addr) \
  (mpi_colwersion_fn_null_ == addr)
#elif OMPI_FORTRAN_DOUBLE_UNDERSCORE
#define OMPI_IS_FORTRAN_COLWERSION_FN_NULL(addr) \
  (mpi_colwersion_fn_null__ == addr)
#else
