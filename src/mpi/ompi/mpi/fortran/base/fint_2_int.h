 ...
 */
  ...
  #define OMPI_ARRAY_NAME_COLWERT(a) a
  #define OMPI_SINGLE_NAME_COLWERT(a) a
  ...
  #define OMPI_ARRAY_NAME_COLWERT(a) c_##a
  #define OMPI_SINGLE_NAME_COLWERT(a) &c_##a
  #define OMPI_INT_2_FINT(a) a
  ...
  #define OMPI_ARRAY_FINT_2_INT_ALLOC(in, n) \
    OMPI_ARRAY_NAME_COLWERT(in) = malloc(n * sizeof(int))

  /* This is for IN/IN-OUT parameters. Does alloc and assignment */
  #define OMPI_ARRAY_FINT_2_INT(in, n) \
    do { \
      int colwerted_n = (int)(n); \
      OMPI_ARRAY_NAME_COLWERT(in) = malloc(colwerted_n * sizeof(int)); \
      while(--colwerted_n >= 0) { \
        OMPI_ARRAY_NAME_COLWERT(in)[colwerted_n] = (int) in[colwerted_n]; \
      } \
    } while (0)
  ...
  #define OMPI_2_DIM_ARRAY_FINT_2_INT(in, n, dim2) \
    do { \
      int colwerted_n = (int)(n); \
      OMPI_ARRAY_NAME_COLWERT(in) = (int (*)[dim2]) malloc(colwerted_n * sizeof(*OMPI_ARRAY_NAME_COLWERT(in))); \
      while(--colwerted_n >= 0) { \
        for(dim2_index = 0; dim2_index < dim2; ++dim2_index) { \
          OMPI_ARRAY_NAME_COLWERT(in)[colwerted_n][dim2_index] = (int)in[colwerted_n][dim2_index]; \
        } \
      } \
    } while (0)
  ...
  #define OMPI_ARRAY_FINT_2_INT_CLEANUP(in) \
    free(OMPI_ARRAY_NAME_COLWERT(in))

  /* This is for single IN parameter */
  #define OMPI_SINGLE_FINT_2_INT(in) \
    OMPI_ARRAY_NAME_COLWERT(in) = (int) *(in)

  /* This is for single OUT parameter */
  #define OMPI_SINGLE_INT_2_FINT(in) \
    *(in) = OMPI_ARRAY_NAME_COLWERT(in)
  ...
  #define OMPI_ARRAY_INT_2_FINT(in, n) \
    do { \
      int colwerted_n = (int)(n); \
      while(--colwerted_n >= 0) { \
        in[colwerted_n] = OMPI_ARRAY_NAME_COLWERT(in)[colwerted_n]; \
      } \
      free(OMPI_ARRAY_NAME_COLWERT(in)); \
    } while (0)
#else /* int > MPI_Fint  */
  ...
  #define OMPI_ARRAY_FINT_2_INT(in, n) \
    do { \
      ...
      OMPI_ARRAY_NAME_COLWERT(in) = malloc(colwerted_n * sizeof(int)); \
      while(--colwerted_n >= 0) { \
        OMPI_ARRAY_NAME_COLWERT(in)[colwerted_n] = in[colwerted_n]; \
      } \
    } while (0)
  ...
  #define OMPI_2_DIM_ARRAY_FINT_2_INT(in, n, dim2) \
    do { \
      ...
      while(--colwerted_n >= 0) { \
        for(dim2_index = 0; dim2_index < dim2; ++dim2_index) { \
          OMPI_ARRAY_NAME_COLWERT(in)[colwerted_n][dim2_index] = in[colwerted_n][dim2_index]; \
        } \
      } \
    } while (0)
  ...
  #define OMPI_SINGLE_FINT_2_INT(in) \
     OMPI_ARRAY_NAME_COLWERT(in) = *(in)

  #define OMPI_SINGLE_INT_2_FINT(in) \
    *in = (MPI_Fint) OMPI_ARRAY_NAME_COLWERT(in)
 ...
 *
 * There used to be an in-place option for the below colwersions of
 * logical arrays.  So if mpi_cart_create(..., periods, ...) took an
 * input array of Fortran logicals, it would walk the array colwerting
 ...
 * before returning.
 *
 * The in-place option is hence only valid if no colwersion is ever needed
...
#  define OMPI_LOGICAL_NAME_COLWERT(in)        in  /* Not needed for int==logical */
#  define OMPI_LOGICAL_SINGLE_NAME_COLWERT(in) in /* Not needed for int==logical */
#  define OMPI_LOGICAL_ARRAY_NAME_DECL(in)         /* Not needed for int==logical */
#  define OMPI_LOGICAL_ARRAY_NAME_COLWERT(in)  in  /* Not needed for int==logical */
...
#  define OMPI_FORTRAN_MUST_COLWERT_LOGICAL_2_INT    0
...
#else
/*
 * For anything other than Fortran-logical == C-int or some .TRUE. is not 1 in C, we have to colwert
 */
#  define OMPI_FORTRAN_MUST_COLWERT_LOGICAL_2_INT    1
#  define OMPI_LOGICAL_NAME_DECL(in)           int c_##in
#  define OMPI_LOGICAL_NAME_COLWERT(in)        c_##in
#  define OMPI_LOGICAL_SINGLE_NAME_COLWERT(in) &c_##in
#  define OMPI_LOGICAL_ARRAY_NAME_DECL(in)     int * c_##in
#  define OMPI_LOGICAL_ARRAY_NAME_COLWERT(in)  c_##in
#  define OMPI_ARRAY_LOGICAL_2_INT_ALLOC(in,n) \
      OMPI_LOGICAL_ARRAY_NAME_COLWERT(in) = malloc(n * sizeof(int))
#  define OMPI_ARRAY_LOGICAL_2_INT_CLEANUP(in) \
      free(OMPI_LOGICAL_ARRAY_NAME_COLWERT(in))

#  if OMPI_FORTRAN_VALUE_TRUE == 1
#    define OMPI_LOGICAL_2_INT(a) (int)a
#    define OMPI_INT_2_LOGICAL(a) (ompi_fortran_logical_t)a
#    define OMPI_SINGLE_INT_2_LOGICAL(a) *a=(OMPI_INT_2_LOGICAL(OMPI_LOGICAL_NAME_COLWERT(a)))
#  else
...
#  define OMPI_ARRAY_LOGICAL_2_INT(in, n) do { \
       int colwerted_n = (int)(n); \
       OMPI_ARRAY_LOGICAL_2_INT_ALLOC(in, colwerted_n + 1); \
       while (--colwerted_n >= 0) { \
         OMPI_LOGICAL_ARRAY_NAME_COLWERT(in)[colwerted_n]=OMPI_LOGICAL_2_INT(in[colwerted_n]); \
       } \
     } while (0)
#  define OMPI_ARRAY_INT_2_LOGICAL(in, n) do { \
       int colwerted_n = (int)(n); \
       while (--colwerted_n >= 0) { \
         in[colwerted_n]=OMPI_INT_2_LOGICAL(OMPI_LOGICAL_ARRAY_NAME_COLWERT(in)[colwerted_n]); \
       } \
