 ...
 *
 * This function is used to colwert "argv" in MPI_COMM_SPAWN (which is
                                ...
                                char ***argv)
{
    ...
    /* Fortran lines up strings in memory, each delimited by \0.  So
       just colwert them until we hit an extra \0. */
 ...
 *
 * This function is used to colwert "array_of_commands" in
                                    ...
                                    int string_len, char ****argv)
{
    char ***argv_array;
    int i;
    char *lwrrent_array = array;
    int ret;

    argv_array = (char ***) malloc (num_argv_arrays * sizeof(char **));

    for (i = 0; i < num_argv_arrays; ++i) {
        ret = ompi_fortran_argv_blank_f2c(lwrrent_array, string_len,
