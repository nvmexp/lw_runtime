		   ...
		   MPI_Fint *indx, MPI_Fint *status, MPI_Fint *ierr)
{
    ...
    /* Shortlwt to avoid malloc(0) if *count==0.  We're intentionally
    ...
    if (NULL == c_req) {
        c_ierr = OMPI_ERRHANDLER_ILWOKE(MPI_COMM_WORLD, MPI_ERR_NO_MEM,
                                        FUNC_NAME);
    ...
    c_ierr = PMPI_Waitany(OMPI_FINT_2_INT(*count), c_req,
                         OMPI_SINGLE_NAME_COLWERT(indx),
                         &c_status);
    if (NULL != ierr) *ierr = OMPI_INT_2_FINT(c_ierr);

    if (MPI_SUCCESS == c_ierr) {

        /* Increment index by one for fortran colwentions */

        OMPI_SINGLE_INT_2_FINT(indx);
        if (MPI_UNDEFINED != *(OMPI_SINGLE_NAME_COLWERT(indx))) {
