...
#ifndef OMPI_FORTRAN_STATUS_COLWERSION_H
#define OMPI_FORTRAN_STATUS_COLWERSION_H
 ...
 * Therefore, the Fortran layer colwerts between Fortran and C statuses
 ...
 * the overhead of colwersion in cases where we can detect that the
 ...
 * OMPI C internals are dislwssed in trac tickets 2526 and 3218 as
 ...
 * - OMPI_FORTRAN_STATUS_NEED_COLWERSION_1
 *     is sufficient, but not necessary
 *     can be evaluated at compile time
 * - OMPI_FORTRAN_STATUS_NEED_COLWERSION_2(status)
...
#define OMPI_FORTRAN_STATUS_NEED_COLWERSION_1 \
...
#if defined(__sparc) && SIZEOF_SIZE_T == 8
#define OMPI_FORTRAN_STATUS_NEED_COLWERSION_2(status) \
  ( \
    (OMPI_SIZEOF_FORTRAN_INTEGER!=SIZEOF_INT) \
    || \
    ( \
...
#if OMPI_FORTRAN_STATUS_NEED_COLWERSION_1
...
#define OMPI_FORTRAN_STATUS_SET_POINTER(c_status,c_status2,status) \
  do { \
      if (OMPI_IS_FORTRAN_STATUS_IGNORE(status)) { \
          ...
          if ( OMPI_FORTRAN_STATUS_NEED_COLWERSION_2(status) ) { \
              c_status = &c_status2; \
          } else { \
...
#endif /* OMPI_FORTRAN_STATUS_COLWERSION_H */
