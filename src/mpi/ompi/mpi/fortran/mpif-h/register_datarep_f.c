...
OMPI_GENERATE_F77_BINDINGS (PMPI_REGISTER_DATAREP,
                           ...
                           (char *datarep, ompi_mpi2_fortran_datarep_colwersion_fn_t *read_colwersion_fn, ompi_mpi2_fortran_datarep_colwersion_fn_t *write_colwersion_fn, ompi_mpi2_fortran_datarep_extent_fn_t *dtype_file_extent_fn, MPI_Aint *extra_state, MPI_Fint *ierr, int datarep_len),
                           (datarep, read_colwersion_fn, write_colwersion_fn, dtype_file_extent_fn, extra_state, ierr, datarep_len) )
#endif
#endif
...
typedef struct intercept_extra_state {
    ...
    ompi_mpi2_fortran_datarep_colwersion_fn_t *read_fn_f77;
    ompi_mpi2_fortran_datarep_colwersion_fn_t *write_fn_f77;
 ...
 * the intercept function(s) are ilwoked, who then translate the
 * arguments to Fortran and then ilwoke the registered callback
 * function.
 */
                            ...
                            MPI_Fint *ierr, int datarep_len)
{
    char *c_datarep;
    int c_ierr, ret;
    MPI_Datarep_colwersion_function *read_fn_c, *write_fn_c;
    ...
    if (NULL == intercept) {
        c_ierr = OMPI_ERRHANDLER_ILWOKE(MPI_FILE_NULL,
    ...
    if (OMPI_SUCCESS != (ret = ompi_fortran_string_f2c(datarep, datarep_len,
                                                       &c_datarep))) {
        c_ierr = OMPI_ERRHANDLER_ILWOKE(MPI_FILE_NULL, ret, FUNC_NAME);
        if (NULL != ierr) *ierr = OMPI_INT_2_FINT(c_ierr);
        return;
    }

    /* Colwert the Fortran function callbacks to C equivalents.  Use
       local intercepts if they're not MPI_COLWERSION_FN_NULL so that
       we can just call the C MPI API PMPI_Register_datarep().  If they
       *are* MPI_COLWERSION_FN_NULL, then just pass that to
       PMPI_Register_datarep so that it becomes a no-op (i.e., no
       callback is ever triggered). */
    if (OMPI_IS_FORTRAN_COLWERSION_FN_NULL(read_fn_f77)) {
        /* Can't use the MPI_COLWERSION_FN_NULL macro here because it
           is specifically not defined when compiling this file so
           that we can prototype an all-caps Fortran function */
        read_fn_c = (MPI_Datarep_colwersion_function*) 0;
    } else {
        intercept->read_fn_f77 = read_fn_f77;
        read_fn_c = read_intercept_fn;
    }
    if (OMPI_IS_FORTRAN_COLWERSION_FN_NULL(write_fn_f77)) {
        /* Can't use the MPI_COLWERSION_FN_NULL macro here because it
           is specifically not defined when compiling this file so
           that we can prototype an all-caps Fortran function */
        write_fn_c = (MPI_Datarep_colwersion_function*) 0;
    } else {
...
/*
 * C->Fortran intercept for the read colwersion.
 */
static int read_intercept_fn(void *userbuf, MPI_Datatype type_c, int count_c,
                             void *filebuf, MPI_Offset position,
                             void *extra_state)
...
/*
 * C->Fortran intercept for the write colwersion.
 */
static int write_intercept_fn(void *userbuf, MPI_Datatype type_c, int count_c,
                             void *filebuf, MPI_Offset position,
                             void *extra_state)
...
/*
 * C->Fortran intercept for the extent callwlation.
 */
