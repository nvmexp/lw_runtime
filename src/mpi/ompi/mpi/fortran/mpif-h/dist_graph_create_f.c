                              ...
                              MPI_Fint *ierr)
{
    ...
    for( i = 0; i < OMPI_FINT_2_INT(*n); i++ )
        count += OMPI_ARRAY_NAME_COLWERT(degrees)[i];
    ...
    if (OMPI_IS_FORTRAN_UNWEIGHTED(weights)) {
        ...
        c_weights = OMPI_ARRAY_NAME_COLWERT(weights);
    }


    c_ierr = PMPI_Dist_graph_create(c_comm_old, OMPI_FINT_2_INT(*n), OMPI_ARRAY_NAME_COLWERT(sources),
                                    OMPI_ARRAY_NAME_COLWERT(degrees), OMPI_ARRAY_NAME_COLWERT(destinations),
