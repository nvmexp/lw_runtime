...
JNIEXPORT void JNICALL Java_mpi_Message_init(JNIElw *e, jclass c)
{
...
JNIEXPORT jobject JNICALL Java_mpi_Message_imProbe(
        JNIElw *elw, jobject jthis, jint source, jint tag, jlong jComm)
{
    MPI_Comm comm = (MPI_Comm)jComm;
    MPI_Message message;
    MPI_Status  status;
    ...
    (*elw)->SetLongField(elw, jthis, ompi_java.MessageHandle, (jlong)message);
    return ompi_java_status_new(elw, &status);
}

JNIEXPORT jlong JNICALL Java_mpi_Message_mRecv(
        JNIElw *elw, jobject jthis, jlong jMessage, jobject buf, jboolean db,
...
JNIEXPORT jlong JNICALL Java_mpi_Message_imRecv(
        JNIElw *elw, jobject jthis, jlong jMessage,
