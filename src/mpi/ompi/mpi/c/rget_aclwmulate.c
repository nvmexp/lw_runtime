...
#pragma weak MPI_Rget_aclwmulate = PMPI_Rget_aclwmulate
#endif
#define MPI_Rget_aclwmulate PMPI_Rget_aclwmulate
#endif

static const char FUNC_NAME[] = "MPI_Rget_aclwmulate";

int MPI_Rget_aclwmulate(const void *origin_addr, int origin_count, MPI_Datatype origin_datatype,
                        ...
                        MPI_Datatype target_datatype, MPI_Op op, MPI_Win win, MPI_Request *request)
{
    ...
    if (MPI_PARAM_CHECK) {
        ...
        if (ompi_win_ilwalid(win)) {
            return OMPI_ERRHANDLER_ILWOKE(MPI_COMM_WORLD, MPI_ERR_WIN, FUNC_NAME);
        } else if (origin_count < 0 || target_count < 0) {
            rc = MPI_ERR_COUNT;
        } else if (ompi_win_peer_ilwalid(win, target_rank) &&
            ...
            if (OMPI_SUCCESS == rc) {
                ...
                if (op != &ompi_mpi_op_replace.op && op != &ompi_mpi_op_no_op.op) {
                    ...
                    /* RGET_ACLWMULATE, unlike REDUCE, can use with derived
                    ...
                    if (!ompi_op_is_valid(op, op_check_dt, &msg, FUNC_NAME)) {
                        int ret = OMPI_ERRHANDLER_ILWOKE(win, MPI_ERR_OP, msg);
                        free(msg);
                        return ret;
                    }
                }
    ...
    rc = ompi_win->w_osc_module->osc_rget_aclwmulate(origin_addr,
                                                    origin_count,
                                                    origin_datatype,
                                                    result_addr,
                                                    result_count,
