...
int MPI_Dims_create(int nnodes, int ndims, int dims[])
{
    ...
    for (i = 0, p = dims; i < ndims; ++i,++p) {
        if (*p == 0) {
            ...
            return OMPI_ERRHANDLER_ILWOKE (MPI_COMM_WORLD, MPI_ERR_DIMS,
                                           FUNC_NAME);
        } else {
            freeprocs /= *p;
        }
    ...
    if (MPI_SUCCESS != (err = getfactors(freeprocs, &nfactors, &factors))) {
       return OMPI_ERRHANDLER_ILWOKE(MPI_COMM_WORLD, err,
                                     FUNC_NAME);
    }
...
getfactors(int num, int *nfactors, int **factors) {
    ...
    *factors = (int *) malloc((unsigned) size * sizeof(int));
    ...
    i = 0;
    /* determine all oclwrences of factor 2 */
    while((num % 2) == 0) {
        num /= 2;
        (*factors)[i++] = 2;
    }
    /* determine all oclwrences of uneven prime numbers up to sqrt(num) */
    d = 3;
