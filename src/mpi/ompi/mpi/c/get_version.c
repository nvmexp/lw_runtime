...
int MPI_Get_version(int *version, int *subversion)
{
    ...
    if (MPI_PARAM_CHECK) {
        /* Per MPI-2:3.1, this function can be ilwoked before
           MPI_INIT, so we don't ilwoke the normal
        ...
        if (NULL == version || NULL == subversion) {
            /* Note that we have to check and see if we have
               ...
               normal OMPI_ERRHANDLER_ILWOKE, because the user may
               have changed the default errhandler on MPI_COMM_WORLD.
               If we have not ilwoked MPI_INIT, then just abort
            ...
            if (state >= OMPI_MPI_STATE_INIT_COMPLETED &&
                ...
                /* We have no MPI object here so call ompi_errhandle_ilwoke
                 * directly */
                return ompi_errhandler_ilwoke(NULL, NULL, -1,
