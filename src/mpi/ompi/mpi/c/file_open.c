                  ...
                  MPI_Info info, MPI_File *fh)
{
    ...
    if (MPI_PARAM_CHECK) {
        ...
        if (NULL == info || ompi_info_is_freed(info)) {
            return OMPI_ERRHANDLER_ILWOKE(MPI_COMM_WORLD, MPI_ERR_INFO,
                                          FUNC_NAME);
        } else if (ompi_comm_ilwalid(comm)) {
    ...
    /* Note that MPI-2:9.7 (p265 in the ps; p261 in the pdf) says that
       ...
       should ilwoke the default error handler on MPI_FILE_NULL.
       Hence, if we get a file handle out of ompi_file_open(), ilwoke
       the error handler on that.  If not, ilwoke the error handler on
       MPI_FILE_NULL. */
    ...
    */
    ...
    if (OMPI_SUCCESS != rc) {
        ...
        return OMPI_ERRHANDLER_ILWOKE(MPI_FILE_NULL, rc, FUNC_NAME);
    }
    opal_mutex_unlock(&ompi_mpi_file_bootstrap_mutex);
