...
#pragma weak MPI_Get_aclwmulate = PMPI_Get_aclwmulate
#endif
#define MPI_Get_aclwmulate PMPI_Get_aclwmulate
#endif

static const char FUNC_NAME[] = "MPI_Get_aclwmulate";

int MPI_Get_aclwmulate(const void *origin_addr, int origin_count, MPI_Datatype origin_datatype,
                       ...
                       MPI_Datatype target_datatype, MPI_Op op, MPI_Win win)
{
        ...
        if (ompi_win_ilwalid(win)) {
            ...
            if (OMPI_SUCCESS == rc) {
                ...
                if (op != &ompi_mpi_op_replace.op && op != &ompi_mpi_op_no_op.op) {
                    ...
                    /* GET_ACLWMULATE, unlike REDUCE, can use with derived
    ...
    rc = ompi_win->w_osc_module->osc_get_aclwmulate(origin_addr,
                                                    origin_count,
                                                    origin_datatype,
                                                    result_addr,
                                                    result_count,
