                            ...
                            int array_of_errcodes[])
{
    ...
    if ( MPI_PARAM_CHECK ) {
        ...
        if ( OMPI_COMM_IS_INTER(comm)) {
            return OMPI_ERRHANDLER_ILWOKE(comm, MPI_ERR_COMM, FUNC_NAME);
        }
    ...
    if ( MPI_PARAM_CHECK ) {
        if ( rank == root ) {
            ...
            if ( NULL == array_of_info ) {
                return OMPI_ERRHANDLER_ILWOKE(comm, MPI_ERR_INFO, FUNC_NAME);
            }
            for (i = 0; i < count; ++i) {
                ...
                if (cumulative != non_mpi) {
                    return OMPI_ERRHANDLER_ILWOKE(MPI_COMM_WORLD,
                                                  MPI_ERR_INFO,
                                                  FUNC_NAME);
                }
            }
    ...
    if (!ompi_mpi_dynamics_is_enabled(FUNC_NAME)) {
        return OMPI_ERRHANDLER_ILWOKE(comm, OMPI_ERR_NOT_SUPPORTED, FUNC_NAME);
    }
