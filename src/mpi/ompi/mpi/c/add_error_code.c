...
int MPI_Add_error_code(int errorclass, int *errorcode)
{
    ...
    if ( MPI_PARAM_CHECK ) {
        ...
        if ( ompi_mpi_errcode_is_ilwalid(errorclass) )
    ...
    if ( 0 > code ) {
        return OMPI_ERRHANDLER_ILWOKE(MPI_COMM_WORLD, MPI_ERR_INTERN,
                                      FUNC_NAME);
    }
    ...
    */
    rc  = ompi_attr_set_fint (COMM_ATTR,
                              ...
                              true);
    if ( MPI_SUCCESS != rc ) {
	return OMPI_ERRHANDLER_ILWOKE(MPI_COMM_WORLD, rc, FUNC_NAME);
    }
