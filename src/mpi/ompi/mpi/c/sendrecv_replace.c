...

{
    ...
    if ( MPI_PARAM_CHECK ) {
        ...
        } else if (dest != MPI_PROC_NULL && ompi_comm_peer_ilwalid(comm, dest)) {
            rc = MPI_ERR_RANK;
        } else if (sendtag < 0 || sendtag > mca_pml.pml_max_tag) {
            rc = MPI_ERR_TAG;
        } else if (source != MPI_PROC_NULL && source != MPI_ANY_SOURCE && ompi_comm_peer_ilwalid(comm, source)) {
            rc = MPI_ERR_RANK;
    ...
    /* initialize colwertor to unpack recv buffer */
    ...
    opal_colwertor_get_packed_size( &colwertor, &packed_size );
