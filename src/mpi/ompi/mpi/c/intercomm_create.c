                         ...
                         int tag, MPI_Comm *newintercomm)
{
    ...
    if ( MPI_PARAM_CHECK ) {
        ...
        if ( local_rank == local_leader ) {
            if ( ompi_comm_ilwalid ( bridge_comm ) ||
                 (bridge_comm->c_flags & OMPI_COMM_INTER) ) {
                OPAL_CR_EXIT_LIBRARY();
                return OMPI_ERRHANDLER_ILWOKE ( local_comm, MPI_ERR_COMM,
                                                FUNC_NAME);
            }
 ...
 err_exit:
    ...
    if ( OMPI_SUCCESS != rc ) {
        *newintercomm = MPI_COMM_NULL;
        return OMPI_ERRHANDLER_ILWOKE(local_comm, MPI_ERR_INTERN,
                                      FUNC_NAME);
    }
