                                   ...
                                   MPI_Comm *comm_dist_graph)
{
    ...
    if (MPI_PARAM_CHECK) {
        ...
        if (ompi_comm_ilwalid(comm_old)) {
            ...
            return OMPI_ERRHANDLER_ILWOKE(comm_old, MPI_ERR_ARG,
                                          "MPI_Dist_graph_create_adjacent negative degree");
        } else if ((indegree > 0 &&
                    (NULL == sources || NULL == sourceweights)) ||
                   (outdegree > 0 &&
                    (NULL == destinations || NULL == destweights))) {
            return OMPI_ERRHANDLER_ILWOKE(comm_old, MPI_ERR_ARG, "MPI_Dist_graph_create_adjacent mismatched sources or destinations");
        }
    ...
    if(OMPI_SUCCESS != (err = mca_topo_base_comm_select(comm_old, NULL,
        ...
        return OMPI_ERRHANDLER_ILWOKE(comm_old, err, FUNC_NAME);
    }
