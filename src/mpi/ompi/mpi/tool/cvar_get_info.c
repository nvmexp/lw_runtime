			...
			int *desc_len, int *bind, int *scope)
{
    ...
    do {
        ...
        if (OPAL_SUCCESS != rc) {
            rc = (OPAL_ERR_VALUE_OUT_OF_BOUNDS == rc || OPAL_ERR_NOT_FOUND == rc) ? MPI_T_ERR_ILWALID_INDEX :
                MPI_T_ERR_ILWALID;
            break;
        }
        ...
        if (OMPI_SUCCESS != rc) {
            rc = MPI_T_ERR_ILWALID;  /* can't really happen as MPI_SUCCESS is the only
        ...
        /* XXX -- TODO -- All bindings are lwrrently 0. Add support for variable binding. */
        if (NULL != bind) {
            *bind = var->mbv_bind;
        }
