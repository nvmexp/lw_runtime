...
static inline int memchecker_colwertor_call (int (*f)(void *, size_t), opal_colwertor_t* pColwertor)
{
    ...
    if( OPAL_LIKELY(pColwertor->flags & COLWERTOR_NO_OP) ) {
        ...
        f( (void *)pColwertor->pBaseBuf , pColwertor->local_size);
    } else {
        ...
        unsigned char   *source_base = pColwertor->pBaseBuf;

        if ( NULL != pColwertor->pDesc )
            stack_disp = pColwertor->pDesc->ub - pColwertor->pDesc->lb;

        for (i = 0; i < pColwertor->count; i++){
...
*/
    ...
    opal_memchecker_base_isdefined (&comm->c_lwbe_dim, sizeof(int));
