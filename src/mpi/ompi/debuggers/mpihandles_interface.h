...
struct mpidbg_comm_info_t {
    ...
    /* Fortran handle; will be MPIDBG_ERR_UNAVAILABLE if lwrrently
...
/* Setup debug information for a specific image, this must save the
   ...
   longer wants to keep information about the given exelwtable.
   ...
   MPIDBG_ERR_NOT_SUPPORTED: if the image does not support the MPIDBG
                   ...
                   will be ilwoked on this image (not even
...
/* This function will be called once when an application image that
   previously had mpidbg_init_per_image() successfully ilwoked that is
...
/* This function will be called once when an application image that
   previously had mpidbg_init_per_process() successfully ilwoked that
   is now ending (e.g., the debugger is exiting, the debugger has
   stopped exelwting this process, etc.).  This function can be used
...
/* Query function to turn a C++ handle into its equivalent C handle
   ...
   --> JMS Need more dislwssion here -- George has some opinion.  He
