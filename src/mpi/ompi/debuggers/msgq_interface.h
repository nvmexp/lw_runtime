 ...
 *              the interface.
 * Oct  1 1998 JHC: Change MQS_ILWALID_PROCESS to -1, TV would never generate
 ...
 * 4) colwert objects from target format to host format.
 *
 ...
 *
 * See below for functions to colwert codes to strings.
 */
enum {
  mqs_ok = 0,
  mqs_no_information,
...
/* Additional error codes and error string colwersion. */
enum {
    err_silent_failure  = mqs_first_user_code,

    err_no_lwrrent_communicator,
    err_bad_request,
    err_no_store,
...
/*
 * We lwrrently assume that all messages are flattened into contiguous buffers.
...
/* Given an exelwtable image look up a named type in it.
...
/* Colwert an error code from the debugger into an error message */
...
/***********************************************************************
 * Calls related to an exelwtable image.
 */
 ...
 * keep information about the given exelwtable.
 *
 * This will be called once for each exelwtable image in the parallel
 * program.
 */
...
/* Prepare to iterate over the pending operations in the lwrrently
