...
{
    if (index == MQS_ILWALID_PROCESS ||
        ((unsigned int)index) >= ((unsigned int) this->entries))
        return MQS_ILWALID_PROCESS;
                                       ...
                                       mqs_taddr_t group_base )
{
    ...
    /**
     * Now colwert the process representation into the local representation.
...
int mqs_get_communicator (mqs_process *proc, mqs_communicator *comm)
{
    ...
    if (extra->lwrrent_communicator) {
        *comm = extra->lwrrent_communicator->comm_info;
    ...
    return err_no_lwrrent_communicator;
...
int mqs_get_comm_group (mqs_process *proc, int *group_members)
{
    mpi_process_info *p_info = (mpi_process_info *)mqs_get_process_info (proc);
    mpi_process_info_extra *extra = (mpi_process_info_extra*) p_info->extra;
    communicator_t     *comm   = extra->lwrrent_communicator;
...
int mqs_next_communicator (mqs_process *proc)
{
    ...
    extra->lwrrent_communicator = extra->lwrrent_communicator->next;
    return (extra->lwrrent_communicator != NULL) ? mqs_ok : mqs_end_of_list;
                                  ...
                                  mqs_opal_list_t_pos* position, mqs_taddr_t* active_item )
{
    ...
    *active_item = position->lwrrent_item;
    if( 0 == position->lwrrent_item )
        return mqs_end_of_list;

    position->lwrrent_item =
        ompi_fetch_pointer( proc,
                            position->lwrrent_item + i_info->opal_list_item_t.offset.opal_list_next,
                            p_info );
    if( position->lwrrent_item == position->sentinel )
        position->lwrrent_item = 0;
    return mqs_ok;
...
{
    printf( "position->opal_list_t_pos.lwrrent_item = 0x%llx\n", (long long)position->opal_list_t_pos.lwrrent_item );
    ...
    printf( "position->lwrrent_item                 = 0x%llx\n", (long long)position->lwrrent_item );
                                         ...
                                         mqs_opal_free_list_t_pos* position, mqs_taddr_t free_list )
{
    ...
    position->lwrrent_item = active_allocation;
                                       ...
                                       mqs_opal_free_list_t_pos* position, mqs_taddr_t* active_item )
{
    ...
    *active_item = position->lwrrent_item;
    if( 0 == position->lwrrent_item )  /* the end ... */
        return mqs_ok;

    position->lwrrent_item += position->header_space;
    if( position->lwrrent_item >= position->upper_bound ) {
    ...
    DEBUG(VERBOSE_LISTS,("Free list actual position 0x%llx next element at 0x%llx\n",
                         (long long)*active_item, (long long)position->lwrrent_item));
    return mqs_ok;
}

static void dump_request( mqs_taddr_t lwrrent_item, mqs_pending_operation *res )
{
    ...
    printf( "\n+===============================================+\n"
        ...
        (long long)lwrrent_item, res->status, (long)res->desired_local_rank,
  ...
  rescan_requests:
    while( 1 ) {
        opal_free_list_t_next_item( proc, p_info,
                                    &extra->next_msg, &lwrrent_item );
        if( 0 == lwrrent_item ) {
            DEBUG(VERBOSE_REQ,("no more items in the %s request queue\n",
                               look_for_user_buffer ? "receive" : "send" ));
            return mqs_end_of_list;
        }
        req_valid = ompi_fetch_int( proc, lwrrent_item + i_info->ompi_request_t.offset.req_state, p_info );
        if( OMPI_REQUEST_ILWALID == req_valid ) continue;
        req_comm = ompi_fetch_pointer( proc, lwrrent_item + i_info->mca_pml_base_request_t.offset.req_comm, p_info );
        if( extra->lwrrent_communicator->comm_ptr == req_comm ) break;
        DEBUG(VERBOSE_REQ,("unmatched request (0x%llx) req_comm = %llx lwrrent_com = %llx\n",
                           (long long)lwrrent_item, (long long)req_comm,
                           (long long)extra->lwrrent_communicator->comm_ptr));
    }

    res->extra_text[0][0] = 0; res->extra_text[1][0] = 0; res->extra_text[2][0] = 0;
    res->extra_text[3][0] = 0; res->extra_text[4][0] = 0;

    req_type = ompi_fetch_int( proc, lwrrent_item + i_info->ompi_request_t.offset.req_type, p_info );
    if( OMPI_REQUEST_PML == req_type ) {
         ...
         */
        res->desired_tag =
            ompi_fetch_int( proc, lwrrent_item + i_info->mca_pml_base_request_t.offset.req_tag, p_info );
        ...
        req_type =
            ompi_fetch_int( proc, lwrrent_item + i_info->mca_pml_base_request_t.offset.req_type,
                            p_info);
        req_complete =
            ompi_fetch_bool( proc,
                             lwrrent_item + i_info->ompi_request_t.offset.req_complete,
                             p_info );
        req_pml_complete =
            ompi_fetch_bool( proc,
                             lwrrent_item + i_info->mca_pml_base_request_t.offset.req_pml_complete,
                             p_info );
        res->status = (0 == req_complete ? mqs_st_pending : mqs_st_complete);

        res->desired_local_rank  = ompi_fetch_int( proc, lwrrent_item + i_info->mca_pml_base_request_t.offset.req_peer, p_info );
        res->desired_global_rank = translate( extra->lwrrent_communicator->group,
                                              res->desired_local_rank );

        res->buffer = ompi_fetch_pointer( proc, lwrrent_item + i_info->mca_pml_base_request_t.offset.req_addr,
                                     p_info );
        ...
        ompi_datatype =
            ompi_fetch_pointer( proc,
                                lwrrent_item + i_info->mca_pml_base_request_t.offset.req_datatype, p_info );
        ...
        res->desired_length *=
            ompi_fetch_size_t( proc,
                               lwrrent_item + i_info->mca_pml_base_request_t.offset.req_count,
                               p_info );

        if( MCA_PML_REQUEST_SEND == req_type ) {
            snprintf( (char *)res->extra_text[0], 64, "Send: 0x%llx", (long long)lwrrent_item );
            req_buffer =
                ompi_fetch_pointer( proc,
                                    lwrrent_item + i_info->mca_pml_base_send_request_t.offset.req_addr,
                                    p_info );
            res->system_buffer = ( req_buffer == res->buffer ? FALSE : TRUE );
            res->actual_length =
                ompi_fetch_size_t( proc,
                                   lwrrent_item + i_info->mca_pml_base_send_request_t.offset.req_bytes_packed, p_info );
            ...
            snprintf( (char *)res->extra_text[0], 64, "Receive: 0x%llx", (long long)lwrrent_item );
            /**
             ...
             */
            res->actual_tag =
                ompi_fetch_int( proc, lwrrent_item + i_info->ompi_request_t.offset.req_status +
                                i_info->ompi_status_public_t.offset.MPI_TAG, p_info );
            if( MPI_ANY_TAG != (int)res->actual_tag ) {
                res->status = mqs_st_matched;
                res->desired_length =
                    ompi_fetch_size_t( proc,
                                       lwrrent_item + i_info->mca_pml_base_recv_request_t.offset.req_bytes_packed,
                                       p_info );
                res->actual_local_rank =
                    ompi_fetch_int( proc, lwrrent_item + i_info->ompi_request_t.offset.req_status +
                                    i_info->ompi_status_public_t.offset.MPI_SOURCE, p_info );
                res->actual_global_rank = translate( extra->lwrrent_communicator->group,
                                                  res->actual_local_rank );
            }
        } else {
            snprintf( (char *)res->extra_text[0], 64, "Unknown type of request 0x%llx", (long long)lwrrent_item );
        }
        ...
        /*res->desired_length      =
          ompi_fetch_int( proc, lwrrent_item + i_info->mca_pml_base_request_t.offset.req_count, p_info );*/

        if( (mqs_st_pending < res->status) && (MCA_PML_REQUEST_SEND != req_type) ) {  /* The real data from the status */
            res->actual_length       =
                ompi_fetch_size_t( proc, lwrrent_item + i_info->ompi_request_t.offset.req_status +
...
/***********************************************************************/
/* Colwert an error code into a printable string */
char * mqs_dll_error_string (int errcode)
{
    switch (errcode) {
    case err_silent_failure:
        return "";
    case err_no_lwrrent_communicator:
