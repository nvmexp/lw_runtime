 ...
 *
 * Note that this function will only be ilwoked for errors during the
                                       ...
                                       va_list arglist)
{
    ...
    /* Per #2152, print out in plain english if something was ilwoked
       before MPI_INIT* or after MPI_FINALIZE */
    if (state < OMPI_MPI_STATE_INIT_STARTED) {
        if (NULL != arg) {
            out("*** The %s() function was called before MPI_INIT was ilwoked.\n"
                "*** This is disallowed by the MPI standard.\n", arg);
        } else {
            out("*** An MPI function was called before MPI_INIT was ilwoked.\n"
                "*** This is disallowed by the MPI standard.\n"
                "*** Unfortunately, no further information is available on *which* MPI\n"
                "*** function was ilwoked, sorry.  :-(\n", NULL);
        }
        out("*** Your MPI job will now abort.\n", NULL);
    } else if (state >= OMPI_MPI_STATE_FINALIZE_PAST_COMM_SELF_DESTRUCT) {
        if (NULL != arg) {
            out("*** The %s() function was called after MPI_FINALIZE was ilwoked.\n"
                "*** This is disallowed by the MPI standard.\n", arg);
        } else {
            out("*** An MPI function was called after MPI_FINALIZE was ilwoked.\n"
                "*** This is disallowed by the MPI standard.\n"
                "*** Unfortunately, no further information is available on *which* MPI\n"
                "*** function was ilwoked, sorry.  :-(\n", NULL);
        }
        ...
        if (NULL != arg) {
            out("*** An error oclwrred in %s\n", arg);
        } else {
            out("*** An error oclwrred\n", NULL);
        }

        if (NULL != name) {
            /* Don't use asprintf() here because there may be stack /
               heap corruption by the time we're ilwoked, so just do
               it on the stack */
            str[0] = '\0';
            len = sizeof(str) - 1;
