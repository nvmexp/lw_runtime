...
int ompi_errhandler_ilwoke(ompi_errhandler_t *errhandler, void *mpi_object,
                           int object_type, int err_code, const char *message)
{
    ...
    /* If we got no errorhandler, then just ilwoke errors_abort */
    if (NULL == errhandler) {
    ...
    /* Figure out what kind of errhandler it is, figure out if it's
       fortran or C, and then ilwoke it */
                                   ...
                                   const char *message)
{
    ...
    /* Find the *first* request that has an error -- that's the one
       that we'll ilwoke the exception on.  In an error condition, the
    ...
    /* Since errors on requests cause them to not be freed (until we
       ...
       errors.  We only ilwoke the exception on the *first* request
       that had an error. */
    for (; i < count; ++i) {
        if (MPI_REQUEST_NULL != requests[i] &&
            MPI_SUCCESS != requests[i]->req_status.MPI_ERROR) {
            /* Ignore the error -- what are we going to do?  We're
               already going to ilwoke an exception */
            ompi_request_free(&(requests[i]));
        }
    }

    /* Ilwoke the exception */
    switch (type) {
    case OMPI_REQUEST_PML:
        return ompi_errhandler_ilwoke(mpi_object.comm->error_handler,
                                      mpi_object.comm,
                                      mpi_object.comm->errhandler_type,
                                      ec, message);
        break;
    case OMPI_REQUEST_IO:
        return ompi_errhandler_ilwoke(mpi_object.file->error_handler,
                                      mpi_object.file,
                                      mpi_object.file->errhandler_type,
                                      ec, message);
        break;
    case OMPI_REQUEST_WIN:
        return ompi_errhandler_ilwoke(mpi_object.win->error_handler,
                                      mpi_object.win,
    ...
    default:
        ...
        return ompi_errhandler_ilwoke(MPI_COMM_WORLD->error_handler,
                                      MPI_COMM_WORLD,
