...
struct ompi_errhandler_t {
    ...
    /* Function pointers.  Note that we *have* to have all 4 types
       ...
       can be ilwoked on any MPI object type, so we need callbacks for
       all of three. */
    ...
    /* Have separate callback for C++ errhandlers.  This pointer is
       ...
       bindings for Create_errhandler.  This function is ilwoked
       when eh_lang==OMPI_ERRHANDLER_LANG_CXX so that the user's
       callback function can be ilwoked with the right language
       semantics. */
...
/**
 * Table for Fortran <-> C errhandler handle colwersion
 */
 ...
 *
 * This macro directly ilwokes the ompi_mpi_errors_are_fatal_handler()
 * when an error oclwrs because MPI_COMM_WORLD does not exist (because
...
/**
 * This is the macro to ilwoke to directly ilwoke an MPI error
 * handler.
 *
 * @param mpi_object The MPI object to ilwoke the errhandler on (a
 *    comm, win, or win)
 * @param err_code The error code
 * @param message Any additional message; typically the name of the
 *    MPI function that is ilwoking the error.
 *
 * This macro is used when you want to directly ilwoke the error
 * handler.  It is exactly equivalent to calling
 * ompi_errhandler_ilwoke() directly, but is provided to have a
 * parallel invocation to OMPI_ERRHANDLER_CHECK() and OMPI_ERRHANDLER_RETURN().
 */
#define OMPI_ERRHANDLER_ILWOKE(mpi_object, err_code, message) \
  ompi_errhandler_ilwoke((mpi_object)->error_handler, \
			 (mpi_object), \
...
/**
 * Conditionally ilwoke an MPI error handler.
 *
 ...
 *    MPI function that is ilwoking the error.
 *
 * This macro will ilwoke the error handler if the return code is not
 * OMPI_SUCCESS.
 */
...
/**
 * Conditionally ilwoke an MPI error handler; if there is no error,
 * return MPI_SUCCESS.
 *
 * @param rc The return code to check
 * @param mpi_object The MPI object to ilwoke the errhandler on (a
...
#define OMPI_ERRHANDLER_RETURN(rc, mpi_object, err_code, message) \
   ...
   *
   * Ilwoked from ompi_mpi_init(); sets up the error handler interface,
   ...
   *
   * Ilwokes from ompi_mpi_finalize(); tears down the error handler
   ...
   * \internal
   *
   * This function should not be ilwoked directly; it should only be
   * ilwoked by OMPI_ERRHANDLER_ILWOKE(), OMPI_ERRHANDLER_CHECK(), or
   * OMPI_ERRHANDLER_RETURN().
   *
   * @param errhandler The MPI_Errhandler to ilwoke
   * @param mpi_object The MPI object to ilwoke the errhandler on (a
   *    comm, win, or win)
   ...
   *
   * This function ilwokes the MPI exception function on the error
   * handler.  If the errhandler was created from fortran, the error
   * handler will be ilwoked with fortran linkage.  Otherwise, it is
   * ilwoked with C linkage.
   *
   * If this function returns, it returns the err_code.  Note that it
   * may not return (e.g., for MPI_ERRORS_ARE_FATAL).
   */
  OMPI_DECLSPEC int ompi_errhandler_ilwoke(ompi_errhandler_t *errhandler, void *mpi_object,
                                           int type, int err_code, const char *message);


  /**
   * Ilwoke an MPI exception on the first request found in the array
   * that has a non-MPI_SUCCESS value for MPI_ERROR in its status.  It
   * is safe to ilwoke this function if none of the requests have an
   * outstanding error; MPI_SUCCESS will be returned.
   */
  int ompi_errhandler_request_ilwoke(int count,
