                            ...
                            ompi_communicator_t **newcomm)
{
    ...
    *newcomm = MPI_COMM_NULL;
    ...
    if (MPI_COMM_WORLD == comm) {
        ...
        rc = opal_colwert_process_name_to_string(&nstring, &pname);
        if (OPAL_SUCCESS != rc) {
            return OMPI_ERROR;
        }
        ...
        for (i=0; i < size; i++) {
            ...
            rc = opal_colwert_process_name_to_string(&nstring, &proc_name);
            if (OPAL_SUCCESS != rc) {
                if (!dense) {
                    free(proc_list);
                    proc_list = NULL;
    ...
    for (i=0; NULL != members[i]; i++) {
        ...
        if (OPAL_SUCCESS != (rc = opal_colwert_string_to_process_name(&nm->name, members[i]))) {
            OMPI_ERROR_LOG(rc);
            opal_argv_free(members);
            free(rport);
     ...
     * of process names for the remote procs - colwert it
     * into an argv array */
    ...
    if (0 < opal_list_get_size(&ilist)) {
        ...
        /* colwert the list of new procs to a proc_t array */
                   ...
                   const char *port_name)
{
    ...
    /* check potentially for:
       ...
       these procs on them
       - "elw": a newline-delimited list of elwar values to be
       placed into the app's environment (of form "foo=bar")
       - "ompi_prefix": the path to the root of the directory tree where ompi
       exelwtables and libraries can be found on all nodes
       used to spawn these procs
       - "arch": desired architecture
       - "wdir": directory, where exelwtable can be found
       - "path": list of directories where to look for the exelwtable
       ...
       - "ompi_preload_binary": move binaries to nodes prior to exelwtion
       - "ompi_preload_files": move specified files to nodes prior to exelwtion
       - "ompi_non_mpi": spawned job will not call MPI_Init
       - "ompi_param": list of MCA params to be in the spawned job's environment
       - "elw": newline (\n) delimited list of elwar values to be passed to spawned procs
    */
    ...
    /* Colwert the list of commands to list of opal_pmix_app_t */
    for (i = 0; i < count; ++i) {
        ...
        /* copy over the name of the exelwtable */
        ...
        /* Add environment variable with the contact information for the
           child processes.
        */
        opal_setelw("OMPI_PARENT_PORT", port_name, true, &app->elw);
        for (j = 0; NULL != elwiron[j]; ++j) {
            if (0 == strncmp(OPAL_MCA_PREFIX, elwiron[j], strlen(OPAL_MCA_PREFIX))) {
                opal_argv_append_nosize(&app->elw, elwiron[j]);
            }
        }
        ...
        if ( array_of_info != NULL && array_of_info[i] != MPI_INFO_NULL ) {
            ...
            /* check for elw */
            ompi_info_get (array_of_info[i], "elw", sizeof(host)-1, host, &flag);
            if ( flag ) {
                elwars = opal_argv_split(host, '\n');
                for (j=0; NULL != elwars[j]; j++) {
                    opal_argv_append_nosize(&app->elw, elwars[j]);
                }
                opal_argv_free(elwars);
            }
            ...
            if ( flag ) {
                opal_argv_append_unique_nosize(&app->elw, params, true);
            }
         ...
         * exelwtable, we assume the current working directory
         */
        if ( !have_wdir ) {
...
int ompi_dpm_open_port(char *port_name)
{
    ...
    r = opal_rand(&rnd);
    opal_colwert_process_name_to_string(&tmp, OMPI_PROC_MY_NAME);
...
int ompi_dpm_dyn_init(void)
{
    ...
    /* check for appropriate elw variable */
    tmp = getelw("OMPI_PARENT_PORT");
    if (NULL == tmp) {
        /* nothing to do */
        return OMPI_SUCCESS;
    }
