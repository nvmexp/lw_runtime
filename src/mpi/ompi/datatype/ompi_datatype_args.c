                                ...
                                int32_t cd, ompi_datatype_t* const * d, int32_t type)
{
    ...
    for( pos = 0; pos < cd; pos++ ) {
        ...
        if( !(ompi_datatype_is_predefined(d[pos])) ) {
             ...
             * (for MPI_Type_get_content and MPI_Type_get_elwelope) we have to make
                                 ...
                                 ompi_datatype_t* dest_data )
{
    ...
    /* Increase the reference count of the datatype elweloppe. This
     * prevent us from making extra copies for the elweloppe (which is mostly
     * a read only memory).
     */
    if( NULL != pArgs ) {
                                        ...
                                        const void** packed_buffer )
{
    ...
    if (NULL == packed_description) {
        ...
        if (opal_atomic_compare_exchange_strong_ptr (&datatype->packed_description, (void *) &_tmp_ptr, (void *) 1)) {
            ...
            relwrsive_buffer = packed_description;
            __ompi_datatype_pack_description( datatype, &relwrsive_buffer, &next_index );
            ...
            if (!ompi_datatype_is_predefined(datatype)) {
                 ...
                 */
                assert(args->total_pack_size >= (uintptr_t)((char*)relwrsive_buffer - (char *) packed_description));
                args->total_pack_size = (uintptr_t)((char*)relwrsive_buffer - (char *) packed_description);
            }
...
{
    ompi_datatype_t *predef = NULL, *lwrrent_type, *lwrrent_predef;
    ...
    for( i = 0; i < args->cd; i++ ) {
        lwrrent_type = args->d[i];
        if( ompi_datatype_is_predefined(lwrrent_type) ) {
            lwrrent_predef = lwrrent_type;
        } else {
            lwrrent_predef = ompi_datatype_get_single_predefined_type_from_args(lwrrent_type);
            if( NULL == lwrrent_predef ) { /* No single predefined datatype */
                return NULL;
            }
        }
#if OMPI_ENABLE_MPI1_COMPAT
        if (lwrrent_predef != MPI_LB && lwrrent_predef != MPI_UB) {
#endif
            if( NULL == predef ) {  /* This is the first iteration */
                predef = lwrrent_predef;
            } else {
                /**
