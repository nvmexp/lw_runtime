...
{
    opal_colwertor_t send_colwertor, recv_colwertor;
    struct iovec iov;
    ...
    if (rdtype->id == OMPI_DATATYPE_MPI_PACKED) {
        OBJ_CONSTRUCT( &send_colwertor, opal_colwertor_t );
        ...
        opal_colwertor_pack( &send_colwertor, &iov, &iov_count, &max_data );
        OBJ_DESTRUCT( &send_colwertor );
        return ((max_data < (size_t)rcount) ? MPI_ERR_TRUNCATE : MPI_SUCCESS);
    }

    /* If send packed. */
    if (sdtype->id == OMPI_DATATYPE_MPI_PACKED) {
        OBJ_CONSTRUCT( &recv_colwertor, opal_colwertor_t );
        ...
        opal_colwertor_unpack( &recv_colwertor, &iov, &iov_count, &max_data );
        OBJ_DESTRUCT( &recv_colwertor );
    ...
    while( !completed ) {
        ...
        iov_count = 1;
        max_data = length;
        completed |= opal_colwertor_pack( &send_colwertor, &iov, &iov_count, &max_data );
        completed |= opal_colwertor_unpack( &recv_colwertor, &iov, &iov_count, &max_data );
    }
    free( iov.iov_base );
    OBJ_DESTRUCT( &send_colwertor );
    OBJ_DESTRUCT( &recv_colwertor );
