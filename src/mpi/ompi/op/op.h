 ...
 * to ilwoke the reduction operation, it seemed like the best idea.
 */
...
/**
 * Table for Fortran <-> C op handle colwersion
 */
 ...
 *
 * Ilwoked from ompi_mpi_init(); sets up the op interface, creates
 ...
 *
 * Ilwokes from ompi_mpi_finalize(); tears down the op interface, and
 ...
 */
OMPI_DECLSPEC void ompi_op_set_java_callback(ompi_op_t *op,  void *jnielw,
...
{
    /* Check:
       - non-intrinsic ddt's cannot be ilwoked on intrinsic op's
       - if intrinsic ddt ilwoked on intrinsic op:
 ...
 * ilwoke and whether to ilwoke it with C- or Fortran-style invocation
 * methods.  If the op is intrinsic and has the operation defined for
 * dtype, the appropriate back-end function will be ilwoked.
                                  ...
                                  ompi_datatype_t * dtype)
{
     ...
     * we have to check whether to ilwoke the callback with the C,
 ...
 * the target buffer).
 *
 * This function will *only* be ilwoked on intrinsic MPI_Ops.
 *
