...
void test_keys(void)
{
    ...
    for(i = 0; i < NUM_KEYS; i++) {
        ...
        lwr_val->val = i;
        tmp = (long) rand();
        lwr_key->base = (void*) tmp;
        tmp += (long) rand();
        lwr_key->bound = (void*) tmp;
        rc = opal_rb_tree_insert(&tree, lwr_key, lwr_val);
        if(OPAL_SUCCESS != rc) {
    ...
    for(i = 0; i < NUM_KEYS; i+=2) {
        lwr_key = &(keys[i]);
        rc = opal_rb_tree_delete(&tree, lwr_key);
        if(OPAL_SUCCESS != rc) {
    ...
    for(i = 1; i < NUM_KEYS; i+=2) {
        lwr_key = &(keys[i]);
        lwr_val = (my_val_t*) opal_rb_tree_find(&tree, lwr_key);
        if(lwr_val == NULL) {
            test_failure("lookup returned NULL item");
        }
        else if(lwr_val->val != i && (lwr_val->key->base > lwr_key->base ||
                                      lwr_val->key->bound < lwr_key->base)) {
