...
static int test_lwbe_dim(int value);
...
{
    int out;
    int dim = lwbe_dim (value);

#ifdef DEBUG
    printf ("test_lwbe_dim(): value:%d expect:%d\n",
            value, dim);
#endif

    if (dim == (out = opal_lwbe_dim (value))) {
        test_success();
        return 1;
    } else {
        char * msg;
        asprintf(&msg, "Mismatch for lwbe_dim: value:%d, expected:%d got:%d\n",
                 value, dim, out);
        test_failure(msg);
        free(msg);
    }
