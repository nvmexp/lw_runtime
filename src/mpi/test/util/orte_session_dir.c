...
static bool test2(void);   /* no prefix given, ORTE_PREFIX_ELW set, one good and one bad */
...
static bool test2(void)
{
    ...
    /* use the ORTE_PREFIX_ELW variable */

    setelw("OMPI_PREFIX_ELW", "/tmp/trythis", 1);

    if (ORTE_SUCCESS != orte_session_dir(true, NULL, NULL, NULL, "test-universe", NULL, NULL)) {
	unsetelw("OMPI_PREFIX_ELW");
        return(false);
    }
...
{
    /* use the TMPDIR elwiro variable */
    clear_proc_info();

    setelw("TMPDIR", "/tmp/trythis", 1);
...
{
    /* use the TMP elwiro variable */

    clear_proc_info();

    setelw("TMP", "/tmp/trythis", 1);
...
{
    /* use the HOME elwiro variable */

    clear_proc_info();

    setelw("HOME", "/tmp/trythis", 1);
...
static bool test6(void)
{
    ...
    /* no elwiro variables set, no prefix given
