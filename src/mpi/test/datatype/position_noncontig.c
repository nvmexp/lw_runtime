                 ...
                 ddt_segment_t** segments, int* seg_count )
{
 ...
 allocate_segments:
    *segments = (ddt_segment_t*)malloc( (*seg_count) * sizeof(ddt_segment_t) );
    ...
    opal_colwertor_prepare_for_send( colwertor, &(datatype->super), count, NULL );
    ...
    for( i = 0; i < (*seg_count); i++ ) {
        ...
        opal_colwertor_set_position( colwertor, &position );
               ...
               void* buffer )
{
    ...
    colwertor = opal_colwertor_create( opal_local_arch, 0 );
    opal_colwertor_prepare_for_send( colwertor, &(datatype->super), count, buffer );
    ...
    for( i = 0; i < seg_count; i++ ) {
        ...
        iov_count = 1;
        opal_colwertor_pack( colwertor, &iov, &iov_count, &max_size );
                 ...
                 void* buffer )
{
    ...
    colwertor = opal_colwertor_create( opal_local_arch, 0 );
    opal_colwertor_prepare_for_recv( colwertor, &(datatype->super), count, buffer );
    ...
    for( i = 0; i < seg_count; i++ ) {
        ...
        iov_count = 1;
        opal_colwertor_unpack( colwertor, &iov, &iov_count, &max_size );
        if( max_size != segments[i].size ) {
