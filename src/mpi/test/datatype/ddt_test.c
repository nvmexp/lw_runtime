...
static int test_upper( unsigned int length )
{
    ...
    if( OPAL_SUCCESS != opal_colwertor_prepare_for_recv( pColw, &(pdt->super), 1, mat2 ) ) {
...
static int
local_copy_with_colwertor_2datatypes( ompi_datatype_t* send_type, int send_count,
                                      ...
                                      int chunk )
{
    ...
    if( OPAL_SUCCESS != opal_colwertor_prepare_for_send( send_colwertor, &(send_type->super), send_count, psrc ) ) {
    ...
    if( OPAL_SUCCESS != opal_colwertor_prepare_for_recv( recv_colwertor, &(recv_type->super), recv_count, pdst ) ) {
...
static int local_copy_with_colwertor( ompi_datatype_t* pdt, int count, int chunk )
{
    void *pdst = NULL, *psrc = NULL, *ptemp = NULL;
    opal_colwertor_t *send_colwertor = NULL, *recv_colwertor = NULL;
    struct iovec iov;
    ...
    send_colwertor = opal_colwertor_create( remote_arch, 0 );
    if( OPAL_SUCCESS != opal_colwertor_prepare_for_send( send_colwertor, &(pdt->super), count, psrc ) ) {
        printf( "Unable to create the send colwertor. Is the datatype committed ?\n" );
        goto clean_and_return;
    }

    recv_colwertor = opal_colwertor_create( remote_arch, 0 );
    if( OPAL_SUCCESS != opal_colwertor_prepare_for_recv( recv_colwertor, &(pdt->super), count, pdst ) ) {
        printf( "Unable to create the recv colwertor. Is the datatype committed ?\n" );
        goto clean_and_return;
    }
