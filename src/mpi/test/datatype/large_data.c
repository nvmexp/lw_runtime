...
static size_t
count_length_via_colwertor_raw(char* msg,
                               MPI_Datatype dtype, int count)
{
    ...
    opal_colwertor_prepare_for_send(pcolw, (const struct opal_datatype_t *)dtype, 1, NULL);
    while( 0 == opal_colwertor_raw(pcolw, iov, &iov_count, &length) ) {
        if( verbose ) {
...
int main(int argc, char * argv[])
{
    ...
    packed = count_length_via_colwertor_raw("1. INDEX", stype, 1);
    ...
    packed = count_length_via_colwertor_raw("2. INDEX", rtype, 1);
    ...
    packed = count_length_via_colwertor_raw("3. VECTOR", ddt, 1);
    ...
    packed = count_length_via_colwertor_raw("4. CONTIG", ddt, 1);
    opal_datatype_type_size(&ddt->super, &length);
    if( length != packed ) {
        printf("Mismatched length of packed data to datatype size (%"PRIsize_t" != %"PRIsize_t")\n",
               packed, length);
