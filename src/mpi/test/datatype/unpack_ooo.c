...
static void print_stack(opal_colwertor_t* colw)
{
    printf("Stack pos %d [colwerted %" PRIsize_t "/%" PRIsize_t "]\n",
           colw->stack_pos, colw->bColwerted, colw->local_size);
    for( uint32_t i = 0; i <= colw->stack_pos; i++ ) {
        printf( "[%u] index %d, type %s count %" PRIsize_t " disp %p\n",
                i, colw->pStack[i].index, opal_datatype_basicDatatypes[colw->pStack[i].type]->name,
                colw->pStack[i].count, (void*)colw->pStack[i].disp);
    }
    printf("\n");
}
...
static int testcase(ompi_datatype_t * newtype, size_t arr[10][2]) {
    ...
    if( OPAL_SUCCESS != opal_colwertor_prepare_for_recv( pColw, &(newtype->super), N, bar ) ) {
    ...
    for (i=0; arr[i][0] != 0; i++) {
        ...
        max_data = a.iov_len;
        pos = arr[i][1];
        opal_colwertor_set_position(pColw, &pos);
        print_stack(pColw);
        assert(arr[i][1] == pos);
