...
static int test_upper( unsigned int length )
{
    ...
    if( OMPI_SUCCESS != opal_colwertor_prepare_for_send( pColw, &(pdt->super), 1, NULL ) ) {
    ...
    for( i = total_length; i > 0; ) {
        iov_count = 5;
        max_data = 0;
        opal_colwertor_raw( pColw, iov, &iov_count, &max_data );
        i -= max_data;
    }
    GET_TIME( end );
...
/**
 * Colwersion function. They deal with datatypes in 3 ways, always making local copies.
 ...
 *  - one which use a 2 colwertors created with the same datatype
 *  - and one using 2 colwertors created from different datatypes.
 *
 */
static int local_copy_ddt_raw( ompi_datatype_t* pdt, int count, int iov_num )
{
    ...
    colwertor = opal_colwertor_create( remote_arch, 0 );
    if( OMPI_SUCCESS != opal_colwertor_prepare_for_send( colwertor, &(pdt->super), count, NULL ) ) {
        printf( "Cannot attach the datatype to a colwertor\n" );
        return OMPI_ERROR;
    }
    ...
    GET_TIME( start );
    while( 0 == opal_colwertor_raw(colwertor, iov, &iov_count, &max_data) ) {
#if 0
 ...
 * colwertor. The goal of this test is to stress the colwertor using several more or less
 * diffilwlt datatype, with a large set of segment sizes for the colwersion. It can be used
 * to highlight the raw capability of the colwertor as well as detecting datatype colwertor
 * problems.
 *
...
int main( int argc, char* argv[] )
{
    ...
    printf( "\n\n#\n * TEST ILWERSED VECTOR\n #\n\n" );
    pdt = create_ilwersed_vector( &ompi_mpi_int.dt, 10 );
