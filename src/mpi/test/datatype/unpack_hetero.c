...
int main( int argc, char* argv[] )
{
    ...
    rbuf[0] = -1; rbuf[1] = -1;
    if( OPAL_SUCCESS != opal_colwertor_prepare_for_recv( pColw, &opal_datatype_int4, 1, rbuf ) ) {
    ...
    iov_count = 1;
    max_data = 4;
    opal_unpack_general( pColw, &a, &iov_count, &max_data );
    ...
    pColw = opal_colwertor_create( remote_arch, 0 );
    rbuf[0] = -1; rbuf[1] = -1;
    if( OPAL_SUCCESS != opal_colwertor_prepare_for_recv( pColw, &opal_datatype_int4, 2, rbuf ) ) {
        printf( "Cannot attach the datatype to a colwertor\n" );
        return OPAL_ERROR;
    }
