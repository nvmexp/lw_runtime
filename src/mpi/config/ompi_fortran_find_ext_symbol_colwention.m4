...
AC_DEFUN([OMPI_FORTRAN_FIND_EXT_SYMBOL_COLWENTION], [
    AC_REQUIRE([AC_PROG_NM])
    AC_REQUIRE([AC_PROG_GREP])
    OPAL_VAR_SCOPE_PUSH([FCFLAGS_NEW LDFLAGS_NEW FLAG])

    # ilwalidate cache if result came from a run where FORTRAN was disabled
...
EOF
    ...
    # conditionally exelwted (i.e., it is not exelwted in the
