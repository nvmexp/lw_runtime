...
AC_DEFUN([OPAL_SETUP_CC],[
    ...
    dnl At this time Open MPI only needs thread local and the atomic colwenience types for C11 support. These
    ...
    AC_DEFINE_UNQUOTED([OPAL_C_HAVE_ATOMIC_COLW_VAR], [$opal_prog_cc_c11_helper_atomic_var_available],
                       [Whether C compiler support atomic colwenience variables in stdatomic.h])
