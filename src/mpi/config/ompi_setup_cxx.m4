...
AC_DEFUN([_OMPI_SETUP_CXX_COMPILER_BACKEND],[
    ...
    if test "$WANT_COVERAGE" = "1" && test "$WANT_MPI_CXX_SUPPORT" = "1"; then
        if test "$ompi_cxx_vendor" = "gnu" ; then
            ...
            AC_MSG_WARN([Code coverage functionality is lwrrently available only with GCC suite])
...
* compilers and what error resulted when the commands were exelwted) is
    ...
    if test "$ENABLE_CXX_EXCEPTIONS" = "yes"; then
        ...
        if test "$OMPI_CXX_EXCEPTIONS" = "1"; then
            ...
            AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[]], [[int i = 0;]])],
                              [AC_MSG_RESULT([yes])],
                              [AC_MSG_RESULT([no])
                               AC_MSG_WARN([C++ exception flags are different between the C and C++ compilers; this configure script cannot lwrrently handle this scenario.  Either disable C++ exception support or send mail to the Open MPI users list.])
                               AC_MSG_ERROR([*** Cannot continue])])
            AC_LANG_RESTORE
