...
AC_DEFUN([OMPI_SETUP_MPI_FORTRAN],[
    ...
    OMPI_FORTRAN_HAVE_ISO_FORTRAN_ELW=0
    ...
    # results are undefined.  We do a lwrsory check to see that FC and
    ...
    # We want to set the #define's for all of these, so ilwoke the macros
    ...
    # assumption on LOGICAL for colwersion into what C considers to be
    # a true value.
    ...
    # The iso_fortran_elw module is needed for MPI_SIZEOF
    AS_IF([test $ompi_fortran_happy -eq 1],
          [OMPI_FORTRAN_CHECK_ISO_FORTRAN_ELW(
               [OMPI_FORTRAN_HAVE_ISO_FORTRAN_ELW=1],
               [OMPI_FORTRAN_HAVE_ISO_FORTRAN_ELW=0])])
    AC_SUBST(OMPI_FORTRAN_HAVE_ISO_FORTRAN_ELW)
    ...
    # We need INTERFACE, ISO_FORTRAN_ELW, and STORAGE_SIZE() support
    # to build MPI_SIZEOF support
    AS_IF([test $ompi_fortran_happy -eq 1 && \
           test $OMPI_FORTRAN_HAVE_INTERFACE -eq 1 && \
           test $OMPI_FORTRAN_HAVE_ISO_FORTRAN_ELW -eq 1 && \
...
EOF
    ...
    # Per dislwssion on the devel list starting here:
  ...
  integer :: MPI_VAL
    ...
    AC_DEFINE_UNQUOTED([OMPI_FORTRAN_HAVE_ISO_FORTRAN_ELW],
                       [$OMPI_FORTRAN_HAVE_ISO_FORTRAN_ELW],
                       [Whether the compiler supports ISO_FORTRAN_ELW or not])
