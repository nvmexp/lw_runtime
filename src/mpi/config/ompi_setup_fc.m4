...
AC_DEFUN([OMPI_SETUP_FC],[
    ...
    # The NAG compiler is too picky about naming colwentions, so use the
    ...
    *)
    ...
    # because it implies tests that should be ilwoked by the above tests
  ...
  integer :: i
    ...
    AS_IF([test $ompi_fc_happy -eq 1],
          [OMPI_FORTRAN_FIND_EXT_SYMBOL_COLWENTION])
...
commands were exelwted) is available in the config.log file in this
directory.
**********************************************************************
EOF
...
I = 3]])],
                      [AC_MSG_RESULT([yes])],
                      [AC_MSG_RESULT([no])
                       AC_MSG_WARN([C++ exception flags are different between the C and Fortran compilers; this configure script cannot lwrrently handle this scenario.  Either disable C++ exception support or send mail to the Open MPI users list.])
