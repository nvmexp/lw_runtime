...
AC_DEFUN([ORTE_CHECK_LSF],[
    AS_IF([test -z "$orte_check_lsf_happy"],[
       ...
       AS_IF([test "$with_lsf" != "no"],[
          ...
          # LSF_INCLUDEDIR, and/or LSF_ELWDIR.
          AS_IF([test -z "$orte_check_lsf_dir" && test -z "$orte_check_lsf_libdir"],
                    [AS_IF([test ! -z "$LSF_ELWDIR" && test -z "$LSF_LIBDIR" && test -f "$LSF_ELWDIR/lsf.conf"],
                           [LSF_LIBDIR=`egrep ^LSF_LIBDIR= $LSF_ELWDIR/lsf.conf | cut -d= -f2-`])
                     AS_IF([test ! -z "$LSF_ELWDIR" && test -z "$LSF_INCLUDEDIR" && test -f "$LSF_ELWDIR/lsf.conf"],
                           [LSF_INCLUDEDIR=`egrep ^LSF_INCLUDEDIR= $LSF_ELWDIR/lsf.conf | cut -d= -f2-`])
