...
# indicates your copyright line (e.g., "cisco"), and set the elw
...
#   year.
#
# NOTE: this script lwrrently doesn't handle multi-line copyright
# statements, such as:
#
...
$my_search_name = $ELW{OMPI_COPYRIGHT_SEARCH_NAME}
    if (defined($ELW{OMPI_COPYRIGHT_SEARCH_NAME}));
$my_formal_name = $ELW{OMPI_COPYRIGHT_FORMAL_NAME}
    if (defined($ELW{OMPI_COPYRIGHT_FORMAL_NAME}));
$my_manual_list = $ELW{OMPI_COPYRIGHT_MANUAL_LIST}
    if (defined($ELW{OMPI_COPYRIGHT_MANUAL_LIST}));
