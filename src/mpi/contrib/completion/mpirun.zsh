...
_mpirun() {
    local state lwrcontext="$lwrcontext" expl lwr parameter available_components already_specified values ret=1
    typeset -A opt_args
	...
	'(-debug --debug -tv --tv)'{-debug,--debug,-tv,--tv}'[Ilwoke the user-level debugger indicated by the orte_base_user_debugger MCA parameter]' \
	...
	'(- *)'{-h,--help}'[Help message]' \
	'*'{-H,-host,--host}'[List of hosts to ilwoke processes on]:hostnames:' \
	...
	'(-path --path)'{-path,--path}'[PATH to be used to look for exelwtables to start processes]:path:_files -/' \
	...
	'(-staged --staged)'{-staged,--staged}'[Used staged exelwtion if inadequate resources are present (cannot support MPI jobs)]' \
	...
	mca_variable_name)
		...
		available_components=($(_set_remove "${mca_component_names[*]}" "$(echo $lwr | tr ',' '\n')"))
		compset -P '*,'
		compset -S ',*'
