...
_mpirun() {
    local lwr prv tb switches already_specified all_variables avail_variables enumerations save_IFS
...
"
        ...
        COMPREPLY=($(_fuzzy_search "$lwr" "${avail_variables[*]}"))
        ...
        if test "${prv#_}" = "${prv}" ; then
            ...
            enumerations=($(_set_remove "$(_get_mca_component_names $prv)" "$(echo $lwr | tr ',' '\n')"))

            # Prepend the current selection if there is one
            if test "${lwr%,*}" = "${lwr}" ; then
                COMPREPLY=($(_fuzzy_search "${lwr##*,}" "${enumerations[*]}"))
            else
                COMPREPLY=($(_fuzzy_search "${lwr##*,}" "${enumerations[*]}" | sed "s/^/${lwr%,*},/g"))
            fi
        else
        ...
        COMPREPLY=($(compgen -W "none hwthread core socket numa board" -- "$lwr"))
    elif test "${prv}" = "--map-by" -o "${prv}" = "-map-by" ; then
        COMPREPLY=($(compgen -W "slot hwthread core socket numa board node" -- "$lwr"))
    elif test "${prv##*-}" = "hostfile" ; then
        COMPREPLY=($(compgen -f -- "$lwr"))
    elif test "${lwr:0:1}" = "-" ; then
        switches=$(_get_mpirun_switches)
        COMPREPLY=($(compgen -W "$switches" -- "$lwr"))
    else
        COMPREPLY=($(compgen -f -- "$lwr"))
    fi
