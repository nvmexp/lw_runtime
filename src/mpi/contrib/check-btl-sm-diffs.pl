...
# colwerts all the BTL specific strings in the BTL to be compared to
# "sm".  For example, with the smlwda, all "smlwda" strings are colwerted
# to "sm" and all "SMLWDA" strings are colwerted to "SM".  In this way,
# we avoid any spurious differences just related to the difference in
# the names of the functions and variables.
#
# Lastly, in the case of smlwda only, it can strip out all code within
...
#
# Do not remove the SMLWDA specific code
#  > check-ob1-pml-diffs.pl -s
#
# Do not remove the SMLWDA specific code and save results in DIFFS
...
Runs a diff between the the files in the sm and the smlwda directory
...
all LWCA specific code in the smlwda files.  Specifically, the script
removes all code that is within the following ifdefs.
#ifdef OMPI_LWDA_SUPPORT
 ...
#endif /*OMPI_LWDA_SUPPORT */
To view the smlwda specific code in the diff, run with the -s switch.

-s   Show all the differences between the files.
-S   Show all the differences between the files that are not
     within \"\#ifdef OMPI_LWDA_SUPPORT\" statements.  (default: -S)
-p   BTL - which BTL to compare to sm (default: smlwda)
...
# Copy smlwda files into temp directory.
@smlwdafiles = <*.[h|c]>;
foreach $smlwdafile (@smlwdafiles) {
  copy ($smlwdafile, $diffdir);
}
if ($verbose_arg) {
...
# Using crude preprocessor, strip out all SMLWDA specific code.
# If -s switch is provided, then leave SMLWDA specific code.
foreach $smlwdafile (@smlwdafiles) {
  $contents = Read($smlwdafile);
  die("Couldn't Read $smlwdafile!\n") if (!$contents);

  if (!$showall_arg) {
    # First, remove all the #if-#else code.
    # #ifdef OPAL_LWDA_SUPPORT
    # ...stuff...
    ...
    #   2. Added the comment OPAL_LWDA_SUPPORT on the #endif to get the right match.
    #   3. Need the \n at the end to avoid leaving extra newlines.
    $contents =~ s/#if OPAL_LWDA_SUPPORT(.*?)((#else \/\* OPAL_LWDA_SUPPORT \*\/\n)|(#endif \/\* OPAL_LWDA_SUPPORT \*\/\n))//gis;
    $contents =~ s/#endif \/\* OPAL_LWDA_SUPPORT \*\/\n//gis;
  }
  ...
  Write($smlwdafile, $contents);
}
if ($verbose_arg) {
  print "All $btl specific code and copyrights has been removed from $btl files\n";
  print "All $btl/$BTL strings colwerted to sm/SM strings in bfo files\n";
}
...
foreach $smfile (@smfiles) {
  ...
  $contents =~ s/\/\*(.*?)\$HEADER\$\n \*\/\n//is;
  # Strip away KNEM as that is not in smlwda
...
foreach $smfile (@smfiles) {
  ...
  close(CMD);
  if ($output_arg eq "") {
    print "Files Compared: $smfile and $smlwdafile\n";
    print "$cmd_output";
  } else {
