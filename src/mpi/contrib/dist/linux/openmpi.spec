...
# The location of the mpi-selector exelwtable (can be a relative path
...
# opal/mca/installdirs/config for pkgdatadir; there is lwrrently no
...
if test "$CC" != ""; then
    ...
    # partilwlarly in the case of multword values for $CC
    eval "set $CC"
...
module-whatis   "Sets up Open MPI/SHMEM v%{version} in your elwiornment"
...
if ("1" == "\$?LD_LIBRARY_PATH") then
    if ("\$LD_LIBRARY_PATH" !~ *%{_libdir}*) then
        setelw LD_LIBRARY_PATH %{_libdir}:\${LD_LIBRARY_PATH}
    endif
else
    setelw LD_LIBRARY_PATH %{_libdir}
endif

# MANPATH
if ("1" == "\$?MANPATH") then
    if ("\$MANPATH" !~ *%{_mandir}*) then
        setelw MANPATH %{_mandir}:\${MANPATH}
    endif
else
    setelw MANPATH %{_mandir}:
endif

# MPI_ROOT
setelw MPI_ROOT %{_prefix}
EOF
%endif
...
# exelwtables (no man pages, no doc files, no header files).  Do *not*
# include wrapper compilers.
cat all.files | egrep '/lib/|/lib64/|/lib32/|/bin/|/etc/|/help-' > tmp.files | /bin/true
# Snip out a bunch of exelwtables (e.g., wrapper compilers, pkgconfig
...
- No longer explicitly list the bin dir exelwtables in the multi-RPM
  sub-packages
...
- Add missing exelwtables to specfile (ompi-server, etc.)
...
- Add more "ofed" shortlwt qualifiers
...
- Remove some outdated "rm -f" lines for exelwtables that we no longer ship
