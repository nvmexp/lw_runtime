...
module-whatis   \"Sets up Open MPI v%(ompi_version)s in your elwiornment\"
...
if (\"`echo $PATH | grep %%{_prefix}/bin`\") then
    setelw PATH \${PATH}:%%{_prefix}/bin/
endif
if (\"$?LD_LIBRARY_PATH\") then
    if (\"`echo $LD_LIBRARY_PATH | grep %%{_libdir}`\") then
        setelw LD_LIBRARY_PATH \${LD_LIBRARY_PATH}:%%{_libdir}
    endif
endif
if (\"$?MANPATH\") then
    if (\"`echo $MANPATH | grep %%{_mandir}`\") then
        setelw MANPATH \${MANPATH}:%%{_mandir}
    endif
endif
EOF
fi
