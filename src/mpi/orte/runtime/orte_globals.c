...
/* elwars to forward */
char **orte_forwarded_elwars = NULL;
...
static void orte_app_context_destructor(orte_app_context_t* app_context)
{
    ...
    /* argv and elw lists created by util/argv copy functions */
    if (NULL != app_context->argv) {
        opal_argv_free(app_context->argv);
        app_context->argv = NULL;
    }

    if (NULL != app_context->elw) {
        opal_argv_free(app_context->elw);
        app_context->elw = NULL;
    }
...
static void orte_job_construct(orte_job_t* job)
{
    ...
    job->originator.jobid = ORTE_JOBID_ILWALID;
    job->originator.vpid = ORTE_VPID_ILWALID;
...
static void orte_job_destruct(orte_job_t* job)
{
    ...
    if (NULL != orte_job_data && ORTE_JOBID_ILWALID != job->jobid) {
...
{
    proc->name = *ORTE_NAME_ILWALID;
    proc->pid = 0;
    proc->local_rank = ORTE_LOCAL_RANK_ILWALID;
    proc->node_rank = ORTE_NODE_RANK_ILWALID;
    proc->app_rank = -1;
...
static void orte_job_map_construct(orte_job_map_t* map)
{
    ...
    map->daemon_vpid_start = ORTE_VPID_ILWALID;
    map->num_nodes = 0;
