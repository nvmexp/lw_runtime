...
{
    return orte_util_colwert_string_to_process_name(name, name_string);
}

static int _colwert_process_name_to_string(char** name_string,
                                          const opal_process_name_t *name)
{
    return orte_util_colwert_process_name_to_string(name_string, name);
}

static int
_colwert_string_to_jobid(opal_jobid_t *jobid, const char *jobid_string)
{
    return orte_util_colwert_string_to_jobid(jobid, jobid_string);
}
/*
...
orte_process_name_t orte_name_ilwalid = {ORTE_JOBID_ILWALID, ORTE_VPID_ILWALID};
...
int orte_init(int* pargc, char*** pargv, orte_proc_type_t flags)
{
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "initializations");
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "opal_init");
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "orte_locks_init");
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "orte_register_params");
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "orte_show_help_init");

    /* register handler for errnum -> string colwersion */
    opal_error_register("ORTE", ORTE_ERR_BASE, ORTE_ERR_MAX, orte_err2str);

    OPAL_TIMING_ELW_NEXT(tmng, "opal_error_register");
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "orte_proc_info");
    ...
    if (ORTE_PROC_IS_DAEMON || ORTE_PROC_IS_HNP) {
        ...
        OPAL_TIMING_ELW_NEXT(tmng, "pmix_server_register_params");
    }
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "framework_open(schizo)");
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "orte_schizo_base_select");

    /* if we are an app, let SCHIZO help us determine our environment */
    if (ORTE_PROC_IS_APP) {
        (void)orte_schizo.check_launch_elwironment();
        OPAL_TIMING_ELW_NEXT(tmng, "orte_schizo.check_launch_elwironment");
    }
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "framework_open(ess)");
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "orte_ess_base_select");
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "orte_ess.init");
    ...
    OPAL_TIMING_ELW_NEXT(tmng, "finalize");
    /* All done */
    return ORTE_SUCCESS;
