...
ORTE_DECLSPEC extern bool orte_exelwte_quiet;  /* instantiated in orte/runtime/orte_globals.c */
...
/* Shortlwt for some commonly used names */
...
#define ORTE_NAME_ILWALID       (&orte_name_ilwalid)
ORTE_DECLSPEC extern orte_process_name_t orte_name_ilwalid;  /** instantiated in orte/runtime/orte_init.c */
 ...
 * The macro provides a colwenient way of doing this
...
* defining it - resolves potential cirlwlar definition
*/
struct orte_proc_t;
...
typedef struct {
    ...
    char  **argv;
    /** Standard elwiron-style array, including a final NULL pointer */
    char  **elw;
...
typedef struct {
    ...
    /** A "hard" limit (if set -- a value of 0 implies no hard limit)
        ...
        node. This is for some elwironments (e.g. grid) there may be
...
struct orte_proc_t {
    ...
    /* pointer to the node where this proc is exelwting */
    orte_node_t *node;
    /* RML contact info */
    char *rml_uri;
    /* some boolean flags */
...
ORTE_DECLSPEC extern int orte_hostname_lwtoff;
...
ORTE_DECLSPEC extern char **orte_launch_elwiron;
...
/* elwars to forward */
ORTE_DECLSPEC extern char **orte_forwarded_elwars;
