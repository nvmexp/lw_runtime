                             ...
                             int32_t num_vals, opal_data_type_t type)
{
    ...
    for (i=0; i < num_vals; i++) {
        ...
        /* pack the number of entries in the elwiro array */
        count = opal_argv_count(app_context[i]->elw);
        if (ORTE_SUCCESS != (rc = opal_dss_pack_buffer(buffer, (void*)(&count), 1, ORTE_STD_CNTR))) {
            ORTE_ERROR_LOG(rc);
            return rc;
        }

        /* if there are entries, pack the elwiro entries */
        if (0 < count) {
            if (ORTE_SUCCESS != (rc = opal_dss_pack_buffer(buffer,
                                                           (void*)(app_context[i]->elw), count, OPAL_STRING))) {
                ORTE_ERROR_LOG(rc);
                return rc;
            }
        }
                             ...
                             int32_t num_vals, opal_data_type_t type)
{
    ...
    for (i=0; i < num_vals; i++) {
        ...
        /* pack the number of nodes ilwolved in the job */
