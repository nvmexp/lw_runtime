                               ...
                               int32_t *num_vals, opal_data_type_t type)
{
    ...
    for (i=0; i < *num_vals; i++) {
        ...
        /* if there are elw strings, allocate the required space for the char * pointers */
        if (0 < count) {
            app_context[i]->elw = (char **)malloc((count+1) * sizeof(char*));
            if (NULL == app_context[i]->elw) {
                ORTE_ERROR_LOG(ORTE_ERR_OUT_OF_RESOURCE);
                return ORTE_ERR_OUT_OF_RESOURCE;
            }
            app_context[i]->elw[count] = NULL;

            /* and unpack them */
            max_n = count;
            if (ORTE_SUCCESS != (rc = opal_dss_unpack_buffer(buffer, app_context[i]->elw, &max_n, OPAL_STRING))) {
                ORTE_ERROR_LOG(rc);
                return rc;
            }
        }
                       ...
                       int32_t *num_vals, opal_data_type_t type)
{
    ...
    for (i=0; i < *num_vals; i++) {
        ...
        /* unpack the number of nodes ilwolved in the job */
        n = 1;
        if (ORTE_SUCCESS != (rc = opal_dss_unpack_buffer(buffer,
                        ...
                        opal_data_type_t type)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        case OPAL_NAME:
            ...
            break;
        case OPAL_ELWAR:
            if (OPAL_SUCCESS != (ret = opal_dss_unpack_buffer(buffer, &ptr[i]->data.elwar, &m, OPAL_ELWAR))) {
                return ret;
            }
            break;
