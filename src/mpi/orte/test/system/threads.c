...
int main(int argc, char **argv) {
    ...
    int rc, i, max_iters = 10, lwr_iter;
    my_value_t *val = NULL;
    int mode;
     ...
     */
    acc_time = 0.0;
    for(lwr_iter = 0; lwr_iter < max_iters; ++lwr_iter) {
        perc_lwrrent = (lwr_iter / ((double)max_iters)) * 100.0;
        if( perc_lwrrent > perc_report_after ) {
            delta = (acc_time / lwr_iter) * 1000000;
            printf("%6.1f %% complete : Iteration %10d / %10d : %6.1f usec / iter\n",
                   perc_lwrrent, lwr_iter+1, max_iters, delta);
        ...
        if( NULL == val ) {
            printf("[%10d / %10d] Error: val = %s\n", lwr_iter+1, max_iters,
                   (NULL == val ? "NULL" : "Valid") );
            exit(-1);
        }
        else if( 999 != val->type ) {
            printf("[%10d / %10d] Error: val->type = %d\n", lwr_iter+1, max_iters, val->type);
            exit(-1);
        }
        else if( (lwr_iter+1)%MAX_VAL != val->data.int16 ) {
            printf("[%10d / %10d] Error: val->data.int16 = %d\n", lwr_iter+1, max_iters, val->data.int16);
            exit(-1);
        }
...
void *value_xfer_thread(void *arg) {
    ...
    while( !time_to_stop ) {
        if( handoff.working ) {
            ...
            // See 'Examples' -> 'Global thread flag' dislwssion here:
