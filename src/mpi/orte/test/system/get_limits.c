...
#if 0
int nfds_needed = callwlate_nfds_needed (nprocs);
if (nfds_needed > rlim->rlim_lwr) {
    if (nfds_needed <= rlim->rlim_max)
        rlim->rlim_lwr = rlim->rlim_max;
    if (setrlimit (RLIMIT_NOFILE, rlim) < 0)
        fprintf (stderr, "setrlimit (RLIMIT_NOFILE, lwr = %d): %m\n";
                 else
