    ...
    if (1 == ORTE_LOCAL_JOBID(ORTE_PROC_MY_NAME->jobid)) {
        ...
        while (read_active) {
            ...
            if (5 == rc) {
                ...
                orte_dfs.seek(fd, OFFSET_VALUE, SEEK_LWR, dfs_seek_cbfunc, &i);
                ORTE_WAIT_FOR_COMPLETION(active);
            }
            active = true;
        }
        ...
        cnt = 1;
        if (OPAL_SUCCESS != (rc = opal_dss.unpack(buf, &lwpids, &cnt, OPAL_INT32))) {
            ORTE_ERROR_LOG(rc);
            return 1;
        }

        opal_output(0, "%s RECVD DATA FROM %d VPIDS",
                    ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), lwpids);
