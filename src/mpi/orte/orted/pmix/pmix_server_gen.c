            ...
            if (0 == strcmp(q->keys[n], OPAL_PMIX_QUERY_NAMESPACES)) {
                ...
                for (k=0; k < jdata->procs->size; k++) {
                    ...
                    if (NULL != app && NULL != app->app) {
                        kv->data.pinfo.exelwtable_name = strdup(app->app);
                    }
...
static void _toolconn(int sd, short args, void *cbdata)
{
    ...
    char *hostname = NULL;
    orte_process_name_t tool = {ORTE_JOBID_ILWALID, ORTE_VPID_ILWALID};
    int rc, i;
    opal_value_t *val;
   ...
   if (NULL != cd->info) {
    ...
    if (ORTE_JOBID_ILWALID == tool.jobid ||
        ...
        if (ORTE_PROC_IS_HNP || ORTE_PROC_IS_MASTER) {
            ...
            /* we lwrrently do not support connections to non-HNP/master
                            ...
                            void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(val, info, opal_value_t) {
        ...
        if (0 == strcmp(val->key, OPAL_PMIX_JOB_CTRL_KILL)) {
            /* colwert the list of targets to a pointer array */
            if (0 == opal_list_get_size(targets)) {
                ptrarray = NULL;
            } else {
