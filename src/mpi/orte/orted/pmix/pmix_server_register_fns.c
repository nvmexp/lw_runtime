...
int orte_pmix_server_register_nspace(orte_job_t *jdata, bool force)
{
    ...
    for (n=0; n < map->nodes->size; n++) {
        ...
        for (i=0; i < node->procs->size; i++) {
            ...
            if (map->num_nodes < orte_hostname_lwtoff) {
                kv = OBJ_NEW(opal_value_t);
    ...
    if (ORTE_PROC_IS_HNP && ORTE_JOB_FAMILY(ORTE_PROC_MY_NAME->jobid) == ORTE_JOB_FAMILY(jdata->jobid)) {
        ...
        /* pack the hostname, daemon vpid and contact URI for each ilwolved node */
        map = jdata->map;
        ...
        kv = OBJ_NEW(opal_value_t);
        orte_util_colwert_jobid_to_string(&kv->key, jdata->jobid);
