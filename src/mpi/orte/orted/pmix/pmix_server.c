...
static void eviction_cbfunc(struct opal_hotel_t *hotel,
                            int room_num, void *oclwpant)
{
    pmix_server_req_t *req = (pmix_server_req_t*)oclwpant;
    bool timeout = false;
    ...
    if (NULL != req->opcbfunc) {
        ...
        req->spcbfunc(OPAL_ERR_TIMEOUT, ORTE_JOBID_ILWALID, req->cbdata);
...
int pmix_server_init(void)
{
    ...
    orte_pmix_server_globals.server = *ORTE_NAME_ILWALID;
                                  ...
                                  orte_rml_tag_t tg, void *cbdata)
{
    ...
    opal_hotel_checkout_and_return_oclwpant(&orte_pmix_server_globals.reqs, room_num, (void**)&req);
...
static void rqcon(pmix_server_req_t *p)
{
    ...
    p->proxy = *ORTE_NAME_ILWALID;
    p->target = *ORTE_NAME_ILWALID;
