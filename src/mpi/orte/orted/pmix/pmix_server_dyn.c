                             ...
                             orte_rml_tag_t tg, void *cbdata)
{
    ...
    opal_hotel_checkout_and_return_oclwpant(&orte_pmix_server_globals.reqs, room, (void**)&req);
    if (NULL == req) {
        /* we are hosed */
...
static void spawn(int sd, short args, void *cbdata)
{
  ...
  callback:
    /* this section gets exelwted solely upon an error */
                         ...
                         opal_pmix_spawn_cbfunc_t cbfunc, void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(papp, apps, opal_pmix_app_t) {
        ...
        if (NULL != papp->elw) {
            app->elw = opal_argv_copy(papp->elw);
        }
        if (NULL != papp->cwd) {
        ...
        OPAL_LIST_FOREACH(info, &papp->info, opal_value_t) {
            ...
            /***   ELWIRONMENTAL VARIABLE DIRECTIVES   ***/
            /* there can be multiple of these, so we add them to the attribute list */
            } else if (0 == strcmp(info->key, OPAL_PMIX_SET_ELWAR)) {
                orte_add_attribute(&app->attributes, ORTE_APP_SET_ELWAR,
                                   ORTE_ATTR_GLOBAL, &info->data.elwar, OPAL_ELWAR);
            } else if (0 == strcmp(info->key, OPAL_PMIX_ADD_ELWAR)) {
                orte_add_attribute(&app->attributes, ORTE_APP_ADD_ELWAR,
                                   ORTE_ATTR_GLOBAL, &info->data.elwar, OPAL_ELWAR);
            } else if (0 == strcmp(info->key, OPAL_PMIX_UNSET_ELWAR)) {
                orte_add_attribute(&app->attributes, ORTE_APP_UNSET_ELWAR,
                                   ORTE_ATTR_GLOBAL, info->data.string, OPAL_STRING);
            } else if (0 == strcmp(info->key, OPAL_PMIX_PREPEND_ELWAR)) {
                orte_add_attribute(&app->attributes, ORTE_APP_PREPEND_ELWAR,
                                   ORTE_ATTR_GLOBAL, &info->data.elwar, OPAL_ELWAR);
            } else if (0 == strcmp(info->key, OPAL_PMIX_APPEND_ELWAR)) {
                orte_add_attribute(&app->attributes, ORTE_APP_APPEND_ELWAR,
                                   ORTE_ATTR_GLOBAL, &info->data.elwar, OPAL_ELWAR);
    ...
    OPAL_LIST_FOREACH_SAFE(info, next, job_info, opal_value_t) {
        ...
        /* there can be multiple of these, so we add them to the attribute list */
            ...
            orte_add_attribute(&jdata->attributes, ORTE_JOB_SET_ELWAR,
                               ORTE_ATTR_GLOBAL, &info->data.elwar, OPAL_ELWAR);
        } else if (0 == strcmp(info->key, OPAL_PMIX_ADD_ELWAR)) {
            orte_add_attribute(&jdata->attributes, ORTE_JOB_ADD_ELWAR,
                               ORTE_ATTR_GLOBAL, &info->data.elwar, OPAL_ELWAR);
        } else if (0 == strcmp(info->key, OPAL_PMIX_UNSET_ELWAR)) {
            orte_add_attribute(&jdata->attributes, ORTE_JOB_UNSET_ELWAR,
                               ORTE_ATTR_GLOBAL, info->data.string, OPAL_STRING);
        } else if (0 == strcmp(info->key, OPAL_PMIX_PREPEND_ELWAR)) {
            orte_add_attribute(&jdata->attributes, ORTE_JOB_PREPEND_ELWAR,
                               ORTE_ATTR_GLOBAL, &info->data.elwar, OPAL_ELWAR);
        } else if (0 == strcmp(info->key, OPAL_PMIX_APPEND_ELWAR)) {
            orte_add_attribute(&jdata->attributes, ORTE_JOB_APPEND_ELWAR,
                               ORTE_ATTR_GLOBAL, &info->data.elwar, OPAL_ELWAR);
...
static void _cnct(int sd, short args, void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(nm, cd->procs, orte_namelist_t) {
        ...
        if (NULL == (jdata = orte_get_job_data_object(nm->name.jobid))) {
            ...
            orte_util_colwert_jobid_to_string(&key, nm->name.jobid);
...
int pmix_server_alloc_fn(const opal_process_name_t *requestor,
                         ...
                         void *cbdata)
{
    /* ORTE lwrrently has no way of supporting allocation requests */
    return ORTE_ERR_NOT_SUPPORTED;
}
