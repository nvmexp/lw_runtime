                      ...
                      void* cbdata)
{
    ...
    orte_proc_t *lwr_proc = NULL, *prev_proc = NULL;
    bool found = false;
    orte_node_t *node;
    ...
    case ORTE_DAEMON_SIGNAL_LOCAL_PROCS:
        ...
        /* Colwert SIGTSTP to SIGSTOP so we can suspend a.out */
        if (SIGTSTP == signal) {
            if (orte_debug_daemons_flag) {
                opal_output(0, "%s orted_cmd: colwerted SIGTSTP to SIGSTOP before delivering",
    ...
    case ORTE_DAEMON_ABORT_PROCS_CALLED:
        ...
        for( i = 0; i < num_procs; ++i) {
            lwr_proc = OBJ_NEW(orte_proc_t);

            n = 1;
            if (ORTE_SUCCESS != (ret = opal_dss.unpack(buffer, &(lwr_proc->name), &n, ORTE_NAME)) ) {
                ORTE_ERROR_LOG(ret);
                goto CLEANUP;
            }
            ...
            OPAL_OUTPUT_VERBOSE((2, orte_debug_output,
                                 ...
                                 ORTE_NAME_PRINT(&(lwr_proc->name)), i, num_procs,
                                 (found ? "Dup" : "New") ));

            /* If not a duplicate, then add to the to_kill list */
            if( !found ) {
                opal_pointer_array_add(procs_to_kill, (void*)lwr_proc);
                OBJ_RETAIN(lwr_proc);
                opal_pointer_array_add(procs_prev_ordered_to_terminate, (void*)lwr_proc);
                num_new_procs++;
            }
        }
    ...
    case ORTE_DAEMON_TOP_CMD:
        ...
        while (ORTE_SUCCESS == opal_dss.unpack(buffer, &proc, &n, ORTE_NAME)) {
             ...
             * the requestor. We need to colwert that to our own job family
             */
            ...
            if (ORTE_PROC_IS_HNP) {
                ...
                if (ORTE_VPID_WILDCARD == proc.vpid) {
                     ...
                     * this rank is on
                     */
                    if (ORTE_VPID_ILWALID == (proc2.vpid = orte_get_proc_daemon_vpid(&proc))) {
                        ORTE_ERROR_LOG(ORTE_ERR_NOT_FOUND);
                        goto SEND_TOP_ANSWER;
                    }
    ...
    case ORTE_DAEMON_GET_STACK_TRACES:
        ...
        // Try to find the "gstack" exelwtable.  Failure to find the
        // exelwtable will be handled below, because the receiver
        ...
        for (i=0; i < orte_local_children->size; i++) {
            if (NULL != (proct = (orte_proc_t*)opal_pointer_array_get_item(orte_local_children, i)) &&
                ...
                // If we were able to find the gstack exelwtable,
