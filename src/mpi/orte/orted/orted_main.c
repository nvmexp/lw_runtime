...
int orte_daemon(int argc, char *argv[])
{
     ...
     * process them.
     */
    mca_base_cmd_line_process_args(cmd_line, &elwiron, &elwiron);
     ...
     * non-MCA elwars that were set in the elwiro when the
     * orted was exelwted - e.g., by .csh
     */
    orte_launch_elwiron = opal_argv_copy(elwiron);

    /* purge any ess/pmix flags set in the elwiron when we were launched */
    ...
    (void) mca_base_var_elw_name ("opal_cr_is_tool", &tmp_elw_var);
    opal_setelw(tmp_elw_var,
                "1",
                true, &elwiron);
    free(tmp_elw_var);
#endif
    ...
    if ((int)ORTE_VPID_ILWALID != orted_debug_failure) {
        ...
        if ((int)ORTE_PROC_MY_NAME->vpid == orted_debug_failure) {
            ...
            if (0 < orted_debug_failure_delay) {
                ...
                opal_output(0, "%s is exelwting clean %s", ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
    ...
    /* We actually do *not* want the orted to voluntarily yield() the
       ...
       (partilwlarly if there is no IO event to wake it up).  Hence,
    ...
    if (orted_globals.uri_pipe > 0) {
        ...
        char **singelw=NULL, *string_key, *elw_str;
        ...
        /* use setup fork to create the elwars needed by the singleton */
        if (OPAL_SUCCESS != (ret = opal_pmix.server_setup_fork(&proc->name, &singelw))) {
            ORTE_ERROR_LOG(ret);
            goto DONE;
        }

        /* append the transport key to the elwars needed by the singleton */
        if (!orte_get_attribute(&jdata->attributes, ORTE_JOB_TRANSPORT_KEY, (void**)&string_key, OPAL_STRING) || NULL == string_key) {
            ORTE_ERROR_LOG(ORTE_ERR_NOT_FOUND);
            goto DONE;
        }
        asprintf(&elw_str, OPAL_MCA_PREFIX"orte_precondition_transports=%s", string_key);
        opal_argv_append_nosize(&singelw, elw_str);
        free(elw_str);

        nptr = opal_argv_join(singelw, '*');
        opal_argv_free(singelw);

        /* create a string that contains our uri + sysinfo + PMIx server URI elwars */
        orte_util_colwert_sysinfo_to_string(&sysinfo, orte_local_cpu_type, orte_local_cpu_model);
        ...
        for (i=0; NULL != elwiron[i]; i++) {
            if (0 == strncmp(elwiron[i], OPAL_MCA_PREFIX, 9)) {
                /* make a copy to manipulate */
                tmp = strdup(elwiron[i]);
                /* find the equal sign */
                nptr = strchr(tmp, '=');
                *nptr = '\0';
                nptr++;
...
static void shutdown_callback(int fd, short flags, void *arg)
{
    ...
    /* if we were ordered to abort, do so */
    if (orted_globals.abort) {
        opal_output(0, "%s is exelwting %s abort", ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
    ...
    opal_output(0, "%s is exelwting clean abnormal termination", ORTE_NAME_PRINT(ORTE_PROC_MY_NAME));
    /* do -not- call finalize as this will send a message to the HNP
     * indicating clean termination! Instead, just forcibly cleanup
     * the local session_dir tree and exit
     */
