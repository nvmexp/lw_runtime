...
static char **global_mca_elw = NULL;
...
static int parse_appfile(orte_job_t *jdata, char *filename, char ***elw);
...
char MPIR_exelwtable_path[MPIR_MAX_PATH_LENGTH] = {0};
 ...
 * future regressions.
 *
 * See the following git issue for more dislwssion:
                     ...
                     opal_cmd_line_init_t *opts)
{
    ...
    /* find our basename (the name of the exelwtable) so that we can
    ...
    for (i=0; NULL != argv[i]; i++) {
        ...
        if (0 == strncmp(argv[i], "-"OPAL_MCA_CMD_LINE_ID, strlen("-"OPAL_MCA_CMD_LINE_ID)) ||
            ...
            0 == strncmp(argv[i], "--g"OPAL_MCA_CMD_LINE_ID, strlen("--g"OPAL_MCA_CMD_LINE_ID))) {
            (void) mca_base_var_elw_name (argv[i+1], &param);
            opal_setelw(param, argv[i+2], true, &elwiron);
            free(param);
        } else if (0 == strcmp(argv[i], "-am") ||
                   0 == strcmp(argv[i], "--am")) {
            (void)mca_base_var_elw_name("mca_base_param_file_prefix", &param);
            opal_setelw(param, argv[i+1], true, &elwiron);
            free(param);
        } else if (0 == strcmp(argv[i], "-tune") ||
                   0 == strcmp(argv[i], "--tune")) {
            (void)mca_base_var_elw_name("mca_base_elwar_file_prefix", &param);
            opal_setelw(param, argv[i+1], true, &elwiron);
            free(param);
        }
    }
    ...
    if (0 == geteuid() && !orte_cmd_options.run_as_root) {
        /* check for two elwars that allow override of this protection */
        char *r1, *r2;
        if (NULL != (r1 = getelw("OMPI_ALLOW_RUN_AS_ROOT")) &&
            NULL != (r2 = getelw("OMPI_ALLOW_RUN_AS_ROOT_CONFIRM"))) {
  ...
  moveon:
    ...
    rc = mca_base_cmd_line_process_args(orte_cmd_line, &elwiron, &elwiron);
    if (ORTE_SUCCESS != rc) {
        return rc;
    }
   ...
   /* set the flags - if they gave us a -hnp option, then
    ...
    if (ORTE_PROC_IS_TOOL) {
        if (0 == strncasecmp(orte_cmd_options.hnp, "file", strlen("file"))) {
            ...
            opal_setelw(OPAL_MCA_PREFIX"orte_hnp_uri", input, true, &elwiron);
        } else {
            /* should just be the uri itself - construct the target hnp info */
            opal_setelw(OPAL_MCA_PREFIX"orte_hnp_uri", orte_cmd_options.hnp, true, &elwiron);
        }
        /* we are never allowed to operate as a distributed tool,
         * so insist on the ess/tool component */
        opal_setelw(OPAL_MCA_PREFIX"ess", "tool", true, &elwiron);
    } else {
    ...
    if (ORTE_PROC_IS_TOOL) {
         ...
         * non-MCA elwars were set in the elwiro prior to calling
         * orterun
         */
        orte_launch_elwiron = opal_argv_copy(elwiron);
        /* clear params from the environment so our children
         * don't pick them up */
        opal_unsetelw(OPAL_MCA_PREFIX"ess", &orte_launch_elwiron);
        opal_unsetelw(OPAL_MCA_PREFIX"pmix", &orte_launch_elwiron);
    }
...
void orte_submit_finalize(void)
{
    ...
    if (NULL != orte_launch_elwiron) {
        opal_argv_free(orte_launch_elwiron);
    }
                    ...
                    void *complete_cbdata)
{
    ...
    /* bozo check - we don't allow relwrsive calls of submit */
    if (NULL != getelw("OMPI_UNIVERSE_SIZE")) {
        fprintf(stderr, "\n\n**********************************************************\n\n");
        fprintf(stderr, "%s does not support relwrsive calls\n", orte_basename);
        fprintf(stderr, "\n**********************************************************\n");
        return ORTE_ERR_FATAL;
    }
    ...
    if (NULL != orte_cmd_options.output_filename) {
         ...
         * colwert it to one so the name will be relative to
    ...
    if (NULL != orte_cmd_options.stdin_target) {
        if (0 == strcmp(orte_cmd_options.stdin_target, "all")) {
            ...
            jdata->stdin_target = ORTE_VPID_ILWALID;
        } else {
    ...
    /* check for debugger test elwars and forward them if necessary */
    if (NULL != getelw("ORTE_TEST_DEBUGGER_ATTACH")) {
        char *evar;
        evar = getelw("ORTE_TEST_DEBUGGER_SLEEP");
        for (n=0; n < (int)jdata->num_apps; n++) {
            if (NULL != (app = (orte_app_context_t*)opal_pointer_array_get_item(jdata->apps, n))) {
                opal_setelw("ORTE_TEST_DEBUGGER_ATTACH", "1", true, &app->elw);
                if (NULL != evar) {
                    opal_setelw("ORTE_TEST_DEBUGGER_SLEEP", evar, true, &app->elw);
                }
            }
        }
    }
    ...
    if (NULL != getelw("ORTE_TEST_HNP_SUICIDE") ||
        NULL != getelw("ORTE_TEST_ORTED_SUICIDE")) {
    ...
    if (0 < orte_cmd_options.timeout ||
        NULL != (param = getelw("MPIEXEC_TIMEOUT"))) {
        if (NULL != param) {
    ...
    if (NULL != (param = getelw("OMPI_MEMPROFILE"))) {
        timeout_seconds = strtol(param, NULL, 10);
    ...
    if (ORTE_PROC_IS_HNP) {
        ...
        /* If we have a prefix, then modify the PATH and
           ...
           have our exelwtables and libraries in their path
        ...
        */
        ...
        if (NULL != (app = (orte_app_context_t*)opal_pointer_array_get_item(jdata->apps, 0)) &&
            ...
            char *oldelw, *newelw, *lib_base, *bin_base;
            ...
            /* Reset PATH */
            newelw = opal_os_path( false, param, bin_base, NULL );
            oldelw = getelw("PATH");
            if (NULL != oldelw) {
                char *temp;
                asprintf(&temp, "%s:%s", newelw, oldelw );
                free( newelw );
                newelw = temp;
            }
            opal_setelw("PATH", newelw, true, &orte_launch_elwiron);
            if (orte_debug_flag) {
                opal_output(0, "%s: reset PATH: %s", orte_basename, newelw);
            }
            free(newelw);
            free(bin_base);

            /* Reset LD_LIBRARY_PATH */
            newelw = opal_os_path( false, param, lib_base, NULL );
            oldelw = getelw("LD_LIBRARY_PATH");
            if (NULL != oldelw) {
                char* temp;
                asprintf(&temp, "%s:%s", newelw, oldelw);
                free(newelw);
            ...
            opal_setelw("LD_LIBRARY_PATH", newelw, true, &orte_launch_elwiron);
            if (orte_debug_flag) {
                opal_output(0, "%s: reset LD_LIBRARY_PATH: %s",
                            orte_basename, newelw);
            }
            free(newelw);
            free(lib_base);
            free(param);
...
static int parse_locals(orte_job_t *jdata, int argc, char* argv[])
{
    ...
    /* NOTE: This bogus elw variable is necessary in the calls to
    ...
    for (app_num = 0, i = 1; i < argc; ++i) {
        if (0 == strcmp(argv[i], ":")) {
            ...
            if (opal_argv_count(temp_argv) > 1) {
                ...
                app = NULL;
                rc = create_app(temp_argc, temp_argv, jdata, &app, &made_app, &elw);
   ...
   /* Once we've created all the apps, add the global MCA params to
       ...
       course -- yay opal_elwiron_merge()).  */

    if (NULL != global_mca_elw) {
        ...
        for (j = 0; j < size1; ++j) {
            ...
            if (NULL != app) {
                ...
                elw = opal_elwiron_merge(global_mca_elw, app->elw);
                opal_argv_free(app->elw);
                app->elw = elw;
            }
        }
    }
    ...
    */
    ...
    if (NULL != global_mca_elw) {
        ...
        if (opal_pointer_array_get_size(jdata->apps) >= 1) {
            /* Remember that pointer_array's can be padded with NULL
               entries; so only use the app's elw if there is exactly
               1 non-NULL entry */
            app = (orte_app_context_t *)
    ...
    if (NULL != elw) {
        size1 = opal_argv_count(elw);
        for (j = 0; j < size1; ++j) {
            /* Use-after-Free error possible here.  putelw does not copy
             * the string passed to it, and instead stores only the pointer.
             * elw[j] may be freed later, in which case the pointer
             * in elwiron will now be left dangling into a deallocated
             * region.
             * So we make a copy of the variable.
             */
            char *value, *s = strdup(elw[j]);
            ...
            opal_setelw(s, value, true, &elwiron);
            free(s);
        }
    }
...
/*
 * This function takes a "char ***app_elw" parameter to handle the
 * specific case:
 *
 ...
 * one or more relwrsive calls back to create_app().  Since the
 ...
 * with a NULL value for app_elw, meaning that there is no "base"
                      ...
                      bool *made_app, char ***app_elw)
{
    ...
    *made_app = false;
    ...
    if (NULL != orte_cmd_options.appfile) {
        return parse_appfile(jdata, strdup(orte_cmd_options.appfile), app_elw);
    }
    ...
    if (0 == count) {
        orte_show_help("help-orterun.txt", "orterun:exelwtable-not-specified",
                       true, orte_basename, orte_basename);
    ...
    app->elw = opal_argv_copy(*app_elw);
    if (ORTE_SUCCESS != (rc = orte_schizo.parse_elw(orte_cmd_options.path,
                                                    orte_cmd_line,
                                                    elwiron, &app->elw))) {
        goto cleanup;
    }
    ...
    if (NULL != orte_cmd_options.wdir) {
        /* if this is a relative path, colwert it to an absolute path */
    ...
    /* Do not try to find argv[0] here -- the starter is responsible
       ...
       the node where orterun is exelwting.  So just strdup() argv[0]
       into app. */
     ...
     * and the "java" command will start the "exelwtable". So we need to ensure
    ...
    if (0 == strcmp(appname, "java")) {
        ...
        if (!found) {
            ...
            for (i=0; NULL != elwiron[i]; i++) {
                if (0 == strncmp(elwiron[i], "CLASSPATH", strlen("CLASSPATH"))) {
                    value = strchr(elwiron[i], '=');
                    ++value; /* step over the = */
                    opal_argv_insert_element(&app->argv, 1, value);
                    /* check for mpi.jar */
                    value = opal_os_path(false, opal_install_dirs.libdir, "mpi.jar", NULL);
        ...
        opal_setelw("OMPI_COMMAND", appname, true, &app->elw);
...
static int parse_appfile(orte_job_t *jdata, char *filename, char ***elw)
{
    ...
    do {
        ...
        if (argc > 0) {
            ...
            /* Create a temporary elw to use in the relwrsive call --
               that is: don't disturb the original elw so that we can
               have a consistent global elw.  This allows for the
               case:
            ...
            rc = create_app(argc, argv, jdata, &app, &made_app, &tmp_elw);
            if (ORTE_SUCCESS != rc) {
                /* Assume that the error message has already been
...
struct MPIR_PROCDESC {
    ...
    char *exelwtable_name;  /* name of binary */
    int pid;                /* process pid */
};
...
static void orte_debugger_init_before_spawn(orte_job_t *jdata)
{
 ...
 launchit:
    ...
    if (!mpir_warning_printed) {
        ...
        /* check for silencing elwar */
        if (NULL == getelw("OMPI_MPIR_DO_NOT_WARN")) {
            orte_show_help("help-orted.txt", "mpir-debugger-detected", true);
        }
    }

    /* tell the procs they are being debugged */
    (void) mca_base_var_elw_name ("orte_in_parallel_debugger", &elw_name);

    for (i=0; i < jdata->apps->size; i++) {
        if (NULL == (app = (orte_app_context_t*)opal_pointer_array_get_item(jdata->apps, i))) {
            continue;
        }
        opal_setelw(elw_name, "1", true, &app->elw);
    }
    free(elw_name);
...
static void orte_debugger_dump(void)
{
    ...
    for (i = 0; i < MPIR_proctable_size; i++) {
        fprintf(stderr,
                ...
                MPIR_proctable[i].exelwtable_name,
                MPIR_proctable[i].pid);
    }
    fprintf(stderr, "MPIR_exelwtable_path: %s\n",
            ('\0' == MPIR_exelwtable_path[0]) ?
            "NULL" : (char*) MPIR_exelwtable_path);
...
static void setup_debugger_job(orte_jobid_t jobid)
{
    ...
    /* dont push stdin */
    debugger->stdin_target = ORTE_VPID_ILWALID;
    ...
    if (NULL != orte_debugger_test_daemon) {
        ...
        app->app = strdup((char*)MPIR_exelwtable_path);
    }
    /* don't lwrrently have an option to pass the debugger
...
void orte_debugger_init_after_spawn(int fd, short event, void *cbdata)
{
    ...
    if (MPIR_proctable || 0 == jdata->num_procs) {
        ...
        if (MPIR_being_debugged || NULL != orte_debugger_test_daemon ||
            NULL != getelw("ORTE_TEST_DEBUGGER_ATTACH")) {
            OBJ_RELEASE(caddy);
            /* if we haven't previously warned about it */
            if (!mpir_warning_printed) {
                mpir_warning_printed = true;
    ...
    for (j=0; j < jdata->num_procs; j++) {
        ...
        if (orte_debugger_dump_proctable) {
            opal_output(orte_clean_output, "%s: Host %s Exe %s Pid %d",
                        ...
                        MPIR_proctable[i].exelwtable_name, MPIR_proctable[i].pid);
        }
    }
    ...
    if (MPIR_being_debugged || NULL != orte_debugger_test_daemon ||
         ...
         */
        if ('\0' == MPIR_exelwtable_path[0] && NULL == orte_debugger_test_daemon) {
            /* record that we have triggered the debugger */
            mpir_breakpoint_fired = true;
            ...
            opal_output_verbose(2, orte_debug_output,
                                ...
                                MPIR_exelwtable_path : orte_debugger_test_daemon);
                   ...
                   int argc, char **argv, char ***new_argv, int num_procs)
{
    ...
    char **orterun_argv = NULL, **exelwtable_argv = NULL, **line_argv = NULL;
    char cwd[OPAL_PATH_MAX];
    bool used_num_procs = false;
    bool single_app = false;
    bool fail_needed_exelwtable = false;
    ...
    /* Get the tail of the command line (i.e., the user exelwtable /
       argv) */

    opal_cmd_line_get_tail(cmd_line, &i, &exelwtable_argv);
    ...
    *new_argv = NULL;
    ...
    for (i = 0; NULL != line_argv[i]; ++i) {
        if (0 == strcmp(line_argv[i], "@mpirun@") ||
            ...
            /* This token is only a flag; it is not replaced with any
               alternate text */
            single_app = true;
        } else if (0 == strcmp(line_argv[i], "@exelwtable@")) {
            /* If we found the exelwtable, paste it in.  Otherwise,
               this is a possible error. */
            if (NULL != exelwtable_argv) {
                opal_argv_append_nosize(new_argv, exelwtable_argv[0]);
            } else {
                fail_needed_exelwtable = true;
            }
        } else if (0 == strcmp(line_argv[i], "@exelwtable_argv@")) {
            /* If we found the tail, paste in the argv.  Otherwise,
               this is a possible error. */
            if (NULL != exelwtable_argv) {
                for (j = 1; NULL != exelwtable_argv[j]; ++j) {
                    opal_argv_append_nosize(new_argv, exelwtable_argv[j]);
                }
            } else {
                fail_needed_exelwtable = true;
            }
    ...
    tmp = opal_path_findv((*new_argv)[0], X_OK, elwiron, cwd);
    if (NULL != tmp) {
        free(tmp);
        ...
        /* Some debuggers do not use orterun/mpirun, and therefore
           must have an exelwtable to run (e.g., cannot use mpirun's
           app context file feature). */
        else if (fail_needed_exelwtable) {
            orte_show_help("help-orterun.txt",
                           "debugger requires exelwtable", true,
 ...
 out:
    ...
    if (NULL != exelwtable_argv) {
        opal_argv_free(exelwtable_argv);
    }
    if (NULL != line_argv) {
...
static void attach_debugger(int fd, short event, void *arg)
{
    ...
    opal_output_verbose(1, orte_debug_output,
                        ...
                        (NULL == orte_debugger_test_daemon) ? MPIR_exelwtable_path : orte_debugger_test_daemon);
                         ...
                         int argc, char *argv[], int num_procs)
{
    ...
    memset((char*)MPIR_exelwtable_path, 0, MPIR_MAX_PATH_LENGTH);
    ...
    /* Set an MCA param so that everyone knows that they are being
       ...
       MPI processes */
    ret = mca_base_var_elw_name ("orte_in_parallel_debugger", &elw_name);
    if (OPAL_SUCCESS == ret && NULL != elw_name) {
        opal_setelw(elw_name, "1", true, &elwiron);
        free(elw_name);
    }
...
void orte_timeout_wakeup(int sd, short args, void *cbdata)
{
    ...
    /* this function gets called when the job exelwtion time
    ...
    if (ORTE_PROC_IS_HNP &&
        NULL != getelw("ORTE_TEST_HNP_SUICIDE")) {
