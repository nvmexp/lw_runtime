...
int main(int argc, char *argv[])
{
    ...
    mca_base_cmd_line_process_args(orte_info_cmd_line, &app_elw, &global_elw);

    /* putelw() all the stuff that we got back from elw (in case the
     * user specified some --mca params on the command line).  This
     * creates a memory leak, but that's unfortunately how putelw()
     * works.  :-(
     */
    ...
    if (NULL != app_elw) {
        opal_argv_free(app_elw);
    }
    if (NULL != global_elw) {
        opal_argv_free(global_elw);
    }
