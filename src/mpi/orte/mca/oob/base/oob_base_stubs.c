...
void orte_oob_base_get_addr(char **uri)
{
    ...
    if (ORTE_SUCCESS != (rc = orte_util_colwert_process_name_to_string(&final, ORTE_PROC_MY_NAME))) {
        ORTE_ERROR_LOG(rc);
        *uri = NULL;
        return;
    }
...
static void process_uri(char *uri)
{
    ...
    *cptr = '\0';
    ...
    /* the first field is the process name, so colwert it */
    orte_util_colwert_string_to_process_name(&peer, uri);
