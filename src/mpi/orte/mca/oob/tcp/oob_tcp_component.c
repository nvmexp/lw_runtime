 ...
 * are returned by these functions under UNIX/Linux elwironments
 */
 ...
 * thus we do not need to do a DNS lookup to colwert it */
                     ...
                     struct sockaddr_storage* inaddr)
{
    ...
    if (AF_INET == af_family) {
        ...
        if (0 == inet_pton(AF_INET6, host, (void*)&in6->sin6_addr)) {
            opal_output (0, "oob_tcp_parse_uri: Could not colwert %s\n", host);
            return ORTE_ERR_BAD_PARAM;
        }
...
static bool component_is_reachable(char *rtmod, orte_process_name_t *peer)
{
    ...
    if (ORTE_JOBID_ILWALID == hop.jobid ||
        ORTE_VPID_ILWALID == hop.vpid) {
