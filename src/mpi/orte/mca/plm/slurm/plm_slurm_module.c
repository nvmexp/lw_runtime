...
static int plm_slurm_start_proc(int argc, char **argv, char **elw,
                                char *prefix);
...
static void launch_daemons(int fd, short args, void *cbdata)
{
   ...
   /* if we don't want to launch, then don't attempt to
...
#if SLURM_CRAY_ELW
    /*
     * If in a SLURM/Cray elw. make sure that Cray PMI is not pulled in,
     * neither as a constructor run when orteds start, nor selected
     * when pmix components are registered
     */

    opal_setelw("PMI_NO_PREINITIALIZE", "1", false, &orte_launch_elwiron);
    opal_setelw("PMI_NO_FORK", "1", false, &orte_launch_elwiron);
    opal_setelw("OMPI_NO_USE_CRAY_PMI", "1", false, &orte_launch_elwiron);
#endif

    /* Append user defined arguments to srun */
    if ( NULL != mca_plm_slurm_component.lwstom_args ) {
        lwstom_strings = opal_argv_split(mca_plm_slurm_component.lwstom_args, ' ');
        num_args       = opal_argv_count(lwstom_strings);
        for (i = 0; i < num_args; ++i) {
            opal_argv_append(&argc, &argv, lwstom_strings[i]);
        }
        opal_argv_free(lwstom_strings);
    }
     ...
     */
    rc = orte_util_colwert_vpid_to_string(&name_string, map->daemon_vpid_start);
    if (ORTE_SUCCESS != rc) {
    ...
    for (n=0; n < state->jdata->apps->size; n++) {
        ...
        /* Check for already set lwr_prefix_dir -- if different,
           complain */
        if (NULL != app_prefix_dir) {
            if (NULL != lwr_prefix &&
                0 != strcmp (lwr_prefix, app_prefix_dir)) {
                orte_show_help("help-plm-slurm.txt", "multiple-prefixes",
                               true, lwr_prefix, app_prefix_dir);
                goto cleanup;
            }
             ...
             */
            if (NULL == lwr_prefix) {
                lwr_prefix = strdup(app_prefix_dir);
                OPAL_OUTPUT_VERBOSE((1, orte_plm_base_framework.framework_output,
     ...
     * dislwssion of using --test-only to see which one works, but
     * --test-only is only effective if you're not already inside a
     * SLURM allocation.  Instead, set the elw var SLURM_CPU_BIND to
     * "none", which should do the same thing as --cpu*bind=none.
     */
    opal_setelw("SLURM_CPU_BIND", "none", true, &elw);
    ...
    /* exec the daemon(s) */
    if (ORTE_SUCCESS != (rc = plm_slurm_start_proc(argc, argv, elw, lwr_prefix))) {
        ORTE_ERROR_LOG(rc);
        goto cleanup;
    }
    ...
    /* flag that launch was successful, so far as we lwrrently know */
    failed_launch = false;
                                ...
                                char *prefix)
{
    int fd;
    int srun_pid;
    char *exec_argv = opal_path_findv(argv[0], 0, elw, NULL);
    orte_proc_t *dummy;
    ...
    if (0 == srun_pid) {  /* child */
        ...
        if (NULL != prefix) {
            ...
            oldelw = getelw("PATH");
            if (NULL != oldelw) {
                asprintf(&newelw, "%s/%s:%s", prefix, bin_base, oldelw);
            } else {
                asprintf(&newelw, "%s/%s", prefix, bin_base);
            }
            opal_setelw("PATH", newelw, true, &elw);
            OPAL_OUTPUT_VERBOSE((1, orte_plm_base_framework.framework_output,
            ...
            if (NULL != oldelw) {
                asprintf(&newelw, "%s/%s:%s", prefix, lib_base, oldelw);
            } else {
                asprintf(&newelw, "%s/%s", prefix, lib_base);
            }
            opal_setelw("LD_LIBRARY_PATH", newelw, true, &elw);
            OPAL_OUTPUT_VERBOSE((1, orte_plm_base_framework.framework_output,
