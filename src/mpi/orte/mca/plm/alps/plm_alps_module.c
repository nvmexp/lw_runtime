...
static int plm_alps_start_proc(int argc, char **argv, char **elw,
                                char *prefix);
...
static void launch_daemons(int fd, short args, void *cbdata)
{
   ...
   /* if we don't want to launch, then don't attempt to
    ...
    orte_util_colwert_jobid_to_string(&jobid_string, daemons->jobid);
    ...
    if ( NULL != mca_plm_alps_component.lwstom_args ) {
        lwstom_strings = opal_argv_split(mca_plm_alps_component.lwstom_args, ' ');
     ...
     *
     * Code below adds elw. variables for aprun to forward which suppresses the action of the Cray PMI ctor.
     */
     ...
     */
    rc = orte_util_colwert_vpid_to_string(&vpid_string, map->daemon_vpid_start);
    if (ORTE_SUCCESS != rc) {
    ...
    /* exec the daemon(s) */
    if (ORTE_SUCCESS != (rc = plm_alps_start_proc(argc, argv, elw, lwr_prefix))) {
        ORTE_ERROR_LOG(rc);
        goto cleanup;
    }
                                ...
                                char *prefix)
{
    ...
    if (0 == alps_pid) {  /* child */
        ...
        if (NULL != prefix) {
            ...
            if (mca_plm_alps_component.debug) {
                opal_output(0, "plm:alps: reset PATH: %s", newelw);
            }
            free(newelw);
