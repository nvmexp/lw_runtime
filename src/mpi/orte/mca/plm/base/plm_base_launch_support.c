...
void orte_plm_base_setup_job(int fd, short args, void *cbdata)
{
    ...
    if (ORTE_JOBID_ILWALID == caddy->jdata->jobid) {
     ...
     */
     ...
     if (orte_get_attribute(&caddy->jdata->attributes, ORTE_JOB_LAUNCH_PROXY, (void**)&nptr, OPAL_NAME)) {
        ...
        if (orte_get_attribute(&parent->attributes, ORTE_JOB_TRANSPORT_KEY, (void**)&key, OPAL_STRING) &&
            ...
            /* add the transport key elwar to each app */
            for (i=0; i < caddy->jdata->apps->size; i++) {
                if (NULL == (app = (orte_app_context_t*)opal_pointer_array_get_item(caddy->jdata->apps, i))) {
                    continue;
                }
                opal_setelw(OPAL_MCA_PREFIX"orte_precondition_transports", key, true, &app->elw);
            }
            free(key);
        } else {
         ...
         * adds the key elwar to each app */
        if (ORTE_SUCCESS != (rc = orte_pre_condition_transports(caddy->jdata, NULL))) {
            ORTE_ERROR_LOG(rc);
            ORTE_FORCED_TERMINATE(ORTE_ERROR_DEFAULT_EXIT_CODE);
            OBJ_RELEASE(caddy);
...
void orte_plm_base_post_launch(int fd, short args, void *cbdata)
{
    ...
    if (ORTE_JOBID_ILWALID == jdata->originator.jobid) {
        OPAL_OUTPUT_VERBOSE((5, orte_plm_base_framework.framework_output,
        ...
        for (i=0; i < orte_node_topologies->size; i++) {
            ...
            if (0 == strcmp(sig, t->sig)) {
                ...
                if (0 != strcmp(ptr, myendian)) {
                    /* we don't lwrrently handle multi-endian operations in the
                     * MPI support */
                                          ...
                                          int *proc_vpid_index)
{
    ...
    if (NULL != getelw("ORTE_TEST_ORTED_SUICIDE")) {
    ...
    if (ORTE_SUCCESS != (rc = orte_util_colwert_jobid_to_string(&param, ORTE_PROC_MY_NAME->jobid))) {
        ORTE_ERROR_LOG(rc);
        return rc;
    }
    ...
    /* colwert the nodes with daemons to a regex */
    param = NULL;
    ...
    if (0 != rc) {
        ...
        loc_id = mca_base_var_find("opal", "mca", "base", "elwar_file_prefix");
        if (loc_id < 0) {
            rc = OPAL_ERR_NOT_FOUND;
            ORTE_ERROR_LOG(rc);
            return rc;
        ...
        if( NULL != tmp_value && NULL != tmp_value[0] ) {
            ...
            opal_argv_append(argc, argv, "mca_base_elwar_file_prefix");
    ...
    if (ORTE_PROC_IS_HNP || ORTE_PROC_IS_DAEMON) {
        ...
        for (i=0; i < cnt; i+=3) {
             ...
             * have a generic way of passing it as some elwironments ignore
             ...
             * Individual elwironments can add these back into the cmd line
             ...
             * end. Only a few elwironments allow this, so the daemon
...
int orte_plm_base_setup_virtual_machine(orte_job_t *jdata)
{
    ...
    if (one_filter) {
        /* at least one filtering option was exelwted, so
 ...
 process:
    ...
    while (NULL != (item = opal_list_remove_first(&nodes))) {
        ...
        if (ORTE_VPID_ILWALID == map->daemon_vpid_start) {
