                        ...
                        void* cbdata)
{
    ...
    case ORTE_PLM_LAUNCH_JOB_CMD:
        ...
        /* if the user asked to forward any elwars, cycle through the app contexts
         * in the comm_spawn request and add them
         */
        if (NULL != orte_forwarded_elwars) {
            for (i=0; i < jdata->apps->size; i++) {
                if (NULL == (app = (orte_app_context_t*)opal_pointer_array_get_item(jdata->apps, i))) {
                    continue;
                }
                elw = opal_elwiron_merge(orte_forwarded_elwars, app->elw);
  ...
  CLEANUP:
    /* see if an error oclwrred - if so, wakeup the HNP so we can exit */
