...
static void launch_daemons(int fd, short args, void *cbdata)
{
    ...
    (void) mca_base_var_elw_name ("plm", &var);
    opal_setelw(var, "rsh", true, &elw);
    free(var);

    /* add our umask -- see big note in orted.c */
    lwrrent_umask = umask(0);
    umask(lwrrent_umask);
    (void)asprintf(&var, "0%o", lwrrent_umask);
    opal_setelw("ORTE_DAEMON_UMASK_VALUE", var, true, &elw);
    free(var);
    ...
    */
    ...
    if (NULL != prefix_dir) {
        ...
        for (i = 0; NULL != elw && NULL != elw[i]; ++i) {
            /* Reset PATH */
            if (0 == strncmp("PATH=", elw[i], 5)) {
                (void)asprintf(&newelw, "%s/%s:%s",
                               prefix_dir, bin_base, elw[i] + 5);
                OPAL_OUTPUT_VERBOSE((1, orte_plm_base_framework.framework_output,
            ...
            else if (0 == strncmp("LD_LIBRARY_PATH=", elw[i], 16)) {
                (void)asprintf(&newelw, "%s/%s:%s",
                               prefix_dir, lib_base, elw[i] + 16);
                OPAL_OUTPUT_VERBOSE((1, orte_plm_base_framework.framework_output,
    ...
    for (i = 0; i < map->nodes->size; i++) {
        ...
        /* setup process name */
        rc = orte_util_colwert_vpid_to_string(&vpid_string, node->daemon->name.vpid);
        if (ORTE_SUCCESS != rc) {
        ...
        rc = tm_spawn(argc, argv, elw, launchid, tm_task_ids + launched, tm_events + launched);
        if (TM_SUCCESS != rc) {
