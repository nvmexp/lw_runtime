...
static int plm_tm_register(void)
{
    ...
    (void) mca_base_component_var_register (comp, "want_path_check",
                                            "Whether the launching process should check for the plm_tm_orted exelwtable in the PATH before launching (the TM API does not give an indication of failure; this is a somewhat-lame workaround; non-zero values enable this check)",
...
static int orte_plm_tm_component_query(mca_base_module_t **module, int *priority)
{
    ...
    if (NULL != getelw("PBS_ELWIRONMENT") &&
        NULL != getelw("PBS_JOBID")) {
