...
static int rsh_init(void)
{
    ...
    if (mca_plm_rsh_component.using_qrsh) {
        ...
        asprintf(&tmp, "%s/bin/%s", getelw("SGE_ROOT"), getelw("ARC"));
...
static void rsh_wait_daemon(int sd, short flags, void *cbdata)
{
    ...
    /* release any delay */
    --num_in_progress;
    if (num_in_progress < mca_plm_rsh_component.num_conlwrrent) {
                        ...
                        int *proc_vpid_index, char *prefix_dir)
{
    ...
    char *opal_prefix = getelw("OPAL_PREFIX");
    ...
    /* Figure out the basenames for the libdir and bindir.  This
       ...
       - After a dislwssion on the devel-core mailing list, the
    ...
    *node_name_index1 = argc;
     ...
     * not from mpirun. Also, they may have a lwstomized version of orted
    ...
    /* we now need to assemble the actual cmd that will be exelwted - this depends
    ...
    if (NULL != lib_base || NULL != bin_base) {
        if (ORTE_PLM_RSH_SHELL_SH == remote_shell ||
            ...
            (void)asprintf (&final_cmd,
                            ...
                            "setelw LD_LIBRARY_PATH %s ; "
                            "if ( $?OMPI_have_llp == 1 ) "
                            "setelw LD_LIBRARY_PATH %s%s$LD_LIBRARY_PATH ; "
                            "if ( $?DYLD_LIBRARY_PATH == 1 ) "
                            "set OMPI_have_dllp ; "
                            "if ( $?DYLD_LIBRARY_PATH == 0 ) "
                            "setelw DYLD_LIBRARY_PATH %s ; "
                            "if ( $?OMPI_have_dllp == 1 ) "
                            "setelw DYLD_LIBRARY_PATH %s%s$DYLD_LIBRARY_PATH ; "
                            "%s %s",
                            (NULL != mca_plm_rsh_component.chdir ? "cd " : " "),
                            (NULL != mca_plm_rsh_component.chdir ? mca_plm_rsh_component.chdir : " "),
                            (NULL != mca_plm_rsh_component.chdir ? " ; " : " "),
                            (opal_prefix != NULL ? "setelw OPAL_PREFIX " : " "),
                            (opal_prefix != NULL ? opal_prefix : " "),
                            (opal_prefix != NULL ? " ;" : " "),
                            (NULL != bin_base ? bin_base : " "),
                            (NULL != lib_base ? lib_base : " "),
    ...
    if (mca_plm_rsh_component.pass_elwiron_mca_params) {
        /* now check our local environment for MCA params - add them
         * only if they aren't already present
         */
        for (i = 0; NULL != elwiron[i]; ++i) {
            if (0 == strncmp(OPAL_MCA_PREFIX"mca_base_elw_list", elwiron[i],
                             strlen(OPAL_MCA_PREFIX"mca_base_elw_list"))) {
                /* ignore this one */
                continue;
            }
            if (0 == strncmp(OPAL_MCA_PREFIX, elwiron[i], 9)) {
...
static void ssh_child(int argc, char **argv)
{
    ...
    /* setup environment */
    elw = opal_argv_copy(orte_launch_elwiron);
    ...
    OPAL_OUTPUT_VERBOSE((1, orte_plm_base_framework.framework_output,
                         "%s plm:rsh: exelwting: (%s) [%s]",
                         ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                         exec_path, (NULL == var) ? "NULL" : var));
    if (NULL != var) free(var);

    execve(exec_path, exec_argv, elw);
...
static int remote_spawn(void)
{
    ...
    OPAL_LIST_FOREACH(child, &coll, orte_namelist_t) {
        ...
        /* pass the vpid */
        rc = orte_util_colwert_vpid_to_string(&var, target.vpid);
        if (ORTE_SUCCESS != rc) {
...
static void process_launch_list(int fd, short args, void *cbdata)
{
    ...
    while (num_in_progress < mca_plm_rsh_component.num_conlwrrent) {
...
static void launch_daemons(int fd, short args, void *cbdata)
{
    ...
    if ((0 < opal_output_get_verbosity(orte_plm_base_framework.framework_output) ||
         orte_leave_session_attached) &&
        mca_plm_rsh_component.num_conlwrrent < map->num_new_daemons) {
        /**
         * If we are in '--debug-daemons' we keep the ssh connection
         * alive for the span of the run. If we use this option
         * AND we launch on more than "num_conlwrrent" machines
        ...
        orte_show_help("help-plm-rsh.txt", "deadlock-params",
                       true, mca_plm_rsh_component.num_conlwrrent, map->num_new_daemons);
    ...
    /*
     * After a dislwssion between Ralph & Jeff, we concluded that we
     * really are handling the prefix dir option incorrectly. It lwrrently
    ...
    launch:
        ...
        /* pass the vpid */
        rc = orte_util_colwert_vpid_to_string(&var, node->daemon->name.vpid);
        if (ORTE_SUCCESS != rc) {
            opal_output(0, "orte_plm_rsh: unable to get daemon vpid as string");
            exit(-1);
        }
...
static int launch_agent_setup(const char *agent, char *path)
{
    ...
    rsh_agent_path = opal_path_findv(rsh_agent_argv[0], X_OK, elwiron, path);
