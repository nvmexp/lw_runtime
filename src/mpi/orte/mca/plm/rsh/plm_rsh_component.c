...
static int rsh_component_register(void)
{
    ...
    mca_plm_rsh_component.num_conlwrrent = 128;
    (void) mca_base_component_var_register (c, "num_conlwrrent",
                                            "How many plm_rsh_agent instances to ilwoke conlwrrently (must be > 0)",
                                            MCA_BASE_VAR_TYPE_INT, NULL, 0, 0,
                                            OPAL_INFO_LVL_5,
                                            MCA_BASE_VAR_SCOPE_READONLY,
                                            &mca_plm_rsh_component.num_conlwrrent);
    ...
    (void) mca_base_component_var_register (c, "delay",
                                            "Delay between ilwocations of the remote agent (sec[:usec])",
    ...
    var_id = mca_base_component_var_register (c, "agent",
                                              "The command used to launch exelwtables on remote nodes (typically either \"ssh\" or \"rsh\")",
                                              MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0,
                                              OPAL_INFO_LVL_2,
                                              MCA_BASE_VAR_SCOPE_READONLY,
    ...
    /* XXX -- var_colwersion -- Why does this component register orte_assume_same_shell? Components should ONLY register THEIR OWN variables. */
    (void) mca_base_var_register_synonym (var_id, "orte", "orte", NULL, "assume_same_shell", 0);

    mca_plm_rsh_component.pass_elwiron_mca_params = true;
    (void) mca_base_component_var_register (c, "pass_elwiron_mca_params",
                                            "If set to false, do not include mca params from the environment on the orted cmd line",
                                            MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0,
                                            OPAL_INFO_LVL_2,
                                            MCA_BASE_VAR_SCOPE_READONLY,
                                            &mca_plm_rsh_component.pass_elwiron_mca_params);
...
static int rsh_component_open(void)
{
    ...
    /* lookup parameters */
    if (mca_plm_rsh_component.num_conlwrrent <= 0) {
        orte_show_help("help-plm-rsh.txt", "conlwrrency-less-than-zero",
                       true, mca_plm_rsh_component.num_conlwrrent);
        mca_plm_rsh_component.num_conlwrrent = 1;
    }
...
static int rsh_component_query(mca_base_module_t **module, int *priority)
{
    ...
    if (!mca_plm_rsh_component.disable_qrsh &&
        NULL != getelw("SGE_ROOT") && NULL != getelw("ARC") &&
        NULL != getelw("PE_HOSTFILE") && NULL != getelw("JOB_ID")) {
        ...
        if (ORTE_SUCCESS != rsh_launch_agent_lookup("qrsh", tmp)) {
            /* can't be SGE */
             opal_output_verbose(1, orte_plm_base_framework.framework_output,
                                ...
                                "or exelwtion permissions not set for launching agent qrsh",
    ...
    if (!mca_plm_rsh_component.disable_llspawn &&
        ...
        if (ORTE_SUCCESS != rsh_launch_agent_lookup("llspawn", NULL)) {
             opal_output_verbose(1, orte_plm_base_framework.framework_output,
                                ...
                                "indicated but cannot find path or exelwtion "
...
char **orte_plm_rsh_search(const char* agent_list, const char *path)
{
    ...
    for (i = 0; NULL != lines[i]; ++i) {
        ...
        tmp = opal_path_findv(tokens[0], X_OK, elwiron, cwd);
        if (NULL != tmp) {
            free(tokens[0]);
            tokens[0] = tmp;
