...
static int delete_route(orte_process_name_t *proc)
{
    if (proc->jobid == ORTE_JOBID_ILWALID ||
        proc->vpid == ORTE_VPID_ILWALID) {
...
static int update_route(orte_process_name_t *target,
                        orte_process_name_t *route)
{
    if (target->jobid == ORTE_JOBID_ILWALID ||
        target->vpid == ORTE_VPID_ILWALID) {
        return ORTE_ERR_BAD_PARAM;
    }
...
static orte_process_name_t get_route(orte_process_name_t *target)
{
    ...
    if (ORTE_VPID_ILWALID == (daemon.vpid = orte_get_proc_daemon_vpid(target))) {
...
{
    /* find out what daemon hosts this proc */
    if (ORTE_VPID_ILWALID == orte_get_proc_daemon_vpid((orte_process_name_t*)target)) {
        return false;
    }
                         ...
                         opal_bitmap_t *relatives, bool mine)
{
    ...
    /* is this me? */
    if (me == rank) {
        bitmap = opal_lwbe_dim(num_procs);
