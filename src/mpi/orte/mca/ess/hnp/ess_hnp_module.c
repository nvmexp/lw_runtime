...
static int rte_init(void)
{
    ...
    if (NULL != orte_ess_base_jobid) {
        if (ORTE_SUCCESS != (ret = orte_util_colwert_string_to_jobid(&ORTE_PROC_MY_NAME->jobid, orte_ess_base_jobid))) {
            error = "colwert_string_to_jobid";
            goto error;
        }
    ...
    opal_setelw("OMPI_MCA_pmix", "^s1,s2,cray,isolated", false, &elwiron);
    ...
    if (ORTE_RML_CONDUIT_ILWALID == (orte_mgmt_conduit = orte_rml.open_conduit(&transports))) {
    ...
    if (ORTE_RML_CONDUIT_ILWALID == (orte_coll_conduit = orte_rml.open_conduit(&transports))) {
        ret = ORTE_ERR_OPEN_CONDUIT_FAIL;
...
static void clean_abort(int fd, short flags, void *arg)
{
     ...
     */
    orte_exelwte_quiet = true;
    /* We are in an event handler; the job completed procedure
       will delete the signal handler that is lwrrently running
