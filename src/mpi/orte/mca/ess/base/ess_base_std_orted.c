...
int orte_ess_base_orted_setup(void)
{
    ...
    /* some elwironments allow remote launches - e.g., ssh - so
     * open and select something -only- if we are given
     * a specific module to use
     */
    (void) mca_base_var_elw_name("plm", &param);
    plm_in_use = !!(getelw(param));
    free (param);
    if (plm_in_use)  {
    ...
    if (orte_create_session_dirs) {
        ...
        /* set the opal_output elw file location to be in the
        ...
        if (orte_debug_daemons_file_flag) {
             ...
             */
            /* get my jobid */
            if (ORTE_SUCCESS != (ret = orte_util_colwert_jobid_to_string(&jobidstring,
