...
int orte_ess_base_proc_binding(void)
{
    ...
    if (NULL != getelw(OPAL_MCA_PREFIX"orte_bound_at_launch")) {
        orte_proc_is_bound = true;
        if (NULL != (map = getelw(OPAL_MCA_PREFIX"orte_base_applied_binding"))) {
        ...
        return ORTE_SUCCESS;
    } else if (NULL != getelw(OPAL_MCA_PREFIX"orte_externally_bound")) {
        orte_proc_is_bound = true;
    ...
    if (!orte_proc_is_bound) {
        ...
        if (0 != hwloc_bitmap_compare(cpus, nodeset) ||
            ...
            if (OPAL_BIND_TO_CPUSET == OPAL_GET_BINDING_POLICY(opal_hwloc_binding_policy)) {
                ...
                hwloc_bitmap_free(cpus);
                /* get the node rank */
                if (ORTE_NODE_RANK_ILWALID == orte_process_info.my_node_rank) {
 ...
 MOVEON:
    ...
    /* push our cpuset so others can callwlate our locality */
    if (NULL != orte_process_info.cpuset) {
