...
static int elw_set_name(void);

static int rte_init(void);
static int rte_finalize(void);

orte_ess_base_module_t orte_ess_elw_module = {
    rte_init,
    rte_finalize,
    NULL,
    NULL
...
static int rte_init(void)
{
    ...
    /* Start by getting a unique name from the elwiro */
    elw_set_name();
...
static int elw_set_name(void)
{
    ...
    OPAL_OUTPUT_VERBOSE((1, orte_ess_base_framework.framework_output,
                         "ess:elw set name to %s", ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
