...
static int rte_init(void)
{
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "orte_ess_base_std_prolog");
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "progress_thread_init");
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "pmix_framework_open");

    /* see if we were direct launched */
    if (ORTE_SCHIZO_DIRECT_LAUNCHED == orte_schizo.check_launch_elwironment()) {
    ...
    if (!opal_pmix.initialized() && (OPAL_SUCCESS != (ret = opal_pmix.init(NULL)))) {
        ...
        if (direct_launched && NULL != (elwar = getelw("ORTE_SCHIZO_DETECTION"))) {
            if (0 == strcmp(elwar, "SLURM")) {
                /* yes to both - so emit a hopefully helpful
                 * error message and abort */
                orte_show_help_finalize();
                orte_show_help("help-ess-base.txt", "slurm-error", true);
                return ORTE_ERR_SILENT;
            } else if (0 == strcmp(elwar, "ALPS")) {
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "pmix_init");
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "pmix_get_job_params");
    ...
    if (NULL == getelw(OPAL_MCA_PREFIX"orte_precondition_transports")) {
        ...
        asprintf(&elwar, OPAL_MCA_PREFIX"orte_precondition_transports=%s", string_key);
        putelw(elwar);
        added_transport_keys = true;
        /* cannot free the elwar as that messes up our elwiron */
        free(string_key);
    }
    OPAL_TIMING_ELW_NEXT(rte_init, "orte_precondition_transport");
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "pmix_set_tdirs");
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "pmix_set_locality");
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "state_framework_open");
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "errmgr_framework_open");
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "create_session_dirs");
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "errmgr_select");
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "ess_base_proc_binding");
    ...
    OPAL_TIMING_ELW_NEXT(rte_init, "rte_init_done");
