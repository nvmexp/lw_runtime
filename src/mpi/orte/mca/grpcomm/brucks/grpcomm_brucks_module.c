                     ...
                     opal_buffer_t *sendbuf)
{
    ...
    /* get my own rank */
    coll->my_rank = ORTE_VPID_ILWALID;
    for (orte_vpid_t lw = 0; lw < coll->ndmns; lw++) {
        if (coll->dmns[lw] == ORTE_PROC_MY_NAME->vpid) {
            coll->my_rank = lw;
            break;
        }
    }
    ...
    /* check for bozo case */
    if (ORTE_VPID_ILWALID == coll->my_rank) {
...
static void brucks_allgather_process_data(orte_grpcomm_coll_t *coll, uint32_t distance) {
    ...
    /* NTH: callwlate in which round we should send the final data. this is the first
    ...
    while (distance < log2ndmns) {
        ...
        lw = (coll->ndmns + coll->my_rank - (1 << distance)) % coll->ndmns;
        peer.vpid = coll->dmns[lw];

        brucks_allgather_send_dist(coll, &peer, distance);

        if (distance == last_round) {
            /* have enough data to send the final round now */
            lw = (coll->ndmns + coll->my_rank - (1 << log2ndmns)) % coll->ndmns;
            peer.vpid = coll->dmns[lw];
