...
static int map_to_ftgrps(orte_job_t *jdata)
{
    ...
    int i, j, k, totnodes;
    opal_list_t node_list;
    opal_list_item_t *item, *next, *lwritem;
    orte_std_cntr_t num_slots;
    int rc = ORTE_SUCCESS;
    float avgload, minload;
    ...
    for (i=0; i < jdata->apps->size; i++) {
        ...
        lwritem = opal_list_get_first(&node_list);
        ...
        for (j=0; j < app->num_procs; j++) {
            ...
            if (NULL == target) {
                OPAL_OUTPUT_VERBOSE((2, orte_rmaps_base_framework.framework_output,
                                     ...
                                     ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
                nd = (orte_node_t*)lwritem;
                lwritem = opal_list_get_next(lwritem);
                if (lwritem == opal_list_get_end(&node_list)) {
                    lwritem = opal_list_get_first(&node_list);
                }
            } else {
