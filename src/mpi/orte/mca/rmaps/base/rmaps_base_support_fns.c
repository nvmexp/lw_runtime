...
orte_node_t* orte_rmaps_base_get_starting_point(opal_list_t *node_list,
                                                orte_job_t *jdata)
{
    opal_list_item_t *item, *lwr_node_item;
    ...
    if (NULL != jdata->bookmark) {
        ...
        if (NULL == lwr_node_item) {
            lwr_node_item = opal_list_get_first(node_list);
        }
    } else {
    ...
    if (node->slots_inuse >= node->slots) {
         ...
         */
        if (lwr_node_item != opal_list_get_last(node_list)) {
            item = opal_list_get_next(lwr_node_item);
        } else {
            item = opal_list_get_first(node_list);
        }
        nd1 = NULL;
        while (item != lwr_node_item) {
        ...
        if (NULL != nd1 &&
            ...
            lwr_node_item = (opal_list_item_t*)ndmin;
        }
    }
 ...
 process:
     ...
     * preserving order
     */
    while (lwr_node_item != (item = opal_list_get_first(node_list))) {
        opal_list_remove_item(node_list, item);
        opal_list_append(node_list, item);
    }

    return (orte_node_t*)lwr_node_item;
}
