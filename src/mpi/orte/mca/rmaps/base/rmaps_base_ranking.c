            ...
            for (m=0; m < jdata->map->nodes->size; m++) {
                ...
                for (i=0; i < num_objs && cnt < app->num_procs; i++) {
                    ...
                    for (j=0; j < node->procs->size && cnt < app->num_procs; j++) {
                        ...
                        if (ORTE_VPID_ILWALID != proc->name.vpid) {
                            continue;
                        }
    ...
    for (i=0; i < map->nodes->size; i++) {
        ...
        for (k=0; k < node->procs->size; k++) {
            ...
            for (j=0; j < node->procs->size; j++) {
                 ...
                 */
                if (proc->name.jobid == jdata->jobid &&
                    ORTE_LOCAL_RANK_ILWALID == proc->local_rank &&
                    proc->name.vpid < milw) {
                    milw = proc->name.vpid;
                    psave = proc;
                }
                /* no matter what job...still have to handle node_rank */
                if (ORTE_NODE_RANK_ILWALID == proc->node_rank &&
                    proc->name.vpid < milw2) {
                    milw2 = proc->name.vpid;
                    psave2 = proc;
                }
            }
