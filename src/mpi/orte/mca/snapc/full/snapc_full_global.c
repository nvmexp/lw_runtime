...
static orte_jobid_t lwrrent_global_jobid = ORTE_JOBID_ILWALID;
static orte_snapc_base_global_snapshot_t global_snapshot;
static int lwrrent_total_orteds = 0;
static bool updated_job_to_running;
static int lwrrent_job_ckpt_state = ORTE_SNAPC_CKPT_STATE_NONE;
static bool cleanup_on_establish = false;
static bool global_coord_has_local_children = false;

static bool lwrrently_migrating = false;
...
static void snapc_full_report_progress(orte_snapc_full_orted_snapshot_t *orted_snapshot,
                                       int total,
                                       int min_state);
static int    report_progress_lwr_loc_finished = 0;
...
{
    lwrrent_global_jobid = ORTE_JOBID_ILWALID;
    orte_snapc_base_snapshot_seq_number = -1;

    orte_checkpoint_sender = orte_name_ilwalid;
...
int global_coord_setup_job(orte_jobid_t jobid) {
    ...
    if( ORTE_JOBID_ILWALID == lwrrent_global_jobid ) {
        lwrrent_global_jobid = jobid;
    ...
    else if ( jobid == lwrrent_global_jobid ) {

        /* look up job data object */
        if (NULL == (jdata = orte_get_job_data_object(lwrrent_global_jobid))) {
    ...
    /* Only allow one job at a time */
        ...
        opal_output(mca_snapc_full_component.super.output_handle,
                    ...
                    ORTE_JOBID_PRINT(jobid), ORTE_JOBID_PRINT(lwrrent_global_jobid));
    ...
    if(orte_snapc_base_establish_global_snapshot_dir) {
        opal_output(0, "Global) Error: Pre-establishment of snapshot directory lwrrently not supported!");
...
int global_coord_start_ckpt(orte_snapc_base_quiesce_t *datum)
{
     ...
     */
    while(((lwrrently_migrating  && lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_MIGRATING) ||
           (!lwrrently_migrating && lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_FINISHED_LOCAL)) &&
          lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_ESTABLISHED &&
          lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_RECOVERED &&
          lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_ERROR &&
          lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_NONE ) {
        opal_progress();
    }
...
int global_coord_end_ckpt(orte_snapc_base_quiesce_t *datum)
{
    ...
    OPAL_OUTPUT_VERBOSE((10, mca_snapc_full_component.super.output_handle,
                         ...
                         lwrrent_job_ckpt_state));

    if( lwrrently_migrating ) {
        OPAL_OUTPUT_VERBOSE((10, mca_snapc_full_component.super.output_handle,
        ...
        if( ORTE_SUCCESS != (ret = orte_snapc_full_global_set_job_ckpt_info(lwrrent_global_jobid,
    ...
    while(lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_RECOVERED &&
          lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_ERROR &&
          lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_NONE ) {
        opal_progress();
    }
    ...
    if( lwrrently_migrating ) {
        ...
        report_progress_lwr_loc_finished = 0;
        report_progress_last_reported_loc_finished = 0;
    }

 cleanup:

    lwrrently_migrating = false;
    if( NULL != migrating_procs ) {
        while( NULL != (item = opal_list_remove_first(migrating_procs)) ) {
            OBJ_RELEASE(item);
        }
...
static int global_init_job_structs(void)
{
    ...
    for (i=0; i < map->nodes->size; i++) {
        if (NULL == (lwr_node = (orte_node_t*)opal_pointer_array_get_item(map->nodes, i))) {
            continue;
        }

        procs = (orte_proc_t**)lwr_node->procs->addr;
        ...
        if( found ) {
            OPAL_OUTPUT_VERBOSE((1, mca_snapc_full_component.super.output_handle,
                                 ...
                                 i, ORTE_NAME_PRINT(&(lwr_node->daemon->name)), lwr_node->num_procs));
            continue;
        }
        ...
        orted_snapshot->process_name.jobid  = lwr_node->daemon->name.jobid;
        orted_snapshot->process_name.vpid   = lwr_node->daemon->name.vpid;
        ...
        for(p = 0; p < lwr_node->num_procs; ++p) {
            OPAL_OUTPUT_VERBOSE((10, mca_snapc_full_component.super.output_handle,
                                 "Global) \t [%d] Found Process %s on Daemon %s",
                                 p, ORTE_NAME_PRINT(&(procs[p]->name)), ORTE_NAME_PRINT(&(lwr_node->daemon->name)) ));
                                           ...
                                           void* cbdata)
{
    ...
    if (ORTE_SNAPC_GLOBAL_INIT_CMD == command) {
        ...
        if( !is_app_checkpointable ) {
            OPAL_OUTPUT_VERBOSE((1, mca_snapc_full_component.super.output_handle,
                                 "Global) request_cmd(): Checkpointing lwrrently disabled, rejecting request"));
            if( ORTE_SUCCESS != (ret = orte_snapc_base_global_coord_ckpt_update_cmd(&orte_checkpoint_sender,
                                                                                    0,
                                                                                    ORTE_SNAPC_CKPT_STATE_ERROR))) {
                ORTE_ERROR_LOG(ret);
         ...
         */
        if( ORTE_JOBID_ILWALID != jobid && jobid != lwrrent_global_jobid) {
            opal_output(mca_snapc_full_component.super.output_handle,
    ...
    /*
     * Terminate the connection (Not lwrrently implemented)
     */
                                              ...
                                              opal_buffer_t* sbuffer)
{
     ...
     ************************************/
         ...
         */
        if( lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_ERROR &&
            lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_NONE ) {
            OPAL_OUTPUT_VERBOSE((3, mca_snapc_full_component.super.output_handle,
                                 "Global) process_request_op(): Wait for ongoing checkpoint to complete..."));
            while( lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_ERROR &&
                   lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_NONE ) {
                opal_progress();
            }
        }
     ...
     ************************************/
        ...
        if( ORTE_SNAPC_CKPT_STATE_ERROR == lwrrent_job_ckpt_state ) {
            op_state = -1;
        } else {
            op_state = 0;
        }
     ...
     ************************************/
         ...
         */
        if( ORTE_SUCCESS != (ret = orte_errmgr_base_restart_job(lwrrent_global_jobid, global_handle, seq_num) ) ) {
            ORTE_ERROR_LOG(ret);
            orte_snapc_ckpt_state_notify(ORTE_SNAPC_CKPT_STATE_NO_RESTART);
            goto cleanup;
        }
     ...
     ************************************/
        ...
        if( ORTE_SUCCESS != (ret = orte_errmgr_base_migrate_job(lwrrent_global_jobid, datum) ) ) {
            ORTE_ERROR_LOG(ret);
            goto cleanup;
        }
     ...
     ************************************/
         ...
         */
        while( lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_ERROR &&
               lwrrent_job_ckpt_state != ORTE_SNAPC_CKPT_STATE_INC_PREPED ) {
            opal_progress();
        }
                                               ...
                                               bool quick)
{
    ...
    /* JJH: Though there is lwrrently no additional information sent in a long
 ...
 post_process:
    ...
    SNAPC_FULL_REPORT_PROGRESS(orted_snapshot, lwrrent_total_orteds, loc_min_state);
    ...
    if( ORTE_SNAPC_CKPT_STATE_RUNNING == loc_min_state &&
        ORTE_SNAPC_CKPT_STATE_RUNNING != lwrrent_job_ckpt_state) {
        lwrrent_job_ckpt_state = loc_min_state;
    ...
    if( ORTE_SNAPC_CKPT_STATE_INC_PREPED == loc_min_state &&
        ORTE_SNAPC_CKPT_STATE_INC_PREPED > lwrrent_job_ckpt_state) {
        lwrrent_job_ckpt_state = loc_min_state;
    ...
    if( ORTE_SNAPC_CKPT_STATE_STOPPED == loc_min_state &&
        ORTE_SNAPC_CKPT_STATE_STOPPED > lwrrent_job_ckpt_state) {
        lwrrent_job_ckpt_state = loc_min_state;
    ...
    if( ORTE_SNAPC_CKPT_STATE_FINISHED_LOCAL == loc_min_state &&
        ORTE_SNAPC_CKPT_STATE_FINISHED_LOCAL > lwrrent_job_ckpt_state) {

        SNAPC_FULL_SET_TIMER(SNAPC_FULL_TIMER_FIN_LOCAL);

        if( ORTE_SNAPC_CKPT_STATE_NONE != lwrrent_job_ckpt_state ) {
            if( loc_min_state == lwrrent_job_ckpt_state) {
                opal_output(0, "Global) JJH WARNING!!: (%d) == (%d)", loc_min_state, lwrrent_job_ckpt_state);
            }
        }

        if( lwrrently_migrating ) {
            write_out_global_metadata();
            lwrrent_job_ckpt_state = ORTE_SNAPC_CKPT_STATE_MIGRATING;
        }
        else {
            lwrrent_job_ckpt_state = ORTE_SNAPC_CKPT_STATE_FINISHED_LOCAL;
        }

        if( NULL != state_str ) {
            free(state_str);
        }
        orte_snapc_ckpt_state_str(&state_str, lwrrent_job_ckpt_state);
        OPAL_OUTPUT_VERBOSE((20, mca_snapc_full_component.super.output_handle,
                             "Global) Job State Changed: %d (%s)\n",
                             (int)lwrrent_job_ckpt_state, state_str ));
        free(state_str);
        state_str = NULL;
         ...
         */
        if( !(global_snapshot.options->stop) && !lwrrently_migrating ) {
            write_out_global_metadata();
        }
    ...
    if( ORTE_SNAPC_CKPT_STATE_RECOVERED == loc_min_state &&
        ORTE_SNAPC_CKPT_STATE_RECOVERED > lwrrent_job_ckpt_state ) {

        /*
         * If this is a job restarting then we do something different
         */
        if( lwrrent_job_ckpt_state == ORTE_SNAPC_CKPT_STATE_NONE ) {
            OPAL_OUTPUT_VERBOSE((5, mca_snapc_full_component.super.output_handle,
         ...
         */
        if(ORTE_SNAPC_CKPT_STATE_ESTABLISHED != lwrrent_job_ckpt_state ) {
         ...
         */
        if( !cleanup_on_establish && ORTE_SNAPC_CKPT_STATE_RECOVERED == lwrrent_job_ckpt_state) {
                                              ...
                                              bool quick)
{
    ...
    if( !quick ) {
        ...
        /* In this case we want to use the lwrrent_options that are cached
...
static int snapc_full_establish_snapshot_dir(bool empty_metadata)
{
     ...
     */
    if( 0 > (idx = mca_base_var_find("opal", "mca", "base", "elwar_file_prefix")) ) {
...
static int snapc_full_global_checkpoint(opal_crs_base_ckpt_options_t *options)
{
    ...
    OPAL_OUTPUT_VERBOSE((10, mca_snapc_full_component.super.output_handle,
                         ...
                         ORTE_JOBID_PRINT(lwrrent_global_jobid)));

    /* opal_output(0, "================> JJH Checkpoint Started"); */

    lwrrent_job_ckpt_state = ORTE_SNAPC_CKPT_STATE_REQUEST;
    ...
    if( ORTE_SUCCESS != (ret = snapc_full_global_notify_checkpoint(lwrrent_global_jobid, options)) ) {
        ORTE_ERROR_LOG(ret);
        exit_status = ret;
        goto cleanup;
    }
                                                     ...
                                                     opal_crs_base_ckpt_options_t *options)
{
    ...
    if (ORTE_SUCCESS != (ret = opal_dss.pack(buffer, &(lwrrently_migrating), 1, OPAL_BOOL))) {
        ORTE_ERROR_LOG(ret);
        exit_status = ret;
        goto cleanup;
    }
                                  ...
                                  opal_crs_base_ckpt_options_t *options)
{
     ...
     ************************/
    lwrrent_job_ckpt_state = job_ckpt_state;
    if( is_orte_checkpoint_connected &&
        ORTE_SUCCESS != (ret = orte_snapc_base_global_coord_ckpt_update_cmd(&orte_checkpoint_sender,
                                                                            global_snapshot.ss_handle,
                                                                            lwrrent_job_ckpt_state)) ) {
...
static int orte_snapc_full_global_reset_coord(void)
{
    ...
    if( global_snapshot.options->term ) {
        SNAPC_FULL_DISPLAY_ALL_TIMERS();
        orte_plm.terminate_job(lwrrent_global_jobid);
    } else {
        SNAPC_FULL_DISPLAY_ALL_TIMERS();
    }
...
static void snapc_full_report_progress(orte_snapc_full_orted_snapshot_t *orted_snapshot, int total, int min_state)
{
    ...
    report_progress_lwr_loc_finished++;
    perc_done = (total-report_progress_lwr_loc_finished)/(total*1.0);
