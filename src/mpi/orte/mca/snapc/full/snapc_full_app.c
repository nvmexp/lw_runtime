...
static bool lwrrently_all_migrating = false;

static bool lwrrently_checkpointing = false;
static int  lwrrent_unique_id = 0;

static int lwrrent_cr_state = OPAL_CRS_NONE;

static orte_sstore_base_handle_t lwrrent_ss_handle = ORTE_SSTORE_HANDLE_ILWALID, last_ss_handle = ORTE_SSTORE_HANDLE_ILWALID;
static opal_crs_base_ckpt_options_t *lwrrent_options = NULL;
...
int snapc_full_app_notify_response(opal_cr_ckpt_cmd_state_t resp)
{
     ...
     */
    if( NULL == lwrrent_options ) {
        lwrrent_options = OBJ_NEW(opal_crs_base_ckpt_options_t);
    }

    if( opal_cr_lwrrently_stalled ) {
        goto STAGE_1;
    }
    ...
    lwrrent_cr_state = cr_state;

#if OPAL_ENABLE_CRDEBUG == 1
    if( lwrrent_options->attach_debugger ) {
        OPAL_OUTPUT_VERBOSE((10, mca_snapc_full_component.super.output_handle,
                             "App) notify_response: C/R Debug: Wait for debugger..."));
        MPIR_debug_with_checkpoint = true;
    }
    if( lwrrent_options->detach_debugger ) {
        OPAL_OUTPUT_VERBOSE((10, mca_snapc_full_component.super.output_handle,
 ...
 STAGE_1:
    opal_cr_lwrrently_stalled = false;
    ...
    if( orte_snapc_full_skip_app ) {
        ...
        if(OPAL_SUCCESS != (ret = opal_cr_inc_core_prep() ) ) {
            if( OPAL_EXISTS == ret ) {
                OPAL_OUTPUT_VERBOSE((5, mca_snapc_full_component.super.output_handle,
                                     ...
                                     app_pid));
                opal_cr_lwrrently_stalled = true;
                return exit_status;
            }
            else {
                opal_output(mca_snapc_full_component.super.output_handle,
         ...
         * operation though.
         */
        if( lwrrent_options->inc_prep_only ) {
            OPAL_OUTPUT_VERBOSE((10, mca_snapc_full_component.super.output_handle,
         ...
         */
        if( lwrrently_all_migrating ) {
            opal_cr_continue_like_restart = true;
            orte_cr_flush_restart_files   = false;
        }
        if( !lwrrently_migrating && lwrrently_all_migrating ) {
            OPAL_OUTPUT_VERBOSE((2, mca_snapc_full_component.super.output_handle,
            ...
            ret = opal_cr_inc_core_ckpt(app_pid, local_snapshot, lwrrent_options, &cr_state);
        }
        lwrrent_cr_state = cr_state;
 ...
 ckpt_cleanup:
    ...
    if( NULL != lwrrent_options ) {
        OBJ_RELEASE(lwrrent_options);
        lwrrent_options = NULL;
    }

    lwrrently_checkpointing = false;
...
static int app_notify_resp_stage_1(opal_cr_ckpt_cmd_state_t resp)
{
    ...
    if( OPAL_SUCCESS != (ret = orte_sstore.register_handle(lwrrent_ss_handle)) ) {
        ORTE_ERROR_LOG(ret);
        exit_status = ret;
        goto cleanup;
    }
    ...
    if( !lwrrently_migrating && lwrrently_all_migrating ) {
        orte_sstore.set_attr(lwrrent_ss_handle,
                             SSTORE_METADATA_LOCAL_SKIP_CKPT,
                             "1");
    }

    orte_sstore.get_attr(lwrrent_ss_handle,
    ...
    OPAL_OUTPUT_VERBOSE((10, mca_snapc_full_component.super.output_handle,
                         "App) notify_response: Start checkpoint... (%d)", (int)lwrrent_ss_handle));

 cleanup:
    OPAL_OUTPUT_VERBOSE((10, mca_snapc_full_component.super.output_handle,
                         "App) notify_response: Are we migrating [%5s]. Am I migrating [%5s]",
                         (lwrrently_all_migrating ? "True" : "False"),
                         (lwrrently_migrating ? "True" : "False") ));
...
static int app_notify_resp_stage_2(int cr_state )
{
    ...
    if( !(lwrrent_options->stop) ) {
        ...
        orte_sstore.sync(lwrrent_ss_handle);
    }
    last_ss_handle = lwrrent_ss_handle;
    lwrrent_ss_handle = 0;
...
static int snapc_full_app_ckpt_handshake_start(opal_cr_ckpt_cmd_state_t resp)
{
    ...
    lwrrently_all_migrating = OPAL_INT_TO_BOOL(opt_rep);
    ...
    lwrrently_migrating = OPAL_INT_TO_BOOL(opt_rep);
    ...
    lwrrent_options->term = OPAL_INT_TO_BOOL(opt_rep);
    ...
    lwrrent_options->stop = OPAL_INT_TO_BOOL(opt_rep);
    ...
    lwrrent_options->inc_prep_only = OPAL_INT_TO_BOOL(opt_rep);
    ...
    lwrrent_options->inc_recover_only = OPAL_INT_TO_BOOL(opt_rep);
    ...
    lwrrent_options->attach_debugger = OPAL_INT_TO_BOOL(opt_rep);
    ...
    lwrrent_options->detach_debugger = OPAL_INT_TO_BOOL(opt_rep);
#endif

    /*
     * Get SStore Handle
     */
    if( sizeof(orte_sstore_base_handle_t) != (ret = read(app_comm_pipe_r_fd, &lwrrent_ss_handle, sizeof(orte_sstore_base_handle_t))) ) {
        opal_output(mca_snapc_full_component.super.output_handle,
    ...
    /*
     * Respond that the checkpoint is lwrrently in progress
     */
    ...
    /*
     * Respond that some error has oclwrred such that the application is
...
static int snapc_full_app_ckpt_handshake_end(int cr_state)
{
    ...
    if( lwrrently_all_migrating && lwrrently_migrating ) {
...
int app_coord_request_op(orte_snapc_base_request_op_t *datum)
{
    ...
    if( ORTE_SNAPC_OP_QUIESCE_END == datum->event) {
        ...
        app_pid = getpid();
        cr_state = OPAL_CRS_RUNNING;
        if( OPAL_SUCCESS != (ret = opal_cr_inc_core_ckpt(app_pid, local_snapshot, lwrrent_options, &cr_state)) ) {
            ORTE_ERROR_LOG(ret);
            exit_status = ret;
        }
    ...
    if( ORTE_SNAPC_OP_CHECKPOINT == datum->event) {
        if( datum->leader == (int)ORTE_PROC_MY_NAME->vpid ) {
             ...
             */
            while(OPAL_CRS_CONTINUE != lwrrent_cr_state &&
                  OPAL_CRS_RESTART  != lwrrent_cr_state &&
                  OPAL_CRS_ERROR    != lwrrent_cr_state ) {
                opal_progress();
                OPAL_CR_TEST_CHECKPOINT_READY();
            }

            /* Do not wait for a response if we are restarting (it will never arrive) */
            if( OPAL_CRS_RESTART == lwrrent_cr_state ) {
                orte_sstore.get_attr(lwrrent_ss_handle,
                                     SSTORE_METADATA_GLOBAL_SNAP_SEQ,
                                     &seq_str);
                if( NULL != seq_str ) {
                ...
                lwrrent_cr_state = OPAL_CRS_NONE;
    ...
    /*
     * No waiting for Quiesce end (barrier oclwrs in protocol)
     */
