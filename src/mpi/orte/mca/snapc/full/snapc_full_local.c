 ...
 ************************************/
static orte_jobid_t lwrrent_local_jobid = ORTE_JOBID_ILWALID;
...
{
    lwrrent_local_jobid = ORTE_JOBID_ILWALID;
...
{
    if( ORTE_JOBID_ILWALID != lwrrent_local_jobid ) {
        return local_coord_release_job(lwrrent_local_jobid);
    }
...
int local_coord_setup_job(orte_jobid_t jobid)
{
    ...
    else if( ORTE_JOBID_ILWALID != lwrrent_local_jobid ) {
        opal_output(mca_snapc_full_component.super.output_handle,
                    "Local) Setup of job %s Failed! Already setup job %s\n",
                    ORTE_JOBID_PRINT(jobid), ORTE_JOBID_PRINT(lwrrent_local_jobid));
        exit_status = ORTE_SUCCESS;
        goto cleanup;
    }

    lwrrent_local_jobid = jobid;
    OPAL_OUTPUT_VERBOSE((10, mca_snapc_full_component.super.output_handle,
                         "Local) Setting up jobid %s\n",
                         ORTE_JOBID_PRINT(lwrrent_local_jobid)));
                                                    ...
                                                    bool quick)
{
    ...
    if( !quick ) {
        ...
        /* In this case we want to use the lwrrent_local_options that are cached
                                                      ...
                                                      opal_crs_base_ckpt_options_t *options)
{
    ...
    opt_rep = (int)(lwrrently_migrating);
...
static void snapc_full_local_comm_read_event(int fd, short flags, void *arg)
{
     ...
     */
    if( lwrrently_migrating && !flushed_modex ) {
        OPAL_OUTPUT_VERBOSE((10, mca_snapc_full_component.super.output_handle,
