                                                ...
                                                orte_jobid_t *job)
{
    ...
    *job = ORTE_JOBID_ILWALID;
    ...
    if (0 != flag) {
        ...
        while (ORTE_SUCCESS == (rc = opal_dss.unpack(bptr, &jptr, &cnt, OPAL_BUFFER))) {
            ...
            if (NULL == orte_get_job_data_object(jdata->jobid)) {
                ...
                jdata->jobid = ORTE_JOBID_ILWALID;
                OBJ_RELEASE(jdata);
                OBJ_RELEASE(jptr);
                cnt=1;
                continue;
            ...
            for (v=0; v < jdata->num_procs; v++) {
                ...
                cnt=1;
                if (ORTE_SUCCESS != (rc = opal_dss.unpack(jptr, &dmlwpid, &cnt, ORTE_VPID))) {
                    ORTE_ERROR_LOG(rc);
                    OBJ_RELEASE(jptr);
                    OBJ_RELEASE(bptr);
                    goto REPORT_ERROR;
                ...
                /* lookup the daemon */
                if (NULL == (dmn = (orte_proc_t*)opal_pointer_array_get_item(daemons->procs, dmlwpid))) {
                    ORTE_ERROR_LOG(ORTE_ERR_NOT_FOUND);
    ...
    if (ORTE_JOBID_ILWALID == jdata->jobid) {
        ORTE_ERROR_LOG(ORTE_ERR_BAD_PARAM);
    ...
    *job = jdata->jobid;
    ...
    if (OPAL_SUCCESS == rc) {
        ...
        while (ORTE_SUCCESS == (rc = opal_dss.unpack(bptr, &kv, &cnt, OPAL_VALUE))) {
            /* if this is an elwar operation, cache it in reverse order
             * so that the order the user provided is preserved */
            if (0 == strcmp(kv->key, OPAL_PMIX_SET_ELWAR) ||
                0 == strcmp(kv->key, OPAL_PMIX_ADD_ELWAR) ||
                0 == strcmp(kv->key, OPAL_PMIX_UNSET_ELWAR) ||
                0 == strcmp(kv->key, OPAL_PMIX_PREPEND_ELWAR) ||
                0 == strcmp(kv->key, OPAL_PMIX_APPEND_ELWAR)) {
        ...
        while (NULL != (kv = (opal_value_t*)opal_list_remove_first(&cache))) {
            if (0 == strcmp(kv->key, OPAL_PMIX_SET_ELWAR)) {
                orte_prepend_attribute(&jdata->attributes, ORTE_JOB_SET_ELWAR,
                                       ORTE_ATTR_GLOBAL, &kv->data.elwar, OPAL_ELWAR);
            } else if (0 == strcmp(kv->key, OPAL_PMIX_ADD_ELWAR)) {
                orte_prepend_attribute(&jdata->attributes, ORTE_JOB_ADD_ELWAR,
                                       ORTE_ATTR_GLOBAL, &kv->data.elwar, OPAL_ELWAR);
            } else if (0 == strcmp(kv->key, OPAL_PMIX_UNSET_ELWAR)) {
                orte_prepend_attribute(&jdata->attributes, ORTE_JOB_UNSET_ELWAR,
                                       ORTE_ATTR_GLOBAL, kv->data.string, OPAL_STRING);
            } else if (0 == strcmp(kv->key, OPAL_PMIX_PREPEND_ELWAR)) {
                orte_prepend_attribute(&jdata->attributes, ORTE_JOB_PREPEND_ELWAR,
                                       ORTE_ATTR_GLOBAL, &kv->data.elwar, OPAL_ELWAR);
            } else if (0 == strcmp(kv->key, OPAL_PMIX_APPEND_ELWAR)) {
                orte_prepend_attribute(&jdata->attributes, ORTE_JOB_APPEND_ELWAR,
                                       ORTE_ATTR_GLOBAL, &kv->data.elwar, OPAL_ELWAR);
            }
            OBJ_RELEASE(kv);
        }
    ...
    for (n=0; n < jdata->procs->size; n++) {
        ...
        if (!ORTE_PROC_IS_HNP &&
            ...
            if (ORTE_VPID_ILWALID == pptr->parent) {
                ORTE_ERROR_LOG(ORTE_ERR_BAD_PARAM);
                rc = ORTE_ERR_BAD_PARAM;
                goto REPORT_ERROR;
            }
...
static int setup_path(orte_app_context_t *app, char **wdir)
{
    ...
    if (!orte_get_attribute(&app->attributes, ORTE_APP_SSNDIR_CWD, NULL, OPAL_BOOL)) {
        /* Try to change to the app's cwd and check that the app
           exists and is exelwtable The function will
        ...
        */
         ...
         * wherever the app is to be exelwted. This could be either where
        ...
        *wdir = strdup(dir);
        opal_setelw("PWD", dir, true, &app->elw);
        /* update the initial wdir value too */
        opal_setelw(OPAL_MCA_PREFIX"initial_wdir", dir, true, &app->elw);
    } else {
        *wdir = NULL;
    }
...
void orte_odls_base_spawn_proc(int fd, short sd, void *cbdata)
{
    ...
    if (OPAL_SUCCESS != (rc = opal_pmix.server_setup_fork(&child->name, &cd->elw))) {
        ORTE_ERROR_LOG(rc);
     ...
     */
    if (ORTE_SUCCESS != (rc = orte_schizo.setup_child(jobdat, child, app, &cd->elw))) {
        ORTE_ERROR_LOG(rc);
        state = ORTE_PROC_STATE_FAILED_TO_LAUNCH;
        goto errorout;
    }
    ...
    if (NULL != orte_xterm && !ORTE_FLAG_TEST(jobdat, ORTE_JOB_FLAG_DEBUGGER_DAEMON)) {
        ...
        cd->cmd = opal_path_findv(orte_fork_agent[0], X_OK, orte_launch_elwiron, NULL);
        if (NULL == cd->cmd) {
            orte_show_help("help-orte-odls-base.txt",
...
void orte_odls_base_default_launch_local(int fd, short sd, void *cbdata)
{
    ...
    char **argvptr;
    char *pathelw = NULL, *mpiexec_pathelw = NULL;
    char *full_search;
    ...
    /* compute the total number of local procs lwrrently alive and about to be launched */
    ...
    for (j=0; j < jobdat->apps->size; j++) {
        ...
        for (argvptr = app->elw; *argvptr != NULL; argvptr++) {
            if (0 == strncmp("OMPI_exec_path=", *argvptr, 15)) {
                mpiexec_pathelw = *argvptr + 15;
            }
        ...
        /* If OMPI_exec_path is set (meaning --path was used), then create a
           temporary environment to be used in the search for the exelwtable.
        ...
        if (NULL != mpiexec_pathelw) {
            argvptr = NULL;
            if (pathelw != NULL) {
                asprintf(&full_search, "%s:%s", mpiexec_pathelw, pathelw);
            } else {
                asprintf(&full_search, "%s", mpiexec_pathelw);
            }
            opal_setelw("PATH", full_search, true, &argvptr);
            free(full_search);
        } else {
            argvptr = app->elw;
        }
        ...
        opal_setelw(OPAL_MCA_PREFIX"orte_launch", "1", true, &app->elw);
                                        ...
                                        orte_odls_base_fork_local_proc_fn_t fork_local)
{
    ...
    /* reset elwars to match this child */
    if (ORTE_SUCCESS != (rc = orte_schizo.setup_child(jobdat, child, app, &app->elw))) {
        ORTE_ERROR_LOG(rc);
        goto CLEANUP;
    }
