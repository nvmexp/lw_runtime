...
/*
 * There is a complicated sequence of events that oclwrs when the
 * parent forks a child process that is intended to launch the target
 * exelwtable.
 *
 * Before the child process exec's the target exelwtable, it might tri
 ...
 * oclwrately to the end user.  This is a complex task in itself
 ...
 *
 * Here's a high-level description of what is oclwrring in this file:
 *
 * - parent opens a pipe
 ...
 *   preparation of exec'ing the target exelwtable
 ...
 *   only entity that has enough information to make an aclwate error string
 ...
 *   (potentially eventually exec'ing the target exelwtable).
...
static int do_child(orte_odls_spawn_caddy_t *cd, int write_fd)
{
    ...
    if (NULL != cd->child) {
        /* setup stdout/stderr so that any error messages that we
           ...
           THEN a user who gives us a bad exelwtable name or
	...
	if (ORTE_FLAG_TEST(cd->jdata, ORTE_JOB_FLAG_FORWARD_OUTPUT)) {
	    if (ORTE_SUCCESS != (i = orte_iof_base_setup_child(&cd->opts, &cd->elw))) {
		ORTE_ERROR_LOG(i);
        ...
        orte_rtc.set(cd->jdata, cd->child, &cd->elw, write_fd);
    ...
    if (10 < opal_output_get_verbosity(orte_odls_base_framework.framework_output)) {
        ...
        for (jout=0; NULL != cd->elw[jout]; jout++) {
            opal_output(0, "%s\tELWIRON[%d]: %s", ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), jout, cd->elw[jout]);
        }
    }

    execve(cd->cmd, cd->argv, cd->elw);
    send_error_show_help(write_fd, 1,
