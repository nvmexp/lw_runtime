...
int orte_odls_alps_get_rdma_creds(void)
{
    ...
    char *ptr;
    char elw_buffer[1024];
    static int already_got_creds = 0;

    /*
     * If we already put the GNI RDMA credentials into orte_launch_elwiron,
     * no need to do anything.
     * TODO: kind of ugly, need to implement an opal_getelw
     */
     ...
     * PMI elw variable format expected by uGNI consumers like the uGNI
     * BTL, etc. Stuff into the orte_launch_elwiron to make sure the
    ...
    if (ORTE_PROC_IS_DAEMON) {
         ...
         * where X == num_creds - 1
         *
         * TODO: need in theory to check for possible overrun of elw_buffer
         */

        num_creds = rdmacred_rsp->count;

        /*
         * first build ptag elw
         */

        memset(elw_buffer,0,sizeof(elw_buffer));
        ptr = elw_buffer;
        ...
        ret = opal_setelw("PMI_GNI_PTAG", elw_buffer, false, &orte_launch_elwiron);
        if (ret != ORTE_SUCCESS) {
            OPAL_OUTPUT_VERBOSE((20, orte_odls_base_framework.framework_output,
                                 "%s odls:alps: opal_setelw for PMI_GNI_TAG returned %d",
                                 ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), ret));
            goto fn_exit;
        }
        ...
        ret = opal_setelw("PMI_GNI_COOKIE", elw_buffer, false, &orte_launch_elwiron);
        if (ret != ORTE_SUCCESS) {
            OPAL_OUTPUT_VERBOSE((20, orte_odls_base_framework.framework_output,
                                 "%s odls:alps: opal_setelw for PMI_GNI_COOKIE returned %d",
                                 ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), ret));
            goto fn_exit;
        }
        ...
        ret = opal_setelw("PMI_GNI_LOC_ADDR", elw_buffer, false, &orte_launch_elwiron);
        if (ret != ORTE_SUCCESS) {
            OPAL_OUTPUT_VERBOSE((20, orte_odls_base_framework.framework_output,
                                 "%s odls:alps: opal_setelw for PMI_GNI_LOC_ADDR returned %d",
                                 ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), ret));
            goto fn_exit;
        }
        ...
        ret = opal_setelw("PMI_GNI_DEV_ID", elw_buffer, false, &orte_launch_elwiron);
        if (ret != ORTE_SUCCESS) {
            OPAL_OUTPUT_VERBOSE((20, orte_odls_base_framework.framework_output,
                                 "%s odls:alps: opal_setelw for PMI_GNI_DEV_ID returned %d",
                                 ORTE_NAME_PRINT(ORTE_PROC_MY_NAME), ret));
            goto fn_exit;
        }
