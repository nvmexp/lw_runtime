                                                        ...
                                                        opal_list_t *local_snapshots)
{
    ...
    int argc = 0;
    orte_app_context_t *lwr_app_context = NULL;
     ...
     */
    lwr_app_context = NULL;
    for(i_app = 0; i_app < opal_pointer_array_get_size(jobdata->apps); ++i_app) {
        lwr_app_context = (orte_app_context_t *)opal_pointer_array_get_item(jobdata->apps,
                                                                            i_app);
        if( NULL == lwr_app_context ) {
            continue;
        }
        if(proc->app_idx == lwr_app_context->idx) {
            break;
        }
    }
     ...
     */
    if( lwr_app_context->num_procs > 1 ) {

        /* Create a new app_context */
        opal_dss.copy((void**)&new_app_context, lwr_app_context, ORTE_APP_CONTEXT);

        /* clear unused attributes */
        new_app_context->idx                    = lwr_app_context->idx;
        ...
        /* Add it to the job_t data structure */
        /*lwrrent_global_jobdata->num_apps++; */
        ...
        --(lwr_app_context->num_procs);
    }
    else {
        new_app_context = lwr_app_context;
...
int orte_errmgr_base_restart_job(orte_jobid_t jobid, char * global_handle, int seq_num)
{
    ...
    orte_job_t *jdata;
    orte_sstore_base_handle_t prev_sstore_handle = ORTE_SSTORE_HANDLE_ILWALID;
...
int orte_errmgr_base_migrate_job(orte_jobid_t jobid, orte_snapc_base_request_op_t *datum)
{
    ...
    for( i = 0; i < datum->mig_num; ++i ) {
        ...
        if( (datum->mig_off_node)[i] ) {
            onto_map->off_lwrrent_node = true;
        } else {
            onto_map->off_lwrrent_node = false;
        }
