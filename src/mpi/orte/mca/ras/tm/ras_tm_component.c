...
static int ras_tm_register(void)
{
    ...
    pbs_nodefile_elw = getelw("PBS_NODEFILE");
    if (NULL != pbs_nodefile_elw) {
        mca_ras_tm_component.nodefile_dir = opal_dirname(pbs_nodefile_elw);
    }
     ...
     * choose to list each node once, but then provide an elwar that
