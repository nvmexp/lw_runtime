...
static int orte_ras_slurm_allocate(orte_job_t *jdata, opal_list_t *nodes)
{
    ...
    if (NULL == (slurm_jobid = getelw("SLURM_JOBID"))) {
    ...
    slurm_node_str = getelw("SLURM_NODELIST");
    if (NULL == slurm_node_str) {
        ...
        orte_show_help("help-ras-slurm.txt", "slurm-elw-var-not-found", 1,
                       "SLURM_NODELIST");
    ...
    if (mca_ras_slurm_component.use_all) {
        ...
        tasks_per_node = getelw("SLURM_JOB_CPUS_PER_NODE");
        ...
        tasks_per_node = getelw("SLURM_TASKS_PER_NODE");
        if (NULL == tasks_per_node) {
            /* couldn't find any version - abort */
            orte_show_help("help-ras-slurm.txt", "slurm-elw-var-not-found", 1,
        ...
        tmp = getelw("SLURM_CPUS_PER_TASK");
        if(NULL != tmp) {
                                   ...
                                   opal_list_t* nodelist)
{
    ...
    do {
        ...
        if(i == 0) {
            ...
            orte_show_help("help-ras-slurm.txt", "slurm-elw-var-bad-value",
    ...
    while (begptr) {
        ...
        if (*endptr == ',') {
            ...
            orte_show_help("help-ras-slurm.txt", "slurm-elw-var-bad-value", 1,
    ...
    /* Colwert the argv of node names to a list of node_t's */
...
static void recv_data(int fd, short args, void *cbdata)
{
    ...
    tpn = strchr(alloc[0], '=');
    orte_util_colwert_string_to_jobid(&jobid, tpn+1);
    ...
    if (opal_list_is_empty(&nds)) {
         ...
         * However, slurm indicated that nothing is lwrrently
...
static int dyn_allocate(orte_job_t *jdata)
{
     ...
     * need to do a little work to build the command. We don't lwrrently
    ...
    /* add the jobid */
    orte_util_colwert_jobid_to_string(&jstring, jdata->jobid);
     ...
     * then tell slurm
     *
     * RHC: we don't lwrrently have the ability to handle
