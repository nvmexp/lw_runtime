...
static int parse_elw(char *path,
...
static opal_cmd_line_init_t cmd_line_init[] = {
    ...
    { "orte_exelwte_quiet", 'q', NULL, "quiet", 0,
    ...
    { "orte_output_filename", '\0', "output-filename", "output-filename", 1,
      ...
      "Redirect output from application processes into filename/job/rank/std[out,err,diag]. A relative path value will be colwerted to an absolute path",
    ...
    { NULL, 'H', "host", "host", 1,
      NULL, OPAL_CMD_LINE_TYPE_STRING,
      "List of hosts to ilwoke processes on",
    ...
    { NULL, '\0', "path", "path", 1,
      ...
      "PATH to be used to look for exelwtables to start processes",
      OPAL_CMD_LINE_OTYPE_LAUNCH },
    ...
    { NULL, '\0', "debug", "debug", 0,
      &orte_cmd_options.debugger, OPAL_CMD_LINE_TYPE_BOOL,
      "Ilwoke the user-level debugger indicated by the orte_base_user_debugger MCA parameter",
      OPAL_CMD_LINE_OTYPE_DEBUG },
    ...
    { NULL, '\0', "allow-run-as-root", "allow-run-as-root", 0,
      ...
      "Allow exelwtion as root (STRONGLY DISCOURAGED)",
      OPAL_CMD_LINE_OTYPE_LAUNCH },
...
static int define_cli(opal_cmd_line_t *cli)
{
     ...
     * if user passes both elwvar & cmdln, the cmdln wins.
     */
    if (NULL != getelw("ORTE_HNP_DVM_URI")) {
        orte_cmd_options.hnp = strdup(getelw("ORTE_HNP_DVM_URI"));
    }
...
static int parse_cli(int argc, int start, char **argv)
{
    ...
    for (i = 0; i < (argc-start); ++i) {
        if (0 == strcmp("-mca",  argv[i]) ||
            0 == strcmp("--mca", argv[i]) ) {
            /* ignore this one */
            if (0 == strcmp(argv[i+1], "mca_base_elw_list")) {
                i += 2;
                continue;
            }
                     ...
                     char ***dstelw)
{
    ...
    for (i = 0; NULL != srcelw[i]; ++i) {
        if (0 == strncmp("OMPI_", srcelw[i], 5) ||
            0 == strncmp("PMIX_", srcelw[i], 5)) {
            /* check for duplicate in app->elw - this
             * would have been placed there by the
             * cmd line processor. By convention, we
             * always let the cmd line override the
             * environment
             */
            param = strdup(srcelw[i]);
            value = strchr(param, '=');
            *value = '\0';
            value++;
            opal_setelw(param, value, false, dstelw);
            free(param);
        }
    }

    /* set necessary elw variables for external usage from tune conf file*/
    int set_from_file = 0;
    vars = NULL;
    if (OPAL_SUCCESS == mca_base_var_process_elw_list_from_file(&vars) &&
            NULL != vars) {
        for (i=0; NULL != vars[i]; i++) {
            ...
            *value = '\0';
            ...
            opal_setelw(vars[i], value, true, dstelw);
            /* save it for any comm_spawn'd apps */
            opal_setelw(vars[i], value, true, &orte_forwarded_elwars);
        }
        set_from_file = 1;
        opal_argv_free(vars);
    }
    /* Did the user request to export any environment variables on the cmd line? */
    elw_set_flag = getelw("OMPI_MCA_mca_base_elw_list");
    if (opal_cmd_line_is_taken(cmd_line, "x")) {
        if (NULL != elw_set_flag) {
            orte_show_help("help-orterun.txt", "orterun:conflict-elw-set", false);
        ...
        for (i = 0; i < j; ++i) {
            ...
            if (NULL != (value = strchr(param, '='))) {
                ...
                *value = '\0';
                ...
                value++;
                /* overwrite any prior entry */
                opal_setelw(param, value, true, dstelw);
                /* save it for any comm_spawn'd apps */
                opal_setelw(param, value, true, &orte_forwarded_elwars);
            } else {
                value = getelw(param);
                if (NULL != value) {
                    /* overwrite any prior entry */
    ...
    } else if (NULL != elw_set_flag) {
        /* if mca_base_elw_list was set, check if some of elw vars were set via -x from a conf file.
         * If this is the case, error out.
         */
        if (!set_from_file) {
            /* set necessary elw variables for external usage */
            vars = NULL;
            if (OPAL_SUCCESS == mca_base_var_process_elw_list(elw_set_flag, &vars) &&
                    NULL != vars) {
                for (i=0; NULL != vars[i]; i++) {
                    value = strchr(vars[i], '=');
                    /* terminate the name of the param */
    ...
    if (NULL != path) {
        ...
        opal_argv_append_nosize(dstelw, value);
        /* save it for any comm_spawn'd apps */
        opal_argv_append_nosize(&orte_forwarded_elwars, value);
        free(value);
    }
                      ...
                      orte_app_context_t *app)
{
    ...
    /* setup base environment: copy the current elwiron and merge
       in the app context elwiron */
    if (NULL != app->elw) {
        /* manually free original context->elw to avoid a memory leak */
        char **tmp = app->elw;
        elwcpy = opal_elwiron_merge(orte_launch_elwiron, app->elw);
        if (NULL != tmp) {
            opal_argv_free(tmp);
        }
    } else {
        elwcpy = opal_argv_copy(orte_launch_elwiron);
    }
    app->elw = elwcpy;
    ...
    /* special case handling for --prefix: this is somewhat icky,
       ...
       LD_LIBRARY_PATH.  So examine the context->elw and see if we
    ...
    for (i = 0; NULL != param && NULL != app->elw && NULL != app->elw[i]; ++i) {
        char *newelw;

        /* Reset PATH */
        if (0 == strncmp("PATH=", app->elw[i], 5)) {
            asprintf(&newelw, "%s/bin:%s", param, app->elw[i] + 5);
            opal_setelw("PATH", newelw, true, &app->elw);
            free(newelw);
        }

        /* Reset LD_LIBRARY_PATH */
        else if (0 == strncmp("LD_LIBRARY_PATH=", app->elw[i], 16)) {
            asprintf(&newelw, "%s/lib:%s", param, app->elw[i] + 16);
            opal_setelw("LD_LIBRARY_PATH", newelw, true, &app->elw);
            free(newelw);
        }
    }
    if (NULL != param) {
    ...
    opal_setelw("OMPI_MCA_orte_local_daemon_uri", orte_process_info.my_daemon_uri, true, &app->elw);

    /* pass the hnp's contact info to the local proc in case it
     * needs it
     */
    if (NULL != orte_process_info.my_hnp_uri) {
        opal_setelw("OMPI_MCA_orte_hnp_uri", orte_process_info.my_hnp_uri, true, &app->elw);
    }

    /* setup yield schedule - do not override any user-supplied directive! */
    if (oversubscribed) {
        opal_setelw("OMPI_MCA_mpi_oversubscribe", "1", true, &app->elw);
    } else {
        opal_setelw("OMPI_MCA_mpi_oversubscribe", "0", true, &app->elw);
    }

    /* set the app_context number into the environment */
    asprintf(&param, "%ld", (long)app->idx);
    opal_setelw("OMPI_MCA_orte_app_num", param, true, &app->elw);
    free(param);

    /* although the total_slots_alloc is the universe size, users
     * would appreciate being given a public elwironmental variable
    ...
    opal_setelw("OMPI_UNIVERSE_SIZE", param, true, &app->elw);
    free(param);

    /* pass the number of nodes ilwolved in this job */
    asprintf(&param, "%ld", (long)(jdata->map->num_nodes));
    opal_setelw("OMPI_MCA_orte_num_nodes", param, true, &app->elw);
    free(param);
    ...
    if (NULL != opal_hwloc_topology) {
        ...
        if (NULL != (htmp = (char*)hwloc_obj_get_info_by_name(obj, "CPUType")) ||
            ...
            opal_setelw("OMPI_MCA_orte_cpu_type", htmp, true, &app->elw);
        }
        if (NULL != (htmp = (char*)hwloc_obj_get_info_by_name(obj, "CPUModel")) ||
            NULL != (htmp = orte_local_cpu_model)) {
            opal_setelw("OMPI_MCA_orte_cpu_model", htmp, true, &app->elw);
        }
    } else {
        if (NULL != orte_local_cpu_type) {
            opal_setelw("OMPI_MCA_orte_cpu_type", orte_local_cpu_type, true, &app->elw);
        }
        if (NULL != orte_local_cpu_model) {
            opal_setelw("OMPI_MCA_orte_cpu_model", orte_local_cpu_model, true, &app->elw);
        }
    }
    ...
    if (NULL != (param = opal_shmem_base_best_runnable_component_name())) {
        opal_setelw("OMPI_MCA_shmem_RUNTIME_QUERY_hint", param, true, &app->elw);
        free(param);
    }
    ...
    if (OPAL_BIND_TO_NONE != OPAL_GET_BINDING_POLICY(jdata->map->binding)) {
        opal_setelw("OMPI_MCA_orte_bound_at_launch", "1", true, &app->elw);
    }

    /* tell the ESS to avoid the singleton component - but don't override
     * anything that may have been provided elsewhere
     */
    opal_setelw("OMPI_MCA_ess", "^singleton", false, &app->elw);

    /* ensure that the spawned process ignores direct launch components,
     * but do not overrride anything we were given */
    opal_setelw("OMPI_MCA_pmix", "^s1,s2,cray", false, &app->elw);

    /* since we want to pass the name as separate components, make sure
     * that the "name" elwironmental variable is cleared!
     */
    opal_unsetelw("OMPI_MCA_orte_ess_name", &app->elw);

    asprintf(&param, "%ld", (long)jdata->num_procs);
    opal_setelw("OMPI_MCA_orte_ess_num_procs", param, true, &app->elw);
     ...
     * We know - just live with it
     */
    opal_setelw("OMPI_COMM_WORLD_SIZE", param, true, &app->elw);
    free(param);

    /* users would appreciate being given a public elwironmental variable
     * that also represents this value - something MPI specific - so
     * do that here.
     *
     * AND YES - THIS BREAKS THE ABSTRACTION BARRIER TO SOME EXTENT.
    ...
    opal_setelw("OMPI_COMM_WORLD_LOCAL_SIZE", param, true, &app->elw);
    free(param);

    /* forcibly set the local tmpdir base and top session dir to match ours */
    opal_setelw("OMPI_MCA_orte_tmpdir_base", orte_process_info.tmpdir_base, true, &app->elw);
    /* TODO: should we use PMIx key to pass this data? */
    opal_setelw("OMPI_MCA_orte_top_session_dir", orte_process_info.top_session_dir, true, &app->elw);
    opal_setelw("OMPI_MCA_orte_jobfam_session_dir", orte_process_info.jobfam_session_dir, true, &app->elw);

    /* MPI-3 requires we provide some further info to the procs,
     * so we pass them as elwars to avoid introducing further
     * ORTE calls in the MPI layer
     */
    asprintf(&num_app_ctx, "%lu", (unsigned long)jdata->num_apps);

    /* build some common elwars we need to pass for MPI-3 compatibility */
    nps = NULL;
    firstranks = NULL;
    ...
    /* add the MPI-3 elwars */
    opal_setelw("OMPI_NUM_APP_CTX", num_app_ctx, true, &app->elw);
    opal_setelw("OMPI_FIRST_RANKS", firstrankstring, true, &app->elw);
    opal_setelw("OMPI_APP_CTX_NUM_PROCS", npstring, true, &app->elw);
    free(num_app_ctx);
    free(firstrankstring);
    free(npstring);

    /* now process any elwar attributes - we begin with the job-level
    ...
    OPAL_LIST_FOREACH(attr, &jdata->attributes, orte_attribute_t) {
        if (ORTE_JOB_SET_ELWAR == attr->key) {
            opal_setelw(attr->data.elwar.elwar, attr->data.elwar.value, true, &app->elw);
        } else if (ORTE_JOB_ADD_ELWAR == attr->key) {
            opal_setelw(attr->data.elwar.elwar, attr->data.elwar.value, false, &app->elw);
        } else if (ORTE_JOB_UNSET_ELWAR == attr->key) {
            opal_unsetelw(attr->data.string, &app->elw);
        } else if (ORTE_JOB_PREPEND_ELWAR == attr->key) {
            /* see if the elwar already exists */
            exists = false;
            for (i=0; NULL != app->elw[i]; i++) {
                saveptr = strchr(app->elw[i], '=');   // cannot be NULL
                *saveptr = '\0';
                if (0 == strcmp(app->elw[i], attr->data.elwar.elwar)) {
                    /* we have the var - prepend it */
                    param = saveptr;
                    ++param;  // move past where the '=' sign was
                    (void)asprintf(&p2, "%s%c%s", attr->data.elwar.value,
                                   attr->data.elwar.separator, param);
                    *saveptr = '=';  // restore the current elwar setting
                    opal_setelw(attr->data.elwar.elwar, p2, true, &app->elw);
                    free(p2);
                    exists = true;
                    break;
                } else {
        ...
        } else if (ORTE_JOB_APPEND_ELWAR == attr->key) {
            /* see if the elwar already exists */
            exists = false;
            for (i=0; NULL != app->elw[i]; i++) {
                saveptr = strchr(app->elw[i], '=');   // cannot be NULL
                *saveptr = '\0';
                if (0 == strcmp(app->elw[i], attr->data.elwar.elwar)) {
                    ...
                    param = saveptr;
                    ++param;  // move past where the '=' sign was
                    (void)asprintf(&p2, "%s%c%s", param, attr->data.elwar.separator,
                                   attr->data.elwar.value);
                    *saveptr = '=';  // restore the current elwar setting
                    opal_setelw(attr->data.elwar.elwar, p2, true, &app->elw);
                    free(p2);
                    exists = true;
    ...
    OPAL_LIST_FOREACH(attr, &app->attributes, orte_attribute_t) {
        if (ORTE_APP_SET_ELWAR == attr->key) {
            opal_setelw(attr->data.elwar.elwar, attr->data.elwar.value, true, &app->elw);
        } else if (ORTE_APP_ADD_ELWAR == attr->key) {
            opal_setelw(attr->data.elwar.elwar, attr->data.elwar.value, false, &app->elw);
        } else if (ORTE_APP_UNSET_ELWAR == attr->key) {
            opal_unsetelw(attr->data.string, &app->elw);
        } else if (ORTE_APP_PREPEND_ELWAR == attr->key) {
            /* see if the elwar already exists */
            exists = false;
            for (i=0; NULL != app->elw[i]; i++) {
                saveptr = strchr(app->elw[i], '=');   // cannot be NULL
        ...
        } else if (ORTE_APP_APPEND_ELWAR == attr->key) {
            /* see if the elwar already exists */
            exists = false;
            for (i=0; NULL != app->elw[i]; i++) {
                saveptr = strchr(app->elw[i], '=');   // cannot be NULL
                       ...
                       char ***elw)
{
    ...
    /* setup the jobid */
    if (ORTE_SUCCESS != (rc = orte_util_colwert_jobid_to_string(&value, child->name.jobid))) {
        ORTE_ERROR_LOG(rc);
        return rc;
    }
    opal_setelw("OMPI_MCA_ess_base_jobid", value, true, elw);
    free(value);

    /* setup the vpid */
    if (ORTE_SUCCESS != (rc = orte_util_colwert_vpid_to_string(&value, child->name.vpid))) {
        ORTE_ERROR_LOG(rc);
        return rc;
    }
    opal_setelw("OMPI_MCA_ess_base_vpid", value, true, elw);
     ...
     * We know - just live with it
     */
    opal_setelw("OMPI_COMM_WORLD_RANK", value, true, elw);
     ...
     * We know - just live with it
     */
    if (ORTE_LOCAL_RANK_ILWALID == child->local_rank) {
        ORTE_ERROR_LOG(ORTE_ERR_VALUE_OUT_OF_BOUNDS);
        rc = ORTE_ERR_VALUE_OUT_OF_BOUNDS;
        return rc;
    }
    asprintf(&value, "%lu", (unsigned long) child->local_rank);
    opal_setelw("OMPI_COMM_WORLD_LOCAL_RANK", value, true, elw);
    free(value);
     ...
     * We know - just live with it
     */
    if (ORTE_NODE_RANK_ILWALID == child->node_rank) {
        ORTE_ERROR_LOG(ORTE_ERR_VALUE_OUT_OF_BOUNDS);
        rc = ORTE_ERR_VALUE_OUT_OF_BOUNDS;
        return rc;
    }
    asprintf(&value, "%lu", (unsigned long) child->node_rank);
    opal_setelw("OMPI_COMM_WORLD_NODE_RANK", value, true, elw);
    /* set an mca param for it too */
    opal_setelw("OMPI_MCA_orte_ess_node_rank", value, true, elw);
    free(value);
     ...
     * required */
    orte_util_colwert_process_name_to_string(&value, &child->name);
    opal_setelw("PMIX_ID", value, true, elw);
    free(value);
    ...
    if (orte_get_attribute(&child->attributes, ORTE_PROC_NRESTARTS, (void**)&nrptr, OPAL_INT32)) {
        ...
        opal_setelw("OMPI_MCA_orte_num_restarts", value, true, elw);
        free(value);
    }
    ...
    if (orte_get_attribute(&child->attributes, ORTE_PROC_NOBARRIER, NULL, OPAL_BOOL)
        || 0 < nrestarts) {
        opal_setelw("OMPI_MCA_orte_do_not_barrier", "1", true, elw);
    }
    ...
    /* pass an elwar so the proc can find any files it had prepositioned */
    param = orte_process_info.proc_session_dir;
    opal_setelw("OMPI_FILE_LOCATION", param, true, elw);
    ...
    if (orte_get_attribute(&app->attributes, ORTE_APP_SSNDIR_CWD, NULL, OPAL_BOOL)) {
         ...
         * adjust the $PWD elwiro variable when it changes the directory. This
         * can cause a user to get a different response when doing getcwd vs
         * looking at the elwiro variable. To keep this consistent, we explicitly
         * ensure that the PWD elwiro variable matches the CWD we moved to.
         *
         * NOTE: if a user's program does a chdir(), then $PWD will once
         * again not match getcwd! This is beyond our control - we are only
         * ensuring they start out matching.
         */
        opal_setelw("PWD", param, true, elw);
        /* update the initial wdir value too */
        opal_setelw("OMPI_MCA_initial_wdir", param, true, elw);
