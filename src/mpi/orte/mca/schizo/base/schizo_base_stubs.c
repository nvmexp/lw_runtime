...
#include "orte/mca/schizo/base/base.h"
 ...
 const char* orte_schizo_base_print_elw(orte_schizo_launch_elwiron_t elw)
 {
    switch(elw) {
...
int orte_schizo_base_parse_elw(char *path,
                               ...
                               char ***dstelw)
{
    ...
    OPAL_LIST_FOREACH(mod, &orte_schizo_base.active_modules, orte_schizo_base_active_module_t) {
        if (NULL != mod->module->parse_elw) {
            rc = mod->module->parse_elw(path, cmd_line, srcelw, dstelw);
            if (ORTE_SUCCESS != rc && ORTE_ERR_TAKE_NEXT_OPTION != rc) {
                ORTE_ERROR_LOG(rc);
                return rc;
            }
                                 ...
                                 char ***elw)
{
    ...
    OPAL_LIST_FOREACH(mod, &orte_schizo_base.active_modules, orte_schizo_base_active_module_t) {
        if (NULL != mod->module->setup_child) {
            rc = mod->module->setup_child(jdata, child, app, elw);
            if (ORTE_SUCCESS != rc && ORTE_ERR_TAKE_NEXT_OPTION != rc) {
                ORTE_ERROR_LOG(rc);
                return rc;
            }
...
orte_schizo_launch_elwiron_t orte_schizo_base_check_launch_elwironment(void)
{
    orte_schizo_launch_elwiron_t rc;
    orte_schizo_base_active_module_t *mod;

    OPAL_LIST_FOREACH(mod, &orte_schizo_base.active_modules, orte_schizo_base_active_module_t) {
        if (NULL != mod->module->check_launch_elwironment) {
            rc = mod->module->check_launch_elwironment();
