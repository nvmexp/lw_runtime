 ...
 * tool to extract any personality-specific elwars that need to be
 * forward to the app's environment upon exelwtion */
typedef int (*orte_schizo_base_module_parse_elw_fn_t)(char *path,
                                                      opal_cmd_line_t *cmd_line,
                                                      char **srcelw,
                                                      char ***dstelw);

/* do whatever preparation work
 * is required to setup the app for exelwtion. This is intended to be
 * used by orterun and other launcher tools to, for example, change
 * an exelwtable's relative-path to an absolute-path, or add a command
...
/* add any personality-specific elwars required at the job level prior
 * to beginning to execute local procs */
typedef int (*orte_schizo_base_module_setup_fork_fn_t)(orte_job_t *jdata,
                                                       orte_app_context_t *context);

/* add any personality-specific elwars required for this specific local
 * proc upon exelwtion */
 ...
 * can push any required elwars into the local environment, but must remember
 * to "unset" them during finalize. The module then returns a flag indicating
 * the launch environment of the process */
typedef orte_schizo_launch_elwiron_t (*orte_schizo_base_module_ck_launch_elwiron_fn_t)(void);
...
typedef struct {
    ...
    orte_schizo_base_module_parse_elw_fn_t              parse_elw;
    ...
    orte_schizo_base_module_ck_launch_elwiron_fn_t      check_launch_elwironment;
