...
int orte_sstore_base_colwert_key_to_string(orte_sstore_base_key_t key, char **key_str)
{
    switch(key) {
...
int orte_sstore_base_colwert_string_to_key(char *key_str, orte_sstore_base_key_t *key)
{
...
int orte_sstore_base_extract_global_metadata(orte_sstore_base_global_snapshot_info_t *global_snapshot)
{
    ...
    do {
        ...
        if( 0 == strncmp(token, SSTORE_METADATA_INTERNAL_PROCESS_STR, strlen(SSTORE_METADATA_INTERNAL_PROCESS_STR)) ) {
            orte_util_colwert_string_to_process_name(&proc, value);
