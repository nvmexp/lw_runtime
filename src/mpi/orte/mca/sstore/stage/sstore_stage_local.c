...
static char * sstore_stage_cache_lwrrent_dir = NULL;
...
void orte_sstore_stage_local_app_snapshot_info_construct(orte_sstore_stage_local_app_snapshot_info_t *info)
{
    info->name.jobid = ORTE_JOBID_ILWALID;
    info->name.vpid  = ORTE_VPID_ILWALID;
    ...
    if( !ORTE_PROC_IS_HNP ) {
        ...
        while( 0 < opal_list_get_size(active_handles) && !done ) {
            ...
            if( done ) {
                ...
                if( lwr_time != 0 && lwr_time % 30 == 0 ) {
                    opal_output(0, "---> Waiting for fin(): %3d / %3d\n",
                                lwr_time, max_time);
                }

                opal_progress();
                if( lwr_time >= max_time ) {
                    break;
                } else {
                    sleep(1);
                }
...
static int sstore_stage_update_cache(orte_sstore_stage_local_snapshot_info_t *handle_info)
{
    ...
    if( NULL != sstore_stage_cache_lwrrent_dir ) {
        sstore_stage_cache_last_dir    = strdup(sstore_stage_cache_lwrrent_dir);
    }
     ...
     */
    if( NULL != sstore_stage_cache_lwrrent_dir ) {
        free(sstore_stage_cache_lwrrent_dir);
        sstore_stage_cache_lwrrent_dir = NULL;
    }
    sstore_stage_cache_lwrrent_dir = strdup(cache_dirname);

    OPAL_OUTPUT_VERBOSE((10, mca_sstore_stage_component.super.output_handle,
                         "sstore:stage:(local): update_cache(): Cache Pointers lwr(%s), last(%s)",
                         sstore_stage_cache_lwrrent_dir, sstore_stage_cache_last_dir));
