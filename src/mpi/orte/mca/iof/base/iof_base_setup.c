...
int
orte_iof_base_setup_child(orte_iof_base_io_conf_t *opts, char ***elw)
{
    int ret;
#if OPAL_PMIX_V1
    char *str;
    ...
    if (!orte_map_stddiag_to_stderr && !orte_map_stddiag_to_stdout ) {
        ...
        if (NULL != str) {
            opal_setelw("OPAL_OUTPUT_STDERR_FD", str, true, elw);
            free(str);
        }
    } else if( orte_map_stddiag_to_stdout ) {
        opal_setelw("OPAL_OUTPUT_INTERNAL_TO_STDOUT", "1", true, elw);
    }
#endif
