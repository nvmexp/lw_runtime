...
static void show_aclwmulated_duplicates(int fd, short event, void *context)
{
    opal_list_item_t *item;
                     ...
                     const char *output, orte_process_name_t *sender)
{
    ...
    if (ORTE_SUCCESS == rc) {
        /* Yes.  But do we want to print anything?  That's complicated.
           ...
           show all aclwmulated duplicate notices.
           ...
           - when a duplicate (filename, topic) tuple arrives
             - if now>(T+5) and timer is not set (due to
               ...
               set!)
               - print all aclwmulated duplicates
               - reset T=now
                            ...
                            bool want_error_header, const char *output)
{
    ...
    /* JMS Note that we *may* have a relwrsion situation here where
