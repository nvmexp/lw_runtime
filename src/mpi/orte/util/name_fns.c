...
#define ORTE_SCHEMA_ILWALID_CHAR        '$'
#define ORTE_SCHEMA_ILWALID_STRING      "$"

/* constructor - used to initialize namelist instance */
static void orte_namelist_construct(orte_namelist_t* list)
{
    list->name.jobid = ORTE_JOBID_ILWALID;
    list->name.vpid = ORTE_VPID_ILWALID;
}
...
int orte_util_colwert_jobid_to_string(char **jobid_string, const orte_jobid_t jobid)
{
    int rc;
    char str[256];
...
int orte_util_colwert_string_to_jobid(orte_jobid_t *jobid, const char* jobidstring)
{
    if (NULL == jobidstring) {  /* got an error */
        ORTE_ERROR_LOG(ORTE_ERR_BAD_PARAM);
        *jobid = ORTE_JOBID_ILWALID;
    ...
    if (0 == strcmp(ORTE_SCHEMA_ILWALID_STRING, jobidstring)) {
        *jobid = ORTE_JOBID_ILWALID;
        return ORTE_SUCCESS;
    }
...
int orte_util_colwert_vpid_to_string(char **vpid_string, const orte_vpid_t vpid)
{
    /* check for wildcard value - handle appropriately */
    ...
    if (ORTE_VPID_ILWALID == vpid) {
        *vpid_string = strdup(ORTE_SCHEMA_ILWALID_STRING);
        return ORTE_SUCCESS;
    }
...
int orte_util_colwert_string_to_vpid(orte_vpid_t *vpid, const char* vpidstring)
{
    if (NULL == vpidstring) {  /* got an error */
        ORTE_ERROR_LOG(ORTE_ERR_BAD_PARAM);
        *vpid = ORTE_VPID_ILWALID;
        return ORTE_ERR_BAD_PARAM;
    }
    ...
    /* check for invalid value */
    if (0 == strcmp(ORTE_SCHEMA_ILWALID_STRING, vpidstring)) {
        *vpid = ORTE_VPID_ILWALID;
        return ORTE_SUCCESS;
    }
...
int orte_util_colwert_string_to_process_name(orte_process_name_t *name,
                                             const char* name_string)
{
    ...
    /* set default */
    name->jobid = ORTE_JOBID_ILWALID;
    name->vpid = ORTE_VPID_ILWALID;
    ...
    *token = '\0';
    ...
    } else if (0 == strcmp(temp, ORTE_SCHEMA_ILWALID_STRING)) {
    ...
    if (0 == strcmp(token, ORTE_SCHEMA_WILDCARD_STRING)) {
        vpid = ORTE_VPID_WILDCARD;
    } else if (0 == strcmp(token, ORTE_SCHEMA_ILWALID_STRING)) {
        vpid = ORTE_VPID_ILWALID;
    } else {
...
int orte_util_colwert_process_name_to_string(char **name_string,
                                             const orte_process_name_t* name)
{
    ...
    } else if (ORTE_JOBID_ILWALID == name->jobid) {
        asprintf(&tmp, "%s", ORTE_SCHEMA_ILWALID_STRING);
    } else {
    ...
    } else if (ORTE_VPID_ILWALID == name->vpid) {
        asprintf(&tmp2, "%s%c%s", tmp, ORTE_SCHEMA_DELIMITER_CHAR, ORTE_SCHEMA_ILWALID_STRING);
    } else {
...
/* sysinfo colwersion to and from string */
int orte_util_colwert_string_to_sysinfo(char **cpu_type, char **cpu_model,
					const char* sysinfo_string)
{
    ...
    *token = '\0';
     ...
     */
    if (0 != strcmp(temp, ORTE_SCHEMA_ILWALID_STRING)) {
        *cpu_type = strdup(temp);
    }

    /* If type is a valid string get the value otherwise leave cpu_type untouched.
     */
    if (0 != strcmp(token, ORTE_SCHEMA_ILWALID_STRING)) {
...
int orte_util_colwert_sysinfo_to_string(char **sysinfo_string,
					const char *cpu_type, const char *cpu_model)
{
    ...
    if (NULL == cpu_model) {
        asprintf(sysinfo_string, "%s%c%s", tmp, ORTE_SCHEMA_DELIMITER_CHAR, ORTE_SCHEMA_ILWALID_STRING);
    } else {
