 ...
 * into the app_context's elw array so it will automatically be pushed
...
int orte_pre_condition_transports(orte_job_t *jdata, char **key)
{
    ...
    /* record it in case this job exelwtes a dynamic spawn */
    if (NULL != jdata) {
        orte_set_attribute(&jdata->attributes, ORTE_JOB_TRANSPORT_KEY, ORTE_ATTR_LOCAL, string_key, OPAL_STRING);

        if (OPAL_SUCCESS != mca_base_var_elw_name ("orte_precondition_transports", &cs_elw)) {
            ORTE_ERROR_LOG(ORTE_ERR_OUT_OF_RESOURCE);
            free(string_key);
            return ORTE_ERR_OUT_OF_RESOURCE;
        }
        ...
        for (n=0; n < jdata->apps->size; n++) {
            ...
            opal_setelw(cs_elw, string_key, true, &app->elw);
        }
        free(cs_elw);
        free(string_key);
    } else if (NULL != key) {
