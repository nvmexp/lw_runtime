...
#define ORTE_APP_SET_ELWAR          17    // opal_elwar_t - set the given elwar to the specified value
#define ORTE_APP_UNSET_ELWAR        18    // string - name of elwar to unset, if present
#define ORTE_APP_PREPEND_ELWAR      19    // opal_elwar_t - prepend the specified value to the given elwar
#define ORTE_APP_APPEND_ELWAR       20    // opal_elwar_t - append the specified value to the given elwar
#define ORTE_APP_ADD_ELWAR          21    // opal_elwar_t - add elwar, do not override pre-existing one
...
#define ORTE_NODE_FLAG_LOC_VERIFIED       0x02   // whether or not the location has been verified - used for
                                                 // elwironments where the daemon's final destination is uncertain
...
#define ORTE_NODE_FLAG_SLOTS_GIVEN        0x10   // the number of slots was specified - used only in non-managed elwironments
...
#define ORTE_JOB_SILENT_TERMINATION     (ORTE_JOB_START_KEY + 54)    // bool - do not generate an event notification when job
                                                                     //        normally terminates
#define ORTE_JOB_SET_ELWAR              (ORTE_JOB_START_KEY + 55)    // opal_elwar_t - set the given elwar to the specified value
#define ORTE_JOB_UNSET_ELWAR            (ORTE_JOB_START_KEY + 56)    // string - name of elwar to unset, if present
#define ORTE_JOB_PREPEND_ELWAR          (ORTE_JOB_START_KEY + 57)    // opal_elwar_t - prepend the specified value to the given elwar
#define ORTE_JOB_APPEND_ELWAR           (ORTE_JOB_START_KEY + 58)    // opal_elwar_t - append the specified value to the given elwar
#define ORTE_JOB_ADD_ELWAR              (ORTE_JOB_START_KEY + 59)    // opal_elwar_t - add elwar, do not override pre-existing one
...
#define ORTE_PROC_PRIOR_NODE      (ORTE_PROC_START_KEY +  5)           // void* - pointer to orte_node_t where this proc last exelwted
...
/*
 * Register a handler for colwerting attr keys to strings
 *
 * Handlers will be ilwoked by orte_attr_key_to_str to return the appropriate value.
 */
