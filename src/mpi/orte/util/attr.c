...
} orte_attr_colwerter_t;

/* all default to NULL */
static orte_attr_colwerter_t colwerters[MAX_COLWERTERS];
                       ...
                       orte_attr2str_fn_t colwerter)
{
    ...
    for (i = 0 ; i < MAX_COLWERTERS ; ++i) {
        if (0 == colwerters[i].init) {
            ...
            colwerters[i].key_base = key_base;
            colwerters[i].key_max = key_max;
...
const char *orte_attr_key_to_str(orte_attribute_key_t key)
{
    ...
    if (ORTE_ATTR_KEY_BASE < key &&
        ...
        case ORTE_APP_SET_ELWAR:
            return "ORTE_APP_SET_ELWAR";
        case ORTE_APP_UNSET_ELWAR:
            return "ORTE_APP_UNSET_ELWAR";
        case ORTE_APP_PREPEND_ELWAR:
            return "ORTE_APP_PREPEND_ELWAR";
        case ORTE_APP_APPEND_ELWAR:
            return "ORTE_APP_APPEND_ELWAR";
        case ORTE_APP_ADD_ELWAR:
            return "ORTE_APP_ADD_ELWAR";
        ...
        case ORTE_JOB_SET_ELWAR:
            return "ORTE_JOB_SET_ELWAR";
        case ORTE_JOB_UNSET_ELWAR:
            return "ORTE_JOB_UNSET_ELWAR";
        case ORTE_JOB_PREPEND_ELWAR:
            return "ORTE_JOB_PREPEND_ELWAR";
        case ORTE_JOB_APPEND_ELWAR:
            return "ORTE_JOB_APPEND_ELWAR";
        case ORTE_JOB_ADD_ELWAR:
            return "ORTE_APP_ADD_ELWAR";
    ...
    /* see if one of the colwerters can handle it */
    for (i = 0 ; i < MAX_COLWERTERS ; ++i) {
        if (0 != colwerters[i].init) {
            if (colwerters[i].key_base < key &&
                key < colwerters[i].key_max) {
                return colwerters[i].colwerter(key);
            }
        }
    }
                   ...
                   void *data, opal_data_type_t type)
{
    ...
    case OPAL_ELWAR:
        ...
        elwar = (opal_elwar_t*)data;
        if (NULL != elwar->elwar) {
            kv->data.elwar.elwar = strdup(elwar->elwar);
        }
        if (NULL != elwar->value) {
            kv->data.elwar.value = strdup(elwar->value);
        }
        kv->data.elwar.separator = elwar->separator;
        break;
                     ...
                     void **data, opal_data_type_t type)
{
    ...
    case OPAL_ELWAR:
        elwar = OBJ_NEW(opal_elwar_t);
        if (NULL != kv->data.elwar.elwar) {
            elwar->elwar = strdup(kv->data.elwar.elwar);
        }
        if (NULL != kv->data.elwar.value) {
            elwar->value = strdup(kv->data.elwar.value);
        }
        elwar->separator = kv->data.elwar.separator;
        *data = elwar;
        break;
