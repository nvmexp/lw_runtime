...
int orte_util_check_context_app(orte_app_context_t *context, char **elw)
{
    char *tmp;
    ...
    /* If the app is a naked filename, we need to do a path search for
        ...
        1. The user specified an absolute pathname for the exelwtable.
        We simply need to verify that it exists and we can run it.

        2. The user specified a relative pathname for the exelwtable.
    ...
    if (strlen(tmp) == strlen(context->app)) {
        /* If this is a naked exelwtable -- no relative or absolute
        pathname -- then search the PATH for it */
        free(tmp);
        tmp = opal_path_findv(context->app, X_OK, elw, context->cwd);
        if (NULL == tmp) {
