...
command line using one exelwtable file. Use command-line parameters to specify
...
- Introduced new components IMB-NBC and IMB-RMA that conform to the MPI-3.0 
  ...
  - Updated Microsoft* Visual Studio* solutions to include the IMB-NBC and 
    IMB-RMA targets.
- Consolidated all first-use dolwments in ReadMe_IMB.txt to improve usability.
...
- Fix of memory corruption when the -msglen command-line option is used
  with the Intel(R) MPI Benchmark exelwtables
- Fix in Accumulate benchmark related to using the CHECK conditional
  compilation macro
- Fix for integer overflow in dynamic callwlations on the number of iterations
- Recipes for building IA-32 exelwtables within Microsoft* Visual Studio* 2005
...
For details on running the resulting exelwtable files, see the Intel(R) MPI 
   ...
   c) Check other settings as required, for example:
    ...
    C/C++ > Preprocessor
       - For the "Preprocessor definitions" within the Visual Studio 
         ...
         _CRT_SELWRE_NO_DEPRECATE. Depending on the components you intend to 
...
3) Use F7 or Build > Build Solution to create an exelwtable.

Building "Win32" Exelwtables
----------------------------
...
4) Use F7 or Build > Build Solution to create an exelwtable.
...
Copies of dolwments which have an order number and are referenced in this 
