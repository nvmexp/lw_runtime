...
function NONRPM_INSTALL_PACKAGE() {
    ...
    local lwr_dir=$(pwd) || DIE "Unable to find current directory."
    ...
    if [ "$noscripts" != "1" ]; then
        if [[ -e "$script_dir/PREIN.$rpm_name" ]]; then
            elw RPM_INSTALL_PREFIX="$dst_dir" /bin/bash "$script_dir/PREIN.$rpm_name" || DIE "Pre-install script failed"
        fi
    fi
    ...
    "$RPM_EXTR_TOOL" "$lwr_dir/$rpm_file" | gunzip --quiet | cpio --quiet -idmu \
    ...
    "$RPM_EXTR_TOOL" "$lwr_dir/$rpm_file" | gunzip --quiet | cpio --quiet -t | sed s@"^\.$rpm_prefix"@"$dst_dir"@g | sed 's/^\.//' > "$dst_dir/tmp12345qwexyz/$log_file" \
    ...
    cd "$lwr_dir" \
        || DIE "Unable to change directory to \"$lwr_dir\"."

    if [ "$noscripts" != "1" ]; then
        if [[ -e "$script_dir/POSTIN.$rpm_name" ]]; then
            elw RPM_INSTALL_PREFIX="$dst_dir" /bin/bash "$script_dir/POSTIN.$rpm_name" || DIE "Post-install script failed"
        fi
    fi
...
function NONRPM_UNINSTALL_PACKAGE() {
    ...
    if [ "$noscripts" != "1" ]; then
        ...
        if [[ -f "$script_dir/PREUN.$rpm_name" ]]; then
            elw RPM_INSTALL_PREFIX="$install_dir" /bin/bash "$script_dir/PREUN.$rpm_name" || DIE "Post-uninstall script failed"
        fi
    fi
    ...
    if [ "$noscripts" != "1" ]; then
        if [[ -f "$script_dir/POSTUN.$rpm_name" ]]; then
            elw RPM_INSTALL_PREFIX="$install_dir" /bin/bash "$script_dir/POSTUN.$rpm_name" || DIE "Post-uninstall script failed"
        fi
    fi
