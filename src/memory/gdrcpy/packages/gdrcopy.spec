...
%{!?LWCA: %define LWCA /usr/local/lwca}
%{!?GDR_VERSION: %define GDR_VERSION 2.0}
%{!?KVERSION: %define KVERSION %(uname -r)}
%{!?MODULE_LOCATION: %define MODULE_LOCATION /kernel/drivers/misc/}
%{!?LWIDIA_DRIVER_VERSION: %define LWIDIA_DRIVER_VERSION UNKNOWN}
%{!?LWIDIA_SRC_DIR: %define LWIDIA_SRC_DIR UNDEFINED}
...
%if %{BUILD_KMOD} > 0
%global kmod_fullname kmod-%{kmod_kernel_version}-%{LWIDIA_DRIVER_VERSION}
%endif
...
%global __requires_exclude ^liblwda\\.so.*$
...
License:        MIT
URL:            https://github.com/LWPU/gdrcopy
...
Recommends: kmod-lwpu-latest-dkms
%endif
...
LWPU GPUDirect RDMA technology.
...
Kernel-mode driver for GDRCopy built for GPU driver %{LWIDIA_DRIVER_VERSION} and Linux kernel %{KVERSION}.
%endif
...
%build
echo "building"
make -j8 LWCA=%{LWCA} config lib exes
%if %{BUILD_KMOD} > 0
make -j8 LWIDIA_SRC_DIR=%{LWIDIA_SRC_DIR} driver
%endif
...
make drv_install DESTDIR=$RPM_BUILD_ROOT LWIDIA_SRC_DIR=%{LWIDIA_SRC_DIR}
%endif
...
cp -a $RPM_BUILD_DIR/%{name}-%{version}/src/gdrdrv/lw-p2p-dummy.c $RPM_BUILD_ROOT%{usr_src_dir}/gdrdrv-%{version}/
...
# Prevent race with kmod-lwpu-latest-dkms triggerin
...
%triggerin %{dkms} -- kmod-lwpu-latest-dkms
if [ "$1" == "2" ] && [ -e "%{old_driver_install_dir}/gdrdrv.ko" ]; then
    echo "kmod-lwpu-latest-dkms is detected but defer installation because of the old gdrcopy-kmod package."
    exit 0;
fi
...
%triggerun %{dkms} -- kmod-lwpu-latest-dkms
# This dkms package has only weak dependency with kmod-lwpu-latest-dkms, which is not enforced by RPM.
# Uninstalling kmod-lwpu-latest-dkms would not result in uninstalling this package.
# However, gdrdrv may prevent the removal of lwpu.ko.
# Hence, we rmmod gdrdrv before starting kmod-lwpu-latest-dkms uninstallation.
service gdrcopy stop||:
%{MODPROBE} -rq gdrdrv||:
...
%{usr_src_dir}/gdrdrv-%{version}/lw-p2p-dummy.c
...
%changelog
* Fri Jul 23 2021 Pak Markthub <pmarkthub@lwpu.com> %{GDR_VERSION}-%{_release}
...
* Mon Feb 01 2021 Pak Markthub <pmarkthub@lwpu.com> 2.2-1
...
* Mon Jan 18 2021 Pak Markthub <pmarkthub@lwpu.com> 2.1-2
- Add DKMS support in gdrcopy-kmod.rpm
* Fri Jul 31 2020 Davide Rossetti <drossetti@lwpu.com> 2.1-1
- fix build problem on RHL8 kernels
- relax checks in gdrdrv to support multi-threading use cases
- fix fd leak in gdr_open()
* Mon Mar 02 2020 Davide Rossetti <drossetti@lwpu.com> 2.0-4
- Introduce copylat test application.
- Introduce basic_with_tokens and ilwalidation_fork_child_gdr_pin_parent_with_tokens sub-tests in sanity.
- Remove the dependency with liblwdart.so.
- Clean up the code in the tests folder.
- Change the package maintainer to Davide Rossetti.
* Mon Sep 16 2019 Pak Markthub <pmarkthub@lwpu.com> 2.0-3
