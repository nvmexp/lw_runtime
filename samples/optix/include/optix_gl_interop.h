...
extern "C" {
  ...
  *
  * - @ref RT_TARGET_GL_TEXTURE_LWBE_MAP
  *
  * - @ref RT_TARGET_GL_TEXTURE_LWBE_MAP_ARRAY
  *
  ...
  *
  * Lwrrently OptiX supports only a limited number of internal OpenGL
  ...
  * as well as multisample buffers are also lwrrently not supported.
  ...
  * If a buffer object is not registered @ref RT_ERROR_ILWALID_VALUE will be returned.
  ...
  * As long as a buffer object is unregistered, OptiX will not be able to access the data and calls will fail with @ref RT_ERROR_ILWALID_VALUE.
  ...
  * A texture sampler must be registered in order to be used by OptiX. Otherwise, @ref RT_ERROR_ILWALID_VALUE is returned.
  ...
  * As long as a texture is unregistered, OptiX will not be able to access the pixel data and calls will fail with @ref RT_ERROR_ILWALID_VALUE.
  ...
  * \a gpu is returned from \a WGL_LW_gpu_affinity, an OpenGL extension.  This enables OptiX to create a context
  ...
  * @param[in]    gpu             A handle to a GPU as returned from the \a WGL_LW_gpu_affinity OpenGL extension
  *
  * <B>Return values</B>
  *
  * Relevant return values:
  ...
  * \a WGL_LW_gpu_affinity
  *
  */
  RTresult RTAPI rtDeviceGetWGLDevice(int* device, HGPULW gpu);
#endif
