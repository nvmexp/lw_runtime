...
#ifndef OPTIX_OPTIONAL_FEATURE_OPTIX6
...
#endif
#else // OPTIX_OPTIONAL_FEATURE_OPTIX6
...
#if defined(__x86_64) || defined(AMD64) || defined(_M_AMD64) || defined(__powerpc64__) || defined(__EDG_IA64_ABI)/*=LWRTC*/ || defined(__aarch64__)
 ...
 ***********************************/
  ...
  *
  *   - @ref RT_GLOBAL_ATTRIBUTE_DEMAND_LOAD_NUM_VIRTUAL_PAGES  sizeof(int)
  *   - @ref RT_GLOBAL_ATTRIBUTE_DEVELOPER_OPTIONS              string
  *
  * @ref RT_GLOBAL_ATTRIBUTE_DEMAND_LOAD_NUM_VIRTUAL_PAGES sets the number of virtual pages
  * used for demand load buffers and textures.  The supplied value must be positive.
  *
  ...
  * Relevant return values:
  * - @ref RT_SUCCESS
  * - @ref RT_ERROR_ILWALID_GLOBAL_ATTRIBUTE - Can be returned if an unknown attribute was addressed.
  * - @ref RT_ERROR_ILWALID_VALUE - Can be returned if \a size does not match the proper size of the attribute, or if \a p
  * is \a NULL
  *
  * <B>History</B>
  *
  ...
  *
  * @ref RT_GLOBAL_ATTRIBUTE_DEMAND_LOAD_NUM_VIRTUAL_PAGES returns the number of virtual pages used by demand load buffers
  * and textures.
  *
  ...
  * - @ref RT_SUCCESS
  * - @ref RT_ERROR_ILWALID_GLOBAL_ATTRIBUTE - Can be returned if an unknown attribute was addressed.
  * - @ref RT_ERROR_ILWALID_VALUE - Can be returned if \a size does not match the proper size of the attribute, if \a p is
  ...
  *   - @ref RT_DEVICE_ATTRIBUTE_EXELWTION_TIMEOUT_ENABLED    sizeof(int)
  ...
  *   - @ref RT_DEVICE_ATTRIBUTE_LWDA_DEVICE_ORDINAL          sizeof(int)
  ...
  * Relevant return values:
  * - @ref RT_SUCCESS
  * - @ref RT_ERROR_ILWALID_VALUE - Can be returned if size does not match the proper size of the attribute, if \a p is
  ...
  * @ref RT_DEVICE_ATTRIBUTE_LWDA_DEVICE_ORDINAL was introduced in OptiX 3.0.
 ...
 ***********************************/
  ...
  * this call has no effect and returns @ref RT_ERROR_ILWALID_VALUE.
  *
  * @param[in]   v          Specifies the program variable to be modified
  ...
  * \a *object to \a NULL and returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * returns @ref RT_ERROR_ILWALID_VALUE.
  *
  * @param[in]   v          Specifies the program variable to be queried
  ...
  * call sets \a *nameReturn to \a NULL and returns @ref RT_ERROR_ILWALID_VALUE.  \a
  ...
  * \a *annotationReturn to \a NULL and returns @ref RT_ERROR_ILWALID_VALUE.  \a
  ...
  * Returns @ref RT_ERROR_ILWALID_VALUE if given a \a NULL pointer.
  *
  * @param[in]   v             Specifies the program variable to be queried
  ...
  * not a valid variable, \a *context is set to \a NULL and @ref RT_ERROR_ILWALID_VALUE is
  * returned.
  *
  * @param[in]   v          Specifies the program variable to be queried
  ...
  * will return @ref RT_ERROR_ILWALID_VALUE.
  *
  * @param[in]   v          Specifies the program variable to be queried
 ...
 ***********************************/
  ...
  * Returns @ref RT_ERROR_ILWALID_VALUE if passed a \a NULL pointer.
  *
  ...
  * this object.  Returns @ref RT_ERROR_ILWALID_VALUE if passed a \a NULL context.  @ref
  ...
  * @param[in]   code            The error code to be colwerted to string
  ...
  *   - @ref RT_CONTEXT_ATTRIBUTE_MAX_CONLWRRENT_LAUNCHES     sizeof(int)
  *   - @ref RT_CONTEXT_ATTRIBUTE_PREFER_WATERTIGHT_TRAVERSAL sizeof(int)
  *   - @ref RT_CONTEXT_ATTRIBUTE_DEMAND_LOAD_MULTITHREADED_CALLBACKS_ENABLED sizeof(int)
  *
  ...
  * automatic inlining.
  *
  * @ref RT_CONTEXT_ATTRIBUTE_DISK_CACHE_ENABLED sets whether or not the OptiX disk cache
  * is enabled.
  *
  ...
  *
  * The high watermark can be overridden with the environment variable \a OPTIX_CACHE_MAXSIZE.  This
  * environment variable takes precedence over the RTcontext attribute.  The low watermark will be
  * set to half the value of \a OPTIX_CACHE_MAXSIZE.  Setting \a OPTIX_CACHE_MAXSIZE to 0 will
  * disable the disk cache, but will not alter the contents of the cache.  Negative and non-integer
  * values will be ignored.  When the cache is overridden via the environment variable, setting the
  * limits through the RTcontext attribute will have no effect, and querying the limits will return
  * 0 for the high and low limits.
  *
  * Note that the disk cache can be disabled via the environment variable, but it cannot be enabled
  * if it is disabled via the API.
  *
  * @ref RT_CONTEXT_ATTRIBUTE_MAX_CONLWRRENT_LAUNCHES sets the maximum number of allowed
  * conlwrrent asynchronous launches per device. The actual number of launches can be less than
  * the set limit, and actual GPU scheduling may affect conlwrrency. This limit affects only
  * asynchronous launches. Valid values are from 1 to the maximum number of LWCA streams
  ...
  *
  * @ref RT_CONTEXT_ATTRIBUTE_DEMAND_LOAD_MULTITHREADED_CALLBACKS_ENABLED enables multithreaded
  * invocation of demand load callbacks.  If this is set to 0, then all the callbacks
  * will be ilwoked from the thread that called @ref rtContextLaunch.  If this is set to non-zero,
  * then the callbacks will be ilwoked from at most the number of CPU threads according to the
  * RT_CONTEXT_ATTRIBUTE_CPU_NUM_THREADS attribute, none of which will be the thread that
  * called @ref rtContextLaunch.  The default value is zero.
  *
  ...
  * disk cache.
  *
  * If the disk cache is disabled by setting the environment variable OPTIX_CACHE_MAXSIZE=0,
  * querying the location will return an empty string.
  *
  * @ref RT_CONTEXT_ATTRIBUTE_DISK_CACHE_MEMORY_LIMITS queries the low and high watermark values
  * for the OptiX disk cache.
  *
  * If the disk cache is disabled by setting the environment variable OPTIX_CACHE_MAXSIZE=0,
  * querying the limits will return 0 for the low and high limits.
  *
  * @ref RT_CONTEXT_ATTRIBUTE_MAX_CONLWRRENT_LAUNCHES queries the number of conlwrrent asynchronous
  ...
  * exelwtion of the subsequent trace kernels. Note that the device numbers are
  * OptiX device ordinals, which may not be the same as LWCA device ordinals.
  * Use @ref rtDeviceGetAttribute with @ref RT_DEVICE_ATTRIBUTE_LWDA_DEVICE_ORDINAL to query the LWCA device
  ...
  * Note that the device numbers are  OptiX device ordinals, which may not be the same as LWCA device ordinals.
  * Use @ref rtDeviceGetAttribute with @ref RT_DEVICE_ATTRIBUTE_LWDA_DEVICE_ORDINAL to query the LWCA device
  * corresponding to a particular OptiX device.
  *
  * @param[in]   context   The context to which the hardware list is applied
  ...
  */
  ...
  /**
  * @brief Query the number of devices lwrrently being used
  *
  * @ingroup Context
  *
  * <B>Description</B>
  *
  * @ref rtContextGetDeviceCount - Query the number of devices lwrrently being used.
  *
  ...
  * \a bytes bytes. Not supported with the RTX exelwtion strategy.
  * With RTX exelwtion strategy @ref rtContextSetMaxTraceDepth and @ref rtContextSetMaxCallableProgramDepth
  * should be used to control stack size.
  * Returns @ref RT_ERROR_ILWALID_VALUE if context is not valid.
  *
  ...
  * \a bytes.  Returns @ref RT_ERROR_ILWALID_VALUE if passed a \a NULL pointer.
  *
  ...
  * Only supported for RTX exelwtion mode. Default value is 5.
  * Returns @ref RT_ERROR_ILWALID_VALUE if context is not valid.
  *
  * @param[in]   context            The context node to be modified
  ...
  * \a maxDepth. Only supported for RTX exelwtion mode. Default value is 5. Maximum trace depth is 31.
  * Returns @ref RT_ERROR_ILWALID_VALUE if context is not valid.
  *
  * @param[in]   context            The context node to be modified
  * @param[in]   maxDepth           The desired maximum depth
  ...
  * \a maxDepth.  Returns @ref RT_ERROR_ILWALID_VALUE if passed a \a NULL pointer.
  *
  * @param[in]   context            The context node to be queried
  ...
  *
  * The provided callback will be ilwoked with the message's verbosity level as
  ...
  * [0, 3] will result in @ref RT_ERROR_ILWALID_VALUE return code.
  *
  ...
  * with this context in \a count.  Returns @ref RT_ERROR_ILWALID_VALUE if
  ...
  * entry point \a entryPointIndex. @ref RT_ERROR_ILWALID_VALUE is returned if \a
  ...
  * rtContextSetRayGenerationProgram.  Returns @ref RT_ERROR_ILWALID_VALUE if given an
  ...
  * \a entryPointIndex. @ref RT_ERROR_ILWALID_VALUE is returned if \a entryPointIndex
  ...
  * rtContextSetExceptionProgram.  Returns @ref RT_ERROR_ILWALID_VALUE if given an invalid
  ...
  * exception program is ilwoked if the condition is met. The exception program can query
  ...
  * within valid bounds. This exception is only supported with the RTX exelwtion strategy.
  *
  ...
  * exception is only supported with the RTX exelwtion strategy.
  *
  * @ref RT_EXCEPTION_TEXTURE_ID_ILWALID verifies that every access of a texture id is
  * valid, including use of RT_TEXTURE_ID_NULL and IDs out of bounds.
  *
  * @ref RT_EXCEPTION_BUFFER_ID_ILWALID verifies that every access of a buffer id is
  ...
  * exelwtion strategy only.
  *
  * @ref RT_EXCEPTION_ILWALID_RAY checks the each ray's origin and direction values
  ...
  * this context in \a rayTypeCount.  Returns @ref RT_ERROR_ILWALID_VALUE if passed a \a
  * NULL pointer.
  *
  * @param[in]   context         The context node to be queried
  ...
  * \a rayTypeIndex. @ref RT_ERROR_ILWALID_VALUE is returned if \a rayTypeIndex
  ...
  * Returns @ref RT_ERROR_ILWALID_VALUE if given an invalid ray type index or a \a NULL pointer.
  *
  ...
  * texture handle, \a *sampler is \a NULL or if \a context is invalid, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  */
  ...
  /**
  * @brief Exelwtes the computation kernel for a given context
  *
  ...
  * ilwoked once for every element (pixel or voxel) of the computation grid specified by
  ...
  */
  /**
  * @ingroup rtContextLaunch
  * @param[in]   context                                    The context to be exelwted
  ...
  */
  ...
  /**
  * @brief Query whether the given context is lwrrently
  * running
  *
  * @ingroup Context
  *
  * <B>Description</B>
  *
  * This function is lwrrently unimplemented and it is provided as a placeholder for a future implementation.
  *
  * @param[in]   context   The context node to be queried
  ...
  */
  ...
  /**
  * @brief Exelwtes a Progressive Launch for a given context
  *
  * @ingroup Context
  *
  * <B>Description</B>
  ...
  * match the initial launch, the call has no effect. Otherwise, the aclwmulated result will be
  ...
  * already been generated and are lwrrently in flight may still arrive at the client). The only
  ...
  *
  * @param[in]   context                The context in which the launch is to be exelwted
  ...
  * <B>Description</B>
  *
  * If a progressive launch is lwrrently in progress, calling @ref rtContextStopProgressive
  ...
  * by the server and are lwrrently in flight.
  *
  ...
  * through @ref rtPrintf is lwrrently enabled for this context.
  *
  ...
  * @ref rtPrintf is lwrrently enabled for this context; \a 0 otherwise.  Returns @ref
  * RT_ERROR_ILWALID_VALUE if passed a \a NULL pointer.
  *
  * @param[in]   context   The context to be queried
  ...
  * Returns @ref RT_ERROR_ILWALID_VALUE if it is called after the first invocation of rtContextLaunch.
  *
  *
  ...
  * data generated by @ref rtPrintf. Returns @ref RT_ERROR_ILWALID_VALUE if passed a \a
  * NULL pointer.
  *
  ...
  * @param[in]   x         The launch index in the x dimension to which to limit the output of @ref rtPrintf ilwocations.
  * If set to \a -1, output is generated for all launch indices in the x dimension
  * @param[in]   y         The launch index in the y dimension to which to limit the output of @ref rtPrintf ilwocations.
  * If set to \a -1, output is generated for all launch indices in the y dimension
  * @param[in]   z         The launch index in the z dimension to which to limit the output of @ref rtPrintf ilwocations.
  ...
  * @param[out]  x         Returns the launch index in the x dimension to which the output of @ref rtPrintf ilwocations
  * is limited. Will not be written to if a \a NULL pointer is passed
  * @param[out]  y         Returns the launch index in the y dimension to which the output of @ref rtPrintf ilwocations
  * is limited. Will not be written to if a \a NULL pointer is passed
  * @param[out]  z         Returns the launch index in the z dimension to which the output of @ref rtPrintf ilwocations
  * is limited. Will not be written to if a \a NULL pointer is passed
  *
  * <B>Return values</B>
  *
  ...
  * RT_ERROR_ILWALID_VALUE if passed a \a NULL pointer.  Return @ref
  ...
  * @ref RT_ERROR_ILWALID_VALUE will be returned if \a name or \a v is \a NULL.
  *
  ...
  * context. Returns @ref RT_ERROR_ILWALID_VALUE if passed an invalid variable.
  *
  ...
  * <B>Description</B>
  *
  * @ref rtContextGetVariableCount returns the number of variables that are lwrrently
  * attached to \a context.  Returns @ref RT_ERROR_ILWALID_VALUE if passed a \a NULL pointer.
  *
 ...
 ***********************************/
  ...
  * create \a program. Returns @ref RT_ERROR_ILWALID_VALUE if \a context is \a NULL.
  *
  ...
  * program is not a valid \a RTprogram, returns @ref RT_ERROR_ILWALID_VALUE.
  ...
  * invalid program ids, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * program handle, \a *program is set to \a NULL. Returns @ref RT_ERROR_ILWALID_VALUE
 ...
 ***********************************/
  ...
  * within \a context. Returns @ref RT_ERROR_ILWALID_VALUE if \a group is \a NULL.
  *
  ...
  * any of the objects attached to \a group are not valid, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  */
   ...
   * Note that the @pre mask is lwrrently limited to 8 bits.
   *
   * @param[in] group   The group handle
  ...
  * by @ref rtGroupSetChild.  Returns @ref RT_ERROR_ILWALID_VALUE if given a \a NULL pointer.
  *
  * @param[in]   group   The parent group handle
  ...
  * Returns @ref RT_ERROR_ILWALID_VALUE if given an invalid child index or \a NULL pointer.
  *
  * @param[in]   group   The parent group handle
  ...
  * @ref RT_OBJECTTYPE_UNKNOWN and @ref RT_ERROR_ILWALID_VALUE is returned.
  * Returns @ref RT_ERROR_ILWALID_VALUE if given a \a NULL pointer.
  *
  * @param[in]   group   The parent group handle
  * @param[in]   index   The index of the child slot to query
 ...
 ***********************************/
  ...
  * created selector within \a context. Returns @ref RT_ERROR_ILWALID_VALUE if \a selector is \a NULL.
  *
  ...
  * <B>Description</B>
  *
  * @ref rtSelectorValidate relwrsively checks consistency of the Selector
  ...
  * exelwted when the Selector node \a selector gets visited by a ray
  ...
  */
  ...
  /**
  * @brief Returns the lwrrently assigned visit program
  *
  * @ingroup SelectorNode
  *
  * <B>Description</B>
  ...
  * visit program lwrently bound to \a selector.
  *
  * @param[in]   selector   Selector node handle
  ...
  * lwrrently attached to \a selector at slot \a index. The index value
  ...
  * variables that are lwrrently attached to the Selector node
  * \a selector.
  *
  * @param[in]   selector   Selector node handle
 ...
 ***********************************/
  ...
  * newly created transform within \a context. Returns @ref RT_ERROR_ILWALID_VALUE if \a transform
  * is \a NULL.
  *
  ...
  * <B>Description</B>
  *
  * @ref rtTransformValidate relwrsively checks consistency of the
  ...
  * \a matrix and \a ilwerseMatrix must be non-\a NULL. If exactly one
  ...
  *
  * @param[in]   transform        Transform node handle
  * @param[in]   transpose        Flag indicating whether \a matrix and \a ilwerseMatrix should be
  * transposed
  * @param[in]   matrix           Affine matrix (4x4 float array)
  * @param[in]   ilwerseMatrix    Ilwerted form of \a matrix
  *
  * <B>Return values</B>
  *
  * Relevant return values:
  ...
  *
  */
  RTresult RTAPI rtTransformSetMatrix(RTtransform transform, int transpose, const float* matrix, const float* ilwerseMatrix);

  /**
  * @brief Returns the affine matrix and its ilwerse associated with a Transform node
  *
  * @ingroup TransformNode
  *
  * <B>Description</B>
  ...
  * is lwrrently used to perform a transformation of the geometry
  * contained in the sub-tree with \a transform as root. The corresponding
  * ilwerse matrix will be returned in \a ilwerseMatrix. One or both
  ...
  * @param[out]  ilwerseMatrix    Ilwerted form of \a matrix
  *
  * <B>Return values</B>
  *
  * Relevant return values:
  ...
  *
  */
  RTresult RTAPI rtTransformGetMatrix(RTtransform transform, int transpose, float* matrix, float* ilwerseMatrix);
  ...
  *   With [pvx, pvy, pvz]^T as pivot point for the rotation.
  *
  *   S = [ sx  a  b  -pvx ]
  *       [  * sy  c  -pvy ]
  *       [  *  * sz  -pvz ]
  ...
  * node lwrrently attached to \a transform. The returned pointer is of
 ...
 ***********************************/
  ...
  * Returns @ref RT_ERROR_ILWALID_VALUE if \a geometrygroup is \a NULL.
  *
  ...
  * any of the objects attached to \a geometrygroup are not valid, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * to \a NULL.  Returns @ref RT_ERROR_ILWALID_VALUE if given an invalid child index or \a NULL
  * pointer.
  *
  * @param[in]   geometrygroup      The parent geometry group handle
  * @param[in]   index              The index of the child slot to query
 ...
 ***********************************/
  ...
  * structure within \a context.  Returns @ref RT_ERROR_ILWALID_VALUE if \a acceleration is \a NULL.
  *
  ...
  * <B>Description</B>
  *
  * @ref rtAcceleratiolwalidate checks \a acceleration for completeness. If \a acceleration is
  * not valid, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * <B>History</B>
  *
  * @ref rtAcceleratiolwalidate was introduced in OptiX 1.0.
  *
  * <B>See also</B>
  * @ref rtAccelerationCreate
  *
  */
  RTresult RTAPI rtAcceleratiolwalidate(RTacceleration acceleration);
  ...
  * computed for the acceleration structure is ilwalidated and the acceleration will be marked
  * dirty.
  *
  ...
  * <B>Description</B>
  *
  * @ref rtAccelerationGetBuilder returns the name of the builder lwrrently
  ...
  * <B>Description</B>
  *
  * @ref rtAccelerationIsDirty returns whether the acceleration structure is lwrrently marked dirty.
 ...
 ***********************************/
  ...
  * Returns @ref RT_ERROR_ILWALID_VALUE if \a geometryinstance is \a NULL.
  *
  ...
  * any of the objects attached to \a geometry are not valid, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * If no GeometryTriangles node is attached or a Geometry node is attached, @ref RT_ERROR_ILWALID_VALUE is returned, else @ref RT_SUCCESS.
  *
  ...
  * \a index in the material list of \a geometryinstance. Returns @ref RT_ERROR_ILWALID_VALUE if \a
  * index is invalid.
  *
  ...
  * should be a \a NULL-terminated string. If there is lwrrently no variable associated with \a
 ...
 ***********************************/
  ...
  * Returns @ref RT_ERROR_ILWALID_VALUE if \a geometry is \a NULL.
  *
  ...
  * objects attached to \a geometry are not valid, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * string. If there is lwrrently no variable associated with \a geometry named \a name, a new
 ...
 ***********************************/
  ...
  * Returns @ref RT_ERROR_ILWALID_VALUE if \a geometrytriangles is \a NULL.
  *
  ...
  * objects attached to \a geometrytriangles are not valid, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * @ref rtGeometryTrianglesSetMotiolwertices
  *
  */
  ...
  * <B>Description</B>
  *
  * @ref rtGeometryTrianglesSetMotiolwertices interprets the buffer \a vertexBuffer as the vertices of triangles of the GeometryTriangles \a geometrytriangles.
  ...
  * <B>History</B>
  *
  * @ref rtGeometryTrianglesSetMotiolwertices was introduced in OptiX 6.0.
  *
  * <B>See also</B>
  * @ref rtGeometryTrianglesSetVertices
  * @ref rtGeometryTrianglesSetMotiolwerticesMultiBuffer
  *
  */
  RTresult RTAPI rtGeometryTrianglesSetMotiolwertices( RTgeometrytriangles geometrytriangles,
                                                       unsigned int        vertexCount,
                                                       RTbuffer            vertexBuffer,
                                                       RTsize              vertexBufferByteOffset,
                                                       RTsize              vertexByteStride,
  ...
  * <B>Description</B>
  *
  * @ref rtGeometryTrianglesSetMotiolwerticesMultiBuffer can be used instead of @ref rtGeometryTrianglesSetMotiolwertices if the vertices for the different motion steps are stored in separate buffers.
  ...
  *
  */
  RTresult RTAPI rtGeometryTrianglesSetMotiolwerticesMultiBuffer( RTgeometrytriangles geometrytriangles,
                                                                  unsigned int        vertexCount,
  ...
  * where timeBegin <= timeEnd.  The default time range is [0.0, 1.0].  The
  * time range has no effect unless @ref rtGeometryTrianglesSetMotiolwertices or
  * @ref rtGeometryTrianglesSetMotiolwerticesMultiBuffer with motionStepCount > 1 is
  ...
  * Splitting is done to increase performance, but as a side-effect may result in multiple exelwtions of
  * the any-hit program for a single intersection.
  * To avoid further side effects (e.g., multiple aclwmulations of a value) that may result of a multiple exelwtion,
  * RT_GEOMETRY_FLAG_NO_SPLITTING needs to be set.
  * RT_GEOMETRY_FLAG_DISABLE_ANYHIT is an optimization due to which the exelwtion of the any-hit program is skipped.
  ...
  * Therefore, this flag still needs to be set to skip the exelwtion of any any-hit program.
 ...
 ***********************************/
  ...
  * created material within \a context. Returns @ref RT_ERROR_ILWALID_VALUE if \a material is \a NULL.
  *
  ...
  * any of the objects attached to \a material are not valid, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * has no effect and returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * Otherwise, the call has no effect and returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * string. If there is lwrrently no variable associated with \a material named \a name, and \a v is
  ...
  * and returns either @ref RT_ERROR_ILWALID_VALUE if either \a name or \a v is \a NULL or @ref
  ...
  * \a NULL. If \a material is not a valid material, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  * @param[in]   material   Specifies the material to query
  ...
  * this call has no effect and returns @ref RT_ERROR_ILWALID_VALUE. If \a v is not
  * a valid variable or does not belong to \a material, this call has no effect and
  * returns @ref RT_ERROR_ILWALID_VALUE or @ref RT_ERROR_VARIABLE_NOT_FOUND, respectively.
  *
  * @param[in]   material   Specifies the material to modify
  ...
  * call has no effect and returns @ref RT_ERROR_ILWALID_VALUE.
  *
  * @param[in]   material   Specifies the material to query
  ...
  * set to \a NULL and either @ref RT_ERROR_ILWALID_VALUE or @ref RT_ERROR_VARIABLE_NOT_FOUND is
 ...
 ***********************************/
  ...
  * Returns @ref RT_ERROR_ILWALID_VALUE if \a texturesampler is \a NULL.
  *
  ...
  *
  * In the context of a demand-loaded texture callback, the memory block describes a subimage
  * within the specified miplevel.  The subimage will not extend beyond the image borders.
  *
  ...
  * Any API object that referenced \a texturesampler will have its reference ilwalidated.
  *
  ...
  * MIP level and array slice configuration then returns @ref RT_ERROR_ILWALID_CONTEXT.
  *
  ...
  * \a texturesampler.  If \a context is \a NULL, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * mode.  These values mirror the LWCA behavior of textures.
  * See LWCA programming guide for details.
  *
  ...
  * <B>Description</B>
  *
  * @ref rtTextureSamplerSetMipLevelBias sets the offset to be applied to the callwlated mipmap level.
  *
  * @param[in]   texturesampler   The texture sampler object to be changed
  ...
  * and apply sRGB to linear colwersion during texture read for 8-bit integer buffer formats.
  ...
  * a read mode of @ref RT_TEXTURE_READ_NORMALIZED_FLOAT will colwert integral values from the
  ...
  * <B>Description</B>
  *
  * @ref rtTextureSamplerSetBuffer attaches \a buffer to \a texturesampler.  If the associated
  * buffer was created by @ref rtBufferCreateFromCallback, the texture is a demand-loaded texture
  * and the associated buffer callback is used to fill the pixels for the texture based on usage.
  * A demand-loaded buffer can only be associated with a single texture sampler.
  *
  ...
  * the host side. If \a textureId is \a NULL, returns @ref RT_ERROR_ILWALID_VALUE.
  *
 ...
 ***********************************/
  ...
  * If RT_BUFFER_LWBEMAP flag is set, buffer depth specifies the number of lwbe faces, not the depth of a 3D buffer.
  ...
  *
  * Returns @ref RT_ERROR_ILWALID_VALUE if \a buffer is \a NULL.
  *
  ...
  * <B>Description</B>
  *
  * @ref RTbuffercallback is implemented by the application.  It is ilwoked by OptiX for each
  ...
  *
  * The @ref RT_CONTEXT_ATTRIBUTE_DEMAND_LOAD_MULTITHREADED_CALLBACKS_ENABLED attribute can be used
  * to have buffer callbacks ilwoked from multiple threads conlwrrently.  Use @ref rtContextSetAttribute
  * with the RT_CONTEXT_ATTRIBUTE_CPU_NUM_THREADS attribute to set the number of CPU threads that will
  * be used.
  *
  * <b>CAUTION</b>: A user provided callback function should not allow exceptions to escape from
  ...
  *
  * Returns @ref RT_ERROR_ILWALID_VALUE if either \a callback or \a buffer is \a NULL.
  *
  * @param[in]   context      The context to create the buffer in.
  * @param[in]   bufferdesc   Bitwise \a or combination of the \a type and \a flags of the new buffer.
  ...
  * Any API object that referenced \a buffer will have its reference ilwalidated.
  *
  ...
  * set, this call will return @ref RT_ERROR_ILWALID_CONTEXT.
  *
  ...
  * If \a *context is \a NULL, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * the buffer does not have format @ref RT_FORMAT_USER or @ref RT_ERROR_ILWALID_VALUE if the
  ...
  * A lwbemap mipmapped buffer is allocated if the @ref RT_BUFFER_LWBEMAP flag was set at buffer creating. \a width must be equal to \a height and the number of lwbe faces is determined by the \a depth,
  * it must be six or a multiple of six, if the @ref RT_BUFFER_LAYERED flag was also set.
  * Layered, mipmapped and lwbemap buffers are supported only as texture buffers.
  *
  * Fails with @ref RT_ERROR_ALREADY_MAPPED if called on a buffer that is mapped.
  *
  * @param[in]   buffer   The buffer to be resized
  ...
  * call will return @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * call will return @ref RT_ERROR_ILWALID_VALUE. @ref rtBufferUnmap is equivalent to @ref rtBufferUnmapEx with \a level = 0.
  *
  * Note that this call does not stop a progressive render if called with a stream buffer.
  *
  * @param[in]   buffer   The buffer to unmap
  ...
  * RT_ERROR_ILWALID_VALUE.  @ref RT_BUFFER_ID_NULL can be used as a sentinel for a
  ...
  * \a *buffer is \a NULL or if \a context is invalid, returns @ref RT_ERROR_ILWALID_VALUE.
  *
  ...
  * @param[out]  subframeCount      The number of subframes aclwmulated in the latest result
  ...
  * <B>Description</B>
  *
  * Sets a buffer attribute. Lwrrently, all available attributes refer to stream buffers only,
  ...
  * 32-bit float, the default is 1.0. Tonemapping is exelwted before encoding the
  * aclwmulated output into the stream, i.e. on the server side if remote rendering is used.
 ...
 ***********************************/
  ...
  * After a post-processing stage was destroyed all command lists containing that stage are ilwalidated and
  ...
  * Returns @ref RT_ERROR_ILWALID_VALUE if \a list is \a NULL.
  *
  ...
  * rtCommandListAppendLaunch2D, respectively. Commands will be exelwted in the order they have been
  ...
  * list multiple times.  Also note that destroying a post-processing stage will ilwalidate all command
  ...
  * must be a subset of the lwrrently active devices, see @ref rtContextSetDevices. If not set then all the
  ...
  */
  ...
  /**
  * @brief Query the number of devices lwrrently being used by the command list.
  *
  * @ingroup Context
  *
  * <B>Description</B>
  *
  * @ref rtCommandListGetDeviceCount queries the number of devices lwrrently being used.
  *
  ...
  */
  ...
  /**
  * @brief Finalize the command list. This must be done before exelwting the command list.
  *
  * @ingroup CommandList
  *
  * <B>Description</B>
  ...
  * prepare the command list for exelwtion. Specifically it will do all work which can be shared
  * between subsequent calls to @ref rtCommandListExelwte.
  * It is invalid to call @ref rtCommandListExelwte before calling @ref rtCommandListFinalize. It is
  ...
  * <B>Description</B>
  *
  * @ref rtCommandListExelwte exelwtes the command list. All added commands will be exelwted in the
  * order in which they were added. Commands can access the results of earlier exelwted commands.
  * This must be called after calling @ref rtCommandListFinalize, otherwise an error will be returned
  * and the command list is not exelwted.
  * @ref rtCommandListExelwte can be called multiple times, but only one call may be active at the
  ...
  * <B>History</B>
  *
  * @ref rtCommandListExelwte was introduced in OptiX 5.0.
  *
  * <B>See also</B>
  * @ref rtCommandListCreate,
  * @ref rtCommandListDestroy,
  ...
  *
  */
  RTresult RTAPI rtCommandListExelwte(RTcommandlist list);
  ...
  * not a valid command list, \a *context is set to \a NULL and @ref RT_ERROR_ILWALID_VALUE is
  * returned.
  *
...
#endif // OPTIX_OPTIONAL_FEATURE_OPTIX6
