...
// Colwerts the SRT transformation srt into a 3x4 matrix with rows m0, m1, and m2
static __forceinline__ __device__ void optixGetMatrixFromSrt( float4& m0, float4& m1, float4& m2, const OptixSRTData& srt )
{
    ...
    const float4 nq      = optixMulFloat4( q, ilw_sql );
...
// Ilwerts a 3x4 matrix in place
static __forceinline__ __device__ void optixIlwertMatrix( float4& m0, float4& m1, float4& m2 )
{
    const float det3 =
    ...
    float ilw3[3][3];
    ilw3[0][0] = ilw_det3 * ( m1.y * m2.z - m2.y * m1.z );
    ilw3[0][1] = ilw_det3 * ( m0.z * m2.y - m2.z * m0.y );
    ilw3[0][2] = ilw_det3 * ( m0.y * m1.z - m1.y * m0.z );

    ilw3[1][0] = ilw_det3 * ( m1.z * m2.x - m2.z * m1.x );
    ilw3[1][1] = ilw_det3 * ( m0.x * m2.z - m2.x * m0.z );
    ilw3[1][2] = ilw_det3 * ( m0.z * m1.x - m1.z * m0.x );

    ilw3[2][0] = ilw_det3 * ( m1.x * m2.y - m2.x * m1.y );
    ilw3[2][1] = ilw_det3 * ( m0.y * m2.x - m2.y * m0.x );
    ilw3[2][2] = ilw_det3 * ( m0.x * m1.y - m1.x * m0.y );
    ...
    m0.y = ilw3[0][1];
    m0.z = ilw3[0][2];
    m0.w = -ilw3[0][0] * b[0] - ilw3[0][1] * b[1] - ilw3[0][2] * b[2];

    m1.x = ilw3[1][0];
    m1.y = ilw3[1][1];
    m1.z = ilw3[1][2];
    m1.w = -ilw3[1][0] * b[0] - ilw3[1][1] * b[1] - ilw3[1][2] * b[2];

    m2.x = ilw3[2][0];
    m2.y = ilw3[2][1];
    m2.z = ilw3[2][2];
    m2.w = -ilw3[2][0] * b[0] - ilw3[2][1] * b[1] - ilw3[2][2] * b[2];
}
...
static __forceinline__ __device__ void optixLoadInterpolatedMatrixKey( float4& m0, float4& m1, float4& m2, const float4* matrix, const float t1 )
{
    ...
    // The conditional prevents conlwrrent loads leading to spills
    if( t1 > 0.0f )
    {
                                                                    ...
                                                                    const float   t1 )
{
    ...
    if( t1 > 0.0f )
    {
        ...
        float ilw_length = 1.f / sqrt( srt2.y * srt2.y + srt2.z * srt2.z + srt2.w * srt2.w + srt3.x * srt3.x );
        srt2.y *= ilw_length;
        srt2.z *= ilw_length;
        srt2.w *= ilw_length;
        srt3.x *= ilw_length;
                                                                                     ...
                                                                                     const bool objectToWorld )
{
    ...
    if( type == OPTIX_TRANSFORM_TYPE_MATRIX_MOTION_TRANSFORM || type == OPTIX_TRANSFORM_TYPE_SRT_MOTION_TRANSFORM )
    {
        ...
        if( !objectToWorld )
            optixIlwertMatrix( trf0, trf1, trf2 );
    }
    else if( type == OPTIX_TRANSFORM_TYPE_INSTANCE || type == OPTIX_TRANSFORM_TYPE_STATIC_TRANSFORM )
    {
        ...
        if( type == OPTIX_TRANSFORM_TYPE_INSTANCE )
        {
            transform = ( objectToWorld ) ? optixGetInstanceTransformFromHandle( handle ) :
                                            optixGetInstanceIlwerseTransformFromHandle( handle );
        }
        else
        {
            const OptixStaticTransform* traversable = optixGetStaticTransformFromHandle( handle );
            transform = (const float4*)( ( objectToWorld ) ? traversable->transform : traversable->ilwTransform );
        }
...
// Note that the given matrix is supposed to be the ilwerse of the actual transformation matrix.
