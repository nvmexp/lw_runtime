...
// __forceinline__ works in LWCA, VS, and with gcc.  Leave it as a macro in case
...
namespace optix {
  ...
  */
  ...
  public:
	...
	/** Returns the ilwerse of the matrix */
	RT_HOSTDEVICE Matrix<4,4>         ilwerse() const;
  ...
  */
  ...
  // Returns the ilwerse of the matrix.
  template<>
  OPTIXU_INLINE RT_HOSTDEVICE Matrix<4,4> Matrix<4,4>::ilwerse() const
  {
    Matrix<4,4> dst;
    const float* m   = m_data;
