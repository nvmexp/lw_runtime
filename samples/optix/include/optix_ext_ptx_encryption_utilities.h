...
{
  public:
    /// Sets up the given device context for encrypted PTX files with a non-deterministic session key.
    ///
    /// This method uses the two vendor keys to generate a session key that is used by the #encrypt()
    /// method below. It also ensures that the same session key is computed inside OptiX without
    /// passing it over the API boundary. The session key is not deterministic since it depends
    /// on two random numbers chosen by both sides.
    ///
    /// Afterwards, all PTX code passed to methods of this context must have been encrypted with
    /// the #encrypt() method of this instance. There is no way to disable encryption again.
    PtxEncryption( OptixDeviceContext context, const char* publicVendorKey, const char* secretVendorKey );

    /// Sets up the given device context for encrypted PTX files with a deterministic session key.
    ///
    /// This method uses the two vendor keys to generate a session key that is used by the #encrypt()
    /// method below. It also ensures that the same session key is computed inside OptiX without
    /// passing it over the API boundary. The session key is deterministic. Both salt values need
    /// to be 32 bytes long.
    ///
    /// Afterwards, all PTX code passed to methods of this context must have been encrypted with
    /// the #encrypt() method of this instance. There is no way to disable encryption again.
    PtxEncryption( OptixDeviceContext context, const char* publicVendorKey, const char* secretVendorKey, const char* optixSalt, const char* vendorSalt );

    /// Encrypts the given PTX code using our session key.
    ::std::string encrypt( const ::std::string& ptx ) const;

    /// The prefix used by the encrypted PTX code.
...
void sha256( const unsigned char* input, unsigned int inputLength, unsigned char* buffer );
...
inline PtxEncryption::PtxEncryption( OptixDeviceContext context, const char* publicVendorKey, const char* secretVendorKeyHex )
{
    // Get optix salt
    const size_t  optixSaltLength = 32;
    unsigned char optixSalt[optixSaltLength];
    OptixResult   result = optixExtPtxEncryptionGetOptixSalt( context, &optixSalt[0], 32 );
    ...
    result = optixExtPtxEncryptionSetVendorSalt( context, &vendorSalt[0], 32 );
    assert( result == OPTIX_SUCCESS );

    // Set vendor public key
    const size_t publicVendorKeyLength = strlen( publicVendorKey );
    result = optixExtPtxEncryptionSetPublicVendorKey( context, &publicVendorKey[0], publicVendorKeyLength );
    assert( result == OPTIX_SUCCESS );
...
inline PtxEncryption::PtxEncryption( OptixDeviceContext context,
                                     ...
                                     const char*        vendorSalt )
{
    ...
    OptixResult  result          = optixExtPtxEncryptionSetOptixSalt( context, &optixSalt[0], optixSaltLength );
    assert( result == OPTIX_SUCCESS );
    (void)result;

    // Set vendor salt
    const size_t vendorSaltLength = strlen( vendorSalt );
    result                        = optixExtPtxEncryptionSetVendorSalt( context, &vendorSalt[0], vendorSaltLength );
    assert( result == OPTIX_SUCCESS );
...
inline void generateSalt( unsigned char* buffer )
{
    ...
    number += tmp.QuadPart + GetLwrrentProcessId() + GetTickCount();
#else
...
// First 32 bits of the fractional parts of the lwbe roots of the first 64 primes 2..311
...
inline void sha256( const unsigned char* input, unsigned int inputLength, unsigned char* buffer )
{
    if( ( inputLength <= 0 ) || ( input == 0 ) || ( buffer == 0 ) )
        return;
    ...
    unsigned int r = ( inputLength * 8 + 1 ) % 512;
    ...
    for( unsigned int chunk = 0; k != 0; ++chunk )
    {
        ...
        unsigned int to_copy = inputLength - pos;
