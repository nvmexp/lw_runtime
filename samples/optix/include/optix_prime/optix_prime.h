 ...
 * from LWPU Corporation is strictly prohibited.
 *
...
#define OPTIX_PRIME_VERSION 60804  /* major =  OPTIX_PRIME_VERSION/10000,        *
...
extern "C" {
   ...
   *
   * By default, a context created with type @ref RTP_CONTEXT_TYPE_LWDA will
   * use the fastest available LWCA device, but note that specific devices can be selected using
   * @ref rtpContextSetLwdaDeviceNumbers. The fastest device will be set as the current device when the
   * function returns. If no LWCA device features compute capability 3.0 or greater,
   ...
   * - @ref RTP_ERROR_ILWALID_VALUE
   * - @ref RTP_ERROR_MEMORY_ALLOCATION_FAILED
   *
   * Example Usage:
   @code
   RTPcontext context;
   if(rtpContextCreate( RTP_CONTEXT_TYPE_LWDA, &context ) == RTP_SUCCESS ) {
     int deviceNumbers[] = {0,1};
     rtpContextSetLwdaDeviceNumbers( 2, deviceNumbers );
   }
   else
  ...
  /**
   * @brief   Sets the LWCA devices used by a context
   *
   * @ingroup Prime_Context
   *
   ...
   *
   * This function will return an error if the provided context is not of type @ref RTP_CONTEXT_TYPE_LWDA
   *
   * <B>Return values</B>
   *
   * Relevant return values:
   ...
   *
   */
  RTPresult RTPAPI rtpContextSetLwdaDeviceNumbers( RTPcontext  context, unsigned deviceCount, const unsigned* deviceNumbers );
   ...
   * @ref RTP_BUFFER_TYPE_LWDA_LINEAR is assumed to reside on the current
   * device. The device number can be changed by calling @ref
   * rtpBufferDescSetLwdaDeviceNumber.
   *
   * @param[in]  context      OptiX Prime context
  ...
  */
  ...
  /**
   * @brief   Sets the LWCA device number for a buffer
   *
   * @ingroup Prime_BufferDesc
   *
   * A buffer of type @ref RTP_BUFFER_TYPE_LWDA_LINEAR is assumed to reside
   * on the device that was current when its buffer descriptor was created unless
   * otherwise specified using this function.
   *
   * @param[in] desc           Buffer descriptor
   * @param[in] deviceNumber   LWCA device number
   *
   * <B>Return values</B>
   *
   * Relevant return values:
   ...
   *
   */
  RTPresult RTPAPI rtpBufferDescSetLwdaDeviceNumber( RTPbufferdesc desc, unsigned deviceNumber );
  ...
  */
   ...
   * chunk size is chosen automatically, lwrrently as 10% of the total
   ...
   * @ingroup Prime_Query
   *
   * The rays buffer is not accessed until rtpQueryExelwte() is called.
  ...
  /**
   * @brief   Exelwtes a raytracing query
   *
   * @ingroup Prime_Query
   *
   * If the flag @ref RTP_QUERY_HINT_ASYNC is specified, rtpQueryExelwte may
   ...
   * Example Usage:
   ...
   rtpQueryExelwte(query, 0);
   // safe to modify ray buffer and process hits
   @endcode
  */
  RTPresult RTPAPI rtpQueryExelwte( RTPquery query, unsigned hints );
   ...
   * @ingroup Prime_Query
   *
   * Specify a Lwca stream used for synchronization. If no stream is specified,
   * the default 0-stream is used. A stream can only be specified for
   * contexts with type @ref RTP_CONTEXT_TYPE_LWDA.
   *
   * @param[in] query      Query
   * @param[in] stream     A lwca stream
   *
   * <B>Return values</B>
   *
   * Relevant return values:
   ...
   *
   */
  RTPresult RTPAPI rtpQuerySetLwdaStream( RTPquery query, lwdaStream_t stream );
