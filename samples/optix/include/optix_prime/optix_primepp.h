...
namespace optix {
  namespace prime {
    ...
    public:
      ...
      /// Sets the LWCA devices used by a context.  See @ref rtpContextSetLwdaDeviceNumbers
      ...
      void setLwdaDeviceNumbers( const std::vector<unsigned>& deviceNumbers );

      /// Sets the LWCA devices used by a context.  See @ref rtpContextSetLwdaDeviceNumbers
      /// Note that this distribution can be rather costly if the rays are stored in device memory though.
      /// For maximum efficiency it is recommended to only ever select one device per context.
      void setLwdaDeviceNumbers( unsigned deviceCount, const unsigned* deviceNumbers );
    ...
    public:
      ...
      /// Sets the LWCA device number for a buffer. See @ref rtpBufferDescSetLwdaDeviceNumber
      void setLwdaDeviceNumber( unsigned deviceNumber );
    ...
    public:
      ...
      /// Sets a stream for a query. See @ref rtpQuerySetLwdaStream
      void setLwdaStream( lwdaStream_t stream );
      ...
      /// Exelwtes a raytracing query. See @ref rtpQueryExelwte
    ...
    inline void ContextObj::setLwdaDeviceNumbers( const std::vector<unsigned>& deviceNumbers )
    {
      if( deviceNumbers.size()==0 ) {
        CHK( rtpContextSetLwdaDeviceNumbers(m_ctx, 0, NULL) );
      } else {
        CHK( rtpContextSetLwdaDeviceNumbers(m_ctx, (unsigned int)deviceNumbers.size(), &deviceNumbers[0]) );
      }
    }

    inline void ContextObj::setLwdaDeviceNumbers( unsigned deviceCount, const unsigned* deviceNumbers )
    {
        CHK( rtpContextSetLwdaDeviceNumbers(m_ctx, deviceCount, deviceNumbers) );
    }
    ...
    inline void BufferDescObj::setLwdaDeviceNumber( unsigned deviceNumber )
    {
      CHK( rtpBufferDescSetLwdaDeviceNumber(m_desc, deviceNumber) );
    }
    ...
    {
      CHK( rtpQueryExelwte(m_query, hint) );
    }
    ...
    inline void QueryObj::setLwdaStream( lwdaStream_t stream )
    {
      CHK( rtpQuerySetLwdaStream(m_query, stream) );
    }
