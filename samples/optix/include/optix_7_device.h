...
#error Device code for OptiX requires at least C++11. Consider adding "--std c++11" to the lwcc command-line.
#endif
...
/// \param[in] missSBTIndex   specifies the miss program ilwoked on a miss
...
/// Return the object space lwrve control vertex data of a linear lwrve in a Geometry
/// Acceleration Structure (GAS) at a given motion time.
/// To access vertex data, the GAS must be built using the flag OPTIX_BUILD_FLAG_ALLOW_RANDOM_VERTEX_ACCESS.
///
...
/// If motion is disabled via OptixPipelineCompileOptions::usesMotionBlur, or the GAS does not contain motion, the
/// time parameter is ignored.
static __forceinline__ __device__ void optixGetLinearLwrveVertexData( OptixTraversableHandle gas, unsigned int primIdx, unsigned int sbtGASIndex, float time, float4 data[2] );

/// Return the object space lwrve control vertex data of a quadratic BSpline lwrve in a Geometry
/// Acceleration Structure (GAS) at a given motion time.
/// To access vertex data, the GAS must be built using the flag OPTIX_BUILD_FLAG_ALLOW_RANDOM_VERTEX_ACCESS.
///
/// data[i] = {x,y,z,w} with {x,y,z} the position and w the radius of control vertex i.
...
/// Return the object space lwrve control vertex data of a cubic BSpline lwrve in a Geometry
/// Acceleration Structure (GAS) at a given motion time.
/// To access vertex data, the GAS must be built using the flag OPTIX_BUILD_FLAG_ALLOW_RANDOM_VERTEX_ACCESS.
///
/// data[i] = {x,y,z,w} with {x,y,z} the position and w the radius of control vertex i.
/// If motion is disabled via OptixPipelineCompileOptions::usesMotionBlur, or the GAS does not contain motion, the
/// time parameter is ignored.
static __forceinline__ __device__ void optixGetLwbicBSplineVertexData( OptixTraversableHandle gas, unsigned int primIdx, unsigned int sbtGASIndex, float time, float4 data[4] );

/// Return the object space lwrve control vertex data of a CatmullRom spline lwrve in a Geometry
/// Acceleration Structure (GAS) at a given motion time.
/// To access vertex data, the GAS must be built using the flag OPTIX_BUILD_FLAG_ALLOW_RANDOM_VERTEX_ACCESS.
///
/// data[i] = {x,y,z,w} with {x,y,z} the position and w the radius of control vertex i.
...
/// 2. call optixTerminateRay       -    hit is recorded, optixReportIntersection does not return, no further traversal oclwrs,
...
/// For a given OptixBuildInputLwstomPrimitiveArray the number of primitives is defined as
/// numAabbs.
///
/// The primitive index returns the index into the array of primitives
/// plus the primitiveIndexOffset.
///
/// In IS and AH this corresponds to the lwrrently intersected primitive.
...
/// In EX with exception code OPTIX_EXCEPTION_CODE_TRAVERSAL_ILWALID_HIT_SBT corresponds to the sbt index within the hit GAS. Returns zero for all other exceptions.
...
/// For custom intersections (primitive type OPTIX_PRIMITIVE_TYPE_LWSTOM),
...
/// Colwenience function interpreting the result of #optixGetHitKind().
...
/// Colwenience function that returns the first two attributes as floats.
///
...
/// Colwenience function that returns the lwrve parameter.
///
/// When using OptixBuildInputLwrveArray objects, during intersection the lwrve parameter
...
/// Returns the generic memory space pointer to the data region (past the header) of the lwrrently active SBT record corresponding to the current program.
...
/// Returns the invalid traversable handle for exceptions with exception code OPTIX_EXCEPTION_CODE_TRAVERSAL_ILWALID_TRAVERSABLE. 
/// 
...
/// Returns the invalid sbt offset for exceptions with exception code OPTIX_EXCEPTION_CODE_TRAVERSAL_ILWALID_MISS_SBT and OPTIX_EXCEPTION_CODE_TRAVERSAL_ILWALID_HIT_SBT. 
/// 
/// Returns zero for all other exception codes. 
/// 
/// Only available in EX.
...
/// Returns the invalid ray for exceptions with exception code OPTIX_EXCEPTION_CODE_ILWALID_RAY.
/// Exceptions of type OPTIX_EXCEPTION_CODE_ILWALID_RAY are thrown when one or more values that were
/// passed into optixTrace are either inf or nan.
///
/// OptixIlwalidRayExceptionDetails::rayTime will always be 0 if OptixPipelineCompileOptions::usesMotionBlur is 0.
...
/// OPTIX_EXCEPTION_CODE_UNSUPPORTED_PRIMITIVE_TYPE, OPTIX_EXCEPTION_CODE_ILWALID_RAY, and for user exceptions.
...
/// The address of the SBT entry is callwlated by OptixShaderBindingTable::callablesRecordBase + ( OptixShaderBindingTable::callablesRecordStrideInBytes * sbtIndex ).
/// 
...
/// The address of the SBT entry is callwlated by OptixShaderBindingTable::callablesRecordBase + ( OptixShaderBindingTable::callablesRecordStrideInBytes * sbtIndex ).
/// As opposed to direct callable programs, continuation callable programs are allowed to call optixTrace relwrsively.
/// 
...
/// optixTexFootprint2D callwlates the footprint of a corresponding 2D texture fetch (non-mipmapped).
///
...
///
/// \param[in] tex      LWCA texture object (cast to 64-bit integer)
...
///   texInfo[9:8]   = horizontal wrap mode (2 bits) (LWaddress_mode)
///   texInfo[7:6]   = vertical wrap mode (2 bits) (LWaddress_mode)
///   texInfo[5]     = mipmap filter mode (1 bit) (LWfilter_mode)
...
/// optixTexFootprint2DLod callwlates the footprint of a corresponding 2D texture fetch (tex2DLod)
/// \param[in] tex      LWCA texture object (cast to 64-bit integer)
/// \param[in] texInfo  Texture info packed into 32-bit integer, described below.
/// \param[in] x        Texture coordinate
/// \param[in] y        Texture coordinate
...
/// optixTexFootprint2DGrad callwlates the footprint of a corresponding 2D texture fetch (tex2DGrad)
/// \param[in] tex      LWCA texture object (cast to 64-bit integer)
/// \param[in] texInfo  Texture info packed into 32-bit integer, described below.
/// \param[in] x        Texture coordinate
/// \param[in] y        Texture coordinate
