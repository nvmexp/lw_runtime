...
__END__
=head1 NAME
Digest - Modules that callwlate message digests
=head1 SYNOPSIS
  $md5  = Digest->new("MD5");
...
=head1 DESCRIPTION
The C<Digest::> modules callwlate digests, also called "fingerprints"
...
dolwmented here should make it easy to change algorithms later.
...
message we callwlate the digest for.  The return value is the $ctx
object itself.
...
or reset the $ctx object if this oclwrs.
...
automatically C<reset> and can be used to callwlate another digest
value.  Call $ctx->clone->digest if you want to callwlate the digest
