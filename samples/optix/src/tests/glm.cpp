...
{
    if( _glmAbs( u[0] - v[0] ) < epsilon && _glmAbs( u[1] - v[1] ) < epsilon && _glmAbs( u[2] - v[2] ) < epsilon )
    {
        return true;
    }
    return false;
 ...
 *
 */
static float* _glmWeldVectors( float* vectors, unsigned int* numvectors, float epsilon )
{
    float*       copies;
    unsigned int copied;
    unsigned int i, j;
...
static int _glmReadMTL( GLMmodel* model, char* name )
{
    ...
    for( i = 0; i < nummaterials; i++ )
    {
        ...
        model->materials[i].spelwlar[0]  = 0.0f;
        model->materials[i].spelwlar[1]  = 0.0f;
        model->materials[i].spelwlar[2]  = 0.0f;
        model->materials[i].spelwlar[3]  = 1.0f;
        ...
        model->materials[i].spelwlar_map[0] = '\0';
        ...
        model->materials[i].spelwlar_map_scaling[0] = 0;
        model->materials[i].spelwlar_map_scaling[1] = 0;
    ...
    while( fscanf( file, "%s", buf ) != EOF )
    {
        switch( buf[0] )
        {
            ...
            case 'm':
            {
                ...
                {
                    map_name = model->materials[nummaterials].spelwlar_map;
                    scaling  = model->materials[nummaterials].ambient_map_scaling;
                }
                ...
                switch( buf[1] )
                {
                    case 'd':
                        fscanf( file, "%f %f %f", &model->materials[nummaterials].diffuse[0],
                                &model->materials[nummaterials].diffuse[1], &model->materials[nummaterials].diffuse[2] );
                        break;
                    case 's':
                        fscanf( file, "%f %f %f", &model->materials[nummaterials].spelwlar[0],
                                &model->materials[nummaterials].spelwlar[1], &model->materials[nummaterials].spelwlar[2] );
                        break;
                    case 'a':
                        fscanf( file, "%f %f %f", &model->materials[nummaterials].ambient[0],
                                &model->materials[nummaterials].ambient[1], &model->materials[nummaterials].ambient[2] );
                        break;
                    default:
                        /* eat up rest of line */
                        fgets( buf, sizeof( buf ), file );
...
static int _glmWriteMTL( GLMmodel* model, char* modelpath, char* mtllibname )
{
    ...
    for( i = 0; i < model->nummaterials; i++ )
    {
        ...
        fprintf( file, "Ks %f %f %f\n", material->spelwlar[0], material->spelwlar[1], material->spelwlar[2] );
        fprintf( file, "Ns %f\n", material->shininess );
        fprintf( file, "\n" );
    }
    return 0;
...
static int _glmFirstPass( GLMmodel* model, FILE* file )
{
    ...
    char*     lwrrent_group_base_name = strdup( default_group_name );
    char*     lwrrent_material_name   = strdup( default_material_name );
    int       v, n, t;
    char      buf[2048];

    /* make a default group */
    group = _glmAddGroup( model, lwrrent_group_base_name );
        ...
        switch( buf[0] )
        {
            ...
            case 'm':
                ...
                if( _glmReadMTL( model, buf ) )
                {
                    ...
                    if( lwrrent_group_base_name )
                        free( lwrrent_group_base_name );
                    if( lwrrent_material_name )
                        free( lwrrent_material_name );
                    return 1;
                }
                break;
            case 'u':
                ...
                if( lwrrent_material_name )
                    free( lwrrent_material_name );
                lwrrent_material_name = strdup( buf );
                sprintf( buf, "%s_MAT_%s", lwrrent_group_base_name, lwrrent_material_name );
            ...
            case 'g': /* group */
                ...
                if( lwrrent_group_base_name )
                    free( lwrrent_group_base_name );
                lwrrent_group_base_name = strdup( buf );
                sprintf( buf, "%s_MAT_%s", lwrrent_group_base_name, lwrrent_material_name );
                group = _glmAddGroup( model, buf );
                break;
            case 'f': /* face */
            ...
            case 'v': /* v, vn, vt */
                switch( buf[1] )
                {
                    case '\0': /* vertex */
                        ...
                        {
                            fscanf( file, "%f %f %f", &vertices[3 * numvertices + X], &vertices[3 * numvertices + Y],
                        ...
                        else
                        {
                            ...
                            fscanf( file, "%f %f %f %d %d %d", &vertices[3 * numvertices + X],
                                    &vertices[3 * numvertices + Y], &vertices[3 * numvertices + Z], &r, &g, &b );
                        ...
                        break;
                    case 'n': /* normal */
                        fscanf( file, "%f %f %f", &normals[3 * numnormals + X], &normals[3 * numnormals + Y],
                                &normals[3 * numnormals + Z] );
                        numnormals++;
                        break;
                    case 't': /* texcoord */
                        fscanf( file, "%f %f", &texcoords[2 * numtexcoords + X], &texcoords[2 * numtexcoords + Y] );
                        numtexcoords++;
                        break;
                }
                break;
 ...
 * scaling it to fit in a unit lwbe around the origin.  Returns the
 * scalefactor used.
 *
 * model - properly initialized GLMmodel structure 
 */
float glmUnitize( GLMmodel* model )
{
    ...
    /* callwlate model width, height, and depth */
    w = _glmAbs( maxx ) + _glmAbs( minx );
    h = _glmAbs( maxy ) + _glmAbs( miny );
    d = _glmAbs( maxz ) + _glmAbs( minz );

    /* callwlate center of the model */
    cx = ( maxx + minx ) / 2.0f;
    cy = ( maxy + miny ) / 2.0f;
    cz = ( maxz + minz ) / 2.0f;

    /* callwlate unitizing scale factor */
...
/* glmDimensions: Callwlates the dimensions (width, height, depth) of
 * a model.
 *
...
/*
 * glmBoundingBox: Callwlates the min/max positions of the model
 */
...
void glmFacetNormals( GLMmodel* model )
{
    ...
    for( i = 0; i < model->numtriangles; i++ )
    {
        ...
        u[X] = model->vertices[3 * T( i ).vindices[1] + X] - model->vertices[3 * T( i ).vindices[0] + X];
        u[Y] = model->vertices[3 * T( i ).vindices[1] + Y] - model->vertices[3 * T( i ).vindices[0] + Y];
        u[Z] = model->vertices[3 * T( i ).vindices[1] + Z] - model->vertices[3 * T( i ).vindices[0] + Z];

        v[X] = model->vertices[3 * T( i ).vindices[2] + X] - model->vertices[3 * T( i ).vindices[0] + X];
        v[Y] = model->vertices[3 * T( i ).vindices[2] + Y] - model->vertices[3 * T( i ).vindices[0] + Y];
        v[Z] = model->vertices[3 * T( i ).vindices[2] + Z] - model->vertices[3 * T( i ).vindices[0] + Z];
 ...
 * average normal callwlation and the corresponding vertex is given
...
void glmVertexNormals( GLMmodel* model, float angle )
{
    ...
    /* callwlate the cosine of the angle (in degrees) */
    ...
    /* callwlate the average normal for each vertex */
    numnormals = 1;
    for( i = 1; i <= model->numvertices; i++ )
    {
        /* callwlate an average normal for this vertex by averaging the
...
void glmLinearTexture( GLMmodel* model )
{
    ...
    scalefactor = 2.0f / _glmAbs( _glmMax( _glmMax( dimensions[0], dimensions[1] ), dimensions[2] ) );
 ...
 * these by using the normal to callwlate where that vertex would map
...
GLMmodel* glmReadOBJ( const char* filename )
{
  ...
  printf("Model: %s\n", filename);
    ...
    model->vertexColors = (unsigned char*)malloc( sizeof( unsigned char ) * 3 * ( model->numvertices + 1 ) );
...
int glmWriteOBJ( GLMmodel* model, char* filename, unsigned int mode )
{
    ...
    else if( mode & GLM_FLAT )
    {
        ...
        for( i = 1; i <= model->numnormals; i++ )
        {
            fprintf( file, "vn %f %f %f\n", model->facetnorms[3 * i + 0], model->facetnorms[3 * i + 1],
        ...
        for( i = 0; i < group->numtriangles; i++ )
        {
            if( mode & GLM_SMOOTH && mode & GLM_TEXTURE )
            {
                fprintf( file, "f %u/%u/%u %u/%u/%u %u/%u/%u\n", T( group->triangles[i] ).vindices[0],
                         T( group->triangles[i] ).nindices[0], T( group->triangles[i] ).tindices[0],
                         T( group->triangles[i] ).vindices[1], T( group->triangles[i] ).nindices[1],
                         T( group->triangles[i] ).tindices[1], T( group->triangles[i] ).vindices[2],
                         T( group->triangles[i] ).nindices[2], T( group->triangles[i] ).tindices[2] );
            }
            ...
            {
                fprintf( file, "f %u/%u %u/%u %u/%u\n", T( group->triangles[i] ).vindices[0],
                         T( group->triangles[i] ).tindices[0], T( group->triangles[i] ).vindices[1],
                         T( group->triangles[i] ).tindices[1], T( group->triangles[i] ).vindices[2],
            ...
            {
                fprintf( file, "f %u//%u %u//%u %u//%u\n", T( group->triangles[i] ).vindices[0],
                         T( group->triangles[i] ).nindices[0], T( group->triangles[i] ).vindices[1],
                         T( group->triangles[i] ).nindices[1], T( group->triangles[i] ).vindices[2],
                         T( group->triangles[i] ).nindices[2] );
            }
            else if( mode & GLM_FLAT )
            {
                fprintf( file, "f %u//%u %u//%u %u//%u\n", T( group->triangles[i] ).vindices[0],
                         T( group->triangles[i] ).findex, T( group->triangles[i] ).vindices[1], T( group->triangles[i] ).findex,
                         T( group->triangles[i] ).vindices[2], T( group->triangles[i] ).findex );
            }
            else
            {
                fprintf( file, "f %u %u %u\n", T( group->triangles[i] ).vindices[0],
                         T( group->triangles[i] ).vindices[1], T( group->triangles[i] ).vindices[2] );
            }
        }
        fprintf( file, "\n" );
        group = group->next;
...
glmDraw(GLMmodel* model, unsigned int mode)
{
  ...
  while (group) {
    if (mode & GLM_MATERIAL) {
      ...
      glMaterialfv(GL_FRONT_AND_BACK, GL_SPELWLAR, 
          model->materials[group->material].spelwlar);
