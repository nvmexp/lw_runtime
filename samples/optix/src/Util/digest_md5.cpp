...
bool MD5::update( const char* input, size_t input_length )
{
...
bool MD5::update( FILE* file )
{
    char   buffer[1024];
    size_t len;

    while( ( len = fread( buffer, 1, 1024, file ) ) > 0 )
        if( !update( buffer, len ) )
            return false;
...
MD5::MD5( const char* input, size_t input_length )
{
    init();  // must be called be all constructors
...
const char* MD5::raw_digest() const
{
    if( finalized )
...
static const char s_hextab[] = "0123456789abcdef";

const char* MD5::hex_digest( char* bffr ) const
{
    if( finalized )
    {
        for( int i = 0; i < 16; i++ )
        {
            //sprintf(bffr + i*2, "%02x", digest[i]);
            bffr[i * 2 + 0] = s_hextab[( digest[i] & 0xf0 ) >> 4];
            bffr[i * 2 + 1] = s_hextab[( digest[i] & 0x0f ) >> 0];
        }
