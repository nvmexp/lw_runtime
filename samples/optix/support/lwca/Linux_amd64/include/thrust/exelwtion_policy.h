...
/*! \file thrust/exelwtion_policy.h
 *  \brief Thrust exelwtion policies.
 */
...
#include <thrust/detail/exelwte_with_allocator.h>
#include <thrust/detail/seq.h>

//! \cond

// #include the host system's exelwtion_policy header
#define __THRUST_HOST_SYSTEM_EXELWTION_POLICY_HEADER <__THRUST_HOST_SYSTEM_ROOT/exelwtion_policy.h>
#include __THRUST_HOST_SYSTEM_EXELWTION_POLICY_HEADER
#undef __THRUST_HOST_SYSTEM_EXELWTION_POLICY_HEADER

// #include the device system's exelwtion_policy.h header
#define __THRUST_DEVICE_SYSTEM_EXELWTION_POLICY_HEADER <__THRUST_DEVICE_SYSTEM_ROOT/exelwtion_policy.h>
#include __THRUST_DEVICE_SYSTEM_EXELWTION_POLICY_HEADER
#undef __THRUST_DEVICE_SYSTEM_EXELWTION_POLICY_HEADER
...
/*! \addtogroup exelwtion_policies Parallel Exelwtion Policies
 *  \{
 */


// define exelwtion_policy for the purpose of Doxygenating it
// it is actually defined elsewhere
#if 0
/*! \p exelwtion_policy is the base class for all Thrust parallel exelwtion policies
 ...
 *
 *  The following code snippet demonstrates how to derive a standalone custom exelwtion policy
 *  from \p thrust::exelwtion_policy to implement a backend which only implements \p for_each:
 *
 *  \code
 *  #include <thrust/exelwtion_policy.h>
 *  #include <iostream>
 *
 *  // define a type derived from thrust::exelwtion_policy to distinguish our custom exelwtion policy:
 *  struct my_policy : thrust::exelwtion_policy<my_policy> {};
 *
 ...
 *  \endcode
 *
 *  \see host_exelwtion_policy
 *  \see device_exelwtion_policy
 */
template<typename DerivedPolicy>
struct exelwtion_policy : thrust::detail::exelwtion_policy_base<DerivedPolicy>
{};
#endif


/*! \p host_exelwtion_policy is the base class for all Thrust parallel exelwtion policies
 ...
 *
 *  The following code snippet demonstrates how to derive a standalone custom exelwtion policy from
 *  \p thrust::host_exelwtion_policy to implement a backend which specializes \p for_each while inheriting
 ...
 *  #include <iostream>
 *
 *  // define a type derived from thrust::host_exelwtion_policy to distinguish our custom exelwtion policy:
 *  struct my_policy : thrust::host_exelwtion_policy<my_policy> {};
 *
 *  // overload for_each on my_policy
 *  template<typename Iterator, typename Function>
 *  Iterator for_each(my_policy, Iterator first, Iterator last, Function f)
 ...
 *  \endcode
 *
 *  \see exelwtion_policy
 *  \see device_exelwtion_policy
 */
template<typename DerivedPolicy>
  struct host_exelwtion_policy
    : thrust::system::__THRUST_HOST_SYSTEM_NAMESPACE::exelwtion_policy<DerivedPolicy>
{};


/*! \p device_exelwtion_policy is the base class for all Thrust parallel exelwtion policies
 ...
 *
 *  The following code snippet demonstrates how to derive a standalone custom exelwtion policy from
 *  \p thrust::device_exelwtion_policy to implement a backend which specializes \p for_each while inheriting
 ...
 *  #include <iostream>
 *
 *  // define a type derived from thrust::device_exelwtion_policy to distinguish our custom exelwtion policy:
 *  struct my_policy : thrust::device_exelwtion_policy<my_policy> {};
 *
 *  // overload for_each on my_policy
 *  template<typename Iterator, typename Function>
 *  Iterator for_each(my_policy, Iterator first, Iterator last, Function f)
 ...
 */
template<typename DerivedPolicy>
  struct device_exelwtion_policy
    : thrust::system::__THRUST_DEVICE_SYSTEM_NAMESPACE::exelwtion_policy<DerivedPolicy>
{};


/*! \p thrust::host is the default parallel exelwtion policy associated with Thrust's host backend system
 ...
 *
 *  Note that even though \p thrust::host targets the host CPU, it is a parallel exelwtion policy. That is,
 *  the order that an algorithm ilwokes functors or dereferences iterators is not defined.
 *
...
/*! \p thrust::device is the default parallel exelwtion policy associated with Thrust's device backend system
 ...
 *
 *  Explicit dispatch can be useful in avoiding the introduction of data copies into containers such as
 *  \p thrust::device_vector or to avoid wrapping e.g. raw pointers allocated by the LWCA API with types
 ...
 *  with a \p thrust::device algorithm invocation when the device backend is LWCA.
 *
...
// it is actually defined elsewhere
#if 0
/*! \p thrust::seq is an exelwtion policy which requires an algorithm invocation to execute sequentially
