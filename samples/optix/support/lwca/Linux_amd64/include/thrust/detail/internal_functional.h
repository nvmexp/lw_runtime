...
// colwert a predicate to a 0 or 1 integral value
...
// note that detail::equal_to does not force colwersion from T2 -> T1 as equal_to does
...
// note that equal_to_value does not force colwersion from T2 -> T1 as equal_to does
template<typename T2>
...
{
  // add __host__ to allow the omp backend to compile with lwcc
  __host__ __device__
  void operator()(T &x) const
  {
    x.~T();
