 ...
 *  \tparam InputIterator must be a model of <a href="http://www.sgi.com/tech/stl/InputIterator.html">Input Iterator</a> and \c InputIterator's \c value_type must be colwertible to \c OutputIterator's \c value_type.
...
__host__ __device__
  OutputIterator copy(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
                      InputIterator first,
                      InputIterator last,
...
__host__ __device__
  OutputIterator copy_n(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
                        InputIterator first,
                        Size n,
                        OutputIterator result);
 ...
 *  oclwrs and \p result is not advanced.
 *
 *  The algorithm's exelwtion is parallelized as determined by \p system.
 *
 *  \param exec The exelwtion policy to use for parallelization.
...
__host__ __device__
  OutputIterator copy_if(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
                         InputIterator first,
                         InputIterator last,
                         OutputIterator result,
                         Predicate pred);
 ...
 *
 *  The following code snippet demonstrates how to use \p copy_if to perform stream compaction
 *  to copy numbers to an output range when corresponding stencil elements are even using the \p thrust::host exelwtion policy:
 *
 *  \code
 *  #include <thrust/copy.h>
 *  #include <thrust/exelwtion_policy.h>
