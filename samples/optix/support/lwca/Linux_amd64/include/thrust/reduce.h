 ...
 *          then <tt>x + y</tt> is defined and is colwertible to \p InputIterator's
 ...
 *  the sum of a sequence of integers using the \p thrust::host exelwtion policy for parallelization:
 *
 *  \code
...
__host__ __device__
  typename thrust::iterator_traits<InputIterator>::value_type
    reduce(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec, InputIterator first, InputIterator last);
 ...
 *  \tparam InputIterator is a model of <a href="http://www.sgi.com/tech/stl/InputIterator.html">Input Iterator</a>
 *          and if \c x and \c y are objects of \p InputIterator's \c value_type,
 *          then <tt>x + y</tt> is defined and is colwertible to \p T.
 *  \tparam T is colwertible to \p InputIterator's \c value_type.
 *
 *  The following code snippet demonstrates how to use \p reduce to compute
...
__host__ __device__
  T reduce(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
           InputIterator first,
           InputIterator last,
           T init);
 ...
 *  \tparam DerivedPolicy The name of the derived exelwtion policy.
 *  \tparam InputIterator is a model of <a href="http://www.sgi.com/tech/stl/InputIterator.html">Input Iterator</a>
 *          and \c InputIterator's \c value_type is colwertible to \c T.
 ...
 *  compute the maximum value of a sequence of integers using the \p thrust::host exelwtion policy
 *  for parallelization:
 *
 *  \code
 *  #include <thrust/reduce.h>
...
__host__ __device__
  ...
  reduce_by_key(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
 ...
 *  Specifically, if conselwtive key iterators \c i and \c (i + 1) are 
 ...
 *  \tparam BinaryPredicate is a model of <a href="http://www.sgi.com/tech/stl/BinaryPredicate.html">Binary Predicate</a>.
 *  \tparam BinaryFunction is a model of <a href="http://www.sgi.com/tech/stl/BinaryFunction.html">Binary Function</a>
 *          and \c BinaryFunction's \c result_type is colwertible to \c OutputIterator2's \c value_type.
 *
 *  \pre The input ranges shall not overlap either output range.
 *
 *  The following code snippet demonstrates how to use \p reduce_by_key to
