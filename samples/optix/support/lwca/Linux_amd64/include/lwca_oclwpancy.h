 ...
 *   lwdaOccAvailableDynamicSMemPerBlock
 *
 * DESCRIPTION
 *
...
/**
 *
 * The LWCA dynamic shared memory calculator computes the maximum size of 
 ...
 */
static __OCC_INLINE
lwdaOccError lwdaOccAvailableDynamicSMemPerBlock(
 ...
 */
static __OCC_INLINE lwdaOccError lwdaOccRegAllocationMaxPerThread(int *limit, const lwdaOccDeviceProp *properties)
{
    int value;
 ...
 */
static __OCC_INLINE lwdaOccError lwdaOccRegAllocationGranularity(int *limit, const lwdaOccDeviceProp *properties)
{
    int value;
...
static __OCC_INLINE lwdaOccError lwdaOccSMemPerBlock(size_t *limit, const lwdaOccDeviceProp *properties, lwdaOccFuncShmemConfig shmemLimitConfig, size_t smemPerCta)
{
    ...
    // Starting Ampere, LWCA driver reserves additional shared memory per block
    ...
    if (dynamicSmemSizeExceeded || totalSmemSizeExceeded) {
        ...
        if (userSmemPreference >= smemAllocatedPerCTA) {
            ...
            if (properties->computeMajor >= 7) {
                ...
                status = lwdaOccAlignUpShmemSizeVoltaPlus(&sharedMemPerMultiprocessor, properties);
                if (status != LWDA_OCC_SUCCESS) {
                    return status;
                }
            }
    ...
    int                          blockSize)
{
    ...
    status = lwdaOccRegAllocationMaxPerThread(
        &maxRegsPerThread,
        properties);
    if (status != LWDA_OCC_SUCCESS) {
        return status;
    ...
    if (properties->regsPerBlock < regsAssumedPerCTA ||   // Hardware check
        ...
        if (regsAllocatedPerWarp > 0) {
            ...
            // if the user forces enablement via PARTITIONED_GC_ON_STRICT to callwlate
    ...
    size_t                       dynamicSmemSize)
{
    ...
    // Therefore, we check the oclwpancy on GP10x when it can run on GP100
    //
    if (properties->computeMajor == 6 && properties->computeMinor == 0 && ctaLimitRegs) {
        lwdaOccDeviceProp propertiesGP10x;
        lwdaOccPartitionedGCConfig gcConfigGP10x = gcConfig;
        ...
        status = lwdaOccMaxBlocksPerSMRegsLimit(&ctaLimitRegsGP10x, &gcConfigGP10x, result, &propertiesGP10x, attributes, blockSize);
        if (status != LWDA_OCC_SUCCESS) {
            return status;
        }
    ...
    int                         blockSize)
{
    ...
    // First compute oclwpancy of potential kernel launch.
    //
    status = lwdaOccMaxActiveBlocksPerMultiprocessor(&result, properties, attributes, state, blockSize, 0);
    if (status != LWDA_OCC_SUCCESS) {
        return status;
    }
    // Check if oclwpancy is achievable given user requested number of blocks. 
    //
    ...
    // Return the per block shared memory limit based on function config.
    //
    status = lwdaOccSMemPerBlock(&smemLimitPerBlock, properties, attributes->shmemLimitConfig, properties->sharedMemPerMultiprocessor);
    if (status != LWDA_OCC_SUCCESS) {
        return status;
    }
    ...
    //
    lwdaOccSMemPerMultiprocessor(&userSmemPreference, properties, state);
    if (numBlocks == 1) {
        ...
        if (!userSmemPreference) {
            ...
            status = lwdaOccAlignUpShmemSizeVoltaPlus(&userSmemPreference, properties);
            if (status != LWDA_OCC_SUCCESS) {
                return status;
            }
        }
    ...
    // Cap computed dynamic SM by user requested limit specified via lwFuncSetAttribute()
    //
