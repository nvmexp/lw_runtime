...
#if !defined(LWRAND_POISSON_H_)
#define LWRAND_POISSON_H_
...
#define CR_LWDART_PI               3.1415926535897931e+0
#define CR_LWDART_TWO_TO_52        4503599627370496.0
...
/* Computes ilwerse of pgammainc */
QUALIFIERS float __cr_pgammaincilw (float a, float y)
{
    float t, alpha, beta;
...
#if defined(__LWDACC_RDC__) && (__cplusplus >= 201703L) && defined(__cpp_inline_variables)
...
QUALIFIERS unsigned int lwrand_poisson_knuth(T *state, float lambda)
{
  unsigned int k = 0;
  float p = expf(lambda);
  do{
      k++;
      p *= lwrand_uniform(state);
  }while (p > 1.0);
  return k-1;
}
...
template <typename T>
// Donald E. Knuth Seminumerical Algorithms. The Art of Computer Programming, Volume 2
QUALIFIERS uint4 lwrand_poisson_knuth4(T *state, float lambda)
{
  uint4 k = {0,0,0,0};
  ...
  do{
      k.x++;
      p.x *= lwrand_uniform(state);
  }while (p.x > 1.0);
  do{
      k.y++;
      p.y *= lwrand_uniform(state);
  }while (p.y > 1.0);
  do{
      k.z++;
      p.z *= lwrand_uniform(state);
  }while (p.z > 1.0);
  do{
      k.w++;
      p.w *= lwrand_uniform(state);
  }while (p.w > 1.0);
...
QUALIFIERS unsigned int _lwrand_M2_double(T x, lwrandDistributionM2Shift_t distributionM2)
{
    double u = _lwrand_uniform_double(x);
    int j = (int) floor(distributionM2->length*u);


#if defined(__LWDA_ARCH__) && (__LWDA_ARCH__ >= 350)
...
template <typename T>
// Marsaglia, Tsang, Wang Journal of Statistical Software, square histogram.
QUALIFIERS uint4 _lwrand_M2_double4(T x, lwrandDistributionM2Shift_t distributionM2)
{
    double4 u;
    uint4 result = {0,0,0,0};
    int4 flag = {1,1,1,1};

    u.x = _lwrand_uniform_double(x.x);
    u.y = _lwrand_uniform_double(x.y);
    u.z = _lwrand_uniform_double(x.z);
    u.w = _lwrand_uniform_double(x.w);
...
QUALIFIERS unsigned int lwrand_M2_double(STATE *state, lwrandDistributionM2Shift_t distributionM2)
{
    return _lwrand_M2_double(lwrand(state), distributionM2);
}

template <typename STATE>
QUALIFIERS uint4 lwrand_M2_double4(STATE *state, lwrandDistributionM2Shift_t distributionM2)
{
    return _lwrand_M2_double4(lwrand4(state), distributionM2);
}


template <typename T>
QUALIFIERS unsigned int _lwrand_binary_search_double(T x, lwrandDistributionShift_t distribution)
{
    double u = _lwrand_uniform_double(x);
    int min = 0;
...
template <typename STATE>
QUALIFIERS unsigned int lwrand_binary_search_double(STATE *state, lwrandDistributionShift_t distribution)
{
    return _lwrand_binary_search_double(lwrand(state), distribution);
}

// Generates uniformly distributed double values in range (0.0; 1.0) from uniformly distributed
// unsigned int. We can't use standard _lwrand_uniform_double since it can generate 1.0.
// This is required only for _lwrand_poisson_ITR_double.
QUALIFIERS double _lwrand_uniform_double_excluding_one(unsigned int x)
{
...
// This is required only for _lwrand_poisson_ITR_double.
QUALIFIERS double _lwrand_uniform_double_excluding_one(unsigned long long x)
{
    return (x >> 11) * LWRAND_2POW53_ILW_DOUBLE + (LWRAND_2POW53_ILW_DOUBLE/4.0);
}

#define MAGIC_DOUBLE_CONST 500.0
template <typename T>
//George S. Fishman Discrete-event simulation: modeling, programming, and analysis
QUALIFIERS unsigned int _lwrand_poisson_ITR_double(T x, double lambda)
{
  double L,p = 1.0;
  double q = 1.0;
  unsigned int k = 0;
  int pow=0;
  // This algorithm requires u to be in (0;1) range, however, _lwrand_uniform_double
  ...
  double u = _lwrand_uniform_double_excluding_one(x);
  do{
...
QUALIFIERS unsigned int lwrand_poisson_gammainc(T state, float lambda){
    float y, x, t, z,v;
    float logl = __cr_log (lambda);
    while (true) {
        y = lwrand_uniform (state);
        x = __cr_pgammaincilw (lambda, y);
        x = floorf (x);
        z = lwrand_uniform (state);
...
template <typename T>
/* Rejection Method for Poisson distribution based on gammainc approximation */
QUALIFIERS uint4 lwrand_poisson_gammainc4(T state, float lambda){
    uint4 result;
    float y, x, t, z,v;
    float logl = __cr_log (lambda);
    while (true) {
...
template <typename T>
QUALIFIERS unsigned int _lwrand_poisson(T x, double lambda)
{
    if (lambda < 1000)
        return _lwrand_poisson_ITR_double(x, lambda);
    return (unsigned int)((sqrt(lambda) * _lwrand_normal_icdf_double(x)) + lambda + 0.5); //Round to nearest
}

template <typename T>
QUALIFIERS unsigned int _lwrand_poisson_from_normal(T x, double lambda)
{
    return (unsigned int)((sqrt(lambda) * _lwrand_normal_icdf(x)) + lambda + 0.5); //Round to nearest
}

template <typename STATE>
QUALIFIERS unsigned int lwrand_poisson_from_normal(STATE state, double lambda)
{
    return (unsigned int)((sqrt(lambda) * lwrand_normal(state)) + lambda + 0.5); //Round to nearest
}

template <typename STATE>
QUALIFIERS uint4 lwrand_poisson_from_normal4(STATE state, double lambda)
{
   uint4 result;
   float4 _res;

   _res = lwrand_normal4(state);
 ...
 */
QUALIFIERS unsigned int lwrand_poisson(lwrandStateXORWOW_t *state, double lambda)
{
    if (lambda < 64)
        return lwrand_poisson_knuth(state, (float)lambda);
    if (lambda > 4000)
        return (unsigned int)((sqrt(lambda) * lwrand_normal_double(state)) + lambda + 0.5); //Round to nearest
    return lwrand_poisson_gammainc(state, (float)lambda);
}
 ...
 * \return Poisson-distributed unsigned int with lambda \p lambda
 */
QUALIFIERS unsigned int lwrand_poisson(lwrandStatePhilox4_32_10_t *state, double lambda)
{
    if (lambda < 64)
        return lwrand_poisson_knuth(state, (float)lambda);
    if (lambda > 4000)
 ...
 * \return Poisson-distributed unsigned int with lambda \p lambda
 */
QUALIFIERS uint4 lwrand_poisson4(lwrandStatePhilox4_32_10_t *state, double lambda)
{
    uint4 result;
    double4 _res;
    if (lambda < 64)
        return lwrand_poisson_knuth4(state, (float)lambda);
    if (lambda > 4000) {
    ...
    return lwrand_poisson_gammainc4(state, (float)lambda);
}
 ...
 * \return Poisson-distributed unsigned int with lambda \p lambda
 */
QUALIFIERS unsigned int lwrand_poisson(lwrandStateMRG32k3a_t *state, double lambda)
{
    if (lambda < 64)
        return lwrand_poisson_knuth(state, (float)lambda);
    if (lambda > 4000)
 ...
 * \return Poisson-distributed unsigned int with lambda \p lambda
 */
QUALIFIERS unsigned int lwrand_poisson(lwrandStateMtgp32_t *state, double lambda)
{
    return _lwrand_poisson(lwrand(state), lambda);
}
...
QUALIFIERS unsigned int lwrand_poisson(lwrandStateSobol32_t *state, double lambda)
{
    return _lwrand_poisson(lwrand(state), lambda);
}
 ...
 * \return Poisson-distributed unsigned int with lambda \p lambda
 */
QUALIFIERS unsigned int lwrand_poisson(lwrandStateScrambledSobol32_t *state, double lambda)
{
    return _lwrand_poisson(lwrand(state), lambda);
}
 ...
 * \return Poisson-distributed unsigned int with lambda \p lambda
 */
QUALIFIERS unsigned int lwrand_poisson(lwrandStateSobol64_t *state, double lambda)
{
    return _lwrand_poisson(lwrand(state), lambda);
}
 ...
 * \return Poisson-distributed unsigned int with lambda \p lambda
 */
QUALIFIERS unsigned int lwrand_poisson(lwrandStateScrambledSobol64_t *state, double lambda)
{
    return _lwrand_poisson(lwrand(state), lambda);
}
#endif // !defined(LWRAND_POISSON_H_)
