 ...
 *      there's no suitable memory lwrrently cached; otherwise, they use memory they have acquired beforehand, to make
 ...
 *      information in memory obtained from \p Upstream; for instance, \p Upstream can be a LWCA non-managed memory
 *      resource, or a LWCA managed memory resource whose memory we would prefer to not migrate back and forth between
 *      host and device when exelwting bookkeeping code.
 *
...
public:
    ...
    static pool_options get_default_options()
    {
        ...
        ret.cached_size_lwtoff_factor = 16;
        ret.cached_alignment_lwtoff_factor = 16;
        ...
        if (bytes > m_options.largest_block_size || alignment > m_options.alignment)
        {
            ...
            if (m_options.cache_oversized && !m_cached_oversized.empty())
            {
                ...
                if (it != m_cached_oversized.end())
                {
                    ...
                    if (size_factor >= m_options.cached_size_lwtoff_factor)
                    {
                ...
                if (it != m_cached_oversized.end())
                {
                    ...
                    if (alignment_factor >= m_options.cached_alignment_lwtoff_factor)
                    {
                        it = m_cached_oversized.end();
                    }
                }
