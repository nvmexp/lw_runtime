...
//   if(T is colwertible to input_host_iterator_tag
//      || T is colwertible to output_host_iterator_tag
//      || T is colwertible to input_device_iterator_tag
//      || T is colwertible to output_device_iterator_tag
//   )
//     return T
//
//   else if (T is not colwertible to incrementable_traversal_tag)
//     the program is ill-formed
//
//   else return a type X satisfying the following two constraints:
//
//     1. X is colwertible to X1, and not to any more-derived
//        type, where X1 is defined by:
//
//        if (T is colwertible to forward_traversal_tag)
//        {
//          if (T is colwertible to random_access_traversal_tag)
//            X1 = random_access_host_iterator_tag
//          else if (T is colwertible to bidirectional_traversal_tag)
...
//        else
//        {
//          if (T is colwertible to single_pass_traversal_tag
//              && R is colwertible to V)
//            X1 = input_host_iterator_tag
//          else
//            X1 = T
//        }
//
//     2. category-to-traversal(X) is colwertible to the most
//        derived traversal tag type to which X is also colwertible,
...
// Thrust does not check is_colwertible<Reference, ValueParam> because Reference
// may not be a complete type at this point, and implementations of is_colwertible
// typically require that both types be complete.
// Instead, it simply assumes that if is_colwertible<Traversal, single_pass_traversal_tag>,
...
template<typename Traversal, typename ValueParam, typename Reference>
  struct iterator_facade_default_category_std :
    thrust::detail::eval_if<
      thrust::detail::is_colwertible<Traversal, thrust::forward_traversal_tag>::value,
      thrust::detail::eval_if<
        thrust::detail::is_colwertible<Traversal, thrust::random_access_traversal_tag>::value,
        thrust::detail::identity_<std::random_access_iterator_tag>,
        thrust::detail::eval_if<
          thrust::detail::is_colwertible<Traversal, thrust::bidirectional_traversal_tag>::value,
      ...
      thrust::detail::eval_if< // XXX note we differ from Boost here
        thrust::detail::is_colwertible<Traversal, thrust::single_pass_traversal_tag>::value,
...
template<typename Traversal, typename ValueParam, typename Reference>
  struct iterator_facade_default_category_device :
    thrust::detail::eval_if<
      ...
      >,
      thrust::detail::eval_if<
        thrust::detail::is_colwertible<Traversal, thrust::single_pass_traversal_tag>::value, // XXX note we differ from Boost here
...
template<typename System, typename Traversal, typename ValueParam, typename Reference>
  struct iterator_facade_default_category
    ...
    : thrust::detail::eval_if<
        thrust::detail::is_colwertible<System, thrust::any_system_tag>::value,
        iterator_facade_default_category_any<Traversal, ValueParam, Reference>,

        // check for host system
        thrust::detail::eval_if<
          thrust::detail::is_colwertible<System, thrust::host_system_tag>::value,
          iterator_facade_default_category_host<Traversal, ValueParam, Reference>,

          // check for device system
          thrust::detail::eval_if<
            thrust::detail::is_colwertible<System, thrust::device_system_tag>::value,
