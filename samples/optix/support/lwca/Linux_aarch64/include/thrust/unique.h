...
/*! For each group of conselwtive elements in the range <tt>[first, last)</tt>
 ...
 *  no two conselwtive elements in the range <tt>[first, new_last)</tt> are
 ...
 *  compact a sequence of numbers to remove conselwtive duplicates using the \p thrust::host exelwtion policy
 *  for parallelization:
 *
 *  \code
...
__host__ __device__
ForwardIterator unique(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
 ...
 *
 *  The following code snippet demonstrates how to use \p unique to
 *  compact a sequence of numbers to remove conselwtive duplicates.
 *
 *  \code
 *  #include <thrust/unique.h>
 *  ...
 ...
 *  \tparam ForwardIterator is a model of <a href="http://www.sgi.com/tech/stl/ForwardIterator.html">Forward Iterator</a>,
 *          and \p ForwardIterator is mutable,
 *          and \p ForwardIterator's \c value_type is colwertible to \p BinaryPredicate's \c first_argument_type and to \p BinaryPredicate's \c second_argument_type.
 ...
 * to a range beginning with \p result, except that in a conselwtive group
 ...
 * are two different definitions of what it means for a conselwtive group of
 ...
 *          and \p InputIterator's \c value_type is colwertible to \c OutputIterator's \c value_type.
 *
 ...
 *  compact a sequence of numbers to remove conselwtive duplicates using the \p thrust::host exelwtion
...
__host__ __device__
OutputIterator unique_copy(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
                           InputIterator first,
                           InputIterator last,
 ...
 *  For each group of conselwtive keys in the range <tt>[keys_first, keys_last)</tt>
 ...
 *  such that no two conselwtive elements in the range <tt>[keys_first, new_keys_last)</tt>
 *  are equal.
 *
 ...
 *  compact a sequence of key/value pairs to remove conselwtive duplicates using the \p thrust::host
...
__host__ __device__
  ...
  unique_by_key(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
 ...
 *
 *  The following code snippet demonstrates how to use \p unique_by_key to
 *  compact a sequence of key/value pairs to remove conselwtive duplicates.
 *
 *  \code
 *  #include <thrust/unique.h>
 *  ...
 ...
 *          and \p InputIterator1's \c value_type is colwertible to \c OutputIterator1's \c value_type.
 *  \tparam OutputIterator2 is a model of <a href="http://www.sgi.com/tech/stl/OutputIterator.html">Output Iterator</a> and
 *          and \p InputIterator2's \c value_type is colwertible to \c OutputIterator2's \c value_type.
 *
 ...
 *  compact a sequence of key/value pairs and with equal keys using the \p thrust::host exelwtion policy
 *  for parallelization:
 *
 *  \code
 *  #include <thrust/unique.h>
...
__host__ __device__
  thrust::pair<OutputIterator1,OutputIterator2>
    unique_by_key_copy(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
 ...
 *
 *  The following code snippet demonstrates how to use \p unique_by_key_copy to
 *  compact a sequence of key/value pairs and with equal keys using the \p thrust::host exelwtion policy for
 *  parallelization:
 *
 *  \code
 *  #include <thrust/unique.h>
