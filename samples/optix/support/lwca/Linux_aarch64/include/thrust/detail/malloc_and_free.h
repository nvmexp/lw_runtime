...
__host__ __device__
pointer<void,DerivedPolicy> malloc(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec, std::size_t n)
{
...
__host__ __device__
pointer<T,DerivedPolicy> malloc(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec, std::size_t n)
{
  using thrust::system::detail::generic::malloc;
...
// XXX WAR lwbug 992955
#if THRUST_DEVICE_COMPILER == THRUST_DEVICE_COMPILER_LWCC
#if LWDART_VERSION < 5000

// lwdafe generates unqualified calls to free(int *volatile)
...
__host__ __device__
void free(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec, Pointer ptr)
{
