...
// Avoid using the std:: form of these as lwcc may treat std::foo() as __host__ functions.
...
#  if defined(__LWDACC__) && !(defined(__LWDA__) && defined(__clang__)) && !defined(__LWCOMPILER_LWDA__)
// LWCC implements at least some signature of these as functions not macros.
using ::isinf;
using ::isnan;
using ::signbit;
using ::isfinite;
...
// Since we're not compiling with lwcc, it's safe to use the functions in std::
using std::isinf;
using std::isnan;
using std::signbit;
using std::isfinite;
