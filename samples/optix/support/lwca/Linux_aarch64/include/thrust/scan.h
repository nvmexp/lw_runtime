 ...
 *                        and \c InputIterator's \c value_type is colwertible to
 ...
 *  prefix sum using the \p thrust::host exelwtion policy for parallelization:
 *
 *  \code
...
__host__ __device__
  OutputIterator inclusive_scan(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
                                InputIterator first,
                                InputIterator last,
 ...
 *                        \c OutputIterator's \c value_type.
 *  \tparam OutputIterator is a model of <a href="http://www.sgi.com/tech/stl/OutputIterator.html">Output Iterator</a>
 *                         and \c OutputIterator's \c value_type is colwertible to
 ...
 *                              colwertible to \c OutputIterator's \c value_type.
 *
 *  \pre \p first may equal \p result but the range <tt>[first, last)</tt> and the range <tt>[result, result + (last - first))</tt> shall not overlap otherwise.
 *
 *  The following code snippet demonstrates how to use \p inclusive_scan to compute an in-place
...
template<typename DerivedPolicy,
         ...
         typename OutputIterator>
__host__ __device__
  OutputIterator exclusive_scan(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
                                InputIterator first,
                                InputIterator last,
                                OutputIterator result);
 ...
 *  \tparam T is colwertible to \c OutputIterator's \c value_type.
 *
 *  \pre \p first may equal \p result but the range <tt>[first, last)</tt> and the range <tt>[result, result + (last - first))</tt> shall not overlap otherwise.
 *
 *  The following code snippet demonstrates how to use \p exclusive_scan to compute an in-place
 ...
 *  predicate used to compare adjacent keys.  Specifically, conselwtive iterators
 ...
 *                         and \c InputIterator2's \c value_type is colwertible to \c OutputIterator's \c value_type.
 *  \tparam OutputIterator is a model of <a href="http://www.sgi.com/tech/stl/OutputIterator.html">Output Iterator</a>,
...
template<typename DerivedPolicy,
         ...
         typename OutputIterator>
__host__ __device__
  OutputIterator inclusive_scan_by_key(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
 ...
 *  \c pred to compare adjacent keys.  Specifically, conselwtive iterators
 *  <tt>i</tt> and <tt>i+1</tt> in the range <tt>[first1, last1)</tt>
...
template<typename DerivedPolicy,
         ...
         typename OutputIterator>
__host__ __device__
  OutputIterator exclusive_scan_by_key(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
                                       InputIterator1 first1,
                                       InputIterator1 last1,
                                       InputIterator2 first2,
                                       OutputIterator result);
 ...
 *  thrust::host exelwtion policy for parallelization:
 *
 *  \code
 *  #include <thrust/scan.h>
 *  #include <thrust/functional.h>
 ...
 *  to compare adjacent keys.  Specifically, conselwtive iterators <tt>i</tt> and
 ...
 *  \tparam BinaryPredicate is a model of <a href="http://www.sgi.com/tech/stl/BinaryPredicate.html">Binary Predicate</a>.
 *  \tparam AssociativeOperator is a model of <a href="http://www.sgi.com/tech/stl/BinaryFunction.html">Binary Function</a>
 *                         and \c AssociativeOperator's \c result_type is colwertible to \c OutputIterator's \c value_type.
 *
 *  \pre \p first1 may equal \p result but the range <tt>[first1, last1)</tt> and the range <tt>[result, result + (last1 - first1))</tt> shall not overlap otherwise.
 *  \pre \p first2 may equal \p result but the range <tt>[first2, first2 + (last1 - first1)</tt> and range <tt>[result, result + (last1 - first1))</tt> shall not overlap otherwise.
 *
