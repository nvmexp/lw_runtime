 ...
 *          and \p T is colwertible to \p ForwardIterator's \c value_type.
 *
...
__host__ __device__
  void replace(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
 ...
 *  a \c device_vector's negative elements with \c 0 using the \p thrust::device exelwtion policy
 *  for parallelization:
 *
 *  \code
 *  #include <thrust/replace.h>
...
__host__ __device__
  void replace_if(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
                  ForwardIterator first, ForwardIterator last,
                  Predicate pred,
                  const T &new_value);
 ...
 *  using the \p thrust::device exelwtion policy for parallelization:
 *
 *  \code
 *  #include <thrust/replace.h>
 *  #include <thrust/device_vector.h>
 ...
 *          and \p T is colwertible to \p OutputIterator's \c value_type.
 *
...
__host__ __device__
  OutputIterator replace_copy(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
...
__host__ __device__
  OutputIterator replace_copy_if(const thrust::detail::exelwtion_policy_base<DerivedPolicy> &exec,
                                 InputIterator first, InputIterator last,
                                 OutputIterator result,
                                 Predicate pred,
                                 const T &new_value);
