...
*/
template<int N>
QUALIFIERS void __lwrand_matvec_inplace(unsigned int *vector, unsigned int *matrix)
{
...
QUALIFIERS void lwrand_MRGmatMul3x3(unsigned int i1[][3],unsigned int i2[][3],unsigned int o[][3],double m)
{
    int i,j;
    double temp[3][3];
    for (i=0; i<3; i++){
...
QUALIFIERS void lwrand_MRGmatVecMul3x3( unsigned int i[][3], unsigned int v[], double m)
{
    int k;
    double t[3];
    for (k = 0; k < 3; k++) {
...
QUALIFIERS void lwrand_MRGmatPow3x3( unsigned int in[][3][3], unsigned int o[][3], double m, unsigned long long pow )
{
    int i,j;
    for ( i = 0; i < 3; i++ ) {
        for ( j = 0; j < 3; j++ ) {
...
QUALIFIERS void _skipahead_inplace(const unsigned long long x, T *state)
{
    ...
    while(p) {
        for(unsigned int t = 0; t < (p & PRECALC_BLOCK_MASK); t++) {
#ifdef __LWDA_ARCH__
            __lwrand_matvec_inplace<N>(state->v, precalc_xorwow_offset_matrix[matrix_num]);
#else
            __lwrand_matvec_inplace<N>(state->v, precalc_xorwow_offset_matrix_host[matrix_num]);
#endif
        }
        p >>= PRECALC_BLOCK_SIZE;
        matrix_num++;
...
#ifdef __LWDA_ARCH__
            __lwrand_matvec_inplace<N>(state->v, precalc_xorwow_matrix[matrix_num]);
#else
            __lwrand_matvec_inplace<N>(state->v, precalc_xorwow_matrix_host[matrix_num]);
#endif
        }
...
{
    _skipahead_inplace<lwrandStateXORWOW_t, 5>(n, state);
}
...
{
    _skipahead_sequence_inplace<lwrandStateXORWOW_t, 5>(n, state);
}
...
QUALIFIERS void _lwrand_init_inplace(unsigned long long seed,
                                     unsigned long long subsequence,
                                     unsigned long long offset,
                                     lwrandStateXORWOW_t *state)
{
    ...
    state->v[3] = 88675123UL ^ t1;
    state->v[4] = 5783321UL + t0;
    _skipahead_sequence_inplace<lwrandStateXORWOW_t, 5>(subsequence, state);
    _skipahead_inplace<lwrandStateXORWOW_t, 5>(offset, state);
    state->boxmuller_flag = 0;
    state->boxmuller_flag_double = 0;
    state->boxmuller_extra = 0.f;
    state->boxmuller_extra_double = 0.;
...
QUALIFIERS void lwrand_init(unsigned long long seed,
                            ...
                            lwrandStateXORWOW_t *state)
{
    _lwrand_init_inplace(seed, subsequence, offset, state);
}
...
#if __LWDA_ARCH__ > 600
QUALIFIERS unsigned long long __lwrand_umad(unsigned int a, unsigned int b, unsigned long long c)
{
...
QUALIFIERS double lwrand_MRG32k3a (lwrandStateMRG32k3a_t *state)
{
    ...
    p1 = __lwrand_umad(a12, state->s1[1], a13n * (unsigned long long)(m1 - state->s1[0]));

    p1 = (p1 & 0xffffffff) + (p1 >> 32) * m1c;
    if (p1 >= m1) p1 -= m1;

    state->s1[0] = state->s1[1]; state->s1[1] = state->s1[2]; state->s1[2] = p1;
    p2 = __lwrand_umad(a21, state->s2[2], a23n * (unsigned long long)(m2 - state->s2[0]));
                            ...
                            lwrandStateMRG32k3a_t *state)
{
    ...
    if (seed != 0ull) {
        ...
        state->s1[0] = (unsigned int)lwrand_MRGmodMul(x1, state->s1[0], MRG32K3A_MOD1);
        state->s1[1] = (unsigned int)lwrand_MRGmodMul(x2, state->s1[1], MRG32K3A_MOD1);
        state->s1[2] = (unsigned int)lwrand_MRGmodMul(x1, state->s1[2], MRG32K3A_MOD1);
        state->s2[0] = (unsigned int)lwrand_MRGmodMul(x2, state->s2[0], MRG32K3A_MOD2);
        state->s2[1] = (unsigned int)lwrand_MRGmodMul(x1, state->s2[1], MRG32K3A_MOD2);
        state->s2[2] = (unsigned int)lwrand_MRGmodMul(x2, state->s2[2], MRG32K3A_MOD2);
    }
...
QUALIFIERS int __lwrand_find_trailing_zero(unsigned int x)
{
#if __LWDA_ARCH__ > 0
    int y = __ffs(~x);
    if(y)
...
QUALIFIERS int __lwrand_find_trailing_zero(unsigned long long x)
{
#if __LWDA_ARCH__ > 0
    int y = __ffsll(~x);
    if(y)
