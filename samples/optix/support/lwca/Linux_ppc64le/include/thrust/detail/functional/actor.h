...
template<typename Action, typename Elw>
  struct apply_actor
{
  typedef typename Action::template result<Elw>::type type;
};
  ...
  struct as_actor
{
  ...
  static inline __host__ __device__ type colwert(const T &x)
  {
    return val(x);
  } // end colwert()
}; // end as_actor
  ...
  struct as_actor<actor<Eval> >
{
  ...
  static inline __host__ __device__ const type &colwert(const actor<Eval> &x)
  {
    return x;
  } // end colwert()
}; // end as_actor
...
{
  return as_actor<T>::colwert(x);
} // end make_actor()

} // end functional

// provide specializations for result_of for nullary, unary, and binary ilwocations of actor
template<typename Eval>
