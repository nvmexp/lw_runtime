...
// Forward declare temporary_array, as it's used by the LWCA copy backend, which
             ...
             >
{
  ...
  public:
    ...
    __host__ __device__
    temporary_array(int uninit, thrust::exelwtion_policy<System> &system, size_type n);
...
// if FromSystem is colwertible to ToSystem, then just make a shallow
      ...
      >::type
{
  ...
  public:
    move_to_system(thrust::exelwtion_policy<FromSystem> &from_system,
                   thrust::exelwtion_policy<ToSystem> &to_system,
                   Iterator first,
                   Iterator last)
