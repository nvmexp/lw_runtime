...
/*! \file thrust/system/cpp/exelwtion_policy.h
 *  \brief Exelwtion policies for Thrust's Standard C++ system.
 */
...
// get the exelwtion policies definitions first
#include <thrust/system/cpp/detail/exelwtion_policy.h>
...
/*! \addtogroup exelwtion_policies
 *  \{
 */


/*! \p thrust::system::cpp::exelwtion_policy is the base class for all Thrust parallel exelwtion
 *  policies which are derived from Thrust's standard C++ backend system.
 */
template<typename DerivedPolicy>
struct exelwtion_policy : thrust::exelwtion_policy<DerivedPolicy>
{};


/*! \p thrust::system::cpp::tag is a type representing Thrust's standard C++ backend system in C++'s type system.
 *  Iterators "tagged" with a type which is colwertible to \p cpp::tag assert that they may be
 *  "dispatched" to algorithm implementations in the \p cpp system.
 */
struct tag : thrust::system::cpp::exelwtion_policy<tag> { unspecified };


/*!
 *  \p thrust::system::cpp::par is the parallel exelwtion policy associated with Thrust's standard
 *  C++ backend system.
 *
 ...
 *  #include <thrust/system/cpp/exelwtion_policy.h>
 *  #include <cstdio>
 *
