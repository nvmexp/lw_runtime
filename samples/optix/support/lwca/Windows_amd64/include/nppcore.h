...
/**
 * Get the current NPP managed LWCA stream context as set by calls to nppSetStream().
 * NPP enables conlwrrent device tasks via an NPP maintained global stream state context.
 * The NPP stream by default is set to stream 0, i.e. non-conlwrrent mode.
 * A user can set the NPP stream to any valid LWCA stream which will update the current NPP managed stream state context 
 * or supply application initialized stream contexts to NPP calls. All LWCA commands
 * issued by NPP (e.g. kernels launched by the NPP library) are then
