/*
 *  Copyright 2020 LWPU Corporation
 *
...
// Proxy reference that ilwokes InputFunction when reading from and
  ...
  class transform_input_output_iterator_proxy
{
  ...
  // std::result_of is deprecated in 2017, replace with std::ilwoke_result
#if THRUST_CPP_DIALECT < 2017
  using Value = typename std::result_of<InputFunction(iterator_value_type)>::type;
#else
  using Value = std::ilwoke_result_t<InputFunction, iterator_value_type>;
#endif
...
#else
      , std::ilwoke_result_t<InputFunction, iterator_value_type>
#endif
      , thrust::use_default
      , thrust::use_default
