...
// std::ilwoke from C++17
// https://stackoverflow.com/questions/38288042/c11-14-ilwoke-workaround
...
template <typename Fn, typename... Args,
          ...
          int = 0>
__host__ __device__
constexpr auto ilwoke(Fn &&f, Args &&... args) noexcept(
...
// std::ilwoke_result from C++17
template <class F, class, class... Us> struct ilwoke_result_impl;

template <class F, class... Us>
struct ilwoke_result_impl<
    F, decltype(detail::ilwoke(std::declval<F>(), std::declval<Us>()...), void()),
    Us...> {
  using type = decltype(detail::ilwoke(std::declval<F>(), std::declval<Us>()...));
};

template <class F, class... Us>
using ilwoke_result = ilwoke_result_impl<F, void, Us...>;

template <class F, class... Us>
using ilwoke_result_t = typename ilwoke_result<F, Us...>::type;
#endif
...
template <class F, class U, class = ilwoke_result_t<F, U>>
using get_map_return = optional<fixup_void<ilwoke_result_t<F, U>>>;

// Check if ilwoking F for some Us returns void
template <class F, class = void, class... U> struct returns_void_impl;
template <class F, class... U>
struct returns_void_impl<F, void_t<ilwoke_result_t<F, U...>>, U...>
    : std::is_void<ilwoke_result_t<F, U...>> {};
template <class F, class... U>
...
using enable_from_other = detail::enable_if_t<
    ...
    !std::is_constructible<T, const optional<U> &&>::value &&
    !std::is_colwertible<optional<U> &, T>::value &&
    !std::is_colwertible<optional<U> &&, T>::value &&
    !std::is_colwertible<const optional<U> &, T>::value &&
    !std::is_colwertible<const optional<U> &&, T>::value>;
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
  ...
  /// object if there is one. \requires `std::ilwoke(std::forward<F>(f),
  /// value())` returns a `std::optional<U>` for some `U`. \returns Let `U` be
  /// the result of `std::ilwoke(std::forward<F>(f), value())`. Returns a
  /// `std::optional<U>`. The return value is empty if `*this` is empty,
  /// otherwise the return value of `std::ilwoke(std::forward<F>(f), value())`
  /// is returned.
  /// \group and_then
  ...
  THRUST_OPTIONAL_CPP11_CONSTEXPR auto and_then(F &&f) & {
    using result = detail::ilwoke_result_t<F, T &>;
    static_assert(detail::is_optional<result>::value,
                  "F must return an optional");

    return has_value() ? detail::ilwoke(std::forward<F>(f), **this)
                       : result(nullopt);
  }
  ...
  THRUST_OPTIONAL_CPP11_CONSTEXPR auto and_then(F &&f) && {
    ...
    return has_value() ? detail::ilwoke(std::forward<F>(f), std::move(**this))
                       : result(nullopt);
  }
  ...
  __host__ __device__
  constexpr auto and_then(F &&f) const & {
    using result = detail::ilwoke_result_t<F, const T &>;
    static_assert(detail::is_optional<result>::value,
                  "F must return an optional");
  ...
  /// \returns Let `U` be the result of `std::ilwoke(std::forward<F>(f),
  /// value())`. Returns a `std::optional<U>`. The return value is empty if
  /// `*this` is empty, otherwise the return value of
  /// `std::ilwoke(std::forward<F>(f), value())` is returned.
  /// \group and_then
  /// \synopsis template <class F>\nconstexpr auto and_then(F &&f) &;
  __thrust_exec_check_disable__
  template <class F>
  __host__ __device__
  THRUST_OPTIONAL_CPP11_CONSTEXPR detail::ilwoke_result_t<F, T &> and_then(F &&f) & {
    using result = detail::ilwoke_result_t<F, T &>;
    static_assert(detail::is_optional<result>::value,
                  "F must return an optional");
  ...
  template <class F>
  __host__ __device__
  constexpr detail::ilwoke_result_t<F, const T &> and_then(F &&f) const & {
    using result = detail::ilwoke_result_t<F, const T &>;
    static_assert(detail::is_optional<result>::value,
                  "F must return an optional");
  ...
  /// return value of `std::ilwoke(std::forward<F>(f), value())` and is
  /// returned.
  ///
  /// \group map
  ...
  /// \requires `std::ilwoke_result_t<F>` must be void or colwertible to
  /// `optional<T>`.
  ...
  template <class F, class U>
  __host__ __device__
  detail::ilwoke_result_t<U> map_or_else(F &&f, U &&u) & {
    return has_value() ? detail::ilwoke(std::forward<F>(f), **this)
                       : std::forward<U>(u)();
  }
  ...
  template <class F, class U>
  __host__ __device__
  detail::ilwoke_result_t<U> map_or_else(F &&f, U &&u) const & {
    return has_value() ? detail::ilwoke(std::forward<F>(f), **this)
                       : std::forward<U>(u)();
  }
  ...
  template <
      class U = T,
      detail::enable_if_t<std::is_colwertible<U &&, T>::value> * = nullptr,
  ...
  /// Colwerting copy constructor.
  /// \synopsis template <class U> optional(const optional<U> &rhs);
  __thrust_exec_check_disable__
  template <
      class U, detail::enable_from_other<T, U, const U &> * = nullptr,
      detail::enable_if_t<std::is_colwertible<const U &, T>::value> * = nullptr>
  __host__ __device__
  ...
  template <class U, detail::enable_from_other<T, U, const U &> * = nullptr,
            detail::enable_if_t<!std::is_colwertible<const U &, T>::value> * =
                nullptr>
  __host__ __device__
  ...
  /// Colwerting copy assignment operator.
  ///
  /// Copies the value from `rhs` if there is one. Otherwise resets the stored
  /// value in `*this`.
  ...
  /// Colwerting move assignment operator.
  ///
  /// Moves the value from `rhs` if there is one. Otherwise resets the stored
  /// value in `*this`.
  ...
  constexpr T value_or(U &&u) const & {
    static_assert(std::is_copy_constructible<T>::value &&
                      std::is_colwertible<U &&, T>::value,
                  "T must be copy constructible and colwertible from U");
  ...
  THRUST_OPTIONAL_CPP11_CONSTEXPR T value_or(U &&u) && {
    static_assert(std::is_move_constructible<T>::value &&
                      std::is_colwertible<U &&, T>::value,
                  "T must be move constructible and colwertible from U");
    return has_value() ? **this : static_cast<T>(std::forward<U>(u));
  }
...
/// \synopsis template <class T>\lwoid swap(optional<T> &lhs, optional<T> &rhs);
__thrust_exec_check_disable__
template <class T,
...
template <class Opt, class F,
          class Ret = decltype(detail::ilwoke(std::declval<F>(),
                                              *std::declval<Opt>())),
          detail::enable_if_t<!std::is_void<Ret>::value> * = nullptr>
__host__ __device__
constexpr auto optional_map_impl(Opt &&opt, F &&f) {
  return opt.has_value()
             ? detail::ilwoke(std::forward<F>(f), *std::forward<Opt>(opt))
