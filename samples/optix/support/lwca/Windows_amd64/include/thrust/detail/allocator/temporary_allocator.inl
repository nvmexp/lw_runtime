...
#if (defined(__LWCOMPILER_LWDA__) || defined(__LWDA_ARCH__)) && \
    THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_LWDA
#include <thrust/system/lwca/detail/terminate.h>
#endif
      ...
      ::allocate(typename temporary_allocator<T,System>::size_type cnt)
{
  ...
  if(result.second < cnt)
  {
    ...
    if (THRUST_IS_HOST_CODE) {
      ...
      #if THRUST_INCLUDE_DEVICE_CODE && THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_LWDA
        thrust::system::lwca::detail::terminate_with_message("temporary_buffer::allocate: get_temporary_buffer failed");
      #endif
    }
  } // end if
