...
// and uses the non-standard math function erfcilw
template<typename RealType>
  class normal_distribution_lwcc
{
  protected:
    ...
    RealType sample(UniformRandomNumberGenerator &urng, const RealType mean, const RealType stddev)
    {
      ...
      // Ensure the colwersion to float will give a value in the range [0,0.5)
      if(u > (urng_range / 2))
      {
        u = urng_range - u;
        S3 = -S3;
      ...
      // Apply ilwerse error function
      return mean + stddev * S3 * erfcilw(2 * p);
    }
...
{
#if THRUST_DEVICE_COMPILER == THRUST_DEVICE_COMPILER_LWCC && !defined(__LWCOMPILER_LWDA__)
  typedef normal_distribution_lwcc<RealType> type;
#else
