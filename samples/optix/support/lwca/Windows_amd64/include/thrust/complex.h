/*
 *  Copyright 2008-2019 LWPU Corporation
...
struct complex_storage;
   ...
   *  <tt>std::complex</tt> lwrrently cannot.
   *
...
public:
   ...
   *  colwertible to this \p complex's \c value_type.
   *
  ...
  /*! This colwerting copy constructor copies from a \p complex with a type
   *  that is colwertible to this \p complex's \c value_type.
   *
   *  \param z The \p complex to copy from.
   *
   *  \tparam U is colwertible to \c value_type.
   */
  template <typename U>
  __host__ __device__
  complex(const complex<U>& z);

  /*! This colwerting copy constructor copies from a <tt>std::complex</tt> with
   *  a type that is colwertible to this \p complex's \c value_type.
   *
   *  \param z The \p complex to copy from.
   */
 ...
 *
 *  \param z The \p complex from which to callwlate the absolute value.
 */
template<typename T>
__host__ __device__
 ...
 *
 *  \param z The \p complex from which to callwlate the phase angle.
 */
template <typename T>
__host__ __device__
 ...
 *
 *  \param z The \p complex from which to callwlate the norm.
 */
template <typename T>
__host__ __device__
 ...
 *
 *  \param z The \p complex from which to callwlate the complex conjugate.
 */
template <typename T>
__host__ __device__
...
/*! Unary minus, returns the additive ilwerse (negation) of its \p complex
 * argument.
 *
 *  \param y The \p complex argument.
 */
...
/* --- Ilwerse Trigonometric Functions --- */
...
/* --- Ilwerse Hyperbolic Functions --- */

/*! Returns the complex ilwerse hyperbolic cosine of a \p complex number.
 *
...
/*! Returns the complex ilwerse hyperbolic sine of a \p complex number.
 *
...
/*! Returns the complex ilwerse hyperbolic tangent of a \p complex number.
 *
 *  The range of the real part of the result is [-inf, +inf] and
 *  the range of the imaginary part is [-Pi/2, Pi/2]
 *
 ...
 * - (real, imaginary)
 *
 * The values read must be colwertible to the \p complex's \c value_type
 *
