...
struct marker_deleter final
{
  ...
  {
    if (nullptr != e)
      thrust::lwca_cub::throw_on_error(lwcaEventDestroy(e));
  }
};
...
public:
    ...
    : handle_(nullptr, marker_deleter())
  {
    ...
    thrust::lwca_cub::throw_on_error(
      lwcaEventCreateWithFlags(&e, lwcaEventDisableTiming)
    );
    handle_.reset(e);
  }
  ...
  bool ready() const
  {
    lwcaError_t const err = lwcaEventQuery(handle_.get());

    if (lwcaErrorNotReady == err)
      return false;

    // Throw on any other error.
    thrust::lwca_cub::throw_on_error(err);
  ...
  void wait() const
  {
    thrust::lwca_cub::throw_on_error(lwcaEventSynchronize(handle_.get()));
  }
...
struct stream_deleter final
{
  ...
  {
    if (nullptr != s)
      thrust::lwca_cub::throw_on_error(lwcaStreamDestroy(s));
  }
};
...
public:
    ...
    : handle_(nullptr, stream_conditional_deleter())
  {
    ...
    thrust::lwca_cub::throw_on_error(
      lwcaStreamCreateWithFlags(&s, lwcaStreamNonBlocking)
    );
    handle_.reset(s);
  }
  ...
  bool ready() const
  {
    lwcaError_t const err = lwcaStreamQuery(handle_.get());
  ...
  {
    thrust::lwca_cub::throw_on_error(
      lwcaStreamSynchronize(handle_.get())
    );
  }
  ...
  {
    thrust::lwca_cub::throw_on_error(
      lwcaStreamWaitEvent(handle_.get(), e.get(), 0)
    );
  }
  ...
  {
    thrust::lwca_cub::throw_on_error(lwcaEventRecord(e.get(), handle_.get()));
  }
...
public:
  ...
  friend __host__
  unique_eager_future_promise_pair<X, XPointer>
  thrust::system::lwca::detail::make_dependent_future(
...
public:
  ...
  {
    thrust::lwca_cub::throw_on_error(lwcaGetDevice(&device_));
  }
  ...
  thrust::system::lwca::detail::try_acquire_stream(
  ...
  friend __host__
  unique_eager_event
  thrust::system::lwca::detail::make_dependent_event(
    std::tuple<Dependencies...>&& deps
  );
};
