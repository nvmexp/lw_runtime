...
#include <thrust/type_traits/is_exelwtion_policy.h>
...
async_reduce(
  thrust::exelwtion_policy<DerivedPolicy>&, ForwardIt, Sentinel, T, BinaryOp
)
{
...
struct reduce_fn final
{
  ...
  __host__
  static auto call(
    thrust::detail::exelwtion_policy_base<DerivedPolicy> const& exec
  ...
  // if T1 is an exelwtion_policy by using SFINAE. Switching to a static
  ...
  THRUST_RETURNS(
    reduce_fn::call3(THRUST_FWD(t1), THRUST_FWD(t2), THRUST_FWD(t3),
                     thrust::is_exelwtion_policy<thrust::remove_cvref_t<T1>>{})
  )
...
event<DerivedPolicy>
async_reduce_into(
  thrust::exelwtion_policy<DerivedPolicy>&
...
struct reduce_into_fn final
{
  ...
  THRUST_RETURNS(
    reduce_into_fn::call5(
      THRUST_FWD(t1), THRUST_FWD(t2), THRUST_FWD(t3), THRUST_FWD(t4),
      THRUST_FWD(t5), thrust::is_exelwtion_policy<thrust::remove_cvref_t<T1>>{})
  )
