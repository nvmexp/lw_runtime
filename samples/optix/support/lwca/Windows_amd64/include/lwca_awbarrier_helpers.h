...
#  define _LWDA_AWBARRIER_ARCH_700_OR_LATER
# endif

# define _LWDA_AWBARRIER_MAX_COUNT ((1 << 14) - 1)

# if (__LWDA_ARCH__ >= 800)
#  define _LWDA_AWBARRIER_HAS_HW_MBARRIER 1
# else
#  define _LWDA_AWBARRIER_HAS_HW_MBARRIER 0
# endif
...
#  define _LWDA_AWBARRIER_ASM_PTR_CONSTRAINT "r"
# else
#  define _LWDA_AWBARRIER_ASM_PTR_CONSTRAINT "l"
# endif
...
void awbarrier_init(uint64_t* barrier, uint32_t expected_count)
{
    ImplementationChooser<_LWDA_AWBARRIER_HAS_HW_MBARRIER>::awbarrier_init(barrier, expected_count);
}

_LWDA_AWBARRIER_QUALIFIER
void awbarrier_ilwal(uint64_t* barrier)
{
    ImplementationChooser<_LWDA_AWBARRIER_HAS_HW_MBARRIER>::awbarrier_ilwal(barrier);
}

_LWDA_AWBARRIER_QUALIFIER
uint32_t awbarrier_token_pending_count(uint64_t token)
{
    return ImplementationChooser<_LWDA_AWBARRIER_HAS_HW_MBARRIER>::awbarrier_token_pending_count(token);
}
...
{
    return ImplementationChooser<_LWDA_AWBARRIER_HAS_HW_MBARRIER>::awbarrier_arrive_drop_no_complete<Drop>(barrier, arrive_count);
}
...
uint64_t awbarrier_arrive_drop(uint64_t* barrier)
{
    return ImplementationChooser<_LWDA_AWBARRIER_HAS_HW_MBARRIER>::awbarrier_arrive_drop<Drop>(barrier);
}

_LWDA_AWBARRIER_QUALIFIER
bool awbarrier_test_wait(uint64_t* barrier, uint64_t token)
{
    return ImplementationChooser<_LWDA_AWBARRIER_HAS_HW_MBARRIER>::awbarrier_test_wait(barrier, token);
}

_LWDA_AWBARRIER_END_INTERNAL_NAMESPACE

# endif /* !_LWDA_AWBARRIER_ARCH_700_OR_LATER */
