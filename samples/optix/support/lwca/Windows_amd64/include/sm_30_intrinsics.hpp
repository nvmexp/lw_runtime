...
unsigned __fns(unsigned mask, unsigned base, int offset) {
  extern __device__ __device_builtin__ unsigned int __lwvm_fns(unsigned int mask, unsigned int base, int offset);
  return __lwvm_fns(mask, base, offset);
}

__SM_30_INTRINSICS_DECL__ 
void  __barrier_sync(unsigned id) {
  extern __device__ __device_builtin__ void __lwvm_barrier_sync(unsigned id);
  return __lwvm_barrier_sync(id);
}

__SM_30_INTRINSICS_DECL__ 
void  __barrier_sync_count(unsigned id, unsigned cnt) {
  extern __device__ __device_builtin__ void __lwvm_barrier_sync_cnt(unsigned id, unsigned cnt);
  return __lwvm_barrier_sync_cnt(id, cnt);
}

__SM_30_INTRINSICS_DECL__ 
void  __syncwarp(unsigned mask) {
  extern __device__ __device_builtin__ void __lwvm_bar_warp_sync(unsigned mask);
  return __lwvm_bar_warp_sync(mask);
}

__SM_30_INTRINSICS_DECL__ 
int __all_sync(unsigned mask, int pred) {
  extern __device__ __device_builtin__ int __lwvm_vote_all_sync(unsigned int mask, int pred); 
  return __lwvm_vote_all_sync(mask, pred);
}

__SM_30_INTRINSICS_DECL__ 
int __any_sync(unsigned mask, int pred) {
  extern __device__ __device_builtin__ int __lwvm_vote_any_sync(unsigned int mask, int pred); 
  return __lwvm_vote_any_sync(mask, pred);
}

__SM_30_INTRINSICS_DECL__ 
int __uni_sync(unsigned mask, int pred) {
  extern __device__ __device_builtin__ int __lwvm_vote_uni_sync(unsigned int mask, int pred); 
  return __lwvm_vote_uni_sync(mask, pred);
}

__SM_30_INTRINSICS_DECL__ 
unsigned __ballot_sync(unsigned mask, int pred) {
  extern __device__ __device_builtin__ unsigned int __lwvm_vote_ballot_sync(unsigned int mask, int pred); 
  return __lwvm_vote_ballot_sync(mask, pred);
}
...
__SM_30_INTRINSICS_DECL__ int __shfl_sync(unsigned mask, int var, int srcLane, int width) {
        extern __device__ __device_builtin__ unsigned __lwvm_shfl_idx_sync(unsigned mask, unsigned a, unsigned b, unsigned c);
	int ret;
	int c = ((warpSize-width) << 8) | 0x1f;
        ret = __lwvm_shfl_idx_sync(mask, var, srcLane, c);
	return ret;
}
...
__SM_30_INTRINSICS_DECL__ int __shfl_up_sync(unsigned mask, int var, unsigned int delta, int width) {
        extern __device__ __device_builtin__ unsigned __lwvm_shfl_up_sync(unsigned mask, unsigned a, unsigned b, unsigned c);
	int ret;
	int c = (warpSize-width) << 8;
        ret = __lwvm_shfl_up_sync(mask, var, delta, c);
	return ret;
}
...
__SM_30_INTRINSICS_DECL__ int __shfl_down_sync(unsigned mask, int var, unsigned int delta, int width) {
        extern __device__ __device_builtin__ unsigned __lwvm_shfl_down_sync(unsigned mask, unsigned a, unsigned b, unsigned c);
	int ret;
	int c = ((warpSize-width) << 8) | 0x1f;
        ret = __lwvm_shfl_down_sync(mask, var, delta, c);
	return ret;
}
...
__SM_30_INTRINSICS_DECL__ int __shfl_xor_sync(unsigned mask, int var, int laneMask, int width) {
        extern __device__ __device_builtin__ unsigned __lwvm_shfl_bfly_sync(unsigned mask, unsigned a, unsigned b, unsigned c);
	int ret;
	int c = ((warpSize-width) << 8) | 0x1f;
        ret = __lwvm_shfl_bfly_sync(mask, var, laneMask, c);
	return ret;
}
...
__SM_30_INTRINSICS_DECL__ float __shfl_sync(unsigned mask, float var, int srcLane, int width) {
        ...
        int c;
	c = ((warpSize-width) << 8) | 0x1f;
        ret = __lwvm_shfl_idx_sync(mask, __float_as_int(var), srcLane, c);
...
__SM_30_INTRINSICS_DECL__ float __shfl_up_sync(unsigned mask, float var, unsigned int delta, int width) {
	...
	int ret;
        int c;
	c = (warpSize-width) << 8;
        ret = __lwvm_shfl_up_sync(mask, __float_as_int(var), delta, c);
	return __int_as_float(ret);
}
...
__SM_30_INTRINSICS_DECL__ float __shfl_down_sync(unsigned mask, float var, unsigned int delta, int width) {
	...
	int ret;
        int c;
	c = ((warpSize-width) << 8) | 0x1f;
        ret = __lwvm_shfl_down_sync(mask, __float_as_int(var), delta, c);
	return __int_as_float(ret);
}
...
__SM_30_INTRINSICS_DECL__ float __shfl_xor_sync(unsigned mask, float var, int laneMask, int width) {
	...
	int ret;
        int c;
	c = ((warpSize-width) << 8) | 0x1f;
        ret = __lwvm_shfl_bfly_sync(mask, __float_as_int(var), laneMask, c);
	return __int_as_float(ret);
}
