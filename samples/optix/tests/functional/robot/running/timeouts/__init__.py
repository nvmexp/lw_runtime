...
class _Timeout(object):
    ...
    def run(self, runnable, args=None, kwargs=None):
        ...
        exelwtable = lambda: runnable(*(args or ()), **(kwargs or {}))
        return Timeout(timeout, self._timeout_error).execute(exelwtable)
...
class TestTimeout(_Timeout):
    type = 'Test'
    _keyword_timeout_oclwrred = False

    def set_keyword_timeout(self, timeout_oclwrred):
        if timeout_oclwrred:
            self._keyword_timeout_oclwrred = True

    def any_timeout_oclwrred(self):
        return self.timed_out() or self._keyword_timeout_oclwrred
