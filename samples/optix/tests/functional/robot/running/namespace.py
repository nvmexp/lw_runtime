    ...
    def _handle_imports(self, import_settings):
        for item in import_settings:
            try:
                ...
                item.report_ilwalid_syntax(unicode(err))
...
class KeywordStore(object):
    ...
    def _filter_stdlib_handler(self, handler1, handler2):
        ...
        if not RUN_KW_REGISTER.is_run_keyword(custom.library.orig_name, custom.name):
            self._lwstom_and_standard_keyword_conflict_warning(custom, standard)
        return [custom]

    def _lwstom_and_standard_keyword_conflict_warning(self, custom, standard):
        lwstom_with_name = standard_with_name = ''
        if custom.library.name != custom.library.orig_name:
            lwstom_with_name = " imported as '%s'" % custom.library.name
        ...
        warning = Message("Keyword '%s' found both from a custom test library "
                          ...
                          % (standard.name,
                             custom.library.orig_name, lwstom_with_name,
...
class _VariableScopes:
    ...
    def set_global(self, name, value):
        ...
        for ns in EXELWTION_CONTEXTS.namespaces:
