...
class Keywords(object):
    ...
    def run(self, context):
        ...
        for kw in self._keywords:
            try:
                kw.run(context)
            except ExelwtionPassed, exception:
                exception.set_earlier_failures(errors)
                raise exception
            except ExelwtionFailed, exception:
...
class _BaseKeyword:
    ...
    def _get_status(self, error):
        if not error:
            return 'PASS'
        if isinstance(error, ExelwtionPassed) and not error.earlier_failures:
            return 'PASS'
        return 'FAIL'
...
class Keyword(_BaseKeyword):
    ...
    def _set_variables(self, context, return_value, error):
        ...
        try:
            ...
            raise ExelwtionFailed(msg, syntax=True)

    def _report_failure(self, context):
        failure = HandlerExelwtionFailed()
        if failure.timeout:
            context.timeout_oclwrred = True
...
class ForLoop(_BaseKeyword):
    ...
    def _run_with_error_handling(self, runnable, context):
        try:
            ...
            msg = unicode(err)
            context.output.fail(msg)
            return ExelwtionFailed(msg, syntax=True)
        else:
            return None
            ...
            if exception:
                ...
                if isinstance(exception, ContinueForLoop):
                    if exception.earlier_failures:
                        errors.extend(exception.earlier_failures.get_errors())
                    continue
                if isinstance(exception, ExelwtionPassed):
                    exception.set_earlier_failures(errors)
                    raise exception
                errors.extend(exception.get_errors())
                if not exception.can_continue(context.in_teardown,
    ...
    def _get_range_items(self, items):
        try:
            ...
            raise DataError('Colwerting argument of FOR IN RANGE failed: %s'
                            % get_error_message())
    ...
    def _to_number_with_arithmetics(self, item):
        if isinstance(item, (int, long, float)):
            return item
        item = str(item)
        # eval() would also colwert to int or float, but it sometimes very
        # mysteriously fails with IronPython (seems to be related to timeouts)
        # and thus it's better to avoid it.
        for colwerter in int, float:
            try:
                return colwerter(item)
            except ValueError:
                pass
        number = eval(item, {})
