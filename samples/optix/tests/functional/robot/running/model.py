...
class TestSuite(model.TestSuite):
    ...
    def randomize(self, suites=True, tests=True, seed=None):
        """Randomizes the order of suites and/or tests, relwrsively."""
        self.visit(Randomizer(suites, tests, seed))

    def run(self, settings=None, **options):
        """Exelwtes the suite based based the given ``settings`` or ``options``.

        :param settings: :class:`~robot.conf.settings.RobotSettings` object
            to configure test exelwtion.
        :param options: Used to construct new
            :class:`~robot.conf.settings.RobotSettings` object if ``settings``
            are not given.
        :return: :class:`~robot.result.exelwtionresult.Result` object with
            information about exelwted suites and tests.
        ...
        Only options related to the actual test exelwtion have an effect.
        ...
        as part of the exelwtion can be configured, though. This includes
        ...
        :class:`~robot.result.exelwtionresult.Result` object does not
        have any information about the exelwted keywords. If that information
        is needed, the created output XML file needs to be read  using the
        :class:`~robot.result.resultbuilder.ExelwtionResult` factory method.

        See the :mod:`package level <robot.running>` documentation for
        more examples, including how to construct exelwtable test suites and
        how to create logs and reports based on the exelwtion results.

        See the :func:`robot.run <robot.run.run>` function for a higher-level
        API for exelwting tests in files or directories.
        """
        if not settings:
...
class Variable(object):
    ...
    def report_ilwalid_syntax(self, message, level='ERROR'):
