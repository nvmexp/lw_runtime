...
class SuiteRunErrors(object):
    _NO_ERROR = None
    _exit_on_failure_error = ('Critical failure oclwrred and ExitOnFailure '
                              'option is in use')
    _exit_on_fatal_error = 'Test exelwtion is stopped due to a fatal error'
    ...
    def __init__(self, run_mode_is_exit_on_failure=False, run_mode_skip_teardowns_on_exit=False):
        ...
        self._earlier_suite_setup_exelwtions = []
        self._init_lwrrent_errors()
        self._exit_runmode = self._exit_fatal = False
        self._lwrrent_suite_setup_exelwted = False

    @property
    def exit(self):
        return self._exit_fatal or self._exit_runmode

    def _init_lwrrent_errors(self):
        self._lwrrent_init_err = self._lwrrent_setup_err = self._NO_ERROR

    def start_suite(self):
        self._earlier_init_errors.append(self._lwrrent_init_err)
        self._earlier_setup_errors.append(self._lwrrent_setup_err)
        self._earlier_suite_setup_exelwtions.append(self._lwrrent_suite_setup_exelwted)
        self._init_lwrrent_errors()
        self._lwrrent_suite_setup_exelwted = False

    def end_suite(self):
        self._lwrrent_setup_err = self._earlier_setup_errors.pop()
        self._lwrrent_init_err = self._earlier_init_errors.pop()
        self._lwrrent_suite_setup_exelwted = self._earlier_suite_setup_exelwtions.pop()

    def is_suite_setup_allowed(self):
        return self._lwrrent_init_err is self._NO_ERROR and \
                not self._earlier_errors()

    def is_suite_teardown_allowed(self):
        if not self.is_test_teardown_allowed():
            return False
        if self._lwrrent_suite_setup_exelwted:
            return True
        return self._lwrrent_init_err is self._NO_ERROR and \
                not self._earlier_errors()
    ...
    def suite_init_err(self, error_message):
        self._lwrrent_init_err = error_message

    def setup_exelwted(self):
        self._lwrrent_suite_setup_exelwted = True

    def suite_setup_err(self, err):
        if err.exit:
            self._exit_fatal = True
        self._lwrrent_setup_err = unicode(err)

    def suite_error(self):
        if self._earlier_init_erros_oclwrred():
            return self._parent_suite_init_error
        if self._earlier_setup_errors_oclwrred():
            return self._parent_suite_setup_error
        if self._lwrrent_init_err:
            return self._lwrrent_init_err
        if self._lwrrent_setup_err:
            return 'Suite setup failed:\n%s' % self._lwrrent_setup_err
        return ''

    def _earlier_init_erros_oclwrred(self):
        return any(self._earlier_init_errors)

    def _earlier_setup_errors_oclwrred(self):
        return any(self._earlier_setup_errors)

    def child_error(self):
        if self._lwrrent_init_err or self._earlier_init_erros_oclwrred():
            return self._parent_suite_init_error
        if self._lwrrent_setup_err or self._earlier_setup_errors_oclwrred():
            return self._parent_suite_setup_error
