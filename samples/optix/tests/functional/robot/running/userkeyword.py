...
from robot.errors import (DataError, ExelwtionFailed, ExelwtionPassed,
                          PassExelwtion, ReturnFromKeyword,
                          UserKeywordExelwtionFailed)
...
class UserKeywordHandler(object):
    ...
    def _dry_run(self, context, arguments):
        ...
        error, return_ = self._exelwte(context, arguments)
        if error:
            raise error
        return None
    ...
    def _exelwte(self, context, arguments):
        ...
        if error or td_error:
            error = UserKeywordExelwtionFailed(error, td_error)
    ...
    def _run_teardown(self, context):
        ...
        try:
            ...
            return ExelwtionFailed(unicode(err), syntax=True)
        if name.upper() in ('', 'NONE'):
            return None
        kw = Keyword(name, self.teardown.args, type='teardown')
        try:
            kw.run(context)
        except PassExelwtion:
            return None
...
class EmbeddedArgsTemplate(UserKeywordHandler):
    ...
    def _get_regexp_pattern(self, variable):
        ...
        return variable, self._format_lwstom_regexp(pattern)

    def _format_lwstom_regexp(self, pattern):
        for formatter in (self._regexp_extensions_are_not_allowed,
                          self._make_groups_non_capturing,
                          self._unescape_closing_lwrly,
    ...
    def _unescape_closing_lwrly(self, pattern):
