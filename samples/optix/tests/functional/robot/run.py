...
"""Module implementing the command line entry point for exelwting tests.
...
=======
 ...
 -N --name name           Set the name of the top level test suite. Underscores
                          in the name are colwerted to spaces. Default name is
                          created from the name of the exelwted data source.
 -D --doc documentation   Set the documentation of the top level test suite.
                          Underscores in the documentation are colwerted to
                          spaces and it may also contain simple HTML formatting
                          (e.g. *bold* and http://url/).
 -M --metadata name:value *  Set metadata of the top level suite. Underscores
                          in the name and value are colwerted to spaces. Value
                          can contain same HTML formatting as --doc.
                          Example: --metadata version:1.2
 -G --settag tag *        Sets given tag(s) to all exelwted test cases.
 ...
 -R --rerunfailed output  Select failed tests from an earlier output file to be
                          re-exelwted. Equivalent to selecting same tests
 ...
 -o --output file         XML output file. Given path, similarly as paths given
                          ...
                          output files after the test exelwtion and XML outputs
 ...
 -b --debugfile file      Debug file written during exelwtion. Not created
 ...
 -T --timestampoutputs    When this option is used, timestamp in a format
    ...
    --logtitle title      Title for the generated test log. The default title
                          ...
                          the title are colwerted into spaces in all titles.
 ...
 -L --loglevel level      Threshold level for logging. Available levels: TRACE,
    ...
    --tagdoc pattern:doc *  Add documentation to tags matching given pattern.
                          ...
                          Examples: --tagdoc mytag:My_dolwmentation
    ...
    --tagstatlink pattern:link:title *  Add external links into `Statistics by
                          ...
                          automatically colwerted to spaces.
    ...
    --listener class *    A class for monitoring test exelwtion. Gets
    ...
    --runemptysuite       Exelwtes tests also if the top level test suite is
                          empty. Useful e.g. with --include/--exclude when it
                          is not an error that no test matches the condition.
    --dryrun              Verifies test data and runs tests so that library
                          keywords are not exelwted.
    --exitonfailure       Stops test exelwtion if any critical test fails.
    --exitonerror         Stops test exelwtion if any error oclwrs when parsing
                          test data, importing libraries, and so on.
    --skipteardownonexit  Causes teardowns to be skipped if test exelwtion is
                          stopped prematurely.
    --randomize all|suites|tests|none  Randomizes the test exelwtion order.
...
# Exelwting `robot.run` module using Python.
...
# Exelwting multiple test case files and using case-insensitive long options.
...
class RobotFramework(Application):
    ...
    def __init__(self):
        Application.__init__(self, USAGE, arg_limits=(1,),
                             elw_options='ROBOT_OPTIONS', logger=LOGGER)
    ...
    def main(self, datasources, **options):
        ...
        with pyloggingconf.robot_handler_enabled(settings.log_level):
            ...
            LOGGER.info("Tests exelwtion ended. Statistics:\n%s"
...
def run_cli(arguments):
    """Command line exelwtion entry point for running tests.
    ...
    """
    RobotFramework().exelwte_cli(arguments)


def run(*datasources, **options):
    """Exelwtes given Robot Framework data sources with given options.
