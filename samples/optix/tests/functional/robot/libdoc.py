...
=======
 ...
 -n --name newname        Sets the name of the dolwmented library or resource.
 -v --version newversion  Sets the version of the dolwmented library or
                          resource.
...
Examples:
  ...
  python -m robot.libdoc Dialogs show PauseExelwtion execute*
...
Jython and IronPython). In the examples above Libdoc is exelwted as an
installed module, but it can also be exelwted as a script like
...
from robot.libdocpkg import LibraryDolwmentation, ConsoleViewer
...
class LibDoc(Application):
    ...
    def main(self, args, name='', version='', format=None, docformat=None):
        ...
        libdoc = LibraryDolwmentation(lib_or_res, name, version,
...
def libdoc_cli(arguments):
    """Exelwtes Libdoc similarly as from the command line.
