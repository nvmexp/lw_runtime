...
class _BaseSettings(object):
    ...
    def _process_value(self, name, value):
        ...
        if name in ['SuiteStatLevel', 'MonitorWidth']:
            return self._colwert_to_positive_integer_or_default(name, value)
    ...
    def _process_randomize_value(self, original):
        ...
        if value not in ('tests', 'suites', 'none', 'all'):
            self._raise_ilwalid_option_value('--randomize', original)
        try:
            seed = int(seed)
        except ValueError:
            self._raise_ilwalid_option_value('--randomize', original)
    ...
    def _raise_ilwalid_option_value(self, option_name, given_value):
    ...
    def _process_runmode_value(self, original_value):
        ...
        if formatted_value not in ('exitonfailure', 'skipteardownonexit',
                                   ...
                                   'random:all'):
            self._raise_ilwalid_option_value('--runmode', original_value)
    ...
    def _colwert_to_positive_integer_or_default(self, name, value):
        value = self._colwert_to_integer(name, value)
        return value if value > 0 else self._get_default_value(name)

    def _colwert_to_integer(self, name, value):
        try:
            return int(value)
        except ValueError:
