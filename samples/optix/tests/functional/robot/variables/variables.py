...
class Variables(utils.NormalizedDict):
    ...
    def _raise_non_existing_variable(self, name, msg=None, elw_vars=False):
        _raise_not_found(name, self.keys(), msg, elw_vars=elw_vars)
        ...
        for var in variables:
            ...
            try:
                name, value = self._get_var_table_name_and_value(
                    var.name, var.value, var.report_ilwalid_syntax)
                if overwrite or not self.contains(name):
                    self.set(name, value)
            except DataError, err:
                var.report_ilwalid_syntax(err)
...
class DelayedVariable(object):
    ...
    @property
    @contextmanager
    def _avoid_relwrsion(self):
        if self._resolving:
            raise DataError('Relwrsive variable definition.')
...
def _raise_not_found(name, candidates, msg=None, elw_vars=False):
