...
class HtmlFormatter(_DataFileFormatter):
    ...
    def _format_row(self, row, table=None):
        row = self._pad(self._escape_conselwtive_whitespace(row), table)
        if self._is_dolwmentation_row(row):
            return self._create_dolwmentation_row(row)
        first_cell = self._create_first_cell(row[0], table)
        if self._is_indented_dolwmentation_row(row[1:], table):
            return self._create_indented_dolwmentation_row(first_cell, row[1:])
        return [first_cell] + [HtmlCell(c) for c in row[1:]]

    def _is_dolwmentation_row(self, row):
        return row[0] == 'Documentation'

    def _create_dolwmentation_row(self, row):
        return [NameCell(row[0]), DolwmentationCell(row[1], span=self._column_count-1)]

    def _is_indented_dolwmentation_row(self, cells, table):
        return self._is_indented_table(table) and cells and \
                    cells[0] == '[Documentation]'

    def _create_indented_dolwmentation_row(self, first_cell, cells):
        start = [first_cell, HtmlCell(cells[0])]
        if any(c.startswith('#') for c in cells):
            return start + [HtmlCell(c) for c in cells[1:]]
        return start + [DolwmentationCell(cells[1], self._column_count-2)]
...
class DolwmentationCell(HtmlCell):
