...
Usage:  python -m robot.tidy [options] inputfile
   ...
   or:  python -m robot.tidy --relwrsive [options] directory
...
Files can also be modified in-place using --inplace or --relwrsive options.
...
=======
 ...
 -r --relwrsive  Process given directory relwrsively. Files in the directory
...
Examples:
  ...
  python -m robot.tidy --format txt --relwrsive mytests
...
Alternative exelwtion
=====================

In the above examples Tidy is used only with Python, but it works also with
Jython and IronPython. Above it is exelwted as an installed module, but it
...
class Tidy(object):
    ...
    def directory(self, path):
        ...
        All files in a directory, relwrsively, are processed in-place.
        """
    ...
    def _parse_data(self, path):
        ...
        if self._is_init_file(path):
            ...
            return TestDataDirectory(source=path).populate(relwrse=False)
        try:
...
class TidyCommandLine(Application):
    ...
    def main(self, arguments, relwrsive=False, inplace=False, format='txt',
...
class ArgumentValidator(object):
    ...
    def mode_and_arguments(self, args, relwrsive, inplace, **others):
        validators = {(True, True): self._relwrsive_and_inplace_together,
                      (True, False): self._relwrsive_mode_arguments,
                      (False, True): self._inplace_mode_arguments,
                      (False, False): self._default_mode_arguments}
        validators[(relwrsive, inplace)](args)

    def _relwrsive_and_inplace_together(self, args):
        raise DataError('--relwrsive and --inplace can not be used together.')

    def _relwrsive_mode_arguments(self, args):
        if len(args) != 1:
            raise DataError('--relwrsive requires exactly one argument.')
        if not os.path.isdir(args[0]):
            raise DataError('--relwrsive requires input to be a directory.')
    ...
    def format(self, args, format, inplace, relwrsive, **others):
        if not format:
            if inplace or relwrsive or len(args) < 2:
                return None
...
def tidy_cli(arguments):
    """Exelwtes `Tidy` similarly as from the command line.
    ...
    """
    TidyCommandLine().exelwte_cli(arguments)
