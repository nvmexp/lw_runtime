...
class Logger(AbstractLogger):
    ...
    loggers at least to get notifications about errors and warnings. A shortlwt
    ...
    def register_file_logger(self, path=None, level='INFO'):
        if not path:
            path = os.elwiron.get('ROBOT_SYSLOG_FILE', 'NONE')
            level = os.elwiron.get('ROBOT_SYSLOG_LEVEL', level)
        if path.upper() == 'NONE':
            return
        try:
