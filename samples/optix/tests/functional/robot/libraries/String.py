...
class String(object):
    ...
    def colwert_to_lowercase(self, string):
        """Colwerts string to lowercase.

        Examples:
        | ${str1} = | Colwert To Lowercase | ABC |
        | ${str2} = | Colwert To Lowercase | 1A2c3D |
    ...
    def colwert_to_uppercase(self, string):
        """Colwerts string to uppercase.

        Examples:
        | ${str1} = | Colwert To Uppercase | abc |
        | ${str2} = | Colwert To Uppercase | 1a2C3d |
        | Should Be Equal | ${str1} | ABC |
        | Should Be Equal | ${str2} | 1A2C3D |
    ...
    def encode_string_to_bytes(self, string, encoding, errors='strict'):
        ...
        Use `Colwert To Bytes` in `BuiltIn` if you want to create bytes based
        on character or integer sequences. Use `Decode Bytes To String` if you
        need to colwert byte strings to Unicode strings and `Colwert To String`
        in `BuiltIn` if you need to colwert arbitrary objects to Unicode.
    ...
    def decode_bytes_to_string(self, bytes, encoding, errors='strict'):
        ...
        Use `Encode String To Bytes` if you need to colwert Unicode strings to
        byte strings, and `Colwert To String` in `BuiltIn` if you need to
        colwert arbitrary objects to Unicode strings.
    ...
    def split_to_lines(self, string, start=0, end=None):
        """Colwerts the `string` into a list of lines.
        ...
        """
        start = self._colwert_to_index(start, 'start')
        end = self._colwert_to_index(end, 'end')
    ...
    def get_line(self, string, line_number):
        ...
        """
        line_number = self._colwert_to_integer(line_number, 'line_number')
    ...
    def replace_string(self, string, search_for, replace_with, count=-1):
        ...
        oclwrrences from left are replaced. Negative `count` means
        that all oclwrrences are replaced (default behaviour) and zero
        ...
        """
        count = self._colwert_to_integer(count, 'count')
    ...
    def split_string(self, string, separator=None, max_split=-1):
        ...
        separator. In that case also possible conselwtive whitespace
        ...
        if separator == '':
            separator = None
        max_split = self._colwert_to_integer(max_split, 'max_split')
    ...
    def fetch_from_left(self, string, marker):
        """Returns contents of the `string` before the first oclwrrence of `marker`.
    ...
    def fetch_from_right(self, string, marker):
        """Returns contents of the `string` after the last oclwrrence of `marker`.
    ...
    def generate_random_string(self, length=8, chars='[LETTERS][NUMBERS]'):
        ...
        if length == '':
            length = 8
        length = self._colwert_to_integer(length, 'length')
    ...
    def _colwert_to_index(self, value, name):
        if value == '':
            return 0
        if value is None:
            return None
        return self._colwert_to_integer(value, name)

    def _colwert_to_integer(self, value, name):
        try:
            return int(value)
        except ValueError:
            raise ValueError("Cannot colwert '%s' argument '%s' to an integer."
                             % (name, value))
