...
for pausing the test exelwtion and getting input from users. The
...
__all__ = ['exelwte_manual_step', 'get_value_from_user',
           'get_selection_from_user', 'pause_exelwtion']


def pause_exelwtion(message='Test exelwtion paused. Press OK to continue.'):
    """Pauses test exelwtion until user clicks `Ok` button.
...
def exelwte_manual_step(message, default_error=''):
    """Pauses test exelwtion until user sets the keyword status.

    User can press either `PASS` or `FAIL` button. In the latter case exelwtion
...
def get_value_from_user(message, default_value='', hidden=False):
    """Pauses test exelwtion and asks user to input a value.
...
def get_selection_from_user(message, *values):
    """Pauses test exelwtion and asks user to select a value.
