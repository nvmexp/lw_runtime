...
class Remote(object):
    ...
    def __init__(self, uri='http://127.0.0.1:8270', timeout=None):
        ...
        a timeout that is shorter than keyword exelwtion time will interrupt
        the keyword.
    ...
    def get_keyword_dolwmentation(self, name):
        try:
            return self._client.get_keyword_dolwmentation(name)
        except TypeError:
            return None
...
class XmlRpcRemoteClient(object):
    ...
    def get_keyword_dolwmentation(self, name):
        try:
            return self._server.get_keyword_dolwmentation(name)
        except xmlrpclib.Error:
            raise TypeError
