...
class _TkDialog(Toplevel):
    ...
    def __init__(self, message, value=None, **extra):
        self._prevent_exelwtion_with_timeouts()
    ...
    def _prevent_exelwtion_with_timeouts(self):
        if 'linux' not in sys.platform \
                and lwrrentThread().getName() != 'MainThread':
...
class SelectionDialog(_TkDialog):
    ...
    def _validate_value(self):
        return bool(self._listbox.lwrselection())

    def _get_value(self):
        return self._listbox.get(self._listbox.lwrselection())
