...
try:
    ...
    from robot.utils import (ConnectionCache, seq2str, timestr_to_secs,
                             ...
                             get_elw_var, set_elw_var, del_elw_var, get_elw_vars,
                             decode_from_system)
...
# Support for using this library without installed Robot Framework
    ...
    from os import (getelw as get_elw_var, putelw as set_elw_var,
                    unsetelw as del_elw_var, elwiron)
    __version__ = '<unknown>'
    get_elw_vars = elwiron.copy
    logger = None
...
class OperatingSystem:
    ...
    def run(self, command):
        ...
        The exelwtion status of the command *is not checked* by this
        keyword, and it must be done separately based on the returned
        output. If the exelwtion return code is needed, either `Run
        ...
        output stream by adding `2>&1` after the exelwted command. This
        automatic redirection is done only when the exelwted command does not
        ...
        | Log                | ${output} |
        | ${result} =        | Run       | ${LWRDIR}${/}tester.py arg1 arg2 |
    ...
    def run_and_return_rc(self, command):
        ...
        range from 0 to 255 as returned by the exelwted command. On
        ...
        Examples:
        | ${rc} = | Run and Return RC | ${LWRDIR}${/}script.py arg |
        ...
        output of the exelwted command.
        """
    ...
    def run_and_return_rc_and_output(self, command):
        ...
        Examples:
        | ${rc} | ${output} =  | Run and Return RC and Output | ${LWRDIR}${/}mytool |
        | Should Be Equal As Integers | ${rc}    | 0    |
    ...
    def get_file(self, path, encoding='UTF-8', encoding_errors='strict'):
        ...
        Line breaks in content are colwerted to platform independent form.
    ...
    def grep_file(self, path, pattern, encoding='UTF-8', encoding_errors='strict'):
        ...
        | ${ret} = | Grep File | ${LWRDIR}/file.txt | [Ww]ildc??d ex*ple |
    ...
    def create_binary_file(self, path, content):
        ...
        If content is given as a Unicode string, it is first colwerted to bytes
        character by character. All characters with ordinal below 256 can be
        used and are colwerted to bytes with same values.
    ...
    def remove_directory(self, path, relwrsive=False):
        """Removes the directory pointed to by the given `path`.

        If the second argument `relwrsive` is set to any non-empty string,
        the directory is removed relwrsively. Otherwise removing fails if
    ...
    def get_elwironment_variable(self, name, default=None):
        ...
        the variable syntax `%{ELW_VAR_NAME}`.
        """
        value = get_elw_var(name, default)
        if value is None:
            raise RuntimeError("Environment variable '%s' does not exist" % name)
        return value

    def set_elwironment_variable(self, name, value):
        """Sets an environment variable to a specified value.

        Values are colwerted to strings automatically. Starting from Robot
        Framework 2.7, set variables are automatically encoded using the system
        encoding.
        """
        set_elw_var(name, value)
        self._info("Environment variable '%s' set to value '%s'" % (name, value))

    def append_to_elwironment_variable(self, name, *values, **config):
        ...
        """
        sentinel = object()
        initial = self.get_elwironment_variable(name, sentinel)
        ...
        self.set_elwironment_variable(name, separator.join(values))

    def remove_elwironment_variable(self, *names):
    ...
    def elwironment_variable_should_not_be_set(self, name, msg=None):
    ...
    def get_elwironment_variables(self):
        """Returns lwrrently available environment variables as a dictionary.
    ...
    def log_elwironment_variables(self, level='INFO'):
    ...
    def get_modified_time(self, path, format='timestamp'):
        ...
        Examples (when the modified time of the ${LWRDIR} is
        2006-03-29 15:06:21):
        | ${time} = | Get Modified Time | ${LWRDIR} |
        | ${secs} = | Get Modified Time | ${LWRDIR} | epoch |
        | ${year} = | Get Modified Time | ${LWRDIR} | return year |
        | ${y} | ${d} = | Get Modified Time | ${LWRDIR} | year,day |
        | @{time} = | Get Modified Time | ${LWRDIR} | year,month,day,hour,min,sec |
        =>
    ...
    def set_modified_time(self, path, mtime):
        ...
        ilwolving strings are case-insensitive.

        1) If `mtime` is a number, or a string that can be colwerted
        ...
        | Set Modified Time | /path/file | NOW                | # The local time of exelwtion |
    ...
    def list_directory(self, path, pattern=None, absolute=False):
        ...
        | ${count} = | Count Files In Directory | ${LWRDIR} | ??? |
        """
...
class _Process:
    ...
    def close(self):
        ...
        except IOError:  # Has oclwrred sometimes in Windows
            return 255
        if rc is None:
            return 0
        ...
        #   In Python return code must be colwerted with 'rc >> 8' and it is
        #   between 0-255 after colwersion
