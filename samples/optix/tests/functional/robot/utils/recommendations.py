...
class RecommendationFinder(object):
    ...
    def find_recommendations(self, name, candidates, max_matches=10):
        ...
        cutoff = self._callwlate_lwtoff(norm_name)
    ...
    def _callwlate_lwtoff(self, string, min_lwtoff=.5, max_lwtoff=.85,
                          step=.03):
        """Callwlate a cutoff depending on string length.

        Default values determined by manual tuning until the results
        "look right".
        """
        cutoff = min_lwtoff + len(string) * step
        return min(cutoff, max_lwtoff)
