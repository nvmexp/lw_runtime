...
class Application(object):
    ...
    def __init__(self, usage, name=None, version=None, arg_limits=None,
                 elw_options=None, logger=None, **auto_options):
        self._ap = ArgumentParser(usage, name, version, arg_limits,
                                  self.validate, elw_options, **auto_options)
    ...
    def exelwte_cli(self, cli_arguments):
        with self._logging():
            options, arguments = self._parse_arguments(cli_arguments)
            rc = self._exelwte(arguments, options)
        self._exit(rc)
    ...
    def execute(self, *arguments, **options):
        with self._logging():
            return self._exelwte(list(arguments), options)

    def _exelwte(self, arguments, options):
        try:
            rc = self.main(arguments, **options)
        except DataError, err:
            return self._report_error(unicode(err), help=True)
        except (KeyboardInterrupt, SystemExit):
            return self._report_error('Exelwtion stopped by user.',
                                      rc=STOPPED_BY_USER)
        except:
