...
class ConnectionCache(object):
    """Cache for test libs to use with conlwrrent connections, processes, etc.
    ...
    conlwrrent connections, processes, etc.
    ...
    def __init__(self, no_lwrrent_msg='No open connection.'):
        self._no_lwrrent = NoConnection(no_lwrrent_msg)
        self.current = self._no_lwrrent  #: Current active connection.
        self._connections = []
        self._aliases = NormalizedDict()

    def _get_lwrrent_index(self):
        if not self:
            return None
    ...
    def _set_lwrrent_index(self, index):
        self.current = self._connections[index - 1] \
            if index is not None else self._no_lwrrent

    lwrrent_index = property(_get_lwrrent_index, _set_lwrrent_index)
    ...
    def switch(self, alias_or_index):
        ...
        as a string that can be colwerted to an integer. Raises an error
    ...
    def __nonzero__(self):
        return self.current is not self._no_lwrrent
