...
class HtmlFormatter(object):
    ...
    def _process_line(self, line):
        if not line.strip():
            self._end_lwrrent()
        elif self._lwrrent and self._lwrrent.handles(line):
            self._lwrrent.add(line)
        else:
            self._end_lwrrent()
            self._lwrrent = self._find_formatter(line)
            self._lwrrent.add(line)

    def _end_lwrrent(self):
        if self._lwrrent:
            self._results.append(self._lwrrent.end())
            self._lwrrent = None
