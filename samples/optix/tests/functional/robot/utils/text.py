...
_ERROR_LWT_EXPLN = '    [ Message content over the limit has been removed. ]'


def lwt_long_message(msg):
    ...
    return '\n'.join(start + [_ERROR_LWT_EXPLN] + end)
...
def _prune_excess_lines(lines, lengths, from_end=False):
    ...
    for line, length in zip(lines[:limit], lengths[:limit]):
        if total + length >= limit:
            ret.append(_lwt_long_line(line, total, from_end))
            break
        total += length
        ret.append(line)
    if from_end:
...
def _lwt_long_line(line, used, from_end):
...
def format_assign_message(variable, value, lwt_long=True):
    value = unic(value) if variable.startswith('$') else seq2str2(value)
    if lwt_long and len(value) > _MAX_ASSIGN_LENGTH:
