...
def decode_from_system(string, can_be_from_java=True):
    """Decodes bytes from system (e.g. cli args or elw vars) to Unicode."""
    if sys.platform == 'cli':
        return string
...
def encode_to_system(string, errors='replace'):
    """Encodes Unicode to system encoding (e.g. cli args and elw vars).

    Non-Unicode strings are first colwerted to Unicode.
    """
