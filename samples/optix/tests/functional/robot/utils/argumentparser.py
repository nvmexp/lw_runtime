...
ESCAPES = dict(
    ...
    square1 = '[', square2 = ']', lwrly1 = '{', lwrly2 = '}', bslash = '\\'
)
...
class ArgumentParser:
    ...
    def __init__(self, usage, name=None, version=None, arg_limits=None,
                 validator=None, elw_options=None, auto_help=True,
    ...
    def parse_args(self, args):
        ...
        """
        if self._elw_options:
            args = os.getelw(self._elw_options, '').split() + list(args)
...
class ArgLimitValidator(object):
    ...
    def __call__(self, args):
        if not (self._min_args <= len(args) <= self._max_args):
            self._raise_ilwalid_args(self._min_args, self._max_args, len(args))

    def _raise_ilwalid_args(self, min_args, max_args, arg_count):
