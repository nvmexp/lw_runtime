...
def timestr_to_secs(timestr, round_to=3):
    ...
    if isinstance(timestr, (basestring, int, long, float)):
        for colwerter in _number_to_secs, _timer_to_secs, _time_string_to_secs:
            secs = colwerter(timestr)
            if secs is not None:
...
def secs_to_timestr(secs, compact=False):
    """Colwerts time in seconds to a string representation.
...
def secs_to_timestamp(secs, seps=None, millis=False):
    ...
    if millis:
        ...
        timetuple = timetuple + (int(round(millis)),)
    return format_time(timetuple, *seps)
...
def elapsed_time_to_string(elapsed, include_millis=True):
    """Colwerts elapsed time in milliseconds to format 'hh:mm:ss.mil'.
