...
class TestSuite(model.TestSuite):
    ...
    def __init__(self, name='', doc='', metadata=None, source=None,
        ...
        #: Suite exelwtion start time in format ``%Y%m%d %H:%M:%S.%f``.
        self.starttime = starttime
        #: Suite exelwtion end time in format ``%Y%m%d %H:%M:%S.%f``.
    ...
    def elapsedtime(self):
        """Total exelwtion time of the suite in milliseconds."""
    ...
    def configure(self, **options):
        """A shortlwt to configure a suite using one method call.
