...
from .exelwtionerrors import ExelwtionErrors
from .testsuite import TestSuite


class Result(object):
    """Test exelwtion results.

    Can be created based on XML output files using
    the :func:`~.resultbuilder.ExelwtionResult` factory method.
    Also returned by exelwted :class:`~robot.running.model.TestSuite`.
    """

    def __init__(self, source=None, root_suite=None, errors=None):
        #: Path to the XML file where results are read from.
        self.source = source
        #: Hierarchical exelwtion results as a
        #: :class:`~.result.testsuite.TestSuite` object.
        self.suite = root_suite or TestSuite()
        #: Exelwtion errors as a
        #: :class:`~.exelwtionerrors.ExelwtionErrors` object.
        self.errors = errors or ExelwtionErrors()
    ...
    def statistics(self):
        ...
        them unnecessarily::
            ...
            from robot.api import ExelwtionResult

            result = ExelwtionResult('output.xml')
    ...
    def return_code(self):
        """Return code (integer) of test exelwtion.
...
class CombinedResult(Result):
    """Combined results of multiple test exelwtions."""
