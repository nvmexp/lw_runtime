...
from .exelwtionresult import Result, CombinedResult
from .flattenkeywordmatcher import FlattenKeywordMatcher
from .merger import Merger
from .xmlelementhandlers import XmlElementHandler


def ExelwtionResult(*sources, **options):
    """Factory method to constructs :class:`~.exelwtionresult.Result` objects.
    ...
    :param options: Configuration options. `rerun_merge` with True value causes
                    ...
                    are passed further to :py:class:`~ExelwtionResultBuilder`.
    :returns: :class:`~.exelwtionresult.Result` instance.
...
def _merge_results(original, merged, options):
    result = ExelwtionResult(original, **options)
    merger = Merger(result)
    for path in merged:
        merged = ExelwtionResult(path, **options)
        merger.merge(merged)
    return result


def _combine_results(sources, options):
    return CombinedResult(ExelwtionResult(src, **options) for src in sources)


def _single_result(source, options):
    ets = ETSource(source)
    try:
        return ExelwtionResultBuilder(ets, **options).build(Result(source))
    except IOError, err:
        error = err.strerror
    except:
...
class ExelwtionResultBuilder(object):

    def __init__(self, source, include_keywords=True, flattened_keywords=None):
        """Builds :class:`~.exelwtionresult.Result` objects from existing
        ...
        :param include_keywords: Include keyword information to the
            :class:`~.exelwtionresult.Result` objects
        """
        self._source = source \
