...
a colwenience factory function :func:`~.model.TestData` can be used to
...
def disable_lwrdir_processing(method):
    """Decorator to disable processing `${LWRDIR}` variable."""
    def decorated(*args, **kwargs):
        original = populators.PROCESS_LWRDIR
        populators.PROCESS_LWRDIR = False
        try:
            return method(*args, **kwargs)
        finally:
            populators.PROCESS_LWRDIR = original
    return decorated
