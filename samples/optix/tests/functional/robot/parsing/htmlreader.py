...
class HtmlReader(HTMLParser):
    ...
    def read(self, htmlfile, populator):
        ...
        self.lwrrent_row = None
        self.lwrrent_cell = None
        for line in htmlfile.readlines():
            self.feed(self._decode(line))
        # Calling close is required by the HTMLParser but may cause problems
        # if the same instance of our HtmlParser is reused. Lwrrently it's
    ...
    def handle_data(self, data):
        if self.state == self.IGNORE or self.lwrrent_cell is None:
            return
        if NON_BREAKING_SPACE in data:
            data = data.replace(NON_BREAKING_SPACE, ' ')
        self.lwrrent_cell.append(data)
    ...
    def table_end(self):
        if self.lwrrent_row is not None:
            self.tr_end()
        self.state = self.IGNORE
    ...
    def tr_end(self):
        if self.lwrrent_row is None:
            return
        if self.lwrrent_cell is not None:
            self.td_end()
        if self.state == self.INITIAL:
            accepted = self.populator.start_table(self.lwrrent_row)
            self.state = self.PROCESS if accepted else self.IGNORE
        elif self.state == self.PROCESS:
            self.populator.add(self.lwrrent_row)
        self.lwrrent_row = None
    ...
    def td_end(self):
        if self.lwrrent_cell is not None and self.state != self.IGNORE:
            cell = ''.join(self.lwrrent_cell)
            self.lwrrent_row.append(cell)
        self.lwrrent_cell = None
