...
# Hook for external tools for altering ${LWRDIR} processing
PROCESS_LWRDIR = True
...
class FromFilePopulator(object):
    ...
    def __init__(self, datafile):
        ...
        self._lwrdir = self._get_lwrdir(datafile.directory)

    def _get_lwrdir(self, path):
    ...
    def add(self, row):
        if PROCESS_LWRDIR and self._lwrdir:
            row = self._replace_lwrdirs_in(row)
        data = DataRow(row)
        if data:
            self._populator.add(data)

    def _replace_lwrdirs_in(self, row):
        return [cell.replace('${LWRDIR}', self._lwrdir) for cell in row]
...
class FromDirectoryPopulator(object):
    ...
    def populate(self, path, datadir, include_suites=None,
                 warn_on_skipped=False, relwrse=True):
