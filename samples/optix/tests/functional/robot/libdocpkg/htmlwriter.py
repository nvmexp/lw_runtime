...
class LibdocModelWriter(ModelWriter):
    ...
    def __init__(self, output, libdoc):
        ...
        self._libdoc = JsonColwerter(formatter).colwert(libdoc)
...
class JsonColwerter(object):
    ...
    def _get_keywords(self, keywords):
        return [self._colwert_keyword(kw) for kw in keywords]

    def _colwert_keyword(self, kw):
        return {
            'name': kw.name,
            'args': ', '.join(kw.args),
...
class DocToHtml(object):
    ...
    def _format_rest(self, doc):
        try:
            from dolwtils.core import publish_parts
        except ImportError:
            raise DataError("reST format requires 'dolwtils' module to be installed.")
