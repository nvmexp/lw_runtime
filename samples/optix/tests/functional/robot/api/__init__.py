...
* :class:`~robot.parsing.model.TestCaseFile`,
  ...
  In addition, a colwenience factory method
...
* :class:`~robot.running.model.TestSuite` class for creating exelwtable
...
* :func:`~robot.result.resultbuilder.ExelwtionResult` factory method
  for reading exelwtion results from XML output files and
...
* :class:`~robot.reporting.resultwriter.ResultWriter` class for writing
  ...
  returned by the :func:`~robot.result.resultbuilder.ExelwtionResult` or
  an exelwted :class:`~robot.running.model.TestSuite`.
...
See dolwmentations of the individual APIs for more details.
...
from robot.result import ExelwtionResult, ResultVisitor
