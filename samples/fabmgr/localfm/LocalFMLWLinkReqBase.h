...
/*
 * LocalFMLWLinkReqBase - This class represents an LWLink request in LFM context, 
 ...
 *
 * LocalFMLWLinkReqConnBase - This class again abstract the connection train request
 ...
 *  LocalFMLWLinkDrvIntf : Provides methods for accessing LWLink driver (ioctl)
 *  LocalFMLWLinkDevRepo : Provides methods to get PCI, Deviceid etc about each LWLink
...
public:
    ...
    uint32 getFmMsgHdrReqId() { return mFmMsgHdrReqId; }
...
protected:
    ...
    // set/get for the final status of the request (on of LWLinkErrorCodes value)
    ...
    uint64 mTrainReqId;
    // a unique id assigned in lower layer FM message for tracking each requests
    uint32 mFmMsgHdrReqId;
...
class LocalFMLWLinkReqConnBase: public LocalFMLWLinkReqBase
{
public:

    LocalFMLWLinkReqConnBase(lwswitch::fmMessage *pFmMessage,
                             FMConnInterface *ctrlConnIntf,
                             LocalFMLWLinkDrvIntf *linkDrvIntf,
                             LocalFMLWLinkDevRepo *linkDevRepo);

    virtual ~LocalFMLWLinkReqConnBase();
...
protected:
    ...
    FMIntReturn_t sendSlaveTrainReqMsg(lwswitch::FabricManagerMessageType msgType);
