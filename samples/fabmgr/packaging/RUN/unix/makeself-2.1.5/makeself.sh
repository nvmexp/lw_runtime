...
# directory and then exelwtes a given script from withing that directory.
#
...
#           Added --noexec to prevent exelwtion of embedded scripts.
...
#
# (C) 1998-2008 by Stphane Peter <megastep@megastep.org>
#
...
MS_Usage()
{
    ...
    echo "                      Implies --notemp."
    echo "    --nomd5         : Don't callwlate an MD5 for archive"
    echo "    --nocrc         : Don't callwlate a CRC for archive"
    echo "    --header file   : Specify location of the header script"
    echo "    --follow        : Follow the symlinks in the archive"
    echo "    --nox11         : Disable automatic spawn of a xterm"
    echo "    --nowait        : Do not wait for user input after exelwting embedded"
...
if test "$NOCRC" = y; then
	...
	crcsum=`cat "$tmpfile" | CMD_ELW=xpg4 cksum | sed -e 's/ /Z/' -e 's/	/Z/' | cut -dZ -f1`
	echo "CRC: $crcsum"
fi
