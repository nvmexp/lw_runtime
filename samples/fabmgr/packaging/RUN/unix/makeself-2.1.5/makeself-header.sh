...
MS_dd()
{
    blocks=\`expr \$3 / 1024\`
    bytes=\`expr \$3 % 1024\`
    dd if="\$1" ibs=\$2 skip=1 obs=1024 colw=sync 2> /dev/null | \\
...
MS_Check()
{
    ...
    do
		...
		if test \$crc = "0000000000"; then
			...
			sum1=\`MS_dd "\$1" \$offset \$s | CMD_ELW=xpg4 cksum | awk '{print \$1}'\`
	...
	:
        ...
        if test x"\$DISPLAY" != x -a x"\$xterm_loop" = x; then  # No, but do we have X?
            if xset q > /dev/null 2>&1; then # Check for valid DISPLAY variable
                ...
                chmod a+x \$0 || echo Please add exelwtion rights on \$0
