                                      ...
                                      SharedFabricCmdParser_t *pCmdParser)
{
    ...
    while (token != NULL) {
        unsigned partitionId = atoi(token);
...
fmReturn_t sharedFabricCmdProcessing(SharedFabricCmdParser_t *pCmdParser)
{
    ...
    if (cmdline_exists(pCmdParser->pCmdLine, SHARED_FABRIC_TEST_CMD_HELP)) {
        pCmdParser->pfCmdHelp(pCmdParser->pCmdLine);
        return FM_ST_SUCCESS;
    }

    if (cmdline_exists(pCmdParser->pCmdLine, SHARED_FABRIC_TEST_CMD_LIST_PARTITION)) {
        pCmdParser->mListPartition = 1;
        return FM_ST_SUCCESS;
    }

    /* Check if partitionId is specified  */
    if (cmdline_exists(pCmdParser->pCmdLine, SHARED_FABRIC_TEST_CMD_ACTIVATE_PARTITION)) {

        unsigned long long tempPartitionId = 0;
        /* Get the partition id */
        if (!cmdline_getIntegerVal(pCmdParser->pCmdLine, SHARED_FABRIC_TEST_CMD_ACTIVATE_PARTITION,
                                   &tempPartitionId)) {
            return FM_ST_BADPARAM;
        }
    ...
    if (cmdline_exists(pCmdParser->pCmdLine, SHARED_FABRIC_TEST_CMD_DEACTIVATE_PARTITION)) {

        unsigned long long tempPartitionId = 0;
        /* Get the partition id */
        if (!cmdline_getIntegerVal(pCmdParser->pCmdLine, SHARED_FABRIC_TEST_CMD_DEACTIVATE_PARTITION,
                                   &tempPartitionId)) {
            return FM_ST_BADPARAM;
        }
    ...
    if (cmdline_exists(pCmdParser->pCmdLine, SHARED_FABRIC_TEST_CMD_SET_ACTIVATED_PARTITION_LIST)) {
        char *partitionList = NULL;
        pCmdParser->mSetActivatedPartitions = 1;
        pCmdParser->mNumPartitions = 0;

        if (cmdline_getStringVal(pCmdParser->pCmdLine, SHARED_FABRIC_TEST_CMD_SET_ACTIVATED_PARTITION_LIST,
    ...
    if (cmdline_exists(pCmdParser->pCmdLine, SHARED_FABRIC_TEST_CMD_ACTIVATE_PARTITION_STRESS)) {
        pCmdParser->mActivationStreeTest = 1;
        return FM_ST_SUCCESS;
    }
