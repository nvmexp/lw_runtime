...
"""
This script will execute or print out the commands that run all FM test
...
use to execute the FM tests on the system and not actually run them.

The script requires access to a prebuilt binary test helper (built inside the
FM tree as part of the normal FM test suite build process) to provide some
platform-specific information and build-time generated information.  The helper
binary is expected to be in the same location as the FM test binaries
...
the prebuilt helper binary. Prior to any FM test list exelwtion or command
output, the script will execute the helper binary to 1) obtain the list of FM
exelwtables, 2) obtain the extensions for each FM exelwtable, and 3) determine
...
"--bin-loc" optional argument can be used to prefix a path to each FM binary
exelwtion; otherwise without the argument, the script will try to execute FM
...
Assuming all FM binaries are located in /vendor/bin and that is part of the
system's PATH, a user could execute all FM tests (according to skiplist in
"SKIP_TESTS" dictionary in this script) via the simple command:

    python run_tests_fm.py --helper-bin-name "testListHelper.exe"

Or to just print out the commands one-by-one:

    python run_tests_fm.py --helper-bin-name "testListHelper.exe" --cmds_only
...
#  {
#    "fixtureName" : "<FM suite fixture name>",
#    "constraint"  : "<constraint reason for skipping>",
...
def get_command_string_suite(blacklist_suite_dict, whitelist_suite_dict,
    ...
    build up a a string like so:
        ...
        fmtext.exe --gtest_filter=-FixtureName.Subtest:FixtureName2.*

    which will execute the "fmtext.exe" and exclude the subtest
    ...
    would be pre-pended to filter, like so:
        ...
        fmtext.exe --gtest_filter=FixtureName3.*:FixtureName2.*:
    ...
    whitelist and/or blacklist, the arguments will be passed in here as well:
        ...
        fmtext.exe --arg-from-whitelist --arg-from-blacklist --gtest_filter=-
...
def exelwte_commands(cmd_list, bin_prefix, called_from_dvs):
    """Exelwtes the commands under the current working directory.  If bin_prefix
       ...
       FM tests are like the following:
    ...
    # suite failures.
    out_results = {"fail_count":0, "pass_count":1, \
...
def print_results(test_info):
    ...
    if test_info["incomplete_suite_list"]:
        ...
        result += str(len(incomplete_list)) + " FM suite commands failed due " + \
    ...
    if test_info["failed_suite_list"]:
        ...
        result += str(len(fail_list)) + " FM suite commands contained failed tests:\n"
...
# first check if fabricmanager is running. If this fails the pther tests shouldn't be run
fm_status = os.system('systemctl is-active lwpu-fabricmanager')
FM_RUNNING = "Checking if Fabric Manager is running\n"
