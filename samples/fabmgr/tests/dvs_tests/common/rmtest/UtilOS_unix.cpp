...
void osAddLWSwitchI2CAdapter(LwU32 *adapters, LwU32 arrLength, string dbdf, LwU32 *numI2cAdapters)
{
    DIR *dir_ptr;
    ...
    while ((dir_entry = readdir(dir_ptr)) != NULL)
    {
        ...
        while ((read = getline(&line, &len, fp)) != -1)
        {
            ...
            if (strstr(line, bdf_str))
            {
                ...
                if (sscanf(dir_entry->d_name, "i2c-%d", &i2c_num) < 0)
                {
                ...
                adapters[num_adapters] = i2c_num;
                num_adapters++;

                if (adapter_idx >= arrLength)
                {
