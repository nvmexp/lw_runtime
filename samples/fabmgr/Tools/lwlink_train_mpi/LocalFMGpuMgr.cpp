...
FMIntReturn_t
LocalFMGpuMgr::getExcludedGpuInfo(std::vector<FMExcludedGpuInfo_t> &excludedGpuInfo)
{
...
LocalFMGpuMgr::allocFmSession()
{
    ...
    rmResult = LwRmAlloc(mRmClientFd, mRmClientFd, FM_SESSION_HANDLE, FABRIC_MANAGER_SESSION, &fmSessionAllocParams);
...
LocalFMGpuMgr::attachAndOpenGpuHandles(LwU32 gpuId, FMPciInfo_t &pciInfo, FMUuid_t &uuid)
{
    ...
    fprintf(stderr, "failed at getting GPU RM dev instance\n");
        detachAndCloseGpuHandles(gpuCtxInfo, uuid);
        return fmResult;
    }
    ...
    if (FM_INT_ST_OK != fmResult) {
        // error already logged, do the clean-up
        fprintf(stderr, "failed at allocating sub devices\n");
    detachAndCloseGpuHandles(gpuCtxInfo, uuid);
        return fmResult;
    }
    ...
    if (false == isGpuArchitectureSupported(gpuCtxInfo)) {
        // error already logged, do the clean-up
    fprintf(stderr, "failed at arch support\n");
        detachAndCloseGpuHandles(gpuCtxInfo, uuid);
    ...
    if (FM_INT_ST_OK != fmResult) {
     fprintf(stderr, "failed at getting gpu lwlink cap info\n");
        // error already logged, do the clean-up
        detachAndCloseGpuHandles(gpuCtxInfo, uuid);
        return fmResult;
    }
...
LocalFMGpuMgr::isGpuArchitectureSupported(GpuCtxInfo_t &gpuCtxInfo)
{
    ...
    rmResult = LwRmControl(mRmClientFd, gpuCtxInfo.lw2080handle, LW2080_CTRL_CMD_MC_GET_ARCH_INFO,
   ...
   //              lw2080GetArchInfoParams.architecture);
    ...
    switch (gpuCtxInfo.archType) {
        ...
        default:
            ...
            //               gpuCtxInfo.pciInfo.busId);
            fprintf(stderr, "not opening/attaching GPU pci bus id: %s as its architecture type is not supported\n", gpuCtxInfo.pciInfo.busId);
        return false;
    }
...
LocalFMGpuMgr::getGpuLWLinkCapInfo(GpuCtxInfo_t &gpuCtxInfo)
{
    ...
    rmResult = LwRmControl(mRmClientFd, gpuCtxInfo.lw2080handle, LW2080_CTRL_CMD_LWLINK_GET_LWLINK_CAPS,
