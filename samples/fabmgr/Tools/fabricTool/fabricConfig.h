#ifndef FABRIC_CONFIG_H
#define FABRIC_CONFIG_H

#include "FMCommonTypes.h"
#include "topology.pb.h"
#include "fm_log.h"
#include <g_lwconfig.h>

#define NUM_PORTS_PER_WILLOW           18
#define NUM_INGR_REQ_ENTRIES_PER_GPU    4    // up to 4 entries, each entry represents a 16G memory region
#define NUM_INGR_RESP_ENTRIES_PER_GPU   6

#define INGRESS_REQ_TABLE_SIZE          8192
#define INGRESS_RESP_TABLE_SIZE         8192
#define GANGED_LINK_TABLE_SIZE          256

#define MAX_NUM_NODES                   16
#define MAX_NUM_GPUS_PER_NODE           16
#define MAX_NUM_LWSWITCH_PER_NODE       12

#define NUM_PORTS_PER_LWSWITCH          64

#define INGRESS_REMAP_TABLE_SIZE        4096
#define INGRESS_RID_TABLE_SIZE          2048
#define INGRESS_RLAN_TABLE_SIZE         2048
#define INGRESS_RID_MAX_PORTS           16
#define INGRESS_RLAN_MAX_GROUPS         16

#define MAX_NUM_GPUS_PER_NODE           16
#define MAX_NUM_LWSWITCHES_PER_NODE     12

#define FAB_ADDR_RANGE_16G              (1LL << 34)
#define GPU_FABRIC_DEFAULT_ADDR_RANGE                ( 1LL << 36 )    // 64G
#define GPU_FABRIC_DEFAULT_ADDR_BASE(gpuEndPointId)  ( (uint64_t)gpuEndPointId << 36 )

#define NUM_INGR_RMAP_ENTRIES_PER_AMPERE 2
// GPA addressing will only be done within a node, and will have the first set of map slots.
// We will reserve space in the RMAP table for the max possible within a node.
// FLA addressing will be done inter-node and intra-node, and will start at LR_FIRST_FLA_RMAP_SLOT
#define LR_FIRST_FLA_RMAP_SLOT              MAX_NUM_GPUS_PER_NODE * NUM_INGR_RMAP_ENTRIES_PER_AMPERE

// Note: The following shared LWSwitch VM partition information is created based on
// Application Note ID https://apps.lwinfo.lwpu.com/pid/contentlibraries/detail?id=1030696
// Table 5: Supported System Partitions: Shared LWSwitches.
struct SharedPartGpuInfoTable_t
{
    uint32_t physicalId;
    uint32_t numEnabledLinks;
    uint64_t enabledLinkMask;
};

struct SharedPartSwitchInfoTable_t
{
    uint32_t physicalId;
    uint32_t numEnabledLinks;
    uint64_t enabledLinkMask;
};

struct  SharedPartInfoTable_t {
    uint32_t partitionId;
    uint32_t gpuCount;
    uint32_t switchCount;
    uint32_t lwLinkIntraTrunkConnCount;
    uint32_t lwLinkInterTrunkConnCount;
    struct SharedPartGpuInfoTable_t     gpuInfo[MAX_NUM_GPUS_PER_NODE];
    struct SharedPartSwitchInfoTable_t  switchInfo[MAX_NUM_LWSWITCH_PER_NODE];
};

//
// Note: When changing this value, update the corresponding FM_MAX_FABRIC_PARTITIONS definition
// used in Shared LWSwitch FM APIs under sdk/public/lw_fm_types.h file
//
#define MAX_SHARED_LWSWITCH_FABRIC_PARTITIONS 35

extern SharedPartInfoTable_t gSharedVMPartInfo[MAX_SHARED_LWSWITCH_FABRIC_PARTITIONS];

#define FABRIC_TOPOLOGY_ENUM(TOPOLOGY) \
    TOPOLOGY(UNKNOWN_TOPOLOGY)    \
    TOPOLOGY(BASIC_E3600_CONFIG1) \
    TOPOLOGY(BASIC_E3600_CONFIG2) \
    TOPOLOGY(BASIC_E3600_CONFIG3) \
    TOPOLOGY(BASIC_E3600_CONFIG4) \
    TOPOLOGY(BASIC_E3600_CONFIG5) \
    TOPOLOGY(BASIC_E3600_CONFIG6) \
    TOPOLOGY(BASIC_E3600_CONFIG7) \
    TOPOLOGY(BASIC_E3600_CONFIG8) \
    TOPOLOGY(BASIC_E3600_CONFIG9) \
    TOPOLOGY(VC_FLIP)             \
    TOPOLOGY(VANGUARD_CONFIG)     \
    TOPOLOGY(EXPLORER8_CONFIG)    \
    TOPOLOGY(DGX2_CONFIG)         \
    TOPOLOGY(EXPLORER2_CONFIG)    \
    TOPOLOGY(EXPLORER8LB_CONFIG)  \
    TOPOLOGY(EXPLORER8BW_CONFIG)  \
    TOPOLOGY(EXPLORER8SL_CONFIG)  \
    TOPOLOGY(EXPLORER_LOOP)       \
    TOPOLOGY(HGX2_BASEBOARD1_LOOP)     \
    TOPOLOGY(HGX2_BASEBOARD2_LOOP)     \
    TOPOLOGY(HGX2_TWO_BASEBOARDS_LOOP) \
    TOPOLOGY(EMULATION_CONFIG)    \
    TOPOLOGY(DGX2_KT_2VM_CONFIG)       \
    TOPOLOGY(LR_EMULATION_CONFIG) \
    TOPOLOGY(DGX2_TRUNK_SPRAY_CONFIG)  \
    TOPOLOGY(BASIC_E4700_PG506_PG506) \
    TOPOLOGY(BASIC_E4700_PG506_PG506_E4705) \
    TOPOLOGY(BASIC_E4700_QUAD_PG506)        \
    TOPOLOGY(BASIC_E4700_QUAD_PG506_2K_ENDPOINT)  \
    TOPOLOGY(DGXA100_HGXA100)            \
    TOPOLOGY(DELTA_SHARED_PARTITION)  \
    TOPOLOGY(LS_FSF_CONFIG) \
    TOPOLOGY(LS_EMULATION_CONFIG) \
    TOPOLOGY(VULCAN_SURROGATE)  \
    TOPOLOGY(VULCAN_SURROGATE_EXTERNAL_LOOPBACK)  \
    TOPOLOGY(VULCAN)  \
    TOPOLOGY(BASIC_E4840_DUAL_PG520) \
    TOPOLOGY(BASIC_E4840_QUAD_PG520) \
    TOPOLOGY(MAX_TOPOLOGY_CONFIG)

#define MAKE_ENUM(VAR) VAR,
typedef enum {
    FABRIC_TOPOLOGY_ENUM(MAKE_ENUM)
} fabricTopologyEnum;

class fabricConfig
{
public:
       fabricConfig( fabricTopologyEnum topology );
       virtual ~fabricConfig();

void makeOneGpu( int nodeIndex,     int gpuIndex,  int endpointID,  int peerID,
                 int peerIdPortMap, int logicalToPhyPortMap,
                 int64_t fabricAddr,int64_t fabricAddrRange, int gpuPhysicalID);

void makeOneGpu( int nodeIndex,     int gpuIndex,  int endpointID,  int peerID,
                 int peerIdPortMap, int logicalToPhyPortMap,
                 int64_t gpaFabricAddr,int64_t gpaFabricAddrRange,
                 int64_t flaFabricAddr,int64_t flaFabricAddrRange,
                 int gpuPhysicalID);

void makeOneGpu( int nodeIndex,     int gpuIndex,  int endpointID,  int peerID,
                 int peerIdPortMap, int logicalToPhyPortMap,
                 int64_t gpaFabricAddr,int64_t gpaFabricAddrRange,
                 int64_t flaFabricAddr,int64_t flaFabricAddrRange,
                 int gpuPhysicalID, uint32_t targetId);

void makeOneGpu( int nodeIndex, int gpuPhysicalID, uint32_t targetId);

void makeOneIngressReqEntry( int nodeIndex, int willowIndex, int portIndex,
                             int index, int64_t address, int routePolicy,
                             int vcModeValid7_0, int vcModeValid15_8,
                             int vcModeValid17_16, int entryValid);

void makeOneIngressRespEntry( int nodeIndex, int willowIndex, int portIndex,
                              int index, int routePolicy, int vcModeValid7_0,
                              int vcModeValid15_8, int vcModeValid17_16,
                              int entryValid);

void makeOneRemapEntry( int nodeIndex, int swIndex, int portIndex,
                        int index, int entryValid, int64_t address,
                        int reqContextChk, int reqContextMask, int reqContextRep,
                        int addressOffset, int addressBase, int addressLimit,
                        int targetId, int rmapFlags, int irlSelect,
                        int p2rSwizEnable, int mult2, int planeSelect,
                        RemapTable remapTable = NORMAL_RANGE );

void makeOneRIDRouteEntry( int nodeIndex, int swIndex, int portIndex,
                           int index, int entryValid, int rMod,
                           int portCount, int *vcMap, int *egressPort);


void makeOneRLANRouteEntry( int nodeIndex, int swIndex, int portIndex,
                            int index, int entryValid, int groupCount,
                            int *groupSelect, int *groupSize);

void makeOneGangedLinkEntry( int nodeIndex, int willowIndex, int portIndex,
                             int index, int data);

void makeOneAccessPort( int nodeIndex, int willowIndex, int portIndex,
                        int farNodeID, int farPeerID,   int farPortNum,
                        PhyMode phyMode);

void makeOneAccessPort( int nodeIndex, int willowIndex, int portIndex,
                        int farNodeID, int farPeerID,   int farPortNum,
                        PhyMode phyMode, uint32_t requesterLinkID, uint32_t rlanID,
                        uint32_t maxTargetID = 0 );

void makeOneTrunkPort( int nodeIndex, int willowIndex, int portIndex,
                       int farNodeID, int farSwitchID, int farPortNum,
                       PhyMode phyMode, uint32_t maxTargetID = 0 );

virtual void makeOneNode( int nodeIndex, int gpuNum, int willowNum ) = 0;
virtual void makeNodes() = 0;
virtual void makeOneWillow( int nodeIndex, int willowIndex ) = 0;
virtual void makeIngressReqTable( int nodeIndex, int willowIndex ) = 0;
virtual void makeIngressRespTable( int nodeIndex, int willowIndex ) = 0;
virtual void makeGangedLinkTable( int nodeIndex, int willowIndex ) = 0;
virtual void makeAccessPorts( int nodeIndex, int willowIndex ) = 0;
virtual void makeTrunkPorts( int nodeIndex, int willowIndex ) = 0;

virtual void makeOneLwswitch( int nodeIndex, int swIndex );
virtual void makeRemapTable( int nodeIndex, int swIndex );
virtual void makeRIDRouteTable( int nodeIndex, int swIndex );
virtual void makeRLANRouteTable( int nodeIndex, int swIndex );

public:
    fabric               *mFabric;
    fabricTopologyEnum    fabricTopology;
    nodeSystemPartitionInfo *mSystemPartInfo;

    const char *getFabricTopologyName(fabricTopologyEnum topology);

protected:
    node                 *nodes[MAX_NUM_NODES];
    lwSwitch             *switches[MAX_NUM_NODES][MAX_NUM_LWSWITCH_PER_NODE];
    GPU                  *gpus[MAX_NUM_NODES][MAX_NUM_GPUS_PER_NODE];
    accessPort           *accesses[MAX_NUM_NODES][MAX_NUM_LWSWITCH_PER_NODE][NUM_PORTS_PER_LWSWITCH];
    trunkPort            *trunks[MAX_NUM_NODES][MAX_NUM_LWSWITCH_PER_NODE][NUM_PORTS_PER_LWSWITCH];
    ingressRequestTable  *reqEntry[MAX_NUM_NODES][MAX_NUM_LWSWITCH_PER_NODE][NUM_PORTS_PER_WILLOW][INGRESS_REQ_TABLE_SIZE];
    ingressResponseTable *respEntry[MAX_NUM_NODES][MAX_NUM_LWSWITCH_PER_NODE][NUM_PORTS_PER_WILLOW][INGRESS_RESP_TABLE_SIZE];
    int32_t              *gangedLinkEntry[MAX_NUM_NODES][MAX_NUM_LWSWITCH_PER_NODE][NUM_PORTS_PER_WILLOW][GANGED_LINK_TABLE_SIZE];
    switchPortConfig     *portConfigs[MAX_NUM_NODES][MAX_NUM_LWSWITCH_PER_NODE][NUM_PORTS_PER_LWSWITCH];

    rmapPolicyEntry      *rmapEntry[MAX_NUM_NODES][MAX_NUM_LWSWITCHES_PER_NODE][NUM_PORTS_PER_LWSWITCH][INGRESS_REMAP_TABLE_SIZE];
    rmapPolicyEntry      *rmapExtAEntry[MAX_NUM_NODES][MAX_NUM_LWSWITCHES_PER_NODE][NUM_PORTS_PER_LWSWITCH][INGRESS_REMAP_TABLE_SIZE];
    rmapPolicyEntry      *rmapExtBEntry[MAX_NUM_NODES][MAX_NUM_LWSWITCHES_PER_NODE][NUM_PORTS_PER_LWSWITCH][INGRESS_REMAP_TABLE_SIZE];

    ridRouteEntry        *ridEntry[MAX_NUM_NODES][MAX_NUM_LWSWITCHES_PER_NODE][NUM_PORTS_PER_LWSWITCH][INGRESS_RID_TABLE_SIZE];
    rlanRouteEntry       *rlanEntry[MAX_NUM_NODES][MAX_NUM_LWSWITCHES_PER_NODE][NUM_PORTS_PER_LWSWITCH][INGRESS_RLAN_TABLE_SIZE];

    uint64_t gpuFabricAddrBase[MAX_NUM_NODES * MAX_NUM_GPUS_PER_NODE];
    uint64_t gpuFabricAddrRange[MAX_NUM_NODES * MAX_NUM_GPUS_PER_NODE];
    uint64_t gpuGpaEgmAddrBase[MAX_NUM_NODES * MAX_NUM_GPUS_PER_NODE];
    uint64_t gpuGpaEgmAddrRange[MAX_NUM_NODES * MAX_NUM_GPUS_PER_NODE];
    uint64_t gpuFlaAddrBase[MAX_NUM_NODES * MAX_NUM_GPUS_PER_NODE];
    uint64_t gpuFlaAddrRange[MAX_NUM_NODES * MAX_NUM_GPUS_PER_NODE];
    uint64_t gpuTargetId[MAX_NUM_NODES * MAX_NUM_GPUS_PER_NODE];

    void setFabricTopologyName();
    void setFabricTopologyTime();

    virtual bool switchOnBaseboard1( uint32_t physicalId );
    virtual bool switchOnBaseboard2( uint32_t physicalId );
    void parsePartitionJsonFile( uint32_t lwLinkIntraTrunkConnCount );

    #define MAX_NUM_SHARED_LWSWITCH_FABRIC_PARTITIONS  35
    SharedPartInfoTable_t mSharedVMPartInfo[MAX_NUM_SHARED_LWSWITCH_FABRIC_PARTITIONS];

    const char *mSharedPartitionJsonFile;
    std::string mSharedPartitionTimeStamp;
    int mSharedPartitiolwersion;
    uint32_t mNumSharedPartitions;

private:
};

#endif
