...
SharedPartInfoTable_t gSharedVMPartInfo[MAX_SHARED_LWSWITCH_FABRIC_PARTITIONS] = {
                             ...
                             //partionId 3 - 8 GPUs, first 4 group from BB1 & first 4 group from BB2, 5 Willows BB1 & 5 Willows BB2, 48 intra-trunk conn, 0 inter-trunk conn
                             { 3, 8, 12, 48, 0,
                               // all the GPUs, id, numlink, mask
                               {  
                                ...
                                {  
                                    // BB1 switches
                                    {0x08, 12, 0x0CF3F}, // link4,5,14,15 enabled for GPU +  Trunk Links
                                    {0x09, 12, 0x03FCF}, // link6,7,12,13 enabled for GPU +  Trunk Links
                                    {0x0A, 8,  0x00F0F}, // Only Trunk Links
                                    {0x0B, 12, 0x31F2F}, // link5,12,16,17 enabled for GPU +  Trunk Links
                                    {0x0C, 12, 0x2CF2F}, // link5,14,15,17 enabled for GPU +  Trunk Links
                                    {0x0D, 12, 0x20FEF}, // link5,6,7,17 enabled for GPU +  Trunk Links
                                    // BB2 switches
                                    {0x18, 12, 0x0CF3F}, // link4,5,14,15 enabled for GPU +  Trunk Links
                                    {0x19, 12, 0x03FCF}, // link6,7,12,13 enabled for GPU +  Trunk Links
                                    {0x1A, 8,  0x00F0F}, // Only Trunk Links
                                    {0x1B, 12, 0x31F2F}, // link5,12,16,17 enabled for GPU +  Trunk Links
                                    {0x1C, 12, 0x2CF2F}, // link5,14,15,17 enabled for GPU +  Trunk Links
                                    {0x1D, 12, 0x20FEF}, }, // link5,6,7,17 enabled for GPU +  Trunk Links
                             },

                             //partionId 4 - 8 GPUs, second 4 group from BB1 & second 4 group from BB2, 6 Willows BB1 & 6 Willows BB2, 48 intra-trunk conn, 0 inter-trunk conn
                             { 4, 8, 12, 48, 0,
                               // all the GPUs, id, numlink, mask
                               {  
                                   // second 4 group from BB1 (refer to BB1 4 GPU partition below)
                                   {1, 5, 0x3E}, // link0 disabled
                                   {2, 5, 0x3E}, // link0 disabled
                                   {4, 5, 0x3E}, // link0 disabled
                                   {7, 5, 0x3E}, // link0 disabled
                                   // second 4 group from BB2 (refer to BB2 4 GPU partition below)
                                ...
                                {  
                                    // BB1 switches
                                    {0x08, 4, 0x33F0F}, // link12,13,16,17 enabled for GPU +  Trunk Links
                                    {0x09, 4, 0x30F3F}, // link4,5,16,17 enabled for GPU +  Trunk Links
                                    {0x0A, 4, 0x26F4F}, // link6,13,14,17 enabled for GPU +  Trunk Links
                                    {0x0B, 8, 0x00F0F}, // Only Trunk Links
                                    {0x0C, 4, 0x03FCF}, // link6,7,12,13 enabled for GPU +  Trunk Links
                                    {0x0D, 4, 0x0FF0F}, // link12,13,14,15 enabled for GPU +  Trunk Links
                                    // BB2 switches
                                    {0x18, 4, 0x33F0F}, // link12,13,16,17 enabled for GPU +  Trunk Links
                                    {0x19, 4, 0x30F3F}, // link4,5,16,17 enabled for GPU +  Trunk Links
                                    {0x1A, 4, 0x26F4F}, // link6,13,14,17 enabled for GPU +  Trunk Links
                                    {0x1B, 8, 0x00F0F}, // Only Trunk Links
                                    {0x1C, 4, 0x03FCF}, // link6,7,12,13 enabled for GPU +  Trunk Links
                                    {0x1D, 4, 0x0FF0F}, }, // link12,13,14,15 enabled for GPU +  Trunk Links
                             },

                             // 
                             ///Note: In Willow/Volta we can't support 8 GPU partitions spanning across baseboard using the following combination
                             //   First 4 GPU group from BB1 & Second 4 GPU group from BB2
                             //   Second 4 GPU group from BB1 & First 4 GPU group from BB2
                             // This is because in each group only 5 LWLinks are enabled and all GPUs are disconnected from 1 LWSwitch. This
                             // disconnected LWSwitch number is different for first 4 GPU group and second 4 GPU group. If we combine them, it will
                             // cause disjoint group and certain GPU traffic arrive on a an LWSwitch which don't have connectivity to other GPUs.
                             // 

                             //partionId 5 - 4 GPUs, 5 Willows base board 1, 0 intra-trunk conn, 0 inter-trunk conn
                             { 5, 4, 5, 0, 0,
                               // all the GPUs, id, numlink, mask
                               {   {0, 5, 0x37}, // link3 disabled
                                   {3, 5, 0x3B}, // link2 disabled
                             ...
                             //partionId 6 - 4 GPUs, 5 Willows base board 1, 0 intra-trunk conn, 0 inter-trunk conn
                             { 6, 4, 5, 0, 0,
                               // all the GPUs, id, numlink, mask
                               {   {1, 5, 0x3E}, // link0 disabled
                                   {2, 5, 0x3E}, // link0 disabled
                             ...
                             //partionId 7 - 4 GPUs, 5 Willows base board 2, 0 intra-trunk conn, 0 inter-trunk conn
                             { 7, 4, 5, 0, 0,
                               // all the GPUs, id, numlink, mask
                               {   {8,  5, 0x37}, // link3 disabled
                                   {11, 5, 0x3B}, // link2 disabled
                             ...
                             //partionId 8 - 4 GPUs, 5 Willows base board 2, 0 intra-trunk conn, 0 inter-trunk conn
                             { 8, 4, 5, 0, 0,
                               // all the GPUs, id, numlink, mask
                               {   {9,  5, 0x3E}, // link0 disabled
                                   {10, 5, 0x3E}, // link0 disabled
                             ...
                             //partionId 11 - 2 GPUs, 5 Willows base board 1, 0 intra-trunk conn, 0 inter-trunk conn
                             { 11, 2, 5, 0, 0,
                               // all the GPUs, id, numlink, mask
                               {   {4, 5, 0x3E}, // link0 disabled
                                   {7, 5, 0x3E}, }, // link0 disabled
                             ...
                             //partionId 12 - 2 GPUs, 5 Willows base board 1, 0 intra-trunk conn, 0 inter-trunk conn
                             { 12, 2, 5, 0, 0,
                               // all the GPUs, id, numlink, mask
                             ...
                             //partionId 15 - 2 GPUs, 5 Willows base board 2 , 0 intra-trunk conn, 0 inter-trunk conn
                             { 15, 2, 5, 0, 0,
                               // all the GPUs, id, numlink, mask
                               {   {12, 5, 0x3E}, // link0 disabled
                                   {15, 5, 0x3E}, }, // link0 disabled
                             ...
                             //partionId 16 - 2 GPUs, 5 Willows base board 2, 0 intra-trunk conn, 0 inter-trunk conn
                             { 16, 2, 5, 0, 0,
                               // all the GPUs, id, numlink, mask
                             ...
                             //partionId 31 - 1 GPUs, 0 Willows, 0 intra-trunk conn, 0 inter-trunk conn
                             { 31, 1, 0, 0, 0,
                               // all the GPUs, id, numlink, mask
                             ...
                             //partionId 32 - 1 GPUs, 0 Willows, 0 intra-trunk conn, 0 inter-trunk conn
                             { 32, 1, 0, 0, 0,
                               // all the GPUs, id, numlink, mask
