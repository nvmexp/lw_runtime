...
#Limerock supports a max of 512 endpoints without using the 1K or 2k modes
DEFAULT_MODE_MAX_ENDPOINTS = 512

memoryPerGpu = (128 * 1024 * 1024 * 1024)
memoryPerGpuReal = (128 *1024 * 1024 * 1024)
portsPerGpu = 12
fabricVersion = 0x100
flaIndexStart =  DEFAULT_MODE_MAX_ENDPOINTS
rmapEntryMaxSize = (64 *1024 * 1024 * 1024)
...
def gpuBaseAddress(nodeId, phyId, gpuPhyIds):
    if args.match_target_phy_id == True:
        return phyId * memoryPerGpu
    else:
        return gpuPhyIds.index((nodeId, phyId)) * memoryPerGpu
...
def gpuFlaBaseAddress(nodeId, phyId, gpuPhyIds, targetId):
    ...
    #gpuIndex = gpuPhyIds.index((nodeId, phyId))
    gpuIndex = targetId
    return (flaIndexStart + (gpuIndex * (memoryPerGpu/rmapEntryMaxSize))) * rmapEntryMaxSize
...
def isLwlinkRackSwitch(switchNodeList, nodeID):
...
def buildGraph(allLinks, switchPorts, portsPerSwitch):
    switchNodeList = getAllSwitchOnlyNodes()

    G = nx.DiGraph()
    for l, r in iter(allLinks.items()):
        G.add_edge(l, r)
        print("adding edge between lwlinks", l, r)
    if isSnake == False:
            ...
            for j in range(len(switchPorts)):
                ...
                if args.unique_access_port:
                    ...
                    if getNodeId(switchPorts[i]) == getNodeId(switchPorts[j]) and getPhyId(switchPorts[i]) == getPhyId(switchPorts[j]):
                        ...
                        if isTrunkPort(portsPerSwitch, switchPorts[i]) and ((args.spray and enable_inter_trunk_spray and args.ring_dateline != None)  or isAccessPort(portsPerSwitch, switchPorts[j]) or (isTrunkPort(portsPerSwitch, switchPorts[j]) and (isLwlinkRackSwitch(switchNodeList, getNodeId(switchPorts[j])))) ):
                                if isTrunkPort(portsPerSwitch, switchPorts[j]) and (isLwlinkRackSwitch(switchNodeList, getNodeId(switchPorts[j]))):
                                    G.add_edge(switchPorts[i], switchPorts[j])
                        ...
                        #If we get here it means switchPorts[i] is an access port. connect an access port to exactly one trunk port
                            ...
                            #a trunk port is assigned to a gpu..this remains same accross all switchPorts.
                                G.add_edge(switchPorts[i], switchPorts[j])
                                G.add_edge(switchPorts[j], switchPorts[i])
                                print("adding access to trunk edge",  switchPorts[i], switchPorts[j])
                                print("adding access to trunk edge",  switchPorts[j], switchPorts[i])
...
def findRoutes(G, gpuPorts, allLinks, portsPerSwitch):
    ...
    for (a, b) in allShortestPaths:
        print("allShortestPaths:", a, b, allShortestPaths[(a, b)] )
    #allShortestPaths = findAllShortestPaths(G, gpuPorts)
    print_verbose("size of allShortestPaths=" + str(len(allShortestPaths)) )    
    for l in gpuPorts:
        for r in gpuPorts:
            print("finding route between:", l , r)
            if (isLwlinkRackSwitch(switchNodeList, int(getNodeId(l)))) or (isLwlinkRackSwitch(switchNodeList, int(getNodeId(l)))):
                continue
            if (getNodeId(l) != getNodeId(r) or getPhyId(l) != getPhyId(r)) and (args.match_gpu_port_num == False or getPortId(l) == getPortId(r)):
...
def buildTopologyInfo(allPaths, nodeIds, gpuPhyIds, switchPhyIds, portsPerSwitch, switchPorts):
    def buildRmapPolicyInfo(topologyInfo, gpuPhyIds, dest, path, hopNum, isFlaOnly):
        ...
        if not isFlaOnly:
            ...
            rmapEntry["index"] = gpaIndex * 2
            rmapEntry["address"] = 0
            #if isAccessPort(portsPerSwitch, path[hopNum + 1]):
                #rmapEntry["address"] = 0
            #else:
                #rmapEntry["address"] = gpuBaseAddress(getNodeId(dest), getPhyId(dest), gpuPhyIds)
            topologyInfo[getNodeId(path[ hopNum ])] [getPhyId(path[ hopNum ])] [getPortId(path[ hopNum ])] ["rmapEntries"][gpaIndex *2] = rmapEntry

            rmapEntry = dict()
            rmapEntry["targetId"] = gpaIndex
            rmapEntry["index"] = gpaIndex * 2 + 1
            rmapEntry["address"] = rmapEntryMaxSize
            #if isAccessPort(portsPerSwitch, path[hopNum + 1]):
                #rmapEntry["address"] = 0
            #else:
                #rmapEntry["address"] = gpuBaseAddress(getNodeId(dest), getPhyId(dest), gpuPhyIds)
            topologyInfo[getNodeId(path[ hopNum ])] [getPhyId(path[ hopNum ])] [getPortId(path[ hopNum ])] ["rmapEntries"][gpaIndex * 2 + 1] = rmapEntry
        ...
        rmapEntry = dict()
        flaIndex = (gpaIndex *2) +  1 + flaIndexStart
        rmapEntry["targetId"] = gpaIndex
        rmapEntry["index"] = flaIndex
        rmapEntry["address"] = gpuFlaBaseAddress(getNodeId(dest), getPhyId(dest), gpuPhyIds, gpaIndex) + rmapEntryMaxSize
        topologyInfo[getNodeId(path[ hopNum ])] [getPhyId(path[ hopNum ])] [getPortId(path[ hopNum ])] ["rmapEntries"][flaIndex] = rmapEntry
...
def buildTopologyProtobuf(topologyInfo, nodeIds, allLinks, portsPerSwitch, gpuPhyIds, allPaths, lwlinkToOsfpPortMapping, switchPhyIdToSlotIdMapping):
    ...
    if args.topology_name != None:
        ...
        fabric.name = "LR_CONFIG"
    fabric.time = strftime("%a %b %d %H:%M:%S %Y", gmtime())
    fabric.arch = topology_pb2.LWSWITCH_ARCH_TYPE_LR10
...
def main():
    ...
    flaIndexStart = numGpus * (memoryPerGpu / rmapEntryMaxSize)
