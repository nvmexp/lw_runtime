...
LwS8 dpauxChRead_v02_05(LwU32 port, LwU32 addr)
{
    LwU8 data;

    // 1 byte to read
    if (dpauxChReadMulti_v02_05(port, addr, &data, 1) == 1)
        return data;
    else
        return -1;
}

void dpauxChWrite_v02_05(LwU32 port, LwU32 addr, LwU8 auxdata)
{
    // 1 byte to write
    dpauxChWriteMulti_v02_05(port, addr, &auxdata, 1);
}

LwU32 dpauxChReadMulti_v02_05
(
    LwU32 port,
    LwU32 addr,
    LwU8 *data,
...
LwU32 dpauxChWriteMulti_v02_05
(
    LwU32 port,
    LwU32 addr,
    LwU8 *data,
...
BOOL
dpauxIsPadPowerUpForPort_v02_05
(
    LwU32 port
)
{
