...
void sigInitInstanceInfo_GK110(InstanceInfo* pInfo, LwU32 ngpc, LwU32 ntpc, LwU32 nfbp, LwU32 option)
{
...
void sigPrintLegend_GK110(FILE* fp)
{
    dprintf("lw: This function needs to be built with SIGDUMP enabled\n");
    dprintf("lw: Add /DSIGDUMP_ENABLE to the C_DEFINES in sources\n");
}
...
void sigInitInstanceInfo_GK110(InstanceInfo* pInfo, LwU32 ngpc, LwU32 ntpc, LwU32 nfbp, LwU32 option)
{
    ...
    for (i = 0; i < NUMINSTANCES; i++)
    {
        ...
        switch(i)
        {
            ...
            case gpc_tpc:
                pInfo[i].chipletLimit = ngpc-1;   //gpc chiplet limit
                pInfo[i].instanceLimit = 2;       //3 instances
                pInfo[i].bValid = TRUE; 
                break;

            case gpc_ppc:
                pInfo[i].chipletLimit = ngpc-1;   //gpc chiplet limit
                pInfo[i].instanceLimit = 1;       // 2 instances 
                pInfo[i].bValid = TRUE; 
                break;
            ...
            case sys_mxbar_cs_daisy:
                ...
                pInfo[i].instanceLimit = 10;
                pInfo[i].bValid = (option == XBAR_DUMP) ? TRUE : FALSE;
                break;

            case sys_wxbar_cs_daisy:
                pInfo[i].chipletLimit = 0;        //only one chiplet
                pInfo[i].instanceLimit = 5;
                pInfo[i].bValid = (option == XBAR_DUMP) ? TRUE : FALSE;
                break;
...
void sigPrintLegend_GK110(FILE* fp)
{
    fprintf(fp, " ****** GK110 Sigdump LEGEND ****** \n");
    ...
    fprintf(fp, "       For 'source' value\n");
    fprintf(fp, "           fbp, sys     => 0 always\n");
    fprintf(fp, "           gpc     => 0..4 based on GPC floorsweeping\n");
    fprintf(fp, "           gpc_tpc => tpc# : 0..2 based on TPC floorsweeping, virtualized when floorswept.\n");
    fprintf(fp, "           gpc_ppc => ppc# : 0..1 based on GPC/PPC floorsweeping, virtualized when floorswept.\n");
    fprintf(fp, "           sys_mxbar_cs_daisy => 0..10 (numGPC + numFBP + numSYS), independent of floorsweeping, not virtualized.\n");
    fprintf(fp, "           sys_wxbar_cs_daisy => 0..5  (numGPC + numSYS), independent of floorsweeping, not virtualized.\n");
