...
void sigInitInstanceInfo_GK208(InstanceInfo* pInfo, LwU32 ngpc, LwU32 ntpc, LwU32 nfbp, LwU32 option)
{
...
void sigPrintLegend_GK208(FILE* fp)
{
   dprintf("lw: This function needs to be built with SIGDUMP enabled\n");
   dprintf("lw: Add /DSIGDUMP_ENABLE to the C_DEFINES in sources\n");
}
    ...
    for (i = 0; i < NUMINSTANCES; i++)
    {
        ...
        switch(i)
        {
            case gpc:
                ...
                pInfo[i].instanceLimit = 0;       //1 instances
                pInfo[i].bValid = TRUE;
                break;
...
void sigPrintLegend_GK208(FILE* fp)
{
    fprintf(fp, " ****** GK208 Sigdump LEGEND ****** \n");
    ...
    fprintf(fp, "           gpc_tpc => tpc# : 0..1 based on TPC floorsweeping, virtualized when floorswept.\n");
    fprintf(fp, "           sys_mxbar_cs_daisy => 0..2 (numGPC + numFBP + numSYS), independent of floorsweeping, not virtualized.\n");
    fprintf(fp, "           sys_wxbar_cs_daisy => 0..1 (numGPC + numSYS), independent of floorsweeping, not virtualized.\n");
