...
void sigInitInstanceInfo_GM200(InstanceInfo* pInfo, LwU32 ngpc, LwU32 ntpc, LwU32 nfbp, LwU32 option)
{
...
void sigPrintLegend_GM200(FILE* fp)
{
   dprintf("lw: This function needs to be built with SIGDUMP enabled\n");
   dprintf("lw: Add /DSIGDUMP_ENABLE to the C_DEFINES in sources\n");
}
    ...
    for (i = 0; i < NUMINSTANCES; i++)
    {
        ...
        switch(i)
        {
            case gpc:
                ...
                pInfo[i].instanceLimit = 5;       //six instances
                pInfo[i].bValid = TRUE;
                break;

            case gpc_tpc:
                pInfo[i].chipletLimit = ngpc-1;   //gpc chiplet limit
                pInfo[i].instanceLimit = 23;       //twenty four instances
                pInfo[i].bValid = TRUE;
                break;
            ...
            case sys_mxbar_cs_daisy:
                ...
                pInfo[i].instanceLimit = 12;       // thirteen instances
                pInfo[i].bValid = TRUE;
                break;

            case sys_wxbar_cs_daisy:
                pInfo[i].chipletLimit = 0;        //only one chiplet
                pInfo[i].instanceLimit = 6;       // seven instances
                pInfo[i].bValid = TRUE;
                break;
...
void sigPrintLegend_GM200(FILE* fp)
{
    fprintf(fp, " ****** GM200 Sigdump LEGEND ****** \n");
    ...
    fprintf(fp, "           gpc_tpc => tpc# : 0..23 based on TPC floorsweeping, virtualized when floorswept.\n");
    fprintf(fp, "           sys_mxbar_cs_daisy => 0..12 (numGPC + numFBP + numSYS), independent of floorsweeping, not virtualized.\n");
    fprintf(fp, "           sys_wxbar_cs_daisy => 0..6  (numGPC + numSYS), independent of floorsweeping, not virtualized.\n");
