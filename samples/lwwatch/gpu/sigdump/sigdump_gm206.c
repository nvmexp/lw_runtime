...
void sigInitInstanceInfo_GM206(InstanceInfo* pInfo, LwU32 ngpc, LwU32 ntpc, LwU32 nfbp, LwU32 option)
{
...
void sigPrintLegend_GM206(FILE* fp)
{
   dprintf("lw: This function needs to be built with SIGDUMP enabled\n");
   dprintf("lw: Add /DSIGDUMP_ENABLE to the C_DEFINES in sources\n");
}
...
void sigInitInstanceInfo_GM206(InstanceInfo* pInfo, LwU32 ngpc, LwU32 ntpc, LwU32 nfbp, LwU32 option)
{
    ...
    for (i = 0; i < NUMINSTANCES; i++)
    {
        ...
        switch(i)
        {
            ...
            case gpc_tpc:
                pInfo[i].chipletLimit = ngpc-1;   //gpc chiplet limit
                pInfo[i].instanceLimit = 7;       //eight instances
                pInfo[i].bValid = TRUE;
                break;
...
void sigPrintLegend_GM206(FILE* fp)
{
    fprintf(fp, " ****** GM206 Sigdump LEGEND ****** \n");
    ...
    fprintf(fp, "           sys_mxbar_cs_daisy => 0..4 (numGPC + numFBP + numSYS), independent of floorsweeping, not virtualized.\n");
    fprintf(fp, "           sys_wxbar_cs_daisy => 0..2  (numGPC + numSYS), independent of floorsweeping, not virtualized.\n");
