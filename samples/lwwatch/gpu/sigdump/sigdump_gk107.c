...
void sigInitInstanceInfo_GK107(InstanceInfo* pInfo, LwU32 ngpc, LwU32 ntpc, LwU32 nfbp, LwU32 option)
{
...
void sigPrintLegend_GK107(FILE* fp)
{
   dprintf("lw: This function needs to be built with SIGDUMP enabled\n");
   dprintf("lw: Add /DSIGDUMP_ENABLE to the C_DEFINES in sources\n");
}
    ...
    for (i = 0; i < NUMINSTANCES; i++)
    {
        ...
        switch(i)
        {
            case gpc:
                ...
                pInfo[i].instanceLimit = 0;       //one instance
                pInfo[i].bValid = TRUE;
                break;
            ...
            case sys_mxbar_cs_daisy:
                ...
                pInfo[i].instanceLimit = 3;       // 4 instances
                pInfo[i].bValid = TRUE;
                break;
...
void sigPrintLegend_GK107(FILE* fp)
{
    fprintf(fp, " ****** GK107 Sigdump LEGEND ****** \n");
    ...
    fprintf(fp, "       gpc_ppc => in GPC(inside TPCs).\n"); 
    ...
    fprintf(fp, "           sys_mxbar_cs_daisy => 0..3 (numGPC + numFBP + numSYS), independent of floorsweeping, not virtualized.\n");
