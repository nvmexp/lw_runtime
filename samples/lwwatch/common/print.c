...
//-----------------------------------------------------
//  Return if byte is printable.  This was added because
//  the standard MS function isprint was crashing when passed 0xC0.
//
//  @param c    Byte (character) to test.
//
//  @return TRUE if c is a printable character, otherwise FALSE.
//
//-----------------------------------------------------
...
void printBuffer(char *buffer, LwU32 length, LwU64 offset, LwU8 size)
{
    ...
    if (size == 1) {
        for (i = 0; i < length/16; i++) {
            ...
            dprintf(LwU40_FMT ": ", i*16 + offset);
            for (j = 0; j < 16; j++) {
                dprintf("%02x ", buffer[i*16 + j] & 0xFF);
            }
            for (j = 0; j < 16; j++) {
                if (_isPrintable(buffer[i*16 + j])) {
...
void printDataByType(PhysAddr addr, LwU32 sizeInBytes, MEM_TYPE memoryType, LwU32 numColumns)
{
    ...
    // 1 dw at least
    if (sizeInBytes < sizeof(LwU32))
        sizeInBytes = sizeof(LwU32);

    buffer = (LwU32*) malloc(sizeInBytes);
...
//
//-----------------------------------------------------
static LwU8 LwClassStrings[][33]={"LW01_NULL_OBJECT                ", /*    (0x00000000) */
