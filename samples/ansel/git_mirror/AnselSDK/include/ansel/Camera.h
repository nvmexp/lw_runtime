...
namespace ansel
{
    ...
    // Colwerts quaternion to rotation matrix vectors.
    ANSEL_SDK_API void quaternionToRotationMatrixVectors(const lw::Quat& q, lw::Vec3& right, lw::Vec3& up, lw::Vec3& forward);

    // Colwerts rotation matrix vectors to quaternion. 
    ANSEL_SDK_API void rotationMatrixVectorsToQuaternion(const lw::Vec3& right, const lw::Vec3& up, const lw::Vec3& forward, lw::Quat& q);
}
