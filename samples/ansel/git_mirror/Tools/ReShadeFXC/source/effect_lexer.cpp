...
static const std::unordered_map<std::string, tokenid> keyword_lookup = {
    ...
    { "textureLWBE", tokenid::reserved },
    { "TextureLwbe", tokenid::reserved },
    { "TextureLwbeArray", tokenid::reserved },
...
{
    bool is_at_line_begin = _lwr_location.column <= 1;

    token tok;
next_token:
    // Reset token data
    tok.location = _lwr_location;
    tok.offset = _lwr - _input.data();
    tok.length = 1;
    tok.literal_as_double = 0;
    tok.literal_as_string.clear();

    // Do a character type lookup for the current character
    switch (type_lookup[uint8_t(*_lwr)])
    {
    case 0xFF: // EOF
    ...
    case SPACE:
        skip_space();
        if (_ignore_whitespace || is_at_line_begin || *_lwr == '\n')
            goto next_token;
        tok.id = tokenid::space;
        tok.length = _lwr - _input.data() - tok.offset;
        return tok;
    case '\n':
        _lwr++;
        _lwr_location.line++;
        _lwr_location.column = 1;
    ...
    case '-':
        ...
        break;
    case '.':
        if (type_lookup[uint8_t(_lwr[1])] == DIGIT)
            parse_numeric_literal(tok);
        else if (_lwr[1] == '.' && _lwr[2] == '.')
...
{
    _lwr += length;
    _lwr_location.column += static_cast<unsigned int>(length);
}
void reshadefx::lexer::skip_space()
{
    // Skip each character until a space is found
    while (type_lookup[uint8_t(*_lwr)] == SPACE && _lwr < _end)
        skip(1);
}
...
{
    auto *const begin = _lwr, *end = begin;
...
bool reshadefx::lexer::parse_pp_directive(token &tok)
{
    ...
    else if (!_ignore_line_directives && tok.literal_as_string == "line") // The #line directive needs special handling
    {
        ...
        if (_lwr[0] == '"')
        {
            ...
            _lwr_location.source = std::move(temptok.literal_as_string);
        }
...
{
    auto *const begin = _lwr, *end = begin + 1; // Skip first quote character right away
...
void reshadefx::lexer::parse_numeric_literal(token &tok) const
{
    ...
    auto *const begin = _lwr, *end = _lwr;
    ...
    for (; mantissa_size <= 18; mantissa_size++, end++)
    {
        ...
        else
        {
            ...
            // Found a decimal character, as such colwert current values
            if (radix == 8)
            {
                radix = 10;
