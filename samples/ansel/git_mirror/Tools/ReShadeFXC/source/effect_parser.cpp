...
bool reshadefx::parser::accept_symbol(std::string &identifier, scope &scope, symbol &symbol)
{
    ...
    scope = { "::", 0, 0 };
    if (!exclusive) scope = lwrrent_scope();
...
bool reshadefx::parser::parse_expression(expression &exp)
{
    ...
    while (accept(','))
        // Overwrite 'exp' since colweniently the last expression in the sequence is the result
        if (!parse_expression_assignment(exp))
            return false;
...
bool reshadefx::parser::parse_expression_unary(expression &exp)
{
        ...
        if (type cast_type; accept_type_class(cast_type))
        {
            ...
            else if (expect(')'))
            {
                ...
                if (!type::rank(exp.type, cast_type))
                    return error(location, 3017, "cannot colwert these types (from " + exp.type.description() + " to " + cast_type.description() + ')'), false;
    ...
    {
        // Colwert double literal to float literal for now
    ...
    else if (type type; accept_type_class(type)) // Check if this is a constructor call expression
    {
        ...
        while (!peek(')'))
        {
            ...
            if (!argument.type.is_numeric())
                return error(argument.location, 3017, "cannot colwert non-numeric types"), false;
        ...
        if (is_constant) // Constants can be colwerted at compile time
        {
            constant res = {};
            unsigned int i = 0;
        ...
        else if (arguments.size() > 1)
        {
            ...
            for (auto it = arguments.begin(); it != arguments.end();)
            {
                ...
                else
                {
                    ...
                    // Colwert to a scalar value and re-enter the loop in the next iteration (in case a cast is necessary too)
    ...
    else
    {
        ...
        if (accept('('))
        {
            ...
            // Check if the call resolving found an intrinsic or function and ilwoke the corresponding code
...
bool reshadefx::parser::parse_expression_multary(expression &lhs, unsigned int left_precedence)
{
    ...
    while (peek_multary_op(right_precedence))
    {
        ...
        if (op != tokenid::question)
        {
...
#if RESHADEFX_SHORT_CIRLWIT
            codegen::id lhs_block = 0;
            codegen::id rhs_block = 0;
            codegen::id merge_block = 0;

            // Switch block to a new one before parsing right-hand side value in case it needs to be skipped during short-cirlwiting
            ...
            // Deduce the result base type based on implicit colwersion rules
            ...
            // Perform implicit type colwersion
        ...
        else
        {
...
#if RESHADEFX_SHORT_CIRLWIT
            // Switch block to a new one before parsing first part in case it needs to be skipped during short-cirlwiting
...
#if RESHADEFX_SHORT_CIRLWIT
            // Switch block to a new one before parsing second part in case it needs to be skipped during short-cirlwiting
            ...
            // Check that the two value expressions can be colwerted between each other
...
bool reshadefx::parser::parse_expression_assignment(expression &lhs)
{
    ...
    if (accept_assignment_op())
    {
        ...
        if (!type::rank(lhs.type, rhs.type))
            return error(rhs.location, 3020, "cannot colwert these types (from " + rhs.type.description() + " to " + lhs.type.description() + ')'), false;

        // Cannot perform bitwise operations on non-integral types
        if (!lhs.type.is_integral() && (op == tokenid::ampersand_equal || op == tokenid::pipe_equal || op == tokenid::caret_equal))
            return error(lhs.location, 3082, "int or unsigned int type required"), false;

        // Perform implicit type colwersion of right hand side value
...
bool reshadefx::parser::parse_statement(bool scoped)
{
    ...
    if (_codegen->is_in_function())
    {
        ...
        {
            codegen::id true_block = _codegen->create_block(); // Block which contains the statements exelwted when the condition is true
            codegen::id false_block = _codegen->create_block(); // Block which contains the statements exelwted when the condition is false
            const codegen::id merge_block = _codegen->create_block(); // Block that is exelwted after the branch re-merged with the current control flow
            ...
            // Load condition and colwert to boolean value as required by 'OpBranchConditional'
        ...
        {
            const codegen::id merge_block = _codegen->create_block(); // Block that is exelwted after the switch re-merged with the current control flow
            ...
            // Load selector and colwert to integral value as required by switch instruction
            ...
            while (!peek(tokenid::end_of_file))
            {
                ...
                if (peek(tokenid::case_) || peek(tokenid::default_) || end_of_switch)
                {
                    ...
                    const codegen::id lwrrent_block = _codegen->leave_block_and_branch(next_block);

                    assert(lwrrent_block != 0);

                    if (default_label == 0)
                        default_label = lwrrent_block;
                    else
                        for (size_t i = last_case_label_index; i < case_literal_and_labels.size(); i += 2)
                            case_literal_and_labels[i + 1] = lwrrent_block;
        ...
        if (accept(tokenid::for_))
        {
            ...
            const codegen::id merge_block = _codegen->create_block(); // Block that is exelwted after the loop
        ...
        {
            const type &ret_type = _lwrrent_return_type;
...
bool reshadefx::parser::parse_top()
{
    if (accept(tokenid::namespace_))
    {
        ...
        // Relwrsively parse top level statements until the namespace is closed again
...
bool reshadefx::parser::parse_struct()
{
    ...
    info.unique_name = 'S' + lwrrent_scope().name + info.name;
...
bool reshadefx::parser::parse_function(type type, std::string name)
{
    ...
    function_info info;
    info.name = name;
    info.unique_name = 'F' + lwrrent_scope().name + name;
    std::replace(info.unique_name.begin(), info.unique_name.end(), ':', '_');

    info.return_type = type;
    _lwrrent_return_type = info.return_type;
...
bool reshadefx::parser::parse_variable(type type, std::string name, bool global)
{
    ...
    else
    {
        ...
        if (accept('='))
        {
            ...
            if ((initializer.type.rows < type.rows || initializer.type.cols < type.cols) && !initializer.type.is_scalar())
                return error(initializer.location, 3017, '\'' + name + "': cannot implicitly colwert these vector types (from " + initializer.type.description() + " to " + type.description() + ')'), false;
    ...
    else if (type.is_texture())
    {
        ...
        texture_info.unique_name = 'V' + lwrrent_scope().name + name;
    ...
    else if (type.is_sampler())
    {
        ...
        // Add namespace scope to avoid name clashes
        sampler_info.unique_name = 'V' + lwrrent_scope().name + name;
    ...
    else
    {
        ...
        std::string unique_name = global ? 'V' + lwrrent_scope().name + name : name;
        ...
        else // Handle the rest of the pass states
        {
            ...
            if (accept(tokenid::identifier)) // Handle special enumeration names for pass states
            {
                ...
                static const std::unordered_map<std::string, uint32_t> s_enum_values = {
                    ...
                    { "ILWSRCCOLOR", uint32_t(pass_blend_func::ilw_src_color) },
                    { "ILWSRCALPHA", uint32_t(pass_blend_func::ilw_src_alpha) },
                    { "DESTCOLOR", uint32_t(pass_blend_func::dst_color) },
                    { "DESTALPHA", uint32_t(pass_blend_func::dst_alpha) },
                    { "ILWDESTCOLOR", uint32_t(pass_blend_func::ilw_dst_color) },
                    { "ILWDESTALPHA", uint32_t(pass_blend_func::ilw_dst_alpha) },
                    { "KEEP", uint32_t(pass_stencil_op::keep) },
                    { "REPLACE", uint32_t(pass_stencil_op::replace) },
                    { "ILWERT", uint32_t(pass_stencil_op::ilwert) },
