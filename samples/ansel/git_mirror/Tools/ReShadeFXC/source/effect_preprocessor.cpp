...
#ifndef _WIN32
    // On Linux systems the native path encoding is UTF-8 already, so no colwersion necessary
...
bool reshadefx::preprocessor::consume()
{
    ...
    _lwrrent_token_raw_data = input.lexer->input_string().substr(_token.offset, _token.length);

    // Get the next token
    input.next_token = input.lexer->lex();

    // Verify string literals (since the lexer cannot throw errors itself)
    if (_token == tokenid::string_literal && _lwrrent_token_raw_data.back() != '\"')
...
void reshadefx::preprocessor::parse_def()
{
    ...
    if (_input_stack[_lwrrent_input_index].lexer->input_string()[macro_name_end_offset] == '(')
    {
        ...
        if (accept(tokenid::ellipsis))
        {
            ...
            error(_token.location, "variadic macros are not lwrrently supported");
            return;
        }
...
void reshadefx::preprocessor::parse_if()
{
    ...
    level.input_index = _lwrrent_input_index;
...
void reshadefx::preprocessor::parse_pragma()
{
    ...
    while (!peek(tokenid::end_of_line) && !peek(tokenid::end_of_file))
    {
        ...
        pragma += _lwrrent_token_raw_data;
    }
...
void reshadefx::preprocessor::parse_include()
{
    ...
    // Detect relwrsive include and abort to avoid infinite loop
    ...
    {
        error(_token.location, "relwrsive #include");
        return;
    }
...
bool reshadefx::preprocessor::evaluate_identifier_as_macro()
{
    ...
    const std::unordered_set<std::string> &hidden_macros = _input_stack[_lwrrent_input_index].hidden_macros;
    if (hidden_macros.find(_token.literal_as_string) != hidden_macros.end())
        return false;

    if (_relwrsion_count++ >= 256)
    {
        error(_token.location, "macro relwrsion too high");
        return false;
    }
    ...
    if (it->second.is_function_like)
    {
        ...
        while (true)
        {
            ...
            while (true)
            {
                ...
                argument += _lwrrent_token_raw_data;
            }
    ...
    if (!input.empty())
    {
        ...
        _input_stack[_lwrrent_input_index].hidden_macros.insert(it->first);
    }
    ...
    for (auto it = macro.replacement_list.begin(); it != macro.replacement_list.end(); ++it)
    {
        ...
        case macro_replacement_argument:
            ...
            while (!accept(tokenid::unknown))
            {
                ...
                if (_token == tokenid::identifier && evaluate_identifier_as_macro())
                    continue;
                out += _lwrrent_token_raw_data;
            }
            assert(_lwrrent_token_raw_data[0] == macro_replacement_argument);
            break;
        }
    }
}
void reshadefx::preprocessor::create_macro_replacement_list(macro &macro)
{
    ...
    while (!peek(tokenid::end_of_file) && !peek(tokenid::end_of_line))
    {
        ...
        macro.replacement_list += _lwrrent_token_raw_data;
    }
}
