...
unsigned int reshadefx::type::rank(const type &src, const type &dst)
{
    ...
    //  - Integer to floating point promotion has a higher rank than floating point to integer colwersion
    //  - Signed to unsigned integer colwersion has a higher rank than unsigned to signed integer colwersion
    ...
    if ((src.is_vector() && dst.is_scalar()) || (src.is_vector() == dst.is_vector() && src.rows > dst.rows && src.cols >= dst.cols))
        return rank >> 2; // Vector to scalar colwersion has an even lower rank
    if ((src.is_vector() != dst.is_vector()) || src.is_matrix() != dst.is_matrix() || src.components() != dst.components())
        return 0; // If components weren't colwerted at this point, the types are not compatible
...
{
    _lwrrent_scope.name = "::";
    _lwrrent_scope.level = 0;
    _lwrrent_scope.namespace_level = 0;
}

void reshadefx::symbol_table::enter_scope()
{
    _lwrrent_scope.level++;
}
void reshadefx::symbol_table::enter_namespace(const std::string &name)
{
    _lwrrent_scope.name += name + "::";
    _lwrrent_scope.level++;
    _lwrrent_scope.namespace_level++;
}
void reshadefx::symbol_table::leave_scope()
{
    assert(_lwrrent_scope.level > 0);
    ...
    for (auto &symbol : _symbol_stack)
    {
        ...
        for (auto scope_it = scope_list.begin(); scope_it != scope_list.end();)
        {
            if (scope_it->scope.level > scope_it->scope.namespace_level &&
                scope_it->scope.level >= _lwrrent_scope.level)
            {
...
{
    assert(_lwrrent_scope.level > 0);
    assert(_lwrrent_scope.namespace_level > 0);

    _lwrrent_scope.name.erase(_lwrrent_scope.name.substr(0, _lwrrent_scope.name.size() - 2).rfind("::") + 2);
    _lwrrent_scope.level--;
    _lwrrent_scope.namespace_level--;
}
...
bool reshadefx::symbol_table::insert_symbol(const std::string &name, const symbol &symbol, bool global)
{
    ...
    if (symbol.op != symbol_type::function && find_symbol(name, _lwrrent_scope, true).id != 0)
        return false;
    ...
    if (global)
    {
        ...
        for (size_t pos = 0; pos != std::string::npos; pos = _lwrrent_scope.name.find("::", pos))
        {
            // Extract scope name
            scope.name = _lwrrent_scope.name.substr(0, pos += 2);
            const auto previous_scope_name = _lwrrent_scope.name.substr(pos);
    ...
    else
    {
        ...
        insert_sorted(_symbol_stack[name], scoped_symbol { symbol, _lwrrent_scope });
    }
...
reshadefx::symbol reshadefx::symbol_table::find_symbol(const std::string &name) const
{
    ...
    return find_symbol(name, _lwrrent_scope, false);
}
