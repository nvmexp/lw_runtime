...
public:
    ...
    bool isNumber() const;
    // Returns number. Colwert to double.
    double asNumber() const;

    // ----------------------------------------------------------------------
    // For Time value

    // Colwerts to time_t if the internal value is Time.
    // We don't have as<std::time_t>(). Since time_t is basically signed long,
    // it's something like a method to colwerting to (normal) integer.
...
private:
    ...
    template<typename T> struct ValueColwerter;
    ...
    template<typename T> friend struct ValueColwerter;
};
...
inline Token Lexer::parseAsTime(const std::string& str)
{
    ...
    char pn;
    int oh, om;
    if (sscanf_s(s, "%c%d:%d", &pn, (unsigned int)sizeof(char), &oh, &om) != 3)
        return Token(TokenType::ERROR, std::string("Invalid token"));
...
template<typename T>
struct Value::ValueColwerter<std::vector<T>>
{
...
{
    return ValueColwerter<T>().is(*this);
}

template<typename T>
inline typename call_traits<T>::return_type Value::as() const
{
    return ValueColwerter<T>().to(*this);
}
...
inline void Value::write(std::ostream* os, const std::string& keyPrefix, int indent) const
{
    ...
    case TIME_TYPE: {
        ...
        char buf[bufSize];
        sprintf_s(buf, bufSize, "%04d-%02d-%02dT%02d:%02d:%02dZ", t.tm_year + 1900, t.tm_mon + 1, t.tm_mday, t.tm_hour, t.tm_min, t.tm_sec);
        (*os) << buf;
        break;
    }
    case ARRAY_TYPE:
...
inline Value Parser::parse()
{
    ...
    while (true) {
        ...
        if (token().type() == TokenType::LBRACKET) {
            lwrrentValue = parseGroupKey(&root);
            if (!lwrrentValue) {
...
inline Value* Parser::parseGroupKey(Value* root)
{
    ...
    while (true) {
        ...
        if (token().type() == TokenType::DOT) {
            nextKey();
            if (Value* candidate = lwrrentValue->findChild(key)) {
                if (candidate->is<Array>() && candidate->size() > 0)
                    candidate = candidate->find(candidate->size() - 1);
                if (!candidate->is<Table>())
                    return nullptr;
                lwrrentValue = candidate;
            } else {
                lwrrentValue = lwrrentValue->setChild(key, Table());
            }
            continue;
        }
            ...
            if (Value* candidate = lwrrentValue->findChild(key)) {
                if (isArray) {
                    if (!candidate->is<Array>())
                        return nullptr;
                    lwrrentValue = candidate->push(Table());
                } else {
                    if (candidate->is<Array>() && candidate->size() > 0)
                        candidate = candidate->find(candidate->size() - 1);
                    if (!candidate->is<Table>())
                ...
                if (isArray) {
                    lwrrentValue = lwrrentValue->setChild(key, Array());
                    lwrrentValue = lwrrentValue->push(Table());
                } else {
                    lwrrentValue = lwrrentValue->setChild(key, Table());
                }
            }
