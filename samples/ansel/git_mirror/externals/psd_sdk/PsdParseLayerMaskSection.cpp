...
namespace
{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	template <typename T>
	void EndianColwert(void* src, unsigned int width, unsigned int height)
	{
	...
	static void* ReadChannelDataRaw(SyncFileReader& reader, Allocator* allocator, unsigned int width, unsigned int height)
	{
		...
		if (size > 0)
		{
			...
			EndianColwert<T>(planarData, width, height);
...
void ExtractLayer(const Document* document, File* file, Allocator* allocator, Layer* layer)
{
	...
	// now move channel data to our own data structures for layer and vector masks, ilwalidating the info stored in
	// that channel.
	for (unsigned int i=0; i < channelCount; ++i)
	{
		Channel* channel = &layer->channels[i];
