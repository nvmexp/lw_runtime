...
namespace
{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	template <typename T>
	void EndianColwert(PlanarImage* images, unsigned int width, unsigned int height, unsigned int channelCount)
	{
...
ImageDataSection* ParseImageDataSection(const Document* document, File* file, Allocator* allocator)
{
	...
	// endian-colwert the data
	switch (bitsPerChannel)
	{
		case 8:
			EndianColwert<uint8_t>(imageData->images, width, height, channelCount);
			break;

		case 16:
			EndianColwert<uint16_t>(imageData->images, width, height, channelCount);
			break;

		case 32:
			EndianColwert<float32_t>(imageData->images, width, height, channelCount);
			break;
