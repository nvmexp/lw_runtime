...
namespace
{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	// ---------------------------------------------------------------------------------------------------------------------
	static uint32_t GetLayerInfoSectionLength(ExportDolwment* document)
	{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	// ---------------------------------------------------------------------------------------------------------------------
	static uint32_t GetMetaDataResourceSize(ExportDolwment* document)
	{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	// ---------------------------------------------------------------------------------------------------------------------
	static uint32_t GetIccProfileResourceSize(ExportDolwment* document)
	{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	// ---------------------------------------------------------------------------------------------------------------------
	static uint32_t GetExifDataResourceSize(ExportDolwment* document)
	{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	// ---------------------------------------------------------------------------------------------------------------------
	static uint32_t GetThumbnailResourceSize(ExportDolwment* document)
	{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	// ---------------------------------------------------------------------------------------------------------------------
	static uint32_t GetDisplayInfoResourceSize(ExportDolwment* document)
	{
	...
	// ---------------------------------------------------------------------------------------------------------------------
	// ---------------------------------------------------------------------------------------------------------------------
	static uint32_t GetChannelNamesResourceSize(ExportDolwment* document)
	{
		size_t size = 0u;
	...
	// ---------------------------------------------------------------------------------------------------------------------
	// ---------------------------------------------------------------------------------------------------------------------
	static uint32_t GetUnicodeChannelNamesResourceSize(ExportDolwment* document)
	{
		size_t size = 0u;
		for (unsigned int i = 0u; i < document->alphaChannelCount; ++i)
		{
...
{
	ExportDolwment* document = memoryUtil::Allocate<ExportDolwment>(allocator);
	memset(document, 0, sizeof(ExportDolwment));

	document->width = calwasWidth;
	document->height = calwasHeight;
...
void CreateDataZipPrediction<float32_t>(Allocator* allocator, ExportLayer* layer, unsigned int channelIndex, const float32_t* planarData, uint32_t width, uint32_t height)
{
	...
	// float data is first colwerted into planar data to allow for better compression.
	...
	// the data is also colwerted to big-endian in the same loop.
...
// ---------------------------------------------------------------------------------------------------------------------
template <typename T>
void UpdateLayerImpl(ExportDolwment* document, Allocator* allocator, unsigned int layerIndex, exportChannel::Enum channel, int left, int top, int right, int bottom, const T* planarData, compressionType::Enum compression)
{
	...
	{
		// raw data, copy directly and colwert to big endian
...
// ---------------------------------------------------------------------------------------------------------------------
template <typename T>
void UpdateChannelImpl(ExportDolwment* document, Allocator* allocator, unsigned int channelIndex, const T* data)
{
	// free old data
...
// ---------------------------------------------------------------------------------------------------------------------
template <typename T>
void UpdateMergedImageImpl(ExportDolwment* document, Allocator* allocator, const T* planarDataR, const T* planarDataG, const T* planarDataB)
{
	// free old data
	memoryUtil::FreeArray(allocator, document->mergedImageData[0]);
	memoryUtil::FreeArray(allocator, document->mergedImageData[1]);
...
void WriteDolwment(ExportDolwment* document, Allocator* allocator, File* file)
{
	...
	// channel count
	const uint16_t dolwmentChannelCount = static_cast<uint16_t>(document->colorMode + document->alphaChannelCount);
	fileUtil::WriteToFileBE(writer, dolwmentChannelCount);
	...
	if (document->bitsPerChannel == 32u)
	{
		...
		// this whole section is undolwmented. there's no information to be found on the web.
	...
	// additionally, for dolwments of a certain size, Photoshop also expects merged data to be there.
