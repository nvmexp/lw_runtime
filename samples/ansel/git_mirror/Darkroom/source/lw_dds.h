...
namespace lw_dds
{
    ...
    // dwCaps2 flags
    const unsigned int DDSF_LWBEMAP         = 0x00000200l;
    const unsigned int DDSF_LWBEMAP_POSITIVEX  = 0x00000400l;
    const unsigned int DDSF_LWBEMAP_NEGATIVEX  = 0x00000800l;
    const unsigned int DDSF_LWBEMAP_POSITIVEY  = 0x00001000l;
    const unsigned int DDSF_LWBEMAP_NEGATIVEY  = 0x00002000l;
    const unsigned int DDSF_LWBEMAP_POSITIVEZ  = 0x00004000l;
    const unsigned int DDSF_LWBEMAP_NEGATIVEZ  = 0x00008000l;
    const unsigned int DDSF_LWBEMAP_ALL_FACES  = 0x0000FC00l;
    ...
    class CDDSImage
    {
        public:
            ...
            void create_textureLwbemap(TextureFormat format, unsigned int components,
            ...
            void clear();
            bool load(const wchar_t * filename, bool flipImage = true);
            bool load(FILE* fp, bool flipImage = true);
            bool save(const wchar_t * filename, bool flipImage = true);
            ...
            inline const CTexture &get_lwbemap_face(unsigned int face) const
            {
                assert(m_valid);
                assert(!m_images.empty());
                assert(m_images.size() == 6);
                assert(m_type == TextureLwbemap);
                assert(face < 6);
            ...
            inline bool is_lwbemap() { return (m_type == TextureLwbemap); }
            ...
            inline bool is_dword_aligned()
            {
                ...
                unsigned dwordLineSize = get_dword_aligned_linesize(get_width(), m_components*8);
                unsigned lwrLineSize = get_width() * m_components;
                
                return (dwordLineSize == lwrLineSize);
            }
        ...
        private:
            ...
            // callwlates 4-byte aligned width of image
