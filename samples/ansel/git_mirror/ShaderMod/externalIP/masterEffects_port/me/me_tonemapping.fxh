float4 BorderPass( float4 colorInput, float2 tex )
{
	...
	float  screen_ratio = (BUFFER_WIDTH/BUFFER_HEIGHT);
  	...
  	// -- callwlate the right fBorderWidth for a given fBorderRatio --
...
float3 CartoonPass( float3 colorInput, float2 tex, float2 pixelsize, Texture2D texColor, SamplerState sampState)
{
  	...
  	colorInput.rgb =  pow(edge,CartoonEdgeSlope) * -(CartoonPower * g_sldCartoon + 0.1) + colorInput.rgb;
...
{
	#define white_point_float ( 1.0 / (g_sldLevelsWhite - g_sldLevelsBlack)) 

 	colorInput.rgb = colorInput.rgb * white_point_float - (g_sldLevelsBlack *  white_point_float);
  	return colorInput;
}
...
{
	float hueMid = g_sldHueFXMid;
	float hueRange = g_sldHueFXRange;
...
float3 SharpPass( float3 colorInput, float2 tex, Texture2D texColor, SamplerState sampState)
{
#if 0
...
#else
  	//float diff1 = dot(LumCoeff, texColor.Sample(sampState, tex + pixelsize).rgb);
    float3 blur_ori = texColor.Sample(sampState, tex + float2(0.5 * PixelSize.x,-PixelSize.y * fSharpBias)).rgb*0.25;  	// South South East
    blur_ori += texColor.Sample(sampState, tex + float2(fSharpBias * -PixelSize.x,0.5 * -PixelSize.y)).rgb*0.25; 		// West South West
   	blur_ori += texColor.Sample(sampState, tex + float2(fSharpBias * PixelSize.x,0.5 * PixelSize.y)).rgb*0.25; 		// East North East
    blur_ori += texColor.Sample(sampState, tex + float2(0.5 * -PixelSize.x,PixelSize.y * fSharpBias)).rgb*0.25;		// North North West
#endif
	...
	float3 done = texColor.Sample(sampState, tex).rgb + sharp_luma; 
...
float3 ExplosionPass(float3 colorInput, float2 tex, Texture2D texColor, SamplerState sampState)
{
  	...
  	tex = (-fExplosionRadius * PixelSize) + tex; //Slightly faster this way because it can be callwlated while we callwlate noise.
  
  	colorInput.rgb = texColor.Sample(sampState, (2.0 * fExplosionRadius * PixelSize) * noise + tex).rgb;
