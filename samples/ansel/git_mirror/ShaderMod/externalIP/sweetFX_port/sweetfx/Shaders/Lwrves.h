...
/*
  ...
  Lwrves, uses S-lwrves to increase contrast, without clipping highlights and shadows.
*/

float4 LwrvesPass( float4 colorInput )
{
  float3 lumCoeff = float3(0.2126, 0.7152, 0.0722);  //Values to callwlate luma with
  ...
  // -- Lwrve 21 --
  #if Lwrves_formula == 21 //Cubic catmull
  ...
  // -- Lwrve 22 --
  #if Lwrves_formula == 22 //Cubic Bezier spline
  ...
  // -- Lwrve 23 --
  #if Lwrves_formula == 23 //Cubic Bezier spline - alternative implementation.
