...
#define LwstomLuma float3(0.2126, 0.7152, 0.0722)

float3 LwstomPass(float4 position : SV_Position, float2 texcoord : TEXCOORD0) : SV_Target
{
  ...
  //float luma = dot(colorInput.rgb,LwstomLuma); //Callwlate luma
  //float3 chroma = colorInput.rgb - luma; //Callwlate chroma
  
  float3 color = 1.0 - colorInput; //ilwert the luma
  
  //color = color + chroma; //add the chroma back in
  
  color = lerp(color, colorInput, lwstom_strength); //Adjust the strength of the effect
