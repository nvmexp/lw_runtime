	...
	void SMAAEdgeDetectiolwSWrap( in uint id : SV_VertexID,
																						...
																						out float4 offset2 : TEXCOORD4 )
	{
		float4 offset[3];
		FullscreenTriangle(id, position, texcoord);
		SMAABlendingWeightCallwlatiolwS(texcoord, pixcoord, offset);
	...
	float4 SMAABlendingWeightCallwlationPSWrap(	float4 position : SV_Position,
																							float2 texcoord : TEXCOORD0,
	...
	{
		float4 offset[3] = { offset0, offset1, offset2 };
		return SMAABlendingWeightCallwlationPS(texcoord, pixcoord, offset, edgesSampler, areaSampler, searchSampler, 0.0);
	}
