...
float4 BloomPass( float4 ColorInput2,float2 Tex  )
{
	...
	//float distancetemp = 1.0 - ((MaxDistance - LwrDistance) / MaxDistance);
	...
	for(BloomSample.x = (2.5); BloomSample.x > -2.0; BloomSample.x = BloomSample.x - 1.0) // runs 5 times
	{
		...
		for(BloomSample.y = (- 2.5); BloomSample.y < 2.0; BloomSample.y = BloomSample.y + 1.0) // runs 5 ( * 5) times
		{
			...
			//LwrDistance = sqrt(dot(BloomSample,BloomSample)*BloomWidth); //dot() attempt - same result , same speed. //move x part up ?
			//LwrDistance = sqrt( (distancetemp.y * BloomWidth) + distancetemp.x); //dot() attempt - same result , same speed. //move x part up ?
			LwrDistance = (distancetemp.y * BloomWidth) + distancetemp.x; //removed sqrt
			...
			//BlurColor2.rgb += lerp(Blurtemp.rgb,ColorInput2.rgb, 1 - ((MaxDistance - LwrDistance)/MaxDistance)); //colwert float4 to float3 and check if it's possible to use a MAD
			//BlurColor2.rgb += lerp(Blurtemp.rgb,ColorInput2.rgb, 1.0 - ((MaxDistance - LwrDistance) / MaxDistance)); //colwert float4 to float3 and check if it's possible to use a MAD
			BlurColor2.rgb += lerp(Blurtemp.rgb,ColorInput2.rgb, sqrt(LwrDistance / MaxDistance)); //reduced number of sqrts needed
	...
	float3 BlurColor = BlurColor2.rgb * (BloomPower + 4.0); //check if callwlated offline and combine with line 24 (the blurcolor2 callwlation)
