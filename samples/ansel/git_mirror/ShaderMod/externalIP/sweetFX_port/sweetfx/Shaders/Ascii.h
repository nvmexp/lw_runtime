...
/*
  Ascii by Christian Cann Schuldt Jensen ~ CeeJay.dk
	...
	Colwerts the image to ASCII characters using a greyscale algoritm,
...
float3 AsciiPass( float2 tex )
{
  ...
  float2 lwrsor_position = trunc((screen_size / Ascii_block) * tex) * (Ascii_block / screen_size);
...
/*
	//-- Pattern 1 --
  float3 color = myTex2D(asciiSampler, lwrsor_position + float2(3.5,4.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2(2.5,2.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2(4.5,1.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2(5.5,3.5) * pixel).rgb;
...
/*
	...
	//-- Pattern 1b --
  float3 color = myTex2D(asciiSampler, lwrsor_position + float2(2.5,0.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2(0.5,4.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2(6.5,2.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2(4.5,6.5) * pixel).rgb;
...
*/
  ...
  float3 color = myTex2D(asciiSampler, lwrsor_position + float2( 1.5, 1.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2( 1.5, 3.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2( 1.5, 5.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, lwrsor_position + float2( 0.5, 6.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2( 3.5, 1.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2( 3.5, 3.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2( 3.5, 5.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, lwrsor_position + float2( 2.5, 6.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2( 5.5, 1.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2( 5.5, 3.5) * pixel).rgb;
  color += myTex2D(asciiSampler, lwrsor_position + float2( 5.5, 5.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, lwrsor_position + float2( 4.5, 6.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, lwrsor_position + float2( 6.5, 0.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, lwrsor_position + float2( 6.5, 2.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, lwrsor_position + float2( 6.5, 4.5) * pixel).rgb;
  //color += myTex2D(asciiSampler, lwrsor_position + float2( 6.5, 6.5) * pixel).rgb;
...
/*	
	//-- Pattern 3 - Just one --
	float3 color = myTex2D(asciiSampler, lwrsor_position + float2(4.0,4.0) * pixel)	.rgb;
*/
  ...
  float gray = luma;
	...
	#if Ascii_ilwert_brightness == 1
		gray = 1.0 - gray;
	#endif
  
  //gray = smoothstep(0.0,1.0,gray); //increase contrast
  //gray = lerp(1.0-luma,gray, 0.);
  //gray = lwrsor_position.x; //horizontal test gradient
  //gray = lwrsor_position.y; //vertical test gradient
  //gray = (lwrsor_position.x + lwrsor_position.y) * 0.5; //diagonal test gradient
...
/*
  //Callwlate grid position
  //float grid_position = frac( dot(lwrsor_position,((screen_size / Ascii_block) * float2(0.6,0.8) )) + 0.25);
  float grid_position = frac( dot(lwrsor_position,((screen_size / Ascii_block) * float2(0.75,0.5) )) + 0.25);
  //float grid_position = frac( dot(lwrsor_position,((screen_size / Ascii_block) * float2(0.75,0.5) )) + 0.25);
...
*/  
  ...
  // -- PRNG 1 - Reference --
  float seed = dot(lwrsor_position, float2(12.9898,78.233)); //I could add more salt here if I wanted to
  float sine = sin(seed); //cos also works well. Sincos too if you want 2D noise.
  float noise = frac(sine * 43758.5453 + lwrsor_position.y);
