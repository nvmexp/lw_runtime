...
float4 DPXPass(float4 InputColor){
	...
	float RedLwrve = Red;
	float GreenLwrve = Green;
	float BlueLwrve = Blue;
	
	float3 RGB_Lwrve = float3(Red,Green,Blue);
  ...
  B = B * (1.0 - DPXContrast) + (0.5 * DPXContrast);
    ...
    //B = (1.0 /(1.0 + exp(- RGB_Lwrve * (B - RGB_C))) - (1.0 / (1.0 + exp(RGB_Lwrve / 2.0))))/(1.0 - 2.0 * (1.0 / (1.0 + exp(RGB_Lwrve / 2.0))));
	
    float3 Btemp = (1.0 / (1.0 + exp(RGB_Lwrve / 2.0)));	  
	  B = ((1.0 / (1.0 + exp(-RGB_Lwrve * (B - RGB_C)))) / (-2.0 * Btemp + 1.0)) + (-Btemp / (-2.0 * Btemp + 1.0));


     //TODO use faster code for colwersion between RGB/HSV  -  see http://www.chilliant.com/rgb2hsv.html
