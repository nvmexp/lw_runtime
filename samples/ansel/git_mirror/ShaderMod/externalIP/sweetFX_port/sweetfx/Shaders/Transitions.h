...
void LwrtainOpen(in float4 position : SV_Position, in float2 texcoord : TEXCOORD0, out float4 color : SV_Target)
{
		const float lwrtain_time = 3500.0; //Time it takes for the curtain to slide away. (timeleft - lwrtain_time) will be the time before it starts to slide away.

		float coord = abs(texcoord.x - 0.5);
		float factor = saturate(1.0 - timeleft / lwrtain_time);
...
void LwrtainClose(in float4 position : SV_Position, in float2 texcoord : TEXCOORD0, out float4 color : SV_Target)
{
  float coord = abs(texcoord.x - 0.5);
