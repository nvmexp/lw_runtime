...
#define R (CRTLwrvatureRadius * ((1.0 - g_sldAdvCRTStrength) * 1.0 + 0.5))
...
// Precallwlate a bunch of useful values we'll need in the fragment
// shader.
...
// Callwlate the influence of a scanline on the current pixel.
//
...
float4 AdvancedCRTPass( float4 colorInput, float2 tex )
{
    ...
    // lwrrently rendering, which pixel are we lwrrently rendering?
	...
	// Callwlate Lanczos scaling coefficients describing the effect
	...
	// Callwlate the effective colour of the current and next
	...
	// Callwlate the influence of the current and next scanlines on
	// the current pixel.
	...
	// Colwert the image gamma for display on our output device.
