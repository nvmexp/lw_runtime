...
float3 paint_filter(in VSOUT i, in float pass_id)
{
	...
	for(int j = 0; j < NUM_DIRS; j++)
	{
		...
		{
			float3 col_lwrr = tex2Dlod(qUINT::sBackBufferTex, float4(i.uv.xy + dir * k * qUINT::PIXEL_SIZE, 0, 0)).rgb;
			col_avg_per_dir += col_lwrr;

			float3 color_diff = abs(col_lwrr - col_prev);

			lwrr_divergence += max(max(color_diff.x, color_diff.y), color_diff.z);
			col_prev = col_lwrr;
		}

		[flatten]
		if(lwrr_divergence < min_divergence)
		{
			least_divergent = col_avg_per_dir;
			min_divergence = lwrr_divergence;
		}
