...
HRESULT CFW1DWriteRenderTarget::createRenderTarget(IDWriteFactory *pDWriteFactory) {
        ...
        if(FAILED(hResult)) {
            ...
            if(hDC == NULL) {
                ...
                if(hBrush == NULL) {
                    ...
                    HBITMAP hBitmap = static_cast<HBITMAP>(GetLwrrentObject(hDC, OBJ_BITMAP));
                    if(hBitmap == NULL) {
                        m_lastError = L"GetLwrrentObject failed";
                        hResult = E_FAIL;
                    }
                    else {
                        DIBSECTION dib;
    ...
    if(SUCCEEDED(hResult)) {
        ...
        for(UINT i=0; i < renderingModeCount; ++i) {
            ...
            hResult = pDWriteFactory->CreateLwstomRenderingParams(
                1.0f,
                0.0f,
                0.0f,
...
void CFW1DWriteRenderTarget::initGlyphData(
    ...
    // Callwlate pixel-space coordinates
