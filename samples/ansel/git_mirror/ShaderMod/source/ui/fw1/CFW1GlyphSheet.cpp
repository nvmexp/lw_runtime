...
HRESULT CFW1GlyphSheet::createDeviceResources() {
    ...
    HRESULT hResult = shadermod::Tools::CreateTexture2D(m_pDevice, &textureDesc, NULL, &pTexture);
    if(FAILED(hResult)) {
...
UINT CFW1GlyphSheet::HeightRange::findMin(UINT width, UINT *outMin) {
    ...
    UINT lwrrentMax = findMax(0, width);
    UINT lwrrentMin = lwrrentMax;
    UINT minX = 0;
    
    for(UINT i=1; i < m_totalWidth-width; ++i) {
        if(m_heights[i+width-1] >= lwrrentMax)
            lwrrentMax = m_heights[i+width-1];
        else if(m_heights[i-1] == lwrrentMax) {
            lwrrentMax = findMax(i, width);
            if(lwrrentMax < lwrrentMin) {
                lwrrentMin = lwrrentMax;
                minX = i;
            }
        }
    }
...
UINT CFW1GlyphSheet::HeightRange::findMax(UINT startX, UINT width) {
    UINT lwrrentMax = m_heights[startX];
    for(UINT i=1; i < width; ++i)
        lwrrentMax = std::max(lwrrentMax, m_heights[startX+i]);
