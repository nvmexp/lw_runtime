...
/*****************************************************************************/
DcgmStatCollection::DcgmStatCollection() : m_globalCollection(0),
                                           m_switchOutput(true)
{
    int st;
...
/*****************************************************************************/
DcgmStatCollection::DcgmStatCollection(bool switchOutput) : m_globalCollection(0),
...
{
    if(m_globalCollection || m_gpuCollections.size() > 0 || 
       m_groupedCollections.size() > 0)
        return 0; /* Already initialized */
    
    m_globalCollection = mcollect_alloc();
    if(!m_globalCollection)
    {
        ER(ER_ERR, "NULL from mcollect_alloc()\n");
        return -1;
    }
...
/*****************************************************************************/
void DcgmStatCollection::RemoveEntity(sc_entity_group_t entityGroupId,
...
/*****************************************************************************/
mcollect_p DcgmStatCollection::GetOrCreateEntityCollection(sc_entity_group_t entityGroupId,
                                                       sc_entity_id_t entityId)
{
...
int DcgmStatCollection::CoerceAndSetFromDouble(mcollect_value_p mcValue, double value)
{
    switch(mcValue->type)
    {
        ...
        default:
            ER1(ER_WARN, "Unable to colwert from type DOUBLE to type %d\n", mcValue->type);
            break;
    }
...
/*****************************************************************************/
int DcgmStatCollection::SetGpuStat(unsigned int lwmlGpuIdx, std::string key, double value)
{
    return SetEntityStat(SC_ENTITY_GROUP_GPU, lwmlGpuIdx, key, value);
}

/*****************************************************************************/
int DcgmStatCollection::SetEntityStat(sc_entity_group_t entityGroupId, sc_entity_id_t entityId,
...
int DcgmStatCollection::CoerceAndSetFromInt64(mcollect_value_p mcValue, long long value)
{
    switch(mcValue->type)
    {
        ...
        default:
            ER1(ER_WARN, "Unable to colwert from type INT64 to type %d\n", mcValue->type);
            break;
    }
...
/*****************************************************************************/
int DcgmStatCollection::SetGpuStat(unsigned int lwmlGpuIdx, std::string key, long long value)
{
    return SetEntityStat(SC_ENTITY_GROUP_GPU, lwmlGpuIdx, key, value);
}
...
int DcgmStatCollection::CoerceAndSetFromString(mcollect_value_p mcValue, std::string value)
{
    ...
    default:
        ER1(ER_WARN, "Unable to colwert from type STRING to type %d\n", mcValue->type);
        break;
    }
...
/*****************************************************************************/
int DcgmStatCollection::SetGpuStat(unsigned int lwmlGpuIdx, std::string key, std::string value)
{
    return SetEntityStat(SC_ENTITY_GROUP_GPU, lwmlGpuIdx, key, value);
}
...
/*****************************************************************************/
int DcgmStatCollection::AppendGpuStat(unsigned int lwmlGpuIdx, std::string key, std::string value,
                                  timelib64_t timestamp)
{
    return AppendEntityStat(SC_ENTITY_GROUP_GPU, lwmlGpuIdx, key, value, timestamp);
}

/*****************************************************************************/
int DcgmStatCollection::AppendEntityStat(sc_entity_group_t entityGroupId, sc_entity_id_t entityId,
                                     std::string key, std::string value,
                                     timelib64_t timestamp)
{
    mcollect_value_p mcValue;
...
/*****************************************************************************/
int DcgmStatCollection::AppendGlobalStat(std::string key, std::string value,
                                     timelib64_t timestamp)
{
    mcollect_value_p mcValue;
    mcollect_p mc = 0;
...
/*****************************************************************************/
int DcgmStatCollection::AppendGlobalStat(std::string key, void *value,
                                     int valueSize, timelib64_t timestamp)
{
    mcollect_value_p mcValue;
    mcollect_p mc = 0;
    int st;
...
/*****************************************************************************/
int DcgmStatCollection::AppendGroupedStat(std::string group, std::string key, std::string value,
                                      timelib64_t timestamp)
{
    mcollect_value_p mcValue;
    mcollect_p mc = 0;
...
/*****************************************************************************/
int DcgmStatCollection::AppendGroupedStat(std::string group, std::string key,
                                      void *value, int valueSize, timelib64_t timestamp)
{
    mcollect_value_p mcValue;
    mcollect_p mc = 0;
    int st;
...
/*****************************************************************************/
int DcgmStatCollection::AppendGpuStat(unsigned int lwmlGpuIdx, std::string key,
                                  void *value, int valueSize, timelib64_t timestamp)
{
    return AppendEntityStat(SC_ENTITY_GROUP_GPU, lwmlGpuIdx, key, value, valueSize, timestamp);
}

/*****************************************************************************/
int DcgmStatCollection::AppendEntityStat(sc_entity_group_t entityGroupId, sc_entity_id_t entityId, std::string key,
                                     void *value, int valueSize, timelib64_t timestamp)
{
    mcollect_value_p mcValue;
    mcollect_p mc = 0;
...
/*****************************************************************************/
int DcgmStatCollection::AppendGroupedStat(std::string group, std::string key, double value,
                                      timelib64_t timestamp)
{
    int st;
    mcollect_value_p mcValue;
...
/*****************************************************************************/
int DcgmStatCollection::AppendGroupedStat(std::string group, std::string key, long long value,
                                      timelib64_t timestamp)
{
    int st;
    mcollect_value_p mcValue;
...
/*****************************************************************************/
int DcgmStatCollection::AppendGpuStat(unsigned int lwmlGpuIdx, std::string key,
                                  double value1, double value2, timelib64_t timestamp)
{
    return AppendEntityStat(SC_ENTITY_GROUP_GPU, lwmlGpuIdx, key, value1, value2, timestamp);
}
...
/*****************************************************************************/
int DcgmStatCollection::AppendGpuStat(unsigned int lwmlGpuIdx, std::string key,
                                  long long value1, long long value2, timelib64_t timestamp)
{
    return AppendEntityStat(SC_ENTITY_GROUP_GPU, lwmlGpuIdx, key, value1, value2, timestamp);
}
...
/*****************************************************************************/
int DcgmStatCollection::AppendEntityStat(sc_entity_group_t entityGroupId, sc_entity_id_t entityId,
                                     std::string key, long long value1, long long value2,
                                     timelib64_t timestamp)
{
    int st;
    mcollect_value_p mcValue;
...
/*****************************************************************************/
int DcgmStatCollection::EnforceGlobalStatQuota(std::string key, timelib64_t oldestKeepTimestamp,
                                           int maxKeepEntries) const
{
    ...
    st = timeseries_enforce_quota(mcValue->val.tseries, oldestKeepTimestamp,
                                  maxKeepEntries);
    if(st)
        return -2;
    return 0;
...
/*****************************************************************************/
int DcgmStatCollection::EnforceGroupedStatQuote(std::string group, std::string key,
...
/*****************************************************************************/
int DcgmStatCollection::EnforceEntityStatQuota(sc_entity_group_t entityGroupId, sc_entity_id_t entityId,
                                           std::string key, timelib64_t oldestKeepTimestamp,
                                           int maxKeepEntries)
{
    int st;
    mcollect_value_p mcValue;
...
static int mcollectToStringCB(char *key, mcollect_value_p value, void *userData)
{
    ...
    switch(value->type)
    {
        ...
        case MC_TYPE_TIMESERIES_INT64:
        {
            ...
            for(entry = (timeseries_entry_p)keyedvector_first(kv, &kvLwrsor);
                entry; entry = (timeseries_entry_p)keyedvector_next(kv, &kvLwrsor))
            {
                iterData->stringVal += indent(iterData->indentLevel + 1);
                switch(value->type)
                {
                    case MC_TYPE_TIMESERIES_DOUBLE:
                        snprintf(buf, sizeof(buf)-1, "ts %lld, val %f",
                                 (long long)entry->usecSince1970, entry->val.dbl);
                        iterData->stringVal += buf;
                        break;
                    case MC_TYPE_TIMESERIES_INT64:
                        snprintf(buf, sizeof(buf)-1, "ts %lld, val %lld",
                                 (long long)entry->usecSince1970, entry->val.i64);
                        iterData->stringVal += buf;
                        break;
...
std::string DcgmStatCollection::ToString(void)
{
    ...
    for(groupIt = m_groupedCollections.begin(); groupIt != m_groupedCollections.end();
        ++groupIt)
    {
        retStr += indent(1);
        retStr += "\"";
    ...
    {
        retStr += indent(1);
        snprintf(buf, sizeof(buf)-1, "Lwml Idx %u\n", gpuIt->first);
        retStr += buf;
...
typedef struct sc_merge_fromCB_t
{
    ...
    unsigned int lwmlGpuIdx; /* Which GPU index this is for. 0xFFFFFFFF = none */
...
static int mcollectMergeFromCB(char *key, mcollect_value_p value, void *userData)
{
    ...
    switch(value->type)
    {
        ...
        case MC_TYPE_TIMESTAMP:
        {
            ...
            else if(cbData->lwmlGpuIdx != 0xFFFFFFFF)
            {
                cbData->mergeInto->SetGpuStat(cbData->lwmlGpuIdx, std::string(key),
                                              (long long)value->val.i64);
            }
            else if(cbData->groupName.size() > 0)
            {
                cbData->mergeInto->SetGroupedStat(std::string(cbData->groupName), std::string(key),
                                                  (long long)value->val.i64);
            }
            break;
        }
        ...
        case MC_TYPE_TIMESERIES_BLOB:
        {
                ...
                entry; entry = (timeseries_entry_p)keyedvector_next(kv, &kvLwrsor))
            {
                switch(value->type)
                {
                    ...
                    {
                        if(cbData->isGlobal)
                            cbData->mergeInto->AppendGlobalStat(std::string(key),
                                                                (double)entry->val.dbl,
                                                                entry->usecSince1970);
                        else if(cbData->lwmlGpuIdx != 0xFFFFFFFF)
                        {
                            cbData->mergeInto->AppendGpuStat(cbData->lwmlGpuIdx, std::string(key),
                                                             (double)entry->val.dbl, 0,
                                                             entry->usecSince1970);
                        }
                        else if(cbData->groupName.size() > 0)
                        {
...
int DcgmStatCollection::MergeFrom(DcgmStatCollection *source)
{
    ...
    for(groupIt = source->m_groupedCollections.begin(); groupIt != source->m_groupedCollections.end();
            ++groupIt)
    {
...
/*****************************************************************************/
mcollect_value_p DcgmStatCollection::GetGpuStat(unsigned int lwmlGpuIdx, std::string key)
{
    mcollect_value_p retVal = 0;
    mcollect_p mc = 0;
    std::map<unsigned int,mcollect_p>::iterator it;

    it = m_gpuCollections.find(lwmlGpuIdx);
...
mcollect_value_p DcgmStatCollection::GetEntityStat(sc_entity_group_t entityGroupId,
                                               sc_entity_id_t entityId, std::string key)
{
    mcollect_value_p retVal = 0;
    mcollect_p mc = 0;
    entity_collection_t::iterator it;
...
static int sc_mcollect_equal(mcollect_p a, mcollect_p b, int printDiffs)
{
    ...
    if(dataA.keyValues.size() != dataB.keyValues.size())
    {
        if(printDiffs)
        {
            fprintf(stderr, "Left collection has %d values and right has %d values\n",
                    (int)dataA.keyValues.size(), (int)dataB.keyValues.size());
        }
        return 0;
    }
    ...
    for(dataAIter = dataA.keyValues.begin(); dataAIter != dataA.keyValues.end(); ++dataAIter)
    {
        ...
        if(valueA->type != valueB->type)
        {
            if(printDiffs)
            {
                fprintf(stderr, "Left collection value %s type %d != right type %d\n",
                        dataAIter->first.c_str(), valueA->type, valueB->type);
            }
            return 0;
        }
        ...
        switch(valueA->type)
        {
            ...
            case MC_TYPE_INT64:
                if(valueA->val.i64 != valueB->val.i64)
                {
                    if(printDiffs)
                    {
                        fprintf(stderr, "key %s, Left value %lld != Right value %lld\n",
                                dataAIter->first.c_str(), valueA->val.i64, valueB->val.i64);
                    }
                    return 0;
                }
                break;
            ...
            case MC_TYPE_DOUBLE:
                if(fabs(valueA->val.dbl - valueB->val.dbl) > DBL_EPSILON)
                {
                    if(printDiffs)
                    {
                        fprintf(stderr, "key %s, Left value %f != Right value %f\n",
                                dataAIter->first.c_str(), valueA->val.dbl, valueB->val.dbl);
                    }
                    return 0;
                }
                break;
            ...
            case MC_TYPE_STRING:
                if(strcmp(valueA->val.str, valueB->val.str))
                {
                    if(printDiffs)
                    {
                        fprintf(stderr, "key %s, Left value %s != Right value %s\n",
                                dataAIter->first.c_str(), valueA->val.str, valueB->val.str);
                    }
                    return 0;
                }
                break;
            ...
            case MC_TYPE_TIMESERIES_BLOB:
            {
                kv_lwrsor_t kvLwrsorA, kvLwrsorB;
                ...
                if(kvASize != kvBSize)
                {
                    if(printDiffs)
                    {
                        fprintf(stderr, "key %s, Left TS size %d != Right TS size %d\n",
                                 dataAIter->first.c_str(), kvASize, kvBSize);
                    }
                    return 0;
                }

                for(entryA = (timeseries_entry_p)keyedvector_first(kvA, &kvLwrsorA),
                    entryB = (timeseries_entry_p)keyedvector_first(kvB, &kvLwrsorB),
                    entryIndex = 0;
                    entryA && entryB;
                    entryA = (timeseries_entry_p)keyedvector_next(kvA, &kvLwrsorA),
                    entryB = (timeseries_entry_p)keyedvector_next(kvB, &kvLwrsorB),
                    entryIndex++)
                {
                    if(entryA->usecSince1970 != entryB->usecSince1970)
                    {
                        if(printDiffs)
                        {
                            fprintf(stderr, "key %s, Left entry %d timestamp %lld != Right timestamp %lld\n",
                                    dataAIter->first.c_str(), entryIndex,
                                    (long long)entryA->usecSince1970, (long long)entryB->usecSince1970);
                        }
                        return 0;
                    }
                    ...
                    switch(valueA->type)
                    {
                        case MC_TYPE_TIMESERIES_DOUBLE:
                            ...
                            {
                                fprintf(stderr, "key %s, Left entry %d value %f != Right value %f\n",
                                        dataAIter->first.c_str(), entryIndex,
                                        entryA->val.dbl, entryB->val.dbl);
                                return 0;
                            }
                            break;
                        ...
                        case MC_TYPE_TIMESERIES_INT64:
                            ...
                            {
                                fprintf(stderr, "key %s, Left entry %d value %lld != Right value %lld\n",
                                        dataAIter->first.c_str(), entryIndex,
                                        entryA->val.i64, entryB->val.i64);
                                return 0;
                            }
                            break;
                        ...
                        case MC_TYPE_TIMESERIES_STRING:
                            ...
                            {
                                fprintf(stderr, "key %s, Left entry %d string %s (%d) != Right value %s (%d)\n",
                                        dataAIter->first.c_str(), entryIndex,
                                        (char *)entryA->val.ptr, (int)entryA->val2.ptrSize,
                                        (char *)entryB->val.ptr, (int)entryB->val2.ptrSize);
                                return 0;
                            }
                            break;
                        ...
                        case MC_TYPE_TIMESERIES_BLOB:
                            if(entryA->val2.ptrSize != entryB->val2.ptrSize ||
                               memcmp((const void *)entryA->val.ptr, (const void *)entryB->val.ptr,
                                      (unsigned long int)entryA->val2.ptrSize))
                            {
                                fprintf(stderr, "key %s, Left entry %d len %d != right entry len %d\n",
                                        dataAIter->first.c_str(), entryIndex,
                                        (int)entryA->val2.ptrSize, (int)entryB->val2.ptrSize);
                                return 0;
                            }
                            break;
...
int DcgmStatCollection::EntityGroupEqualTo(sc_entity_group_t entityGroup, DcgmStatCollection *other, int printDiffs)
{
    ...
    if(collection->size() != otherCollection->size())
    {
        if(printDiffs)
        {
            fprintf(stderr, "%s counts mismatch. Left: %d. Right: %d\n",
                    g_entityJsonTags[entityGroup].c_str(), (int)collection->size(),
    ...
    for(entityIt = collection->begin(); entityIt != collection->end(); ++entityIt)
    {
        ...
        if(otherEntityIt == otherCollection->end())
        {
            if(printDiffs)
            {
                fprintf(stderr, "%s %u is present in left but missing from right\n",
...
int DcgmStatCollection::EqualTo(DcgmStatCollection *other, int printDiffs)
{
    ...
    if(m_groupedCollections.size() != other->m_groupedCollections.size())
    {
        if(printDiffs)
        {
            fprintf(stderr, "Group counts mismatch. Left: %d. Right: %d\n",
                    (int)m_groupedCollections.size(), (int)other->m_groupedCollections.size());
        }
...
static int mcollectToJsonCB(char *key, mcollect_value_p value, void *userData)
{
    ...
    switch(value->type)
    {
        ...
        case MC_TYPE_STRING:
        {
            iterData->stringVal += std::string("\"") + std::string(value->val.str) +
                                   std::string("\"");
            break;
        }
        ...
        case MC_TYPE_TIMESERIES_BLOB:
        {
                ...
                entry; entry = (timeseries_entry_p)keyedvector_next(kv, &kvLwrsor))
            {
                ...
                {
                    case MC_TYPE_TIMESERIES_DOUBLE:
                        snprintf(buf, sizeof(buf)-1, "\"timestamp\":%lld,\"value\":%.17g",
                                 (long long)entry->usecSince1970, entry->val.dbl);
                        iterData->stringVal += buf;
                        break;
                    case MC_TYPE_TIMESERIES_INT64:
                        snprintf(buf, sizeof(buf)-1, "\"timestamp\":%lld,\"value\":%lld",
                                 (long long)entry->usecSince1970, entry->val.i64);
                        iterData->stringVal += buf;
                        break;
                    case MC_TYPE_TIMESERIES_STRING:
                        snprintf(buf, sizeof(buf)-1, "\"timestamp\":%lld,\"subtype\":\"%s\",\"value\":\"",
                                 (long long)entry->usecSince1970, SC_JSON_SUBTYPE_STRING);
                        iterData->stringVal += buf;
                        iterData->stringVal += (char *)entry->val.ptr;
                        iterData->stringVal += "\"";
                        break;
                    case MC_TYPE_TIMESERIES_BLOB:
                    {
                        ...
                        snprintf(buf, sizeof(buf)-1, "\"timestamp\":%lld,\"subtype\":\"%s\",\"value\":\"",
                                 (long long)entry->usecSince1970, SC_JSON_SUBTYPE_HEX);
                        iterData->stringVal += buf;
                        for(i=0; i<entry->val2.ptrSize; i++)
                        {
                            c = ((unsigned char *)entry->val.ptr)[i];
...
/*****************************************************************************/
int DcgmStatCollection::GetEntityFieldBytesUsed(sc_entity_group_t entityGroupId, sc_entity_id_t entityId,
                                            std::string key, long long *bytesUsed)
{
    entity_collection_t *collection = GetCollectionByEntityGroupId(entityGroupId);
    if(!collection)
        return -1;

    // ensure that the lwmlGpuIdx is valid
