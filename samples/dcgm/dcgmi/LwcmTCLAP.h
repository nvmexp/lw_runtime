#ifndef _LWCMI_TCLAP_DEFS_H
#define _LWCMI_TCLAP_DEFS_H
...
: CmdLine (message, delimiter, version, helpAndVersion)
{
        ...
        if (!helpAndVersion){
            ...
            TCLAP::SwitchArg* help = new TCLAP::SwitchArg("h","help",
...
private:
    void _shortUsageGroup(TCLAP::CmdLineInterface & _cmd, std::ostream& os)
    {
        ...
        for (groupsIt = groups.begin(); groupsIt != groups.end(); groupsIt++)
        {
            ...
            for (std::list <TCLAP::Arg *>::iterator it = groupList.begin();
                    it != groupList.end(); it++)
            {
                s = s + (*it)->shortID() + " ";
            }
    ...
    virtual void _longUsage( TCLAP::CmdLineInterface& _cmd,
            std::ostream& os ) const
    {
        ...
        for (TCLAP::ArgListIterator it = argList.begin(); it != argList.end(); it++)
            ...
            {
                ss.str("");
                ss << std::left <<  std::setw(17) << std::setfill(' ')<< ((*it)->getFlag().empty() ? "  ":"-") + (*it)->getFlag() + "  --" + (*it)->getName();
...
protected:
            ...
            int secondLineOffset ) const
    {
        ...
        if ( (len + indentSpaces > maxWidth) && maxWidth > 0 )
        {
            ...
            while ( start < len )
            {
                ...
                while ( buffer[stringLen+start] == ' ' && start < len )
                    start++;
    ...
    virtual void failure( TCLAP::CmdLineInterface& _cmd,
...
class DCGMEntryOutput : public DCGMOutput
{
            ...
            std::ostream& os ) const
    {
        ...
        for (TCLAP::ArgListIterator it = argList.begin(); it != argList.end(); it++)
            if ( !xorHandler.contains( (*it) ) && !(*it)->isRequired() )
            {
                std::string prefix = (!(*it)->getName().compare("help") || !(*it)->getName().compare("version"))? "  --" : "    ";
                ss.str("");
                ss << std::left <<  std::setw(17) << std::setfill(' ')<< ((*it)->getFlag().empty() ? "  ":"-") + (*it)->getFlag() + prefix + (*it)->getName();
                DCGMOutput::spacePrint( os, ss.str() +" "+ (*it)->getDescription(), 80, 2, 19 );
            }
