 ...
 */
DeviceInfo::DeviceInfo(std::string hostname, std::string requestedEntityIds, std::string grpIdStr,
                       std::string fldIds, int fieldGroupId, int dlay, int cnt, bool lst, bool watchFields)
{
    deviceMonitorShouldStop = false;

    mHostName = hostname;
    m_requestedEntityIds = requestedEntityIds;
    m_groupIdStr = grpIdStr;
    m_fieldIdsStr = fldIds;
    m_fieldGrpId_int = fieldGroupId;
    m_delay = dlay;
    m_count = cnt;
    m_list = lst;
    m_entityStats.clear();
    m_watchFields = watchFields;
}
 ...
 *
 * @return             : LWML_SUCCESS successful
 */
dcgmReturn_t DeviceInfo::DisplayFieldDetails()
{
    int i=0;
    std::cout << FORMAT_LINE <<std::endl;
    std::cout<< std::setw(WIDTH_20) << "Long Name" << std::setw(WIDTH_40) << "Short Name" << std::setw(WIDTH_15) << "Field Id"<<std::endl;
    std::cout << FORMAT_LINE <<std::endl;
    for(i=0 ; i< (int)m_fieldDetails.size() ; i++)
    {
        std::cout << m_fieldDetails[i].m_longName << std::setw(WIDTH_20 + WIDTH_40 - m_fieldDetails[i].m_longName.length())
                  << m_fieldDetails[i].m_shortName << std::setw(WIDTH_15) << m_fieldDetails[i].m_fieldId << std::endl;
    }
    return DCGM_ST_OK;
}

int DeviceInfo::Execute()
{
    dcgmReturn_t connection = (dcgmReturn_t)Command::Execute();

    if (connection != DCGM_ST_OK)
    {
        PRINT_ERROR("%d", "Error! Unable to connect to host engine. Return: %d", DCGM_ST_BADPARAM);
        std::cout << "Error: Unable to connect to host engine. " << errorString(connection) << "."
                << std::endl;
        return DCGM_ST_BADPARAM;
    }

    m_dcgmHandle = mLwcmHandle;
 ...
 * @return int.
 */
int list_field_values(dcgm_field_entity_group_t entityGroupId, dcgm_field_eid_t entityId, dcgmFieldValue_v1* values, int numValues, void* userdata)
{
    ...
    std::map<dcgmi_entity_pair_t,std::vector<std::string> > &entityStats = *(static_cast<std::map<dcgmi_entity_pair_t, std::vector<std::string> > *> (userdata));
    ...
    for (int i = 0; i < numValues; i++)
    {
        ...
        strs.str(std::string());
        strs << std::fixed;
        strs << std::setprecision(FLOAT_VAL_PREC); // fixing the number of values after decimal for consistency in display.

        /* Failed status? */
        if(values[i].status != DCGM_ST_OK)
        {
            if(values[i].status == DCGM_ST_NOT_WATCHED)
                strs << NW_STR;
            else
                strs << NA_STR;
        ...
        switch (DcgmFieldGetById(values[i].fieldId)->fieldType)
        {
            ...
            case DCGM_FT_DOUBLE:
                ...
                {
                    entityStats[entityKey].push_back(NA_STR);
                }
                else
                {
    ...
    // Program exelwted correctly. Return 0 to notify DCGM (callee) that it
    // was successful.
    return 0;
}
...
dcgmReturn_t DeviceInfo::LockWatchAndUpdate()
{
    ...
    if(m_watchFields)
    {
        result = dcgmWatchFields(m_dcgmHandle, m_myGroupId, m_fieldGroupId, m_delay*MILLI_SEC, MAX_KEEP_AGE, MAX_KEEP_SAMPLES);
        ...
        if (result != DCGM_ST_OK)
        {
            ...
            switch(result)
            {
                ...
                case DCGM_ST_INSUFFICIENT_DRIVER_VERSION:
                    e = "Unable to watch one or more of the requested fields because doing so requires a newer driver version than what is lwrrently installed.";
                    break;
    ...
    while (running && !deviceMonitorShouldStop)
    {
        ...
        for (std::map<dcgmi_entity_pair_t, std::vector<std::string> >::iterator it = m_entityStats.begin();
             it != m_entityStats.end(); ++it)
        {
            std::cout << std::setw(m_widthArray[0]);
            /* Only output the entityId for now. This is fine since gpuIds are 0-15 and LwSwitch IDs are 9xx */
            std::cout << it->first.entityId;
...
/**
 * setHeaderForOutput - The functions intializes the header for tabular output with alignment by
...
void DeviceInfo::SetHeaderForOutput(unsigned short a[], unsigned int b)
{
    ...
    for (i = 0; i < (int)b; i++)
    {
        ...
        unsigned short width = meta_p->valueFormat->width;
        m_widthArray[i+1] = (width+PADDING);
        ss << std::setw(width + (PADDING));
        ss << meta_p->valueFormat->shortName;

        ss_unit << std::setw(width+ PADDING);
        ss_unit << meta_p->valueFormat->unit;
    }
    m_header = ss.str();
...
dcgmReturn_t DeviceInfo::CreateEntityGroupFromEntityList(void)
{
    ...
    dcgmReturn = dcgmi_parse_entity_list_string(m_requestedEntityIds, entityList);
    if(dcgmReturn != DCGM_ST_OK)
        return dcgmReturn;

    /* Create a group based on this list of entities */
    dcgmReturn = dcgmi_create_entity_group(m_dcgmHandle, DCGM_GROUP_EMPTY,
                                           &m_myGroupId, entityList);
    return dcgmReturn;
}
...
dcgmReturn_t DeviceInfo::ValidateOrCreateEntityGroup(void)
{
    ...
    dcgmReturn = dcgmGroupGetInfo(m_dcgmHandle, m_myGroupId, &stLwcmGroupInfo);
...
dcgmReturn_t DeviceInfo::ValidateOrCreateFieldGroup(void)
{
    ...
    if (DCGM_ST_OK != dcgmReturn)
    {
        std::string error = (dcgmReturn == DCGM_ST_NOT_CONFIGURED) ? "The Field Group is not found"
                                                                : errorString(dcgmReturn);
        std::cout << "Error: Unable to retrieve information about field group "
                    << m_fieldGrpId_int << ". Return: " << error << "." << std::endl;
        PRINT_ERROR("%u %d", "Error! retrieving info on field group with ID: %u. Return: %d",
            (int)m_fieldGrpId_int, dcgmReturn);
