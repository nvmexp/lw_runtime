 ...
 *         - \ref DCGM_ST_INIT_ERROR        if an error oclwrred while trying to start the host engine.
 */
 ...
 * by running the lw-hostengine command.
 *
 ...
 *         - \ref DCGM_ST_GENERIC_ERROR     if an unspecified internal error oclwrred
 */
...
/**
 * Get the LwLink link status for every LwLink in this system. This includes the LwLinks of both GPUs and
 * LwSwitches. Note that only LwSwitches and GPUs that are visible to the current environment will be
 * returned in this structure.
 * 
 * @param dcgmHandle  IN: DCGM Handle
 * @param linkStatus OUT: Structure in which to store LwLink link statuses. .version should be set to
 *                        dcgmLwLinkStatus_version1 before calling this. 
 * 
 * @return
 *        - \ref DCGM_ST_OK                if the call was successful.
 *        - \ref DCGM_ST_NOT_SUPPORTED     if the given entityGroup does not support enumeration.
 *        - \ref DCGM_ST_BADPARAM          if any parameter is invalid
 */
dcgmReturn_t DECLDIR dcgmGetLwLinkLinkStatus(dcgmHandle_t dcgmHandle, dcgmLwLinkStatus_v2 *linkStatus);
 ...
 *  DCGM_GROUP_ALL_LWSWITCHES in place of group IDs when needed.
 *  @{
 */
/***************************************************************************************************/
 ...
 * returned in \a pDcgmGrpId. Instead of exelwting an operation separately for each entity, the
 ...
 * specified as \a DCGM_GROUP_DEFAULT or \a DCGM_GROUP_ALL_LWSWITCHES. To create an empty group, 
 ...
 * using the API \ref dcgmStatusPopError. The user can ilwoke \ref dcgmStatusPopError 
 ...
 * can ilwoke this API for the number of errors or until all the errors are fetched.
 * @param statusHandle      IN  :   Handle to list of statuses
 ...
 *                                \ref dcgmGroupCreate for details on creating the group.
 *                                Alternatively, pass in the group id as \a DCGM_GROUP_ALL_GPUS
 *                                to perform operation on all the GPUs or \a DCGM_GROUP_ALL_LWSWITCHES to
 *                                to perform the operation on all LwSwitches.
 ...
 * Request updates for all field values that have updated since a given timestamp
 * 
 * This version works with non-GPU entities like LwSwitches
 *
 * @param pDcgmHandle         IN: DCGM Handle
 * @param groupId             IN: Group ID representing collection of one or more entities. Look at
 *                                \ref dcgmGroupCreate for details on creating the group.
 *                                Alternatively, pass in the group id as \a DCGM_GROUP_ALL_GPUS
 *                                to perform operation on all the GPUs or \a DCGM_GROUP_ALL_LWSWITCHES to
 *                                perform the operation on all LwSwitches.
 * @param fieldGroupId        IN: Fields to return data for
 * @param sinceTimestamp      IN: Timestamp to request values since in usec since 1970. This will
 *                                be returned in nextSinceTimestamp for subsequent calls
 *                                0 = request all data
 ...
 *       in order to optimize calls to the LWPU driver. 
 * 
 * @param pDcgmHandle        IN: DCGM Handle
 ...
 *        - \ref DCGM_ST_GENERIC_ERROR        if an unspecified DCGM error oclwrs
 */
 ...
 *                                            Run "lwpu-smi -am 1" as root on the node before starting
 *                                            DCGM to fix this.
 */
 ...
 * The client can ilwoke DCGM APIs to start and stop collecting the stats at the process boundaries 
...
/**
 * This API is used by the client to notify DCGM about the job to be started. Should be ilwoked as
 ...
 * by job id. Should be ilwoked as part of job epilogue.
 ...
 *                                          Alternatively, pass in the group id as \a DCGM_GROUP_ALL_GPUS
 *                                          to perform operation on all the GPUs or \a DCGM_GROUP_ALL_LWSWITCHES
 *                                          to perform operation on all the LwSwitches.
 ...
 * \ref dcgmPolicy_t structure, if a violation has oclwrred, subsequent action(s) may be performed to
 ...
 *        - DCGM_ST_*                         a different error has oclwrred and is stored in \a statusHandle.
 ...
 *        - \ref DCGM_ST_GENERIC_ERROR        an internal error has oclwrred
 *        - \ref DCGM_ST_GROUP_INCOMPATIBLE   if \a groupId refers to a group of non-homogeneous GPUs. This is lwrrently not allowed.
 */
dcgmReturn_t dcgmActiolwalidate(dcgmHandle_t pDcgmHandle, dcgmGpuGrp_t groupId, dcgmPolicyValidation_t validate,
 ...
 *        - \ref DCGM_ST_GROUP_INCOMPATIBLE   if \a groupId refers to a group of non-homogeneous GPUs. This is lwrrently not allowed.
 */
dcgmReturn_t dcgmActiolwalidate_v2(dcgmHandle_t pDcgmHandle, dcgmRunDiag_t *drd, dcgmDiagResponse_t *response);
...
/*************************************************************************/
/**
 * Retrieve the total amount of memory that the hostengine process is lwrrently using.
...
/*************************************************************************/
/**
 * Get introspection info relating to exelwtion time needed to update the fields
 ...
 * You can also pass --blacklist-modules to the lw-hostengine binary to make sure modules
 ...
 * 
 * Some groups cannot be watched conlwrrently as others as they utilize the same hardware resource. For instance,
 ...
 * DCGM_FI_PROF_LWLINK_TX_DATA.
 * 
 * Metrics that can be watched conlwrrently will have different .majorId fields in their dcgmProfMetricGroupInfo_t
 * 
 ...
 *        - \ref DCGM_ST_PROFILING_MULTI_PASS if any of the metric groups could not be watched conlwrrently due to
