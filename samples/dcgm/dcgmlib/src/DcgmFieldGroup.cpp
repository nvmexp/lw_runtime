...
static volatile unsigned int g_nextFieldGrpId = 0; /* Next field group ID to use. */


/*****************************************************************************/
DcgmFieldGroup::DcgmFieldGroup(unsigned int fieldGroupId, std::vector<unsigned short>&fieldIds,
                               std::string name, DcgmWatcher watcher)
{
    m_id = fieldGroupId;
    m_fieldIds = fieldIds;
    m_name = name;
                                                  ...
                                                  DcgmWatcher watcher)
{
    ...
    std::map<unsigned int,DcgmFieldGroup *>::iterator fieldGrpIter;
    DcgmFieldGroup *newFieldGrpObj = 0;
    ...
    newFieldGrpId = lwosInterlockedIncrement(&g_nextFieldGrpId);

    newFieldGrpObj = new DcgmFieldGroup(newFieldGrpId, fieldIds, name, watcher);
    if(!newFieldGrpObj)
    {
        Unlock();
    ...
    m_fieldGroups[newFieldGrpId] = newFieldGrpObj;
    ...
    *fieldGrp = (dcgmFieldGrp_t)(intptr_t)newFieldGrpId;
    PRINT_DEBUG("%u %s %u", "Added field group id %u, name %s, connectionId %u", 
                newFieldGrpId, name.c_str(), watcher.connectionId);
    return DCGM_ST_OK;
}
...
dcgmReturn_t DcgmFieldGroupManager::RemoveFieldGroup(dcgmFieldGrp_t fieldGrp, DcgmWatcher callerWatcher)
{
    ...
    if(fieldGroupWatcher.connectionId != DCGM_CONNECTION_ID_NONE)
    {
        ...
        else
        {
            ...
            {
                PRINT_ERROR("%u %u", "fieldGroupId %u missing from connection %u",
                            fieldGrpId, fieldGroupWatcher.connectionId);
            }
            else
            {
                connectionIt->second.erase(fieldGroupIdIter);
                PRINT_DEBUG("%u %u", "Removed fieldGroupId %u from connection %u in m_connectionFieldGroupIds",
                            fieldGrpId, fieldGroupWatcher.connectionId);
            }
        }
    }
...
/*****************************************************************************/
dcgmReturn_t DcgmFieldGroupManager::GetFieldGroupFields(dcgmFieldGrp_t fieldGrp,
                                                        std::vector<unsigned short> &fieldIds)
{
...
dcgmReturn_t DcgmFieldGroupManager::PopulateFieldGroupInfo(dcgmFieldGroupInfo_t *fieldGroupInfo)
{
    ...
    strncpy(fieldGroupInfo->fieldGroupName,
            fieldGrpObj->GetName().c_str(),
            sizeof(fieldGroupInfo->fieldGroupName)-1);
...
dcgmReturn_t DcgmFieldGroupManager::PopulateFieldGroupGetAll(dcgmAllFieldGroup_t *allGroupInfo)
{
    ...
    for(fieldGrpIter = m_fieldGroups.begin(); fieldGrpIter != m_fieldGroups.end(); fieldGrpIter++)
    {
        ...
        strncpy(allGroupInfo->fieldGroups[allGroupInfo->numFieldGroups].fieldGroupName,
                fieldGrpObj->GetName().c_str(),
                sizeof(allGroupInfo->fieldGroups[allGroupInfo->numFieldGroups].fieldGroupName)-1);

        allGroupInfo->fieldGroups[allGroupInfo->numFieldGroups].fieldGroupId = (dcgmFieldGrp_t)(intptr_t)fieldGrpObj->GetId();
...
/*****************************************************************************/
void DcgmFieldGroupManager::OnConnectionRemove(dcgm_connection_id_t connectionId, LwcmServerConnection *pConnection)
{
    dcgmReturn_t dcgmReturn;
    ...
    for(uint_it = groupIdsToRemove.begin(); uint_it != groupIdsToRemove.end(); ++uint_it)
    {
        ...
        {
            PRINT_WARNING("%u %d", "RemoveFieldGroup of fieldGroupId %u returned %d.", 
                          *uint_it, (int)dcgmReturn);
        }
    }
}
