...
#define DCGM_FR_LWLINK_ERROR_THRESHOLD_MSG      "Detected %ld %s LwLink errors on GPU %u's LWLink which exceeds "\
                                                "threshold of %u"
// gpu id, lwlink id
#define DCGM_FR_LWLINK_DOWN_MSG                 "GPU %u's LwLink link %d is lwrrently down"
// lwswitch id, lwlink id
#define DCGM_FR_LWSWITCH_FATAL_ERROR_MSG        "Detected fatal errors on LwSwitch %u link %u"
// lwswitch id, lwlink id
#define DCGM_FR_LWSWITCH_NON_FATAL_ERROR_MSG    "Detected nonfatal errors on LwSwitch %u link %u"
// lwswitch id, lwlink port
#define DCGM_FR_LWSWITCH_DOWN_MSG               "LwSwitch physical ID %u's LwLink port %d is lwrrently down."
...
#define DCGM_FR_LWML_API_MSG                    "Error calling LWML API %s: %s"
#define DCGM_FR_DEVICE_COUNT_MISMATCH_MSG       "The number of devices LWML returns is different than the number "\
                                                "of devices in /dev."
// function name
...
#define DCGM_FR_LWML_LIB_BAD_MSG                "Cannot get pointer to %s from liblwidia-ml.so"
#define DCGM_FR_GRAPHICS_PROCESSES_MSG          "LWVS has detected graphics processes running on at least one "\
...
#define DCGM_FR_BAD_LWDA_ELW_MSG                "Found LWCA performance-limiting environment variable '%s'."
// gpu id
#define DCGM_FR_PERSISTENCE_MODE_MSG            "Persistence mode for GPU %u is lwrrently disabled. The DCGM "\
...
#define DCGM_FR_STRESS_LEVEL_MSG                "Max stress level of %.1f did not reach desired stress level of "\
                                                "%.1f for GPU %u"
// LWCA API name
#define DCGM_FR_LWDA_API_MSG                    "Error using LWCA API %s"
// count, gpu id
...
// gpu id
#define DCGM_FR_LWDA_UNBOUND_MSG                "Lwca GPU %d is no longer bound to a LWCA context...Aborting"
// Test name, gpu id
...
// gpu id
#define DCGM_FR_LWDA_DBE_MSG                    "LWCA APIs have indicated that a double-bit ECC error has "\
                                                "oclwred on GPU %u."
// gpu id
#define DCGM_FR_MEMORY_MISMATCH_MSG             "A memory mismatch was detected on GPU %u, but no error was "\
                                                "reported by LWCA or LWML."
// gpu id, error detail
#define DCGM_FR_LWDA_DEVICE_MSG                 "Unable to find a corresponding LWCA device for GPU %u: '%s'"
...
// gpu id, error detail
#define DCGM_FR_LWDA_CONTEXT_MSG                "GPU %u failed to create a LWCA context: %s"
// DCGM API name
#define DCGM_FR_DCGM_API_MSG                    "Error using DCGM API %s"
#define DCGM_FR_CONLWRRENT_GPUS_MSG             "Unable to run conlwrrent pair bandwidth test without 2 or more "\
                                                "gpus. Skipping"
#define DCGM_FR_TOO_MANY_ERRORS_MSG             "This API can only return up to four errors per system. "\
                                                ...
                                                "communicated."
#define DCGM_FR_LWLINK_CRC_ERROR_THRESHOLD_MSG  "%.1f %s LwLink errors found oclwring per second on GPU %u, "\
                                                "exceeding the limit of 100 per second."
#define DCGM_FR_LWLINK_ERROR_CRITICAL_MSG       "Detected %ld %s LwLink errors on GPU %u's LWLink (should be 0)"
...
#define DCGM_FR_HAD_TO_RESTORE_STATE_MSG        "Had to restore GPU state on LWML GPU(s): %s"
...
#define DCGM_FR_LWLINK_ERROR_THRESHOLD_NEXT     "Monitor the LWLink. It can still perform workload."
#define DCGM_FR_LWLINK_DOWN_NEXT                TRIAGE_RUN_FIELD_DIAG_MSG
#define DCGM_FR_LWSWITCH_FATAL_ERROR_NEXT       TRIAGE_RUN_FIELD_DIAG_MSG
#define DCGM_FR_LWSWITCH_NON_FATAL_ERROR_NEXT   "Monitor the LWSwitch. It can still perform workload."
#define DCGM_FR_LWSWITCH_DOWN_NEXT              ""
#define DCGM_FR_NO_ACCESS_TO_FILE_NEXT          "Check relevant permissions, access, and existence of the file."
#define DCGM_FR_LWML_API_NEXT                   "Check the error condition and ensure that appropriate libraries "\
...
#define DCGM_FR_LWML_LIB_BAD_NEXT               "Make sure that the required version of liblwidia-ml.so "\
...
#define DCGM_FR_BAD_LWDA_ELW_NEXT               "Please unset this environment variable to address test failures."
#define DCGM_FR_PERSISTENCE_MODE_NEXT           "Enable persistence mode by running \"lwpu-smi -i <gpuId> -pm "\
                                                "1 \" as root."
...
#define DCGM_FR_CANNOT_GET_STAT_NEXT            "If running a standalone lw-hostengine, verify that it is up "\
                                                "and responsive."
#define DCGM_FR_STRESS_LEVEL_NEXT               ""
#define DCGM_FR_LWDA_API_NEXT                   ""
#define DCGM_FR_FAULTY_MEMORY_NEXT              TRIAGE_RUN_FIELD_DIAG_MSG
#define DCGM_FR_CANNOT_SET_WATCHES_NEXT         ""
#define DCGM_FR_LWDA_UNBOUND_NEXT               ""
#define DCGM_FR_ECC_DISABLED_NEXT               "Enable ECC memory by running \"lwpu-smi -i <gpuId> -e 1\" "\
                                                "to enable. This may require a GPU reset or reboot to take effect."
#define DCGM_FR_MEMORY_ALLOC_NEXT               ""
#define DCGM_FR_LWDA_DBE_NEXT                   TRIAGE_RUN_FIELD_DIAG_MSG 
#define DCGM_FR_MEMORY_MISMATCH_NEXT            TRIAGE_RUN_FIELD_DIAG_MSG
#define DCGM_FR_LWDA_DEVICE_NEXT                "Make sure LWDA_VISIBLE_DEVICES is not preventing visibility of "\
                                                "this GPU. Also check if LWCA libraries are compatible and "\
                                                "correctly installed."
...
#define DCGM_FR_LWDA_CONTEXT_NEXT               "Please make sure the correct driver version is installed and "\
                                                "verify that no conflicting libraries are present."
#define DCGM_FR_DCGM_API_NEXT                   ""
#define DCGM_FR_CONLWRRENT_GPUS_NEXT            ""
#define DCGM_FR_TOO_MANY_ERRORS_NEXT            ""
#define DCGM_FR_LWLINK_CRC_ERROR_THRESHOLD_NEXT TRIAGE_RUN_FIELD_DIAG_MSG
#define DCGM_FR_LWLINK_ERROR_CRITICAL_NEXT      TRIAGE_RUN_FIELD_DIAG_MSG
...
#define DCGM_FR_GPU_OP_MODE_NEXT                "Fix by running lwpu-smi as root with: lwpu-smi --gom=0 -i "\
                                                "<gpu index>"
