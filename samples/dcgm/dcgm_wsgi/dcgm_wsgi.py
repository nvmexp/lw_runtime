...
class DcgmHttpServer:
    ...
    '''
    def WsgiMain(self, elwiron, start_response):
        responseStr = ""

        filePath = elwiron['PATH_INFO'].lstrip('/')

        queryParams = cgi.parse_qs(elwiron['QUERY_STRING'])

        #for k in elwiron.keys():
        #    responseStr += "%s => %s\n" % (k, elwiron[k])
...
###############################################################################
def dcmg_http_app(elwiron, start_response):
    '''
    Main entry point
    '''
    responseStr = g_dcgmServer.WsgiMain(elwiron, start_response)
    return [responseStr]

###############################################################################
def application(elwiron, start_response):
    '''
    Callback for uWSGI
    '''
    return dcmg_http_app(elwiron, start_response)
