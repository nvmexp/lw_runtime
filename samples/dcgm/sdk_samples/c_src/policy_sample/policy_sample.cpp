...
main(int argc, char **argv)
{
    ...
    myGroupPolicy.parms[0].tag = dcgmPolicyConditionParms_t::BOOL;
    myGroupPolicy.parms[0].val.boolean = true;

    conditionBuffer |= DCGM_POLICY_COND_PCI;
    myGroupPolicy.parms[1].tag = dcgmPolicyConditionParms_t::BOOL;
    myGroupPolicy.parms[1].val.boolean = true;

    conditionBuffer |= DCGM_POLICY_COND_THERMAL;
    myGroupPolicy.parms[3].tag = dcgmPolicyConditionParms_t::LLONG;
    myGroupPolicy.parms[3].val.llval = 90;

    conditionBuffer |= DCGM_POLICY_COND_LWLINK;
    myGroupPolicy.parms[5].tag = dcgmPolicyConditionParms_t::BOOL;
    ...
    // violations oclwrs we can register for policy updates. This allows us to handle violations
    ...
    // to the screen when a violation oclwrs.
    ...
    result =  dcgmPolicyRegister(dcgmHandle, myGroupId, myGroupPolicy.condition,
            violationRegistration, violationRegistration);
...
int violationRegistration(void *data)
{
    ...
    switch (response->condition)
    {
        ...
        case DCGM_POLICY_COND_LWLINK:
            errorMessage = "Lwlink policy violation";
            break;
        default:
