...
int list_field_values_since(unsigned int gpuId, dcgmFieldValue_v1 *values,
                            int numValues, void *userData);
int list_field_values(unsigned int gpuId, dcgmFieldValue_v1 *values,
                      int numValues, void *userData);
...
int main(int argc, char **argv)
{
    ...
    map<unsigned int, dcgmFieldValue_v1> field_val_map;
    map<unsigned int,vector<dcgmFieldValue_v1> > field_val_vec_map;
    ...
    if (result != DCGM_ST_OK){
        std::cout << "Error initializing DCGM engine. Return: "
    ...
    if(standalone)
    {
        ...
        if (result != DCGM_ST_OK){
            std::cout << "Error connecting to remote DCGM engine. Return: "
                      << errorString(result) << std::endl;
            goto cleanup;
        }
    }
    else //Embedded
    {
        ...
        if (result != DCGM_ST_OK){
            std::cout << "Error starting embedded DCGM engine. Return: "
                      << errorString(result) << std::endl;
            goto cleanup;
        }
    }
    ...
    if (result != DCGM_ST_OK){
        std::cout << "Error fetching devices. Return: " << errorString(result)
                  << std::endl;
        goto cleanup;
    }
    ...
    result = dcgmGroupCreate(dcgmHandle, DCGM_GROUP_DEFAULT, groupName,
                             &myGroupId);

    // Check the result to see if our DCGM operation was successful.
    if (result != DCGM_ST_OK){
        std::cout << "Error creating group. Return: " << errorString(result)
                  << std::endl;
        goto cleanup;
    } else {
        std::cout << "Successfully created group with group ID: "
                  << (unsigned long) myGroupId  << std::endl;
    }
    ...
    result = dcgmFieldGroupCreate(dcgmHandle, 2, &fieldIds[0],
                                  (char *)"interesting_fields", &fieldGroupId);
    if(result != DCGM_ST_OK){
        std::cout << "Error creating field group. Result: " << errorString(result)
                  << std::endl;;
        goto cleanup;
    }

    result = dcgmWatchFields(dcgmHandle, myGroupId, fieldGroupId, 1000000,
                             3600.0, 3600);

    // Check result to see if DCGM operation was successful.
    if (result != DCGM_ST_OK){
        std::cout << "Error setting watches. Result: " << errorString(result)
                  << std::endl;;
        goto cleanup;
    }
    ...
    {
        result = dcgmGetLatestValues(dcgmHandle, myGroupId, fieldGroupId,
                                     &list_field_values, &field_val_map);
    ...
    while(i < 3)
    {
        ...
        result = dcgmGetValuesSince(dcgmHandle, myGroupId, fieldGroupId,
    ...
    std::cout<<"Clearing the map"<<endl;
    field_val_map.clear();
...
/**
 *
 * This function prints the collected value over a time period. It is ilwoked
...
{
    int i =0;
    vector<dcgmFieldValue_v1> dcgm_field_vals_vec;

    map <unsigned int,vector<dcgmFieldValue_v1> > field_val_vec_map;
    //note this is a pointer to a map.
    field_val_vec_map = *static_cast<std::map<unsigned int,
            vector<dcgmFieldValue_v1> > *> (userdata);
...
int list_field_values(unsigned int gpuId, dcgmFieldValue_v1 *values,
    ...
    for (map<unsigned int, dcgmFieldValue_v1>::iterator it = field_val_map.begin();
