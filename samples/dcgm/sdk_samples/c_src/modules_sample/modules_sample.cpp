...
main(int argc, char **argv)
{
    ...
    std::cout << std::left << std::setw(12) << "Module ID"
              << std::left << std::setw(16) << "Module name"
              << "Status" << std::endl;

    for (unsigned int i = 0 ; i < getStatuses.numStatuses ; i++) {
        std::cout << std::left << std::setw(12) << getStatuses.statuses[i].id
                  << std::left << std::setw(16) << moduleIdToName(getStatuses.statuses[i].id)
                  << statusToStr(getStatuses.statuses[i].status) << std::endl;
    }
