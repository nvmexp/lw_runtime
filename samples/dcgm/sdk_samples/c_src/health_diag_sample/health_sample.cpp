...
main(int argc, char **argv)
{
    ...
    // Both PCIe and memory watches are now enabled and any errors ilwolving them will be recorded.
    ...
    // during it's exelwtion. To do this we need our previously allocated dcgmHealthResponse_t results.
    ...
    results.version = dcgmHealthResponse_version1;
    ...
    if (results.overallHealth == DCGM_HEALTH_RESULT_PASS){
        ...
        std::cout << "Group has a " << (results.overallHealth == DCGM_HEALTH_RESULT_WARN)? "warning.\n":"failure.\n";
        std::cout << "GPU ID : Health \n";
        for (unsigned int gpu = 0; gpu < results.gpuCount ; gpu++){
            std::cout << results.gpu[gpu].gpuId << " : ";

            switch (results.gpu[gpu].overallHealth){
            ...
            // Now we check the incidents reported per GPU
            for (unsigned int incident = 0; incident < results.gpu[gpu].incidentCount; incident++){
                // A more in depth case check may be required here, but since we are only interested in PCIe and memory watches
                // This is all we are going to check for here.
                std::cout << "Error: " << (results.gpu[gpu].systems[incident].system == DCGM_HEALTH_WATCH_PCIE)? "PCIe ":"Memory ";
                std::cout << "watches detected a " << (results.gpu[gpu].systems[incident].health == DCGM_HEALTH_RESULT_WARN)? "warning.\n":"failure.\n";
                std::cout << results.gpu[gpu].systems[incident].errorString << std::endl;
            }
        }
        std::cout << std::endl;
    }

    // And let's also display some of the samples recorded by our watches on the temperature and power usage of the GPUs.
    // We demonstrate how we can pass through information using the userDate void pointer parameter in this function by passing
    // our testString through. More information on this can be seen in the function below.
    ...
    // the group is still healthy and ready for the next process to be run. We will then print out the results of the diagnostic.
    // Medium and long LWVS diagnostics can offer per GPU results and it's likely that a more thorough diagnostic would be
    // preferred, but this is just an example.
    ...
    if (result != DCGM_ST_OK){
        if(result == DCGM_ST_GROUP_INCOMPATIBLE)
            ...
            std::cout << "Error running diagnostic. Result: " << errorString(result) << std::endl;
    } else {
        ...
        std::cout << "LWCA Main Library: " << 
            diagnosticResults.levelOneResults[DCGM_SWTEST_LWDA_MAIN_LIBRARY].status << std::endl;
        std::cout << "LWCA Runtime Library: " <<
            diagnosticResults.levelOneResults[DCGM_SWTEST_LWDA_RUNTIME_LIBRARY].status << std::endl;
        std::cout << "Environment: " <<
            diagnosticResults.levelOneResults[DCGM_SWTEST_ELWIRONMENT].status << std::endl;
        std::cout << "Graphics Processes: " <<
            diagnosticResults.levelOneResults[DCGM_SWTEST_GRAPHICS_PROCESSES].status << std::endl;
        std::cout << "LWML Library: " <<
            diagnosticResults.levelOneResults[DCGM_SWTEST_LWML_LIBRARY].status << std::endl;
        std::cout << "Page Retirement: " <<
            diagnosticResults.levelOneResults[DCGM_SWTEST_PAGE_RETIREMENT].status << std::endl;
        std::cout << "Permissions: " <<
            diagnosticResults.levelOneResults[DCGM_SWTEST_PERMISSIONS].status << std::endl;
        std::cout << "Persistence Mode: " <<
            diagnosticResults.levelOneResults[DCGM_SWTEST_PERSISTENCE_MODE].status << std::endl;
    }
...
int displayFieldValue(unsigned int gpuId, dcgmFieldValue_v1 *values, int numValues, void *userData){
    ...
    for (int i = 0; i < numValues; i++){
        ...
        std::cout << "Field ID: " << (DcgmFieldGetById(values[i].fieldId)?
                DcgmFieldGetById(values[i].fieldId)->tag:"Unknown") << std::endl;
    ...
    // Program exelwted correctly. Return 0 to notify DCGM (callee) that it was successful.
    return 0;
}
