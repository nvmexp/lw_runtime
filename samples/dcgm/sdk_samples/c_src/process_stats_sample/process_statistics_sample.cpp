...
main(int argc, char **argv)
{
    ...
    // Since process watches are enabled, we can run our process and and fetch the process exelwtion statistics
    ...
    // corresponding to the PID given. Right now we do not have a LWCA application to start so the PID we pass
    ...
    if (result == DCGM_ST_OK){
        std::cout << "PID Info\n";
        std::cout << "Start Time timestamp: " << (DCGM_INT64_IS_BLANK(pidInfo.summary.startTime)? -1 : pidInfo.summary.startTime )  << std::endl;
        std::cout << "End Time timestamp: " << (DCGM_INT64_IS_BLANK(pidInfo.summary.endTime)?  -1 : pidInfo.summary.endTime) << std::endl;
        std::cout << "Energy Consumed: " << (DCGM_INT64_IS_BLANK(pidInfo.summary.energyConsumed)?  -1 : pidInfo.summary.energyConsumed) << std::endl;
        std::cout << "Max GPU Memory Used: " << (DCGM_INT64_IS_BLANK(pidInfo.summary.maxGpuMemoryUsed)?  -1 : pidInfo.summary.maxGpuMemoryUsed) << std::endl;
        std::cout << "Average Memory Utilization: " << (DCGM_INT32_IS_BLANK(pidInfo.summary.memoryUtilization.average)?  -1 : pidInfo.summary.memoryUtilization.average) << std::endl;
        std::cout << "No. Other Compute PIDs: " << (DCGM_INT32_IS_BLANK(pidInfo.summary.numOtherComputePids)?  -1 : pidInfo.summary.numOtherComputePids) << std::endl;
        std::cout << "No. PCIe Replays: " << (DCGM_INT64_IS_BLANK(pidInfo.summary.pcieReplays)?  -1 : pidInfo.summary.pcieReplays) << std::endl;
        std::cout << "No. XID Critical Errors: " << (DCGM_INT32_IS_BLANK(pidInfo.summary.numXidCriticalErrors)?  -1 : pidInfo.summary.numXidCriticalErrors) << std::endl;
    } else {
