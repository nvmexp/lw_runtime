/*
 * Copyright 1993-2010 LWPU Corporation.  All rights reserved.
 *
 * NOTICE TO USER:   
 *
...
#if defined(__LWDABE__) /* lwdabe compiler */
...
#else /* __LWDA_ARCH__ >= 200 */

#define ___device__(sc) \
        static

#endif /* __LWDA_ARCH__ >= 200 */
...
#define __gen_lwvm_memcpy_aligned1(dest, src, size) \
({ \
  __lwvm_memcpy(dest, src, size, 1); \
  (void *)(dest); \
})
#define __gen_lwvm_memcpy_aligned2(dest, src, size) \
({ \
  if (((unsigned long long)(dest) % 2) == 0 && ((unsigned long long)(src) % 2) == 0) \
    __lwvm_memcpy(dest, src, size, 2); \
  else \
    __lwvm_memcpy(dest, src, size, 1); \
  (void *)(dest); \
})
#define __gen_lwvm_memcpy_aligned4(dest, src, size) \
({ \
  if (((unsigned long long)(dest) % 4) == 0 && ((unsigned long long)(src) % 4) == 0) \
    __lwvm_memcpy(dest, src, size, 4); \
  else \
    __lwvm_memcpy(dest, src, size, 1); \
  (void *)(dest); \
})
#define __gen_lwvm_memcpy_aligned8(dest, src, size) \
({ \
  if (((unsigned long long)(dest) % 8) == 0 && ((unsigned long long)(src) % 8) == 0) \
    __lwvm_memcpy(dest, src, size, 8); \
  else \
    __lwvm_memcpy(dest, src, size, 1); \
  (void *)(dest); \
})
#define __gen_lwvm_memcpy_aligned16(dest, src, size) \
({ \
  if (((unsigned long long)(dest) % 16) == 0 && ((unsigned long long)(src) % 16) == 0) \
    __lwvm_memcpy(dest, src, size, 16); \
  else \
    __lwvm_memcpy(dest, src, size, 1); \
  (void *)(dest); \
})
...
#elif defined(__LWDACC__) /* lwdafe compiler */
