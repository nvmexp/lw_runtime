...
/* Colwert unsigned int to float, use no intrinsics */
QUALIFIERS float __lwrand_uint32AsFloat (unsigned int i)
{
    union {
        float f;
        unsigned int i;
...
/* Colwert two unsigned ints to double, use no intrinsics */
QUALIFIERS double __lwrand_hilouint32AsDouble (unsigned int hi, unsigned int lo)
{
    union {
        double f;
        unsigned int hi;
...
/* Colwert unsigned int to float, as efficiently as possible */
QUALIFIERS float __lwrand_uint32_as_float(unsigned int x)
{
#if __LWDA_ARCH__ > 0
    return __int_as_float(x);
#else
    return __lwrand_uint32AsFloat(x);
#endif
}

/*
QUALIFIERS double __lwrand_hilouint32_as_double(unsigned int hi, unsigned int lo)
{
#if __LWDA_ARCH__ > 0
