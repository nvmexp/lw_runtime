  ...
  * Users Notice. 
  */ 
#ifndef LW_NPPI_COMPRESSION_FUNCTIONS_H
#define LW_NPPI_COMPRESSION_FUNCTIONS_H
 ...
 * forward JPEG transform and ilwerse level shift, IDCT and de-quantization
 * for ilwerse JPEG transform. This group has the functions for both forward
 * and ilwerse functions. 
 *
 * @{
 *
 ...
 *    The method creates a 16-bit version of the raw table and colwerts the 
...
/**
 * Initializes a quantization table for nppiDCTQuantIlw8x8LS_JPEG_16s8u_C1R().
 *      The nppiDCTQuantFwd8x8LS_JPEG_8u16s_C1R() method uses a quantization table
 * in a 16-bit format allowing for faster processing. In addition it colwerts the 
 ...
 *
 * \param hpQuantRawTable Raw quantization table.
 * \param hpQuantFwdRawTable Ilwerse quantization table.
 * \return ::NPP_NULL_POINTER_ERROR pQuantRawTable or pQuantFwdRawTable is0.
 */
NppStatus 
nppiQuantIlwTableInit_JPEG_8u16u(const Npp8u * hpQuantRawTable, Npp16u * hpQuantFwdRawTable);
 ...
 *          using nppiQuantIlwTableInit_JPEG_8u16u().
...
/**
 * Ilwerse DCT, de-quantization and level shift part of the JPEG decoding.
 ...
 * \param pQuantIlwTable Ilwerse quantization tables for JPEG decoding created
 *           using nppiQuantIlwTableInit_JPEG_8u16u().
 * \param oSizeROI \ref roi_specification.
 * \return Error codes:
 *         - ::NPP_SIZE_ERROR For negative input height/width or not a multiple of
 ...
 */
NppStatus 
nppiDCTQuantIlw8x8LS_JPEG_16s8u_C1R(const Npp16s * pSrc, int nSrcStep, 
                                          Npp8u  * pDst, int nDstStep, 
                                    const Npp16u * pQuantIlwTable, NppiSize oSizeROI);
 ...
 *
 * \see nppiDCTQuantFwd8x8LS_JPEG_8u16s_C1R_NEW(), nppiDCTQuantIlw8x8LS_JPEG_16s8u_C1R_NEW.
 * 
 ...
 */
NppStatus 
nppiDCTQuantIlw8x8LS_JPEG_16s8u_C1R_NEW(const Npp16s * pSrc, int nSrcStep, 
                                        Npp8u  * pDst, int nDstStep, 
                                        const Npp8u * pQuantizationTable, NppiSize oSizeROI,
                                        NppiDCTState* pState);
...
/**
 * Ilwerse DCT, de-quantization and level shift part of the JPEG decoding, 16-bit short integer.
 * Input is expected in 64x1 macro blocks and output is expected to be in 8x8
 * macro blocks. The new version of the primitive takes the ROI in image pixel size and
 * works with DCT coefficients that are in zig-zag order.
 *
 ...
 */
NppStatus
nppiDCTQuant16Ilw8x8LS_JPEG_16s8u_C1R_NEW(const Npp16s * pSrc, int nSrcStep,
                                          Npp8u  * pDst, int nDstStep,
                                          const Npp16u * pQuantizationTable, NppiSize oSizeROI,
                                          NppiDCTState* pState);
 ...
 * \param nRestartInterval Restart Interval, see JPEG standard. Lwrrently only values <=0 are supported.
 * \param nSs Start Coefficient, see JPEG standard.
 * \param nSe End Coefficient, see JPEG standard.
 * \param nAh Bit Approximation High, see JPEG standard.
 * \param nAl Bit Approximation Low, see JPEG standard.
...
/**
 * Callwlates the size of the temporary buffer for baseline Huffman encoding.
 *
...
/**
 * Callwlates the size of the temporary buffer for optimize Huffman coding.
 *
 * 
 * \param oSize Image Dimension.
 * \param nChannels Number of channels in the image.
...
typedef struct {
    ...
    Npp32s restartInterval; /**< Restart interval in MLWs. Use 0 or -1 when none */
...
/**
 * Callwlates sizes of additional buffers used by the job.
 *
...
/**
 * Exelwtes a job -- part of decoding.
 *
 ...
 *    and then by CPU job, caller has to call lwdaStreamSynchronize before CPU job.
 ...
 * @code
 ...
 for (int i = 0; i < nScans; ++i)
 {
     ...
     lwdaStreamSynchronize(nppGetStream());
 }
 ...
 * * \ref NPP_ERROR when encountered LWCA error
 */
 ...
 * this job doesn't have to be exelwted.
 */
 ...
 * in order to colwert temporary representation of DCT coefficients
 * to the final one.
 *
 * \param pJob:
 * pJob.pFrame should point to valid frame description
 ...
 * some spelwlative reads after the end of compressed scan data.
 */
...
/**
 * Ilwerse DCT in WebP decoding. Input is the bitstream that contains the coefficients of 16x16 blocks.
 * These coefficients are based on a 4x4 sub-block unit, e.g., 
 * Coeffs in 0th 4x4 block, 1st 4x4 block 2nd 4x4 block, etc.
 * Output is the coefficients after ilwerse DCT transform.
 ...
 *         - ::NPP_NULL_POINTER_ERROR If the destination pointer is 0.
 */
NppStatus nppiDCTIlw4x4_WebP_16s_C1R(const Npp16s * pSrc, int nSrcStep, Npp16s * pDst, int nDstStep, NppiSize oSizeROI);
...
#endif /* LW_NPPI_COMPRESSION_FUNCTIONS_H */
