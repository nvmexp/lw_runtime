  ...
  * Users Notice. 
  */ 
#ifndef LW_NPPI_GEOMETRY_TRANSFORMS_H
#define LW_NPPI_GEOMETRY_TRANSFORMS_H
 ...
 *   NPPI_INTER_LINEAR
 *   NPPI_INTER_LWBIC
 *   NPPI_INTER_LWBIC2P_BSPLINE
 *   NPPI_INTER_LWBIC2P_CATMULLROM
 *   NPPI_INTER_LWBIC2P_B05C03
 *   NPPI_INTER_SUPER
 *   NPPI_INTER_LANCZOS
 * \endcode
 *
 ...
 * \param eInterpolationMode The type of eInterpolation to perform resampling. Lwrrently only supports NPPI_INTER_LANCZOS3_Advanced.
 ...
 * \param nXFactor Factor by which x dimension is changed. 
 * \param nYFactor Factor by which y dimension is changed. 
 * \param pBuffer Device buffer that is used during callwlations.
 * \param eInterpolationMode The type of eInterpolation to perform resampling. Lwrrently only supports NPPI_INTER_LANCZOS3_Advanced.
 * \return \ref image_data_error_codes, \ref roi_error_codes, \ref resize_error_codes
 */
NppStatus 
 ...
 * Resize except that there is a list of N instances of those parameters (N > 1) and that list is passed in device memory. A colwenient
 ...
 * \param eInterpolation The type of eInterpolation to perform resampling. Lwrrently limited to NPPI_INTER_NN, NPPI_INTER_LINEAR, NPPI_INTER_LWBIC, or NPPI_INTER_SUPER. 
 ...
 * and that list is passed in device memory.  A colwenient data structure is provided that allows for easy initialization of the 
 ...
 *
 * The function cirlwmvents this problem by only looking at the first three vertices of
 ...
 *        NPPI_INTER_LINEAR or NPPI_INTER_LWBIC.
 ...
 * WarpAffine except that there is a list of N instances of those parameters (N > 1) and that list is passed in device memory. A colwenient
 ...
 * \param eInterpolation The type of eInterpolation to perform resampling. Lwrrently limited to NPPI_INTER_NN, NPPI_INTER_LINEAR, or NPPI_INTER_LWBIC. 
 ...
 * using the ilwerse matrix \f$C^{-1}\f$:
 * \f[
...
/**
 * Callwlates perspective transform coefficients given source rectangular ROI
...
/**
 * Callwlates perspective transform projection of given source rectangular
 * ROI
 *
 * \param oSrcROI Source ROI
 * \param quad Destination quadrangle
...
/**
 * Callwlates bounding box of the perspective transform projection of the
 ...
 * WarpPerspective except that there is a list of N instances of those parameters (N > 1) and that list is passed in device memory. A colwenient
 * data structure is provided that allows for easy initialization of the parameter lists.  The aTransformedCoeffs array is for internal use only
 * and should not be directly initialized by the application.  The only restriction on these functions is
 * that there is one single source ROI rectangle and one single destination ROI rectangle which are applied respectively to each image 
 * in the batch.  The primary purpose of this function is to provide improved performance for batches of smaller images as long as GPU 
...
#endif /* LW_NPPI_GEOMETRY_TRANSFORMS_H */
