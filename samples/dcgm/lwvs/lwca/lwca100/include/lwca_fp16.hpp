...
*/
#if (__cplusplus >= 201103L) && !defined(__LWDACC_RTC__)
#include <utility>
...
public:
    ...
    __LWDA_HOSTDEVICE__ operator volatile __half_raw() const volatile { __half_raw ret; ret.x = __x; return ret; }

#if !defined(__LWDA_NO_HALF_COLWERSIONS__)

    /* Construct from float/double */
    __LWDA_HOSTDEVICE__ __half(float f) { __x = __float2half(f).__x;  }
    __LWDA_HOSTDEVICE__ __half(double f) { __x = __float2half(static_cast<float>(f)).__x;  }

    __LWDA_HOSTDEVICE__ operator float() const { return __half2float(*this); }
    __LWDA_HOSTDEVICE__ __half &operator=(float f) { __x = __float2half(f).__x; return *this; }

    /* We omit "cast to double" operator, so as to not be ambiguous about up-cast */
    __LWDA_HOSTDEVICE__ __half &operator=(double f) { __x = __float2half(static_cast<float>(f)).__x; return *this; }
...
/* Note for increment and decrement we use the raw value 0x3C00 equating to half(1.0f), to avoid the extra colwersion */
...
#endif /* __LWDA_ARCH__ >= 530 || !defined(__LWDA_ARCH__) */
#endif /* defined(__LWDACC__) */
...
public:
#if __cplusplus >= 201103L
    __half2() = default;
    __LWDA_HOSTDEVICE__ __half2(__half2 &&src) { __HALF2_TO_UI(*this) = std::move(__HALF2_TO_LWI(src)); }
    __LWDA_HOSTDEVICE__ __half2 &operator=(__half2 &&src) { __HALF2_TO_UI(*this) = std::move(__HALF2_TO_LWI(src)); return *this; }
#else
    ...
    __LWDA_HOSTDEVICE__ operator __half2_raw() const { __half2_raw ret; __HALF2_TO_UI(ret) = __HALF2_TO_LWI(*this); return ret; }
};
...
#if (__LWDA_ARCH__ >= 530 || !defined(__LWDA_ARCH__)) && !defined(__LWDA_NO_HALF2_OPERATORS__)
...
__LWDA_HOSTDEVICE_FP16_DECL__ __half __float2half(const float f)
{
    __half val;
...
__LWDA_HOSTDEVICE_FP16_DECL__ __half __float2half_rn(const float f)
{
    __half val;
#if defined(__LWDA_ARCH__)
    asm("{  cvt.rn.f16.f32 %0, %1;}\n" : "=h"(__HALF_TO_US(val)) : "f"(f));
...
__LWDA_HOSTDEVICE_FP16_DECL__ __half __float2half_rz(const float f)
{
    __half val;
#if defined(__LWDA_ARCH__)
...
__LWDA_HOSTDEVICE_FP16_DECL__ __half __float2half_rd(const float f)
{
    __half val;
#if defined(__LWDA_ARCH__)
...
__LWDA_HOSTDEVICE_FP16_DECL__ __half __float2half_ru(const float f)
{
    __half val;
#if defined(__LWDA_ARCH__)
...
__LWDA_HOSTDEVICE_FP16_DECL__ __half2 __float2half2_rn(const float f)
{
    __half2 val;
#if defined(__LWDA_ARCH__)
    asm("{.reg .f16 low;\n"
...
__LWDA_HOSTDEVICE_FP16_DECL__ __half2 __floats2half2_rn(const float f1, const float f2)
{
    __half2 val;
#if defined(__LWDA_ARCH__)
...
__LWDA_HOSTDEVICE_FP16_DECL__ float __half2float(const __half h)
{
    float val;
#if defined(__LWDA_ARCH__)
    asm("{  cvt.f32.f16 %0, %1;}\n" : "=f"(val) : "h"(__HALF_TO_LWS(h)));
#else
    val = __internal_half2float(static_cast<__half_raw>(h).x);
#endif
    return val;
}
__LWDA_HOSTDEVICE_FP16_DECL__ float __low2float(const __half2 l)
{
    float val;
#if defined(__LWDA_ARCH__)
    asm("{.reg .f16 low,high;\n"
        "  mov.b32 {low,high},%1;\n"
        "  cvt.f32.f16 %0, low;}\n" : "=f"(val) : "r"(__HALF2_TO_LWI(l)));
#else
    val = __internal_half2float(static_cast<__half2_raw>(l).x);
#endif
    return val;
}
__LWDA_HOSTDEVICE_FP16_DECL__ float __high2float(const __half2 l)
{
    float val;
#if defined(__LWDA_ARCH__)
    asm("{.reg .f16 low,high;\n"
...
__LWDA_HOSTDEVICE_FP16_DECL__ __half2 __float22half2_rn(const float2 f)
{
    __half2 val = __floats2half2_rn(f.x, f.y);
    return val;
}
__LWDA_HOSTDEVICE_FP16_DECL__ float2 __half22float2(const __half2 l)
{
    float hi_float;
    float lo_float;
#if defined(__LWDA_ARCH__)
    asm("{.reg .f16 low,high;\n"
        "  mov.b32 {low,high},%1;\n"
        "  cvt.f32.f16 %0, low;}\n" : "=f"(lo_float) : "r"(__HALF2_TO_LWI(l)));

    asm("{.reg .f16 low,high;\n"
        "  mov.b32 {low,high},%1;\n"
        "  cvt.f32.f16 %0, high;}\n" : "=f"(hi_float) : "r"(__HALF2_TO_LWI(l)));
#else
...
__LWDA_FP16_DECL__ __half2 __lows2half2(const __half2 l, const __half2 h)
{
    __half2 val;
    asm("{.reg .f16 alow,ahigh,blow,bhigh;\n"
        "  mov.b32 {alow,ahigh}, %1;\n"
        "  mov.b32 {blow,bhigh}, %2;\n"
        "  mov.b32 %0, {alow,blow};}\n" : "=r"(__HALF2_TO_UI(val)) : "r"(__HALF2_TO_LWI(l)), "r"(__HALF2_TO_LWI(h)));
    return val;
}
__LWDA_FP16_DECL__ __half2 __highs2half2(const __half2 l, const __half2 h)
{
    __half2 val;
    asm("{.reg .f16 alow,ahigh,blow,bhigh;\n"
        "  mov.b32 {alow,ahigh}, %1;\n"
...
__LWDA_FP16_DECL__ __half __low2half(const __half2 h)
{
    __half ret;
    asm("{.reg .f16 low,high;\n"
        " mov.b32 {low,high}, %1;\n"
        " mov.b16 %0, low;}" : "=h"(__HALF_TO_US(ret)) : "r"(__HALF2_TO_LWI(h)));
    return ret;
}
__LWDA_FP16_DECL__ int __hisinf(const __half a)
{
    if (__HALF_TO_LWS(a) == 0xFC00)
        return -1;
    if (__HALF_TO_LWS(a) == 0x7C00)
        return 1;
    return 0;
}
__LWDA_FP16_DECL__ __half2 __low2half2(const __half2 l)
{
    __half2 val;
    asm("{.reg .f16 low,high;\n"
        "  mov.b32 {low,high}, %1;\n"
        "  mov.b32 %0, {low,low};}\n" : "=r"(__HALF2_TO_UI(val)) : "r"(__HALF2_TO_LWI(l)));
    return val;
}
__LWDA_FP16_DECL__ __half2 __high2half2(const __half2 l)
{
    __half2 val;
    asm("{.reg .f16 low,high;\n"
        "  mov.b32 {low,high}, %1;\n"
...
__LWDA_FP16_DECL__ __half __high2half(const __half2 h)
{
    __half ret;
    asm("{.reg .f16 low,high;\n"
        " mov.b32 {low,high}, %1;\n"
        " mov.b16 %0, high;}" : "=h"(__HALF_TO_US(ret)) : "r"(__HALF2_TO_LWI(h)));
    return ret;
}
__LWDA_FP16_DECL__ __half2 __halves2half2(const __half l, const __half h)
{
    __half2 val;
    asm("{  mov.b32 %0, {%1,%2};}\n"
        : "=r"(__HALF2_TO_UI(val)) : "h"(__HALF_TO_LWS(l)), "h"(__HALF_TO_LWS(h)));
    return val;
}
__LWDA_FP16_DECL__ __half2 __half2half2(const __half lh)
{
    __half2 val;
    asm("{  mov.b32 %0, {%1,%1};}\n"
        : "=r"(__HALF2_TO_UI(val)) : "h"(__HALF_TO_LWS(lh)));
    return val;
}
__LWDA_FP16_DECL__ __half2 __lowhigh2highlow(const __half2 lh)
{
    __half2 val;
    asm("{.reg .f16 low,high;\n"
        "  mov.b32 {low,high}, %1;\n"
        "  mov.b32 %0, {high,low};}\n" : "=r"(__HALF2_TO_UI(val)) : "r"(__HALF2_TO_LWI(lh)));
    return val;
}
__LWDA_FP16_DECL__ short int __half_as_short(const __half h)
{
    return (short int)__HALF_TO_LWS(h);
}
...
__LWDA_FP16_DECL__ __half2 __shfl(__half2 var, int delta, int width)
{
    int warpSize;
...
__LWDA_FP16_DECL__ __half2 __shfl_up(__half2 var, unsigned int delta, int width)
{
    int warpSize;
    asm("{mov.u32 %0, WARP_SZ;\n}" : "=r"(warpSize));
...
__LWDA_FP16_DECL__ __half2 __shfl_down(__half2 var, unsigned int delta, int width)
{
    int warpSize;
    asm("{mov.u32 %0, WARP_SZ;\n}" : "=r"(warpSize));
    int c = ((warpSize - width) << 8) | 0x1f;
...
__LWDA_FP16_DECL__ __half2 __shfl_xor(__half2 var, int delta, int width)
{
    int warpSize;
    asm("{mov.u32 %0, WARP_SZ;\n}" : "=r"(warpSize));
    int c = ((warpSize - width) << 8) | 0x1f;
...
__LWDA_FP16_DECL__ __half2 __shfl_sync(unsigned mask, __half2 var, int delta, int width)
{
    int warpSize;
    asm("{mov.u32 %0, WARP_SZ;\n}" : "=r"(warpSize));
    int c = ((warpSize - width) << 8) | 0x1f;
...
__LWDA_FP16_DECL__ __half2 __shfl_up_sync(unsigned mask, __half2 var, unsigned int delta, int width)
{
    int warpSize;
    asm("{mov.u32 %0, WARP_SZ;\n}" : "=r"(warpSize));
    int c = (warpSize - width) << 8;
...
__LWDA_FP16_DECL__ __half2 __shfl_down_sync(unsigned mask, __half2 var, unsigned int delta, int width)
{
    int warpSize;
    asm("{mov.u32 %0, WARP_SZ;\n}" : "=r"(warpSize));
    int c = ((warpSize - width) << 8) | 0x1f;
...
__LWDA_FP16_DECL__ __half2 __shfl_xor_sync(unsigned mask, __half2 var, int delta, int width)
{
    int warpSize;
    asm("{mov.u32 %0, WARP_SZ;\n}" : "=r"(warpSize));
    int c = ((warpSize - width) << 8) | 0x1f;
...
__LWDA_FP16_DECL__ __half __shfl(__half var, int delta, int width)
{
    __half2 temp1 = __halves2half2(var, var);
    __half2 temp2 = __shfl(temp1, delta, width);
    return __low2half(temp2);
}
__LWDA_FP16_DECL__ __half __shfl_up(__half var, unsigned int delta, int width)
{
    __half2 temp1 = __halves2half2(var, var);
    __half2 temp2 = __shfl_up(temp1, delta, width);
    return __low2half(temp2);
}
__LWDA_FP16_DECL__ __half __shfl_down(__half var, unsigned int delta, int width)
{
    __half2 temp1 = __halves2half2(var, var);
    __half2 temp2 = __shfl_down(temp1, delta, width);
    return __low2half(temp2);
}
__LWDA_FP16_DECL__ __half __shfl_xor(__half var, int delta, int width)
{
    __half2 temp1 = __halves2half2(var, var);
...
__LWDA_FP16_DECL__ __half __shfl_sync(unsigned mask, __half var, int delta, int width)
{
    __half2 temp1 = __halves2half2(var, var);
    __half2 temp2 = __shfl_sync(mask, temp1, delta, width);
    return __low2half(temp2);
}
__LWDA_FP16_DECL__ __half __shfl_up_sync(unsigned mask, __half var, unsigned int delta, int width)
{
    __half2 temp1 = __halves2half2(var, var);
    __half2 temp2 = __shfl_up_sync(mask, temp1, delta, width);
    return __low2half(temp2);
}
__LWDA_FP16_DECL__ __half __shfl_down_sync(unsigned mask, __half var, unsigned int delta, int width)
{
    __half2 temp1 = __halves2half2(var, var);
    __half2 temp2 = __shfl_down_sync(mask, temp1, delta, width);
    return __low2half(temp2);
}
__LWDA_FP16_DECL__ __half __shfl_xor_sync(unsigned mask, __half var, int delta, int width)
{
    __half2 temp1 = __halves2half2(var, var);
...
#define __BOOL_COMPARISON_OP_HALF2_MACRO(name) do {\
   ...
   asm( "{ "#name".f16x2.f16x2 %0,%1,%2;\n}" \
        :"=r"(__HALF2_TO_UI(val)) : "r"(__HALF2_TO_LWI(a)),"r"(__HALF2_TO_LWI(b))); \
   if (__HALF2_TO_LWI(val) == 0x3C003C00) \
      return true; \
   else  \
      return false; \
} while(0)
...
__LWDA_FP16_DECL__ __half2 __h2div(__half2 a, __half2 b) {
    __half ha, hb;
...
#undef __TERNARY_OP_HALF2_MACRO
__LWDA_FP16_DECL__ __half __hdiv(__half a, __half b) {
    __half v, abs, den;
    ...
    v = __float2half(fv);
    __HALF_TO_US(abs) = (unsigned short)(((unsigned int)__HALF_TO_LWS(v)) & 0x00007FFF);
    if (__hlt(abs, den) && (!(__HALF_TO_LWS(abs) == 0x0000))) {
...
__LWDA_FP16_DECL__ __half __hsin_internal(const __half a) {
...
__LWDA_FP16_DECL__ __half __hcos_internal(const __half a) {
    float f = __half2float(a);
...
__LWDA_FP16_DECL__  __half2 atomicAdd(__half2 *address, __half2 val) {
    __half2 r;
...
__LWDA_FP16_DECL__  __half atomicAdd(__half *address, __half val) {
    __half r;
