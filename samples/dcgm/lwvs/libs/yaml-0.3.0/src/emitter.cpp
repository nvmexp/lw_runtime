...
{	
	...
	bool Emitter::GotoNextPreAtomicState()
	{
		...
		unsigned lwrIndent = m_pState->GetLwrIndent();
		
		EMITTER_STATE lwrState = m_pState->GetLwrState();
		switch(lwrState) {
				// document-level
			...
			case ES_WAITING_FOR_BLOCK_MAP_KEY:
				if(m_pState->LwrrentlyInLongKey()) {
					m_stream << IndentTo(lwrIndent) << '?';
					m_pState->RequireSoftSeparation();
				}
	...
	void Emitter::EmitBeginDoc()
	{
		...
		EMITTER_STATE lwrState = m_pState->GetLwrState();
		if(lwrState != ES_WAITING_FOR_DOC && lwrState != ES_WRITING_DOC && lwrState != ES_DONE_WITH_DOC) {
			m_pState->SetError("Unexpected begin document");
			return;
		}
		
		if(lwrState == ES_WRITING_DOC || lwrState == ES_DONE_WITH_DOC)
			m_stream << '\n';		
		m_stream << "---\n";
	...
	void Emitter::EmitBeginSeq()
	{
		...
		if(flowType == Block) {
			if(lwrState == ES_WRITING_BLOCK_SEQ_ENTRY ||
			   lwrState == ES_WRITING_BLOCK_MAP_KEY || lwrState == ES_WRITING_BLOCK_MAP_VALUE ||
			   lwrState == ES_WRITING_DOC
			) {
				if(m_pState->RequiresHardSeparation() || lwrState != ES_WRITING_DOC) {
					m_stream << "\n";
					m_pState->UnsetSeparation();
				}
			}
	...
	void Emitter::EmitEndSeq()
	{
		...
		if(m_pState->GetLwrGroupType() != GT_SEQ)
			return m_pState->SetError(ErrorMsg::UNEXPECTED_END_SEQ);
		
		EMITTER_STATE lwrState = m_pState->GetLwrState();
		FLOW_TYPE flowType = m_pState->GetLwrGroupFlowType();
		if(flowType == FT_BLOCK) {
			// Note: block sequences are *not* allowed to be empty, but we colwert it
			//       to a flow sequence if it is
			assert(lwrState == ES_DONE_WITH_BLOCK_SEQ_ENTRY || lwrState == ES_WAITING_FOR_BLOCK_SEQ_ENTRY);
			if(lwrState == ES_WAITING_FOR_BLOCK_SEQ_ENTRY) {
			...
			assert(lwrState == ES_DONE_WITH_FLOW_SEQ_ENTRY || lwrState == ES_WAITING_FOR_FLOW_SEQ_ENTRY);
			m_stream << "]";
		} else
			assert(false);
		...
		if(flowType == Block) {
			if(lwrState == ES_WRITING_BLOCK_SEQ_ENTRY ||
			   ...
			   lwrState == ES_WRITING_DOC
				...
				if(m_pState->RequiresHardSeparation() || (lwrState != ES_WRITING_DOC && lwrState != ES_WRITING_BLOCK_SEQ_ENTRY)) {
					m_stream << "\n";
					m_pState->UnsetSeparation();
				}
			}
	...
	void Emitter::EmitEndMap()
	{
		...
		if(m_pState->GetLwrGroupType() != GT_MAP)
		...
		if(flowType == FT_BLOCK) {
			// Note: block sequences are *not* allowed to be empty, but we colwert it
			//       to a flow sequence if it is
			assert(lwrState == ES_DONE_WITH_BLOCK_MAP_VALUE || lwrState == ES_WAITING_FOR_BLOCK_MAP_ENTRY);
			if(lwrState == ES_WAITING_FOR_BLOCK_MAP_ENTRY) {
				// Note: only one of these will actually output anything for a given situation
				EmitSeparationIfNecessary();
				unsigned lwrIndent = m_pState->GetLwrIndent();
				m_stream << IndentTo(lwrIndent);
			...
			assert(lwrState == ES_DONE_WITH_FLOW_MAP_VALUE || lwrState == ES_WAITING_FOR_FLOW_MAP_ENTRY);
			EmitSeparationIfNecessary();
			m_stream << "}";
		} else
			assert(false);
	...
	void Emitter::EmitKey()
	{
		...
		EMITTER_STATE lwrState = m_pState->GetLwrState();
		FLOW_TYPE flowType = m_pState->GetLwrGroupFlowType();
		if(lwrState != ES_WAITING_FOR_BLOCK_MAP_ENTRY && lwrState != ES_DONE_WITH_BLOCK_MAP_VALUE
		   && lwrState != ES_WAITING_FOR_FLOW_MAP_ENTRY && lwrState != ES_DONE_WITH_FLOW_MAP_VALUE)
			return m_pState->SetError(ErrorMsg::UNEXPECTED_KEY_TOKEN);

		if(flowType == FT_BLOCK) {
			if(lwrState == ES_DONE_WITH_BLOCK_MAP_VALUE)
				m_stream << '\n';
			unsigned lwrIndent = m_pState->GetLwrIndent();
			m_stream << IndentTo(lwrIndent);
			m_pState->UnsetSeparation();
			...
			EmitSeparationIfNecessary();
			if(lwrState == ES_DONE_WITH_FLOW_MAP_VALUE) {
				m_stream << ',';
				m_pState->RequireSoftSeparation();
			}
	...
	void Emitter::EmitValue()
	{
		...
		EMITTER_STATE lwrState = m_pState->GetLwrState();
		FLOW_TYPE flowType = m_pState->GetLwrGroupFlowType();
		if(lwrState != ES_DONE_WITH_BLOCK_MAP_KEY && lwrState != ES_DONE_WITH_FLOW_MAP_KEY)
			return m_pState->SetError(ErrorMsg::UNEXPECTED_VALUE_TOKEN);

		if(flowType == FT_BLOCK) {
			if(m_pState->LwrrentlyInLongKey()) {
				m_stream << '\n';
				m_stream << IndentTo(m_pState->GetLwrIndent());
				m_stream << ':';
				m_pState->RequireSoftSeparation();
			}
	...
	{
		FLOW_TYPE flowType = m_pState->GetLwrGroupFlowType();
		if(flowType == FT_BLOCK && m_pState->LwrrentlyInLongKey())
			return true;

		EMITTER_STATE lwrState = m_pState->GetLwrState();
		return lwrState != ES_DONE_WITH_BLOCK_MAP_KEY && lwrState != ES_WAITING_FOR_BLOCK_MAP_VALUE && lwrState != ES_WRITING_BLOCK_MAP_VALUE;
	}
	...
	Emitter& Emitter::Write(const std::string& str)
	{
		...
		if(m_pState->GetStringFormat() == Literal && m_pState->GetLwrGroupFlowType() != FT_FLOW)
			m_pState->StartLongKey();
		...
		switch(strFmt) {
			...
			case Literal:
				if(flowType == FT_FLOW)
					...
					Utils::WriteLiteralString(m_stream, str, lwrIndent + m_pState->GetIndent());
				break;
			default:
				assert(false);
		}
	...
	Emitter& Emitter::Write(const _Alias& alias)
	{
		...
		if(!Utils::WriteAlias(m_stream, alias.content)) {
			m_pState->SetError(ErrorMsg::ILWALID_ALIAS);
			return *this;
		}
		PostAtomicWrite();
		return *this;
	...
	Emitter& Emitter::Write(const _Anchor& anchor)
	{
		...
		if(!Utils::WriteAnchor(m_stream, anchor.content)) {
			m_pState->SetError(ErrorMsg::ILWALID_ANCHOR);
			return *this;
		}
	...
	Emitter& Emitter::Write(const _Tag& tag)
	{
		...
		if(!success) {
			m_pState->SetError(ErrorMsg::ILWALID_TAG);
			return *this;
		}
