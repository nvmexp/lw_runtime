...
namespace YAML
{
	SingleDocParser::SingleDocParser(Scanner& scanner, const Directives& directives): m_scanner(scanner), m_directives(directives), m_pCollectionStack(new CollectionStack), m_lwrAnchor(0)
	{
	}
	...
	void SingleDocParser::HandleDolwment(EventHandler& eventHandler)
	{
		assert(!m_scanner.empty()); // guaranteed that there are tokens
		assert(!m_lwrAnchor);

		eventHandler.OnDolwmentStart(m_scanner.peek().mark);
	...
	void SingleDocParser::HandleNode(EventHandler& eventHandler)
	{
		...
		switch(token.type) {
			...
			case Token::KEY:
				...
				if(m_pCollectionStack->GetLwrCollectionType() == CollectionType::FlowSeq) {
					eventHandler.OnMapStart(mark, tag, anchor);
					HandleMap(eventHandler);
					eventHandler.OnMapEnd();
					return;
	...
	anchor_t SingleDocParser::RegisterAnchor(const std::string& name)
	{
		...
		return m_anchors[name] = ++m_lwrAnchor;
	}
