...
namespace YAML
{
	EmitterState::EmitterState(): m_isGood(true), m_lwrIndent(0), m_requiresSoftSeparation(false), m_requiresHardSeparation(false)
	{
		// start up
	...
	GROUP_TYPE EmitterState::GetLwrGroupType() const
	{
		if(m_groups.empty())
			return GT_NONE;
	...
	FLOW_TYPE EmitterState::GetLwrGroupFlowType() const
	{
		if(m_groups.empty())
			return FT_NONE;
	...
	bool EmitterState::LwrrentlyInLongKey()
	{
		if(m_groups.empty())
			return false;
	...
	{
		// force flow style if we're lwrrently in a flow
		FLOW_TYPE flowType = GetLwrGroupFlowType();
		if(flowType == FT_FLOW)
			return Flow;
