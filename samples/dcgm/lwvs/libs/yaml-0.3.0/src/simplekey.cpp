...
namespace YAML
{
	...
	void Scanner::SimpleKey::Ilwalidate()
	{
		if(pIndent)
	...
	void Scanner::InsertPotentialSimpleKey()
	{
		...
		if(InBlockContext()) {
			...
			if(key.pIndent) {
				...
				key.pMapStart->status = Token::ULWERIFIED;
			}
		}

		// then add the (now ulwerified) key
		m_tokens.push(Token(Token::KEY, INPUT.mark()));
		key.pKey = &m_tokens.back();
		key.pKey->status = Token::ULWERIFIED;

		m_simpleKeys.push(key);
	}

	// IlwalidateSimpleKey
	// . Automatically ilwalidate the simple key in our flow level
	void Scanner::IlwalidateSimpleKey()
	{
		if(m_simpleKeys.empty())
			return;
