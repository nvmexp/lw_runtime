...
DcgmEntityTimeSeries::DcgmEntityTimeSeries() : m_entityId(0),
                                               m_fieldValueTimeSeries()
{
}

DcgmEntityTimeSeries::DcgmEntityTimeSeries(dcgm_field_eid_t entityId) : m_entityId(entityId),
                                                                        m_fieldValueTimeSeries()
{
}
...
DcgmEntityTimeSeries::DcgmEntityTimeSeries(
    ...
    const DcgmEntityTimeSeries &other) : m_entityId(other.m_entityId),
...
void DcgmEntityTimeSeries::AddToJson(Json::Value &jv, unsigned int jsonIndex)
{
         ...
         iter++)
    {
        ...
        {
            Json::Value entry;
            entry["timestamp"] = static_cast<long long>(iter->second[i].ts);
            switch (iter->second[i].fieldType)
            {
                case DCGM_FT_INT64:
                    entry["value"] = static_cast<long long>(iter->second[i].value.i64);
                    break;
...
void DcgmValuesSinceHolder::GetFirstNonZero(dcgm_field_entity_group_t entityGroupId, dcgm_field_eid_t entityId,
...
void DcgmValuesSinceHolder::AddValue(dcgm_field_entity_group_t entityGroupId, dcgm_field_eid_t entityId,
                                     unsigned short fieldId, dcgmFieldValue_v1 &val)
{
    m_values[entityGroupId][entityId].AddValue(fieldId, val);
}

bool DcgmValuesSinceHolder::IsStored(dcgm_field_entity_group_t entityGroupId, dcgm_field_eid_t entityId,
                                     unsigned short fieldId)
{
    return m_values[entityGroupId][entityId].IsFieldStored(fieldId);
}

void DcgmValuesSinceHolder::ClearEntries(dcgm_field_entity_group_t entityGroupId, dcgm_field_eid_t entityId,
                                         unsigned short fieldId)
{
    m_values[entityGroupId][entityId].m_fieldValueTimeSeries[fieldId].clear();
}
...
void DcgmValuesSinceHolder::AddToJson(Json::Value &jv)
{
    ...
    for (typeIter = m_values.begin(); typeIter != m_values.end(); typeIter++)
    {
             ...
             entityIter++)
        {
            ...
            // are running on non-conselwtive GPU ids.
...
bool DcgmValuesSinceHolder::DoesValuePassPerSecondThreshold(unsigned short fieldId, const dcgmFieldValue_v1 &dfv,
        ...
        switch (dfv.fieldType)
        {
            case DCGM_FT_DOUBLE:
            {
                ...
                if (delta >= dfv.value.dbl)
                {
                    ...
                    DcgmError d;
                    DCGM_ERROR_FORMAT_MESSAGE(DCGM_FR_FIELD_THRESHOLD_TS_DBL, d, fieldName, dfv.value.dbl,
                                                      delta, timeDelta);
            ...
            case DCGM_FT_INT64:
            {
                ...
                if (delta >= dfv.value.i64)
                {
                    double timeDelta = (values[i].ts - startTime) / 1000000.0;
                    DcgmError d;
                    DCGM_ERROR_FORMAT_MESSAGE(DCGM_FR_FIELD_THRESHOLD_TS, d, fieldName, dfv.value.i64,
                                                      delta, timeDelta);
                    errorList.push_back(d);
                    return true;
                }
