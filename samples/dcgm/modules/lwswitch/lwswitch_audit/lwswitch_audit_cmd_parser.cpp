...
static bool getNextInt(char *str, const char *delim, int &val)
{
    ...
    val = strtoul(sub_str, &end_ptr, 10);
    if ((val == 0) && ((errno != 0) || (end_ptr == sub_str)))
        return false;
    else
        return true;
}
...
{
    char *res_entry=NULL;
    if(cmdline_getStringVal(pCmdParser->pCmdLine, NA_CMD_RES_ENTRY,(const char **) &res_entry) == 0)
    {
        fprintf(stderr, "Incorrect value for response entry\n");
        return false;
    }

    if(!getNextInt(res_entry, ":", pCmdParser->mSwitch))
    {
        fprintf(stderr, "Incorrect LWSwitch id\n");
        return false;
    }
...
{
    char *req_entry=NULL;
    if(cmdline_getStringVal(pCmdParser->pCmdLine, NA_CMD_REQ_ENTRY,(const char **) &req_entry) == 0)
    {
        fprintf(stderr, "Incorrect value for request entry\n");
        return false;
    }

    if(!getNextInt(req_entry, ":", pCmdParser->mSwitch))
    {
        fprintf(stderr, "Incorrect LWSwitch id\n");
        return false;
    }
...
NAReturn_t naCmdProcessing(naCmdParser_t *pCmdParser)
{
    ...
    if (cmdline_exists(pCmdParser->pCmdLine, NA_CMD_SOURCE_GPU))
    {
        ...
        unsigned long long src_gpu_id;
        if(!cmdline_getIntegerVal(pCmdParser->pCmdLine, NA_CMD_SOURCE_GPU, &src_gpu_id))
        {
            return NA_ST_BADPARAM;
        }
        
        if((src_gpu_id < 0) || (src_gpu_id >= MAX_GPU)) 
        {
            fprintf(stderr, "Bad source GPU ID. Valid Range %d to %d\n", 0, MAX_GPU - 1);
            return NA_ST_BADPARAM;
        }
        pCmdParser->mSrcGPU = (int)src_gpu_id;
    }

    if (cmdline_exists(pCmdParser->pCmdLine, NA_CMD_DEST_GPU))
    {
        // Get the Destination GPU ID
        unsigned long long dest_gpu_id;
        if(!cmdline_getIntegerVal(pCmdParser->pCmdLine, NA_CMD_DEST_GPU, &dest_gpu_id))
        {
            return NA_ST_BADPARAM;
        }
        
        if((dest_gpu_id < 0) || (dest_gpu_id >= MAX_GPU)) 
        {
            fprintf(stderr, "Bad source GPU ID. Valid Range %d to %d\n", 0, MAX_GPU - 1);
            return NA_ST_BADPARAM;
        }
        pCmdParser->mDestGPU = (int)dest_gpu_id;
    }
