...
#include "explorer8LBConfig.h"
	...
	static HWLink8LB HWLinks[48] = {
        ...
        static uint32_t trunkLBPort[8] = {      // these ports ordinarily connect to another baseboard.
...
explorer8LBConfig::explorer8LBConfig( fabricTopologyEnum topo ) : fabricConfig( topo )
{
...
explorer8LBConfig::~explorer8LBConfig() 
{
    if ( mFabric )
    {
        delete( mFabric );
...
void explorer8LBConfig::makeIngressReqTable( int nodeIndex, int willowIndex ) 
{
    ...
    uint32_t validLB7_0[8]   =   {  0x00000001,     // vc_valid bits to get to loopback port
                                    0x00000010, 
                                    0x00000100, 
                                    0x00001000,
                                    0x0,
                                    ...
                                    0x0,
                                    0x0     };   
    uint32_t validLB15_8[8]  =   {  0x0,            // vc_valid bits to get to loopback port
                                    0x0, 
                                    0x0, 
                                    0x00,
                                    0x00000001,    
    ...
    for ( portIndex = 0; portIndex <= 7; portIndex++ )
    {
        for ( gpuIndex = 0; gpuIndex <= 7; gpuIndex++ )
        {
            ...
            for ( i = 0; i < NUM_INGR_REQ_ENTRIES_PER_GPU; i++, index++)
            {
                ...
                //Now come back from that port to the target GPU
                //Everything in Explorer 8 is last hop and mapped back to 0
                mappedAddr = i << 34;
printf("switch %d,gpu %d, port %d index %d, valid %x %x %x\n",willowIndex,gpuIndex,trunkLBPort[portIndex],index,valid7_0[gpuIndex],valid15_8[gpuIndex],valid17_16[gpuIndex]);
                makeOneIngressReqEntry( nodeIndex,                          // nodeIndex
                                        willowIndex,                        // willowIndex
                                        trunkLBPort[portIndex],                          
                                                                            // portIndex
                                        index,                              // ingress req table index
                                        mappedAddr,                         // 64 bits fabric address
                                        0,                                  // routePolicy
...
void explorer8LBConfig::makeIngressRespTable( int nodeIndex, int willowIndex ) 
{
    ...
    { 
        ...
        // each egress serves 1 ingress from each of the other GPUs
            ...
            {
                // first build the hop to a trunk. 
                index = rlid[gpuIndex];
...
void explorer8LBConfig::makeGangedLinkTable( int nodeIndex, int willowIndex ) 
{
    int32_t portIndex, i;
...
void explorer8LBConfig::makeAccessPorts( int nodeIndex, int willowIndex ) 
{
    int i;
...
void explorer8LBConfig::makeTrunkPorts( int nodeIndex, int willowIndex ) 
{
    int i;
...
void explorer8LBConfig::makeOneWillow( int nodeIndex, int willowIndex ) 
{
...
void explorer8LBConfig::makeOneNode( int nodeIndex, int gpuNum, int willowNum )
{
    int i, j;
...
void explorer8LBConfig::makeNodes() 
{
    int i, nodeIndex = 0;
...
void
explorer8LBConfig::makeOneLwswitch( int nodeIndex, int swIndex )
{
    return;
}

void
explorer8LBConfig::makeRemapTable( int nodeIndex, int swIndex )
{
    return;
}

void
explorer8LBConfig::makeRIDRouteTable( int nodeIndex, int swIndex )
{
    return;
}

void
explorer8LBConfig::makeRLANRouteTable( int nodeIndex, int swIndex )
{
    return;
}
#endif
