...
basicE3600Config6::basicE3600Config6( fabricTopologyEnum topo ) : fabricConfig( topo )
{
    gpuFabricAddrBase[0]  = (uint64_t)0x6AC << 34;
    gpuFabricAddrRange[0] = FAB_ADDR_RANGE_16G * 4;
};

basicE3600Config6::~basicE3600Config6()
{
    if ( mFabric )
    {
        delete( mFabric );
...
void basicE3600Config6::makeIngressReqTable( int nodeIndex, int willowIndex )
{
        ...
        for ( portIndex = 6; portIndex <= 7; portIndex++ )
        {
            ...
            for ( i = 0; i < NUM_INGR_REQ_ENTRIES_PER_GPU; i++, index++)
            {
                ...
                makeOneIngressReqEntry( nodeIndex,     // nodeIndex
                                        ...
                                        mappedAddr,    // 64 bits fabric address
                                        0,             // routePolicy
                                        0x11000000,    // vcModeValid7_0, port 6 to 7
                                        0x00000000,    // vcModeValid15_8
                                        0x00000000,    // vcModeValid17_16
                                        1);            // entryValid
            }
...
void basicE3600Config6::makeIngressRespTable( int nodeIndex, int willowIndex )
{
    ...
    if ( (nodeIndex == 0) && (willowIndex == 0) )
    {
        ...
        for ( portIndex = 0; portIndex <= 5; portIndex++ )
        {
            // gpuIndex 0 is connected to outPort trunk 14 and 15
...
void basicE3600Config6::makeGangedLinkTable( int nodeIndex, int willowIndex )
{
    return;
}

void basicE3600Config6::makeAccessPorts( int nodeIndex, int willowIndex )
{
    if ( (nodeIndex == 0) && (willowIndex == 0) )
    {
...
void basicE3600Config6::makeTrunkPorts( int nodeIndex, int willowIndex )
{
    if ( (nodeIndex == 0) && (willowIndex == 0) )
    {
...
void basicE3600Config6::makeOneWillow( int nodeIndex, int willowIndex )
{
...
void basicE3600Config6::makeOneNode( int nodeIndex, int gpuNum, int willowNum )
{
    int i, j;
...
void basicE3600Config6::makeNodes()
{
    int i, nodeIndex = 0;
    ...
    for (nodeIndex = 0; nodeIndex < 1; nodeIndex++)
    {
        ...
        switch ( nodeIndex ) {
        case 0:
            // node 0 has 1 GPUs, 2 Willow
            makeOneNode( nodeIndex, 1, 2);
            break;
...
void
basicE3600Config6::makeOneLwswitch( int nodeIndex, int swIndex )
{
    return;
}

void
basicE3600Config6::makeRemapTable( int nodeIndex, int swIndex )
{
    return;
}

void
basicE3600Config6::makeRIDRouteTable( int nodeIndex, int swIndex )
{
    return;
}

void
basicE3600Config6::makeRLANRouteTable( int nodeIndex, int swIndex )
{
    return;
}
#endif
