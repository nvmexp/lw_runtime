...
basicE3600Config2::basicE3600Config2( fabricTopologyEnum topo ) : fabricConfig( topo )
{
...
basicE3600Config2::~basicE3600Config2()
{
    if ( mFabric )
    {
        delete( mFabric );
...
void basicE3600Config2::makeIngressReqTable( int nodeIndex, int willowIndex )
{
...
void basicE3600Config2::makeIngressRespTable( int nodeIndex, int willowIndex )
{
    int gpuIndex, portIndex, index, i, enpointID, outPortNum;
    accessPort *outPort;
    ...
    if ( (nodeIndex == 0) && (willowIndex == 0) )
    {
        ...
        {
            gpuIndex  = 1;
            enpointID = nodeIndex * 8 + gpuIndex;
            index     = enpointID * NUM_LWLINKS_PER_GPU;
            ...
            for ( i = 0; i < NUM_LWLINKS_PER_GPU; i++ )
            {
                makeOneIngressRespEntry( nodeIndex,     // nodeIndex
                                         willowIndex,   // willowIndex
                                         portIndex,     // portIndex
                                         index + i,     // Ingress resp table index
                                         0,             // routePolicy
                                         0x11000000,    // vcModeValid7_0
                                         0x00000000,    // vcModeValid15_8
                                         0x00000000,    // vcModeValid17_16
...
void basicE3600Config2::makeGangedLinkTable( int nodeIndex, int willowIndex )
{
    return;
}

void basicE3600Config2::makeAccessPorts( int nodeIndex, int willowIndex )
{
    if ( (nodeIndex == 0) && (willowIndex == 0) )
    {
...
void basicE3600Config2::makeTrunkPorts( int nodeIndex, int willowIndex )
{
    if ( (nodeIndex == 0) && (willowIndex == 0) )
    {
...
void basicE3600Config2::makeOneWillow( int nodeIndex, int willowIndex )
{
...
void basicE3600Config2::makeOneNode( int nodeIndex, int gpuNum, int willowNum )
{
    int i, j;
...
void basicE3600Config2::makeNodes()
{
    int i, nodeIndex = 0;
...
void
basicE3600Config2::makeOneLwswitch( int nodeIndex, int swIndex )
{
    return;
}

void
basicE3600Config2::makeRemapTable( int nodeIndex, int swIndex )
{
    return;
}

void
basicE3600Config2::makeRIDRouteTable( int nodeIndex, int swIndex )
{
    return;
}

void
basicE3600Config2::makeRLANRouteTable( int nodeIndex, int swIndex )
{
    return;
}
#endif
