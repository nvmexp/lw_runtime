...
int
explorerSkylakeConfig::getHwWillowIndex ( int willowIndex )
{
	switch ( willowIndex ) {
	case 0:
		return 2;
	...
	case 2:
		return 0;
	default:
		PRINT_ERROR("%d", "Invalid Willow willowIndex %d.\n", willowIndex);
		return 0;
	}
}

explorerSkylakeConfig::explorerSkylakeConfig( fabricTopologyEnum topo ) : fabricConfig( topo )
{
...
explorerSkylakeConfig::~explorerSkylakeConfig()
{
    if ( mFabric )
    {
        delete( mFabric );
...
void explorerSkylakeConfig::makeIngressReqTable( int nodeIndex, int willowIndex )
{
    int32_t gpuEndpointId, portIndex, portGPU, peerGPU, peerGPULink, linkBase, linkIndex, index, routeIndex;
    int64_t mappedAddr, range, i;
    unsigned int gpuIndex;
    GPU *gpu;
    vcValid valid[16];  // vc_valid bits for each GPU times two ports
    linkBase = getHwWillowIndex(willowIndex) * 16;

    // first populate the vcValid array with correct egress for each ingress
...
void explorerSkylakeConfig::makeIngressRespTable( int nodeIndex, int willowIndex )
{
    int portIndex, i, index, enpointID, outPortNum, linkBase, subIndex, routeIndex;
    accessPort *outPort;
    vcValid valid[16];  // vc_valid bits for each GPU times two ports
...
void explorerSkylakeConfig::makeGangedLinkTable( int nodeIndex, int willowIndex )
{
    int32_t portIndex, i;
...
void explorerSkylakeConfig::makeAccessPorts( int nodeIndex, int willowIndex )
{
    int i;
    int baseIndex = getHwWillowIndex(willowIndex) * 16;
...
void explorerSkylakeConfig::makeTrunkPorts( int nodeIndex, int willowIndex )
{
    return;
}

void explorerSkylakeConfig::makeOneWillow( int nodeIndex, int willowIndex )
{
...
void explorerSkylakeConfig::makeOneNode( int nodeIndex, int gpuNum, int willowNum )
{
    int i, j;
...
void explorerSkylakeConfig::makeNodes()
{
    int i, nodeIndex = 0;
...
void explorerSkylakeConfig::fillSystemPartitionInfo(int nodeIndex)
{
    ...
    // explorer8 bare metal partition information
    ...
    // total interanode trunk connections 0
    bareMetaData1->set_lwlinkintratrunkconncount( 0 );
    // no internode trunk connection for explorer8
...
void
explorerSkylakeConfig::makeOneLwswitch( int nodeIndex, int swIndex )
{
    return;
}

void
explorerSkylakeConfig::makeRemapTable( int nodeIndex, int swIndex )
{
    return;
}

void
explorerSkylakeConfig::makeRIDRouteTable( int nodeIndex, int swIndex )
{
    return;
}

void
explorerSkylakeConfig::makeRLANRouteTable( int nodeIndex, int swIndex )
{
    return;
}
#endif
