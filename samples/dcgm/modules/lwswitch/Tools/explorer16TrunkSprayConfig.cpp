...
explorer16TrunkSprayConfig::explorer16TrunkSprayConfig( fabricTopologyEnum topo ) : fabricConfig( topo )
{
    uint64_t i;
    for (i = 0; i < 16; i++) 
    {
...
explorer16TrunkSprayConfig::~explorer16TrunkSprayConfig() 
{
    if ( mFabric )
    {
        delete( mFabric );
...
void explorer16TrunkSprayConfig::makeIngressReqTable( int nodeIndex, int willowIndex ) 
{
    ...
    // for the far GPUs, spray to all local trunk ports
    for ( gpuIndex = gpuBaseFar; gpuIndex < gpuBaseFar + 8; gpuIndex++ )
    {
        for (int trunkPort = 0; trunkPort < EXPLORER16_NUM_TRUNK_PORTS; trunkPort++ )
        {
            if (trunkPortsNear[trunkPort] < 8)
            {
                valid7_0[gpuIndex] |= 1 << (4 * trunkPortsNear[trunkPort]);
            }
            else if (trunkPortsNear[trunkPort] < 16)
            {
                valid15_8[gpuIndex] |= 1 << (4 * (trunkPortsNear[trunkPort] - 8) );
            }
            else //these are not used for trunk links but we may want this code elsewhere so keeping it generic.
            {
                valid17_16[gpuIndex] |= 1 << (4 * (trunkPortsNear[trunkPort] - 16) );
            }
        }
    }
...
void explorer16TrunkSprayConfig::makeIngressRespTable( int nodeIndex, int willowIndex ) 
{
    ...
    for ( gpuIndex = gpuBaseFar; gpuIndex < gpuBaseFar + 8; gpuIndex++ )
    {
        for ( portIndex = linkBaseFar; portIndex < linkBaseFar + 8; portIndex++ )
        {
            if ( HWLinks16[portIndex].GPUIndex == gpuIndex ) 
            {
                // spray to all local trunk ports
                for (int trunkPort = 0; trunkPort < EXPLORER16_NUM_TRUNK_PORTS; trunkPort++ )
                {
                    if (trunkPortsNear[trunkPort] < 8)
                    {
...
void explorer16TrunkSprayConfig::makeGangedLinkTable( int nodeIndex, int willowIndex ) 
{
    int32_t portIndex, i;
...
void explorer16TrunkSprayConfig::makeAccessPorts( int nodeIndex, int willowIndex ) 
{
    int i;
...
void explorer16TrunkSprayConfig::makeTrunkPorts( int nodeIndex, int willowIndex ) 
{
    int i, farWillowID;
...
void explorer16TrunkSprayConfig::makeOneWillow( int nodeIndex, int willowIndex ) 
{
...
void explorer16TrunkSprayConfig::fillSystemPartitionInfo(int nodeIndex)
{
...
void explorer16TrunkSprayConfig::makeOneNode( int nodeIndex, int gpuNum, int willowNum )
{
    int i, j;
...
void explorer16TrunkSprayConfig::makeNodes() 
{
    int i, nodeIndex = 0;
...
void
explorer16TrunkSprayConfig::makeOneLwswitch( int nodeIndex, int swIndex )
{
    return;
}

void
explorer16TrunkSprayConfig::makeRemapTable( int nodeIndex, int swIndex )
{
    return;
}

void
explorer16TrunkSprayConfig::makeRIDRouteTable( int nodeIndex, int swIndex )
{
    return;
}

void
explorer16TrunkSprayConfig::makeRLANRouteTable( int nodeIndex, int swIndex )
{
    return;
}
#endif
