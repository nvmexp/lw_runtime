...
#define SHARED_FABRIC_SELECTOR_PARTITION_ID_MAX (DCGM_MAX_FABRIC_PARTITIONS-1)
...
typedef struct SharedFabricCmdParser
{
    ...
    unsigned mPartitionId;              /* Partition Id to be activated/deactivated */
    unsigned mNumPartitions;            /* number of partitions in mPartitionList */
    unsigned mPartitionIds[DCGM_MAX_FABRIC_PARTITIONS]; /* List of partitionIds */
...
typedef enum SharedFabricCmdEnum {
    ...
    SHARED_FABRIC_CMD_HOSTNAME,              /* Command-line to host IP address to connect to */
