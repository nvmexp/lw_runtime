...
basicE3600Config7::basicE3600Config7( fabricTopologyEnum topo ) : fabricConfig( topo )
{
...
basicE3600Config7::~basicE3600Config7()
{
    if ( mFabric )
    {
        delete( mFabric );
...
void basicE3600Config7::makeIngressReqTable( int nodeIndex, int willowIndex )
{
...
void basicE3600Config7::makeIngressRespTable( int nodeIndex, int willowIndex )
{
    ...
    if ( (nodeIndex == 0) && (willowIndex == 0) )
    {
        ...
        for ( portIndex = 0; portIndex <= 5; portIndex++ )
        {
            // gpuIndex 1 is connected to outPort trunk 14 and 15
    ...
    if ( (nodeIndex == 0) && (willowIndex == 1) )
    {
        ...
        for ( portIndex = 0; portIndex <= 5; portIndex++ )
        {
            // gpuIndex 0 is connected to outPort trunk 6 and 7
        ...
        for ( portIndex = 6; portIndex <= 7; portIndex++ )
        {
            // gpuIndex 1 is connected to outPort 0 to 5
            for ( outPortNum = 0; outPortNum <= 5; outPortNum++ )
            {
                outPort = accesses[nodeIndex][willowIndex][outPortNum];
                if ( outPort && outPort->has_farpeerid() && outPort->has_farportnum() )
...
void basicE3600Config7::makeGangedLinkTable( int nodeIndex, int willowIndex )
{
    return;
}

void basicE3600Config7::makeAccessPorts( int nodeIndex, int willowIndex )
{
    if ( (nodeIndex == 0) && (willowIndex == 0) )
    {
    ...
    {
        //                nodeIndex willowIndex portIndex farNodeID farPeerID farPortNum portMode
        makeOneAccessPort(0,        1,          0,        0,        1,        1,         DC_COUPLED);
        makeOneAccessPort(0,        1,          1,        0,        1,        0,         DC_COUPLED);
        makeOneAccessPort(0,        1,          2,        0,        1,        5,         DC_COUPLED);
        makeOneAccessPort(0,        1,          3,        0,        1,        4,         DC_COUPLED);
        makeOneAccessPort(0,        1,          4,        0,        1,        2,         DC_COUPLED);
        makeOneAccessPort(0,        1,          5,        0,        1,        3,         DC_COUPLED);
    }
    else
    {
...
void basicE3600Config7::makeTrunkPorts( int nodeIndex, int willowIndex )
{
    if ( (nodeIndex == 0) && (willowIndex == 0) )
    {
        //               nodeIndex willowIndex portIndex farNodeID farSwitchID farPortNum portMode
        makeOneTrunkPort(nodeIndex,willowIndex,14,       0,        1,          7,         AC_COUPLED);
        makeOneTrunkPort(nodeIndex,willowIndex,15,       0,        1,          6,         AC_COUPLED);

    }
    else if ( (nodeIndex == 0) && (willowIndex == 1) )
    {
        //               nodeIndex willowIndex portIndex farNodeID farSwitchID farPortNum portMode
        makeOneTrunkPort(nodeIndex,willowIndex,6,        0,        0,          15,        AC_COUPLED);
        makeOneTrunkPort(nodeIndex,willowIndex,7,        0,        0,          14,        AC_COUPLED);
    }
    else
    {
        PRINT_ERROR("%d,%d", "Invalid Willow nodeIndex %d willowIndex %d.\n", nodeIndex, willowIndex);
...
void basicE3600Config7::makeOneWillow( int nodeIndex, int willowIndex )
{
...
void basicE3600Config7::makeOneNode( int nodeIndex, int gpuNum, int willowNum )
{
    int i, j;
...
void basicE3600Config7::makeNodes()
{
    int i, nodeIndex = 0;
    ...
    for (nodeIndex = 0; nodeIndex < 1; nodeIndex++)
    {
        ...
        switch ( nodeIndex ) {
        case 0:
            // node 0 has 2 GPUs, 2 Willow
            makeOneNode( nodeIndex, 2, 2);
            break;
...
void
basicE3600Config7::makeOneLwswitch( int nodeIndex, int swIndex )
{
    return;
}

void
basicE3600Config7::makeRemapTable( int nodeIndex, int swIndex )
{
    return;
}

void
basicE3600Config7::makeRIDRouteTable( int nodeIndex, int swIndex )
{
    return;
}

void
basicE3600Config7::makeRLANRouteTable( int nodeIndex, int swIndex )
{
    return;
}
#endif
