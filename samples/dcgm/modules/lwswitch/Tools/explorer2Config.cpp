...
// Select the GPU pair you want to use here
                            
        static int selectGPUs[2] = { 0, 1 };




explorer2Config::explorer2Config( fabricTopologyEnum topo ) : fabricConfig( topo )
{
...
explorer2Config::~explorer2Config() 
{
    if ( mFabric )
    {
        delete( mFabric );
...
void explorer2Config::makeIngressReqTable( int nodeIndex, int willowIndex ) 
{
    ...
    {
        for ( selectIndex = 0; selectIndex <= 1; selectIndex++ )
        {
            gpuIndex = selectGPUs[selectIndex];
            if ( HWLinks[linkBase + portIndex].GPUIndex == gpuIndex )
            {
                // skip over the loopback case
                continue;
            }
            if ( HWLinks[linkBase + portIndex].GPUIndex != selectGPUs[0] &&
                 HWLinks[linkBase + portIndex].GPUIndex != selectGPUs[1] )
//            if ( HWLinks[linkBase + portIndex].GPUIndex != selectGPUs[selectIndex ^ 1] )
            {
                // skip over the loopback case
                continue;
            }
...
void explorer2Config::makeIngressRespTable( int nodeIndex, int willowIndex ) 
{
    int gpuIndex, portIndex, i, index, enpointID, outPortNum, linkBase, subIndex, routeIndex, selectIndex;
    accessPort *outPort;
    unsigned int rlid[8];
    uint32_t valid7_0[8]   = {0};  // vc_valid bits for each GPU 
    uint32_t valid15_8[8]  = {0};
    ...
    { 
        for ( selectIndex = 0; selectIndex < 2; selectIndex++ )
        {
            gpuIndex = selectGPUs[selectIndex];
...
void explorer2Config::makeGangedLinkTable( int nodeIndex, int willowIndex ) 
{
    int32_t portIndex, i;
...
void explorer2Config::makeAccessPorts( int nodeIndex, int willowIndex ) 
{
    int i;
    ...
    for ( i = 0; i < 8; i++ )   
    {
        ...
        if ( HWLinks[baseIndex + i].GPUIndex == selectGPUs[0] || HWLinks[baseIndex + i].GPUIndex == selectGPUs[1] )
        {
...
void explorer2Config::makeTrunkPorts( int nodeIndex, int willowIndex ) 
{
    return;
}

void explorer2Config::makeOneWillow( int nodeIndex, int willowIndex ) 
{
...
void explorer2Config::makeOneNode( int nodeIndex, int gpuNum, int willowNum )
{
    int i, j;
    for ( i = 0; i < 8; i++)
    {
        gpus[nodeIndex][i] = nodes[nodeIndex]->add_gpu();
        gpus[nodeIndex][i]->set_version( FABRIC_MANAGER_VERSION );
    }
    // Add GPUs
    for ( i = 0; i < gpuNum; i++ )
    {
        
        makeOneGpu( nodeIndex, selectGPUs[i], ((nodeIndex * 8) + selectGPUs[i]), 0, 0x3F, 0xFFFFFFFF,
                    gpuFabricAddrBase[selectGPUs[i]], gpuFabricAddrRange[selectGPUs[i]], selectGPUs[i]);
...
void explorer2Config::makeNodes() 
{
    int i, nodeIndex = 0;
    ...
    for (nodeIndex = 0; nodeIndex < 1; nodeIndex++)
    {
        ...
        switch ( nodeIndex ) {
        case 0:
            // node 0 has 2 GPUs, 6 Willows
            makeOneNode( nodeIndex, 2, 6);
            break;
...
void
explorer2Config::makeOneLwswitch( int nodeIndex, int swIndex )
{
    return;
}

void
explorer2Config::makeRemapTable( int nodeIndex, int swIndex )
{
    return;
}

void
explorer2Config::makeRIDRouteTable( int nodeIndex, int swIndex )
{
    return;
}

void
explorer2Config::makeRLANRouteTable( int nodeIndex, int swIndex )
{
    return;
}
#endif
