...
extern "C" {
#include "dcgm_vgpu_entry_point.h"
}
...
//   module_* numbering
DCGM_INIT_EXTERN_CONST etblDCGMVgpuInternal g_etblDCGMVgpuInternal =
{
        sizeof (g_etblDCGMVgpuInternal),
        dcgmVgpuStart,                // 1
        dcgmVgpuShutdown,             // 2
};
...
/*****************************************************************************/
dcgmReturn_t tsapiVgpuStart(dcgmHandle_t pDcgmHandle, dcgm_vgpu_msg_start_t *startMsg)
{
    dcgmReturn_t dcgmReturn;

    if(startMsg->header.version != dcgm_vgpu_msg_start_version)
    {
        PRINT_ERROR("", "dcgm_vgpu_msg_start_version version mismatch");
        return DCGM_ST_VER_MISMATCH;
    }

    startMsg->header.length = sizeof(*startMsg);
    startMsg->header.moduleId = DcgmModuleIdVGPU;
    startMsg->header.subCommand = DCGM_VGPU_SR_START;
...
/*****************************************************************************/
dcgmReturn_t tsapiVgpuShutdown(dcgmHandle_t pDcgmHandle, dcgm_vgpu_msg_shutdown_t *shutdownMsg)
{
    dcgmReturn_t dcgmReturn;

    if(shutdownMsg->header.version != dcgm_vgpu_msg_shutdown_version)
    {
        PRINT_ERROR("", "dcgm_vgpu_shutdown_version version mismatch");
        return DCGM_ST_VER_MISMATCH;
    }

    shutdownMsg->header.length = sizeof(*shutdownMsg);
    shutdownMsg->header.moduleId = DcgmModuleIdVGPU;
    shutdownMsg->header.subCommand = DCGM_VGPU_SR_SHUTDOWN;
