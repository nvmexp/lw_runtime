 ...
 * ctor
 *****************************************************************************/
DcgmPolicyManager::DcgmPolicyManager(LwcmHostEngineHandler *hostEngineHandler)
{
	mpHostEngineHandler = hostEngineHandler;
    mpCacheManager = hostEngineHandler->GetCacheManager();
    mpGroupManager = hostEngineHandler->GetGroupManager();
...
{
    int deviceCount = mpCacheManager->GetGpuCount(0);
    if (deviceCount < 0) 
    {
    ...
    for (int i = 0; i < DCGM_MAX_NUM_DEVICES; i++)
    {
        ...
        memset(&m_gpus[i].lwrrentPolicies, 0, sizeof(m_gpus[i].lwrrentPolicies));
        m_gpus[i].lwrrentPolicies.version = dcgmPolicy_version;
...
/****************************************************************************/
void DcgmPolicyManager::SetViolation(DcgmViolationPolicyAlert_t alertType, unsigned int gpuId, 
                                     int64_t timestamp,
...
{   
    ...
    for(watcherIt = m_gpus[gpuId].watchers.begin(); 
        watcherIt != m_gpus[gpuId].watchers.end(); ++watcherIt)
    {
        ...
        {
            PRINT_DEBUG("%u %lld", "Not violating type %u due to timestamp difference being < %lld", 
                        alertType, (long long)minimumSignalTimeDiff);
            continue;
        }

        /* OK. We're going to signal this watcher */
        PRINT_DEBUG("%u %u %u %u %lld", "Notifying alertType %u, gpuId %u, connectionId %u, requestId %u, ts %lld",
                    alertType, gpuId, watcherIt->connectionId, watcherIt->requestId, (long long)timestamp);
        
        notify.begin = 1;
        mpHostEngineHandler->SendRawMessageToClient(watcherIt->connectionId, DCGM_MSG_POLICY_NOTIFY, 
                                                    watcherIt->requestId, &notify, sizeof(notify));
...
dcgmReturn_t DcgmPolicyManager::UnregisterForPolicy(dcgm_policy_msg_unregister_t *msg)
{
    ...
    {
        PRINT_ERROR("%u %d", "Error: Bad group id parameter: %u, ret %d", 
                    groupId, dcgmReturn);
        return dcgmReturn;
    }
...
dcgmReturn_t DcgmPolicyManager::CheckEccErrors(dcgmBufferedFv_t *fv)
{
    ...
    {
        PRINT_DEBUG("%u %u %d", "Skipping gpuId %u ECC fieldId %u with status %d",
                    fv->entityId, fv->fieldId, fv->status);
        return DCGM_ST_OK;
    }

    if(!(m_gpus[fv->entityId].lwrrentPolicies.condition & DCGM_POLICY_COND_DBE))
    {
        PRINT_DEBUG("%u %u %X", "Skipping gpuId %u ECC fieldId %u with condition mask x%X",
                    fv->entityId, fv->fieldId, m_gpus[fv->entityId].lwrrentPolicies.condition);
        return DCGM_ST_OK;
    }

    unsigned int errorCount = fv->value.i64;
    PRINT_DEBUG("%u %u", "CheckEccErrors gpuId %u, errorCount %u", 
                fv->entityId, errorCount);

    if (errorCount > 0) // violation has oclwrred 
    {
        ...
        PRINT_ERROR("%u %u", "gpuId %u has > 0 ECC double-bit errors: %u", 
                    fv->entityId, errorCount);
        SetViolation(DCGM_VIOLATION_POLICY_FAIL_ECC_DBE, fv->entityId, 
                     fv->timestamp, &callbackResponse);
    }
...
dcgmReturn_t DcgmPolicyManager::CheckPcieErrors(dcgmBufferedFv_t *fv)
{
    if(fv->status != DCGM_ST_OK || DCGM_INT64_IS_BLANK(fv->value.i64))
    {
        PRINT_DEBUG("%u %u %d", "Skipping gpuId %u PCIe fieldId %u with status %d",
                    fv->entityId, fv->fieldId, fv->status);
        return DCGM_ST_OK;
    }

    if(!(m_gpus[fv->entityId].lwrrentPolicies.condition & DCGM_POLICY_COND_PCI))
    {
        PRINT_DEBUG("%u %u %X", "Skipping gpuId %u PCIe fieldId %u with condition mask x%X",
                    fv->entityId, fv->fieldId, m_gpus[fv->entityId].lwrrentPolicies.condition);
        return DCGM_ST_OK;
    }
    ...
    if (errorCount > 0)
    {
        ...
        PRINT_ERROR("%u %u", "gpuId %u has > 0 PCIe replays: %u. This may be causing throughput issues.", 
                    fv->entityId, errorCount);
        SetViolation(DCGM_VIOLATION_POLICY_FAIL_PCIE, fv->entityId, 
                     fv->timestamp, &callbackResponse);
    }
...
dcgmReturn_t DcgmPolicyManager::CheckRetiredPages(dcgmBufferedFv_t *fv)
{
    if(fv->status != DCGM_ST_OK || DCGM_INT64_IS_BLANK(fv->value.i64))
    {
        PRINT_DEBUG("%u %u %d", "Skipping gpuId %u retired pages fieldId %u with status %d",
                    fv->entityId, fv->fieldId, fv->status);
        return DCGM_ST_OK;
    }

    if(!(m_gpus[fv->entityId].lwrrentPolicies.condition & DCGM_POLICY_COND_MAX_PAGES_RETIRED))
    {
        PRINT_DEBUG("%u %u %X", "Skipping gpuId %u retired pages fieldId %u with condition mask x%X",
                    fv->entityId, fv->fieldId, m_gpus[fv->entityId].lwrrentPolicies.condition);
        return DCGM_ST_OK;
    }
    ...
    if(fv->fieldId == DCGM_FI_DEV_RETIRED_DBE)
    {
        ...
        dcgmReturn = mpCacheManager->GetLatestSample(DCGM_FE_GPU, fv->entityId, DCGM_FI_DEV_RETIRED_SBE, &sample, 0);
        if (dcgmReturn)
        {
    ...
    else
    {
        ...
        dcgmReturn = mpCacheManager->GetLatestSample(DCGM_FE_GPU, fv->entityId, DCGM_FI_DEV_RETIRED_DBE, &sample, 0);
        if (dcgmReturn)
        {
    ...
    unsigned int maxRetiredPages = (unsigned int)m_gpus[fv->entityId].lwrrentPolicies.parms[DCGM_VIOLATION_POLICY_FAIL_MAX_RETIRED_PAGES].val.llval;
    ...
    if (pageCountDbe + pageCountSbe > maxRetiredPages)
    {
        ...
        PRINT_ERROR("%u %u %u", "gpuId %u exceeds the max retired pages count: %u > maximum allowed %u.", 
                    fv->entityId, pageCountDbe + pageCountSbe, maxRetiredPages);
        SetViolation(DCGM_VIOLATION_POLICY_FAIL_MAX_RETIRED_PAGES, fv->entityId, 
                     fv->timestamp, &callbackResponse);
    }
...
dcgmReturn_t DcgmPolicyManager::CheckThermalValues(dcgmBufferedFv_t *fv)
{
    if(fv->status != DCGM_ST_OK || DCGM_INT64_IS_BLANK(fv->value.i64))
    {
        PRINT_DEBUG("%u %u %d", "Skipping gpuId %u temperature fieldId %u with status %d",
                    fv->entityId, fv->fieldId, fv->status);
        return DCGM_ST_OK;
    }

    if(!(m_gpus[fv->entityId].lwrrentPolicies.condition & DCGM_POLICY_COND_THERMAL))
    {
        PRINT_DEBUG("%u %u %X", "Skipping gpuId %u temperature fieldId %u with condition mask x%X",
                    fv->entityId, fv->fieldId, m_gpus[fv->entityId].lwrrentPolicies.condition);
        return DCGM_ST_OK;
    }

    unsigned int gpuTemp = (unsigned int)fv->value.i64;
    unsigned int maxTemp = (unsigned int)m_gpus[fv->entityId].lwrrentPolicies.parms[DCGM_VIOLATION_POLICY_FAIL_THERMAL].val.llval;
    if ((unsigned int) gpuTemp > maxTemp)
    {
        ...
        PRINT_ERROR("%u %u %u", "gpuId %u has violated thermal settings: %u > max allowed temp %u.", 
                    fv->entityId, gpuTemp, maxTemp);
        SetViolation(DCGM_VIOLATION_POLICY_FAIL_THERMAL, fv->entityId, 
                     fv->timestamp, &callbackResponse);
    }
...
dcgmReturn_t DcgmPolicyManager::CheckPowerValues(dcgmBufferedFv_t *fv)
{
    if(fv->status != DCGM_ST_OK || DCGM_INT64_IS_BLANK(fv->value.i64))
    {
        PRINT_DEBUG("%u %u %d", "Skipping gpuId %u power fieldId %u with status %d",
                    fv->entityId, fv->fieldId, fv->status);
        return DCGM_ST_OK;
    }

    if(!(m_gpus[fv->entityId].lwrrentPolicies.condition & DCGM_POLICY_COND_POWER))
    {
        PRINT_DEBUG("%u %u %X", "Skipping gpuId %u power fieldId %u with condition mask x%X",
                    fv->entityId, fv->fieldId, m_gpus[fv->entityId].lwrrentPolicies.condition);
        return DCGM_ST_OK;
    }

    unsigned int gpuPower = (unsigned int)fv->value.dbl;
    unsigned int maxPower = (unsigned int)m_gpus[fv->entityId].lwrrentPolicies.parms[DCGM_VIOLATION_POLICY_FAIL_POWER].val.llval;
    if (gpuPower > maxPower)
    {
        ...
        PRINT_ERROR("%u %u %u", "gpuId %u has violated power settings: %u > max allowed %u", 
                    fv->entityId, gpuPower, maxPower);
...
/*****************************************************************************/
dcgmReturn_t DcgmPolicyManager::CheckLWLinkErrors(dcgmBufferedFv_t *fv)
{
    if(fv->status != DCGM_ST_OK || DCGM_INT64_IS_BLANK(fv->value.i64))
    {
        PRINT_DEBUG("%u %u %d", "Skipping gpuId %u LwLink counter fieldId %u with status %d",
                    fv->entityId, fv->fieldId, fv->status);
        return DCGM_ST_OK;
    }

    if(!(m_gpus[fv->entityId].lwrrentPolicies.condition & DCGM_POLICY_COND_LWLINK))
    {
        PRINT_DEBUG("%u %u %X", "Skipping gpuId %u LwLink counter fieldId %u with condition mask x%X",
                    fv->entityId, fv->fieldId, m_gpus[fv->entityId].lwrrentPolicies.condition);
        return DCGM_ST_OK;
    }
    ...
    {
        dcgmPolicyCallbackResponse_t callbackResponse;
        dcgmPolicyConditionLwlink_t lwlinkResponse;

        callbackResponse.version = dcgmPolicyCallbackResponse_version;
        callbackResponse.condition = DCGM_POLICY_COND_LWLINK;
        lwlinkResponse.timestamp = fv->timestamp;
        lwlinkResponse.fieldId = (unsigned short)fv->fieldId;
        lwlinkResponse.counter = fv->value.i64;

        callbackResponse.val.lwlink = lwlinkResponse;

        PRINT_ERROR("%u %s %lld", "gpuId %u has > 0 Lwlink %s: %lld. This may be causing throughput issues.", 
                    fv->entityId, ColwertLWLinkCounterTypeToString(fv->fieldId), (long long)fv->value.i64);
        SetViolation(DCGM_VIOLATION_POLICY_FAIL_LWLINK, fv->entityId, fv->timestamp, &callbackResponse);
    }
    return DCGM_ST_OK;
}
...
dcgmReturn_t DcgmPolicyManager::CheckXIDErrors(dcgmBufferedFv_t *fv)
{
	if(fv->status != DCGM_ST_OK || DCGM_INT64_IS_BLANK(fv->value.i64))
    {
        PRINT_DEBUG("%u %u %d", "Skipping gpuId %u XID fieldId %u with status %d",
                    fv->entityId, fv->fieldId, fv->status);
        return DCGM_ST_OK;
    }

    if(!(m_gpus[fv->entityId].lwrrentPolicies.condition & DCGM_POLICY_COND_XID))
    {
        PRINT_DEBUG("%u %u %X", "Skipping gpuId %u XID fieldId %u with condition mask x%X",
                    fv->entityId, fv->fieldId, m_gpus[fv->entityId].lwrrentPolicies.condition);
        return DCGM_ST_OK;
    }
    ...
    PRINT_ERROR("%u %d", "gpuId %u has XID error: %d.", 
                fv->entityId, (int)fv->value.i64);
    SetViolation(DCGM_VIOLATION_POLICY_FAIL_XID, fv->entityId, 
                 fv->timestamp, &callbackResponse);
    return DCGM_ST_OK;
}

/*****************************************************************************/
char* DcgmPolicyManager::ColwertLWLinkCounterTypeToString(unsigned short fieldId)
{
...
dcgmReturn_t DcgmPolicyManager::WatchFields(dcgm_connection_id_t connectionId)
{
    ...
    static unsigned short fieldIds[11] = {DCGM_FI_DEV_LWLINK_CRC_FLIT_ERROR_COUNT_TOTAL,
    ...
    for(gpuId = 0; gpuId < (unsigned int)deviceCount; gpuId++)
    {
        for(i = 0; i < numFieldIds; i++)
        {
            ...
            dcgmReturn = mpCacheManager->AddFieldWatch(DCGM_FE_GPU, gpuId, fieldIds[i], 10000000, 
                                                       3600.0, 0, watcher, true);
    ...
    mpCacheManager->UpdateAllFields(1);
...
dcgmReturn_t DcgmPolicyManager::RegisterForPolicy(dcgm_policy_msg_register_t *msg)
{
    ...
    /* Does this group of GPUs allow setting policy? */
    int supportPolicyMgmt = 0;
    dcgmReturn = mpGroupManager->AllSupportPolicyManagement(0, groupId, &supportPolicyMgmt);
    if(dcgmReturn != DCGM_ST_OK)
    {
        PRINT_ERROR("%u %d", "AllSupportPolicyManagement() for groupId %u failed with %d", 
                    groupId, (int)dcgmReturn);
        return dcgmReturn;
    }
    if(!supportPolicyMgmt)
    {
        PRINT_DEBUG("%u", "Policy management is not supported for at least one GPU of group %u",
                    groupId);
        return DCGM_ST_NOT_SUPPORTED;
    }
    else
    {
        PRINT_DEBUG("%u", "policy management is supported for gpu group %u", groupId);
    }
    ...
    for (unsigned int i = 0; i < entities.size(); i++)
    {
        ...
        PRINT_DEBUG("%X %u %u %u", "Added policy condition x%X, gpuId %u, connectionId %u, requestId %u",
                    newWatcher.conditions, entities[i].entityId,
                    newWatcher.connectionId, newWatcher.requestId);
    }
...
dcgmReturn_t DcgmPolicyManager::ProcessSetPolicy(dcgm_policy_msg_set_policy_t *msg)
{
    ...
    /* Does this group of GPUs allow setting policy? */
    int supportPolicyMgmt = 0;
    dcgmReturn = mpGroupManager->AllSupportPolicyManagement(msg->header.connectionId, groupId, 
                                                            &supportPolicyMgmt);
    if(dcgmReturn != DCGM_ST_OK)
    {
        PRINT_ERROR("%u %d", "AllSupportPolicyManagement() for groupId %u failed with %d",
    ...
    if (!entities.size())
    {
        ...
        PRINT_ERROR("%u", "Group not set for group ID %u for SET_LWRRENT_VIOL_POLICY",
                    groupId);
    ...
    for (i = 0; i < entities.size(); i++) 
    {
        ...
        memcpy(&m_gpus[gpuId].lwrrentPolicies, &msg->policy, 
               sizeof(m_gpus[gpuId].lwrrentPolicies));
        
        PRINT_DEBUG("%u %X %u", "connectionId %u set policy mask x%X for gpuId %u", 
                    msg->header.connectionId, msg->policy.condition, gpuId);
    }
...
dcgmReturn_t DcgmPolicyManager::ProcessGetPolicies(dcgm_policy_msg_get_policies_t *msg)
{
    ...
    for (i = 0; i < entities.size() && msg->numPolicies < DCGM_MAX_NUM_DEVICES; i++)
    {
        ...
        memcpy(&msg->policies[msg->numPolicies], &m_gpus[gpuId].lwrrentPolicies, 
               sizeof(msg->policies[0]));
        msg->numPolicies++;
    }
...
void DcgmPolicyManager::RemoveWatchersForConnection(dcgm_connection_id_t connectionId)
{
    ...
    for(i = 0; i < m_numGpus; i++)
    {
        ...
        for(watcherIt = m_gpus[i].watchers.begin(); 
            watcherIt != m_gpus[i].watchers.end(); )
        {
            ...
            PRINT_DEBUG("%u %u %d", "Saw connectionId %u request Id %u on gpuId %d",
                        connectionId, watcherIt->requestId, i);
    ...
    for(requestIt = seenRequestIds.begin(); 
        requestIt != seenRequestIds.end(); 
        ++requestIt)
    {
        PRINT_DEBUG("%u %u", "Notifying connectionId %u, requestId %u of completion.",
                    connectionId, *requestIt);
        mpHostEngineHandler->NotifyRequestOfCompletion(connectionId, *requestIt);
    }
}
