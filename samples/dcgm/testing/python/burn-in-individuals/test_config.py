...
def helper_verify_config_values_standalone(handle, groupId, expected_power, expected_ecc, \
    ...
    groupInfo = dcgm_agent.dcgmGroupGetInfo(handle, groupId, dcgm_structs.c_dcgmGroupInfo_version1)
    ...
    for x in xrange(0,groupInfo.count):
            ...
            assert config_values[x].mPerfState.milwPState.memClk == expected_mem_clock, "The min mem clock value for gpuID %d is incorrect."\
                                    " Returned: %d Expected: %d" \
                                    % (x, config_values.mPerfState.milwPState.memClk , expected_mem_clock)
                                    
            assert config_values[x].mPerfState.milwPState.procClk  == expected_proc_clock, "The min proc clock value for gpuID %d is incorrect."\
                                    " Returned: %d Expected: %d" \
                                    % (x, config_values[x].mPerfState.milwPState.procClk , expected_proc_clock)
...
ecc_set = 1
groupInfo = dcgm_agent.dcgmGroupGetInfo(handle, groupId, dcgm_structs.c_dcgmGroupInfo_version1)
config_values = dcgm_agent.dcgmConfigGet(handle, groupId, dcgm_structs.DCGM_CONFIG_LWRRENT_STATE, groupInfo.count, 0)
...
config_values.mPerfState.milwPState.memClk =  mem_clk_set
config_values.mPerfState.milwPState.procClk = proc_clk_set
...
if len(errors) > 0:
    ...
    for error in errors:
        if error.fieldId == dcgm_fields.DCGM_FI_DEV_ECC_LWRRENT:
