...
def create_parser(
        ...
        log_file = None,
        log_level = 'CRITICAL',
        dcgm_hostname = elwiron.get('DCGM_HOSTNAME') or 'localhost',
):
    '''
    ...
    parser.add_argument('--log-level', dest='loglevel', type=str, default=log_level,
                        help='Specify a log level to use for logging.\n\tCRITICAL (0) - log only critical errors that drastically affect exelwtion' +
                             '\n\tERROR (1) - Log any error in exelwtion\n\tWARNING (2) - Log all warnings and errors that occur' +
                             '\n\tINFO (3) - Log informational messages about program exelwtion in addition to warnings and errors' +
                             '\n\tDEBUG (4) - Log debugging information in addition to all information about exelwtion' +
    ...
    group.add_argument('-e', '--embedded', dest='embedded', action='store_true',
                       help='Launch DCGM from within this process instead of connecting to lw-hostengine.')

    return parser

def add_lwstom_argument(parser, *args, **kwargs):
