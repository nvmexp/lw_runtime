...
def dcgmGetAllSupportedDevices(dcgm_handle):
    ...
    dcgm_structs._dcgmCheckReturn(ret)
    return map (None, c_gpuid_list[0:c_count.value])
...
def dcgmGetDeviceAttributes(dcgm_handle, gpuId):
    ...
    device_values.version = dcgm_structs.dcgmDeviceAttributes_version1
...
def dcgmGetEntityGroupEntities(dcgm_handle, entityGroup, flags):
    ...
    dcgm_structs._dcgmCheckReturn(ret)
    return c_entityIds[0:c_count.value]

def dcgmGetLwLinkLinkStatus(dcgm_handle):
    linkStatus = dcgm_structs.c_dcgmLwLinkStatus_v2()
    linkStatus.version = dcgm_structs.dcgmLwLinkStatus_version2
    fn = dcgmFP("dcgmGetLwLinkLinkStatus")
...
def dcgmGroupGetInfo(dcgm_handle, group_id, version=dcgm_structs.c_dcgmGroupInfo_version2):
    ...
    if version == dcgm_structs.c_dcgmGroupInfo_version1:
        device_values = dcgm_structs.c_dcgmGroupInfo_v1()
        device_values.version = dcgm_structs.c_dcgmGroupInfo_version1
    else:
...
def dcgmGroupGetAllIds(dcgmHandle):
    ...
    dcgm_structs._dcgmCheckReturn(ret)
    return map (None, c_groupIdList[0:c_count.value])
...
def dcgmHealthCheck_V1(dcgm_handle, groupId):
    c_results = dcgm_structs.c_dcgmHealthResponse_v1()
    c_results.version = dcgm_structs.dcgmHealthResponse_version1
    fn = dcgmFP("dcgmHealthCheck")
    ret = fn(dcgm_handle, groupId, byref(c_results))
    dcgm_structs._dcgmCheckReturn(ret)
    return c_results

def dcgmHealthCheck(dcgm_handle, groupId, version=dcgm_structs.dcgmHealthResponse_version2):
    if version == dcgm_structs.dcgmHealthResponse_version1:
        c_results = dcgm_structs.c_dcgmHealthResponse_v1()
        c_results.version = dcgm_structs.dcgmHealthResponse_version1
    elif version == dcgm_structs.dcgmHealthResponse_version2:
        c_results = dcgm_structs.c_dcgmHealthResponse_v2()
        c_results.version = dcgm_structs.dcgmHealthResponse_version2
    else:
        c_results = dcgm_structs.c_dcgmHealthResponse_v3()
        c_results.version = dcgm_structs.dcgmHealthResponse_version3
    fn = dcgmFP("dcgmHealthCheck")
    ret = fn(dcgm_handle, groupId, byref(c_results))
    dcgm_structs._dcgmCheckReturn(ret)
    return c_results
...
def dcgmActiolwalidate_v2(dcgm_handle, runDiagInfo, runDiagVersion=dcgm_structs.dcgmRunDiag_version1):
    response = dcgm_structs.c_dcgmDiagResponse_v5()
    runDiagInfo.version = runDiagVersion
    response.version = dcgm_structs.dcgmDiagResponse_version5
    ...
    try:
        ...
        if response.systemError != "":
            ...
            import sys
            info = "%s" % response.systemError
            e.SetAdditionalInfo(info)
            raise type(e), e, sys.exc_info()[2] # pylint: disable=E0710
        else:
            raise
    return response

def dcgmActiolwalidate(dcgm_handle, group_id, validate):
    response = dcgm_structs.c_dcgmDiagResponse_v4()
    response.version = dcgm_structs.dcgmDiagResponse_version4
...
def dcgmGetPidInfo(dcgm_handle, groupId, pid):
    ...
    pidInfo.version = dcgm_structs.dcgmPidInfo_version1
    pidInfo.pid = pid
...
def dcgmJobGetStats(dcgm_handle, jobid):
    ...
    jobInfo.version = dcgm_structs.dcgmJobInfo_version2
...
def dcgmVersionInfo():
    msg = dcgm_structs.c_dcgmVersionInfo_v1()
    msg.version = dcgm_structs.dcgmVersionInfo_version1
