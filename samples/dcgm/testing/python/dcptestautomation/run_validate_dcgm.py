...
# 3.) Kills any existing instance of lw-hostengine and starts a new one.
...
DCGM_FI_PROF_SM_OCLWPANCY = 1003
...
class RulwalidateDcgm:
    def __init__(self):
        ...
        self.metrics_util_list = [DCGM_FI_PROF_GR_ENGINE_ACTIVE, DCGM_FI_PROF_SM_ACTIVE, \
                                  DCGM_FI_PROF_SM_OCLWPANCY, DCGM_FI_PROF_DRAM_ACTIVE, \
        ...
        self.metrics_label = {DCGM_FI_PROF_GR_ENGINE_ACTIVE:'GRACT', \
                            ...
                            DCGM_FI_PROF_SM_OCLWPANCY:'SMOCC', \
    ...
    def getLatestDcgm(self):
        ...
        cmd = 'wget -q -O - http://cqa-fs01/dvsshare/dcgm/daily/r418_00/ | grep -Eo \
                \\2019[0-9]{8} | tail -1'
        ret, folder_name = util.exelwteBashCmd(cmd, True)
        if "$folder_name" == "":
    ...
    def removeBinaries(self, prnt):
        ...
        ret = util.exelwteBashCmd("sudo rm -rf testing_dcgm*", prnt)
        ret = util.exelwteBashCmd("sudo rm -rf datacenter-gpu-manager*.deb", prnt)
        ret = util.exelwteBashCmd("sudo rm -rf _out/", prnt)
        ret = util.exelwteBashCmd("sudo rm -rf /usr/bin/dcgmproftester", prnt)
        ret = util.exelwteBashCmd("sudo rm -rf *.txt", prnt)
        ret = util.exelwteBashCmd("sudo rm -rf *.csv", prnt)
        ret = util.exelwteBashCmd("sudo rm -rf *.pyc", prnt)

        #Uninstall dcgmi
        print "Removing existing installation of dcgmi"
        uninstall_cmd = "sudo dpkg --purge datacenter-gpu-manager-dcp-nda-only"
        ret = util.exelwteBashCmd(uninstall_cmd, prnt)
        if ret[0] != 0:
    ...
    def killLwHostEngine(self):
        print "\n&&&& INFO: Killing any existing lwhostengine instance"
        ret = util.exelwteBashCmd("sudo /usr/bin/lw-hostengine -t", True)

        print "\n&&&& INFO: Stopping dcgm service "
        ret = util.exelwteBashCmd("sudo service dcgm stop", True)

    def startLwHostEngine(self):
        print "\n&&&& INFO: Killing any existing lwhostengine instance"
        ret = util.exelwteBashCmd("sudo /usr/bin/lw-hostengine -t", True)
        ...
        print "\n&&&& INFO: Starting lwhostengine"
        ret = util.exelwteBashCmd("sudo /usr/bin/lw-hostengine", True)

        print "\n&&&& INFO: dcgmi discovery output"
        ret = util.exelwteBashCmd("sudo /usr/bin/dcgmi discovery -l", True)

        return ret

    def installDcgm(self):
        print "\n&&&& INFO: Installing latest version of datacenter-gpu-manager-dcp-nda-on"
        ret = util.exelwteBashCmd("sudo dpkg -i datacenter-gpu-manager-dcp-nda-only_1.6.4_amd64.deb", True)
        if ret[0] != 0:
    ...
    def installProfTester(self):
        ret = util.exelwteBashCmd("tar xvf testing_dcgm.tar.gz", True)
        if ret[0] == 0:
            ret = util.exelwteBashCmd("sudo cp _out/Linux_amd64_release/testing/apps/amd64/dcgmproftester /usr/bin/", True)
        else:
    ...
    def _runDcgmLoadProfilingModule(self):
        ...
        ret = util.exelwteBashCmd("timeout 3s /usr/bin/dcgmi dmon -e 1001 -i 0", False)

    #############################################################################################
    #
    # This function is called when dcgm thread is spawned to collect the metrics.
    # It exelwtes dcgmi to collect metrics on all GPU's under test for a specified amount of time.
    # All information regarding how many GPU's to run the test on, time to run the tests, which
    # metrics to gather is specified by user. This thread is exelwting in parallel, control
    ...
    def _runDcgm(self, metrics, gpuid_list, time):
        ...
        ret = util.exelwteBashCmd("echo {0} | timeout {0}s /usr/bin/dcgmi dmon -e {1} -i {2} 2>&1 | tee dcgmLogs_{3}.txt".format(time, metrics, gpuid_list, metrics), False)

    #############################################################################################
    #
    # This function is called when dcgmproftester thread is spawned to generate a workload.
    # It exelwtes dcgmproftester to generate a workload on one GPU under test for a specified
    ...
    # This thread is exelwting in parallel, control immediately returns to the calling function.
    #
    #############################################################################################
    def _runProftester(self, gpuIndex, metric, time):
        metrics = str(metric)
        print "\n&&&& INFO: Running dcgmproftester to collect metrics on gpu {0}".format(gpuIndex)
        os.elwiron['LWDA_VISIBLE_DEVICES'] = str(gpuIndex)
        util.exelwteBashCmd("echo {0} | /usr/bin/dcgmproftester -d {0} -t {1} 2>&1 | tee dcgmLogsproftester_{2}_gpu{3}.txt".format(time, metrics, metrics, gpuIndex), False)
    ...
    def getMarginRange(self, metrics):
        if metrics == DCGM_FI_PROF_PCIE_RX_BYTES or metrics == DCGM_FI_PROF_PCIE_TX_BYTES:
            return 17.0, -10.0, 17.0
                ...
                or metrics == DCGM_FI_PROF_SM_OCLWPANCY or metrics == DCGM_FI_PROF_DRAM_ACTIVE:
    ...
    def getClosestIndex(self, dcgm_list, prof_list, metrics, dcgm_init_val):
        ...
        elif metrics == DCGM_FI_PROF_GR_ENGINE_ACTIVE or metrics == DCGM_FI_PROF_SM_ACTIVE or metrics == DCGM_FI_PROF_SM_OCLWPANCY:
    ...
    #
    #############################################################################################
    def validateAclwracyForRanges(self, dcgmCsvFile, gpu_index, metrics):
        ret = 0
        f1 = file(dcgmCsvFile, 'r')
    ...
    #
    #############################################################################################
    def validateAclwracyForUtilForUtil(self, dcgmCsvFile, dcgmProfTesterCsvFile, gpu_index, metrics):
        i = 0
        mismatches = 0
        spikes = 0
        dcgm_init_index = 5
        self.upper_range, self.lower_range, self.init_range = self.getMarginRange(metrics)
        f2 = file(dcgmProfTesterCsvFile, 'r')

        c1 = csv.reader(f2)

        prof_list = list(c1)
        dcgm_init_val = float(prof_list[dcgm_init_index][1])
        print '\lwalidating the accuracy of gpu' + str(gpu_index)
        ...
        try:
            ...
            print "\nIndex or ZeroDiv Exception oclwred..Ignoring: "
            i = i+1
            dcgm_init_index = dcgm_init_index + 1
    ...
    def validateAclwracy(self, dcgmCsvFile, dcgmProfTesterCsvFile, gpu_index, metrics):
        ret = 0
        if metrics in self.metrics_util_list:
            ret = self.validateAclwracyForUtilForUtil(dcgmCsvFile, dcgmProfTesterCsvFile, \
                    gpu_index, metrics)
        elif metrics in self.metrics_range:
            ret = self.validateAclwracyForRanges(dcgmCsvFile, gpu_index, metrics)
        else:
    ...
    ##############################################################################################
    #
    # This function gets the output of lwpu-smi for the calling function to get the memory
    # information
    #
    ##############################################################################################
    def getSmiOp(self):
        out = subprocess.Popen(['lwpu-smi'], \
    ...
    ##############################################################################################
    #
    # This function gets memory information out of lwpu-smi output
    #
    ##############################################################################################
...
def main(cmdArgs):
    ...
    cmd = 'python parse_dcgm_single_metric.py -f dcgmLogs_{0}.txt -m {1} -i {2}'.format(metrics, \
            metrics, gpuid_list)
    ...
    if metrics in ro.metrics_util_list:
        for i in range(0, len(gpu_list)):
            cmd = 'python parse_dcgmproftester_single_metric.py -f \
    ...
    for i in range(0, len(gpu_list)):
        ...
        ret = ro.validateAclwracy(dcgm_file, dcgmproftester_file, int(gpu_list[i]), metrics)
        if ret == 0:
