...
class RmCall:
    ...
    regexpRmCallSrc            = re.compile(".*dcgmRmCall.*(LW\d{4}_CTRL_CMD_[A-Z0-9_]*).*")
...
class LwmlCall:
    """ Class that matches Lwml Calls """
    # TODO doesn't handle apiEnter failures!
    regexpLwmlCall             = re.compile("^Entering (dcgm[a-zA-Z0-9_]*)(\(.*\)) *(\(.*\))$")
    regexpLwmlIntRelCall       = re.compile("^()()(\(.*\))$")
    regexpLwmlCallReturn       = re.compile("^Returning (\d*) \(([a-zA-Z ]*)\)$")
    regexpLwmlIntRelCallReturn = re.compile("^(\d*) ([a-zA-Z ]*)$")
    regexpFilePath             = re.compile(".*entry_points.h$")
    regexpFilePathNonTsapi     = re.compile(".*dcgm.c$")
    regexpLwmlCallSrc          = re.compile("^ *LWML_INT_ENTRY_POINT\((dcgm[A-Z][a-zA-Z0-9_]*) *,.*")
    
    @staticmethod
    def construct(debugLines, i):
        try:
            return LwmlCall(debugLines, i)
        except ValueError:
            return None
    ...
    def __init__(self, debugLines, i):
        ...
        verbose_debug("LwmlCall: Matching line %d %s" % (i, str(line1)))
        self.istsapi = 1
        if not LwmlCall.regexpFilePath.match(line1.srcfilename):
            self.istsapi = 0
            verbose_debug("LwmlCall: Wrong file name also matching non tsapi regexp")
            if not LwmlCall.regexpFilePathNonTsapi.match(line1.srcfilename):
                verbose_debug("LwmlCall: Wrong file also doesn't match non tsapi regexp")
                raise ValueError

        self.internal = 0
        self.dcgmCallName = 1
        match1 = LwmlCall.regexpLwmlCall.match(line1.message)
        if not match1:
            self.internal = 1
            self.dcgmCallName = 0
            verbose_debug("LwmlCall: Failed match regexpLwmlCall but need to also try regexpLwmlIntRelCall")
            match1 = LwmlCall.regexpLwmlIntRelCall.match(line1.message)
            if not match1:
                verbose_debug("LwmlCall: Failed match regexpLwmlIntRelCall")
                raise ValueError

        verbose_debug("LwmlCall: Matching the end of the dcgm call")
        for j in range(i + 1, len(debugLines)):
            ...
            match2 = LwmlCall.regexpLwmlIntRelCallReturn.match(line2.message) if self.internal else LwmlCall.regexpLwmlCallReturn.match(line2.message)
            if not match2:
                verbose_debug("LwmlCall: Sth went wrong. Found line2 \"%s\" that doesn't match the return but is in the same line" % (str(line2)))
                raise ValueError
                return
        
            verbose_debug("LwmlCall: Matched the end line %d %s" % (j, str(line2)))

            # TODO match device
            self.src = line1;
            self.srcEnd = line2;
            self.function = match1.group(1) if self.dcgmCallName else "LWML INT " + self.src.srcStr()
            ...
            self.times = [self.time] # so that stats from multiple runs could be appended
            return
        verbose_debug("LwmlCall: End of dcgm call wasn't found")
        raise ValueError
    ...
    def is_simillar(self, b):
        return isinstance(b, LwmlCall) and self.src.isInTheSamePlace(b.src) and self.function == b.function and self.errcode == b.errcode
...
class PerformanceStats(object):
    def __init__(self, input_fname):
        ...
        lines = filter(lambda x: x, map(lambda y: DebugLine.construct(y), rawlines))
        ...
        for i in range(len(lines)):
            ...
            dcgmCall = LwmlCall.construct(lines, i)
            if dcgmCall:
                lastLwmlCall = dcgmCall
                self.time_in_dcgm += dcgmCall.time
                self.stats.append(dcgmCall)
                continue
        
            rmCall = RmCall.construct(lines, i, lastLwmlCall)
            if rmCall:
    ...
    def write_to_file(self, fname, dcgm_stats=True, rm_stats=True):
        with open(fname, "w") as fout:
            ...
            for stat in self.stats:
                if not dcgm_stats and isinstance(stat, LwmlCall):
                    continue
            ...
            fout.write("%s\t%s (%.2f%% of total time)\n" % (_time_str(self.times_in_dcgm), "Time spent in LWML", average(self.times_in_dcgm) / average(self.times_total) * 100))
    ...
    def combine_stat(self, perf_stat):
        """
        Merges into self additional stats so that average and stdev for each entry could be callwlated.
        perf_stat must contain the same LWML/RM calls in the same order.
        ...
        if (len(perf_stat.stats) != len(self.stats)):
            log_perf_stat = ""
            log_self_stat = ""
            for i in xrange(len(perf_stat.stats)):
        ...
        # Some dcgm calls (e.g. dcgmDeviceGetLwrrentClocksThrottleReasons) can take different RM calls depending on the
        ...
        if (len(perf_stat.stats) != len(self.stats)):
            logger.warning("Perf logs mismatch. lwsmi perf data collected for %s run(s)" % str(self._combined_stats_count))
            return
