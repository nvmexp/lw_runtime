...
def get_dcgmi_bin_path():
    ...
    if platform.machine() == 'x86_64' and platform.system() == 'Linux':
        path = "apps/amd64/dcgmi"

    if platform.machine() == 'AMD64' and platform.system() == 'Windows':
        path = "apps/amd64/dcgmi.exe"

    if platform.machine() == 'aarch64' and platform.system() == 'Linux':
        path = "apps/aarch64/dcgmi"

    if platform.machine() == 'ppc64le' and platform.system() == 'Linux':
        path = "apps/ppc64le/dcgmi"
...
def get_lwrrent_field_group_id(hostIP):
    data = {}
    field_group_id = ""
    try:
        cmd = "%s fieldgroup --host %s -l" % (dcgmi_absolute_path, hostIP)
        output = check_output(cmd, shell=True)
        field_group_id = output.splitlines()[-5].split()[-2]  # Parser output to get the latest fieldgroup id
...
def setupElwironment():
    ...
    if utils.is_root():
        ...
        print("Unable to start persistence mode, root access is required")
        sys.exit(1)
...
class RunHostEngine(apps.LwHostEngineApp):
    ...
    def cpu_usage(self, timeout):
        ...
        timeout_start = time.time()
        while time.time() < timeout_start + float(timeout):
            cmd = check_output(shlex.split("ps -p %d -o %%cpu" % pid))
...
class BurnInHandle(object):
    ...
    def GpuSupportsEcc(self, gpuId):
        ...
        '''
        value = self.GetValueForFieldId(gpuId, dcgm_fields.DCGM_FI_DEV_ECC_LWRRENT)
...
class RunLwdaCtxCreate:
    def __init__(self, gpuIds, burnInHandle, runTimeSeconds, timeoutSeconds):
        ...
        for deviceAttrib in deviceAttribs:
            ...
            app = apps.LwdaCtxCreateAdvancedApp(args)
            app.busId = busId
            self._apps.append(app)

    def start(self):
        for app in self._apps:
            print "Generating Lwca Workload for GPU %s " % app.busId + " at %s \n" % time.asctime()
...
class ConfigTests:
    ...
    def test3_set_min_power_limit(self, gpuIds):
        ...
        if defPowerLimit == minPowerLimit:
            ...
            minPowerLimit = int(minPowerLimit) + 1  # +1 to address fractions lost in data type colwersion
    ...
    def test4_set_max_power_limit(self, gpuIds):
        ...
        maxPowerLimit = int(maxPowerLimit) - 1  # -1 to address fractions lost in data type colwersion
...
class DiagnosticsTests:
    ...
    def _set_diag_helper(self, gpuIds, flag):
        ...
        if not test_utils.is_lwvs_installed():
            return False #Don't try to run LWVS if it's not installed
...
class PolicyTests:
    ...
    def test2_get_lwrrent_policy_violation_by_groupId(self, gpuIds):
    ...
    def test3_set_pcierrors_policy_violation(self, gpuIds):
        ...
        validations = ["0","1","2","3"] # 0->None, 1-> LWVS(short), 2-> LWVS(medium), 3-> LWVS(long)
...
class ProcessStatsTests:
    ...
    def test3_get_pid_stats(self, gpuIds):
        ...
        app = RunLwdaCtxCreate(self.gpuIds, self.burnInHandle, runTimeSeconds=10, timeoutSeconds=(20 * len(gpuIds)))
        app.start()
...
# Run the LWLINK subsystem tests
class LwlinkTests:
    ...
    def test1_create_group(self, gpuIds):
        """ Creates a group for testing the subsystem lwlink """
        self.group_id = self.groups_op.create_group(None, gpuIds)
        return self.group_id > 1

    def test2_check_lwlink_status(self, gpuIds):
        """ Gets the lwlink error counts for various links and GPUs on the system  """

        args = ["lwlink", "--host", self.host_ip, "-g", str(self.group_id), "-s"]
        print "Reporting current lwlink status"
        return args

    def test3_query_lwlink_errors(self, gpuIds):
        for gpuId in gpuIds:
            args = ["lwlink", "--host", self.host_ip, "-g", str(gpuId), "-e"]
            print "Running queries to get errors for various lwlinks on the system: %s" % args
        return args

    def test4_query_lwlink_errors_json_output(self, gpuIds):
        for gpuId in gpuIds:
            args = ["lwlink", "--host", self.host_ip, "-g", str(gpuId), "-e", "-j"]
            print "Running queries to get errors for various lwlinks on the system in Json format: %s" % args
        return args
...
class FieldGroupsTests():
    ...
    def test5_get_fieldgroup_info(self, gpuIds):
        fieldGroupId = get_lwrrent_field_group_id(self.host_ip)
...
class RunDcgmi():
    ...
    def __init__(self, burnInHandle):
        ...
        self.lwlink_tests = LwlinkTests(burnInHandle)
    ...
    def _get_sorted_tests(self, obj):
        ...
        """
        return sorted(filter(lambda x:x[0].startswith("test"), inspect.getmembers(obj)), key=lambda x:int(x[0][4:x[0].find('_')]))
    ...
    def get_lwlink_tests(self):
        return self._get_sorted_tests(self.lwlink_tests)
    ...
    def start(self, timeout=None, server=None):
        ...
        # checks if the file has exelwtable permission
        if os.path.exists(self.dcgmi_path):
            assert os.access(self.dcgmi_path, os.X_OK), "Application binary %s is not exelwtable! Make sure that the testing archive has been correctly extracted." % (self.dcgmi_path)
                ...
                for test in all_tests:
                    for (testName, testMethod) in test:
                        ...
                        if diagTest:
                            ...
                            nsc.start()
                            dd = DcgmiDiag.DcgmiDiag(runMode=diagTest)
                        ...
                        if rc == 0:
                            ...
                            updateTestResults("PASSED")
                            updateTestResults("COUNT")
                        elif rc == ctypes.c_uint8(dcgm_structs.DCGM_ST_NOT_SUPPORTED).value:
                            print bcolors.YELLOW + "&&&& WAIVED" + bcolors.ENDC
...
def run_local_host_engine(burnInCfg):
    ...
    try:
        thread.start_new_thread(lambda: host_engine.mem_usage(burnInCfg.runtime), ())
        thread.start_new_thread(lambda: host_engine.cpu_usage(burnInCfg.runtime), ())
    except:
...
# Start lwca workload
def run_lwda_workload(burnInCfg):
    burnInHandle = BurnInHandle(burnInCfg.ip, burnInCfg)

    #Creates lwca workload
    lwda_workload = RunLwdaCtxCreate(None, burnInHandle, int(burnInCfg.runtime)+4, timeoutSeconds=10)
    lwda_workload.start()
    print "\nGenerating Lwca Workload on Clients...\n"
    lwda_workload.wait()
...
def run_remote(runtime, address, srv, nodes):
    ...
    py_cmd = "python burn_in_stress.py"

    # Run the tests on remote systems
    cmd="ssh %s@%s \"MODULEPATH=%s %s;cd testing; LD_LIBRARY_PATH=~/testing %s -t %s -s %s \"" % (user, address, os.elwiron["MODULEPATH"], module, py_cmd, runtime, srv)
...
def run_tests(burnInCfg):
    ...
    if not test_utils.is_hostengine_running():
        ...
        print "\nHostengine detected, using existing lw-hostegine..."
...
class BurnInGlobalConfig:
    def __init__(self):
        ...
        #Undolwmented globals I'm pulling out of the global namespace for sanity's sake
        self.srv = None
        self.ip = None
        self.nodes = []
        self.server = []
...
def parseCommandLine():
    ...
    parser.add_argument("-d", "--debug", action="store_true", help="Write debug logs from lw-hostengine")
...
def main_wrapped():
    ...
    try:
        ...
        print >>sys.stderr, "DCGM Library hasn't been found in the system, is the driver correctly installed?"
        sys.exit(1)

    if not burnInCfg.remote:
        run_tests(burnInCfg)
    elif len(burnInCfg.ip)==0:
        run_lwda_workload(burnInCfg)
