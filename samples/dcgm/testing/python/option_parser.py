...
def parse_options():
    ...
    parser.add_option(
            ...
            dest="fm_testing",
            action="store",
            help="""Sets option for running the FM tests as shown:
                [include] - default. Run FM tests with framework
                [exclude] - no FM tests. Only run DCGM core tests
                [only] - Only run FM tests"""
            )
    parser.add_option(
            "--eris",
            dest="eris",
    ...
    parser.add_option(
            ...
            help="Run the test framework in developer mode. This mode runs additional tests "+
                 ...
                 "or heavy reliance on elwironmental conditions."
            )
    parser.add_option(
            "--no-lint",
            dest="lint",
            action="store_false",
            default=True, 
            help="Do not lint python files before running tests to make sure that they " +
                 "do not contain any errors./n" +
                 "WARNING: you must have a good reason for doing this and developers " +
                 "SHOULD NEVER disable linting before they submit new code."
            )
    parser.add_option(
    ...
    parser.add_option(
            "--profile",
            dest="profile",
            choices=apps.lw_hostengine_app.LwHostEngineApp.supported_profile_tools,
            help="Can be one of: %s " % apps.lw_hostengine_app.LwHostEngineApp.supported_profile_tools +
                 "Turns on profiling of lw-hostengine while tests are running.  " +
                 "This only works for tests that run a standalone hostengine (not embedded).  " +
                 "Valgrind must also be installed.  The selected tool will profile lw-hostengine and " +
    ...
    test_group.add_option(
            "-d", "--device",
            dest="device",
            help="Run only on target device (DEVICE_LWML_ID) + global tests"
            )
    test_group.add_option(
            "-f", "--filter-tests",
            dest="filter_tests",
...
def validate():
    ...
    if options.fm_testing is None:
        options.fm_testing = test_utils.DCGM_FM_TEST_STATE_INCLUDE
    elif options.fm_testing == "include":
        options.fm_testing = test_utils.DCGM_FM_TEST_STATE_INCLUDE
    elif options.fm_testing == "exclude":
        options.fm_testing = test_utils.DCGM_FM_TEST_STATE_EXCLUDE
    elif options.fm_testing == "only":
        options.fm_testing = test_utils.DCGM_FM_TEST_STATE_ONLY
    else:
        logger.fatal("Invalid option detected for --fm-testing. Please one of the following: [include | excluce | only]")
