...
class InjectionThread(threading.Thread):
    ...
    def run(self):
        ...
        while i < self._iterations or not self.ShouldStop():
            self.retCode = inject_value(self._handle, self._gpuId, self._fieldId, self._value, self._offset,
                                         self._isInt, False)
...
## Injection helpers
def inject_value(handle, gpuId, fieldId, value, offset, isInt=True, verifyInsertion=True):
    if isInt:
...
# Verifies that the LWVS process is running. This is used to make tests more deterministic instead of sleeping
# and hoping that the LWVS process has started at the end of the sleep
def check_lwvs_process(want_running, delay=0.5, attempts=20):
    """
    Checks status of lwvs process.
    If want_running is True, method returns True if lwvs is running.
    If want_running is False, method returns True if lwvs is NOT running.
    """
    retry_count = 0
    debug_output = ''
    while retry_count < attempts:
        ...
        try:
            ...
            debug_output = subprocess.check_output(["pgrep", "-l", "-f", "apps/lwvs/lwvs"])
            if want_running:
