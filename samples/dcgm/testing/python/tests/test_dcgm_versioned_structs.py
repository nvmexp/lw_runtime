...
def vtDcgmGroupGetInfo(dcgm_handle, group_id, versionTest):
    ...
    device_values = dcgm_structs.c_dcgmGroupInfo_v1()
    device_values.version = dcgm_structs.make_dcgm_version(device_values, 1)
    logger.debug("Structure version: %d" % device_values.version)
...
def test_dcgm_config_get_validate(handle):
    ...
    with test_utils.assert_raises(dcgmExceptionClass(dcgm_structs.DCGM_ST_VER_MISMATCH)):
        versionTest = 0 #invalid version
        ret = vtDcgmConfigGet(handle, groupId, dcgm_structs.DCGM_CONFIG_LWRRENT_STATE, groupInfo.count, status_handle, versionTest)
...
def vtDcgmHealthCheck(dcgm_handle, groupId, versionTest):
    c_results = dcgm_structs.c_dcgmHealthResponse_v1()
    c_results.version = dcgm_structs.make_dcgm_version(c_results, 1)
...
def vtDcgmActiolwalidate_v2(dcgm_handle, runDiagInfo, versionTest):
    response = dcgm_structs.c_dcgmDiagResponse_v3()
    response.version = dcgm_structs.make_dcgm_version(response, 3)
    logger.debug("Structure version: %d" % response.version)

    runDiagInfo = dcgm_structs.c_dcgmRunDiag_v1()
    runDiagInfo.version = dcgm_structs.make_dcgm_version(runDiagInfo, 1)
    logger.debug("Structure version: %d" % runDiagInfo.version)

    runDiagInfo.version = versionTest
    response.version = versionTest
    fn = dcgmFP("dcgmActiolwalidate_v2")
    ret = fn(dcgm_handle, byref(runDiagInfo), byref(response))
    dcgm_structs._dcgmCheckReturn(ret)
    return response

def vtDcgmActiolwalidate(dcgm_handle, group_id, validate, versionTest):
    response = dcgm_structs.c_dcgmDiagResponse_v3()
    response.version = versionTest
...
def test_dcgm_run_diagnostic_validate(handle, gpuIds):
    ...
    with test_utils.assert_raises(dcgmExceptionClass(dcgm_structs.DCGM_ST_VER_MISMATCH)):
        versionTest = 0 #invalid version
        ret = vtDcgmActiolwalidate_v2(handle, drd, versionTest)
    ...
    with test_utils.assert_raises(dcgmExceptionClass(dcgm_structs.DCGM_ST_VER_MISMATCH)):
        versionTest = 0 #invalid version
        ret = vtDcgmActiolwalidate(handle, drd.groupId, drd.validate, versionTest)
...
def vtDcgmGetPidInfo(dcgm_handle, groupId, pid, versionTest):
    ...
    pidInfo = dcgm_structs.c_dcgmPidInfo_v1()
    pidInfo.version = dcgm_structs.make_dcgm_version(dcgm_structs.c_dcgmPidInfo_v1, 1)
...
def StartAppOnGpus(handle):
    ...
    for gpuId in allGpuIds:
        ...
        busIdList.append(gpuAttrib.identifiers.pciBusId)
...
def vtDcgmJobGetStats(dcgm_handle, jobid, versionTest):
    ...
    jobInfo = dcgm_structs.c_dcgmJobInfo_v2()
    jobInfo.version = dcgm_structs.make_dcgm_version(jobInfo, 2)
...
def vtDcgmIntrospectGetFieldsExecTime(dcgm_handle, introspectContext, versionTest, waitIfNoData=True):
    ...
    fullExecTime = dcgm_structs.c_dcgmIntrospectFullFieldsExecTime_v1()
    fullExecTime.version = dcgm_structs.make_dcgm_version(fullExecTime, 1)
...
def vtDcgmGetVgpuDeviceAttributes(dcgm_handle, gpuId, versionTest):
    fn = dcgm_agent_internal.etblDcgmEngineInternal().get_function_pointer_by_name("dcgmGetVgpuDeviceAttributes")
...
def vtDcgmGetVgpuInstanceAttributes(dcgm_handle, vgpuId, versionTest):
    fn = dcgm_agent_internal.etblDcgmEngineInternal().get_function_pointer_by_name("dcgmGetVgpuInstanceAttributes")
...
def vtDcgmVgpuConfigSet(dcgm_handle, group_id, configToSet, status_handle, versionTest):
    fn = dcgm_agent_internal.etblDcgmEngineInternal().get_function_pointer_by_name("dcgmVgpuConfigSet")
    configToSet.version = versionTest
    ret = fn(dcgm_handle, group_id, byref(configToSet), status_handle)
    dcgm_structs._dcgmCheckReturn(ret)
    return ret
...
def vtDcgmVgpuConfigGet(dcgm_handle, group_id, reqCfgType, count, status_handle, versionTest):
    fn = dcgm_agent_internal.etblDcgmEngineInternal().get_function_pointer_by_name("dcgmVgpuConfigGet")
...
def test_dcgm_vgpu_config_get_validate(handle):
    ...
    with test_utils.assert_raises(dcgmExceptionClass(dcgm_structs.DCGM_ST_VER_MISMATCH)):
        versionTest = 0 #invalid version
        ret = vtDcgmVgpuConfigGet(handle, groupId, dcgm_structs.DCGM_CONFIG_LWRRENT_STATE, groupInfo.count, status_handle, versionTest)
...
def vtDcgmIntrospectGetFieldExecTime(dcgm_handle, fieldId, versionTest, waitIfNoData=True):
    fn = dcgm_agent_internal.etblDcgmEngineInternal().get_function_pointer_by_name("dcgmIntrospectGetFieldExecTime")
...
def vtDcgmIntrospectGetFieldMemoryUsage(dcgm_handle, fieldId, versionTest, waitIfNoData=True):
    fn = dcgm_agent_internal.etblDcgmEngineInternal().get_function_pointer_by_name("dcgmIntrospectGetFieldMemoryUsage")
