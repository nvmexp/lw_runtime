...
@test_utils.run_with_initialized_client()
@test_utils.run_only_with_all_supported_gpus()
def test_dcgm_topology_device_lwlink_standalone(handle, gpuIds):
    """
    Verifies that the topology get for the default group returns valid LWLINK info
    """
    handleObj = pydcgm.DcgmHandle(handle=handle)
    systemObj = handleObj.GetSystem()
    groupObj = systemObj.GetDefaultGroup()
    ...
    if topologyInfo.gpuPaths[0].localLwLinkIds == 0:
        test_utils.skip_test("Needs LWLINK support")

    assert ((topologyInfo.gpuPaths[0].path & 0xFFFFFF00) > 0), "No LWLINK state set when localLwLinkIds is > 0"
