...
def helper_dcgm_config_get_attributes(handle):
    ...
    for gpuId in gpuIdList:
        ...
        assert attributes.clockSets.count > 0, "Expected clock sets. Got 0 for gpuId %d" % gpuId
...
def helper_dcgm_config_powerbudget(handle, gpuIds):
    ...
    config_values = groupObj.config.Get(dcgm_structs.DCGM_CONFIG_LWRRENT_STATE)
...
def helper_test_config_config_power_enforce(handle, gpuIds):
    ...
    powerLimit_set_lwsmi = attributes.powerLimits.maxPowerLimit
    ...
    ## Change Power limit to max from external entity like lwpu-smi
    assert 0 == apps.LwidiaSmiApp(["-pl", str(powerLimit_set_lwsmi), "-i", str(gpuIds[0])]).run(), \
        "Lwpu smi couldn't set the power limit"

    logger.info("Verify if lwsmi configured value has taken effect")
    helper_verify_power_value(groupObj, powerLimit_set_lwsmi)
...
def test_dcgm_configure_ecc_mode(handle, gpuIds):
    ...
    validDevice = -1
    for x in gpuIds:
        fvSupported = dcgm_agent_internal.dcgmGetLatestValuesForFields(handle, x, [dcgm_fields.DCGM_FI_DEV_ECC_LWRRENT, ])
    ...
    config_values = dcgm_agent.dcgmConfigGet(handle, groupId, dcgm_structs.DCGM_CONFIG_LWRRENT_STATE, groupInfo.count, status_handle)
    ...
    fvs = dcgm_agent_internal.dcgmGetLatestValuesForFields(handle, validDevice, [dcgm_fields.DCGM_FI_DEV_ECC_PENDING, dcgm_fields.DCGM_FI_DEV_ECC_LWRRENT])
