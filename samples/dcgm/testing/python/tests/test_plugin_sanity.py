...
The main purpose of this module is to provide a very basic sanity test for large changes made to lwvs plugins.

Note: When DCGM and LWVS have been updated to use error codes, these tests can be updated to perform automatic
...
from apps.lw_hostengine_app import LwHostEngineApp
from dcgm_internal_helpers import InjectionThread, check_lwvs_process
...
def copy_lwvs_log(lwvsLogFile, outputLogFile):
    """
    Copy lwvs log file to the output dir. This method is needed because of path length limitations when using DCGM
    to run LWVS.
    """
    try:
        if os.path.exists(lwvsLogFile):
            shutil.copyfile(lwvsLogFile, outputLogFile)
    except IOError as e:
        logger.error("Could not copy lwvs log to output dir.")
        logger.error(e)
...
def no_errors_run(handle, gpuIds, name, testname, parms=None):
    ...
    name is the name of the plugin in lwvs (e.g. constant_perf)
    """
    output_file = OUTPUT_DIR + "/dcgmi_%s_no_err_%s.json" % (name, gpuIds[0])
    log_file = OUTPUT_DIR + "/lwvs_%s_no_err_%s.log" % (name, gpuIds[0])
    gpu_list = ",".join(map(str, gpuIds))

    # Note: Although using the dcgmActiolwalidate api (via DcgmDiag.Execute()) would allow for some automatic
    # verification, we use dcgmi diag and log output to a file for easier debugging when something goes wrong.
    args = ["diag", "-r", "%s" % testname, "-i", gpu_list, "-j", "-v", "-d", "5", "--debugLogFile", "/tmp/lwvs.log"]
    if parms != None:
    ...
    retcode = dcgmi.wait()
    copy_lwvs_log("/tmp/lwvs.log", log_file)
    if retcode != 0:
...
def with_error_run(handle, gpuIds, name, testname, parms=None):
    ...
    -30/DCGM_ST_LWVS_ERROR) which is expected since the test should fail due to inserted errors.
    ...
    systems (lwrrently a delay of 15 seconds works for the bstolle-dgx machine).
    """
    output_file = OUTPUT_DIR + "/dcgmi_%s_with_err_%s.json" % (name, gpuIds[0])
    log_file = OUTPUT_DIR + "/lwvs_%s_with_err_%s.log" % (name, gpuIds[0])
    gpu_list = ",".join(map(str, gpuIds))
    
    args = ["diag", "-r", "%s" % testname, "-i", gpu_list, "-j", "-v", "-d", "5", "--debugLogFile", "/tmp/lwvs.log",
            "--throttle-mask", "0"] # don't ignore throttle errors
    if parms != None:
        args.extend(["-p", "%s" % parms])
    dcgmi = DcgmiApp(args=args)
    
    field_id = dcgm_fields.DCGM_FI_DEV_CLOCK_THROTTLE_REASONS
    value = 8
    delay = 0
    if name == "busgrind":
    ...
    if delay != 0:
        running, _ = dcgm_internal_helpers.check_lwvs_process(want_running=True)
        assert running, "lwvs did not start"
        logger.info("Lwvs started after %.1fs" % (time.time() - start))
        time.sleep(delay)
        logger.info("Injecting errors now (field %s, value %s)" % (field_id, value))
        inject_error.start()
    ...
    copy_lwvs_log("/tmp/lwvs.log", log_file)
    expected_retcode = ctypes.c_uint8(dcgm_structs.DCGM_ST_LWVS_ERROR).value
