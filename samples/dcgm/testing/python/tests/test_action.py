...
def helper_validate_action(groupObj):
    ...
    for validation in validations:
        ...
        assert response.version == dcgm_structs.dcgmDiagResponse_version4, "Version mismatch. Expected %d. got %d" % \
                                                                           (dcgm_structs.dcgmDiagResponse_version4, response.version)
...
def helper_dcgm_action_run_diag_gpu_list(handle, gpuIds):
    ...
    ret = dcgm_agent.dcgmActiolwalidate_v2(handle, drd, dcgm_structs.dcgmRunDiag_version)

    drd = dcgm_structs.c_dcgmRunDiag_v1()
    drd.version = dcgm_structs.dcgmRunDiag_version1
    drd.validate = dcgm_structs.DCGM_POLICY_VALID_SV_SHORT
    drd.groupId = 0 #Initializing to 0 in case the constructor above doesn't
    drd.gpuList = gpuIdStr
    #this will throw an exception on error
    ret = dcgm_agent.dcgmActiolwalidate_v2(handle, drd, dcgm_structs.dcgmRunDiag_version1)
