...
def helper_inject_vgpu_configuration(handle, gpuId, eccModeVal, powerLimitVal, computeModeVal):
    ...
    if (eccModeVal != None):
        ...
        eccMode.fieldId = dcgm_fields.DCGM_FI_DEV_ECC_LWRRENT
        eccMode.status = 0
...
def helper_dcgm_vgpu_config_injection(handle):
    ...
    ## Get the target vGPU configuration to make sure that it's exact same as the one configured
    vgpu_config_values = groupObj.vgpu.Get(dcgm_structs.DCGM_CONFIG_LWRRENT_STATE)
...
def test_dcgm_vgpu_config_power_enforce_standalone(handle, gpuIds):
    ...
    helper_verify_power_value_standalone(groupObj, powerLimit_set_lwsmi)
...
def test_dcgm_vgpu_configure_ecc_mode(handle, gpuIds):
    ...
    vgpu_config_values = dcgm_agent_internal.dcgmVgpuConfigGet(handle, groupId, dcgm_structs.DCGM_CONFIG_LWRRENT_STATE, groupInfo.count, status_handle)
    assert len(vgpu_config_values) > 0, "Failed to work with NULL status handle"
    ...
    config_values = dcgm_agent_internal.dcgmVgpuConfigGet(handle, groupId, dcgm_structs.DCGM_CONFIG_LWRRENT_STATE, groupInfo.count, status_handle)
