...
g_profilingFieldIds = [
    ...
    dcgm_fields.DCGM_FI_PROF_SM_OCLWPANCY,
    ...
    dcgm_fields.DCGM_FI_PROF_LWLINK_TX_BYTES,
    dcgm_fields.DCGM_FI_PROF_LWLINK_RX_BYTES
]
...
def helper_validate_entities_lastest_values_request(handle, gpuIds, fieldIds):
    ...
    for i, fieldValue in enumerate(fieldValues):
        ...
        assert responses.has_key(dictKey) and responses[dictKey] == 0, "Mismatch on dictKey %s. Responses: %s" % (dictKey, str(responses))
        assert(fieldValue.status == dcgm_structs.DCGM_ST_OK), "idx %d status was %d" % (i, fieldValue.status)
        assert(fieldValue.ts != 0), "idx %d timestamp was 0" % i
...
def helper_dcgm_entities_get_latest_values(handle, gpuIds):
    ...
    #have LWML mappings and those that don't in order to try and cause failures
...
def test_dcgm_live_accounting_data(handle, gpuIds):
    ...
    if test_utils.is_lwswitch_detected():
        test_utils.skip_test("Skipping GPU Lwca tests on LwSwitch systems since they require the FM to be loaded")
    ...
    #Start a lwca app so we have something to accounted
    appTimeout = 1000
    appParams = ["--ctxCreate", busId,
                 "--busyGpu", busId, str(appTimeout),
                 "--ctxDestroy", busId]
    app = apps.LwdaCtxCreateAdvancedApp(appParams)
...
def helper_dcgm_values_pid_stats(handle, gpuIds):
    ...
    appTimeout = 5000
    app = _create_lwda_app_for_pid_stats(busId, appTimeout)
    app2 = _create_lwda_app_for_pid_stats(busId, appTimeout)
    app3 = _create_lwda_assert_app_for_pid_stats(busId, appTimeout)
    ...
    _assert_pid_lwda_assert_oclwrence(systemObj, groupObj, app.getpid())

def _assert_pid_lwda_assert_oclwrence(dcgmSystem, dcgmGroup, appPid):
    ''' Force an update and verifies that a xid error oclwrred during the life of the process '''
...
def _create_lwda_app_for_pid_stats(busId, appTimeout):
    app = apps.LwdaCtxCreateAdvancedApp(["--ctxCreate", busId,
                                         "--busyGpu", busId, str(appTimeout),
                                         "--ctxDestroy", busId])
    app.start(appTimeout*2)
    return app

def _create_lwda_assert_app_for_pid_stats(busId, appTimeout):
    app = apps.RunLwdaAssert(["--ctxCreate", busId,
                              "--lwMemAlloc", busId, "200",
                              "--lwMemFree", busId,
...
def helper_dcgm_values_pid_stats_realtime(handle, gpuIds):
    ...
    app = _create_lwda_app_for_pid_stats(busId, appTimeout)
    appPid = app.getpid()
    app.stdout_readtillmatch(lambda s: s.find("Calling lwInit") != -1)
    
    #Start a 2nd app that will appear in the compute app list
    app2 = _create_lwda_app_for_pid_stats(busId, appTimeout)
    app2Pid = app2.getpid()
    app2.stdout_readtillmatch(lambda s: s.find("Calling lwInit") != -1)
...
def test_dcgm_values_job_stats_get(handle, gpuIds):
    ...
    #Start a few lwca apps so we have something to accounted
    appParams = ["--ctxCreate", busId,
                "--busyGpu", busId, str(appTimeout),
                "--ctxDestroy", busId]
    ...
    for i in range (0,3):
        ...
        app_xid = apps.RunLwdaAssert(xidArgs)
    ...
    assert jobInfo.summary.numComputePids >= 3, "Not all LWCA process ran captured during job. Expected 3, got %d" %jobInfo.summary.numComputePids
    for i in range (0,2):
        pidFound = 0
        for j in range (0,dcgm_structs.DCGM_MAX_PID_INFO_NUM):
            if appPid[i] == jobInfo.summary.computePids[j].pid:
                pidFound = 1
                break
        assert pidFound, "LWCA Process PID not captured during job. Missing: %d" % appPid[i]
...
def test_dcgm_fields_all_fieldids_valid(handle, gpuIds):
    ...
    exceptionFieldIds = [dcgm_fields.DCGM_FI_DEV_COMPUTE_PIDS,
                         ...
                         dcgm_fields.DCGM_FI_DEV_VGPU_LICENSE_STATUS,
                         ...
                         dcgm_fields.DCGM_FI_DEV_GPU_LWLINK_ERRORS]
    ...
    for moduleAttribute in dcgm_fields.__dict__.keys():
    ...
    for fieldIdName in fieldIdVars.keys():
    ...
    #fieldIdVars = {'DCGM_FI_GPU_TOPOLOGY_LWLINK' : 61}
    ...
    for fieldIdName in fieldIdVars.keys():
        ...
        #Skip LwSwitch fields since they are pushed from fabric manager rather than polled
        if fieldId >= dcgm_fields.DCGM_FI_DEV_LWSWITCH_LATENCY_LOW_P00 and fieldId <= dcgm_fields.DCGM_FI_DEV_LWSWITCH_NON_FATAL_ERRORS:
            continue
...
@test_utils.run_with_embedded_host_engine()
@test_utils.run_only_with_live_gpus()
def test_dcgm_lwlink_bandwidth(handle, gpuIds):
    handleObj = pydcgm.DcgmHandle(handle=handle)
    systemObj = handleObj.GetSystem()
    groupObj = systemObj.GetDefaultGroup()
    gpuId = gpuIds[0]

    fieldIds = [ dcgm_fields.DCGM_FI_DEV_LWLINK_BANDWIDTH_L0,
                 dcgm_fields.DCGM_FI_DEV_LWLINK_BANDWIDTH_L1,
                 dcgm_fields.DCGM_FI_DEV_LWLINK_BANDWIDTH_L2,
                 dcgm_fields.DCGM_FI_DEV_LWLINK_BANDWIDTH_L3 ]
    ...
    assert len(fieldWatcher.values[gpuId]) == 4, "Expected 4 LWlink bandwidth values, got %d" % len(fieldWatcher.values[gpuId])

    for fieldId in fieldIds:
        for value in fieldWatcher.values[gpuId][fieldId]:
            # Either the GPU supports LwLink, in which case a
            # non-blank, actual value should be read, or the GPU does
            # not support LwLink, in which case the field should be
            # blank and the value should reflect that it's not
            # supported.
            assert ((value.isBlank == True and value.value == dcgmvalue.DCGM_INT64_NOT_SUPPORTED) or (value.isBlank == False and value.value >= 0)), "Unexpected error reading field %d on GPU %d" % (fieldId, gpuId)
