...
def helper_dcgm_policy_inject_eccerror(handle, gpuIds):
    ...
    # check that the callback oclwrred with the correct arguments
...
@test_utils.run_with_initialized_client()
@test_utils.run_with_injection_gpus()
def test_dcgm_policy_inject_lwlinkerror_standalone(handle, gpuIds):
    """ 
    Verifies that we can inject an error into the LWLINK error and receive a callback
    """
    newPolicy = dcgm_structs.c_dcgmPolicy_v1()
    
    newPolicy.version = dcgm_structs.dcgmPolicy_version1
    newPolicy.condition = dcgm_structs.DCGM_POLICY_COND_LWLINK
    newPolicy.parms[5].tag = 0
    newPolicy.parms[5].val.boolean = True

    # find a GPU that supports lwlink (otherwise internal test will ignore it)
    dcgmHandle = pydcgm.DcgmHandle(handle)
    dcgmSystem = dcgmHandle.GetSystem()
    group = dcgmSystem.GetGroupWithGpuIds('test1', gpuIds)
    group.policy.Set(newPolicy)
    ...
    callbackQueue = Queue.Queue()
    c_callback = create_c_callback(callbackQueue)
    group.policy.Register(dcgm_structs.DCGM_POLICY_COND_LWLINK, finishCallback=c_callback)

    field = dcgm_structs_internal.c_dcgmInjectFieldValue_v1()
    field.version = dcgm_structs_internal.dcgmInjectFieldValue_version1
    field.fieldId = dcgm_fields.DCGM_FI_DEV_LWLINK_CRC_FLIT_ERROR_COUNT_TOTAL
    field.status = 0
    field.fieldType = ord(dcgm_fields.DCGM_FT_INT64)
    field.ts = int((time.time()+60) * 1000000.0) # set the injected data into the future
    field.value.i64 = 1
    ...
    # check that the callback oclwrred with the correct arguments
    assert(dcgm_structs.DCGM_POLICY_COND_LWLINK == callbackData.condition), \
            ("LWLINK error callback was not for a LWLINK error, got: %s" % callbackData.condition)
    assert(dcgm_fields.DCGM_FI_DEV_LWLINK_CRC_FLIT_ERROR_COUNT_TOTAL == callbackData.val.lwlink.fieldId), \
            ("Expected 130 fieldId but got %s" % callbackData.val.lwlink.fieldId)
    assert(1 == callbackData.val.lwlink.counter), 'Expected 1 PCI error but got %s' % callbackData.val.lwlink.counter
