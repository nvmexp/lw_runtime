...
def test_dcgm_standalone_connection_disconnect_error_after_hostengine_terminate(handle, hostengineApp):
    ...
    the hostengine process is terminated via `lw-hostengine --term`.
    '''
...
@test_utils.run_with_standalone_host_engine(passAppAsArg=True, checkAndEnableFM=False)
@test_utils.run_with_initialized_client()
...
domainSocketFilename = '/tmp/dcgm_test'
...
def test_dcgm_connection_domain_socket():
    ...
    v2Handle = dcgm_agent.dcgmConnect_v2(domainSocketFilename, v2Struct, dcgm_structs.c_dcgmConnectV2Params_version2)
    #Use the handle, which will throw an exception on error
    gpuIds2 = dcgm_agent.dcgmGetAllSupportedDevices(v2Handle)
    dcgm_agent.dcgmDisconnect(v2Handle)
    
    #Now use the DcgmHandle method
    dcgmHandle = pydcgm.DcgmHandle(unixSocketPath=domainSocketFilename)
