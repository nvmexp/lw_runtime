...
def _plotFinalValueOrderedBarChart(points, title, ylabel, filenameBase, topValCount=20):
    ...
    x = range(0, len(topSortedPts)*width, width)
...
def _generate_metadata_line_charts(metadataTSeries, ylabel, title):
    ...
    if metadataTSeries.fieldVals:
        _plot_metadata(x=metadataTSeries.timestamps, 
                       yLists=metadataTSeries.fieldVals.values(), 
    ...
    if metadataTSeries.fieldGroupVals:
        _plot_metadata(x=metadataTSeries.timestamps, 
                       yLists=metadataTSeries.fieldGroupVals.values(),
...
def test_dcgm_standalone_perf_bounded(handle):
    ...
    Plots of the memory usage and exelwtion time generated during this test are saved and the 
    ...
    # test that exelwtion time grows at a linear rate
    ...
    _generate_metadata_line_charts(execTimeTS, ylabel='usec', title='Exelwtion Time')
...
def _test_exectime_bounded_linear_growth(execTimeTS):
    ...
    for fieldId, series in execTimeTS.fieldVals.iteritems():
        tailStart = int(0.4*len(series))
        tailLen = len(series) - tailStart
        
        # take a linear regression of the exelwtion timeseries 
        ...
        assert(rxy >= 0.90), (
            'exelwtion time for field %s did not have a strong linear correlation. ' % fieldId + 
        ...
        assert(minSlope <= slope <= maxSlope), (
            'exelwtion time growth for field %s was not linear. ' % fieldId +
...
def _test_mem_bounded_flatlines_fields(memUsageTS):
    for id, series in memUsageTS.fieldVals.iteritems():
        _assert_flatlines('field', id, series)
        
def _test_mem_bounded_flatlines_fieldgroups(memUsageTS):
    for id, series in memUsageTS.fieldGroupVals.iteritems():
...
def _test_mem_bounded_golden_values_fields(activeGpuCount, memUsageTS, tailStart):
    goldelwal = 1148      # 1 KB plus some swag per field instance (Global, GPU). This is based off of the keyed vector block size and default number of blocks
    tolerance = 0.10      # low tolerance, amount of records stored is bounded
    
    for fieldId, series in memUsageTS.fieldVals.iteritems():
        ...
        lowLimit = (1-tolerance)*goldelwal
        highLimit = (1+tolerance)*goldelwal*activeGpuCount
...
def _test_mem_bounded_golden_values_allfields(activeGpuCount, memUsageTS, tailStart, numFieldIds):
    goldelwal = 2000 * numFieldIds * activeGpuCount # 2 KiB per fieldId per GPU. This gives some swag for the binary fields that are larger
