...
try:
    ...
    if 'PYTHONPATH' in os.elwiron:
        os.elwiron['PYTHONPATH'] = os.elwiron['PYTHONPATH'] + ":/usr/local/dcgm/bindings"
    else:
        os.elwiron['PYTHONPATH'] = '/usr/local/dcgm/bindings'
...
def check_passive_health_checks(response, activeGpuIds):
    unhealthy = False
    if response.version < dcgm_structs.dcgmDiagResponse_version4:
        if response.blacklist == dcgm_structs.DCGM_DIAG_RESULT_FAIL:
            mark_all_unhealthy(activeGpuIds, response.systemError)
            unhealthy = True
        elif response.lwmlLibrary == dcgm_structs.DCGM_DIAG_RESULT_FAIL:
            mark_all_unhealthy(activeGpuIds, response.systemError)
            unhealthy = True
        elif response.lwdaMainLibrary == dcgm_structs.DCGM_DIAG_RESULT_FAIL:
            mark_all_unhealthy(activeGpuIds, response.systemError)
            unhealthy = True
        elif response.lwdaRuntimeLibrary == dcgm_structs.DCGM_DIAG_RESULT_FAIL:
            mark_all_unhealthy(activeGpuIds, response.systemError)
            unhealthy = True
        elif response.permissions == dcgm_structs.DCGM_DIAG_RESULT_FAIL:
            mark_all_unhealthy(activeGpuIds, response.systemError)
            unhealthy = True
        elif response.persistenceMode == dcgm_structs.DCGM_DIAG_RESULT_FAIL:
            mark_all_unhealthy(activeGpuIds, response.systemError)
            unhealthy = True
        elif response.environment == dcgm_structs.DCGM_DIAG_RESULT_FAIL:
            mark_all_unhealthy(activeGpuIds, response.systemError)
            unhealthy = True
        elif response.pageRetirement == dcgm_structs.DCGM_DIAG_RESULT_FAIL:
            mark_all_unhealthy(activeGpuIds, response.systemError)
            unhealthy = True
        elif response.graphicsProcesses == dcgm_structs.DCGM_DIAG_RESULT_FAIL:
            mark_all_unhealthy(activeGpuIds, response.systemError)
            unhealthy = True
        elif response.inforom == dcgm_structs.DCGM_DIAG_RESULT_FAIL:
            mark_all_unhealthy(activeGpuIds, response.systemError)
            unhealthy = True
    else:
...
def check_gpu_diagnostic(handleObj, settings):
    ...
    response = dcgm_agent.dcgmActiolwalidate_v2(handleObj.handle, runDiagInfo)
            ...
            for testIndex in range(dcgm_structs.DCGM_PER_GPU_TEST_COUNT):
                if response.perGpuResponses[gpuIndex].results[testIndex].result == dcgm_structs.DCGM_DIAG_RESULT_FAIL:
                    ...
                    mark_entity_unhealthy(g_gpus, gpuId, BR_ST_FAILED_ACTIVE_HEALTH,
                                              response.perGpuResponses[gpuIndex].results[testIndex].error.msg)

                    # LWVS marks all subsequent tests as failed so there's no point in continuing
                    break 
...
def blacklist_from_passive_health_check(response):
    for entityIndex in range(response.entityCount):
        if response.entities[entityIndex].overallHealth != dcgm_structs.DCGM_HEALTH_RESULT_FAIL:
            # Only blacklist for failures; ignore warnings
            continue

        entityResponse = response.entities[entityIndex]
        entityId = entityResponse.entityId
        for incidentIndex in range(entityResponse.incidentCount):
            incident = entityResponse.systems[incidentIndex]

            # Again, only record the failures
            if incident.health != dcgm_structs.DCGM_HEALTH_RESULT_FAIL:
                continue

            if entityResponse.entityGroupId == dcgm_fields.DCGM_FE_GPU:
                mark_entity_unhealthy(g_gpus, entityId, BR_ST_FAILED_PASSIVE_HEALTH, incident.errorString)
            else:
                mark_entity_unhealthy(g_switches, entityId, BR_ST_FAILED_PASSIVE_HEALTH, incident.errorString)
...
def check_passive_health(handleObj, watches):
    ...
    if response.overallHealth != dcgm_structs.DCGM_HEALTH_RESULT_PASS:
        blacklist_from_passive_health_check(response)
...
def __process_command_line__(settings):
    ...
    parser.add_argument('-s', '--num-switches', dest='num_switches', type=int,
                        help='The expected number of LwSwitches.')
    parser.add_argument('-n', '--hostname', dest='hostname', type=str,
                        help='The hostname of the lw-hostengine we want to query.')
    ...
    if args.watches:
        ...
        for c in args.watches:
            if c == 'p':
                ...
                health_watches |= dcgm_structs.DCGM_HEALTH_WATCH_LWLINK
            elif c == 'a':
        ...
        if 'outfmtCSV' in settings:       # show all health, then all un-healthy
            ...
            for gpuObj in g_gpus:
                if gpuObj.IsHealthy() == False:
                    ...
                    blacklistGpus[gpuObj.GetEntityId()] = details
            ...
            jsonTop['blacklistedGpus'] = blacklistGpus
