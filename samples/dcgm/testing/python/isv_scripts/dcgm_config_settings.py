...
def agent_worker_function(handle, groupId, groupInfo, status_handle):
    ...
    while True:
        ...
        helper_ilwestigate_status(status_handle)
        ...
        for x in xrange(0,groupInfo.count):
            ...
            print "Auto Boost  : %s" % (colwert_value_to_string(config_values[x].mPerfState.autoBoost))
            print "Sync Boost  : %s" % (colwert_value_to_string(config_values[x].mPerfState.autoBoost))
            print "Mem Clock   : %s" % (colwert_value_to_string(config_values[x].mPerfState.milwPState.memClk))
            print "SM  Clock   : %s" % (colwert_value_to_string(config_values[x].mPerfState.milwPState.procClk))
