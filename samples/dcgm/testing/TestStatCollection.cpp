...
/*****************************************************************************/
int TestStatCollection::Init(std::vector<std::string>argv, std::vector<test_lwcm_gpu_t>gpus)
{
    return 0;
}
...
int do_stat_collection_op(DcgmStatCollection *statCollection, int opIndex)
{
    ...
    switch(SRT_OP_CAT(opIndex))
    {
        ...
        case MC_TYPE_INT64:
        {
            ...
            {
                default:
                case SRT_CAT_GLOBAL: statCollection->SetGlobalStat(key, val); break;
                case SRT_CAT_GROUP: statCollection->SetGroupedStat(group, key, val); break;
                case SRT_CAT_GPU: statCollection->SetGpuStat(gpuIndex, key, val); break;
                case SRT_CAT_ENTITY: statCollection->SetEntityStat(SC_ENTITY_GROUP_GPU, gpuIndex, key, val); break;
            }
            break;
        }
        ...
        case MC_TYPE_TIMESERIES_DOUBLE:
        {
            ...
            {
                 default:
                 case SRT_CAT_GLOBAL: statCollection->AppendGlobalStat(key, val, timestamp); break;
                 case SRT_CAT_GROUP: statCollection->AppendGroupedStat(group, key, val, timestamp); break;
                 case SRT_CAT_GPU: statCollection->AppendGpuStat(gpuIndex, key, val, 0, timestamp); break;
                 case SRT_CAT_ENTITY: statCollection->AppendEntityStat(SC_ENTITY_GROUP_GPU, gpuIndex, key, val, 0.0, timestamp); break;
            }
            break;
        }
        ...
        case MC_TYPE_TIMESERIES_INT64:
        {
            ...
            switch(SRT_OP_MCT(opIndex))
            {
                ...
                case SRT_CAT_GROUP: statCollection->AppendGroupedStat(group, key, val, timestamp); break;
                case SRT_CAT_GPU: statCollection->AppendGpuStat(gpuIndex, key, val, 0, timestamp); break;
                case SRT_CAT_ENTITY: statCollection->AppendEntityStat(SC_ENTITY_GROUP_GPU, gpuIndex, key, val, 0, timestamp); break;
            }
            break;
        }
        ...
        case MC_TYPE_TIMESERIES_STRING:
        {
            ...
            switch(SRT_OP_MCT(opIndex))
            {
                ...
                case SRT_CAT_GLOBAL: statCollection->AppendGlobalStat(key, val, timestamp); break;
                case SRT_CAT_GROUP: statCollection->AppendGroupedStat(group, key, val, timestamp); break;
                case SRT_CAT_GPU: statCollection->AppendGpuStat(gpuIndex, key, val, timestamp); break;
                case SRT_CAT_ENTITY: statCollection->AppendEntityStat(SC_ENTITY_GROUP_GPU, gpuIndex, key, val, timestamp); break;
            }
        ...
        case MC_TYPE_TIMESERIES_BLOB:
        {
            ...
            {
                default:
                case SRT_CAT_GLOBAL: statCollection->AppendGlobalStat(key, &val, sizeof(val), timestamp); break;
                case SRT_CAT_GROUP: statCollection->AppendGroupedStat(group, key, &val, sizeof(val), timestamp); break;
                case SRT_CAT_GPU: statCollection->AppendGpuStat(gpuIndex, key, &val, sizeof(val), timestamp); break;
                case SRT_CAT_ENTITY: statCollection->AppendEntityStat(SC_ENTITY_GROUP_GPU, gpuIndex, key, &val, sizeof(val), timestamp); break;
            }
            break;
        }
    }
...
/*****************************************************************************/
static void get_temp_filename(char *filename, int maxFilename, int seed)
{
    snprintf(filename, maxFilename-1, "./tsr_test_file_%d.tmp", seed);
}
...
int TestStatCollection::TestPerformance(void)
{
    ...
    if(!statCollection)
    {
        fprintf(stderr, "Unable to allocate StatCollection");
        return 100;
    }
    ...
    for(i = 0; i < numGpus; i++)
    {
        for(j = 0; j < numFieldsPerGpu; j++)
        {
             ...
             */
            statCollection->AppendGpuStat((unsigned int)i, (std::string)statKey, value1,
                                          value2, nowUsec);
        }
    }
    ...
    printf("TestPerformance FirstInserted %d records at %.2f usec/record (IsDebug %d, loggingLevel %d)\n",
           (int)numInserted, usecPerInsert, IsDebugBuild(), loggingDebugLevel);
    ...
    printf("TestPerformance Appended %d records at %.2f usec/record (IsDebug %d, loggingLevel %d)\n",
           (int)numInserted, usecPerInsert, IsDebugBuild(), loggingDebugLevel);
    ...
    for(i = 0; i < numGpus; i++)
    {
        for(j = 0; j < numFieldsPerGpu; j++)
        {
            ...
            for(k = 0; k < numRecordsPerStat; k++)
            {
                ...
                if(!gpuStat)
                {
                    fprintf(stderr, "Unexpected not found: lwmlIndex %d, statKey %s\n", i, statKey);
                    return 200;
                }
            }
        }
    ...
    printf("TestPerformance Searched for %d records at %.2f usec/record (IsDebug %d, loggingLevel %d)\n",
           (int)numSearched, usecPerSearch, IsDebugBuild(), loggingDebugLevel);
