...
/*****************************************************************************/
int TestFieldGroups::Init(std::vector<std::string>argv, std::vector<test_lwcm_gpu_t>gpus)
{
    m_gpus = gpus;
    return 0;
}
...
int TestFieldGroups::TestFieldGroupObject(void)
{
    ...
    {
        std::cerr << "TestFieldGroupObject groupNameBefore " << groupNameBefore  <<
                  " != groupNameAfter " << groupNameAfter << std::endl;
        numErrors++;
    }
...
int TestFieldGroups::TestGetAll(void)
{
    ...
    for(i = 0; i < DCGM_MAX_NUM_FIELD_GROUPS; i++)
    {
        ...
        snprintf(fgi->fieldGroupName, sizeof(fgi->fieldGroupName)-1,
                 "bestgroupever%02d", i);
        ...
        dcgmReturn = fieldGroupManager->AddFieldGroup(fgi->fieldGroupName, fieldIds,
                                                      &fieldGroupHandles[i], watcher);
        if(dcgmReturn != DCGM_ST_OK)
        {
            std::cerr << "TestFieldGroupManager AddFieldGroup() returned " << (int)dcgmReturn << std::endl;
            numErrors++;
