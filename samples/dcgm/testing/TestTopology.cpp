...
int TestTopology::Init(std::vector<std::string>argv, std::vector<test_lwcm_gpu_t>gpus)
{
    m_gpus = gpus;
    return 0;
}
...
int TestTopology::TestTopologyDevice()
{
    ...
    {
        fprintf(stderr, "topologyInfo.numGpus %u != m_gpus.size() - 1 %u for gpuId %u\n",
                topologyInfo.numGpus, (unsigned int)m_gpus.size() - 1, m_gpus[0].gpuId);
        return 1;
    }
