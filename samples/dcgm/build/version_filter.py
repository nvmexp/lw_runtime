...
def ShowLine(line):
    # colwert all DCGM_VERSION_DEFINEs to the version number
    truncatedVersion = ".".join(version.split('.')[:2])
    line = line.replace("&&&_DCGM_VERSION_DEFINE_&&&", truncatedVersion)

    # colwert all DATE to the current date
    line = line.replace("&&&_LWRRENT_DATE_&&&", ("%d/%d/%d" % (now.year, now.month, now.day)))
    line = line.replace("&&&_LWRRENT_YEAR_&&&", ("%d" % (now.year)))
...
def CheckForUnexpectedTokens(line):
    if ("&&&_" in line) and ("_&&&" in line) and (not ("&&&_DCGM_VERSION_DEFINE_&&&" in line)) and (not ("&&&_DCGM_VERSION_ERROR_&&&" in line)) \
            and (not ("&&&_LWRRENT_DATE_&&&" in line)) and (not ("&&&_LWRRENT_YEAR_&&&" in line)):
