...
/* The current point */
double lwrrent_x = 0.0, lwrrent_y = 0.0 ;
...
static void draw_level ( int num, double m00, double m01, double m10, double m11, double n0, double n1 )
{
  ...
  for ( i = 0; i < 10; i++ )
  {
    ...
    double new_x = affine[random].a00 * lwrrent_x + affine[random].a01 * lwrrent_y + affine[random].b0 ;
    double new_y = affine[random].a10 * lwrrent_x + affine[random].a11 * lwrrent_y + affine[random].b1 ;
...
void readConfigFile ( char *fnme )
{
  ...
  for ( i = 0; i < num_trans; i++ )
  {
    ...
    /* Callwlate the stationary point */
