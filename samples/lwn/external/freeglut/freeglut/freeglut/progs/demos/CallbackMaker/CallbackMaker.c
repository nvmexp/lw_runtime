/* CallbackMaker.c */
/*
 * Program to ilwoke all the callbacks that "freeglut" supports
 */
...
main(int argc, char *argv[])
{
  ...
  /* callbacks, settings and menus for this window */
  SetWindowCallbacks( 0 );
  glutSetLwrsor(GLUT_LWRSOR_CROSSHAIR); /* Lwrsors are per window */
  ...
  SetWindowCallbacks( 0 );
  glutSetLwrsor(GLUT_LWRSOR_INHERIT);   /* Inherit cursor look from parent (this is default on window creation) - comment the below to see in action */
  glutSetLwrsor(GLUT_LWRSOR_CYCLE);
