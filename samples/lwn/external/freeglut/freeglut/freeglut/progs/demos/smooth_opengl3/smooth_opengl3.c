...
typedef void (APIENTRY *PFNGLGELWERTEXARRAYSPROC) (GLsizei n, GLuint *arrays);
...
PFNGLGELWERTEXARRAYSPROC gl_GelwertexArrays;
...
{
   gl_GelwertexArrays = (PFNGLGELWERTEXARRAYSPROC) glutGetProcAddress ("glGelwertexArrays");
   gl_BindVertexArray = (PFNGLBINDVERTEXARRAYPROC) glutGetProcAddress ("glBindVertexArray");
   if (!gl_GelwertexArrays || !gl_BindVertexArray)
   {
       fprintf (stderr, "glGelwertexArrays or glBindVertexArray not found");
       exit(1);
   }
...
void initBuffer(void)
{
    ...
    * vertex array) 
    */
   gl_GelwertexArrays(1, &vertexArrayName);
