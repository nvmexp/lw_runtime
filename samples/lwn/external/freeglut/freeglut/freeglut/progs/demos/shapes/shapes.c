...
const ourGLchar *vertexShaderSource[] = {
    "/**",
    " * From the OpenGL Programming wikibook: http://en.wikibooks.org/wiki/GLSL_Programming/GLUT/Smooth_Spelwlar_Highlights",
    ...
    "uniform mat3 m_3x3_ilw_transp;",
    " ",
    "void main()",
    "{",
    "  vec4 fg_coord4 = vec4(fg_coord, 1.0);",
    "  position = m * fg_coord4;",
    "  varyingNormalDirection = normalize(m_3x3_ilw_transp * fg_normal);",
    " ",
...
const ourGLchar *fragmentShaderSource[] = {
    ...
    "varying vec4 position;  /* position of the vertex (and fragment) in world space */",
    "varying vec3 varyingNormalDirection;  /* surface normal vector in world space */",
    "/* uniform mat4 v_ilw;   // in this demo, the view matrix is always an identity matrix */",
    " ",
    "struct lightSource",
    "{",
    "  vec4 position;",
    ...
    "  /* vec3 viewDirection = normalize(vec3(v_ilw * vec4(0.0, 0.0, 0.0, 1.0) - position)); */",
    ...
    " ",
    "      if (light0.spotLwtoff <= 90.0) /* spotlight? */",
    "        {",
    "          float clampedCosine = max(0.0, dot(-lightDirection, light0.spotDirection));",
    "          if (clampedCosine < cos(radians(light0.spotLwtoff))) /* outside of spotlight cone? */",
    "            {",
    "              attenuation = 0.0;",
    "            }",
    "          else",
    ...
    " ",
    "  vec3 spelwlarReflection;",
    "  if (dot(normalDirection, lightDirection) < 0.0) /* light source on the wrong side? */",
    "    {",
    "      spelwlarReflection = vec3(0.0, 0.0, 0.0); /* no spelwlar reflection */",
    "    }",
    "  else /* light source on the right side */",
    "    {",
    "      spelwlarReflection = attenuation * vec3(light0.spelwlar) * vec3(frontMaterial.spelwlar) ",
    "        * pow(max(0.0, dot(reflect(-lightDirection, normalDirection), viewDirection)), frontMaterial.shininess);",
    "    }",
    " ",
    "  gl_FragColor = vec4(ambientLighting + diffuseReflection + spelwlarReflection, 1.0);",
    "}"
};
...
GLint uniform_m = -1, uniform_p = -1, uniform_m_3x3_ilw_transp = -1;
...
void initShader(void)
{
    ...
    uniform_m_3x3_ilw_transp= getAttribOrUniformLocation("m_3x3_ilw_transp" , program, GL_FALSE);

    gl_UseProgram (0);

    if (attribute_fg_coord==-1 || attribute_fg_normal==-1 ||
        uniform_m==-1 || uniform_p==-1 || uniform_m_3x3_ilw_transp==-1)
        shaderReady = -1;
    else
        shaderReady = 1;
...
static void drawSolidLwbe(void)                { glutSolidLwbe(orad);                            }  /* orad doubles as size input */
static void drawWireLwbe(void)                 { glutWireLwbe(orad);                             }  /* orad doubles as size input */
 ...
 * Same for the tealwp and teaspoon
 */
...
static void drawSolidTealwp(void)
{   glFrontFace(GL_CW);    glutSolidTealwp(orad);   glFrontFace(GL_CCW);    /* orad doubles as size input */}
static void drawWireTealwp(void)
{   glFrontFace(GL_CW);    glutWireTealwp(orad);    glFrontFace(GL_CCW);    /* orad doubles as size input */}
...
static void drawSolidLwboctahedron(void)
{
...
static void drawWireLwboctahedron(void)
{
  GLfloat radius = RADIUSFAC*(GLfloat)orad; /* orad doubles as size */
...
static const entry table [] =
{
    ...
    ENTRY (Lwboctahedron,GEO_SIZE)   /* This one doesn't work when in shader mode and is then skipped */
};
#undef ENTRY
...
static void display(void)
{
    ...
    if (useShader && shaderReady)
    {
        ...
        /* There is also a glutSetVertexAttribTexCoord2, which is used only when drawing the teapot, tealwp or teaspoon */
        ...
        gl_push_matrix();
            ...
            gl_UniformMatrix3fv (uniform_m_3x3_ilw_transp, 1, GL_FALSE, get_ilw_transpose_3x3(GL_MODELVIEW));
...
key(unsigned char key, int x, int y)
{
    ...
    case 's':
        useShader=!useShader;
        /* Lwboctahedron can't be shown when in shader mode, move to next */
...
static void special (int key, int x, int y)
{
    ...
    /* Lwboctahedron can't be shown when in shader mode, skip it */
    if (useShader && NUMBEROF (table)-1 == ( unsigned )function_index)
    {
...
const GLfloat light_spelwlar[] = { 1.0f, 1.0f, 1.0f, 1.0f };
const GLfloat light_position[] = { 2.0f, 5.0f, 5.0f, 0.0f };

const GLfloat mat_ambient[]    = { 0.7f, 0.7f, 0.7f, 1.0f };
const GLfloat mat_diffuse[]    = { 0.8f, 0.8f, 0.8f, 1.0f };
const GLfloat mat_spelwlar[]   = { 1.0f, 1.0f, 1.0f, 1.0f };
...
main(int argc, char *argv[])
{
    ...
    glutSetOption ( GLUT_ACTION_ON_WINDOW_CLOSE, GLUT_ACTION_CONTINUE_EXELWTION ) ;
    ...
    glLightfv(GL_LIGHT0, GL_SPELWLAR, light_spelwlar);
    glLightfv(GL_LIGHT0, GL_POSITION, light_position);

    glMaterialfv(GL_FRONT, GL_AMBIENT,   mat_ambient);
    glMaterialfv(GL_FRONT, GL_DIFFUSE,   mat_diffuse);
    glMaterialfv(GL_FRONT, GL_SPELWLAR,  mat_spelwlar);
