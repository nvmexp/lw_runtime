...
/************************************** Defined Constants ***************************************/
/* Number of points to draw in the lwrves */
...
{
  /* Callwlate the Lorenz attractor derivatives */
...
void key_cb ( unsigned char key, int x, int y )
{
  ...
  case 'm' :  case 'M' :  /* Modify the Lorenz parameters */
    printf ( "Please enter new value for <sigma> (default %f, lwrrently %f): ", s0, sigma ) ;
    checkedFGets ( inputline, sizeof ( inputline ), stdin ) ;
    sscanf ( inputline, "%lf", &sigma ) ;

    printf ( "Please enter new value for <b> (default %f, lwrrently %f): ", b0, b ) ;
    checkedFGets ( inputline, sizeof ( inputline ), stdin ) ;
    sscanf ( inputline, "%lf", &b ) ;

    printf ( "Please enter new value for <r> (default %f, lwrrently %f): ", r0, r ) ;
    checkedFGets ( inputline, sizeof ( inputline ), stdin ) ;
...
void draw_lwrve ( int index, double position [ NUM_POINTS ][3] )
{
  int i = index ;
...
void display_cb ( void )
{
  ...
  draw_lwrve ( array_index, red_position ) ;

  glColor3d ( 0.0, 1.0, 0.0 ) ;  /* Green */
  draw_lwrve ( array_index, grn_position ) ;
