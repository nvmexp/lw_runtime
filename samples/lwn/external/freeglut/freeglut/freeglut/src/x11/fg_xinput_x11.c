...
/* colwert the XInput button state mask to the regular X mouse event button mask */
...
void fgRegisterDevices( Display* dpy, Window* win ) {
	...
	XISetMask(mask.mask, XI_FolwsIn);
	XISetMask(mask.mask, XI_FolwsOut);
...
void fgHandleExtensionEvents( XEvent* base_ev )
{
    ...
    if ( XGetEventData( fgDisplay.pDisplay.Display, cookie ) && (cookie->type == GenericEvent) && (cookie->extension == xi_opcode) ) {
        ...
        case XI_Motion:
            ...
            if (button) {
                ILWOKE_WCB( *window, MultiMotion,  ( event->deviceid, event->event_x, event->event_y ) );
            } else {
                ILWOKE_WCB( *window, MultiPassive, ( event->deviceid, event->event_x, event->event_y ) );
            }
            #if _DEBUG
