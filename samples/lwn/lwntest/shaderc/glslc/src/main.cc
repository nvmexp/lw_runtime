...
Options:
  ...
  -fhlsl_functionality1, -fhlsl-functionality1
                    ...
                    compilation.
  -filwert-y        Ilwert position.Y output in vertex shader.
  ...
  -g                Generate source-level debug information.
                    Lwrrently this option has no effect.
...
int main(int argc, char** argv) {
  ...
  shaderc_shader_kind lwrrent_fshader_stage = shaderc_glsl_infer_from_source;
  bool source_language_forced = false;
  shaderc_source_language lwrrent_source_language =
      shaderc_source_language_glsl;
  std::string lwrrent_entry_point_name("main");
...
#undef RESOURCE
      ...
      std::cout << "Target: " << spvTargetElwDescription(SPV_ELW_UNIVERSAL_1_0)
                << std::endl;
      return 0;
      ...
      lwrrent_fshader_stage = glslc::GetForcedShaderKindFromCmdLine(arg);
      if (lwrrent_fshader_stage == shaderc_glsl_infer_from_source) {
    ...
    } else if (arg == "-filwert-y") {
      compiler.options().SetIlwertY(true);
    ...
    } else if (arg.starts_with("--target-elw=")) {
      shaderc_target_elw target_elw = shaderc_target_elw_default;
      const string_piece target_elw_str =
          arg.substr(std::strlen("--target-elw="));
      uint32_t version = 0;  // Will default appropriately.
      if (target_elw_str == "vulkan") {
        target_elw = shaderc_target_elw_vulkan;
      } else if (target_elw_str == "vulkan1.0") {
        target_elw = shaderc_target_elw_vulkan;
        version = shaderc_elw_version_vulkan_1_0;
      } else if (target_elw_str == "vulkan1.1") {
        target_elw = shaderc_target_elw_vulkan;
        version = shaderc_elw_version_vulkan_1_1;
      } else if (target_elw_str == "vulkan1.2") {
        target_elw = shaderc_target_elw_vulkan;
        version = shaderc_elw_version_vulkan_1_2;
      } else if (target_elw_str == "opengl") {
        target_elw = shaderc_target_elw_opengl;
      } else if (target_elw_str == "opengl4.5") {
        target_elw = shaderc_target_elw_opengl;
        version = shaderc_elw_version_opengl_4_5;
      } else if (target_elw_str == "opengl_compat") {
        target_elw = shaderc_target_elw_opengl_compat;
      } else {
        std::cerr << "glslc: error: invalid value '" << target_elw_str
                  << "' in '--target-elw=" << target_elw_str << "'"
                  << std::endl;
        return 1;
      }
      compiler.options().SetTargetElwironment(target_elw, version);
      ...
      if (!shaderc_util::GetOptionArgument(argc, argv, &i, "-x", &option_arg)) {
        ...
        if (option_arg == "glsl") {
          lwrrent_source_language = shaderc_source_language_glsl;
        } else if (option_arg == "hlsl") {
          lwrrent_source_language = shaderc_source_language_hlsl;
        } else {
      ...
      if (length <= 2) {
        ...
        if (name_piece.find("__") != string_piece::npos) {
          std::cerr
              << "glslc: warning: names containing conselwtive underscores "
                 "are reserved: "
              << arg << std::endl;
        }
      ...
      const auto language = source_language_forced
                                ? lwrrent_source_language
                                : ((glslc::GetFileExtension(arg) == "hlsl")
                                       ? shaderc_source_language_hlsl
                                       : shaderc_source_language_glsl);

      // If lwrrent_fshader_stage is shaderc_glsl_infer_from_source, that means
      ...
      // from the file name. If lwrrent_fshader_stage is specifed to one of
      // the forced shader kinds, use that for the following compilation.
      input_files.emplace_back(glslc::InputFileSpec{
          arg.str(), (lwrrent_fshader_stage == shaderc_glsl_infer_from_source
                          ? glslc::DeduceDefaultShaderKindFromFileName(arg)
                          : lwrrent_fshader_stage),
          language, lwrrent_entry_point_name});
    }
  }
