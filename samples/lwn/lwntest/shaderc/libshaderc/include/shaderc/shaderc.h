...
typedef enum {
  ...
  shaderc_limit_max_lwll_distances,
  shaderc_limit_max_combined_clip_and_lwll_distances,
...
//          compiler, "#version 450\lwoid main() {}", 27,
...
// ilwolving this shaderc_compiler_t.
...
// second callback is ilwoked to release the result.  Both callbacks take a
...
// a value listed in shaderc_elw_version.  The 0 value maps to Vulkan 1.0 if
// |target| is Vulkan, and it maps to OpenGL 4.5 if |target| is OpenGL.
SHADERC_EXPORT void shaderc_compile_options_set_target_elw(
    shaderc_compile_options_t options,
...
// Sets whether the compiler should ilwert position.Y output in vertex shader.
SHADERC_EXPORT void shaderc_compile_options_set_ilwert_y(
    shaderc_compile_options_t options, bool enable);
...
// string and a failure in deducing will generate an error. Lwrrently only
