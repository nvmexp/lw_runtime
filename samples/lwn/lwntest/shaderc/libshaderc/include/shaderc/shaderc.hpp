...
class CompileOptions {
 public:
  ...
  // or a value listed in shaderc_elw_version.  The 0 value maps to Vulkan 1.0
  // if |target| is Vulkan, and it maps to OpenGL 4.5 if |target| is OpenGL.
  void SetTargetElwironment(shaderc_target_elw target, uint32_t version) {
    shaderc_compile_options_set_target_elw(options_, target, version);
  }
  ...
  void SetIlwertY(bool enable) {
    shaderc_compile_options_set_ilwert_y(options_, enable);
  }
...
class Compiler {
 public:
  ...
  // a failure in this proess will generate an error. Lwrrently only #pragma
