...
// Colwerts the target elw to the corresponding one in shaderc_util::Compiler.
shaderc_util::Compiler::TargetElw GetCompilerTargetElw(shaderc_target_elw elw) {
  switch (elw) {
    case shaderc_target_elw_opengl:
      return shaderc_util::Compiler::TargetElw::OpenGL;
    case shaderc_target_elw_opengl_compat:
      return shaderc_util::Compiler::TargetElw::OpenGLCompat;
    case shaderc_target_elw_webgpu:
      return shaderc_util::Compiler::TargetElw::WebGPU;
    case shaderc_target_elw_vulkan:
    default:
      break;
  }

  return shaderc_util::Compiler::TargetElw::Vulkan;
}

shaderc_util::Compiler::TargetElwVersion GetCompilerTargetElwVersion(
    uint32_t version_number) {
  using namespace shaderc_util;

  if (static_cast<uint32_t>(Compiler::TargetElwVersion::Vulkan_1_0) ==
      version_number) {
    return Compiler::TargetElwVersion::Vulkan_1_0;
  }
  if (static_cast<uint32_t>(Compiler::TargetElwVersion::Vulkan_1_1) ==
      version_number) {
    return Compiler::TargetElwVersion::Vulkan_1_1;
  }
  if (static_cast<uint32_t>(Compiler::TargetElwVersion::Vulkan_1_2) ==
      version_number) {
    return Compiler::TargetElwVersion::Vulkan_1_2;
  }
  if (static_cast<uint32_t>(Compiler::TargetElwVersion::OpenGL_4_5) ==
      version_number) {
    return Compiler::TargetElwVersion::OpenGL_4_5;
  }

  return Compiler::TargetElwVersion::Default;
}
...
struct shaderc_compile_options {
  shaderc_target_elw target_elw = shaderc_target_elw_default;
  uint32_t target_elw_version = 0;
...
void shaderc_compile_options_set_target_elw(shaderc_compile_options_t options,
                                            shaderc_target_elw target,
                                            uint32_t version) {
  options->target_elw = target;
  options->compiler.SetTargetElw(GetCompilerTargetElw(target),
                                 GetCompilerTargetElwVersion(version));
}
...
void shaderc_compile_options_set_ilwert_y(
    shaderc_compile_options_t options, bool enable) {
  options->compiler.EnableIlwertY(enable);
}
...
shaderc_compilation_result_t CompileToSpecifiedOutputType(
  ...
  result->compilation_status = shaderc_compilation_status_ilwalid_stage;
  ...
  TRY_IF_EXCEPTIONS_ENABLED {
    ...
    if (compilation_succeeded) {
      ...
      result->compilation_status =
          stage_deducer.error() ? shaderc_compilation_status_ilwalid_stage
...
shaderc_compilation_result_t shaderc_assemble_into_spv(
  ...
  if (!result) return nullptr;
  result->compilation_status = shaderc_compilation_status_ilwalid_assembly;
  if (!compiler->initializer) return result;
  ...
  TRY_IF_EXCEPTIONS_ENABLED {
    ...
    const uint32_t target_elw_version =
        additional_options ? additional_options->target_elw_version : 0;
    const bool assembling_succeeded = shaderc_util::SpirvToolsAssemble(
        GetCompilerTargetElw(target_elw),
