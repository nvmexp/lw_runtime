...
// target elwironments exposed by shaderc. So the following is just testing the
// target environment is accepted.
TEST_F(AssembleStringTest, AcceptTargetElw) {
  ASSERT_NE(nullptr, compiler_.get_compiler_handle());
  shaderc_compile_options_set_target_elw(options_, shaderc_target_elw_opengl,
                                         /* version = */ 0);
...
TEST_F(CompileStringTest, ErrorTypeUnknownShaderStage) {
  ...
  EXPECT_EQ(shaderc_compilation_status_ilwalid_stage,
...
TEST_F(CompileStringWithOptionsTest, MacroCompileOptions) {
  ...
  shaderc_compile_options_add_macro_definition(options_.get(), "E", 1u, "main",
                                               4u);
  const std::string kMinimalExpandedShader = "#version 140\lwoid E(){}";
...
TEST_F(CompileStringWithOptionsTest, ForcedVersionProfileIlwalidModule) {
...
TEST_F(CompileStringWithOptionsTest, ForcedVersionProfileUnknowlwersionStd) {
...
TEST_F(CompileStringWithOptionsTest, CompileAndOptimizeForVulkan10Failure) {
  ...
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_0);
  shaderc_compile_options_set_optimization_level(
      options_.get(), shaderc_optimization_level_performance);
...
TEST_F(CompileStringWithOptionsTest, CompileAndOptimizeForVulkan11Success) {
  ...
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_1);
  shaderc_compile_options_set_optimization_level(
      options_.get(), shaderc_optimization_level_performance);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::ValuesIn(std::vector<ShaderKindTestCase>{
        ...
        {kLWMeshShader, shaderc_glsl_default_mesh_shader},
        {kLWTaskShader, shaderc_glsl_default_task_shader},
        ...
        {kLWMeshShaderWithPragma, shaderc_glsl_default_geometry_shader},
        {kLWTaskShaderWithPragma, shaderc_glsl_default_geometry_shader},

        // Infer from source
        {kVertexOnlyShaderWithPragma, shaderc_glsl_infer_from_source},
        {kLWMeshShaderWithPragma, shaderc_glsl_infer_from_source},
        {kLWTaskShaderWithPragma, shaderc_glsl_infer_from_source},

        // Specified non-default shader kind overrides #pragma annotation.
        {kVertexOnlyShaderWithIlwalidPragma, shaderc_glsl_vertex_shader},
    }));

using IlwalidShaderKind = testing::TestWithParam<ShaderKindTestCase>;

// Invalid shader kind settings should generate errors.
TEST_P(IlwalidShaderKind, CompilationShouldFail) {
  const ShaderKindTestCase& test_case = GetParam();
  Compiler compiler;
  EXPECT_FALSE(
      CompilesToValidSpv(compiler, test_case.shader_, test_case.shader_kind_));
...
INSTANTIATE_TEST_SUITE_P(
    CompileStringTest, IlwalidShaderKind,
    testing::ValuesIn(std::vector<ShaderKindTestCase>{
        ...
        {kVertexOnlyShaderWithIlwalidPragma,
...
TEST_F(CompileStringWithOptionsTest, GlobalWarnings) {
  ...
  shaderc_compile_options_set_forced_version_profile(options_.get(), 400,
                                                     shaderc_profile_core);
  EXPECT_THAT(CompilationWarnings(kMinimalUnknowlwersionShader,
                                  shaderc_glsl_vertex_shader, options_.get()),
...
TEST_F(CompileStringWithOptionsTest, GlobalWarningsAsErrors) {
  ...
  shaderc_compile_options_set_forced_version_profile(options_.get(), 400,
                                                     shaderc_profile_core);
  EXPECT_THAT(CompilationErrors(kMinimalUnknowlwersionShader,
                                shaderc_glsl_vertex_shader, options_.get()),
              HasSubstr("(version, profile) forced to be (400, core),"
                        " while in source code it is (550, none)\n"));
}
...
TEST_F(CompileStringWithOptionsTest, SuppressGlobalWarnings) {
  ...
  shaderc_compile_options_set_forced_version_profile(options_.get(), 400,
                                                     shaderc_profile_core);
  EXPECT_EQ("",
            CompilationWarnings(kMinimalUnknowlwersionShader,
                                shaderc_glsl_vertex_shader, options_.get()));
}
...
TEST_F(CompileStringWithOptionsTest,
  ...
  Compilation comp_global(
      compiler_.get_compiler_handle(), kMinimalUnknowlwersionShader,
...
TEST_F(
    ...
    TargetElwRespectedWhenCompilingOpenGLCompatibilityShaderToBinaryAndAlwaysFails) {
  ...
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_opengl_compat, 0);
  EXPECT_FALSE(CompilesToValidSpv(compiler_, kOpenGLCompatibilityFragmentShader,
                                  shaderc_glsl_fragment_shader,
                                  options_.get()));

  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_opengl, 0);
  EXPECT_FALSE(CompilesToValidSpv(compiler_, kOpenGLCompatibilityFragmentShader,
                                  shaderc_glsl_fragment_shader,
                                  options_.get()));

  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan, 0);
  EXPECT_FALSE(CompilesToValidSpv(compiler_, kOpenGLCompatibilityFragmentShader,
                                  shaderc_glsl_fragment_shader,
                                  options_.get()));
}
...
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingOpenGLCoreShaderToBinary) {
...
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_0ShaderToVulkan1_0Succeeds) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_0);
...
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_0ShaderToVulkan1_1Succeeds) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_1);
  EXPECT_TRUE(CompilesToValidSpv(compiler_, kGlslShaderComputeBarrier,
...
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_1ShaderToVulkan1_0Fails) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_0);
...
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_1ShaderToVulkan1_1Succeeds) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_1);
...
// task shader
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_0TaskShaderToVulkan1_0Succeeds) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_0);
...
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_0TaskShaderToVulkan1_1Succeeds) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_1);
  EXPECT_TRUE(CompilesToValidSpv(compiler_, kGlslShaderTaskBarrier,
...
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_1TaskShaderToVulkan1_0Fails) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_0);
...
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_1TaskShaderToVulkan1_1Succeeds) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_1);
...
// mesh shader
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_0MeshShaderToVulkan1_0Succeeds) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_0);
...
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_0MeshShaderToVulkan1_1Succeeds) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_1);
  EXPECT_TRUE(CompilesToValidSpv(compiler_, kGlslShaderMeshBarrier,
...
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_1MeshShaderToVulkan1_0Fails) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_0);
...
TEST_F(CompileStringWithOptionsTest,
       TargetElwRespectedWhenCompilingVulkan1_1MeshShaderToVulkan1_1Succeeds) {
  shaderc_compile_options_set_target_elw(options_.get(),
                                         shaderc_target_elw_vulkan,
                                         shaderc_elw_version_vulkan_1_1);
...
TEST_F(CompileStringWithOptionsTest,
       DISABLED_TargetElwIgnoredWhenPreprocessing) {
  ...
  // test once we have a differential set of elwironments to test.
...
TEST_F(CompileStringTest, ShaderKindRespected) {
  ASSERT_NE(nullptr, compiler_.get_compiler_handle());
  const std::string kVertexShader =
      "#version 140\lwoid main(){ gl_Position = vec4(0);}";
...
TEST_F(CompileStringTest, MultipleThreadsCalling) {
  ...
  for (auto& r : results) {
    threads.emplace_back([&r, this]() {
      r = CompilationSuccess("#version 140\lwoid main(){}",
                             shaderc_glsl_vertex_shader);
    });
  }
  for (auto& t : threads) {
...
TEST_F(CompileKindsTest, Fragment) {
  ...
  const std::string kFragShader =
      "#version 140\lwoid main(){ gl_FragColor = vec4(0);}";
