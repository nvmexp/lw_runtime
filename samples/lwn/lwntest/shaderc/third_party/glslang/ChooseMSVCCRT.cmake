...
#
# The macro is ilwoked at the end of the file.
#
...
macro(get_lwrrent_crt crt_lwrrent regex flagsvar)
  # Find the selected-by-CMake CRT for each build type, if any.
  # Strip off the leading slash and any whitespace.
  string(REGEX MATCH "${${regex}}" ${crt_lwrrent} "${${flagsvar}}")
  string(REPLACE "/" " " ${crt_lwrrent} "${${crt_lwrrent}}")
  string(STRIP "${${crt_lwrrent}}" ${crt_lwrrent})
endmacro(get_lwrrent_crt)

# Replaces or adds a flag to a variable.
# Expects 'flag' to be padded with spaces.
macro(set_flag_in_var flagsvar regex flag)
  string(REGEX MATCH "${${regex}}" lwrrent_flag "${${flagsvar}}")
  if("${lwrrent_flag}" STREQUAL "")
...
variables (LLVM_USE_CRT_DEBUG, etc) instead.")
  ...
  foreach(build_type ${CMAKE_CONFIGURATION_TYPES} ${CMAKE_BUILD_TYPE})
    ...
    if (NOT LLVM_USE_CRT_${build})
      get_lwrrent_crt(LLVM_USE_CRT_${build}
        MSVC_CRT_REGEX
