...
class FloatProxy {
 public:
  ...
  // implicit colwersions allow us to use it more transparently.
...
class HexFloat {
 public:
  ...
  // These are all written like this because it is colwenient to have
  ...
  // even if they are not going to be exelwted. Eg:
  // constant_number < 0? 0: constant_number
  // These colwert the negative left-shifts into right shifts.
...
std::ostream& operator<<(std::ostream& os, const HexFloat<T, Traits>& value) {
  ...
  // exponent contains the biased exponent we have to colwert it back into
  // the normal range.
...
ParseNormalFloat<FloatProxy<Float16>, HexFloatTraits<FloatProxy<Float16>>>(
  ...
  // Then colwert to 16-bit float, saturating at infinities, and
