...
def colwert_min_required_version(version):
    """Colwerts the minimal required SPIR-V version encoded in the grammar to
...
def colwert_max_required_version(version):
    """Colwerts the maximum required SPIR-V version encoded in the grammar to
    the symbol in SPIRV-Tools."""
    if version is None:
        return '0xffffffffu'
    return 'SPV_SPIRV_VERSION_WORD({})'.format(version.replace('.', ','))
...
def colwert_operand_kind(operand_tuple):
...
class InstInitializer(object):
    ...
    def __init__(self, opname, caps, exts, operands, version, lastVersion):
        ...
        self.operands = [colwert_operand_kind(o) for o in operands]
        ...
        self.version = colwert_min_required_version(version)
        self.lastVersion = colwert_max_required_version(lastVersion)
...
class EnumerantInitializer(object):
    ...
    def __init__(self, enumerant, value, caps, exts, parameters, version, lastVersion):
        ...
        self.num_exts = len(exts)
        self.exts = get_extension_array_name(exts)
        self.parameters = [colwert_operand_kind(p) for p in parameters]
        self.version = colwert_min_required_version(version)
        self.lastVersion = colwert_max_required_version(lastVersion)
...
def generate_operand_kind_table(enums):
    ...
    enum_kinds = [colwert_operand_kind(e)
...
def get_extension_list(instructions, operand_kinds):
    ...
    # diffilwlt. Shouldn't be used for any other reasons.
...
def generate_capability_to_string_mapping(operand_kinds):
    ...
    function += '    case SpvCapabilityMax:\n' \
        '      assert(0 && "Attempting to colwert SpvCapabilityMax to string");\n' \
        '      return "";\n'
    function += '  };\n\n  return "";\n}'
    return function
