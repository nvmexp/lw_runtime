...
typedef enum spv_result_t {
  ...
  SPV_ERROR_ILWALID_POINTER = -3,
  SPV_ERROR_ILWALID_BINARY = -4,
  SPV_ERROR_ILWALID_TEXT = -5,
  SPV_ERROR_ILWALID_TABLE = -6,
  SPV_ERROR_ILWALID_VALUE = -7,
  SPV_ERROR_ILWALID_DIAGNOSTIC = -8,
  SPV_ERROR_ILWALID_LOOKUP = -9,
  SPV_ERROR_ILWALID_ID = -10,
  SPV_ERROR_ILWALID_CFG = -11,
  SPV_ERROR_ILWALID_LAYOUT = -12,
  SPV_ERROR_ILWALID_CAPABILITY = -13,
  SPV_ERROR_ILWALID_DATA = -14,  // Indicates data rules validation failure.
...
typedef enum spv_operand_type_t {
  ...
  SPV_OPERAND_TYPE_EXELWTION_MODEL,               // SPIR-V Sec 3.3
  SPV_OPERAND_TYPE_ADDRESSING_MODEL,              // SPIR-V Sec 3.4
  SPV_OPERAND_TYPE_MEMORY_MODEL,                  // SPIR-V Sec 3.5
  SPV_OPERAND_TYPE_EXELWTION_MODE,                // SPIR-V Sec 3.6
...
typedef struct spv_parsed_operand_t {
  ...
  uint16_t offset;
  // Number of words oclwpied by this operand.
  uint16_t num_words;
...
// Certain target elwironments impose additional restrictions on SPIR-V, so it's
// often necessary to specify which one applies.  SPV_ELW_UNIVERSAL_* implies an
...
//    SPV_ELW_VULKAN_1_0           ->  SPIR-V 1.0
//    SPV_ELW_VULKAN_1_1           ->  SPIR-V 1.3
//    SPV_ELW_VULKAN_1_1_SPIRV_1_4 ->  SPIR-V 1.4
//    SPV_ELW_VULKAN_1_2           ->  SPIR-V 1.5
// Consult the description of API entry points for specific rules.
typedef enum {
  SPV_ELW_UNIVERSAL_1_0,  // SPIR-V 1.0 latest revision, no other restrictions.
  SPV_ELW_VULKAN_1_0,     // Vulkan 1.0 latest revision.
  SPV_ELW_UNIVERSAL_1_1,  // SPIR-V 1.1 latest revision, no other restrictions.
  SPV_ELW_OPENCL_2_1,     // OpenCL Full Profile 2.1 latest revision.
  SPV_ELW_OPENCL_2_2,     // OpenCL Full Profile 2.2 latest revision.
  SPV_ELW_OPENGL_4_0,     // OpenGL 4.0 plus GL_ARB_gl_spirv, latest revisions.
  SPV_ELW_OPENGL_4_1,     // OpenGL 4.1 plus GL_ARB_gl_spirv, latest revisions.
  SPV_ELW_OPENGL_4_2,     // OpenGL 4.2 plus GL_ARB_gl_spirv, latest revisions.
  SPV_ELW_OPENGL_4_3,     // OpenGL 4.3 plus GL_ARB_gl_spirv, latest revisions.
  // There is no variant for OpenGL 4.4.
  SPV_ELW_OPENGL_4_5,     // OpenGL 4.5 plus GL_ARB_gl_spirv, latest revisions.
  SPV_ELW_UNIVERSAL_1_2,  // SPIR-V 1.2, latest revision, no other restrictions.
  SPV_ELW_OPENCL_1_2,     // OpenCL Full Profile 1.2 plus cl_khr_il_program,
                          // latest revision.
  SPV_ELW_OPENCL_EMBEDDED_1_2,  // OpenCL Embedded Profile 1.2 plus
                                // cl_khr_il_program, latest revision.
  SPV_ELW_OPENCL_2_0,  // OpenCL Full Profile 2.0 plus cl_khr_il_program,
                       // latest revision.
  SPV_ELW_OPENCL_EMBEDDED_2_0,  // OpenCL Embedded Profile 2.0 plus
                                // cl_khr_il_program, latest revision.
  SPV_ELW_OPENCL_EMBEDDED_2_1,  // OpenCL Embedded Profile 2.1 latest revision.
  SPV_ELW_OPENCL_EMBEDDED_2_2,  // OpenCL Embedded Profile 2.2 latest revision.
  SPV_ELW_UNIVERSAL_1_3,  // SPIR-V 1.3 latest revision, no other restrictions.
  SPV_ELW_VULKAN_1_1,     // Vulkan 1.1 latest revision.
  SPV_ELW_WEBGPU_0,       // Work in progress WebGPU 1.0.
  SPV_ELW_UNIVERSAL_1_4,  // SPIR-V 1.4 latest revision, no other restrictions.

  // Vulkan 1.1 with VK_KHR_spirv_1_4, i.e. SPIR-V 1.4 binary.
  SPV_ELW_VULKAN_1_1_SPIRV_1_4,

  SPV_ELW_UNIVERSAL_1_5,  // SPIR-V 1.5 latest revision, no other restrictions.
  SPV_ELW_VULKAN_1_2,     // Vulkan 1.2 latest revision.
} spv_target_elw;
...
SPIRV_TOOLS_EXPORT const char* spvTargetElwDescription(spv_target_elw elw);

// Parses s into *elw and returns true if successful.  If unparsable, returns
// false and sets *elw to SPV_ELW_UNIVERSAL_1_0.
SPIRV_TOOLS_EXPORT bool spvParseTargetElw(const char* s, spv_target_elw* elw);

// Determines the target elw value with the least features but which enables
// the given Vulkan and SPIR-V versions. If such a target is supported, returns
// true and writes the value to |elw|, otherwise returns false.
//
...
SPIRV_TOOLS_EXPORT bool spvParseVulkanElw(uint32_t vulkan_ver,
                                          uint32_t spirv_ver,
                                          spv_target_elw* elw);

// Creates a context object for most of the SPIRV-Tools API.
// Returns null if elw is invalid.
//
// See specific API calls for how the target environment is interpeted
// (partilwlarly assembly and validation).
SPIRV_TOOLS_EXPORT spv_context spvContextCreate(spv_target_elw elw);
...
// 2) OpReturlwalue returning a pointer value
...
SPIRV_TOOLS_EXPORT void spvOptimizerOptionsSetRulwalidator(
...
// kStateIlwalid if a reduction step yields a state that fails SPIR-V
// validation. Otherwise, an invalid state is treated as uninteresting and the
// reduction backtracks and continues.
SPIRV_TOOLS_EXPORT void spvReducerOptionsSetFailOlwalidationError(
