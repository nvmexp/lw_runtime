...
class Optimizer {
 public:
  ...
  // Constructs an instance with the given target |elw|, which is used to decode
  ...
  // another exelwtion environment, then the source environment should be
  // supplied. e.g. for VulkanToWebGPUPasses the environment should be
  // SPV_ELW_VULKAN_1_1 not SPV_ELW_WEBGPU_0.
  explicit Optimizer(spv_target_elw elw);
  ...
  // ilwoked once for each message communicated from the library.
  ...
  // Registers passes that have been prescribed for colwerting from Vulkan to
  // WebGPU. This sequence of passes is subject to constant review and will
  // change from time to time.
  Optimizer& RegisterVulkanToWebGPUPasses();

  // Registers passes that have been prescribed for colwerting from WebGPU to
  ...
  void SetTargetElw(const spv_target_elw elw);
  ...
  // In that case, no further passes are exelwted and the contents in
...
// Vulkan, so isn't needed in that elw. And the related capability is not
// allowed in WebGPU, so it is not allowed in that elw.
Optimizer::PassToken CreateStripAtomicCounterMemoryPass();

// Creates a strip-debug-info pass.
// A strip-debug-info pass removes all debug instructions (as dolwmented in
...
//   OpSColwert, OpFColwert, OpQuantizeToF16 and
...
//
// While this pass reduces the number of oclwrrences of this sequence, at
...
// in either its parameter types or return type. An opaque type is lwrrently
...
// LocalAccessChainColwert. This pass will reduce the work needed to be done
...
// BranchConditionals with constant condition and colwert to a Branch to
...
// are lwrrently processed. Lwrrently modules with any extensions enabled are
// not processed. This is left for future work.
//
// This pass is most effective if preceeded by Inlining and
// LocalAccessChainColwert. LocalSingleStoreElim and LocalSingleBlockElim
// will reduce the work that this pass has to do.
Optimizer::PassToken CreateLocalMultiStoreElimPass();

// Creates a local access chain colwersion pass.
// A local access chain colwersion pass identifies all function scope
// variables which are accessed only with loads, stores and access chains
// with constant indices. It then colwerts all loads and stores of such
// variables into equivalent sequences of loads, stores, extracts and inserts.
//
// This pass only processes entry point functions. It lwrrently only colwerts
...
// possible.
Optimizer::PassToken CreateLocalAccessChainColwertPass();
...
//
// Lwrrently, the presence of access chains and function calls can inhibit this
// pass, however the Inlining and LocalAccessChainColwert passes can make it
...
// colwerted to inserts and extracts and local loads and stores are eliminated.
...
//
// This pass performs best after access chains are colwerted to inserts and
// extracts and local loads and stores are eliminated. While exelwting this
...
// lwrrently will not process functions with function calls. Unreachable
...
// Colwersion, which tends to cause cycles of dead code to be left after
...
// reachable from entry points and exported functions. It lwrrently includes the
...
// Create LICM pass.
// This pass will look for ilwariant instructions inside loops and hoist them to
// the loops preheader.
Optimizer::PassToken CreateLoopIlwariantCodeMotionPass();
...
// iterations of the loop will be exelwted outside of the main loop.
...
// simplified. This may reduce code size by removing never exelwted jump targets
...
// Creates a pass that colwerts if-then-else like assignments into OpSelect.
Optimizer::PassToken CreateIfColwersionPass();
...
Optimizer::PassToken CreateReplaceIlwalidOpcodePass();
...
// This pass colwerts load/store operations on function local variables into
...
// Create pass to colwert relaxed precision instructions to half precision.
// This pass colwerts as many relaxed float32 arithmetic operations to half as
// possible. It colwerts any float32 operands to half if needed. It colwerts
...
Optimizer::PassToken CreateColwertRelaxedToHalfPass();
...
//
// TODO(greg-lunarg): Add support for buffer references. Lwrrently only does
...
// optimize instrument code ilwolving the testing of compile-time constants.
...
// in WebGPU. Lwrrently this pass naively initializes variables that are
...
Optimizer::PassToken CreateSplitIlwalidUnreachablePass();
...
//
// TODO(dneto): Clamps coordinates and sample index for pointer callwlations
// into storage images (OpImageTexelPointer).  For an lwbe array image, it
