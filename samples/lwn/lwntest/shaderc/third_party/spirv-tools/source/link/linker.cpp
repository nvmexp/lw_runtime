...
//                     lwrrently not handled. (You could have a group being
...
//
// TODO(pierremoreau): Decorations on functions parameters are lwrrently not
// checked.
...
spv_result_t ShiftIdsInModules(const MessageConsumer& consumer,
  ...
  if (modules == nullptr)
    return DiagnosticStream(position, consumer, "", SPV_ERROR_ILWALID_DATA)
  ...
  for (auto module_iter = modules->begin() + 1; module_iter != modules->end();
    ...
    if (id_bound > 0x3FFFFF)
      return DiagnosticStream(position, consumer, "", SPV_ERROR_ILWALID_ID)
             << "The limit of IDs, 4194303, was exceeded:"
             << " " << id_bound << " is the current ID bound.";

    // Ilwalidate the DefUseManager
    module->context()->IlwalidateAnalyses(opt::IRContext::kAnalysisDefUse);
  }
  ++id_bound;
  if (id_bound > 0x3FFFFF)
    return DiagnosticStream(position, consumer, "", SPV_ERROR_ILWALID_ID)
...
spv_result_t MergeModules(const MessageConsumer& consumer,
  ...
  do {
    ...
    for (const auto& module : input_modules) {
      ...
      if (addressing_model != memory_model_inst->GetSingleWordOperand(0u)) {
        spv_operand_desc initial_desc = nullptr, lwrrent_desc = nullptr;
  ...
  for (const auto& module : input_modules)
    for (const auto& inst : module->entry_points()) {
      ...
      if (i != entry_points.end()) {
        ...
        grammar.lookupOperand(SPV_OPERAND_TYPE_EXELWTION_MODEL, model, &desc);
        return DiagnosticStream(position, consumer, "", SPV_ERROR_INTERNAL)
               << "The entry point \"" << name << "\", with exelwtion model "
  ...
  for (const auto& module : input_modules)
    for (const auto& inst : module->exelwtion_modes())
      linked_module->AddExelwtionMode(
          std::unique_ptr<Instruction>(inst.Clone(linked_context)));
...
spv_result_t GetImportExportPairs(const MessageConsumer& consumer,
  ...
  for (const auto& decoration : linked_context.annotations()) {
    ...
    if (def_inst == nullptr)
      return DiagnosticStream(position, consumer, "", SPV_ERROR_ILWALID_BINARY)
...
spv_result_t RemoveLinkageSpecificInstructions(
  ...
  // Remove export linkage attributes if making an exelwtable
  ...
  // Remove Linkage capability if making an exelwtable and partial linkage is
  // not allowed
...
spv_result_t Link(const Context& context, const uint32_t* const* binaries,
  ...
  for (size_t i = 0u; i < num_binaries; ++i) {
    ...
    std::unique_ptr<IRContext> ir_context = BuildModule(
        c_context->target_elw, consumer, binaries[i], binary_sizes[i]);
  ...
  if (res != SPV_SUCCESS) return res;
  IRContext linked_context(c_context->target_elw, consumer);
  ...
  if (pass_res == opt::Pass::Status::Failure) return SPV_ERROR_ILWALID_DATA;
