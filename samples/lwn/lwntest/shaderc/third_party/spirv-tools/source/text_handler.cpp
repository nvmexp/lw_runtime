...
spv_result_t getWord(spv_text text, spv_position position, std::string* word) {
  if (!text->str || !text->length) return SPV_ERROR_ILWALID_TEXT;
  if (!position) return SPV_ERROR_ILWALID_POINTER;
...
char AssemblyContext::peek() const {
  return text_->str[lwrrent_position_.index];
}

bool AssemblyContext::hasText() const {
  return text_->length > lwrrent_position_.index;
}

void AssemblyContext::seekForward(uint32_t size) {
  lwrrent_position_.index += size;
  lwrrent_position_.column += size;
}
...
spv_result_t AssemblyContext::binaryEncodeNumericLiteral(
  ...
  switch (parse_status) {
    case EncodeNumberStatus::kSuccess:
      return SPV_SUCCESS;
    case EncodeNumberStatus::kIlwalidText:
      return diagnostic(error_code) << error_msg;
    case EncodeNumberStatus::kUnsupported:
      return diagnostic(SPV_ERROR_INTERNAL) << error_msg;
    case EncodeNumberStatus::kIlwalidUsage:
      return diagnostic(SPV_ERROR_ILWALID_TEXT) << error_msg;
  }
