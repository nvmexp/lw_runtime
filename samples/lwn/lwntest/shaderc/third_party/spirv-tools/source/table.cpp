...
spv_context spvContextCreate(spv_target_elw elw) {
  switch (elw) {
  ...
  spvOpcodeTableGet(&opcode_table, elw);
  spvOperandTableGet(&operand_table, elw);
  spvExtInstTableGet(&ext_inst_table, elw);

  return new spv_context_t{elw, opcode_table, operand_table, ext_inst_table,
