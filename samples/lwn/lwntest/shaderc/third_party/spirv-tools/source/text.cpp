...
/// and returns SPV_ERROR_ILWALID_TEXT.
spv_result_t encodeImmediate(spvtools::AssemblyContext* context,
  ...
  if (!spvtools::utils::ParseNumber(text + 1, &parse_result)) {
    return context->diagnostic(SPV_ERROR_ILWALID_TEXT)
...
spv_result_t spvTextEncodeOperand(const spvtools::AssemblyGrammar& grammar,
  ...
  spv_result_t error_code_for_literals =
      spvOperandIsOptional(type) ? SPV_FAILED_MATCH : SPV_ERROR_ILWALID_TEXT;
...
// |elw| and Id bound is via |bound|.
spv_result_t SetHeader(spv_target_elw elw, const uint32_t bound,
                       uint32_t* header) {
  if (!header) return SPV_ERROR_ILWALID_BINARY;

  header[SPV_INDEX_MAGIC_NUMBER] = SpvMagicNumber;
  header[SPV_INDEX_VERSION_NUMBER] = spvVersionForTargetElw(elw);
...
spv_result_t GetNumericIds(const spvtools::AssemblyGrammar& grammar,
  ...
  if (!grammar.isValid()) {
    return SPV_ERROR_ILWALID_TABLE;
  }
...
spv_result_t spvTextToBinaryInternal(const spvtools::AssemblyGrammar& grammar,
  ...
  if (!pBinary) return SPV_ERROR_ILWALID_POINTER;
  ...
  uint64_t lwrrentIndex = SPV_INDEX_INSTRUCTION;
  for (auto& inst : instructions) {
    memcpy(data + lwrrentIndex, inst.words.data(),
           sizeof(uint32_t) * inst.words.size());
    lwrrentIndex += inst.words.size();
  }

  if (auto error = SetHeader(grammar.target_elw(), context.getBound(), data))
    return error;
