...
spv_result_t spvOperandTableGet(spv_operand_table* pOperandTable,
                                spv_target_elw) {
  if (!pOperandTable) return SPV_ERROR_ILWALID_POINTER;

  *pOperandTable = &kOperandTable;
  return SPV_SUCCESS;
}

spv_result_t spvOperandTableNameLookup(spv_target_elw elw,
                                       const spv_operand_table table,
                                       const spv_operand_type_t type,
                                       const char* name,
                                       const size_t nameLength,
                                       spv_operand_desc* pEntry) {
  if (!table) return SPV_ERROR_ILWALID_TABLE;
  if (!name || !pEntry) return SPV_ERROR_ILWALID_POINTER;

  const auto version = spvVersionForTargetElw(elw);
  for (uint64_t typeIndex = 0; typeIndex < table->count; ++typeIndex) {
    ...
    for (uint64_t index = 0; index < group.count; ++index) {
      ...
      // validator's work.
      if (((version >= entry.milwersion && version <= entry.lastVersion) ||
  ...
  return SPV_ERROR_ILWALID_LOOKUP;
}

spv_result_t spvOperandTableValueLookup(spv_target_elw elw,
                                        const spv_operand_table table,
                                        const spv_operand_type_t type,
                                        const uint32_t value,
                                        spv_operand_desc* pEntry) {
  if (!table) return SPV_ERROR_ILWALID_TABLE;
  if (!pEntry) return SPV_ERROR_ILWALID_POINTER;
  ...
  for (uint64_t typeIndex = 0; typeIndex < table->count; ++typeIndex) {
    ...
    // elwironments, which means they can have different minimal version
    ...
    for (auto it = std::lower_bound(beg, end, needle, comp);
      ...
      // is indeed requested in the SPIR-V code; checking that should be
      // validator's work.
      if ((version >= it->milwersion && version <= it->lastVersion) ||
...
void spvPushOperandTypesForMask(spv_target_elw elw,
  ...
  for (uint32_t candidate_bit = (1u << 31u); candidate_bit;
    ...
    if (candidate_bit & mask) {
      ...
      if (SPV_SUCCESS == spvOperandTableValueLookup(elw, operandTable, type,
                                                    candidate_bit, &entry)) {
...
std::function<bool(unsigned)> spvDbgInfoExtOperandCanBeForwardDeclaredFunction(
  ...
  // references for debug info instructions are still in dislwssion. We must
