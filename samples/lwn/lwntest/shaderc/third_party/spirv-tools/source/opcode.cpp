...
spv_result_t spvOpcodeTableGet(spv_opcode_table* pInstTable, spv_target_elw) {
  if (!pInstTable) return SPV_ERROR_ILWALID_POINTER;
...
spv_result_t spvOpcodeTableNameLookup(spv_target_elw elw,
...
spv_result_t spvOpcodeTableValueLookup(spv_target_elw elw,
                                       const spv_opcode_table table,
                                       const SpvOp opcode,
                                       spv_opcode_desc* pEntry) {
  if (!table) return SPV_ERROR_ILWALID_TABLE;
  ...
  // opcode value, and they can be introduced in different target elwironments,
...
int32_t spvOpcodeGeneratesType(SpvOp op) {
  switch (op) {
    ...
    case SpvOpTypeAccelerationStructureLW:
    case SpvOpTypeCooperativeMatrixLW:
...
bool spvOpcodeIsNonUniformGroupOperation(SpvOp opcode) {
  switch (opcode) {
    ...
    case SpvOpGroupNonUniformIlwerseBallot:
...
bool spvOpcodeIsScalarizable(SpvOp opcode) {
  switch (opcode) {
    case SpvOpPhi:
    case SpvOpCopyObject:
    case SpvOpColwertFToU:
    case SpvOpColwertFToS:
    case SpvOpColwertSToF:
    case SpvOpColwertUToF:
    case SpvOpUColwert:
    case SpvOpSColwert:
    case SpvOpFColwert:
