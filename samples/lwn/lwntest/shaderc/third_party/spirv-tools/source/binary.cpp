...
spv_result_t spvBinaryHeaderGet(const spv_const_binary binary,
  ...
  if (!binary->code) return SPV_ERROR_ILWALID_BINARY;
  if (binary->wordCount < SPV_INDEX_INSTRUCTION)
    return SPV_ERROR_ILWALID_BINARY;
  if (!pHeader) return SPV_ERROR_ILWALID_POINTER;
...
class Parser {
 ...
 private:
  ...
  // endian_colwerted_inst_words parameter is ignored.  Otherwise, this method
  // appends the words for this operand, colwerted to host native endianness,
  // to the end of endian_colwerted_inst_words.  This method also updates the
  ...
  spv_result_t parseOperand(size_t inst_offset, spv_parsed_instruction_t* inst,
                            ...
                            std::vector<uint32_t>* endian_colwerted_inst_words,
  ...
  struct State {
    ...
    // endianness?
    bool requires_endian_colwersion;
    ...
    std::vector<spv_parsed_operand_t> operands;
    std::vector<uint32_t> endian_colwerted_words;
    spv_operand_pattern_t expected_operands;
  } _;
};
...
spv_result_t Parser::parseModule() {
  ...
  _.requires_endian_colwersion = !spvIsHostEndian(_.endian);
...
spv_result_t Parser::parseInstruction() {
  ...
  // then colwerted_words contains the the endian-translated words in the
  // instruction.
  _.endian_colwerted_words.clear();
  _.endian_colwerted_words.push_back(first_word);
  ...
  // the list as we parse the operands. This oclwrs when an operand
  // has its own logical operands (such as the LocalSize operand for
  // ExelwtionMode), or for extended instructions that may have their
  ...
  while (_.word_index < inst_offset + inst_word_count) {
    ...
    if (auto error =
            parseOperand(inst_offset, &inst, type, &_.endian_colwerted_words,
  ...
  // Check the computed length of the endian-colwerted words vector against
  // the declared number of words in the instruction.  If endian colwersion
  // is required, then they should match.  If no endian colwersion was
  // performed, then the vector only contains the initial opcode/word-count
  // word.
  assert(!_.requires_endian_colwersion ||
         (inst_word_count == _.endian_colwerted_words.size()));
  assert(_.requires_endian_colwersion ||
         (_.endian_colwerted_words.size() == 1));

  recordNumberType(inst_offset, &inst);

  if (_.requires_endian_colwersion) {
    // We must wait until here to set this pointer, because the vector might
    // have been be resized while we aclwmulated its elements.
    inst.words = _.endian_colwerted_words.data();
  } else {
    // If no colwersion is required, then just point to the underlying binary.
...
spv_result_t Parser::parseOperand(size_t inst_offset,
  ...
  // Do the words in this operand have to be colwerted to native endianness?
  // True for all but literal strings.
  bool colwert_operand_endianness = true;

  switch (type) {
    case SPV_OPERAND_TYPE_TYPE_ID:
      if (!word)
        return diagnostic(SPV_ERROR_ILWALID_ID) << "Error: Type Id is 0";
      inst->type_id = word;
      break;

    case SPV_OPERAND_TYPE_RESULT_ID:
      if (!word)
        return diagnostic(SPV_ERROR_ILWALID_ID) << "Error: Result Id is 0";
      inst->result_id = word;
      ...
      if (_.id_to_type_id.find(inst->result_id) != _.id_to_type_id.end())
        return diagnostic(SPV_ERROR_ILWALID_ID)
    ...
    case SPV_OPERAND_TYPE_OPTIONAL_ID:
      if (!word) return diagnostic(SPV_ERROR_ILWALID_ID) << "Id is 0";
    ...
    case SPV_OPERAND_TYPE_OPTIONAL_LITERAL_STRING: {
      colwert_operand_endianness = false;
      const char* string =
      ...
      //
      // This error can't lwrrently be triggered because of validity
      // checks elsewhere.
    ...
    case SPV_OPERAND_TYPE_EXELWTION_MODEL:
    case SPV_OPERAND_TYPE_ADDRESSING_MODEL:
    case SPV_OPERAND_TYPE_MEMORY_MODEL:
    case SPV_OPERAND_TYPE_EXELWTION_MODE:
