...
class CFA {
 ...
 public:
  ...
  /// @brief Callwlates dominator edges for a set of blocks
  ///
  ...
  ///
  /// This function callwlates the dominator edges for a set of blocks in the
  /// CFG.
  ...
  /// dominator.
  static std::vector<std::pair<BB*, BB*>> CallwlateDominators(
...
template <class BB>
std::vector<std::pair<BB*, BB*>> CFA<BB>::CallwlateDominators(
    const std::vector<cbb_ptr>& postorder, get_blocks_func predecessor_func) {
  struct block_detail {
  ...
  for (auto idom : idoms) {
    // NOTE: performing a const cast for colwenient usage with
...
void CFA<BB>::ComputeAugmentedCFG(
  ...
  // will affect the post-dominance callwlation as follows:
