...
class AssemblyContext {
 public:
  ...
  DiagnosticStream diagnostic(spv_result_t error) {
    return DiagnosticStream(lwrrent_position_, consumer_, "", error);
  }
  ...
  DiagnosticStream diagnostic() {
    ...
    return diagnostic(SPV_ERROR_ILWALID_TEXT);
  }
  ...
  void setPosition(const spv_position_t& newPosition) {
    lwrrent_position_ = newPosition;
  }

  // Returns the current position in the input stream.
  const spv_position_t& position() const { return lwrrent_position_; }
  ...
  // is called.
  // Returns SPV_SUCCESS on success, or SPV_ERROR_ILWALID_VALUE on error.
 ...
 private:
  ...
  spv_position_t lwrrent_position_;
