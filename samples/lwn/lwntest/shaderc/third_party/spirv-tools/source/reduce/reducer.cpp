...
Reducer::Reducer(spv_target_elw target_elw) : target_elw_(target_elw) {}
...
Reducer::ReductionResultStatus Reducer::Run(
  ...
  std::vector<uint32_t> lwrrent_binary(std::move(binary_in));

  spvtools::SpirvTools tools(target_elw_);
  ...
  if (!tools.Validate(&lwrrent_binary[0], lwrrent_binary.size(),
                      validator_options)) {
    consumer_(SPV_MSG_INFO, nullptr, {},
              "Initial binary is invalid; stopping.");
    return Reducer::ReductionResultStatus::kInitialStateIlwalid;
  }

  // Initial state should be interesting.
  if (!interestingness_function_(lwrrent_binary, reductions_applied)) {
    consumer_(SPV_MSG_INFO, nullptr, {},
  ...
  *binary_out = std::move(lwrrent_binary);
...
void Reducer::AddReductionPass(
  ...
  passes_.push_back(
      spvtools::MakeUnique<ReductionPass>(target_elw_, std::move(finder)));
}
...
bool Reducer::ReachedStepLimit(uint32_t lwrrent_step,
                               spv_const_reducer_options options) {
  return lwrrent_step >= options->step_limit;
}
...
Reducer::ReductionResultStatus Reducer::RunPasses(
    ...
    const SpirvTools& tools, std::vector<uint32_t>* lwrrent_binary,
  ...
  while (!ReachedStepLimit(*reductions_applied, options) &&
    ...
    for (auto& pass : *passes) {
      ...
      do {
        auto maybe_result = pass->TryApplyReduction(*lwrrent_binary);
        ...
        if (!tools.Validate(&maybe_result[0], maybe_result.size(),
          ...
          if (options->fail_on_validation_error) {
            ...
            *lwrrent_binary = std::move(maybe_result);
            return Reducer::ReductionResultStatus::kStateIlwalid;
          }
