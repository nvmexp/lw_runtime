...
void StructuredLoopToSelectionReductionOpportunity::Apply() {
  ...
  // were performed.
  context_->IlwalidateAnalysesExceptFor(IRContext::Analysis::kAnalysisNone);
  ...
  // Ilwalidate the analyses we just used.
  context_->IlwalidateAnalysesExceptFor(IRContext::Analysis::kAnalysisNone);
}
...
void StructuredLoopToSelectionReductionOpportunity::RedirectEdge(
  ...
  // source_id->new_target_id, where the blocks ilwolved are all different.
          ...
          if (def.opcode() == SpvOpAccessChain) {
            ...
            switch (pointer_type->storage_class()) {
              case SpvStorageClassFunction:
                use->SetOperand(
                    index, {FindOrCreateFunctiolwariable(
...
uint32_t
StructuredLoopToSelectionReductionOpportunity::FindOrCreateFunctiolwariable(
    uint32_t pointer_type_id) {
