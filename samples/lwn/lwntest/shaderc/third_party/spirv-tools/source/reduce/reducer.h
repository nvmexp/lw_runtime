...
class Reducer {
 public:
  ...
  // Constructs an instance with the given target |target_elw|, which is used to
  ...
  explicit Reducer(spv_target_elw target_elw);
 ...
 private:
  static bool ReachedStepLimit(uint32_t lwrrent_step,
                               spv_const_reducer_options options);
  ...
  ReductionResultStatus RunPasses(
      ...
      std::vector<uint32_t>* lwrrent_binary, uint32_t* reductions_applied);

  const spv_target_elw target_elw_;
