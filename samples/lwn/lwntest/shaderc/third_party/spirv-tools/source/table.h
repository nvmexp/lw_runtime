...
typedef struct spv_opcode_desc_t {
  ...
  // value is in core and its availability is subject to milwersion. The
...
struct spv_context_t {
  const spv_target_elw target_elw;
...
// Populates *table with entries for elw.
spv_result_t spvOpcodeTableGet(spv_opcode_table* table, spv_target_elw elw);

// Populates *table with entries for elw.
spv_result_t spvOperandTableGet(spv_operand_table* table, spv_target_elw elw);

// Populates *table with entries for elw.
spv_result_t spvExtInstTableGet(spv_ext_inst_table* table, spv_target_elw elw);
