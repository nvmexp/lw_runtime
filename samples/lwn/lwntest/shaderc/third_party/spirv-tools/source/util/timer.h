...
class Timer {
 public:
  ...
  // exelwtion. If kClockGettimeCPUtimeFailed is set by the failure of calling
  ...
  // exelwtion. If kClockGettimeWalltimeFailed is set by the failure of
  ...
  // Returns the measured USR Time for a range of code exelwtion. If
  ...
  // Returns the measured SYS Time for a range of code exelwtion. If
  // kGetrusageFailed is set because of the failure of calling getrusage(), it
  // returns -1.
  ...
  // Returns the measured RSS delta for a range of code exelwtion. If
  // kGetrusageFailed is set because of the failure of calling getrusage(), it
  // returns -1.
  ...
  // code exelwtion. If kGetrusageFailed is set because of the failure of
...
//              the resource utilization to std::cout.
//
// The template<class TimerType> is used to choose a Timer class. Lwrrently,
...
// LwmulativeTimer is the same as Timer class, but it supports a cumulative
// measurement as the following example:
//
//   LwmulativeTimer *ctimer = new LwmulativeTimer(std::cout);
//   ctimer->Start();
//
//   /* ... lines of code that we want to know its resource usage ... */
//
...
//   delete ctimer;
//
class LwmulativeTimer : public Timer {
 public:
  LwmulativeTimer(std::ostream* out, bool measure_mem_usage = false)
  ...
  // Returns the cumulative USR Time for a range of code exelwtion.
  double UserTime() override { return usr_time_; }

  // Returns the cumulative SYS Time for a range of code exelwtion.
  double SystemTime() override { return sys_time_; }

  // Returns the cumulative RSS delta for a range of code exelwtion.
