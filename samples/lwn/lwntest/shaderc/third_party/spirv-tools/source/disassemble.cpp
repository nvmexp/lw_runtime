...
// This file contains a disassembler:  It colwerts a SPIR-V binary
// to text.
...
// A Disassembler instance colwerts a SPIR-V binary to its assembly
// representation.
class Disassembler {
 public:
  ...
  // If not printing, populates text_result with the aclwmulated text.
...
spv_result_t spvBinaryToText(const spv_const_context context,
  ...
  if (!grammar.isValid()) return SPV_ERROR_ILWALID_TABLE;
...
std::string spvtools::spvInstructionBinaryToText(const spv_target_elw elw,
                                                 const uint32_t* instCode,
                                                 const size_t instWordCount,
                                                 const uint32_t* code,
                                                 const size_t wordCount,
                                                 const uint32_t options) {
  spv_context context = spvContextCreate(elw);
