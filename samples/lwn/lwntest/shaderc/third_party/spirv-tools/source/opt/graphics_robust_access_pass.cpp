...
//        exelwtion of a shader instruction) is the result of an OpVariable.
//
...
//    conselwtive locations are fully determined by the pointer.
//
...
//      variable, output variable. The exelwtion environment is responsible for
...
//      indices contributing to address callwlations.  As noted above, the
...
spvtools::DiagnosticStream GraphicsRobustAccessPass::Fail() {
  ...
  return std::move(
      spvtools::DiagnosticStream({}, consumer(), "", SPV_ERROR_ILWALID_BINARY)
      << name() << ": ");
}
...
spv_result_t GraphicsRobustAccessPass::ProcessLwrrentModule() {
...
void GraphicsRobustAccessPass::ClampIndicesForAccessChain(
  ...
  auto clamp_to_literal_count =
    ...
    if (auto* index_constant = constant_mgr->GetConstantFromInst(index_inst)) {
      ...
      if (maxval_width > index_width) {
        ...
        // From how we callwlated maxval_width, widening won't require adding
  ...
  auto clamp_to_count = [&inst, this, &constant_mgr, &clamp_to_literal_count,
    ...
    if (const auto* count_constant =
      ...
      // UColwert requires the result type to have 0 signedness.  So enforce
      // that here.
      ...
      // That ensures we satisfy the ilwariant for SClamp, where the "min"
...
opt::Instruction* opt::GraphicsRobustAccessPass::WidenInteger(
  ...
  auto colwersion_id = TakeNextId();
  auto* colwersion = InsertInst(
      before_inst, (sign_extend ? SpvOpSColwert : SpvOpUColwert), type_id,
      colwersion_id, {{SPV_OPERAND_TYPE_ID, {value->result_id()}}});
  return colwersion;
}
...
Instruction* GraphicsRobustAccessPass::MakeRuntimeArrayLengthInst(
  ...
  // Walk backward through pointer address callwlations until we either get
  ...
  // struct.
  Instruction* lwrrent_access_chain = access_chain;
  Instruction* pointer_to_containing_struct = nullptr;
  while (steps_remaining > 0) {
    switch (lwrrent_access_chain->opcode()) {
      case SpvOpCopyObject:
        // Whoops. Walk right through this one.
        lwrrent_access_chain =
            GetDef(lwrrent_access_chain->GetSingleWordInOperand(0));
        break;
      case SpvOpAccessChain:
      case SpvOpInBoundsAccessChain: {
        ...
        const auto num_contributing_indices =
            lwrrent_access_chain == access_chain
                ? operand_index - (first_index_operand - 1)
                : lwrrent_access_chain->NumInOperands() - 1 /* skip the base */;
        Instruction* base =
            GetDef(lwrrent_access_chain->GetSingleWordInOperand(0));
        if (num_contributing_indices == steps_remaining) {
          ...
          auto* new_access_chain =
              InsertInst(lwrrent_access_chain, lwrrent_access_chain->opcode(),
      ...
      default:
        Fail() << "Unhandled access chain in logical addressing mode passes "
                  "through "
               << lwrrent_access_chain->PrettyPrint(
...
spv_result_t GraphicsRobustAccessPass::ClampCoordinateForImageTexelPointer(
  ...
  //       index from 0 to 5).
  // For arrayed images (in Vulkan the Dim is 1D, 2D, or Lwbe):
  //   non-Lwbe:
  ...
  //     - The third component in the texel pointer callwlation is
  //       6 * layer + face, where 0 <= face < 6.
  //   Lwbe: Use all but the last component of the coordinate (which is the face
  //   index from 0 to 5).
  ...
  const auto query_num_components = [dim, arrayed, this]() -> int {
    ...
    switch (dim) {
      ...
      case SpvDim1D:
        ...
        break;
      case SpvDimLwbe:
        // For lwbe, we need bounds for x, y, but not face.
      case SpvDimRect:
      case SpvDim2D:
        num_coords = 2;
        break;
  ...
  // If the image is a lwbe array, then the last component of the queried
