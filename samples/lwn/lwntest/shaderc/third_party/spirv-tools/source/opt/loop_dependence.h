...
class LoopDependenceAnalysis {
 ...
 private:
  ...
  // When c1 and c2 are loop ilwariant and a is constant
  // distance = (c1 - c2)/a
  //              < if distance > 0
  ...
  // where c1 and c2 are loop ilwariant and a is constant.
  ...
  // distance = (c1 - c2) / a2
  // Returns true if independence is proven and false if it can't be proven.
  bool WeakZeroSourceSIVTest(SENode* source, SERelwrrentNode* destination,
                             SENode* coefficient,
                             DistanceEntry* distance_entry);
  ...
  // distance = (c2 - c1) / a1
  // Returns true if independence is proven and false if it can't be proven.
  bool WeakZeroDestinationSIVTest(SERelwrrentNode* source, SENode* destination,
                                  SENode* coefficient,
                                  DistanceEntry* distance_entry);
  ...
  int64_t CountInductiolwariables(SENode* node);

  // Finds the number of induction variables shared between |source| and
  // |destination|.
  // Returns -1 on failure.
  int64_t CountInductiolwariables(SENode* source, SENode* destination);
  ...
  SENode* GetConstantTerm(const Loop* loop, SERelwrrentNode* induction);
  ...
  // Colwerts |value| to a std::string and returns the result.
