...
//
// 1- Initially, all edges of the CFG are marked not exelwtable and the CFG
...
//             as exelwtable so they are traversed one more time.  Eventually
...
class SSAPropagator {
 public:
  ...
  // has been marked exelwtable. |i| should be an index value accepted by
  // Instruction::GetSingleWordOperand.
  bool IsPhiArgExelwtable(Instruction* phi, uint32_t i) const;
 ...
 private:
  ...
  // Simulate the exelwtion |block| by calling |visit_fn_| on every instruction
  // in it.
  bool Simulate(BasicBlock* block);

  // Simulate the exelwtion of |instr| by replacing all the known values in
  // every operand and determining whether the result is interesting for
  // propagation. This ilwokes the callback function |visit_fn_| to determine
  ...
  // Marks |edge| as exelwtable.  Returns false if the edge was already marked
  // as exelwtable.
  bool MarkEdgeExelwtable(const Edge& edge) {
    return exelwtable_edges_.insert(edge).second;
  }

  // Returns true if |edge| has been marked as exelwtable.
  bool IsEdgeExelwtable(const Edge& edge) const {
    return exelwtable_edges_.find(edge) != exelwtable_edges_.end();
  }
  ...
  // If the CFG edge |e| has not been exelwted, this function adds |e|'s
  ...
  // Set of exelwtable CFG edges.
  std::set<Edge> exelwtable_edges_;
