...
SSAPropagator::PropStatus CCPPass::MarkInstructiolwarying(Instruction* instr) {
...
SSAPropagator::PropStatus CCPPass::VisitPhi(Instruction* phi) {
  ...
  // exelwtable edges yields the same constant value.
  for (uint32_t i = 2; i < phi->NumOperands(); i += 2) {
    if (!propagator_->IsPhiArgExelwtable(phi, i)) {
      // Ignore arguments coming through non-exelwtable edges.
      continue;
    }
    ...
    if (it != values_.end()) {
      ...
      if (it->second == kVaryingSSAId) {
        ...
        return MarkInstructiolwarying(phi);
  ...
  // If there are no incoming exelwtable edges, the meet ID will still be 0. In
  ...
  if (instr->opcode() == SpvOpCopyObject) {
    ...
    if (it != values_.end()) {
      if (IsVaryingValue(it->second)) {
        return MarkInstructiolwarying(instr);
      } else {
