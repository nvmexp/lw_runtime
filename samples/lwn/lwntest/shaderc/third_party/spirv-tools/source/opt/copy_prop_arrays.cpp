...
CopyPropagateArrays::BuildMemoryObjectFromLoad(Instruction* load_inst) {
  ...
  Instruction* lwrrent_inst = def_use_mgr->GetDef(
  ...
  while (lwrrent_inst->opcode() == SpvOpAccessChain) {
    for (uint32_t i = lwrrent_inst->NumInOperands() - 1; i >= 1; --i) {
      uint32_t element_index_id = lwrrent_inst->GetSingleWordInOperand(i);
      components_in_reverse.push_back(element_index_id);
    }
    lwrrent_inst = def_use_mgr->GetDef(lwrrent_inst->GetSingleWordInOperand(0));
  }
  ...
  // exactly.
  if (lwrrent_inst->opcode() != SpvOpVariable) {
    return nullptr;
  }
  ...
  return std::unique_ptr<CopyPropagateArrays::MemoryObject>(
      new MemoryObject(lwrrent_inst, components_in_reverse.rbegin(),
...
CopyPropagateArrays::BuildMemoryObjectFromExtract(Instruction* extract_inst) {
  ...
  if (result) {
    ...
    // Colwert the indices in the extract instruction to a series of ids that
...
CopyPropagateArrays::BuildMemoryObjectFromInsert(Instruction* insert_inst) {
  ...
  Instruction* lwrrent_insert =
      def_use_mgr->GetDef(insert_inst->GetSingleWordInOperand(1));
  for (uint32_t i = number_of_elements - 1; i > 0; --i) {
    if (lwrrent_insert->opcode() != SpvOpCompositeInsert) {
      return nullptr;
    }

    if (lwrrent_insert->NumInOperands() != 3) {
      return nullptr;
    }

    if (lwrrent_insert->GetSingleWordInOperand(2) != i - 1) {
      return nullptr;
    }

    std::unique_ptr<MemoryObject> lwrrent_memory_object =
        GetSourceObjectIfAny(lwrrent_insert->GetSingleWordInOperand(0));

    if (!lwrrent_memory_object) {
      return nullptr;
    }

    if (!lwrrent_memory_object->IsMember()) {
      return nullptr;
    }

    if (memory_object->AccessChain().size() + 1 !=
        lwrrent_memory_object->AccessChain().size()) {
      return nullptr;
    }

    if (!memory_object->Contains(lwrrent_memory_object.get())) {
      return nullptr;
    }

    const analysis::Constant* lwrrent_last_access =
        const_mgr->FindDeclaredConstant(
            lwrrent_memory_object->AccessChain().back());
    if (!lwrrent_last_access || !lwrrent_last_access->type()->AsInteger()) {
      return nullptr;
    }

    if (lwrrent_last_access->GetU32() != i - 1) {
      return nullptr;
    }
    lwrrent_insert =
        def_use_mgr->GetDef(lwrrent_insert->GetSingleWordInOperand(1));
  }
  ...
  for (auto pair : uses) {
    ...
    switch (use->opcode()) {
      ...
      case SpvOpAccessChain: {
        ...
        // Colwert the ids on the OpAccessChain to indices that can be used to
