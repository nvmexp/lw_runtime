...
class Instruction : public utils::IntrusiveNodeBase<Instruction> {
 public:
  ...
  // Gets the number of SPIR-V words oclwpied by all logical operands.
  ...
  // callwlation.  The instruction must be a load, as defined by |IsLoad|,
  ...
  // Returns true if this instruction causes the function to finish exelwtion
  // and return to its caller
  bool IsReturn() const { return spvOpcodeIsReturn(opcode()); }

  // Returns true if this instruction exits this function or aborts exelwtion.
 ...
 private:
  ...
  // same caveats dolwmented in the comment for IsReadOnlyPointer.  The first
