...
// invalid.
const uint32_t kIlwalidId = std::numeric_limits<uint32_t>::max();
...
class InstructionBuilder {
 public:
  ...
  // merge instruction. If |merge_id| equals kIlwalidId then no selection merge
  ...
  Instruction* AddConditionalBranch(
      ...
      uint32_t merge_id = kIlwalidId,
      uint32_t selection_control = SpvSelectionControlMaskNone) {
    if (merge_id != kIlwalidId) {
