...
class ScalarEvolutionAnalysis {
 public:
  ...
  // Create a new relwrrent node with |offset| and |coefficient|, with respect
  // to |loop|.
  SENode* CreateRelwrrentExpression(const Loop* loop, SENode* offset,
                                    SENode* coefficient);

  // Construct the DAG by traversing use def chain of |inst|.
  SENode* AnalyzeInstruction(const Instruction* inst);

  // Simplify the |node| by grouping like terms or if contains a relwrrent
  // expression, rewrite the graph so the whole DAG (from |node| down) is in
  // terms of that relwrrent expression.
  //
  // For example.
  ...
  // Checks that the graph starting from |node| is ilwariant to the |loop|.
  bool IsLoopIlwariant(const Loop* loop, const SENode* node) const;
  ...
  // Find the relwrrent term belonging to |loop| in the graph starting from
  // |node| and return the coefficient of that relwrrent term. Constant zero
  // will be returned if no relwrrent could be found. |node| should be in
  // simplest form.
  SENode* GetCoefficientFromRelwrrentTerm(SENode* node, const Loop* loop);

  // Return a rebuilt graph starting from |node| with the relwrrent expression
  // belonging to |loop| being zeroed out. Returned node will be simplified.
  SENode* BuildGraphWithoutRelwrrentTerm(SENode* node, const Loop* loop);

  // Return the relwrrent term belonging to |loop| if it appears in the graph
  // starting at |node| or null if it doesn't.
  SERelwrrentNode* GetRelwrrentTerm(SENode* node, const Loop* loop);
  ...
  // SERelwrrentNode objects. This enables analysing dependencies that will be
 ...
 private:
  ...
  // A map of instructions to SENodes. This is used to track relwrrent
  // expressions as they are added when analyzing instructions. Relwrrent
  // expressions come from phi nodes which by nature can include relwrsion so we
  // check if nodes have already been built when analyzing instructions.
  std::map<const Instruction*, SENode*> relwrrent_node_map_;
