...
// instructions by traversing each instructions users and operands relwrsively.
...
class LoopFissionImpl {
 ...
 private:
  ...
  // relwrsion and also so we can ignore certain instructions by preemptively
...
void LoopFissionImpl::TraverseUseDef(Instruction* inst,
  ...
  traverser_functor = [this, def_use, &inst_set, &traverser_functor,
    ...
    // relwrsion and allows us to ignore certain instructions.
  ...
  // We start the traversal of the use def graph by ilwoking the above
...
bool LoopFissionImpl::CanPerformSplit() {
  ...
  // We need to callwlate the depth of the loop to create the loop dependency
  // distance vectors.
...
Pass::Status LoopFissionPass::Process() {
  ...
  for (Function& f : *context()->module()) {
    ...
    // over each, as creating new loops will ilwalidate the the loop iterator.
    ...
    while (!inner_most_loops.empty()) {
      for (Loop* loop : inner_most_loops) {
        ...
        if (impl.CanPerformSplit()) {
          ...
          changed = true;
          context()->IlwalidateAnalysesExceptFor(
