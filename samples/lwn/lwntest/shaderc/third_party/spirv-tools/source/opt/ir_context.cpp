...
void IRContext::BuildIlwalidAnalyses(IRContext::Analysis set) {
...
void IRContext::IlwalidateAnalysesExceptFor(
    IRContext::Analysis preserved_analyses) {
  uint32_t analyses_to_ilwalidate = valid_analyses_ & (~preserved_analyses);
  IlwalidateAnalyses(static_cast<IRContext::Analysis>(analyses_to_ilwalidate));
}

void IRContext::IlwalidateAnalyses(IRContext::Analysis analyses_to_ilwalidate) {
  // The ConstantManager contains Type pointers. If the TypeManager goes
  // away, the ConstantManager has to go away.
  if (analyses_to_ilwalidate & kAnalysisTypes) {
    analyses_to_ilwalidate |= kAnalysisConstants;
    analyses_to_ilwalidate |= kAnalysisDebugInfo;
  }
  ...
  // dominator analysis should be ilwalidated as well.
  if (analyses_to_ilwalidate & kAnalysisCFG) {
    analyses_to_ilwalidate |= kAnalysisDominatorAnalysis;
  }

  if (analyses_to_ilwalidate & kAnalysisDefUse) {
    def_use_mgr_.reset(nullptr);
  }
  if (analyses_to_ilwalidate & kAnalysisInstrToBlockMapping) {
    instr_to_block_.clear();
  }
  if (analyses_to_ilwalidate & kAnalysisDecorations) {
    decoration_mgr_.reset(nullptr);
  }
  if (analyses_to_ilwalidate & kAnalysisCombinators) {
    combinator_ops_.clear();
  }
  if (analyses_to_ilwalidate & kAnalysisBuiltilwarId) {
    builtin_var_id_map_.clear();
  }
  if (analyses_to_ilwalidate & kAnalysisCFG) {
    cfg_.reset(nullptr);
  }
  if (analyses_to_ilwalidate & kAnalysisDominatorAnalysis) {
    dominator_trees_.clear();
    post_dominator_trees_.clear();
  }
  if (analyses_to_ilwalidate & kAnalysisNameMap) {
    id_to_name_.reset(nullptr);
  }
  if (analyses_to_ilwalidate & kAnalysisValueNumberTable) {
    vn_table_.reset(nullptr);
  }
  if (analyses_to_ilwalidate & kAnalysisStructuredCFG) {
    struct_cfg_analysis_.reset(nullptr);
  }
  if (analyses_to_ilwalidate & kAnalysisIdToFuncMapping) {
    id_to_func_.clear();
  }
  if (analyses_to_ilwalidate & kAnalysisConstants) {
  ...
  if (analyses_to_ilwalidate & kAnalysisDebugInfo) {
    debug_info_mgr_.reset(nullptr);
  }

  valid_analyses_ = Analysis(valid_analyses_ & ~analyses_to_ilwalidate);
}
...
uint32_t IRContext::GetBuiltinInputVarId(uint32_t builtin) {
  if (!AreAnalysesValid(kAnalysisBuiltilwarId)) ResetBuiltinAnalysis();
  // If cached, return it.
  ...
  if (var_id == 0) {
    ...
    get_decoration_mgr()->AddDecoratiolwal(var_id, SpvDecorationBuiltIn,
                                           builtin);
...
bool IRContext::ProcessReachableCallTree(ProcessFunction& pfn) {
  ...
  for (auto& a : annotations()) {
    // TODO: Handle group decorations as well.  Lwrrently not generate by any
