...
class PipeStorage;
class NamedBarrier;
class AccelerationStructureLW;
class CooperativeMatrixLW;
...
class Type {
 public:
  ...
  enum Kind {
    ...
    kPipeStorage,
    kNamedBarrier,
    kAccelerationStructureLW,
    kCooperativeMatrixLW,
  ...
  // Returns true if this type is exactly the same as |that| type, including
  // decorations.  |seen| is the set of |Pointer*| pair that are lwrrently being
  ...
  // Adds necessary extra words for a subtype to callwlate a hash value into
  // |words|.
...
#define DeclareCastMethod(target)                  \
  ...
  DeclareCastMethod(AccelerationStructureLW)
  DeclareCastMethod(CooperativeMatrixLW)
...
class CooperativeMatrixLW : public Type {
 public:
  CooperativeMatrixLW(const Type* type, const uint32_t scope,
                      const uint32_t rows, const uint32_t columns);
  CooperativeMatrixLW(const CooperativeMatrixLW&) = default;

  std::string str() const override;

  CooperativeMatrixLW* AsCooperativeMatrixLW() override { return this; }
  const CooperativeMatrixLW* AsCooperativeMatrixLW() const override {
    return this;
  }
...
DefineParameterlessType(AccelerationStructureLW, accelerationStructureLW);
