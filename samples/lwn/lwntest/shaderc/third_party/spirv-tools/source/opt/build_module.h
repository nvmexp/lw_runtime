...
// decoded according to the given target |elw|. Returns nullptr if errors occur
// and sends the errors to |consumer|.
std::unique_ptr<opt::IRContext> BuildModule(spv_target_elw elw,
...
// target |elw|. Returns nullptr if errors occur and sends the errors to
// |consumer|.
std::unique_ptr<opt::IRContext> BuildModule(
    spv_target_elw elw, MessageConsumer consumer, const std::string& text,
