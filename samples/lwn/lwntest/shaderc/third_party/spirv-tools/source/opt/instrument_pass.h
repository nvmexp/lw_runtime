...
// associated with that validation. Lwrrently one such function,
...
// a validation pass are permanantly assigned and fixed and dolwmented by
...
class InstrumentPass : public Pass {
 ...
 public:
  ...
  IRContext::Analysis GetPreservedAnalyses() override {
    return IRContext::kAnalysisDefUse | IRContext::kAnalysisDecorations |
           ...
           IRContext::kAnalysisBuiltilwarId | IRContext::kAnalysisConstants;
  }
 ...
 protected:
  ...
  // error as well as the values ilwolved in the error.
  //
  ...
  // by the SpvExelwtionModel_ enumeration. This is used to interpret the
  ...
  // FragCoord.xy. Compute shaders will write the GlobalIlwocation ID.
  ...
  //
  // The Validation Error Code specifies the exact error which has oclwrred.
  ...
  // oclwrred. These are dolwmented further in each file containing the
  ...
  uint32_t GetVelwintId(uint32_t length);
  ...
  // its result id.
  uint32_t GelwarLoad(uint32_t var_id, InstructionBuilder* builder);
