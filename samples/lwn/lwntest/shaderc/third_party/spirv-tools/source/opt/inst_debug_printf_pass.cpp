...
void InstDebugPrintfPass::GenOutputValues(Instruction* val_inst,
  ...
  switch (val_ty->kind()) {
    ...
    case analysis::Type::kFloat: {
      ...
      switch (f_ty->width()) {
        case 16: {
          // Colwert float16 to float32 and relwrse
          Instruction* f32_inst = builder->AddUnaryOp(
              GetFloatId(), SpvOpFColwert, val_inst->result_id());
          GenOutputValues(f32_inst, val_ids, builder);
          return;
        }
        case 64: {
          // Bitcast float64 to uint64 and relwrse
      ...
      switch (i_ty->width()) {
        case 64: {
          ...
          Instruction* lo_ui64_inst = builder->AddUnaryOp(
              GetUintId(), SpvOpUColwert, ui64_inst->result_id());
          ...
          Instruction* hi_ui64_inst = builder->AddUnaryOp(
              GetUintId(), SpvOpUColwert, rshift_ui64_inst->result_id());
        ...
        case 8: {
          ...
          // Colwert uint8 to uint32
          Instruction* ui32_inst = builder->AddUnaryOp(
              GetUintId(), SpvOpUColwert, ui8_inst->result_id());
...
void InstDebugPrintfPass::GenOutputCode(
  ...
  // component values. float16 will need to be colwerted to float32. Pointer
  // and uint64 will need to be colwerted to two uint32 values. float32 will
