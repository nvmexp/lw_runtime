...
bool LoopFusion::AreCompatible() {
  ...
  loop_1_->GetInductiolwariables(inductions_1);
...
bool LoopFusion::CheckStep() {
  ...
  if (!induction_node_0->AsSERelwrrentNode()) {
    return false;
  }

  SENode* induction_step_0 =
      induction_node_0->AsSERelwrrentNode()->GetCoefficient();
  ...
  if (!induction_node_1->AsSERelwrrentNode()) {
    return false;
  }

  SENode* induction_step_1 =
      induction_node_1->AsSERelwrrentNode()->GetCoefficient();
...
bool LoopFusion::IsLegal() {
  ...
  loop_0_->GetInductiolwariables(phi_instructions);
  ...
  // Find the parents.
  for (auto lwrrent_loop = loop_0_; lwrrent_loop != nullptr;
       lwrrent_loop = lwrrent_loop->GetParent()) {
    loops.push_back(lwrrent_loop);
  }
...
void LoopFusion::Fuse() {
  ...
  // Update the CFG, so it wouldn't need ilwalidating.
  ...
  // Update the LoopDescriptor, so it wouldn't need ilwalidating.
  auto ld = context_->GetLoopDescriptor(containing_function_);

  // Create a copy, so the iterator wouldn't be ilwalidated.
