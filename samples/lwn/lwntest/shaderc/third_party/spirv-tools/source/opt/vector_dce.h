...
class VectorDCE : public MemPass {
 ...
 private:
  ...
  // Rewrites the OpCompositeInsert instruction |lwrrent_inst| to avoid
  // unnecessary computes given that the only components of the result that are
  // live are |live_components|.
  //
  // If the value being inserted is not live, then the result of |lwrrent_inst|
  // is replaced by the composite input to |lwrrent_inst|.
  //
  // If the composite input to |lwrrent_inst| is not live, then it is replaced
  // by and OpUndef in |lwrrent_inst|.
  bool RewriteInsertInstruction(Instruction* lwrrent_inst,
  ...
  // Marks the components |live_elements| of the uses in |lwrrent_inst| as live
  // according to |live_components|. If they were not live before, then they are
  // added to |work_list|.
  void MarkUsesAsLive(Instruction* lwrrent_inst,
                      const utils::BitVector& live_elements,
                      LiveComponentMap* live_components,
                      std::vector<WorkListItem>* work_list);

  // Marks the uses in the OpVectorShuffle instruction in |lwrrent_item| as live
  // based on the live components in |lwrrent_item|. If anything becomes live
  // they are added to |work_list| and |live_components| is updated
  // accordingly.
  void MarkVectorShuffleUsesAsLive(const WorkListItem& lwrrent_item,
                                   VectorDCE::LiveComponentMap* live_components,
                                   std::vector<WorkListItem>* work_list);

  // Marks the uses in the OpCompositeInsert instruction in |lwrrent_item| as
  // live based on the live components in |lwrrent_item|. If anything becomes
  // live they are added to |work_list| and |live_components| is updated
  // accordingly.
  void MarkInsertUsesAsLive(const WorkListItem& lwrrent_item,
                            LiveComponentMap* live_components,
                            std::vector<WorkListItem>* work_list);

  // Marks the uses in the OpCompositeExtract instruction |lwrrent_inst| as
  // live. If anything becomes live they are added to |work_list| and
  // |live_components| is updated accordingly.
  void MarkExtractUseAsLive(const Instruction* lwrrent_inst,
                            const utils::BitVector& live_elements,
                            LiveComponentMap* live_components,
                            std::vector<WorkListItem>* work_list);

  // Marks the uses in the OpCompositeConstruct instruction |lwrrent_inst| as
  // live. If anything becomes live they are added to |work_list| and
  // |live_components| is updated accordingly.
