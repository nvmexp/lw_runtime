...
#ifndef SOURCE_OPT_REPLACE_ILWALID_OPC_H_
#define SOURCE_OPT_REPLACE_ILWALID_OPC_H_
...
class ReplaceIlwalidOpcodePass : public Pass {
 public:
  const char* name() const override { return "replace-invalid-opcode"; }
  Status Process() override;

 private:
  // Returns the exelwtion model that is used by every entry point in the
  // module. If more than one exelwtion model is used in the module, then the
  // return value is SpvExelwtionModelMax.
  SpvExelwtionModel GetExelwtionModel();

  // Replaces all instructions in |function| that are invalid with exelwtion
  // model |mode|, but valid for another shader model, with a special constant
  // value.  See |GetSpecialConstant|.
  bool RewriteFunction(Function* function, SpvExelwtionModel mode);
...
#endif  // SOURCE_OPT_REPLACE_ILWALID_OPC_H_
