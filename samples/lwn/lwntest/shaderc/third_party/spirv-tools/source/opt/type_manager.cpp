...
#define DefineParameterlessCase(kind)                                     \
  case Type::k##kind:                                                     \
    ...
    DefineParameterlessCase(AccelerationStructureLW);
#undef DefineParameterlessCase
    ...
    case Type::kOpaque: {
      ...
      // Colwert to null-terminated packed UTF-8 string.
    ...
    case Type::kForwardPointer:
      ...
      break;
    case Type::kCooperativeMatrixLW: {
      auto coop_mat = type->AsCooperativeMatrixLW();
      ...
      typeInst = MakeUnique<Instruction>(
          context(), SpvOpTypeCooperativeMatrixLW, 0, id,
          std::initializer_list<Operand>{
...
#define DefineNoSubtypeCase(kind)             \
  case Type::k##kind:                         \
    ...
    DefineNoSubtypeCase(AccelerationStructureLW);
#undef DefineNoSubtypeCase
    ...
    case Type::kCooperativeMatrixLW: {
      const CooperativeMatrixLW* cm_type = type.AsCooperativeMatrixLW();
      const Type* component_type = cm_type->component_type();
      rebuilt_ty = MakeUnique<CooperativeMatrixLW>(
...
Type* TypeManager::RecordIfTypeDefinition(const Instruction& inst) {
  ...
  switch (inst.opcode()) {
    ...
    case SpvOpTypeAccelerationStructureLW:
      type = new AccelerationStructureLW();
      break;
    case SpvOpTypeCooperativeMatrixLW:
      type = new CooperativeMatrixLW(GetType(inst.GetSingleWordInOperand(0)),
                                     inst.GetSingleWordInOperand(1),
