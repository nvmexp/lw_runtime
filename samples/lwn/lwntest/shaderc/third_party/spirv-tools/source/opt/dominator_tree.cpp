...
// Callwlates the dominator or postdominator tree for a given function.
...
// callwlated in step 1 to traverse the tree.
// 3 - Pass the list callwlated in step 2 to the CFA::CallwlateDominators using
...
// 5 - Using the tree from 4, perform a depth first traversal to callwlate the
...
//
// When computing the post-dominator tree, all edges are ilwerted. So successors
...
class BasicBlockSuccessorHelper {
 ...
 public:
  ...
  // CFA::CallwlateDominators requires std::vector<BasicBlock*>.
 ...
 private:
  ...
  // If |ilwert_graph_| is true, all edges are reversed (successors becomes
  // predecessors and vice versa).
  // For colwenience, the start of the graph is |dummy_start_node|.
...
BasicBlockSuccessorHelper<BBType>::BasicBlockSuccessorHelper(
    Function& func, const BasicBlock* dummy_start_node, bool ilwert)
    : ilwert_graph_(ilwert) {
...
void BasicBlockSuccessorHelper<BBType>::CreateSuccessorMap(
  ...
  if (ilwert_graph_) {
    // For the post dominator tree, we see the ilwerted graph.
    // successors_ in the ilwerted graph are the predecessors in the CFG.
    ...
    // OpReturn or OpReturlwalue as terminator instruction.
    for (BasicBlock& bb : f) {
      if (bb.hasSuccessor()) {
        ...
        const_bb.ForEachSuccessorLabel(
            [this, &pred_list, &bb,
             &GetSuccessorBasicBlock](const uint32_t successor_id) {
              ...
              // Ilwerted graph: our successors in the CFG
              // are our predecessors in the ilwerted graph.
...
void DominatorTree::GetDominatorEdges(
  ...
  DepthFirstSearchPostOrder(dummy_start_node, successor_functor,
                            postorder_function);
  *edges = CFA<BasicBlock>::CallwlateDominators(postorder, predecessor_functor);
}
