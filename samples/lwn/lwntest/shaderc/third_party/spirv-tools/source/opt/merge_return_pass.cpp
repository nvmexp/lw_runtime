...
bool MergeReturnPass::ProcessStructured(
  ...
  for (auto block : order) {
    ...
    if (blockId == LwrrentState().LwrrentMergeId()) {
  ...
  // Ilwalidate it at this point to make sure it will be rebuilt.
...
void MergeReturnPass::ProcessStructuredBlock(BasicBlock* block) {
  ...
  if (tail_opcode == SpvOpReturn || tail_opcode == SpvOpReturlwalue) {
    if (!return_flag_) {
      AddReturnFlag();
    }
  }
  ...
  if (tail_opcode == SpvOpReturn || tail_opcode == SpvOpReturlwalue ||
    ...
    assert(LwrrentState().InBreakable() && "Should be in the dummy construct.");
    BranchToBlock(block, LwrrentState().BreakMergeId());
...
void MergeReturnPass::BranchToBlock(BasicBlock* block, uint32_t target) {
  if (block->tail()->opcode() == SpvOpReturn ||
      block->tail()->opcode() == SpvOpReturlwalue) {
    RecordReturned(block);
    RecordReturlwalue(block);
  }
...
bool MergeReturnPass::PredicateBlocks(
  ...
  if (block->id() == state->LwrrentMergeId()) {
    state++;
...
bool MergeReturnPass::BreakFromConstruct(
  ...
  context()->BuildIlwalidAnalyses(IRContext::kAnalysisCFG);
  ...
  // subgraph. This can act relwrsively.
...
void MergeReturnPass::RecordReturlwalue(BasicBlock* block) {
  auto terminator = *block->tail();
  if (terminator.opcode() != SpvOpReturlwalue) {
    return;
  }
...
void MergeReturnPass::AddReturlwalue() {
  ...
  std::unique_ptr<Instruction> returlwalue(new Instruction(
      context(), SpvOpVariable, return_ptr_type, var_id,
      std::initializer_list<Operand>{
          {SPV_OPERAND_TYPE_STORAGE_CLASS, {SpvStorageClassFunction}}}));

  auto insert_iter = function_->begin()->begin();
  insert_iter.InsertBefore(std::move(returlwalue));
...
std::vector<BasicBlock*> MergeReturnPass::CollectReturnBlocks(
  ...
  for (auto& block : *function) {
    ...
    if (terminator.opcode() == SpvOpReturn ||
        terminator.opcode() == SpvOpReturlwalue) {
...
void MergeReturnPass::MergeReturnBlocks(
  ...
  for (auto block : return_blocks) {
    if (block->tail()->opcode() == SpvOpReturlwalue) {
      phi_ops.push_back(
  ...
  if (!phi_ops.empty()) {
    ...
    std::unique_ptr<Instruction> return_inst(
        new Instruction(context(), SpvOpReturlwalue, 0u, 0u,
...
void MergeReturnPass::AddNewPhiNodes(BasicBlock* bb) {
  ...
  BasicBlock* lwrrent_bb = context()->get_instr_block(original_dominator_[bb]);
  while (lwrrent_bb != nullptr && lwrrent_bb != dominator) {
    for (Instruction& inst : *lwrrent_bb) {
      CreatePhiNodesForInst(bb, inst);
    }
    lwrrent_bb = dom_tree->ImmediateDominator(lwrrent_bb);
  }
}
