...
#define DeclareKindCase(kind)                   \
  case k##kind:                                 \
    ...
    DeclareKindCase(AccelerationStructureLW);
    DeclareKindCase(CooperativeMatrixLW);
...
CooperativeMatrixLW::CooperativeMatrixLW(const Type* type, const uint32_t scope,
                                         const uint32_t rows,
                                         const uint32_t columns)
    : Type(kCooperativeMatrixLW),
...
std::string CooperativeMatrixLW::str() const {
  std::ostringstream oss;
...
void CooperativeMatrixLW::GetExtraHashWords(
...
bool CooperativeMatrixLW::IsSameImpl(const Type* that,
                                     IsSameCache* seen) const {
  const CooperativeMatrixLW* mt = that->AsCooperativeMatrixLW();
  if (!mt) return false;
