...
class LICMPass : public Pass {
 public:
  ...
  const char* name() const override { return "loop-ilwariant-code-motion"; }
  Status Process() override;

 private:
  // Searches the IRContext for functions and processes each, moving ilwariants
  ...
  // Checks for ilwariants in the loop and attempts to move them to the loops
  // preheader. Works from inner loop to outer when nested loops are found.
  // Returns the status depending on whether or not there was a failure or
  // change.
  Pass::Status ProcessLoop(Loop* loop, Function* f);

  // Analyses each instruction in |bb|, hoisting ilwariants to |pre_header_bb|.
