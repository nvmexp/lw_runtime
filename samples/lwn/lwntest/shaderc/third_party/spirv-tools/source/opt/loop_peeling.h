...
// be exelwted either before (peel before) or after (peel after) the transformed
// loop.
//
...
class LoopPeeling {
 public:
  ...
  // count, must be ilwariant for |loop| and must be of an int 32 type (signed
  // or unsigned).
  ...
  // To be peelable, all operation ilwolved in the update of the loop iterators
  ...
  // Moves the exelwtion of the |factor| first iterations of the loop into a
  // dedicated loop.
  void PeelBefore(uint32_t factor);

  // Moves the exelwtion of the |factor| last iterations of the loop into a
  // dedicated loop.
 ...
 private:
  ...
  // counter.
  void InsertCanonicalInductiolwariable(
  ...
  // Gathers all operations ilwolved in the update of |iterator| into
  // |operations|.
  ...
  // SSA value when it exit the loop. If no exit value can be aclwrately found,
...
class LoopPeelingPass : public Pass {
 ...
 private:
  ...
  class LoopPeelingInfo {
   ...
   private:
    // Returns the id of the loop ilwariant operand of the conditional
    // expression |condition|. It returns if no operand is ilwariant.
    uint32_t GetFirstLoopIlwariantOperand(Instruction* condition) const;
    // Returns the id of the non loop ilwariant operand of the conditional
    // expression |condition|. It returns if all operands are ilwariant.
    uint32_t GetFirstNonLoopIlwariantOperand(Instruction* condition) const;

    // Returns the value of |rec| at the first loop iteration.
    SExpression GetValueAtFirstIteration(SERelwrrentNode* rec) const;
    // Returns the value of |rec| at the given |iteration|.
    SExpression GetValueAtIteration(SERelwrrentNode* rec,
                                    int64_t iteration) const;
    // Returns the value of |rec| at the last loop iteration.
    SExpression GetValueAtLastIteration(SERelwrrentNode* rec) const;
