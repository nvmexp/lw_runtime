...
class LoopUnrollerUtilsImpl {
 public:
  ...
  // Remove all the instructions in the ilwalidated_instructions_ vector.
 ...
 private:
  ...
  std::vector<Instruction*> ilwalidated_instructions_;
...
void LoopUnrollerUtilsImpl::Init(Loop* loop) {
  ...
  loop_induction_variable_ = loop->FindConditiolwariable(loop_condition_block_);
...
void LoopUnrollerUtilsImpl::PartiallyUnrollResidualFactor(Loop* loop,
  ...
  // We need to account for the initial body when callwlating the remainder.
  int64_t remainder = Loop::GetResidualConditiolwalue(
  ...
  new_loop->GetInductiolwariables(new_inductions);

  std::vector<Instruction*> old_inductions;
  loop->GetInductiolwariables(old_inductions);
...
void LoopUnrollerUtilsImpl::Unroll(Loop* loop, size_t factor) {
  ...
  // This is due to the current method of callwlating the number of loop
  // iterations.
...
void LoopUnrollerUtilsImpl::RemoveDeadInstructions() {
  ...
  for (Instruction* inst : ilwalidated_instructions_) {
...
void LoopUnrollerUtilsImpl::ReplaceInductionUseWithFinalValue(Loop* loop) {
  ...
  for (size_t index = 0; index < inductions.size(); ++index) {
    ...
    ilwalidated_instructions_.push_back(inductions[index]);
  }
}
...
void LoopUnrollerUtilsImpl::CopyBasicBlock(Loop* loop, const BasicBlock* itr,
  ...
  if (itr == loop->GetHeaderBlock()) {
    ...
    if (!preserve_instructions) {
      ...
      if (merge_inst) ilwalidated_instructions_.push_back(merge_inst);
    }
  }
...
void LoopUnrollerUtilsImpl::CopyBody(Loop* loop, bool eliminate_conditions) {
  ...
  for (Instruction* dead_phi : state_.new_phis_)
    ilwalidated_instructions_.push_back(dead_phi);
...
void LoopUnrollerUtilsImpl::CloseUnrolledLoop(Loop* loop) {
  ...
  Instruction* merge_inst = loop->GetHeaderBlock()->GetLoopMergeInst();
  ilwalidated_instructions_.push_back(merge_inst);
...
bool LoopUtils::CanPerformUnroll() {
  ...
  const Instruction* induction = loop_->FindConditiolwariable(condition);
  ...
  std::vector<Instruction*> inductions;
  loop_->GetInductiolwariables(inductions);
  ...
  for (uint32_t label_id : loop_->GetBlocks()) {
    ...
    if (block->ctail()->opcode() == SpvOp::SpvOpKill ||
        ...
        block->ctail()->opcode() == SpvOp::SpvOpReturlwalue) {
      return false;
    }
  }
