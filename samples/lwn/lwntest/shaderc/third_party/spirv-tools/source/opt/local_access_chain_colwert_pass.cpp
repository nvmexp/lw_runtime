...
#include "source/opt/local_access_chain_colwert_pass.h"
...
void LocalAccessChainColwertPass::BuildAndAppendInst(
...
uint32_t LocalAccessChainColwertPass::BuildAndAppendVarLoad(
...
void LocalAccessChainColwertPass::AppendConstantOperands(
...
bool LocalAccessChainColwertPass::ReplaceAccessChainLoad(
...
bool LocalAccessChainColwertPass::GenAccessChainStoreReplacement(
...
bool LocalAccessChainColwertPass::IsConstantIndexAccessChain(
...
bool LocalAccessChainColwertPass::HasOnlySupportedRefs(uint32_t ptrId) {
...
void LocalAccessChainColwertPass::FindTargetVars(Function* func) {
    ...
    for (auto ii = bi->begin(); ii != bi->end(); ++ii) {
      switch (ii->opcode()) {
        ...
        case SpvOpLoad: {
          ...
          // TODO(): Colwert nested access chains
...
Pass::Status LocalAccessChainColwertPass::ColwertLocalAccessChains(
    Function* func) {
  FindTargetVars(func);
...
void LocalAccessChainColwertPass::Initialize() {
...
bool LocalAccessChainColwertPass::AllExtensionsSupported() const {
...
Pass::Status LocalAccessChainColwertPass::ProcessImpl() {
  ...
  for (Function& func : *get_module()) {
    status = CombineStatus(status, ColwertLocalAccessChains(&func));
...
LocalAccessChainColwertPass::LocalAccessChainColwertPass() {}

Pass::Status LocalAccessChainColwertPass::Process() {
  Initialize();
  return ProcessImpl();
}

void LocalAccessChainColwertPass::InitExtensions() {
  ...
  extensions_whitelist_.insert({
      ...
      //   Lwrrently do not support extended pointer expressions
      ...
      "SPV_EXT_demote_to_helper_ilwocation",
      ...
      "SPV_KHR_ray_tracing",
      "SPV_KHR_ray_query",
      "SPV_EXT_fragment_ilwocation_density",
  });
}
