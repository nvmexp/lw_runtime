...
void LoopPeeling::DuplicateAndConnectLoop(
  ...
  // When cloning the loop, we didn't cloned the merge block, so lwrrently
...
void LoopPeeling::InsertCanonicalInductiolwariable(
    LoopUtils::LoopCloningResult* clone_results) {
...
void LoopPeeling::PeelBefore(uint32_t peel_factor) {
  ...
  InsertCanonicalInductiolwariable(&clone_results);
  ...
  // "Protect" the second loop: the second loop can only be exelwted if
  ...
  // Prevent the second loop from being exelwted if we already exelwted all the
  // required iterations.
  BasicBlock* if_block =
  ...
  context_->IlwalidateAnalysesExceptFor(
      IRContext::kAnalysisDefUse | IRContext::kAnalysisInstrToBlockMapping |
...
void LoopPeeling::PeelAfter(uint32_t peel_factor) {
  ...
  // "Protect" the first loop: the first loop can only be exelwted if
...
std::pair<bool, Loop*> LoopPeelingPass::ProcessLoop(Loop* loop,
  ...
  Instruction* exiting_iv = loop->FindConditiolwariable(exit_block);
  if (!exiting_iv) {
    return bail_out;
  }
  size_t iterations = 0;
  ...
  loop->GetHeaderBlock()->WhileEachPhiInst([&canonical_induction_variable,
    ...
    if (const SERelwrrentNode* iv =
            scev_analysis->AnalyzeInstruction(insn)->AsSERelwrrentNode()) {
...
uint32_t LoopPeelingPass::LoopPeelingInfo::GetFirstLoopIlwariantOperand(
...
uint32_t LoopPeelingPass::LoopPeelingInfo::GetFirstNonLoopIlwariantOperand(
    Instruction* condition) const {
  for (uint32_t i = 0; i < condition->NumInOperands(); i++) {
    BasicBlock* bb =
        context_->get_instr_block(condition->GetSingleWordInOperand(i));
...
LoopPeelingPass::LoopPeelingInfo::GetPeelingInfo(BasicBlock* bb) const {
  ...
  if (!GetFirstLoopIlwariantOperand(condition)) {
    // No loop ilwariant, it cannot be peeled by this pass.
    return GetNoneDirection();
  }
  if (!GetFirstNonLoopIlwariantOperand(condition)) {
  ...
  // Only take into account relwrrent expression over the current loop.
  bool is_lhs_rec = !scev_analysis_->IsLoopIlwariant(loop_, lhs);
  bool is_rhs_rec = !scev_analysis_->IsLoopIlwariant(loop_, rhs);
  ...
  if (is_lhs_rec) {
    if (!lhs->AsSERelwrrentNode() ||
        lhs->AsSERelwrrentNode()->GetLoop() != loop_) {
      return GetNoneDirection();
    }
  }
  if (is_rhs_rec) {
    if (!rhs->AsSERelwrrentNode() ||
        rhs->AsSERelwrrentNode()->GetLoop() != loop_) {
      return GetNoneDirection();
    }
  }
  ...
  // constant expression is on the left hand side and the relwrring expression
  ...
  // Force the left hand side to be the non relwrring expression.
  if (is_lhs_rec) {
    std::swap(lhs, rhs);
    switch (cmp_operator) {
...
bool LoopPeelingPass::LoopPeelingInfo::EvalOperator(CmpOperator cmp_op,
                                                    ...
                                                    SExpression rhs,
                                                    bool* result) const {
  assert(scev_analysis_->IsLoopIlwariant(loop_, lhs));
  assert(scev_analysis_->IsLoopIlwariant(loop_, rhs));
...
LoopPeelingPass::LoopPeelingInfo::HandleEquality(SExpression lhs,
                                                 SExpression rhs) const {
  {
    ...
    if (SERelwrrentNode* rec_node = lhs->AsSERelwrrentNode()) {
      lhs_cst = rec_node->GetOffset();
    }
    SExpression rhs_cst = rhs;
    if (SERelwrrentNode* rec_node = rhs->AsSERelwrrentNode()) {
...
LoopPeelingPass::LoopPeelingInfo::HandleInequality(CmpOperator cmp_op,
  ...
  if (!flip_iteration.second &&
    ...
    if (first_iteration == lwrrent_iteration) {
      iteration++;
    }
  }
