...
void AggressiveDCEPass::ComputeBlock2HeaderMaps(
  ...
  std::stack<Instruction*> lwrrentHeaderBranch;
  lwrrentHeaderBranch.push(nullptr);
  uint32_t lwrrentMergeBlockId = 0;
  uint32_t index = 0;
  for (auto bi = structuredOrder.begin(); bi != structuredOrder.end();
    ...
    if ((*bi)->id() == lwrrentMergeBlockId) {
      lwrrentHeaderBranch.pop();
      Instruction* chb = lwrrentHeaderBranch.top();
      if (chb != nullptr)
        lwrrentMergeBlockId = branch2merge_[chb]->GetSingleWordInOperand(0);
    }
    Instruction* mergeInst;
    Instruction* branchInst;
    uint32_t mergeBlockId;
    ...
    if (mergeBlockId != 0) {
      ...
      if (merge_terminator->opcode() == SpvOpUnreachable) {
        ...
        if (func_ret_type_inst->opcode() == SpvOpTypeVoid) {
          ...
          merge_terminator->SetOpcode(SpvOpReturlwalue);
...
void AggressiveDCEPass::InitializeModuleScopeLiveInstructions() {
  // Keep all exelwtion modes.
  for (auto& exec : get_module()->exelwtion_modes()) {
    AddToWorklist(&exec);
  }
...
Pass::Status AggressiveDCEPass::ProcessImpl() {
  ...
  // when the context tries to update it.  To avoid this problem ilwalidate
  // the decoration manager upfront.
  //
  // We kill it at now because it is used when processing the entry point
  // functions.
  context()->IlwalidateAnalyses(IRContext::Analysis::kAnalysisDecorations);
  ...
  if (get_module()->version() >= SPV_SPIRV_VERSION_WORD(1, 4)) {
    ...
    for (auto& entry : get_module()->entry_points()) {
      ...
      for (uint32_t i = 0; i < entry.NumInOperands(); ++i) {
        if (i < 3) {
          // Exelwtion model, function id and name are always valid.
