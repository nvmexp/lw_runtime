...
void CFG::ComputeStructuredOrder(Function* func, BasicBlock* root,
  ...
  // out of the cfa.h prototypes and into the ilwoking code.
...
void CFG::ForEachBlockInPostOrder(BasicBlock* bb,
  ...
  for (BasicBlock* lwrrent_bb : po) {
    if (!IsPseudoExitBlock(lwrrent_bb) && !IsPseudoEntryBlock(lwrrent_bb)) {
      f(lwrrent_bb);
    }
  }
}
...
bool CFG::WhileEachBlockInReversePostOrder(
  ...
  for (auto lwrrent_bb = po.rbegin(); lwrrent_bb != po.rend(); ++lwrrent_bb) {
    if (!IsPseudoExitBlock(*lwrrent_bb) && !IsPseudoEntryBlock(*lwrrent_bb)) {
      if (!f(*lwrrent_bb)) {
        return false;
      }
