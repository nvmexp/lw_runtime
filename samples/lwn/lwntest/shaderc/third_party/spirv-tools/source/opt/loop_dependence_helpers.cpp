...
bool LoopDependenceAnalysis::IsZIV(
  ...
  return CountInductiolwariables(subscript_pair.first, subscript_pair.second) ==
         0;
}
...
bool LoopDependenceAnalysis::IsProvablyOutsideOfLoopBounds(
  ...
  // If the coefficient is positive we callwlate bounds as upper - lower
  // If the coefficient is negative we callwlate bounds as lower - upper
...
const Loop* LoopDependenceAnalysis::GetLoopForSubscriptPair(
    const std::pair<SENode*, SENode*>& subscript_pair) {
  // Collect all the SERelwrrentNodes.
  std::vector<SERelwrrentNode*> source_nodes =
      std::get<0>(subscript_pair)->CollectRelwrrentNodes();
  std::vector<SERelwrrentNode*> destination_nodes =
      std::get<1>(subscript_pair)->CollectRelwrrentNodes();

  // Collect all the loops stored by the SERelwrrentNodes.
...
SENode* LoopDependenceAnalysis::GetTripCount(const Loop* loop) {
  ...
  Instruction* induction_instr = loop->FindConditiolwariable(condition_block);
  if (!induction_instr) {
    return nullptr;
  }
  ...
  // isn't a supported type we can't callwlate the trip count.
...
std::set<const Loop*> LoopDependenceAnalysis::CollectLoops(
    const std::vector<SERelwrrentNode*>& relwrrent_nodes) {
  ...
  // loops the collected relwrrent nodes belong to.
  std::set<const Loop*> loops{};
  for (auto relwrrent_nodes_it = relwrrent_nodes.begin();
       relwrrent_nodes_it != relwrrent_nodes.end(); ++relwrrent_nodes_it) {
    loops.insert((*relwrrent_nodes_it)->GetLoop());
  }

  return loops;
}

int64_t LoopDependenceAnalysis::CountInductiolwariables(SENode* node) {
  if (!node) {
    return -1;
  }

  std::vector<SERelwrrentNode*> relwrrent_nodes = node->CollectRelwrrentNodes();

  // We don't handle loops with more than one induction variable. Therefore we
  // can identify the number of induction variables by collecting all of the
  // loops the collected relwrrent nodes belong to.
  std::set<const Loop*> loops = CollectLoops(relwrrent_nodes);
...
std::set<const Loop*> LoopDependenceAnalysis::CollectLoops(
  ...
  std::vector<SERelwrrentNode*> source_nodes = source->CollectRelwrrentNodes();
  std::vector<SERelwrrentNode*> destination_nodes =
      destination->CollectRelwrrentNodes();
...
int64_t LoopDependenceAnalysis::CountInductiolwariables(SENode* source,
                                                        SENode* destination) {
  if (!source || !destination) {
    return -1;
  }
...
void LoopDependenceAnalysis::MarkUnsusedDistanceEntriesAsIrrelevant(
  ...
  for (Instruction* source_inst : source_subscripts) {
    ...
    std::vector<SERelwrrentNode*> relwrrent_nodes =
        source_node->CollectRelwrrentNodes();
    for (SERelwrrentNode* relwrrent_node : relwrrent_nodes) {
      used_loops.insert(relwrrent_node->GetLoop());
    }
  }
...
bool LoopDependenceAnalysis::IsSupportedLoop(const Loop* loop) {
  ...
  loop->GetInductiolwariables(inductions);
  ...
  if (!induction_node->AsSERelwrrentNode()) {
    return false;
  }
  SENode* induction_step =
      induction_node->AsSERelwrrentNode()->GetCoefficient();
