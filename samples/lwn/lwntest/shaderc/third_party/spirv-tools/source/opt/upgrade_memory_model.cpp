...
Pass::Status UpgradeMemoryModel::Process() {
  ...
  if (context()->get_feature_mgr()->HasCapability(
          SpvCapabilityCooperativeMatrixLW)) {
...
std::pair<bool, bool> UpgradeMemoryModel::TraceInstruction(
  ...
  switch (inst->opcode()) {
    ...
    case SpvOpFunctionParameter:
      ...
      is_volatile |= HasDecoration(inst, 0, SpvDecoratiolwolatile);
            ...
            switch (dec.opcode()) {
              ...
              case SpvOpDecorateId:
                if (dec.GetSingleWordInOperand(1u) == SpvDecorationCoherent ||
                    dec.GetSingleWordInOperand(1u) == SpvDecoratiolwolatile)
                  return true;
                break;
              case SpvOpMemberDecorate:
                if (dec.GetSingleWordInOperand(2u) == SpvDecorationCoherent ||
                    dec.GetSingleWordInOperand(2u) == SpvDecoratiolwolatile)
                  return true;
                break;
              default:
                break;
...
void UpgradeMemoryModel::UpgradeBarriers() {
  ...
  for (auto& e : get_module()->entry_points())
    if (e.GetSingleWordInOperand(0u) == SpvExelwtionModelTessellationControl) {
