...
static const int kSpvReturlwalueId = 0;
...
uint32_t InlinePass::CreateReturlwar(
    Function* calleeFn, std::vector<std::unique_ptr<Instruction>>* new_vars) {
  uint32_t returlwarId = 0;
  ...
  uint32_t returlwarTypeId =
      type_mgr->FindPointerToType(calleeTypeId, SpvStorageClassFunction);

  if (returlwarTypeId == 0) {
    returlwarTypeId = AddPointerToType(calleeTypeId, SpvStorageClassFunction);
    if (returlwarTypeId == 0) {
      return 0;
    }
  }
  ...
  returlwarId = context()->TakeNextId();
  if (returlwarId == 0) {
    return 0;
  }

  std::unique_ptr<Instruction> var_inst(
      new Instruction(context(), SpvOpVariable, returlwarTypeId, returlwarId,
                      {{spv_operand_type_t::SPV_OPERAND_TYPE_STORAGE_CLASS,
                        {SpvStorageClassFunction}}}));
  new_vars->push_back(std::move(var_inst));
  get_decoration_mgr()->CloneDecorations(calleeFn->result_id(), returlwarId);
  return returlwarId;
}
...
bool InlinePass::GenInlineCode(
  ...
  // Ilwalidate the def-use chains.  They are not kept up to date while
  // inlining.  However, certain calls try to keep them up-to-date if they are
  // valid.  These operations can fail.
  context()->IlwalidateAnalyses(IRContext::kAnalysisDefUse);
  ...
  if (calleeType->AsVoid() == nullptr) {
    returlwarId = CreateReturlwar(calleeFn, new_vars);
    if (returlwarId == 0) {
      return false;
    }
  }
  ...
  bool successful = calleeFn->WhileEachInst(
      [&new_blocks, &callee2caller, &call_block_itr, &call_inst_itr,
       &new_blk_ptr, &prevInstWasReturn, &returnLabelId, &returlwarId,
       ...
       this](const Instruction* cpi) {
        switch (cpi->opcode()) {
          ...
          case SpvOpLabel: {
            ...
            if (firstBlock) {
              ...
              if (caller_is_loop_header &&
                ...
                // ilwalidating various iterators.
          ...
          case SpvOpReturlwalue: {
            ...
            assert(returlwarId != 0);
            uint32_t valId = cpi->GetInOperand(kSpvReturlwalueId).words[0];
            const auto mapItr = callee2caller.find(valId);
            if (mapItr != callee2caller.end()) {
              valId = mapItr->second;
            }
            AddStore(returlwarId, valId, &new_blk_ptr);
          ...
          case SpvOpFunctionEnd: {
            ...
            if (returlwarId != 0) {
              ...
              assert(resId != 0);
              AddLoad(calleeTypeId, resId, returlwarId, &new_blk_ptr);
            }
...
bool InlinePass::IsInlinableFunction(Function* func) {
  ...
  // Do not inline functions with returns in loops. Lwrrently early return
