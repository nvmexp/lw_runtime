...
// Callwlate the greatest common divisor of a & b using Stein's algorithm.
...
// and contains only the following types of nodes: SERelwrrentNode, SEAddNode
// and SEConstantNode
bool IsInCorrectFormForGCDTest(SENode* node) {
  ...
  bool this_ok = node->AsSERelwrrentNode() || node->AsSEAddNode() ||
                 node->AsSEConstantNode();

  return children_ok && this_ok;
}

// If |node| is an SERelwrrentNode then returns |node| or if |node| is an
// SEAddNode returns a vector of SERelwrrentNode that are its children.
std::vector<SERelwrrentNode*> GetAllTopLevelRelwrrences(SENode* node) {
  auto nodes = std::vector<SERelwrrentNode*>{};
  if (auto relwrrent_node = node->AsSERelwrrentNode()) {
    nodes.push_back(relwrrent_node);
  }

  if (auto add_node = node->AsSEAddNode()) {
    for (auto child : add_node->GetChildren()) {
      auto child_nodes = GetAllTopLevelRelwrrences(child);
...
std::vector<SEConstantNode*> GetAllTopLevelConstants(SENode* node) {
  ...
  if (auto relwrrent_node = node->AsSEConstantNode()) {
    nodes.push_back(relwrrent_node);
  }
...
bool AreOffsetsAndCoefficientsConstant(
    const std::vector<SERelwrrentNode*>& nodes) {
  for (auto node : nodes) {
...
// Fold all SEConstantNode that appear in |relwrrences| and |constants| into a
// single integer value.
int64_t CallwlateConstantTerm(const std::vector<SERelwrrentNode*>& relwrrences,
                              const std::vector<SEConstantNode*>& constants) {
  int64_t constant_term = 0;
  for (auto relwrrence : relwrrences) {
    constant_term +=
        relwrrence->GetOffset()->AsSEConstantNode()->FoldToSingleValue();
  }
...
int64_t CallwlateGCDFromCoefficients(
    const std::vector<SERelwrrentNode*>& relwrrences, int64_t running_gcd) {
  for (SERelwrrentNode* relwrrence : relwrrences) {
    auto coefficient = relwrrence->GetCoefficient()->AsSEConstantNode();
  ...
  for (auto it = std::begin(sets_of_subscripts); it < first_coupled; ++it) {
    ...
    if (IsMIV(subscript_pair)) {
      ...
      if (GCDMIVTest(subscript_pair)) {
        ...
        auto lwrrent_loops = CollectLoops(source_node, destination_node);

        for (auto lwrrent_loop : lwrrent_loops) {
          auto distance_entry =
              GetDistanceEntryForLoop(lwrrent_loop, distance_vector);
...
bool LoopDependenceAnalysis::SIVTest(
  ...
  int64_t source_induction_count = CountInductiolwariables(source_node);
  int64_t destination_induction_count =
      CountInductiolwariables(destination_node);
  ...
  if (source_induction_count == 0) {
    ...
    if (WeakZeroSourceSIVTest(
            source_node, destination_node->AsSERelwrrentNode(),
            destination_node->AsSERelwrrentNode()->GetCoefficient(),
            distance_entry)) {
  ...
  if (destination_induction_count == 0) {
    ...
    if (WeakZeroDestinationSIVTest(
            source_node->AsSERelwrrentNode(), destination_node,
            source_node->AsSERelwrrentNode()->GetCoefficient(),
            distance_entry)) {
  ...
  // We now need to collect the SERelwrrentExpr nodes from source and
  // destination. We do not handle cases where source or destination have
  // multiple SERelwrrentExpr nodes.
  std::vector<SERelwrrentNode*> source_relwrrent_nodes =
      source_node->CollectRelwrrentNodes();
  std::vector<SERelwrrentNode*> destination_relwrrent_nodes =
      destination_node->CollectRelwrrentNodes();

  if (source_relwrrent_nodes.size() == 1 &&
      destination_relwrrent_nodes.size() == 1) {
    PrintDebug("Found source and destination have 1 induction variable.");
    SERelwrrentNode* source_relwrrent_expr = *source_relwrrent_nodes.begin();
    SERelwrrentNode* destination_relwrrent_expr =
        *destination_relwrrent_nodes.begin();

    // If the coefficients are identical we can apply a StrongSIVTest.
    if (source_relwrrent_expr->GetCoefficient() ==
        destination_relwrrent_expr->GetCoefficient()) {
      PrintDebug("Found source and destination share coefficient.");
      if (StrongSIVTest(source_node, destination_node,
                        source_relwrrent_expr->GetCoefficient(),
                        distance_entry)) {
...
bool LoopDependenceAnalysis::StrongSIVTest(SENode* source, SENode* destination,
  ...
  // If both source and destination are SERelwrrentNodes we can perform tests
  // based on distance.
  // If either source or destination contain value unknown nodes or if one or
  // both are not SERelwrrentNodes we must attempt a symbolic test.
  ...
  if (!source->AsSERelwrrentNode() || !destination->AsSERelwrrentNode()) {
    PrintDebug(
        "StrongSIVTest could not simplify source and destination to "
        "SERelwrrentNodes so will exit.");
    distance_entry->direction = DistanceEntry::Directions::ALL;
    return false;
  }
  ...
  SENode* source_constant_term =
      GetConstantTerm(subscript_loop, source->AsSERelwrrentNode());
  SENode* destination_constant_term =
      GetConstantTerm(subscript_loop, destination->AsSERelwrrentNode());
...
bool LoopDependenceAnalysis::WeakZeroSourceSIVTest(
    SENode* source, SERelwrrentNode* destination, SENode* coefficient,
    DistanceEntry* distance_entry) {
  ...
  if (delta_constant && coefficient_constant) {
    ...
    if (delta_value % coefficient_value != 0) {
      ...
      distance = delta_value / coefficient_value;
      PrintDebug(
          "WeakZeroSourceSIVTest callwlated distance with the following "
          "values\n"
          "\tdelta value: " +
          ToString(delta_value) +
          "\n\tcoefficient value: " + ToString(coefficient_value) +
...
bool LoopDependenceAnalysis::WeakZeroDestinationSIVTest(
    SERelwrrentNode* source, SENode* destination, SENode* coefficient,
    DistanceEntry* distance_entry) {
  ...
  if (delta_constant && coefficient_constant) {
    ...
    if (delta_value % coefficient_value != 0) {
      ...
      distance = delta_value / coefficient_value;
      PrintDebug(
          "WeakZeroDestinationSIVTest callwlated distance with the following "
          "values\n"
          "\tdelta value: " +
          ToString(delta_value) +
          "\n\tcoefficient value: " + ToString(coefficient_value) +
...
bool LoopDependenceAnalysis::WeakCrossingSIVTest(
  ...
  // We lwrrently can't handle symbolic WeakCrossingSIVTests. If either source
  // or destination are not SERelwrrentNodes we must exit.
  if (!source->AsSERelwrrentNode() || !destination->AsSERelwrrentNode()) {
    PrintDebug(
        "WeakCrossingSIVTest found source or destination != SERelwrrentNode. "
        "Exiting");
    distance_entry->direction = DistanceEntry::Directions::ALL;
    return false;
  }
  ...
  SENode* offset_delta =
      scalar_evolution_.SimplifyExpression(scalar_evolution_.CreateSubtraction(
          destination->AsSERelwrrentNode()->GetOffset(),
          source->AsSERelwrrentNode()->GetOffset()));
...
bool LoopDependenceAnalysis::GCDMIVTest(
  ...
  auto source_relwrrences = GetAllTopLevelRelwrrences(source);
  auto dest_relwrrences = GetAllTopLevelRelwrrences(destination);

  // Bail out if all offsets and coefficients aren't constant.
  if (!AreOffsetsAndCoefficientsConstant(source_relwrrences) ||
      !AreOffsetsAndCoefficientsConstant(dest_relwrrences)) {
    return false;
  }

  // Callwlate the GCD of all coefficients.
  auto source_constants = GetAllTopLevelConstants(source);
  int64_t source_constant =
      CallwlateConstantTerm(source_relwrrences, source_constants);

  auto dest_constants = GetAllTopLevelConstants(destination);
  int64_t destination_constant =
      CallwlateConstantTerm(dest_relwrrences, dest_constants);

  int64_t delta = std::abs(source_constant - destination_constant);

  int64_t running_gcd = 0;

  running_gcd = CallwlateGCDFromCoefficients(source_relwrrences, running_gcd);
  running_gcd = CallwlateGCDFromCoefficients(dest_relwrrences, running_gcd);
...
PartitionedSubscripts LoopDependenceAnalysis::PartitionSubscripts(
  ...
  for (auto loop : loops_) {
    ...
    for (size_t j = 0; j < partitions.size(); ++j) {
      auto& lwrrent_partition = partitions[j];

      // Does |loop| appear in |lwrrent_partition|
      auto it = std::find_if(
          lwrrent_partition.begin(), lwrrent_partition.end(),
          [loop,
           this](const std::pair<Instruction*, Instruction*>& elem) -> bool {
            auto source_relwrrences =
                scalar_evolution_.AnalyzeInstruction(std::get<0>(elem))
                    ->CollectRelwrrentNodes();
            auto destination_relwrrences =
                scalar_evolution_.AnalyzeInstruction(std::get<1>(elem))
                    ->CollectRelwrrentNodes();

            source_relwrrences.insert(source_relwrrences.end(),
                                      destination_relwrrences.begin(),
                                      destination_relwrrences.end());

            auto loops_in_pair = CollectLoops(source_relwrrences);
            auto end_it = loops_in_pair.end();

            return std::find(loops_in_pair.begin(), end_it, loop) != end_it;
          });

      auto has_loop = it != lwrrent_partition.end();
      ...
      if (has_loop) {
        if (k == -1) {
          ...
          partitions[static_cast<size_t>(k)].insert(lwrrent_partition.begin(),
                                                    lwrrent_partition.end());
          lwrrent_partition.clear();
        }
      }
    }
  }
...
SubscriptPair LoopDependenceAnalysis::PropagateConstraints(
  ...
  for (auto& constraint : constraints) {
    ...
    // relwrrent expression with respect to the loop 'k'.
    SENode* coefficient_of_relwrrent =
        scalar_evolution_.GetCoefficientFromRelwrrentTerm(
            new_first, constraint->GetLoop());

    // In the paper this is a'[k].
    SENode* coefficient_of_relwrrent_prime =
        scalar_evolution_.GetCoefficientFromRelwrrentTerm(
    ...
    if (constraint->GetType() == Constraint::Distance) {
      ...
      SENode* rhs = scalar_evolution_.CreateMultiplyNode(
          coefficient_of_relwrrent, as_distance->GetDistance());

      // In the paper this is a[k] <- 0
      SENode* zeroed_coefficient =
          scalar_evolution_.BuildGraphWithoutRelwrrentTerm(
              new_first, constraint->GetLoop());
      ...
      SENode* new_child = scalar_evolution_.SimplifyExpression(
          scalar_evolution_.CreateSubtraction(coefficient_of_relwrrent_prime,
                                              coefficient_of_relwrrent));

      // In the paper this is a'[k]'i[k].
      SERelwrrentNode* prime_relwrrent =
          scalar_evolution_.GetRelwrrentTerm(new_second, constraint->GetLoop());

      if (!prime_relwrrent) continue;

      // As we hash the nodes we need to create a new node when we update a
      // child.
      SENode* new_relwrrent = scalar_evolution_.CreateRelwrrentExpression(
          constraint->GetLoop(), prime_relwrrent->GetOffset(), new_child);
      // In the paper this is a'[k] <- a'[k] - a[k].
      new_second = scalar_evolution_.UpdateChildNode(
          new_second, prime_relwrrent, new_relwrrent);
    }
  }
...
bool LoopDependenceAnalysis::DeltaTest(
  ...
  while (!siv_subscripts.empty()) {
    ...
    std::vector<DistanceVector> lwrrent_distances(
    ...
    std::transform(
        ...
        std::begin(lwrrent_distances), std::begin(results),
    ...
    for (size_t i = 0; i < siv_subscripts.size(); ++i) {
      ...
      auto distance_entry = lwrrent_distances[i].GetEntries()[loop_id];
      ...
      if (distance_entry.dependence_information ==
        ...
        if (destination_induction_count != 0) {
          a = destination_node->AsSERelwrrentNode()->GetCoefficient();
          c = scalar_evolution_.CreateNegation(
              destination_node->AsSERelwrrentNode()->GetOffset());
        } else {
        ...
        if (source_induction_count != 0) {
          ...
          c = scalar_evolution_.CreateAddNode(
              c, source_node->AsSERelwrrentNode()->GetOffset());
        } else {
    ...
    // Callwlate the intersection between the new and existing constraints.
  ...
  for (size_t i = 0; i < loops_.size(); ++i) {
    ...
    if (loop_appeared[i]) {
      auto lwrrent_constraint = constraints[i];
      auto& lwrrent_distance_entry = (*dv_entry).GetEntries()[i];

      if (auto dependence_distance =
              lwrrent_constraint->AsDependenceDistance()) {
        if (auto constant_node =
                dependence_distance->GetDistance()->AsSEConstantNode()) {
          lwrrent_distance_entry.dependence_information =
              DistanceEntry::DependenceInformation::DISTANCE;

          lwrrent_distance_entry.distance = constant_node->FoldToSingleValue();
          if (lwrrent_distance_entry.distance == 0) {
            lwrrent_distance_entry.direction = DistanceEntry::Directions::EQ;
          } else if (lwrrent_distance_entry.distance < 0) {
            lwrrent_distance_entry.direction = DistanceEntry::Directions::GT;
          } else {
            lwrrent_distance_entry.direction = DistanceEntry::Directions::LT;
          }
        }
      } else if (auto dependence_point =
                     lwrrent_constraint->AsDependencePoint()) {
        ...
        if (source->AsSEConstantNode() && destination->AsSEConstantNode()) {
          lwrrent_distance_entry = DistanceEntry(
