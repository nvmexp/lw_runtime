...
void SSAPropagator::AddControlEdge(const Edge& edge) {
  ...
  // Try to mark the edge exelwtable.  If it was already in the set of
  // exelwtable edges, do nothing.
  if (!MarkEdgeExelwtable(edge)) {
    return;
  }

  // If the edge had not already been marked exelwtable, add the destination
...
bool SSAPropagator::IsPhiArgExelwtable(Instruction* phi, uint32_t i) const {
  ...
  return IsEdgeExelwtable(Edge(in_bb, phi_bb));
}
...
bool SSAPropagator::Simulate(Instruction* instr) {
  ...
  if (instr->opcode() == SpvOpPhi) {
    ...
    for (uint32_t i = 2; i < instr->NumOperands(); i += 2) {
      ...
      if (!IsPhiArgExelwtable(instr, i) || ShouldSimulateAgain(arg_def_instr)) {
...
bool SSAPropagator::Simulate(BasicBlock* block) {
  ...
  // incoming edges. When those edges are marked exelwtable, the corresponding
