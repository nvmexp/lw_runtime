...
int64_t Loop::GetResidualConditiolwalue(SpvOp condition, int64_t initial_value,
                                        int64_t step_value,
  ...
  // We subtract or add one as the above formula callwlates the remainder if the
  ...
  for (DominatorTreeNode& node :
    ...
    if (merge_inst) {
      ...
      for (auto itr = loops_.rbegin() + 1; itr != loops_.rend(); ++itr) {
        ...
        lwrrent_loop->AddNestedLoop(previous_loop);
      }
      ...
      for (DominatorTreeNode& loop_node :
        ...
        lwrrent_loop->AddBasicBlock(loop_node.bb_);
        basic_block_to_loop_.insert(
            std::make_pair(loop_node.bb_->id(), lwrrent_loop));
      }
    }
  }
...
// |step_value| where diff is callwlated differently according to the
...
void Loop::GetInductiolwariables(
...
Instruction* Loop::FindConditiolwariable(
...
Loop* LoopDescriptor::AddLoopNest(std::unique_ptr<Loop> new_loop) {
  ...
  // as we go.
  for (Loop& lwrrent_loop :
       make_range(iterator::begin(loop), iterator::end(nullptr))) {
    loops_.push_back(&lwrrent_loop);
    for (uint32_t bb_id : lwrrent_loop.GetBlocks())
      basic_block_to_loop_.insert(std::make_pair(bb_id, &lwrrent_loop));
  }
