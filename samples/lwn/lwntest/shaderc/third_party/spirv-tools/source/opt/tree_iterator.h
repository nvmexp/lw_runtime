...
class TreeDFIterator {
 ...
 public:
  ...
  explicit inline TreeDFIterator(NodePtr top_node) : lwrrent_(top_node) {
    if (lwrrent_ && lwrrent_->begin() != lwrrent_->end())
      parent_iterators_.emplace(make_pair(lwrrent_, lwrrent_->begin()));
  }
  ...
  bool operator==(const TreeDFIterator& x) const {
    return lwrrent_ == x.lwrrent_;
  }

  bool operator!=(const TreeDFIterator& x) const { return !(*this == x); }

  reference operator*() const { return *lwrrent_; }

  NodePtr operator->() const { return lwrrent_; }
 ...
 private:
  ...
  inline void MoveToNextNode() {
    ...
    if (lwrrent_->begin() != lwrrent_->end())
      parent_iterators_.emplace(make_pair(lwrrent_, lwrrent_->begin()));
  }
...
class PostOrderTreeDFIterator {
 ...
 private:
  ...
  inline void WalkToLeaf() {
    while (lwrrent_->begin() != lwrrent_->end()) {
      NodeIterator next = ++lwrrent_->begin();
      parent_iterators_.emplace(make_pair(lwrrent_, next));
      // Set the first child as the new node.
      lwrrent_ = *lwrrent_->begin();
    }
  }
