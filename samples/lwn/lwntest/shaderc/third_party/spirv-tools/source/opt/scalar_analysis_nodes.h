...
class SENode {
 public:
  ...
  // Dump the SENode and its immediate children, if |relwrse| is true then it
  // will relwrse through all children to print the DAG starting from this node
  // as a root.
  void DumpDot(std::ostream& out, bool relwrse = false) const;
  ...
  // Collect all the relwrrent nodes in this SENode
  std::vector<SERelwrrentNode*> CollectRelwrrentNodes() {
    std::vector<SERelwrrentNode*> relwrrent_nodes{};

    if (auto relwrrent_node = AsSERelwrrentNode()) {
      relwrrent_nodes.push_back(relwrrent_node);
    }

    for (auto child : GetChildren()) {
      auto child_relwrrent_nodes = child->CollectRelwrrentNodes();
      relwrrent_nodes.insert(relwrrent_nodes.end(),
                             child_relwrrent_nodes.begin(),
                             child_relwrrent_nodes.end());
    }

    return relwrrent_nodes;
  }
...
#define DeclareCastMethod(target)                  \
  ...
  DeclareCastMethod(SERelwrrentNode)
...
// A node representing a relwrrent expression in the code. A relwrrent
...
// value of a relwrrent expression is coefficent_ * iteration + offset_, hence
// an induction variable i=0, i++ becomes a relwrrent expression with an offset
// of zero and a coefficient of one.
class SERelwrrentNode : public SENode {
 public:
  SERelwrrentNode(ScalarEvolutionAnalysis* parent_analysis, const Loop* loop)
      : SENode(parent_analysis), loop_(loop) {}

  SENodeType GetType() const final { return RelwrrentAddExpr; }
  ...
  // Return the loop which this relwrrent expression is relwrring within.
  const Loop* GetLoop() const { return loop_; }

  SERelwrrentNode* AsSERelwrrentNode() override { return this; }
  const SERelwrrentNode* AsSERelwrrentNode() const override { return this; }
