 ...
 *
 * Structured control flow guarantees that the CFG will colwerge at a given
...
class MergeReturnPass : public MemPass {
 ...
 private:
  ...
  class StructuredControlState {
   public:
    StructuredControlState(Instruction* break_merge, Instruction* merge)
        : break_merge_(break_merge), lwrrent_merge_(merge) {}

    StructuredControlState(const StructuredControlState&) = default;

    bool InBreakable() const { return break_merge_; }
    bool InStructuredFlow() const { return LwrrentMergeId() != 0; }

    uint32_t LwrrentMergeId() const {
      return lwrrent_merge_ ? lwrrent_merge_->GetSingleWordInOperand(0u) : 0u;
    }

    uint32_t LwrrentMergeHeader() const {
      return lwrrent_merge_ ? lwrrent_merge_->context()
                                  ->get_instr_block(lwrrent_merge_)
                                  ->id()
                            : 0;
    }
   ...
   private:
    ...
    Instruction* lwrrent_merge_;
  };

  // Returns all BasicBlocks terminated by OpReturn or OpReturlwalue in
  // |function|.
  ...
  //
  // Is is assumed that |AddReturlwalue| have already been called to created the
  ...
  // a block that use to contain an OpReturlwalue.
  void AddReturlwalue();
  ...
  // OpReturlwalue in |block|.  The value is stored to |return_value_|, and the
  // store is placed before the OpReturlwalue.
  //
  // If |block| does not contain an OpReturlwalue, then this function has no
  // effect. If |block| contains an OpReturlwalue, then |AddReturlwalue| must
  // have already been called to create the variable to store to.
  void RecordReturlwalue(BasicBlock* block);
  ...
  // |AddReturnFlag| and |AddReturlwalue| must have already been called.
  ...
  // added to jump around any code that should not be exelwted because the
  // original code would have already returned. This ilwolves adding new
  ...
  // Add an |OpReturn| or |OpReturlwalue| to the end of |block|.  If an
  // |OpReturlwalue| is needed, the return value is loaded from |return_value_|.
  ...
  StructuredControlState& LwrrentState() { return state_.back(); }

  // Inserts |new_element| into |list| after the first oclwrrence of |element|.
  ...
  // default is the merge block. Returns after the switch is exelwted. Sets
  // |final_return_block_|.
  ...
  // Creates a one case switch around the exelwtable code of the function with
