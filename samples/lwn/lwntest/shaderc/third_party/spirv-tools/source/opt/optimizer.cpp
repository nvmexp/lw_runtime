...
struct Optimizer::Impl {
  explicit Impl(spv_target_elw elw) : target_elw(elw), pass_manager() {}

  spv_target_elw target_elw;      // Target environment.
  opt::PassManager pass_manager;  // Internal implementation pass manager.
};

Optimizer::Optimizer(spv_target_elw elw) : impl_(new Impl(elw)) {}
...
Optimizer& Optimizer::RegisterPerformancePasses() {
  return RegisterPass(CreateWrapOpKillPass())
      ...
      .RegisterPass(CreateLocalAccessChainColwertPass())
      .RegisterPass(CreateLocalSingleBlockLoadStoreElimPass())
      .RegisterPass(CreateLocalSingleStoreElimPass())
      .RegisterPass(CreateAggressiveDCEPass())
      .RegisterPass(CreateLocalMultiStoreElimPass())
      ...
      .RegisterPass(CreateDeadBranchElimPass())
      .RegisterPass(CreateSimplificationPass())
      .RegisterPass(CreateIfColwersionPass())
      .RegisterPass(CreateCopyPropagateArraysPass())
      .RegisterPass(CreateReduceLoadSizePass())
      .RegisterPass(CreateAggressiveDCEPass())
...
Optimizer& Optimizer::RegisterVulkanToWebGPUPasses() {
  return RegisterPass(CreateStripAtomicCounterMemoryPass())
      ...
      .RegisterPass(CreateSplitIlwalidUnreachablePass())
...
bool Optimizer::RegisterPassFromFlag(const std::string& flag) {
  ...
  // can be ilwoked without creating a pass instance.
  if (pass_name == "strip-atomic-counter-memory") {
  ...
  } else if (pass_name == "if-colwersion") {
    RegisterPass(CreateIfColwersionPass());
    ...
    RegisterPass(CreateCombineAccessChainsPass());
  } else if (pass_name == "colwert-local-access-chains") {
    RegisterPass(CreateLocalAccessChainColwertPass());
  ...
  } else if (pass_name == "loop-ilwariant-code-motion") {
    RegisterPass(CreateLoopIlwariantCodeMotionPass());
    ...
    RegisterPass(CreateReplaceIlwalidOpcodePass());
    ...
    RegisterPass(CreateAggressiveDCEPass());
  } else if (pass_name == "colwert-relaxed-to-half") {
    RegisterPass(CreateColwertRelaxedToHalfPass());
...
void Optimizer::SetTargetElw(const spv_target_elw elw) {
  impl_->target_elw = elw;
}
...
bool Optimizer::Run(const uint32_t* original_binary,
  ...
  std::unique_ptr<opt::IRContext> context = BuildModule(
      impl_->target_elw, consumer(), original_binary, original_binary_size);
  ...
  impl_->pass_manager.SetTargetElw(impl_->target_elw);
  ...
  // buffer and the below will ilwalidate |original_binary|.
...
Optimizer::PassToken CreateLocalAccessChainColwertPass() {
  return MakeUnique<Optimizer::PassToken::Impl>(
      MakeUnique<opt::LocalAccessChainColwertPass>());
}
...
Optimizer::PassToken CreateIfColwersionPass() {
  return MakeUnique<Optimizer::PassToken::Impl>(
      MakeUnique<opt::IfColwersion>());
}

Optimizer::PassToken CreateReplaceIlwalidOpcodePass() {
  return MakeUnique<Optimizer::PassToken::Impl>(
      MakeUnique<opt::ReplaceIlwalidOpcodePass>());
}
...
Optimizer::PassToken CreateColwertRelaxedToHalfPass() {
  return MakeUnique<Optimizer::PassToken::Impl>(
      MakeUnique<opt::ColwertToHalfPass>());
}
...
Optimizer::PassToken CreateSplitIlwalidUnreachablePass() {
  return MakeUnique<Optimizer::PassToken::Impl>(
      MakeUnique<opt::SplitIlwalidUnreachablePass>());
}
