...
Instruction::Instruction(IRContext* c, const spv_parsed_instruction_t& inst,
  ...
  for (uint32_t i = 0; i < inst.num_operands; ++i) {
    const auto& lwrrent_payload = inst.operands[i];
    std::vector<uint32_t> words(
        inst.words + lwrrent_payload.offset,
        inst.words + lwrrent_payload.offset + lwrrent_payload.num_words);
    operands_.emplace_back(lwrrent_payload.type, std::move(words));
  }
}
...
std::string Instruction::PrettyPrint(uint32_t options) const {
  // Colwert the module to binary.
  std::vector<uint32_t> module_binary;
  context()->module()->ToBinary(&module_binary, /* skip_nop = */ false);

  // Colwert the instruction to binary. This is used to identify the correct
  ...
  return spvInstructionBinaryToText(
      context()->grammar().target_elw(), inst_binary.data(), inst_binary.size(),
