...
SENode* ScalarEvolutionAnalysis::CreateRelwrrentExpression(
    const Loop* loop, SENode* offset, SENode* coefficient) {
  assert(loop && "Relwrrent add expressions must have a valid loop.");
  ...
  std::unique_ptr<SERelwrrentNode> phi_node{
      new SERelwrrentNode(this, loop_to_use)};
...
SENode* ScalarEvolutionAnalysis::AnalyzeInstruction(const Instruction* inst) {
  auto itr = relwrrent_node_map_.find(inst);
  if (itr != relwrrent_node_map_.end()) return itr->second;
...
SENode* ScalarEvolutionAnalysis::AnalyzePhiInstruction(const Instruction* phi) {
  ...
  if (!loop || !loop->GetLatchBlock() || !loop->GetPreHeaderBlock() ||
    ...
    return relwrrent_node_map_[phi] = CreateCantComputeNode();
  ...
  // immediately in AnalyzeInstruction to break the relwrsion.
  relwrrent_node_map_[phi] = phi_node.get();
  ...
  for (uint32_t i = 0; i < phi->NumInOperands(); i += 2) {
    ...
    if (incoming_label_id == loop->GetPreHeaderBlock()->id()) {
      ...
      if (!operand_1->AsSERelwrrentNode())
        step_node = operand_1;
      else if (!operand_2->AsSERelwrrentNode())
        step_node = operand_2;

      // Find which node is the relwrrent expression.
      if (operand_1->AsSERelwrrentNode())
      ...
      if (!IsLoopIlwariant(loop, step_node))
        return relwrrent_node_map_[phi] = CreateCantComputeNode();
  ...
  return relwrrent_node_map_[phi] = GetCachedOrAdd(std::move(phi_node));
}
...
bool ScalarEvolutionAnalysis::IsLoopIlwariant(const Loop* loop,
                                              const SENode* node) const {
  for (auto itr = node->graph_cbegin(); itr != node->graph_cend(); ++itr) {
    if (const SERelwrrentNode* rec = itr->AsSERelwrrentNode()) {
      const BasicBlock* header = rec->GetLoop()->GetHeaderBlock();

      // If the loop which the relwrrent expression belongs to is either |loop
...
SENode* ScalarEvolutionAnalysis::GetCoefficientFromRelwrrentTerm(
    SENode* node, const Loop* loop) {
  // Traverse the DAG to find the relwrrent expression belonging to |loop|.
  for (auto itr = node->graph_begin(); itr != node->graph_end(); ++itr) {
    SERelwrrentNode* rec = itr->AsSERelwrrentNode();
...
// Rebuild the |node| eliminating, if it exists, the relwrrent term which
// belongs to the |loop|.
SENode* ScalarEvolutionAnalysis::BuildGraphWithoutRelwrrentTerm(
    SENode* node, const Loop* loop) {
  // If the node is already a relwrrent expression belonging to loop then just
  // return the offset.
  SERelwrrentNode* relwrrent = node->AsSERelwrrentNode();
  if (relwrrent) {
    if (relwrrent->GetLoop() == loop) {
      return relwrrent->GetOffset();
    } else {
      return node;
    }
  }
  ...
  std::vector<SENode*> new_children;
  // Otherwise find the relwrrent node in the children of this node.
  for (auto itr : *node) {
    relwrrent = itr->AsSERelwrrentNode();
    if (relwrrent && relwrrent->GetLoop() == loop) {
      new_children.push_back(relwrrent->GetOffset());
    } else {
...
// Return the relwrrent term belonging to |loop| if it appears in the graph
// starting at |node| or null if it doesn't.
SERelwrrentNode* ScalarEvolutionAnalysis::GetRelwrrentTerm(SENode* node,
                                                           const Loop* loop) {
  for (auto itr = node->graph_begin(); itr != node->graph_end(); ++itr) {
    SERelwrrentNode* rec = itr->AsSERelwrrentNode();
    if (rec && rec->GetLoop() == loop) {
...
std::string SENode::AsString() const {
  switch (GetType()) {
    case Constant:
      return "Constant";
    case RelwrrentAddExpr:
      return "RelwrrentAddExpr";
    case Add:
      return "Add";
    case Negative:
      return "Negative";
...
bool SENode::operator==(const SENode& other) const {
  ...
  const SERelwrrentNode* this_as_relwrrent = AsSERelwrrentNode();

  // Check the children are the same, for SERelwrrentNodes we need to check the
  // offset and coefficient manually as the child vector is sorted by ids so the
  // offset/coefficient information is lost.
  if (!this_as_relwrrent) {
    for (size_t index = 0; index < children_.size(); ++index) {
      if (other.GetChildren()[index] != children_[index]) return false;
    }
  } else {
    const SERelwrrentNode* other_as_relwrrent = other.AsSERelwrrentNode();

    // We've already checked the types are the same, this should not fail if
    // this->AsSERelwrrentNode() succeeded.
    assert(other_as_relwrrent);

    if (this_as_relwrrent->GetCoefficient() !=
        other_as_relwrrent->GetCoefficient())
      return false;

    if (this_as_relwrrent->GetOffset() != other_as_relwrrent->GetOffset())
      return false;

    if (this_as_relwrrent->GetLoop() != other_as_relwrrent->GetLoop())
      return false;
  }
...
size_t SENodeHash::operator()(const SENode* node) const {
  ...
  const SERelwrrentNode* relwrrent = node->AsSERelwrrentNode();

  // If we're dealing with a relwrrent expression hash the loop as well so that
  // nested inductions like i=0,i++ and j=0,j++ correspond to different nodes.
  if (relwrrent) {
    PushToString(reinterpret_cast<uintptr_t>(relwrrent->GetLoop()),
                 &hash_string);

    // Relwrrent expressions can't be hashed using the normal method as the
    // order of coefficient and offset matters to the hash.
    PushToString(reinterpret_cast<uintptr_t>(relwrrent->GetCoefficient()),
                 &hash_string);
    PushToString(reinterpret_cast<uintptr_t>(relwrrent->GetOffset()),
                 &hash_string);
...
void SENode::DumpDot(std::ostream& out, bool relwrse) const {
  ...
  for (const SENode* child : children_) {
    ...
    if (relwrse) child->DumpDot(out, true);
  }
}
...
class IsGreaterThanZero {
 ...
 private:
  ...
  Signedness Visit(const SENode* node) {
    switch (node->GetType()) {
      case SENode::Constant:
        ...
        break;
      case SENode::RelwrrentAddExpr:
        return Visit(node->AsSERelwrrentNode());
        break;
  ...
  // Returns the signedness of a relwrring expression.
  Signedness Visit(const SERelwrrentNode* node) {
    Signedness coeff_sign = Visit(node->GetCoefficient());
    // SERelwrrentNode represent an affine expression in the range [0,
