...
namespace {
using Instructiolwector = std::vector<const spvtools::opt::Instruction*>;
...
bool DecorationManager::HaveTheSameDecorations(uint32_t id1,
                                               uint32_t id2) const {
  const Instructiolwector decorations_for1 = GetDecorationsFor(id1, false);
  const Instructiolwector decorations_for2 = GetDecorationsFor(id2, false);
  ...
  const auto fillDecorationSets =
      [](const Instructiolwector& decoration_list, DecorationSet* decorate_set,
...
void DecorationManager::AddDecoratiolwal(uint32_t inst_id, uint32_t decoration,
