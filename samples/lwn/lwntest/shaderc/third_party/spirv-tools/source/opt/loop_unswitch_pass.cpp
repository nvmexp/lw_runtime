...
//  - The loop ilwariant condition is not uniform.
class LoopUnswitch {
 public:
  ...
  //  loop;
  //  - The loop ilwariant condition is uniform;
  bool CanUnswitchLoop() {
    ...
    for (uint32_t bb_id : loop_->GetBlocks()) {
      ...
      if (bb->terminator()->IsBranch() &&
        ...
        if (IsConditionNonConstantLoopIlwariant(bb->terminator())) {
          switch_block_ = bb;
          break;
        }
      }
  ...
  void PerformUnswitch() {
    ...
    if (loop_merge_block) {
      ...
      // Copy the predecessor list (will get ilwalidated otherwise).
    ...
    for (auto& specialisation_pair : constant_branch) {
...

      {
        ...
        ///////////////////////////////////////////////////////////
        // Step 5: Connect colwergent edges to the landing pads. //
        ///////////////////////////////////////////////////////////
    ...
    if (iv_opcode == SpvOpBranchConditional) {
      ...
      builder.AddConditionalBranch(
          ...
          if_merge_block ? if_merge_block->id() : kIlwalidId);
    } else {
 ...
 private:
  ...
  // Note: this is lwrrently approximated as uniform.
  ...
  // Returns true if |insn| is not a constant, but is loop ilwariant and
  // dynamically uniform.
  bool IsConditionNonConstantLoopIlwariant(Instruction* insn) {
