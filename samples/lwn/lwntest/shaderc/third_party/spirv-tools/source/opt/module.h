...
class Module {
 public:
  ...
  // Iterators for exelwtion_modes instructions contained in this module.
  inline inst_iterator exelwtion_mode_begin();
  inline inst_iterator exelwtion_mode_end();
  inline IteratorRange<inst_iterator> exelwtion_modes();
  inline IteratorRange<const_inst_iterator> exelwtion_modes() const;
  ...
  // Ilwokes function |f| on all instructions in this module, and optionally on
 ...
 private:
  ...
  InstructionList exelwtion_modes_;
...
inline void Module::AddExelwtionMode(std::unique_ptr<Instruction> e) {
  exelwtion_modes_.push_back(std::move(e));
}
...
inline Module::inst_iterator Module::exelwtion_mode_begin() {
  return exelwtion_modes_.begin();
}
inline Module::inst_iterator Module::exelwtion_mode_end() {
  return exelwtion_modes_.end();
}

inline IteratorRange<Module::inst_iterator> Module::exelwtion_modes() {
  return make_range(exelwtion_modes_.begin(), exelwtion_modes_.end());
}

inline IteratorRange<Module::const_inst_iterator> Module::exelwtion_modes()
    const {
  return make_range(exelwtion_modes_.begin(), exelwtion_modes_.end());
}
