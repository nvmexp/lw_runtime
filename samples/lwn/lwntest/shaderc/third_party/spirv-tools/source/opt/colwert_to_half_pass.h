...
#ifndef LIBSPIRV_OPT_COLWERT_TO_HALF_PASS_H_
#define LIBSPIRV_OPT_COLWERT_TO_HALF_PASS_H_
...
class ColwertToHalfPass : public Pass {
 public:
  ColwertToHalfPass() : Pass() {}

  ~ColwertToHalfPass() override = default;
 ...
 private:
  ...
  // Append instructions to builder to colwert value |*val_idp| to type
  // |ty_id| but with |width|. Set |*val_idp| to the new id.
  void GenColwert(uint32_t* val_idp, uint32_t width, Instruction* inst);
  ...
  // If |inst| is an arithmetic, phi, extract or colwert instruction of float32
  ...
  // colwert all operands to float16 (if needed) and change its type to the
  // equivalent float16 type. Otherwise, insert instructions to colwert its
  ...
  // Gen code for relaxed colwert |inst|
  bool ProcessColwert(Instruction* inst);
  ...
  // If |inst| is an FColwert of a matrix type, decompose it to a series
  // of vector extracts, colwerts and inserts into an Undef. These are
  // generated by GenHalfInst because they are easier to manipulate, but are
  // invalid so we need to clean them up.
  bool MatColwertCleanup(Instruction* inst);
  ...
  // Ids of all colwerted instructions
  std::unordered_set<uint32_t> colwerted_ids_;
};

}  // namespace opt
}  // namespace spvtools

#endif  // LIBSPIRV_OPT_COLWERT_TO_HALF_PASS_H_
