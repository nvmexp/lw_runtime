...
class Loop {
 ...
 public:
  ...
  void GetInductiolwariables(std::vector<Instruction*>& inductions) const;
  ...
  Instruction* FindConditiolwariable(const BasicBlock* condition) const;
  ...
  // Returns true is the instruction is ilwariant and safe to move wrt loop
  ...
  static int64_t GetResidualConditiolwalue(SpvOp condition,
 ...
 private:
  ...
  // Each differnt loop |condition| affects how we callwlate the number of
  ...
  // This is to allow for loops to be removed mid iteration without ilwalidating
  // the iterators.
