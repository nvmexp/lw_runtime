...
class UptrVectorIterator
 ...
 public:
  ...
  // ilwalidated. Otherwise, those previous iterators pointing to after the
  // insertion point will be ilwalidated.
  ...
  // If the underlying vector changes capacity, all previous iterators will be
  // ilwalidated. Otherwise, those previous iterators pointing to after the
  // erasure point will be ilwalidated.
  template <bool IsConstForMethod = IsConst>
  inline typename std::enable_if<!IsConstForMethod, UptrVectorIterator>::type
  Erase();
...
//
// Lwrrently this iterator is always an input iterator.
...
class FilterIterator
 ...
 public:
  ...
  reference operator*() const { return *lwr_; }
  pointer operator->() { return &*lwr_; }

  inline bool operator==(const FilterIterator& rhs) const {
    return lwr_ == rhs.lwr_ && end_ == rhs.end_;
  }
  inline bool operator!=(const FilterIterator& rhs) const {
    return !(*this == rhs);
  }
  ...
  // Returns the underlying iterator.
  SubIterator Get() const { return lwr_; }
 ...
 private:
  ...
  // the end.
  bool IsPredicateSatisfied() { return lwr_ == end_ || predicate_(*lwr_); }
