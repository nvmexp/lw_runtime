...
class AssemblyGrammar {
 public:
  explicit AssemblyGrammar(const spv_const_context context)
      : target_elw_(context->target_elw),
  ...
  spv_target_elw target_elw() const { return target_elw_; }

  // Removes capabilities not available in the current target environment and
  // returns the rest.
  CapabilitySet filterCapsAgainstTargetElw(const SpvCapability* cap_array,
  ...
  // SPV_ERROR_ILWALID_LOOKUP if the opcode does not exist.
  ...
  // SPV_ERROR_ILWALID_LOOKUP otherwise.
  ...
  // parameter.  On failure, returns SPV_ERROR_ILWALID_LOOKUP.
