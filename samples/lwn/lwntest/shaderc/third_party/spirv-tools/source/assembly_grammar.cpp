...
///
/// @return result code
spv_result_t spvTextParseMaskOperand(spv_target_elw elw,
                                     const spv_operand_table operandTable,
                                     const spv_operand_type_t type,
                                     const char* textValue, uint32_t* pValue) {
  if (textValue == nullptr) return SPV_ERROR_ILWALID_TEXT;
  size_t text_length = strlen(textValue);
  if (text_length == 0) return SPV_ERROR_ILWALID_TEXT;
  ...
  do {
    ...
    if (spvOperandTableNameLookup(elw, operandTable, type, begin, end - begin,
                                  &entry)) {
      return SPV_ERROR_ILWALID_TEXT;
    }
    value |= entry->value;
...
const SpecConstantOpcodeEntry kOpSpecConstantOpcodes[] = {
    ...
    CASE(CooperativeMatrixLengthLW)
};
...
CapabilitySet AssemblyGrammar::filterCapsAgainstTargetElw(
...
spv_result_t AssemblyGrammar::lookupOpcode(const char* name,
  ...
  return spvOpcodeTableNameLookup(target_elw_, opcodeTable_, name, desc);
}

spv_result_t AssemblyGrammar::lookupOpcode(SpvOp opcode,
                                           spv_opcode_desc* desc) const {
  return spvOpcodeTableValueLookup(target_elw_, opcodeTable_, opcode, desc);
}

spv_result_t AssemblyGrammar::lookupOperand(spv_operand_type_t type,
                                            const char* name, size_t name_len,
                                            spv_operand_desc* desc) const {
  return spvOperandTableNameLookup(target_elw_, operandTable_, type, name,
                                   name_len, desc);
}
...
spv_result_t AssemblyGrammar::lookupOperand(spv_operand_type_t type,
                                            uint32_t operand,
                                            spv_operand_desc* desc) const {
  return spvOperandTableValueLookup(target_elw_, operandTable_, type, operand,
                                    desc);
}
...
spv_result_t AssemblyGrammar::lookupSpecConstantOpcode(const char* name,
  ...
  if (found == last) return SPV_ERROR_ILWALID_LOOKUP;
  *opcode = found->opcode;
  return SPV_SUCCESS;
}
...
spv_result_t AssemblyGrammar::parseMaskOperand(const spv_operand_type_t type,
  ...
  return spvTextParseMaskOperand(target_elw_, operandTable_, type, textValue,
                                 pValue);
}
...
void AssemblyGrammar::pushOperandTypesForMask(
  ...
  spvPushOperandTypesForMask(target_elw_, operandTable_, type, mask, pattern);
}
