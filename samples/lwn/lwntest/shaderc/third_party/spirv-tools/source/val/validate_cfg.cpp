...
spv_result_t ValidateBranchConditional(ValidationState_t& _,
  ...
  if (!cond_op || !cond_op->type_id() ||
    ...
    return _.diag(SPV_ERROR_ILWALID_ID, inst) << "Condition operand for "
...
spv_result_t ValidateLoopMerge(ValidationState_t& _, const Instruction* inst) {
  ...
  if ((loop_control >> SpvLoopControlDontUnrollShift) & 0x1 &&
    ...
    return _.diag(SPV_ERROR_ILWALID_DATA, inst) << "PeelCount and DontUnroll "
                                                   "loop controls must not "
                                                   "both be specified";
  }
  if ((loop_control >> SpvLoopControlDontUnrollShift) & 0x1 &&
      (loop_control >> SpvLoopControlPartialCountShift) & 0x1) {
    return _.diag(SPV_ERROR_ILWALID_DATA, inst) << "PartialCount and "
  ...
  if ((loop_control >> SpvLoopControlIterationMultipleShift) & 0x1) {
    if (inst->operands().size() < operand ||
      ...
      return _.diag(SPV_ERROR_ILWALID_DATA, inst) << "IterationMultiple loop "
...
spv_result_t FirstBlockAssert(ValidationState_t& _, uint32_t target) {
  if (_.lwrrent_function().IsFirstBlock(target)) {
    return _.diag(SPV_ERROR_ILWALID_CFG, _.FindDef(_.lwrrent_function().id()))
           << "First block " << _.getIdName(target) << " of function "
           << _.getIdName(_.lwrrent_function().id()) << " is targeted by block "
           << _.getIdName(_.lwrrent_function().lwrrent_block()->id());
  }
  return SPV_SUCCESS;
}

spv_result_t MergeBlockAssert(ValidationState_t& _, uint32_t merge_block) {
  if (_.lwrrent_function().IsBlockType(merge_block, kBlockTypeMerge)) {
    return _.diag(SPV_ERROR_ILWALID_CFG, _.FindDef(_.lwrrent_function().id()))
...
// |case_fall_through|. Returns SPV_ERROR_ILWALID_CFG if the case construct
  ...
  while (!stack.empty()) {
    ...
    if (target_reachable && block->reachable() &&
      ...
      if (!case_targets.count(block->id())) {
        ...
        return _.diag(SPV_ERROR_ILWALID_CFG, target_block->label())
...
spv_result_t StructuredSwitchChecks(ValidationState_t& _, Function* function,
  ...
  for (uint32_t i = 1; i < switch_inst->operands().size(); i += 2) {
    ...
    if (seen_iter == seen_to_fall_through.end()) {
      ...
      if (header->reachable() && target_block->reachable() &&
        ...
        return _.diag(SPV_ERROR_ILWALID_CFG, header->label())
    ...
    if (case_fall_through != 0u) {
      ...
      if (is_default) {
        ...
        if ((switch_inst->operands().size() < j + 2) ||
          ...
          return _.diag(SPV_ERROR_ILWALID_CFG, switch_inst)
  ...
  for (const auto& pair : num_fall_through_targeted) {
    if (pair.second > 1) {
      return _.diag(SPV_ERROR_ILWALID_CFG, _.FindDef(pair.first))
  ...
  for (auto iter = postorder.rbegin(); iter != postorder.rend(); ++iter) {
    ...
    if (terminator->opcode() == SpvOpBranchConditional) {
      ...
      if (!merge && both_unseen) {
        return _.diag(SPV_ERROR_ILWALID_CFG, terminator)
...
spv_result_t StructuredControlFlowChecks(
  ...
  for (auto back_edge : back_edges) {
    ...
    if (!function->IsBlockType(header_block, kBlockTypeLoop)) {
      return _.diag(SPV_ERROR_ILWALID_CFG, _.FindDef(back_edge_block))
  ...
  for (BasicBlock* loop_header : function->ordered_blocks()) {
    ...
    if (num_latch_blocks != 1) {
      return _.diag(SPV_ERROR_ILWALID_CFG, _.FindDef(loop_header_id))
  ...
  for (const Construct& construct : function->constructs()) {
    ...
    if (merge && merge->reachable()) {
      if (!header->dominates(*merge)) {
        return _.diag(SPV_ERROR_ILWALID_CFG, _.FindDef(merge->id()))
    ...
    for (auto block : construct_blocks) {
      ...
      for (auto succ : *block->successors()) {
        if (block->reachable() && !construct_blocks.count(succ) &&
          ...
          return _.diag(SPV_ERROR_ILWALID_CFG, _.FindDef(block->id()))
      ...
      for (auto pred : *block->predecessors()) {
        if (pred->reachable() && !construct_blocks.count(pred)) {
          return _.diag(SPV_ERROR_ILWALID_CFG, _.FindDef(pred->id()))
...
spv_result_t PerformCfgChecks(ValidationState_t& _) {
  for (auto& function : _.functions()) {
    ...
    if (function.undefined_block_count() != 0) {
      ...
      return _.diag(SPV_ERROR_ILWALID_CFG, _.FindDef(function.id()))
    ...
    if (!function.ordered_blocks().empty()) {
      ...
      CFA<BasicBlock>::DepthFirstTraversal(
          ...
          ignore_edge);
      auto edges = CFA<BasicBlock>::CallwlateDominators(
      ...
      for (auto block = begin(blocks) + 1; block != end(blocks); ++block) {
        if (auto idom = (*block)->immediate_dominator()) {
          if (idom != function.pseudo_entry_block() &&
            ...
            return _.diag(SPV_ERROR_ILWALID_CFG, _.FindDef(idom->id()))
...
spv_result_t CfgPass(ValidationState_t& _, const Instruction* inst) {
  ...
  switch (opcode) {
    case SpvOpLabel:
      if (auto error = _.lwrrent_function().RegisterBlock(inst->id()))
        return error;
      ...
      _.lwrrent_function().lwrrent_block()->set_label(inst);
      break;
    case SpvOpLoopMerge: {
      ...
      if (auto error = _.lwrrent_function().RegisterLoopMerge(merge_block,
                                                              continue_block))
        return error;
    } break;
    case SpvOpSelectionMerge: {
      ...
      if (auto error = _.lwrrent_function().RegisterSelectionMerge(merge_block))
        return error;
    } break;
    case SpvOpBranch: {
      ...
      _.lwrrent_function().RegisterBlockEnd({target}, opcode);
    } break;
    case SpvOpBranchConditional: {
      ...
      _.lwrrent_function().RegisterBlockEnd({tlabel, flabel}, opcode);
    } break;
    ...
    case SpvOpSwitch: {
      ...
      _.lwrrent_function().RegisterBlockEnd({cases}, opcode);
    } break;
    case SpvOpReturn: {
      const uint32_t return_type = _.lwrrent_function().GetResultTypeId();
      const Instruction* return_type_inst = _.FindDef(return_type);
      assert(return_type_inst);
      if (return_type_inst->opcode() != SpvOpTypeVoid)
        return _.diag(SPV_ERROR_ILWALID_CFG, inst)
               << "OpReturn can only be called from a function with void "
               << "return type.";
      _.lwrrent_function().RegisterBlockEnd(std::vector<uint32_t>(), opcode);
      break;
    }
    case SpvOpKill:
    ...
    case SpvOpUnreachable:
      _.lwrrent_function().RegisterBlockEnd(std::vector<uint32_t>(), opcode);
      if (opcode == SpvOpKill) {
        _.lwrrent_function().RegisterExelwtionModelLimitation(
            SpvExelwtionModelFragment,
            "OpKill requires Fragment exelwtion model");
      }
      break;
    default:
      break;
...
spv_result_t ControlFlowPass(ValidationState_t& _, const Instruction* inst) {
  switch (inst->opcode()) {
    ...
    case SpvOpBranchConditional:
      ...
      break;
    case SpvOpReturlwalue:
      if (auto error = ValidateReturlwalue(_, inst)) return error;
      break;
    case SpvOpSwitch:
