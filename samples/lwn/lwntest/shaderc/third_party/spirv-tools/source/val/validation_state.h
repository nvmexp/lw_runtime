...
enum ModuleLayoutSection {
  ...
  kLayoutExelwtionMode,         /// < Section 2.4 #6
...
class ValidationState_t {
 public:
  ...
  struct Feature {
    ...
    bool elw_relaxed_block_layout = false;
    ...
    // SPIR-V 1.4 allows UColwert as a spec constant op in any environment.
    // The Kernel capability already enables it, separately from this flag.
    bool ucolwert_spec_constant_op = false;
  ...
  /// Allocates internal storage. Note, calling this will ilwalidate any
  ...
  ModuleLayoutSection lwrrent_layout_section() const;

  /// Increments the module_layout_order_section_
  void ProgressToNextLayoutSectionOrder();

  /// Determines if the op instruction is part of the current section
  bool IsOpcodeInLwrrentLayoutSection(SpvOp op);
  ...
  /// Returns the function states
  Function& lwrrent_function();
  const Function& lwrrent_function() const;
  ...
  /// Registers |id| as an entry point with |exelwtion_model| and |interfaces|.
  void RegisterEntryPoint(const uint32_t id, SpvExelwtionModel exelwtion_model,
                          EntryPointDescription&& desc) {
    entry_points_.push_back(id);
    entry_point_to_exelwtion_models_[id].insert(exelwtion_model);
  ...
  /// relwrsion.
  const std::set<uint32_t>& relwrsive_entry_points() const {
    return relwrsive_entry_points_;
  }

  /// Registers exelwtion mode for the given entry point.
  void RegisterExelwtionModeForEntryPoint(uint32_t entry_point,
                                          SpvExelwtionMode exelwtion_mode) {
    entry_point_to_exelwtion_modes_[entry_point].insert(exelwtion_mode);
  }
  ...
  /// Returns Exelwtion Models for the given Entry Point.
  /// Returns nullptr if none found (would trigger assertion).
  const std::set<SpvExelwtionModel>* GetExelwtionModels(
      uint32_t entry_point) const {
    const auto it = entry_point_to_exelwtion_models_.find(entry_point);
    if (it == entry_point_to_exelwtion_models_.end()) {
      assert(0);
      return nullptr;
    }
    return &it->second;
  ...
  /// Returns Exelwtion Modes for the given Entry Point.
  /// Returns nullptr if none found.
  const std::set<SpvExelwtionMode>* GetExelwtionModes(
      uint32_t entry_point) const {
    const auto it = entry_point_to_exelwtion_modes_.find(entry_point);
    if (it == entry_point_to_exelwtion_modes_.end()) {
      return nullptr;
    }
    return &it->second;
  }
  ...
  void AddFunctionCallTarget(const uint32_t id) {
    ...
    lwrrent_function().AddFunctionCallTarget(id);
  }
  ...
  void RegisterDecorationsForId(uint32_t id, InputIt begin, InputIt end) {
    std::vector<Decoration>& lwr_decs = id_decorations_[id];
    lwr_decs.insert(lwr_decs.end(), begin, end);
  }
  ...
  bool IsRelaxedBlockLayout() const {
    return features_.elw_relaxed_block_layout || options()->relax_block_layout;
  }
  ...
  // Should not be called with any other arguments (will return zero and ilwoke
  // assertion).
  ...
  // - scalar, vector or matrix type
  // - object of either scalar, vector or matrix type
  // Will ilwoke assertion and return 0 if |id| is none of the above.
 ...
 private:
  ...
  ModuleLayoutSection lwrrent_layout_section_;
  ...
  /// graph that relwrses.
  std::set<uint32_t> relwrsive_entry_points_;
  ...
  /// Mapping entry point -> exelwtion models. It is presumed that the same
  /// function could theoretically be used as 'main' by multiple OpEntryPoint
  /// instructions.
  std::unordered_map<uint32_t, std::set<SpvExelwtionModel>>
      entry_point_to_exelwtion_models_;

  /// Mapping entry point -> exelwtion modes.
  std::unordered_map<uint32_t, std::set<SpvExelwtionMode>>
      entry_point_to_exelwtion_modes_;
