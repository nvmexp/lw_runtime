...
class Function {
 public:
  ...
  /// Returns the block that is lwrrently being parsed in the binary
  BasicBlock* lwrrent_block();

  /// Returns the block that is lwrrently being parsed in the binary
  const BasicBlock* lwrrent_block() const;

  // For dominance callwlations, we want to analyze all the
  ...
  /// Registers exelwtion model limitation such as "Feature X is only available
  /// with Exelwtion Model Y".
  void RegisterExelwtionModelLimitation(SpvExelwtionModel model,
                                        const std::string& message);

  /// Registers exelwtion model limitation with an |is_compatible| functor.
  void RegisterExelwtionModelLimitation(
      std::function<bool(SpvExelwtionModel, std::string*)> is_compatible) {
    exelwtion_model_limitations_.push_back(is_compatible);
  }
  ...
  /// Returns true if the given exelwtion model passes the limitations stored in
  /// exelwtion_model_limitations_. Returns false otherwise and fills optional
  /// |reason| parameter.
  bool IsCompatibleWithExelwtionModel(SpvExelwtionModel model,
 ...
 private:
  ...
  /// The block that is lwrrently being parsed
  BasicBlock* lwrrent_block_;
  ...
  ///     ending with an OpReturn, OpReturlwalue or similar instructions.
  ///   - A single node from each otherwise unreachable cycle in the CFG, if
  ///     such cycles exist.
  ...
  /// Stores exelwtion model limitations imposed by instructions used within the
  /// function. The functor stored in the list return true if exelwtion model
  ...
  std::list<std::function<bool(SpvExelwtionModel, std::string*)>>
      exelwtion_model_limitations_;

  /// Stores limitations imposed by instructions used within the function.
  /// Similar to exelwtion_model_limitations_;
