...
static const uint32_t kIlwalidId = 0x400000;
...
spv_result_t Function::RegisterLoopMerge(uint32_t merge_id,
  ...
  merge_block_header_[&merge_block] = lwrrent_block_;
  if (continue_target_headers_.find(&continue_target_block) ==
      continue_target_headers_.end()) {
    continue_target_headers_[&continue_target_block] = {lwrrent_block_};
  } else {
    continue_target_headers_[&continue_target_block].push_back(lwrrent_block_);
  }
...
spv_result_t Function::RegisterSelectionMerge(uint32_t merge_id) {
  RegisterBlock(merge_id, false);
  BasicBlock& merge_block = blocks_.at(merge_id);
  lwrrent_block_->set_type(kBlockTypeSelection);
  merge_block.set_type(kBlockTypeMerge);
  merge_block_header_[&merge_block] = lwrrent_block_;

  AddConstruct({ConstructType::kSelection, lwrrent_block(), &merge_block});
...
spv_result_t Function::RegisterBlock(uint32_t block_id, bool is_definition) {
  ...
  if (is_definition) {  // new block definition
    ...
    lwrrent_block_ = &inserted_block->second;
    ordered_blocks_.push_back(lwrrent_block_);
    if (IsFirstBlock(block_id)) lwrrent_block_->set_reachable(true);
...
void Function::RegisterBlockEnd(std::vector<uint32_t> next_list,
  ...
  if (lwrrent_block_->is_type(kBlockTypeLoop)) {
    ...
    std::vector<BasicBlock*>& next_blocks_plus_continue_target =
        loop_header_successors_plus_continue_target_map_[lwrrent_block_];
    next_blocks_plus_continue_target = next_blocks;
    auto continue_target =
        FindConstructForEntryBlock(lwrrent_block_, ConstructType::kLoop)
            .corresponding_constructs()
            .back()
            ->entry_block();
    if (continue_target != lwrrent_block_) {
      next_blocks_plus_continue_target.push_back(continue_target);
    }
  }

  lwrrent_block_->RegisterBranchInstruction(branch_instruction);
  lwrrent_block_->RegisterSuccessors(next_blocks);
  lwrrent_block_ = nullptr;
  return;
}
...
const BasicBlock* Function::lwrrent_block() const { return lwrrent_block_; }
BasicBlock* Function::lwrrent_block() { return lwrrent_block_; }
...
int Function::GetBlockDepth(BasicBlock* bb) {
  ...
  // Only callwlate the depth if it's not already callwlated.
  // This function uses memoization to avoid duplicate CFG depth callwlations.
...
void Function::RegisterExelwtionModelLimitation(SpvExelwtionModel model,
                                                const std::string& message) {
  exelwtion_model_limitations_.push_back(
      [model, message](SpvExelwtionModel in_model, std::string* out_message) {
        if (model != in_model) {
          if (out_message) {
            *out_message = message;
          }
...
bool Function::IsCompatibleWithExelwtionModel(SpvExelwtionModel model,
                                              std::string* reason) const {
  bool return_value = true;
  std::stringstream ss_reason;

  for (const auto& is_compatible : exelwtion_model_limitations_) {
    std::string message;
