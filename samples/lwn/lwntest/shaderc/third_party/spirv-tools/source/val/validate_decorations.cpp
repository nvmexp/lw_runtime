...
bool isBuiltIlwar(uint32_t var_id, ValidationState_t& vstate) {
  ...
  return std::any_of(
      decorations.begin(), decorations.end(), [](const Decoration& d) {
        return SpvDecorationBuiltIn == d.dec_type() &&
               Decoration::kIlwalidMember != d.struct_member_index();
      });
}
...
bool isMissingOffsetInStruct(uint32_t struct_id, ValidationState_t& vstate) {
  ...
  for (auto& decoration : vstate.id_decorations(struct_id)) {
    if (SpvDecorationOffset == decoration.dec_type() &&
        Decoration::kIlwalidMember != decoration.struct_member_index()) {
...
spv_result_t checkLayout(uint32_t struct_id, const char* storage_class_str,
  ...
  auto fail = [&vstate, struct_id, storage_class_str, decoration_str,
    ...
    DiagnosticStream ds =
        std::move(vstate.diag(SPV_ERROR_ILWALID_ID, vstate.FindDef(struct_id))
  ...
  for (size_t ordered_member_idx = 0;
    ...
    // Check struct members relwrsively.
    spv_result_t relwrsive_status = SPV_SUCCESS;
    if (SpvOpTypeStruct == opcode &&
        SPV_SUCCESS != (relwrsive_status = checkLayout(
                            id, storage_class_str, decoration_str, blockRules,
                            offset, constraints, vstate)))
      return relwrsive_status;
    // Check matrix stride.
    ...
    // Check arrays and runtime arrays relwrsively.
    ...
    while (array_inst->opcode() == SpvOpTypeArray ||
      ...
      // Check each element relwrsively if it is a struct. There is a
  ...
  for (const auto& function : vstate.functions()) {
    if (function.block_count() == 0u) {
      ...
      if (!hasImportLinkageAttribute(function.id(), vstate)) {
        return vstate.diag(SPV_ERROR_ILWALID_BINARY,
...
spv_result_t CheckImportedVariableInitialization(ValidationState_t& vstate) {
  ...
  for (auto global_var_id : vstate.global_vars()) {
    ...
    if (variable_instr->words().size() == 5u &&
      ...
      return vstate.diag(SPV_ERROR_ILWALID_ID, variable_instr)
...
spv_result_t CheckBuiltIlwariable(uint32_t var_id, ValidationState_t& vstate) {
  const auto& decorations = vstate.id_decorations(var_id);
  for (const auto& d : decorations) {
    if (spvIsVulkanElw(vstate.context()->target_elw)) {
      if (d.dec_type() == SpvDecorationLocation ||
          d.dec_type() == SpvDecorationComponent) {
        return vstate.diag(SPV_ERROR_ILWALID_ID, vstate.FindDef(var_id))
    ...
    for (const auto& desc : descs) {
      ...
      for (auto interface : desc.interfaces) {
        ...
        if (!var_instr || SpvOpVariable != var_instr->opcode()) {
          return vstate.diag(SPV_ERROR_ILWALID_ID, var_instr)
        ...
        if (type_instr && SpvOpTypeStruct == type_instr->opcode() &&
          ...
          if (auto error = CheckBuiltIlwariable(interface, vstate))
            return error;
        } else if (isBuiltIlwar(interface, vstate)) {
          if (auto error = CheckBuiltIlwariable(interface, vstate))
            return error;
        }
      }
...
spv_result_t CheckDecorationsOfBuffers(ValidationState_t& vstate) {
  ...
  for (const auto& inst : vstate.ordered_instructions()) {
    ...
    if (SpvOpVariable == inst.opcode()) {
      ...
      if (spvIsOpenGLElw(vstate.context()->target_elw)) {
      ...
      if (uniform || push_constant || storage_buffer || phys_storage_buffer) {
        ...
        if (spvIsVulkanElw(vstate.context()->target_elw)) {
          ...
          if (push_constant && !block) {
            return vstate.diag(SPV_ERROR_ILWALID_ID, vstate.FindDef(id))
                   << "PushConstant id '" << id
...
spv_result_t CheckFPRoundingModeForShaders(ValidationState_t& vstate,
  ...
  // Validates width-only colwersion instruction for floating-point object
  // i.e., OpFColwert
  if (inst.opcode() != SpvOpFColwert) {
    return vstate.diag(SPV_ERROR_ILWALID_ID, &inst)
           << "FPRoundingMode decoration can be applied only to a "
              "width-only colwersion instruction for floating-point "
              "object.";
  }
  ...
  for (const auto& use : inst.uses()) {
    ...
    if (store->opcode() == SpvOpFColwert) continue;
...
spv_result_t CheckUniformDecoration(ValidationState_t& vstate,
  ...
  if (decoration.dec_type() == SpvDecorationUniformId) {
    ...
    // The scope id is an exelwtion scope.
    if (auto error =
            ValidateExelwtionScope(vstate, &inst, decoration.params()[0]))
      return error;
  }
