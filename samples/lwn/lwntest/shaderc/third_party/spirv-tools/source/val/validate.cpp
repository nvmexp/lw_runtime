...
spv_result_t ValidateForwardDecls(ValidationState_t& _) {
  ...
  auto id_str = ss.str();
  return _.diag(SPV_ERROR_ILWALID_ID, nullptr)
         << "The following forward referenced IDs have not been defined:\n"
         << id_str.substr(0, id_str.size() - 1);
}

std::vector<std::string> CallwlateNamesForEntryPoint(ValidationState_t& _,
                                                     const uint32_t id) {
...
spv_result_t ValidateEntryPointNameUnique(ValidationState_t& _,
                                          const uint32_t id) {
  auto id_names = CallwlateNamesForEntryPoint(_, id);
  const auto names =
  ...
  if (id_names.size() != names.size()) {
    ...
    for (size_t i = 0; i < id_names.size() - 1; i++) {
      if (id_names[i] == id_names[i + 1]) {
        return _.diag(SPV_ERROR_ILWALID_BINARY, _.FindDef(id))
               << "Entry point name \"" << id_names[i]
               << "\" is not unique, which is not allow in WebGPU elw.";
      }
    }
  }
  ...
  for (const auto other_id : _.entry_points()) {
    ...
    const auto other_id_names = CallwlateNamesForEntryPoint(_, other_id);
...
spv_result_t ValidateEntryPoints(ValidationState_t& _) {
  ...
  for (const auto& entry_point : _.entry_points()) {
    if (_.IsFunctionCallTarget(entry_point)) {
      return _.diag(SPV_ERROR_ILWALID_BINARY, _.FindDef(entry_point))
    ...
    if (spvIsVulkanOrWebGPUElw(_.context()->target_elw)) {
      if (_.relwrsive_entry_points().find(entry_point) !=
          _.relwrsive_entry_points().end()) {
        return _.diag(SPV_ERROR_ILWALID_BINARY, _.FindDef(entry_point))
...
spv_result_t ValidateBinaryUsingContextAndValidationState(
  ...
  if (spvIsWebGPUElw(context.target_elw) && endian != SPV_ENDIANNESS_LITTLE) {
    return DiagnosticStream(position, context.consumer, "",
                            SPV_ERROR_ILWALID_BINARY)
  ...
  if (header.version > spvVersionForTargetElw(context.target_elw)) {
    return DiagnosticStream(position, context.consumer, "",
           ...
           << spvTargetElwDescription(context.target_elw) << ".";
  }
  ...
  for (auto& instruction : vstate->ordered_instructions()) {
    {
      ...
      if (inst->opcode() == SpvOpEntryPoint) {
        ...
        const auto exelwtion_model = inst->GetOperandAs<SpvExelwtionModel>(0);
        ...
        vstate->RegisterEntryPoint(entry_point, exelwtion_model,
                                   std::move(desc));

        if (visited_entry_points.size() > 0) {
          for (const Instruction* check_inst : visited_entry_points) {
            const auto check_exelwtion_model =
                check_inst->GetOperandAs<SpvExelwtionModel>(0);
            ...
            if (desc.name == check_name &&
                exelwtion_model == check_exelwtion_model) {
              return vstate->diag(SPV_ERROR_ILWALID_DATA, inst)
      ...
      if (inst->opcode() == SpvOpFunctionCall) {
        if (!vstate->in_function_body()) {
          return vstate->diag(SPV_ERROR_ILWALID_LAYOUT, &instruction)
      ...
      if (vstate->in_function_body()) {
        inst->set_function(&(vstate->lwrrent_function()));
        inst->set_block(vstate->lwrrent_function().lwrrent_block());

        if (vstate->in_block() && spvOpcodeIsBlockTerminator(inst->opcode())) {
          vstate->lwrrent_function().lwrrent_block()->set_terminator(inst);
        }
      }
  ...
  if (!vstate->has_memory_model_specified())
    return vstate->diag(SPV_ERROR_ILWALID_LAYOUT, nullptr)
  ...
  for (size_t i = 0; i < vstate->ordered_instructions().size(); ++i) {
    ...
    if (auto error = ColwersionPass(*vstate, &instruction)) return error;
  ...
  // Validate the preconditions ilwolving adjacent instructions. e.g. SpvOpPhi
  ...
  for (const auto& inst : vstate->ordered_instructions()) {
    if (auto error = ValidateExelwtionLimitations(*vstate, &inst)) return error;
