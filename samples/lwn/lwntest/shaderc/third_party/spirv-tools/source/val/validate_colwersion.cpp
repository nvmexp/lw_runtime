...
spv_result_t ColwersionPass(ValidationState_t& _, const Instruction* inst) {
  ...
  switch (opcode) {
    ...
    case SpvOpSatColwertSToU:
    case SpvOpSatColwertUToS: {
      if (!_.IsIntScalarType(result_type) && !_.IsIntVectorType(result_type))
        return _.diag(SPV_ERROR_ILWALID_DATA, inst)
               << "Expected int scalar or vector type as Result Type: "
               << spvOpcodeString(opcode);
    ...
    case SpvOpBitcast: {
      ...
      if (_.version() >= SPV_SPIRV_VERSION_WORD(1, 5) ||
        ...
        if (input_is_pointer && !result_is_pointer && !result_is_int_scalar &&
          ...
          return _.diag(SPV_ERROR_ILWALID_DATA, inst)
                 << "Pointer can only be colwerted to another pointer, int "
        ...
        if (input_is_pointer && !result_is_pointer && !result_is_int_scalar)
          return _.diag(SPV_ERROR_ILWALID_DATA, inst)
                 << "Pointer can only be colwerted to another pointer or int "
                    "scalar: "
                 << spvOpcodeString(opcode);
      }
