...
uint32_t GetPlaneCoordSize(const ImageTypeInfo& info) {
  ...
  switch (info.dim) {
    ...
    case SpvDim3D:
    case SpvDimLwbe:
      // For Lwbe direction vector is used instead of UV.
      plane_size = 3;
      break;
    case SpvDimMax:
      assert(0);
...
uint32_t GetMinCoordSize(SpvOp opcode, const ImageTypeInfo& info) {
  if (info.dim == SpvDimLwbe &&
      (opcode == SpvOpImageRead || opcode == SpvOpImageWrite ||
       opcode == SpvOpImageSparseRead)) {
    // These opcodes use UV for Lwbe, not direction vector.
    return 3;
  }
...
spv_result_t ValidateImageOperands(ValidationState_t& _,
  ...
  if (mask & SpvImageOperandsConstOffsetMask) {
    if (info.dim == SpvDimLwbe) {
      return _.diag(SPV_ERROR_ILWALID_DATA, inst)
             << "Image Operand ConstOffset cannot be used with Lwbe Image "
                "'Dim'";
    }
  ...
  if (mask & SpvImageOperandsOffsetMask) {
    if (info.dim == SpvDimLwbe) {
      return _.diag(SPV_ERROR_ILWALID_DATA, inst)
             << "Image Operand Offset cannot be used with Lwbe Image 'Dim'";
    }
  ...
  if (mask & SpvImageOperandsConstOffsetsMask) {
    ...
    if (info.dim == SpvDimLwbe) {
      return _.diag(SPV_ERROR_ILWALID_DATA, inst)
             << "Image Operand ConstOffsets cannot be used with Lwbe Image "
                "'Dim'";
    }
  ...
  if (mask & SpvImageOperandsSignExtendMask) {
    ...
    // "The texel value is colwerted to the target value via sign extension.
  ...
  if (mask & SpvImageOperandsZeroExtendMask) {
    ...
    // "The texel value is colwerted to the target value via zero extension.
    // Only valid when the texel type is a scalar or vector of integer type."
    //
    // We don't have enough information to know what the texel type is.
    // In OpenCL, knowledge is deferred until runtime: the image SampledType is
...
spv_result_t ValidateImageCommon(ValidationState_t& _, const Instruction* inst,
  ...
  if (opcode == SpvOpImageRead || opcode == SpvOpImageSparseRead ||
    ...
    if (info.sampled == 0) {
      ...
      } else if (info.dim == SpvDimLwbe && info.arrayed == 1 &&
                 !_.HasCapability(SpvCapabilityImageLwbeArray)) {
        return _.diag(SPV_ERROR_ILWALID_DATA, inst)
               << "Capability ImageLwbeArray is required to access "
               << "storage image";
      }
...
spv_result_t ValidateTypeImage(ValidationState_t& _, const Instruction* inst) {
  ...
  } else if (spvIsOpenCLElw(_.context()->target_elw)) {
  ...
  if (spvIsOpenCLElw(_.context()->target_elw)) {
    if (info.multisampled != 0) {
      return _.diag(SPV_ERROR_ILWALID_DATA, inst)
             << "MS must be 0 in the OpenCL elwironement.";
    }
  }
...
spv_result_t ValidateImageTexelPointer(ValidationState_t& _,
  ...
  if (info.arrayed == 0) {
    ...
    switch (info.dim) {
      ...
      default:
        return _.diag(SPV_ERROR_ILWALID_DATA, inst)
               << "Expected Image 'Dim' must be one of 1D, 2D, or Lwbe when "
                  "Arrayed is 1";
        break;
    }
  }
...
spv_result_t ValidateImageFetch(ValidationState_t& _, const Instruction* inst) {
  ...
  if (info.dim == SpvDimLwbe) {
    return _.diag(SPV_ERROR_ILWALID_DATA, inst) << "Image 'Dim' cannot be Lwbe";
  }
...
spv_result_t ValidateImageGather(ValidationState_t& _,
  ...
  if (info.dim != SpvDim2D && info.dim != SpvDimLwbe &&
      info.dim != SpvDimRect) {
    return _.diag(SPV_ERROR_ILWALID_DATA, inst)
           << "Expected Image 'Dim' cannot be Lwbe";
  }
  ...
  if (info.dim == SpvDimSubpassData) {
    ...
    _.function(inst->function()->id())
        ->RegisterExelwtionModelLimitation(
            ...
            std::string("Dim SubpassData requires Fragment exelwtion model: ") +
                spvOpcodeString(opcode));
  }
...
spv_result_t ValidateImageQuerySizeLod(ValidationState_t& _,
  ...
  switch (info.dim) {
    ...
    default:
      return _.diag(SPV_ERROR_ILWALID_DATA, inst)
             << "Image 'Dim' must be 1D, 2D, 3D or Lwbe";
  }

  if (info.multisampled != 0) {
    return _.diag(SPV_ERROR_ILWALID_DATA, inst) << "Image 'MS' must be 0";
  }
...
spv_result_t ValidateImageQuerySize(ValidationState_t& _,
  ...
  switch (info.dim) {
    ...
    default:
      return _.diag(SPV_ERROR_ILWALID_DATA, inst)
             << "Image 'Dim' must be 1D, Buffer, 2D, Lwbe, 3D or Rect";
  }
...
spv_result_t ValidateImageQueryLod(ValidationState_t& _,
      ...
      ->RegisterExelwtionModelLimitation(
          [&](SpvExelwtionModel model, std::string* message) {
            if (model != SpvExelwtionModelFragment &&
              ...
              if (message) {
                *message = std::string(
                    "OpImageQueryLod requires Fragment or GLCompute exelwtion "
                    "model");
              }
              return false;
            }
  ...
  _.function(inst->function()->id())
      ->RegisterLimitation([](const ValidationState_t& state,
                              ...
                              std::string* message) {
        const auto* models = state.GetExelwtionModels(entry_point->id());
        const auto* modes = state.GetExelwtionModes(entry_point->id());
        if (models->find(SpvExelwtionModelGLCompute) != models->end() &&
            modes->find(SpvExelwtionModeDerivativeGroupLinearLW) ==
                modes->end() &&
            modes->find(SpvExelwtionModeDerivativeGroupQuadsLW) ==
                modes->end()) {
          if (message) {
            *message = std::string(
                "OpImageQueryLod requires DerivativeGroupQuadsLW "
                "or DerivativeGroupLinearLW exelwtion mode for GLCompute "
                "exelwtion model");
          }
          return false;
        }
...
spv_result_t ValidateImageQueryLevelsOrSamples(ValidationState_t& _,
  ...
  if (opcode == SpvOpImageQueryLevels) {
    ...
    if (info.dim != SpvDim2D) {
      return _.diag(SPV_ERROR_ILWALID_DATA, inst) << "Image 'Dim' must be 2D";
    }

    if (info.multisampled != 1) {
      return _.diag(SPV_ERROR_ILWALID_DATA, inst) << "Image 'MS' must be 1";
    }
  }
  return SPV_SUCCESS;
}
...
spv_result_t ImagePass(ValidationState_t& _, const Instruction* inst) {
  ...
  if (IsImplicitLod(opcode)) {
    _.function(inst->function()->id())
        ->RegisterExelwtionModelLimitation([opcode](SpvExelwtionModel model,
                                                    std::string* message) {
          if (model != SpvExelwtionModelFragment &&
              model != SpvExelwtionModelGLCompute) {
            if (message) {
        ...
        ->RegisterLimitation([opcode](const ValidationState_t& state,
          ...
          if (models->find(SpvExelwtionModelGLCompute) != models->end() &&
            ...
            if (message) {
              *message =
                  std::string(
                      "ImplicitLod instructions require DerivativeGroupQuadsLW "
                      "or DerivativeGroupLinearLW exelwtion mode for GLCompute "
                      "exelwtion model: ") +
                  spvOpcodeString(opcode);
            }
