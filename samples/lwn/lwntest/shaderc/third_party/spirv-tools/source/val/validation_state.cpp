...
bool IsInstructionInLayoutSection(ModuleLayoutSection layout, SpvOp op) {
  ...
  switch (layout) {
    ...
    case kLayoutExelwtionMode:
      out = op == SpvOpExelwtionMode || op == SpvOpExelwtionModeId;
      break;
    case kLayoutDebug1:
      switch (op) {
...
void UpdateFeaturesBasedOnSpirvVersion(ValidationState_t::Feature* features,
  ...
  if (version >= SPV_SPIRV_VERSION_WORD(1, 4)) {
    ...
    features->ucolwert_spec_constant_op = true;
...
ValidationState_t::ValidationState_t(const spv_const_context ctx,
    ...
    : context_(ctx),
      ...
      operand_names_{},
      lwrrent_layout_section_(kLayoutCapabilities),
      module_functions_(),
  ...
  const auto elw = context_->target_elw;

  if (spvIsVulkanElw(elw)) {
    // Vulkan 1.1 includes VK_KHR_relaxed_block_layout in core.
    if (elw != SPV_ELW_VULKAN_1_0) {
      features_.elw_relaxed_block_layout = true;
    }
  }
...
ModuleLayoutSection ValidationState_t::lwrrent_layout_section() const {
  return lwrrent_layout_section_;
}
...
void ValidationState_t::ProgressToNextLayoutSectionOrder() {
  ...
  if (lwrrent_layout_section_ <= kLayoutFunctionDefinitions) {
    lwrrent_layout_section_ =
        static_cast<ModuleLayoutSection>(lwrrent_layout_section_ + 1);
  }
}

bool ValidationState_t::IsOpcodeInLwrrentLayoutSection(SpvOp op) {
  return IsInstructionInLayoutSection(lwrrent_layout_section_, op);
}
...
Function& ValidationState_t::lwrrent_function() {
  assert(in_function_body());
  return module_functions_.back();
}

const Function& ValidationState_t::lwrrent_function() const {
  assert(in_function_body());
  return module_functions_.back();
}
...
bool ValidationState_t::in_block() const {
  return module_functions_.empty() == false &&
         module_functions_.back().lwrrent_block() != nullptr;
}

void ValidationState_t::RegisterCapability(SpvCapability cap) {
  // Avoid redundant work.  Otherwise the relwrsion could induce work
...
void ValidationState_t::RegisterExtension(Extension ext) {
  ...
  switch (ext) {
    ...
    case kSPV_AMD_gpu_shader_int16:
      ...
      features_.ucolwert_spec_constant_op = true;
      break;
...
spv_result_t ValidationState_t::RegisterFunction(
  ...
  module_functions_.emplace_back(id, ret_type_id, function_control,
                                 function_type_id);
  id_to_function_.emplace(id, &lwrrent_function());
...
spv_result_t ValidationState_t::RegisterFunctionEnd() {
  ...
  assert(in_block() == false &&
         ...
         "ouside of a block");
  lwrrent_function().RegisterFunctionEnd();
  in_function_ = false;
  return SPV_SUCCESS;
}
...
bool ValidationState_t::IsCooperativeMatrixType(uint32_t id) const {
  const Instruction* inst = FindDef(id);
  assert(inst);
  return inst->opcode() == SpvOpTypeCooperativeMatrixLW;
}
...
spv_result_t ValidationState_t::CooperativeMatrixShapesMatch(
  ...
  if (m1_type->opcode() != SpvOpTypeCooperativeMatrixLW ||
      m2_type->opcode() != SpvOpTypeCooperativeMatrixLW) {
    return diag(SPV_ERROR_ILWALID_DATA, inst)
...
void ValidationState_t::ComputeRelwrsiveEntryPoints() {
    ...
    while (!call_stack.empty()) {
      ...
      if (called_func_id == func.id()) {
        for (const uint32_t entry_point :
          ...
          relwrsive_entry_points_.insert(entry_point);
        break;
      }
...
std::set<uint32_t> ValidationState_t::EntryPointReferences(uint32_t id) const {
  ...
  while (!stack.empty()) {
    const auto lwrrent_inst = stack.back();
    stack.pop_back();

    if (const auto func = lwrrent_inst->function()) {
      ...
      for (auto pair : lwrrent_inst->uses()) {
...
std::string ValidationState_t::Disassemble(const uint32_t* words,
  ...
  return spvInstructionBinaryToText(context()->target_elw, words, num_words,
...
bool ValidationState_t::IsValidStorageClass(
  ...
  if (spvIsWebGPUElw(context()->target_elw)) {
  ...
  if (spvIsVulkanElw(context()->target_elw)) {
    switch (storage_class) {
      case SpvStorageClassUniformConstant:
      case SpvStorageClassUniform:
      case SpvStorageClassStorageBuffer:
      ...
      case SpvStorageClassRayPayloadLW:
      case SpvStorageClassIncomingRayPayloadLW:
      case SpvStorageClassHitAttributeLW:
      case SpvStorageClassCallableDataLW:
      case SpvStorageClassIncomingCallableDataLW:
      case SpvStorageClassShaderRecordBufferLW:
        return true;
      default:
        return false;
    }
