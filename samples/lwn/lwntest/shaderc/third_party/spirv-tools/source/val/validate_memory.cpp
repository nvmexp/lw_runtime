...
// it.
bool ContainsIlwalidBool(ValidationState_t& _, const Instruction* storage,
                         bool skip_builtin) {
  if (skip_builtin) {
  ...
  switch (storage->opcode()) {
    ...
    case SpvOpTypeRuntimeArray:
      ...
      return ContainsIlwalidBool(_, elem_type, skip_builtin);
    case SpvOpTypeStruct:
      for (size_t member_type_index = 1;
        ...
        if (ContainsIlwalidBool(_, member_type, skip_builtin)) return true;
      }
    default:
      break;
  }
...
std::pair<SpvStorageClass, SpvStorageClass> GetStorageClass(
  ...
  switch (inst->opcode()) {
    case SpvOpCooperativeMatrixLoadLW:
    case SpvOpLoad: {
    ...
    case SpvOpCooperativeMatrixStoreLW:
    case SpvOpStore: {
...
// OpCopyMemorySized, OpCooperativeMatrixLoadLW, and
// OpCooperativeMatrixStoreLW.
...
spv_result_t CheckMemoryAccess(ValidationState_t& _, const Instruction* inst,
  ...
  if (mask & SpvMemoryAccessMakePointerAvailableKHRMask) {
    if (inst->opcode() == SpvOpLoad ||
        inst->opcode() == SpvOpCooperativeMatrixLoadLW) {
      return _.diag(SPV_ERROR_ILWALID_ID, inst)
  ...
  if (mask & SpvMemoryAccessMakePointerVisibleKHRMask) {
    if (inst->opcode() == SpvOpStore ||
        inst->opcode() == SpvOpCooperativeMatrixStoreLW) {
      return _.diag(SPV_ERROR_ILWALID_ID, inst)
...
spv_result_t ValidateVariable(ValidationState_t& _, const Instruction* inst) {
  ...
  if (storage_class != SpvStorageClassWorkgroup &&
      ...
      storage_class != SpvStorageClassRayPayloadLW &&
      storage_class != SpvStorageClassIncomingRayPayloadLW &&
      storage_class != SpvStorageClassHitAttributeLW &&
      storage_class != SpvStorageClassCallableDataLW &&
      storage_class != SpvStorageClassIncomingCallableDataLW) {
    ...
    if (!(storage_input_or_output && builtin) &&
        ContainsIlwalidBool(_, storage, storage_input_or_output)) {
      return _.diag(SPV_ERROR_ILWALID_ID, inst)
    ...
    if (storage_class == SpvStorageClassUniformConstant) {
      if (!IsAllowedTypeOrArrayOfSame(
              ...
              {SpvOpTypeImage, SpvOpTypeSampler, SpvOpTypeSampledImage,
               SpvOpTypeAccelerationStructureLW,
        ...
        return _.diag(SPV_ERROR_ILWALID_ID, inst)
               ...
               << "OpTypeSampledImage, OpTypeAccelerationStructureLW, "
  ...
  if (inst->operands().size() > 3 && storage_class != SpvStorageClassOutput &&
    ...
    if (spvIsVulkanOrWebGPUElw(_.context()->target_elw)) {
      return _.diag(SPV_ERROR_ILWALID_ID, inst)
             ...
             << "combination.\n"
             << "From " << spvLogStringForElw(_.context()->target_elw)
             << " spec:\n"
  ...
  // have an initializer.
  if (spvIsWebGPUElw(_.context()->target_elw) && inst->operands().size() <= 3 &&
      (storage_class == SpvStorageClassOutput ||
       storage_class == SpvStorageClassPrivate ||
       storage_class == SpvStorageClassFunction)) {
    return _.diag(SPV_ERROR_ILWALID_ID, inst)
           ...
           << "From WebGPU exelwtion environment spec:\n"
...
spv_result_t ValidateAccessChain(ValidationState_t& _,
  ...
  for (size_t i = starting_index; i < inst->words().size(); ++i) {
    const uint32_t lwr_word = inst->words()[i];
    // Earlier ID checks ensure that lwr_word definition exists.
    auto lwr_word_instr = _.FindDef(lwr_word);
    // The index must be a scalar integer type (See OpAccessChain in the Spec.)
    auto index_type = _.FindDef(lwr_word_instr->type_id());
    ...
    switch (type_pointee->opcode()) {
      ...
      case SpvOpTypeArray:
      case SpvOpTypeRuntimeArray: {
        // In OpTypeMatrix, OpTypeVector, SpvOpTypeCooperativeMatrixLW,
      ...
      case SpvOpTypeStruct: {
        ...
        if (SpvOpConstant != lwr_word_instr->opcode()) {
          return _.diag(SPV_ERROR_ILWALID_ID, lwr_word_instr)
        ...
        // billion members.
        const uint32_t lwr_index = lwr_word_instr->word(3);
        ...
        if (lwr_index >= num_struct_members) {
          return _.diag(SPV_ERROR_ILWALID_ID, lwr_word_instr)
                 << "Index is out of bounds: " << instr_name
                 << " can not find index " << lwr_index
        ...
        auto structMemberId = type_pointee->word(lwr_index + 2);
...
spv_result_t ValidateArrayLength(ValidationState_t& state,
  ...
  if (result_type->opcode() != SpvOpTypeInt ||
    ...
    return state.diag(SPV_ERROR_ILWALID_ID, inst)
           << "The Result Type of " << instr_name << " <id> '"
...
spv_result_t ValidateCooperativeMatrixLengthLW(ValidationState_t& state,
                                               const Instruction* inst) {
  std::string instr_name =
      "Op" + std::string(spvOpcodeString(static_cast<SpvOp>(inst->opcode())));
  ...
  if (type->opcode() != SpvOpTypeCooperativeMatrixLW) {
    return state.diag(SPV_ERROR_ILWALID_ID, inst)
           << "The type in " << instr_name << " <id> '"
           << state.getIdName(type_id)
           << "' must be OpTypeCooperativeMatrixLW.";
  }
  return SPV_SUCCESS;
}

spv_result_t ValidateCooperativeMatrixLoadStoreLW(ValidationState_t& _,
                                                  const Instruction* inst) {
  uint32_t type_id;
  const char* opname;
  if (inst->opcode() == SpvOpCooperativeMatrixLoadLW) {
    type_id = inst->type_id();
    opname = "SpvOpCooperativeMatrixLoadLW";
  } else {
    // get Object operand's type
    type_id = _.FindDef(inst->GetOperandAs<uint32_t>(1))->type_id();
    opname = "SpvOpCooperativeMatrixStoreLW";
  }

  auto matrix_type = _.FindDef(type_id);

  if (matrix_type->opcode() != SpvOpTypeCooperativeMatrixLW) {
    if (inst->opcode() == SpvOpCooperativeMatrixLoadLW) {
      return _.diag(SPV_ERROR_ILWALID_ID, inst)
             << "SpvOpCooperativeMatrixLoadLW Result Type <id> '"
             << _.getIdName(type_id) << "' is not a cooperative matrix type.";
    } else {
      return _.diag(SPV_ERROR_ILWALID_ID, inst)
             << "SpvOpCooperativeMatrixStoreLW Object type <id> '"
             << _.getIdName(type_id) << "' is not a cooperative matrix type.";
    }
  }
  ...
  const auto pointer_index =
      (inst->opcode() == SpvOpCooperativeMatrixLoadLW) ? 2u : 0u;
  const auto pointer_id = inst->GetOperandAs<uint32_t>(pointer_index);
  const auto pointer = _.FindDef(pointer_id);
  if (!pointer ||
      ((_.addressing_model() == SpvAddressingModelLogical) &&
  ...
  const auto stride_index =
      (inst->opcode() == SpvOpCooperativeMatrixLoadLW) ? 3u : 2u;
  ...
  const auto colmajor_index =
      (inst->opcode() == SpvOpCooperativeMatrixLoadLW) ? 4u : 3u;
  ...
  const auto memory_access_index =
      (inst->opcode() == SpvOpCooperativeMatrixLoadLW) ? 5u : 4u;
...
spv_result_t MemoryPass(ValidationState_t& _, const Instruction* inst) {
  switch (inst->opcode()) {
    ...
    case SpvOpCooperativeMatrixLoadLW:
    case SpvOpCooperativeMatrixStoreLW:
      if (auto error = ValidateCooperativeMatrixLoadStoreLW(_, inst))
        return error;
      break;
    case SpvOpCooperativeMatrixLengthLW:
      if (auto error = ValidateCooperativeMatrixLengthLW(_, inst)) return error;
      break;
    case SpvOpPtrEqual:
