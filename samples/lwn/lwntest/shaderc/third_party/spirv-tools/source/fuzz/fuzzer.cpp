...
// A colwenience method to add a fuzzer pass to |passes| with probability 0.5.
...
struct Fuzzer::Impl {
  Impl(spv_target_elw elw, uint32_t random_seed, bool validate_after_each_pass,
  ...
  bool validate_after_each_fuzzer_pass;  // Determines whether the validator
                                         // should be ilwoked after every fuzzer
                                         // pass.
  spv_validator_options validator_options;  // Options to control validation.
};

Fuzzer::Fuzzer(spv_target_elw elw, uint32_t seed,
               bool validate_after_each_fuzzer_pass,
               spv_validator_options validator_options)
    : impl_(MakeUnique<Impl>(elw, seed, validate_after_each_fuzzer_pass,
                             validator_options)) {}
...
Fuzzer::FuzzerResultStatus Fuzzer::Run(
  ...
  if (!tools.Validate(&binary_in[0], binary_in.size(),
    ...
    return Fuzzer::FuzzerResultStatus::kInitialBinaryIlwalid;
  }

  // Build the module from the input binary.
  std::unique_ptr<opt::IRContext> ir_context = BuildModule(
      impl_->target_elw, impl_->consumer, binary_in.data(), binary_in.size());
  assert(ir_context);
  ...
  if (!impl_->ApplyPassAndCheckValidity(&add_useful_constructs, *ir_context,
                                        tools)) {
    return Fuzzer::FuzzerResultStatus::kFuzzerPassLedToIlwalidModule;
  }
