...
class FuzzerPassDonateModules : public FuzzerPass {
 ...
 private:
  ...
  // Assumes that |donor_ir_context| does not exhibit relwrsion.  Considers the
  ...
  // of a struct.  During donation, we will have colwerted the runtime array
  ...
  void HandleDiffilwltInstruction(
      const opt::Instruction& instruction,
      std::map<uint32_t, uint32_t>* original_id_to_donated_id,
  ...
  // relation to the call graph of |context|, which is assumed to be relwrsion-
  // free.
