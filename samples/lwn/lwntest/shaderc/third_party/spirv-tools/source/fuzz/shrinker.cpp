...
struct Shrinker::Impl {
  Impl(spv_target_elw elw, uint32_t limit, bool validate,
  ...
  const spv_target_elw target_elw;          // Target environment.
...
Shrinker::Shrinker(spv_target_elw elw, uint32_t step_limit,
                   bool validate_during_replay,
                   spv_validator_options validator_options)
    : impl_(MakeUnique<Impl>(elw, step_limit, validate_during_replay,
                             validator_options)) {}
...
Shrinker::ShrinkerResultStatus Shrinker::Run(
  ...
  spvtools::SpirvTools tools(impl_->target_elw);
  if (!tools.IsValid()) {
  ...
  if (!tools.Validate(&binary_in[0], binary_in.size(),
    ...
    return Shrinker::ShrinkerResultStatus::kInitialBinaryIlwalid;
  }

  std::vector<uint32_t> lwrrent_best_binary;
  protobufs::TransformationSequence lwrrent_best_transformations;
  ...
  if (Replayer(impl_->target_elw, impl_->validate_during_replay,
               impl_->validator_options)
          .Run(binary_in, initial_facts, transformation_sequence_in,
               &lwrrent_best_binary, &lwrrent_best_transformations) !=
  ...
  // indeed interesting.
  if (!interestingness_function(lwrrent_best_binary, 0)) {
    impl_->consumer(SPV_MSG_INFO, nullptr, {},
  ...
  uint32_t chunk_size =
      std::max(1u, NumRemainingTransformations(lwrrent_best_transformations) /
  ...
  while (attempt < impl_->step_limit &&
         !lwrrent_best_transformations.transformation().empty() &&
         chunk_size > 0) {
    ...
    assert(chunk_size <=
               NumRemainingTransformations(lwrrent_best_transformations) &&
    ...
    for (int chunk_index = num_chunks - 1;
      ...
      auto transformations_with_chunk_removed =
          RemoveChunk(lwrrent_best_transformations, chunk_index, chunk_size);
      ...
      if (interestingness_function(next_binary, attempt)) {
        ...
        // sequence.
        lwrrent_best_binary = next_binary;
        lwrrent_best_transformations = next_transformation_sequence;
  ...
  *binary_out = lwrrent_best_binary;
  *transformation_sequence_out = lwrrent_best_transformations;
