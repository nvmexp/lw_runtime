...
opt::Function* FindFragmentShaderEntryPoint(opt::IRContext* ir_context,
  ...
  for (auto& entry_point : ir_context->module()->entry_points()) {
    if (entry_point.GetSingleWordInOperand(0) == SpvExelwtionModelFragment) {
...
bool ForceRenderRed(
    const spv_target_elw& target_elw, spv_validator_options validator_options,
    const std::vector<uint32_t>& binary_in,
    const spvtools::fuzz::protobufs::FactSequence& initial_facts,
    std::vector<uint32_t>* binary_out) {
  auto message_consumer = spvtools::utils::CLIMessageConsumer;
  spvtools::SpirvTools tools(target_elw);
  if (!tools.IsValid()) {
    message_consumer(SPV_MSG_ERROR, nullptr, {},
  ...
  std::unique_ptr<opt::IRContext> ir_context = BuildModule(
      target_elw, message_consumer, binary_in.data(), binary_in.size());
  assert(ir_context);
