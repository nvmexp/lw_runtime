...
bool TransformationFunctionCall::IsApplicable(
  ...
  // Introducing the call must not lead to relwrsion.
  if (message_.callee_id() == enclosing_function->result_id()) {
    // This would be direct relwrsion.
    return false;
  }
  // Ensure the call would not lead to indirect relwrsion.
...
void TransformationFunctionCall::Apply(
  ...
  FindInstruction(message_.instruction_to_insert_before(), ir_context)
      ->InsertBefore(MakeUnique<opt::Instruction>(
          ...
          operands));
  // Ilwalidate all analyses since we have changed the module.
