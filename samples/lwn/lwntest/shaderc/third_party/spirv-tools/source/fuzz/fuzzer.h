...
class Fuzzer {
 public:
  ...
  enum class FuzzerResultStatus {
    ...
    kFuzzerPassLedToIlwalidModule,
    kInitialBinaryIlwalid,
  };

  // Constructs a fuzzer from the given target environment |elw|.  |seed| is a
  // seed for pseudo-random number generation.
  // |validate_after_each_fuzzer_pass| controls whether the validator will be
  // ilwoked after every fuzzer pass is applied.
  Fuzzer(spv_target_elw elw, uint32_t seed,
