...
class FactManager::DataSynonymAndIdEquationFacts {
 ...
 private:
  ...
  // Adds the synonym |dd1| = |dd2| to the set of managed facts, and relwrses
  ...
  void AddDataSynonymFactRelwrsive(const protobufs::DataDescriptor& dd1,
  ...
  //
  // All data descriptors oclwrring in equations are required to be present in
...
void FactManager::DataSynonymAndIdEquationFacts::AddFact(
  ...
  // descriptors that are ilwolved.
  AddDataSynonymFactRelwrsive(fact.data1(), fact.data2(), context);
}
...
void FactManager::DataSynonymAndIdEquationFacts::AddFact(
  ...
  // Now add the fact.
  AddEquationFactRelwrsive(lhs_dd, static_cast<SpvOp>(fact.opcode()),
                           rhs_dd_ptrs, context);
}
...
void FactManager::DataSynonymAndIdEquationFacts::AddEquationFactRelwrsive(
    const protobufs::DataDescriptor& lhs_dd, SpvOp opcode,
    const std::vector<const protobufs::DataDescriptor*>& rhs_dds,
    opt::IRContext* context) {
      ...
      for (auto equation : GetEquations(rhs_dds[0])) {
        if (equation.opcode == SpvOpISub) {
          ...
          if (synonymous_.IsEquivalent(*equation.operands[1], *rhs_dds[1])) {
            ...
            AddDataSynonymFactRelwrsive(lhs_dd, *equation.operands[0], context);
          }
          if (synonymous_.IsEquivalent(*equation.operands[0], *rhs_dds[1])) {
            // Equation form: "a = (c - e) + c"
            // We can thus infer "a = -e"
            AddEquationFactRelwrsive(lhs_dd, SpvOpSNegate,
      ...
      for (auto equation : GetEquations(rhs_dds[0])) {
        if (equation.opcode == SpvOpIAdd) {
          ...
          if (synonymous_.IsEquivalent(*equation.operands[0], *rhs_dds[1])) {
            // Equation form: "a = (c + e) - c"
            // We can thus infer "a = e"
            AddDataSynonymFactRelwrsive(lhs_dd, *equation.operands[1], context);
          }
          if (synonymous_.IsEquivalent(*equation.operands[1], *rhs_dds[1])) {
            // Equation form: "a = (d + c) - c"
            // We can thus infer "a = d"
...
void FactManager::DataSynonymAndIdEquationFacts::AddDataSynonymFactRelwrsive(
  ...
  // we can relwrsively add synonym facts about their associated sub-components.
      ...
      for (auto dd1_it = equivalence_class.begin();
        ...
        for (++dd2_it; dd2_it != equivalence_class.end(); ++dd2_it) {
          ...
          if (all_components_match) {
            ...
            // they are synonymous.  We add this fact *non-relwrsively*, as we
...
FactManager::DataSynonymAndIdEquationFacts::GetIdsForWhichSynonymsAreKnown()
    const {
  std::vector<uint32_t> result;
  for (auto& data_descriptor : synonymous_.GetAllKnowlwalues()) {
