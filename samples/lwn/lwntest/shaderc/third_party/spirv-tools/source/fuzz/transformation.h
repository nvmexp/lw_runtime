...
//     transformation ilwolves a block id, 'IsApplicable' should check whether
//     the module indeed has a block with that id, and return false if not.  It
//     must not assume that there is such a block.
// - Dolwmented precondition: while the implementation of 'IsApplicable' should
...
//     the precondition.
// - Dolwmented effect: while the implementation of 'Apply' should codify the
...
class Transformation {
 public:
  ...
  //
  // Preconditions for individual transformations must be dolwmented in the
