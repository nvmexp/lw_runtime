...
bool TransformationAddFunction::TryToAddFunction(
  ...
  // Keeps track of which instruction protobuf message we are lwrrently
  // considering.
  ...
  while (instruction_index < num_instructions &&
    ...
    // Ilwariant: we should always be at a label instruction at this point.
...
bool TransformationAddFunction::TryToTurnKillOrUnreachableIntoReturn(
  ...
  if (function_return_type_inst->opcode() == SpvOpTypeVoid) {
    ...
    // to OpReturlwalue, using the value id provided with the
    // transformation.
    ...
    // OpReturlwalue %id has the same type as the function's return type.
    if (ir_context->get_def_use_mgr()
            ->GetDef(message_.kill_unreachable_return_value_id())
            ->type_id() != function_return_type_inst->result_id()) {
      return false;
    }
    kill_or_unreachable_inst->SetOpcode(SpvOpReturlwalue);
...
uint32_t TransformationAddFunction::GetBoundForCompositeIndex(
  ...
  switch (composite_type_inst.opcode()) {
    ...
    case SpvOpTypeRuntimeArray:
      assert(false &&
             "GetBoundForCompositeIndex should not be ilwoked with an "
