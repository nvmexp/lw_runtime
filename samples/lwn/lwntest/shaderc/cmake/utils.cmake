...
function(shaderc_add_asciidoc TARGET FILE)
  if (ASCIIDOCTOR_EXE)
    set(DEST ${CMAKE_LWRRENT_BINARY_DIR}/${FILE}.html)
    add_lwstom_command(
      COMMAND ${ASCIIDOCTOR_EXE} -a toc -o ${DEST}
        ${CMAKE_LWRRENT_SOURCE_DIR}/${FILE}.asciidoc
    ...
    add_lwstom_target(${TARGET} DEPENDS ${DEST})
...
function(shaderc_add_tests)
  if(${SHADERC_ENABLE_TESTS})
    ...
    foreach(TARGET ${PARSED_ARGS_TEST_NAMES})
      ...
      add_exelwtable(${TEST_NAME} src/${TARGET}_test.cc)
...
function(shaderc_combine_static_lib new_target target)
  ...
  set(libname
      ${CMAKE_LWRRENT_BINARY_DIR}/${CMAKE_CFG_INTDIR}/${CMAKE_STATIC_LIBRARY_PREFIX}${new_target}${CMAKE_STATIC_LIBRARY_SUFFIX})

  if (MSVC)
    string(REPLACE ";" ">;$<TARGET_FILE:" temp_string "${all_libs}")
    set(lib_target_list "$<TARGET_FILE:${temp_string}>")

    add_lwstom_command(OUTPUT ${libname}
      DEPENDS ${all_libs}
  ...
  elseif(APPLE)
    ...
    file(GENERATE OUTPUT "${CMAKE_LWRRENT_BINARY_DIR}/${new_target}.ar"
  ...
  add_lwstom_target(${new_target}_genfile ALL
    DEPENDS ${libname})
