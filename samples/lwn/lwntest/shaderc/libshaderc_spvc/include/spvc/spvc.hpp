...
class CompilationResult {
 public:
  ...
  shaderc_spvc_status GetStringOutput(std::string* str) const {
    if (!str) return shaderc_spvc_status_ilwalid_out_param;
  ...
  shaderc_spvc_status GetBinaryOutput(std::vector<uint32_t>* data) const {
    if (!data) return shaderc_spvc_status_ilwalid_out_param;
...
class CompileOptions {
 public:
  CompileOptions(shaderc_spvc_spv_elw source_elw,
                 shaderc_spvc_spv_elw target_elw)
      : options_(shaderc_spvc_compile_options_create(source_elw, target_elw),
                 shaderc_spvc_compile_options_destroy) {}
  // DEPRECATED
  CompileOptions()
      : CompileOptions(shaderc_spvc_spv_elw_universal_1_0,
  ...
  shaderc_spvc_status SetSourceElwironment(shaderc_target_elw elw,
                                           shaderc_elw_version version) {
    return shaderc_spvc_compile_options_set_source_elw(options_.get(), elw,
                                                       version);
  }
  ...
  // Lwrrently only Vulkan 1.1 <-> WebGPU transforms are defined. Default is
  // Vulkan 1.0.
  shaderc_spvc_status SetTargetElwironment(shaderc_target_elw elw,
                                           shaderc_elw_version version) {
    return shaderc_spvc_compile_options_set_target_elw(options_.get(), elw,
                                                       version);
  }
...
class Context {
 public:
  ...
  // Gets workgroup size for an entry point defined by a given exelwtion model
  // and function name.
  shaderc_spvc_status GetWorkgroupSize(
    ...
    return shaderc_spvc_get_workgroup_size(
        context_.get(), function_name.c_str(), exelwtion_model, workgroup_size);
  }
  ...
  // Gets the exelwtion model for the shader.
  shaderc_spvc_status GetExelwtionModel(shaderc_spvc_exelwtion_model* model) {
    return shaderc_spvc_get_exelwtion_model(context_.get(), model);
  }
