...
struct shaderc_spvc_compile_options {
  ...
  spv_target_elw source_elw = SPV_ELW_VULKAN_1_0;
  spv_target_elw target_elw = SPV_ELW_VULKAN_1_0;
...
// Colwert from shaderc values to spirv-tools values.
...
// environment. Ilwoke spirv-val to perform this operation.
shaderc_spvc_status validate_spirv(shaderc_spvc_context* context,
                                   spv_target_elw elw, const uint32_t* source,
                                   size_t source_len);

// Colwert SPIR-V from one environment to another, if there is a known
// colwersion. If the origin and destination elwironments are the same, then
// the binary is just copied to the output buffer. Ilwokes spirv-opt to perform
...
// Given a pointer to an SPIRV-Cross IR (with initialized spirv field), Ilwokes
...
// Colwerts a shaderc spvc decoration value to the equivalent spirv-cross
// decoration value and sends the result out through |spirv_cross_decoration|.
// If the colwersion fails, returns an error result.
