...
spv_target_elw get_spv_target_elw(shaderc_target_elw elw,
  ...
  switch (elw) {
    ...
    case shaderc_target_elw_vulkan:
      switch (version) {
        case shaderc_elw_version_vulkan_1_0:
          return SPV_ELW_VULKAN_1_0;
        case shaderc_elw_version_vulkan_1_1:
...
shaderc_spvc_status validate_spirv(shaderc_spvc_context* context,
                                   spv_target_elw elw, const uint32_t* source,
                                   size_t source_len) {
  spvtools::SpirvTools tools(elw);
  if (!tools.IsValid()) {
...
shaderc_spvc_status translate_spirv(shaderc_spvc_context* context,
                                    spv_target_elw source_elw,
                                    spv_target_elw target_elw,
  ...
  if (source_elw == target_elw) {
    target->resize(source_len);
    memcpy(target->data(), source, source_len * sizeof(uint32_t));
    return shaderc_spvc_status_success;
  }

  spvtools::Optimizer opt(source_elw);
  opt.SetMessageConsumer(std::bind(
      consume_spirv_tools_message, context, std::placeholders::_1,
      std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));

  if (source_elw == SPV_ELW_WEBGPU_0 && target_elw == SPV_ELW_VULKAN_1_1) {
    opt.RegisterWebGPUToVulkanPasses();
  } else if (source_elw == SPV_ELW_VULKAN_1_1 &&
             target_elw == SPV_ELW_WEBGPU_0) {
    opt.RegisterVulkanToWebGPUPasses();
  } else {
    shaderc_spvc::ErrorLog(context)
        << "No defined transformation between source and target exelwtion "
           "elwironments.";
    return shaderc_spvc_status_transformation_error;
  }
  ...
  if (!opt.Run(source, source_len, target)) {
    shaderc_spvc::ErrorLog(context) << "Transformations between source and "
                                       "target exelwtion elwironments failed.";
    return shaderc_spvc_status_transformation_error;
  }
...
shaderc_spvc_status validate_and_translate_spirv(
  ...
  if (options->validate) {
    status = validate_spirv(context, options->source_elw, source, source_len);
  ...
  status = translate_spirv(context, options->source_elw, options->target_elw,
                           source, source_len, options, target);
  if (status != shaderc_spvc_status_success) return status;

  if (options->validate && (options->source_elw != options->target_elw)) {
    // Re-run validation on input if actually transformed.
    status = validate_spirv(context, options->target_elw, target->data(),
                            target->size());
    if (status != shaderc_spvc_status_success) {
      shaderc_spvc::ErrorLog(context)
...
shaderc_spvc_status generate_glsl_compiler(
  ...
  if (context->use_spvc_parser) {
    ...
    if (status != shaderc_spvc_status_success) {
      shaderc_spvc::ErrorLog(context)
          << "Transformations between source and target exelwtion elwironments "
             "failed (spvc-ir-pass).";
      return status;
    } else {
  ...
  spv::ExelwtionModel model = spv::ExelwtionModelMax;
...
shaderc_spvc_status generate_spvcir(const shaderc_spvc_context_t context,
  ...
  if (context->use_spvc_parser) {
    ...
    spvtools::Optimizer opt(options->source_elw);
    opt.SetMessageConsumer(std::bind(
        consume_spirv_tools_message, context, std::placeholders::_1,
        std::placeholders::_2, std::placeholders::_3, std::placeholders::_4));
