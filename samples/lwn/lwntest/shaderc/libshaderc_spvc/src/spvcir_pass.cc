...
SpvcIrPass::SpvcIrPass(spirv_cross::ParsedIR *ir) {
  ir_ = ir;
  lwrrent_function_ = nullptr;
  lwrrent_block_ = nullptr;
  ...
  switch (inst->opcode()) {
    ...
    case SpvOpUndef: {
      ...
      if (lwrrent_block_) {
        ...
        lwrrent_block_->ops.push_back(instr);
      }
      break;
    }
    ...
    case SpvOpExtInstImport: {
      ...
      if (ext == "GLSL.std.450")
        ...
        // spirv-cross comment:
        // Other SPIR-V extensions which have ExtInstrs are lwrrently not
        // supported.
    ...
    case SpvOpExtInst: {
      ...
      if (lwrrent_block_) lwrrent_block_->ops.push_back(instruction);
      break;
    }
    ...
    case SpvOpEntryPoint: {
      ...
      auto exelwtion_mode =
          static_cast<spv::ExelwtionModel>(inst->GetSingleWordInOperand(0u));

      const char *entry_name =
          reinterpret_cast<const char *>(inst->GetInOperand(2u).words.data());

      auto itr = ir_->entry_points.insert(std::make_pair(
          function_id, spirv_cross::SPIREntryPoint(function_id, exelwtion_mode,
                                                   entry_name)));
    ...
    case SpvOpExelwtionMode: {
      auto &exelwtion = ir_->entry_points[inst->GetSingleWordInOperand(0u)];
      auto mode =
          static_cast<SpvExelwtionMode>(inst->GetSingleWordInOperand(1u));
      exelwtion.flags.set(mode);

      switch (mode) {
        case SpvExelwtionModeIlwocations: {
          exelwtion.ilwocations = inst->GetSingleWordInOperand(2u);
          break;
        }

        case SpvExelwtionModeLocalSize: {
          exelwtion.workgroup_size.x = inst->GetSingleWordInOperand(2u);
          exelwtion.workgroup_size.y = inst->GetSingleWordInOperand(3u);
          exelwtion.workgroup_size.z = inst->GetSingleWordInOperand(4u);
          break;
        }

        case SpvExelwtionModeOutputVertices: {
          exelwtion.output_vertices = inst->GetSingleWordInOperand(2u);
          break;
        }

        default: {
          break;
          // Other exelwtions modes are set as part of "Bitset flags" member
    ...
    case SpvOpVariable: {
      ...
      if (storage == spv::StorageClassFunction) {
        if (!CheckConditionAndSetErrorMessage(
                ...
                "no function lwrrently in scope"))
          return;
        lwrrent_function_->add_local_variable(id);
      }
    ...
    case SpvOpPhi: {
      ...
      if (!CheckConditionAndSetErrorMessage(
              lwrrent_block_, "SpvcIrPass: No block lwrrently in scope"))
        return;
      ...
      for (uint32_t i = 0; i + 1 < inst->NumInOperands(); i += 2)
        lwrrent_block_->phi_variables.push_back(
            {inst->GetSingleWordInOperand(i),
    ...
    case SpvOpLoopMerge: {
      ...
      lwrrent_block_->merge_block = inst->GetSingleWordInOperand(0u);
      lwrrent_block_->continue_block = inst->GetSingleWordInOperand(1u);
      lwrrent_block_->merge = spirv_cross::SPIRBlock::MergeLoop;

      ir_->block_meta[lwrrent_block_->self] |=
          spirv_cross::ParsedIR::BLOCK_META_LOOP_HEADER_BIT;
      ir_->block_meta[lwrrent_block_->merge_block] |=
          spirv_cross::ParsedIR::BLOCK_META_LOOP_MERGE_BIT;

      ir_->continue_block_to_loop_header[lwrrent_block_->continue_block] =
          spirv_cross::BlockID(lwrrent_block_->self);
      // spirv-cross comment:
      // Don't add loop headers to continue blocks,
      // which would make it impossible branch into the loop header since
      // they are treated as continues.
      if (lwrrent_block_->continue_block !=
          spirv_cross::BlockID(lwrrent_block_->self))
        ir_->block_meta[lwrrent_block_->continue_block] |=
            spirv_cross::ParsedIR::BLOCK_META_CONTINUE_BIT;

      if (inst->NumInOperands() > 3) {
        if (inst->GetSingleWordInOperand(2u) & spv::LoopControlUnrollMask)
          lwrrent_block_->hint = spirv_cross::SPIRBlock::HintUnroll;
        else if (inst->GetSingleWordInOperand(2u) &
                 spv::LoopControlDontUnrollMask)
          lwrrent_block_->hint = spirv_cross::SPIRBlock::HintDontUnroll;
      }
      break;
    }
    ...
    case SpvOpSelectionMerge: {
      if (!CheckConditionAndSetErrorMessage(
              ...
              "SpvcIrPass: Trying to end a non-existing block."))
        return;
      lwrrent_block_->next_block = inst->GetSingleWordInOperand(0u);
      lwrrent_block_->merge = spirv_cross::SPIRBlock::MergeSelection;
      ir_->block_meta[lwrrent_block_->next_block] |=
      ...
      if (inst->NumInOperands() - 1 >= 2) {
        if (inst->GetSingleWordInOperand(1u) & spv::SelectionControlFlattenMask)
          lwrrent_block_->hint = spirv_cross::SPIRBlock::HintFlatten;
        else if (inst->GetSingleWordInOperand(1u) &
                 spv::SelectionControlDontFlattenMask)
          lwrrent_block_->hint = spirv_cross::SPIRBlock::HintDontFlatten;
      }
      break;
    }
    ...
    case SpvOpLabel: {
      ...
      lwrrent_function_->blocks.push_back(id);

      if (!lwrrent_function_->entry_block) {
        lwrrent_function_->entry_block = id;
      }
      if (!CheckConditionAndSetErrorMessage(
              !lwrrent_block_,
      ...
      lwrrent_block_ = &set<spirv_cross::SPIRBlock>(id);
      break;
    }
    ...
    case SpvOpBranch: {
      ...
      lwrrent_block_->terminator = spirv_cross::SPIRBlock::Direct;
      lwrrent_block_->next_block = target;
      lwrrent_block_ = nullptr;
      break;
    }
    ...
    case SpvOpBranchConditional: {
      ...
      lwrrent_block_->condition = inst->GetSingleWordInOperand(0u);
      lwrrent_block_->true_block = inst->GetSingleWordInOperand(1u);
      lwrrent_block_->false_block = inst->GetSingleWordInOperand(2u);

      lwrrent_block_->terminator = spirv_cross::SPIRBlock::Select;
      lwrrent_block_ = nullptr;
      break;
    }
    ...
    case SpvOpSwitch: {
      if (!CheckConditionAndSetErrorMessage(
              ...
              "SpvcIrPass: Trying to end a non-existing block."))
        return;
      lwrrent_block_->terminator = spirv_cross::SPIRBlock::MultiSelect;

      lwrrent_block_->condition = inst->GetSingleWordInOperand(0u);
      lwrrent_block_->default_block = inst->GetSingleWordInOperand(1u);

      for (uint32_t i = 2; i + 1 < inst->NumInOperands(); i += 2)
        lwrrent_block_->cases.push_back({inst->GetSingleWordInOperand(i),
    ...
    case SpvOpKill: {
      if (!CheckConditionAndSetErrorMessage(
              ...
              "SpvcIrPass: Trying to end a non-existing block."))
        return;
      lwrrent_block_->terminator = spirv_cross::SPIRBlock::Kill;
      lwrrent_block_ = nullptr;
      break;
    }
    ...
    case SpvOpReturn: {
      if (!CheckConditionAndSetErrorMessage(
        ...
        return;
      lwrrent_block_->terminator = spirv_cross::SPIRBlock::Return;
      lwrrent_block_ = nullptr;
      break;
    }

    // opcode: 254
    case SpvOpReturlwalue: {
      if (!CheckConditionAndSetErrorMessage(
              lwrrent_block_,
              "SpvcIrPass: Trying to end a non-existing block."))
        return;
      lwrrent_block_->terminator = spirv_cross::SPIRBlock::Return;
      lwrrent_block_->return_value = inst->GetSingleWordInOperand(0u);
      lwrrent_block_ = nullptr;
      break;
    }
    ...
    case SpvOpUnreachable: {
      if (!CheckConditionAndSetErrorMessage(
        ...
        return;
      lwrrent_block_->terminator = spirv_cross::SPIRBlock::Unreachable;
      lwrrent_block_ = nullptr;
      break;
    }
    ...
    case SpvOpFunction: {
      ...
      lwrrent_function_ = &set<spirv_cross::SPIRFunction>(id, result, type);
      break;
    }
    ...
    case SpvOpFunctionParameter: {
      ...
      if (!CheckConditionAndSetErrorMessage(
        ...
        return;
      lwrrent_function_->add_parameter(type, id);
    ...
    case SpvOpLine: {
      ...
      if (lwrrent_function_) {
        // Store the first one we find and emit it before creating the function
        // prototype.
        if (lwrrent_function_->entry_line.file_id == 0) {
          lwrrent_function_->entry_line.file_id =
              inst->GetSingleWordInOperand(0u);
          lwrrent_function_->entry_line.line_literal =
    ...
    default: {
      if (!CheckConditionAndSetErrorMessage(
              lwrrent_block_,
              "SpvcIrPass: Lwrrently no block to insert opcode."))
        return;
      spirv_cross::Instruction instr = {};
      instr.op = inst->opcode();
      instr.count = inst->NumOperandWords() + 1;
