 ...
 public:
  void SetUp() override {
    ...
    options_ = shaderc_spvc_compile_options_create(
        shaderc_spvc_spv_elw_vulkan_1_0, shaderc_spvc_spv_elw_vulkan_1_0);
...
TEST_F(CompileTest, IlwalidShaderIntoGlslPasses) {
  shaderc_spvc_status status = shaderc_spvc_initialize_for_glsl(
      context_, kIlwalidShaderBinary,
      sizeof(kIlwalidShaderBinary) / sizeof(uint32_t), options_);
...
TEST_F(CompileTest, IlwalidShaderIntoHlslPasses) {
  shaderc_spvc_status status = shaderc_spvc_initialize_for_hlsl(
      context_, kIlwalidShaderBinary,
      sizeof(kIlwalidShaderBinary) / sizeof(uint32_t), options_);
  EXPECT_NE(shaderc_spvc_status_success, status);
...
TEST_F(CompileTest, IlwalidShaderIntoMslPasses) {
  shaderc_spvc_status status = shaderc_spvc_initialize_for_msl(
      context_, kIlwalidShaderBinary,
      sizeof(kIlwalidShaderBinary) / sizeof(uint32_t), options_);
  EXPECT_NE(shaderc_spvc_status_success, status);
...
TEST_F(CompileTest, IlwalidShaderIntoVulkanPasses) {
  shaderc_spvc_status status = shaderc_spvc_initialize_for_vulkan(
      context_, kIlwalidShaderBinary,
      sizeof(kIlwalidShaderBinary) / sizeof(uint32_t), options_);
  EXPECT_NE(shaderc_spvc_status_success, status);
