...
bool createSpvcIr(spirv_cross::ParsedIR* ir, std::string text) {
  ...
  std::unique_ptr<IRContext> context =
      BuildModule(SPV_ELW_UNIVERSAL_1_1, nullptr, text,
...
class SpvcIrParsingTest : public PassTest<::testing::Test> {
 protected:
  void SetUp() override {
    ...
    std::vector<uint32_t> binary;
    std::unique_ptr<IRContext> context =
        BuildModule(SPV_ELW_UNIVERSAL_1_1, nullptr, input_,
                    SPV_TEXT_TO_BINARY_OPTION_PRESERVE_NUMERIC_IDS);
...
TEST_F(SpvcIrParsingTest, OpEntryPointInstruction) {
  ...
  EXPECT_EQ(entry.second.model, spv::ExelwtionModelVertex);
...
TEST_F(SpvcIrParsingTest, SpvOpExelwtionModeInstruction) {
  const std::vector<const char*> middle = {
      "OpExelwtionMode %1 LocalSize 4 3 5",
      "OpExelwtionMode %1 OutputVertices 1",
      "OpExelwtionMode %1 Ilwocations 1"};
  std::string spirv = JoinAllInsts(Concat(Concat(before_, middle), after_));
  spirv_cross::ParsedIR ir;
  createSpvcIr(&ir, spirv);
  ...
  const auto& exelwtion = ir.entry_points[1];
  EXPECT_EQ(exelwtion.workgroup_size.x, 4);
  EXPECT_EQ(exelwtion.workgroup_size.y, 3);
  EXPECT_EQ(exelwtion.workgroup_size.z, 5);
  EXPECT_EQ(exelwtion.output_vertices, 1);
  EXPECT_EQ(exelwtion.ilwocations, 1);
}
...
TEST_F(SpvcIrParsingTest, OpTypeAccelerationStructureLWInstruction) {
  const std::vector<const char*> middle = {
      // clang-format off
