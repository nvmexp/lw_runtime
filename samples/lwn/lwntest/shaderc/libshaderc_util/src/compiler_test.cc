...
const char kVulkalwertexShader[] =
    R"(#version 310 es
...
std::string Disassemble(const std::vector<uint32_t> binary) {
  std::string result;
  shaderc_util::SpirvToolsDisassemble(Compiler::TargetElw::Vulkan,
                                      Compiler::TargetElwVersion::Vulkan_1_2,
                                      binary, &result);
  return result;
}
...
TEST_F(CompilerTest, SimpleVulkanShaderCompilesWithDefaultCompilerSettings) {
  EXPECT_TRUE(SimpleCompilationSucceeds(kVulkalwertexShader, EShLangVertex));
}

TEST_F(CompilerTest, RespectTargetElwOnOpenGLShader) {
  const EShLanguage stage = EShLangVertex;

  compiler_.SetTargetElw(Compiler::TargetElw::OpenGLCompat);
  EXPECT_TRUE(SimpleCompilationSucceeds(kOpenGLVertexShader, stage));

  compiler_.SetTargetElw(Compiler::TargetElw::OpenGL);
  EXPECT_TRUE(SimpleCompilationSucceeds(kOpenGLVertexShader, stage));
}

TEST_F(CompilerTest, RespectTargetElwOnOpenGLShaderWhenDeducingStage) {
...
TEST_F(CompilerTest, RespectTargetElwOlwulkanShader) {
  compiler_.SetTargetElw(Compiler::TargetElw::Vulkan);
  EXPECT_TRUE(SimpleCompilationSucceeds(kVulkalwertexShader, EShLangVertex));
}

TEST_F(CompilerTest, VulkanSpecificShaderFailsUnderOpenGLCompatibilityRules) {
  compiler_.SetTargetElw(Compiler::TargetElw::OpenGLCompat);
  EXPECT_FALSE(SimpleCompilationSucceeds(kVulkalwertexShader, EShLangVertex));
}
...
TEST_F(CompilerTest, BadTargetElwFails) {
  compiler_.SetTargetElw(static_cast<Compiler::TargetElw>(32767));
  EXPECT_FALSE(SimpleCompilationSucceeds(kVulkalwertexShader, EShLangVertex));
  EXPECT_THAT(errors_, HasSubstr("Invalid target client environment 32767"));
}

TEST_F(CompilerTest, BadTargetElwVulkalwersionFails) {
  compiler_.SetTargetElw(Compiler::TargetElw::Vulkan,
                         static_cast<Compiler::TargetElwVersion>(123));
  EXPECT_FALSE(SimpleCompilationSucceeds(kVulkalwertexShader, EShLangVertex));
  EXPECT_THAT(errors_,
              HasSubstr("Invalid target client version 123 for Vulkan environment 0"));
}

TEST_F(CompilerTest, BadTargetElwOpenGLVersionFails) {
  compiler_.SetTargetElw(Compiler::TargetElw::OpenGL,
                         static_cast<Compiler::TargetElwVersion>(123));
  EXPECT_FALSE(SimpleCompilationSucceeds(kVulkalwertexShader, EShLangVertex));
  EXPECT_THAT(errors_,
...
// A colwert-string-to-vector test case consists of 1) an input string; 2) an
// expected vector after the colwersion.
struct ColwertStringToVectorTestCase {
  std::string input_str;
  std::vector<uint32_t> expected_output_vec;
};

// Test the shaderc_util::ColwertStringToVector() function. The content of the
...
using ColwertStringToVectorTestFixture =
    testing::TestWithParam<ColwertStringToVectorTestCase>;

TEST_P(ColwertStringToVectorTestFixture, VariousStringSize) {
  const ColwertStringToVectorTestCase& test_case = GetParam();
  EXPECT_EQ(test_case.expected_output_vec,
            shaderc_util::ColwertStringToVector(test_case.input_str))
      << "test_case.input_str: " << test_case.input_str << std::endl;
}

INSTANTIATE_TEST_SUITE_P(
    ColwertStringToVectorTest, ColwertStringToVectorTestFixture,
    testing::ValuesIn(std::vector<ColwertStringToVectorTestCase>{
        {"", {0x00000000}},
        {"1", {0x00000031}},
        {"12", {0x00003231}},
        {"123", {0x00333231}},
...
TEST_F(CompilerTest, EmitMessageTextOnlyOnce) {
  ...
  std::tie(result, std::ignore, std::ignore) = c.Compile(
      "#version 150\lwoid MyEntryPoint(){}", EShLangVertex, "shader", "",
...
struct GetGlslangClientInfoCase {
  std::string prefix;
  Compiler::TargetElw elw;
  Compiler::TargetElwVersion elw_version;
...
TEST_P(GetGlslangClientInfoTest, Sample) {
  ...
  auto result = shaderc_util::GetGlslangClientInfo(
      c.prefix, c.elw, c.elw_version, c.spv_version, c.spv_forced);
...
#define CASE_VK(VKVER, SPVVER)                                                 \
  "", Compiler::TargetElw::Vulkan, Compiler::TargetElwVersion::Vulkan_##VKVER, \
      Compiler::SpirvVersion::v##SPVVER

#define BADCASE_VK(STR, VKVER, SPVVER)                \
  STR, Compiler::TargetElw::Vulkan,                   \
      static_cast<Compiler::TargetElwVersion>(VKVER), \
      static_cast<Compiler::SpirvVersion>(SPVVER)

#define CASE_GL(GLVER, SPVVER)                                                 \
  "", Compiler::TargetElw::OpenGL, Compiler::TargetElwVersion::OpenGL_##GLVER, \
      Compiler::SpirvVersion::v##SPVVER

#define BADCASE_GL(STR, GLVER, SPVVER)                \
  STR, Compiler::TargetElw::OpenGL,                   \
      static_cast<Compiler::TargetElwVersion>(GLVER), \
      static_cast<Compiler::SpirvVersion>(SPVVER)
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::ValuesIn(std::vector<GetGlslangClientInfoCase>{
        ...
        {BADCASE_VK("foo", Compiler::TargetElwVersion::Vulkan_1_0, 999), true,
         GCASE_VK("error:foo: Unknown SPIR-V version 3e7", 1_0, 1_0)},
        {BADCASE_GL("foo", Compiler::TargetElwVersion::OpenGL_4_5, 999), true,
