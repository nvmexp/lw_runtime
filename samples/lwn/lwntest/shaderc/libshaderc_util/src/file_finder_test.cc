...
class FileFinderTest : public testing::Test {
 protected:
  ...
  const std::string lwrrent_dir = GetLwrrentDir();
};
...
TEST_F(FileFinderTest, LwrrentDirectory) {
  ASSERT_GE(lwrrent_dir.size(), 0u);
  // Either the directory should start with / (if we are on Linux),
  // Or it should beither X:/ or X:\ or // (if we are on Windows).
  ASSERT_TRUE(lwrrent_dir.front() == '\\' || lwrrent_dir.front() == '/' ||
    (lwrrent_dir.size() >= 3u && lwrrent_dir[1] == ':' &&
    (lwrrent_dir[2] == '\\' || lwrrent_dir[2] == '/')));
}

TEST_F(FileFinderTest, AbsolutePath) {
  ASSERT_NE('/', lwrrent_dir.back());
  finder.search_path() = {lwrrent_dir};
  EXPECT_EQ(lwrrent_dir + "/include_file.1",
            finder.FindReadableFilepath("include_file.1"));
  EXPECT_EQ(lwrrent_dir + "/dir/subdir/include_file.2",
            finder.FindReadableFilepath("dir/subdir/include_file.2"));
}
...
TEST_F(FileFinderTest, AbsoluteFilename) {
  ...
  finder.search_path() = {""};
  const std::string absolute_file1 = lwrrent_dir + "/include_file.1";
