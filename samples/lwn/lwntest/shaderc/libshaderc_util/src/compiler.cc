...
EShMessages GetMessageRules(shaderc_util::Compiler::TargetElw elw,
  ...
  switch (elw) {
    case Compiler::TargetElw::OpenGLCompat:
      break;
...
std::tuple<bool, std::vector<uint32_t>, size_t> Compiler::Compile(
  ...
  // glslang doesn't lwrrently support WebGPU, so we need to fake it by having
  // it generate Vulkan1.1 and then use spirv-opt later to colwert to WebGPU.
  bool is_webgpu = target_elw_ == Compiler::TargetElw::WebGPU;
  TargetElw internal_target_elw =
      !is_webgpu ? target_elw_ : Compiler::TargetElw::Vulkan;
  TargetElwVersion internal_target_elw_version =
      !is_webgpu ? target_elw_version_ : Compiler::TargetElwVersion::Vulkan_1_1;

  // Check target environment.
  const auto target_client_info = GetGlslangClientInfo(
      error_tag, internal_target_elw, internal_target_elw_version,
  ...
  if (output_type == OutputType::PreprocessedText ||
    ...
    if (output_type == OutputType::PreprocessedText) {
      ...
      succeeded = true;
      compilation_output_data = ColwertStringToVector(preprocessed_shader);
  ...
  shader.setElwClient(target_client_info.client,
                      target_client_info.client_version);
  shader.setElwTarget(target_client_info.target_language,
                      target_client_info.target_language_version);
  if (hlsl_functionality1_enabled_) {
    shader.setElwTargetHlslFunctionality1();
  }
  shader.setIlwertY(ilwert_y_enabled_);
  shader.setNanMinMaxClamp(nan_clamp_);

  const EShMessages rules =
      GetMessageRules(internal_target_elw, source_language_, hlsl_offsets_,
...
void Compiler::SetTargetElw(Compiler::TargetElw elw,
                            Compiler::TargetElwVersion version) {
  target_elw_ = elw;
  target_elw_version_ = version;
}
...
void Compiler::EnableIlwertY(bool enable) { ilwert_y_enabled_ = enable; }
...
std::pair<EShLanguage, std::string> Compiler::GetShaderStageFromSourceCode(
  ...
  for (size_t i = 0, logical_line_no = 1; i < lines.size(); ++i) {
    const string_piece lwrrent_line = lines[i].strip_whitespace();
    if (lwrrent_line.starts_with(kPragmaShaderStageDirective)) {
      const string_piece stage_value =
          lwrrent_line.substr(kPragmaShaderStageDirective.size()).strip("()");
    ...
    } else if (!lwrrent_line.empty() && !lwrrent_line.starts_with("#")) {
      first_non_pp_line = std::min(first_non_pp_line, i + 1);
    }

    // Update logical line number for the next line.
    if (lwrrent_line.starts_with(kLineDirective)) {
      string_piece name;
      std::tie(logical_line_no, name) = DecodeLineDirective(lwrrent_line);
  ...
  for (size_t i = 1; i < stages.size(); ++i) {
    const string_piece& lwrrent_stage = std::get<2>(stages[i]);
    if (lwrrent_stage != first_pragma_stage) {
      const string_piece& lwrrent_filename = std::get<0>(stages[i]);
      const std::string lwrrent_line = std::to_string(std::get<1>(stages[i]));
      error_message += lwrrent_filename.str() + ":" + lwrrent_line +
                       ": error: '#pragma': conflicting stages for "
                       "'shader_stage' #pragma: '" +
                       lwrrent_stage.str() + "' (was '" +
...
// Colwerts a string to a vector of uint32_t by copying the content of a given
// string to a vector<uint32_t> and returns it. Appends '\0' at the end if extra
// bytes are required to complete the last element.
std::vector<uint32_t> ColwertStringToVector(const std::string& str) {
...
GlslangClientInfo GetGlslangClientInfo(
    const std::string& error_tag, shaderc_util::Compiler::TargetElw elw,
    shaderc_util::Compiler::TargetElwVersion elw_version,
  ...
  switch (elw) {
    case Compiler::TargetElw::Vulkan:
      ...
      if (elw_version == Compiler::TargetElwVersion::Default ||
          elw_version == Compiler::TargetElwVersion::Vulkan_1_0) {
        result.client_version = glslang::EShTargetVulkan_1_0;
      } else if (elw_version == Compiler::TargetElwVersion::Vulkan_1_1) {
        result.client_version = glslang::EShTargetVulkan_1_1;
        result.target_language_version = glslang::EShTargetSpv_1_3;
      } else if (elw_version == Compiler::TargetElwVersion::Vulkan_1_2) {
        ...
        errs << "error:" << error_tag << ": Invalid target client version "
             << static_cast<uint32_t>(elw_version) << " for Vulkan environment "
             << int(elw);
      }
      break;
    case Compiler::TargetElw::OpenGLCompat:  // TODO(dneto): remove this
    case Compiler::TargetElw::OpenGL:
      result.client = glslang::EShClientOpenGL;
      if (elw_version == Compiler::TargetElwVersion::Default ||
          elw_version == Compiler::TargetElwVersion::OpenGL_4_5) {
        result.client_version = glslang::EShTargetOpenGL_450;
      } else {
        errs << "error:" << error_tag << ": Invalid target client version "
             << static_cast<uint32_t>(elw_version) << " for OpenGL environment "
             << int(elw);
      }
      break;
    default:
