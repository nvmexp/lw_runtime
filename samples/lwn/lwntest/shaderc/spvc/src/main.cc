...
Options:
  ...
  --source-elw=<elw>       Exelwtion environment of the input source.
                             <elw> is vulkan1.0 (the default), vulkan1.1,
                             or webgpu0
  ...
  --msl-version=<ver>      Specify MSL output language version, e.g. 100
                             Default is 10200.
  --target-elw=<elw>       Target intermediate exelwtion environment to
                           transform the source to before cross-compiling.
                           Defaults to the value set for source-elw.
                           <elw> must be one of the legal values for source-elw.

                           If target-elw and source-elw are the same, then no
...
bool StringPieceToElwEnum(const string_piece& str, shaderc_spvc_spv_elw* elw) {
  if (!elw) {
    std::cerr << "spvc: error: internal error calling StringPieceToElwEnum"
              << std::endl;
    return false;
  }

  if (str == "vulkan1.0") {
    *elw = shaderc_spvc_spv_elw_vulkan_1_0;
  } else if (str == "vulkan1.1") {
    *elw = shaderc_spvc_spv_elw_vulkan_1_1;
  } else if (str == "webgpu0") {
    *elw = shaderc_spvc_spv_elw_webgpu_0;
  } else {
    std::cerr << "spvc: error: invalid value '" << str
              << "' in --source-elw= or --target-elw=" << std::endl;
    return false;
  }
  return true;
}
...
int main(int argc, char** argv) {
  ...
  // Scan and find any source and target elw flags, since we need to know those
  // to create options.
  for (int i = 1; i < argc; ++i) {
    const string_piece arg = argv[i];
    if (arg.starts_with("--source-elw=")) {
      string_piece elw;
      shaderc_util::GetOptionArgument(argc, argv, &i, "--source-elw=", &elw);
      source_str = elw;
    } else if (arg.starts_with("--target-elw=")) {
      string_piece elw;
      shaderc_util::GetOptionArgument(argc, argv, &i, "--target-elw=", &elw);
      target_str = elw;
    }
  }
  ...
  shaderc_spvc_spv_elw source_elw;
  if (!StringPieceToElwEnum(source_str, &source_elw)) {
    return 1;
  }

  shaderc_spvc_spv_elw target_elw;
  if (!StringPieceToElwEnum(target_str, &target_elw)) {
    return 1;
  }

  shaderc_spvc::CompileOptions options(source_elw, target_elw);
  for (int i = 1; i < argc; ++i) {
    const string_piece arg = argv[i];
    if (arg == "--help") {
      ::PrintHelp(&std::cout);
    ...
    } else if (arg.starts_with("--source-elw=")) {
