        ...
        for (Index = 0; /* until all devices found or error */; ++Index)
        {
            ...
            if (PCI_OK == PciRc)
            {
                ...
                if (ModsDrvPciRd16(Domain, Bus, Device, Function, 0)
                        == LW_PBUS_PCI_LW_0_VENDOR_ID_LWIDIA)
                {
...
#define MODS_NUM_BARS 3
                    ...
                    pDevice = lw_malloc(sizeof(ModsGpuDevice));
                    lw_memset(pDevice, 0, sizeof(ModsGpuDevice));
                    pSubdevice = lw_malloc(sizeof(ModsGpuSubdevice));
                    ...
                    for (i = 0; i < MODS_NUM_BARS; i++)
                    {
                        ...
                        if (DRF_VAL(_PBUS, _PCI_LW_5, _ADDRESS_TYPE, baseAddressLow) == LW_PBUS_PCI_LW_5_ADDRESS_TYPE_64_BIT)
                        {
                            BarOffset += 4;
                            barIs64Bit = TRUE;
                        }
                    ...
                    PciLw1 |=   DRF_DEF(_PBUS, _PCI_LW_1, _MEMORY_SPACE, _ENABLED)
                              | DRF_DEF(_PBUS, _PCI_LW_1, _BUS_MASTER, _ENABLED);
...
//-------------------------------------------------------------------------------------
ULONG64 lw_strtoull(const char* args, char** endp)
{
    int i = 0;
    int deg = 10;
    ULONG64 res = 0L;
...
BOOL GetExpressionEx(const char *args, ULONG64 *val, char **endp)
{
    ...
    //*val = (ULONG64)simple_strtoull(args, endp, 0);
    //*val = (ULONG64)strtoull(args, endp, 0);
    *val = (ULONG64)lw_strtoull(args, endp);
...
{
    void *Virt = ModsDrvPhysicalToVirtual(physAddr);
    LwU64 Virt64 = (size_t)Virt;
    if (sizeof(Virt) == 4)
        Virt64 &= 0xffffffff;
    return Virt64;
}
...
{
    return ModsDrvVirtualToPhysical((void*)(size_t)addr);
}

//-----------------------------------------------------
// readPhysicalMem
//-----------------------------------------------------
LW_STATUS readPhysicalMem(ULONG64 address, PVOID buf, ULONG size, LwU32 *pSizer)
{
    ...
    ULONG64 virtAddress64 = (size_t)virtAddress;
    if (sizeof(virtAddress) == 4)
        virtAddress64 &= 0xffffffff;

    if (readVirtMem(virtAddress64, buf, size, (PULONG)pSizer) == LW_OK)
    {
        *pSizer = size;
...
LwU32 osGetInputLine(LwU8 *prompt, LwU8 *buffer, LwU32 size)
{
    ...
    while (fgets((char *)buffer, 1024, stdin) == NULL);
    //
    // In case of an empty input strlen will return 1 because 
    // of the trailing '\n'
    //
    charsRead = strlen((char *)buffer) - 1;
...
void* lw_malloc(unsigned int bytes)
{
    ...
    buf = ModsDrvAlloc(bytes);
    if (buf != NULL)
    {
        for(i = 0; i < bytes; i++)
        {
            ModsDrvMemWr08((LwU8 *)buf+i, 0);
        }
...
LW_STATUS writePhysicalMem(ULONG64 address, PVOID buf, ULONG size, LwU32 *pSizer)
{
    void *virtAddress = ModsDrvMapDeviceMemory(address, size, ATTRIB_UC, PROTECT_READ_WRITE);
    ...
    if (writeVirtMem(virtAddress64, buf, size, (PULONG)pSizer) == LW_OK)
    {
        *pSizer = size;
...
LW_STATUS readVirtMem(ULONG64 address, PVOID buf, ULONG size, PULONG pSizer)
{
    unsigned int i;
    int leftBytes;
    int bytesRead = 0;
...
LW_STATUS writeVirtMem(ULONG64 address, PVOID buf, ULONG size, PULONG pSizer)
{
    unsigned int i;
...
void REG_WR32(PhysAddr off, LwU32 data)
{
    if (regBase)
    {
...
LwU8 REG_RD08(PhysAddr off)
{
    assert(regBase);
    return ModsDrvMemRd08(&regBase->Reg008[off]);
}

void REG_WR08(PhysAddr off, LwU8 data)
{
    assert(regBase);
...
void WriteIoSpace(LwU16 offset, ULONG val, ULONG *size)
{
    switch(*size)
    {
        case '1': ModsDrvIoWr08(offset, (UINT08)val); break;
        case '2': ModsDrvIoWr16(offset, (UINT16)val); break;
        case '4': ModsDrvIoWr32(offset, val); break;
    }
}

void ReadIoSpace(LwU16 offset, ULONG *val, ULONG *size)
{
    switch(*size)
    {
        case '1': *val = ModsDrvIoRd08(offset); break;
        case '2': *val = ModsDrvIoRd16(offset); break;
        case '4': *val = ModsDrvIoRd32(offset); break;
    }
}
...
char* lw_strcpy(char *dest, const char *source)
{
    ...
    lens = lw_strlen(source);

    // copy all the source string and its termination zero to dest
    lw_memcpy(dest,source,lens+1);
#endif

    // this should be done because strcpy is defined to call lw_strcpy
    // and we get infinite relwrsion
#undef strcpy
    strcpy(dest,source);
#define strcpy lw_strcpy
...
int lw_strcmp(char *dest, const char *source)
{
    ...
    // process the strings until they are equal
    while(*((LwU8*)dest+len) == *((LwU8*)source+len))
    {
        // if this is the end of both strings
        if(*((LwU8*)dest+len) == 0)
            return 0;
    ...
    // compare the different chars
    if(*((LwU8*)dest+len) < *((LwU8*)source+len))
        return -1;
    else
        return 1;
#endif

    // this should be done because strcmp is defined to call lw_strcmp
    // and we get infinite relwrsion
#undef strcmp
...
char* lw_strcat(char *dest, const char *source)
{
    ...
    // get both string's lengths
    lend = lw_strlen(dest);
    lens = lw_strlen(source);

    // copy source after the end of dest and copy
    // the termination zero, too
    lw_memcpy(dest+lend, source, lens+1);

    return dest;
#endif

    // this should be done because strcat is defined to call lw_strcat
    // and we get infinite relwrsion
#undef strcat
    return (char*)strcat(dest,source);
#define strcat lw_strcat
}
...
int lw_strlen(const char *string)
{
    ...
    // process the string until end is reached
    while(*((LwU8*)string+len) != 0) len++;

    return len;
#endif

    // this should be done because strlen is defined to call lw_strlen
    // and we get infinite relwrsion
#undef strlen
...
{
    // this should be done because getelw is defined to call lw_getelw
    // and we get infinite relwrsion
#undef  getelw
    return (char*)getelw(str);
...
LwU8 SYSMEM_RD08(LwU64 pa)
{
    ...
    readPhysicalMem(pa, &buf, 1, &pSizer);
...
LwU16 SYSMEM_RD16(LwU64 pa)
{
    ...
    readPhysicalMem(pa, &buf, 2, &pSizer);
...
LwU32 SYSMEM_RD32(LwU64 pa)
{
   ...
   if (readPhysicalMem(pa, &buf, 4, &pSizer) != LW_OK)
       buf = (LwU32) -1;
   return buf;
}
...
{
    return (LwU32)ModsDrvPciRd32(DomainNumber, BusNumber, Device, Function, Offset);
}
...
{
    return (LwU32)ModsDrvPciRd16(DomainNumber, BusNumber, Device, Function, Offset);
}
...
{
    return (LwU32)ModsDrvPciRd08(DomainNumber, BusNumber, Device, Function, Offset);
}
