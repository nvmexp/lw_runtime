...
#if VER_NEED_LWRRENT != 1
#error libelf lwrrently does not support VER_NEED_LWRRENT != 1
#endif /* VER_NEED_LWRRENT != 1 */
...
xlt_verneed(unsigned char *dst, const unsigned char *src, size_t n, unsigned enc) {
	...
	if (vn.vn_version < 1
	 || vn.vn_version > VER_NEED_LWRRENT) {
