...
get_attr_dbg(Dwarf_Debug *dbg,
    ...
    Dwarf_Error *error)
{
    Dwarf_LW_Context lwp;
    if (attr == NULL) {
        _dwarf_error(NULL, error, DW_DLE_ATTR_NULL);
        return (DW_DLV_ERROR);
    }

    lwp = attr->ar_lw_context;
    if (lwp == NULL) {
        _dwarf_error(NULL, error, DW_DLE_ATTR_NO_LW_CONTEXT);
        return (DW_DLV_ERROR);
    }
    ...
    Dwarf_Bool * return_bool, Dwarf_Error * error)
{
    ...
    int res  =get_attr_dbg(&dbg,&lw_context, attr,error);
    if (res != DW_DLV_OK) {
        return res;
    }
...
/*  Colwert an offset within the local LW into a section-relative
    debug_info offset. See dwarf_global_formref() and dwarf_formref()
    for additional information on colwersion rules.
*/
int
dwarf_colwert_to_global_offset(Dwarf_Attribute attr,
...
/*  A global offset cannot be returned by this interface:
    ...
    (to various sections, not a LW relative offset.

    DW_FORM_ref_addr has a value which was dolwmented in
   ...
   Dwarf_Off * ret_offset, Dwarf_Error * error)
{
    ...
    *ret_offset = 0;
    res  = get_attr_dbg(&dbg,&lw_context,attr,error);
    if (res != DW_DLV_OK) { 
        return res;
    } 
    switch (attr->ar_attribute_form) {
    ...
    case DW_FORM_ref_sig8: 
        /*  We cannot handle this here.
            ...
            not a .debug_info LW local offset. */
    ...
    /* Check that offset is within current lw portion of .debug_info. */

    maximumoffset = lw_context->cc_length +
        lw_context->cc_length_size +
        lw_context->cc_extension_size;
    if (offset >= maximumoffset) {
        /*  For the DW_TAG_compile_unit is legal to have the
            DW_AT_sibling attribute outside the current lw portion of
            .debug_info. 
            In other words, sibling points to the end of the LW.
    ...
    Dwarf_Error*     error)
{
    ...
    dataptr = lw_context->cc_is_info? dbg->de_debug_info.dss_data:
        dbg->de_debug_types.dss_data;

    field_end_offset = attr->ar_debug_ptr + sizeof(Dwarf_Sig8) -
        (dataptr + lw_context->cc_debug_offset);
    /* Check that offset is within current lw portion of .debug_info. */
    if (field_end_offset > lw_context->cc_length +
    ...
    Dwarf_Off * ret_offset, Dwarf_Error * error)
{
    ...
    context_version = lw_context->cc_version_stamp;
    switch (attr->ar_attribute_form) {
    ...
    case DW_FORM_ref_udata:
        ...
        if (offset >= lw_context->cc_length +
            lw_context->cc_length_size +
            lw_context->cc_extension_size) {
            _dwarf_error(dbg, error, DW_DLE_ATTR_FORM_OFFSET_BAD);
            return (DW_DLV_ERROR);
    ...
    /*  The DWARF2 document did not make clear that
        ...
        That was first clearly dolwmented in DWARF3.
        In DWARF4 these two forms are no longer references. */
    case DW_FORM_data4:
        if (context_version == DW_LW_VERSION4) {
    ...
    case DW_FORM_sec_offset:
        {
            ...
            unsigned length_size = lw_context->cc_length_size;
            if (length_size == 4) {
                READ_UNALIGNED(dbg, offset, Dwarf_Unsigned,
                    attr->ar_debug_ptr, sizeof(Dwarf_ufixed));
    ...
    Dwarf_Addr * return_addr, Dwarf_Error * error)
{
    ...
    if (attr->ar_attribute_form == DW_FORM_addr
        /*  || attr->ar_attribute_form == DW_FORM_ref_addr Allowance of
            ...
            not LW-relative offset within the LW in debug_info). The
    ...
    Dwarf_Bool * ret_bool, Dwarf_Error * error)
{
    ...
    lw_context = attr->ar_lw_context;
    if (lw_context == NULL) {
        _dwarf_error(NULL, error, DW_DLE_ATTR_NO_LW_CONTEXT);
        return (DW_DLV_ERROR);
    }

    if (lw_context->cc_dbg == NULL) {
        _dwarf_error(NULL, error, DW_DLE_ATTR_DBG_NULL);
        return (DW_DLV_ERROR);
    }
    ...
    _dwarf_error(lw_context->cc_dbg, error, DW_DLE_ATTR_FORM_BAD);
    return (DW_DLV_ERROR);
}
    ...
    Dwarf_Block ** return_block, Dwarf_Error * error)
{
    ...
    /* Check that block lies within current lw in .debug_info. */
    if (attr->ar_debug_ptr + length >=
        dbg->de_debug_info.dss_data + lw_context->cc_debug_offset +
    ...
    char **return_str, Dwarf_Error * error)
{
    ...
    if (attr->ar_attribute_form == DW_FORM_string) {
        ...
        if (0 == dbg->de_assume_string_in_bounds) {
            /* Check that string lies within current lw in .debug_info. 
            */
            void *end = dbg->de_debug_info.dss_data +
                lw_context->cc_debug_offset +
                lw_context->cc_length + lw_context->cc_length_size +
                lw_context->cc_extension_size;
