    ...
    do {
        ...
        do {
            ...
            /*  The current set of ranges is terminated by
                ...
                does not necessarily terminate the ranges for this LW! 
                There can be multiple sets in that DWARF
                does not explicitly forbid multiple sets. 
                DWARF2,3,4 section 7.20 
                We stop short to avoid overrun of the end of the LW.  */
    ...
    Dwarf_Signed * returned_count, Dwarf_Error * error)
{
    ...
    for (i = 0; i < arange_count; i++) {
        *(arange_block + i) = lwrr_chain->ch_item;
        prev_chain = lwrr_chain;
    ...
    Dwarf_Error * error)
{
    ...
    lwrr_chain = head_chain;
    for (i = 0; i < arange_count; i++) {
        Dwarf_Arange ar = lwrr_chain->ch_item;
    ...
    Dwarf_Arange * returned_arange, Dwarf_Error * error)
{
    ...
    for (i = 0; i < arange_count; i++) {
        lwrr_arange = *(aranges + i);
        if (address >= lwrr_arange->ar_address &&
            address <
            lwrr_arange->ar_address + lwrr_arange->ar_length) {
            *returned_arange = lwrr_arange;
            return (DW_DLV_OK);
        }
    }
...
/*
    ...
    For an arange, the lw_die can only be from debug_info,
    not debug_types, it seems.
*/
int
dwarf_get_lw_die_offset(Dwarf_Arange arange,
    ...
    Dwarf_Error * error)
{
    ...
    *returned_offset = offset + _dwarf_length_of_lw_header(dbg, offset, true);
    return DW_DLV_OK;
}

/*  This function takes an Dwarf_Arange,
    and returns the offset of the LW header
    in the compilation-unit that the
    arange belongs to.  Returns DW_DLV_ERROR
    on error.   
    Ensures .debug_info loaded so
    the lw_offset is meaningful.  */
int
dwarf_get_arange_lw_header_offset(Dwarf_Arange arange,
    Dwarf_Off * lw_header_offset_returned,
    Dwarf_Error * error)
{
    Dwarf_Debug dbg = 0;
    if (arange == NULL) {
    ...
    /*  Like dwarf_get_arange_info this ensures debug_info loaded:
        the lw_header is in debug_info and will be used else
        we would not call dwarf_get_arange_lw_header_offset. */
    if (!dbg->de_debug_info.dss_data) {
        int res = _dwarf_load_debug_info(dbg, error);
        if (res != DW_DLV_OK) {
            return res;
    ...
    *lw_header_offset_returned = arange->ar_info_offset;
    return DW_DLV_OK;
}
...
/*
    ...
    in the compilation-unit in *lw_die_offset.  It
    returns false on error.
    If lw_die_offset returned ensures .debug_info loaded so
    the lw_die_offset is meaningful.
*/
int
dwarf_get_arange_info(Dwarf_Arange arange,
    Dwarf_Addr * start,
    Dwarf_Unsigned * length,
    Dwarf_Off * lw_die_offset, Dwarf_Error * error)
{
    if (arange == NULL) {
        _dwarf_error(NULL, error, DW_DLE_ARANGE_NULL);
        return (DW_DLV_ERROR);
    ...
    if (lw_die_offset != NULL) {
        ...
        *lw_die_offset =
            offset + _dwarf_length_of_lw_header(dbg, offset,true);
    }
    return (DW_DLV_OK);
}
