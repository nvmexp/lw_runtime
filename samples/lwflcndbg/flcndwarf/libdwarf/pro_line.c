    ...
    Dwarf_Unsigned length, Dwarf_Error * error)
{
    ...
    if (dbg->de_file_entries == NULL) {
        ...
        lwr = dbg->de_file_entries;
        dbg->de_last_file_entry = lwr;
        dbg->de_n_file_entries = 1;
    } else {
        lwr = dbg->de_last_file_entry;
        lwr->dfe_next = (Dwarf_P_F_Entry)
            _dwarf_p_get_alloc(dbg, sizeof(struct Dwarf_P_F_Entry_s));
        if (lwr->dfe_next == NULL) {
            DWARF_P_DBG_ERROR(dbg, DW_DLE_FILE_ENTRY_ALLOC,
                DW_DLV_NOCOUNT);
        }
        lwr = lwr->dfe_next;
    ...
    lwr->dfe_name = (char *) _dwarf_p_get_alloc(dbg, strlen(name) + 1);
    if (lwr->dfe_name == NULL) {
        DWARF_P_DBG_ERROR(dbg, DW_DLE_ALLOC_FAIL, DW_DLV_NOCOUNT);
    }
    strcpy((char *) lwr->dfe_name, name);
    ...
    ptr += nbytes_len;
    lwr->dfe_nbytes = nbytes_idx + nbytes_time + nbytes_len;
    lwr->dfe_next = NULL;
...
void
_dwarf_pro_reg_init(Dwarf_P_Debug dbg, Dwarf_P_Line lwr_line)
{
    lwr_line->dpl_address = 0;
    lwr_line->dpl_file = 1;
    lwr_line->dpl_line = 1;
    lwr_line->dpl_column = 0;
    lwr_line->dpl_is_stmt = dbg->de_line_inits.pi_default_is_stmt;
    lwr_line->dpl_basic_block = false;
    lwr_line->dpl_next = NULL;
    lwr_line->dpl_prologue_end = 0;
    lwr_line->dpl_epilogue_begin = 0;
    lwr_line->dpl_isa = 0;
    lwr_line->dpl_discriminator = 0;
    lwr_line->dpl_opc = 0;
}
