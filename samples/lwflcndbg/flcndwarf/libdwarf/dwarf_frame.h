...
/*  The dwarf 2.0 standard dictates that only the following
    ...
    See dwarf_frame.c for the lwrrently known augmentation strings.
...
struct Dwarf_Reg_Rule_s {
    ...
    /* Register ilwolved in this rule. */
...
enum Dwarf_augmentation_type {
    ...
    aug_armcc,  /* "armcc+" meaning the  cfa callwlation
...
struct Dwarf_Cie_s {
    ...
    /*  These are extracted from the gnu eh_frame
        augmentation if the
        augmentation begins with 'z'. See Linux LSB dolwments.
...
struct cie_fde_prefix_s {
    ...
    Dwarf_Small *  cf_cie_id_addr; /* used for eh_frame callwlations. */
