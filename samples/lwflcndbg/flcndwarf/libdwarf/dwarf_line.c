    ...
    Dwarf_Signed * srcfilecount, Dwarf_Error * error)
{
    ...
    lwrr_chain = (Dwarf_Chain) _dwarf_get_alloc(dbg, DW_DLA_CHAIN, 1);
    if (lwrr_chain == NULL) {
        dwarf_free_line_table_prefix(&line_prefix);
        _dwarf_error(dbg, error, DW_DLE_ALLOC_FAIL);
        return (DW_DLV_ERROR);
    ...
    for (i = 0; i < line_prefix.pf_files_count; i++) {
        *(ret_files + i) = lwrr_chain->ch_item;
        prev_chain = lwrr_chain;
...
static void
update_file_entry(Dwarf_File_Entry  lwr_file_entry,
    Dwarf_File_Entry *file_entries,
    Dwarf_File_Entry *prev_file_entry,
    Dwarf_Sword      *file_entry_count)
{
    if (*file_entries == NULL) {
        *file_entries = lwr_file_entry;
    } else {
        (*prev_file_entry)->fi_next = lwr_file_entry;
    }
    *prev_file_entry = lwr_file_entry;
    (*file_entry_count)++;
}

static void 
update_chain_list( Dwarf_Chain chain_line,
    Dwarf_Chain *head_chain, Dwarf_Chain *lwrr_chain)
{
    if (*head_chain == NULL) {
        *head_chain = chain_line;
    } else {
        (*lwrr_chain)->ch_next = chain_line;
    }
    *lwrr_chain = chain_line;
}
    ...
    Dwarf_Bool dolines, Dwarf_Error * error)
{
    ...
    /*  These pointers are used to build the list of files names by this 
        lw.  lwr_file_entry points to the file name being added, and
        prev_file_entry to the previous one. */
    Dwarf_File_Entry lwr_file_entry = 0; 
    ...
    for (u = 0; u < prefix.pf_files_count; ++u) {
        ...
        lwr_file_entry = (Dwarf_File_Entry)
            _dwarf_get_alloc(dbg, DW_DLA_FILE_ENTRY, 1);
        if (lwr_file_entry == NULL) {
            dwarf_free_line_table_prefix(&prefix);
            _dwarf_error(dbg, error, DW_DLE_ALLOC_FAIL);
            return (DW_DLV_ERROR);
        }

        lwr_file_entry->fi_file_name = pfxfile->lte_filename;
        lwr_file_entry->fi_dir_index = pfxfile->lte_directory_index;
        lwr_file_entry->fi_time_last_mod =
            pfxfile->lte_last_modification_time;

        lwr_file_entry->fi_file_length = pfxfile->lte_length_of_file;

        update_file_entry(lwr_file_entry,&file_entries,
    ...
    while (line_ptr < line_ptr_end) {
        ...
        if (type == LOP_DISCARD) {
            ...
            if (dolines) {
                ...
                /* Mark a line record as being DW_LNS_set_address */
                lwrr_line->li_addr_line.li_l_data.li_is_addr_set = is_addr_set;
                is_addr_set = false;

                lwrr_line->li_address = address;
                lwrr_line->li_addr_line.li_l_data.li_file =
                    (Dwarf_Sword) file;
                lwrr_line->li_addr_line.li_l_data.li_line =
                    (Dwarf_Sword) line;
                lwrr_line->li_addr_line.li_l_data.li_column =
                    (Dwarf_Half) column;
                lwrr_line->li_addr_line.li_l_data.li_is_stmt = is_stmt;
                lwrr_line->li_addr_line.li_l_data.li_basic_block =
                    basic_block;
                lwrr_line->li_addr_line.li_l_data.li_end_sequence =
                    lwrr_line->li_addr_line.li_l_data.
                    li_epilogue_begin = epilogue_begin;
                lwrr_line->li_addr_line.li_l_data.li_prologue_end =
                    prologue_end;
                lwrr_line->li_addr_line.li_l_data.li_isa = isa;
                lwrr_line->li_addr_line.li_l_data.li_discriminator = discriminator;
                lwrr_line->li_context = line_context;
                line_count++;
                ...
                chain_line->ch_item = lwrr_line;
                update_chain_list(chain_line,&head_chain,&lwrr_chain);
            }
            ...
            case DW_LNS_copy:{
                if (dolines) {
                    lwrr_line = (Dwarf_Line) _dwarf_get_alloc(dbg,
                        DW_DLA_LINE, 1);
                    if (lwrr_line == NULL) {
                        dwarf_free_line_table_prefix(&prefix);
                        _dwarf_error(dbg, error, DW_DLE_ALLOC_FAIL);
                    ...
                    /* Mark a line record as being DW_LNS_set_address */
                    lwrr_line->li_addr_line.li_l_data.li_is_addr_set = 
                        is_addr_set;
                    is_addr_set = false;
                    ...
                    lwrr_line->li_addr_line.li_l_data.li_column =
                        (Dwarf_Half) column;
                    lwrr_line->li_addr_line.li_l_data.li_is_stmt =
                        is_stmt;
                    lwrr_line->li_addr_line.li_l_data.
            ...
            case DW_LNE_set_address:{
                ...
                if (doaddrs) {
                    ...
                    is_addr_set = false;
                    lwrr_line->li_address = address;
                    lwrr_line->li_addr_line.li_offset =
            ...
            case DW_LNE_define_file:{
                if (dolines) {
                    ...
                    lwr_file_entry->fi_file_name = (Dwarf_Small *) line_ptr;
                    line_ptr = line_ptr + strlen((char *) line_ptr) + 1;
                    lwr_file_entry->fi_dir_index = (Dwarf_Sword)
                        _dwarf_decode_u_leb128(line_ptr, &leb128_length);
                    line_ptr = line_ptr + leb128_length;
                    lwr_file_entry->fi_time_last_mod =
                        _dwarf_decode_u_leb128(line_ptr, &leb128_length);
                    line_ptr = line_ptr + leb128_length;
                    lwr_file_entry->fi_file_length =
                        _dwarf_decode_u_leb128(line_ptr, &leb128_length);
                    line_ptr = line_ptr + leb128_length;
                    update_file_entry(lwr_file_entry,&file_entries,
                        &prev_file_entry,&file_entry_count);
    ...
    for (i = 0; i < line_count; i++) {
        *(block_line + i) = lwrr_chain->ch_item;
        head_chain = lwrr_chain;
        lwrr_chain = lwrr_chain->ch_next;
...
dwarf_pclines(Dwarf_Debug dbg,
    ...
    Dwarf_Signed * linecount, Dwarf_Error * error)
{
    /*  Scans the line matrix for the current lw to which a pointer
        exists in dbg. */
    Dwarf_Line line;
    ...
    for (i = 0; i < dbg->de_lw_line_count; i++) {

        line = *(dbg->de_lw_line_ptr + i);
        prev_line = first_line ? NULL : *(dbg->de_lw_line_ptr + i - 1);
    ...
    int *err_count_out)
{
    ...
    if (version != LWRRENT_VERSION_STAMP &&
        version != LWRRENT_VERSION_STAMP3 &&
        version != LWRRENT_VERSION_STAMP4) {
    ...
    if (version == LWRRENT_VERSION_STAMP4) { 
    ...
    while (*(char *) line_ptr != '\0') {
        ...
        struct Line_Table_File_Entry_s *lwrline;
        Dwarf_Word leb128_length = 0;
        ...
        lwrline = prefix_out->pf_line_table_file_entries + files_count;

        lwrline->lte_filename = line_ptr;
        line_ptr = line_ptr + strlen((char *) line_ptr) + 1;
