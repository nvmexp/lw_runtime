...
struct ial_s index_into_allocated[ALLOC_AREA_INDEX_TABLE_MAX] = {
    ...
    /* 32 DW_DLA_LW_CONTEXT */
    {20, sizeof(struct Dwarf_LW_Context_s), BASE_ALLOC, 0, 0},
    ...
    if (type == DW_DLA_ERROR) {
        ...
        if (alloc_area == 0) {
            /*  This is the special case of a failed dwarf_init(). Also
                ...
                ilwolved (because of a libdwarf-caller-error) so
...
/*
    ...
    allocation types lwrrently defined.  
    ...
    types that are lwrrently being used.
...
dwarf_print_memory_stats(Dwarf_Debug dbg)
{
    ...
    printf("   Alloc Type                   Lwrr  Structs byt   str\n");
    printf("   ----------                   ----  ------- per   per\n");
    for (i = 1; i <= MAX_DW_DLA; i++) {
...
#ifndef DWARF_SIMPLE_MALLOC
/*
    This relwrsively frees
    the chunks still allocated, and
    forward chained through the aa_next
    pointer.
*/
static void
_dwarf_relwrsive_free(Dwarf_Alloc_Area alloc_area)
{
    if (alloc_area->aa_next != NULL) {
        _dwarf_relwrsive_free(alloc_area->aa_next);
    }
...
freecontextlist(Dwarf_Debug dbg, Dwarf_Debug_InfoTypes dis)
{
    ...
    Dwarf_LW_Context nextcontext = 0;
    for (context = dis->de_lw_context_list;
        ...
        dwarf_dealloc(dbg, context, DW_DLA_LW_CONTEXT);
    }
}
...
/*
    ...
    In case of diffilwlty, this function simply returns quietly.
*/
int
_dwarf_free_all_of_one_debug(Dwarf_Debug dbg)
{
    ...
    if (dbg->de_simple_malloc_base) {
        ...
        while (smp) {
            ...
            for (i = 0; i < smp->sr_used; ++i) {
                struct simple_malloc_entry_s *lwr;
    ...
    for (i = 1; i < ALLOC_AREA_REAL_TABLE_MAX; i++) {
        ...
        alloc_hdr = &dbg->de_alloc_hdr[indx];
        if (alloc_hdr->ah_alloc_area_head != NULL) {
            _dwarf_relwrsive_free(alloc_hdr->ah_alloc_area_head);
        }
    }
...
{
    struct simple_malloc_record_s *lwr;
    ...
    lwr = dbg->de_simple_malloc_base;

    if (lwr->sr_used >= DSM_BLOCK_COUNT) {
        ...
        newblock->sr_next = lwr;
        lwr = newblock;
    }
    newentry = &lwr->sr_entry[lwr->sr_used];
    ...
    if (dbg->de_simple_malloc_base) {
        ...
        while (smp) {
            ...
            for (i = 0; i < smp->sr_used; ++i) {
                ...
                lwr = &smp->sr_entry[i];
                if (lwr->se_addr == space) {
                    if (lwr->se_type != alloc_type) {
