...
static int
dwarf_next_lw_header_internal(Dwarf_Debug dbg,
    Dwarf_Bool is_info,
    Dwarf_Unsigned * lw_header_length,
...
/*
    ...
    if a LW that includes the given offset has been read 
    or not.  If yes, it returns the Dwarf_LW_Context 
    for the LW.  Otherwise it returns NULL.  Being an 
    ...
    **This is a sequential search.  May be too slow.
    ...
    this.
*/
static Dwarf_LW_Context
_dwarf_find_LW_Context(Dwarf_Debug dbg, Dwarf_Off offset,Dwarf_Bool is_info)
{
    Dwarf_LW_Context lw_context = 0;
    ...
    if (dis->de_lw_context != NULL &&
        dis->de_lw_context->cc_next != NULL &&
        dis->de_lw_context->cc_next->cc_debug_offset == offset) {

        return (dis->de_lw_context->cc_next);
    }

    if (dis->de_lw_context != NULL &&
        dis->de_lw_context->cc_debug_offset <= offset) {

        for (lw_context = dis->de_lw_context;
            lw_context != NULL; lw_context = lw_context->cc_next) {

            if (offset >= lw_context->cc_debug_offset &&
                offset < lw_context->cc_debug_offset +
                lw_context->cc_length + lw_context->cc_length_size
                + lw_context->cc_extension_size) {
    ...
    for (lw_context = dis->de_lw_context_list;
        lw_context != NULL; lw_context = lw_context->cc_next) {
...
/*  This routine checks the dwarf_offdie() list of 
    LW contexts for the right LW context.  */
static Dwarf_LW_Context
_dwarf_find_offdie_LW_Context(Dwarf_Debug dbg, Dwarf_Off offset,
    Dwarf_Bool is_info)
{
    Dwarf_LW_Context lw_context = 0;
    Dwarf_Debug_InfoTypes dis = is_info? &dbg->de_info_reading: 
    ...
    for (lw_context = dis->de_offdie_lw_context;
        lw_context != NULL; lw_context = lw_context->cc_next)
...
/*  This function is used to create a LW Context for
    a compilation-unit that begins at offset in 
    .debug_info.  The LW Context is attached to the
    list of LW Contexts for this dbg.  It is assumed
    that the LW at offset has not been read before,
    and so do not call this routine before making
    sure of this with _dwarf_find_LW_Context().
...
static Dwarf_LW_Context
_dwarf_make_LW_Context(Dwarf_Debug dbg,
    Dwarf_Off offset,Dwarf_Bool is_info,Dwarf_Error * error)
{
    ...
    lw_context =
        (Dwarf_LW_Context) _dwarf_get_alloc(dbg, DW_DLA_LW_CONTEXT, 1);
    if (lw_context == NULL) {
        _dwarf_error(dbg, error, DW_DLE_ALLOC_FAIL);
        return (NULL);
    }
    lw_context->cc_dbg = dbg;
    lw_context->cc_is_info = is_info;

    {
        Dwarf_Small *dataptr = is_info? dbg->de_debug_info.dss_data:
            dbg->de_debug_types.dss_data;
        lw_ptr = (Dwarf_Byte_Ptr) (dataptr+offset);
    }

    /* READ_AREA_LENGTH updates lw_ptr for consumed bytes */
    READ_AREA_LENGTH(dbg, length, Dwarf_Unsigned,
        lw_ptr, local_length_size, local_extension_size);
    lw_context->cc_length_size = local_length_size;
    lw_context->cc_extension_size = local_extension_size;


    lw_context->cc_length = (Dwarf_Word) length;

    READ_UNALIGNED(dbg, lw_context->cc_version_stamp, Dwarf_Half,
        lw_ptr, sizeof(Dwarf_Half));
    lw_ptr += sizeof(Dwarf_Half);

    READ_UNALIGNED(dbg, abbrev_offset, Dwarf_Unsigned,
        lw_ptr, local_length_size);
    lw_ptr += local_length_size;
    lw_context->cc_abbrev_offset = (Dwarf_Sword) abbrev_offset;

    lw_context->cc_address_size = *(Dwarf_Small *) lw_ptr;
    ++lw_ptr;
    
    

    if (lw_context->cc_address_size  > sizeof(Dwarf_Addr)) {
        _dwarf_error(dbg, error, DW_DLE_LW_ADDRESS_SIZE_BAD);
        return (NULL);
    }
    if (!is_info) {
        /* debug_types LW headers have extra header bytes. */
        types_extra_len = sizeof(signaturedata) + local_length_size;
    }
    if ((length < (LW_VERSION_STAMP_SIZE + local_length_size + 
        LW_ADDRESS_SIZE_SIZE + types_extra_len)) ||
        ((offset + length + local_length_size + local_extension_size) > 
            section_size)) {

        dwarf_dealloc(dbg, lw_context, DW_DLA_LW_CONTEXT);
        _dwarf_error(dbg, error, DW_DLE_LW_LENGTH_ERROR);
        return (NULL);
    }

    if (lw_context->cc_version_stamp != LWRRENT_VERSION_STAMP
        && lw_context->cc_version_stamp != LWRRENT_VERSION_STAMP3
        && lw_context->cc_version_stamp != LWRRENT_VERSION_STAMP4) {
        dwarf_dealloc(dbg, lw_context, DW_DLA_LW_CONTEXT);
        _dwarf_error(dbg, error, DW_DLE_VERSION_STAMP_ERROR);
        return (NULL);
    }
    if (!is_info) {
        if (lw_context->cc_version_stamp != LWRRENT_VERSION_STAMP4) {
            dwarf_dealloc(dbg, lw_context, DW_DLA_LW_CONTEXT);
        ...
        memcpy(&signaturedata,lw_ptr,sizeof(signaturedata));
        lw_ptr += sizeof(signaturedata);
        READ_UNALIGNED(dbg, typeoffset, Dwarf_Unsigned,
            lw_ptr, local_length_size);
        lw_context->cc_typeoffset = typeoffset;
        lw_context->cc_signature = signaturedata;
        {
            Dwarf_Unsigned lw_len = length - (local_length_size + 
                local_extension_size);
            if (typeoffset >= lw_len) {
                dwarf_dealloc(dbg, lw_context, DW_DLA_LW_CONTEXT);
    ...
    lw_context->cc_abbrev_hash_table =
        (Dwarf_Hash_Table) _dwarf_get_alloc(dbg, DW_DLA_HASH_TABLE, 1);
    if (lw_context->cc_abbrev_hash_table == NULL) {
        _dwarf_error(dbg, error, DW_DLE_ALLOC_FAIL);
        return (NULL);
    }

    lw_context->cc_debug_offset = (Dwarf_Word) offset;

    dis->de_last_offset = (Dwarf_Word) (offset + length +
        local_extension_size + local_length_size);

    if (dis->de_lw_context_list == NULL) {
        dis->de_lw_context_list = lw_context;
        dis->de_lw_context_list_end = lw_context;
    } else {
        dis->de_lw_context_list_end->cc_next = lw_context;
        dis->de_lw_context_list_end = lw_context;
    }
...
/*  Returns offset of next compilation-unit thru next_lw_offset
    pointer.
    It sequentially moves from one
    lw to the next.  The current lw is recorded
    internally by libdwarf.

    The _b form is new for DWARF4 adding new returned fields.  */
int
dwarf_next_lw_header(Dwarf_Debug dbg,
    Dwarf_Unsigned * lw_header_length,
    Dwarf_Half * version_stamp,
    Dwarf_Unsigned * abbrev_offset,
    Dwarf_Half * address_size,
    ...
    Dwarf_Error * error)
{
    ...
    return dwarf_next_lw_header_internal(dbg,
        is_info,
        lw_header_length,
        version_stamp,
        abbrev_offset,
...
int
dwarf_next_lw_header_b(Dwarf_Debug dbg,
    Dwarf_Unsigned * lw_header_length,
    Dwarf_Half * version_stamp,
    Dwarf_Unsigned * abbrev_offset,
    Dwarf_Half * address_size,
...
int
dwarf_next_lw_header_c(Dwarf_Debug dbg,
    Dwarf_Bool is_info,
    Dwarf_Unsigned * lw_header_length,
    Dwarf_Half * version_stamp,
    Dwarf_Unsigned * abbrev_offset,
...
dwarf_next_lw_header_internal(Dwarf_Debug dbg,
    ...
    Dwarf_Error * error)
{
    /* Offset for current and new LW. */
    Dwarf_Unsigned new_offset = 0;

    /* LW Context for current LW. */
    Dwarf_LW_Context lw_context = 0;
    ...
    /*  Get offset into .debug_info of next LW. If dbg has no context,
    ...
    if (dis->de_lw_context == NULL) {
        ...
        new_offset = dis->de_lw_context->cc_debug_offset +
            dis->de_lw_context->cc_length +
            dis->de_lw_context->cc_length_size +
            dis->de_lw_context->cc_extension_size;
    }

    /*  Check that there is room in .debug_info beyond the new offset
        for at least a new lw header. If not, return 0 to indicate end
        of debug_info section, and reset de_lw_debug_info_offset to
        enable looping back through the lw's. */
    section_size = is_info? dbg->de_debug_info.dss_size:
        dbg->de_debug_types.dss_size;
    if ((new_offset + _dwarf_length_of_lw_header_simple(dbg,is_info)) >= 
        section_size) {
        dis->de_lw_context = NULL;
        return (DW_DLV_NO_ENTRY);
    }

    /* Check if this LW has been read before. */
    lw_context = _dwarf_find_LW_Context(dbg, new_offset,is_info);

    /* If not, make LW Context for it. */
    if (lw_context == NULL) {
        lw_context = _dwarf_make_LW_Context(dbg, new_offset,is_info, error);
        if (lw_context == NULL) {
            /*  Error if LW Context could not be made. Since
                _dwarf_make_LW_Context has already registered an error
    ...
    dis->de_lw_context = lw_context;

    if (lw_header_length != NULL) {
        *lw_header_length = lw_context->cc_length;
    }

    if (version_stamp != NULL) {
        *version_stamp = lw_context->cc_version_stamp;
    }
    if (abbrev_offset != NULL) {
        *abbrev_offset = lw_context->cc_abbrev_offset;
    }

    if (address_size != NULL) {
        *address_size = lw_context->cc_address_size;
    }
    if (offset_size != NULL) {
        *offset_size = lw_context->cc_length_size;
    }
    if (extension_size != NULL) {
        *extension_size = lw_context->cc_extension_size;
    }
    if (!is_info) {  
        if (signature) {
            *signature = lw_context->cc_signature;
        }
        if (typeoffset) {
            *typeoffset = lw_context->cc_typeoffset;
        }
    }

    new_offset = new_offset + lw_context->cc_length +
        lw_context->cc_length_size + lw_context->cc_extension_size;
    *next_lw_offset = new_offset;
    return (DW_DLV_OK);
}

static int 
dwarf_ptr_LW_offset(Dwarf_LW_Context lw_context,
    Dwarf_Byte_Ptr di_ptr,
    Dwarf_Bool is_info,
    Dwarf_Off * lw_off)
{
    Dwarf_Debug dbg = lw_context->cc_dbg;
    Dwarf_Small *dataptr = is_info? dbg->de_debug_info.dss_data:
        dbg->de_debug_types.dss_data;
    *lw_off = (di_ptr - dataptr);
    return DW_DLV_OK;
}
#if 0
...
void print_ptr_offset(Dwarf_LW_Context lw_context,Dwarf_Byte_Ptr di_ptr)
{
    Dwarf_Off ptr_off;
    dwarf_ptr_LW_offset(lw_context,di_ptr,&ptr_off);
...
dwarf_validate_die_sibling(Dwarf_Die sibling,Dwarf_Off *offset)
{
    ...
    dbg = sibling->di_lw_context->cc_dbg;
    ...
    *offset = 0;
    ...
    /* Callwlate global offset used for error reporting */
    dwarf_ptr_LW_offset(sibling->di_lw_context,
...
/*  This function does two slightly different things
    ...
    portion for the lw the die belongs to.  It is used 
    to check that the search for the next die does not 
    cross the end of the current lw.  Lw_info_start points 
    to the start of the .debug_info portion for the 
    current lw, and is used to add to the offset for 
    ...
    die_info_end  points to the last byte+1 of the lw.  */
static Dwarf_Byte_Ptr
_dwarf_next_die_info_ptr(Dwarf_Byte_Ptr die_info_ptr,
    Dwarf_LW_Context lw_context,
    Dwarf_Byte_Ptr die_info_end,
    Dwarf_Byte_Ptr lw_info_start,
    ...
    Dwarf_Bool * has_die_child)
{
    ...
    abbrev_list = _dwarf_get_abbrev_for_code(lw_context, abbrev_code);
    if (abbrev_list == NULL) {
        return (NULL);
    }
    dbg = lw_context->cc_dbg;
        ...
        if (want_AT_sibling && attr == DW_AT_sibling) {
            ...
            case DW_FORM_ref_addr:
                /*  Very unusual.  The FORM is intended to refer to
                    a different LW, but a different LW cannot
                    be a sibling, can it? 
                    We could ignore this and treat as if no DW_AT_sibling
                    present.   Or derive the offset from it and if
                    it is in the same LW use it directly. 
                    The offset here is *supposed* to be a global offset,
                    so adding lw_info_start is wrong  to any offset
                    we find here unless lw_info_start
            ...
            *has_die_child = false;
            ...
            /*  A value beyond die_info_end indicates an error. Exactly
                at die_info_end means 1-past-lw-end and simply means we
                are at the end, do not return NULL. Higher level code
                will detect that we are at the end. */
            if (lw_info_start + offset > die_info_end) {
                /* Error case, bad DWARF. */
                return (NULL);
            }
            /* At or before end-of-lw */
            return (lw_info_start + offset);
        }

        no_sibling_attr:
        if (attr_form != 0) {
            info_ptr += _dwarf_get_size_of_val(lw_context->cc_dbg,
                attr_form, 
                lw_context->cc_address_size,
                info_ptr,
                lw_context->cc_length_size);
...
/*  Given a Dwarf_Debug dbg, and a Dwarf_Die die, it returns 
    ...
    lw in dbg.  Returns DW_DLV_ERROR on error.
    ...
    Dwarf_Die * caller_ret_die, Dwarf_Error * error)
{
    ...
    Dwarf_Byte_Ptr lw_info_start = 0;
    ...
    if (die == NULL) {
        ...
        /*  die_info_end is untouched here, need not be set in this
            branch. */
        Dwarf_Off off2;
        Dwarf_LW_Context context=0;

        /*  If we've not loaded debug_info, de_lw_context will be NULL,
            so no need to laod */

        context = dis->de_lw_context;
        if (context == NULL) {
            _dwarf_error(dbg, error, DW_DLE_DBG_NO_LW_CONTEXT);
            return (DW_DLV_ERROR);
        }

        off2 = context->cc_debug_offset;
        lw_info_start = dataptr + off2;
        die_info_ptr = lw_info_start +
            _dwarf_length_of_lw_header(dbg, off2,is_info);
        die_info_end = lw_info_start + context->cc_length +
        ...
        context = die->di_lw_context;
        lw_info_start = dataptr+ context->cc_debug_offset;
        die_info_end = lw_info_start + context->cc_length +
            context->cc_length_size +
            context->cc_extension_size;
        ...
        do {
            die_info_ptr = _dwarf_next_die_info_ptr(die_info_ptr,
                die->di_lw_context, die_info_end,
                lw_info_start, true, &has_child);
    ...
    ret_die->di_lw_context =
        die == NULL ? dis->de_lw_context : die->di_lw_context;

    DECODE_LEB128_UWORD(die_info_ptr, utmp);
    if (die_info_ptr > die_info_end) {
        /*  We managed to go past the end of the LW!. 
    ...
    ret_die->di_abbrev_list =
        _dwarf_get_abbrev_for_code(ret_die->di_lw_context, abbrev_code);
    if (ret_die->di_abbrev_list == NULL ) {
        dwarf_dealloc(dbg, ret_die, DW_DLA_DIE);
        _dwarf_error(dbg, error, DW_DLE_DIE_ABBREV_LIST_NULL);
        return (DW_DLV_ERROR);
    }
    if (die == NULL && !is_lw_tag(ret_die->di_abbrev_list->ab_tag)) { 
        dwarf_dealloc(dbg, ret_die, DW_DLA_DIE);
        _dwarf_error(dbg, error, DW_DLE_FIRST_DIE_NOT_LW);
        return (DW_DLV_ERROR);
    }
    ...
    Dwarf_Die * caller_ret_die, Dwarf_Error * error)
{
    ...
    die_info_end = dataptr +
        die->di_lw_context->cc_debug_offset +
        die->di_lw_context->cc_length +
        die->di_lw_context->cc_length_size +
        die->di_lw_context->cc_extension_size;

    die_info_ptr =
        _dwarf_next_die_info_ptr(die_info_ptr, die->di_lw_context,
    ...
    ret_die->di_debug_ptr = die_info_ptr;
    ret_die->di_lw_context = die->di_lw_context;
    ...
    if (abbrev_code == 0) {
        ...
        /*  We have arrived at a null DIE, at the end of a LW or the end 
    ...
    ret_die->di_abbrev_code = abbrev_code;
    ret_die->di_abbrev_list =
        _dwarf_get_abbrev_for_code(die->di_lw_context, abbrev_code);
    if (ret_die->di_abbrev_list == NULL) {
        dwarf_dealloc(dbg, ret_die, DW_DLA_DIE);
...
/*  Given a (global, not lw_relative) die offset, this returns
    ...
    Dwarf_Die * new_die, Dwarf_Error * error)
{
    ...
    lw_context = _dwarf_find_LW_Context(dbg, offset,is_info);
    if (lw_context == NULL)
        lw_context = _dwarf_find_offdie_LW_Context(dbg, offset,is_info);
    ...
    if (lw_context == NULL) {
        ...
        if (dis->de_offdie_lw_context_end != NULL) {
            Dwarf_LW_Context llw_context =
                dis->de_offdie_lw_context_end;
            new_lw_offset =
                llw_context->cc_debug_offset +
                llw_context->cc_length +
                llw_context->cc_length_size +
                llw_context->cc_extension_size;
        }


        do {
            if ((new_lw_offset +
                _dwarf_length_of_lw_header_simple(dbg,is_info)) >= section_size) {
                _dwarf_error(dbg, error, DW_DLE_OFFSET_BAD);
                return (DW_DLV_ERROR);
            }

            lw_context =
                _dwarf_make_LW_Context(dbg, new_lw_offset,is_info, error);
            if (lw_context == NULL) {
                /*  Error if LW Context could not be made. Since
                    _dwarf_make_LW_Context has already registered an
            ...
            if (dis->de_offdie_lw_context == NULL) {
                dis->de_offdie_lw_context = lw_context;
                dis->de_offdie_lw_context_end = lw_context;
            } else {
                dis->de_offdie_lw_context_end->cc_next = lw_context;
                dis->de_offdie_lw_context_end = lw_context;
            }

            new_lw_offset = new_lw_offset + lw_context->cc_length +
                lw_context->cc_length_size;

        } while (offset >= new_lw_offset);
    }
    ...
    die->di_abbrev_list =
        _dwarf_get_abbrev_for_code(lw_context, abbrev_code);
