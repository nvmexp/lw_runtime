    ...
    Dwarf_Error * error)
{
    ...
    Dwarf_Small *lwr_stream_bytes_ptr;
    ...
    for (debug_sect = dbg->de_debug_sects; debug_sect != NULL;
        ...
        /*  We want the size of the .debug_info section for this LW
    ...
    lwr_stream_bytes_ptr = stream_bytes;

    if (extension_size) {
        Dwarf_Unsigned x = DISTINGUISHED_VALUE;

        WRITE_UNALIGNED(dbg, lwr_stream_bytes_ptr,
            (const void *) &x, sizeof(x), extension_size);
        lwr_stream_bytes_ptr += extension_size;
    ...
    lwr_stream_bytes_ptr += uword_size;

    /* Write the version as 2 bytes. */
    {
        Dwarf_Half verstamp = LWRRENT_VERSION_STAMP;

        WRITE_UNALIGNED(dbg, lwr_stream_bytes_ptr,
            (const void *) &verstamp,
            sizeof(verstamp), sizeof(Dwarf_Half));
        lwr_stream_bytes_ptr += sizeof(Dwarf_Half);
    }
    ...
    for (nameentry = nameentry_original;
        ...
        /* Copy the type name. */
        strcpy((char *) lwr_stream_bytes_ptr, nameentry->sne_name);
        lwr_stream_bytes_ptr += nameentry->sne_name_len + 1;
    }
