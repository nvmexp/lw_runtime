...
/*  This  implements _dwarf_insert_harmless_error
    ...
    It is a fixed length cirlwlar list to constrain
...
/*  The pointers returned here through errmsg_ptrs_array
    become ilwalidated by any call to libdwarf. Any call.
*/
    ...
    unsigned * errs_count)
{
    ...
    if (count) {
        ...
        if (dhp->dh_next_to_use != dhp->dh_first) {
            unsigned i = 0;
            unsigned lwr = dhp->dh_first;
            for (i = 0;  lwr != dhp->dh_next_to_use; ++i) {
                if (i >= count ) {
                    /* All output spaces are used. */
                    break;
                }
                errmsg_ptrs_array[i] = dhp->dh_errors[lwr];
                lwr = (lwr +1) % dhp->dh_maxcount;
            }
    ...
    char *newerror)
{
    struct Dwarf_Harmless_s *dhp = &dbg->de_harmless_errors;
    unsigned next = 0;
    unsigned lwr = dhp->dh_next_to_use;
    char *msgspace;
    if (!dhp->dh_errors) {
        dhp->dh_errs_count++;
        return;
    }
    msgspace = dhp->dh_errors[lwr];
    safe_strncpy(msgspace, newerror,DW_HARMLESS_ERROR_MSG_STRING_SIZE);
    next = (lwr+1) % dhp->dh_maxcount;
    dhp->dh_errs_count++;
    dhp->dh_next_to_use = next;
    if (dhp->dh_next_to_use ==  dhp->dh_first) {
        /* Array is full set full ilwariant. */
