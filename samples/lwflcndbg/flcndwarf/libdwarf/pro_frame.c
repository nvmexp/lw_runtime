    ...
    Dwarf_Unsigned init_n_bytes, Dwarf_Error * error)
{
    ...
    if (dbg->de_frame_cies == NULL) {
        ...
        dbg->de_n_cie = 1;
        dbg->de_last_cie = lwrcie;
    } else {
        lwrcie = dbg->de_last_cie;
        lwrcie->cie_next = (Dwarf_P_Cie)
            _dwarf_p_get_alloc(dbg, sizeof(struct Dwarf_P_Cie_s));
        if (lwrcie->cie_next == NULL) {
            DWARF_P_DBG_ERROR(dbg, DW_DLE_CIE_ALLOC, DW_DLV_NOCOUNT);
        }
    ...
    lwrcie->cie_version = DW_CIE_VERSION;
    lwrcie->cie_aug = augmenter;
    lwrcie->cie_code_align = code_align;
    lwrcie->cie_data_align = data_align;
    lwrcie->cie_ret_reg = return_reg;
    lwrcie->cie_inst = (char *) init_bytes;
    lwrcie->cie_inst_bytes = (long) init_n_bytes;
    lwrcie->cie_next = NULL;
    return dbg->de_n_cie;
}
    ...
    Dwarf_Signed offset, Dwarf_Error * error)
{
    ...
    char *ptr = 0;
    Dwarf_P_Frame_Pgm lwrinst;
    int nbytes = 0;
    int res = 0;
    char buff1[ENCODE_SPACE_NEEDED];
    Dwarf_P_Debug dbg = fde->fde_dbg;

    lwrinst = (Dwarf_P_Frame_Pgm)
    ...
    lwrinst->dfp_args = ptr;
    lwrinst->dfp_nbytes = nbytes;
    lwrinst->dfp_next = NULL;

    _dwarf_pro_add_to_fde(fde, lwrinst);
    return fde;
}
...
/*  Generic routine to add opcode to fde instructions. val1 and
    ...
    Lwrrently this does not check that the frame
...
void
_dwarf_pro_add_to_fde(Dwarf_P_Fde fde, Dwarf_P_Frame_Pgm lwrinst)
{
    if (fde->fde_last_inst) {
        fde->fde_last_inst->dfp_next = lwrinst;
        fde->fde_last_inst = lwrinst;
        fde->fde_n_inst++;
        fde->fde_n_bytes +=
            (long) (lwrinst->dfp_nbytes + sizeof(Dwarf_Ubyte));
    } else {
        fde->fde_last_inst = lwrinst;
        fde->fde_inst = lwrinst;
        fde->fde_n_inst = 1;
        fde->fde_n_bytes =
            (long) (lwrinst->dfp_nbytes + sizeof(Dwarf_Ubyte));
    }
