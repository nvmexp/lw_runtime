...
/*  Given a Dwarf_Block that represents a location expression,
    ...
    per-LW, not per-object or per dbg. 
    ...
    Dwarf_Error * error)
{
    ...
    while (offset < loc_len) {
        ...
        lwrr_loc->lc_offset = offset;
        lwrr_loc->lc_atom = atom;
        switch (atom) {
        ...
        case DW_OP_bit_piece:   /* DWARF3f */
            ...
            /*  The operator means: push the lwrrently computed 
        ...
        case DW_OP_GNU_implicit_pointer:{  /*  0xf2  GNU */
            ...
            if (version_stamp == LWRRENT_VERSION_STAMP /* 2 */ ) {
                iplen = address_size;
            }
            READ_UNALIGNED(dbg, operand1, Dwarf_Unsigned, loc_ptr,
                iplen);
        ...
        case DW_OP_GNU_regval_type:       /*  0xf5  GNU */
            ...
            offset = offset + leb128_length;
            break;
        case DW_OP_GNU_colwert:       /*  0xf7  GNU */
        ...
        lwrr_loc->lc_number = operand1;
        lwrr_loc->lc_number2 = operand2;

        if (head_loc == NULL)
            head_loc = prev_loc = lwrr_loc;
        else {
            prev_loc->lc_next = lwrr_loc;
            prev_loc = lwrr_loc;
        }
    }
    ...
    for (i = 0; i < op_count; i++) {
        (block_loc + i)->lr_atom = lwrr_loc->lc_atom;
        (block_loc + i)->lr_number = lwrr_loc->lc_number;
        (block_loc + i)->lr_number2 = lwrr_loc->lc_number2;
        (block_loc + i)->lr_offset = lwrr_loc->lc_offset;

        prev_loc = lwrr_loc;
        lwrr_loc = lwrr_loc->lc_next;
...
_dwarf_setup_loc(Dwarf_Attribute attr,
    Dwarf_Debug * dbg_ret,
    Dwarf_LW_Context *lwcontext_ret,
    Dwarf_Half * form_ret, Dwarf_Error * error)
{
    ...
    if (attr->ar_lw_context == NULL) {
        _dwarf_error(NULL, error, DW_DLE_ATTR_NO_LW_CONTEXT);
        return (DW_DLV_ERROR);
    }
    *lwcontext_ret = attr->ar_lw_context;

    dbg = attr->ar_lw_context->cc_dbg;
    if (dbg == NULL) {
    ...
    Dwarf_Signed * listlen_out, Dwarf_Error * error)
{
    ...
    /* ***** BEGIN CODE ***** */
    setup_res = _dwarf_setup_loc(attr, &dbg,&lwcontext, &form, error);
    if (setup_res != DW_DLV_OK) {
        return setup_res;
    }
    address_size = lwcontext->cc_address_size;
    /*  If this is a form_block then it's a location expression. If it's 
        DW_FORM_data4 or DW_FORM_data8 it's a loclist offset */
    if (((lwcontext->cc_version_stamp == LWRRENT_VERSION_STAMP ||
        lwcontext->cc_version_stamp == LWRRENT_VERSION_STAMP3) &&
        (form == DW_FORM_data4 || form == DW_FORM_data8)) ||
        (lwcontext->cc_version_stamp == LWRRENT_VERSION_STAMP4 &&
        ...
        for (lli = 0; lli < loclist_count; ++lli) {
            ...
            locdesc = _dwarf_get_locdesc(dbg, &loc_block,
                address_size,
                lwcontext->cc_length_size,
                lwcontext->cc_version_stamp, 
                lowpc, highpc, 
                error);
            if (locdesc == NULL) {
                _dwarf_cleanup_llbuf(dbg, llbuf, lli);
...
/*  New April 27 2009. Adding addr_size argument for the rare
    cases where an object has LWs with a different address_size.
    ...
    Dwarf_Error * error)
{
    ...
    Dwarf_LW_Context lwrrent_lw_context = 
        info_reading->de_lw_context; 
    Dwarf_Small version_stamp = LWRRENT_VERSION_STAMP;
    Dwarf_Half offset_size = dbg->de_length_size;

    if (lwrrent_lw_context) {
        ...
        version_stamp = lwrrent_lw_context->cc_version_stamp;
        offset_size = lwrrent_lw_context->cc_length_size;
        if (version_stamp < 2) {
            /* This is probably totally silly.  */
            version_stamp = LWRRENT_VERSION_STAMP;
        }
    }
    res = dwarf_loclist_from_expr_b(dbg,
        ...
        addr_size,
        offset_size,
        version_stamp, /* LW context DWARF version */
        llbuf,
        listlen,
        error);
    return res;
