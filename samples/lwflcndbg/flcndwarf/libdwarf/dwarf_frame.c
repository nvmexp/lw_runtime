...
/* 
    ...
    consumer API carefully first.  This function basically exelwtes     
    ...
    exelwted.  A pointer to this list is returned in ret_frame_instr.    
    ...
    (2) If search_pc is true, frame instructions are exelwted till       
    either a location is reached that is greater than the search_pc_val
    provided, or all instructions are exelwted.  At this point the       
...
#define SIMPLE_ERROR_RETURN(code) \
    ...
    Dwarf_Addr lwrrent_loc = initial_loc;       /* code location/
    ...
    /*  This is used to end exelwting frame instructions.  */
    /*  Becomes true when search_pc is true and lwrrent_loc */
    ...
    /*  Counts the number of frame instructions exelwted.  */
    ...
    /*  These are used only when make_instr is true. Lwrr_instr is a
        pointer to the current frame instruction exelwted.
        Lwrr_instr_ptr, head_instr_list, and lwrr_instr_list are used to 
        ...
        Dwarf_Frame_Op structs exelwted. */
    Dwarf_Frame_Op *lwrr_instr;
    Dwarf_Chain lwrr_instr_item, dealloc_instr_item;
    ...
    }
    {
        ...
        if (cie != NULL && cie->ci_initial_table != NULL) {
            ...
            for (; lwrreg < minregcount ;lwrreg++, t1reg++, t2reg++) {
                *t1reg = *t2reg;
            }
    ...
    while ((instr_ptr < final_instr_ptr) && (!search_over)) {
        ...
        case DW_CFA_advance_loc:
            {
                ...
                search_over = search_pc &&
                    (lwrrent_loc + adv_pc > search_pc_val);
                /* If gone past pc needed, retain old pc.  */
                if (!search_over) {
                    lwrrent_loc = lwrrent_loc + adv_pc;
                }
                break;
            }
        ...
        case DW_CFA_set_loc:
            {
                ...
                instr_ptr += address_size;
                if (new_loc != 0 && lwrrent_loc != 0) {
                    /*  Pre-relocation or before lwrrent_loc is set the
                        test comparing new_loc and lwrrent_loc makes no
                        sense. Testing for non-zero (above) is a way
                        (fallible) to check that lwrrent_loc, new_loc
                        are already relocated.  */
                    if (new_loc <= lwrrent_loc) {
        ...
        case DW_CFA_advance_loc1:
            {
                ...
                search_over = search_pc &&
                    (lwrrent_loc + adv_loc > search_pc_val);

                /* If gone past pc needed, retain old pc.  */
                if (!search_over) {
                    lwrrent_loc = lwrrent_loc + adv_loc;
                }
                break;
            }
        ...
        if (make_instr) {
            ...
            lwrr_instr = (Dwarf_Frame_Op *)
                _dwarf_get_alloc(dbg, DW_DLA_FRAME_OP, 1);
            if (lwrr_instr == NULL) {
                SIMPLE_ERROR_RETURN(DW_DLE_DF_ALLOC_FAIL);
            }

            lwrr_instr->fp_base_op = fp_base_op;
            lwrr_instr->fp_extended_op = fp_extended_op;
            lwrr_instr->fp_register = fp_register;
            lwrr_instr->fp_offset = fp_offset;
            lwrr_instr->fp_instr_offset = fp_instr_offset;

            lwrr_instr_item = (Dwarf_Chain)
                _dwarf_get_alloc(dbg, DW_DLA_CHAIN, 1);
            if (lwrr_instr_item == NULL) {
                SIMPLE_ERROR_RETURN(DW_DLE_DF_ALLOC_FAIL);
            }

            lwrr_instr_item->ch_item = lwrr_instr;
            if (head_instr_chain == NULL)
                head_instr_chain = tail_instr_chain = lwrr_instr_item;
            else {
                tail_instr_chain->ch_next = lwrr_instr_item;
                tail_instr_chain = lwrr_instr_item;
            }
        }
    }
    ...
    if (table != NULL) {
        ...
        table->fr_loc = lwrrent_loc;
        for (; lwrreg < minregcount ; lwrreg++, t3reg++, t2reg++) {
            *t2reg = *t3reg;
        }
    ...
    if (make_instr) {
        ...
        lwrr_instr_item = head_instr_chain;
        for (i = 0; i < instr_count; i++) {
            *(head_instr_block + i) =
                *(Dwarf_Frame_Op *) lwrr_instr_item->ch_item;
            dealloc_instr_item = lwrr_instr_item;
            lwrr_instr_item = lwrr_instr_item->ch_next;
...
/*  For g++ .eh_frame fde and cie.
    ...
    colwenient at run time for an allocated 
    ...
    Dwarf_Addr * hipc, Dwarf_Error * error)
{
        ...
        dbg->de_fde_count_eh:dbg->de_fde_count;
    {
        ...
        while (low <= high) {
            middle = (low + high) / 2;
            lwr_fde = fde_data[middle];
            if (pc_of_interest < lwr_fde->fd_initial_location) {
                high = middle - 1;
            } else if (pc_of_interest >=
                (lwr_fde->fd_initial_location +
                lwr_fde->fd_address_range)) {
                low = middle + 1;
            } else {
                fde = fde_data[middle];
                break;
