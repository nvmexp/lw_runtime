...
static int handle_debug_line(Dwarf_Debug dbg, 
    Dwarf_Die lw_die, Dwarf_addr_callback_func cb_func, int *errval);
...
/*  A function which should probably not be used outside
    of SGI.   Intended to be used by exelwtable and
...
handle_debug_info(Dwarf_Debug dbg, int *errval)
{
    ...
    for (nres = dwarf_next_lw_header(dbg, &hdr_length, &version_stamp,
        &abbrev_offset,
        &addr_size, &nxtoff, &err);
        terminate_now == 0 && nres == DW_DLV_OK;
        nres = dwarf_next_lw_header(dbg, &hdr_length, &version_stamp,
            &abbrev_offset,
            &addr_size, &nxtoff, &err)) {
        ...
        sibres = dwarf_siblingof(dbg, lwrdie, &sibdie, &err);
        if (sibres == DW_DLV_OK) {
...
process_this_die_attrs(Dwarf_Debug dbg, Dwarf_Die newdie, int *errval)
{
    ...
    if (DW_TAG_compile_unit == ltag) {
        /*  Because of the way the dwarf_line code works, we do lines 
            ...
            we have lines left unconnected to a LW. of course such 
...
static int
handle_debug_line(Dwarf_Debug dbg, Dwarf_Die lw_die,
    Dwarf_addr_callback_func cb_func, int *errval)
{
    int retval = DW_DLV_OK;
    int res;
    ...
    res =
        _dwarf_line_address_offsets(dbg, lw_die, &addrlist, &offsetlist,
            &count, &err);
    if (res == DW_DLV_OK) {
        for (i = 0; i < count; i++) {
...
/*  We need to add support for this. Lwrrently we do not
