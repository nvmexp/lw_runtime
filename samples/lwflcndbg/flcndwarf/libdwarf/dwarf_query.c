    ...
    Dwarf_Half * ret_addr_size, Dwarf_Error * error)
{
    ...
    address_size = die->di_lw_context->cc_address_size;
    *ret_addr_size = address_size;
    return DW_DLV_OK;
}
...
int
dwarf_die_LW_offset(Dwarf_Die die,
    Dwarf_Off * lw_off, Dwarf_Error * error)
{
    ...
    CHECK_DIE(die, DW_DLV_ERROR);
    lw_context = die->di_lw_context;
    dbg = die->di_lw_context->cc_dbg;
    dataptr = die->di_is_info? dbg->de_debug_info.dss_data:
        dbg->de_debug_types.dss_data;

    *lw_off = (die->di_debug_ptr - dataptr - lw_context->cc_debug_offset);
    return DW_DLV_OK;
}
    ...
    Dwarf_Error *error)
{
    ...
    *lw_off = 0;
    if (DW_DLV_OK == dwarf_dieoffset(die,off,error) &&
        DW_DLV_OK == dwarf_die_LW_offset(die,lw_off,error)) {
        return DW_DLV_OK;
    }
    return DW_DLV_ERROR;
}
...
/*  This function returns the global offset 
    ...
    the LW that this die is a part of.
    Used for correctness checking by dwarfdump.  */
int
dwarf_die_LW_offset_range(Dwarf_Die die,
    Dwarf_Off * lw_off, 
    Dwarf_Off * lw_length,
    Dwarf_Error * error)
{
    ...
    *lw_off = lw_context->cc_debug_offset;
    *lw_length = lw_context->cc_length + lw_context->cc_length_size
    ...
    Dwarf_Signed * attrcnt, Dwarf_Error * error)
{
    ...
    Dwarf_Attribute lwrr_attr = NULL;
    ...
    abbrev_list = _dwarf_get_abbrev_for_code(die->di_lw_context,
    ...
    do {
        ...
        if (attr != 0) {
            ...
            if (head_attr == NULL)
                head_attr = lwrr_attr = new_attr;
            else {
                lwrr_attr->ar_next = new_attr;
                lwrr_attr = new_attr;
            }
            attr_count++;
        }
    ...
    for (i = 0; i < attr_count; i++) {
        *(attr_ptr + i) = lwrr_attr;
        lwrr_attr = lwrr_attr->ar_next;
    }
    ...
    Dwarf_Half attr, Dwarf_Half * attr_form)
{
    ...
    Dwarf_Half lwrr_attr = 0;
    Dwarf_Half lwrr_attr_form = 0;
    Dwarf_Byte_Ptr info_ptr = 0;
    ...
    do {
        ...
        lwrr_attr = (Dwarf_Half) utmp3;
        DECODE_LEB128_UWORD(abbrev_ptr, utmp3);
        lwrr_attr_form = (Dwarf_Half) utmp3;
        if (lwrr_attr_form == DW_FORM_indirect) {
            Dwarf_Unsigned utmp6;

            /* DECODE_LEB128_UWORD updates info_ptr */
            DECODE_LEB128_UWORD(info_ptr, utmp6);
            lwrr_attr_form = (Dwarf_Half) utmp6;
        }

        if (lwrr_attr == attr) {
            *attr_form = lwrr_attr_form;
            return (info_ptr);
        }

        info_ptr += _dwarf_get_size_of_val(die->di_lw_context->cc_dbg,
            lwrr_attr_form, 
            die->di_lw_context->cc_address_size,
            info_ptr, 
            die->di_lw_context->cc_length_size);
    } while (lwrr_attr != 0 || lwrr_attr_form != 0);
...
dwarf_diename(Dwarf_Die die, char **ret_name, Dwarf_Error * error)
{
    ...
    if (info_ptr == NULL) {
        if (attr_form == 0) {
            _dwarf_error(die->di_lw_context->cc_dbg, error,
                DW_DLE_DIE_BAD);
            return (DW_DLV_ERROR);
        }
        return DW_DLV_NO_ENTRY;
    ...
    READ_UNALIGNED(dbg, string_offset, Dwarf_Unsigned,
        info_ptr, die->di_lw_context->cc_length_size);
    ...
    Dwarf_Attribute * ret_attr, Dwarf_Error * error)
{
    ...
    attrib->ar_lw_context = die->di_lw_context;
...
/*
    ...
    oclwrs in die.  Attr is required to be an attribute
    whose form is in the "constant" class.  If attr oclwrs 
...
/* It takes 4 pieces of data (including the FORM)
   to aclwrately determine the form 'class' as dolwmented
    ...
    Dwarf_Half *offset_size)
{
    ...
    *version = lwcontext->cc_version_stamp;
    *offset_size = lwcontext->cc_length_size;
    return DW_DLV_OK;
}
