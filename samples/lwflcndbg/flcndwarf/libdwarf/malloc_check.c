...
{
    struct mc_data_s *lwr = base;

    for (; lwr; lwr = lwr->mc_prev) {
        if (lwr->mc_address == newd->mc_address) {
            if (lwr->mc_type == MC_TYPE_ALLOC) {
                if (lwr->mc_alloc_code == newd->mc_alloc_code) {
                    *addr_match = lwr;
                    *addr_match_num = lwr->mc_alloc_number;
                    return lwr->mc_alloc_number;
                } else {
                    /* code mismatch */
                    *addr_match = lwr;
                    *addr_match_num = lwr->mc_alloc_number;
...
dwarf_malloc_check_complete(char *msg)
{
    ...
    for (; i < HASH_TABLE_SIZE; ++i) {
        struct mc_data_s *lwr = mc_data_hash[i];
        long lwr_chain_length = 0;
        ...
        for (; lwr; lwr = lwr->mc_prev) {
            ++lwr_chain_length;
            if (lwr->mc_type == MC_TYPE_ALLOC) {
                if (lwr->mc_dealloc_noted) {
                    if (lwr->mc_dealloc_noted > 1) {
                        fprintf(stderr,
                            " Duplicate dealloc! entry %ld\n",
                            lwr->mc_alloc_number);
                        print_entry("duplicate dealloc", lwr);
        ...
        if (lwr_chain_length > max_chain_length) {
            max_chain_length = lwr_chain_length;
        }
    }
