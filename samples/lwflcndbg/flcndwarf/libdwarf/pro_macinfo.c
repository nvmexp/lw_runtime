    ...
    size_t maxlen, int *compose_error_type)
{
    ...
    struct dw_macinfo_block_s *lwrblk = dbg->de_lwrrent_macinfo;
    ...
    if (lwrblk == 0) {
        ...
        dbg->de_lwrrent_macinfo = newb;
        lwrblk = newb;
    } else if (lwrblk->mb_avail_len < maxlen) {
        struct dw_macinfo_block_s *newb;
        size_t len;

        /* no space left in block: allocate a new block */
        size_t blen =
            dbg->de_lwrrent_macinfo->mb_macinfo_data_space_len * 2;
        if (blen < maxlen) {
            blen = 2 * maxlen;
        }
        len = sizeof(struct dw_macinfo_block_s) + blen;
    ...
    /* now lwrblk has enough room */
    dbg->de_compose_avail = lwrblk->mb_avail_len;
    dbg->de_compose_used_len = lwrblk->mb_used_len;
    nextchar =
        (unsigned char *) (lwrblk->mb_data + dbg->de_compose_used_len);
    *nextchar = code;
...
libdwarf_compose_complete(Dwarf_P_Debug dbg, int *compose_error_type)
{
    ...
    if (dbg->de_compose_used_len > lwrblk->mb_macinfo_data_space_len) {
        *compose_error_type = DW_DLE_MACINFO_INTERNAL_ERROR_SPACE;
        return DW_DLV_ERROR;
    }
    lwrblk->mb_avail_len = dbg->de_compose_avail;
    lwrblk->mb_used_len = dbg->de_compose_used_len;
    return DW_DLV_OK;
}
