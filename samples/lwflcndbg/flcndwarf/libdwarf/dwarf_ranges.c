    ...
    Dwarf_Error * error)
{
    ...
    for (;;) {
        ...
        entry_count++;
        READ_UNALIGNED(dbg,re->lwr.dwr_addr1,
            Dwarf_Addr, rangeptr,
            address_size);
        rangeptr +=  address_size;
        READ_UNALIGNED(dbg,re->lwr.dwr_addr2 ,
            Dwarf_Addr, rangeptr,
            address_size);
        rangeptr +=  address_size;
        if (!base) {
        ...
        if (re->lwr.dwr_addr1 == 0 && re->lwr.dwr_addr2 == 0) {
            re->lwr.dwr_type =  DW_RANGES_END;
            break;
        } else if (re->lwr.dwr_addr1 == MAX_ADDR) {
            re->lwr.dwr_type =  DW_RANGES_ADDRESS_SELECTION;
        } else {
            re->lwr.dwr_type =  DW_RANGES_ENTRY;
        }
    }
    ...
    *listlen = entry_count;
    for (copyindex = 0; lwrre && (copyindex < entry_count); 
        ++copyindex,++ranges_data_out) {

        struct ranges_entry *r = lwrre;
        *ranges_data_out = lwrre->lwr;
        lwrre = lwrre->next;
        free(r);
    }
