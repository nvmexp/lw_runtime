...
/* The 'fixup' here intended for IRIX targets only.
   With a  2+GB Elf64 IRIX exelwtable (under 4GB in size),  
   some DIE offsets wrongly
   got the 32bit upper bit sign extended.  For the lw-header
    ...
    int version_err_num)
{
    ...
    do {
        ...
        /*  Some compilers emit padding at the end of each lw's area.
            pubnames_ptr_past_end_lw records the true area end for this
            lw's data.  Essentially the length in the header and the 0
        ...
        Dwarf_Small *pubnames_ptr_past_end_lw = 0;
        ...
        pubnames_ptr_past_end_lw = pubnames_like_ptr + length;

        READ_UNALIGNED(dbg, version, Dwarf_Half,
            pubnames_like_ptr, sizeof(Dwarf_Half));
        pubnames_like_ptr += sizeof(Dwarf_Half);
        if (version != LWRRENT_VERSION_STAMP) {
            _dwarf_error(dbg, error, version_err_num);
            return (DW_DLV_ERROR);
        }

        /* Offset of LW header in debug section. */
        READ_UNALIGNED(dbg, pubnames_context->pu_offset_of_lw_header,
        ...
        FIX_UP_OFFSET_IRIX_BUG(dbg,
            pubnames_context->pu_offset_of_lw_header,
            "pubnames lw header offset");
        ...
        /*  Read initial offset (of DIE within LW) of a pubname, final
            entry is not a pair, just a zero offset. */
        READ_UNALIGNED(dbg, die_offset_in_lw, Dwarf_Off,
            pubnames_like_ptr,
            pubnames_context->pu_length_size);
        pubnames_like_ptr += pubnames_context->pu_length_size;
        FIX_UP_OFFSET_IRIX_BUG(dbg,
            die_offset_in_lw, "offset of die in lw");

        /* Loop thru pairs. DIE off with LW followed by string. */
        while (die_offset_in_lw != 0) {
            ...
            global->gl_named_die_offset_within_lw = die_offset_in_lw;
            ...
            lwrr_chain->ch_item = (Dwarf_Global) global;

            if (head_chain == NULL)
                head_chain = prev_chain = lwrr_chain;
            else {
                prev_chain->ch_next = lwrr_chain;
                prev_chain = lwrr_chain;
            }
            ...
            pubnames_like_ptr += pubnames_context->pu_length_size;
            FIX_UP_OFFSET_IRIX_BUG(dbg,
                die_offset_in_lw, "offset of next die in lw");
        ...
        /*  If there is some kind of padding at the end of the section,
            ...
            compilers, pubnames_like_ptr == pubnames_ptr_past_end_lw at
            this point */
        pubnames_like_ptr = pubnames_ptr_past_end_lw;
    ...
    lwrr_chain = head_chain;
    for (i = 0; i < global_count; i++) {
        *(ret_globals + i) = lwrr_chain->ch_item;
        prev_chain = lwrr_chain;
        lwrr_chain = lwrr_chain->ch_next;
    ...
    Dwarf_Off * ret_off, Dwarf_Error * error)
{
    ...
    *ret_off = (global->gl_named_die_offset_within_lw +
        global->gl_context->pu_offset_of_lw_header);
    return DW_DLV_OK;
}
...
/*  Given a pubnames entry (or other like section entry)
    ...
    other lw-local die offsets were faked so adding this to 
    such a lw-local offset got a true section offset.
    Now things do as they say (adding *lw_header_offset to
    a lw-local offset gets the section offset).  */
int
dwarf_global_lw_offset(Dwarf_Global global,
    Dwarf_Off * lw_header_offset,
    Dwarf_Error * error)
{
    ...
    /*  In early libdwarf, this incorrectly returned the offset of the
        LW DIE. Now correctly returns the header offset. */
    *lw_header_offset = con->pu_offset_of_lw_header;
...
/*
  ...
  name, symbol DIE offset, and the lw-DIE offset.
    ...
    Dwarf_Error * error)
{
    ...
    off = con->pu_offset_of_lw_header;
    /*  The offset had better not be too close to the end. If it is,
        _dwarf_length_of_lw_header() will step off the end and therefore 
        must not be used. 10 is a meaningless heuristic, but no LW
        header is that small so it is safe. An erroneous offset is due
        to a bug in the tool chain. A bug like this has been seen on
        IRIX with MIPSpro 7.3.1.3 and an exelwtable > 2GB in size and
        with 2 million pubnames entries. */
#define MIN_LW_HDR_SIZE 10
    dbg = con->pu_dbg;
    if (dbg == NULL) {
        _dwarf_error(NULL, error, DW_DLE_DBG_NULL);
        return (DW_DLV_ERROR);
    ...
    if (dbg->de_debug_info.dss_size &&
        ((off + MIN_LW_HDR_SIZE) >= dbg->de_debug_info.dss_size)) {
        _dwarf_error(NULL, error, DW_DLE_OFFSET_BAD);
        return (DW_DLV_ERROR);
    }
#undef MIN_LW_HDR_SIZE
    if (die_offset != NULL) {
        *die_offset = global->gl_named_die_offset_within_lw + off;
    }

    *ret_name = (char *) global->gl_name;

    if (lw_die_offset != NULL) {
        ...
        /*  The offset had better not be too close to the end. If it is, 
            _dwarf_length_of_lw_header() will step off the end and
            therefore must not be used. 10 is a meaningless heuristic,
            but no LW header is that small so it is safe. */
        /* Globals cannot refer to debug_types */
        if ((off + 10) >= dbg->de_debug_info.dss_size) {
            _dwarf_error(NULL, error, DW_DLE_OFFSET_BAD);
            return (DW_DLV_ERROR);
        }
        *lw_die_offset = off + _dwarf_length_of_lw_header(dbg, off,true);
    }
...
/*  We have the offset to a LW header.
    Return thru outFileOffset the offset of the LW DIE.
...
/* ARGSUSED */
int
dwarf_get_lw_die_offset_given_lw_header_offset(Dwarf_Debug dbg,
    Dwarf_Off in_lw_header_offset,
    Dwarf_Off * out_lw_die_offset,
    Dwarf_Error * err)
{
    Dwarf_Off len =
        _dwarf_length_of_lw_header(dbg, in_lw_header_offset,true);

    Dwarf_Off newoff = in_lw_header_offset + len;

    *out_lw_die_offset = newoff;
    return DW_DLV_OK;
}
...
/*  The following version new in October 2011, does allow finding
    ...
    */
int
dwarf_get_lw_die_offset_given_lw_header_offset_b(Dwarf_Debug dbg,
    Dwarf_Off in_lw_header_offset,
    Dwarf_Bool is_info,
    Dwarf_Off * out_lw_die_offset,
    Dwarf_Error * err)
{
    Dwarf_Off len =
        _dwarf_length_of_lw_header(dbg, in_lw_header_offset,is_info);
...
/*  dwarf_LW_dieoffset_given_die returns
    ...
    that is the LW containing the given (passed-in) die. 
    ...
    to get a die pointer to the LW die.  */  
int 
dwarf_LW_dieoffset_given_die(Dwarf_Die die,
    Dwarf_Off*       return_offset,
    Dwarf_Error*     error)
{   
    Dwarf_Off  dieoff = 0;
    Dwarf_LW_Context lwcontext = 0;
    
    CHECK_DIE(die, DW_DLV_ERROR);
    lwcontext = die->di_lw_context;
    dieoff =  lwcontext->cc_debug_offset;
    /*  The following call cannot fail, so no error check. */
    dwarf_get_lw_die_offset_given_lw_header_offset_b(
        lwcontext->cc_dbg, dieoff,
