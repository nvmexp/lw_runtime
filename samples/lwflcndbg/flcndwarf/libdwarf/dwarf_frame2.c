...
#ifdef WIN32
static int __cdecl qsort_compare(const void *elem1, const void *elem2);
#else
...
/*  Adds 'newone' to the end of the list starting at 'head'
    and makes the new one 'lwr'rent. */
static void
chain_up_fde(Dwarf_Fde newone, Dwarf_Fde * head, Dwarf_Fde * lwr)
{
    if (*head == NULL)
        *head = newone;
    else {
...
/*  Adds 'newone' to the end of the list starting at 'head'
    and makes the new one 'lwr'rent. */
static void
chain_up_cie(Dwarf_Cie newone, Dwarf_Cie * head, Dwarf_Cie * lwr)
{
    if (*head == NULL) {
        *head = newone;
    } else {
    ...
    int use_gnu_cie_calc, Dwarf_Error * error)
{
    ...
    /*  New_cie points to the Cie being read, and head_cie_ptr and
        lwr_cie_ptr are used for chaining them up in sequence. 
    ...
    */
    ...
    /*  New_fde points to the Fde being created, and head_fde_ptr and
        lwr_fde_ptr are used to chain them up. */
    Dwarf_Fde head_fde_ptr = NULL;
    Dwarf_Fde lwr_fde_ptr = NULL;
    ...
    */
    ...
    while (frame_ptr < frame_ptr_end) {
        ...
        if (prefix.cf_cie_id == cie_id_value) {
            ...
            chain_up_fde(fde_ptr_to_use, &head_fde_ptr, &lwr_fde_ptr);
            fde_count++;
            /* ASSERT: DW_DLV_OK. */
    ...
    lwr_cie_ptr = head_cie_ptr;
    for (i = 0; i < cie_count; i++) {
        *(cie_list_ptr + i) = lwr_cie_ptr;
        lwr_cie_ptr = lwr_cie_ptr->ci_next;
    }
    ...
    lwr_fde_ptr = head_fde_ptr;
    for (i = 0; i < fde_count; i++) {
        *(fde_list_ptr + i) = lwr_fde_ptr;
        lwr_fde_ptr = lwr_fde_ptr->fd_next;
    }
...
/*  Internal function, not called by consumer code.
    'prefix' has aclwmulated the info up thru the cie-id
    ...
    Dwarf_Cie head_cie_ptr)
{
    ...
    for (lwrfde = head_fde_ptr; lwrfde; lwrfde = nextfde) {
        nextfde = lwrfde->fd_next;
        dwarf_dealloc(lwrfde->fd_dbg, lwrfde, DW_DLA_FDE);
    }
    for (lwrcie = head_cie_ptr; lwrcie; lwrcie = nextcie) {
        Dwarf_Frame frame = lwrcie->ci_initial_table;

        nextcie = lwrcie->ci_next;
        if (frame)
            dwarf_dealloc(lwrcie->ci_dbg, frame, DW_DLA_FRAME);
        dwarf_dealloc(lwrcie->ci_dbg, lwrcie, DW_DLA_CIE);
    }
}
    ...
    Dwarf_Cie head_cie_ptr)
{
    ...
    if (lwr_cie_ptr && cie_ptr == lwr_cie_ptr->ci_cie_start) {
        /* Usually, we use the same cie again and again. */
        *cie_ptr_to_use_out = lwr_cie_ptr;
        return DW_DLV_OK;
    }
...
{
    char *nc = 0;
    Dwarf_Small *lwr_aug_p = aug_data;
    ...
    for (nc = augmentation; *nc; ++nc) {
        ...
        case 'L':
            if (lwr_aug_p > end_aug_p) {
                return DW_DLV_ERROR;
            }
            *lsda_enc_out = *(unsigned char *) lwr_aug_p;
            ++lwr_aug_p;
            break;
        case 'R':
            ...
            *fde_begin_enc_out = *(unsigned char *) lwr_aug_p;
            ++lwr_aug_p;
            break;
        case 'P':{
            int res = DW_DLV_ERROR;
            ...
            encoding = *(unsigned char *) lwr_aug_p;
...
/*  Given augmentation character (the encoding) giving the
    ...
    See LSB (Linux Standard Base)  exception handling dolwments.  */
static int
...
/*  Make the 'cie_id_addr' consistent across .debug_frame and .eh_frame.
    Callwlate a pointer into section bytes given a cie_id, which is
