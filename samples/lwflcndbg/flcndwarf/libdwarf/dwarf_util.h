...
#define CHECK_DIE(die, error_ret_value)                          \
    do {                                                         \
        ...
        if (die->di_lw_context == NULL) {                        \
            _dwarf_error(NULL, error, DW_DLE_DIE_NO_LW_CONTEXT); \
            return(error_ret_value);                             \
        }                                                        \
        if (die->di_lw_context->cc_dbg == NULL) {                \
...
/*  This one handles the v3 64bit extension  
    ...
    It assumes that no LW will have a length
...
/* This single struct is the base for the hash table.
   ...
   all the entries is greater than  10*lwrrent_table_entry_count
   ...
   count gets to 30 or 40 times lwrrent_table_entry_count
...
Dwarf_Abbrev_List
_dwarf_get_abbrev_for_code(Dwarf_LW_Context lw_context,
    Dwarf_Unsigned code);
...
Dwarf_Unsigned _dwarf_length_of_lw_header(Dwarf_Debug,
    Dwarf_Unsigned offset,Dwarf_Bool dinfo);
Dwarf_Unsigned _dwarf_length_of_lw_header_simple(Dwarf_Debug,Dwarf_Bool dinfo);
