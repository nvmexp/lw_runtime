...
/*  Given a form, and a pointer to the bytes encoding 
    ...
    a relwrsive DW_FORM_INDIRECT value.  */
Dwarf_Unsigned
...
/*  This function returns a pointer to a Dwarf_Abbrev_List_s
    ...
    section from the last byte scanned for that LW till either
...
Dwarf_Abbrev_List
_dwarf_get_abbrev_for_code(Dwarf_LW_Context lw_context, Dwarf_Unsigned code)
{
    Dwarf_Debug dbg = lw_context->cc_dbg;
    Dwarf_Hash_Table hash_table_base = lw_context->cc_abbrev_hash_table;
    Dwarf_Hash_Table_Entry entry_base = 0; 
    Dwarf_Hash_Table_Entry entry_lwr = 0; 
    ...
    entry_lwr  = entry_base + hash_num;
   
    /* Determine if the 'code' is the list of synonyms already. */
    for (hash_abbrev_entry = entry_lwr->at_head;
    ...
    abbrev_ptr = lw_context->cc_last_abbrev_ptr != NULL ?
        lw_context->cc_last_abbrev_ptr :
        dbg->de_debug_abbrev.dss_data + lw_context->cc_abbrev_offset;
    ...
    /*  End of abbrev's for this lw, since abbrev code is 0. */
    if (*abbrev_ptr == 0) {
        return (NULL);
    }
    ...
    do {
        ...
        inner_list_entry = (Dwarf_Abbrev_List)
            _dwarf_get_alloc(lw_context->cc_dbg, DW_DLA_ABBREV_LIST, 1);
    ...
    lw_context->cc_last_abbrev_ptr = abbrev_ptr;
...
/*  This callwlation used to be sprinkled all over.
    Now brought to one place.

    We try to aclwrately compute the size of a lw header
    given a known lw header location ( an offset in .debug_info
    or debug_types).  */
/* ARGSUSED */
Dwarf_Unsigned
_dwarf_length_of_lw_header(Dwarf_Debug dbg, Dwarf_Unsigned offset,
    Dwarf_Bool is_info)
{
    ...
    READ_AREA_LENGTH(dbg, length, Dwarf_Unsigned,
        lwptr, local_length_size, local_extension_size);

    final_size = local_extension_size +  /* initial extension, if present */
        local_length_size +     /* Size of lw length field. */
...
/*  Pretend we know nothing about the LW
    and just roughly compute the result.  */
Dwarf_Unsigned
_dwarf_length_of_lw_header_simple(Dwarf_Debug dbg,
    Dwarf_Bool dinfo)
{
    Dwarf_Unsigned finalsize = 0;
    finalsize =  dbg->de_length_size +        /* Size of lw length field. */
        sizeof(Dwarf_Half) +    /* Size of version stamp field. */
