...
/*  On entry:
    ...
    *err_code in this case)
    ...
    mallocs pieces of it (another malloc) for sorting a LW entries
    ...
    int *err_code, Dwarf_Small ** new_line_ptr)
{
    ...
    /*  This points to the end of the statement program prologue for the 
        current lw, and serves to check that the prologue was correctly
        decoded. */
    ...
    struct a_line_area *area_lwrrent = 0;
    long area_count = 0;
    ...
    *any_change = 0;
    ...
    if (remaining_bytes < MINIMUM_POSSIBLE_PROLOG_LEN) {
        /*  We are at the end. Remaining should be zero bytes, padding.
            This is really just 'end of LW buffer' not an error. The is
    ...
    while (line_ptr < line_ptr_end) {
        ...
        if (type == LOP_DISCARD) {
            ...
            case DW_LNE_set_address:{
                ...
                area->ala_length = 0;
                if (area_lwrrent) {
                    area_lwrrent->ala_next = area;
                    area_lwrrent->ala_length =
                        area->ala_offset -
                        area_lwrrent->ala_offset;
                }
                ++area_count;
                area_lwrrent = area;
                if (area_base == 0) {
    ...
    *new_line_ptr = line_ptr;
    ...
    /*  So now we have something to sort. First, finish off the last
        area record: */
    area_lwrrent->ala_length = (line_ptr - orig_line_ptr)
        -area_lwrrent->ala_offset;
