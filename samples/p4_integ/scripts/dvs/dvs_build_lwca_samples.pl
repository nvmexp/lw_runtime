...
sub main
{
  ...
  read_command_line_options (\$mode, \$action, \$environment, \%keywords_include, \%keywords_exclude, \$use_docker);
 ...
 if ($mode eq "dvs" and $action eq "build"  and $use_docker)
    {
      set_paths ($mode);
      set_elwironment_docker();
      set_commands_docker(\$make, \$lwmake, \$msbuild);
      set_compression_commands ();
      ...
      build_samples_docker (\@samples, $make, $msbuild, $lwmake);
    
      package_samples (\@samples);
    }
 elsif ($mode eq "dvs" and $action eq "build")
    {
      set_paths ($mode);
      set_elwironment ();
      set_commands (\$make, \$lwmake, \$msbuild);
