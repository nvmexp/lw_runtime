...
ifeq ($(USE_P4),1)
	...
	ALL_COMPLIANT_SMS_ARCH := 35 37 50 52 53 60 61 70 72 75 80 86 87
	ADDITIONAL_OPTIONS += --lwca-version="11.6"
endif
...
ifeq ($(TARGET_GEN_OS),windows)
	GEN_BUILD_FILE ?= vs2017, vs2019, vs2022
else
	GEN_BUILD_FILE ?= makefile, vs2017, vs2019, vs2022, vscode
endif

AVAILABLE_DEPENDENCIES ?= LWBLAS,LWSPARSE,LWSOLVER,LWRAND,LWFFT,NPP,LWRTC,LWJPEG,X11,screen,OpenMP,IPC,MPI,GL,GLES,DirectX,DirectX12,UVM,FreeImage,CDP,callback,only-64-bit,Stream-Priorities,EGLOutput,FP16,CPP11,EGL,MBCG,MDCG,EGLSync,VULKAN,LWSCI,LwMedia
...
%.ph_clean : 
	...
	@rm -rf $*README.md
	@rm -rf $*findgllib.mk
	@rm -rf $*findgleslib.mk
	@rm -rf $*findvulkan.mk
	@rm -rf $*findegl.mk
	@rm -rf $*findlwsci.mk
	@rm -rf $*findlwmedia.mk
	...
	@rm -rf $*$(shell echo $* | cut -d '/' -f3)_vs2022.sln
	@rm -rf $*$(shell echo $* | cut -d '/' -f3)_vs2022.vcxproj
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2010.sln
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2010.vcxproj
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2012.sln
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2012.vcxproj
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2013.sln
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2013.vcxproj
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2015.sln
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2015.vcxproj
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2017.sln
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2017.vcxproj
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2019.sln
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2019.vcxproj
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2022.sln
	@rm -rf $*$(shell echo $* | cut -d '/' -f4)_vs2022.vcxproj
	@rm -rf $*.vscode
