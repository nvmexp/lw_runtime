LWPU LWCA Sample "radixSortThrust"
----------------------------------
...
This sample demonstrates a very fast and efficient parallel radix sort implemented in C for LWCA.  The included RadixSort class can sort either key-value pairs (with float or unsigned integer keys) or keys only. It can also sort unsigned integer keys based on a varying number of least-significant bits ranging from 4 to 32 in multiples of 4.

This radix sort code and the underlying algorithm is dislwssed in detail in the paper "Designing Efficient Sorting Algorithms for Manycore GPUs".  A PDF version of this paper is available at http://mgarland.org/files/papers/gpusort-ipdps09.pdf

-----
USAGE
-----

To run a sort with default options (Sort 1M unsigned integer key-value pairs), just ilwoke the exelwtable ("radixSort.exe" on Windows, "radixSort" otherwise).
...
The RadixSort class can also be used within your application by building the radixsort.lw file into your application or library, and including the radixsort.h header file.
