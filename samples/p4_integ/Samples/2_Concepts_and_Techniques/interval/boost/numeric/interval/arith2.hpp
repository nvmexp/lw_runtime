 ...
 * functions: fmod, sqrt, square, pov, ilwerse and
...
template<class T, class Policies> inline
interval<T, Policies> multiplicative_ilwerse(const interval<T, Policies>& x)
{
  typedef interval<T, Policies> I;
...
interval<T, Policies> pow(const interval<T, Policies>& x, int pwr)
{
  ...
  if (pwr == 0)
    ...
    return interval_lib::multiplicative_ilwerse(pow(x, -pwr));
