...
// LWCA-OpenGLES interoperability to update the particle field directly
...
GLuint m_texture = 0;
struct lwdaGraphicsResource *lwda_vbo_resource; // handles OpenGLES-LWCA exchange
...
static void InitGraphicsState(int argc, char** argv)
{
    ...
    checkLwdaErrors(lwdaGraphicsGLRegisterBuffer(&lwda_vbo_resource, vbo, lwdaGraphicsMapFlagsNone));
...
void autoTest(char **argv)
{
    ...
    for (int count=0; count<g_iFrameToCompare; count++)
    {
        ...
        if (ref_file)
        {
            ...
            lastx = x;
            lasty = y;
            getLastLwdaError("addForces kernel failed");
        }
    }
...
void cleanup(void)
{
    ...
    free(hvfield);
    free(particles);
    checkLwdaErrors(lwdaFree(dvfield));
    checkLwdaErrors(lwdaFree(vxfield));
    checkLwdaErrors(lwdaFree(vyfield));
    checkLwdaErrors(lwfftDestroy(planr2c));
    checkLwdaErrors(lwfftDestroy(planc2r));
...
int main(int argc, char **argv)
{
    ...
    printf("NOTE: The LWCA Samples are not meant for performance measurements. Results may vary when GPU Boost is enabled.\n\n");

#if defined (__aarch64__) || defined(__arm__)
    // find iGPU on the system which is compute capable which will perform GLES-LWCA interop
    devID = findIntegratedGPU();
#else
    // use command-line specified LWCA device, otherwise use device with highest Gflops/s
    ...
    checkLwdaErrors(lwdaMallocPitch((void **)&dvfield, &tPitch, sizeof(cData)*DIM, DIM));

    checkLwdaErrors(lwdaMemcpy(dvfield, hvfield, sizeof(cData) * DS, lwdaMemcpyHostToDevice));
    // Temporary complex velocity field data
    checkLwdaErrors(lwdaMalloc((void **)&vxfield, sizeof(cData) * PDS));
    checkLwdaErrors(lwdaMalloc((void **)&vyfield, sizeof(cData) * PDS));
