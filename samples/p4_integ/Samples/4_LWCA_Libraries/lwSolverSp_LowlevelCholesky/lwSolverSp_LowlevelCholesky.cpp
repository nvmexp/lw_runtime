...
int main (int argc, char *argv[])
{
    ...
    lwsolverSpHandle_t lwsolverSpH = NULL; // reordering, permutation and 1st LU factorization
    ...
    checkLwdaErrors(lwdaMalloc((void **)&d_csrRowPtrA, sizeof(int)*(rowsA+1)));
    ...
    checkLwdaErrors(lwsolverSpCreateCsrcholInfoHost(&h_info));

    printf("step 3: analyze chol(A) to know structure of L\n");
    checkLwdaErrors(lwsolverSpXcsrcholAnalysisHost(
        lwsolverSpH, rowsA, nnzA,
        descrA, h_csrRowPtrA, h_csrColIndA,
        h_info));

    printf("step 4: workspace for chol(A)\n");
    checkLwdaErrors(lwsolverSpDcsrcholBufferInfoHost(
        lwsolverSpH, rowsA, nnzA,
    ...
    checkLwdaErrors(lwsolverSpDcsrcholFactorHost(
        lwsolverSpH, rowsA, nnzA,
        descrA, h_csrValA, h_csrRowPtrA, h_csrColIndA,
        h_info,
        buffer_cpu));
    ...
    checkLwdaErrors(lwsolverSpDcsrcholZeroPivotHost(
        lwsolverSpH, h_info, tol, &singularity));
    ...
    checkLwdaErrors(lwsolverSpDcsrcholSolveHost(
        lwsolverSpH, rowsA, h_b, h_x, h_info, buffer_cpu));

    printf("step 8: evaluate residual r = b - A*x (result on CPU)\n");
    // use GPU gemv to compute r = b - A*x
    checkLwdaErrors(lwdaMemcpy(d_csrRowPtrA, h_csrRowPtrA, sizeof(int)*(rowsA+1), lwdaMemcpyHostToDevice));
    checkLwdaErrors(lwdaMemcpy(d_csrColIndA, h_csrColIndA, sizeof(int)*nnzA     , lwdaMemcpyHostToDevice));
    checkLwdaErrors(lwdaMemcpy(d_csrValA   , h_csrValA   , sizeof(double)*nnzA  , lwdaMemcpyHostToDevice));

    checkLwdaErrors(lwdaMemcpy(d_r, h_b, sizeof(double)*rowsA, lwdaMemcpyHostToDevice));
    checkLwdaErrors(lwdaMemcpy(d_x, h_x, sizeof(double)*colsA, lwdaMemcpyHostToDevice));

    /* Wrap raw data into lwSPARSE generic API objects */
    lwsparseSpMatDescr_t matA = NULL;
    if (baseA)
    {
        checkLwdaErrors(lwsparseCreateCsr(
        &matA, rowsA, colsA, nnzA, d_csrRowPtrA, d_csrColIndA, d_csrValA, LWSPARSE_INDEX_32I,
        LWSPARSE_INDEX_32I, LWSPARSE_INDEX_BASE_ONE, LWDA_R_64F));
    }
    else
    {
        checkLwdaErrors(lwsparseCreateCsr(
        &matA, rowsA, colsA, nnzA, d_csrRowPtrA, d_csrColIndA, d_csrValA, LWSPARSE_INDEX_32I,
        LWSPARSE_INDEX_32I, LWSPARSE_INDEX_BASE_ZERO, LWDA_R_64F));
    }
    ...
    checkLwdaErrors(lwsparseSpMV_bufferSize(
        lwsparseH, LWSPARSE_OPERATION_NON_TRANSPOSE, &minus_one, matA, vecx,
    ...
    checkLwdaErrors(lwsparseSpMV(
        lwsparseH, LWSPARSE_OPERATION_NON_TRANSPOSE, &minus_one, matA, vecx,
        &one, vecAx, LWDA_R_64F, LWSPARSE_SPMV_ALG_DEFAULT, buffer));

    checkLwdaErrors(lwdaMemcpy(h_r, d_r, sizeof(double)*rowsA, lwdaMemcpyDeviceToHost));
    ...
    checkLwdaErrors(lwsolverSpCreateCsrcholInfo(&d_info));

    checkLwdaErrors(lwdaMemcpy(d_csrRowPtrA, h_csrRowPtrA, sizeof(int)*(rowsA+1), lwdaMemcpyHostToDevice));
    checkLwdaErrors(lwdaMemcpy(d_csrColIndA, h_csrColIndA, sizeof(int)*nnzA     , lwdaMemcpyHostToDevice));
    checkLwdaErrors(lwdaMemcpy(d_csrValA   , h_csrValA   , sizeof(double)*nnzA  , lwdaMemcpyHostToDevice));
    checkLwdaErrors(lwdaMemcpy(d_b, h_b, sizeof(double)*rowsA, lwdaMemcpyHostToDevice));

    printf("step 10: analyze chol(A) to know structure of L\n");
    checkLwdaErrors(lwsolverSpXcsrcholAnalysis(
        lwsolverSpH, rowsA, nnzA,
        descrA, d_csrRowPtrA, d_csrColIndA,
        d_info));

    printf("step 11: workspace for chol(A)\n");
    checkLwdaErrors(lwsolverSpDcsrcholBufferInfo(
        lwsolverSpH, rowsA, nnzA,
    ...
    checkLwdaErrors(lwsolverSpDcsrcholFactor(
        lwsolverSpH, rowsA, nnzA,
        descrA, d_csrValA, d_csrRowPtrA, d_csrColIndA,
        d_info,
        buffer_gpu));
    ...
    checkLwdaErrors(lwsolverSpDcsrcholZeroPivot(
        lwsolverSpH, d_info, tol, &singularity));
    ...
    checkLwdaErrors(lwsolverSpDcsrcholSolve(
        lwsolverSpH, rowsA, d_b, d_x, d_info, buffer_gpu));
    ...
    if (lwsolverSpH) { checkLwdaErrors(lwsolverSpDestroy(lwsolverSpH)); }
    if (lwsparseH  ) { checkLwdaErrors(lwsparseDestroy(lwsparseH)); }
    if (stream     ) { checkLwdaErrors(lwdaStreamDestroy(stream)); }
    if (descrA     ) { checkLwdaErrors(lwsparseDestroyMatDescr(descrA)); }
    if (h_info     ) { checkLwdaErrors(lwsolverSpDestroyCsrcholInfoHost(h_info)); }
    if (d_info     ) { checkLwdaErrors(lwsolverSpDestroyCsrcholInfo(d_info)); }
    if (matA       ) { checkLwdaErrors(lwsparseDestroySpMat(matA)); }
    if (vecx       ) { checkLwdaErrors(lwsparseDestroyDlwec(vecx)); }
    if (vecAx      ) { checkLwdaErrors(lwsparseDestroyDlwec(vecAx)); }
    ...
    if (buffer_gpu) { checkLwdaErrors(lwdaFree(buffer_gpu)); }

    if (d_csrValA   ) { checkLwdaErrors(lwdaFree(d_csrValA)); }
    if (d_csrRowPtrA) { checkLwdaErrors(lwdaFree(d_csrRowPtrA)); }
    if (d_csrColIndA) { checkLwdaErrors(lwdaFree(d_csrColIndA)); }
    if (d_x) { checkLwdaErrors(lwdaFree(d_x)); }
    if (d_b) { checkLwdaErrors(lwdaFree(d_b)); }
    if (d_r) { checkLwdaErrors(lwdaFree(d_r)); }
