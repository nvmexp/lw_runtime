...
template <typename T_ELEM> __inline__ T_ELEM lwGet (int );
template <> __inline__ float lwGet<float >(int x)
{
    return float(x);
}

template <> __inline__ double lwGet<double>(int x)
{
    return double(x);
}

template <> __inline__ lwComplex lwGet<lwComplex>(int x)
{
    return (make_lwComplex( float(x), 0.0f ));
}

template <> __inline__ lwDoubleComplex  lwGet<lwDoubleComplex>(int x)
{
    return (make_lwDoubleComplex( double(x), 0.0 ));
}


template <typename T_ELEM> __inline__ T_ELEM lwGet (int , int );
template <> __inline__ float lwGet<float >(int x, int y)
{
    return float(x);
}

template <> __inline__ double lwGet<double>(int x, int y)
{
    return double(x);
}

template <> __inline__ lwComplex lwGet<lwComplex>(int x, int y)
{
    return make_lwComplex( float(x), float(y) );
}

template <> __inline__ lwDoubleComplex  lwGet<lwDoubleComplex>(int x, int y)
{
    return (make_lwDoubleComplex( double(x), double(y) ));
}


template <typename T_ELEM> __inline__ T_ELEM lwGet (float );
template <> __inline__ float lwGet<float >(float x)
{
    return float(x);
}

template <> __inline__ double lwGet<double>(float x)
{
    return double(x);
}

template <> __inline__ lwComplex lwGet<lwComplex>(float x)
{
    return (make_lwComplex( float(x), 0.0f ));
}

template <> __inline__ lwDoubleComplex  lwGet<lwDoubleComplex>(float x)
{
    return (make_lwDoubleComplex( double(x), 0.0 ));
}


template <typename T_ELEM> __inline__  T_ELEM lwGet (float, float );
template <> __inline__  float lwGet<float >(float x, float y)
{
    return float(x);
}

template <> __inline__  double lwGet<double>(float x, float y)
{
    return double(x);
}

template <> __inline__  lwComplex lwGet<lwComplex>(float x, float y)
{
    return (make_lwComplex( float(x), float(y) ));
}

template <> __inline__  lwDoubleComplex  lwGet<lwDoubleComplex>(float x, float y)
{
    return (make_lwDoubleComplex( double(x), double(y) ));
}


template <typename T_ELEM> __inline__ T_ELEM lwGet (double );
template <> __inline__ float lwGet<float >(double x)
{
    return float(x);
}

template <> __inline__ double lwGet<double>(double x)
{
    return double(x);
}

template <> __inline__ lwComplex lwGet<lwComplex>(double x)
{
    return (make_lwComplex( float(x), 0.0f ));
}

template <> __inline__ lwDoubleComplex  lwGet<lwDoubleComplex>(double x)
{
    return (make_lwDoubleComplex( double(x), 0.0 ));
}


template <typename T_ELEM> __inline__  T_ELEM lwGet (double, double );
template <> __inline__  float lwGet<float >(double x, double y)
{
    return float(x);
}

template <> __inline__  double lwGet<double>(double x, double y)
{
    return double(x);
}

template <> __inline__  lwComplex lwGet<lwComplex>(double x, double y)
{
    return (make_lwComplex( float(x), float(y) ));
}

template <> __inline__  lwDoubleComplex  lwGet<lwDoubleComplex>(double x, double y)
{
    return (make_lwDoubleComplex( double(x), double(y) ));
}
...
// use qsort to sort COO format 
    ...
    /* transfrom the matrix values of type double into one of the lwsparse library types */ 
    *aVal = (T_ELEM *)malloc((*nnz) * sizeof(T_ELEM));
    ...
    for (i=0; i<(*nnz); i++) {        
        ...
        if (mm_is_real(matcode) || mm_is_integer(matcode)){
            (*aVal)[i] = lwGet<T_ELEM>( tempVal[ work[i].p ] );
        }
        else{
            (*aVal)[i] = lwGet<T_ELEM>(tempVal[2*work[i].p], tempVal[2*work[i].p+1]);
        }
    }
...
template int loadMMSparseMatrix<lwComplex>(
    char *filename, 
    char elem_type, 
    bool csrFormat, 
    int *m, 
...
template int loadMMSparseMatrix<lwDoubleComplex>(
    char *filename, 
    char elem_type, 
    bool csrFormat, 
    int *m, 
