...
typedef enum D3DKMT_MULTIPLANE_OVERLAY_YCbCr_FLAGS
{
    ...
    D3DKMT_MULTIPLANE_OVERLAY_YCbCr_FLAG_xvYCC         = 0x4, // xvYCC vs. colwentional YCbCr
...
#if (DXGKDDI_INTERFACE_VERSION >= DXGKDDI_INTERFACE_VERSION_WIN8)
    UINT    NtSelwritySharing           :  1;    // 0x00000040
...
typedef struct _D3DKMT_CREATEALLOCATION
{
                                            ...
                                            D3DKMT_HANDLE                   hGlobalShare;   //out:Shared handle if CreateShared and not NtSelwritySharing
...
typedef struct _D3DKMT_LWRRENTDISPLAYMODE
{
    D3DDDI_VIDEO_PRESENT_SOURCE_ID VidPnSourceId;
    D3DKMT_DISPLAYMODE DisplayMode;
} D3DKMT_LWRRENTDISPLAYMODE;
...
typedef enum _KMTQUERYADAPTERINFOTYPE
{
     ...
     KMTQAITYPE_LWRRENTDISPLAYMODE      =  9,
...
typedef struct _D3DKMT_GETSHAREDPRIMARYHANDLE
{
    D3DKMT_HANDLE                   hAdapter;       // in: adapter handle
    D3DDDI_VIDEO_PRESENT_SOURCE_ID  VidPnSourceId;  // in: adapter's VidPN source ID
    D3DKMT_HANDLE                   hSharedPrimary; // out: global shared primary handle (if one exists lwrrently)
...
typedef enum _D3DKMT_TDRDBGCTRLTYPE
{
    ...
    D3DKMT_TDRDBGCTRLTYPE_UNCONDITIONAL     = 3, //Disables all safety conditions (e.g. check for conselwtive recoveries)
...
typedef struct _D3DKMT_OUTPUTDUPL_SNAPSHOT
{
    ...
    UINT SessionProcessCount;           // _Out_: Number of processes lwrrently duplicating output in this session (max possible will be equal to NumOutputDuplContexts)
...
typedef struct _D3DKMT_FLIPOVERLAY
{
    D3DKMT_HANDLE        hDevice;               // in: Indentifies the device
    D3DKMT_HANDLE        hOverlay;              // in: Kernel overlay handle
    D3DKMT_HANDLE        hSource;               // in: Allocation lwrrently displayed
    VOID*                pPrivateDriverData;    // in: Private driver data
    UINT                 PrivateDriverDataSize; // in: Size of private driver data
} D3DKMT_FLIPOVERLAY;
...
typedef enum _D3DKMT_DEVICEEXELWTION_STATE
{
    D3DKMT_DEVICEEXELWTION_ACTIVE               = 1,
    D3DKMT_DEVICEEXELWTION_RESET                = 2,
    D3DKMT_DEVICEEXELWTION_HUNG                 = 3,
    D3DKMT_DEVICEEXELWTION_STOPPED              = 4,
    D3DKMT_DEVICEEXELWTION_ERROR_OUTOFMEMORY    = 5,
    D3DKMT_DEVICEEXELWTION_ERROR_DMAFAULT       = 6,
} D3DKMT_DEVICEEXELWTION_STATE;
...
{
    D3DKMT_DEVICESTATE_EXELWTION = 1,
...
typedef struct _D3DKMT_GETDEVICESTATE
{
    ...
    union
    {
        D3DKMT_DEVICEEXELWTION_STATE ExelwtionState; // out: device state
...
typedef struct _D3DKMT_GETSCANLINE
{
    D3DKMT_HANDLE                   hAdapter;           // in: Adapter handle
    D3DDDI_VIDEO_PRESENT_SOURCE_ID  VidPnSourceId;      // in: Adapter's VidPN Source ID
    BOOLEAN                         IlwerticalBlank;    // out: Within vertical blank
...
typedef struct _D3DKMT_ILWALIDATEACTIVEVIDPN
{
    D3DKMT_HANDLE                   hAdapter;               // in: Adapter handle
    VOID*                           pPrivateDriverData;     // in: Private driver data
    UINT                            PrivateDriverDataSize;  // in: Size of private driver data
} D3DKMT_ILWALIDATEACTIVEVIDPN;
...
typedef struct _D3DKMT_CREATEKEYEDMUTEX2_FLAGS
{
    union
    {
        struct
        {
            UINT NtSelwritySharing      : 1;    // If set, the keyed mutex will be shared using DxgkShareObjects instead of D3DKMT_CREATEKEYEDMUTEX2::hSharedHandle
            UINT Reserved               : 31;
        };
        UINT Value;
    };
...
{
    UINT64                                        InitialValue;           // in:  Initial value to release to
    D3DKMT_HANDLE                                 hSharedHandle;          // out: Global handle to keyed mutex, NULL if NtSelwritySharing is set.
    D3DKMT_HANDLE                                 hKeyedMutex;            // out: Handle to the keyed mutex in this process
...
typedef enum _D3DKMT_OFFER_PRIORITY
{
    ...
    D3DKMT_OFFER_PRIORITY_HIGH,     // Content is useful and diffilwlt to regenerate
...
typedef _Check_return_ NTSTATUS (APIENTRY *PFND3DKMT_ILWALIDATEACTIVEVIDPN)(_In_ CONST D3DKMT_ILWALIDATEACTIVEVIDPN*);
...
EXTERN_C _Check_return_ NTSTATUS APIENTRY D3DKMTIlwalidateActiveVidPn(_In_ CONST D3DKMT_ILWALIDATEACTIVEVIDPN*);
