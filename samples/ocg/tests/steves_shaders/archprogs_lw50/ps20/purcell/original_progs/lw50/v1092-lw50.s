...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v1092-lw40.s -o allprogs-new32//v1092-lw50.s
#vendor LWPU
...
R2A      A3, R11;
R2A      A1, R8;
FMUL32   R8, R10, c[A3 + 92];
FMUL32   R11, R10, c[A3 + 93];
FMUL32   R10, R10, c[A3 + 94];
FMAD     R8, R9, c[A2 + 84], R8;
FMAD     R11, R9, c[A2 + 85], R11;
FMAD     R9, R9, c[A2 + 86], R10;
FMAD     R8, R7, c[A1 + 100], R8;
FMAD     R10, R7, c[A1 + 101], R11;
FMAD     R7, R7, c[A1 + 102], R9;
...
MOV32    o[7], R0;
END
# 126 instructions, 16 R-regs
# 126 inst, (40 mov, 0 mvi, 0 tex, 0 complex, 86 math)
#    73 64-bit, 53 32-bit, 0 32-bit-const
