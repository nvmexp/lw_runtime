...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v848-lw40.s -o allprogs-new32//v848-lw50.s
#vendor LWPU
...
FMAX     R3, R3, c[0];
FSET     R7, R2, c[13], LT;
FMUL32   R8, R6, c[A2 + 93];
R2A      A0, R7;
FMAD     R8, R5, c[A1 + 85], R8;
FMUL32   R7, R6, c[A2 + 92];
FMAD     R8, R4, c[A0 + 101], R8;
FMAD     R7, R5, c[A1 + 84], R7;
FMAD     R8, R3, c[109], R8;
FMAD     R7, R4, c[A0 + 100], R7;
LG2      R8, |R8|;
FMAD     R7, R3, c[108], R7;
FMUL32   R8, R8, c[4];
LG2      R7, |R7|;
RRO      R8, R8, 1;
FMUL32   R6, R6, c[A2 + 94];
FMUL32   R7, R7, c[4];
EX2      R8, R8;
FMAD     R5, R5, c[A1 + 86], R6;
RRO      R6, R7, 1;
FMUL32   R8, R8, c[7];
FMAD     R4, R4, c[A0 + 102], R5;
EX2      R5, R6;
FMAD     R3, R3, c[110], R4;
FMUL32   R7, R5, c[7];
LG2      R3, |R3|;
...
FMUL     R3, v[12], c[173];
FMUL32   R9, R4, c[7];
FMAD     R3, v[11], c[172], R3;
FMAX     R6, R9, c[12];
FMAD     R4, v[13], c[174], R3;
FMUL     R3, v[12], c[177];
FMAX     R6, R5, R6;
FMUL32   R5, R4, R2;
FMAD     R3, v[11], c[176], R3;
RCP      R10, R6;
FMUL     R6, v[12], c[169];
FMAD     R3, v[13], c[178], R3;
FMUL32   o[4], R7, R10;
FMUL32   o[5], R8, R10;
FMUL32   o[6], R9, R10;
FMAD     R5, R1, R3, -R5;
FMAD     R7, v[11], c[168], R6;
FMUL32   R6, R3, R0;
FMUL     o[23], v[14], R5;
FMAD     R5, v[13], c[170], R7;
FMUL     R7, v[1], c[169];
FMAD     R6, R2, R5, -R6;
FMAD     R8, v[0], c[168], R7;
FMUL32   R7, R5, R1;
FMUL     o[24], v[14], R6;
FMAD     R6, v[2], c[170], R8;
FMAD     R7, R0, R4, -R7;
FMUL     R8, v[1], c[173];
FMAD     R6, v[3], c[171], R6;
FMUL     o[25], v[14], R7;
FMAD     R7, v[0], c[172], R8;
FMUL     R8, v[1], c[177];
...
MOV32    o[7], R1;
END
# 158 instructions, 16 R-regs
# 158 inst, (31 mov, 0 mvi, 0 tex, 7 complex, 120 math)
#    104 64-bit, 54 32-bit, 0 32-bit-const
