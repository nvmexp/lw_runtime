...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v856-lw40.s -o allprogs-new32//v856-lw50.s
#vendor LWPU
...
R2A      A2, R5;
I2I.M4   R10, R4;
FMUL32   R4, R9, c[A2 + 85];
R2A      A0, R10;
MVI      R10, -127.996;
FMAD     R4, R2, c[A1 + 93], R4;
FMAD     R4, R0, c[A0 + 101], R4;
FMAX     R10, R10, c[4];
FMAX     R5, R5, c[0];
FMAX     R4, R4, c[0];
FMIN     R10, R10, c[0];
FSET     R5, R5, c[0], GT;
LG2      R11, R4;
FMUL32   R4, R9, c[A2 + 84];
FMUL32   R11, R10, R11;
FMAD     R2, R2, c[A1 + 92], R4;
RRO      R4, R11, 1;
FMAD     R0, R0, c[A0 + 100], R2;
EX2      R4, R4;
FMAX     R2, R0, c[0];
FMAX     R0, R0, c[0];
FCMP     R4, -R5, R4, c[0];
FSET     R2, R2, c[0], GT;
LG2      R0, R0;
FMUL32   R5, R9, c[A0 + 86];
FMUL32   R4, R4, c[7];
FMUL32   R0, R10, R0;
FMAD     R3, R3, c[A3 + 94], R5;
RRO      R0, R0, 1;
FMAD     R1, R1, c[A2 + 102], R3;
EX2      R0, R0;
FMAX     R3, R1, c[0];
FMAX     R1, R1, c[0];
FCMP     R0, -R2, R0, c[0];
...
MOV32    o[7], R0;
END
# 142 instructions, 12 R-regs
# 142 inst, (19 mov, 1 mvi, 0 tex, 8 complex, 114 math)
#    95 64-bit, 47 32-bit, 0 32-bit-const
