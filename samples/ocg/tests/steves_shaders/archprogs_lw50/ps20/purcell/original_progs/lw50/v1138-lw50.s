...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v1138-lw40.s -o allprogs-new32//v1138-lw50.s
#vendor LWPU
...
FMUL     R1, v[5], c[177];
FMUL     R0, v[5], c[173];
FMAD     R2, v[12], c[172], R2;
FMAD     R1, v[4], c[176], R1;
FMAD     R0, v[4], c[172], R0;
FMAD     R4, v[14], c[174], R2;
FMAD     R2, v[6], c[178], R1;
FMAD     R1, v[6], c[174], R0;
FMUL     R3, v[13], c[177];
FMUL32   R6, R4, R2;
FMUL     R0, v[5], c[169];
FMAD     R3, v[12], c[176], R3;
FMUL     R5, v[13], c[169];
FMAD     R0, v[4], c[168], R0;
FMAD     R3, v[14], c[178], R3;
FMAD     R5, v[12], c[168], R5;
FMAD     R0, v[6], c[170], R0;
FMAD     R7, R1, R3, -R6;
FMAD     R5, v[14], c[170], R5;
FMUL32   R6, R3, R0;
FMUL     o[24], v[15], R7;
FMUL32   R8, R5, R1;
FMAD     R6, R2, R5, -R6;
...
FMUL     R10, v[7], c[5];
R2A      A0, R9;
FMUL32   R9, R8, c[A2 + 92];
LG2      R11, |R10|;
MOV32    R10, c[4];
FMAD     R9, R7, c[A1 + 84], R9;
FMUL32   R12, R8, c[A2 + 93];
RCP      R10, R10;
FMAD     R9, R6, c[A0 + 100], R9;
FMAD     R12, R7, c[A1 + 85], R12;
FMUL32   R11, R11, R10;
FMUL     R13, v[8], c[5];
FMAD     R12, R6, c[A0 + 101], R12;
RRO      R11, R11, 1;
FMUL32   R8, R8, c[A2 + 94];
LG2      R13, |R13|;
EX2      R11, R11;
FMAD     R7, R7, c[A1 + 86], R8;
FMUL32   R8, R13, R10;
FADD32   o[4], R9, R11;
FMAD     R7, R6, c[A0 + 102], R7;
RRO      R8, R8, 1;
FMUL     R9, v[9], c[5];
FMUL     R6, v[1], c[169];
EX2      R8, R8;
...
FMAD     R8, v[0], c[172], R8;
EX2      R10, R9;
FMUL     R9, v[1], c[177];
FMAD     R8, v[2], c[174], R8;
FADD32   o[6], R7, R10;
FMAD     R10, v[0], c[176], R9;
FMAD     R7, v[3], c[175], R8;
MOV32    R9, c[43];
FMAD     R8, v[2], c[178], R10;
FMUL32   R10, R7, c[41];
...
MOV32    o[7], R0;
END
# 137 instructions, 16 R-regs
# 137 inst, (40 mov, 0 mvi, 0 tex, 7 complex, 90 math)
#    78 64-bit, 59 32-bit, 0 32-bit-const
