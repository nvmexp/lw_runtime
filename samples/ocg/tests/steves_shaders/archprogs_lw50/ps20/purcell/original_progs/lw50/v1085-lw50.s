...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v1085-lw40.s -o allprogs-new32//v1085-lw50.s
#vendor LWPU
...
FMUL32   R8, R0, R0;
F2I.FLOOR R2, R5;
FMAD     R5, R15, c[A1 + 18], R16;
FMAD     R15, R8, c[A2 + 100], R17;
I2I.M4   R16, R2;
RCP      R2, R5;
R2A      A0, R16;
FMUL32   R5, c[A1], R2;
...
FMAD     R5, R5, R10, R15;
FMAD     R4, -c[A0 + 4], R4, R13;
FMAX     R1, R1, c[4];
FMAD     R0, -c[A0 + 6], R0, R4;
FMIN     R16, R1, c[0];
FMAX     R13, R0, c[0];
FMAD     R0, c[A0], R13, R5;
FMAX     R5, R0, c[0];
...
MOV32    o[7], R0;
END
# 198 instructions, 20 R-regs
# 198 inst, (22 mov, 1 mvi, 0 tex, 12 complex, 163 math)
#    130 64-bit, 68 32-bit, 0 32-bit-const
