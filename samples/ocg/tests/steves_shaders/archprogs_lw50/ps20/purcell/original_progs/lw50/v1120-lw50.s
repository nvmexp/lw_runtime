...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v1120-lw40.s -o allprogs-new32//v1120-lw50.s
#vendor LWPU
...
FMUL32   R6, R1, -c[133];
R2A      A1, R13;
FMUL32   R13, R12, c[A3 + 92];
FMAD     R6, R0, -c[132], R6;
FMUL32   R14, R7, c[109];
FMAD     R13, R11, c[A2 + 84], R13;
FMAD     R6, R2, -c[134], R6;
FMUL32   R15, R12, c[A3 + 93];
FMAD     R13, R10, c[A1 + 100], R13;
FMAX     R6, R6, c[0];
FMUL32   R12, R12, c[A3 + 94];
FMAD     R8, R8, R9, R13;
FMAD     R13, R11, c[A2 + 85], R15;
FMAD     R11, R11, c[A2 + 86], R12;
FMAD     o[4], R6, c[128], R8;
FMAD     R8, R10, c[A1 + 101], R13;
FMAD     R10, R10, c[A1 + 102], R11;
FMUL32   R7, R7, c[110];
FMAD     R11, R14, R9, R8;
FMUL     R8, v[11], c[A0 + 173];
FMAD     R7, R7, R9, R10;
FMAD     o[5], R6, c[129], R11;
FMAD     R8, v[10], c[A0 + 172], R8;
FMAD     o[6], R6, c[130], R7;
FMUL     R6, v[11], c[A0 + 177];
FMAD     R7, v[12], c[A0 + 174], R8;
...
# 158 inst, (44 mov, 0 mvi, 0 tex, 2 complex, 112 math)
#    88 64-bit, 70 32-bit, 0 32-bit-const
