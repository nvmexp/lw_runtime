...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v1131-lw40.s -o allprogs-new32//v1131-lw50.s
#vendor LWPU
...
FMAD     R12, v[3], c[A0 + 171], R2;
FMAD     R13, v[3], c[A0 + 175], R1;
FMAD     R0, v[0], c[A0 + 176], R0;
FADD32   R5, -R12, c[116];
FADD32   R4, -R13, c[117];
FMAD     R2, v[2], c[A0 + 178], R0;
MOV32    R1, c[125];
FMUL32   R0, R4, R4;
FMAD     R11, v[3], c[A0 + 179], R2;
FMAD     R0, R5, R5, R0;
FADD32   R3, -R11, c[118];
FMAD     R0, R3, R3, R0;
RSQ      R2, |R0|;
...
FMUL32   R3, -R17, c[113];
FMAD     R1, R1, R2, c[124];
FMAD     R2, -R16, c[112], R3;
FMAD     R0, R0, c[126], R1;
FMAD     R1, -R15, c[114], R2;
RCP      R14, R0;
FADD32   R0, R1, -c[122];
FMUL32   R0, R0, c[123];
FMAX     R1, R0, c[0];
...
FMAD     R3, v[10], c[A0 + 172], R2;
RRO      R2, R1, 1;
FMUL     R1, v[6], c[A0 + 173];
FMAD     R6, v[12], c[A0 + 174], R3;
EX2      R2, R2;
...
FMAD     R4, v[12], c[A0 + 178], R3;
FMUL     R3, v[11], c[A0 + 169];
FMAD     R1, v[7], c[A0 + 170], R1;
FMAD     R7, R2, R4, -R5;
FMAD     R5, v[10], c[A0 + 168], R3;
FMUL32   R3, R1, R4;
FMUL     R8, v[13], R7;
...
MOV32    o[7], R1;
END
# 247 instructions, 28 R-regs
# 247 inst, (41 mov, 0 mvi, 0 tex, 4 complex, 202 math)
#    155 64-bit, 92 32-bit, 0 32-bit-const
