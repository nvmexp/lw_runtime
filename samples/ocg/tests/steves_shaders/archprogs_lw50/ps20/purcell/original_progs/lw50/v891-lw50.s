...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v891-lw40.s -o allprogs-new32//v891-lw50.s
#vendor LWPU
...
R2A      A3, R10;
R2A      A1, R7;
FMUL32   R10, R9, c[A3 + 93];
FMUL32   R7, R9, c[A3 + 92];
FMAD     R10, R8, c[A2 + 85], R10;
FMAD     R7, R8, c[A2 + 84], R7;
FMAD     R10, R6, c[A1 + 101], R10;
FMAD     R7, R6, c[A1 + 100], R7;
LG2      R10, |R10|;
LG2      R7, |R7|;
FMUL32   R10, R10, c[4];
FMUL32   R7, R7, c[4];
FMUL32   R9, R9, c[A3 + 94];
RRO      R10, R10, 1;
RRO      R7, R7, 1;
FMAD     R8, R8, c[A2 + 86], R9;
EX2      R9, R10;
EX2      R7, R7;
FMAD     R6, R6, c[A1 + 102], R8;
FMUL32   R10, R9, c[7];
FMUL32   R9, R7, c[7];
LG2      R6, |R6|;
FMAX     R8, R10, R9;
...
MOV32    o[7], R0;
END
# 153 instructions, 16 R-regs
# 153 inst, (32 mov, 0 mvi, 0 tex, 7 complex, 114 math)
#    100 64-bit, 53 32-bit, 0 32-bit-const
