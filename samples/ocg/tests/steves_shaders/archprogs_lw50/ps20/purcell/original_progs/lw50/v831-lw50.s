...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v831-lw40.s -o allprogs-new32//v831-lw50.s
#vendor LWPU
...
FMUL32   R6, R1, -c[133];
R2A      A0, R8;
FMUL32   R8, R12, c[A2 + 93];
FMAD     R6, R0, -c[132], R6;
FMAD     R8, R11, c[A1 + 85], R8;
FMAD     R6, R2, -c[134], R6;
FMAD     R14, R10, c[A0 + 101], R8;
FMAX     R6, R6, c[0];
FMUL32   R8, R7, c[108];
FMAD     R14, R13, R9, R14;
FMUL32   R13, R12, c[A2 + 92];
FMAD     R14, R6, c[129], R14;
FMAD     R13, R11, c[A1 + 84], R13;
LG2      R14, |R14|;
FMAD     R13, R10, c[A0 + 100], R13;
FMUL32   R14, R14, c[4];
FMAD     R8, R8, R9, R13;
RRO      R13, R14, 1;
FMAD     R8, R6, c[128], R8;
EX2      R13, R13;
LG2      R8, |R8|;
FMUL32   R12, R12, c[A2 + 94];
FMUL32   R13, R13, c[7];
FMUL32   R8, R8, c[4];
FMAD     R11, R11, c[A1 + 86], R12;
FMUL32   R7, R7, c[110];
RRO      R8, R8, 1;
...
# 192 inst, (37 mov, 0 mvi, 0 tex, 9 complex, 146 math)
#    119 64-bit, 73 32-bit, 0 32-bit-const
