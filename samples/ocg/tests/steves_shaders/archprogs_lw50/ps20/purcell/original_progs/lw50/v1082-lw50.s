...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v1082-lw40.s -o allprogs-new32//v1082-lw50.s
#vendor LWPU
...
FADD32   R8, -R3, c[136];
R2A      A1, R6;
FMUL32   R6, R18, c[A3 + 92];
FADD32   R9, -R4, c[137];
FADD32   R7, -R5, c[138];
FMAD     R11, R17, c[A2 + 84], R6;
FMUL32   R6, R9, R9;
FMAD     R11, R16, c[A1 + 100], R11;
FMAD     R6, R8, R8, R6;
...
FMAD     R12, R17, c[A2 + 85], R6;
FMUL32   R6, R7, c[129];
FMAD     R11, R17, c[A2 + 86], R11;
FMAD     R12, R16, c[A1 + 101], R12;
FMUL32   R7, R7, c[130];
...
FMAD     o[5], R6, R8, R12;
FMAD     R6, v[10], c[A0 + 172], R9;
FMAD     o[6], R7, R8, R10;
FMUL     R9, v[11], c[A0 + 177];
FMAD     R7, v[12], c[A0 + 174], R6;
FMUL     R8, v[11], c[A0 + 169];
FMAD     R6, v[10], c[A0 + 176], R9;
FMUL32   R9, R7, R2;
...
MOV32    o[7], R0;
END
# 197 instructions, 20 R-regs
# 197 inst, (45 mov, 0 mvi, 0 tex, 6 complex, 146 math)
#    104 64-bit, 93 32-bit, 0 32-bit-const
