...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v818-lw40.s -o allprogs-new32//v818-lw50.s
#vendor LWPU
...
FSET     R17, R2, c[14], LT;
FMAD     R8, R0, R8, R9;
FMUL32   R9, R16, c[A3 + 93];
R2A      A1, R17;
FMAD     R7, R2, R7, R8;
FMAD     R9, R15, c[A2 + 85], R9;
FMAX     R8, R7, c[0];
FMAD     R9, R14, c[A1 + 101], R9;
FMUL32   R7, R6, c[128];
FMAD     R9, R19, R13, R9;
FMUL32   R7, R7, R10;
FMUL32   R17, R11, c[108];
FMAD     R18, R18, R8, R9;
FMUL32   R9, R16, c[A3 + 92];
FMUL32   R17, R17, R12;
LG2      R18, |R18|;
FMAD     R9, R15, c[A2 + 84], R9;
FMUL32   R18, R18, c[4];
FMAD     R9, R14, c[A1 + 100], R9;
RRO      R18, R18, 1;
FMAD     R9, R17, R13, R9;
EX2      R17, R18;
FMAD     R7, R7, R8, R9;
...
LG2      R7, |R7|;
FMUL32   R6, R6, R10;
FMUL32   R10, R16, c[A3 + 94];
FMUL32   R7, R7, c[4];
FMUL32   R11, R11, c[110];
FMAD     R10, R15, c[A2 + 86], R10;
RRO      R7, R7, 1;
FMUL32   R11, R11, R12;
FMAD     R10, R14, c[A1 + 102], R10;
EX2      R7, R7;
...
FMAX     R11, R12, c[13];
FMAD     R8, v[14], c[A0 + 174], R6;
FMUL     R6, v[13], c[A0 + 177];
FMAX     R11, R10, R11;
FMUL32   R10, R8, R2;
FMAD     R6, v[12], c[A0 + 176], R6;
RCP      R13, R11;
FMUL     R11, v[13], c[A0 + 169];
FMAD     R6, v[14], c[A0 + 178], R6;
FMUL32   o[4], R7, R13;
FMUL32   o[5], R9, R13;
FMUL32   o[6], R12, R13;
FMAD     R7, R1, R6, -R10;
FMAD     R10, v[12], c[A0 + 168], R11;
FMUL32   R9, R6, R0;
FMUL     o[23], v[15], R7;
FMAD     R7, v[14], c[A0 + 170], R10;
FMUL32   R11, R4, c[41];
MOV32    R10, c[43];
FMAD     R9, R2, R7, -R9;
...
MOV32    o[7], R0;
END
# 247 instructions, 20 R-regs
# 247 inst, (37 mov, 0 mvi, 0 tex, 15 complex, 195 math)
#    144 64-bit, 103 32-bit, 0 32-bit-const
