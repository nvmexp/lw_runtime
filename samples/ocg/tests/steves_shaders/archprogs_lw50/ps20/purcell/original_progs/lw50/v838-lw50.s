...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v838-lw40.s -o allprogs-new32//v838-lw50.s
#vendor LWPU
...
R2A      A3, R12;
R2A      A1, R7;
FMUL32   R7, R11, c[A3 + 93];
FMAD     R14, R10, c[A2 + 85], R7;
FMUL32   R7, R6, c[108];
FMUL32   R12, R11, c[A3 + 92];
FMAD     R14, R9, c[A1 + 101], R14;
FMAD     R12, R10, c[A2 + 84], R12;
FMAD     R13, R13, R8, R14;
FMAD     R12, R9, c[A1 + 100], R12;
LG2      R13, |R13|;
FMAD     R7, R7, R8, R12;
FMUL32   R12, R13, c[4];
LG2      R7, |R7|;
RRO      R12, R12, 1;
FMUL32   R13, R6, c[110];
FMUL32   R6, R11, c[A3 + 94];
EX2      R11, R12;
FMUL32   R7, R7, c[4];
FMAD     R10, R10, c[A2 + 86], R6;
FMUL32   R6, R11, c[7];
RRO      R7, R7, 1;
FMAD     R9, R9, c[A1 + 102], R10;
EX2      R7, R7;
FMAD     R8, R13, R8, R9;
FMUL32   R11, R7, c[7];
LG2      R7, |R8|;
...
RRO      R7, R7, 1;
EX2      R8, R7;
FMUL     R7, v[13], c[A0 + 173];
FMUL32   R12, R8, c[7];
FMAD     R7, v[12], c[A0 + 172], R7;
FMAX     R10, R12, c[13];
FMAD     R8, v[14], c[A0 + 174], R7;
FMUL     R7, v[13], c[A0 + 177];
FMAX     R10, R9, R10;
FMUL32   R9, R8, R2;
FMAD     R7, v[12], c[A0 + 176], R7;
RCP      R13, R10;
FMUL     R10, v[13], c[A0 + 169];
FMAD     R7, v[14], c[A0 + 178], R7;
FMUL32   o[4], R11, R13;
FMUL32   o[5], R6, R13;
FMUL32   o[6], R12, R13;
FMAD     R6, R1, R7, -R9;
...
MOV32    o[7], R0;
END
# 187 instructions, 16 R-regs
# 187 inst, (38 mov, 0 mvi, 0 tex, 9 complex, 140 math)
#    115 64-bit, 72 32-bit, 0 32-bit-const
