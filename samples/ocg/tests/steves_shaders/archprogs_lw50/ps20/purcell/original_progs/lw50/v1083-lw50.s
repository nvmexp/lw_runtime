...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v1083-lw40.s -o allprogs-new32//v1083-lw50.s
#vendor LWPU
...
FMUL32   R3, R3, c[120];
FMAD     R7, v[7], c[A0 + 178], R5;
FMAD     R5, v[10], c[A0 + 172], R4;
RRO      R4, R3, 1;
FMUL     R3, v[6], c[A0 + 173];
FMAD     R14, v[12], c[A0 + 174], R5;
EX2      R5, R4;
FMAD     R3, v[5], c[A0 + 172], R3;
FMUL32   R4, R7, R14;
FMIN     R26, R5, c[1];
FMAD     R9, v[7], c[A0 + 174], R3;
...
FMAD     R12, v[12], c[A0 + 178], R3;
FMUL     R3, v[11], c[A0 + 169];
FMAD     R8, v[7], c[A0 + 170], R5;
FMAD     R5, R9, R12, -R4;
FMAD     R4, v[10], c[A0 + 168], R3;
FMUL32   R3, R8, R12;
FMUL     R18, v[13], R5;
FMAD     R13, v[12], c[A0 + 170], R4;
FMAD     R5, R7, R13, -R3;
FMUL32   R4, R9, R13;
FMUL32   R3, R14, c[7];
FMUL     R19, v[13], R5;
...
MOV32    o[7], R1;
END
# 265 instructions, 36 R-regs
# 265 inst, (41 mov, 0 mvi, 0 tex, 6 complex, 218 math)
#    162 64-bit, 103 32-bit, 0 32-bit-const
