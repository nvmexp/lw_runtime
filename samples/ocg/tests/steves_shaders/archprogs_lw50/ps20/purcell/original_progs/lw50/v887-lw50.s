...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v887-lw40.s -o allprogs-new32//v887-lw50.s
#vendor LWPU
...
FMUL32   R6, R1, -c[133];
R2A      A0, R8;
FMUL32   R8, R13, c[A2 + 93];
FMAD     R6, R0, -c[132], R6;
FMAD     R8, R12, c[A1 + 85], R8;
FMAD     R6, R2, -c[134], R6;
FMAD     R15, R11, c[A0 + 101], R8;
FMAX     R6, R6, c[0];
FMUL32   R8, R7, c[108];
...
FMUL32   R14, R14, c[7];
FMUL32   R7, R7, R9;
FMAD     R9, R12, c[A1 + 86], R13;
FMUL32   R8, R8, c[4];
FMAD     R9, R11, c[A0 + 102], R9;
RRO      R8, R8, 1;
FMAD     R7, R7, R10, R9;
EX2      R8, R8;
FMAD     R6, R6, c[130], R7;
...
MOV32    o[7], R1;
END
# 209 instructions, 16 R-regs
# 209 inst, (37 mov, 0 mvi, 0 tex, 11 complex, 161 math)
#    126 64-bit, 83 32-bit, 0 32-bit-const
