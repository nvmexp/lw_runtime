...
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v1094-lw40.s -o allprogs-new32//v1094-lw50.s
#vendor LWPU
...
FMAX     R13, R8, c[0];
FMAD     R7, R6, c[126], R9;
FMAD     R8, R6, -c[127], R12;
FMUL32   R10, R13, c[108];
RCP      R6, R7;
FMAX     R7, R8, c[0];
FMUL32   R16, R0, R0;
FSET     R8, R0, c[0], LT;
...
FADD32   R8, -R3, c[136];
R2A      A1, R6;
FMUL32   R6, R17, c[A3 + 92];
FADD32   R9, -R4, c[137];
FADD32   R7, -R5, c[138];
FMAD     R11, R16, c[A2 + 84], R6;
FMUL32   R6, R9, R9;
FMAD     R11, R15, c[A1 + 100], R11;
FMAD     R6, R8, R8, R6;
FMAD     R18, R10, R14, R11;
FMAD     R6, R7, R7, R6;
RSQ      R10, |R6|;
FMUL32   R8, R8, R10;
FMUL32   R9, R9, R10;
...
FMAX     R7, R7, c[0];
RCP      R9, R6;
FMUL32   R10, R13, c[109];
FMUL32   R6, R7, c[128];
FMUL32   R8, R9, R8;
FMUL32   R11, R13, c[110];
FMUL32   R9, R17, c[A3 + 93];
FMAD     o[4], R6, R8, R18;
FMUL32   R6, R17, c[A3 + 94];
FMAD     R12, R16, c[A2 + 85], R9;
FMUL32   R9, R7, c[129];
FMAD     R6, R16, c[A2 + 86], R6;
FMAD     R12, R15, c[A1 + 101], R12;
FMUL32   R7, R7, c[130];
FMAD     R6, R15, c[A1 + 102], R6;
FMAD     R12, R10, R14, R12;
FMUL     R10, v[11], c[A0 + 173];
FMAD     R6, R11, R14, R6;
FMAD     o[5], R9, R8, R12;
FMAD     R9, v[10], c[A0 + 172], R10;
FMAD     o[6], R7, R8, R6;
FMUL     R6, v[11], c[A0 + 177];
FMAD     R7, v[12], c[A0 + 174], R9;
...
MOV32    o[7], R0;
END
# 180 instructions, 20 R-regs
# 180 inst, (45 mov, 0 mvi, 0 tex, 4 complex, 131 math)
#    97 64-bit, 83 32-bit, 0 32-bit-const
