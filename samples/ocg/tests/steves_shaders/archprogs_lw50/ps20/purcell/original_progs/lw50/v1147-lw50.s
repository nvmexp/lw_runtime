!!SPA1.0
.CONST_MODE  PAGE
.THREAD_TYPE VERTEX
.MAX_REG     19
.MAX_IBUF    13
.MAX_OBUF    36
# parseasm build date Mar 10 2004 15:40:49
# -profile vp50 -po tbat3 -po lat3 -if vs2x -i allprogs-new32//v1147-lw40.s -o allprogs-new32//v1147-lw50.s
#vendor LWPU
#version parseasm.0.0
#profile vp50
#program fp30entry
#semantic C[91].C[91]
#semantic C[90].C[90]
#semantic C[32].C[32]
#semantic C[35].C[35]
#semantic C[33].C[33]
#semantic C[36].C[36]
#semantic C[30].C[30]
#semantic C[27].C[27]
#semantic C[28].C[28]
#semantic C[31].C[31]
#semantic C[34].C[34]
#semantic C[29].C[29]
#semantic C[16].C[16]
#semantic C[11].C[11]
#semantic C[10].C[10]
#semantic C[2].C[2]
#semantic C[9].C[9]
#semantic C[8].C[8]
#semantic C[0].C[0]
#semantic c.c
#semantic C[1].C[1]
#var float4 o[TEX7] : $vout.O : O[0] : -1 : 0
#var float4 o[TEX6] : $vout.O : O[0] : -1 : 0
#var float4 o[TEX5] : $vout.O : O[0] : -1 : 0
#var float4 o[TEX4] : $vout.O : O[0] : -1 : 0
#var float4 o[TEX3] : $vout.O : O[0] : -1 : 0
#var float4 o[TEX2] : $vout.O : O[0] : -1 : 0
#var float4 o[TEX1] : $vout.O : O[0] : -1 : 0
#var float4 o[TEX0] : $vout.O : O[0] : -1 : 0
#var float4 o[FOGC] : $vout.O : O[0] : -1 : 0
#var float4 o[BCOL1] : $vout.O : O[0] : -1 : 0
#var float4 o[BCOL0] : $vout.O : O[0] : -1 : 0
#var float4 o[HPOS] : $vout.O : O[0] : -1 : 0
#var float4 C[91] :  : c[91] : -1 : 0
#var float4 C[90] :  : c[90] : -1 : 0
#var float4 v[COL0] : $vin.F : F[0] : -1 : 0
#var float4 C[32] :  : c[32] : -1 : 0
#var float4 C[35] :  : c[35] : -1 : 0
#var float4 C[33] :  : c[33] : -1 : 0
#var float4 C[36] :  : c[36] : -1 : 0
#var float4 C[30] :  : c[30] : -1 : 0
#var float4 C[27] :  : c[27] : -1 : 0
#var float4 C[28] :  : c[28] : -1 : 0
#var float4 C[31] :  : c[31] : -1 : 0
#var float4 C[34] :  : c[34] : -1 : 0
#var float4 C[29] :  : c[29] : -1 : 0
#var float4 C[16] :  : c[16] : -1 : 0
#var float4 C[11] :  : c[11] : -1 : 0
#var float4 C[10] :  : c[10] : -1 : 0
#var float4 C[2] :  : c[2] : -1 : 0
#var float4 C[9] :  : c[9] : -1 : 0
#var float4 C[8] :  : c[8] : -1 : 0
#var float4 C[0] :  : c[0] : -1 : 0
#var float4 v[OPOS] : $vin.F : F[0] : -1 : 0
#var float4 v[NRML] : $vin.F : F[0] : -1 : 0
#var float4 c :  : c[0] : -1 : 0
#var float4 v[FOGC] : $vin.F : F[0] : -1 : 0
#var float4 C[1] :  : c[1] : -1 : 0
#var float4 v[WGHT] : $vin.F : F[0] : -1 : 0
#ibuf 0 = v[OPOS].x
#ibuf 1 = v[OPOS].y
#ibuf 2 = v[OPOS].z
#ibuf 3 = v[OPOS].w
#ibuf 4 = v[WGT].z
#ibuf 5 = v[NOR].x
#ibuf 6 = v[NOR].y
#ibuf 7 = v[NOR].z
#ibuf 8 = v[COL0].x
#ibuf 9 = v[COL0].y
#ibuf 10 = v[FOG].x
#ibuf 11 = v[FOG].y
#ibuf 12 = v[FOG].z
#ibuf 13 = v[FOG].w
#obuf 0 = o[HPOS].x
#obuf 1 = o[HPOS].y
#obuf 2 = o[HPOS].z
#obuf 3 = o[HPOS].w
#obuf 4 = o[BCOL0].x
#obuf 5 = o[BCOL0].y
#obuf 6 = o[BCOL0].z
#obuf 7 = o[BCOL0].w
#obuf 8 = o[BCOL1].x
#obuf 9 = o[BCOL1].y
#obuf 10 = o[BCOL1].z
#obuf 11 = o[BCOL1].w
#obuf 12 = o[TEX0].x
#obuf 13 = o[TEX0].y
#obuf 14 = o[TEX1].x
#obuf 15 = o[TEX1].y
#obuf 16 = o[TEX2].x
#obuf 17 = o[TEX2].y
#obuf 18 = o[TEX3].x
#obuf 19 = o[TEX3].y
#obuf 20 = o[TEX3].z
#obuf 21 = o[TEX4].x
#obuf 22 = o[TEX4].y
#obuf 23 = o[TEX4].z
#obuf 24 = o[TEX5].x
#obuf 25 = o[TEX5].y
#obuf 26 = o[TEX5].z
#obuf 27 = o[TEX6].x
#obuf 28 = o[TEX6].y
#obuf 29 = o[TEX6].z
#obuf 30 = o[TEX7].x
#obuf 31 = o[TEX7].y
#obuf 32 = o[TEX7].z
#obuf 33 = o[FOGC].x
#obuf 34 = o[FOGC].y
#obuf 35 = o[FOGC].z
#obuf 36 = o[FOGC].w
BB0:
FMUL     R0, v[4], c[4];
R2A      A0, R0;
FMUL     R1, v[1], c[A0 + 169];
FMUL     R0, v[1], c[A0 + 173];
FMAD     R1, v[0], c[A0 + 168], R1;
FMAD     R0, v[0], c[A0 + 172], R0;
FMAD     R2, v[2], c[A0 + 170], R1;
FMAD     R1, v[2], c[A0 + 174], R0;
FMUL     R0, v[1], c[A0 + 177];
FMAD     R3, v[3], c[A0 + 171], R2;
FMAD     R4, v[3], c[A0 + 175], R1;
FMAD     R0, v[0], c[A0 + 176], R0;
FADD32   R8, -R3, c[136];
FADD32   R9, -R4, c[137];
FMAD     R1, v[2], c[A0 + 178], R0;
FMUL32   R0, R9, R9;
FMAD     R5, v[3], c[A0 + 179], R1;
MOV32    R1, c[1];
FMAD     R0, R8, R8, R0;
FADD32   R6, -R5, c[138];
FMUL32   R1, R1, c[1];
FMAD     R0, R6, R6, R0;
MOV32    R2, c[1];
RSQ      R10, |R0|;
FMUL32   R7, R0, c[1];
FMUL32   R11, R0, R10;
FMAD     R0, R0, -c[147], R2;
FMUL32   R11, R11, c[145];
FMAX     R0, R0, c[0];
FMUL32   R8, R8, R10;
FMAD     R11, R1, c[144], R11;
FMIN     R0, R0, c[1];
FMUL32   R9, R9, R10;
FMAD     R11, R7, c[146], R11;
FMUL32   R7, R6, R10;
FMUL32   R6, -R9, c[133];
RCP      R10, R11;
FMAD     R11, -R8, c[132], R6;
FMUL32   R6, R10, R0;
FMAD     R0, -R7, c[134], R11;
FMUL32   R11, R6, c[128];
FADD32   R0, R0, -c[142];
FMUL32   R0, R0, c[143];
FADD32   R16, -R3, c[116];
FADD32   R13, -R4, c[117];
FMAX     R0, R0, c[0];
FMUL32   R10, R13, R13;
LG2      R0, |R0|;
FADD32   R12, -R5, c[118];
FMAD     R10, R16, R16, R10;
FMUL32   R0, R0, c[140];
FMAD     R15, R12, R12, R10;
RRO      R0, R0, 1;
RSQ      R14, |R15|;
EX2      R10, R0;
FMUL32   R0, R15, c[1];
FMAD     R2, R15, -c[127], R2;
FMIN     R10, R10, c[1];
FMUL32   R15, R15, R14;
FMAX     R2, R2, c[0];
FMUL32   R11, R11, R10;
FMUL32   R15, R15, c[125];
FMIN     R2, R2, c[1];
FMUL32   R16, R16, R14;
FMAD     R1, R1, c[124], R15;
FMUL32   R13, R13, R14;
FMUL32   R15, R12, R14;
FMAD     R1, R0, c[126], R1;
FMUL32   R0, -R13, c[113];
RCP      R1, R1;
FMAD     R0, -R16, c[112], R0;
FMUL32   R12, R1, R2;
FMAD     R0, -R15, c[114], R0;
FMUL32   R14, R12, c[108];
FADD32   R0, R0, -c[122];
FMUL32   R1, R0, c[123];
FMUL     R0, v[6], c[A0 + 173];
FMAX     R1, R1, c[0];
FMAD     R0, v[5], c[A0 + 172], R0;
LG2      R2, |R1|;
FMAD     R1, v[7], c[A0 + 174], R0;
FMUL     R0, v[6], c[A0 + 169];
FMUL32   R2, R2, c[120];
FMUL32   R17, R1, R13;
FMAD     R0, v[5], c[A0 + 168], R0;
RRO      R13, R2, 1;
FMUL     R2, v[6], c[A0 + 177];
FMAD     R0, v[7], c[A0 + 170], R0;
EX2      R13, R13;
FMAD     R2, v[5], c[A0 + 176], R2;
FMAD     R16, R0, R16, R17;
FMIN     R13, R13, c[1];
FMAD     R2, v[7], c[A0 + 178], R2;
FMUL32   R17, R0, R0;
FMUL32   R14, R14, R13;
FMAD     R15, R2, R15, R16;
FSET     R16, R0, c[0], LT;
FMUL32   R18, R1, R1;
FMAX     R15, R15, c[0];
R2A      A2, R16;
FSET     R19, R1, c[0], LT;
FMUL32   R16, R2, R2;
FMUL32   R9, R1, R9;
R2A      A3, R19;
FSET     R19, R2, c[0], LT;
FMAD     R8, R0, R8, R9;
FMUL32   R9, R18, c[A3 + 92];
R2A      A1, R19;
FMAD     R7, R2, R7, R8;
FMAD     R8, R17, c[A2 + 84], R9;
FMUL32   R9, R6, c[129];
FMAX     R7, R7, c[0];
FMAD     R8, R16, c[A1 + 100], R8;
FMUL32   R9, R9, R10;
FMUL32   R19, R12, c[109];
FMAD     R8, R14, R15, R8;
FMUL32   R6, R6, c[130];
FMUL32   R14, R19, R13;
FMAD     o[4], R11, R7, R8;
FMUL32   R6, R6, R10;
FMUL32   R10, R12, c[110];
FMUL32   R8, R18, c[A3 + 93];
FMUL32   R11, R18, c[A3 + 94];
FMUL32   R10, R10, R13;
FMAD     R8, R17, c[A2 + 85], R8;
FMAD     R12, R17, c[A2 + 86], R11;
FMUL     R11, v[11], c[A0 + 173];
FMAD     R8, R16, c[A1 + 101], R8;
FMAD     R12, R16, c[A1 + 102], R12;
FMAD     R11, v[10], c[A0 + 172], R11;
FMAD     R8, R14, R15, R8;
FMAD     R12, R10, R15, R12;
FMAD     R10, v[12], c[A0 + 174], R11;
FMAD     o[5], R9, R7, R8;
FMAD     o[6], R6, R7, R12;
FMUL32   R8, R10, R2;
FMUL     R6, v[11], c[A0 + 177];
FMUL     R7, v[11], c[A0 + 169];
FMAD     R6, v[10], c[A0 + 176], R6;
FMAD     R7, v[10], c[A0 + 168], R7;
FMAD     R6, v[12], c[A0 + 178], R6;
FMAD     R7, v[12], c[A0 + 170], R7;
FMAD     R8, R1, R6, -R8;
FMUL32   R9, R6, R0;
FMUL32   R11, R7, R1;
FMUL     o[24], v[13], R8;
FMAD     R8, R2, R7, -R9;
FMAD     R9, R0, R10, -R11;
FMUL32   R11, R4, c[41];
FMUL     o[25], v[13], R8;
FMUL     o[26], v[13], R9;
FMAD     R11, R3, c[40], R11;
MOV32    R8, c[43];
FADD32   R9, -R5, c[11];
FMAD     R12, R5, c[42], R11;
FMAX     R9, R9, c[0];
FADD32   R11, -R5, c[10];
FMAD     R8, R8, c[1], R12;
MOV32    R12, c[67];
RCP      R11, R11;
FMUL32   R13, R4, c[33];
FMUL32   R9, R9, R11;
FMAD     R13, R3, c[32], R13;
MOV32    R11, c[35];
FMUL32   R9, R8, R9;
FMAD     R13, R5, c[34], R13;
FMAD     R9, -R9, R12, c[65];
FMUL32   R12, R4, c[37];
FMAD     o[0], R11, c[1], R13;
MOV32    o[33], R9;
FMAD     R11, R3, c[36], R12;
MOV32    o[2], R8;
MOV32    o[34], R9;
FMAD     R8, R5, c[38], R11;
MOV32    o[35], R9;
MOV32    o[36], R9;
MOV32    R9, c[39];
FMUL32   R12, R4, c[45];
MOV32    R11, c[47];
FMAD     R12, R3, c[44], R12;
FMAD     o[1], R9, c[1], R8;
FMAD     R8, R5, c[46], R12;
MOV32    o[27], R0;
MOV32    o[28], R1;
FMAD     o[3], R11, c[1], R8;
MOV32    o[29], R2;
MOV32    o[21], R7;
MOV32    o[22], R10;
MOV32    o[23], R6;
FADD32   o[18], -R3, c[8];
FADD32   o[19], -R4, c[9];
FADD32   o[20], -R5, c[10];
MOV32    o[30], c[0];
MOV32    R0, c[0];
MOV32    R1, c[0];
FMUL     R2, v[8], c[360];
MOV32    o[31], R0;
MOV32    o[32], R1;
FMAD     R0, v[9], c[361], R2;
FMUL     R1, v[8], c[364];
MOV32    o[8], c[0];
MOV32    o[16], R0;
FMAD     R1, v[9], c[365], R1;
MOV32    o[14], R0;
MOV32    o[12], R0;
MOV32    o[17], R1;
MOV32    o[15], R1;
MOV32    o[13], R1;
MOV32    R0, c[0];
MOV32    R1, c[0];
MOV32    R2, c[0];
MOV32    o[9], R0;
MOV32    o[10], R1;
MOV32    o[11], R2;
MOV32    R0, c[0];
MOV32    o[7], R0;
END
# 216 instructions, 20 R-regs
# 216 inst, (42 mov, 0 mvi, 0 tex, 9 complex, 165 math)
#    113 64-bit, 103 32-bit, 0 32-bit-const
