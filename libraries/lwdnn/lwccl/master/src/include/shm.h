...
static ncclResult_t shmOpen(const char* shmname, const int shmsize, void** shmPtr, void** devShmPtr, int create) {
  ...
  LWDACHECKGOTO(lwdaHostRegister(ptr, shmsize, lwdaHostRegisterMapped), res, lwdaError);
  LWDACHECKGOTO(lwdaHostGetDevicePointer(devShmPtr, ptr, 0), res, lwdaError);
...
static ncclResult_t shmClose(void* shmPtr, void* devShmPtr, const int shmsize) {
  LWDACHECK(lwdaHostUnregister(shmPtr));
