...
extern ncclResult_t getLwmlDevice(int lwdaDev, int *lwmlDev);
...
static inline void initDebug() {
  const char* nccl_debug = getelw("NCCL_DEBUG");
  if (nccl_debug == NULL) {
  ...
  /* Parse the NCCL_DEBUG_SUBSYS elw var
   * This can be a comma separated list such as INIT,COLL
   * or ^INIT,COLL etc
   */
  char* nccl_debug_subsys = getelw("NCCL_DEBUG_SUBSYS");
  if (nccl_debug_subsys != NULL) {
    char *subsys = strtok(nccl_debug_subsys, ",");
    while (subsys != NULL) {
      int ilwert = 0;
      uint64_t mask = 0;
      if (subsys[0] == '^') { ilwert = 1; subsys++; }
      ...
      if (mask) {
        if (ilwert) ncclDebugMask &= ~mask; else ncclDebugMask |= mask;
      }
      subsys = strtok(NULL, ",");
    }
  }
   ...
   */
  const char* nccl_debug_file = getelw("NCCL_DEBUG_FILE");
