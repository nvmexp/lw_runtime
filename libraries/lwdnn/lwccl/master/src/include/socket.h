...
static inline int elwSocketFamily(void) {
  int family = -1; // Family selection is not forced, will use first one found
  char* elw = getelw("NCCL_SOCKET_FAMILY");
  if (elw == NULL)
    return family;

  if (strcmp(elw, "AF_INET") == 0)
    family = AF_INET;  // IPv4
  else if (strcmp(elw, "AF_INET6") == 0)
...
static int findInterfaces(char* ifNames, union socketAddress *ifAddrs, int ifNameMaxSize, int maxIfs) {
  ...
  int sock_family = elwSocketFamily();
  // User specified interface
  char* elw = getelw("NCCL_SOCKET_IFNAME");
  if (elw && strlen(elw) > 1) {
    // Specified by user : find or fail
    nIfs = findInterfaces(elw, ifNames, ifAddrs, sock_family, ifNameMaxSize, maxIfs);
  } else {
    ...
    if (nIfs == 0) {
      char* commId = getelw("NCCL_COMM_ID");
...
static ncclResult_t createListenSocket(int *fd, union socketAddress *localAddr) {
  ...
  if (socketToPort(&localAddr->sa)) {
    // Port is forced by elw. Make sure we get the port.
    int opt = 1;
