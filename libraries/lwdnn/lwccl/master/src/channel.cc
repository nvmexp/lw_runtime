...
ncclResult_t initChannel(struct ncclComm* comm, int channelid) {
  ...
  NCCLCHECK(ncclLwdaCalloc(&channel->ring.devUserRanks, comm->nRanks));
  NCCLCHECK(ncclCalloc(&channel->ring.userRanks, comm->nRanks));

  // Communication structures with peers.
  NCCLCHECK(ncclLwdaCalloc(&channel->devPeers, comm->nRanks));
  ...
  NCCLCHECK(ncclLwdaHostAlloc((void**)&channel->collectives, (void**)&channel->devCollectives, sizeof(struct ncclColl)*NCCL_MAX_OPS));
  return ncclSuccess;
}

ncclResult_t freeChannel(struct ncclChannel* channel, int nRanks) {
  // Operation list
  NCCLCHECK(ncclLwdaHostFree(channel->collectives));

  // Free Ring index to rank tables
  free(channel->ring.userRanks);
  LWDACHECK(lwdaFree(channel->ring.devUserRanks));
  ...
  LWDACHECK(lwdaFree(channel->devPeers));
  free(channel->peers);
