...
ncclResult_t ncclSetDevice(int lwdaDev) {
  LWDACHECK(lwdaSetDevice(lwdaDev));
  return ncclSuccess;
}
...
void* ncclAsyncThreadMain(void* args_) {
  ...
  CHECK(ncclSetDevice(args->init.lwdaDev));
...
ncclResult_t ncclGroupEnd() {
   ...
   * 1. Barrier Check In. Only the last call may call lwdaLaunchKernel[cooperative]
   * 2. Barrier Wait. No LWCA call is permitted
   * 3. Enqueue Events. LWCA event wait/enqueue.
   * This is needed because step 2 cannot call any LWCA primitive, otherwise if
   * lwdaFree happens between 1 and 3, it could block that LWCA call and
   * prevent some ranks from launching their network threads, which would
   * prevent the LWCL call from completing, blocking the lwdaFree call.
   */
  for (int i=0; i<ncclGroupIndex; i++) {
    struct ncclAsyncArgs* args = ncclGroupArgs+i;
    if (args->funcType == ASYNC_FUNC_COLL) {
      if (args->coll.comm->userStream == NULL)
        LWDACHECKGOTO(lwdaSetDevice(args->coll.comm->lwdaDev), ret, end);
...
group_cleanup:
  ...
  ncclGroupIndex = 0;
  LWDACHECK(lwdaSetDevice(savedDev)); // do other clean-ups first before calling lwdaSetDevice, because this call can fail too
  return ret;
}
