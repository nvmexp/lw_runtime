...
// Colwert a logical lwdaDev index to the LWML device minor number
ncclResult_t getLwmlDevice(int lwdaDev, int *lwmlDev) {
  ...
  *lwmlDev = -1;
  ...
  NCCLCHECK(wrapLwmlDeviceGetHandleByPciBusId(busId, &lwmlDevice));
  NCCLCHECK(wrapLwmlDeviceGetMinorNumber(lwmlDevice, &dev));
...
void ncclDebugLog(ncclDebugLogLevel level, unsigned long flags, const char *filefunc, int line, const char *fmt, ...) {
  ...
  if (level == NCCL_LOG_WARN && ncclDebugLevel >= NCCL_LOG_WARN)
    len = snprintf(buffer, sizeof(buffer),
                   "\n%s:%d:%d [%d] %s:%d LWCL WARN ", hostname, getpid(), gettid(), lwdaDev, filefunc, line);
  else if (level == NCCL_LOG_INFO && ncclDebugLevel >= NCCL_LOG_INFO && (flags & ncclDebugMask))
    len = snprintf(buffer, sizeof(buffer),
                   "%s:%d:%d [%d] LWCL INFO ", hostname, getpid(), gettid(), lwdaDev);
#ifdef ENABLE_TRACE
    ...
    len = snprintf(buffer, sizeof(buffer),
                   "%s:%d:%d [%d] %f %s:%d LWCL TRACE ", hostname, getpid(), gettid(), lwdaDev, timestamp, filefunc, line);
  }
#endif
  if (len) {
    va_list vargs;
  ...
  if (level == NCCL_LOG_WARN && ncclDebugLevel == NCCL_LOG_ABORT) {
    fprintf(stderr,"\n%s:%d:%d [%d] %s:%d LWCL ABORT\n",
            hostname, getpid(), gettid(), lwdaDev, filefunc, line);
    abort();
  }
}
