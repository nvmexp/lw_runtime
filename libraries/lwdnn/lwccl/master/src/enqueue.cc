...
/*****************************************************************************/
/*       Launch system : synchronization and LWCA kernel launch              */
/*****************************************************************************/

ncclResult_t ncclLaunchCooperativeKernelMultiDevice(struct lwdaLaunchParams *paramsList, int* lwdaDevs, int numDevices, int cgMode) {
#if LWDART_VERSION >= 9000
  if (cgMode & 0x01) {
    LWDACHECK(lwdaLaunchCooperativeKernelMultiDevice(paramsList, numDevices,
            // These flags are to reduce the latency of using this API
            lwdaCooperativeLaunchMultiDeviceNoPreSync|lwdaCooperativeLaunchMultiDeviceNoPostSync));
    return ncclSuccess;
  }
#endif
  int savedDev;
  LWDACHECK(lwdaGetDevice(&savedDev));
  for (int i = 0; i < numDevices; i++) {
    struct lwdaLaunchParams* params = paramsList+i;
    LWDACHECK(lwdaSetDevice(lwdaDevs[i]));
    LWDACHECK(lwdaLaunchKernel(params->func, params->gridDim, params->blockDim, params->args, params->sharedMem, params->stream));
  }
  LWDACHECK(lwdaSetDevice(savedDev));
  return ncclSuccess;
}

ncclResult_t setupLaunch(struct ncclComm* comm, struct lwdaLaunchParams* params) {
...
ncclResult_t ncclBarrierEnqueue(struct ncclComm* comm) {
  ...
  struct lwdaLaunchParams* params = comm->myParams;

  NCCLCHECK(setupLaunch(comm, params));

  // Use internal LWCL stream for CGMD/GROUP launch if required or if the user stream is NULL
  if (comm->launchMode == ncclComm::GROUP && (comm->groupLwdaStream || comm->userStream == NULL)) {
    // Enqueue event in user stream
    LWDACHECK(lwdaEventRecord(comm->doneEvent, comm->userStream));
    // Create dependency between user stream and internal LWCL stream
    LWDACHECK(lwdaStreamWaitEvent(comm->groupStream, comm->doneEvent, 0));
    params->stream = comm->groupStream;
  } else {
    if (comm->userStream != params->stream) {
      // Stream changed from last call, create dependency against last LWCL kernel launch
      LWDACHECK(lwdaStreamWaitEvent(comm->userStream, comm->doneEvent, 0));
    }
  ...
  if (isLast) {
    if (comm->launchMode == ncclComm::GROUP) {
      ...
      NCCLCHECK(ncclLaunchCooperativeKernelMultiDevice(comm->intraParams, comm->intraLwdaDevs, comm->intraRanks, *comm->intraCGMode));
    }
...
ncclResult_t ncclBarrierEnqueueWait(ncclComm_t comm) {
    ...
    *comm->intraCGMode ^= 0x10;
    INFO(NCCL_INIT,"Launch mode %s%s%s",
        ...
        *comm->intraCGMode ? "/CGMD" : "",
        (comm->launchMode == ncclComm::GROUP && comm->groupLwdaStream) ? "/Stream" : "");
  }
  ...
  // perform any LWCA call between the two or having a lwdaFree between the LWCA
  // launch and the transportStartProxy call could cause a deadlock.
  // Also, starting the proxies after the LWCA launch seems to be better for
  // performance (latency).
  for (int r=0; r<params->gridDim.x; r++) {
    struct ncclChannel* channel = comm->channels+r;
...
ncclResult_t ncclEnqueueEvents(ncclComm_t comm) {
  struct lwdaLaunchParams *params = comm->myParams;
  // Enqueue event after LWCL kernel
  LWDACHECK(lwdaEventRecord(comm->doneEvent, params->stream));
  // Use internal LWCL stream for CGMD/GROUP launch if required or if the user stream is NULL
  if (comm->launchMode == ncclComm::GROUP && (comm->groupLwdaStream || comm->userStream == NULL)) {
    // Create dependency between LWCL internal stream and user stream
    LWDACHECK(lwdaStreamWaitEvent(comm->userStream, comm->doneEvent, 0));
  }
...
static ncclResult_t saveKernel(struct ncclInfo* info) {
  if (info->comm->nRanks == 1) {
    if (info->sendbuff != info->recvbuff)
      LWDACHECK(lwdaMemcpyAsync(info->recvbuff, info->sendbuff, info->nBytes, lwdaMemcpyDeviceToDevice, info->stream));
    return ncclSuccess;
  }
...
ncclResult_t ncclEnqueueCheck(struct ncclInfo* info) {
  if (info->comm == NULL) return ncclIlwalidArgument;
  ...
  if (ncclAsyncMode()) {
    ...
    if (info->comm->checkPointers) {
      LWDACHECKGOTO(lwdaGetDevice(&savedDev), ret, end);
      LWDACHECKGOTO(lwdaSetDevice(info->comm->lwdaDev), ret, end);
    }
    // Check arguments
    ...
    if (savedDev != -1) LWDACHECK(lwdaSetDevice(savedDev));
