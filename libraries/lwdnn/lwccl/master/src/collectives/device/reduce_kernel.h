...
#if (__LWDA_ARCH__ >= 300) && (__LWDA_ARCH__ < 500)
    int32_t rv, z=0;
...
static __device__ uint32_t mulChar4(const uint32_t x, const uint32_t y) {
  ...
  union colwerter { uint32_t storage; char4 a; };
  colwerter cx, cy, cr;
  cx.storage = x;
  cy.storage = y;
...
struct FuncMax<uint8_t> {
  union colwerter { uint32_t storage; uchar4 a; };
  __device__ uint32_t operator()(const uint32_t x, const uint32_t y) const {
#if (__LWDA_ARCH__ >= 300) && (__LWDA_ARCH__ < 500)
    int32_t rv, z=0;
...
#if __LWDA_ARCH__ >= 530 && __LWDA_ARCH__ != 610
    return __hadd2(x, y);
#else
    float2 fx, fy, fr;
    fx = __half22float2(x);
