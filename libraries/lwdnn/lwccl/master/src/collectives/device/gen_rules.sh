...
for base in all_reduce all_gather broadcast reduce reduce_scatter; do
  ...
  for op in sum prod min max; do
    ...
    for dt in i8 u8 i32 u32 i64 u64 f16 f32 f64; do
      echo "${dir}/${base}_${op}_${dt}.o : ${base}.lw ${dir}/${base}.dep"
      echo "	@printf \"Compiling  %-35s > %s\\\\n\" ${base}.lw ${dir}/${base}_${op}_${dt}.o"
      echo "	mkdir -p ${dir}"
      echo "	\${LWCC} -DNCCL_OP=${opn} -DNCCL_TYPE=${dtn} \${LWLWFLAGS} -dc ${base}.lw -o ${dir}/${base}_${op}_${dt}.o"
      echo ""
