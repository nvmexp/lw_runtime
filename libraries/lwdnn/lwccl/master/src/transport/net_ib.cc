...
ncclResult_t ncclIbInit(ncclDebugLogger_t logFunction) {
  ...
  if (ncclNIbDevs == -1) {
    ...
    if (ncclNIbDevs == -1) {
      ...
      char* userIbElw = getelw("NCCL_IB_HCA");
      struct netIf userIfs[MAX_IB_DEVS];
      bool searchNot = userIbElw && userIbElw[0] == '^';
      int nUserIfs = parseStringList(userIbElw, userIfs, MAX_IB_DEVS);
...
// Detect whether GDR can work on a given NIC with the current LWCA device
// Returns :
...
ncclResult_t ncclIbGdrSupport(int ibDev) {
  ...
  if (moduleLoaded == -1) {
    moduleLoaded = (access("/sys/kernel/mm/memory_peers/lw_mem/version", F_OK) == -1) ? 0 : 1;
  }
  if (moduleLoaded == 0) return ncclSystemError;
  ncclResult_t ret = ncclSystemError;
  void* ptr;
  if (lwdaMalloc(&ptr, sizeof(int)) == lwdaSuccess) {
    struct ibv_mr* mr;
    struct ibv_pd* pd;
...
ncclResult_t ncclIbPtrSupport(int dev, int* supportedTypes) {
  *supportedTypes = NCCL_PTR_HOST;
  ...
  if (ncclIbGdrSupport(dev) != ncclSuccess) {
    INFO(NCCL_NET,"NET/IB : GPU Direct RDMA Disabled for GPU %d[%d] / HCA %d '%s' (no module or not supported by GPU)", lwdaDev, lwmlDev, dev, ncclIbDevs[dev].devName);
    return ncclSuccess;
  }
  *supportedTypes |= NCCL_PTR_LWDA;
  return ncclSuccess;
}
