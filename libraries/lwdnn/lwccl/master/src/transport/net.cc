...
static ncclResult_t netDistance(int lwdaDev, int dev, short* distance) {
  char* lwdaPath = NULL;
  char* nicPath = NULL;
  ncclResult_t err;
  NCCLCHECK(getLwdaPath(lwdaDev, &lwdaPath));
  err = ncclNetPciPath(dev, &nicPath);
  *distance = (err != ncclSuccess || nicPath == NULL || lwdaPath == NULL) ? PATH_SYS : pciDistance(nicPath, lwdaPath);
  if (nicPath) free(nicPath);
  if (lwdaPath) free(lwdaPath);
  return ncclSuccess;
}
...
static ncclResult_t netDevices(int* ndev, short** distances) {
  ...
  *distances = (short*)malloc(*ndev*sizeof(short));
  ...
  NCCLCHECK(getLwmlDevice(lwdaDev, &lwmlDev))
  char line[1024];
  sprintf(line, "LWCA Dev %d[%d], %s NIC distance : ", lwdaDev, lwmlDev, ncclNetName());
  for (int d=0; d<*ndev; d++) {
    NCCLCHECK(netDistance(lwdaDev, d, *distances+d));
...
ncclResult_t netCanConnect(ncclTvalue_t* ret, struct ncclPeerInfo* myInfo, struct ncclPeerInfo* peerInfo) {
  int lwdaDev;
  LWDACHECK(lwdaGetDevice(&lwdaDev));
  ret[0] = ncclNetTvalues[lwdaDev];
  if (ret[0] == NET_TVALUE_UNKNOWN) {
    if (lwdaDev >= NET_MAX_GPUS) {
      WARN("LWCA device %d >= MAX %d\n", lwdaDev, NET_MAX_GPUS);
      return ncclInternalError;
    }
    int nDev;
    short* distances;
    NCCLCHECK(netDevices(&nDev, &distances));
    ncclNetTvalues[lwdaDev] = ret[0] = getTvalue(distances, nDev);
    ncclNetNDev = nDev;
    free(distances);
  }
  return ncclSuccess;
...
int getDev(int lwdaDev, int ringId) {
  ncclTvalue_t tvalues = ncclNetTvalues[lwdaDev];
...
static ncclResult_t netGetGdrSupport(int dev, int read, int* useGdr) {
  *useGdr = 0;
  ...
  short distance;
  NCCLCHECK(netDistance(lwdaDev, dev, &distance));
  if (distance >= netGdrLevel) {
    INFO(NCCL_NET,"NET/%s : GPU Direct RDMA Disabled for GPU %d[%d] / HCA %d (distance %d >= %d)", ncclNetName(), lwdaDev, lwmlDev, dev, distance, netGdrLevel);
    return ncclSuccess;
  }
  ...
  if ((flags & NCCL_PTR_LWDA) == 0) return ncclSuccess;
  *useGdr = 1;
  INFO(NCCL_NET,"NET/%s : GPU Direct RDMA Enabled for GPU %d[%d] / HCA %d (distance %d < %d), read %d", ncclNetName(), lwdaDev, lwmlDev, dev, distance, netGdrLevel, read);
  return ncclSuccess;
}
...
ncclResult_t netSendSetup(struct ncclPeerInfo* myInfo, struct ncclPeerInfo* peerInfo, struct ncclConnect* connectInfo, struct ncclConnector* send, int buffSize, int channelId) {
  ...
  int lwdaDev;
  LWDACHECK(lwdaGetDevice(&lwdaDev));
  resources->netDev = getDev(lwdaDev, channelId);
  NCCLCHECK(netGetGdrSupport(resources->netDev, 1, &resources->useGdr));

  int sendSize = sizeof(struct ncclSendMem);
  NCCLCHECK(ncclLwdaHostAlloc((void**)&resources->hostSendMem, (void**)&resources->devHostSendMem, sendSize));

  int recvSize = offsetof(struct ncclRecvMem, buff)+buffSize;
  if (resources->useGdr) {
    NCCLCHECK(ncclLwdaCalloc((char**)(&resources->devRecvMem), recvSize));
  }
  NCCLCHECK(ncclLwdaHostAlloc((void**)&resources->hostRecvMem, (void**)&resources->devHostRecvMem, recvSize));
...
ncclResult_t netSendConnect(struct ncclConnect* connectInfo, struct ncclConnector* send) {
  ...
  NCCLCHECK(ncclNetRegMr(resources->netSendComm, recvMem->buff, resources->buffSize,
        resources->useGdr ? NCCL_PTR_LWDA : NCCL_PTR_HOST, &resources->mhandle));
...
ncclResult_t netRecvConnect(struct ncclConnect* connectInfo, struct ncclConnector* recv) {
  ...
  NCCLCHECK(ncclNetRegMr(resources->netRecvComm, recvMem->llBuff, NCCL_LL_BUFF_SIZE,
        resources->useGdr ? NCCL_PTR_LWDA : NCCL_PTR_HOST, &resources->llMhandle));
...
ncclResult_t netSendFree(void* transportResources) {
  ...
  NCCLCHECK(ncclLwdaHostFree(resources->hostSendMem));
  ...
  NCCLCHECK(ncclLwdaHostFree(resources->hostRecvMem));
  if (resources->useGdr)
    LWDACHECK(lwdaFree(resources->devRecvMem));
