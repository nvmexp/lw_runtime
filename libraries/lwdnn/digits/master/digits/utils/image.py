...
def load_image(path):
    ...
    if image.mode in ['L', 'RGB']:
        # No colwersion necessary
        return image
    elif image.mode in ['1']:
        # Easy colwersion to L
        return image.colwert('L')
    elif image.mode in ['LA']:
        # Deal with transparencies
        new = PIL.Image.new('L', image.size, 255)
        new.paste(image, mask=image.colwert('RGBA'))
        return new
    elif image.mode in ['CMYK', 'YCbCr']:
        # Easy colwersion to RGB
        return image.colwert('RGB')
...
def image_to_array(image,
    ...
    if isinstance(image, PIL.Image.Image):
        ...
        if image.mode != image_mode:
            image = image.colwert(image_mode)
...
def get_layer_vis_square(data,
    ...
    Keyword arguments:
    allow_heatmap -- if True, colwert single channel images to heatmaps
...
def vis_square(images,
    ...
    # colwert to float since we're going to do some math
    ...
    if images.ndim == 3:
        # they're grayscale - colwert to a colormap
