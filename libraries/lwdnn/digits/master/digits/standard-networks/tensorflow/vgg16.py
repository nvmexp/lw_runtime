...
class UserModel(Tower):
    ...
    def inference(self):
        ...
        with slim.arg_scope([slim.colw2d, slim.fully_connected],
                            weights_initializer=tf.contrib.layers.xavier_initializer(),
                            weights_regularizer=slim.l2_regularizer(0.0005)):
            model = slim.repeat(x, 2, slim.colw2d, 64, [3, 3], scope='colw1')
            model = slim.max_pool2d(model, [2, 2], scope='pool1')
            model = slim.repeat(model, 2, slim.colw2d, 128, [3, 3], scope='colw2')
            model = slim.max_pool2d(model, [2, 2], scope='pool2')
            model = slim.repeat(model, 3, slim.colw2d, 256, [3, 3], scope='colw3')
            model = slim.max_pool2d(model, [2, 2], scope='pool3')
            model = slim.repeat(model, 3, slim.colw2d, 512, [3, 3], scope='colw4')
            model = slim.max_pool2d(model, [2, 2], scope='pool4')
            model = slim.repeat(model, 3, slim.colw2d, 512, [3, 3], scope='colw5')
    ...
    def loss(self):
        ...
        accuracy = digits.classification_aclwracy(self.inference, self.y)
