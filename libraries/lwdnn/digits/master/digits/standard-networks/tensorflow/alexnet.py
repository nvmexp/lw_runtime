...
class UserModel(Tower):
    ...
    def inference(self):
        ...
        with slim.arg_scope([slim.colw2d, slim.fully_connected],
            ...
            model = slim.colw2d(x, 96, [11, 11], 4, padding='VALID', scope='colw1')
            model = slim.max_pool2d(model, [3, 3], 2, scope='pool1')
            model = slim.colw2d(model, 256, [5, 5], 1, scope='colw2')
            model = slim.max_pool2d(model, [3, 3], 2, scope='pool2')
            model = slim.colw2d(model, 384, [3, 3], 1, scope='colw3')
            model = slim.colw2d(model, 384, [3, 3], 1, scope='colw4')
            model = slim.colw2d(model, 256, [3, 3], 1, scope='colw5')
