...
from digits.device_query import get_device, get_lwml_info
...
class ModelForm(Form):
    ...
    batch_aclwmulation = utils.forms.IntegerField(
        'Batch Aclwmulation',
        validators=[
            validators.NumberRange(min=1),
            validators.Optional(),
    ...
    lr_ilw_gamma = wtforms.FloatField('Gamma', default=0.1)
    lr_ilw_power = wtforms.FloatField('Power', default=0.5)
    ...
    framework = wtforms.HiddenField(
        ...
        validators=[
            validators.AnyOf(
                ...
                message='The framework you choose is not lwrrently supported.'
            )
        ],
    ...
    lwstom_network = utils.forms.TextAreaField(
        'Custom Network',
        validators=[
    ...
    lwstom_network_snapshot = utils.forms.TextField(
        'Pretrained model(s)',
    ...
    def validate_lwstom_network_snapshot(form, field):
        pass
    ...
    select_gpu = wtforms.RadioField(
        ...
        choices=[('next', 'Next available')] + [(
            ...
            '#%s - %s (%s memory)' % (
                ...
                sizeof_fmt(
                    get_lwml_info(index)['memory']['total']
                    if get_lwml_info(index) and 'memory' in get_lwml_info(index)
