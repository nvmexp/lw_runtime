...
class BaseTestViews(BaseViewsTest):
    ...
    def test_visualize_network(self):
        rv = self.app.post('/models/visualize-network?framework=' + self.FRAMEWORK,
                           data={'lwstom_network': self.network()}
                           )
        s = BeautifulSoup(rv.data, 'html.parser')
        if rv.status_code != 200:
            body = s.select('body')[0]
            if 'IlwocationException' in str(body):
    ...
    def test_lwstomize(self):
        rv = self.app.post('/models/lwstomize?network=lenet&framework=' + self.FRAMEWORK)
        s = BeautifulSoup(rv.data, 'html.parser')
        body = s.select('body')
...
class BaseTestCreation(BaseViewsTestWithDataset):
    ...
    @unittest.skipIf(
        ...
        'no GPUs selected')
    @unittest.skipIf(
        not config_value('caffe')['lwda_enabled'],
        'LWCA disabled')
    @unittest.skipIf(
...
class BaseTestCreated(BaseViewsTestWithModel):
    ...
    def test_classify_many_ilwalid_ground_truth(self):
        textfile_images = ''
        label_id = 0
        for label, images in self.imageset_paths.iteritems():
            for image in images:
    ...
    def test_inference_while_training(self):
        ...
        if (config_value('gpu_list') and
                config_value('caffe')['lwda_enabled'] and
                config_value('caffe')['multi_gpu']):
...
@unittest.skipIf(
    not CaffeFramework().can_aclwmulate_gradients(),
    'This version of Caffe cannot accumulate gradients')
class TestBatchAclwmulationCaffe(BaseViewsTestWithDataset, test_utils.CaffeMixin):
    TRAIN_EPOCHS = 1
    IMAGE_COUNT = 10  # per class

    def test_batch_aclwmulation_callwlations(self):
        batch_size = 10
        batch_aclwmulation = 2

        job_id = self.create_model(
            batch_size=batch_size,
            batch_aclwmulation=batch_aclwmulation,
        )
        assert self.model_wait_completion(job_id) == 'Done', 'create failed'
        ...
        assert solver.iter_size == batch_aclwmulation, \
            'iter_size is %d instead of %d' % (solver.iter_size, batch_aclwmulation)
        max_iter = int(math.ceil(
            float(self.TRAIN_EPOCHS * self.IMAGE_COUNT * 3) /
            (batch_size * batch_aclwmulation)
        ))
...
class TestTorchLeNet(BaseTestCreated, test_utils.TorchMixin):
    ...
    def test_inference_while_training(self):
        ...
        # model for LeNet uses LwDNN by default and it diffilwlt to
        # perform inference on a LwDNN-trained model without non-trivial
        # model tweaking
        raise unittest.SkipTest('Torch CPU inference on LwDNN-trained model not supported')
