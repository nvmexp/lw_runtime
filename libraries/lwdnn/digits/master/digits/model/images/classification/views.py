    ...
    for sweep in sweeps:
        ...
        try:
            ...
            if form.method.data == 'standard':
                ...
                network = fw.get_network_from_desc(form.lwstom_network.data)
                pretrained_model = form.lwstom_network_snapshot.data.strip()
            else:
                raise werkzeug.exceptions.BadRequest(
            ...
            elif form.lr_policy.data == 'ilw':
                policy['gamma'] = form.lr_ilw_gamma.data
                policy['power'] = form.lr_ilw_power.data
            ...
            job.tasks.append(fw.create_train_task(
                ...
                batch_aclwmulation=form.batch_aclwmulation.data,
...
def classify_one():
    ...
    if inputs is not None and len(inputs['data']) == 1:
        ...
        # colwert to class probabilities for viewing
...
def classify_many():
    ...
    if outputs is not None:
        ...
        if show_ground_truth:
            top1_aclwracy = round(100.0 * n_top1_aclwrate / n_ground_truth, 2)
            top5_aclwracy = round(100.0 * n_top5_aclwrate / n_ground_truth, 2)
            per_class_aclwracy = []
            for x in xrange(n_labels):
                n_examples = sum(confusion_matrix[x])
                per_class_aclwracy.append(
    ...
    if request_wants_json():
        ...
        return flask.render_template('models/images/classification/classify_many.html',
                                     ...
                                     top1_aclwracy=top1_aclwracy,
                                     top5_aclwracy=top5_aclwracy,
                                     confusion_matrix=confusion_matrix,
                                     per_class_aclwracy=per_class_aclwracy,
                                     labels=labels,
                                     ), status_code
...
def top_n():
    ...
    if outputs is not None and len(outputs) > 0:
        ...
        if scores is None:
            raise RuntimeError('An error oclwrred while processing the images')
