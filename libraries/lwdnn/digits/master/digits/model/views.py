...
@blueprint.route('/lwstomize', methods=['POST'])
def lwstomize():
    """
    Returns a lwstomized file for the ModelJob based on completed form fields
    """
...
def visualize_network():
    ...
    ret = fw.get_network_visualization(
        desc=flask.request.form['lwstom_network'],
        dataset=dataset,
...
def visualize_lr():
    ...
    if policy == 'fixed':
        ...
        gamma = float(flask.request.form['lr_ilw_gamma'])
        power = float(flask.request.form['lr_ilw_power'])
    elif policy == 'poly':
    ...
    for j, lr in enumerate(lrs):
        ...
        for i in xrange(101):
            if policy == 'fixed':
                ...
                if lwrrent_step < len(steps) and i >= steps[lwrrent_step]:
                    lwrrent_step += 1
                data.append(lr * math.pow(gamma, lwrrent_step))
            elif policy == 'exp':
...
def publish_inference(job_id):
    ...
    """
    rie_url = os.elwiron.get('RIE_URL', "http://localhost:5055")
