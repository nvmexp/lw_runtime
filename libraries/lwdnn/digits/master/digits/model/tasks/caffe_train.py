...
class DigitsTransformer(caffe.io.Transformer):
    ...
    def preprocess(self, in_, data):
        ...
        """
        if not self.resize:
            # update target input dimension such that no resize oclwrs
...
class CaffeTrainTask(TrainTask):
        ...
        if state['pickver_task_caffe_train'] <= 3:
            try:
                ...
                if isinstance(self.network, caffe.proto.caffe_pb2.NetParameter):
                    # Colwert from NetParameter to string back to NetParameter
                    #   to avoid this error:
    ...
    def save_files_classification(self):
        ...
        # Set CPU/GPU mode
        if config_value('caffe')['lwda_enabled'] and \
        ...
        if self.batch_aclwmulation and CaffeFramework().can_aclwmulate_gradients():
            solver.iter_size = self.batch_aclwmulation
    ...
    @override
    def task_arguments(self, resources, elw):
        """
        ...
        elw -- dict of environment variables
        """
        if platform.system() == 'Windows':
            if any([layer.type == 'Python' for layer in self.network.layer]):
                ...
                # We can not ilwoke caffe.exe and need to fallback to pycaffe
        ...
        args = [config_value('caffe')['exelwtable'],
                'train',
        ...
        if 'gpus' in resources:
            ...
            if len(identifiers) == 1:
                ...
                if config_value('caffe')['flavor'] == 'LWPU':
                    if (utils.parse_version(config_value('caffe')['version']) < utils.parse_version('0.14.0-alpha')):
                        # Prior to version 0.14, LWcaffe used the --gpus switch
                    ...
                    raise ValueError('Unknown flavor.  Support LWPU and BVLC flavors only.')
    ...
    def _pycaffe_args(self, gpu_id):
        ...
        args = [sys.exelwtable + ' -c ' + '\"' + command_script + '\"']
        return args
    ...
    def new_iteration(self, it):
        """
        Update lwrrent_iteration
        """
        if self.lwrrent_iteration == it:
            return

        self.lwrrent_iteration = it
    ...
    def est_next_snapshot(self):
        if self.status != Status.RUN or self.lwrrent_iteration == 0:
            return None
        elapsed = time.time() - self.status_updates[-1][1]
        next_snapshot_iteration = (1 + self.lwrrent_iteration // self.snapshot_interval) * self.snapshot_interval
        return (next_snapshot_iteration - self.lwrrent_iteration) * elapsed // self.lwrrent_iteration
    ...
    def get_layer_statistics(self, data):
        ...
        data -- a np.ndarray
        """
        # XXX These callwlations can be super slow
...
def cleanedUpClassificationNetwork(original_network, num_categories):
    ...
    for i, layer in enumerate(network.layer):
        if 'Data' in layer.type:
            ...
            if (layer.aclwracy_param.HasField('top_k') and
                    layer.aclwracy_param.top_k > num_categories):
                del network.layer[i]
