...
class TrainTask(Task):
    ...
    def __init__(self, job, dataset, train_epochs, snapshot_interval, learning_rate, lr_policy, **kwargs):
        ...
        batch_aclwmulation -- accumulate gradients over multiple batches
        ...
        self.batch_aclwmulation = kwargs.pop('batch_aclwmulation', None)
    ...
    def __setstate__(self, state):
        if state['pickver_task_train'] < 2:
            ...
            va = state.pop('val_aclwracy_updates', None)
    ...
    def before_run(self):
        ...
        gpus = None
        if 'gpus' in self.lwrrent_resources:
            gpus = [identifier for (identifier, value) in self.lwrrent_resources['gpus']]
    ...
    def hw_socketio_updater(self, gpus):
        ...
        Arguments:
        gpus -- a list of identifiers for the GPUs lwrrently being used
        """
        ...
        while True:
            ...
            for index, device in devices:
                ...
                lwml_info = device_query.get_lwml_info(index)
                if lwml_info is not None:
                    update.update(lwml_info)
    ...
    def send_progress_update(self, epoch):
        ...
        """
        if self.lwrrent_epoch == epoch:
            return

        self.lwrrent_epoch = epoch
    ...
    def save_train_output(self, *args):
        ...
        self.logger.debug('Training %s%% complete.' % round(100 * self.lwrrent_epoch / self.train_epochs, 2))
    ...
    def save_output(self, d, name, kind, value):
        ...
        # update d['epoch']
        if 'epoch' not in d:
            d['epoch'] = NetworkOutput('Epoch', [self.lwrrent_epoch])
        elif d['epoch'].data[-1] != self.lwrrent_epoch:
            d['epoch'].data.append(self.lwrrent_epoch)
    ...
    def combined_graph_data(self, lwll=True):
        """
        Returns all train/val outputs in data for one C3.js graph

        Keyword arguments:
        lwll -- if True, cut down the number of data points returned to a reasonable size
        """
        data = {
            'columns': [],
            'xs': {},
