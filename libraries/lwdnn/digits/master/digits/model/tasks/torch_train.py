...
def subprocess_visible_devices(gpus):
    """
    Callwlates LWDA_VISIBLE_DEVICES for a subprocess
    """
    if not isinstance(gpus, list):
        raise ValueError('gpus should be a list')
    gpus = [int(g) for g in gpus]

    old_cvd = os.elwiron.get('LWDA_VISIBLE_DEVICES', None)
    if old_cvd is None:
        real_gpus = gpus
    else:
        map_visible_to_real = {}
...
class TorchTrainTask(TrainTask):
    ...
    def create_mean_file(self):
        ...
        if not os.path.exists(filename):
            ...
            if blob.channels == 3:
                # colwerting from BGR to RGB
                data = data[[2, 1, 0], ...]  # channel swap
                # colwert to (height, width, channels)
                data = data.transpose((1, 2, 0))
            else:
                assert blob.channels == 1
                # colwert to (height, width)
                data = data[0]
            # save to file
    ...
    def task_arguments(self, resources, elw):
        ...
        args = [config_value('torch')['exelwtable'],
                os.path.join(
        ...
        if self.lr_policy['policy'] == 'fixed':
            ...
            args.append('--gamma=%s' % self.lr_policy['gamma'])
        elif self.lr_policy['policy'] == 'ilw':
            args.append('--gamma=%s' % self.lr_policy['gamma'])
        ...
        if 'gpus' in resources:
            ...
            # LWCA libraries and allocate memory on all visible GPUs by
            # default.
            elw['LWDA_VISIBLE_DEVICES'] = subprocess_visible_devices(identifiers)
            # switch to GPU mode
            args.append('--type=lwca')
        else:
            # switch to CPU mode
    ...
    def est_next_snapshot(self):
        # TODO: Lwrrently this function is not in use. Probably in future we may have to implement this
        return None
    ...
    def infer_one_image(self, image, snapshot_epoch=None, layers=None, gpu=None):
        ...
        if gpu is not None:
            ...
            elw['LWDA_VISIBLE_DEVICES'] = subprocess_visible_devices([gpu])
        else:
            args.append('--type=float')
