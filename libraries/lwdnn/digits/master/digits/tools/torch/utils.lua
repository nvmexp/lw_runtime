...
-- Lwrrently zeroDataSize() and cleanupModel() routines aren't used but in future while implementing "recovery from crash" feature we may need to use these routines to clean the model before saving. This decreases the size of model by 80%.
...
function cleanupModel(node)
  ...
  -- Clear the references to the spatial colwolution outputs as well
  if _spatial_colwolution_mm_out ~= nil then
    _spatial_colwolution_mm_out = {}
  end

  if _spatial_colwolution_mm_gradout ~= nil then
    _spatial_colwolution_mm_gradout = {}
  end
...
-- colwert a string into a torch.CharTensor
