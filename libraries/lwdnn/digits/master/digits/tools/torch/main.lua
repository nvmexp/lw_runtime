...
-p,--type (default lwca) float or lwca
...
-q,--policy (default torch_sgd) Learning Rate Policy. Valid policies : fixed, step, exp, ilw, multistep, poly, sigmoid and torch_sgd. Note: when power value is -1, then "ilw" policy with "gamma" is similar to "torch_sgd" with "learningRateDecay".
-h,--gamma (default -1) Required to callwlate learning rate, when any of the following learning rate policies are used: step, exp, ilw, multistep & sigmoid
-j,--power (default inf) Required to callwlate learning rate, when any of the following learning rate policies are used: ilw & poly
-x,--stepvalues (default '') Required to callwlate stepsize for the following learning rate policies: step, multistep & sigmoid. Note: if it is 'step' or 'sigmoid' policy, then this parameter expects single value, if it is 'multistep' policy, then this parameter expects a string which has all the step values delimited by comma (ex: "10,25,45,80")
]]
...
COMPUTE_TRAIN_ACLWRACY = false

----------------------------------------------------------------------
-- Initial parameter checks

-- Colwert boolean options
...
-- verify whether required learning rate parameters are provided to callwlate learning rate when caffe-like learning rate policies are used
if opt.policy == 'fixed' or opt.policy == 'step' or opt.policy == 'exp' or opt.policy == 'ilw' or opt.policy == 'multistep' or opt.policy == 'poly' or opt.policy == 'sigmoid' then

    if opt.policy == 'step' or opt.policy == 'exp' or opt.policy == 'ilw' or opt.policy == 'multistep' or opt.policy == 'sigmoid' then
        if opt.gamma ==-1 then
            logmessage.display(2,'gamma parameter missing and is required to callwlate learning rate when ' .. opt.policy .. ' learning rate policy is used')
            os.exit(-1)
        end
    end

    if opt.policy == 'ilw' or opt.policy == 'poly' then
        if opt.power == math.huge then
            logmessage.display(2,'power parameter missing and is required to callwlate learning rate when ' .. opt.policy .. ' learning rate policy is used')
            os.exit(-1)
        end
    end
    ...
    if opt.policy == 'step' or opt.policy == 'multistep' or opt.policy == 'sigmoid' then
        if opt.stepvalues =='' then
            logmessage.display(2,'step parameter missing and is required to callwlate learning rate when ' .. opt.policy .. ' learning rate policy is used')
            os.exit(-1)
        else
...
elseif opt.policy ~= 'torch_sgd' then
    logmessage.display(2,'invalid learning rate policy - '.. opt.policy .. '. Valid policies : fixed, step, exp, ilw, multistep, poly, sigmoid and torch_sgd')
    os.exit(-1)
end
...
local nGpus = 0
if opt.type =='lwca' then
    require 'lwnn'
    require 'lwtorch'
    nGpus = lwtorch.getDeviceCount()
end
...
-- NOTE: lwrrently randomState option wasn't used in DIGITS. This option was provided to be used from command line, if required.
...
-- NOTE: lwrrently retrain option wasn't used in DIGITS. This option was provided to be used from command line, if required.
...
local function switchType(newType, model, loss)
    if newType == 'float' then
        ...
        model:float()
        loss = loss:float()
    elseif newType =='lwca' then
        lwtorch.setDevice(1)
        logmessage.display(0,'switching to LWCA')
        model:lwca()
        loss = loss:lwca()
    end
    return loss
...
if opt.policy ~= 'torch_sgd' then
    ...
    if max_iterations < #stepvalues_list then
        ...
        -- colwerting stepsize percentages into values
        for i=1,#stepvalues_list do
            stepvalues_list[i] = utils.round(max_iterations*stepvalues_list[i]/100)

            -- avoids 'nan' values during learning rate callwlation
...
-- NOTE: lwrrently lrpolicyState option wasn't used in DIGITS. This option was provided to be used from command line, if required.
...
--resetting "learningRateDecay = 0", so that sgd.lua won't recallwlates the learning rate
...
-- NOTE: lwrrently optimState option wasn't used in DIGITS. This option was provided to be used from command line, if required.
if opt.optimState ~= '' then
    if paths.filep(opt.optimState) then
        ...
        -- this makes sure that sgd.lua won't recallwlates the learning rate while using learning rate policy
        if lrpolicy.policy ~= 'torch_sgd' then
...
local optimizer = Optimizer{
    ...
    HookFunction = COMPUTE_TRAIN_ACLWRACY and updateConfusion or nil,
    lrPolicy = lrpolicy,
...
-- epoch value will be callwlated for every batch size. To maintain unique epoch value between batches, it needs to be rounded to the required number of significant digits.
...
-- NOTE: lwrrently this routine wasn't used in DIGITS.
...
local function Validation(model, loss, epoch, data_loader, data_size, batch_size, confusion, label_function)
    ...
    while count <= data_size do
        ...
        while data_loader:acceptsjob() do
            local lwrr_batch_size = math.min(data_size - data_index + 1, batch_size)
            if lwrr_batch_size > 0 then
                data_loader:scheduleNextBatch(lwrr_batch_size, data_index, data, true)
                data_index = data_index + lwrr_batch_size
            else break end
        end
...
local function Train(epoch, dataLoader)
    ...
    while t <= trainSize do
        ...
        if inputs then
            ...
            local lwrrent_epoch = (epoch-1)+utils.round((math.min(t+trainBatchSize-1,trainSize))/trainSize, epoch_round)

            -- log details on first iteration, or when required number of images are processed
            lwrr_images_cnt = lwrr_images_cnt + thisBatchSize
            if (epoch==1 and t==1) or lwrr_images_cnt >= logging_check then
                logmessage.display(0, 'Training (epoch ' .. lwrrent_epoch .. '): loss = ' .. (loss_sum/loss_batches_cnt) .. ', lr = ' .. learningrate)
                lwrr_images_cnt = 0 -- For accurate values we may assign lwrr_images_cnt % logging_check to lwrr_images_cnt, instead of 0
                loss_sum = 0
                loss_batches_cnt = 0
            end

            if opt.validation ~= '' and lwrrent_epoch >= next_validation then
                Validation(model, loss, lwrrent_epoch, valDataLoader, valSize, valBatchSize, valConfusion, labelFunction)

                next_validation = (utils.round(lwrrent_epoch/opt.interval) + 1) * opt.interval -- To find next nearest epoch value that exactly divisible by opt.interval
                last_validation_epoch = lwrrent_epoch
                model:training() -- to reset model to training
            end

            if lwrrent_epoch >= next_snapshot_save then
                saveModel(model, opt.save, snapshot_prefix, lwrrent_epoch)
                next_snapshot_save = (utils.round(lwrrent_epoch/opt.snapshotInterval) + 1) * opt.snapshotInterval -- To find next nearest epoch value that exactly divisible by opt.snapshotInterval
                last_snapshot_save_epoch = lwrrent_epoch
            end
