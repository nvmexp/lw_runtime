...
function LRPolicy:__init(...)
    ...
    elseif self.policy == 'multistep' then
      self.lwrrent_step = 1       -- this counter is important to take arbitrary steps
...
function LRPolicy:GetLearningRate(iter)
  ...
  elseif self.policy == "step" then
    local lwrrent_step = math.floor(iter/self.step_size)
    rate = self.baselr * math.pow(self.gamma, lwrrent_step)
  elseif self.policy == "exp" then
    rate = self.baselr * math.pow(self.gamma, progress)
  elseif self.policy == "ilw" then
    rate = self.baselr * math.pow(1 + self.gamma * progress, - self.power)
  elseif self.policy == "multistep" then
    if (self.lwrrent_step <= self.stepvalue_size and iter >= self.step_values[self.lwrrent_step]) then
      self.lwrrent_step = self.lwrrent_step + 1
    end
    rate = self.baselr * math.pow(self.gamma, self.lwrrent_step - 1);
