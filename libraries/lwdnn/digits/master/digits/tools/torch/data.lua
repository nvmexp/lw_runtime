...
local function all_keys(lwrsor_,key_,op_)
    return coroutine.wrap(
        function()
            local k = key_
            local v
            repeat
                k,v = lwrsor_:get(k,op_ or MDB.NEXT)
                if k then
                    coroutine.yield(k,v)
                end
            until (not k)
...
local warp = function(im, augRot, augScale)
    ...
    if augScale > 0 then
        ...
        -- Given a zoom in or out, we move around our calwas.
...
function LMDBSource:new(lighningmdb, path)
    ...
    self.e = self.lightningmdb.elw_create()
    ...
    if not db then
        ...
        -- conlwrrency issue - note that this shouldn't happen in DIGITS since the
    ...
    self.c = self.t:lwrsor_open(self.d)
    self.path = path
...
function DBSource:new (backend, db_path, labels_db_path, meanTensor, isTrain, shuffle, classification)
    ...
    elseif backend == 'hdf5' then
        ...
        -- which DB is lwrrently being used (initially set to nil to defer
...
function DataLoader:new (numThreads, package_path, backend, db_path, labels_db_path, mean, isTrain, shuffle, classification)
    ...
    self.threadPool = threads.Threads(
        ...
        function(threadid)
            ...
            require('data')
            -- exelwtes in reader thread, variables are local to this thread
...
function DataLoader:getInfo()
    ...
    -- handle (as happens when opening too many conlwrrent instances)
    for i=1,self.numThreads do
        self.threadPool:addjob(
                   i, -- thread to add job to
                   function()
                       -- exelwtes in reader thread, return values passed to
