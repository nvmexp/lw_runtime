...
-d,--devid (default 1) device ID (if using LWCA)
...
if pcall(function() require('lwtorch') end) then
    require 'lwnn'
    if opt.type =='lwca' then
        lwtorch.setDevice(opt.devid)
    end
end
...
function loadNetwork(dir, name, labels, weightsFile, tensorType, inputTensorShape)
    ...
    local parameters = {
        ngpus = (tensorType =='lwca') and 1 or 0,
...
local function loadImage(img_path)
    ...
    -- Torch image.load() always loads image with each pixel value between 0-1. As during training, images were taken from LMDB directly, their pixel values ranges from 0-255. As, model was trained with images whose pixel values are between 0-255, we may have to colwert test image also to have 0-255 for each pixel.
    im:mul(255)
...
local function preprocess(im, mean, croplen)
    ...
    -- Depending on the function arguments, image preprocess may include colwersion from RGB to BGR and mean subtraction, image resize after mean subtraction
...
local function predictBatch(inputs, model)
    ...
    elseif opt.type =='lwca' then
        predictions = model:forward(inputs:lwca())
    end
    ...
    for i=1,counter do
        ...
        if class_labels then
            -- assume logSoftMax and colwert to probabilities
            prediction:exp()
        end
...
if opt.testMany == 'yes' then
    ...
    if opt.visualization=='yes' then
        ...
        for i,layer in ipairs(model:listModules()) do
            ...
            name = tostring(layer)
            -- colwert 'name' string to Tensor as torch.hdf5 only
