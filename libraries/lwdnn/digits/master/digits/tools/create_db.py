...
class Hdf5Writer(DbWriter):
    ...
    def write_batch(self, batch):
        ...
        lwrrent_count = self._db['data'].len()

        # will fit in current dataset
        if lwrrent_count + len(batch) <= self._max_count:
            self._db['data'].resize(lwrrent_count + len(batch), axis=0)
            self._db['label'].resize(lwrrent_count + len(batch), axis=0)
            self._db['data'][-len(batch):] = data_batch
            self._db['label'][-len(batch):] = label_batch
            self._count += len(batch)
            return

        # callwlate how many will fit in current dataset
        split = self._max_count - lwrrent_count
...
def create_db(input_file, output_dir,
    ...
    batch_size = _callwlate_batch_size(image_count,
                                       bool(backend == 'hdf5'), kwargs.get('hdf5_dset_limit'),
                                       image_channels, image_height, image_width)
    num_threads = _callwlate_num_threads(batch_size, shuffle)
...
def _callwlate_batch_size(image_count, is_hdf5=False, hdf5_dset_limit=None,
                          image_channels=None, image_height=None, image_width=None):
    """
    Callwlates an appropriate batch size for creating this database
    """
...
def _callwlate_num_threads(batch_size, shuffle):
    """
    Callwlates an appropriate number of threads for creating this database
    """
    if shuffle:
...
def _initial_image_sum(width, height, channels):
    """
    Returns an array of zeros that will be used to store the aclwmulated sum of images
    """
    if channels == 1:
...
def _write_batch_lmdb(db, batch, image_count):
    ...
    try:
        ...
        # double the map_size
        lwrr_limit = db.info()['map_size']
        new_limit = lwrr_limit * 2
        try:
