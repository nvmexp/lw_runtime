...
class S3Walker(object):
    ...
    def __init__(self, endpoint, accesskey, secretkey):
        self.is_selwre = endpoint.find('https://') > -1
        ...
        logger.info('Host %s is secure: %s port: %s.' % (self.host, str(self.is_selwre), str(self.port)))

    def connect(self):

        self.conn = S3Connection(aws_access_key_id=self.accesskey, aws_secret_access_key=self.secretkey,
                                 is_selwre=self.is_selwre, host=self.host, port=self.port)
