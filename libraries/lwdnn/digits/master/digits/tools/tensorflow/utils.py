...
def classification_aclwracy_top_n(pred, y, top_n):
    single_acc_t = tf.nn.in_top_k(pred, y, top_n)
    return tf.reduce_mean(tf.cast(single_acc_t, tf.float32), name='aclwracy_top_%d' % top_n)


def classification_aclwracy(pred, y):
    """
...
def get_available_gpus():
    """
    Queries the LWCA GPU devices visible to Tensorflow.
    Returns:
