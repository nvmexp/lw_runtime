    ...
    def reshape_decode(self, data, shape):
        if self.float_data:  # @TODO(tzaman): this is LMDB specific - Make generic!
            ...
            if self.data_encoded:
                ...
                # if data is in CHW, set the shape and colwert to HWC
            ...
            # data = tf.image.colwert_image_dtype(data, tf.float32) # normalize to [0:1) range
        return data
    ...
    def create_input_pipeline(self):
        ...
        if self.aug_dict:
            with tf.name_scope('augmentation'):
                ...
                # @TODO(tzaman) whitening is so ilwasive that we need a way to add it to the val/inf too in a
                ...
                if aug_whitening:
                    ...
                    with tf.name_scope('whitening'):
                        single_data = tf.image.per_image_standardization(single_data)  # N.B. also colwerts to float
...
class LmdbLoader(LoaderFactory):
    ...
    def initialize(self):
        ...
        self.lmdb_elw = lmdb.open(self.db_path, readonly=True, lock=False)
        self.lmdb_txn = self.lmdb_elw.begin(buffers=False)
