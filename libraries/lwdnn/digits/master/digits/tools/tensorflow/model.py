...
def average_gradients(tower_grads):
    """Callwlate the average gradient for each shared variable across all towers.
    Note that this function provides a synchronization point across all towers.
    Args:
      tower_grads: List of lists of (gradient, variable) tuples. The outer list
        is over individual gradients. The inner list is over the gradient
        callwlation for each tower.
    Returns:
...
class Model(object):
    ...
    This class is exelwted to start a tensorflow workflow.
    """
    ...
    def create_model(self, obj_UserModel, stage_scope, batch_x=None):
        ...
        for dev_i, dev_name in enumerate(available_devices):
            with tf.device(dev_name):
                lwrrent_scope = stage_scope if len(available_devices) == 1 else ('tower_%d' % dev_i)
                with tf.name_scope(lwrrent_scope) as scope_tower:
