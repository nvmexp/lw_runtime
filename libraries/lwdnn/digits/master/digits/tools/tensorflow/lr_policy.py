...
class LRPolicy(object):
    ...
    Callwlates and returns the current learning rate. The lwrrently implemented learning rate
    ...
    def __init__(self, policy, base_rate, gamma, power, max_steps, step_values):
        ...
        else:            # Colwerting stepsize percentages into values
        ...
        if (self.policy == 'step') or (self.policy == 'sigmoid'):
            ...
            self.lwrrent_step = 0  # This counter is important to take arbitary steps
    ...
    def get_learning_rate(self, step):
        ...
        if self.policy == "fixed":
            ...
            lwrrent_step = math.floor(step/self.step_size)
            rate = self.base_rate * math.pow(self.gamma, lwrrent_step)
        elif self.policy == "exp":
            rate = self.base_rate * math.pow(self.gamma, progress)
        elif self.policy == "ilw":
            rate = self.base_rate * math.pow(1 + self.gamma * progress, - self.power)
        elif self.policy == "multistep":
            if ((self.lwrrent_step < self.stepvalue_size) and (step > self.stepvalues_list[self.lwrrent_step])):
                self.lwrrent_step = self.lwrrent_step + 1
            rate = self.base_rate * math.pow(self.gamma, self.lwrrent_step)
