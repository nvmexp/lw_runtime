...
"""
TensorFlow training exelwtable for DIGITS
Defines the training procedure

Usage:
See the self-dolwmenting flags below.
...
tf.app.flags.DEFINE_string(
    'lr_policy', 'fixed',
    """Learning rate policy. (fixed, step, exp, ilw, multistep, poly, sigmoid)""")
tf.app.flags.DEFINE_float(
    'lr_gamma', -1,
    """Required to callwlate learning rate. Applies to: (step, exp, ilw, multistep, sigmoid)""")
tf.app.flags.DEFINE_float(
    'lr_power', float('Inf'),
    """Required to callwlate learning rate. Applies to: (ilw, poly)""")
tf.app.flags.DEFINE_string(
    'lr_stepvalues', '',
    """Required to callwlate stepsize of the learning rate. Applies to: (step, multistep, sigmoid).
...
def Validation(sess, model, lwrrent_epoch):
    """
    ...
    logging.info("Validation (epoch " + str(lwrrent_epoch) + "): " + print_list)
...
def main(_):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        if FLAGS.train_db:
            ...
            # epoch value will be callwlated for every batch size. To maintain unique epoch value between batches,
            ...
            try:
                ...
                while not train_model.queue_coord.should_stop():
                    ...
                    run_metadata = None
                    if log_runtime:
                        # For a HARDWARE_TRACE you need LWPU LWPTI, a 'LWCA-EXTRA'
                    ...
                    lwrrent_epoch = round((step * batch_size_train) / train_model.dataloader.get_total(), epoch_round)
                    ...
                    if ((step % logging_interval_step) == 0):
                        ...
                        logging.info("Training (epoch " + str(lwrrent_epoch) + "): " + print_list)
                        print_vals_sum = 0
                        step_last_log = step

                    # Potential Validation Pass
                    if FLAGS.validation_db and lwrrent_epoch >= next_validation:
                        Validation(sess, val_model, lwrrent_epoch)
                        # Find next nearest epoch value that exactly divisible by FLAGS.validation_interval:
                        next_validation = (round(float(lwrrent_epoch)/FLAGS.validation_interval) + 1) * \
                            FLAGS.validation_interval

                    # Saving Snapshot
                    if FLAGS.snapshotInterval > 0 and lwrrent_epoch >= next_snapshot_save:
                        save_snapshot(sess, saver, FLAGS.save, snapshot_prefix, lwrrent_epoch, FLAGS.serving_export)

                        # To find next nearest epoch value that exactly divisible by FLAGS.snapshotInterval
                        next_snapshot_save = (round(float(lwrrent_epoch)/FLAGS.snapshotInterval) + 1) * \
