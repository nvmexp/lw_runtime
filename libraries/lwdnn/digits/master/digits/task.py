...
class Task(StatusCls):
    ...
    A Task is a compute-heavy operation that runs in a separate exelwtable
    Communication is done by processing the stdout of the exelwtable
    """
    ...
    def task_arguments(self, resources, elw):
        ...
        elwiron   -- os.elwiron instance to run process in
        """
        raise NotImplementedError

    def before_run(self):
        """
        Called before run() exelwtes
        Raises exceptions
        """
        pass
    ...
    def run(self, resources):
        ...
        elw = os.elwiron.copy()
        args = self.task_arguments(resources, elw)
        if not args:
        ...
        import sys
        elw['PYTHONPATH'] = os.pathsep.join(['.', self.job_dir, elw.get('PYTHONPATH', '')] + sys.path)

        # https://docs.python.org/2/library/subprocess.html#colwerting-argument-sequence
