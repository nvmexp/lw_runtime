...
def load_logfile_filename():
    ...
    if 'DIGITS_MODE_TEST' in os.elwiron:
        filename = None
    elif 'DIGITS_LOGFILE_FILENAME' in os.elwiron:
        filename = os.elwiron['DIGITS_LOGFILE_FILENAME']
        throw_error = True
    else:
    ...
    if filename is not None:
        try:
            ...
            if throw_error:
                ...
                print 'Set the elwvar DIGITS_LOGFILE_FILENAME to fix your configuration.'
                raise
            else:
                filename = None
    return filename
...
def load_logfile_level():
    ...
    elif 'DIGITS_LOGFILE_LEVEL' in os.elwiron:
        level = os.elwiron['DIGITS_LOGFILE_LEVEL'].strip().lower()
        if level == 'debug':
            return logging.DEBUG
        elif level == 'info':
            return logging.INFO
