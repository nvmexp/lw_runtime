...
class UserModel(Tower):
    ...
    def inference(self):
        ...
        with slim.arg_scope([slim.colw2d, slim.colw2d_transpose],
                            weights_initializer=tf.contrib.layers.xavier_initializer(),
                            weights_regularizer=slim.l2_regularizer(0.05)):

            # 1*H*W -> 32*H*W
            model = slim.colw2d(_x, 32, [3, 3], padding='SAME', scope='colw1')
            # 32*H*W -> 1024*H/16*W/16
            model = slim.colw2d(model, 1024, [16, 16], padding='VALID', scope='colw2', stride=16)
            model = slim.colw2d_transpose(model, self.input_shape[2], [16, 16],
                                          stride=16, padding='VALID', activation_fn=None, scope='decolw_1')
            return model
