...
os.elwiron['GLOG_minloglevel'] = '2'  # Suppress most caffe output
import caffe  # noqa
...
def load_image(path, height, width, mode='RGB'):
    ...
    Keyword arguments:
    mode -- the PIL mode that the image should be colwerted to
        (RGB for color or L for grayscale)
    """
    image = PIL.Image.open(path)
    image = image.colwert(mode)
...
def read_labels(labels_file):
    ...
    """
    if not labels_file:
        print 'WARNING: No labels file provided. Results will be diffilwlt to interpret.'
        return None
