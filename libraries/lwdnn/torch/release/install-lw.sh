...
while getopts 'aAbsvnh:' x; do
    case "$x" in
        a)
            export TORCH_LWDA_ARCH_LIST="3.0 3.5 5.0 5.2+PTX"
            ;;
        A)
            export TORCH_LWDA_ARCH_LIST="3.0 3.5 5.0 5.2 6.0 6.1+PTX"
...
setup_lua_elw_cmd=`fix_path "${HOME}/.luarocks" "$PREFIX"`
...
cd ${THIS_DIR}/exe/elw              && $LUAROCKS make || exit 1
...
#LWCL (experimental) support
cd ${THIS_DIR}/extra/lwcl         && $LUAROCKS make lwcl-scm-1.rockspec || exit 1

if [ -x "$path_to_lwcc" ] || [ -x "$path_to_lwidiasmi" ]
then
    echo "Found LWCA on your machine. Installing LWCA packages for ${LWDA_SELECT_LWCC_ARCH_TARGETS}"
    cd ${THIS_DIR}/extra/lwtorch  && $LUAROCKS  make LWDA_SELECT_LWCC_ARCH_TARGETS="${LWDA_SELECT_LWCC_ARCH_TARGETS}" rocks/lwtorch-scm-1.rockspec || exit 1
    cd ${THIS_DIR}/extra/lwnn     && $LUAROCKS  make LWDA_SELECT_LWCC_ARCH_TARGETS="${LWDA_SELECT_LWCC_ARCH_TARGETS}" rocks/lwnn-scm-1.rockspec    || exit 1
fi
...
then
    ...
    cd ${THIS_DIR}/extra/lwdnn   && $LUAROCKS make lwdnn-scm-1.rockspec || exit 1
    cd ${THIS_DIR}/extra/lwnnx   && $LUAROCKS make rocks/lwnnx-scm-1.rockspec || exit 1
    # lwca-colwnet2
