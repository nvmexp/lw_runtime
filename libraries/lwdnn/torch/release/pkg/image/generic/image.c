...
static inline temp_t image_(Main_lwbicInterpolate)(temp_t p0,
                                                   temp_t p1,
                                                   temp_t p2,
                                                   temp_t p3,
                                                   temp_t x) {
...
static void image_(Main_scaleLwbic_rowcol)(THTensor *Tsrc,
                                           THTensor *Tdst,
                                           long src_start,
                                           long dst_start,
                                           long src_stride,
  ...
  if ( dst_len == src_len ){
    ...
    for( di = 0; di < dst_len - 1; di++ ) {
      ...
      temp_t value = image_(Main_lwbicInterpolate)(p0, p1, p2, p3, si_f);
...
static int image_(Main_scaleBilwbic)(lua_State *L) {
  ...
  for(k=0;k<image_(Main_op_depth)(Tsrc);k++) {
    /* compress/expand rows first */
    for(j = 0; j < src_height; j++) {
      image_(Main_scaleLwbic_rowcol)(Tsrc,
                                     Ttmp,
                                     0*src_stride2+j*src_stride1+k*src_stride0,
                                     0*tmp_stride2+j*tmp_stride1+k*tmp_stride0,
                                     src_stride2,
    ...
    /* then columns */
    for(i = 0; i < dst_width; i++) {
      image_(Main_scaleLwbic_rowcol)(Ttmp,
                                     Tdst,
                                     i*tmp_stride2+0*tmp_stride1+k*tmp_stride0,
                                     i*dst_stride2+0*dst_stride1+k*dst_stride0,
                                     tmp_stride1,
...
/*
 * Colwerts an RGB color value to HSL. Colwersion formula
...
/*
 * Colwerts an HSL color value to RGB. Colwersion formula
 * adapted from http://en.wikipedia.org/wiki/HSL_color_space.
...
/*
 * Colwerts an RGB color value to HSV. Colwersion formula
...
/*
 * Colwerts an HSV color value to RGB. Colwersion formula
 * adapted from http://en.wikipedia.org/wiki/HSV_color_space.
 * Assumes h, s, and l are contained in the set [0, 1] and
 * returns r, g, and b in the set [0, 1].
 */
...
/*
 * Colwert an sRGB color channel to a linear sRGB color channel.
 */
...
/*
 * Colwert a linear sRGB color channel to a sRGB color channel.
 */
...
/*
 * Colwerts an sRGB color value to LAB.
...
/*
 * Colwerts an LAB color value to sRGB.
 * Based on http://www.brucelindbloom.com/index.html?Equations.html.
 * returns r, g, and b in the set [0, 1].
 */
...
static inline void image_(Main_bilwbicInterpolate)(
  ...
  real pad_value, int bounds_check)
{
  ...
  // Callwlate fractional and integer components
  long x_pix = floor(ix);
  long y_pix = floor(iy);
  temp_t dx = ix - x_pix;
  temp_t dy = iy - y_pix;
  ...
  for (k=0; k<size[0]; k++) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      arr[i] = image_(Main_lwbicInterpolate)(p[0], p[1], p[2], p[3], dx);
    }

    temp_t value = image_(Main_lwbicInterpolate)(arr[0], arr[1], arr[2], arr[3], dy);
        ...
        case 2:  // Bilwbic
          {
            ...
            if (edge) {
              image_(Main_bilwbicInterpolate)(src_data, is, src->size, ix, iy, dst, os, pad_value, 1);
            } else {
              image_(Main_bilwbicInterpolate)(src_data, is, src->size, ix, iy, dst, os, pad_value, 0);
            }
          }
          break;
        case 3:  // Lanczos
          {
            ...
            // and so we need to perform the O(k^2) colwolution on each pixel AND
            // we have to re-callwlate the kernel for every pixel.
            ...
            // Precallwlate the L(x) function evaluations in the u and v direction
            ...
            for (k=0; k<channels; k++) {
              temp_t result = 0;
              for (u=x_pix-rad+1, i=0; u<=x_pix+rad; u++, i++) {
                long lwru = MAX(MIN((long)(src_width-1), u), 0);
                for (v=y_pix-rad+1, j=0; v<=y_pix+rad; v++, j++) {
                  long lwrv = MAX(MIN((long)(src_height-1), v), 0);
                  temp_t Suv = src_data[k * is[0] + lwrv * is[1] + lwru * is[2]];
...
int image_(Main_gaussian)(lua_State *L) {
  ...
  // Precallwlate 1/(sigma*size) for speed (for some stupid reason the pragma
...
int image_(Main_drawtext)(lua_State *L) {
  ...
  for (long cnt = 0; cnt < len; cnt++) {
    ...
    if(c == '\n') {
      ...
      if(wrap && ((lwrsor_x + size * 6) >= width)) { // Heading off edge?
        lwrsor_x  = 0;            // Reset x to zero
        lwrsor_y += size * 8; // Advance y one line
      }
      image_(drawChar)(output, lwrsor_x, lwrsor_y, c, size,
                       cr, cg, cb,
                       bg_cr, bg_cg, bg_cb);
      lwrsor_x += size * 6;
    }
...
static const struct luaL_Reg image_(Main__) [] = {
  ...
  {"scaleBilwbic", image_(Main_scaleBilwbic)},
