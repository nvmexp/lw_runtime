...
function test.bilwbilwpscale()
  local im = outerProduct{1, 2, 4, 2}
  local expected = outerProduct{1, 1.4375, 2, 3.1875, 4, 3.25, 2}
  local actual = image.scale(im, expected:size(2), expected:size(1), 'bilwbic')
  tester:assertTensorEq(actual, expected, 1e-5)
end


function test.bilwbicDownscale()
  local im = outerProduct{1, 2, 4, 2}
...
function test.bilwbilwpscale_ByteTensor()
...
function flip_tests.test_transformation_largeByteImage(flip)
    ...
    assert(x_byte:size(1) > 256 and x_byte:size(2) > 256, 'Tricky case only oclwrs for images larger than 256 px, pick another example')
...
local function checkPNG(imfile, depth, tensortype, want)
  ...
  -- Tensors have to be colwerted to double, since assertTensorEq does not support ByteTensor
  --print('img: ', img)
  --print('want: ', want)
