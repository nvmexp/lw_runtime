...
function CInterface:__writechecks(txt, args, argset)
   ...
   for variant=0,(2^nopt)-1 do
      ...
      if hasvararg and not lwrrentargs[#lwrrentargs].vararg then
      ...
      if variant == 0 and argset == 1 then
         table.insert(txt, string.format('if(narg %s %d', compop, #lwrrentargs))
      else
         table.insert(txt, string.format('else if(narg %s %d', compop, #lwrrentargs))
      end

      for stackidx, arg in ipairs(lwrrentargs) do
