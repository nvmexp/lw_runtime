...
# must colwert font fo svg and ps
...
local function getexec()
   if not _gptable.exe then
      error('gnuplot exelwtable is not set')
   end
   return _gptable.exe
end
...
local function writeToLwrrent(str)
   writeToPlot(getLwrrentPlot(),str)
end
local function refreshLwrrent()
   refreshPlot(getLwrrentPlot())
end
...
local function filefigure(fname,term,n)
   ...
   gp.fname = fname
   gp.term = term
   writeToLwrrent('set term '.. gp.term)
   --writeToLwrrent('set output \'' .. gp.fname .. '\'')
end
...
function gnuplot.figprint(fname)
   ...
   writeToLwrrent('set term ' .. term)
   writeToLwrrent('set output \''.. fname .. '\'')
   refreshLwrrent()
   writeToLwrrent('unset output')
   writeToLwrrent('set term ' .. _gptable[_gptable.current].term .. ' ' .. _gptable.current .. '\n')
end
...
function gnuplot.xlabel(label)
   ...
   writeToLwrrent('set xlabel "' .. label .. '"')
   refreshLwrrent()
end
function gnuplot.ylabel(label)
   ...
   writeToLwrrent('set ylabel "' .. label .. '"')
   refreshLwrrent()
end
function gnuplot.zlabel(label)
   ...
   writeToLwrrent('set zlabel "' .. label .. '"')
   refreshLwrrent()
end
function gnuplot.title(label)
   ...
   writeToLwrrent('set title "' .. label .. '"')
   refreshLwrrent()
end
function gnuplot.grid(toggle)
   ...
   if toggle then
      writeToLwrrent('set grid')
      refreshLwrrent()
   else
      writeToLwrrent('unset grid')
      refreshLwrrent()
   end
end
function gnuplot.logscale(toggle)
   ...
   if toggle then
      writeToLwrrent('set logscale y')
      refreshLwrrent()
   else
      writeToLwrrent('unset logscale y')
      refreshLwrrent()
   end
end
function gnuplot.movelegend(hloc,vloc)
   ...
   writeToLwrrent('set key ' .. hloc .. ' ' .. vloc)
   refreshLwrrent()
end
...
function gnuplot.axis(axis)
   ...
   if axis == 'auto' then
      writeToLwrrent('set size nosquare')
      writeToLwrrent('set autoscale')
      refreshLwrrent()
   elseif axis == 'image' or axis == 'equal' then
      writeToLwrrent('set size ratio -1')
      refreshLwrrent()
   elseif axis == 'fill' then
      writeToLwrrent('set size ratio 1,1')
      refreshLwrrent()
   elseif type(axis) == 'table' then
      if #axis ~= 4 then print('axis should have 4 componets {xmin,xmax,ymin,ymax}'); return end
      writeToLwrrent('set xrange [' .. axis[1] .. ':' .. axis[2] .. ']')
      writeToLwrrent('set yrange [' .. axis[3] .. ':' .. axis[4] .. ']')
      refreshLwrrent()
   end
end
