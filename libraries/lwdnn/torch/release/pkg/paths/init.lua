...
if paths.is_win() then
   paths.home = os.getelw('HOMEDRIVE') or 'C:'
   paths.home = paths.home .. ( os.getelw('HOMEPATH') or '\\' )
else
   paths.home = os.getelw('HOME') or '.'
end
...
function paths.findprogram(...)
   for _,exe in ipairs{...} do
      if paths.is_win() then
         ...
         local path, k, x = os.getelw("PATH") or "."
         ...
         k = 'SOFTWARE\\Microsoft\\Windows\\LwrrentVersion\\App Paths\\' .. exe
         x = paths.getregistryvalue('HKEY_LWRRENT_USER', k, '')
