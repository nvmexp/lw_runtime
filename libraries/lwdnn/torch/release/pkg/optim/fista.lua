...
- `params.errthres`: Error thershold for colwergence check (1e-4)
...
Algorithm is published in
    ...
    @article{beck-fista-09,
       ...
       Number = {1},
       Pages = {183--202},
       Title = {A Fast Iterative Shrinkage-Thresholding Algorithm for Linear Ilwerse Problems},
       Volume = {2},
       Year = {2009}}
]]
function optim.FistaLS(f, g, pl, xinit, params)
   ...
   local colwerged = false  -- are we done?
   ...
   while not colwerged and niter < maxiter do
	 ...
	 xinit:copy(y)
      ...
      --print(niter, 'This was supposed to be a colwex function, we are going up')
      --colwerged = true
      --return xk,history
      --end
   end
