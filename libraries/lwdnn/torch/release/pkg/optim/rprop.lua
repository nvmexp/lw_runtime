...
(Martin Riedmiller, Koray Kavuklwoglu 2013)
--]]
function optim.rprop(opfunc, x, config, state)
   ...
   for i=1,niter do
      ...
      if not state.delta then
         ...
         if torch.type(x)=='torch.LwdaTensor' then
            -- Push to GPU
            state.psign    = state.psign:lwca()
            state.nsign    = state.nsign:lwca()
            state.zsign    = state.zsign:lwca()
            state.dminmax  = state.dminmax:lwca()
         end
      end
