...
function optim.cmaes(opfunc, x, config, state)
   ...
   local cc = (4 + mueff/N) / (N+4 + 2 * mueff/N)  -- time constant for lwmulation for C
   local cs = (mueff + 2) / (N + mueff + 5)  -- t-const for lwmulation for sigma control
   ...
   if not pcall(function () torch.symeig(C,'V') end) then -- if error oclwrs trying to use symeig
   ...
   local ilwsqrtC = torch.eye(N):typeAs(x)  -- C^-1/2
   ...
   local function ask()
      --[[return a list of lambda candidate solutions according to
       ...
       --]]
      -- Eigendecomposition: first update B, D and ilwsqrtC from C
      ...
      if counteval - eigeneval > lambda/(c1+cmu)/C:size(1)/10 then
         ...
         D, B = torch.symeig(C,'V') -- eigen decomposition, B==normalized eigelwectors, O(N^3)
         D = torch.sqrt(D)  -- D contains standard deviations now
         ilwsqrtC = (B * torch.diag(torch.pow(D,-1)) * B:t())
      end
   ...
   local function tell(arx)
      ...
      -- Lwmulation: update evolution paths
      local y = xmean - xold
      local z = ilwsqrtC * y -- == C^(-1/2) * (xnew - xold)
