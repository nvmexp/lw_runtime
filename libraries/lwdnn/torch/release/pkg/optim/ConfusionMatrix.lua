...
function ConfusionMatrix:__init(nclasses, classes)
   ...
   self.uniolwalids = torch.FloatTensor(nclasses):zero()
...
function ConfusionMatrix:updateValids()
   ...
   for t = 1,self.nclasses do
      ...
      self.uniolwalids[t] = self.mat[t][t] / (self.mat:select(1,t):sum()+self.mat:select(2,t):sum()-self.mat[t][t])
      total = total + self.mat[t][t]
   end
   self.totalValid = total / self.mat:sum()
   self.averageValid = 0
   self.averageUniolwalid = 0
   local lwalids = 0
   local nuniolwalids = 0
   for t = 1,self.nclasses do
      if not isNaN(self.valids[t]) then
         self.averageValid = self.averageValid + self.valids[t]
         lwalids = lwalids + 1
      end
      if not isNaN(self.valids[t]) and not isNaN(self.uniolwalids[t]) then
         self.averageUniolwalid = self.averageUniolwalid + self.uniolwalids[t]
         nuniolwalids = nuniolwalids + 1
      end
   end
   self.averageValid = self.averageValid / lwalids
   self.averageUniolwalid = self.averageUniolwalid / nuniolwalids
end

-- Callwlating FAR/FRR associated with the confusion matrix
...
function ConfusionMatrix:__tostring__()
   ...
   table.insert(str, ' + average rowUcol correct (VOC measure): ' .. (self.averageUniolwalid*100) .. '% \n')
...
function ConfusionMatrix:render(sortmode, display, block, legendwidth)
   ...
   local sortmode = sortmode or 'score' -- 'score' or 'oclwrrence'
   ...
   local legend = {
      ...
      ['oclwrrence'] = 'Confusion matrix [sorted by oclwrrences, accuracy = %0.3f%%, per-class accuracy = %0.3f%%]'
   }
   ...
   elseif sortmode == 'oclwrrence' then
      _,order = torch.sort(freqs,1,true)
   else
      error('sort mode must be one of: score | oclwrrence')
   end
