...
local TYPE_RELWR_FUNCTION = 8
local LEGACY_TYPE_RELWR_FUNCTION = 7
...
function File:referenced(ref)
   ...
   if not torch.getelw(self).writeObjects then
      torch.setelw(self, {
   ...
   local elw = torch.getelw(self)
   elw.force = not ref
   torch.setelw(self,elw)
   return self
end
...
function File:writeObject(object, debugname, hook)
   ...
   -- (e.g. Lwca to Float tensors, lwdnn to nn, ...)
   object = hook(object)
   local force = torch.getelw(self).force
   ...
   local objectNameStack = torch.getelw(self).objectNameStack
   ...
   elseif typeidx == TYPE_TORCH or typeidx == TYPE_TABLE or  typeidx == TYPE_RELWR_FUNCTION then
      -- check it exists already (we look at the pointer!)
      local objects = torch.getelw(self).writeObjects
      local objectsRef = torch.getelw(self).writeObjectsRef
      ...
      if index and (not force) then
         ...
         if typeidx == TYPE_RELWR_FUNCTION then
            local upvalueRefToId = torch.getelw(self).upvalueRefToId
            ...
            while true do
               ...
               if name == '_ELW' then value = nil end
               local id=nil
...
function File:readObject()
   ...
   elseif typeidx == TYPE_FUNCTION then
       ...
       return func
   elseif typeidx == TYPE_TABLE or typeidx == TYPE_TORCH or typeidx == TYPE_RELWR_FUNCTION or typeidx == LEGACY_TYPE_RELWR_FUNCTION then
      -- read the index
      local index = self:readInt()

      -- check it is loaded already
      local objects = torch.getelw(self).readObjects
      if objects[index] and not force then
         return objects[index]
      end

      -- otherwise read it
      if typeidx == TYPE_RELWR_FUNCTION or typeidx == LEGACY_TYPE_RELWR_FUNCTION then
         local size = self:readInt()
         local dumped = self:readChar(size):string()
         local func, err = loadstring(dumped)
         if not func then
	    io.stderr:write(string.format('Warning: Failed to load function from bytecode: %s', err))
         ...
         local upvalueIdToClosure = torch.getelw(self).upvalueIdToClosure
         local upvalues = self:readObject()
         for index,upvalue in ipairs(upvalues) do
            if typeidx == LEGACY_TYPE_RELWR_FUNCTION then
               debug.setupvalue(func, index, upvalue)
            elseif upvalue.name == '_ELW' then
               debug.setupvalue(func, index, _ELW)
            else
