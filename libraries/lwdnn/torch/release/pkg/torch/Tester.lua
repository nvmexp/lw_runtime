...
function Tester:_success()
   local name = self._lwrrentTestName
...
for colwenience). (We still document the ordering as "tolerance, message" for
...
if arg and enable_colors then  -- have we been ilwoked from the commandline?
...
function Tester:_run(tests)
   ...
   for name, fn in pairs(tests) do
      ...
      local strinit = coloured(string.format(cfmt, i), c.cyan)
                      ...
                      .. string.rep('.',
                                    NCOLS - 6 - 2 -
                                    cfmtlen - self._lwrrentTestName:len())
                      .. ' '
      ...
      if self.earlyAbort and (i < ntests) and (not status or not pass)
            and (not skip) then
         io.write('Aborting on first error, not all tests have been exelwted\n')
         break
      end
...
* `negate` (boolean) if true, we ilwert success and failure.
...
* `tb` (tensor)
* `tolerance` (number) maximum elementwise difference between `ta` and `tb`.
* `negate` (boolean) if negate is true, we ilwert success and failure.
...
Tables are compared relwrsively. Keys and types of the associated values must
match, relwrsively. Numbers are compared with the given tolerance.
...
function check.areTablesEq(t1, t2, tolerance, negate)
   ...
   -- check (for example, using relwrsion), let's do a breadth-first search
