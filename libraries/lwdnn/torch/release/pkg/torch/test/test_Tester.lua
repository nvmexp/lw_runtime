...
function tests.setEarlyAbort()
   ...
   for _, earlyAbort in ipairs{false, true} do
      ...
      local ilwokedCount = 0
      local myTests = {}
      function myTests.t1()
         ilwokedCount = ilwokedCount + 1
      ...
      tester:assert(ilwokedCount == (earlyAbort and 1 or 2),
                    "wrong number of tests ilwoked for use with earlyAbort")
   end
...
function tests.disable()
   ...
   for disableCount = 1, 2 do
      local myTester = torch.Tester()
      local tests = {}
      local test1Ilwoked = false
      local test2Ilwoked = false
      ...
      if disableCount == 1 then
         ...
         tester:assert((not test1Ilwoked) and test2Ilwoked,
                       "disabled test shouldn't have been ilwoked")
      else
         myTester:disable({'test1', 'test2'}):run()
         tester:assert((not test1Ilwoked) and (not test2Ilwoked),
                       "disabled tests shouldn't have been ilwoked")
      end
   end
