...
function torchtest.rsqrt()
   ...
   local elw = { TH_rsqrt=TH_rsqrt, torch=torch, math=math }
   if not setfelw then -- Lua 5.2
      f = load(t, 'test', 't', elw)
   else
      f = loadstring(t)
      setfelw(f, elw)
...
function torchtest.frac()
   ...
   local elw = { TH_frac=TH_frac, torch=torch, math=math }
   if not setfelw then -- Lua 5.2
      f = load(t, 'test', 't', elw)
   else
      f = loadstring(t)
...
function torchtest.trunc()
   ...
   local elw = { TH_trunc=TH_trunc, torch=torch, math=math }
   if not setfelw then -- Lua 5.2
      f = load(t, 'test', 't', elw)
   else
      f = loadstring(t)
...
function torchtest.cilw()
   ...
   local res_ilw = a:clone()
   res_ilw:cilw()

   mytester:assertlt((res_pow - res_ilw):abs():max(), 0.00001)
...
function torchtest.lwmsum()
   local x = torch.rand(msize,msize)
   local mx = torch.lwmsum(x,2)
   local mxx = torch.Tensor()
   torch.lwmsum(mxx,x,2)
   mytester:asserteq(maxdiff(mx,mxx),0,'torch.lwmsum value')
end
function torchtest.lwmprod()
   local x = torch.rand(msize,msize)
   local mx = torch.lwmprod(x,2)
   local mxx = torch.Tensor()
   torch.lwmprod(mxx,x,2)
   mytester:asserteq(maxdiff(mx,mxx),0,'torch.lwmprod value')
end
...
function torchtest.ilwerse()
   if not torch.ilwerse then return end
   local M = torch.randn(5,5)
   local MI = torch.ilwerse(M)
   local E = torch.eye(5)
   mytester:assert(not MI:isContiguous(), 'MI is contiguous')
   mytester:assertalmosteq(maxdiff(E,torch.mm(M,MI)), 0, 1e-8, 'ilwerse value')
   mytester:assertalmosteq(maxdiff(E,torch.mm(MI,M)), 0, 1e-8, 'ilwerse value')

   local MII = torch.Tensor(5,5)
   torch.ilwerse(MII, M)
   mytester:assert(not MII:isContiguous(), 'MII is contiguous')
   mytester:asserteq(maxdiff(MII, MI), 0, 'ilwerse value in-place')
   -- second call, now that MII is transposed
   torch.ilwerse(MII, M)
   mytester:assert(not MII:isContiguous(), 'MII is contiguous')
   mytester:asserteq(maxdiff(MII, MI), 0, 'ilwerse value in-place')
end
function torchtest.colw2()
   ...
   local imvc = torch.colw2(x,k)
   local imvc2 = torch.colw2(x,k,'V')
   local imfc = torch.colw2(x,k,'F')
   ...
   mytester:asserteq(maxdiff(imvc,imvc2),0,'torch.colw2')
   mytester:asserteq(maxdiff(imvc,imvx),0,'torch.colw2')
   mytester:asserteq(maxdiff(imvc,imvx2),0,'torch.colw2')
   mytester:asserteq(maxdiff(imfc,imfx),0,'torch.colw2')
   mytester:assertlt(math.abs(x:dot(x)-torch.xcorr2(x,x)[1][1]),1e-10,'torch.colw2')
   ...
   local immvc = torch.colw2(xx,kk)
   local immvc2 = torch.colw2(xx,kk,'V')
   local immfc = torch.colw2(xx,kk,'F')

   mytester:asserteq(maxdiff(immvc[1],immvc[2]),0,'torch.colw2')
   mytester:asserteq(maxdiff(immvc[1],imvc),0,'torch.colw2')
   mytester:asserteq(maxdiff(immvc2[1],imvc2),0,'torch.colw2')
   mytester:asserteq(maxdiff(immfc[1],immfc[2]),0,'torch.colw2')
   mytester:asserteq(maxdiff(immfc[1],imfc),0,'torch.colw2')
end

function torchtest.colw3()
   ...
   local k = torch.rand(math.floor(torch.uniform(5,10)),
                        ...
                        math.floor(torch.uniform(5,10)))
   local imvc = torch.colw3(x,k)
   local imvc2 = torch.colw3(x,k,'V')
   local imfc = torch.colw3(x,k,'F')
   ...
   mytester:asserteq(maxdiff(imvc,imvc2),0,'torch.colw3')
   mytester:asserteq(maxdiff(imvc,imvx),0,'torch.colw3')
   mytester:asserteq(maxdiff(imvc,imvx2),0,'torch.colw3')
   mytester:asserteq(maxdiff(imfc,imfx),0,'torch.colw3')
   mytester:assertlt(math.abs(x:dot(x)-torch.xcorr3(x,x)[1][1][1]),4*1e-10,'torch.colw3')
   ...
   local immvc = torch.colw3(xx,kk)
   local immvc2 = torch.colw3(xx,kk,'V')
   local immfc = torch.colw3(xx,kk,'F')

   mytester:asserteq(maxdiff(immvc[1],immvc[2]),0,'torch.colw3')
   mytester:asserteq(maxdiff(immvc[1],imvc),0,'torch.colw3')
   mytester:asserteq(maxdiff(immvc2[1],imvc2),0,'torch.colw3')
   mytester:asserteq(maxdiff(immfc[1],immfc[2]),0,'torch.colw3')
   mytester:asserteq(maxdiff(immfc[1],imfc),0,'torch.colw3')
end
...
function torchtest.xcorr3_xcorr2_eq()
    ...
    mytester:assertlt(maxdiff(o3,o32),precision,'torch.colw3_colw2_eq')
end
...
function torchtest.colw3_colw2_eq()
    local ix = math.floor(torch.uniform(20,40))
    local iy = math.floor(torch.uniform(20,40))
    local iz = math.floor(torch.uniform(20,40))
    local kx = math.floor(torch.uniform(5,10))
    ...
    local o3 = torch.colw3(x,k)
    local o32 = torch.zeros(o3:size())

    for i=1,o3:size(1) do
        for j=1,k:size(1) do
            o32[i]:add(torch.colw2(x[i+j-1],k[k:size(1)-j+1]))
        end
    end
...
function torchtest.fcolw3_fcolw2_eq()
    local ix = math.floor(torch.uniform(20,40))
    local iy = math.floor(torch.uniform(20,40))
    local iz = math.floor(torch.uniform(20,40))
    local kx = math.floor(torch.uniform(5,10))
    ...
    local o3 = torch.colw3(x,k,'F')

    local o32 = torch.zeros(o3:size())

    for i=1,x:size(1) do
        for j=1,k:size(1) do
            o32[i+j-1]:add(torch.colw2(x[i],k[j],'F'))
        end
    end
...
function torchtest.potri()
   ...
   ---- Compute ilwerse directly
   local ilw0 = torch.ilwerse(a)

   ---- Default case
   local chol = torch.potrf(a)
   local ilw1 = torch.potri(chol)
   mytester:assertlt(ilw0:dist(ilw1),1e-12,"torch.potri; uplo=''")

   ---- Upper Triangular Test
   chol = torch.potrf(a, 'U')
   ilw1 = torch.potri(chol, 'U')
   mytester:assertlt(ilw0:dist(ilw1),1e-12,"torch.potri; uplo='U'")

   ---- Lower Triangular Test
   chol = torch.potrf(a, 'L')
   ilw1 = torch.potri(chol, 'L')
   mytester:assertlt(ilw0:dist(ilw1),1e-12,"torch.potri; uplo='L'")
end
...
local function conselwtive(size, start)
    local sequence = torch.ones(torch.Tensor(size):prod(1)[1]):lwmsum(1)
    if start then
...
function torchtest.index()
    ...
    local reference = conselwtive{3, 3, 3}
    mytester:assertTensorEq(reference[1], conselwtive{3, 3}, 1e-16, badIndexMsg)
    mytester:assertTensorEq(reference[2], conselwtive({3, 3}, 10), 1e-16, badIndexMsg)
    mytester:assertTensorEq(reference[3], conselwtive({3, 3}, 19), 1e-16, badIndexMsg)
    mytester:assertTensorEq(reference[{1}], conselwtive{3, 3}, 1e-16, badIndexMsg)
    mytester:assertTensorEq(reference[{2}], conselwtive({3, 3}, 10), 1e-16, badIndexMsg)
    mytester:assertTensorEq(reference[{3}], conselwtive({3, 3}, 19), 1e-16, badIndexMsg)
    mytester:assertTensorEq(reference[{1,2}], conselwtive({3}, 4), 1e-16, badIndexMsg)
    mytester:assertTensorEq(reference[{{1,2}}], conselwtive({2, 3, 3}), 1e-16, badIndexMsg)
    mytester:asserteq(reference[{3, 3, 3}], 27, badIndexMsg)
    mytester:assertTensorEq(reference[{}], conselwtive{3, 3, 3}, 1e-16, badIndexMsg)
...
function torchtest.newIndex()
    ...
    local function checkPartialAssign(index)
        ...
        reference[index] = conselwtive{3, 3, 3}[index]
        mytester:assertTensorEq(reference[index], conselwtive{3, 3, 3}[index], 1e-16, badIndexMsg)
        reference[index] = 0
...
function torchtest.table()
   ...
   for k,v in ipairs(colwStorage) do
