...
static size_t THDiskFile_position(THFile *self)
{
  ...
  if (offset > -1)
      ...
      THError("unable to obtain disk file offset (maybe a long overflow oclwrred)");
...
THFile *THPipeFile_new(const char *name, const char *mode, int isQuiet)
{
  ...
  if(!handle)
  {
    if(isQuiet)
      ...
      THError("cannot open <%s> in mode %c%c.  This might be because eg the exelwtable doesn't exist, but it could also be because you are out of memory.", name, (isReadable ? 'r' : ' '), (isWritable ? 'w' : ' '));
  }
