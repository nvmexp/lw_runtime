...
void THTensor_(lwmsum)(THTensor *r_, THTensor *t, int dimension)
{
  THArgCheck(dimension >= 0 && dimension < THTensor_(nDimension)(t), 2, "dimension %d out of range",
      dimension + TH_INDEX_BASE);
  ...
  TH_TENSOR_DIM_APPLY2(real, t, real, r_, dimension,
                       ...
                       for(i = 0; i < t_size; i++)
                       {
                         lwmsum += t_data[i*t_stride];
                         r__data[i*r__stride] = (real)lwmsum;
                       });
}

void THTensor_(lwmprod)(THTensor *r_, THTensor *t, int dimension)
{
  THArgCheck(dimension >= 0 && dimension < THTensor_(nDimension)(t), 2, "dimension %d out of range",
      dimension + TH_INDEX_BASE);
  ...
  TH_TENSOR_DIM_APPLY2(real, t, real, r_, dimension,
                       ...
                       for(i = 0; i < t_size; i++)
                       {
                         lwmprod *= t_data[i*t_stride];
                         r__data[i*r__stride] = (real)lwmprod;
                       });
}
...
void THTensor_(cmilwalue)(THTensor *r, THTensor *t, real value) {
  THTensor_(resizeAs)(r, t);
  TH_TENSOR_APPLY2(real, r, real, t,
                   *r_data = *t_data < value ? *t_data : value;);
}
...
LAB_IMPLEMENT_BASIC_FUNCTION(neg,-)
LAB_IMPLEMENT_BASIC_FUNCTION(cilw, TH_MATH_NAME(1.0) / )
...
void THTensor_(histc)(THTensor *hist, THTensor *tensor, long nbins, real milwalue, real maxvalue)
{
  real milwal;
  real maxval;
  real *h_data;
  ...
  if (milwal == maxval)
  {
    milwal = THTensor_(minall)(tensor);
  ...
  TH_TENSOR_APPLY(real, tensor,
    if (*tensor_data >= milwal && *tensor_data <= maxval) {
      const int bin = (int)((*tensor_data-milwal) / (maxval-milwal) * nbins);
      h_data[THMin(bin, nbins-1)] += 1;
    }
  );
}

void THTensor_(bhistc)(THTensor *hist, THTensor *tensor, long nbins, real milwalue, real maxvalue)
{
  ...
  TH_TENSOR_DIM_APPLY2(real, tensor, real, hist, dimension, long i;
                        ...
                        {
                          if(tensor_data[i*tensor_stride] >= milwal && tensor_data[i*tensor_stride] <= maxval) {
                            const int bin = (int)((tensor_data[i*tensor_stride]-milwal) / (maxval-milwal) * nbins);
