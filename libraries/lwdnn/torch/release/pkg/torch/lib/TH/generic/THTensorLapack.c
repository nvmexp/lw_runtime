...
void THTensor_(gels)(THTensor *rb_, THTensor *ra_, THTensor *b, THTensor *a)
{
  ...
  /* rb__ is lwrrently ldb by nrhs; resize it to n by nrhs */
  rb__->size[0] = n;
  if (rb__ != rb_)
...
void THTensor_(geev)(THTensor *re_, THTensor *rv_, THTensor *a_, const char *jobvr)
{
  ...
  THLapackCheckWithCleanup(" Lapack Error in %s : %d off-diagonal elements of an didn't colwerge to zero",
...
void THTensor_(syev)(THTensor *re_, THTensor *rv_, THTensor *a, const char *jobz, const char *uplo)
{
  ...
  THLapackCheckWithCleanup("Lapack Error %s : %d off-diagonal elements didn't colwerge to zero",
...
void THTensor_(gesvd2)(THTensor *ru_, THTensor *rs_, THTensor *rv_, THTensor *ra_, THTensor *a, const char* jobu)
{
  ...
  THLapackCheckWithCleanup(" Lapack Error %s : %d superdiagonals failed to colwerge.",
                           THCleanup(
                               THTensor_(free)(ru__);
                               THTensor_(free)(rs__);
                               THTensor_(free)(rv__);
...
/*
  ...
  directly, below - but to make the common usage colwenient, we also provide
