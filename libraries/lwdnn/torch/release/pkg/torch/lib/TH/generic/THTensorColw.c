...
#define TH_GENERIC_FILE "generic/THTensorColw.c"
#else

/*
  2D Input, 2D kernel  : colwolve given image with the given kernel.
*/
...
/*
  2D Input, 2D kernel  : colwolve given image with the given kernel.
*/
void THTensor_(validColw2Dptr)(real *r_,
                                      real alpha,
                                      real *t_, long ir, long ic,
                                      real *k_, long kr, long kc,
                                      long sr, long sc)
...
/*
  2D Input, 2D kernel  : colwolve given image with the given kernel, full colwolution.
*/
void THTensor_(fullColw2Dptr)(real *r_,
                                     real alpha,
                                     real *t_, long ir, long ic,
                                     real *k_, long kr, long kc,
                                     long sr, long sc)
...
/*
  2D Input, 2D kernel  : colwolve given image with the given kernel, valid colwolution.
...
/*
  3D Input, 3D kernel  : colwolve given volume with the given kernel.
*/
...
/*
  3D Input, 3D kernel  : colwolve given volume with the given kernel.
*/
void THTensor_(validColw3Dptr)(real *r_,
                                      real alpha,
                                      real *t_, long it, long ir, long ic,
                                      real *k_, long kt, long kr, long kc,
                                      long st, long sr, long sc)
...
/*
  3D Input, 3D kernel  : colwolve given volume with the given kernel, full colwolution.
*/
void THTensor_(fullColw3Dptr)(real *r_,
                                     real alpha,
                                     real *t_, long it, long ir, long ic,
                                     real *k_, long kt, long kr, long kc,
                                     long st, long sr, long sc)
...
/*
  3D Input, 3D kernel  : colwolve given image with the given kernel, valid colwolution.
...
void THTensor_(colw2d)(real* output_data,
                       real alpha,
...
{
  THArgCheck(*vf == 'V' || *vf == 'F', 7, "type of colwolution can be 'V' or 'F'");
  THArgCheck(*xc == 'C' || *xc == 'X', 7, "type of colwolution can be 'X' or 'C'");
  if (*vf == 'F')
    if (*xc == 'X')
      ...
      THTensor_(fullColw2Dptr)(output_data,
                               alpha,
                               ptr_input,  nInputRows,  nInputCols,
                               ptr_weight, nKernelRows, nKernelCols,
                               srow, scol);
    ...
    if (*xc == 'X')
      ...
      THTensor_(validColw2Dptr)(output_data,
                                alpha,
                                ptr_input,  nInputRows,  nInputCols,
                                ptr_weight, nKernelRows, nKernelCols,
                                srow, scol);
...
void THTensor_(colw3d)(real* output_data,
                       real alpha,
                       ...
                       const char *vf, const char *xc)
{
  ...
  if (*vf == 'F')
    if (*xc == 'X')
      ...
      THTensor_(fullColw3Dptr)(output_data,
                               alpha,
                               ptr_input, nInputDepth, nInputRows,  nInputCols,
                               ptr_weight, nKernelDepth, nKernelRows, nKernelCols,
                               sdepth, srow, scol);
    ...
    if (*xc == 'X')
      ...
      THTensor_(validColw3Dptr)(output_data,
                                alpha,
                                ptr_input, nInputDepth, nInputRows,  nInputCols,
                                ptr_weight, nKernelDepth, nKernelRows, nKernelCols,
                                sdepth, srow, scol);
...
long THTensor_(colwsize)(long x, long k, long s, const char* vf)
{
  THArgCheck(*vf == 'V' || *vf == 'F', 1, "type of colwolution can be 'V' or 'F'");
  if (*vf == 'V')
    return (x-k)/s + 1;
  else
    return (x-1)*s + k;
...
/*
  ...
  like rank1 update
  A <- xx' + beta*A
  for sr,sc=1 this is equivalent to colw2Dger, but otherwise it is useful for
  callwlating derivatives wrt a kernel that is applied with stride sr,sc != 1
*/
void THTensor_(colw2DRevger)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_, long srow, long scol)
{
...
/*
  ...
  callwlating derivatives wrt a kernel that is applied with stride sr,sc != 1
*/
void THTensor_(colw2DRevgerm)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_, long srow, long scol)
{
  ...
  THArgCheck(nInputRows >= nKernelRows && nInputCols >= nKernelCols , 2, "colw2DRevger : Input image is smaller than kernel");
  THArgCheck(kernel->size[0] == input->size[0] , 2, "colw2DRevger : Input batch and kernel batch is not same size");
...
/*
  ...
  A <- xx' + beta*A
*/
void THTensor_(colw2Dger)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_, long srow, long scol, const char *vf, const char *xc)
{
  long nInputPlane, nInputRows, nInputCols;
  long nKernelPlane, nKernelRows, nKernelCols;
  long nOutputPlane, nOutputRows, nOutputCols;
  ...
  THArgCheck(srow >= 1, 5, "Stride should be a positive integer");
  THArgCheck(scol >= 1, 6, "Stride should be a positive integer");
  THArgCheck(*vf == 'V' || *vf == 'F', 7, "type of colwolution can 'V' or 'F'");
  THArgCheck(*xc == 'C' || *xc == 'X', 7, "type of colwolution can 'X' or 'C'");
  ...
  THArgCheck((nInputRows >= nKernelRows && nInputCols >= nKernelCols) || *vf == 'F', 2, "colw2Dger : Input image is smaller than kernel");
    ...
    for(i = 0; i < nInputPlane; i++)
    {
      ...
      if (*vf == 'F')
        if (*xc == 'X')
          ...
          THTensor_(fullColw2Dptr)(ptr_output,
                                   alpha,
                                   ptr_input,  nInputRows,  nInputCols,
                                   ptr_weight, nKernelRows, nKernelCols,
                                   srow, scol);
        ...
        if (*xc == 'X')
          ...
          THTensor_(validColw2Dptr)(ptr_output,
                                    alpha,
                                    ptr_input,  nInputRows,  nInputCols,
                                    ptr_weight, nKernelRows, nKernelCols,
                                    srow, scol);
...
/*
  ...
  y <- Ax + beta*y
*/
void THTensor_(colw2Dmv)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_, long srow, long scol, const char *vf, const char *xc)
{
  long nInputPlane, nInputRows, nInputCols;
  ...
  THArgCheck( (nInputRows >= nKernelRows && nInputCols >= nKernelCols) || *vf == 'F', 2, "colw2Dmv : Input image is smaller than kernel");
...
/*
  ...
  y <- Ax + beta*y
*/
void THTensor_(colw2Dmm)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_, long srow, long scol, const char *vf, const char *xc)
{
  long nInputPlane, nInputRows, nInputCols;
  long nKernelRows, nKernelCols;
  long nOutputPlane, nOutputRows, nOutputCols;
...
/*
  ...
  y <- x*y + beta*y
*/
void THTensor_(colw2Dmul)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_, long srow, long scol, const char *vf, const char *xc)
{
  THTensor *input;
  THTensor* kernel;
  long nInputRows;
  ...
  THArgCheck((nInputRows >= nKernelRows && nInputCols >= nKernelCols) || *vf == 'F', 2, "colw2Dmul : Input image is smaller than kernel");

  nOutputRows = THTensor_(colwsize)(nInputRows, nKernelRows, srow, vf);
  nOutputCols = THTensor_(colwsize)(nInputCols, nKernelCols, scol, vf);
...
/*
  ...
  y <- y.*x + beta*y
*/
void THTensor_(colw2Dcmul)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_, long srow, long scol, const char *vf, const char *xc)
{
  long nInputPlane, nInputRows, nInputCols;
  long nKernelRows, nKernelCols;
  long nOutputPlane, nOutputRows, nOutputCols;
  ...
  THArgCheck( (nInputRows >= nKernelRows && nInputCols >= nKernelCols) || *vf == 'F', 2, "colw2Dcmul : Input image is smaller than kernel");
...
/*
  ...
  y <- y.*x + beta*y
*/
void THTensor_(colw2Dmap)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_, THTensor *map, long srow, long scol, const char *vf, const char *xc)
{
  long nInputPlane, nInputRows, nInputCols;
  long nKernelRows, nKernelCols;
  long nOutputPlane, nOutputRows, nOutputCols;
  ...
  THArgCheck( (nInputRows >= nKernelRows && nInputCols >= nKernelCols)
              || *vf == 'F', 2, "colw2Dmap : Input image is smaller than kernel");
...
/*
  ...
  callwlating derivatives wrt a kernel that is applied with stride sr,sc != 1
*/
void THTensor_(colw3DRevger)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_,
                             long sdepth, long srow, long scol)
{
  ...
  THArgCheck(nInputDepth >= nKernelDepth && nInputRows >= nKernelRows && nInputCols >= nKernelCols , 2, "colw3DRevger : Input image is smaller than kernel");
...
/*
  ...
  A <- xx' + beta*A
*/
void THTensor_(colw3Dger)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_,
                          long sdepth, long srow, long scol, const char *vf, const char *xc)
{
  ...
  THArgCheck(srow >= 1, 6, "Stride should be a positive integer");
  THArgCheck(scol >= 1, 7, "Stride should be a positive integer");
  THArgCheck(*vf == 'V' || *vf == 'F', 8, "type of colwolution can 'V' or 'F'");
  THArgCheck(*xc == 'C' || *xc == 'X', 8, "type of colwolution can 'X' or 'C'");
  ...
  THArgCheck((nInputDepth >= nKernelDepth
             ...
             || *vf == 'F', 2, "colw3Dger : Input image is smaller than kernel");

  nOutputDepth = THTensor_(colwsize)(nInputDepth, nKernelDepth, sdepth, vf);
  nOutputRows = THTensor_(colwsize)(nInputRows, nKernelRows, srow, vf);
  nOutputCols = THTensor_(colwsize)(nInputCols, nKernelCols, scol, vf);
...
/*
  ...
  y <- Ax + beta*y
*/
void THTensor_(colw3Dmv)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_,
                         long sdepth, long srow, long scol, const char *vf, const char *xc)
{
  long nInputPlane, nInputDepth, nInputRows, nInputCols;
  ...
  THArgCheck( (nInputDepth >= nKernelDepth && nInputRows >= nKernelRows && nInputCols >= nKernelCols) || *vf == 'F', 2, "colw3Dmv : Input image is smaller than kernel");
...
/*
  ...
  y <- x*y + beta*y
*/
void THTensor_(colw3Dmul)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_,
                          long sdepth, long srow, long scol, const char *vf, const char *xc)
{
  THTensor *input;
  THTensor* kernel;
  ...
  THArgCheck((nInputDepth >= nKernelDepth && nInputRows >= nKernelRows && nInputCols >= nKernelCols) || *vf == 'F', 2, "colw3Dmul : Input image is smaller than kernel");
...
/*
  ...
  y <- y.*x + beta*y
*/
void THTensor_(colw3Dcmul)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_,
                           long sdepth, long srow, long scol, const char *vf, const char *xc)
{
  long nInputPlane, nInputDepth, nInputRows, nInputCols;
  long nKernelDepth, nKernelRows, nKernelCols;
  ...
  THArgCheck(nOutputPlane == nInputPlane, 2, "invalid number of input/kernel planes");
  THArgCheck( (nInputDepth >= nKernelDepth && nInputRows >= nKernelRows && nInputCols >= nKernelCols) || *vf == 'F', 2, "colw3Dcmul : Input image is smaller than kernel");
...
/*
  ...
  y <- y.*x + beta*y
*/
void THTensor_(colw3Dmap)(THTensor *r_, real beta, real alpha, THTensor *t_, THTensor *k_, THTensor *map,
                          long sdepth, long srow, long scol, const char *vf, const char *xc)
{
  long nInputPlane, nInputDepth, nInputRows, nInputCols;
  long nKernelDepth, nKernelRows, nKernelCols;
  ...
  THArgCheck((nInputDepth >= nKernelDepth
             ...
             2, "colw3Dmap : Input image is smaller than kernel");
