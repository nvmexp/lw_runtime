...
# The elwirinment set by create-ubuntu-elwironmens sscript is trusty, so we should match it.
...
#setup_lua_elw_cmd=$(luarocks path -bin)
#echo "Running: $setup_lua_elw_cmd"
#eval "$setup_lua_elw_cmd"


gbp buildpackage --git-pbuilder --git-dist=lwpu-8-0 --git-no-create-orig $1 $2 $3 $4 $5 > ../torch-buildpackage.log 2>&1 &
