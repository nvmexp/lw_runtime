...
local lwtils = require 'treplutils'
...
if isWindows() then
  hasTPut = sys.fexelwte('where tput'):find('tput')
end

if not hasTPut or not lwtils.isatty() then
   noColors()
else
...
-- This is a new relwrsive, colored print.
local ndepth = 4
function print_new(...)
   ...
   local objs = {...}
   local function printrelwrsive(obj,depth)
         ...
         for k,v in pairs(obj) do
            if type(v) == 'table' then
               if depth >= (ndepth-1) or next(v) == nil then
                  ...
                  line(tostring(k) .. ' : ') printrelwrsive(v,depth+1)
               end
            else
      ...
      __newindex = function(G,key,val)
         if not evercreated[key] then
            if cb then
               ...
               local file = debug.getinfo(2).source:gsub('^@','')
               local line = debug.getinfo(2).lwrrentline
               local report = print
               if strict then
                  report = error
               end
...
if readline_ok and (os.getelw('TREPL_HISTORY') or os.getelw('HOME') or os.getelw('USERPROFILE')) ~= nil then
   -- Readline found:
   local history = os.getelw('TREPL_HISTORY') or ((os.getelw('HOME') or os.getelw('USERPROFILE')) .. '/.luahistory')
   readline.setup()
   ...
   while true do
	 ...
	 elseif dok_loaded_ok then
      ...
      if line then
         ...
         -- If not done exelwting, execute normally:
         if not done then
