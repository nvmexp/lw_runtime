...
static unsigned int partition (lua_State *L, unsigned int lo,
  ...
  /* loop ilwariant: a[lo .. i] <= P <= a[j .. up] */
  for (;;) {
    ...
    /* otherwise, swap a[i] - a[j] to restore ilwariant and repeat */
    set2(L, i, j);
  }
}
...
/*
** QuickSort algorithm (relwrsive function)
*/
static void auxsort (lua_State *L, unsigned int lo, unsigned int up,
                                   unsigned int rnd) {
  while (lo < up) {  /* loop for tail relwrsion */
    ...
    if (p - lo < up - p) {  /* lower interval is smaller? */
      auxsort(L, lo, p - 1, rnd);  /* call relwrsively for lower interval */
      n = p - lo;  /* size of smaller interval */
      lo = p + 1;  /* tail call for [p + 1 .. up] (upper interval) */
    }
    else {
      auxsort(L, p + 1, up, rnd);  /* call relwrsively for upper interval */
