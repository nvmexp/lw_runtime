...
-- @param elwtable optional table of temporary environment variables
local function run(cmd, elwtable)
   cmd = expand_variables(cmd)
   local elw = {}
   for var, val in pairs(elwtable) do
      table.insert(elw, var.."='"..expand_variables(val).."' ")
   end
   local code = os.execute(table.concat(elw)..cmd)
   return (code == 0 or code == true)
end

local function cd_run(dir, cmd, elwtable)
   return run("cd "..dir.." && "..cmd, elwtable)
end
...
local tests = {
   ...
   fail_build_ilwalid = function() return run "$luarocks build invalid" end,
   fail_download_ilwalid = function() return run "$luarocks download invalid" end,
   fail_install_ilwalid = function() return run "$luarocks install invalid" end,
   fail_lint_ilwalid = function() return run "$luarocks lint invalid" end,
   fail_show_ilwalid = function() return run "$luarocks show invalid" end,
   fail_new_version_ilwalid = function() return run "$luarocks new_version invalid" end,
   test_list_ilwalidtree = function() return run "$luarocks --tree=/some/invalid/tree list" end,
   ...
   fail_build_ilwalidpatch = function()
      need_luasocket()
      return run '$luarocks build "$testing_dir/testfiles/ilwalid_patch-0.1-1.rockspec"'
   end,
   ...
   fail_help_ilwalid = function() return run "$luarocks help invalid" end,
   ...
   fail_install_ilwalidpatch = function()
      need_luasocket()
      return run '$luarocks install "$testing_dir/testfiles/ilwalid_patch-0.1-1.rockspec"'
   end,
   fail_install_ilwalid_filename = function() return run '$luarocks install "invalid.rock"' end,
   fail_install_ilwalid_arch = function() return run '$luarocks install "foo-1.0-1.impossible-x86.rock"' end,
   ...
   fail_remove_ilwalid_name = function() return run "$luarocks remove invalid.rock" end,
   ...
   fail_unpack_ilwalidpatch = function() 
      need_luasocket()
      return run '$luarocks unpack "$testing_dir/testfiles/ilwalid_patch-0.1-1.rockspec"'
   end,
   fail_unpack_ilwalidrockspec = function()
      need_luasocket()
      return run '$luarocks unpack "invalid.rockspec"'
   end,
   fail_upload_ilwalidrockspec = function() return run '$luarocks upload "invalid.rockspec"' end,
   fail_upload_ilwalidkey = function() return run '$luarocks upload --api-key="invalid" "invalid.rockspec"' end,
   ...
   fail_admin_ilwalidserver = function() return run '$luarocks_admin --server=invalid add "$testing_server/luasocket-${verrev_luasocket}.src.rock"' end,
   fail_admin_ilwalidrock = function() return run "$luarocks_admin --server=testing add invalid" end,
   ...
   fail_deps_mode_ilwalid_arg = function() return run "$luarocks remove luacov --deps-mode" end,
   ...
   fail_config_system_config_ilwalid = function()
      local ok = mkdir "$testing_lrprefix/etc/luarocks"
