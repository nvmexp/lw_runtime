...
fail_build_ilwalid() { $luarocks build invalid; }
fail_download_ilwalid() { $luarocks download invalid; }
fail_install_ilwalid() { $luarocks install invalid; }
fail_lint_ilwalid() { $luarocks lint invalid; }
fail_show_ilwalid() { $luarocks show invalid; }
fail_new_version_ilwalid() { $luarocks new_version invalid; }

test_list_ilwalidtree() { $luarocks --tree=/some/invalid/tree list; }
...
fail_build_ilwalidpatch() { need_luasocket; $luarocks build "$testing_dir/testfiles/ilwalid_patch-0.1-1.rockspec"; }
...
fail_help_ilwalid() { $luarocks help invalid; }
...
fail_install_ilwalidpatch() { need_luasocket; $luarocks install "$testing_dir/testfiles/ilwalid_patch-0.1-1.rockspec"; }
fail_install_ilwalid_filename() { $luarocks install "invalid.rock"; }
fail_install_ilwalid_arch() { $luarocks install "foo-1.0-1.impossible-x86.rock"; }
...
fail_remove_ilwalid_name() { $luarocks remove invalid.rock; }
...
fail_unpack_ilwalidpatch() { need_luasocket; $luarocks unpack "$testing_dir/testfiles/ilwalid_patch-0.1-1.rockspec"; }
fail_unpack_ilwalidrockspec() { need_luasocket; $luarocks unpack "invalid.rockspec"; }

fail_upload_ilwalidrockspec() { $luarocks upload "invalid.rockspec"; }
fail_upload_ilwalidkey() { $luarocks upload --api-key="invalid" "invalid.rockspec"; }
...
fail_admin_ilwalidserver() { $luarocks_admin --server=invalid add "$testing_server/luasocket-${verrev_luasocket}.src.rock"; }
fail_admin_ilwalidrock() { $luarocks_admin --server=testing add invalid; }
...
fail_deps_mode_ilwalid_arg() { $luarocks remove luacov --deps-mode; }
...
fail_config_system_config_ilwalid() { mkdir -p "$testing_lrprefix/etc/luarocks"; echo "if if if" > "$testing_lrprefix/etc/luarocks/config.lua"; $luarocks config --system-config; err=$?; rm -f "$testing_lrprefix/etc/luarocks/config.lua"; return $err; }
...
run_with_minimal_elwironment() {
   echo "==========================================="
   echo "Running with minimal environment"
   echo "==========================================="
   build_elwironment luacov
   run_tests $1
}

run_with_full_elwironment() {
   echo "==========================================="
   ...
   build_elwironment luacov luafilesystem luasocket $bitop luaposix md5 lzlib
   run_tests $1
}

run_all_tests() {
   run_with_minimal_elwironment $1
   run_with_full_elwironment $1
}
...
then
   if [ "$TRAVIS" ]
   then
      build_elwironment luacov luafilesystem luacov-coveralls
