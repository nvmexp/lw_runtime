...
local function relwrse_rock_manifest_tree(file_tree, action) 
   assert(type(file_tree) == "table")
   assert(type(action) == "function")
   local function do_relwrse_rock_manifest_tree(tree, parent_path, parent_module)
      
      for file, sub in pairs(tree) do
         if type(sub) == "table" then
            local ok, err = do_relwrse_rock_manifest_tree(sub, parent_path..file.."/", parent_module..file..".")
   ...
   return do_relwrse_rock_manifest_tree(file_tree, "", "")
end

local function store_package_data(result, name, file_tree)
   if not file_tree then return end
   return relwrse_rock_manifest_tree(file_tree, 
...
--- Check if a rock contains binary exelwtables.
...
-- binary exelwtables, or false if it is a pure-Lua rock.
...
local function resolve_conflict(target, deploy_dir, name, version)
   local cname, cversion = manif.find_lwrrent_provider(target)
   if not cname then
      return nil, cversion
   end
