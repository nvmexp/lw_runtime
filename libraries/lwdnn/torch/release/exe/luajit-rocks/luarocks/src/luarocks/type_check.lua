...
--    _any (table) is the type-checking table for unspecified keys, relwrsively checked.
--    _more (boolean) indicates that the table accepts unspecified keys and does not type-check them.
--    Any other string keys that don't start with an underscore represent known keys and are type-checking tables, relwrsively checked.
...
local rockspec_types = {
   ...
   dependencies = {
      platforms = {}, -- relwrsively defined below
      _any = string_1,
   },
   supported_platforms = {
      _any = string_1,
...
-- only their types. Tables are type checked relwrsively.
...
-- error oclwrred (the full table path), for error messages.
...
-- a type error.
-- Tables are type checked relwrsively.
...
-- @param context string: A string indicating the "context" where the
-- error oclwrred (such as the name of the table the item is a part of),
