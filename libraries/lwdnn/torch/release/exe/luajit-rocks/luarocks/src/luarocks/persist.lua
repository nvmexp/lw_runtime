...
-- @param elw table: the environment table.
...
local function run_file(filename, elw)
   ...
   if _VERSION == "Lua 5.1" then -- Lua 5.1
      ...
      chunk, err = load(str, filename, "t", elw)
      if chunk then
         ran, err = pcall(chunk)
      end
   end
...
-- This function handles only numbers and strings, ilwoking write_table
-- to write tables.
...
--- Write a table as Lua code in lwrly brackets notation to a writer object.
-- Only numbers, strings and tables (containing numbers, strings
-- or other relwrsively processed tables) are supported.
-- @param out table or userdata: a writer object supporting :write() method.
