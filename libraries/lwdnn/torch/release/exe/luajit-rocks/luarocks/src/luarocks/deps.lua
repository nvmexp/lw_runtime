...
-- (e.g. "2.0beta3"). Internally, LuaRocks colwerts them to a purely
...
local operators = {
   ...
   ["<="] = "<=",
   ["~>"] = "~>",
   -- plus some colwenience translations
   [""] = "==",
   ["="] = "==",
   ["!="] = "~="
}
...
--- Parse a version string, colwerting to table format.
-- A version table contains all components of the version string
-- colwerted to numeric format, stored in the array part of the table.
...
--- Consumes a constraint from a string, colwerting it to table format.
-- For example, a string ">= 1.0, > 2.0" is colwerted to a table in the
...
--- Colwert a list of constraints from string to table format.
-- For example, a string ">= 1.0, < 2.0" is colwerted to a table in the format
...
--- Colwert a dependency from string to table format.
-- For example, a string "foo >= 1.0, < 2.0"
-- is colwerted to a table in the format
...
--- Colwert a version table to a string.
...
--- Colwert a dependency in table format to a string.
...
-- @return boolean or (nil, string, [string]): True if no errors oclwrred, or
...
function deps.fulfill_dependencies(rockspec, deps_mode)
   ...
   if next(missing) then
      ...
      for _, dep in pairs(missing) do
         -- Double-check in case dependency was filled during relwrsion.
...
-- Run through the patterns array replacing all oclwrrences of "?"
...
-- @return boolean or (nil, string): True if no errors oclwrred, or
...
--- Relwrsively scan dependencies, to build a transitive closure of all
-- dependent packages.
-- @param results table: The results table being built.
-- @param missing table: The table of missing dependencies being relwrsively built.
