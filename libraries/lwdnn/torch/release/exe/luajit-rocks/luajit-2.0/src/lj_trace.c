...
static GCtrace *trace_save_alloc(jit_State *J)
{
  ...
  size_t sz = sztr + szins +
	      J->lwr.nsnap*sizeof(SnapShot) +
	      J->lwr.nsnapmap*sizeof(SnapEntry);
...
static void trace_save(jit_State *J, GCtrace *T)
{
  ...
  newwhite(J2G(J), T);
  T->gct = ~LJ_TTRACE;
  T->ir = (IRIns *)p - J->lwr.nk;
  memcpy(p, J->lwr.ir+J->lwr.nk, szins);
  p += szins;
...
static int trace_abort(jit_State *J)
{
  ...
  if (J->parent == 0 && !bc_isret(bc_op(J->lwr.startins)))
    penalty_pc(J, &gcref(J->lwr.startpt)->pt, mref(J->lwr.startpc, BCIns), e);
