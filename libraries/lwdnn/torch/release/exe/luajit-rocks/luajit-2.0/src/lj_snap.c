...
static MSize snapshot_slots(jit_State *J, SnapEntry *map, BCReg nslots)
{
  ...
  for (s = 0; s < nslots; s++) {
	...
	if (!(LJ_DUALNUM && (ir->op2 & IRSLOAD_COLWERT)) &&
...
static void snapshot_stack(jit_State *J, SnapShot *snap, MSize nsnapmap)
{
  ...
  snap->count = 0;
  J->lwr.nsnapmap = (uint16_t)(nsnapmap + nent + 1 + J->framedepth);
}

/* Add or merge a snapshot. */
void lj_snap_add(jit_State *J)
{
  MSize nsnap = J->lwr.nsnap;
  MSize nsnapmap = J->lwr.nsnapmap;
  /* Merge if no ins. inbetween or if requested and no guard inbetween. */
  if (J->mergesnap ? !irt_isguard(J->guardemit) :
      (nsnap > 0 && J->lwr.snap[nsnap-1].ref == J->lwr.nins)) {
    if (nsnap == 1) {  /* But preserve snap #0 PC. */
      emitir_raw(IRT(IR_NOP, IRT_NIL), 0, 0);
      goto nomerge;
    }
    nsnapmap = J->lwr.snap[--nsnap].mapofs;
  } else {
  nomerge:
    lj_snap_grow_buf(J, nsnap+1);
    J->lwr.nsnap = (uint16_t)(nsnap+1);
  }
  J->mergesnap = 0;
  J->guardemit.irt = 0;
  snapshot_stack(J, &J->lwr.snap[nsnap], nsnapmap);
}
...
void lj_snap_shrink(jit_State *J)
{
  ...
  snap->nent = (uint8_t)m;
  nlim = J->lwr.nsnapmap - snap->mapofs - 1;
  while (n <= nlim) map[m++] = map[n++];  /* Move PC + frame links down. */
  J->lwr.nsnapmap = (uint16_t)(snap->mapofs + m);  /* Free up space in map. */
}
...
void lj_snap_replay(jit_State *J, GCtrace *T)
{
  ...
  if (pass23) {
	...
	lua_assert(ir->o == IR_COLW && ir->op2 == IRCOLW_NUM_INT);
	...
	if (LJ_HASFFI && ir->o == IR_CNEWI) {
	  ...
	  for (irs = ir+1; irs < irlast; irs++)
		...
		lua_assert(irc->o == IR_COLW && irc->op2 == IRCOLW_NUM_INT);
			     ...
			     IRRef ref, void *dst, CTSize sz)
{
  ...
  if (irref_isk(ref)) {
	...
	lua_assert(sz == 8 && ir->o == IR_COLW && ir->op2 == IRCOLW_NUM_INT);
...
const BCIns *lj_snap_restore(jit_State *J, void *exptr)
{
  ...
  if (LJ_UNLIKELY(L->base + snap->topslot >= tvref(L->maxstack))) {
    L->top = lwrr_topL(L);
    lj_state_growstack(L, snap->topslot - lwrr_proto(L)->framesize);
  }
