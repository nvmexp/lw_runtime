		       ...
		       void *svisnz)
{
  ...
  */
  ...
  case CCX(I, F):
    if (dt == IRT_CDATA || st == IRT_CDATA) goto err_nyi;
    sp = emitcolw(sp, dsize < 4 ? IRT_INT : dt, st, IRCOLW_TRUNC|IRCOLW_ANY);
    goto xstore;
  case CCX(I, P):
  case CCX(I, A):
...
static void crec_alloc(jit_State *J, RecordFFData *rd, CTypeID id)
{
  ...
  if (!sz || sz > 128 || (info & CTF_VLA) || ctype_align(info) > CT_MEMALIGN)
    lj_trace_err(J, LJ_TRERR_NYICOLW);  /* NYI: large/special allocations. */
  ...
  if (ctype_isptr(info) || (ctype_isinteger(info) && (sz == 4 || sz == 8))) {
	...
	lj_trace_err(J, LJ_TRERR_NYICOLW);  /* NYI: init array of aggregates. */
...
static TRef crec_arith_int64(jit_State *J, TRef *sp, CType **s, MMS mm)
{
  if (sp[0] && sp[1] && ctype_isnum(s[0]->info) && ctype_isnum(s[1]->info)) {
	...
	sp[i] = emitcolw(sp[i], dt, st, IRCOLW_TRUNC|IRCOLW_ANY);
...
static TRef crec_arith_ptr(jit_State *J, TRef *sp, CType **s, MMS mm)
{
  ...
  }
  {
    ...
    if (t == IRT_NUM || t == IRT_FLOAT)
      tr = emitcolw(tr, IRT_INTP, t, IRCOLW_TRUNC|IRCOLW_ANY);
    else if (!(t == IRT_I64 || t == IRT_U64))
