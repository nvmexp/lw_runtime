...
/*
** Rationale for the builtin string to number colwersion library:
**
...
**
** OTOH the builtin colwersion function is very compact. Even though it
...
** platforms. It correctly colwerts and rounds any input to a double.
** If this is not the case, please send a bug report -- but PLEASE verify
** that the implementation you're comparing to is not the lwlprit!
**
...
**
** Decimal to binary colwersion uses a fixed-length cirlwlar buffer in
...
** is in the proper range. Then the integer part is rounded and colwerted
** to a double which is finally rescaled to the result. Denormals need
** special treatment to prevent incorrect 'double rounding'.
*/

/* Definitions for cirlwlar decimal digit buffer (base 100 = 2 digits/byte). */
...
static void strscan_double(uint64_t x, TValue *o, int32_t ex2, int32_t neg)
{
  ...
  /* Colwert to double using a signed int64_t colwersion, then rescale. */
			      ...
			      int32_t ex2, int32_t neg, uint32_t dig)
{
  ...
  /* Reduce range then colwert to double. */
			      ...
			      int32_t ex10, int32_t neg, uint32_t dig)
{
  ...
  }
  {
    ...
    /* Collect integer part digits and colwert to rescaled double. */
    {
      uint64_t x = xi[hi];
      uint32_t i;
...
StrScanFmt lj_strscan_scan(const uint8_t *p, TValue *o, uint32_t opt)
{
  ...
  /* Parse regular number. */
  {
    ...
    /* Try to colwert number to integer, if requested. */
