...
/* C API checks. */
ERRDEF(NOELW,	"no calling environment")
...
ERRDEF(ILWLVL,	"invalid level")
ERRDEF(ILWOPT,	"invalid option")
ERRDEF(ILWOPTM,	"invalid option " LUA_QS)
ERRDEF(ILWFMT,	"invalid format")
ERRDEF(SETFELW,	LUA_QL("setfelw") " cannot change environment of given object")
...
#if LJ_HASFFI
/* FFI errors. */
ERRDEF(FFI_ILWTYPE,	"invalid C type")
ERRDEF(FFI_ILWSIZE,	"size of C type is unknown or too large")
...
ERRDEF(FFI_BADCOLW,	"cannot colwert " LUA_QS " to " LUA_QS)
