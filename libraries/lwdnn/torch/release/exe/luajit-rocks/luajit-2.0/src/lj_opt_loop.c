...
/* Loop optimization:
**
** Traditional Loop-Ilwariant Code Motion (LICM) splits the instructions
** of a loop into ilwariant and variant instructions. The ilwariant
...
** contains a mix of ilwariant and variant instructions and performs
...
** kinds of guards. The major advantage is that all ilwariant guards can
** be hoisted, too.
**
...
** become a forwarded loop-relwrrence after inlining.
**
			  ...
			  SnapNo onsnap)
{
  int passx = 0;
  IRRef i, j, nslots;
  IRRef ilwar = J->chain[IR_LOOP];
  /* Pass #1: mark redundant and potentially redundant PHIs. */
  for (i = 0, j = 0; i < nphi; i++) {
    IRRef lref = phi[i];
    IRRef rref = subst[lref];
    if (lref == rref || rref == REF_DROP) {  /* Ilwariants are redundant. */
      irt_clearphi(IR(lref)->t);
    } else {
      phi[j++] = (IRRef1)lref;
      if (!(IR(rref)->op1 == lref || IR(rref)->op2 == lref)) {
	/* Quick check for simple relwrrences failed, need pass2. */
  ...
  if (passx) {
    ...
    for (s = J->lwr.nsnap-1; s >= onsnap; s--) {
      SnapShot *snap = &J->lwr.snap[s];
      SnapEntry *map = &J->lwr.snapmap[snap->mapofs];
...
{
  SnapEntry *nmap, *omap = &J->lwr.snapmap[osnap->mapofs];
  SnapEntry *nextmap = &J->lwr.snapmap[snap_nextofs(&J->lwr, osnap)];
  MSize nmapofs;
  MSize on, ln, nn, onent = osnap->nent;
  BCReg nslots = osnap->nslots;
  SnapShot *snap = &J->lwr.snap[J->lwr.nsnap];
  if (irt_isguard(J->guardemit)) {  /* Guard inbetween? */
    nmapofs = J->lwr.nsnapmap;
    J->lwr.nsnap++;  /* Add new snapshot. */
  ...
  snap->ref = (IRRef1)J->lwr.nins;
  snap->nslots = nslots;
  snap->topslot = osnap->topslot;
  snap->count = 0;
  nmap = &J->lwr.snapmap[nmapofs];
  ...
  while (omap < nextmap)  /* Copy PC + frame links. */
    *nmap++ = *omap++;
  J->lwr.nsnapmap = (uint16_t)(nmap - J->lwr.snapmap);
}
...
static void loop_unroll(jit_State *J)
{
  ...
  ** Caveat: both calls may reallocate J->lwr.snap and J->lwr.snapmap!
  */
  onsnap = J->lwr.nsnap;
  lj_snap_grow_buf(J, 2*onsnap-2);
  lj_snap_grow_map(J, J->lwr.nsnapmap*2+(onsnap-2)*J->lwr.snap[onsnap-1].nent);

  /* The loop snapshot is used for fallback substitutions. */
  loopsnap = &J->lwr.snap[onsnap-1];
  loopmap = &J->lwr.snapmap[loopsnap->mapofs];
  /* The PC of snapshot #0 and the loop snapshot must match. */
  psentinel = &loopmap[loopsnap->nent];
  lua_assert(*psentinel == J->lwr.snapmap[J->lwr.snap[0].nent]);
  *psentinel = SNAP(255, 0, 0);  /* Replace PC with temporary sentinel. */
  ...
  for (ins = REF_FIRST; ins < ilwar; ins++) {
    IRIns *ir;
    IRRef op1, op2;
	...
	op1 == ir->op1 && op2 == ir->op2) {  /* Regular ilwariant ins? */
      subst[ins] = (IRRef1)ins;  /* Shortlwt. */
    } else {
      /* Re-emit substituted instruction to the FOLD/CSE/etc. pipeline. */
      IRType1 t = ir->t;  /* Get this first, since emitir may ilwalidate ir. */
      IRRef ref = tref_ref(emitir(ir->ot & ~IRT_ISPHI, op1, op2));
      subst[ins] = (IRRef1)ref;
      if (ref != ins) {
	IRIns *irr = IR(ref);
	if (ref < ilwar) {  /* Loop-carried dependency? */
	  /* Potential PHI? */
	  ...
	  if (!irt_sametype(t, irr->t)) {
	    if (irt_isinteger(t) && irt_isinteger(irr->t))
	      ...
	      ref = tref_ref(emitir(IRTN(IR_COLW), ref, IRCOLW_NUM_INT));
	    else if (irt_isnum(irr->t) && irt_isinteger(t))  /* Fix num->int. */
	      ref = tref_ref(emitir(IRTGI(IR_COLW), ref,
				    IRCOLW_INT_NUM|IRCOLW_CHECK));
	    else
	...
	} else if (ref != REF_DROP && irr->o == IR_COLW &&
		   ref > ilwar && irr->op1 < ilwar) {
	  /* May need an extra PHI for a COLW. */
	  ref = irr->op1;
	  irr = IR(ref);
	phicolw:
	  if (ref < ilwar && !irref_isk(ref) && !irt_isphi(irr->t)) {
	    irt_setphi(irr->t);
	    if (nphi >= LJ_MAX_PHI)
	      lj_trace_err(J, LJ_TRERR_PHIOV);
	    phi[nphi++] = (IRRef1)ref;
  ...
  if (!irt_isguard(J->guardemit))  /* Drop redundant snapshot. */
    J->lwr.nsnapmap = (uint16_t)J->lwr.snap[--J->lwr.nsnap].mapofs;
  lua_assert(J->lwr.nsnapmap <= J->sizesnapmap);
  *psentinel = J->lwr.snapmap[J->lwr.snap[0].nent];  /* Restore PC. */
...
{
  ptrdiff_t i;
  SnapShot *snap = &J->lwr.snap[nsnap-1];
  SnapEntry *map = J->lwr.snapmap;
  map[snap->mapofs + snap->nent] = map[J->lwr.snap[0].nent];  /* Restore PC. */
  J->lwr.nsnapmap = (uint16_t)nsnapmap;
  J->lwr.nsnap = nsnap;
  J->guardemit.irt = 0;
...
{
  IRRef nins = J->lwr.nins;
  SnapNo nsnap = J->lwr.nsnap;
  MSize nsnapmap = J->lwr.nsnapmap;
