...
/* This solves a cirlwlar dependency problem -- change FF_next_N as needed. */
...
LJLIB_CF(getfelw)
{
  ...
  if (!(o < L->top && tvisfunc(o))) {
    ...
    if (o == NULL)
      lj_err_arg(L, 1, LJ_ERR_ILWLVL);
  }
  fn = &gcval(o)->fn;
  settabV(L, L->top++, isluafunc(fn) ? tabref(fn->l.elw) : tabref(L->elw));
  return 1;
}
...
LJLIB_CF(setfelw)
{
  ...
  fn = &gcval(o)->fn;
  if (!isluafunc(fn))
    lj_err_caller(L, LJ_ERR_SETFELW);
  setgcref(fn->l.elw, obj2gco(t));
...
LJLIB_ASM(tonumber)		LJLIB_REC(.)
{
  ...
  if (base == 10) {
	...
	if (LJ_DUALNUM && ctype_isinteger_or_bool(ct->info) &&
	  ...
	  int32_t i;
	  lj_ccolw_ct_tv(cts, ctype_get(cts, CTID_INT32), (uint8_t *)&i, o, 0);
	  setintV(L->base-1, i);
	  return FFH_RES(1);
	}
	lj_ccolw_ct_tv(cts, ctype_get(cts, CTID_DOUBLE),
...
LJLIB_ASM(tostring)		LJLIB_REC(.)
{
  ...
  if (!tvisnil(mo = lj_meta_lookup(L, o, MM_tostring))) {
	...
	lua_pushfstring(L, "%s: %p", lj_typename(o), lua_topointer(L, 1));
      /* Note: lua_pushfstring calls the GC which may ilwalidate o. */
      s = strV(L->top-1);
    }
...
static int load_aux(lua_State *L, int status, int elwarg)
{
  if (status == 0) {
    if (tvistab(L->base+elwarg-1)) {
      GCfunc *fn = funcV(L->top-1);
      GCtab *t = tabV(L->base+elwarg-1);
      setgcref(fn->c.elw, obj2gco(t));
...
LJLIB_CF(loadfile)
{
  ...
  int status;
  lua_settop(L, 3);  /* Ensure elw arg exists. */
...
LJLIB_CF(load)
{
  ...
  if (L->base < L->top && (tvisstr(L->base) || tvisnumber(L->base))) {
    ...
    lua_settop(L, 4);  /* Ensure elw arg exists. */
...
LJLIB_CF(print)
{
  ...
  cTValue *tv = lj_tab_getstr(tabref(L->elw), strV(lj_lib_upvalue(L, 1)));
  int shortlwt;
  if (tv && !tvisnil(tv)) {
    copyTV(L, L->top++, tv);
  } else {
  ...
  shortlwt = (tvisfunc(tv) && funcV(tv)->c.ffid == FF_tostring);
  for (i = 0; i < nargs; i++) {
    const char *str;
    size_t size;
    cTValue *o = &L->base[i];
    if (shortlwt && tvisstr(o)) {
      str = strVdata(o);
      size = strV(o)->len;
    } else if (shortlwt && tvisint(o)) {
      char buf[LJ_STR_INTBUF];
      char *p = lj_str_bufint(buf, intV(o));
      size = (size_t)(buf+LJ_STR_INTBUF-p);
      str = p;
    } else if (shortlwt && tvisnum(o)) {
...
LUALIB_API int luaopen_base(lua_State *L)
{
  ...
  GCtab *elw = tabref(L->elw);
  settabV(L, lj_tab_setstr(L, elw, lj_str_newlit(L, "_G")), elw);
