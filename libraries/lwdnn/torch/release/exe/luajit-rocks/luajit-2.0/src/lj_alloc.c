...
**
** Beware: this is a HEAVILY LWSTOMIZED version of dlmalloc.
...
**
** No additional copyright is claimed over the lwstomizations.
...
/* Undolwmented, but hey, that's what we all love so much about Windows. */
...
static LJ_AINLINE void *CALL_MMAP(size_t size)
{
  ...
  void *ptr = NULL;
  long st = ntavm(ILWALID_HANDLE_VALUE, &ptr, NTAVM_ZEROBITS, &size,
...
static LJ_AINLINE void *CALL_MMAP(size_t size)
{
  ...
  if (LJ_UNLIKELY(rlimit_modified == 0)) {
    struct rlimit rlim;
    rlim.rlim_lwr = rlim.rlim_max = MMAP_REGION_START;
