...
static void read_string(LexState *ls, int delim, TValue *tv)
{
  ...
  while (ls->current != delim) {
	...
	while (lj_char_isspace(ls->current))
	  if (lwrrIsNewline(ls)) inclinenumber(ls); else next(ls);
	continue;
...
static int llex(LexState *ls, TValue *tv)
{
  ...
  for (;;) {
	...
	if (sep >= 0) {
      ...
      /* else short comment */
      while (!lwrrIsNewline(ls) && ls->current != END_OF_STREAM)
	next(ls);
      continue;
    case '[': {
      int sep = skip_sep(ls);
...
int lj_lex_setup(lua_State *L, LexState *ls)
{
  ...
  if (ls->current == LUA_SIGNATURE[0]) {  /* Bytecode dump. */
    if (header) {
      ...
      ** reasons. This may cirlwmvent the usual check for bytecode vs.
...
void lj_lex_next(LexState *ls)
{
  ...
  if (LJ_LIKELY(ls->lookahead == TK_eof)) {  /* No lookahead token? */
    ls->token = llex(ls, &ls->tokelwal);  /* Get next token. */
  } else {  /* Otherwise return lookahead token. */
    ls->token = ls->lookahead;
    ls->lookahead = TK_eof;
    ls->tokelwal = ls->lookaheadval;
  }
}
