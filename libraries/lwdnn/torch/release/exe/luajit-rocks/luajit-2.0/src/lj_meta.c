		    ...
		    cTValue *a, cTValue *b)
{
  ...
  */
  TValue *top = L->top;
  if (lwrr_funcisL(L)) top = lwrr_topL(L);
...
TValue *lj_meta_tset(lua_State *L, cTValue *o, cTValue *k)
{
  ...
  for (loop = 0; loop < LJ_MAX_IDXCHAIN; loop++) {
	...
	t->nomm = 0;  /* Ilwalidate negative metamethod cache. */
...
TValue *lj_meta_cat(lua_State *L, TValue *top, int left)
{
  ...
  do {
    ...
    } else if (strV(top)->len == 0) {  /* Shortlwt. */
...
{
  /* Field metatable must be at same offset for GCtab and GLwdata! */
...
TValue * LJ_FASTCALL lj_meta_equal_cd(lua_State *L, BCIns ins)
{
  ...
  if (op == BC_ISEQV) {
    ...
    setstrV(L, &tv, gco2str(proto_kgc(lwrr_proto(L), ~(ptrdiff_t)bc_d(ins))));
    o2 = &tv;
  } else if (op == BC_ISEQN) {
    o2 = &mref(lwrr_proto(L)->k, cTValue)[bc_d(ins)];
  } else {
