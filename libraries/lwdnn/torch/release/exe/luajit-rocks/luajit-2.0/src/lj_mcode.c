...
#if LJ_TARGET_IOS
void sys_icache_ilwalidate(void *start, size_t len);
#endif
...
#elif LJ_TARGET_IOS
  sys_icache_ilwalidate(start, (char *)end-(char *)start);
#elif LJ_TARGET_PPC
...
#define MCPROT_RX	PAGE_EXELWTE_READ
#define MCPROT_RWX	PAGE_EXELWTE_READWRITE
...
#error "Missing OS support for explicit placement of exelwtable memory"

#else

/* Fallback allocator. This will fail if memory is not exelwtable by default. */
...
**
** Most of the time the memory pages holding machine code are exelwtable,
** but NONE of them is writable.
**
** The current memory area is marked read-write (but NOT exelwtable) only
...
{
#ifdef __OpenBSD__
  /* Allow better exelwtable memory allocation for OpenBSD W^X mode. */
