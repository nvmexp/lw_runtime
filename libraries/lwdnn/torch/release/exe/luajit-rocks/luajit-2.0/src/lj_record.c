...
static void rec_loop_interp(jit_State *J, const BCIns *pc, LoopEvent ev)
{
  if (J->parent == 0) {
	...
	lj_trace_err(J, LJ_TRERR_LLEAVE);
      rec_stop(J, LJ_TRLINK_LOOP, J->lwr.traceno);  /* Looping root trace. */
...
static void rec_loop_jit(jit_State *J, TraceNo lnk, LoopEvent ev)
{
  if (J->parent == 0) {  /* Root trace hit an inner loop. */
    ...
    if (J->pc == J->startpc && J->framedepth + J->retdepth == 0)
      rec_stop(J, LJ_TRLINK_LOOP, J->lwr.traceno);  /* Form an extra loop. */
    else
...
void lj_record_ret(jit_State *J, BCReg rbase, ptrdiff_t gotresults)
{
  ...
  if (J->framedepth == 0 && J->pt && bc_isret(bc_op(*J->pc)) &&
       (!frame_islua(frame) ||
	(J->parent == 0 && !bc_isret(bc_op(J->lwr.startins))))) {
  ...
  if (frame_islua(frame)) {  /* Return to Lua frame. */
	...
	lj_snap_purge(J);
	rec_stop(J, LJ_TRLINK_DOWNREC, J->lwr.traceno);  /* Down-relwrsion. */
	return;
      }
      lj_snap_add(J);
    }
    ...
    if (J->framedepth > 0) {  /* Return to a frame that is part of the trace. */
      ...
      J->baseslot -= cbase+1;
      J->base -= cbase+1;
    } else if (J->parent == 0 && !bc_isret(bc_op(J->lwr.startins))) {
...
{
  BCReg s, top = lwrr_proto(J->L)->framesize;
  TRef trcont;
...
static void check_call_unroll(jit_State *J, TraceNo lnk)
{
  ...
  if (J->pc == J->startpc) {
	...
	rec_stop(J, LJ_TRLINK_TAILREC, J->lwr.traceno);  /* Tail-relwrsion. */
      else
	rec_stop(J, LJ_TRLINK_UPREC, J->lwr.traceno);  /* Up-relwrsion. */
    }
  } else {
...
static void rec_func_jit(jit_State *J, TraceNo lnk)
{
  ...
  if (J->pc == J->startpc && J->framedepth + J->retdepth == 0)
    rec_stop(J, LJ_TRLINK_TAILREC, J->lwr.traceno);  /* Extra tail-relwrsion. */
  else
...
static void rec_varg(jit_State *J, BCReg dst, ptrdiff_t nresults)
{
  ...
  ptrdiff_t lwararg = frame_delta(J->L->base-1) - numparams - 1;
  ...
  if (J->framedepth > 0) {  /* Simple case: varargs defined on-trace. */
    ...
    for (i = 0; i < nresults; i++)
      J->base[dst+i] = i < lwararg ? getslot(J, i - lwararg - 1) : TREF_NIL;
	...
	for (i = 0; i < nload; i++) {
	  IRType t = itype2irt(&J->L->base[i-1-lwararg]);
	...
	vbase = emitir(IRT(IR_ADD, IRT_P32), vbase, lj_ir_kint(J, frofs-8));
	t = itype2irt(&J->L->base[idx-2-lwararg]);
...
void lj_record_setup(jit_State *J)
{
  ...
  J->baseslot = 1;  /* Ilwoking function is at base[-1]. */
