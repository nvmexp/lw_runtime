...
static void asm_uref(ASMState *as, IRIns *ir)
{
  ...
  if (irref_isk(ir->op1)) {
    ...
    if (ir->o == IR_UREFC) {
      ...
      emit_tsi(as, MIPSI_ADDIU, dest, uv, (int32_t)offsetof(GLwpval, tv));
      emit_tsi(as, MIPSI_LBU, RID_TMP, uv, (int32_t)offsetof(GLwpval, closed));
    } else {
      emit_tsi(as, MIPSI_LW, dest, uv, (int32_t)offsetof(GLwpval, v));
    }
...
static void asm_sload(ASMState *as, IRIns *ir)
{
  ...
  lua_assert(!irt_isint(t) || (ir->op2 & (IRSLOAD_COLWERT|IRSLOAD_FRAME)));
