...
{
  const char *path;
  lua_getfield(L, LUA_ELWIRONINDEX, pname);
...
{
  const char *name = luaL_checkstring(L, 1);
  lua_getfield(L, LUA_ELWIRONINDEX, "preload");
  if (!lua_istable(L, -1))
...
static int lj_cf_package_require(lua_State *L)
{
  ...
  lua_getfield(L, LUA_ELWIRONINDEX, "loaders");
  if (!lua_istable(L, -1))
    luaL_error(L, LUA_QL("package.loaders") " must be a table");
  lua_pushliteral(L, "");  /* error message aclwmulator */
  for (i = 1; ; i++) {
...
static void setfelw(lua_State *L)
{
  lua_Debug ar;
...
static void setpath(lua_State *L, const char *fieldname, const char *elwname,
		    const char *def, int noelw)
{
#if LJ_TARGET_CONSOLE
  const char *path = NULL;
  UNUSED(elwname);
#else
  const char *path = getelw(elwname);
#endif
  if (path == NULL || noelw) {
...
LUALIB_API int luaopen_package(lua_State *L)
{
  ...
  lua_getfield(L, LUA_REGISTRYINDEX, "LUA_NOELW");
  noelw = lua_toboolean(L, -1);
  lua_pop(L, 1);
  setpath(L, "path", LUA_PATH, LUA_PATH_DEFAULT, noelw);
  setpath(L, "cpath", LUA_CPATH, LUA_CPATH_DEFAULT, noelw);
