...
** because rematerializations may ilwalidate the flags.
*/
static void asm_guardcc(ASMState *as, int cc)
{
  MCode *target = exitstub_addr(as->J, as->snapno);
  MCode *p = as->mcp;
  if (LJ_UNLIKELY(p == as->ilwmcp)) {
    as->loopilw = 1;
...
/* Check if there's no conflicting instruction between lwrins and ref.
...
static void asm_fuseahuref(ASMState *as, IRRef ref, RegSet allow)
{
  ...
  if (ra_noreg(ir->r)) {
	...
	GLwpval *uv = &gcref(fn->l.uvptr[(ir->op2 >> 8)])->uv;
...
static Reg asm_fuseload(ASMState *as, IRRef ref, RegSet allow)
{
  ...
  if (ir->o == IR_KNUM) {
    ...
    if (ir->o == IR_SLOAD) {
      if (!(ir->op2 & (IRSLOAD_PARENT|IRSLOAD_COLWERT)) &&
...
static void asm_colw(ASMState *as, IRIns *ir)
{
  IRType st = (IRType)(ir->op2 & IRCOLW_SRCMASK);
  ...
  if (irt_isfp(ir->t)) {
    ...
    if (stfp) {  /* FP to FP colwersion. */
      Reg left = asm_fuseload(as, lref, RSET_FPR);
      emit_mrm(as, st == IRT_NUM ? XO_CVTSD2SS : XO_CVTSS2SD, dest, left);
      if (left == dest) return;  /* Avoid the XO_XORPS. */
    } else if (LJ_32 && st == IRT_U32) {  /* U32 to FP colwersion on x86. */
  ...
  } else if (stfp) {  /* FP to integer colwersion. */
    if (irt_isguard(ir->t)) {
      /* Checked colwersions are only supported from number to int. */
      lua_assert(irt_isint(ir->t) && st == IRT_NUM);
      asm_tointg(as, ir, ra_alloc1(as, lref, RSET_FPR));
    } else {
      Reg dest = ra_dest(as, ir, RSET_GPR);
      x86Op op = st == IRT_NUM ?
		 ((ir->op2 & IRCOLW_TRUNC) ? XO_CVTTSD2SI : XO_CVTSD2SI) :
		 ((ir->op2 & IRCOLW_TRUNC) ? XO_CVTTSS2SI : XO_CVTSS2SI);
      if (LJ_64 ? irt_isu64(ir->t) : irt_isu32(ir->t)) {
	/* LJ_64: For inputs >= 2^63 add -2^64, colwert again. */
	/* LJ_32: For inputs >= 2^31 add -2^31, colwert again and add 2^31. */
...
#if LJ_32 && LJ_HASFFI
/* No SSE colwersions to/from 64 bit on x86, so resort to ugly x87 code. */

/* 64 bit integer to FP colwersion in 32 bit mode. */
static void asm_colw_fp_int64(ASMState *as, IRIns *ir)
{
  ...
  emit_rmro(as, irt_isnum(ir->t) ? XO_FSTPq : XO_FSTPd,
	    irt_isnum(ir->t) ? XOg_FSTPq : XOg_FSTPd, RID_ESP, ofs);
  if (((ir-1)->op2 & IRCOLW_SRCMASK) == IRT_U64) {
    ...
    lua_assert(((ir-1)->op2 & IRCOLW_SRCMASK) == IRT_I64);
  }
...
/* FP to 64 bit integer colwersion in 32 bit mode. */
static void asm_colw_int64_fp(ASMState *as, IRIns *ir)
{
  ...
  lua_assert(((ir-1)->op2 & IRCOLW_TRUNC));
  ...
  if (dt == IRT_U64) {
    /* For inputs in [2^63,2^64-1] add -2^64 and colwert again. */
...
static MCode *merge_href_niltv(ASMState *as, IRIns *ir)
{
  ...
  if ((ir[1].o == IR_NE || ir[1].o == IR_EQ) && ir[1].op1 == as->lwrins &&
      ra_hasreg(ir->r)) {
    MCode *p = as->mcp;
    p += (LJ_64 && *p != XI_ARITHi) ? 7+6 : 6+6;
    /* Ensure no loop branch ilwersion happened. */
...
static void asm_uref(ASMState *as, IRIns *ir)
{
  ...
  if (irref_isk(ir->op1)) {
    ...
    if (ir->o == IR_UREFC) {
      emit_rmro(as, XO_LEA, dest, uv, offsetof(GLwpval, tv));
      asm_guardcc(as, CC_NE);
      emit_i8(as, 1);
      emit_rmro(as, XO_ARITHib, XOg_CMP, uv, offsetof(GLwpval, closed));
    } else {
      emit_rmro(as, XO_MOV, dest, uv, offsetof(GLwpval, v));
    }
...
static void asm_sload(ASMState *as, IRIns *ir)
{
  ...
  lua_assert(LJ_DUALNUM ||
	     !irt_isint(t) || (ir->op2 & (IRSLOAD_COLWERT|IRSLOAD_FRAME)));
  if ((ir->op2 & IRSLOAD_COLWERT) && irt_isguard(t) && irt_isint(t)) {
...
static void asm_cnew(ASMState *as, IRIns *ir)
{
  ...
  emit_opgl(as, XO_MOVZXb, RID_ECX, gc.lwrrentwhite);
...
static void asm_obar(ASMState *as, IRIns *ir)
{
  ...
  emit_rmro(as, XO_GROUP3b, XOg_TEST, obj,
	    (int32_t)offsetof(GLwpval, marked)-(int32_t)offsetof(GLwpval, tv));
}
...
static void asm_x87load(ASMState *as, IRRef ref)
{
  ...
  } else if (ir->o == IR_COLW && ir->op2 == IRCOLW_NUM_INT && !ra_used(ir) &&
...
static int asm_swapops(ASMState *as, IRIns *ir)
{
  ...
  if (as->lwrins > as->loopref) {  /* In variant part? */
    if (ir->op2 < as->loopref && !irt_isphi(irr->t))
      return 0;  /* Keep ilwariants on the right. */
    if (ir->op1 < as->loopref && !irt_isphi(irl->t))
      return 1;  /* Swap ilwariants to the right. */
  }
...
** instructions.
**
** Lwrrently only a few common cases are supported:
...
#define COMPFLAGS(ci, cin, lw, cf)	((ci)+((lw)<<4)+((cin)<<8)+(cf))
...
static void asm_comp(ASMState *as, IRIns *ir, uint32_t cc)
{
  if (irt_isnum(ir->t)) {
    ...
    ** the operands and ilwerting the condition (except for EQ and UNE).
    ...
    */
	...
	if (!asm_isk32(as, irl->op2, &imm)) {
	  ...
	  if (opisfusableload((IROp)irll->o) &&
	    ...
	    as->lwrins--;  /* Skip to BAND to avoid failing in noconflict(). */
...
static void asm_comp_int64(ASMState *as, IRIns *ir)
{
  ...
  as->lwrins--;  /* Skip loword ins. Avoids failing in noconflict(), too. */
  ...
  l_around = emit_label(as);
  as->ilwmcp = as->flagmcp = NULL;  /* Cannot use these optimizations. */
...
static void asm_hiop(ASMState *as, IRIns *ir)
{
  ...
  if ((ir-1)->o == IR_COLW) {  /* Colwersions to/from 64 bit. */
    if (usehi || uselo) {
      if (irt_isfp(ir->t))
	asm_colw_fp_int64(as, ir);
      else
	asm_colw_int64_fp(as, ir);
    }
    as->lwrins--;  /* Always skip the COLW. */
    return;
...
static void asm_loop_fixup(ASMState *as)
{
  ...
  if (as->realign) {  /* Realigned loops use short jumps. */
    ...
    if (as->loopilw) {  /* Ilwerted loop branch? */
      p -= 5;
      p[0] = XI_JMP;
    ...
    p[-5] = XI_JMP;
    if (as->loopilw) {  /* Ilwerted loop branch? */
      /* asm_guardcc already ilwerted the jcc and patched the jmp. */
      p -= 5;
      newloop = target+4;
	...
	*(int32_t *)(p-10) = (int32_t)(target - p + 6);  /* Patch opt. jp. */
    ...
    if (newloop >= p - 128) {
      ...
      as->lwrins = as->stopins;  /* Abort asm_trace now. */
...
static void asm_tail_prep(ASMState *as)
{
  ...
  if (as->realign) {
    ...
    while (i-- > 0)
      *--p = XI_NOP;
    as->mctop = p;
    p -= (as->loopilw ? 5 : 2);  /* Space for short/near jmp. */
  } else {
