...
#endif
  IRRef lref = ir->op1;
  /* 64 bit integer colwersions are handled by SPLIT. */
  ...
  if (irref_isk(ir->op1)) {
    ...
    if (ir->o == IR_UREFC) {
      ...
      emit_opk(as, ARMI_ADD, dest, uv,
	       (int32_t)offsetof(GLwpval, tv), RSET_GPR);
      emit_lso(as, ARMI_LDRB, RID_TMP, uv, (int32_t)offsetof(GLwpval, closed));
    } else {
      emit_lso(as, ARMI_LDR, dest, uv, (int32_t)offsetof(GLwpval, v));
    }
...
#else
  if ((ir->op2 & IRSLOAD_COLWERT) && irt_isguard(ir->t) && t == IRT_INT) {
...
static void asm_loop_fixup(ASMState *as)
{
  ...
  if (as->loopilw) {  /* Ilwerted loop branch? */
    /* asm_guardcc already ilwerted the bcc and patched the final bl. */
