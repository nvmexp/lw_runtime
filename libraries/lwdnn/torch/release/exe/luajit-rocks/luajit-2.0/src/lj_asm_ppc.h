...
static void asm_colw(ASMState *as, IRIns *ir)
{
  ...
  if (irt_isfp(ir->t)) {
    ...
    if (stfp) {  /* FP to FP colwersion. */
      if (st == IRT_NUM)  /* double -> float colwersion. */
	emit_fb(as, PPCI_FRSP, dest, ra_alloc1(as, lref, RSET_FPR));
      else  /* float -> double colwersion is a no-op on PPC. */
    ...
    if (irt_isguard(ir->t)) {
      ...
      if (irt_isu32(ir->t)) {
	/* Colwert both x and x-2^31 to int and merge results. */
...
static void asm_uref(ASMState *as, IRIns *ir)
{
  ...
  if (irref_isk(ir->op1)) {
    ...
    if (ir->o == IR_UREFC) {
      ...
      emit_tai(as, PPCI_ADDI, dest, uv, (int32_t)offsetof(GLwpval, tv));
      emit_tai(as, PPCI_LBZ, RID_TMP, uv, (int32_t)offsetof(GLwpval, closed));
    } else {
      emit_tai(as, PPCI_LWZ, dest, uv, (int32_t)offsetof(GLwpval, v));
    }
...
static void asm_loop_fixup(ASMState *as)
{
  ...
  if (as->loopilw) {  /* Ilwerted loop branch? */
    /* asm_guardcc already ilwerted the cond branch and patched the final b. */
...
void lj_asm_patchexit(jit_State *J, GCtrace *T, ExitNo exitno, MCode *target)
{
  ...
  if (clearso) {  /* Extend the current trace. Ugly workaround. */
    MCode *pp = J->lwr.mcode;
    J->lwr.szmcode += sizeof(MCode);
