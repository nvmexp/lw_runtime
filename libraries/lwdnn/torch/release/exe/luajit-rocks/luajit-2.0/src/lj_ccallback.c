...
/* Colwert callback slot number to callback function pointer. */
static void *callback_slot2ptr(CTState *cts, MSize slot)
{
  return (uint8_t *)cts->cb.mcode + CALLBACK_SLOT2OFS(slot);
}

/* Colwert callback function pointer to slot number. */
...
static void callback_mcode_new(CTState *cts)
{
  ...
  if (p == MAP_FAILED)
    lj_err_caller(cts->L, LJ_ERR_FFI_CBACKOV);
#else
  /* Fallback allocator. Fails if memory is not exelwtable by default. */
  ...
  {
    DWORD oprot;
    VirtualProtect(p, sz, PAGE_EXELWTE_READ, &oprot);
  }
#elif LJ_TARGET_POSIX
...
/* Colwert and push callback arguments to Lua stack. */
static void callback_colw_args(CTState *cts, lua_State *L)
{
  TValue *o = L->top;
...
#if LJ_TARGET_X86
  /* x86 has several different calling colwentions. */
  switch (ctype_ccolw(ct->info)) {
  ...
  while (fid) {
	...
	sp = (void *)((uint8_t *)sp + CTSIZE_PTR-cta->size);
      gcsteps += lj_ccolw_tv_ct(cts, cta, 0, o++, sp);
    }
    fid = ctf->sib;
  }
  L->top = o;
  ...
  if (ctype_ccolw(ct->info) != CTCC_CDECL)
...
/* Colwert Lua object to callback result. */
static void callback_colw_result(CTState *cts, lua_State *L, TValue *o)
{
...
void LJ_FASTCALL lj_ccallback_leave(CTState *cts, TValue *o)
{
  ...
  if (o >= L->base) {
    /* PC of RET* is lost. Point to last line for result colw. errors. */
    fn = lwrr_func(L);
    if (isluafunc(fn)) {
