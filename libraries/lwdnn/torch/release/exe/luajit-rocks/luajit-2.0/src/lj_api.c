...
static TValue *index2adr(lua_State *L, int idx)
{
  if (idx > 0) {
    ...
    TValue *o = &G(L)->tmptv;
    settabV(L, o, tabref(L->elw));
    return o;
  } else if (idx == LUA_REGISTRYINDEX) {
    return registry(L);
  } else {
    GCfunc *fn = lwrr_func(L);
    api_check(L, fn->c.gct == ~LJ_TFUNC && !isluafunc(fn));
    if (idx == LUA_ELWIRONINDEX) {
      TValue *o = &G(L)->tmptv;
      settabV(L, o, tabref(fn->c.elw));
      return o;
    } else {
...
static GCtab *getlwrrelw(lua_State *L)
{
  GCfunc *fn = lwrr_func(L);
  return fn->c.gct == ~LJ_TFUNC ? tabref(fn->c.elw) : tabref(L->elw);
}
...
LUA_API void lua_replace(lua_State *L, int idx)
{
  ...
  if (idx == LUA_GLOBALSINDEX) {
    ...
    setgcref(L->elw, obj2gco(tabV(L->top-1)));
  } else if (idx == LUA_ELWIRONINDEX) {
    GCfunc *fn = lwrr_func(L);
    if (fn->c.gct != ~LJ_TFUNC)
      lj_err_msg(L, LJ_ERR_NOELW);
    api_check(L, tvistab(L->top-1));
    setgcref(fn->c.elw, obj2gco(tabV(L->top-1)));
    ...
    if (idx < LUA_GLOBALSINDEX)  /* Need a barrier for upvalues. */
      lj_gc_barrier(L, lwrr_func(L), L->top-1);
  }
  L->top--;
}
...
LUA_API int lua_type(lua_State *L, int idx)
{
  ...
  if (tvisnumber(o)) {
    ...
    return LUA_TNONE;
  } else {  /* Magic internal/external tag colwersion. ORDER LJ_T */
    uint32_t t = ~itype(o);
#if LJ_64
				...
				const char *const lst[])
{
  ...
  for (i = 0; lst[i]; i++)
    if (strcmp(lst[i], s) == 0)
      return (int)i;
  lj_err_argv(L, idx, LJ_ERR_ILWOPTM, s);
}
...
LUA_API void lua_pushnumber(lua_State *L, lua_Number n)
{
  ...
  if (LJ_UNLIKELY(tvisnan(L->top)))
    setnalw(L->top);  /* Canonicalize injected NaNs. */
  incr_top(L);
}
...
LUA_API void lua_pushcclosure(lua_State *L, lua_CFunction f, int n)
{
  ...
  lj_gc_check(L);
  api_checknelems(L, n);
  fn = lj_func_newC(L, (MSize)n, getlwrrelw(L));
  fn->c.f = f;
  L->top -= n;
  while (n--)
...
LUA_API void *lua_newuserdata(lua_State *L, size_t size)
{
  ...
  ud = lj_udata_new(L, (MSize)size, getlwrrelw(L));
...
LUA_API void lua_getfelw(lua_State *L, int idx)
{
  ...
  api_checkvalidindex(L, o);
  if (tvisfunc(o)) {
    settabV(L, L->top, tabref(funcV(o)->c.elw));
  } else if (tvisudata(o)) {
    settabV(L, L->top, tabref(udataV(o)->elw));
  } else if (tvisthread(o)) {
    settabV(L, L->top, tabref(threadV(o)->elw));
  } else {
    setnilV(L->top);
  }
  incr_top(L);
...
LUA_API int lua_setfelw(lua_State *L, int idx)
{
  ...
  t = tabV(L->top-1);
  if (tvisfunc(o)) {
    setgcref(funcV(o)->c.elw, obj2gco(t));
  } else if (tvisudata(o)) {
    setgcref(udataV(o)->elw, obj2gco(t));
  } else if (tvisthread(o)) {
    setgcref(threadV(o)->elw, obj2gco(t));
  } else {
    L->top--;
    return 0;
  }
...
{
  GCfunc *fn = lj_func_newC(L, 0, getlwrrelw(L));
  fn->c.f = func;
  setfuncV(L, L->top, fn);
