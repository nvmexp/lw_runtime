...
LJLIB_CF(table_insert)		LJLIB_REC(.)
{
  ...
  if (nargs != 2*sizeof(TValue)) {
    ...
    for (n = lj_lib_checkint(L, 2); i > n; i--) {
      /* The set may ilwalidate the get pointer, so need to do it first! */
...
static void auxsort(lua_State *L, int l, int u)
{
  while (l < u) {  /* for tail relwrsion */
    ...
    auxsort(L, j, i);  /* call relwrsively the smaller one */
