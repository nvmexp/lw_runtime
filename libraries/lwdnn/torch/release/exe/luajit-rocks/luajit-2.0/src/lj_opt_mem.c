...
{
  IRRef ref = (IRRef)(ir - J->lwr.ir);  /* The ref that might be stored. */
  for (ir++; ir < stop; ir++)
    if (ir->op2 == ref &&
...
static AliasRet aa_ahref(jit_State *J, IRIns *refa, IRIns *refb)
{
  ...
  IRRef ta, tb;
  if (refa == refb)
    return ALIAS_MUST;  /* Shortlwt for same refs. */
  keya = IR(ka);
...
TRef LJ_FASTCALL lj_opt_dse_ahstore(jit_State *J)
{
  ...
  while (ref > xref) {  /* Search for redundant or conflicting stores. */
	...
	for (ir = IR(J->lwr.nins-1); ir > store; ir--)
...
static IRIns *aa_findcnew(jit_State *J, IRIns *ir)
{
  while (ir->o == IR_ADD) {
    if (!irref_isk(ir->op1)) {
      IRIns *ir1 = aa_findcnew(J, IR(ir->op1));  /* Left-relwrsion. */
      if (ir1) return ir1;
    }
    if (irref_isk(ir->op2)) return NULL;
    ir = IR(ir->op2);  /* Flatten right-relwrsion. */
  }
...
static AliasRet aa_xref(jit_State *J, IRIns *refa, IRIns *xa, IRIns *xb)
{
  ...
  if (refa == refb && irt_sametype(xa->t, xb->t))
    return ALIAS_MUST;  /* Shortlwt for same refs with identical type. */
  ...
  */
  if (basea == baseb) {
	...
	return ALIAS_MUST;  /* Same-sized, same-kind. May need to colwert. */
...
retry:
  ...
  while (ref > lim) {
    ...
    case ALIAS_MAY:  lim = ref; goto cselim;  /* Limit search for load. */
    case ALIAS_MUST:
      /* Emit colwersion if the loaded type doesn't match the forwarded type. */
