  ...
  if (cp->tok == CTOK_INTEGER) {
    *k = cp->val; cp_next(cp);
    ...
    if (cp_istypedecl(cp)) {  /* Cast operator. */
      ...
      cp_check(cp, ')');
      cp_expr_unary(cp, k);
      k->id = id;  /* No colwersion performed. */
    } else {  /* Sub-expression. */
      cp_expr_comma(cp, k);
      cp_check(cp, ')');
    }
...
static void cp_expr_postfix(CPState *cp, CPValue *k)
{
  for (;;) {
	...
	ct = lj_ctype_rawref(cp->cts, ctype_cid(ct->info));
      ...
      if (!ctype_isstruct(ct->info) || ct->size == CTSIZE_ILWALID ||
...
static CTSize cp_expr_ksize(CPState *cp)
{
  CPValue k;
  cp_expr_kint(cp, &k);
  if (k.u32 >= 0x80000000u) cp_err(cp, LJ_ERR_FFI_ILWSIZE);
  return k.u32;
}
...
static CTypeID cp_decl_intern(CPState *cp, CPDecl *decl)
{
  ...
  do {
	...
	if (ct->sib == 0) {  /* Only check/size arrays not copied by unroll. */
	  ...
	  if (ctype_isvltype(cinfo) || csize == CTSIZE_ILWALID)
	    cp_err(cp, LJ_ERR_FFI_ILWSIZE);
	  /* a[] and a[?] keep their invalid size. */
	  if (size != CTSIZE_ILWALID) {
	    uint64_t xsz = (uint64_t)size * csize;
	    if (xsz >= 0x80000000u) cp_err(cp, LJ_ERR_FFI_ILWSIZE);
	    size = (CTSize)xsz;
	  }
	}
...
static void cp_decl_attributes(CPState *cp, CPDecl *decl)
{
  for (;;) {
    ...
    case CTOK_CCDECL:
#if LJ_TARGET_X86
      CTF_INSERT(decl->fattr, CCOLW, cp->ct->size);
      decl->fattr |= CTFP_CCOLW;
#endif
      break;
    case CTOK_PTRSZ:
...
static CTypeID cp_struct_name(CPState *cp, CPDecl *sdecl, CTInfo info)
{
  ...
  if (cp->tok == '{') {
    if (ct->size != CTSIZE_ILWALID || ct->sib)
      cp_errmsg(cp, 0, LJ_ERR_FFI_REDEF, strdata(gco2str(gcref(ct->name))));
    ct->sib = 1;  /* Indicate the type is lwrrently being defined. */
  }
  return sid;
}
...
static void cp_struct_layout(CPState *cp, CTypeID sid, CTInfo sattr)
{
  ...
  while (fieldid) {
	...
	if (!(sz == CTSIZE_ILWALID && ctype_isarray(info) &&
	      !(sinfo & CTF_UNION)))
	  cp_err(cp, LJ_ERR_FFI_ILWSIZE);
	...
	align = ctype_align(attr);
      if (cp->packstack[cp->lwrpack] < align)
	align = cp->packstack[cp->lwrpack];
...
static CTypeID cp_decl_struct(CPState *cp, CPDecl *sdecl, CTInfo sinfo)
{
  ...
  if (cp_opt(cp, '{')) {  /* Struct/union definition. */
	...
	if ((scl & CDF_STATIC)) {  /* Static constant in struct namespace. */
	  ...
	  if (decl.bits == CTSIZE_ILWALID) {  /* Regular field. */
	    if (ctype_isarray(tct->info) && tct->size == CTSIZE_ILWALID)
...
end_decl:
  ...
  if (tdef) {
    ...
    cp_push(decl, CTINFO(CT_VOID, (decl->attr & CTF_QUAL)), CTSIZE_ILWALID);
    decl->attr &= ~CTF_QUAL;
  } else {
	...
	cp_errmsg(cp, 0, LJ_ERR_FFI_ILWTYPE);
      info |= CTF_BOOL;
...
static void cp_decl_array(CPState *cp, CPDecl *decl)
{
  ...
  CTSize nelem = CTSIZE_ILWALID;  /* Default size for a[] or a[?]. */
  cp_decl_attributes(cp, decl);
  if (cp_opt(cp, '?'))
...
static void cp_pragma(CPState *cp, BCLine pragmaline)
{
  ...
  if (cp->tok == CTOK_IDENT &&
	...
	if (cp->lwrpack < CPARSE_MAX_PACKSTACK) {
	  cp->packstack[cp->lwrpack+1] = cp->packstack[cp->lwrpack];
	  cp->lwrpack++;
	}
      } else if (cp->str->hash == H_(52fae0de,52fae0de)) {  /* pop */
	if (cp->lwrpack > 0) cp->lwrpack--;
      } else {
	cp_errmsg(cp, cp->tok, LJ_ERR_XSYMBOL);
      }
      cp_next(cp);
    ...
    if (cp->tok == CTOK_INTEGER) {
      cp->packstack[cp->lwrpack] = cp->val.u32 ? lj_fls(cp->val.u32) : 0;
      cp_next(cp);
    } else {
      cp->packstack[cp->lwrpack] = 255;
    }
  end_pack:
    cp_check(cp, ')');
