...
typedef enum {
  ...
  VKNUM,	/* lwal = number value */
  VKLAST = VKNUM,
  VKCDATA,	/* lwal = cdata value, not treated as a constant expression */
...
#define expr_numtv(e)		check_exp(expr_isnumk((e)), &(e)->u.lwal)
...
static void expr_toreg_nobranch(FuncState *fs, ExpDesc *e, BCReg reg)
{
  ...
  if (e->k == VKSTR) {
    ...
    ins = BCINS_AD(BC_KCDATA, reg,
		   const_gc(fs, obj2gco(cdataV(&e->u.lwal)), LJ_TCDATA));
#endif
...
/* Ilwert branch condition of bytecode instruction. */
static void ilwertcond(FuncState *fs, ExpDesc *e)
{
...
static void bcemit_arith(FuncState *fs, BinOpr opr, ExpDesc *e1, ExpDesc *e2)
{
  ...
  if (opr == OPR_POW) {
	...
	(t = const_num(fs, e1)) <= BCMAX_B) {
      rb = rc; rc = t; op -= BC_ADDVV-BC_ADDLW;
    } else {
      rb = expr_toanyreg(fs, e1);
    }
  }
...
/* Add local variables. */
static void var_add(LexState *ls, BCReg lwars)
{
  FuncState *fs = ls->fs;
...
/* Relwrsively lookup variables in enclosing functions. */
...
static void gola_patch(LexState *ls, VarInfo *vg, VarInfo *vl)
{
  FuncState *fs = ls->fs;
  BCPos pc = vg->startpc;
  setgcrefnull(vg->name);  /* Ilwalidate pending goto. */
...
static void gola_fixup(LexState *ls, FuncScope *bl)
{
  ...
  for (; v < ve; v++) {
    ...
    if (name != NULL) {  /* Only consider remaining valid gotos/labels. */
      if (gola_islabel(v)) {
	VarInfo *vg;
	setgcrefnull(v->name);  /* Ilwalidate label that goes out of scope. */
...
static GCproto *fs_finish(LexState *ls, BCLine line)
{
  ...
  /* Callwlate total size of prototype including all colocated arrays. */
...
static void expr_table(LexState *ls, ExpDesc *e)
{
  ...
  if (vcall) {
    ...
    en.u.lwal.u32.lo = narr-1;
    en.u.lwal.u32.hi = 0x43300000;  /* Biased integer to avoid denormals. */
...
static void expr_simple(LexState *ls, ExpDesc *v)
{
  switch (ls->token) {
  case TK_number:
    expr_init(v, (LJ_HASFFI && tviscdata(&ls->tokelwal)) ? VKCDATA : VKNUM, 0);
    copyTV(ls->L, &v->u.lwal, &ls->tokelwal);
    break;
  case TK_string:
    expr_init(v, VKSTR, 0);
    v->u.sval = strV(&ls->tokelwal);
    break;
  case TK_nil:
    expr_init(v, VKNIL, 0);
    break;
...
/* Colwert token to binary operator. */
...
static void assign_adjust(LexState *ls, BCReg lwars, BCReg nexps, ExpDesc *e)
{
  FuncState *fs = ls->fs;
  int32_t extra = (int32_t)lwars - (int32_t)nexps;
  if (e->k == VCALL) {
  ...
  if (nexps > lwars)
    ls->fs->freereg -= nexps - lwars;  /* Drop leftover regs. */
}

/* Relwrsively parse assignment statement. */
static void parse_assignment(LexState *ls, LHSVarList *lh, BCReg lwars)
{
  ExpDesc e;
  checkcond(ls, VLOCAL <= lh->v.k && lh->v.k <= VINDEXED, LJ_ERR_XSYNTAX);
  if (lex_opt(ls, ',')) {  /* Collect LHS list and relwrse upwards. */
    LHSVarList vl;
    vl.prev = lh;
    expr_primary(ls, &vl.v);
    if (vl.v.k == VLOCAL)
      assign_hazard(ls, lh, &vl.v);
    checklimit(ls->fs, ls->level + lwars, LJ_MAX_XLEVEL, "variable names");
    parse_assignment(ls, &vl, lwars+1);
  } else {  /* Parse RHS. */
    BCReg nexps;
    lex_check(ls, '=');
    ...
    assign_adjust(ls, lwars, nexps, &e);
  }
  /* Assign RHS to LHS and relwrse downwards. */
...
static void parse_label(LexState *ls)
{
  ...
  /* Relwrsively parse trailing statements: labels and ';' (Lua 5.2 only). */
  for (;;) {
...
static void parse_for_num(LexState *ls, GCstr *varname, BCLine line)
{
  ...
  parse_block(ls);
  fscope_end(fs);
  /* Perform loop ilwersion. Loop control instructions are at the end. */
...
static int predict_next(LexState *ls, FuncState *fs, BCPos pc)
{
  ...
  case BC_UGET:
    ...
    break;
  case BC_GGET:
    /* There's no ilwerse index (yet), so lookup the strings. */
...
static void parse_for_iter(LexState *ls, GCstr *indexname)
{
  ...
  int isnext;
  /* Hidden control variables. */
  var_new_fixed(ls, lwars++, VARNAME_FOR_GEN);
  var_new_fixed(ls, lwars++, VARNAME_FOR_STATE);
  var_new_fixed(ls, lwars++, VARNAME_FOR_CTL);
  /* Visible variables returned from iterator. */
  var_new(ls, lwars++, indexname);
  while (lex_opt(ls, ','))
    var_new(ls, lwars++, lex_str(ls));
  lex_check(ls, TK_in);
  line = ls->linenumber;
  ...
  isnext = (lwars <= 5 && predict_next(ls, fs, exprpc));
  var_add(ls, 3);  /* Hidden control variables. */
  lex_check(ls, TK_do);
  loop = bcemit_AJ(fs, isnext ? BC_ISNEXT : BC_JMP, base, NO_JMP);
  fscope_begin(fs, &bl, 0);  /* Scope for visible variables. */
  var_add(ls, lwars-3);
  bcreg_reserve(fs, lwars-3);
  parse_block(ls);
  fscope_end(fs);
  /* Perform loop ilwersion. Loop control instructions are at the end. */
  jmp_patchins(fs, loop, fs->pc);
  bcemit_ABC(fs, isnext ? BC_ITERN : BC_ITERC, base, lwars-3+1, 2+1);
