...
** which lack native 64 bit integer operations (the FFI is lwrrently the
...
**
** On soft-float targets, floating-point instructions are directly colwerted
...
#if LJ_SOFTFP
/* Emit a (checked) number to integer colwersion. */
...
{
  IRRef nins = J->lwr.nins, nk = J->lwr.nk;
  MSize irlen = nins - nk;
  ...
  for (ref = REF_FIRST; ref < nins; ref++) {
    ...
    if (ref >= snref) {
      ...
      snref = snap < &J->lwr.snap[J->lwr.nsnap] ? snap->ref : ~(IRRef)0;
    }
	...
	nir->o = IR_COLW;  /* Pass through loword. */
	nir->op2 = (IRT_INT << 5) | IRT_INT;
	hi = split_emit(J, IRT(ir->o == IR_NEG ? IR_BXOR : IR_BAND, IRT_SOFTFP),
			hisubst[ir->op1], hisubst[ir->op2]);
	break;
      case IR_SLOAD:
	if ((nir->op2 & IRSLOAD_COLWERT)) {  /* Colwert from int to number. */
	  nir->op2 &= ~IRSLOAD_COLWERT;
	...
	*nir = inslo;  /* Re-emit lo XLOAD immediately before hi XLOAD. */
	...
	break;
      case IR_COLW: {  /* Colwersion to number. Others handled below. */
	...
	break;
      case IR_COLW: {  /* Colwersion to 64 bit integer. Others handled below. */
	IRType st = (IRType)(ir->op2 & IRCOLW_SRCMASK);
#if LJ_SOFTFP
	if (st == IRT_NUM) {  /* NUM to 64 bit int colw. */
	  hi = split_call_l(J, hisubst, oir, ir,
		 irt_isi64(ir->t) ? IRCALL_fp64_d2l : IRCALL_fp64_d2ul);
	} else if (st == IRT_FLOAT) {  /* FLOAT to 64 bit int colw. */
	  nir->o = IR_CALLN;
	  nir->op2 = irt_isi64(ir->t) ? IRCALL_fp64_f2l : IRCALL_fp64_f2ul;
	  hi = split_emit(J, IRTI(IR_HIOP), nref, nref);
	}
#else
	if (st == IRT_NUM || st == IRT_FLOAT) {  /* FP to 64 bit int colw. */
	  ...
	  hi = hiref;
	  goto fwdlo;
	} else if ((ir->op2 & IRCOLW_SEXT)) {  /* Sign-extend to 64 bit. */
...
#if LJ_SOFTFP
    if (ir->o == IR_SLOAD) {
      if ((nir->op2 & IRSLOAD_COLWERT)) {  /* Colwert from number to int. */
	nir->op2 &= ~IRSLOAD_COLWERT;
...
#endif
    if (ir->o == IR_COLW) {  /* See above, too. */
      IRType st = (IRType)(ir->op2 & IRCOLW_SRCMASK);
#if LJ_32 && LJ_HASFFI
      if (st == IRT_I64 || st == IRT_U64) {  /* Colwersion from 64 bit int. */
#if LJ_SOFTFP
	if (irt_isfloat(ir->t)) {
	  split_call_l(J, hisubst, oir, ir,
		       st == IRT_I64 ? IRCALL_fp64_l2f : IRCALL_fp64_ul2f);
	  J->lwr.nins--;  /* Drop unused HIOP. */
	}
#else
	if (irt_isfp(ir->t)) {  /* 64 bit integer to FP colwersion. */
  ...
  /* Add PHI marks. */
  for (ref = J->lwr.nins-1; ref >= REF_FIRST; ref--) {
    IRIns *ir = IR(ref);
...
static int split_needsplit(jit_State *J)
{
  IRIns *ir, *irend;
  IRRef ref;
  for (ir = IR(REF_FIRST), irend = IR(J->lwr.nins); ir < irend; ir++)
    if (LJ_SOFTFP ? irt_is64orfp(ir->t) : irt_isint64(ir->t))
      return 1;
  if (LJ_SOFTFP) {
    for (ref = J->chain[IR_SLOAD]; ref; ref = IR(ref)->prev)
      if ((IR(ref)->op2 & IRSLOAD_COLWERT))
	return 1;
    if (J->chain[IR_TOBIT])
      return 1;
  }
  for (ref = J->chain[IR_COLW]; ref; ref = IR(ref)->prev) {
    IRType st = (IR(ref)->op2 & IRCOLW_SRCMASK);
