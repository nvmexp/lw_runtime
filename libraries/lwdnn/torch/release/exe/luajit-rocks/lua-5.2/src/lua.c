...
static int handle_luainit (lua_State *L) {
  ...
  const char *init = getelw(name + 1);
  if (init == NULL) {
    name = "=" LUA_INIT;
    init = getelw(name + 1);  /* try alternative name */
  }
...
static int pmain (lua_State *L) {
  ...
  if (args[has_E]) {  /* option '-E'? */
    lua_pushboolean(L, 1);  /* signal for libraries to ignore elw. vars. */
  ...
  if (args[has_i])  /* -i option? */
    ...
    else dofile(L, NULL);  /* exelwtes stdin as a file */
  }
