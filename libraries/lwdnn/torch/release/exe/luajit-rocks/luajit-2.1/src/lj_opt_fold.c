...
**   shortlwt for FAILFOLD + cond (i.e. drop if true, otherwise fail).
**
...
** frontier to an ilwariant instruction. Adding a PHI for this instruction
...
** The only exception is for relwrrences with high latencies like
** repeated int->num->int colwersions.
**
...
** all other ops up the chain. Colwersely, if an op is unused, all ops
...
LJFOLDF(bufput_kgc)
{
  if (LJ_LIKELY(J->flags & JIT_F_OPT_FOLD) && fright->o == IR_KGC) {
	...
	/* lj_ir_kstr() may realloc the IR and ilwalidates any IRIns *. */
...
LJFOLDF(bufstr_kfold_cse)
{
  ...
  if (LJ_LIKELY(J->flags & JIT_F_OPT_FOLD)) {
	...
	return fleft->op2;  /* Shortlwt for a single put operation. */
    }
  }
...
/* -- Constant folding of colwersions ------------------------------------- */
...
LJFOLD(COLW KINT IRCOLW_NUM_INT)
LJFOLDF(kfold_colw_kint_num)
{
  return lj_ir_knum(J, (lua_Number)fleft->i);
}

LJFOLD(COLW KINT IRCOLW_NUM_U32)
LJFOLDF(kfold_colw_kintu32_num)
{
  return lj_ir_knum(J, (lua_Number)(uint32_t)fleft->i);
}

LJFOLD(COLW KINT IRCOLW_INT_I8)
LJFOLD(COLW KINT IRCOLW_INT_U8)
LJFOLD(COLW KINT IRCOLW_INT_I16)
LJFOLD(COLW KINT IRCOLW_INT_U16)
LJFOLDF(kfold_colw_kint_ext)
{
  int32_t k = fleft->i;
  if ((fins->op2 & IRCOLW_SRCMASK) == IRT_I8) k = (int8_t)k;
  else if ((fins->op2 & IRCOLW_SRCMASK) == IRT_U8) k = (uint8_t)k;
  else if ((fins->op2 & IRCOLW_SRCMASK) == IRT_I16) k = (int16_t)k;
  else k = (uint16_t)k;
  return INTFOLD(k);
}

LJFOLD(COLW KINT IRCOLW_I64_INT)
LJFOLD(COLW KINT IRCOLW_U64_INT)
LJFOLD(COLW KINT IRCOLW_I64_U32)
LJFOLD(COLW KINT IRCOLW_U64_U32)
LJFOLDF(kfold_colw_kint_i64)
{
  if ((fins->op2 & IRCOLW_SEXT))
    return INT64FOLD((uint64_t)(int64_t)fleft->i);
  else
    return INT64FOLD((uint64_t)(int64_t)(uint32_t)fleft->i);
}

LJFOLD(COLW KINT64 IRCOLW_NUM_I64)
LJFOLDF(kfold_colw_kint64_num_i64)
{
  return lj_ir_knum(J, (lua_Number)(int64_t)ir_kint64(fleft)->u64);
}

LJFOLD(COLW KINT64 IRCOLW_NUM_U64)
LJFOLDF(kfold_colw_kint64_num_u64)
{
  return lj_ir_knum(J, (lua_Number)ir_kint64(fleft)->u64);
}

LJFOLD(COLW KINT64 IRCOLW_INT_I64)
LJFOLD(COLW KINT64 IRCOLW_U32_I64)
LJFOLDF(kfold_colw_kint64_int_i64)
{
  return INTFOLD((int32_t)ir_kint64(fleft)->u64);
}

LJFOLD(COLW KNUM IRCOLW_INT_NUM)
LJFOLDF(kfold_colw_knum_int_num)
{
  lua_Number n = knumleft;
  int32_t k = lj_num2int(n);
  if (irt_isguard(fins->t) && n != (lua_Number)k) {
    /* We're about to create a guard which always fails, like COLW +1.5.
...
LJFOLD(COLW KNUM IRCOLW_U32_NUM)
LJFOLDF(kfold_colw_knum_u32_num)
{
#ifdef _MSC_VER
...
LJFOLD(COLW KNUM IRCOLW_I64_NUM)
LJFOLDF(kfold_colw_knum_i64_num)
{
  return INT64FOLD((uint64_t)(int64_t)knumleft);
}

LJFOLD(COLW KNUM IRCOLW_U64_NUM)
LJFOLDF(kfold_colw_knum_u64_num)
{
...
LJFOLDF(shortlwt_left)
{
  return LEFTFOLD;  /* f(g(x)) ==> g(x) */
}

LJFOLD(ABS NEG FLOAD)
LJFOLDF(shortlwt_dropleft)
{
  PHIBARRIER(fleft);
...
/* Note: no safe shortlwts with STRTO and TOSTR ("1e2" ==> +100 ==> "100"). */
LJFOLD(NEG NEG any)
LJFOLD(BNOT BNOT)
LJFOLD(BSWAP BSWAP)
LJFOLDF(shortlwt_leftleft)
{
...
/* -- Simplify colwersions ------------------------------------------------ */

LJFOLD(COLW COLW IRCOLW_NUM_INT)  /* _NUM */
LJFOLDF(shortlwt_colw_num_int)
{
  PHIBARRIER(fleft);
  /* Only safe with a guarded colwersion to int. */
  if ((fleft->op2 & IRCOLW_SRCMASK) == IRT_NUM && irt_isguard(fleft->t))
    return fleft->op1;  /* f(g(x)) ==> x */
  return NEXTFOLD;
}

LJFOLD(COLW COLW IRCOLW_INT_NUM)  /* _INT */
LJFOLD(COLW COLW IRCOLW_U32_NUM)  /* _U32*/
LJFOLDF(simplify_colw_int_num)
{
  /* Fold even across PHI to avoid expensive num->int colwersions in loop. */
  if ((fleft->op2 & IRCOLW_SRCMASK) ==
      ((fins->op2 & IRCOLW_DSTMASK) >> IRCOLW_DSH))
    return fleft->op1;
  return NEXTFOLD;
}

LJFOLD(COLW COLW IRCOLW_I64_NUM)  /* _INT or _U32 */
LJFOLD(COLW COLW IRCOLW_U64_NUM)  /* _INT or _U32 */
LJFOLDF(simplify_colw_i64_num)
{
  PHIBARRIER(fleft);
  if ((fleft->op2 & IRCOLW_SRCMASK) == IRT_INT) {
    /* Reduce to a sign-extension. */
    fins->op1 = fleft->op1;
    fins->op2 = ((IRT_I64<<5)|IRT_INT|IRCOLW_SEXT);
    return RETRYFOLD;
  } else if ((fleft->op2 & IRCOLW_SRCMASK) == IRT_U32) {
#if LJ_TARGET_X64
    return fleft->op1;
#else
...
LJFOLD(COLW COLW IRCOLW_INT_I64)  /* _INT or _U32 */
LJFOLD(COLW COLW IRCOLW_INT_U64)  /* _INT or _U32 */
LJFOLD(COLW COLW IRCOLW_U32_I64)  /* _INT or _U32 */
LJFOLD(COLW COLW IRCOLW_U32_U64)  /* _INT or _U32 */
LJFOLDF(simplify_colw_int_i64)
{
  int src;
  PHIBARRIER(fleft);
  src = (fleft->op2 & IRCOLW_SRCMASK);
  if (src == IRT_INT || src == IRT_U32) {
    if (src == ((fins->op2 & IRCOLW_DSTMASK) >> IRCOLW_DSH)) {
      return fleft->op1;
    } else {
      fins->op2 = ((fins->op2 & IRCOLW_DSTMASK) | src);
      fins->op1 = fleft->op1;
      return RETRYFOLD;
    }
  }
...
LJFOLD(COLW COLW IRCOLW_FLOAT_NUM)  /* _FLOAT */
LJFOLDF(simplify_colw_flt_num)
{
  PHIBARRIER(fleft);
  if ((fleft->op2 & IRCOLW_SRCMASK) == IRT_FLOAT)
    return fleft->op1;
  return NEXTFOLD;
}

/* Shortlwt TOBIT + IRT_NUM <- IRT_INT/IRT_U32 colwersion. */
LJFOLD(TOBIT COLW KNUM)
LJFOLDF(simplify_tobit_colw)
{
  /* Fold even across PHI to avoid expensive num->int colwersions in loop. */
  if ((fleft->op2 & IRCOLW_SRCMASK) == IRT_INT) {
...
/* Shortlwt floor/ceil/round + IRT_NUM <- IRT_INT/IRT_U32 colwersion. */
LJFOLD(FPMATH COLW IRFPM_FLOOR)
LJFOLD(FPMATH COLW IRFPM_CEIL)
LJFOLD(FPMATH COLW IRFPM_TRUNC)
LJFOLDF(simplify_floor_colw)
{
  if ((fleft->op2 & IRCOLW_SRCMASK) == IRT_INT ||
      (fleft->op2 & IRCOLW_SRCMASK) == IRT_U32)
    return LEFTFOLD;
  return NEXTFOLD;
}

/* Strength reduction of widening. */
LJFOLD(COLW any IRCOLW_I64_INT)
LJFOLD(COLW any IRCOLW_U64_INT)
LJFOLDF(simplify_colw_sext)
{
  IRRef ref = fins->op1;
  int64_t ofs = 0;
  if (!(fins->op2 & IRCOLW_SEXT))
...
LJFOLD(COLW ADD IRCOLW_INT_I64)
LJFOLD(COLW SUB IRCOLW_INT_I64)
LJFOLD(COLW MUL IRCOLW_INT_I64)
LJFOLD(COLW ADD IRCOLW_INT_U64)
LJFOLD(COLW SUB IRCOLW_INT_U64)
LJFOLD(COLW MUL IRCOLW_INT_U64)
LJFOLD(COLW ADD IRCOLW_U32_I64)
LJFOLD(COLW SUB IRCOLW_U32_I64)
LJFOLD(COLW MUL IRCOLW_U32_I64)
LJFOLD(COLW ADD IRCOLW_U32_U64)
LJFOLD(COLW SUB IRCOLW_U32_U64)
LJFOLD(COLW MUL IRCOLW_U32_U64)
LJFOLDF(simplify_colw_narrow)
{
  IROp op = (IROp)fleft->o;
  IRType t = irt_type(fins->t);
  IRRef op1 = fleft->op1, op2 = fleft->op2, mode = fins->op2;
  PHIBARRIER(fleft);
  op1 = emitir(IRTI(IR_COLW), op1, mode);
  op2 = emitir(IRTI(IR_COLW), op2, mode);
  fins->ot = IRT(op, t);
  fins->op1 = op1;
  fins->op2 = op2;
  return RETRYFOLD;
...
/* Special CSE rule for COLW. */
LJFOLD(COLW any any)
LJFOLDF(cse_colw)
{
  if (LJ_LIKELY(J->flags & JIT_F_OPT_CSE)) {
    IRRef op1 = fins->op1, op2 = (fins->op2 & IRCOLW_MODEMASK);
    uint8_t guard = irt_isguard(fins->t);
    IRRef ref = J->chain[IR_COLW];
    while (ref > op1) {
      IRIns *ir = IR(ref);
      /* Commoning with stronger checks is ok. */
      if (ir->op1 == op1 && (ir->op2 & IRCOLW_MODEMASK) == op2 &&
...
/* FP colwersion narrowing. */
LJFOLD(TOBIT ADD KNUM)
LJFOLD(TOBIT SUB KNUM)
LJFOLD(COLW ADD IRCOLW_INT_NUM)
LJFOLD(COLW SUB IRCOLW_INT_NUM)
LJFOLD(COLW ADD IRCOLW_I64_NUM)
LJFOLD(COLW SUB IRCOLW_I64_NUM)
LJFOLDF(narrow_colwert)
{
  PHIBARRIER(fleft);
  /* Narrowing ignores PHIs and repeating it inside the loop is not useful. */
  if (J->chain[IR_LOOP])
    return NEXTFOLD;
  lua_assert(fins->o != IR_COLW || (fins->op2&IRCOLW_COLWMASK) != IRCOLW_TOBIT);
  return lj_opt_narrow_colwert(J);
}
...
/* Eliminate ilwariant ABC inside loop. */
LJFOLD(ABC any any)
LJFOLDF(abc_ilwar)
{
  /* Ilwariant ABC marked as PTR. Drop if op1 is ilwariant, too. */
...
LJFOLDF(cse_uref)
{
  if (LJ_LIKELY(J->flags & JIT_F_OPT_CSE)) {
    ...
    GLwpval *uv = gco2uv(gcref(fn->l.uvptr[(fins->op2 >> 8)]));
    while (ref > 0) {
      IRIns *ir = IR(ref);
      if (irref_isk(ir->op1)) {
...
** an aliased table, as it may ilwalidate all of the pointers and fields.
...
LJFOLD(FLOAD any IRFL_FUNC_ELW)
LJFOLD(FLOAD any IRFL_THREAD_ELW)
...
LJFOLDF(prof)
{
  ...
  if (ref+1 == J->lwr.nins)  /* Drop neighbouring IR_PROF. */
    return ref;
  return EMITFOLD;
}
