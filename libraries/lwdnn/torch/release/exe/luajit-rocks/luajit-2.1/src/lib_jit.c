...
LJLIB_CF(jit_attach)
{
  ...
  if (s) {  /* Attach to given event. */
    ...
    G(L)->vmevmask = VMEVENT_NOCACHE;  /* Ilwalidate cache. */
...
LJLIB_CF(jit_util_funcinfo)
{
  ...
  if (pt) {
    ...
    if (pc < pt->sizebc)
      setintfield(L, t, "lwrrentline", lj_debug_line(pt, pc));
...
/* local name = jit.util.funlwvname(func, idx) */
LJLIB_CF(jit_util_funlwvname)
{
  GCproto *pt = check_Lproto(L, 0);
...
static const char *const jit_trlinkname[] = {
  "none", "root", "loop", "tail-relwrsion", "up-relwrsion", "down-relwrsion",
...
#define JIT_F_SSE2	2
  ...
  if (!(flags & JIT_F_MIPSXXR2)) {
    int x;
#ifdef __mips16
    x = 0;  /* Runtime detection is diffilwlt. Ensure optimal -march flags. */
#else
...
LUALIB_API int luaopen_jit(lua_State *L)
{
  ...
  lj_lib_prereg(L, LUA_JITLIBNAME ".util", luaopen_jit_util, tabref(L->elw));
#endif
#if LJ_HASJIT
