...
static void asm_tvptr(ASMState *as, Reg dest, IRRef ref)
{
  ...
  if (irt_isnum(ir->t)) {
	...
	/* Lwrrently, no caller passes integers that might end up here. */
...
static void asm_uref(ASMState *as, IRIns *ir)
{
  ...
  if (irref_isk(ir->op1)) {
    ...
    if (ir->o == IR_UREFC) {
      emit_rmro(as, XO_LEA, dest|REX_GC64, uv, offsetof(GLwpval, tv));
      asm_guardcc(as, CC_NE);
      emit_i8(as, 1);
      emit_rmro(as, XO_ARITHib, XOg_CMP, uv, offsetof(GLwpval, closed));
    } else {
      emit_rmro(as, XO_MOV, dest|REX_GC64, uv, offsetof(GLwpval, v));
    }
...
static void asm_cnew(ASMState *as, IRIns *ir)
{
  ...
  IRRef args[4];
  lua_assert(sz != CTSIZE_ILWALID || (ir->o == IR_CNEW && ir->op2 != REF_NIL));
