...
/* This oclwpies 6 bytes, so use the next 2 bytes for non-32 bit fields. */
...
**
** This is to ensure the integrity of the incremental GC. The ilwariant
...
**   sure nothing ilwokes the GC inbetween.
...
** may ilwalidate the incremental GC ilwariant.
*/
...
typedef struct GLwdata {
  GCHeader;
  uint8_t udtype;	/* Userdata type. */
  uint8_t unused2;
  GCRef elw;		/* Should be at same offset in GCfunc. */
...
#define sizeudata(u)	(sizeof(struct GLwdata)+(u)->len)
...
/* Common header for functions. elw should be at same offset in GLwdata. */
#define GCfuncHeader \
  GCHeader; uint8_t ffid; uint8_t nupvalues; \
  GCRef elw; GCRef gclist; MRef pc
...
typedef struct GCfuncL {
  GCfuncHeader;
  GCRef uvptr[1];	/* Array of _pointers_ to upvalue objects (GLwpval). */
} GCfuncL;
...
typedef struct GCtab {
  ...
  GCRef gclist;
  GCRef metatable;	/* Must be at same offset in GLwdata. */
  MRef node;		/* Hash part. */
...
typedef struct GCState {
  GCSize total;		/* Memory lwrrently allocated. */
  GCSize threshold;	/* Memory threshold. */
  uint8_t lwrrentwhite;	/* Current white color. */
...
typedef struct global_State {
  ...
  GLwpval uvhead;	/* Head of double-linked list of all open upvalues. */
  ...
  GCRef lwr_L;		/* Lwrrently exelwting lua_State. */
...
struct lua_State {
  GCHeader;
  uint8_t dummy_ffid;	/* Fake FF_C for lwrr_funcisL() on dummy frames. */
  uint8_t status;	/* Thread status. */
  MRef glref;		/* Link to global state. */
  GCRef gclist;		/* GC chain. */
  TValue *base;		/* Base of lwrrently exelwting function. */
  TValue *top;		/* First free slot in the stack. */
  MRef maxstack;	/* Last free slot in the stack. */
  MRef stack;		/* Stack base. */
  GCRef openupval;	/* List of open upvalues in the stack. */
  GCRef elw;		/* Thread environment (table of globals). */
...
/* Macros to access the lwrrently exelwting (Lua) function. */
#if LJ_GC64
#define lwrr_func(L)		(&gcval(L->base-2)->fn)
#elif LJ_FR2
#define lwrr_func(L)		(&gcref((L->base-2)->gcr)->fn)
#else
#define lwrr_func(L)		(&gcref((L->base-1)->fr.func)->fn)
#endif
#define lwrr_funcisL(L)		(isluafunc(lwrr_func(L)))
#define lwrr_proto(L)		(funcproto(lwrr_func(L)))
#define lwrr_topL(L)		(L->base + lwrr_proto(L)->framesize)
#define lwrr_top(L)		(lwrr_funcisL(L) ? lwrr_topL(L) : L->top)

/* -- GC object definition and colwersions -------------------------------- */
...
/* The elw field SHOULD be at the same offset for all GC objects. */
LJ_STATIC_ASSERT(offsetof(GChead, elw) == offsetof(GCfuncL, elw));
LJ_STATIC_ASSERT(offsetof(GChead, elw) == offsetof(GLwdata, elw));
...
LJ_STATIC_ASSERT(offsetof(GChead, metatable) == offsetof(GLwdata, metatable));
...
/* Macros to colwert a GCobj pointer into a specific value. */
...
/* Macro to colwert any collectable object into a GCobj pointer. */
...
define_setV(setudataV, GLwdata, LJ_TUDATA)

#define setnumV(o, x)		((o)->n = (x))
#define setnalw(o)		((o)->u64 = U64x(fff80000,00000000))
