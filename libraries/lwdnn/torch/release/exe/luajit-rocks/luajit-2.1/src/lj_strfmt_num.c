...
/* -- Formatted colwersions to buffer ------------------------------------- */
...
static char *lj_strfmt_wfnum(SBuf *sb, SFormat sf, lua_Number n, char *p)
{
  ...
  if (LJ_UNLIKELY((t.u32.hi << 1) >= 0xffe00000)) {
    ...
    *p++ = eprefix; /* + or - */
    ...
    /* %e or %f or %g - begin by colwerting n to "nd" format. */
    uint32_t nd[64];
	...
	t.u64 -= 2; /* Colwert 2ulp below (later we colwert 2ulp above). */
...
#error "ND_MUL2K_MAX_SHIFT too small"
    ...
    if ((sf & STRFMT_T_FP_E)) {
      /* %e or %g - assume %e and start by callwlating nd's exponent (nde). */
      char eprefix = '+';
      int32_t nde = -1;
      MSize hilen;
      if (ndlo && !nd[ndhi]) {
	...
	** crossed. Having already colwerted the -2ulp case, we save off its
	** most significant digits, colwert the +2ulp case, and compare them.
	*/
...
/* -- Colwersions to strings ---------------------------------------------- */

/* Colwert number to string. */
