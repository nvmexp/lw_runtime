...
void LJ_FASTCALL lj_ir_growtop(jit_State *J)
{
  ...
  J->lwr.ir = J->irbuf = baseir - J->irbotlim;
}
...
static void lj_ir_growbot(jit_State *J)
{
  ...
  MSize szins = J->irtoplim - J->irbotlim;
  lua_assert(szins != 0);
  lua_assert(J->lwr.nk == J->irbotlim || J->lwr.nk-1 == J->irbotlim);
  if (J->lwr.nins + (szins >> 1) < J->irtoplim) {
    /* More than half of the buffer is free on top: shift up by a quarter. */
    MSize ofs = szins >> 2;
    memmove(baseir + ofs, baseir, (J->lwr.nins - J->irbotlim)*sizeof(IRIns));
    J->irbotlim -= ofs;
    J->irtoplim -= ofs;
    J->lwr.ir = J->irbuf = baseir - J->irbotlim;
  } else {
    ...
    memcpy(newbase + ofs, baseir, (J->lwr.nins - J->irbotlim)*sizeof(IRIns));
    lj_mem_free(G(J->L), baseir, szins*sizeof(IRIns));
    J->irbotlim -= ofs;
    J->irtoplim = J->irbotlim + 2*szins;
    J->lwr.ir = J->irbuf = newbase - J->irbotlim;
  }
}
...
/*
** IR instructions for constants are kept between J->lwr.nk >= ref < REF_BIAS.
...
** Note: this may ilwalidate all IRIns *!
*/
...
/* -- Colwert IR operand types -------------------------------------------- */

/* Colwert from string to number. */
...
/* Colwert from integer or string to number. */
TRef LJ_FASTCALL lj_ir_tonum(jit_State *J, TRef tr)
{
  if (!tref_isnum(tr)) {
    if (tref_isinteger(tr))
      tr = emitir(IRTN(IR_COLW), tr, IRCOLW_NUM_INT);
...
/* Colwert from integer or number to string. */
