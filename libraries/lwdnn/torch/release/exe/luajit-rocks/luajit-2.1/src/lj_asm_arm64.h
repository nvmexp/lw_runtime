...
static uint32_t asm_fuseopm(ASMState *as, A64Ins ai, IRRef ref, RegSet allow)
{
  ...
  if (ra_hasreg(ir->r)) {
	...
	(ir->o == IR_ADD && ir->op1 == ir->op2)) {
      ...
      if (sh == A64SH_LSL &&
	  irl->o == IR_COLW &&
	  irl->op2 == ((IRT_I64<<IRCOLW_DSH)|IRT_INT|IRCOLW_SEXT) &&
	  shift <= 4 &&
	  canfuse(as, irl)) {
    ...
    } else if (ir->o == IR_COLW &&
	       ir->op2 == ((IRT_I64<<IRCOLW_DSH)|IRT_INT|IRCOLW_SEXT)) {
      Reg m = ra_alloc1(as, ir->op1, allow);
...
static void asm_uref(ASMState *as, IRIns *ir)
{
  ...
  if (irref_isk(ir->op1)) {
    ...
    if (ir->o == IR_UREFC) {
      ...
      emit_lso(as, A64I_LDRB, RID_TMP, uv, (int32_t)offsetof(GLwpval, closed));
    } else {
      emit_lso(as, A64I_LDRx, dest, uv, (int32_t)offsetof(GLwpval, v));
    }
...
static void asm_sload(ASMState *as, IRIns *ir)
{
  ...
  if ((ir->op2 & IRSLOAD_COLWERT) && irt_isguard(t) && irt_isint(t)) {
    ...
    if (irt_isaddr(t)) {
      emit_dn(as, A64I_ANDx^emit_isk13(LJ_GCVMASK, 1), dest, dest);
    } else if ((ir->op2 & IRSLOAD_COLWERT)) {
      if (irt_isint(t)) {
...
dotypecheck:
  ...
  if ((ir->op2 & IRSLOAD_TYPECHECK)) {
    ...
    if (irt_isnum(t) && !(ir->op2 & IRSLOAD_COLWERT))
      emit_dn(as, A64I_FMOV_D_R, (dest & 31), tmp);
...
static int asm_swapops(ASMState *as, IRRef lref, IRRef rref)
{
  ...
  if ((ir->o >= IR_BSHL && ir->o <= IR_BSAR) ||
      (ir->o == IR_ADD && ir->op1 == ir->op2) ||
      (ir->o == IR_COLW && ir->op2 == ((IRT_I64<<IRCOLW_DSH)|IRT_INT|IRCOLW_SEXT)))
			    ...
			    IRIns *irp, RegSet allow, ExitNo exitno)
{
  ...
  if (irp) {  /* Must not spill arbitrary registers in head of side trace. */
    ...
    emit_lso(as, A64I_LDRx, RID_TMP, RID_GL, glofs(as, &J2G(as->J)->lwr_L));
...
static void asm_loop_fixup(ASMState *as)
{
  ...
  if (as->loopilw) {  /* Ilwerted loop branch? */
    ...
    /* asm_guard* already ilwerted the bcc/tnb/cnb and patched the final b. */
