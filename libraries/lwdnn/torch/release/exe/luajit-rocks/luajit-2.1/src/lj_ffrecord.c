...
/* Colwentions for fast function call handlers:
**
...
{
  if (J->lwr.nins < (IRRef)J->param[JIT_P_minstitch] + REF_BASE) {
...
static void LJ_FASTCALL recff_tonumber(jit_State *J, RecordFFData *rd)
{
  ...
  if (tref_isnumber_str(tr)) {
	...
	recff_nyiu(J, rd);  /* Would need an ilwerted STRTO for this case. */
	return;
      }
...
static void LJ_FASTCALL recff_getfelw(jit_State *J, RecordFFData *rd)
{
  TRef tr = J->base[0];
  /* Only support getfelw(0) for now. */
  if (tref_isint(tr) && tref_isk(tr) && IR(tref_ref(tr))->i == 0) {
    TRef trl = emitir(IRT(IR_LREF, IRT_THREAD), 0, 0);
    J->base[0] = emitir(IRT(IR_FLOAD, IRT_TAB), trl, IRFL_THREAD_ELW);
    return;
  }
  recff_nyiu(J, rd);
}
...
static void LJ_FASTCALL recff_math_round(jit_State *J, RecordFFData *rd)
{
  ...
  if (!tref_isinteger(tr)) {  /* Pass through integers unmodified. */
    ...
    if (LJ_DUALNUM) {  /* Try to narrow using a guarded colwersion to int. */
      lua_Number n = lj_vm_foldfpm(numberVnum(&rd->argv[0]), rd->data);
      if (n == (lua_Number)lj_num2int(n))
	tr = emitir(IRTGI(IR_COLW), tr, IRCOLW_INT_NUM|IRCOLW_CHECK);
    }
    J->base[0] = tr;
  }
}
...
static void LJ_FASTCALL recff_math_minmax(jit_State *J, RecordFFData *rd)
{
  ...
  for (i = 1; J->base[i] != 0; i++) {
    ...
    if (!(tref_isinteger(tr) && tref_isinteger(tr2))) {
      if (tref_isinteger(tr)) tr = emitir(IRTN(IR_COLW), tr, IRCOLW_NUM_INT);
      if (tref_isinteger(tr2)) tr2 = emitir(IRTN(IR_COLW), tr2, IRCOLW_NUM_INT);
      t = IRT_NUM;
    }
...
{
  GLwdata *ud = udataV(&J->fn->c.upvalue[0]);
  TRef tr, one;
...
static void LJ_FASTCALL recff_string_char(jit_State *J, RecordFFData *rd)
{
  ...
  BCReg i;
  for (i = 0; J->base[i] != 0; i++) {  /* Colwert char values to strings. */
...
static void LJ_FASTCALL recff_string_format(jit_State *J, RecordFFData *rd)
{
  ...
  while ((sf = lj_strfmt_parse(&fs)) != STRFMT_EOF) {  /* Parse format. */
	...
	goto handle_num;
      if (sf == STRFMT_INT) { /* Shortlwt for plain %d. */
	tr = emitir(IRT(IR_BUFPUT, IRT_PGC), tr,
		    emitir(IRT(IR_TOSTR, IRT_STR), tra, IRTOSTR_INT));
      } else {
#if LJ_HASFFI
	tra = emitir(IRT(IR_COLW, IRT_U64), tra,
		     (IRT_INT|(IRT_U64<<5)|IRCOLW_SEXT));
	...
	return;
      ...
      if (sf == STRFMT_STR)  /* Shortlwt for plain %s. */
    ...
    case STRFMT_CHAR:
      ...
      if (sf == STRFMT_CHAR)  /* Shortlwt for plain %c. */
	tr = emitir(IRT(IR_BUFPUT, IRT_PGC), tr,
