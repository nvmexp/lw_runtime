...
/* Bad colwersion. */
LJ_NORET static void ccolw_err_colw(CTState *cts, CType *d, CType *s,
				    CTInfo flags)
{
  ...
  if (CCF_GETARG(flags))
    lj_err_argv(cts->L, CCF_GETARG(flags), LJ_ERR_FFI_BADCOLW, src, dst);
  else
    lj_err_callerv(cts->L, LJ_ERR_FFI_BADCOLW, src, dst);
}

/* Bad colwersion from TValue. */
LJ_NORET static void ccolw_err_colwtv(CTState *cts, CType *d, TValue *o,
				      CTInfo flags)
{
  const char *dst = strdata(lj_ctype_repr(cts->L, ctype_typeid(cts, d), NULL));
...
LJ_NORET static void ccolw_err_initov(CTState *cts, CType *d)
{
  const char *dst = strdata(lj_ctype_repr(cts->L, ctype_typeid(cts, d), NULL));
...
static CType *ccolw_childqual(CTState *cts, CType *ct, CTInfo *qual)
{
...
/* Check for compatible types when colwerting to a pointer.
** Note: these checks are more relaxed than what C99 mandates.
*/
int lj_ccolw_compatptr(CTState *cts, CType *d, CType *s, CTInfo flags)
{
  if (!((flags & CCF_CAST) || d == s)) {
    CTInfo dqual = 0, squal = 0;
    d = ccolw_childqual(cts, d, &dqual);
    if (!ctype_isstruct(s->info))
      s = ccolw_childqual(cts, s, &squal);
    if ((flags & CCF_SAME)) {
      if (dqual != squal)
	...
	return 1;  /* Colwerting to/from void * is always ok. */
    }
	...
	return 0;  /* Different numeric types. */
      ...
      return lj_ccolw_compatptr(cts, d, s, flags|CCF_SAME);
...
/* -- C type to C type colwersion ----------------------------------------- */

/* Colwert C type to C type. Caveat: expects to get the raw CType!
**
...
*/
void lj_ccolw_ct_ct(CTState *cts, CType *d, CType *s,
		    uint8_t *dp, uint8_t *sp, CTInfo flags)
{
  ...
  if (ctype_type(dinfo) > CT_MAYCOLWERT || ctype_type(sinfo) > CT_MAYCOLWERT)
    goto err_colw;
  ...
  case CCX(B, F): {
    ...
    else goto err_colw;  /* NYI: long double. */
    if (dsize == 1) *dp = b; else *(int *)dp = b;
    break;
    }
  ...
  colw_I_I:
    ...
    break;
  case CCX(I, F): {
    double n;  /* Always colwert via double. */
  colw_I_F:
    /* Colwert source to double. */
    if (ssize == sizeof(double)) n = *(double *)sp;
    else if (ssize == sizeof(float)) n = (double)*(float *)sp;
    else goto err_colw;  /* NYI: long double. */
    /* Then colwert double to integer. */
    /* The colwersion must exactly match the semantics of JIT-compiled code! */
	...
	*(uint64_t *)dp = lj_num2u64(n);
      ...
      goto err_colw;  /* NYI: colwersion to >64 bit integers. */
    }
    break;
    }
  case CCX(I, C):
    ...
    sinfo = s->info;
    ssize = s->size;
    goto colw_I_F;  /* Just colwert re. */
  case CCX(I, P):
    if (!(flags & CCF_CAST)) goto err_colw;
  ...
  case CCX(F, I): {
    double n;  /* Always colwert via double. */
  colw_F_I:
    /* First colwert source to double. */
    /* The colwersion must exactly match the semantics of JIT-compiled code! */
	...
	if (ssize == 2) i = *(uint16_t *)sp;
      ...
      goto err_colw;  /* NYI: colwersion from >64 bit integers. */
    }
    /* Colwert double to destination. */
  ...
  case CCX(F, C):
    ...
    sinfo = s->info;
    ssize = s->size;
    goto colw_F_F;  /* Ignore im, and colwert from re. */
  ...
  case CCX(C, I):
    ...
    goto colw_F_I;  /* Colwert to re. */
  case CCX(C, F):
    d = ctype_child(cts, d);
    dinfo = d->info;
    dsize = d->size;
    memset(dp + dsize, 0, dsize);  /* Clear im. */
    goto colw_F_F;  /* Colwert to re. */

  case CCX(C, C):
    if (dsize != ssize) {  /* Different types: colwert re/im separately. */
      CType *dc = ctype_child(cts, d);
      CType *sc = ctype_child(cts, s);
      lj_ccolw_ct_ct(cts, dc, sc, dp, sp, flags);
      lj_ccolw_ct_ct(cts, dc, sc, dp + dc->size, sp + sc->size, flags);
      return;
    }
  ...
  case CCX(V, C): {
    CType *dc = ctype_child(cts, d);
    CTSize esize;
    /* First colwert the scalar to the first element. */
    lj_ccolw_ct_ct(cts, dc, s, dp, sp, flags);
  ...
  case CCX(P, F):
    if (!(flags & CCF_CAST) || !(flags & CCF_FROMTV)) goto err_colw;
    /* The signed colwersion is cheaper. x64 really has 47 bit pointers. */
    dinfo = CTINFO(CT_NUM, (LJ_64 && dsize == 8) ? 0 : CTF_UNSIGNED);
    goto colw_I_F;

  case CCX(P, P):
    if (!lj_ccolw_compatptr(cts, d, s, flags)) goto err_colw;
  ...
  case CCX(A, A):
	...
	d->size == CTSIZE_ILWALID || !lj_ccolw_compatptr(cts, d, s, flags))
      goto err_colw;
    goto copyval;
  ...
  case CCX(S, S):
    if ((flags & CCF_CAST) || (d->info & CTF_VLA) || d != s)
      goto err_colw;  /* Must be exact same type. */
copyval:  /* Copy value. */
  ...
  default:
  err_colw:
    ccolw_err_colw(cts, d, s, flags);
  }
}

/* -- C type to TValue colwersion ----------------------------------------- */

/* Colwert C type to TValue. Caveat: expects to get the raw CType! */
int lj_ccolw_tv_ct(CTState *cts, CType *s, CTypeID sid,
		   TValue *o, uint8_t *sp)
{
  CTInfo sinfo = s->info;
  if (ctype_isnum(sinfo)) {
	...
	int32_t i;
	lj_ccolw_ct_ct(cts, ctype_get(cts, CTID_INT32), s,
		       (uint8_t *)&i, sp, 0);
	...
	lj_ccolw_ct_ct(cts, ctype_get(cts, CTID_DOUBLE), s,
		       (uint8_t *)&o->n, sp, 0);
...
/* Colwert bitfield to TValue. */
int lj_ccolw_tv_bf(CTState *cts, CType *s, TValue *o, uint8_t *sp)
{
  CTInfo info = s->info;
  CTSize pos, bsz;
  uint32_t val;
...
/* -- TValue to C type colwersion ----------------------------------------- */

/* Colwert table to array. */
static void ccolw_array_tab(CTState *cts, CType *d,
			    uint8_t *dp, GCtab *t, CTInfo flags)
{
  ...
  for (i = 0; ; i++) {
    ...
    if (ofs >= size)
      ccolw_err_initov(cts, d);
    lj_ccolw_ct_tv(cts, dc, dp + ofs, tv, flags);
    ofs += esize;
  }
  if (size != CTSIZE_ILWALID) {  /* Only fill up arrays with known size. */
...
/* Colwert table to sub-struct/union. */
static void ccolw_substruct_tab(CTState *cts, CType *d, uint8_t *dp,
				GCtab *t, int32_t *ip, CTInfo flags)
{
  ...
  while (id) {
	...
	*ip = i + 1;
	...
	lj_ccolw_ct_tv(cts, ctype_rawchild(cts, df), dp+df->size, tv, flags);
      else
	lj_ccolw_bf_tv(cts, df, dp+df->size, tv);
      if ((d->info & CTF_UNION)) break;
    } else if (ctype_isxattrib(df->info, CTA_SUBTYPE)) {
      ccolw_substruct_tab(cts, ctype_rawchild(cts, df),
			  dp+df->size, t, ip, flags);
    }  /* Ignore all other entries in the chain. */
  }
}

/* Colwert table to struct/union. */
static void ccolw_struct_tab(CTState *cts, CType *d,
			     uint8_t *dp, GCtab *t, CTInfo flags)
{
  int32_t i = 0;
  memset(dp, 0, d->size);  /* Much simpler to clear the struct first. */
  ccolw_substruct_tab(cts, d, dp, t, &i, flags);
}

/* Colwert TValue to C type. Caveat: expects to get the raw CType! */
void lj_ccolw_ct_tv(CTState *cts, CType *d,
		    uint8_t *dp, TValue *o, CTInfo flags)
{
  ...
  if (LJ_LIKELY(tvisint(o))) {
    ...
    if (ctype_isarray(d->info)) {
      ccolw_array_tab(cts, d, dp, tabV(o), flags);
      return;
    } else if (ctype_isstruct(d->info)) {
      ccolw_struct_tab(cts, d, dp, tabV(o), flags);
      return;
    } else {
      goto err_colw;
    }
  ...
  err_colw:
    ccolw_err_colwtv(cts, d, o, flags);
  }
  s = ctype_get(cts, sid);
docolw:
  if (ctype_isenum(d->info)) d = ctype_child(cts, d);
  lj_ccolw_ct_ct(cts, d, s, dp, sp, flags);
}

/* Colwert TValue to bitfield. */
void lj_ccolw_bf_tv(CTState *cts, CType *d, uint8_t *dp, TValue *o)
{
  CTInfo info = d->info;
  CTSize pos, bsz;
  uint32_t val, mask;
  ...
  if ((info & CTF_BOOL)) {
    ...
    lj_ccolw_ct_tv(cts, ctype_get(cts, CTID_BOOL), &tmpbool, o, 0);
    val = tmpbool;
  } else {
    CTypeID did = (info & CTF_UNSIGNED) ? CTID_UINT32 : CTID_INT32;
    lj_ccolw_ct_tv(cts, ctype_get(cts, did), (uint8_t *)&val, o, 0);
  }
  pos = ctype_bitpos(info);
  bsz = ctype_bitbsz(info);
  lua_assert(pos < 8*ctype_bitcsz(info));
...
static void ccolw_array_init(CTState *cts, CType *d, CTSize sz, uint8_t *dp,
			     TValue *o, MSize len)
{
  CType *dc = ctype_rawchild(cts, d);  /* Array element type. */
  ...
  for (i = 0, ofs = 0; i < len; i++, ofs += esize)
    lj_ccolw_ct_tv(cts, dc, dp + ofs, o + i, 0);
  if (ofs == esize) {  /* Replicate a single element. */
...
static void ccolw_substruct_init(CTState *cts, CType *d, uint8_t *dp,
				 TValue *o, MSize len, MSize *ip)
{
  ...
  while (id) {
    ...
    if (ctype_isfield(df->info) || ctype_isbitfield(df->info)) {
      ...
      *ip = i + 1;
      if (ctype_isfield(df->info))
	lj_ccolw_ct_tv(cts, ctype_rawchild(cts, df), dp+df->size, o + i, 0);
      else
	lj_ccolw_bf_tv(cts, df, dp+df->size, o + i);
      if ((d->info & CTF_UNION)) break;
    } else if (ctype_isxattrib(df->info, CTA_SUBTYPE)) {
      ccolw_substruct_init(cts, ctype_rawchild(cts, df),
			   dp+df->size, o, len, ip);
      if ((d->info & CTF_UNION)) break;
    }  /* Ignore all other entries in the chain. */
  }
...
static void ccolw_struct_init(CTState *cts, CType *d, CTSize sz, uint8_t *dp,
			      TValue *o, MSize len)
{
  MSize i = 0;
  memset(dp, 0, sz);  /* Much simpler to clear the struct first. */
  ccolw_substruct_init(cts, d, dp, o, len, &i);
  if (i < len)
    ccolw_err_initov(cts, d);
}
...
*/
int lj_ccolw_multi_init(CTState *cts, CType *d, TValue *o)
{
...
void lj_ccolw_ct_init(CTState *cts, CType *d, CTSize sz,
		      uint8_t *dp, TValue *o, MSize len)
{
  if (len == 0)
    memset(dp, 0, sz);
  else if (len == 1 && !lj_ccolw_multi_init(cts, d, o))
    lj_ccolw_ct_tv(cts, d, dp, o, 0);
  else if (ctype_isarray(d->info))  /* Also handles valarray init with len>1. */
    ccolw_array_init(cts, d, sz, dp, o, len);
  else if (ctype_isstruct(d->info))
    ccolw_struct_init(cts, d, sz, dp, o, len);
  else
    ccolw_err_initov(cts, d);
}
