...
** would ilwalidate passed TValue pointers. The stack check is performed
...
void lj_state_shrinkstack(lua_State *L, MSize used)
{
  ...
  if (4*used < L->stacksize &&
      ...
      (tvref(G(L)->jit_base) == NULL || obj2gco(L) != gcref(G(L)->lwr_L)))
...
static void stack_init(lua_State *L1, lua_State *L)
{
  ...
  setthreadV(L1, st++, L1);  /* Needed for lwrr_funcisL() on empty stack. */
...
static TValue *cpluaopen(lua_State *L, lua_CFunction dummy, void *ud)
{
  ...
  setgcref(L->elw, obj2gco(lj_tab_new(L, 0, LJ_MIN_GLOBAL)));
...
#endif
{
  ...
  L->dummy_ffid = FF_C;
  setmref(L->glref, g);
  g->gc.lwrrentwhite = LJ_GC_WHITE0 | LJ_GC_FIXED;
