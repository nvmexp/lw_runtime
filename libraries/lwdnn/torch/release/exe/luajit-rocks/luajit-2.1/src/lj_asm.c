...
typedef struct ASMState {
  ...
  int loopilw;		/* Loop branch ilwersion (0:no, 1:yes, 2:yes+CC_P). */

  int32_t evenspill;	/* Next even spill slot. */
  int32_t oddspill;	/* Next odd spill slot (or 0). */

  IRRef lwrins;		/* Reference of current instruction. */
  ...
  int32_t gcsteps;	/* Aclwmulated number of GC steps (per section). */
  ...
  MCode *ilwmcp;	/* Points to ilwertible loop branch (or NULL). */
...
/* Check for variant to ilwariant references. */
#define iscrossref(as, ref)	((ref) < as->sectref)

/* Inhibit memory op fusion from variant to ilwariant references. */
...
static LJ_AINLINE void checkmclim(ASMState *as)
{
  ...
  if (as->mcp + MCLIM_REDZONE < as->mcp_prev) {
    IRIns *ir = IR(as->lwrins+1);
    fprintf(stderr, "RED ZONE OVERFLOW: %p IR %04d  %02d %04d %04d\n", as->mcp,
	    as->lwrins+1-REF_BIAS, ir->o, ir->op1-REF_BIAS, ir->op2-REF_BIAS);
    lua_assert(0);
  }
#endif
...
static void ra_dprintf(ASMState *as, const char *fmt, ...)
{
  ...
  ra_dbg_mcp = NULL;
  p += sprintf(p, "%08x  \e[36m%04d ", (uintptr_t)as->mcp, as->lwrins-REF_BIAS);
  for (;;) {
...
static Reg ra_allock(ASMState *as, intptr_t k, RegSet allow)
{
  ...
  if (pick) {
    ...
    if ((pick & ~as->modset))
      pick &= ~as->modset;
    r = rset_pickbot(pick);  /* Reduce conflicts with ilwerse allocation. */
  } else {
    r = ra_evict(as, allow);
  }
...
static Reg ra_allocref(ASMState *as, IRRef ref, RegSet allow)
{
  ...
  if (pick) {
    ...
    /* Ilwariants should preferably get unmodified registers. */
...
static void ra_addrename(ASMState *as, Reg down, IRRef ref, SnapNo snapno)
{
  ...
  as->J->lwr.ir[ren].r = (uint8_t)down;
  as->J->lwr.ir[ren].s = SPS_NONE;
}
...
static void ra_rename(ASMState *as, Reg down, Reg up)
{
  ...
  emit_movrr(as, ir, down, up);  /* Backwards codegen needs ilwerse move. */
...
static void asm_snap_alloc1(ASMState *as, IRRef ref)
{
  ...
  if (!irref_isk(ref) && (!(ra_used(ir) || ir->r == RID_SUNK))) {
	...
	for (irs = IR(as->snapref-1); irs > ir; irs--)
      ...
      RegSet allow;
      if (ir->o == IR_COLW && ir->op2 == IRCOLW_NUM_INT) {
	IRIns *irc;
	for (irc = IR(as->lwrins); irc > ir; irc--)
	  if ((irc->op1 == ref || irc->op2 == ref) &&
	      !(irc->r == RID_SINK || irc->r == RID_SUNK))
	    goto nosink;  /* Don't sink colwersion if result is used. */
...
/* All guards for a snapshot use the same exitno. This is lwrrently the
...
static void asm_bufput(ASMState *as, IRIns *ir)
{
  ...
  if (irs->o == IR_KGC) {
    ...
    if (irs->o == IR_TOSTR) {  /* Fuse number to string colwersions. */
...
static void asm_colw64(ASMState *as, IRIns *ir)
{
  IRType st = (IRType)((ir-1)->op2 & IRCOLW_SRCMASK);
  IRType dt = (((ir-1)->op2 & IRCOLW_DSTMASK) >> IRCOLW_DSH);
  IRCallID id;
  IRRef args[2];
  lua_assert((ir-1)->o == IR_COLW && ir->o == IR_HIOP);
  args[LJ_BE] = (ir-1)->op1;
  args[LJ_LE] = ir->op1;
...
#if LJ_TARGET_X86
    nargs |= (ctype_ccolw(ct->info) << CCI_CC_SHIFT);
#endif
  }
#endif
...
** - the register is lwrrently free: rename it.
** - the register is blocked by an ilwariant: restore/remat and rename it.
...
static void asm_phi_shuffle(ASMState *as)
{
  ...
  for (;;) {
	...
	if (!rset_test(as->freeset, r)) {  /* PHI register blocked? */
	  ...
	  if (!ra_iskref(ref) && irt_ismarked(IR(ref)->t)) {
	    ...
	    left = RID_NONE;
	  } else {  /* Otherwise grab register from ilwariant. */
	    ra_restore(as, ref);
	    checkmclim(as);
	  }
	}
  ...
  /* Restore/remat ilwariants whose registers are modified inside the loop. */
#if !LJ_SOFTFP
...
static void asm_loop(ASMState *as)
{
  ...
  if (as->gcsteps)
    asm_gc_check(as);
  /* LOOP marks the transition from the variant to the ilwariant part. */
  as->flagmcp = as->ilwmcp = NULL;
  as->sectref = 0;
...
static void asm_ir(ASMState *as, IRIns *ir)
{
  ...
  case IR_EQ: case IR_NE:
    if ((ir-1)->o == IR_HREF && ir->op1 == as->lwrins-1) {
      as->lwrins--;
  ...
  /* Type colwersions. */
  case IR_COLW: asm_colw(as, ir); break;
...
static void asm_head_side(ASMState *as)
{
  ...
  RegSet allow = RSET_ALL;  /* Ilwerse of all coalesced registers. */
  ...
  /* Callwlate stack frame adjustment. */
  spadj = asm_stack_adjust(as);
...
static void asm_tail_link(ASMState *as)
{
  ...
  if (as->J->ktrace) {  /* Patch ktrace slot with the final GCtrace pointer. */
    setgcref(IR(as->J->ktrace)[LJ_GC64].gcr, obj2gco(as->J->lwrfinal));
...
static void asm_setup_regsp(ASMState *as)
{
  ...
  for (lastir = IR(nins); ir < lastir; ir++) {
	...
	if (inloop)
      ...
      break;
#endif
    /* Do not propagate hints across type colwersions or loads. */
    case IR_TOBIT:
    case IR_XLOAD:
#if !LJ_TARGET_ARM
    case IR_ALOAD: case IR_HLOAD: case IR_ULOAD: case IR_VLOAD:
      ...
      break;
    case IR_COLW:
      if (irt_isfp(ir->t) || (ir->op2 & IRCOLW_SRCMASK) == IRT_NUM ||
	  (ir->op2 & IRCOLW_SRCMASK) == IRT_FLOAT)
	break;
      /* fallthrough */
    default:
...
void lj_asm_trace(jit_State *J, GCtrace *T)
{
  ...
  /* This also allows one RENAME to be added without reallocating lwrfinal. */
  as->orignins = lj_ir_nextins(J);
  J->lwr.ir[as->orignins].o = IR_NOP;

  /* Setup initial state. Copy some fields to reduce indirections. */
  as->J = J;
  as->T = T;
  J->lwrfinal = lj_trace_alloc(J->L, T);  /* This copies the IR, too. */
  as->flags = J->flags;
  ...
  **
  **    This is lwrrently only the case on x86/x64, where small loops get
  ...
  **    at the end if the IR (in J->lwr.ir) has actually grown, resize the
  **    copy (in J->lwrfinal.ir) and try again.
  **
  ...
  */
  for (;;) {
    ...
    as->mcp_prev = as->mcp;
#endif
    as->ir = J->lwrfinal->ir;  /* Use the copied IR. */
    as->lwrins = J->lwr.nins = as->orignins;
    ...
    for (as->lwrins--; as->lwrins > as->stopins; as->lwrins--) {
      IRIns *ir = IR(as->lwrins);
    ...
    if (as->realign && J->lwrfinal->nins >= T->nins)
    ...
    checkmclim(as);
    if (as->gcsteps > 0) {
      as->lwrins = as->T->snap[0].ref;
      asm_snap_prep(as);  /* The GC check is a guard. */
      asm_gc_check(as);
      as->lwrins = as->stopins;
    }
    ra_evictk(as);
    if (as->parent)
      asm_head_side(as);
    ...
    if (J->lwrfinal->nins >= T->nins) {  /* IR didn't grow? */
      lua_assert(J->lwrfinal->nk == T->nk);
      memcpy(J->lwrfinal->ir + as->orignins, T->ir + as->orignins,
	     (T->nins - as->orignins) * sizeof(IRIns));  /* Copy RENAMEs. */
      T->nins = J->lwrfinal->nins;
      break;  /* Done. */
    }

    /* Otherwise try again with a bigger IR. */
    lj_trace_free(J2G(J), J->lwrfinal);
    J->lwrfinal = NULL;  /* In case lj_trace_alloc() OOMs. */
    J->lwrfinal = lj_trace_alloc(J->L, T);
    as->realign = NULL;
  }
