...
** amount of exelwtion resources as their integer counterparts, except
...
**
** The total CPU exelwtion bandwidth is the sum of the bandwidth of the FP
...
** them means losing exelwtion bandwidth. Moving work away from them to
...
** requires narrowing colwersions with high latencies and additional
** guards (to check that the index is really an integer). And many common
** optimizations only work on integers.
**
** One solution would be spelwlative, eager narrowing of all number loads.
...
** exelwtion port for the branch unit and tie up reorder buffers. This is
...
** dependencies can be well hidden by out-of-order exelwtion.
**
...
**
** The integer type in the IR has colwenient wrap-around semantics and
...
/* -- Elimination of narrowing type colwersions --------------------------- */

/* Narrowing of index expressions and bit operations is demand-driven. The
** trace recorder emits a narrowing type colwersion (COLW.int.num or TOBIT)
** in all of these cases (e.g. array indexing or string indexing). FOLD
** already takes care of eliminating simple redundant colwersions like
** COLW.int.num(COLW.num.int(x)) ==> x.
**
...
**   COLW.int.num(ADD(COLW.num.int(i), 1))
** which is clearly suboptimal.
**
** One can do better by relwrsively backpropagating the narrowing type
** colwersion across FP arithmetic operations. This turns FP ops into
** their corresponding integer counterparts. Depending on the semantics of
** the colwersion they also need to check for overflow. Lwrrently only ADD
** and SUB are supported.
**
** The above example can be rewritten as
**   ADDOV(COLW.int.num(COLW.num.int(i)), 1)
** and then into ADDOV(i, 1) after folding of the colwersions. The original
...
** a previous colwersion). As a not-so-obvious corollary this logic also
** applies for a whole expression tree (e.g. t[(a+1)+(b+1)]).
**
** Correctness of the transformation is guaranteed by avoiding to expand
** the tree by adding more colwersions than the one we would need to emit
** if not backpropagating. TOBIT employs a more optimistic rule, because
** the colwersion has special semantics, designed to make the life of the
...
** This either requires IR rollback and cache ilwalidation for every
** subtree or a two-pass algorithm. The former didn't work out too well,
** so the code now combines a relwrsive collector with a stack-based
** emitter.
**
** [A relwrsive backpropagation algorithm with backtracking, employing
...
enum {
  NARROW_REF,		/* Push ref. */
  NARROW_COLW,		/* Push colwersion of ref. */
...
/* Context used for narrowing of type colwersions. */
typedef struct NarrowColw {
  jit_State *J;		/* JIT compiler state. */
  NarrowIns *sp;	/* Current stack pointer. */
  NarrowIns *maxsp;	/* Maximum stack pointer minus redzone. */
  IRRef mode;		/* Colwersion mode (IRCOLW_*). */
...
static BPropEntry *narrow_bpc_get(jit_State *J, IRRef1 key, IRRef mode)
{
  ...
  for (i = 0; i < BPROP_SLOTS; i++) {
	...
	((bp->mode ^ mode) & IRCOLW_MODEMASK) == 0)
      return bp;
  }
  return NULL;
}
...
static void narrow_stripov_backprop(NarrowColw *nc, IRRef ref, int depth)
{
  jit_State *J = nc->J;
  IRIns *ir = IR(ref);
  if (ir->o == IR_ADDOV || ir->o == IR_SUBOV ||
      (ir->o == IR_MULOV && (nc->mode & IRCOLW_COLWMASK) == IRCOLW_ANY)) {
    BPropEntry *bp = narrow_bpc_get(nc->J, ref, IRCOLW_TOBIT);
    if (bp) {
      ref = bp->val;
...
/* Backpropagate narrowing colwersion. Return number of needed colwersions. */
static int narrow_colw_backprop(NarrowColw *nc, IRRef ref, int depth)
{
  jit_State *J = nc->J;
  IRIns *ir = IR(ref);
  IRRef cref;
  ...
  if (ir->o == IR_COLW && (ir->op2 & IRCOLW_SRCMASK) == IRT_INT) {
    if ((nc->mode & IRCOLW_COLWMASK) <= IRCOLW_ANY)
      narrow_stripov_backprop(nc, ir->op1, depth+1);
    else
      *nc->sp++ = NARROWINS(NARROW_REF, ir->op1);  /* Undo colwersion. */
    if (nc->t == IRT_I64)
      *nc->sp++ = NARROWINS(NARROW_SEXT, 0);  /* Sign-extend integer. */
    return 0;
  } else if (ir->o == IR_KNUM) {  /* Narrow FP constant. */
    lua_Number n = ir_knum(ir)->n;
    if ((nc->mode & IRCOLW_COLWMASK) == IRCOLW_TOBIT) {
  ...
  /* Try to CSE the colwersion. Stronger checks are ok, too. */
  cref = J->chain[fins->o];
  while (cref > ref) {
    IRIns *cr = IR(cref);
    if (cr->op1 == ref &&
	(fins->o == IR_TOBIT ||
	 ((cr->op2 & IRCOLW_MODEMASK) == (nc->mode & IRCOLW_MODEMASK) &&
	  irt_isguard(cr->t) >= irt_isguard(fins->t)))) {
      *nc->sp++ = NARROWINS(NARROW_REF, cref);
      return 0;  /* Already there, no additional colwersion needed. */
    }
    cref = cr->prev;
  }
  ...
  if (ir->o == IR_ADD || ir->o == IR_SUB) {
    ...
    IRRef mode = nc->mode;
    BPropEntry *bp;
    /* Inner colwersions need a stronger check. */
    if ((mode & IRCOLW_COLWMASK) == IRCOLW_INDEX && depth > 0)
      mode += IRCOLW_CHECK-IRCOLW_INDEX;
    bp = narrow_bpc_get(nc->J, (IRRef1)ref, mode);
    if (bp) {
      *nc->sp++ = NARROWINS(NARROW_REF, bp->val);
      return 0;
    } else if (nc->t == IRT_I64) {
      /* Try sign-extending from an existing (checked) colwersion to int. */
      mode = (IRT_INT<<5)|IRT_NUM|IRCOLW_INDEX;
      bp = narrow_bpc_get(nc->J, (IRRef1)ref, mode);
      if (bp) {
	*nc->sp++ = NARROWINS(NARROW_REF, bp->val);
	*nc->sp++ = NARROWINS(NARROW_SEXT, 0);
    ...
    if (++depth < NARROW_MAX_BACKPROP && nc->sp < nc->maxsp) {
      NarrowIns *savesp = nc->sp;
      int count = narrow_colw_backprop(nc, ir->op1, depth);
      count += narrow_colw_backprop(nc, ir->op2, depth);
      if (count <= 1) {  /* Limit total number of colwersions. */
	*nc->sp++ = NARROWINS(IRT(ir->o, nc->t), ref);
	return count;
      }
      nc->sp = savesp;  /* Too many colwersions, need to backtrack. */
    }
  }

  /* Otherwise add a colwersion. */
  *nc->sp++ = NARROWINS(NARROW_COLW, ref);
  return 1;
}

/* Emit the colwersions collected during backpropagation. */
static IRRef narrow_colw_emit(jit_State *J, NarrowColw *nc)
{
  /* The fins fields must be saved now -- emitir() overwrites them. */
  IROpT guardot = irt_isguard(fins->t) ? IRTG(IR_ADDOV-IR_ADD, 0) : 0;
  IROpT colwot = fins->ot;
  IRRef1 colwop2 = fins->op2;
  ...
  while (next < last) {  /* Simple stack machine to process the ins. list. */
    ...
    if (op == NARROW_REF) {
      *sp++ = ref;
    } else if (op == NARROW_COLW) {
      *sp++ = emitir_raw(colwot, ref, colwop2);  /* Raw emit avoids a loop. */
    } else if (op == NARROW_SEXT) {
      lua_assert(sp >= nc->stack+1);
      sp[-1] = emitir(IRT(IR_COLW, IRT_I64), sp[-1],
		      (IRT_I64<<5)|IRT_INT|IRCOLW_SEXT);
      ...
      *sp++ = nc->t == IRT_I64 ?
      ...
      if ((mode & IRCOLW_COLWMASK) == IRCOLW_INDEX) {
...
/* Narrow a type colwersion of an arithmetic operation. */
TRef LJ_FASTCALL lj_opt_narrow_colwert(jit_State *J)
{
  if ((J->flags & JIT_F_OPT_NARROW)) {
    ...
    if (fins->o == IR_TOBIT) {
      nc.mode = IRCOLW_TOBIT;  /* Used only in the backpropagation cache. */
    } else {
      nc.mode = fins->op2;
    }
    if (narrow_colw_backprop(&nc, fins->op1, 0) <= 1)
      return narrow_colw_emit(J, &nc);
  }
  return NEXTFOLD;
}

/* -- Narrowing of implicit colwersions ----------------------------------- */

/* Relwrsively strip overflow checks. */
  ...
  if (op >= IR_ADDOV && op <= lastop) {
    ...
    if (bp) {
      ...
      tr = emitir(IRT(op - IR_ADDOV + IR_ADD,
		      ((mode & IRCOLW_DSTMASK) >> IRCOLW_DSH)), op1, op2);
      narrow_bpc_set(J, ref, tref_ref(tr), mode);
    }
  } else if (LJ_64 && (mode & IRCOLW_SEXT) && !irt_is64(ir->t)) {
    tr = emitir(IRT(IR_COLW, IRT_INTP), tr, mode);
  }
  return tr;
}
...
TRef LJ_FASTCALL lj_opt_narrow_index(jit_State *J, TRef tr)
{
  ...
  if (tref_isnum(tr))  /* Colwersion may be narrowed, too. See above. */
    return emitir(IRTGI(IR_COLW), tr, IRCOLW_INT_NUM|IRCOLW_INDEX);
  /* Omit some overflow checks for array indexing. See comments above. */
  ir = IR(tref_ref(tr));
...
/* Narrow colwersion to integer operand (overflow undefined). */
TRef LJ_FASTCALL lj_opt_narrow_toint(jit_State *J, TRef tr)
{
  if (tref_isstr(tr))
    tr = emitir(IRTG(IR_STRTO, IRT_NUM), tr, 0);
  if (tref_isnum(tr))  /* Colwersion may be narrowed, too. See above. */
    return emitir(IRTI(IR_COLW), tr, IRCOLW_INT_NUM|IRCOLW_ANY);
  if (!tref_isinteger(tr))
    lj_trace_err(J, LJ_TRERR_BADTYPE);
  /*
  ** Undefined overflow semantics allow stripping of ADDOV, SUBOV and MULOV.
  ** Use IRCOLW_TOBIT for the cache entries, since the semantics are the same.
  */
  return narrow_stripov(J, tr, IR_MULOV, (IRT_INT<<5)|IRT_INT|IRCOLW_TOBIT);
}

/* Narrow colwersion to bitop operand (overflow wrapped). */
TRef LJ_FASTCALL lj_opt_narrow_tobit(jit_State *J, TRef tr)
{
  ...
  */
  return narrow_stripov(J, tr, IR_SUBOV, (IRT_INT<<5)|IRT_INT|IRCOLW_TOBIT);
}
...
TRef LJ_FASTCALL lj_opt_narrow_cindex(jit_State *J, TRef tr)
{
  lua_assert(tref_isnumber(tr));
  if (tref_isnum(tr))
    return emitir(IRT(IR_COLW, IRT_INTP), tr, (IRT_INTP<<5)|IRT_NUM|IRCOLW_ANY);
  /* Undefined overflow semantics allow stripping of ADDOV, SUBOV and MULOV. */
  return narrow_stripov(J, tr, IR_MULOV,
			LJ_64 ? ((IRT_INTP<<5)|IRT_INT|IRCOLW_SEXT) :
				((IRT_INTP<<5)|IRT_INT|IRCOLW_TOBIT));
}
#endif
...
/* Colwert string to number. Error out for non-numeric string values. */
static TRef colw_str_tonum(jit_State *J, TRef tr, TValue *o)
{
  if (tref_isstr(tr)) {
    tr = emitir(IRTG(IR_STRTO, IRT_NUM), tr, 0);
    /* Would need an ilwerted STRTO for this rare and useless case. */
    if (!lj_strscan_num(strV(o), o))  /* Colwert in-place. Value used below. */
...
{
  rb = colw_str_tonum(J, rb, vb);
  rc = colw_str_tonum(J, rc, vc);
  ...
  if (!tref_isnum(rb)) rb = emitir(IRTN(IR_COLW), rb, IRCOLW_NUM_INT);
  if (!tref_isnum(rc)) rc = emitir(IRTN(IR_COLW), rc, IRCOLW_NUM_INT);
...
TRef lj_opt_narrow_unm(jit_State *J, TRef rc, TValue *vc)
{
  ...
  if (tref_isinteger(rc)) {
    ...
    rc = emitir(IRTN(IR_COLW), rc, IRCOLW_NUM_INT);
  }
...
TRef lj_opt_narrow_pow(jit_State *J, TRef rb, TRef rc, TValue *vb, TValue *vc)
{
  ...
  if (tvisint(vc) || numisint(numV(vc))) {
    ...
    if (!tref_isinteger(rc)) {
      /* Guarded colwersion to integer! */
      rc = emitir(IRTGI(IR_COLW), rc, IRCOLW_INT_NUM|IRCOLW_CHECK);
    }
