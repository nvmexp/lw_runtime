...
static const uint8_t *lib_read_lfunc(lua_State *L, const uint8_t *p, GCtab *tab)
{
  ...
  fn = lj_func_newL_empty(L, pt, tabref(L->elw));
		     ...
		     const uint8_t *p, const lua_CFunction *cf)
{
  ...
  for (;;) {
    ...
    if (tag != LIBINIT_STRING) {
      ...
      GCfunc *fn = lj_func_newC(L, nuv, elw);
      if (nuv) {
	L->top = L->base + tpos;
...
void lj_lib_prereg(lua_State *L, const char *name, lua_CFunction f, GCtab *elw)
{
  luaL_findtable(L, LUA_REGISTRYINDEX, "_PRELOAD", 4);
  lua_pushcfunction(L, f);
  /* NOBARRIER: The function is new (marked white). */
  setgcref(funcV(L->top-1)->c.elw, obj2gco(elw));
...
int lj_lib_postreg(lua_State *L, lua_CFunction cf, int id, const char *name)
{
  ...
  GCtab *t = tabref(lwrr_func(L)->c.elw);  /* Reference to parent table. */
...
int lj_lib_checkopt(lua_State *L, int narg, int def, const char *lst)
{
  ...
  if (s) {
    ...
    lj_err_argv(L, narg, LJ_ERR_ILWOPTM, opt);
  }
  return def;
}
