...
{
  uint32_t ilwai, i, m = (uint32_t)n;
  /* K12: unsigned 8 bit value, rotated in steps of two bits. */
  for (i = 0; i < 4096; i += 256, m = lj_rol(m, 2))
    if (m <= 255) return ARMI_K12|m|i;
  /* Otherwise try negation/complement with the ilwerse instruction. */
  ilwai = emit_ilwai[((ai >> 21) & 15)];
  if (!ilwai) return 0;  /* Failed. No ilwerse instruction. */
  m = ~(uint32_t)n;
  if (ilwai == ((ARMI_SUB^ARMI_ADD) >> 21) ||
      ilwai == (ARMI_CMP^ARMI_CMN) >> 21) m++;
  for (i = 0; i < 4096; i += 256, m = lj_rol(m, 2))
    if (m <= 255) return ARMI_K12|(ilwai<<21)|m|i;
  return 0;  /* Failed. */
}
...
static int emit_kdelta2(ASMState *as, Reg d, int32_t i)
{
  ...
  while (work) {
	...
	uint32_t sh, ilw = 0, k2, k;
	if (delta < 0) { delta = -delta; ilw = ARMI_ADD^ARMI_SUB; }
	sh = lj_ffs(delta) & ~1;
	k2 = emit_isk12(0, delta & (255 << sh));
	k = emit_isk12(0, delta & ~(255 << sh));
	if (k) {
	  emit_dn(as, ARMI_ADD^k2^ilw, d, d);
	  emit_dn(as, ARMI_ADD^k^ilw, d, r);
	  return 1;
	}
      }
    }
