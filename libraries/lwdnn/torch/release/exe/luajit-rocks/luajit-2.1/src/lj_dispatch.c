...
/* Relwrsively set the JIT mode for all children of a prototype. */
...
/* Callwlate number of used stack slots in the current frame. */
static BCReg lwr_topslot(GCproto *pt, const BCIns *pc, uint32_t nres)
{
  BCIns ins = pc[-1];
...
void LJ_FASTCALL lj_dispatch_ins(lua_State *L, const BCIns *pc)
{
  ...
  BCReg slots;
  setcframe_pc(cf, pc);
  slots = lwr_topslot(pt, pc, cframe_multres_n(cf));
...
void LJ_FASTCALL lj_dispatch_stitch(jit_State *J, const BCIns *pc)
{
  ...
  L->top = L->base + lwr_topslot(lwrr_proto(L), pc+1, cframe_multres_n(cf));
...
void LJ_FASTCALL lj_dispatch_profile(lua_State *L, const BCIns *pc)
{
  ...
  global_State *g;
  setcframe_pc(cf, pc);
  L->top = L->base + lwr_topslot(pt, pc, cframe_multres_n(cf));
