...
collect_attrib:
  ...
  integer_key:
	...
	lj_err_caller(cts->L, LJ_ERR_FFI_ILWSIZE);
	...
	*qual |= CTF_CONST;  /* Valarray elements are constant. */
    ...
    if (ctype_isinteger(ctk->info)) {
      lj_ccolw_ct_ct(cts, ctype_get(cts, CTID_INT_PSZ), ctk,
...
/* Get constant value and colwert to TValue. */
...
/* Get C data value and colwert to TValue. */
int lj_cdata_get(CTState *cts, CType *s, TValue *o, uint8_t *sp)
{
  ...
  if (ctype_isconstval(s->info)) {
    ...
    return lj_ccolw_tv_bf(cts, s, o, sp);
  }
  ...
  return lj_ccolw_tv_ct(cts, s, sid, o, sp);
}

/* -- C data setters ------------------------------------------------------ */

/* Colwert TValue and set C data value. */
