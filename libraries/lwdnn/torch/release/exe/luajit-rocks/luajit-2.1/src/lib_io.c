...
static IOFileUD *io_file_new(lua_State *L)
{
  ...
  GLwdata *ud = udataV(L->top-1);
  ud->udtype = UDTYPE_IO_FILE;
  /* NOBARRIER: The GLwdata is new (marked white). */
  setgcrefr(ud->metatable, lwrr_func(L)->c.elw);
  iof->fp = NULL;
...
LJLIB_CF(io_method_seek)
{
  ...
  int opt = lj_lib_checkopt(L, 2, 1, "\3set\3lwr\3end");
  int64_t ofs = 0;
  cTValue *o;
  int res;
  if (opt == 0) opt = SEEK_SET;
  else if (opt == 1) opt = SEEK_LWR;
