...
CTSize lj_ctype_size(CTState *cts, CTypeID id)
{
  ...
  return ctype_hassize(ct->info) ? ct->size : CTSIZE_ILWALID;
}
...
CTSize lj_ctype_vlsize(CTState *cts, CType *ct, CTSize nelem)
{
  ...
  /* Callwlate actual size of VLA and check for overflow. */
  xsz += (uint64_t)ct->size * nelem;
  return xsz < 0x80000000u ? (CTSize)xsz : CTSIZE_ILWALID;
}
...
CTInfo lj_ctype_info(CTState *cts, CTypeID id, CTSize *szp)
{
  ...
  for (;;) {
	...
	qual |= CTFP_ALIGNED + CTALIGN(ct->size);
      ...
      *szp = ctype_isfunc(info) ? CTSIZE_ILWALID : ct->size;
      break;
    }
...
/* Colwert int64_t/uint64_t to string with 'LL' or 'ULL' suffix. */
...
/* Colwert complex to string with 'i' or 'I' suffix. */
