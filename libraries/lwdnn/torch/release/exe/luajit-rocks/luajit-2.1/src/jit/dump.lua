...
-- Lookup tables to colwert some literals into names.
local litname = {
  ...
  ["COLW  "] = setmetatable({}, { __index = function(t, mode)
...
-- Relwrsively gather CALL* args and dump them.
...
local function dumpon(opt, outfile)
  ...
  local term = os.getelw("TERM")
  local colormode = (term and term:match("color") or os.getelw("COLORTERM")) and "A" or "T"
  if opt then
  ...
  if not outfile then outfile = os.getelw("LUAJIT_DUMPFILE") end
  if outfile then
