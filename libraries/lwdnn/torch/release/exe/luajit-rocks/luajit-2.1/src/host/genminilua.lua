----------------------------------------------------------------------------
-- Lua script to generate a lwstomized, minified version of Lua.
...
local REMOVE_LIB = {}
gsub([[
collectgarbage dofile gcinfo getfelw getmetatable load print rawequal rawset
select tostring xpcall
foreach foreachi getn maxn setn
popen tmpfile seek setvbuf __tostring
clock date difftime execute getelw rename setlocale time tmpname
...
/* This is a heavily lwstomized and minimized copy of Lua %s. */
...
local function strip_unused2(src)
  return gsub(src, "Symbolic Exelwtion.-}=", "")
end
