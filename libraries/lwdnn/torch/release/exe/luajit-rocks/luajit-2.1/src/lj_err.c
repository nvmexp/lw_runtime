...
**   are colwerted to the string "C++ exception". Lua errors can be caught
**   with catch (...) in C++.
**
...
** undolwmented or just plain wrong on MSDN. Some of it can be gathered
...
typedef struct UndolwmentedDispatcherContext {
  ULONG64 ControlPc;
  ULONG64 ImageBase;
  ...
  ULONG ScopeIndex;
  ULONG Fill0;
} UndolwmentedDispatcherContext;
#else
typedef void *UndolwmentedDispatcherContext;
#endif
...
LJ_FUNCA int lj_err_unwind_win(EXCEPTION_RECORD *rec,
  void *f, CONTEXT *ctx, UndolwmentedDispatcherContext *dispatch)
{
#if LJ_TARGET_X64
  void *cf = f;
#else
...
/* Non-vararg variant for better calling colwentions. */
