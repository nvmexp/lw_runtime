...
#define JIT_PARAMDEF(_) \
  ...
  _(\012, callunroll,	3)	/* Max. unroll for relwrsive calls. */ \
  _(\011, relwnroll,	2)	/* Min. unroll for true relwrsion. */ \
  \
...
typedef enum {
  ...
  LJ_TRLINK_TAILREC,		/* Tail-relwrsion. */
  LJ_TRLINK_UPREC,		/* Up-relwrsion. */
  LJ_TRLINK_DOWNREC,		/* Down-relwrsion. */
...
/* Round-robin backpropagation cache for narrowing colwersions. */
typedef struct BPropEntry {
  IRRef1 key;		/* Key: original reference. */
  IRRef1 val;		/* Value: reference after colwersion. */
  IRRef mode;		/* Mode for this entry (lwrrently IRCOLW_*). */
} BPropEntry;
...
typedef struct FoldState {
  IRIns ins;		/* Lwrrently emitted instruction. */
...
typedef struct jit_State {
  GCtrace lwr;		/* Current trace. */
  GCtrace *lwrfinal;	/* Final address of current trace (set during asm). */
  ...
  IRType1 guardemit;	/* Aclwmulated IRT_GUARD for emitted instructions. */
  ...
  PostProc postproc;	/* Required post-processing after exelwtion. */
#if LJ_SOFTFP || (LJ_32 && LJ_HASFFI)
