...
#define emitcolw(a, dt, st, flags) \
  emitir(IRT(IR_COLW, (dt)), (a), (st)|((dt) << 5)|(flags))
...
/* Colwert CType to IRType (if possible). */
		      ...
		      CTSize step)
{
  if (tref_isk(trlen)) {  /* Length must be constant. */
    ...
    if (tref_isk(trfill) || ml[0].tp != IRT_U8)
      trfill = emitcolw(trfill, IRT_INT, IRT_U8, 0);
	...
	if (tref_isk(trfill))  /* Pointless on x64 with zero-extended regs. */
	  trfill = emitcolw(trfill, IRT_U64, IRT_U32, 0);
...
/* -- Colwert C type to C type -------------------------------------------- */

/*
** This code mirrors the code in lj_ccolw.c. It performs the same steps
** for the trace recorder that lj_ccolw.c does for the interpreter.
**
		       ...
		       void *svisnz)
{
  ...
  /*
  ** Note: Unlike lj_ccolw_ct_ct(), sp holds the _value_ of pointers and
  ...
  */
  ...
  colw_I_I:
    ...
    if (dsize == 8 && ssize < 8 && !(LJ_64 && (sinfo & CTF_UNSIGNED)))
      sp = emitcolw(sp, dt, ssize < 4 ? IRT_INT : st,
		    (sinfo & CTF_UNSIGNED) ? 0 : IRCOLW_SEXT);
    else if (dsize < 8 && ssize == 8)  /* Truncate from 64 bit integer. */
      sp = emitcolw(sp, dsize < 4 ? IRT_INT : dt, st, 0);
    else if (st == IRT_INT)
  ...
  case CCX(I, F):
    if (dt == IRT_CDATA || st == IRT_CDATA) goto err_nyi;
    sp = emitcolw(sp, dsize < 4 ? IRT_INT : dt, st, IRCOLW_ANY);
    goto xstore;
  case CCX(I, P):
  case CCX(I, A):
    ...
    ssize = CTSIZE_PTR;
    st = IRT_UINTP;
    if (((dsize ^ ssize) & 8) == 0) {  /* Must insert no-op type colwersion. */
      sp = emitcolw(sp, dsize < 4 ? IRT_INT : dt, IRT_PTR, 0);
      goto xstore;
    }
    goto colw_I_I;
  ...
  colw_F_I:
    if (dt == IRT_CDATA || st == IRT_CDATA) goto err_nyi;
    sp = emitcolw(sp, dt, ssize < 4 ? IRT_INT : st, 0);
    goto xstore;
  case CCX(F, C):
    sp = emitir(IRT(IR_XLOAD, st), sp, 0);  /* Load re. */
    /* fallthrough */
  ...
  colw_F_F:
    if (dt == IRT_CDATA || st == IRT_CDATA) goto err_nyi;
    if (dt != st) sp = emitcolw(sp, dt, st, 0);
    goto xstore;
  ...
  case CCX(C, F):
    ...
    /* Colwert to re. */
    if ((sinfo & CTF_FP)) goto colw_F_F; else goto colw_F_I;
  ...
  case CCX(C, C):
    if (dt == IRT_CDATA || st == IRT_CDATA) goto err_nyi;
    {
      ...
      if (dt != st) {
	re = emitcolw(re, dt, st, 0);
	im = emitcolw(im, dt, st, 0);
      }
  ...
  case CCX(P, I):
    ...
    if (!LJ_64 && ssize == 8)  /* Truncate from 64 bit integer. */
      sp = emitcolw(sp, IRT_U32, st, 0);
    goto xstore;
  case CCX(P, F):
    if (st == IRT_CDATA) goto err_nyi;
    /* The signed colwersion is cheaper. x64 really has 47 bit pointers. */
    sp = emitcolw(sp, (LJ_64 && dsize == 8) ? IRT_I64 : IRT_U32,
		  st, IRCOLW_ANY);
    goto xstore;
  ...
  err_colw:
  err_nyi:
    lj_trace_err(J, LJ_TRERR_NYICOLW);
    break;
  }
  return 0;
}

/* -- Colwert C type to TValue (load) ------------------------------------- */
...
static TRef crec_tv_ct(jit_State *J, CType *s, CTypeID sid, TRef sp)
{
  ...
  if (ctype_isnum(sinfo)) {
    ...
    if (t == IRT_FLOAT || t == IRT_U32) {  /* Keep uint32_t/float as numbers. */
      return emitcolw(tr, IRT_NUM, t, 0);
...
/* -- Colwert TValue to C type (store) ------------------------------------ */
...
static TRef crec_ct_tv(jit_State *J, CType *d, TRef dp, TRef sp, cTValue *sval)
{
  ...
  if (LJ_LIKELY(tref_isinteger(sp))) {
    ...
    GLwdata *ud = udataV(sval);
    if (ud->udtype == UDTYPE_IO_FILE) {
      TRef tr = emitir(IRT(IR_FLOAD, IRT_U8), sp, IRFL_UDATA_UDTYPE);
      emitir(IRTGI(IR_EQ), tr, lj_ir_kint(J, UDTYPE_IO_FILE));
      sp = emitir(IRT(IR_FLOAD, IRT_PTR), sp, IRFL_UDATA_FILE);
    } else {
      sp = emitir(IRT(IR_ADD, IRT_PTR), sp, lj_ir_kintp(J, sizeof(GLwdata)));
    }
...
/* This would be rather diffilwlt in FOLD, so do it here:
...
again:
  ...
  integer_key:
	...
	idx = emitcolw(idx, IRT_INTP, IRT_INT, IRCOLW_SEXT);
      if (!LJ_64 && ctk->size > sizeof(intptr_t)) {
	idx = emitcolw(idx, IRT_INTP, t, 0);
	lj_needsplit(J);
      }
      goto integer_key;
    }
...
static void crec_alloc(jit_State *J, RecordFFData *rd, CTypeID id)
{
  ...
  if (ctype_isptr(info) || (ctype_isinteger(info) && (sz == 4 || sz == 8))) {
    ...
    TRef trsz = TREF_NIL;
    if ((info & CTF_VLA)) {  /* Callwlate VLA/VLS size at runtime. */
      CTSize sz0, sz1;
      if (!J->base[1] || J->base[2])
	lj_trace_err(J, LJ_TRERR_NYICOLW);  /* NYI: init VLA/VLS. */
    ...
    special:  /* Only handle bulk zero-fill for large/VLA/VLS types. */
      if (J->base[1])
	lj_trace_err(J, LJ_TRERR_NYICOLW);  /* NYI: init large/VLA/VLS types. */
	...
	!lj_ccolw_multi_init(cts, d, &rd->argv[1])) {
      goto single_init;
	...
	if (ctype_isfield(df->info)) {
	  ...
	  if (!(ctype_isnum(dc->info) || ctype_isptr(dc->info) ||
	    ...
	    lj_trace_err(J, LJ_TRERR_NYICOLW);  /* NYI: init aggregates. */
	  if (J->base[i]) {
	    sp = J->base[i];
	    sval = &rd->argv[i];
	    i++;
			   ...
			   CTState *cts, CType *ct)
{
...
#endif
  int ngpr = 0;
  if (ctype_ccolw(ct->info) == CTCC_THISCALL)
    ngpr = 1;
  else if (ctype_ccolw(ct->info) == CTCC_FASTCALL)
    ngpr = 2;
#endif
  ...
  for (n = 0, base = J->base+1, o = rd->argv+1; *base; n++, base++, o++) {
	...
	if ((d->info & CTF_UNSIGNED))
	  tr = emitcolw(tr, IRT_INT, d->size==1 ? IRT_U8 : IRT_U16, 0);
	else
	  tr = emitcolw(tr, IRT_INT, d->size==1 ? IRT_I8 : IRT_I16,IRCOLW_SEXT);
      }
...
static int crec_call(jit_State *J, RecordFFData *rd, GCcdata *cd)
{
  ...
  if (ctype_isfunc(ct->info)) {
    ...
    if ((ct->info & CTF_VARARG)
#if LJ_TARGET_X86
	|| ctype_ccolw(ct->info) != CTCC_CDECL
#endif
	)
	...
	tr = TREF_TRUE;
      ...
      tr = emitcolw(tr, IRT_NUM, t, 0);
    } else if (t == IRT_I8 || t == IRT_I16) {
      tr = emitcolw(tr, IRT_INT, t, IRCOLW_SEXT);
    } else if (t == IRT_U8 || t == IRT_U16) {
      tr = emitcolw(tr, IRT_INT, t, 0);
    }
    J->base[0] = tr;
    J->needsnap = 1;
    return 1;
...
static TRef crec_arith_int64(jit_State *J, TRef *sp, CType **s, MMS mm)
{
  if (sp[0] && sp[1] && ctype_isnum(s[0]->info) && ctype_isnum(s[1]->info)) {
	...
	sp[i] = emitcolw(sp[i], dt, st, IRCOLW_ANY);
      else if (!(st == IRT_I64 || st == IRT_U64))
	sp[i] = emitcolw(sp[i], dt, IRT_INT,
			 (s[i]->info & CTF_UNSIGNED) ? 0 : IRCOLW_SEXT);
    }
    if (mm < MM_add) {
    comp:
...
static TRef crec_arith_ptr(jit_State *J, TRef *sp, CType **s, MMS mm)
{
...
#if LJ_64
	tr = emitcolw(tr, IRT_NUM, IRT_INTP, 0);
#endif
	return tr;
  ...
  }
  {
    ...
    if (t == IRT_NUM || t == IRT_FLOAT)
      tr = emitcolw(tr, IRT_INTP, t, IRCOLW_ANY);
    else if (!(t == IRT_I64 || t == IRT_U64))
      tr = emitcolw(tr, IRT_INTP, IRT_INT,
		    ((t - IRT_I8) & 1) ? 0 : IRCOLW_SEXT);
#else
    if (!tref_typerange(sp[1], IRT_I8, IRT_U32)) {
      tr = emitcolw(tr, IRT_INTP, t,
		    (t == IRT_NUM || t == IRT_FLOAT) ? IRCOLW_ANY : 0);
    }
#endif
...
void LJ_FASTCALL recff_bit64_tobit(jit_State *J, RecordFFData *rd)
{
  ...
  if (!tref_isinteger(tr))
    tr = emitcolw(tr, IRT_INT, tref_type(tr), 0);
  J->base[0] = tr;
}
...
int LJ_FASTCALL recff_bit64_shift(jit_State *J, RecordFFData *rd)
{
  ...
  if (J->base[0] && tref_iscdata(J->base[1])) {
    ...
    if (!tref_isinteger(tsh))
      tsh = emitcolw(tsh, IRT_INT, tref_type(tsh), 0);
    J->base[1] = tsh;
  }
...
TRef recff_bit64_tohex(jit_State *J, RecordFFData *rd, TRef hdr)
{
  ...
  if (id) {
    ...
    tr = emitcolw(tr, IRT_U64, IRT_INT, 0);  /* No sign-extension. */
    lj_needsplit(J);
  }
