...
/* Binary operands of an operator colwerted to ctypes. */
...
static int carith_ptr(lua_State *L, CTState *cts, CDArith *ca, MMS mm)
{
  ...
  if (ctype_isptr(ctp->info) || ctype_isrefarray(ctp->info)) {
	...
	return 1;
      ...
      if (!lj_ccolw_compatptr(cts, ctp, ca->ct[1], CCF_IGNQUAL))
	return 0;
      if (mm == MM_sub) {  /* Pointer difference. */
	intptr_t diff;
	sz = lj_ctype_size(cts, ctype_cid(ctp->info));  /* Element size. */
	if (sz == 0 || sz == CTSIZE_ILWALID)
	  return 0;
	...
	*/
    ...
    if (!((mm == MM_add || mm == MM_sub) && ctype_isnum(ca->ct[1]->info)))
      return 0;
    lj_ccolw_ct_ct(cts, ctype_get(cts, CTID_INT_PSZ), ca->ct[1],
    ...
    lj_ccolw_ct_ct(cts, ctype_get(cts, CTID_INT_PSZ), ca->ct[0],
...
static int carith_int64(lua_State *L, CTState *cts, CDArith *ca, MMS mm)
{
  if (ctype_isnum(ca->ct[0]->info) && ca->ct[0]->size <= 8 &&
    ...
    GCcdata *cd;
    uint64_t u0, u1, *up;
    lj_ccolw_ct_ct(cts, ct, ca->ct[0], (uint8_t *)&u0, ca->p[0], 0);
    if (mm != MM_unm)
      lj_ccolw_ct_ct(cts, ct, ca->ct[1], (uint8_t *)&u1, ca->p[1], 0);
    switch (mm) {
    case MM_eq:
...
static int lj_carith_meta(lua_State *L, CTState *cts, CDArith *ca, MMS mm)
{
  ...
  if (!tv) {
    ...
    if ((isenum ^ isstr) == 1)
      lj_err_callerv(L, LJ_ERR_FFI_BADCOLW, repr[isstr], repr[isenum]);
...
uint64_t lj_carith_check64(lua_State *L, int narg, CTypeID *id)
{
  ...
  err:
    ...
    lj_ccolw_ct_ct(cts, ctype_get(cts, *id), s,
