...
#define IRFLDEF(_) \
  ...
  _(FUNC_ELW,	offsetof(GCfunc, l.elw)) \
  _(FUNC_PC,	offsetof(GCfunc, l.pc)) \
  _(FUNC_FFID,	offsetof(GCfunc, l.ffid)) \
  _(THREAD_ELW,	offsetof(lua_State, elw)) \
  ...
  _(UDATA_META,	offsetof(GLwdata, metatable)) \
  _(UDATA_UDTYPE, offsetof(GLwdata, udtype)) \
  _(UDATA_FILE,	sizeof(GLwdata)) \
...
#define IRSLOAD_COLWERT		0x08	/* Number to integer colwersion. */
...
/* COLW mode, stored in op2. */
#define IRCOLW_SRCMASK		0x001f	/* Source IRType. */
#define IRCOLW_DSTMASK		0x03e0	/* Dest. IRType (also in ir->t). */
#define IRCOLW_DSH		5
#define IRCOLW_NUM_INT		((IRT_NUM<<IRCOLW_DSH)|IRT_INT)
#define IRCOLW_INT_NUM		((IRT_INT<<IRCOLW_DSH)|IRT_NUM)
#define IRCOLW_SEXT		0x0800	/* Sign-extend integer to integer. */
#define IRCOLW_MODEMASK		0x0fff
#define IRCOLW_COLWMASK		0xf000
#define IRCOLW_CSH		12
/* Number to integer colwersion mode. Ordered by strength of the checks. */
#define IRCOLW_TOBIT  (0<<IRCOLW_CSH)	/* None. Cache only: TOBIT colw. */
#define IRCOLW_ANY    (1<<IRCOLW_CSH)	/* Any FP number is ok. */
#define IRCOLW_INDEX  (2<<IRCOLW_CSH)	/* Check + special backprop rules. */
#define IRCOLW_CHECK  (3<<IRCOLW_CSH)	/* Number checked for integerness. */

/* TOSTR mode, stored in op2. */
#define IRTOSTR_INT		0	/* Colwert integer to string. */
#define IRTOSTR_NUM		1	/* Colwert number to string. */
#define IRTOSTR_CHAR		2	/* Colwert char value to string. */
...
** a TValue after implicit or explicit colwersion. Their types must be
...
** - CSE callwlates the maximum reference of two operands.
