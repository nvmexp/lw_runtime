...
#define otherwhite(g)	(g->gc.lwrrentwhite ^ LJ_GC_WHITES)
#define isdead(g, v)	((v)->gch.marked & otherwhite(g) & LJ_GC_WHITES)

#define lwrwhite(g)	((g)->gc.lwrrentwhite & LJ_GC_WHITES)
#define newwhite(g, x)	(obj2gco(x)->gch.marked = (uint8_t)lwrwhite(g))
#define makewhite(g, x) \
  ((x)->gch.marked = ((x)->gch.marked & (uint8_t)~LJ_GC_COLORS) | lwrwhite(g))
...
LJ_FUNC void lj_gc_closeuv(global_State *g, GLwpval *uv);
#if LJ_HASJIT
