...
/* The current trace is first assembled in J->lwr. The variable length
...
#define TRACE_APPENDVEC(field, szfield, tp) \
  T->field = (tp *)p; \
  memcpy(p, J->lwr.field, J->lwr.szfield*sizeof(tp)); \
  p += J->lwr.szfield*sizeof(tp);
...
{
  size_t sztr = ((sizeof(GCtrace)+7)&~7);
  size_t szins = (J->lwr.nins-J->lwr.nk)*sizeof(IRIns);
  char *p = (char *)T + sztr;
  memcpy(T, &J->lwr, sizeof(GCtrace));
  setgcrefr(T->nextgc, J2G(J)->gc.root);
  setgcrefp(J2G(J)->gc.root, T);
  newwhite(J2G(J), T);
  T->gct = ~LJ_TTRACE;
  T->ir = (IRIns *)p - J->lwr.nk;  /* The IR has already been copied above. */
  p += szins;
...
int lj_trace_flushall(lua_State *L)
{
  ...
  /* Free the whole machine code and ilwalidate all exit stub groups. */
  lj_mcode_free(J);
...
void lj_trace_freestate(global_State *g)
{
  ...
  {  /* This assumes all traces have already been freed. */
    ...
    for (i = 1; i < (ptrdiff_t)J->sizetrace; i++)
      lua_assert(i == (ptrdiff_t)J->lwr.traceno || traceref(J, i) == NULL);
  }
#endif
  lj_mcode_free(J);
...
static void trace_start(jit_State *J)
{
  ...
  setgcrefp(J->trace[traceno], &J->lwr);

  /* Setup enough of the current trace to be able to send the vmevent. */
  memset(&J->lwr, 0, sizeof(GCtrace));
  J->lwr.traceno = traceno;
  J->lwr.nins = J->lwr.nk = REF_BASE;
  J->lwr.ir = J->irbuf;
  J->lwr.snap = J->snapbuf;
  J->lwr.snapmap = J->snapmapbuf;
  J->mergesnap = 0;
  J->needsnap = 0;
  J->bcskip = 0;
  J->guardemit.irt = 0;
  ...
  J->retryrec = 0;
  J->ktrace = 0;
  setgcref(J->lwr.startpt, obj2gco(J->pt));
...
{
  BCIns *pc = mref(J->lwr.startpc, BCIns);
  BCOp op = bc_op(J->lwr.startins);
  GCproto *pt = &gcref(J->lwr.startpt)->pt;
  TraceNo traceno = J->lwr.traceno;
  GCtrace *T = J->lwrfinal;
  lua_State *L;

  switch (op) {
  case BC_FORL:
    setbc_op(pc+bc_j(J->lwr.startins), BC_JFORI);  /* Patch FORI, too. */
    /* fallthrough */
  case BC_LOOP:
  case BC_ITERL:
  case BC_FUNCF:
  ...
  addroot:
    ...
    J->lwr.nextroot = pt->trace;
    pt->trace = (TraceNo1)traceno;
    break;
  case BC_RET:
  case BC_RET0:
  case BC_RET1:
    *pc = BCINS_AD(BC_JLOOP, J->lwr.snap[0].nslots, traceno);
    goto addroot;
  case BC_JMP:
    /* Patch exit branch in parent to side trace entry. */
    lua_assert(J->parent != 0 && J->lwr.root != 0);
    lj_asm_patchexit(J, traceref(J, J->parent), J->exitno, J->lwr.mcode);
    /* Avoid compiling a side trace twice (stack resizing uses parent exit). */
    traceref(J, J->parent)->snap[J->exitno].count = SNAPCOUNT_DONE;
    /* Add to side trace chain in root trace. */
    {
      GCtrace *root = traceref(J, J->lwr.root);
      root->nchild++;
      J->lwr.nextside = root->nextside;
...
/* Start a new root trace for down-relwrsion. */
static int trace_downrec(jit_State *J)
{
  ...
  if (bc_op(*J->pc) == BC_RETM)
    return 0;  /* NYI: down-relwrsion with RETM. */
  J->parent = 0;
  J->exitno = 0;
...
static int trace_abort(jit_State *J)
{
  ...
  if (J->parent == 0 && !bc_isret(bc_op(J->lwr.startins))) {
    if (J->exitno == 0) {
      BCIns *startpc = mref(J->lwr.startpc, BCIns);
      if (e == LJ_TRERR_RETRY)
	hotcount_set(J2GG(J), startpc+1, 1);  /* Immediate retry. */
      else
	penalty_pc(J, &gcref(J->lwr.startpt)->pt, startpc, e);
    } else {
  ...
  traceno = J->lwr.traceno;
  if (traceno) {
    ptrdiff_t errobj = savestack(L, L->top-1);  /* Stack may be resized. */
    J->lwr.link = 0;
    J->lwr.linktype = LJ_TRLINK_NONE;
    lj_vmevent_send(L, TRACE,
      TValue *frame;
      const BCIns *pc;
      GCfunc *fn;
...
static TValue *trace_state(lua_State *L, lua_CFunction dummy, void *ud)
{
  ...
  retry:
    ...
    case LJ_TRACE_END:
	  ...
	  J->lwr.link == J->lwr.traceno && J->framedepth + J->retdepth == 0) {
	...
	J->loopref = J->chain[IR_LOOP];  /* Needed by assembler. */
      ...
      lj_opt_split(J);
      lj_opt_sink(J);
      if (!J->loopref) J->lwr.snap[J->lwr.nsnap-1].count = SNAPCOUNT_DONE;
      J->state = LJ_TRACE_ASM;
      break;

    case LJ_TRACE_ASM:
      setvmstate(J2G(J), ASM);
      lj_asm_trace(J, &J->lwr);
      trace_stop(J);
...
/* A bytecode instruction is about to be exelwted. Record it. */
...
void LJ_FASTCALL lj_trace_stitch(jit_State *J, const BCIns *pc)
{
  ...
  if (J->state == LJ_TRACE_IDLE &&
    ...
    /* J->exitno is set to the ilwoking trace. */
    J->state = LJ_TRACE_START;
    lj_trace_ins(J, pc);
  }
}
