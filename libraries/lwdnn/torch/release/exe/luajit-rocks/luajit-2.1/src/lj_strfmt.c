...
/* -- Unformatted colwersions to buffer ----------------------------------- */
...
SBuf *lj_strfmt_putfxint(SBuf *sb, SFormat sf, uint64_t k)
{
  ...
  /* Colwert number and store to fixed-size buffer in reverse order. */
  prec = STRFMT_PREC(sf);
...
SBuf *lj_strfmt_putfnum_int(SBuf *sb, SFormat sf, lua_Number n)
{
  ...
  if (checki32(k) && sf == STRFMT_INT)
    return lj_strfmt_putint(sb, (int32_t)k);  /* Shortlwt for plain %d. */
  else
...
/* Colwert integer or number to string. */
...
/* Raw colwersion of object to string. */
