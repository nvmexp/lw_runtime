...
static void asm_guard(ASMState *as, MIPSIns mi, Reg rs, Reg rt)
{
  ...
  if (LJ_UNLIKELY(p == as->ilwmcp)) {
    ...
    as->loopilw = 1;
    as->mcp = p+1;
    mi = mi ^ ((mi>>28) == 1 ? 0x04000000u : 0x00010000u);  /* Ilwert cond. */
...
static void asm_colw(ASMState *as, IRIns *ir)
{
...
#endif
#if LJ_32 && LJ_SOFTFP
  /* FP colwersions are handled by SPLIT. */
  lua_assert(!irt_isfp(ir->t) && !(st == IRT_NUM || st == IRT_FLOAT));
  /* Can't check for same types: SPLIT uses COLW int.int + BXOR for sfp NEG. */
#else
  ...
  if (irt_isfp(ir->t)) {
    ...
    } else if (st == IRT_U32) {  /* U32 to FP colwersion. */
...
#if LJ_64
    } else if(st == IRT_U64) {  /* U64 to FP colwersion. */
    ...
    if (irt_isguard(ir->t)) {
      ...
      Reg tmp = ra_scratch(as, rset_exclude(RSET_FPR, left));
      if (irt_isu32(ir->t)) {  /* FP to U32 colwersion. */
	...
	if (st == IRT_FLOAT)
	  ...
	  emit_lsptr(as, MIPSI_LDC1, (tmp & 31),
		     (void *)&as->J->k64[LJ_K64_2P31], RSET_GPR);
#if LJ_64
      } else if (irt_isu64(ir->t)) {  /* FP to U64 colwersion. */
	MCLabel l_end;
	emit_tg(as, MIPSI_DMFC1, dest, tmp);
	l_end = emit_label(as);
	/* For inputs >= 2^63 add -2^64 and colwert again. */
	if (st == IRT_NUM) {
...
#endif
  {
	...
	if (st64) {
	  ...
	  Reg left = ra_alloc1(as, lref, RSET_GPR);
	  if ((ir->op2 & IRCOLW_SEXT)) {  /* 32 to 64 bit sign extension. */
...
static void asm_uref(ASMState *as, IRIns *ir)
{
  ...
  if (irref_isk(ir->op1)) {
    ...
    if (ir->o == IR_UREFC) {
      ...
      emit_tsi(as, MIPSI_AADDIU, dest, uv, (int32_t)offsetof(GLwpval, tv));
      emit_tsi(as, MIPSI_LBU, RID_TMP, uv, (int32_t)offsetof(GLwpval, closed));
    } else {
      emit_tsi(as, MIPSI_AL, dest, uv, (int32_t)offsetof(GLwpval, v));
    }
...
static void asm_sload(ASMState *as, IRIns *ir)
{
...
#if LJ_32 && LJ_SOFTFP
  lua_assert(!(ir->op2 & IRSLOAD_COLWERT));  /* Handled by LJ_SOFTFP SPLIT. */
  ...
  if (ra_used(ir)) {
    ...
    rset_clear(allow, base);
    if (!LJ_SOFTFP && (ir->op2 & IRSLOAD_COLWERT)) {
      if (irt_isint(t)) {
...
static void asm_hiop(ASMState *as, IRIns *ir)
{
  ...
  if ((ir-1)->o == IR_COLW) {  /* Colwersions to/from 64 bit. */
    ...
    as->lwrins--;  /* Always skip the loword comparison. */
#if LJ_SOFTFP
    if (!irt_isint(ir->t)) {
      asm_sfpcomp(as, ir-1);
      return;
    ...
    return;
      ...
      as->lwrins--;  /* Always skip the loword min/max. */
    if (uselo || usehi)
      asm_sfpmin_max(as, ir-1);
    return;
#endif
  } else if ((ir-1)->o == IR_XSTORE) {
    as->lwrins--;  /* Handle both stores here. */
    if ((ir-1)->r != RID_SINK) {
  ...
  switch ((ir-1)->o) {
#if LJ_HASFFI
  case IR_ADD: as->lwrins--; asm_add64(as, ir); break;
  case IR_SUB: as->lwrins--; asm_sub64(as, ir); break;
  case IR_NEG: as->lwrins--; asm_neg64(as, ir); break;
#endif
#if LJ_SOFTFP
...
static void asm_loop_fixup(ASMState *as)
{
  ...
  if (as->loopilw) {  /* Ilwerted loop branch? */
    /* asm_guard already ilwerted the cond branch. Only patch the target. */
...
static void asm_tail_prep(ASMState *as)
{
  ...
  as->ilwmcp = as->loopref ? as->mcp : NULL;
}
...
void lj_asm_patchexit(jit_State *J, GCtrace *T, ExitNo exitno, MCode *target)
{
  ...
  for (p++; p < pe; p++) {
	...
	/* Patch NOP after code for ilwerted loop branch. Use of J is ok. */
