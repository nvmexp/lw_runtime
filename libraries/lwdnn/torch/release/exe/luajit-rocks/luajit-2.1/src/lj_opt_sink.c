...
/* Relwrsively check whether a value depends on a PHI. */
...
/* Check whether a value is a sinkable PHI or loop-ilwariant. */
static int sink_checkphi(jit_State *J, IRIns *ira, IRRef ref)
{
  if (ref >= REF_FIRST) {
    IRIns *ir = IR(ref);
    if (irt_isphi(ir->t) || (ir->o == IR_COLW && ir->op2 == IRCOLW_NUM_INT &&
			     irt_isphi(IR(ir->op1)->t))) {
      ira->prev++;
      return 1;  /* Sinkable PHI. */
    }
    /* Otherwise the value must be loop-ilwariant. */
...
{
  IRIns *ir, *irlast = IR(J->lwr.nins-1);
  for (ir = irlast ; ; ir--) {
    switch (ir->o) {
    case IR_BASE:
      return;  /* Finished. */
...
static void sink_remark_phi(jit_State *J)
{
  ...
  do {
    remark = 0;
    for (ir = IR(J->lwr.nins-1); ir->o == IR_PHI; ir--) {
      IRIns *irl = IR(ir->op1), *irr = IR(ir->op2);
...
static void sink_sweep_ins(jit_State *J)
{
  ...
  for (ir = IR(J->lwr.nins-1) ; ir >= irbase; ir--) {
    switch (ir->o) {
    case IR_ASTORE: case IR_HSTORE: case IR_FSTORE: case IR_XSTORE: {
      IRIns *ira = sink_checkalloc(J, ir);
	...
	ir->t.irt &= ~IRT_GUARD;
	ir->prev = REGSP(RID_SINK, 0);
	J->lwr.sinktags = 1;  /* Signal present SINK tags to assembler. */
      } else {
	irt_clearmark(ir->t);
	ir->prev = REGSP_INIT;
      }
  ...
  for (ir = IR(J->lwr.nk); ir < irbase; ir++) {
    irt_clearmark(ir->t);
    ir->prev = REGSP_INIT;
...
void lj_opt_sink(jit_State *J)
{
  ...
  if ((J->flags & need) == need &&
    ...
    if (!J->loopref)
      sink_mark_snap(J, &J->lwr.snap[J->lwr.nsnap-1]);
    sink_mark_ins(J);
    if (J->loopref)
      sink_remark_phi(J);
    sink_sweep_ins(J);
