...
{
  IRRef i, nins = J->lwr.nins, nk = J->lwr.nk;
...
static void rec_check_slots(jit_State *J)
{
  ...
  for (s = 0; s < nslots; s++) {
	...
	lua_assert(ref >= J->lwr.nk && ref < J->lwr.nins);
	ir = IR(ref);
...
static TRef getlwrrf(jit_State *J)
{
  if (J->base[-1-LJ_FR2])
...
int lj_record_objcmp(jit_State *J, TRef a, TRef b, cTValue *av, cTValue *bv)
{
  ...
  if (!tref_isk2(a, b)) {  /* Shortlwt, also handles primitives. */
	...
	a = emitir(IRTN(IR_COLW), a, IRCOLW_NUM_INT);
	ta = IRT_NUM;
      } else if (ta == IRT_NUM && tb == IRT_INT) {
	b = emitir(IRTN(IR_COLW), b, IRCOLW_NUM_INT);
      } else {
...
/* Canonicalize slots: colwert integers to numbers. */
static void canonicalize_slots(jit_State *J)
{
  ...
  for (s = J->baseslot+J->maxslot-1; s >= 1; s--) {
	...
	J->slot[s] = emitir(IRTN(IR_COLW), tr, IRCOLW_NUM_INT);
    }
  }
}
...
#endif
  lj_trace_end(J);
  J->lwr.linktype = (uint8_t)linktype;
  J->lwr.link = (uint16_t)lnk;
  /* Looping back at the same stack level? */
  if (lnk == J->lwr.traceno && J->framedepth + J->retdepth == 0) {
    if ((J->flags & JIT_F_OPT_LOOP))  /* Shall we try to create a loop? */
      goto nocanon;  /* Do not canonicalize or we lose the narrowing. */
    if (J->lwr.root)  /* Otherwise ensure we always link to the root trace. */
      J->lwr.link = J->lwr.root;
  }
...
/* Load and optionally colwert a FORI argument from a slot. */
static TRef fori_load(jit_State *J, BCReg slot, IRType t, int mode)
{
  int colw = (tvisint(&J->L->base[slot]) != (t==IRT_INT)) ? IRSLOAD_COLWERT : 0;
  return sloadt(J, (int32_t)slot,
		t + (((mode & IRSLOAD_TYPECHECK) ||
		      (colw && t == IRT_INT && !(mode >> 16))) ?
		     IRT_GUARD : 0),
		mode + colw);
}
...
static LoopEvent rec_for(jit_State *J, const BCIns *fori, int isforl)
{
  ...
  if (isforl) {  /* Handle FORL/JFORL opcodes. */
	...
	if (!tref_isinteger(tr[i]))
	  tr[i] = emitir(IRTGI(IR_COLW), tr[i], IRCOLW_INT_NUM|IRCOLW_CHECK);
      } else {
	if (!tref_isnum(tr[i]))
	  tr[i] = emitir(IRTN(IR_COLW), tr[i], IRCOLW_NUM_INT);
      }
    }
...
static void rec_loop_interp(jit_State *J, const BCIns *pc, LoopEvent ev)
{
  if (J->parent == 0 && J->exitno == 0) {
	...
	lj_trace_err(J, LJ_TRERR_LLEAVE);
      lj_record_stop(J, LJ_TRLINK_LOOP, J->lwr.traceno);  /* Looping trace. */
	...
	lj_trace_err(J, LJ_TRERR_LINNER);  /* Root trace hit an inner loop. */
      if ((ev != LOOPEV_ENTERLO &&
	   J->loopref && J->lwr.nins - J->loopref > 24) || --J->loopunroll < 0)
	lj_trace_err(J, LJ_TRERR_LUNROLL);  /* Limit loop unrolling. */
      J->loopref = J->lwr.nins;
    }
...
static void rec_loop_jit(jit_State *J, TraceNo lnk, LoopEvent ev)
{
  if (J->parent == 0 && J->exitno == 0) {  /* Root trace hit an inner loop. */
    ...
    if (J->pc == J->startpc && J->framedepth + J->retdepth == 0)
      lj_record_stop(J, LJ_TRLINK_LOOP, J->lwr.traceno);  /* Form extra loop. */
    else
...
/* Check unroll limits for down-relwrsion. */
static int check_downrec_unroll(jit_State *J, GCproto *pt)
{
  ...
  for (ptref = J->chain[IR_KGC]; ptref; ptref = IR(ptref)->prev)
	...
	if (IR(ref)->op1 == ptref)
      ...
      if (count) {
	if (J->pc == J->startpc) {
	  if (count + J->tailcalled > J->param[JIT_P_relwnroll])
	    return 1;
	} else {
...
void lj_record_ret(jit_State *J, BCReg rbase, ptrdiff_t gotresults)
{
  ...
  if (frame_islua(frame)) {  /* Return to Lua frame. */
	...
	lj_snap_purge(J);
	lj_record_stop(J, LJ_TRLINK_DOWNREC, J->lwr.traceno);  /* Down-rec. */
	return;
      }
      lj_snap_add(J);
    }
...
{
  BCReg s, top = cont == lj_cont_cat ? J->maxslot : lwrr_proto(J->L)->framesize;
#if LJ_FR2
...
{
  /* Try to emit ilwariant bounds checks. */
  if ((J->flags & (JIT_F_OPT_LOOP|JIT_F_OPT_ABC)) ==
	...
	/* Emit ilwariant bounds check for stop. */
	emitir(IRTG(IR_ABC, IRT_P32), asizeref, ofs == 0 ? J->scev.stop :
	       emitir(IRTI(IR_ADD), J->scev.stop, ofsref));
	/* Emit ilwariant bounds check for start, if not const or negative. */
  ...
  *rbref = 0;
  ...
  if (tref_isnumber(key)) {
    ...
    if ((MSize)k < LJ_MAX_ASIZE) {  /* Potential array key? */
      ...
      if ((MSize)k < t->asize) {  /* Lwrrently an array key? */
	TRef arrayref;
	rec_idx_abc(J, asizeref, ikey, t->asize);
	arrayref = emitir(IRT(IR_FLOAD, IRT_PGC), ix->tab, IRFL_TAB_ARRAY);
	return emitir(IRT(IR_AREF, IRT_PGC), arrayref, ikey);
      } else {  /* Lwrrently not in array (may be an array extension)? */
	emitir(IRTGI(IR_ULE), asizeref, ikey);  /* Ilw. bounds check. */
	if (k == 0 && tref_isk(key))
  ...
  if (t->hmask == 0) {  /* Shortlwt for empty hash part. */
  ...
  if (tref_isinteger(key))  /* Hash keys are based on numbers, not ints. */
    key = emitir(IRTN(IR_COLW), key, IRCOLW_NUM_INT);
  if (tref_isk(key)) {
	...
	hslot <= 65535*(MSize)sizeof(Node)) {
      TRef node, kslot, hm;
      *rbref = J->lwr.nins;  /* Mark possible rollback point. */
      *rbguard = J->guardemit;
...
TRef lj_record_idx(jit_State *J, RecordIndex *ix)
{
  ...
  if (ix->val == 0) {  /* Indexed load */
    ...
    /* Colwert int to number before storing. */
    if (!LJ_DUALNUM && tref_isinteger(ix->val))
      ix->val = emitir(IRTN(IR_COLW), ix->val, IRCOLW_NUM_INT);
    emitir(IRT(loadop+IRDELTA_L2S, tref_type(ix->val)), xref, ix->val);
    if (keybarrier || tref_isgcv(ix->val))
      emitir(IRT(IR_TBAR, IRT_NIL), ix->tab, 0);
    /* Ilwalidate neg. metamethod cache for stores with certain string keys. */
    if (!nommstr(J, ix->key)) {
...
static int rec_upvalue_constify(jit_State *J, GLwpval *uvp)
{
  if (uvp->immutable) {
    cTValue *o = uvval(uvp);
...
{
  GLwpval *uvp = &gcref(J->fn->l.uvptr[uv])->uv;
  TRef fn = getlwrrf(J);
  IRRef uref;
  int needbarrier = 0;
...
noconstify:
  ...
  if (val == 0) {  /* Upvalue load */
    ...
    if (!LJ_DUALNUM && tref_isinteger(val))
      val = emitir(IRTN(IR_COLW), val, IRCOLW_NUM_INT);
...
static void check_call_unroll(jit_State *J, TraceNo lnk)
{
  ...
  if (J->pc == J->startpc) {
	...
	lj_record_stop(J, LJ_TRLINK_TAILREC, J->lwr.traceno);  /* Tail-rec. */
      else
	lj_record_stop(J, LJ_TRLINK_UPREC, J->lwr.traceno);  /* Up-relwrsion. */
    }
  } else {
    if (count > J->param[JIT_P_callunroll]) {
      if (lnk) {  /* Possible tail- or up-relwrsion. */
	lj_trace_flush(J, lnk);  /* Flush trace that only returns. */
	/* Set a small, pseudo-random hotcount for a quick retry of JFUNC*. */
	hotcount_set(J2GG(J), J->pc+1, LJ_PRNG_BITS(J, 4));
      }
      lj_trace_err(J, LJ_TRERR_LWNROLL);
    }
  }
}
...
static void rec_func_jit(jit_State *J, TraceNo lnk)
{
  ...
  if (J->pc == J->startpc && J->framedepth + J->retdepth == 0)
    lj_record_stop(J, LJ_TRLINK_TAILREC, J->lwr.traceno);  /* Extra tail-rec. */
  else
...
static void rec_varg(jit_State *J, BCReg dst, ptrdiff_t nresults)
{
  ...
  ptrdiff_t lwararg = frame_delta(J->L->base-1) - numparams - 1 - LJ_FR2;
  lua_assert(frame_isvarg(J->L->base-1));
  if (LJ_FR2 && dst > J->maxslot)
    J->base[dst-1] = 0;  /* Prevent resurrection of unrelated slot. */
  if (J->framedepth > 0) {  /* Simple case: varargs defined on-trace. */
    ptrdiff_t i;
    if (lwararg < 0) lwararg = 0;
    if (nresults == -1) {
      nresults = lwararg;
      J->maxslot = dst + (BCReg)lwararg;
    } else if (dst + nresults > J->maxslot) {
      J->maxslot = dst + (BCReg)nresults;
    }
    for (i = 0; i < nresults; i++)
      J->base[dst+i] = i < lwararg ? getslot(J, i - lwararg - 1 - LJ_FR2) : TREF_NIL;
    ...
    if (nresults >= 0) {  /* Known fixed number of results. */
      ptrdiff_t i;
      if (lwararg > 0) {
	ptrdiff_t nload = lwararg >= nresults ? nresults : lwararg;
	TRef vbase;
	if (lwararg >= nresults)
	...
	for (i = 0; i < nload; i++) {
	  IRType t = itype2irt(&J->L->base[i-1-LJ_FR2-lwararg]);
	...
	tridx = emitir(IRTGI(IR_COLW), tridx, IRCOLW_INT_NUM|IRCOLW_INDEX);
      if (idx != 0 && tref_isk(tridx)) {
	emitir(IRTGI(idx <= lwararg ? IR_GE : IR_LT),
	       fr, lj_ir_kint(J, frofs+8*(int32_t)idx));
	frofs -= 8;  /* Bias for 1-based index. */
      } else if (idx <= lwararg) {  /* Compute size. */
	...
	if (idx != 0) {
	  ...
	  rec_idx_abc(J, tr, tridx, (uint32_t)lwararg);
	}
      } else {
	...
	emitir(IRTGI(IR_LT), fr, tmp);
      ...
      if (idx != 0 && idx <= lwararg) {
	IRType t;
	TRef aref, vbase = emitir(IRT(IR_SUB, IRT_IGC), REF_BASE, fr);
	vbase = emitir(IRT(IR_ADD, IRT_PGC), vbase,
		       lj_ir_kint(J, frofs-(8<<LJ_FR2)));
	t = itype2irt(&J->L->base[idx-2-LJ_FR2-lwararg]);
...
static TRef rec_cat(jit_State *J, BCReg baseslot, BCReg topslot)
{
  ...
  if (tref_isnumber_str(top[0]) && tref_isnumber_str(top[-1])) {
    ...
    /* First colwert numbers to strings. */
...
static void rec_comp_prep(jit_State *J)
{
  ...
  if (J->lwr.nsnap == 1 && J->lwr.snap[0].ref == J->lwr.nins)
...
static void rec_comp_fixup(jit_State *J, const BCIns *pc, int cond)
{
  ...
  SnapShot *snap = &J->lwr.snap[J->lwr.nsnap-1];
  /* Set PC to opposite target to avoid re-recording the comp. in side trace. */
#if LJ_FR2
  SnapEntry *flink = &J->lwr.snapmap[snap->mapofs + snap->nent];
  uint64_t pcbase;
  memcpy(&pcbase, flink, sizeof(uint64_t));
  pcbase = (pcbase & 0xff) | (u64ptr(npc) << 8);
  memcpy(flink, &pcbase, sizeof(uint64_t));
#else
  J->lwr.snapmap[snap->mapofs + snap->nent] = SNAP_MKPC(npc);
#endif
  J->needsnap = 1;
...
/* Record the next bytecode instruction (_before_ it's exelwted). */
void lj_record_ins(jit_State *J)
{
  ...
  if (LJ_UNLIKELY(J->postproc != LJ_POST_NONE)) {
	...
	if (J->postproc == LJ_POST_FIXGUARDSNAP) {
	  SnapShot *snap = &J->lwr.snap[J->lwr.nsnap-1];
	  J->lwr.snapmap[snap->mapofs+snap->nent-1]--;  /* False -> true. */
	}
      }
...
#define rcv	(&ix.keyv)
  ...
  case BC_ISLT: case BC_ISGE: case BC_ISLE: case BC_ISGT:
	...
	if (ta == IRT_INT && tc == IRT_NUM) {
	  ra = emitir(IRTN(IR_COLW), ra, IRCOLW_NUM_INT);
	  ta = IRT_NUM;
  ...
  case BC_ADDLW: case BC_SUBLW: case BC_MULLW: case BC_DIVLW: case BC_MODLW:
  ...
  case BC_GGET: case BC_GSET:
    settabV(J->L, &ix.tabv, tabref(J->fn->l.elw));
    ix.tab = emitir(IRT(IR_FLOAD, IRT_TAB), getlwrrf(J), IRFL_FUNC_ELW);
...
#undef rcv
  ...
  if (J->lwr.nins > REF_FIRST+(IRRef)J->param[JIT_P_maxrecord])
...
static const BCIns *rec_setup_root(jit_State *J)
{
  ...
  case BC_RET0:
  case BC_RET1:
    /* No bytecode range check for down-relwrsive root traces. */
...
void lj_record_setup(jit_State *J)
{
  ...
  J->baseslot = 1+LJ_FR2;  /* Ilwoking function is at base[-1-LJ_FR2]. */
  ...
  J->startpc = J->pc;
  setmref(J->lwr.startpc, J->pc);
  if (J->parent) {  /* Side trace. */
    GCtrace *T = traceref(J, J->parent);
    TraceNo root = T->root ? T->root : J->parent;
    J->lwr.root = (uint16_t)root;
    J->lwr.startins = BCINS_AD(BC_JMP, 0, 0);
    ...
    lj_snap_replay(J, T);
  sidecheck:
    if (traceref(J, J->lwr.root)->nchild >= J->param[JIT_P_maxside] ||
    ...
    */
    lj_snap_add(J);
    if (bc_op(J->lwr.startins) == BC_FORL)
      rec_for_loop(J, J->pc-1, &J->scev, 1);
    else if (bc_op(J->lwr.startins) == BC_ITERC)
      J->startpc = NULL;
  ...
  ** Use this only if you want to *asynchronously* interrupt the exelwtion.
  **
