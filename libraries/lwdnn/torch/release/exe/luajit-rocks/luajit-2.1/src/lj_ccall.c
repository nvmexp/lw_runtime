...
/* -- Windows/x64 calling colwentions ------------------------------------- */
...
/* Relwrsively classify a struct based on its fields. */
static int ccall_classify_struct(CTState *cts, CType *ct, int *rcl, CTSize ofs)
{
			    ...
			    TValue *o, int narg)
{
  GPRArg dp[2];
  dp[0] = dp[1] = 0;
  /* Colwert to temp. struct. */
  lj_ccolw_ct_tv(cts, d, (uint8_t *)dp, o, CCF_ARG(narg));
			     ...
			     CCallState *cc, int *ret)
{
  ...
  *ret = 1;  /* One result. */
  ...
  return lj_ccolw_tv_ct(cts, ctr, 0, L->top-1, sp);
}
...
int lj_ccall_func(lua_State *L, GCcdata *cd)
{
  ...
  if (ctype_isfunc(ct->info)) {
    ...
    if (cc.spadj && ctype_ccolw(ct->info) == CTCC_CDECL) {
      CTF_INSERT(ct->info, CCOLW, CTCC_STDCALL);
      lj_trace_abort(G(L));
    }
#endif
    while (gcsteps-- > 0)
