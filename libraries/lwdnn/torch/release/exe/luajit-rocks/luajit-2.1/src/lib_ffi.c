...
static void *ffi_checkptr(lua_State *L, int narg, CTypeID id)
{
  ...
  lj_ccolw_ct_tv(cts, ctype_get(cts, id), (uint8_t *)&p, o, CCF_ARG(narg));
  return p;
}

/* Colwert argument to int32_t. */
static int32_t ffi_checkint(lua_State *L, int narg)
{
  ...
  if (o >= L->top)
    lj_err_arg(L, narg, LJ_ERR_NOVAL);
  lj_ccolw_ct_tv(cts, ctype_get(cts, CTID_INT32), (uint8_t *)&i, o,
		 CCF_ARG(narg));
  return i;
}
...
{
  MMS mm = (MMS)(lwrr_func(L)->c.ffid - (int)FF_ffi_meta___eq + (int)MM_eq);
...
LJLIB_CF(ffi_clib___index)	LJLIB_REC(clib_index 1)
{
  ...
  if (tviscdata(tv)) {
    ...
    if (ctype_isextern(s->info)) {
      ...
      if (lj_ccolw_tv_ct(cts, ct, sid, L->top-1, sp))
	lj_gc_check(L);
      return 1;
    }
  }
...
LJLIB_CF(ffi_clib___newindex)	LJLIB_REC(clib_index 0)
{
  ...
  if (o < L->top && tviscdata(tv)) {
	...
	lj_ccolw_ct_tv(cts, d, *(void **)cdataptr(cd), o, 0);
	return 0;
      }
    }
  }
...
LJLIB_CF(ffi_new)	LJLIB_REC(.)
{
  ...
  if (sz == CTSIZE_ILWALID)
    lj_err_arg(L, 1, LJ_ERR_FFI_ILWSIZE);
  cd = lj_cdata_newx(cts, id, sz, info);
  setcdataV(L, o-1, cd);  /* Anchor the uninitialized cdata. */
  lj_ccolw_ct_init(cts, ct, sz, cdataptr(cd),
...
LJLIB_CF(ffi_cast)	LJLIB_REC(ffi_new)
{
  ...
  if (!(ctype_isnum(d->info) || ctype_isptr(d->info) || ctype_isenum(d->info)))
    lj_err_arg(L, 1, LJ_ERR_FFI_ILWTYPE);
  if (!(tviscdata(o) && cdataV(o)->ctypeid == id)) {
    GCcdata *cd = lj_cdata_new(cts, id, d->size);
    lj_ccolw_ct_tv(cts, d, cdataptr(cd), o, CCF_CAST);
    setcdataV(L, o, cd);
    lj_gc_check(L);
  }
  return 1;
...
LJLIB_CF(ffi_istype)	LJLIB_REC(.)
{
  ...
  if (tviscdata(o)) {
	...
	b = lj_ccolw_compatptr(cts, ct1, ct2, CCF_IGNQUAL);
...
LJLIB_CF(ffi_sizeof)	LJLIB_REC(ffi_xof FF_ffi_sizeof)
{
  ...
  if (LJ_UNLIKELY(tviscdata(L->base) && cdataisv(cdataV(L->base)))) {
    ...
    if (ctype_isvltype(ct->info))
      ...
      sz = ctype_hassize(ct->info) ? ct->size : CTSIZE_ILWALID;
    if (LJ_UNLIKELY(sz == CTSIZE_ILWALID)) {
      setnilV(L->top-1);
      return 1;
    }
  }
...
LJLIB_CF(ffi_string)	LJLIB_REC(.)
{
  ...
  if (o+1 < L->top && !tvisnil(o+1)) {
    ...
    lj_ccolw_ct_tv(cts, ctype_get(cts, CTID_P_CVOID), (uint8_t *)&p, o,
		   CCF_ARG(1));
  } else {
    lj_ccolw_ct_tv(cts, ctype_get(cts, CTID_P_CCHAR), (uint8_t *)&p, o,
		   CCF_ARG(1));
    len = strlen(p);
  }
  L->top = o+1;  /* Make sure this is the last item on the stack. */
...
LJLIB_CF(ffi_load)
{
  ...
  lj_clib_load(L, tabref(lwrr_func(L)->c.elw), name, global);
  return 1;
}
