...
/*
** colwerts an integer to a "floating point byte", represented as
...
int luaO_str2d (const char *s, lua_Number *result) {
  ...
  *result = lua_str2number(s, &endptr);
  if (endptr == s) return 0;  /* colwersion failed */
  ...
  switch (o->gch.tt)
  {
    ...
    case LUA_TUSERDATA: {
      ...
      if (G(L)->tmudata == NULL)  /* list is empty? */
        G(L)->tmudata = o->gch.next = o;  /* creates a cirlwlar list */
      else {
      ...
      if (gco2u(o)->elw) {
        luarc_releasetable(L, gco2u(o)->elw);
      }
