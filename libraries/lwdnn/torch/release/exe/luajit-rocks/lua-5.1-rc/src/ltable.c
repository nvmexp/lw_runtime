...
** A main ilwariant of these tables is that, if an element is not
...
static void resize (lua_State *L, Table *t, int nasize, int nhsize) {
  ...
  if (oldhsize) {
    for (i = twoto(oldhsize) - 1; i >= 0; i--) {
      Node *old = nold+i;
      if (!ttisnil(gval(old))) {
        luarc_clealwalue(gkey(old));
        luarc_clealwalue(gval(old));
      }
    }
  }
#endif /* LUA_REFCOUNT */
...
Node *luaH_getkey (Table *t, const TValue *key) {
  switch (ttype(key)) {
    ...
    case LUA_TNUMBER: {
      ...
      lua_Number n = lwalue(key);
      lua_number2int(k, n);
      if (luai_numeq(cast_num(k), lwalue(key))) /* index is int? */
