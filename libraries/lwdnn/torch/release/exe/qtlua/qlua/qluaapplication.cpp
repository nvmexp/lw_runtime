...
QLuaApplication::Private::printUsage()
{
  ...
  fprintf(stderr,
          ("Usage: %s [options] [script <scriptargs>]\n"
           ...
           "  -i            enter interactive mode after exelwting 'script'\n"
  ...
  if (state == QtLuaEngine::Running)
    {
      ...
      startTime = QDateTime::lwrrentDateTime();
    }
  else
    {
    ...
    d(new Private(this))
{
  ...
  QString lwteName = QFileInfo(applicationFilePath()).baseName();
  d->programNameData = lwteName.toLocal8Bit();
  d->programName = d->programNameData.constData();
  QRegExp re("^(mac(?=qlua)|win(?=qlua)|)(q?)(.*)", Qt::CaseInsensitive);
  lwteName = capitalize(lwteName);
  if (re.indexIn(lwteName) >= 0 && re.numCaptures() == 3)
    lwteName = capitalize(re.cap(2)) + capitalize(re.cap(3));

  // basic setup
  setApplicationName(lwteName);
...
/*! Return the lwrrently running QtLuaEngine.
...
/*! \signal newEngine()
  Ilwoked when a new lua engine is created after \a restart().
 */

/*! \signal acceptingCommands(bool)
  Indicates whether the application is lwrrently 
  accepting Lua commands using \a runCommand().
 */

/*! \signal luaCommandEcho(QByteArray ps1,QByteArray ps2,QByteArray cmd)
  Ilwoked when one calls \a runCommand.
...
/*! \signal luaConsoleOutput
  ...
  signal \a consoleOutput for colwenience.
 */
...
/*! Execute an interactive Lua command.
  ...
  for exelwting lua command entered by the user.
...
/*! Restart the interpreter afresh.
  ...
  the command line arguments are re-exelwted
