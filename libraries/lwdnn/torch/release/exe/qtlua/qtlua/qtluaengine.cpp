...
{
  // setup paths
#if HAVE_LUA_EXELWTABLE_DIR
  QString path = QCoreApplication::applicationFilePath();
  lua_exelwtable_dir(QFile::encodeName(path).constData());
#endif
...
{
  QThread *mythread = QThread::lwrrentThread();
...
/*! Returns true if the engine is paused because an error has oclwred */
...
/*! Alternate constructor. 
  ...
  use the \a lua_State* colwersion operator
...
/*! Calling this function after a successful lock 
  ...
  and the exelwtion of the command queue.
...
/*! \class QtLuaEngine
  ...
  and additional support for multi-threaded exelwtion.
  ...
  is lwrrently exelwting a Lua program.
  State \a QtLuaEngine::Paused indicates that the interpreter
  was suspended while exelwting a Lua program. One can then
  use the Lua debug library to ilwestigage the Lua state.

  Class \a QtLuaEngine provides member functions to 
  submit Lua strings to the interpreter and to collect 
  the evaluation results.  If these functions are ilwoked
  from the thread owning the Lua engine object,
  the Lua code is exelwted right away. 
  Otherwise a thread hopping operation with \a luaQ_pcall
  ensures that the exelwtion of a Lua program happens 
  in the thread owning the Lua engine object. */


QtLuaEngine::~QtLuaEngine()
{
  Q_ASSERT(QThread::lwrrentThread() == thread());
...
/*! \property QtLuaEngine::pauseOnError
  ...
  should pause exelwtion when an error oclwrs
...
/*! \property QtLuaEngine::runSignalHandlers
  ...
  honors the signal handler ilwokations immediately 
      ...
      if (ar.lwrrentline > 0)
        {
          loc = QByteArray(ar.source) + ":" + 
            QByteArray::number(ar.lwrrentline);
...
/*! Stops the lua exelwtion as soon as practicable.
  ...
  exelwting, unwinds the stack, and returns to ready state. */
...
QtLuaEngine::eval(QByteArray s, bool async)
{
  ...
  if (! lua.isReady())
    return false;
  if (async && QThread::lwrrentThread() == thread())
    return false;
  lua_settop(L, 0);
...
/*! Synchronously evaluate string \a s and returns the result.
  ...
  If an error oclwrs during evaluation, it returns a list
...
QtLuaEngine::Catcher::event(QEvent *e)
{
  ...
  // lock
  d->lockThread = QThread::lwrrentThread();
  d->lockCount = 1;
  d->hopEvent = 0;
  bool rflag = d->rflag;
...
luaQ_pcall(lua_State *L, int na, int nr, int eh, QObject *obj, bool async)
{
  ...
  if (obj->thread() == QThread::lwrrentThread())
    {
      bool rflag = d->rflag;
      d->rflag = true;
      d->mutex.unlock();
  ...
  else
    {
      ...
      if (async)
      ...
      else
        {
          ...
          // event loop and resume exelwtion in this thread.
...
/*! Thread hopping version of \a lua_pcall().
  ...
  arranges the exelwtion to happen in the
...
/*! Extract \a QVariant from the lua value located 
  ...
  Standard lua types are colwerted to \a QVariant
  as needed. */
  ...
  if  (type)
    {
      ...
      else if (! v.colwert(QVariant::Type(type)))
        v = QVariant();
    }
  return v;
}
...
{
  if (QThread::lwrrentThread() == QCoreApplication::instance()->thread())
    return false;
...
{
  // LUA: "_val_:tostring()"
  // Colwerts a qt value to a nice printable string.
  // Returns nil if _val_ is not a qt value.
  const QVariant *vp = luaQ_toqvariantp(L, 1);
  if (! vp)
    lua_pushnil(L);
  ...
  else
    {
      ...
      else if (var.colwert(QVariant::ByteArray))
...
{
  // LUA: "_val_:tonumber()"
  // Colwerts a qt value to a number.
  // Returns nil if the colwersion is impossible
  QVariant var = luaQ_toqvariant(L, 1);
  if (var.colwert(QVariant::Double))
...
{
  // LUA: "_val_:tobool()"
  // Colwerts a qt value to a boolean.
  const QVariant *vp = luaQ_toqvariantp(L, 1);
  if (! vp)
    lua_pushnil(L);
  else if (qvariant_has_object_type(vp))
  ...
  if (lua_gettop(L) == 1)
    {
      ...
      // colwert enum codes into enum names
      QByteArray n;
      if (mp.isFlagType() && v.canColwert(QVariant::Int))
        v = mp.enumerator().valueToKeys(v.toInt());
      else if (mp.isEnumType() && v.canColwert(QVariant::Int))
  ...
  else
    {
      ...
      // check types
      if (! mp.write(obj, v))
        luaL_error(L, "cannot colwert value for property '%s'", mp.name());
    }
  return 0;
}


/* metatable material: ilwoking methods */
...
construct_arg(QVariant &var, void *vtype)
{
  ...
  else if (type && (var.userType() == type || var.colwert(QVariant::Type(type))))
        ...
        for (j=1; j<narg; j++)
          {
            ...
            s += 10;
            QVariant var = vars[j];
            if (var.canColwert(QVariant::Type(type)))
              continue;
            s += INT_MAX / 2;
            break;
          }
  ...
  try
    {
      ...
      if (errarg < 0)
        {
          obj->qt_metacall(QMetaObject::IlwokeMetaMethod, d.id, pargs.data());
...
static int
luaQ_m__call_ilwokemethod(lua_State *L)
{
  QObject *obj = luaQ_toqobject(L, 1);
  if (! obj)
    luaL_error(L, "bad 'self' argument (not a qobject)");
  int narg = lua_gettop(L);
  lua_pushcfunction(L, luaQ_m__ilwokemethod);
  lua_insert(L, 1);
  lua_pushvalue(L, lua_upvalueindex(1));
      ...
      if (lua_istable(L, -1))
        {
          ...
          if (type == qMetaTypeId<QtLuaMethodInfo>())
            {
              lua_pushcclosure(L, luaQ_m__call_ilwokemethod, 1);
              return 1;
            }
      ...
      if (lua_istable(L, -1))
        {
          lua_pushvalue(L, 2);
          lua_gettable(L, -2);
          // ..stack: object key value metatable metaclass lwrval
          const QVariant *vp = luaQ_toqvariantp(L, -1);
          int type = (vp) ? vp->userType() : QMetaType::Void;
...
/*! Pushes value \a var onto the stack.
  Common variant types are colwerted 
...
/*! Pushes qobject \obj onto the stack.
  ...
  Null object pointers are colwerted to lua \a nil value. */
