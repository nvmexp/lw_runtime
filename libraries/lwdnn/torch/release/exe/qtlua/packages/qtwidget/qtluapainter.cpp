...
{
  bool ilwertible;
  QTransform i = m.ilwerted(&ilwertible);
  if (combine)
    m = m * matrix;
  else
    i = matrix * i;
  matrix = m;
  point = (ilwertible) ? i.map(point) : QPointF();
  path = (ilwertible) ? i.map(path) : QPainterPath();
  clip = (ilwertible) ? i.map(clip) : QPainterPath();
}
  ...
  if (object)
    {
      ...
      mo->ilwokeMethod(object, "printer", Q_RETURN_ARG(QPrinter*, printer));
      mo->ilwokeMethod(object, "device", Q_RETURN_ARG(QPaintDevice*, device));
      d->object = object;
  ...
  : d(new Private(this))
{
  ...
  if (! image.isNull())
    if (image.format() != QImage::Format_Mono &&
      ...
      image = image.colwertToFormat(QImage::Format_ARGB32_Premultiplied);
  d->image = image;
  d->device = &d->image;
  if (d->image.isNull())
    d->p = new QPainter();
...
void 
QtLuaPainter::gend(bool ilwalidate)
{
  if (ilwalidate)
    d->damaged = rect();
  d->count -= 1;
...
QPen 
QtLuaPainter::lwrrentpen() const
{
  return d->state.pen;
}

QBrush 
QtLuaPainter::lwrrentbrush() const
{
...
QPointF 
QtLuaPainter::lwrrentpoint() const
{
...
QPainterPath 
QtLuaPainter::lwrrentpath() const
{
  return d->state.path;
}
...
QPainterPath 
QtLuaPainter::lwrrentclip() const
{
  return d->state.clip;
}
...
QFont 
QtLuaPainter::lwrrentfont() const
{
  return d->state.font;
}
...
QTransform 
QtLuaPainter::lwrrentmatrix() const
{
...
QBrush 
QtLuaPainter::lwrrentbackground() const
{
...
QtLuaPainter::lwrrentmode() const
{
  return d->state.mode;
}
...
QtLuaPainter::lwrrenthints() const
{
...
QtLuaPainter::lwrrentangleunit() const
{
  return d->state.unit;
}
...
QString
QtLuaPainter::lwrrentstylesheet() const
{
...
void 
QtLuaPainter::lwrveto(qreal x1, qreal y1, qreal x2, qreal y2, qreal x3, qreal y3)
{
  if (! d->state.hasPoint)
    d->state.path.moveTo(d->state.point);
  QPointF c1(x1,y1);
  ...
  d->state.path.lwbicTo(c1,c2,d->state.point);
  d->state.hasPoint = true;
}
...
QtLuaPainter::arc(qreal x, qreal y, qreal r, qreal a1, qreal a2)
{
  ...
  d->state.point = d->state.path.lwrrentPosition();
  d->state.hasPoint = true;
}
...
void 
QtLuaPainter::rlwrveto(qreal x1, qreal y1, qreal x2, qreal y2, qreal x3, qreal y3)
{
  if (! d->state.hasPoint)
    d->state.path.moveTo(d->state.point);
  QPointF c1(x1,y1);
...
static QTextDolwment*
makeTextDolwment(QString text, QRectF &rect, const QtLuaPainter::State &state, int flags)
{
  QTextDolwment *doc = new QTextDolwment;
  ...
  QRectF lr = QRectF(QPointF(0,0), doc->dolwmentLayout()->dolwmentSize());
  qreal dy = rect.top();
  if (flags & Qt::AlignBottom)
    dy = rect.bottom() - lr.bottom();
  else if (flags & Qt::AliglwCenter)
...
QtLuaPainter::show(QString text, qreal x, qreal y, qreal w, qreal h, int flags)
{
  ...
  if ((flags & TextRich) == TextRich)
    {
      // rich text
      QTextDolwment *doc = makeTextDolwment(text, rect, d->state, flags);
      d->p->save();
