...
static int
qapplication_set_override_lwrsor(lua_State *L)
{
  QApplication::setOverrideLwrsor(luaQ_checkqvariant<QLwrsor>(L, 1));
  return 0;
}


static int
qapplication_change_override_lwrsor(lua_State *L)
{
  QApplication::changeOverrideLwrsor(luaQ_checkqvariant<QLwrsor>(L, 1));
  return 0;
}


static int
qapplication_restore_override_lwrsor(lua_State *L)
{
...
static int
qapplication_override_lwrsor(lua_State *L)
{
  QLwrsor *c = QApplication::overrideLwrsor();
  if (c)
    luaQ_pushqt(L, *c);
  else
    lua_pushnil(L);
...
static struct luaL_Reg qapplication_lib[] = {
  ...
  {"setOverrideLwrsor", qapplication_set_override_lwrsor},
  {"changeOverrideLwrsor", qapplication_change_override_lwrsor},
  {"restoreOverrideLwrsor", qapplication_restore_override_lwrsor},
  {"overrideLwrsor", qapplication_override_lwrsor},
  {0,0}
};
...
static int
qlwrsor_new(lua_State *L)
{
  if (lua_isstring(L, 1))
    {
      QMetaEnum me = f_enumerator("LwrsorShape");
      int shape = me.keyToValue(lua_tostring(L, 1));
      if (shape < 0)
        luaL_error(L, "unrecognized cursor shape \"%s\".", lua_tostring(L, 1));
      luaQ_pushqt(L, QLwrsor(Qt::LwrsorShape(shape)));
      return 1;
    }
  else
    {
      ...
      if (i <= 4)
        luaQ_pushqt(L, QLwrsor(pm , hx, hy));
      else if (pm.depth() == 1 && mask.depth() == 1)
        luaQ_pushqt(L, QLwrsor(QBitmap(pm), QBitmap(mask), hx, hy));
      else
...
static int
qlwrsor_hotspot(lua_State *L)
{
  QLwrsor cursor = luaQ_checkqvariant<QLwrsor>(L, 1);
  luaQ_pushqt(L, cursor.hotSpot());
  return 1;
}

static int
qlwrsor_mask(lua_State *L)
{
  QLwrsor cursor = luaQ_checkqvariant<QLwrsor>(L, 1);
...
static int
qlwrsor_pos(lua_State *L)
{
  luaQ_pushqt(L, QLwrsor::pos());
  return 1;
}

static int
qlwrsor_setpos(lua_State *L)
{
  QLwrsor::setPos(luaQ_checkqvariant<QPoint>(L, 1));
  return 0;
}

static struct luaL_Reg qlwrsor_lib[] = {
  {"new", qlwrsor_new},
  {"hotSpot", qlwrsor_hotspot},
  {"mask", qlwrsor_mask},
  {"pixmap", qlwrsor_pixmap},
  {"shape", qlwrsor_shape},
  {"pos", qlwrsor_pos},
  {"setPos", qlwrsor_setpos},
  {0,0}
};
...
static int
qtransform_ilwerted(lua_State *L)
{
  bool ilwertible;
  QTransform c = luaQ_checkqvariant<QTransform>(L, 1);
  QTransform d = c.ilwerted(&ilwertible);
  if (ilwertible)
                 ...
                 Qt::WidgetAttribute &attrib)
{
  static struct { const char *n; int a; } s[] = {
    ...
    { "WA_KeyboardFolwsChange", Qt::WA_KeyboardFolwsChange },
    ...
    { "WA_MacShowFolwsRect", Qt::WA_MacShowFolwsRect },
...
#endif
    {"LwstomizeWindowHint", Qt::LwstomizeWindowHint, 0},
    {0,0,0}
  };
  for (int i=0; s[i].n; i++)
    if (! strcmp(name, s[i].n))
