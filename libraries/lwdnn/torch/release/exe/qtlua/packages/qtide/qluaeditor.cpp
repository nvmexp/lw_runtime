...
QLuaEditor::Private::setFileName(QString fname)
{
  ...
  e->setDolwmentTitle(QFileInfo(fname).fileName());
...
QLuaEditor::Private::luaLoad(bool restart)
{
  ...
  if (restart)
    ...
    return QLuaIde::instance()->luaExelwte(cmd);
}
...
QLuaEditor::doPrint()
{
  ...
  if (d->e->textLwrsor().hasSelection())
    options |= QPrintDialog::PrintSelection;
      ...
      if (cursor.hasSelection())
        {
          ...
          if (s.simplified().isEmpty() ||
              QLuaIde::instance()->luaExelwte(s.toLocal8Bit()))
            {
              QLuaIde::instance()->activateConsole(this);
              return;
            }
...
QLuaEditor::canClose()
{
  QTextDolwment *doc = d->e->document();
