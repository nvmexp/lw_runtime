...
namespace QLuaActionHelpers {
  ...
  QAction * 
  operator<<(QAction *action, QKeySequence shortlwt)
  {
    QList<QKeySequence> shortlwts = action->shortlwts();
    shortlwts.prepend(shortlwt);
    action->setShortlwts(shortlwts);
    return action;
  }
  ...
  operator<<(QAction *a, NewAction b)
  {
    ...
    a->setShortlwts(QList<QKeySequence>());
    return a;
  }
...
public:
  ...
  QString lwrrentPath;
  QByteArray exelwteWhenAccepting;
...
{
  if (accepting && !exelwteWhenAccepting.isEmpty())
    {
      QByteArray cmd = exelwteWhenAccepting;
      exelwteWhenAccepting = QByteArray();
      q->luaExelwte(cmd);
      return;
    }
  updateLuaActions();
      ...
      foreach(QObject *o, windows)
        {
          ...
          if (++k < 10)
            action->setShortlwt(QKeySequence(Qt::CTRL + Qt::Key_0 + k));
        }
    }
}
  ...
  // actions
  ...
  else if (name == "ActionLuaStop")
    {
      action = newAction(tr("&Stop","lua|stop"))
        ...
        << tr("Stop the exelwtion of the current Lua command.");
    }
  else if (name == "ActionLuaPause")
    {
      action = newAction(tr("&Pause","lua|pause"))
        ...
        << tr("Suspend or resume the exelwtion of the current Lua command.");
      action->setCheckable(true);
    }
...
QLuaIde::openFile(QString fileName, bool inOther, QWidget *window)
{
  ...
  if (mo && mo->ilwokeMethod(window, "openFile", 
...
bool 
QLuaIde::newDolwment(QWidget *window)
{
  bool okay = false;
  const QMetaObject *mo = (window) ? window->metaObject() : 0;
  // try calling a method 'bool newDolwment(QString)'
  if (mo && mo->ilwokeMethod(window, "newDolwment", 
                             Q_RETURN_ARG(bool, okay) ))
    if (okay)
      return okay;
...
bool
QLuaIde::luaExelwte(QByteArray cmd)
{
  QLuaApplication *app = QLuaApplication::instance();
...
QLuaIde::doOpen()
{
  ...
  foreach(QString fname, files)
    if (! fname.isEmpty())
      {
        d->lwrrentPath = QFileInfo(fname).absolutePath();
