...
public slots:
  void dockLwrrentWindow(bool b);
  void setActive(Client *client);
  void folwsChange(QWidget *old, QWidget *now);
  void guiUpdateLater();
  void guiUpdate();
public:
  QLuaMdiMain *q;
    ...
    tabMode(false)
{
  ...
  connect(QApplication::instance(), SIGNAL(folwsChanged(QWidget*,QWidget*)),
          this, SLOT(folwsChange(QWidget*,QWidget*)) );
}
...
void 
QLuaMdiMain::Private::folwsChange(QWidget *old, QWidget *now)
{
  QWidget *w = now;
...
void 
QLuaMdiMain::Private::dockLwrrentWindow(bool b)
{
  if (active)
    {
  ...
  if (! subWindow)
    {
      ...
      foreach (QAction *action, menu->actions())
        {
          if (action->shortlwt() == cseq)
            action->setShortlwt(QKeySequence());
          if ((action->shortlwt() == cseq || action->isSeparator() ) ) 
            {
              if (dockAction)
      ...
      d->setActive(this);
      QWidget *fw = widget;
      fw = fw->folwsWidget() ? fw->folwsWidget() : fw;
      fw->setFolws(Qt::OtherFolwsReason);
    }
}
  ...
  if (! dockWidget)
    {
      ...
      // mark widget as a subwindow (to isolate shortlwts)
      Qt::WindowFlags flags = widget->windowFlags() & ~Qt::WindowType_Mask;
  ...
  if (d->table.contains(w))
    {
      ...
      w = w->folwsWidget() ? w->folwsWidget() : w;
      w->setFolws(Qt::OtherFolwsReason);
      return true;
    }
  return false;
}
  ...
  if (! d->dockAction)
    {
      ...
      connect(a, SIGNAL(triggered(bool)), d, SLOT(dockLwrrentWindow(bool)));
      a->setCheckable(true);
      d->dockAction =a;
    }
  return d->dockAction;
