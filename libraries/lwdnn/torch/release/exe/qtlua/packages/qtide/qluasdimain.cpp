                                           ...
                                           QByteArray ps2, QByteArray cmd)
{
  ...
  if (printTimings)
    {
      QDateTime now = QDateTime::lwrrentDateTime();
...
#ifdef Q_WS_MAC
  if (ev->type() == QEvent::ShortlwtOverride && obj == e)
    {
...
{
  bool hasFolws = e->hasFolws();
  if  (q->hasAction("ActionEditUndo"))
    q->stdAction("ActionEditUndo")->setEnabled(hasFolws && canUndo);
  if  (q->hasAction("ActionEditRedo"))
    q->stdAction("ActionEditRedo")->setEnabled(hasFolws && canRedo);
}
...
QLuaSdiMain::Private::updateLuaActions()
{
  ...
  bool okeval = false;
  if (c->hasFolws())
    okeval = c->textLwrsor().hasSelection();
  else if (e->hasFolws())
    okeval = true;
  ...
  QLwrsor cursor((accepting) ? Qt::IBeamLwrsor : Qt::WaitLwrsor);
  e->viewport()->setLwrsor(cursor);
  ...
  if (n >= -1 && n < history.size())
    {
      ...
      historyLwrsorPos[historyPos] = e->textLwrsor().position();
      QString s;
      if (historyOverlay.contains(n))
        ...
        s = history[n];
      int cpos = s.size();
      if (historyLwrsorPos.contains(n))
        cpos = historyLwrsorPos[n];
      if (n != historyPos)
        {
          e->setPlainText(s);
          QTextLwrsor c = e->textLwrsor();
          c.setPosition(cpos);
          e->setTextLwrsor(c);
          historyPos = n;
          return true;
  ...
  : QFrame(d->q), d(d), loop(0)
{
  ...
  // focus
  c->setFolwsPolicy(Qt::NoFolws);
  v->setFolwsPolicy(Qt::NoFolws);
      ...
      while ((pos = line.indexOf(search, pos)) >= 0)
        {
          ...
          extra.cursor = d->e->textLwrsor();
          extra.cursor.setPosition(pos);
          pos += search.size();
          extra.cursor.setPosition(pos, QTextLwrsor::KeepAnchor);
...
QLuaSdiMain::Private::historySearch()
{
  ...
  historyLwrsorPos.clear();
  int pos = historyPos;
  HSWidget *hsw = new HSWidget(this);
  int selected = hsw->exec(e->lwrsorRect().bottomRight());
  delete hsw;
  q->activateWindow();
    ...
    d(new Private(this))
{
  ...
  connect(d->e, SIGNAL(textChanged()),
          this, SLOT(updateActionsLater()) );
  connect(d->e, SIGNAL(lwrsorPositionChanged()),
...
QLuaSdiMain::createAction(QByteArray name)
{
  if (name == "MenuFile")
  ...
  else if (name == "MenuEdit")
    {
      ...
      menu->addSeparator();
      menu->addAction(stdAction("ActionEditLwt"));
  ...
  else if (name == "ActionEditLwt")
    {
      return QLuaMainWindow::createAction(name)
        << Connection(this, SLOT(doLwt()));
    }
...
QLuaSdiMain::doPrint()
{
  ...
  if (d->c->textLwrsor().hasSelection())
    options |= QPrintDialog::PrintSelection;
...
QLuaSdiMain::doEval()
{
  ...
  if (d->c->hasFolws()) 
    {
      cursor = d->c->textLwrsor();
    }
  else if (d->e->hasFolws())
    {
      d->historyPos = -1;
      d->historyOverlay.clear();
      d->historyLwrsorPos.clear();
      d->c->moveToEnd();
      cursor = d->e->textLwrsor();
  ...
  if (cursor.hasSelection())
    {
      ...
      if (QLuaIde::instance()->luaExelwte(s.toLocal8Bit()))
        return;
    }
...
QLuaSdiMain::updateActions()
{
  ...
  d->updateUndoRedo();
  // cut copy paste
  QLuaTextEdit *a = qobject_cast<QLuaTextEdit*>(folwsWidget());
  bool readOnly = (a) ? a->isReadOnly() : true;
  bool canPaste = (a) ? a->canPaste() : false;
  bool canCopy = (a) ? a->textLwrsor().hasSelection() : false;
  if (hasAction("ActionEditPaste"))
    stdAction("ActionEditPaste")->setEnabled(canPaste && !readOnly);
  if (hasAction("ActionEditLwt"))
    stdAction("ActionEditLwt")->setEnabled(canCopy && !readOnly);
  if (hasAction("ActionEditCopy"))
    stdAction("ActionEditCopy")->setEnabled(canCopy);
  // misc
  bool eHasFolws = d->e->hasFolws();
  bool cHasFolws = d->c->hasFolws();
  if (hasAction("ActionConsoleClear"))
    stdAction("ActionConsoleClear")->setEnabled(cHasFolws);
  if (hasAction("ActionEditFind"))
    stdAction("ActionEditFind")->setEnabled(cHasFolws);
  if (hasAction("ActionModeBalance"))
    stdAction("ActionModeBalance")->setEnabled(eHasFolws);
  // tools
  ...
  if (hasAction("ActionHistoryUp"))
    stdAction("ActionHistoryUp")->setEnabled(eHasFolws && (hp+1 < hs) );
  if (hasAction("ActionHistoryDown"))
    stdAction("ActionHistoryDown")->setEnabled(eHasFolws && (hp >= 0));
  if (hasAction("ActionHistorySearch"))
    stdAction("ActionHistorySearch")->setEnabled(eHasFolws && (hs > 0));
  // lua actions
  d->updateLuaActions();
}
