    ...
    view(0)
{
  ...
  findNextSLwt = new QShortlwt(QKeySequence::FindNext, browser);
  findPrevSLwt = new QShortlwt(QKeySequence::FindPrevious, browser);
  ...
  : QLuaMainWindow("browser1", parent), d(new Private(this))
{
  ...
  // shortlwts
#if HAVE_QTWEBKIT
  new QShortlwt(QKeySequence("Ctrl+0"), this, SLOT(doZoomReset()));
#endif
  // update actions
  updateActions();
}
  ...
  // items
  ...
  else if (name == "ActionEditLwt")
    {
      return d->w->pageAction(QWebPage::Cut)
        << NewAction(tr("Lw&t", "edit|cut"))
        << QKeySequence(QKeySequence::Cut)
        << QIcon(":/images/editlwt.png")
