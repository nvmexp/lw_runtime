  ...
  for(;;)
    {
      ...
      c.setPosition(si->pos + si->len, QTextLwrsor::KeepAnchor);
...
QLuaMode::doBalance()
{
  ...
  if ((nspos < spos && nepos >= epos) ||
    ...
    {
      c.setPosition(nspos);
      c.setPosition(nepos, QTextLwrsor::KeepAnchor);
      e->setTextLwrsor(c);
      return true;
    }
  return false;
...
QLuaMode::askCompletion(QString stem, QStringList completions)
{
  ...
  int selected = view->exec(e->lwrsorRect().bottomRight());
  delete view;
  e->activateWindow();
  return selected;
}
