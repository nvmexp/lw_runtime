...
#include <QtGlobal>
#include <QAbstractTextDolwmentLayout>
...
{
  QTextDolwment *d = editor->document();
  QTextLwrsor c = editor->textLwrsor();
  ...
  if (b.isValid())
    {
      ...
      editor->setTextLwrsor(c);
      editor->ensureLwrsorVisible();
      return;
    }
  QApplication::beep();
}
...
protected:
  ...
  QLuaTextEdit *editor;
  QTextDolwment *document;
  QPointer<QShortlwt> findNextSLwt;
  QPointer<QShortlwt> findPrevSLwt;
public:
    ...
    document(editor->document())
{
  ...
  new QShortlwt(QKeySequence::FindNext, this, SLOT(findNext()));
  new QShortlwt(QKeySequence::FindPrevious, this, SLOT(findPrevious()));
  findNextSLwt = new QShortlwt(QKeySequence::FindNext, editor);
  findPrevSLwt = new QShortlwt(QKeySequence::FindPrevious, editor);
  connect(findNextSLwt, SIGNAL(activated()), this, SLOT(findNext()));
  connect(findPrevSLwt, SIGNAL(activated()), this, SLOT(findPrevious()));
  update();
  // settings
  QSettings s;
...
{
  QTextLwrsor cursor = editor->textLwrsor();
...
QLuaTextEdit::FindDialog::find(bool backward)
{
  if (ui.findEdit->text().isEmpty())
    return false;
  QTextDolwment::FindFlags flags = 0;
  if (backward)
    flags |= QTextDolwment::FindBackward;
  if (ui.caseSensitiveBox->isChecked())
    flags |= QTextDolwment::FindCaseSensitively;
  if (ui.wholeWordsBox->isChecked())
    flags |= QTextDolwment::FindWholeWords;
  QTextLwrsor cursor = editor->textLwrsor();
...
{
  ui.findButton->setEnabled(!ui.findEdit->text().isEmpty());
  bool match = (ui.findEdit->text() == editor->textLwrsor().selectedText());
...
QLuaTextEdit::ReplaceDialog::replaceAll()
{
  ...
  QMessageBox::warning(this, tr("Replace All"), 
                       tr("Replaced %n oclwrrence(s).", 0, n));
}
...
QLuaTextEdit::LineNumbers::paintEvent(QPaintEvent *event)
{
  ...
  QFont font = e->font();
  QTextBlock sblock = e->textLwrsor().block();
  font.setWeight(QFont::Bold);
...
{
  connect(q, SIGNAL(lwrsorPositionChanged()),
...
{
  QTextLwrsor cursor = q->textLwrsor();
...
QLuaTextEdit::readFile(QFile &file)
{
  if (file.open(QIODevice::ReadOnly))
    {
      QTextStream in(&file);
      QApplication::setOverrideLwrsor(Qt::WaitLwrsor);
      QString data = d->filterText(in.readAll());
      if (! file.error())
        {
          setPlainText(data);
          QApplication::restoreOverrideLwrsor();
          return true;
        }
      QApplication::restoreOverrideLwrsor();
    }
  ...
  if (block.isValid())
    {
      ...
      c.setPosition(block.position()+block.length()-1, QTextLwrsor::KeepAnchor);
      extra.cursor = c;
...
{
  QTextDolwment *doc = 0;
  QTextDolwment *odoc = document();
  QString title = odoc->metaInformation(QTextDolwment::DolwmentTitle);
  if (printer->printRange() == QPrinter::Selection)
    {
      // print selection
      doc = new QTextDolwment(this);
      doc->setDefaultTextOption(odoc->defaultTextOption());
      doc->setMetaInformation(QTextDolwment::DolwmentTitle, title);
      doc->setDefaultFont(odoc->defaultFont());
      doc->setUseDesignMetrics(odoc->useDesignMetrics());
      QTextLwrsor cursor(doc);
      cursor.insertText(textLwrsor().selectedText());
    }
  else
    {
      // print document
  ...
  // page layout
  QAbstractTextDolwmentLayout *layout = doc->dolwmentLayout();
          ...
          if (state != QPrinter::Aborted && state != QPrinter::Error)
            {
              ...
              p.translate(-top);
              // print page
              QAbstractTextDolwmentLayout::PaintContext ctx;
              p.setClipRect(view);
              ctx.clip = view;
...
QLuaTextEdit::setBlockIndent(QTextBlock block, int indent)
{
  ...
  if (oindent == indent)
    return cpos;
  QTextLwrsor cursor(block);
  cursor.setPosition(cpos, QTextLwrsor::KeepAnchor);
  QString spaces;
