#ifndef THLWNN_IM2COL_H
#define THLWNN_IM2COL_H

#include "common.h"
#include "THCNumerics.lwh"

// Kernel for fast unfold+copy
// (borrowed from Caffe: https://github.com/BVLC/caffe/blob/master/src/caffe/layers/colw_layer.lw)
    ...
    for (int i = 0; i < ksize_h; ++i) {
      for (int j = 0; j < ksize_w; ++j) {
        ...
        *data_col = (h >= 0 && w >= 0 && h < height && w < width) ?
          data_im[i * dilation_h * width + j * dilation_w] : ScalarColwert<int, Dtype>::to(0);
...
template <typename Dtype>
void im2col(lwdaStream_t stream, const Dtype* data_im, const int channels,
            const int height, const int width,
  ...
  // Launch
  im2col_kernel <<<GET_BLOCKS(num_kernels), LWDA_NUM_THREADS, 0, stream>>> (
...
__global__ void col2im_kernel(const int n, const Dtype* data_col,
  ...
  LWDA_KERNEL_LOOP(index, n) {
    ...
    data_im[index] = ScalarColwert<Acctype, Dtype>::to(val);
  }
}

template <typename Dtype, typename Acctype>
void col2im(lwdaStream_t stream, const Dtype* data_col, const int channels,
            const int height, const int width,
  ...
  // To avoid ilwolving atomic operations, we will launch one kernel per
  // bottom dimension, and then in the kernel add up the top dimensions.
  col2im_kernel<Dtype, Acctype> <<<GET_BLOCKS(num_kernels), LWDA_NUM_THREADS, 0, stream>>> (
