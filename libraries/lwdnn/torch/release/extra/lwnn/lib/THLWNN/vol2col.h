#ifndef THLWNN_VOL2COL_H
#define THLWNN_VOL2COL_H
      ...
      for (int j = 0; j < ksize_h; ++j) {
        for (int k = 0; k < ksize_w; ++k) {
          ...
          *data_col = (t >= 0 && h >= 0 && w >= 0 && t < depth && h < height && w < width) ?
            data_vol[i * dilation_t * height * width + j * dilation_h * width + k * dilation_w] : ScalarColwert<int, Dtype>::to(0);
...
template <typename Dtype>
void vol2col(lwdaStream_t stream, const Dtype* data_vol, const int channels,
    const int depth, const int height, const int width,
    const int ksize_t, const int ksize_h, const int ksize_w,
    const int pad_t, const int pad_h, const int pad_w,
    const int stride_t, const int stride_h, const int stride_w,
  ...
  // Launch
  vol2col_kernel <<<GET_BLOCKS(num_kernels), LWDA_NUM_THREADS, 0, stream>>> (
...
__global__ void vol2im_kernel(const int n, const Dtype* data_col,
  ...
  LWDA_KERNEL_LOOP(index, n) {
    ...
    data_vol[index] = ScalarColwert<Acctype, Dtype>::to(val);
  }
}

template <typename Dtype, typename Acctype>
void col2vol(lwdaStream_t stream, const Dtype* data_col, const int channels,
    const int depth, const int height, const int width,
  ...
  vol2im_kernel<Dtype, Acctype> <<<GET_BLOCKS(num_kernels), LWDA_NUM_THREADS, 0, stream>>> (
