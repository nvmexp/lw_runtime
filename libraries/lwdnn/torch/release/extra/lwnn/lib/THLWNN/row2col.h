#ifndef THLWNN_ROW2COL_H
#define THLWNN_ROW2COL_H
...
template <typename Dtype>
void row2col(lwdaStream_t stream, const Dtype *data_row, const int channels,
  ...
  // Launch
  row2col_kernel<<<GET_BLOCKS(num_kernels), LWDA_NUM_THREADS, 0, stream>>>(
...
__global__ void col2row_kernel(const int n, const Dtype *data_col,
  ...
  LWDA_KERNEL_LOOP(index, n) {
    ...
    data_row[index] = ScalarColwert<Acctype, Dtype>::to(val);
  }
  }

template <typename Dtype, typename Acctype>
void col2row(lwdaStream_t stream, const Dtype *data_col, const int channels,
  ...
  col2row_kernel<
      Dtype, Acctype><<<GET_BLOCKS(num_kernels), LWDA_NUM_THREADS, 0, stream>>>(
