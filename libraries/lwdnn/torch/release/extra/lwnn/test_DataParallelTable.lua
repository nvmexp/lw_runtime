...
-- If fblwnn and fbnn exists we'll do a profile of DataParallel
local profileDp = pcall(function() require 'fblwnn'; require 'fbnn' end)

local baseGpu = 1  -- First GPU to use
local numGpus = lwtorch.getDeviceCount()
torch.setdefaulttensortype('torch.DoubleTensor')
torch.setnumthreads(8)
lwtorch.setDevice(baseGpu)
lwtorch.reserveStreams(1)
...
local function checkHalf()
   if lwtorch.hasHalf then
       ...
       t2cpu['torch.LwdaHalfTensor'] = 'torch.HalfTensor'
   end
end
...
local function buildNet(width, height, pool, feat, filt, tableInOut, numColws)
   ...
   for i = 1, numColws do
      ...
      net:add(nn.SpatialColwolutionMM(fin, feat, filt, filt, 1, 1, (filt-1)/2))
...
function test_DataParallelTable(gtype)
   ...
   for _,batchSize in ipairs {2 * numGpus, 9, 15} do
      ...
      for i = 1, numGpus do
         local lwrGpu = math.fmod(baseGpu+(i-1)-1, lwtorch.getDeviceCount()) + 1
         lwtorch.setDevice(lwrGpu)
         gClassifier:add(cpuClassifier:clone():type(gtype), lwrGpu)
      end
      lwtorch.setDevice(baseGpu)
      ...
      assert(lwtorch.getDevice() == baseGpu,
...
function test_DataParallelTable_smallBatch(gtype)
   local net = nn.SpatialColwolution(3, 3, 3, 5):type(gtype)

   local dpt = nn.DataParallelTable(1):type(gtype)
   for i=1,numGpus do
      lwtorch.withDevice(i, function()
...
function test_DataParallelTable_type(gtype)
   ...
   local net = nn.SpatialColwolution(3, 3, 3, 5):type(ctype)
...
function test_DataParallelTable_accGradParameters(gtype)
   ...
   local function aclwmulateGradient(m)
...
function test_ProfileDataParallelTable(gtype)
   ...
   for moduleName, module in pairs(modulesToTest) do
      for numGpus = 1, deviceCount do
         ...
         -- Build a CPU model
         local cNet = buildNet(width, height, pool, feat, filt, false, numColws)
         ...
         for i = 1, numGpus do
            local lwrGpu = math.fmod(baseGpu+(i-1)-1, lwtorch.getDeviceCount())+1
            lwtorch.setDevice(lwrGpu)
            gNet:add(cNet:clone():type(gtype), lwrGpu)
         end
         lwtorch.setDevice(baseGpu)
