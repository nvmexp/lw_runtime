...
function usage.usage(truth, rules, ...)
   if truth then
      ...
      local args = {}
      for i=1,select('#', ...) do
         table.insert(args, string.format("**%s**", elw.type(select(i, ...))))
      end
      local argtblidx
      if self:hasruletype('N') then
         if select("#", ...) == 1 and elw.istype(select(1, ...), "table") then
            argtblidx = 1
         end
      elseif self:hasruletype('M') then
         if select("#", ...) == 2 and elw.istype(select(2, ...), "table") then
            argtblidx = 2
         end
      end
      if argtblidx then
         ...
         for k,v in pairs(tbl) do
            ...
            if type(k) == 'string' then
               table.insert(argtbl, string.format("**%s=%s**", k, elw.type(v)))
            else
               table.insert(argtbl, string.format("**[%s]**=?", elw.type(k)))
            end
         end
