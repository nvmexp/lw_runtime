...
local function generaterules(rules)
   ...
   local upvalues = {istype=elw.istype, graph=graph}
   ...
   for ridx=#rules,1,-1 do
      optperrulestride[ridx] = lwariant
      lwariant = lwariant * optperrule[ridx]
   end

   -- note: we keep the original rules (id) for all path variants
   -- hence, the mask.
   for variant=lwariant,1,-1 do
      local r = variant
