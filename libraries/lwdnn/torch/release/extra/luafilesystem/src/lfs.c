...
static int lfs_lock_dir(lua_State *L) {
  ...
  if((fd = CreateFile(ln, GENERIC_WRITE, 0, NULL, CREATE_NEW,
                FILE_ATTRIBUTE_NORMAL | FILE_FLAG_DELETE_ON_CLOSE, NULL)) == ILWALID_HANDLE_VALUE) {
        int en = GetLastError();
...
static int lfs_unlock_dir(lua_State *L) {
  ...
  if(lock->fd != ILWALID_HANDLE_VALUE) {    
    CloseHandle(lock->fd);
    lock->fd=ILWALID_HANDLE_VALUE;
  }
  return 0;
}
#else
...
#endif
/*
** Colwert the inode protection mode to a string.
*/
#ifdef _WIN32
...
static void push_st_blksize (lua_State *L, STAT_STRUCT *info) {
 ...
 /*
** Colwert the inode protection mode to a permission list.
*/
...
static int push_link_target(lua_State *L) {
        ...
        while (1) {
            ...
            if (tsize < 0) { /* a readlink() error oclwrred */
                free(target);
                return 0;
            }
            if (tsize < size)
