...
enum etoken {
    ...
    TOK_1_BEGIN,
    TOK_OPEN_LWRLY, TOK_CLOSE_LWRLY, TOK_SEMICOLON, TOK_COMMA, TOK_COLON,
...
int64_t callwlate_constant(lua_State* L, struct parser* P);
...
/* Parses an enum definition from after the open lwrly through to the close
 * lwrly. Expects the user table to be on the top of the stack
 */
static int parse_enum(lua_State* L, struct parser* P, struct ctype* type)
{
    ...
    for (;;) {
        ...
        if (tok.type == TOK_COMMA || tok.type == TOK_CLOSE_LWRLY) {
            /* we have an auto callwlated enum value */
            value++;
        } else if (tok.type == TOK_ASSIGN) {
            /* we have an explicit enum value */
            value = (int) callwlate_constant(L, P);
            require_token(L, P, &tok);
        } else {
            return luaL_error(L, "unexpected token in enum at line %d", P->line);
        }
...
static void callwlate_member_position(lua_State* L, struct parser* P, struct ctype* ct, struct ctype* mt, int* pbit_offset, int* pbitfield_type)
{
...
/* Parses a struct from after the open lwrly through to the close lwrly.
 */
...
static int callwlate_struct_offsets(lua_State* L, struct parser* P, int ct_usr, struct ctype* ct, int tmp_usr)
{
    int i;
    int midx = 1;
    ...
    for (i = 1; i <= sz; i++) {
        ...
        callwlate_member_position(L, P, ct, &mt, &bit_offset, &bitfield_type);
...
static int parse_record(lua_State* L, struct parser* P, struct ctype* ct)
{
    ...
    if (ct->type == ENUM_TYPE) {
         ...
         * the temp user table. We then iterate over that to callwlate the offsets
        ...
        callwlate_struct_offsets(L, P, -2, ct, -1);
...
static struct ctype* parse_argument2(lua_State* L, struct parser* P, int ct_usr, struct ctype* ct, struct token* name, struct parser* asmname)
{
    ...
    for (;;) {
        if (!next_token(L, P, &tok)) {
            ...
            int64_t bsize = callwlate_constant(L, P);
...
{
    int64_t val = callwlate_constant(L, P);
            ...
            for (;;) {
                ...
                if (tok.type == TOK_OPEN_LWRLY) {
                    ...
                    while (remaining > 0 && next_token(L, P, &tok)) {
                        if (tok.type == TOK_CLOSE_LWRLY) {
                            remaining--;
                        } else if (tok.type == TOK_OPEN_LWRLY) {
                            remaining++;
                        }
                    }
                    if (remaining > 0) {
...
/* callwlate_constant handles operator precedence by having a number of
 * relwrsive commands each of which computes the result at that level of
 * precedence and above. callwlate_constant1 is the highest precedence
 */
...
static int64_t callwlate_constant2(lua_State* L, struct parser* P, struct token* tok);

/* () */
static int64_t callwlate_constant1(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t ret;
    ...
    if (tok->type == TOK_NUMBER) {
        ...
        if (!lua_pcall(L, 1, 0, 0)) {
            next_token(L, P, tok);
            return callwlate_constant2(L, P, tok);
        }
        lua_settop(L, top);

        *P = before_cast;
        ret = callwlate_constant(L, P);
...
static int64_t callwlate_constant2(lua_State* L, struct parser* P, struct token* tok)
{
    if (tok->type == TOK_LOGICAL_NOT) {
        ...
        return callwlate_constant1(L, P, tok);
    }
}

/* binary * / and % (left associative) */
static int64_t callwlate_constant3(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t left = callwlate_constant2(L, P, tok);

    for (;;) {
        if (tok->type == TOK_MULTIPLY) {
            require_token(L, P, tok);
            left *= callwlate_constant2(L, P, tok);
...
/* binary + and - (left associative) */
static int64_t callwlate_constant4(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t left = callwlate_constant3(L, P, tok);

    for (;;) {
        if (tok->type == TOK_PLUS) {
            require_token(L, P, tok);
            left += callwlate_constant3(L, P, tok);
...
/* binary << and >> (left associative) */
static int64_t callwlate_constant5(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t left = callwlate_constant4(L, P, tok);

    for (;;) {
        if (tok->type == TOK_LEFT_SHIFT) {
            require_token(L, P, tok);
            left <<= callwlate_constant4(L, P, tok);
...
/* binary <, <=, >, and >= (left associative) */
static int64_t callwlate_constant6(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t left = callwlate_constant5(L, P, tok);

    for (;;) {
        if (tok->type == TOK_LESS) {
            require_token(L, P, tok);
            left = (left < callwlate_constant5(L, P, tok));
...
static int64_t callwlate_constant7(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t left = callwlate_constant6(L, P, tok);

    for (;;) {
        if (tok->type == TOK_EQUAL) {
            require_token(L, P, tok);
            left = (left == callwlate_constant6(L, P, tok));
...
/* binary & (left associative) */
static int64_t callwlate_constant8(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t left = callwlate_constant7(L, P, tok);

    for (;;) {
        if (tok->type == TOK_BITWISE_AND) {
            require_token(L, P, tok);
            left = (left & callwlate_constant7(L, P, tok));
...
/* binary ^ (left associative) */
static int64_t callwlate_constant9(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t left = callwlate_constant8(L, P, tok);

    for (;;) {
        if (tok->type == TOK_BITWISE_XOR) {
            require_token(L, P, tok);
            left = (left ^ callwlate_constant8(L, P, tok));
...
/* binary | (left associative) */
static int64_t callwlate_constant10(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t left = callwlate_constant9(L, P, tok);

    for (;;) {
        if (tok->type == TOK_BITWISE_OR) {
            require_token(L, P, tok);
            left = (left | callwlate_constant9(L, P, tok));
...
/* binary && (left associative) */
static int64_t callwlate_constant11(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t left = callwlate_constant10(L, P, tok);

    for (;;) {
        if (tok->type == TOK_LOGICAL_AND) {
            require_token(L, P, tok);
            left = (left && callwlate_constant10(L, P, tok));
...
/* binary || (left associative) */
static int64_t callwlate_constant12(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t left = callwlate_constant11(L, P, tok);

    for (;;) {
        if (tok->type == TOK_LOGICAL_OR) {
            require_token(L, P, tok);
            left = (left || callwlate_constant11(L, P, tok));
...
static int64_t callwlate_constant13(lua_State* L, struct parser* P, struct token* tok)
{
    int64_t left = callwlate_constant12(L, P, tok);

    if (tok->type == TOK_QUESTION) {
        int64_t middle, right;
        require_token(L, P, tok);
        middle = callwlate_constant13(L, P, tok);
        if (tok->type != TOK_COLON) {
            luaL_error(L, "invalid ternery (? :) in constant on line %d", P->line);
        }
        require_token(L, P, tok);
        right = callwlate_constant13(L, P, tok);
...
int64_t callwlate_constant(lua_State* L, struct parser* P)
{
    ...
    int64_t ret;
    require_token(L, P, &tok);
    ret = callwlate_constant13(L, P, &tok);
