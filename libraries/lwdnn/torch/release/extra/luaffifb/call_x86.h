...
int x86_return_size(lua_State* L, int usr, const struct ctype* ct)
{
    ...
    if (ct->calling_colwention != C_CALL) {
        size_t i;
...
#else
#define MAX_INT_REGISTERS(ct) ((ct)->calling_colwention == FAST_CALL ? 2 /* ecx, edx */ : 0)
...
#else
#define REGISTER_STACK_SPACE(ct) ALIGN_UP(((ct)->calling_colwention == FAST_CALL ? 2*4 : 0), 15)
#endif
...
cfunction compile_callback(lua_State* L, int fidx, int ct_usr, const struct ctype* ct)
{
    ...
    if (ct->calling_colwention == FAST_CALL) {
        dasm_put(Dst, 105);
    }
...
void compile_function(lua_State* L, cfunction func, int ct_usr, const struct ctype* ct)
{
    ...
    *(cfunction*) p = func;
    ...
    if (ct->calling_colwention != C_CALL && ct->has_var_arg) {
