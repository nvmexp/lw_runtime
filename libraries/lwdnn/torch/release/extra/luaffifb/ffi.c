...
static int type_error(lua_State* L, int idx, const char* to_type, int to_usr, const struct ctype* to_ct)
{
    ...
    if (ft.type != ILWALID_TYPE) {
        push_type_name(L, -1, &ft);
        lua_pushfstring(L, "unable to colwert argument %d from cdata<%s> to cdata<", idx, lua_tostring(L, -1));
        lua_remove(L, -2);
        luaL_addvalue(&B);
    } else {
        lua_pushfstring(L, "unable to colwert argument %d from lua<%s> to cdata<", idx, luaL_typename(L, idx));
        luaL_addvalue(&B);
    }
...
/* to_enum tries to colwert a value at idx to the enum type indicated by to_ct
...
/* to_pointer tries colwerts a value at idx to a pointer. It fills out ct and
 * pushes the uv of the found type. It will throw a lua error if it can not
 * colwert the value to a pointer. */
...
/* to_typed_pointer colwerts a value at idx to a type tt with target uv to_usr
...
void* check_typed_pointer(lua_State* L, int idx, int to_usr, const struct ctype* tt)
{
    ...
    if (is_void_ptr(tt)) {
        /* any pointer can colwert to void* */
        goto suc;

    } else if (is_void_ptr(&ft) && (ft.pointers || ft.is_reference)) {
        /* void* can colwert to any pointer */
        goto suc;

    } else if (ft.is_null) {
        /* NULL can colwert to any pointer */
        goto suc;
...
/* to_cfunction colwerts a value at idx with usr table at to_usr and type tt
...
static cfunction check_cfunction(lua_State* L, int idx, int to_usr, const struct ctype* tt, int check_pointers)
{
    ...
    case LUA_TUSERDATA:
        ...
        } else if (ft.calling_colwention != tt->calling_colwention) {
            goto err;
...
/* to_type_cfunction colwerts a value at idx with uv at to_usr and type tt to
...
static int ffi_istype(lua_State* L)
{
    ...
    if (tt.calling_colwention != ft.calling_colwention) {
        goto fail;
    }
...
static int ffi_string(lua_State* L)
{
    ...
    return luaL_error(L, "unable to colwert cdata to string");
}
