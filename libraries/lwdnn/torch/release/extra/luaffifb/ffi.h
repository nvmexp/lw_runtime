...
#define lua_setuservalue lua_setfelw
#define lua_getuservalue lua_getfelw
...
#   define EnableExelwte(data, size) do {DWORD old; VirtualProtect(data, size, PAGE_EXELWTE, &old); FlushInstructionCache(GetLwrrentProcess(), data, size);} while (0)
...
#   define EnableExelwte(data, size) mprotect(data, size, PROT_READ|PROT_EXEC)
