...
--[[ Relwrsively applies type(type_str) to any tensors in the argument. If the
argument is a tensor, type(type_str) is applied; if the argument is an array,
this function relwrses into it. ]]
local function relwrsiveType(param, type_str)
   if torch.type(param) == 'table' then
      for i = 1, #param do
         param[i] = relwrsiveType(param[i], type_str)
      end
...
function gModule:type(type, tensorCache)
   ...
   local function applyTypeToTable(table)
      for key, value in pairs(table) do
         table[key] = relwrsiveType(table[key], type)
      end
   end

   -- Colwert any stored data in self, and in the in and out nodes
   ...
   -- Loop through modules and colwert data
   ...
   for i,node in ipairs(self.backwardnodes) do
      if node.data.gradOutputBuffer ~= nil then
         node.data.gradOutputBuffer =
            relwrsiveType(node.data.gradOutputBuffer, type)
      end
   ...
   for i,node in ipairs(self.forwardnodes) do
      if node.data.input ~= nil then
         node.data.input = relwrsiveType(node.data.input, type)
      end
      for k, child in ipairs(node.children) do
         applyTypeToTable(child.data)
      end
