...
-- only numerical columns (deduced from first data row) are colwerted by default
...
-- fieldnames are colwerted into valid identifiers by substituting _
...
-- the `colwert` option provides custom colwerters for each specified column.
-- Here we colwert the timestamps into Date objects and collect everything
-- else into one field
local Date = require 'pl.Date'

local function date_colwert (ds)
    return Date(tonumber(ds))
end

local d = data.read(f,{colwert={[1]=date_colwert},last_field_collect=true})
