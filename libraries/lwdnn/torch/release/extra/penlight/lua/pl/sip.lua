...
-- one-letter type, and then an optional variable in lwrly braces.
--
...
function sip.lwstom_pattern(flag,patt)
    pattern_map[flag] = patt
end

--- colwert a SIP pattern into the equivalent Lua string pattern.
-- @param spec a SIP pattern
-- @param options a table; only the <code>at_start</code> field is
-- lwrrently meaningful and ensures that the pattern is anchored
...
--- colwert a SIP pattern into a matching function.
