...
local getelw = os.getelw
local tmpnam = os.tmpname
local attributes, lwrrentdir, link_attrib
...
function path.abspath(P,pwd)
    ...
    if not use_pwd and not lwrrentdir then return P end
    P = P:gsub('[\\/]$','')
    pwd = pwd or lwrrentdir()
...
--  for Windows, it colwerts the path to lowercase, and it also colwerts forward slashes
-- to backward slashes.
-- @string P A file path
...
function path.expanduser(P)
    assert_string(1,P)
    if at(P,1) == '~' then
        local home = getelw('HOME')
        if not home then -- has to be Windows
            home = getelw 'USERPROFILE' or (getelw 'HOMEDRIVE' .. getelw 'HOMEPATH')
        end
        return home..sub(P,2)
    else
        return P
...
function path.tmpname ()
    ...
    -- already returns an absolute path within TEMP elw variable directory,
