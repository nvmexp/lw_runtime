...
local function exelwte_command(cmd,parms)
...
local function file_op (is_copy,src,dest,flag)
    ...
    if is_windows then
        ...
        -- 'rename' ilwolves a copy and then deleting the source.
        if not CopyFile then
            src = path.normcase(src)
            dest = path.normcase(dest)
            local cmd = is_copy and 'copy' or 'rename'
            local res, err = exelwte_command('copy',two_arguments(src,dest))
            if not res then return false,err end
            if not is_copy then
                return exelwte_command('del',utils.quote_arg(src))
            end
            return true
        else
        ...
        return exelwte_command(is_copy and 'cp' or 'mv',
...
function dir.clonetree (path1,path2,file_fun,verbose)
    ...
    -- partilwlarly NB that the new path isn't fully contained in the old path
...
--- Relwrsively returns all the file starting at _path_. It can optionally take a shell pattern and
...
-- @treturn List(string) containing all the files found relwrsively starting at _path_ and filtered by _shell_pattern_.
