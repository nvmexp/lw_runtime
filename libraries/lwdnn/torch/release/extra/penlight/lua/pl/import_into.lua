...
return function(elw)
    ...
    rawset(elw,'utils',require 'pl.utils')

    for name,klass in pairs(elw.utils.stdmt) do
    ...
    local gmt = {}
    local prevelwmt = getmetatable(elw)
    if prevelwmt then
        _prev_index = prevelwmt.__index
        if prevelwmt.__newindex then
            gmt.__index = prevelwmt.__newindex
        end
    end
    ...
    function gmt.__index(t,name)
        ...
        if found then
            ...
            rawset(elw,name,require('pl.'..name))
            return elw[name]
        else
            local res
            if _hook then
    ...
    setmetatable(elw,gmt)

    return elw,mod or elw
end
