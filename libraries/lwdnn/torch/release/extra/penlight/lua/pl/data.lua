...
-- @bool cnfg.no_colwert (default is to try colwersion on first data line)
-- @tab cnfg.colwert table of custom colwersion functions with column keys
...
function data.read(file,cnfg)
    ...
    -- requires a special colwersion function! For CSV, _empty fields_ are
    ...
    local colwersion
    local numfields = {}
    local function append_colwersion (idx,colw)
        colwersion = colwersion or {}
        append(numfields,idx)
        append(colwersion,colw)
    end
    if cnfg.numfields then
        for _,n in ipairs(cnfg.numfields) do append_colwersion(n,try_number) end
    end
    ...
    if not cnfg.fieldnames then
        ...
        if not cnfg.colwert then
            nums = map(tonumber,fields)
            if #nums == #fields then -- they're ALL numbers!
                append(D,nums) -- add the first colwerted row
                -- and specify colwersions for subsequent rows
                for i = 1,#nums do append_colwersion(i,try_number) end
            else -- we'll try to check numbers just now..
                nums = nil
            end
        else -- [explicit column colwersions] (any deduced number colwersions will be added)
            for idx,colw in pairs(cnfg.colwert) do append_colwersion(idx,colw) end
        end
        if nums == nil then
    ...
    while line do
        if not line:find ('^%s*$') then -- [blank lines] ignore them!
            ...
            if colwersion then -- there were field colwersions...
                for k = 1,#numfields do
                    local i,colw = numfields[k],colwersion[k]
                    local val,err = colw(fields[i])
                    if val == nil then
