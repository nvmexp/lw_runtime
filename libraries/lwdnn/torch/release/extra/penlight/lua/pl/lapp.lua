...
local function xtonumber(s)
    ...
    if not val then lapp.error("unable to colwert to number: "..s) end
    return val
end
...
local function colwert_parameter(ps,val)
    if ps.colwerter then
        val = ps.colwerter(val)
    end
...
-- @param colwerter either a function to colwert values, or a Lua type name.
-- @func[opt] constraint optional function to verify values, should use lapp.error
-- if failed.
function lapp.add_type (name,colwerter,constraint)
    types[name] = {colwerter=colwerter,constraint=constraint}
end
...
local function process_default (sval,vtype)
    ...
    elseif filetypes[sval] then
        ...
        success, val = pcall(colwert_parameter, ps, sval)
...
function lapp.process_options_string(str,args)
    ...
    for line in lines(str) do
        ...
        if res.rest then
            ...
            varargs = nil
            if types[vtype] then
                local colwerter = types[vtype].colwerter
                if type(colwerter) == 'string' then
                    ps.type = colwerter
                else
                    ps.colwerter = colwerter
                end
