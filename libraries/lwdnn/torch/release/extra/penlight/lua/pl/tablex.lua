...
--- make a deep copy of a table, relwrsively copying all the keys and fields.
...
--- compare two values.
-- if they are tables, then compare their keys and fields relwrsively.
-- @within Comparing
-- @param t1 A value
-- @param t2 A value
...
-- are not nil, then this pair is inserted into the result: if the key already exists, we colwert the value for that
...
-- set operations for sets () but more general. Not partilwlarly
...
--- find a value in a table by relwrsive search.
-- @within Finding
-- @tab t the table
