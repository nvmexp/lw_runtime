...
local function parseDollarParen(pieces, chunk, exec_pat, newline)
    local s = 1
    for term, exelwted, e in chunk:gmatch(exec_pat) do
        exelwted = '('..strsub(exelwted,2,-2)..')'
        append(pieces, APPENDER..format("%q", strsub(chunk,s, term - 1)))
        append(pieces, APPENDER..format("(%s or '')", exelwted))
        s = e
    end
    local r
    if newline then
...
-- relwrring usage use the two step approach by using `compile` and `ct:render`.
-- There are six special fields in the environment table `elw`
--
...
function template.substitute(str,elw)
    elw = elw or {}
    local t, err = template.compile(str, {
        chunk_name = rawget(elw,"_chunk_name"),
        escape = rawget(elw,"_escape"),
        inline_escape = rawget(elw,"_inline_escape"),
        inline_brackets = rawget(elw,"_brackets"),
        newline = nil,
        debug = rawget(elw,"_debug")
    })
    if not t then return t, err end
    
    return t:render(elw, rawget(elw,"_parent"), rawget(elw,"_debug"))
end

--- exelwtes the previously compiled template and renders it.
-- @function ct:render
-- @tab[opt] elw the environment.
...
-- local rendered , err = ct:render(my_elw, parent)
local render = function(self, elw, parent, db)
    elw = elw or {}
    if parent then  -- parent is a bit silly, but for backward compatibility retained
        setmetatable(elw, {__index = parent})
    end
    setmetatable(self.elw, {__index = elw})
...
function template.compile(str, opts)
    ...
    local elw = {}
    local fn, err = utils.load(code, chunk_name,'t',elw)
