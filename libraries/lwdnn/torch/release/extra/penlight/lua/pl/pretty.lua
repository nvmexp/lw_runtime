...
local function save_global_elw()
    local elw = {}
    elw.hook, elw.mask, elw.count = debug.gethook()
    
    -- elw.hook is "external hook" if is a C hook function
    if elw.hook~="external hook" then
        debug.sethook()
    end
    
    elw.string_mt = getmetatable("")
    debug.setmetatable("", nil)
    return elw
end

local function restore_global_elw(elw)
    if elw then
        debug.setmetatable("", elw.string_mt)
        if elw.hook~="external hook" then
            debug.sethook(elw.hook, elw.mask, elw.count)
        end
    end
end
...
-- Loaded string is exelwted in an empty environment.
-- @string s string to read in `{...}` format, possibly with some whitespace
-- before or after the lwrly braces. A single line comment may be present
-- at the beginning.
-- @return a table in case of success.
-- If loading the string failed, return `nil` and error message.
-- If exelwting loaded string failed, return `nil` and the error it raised.
function pretty.read(s)
    ...
    local global_elw = save_global_elw()
    local ok,ret = pcall(chunk)
    restore_global_elw(global_elw)
...
--- Read a Lua chunk.
-- @string s Lua code.
-- @tab[opt] elw environment used to run the code, empty by default.
...
function pretty.load (s, elw, paranoid)
    elw = elw or {}
    if paranoid then
        local tok = lexer.lua(s)
        for t,v in tok do
    ...
    local chunk,err = utils.load(s,'tbl','t',elw)
    if not chunk then return nil,err end
    local global_elw = paranoid and save_global_elw()
