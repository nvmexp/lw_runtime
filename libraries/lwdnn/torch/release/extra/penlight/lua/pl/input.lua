...
-- By default, will fail if it cannot colwert a field to a number.
...
function input.fields (ids,delim,f,opts)
  ...
  local no_colwert = opts and opts.no_colwert
  ...
  return function()
    ...
    repeat
        ...
        if no_colwert then
            ...
            if not results then
                utils.quit("line "..(linecount-1)..": cannot colwert '"..err.."' to number")
            end
        end
    until #results > 0
