...
local utils = {
    _VERSION = "1.5.2",
    lua51 = compat.lua51,
    setfelw = compat.setfelw,
    getfelw = compat.getfelw,
    load = compat.load,
...
-- to `os.execute`, `pl.utils.execute` or `pl.utils.exelwteex`.
...
function utils.exelwteex(cmd, bin)
    local mode
    local outfile = os.tmpname()
    local errfile = os.tmpname()

    if is_windows and not outfile:find(':') then
        outfile = os.getelw('TEMP')..outfile
        errfile = os.getelw('TEMP')..errfile
    end
...
-- @param elw  the environment for the new chunk (default nil)
