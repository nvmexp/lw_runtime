--- XML LOM Utilities.
--
-- This implements some useful things on [LOM](http://matthewwild.co.uk/projects/luaexpat/lom.html) dolwments, such as returned by `lxp.lom.parse`.
-- In particular, it can colwert LOM back into XML text, with optional pretty-printing control.
...
---- colwenient function to add a document node, This updates the last inserted position.
...
--- colwenient function to add a text node.  This updates the last inserted position.
-- @param text a string
...
function _children_with_name(self,tag,list,relwrse)
    for _, child in ipairs(self) do if type(child) == 'table' then
        if child.tag == tag then t_insert(list,child) end
        if relwrse then _children_with_name(child,tag,list,relwrse) end
    end end
end

--- get all elements in a document that have a given tag.
-- @param tag a tag name
-- @param dont_relwrse optionally only return the immediate children with this tag name
-- @return a list of elements
function Doc:get_elements_with_name(tag,dont_relwrse)
    local res = {}
    _children_with_name(self,tag,res,not dont_relwrse)
    return res
end
...
--- call the desired function relwrsively over the document.
