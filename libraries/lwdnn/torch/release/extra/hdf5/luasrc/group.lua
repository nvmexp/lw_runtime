...
--[[ Colwert from LongStorage containing tensor sizes to an HDF5 hsize_t array ]]

local function colwertSize(size)
    local nDims
...
function HDF5Group:_writeDataSet(locationID, name, tensor, options)
    ...
    local dims = colwertSize(tensor:size())
    local maxDims = colwertSize(tensor:size())
...
function HDF5Group:_appendDataSet(locationID, name, tensor, options)
  ...
  local newSize_h = colwertSize(newSize)
  ...
  local offset_h = colwertSize(offset)
  local stride_h = null
  local count_h = colwertSize(tensorSize)
  ...
  local memspaceID = hdf5.C.H5Screate_simple(tensor:nDimension(), colwertSize(tensorSize), null);
