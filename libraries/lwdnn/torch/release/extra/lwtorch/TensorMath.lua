...
wrap.types[typename] = {
   ...
   init = function(arg)
      if type(arg.default) == 'boolean' then
         return string.format('arg%d = TH%s_new(lwtorch_getstate(L));', arg.i, typename)
...
wrap.types.LongArg = {
   ...
   check = function(arg, idx)
      return string.format("lwtorch_islongargs(L, %d)", idx)
   end,

   read = function(arg, idx)
      return string.format("arg%d = lwtorch_checklongargs(L, %d);", arg.i, idx)
   end,
...
lwtorch_state_code = function(varname)
  local txt = {}
  table.insert(txt, 'lua_getglobal(L, "lwtorch");')
...
interface:registerDefaultArgument(lwtorch_state_code)
method:registerDefaultArgument(lwtorch_state_code)
...
-- functions to help take in arguments that are Tensor or LwdaLongTensor (for backward compatibility)
-- used in scatter / gather for example
local function TensorToLwdaLong_declare(dummy)
      return function(arg)
	 local txt = {}
	 table.insert(txt, string.format("THLwdaLongTensor *arg%d = NULL;", arg.i))
	 if dummy then
	    table.insert(txt, string.format("THLwdaLongTensor *indexLongTensor = NULL;"))
	    table.insert(txt, string.format("TH%s *dummyIndexTensor = NULL;", Tensor))
	 end
	 return table.concat(txt, '\n')
      end
end
local function TensorToLwdaLong_check(arg, idx)
   return string.format('(dummyIndexTensor = luaT_toudata(L, %d, "torch.%s"))', idx, Tensor)
end
local function TensorToLwdaLong_read(arg, idx)
   local copyname = Tensor:match("(%a+)Tensor")
   if copyname == 'Lwca' then
      copyname = 'LwdaFloat'
   end
   local txt = {}
   table.insert(txt, string.format('arg%d = THLwdaLongTensor_new(default_arg1);', arg.i))
   table.insert(txt, string.format('THLongStorage *indexSize = TH%s_newSizeOf(default_arg1, dummyIndexTensor);', Tensor))
   table.insert(txt, string.format('THLwdaLongTensor_resize(default_arg1, arg%d, indexSize, NULL);', arg.i))
   table.insert(txt, string.format('THLongStorage_free(indexSize);'))
   table.insert(txt, string.format('THLwdaLongTensor_copy%s(default_arg1, arg%d, dummyIndexTensor);', copyname, arg.i))
   table.insert(txt, string.format('indexLongTensor = arg%d;', arg.i))
   return table.concat(txt, '\n')
end

local function TensorToLwdaLong_postcall(arg)
   return "if (indexLongTensor != NULL) THLwdaLongTensor_free(default_arg1, indexLongTensor);\n"
end
...
local function gatherInit(arg)
   return table.concat(
	 ...
	 string.format("TH%s_checkGPU(lwtorch_getstate(L), 1, %s);",
		       Tensor, arg.args[4]:carg()),
	 string.format(
	    [[
		  THCState *state = lwtorch_getstate(L);
		  THLongStorage *indicesSize = THLwdaLongTensor_newSizeOf(state, %s);
...
--
-- Non-LwdaTensor type math, since these are less fully implemented than
-- LwdaTensor
--
...
for k, Tensor_ in pairs(handledTypenames) do
    Tensor = Tensor_
    if Tensor == 'LwdaHalfTensor' then
        interface:print("#ifdef LWDA_HALF_TENSOR")
    end
    local real = handledTypereals[k]
    local accreal = handledTypeaccreals[k]

    function interface.luaname2wrapname(self, name)
        return string.format('lwtorch_%s_%s', Tensor, name)
    end

    function method.luaname2wrapname(self, name)
        return string.format('m_lwtorch_%s_%s', Tensor, name)
    end
    ...
    local function lastdim(argn)
        return function(arg)
            return string.format('TH%s_nDimension(lwtorch_getstate(L), %s)',
    ...
    local function lastdimarray(argn)
        return function(arg)
            return string.format('TH%s_nDimension(lwtorch_getstate(L), arg%d_data[0])',
    ...
    for _,name in ipairs({"min", "max"}) do
       wrap(name,
            ...
            cname(name),
            {{name=Tensor, default=true, returned=true},
               {name='LwdaLongTensor', default=true, returned=true},
               {name=Tensor},
               {name="index"},
   ...
   for _,name in pairs({'lt','gt','le','ge','eq','ne'}) do
      wrap(name,
           cname(name .. 'Value'),
           {{name='LwdaByteTensor',default=true, returned=true},
            {name=Tensor},
            {name=real}},
           cname(name .. 'ValueT'),
           {{name=Tensor, returned=true},
    ...
    for _, name in ipairs({"lwmsum", "lwmprod"}) do
        wrap(name,
             cname(name),
             {{name=Tensor, default=true, returned=true},
                 {name=Tensor},
    ...
    wrap("gather",
	 ...
	 {{name=Tensor, default=true, returned=true, init=gatherInit},
	    {name=Tensor},
	    {name="index"},
	    {name=Tensor, declare=TensorToLwdaLong_declare(true), check=TensorToLwdaLong_check, read=TensorToLwdaLong_read, postcall=TensorToLwdaLong_postcall}})
    ...
    wrap("scatter",
	 ...
	 {{name=Tensor, returned=true},
	    {name="index"},
	    {name=Tensor, declare=TensorToLwdaLong_declare(false), check=TensorToLwdaLong_check, read=TensorToLwdaLong_read, postcall=TensorToLwdaLong_postcall},
	    {name=real}}
    )
    ...
    wrap("sort",
         cname("sort"),
         {{name=Tensor, default=true, returned=true},
             {name="LwdaLongTensor", default=true, returned=true, noreadadd=true},
             {name=Tensor},
   ...
   wrap("topk",
    ...
    wrap("squeeze",
         ...
         {{name=Tensor, default=true, returned=true, postcall=function(arg)
              ...
              if arg.returned then
                 ...
                 if Tensor == 'LwdaHalfTensor' then
                    table.insert(txt, string.format('lua_pushnumber(L, (lua_Number)THC_half2float(TH%s_get1d(lwtorch_getstate(L), arg%d, 0)));', Tensor, arg.i))
                 else
                    table.insert(txt, string.format('lua_pushnumber(L, (lua_Number)(TH%s_get1d(lwtorch_getstate(L), arg%d, 0)));', Tensor, arg.i))
                 end
              end
              return table.concat(txt, '\n')
          end},
    ...
    if real == 'float' or real == 'double' or real == 'half' then
     ...
     wrap("multinomial",
          cname("multinomial"),
          {{name='LwdaLongTensor', default=true, returned=true, method={default='nil'}},
           {name=Tensor},
           {name="int"},
                 ...
                 return table.concat(
                    {
                       ...
                       string.format("TH%s_resize1d(lwtorch_getstate(L), %s, %s->size[0]);", Tensor, arg:carg(), arg.args[5]:carg())
                    }, '\n')
              end,
              precall=function(arg)
                 return table.concat(
                    {
                       string.format("TH%s_zero(lwtorch_getstate(L), %s);", Tensor, arg:carg()),
            ...
            {{name=Tensor, default=true, returned=true, method={default='nil'},
              init=function(arg)
                 return table.concat(
                    {
                       arg.__metatable.init(arg),
                       string.format("TH%s_checkGPU(lwtorch_getstate(L), 2, %s, %s);",
                                     Tensor, arg.args[5]:carg(), arg.args[6]:carg()),
                       string.format("TH%s_resize2d(lwtorch_getstate(L), %s, %s->size[0], %s->size[1]);",
                 ...
                 return table.concat(
                    {
                       ...
                       string.format("TH%s_checkGPU(lwtorch_getstate(L), 2, %s, %s);",
                                     Tensor, arg.args[5]:carg(), arg.args[6]:carg()),
                       string.format("TH%s_resize3d(lwtorch_getstate(L), %s, %s->size[0], %s->size[1], %s->size[2]);",
                 ...
                 return table.concat(
                    {
                       ...
                       string.format("TH%s_checkGPU(lwtorch_getstate(L), 2, %s, %s);",
                                     Tensor, arg.args[5]:carg(), arg.args[6]:carg()),
                       string.format("TH%s_resize2d(lwtorch_getstate(L), %s, %s->size[0], %s->size[0]);", Tensor, arg:carg(), arg.args[5]:carg(), arg.args[6]:carg())
                    }, '\n')
              end,
              precall=function(arg)
                 return table.concat(
    ...
    method:register("m_lwtorch_" .. Tensor .. "Math__")
    interface:print(method:tostring())
    method:clearhistory()
    method:registerDefaultArgument(lwtorch_state_code)
    interface:register("lwtorch_" .. Tensor .. "Math__")

    interface:print(string.format([[
void lwtorch_%sMath_init(lua_State *L)
{
  luaT_pushmetatable(L, "torch.%s");

  /* register methods */
  luaL_setfuncs(L, m_lwtorch_%sMath__, 0);

  /* register functions into the "torch" field of the tensor metaclass */
  lua_pushstring(L, "torch");
  lua_newtable(L);
  luaL_setfuncs(L, lwtorch_%sMath__, 0);
  lua_rawset(L, -3);
  lua_pop(L, 1);
}
...
--
-- LwdaTensor special handling, since it is more fully implemented
--
...
wrap("squeeze",
     ...
     {{name=Tensor, default=true, returned=true, postcall=function(arg)
          ...
          if arg.returned then
             table.insert(txt, string.format('if(arg%d->nDimension == 1 && arg%d->size[0] == 1)', arg.i, arg.i)) -- number
             table.insert(txt, string.format('lua_pushnumber(L, (lua_Number)(THLwdaTensor_get1d(lwtorch_getstate(L), arg%d, 0)));', arg.i))
          end
          return table.concat(txt, '\n')
     end},
      {name=Tensor}},
