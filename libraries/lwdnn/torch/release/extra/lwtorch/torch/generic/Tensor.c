...
{
  lua_pushnumber(L, THCStorage_(elementSize)(lwtorch_getstate(L)));
  return 1;
}
...
static int torch_Tensor_(storage)(lua_State *L)
{
  ...
  if(tensor->storage)
  {
    THCStorage_(retain)(lwtorch_getstate(L), tensor->storage);
...
static int torch_Tensor_(set)(lua_State *L)
{
  ...
  THCTensor_(setStorage)(lwtorch_getstate(L), self, storage, storageOffset, size, stride);
...
{
  THCTensor *self = luaT_checkudata(L, 1, torch_Tensor);
  self = THCTensor_(newClone)(lwtorch_getstate(L), self);
...
{
  THCTensor *self = luaT_checkudata(L, 1, torch_Tensor);
  self = THCTensor_(newContiguous)(lwtorch_getstate(L), self);
  luaT_pushudata(L, self, torch_Tensor);
  return 1;
}
...
static int torch_Tensor_(resizeAs)(lua_State *L)
{
  ...
  THCTensor_(resizeAs)(lwtorch_getstate(L), tensor, src);
  lua_settop(L, 1);
  return 1;
}
...
static int torch_Tensor_(resize)(lua_State *L)
{
  ...
  THCTensor_(resize)(lwtorch_getstate(L), tensor, size, stride);
...
static int torch_Tensor_(indexSelect)(lua_State *L)
{
  ...
  if (narg == 3)
  {
    ...
    index = luaT_toudata(L, 3, "torch.LwdaLongTensor");
    longIndex = luaT_toudata(L, 3, "torch.LongTensor");
    realIndex = luaT_toudata(L, 3, torch_Tensor);
    if (!index && !longIndex && !realIndex) luaT_typerror(L, 3, "LwdaLongTensor | LongTensor | Tensor");
    luaT_pushudata(L,tensor,torch_Tensor);
  }
  else if(narg == 4)
  {
    ...
    index = luaT_toudata(L, 4, "torch.LwdaLongTensor");
  ...
  {
    luaL_error(L, "[Tensor,] Tensor, number, Tensor | LongTensor | LwdaLongTensor expected");
    return 0;
  }
...
#endif
    THCState *state = lwtorch_getstate(L);
    index = THLwdaLongTensor_new(state);
    THLongStorage *indexSize = THCTensor_(newSizeOf)(state, realIndex);
    THLwdaLongTensor_resize(state, index, indexSize, NULL);
    THLongStorage_free(indexSize);
    TH_CONCAT_2(THLwdaLongTensor_copyLwda, Real)(state, index, realIndex);
    THCTensor_(indexSelect)(state, tensor, src, dim, index);
    THLwdaLongTensor_free(state, index);
  }
...
static int torch_Tensor_(indexAdd)(lua_State *L)
{
  ...
  {
    luaL_error(L,"Tensor, number, Tensor | LongTensor | LwdaLongTensor, Tensor expected");
    return 0;
  }

  if (index)
    THCTensor_(indexAdd)(lwtorch_getstate(L), tensor, dim, index, src);
  else if (longIndex)
    THCTensor_(indexAdd_long)(lwtorch_getstate(L), tensor,dim,longIndex,src);
  else { // realIndex
#if defined THC_REAL_IS_BYTE || defined THC_REAL_IS_CHAR
    THError("indices are in a ByteTensor or CharTensor which is not allowed. "
            "ByteTensor | CharTensor have a small range of indices to "
...
static int torch_Tensor_(indexFill)(lua_State *L)
{
  ...
  {
    luaL_error(L,"Tensor, number, Tensor | LongTensor | LwdaLongTensor, number expected");
    return 0;
  }

  if (index)
    THCTensor_(indexFill)(lwtorch_getstate(L), tensor, dim, index, val);
  else if (longIndex)
    THCTensor_(indexFill_long)(lwtorch_getstate(L), tensor, dim, longIndex, val);
  else { // realIndex
#if defined THC_REAL_IS_BYTE || defined THC_REAL_IS_CHAR
    THError("indices are in a ByteTensor or CharTensor which is not allowed. "
            "ByteTensor | CharTensor have a small range of indices to "
...
{
  THCTensor *tensor = luaT_checkudata(L, 1, torch_Tensor);
  lua_pushboolean(L, THCTensor_(isContiguous)(lwtorch_getstate(L), tensor));
  return 1;
}
...
static int torch_Tensor_(isSize)(lua_State *L)
{
  ...
  lua_pushboolean(L, THCTensor_(isSize)(lwtorch_getstate(L), tensor, size));
  return 1;
}
...
static int torch_Tensor_(isSetTo)(lua_State *L)
{
  THCTensor *self = luaT_checkudata(L, 1, torch_Tensor);
  THCTensor *src = luaT_checkudata(L, 2, torch_Tensor);
  lua_pushboolean(L, THCTensor_(isSetTo)(lwtorch_getstate(L), self, src));
  return 1;
}
...
static int torch_Tensor_(isSameSizeAs)(lua_State *L)
{
  THCTensor *self = luaT_checkudata(L, 1, torch_Tensor);
  THCTensor *src = luaT_checkudata(L, 2, torch_Tensor);
  lua_pushboolean(L, THCTensor_(isSameSizeAs)(lwtorch_getstate(L), self, src));
  return 1;
}
...
{
  THCTensor *tensor = luaT_checkudata(L, 1, torch_Tensor);
  lua_pushinteger(L, THCTensor_(nElement)(lwtorch_getstate(L), tensor));
  return 1;
}
...
static int torch_Tensor_(__newindex__)(lua_State *L)
{
  ...
  THByteTensor *mask;
  THLwdaByteTensor *maskLwda;
  THCTensor *maskLwdaReal;
  ...
  else if((maskLwda = luaT_toudata(L, 2, "torch.LwdaByteTensor")))
  {
    THCTensor *vals;
    if (lua_isnumber(L, 3))
    {
    ...
    {
      THCTensor_(maskedCopy)(state, tensor, maskLwda, vals);
    }
    else
    {
      luaL_error(L,"number or tensor expected");
  ...
  else if((maskLwdaReal = luaT_toudata(L, 2, torch_Tensor)))
  {
    maskLwda = THLwdaByteTensor_new(state);
    THLongStorage *maskLwdaSize = THCTensor_(newSizeOf)(state, maskLwdaReal);
    THLwdaByteTensor_resize(state, maskLwda, maskLwdaSize, NULL);
    THLongStorage_free(maskLwdaSize);
    TH_CONCAT_2(THLwdaByteTensor_copyLwda, Real)(state, maskLwda, maskLwdaReal);
...
static int torch_Tensor_(__index__)(lua_State *L)
{
  ...
  {
    THCTensor *vals = THCTensor_(new)(state);
    THCTensor_(maskedSelect)(state, vals, tensor, maskLwda);
    luaT_pushudata(L, vals, torch_Tensor);
    lua_pushboolean(L, 1);
...
{
  THCTensor *tensor = luaT_checkudata(L, 1, torch_Tensor);
  THCTensor_(retain)(lwtorch_getstate(L), tensor);
  return 0;
}
...
{
  THCTensor *tensor = luaT_checkudata(L, 1, torch_Tensor);
  THCTensor_(free)(lwtorch_getstate(L), tensor);
  return 0;
}
...
{
  THCTensor *tensor = THCTensor_(new)(lwtorch_getstate(L));
  luaT_pushudata(L, tensor, torch_Tensor);
  return 1;
}
