...
static int torch_Storage_(retain)(lua_State *L)
{
  ...
  THCStorage_(retain)(lwtorch_getstate(L), storage);
  return 0;
}
...
{
  THCStorage *storage = luaT_checkudata(L, 1, torch_Storage);
  THCStorage_(free)(lwtorch_getstate(L), storage);
  return 0;
}
...
/*  int keepContent = luaT_optboolean(L, 3, 0); */
  THCStorage_(resize)(lwtorch_getstate(L), storage, size);/*, keepContent); */
  lua_settop(L, 1);
  return 1;
}
...
#endif
  THCStorage_(fill)(lwtorch_getstate(L), storage, value);
  lua_settop(L, 1);
  return 1;
}
...
#endif
    THCStorage_(set)(lwtorch_getstate(L), storage, index, value);
    lua_pushboolean(L, 1);
  }
  else
    lua_pushboolean(L, 0);
...
static int torch_Storage_(__index__)(lua_State *L)
{
  if(lua_isnumber(L, 2))
  {
    THCStorage *storage = luaT_checkudata(L, 1, torch_Storage);
    ptrdiff_t index = luaL_checkinteger(L, 2) - 1;
    real v = THCStorage_(get)(lwtorch_getstate(L), storage, index);
...
static int torch_Storage_(totable)(lua_State *L)
{
  ...
  THStorage_(copyLwda)(state, host_storage, storage);
...
{
  THCStorage *storage = THCStorage_(new)(lwtorch_getstate(L));
  luaT_pushudata(L, storage, torch_Storage);
  return 1;
}
...
static int torch_Storage_(read)(lua_State *L)
{
  ...
  THCStorage_(resize)(lwtorch_getstate(L), storage, size);
