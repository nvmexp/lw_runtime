...
void destroyGenerator(THCState *state, Generator* gen)
{
  if (gen->gen_states)
  {
    THLwdaCheck(THLwdaFree(state, gen->gen_states));
    gen->gen_states = NULL;
  }
  if (gen->kernel_params)
  {
    THLwdaCheck(THLwdaFree(state, gen->kernel_params));
...
void THCRandom_init(THCState* state, int devices, int lwrrent_device)
{
...
struct lwrandStateMtgp32* THCRandom_generatorStates(struct THCState* state)
{
