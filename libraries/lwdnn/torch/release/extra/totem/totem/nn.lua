...
local function checkpointRNG()
    ...
    local lwstate = rawget(_G, 'lwtorch') and lwtorch.getRNGState() or nil
    local function revertRNG()
        torch.setRNGState(state)
        if lwstate then
            lwtorch.setRNGState(lwstate)
        end
    end
    return revertRNG
end
...
function totem.nn.checkGradients(tester, module, input, precision)
    ...
    for _, pair in ipairs(extractParamTensors(module, input)) do
        ...
        -- will be aclwmulated to non-zero initial gradParams.
...
function totem.nn.checkTypeCastable(tester, module, input, toType, precision)
    ...
    -- relwrsively traverse an object and return the names of all objects of type
    -- original and of type toType. Take care that cirlwlar references in the object
    -- are avoided by keeping track of the objects we have already visited
    local function findTensorsByType(obj, lwrlevel, accOrig, accToType, visitedObj)
        lwrlevel = lwrlevel or 'self'
        accOrig = accOrig or {}
        ...
        if type(obj) == 'table' then
            for k, v in pairs(obj) do
                ...
                if (not tableContains(visitedObj, v)) and
                    ...
                    accOrig, accToType, visitedObj = findTensorsByType(v, lwrlevel .. '.' .. k, accOrig, accToType, visitedObj)
                end
            end
        ...
        elseif torch.isTensor(obj) then
            ...
            if obj_type == origType then
                table.insert(accOrig, lwrlevel)
            end
            if obj_type == toType then
                table.insert(accToType, lwrlevel)
            end
            return accOrig, accToType, visitedObj
        else
            return accOrig, accToType, visitedObj
