...
function SpatialCrossMaxPooling:updateOutput(input)
  ...
  C['colwCrossMapMaxPool'](lwtorch.getState(), inputC:cdata(), self.output:cdata(), 0, self.kD, outputD, self.dD, input:size(2))
...
function SpatialCrossMaxPooling:updateGradInput(input, gradOutput)
  ...
  C['colwCrossMapMaxPoolUndo'](lwtorch.getState(), inputC:cdata(), gradOutputC:cdata(), outputC:cdata(), self.gradInput:cdata(),
