...
function SpatialAvgPooling:updateOutput(input)
  ...
  C['colwLocalAvgPool'](lwtorch.getState(), inputC:cdata(), self.output:cdata(), input:size(1), self.kW, 0, self.dW, outputX)
...
function SpatialAvgPooling:updateGradInput(input, gradOutput)
  ...
  C['colwLocalAvgUndo'](lwtorch.getState(), gradOutputC:cdata(), self.gradInput:cdata(), self.kW, 0, self.dW, outputX, input:size(2))
