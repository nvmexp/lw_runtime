...
local SpatialColwolution, parent = torch.class('ccn2.SpatialColwolution', 'nn.Module')

function SpatialColwolution:__init(nInputPlane, nOutputPlane, kH, dH, padding, groups, partialSum)
   parent.__init(self)
...
function SpatialColwolution:reset(stdv)
   if stdv then
...
function SpatialColwolution:updateOutput(input)
   ...
   -- do colwolution
   C['colwFilterActs'](lwtorch.getState(), inputC:cdata(), self.weight:cdata(), self.output:cdata(),
                       input:size(2), oH, oH,
...
function SpatialColwolution:updateGradInput(input, gradOutput)
   ...
   C['colwImgActs'](lwtorch.getState(), gradOutputC:cdata(), self.weight:cdata(), self.gradInput:cdata(),
                    iH, iH, oH,
                       -self.padding, self.dH, self.nInputPlane, self.groups);
...
function SpatialColwolution:accGradParameters(input, gradOutput, scale)
   ...
   if doPartialSum then
      ...
      C['colwWeightActsSt'](lwtorch.getState(), inputC:cdata(), gradOutputC:cdata(), self.wTemp:cdata(),
                            iH, oH, oH, self.kH,
      ...
      C['addSumCols'](lwtorch.getState(), self.gradWeight:cdata(), self.wTemp:cdata());
      self.gradWeight = self.gradWeight:viewAs(self.weight)
   else
      C['colwWeightActsSt'](lwtorch.getState(), inputC:cdata(), gradOutputC:cdata(), self.gradWeight:cdata(),
                            iH, oH, oH, self.kH,
