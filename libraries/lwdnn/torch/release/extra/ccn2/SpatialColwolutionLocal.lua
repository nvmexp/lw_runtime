...
local SpatialColwolutionLocal, parent = torch.class('ccn2.SpatialColwolutionLocal', 'nn.Module')

function SpatialColwolutionLocal:__init(nInputPlane, nOutputPlane, iH, kH, dH, padding)
   parent.__init(self)
...
function SpatialColwolutionLocal:reset(stdv)
   if stdv then
...
function SpatialColwolutionLocal:updateOutput(input)
   ...
   local inputC = input:view(input:size(1) * input:size(2) * input:size(3),
                             input:size(4))
   -- do colwolution
   C['localFilterActs'](lwtorch.getState(), inputC:cdata(), self.weight:cdata(), self.output:cdata(),
                       input:size(2), oH, oH, -self.padding, self.dH, self.nInputPlane, 1);
   -- add bias
   self.output = self.output:view(self.nOutputPlane, oH*oH*nBatch)
   C['addBias'](lwtorch.getState(), self.output:cdata(), self.bias:cdata());
   self.output = self.output:view(self.nOutputPlane, oH, oH, nBatch)
   return self.output
end

function SpatialColwolutionLocal:updateGradInput(input, gradOutput)
   ...
   C['localImgActs'](lwtorch.getState(), gradOutputC:cdata(), self.weight:cdata(), self.gradInput:cdata(),
                    iH, iH, oH, -self.padding, self.dH, self.nInputPlane, 1);
   self.gradInput = self.gradInput:view(self.nInputPlane, iH, iH, nBatch)
   return self.gradInput
end

function SpatialColwolutionLocal:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   ccn2.typecheck(input); ccn2.typecheck(gradOutput);
   ccn2.inputcheck(input); ccn2.inputcheck(gradOutput);
   local oH = gradOutput:size(2);
   ...
   C['localWeightActsSt'](lwtorch.getState(), inputC:cdata(), gradOutputC:cdata(), self.gradWeight:cdata(),
                         iH, oH, oH, self.kH,
                            -self.padding, self.dH, self.nInputPlane, 1, 0, scale);
   gradOutputC = gradOutput:view(self.nOutputPlane, oH * oH * nBatch)
   C['gradBias'](lwtorch.getState(), gradOutputC:cdata(), self.gradBias:cdata(), scale);
end
