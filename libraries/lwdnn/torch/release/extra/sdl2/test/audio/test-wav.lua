...
if sdl.buildAudioCVT(cvt, soundfile.format, soundfile.channels, soundfile.freq,
   ...
   error('could not build audio colwerter')
end
...
if sdl.colwertAudio(cvt) ~= 0 then
   error(string.format('problem during audio colwersion: %s', sdl.getError()))
end
...
print(string.format('colwerted audio size: %s bytes', sound.length))
