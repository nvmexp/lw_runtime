...
cmd:option('-colwfast', false, 'use "fast" colwolution code instead of standard')
cmd:option('-colwmm', false, 'use "mm" colwolution code instead of standard')
...
cmd:option('-lwca', false, 'use LWCA with specified precision')
...
if params.colwfast then
   dofile('SpatialColwolutionFast.lua')
   nn.SpatialColwolution = nn.SpatialColwolutionFast
end

if params.colwmm then
   nn.SpatialColwolution = nn.SpatialColwolutionMM
end
...
local function gpuType()
   if params.double then
     return 'torch.LwdaDoubleTensor'
   elseif params.half then
     return 'torch.LwdaHalfTensor'
   else
     return 'torch.LwdaTensor'
   end
end
...
if params.half and not params.lwca then
   error('half not supported without lwca')
end

if params.lwca then
   require 'lwnn'
   --dofile('lwdahacks.lua')
   torch.setdefaulttensortype(gpuType())
   print(  lwtorch.getDeviceProperties(lwtorch.getDevice()) )
else
...
if not params.nocnn then
   ...
   if true then --LeNet5-like 32x32
      ...
      local mlp = nn.Sequential();                 -- make a multi-layer perceptron
      mlp:add(nn.SpatialColwolution(1, 6, 5, 5)) -- output 28x28
      mlp:add(nn.Tanh())
      mlp:add(nn.SpatialSubSampling(6, 2, 2, 2, 2)) --output 14x14
      mlp:add(nn.Tanh())
      mlp:add(nn.SpatialColwolution(6, 16, 5, 5)) -- output 10x10
      mlp:add(nn.Tanh())
   ...
   if true then --LeNet5-like 96x96
      ...
      local mlp = nn.Sequential();                 -- make a multi-layer perceptron
      mlp:add(nn.SpatialColwolution(1, 6, 7, 7)) -- output 90x90
      mlp:add(nn.Tanh())
      mlp:add(nn.SpatialSubSampling(6, 3, 3, 3, 3)) --output 30x30
      mlp:add(nn.Tanh())
      mlp:add(nn.SpatialColwolution(6, 16, 7, 7)) -- output 24x24
      mlp:add(nn.Tanh())
   ...
   if true then --LeNet5-like 256x256
      ...
      local mlp = nn.Sequential();                 -- make a multi-layer perceptron
      mlp:add(nn.SpatialColwolution(1, 6, 7, 7)) -- output 250x250
      mlp:add(nn.Tanh())
      mlp:add(nn.SpatialSubSampling(6, 5, 5, 5, 5)) --output 50x50
      mlp:add(nn.Tanh())
      mlp:add(nn.SpatialColwolution(6, 16, 7, 7)) -- output 44x44
      mlp:add(nn.Tanh())
