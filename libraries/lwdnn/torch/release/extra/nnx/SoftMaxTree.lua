...
function SoftMaxTree:__init(inputSize, hierarchy, rootId, aclwpdate, static, verbose)
   parent.__init(self)
   self.rootId = rootId or 1
   self.inputSize = inputSize
   self.aclwpdate = aclwpdate
...
function SoftMaxTree:updateOutput(inputTable)
   ...
   if self.batchSize ~= input:size(1) then
      ...
      if self._nodeUpdateHost then
         ...
         self._nodeUpdateLwda:resize(input:size(1),self.maxDept)
      end
   end
...
function SoftMaxTree:updateGradInput(inputTable, gradOutput)
   local input, target = unpack(inputTable)
   if not gradOutput:isContiguous() and torch.type(gradOutput) == 'torch.LwdaTensor' then
...
function SoftMaxTree:type(type, typecache)
   ...
   if (type == 'torch.LwdaTensor') then
      -- lwnnx needs this for filling self.updates
      self._nodeUpdateHost = torch.IntTensor()
      self._nodeUpdateLwda = torch.LwdaIntTensor()
      self._paramUpdateHost = torch.IntTensor()
      self._paramUpdateLwda = torch.LwdaTensor()
      self.parentChildrenLwda = self.parentChildren:type(type)
      self.childParentLwda = self.childParent:type(type)
...
function SoftMaxTree:momentumGradParameters()
   ...
   if not self.momGradParams or _.isEmpty(self.momGradParams) then
      assert(not self.aclwpdate, "cannot use momentum with aclwpdate")
...
SoftMaxTree.sharedAclwpdateGradParameters = SoftMaxTree.aclwpdateGradParameters
