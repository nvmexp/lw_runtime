...
function DataSetSamplingPascal:__index__(key)
   ...
   if type(key)=='number' then
      ...
      local full_sample = self.lwrrentSample
      local full_mask = self.lwrrentMask
      ...
      self.lwrrentX = box_x/full_sample:size(3)
      local box_y = math.floor(ctr_y - box_size/2) + 1
      self.lwrrentY = box_y/full_sample:size(2)
      self.lwrrentSegment = self.tags[ctr_target].data[tag_idx+3]
      ...
      if self.labelGenerator then
         ...
         local ret = self:labelGenerator(full_sample, full_mask, sample, mask,
                                         ctr_target, ctr_x, ctr_y, box_x, box_y, box_size,self.lwrrentSegment,self.realIndex)
         return ret, true
...
function DataSetSamplingPascal:loadSample(index)
   ...
   if self.preloadedDone then
      if index ~= self.lwrrentIndex then
         -- load new sample
         self.lwrrentSample = self.preloaded.samples[index]
         self.lwrrentMask = self.preloaded.masks[index]
         -- remember index
         self.lwrrentIndex = index
      end
   elseif index ~= self.lwrrentIndex then

      self.realIndex = self.rawdata[index].imgfile:gsub('.jpg$','')
      -- clean up
      self.lwrrentSample = nil
      self.lwrrentMask = nil
      collectgarbage()
      ...
      if self.rawSampleSize then
         ...
         self.lwrrentSample = torch.Tensor(img_loaded:size(1),h,w)
         image.scale(img_loaded, self.lwrrentSample, 'bilinear')
         self.lwrrentMask = torch.Tensor(h,w)
         image.scale(mask_loaded, self.lwrrentMask, 'simple')
      ...
      elseif self.rawSampleMaxSize and (self.rawSampleMaxSize < img_loaded:size(3)
         ...
         self.lwrrentSample = img_loaded
         self.lwrrentMask = mask_loaded
      end
      -- process mask
      if matlab then
         if self.lwrrentMask:min() == 0 then
            self.lwrrentMask:add(1)
         end
      elseif self.rawMaskRescale then
         -- stanford dataset style (png contains 0 and 255)
         self.lwrrentMask:mul(self.nbClasses-1):add(0.5):floor():add(1)
      else
         -- PNG already stores values at the correct classes
         -- only holds values from 0 to nclasses
         self.lwrrentMask:mul(255):add(1):add(0.5):floor()
      end
      self.lwrrentIndex = index
   end
end
...
function DataSetSamplingPascal:preload(saveFile)
   ...
   for i = 1,self.nbRawSamples do
      ...
      self:loadSample(i)
      local rawTensor = torch.Tensor(self.lwrrentSample:size()):copy(self.lwrrentSample)
      local rawMask = torch.Tensor(self.lwrrentMask:size()):copy(self.lwrrentMask)
...
function DataSetSamplingPascal:parseMask(existing_tags)
   ...
   for ids = 1, nb_segs do
      ...
      self.lwrrentSegment = ids    

      mask.nn.DataSetSegmentSampling_extract(tags, segment1, 
					     x_start, x_end, 
					     y_start, y_end, self.lwrrentIndex, self.lwrrentSegment,
...
function DataSetSamplingPascal:display(...)
   ...
   for i=1,samples do
      self:loadSample(i)
      local dispTensor = self.lwrrentSample:clone()
      local dispMask = self.lwrrentMask:clone()
