...
function nnxtest.SpatialColwolution()
...
local function template_SpatialFovea(fx,fy,bilinear)
   ...
   local module = nn.SpatialFovea{nInputPlane = channels,
                                  ...
                                  preProcessors = {nn.Identity(),
                                                   nn.Identity()},
                                  processors = {nn.SpatialColwolution(channels,4,3,3),
                                                nn.SpatialColwolution(channels,4,3,3)},
                                  bilinear = bilinear,
                                  fov = 3,
                                  sub = 1}
...
function nnxtest.SpatialFovea_folwsed() template_SpatialFovea(4,7) end
function nnxtest.SpatialFovea_unfolwsed() template_SpatialFovea() end
...
local function template_SpatialPyramid(fx,fy)
   ...
   local pyr = nn.SpatialPyramid({1,2},{nn.SpatialColwolution(channels,4,3,3),
				       nn.SpatialColwolution(channels,4,3,3)},
				 3, 3, 1, 1)
...
function nnxtest.SpatialPyramid_folwsed() template_SpatialPyramid(5,3) end
function nnxtest.SpatialPyramid_unfolwsed() template_SpatialPyramid() end
