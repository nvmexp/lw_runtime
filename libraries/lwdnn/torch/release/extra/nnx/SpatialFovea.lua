...
In sampling mode, the fovea is first folwsed on a particular (x,y) point, and no
...
function SpatialFovea:updateOutput(input)
   ...
   -- (6) upscale, only if fovea is not folwsed
   if self.folwsed then
      for idx = 1,nscales do
   ...
   local lwrrentslice = 1
   for idx = 1,nscales do
      lwrrentslice = lwrrentslice + self.processed[idx]:size(1)
   end
   self.output:resize(lwrrentslice-1, self.upsampled[1]:size(2), self.upsampled[1]:size(3))
   lwrrentslice = 1
   for idx = 1,nscales do
      local omap = self.output:narrow(1, lwrrentslice, self.upsampled[idx]:size(1))
      omap:copy( self.upsampled[idx] )
      lwrrentslice = lwrrentslice + self.upsampled[idx]:size(1)
   end
   return self.output
end
...
function SpatialFovea:updateGradInput(input, gradOutput)
   ...
   local lwrrentslice = 1
   for idx = 1,nscales do
      self.gradUpsampled[idx] = gradOutput:narrow(1, lwrrentslice, self.processed[idx]:size(1))
      lwrrentslice = lwrrentslice + self.upsampled[idx]:size(1)
   end
