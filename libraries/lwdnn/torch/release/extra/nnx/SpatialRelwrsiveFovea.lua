local SpatialRelwrsiveFovea, parent = torch.class('nn.SpatialRelwrsiveFovea', 'nn.Module')
...
The result of each module/scale is upsampled to feed the next stage of relwrsion.
...
function SpatialRelwrsiveFovea:__init(...)
   parent.__init(self)
   ...
   xlua.unpack_class(
      self,
      {...},
      'nn.SpatialRelwrsiveFovea',
      help_desc,
      {arg='nInputPlane',       type='number',  help='number of input planes', req=true},
      {arg='nRelwrsivePlane',   type='number',  help='number of relwrsive planes (e.g. nb of output planes used by next input stage', req=true},
...
function SpatialRelwrsiveFovea:configure(fov, sub, input_w, input_h)
   ...
   for idx = 1,nscales do
      if ratios[idx] < 1 then
         xerror('ratios should be >= 1','nn.SpatialRelwrsiveFovea')
   ...
   for idx = nscales,1,-1 do
      ...
      if idx > 1 and self.ratios[idx] > self.ratios[idx-1] then
         xerror('downsampling ratios should be provided in decreasing order, for proper coarse-to-fine relwrsion',
                'nn.SpatialRelwrsiveFovea')
      end
      -- pyramid size
      ...
      if idx == nscales then
         ...
         -- infer processed size from next stage in relwrsion
   ...
   if self.debug then
      print('')
      xprint('reconfig complete:','nn.SpatialRelwrsiveFovea')
      for idx = 1,nscales do
         print('scale ' .. idx .. ' :')
...
function SpatialRelwrsiveFovea:__tostring__()
   local str = 'nn.SpatialRelwrsiveFovea:\n'
   str = str .. '  + number of relwrsion stages : '..(#self.ratios) .. '\n'
...
function SpatialRelwrsiveFovea:focus(x,y)
...
function SpatialRelwrsiveFovea:updateOutput(input,target,x,y)
   -- input must be 3D
   if input:nDimension() ~= 3 or input:size(1) ~= self.nInputPlane then
      xerror('input must be 3d and have ' .. self.nInputPlane .. ' input planes','nn.SpatialRelwrsiveFovea')
   end
   ...
   for idx = 1,nscales do
      ...
      -- (5) concatenate current input and upsampled result from previous stage in the relwrsion
      self.concatenated[idx] = self.concatenated[idx] or torch.Tensor()
      self.concatenated[idx]:resize(self.narrowed[idx]:size(1) + self.nRelwrsivePlane,
      ...
      if idx > 1 then
         local p = self.concatenated[idx]:narrow(1,self.narrowed[idx]:size(1)+1, self.nRelwrsivePlane)
         ...
         self.concatenated[idx]:narrow(1,self.narrowed[idx]:size(1)+1,self.nRelwrsivePlane):zero()
      end
   ...
   if self.criterions[1] and target then
      ...
      if self.debug then
         print('')
         xprint('adjusted focus points','nn.SpatialRelwrsiveFovea')
         for idx = 1,nscales do
            if self.folwsed then
               print('  + at scale ' .. idx .. ', folwsed on corner: ' .. corners[idx].x .. ',' .. corners[idx].y ..
   ...
   -- (10) return output (last stage in the relwrsion)
   self.output = self.postProcessed[nscales]
   return self.output, error
end

function SpatialRelwrsiveFovea:updateGradInput(input)
   -- local params
   local nscales = #self.ratios
   local fov = self.fov
   local sub = self.sub
   ...
   -- (7) relwrsive gradient: not done for now (needs to see if it's really worth it)
   --
...
function SpatialRelwrsiveFovea:reset(stdv)
   for idx = 1,#self.processors do
      self.processors[idx]:reset(stdv)
   end
end

function SpatialRelwrsiveFovea:zeroGradParameters(momentum)
   for idx = 1,#self.processors do
      self.processors[idx]:zeroGradParameters(momentum)
   end
end

function SpatialRelwrsiveFovea:updateParameters(learningRate)
   for idx = 1,#self.processors do
      self.processors[idx]:updateParameters(learningRate)
   end
end

function SpatialRelwrsiveFovea:decayParameters(decay)
   for idx = 1,#self.processors do
