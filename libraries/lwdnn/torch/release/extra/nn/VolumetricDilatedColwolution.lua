...
local VolumetricDilatedColwolution, parent = torch.class('nn.VolumetricDilatedColwolution', 'nn.VolumetricColwolution')

function VolumetricDilatedColwolution:__init(nInputPlane, nOutputPlane, kT, kW, kH, dT, dW, dH, padT, padW, padH, dilationT, dilationW, dilationH)
...
function VolumetricDilatedColwolution:updateOutput(input)
   self.finput = self.finput or self.weight.new()
   self.fgradInput = self.fgradInput or self.weight.new()
   input.THNN.VolumetricDilatedColwolution_updateOutput(
      input:cdata(),
      self.output:cdata(),
      self.weight:cdata(),
...
function VolumetricDilatedColwolution:updateGradInput(input, gradOutput)
   if self.gradInput then
      self.fgradInput = self.fgradInput or self.weight.new()
      input.THNN.VolumetricDilatedColwolution_updateGradInput(
         input:cdata(),
         gradOutput:cdata(),
...
function VolumetricDilatedColwolution:accGradParameters(input, gradOutput, scale)
   scale = scale or 1
   self.fgradInput = self.fgradInput or self.weight.new()
   input.THNN.VolumetricDilatedColwolution_accGradParameters(
      input:cdata(),
      gradOutput:cdata(),
...
function VolumetricDilatedColwolution:__tostring__()
