...
function IndexLinear:__init(inputSize, outputSize, doGradInput, keysOffset, weight, bias, normalize)
   ...
   -- if we normalize:
   -- * The max-abs value
   -- * The ilwerse of the max-abs value
   ...
   -- gradWeight is not initialized as we're doing dense gradient aclwmulation
   ...
   -- when doing gradients aclwmulations
   self.running = {
      lwmSumSizes = {},
      keys = {},
      gradWeight = {},
   ...
   -- self.sizes, self.lwmSumSizes are callwlated on the CPU even when using LWCA.
   ...
   -- If LWCA is being used, self.*Cache[2] contains values on GPU.
   self.sizesCache = {}
   self.lwmSumSizesCache = {}
...
function IndexLinear:longTensor(...)
   if (self:type() == 'torch.LwdaTensor') then
      return torch.LwdaLongTensor(...)
   else
...
function IndexLinear:flattenInputs(input)
   ...
   local isLwda = (self:type() == 'torch.LwdaTensor')
   ...
   if self.isFlat then
      ...
      self.sizes = sizes
      self.lwmSumSizes = self.lwmSumSizes or self.sizes.new()
      self.lwmSumSizes:lwmsum(self.sizes)
   else
      ...
      self.lwmSumSizesCache[1] = self.lwmSumSizesCache[1] or torch.LongTensor(batchSize)

      self.sizes = self.sizesCache[1]
      self.lwmSumSizes = self.lwmSumSizesCache[1]

      self.sizes:resize(batchSize)
      self.lwmSumSizes:resize(batchSize)
      ...
      if isLwda then
         -- Get the GPU cache
         self.sizesCache[2] = self.sizesCache[2] or torch.LwdaLongTensor()
         self.lwmSumSizesCache[2] = self.lwmSumSizesCache[2] or torch.LwdaLongTensor()

         self.sizes = self.sizesCache[2]
         self.lwmSumSizes = self.lwmSumSizesCache[2]

         -- Resize and copy to GPU
         self.sizes:resize(batchSize):copy(self.sizesCache[1])
         self.lwmSumSizes:resize(batchSize):copy(self.lwmSumSizesCache[1])
      end
   end
   self.running.lwmSumSizes[counter] = self.lwmSumSizes
end
...
function IndexLinear:aclwpdateGradParameters(input, gradOutput, scale)
   self.values.THNN.IndexLinear_aclwpdateGradParameters(
      self.keys:cdata(),
      self.offset,
...
function IndexLinear:zeroGradParameters()
   ...
   -- piece of code in lib/THNN/generic/IndexLinear.c, in the aclwpdateGradParameters function.
