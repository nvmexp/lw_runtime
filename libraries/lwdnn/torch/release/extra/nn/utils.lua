...
-- colwerted (relwrsively) by calls to relwrsiveType() and type().
-- It caches colwersions in order to preserve sharing semantics
-- i.e. if two tensors share a common storage, then type colwersion
...
--
-- > tensorCache = {}
-- > net1:type('torch.LwdaTensor', tensorCache)
-- > net2:type('torch.LwdaTensor', tensorCache)
-- > nn.utils.relwrsiveType(anotherTensor, 'torch.LwdaTensor', tensorCache)
--
...
function nn.utils.relwrsiveType(param, type, tensorCache)
   tensorCache = tensorCache or {}

   if torch.type(param) == 'table' then
      for k, v in pairs(param) do
         param[k] = nn.utils.relwrsiveType(v, type, tensorCache)
      end
...
function nn.utils.relwrsiveResizeAs(t1,t2)
   if torch.type(t2) == 'table' then
      t1 = (torch.type(t1) == 'table') and t1 or {t1}
      for key,_ in pairs(t2) do
         t1[key], t2[key] = nn.utils.relwrsiveResizeAs(t1[key], t2[key])
      end
...
function nn.utils.relwrsiveFill(t2, val)
   if torch.type(t2) == 'table' then
      for key,_ in pairs(t2) do
         t2[key] = nn.utils.relwrsiveFill(t2[key], val)
      end
   elseif torch.isTensor(t2) then
      t2:fill(val)
   else
...
function nn.utils.relwrsiveAdd(t1, val, t2)
   if not t2 then
   ...
   if torch.type(t2) == 'table' then
      t1 = (torch.type(t1) == 'table') and t1 or {t1}
      for key,_ in pairs(t2) do
         t1[key], t2[key] = nn.utils.relwrsiveAdd(t1[key], val, t2[key])
      end
...
function nn.utils.relwrsiveCopy(t1,t2,async)
   if torch.type(t2) == 'table' then
      t1 = (torch.type(t1) == 'table') and t1 or {t1}
      for key,_ in pairs(t2) do
         t1[key], t2[key] = nn.utils.relwrsiveCopy(t1[key], t2[key], async)
      end
   elseif torch.isTensor(t2) then
      t1 = torch.isTensor(t1) and t1 or t2.new()
      t1:resize(t2:size())
