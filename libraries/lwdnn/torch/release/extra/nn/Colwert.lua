------------------------------------------------------------------------
--[ nn.Colwert ]--
-- Module to colwert between different data formats
-- nn.Colwert('bchw', 'bf') or nn.Colwert('chw', 'f')
-- Automatically colwerts input to same type as self.output
-- Simplest use is for automatic input type colwerions : nn.Colwert()
------------------------------------------------------------------------
local _ = require 'moses'
local Colwert, parent = torch.class("nn.Colwert", "nn.Container")

function Colwert:__init(inputShape, outputShape)
...
-- post-initialization
function Colwert:buildColwerter(input)
   if self.transposition then
      self.colwerter = self:transpose(self.outputShape)
   else
      if (torch.type(self[self.outputShape]) ~= 'function') then
         error(string.format("Unrecognized colwersion of shape %s to %s", self.inputShape, self.outputShape))
      end
      self.colwerter = self[self.outputShape](self, input)
   end
   assert(torch.isTensor(self.output), "Expecting Tensor output")

   self.colwerter:type(torch.type(self.output))

   self.modules[1] = self.colwerter
end

function Colwert:updateOutput(input)
   ...
   if not self.colwerter then
      self:buildColwerter(input)
   end

   self.output = self.colwerter:updateOutput(input)
...
function Colwert:updateGradInput(input, gradOutput)
   local input_ = input
   input = self._input or input
   if not self.batchMode then
      input = self.__input
      self.__gradOutput = self.__gradOutput or gradOutput.new()
      self.__gradOutput:set(gradOutput):resize(self.colwerter.output:size())
      gradOutput = self.__gradOutput
   end

   local gradInput = self.colwerter:updateGradInput(input, gradOutput)
...
function Colwert:accGradParameters(input, gradOutput, scale)
   ...
   self.colwerter:accGradParameters(input, gradOutput, scale)
end

function Colwert:aclwpdateGradParameters(input, gradOutput, lr)
   input = self.batchMode and self.__input or self._input or input
   gradOutput = self.batchMode and self.__gradOutput or gradOutput
   self.colwerter:aclwpdateGradParameters(input, gradOutput, lr)
end

-- batch feature
function Colwert:bf(input)
   local b_pos = self:findAxis('b', self.inputShape)
   local dim = #self.inputShape
   if self.inputShape == 'bt' then
      error"Colwersion of shape bt to bf not supported: open an issue on github"
   end
   -- was b
   if dim == 1 then
      return nn.Reshape(1)
...
function Colwert:b(input)
   ...
   elseif self.inputShape == 'bf' or self.inputShape == 'fb' then
      ...
      if input:size(f_pos) > 1 then
         error("Cannot colwert shape "..self.inputShape.." to b when feature > 1")
      end
      return nn.Select(f_pos, 1)
   else
      error("Cannot colwert shape "..self.inputShape.." to shape b")
   end
end

-- returns the current shape of the data
function Colwert:default()
   return nn.Identity()
end
...
function Colwert:bt()
   ...
   if self.inputShape == 'b' then
      ...
      error("cannot colwert shape '"..self.inputShape.."' to bt")
   end
   return modula
end

-- a generic function for transposing shape axes
function Colwert:transpose(newShape)
...
function Colwert:findAxis(axis_char, shape, silent)
