...
function MaskedSelect:updateGradInput(input, gradOutput)
  local input, mask = unpack(input)
  if input:type() == 'torch.LwdaTensor' then
...
function MaskedSelect:type(type, tensorCache)
  ...
  -- These casts apply when switching between lwca/non-lwca types
  ...
  elseif  type == 'torch.LwdaTensor' then
    self._maskIndexBuffer = self._maskIndexBuffer:lwca()
    self._maskIndices = self._maskIndices:lwca()
    self._gradMask = self._gradMask:lwca()
  end
  self._type = type
  return self
end
