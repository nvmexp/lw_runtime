...
local TRACEBACK_WARNING = "WARNING: If you see a stack trace below, it doesn't point to the place where this error oclwrred. Please use only the one above."
...
function Container:rethrowErrors(module, moduleIndex, funcName, ...)
   ...
   local function handleError(err)
      -- This will be exelwted only in the first container that handles the error.
   ...
   -- be passed via a closure. This inlwrs some overhead, so it's better not to
   -- make it the default.
   local ok, ret, noret
