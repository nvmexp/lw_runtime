...
function MultiCriterion:updateGradInput(input, target)
   self.gradInput = nn.utils.relwrsiveResizeAs(self.gradInput, input)
   nn.utils.relwrsiveFill(self.gradInput, 0)
   for i=1,#self.criterions do
      nn.utils.relwrsiveAdd(self.gradInput, self.weights[i], self.criterions[i]:updateGradInput(input, target))
   end
