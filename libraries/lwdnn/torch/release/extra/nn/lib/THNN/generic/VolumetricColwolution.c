...
#define TH_GENERIC_FILE "generic/VolumetricColwolution.c"
#else
...
void THNN_(VolumetricColwolution_updateOutput)(
          ...
          THTensor *weight,
          THTensor *bias,
          THTensor *finput,     // only used by lwca impl
          ...
          int pH)
{
  THArgCheck(pT != 0 || pW != 0 || pH != 0, 9, "padding not supported by CPU backend");   // sharing signature with LWCA version
  ...
  if (input->nDimension == 4) /* non-batch mode */
  {
    ...
    /* do colwolutions */
    THTensor_(colw3Dmv)(output, 1.0, 1.0, input, weight, dT, dH, dW, "V", "X");
  }
  else /* batch mode */
  {
    ...
    for (j = 0; j < nBatch; j++)
    {
      ...
      /* do colwolutions */
      THTensor_(colw3Dmv)(outb, 1.0, 1.0, inb, weight, dT, dH, dW, "V", "X");
    }
          ...
          int pH)
{
  ...
  {
    THTensor_(colw3Dmv)(gradInput, 0.0, 1.0, gradOutput, tweight, dT, dH, dW, "F", "C");
  }
  else /* batch mode */
  {
    ...
    for (j = 0; j < nBatch; j++)
    {
      ...
      THTensor_(colw3Dmv)(ginpb, 0.0, 1.0, goutb, tweight, dT, dH, dW, "F", "C");
    }
          ...
          accreal scale_)
{
  ...
  if (gradOutput->nDimension == 4) /* non-batch mode */
  {
    ...
    /* gradient to kernels */
    THTensor_(colw3DRevger)(gradWeight, 1.0, scale, input, gradOutput, dT, dH, dW);
  }
  else /* batch mode */
  {
    long nBatch = gradOutput->size[0];
    ...
    for (j = 0; j < nBatch; j++)
    {
      ...
      /* gradient to kernels */
      THTensor_(colw3DRevger)(gradWeight, 1.0, scale, inpb, goutb, dT, dH, dW);
    }
    THTensor_(free)(inpb);
    THTensor_(free)(goutb);
  }
