          ...
          THTensor *total_weight)
{
  ...
  for (int b = 0; b < batch_size; b++) {
    for (int elem = 0; elem < map_size; elem++) {
      int lwr_target = target_data[b * map_size + elem] - TH_INDEX_BASE;
      THAssert(lwr_target >= 0 && lwr_target < n_classes);

      real lwr_weight = weights ? weights_data[lwr_target] : 1.0f;
      total_weight_acc += lwr_weight;
      output_acc -= input_data[b * sample_size + lwr_target * map_size + elem] * lwr_weight;
    }
  }
          ...
          THTensor *total_weight)
{
  ...
  for (b = 0; b < batch_size; b++) {
    ...
    for (elem = 0; elem < map_size; elem++) {
      ...
      gradInput_data[b * sample_size + lwr_target * map_size + elem] =
        -(weights ? weights_data[lwr_target] : 1.0f) / normalize;
    }
  }
