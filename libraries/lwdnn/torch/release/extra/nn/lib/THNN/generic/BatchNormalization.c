  ...
  for (f = 0; f < nInput; ++f) {
    ...
    if (train) {
      ...
      if (sum == 0 && eps == 0.0) {
        ...
        ilwstd = (real) (1 / sqrt(sum/n + eps));
      }
      THTensor_(set1d)(save_std, f, (real) ilwstd);
      ...
      ilwstd = 1 / sqrt(THTensor_(get1d)(running_var, f) + eps);
    }
    ...
    TH_TENSOR_APPLY2(real, in, real, out,
      *out_data = (real) (((*in_data - mean) * ilwstd) * w + b););
  ...
  bool train, double scale, double eps)
{
  ...
  for (f = 0; f < nInput; ++f) {
    ...
    if (train) {
      ...
      ilwstd = THTensor_(get1d)(save_std, f);
    } else {
      mean = THTensor_(get1d)(running_mean, f);
      ilwstd = 1 / sqrt(THTensor_(get1d)(running_var, f) + eps);
    }
    ...
    if (gradInput) {
      ...
      if (train) {
        ...
        real k = (real) dotp * ilwstd * ilwstd / n;
        ...
        TH_TENSOR_APPLY2(real, gradIn, real, gradOut,
          *gradIn_data = (*gradOut_data - gradMean - *gradIn_data) * ilwstd * w;);
        ...
        // dL/dX = w / running_std
        TH_TENSOR_APPLY2(real, gradIn, real, gradOut,
          *gradIn_data = *gradOut_data * ilwstd * w;);
      }
    ...
    if (gradWeight) {
      ...
      THTensor_(set1d)(gradWeight, f, val + scale * dotp * ilwstd);
    }
