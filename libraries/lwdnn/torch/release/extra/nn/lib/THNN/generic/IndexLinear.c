          ...
          int  train)
{
  ...
  long* lwmSumSizesData = THLongTensor_data(lwmSumSizes);
...
#pragma omp parallel                                                    \
    ...
    if(keysSize*outDim > THNN_SPARSE_OMP_THRESHOLD && batchSize > 1)
      for (j = 0; j < batchSize; j++)
      {
        ...
        real val = 0;
        real absVal = 0;
        long offset = j == 0 ? 0 : lwmSumSizesData[j - 1];
...
{
  real weightDecay = TH_COLWERT_ACCREAL_TO_REAL(weightDecay_);
  real learningRate = TH_COLWERT_ACCREAL_TO_REAL(learningRate_);
  /* Retrieve all the dimensions of the problem */
  long outDim = THTensor_(size)(bias, 0);
  long woutDim = THTensor_(size)(weight, 1);
  int maxNormalize = woutDim - outDim;
          ...
          accreal scale_)
{
  ...
  THLongTensor* lwmSizes = THLongTensor_new();
  THLongTensor_lwmsum(lwmSizes, sizes, 0);
  long* lwmSizesData = THLongTensor_data(lwmSizes);
  ...
  if (outDim == 1)
  {
    for (j = 0; j < batchSize; j++)
    {
      long offset = j==0?0:lwmSizesData[j-1];
      real val = gradOutputData[j] * scale;
