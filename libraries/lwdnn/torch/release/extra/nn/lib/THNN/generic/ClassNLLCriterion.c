          ...
          long ignore_index)
{
  ...
  if (THTensor_(nDimension)(input) == 1) {
    int lwr_target = target_data[0] - TH_INDEX_BASE;
    if (lwr_target != ignore_index) {
      THAssert(lwr_target >= 0 && lwr_target < n_classes);
      total_weight_data[0] = weights ? weights_data[lwr_target] : 1.0f;
      output_data[0] = -input_data[lwr_target] * total_weight_data[0];
    }
    ...
    for (i = 0; i < batch_size; i++) {
      int lwr_target = target_data[i] - TH_INDEX_BASE;
      if (lwr_target != ignore_index) {
        THAssert(lwr_target >= 0 && lwr_target < n_classes);

        real lwr_weight = weights ? weights_data[lwr_target] : 1.0f;
        total_weight_data[0] += lwr_weight;
        output_data[0] -= input_data[i * n_target + lwr_target] * lwr_weight;
      }
    }
  }
          ...
          long ignore_index)
{
  ...
  if (THTensor_(nDimension)(input) == 1) {
    ...
    if (lwr_target != ignore_index) {
      ...
      gradInput_data[lwr_target] =
        (!sizeAverage && weights) ? -weights_data[lwr_target] : -1;
    }
    ...
    for (i = 0; i < batch_size; i++){
      ...
      if (lwr_target != ignore_index) {
        ...
        gradInput_data[i * n_target + lwr_target] =
          -(weights ? weights_data[lwr_target] : 1.0f);

        if (sizeAverage && *total_weight_data) {
          gradInput_data[i * n_target + lwr_target] /= *total_weight_data;
        }
      }
    }
  }
