...
void THNN_(LookupTable_accGradParameters)(
          ...
          accreal ascale)
{
  real scale = TH_COLWERT_ACCREAL_TO_REAL(ascale);
  ptrdiff_t i;
  ...
  if (numel > 1000)
  {
    ...
    // thread 1 handles updates to gradWeight[0..lwocab/nThreads]. Every thread
...
void THNN_(LookupTable_renorm)(
          ...
          accreal normType_)
{
  real maxNorm = TH_COLWERT_ACCREAL_TO_REAL(maxNorm_);
  real normType = TH_COLWERT_ACCREAL_TO_REAL(normType_);
