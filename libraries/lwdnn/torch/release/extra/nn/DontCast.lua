...
local function relwrsiveTypeCopy(dst, src, type_str)
   if torch.type(src) == 'table' then
      dst = (torch.type(dst) == 'table') and dst or {}
      for k, v in pairs(src) do
         dst[k] = relwrsiveTypeCopy(dst[k], v, type_str)
      end
...
function DontCast:updateOutput(input)
   if self.castin and tableTensorType(input) ~= self.moduleType then
      self._input = relwrsiveTypeCopy(self._input, input, self.moduleType)
      input = self._input
   end
   ...
   if self.castout then
      self.output = relwrsiveTypeCopy(self.output, output, tableTensorType(self.output))
   else
      self.output = output
   end
   return self.output
...
function DontCast:updateGradInput(input, gradOutput)
   ...
   if self.castout and tableTensorType(gradOutput) ~= self.moduleType then
      self._gradOutput = relwrsiveTypeCopy(self._gradOutput, gradOutput, self.moduleType)
   ...
   if self.castin then
      self.gradInput = relwrsiveTypeCopy(self.gradInput, gradInput, tableTensorType(self.gradInput))
   else
...
function DontCast:aclwpdateGradParameters(input, gradOutput, lr)
   if self.castin and tableTensorType(input) ~= self.moduleType then
      input = self._input
   end
   if self.castout and tableTensorType(gradOutput) ~= self.moduleType then
...
function DontCast:type(type)
   if self.castout and tableTensorType(self.output) ~= type then
      self.output = relwrsiveTypeCopy(nil, self.output, type)
   end
   if self.castin and tableTensorType(self.gradInput) ~= type then
      self.gradInput = relwrsiveTypeCopy(nil, self.gradInput, type)
   end
   return self
end
