...
-- Relwrsive function to flatten a table (output is a table)
...
-- Relwrsive function to check if we need to rebuild the output table
...
-- input.  This is a relwrsive function to build up a gradInput
local function ilwerseFlatten(gradOutput, input_map)
  if torch.type(input_map) == 'table' then
    local gradInput = {}
    for i = 1, #input_map do
      gradInput[#gradInput + 1] = ilwerseFlatten(gradOutput, input_map[i])
    end
    return gradInput
  else
...
function FlattenTable:updateGradInput(input, gradOutput)
  ...
  if not checkMapping(gradOutput, self.gradInput, self.input_map) then
    self.gradInput = ilwerseFlatten(gradOutput, self.input_map)
  end
...
function FlattenTable:type(type, tensorCache)
  ...
  -- colwersions.  Just force the tables to be empty.
  self:clearState()
end
