...
function OneHot:updateOutput(input)
   ...
   if type(input) == 'number' then
      if self:type() == 'torch.LwdaTensor' then
         self._single = self._single or torch.LwdaTensor():resize(1);
      else
   ...
   if torch.type(input) == 'torch.LwdaTensor' or torch.type(input) == 'torch.ClTensor' then
      self.output:scatter(self.output:dim(), input_, 1)
   else
      if torch.type(self.output) == 'torch.LwdaTensor' then
         -- input is not lwca, module is, cast input to lwca
         self._input = self._input or torch.LwdaTensor()
         self._input:resize(input_:size()):copy(input_)
         input_ = self._input
      elseif torch.type(input) ~= 'torch.LongTensor' then
         -- input is not long, module isnot lwca, cast input to long
