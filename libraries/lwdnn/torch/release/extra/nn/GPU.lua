...
-- The operations are also exelwted on that device.
-- Arguments input and gradOutput are colwerted to the specified device 
...
-- The unit test is located in lwnn.
------------------------------------------------------------------------
...
function GPU:__init(module, device, outdevice)
   ...
   if module:type():find('torch%.Lwca.*Tensor') then
      self:type(module:type())
   end
end

function GPU.relwrsiveModuleDevice(obj, device)
   if type(obj) == 'table' and not torch.isTypeOf(obj, 'nn.GPU') and not obj.__noGPU__ then
      for k,v in pairs(obj) do
         obj[k] = GPU.relwrsiveModuleDevice(v, device)
      end
   elseif torch.type(obj):match('torch.Lwca.*Tensor') then
...
function GPU:setDevice(device)
   ...
   self.modules[1] = lwtorch.withDevice(self.device, function() 
      return self.relwrsiveModuleDevice(self.modules[1], self.device) 
   end)
   return self
end
...
function GPU.relwrsiveSetDevice(dst, src, proto)
   ...
   if torch.type(src) == 'table' then
      ...
      for k,v in ipairs(src) do
         dst[k] = GPU.relwrsiveSetDevice(dst[k], v, prototable and proto[k] or device)
      end
      for k=#src+1,#dst do
         dst[k] = nil
      end
   elseif torch.type(src):match('torch.Lwca.*Tensor') and src:getDevice() ~= device and src:getDevice() ~= 0 then
      if not (torch.type(dst):match('torch.Lwca.*Tensor') and dst:getDevice() == device) then
         dst = src.new()
      end
      lwtorch.withDevice(device, function() dst:resizeAs(src):copy(src) end)
   else
      dst = src
   end
   return dst 
...
function GPU:updateOutput(input)
   if self._type:find('torch%.Lwca.*Tensor') then
      self._input = self.relwrsiveSetDevice(self._input, input, self.device)
      
      local output = lwtorch.withDevice(self.device, function()
         return self.modules[1]:updateOutput(self._input)
      end)
      
      if self.device ~= self.outdevice then
         self.output = self.relwrsiveSetDevice(self.output, output, self.outdevice)
      else
         self.output = output
      end
   else
...
function GPU:updateGradInput(input, gradOutput)
   if self._type:find('torch%.Lwca.*Tensor') then
      self._gradOutput = self.relwrsiveSetDevice(self._gradOutput, gradOutput, self.device)
      
      local gradInput = lwtorch.withDevice(self.device, function()
         return self.modules[1]:updateGradInput(self._input, self._gradOutput)
      end)
      
      self.gradInput = self.relwrsiveSetDevice(self.gradInput, gradInput, input)
   else
...
function GPU:accGradParameters(input, gradOutput, scale) 
   if self._type:find('torch%.Lwca.*Tensor') then
      lwtorch.withDevice(self.device, function()
...
function GPU:apply(callback)
   if self._type:find('torch%.Lwca.*Tensor') then
      lwtorch.withDevice(self.device, function() parent.apply(self, callback) end)
   else
...
function GPU:type(type, typecache)
   if type and type:find('torch%.Lwca.*Tensor') then
      lwtorch.withDevice(self.device, function() parent.type(self, type, typecache) end)
      self:setDevice()
   else
      self.output = nil
      self.gradInput = nil
...
function GPU:clearState()
   ...
   self._gradOutput = nil
   if self._type:find('torch%.Lwca.*Tensor') then
      lwtorch.withDevice(self.device, function() parent.clearState(self) end)
   else
...
function GPU:zeroGradParameters()
   if self._type:find('torch%.Lwca.*Tensor') then
      lwtorch.withDevice(self.device, function() parent.zeroGradParameters(self) end)
   else
...
function GPU:updateParameters(lr)
   if self._type:find('torch%.Lwca.*Tensor') then
      lwtorch.withDevice(self.device, function() parent.updateParameters(self, lr) end)
   else
...
function GPU:training()
   if self._type:find('torch%.Lwca.*Tensor') then
      lwtorch.withDevice(self.device, function() parent.training(self) end)
   else
...
function GPU:evaluate()
   if self._type:find('torch%.Lwca.*Tensor') then
      lwtorch.withDevice(self.device, function() parent.evaluate(self) end)
   else
...
function GPU:share(mlp, ...)
   local args = {...}
   if self._type:find('torch%.Lwca.*Tensor') then
      lwtorch.withDevice(self.device, function() parent.share(self, mlp, unpack(args)) end)
   else
...
function GPU:reset(...)
   local args = {...}
   if self._type:find('torch%.Lwca.*Tensor') then
      lwtorch.withDevice(self.device, function() parent.reset(self, unpack(args)) end)
   else
...
function GPU:clone(...)
   local args = {...}
   if self._type:find('torch%.Lwca.*Tensor') then
      return lwtorch.withDevice(self.device, function() parent.clone(self, unpack(args)) end)
   else
...
function GPU:read(file)
   ...
   local object
   if header[1] and header[1]:find('torch%.Lwca.*Tensor') then
      local device = header[2] 
      if device > lwtorch.getDeviceCount() then
      ...
      object = lwtorch.withDevice(device, function() return file:readObject() end)
   else
...
function GPU:aclwpdateGradParameters(input, gradOutput, lr)
   error("Not Implemented for "..torch.type(self))
end

function GPU:sharedAclwpdateGradParameters(input, gradOutput, lr)
   error("Not Implemented for "..torch.type(self))
end
