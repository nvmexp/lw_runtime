...
function nn.Jacobian.backwardUpdate(module, input, param)
   ...
   for i=1,sdout:nElement() do
      ...
      module:aclwpdateGradParameters(input, dout, 1)
...
function nn.Jacobian.backwardDiagHessian(module, input, diagHessianParamName)
   ...
   --
   -- The function F is chosen for its colwenient properties:
   --
   -- dF / dy_i = y_i,
   -- d^2F / dy_i^2 = 1.
   --
...
function nn.Jacobian.linearModuleDiagHessian(module, input, gradParamName)
   ...
   -- (like nn.Linear, nn.SpatialColwolution, etc.)
   --
   -- This function computes the diagonal Hessian elements of the following function:
   --
   -- F(x_1, x_2, ..., x_n) = y_1^2/2 + y_2^2/2 + ... + y_m^2/2.
...
function nn.Jacobian.testJacobian(module, input, milwal, maxval, perturbation)
   milwal = milwal or -2
   maxval = maxval or 2
   local inrange = maxval - milwal
   input:copy(torch.rand(input:nElement()):mul(inrange):add(milwal))
...
function nn.Jacobian.testJacobianParameters(module, input, param, dparam, milwal, maxval, perturbation)
   milwal = milwal or -2
   maxval = maxval or 2
   local inrange = maxval - milwal
   input:copy(torch.rand(input:nElement()):mul(inrange):add(milwal))
   param:copy(torch.rand(param:nElement()):mul(inrange):add(milwal))
...
function nn.Jacobian.testJacobianUpdateParameters(module, input, param, milwal, maxval, perturbation)
   milwal = milwal or -2
   maxval = maxval or 2
   local inrange = maxval - milwal
   input:copy(torch.rand(input:nElement()):mul(inrange):add(milwal))
...
function nn.Jacobian.testDiagHessian(module, input, gradParamName, diagHessianParamName, milwal, maxval)
...
function nn.Jacobian.testDiagHessianInput(module, input, milwal, maxval)
   return nn.Jacobian.testDiagHessian(module, input, 'gradInput', 'diagHessianInput', milwal, maxval)
end

function nn.Jacobian.testDiagHessianWeight(module, input, milwal, maxval)
   return nn.Jacobian.testDiagHessian(module, input, 'gradWeight', 'diagHessianWeight', milwal, maxval)
end

function nn.Jacobian.testDiagHessianBias(module, input, milwal, maxval)
   return nn.Jacobian.testDiagHessian(module, input, 'gradBias', 'diagHessianBias', milwal, maxval)
end

function nn.Jacobian.testIO(module,input, milwal, maxval)
   milwal = milwal or -2
   maxval = maxval or 2
   local inrange = maxval - milwal
   ...
   -- run module
   module:forward(input)
   local go = module.output:clone():copy(torch.rand(module.output:nElement()):mul(inrange):add(milwal))
...
function nn.Jacobian.testAllUpdate(module, input, weight, gradWeight)
   ...
   -- aclwpdateGradParameters
   local maclwgp = module:clone()
   maclwgp:forward(input)
   maclwgp:updateGradInput(input, gradOutput)
   maclwgp:aclwpdateGradParameters(input, gradOutput, lr)
   errors["aclwpdateGradParameters"] = (maclwgp[weight]-maccgp[weight]):norm()
   ...
   -- shared, aclwpdateGradParameters
   ...
   macshu1:aclwpdateGradParameters(input, gradOutput, lr)
   macshu2:aclwpdateGradParameters(input, gradOutput, lr)
   err = (weightc-maccgp[gradWeight]*(lr*2)-macshu1[weight]):norm()
   err = err + (weightc-maccgp[gradWeight]*(lr*2)-macshu2[weight]):norm()
   errors["aclwpdateGradParameters [shared]"] = err
