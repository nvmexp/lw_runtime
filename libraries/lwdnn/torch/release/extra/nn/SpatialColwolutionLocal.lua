local SpatialColwolutionLocal, parent = torch.class('nn.SpatialColwolutionLocal', 'nn.Module')

function SpatialColwolutionLocal:__init(nInputPlane, nOutputPlane, iW, iH ,kW, kH, dW, dH, padW, padH)
   parent.__init(self)
...
local function ulwiewWeight(self)
   self.weight = self.weight:view(self.oH, self.oW, self.nOutputPlane, self.nInputPlane, self.kH, self.kW)
   if self.gradWeight and self.gradWeight:dim() > 0 then
      self.gradWeight = self.gradWeight:view(self.oH, self.oW, self.nOutputPlane, self.nInputPlane, self.kH, self.kW)
   end
...
function SpatialColwolutionLocal:updateOutput(input)
   ...
   viewWeight(self)
   input.THNN.SpatialColwolutionLocal_updateOutput(
      input:cdata(),
      self.output:cdata(),
      self.weight:cdata(),
      self.bias:cdata(),
...
function SpatialColwolutionLocal:updateGradInput(input, gradOutput)
   ...
   if self.gradInput then
      viewWeight(self)
      input.THNN.SpatialColwolutionLocal_updateGradInput(
         input:cdata(),
         gradOutput:cdata(),
...
function SpatialColwolutionLocal:accGradParameters(input, gradOutput, scale)
   ...
   checkOutputSize(self, input, gradOutput)
   viewWeight(self)
   input.THNN.SpatialColwolutionLocal_accGradParameters(
      input:cdata(),
      gradOutput:cdata(),
...
function SpatialColwolutionLocal:type(type,tensorCache)
   self.finput = self.finput and torch.Tensor()
   self.fgradInput = self.fgradInput and torch.Tensor()
   return parent.type(self,type,tensorCache)
end

function SpatialColwolutionLocal:__tostring__()
...
function SpatialColwolutionLocal:clearState()
