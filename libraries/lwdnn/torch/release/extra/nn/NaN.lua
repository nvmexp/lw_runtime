...
function NaN:relwrsiveIsNaN(tensor)
   local isNaN = false
   if torch.type(tensor) == 'table' then
      for k,v in pairs(tensor) do
         isNaN = self:relwrsiveIsNaN(v)
...
function NaN:updateOutput(input)
   ...
   if self:relwrsiveIsNaN(self.output) then
      if self:relwrsiveIsNaN(input) then
         error(string.format("NaN found in input of module :\n%s", self:__tostring__()))
      elseif self:relwrsiveIsNaN(self:parameters()) then
...
function NaN:updateGradInput(input, gradOutput)
   ...
   if self:relwrsiveIsNaN(self.gradInput) then
      if self:relwrsiveIsNaN(gradOutput) then
...
function NaN:accGradParameters(input, gradOutput, scale)
   ...
   if self:relwrsiveIsNaN(gradParams) then
