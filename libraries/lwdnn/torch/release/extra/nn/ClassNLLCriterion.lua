...
function ClassNLLCriterion:updateOutput(input, target)
   if type(target) == 'number' then
      if torch.typename(input):find('torch%.Lwca.*Tensor') then
          self.target = torch.LwdaLongTensor and self.target:lwdaLong() or self.target:lwca()
      else
          self.target = self.target:long()
      end
      self.target:resize(1)
      self.target[1] = target
   elseif torch.typename(input):find('torch%.Lwca.*Tensor') then
      self.target = torch.LwdaLongTensor and target:lwdaLong() or target
   else
