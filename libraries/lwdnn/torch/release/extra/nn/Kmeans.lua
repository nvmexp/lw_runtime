...
local function isLwdaTensor(tensor)
   local typename = torch.typename(tensor)
   if typename and typename:find('torch.Lwca*Tensor') then
      return true
   end
   return false
end
...
function Kmeans:updateOutput(input)
   ...
   self._minIndx = self._minIndx or (isLwdaTensor(input) and torch.LwdaLongTensor() or torch.LongTensor())
