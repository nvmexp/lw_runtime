...
local function backward(self, method, input, gradOutput, scale)
   ...
   if isTable then
      for i,module in ipairs(self.modules) do
         local lwrrentGradInput = self:rethrowErrors(module, i, method, input, gradOutput[i], scale)
      ...
      for i,module in ipairs(self.modules) do
         local lwrrentGradInput = self:rethrowErrors(module, i, method, input, gradOutput[i], scale)
         if i == 1 then
            self.gradInput:resize(lwrrentGradInput:size()):copy(lwrrentGradInput)
         else
...
function ConcatTable:aclwpdateGradParameters(input, gradOutput, lr)
   for i,module in ipairs(self.modules) do
      self:rethrowErrors(module, i, 'aclwpdateGradParameters', input, gradOutput[i], lr)
   end
end
