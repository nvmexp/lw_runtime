...
function nntest.WeightNorm()
   ...
   -- temporal colwolution
   local model = nn.WeightNorm(nn.TemporalColwolution(5, 20, 2, 1))
   local err = nn.Jacobian.testJacobianParameters(model, input,
                                                model.bias, model.gradBias)
   mytester:assert(err < precision, 'Temporal Colwolution bias')
   err = nn.Jacobian.testJacobianParameters(model, input,
                                                model.g, model.gradG)
   mytester:assert(err < precision, 'Temporal Colwolution g')
   err = nn.Jacobian.testJacobianParameters(model, input,
                                                model.v, model.gradV)
   mytester:assert(err < precision, 'Temporal Colwolution v')
   ...
   -- spatial colwolution with 4D weights
   input = torch.rand(5, 10, 10)
   model = nn.WeightNorm(nn.SpatialColwolution(5, 20, 2, 2, 3, 3, 1, 1), 2)
   err = nn.Jacobian.testJacobianParameters(model, input,
                                                model.bias, model.gradBias)
   mytester:assert(err < precision, 'Spatial Colwolution bias')
   err = nn.Jacobian.testJacobianParameters(model, input,
                                                model.g, model.gradG)
   mytester:assert(err < precision, 'Spatial Colwolution g')
   err = nn.Jacobian.testJacobianParameters(model, input,
                                                model.v, model.gradV)
   mytester:assert(err < precision, 'Spatial Colwolution v')
...
function nntest.CReLU()
   ...
   model:add(nn.SpatialColwolution(1, 3, 3, 3, 1, 1, 1, 1))
...
function nntest.SpatialColwolution()
   ...
   local module = nn.SpatialColwolution(from, to, ki, kj, si, sj)
   ...
   local function jacTests(module)
      ...
      ini = (outi-1)*si+ki
      inj = (outj-1)*sj+kj
      module = nn.SpatialColwolution(from, to, ki, kj, si, sj)
...
function nntest.SpatialColwolutionMM()
   ...
   local module = nn.SpatialColwolutionMM(from, to, ki, kj, di, dj, padW, padH)
   local input = torch.Tensor(from, inj, ini):zero()
   ...
   module = nn.SpatialColwolutionMM(from, to, ki, kj, di, dj, padW, padH)
   input = torch.Tensor(batch,from,inj,ini):zero()
...
function nntest.SpatialColwolutionLocal()
   ...
   local ini = (outi-1)*si+ki
   local inj = (outj-1)*sj+kj
   local module = nn.SpatialColwolutionLocal(from, to, ini, inj, ki, kj, si, sj)
   local input = torch.Tensor(from, inj, ini):zero()
   ...
   ini = (outi-1)*si+ki
   inj = (outj-1)*sj+kj
   module = nn.SpatialColwolutionLocal(from, to, ini, inj, ki, kj, si, sj)
   input = torch.Tensor(batch, from, inj, ini):zero()
   ...
   -- check against nn.SpatialColwolution
   local colw = nn.SpatialColwolution(from, to, ki, kj, si, sj)
   torch.repeatTensor(module.bias, colw.bias:view(to, 1, 1), 1, outj, outi)
   torch.repeatTensor(module.weight, colw.weight:view(1, 1, from, to, ki, kj), outi, outj, 1, 1, 1, 1)
   local input = torch.rand(batch, from, inj, ini)
   local output = module:forward(input)
   local outputColw = colw:forward(input)
   local err = torch.dist(output, outputColw)
   mytester:assertlt(err, precision, 'error checking against nn.SpatialColwolution')
end

function nntest.SpatialFullColwolution()
   local from = math.random(2,5)
   local to = math.random(1,5)
   local ki = math.random(1,5)
   local kj = math.random(1,5)
   ...
   local module = nn.SpatialFullColwolution(from, to, ki, kj, di, dj, padW, padH, adjW, adjH)
   local input = torch.Tensor(from, inj, ini):zero()
   ...
   local function jacTests(module)
      ...
      module = nn.SpatialFullColwolution(from, to, ki, kj, di, dj, padW, padH, adjW, adjH)
      input = torch.Tensor(batch,from,inj,ini):zero()
...
function nntest.SpatialFullColwolutionDualInput()
   local from = math.random(2,5)
   local to = math.random(1,5)
   local ki = math.random(1,5)
   local kj = math.random(1,5)
   ...
   local module = nn.SpatialFullColwolution(from, to, ki, kj, di, dj, padW, padH)
   local moduleRef = nn.SpatialFullColwolution(from, to, ki, kj, di, dj, padW, padH, adjW, adjH)
...
function nntest.SpatialDilatedColwolution()
   local from = math.random(1,5)
   local to = math.random(1,5)
   local ki = math.random(1,5)
   local kj = math.random(1,5)
   ...
   local module = nn.SpatialDilatedColwolution(from, to, ki, kj, di, dj, padW, padH, dilationW, dilationH)
   local input = torch.Tensor(from, inj, ini):zero()
   ...
   module = nn.SpatialDilatedColwolution(from, to, ki, kj, di, dj, padW, padH, dilationW, dilationH)
   input = torch.Tensor(batch,from,inj,ini):zero()
...
function nntest.SpatialColwolutionMap()
   local from = math.random(1,5)
   ...
   local module = nn.SpatialColwolutionMap(nn.tables.random(from, to, fanin), ki, kj, si, sj)
   local input = torch.Tensor(from, inj, ini):zero()
   ...
   module = nn.SpatialColwolutionMap(nn.tables.random(from, to, fanin), ki, kj, si, sj)
   input = torch.Tensor(batch,from,inj,ini):zero()
...
function nntest.SpatialFullColwolutionMap()
   ...
   local ini = math.random(5,7)
   local inj = math.random(5,7)
   local module = nn.SpatialFullColwolutionMap(tt, ki, kj, si, sj)
   local input = torch.Tensor(from, inj, ini):zero()
...
function nntest.SpatialFullColwolutionCompare()
    local from = math.random(2,4)
    local to = math.random(2,5)
    ...
    local module1 = nn.SpatialFullColwolutionMap(tt, ki, kj, si, sj)
    local module2 = nn.SpatialFullColwolution(from, to, ki, kj, si, sj)
...
function nntest.SpatialColwolutionBatchCompare()
   local from = math.random(1,5)
   local to = math.random(1,5)
   local ki = math.random(1,5)
   local kj = math.random(1,5)
...
function nntest.SpatialFullColwolutionBatchCompare()
   local from = math.random(1,5)
   local to = math.random(1,5)
   local ki = math.random(1,5)
   local kj = math.random(1,5)
   ...
   local module = nn.SpatialFullColwolution(from, to, ki, kj, si, sj)
   module:zeroGradParameters()
   local input = torch.randn(from, inj, ini)
...
function nntest.TemporalColwolution()
   -- 1D
   local from = math.random(1,5)
   local to = math.random(1,5)
   local ki = math.random(1,5)
   ...
   local outi = math.random(5,7)
   local ini = (outi-1)*si+ki
   local module = nn.TemporalColwolution(from, to, ki,si)
...
function nntest.TemporalRowColwolution()
  ...
  local module = nn.TemporalRowColwolution(from, ki, si)
  jacTest(module)
  module:noBias()
  jacTest(module)
...
function nntest.VolumetricFullColwolution_simple_test()
    local module = nn.VolumetricFullColwolution(3, 1, 3, 3, 3, 3, 3, 3);
...
function nntest.VolumetricFullColwolution()
    ...
    local module = nn.VolumetricFullColwolution(from, to, kt, ki, kj, st, si, sj)
...
function nntest.VolumetricFullColwolutionDualInput()
   local from = math.random(2,3)
   local to = math.random(2,3)
   local kt = math.random(3,4)
   local ki = math.random(3,4)
   ...
   local module = nn.VolumetricFullColwolution(from, to, kt, ki, kj, dt, di, dj, padT, padW, padH)
   local moduleRef = nn.VolumetricFullColwolution(from, to, kt, ki, kj, dt, di, dj, padT, padW, padH, adjT, adjW, adjH)
   moduleRef.weight:copy(module.weight)
   moduleRef.bias:copy(module.bias)
...
function nntest.VolumetricColwolution()
   local from = math.random(2,4)
   local to = math.random(1,4)
   ...
   local module = nn.VolumetricColwolution(from, to, kt, ki, kj, st, si, sj, padT, padW, padH)
   local input = torch.Tensor(from, int, inj, ini):zero()
...
function nntest.VolumetricDilatedColwolution()
   local from = math.random(1,5)
   local to = math.random(1,5)
   local ki = math.random(1,5)
   local kj = math.random(1,5)
   ...
   local module = nn.VolumetricDilatedColwolution(from, to, kk, ki, kj, dk, di, dj, padT, padW, padH, dilationT, dilationW, dilationH)
   ...
   module = nn.VolumetricDilatedColwolution(from, to, kk, ki, kj, dk, di, dj, padT, padW, padH, dilationT, dilationW, dilationH)
...
function nntest.VolumetricColwolutionBatchCompare()
   local from = math.random(2,3)
   local to = math.random(2,3)
   local kt = math.random(3,4)
   local ki = math.random(3,4)
...
function nntest.VolumetricMaxPooling_boundary()
   ...
   for c = 1,nip do
      ...
      mytester:asserteq(output[c][1][1][4], max_val, 'wrong forward exelwtion')
   end
   ...
   for c = 1,nip do
       ...
       mytester:asserteq(output[c][1][4][1], max_val, 'wrong forward exelwtion')
   end

   -- check output signals at right bottom
    for c = 1,nip do
      local max_val = math.max(input[c][1][7][7], input[c][2][7][7])
      mytester:asserteq(output[c][1][4][4], max_val, 'wrong forward exelwtion')
    end
...
function nntest.PairwiseDistance()
   ...
   for p = 1,4 do  -- test a few Lp norms
      ...
      -- Also check that the forward prop result is correct.
      -- manually callwlate each distance separately
...
function nntest.LookupTable()
   ...
   local function dotest(module, input, milwal, maxval)
       local output = module:forward(input)
       module:backwardUpdate(input, output, 0.1)
       input:zero()

       -- 1D
       local err = jac.testJacobianParameters(module, input, module.weight, module.gradWeight, milwal, maxval)
       mytester:assertlt(err,precision, '1D error on weight ')

       local err = jac.testJacobianUpdateParameters(module, input, module.weight, milwal, maxval)
       ...
       local ferr,berr = jac.testIO(module,input,milwal,maxval)
       mytester:eq(ferr, 0, torch.typename(module) .. ' - i/o forward err ', precision)
       mytester:eq(berr, 0, torch.typename(module) .. ' - i/o backward err ', precision)

       -- aclwpdate
       module:aclwpdateOnly()
   ...
   for i = 1, 100 do
       ...
       module:aclwpdateGradParameters(input, y, 0.1)
   end
...
-- Define a test for SpatialUpSamplingLwda
...
function nntest.DepthConcat()
   ...
   concat:add(nn.SpatialColwolutionMM(3, outputSize[1], 1, 1, 1, 1)) --> 2, 5, 12, 12
   concat:add(nn.SpatialColwolutionMM(3, outputSize[2], 3, 3, 1, 1)) --> 2, 6, 10, 10
   concat:add(nn.SpatialColwolutionMM(3, outputSize[3], 4, 4, 1, 1)) --> 2, 7, 9, 9
   concat:add(nn.SpatialColwolutionMM(3, outputSize[4], 5, 5, 1, 1)) --> 2, 8, 8, 8
   ...
   for i=1,4 do
      local colw = concat:get(i)
      local gradWeight = colw.gradWeight:clone()
      colw:zeroGradParameters()
      output[narrows[i]]:copy(colw:forward(input))
      gradInput:add(colw:backward(input, gradOutput[narrows[i]]))
      mytester:assertTensorEq(gradWeight, colw.gradWeight, 0.000001, "Error in SpatialConcat:accGradParameters for colw "..i)
   end
...
function nntest.PixelShuffle()
   ...
   -- Sub-Pixel Colwolutional Neural Network", Shi et al.
   ...
   -- essentially performs the ilwerse of Eq 4. in
   -- "Real-Time Single Image and Video Super-Resolution Using an Efficient
   -- Sub-Pixel Colwolutional Neural Network", Shi et al.
   -- This function tests for multip batch sizes, multiple channels and multiple input dimensions (square)
   -- It also tests for normal tensors (un-batched)
...
function nntest.Typecast()
  ...
  -- check relwrsiveType on a table
  ...
  nn.utils.relwrsiveType({net1, net2}, 'torch.FloatTensor')
  net1.modules[1].bias:fill(3)
  assert(net2.modules[1].bias[1] == 3)

  -- smoke test some modules with custom type methods
  local lwstom_type_modules = {
    nn.MixtureTable(3),
    nn.ConcatTable(),
    nn.Copy(),
  ...
  for _, module in ipairs(lwstom_type_modules) do
    module:float()
  end
end
...
function nntest.SpatialDepthWiseColwolution()
   ...
   local model = spatialDepthWiseColw(n, m, k, st, p, s, weight, bias)
   local model_util = spatialDepthWiseColw_util(n, m, k, st, p, s, weight, bias)
...
function nntest.WhiteNoise()
   ...
   local mealwalue = output:mean()
   local stdValue = output:std()
   mytester:assert(mealwalue > -0.01 and mealwalue < 0.01)
   mytester:assert(stdValue < 0.15 and stdValue >= 0)

   -- Evaluate
   addNoise:evaluate()
   output = addNoise:forward(input)
   mealwalue = output:mean()
   stdValue = output:std()
   mytester:assert(mealwalue == 0)
...
function nntest.OneHot()
   ...
   if pcall(function() require 'lwnn' end) then
      ...
      local output = oh:forward(input)
      mytester:assert(torch.type(output) == 'torch.LwdaTensor')
      mytester:assertTensorEq(output:double(), output2, 0.000001, "OneHot forward batch long-lwca err")

      -- test with lwca input
      local input = input:lwca()
      gradOutput = gradOutput:lwca()

      local output = oh:forward(input)
      mytester:assert(torch.type(output) == 'torch.LwdaTensor')
      mytester:assertTensorEq(output:double(), output2, 0.000001, "OneHot forward batch lwca err")
      ...
      -- non-batch mode (number input)
      mytester:assertTensorEq(oh:forward(num), output3:lwca(), 0.000001, "OneHot forward number err")
   end
   ...
   if pcall(function() require 'lwnn' end) then
      ...
      local output = oh:forward(input)
      mytester:assert(torch.type(output) == 'torch.LwdaTensor')
      mytester:assertTensorEq(output:double(), output2, 0.000001, "OneHot 2d forward batch long-lwca err")
      ...
      local output = oh:forward(input)
      mytester:assert(torch.type(output) == 'torch.LwdaTensor')
      mytester:assertTensorEq(output:double(), output2, 0.000001, "OneHot 2d forward batch lwca err")
      ...
      if benchmark then
         local input = torch.FloatTensor(50, 50):random(1,65):lwca()

         local oh = nn.OneHot(65):lwca()
...
function nntest.Colwert()
   -- batch mode
   local c = nn.Colwert('bchw', 'chwb')
   local input = torch.randn(8,3,5,5)
   local output = c:forward(input)
   local output2 = input:transpose(1,4):transpose(1,3):transpose(1,2)
   mytester:assertTensorEq(output, output2, 0.000001, "Colwert fwd bchw->chwb")
   local gradInput = c:backward(input, output)
   mytester:assertTensorEq(gradInput, input, 0.000001, "Colwert bwd bchw->chwb")
   local c = nn.Colwert('bchw', 'bf')
   local output = c:forward(input)
   local output2 = input:view(8,-1)
   mytester:assertTensorEq(output, output2, 0.000001, "Colwert fwd bchw->bf")
   c:float()
   local output = c:forward(input:float())
   mytester:assertTensorEq(output, output2:float(), 0.000001, "Colwert:type()")
   local output = c:forward(input)
   mytester:assertTensorEq(output, output2:float(), 0.000001, "Colwert:type() double->float")
   -- non-batch mode
   local c = nn.Colwert('chw', 'hwc')
   local input = torch.randn(3,5,5)
   local output = c:forward(input)
   local output2 = input:transpose(1,3):transpose(1,2)
   mytester:assertTensorEq(output, output2, 0.000001, "Colwert fwd chw->hwc non-batch")
   local gradInput = c:backward(input, output)
   mytester:assertTensorEq(gradInput, input, 0.000001, "Colwert bwd chw->hwc non-batch")
   local c = nn.Colwert('chw', 'f')
   local output = c:forward(input)
   local output2 = input:view(-1)
   mytester:assertTensorEq(output, output2, 0.000001, "Colwert fwd chw->bf non-batch")
   c:float()
   local output = c:forward(input:float())
   mytester:assertTensorEq(output, output2:float(), 0.000001, "Colwert:type() non-batch")
   local output = c:forward(input)
   mytester:assertTensorEq(output, output2:float(), 0.000001, "Colwert:type() double->float non-batch")
end
...
function nntest.Kmeans()
   ...
   local useLwdas = {false}
   if pcall(function() require 'lwnn' end) then
      useLwdas[2] = true
   end
   for _, initType in pairs(initTypes) do
      for _, useLwda in pairs(useLwdas) do
         ...
         if verbose then
            print("InitType: " .. initType .. " useLwda: " .. tostring(useLwda))
            print("Best Loss: " .. bestLoss)
            print("Total time: " .. timer:time().real)
         end
         if initType == 'kmeans++' then
            mytester:assert(bestLoss < 0.00001, "Kmeans++ error ("..(useLwda and 'lwca' or 'double')..")")
         else
            mytester:assert(bestLoss < 500, "Kmeans error ("..(useLwda and 'lwca' or 'double')..")")
         end
      end
   end
end
