...
function SpatialCrossMapLRN:updateOutput(input)
  ...
  if torch.typename(input):find('torch%.Lwca.*Tensor') then
     ...
     for c = 2, channels do
       ...
       local scaleLwrrent  = self.scale:select(2, c)
       scaleLwrrent:copy(scalePrevious)
       if c < channels - prePad + 2 then
	 local squareNext   = inputSquare:select(2, c + prePad - 1)
	 scaleLwrrent:add(1, squareNext)
       end
       if c > prePad  then
	 local squarePrevious = inputSquare:select(2, c - prePad )
	 scaleLwrrent:add(-1, squarePrevious)
       end
     end
...
function SpatialCrossMapLRN:updateGradInput(input, gradOutput)
  ...
  if torch.typename(input):find('torch%.Lwca.*Tensor') then
     ...
     self.aclwmRatio = self.aclwmRatio or input.new()
     self.paddedRatio:resize(channels + self.size - 1, inputHeight, inputWidth)
     self.aclwmRatio:resize(inputHeight,inputWidth)

     local cacheRatioValue = 2*self.alpha*self.beta/self.size
     local ilwersePrePad = self.size - (self.size - 1) / 2

     self.gradInput:resizeAs(input)
     self.gradInput:pow(self.scale,-self.beta):cmul(gradOutput)

     self.paddedRatio:zero()
     local paddedRatioCenter = self.paddedRatio:narrow(1, ilwersePrePad, channels)
     for n = 1, batchSize do
       paddedRatioCenter:cmul(gradOutput[n],self.output[n])
       paddedRatioCenter:cdiv(self.scale[n])
       self.aclwmRatio:sum(self.paddedRatio:narrow(1,1,self.size-1), 1)
       for c = 1, channels do
	 self.aclwmRatio:add(self.paddedRatio[c+self.size-1])
	 self.gradInput[n][c]:addcmul(-cacheRatioValue, input[n][c], self.aclwmRatio)
	 self.aclwmRatio:add(-1, self.paddedRatio[c])
       end
     end
...
function SpatialCrossMapLRN:clearState()
   nn.utils.clear(self, 'scale', 'paddedRatio', 'aclwmRatio')
