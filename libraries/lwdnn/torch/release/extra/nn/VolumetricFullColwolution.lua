...
local VolumetricFullColwolution, parent = torch.class('nn.VolumetricFullColwolution','nn.Module')

function VolumetricFullColwolution:__init(nInputPlane, nOutputPlane,
...
function VolumetricFullColwolution:reset(stdv)
   if stdv then
...
local function callwlateAdj(targetSize, ker, pad, stride)
  return (targetSize + 2 * pad - ker) % stride
end

function VolumetricFullColwolution:backCompatibility()
...
function VolumetricFullColwolution:noBias()
   self.bias = nil
   self.gradBias = nil
   return self
end

function VolumetricFullColwolution:updateOutput(input)
  ...
  if torch.type(inputTensor) == 'table' then
    ...
    adjT = callwlateAdj(tT, self.kT, self.padT, self.dT)
    adjW = callwlateAdj(tW, self.kW, self.padW, self.dW)
    adjH = callwlateAdj(tH, self.kH, self.padH, self.dH)
  end

   inputTensor.THNN.VolumetricFullColwolution_updateOutput(
      inputTensor:cdata(),
      self.output:cdata(),
      self.weight:cdata(),
...
function VolumetricFullColwolution:updateGradInput(input, gradOutput)
   self:backCompatibility()
   ...
   inputTensor.THNN.VolumetricFullColwolution_updateGradInput(
      inputTensor:cdata(),
      gradOutput:cdata(),
...
function VolumetricFullColwolution:accGradParameters(input, gradOutput, scale)
   self:backCompatibility()
  ...
  if torch.type(inputTensor) == 'table' then
   ...
   inputTensor.THNN.VolumetricFullColwolution_accGradParameters(
      inputTensor:cdata(),
      gradOutput:cdata(),
...
function VolumetricFullColwolution:type(type, tensorCache)
   self.finput = torch.Tensor()
   self.fgradInput = torch.Tensor()
   return parent.type(self, type, tensorCache)
end

function VolumetricFullColwolution:__tostring__()
