...
function Module:backwardUpdate(input, gradOutput, lr)
   self:updateGradInput(input, gradOutput)
   self:aclwpdateGradParameters(input, gradOutput, lr)
   return self.gradInput
end
...
function Module:aclwpdateGradParameters(input, gradOutput, lr)
   if self.shared then
      self:sharedAclwpdateGradParameters(input, gradOutput, lr)
   else
      self:defaultAclwpdateGradParameters(input, gradOutput, lr)
   end
end

function Module:defaultAclwpdateGradParameters(input, gradOutput, lr)
...
function Module:sharedAclwpdateGradParameters(input, gradOutput, lr)
...
-- the same type (and, in the case of LWCA, on the same device), which
-- is not always true. Use for_each() to iterate over this module and
-- children instead.
--
-- Module._flattenTensorBuffer can be used by other packages (e.g. lwnn)
-- to specify the type of temporary buffers. For example, the temporary
-- buffers for LwdaTensor could be FloatTensor, to avoid GPU memory usage.
--
-- TODO: This logically belongs to torch.Tensor, not nn.
Module._flattenTensorBuffer = {}
function Module.flatten(parameters)

   -- returns true if tensor oclwpies a contiguous region of memory (no holes)
   ...
   local compactOffsets  = flatParameters:long():lwmsum(1)
  ...
  if (self.modules ~= nil) then
    if (torch.type(self.modules) == 'table') then
      for i = 1, #self.modules do
        ...
        for j = 1, #lwr_nodes do
          nodes[#nodes+1] = lwr_nodes[j]
          containers[#containers+1] = lwr_containers[j]
        end
      end
    end
  end
...
-- similar to apply, relwrsively goes over network and calls
