...
function LookupTable:aclwpdateOnly()
   self.gradWeight = nil
   return self
end
...
function LookupTable:updateGradInput(input, gradOutput)
   ...
   -- colwerted anyway to LongTensor) thus, need to allocate
...
function LookupTable:type(type, tensorCache)
   ...
   if type and type:find('torch%.Lwca.*Tensor') then
      -- LWCA uses _sorted and _indices temporary tensors
      self._sorted = torch.LwdaLongTensor and torch.LwdaLongTensor.new() or torch.LwdaTensor.new()
      self._indices = torch.LwdaLongTensor and torch.LwdaLongTensor.new() or torch.LwdaTensor.new()
      self._count = torch.LwdaLongTensor and torch.LwdaLongTensor.new() or torch.LwdaTensor.new()
      self._input = torch.LwdaLongTensor and torch.LwdaLongTensor.new() or torch.LwdaTensor.new()
   else
      -- self._count and self._input should only be colwerted if using Lwca
...
function LookupTable:sharedAclwpdateGradParameters(input, gradOutput, lr)
