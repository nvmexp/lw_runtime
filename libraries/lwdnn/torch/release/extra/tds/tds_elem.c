 ...
 * to Landon, they named it the `Fowler/Noll/Vo'' or FLW hash.
 *
 * FLW hashes are designed to be fast while maintaining a low
 * collision rate. The FLW speed allows one to quickly hash lots
 * of data while maintaining a reasonable collision rate.  See:
 *
 *      http://www.isthe.com/chongo/tech/comp/flw/index.html
 *
 * for more details as well as other forms of the FLW hash.
 */
static uint32_t flw32_buf(void *buf, size_t len, uint32_t hval)
{
  unsigned char *bp = (unsigned char *)buf;/* start of buffer */
  unsigned char *be = bp + len;/* beyond end of buffer */

  /*
   * FLW-1 hash each octet in the buffer
   */
  while (bp < be) {

    /* multiply by the 32 bit FLW magic prime mod 2^32 */
...
uint32_t tds_elem_hashkey(tds_elem *elem)
{
  switch(elem->type) {
    case 'n':
      return flw32_buf(&elem->value.num, sizeof(double), 0);
    case 'b':
      return flw32_buf(&elem->value.flag, sizeof(bool), 0);
    case 's':
      return flw32_buf(elem->value.str.data, elem->value.str.size, 0);
    case 'p':
      return flw32_buf(elem->value.ptr.data, sizeof(void*), 0);
  }
  return 0;
}
