...
--[[
   complex to real dft. This function is the exact ilwerse of signal.rfft
]]--
...
--[[
   2D complex to real dft. This function is the exact ilwerse of signal.rfft2
]]--
...
--[[
   ...
   Run a window across your signal and callwlate fourier transforms on that window.
...
function signal.spectrogram(inp, window_size, window_stride)
   ...
   local stftout = signal.rstft(inp, window_size, window_stride)
      -- callwlate magnitude of signal and colwert to dB to make it look prettier
...
--[[
   ...
   Returns the cepstrum and a phase shift factor "nd" that is useful to ilwert the signal back.
...
function signal.cceps(x)
   typecheck(x)
   --[[
      logh = log(abs(h)) + sqrt(-1)*rlwnwrap(complex.angle(h));
      y = real(ifft(logh));
   ]]--
   ...
   local nd
   logh[{{},2}],nd = signal.rlwnwrap(complex.angle(h))
   local y = signal.ifft(logh)
   return y[{{},1}], nd -- real part
end

--[[
   1D Ilwerse Complex cepstral analysis.
...
--[[
   ...
   The real cepstrum is the ilwerse Fourier transform of the real logarithm of the magnitude of the Fourier transform of a sequence.
...
--[[
   ilwerse 1D Discrete Cosine Transform (DCT)
   Takes Real inputs (1D tensor of N points)
...
--[[
   ilwerse 2D Discrete Cosine Transform (DCT)
   Takes Real inputs (2D tensor of NxM points)
...
--[[
   ilwerse 3D Discrete Cosine Transform (DCT)
   Takes Real inputs (3D tensor of NxMxP points)
