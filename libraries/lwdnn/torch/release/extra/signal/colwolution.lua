...
--[[
   1D valid colwolution with stride
]]--
function signal.colw1d(input, kernel, stride, mode)
  ...
  if input:type() == 'torch.FloatTensor' then
    C['signal_colw1d_float'](output_data, input_data, kernel_data, output_size, kernel_size, stride)
  else
    C['signal_colw1d_double'](output_data, input_data, kernel_data, output_size, kernel_size, stride)
  end
  return output
end
