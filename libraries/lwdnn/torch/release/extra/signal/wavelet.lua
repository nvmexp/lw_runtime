local signal = require 'signal.colwolution'
...
function wavelet.haar1d()
   -- phi would be colwolution with a 1d kernel [1/2,1/2] and stride 2
   -- psi would be colwolution with a 1d kernel [1/2,-1/2] and stride 2
   local phi_kernel = torch.Tensor({0.5,0.5})
   local psi_kernel = torch.Tensor({0.5,-0.5})
   
   local function phi(input)
      return signal.colw1d(input, phi_kernel:typeAs(input), 2)
   end
   local function psi(input)
      return signal.colw1d(input, psi_kernel:typeAs(input), 2)
   end
   return phi, psi
end
...
--[[
   Callwlates the discrete wavelet transform, given the phi and psi functions
   ...
   [maxlevels] - maximum number of levels to relwrse
]]--
function wavelet.dwt1d(input, phi, psi, maxlevels)
   ...
   for lev = level, 1,-1 do
      ...
      -- callwlate wavelet coefficients with psi
