...
function BlockSparse:__init(nInputBlock, inputSize, nOutputBlock, outputSize, aclwpdate)
   parent.__init(self)
...
function BlockSparse:type(type)
   if type and (type == 'torch.FloatTensor' or type == 'torch.DoubleTensor' or type == 'torch.LwdaTensor') then
      ...
      if type == 'torch.LwdaTensor' then
         ...
         self.outputBatched = torch.LwdaTensor()
         self.gradInputBatched = torch.LwdaTensor()
         self._gradOutput = torch.LwdaTensor()
      end
   end
   return self
end
...
BlockSparse.sharedAclwpdateGradParameters = BlockSparse.aclwpdateGradParameters
