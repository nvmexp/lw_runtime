...
function WindowSparse:__init(inputSize, outputSize, outputWindowSize, aclwpdate)
   parent.__init(self)
   self.inputSize = inputSize
   self.outputSize = outputSize
   self.outputWindowSize = outputWindowSize
   self.aclwpdate = aclwpdate or false
   ...
   -- lwblasSgemmBatched. If errors, set this to 100000
...
function WindowSparse:type(type)
   if type and (type == 'torch.FloatTensor' or type == 'torch.DoubleTensor' or type == 'torch.LwdaTensor') then
      ...
      if type == 'torch.LwdaTensor' then
         self.inputLwda = torch.LwdaTensor()
         self.weightLwda = torch.LwdaTensor()
         self.biasLwda = torch.LwdaTensor()
         self.outputLwda = torch.LwdaTensor()
         
         self.inputIndiceLwda = torch.LwdaTensor()
         self.outputIndiceLwda = torch.LwdaTensor()
      end
   end
   return self
end
...
WindowSparse.sharedAclwpdateGradParameters = WindowSparse.aclwpdateGradParameters
