...
--e.g. usage: th -llwnnx -e "nn.testlwdax{'SoftMaxTree','BlockSparse'}"
...
function lwnnxtest.SoftMaxTree()
   ...
   input = input:lwca()
   target = target:float():lwca()
   grad = grad:lwca()
   smt2:zeroGradParameters()
   smt2:forward{input, target}
   smt2._multiBuffer:zero()
   local reslwdaF = smt2:forward{input, target}:clone()
   local logsoftOutputLwda = smt2._multiBuffer:clone():float()
   local reslwdaB = smt2:backward({input, target}, grad)[1]:clone()
   local linearOutputLwda = smt2._multiBuffer:clone():float()
   smt2._multiBuffer:zero()
   local gradWeightLwda = smt2.gradWeight:clone()
   local gradBiasLwda = smt2.gradBias:clone()
   local weightLwda = smt2.weight:clone()
   local biasLwda = smt2.bias:clone()
   smt2:zeroGradParameters()
   a:reset()
   for i = 1,nloop do
      smt2:forward{input, target}
   ...
   mytester:assertTensorEq(reslwdaF:float(), groundtruthF, precision_forward, 'error on state (forward) ')
   mytester:assertTensorEq(logsoftOutput, logsoftOutputLwda, precision_forward, 'error on state (logsoftOutput) ')
   mytester:assertTensorEq(reslwdaB:float(), groundtruthB, precision_backward, 'error on state (backward) ')
   mytester:assertTensorEq(linearOutput, linearOutputLwda, precision_forward, 'error on state (linearOutput) ')
   mytester:assertTensorEq(gradWeightLwda:float(), gradWeight, precision_backward*10, 'error on state (accGradParameters gradWeight) ')
   mytester:assertTensorEq(gradBiasLwda:float(), gradBias, precision_backward*10, 'error on state (accGradParameters gradBias) ')
   mytester:assertTensorEq(weightLwda:float(), weight, precision_backward, 'error on state (weight) ')
   mytester:assertTensorEq(biasLwda:float(), bias, precision_backward, 'error on state (bias) ')
...
function lwnnxtest.SoftMaxTree_issue24()
   ...
   local inputGPU = input:lwca()
   local targetGPU = target:lwca()
   local smtGPU = smt:clone():lwca()
   local tllGPU = tll:clone():lwca()
...
function lwnnxtest.BlockSparse()
   ...
   local input = torch.randn(batchSize,inputWindowSize,inputSize):lwca()
   local gradOutput = torch.randn(batchSize,outputWindowSize,outputSize):lwca()
   local inputIndice = torch.LwdaTensor(batchSize, inputWindowSize)
   local outputIndice = torch.LwdaTensor(batchSize, outputWindowSize)
   ...
   local inputScale = torch.randn(batchSize, inputWindowSize):lwca()
   --inputScale:fill(1)
   local outputScale = torch.randn(batchSize, outputWindowSize):lwca()
   --outputScale:fill(1)   
...
function lwnnxtest.BlockSparse_benchmark()
   ...
   local inputScale = torch.LwdaTensor(batchSize, inputWindowSize)
   inputScale:fill(1)
   local outputScale = torch.LwdaTensor(batchSize, outputWindowSize)
   outputScale:fill(1)   
   local gradOutputTable = {gradOutput, {outputIndice, outputScale}}
   ...
   mlp:lwca()
   local input3 = torch.randn(batchSize, nInputBlock*inputSize):lwca()
   local gradOutput3 = torch.randn(batchSize, nOutputBlock*outputSize):lwca()
   mlp:forward(input3)
   a:reset()
   for i=1,nloop do
      mlp:forward(input3)
   ...
   mlp:lwca()
   input3 = torch.randn(batchSize, inputWindowSize*inputSize):lwca()
   gradOutput3 = torch.randn(batchSize, outputWindowSize*outputSize):lwca()
   mlp:forward(input3)
   a:reset()
   for i=1,nloop do
      mlp:forward(input3)
...
function lwnnxtest.BlockSparse_dense()
...
function lwnnxtest.BlockMixture()
   local inputSize = 256
   ...
   local input = torch.randn(batchSize, inputSize):lwca()
   local gradOutput = torch.randn(batchSize, outputSize):lwca()
   ...
   mlp:add(nn.Tanh())
   mlp:lwca()
   local input3 = torch.randn(batchSize, inputSize):lwca()
   local gradOutput3 = torch.randn(batchSize, outputSize):lwca()
   mlp:forward(input3)
   a:reset()
   for i=1,nloop do
      mlp:forward(input3)
   ...
   mlp:add(nn.Tanh())
   mlp:lwca()
   input3 = torch.randn(batchSize, inputSize):lwca()
   gradOutput3 = torch.randn(batchSize, outputSize):lwca()
   mlp:forward(input3)
   a:reset()
   for i=1,nloop do
      mlp:forward(input3)
...
function lwnnxtest.WindowSparse()
   local inputSize = 32
   local outputSize = 32
   local inputWindowSize = 8
   local outputWindowSize = 8
   ...
   -- windowSparse
   local input = torch.randn(batchSize,inputWindowSize):lwca()
   local gradOutput = torch.randn(batchSize,outputWindowSize):lwca()
   ...
   -- linear
   local input2 = torch.zeros(batchSize, inputSize):lwca()
   local gradOutput2 = torch.zeros(batchSize, outputSize):lwca()
...
function lwnnxtest.WindowSparse_benchmark()
   ...
   mlp:lwca()
   input3 = torch.randn(batchSize, inputWindowSize):lwca()
   gradOutput3 = torch.randn(batchSize, outputWindowSize):lwca()
   mlp:forward(input3)
   a:reset()
   for i=1,nloop do
      --mlp:zeroGradParameters()
...
function lwnnxtest.Sort()
   ...
   s:lwca()
   input = torch.randn(batchSize, nInput):lwca()
   output = s:forward(input)
   gradInput = s:backward(input, output)
   mytester:assertTensorEq(gradInput:float(), input:float(), precision_forward, 'error on state (forward/backward lwca)')
end

function lwnnxtest.LazyKBest()
   local batchSize = 8
   local nInput = 16
   local nOutput = 4
   local s = nn.LazyKBest(4)
   s:lwca()
   local input = torch.randn(batchSize, nInput):lwca()
   local output = s:forward(input)
   local gradInput = s:backward(input, output)
...
function lwnnxtest.WindowGate()
   ...
   local input = torch.randn(batchSize, inputSize):lwca()
   --[[input = torch.zeros(batchSize, inputSize):lwca()
   input[1][20] = 100
   input[2][1] = 100
   input[3][10] = 100
   input[3][11] = 100--]]
...
function lwnnxtest.MultinomialStatistics()
   local inputSize = 7 
   local batchSize = 3
...
function nn.testlwdax(tests)
   local oldtype = torch.getdefaulttensortype()
   torch.setdefaulttensortype('torch.FloatTensor')
   math.randomseed(os.time())
   jac = nn.Jacobian
   mytester = torch.Tester()
   mytester:add(lwnnxtest)
   mytester:run(tests)
   print ''
