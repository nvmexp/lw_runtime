...
function BlockMixture:aclwpdateGradParameters(input, gradOutput, lr)
   if self.expertScale > 0 then
      self.experts[#self.experts]:aclwpdateGradParameters(self.expertInputs[#self.expertInputs][1], gradOutput, lr)
      for i=#self.experts-1,1,-1 do
         self.experts[i]:aclwpdateGradParameters(self.expertInputs[i], self.expertGradInputs[i+1][1], lr*self.expertScale)
      end
   end
   
   if self.gaterScale > 0 then
      if #self.experts == 2 then
         self.gater:aclwpdateGradParameters(input, self.gaterGradOutputs[1], lr*self.gaterScale)
      else
         self.gater:aclwpdateGradParameters(input, self.gaterGradOutputs, lr*self.gaterScale)
      end
   end
end
