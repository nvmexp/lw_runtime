local RNN, parent = torch.class('lwdnn.RNN', 'nn.Module')
...
function RNN:__init(inputSize, hiddenSize, numLayers, batchFirst, dropout, rememberStates)
   ...
   self.datatype = 'LWDNN_DATA_FLOAT'
   self.inputSize = inputSize
   self.hiddenSize = hiddenSize
   self.seqLength = 1
   self.miniBatch = 1
   self.numLayers = numLayers
   self.bidirectional = 'LWDNN_UNIDIRECTIONAL'
   self.numDirections = 1 -- set to 2 for bi-directional.
   self.inputMode = 'LWDNN_LINEAR_INPUT'
   ...
   self.gradInput = torch.LwdaTensor()
   self.output = torch.LwdaTensor()
   self.weight = torch.LwdaTensor()
   self.gradWeight = torch.LwdaTensor()
   self.reserve = torch.LwdaTensor()
   self.hiddenOutput = torch.LwdaTensor()
   self.cellOutput = torch.LwdaTensor()
   self.gradHiddenInput = torch.LwdaTensor()
   self.gradCellInput = torch.LwdaTensor()
...
function RNN:reset(stdv)
   ...
   errcheck('lwdnnGetRNNParamsSize',
            lwdnn.getHandle(),
            self.rnnDesc[0],
            self.xDescs[0],
            weightSize:data(),
...
function RNN:createDropoutDescriptors(count)
   return lwdnn.createDescriptors(count,
                            'lwdnnDropoutDescriptor_t[?]',
                            'lwdnnCreateDropoutDescriptor',
                            'lwdnnDestroyDropoutDescriptor')
end

function RNN:createFilterDescriptors(count)
   return lwdnn.createDescriptors(count,
                            'lwdnnFilterDescriptor_t[?]',
                            'lwdnnCreateFilterDescriptor',
                            'lwdnnDestroyFilterDescriptor')
end

function RNN:createRNNDescriptors(count)
   return lwdnn.createDescriptors(count,
                            'lwdnnRNNDescriptor_t[?]',
                            'lwdnnCreateRNNDescriptor',
                            'lwdnnDestroyRNNDescriptor')
end

function RNN:createTensorDescriptors(count)
   return lwdnn.createDescriptors(count,
                            'lwdnnTensorDescriptor_t[?]',
                            'lwdnnCreateTensorDescriptor',
                            'lwdnnDestroyTensorDescriptor')
end
...
function RNN:resetDropoutDescriptor()
   ...
   errcheck('lwdnnDropoutGetStatesSize',
            lwdnn.getHandle(),
            self.dropoutStatesSize:data())
            self.dropoutStates = torch.LwdaTensor(self.dropoutStatesSize[1])

   errcheck('lwdnnSetDropoutDescriptor',
            self.dropoutDesc[0],
            lwdnn.getHandle(),
            self.dropout,
...
function RNN:resetRNNDescriptor()
   ...
   errcheck('lwdnnSetRNNDescriptor',
            self.rnnDesc[0],
            self.hiddenSize,
            self.numLayers,
            self.dropoutDesc[0],
...
function RNN:resetWeightDescriptor()
   ...
   errcheck('lwdnnSetFilterNdDescriptor',
            self.wDesc[0],
            self.datatype,
            'LWDNN_TENSOR_NCHW',
            3,
...
function RNN:resetIODescriptors()
   ...
   for i = 0, self.seqLength - 1 do
      ...
      errcheck('lwdnnSetTensorNdDescriptor',
               self.xDescs[i],
               self.datatype,
               3,
               dim:data(),
...
function RNN:updateOutput(input)
   ...
   assert(self.dropout == 0 or lwdnn.version >= 5103, 'dropout supported only in lwdnn v5.1 and above')
   ...
   self.workspace = lwdnn.getSharedWorkspace()
   local workspaceSize = torch.LongTensor(1)
   errcheck('lwdnnGetRNNWorkspaceSize',
            lwdnn.getHandle(),
            self.rnnDesc[0],
	    self.seqLength,
            self.xDescs,
   ...
   if self.train then
      ...
      errcheck('lwdnnGetRNNTrainingReserveSize',
               lwdnn.getHandle(),
               self.rnnDesc[0],
	       self.seqLength,
               self.xDescs,
      ...
      errcheck('lwdnnRNNForwardTraining',
               lwdnn.getHandle(),
               self.rnnDesc[0],
	       self.seqLength,
               self.xDescs, x:data(),
      ...
      errcheck('lwdnnRNNForwardInference',
               lwdnn.getHandle(),
               self.rnnDesc[0],
	       self.seqLength,
               self.xDescs, x:data(),
...
function RNN:updateGradInput(input, gradOutput)
   ...
   assert(self.dropout == 0 or lwdnn.version >= 5103, 'dropout supported only in lwdnn v 5.1 and above')
...
function RNN:accGradParameters(input, gradOutput, scale)
   ...
   assert(self.dropout == 0 or lwdnn.version >= 5103, 'dropout supported only in lwdnn 5.1 and above')
   assert(input:dim() == 3, 'input should have 3 dimensions: seqLength, miniBatch, inputSize')
   assert(input:size(1) == self.seqLength, 'input has incorrect sequence length!')
   assert(input:size(2) == self.miniBatch, 'input has incorrect minibatch size!')
   assert(input:size(3) == self.inputSize, 'input has incorrect size!')
   ...
   -- lwdnnRNNBackwardWeights doesn't accept a scale parameter so instead
   ...
   errcheck('lwdnnRNNBackwardWeights',
            lwdnn.getHandle(),
            self.rnnDesc[0],
	    self.seqLength,
            self.xDescs, x:data(),
...
local function numberOfLayers(self)
    if self.bidirectional == 'LWDNN_BIDIRECTIONAL' then
...
-- Function gets either the matrix or bias param x on lwDNN method given, at each layer and linear layerId.
local function retrieveLinearParams(self, lwDNNMethod)
    if not self.wDesc then
        self:resetWeightDescriptor()
    end
    local linearParams = {}
    ...
    for layer = 0, numLayers - 1 do
        ...
        for layerId = 0, numberOfLinearLayers - 1 do
            ...
            local dataType = ffi.new("lwdnnDataType_t[1]")
            local format = ffi.new("lwdnnTensorFormat_t[1]")
            local nbDims = torch.IntTensor(1)

            local minDim = 3
            local filterDimA = torch.ones(minDim):int()
            errcheck('lwdnnGetFilterNdDescriptor',
                linLayerMatDesc[0],
                minDim,
                dataType,
                format,
            ...
            local params = torch.LwdaTensor(self.weight:storage(), offset + 1, filterDimA:prod())
...
function RNN:weights()
    return retrieveLinearParams(self, 'lwdnnGetRNNLinLayerMatrixParams')
end

function RNN:biases()
    return retrieveLinearParams(self, 'lwdnnGetRNNLinLayerBiasParams')
end
