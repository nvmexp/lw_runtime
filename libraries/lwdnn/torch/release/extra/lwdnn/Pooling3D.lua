local Pooling, parent = torch.class('lwdnn._Pooling3D', 'nn.Module')
...
function Pooling:resetPoolDescriptors()
   ...
   self.poolDesc = ffi.new('struct lwdnnPoolingStruct*[1]')
   errcheck('lwdnnCreatePoolingDescriptor', self.poolDesc)
   ...
   errcheck('lwdnnSetPoolingNdDescriptor', self.poolDesc[0], self.mode, 'LWDNN_PROPAGATE_NAN', 3,
            ker:data(), pad:data(), str:data());
   local function destroyPoolDesc(d)
      errcheck('lwdnnDestroyPoolingDescriptor', d[0]);
   end
...
function Pooling:updateOutput(input)
   ...
   errcheck('lwdnnPoolingForward', lwdnn.getHandle(),
            self.poolDesc[0],
...
function Pooling:updateGradInput(input, gradOutput)
   ...
   if not self.poolDesc then self:resetPoolDescriptors() end
   self:createIODescriptors(input)
   errcheck('lwdnnPoolingBackward',
            lwdnn.getHandle(), self.poolDesc[0],
            lwdnn.scalar(input, 1),
            self.oDesc[0], self.output:data(),
