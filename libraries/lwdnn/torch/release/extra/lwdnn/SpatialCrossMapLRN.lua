local LRN, parent = torch.class('lwdnn.SpatialCrossMapLRN', 'nn.Module')
...
function LRN:resetDescriptors()
   ...
   self.LRNDesc = ffi.new('struct lwdnnLRNStruct*[1]')
   errcheck('lwdnnCreateLRNDescriptor', self.LRNDesc)
   errcheck('lwdnnSetLRNDescriptor', self.LRNDesc[0], self.size,
            self.alpha, self.beta, self.k);
   local function destroyDesc(d)
      errcheck('lwdnnDestroyLRNDescriptor', d[0]);
   end
...
function LRN:updateOutput(input)
   ...
   errcheck('lwdnnLRNCrossChannelForward', lwdnn.getHandle(),
            self.LRNDesc[0],
...
function LRN:updateGradInput(input, gradOutput)
   ...
   if not self.LRNDesc then self:resetDescriptors() end
   self:createIODescriptors(input)
   errcheck('lwdnnLRNCrossChannelBackward',
            lwdnn.getHandle(), self.LRNDesc[0],
            'LWDNN_LRN_CROSS_CHANNEL_DIM1',
            lwdnn.scalar(input, 1),
            self.iDesc[0], self.output:data(),
