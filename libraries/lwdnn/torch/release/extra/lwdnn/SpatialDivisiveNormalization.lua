local DivisiveNorm, parent = torch.class('lwdnn.SpatialDivisiveNormalization', 'nn.Module')
...
function DivisiveNorm:resetDescriptors()
   ...
   self.DivisiveNormDesc = ffi.new('struct lwdnnDivisiveNormDescriptor_t*[1]')
   errcheck('lwdnnCreateDivisiveNormDescriptor', self.DivisiveNormDesc)
   errcheck('lwdnnSetDivisiveNormDescriptor', self.DivisiveNormDesc[0], self.size,
            self.alpha, self.beta, self.K);
   local function destroyDesc(d)
      errcheck('lwdnnDestroyDivisiveNormDescriptor', d[0]);
   end
...
function DivisiveNorm:updateOutput(input)
   ...
   errcheck('lwdnnDivisiveNormCrossChannelForward', lwdnn.getHandle(),
            self.DivisiveNormDesc[0],
            'LWDNN_DivisiveNorm_CROSS_CHANNEL_DIM1',
...
function DivisiveNorm:updateGradInput(input, gradOutput)
   ...
   if not self.DivisiveNormDesc then self:resetPoolDescriptors() end
   self:createIODescriptors(input)
   errcheck('lwdnnDivisiveNormCrossChannelBackward',
            lwdnn.getHandle(), self.DivisiveNormDesc[0],
            'LWDNN_DivisiveNorm_CROSS_CHANNEL_DIM1',
            lwdnn.scalar(input, 1),
            self.iDesc[0], self.output:data(),
