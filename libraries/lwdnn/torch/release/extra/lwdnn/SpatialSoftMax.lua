local SpatialSoftMax, parent = torch.class('lwdnn.SpatialSoftMax', 'nn.Module')
local errcheck = lwdnn.errcheck

function SpatialSoftMax:__init(fast)
   parent.__init(self)
   if fast then
      self.algorithm = 'LWDNN_SOFTMAX_FAST'
   else
      self.algorithm = 'LWDNN_SOFTMAX_ACLWRATE'
   end
end

function SpatialSoftMax:createIODescriptors(input)
   self.mode = self.mode or 'LWDNN_SOFTMAX_MODE_CHANNEL'
   -- after colwerting from nn use accurate
   self.algorithm = self.algorithm or 'LWDNN_SOFTMAX_ACLWRATE'
