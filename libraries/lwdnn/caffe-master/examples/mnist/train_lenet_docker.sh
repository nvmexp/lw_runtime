...
#
# The GPU-enabled version of Caffe can be used, assuming that lwpu-docker
# is installed, and the GPU-enabled Caffe image has been built.
# Setting the GPU environment variable to 1 will enable the use of lwpu-docker.
# e.g.
...
#
# For each of these, a step is exelwted to ensure that certain prerequisites
...
# variables can be used to controll the exelwtion of each of the phases:
#
...
# Check if GPU mode has been enabled and set the docker exelwtable accordingly
if [ ${GPU:-0} -eq 1 ]
then
DOCKER_CMD=lwpu-docker
IMAGE=caffe:gpu
else
DOCKER_CMD=docker
IMAGE=caffe:cpu
...
# Note:
#   -u $LWID:$CGID             runs the docker image as the current user to ensure
#                              that the file permissions are compatible with the
#                              host system. The variables LWID and CGID have been
...
#                              working directory
DOCKER_OPTIONS="--rm -ti -u $LWID:$CGID --volume=$(pwd):/workspace --workdir=/workspace"
