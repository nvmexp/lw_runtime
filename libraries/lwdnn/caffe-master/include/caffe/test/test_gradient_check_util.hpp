...
void GradientChecker<Dtype>::CheckGradientSingle(LayerBase* layer, const vector<Blob*>& bottom,
  ...
  for (int blob_id = 0; blob_id < blobs_to_check.size(); ++blob_id) {
    Blob* lwrrent_blob = blobs_to_check[blob_id];
    computed_gradient_blobs[blob_id].reset(new TBlob<Dtype>());
    computed_gradient_blobs[blob_id]->ReshapeLike(*lwrrent_blob);
  ...
  for (int blob_id = 0; blob_id < blobs_to_check.size(); ++blob_id) {
    ...
    for (int feat_id = 0; feat_id < lwrrent_blob->count(); ++feat_id) {
      float feature_val = lwrrent_blob->cpu_data<float>()[feat_id];
      ...
      if (!element_wise || (feat_id == top_data_id)) {
        ...
        lwrrent_blob->mutable_cpu_data<float>()[feat_id] = feature_val;
        Caffe::set_random_seed(seed_);
        layer->Forward(bottom, top);
      ...
      float feature = lwrrent_blob->cpu_data<float>()[feat_id];
