...
class DataReader : public InternalThread {
 private:
  class LwrsorManager {
   ...
   public:
    LwrsorManager(db::DB* db, DataReader* reader, size_t solver_count,
    ...
    DISABLE_COPY_MOVE_AND_ASSIGN(LwrsorManager);
  };
 ...
 public:
  ...
  bool cached_all() const {
    return lwrsors_cached_.load() == this->threads_num();
  }
 ...
 private:
  ...
  const bool epoch_count_required_;
  std::atomic_int lwrsors_cached_;
