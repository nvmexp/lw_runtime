 ...
 *        Instances can be colwerted to any other supported Type.
 *
...
class Blob {
 ...
 public:
  ...
  // Lwrrently it's used in tests only. We better keep it this way.
  ...
  // These ones are to be used with care: they don't colwert.
  void* lwrrent_mutable_data_memory(bool is_gpu, bool flush = true) {
    return data_tensor_->lwrrent_mutable_memory(is_gpu, flush);
  }

  void* lwrrent_mutable_diff_memory(bool is_gpu, bool flush = true) {
    return diff_tensor_->lwrrent_mutable_memory(is_gpu, flush);
  }

  const void* lwrrent_data_memory(bool is_gpu) const {
    return data_tensor_->lwrrent_memory(is_gpu);
  }

  const void* lwrrent_diff_memory(bool is_gpu) const {
    return diff_tensor_->lwrrent_memory(is_gpu);
  }
  ...
  void set_value_at(bool set_data, int idx, Dtype val) {
    void* ptr = set_data ? lwrrent_mutable_data_memory(false, false) :
                lwrrent_mutable_diff_memory(false, false);
    CHECK_NOTNULL(ptr);
 ...
 protected:
  ...
  bool is_lwrrent_data_valid() const {
    return data_tensor_->is_lwrrent_valid();
  }

  bool is_lwrrent_diff_valid() const {
    return diff_tensor_->is_lwrrent_valid();
  }

  void colwert_data(Type new_data_type) const {
    data_tensor_->colwert(new_data_type);
  }

  void colwert_diff(Type new_diff_type) const {
    diff_tensor_->colwert(new_diff_type);
  }
  ...
  static void check_integrity(bool do_data, Type lwrrent_type, Type new_type) {
    CHECK_EQ(lwrrent_type, new_type)
      << "Attempt to change TBlob native " << (do_data ? "data" : "diff")
      << " type from " << Type_Name(lwrrent_type) << " to " << Type_Name(new_type);
  }
};  // class Blob
...
class TBlob : public Blob {
 public:
  ...
  // in this case. These are the shortlwts allowing to keep current
