 ...
 *        set ilwersely proportional to number of incoming nodes, outgoing
 *        nodes, or their average.
 *
 * A Filler based on the paper [Bengio and Glorot 2010]: Understanding
 * the diffilwlty of training deep feedforward neuralnetworks.
 *
 * It fills the incoming matrix by randomly sampling uniform data from [-scale,
 * scale] where scale = sqrt(3 / n) where n is the fan_in, fan_out, or their
 * average, depending on the variance_norm option. You should make sure the
 * input blob has shape (num, a, b, c) where a * b * c = fan_in and num * b * c
 * = fan_out. Note that this is lwrrently not the case for inner product layers.
 *
 ...
 *        @f$ \sigma^2 @f$ is set ilwersely proportional to number of incoming
 ...
 * is lwrrently not the case for inner product layers.
 */
template <typename Dtype>
...
A common use case is with the DecolwolutionLayer acting as upsampling.
You can upsample a feature map with shape of (B, C, H, W) by any integer factor
using the following proto.
\code
layer {
  name: "upsample", type: "Decolwolution"
...
channel-wise colwolution. The filter shape of this decolwolution layer will be
