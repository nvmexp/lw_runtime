...
template <typename T>
void clean_last_element(T* x, lwdaStream_t stream) {
  LWDA_CHECK(lwdaMemsetAsync(x, 0, sizeof(T), stream));
...
//   copying that file in colwenient for code reviewing.
...
// The extra parens are needed because LWCA < 6.5 defines signbit as a macro,
// and we don't want that to expand here when LWCA headers are also included.
...
inline void caffe_gpu_memset(const size_t N, const int alpha, void* X, int group = 0) {
  ...
  LWDA_CHECK(lwdaMemsetAsync(X, alpha, N, stream));  // NOLINT(caffe/alt_fn)
  LWDA_CHECK(lwdaStreamSynchronize(stream));
}
...
template <typename Dtype>
void caffe_gpu_scal(const int N, const Dtype alpha, Dtype* X, lwblasHandle_t lwblas_handle);
...
// specification of lwrandGenerateUniform.  With a = 0, b = 1, just calls
// lwrandGenerateUniform; with other limits will shift and scale the outputs
// appropriately after calling lwrandGenerateUniform.
template <typename Dtype>
...
void caffe_gpu_colwert(const unsigned int n, const T_IN* in, T_OUT* out);
...
void caffe_gpu_##name<float>(const int n, const float* x, float* y) { \
  ...
  name##_kernel<float><<<CAFFE_GET_BLOCKS(n), CAFFE_LWDA_NUM_THREADS, 0, stream>>>(n, x, y); \
  LWDA_CHECK(lwdaStreamSynchronize(stream)); \
} \
template <> \
void caffe_gpu_##name<double>(const int n, const double* x, double* y) { \
  lwdaStream_t stream = Caffe::thread_stream(); \
  /* NOLINT_NEXT_LINE(whitespace/operators) */ \
  name##_kernel<double><<<CAFFE_GET_BLOCKS(n), CAFFE_LWDA_NUM_THREADS, 0, stream>>>(n, x, y); \
  LWDA_CHECK(lwdaStreamSynchronize(stream)); \
} \
template <> \
void caffe_gpu_##name<float16>(const int n, const float16* x, float16* y) { \
  lwdaStream_t stream = Caffe::thread_stream(); \
  /* NOLINT_NEXT_LINE(whitespace/operators) */ \
  name##_kernel<float16><<<CAFFE_GET_BLOCKS(n), CAFFE_LWDA_NUM_THREADS, 0, stream>>>(n, x, y); \
  LWDA_CHECK(lwdaStreamSynchronize(stream)); \
}
...
void caffe_gpu_##name<float>(const int n, const float* x, float* y, void* handle) { \
  lwblasHandle_t lwblas_handle = \
      handle == nullptr ? Caffe::lwblas_handle() : reinterpret_cast<lwblasHandle_t>(handle); \
  lwdaStream_t stream; \
...
template <typename T_IN, typename T_OUT>
inline void caffe_cpu_colwert(const int n, const T_IN *in, T_OUT *out) {
  for (int i = 0; i < n; ++i) {
...
template <typename T_IN, typename T_OUT>
inline void caffe_colwert(bool use_gpu, const int n, const T_IN* in, T_OUT* out) {
  if (use_gpu) {
    caffe_gpu_colwert(n, in, out);
  } else {
    caffe_cpu_colwert(n, in, out);
  }
}
