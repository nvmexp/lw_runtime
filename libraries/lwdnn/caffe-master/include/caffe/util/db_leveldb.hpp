...
class LevelDBLwrsor : public Cursor {
 public:
  explicit LevelDBLwrsor(leveldb::Iterator* iter)
    : iter_(iter) { SeekToFirst(); }
  ~LevelDBLwrsor() { delete iter_; }
...
class LevelDB : public DB {
 public:
  ...
  virtual LevelDBLwrsor* NewLwrsor() {
    return new LevelDBLwrsor(db_->NewIterator(leveldb::ReadOptions()));
  }
