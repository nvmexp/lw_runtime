 ...
 * toggle whether the network is aclwmulating or using the statistics via the
 ...
 * scaling factor.  It is possible (though a bit lwmbersome) to implement
 * this in caffe using a single-channel DummyDataLayer filled with zeros,
 * followed by a Colwolution layer with output the same size as the current.
...
class BatchNormLayer : public Layer<Ftype, Btype> {
 ...
 protected:
  ...
  shared_ptr<Blob> mean_, var_, ilw_var_, x_norm_;
