...
class SyncedMemory {
 public:
  explicit SyncedMemory(size_t size = 0UL)
      : cpu_ptr_(nullptr), gpu_ptr_(nullptr), size_(size), head_(UNINITIALIZED),
        own_cpu_data_(false), cpu_malloc_use_lwda_(false), own_gpu_data_(false),
 ...
 protected:
  void MallocHost(void** ptr, size_t size, bool* use_lwda);
  void FreeHost(void* ptr, bool use_lwda);
