...
be mapped output-to-input (backward) by the ilwerse mapping too.
...
def colw_params(fn):
    ...
    Implementation detail: Colwolution, Decolwolution, and Im2col layers
    define these in the colwolution_param message, while Pooling has its
    own fields in pooling_param. This method deals with these details to
    extract canonical parameters.
    """
    params = fn.params.get('colwolution_param', fn.params)
...
def coord_map(fn):
    ...
    """
    if fn.type_name in ['Colwolution', 'Pooling', 'Im2col']:
        axis, stride, ks, pad = colw_params(fn)
        return axis, 1 / stride, (pad - (ks - 1) / 2) / stride
    elif fn.type_name == 'Decolwolution':
        axis, stride, ks, pad = colw_params(fn)
...
def ilwerse(coord_map):
    """
    Ilwert a coord map by de-scaling and un-shifting;
...
def coord_map_from_to(top_from, top_to):
    ...
    from and to until they meet. As a last step the from map is ilwerted.
    """
    ...
    while frontier:
        top = frontier.pop()
        if top in from_maps:
            return compose(to_maps[top], ilwerse(from_maps[top]))
        try:
            bottoms = collect_bottoms(top)
            for bottom in bottoms:
