...
Lwrrently this works as a thin wrapper around the Python protobuf interface,
...
def assign_proto(proto, name, val):
    ...
    type (in relwrsive fashion). Lists become repeated fields/messages, dicts
    become messages, and other types are assigned directly. For colwenience,
    repeated fields whose values are not lists are colwerted to single-element
    lists; e.g., `my_repeated_int_field=3` is colwerted to
...
class Layers(object):
    ...
    layers; e.g., Layers().Colwolution(bottom, kernel_size=3) will produce a Top
    specifying a 3x3 colwolution applied to bottom."""
