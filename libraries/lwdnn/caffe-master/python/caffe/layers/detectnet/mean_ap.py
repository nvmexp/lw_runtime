...
class mAP(caffe.Layer):
    """
    * callwlates precision, recall, mean average precision from marked up bbox list
...
def score_det(gt_bbox_list, det_bbox_list):
    ...
    for k in range(gt_bbox_list.shape[0]):
        ...
        lwr_det_bbox = det_bbox_list[k, :, 0:4]
        lwr_det_bbox = np.asarray(list(filter(lambda a: a.tolist() != [0, 0, 0, 0], lwr_det_bbox)))

        # Remove  zeros from label bboxes
        lwr_gt_bbox = gt_bbox_list[k, :, 0:4]
        lwr_gt_bbox = np.asarray(list(filter(lambda a: a.tolist() != [0, 0, 0, 0], lwr_gt_bbox)))

        gt_matched = np.zeros([lwr_gt_bbox.shape[0]])
        det_matched = np.zeros([lwr_det_bbox.shape[0]])

        for i in range(lwr_gt_bbox.shape[0]):
            for j in range(lwr_det_bbox.shape[0]):
                if (iou(lwr_det_bbox[j], lwr_gt_bbox[i]) >= threshold) and (det_matched[j] == 0):
                    gt_matched[i] = 1
                    det_matched[j] = 1
                    break

        tp = np.asarray([np.append(j, 1) for j in lwr_det_bbox[np.where(det_matched == 1)]])
        fp = np.asarray([np.append(j, 2) for j in lwr_det_bbox[np.where(det_matched == 0)]])
        tn = np.asarray([np.append(j, 3) for j in lwr_gt_bbox[np.where(gt_matched == 0)]])
