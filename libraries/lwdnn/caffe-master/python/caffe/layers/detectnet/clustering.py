...
class ClusterGroundtruth(caffe.Layer):
    """
    * colwerts ground truth labels from grid format to list
...
class ClusterDetections(caffe.Layer):
    """
    * colwert network output in grid format to list using group rectangles clustering
...
def cluster(self, net_cvg, net_boxes):
    ...
    for i in range(batch_size):
        ...
        lwr_cvg = net_cvg[i]
        lwr_boxes = net_boxes[i]

        if (self.is_groundtruth):
            # Gather proposals that pass a threshold -
            propose_boxes, propose_cvgs, mask = gridbox_to_boxes(
                lwr_cvg, lwr_boxes, self)
            # Remove duplicates from ground truth
            new_array = list({tuple(row) for row in propose_boxes})
            boxes_lwr_image = np.asarray(new_array, dtype=np.float16)
        else:
            # Gather proposals that pass a threshold -
            propose_boxes, propose_cvgs, mask = gridbox_to_boxes(lwr_cvg, lwr_boxes, self)
            # Vote across the proposals to get bboxes
            boxes_lwr_image = vote_boxes(propose_boxes, propose_cvgs, mask, self)
            boxes_lwr_image = np.asarray(boxes_lwr_image, dtype=np.float16)

        if (boxes_lwr_image.shape[0] != 0):
            [r, c] = boxes_lwr_image.shape
            boxes[i, 0:r, 0:c] = boxes_lwr_image
