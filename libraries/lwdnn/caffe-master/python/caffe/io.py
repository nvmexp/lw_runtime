...
## proto / datum / ndarray colwersion
def blobproto_to_array(blob, return_diff=False):
    """
    Colwert a blob proto to an array. In default, we will just return the data,
...
def array_to_blobproto(arr, diff=None):
    """Colwerts a N-dimensional array to blob proto. If diff is given, also
    colwert the diff. You need to make sure that arr and diff have the same
...
def arraylist_to_blobprotovecor_str(arraylist):
    """Colwerts a list of arrays to a serialized blobprotovec, which could be
...
def blobprotovector_str_to_arraylist(str):
    """Colwerts a serialized blobprotovec to a list of arrays.
    """
    vec = caffe_pb2.BlobProtoVector()
...
def array_to_datum(arr, label=None):
    """Colwerts a 3-dimensional array to datum. If the array has dtype uint8,
...
def datum_to_array(datum):
    """Colwerts a datum to an array. Note that the label is not returned,
...
class Transformer:
    ...
    def deprocess(self, in_, data):
        """
        Ilwert Caffe formatting; see preprocess().
        """
        self.__check_input(in_)
    ...
    def set_transpose(self, in_, order):
        """
        Set the input channel order for e.g. RGB to BGR colwersion
...
def load_image(filename, color=True):
    """
    Load an image colwerting from grayscale or alpha as needed.
...
def oversample(images, crop_dims):
    ...
    for i in h_indices:
        for j in w_indices:
            crops_ix[lwrr] = (i, j, i + crop_dims[0], j + crop_dims[1])
            lwrr += 1
