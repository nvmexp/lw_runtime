...
class Detector(caffe.Net):
    ...
    def configure_crop(self, context_pad):
        ...
        ilw_tpose = [tpose[t] for t in tpose]
        self.crop_dims = np.array(self.blobs[in_].data.shape[1:])[ilw_tpose]
        #.transpose(ilw_tpose)
        # context padding
        ...
        if self.context_pad:
            ...
            if mean is not None:
                ilw_transpose = [transpose[t] for t in transpose]
                crop_mean = mean.copy().transpose(ilw_transpose)
                if channel_order is not None:
                    channel_order_ilwerse = [channel_order.index(i)
                                             for i in range(crop_mean.shape[2])]
                    crop_mean = crop_mean[:, :, channel_order_ilwerse]
                if raw_scale is not None:
                    crop_mean /= raw_scale
