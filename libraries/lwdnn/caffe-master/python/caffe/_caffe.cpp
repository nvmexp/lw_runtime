...
// Hack to colwert macro to string
...
#define BP_REGISTER_SHARED_PTR_TO_PYTHON(PTR) do { \
  ...
  const boost::python::colwerter::registration* reg = \
    boost::python::colwerter::registry::query(info); \
  if (reg == NULL) { \
...
void initialize_gpu_memory_scope(const vector<int>& gpus) {
  ...
  if (!google_initialized) {
    ...
    google::InitGoogleLogging("pyLWCaffe");
  }
  if (!gpu_memory_scope) {
...
// For colwenience, check that input files can be opened, and raise an
...
shared_ptr<Net> Net_Init(string param_file, int phase,
  ...
  // Colwert stages from list to vector
...
// Net construct-and-load colwenience constructor
...
struct NdarrayColwerterGenerator {
  template <typename T> struct apply;
};

template <>
struct NdarrayColwerterGenerator::apply<Dtype*> {
  struct type {
...
struct NdarrayCallPolicies : public bp::default_call_policies {
  typedef NdarrayColwerterGenerator result_colwerter;
...
BOOST_PYTHON_MODULE(_caffe) {
  ...
  bp::class_<Net, shared_ptr<Net>, boost::noncopyable >("Net",
    ...
    .def("_set_input_arrays", &Net_SetInputArrays,
        bp::with_lwstodian_and_ward<1, 2, bp::with_lwstodian_and_ward<1, 3> >())
    .def("save", &Net_Save);
