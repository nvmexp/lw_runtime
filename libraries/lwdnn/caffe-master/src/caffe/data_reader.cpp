...
template<typename DatumType>
DataReader<DatumType>::LwrsorManager::LwrsorManager(db::DB* db, DataReader<DatumType>* reader,
...
template<typename DatumType>
DataReader<DatumType>::LwrsorManager::~LwrsorManager() {
  lwrsor_.reset();
  db_->Close();
}

template<typename DatumType>
void DataReader<DatumType>::LwrsorManager::next(shared_ptr<DatumType>& datum) {
  if (cached_all_) {
...
*/
template<typename DatumType>
void DataReader<DatumType>::LwrsorManager::rewind() {
...
template<>
void DataReader<Datum>::LwrsorManager::fetch(Datum* datum) {
  C2TensorProtos protos;
  if (lwrsor_->parse(&protos) && protos.protos_size() >= 2) {
  ...
  } else if (!lwrsor_->parse(datum)) {
    LOG(ERROR) << "Database cursor failed to parse Datum record";
  }
  // DLOG(INFO) << lwrsor_->key() << " " << datum->label();
}

template<>
void DataReader<AnnotatedDatum>::LwrsorManager::fetch(AnnotatedDatum* datum) {
  if (!lwrsor_->parse(datum)) {
    LOG(ERROR) << "Database cursor failed to parse Datum record";
  }
}
