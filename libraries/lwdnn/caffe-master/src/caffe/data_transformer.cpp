...
void DataTransformer<Dtype>::apply_mean_scale_mirror(const cv::Mat& src, cv::Mat& dst) const {
  ...
  if (has_mean_file) {
    ...
    if (src.rows != mean_mat_.rows || src.cols != mean_mat_.cols) {
      ...
      // scale & colwert in place
      mean_mat_.colwertTo(mean_mat_, CVFC<float>(ch), scale);
    }
  ...
  src.colwertTo(tmp_, CVFC<float>(ch), scale);  // scale & colwert
  dst = tmp_;
...
void DataTransformer<Dtype>::Copy(const Datum& datum, Dtype* data, size_t& out_sizeof_element) {
  ...
  LWDA_CHECK(lwdaMemcpyAsync(data, src_ptr, N * out_sizeof_element,
...
template<typename Dtype>
void DataTransformer<Dtype>::TransformIlw(const Dtype* data, cv::Mat* cv_img,
...
template<typename Dtype>
void DataTransformer<Dtype>::TransformIlw(const Blob* blob,
  ...
  for (int i = 0; i < num; ++i) {
    cv::Mat cv_img;
    TransformIlw(image_data, &cv_img, height, width, channels);
...
void DataTransformer<Dtype>::Transform(const Datum& datum,
  ...
  if (has_uint8) {
    Dtype datum_element, mlw;

    if (scale == 1.F) {
      for (int c = 0; c < datum_channels; ++c) {
        cdho = c * datum_height + h_off;
        ch = c * height;
        mlw = has_mean_values && !has_mean_file ? mean_values_[c] : 0.F;
        for (int h = 0; h < height; ++h) {
          ...
          for (int w = 0; w < width; ++w) {
            ...
            if (has_mean_file) {
              ...
              if (has_mean_values) {
                transformed_data[top_index] = datum_element - mlw;
              } else {
        ...
        for (int h = 0; h < height; ++h) {
          ...
          for (int w = 0; w < width; ++w) {
            ...
            if (has_mean_file) {
              ...
              if (has_mean_values) {
                transformed_data[top_index] = (datum_element - mlw) * scale;
              } else {
                transformed_data[top_index] = datum_element * scale;
              }
            }
        ...
        for (int h = 0; h < height; ++h) {
          ...
          for (int w = 0; w < width; ++w) {
            ...
            if (has_mean_file) {
              ...
              if (has_mean_values) {
                buf[top_index] = datum_element - mlw;
              } else {
        ...
        for (int h = 0; h < height; ++h) {
          ...
          for (int w = 0; w < width; ++w) {
            ...
            if (has_mean_file) {
              ...
              if (has_mean_values) {
                buf[top_index] = (datum_element - mlw) * scale;
              } else {
