 ...
 protected:
  ...
  string RunLeastSquaresSolver(const float learning_rate,
    ...
    if (Caffe::mode() == Caffe::GPU) {
      LWDA_CHECK(lwdaGetDevice(&device_id));
    }
    proto <<
  ...
  void CheckAclwmulation(const float kLearningRate, const float kWeightDecay,
    ...
    // Solve without aclwmulation and save parameters.
    ...
    const vector<shared_ptr<Blob> >& param_blobs =
        net.layer_by_name("innerprod")->blobs();
    vector<shared_ptr<TBlob<double> > > noaclwm_params(param_blobs.size());
    for (int i = 0; i < param_blobs.size(); ++i) {
      noaclwm_params[i].reset(new TBlob<double>());
      noaclwm_params[i]->CopyFrom(*param_blobs[i], false, true);
    }
    // Solve by equivalent aclwmulation of gradients over divided batches.
    this->RunLeastSquaresSolver(kLearningRate, kWeightDecay, kMomentum,
        kNumIters, kIterSize);
    Net& net_aclwm = *this->solver_->net();
    const vector<shared_ptr<Blob> >& aclwm_params =
        net_aclwm.layer_by_name("innerprod")->blobs();
    // Compare aclwmulated parameters against no aclwmulation standard.
    const int D = this->channels_ * this->height_ * this->width_;
    for (int i = 0; i < D; ++i) {
      const double expected_param = noaclwm_params[0]->cpu_data()[i];
      const double aclwm_param = aclwm_params[0]->cpu_data<double>()[i];
      const double error_margin = std::max(kMinPrecision, kPrecision *
          std::min(fabs(expected_param), fabs(aclwm_param)));
      EXPECT_NEAR(expected_param, aclwm_param, error_margin);
    }
    ASSERT_EQ(1, aclwm_params[1]->count());
    const double expected_bias = noaclwm_params[1]->cpu_data()[0];
    const double aclwm_bias = aclwm_params[1]->cpu_data<double>()[0];
    const double error_margin = std::max(kMinPrecision, kPrecision *
        std::min(fabs(expected_bias), fabs(aclwm_bias)));
    EXPECT_NEAR(expected_bias, aclwm_bias, error_margin);
  }
  ...
  void TestLeastSquaresUpdate(const float learning_rate = 1.0,
    ...
    if (Caffe::mode() == Caffe::GPU) {
      LWDA_CHECK(lwdaGetDeviceCount(&available_devices));
    }
    for (int devices = 1; devices <= available_devices; ++devices) {
      // Configure batch size for single / multi device equivalence.
      // Constant data is needed for multi device as for aclwmulation.
...
TYPED_TEST(SGDSolverTest, TestLeastSquaresUpdateWithEverythingAclwm) {
  const float kLearningRate = 0.01;
  const float kWeightDecay = 0.5;
  const float kMomentum = 0.9;
  const int kNumIters = 4;
  const int kIterSize = 2;
  this->CheckAclwmulation(kLearningRate, kWeightDecay, kMomentum, kNumIters,
      kIterSize);
}

TYPED_TEST(SGDSolverTest, TestLeastSquaresUpdateWithEverythingAclwmShare) {
  const float kLearningRate = 0.01;
  const float kWeightDecay = 0.5;
  const float kMomentum = 0.9;
  const int kNumIters = 4;
...
TYPED_TEST(AdaGradSolverTest, TestLeastSquaresUpdateWithEverythingAclwm) {
  const float kLearningRate = 0.01;
  const float kWeightDecay = 0.5;
  const float kMomentum = 0;
  const int kNumIters = 4;
...
TYPED_TEST(AdaGradSolverTest, TestLeastSquaresUpdateWithEverythingAclwmShare) {
  const float kLearningRate = 0.01;
  const float kWeightDecay = 0.5;
  const float kMomentum = 0;
  const int kNumIters = 4;
...
TYPED_TEST(NesterovSolverTest, TestLeastSquaresUpdateWithEverythingAclwm) {
  const float kLearningRate = 0.01;
  const float kWeightDecay = 0.5;
  const float kMomentum = 0.9;
  const int kNumIters = 4;
...
TYPED_TEST(NesterovSolverTest, TestLeastSquaresUpdateWithEverythingAclwmShare) {
  const float kLearningRate = 0.01;
  const float kWeightDecay = 0.5;
  const float kMomentum = 0.9;
  const int kNumIters = 4;
...
TYPED_TEST(AdaDeltaSolverTest, TestLeastSquaresUpdateWithEverythingAclwm) {
  const float kLearningRate = 0.1;
  const float kWeightDecay = 0.1;
  const float kMomentum = 0.95;
  const int kNumIters = 4;
...
TYPED_TEST(AdaDeltaSolverTest, TestLeastSquaresUpdateWithEverythingAclwmShare) {
  const float kLearningRate = 0.1;
  const float kWeightDecay = 0.1;
  const float kMomentum = 0.95;
  const int kNumIters = 4;
...
TYPED_TEST(AdamSolverTest, TestLeastSquaresUpdateWithEverythingAclwm) {
  const float kLearningRate = 0.01;
  const float kWeightDecay = 0.5;
  const float kMomentum = 0.9;
  const int kNumIters = 4;
...
TYPED_TEST(AdamSolverTest, TestLeastSquaresUpdateWithEverythingAclwmShare) {
  const float kLearningRate = 0.01;
  const float kWeightDecay = 0.5;
  const float kMomentum = 0.9;
  const int kNumIters = 4;
...
TYPED_TEST(RMSPropSolverTest, TestLeastSquaresUpdateWithEverythingAclwm) {
  const float kLearningRate = 0.01;
  const float kWeightDecay = 0.5;
  const float kMomentum = 0.0;
  const int kNumIters = 4;
...
TYPED_TEST(RMSPropSolverTest, TestLeastSquaresUpdateWithEverythingAclwmShare) {
  const float kLearningRate = 0.01;
  const float kWeightDecay = 0.5;
  const float kMomentum = 0.0;
  const int kNumIters = 4;
