...
class LwDNNSoftmaxLayerTest : public GPUDeviceTest<Dtype> {
 protected:
  LwDNNSoftmaxLayerTest() : blob_bottom_(new TBlob<Dtype>(2, 10, 2, 3)),
...
TYPED_TEST_CASE(LwDNNSoftmaxLayerTest, TestDtypes);

TYPED_TEST(LwDNNSoftmaxLayerTest, TestForwardLwDNN) {
  LayerParameter layer_param;
  ...
  LwDNNSoftmaxLayer<TypeParam, TypeParam> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  // Test sum
  for (int i = 0; i < this->blob_bottom_->num(); ++i) {
...
TYPED_TEST(LwDNNSoftmaxLayerTest, TestGradientLwDNN) {
  LayerParameter layer_param;
  layer_param.set_forward_type(tp<TypeParam>());
  layer_param.set_backward_type(tp<TypeParam>());
  layer_param.set_forward_math(tp<TypeParam>());
