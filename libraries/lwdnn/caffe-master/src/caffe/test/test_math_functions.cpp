...
TYPED_TEST(GPUMathFunctionsTest, TestHistogram) {
  ...
  hg.Reshape(vector<int>{CAFFE_LWDA_NUM_THREADS});
  ...
  double modev = 0.;
  unsigned mode = 0U;
  for (unsigned int i = 0; i < CAFFE_LWDA_NUM_THREADS; ++i) {
  ...
  EXPECT_EQ(CAFFE_LWDA_NUM_THREADS / 2, mode);
}
#endif
...
TYPED_TEST(GPUMathFunctionsTest, TestExtFP16) {
  ...
  caffe_gpu_colwert(n, xgpu, fp16gpu);
  ...
  caffe_gpu_colwert(n, fp16gpu, x2gpu);
...
TYPED_TEST(GPUMathFunctionsTest, TestColwertToAndFrom16) {
  const int n = this->blob_bottom_->count();
  ...
  caffe_gpu_colwert(n, bottom_data, b16_data);
  ...
  TypeParam* top_data = this->blob_top_->mutable_gpu_data();
  caffe_gpu_colwert(n, b16_data, top_data);
