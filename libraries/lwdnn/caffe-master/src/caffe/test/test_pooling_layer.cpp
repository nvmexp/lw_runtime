...
class LwDNNPoolingLayerTest : public GPUDeviceTest<Dtype> {
 protected:
  LwDNNPoolingLayerTest() : blob_bottom_(new TBlob<Dtype>()), blob_top_(new TBlob<Dtype>()),
                            blob_top_mask_(new TBlob<Dtype>()) {}
  ...
  void TestForwardSquare() {
    ...
    LwDNNPoolingLayer<Dtype, Dtype> layer(layer_param);
    layer.SetUp(blob_bottom_vec_, blob_top_vec_);
    EXPECT_EQ(blob_top_->num(), num);
    EXPECT_EQ(blob_top_->channels(), channels);
    EXPECT_EQ(blob_top_->height(), 2);
...
TYPED_TEST_CASE(LwDNNPoolingLayerTest, TestDtypes);

TYPED_TEST(LwDNNPoolingLayerTest, TestSetupLwDNN) {
  LayerParameter layer_param;
  ...
  pooling_param->set_kernel_size(3);
  pooling_param->set_stride(2);
  LwDNNPoolingLayer<TypeParam, TypeParam> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  EXPECT_EQ(this->blob_top_->num(), this->blob_bottom_->num());
  EXPECT_EQ(this->blob_top_->channels(), this->blob_bottom_->channels());
  EXPECT_EQ(this->blob_top_->height(), 3);
...
TYPED_TEST(LwDNNPoolingLayerTest, TestSetupPaddedLwDNN) {
  LayerParameter layer_param;
  layer_param.set_forward_type(tp<TypeParam>());
  layer_param.set_backward_type(tp<TypeParam>());
  layer_param.set_forward_math(tp<TypeParam>());
...
/*
TYPED_TEST(LwDNNPoolingLayerTest, PrintBackwardLwDNN) {
  LayerParameter layer_param;
  layer_param.set_kernelsize(3);
  layer_param.set_stride(2);
  layer_param.set_pool(LayerParameter_PoolMethod_MAX);
...
TYPED_TEST(LwDNNPoolingLayerTest, TestForwardMaxLwDNN) {
  this->TestForwardSquare();
  this->TestForwardRectHigh();
  this->TestForwardRectWide();
}

// Lwrrently, lwDNN does not support a top mask, so we comment this and
// the corresponding backward test.
/*
TYPED_TEST(LwDNNPoolingLayerTest, TestForwardMaxTopMaskLwDNN) {
  this->blob_top_vec_.push_back(this->blob_top_mask_);
  this->TestForwardSquare();
  this->TestForwardRectHigh();
  this->TestForwardRectWide();
...
TYPED_TEST(LwDNNPoolingLayerTest, TestGradientMaxLwDNN) {
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      LayerParameter layer_param;
      layer_param.set_forward_type(tp<TypeParam>());
      ...
      pooling_param->set_kernel_w(kernel_w);
      pooling_param->set_stride(2);
      // lwrrenty, lwDNN pooling does not support padding
...
TYPED_TEST(LwDNNPoolingLayerTest, TestForwardMaxPaddedLwDNN) {
  LayerParameter layer_param;
  layer_param.set_forward_type(tp<TypeParam>());
  layer_param.set_backward_type(tp<TypeParam>());
  layer_param.set_forward_math(tp<TypeParam>());
...
/*
TYPED_TEST(LwDNNPoolingLayerTest, TestGradientMaxTopMaskLwDNN) {
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      LayerParameter layer_param;
      PoolingParameter* pooling_param = layer_param.mutable_pooling_param();
...
TYPED_TEST(LwDNNPoolingLayerTest, TestForwardAveLwDNN) {
  LayerParameter layer_param;
  layer_param.set_forward_type(tp<TypeParam>());
  layer_param.set_backward_type(tp<TypeParam>());
  layer_param.set_forward_math(tp<TypeParam>());
  ...
  pooling_param->set_kernel_size(3);
  pooling_param->set_stride(1);
  // Lwrrently, lwDNN pooling does not support padding, so we use
...
TYPED_TEST(LwDNNPoolingLayerTest, TestGradientAveLwDNN) {
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      LayerParameter layer_param;
      layer_param.set_forward_type(tp<TypeParam>());
...
TYPED_TEST(LwDNNPoolingLayerTest, TestGradientAvePaddedLwDNN) {
  for (int kernel_h = 3; kernel_h <= 4; kernel_h++) {
    for (int kernel_w = 3; kernel_w <= 4; kernel_w++) {
      LayerParameter layer_param;
      layer_param.set_forward_type(tp<TypeParam>());
