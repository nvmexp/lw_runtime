...
class LwDNNBatchNormLayerTest : public GPUDeviceTest<Dtype> {
 protected:
  LwDNNBatchNormLayerTest()
  ...
  virtual ~LwDNNBatchNormLayerTest() { delete blob_bottom_; delete blob_top_; }
  void checkMealwar(const TBlob<Dtype> *blob_bottom, int num,
...
template <typename TypeParam>
void LwDNNBatchNormLayerTest<TypeParam>::checkMealwar(
...
TYPED_TEST_CASE(LwDNNBatchNormLayerTest, TestDtypes);

TYPED_TEST(LwDNNBatchNormLayerTest, TestForward) {
  ...
  LwDNNBatchNormLayer<Dtype, Dtype> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  layer.Reshape(this->blob_bottom_vec_, this->blob_top_vec_);
  layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  ...
  this->checkMealwar(this->blob_top_, num, channels, height, width);
}

TYPED_TEST(LwDNNBatchNormLayerTest, TestGradient) {
  typedef TypeParam Dtype;
  LayerParameter layer_param;
  layer_param.set_forward_type(tp<Dtype>());
  layer_param.set_backward_type(tp<Dtype>());
