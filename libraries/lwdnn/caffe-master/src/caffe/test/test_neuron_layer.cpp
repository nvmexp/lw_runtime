...
class LwDNNNeuronLayerTest : public GPUDeviceTest<Dtype> {
 protected:
  LwDNNNeuronLayerTest() : blob_bottom_(new TBlob<Dtype>(2, 3, 4, 5)),
...
TYPED_TEST_CASE(LwDNNNeuronLayerTest, TestDtypes);

TYPED_TEST(LwDNNNeuronLayerTest, TestReLULwDNN) {
  LayerParameter layer_param;
  LwDNNReLULayer<TypeParam, TypeParam> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  // Now, check values
...
TYPED_TEST(LwDNNNeuronLayerTest, TestReLUGradientLwDNN) {
  LayerParameter layer_param;
  LwDNNReLULayer<TypeParam, TypeParam> layer(layer_param);
...
TYPED_TEST(LwDNNNeuronLayerTest, TestReLUWithNegativeSlopeLwDNN) {
  LayerParameter layer_param;
  CHECK(google::protobuf::TextFormat::ParseFromString("relu_param { negative_slope: 0.01 }",
      &layer_param));
  LwDNNReLULayer<TypeParam, TypeParam> layer(layer_param);
...
TYPED_TEST(LwDNNNeuronLayerTest, TestReLUGradientWithNegativeSlopeLwDNN) {
  LayerParameter layer_param;
  CHECK(google::protobuf::TextFormat::ParseFromString("relu_param { negative_slope: 0.01 }",
      &layer_param));
  LwDNNReLULayer<TypeParam, TypeParam> layer(layer_param);
...
TYPED_TEST(LwDNNNeuronLayerTest, TestSigmoidLwDNN) {
  LayerParameter layer_param;
  LwDNNSigmoidLayer<TypeParam, TypeParam> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  // Now, check values
  const TypeParam* bottom_data = this->blob_bottom_->cpu_data();
...
TYPED_TEST(LwDNNNeuronLayerTest, TestSigmoidGradientLwDNN) {
  LayerParameter layer_param;
  LwDNNSigmoidLayer<TypeParam, TypeParam> layer(layer_param);
...
TYPED_TEST(LwDNNNeuronLayerTest, TestTanHLwDNN) {
  LayerParameter layer_param;
  LwDNNTanHLayer<TypeParam, TypeParam> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  // Test exact values
  for (int i = 0; i < this->blob_bottom_->num(); ++i) {
...
TYPED_TEST(LwDNNNeuronLayerTest, TestTanHGradientLwDNN) {
  LayerParameter layer_param;
  LwDNNTanHLayer<TypeParam, TypeParam> layer(layer_param);
...
TYPED_TEST(LwDNNNeuronLayerTest, TestDropoutHalf) {
  const float kDropoutRatio = 0.5;
  this->TestDropoutForward(kDropoutRatio);
}

TYPED_TEST(LwDNNNeuronLayerTest, TestDropoutThreeQuarters) {
  const float kDropoutRatio = 0.75;
  this->TestDropoutForward(kDropoutRatio);
}

TYPED_TEST(LwDNNNeuronLayerTest, TestDropoutTestPhase) {
  LayerParameter layer_param;
  layer_param.set_phase(TEST);
  LwDNNDropoutLayer<TypeParam, TypeParam> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  // Now, check values
  const TypeParam* bottom_data = this->blob_bottom_->cpu_data();
...
TYPED_TEST(LwDNNNeuronLayerTest, TestDropoutGradientTest) {
  LayerParameter layer_param;
  layer_param.set_phase(TEST);
  LwDNNDropoutLayer<TypeParam, TypeParam> layer(layer_param);
  GradientChecker<TypeParam> checker(tol<TypeParam>(1e-2, 1e-1), tol<TypeParam>(1e-3, 1e-1));
