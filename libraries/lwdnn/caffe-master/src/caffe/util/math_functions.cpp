...
void caffe_cpu_gemm<float16>(const CBLAS_TRANSPOSE TransA,
  ...
  caffe_cpu_colwert(a.size(), A, &a.front());
  caffe_cpu_colwert(b.size(), B, &b.front());
  caffe_cpu_colwert(c.size(), C, &c.front());
  ...
  caffe_cpu_colwert(c.size(), &c.front(), C);
}
...
void caffe_rng_gaussian(int n, Ftype mu, Ftype sigma, Blob* blob) {
  ...
  void* ptr = blob->lwrrent_mutable_data_memory(false, false);
