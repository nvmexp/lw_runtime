...
bool UpgradeV0Net(const NetParameter& v0_net_param_padding_layers,
  ...
  // First upgrade padding layers to padded colw layers.
  ...
  for (int i = 0; i < param.layers_size(); ++i) {
    ...
    for (int j = 0; j < layer_connection.bottom_size(); ++j) {
      ...
      if (source_layer.layer().type() == "padding") {
        // This layer has a padding layer as input -- check that it is a colw
        ...
        CHECK((layer_param.type() == "colw") || (layer_param.type() == "pool"))
            << "Padding layer input to "
            "non-colwolutional / non-pooling layer type "
            << layer_param.type();
        CHECK_EQ(layer_connection.bottom_size(), 1)
            << "Colw Layer takes a single blob as input.";
...
bool UpgradeV0LayerParameter(const V1LayerParameter& v0_layer_connection,
  ...
  if (v0_layer_connection.has_layer()) {
    ...
    if (v0_layer_param.has_num_output()) {
      if (type == "colw") {
        layer_param->mutable_colwolution_param()->set_num_output(
    ...
    if (v0_layer_param.has_biasterm()) {
      if (type == "colw") {
        layer_param->mutable_colwolution_param()->set_bias_term(
    ...
    if (v0_layer_param.has_weight_filler()) {
      if (type == "colw") {
        layer_param->mutable_colwolution_param()->
    ...
    if (v0_layer_param.has_pad()) {
      if (type == "colw") {
        layer_param->mutable_colwolution_param()->add_pad(v0_layer_param.pad());
      } else if (type == "pool") {
    ...
    if (v0_layer_param.has_kernelsize()) {
      if (type == "colw") {
        layer_param->mutable_colwolution_param()->add_kernel_size(
    ...
    if (v0_layer_param.has_group()) {
      if (type == "colw") {
        layer_param->mutable_colwolution_param()->set_group(
    ...
    if (v0_layer_param.has_stride()) {
      if (type == "colw") {
        layer_param->mutable_colwolution_param()->add_stride(
...
V1LayerParameter_LayerType UpgradeV0LayerType(const string& type) {
  if (type == "accuracy") {
    return V1LayerParameter_LayerType_ACLWRACY;
  } else if (type == "bnll") {
    return V1LayerParameter_LayerType_BNLL;
  } else if (type == "concat") {
    return V1LayerParameter_LayerType_CONCAT;
  } else if (type == "colw") {
    return V1LayerParameter_LayerType_COLWOLUTION;
  } else if (type == "data") {
...
#define COLWERT_LAYER_TRANSFORM_PARAM(TYPE, Name, param_name) \
  do { \
...
void UpgradeNetDataTransformation(NetParameter* net_param) {
  for (int i = 0; i < net_param->layers_size(); ++i) {
    COLWERT_LAYER_TRANSFORM_PARAM(DATA, Data, data);
    COLWERT_LAYER_TRANSFORM_PARAM(IMAGE_DATA, ImageData, image_data);
    COLWERT_LAYER_TRANSFORM_PARAM(WINDOW_DATA, WindowData, window_data);
  }
}
...
bool UpgradeV1LayerParameter(const V1LayerParameter& v1_layer_param,
  ...
  if (v1_layer_param.has_aclwracy_param()) {
    layer_param->mutable_aclwracy_param()->CopyFrom(
        v1_layer_param.aclwracy_param());
  }
  ...
  if (v1_layer_param.has_colwolution_param()) {
    layer_param->mutable_colwolution_param()->CopyFrom(
        v1_layer_param.colwolution_param());
  }
...
const char* UpgradeV1LayerType(const V1LayerParameter_LayerType type) {
  ...
  case V1LayerParameter_LayerType_ABSVAL:
    return "AbsVal";
  case V1LayerParameter_LayerType_ACLWRACY:
    return "Accuracy";
  ...
  case V1LayerParameter_LayerType_COLWOLUTION:
    return "Colwolution";
  case V1LayerParameter_LayerType_DECOLWOLUTION:
    return "Decolwolution";
...
void UpgradeNetInput(NetParameter* net_param) {
  // Collect inputs and colwert to Input layer definitions.
  ...
  if (has_shape || has_dim) {
    ...
    // Colwert input fields into a layer.
