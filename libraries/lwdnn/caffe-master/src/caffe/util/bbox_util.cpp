...
void ClipBBox(const NormalizedBBox& bbox, NormalizedBBox* clip_bbox) {
  ...
  clip_bbox->set_diffilwlt(bbox.diffilwlt());
}
...
void ScaleBBox(const NormalizedBBox& bbox, const int height, const int width,
  ...
  scale_bbox->set_diffilwlt(bbox.diffilwlt());
}
...
void LocateBBox(const NormalizedBBox& src_bbox, const NormalizedBBox& bbox,
  ...
  loc_bbox->set_diffilwlt(bbox.diffilwlt());
}
...
bool ProjectBBox(const NormalizedBBox& src_bbox, const NormalizedBBox& bbox,
  ...
  proj_bbox->set_diffilwlt(bbox.diffilwlt());
...
void GetGroundTruth(const Dtype* gt_data, const int num_gt,
      const int background_label_id, const bool use_diffilwlt_gt,
  ...
  for (int i = 0; i < num_gt; ++i) {
    ...
    bool diffilwlt = static_cast<bool>(gt_data[start_idx + 7]);
    if (!use_diffilwlt_gt && diffilwlt) {
      // Skip reading diffilwlt ground truth.
      continue;
    }
    NormalizedBBox bbox;
...
void ApplyNMS(const vector<NormalizedBBox>& bboxes, const vector<float>& scores,
  ...
  while (score_index_vec.size() != 0) {
    ...
    for (vector<pair<float, int> >::iterator it = score_index_vec.begin();
      ...
      int lwr_idx = it->second;
      const NormalizedBBox& lwr_bbox = bboxes[lwr_idx];
      if (BBoxSize(lwr_bbox) < 1e-5) {
        // Erase small box.
      ...
      if (reuse_overlaps) {
        if (overlaps->find(best_idx) != overlaps->end() &&
            overlaps->find(best_idx)->second.find(lwr_idx) !=
            (*overlaps)[best_idx].end()) {
          // Use the computed overlap.
          lwr_overlap = (*overlaps)[best_idx][lwr_idx];
        } else if (overlaps->find(lwr_idx) != overlaps->end() &&
                   overlaps->find(lwr_idx)->second.find(best_idx) !=
                   (*overlaps)[lwr_idx].end()) {
          // Use the computed overlap.
          lwr_overlap = (*overlaps)[lwr_idx][best_idx];
        } else {
          lwr_overlap = JaccardOverlap(best_bbox, lwr_bbox);
          // Store the overlap for future use.
          (*overlaps)[best_idx][lwr_idx] = lwr_overlap;
        }
      } else {
        lwr_overlap = JaccardOverlap(best_bbox, lwr_bbox);
      }
...
void ApplyNMS(const bool* overlapped, const int num, vector<int>* indices) {
  ...
  while (index_vec.size() != 0) {
    ...
    for (vector<int>::iterator it = index_vec.begin(); it != index_vec.end();) {
      ...
      // Remove it if necessary
      if (overlapped[best_idx * num + lwr_idx]) {
...
void ApplyNMSFast(const float16* bboxes, const float16* scores, const int num,
      const float score_threshold, const float nms_threshold,
      const float eta, const int top_k, vector<int>* indices);
void LwmSum(const vector<pair<float, int> >& pairs, vector<int>* lwmsum) {
  ...
  for (int i = 0; i < sort_pairs.size(); ++i) {
    if (i == 0) {
      lwmsum->push_back(sort_pairs[i].second);
    } else {
      lwmsum->push_back(lwmsum->back() + sort_pairs[i].second);
    }
  }
}
...
void ComputeAP(const vector<pair<float, int> >& tp, const int num_pos,
  ...
  *ap = 0;
  ...
  // Compute lwmsum of tp.
  vector<int> tp_lwmsum;
  LwmSum(tp, &tp_lwmsum);
  CHECK_EQ(tp_lwmsum.size(), num);

  // Compute lwmsum of fp.
  vector<int> fp_lwmsum;
  LwmSum(fp, &fp_lwmsum);
  CHECK_EQ(fp_lwmsum.size(), num);

  // Compute precision.
  for (int i = 0; i < num; ++i) {
    prec->push_back(static_cast<float>(tp_lwmsum[i]) /
                    (tp_lwmsum[i] + fp_lwmsum[i]));
  }

  // Compute recall.
  for (int i = 0; i < num; ++i) {
    CHECK_LE(tp_lwmsum[i], num_pos);
    rec->push_back(static_cast<float>(tp_lwmsum[i]) / num_pos);
  }
  ...
  if (ap_version == "11point") {
    ...
    float lwr_rec = rec->back();
    float lwr_prec = prec->back();
    for (int i = num - 2; i >= 0; --i) {
      lwr_prec = std::max<float>((*prec)[i], lwr_prec);
      if (fabs(lwr_rec - (*rec)[i]) > eps) {
        *ap += lwr_prec * fabs(lwr_rec - (*rec)[i]);
      }
      lwr_rec = (*rec)[i];
    }
    *ap += lwr_rec * lwr_prec;
