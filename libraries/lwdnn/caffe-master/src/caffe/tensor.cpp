...
void Tensor::colwert(Type new_type) {
  if (new_type == type_) {
    return;
  }

  const shared_ptr<SyncedMemory>& lwrrent_mem = synced_mem();
  ...
  if (!new_mem || !new_mem->is_valid()) {
    ...
    if (lwrrent_mem->head() != SyncedMemory::UNINITIALIZED) {
      copy_helper(data_gpu, count_,
          data_gpu ? lwrrent_mem->gpu_data() : lwrrent_mem->cpu_data(),
          type_,
...
void Tensor::copy_helper(bool use_gpu, int count, const void* p_src, Type src_type,
  ...
  if (is_type<float>(src_type)) {
    if (is_type<float>(dst_type)) {  // FP32 -> FP32
      ...
      caffe_colwert(use_gpu, count, static_cast<const float*>(p_src),
    ...
    if (is_type<float>(dst_type)) {  // FP16 -> FP32
      caffe_colwert(use_gpu, count, static_cast<const float16*>(p_src),
          static_cast<float*>(p_dst));
    ...
    if (is_type<float>(dst_type)) {  // FP64 -> FP32
      caffe_colwert(use_gpu, count, static_cast<const double*>(p_src),
          static_cast<float*>(p_dst));
...
void Tensor::scale(float scale, void* handle) {
  ...
  if (is_gpu_head()) {
    ...
    gpu_scal(count_, type_, mem->mutable_gpu_data(), scale, lwblas_handle);
  } else {
...
void Tensor::gpu_scal(int count, Type dtype, void* data, float scal, lwblasHandle_t lwblas_handle) {
  if (is_type<float>(dtype)) {
    caffe_gpu_scal(count, scal, static_cast<float*>(data), lwblas_handle);
  } else if (is_type<float16>(dtype)) {
    caffe_gpu_scal(count, static_cast<float16>(scal), static_cast<float16*>(data), lwblas_handle);
  } else if (is_type<double>(dtype)) {
    caffe_gpu_scal(count, static_cast<double>(scal), static_cast<double*>(data), lwblas_handle);
  } else {
    LOG(FATAL) << "Unsupported data type: " << Type_Name(dtype);
  }
}
