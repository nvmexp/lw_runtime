...
void Net::Init(const NetParameter& in_param) {
  ...
  for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
    ...
    if (!layer_param.has_backward_type()) {
      if (is_data_layer) {
        // In majority of cases we manage to avoid redundant colwersion:
    ...
    // Colwolution algorithms
    if (param.has_default_colw_algos_override() && layer_param.has_colwolution_param() &&
        !layer_param.colwolution_param().has_colw_algos_override()) {
      mutable_layer_param->mutable_colwolution_param()->
          set_colw_algos_override(param.default_colw_algos_override());
    }

    // lwDNN math
    if (param.has_default_lwdnn_math_override() &&
        !layer_param.has_lwdnn_math_override()) {
      mutable_layer_param->set_lwdnn_math_override(param.default_lwdnn_math_override());
    }
  ...
  // ilwert param_layer_indices_ to give map of
...
void Net::ReduceAndUpdate() {
  ...
  while (true) {
    ...
    lwblasHandle_t handle = Caffe::lwblas_handle(1);
...
//#endif
  ...
  DLOG(INFO) << print_lwrrent_device() << " Leaving ReduceAndUpdate thread " << lwp_id();
}
...
void Net::Reduce(int param_id) {
  this->learnable_params()[param_id]->scale_diff(1.F / global_grad_scale(),  // TODO
      Caffe::lwblas_handle(1));
#ifdef USE_NCCL
  Solver::Callback* cb = solver_->callback();
  NCCL_CHECK(ncclGroupStart());
  cb->allreduce(param_id);
  NCCL_CHECK(ncclGroupEnd());
  LWDA_CHECK(lwdaStreamSynchronize(cb->comm_stream()));
#endif
...
void Net::InitializeLearnableDiffSpace(int type_id) {
  ...
  if (learnable_params_ptrs_[type_id].size() == learnable_params_.size()) {
    LOG_IF(INFO, learnable_space_size_[type_id] > 0) << print_lwrrent_device()
  ...
  // Size have at least one byte, otherwise lwdaMalloc fails if net has no
  // learnable parameters. Times two.
  if (learnable_space_size_[type_id] < 2) {
    learnable_space_size_[type_id] = 2;
  }
  LOG(INFO) << print_lwrrent_device() << " Reserving "
        ...
        if (param_owners_[lip] < 0) {
          ...
          if (learnable_params_[param_id]->diff_type() == t) {
            ...
            void *p = learnable_params_[param_id]->
                lwrrent_mutable_data_memory(Caffe::mode() == Caffe::GPU);
            (void) p;
          }
        }
      } else {
        DLOG(INFO) << print_lwrrent_device()
...
std::string Net::print_lwrrent_device() const {
