...
void Blob::Reshape(const vector<int>& shape) {
  ...
  CHECK(is_lwrrent_data_valid());
  CHECK(is_lwrrent_diff_valid());
}
...
void Blob::Update() {
  colwert_diff(data_type());  // align data&diff types
...
void Blob::CopyFrom(const Blob& source, bool copy_diff, bool reshape,
  ...
  CHECK_EQ(src_packing, dst_packing);
    ...
    do {
      if (src_type == dst_type) {
        ...
        if (srct->is_cpu_head() && dstt->is_gpu_head()) {
          ...
          LWDA_CHECK(lwdaMemcpyAsync(dst->mutable_gpu_data(false, group), src->cpu_data(),
          ...
          lwdaStream_t stream = Caffe::thread_stream(group);
          LWDA_CHECK(lwdaMemcpyAsync(dst->mutable_cpu_data(false, group), src->gpu_data(group),
              srct->count_ * tsize(src_type),
    ...
    CHECK(srct != dstt);
    lwdnnHandle_t handle = Caffe::lwdnn_handle(group);
    lwdnnTensorDescriptor_t src_desc, dst_desc;
    LWDNN_CHECK(lwdnnCreateTensorDescriptor(&src_desc));
    LWDNN_CHECK(lwdnnCreateTensorDescriptor(&dst_desc));
    lwdnn::setTensor4dDesc(&src_desc, src_type, src_packing, source.shape_);
    lwdnn::setTensor4dDesc(&dst_desc, dst_type, dst_packing, shape_);

    LWDNN_CHECK(lwdnnTransformTensor(handle,
        lwdnn::one(src_type), src_desc, src->gpu_data(group),
        lwdnn::zero(dst_type), dst_desc, dst->mutable_gpu_data(false, group)));
    LWDA_CHECK_ARG(lwdaStreamSynchronize(Caffe::thread_stream(group)), group);
    LWDNN_CHECK(lwdnnDestroyTensorDescriptor(src_desc));
    LWDNN_CHECK(lwdnnDestroyTensorDescriptor(dst_desc));
  }
#endif
  dst->validate();
}
...
void Blob::FromProto(const BlobProto& proto, bool reshape) {
  ...
  if (proto.double_data_size() > 0) {
    ...
    data_tensor_->ilwalidate_others();
    ...
    data_tensor_->colwert(dt);  // we have to restore its original type
    data_tensor_->ilwalidate_others();
  }
  // copy diff
  if (proto.double_diff_size() > 0) {
    ...
    diff_tensor_->ilwalidate_others();
    ...
    diff_tensor_->colwert(dt);  // we have to restore its original type
    diff_tensor_->ilwalidate_others();
  }
}
...
void Blob::ToProto(BlobProto* proto, bool store_in_old_format, bool write_diff) const {
  ...
  const void* pdata = lwrrent_data_memory(false);
  proto->set_raw_data_type(dt);
  proto->set_raw_data(pdata, count_ * tsize(dt));
  if (write_diff) {
    dt = diff_type();
    const void* pdiff = lwrrent_diff_memory(false);
