...
// Return the current learning rate. The lwrrently implemented learning rate
// policies are as follows:
//    - fixed: always return base_lr.
//    - step: return base_lr * gamma ^ (floor(iter / step))
//    - exp: return base_lr * gamma ^ iter
//    - ilw: return base_lr * (1 + gamma * iter) ^ (- power)
...
float SGDSolver<Dtype>::GetLearningRate() const {
  ...
  if (this->iter_ < this->param_.rampup_interval()) {
    ...
    this->lwrrent_step_ = this->iter_ / this->param_.stepsize();
    rate = this->param_.base_lr() * std::pow(this->param_.gamma(), this->lwrrent_step_);
  } else if (lr_policy == "exp") {
    rate = this->param_.base_lr() * std::pow(this->param_.gamma(), this->iter_);
  } else if (lr_policy == "ilw") {
    rate = this->param_.base_lr() *
           pow(1.F + this->param_.gamma() * float(this->iter_), -this->param_.power());
  } else if (lr_policy == "multistep") {
    if (this->lwrrent_step_ < this->param_.stepvalue_size() &&
        this->iter_ >= this->param_.stepvalue(this->lwrrent_step_)) {
      this->lwrrent_step_++;
...
void SGDSolver<Dtype>::ClipGradientsAndNormalize(void* handle, int type_id,
  ...
  const float aclwm_normalization = 1.F / this->param_.iter_size();
  const vector<shared_ptr<Blob>>& net_params = this->net_->learnable_params();
  ...
  if (l2norm_diff > clip_gradients) {
    ...
    for (int param_id : param_ids) {
      net_params[param_id]->scale_diff(scale_factor * aclwm_normalization, handle);
    }
  } else if (this->param_.iter_size() != 1) {
    for (int param_id : param_ids) {
      net_params[param_id]->scale_diff(aclwm_normalization, handle);
    }
  }
}
...
void SGDSolver<Dtype>::PrintRate(float rate) {
  if (Caffe::root_solver() && this->param_.display() && this->iter_ % this->param_.display() == 0) {
    ...
    LOG_IF(INFO, rank_ == 0) << net_->print_lwrrent_device()
...
void SGDSolver<Dtype>::Normalize(int param_id, void* handle) {
  ...
  // Scale gradient to counterbalance aclwmulation.
  const vector<shared_ptr<Blob>>& net_params = this->net_->learnable_params();
  const float aclwm_normalization = 1.F / this->param_.iter_size();
  net_params[param_id]->scale_diff(aclwm_normalization, handle);
}
...
void SGDSolver<Dtype>::PrintParams(int param_id) {
  if (Caffe::root_solver()
    ...
    if ((layer_type == "Colwolution") || (layer_type == "InnerProduct")) {
...
void SGDSolver<Dtype>::SnapshotSolverStateToBinaryProto(const string& model_filename) {
  ...
  state.set_lwrrent_step(this->lwrrent_step_);
  state.clear_history();
...
void SGDSolver<Dtype>::SnapshotSolverStateToHDF5(const string& model_filename) {
  ...
  hdf5_save_int(file_hid, "lwrrent_step", this->lwrrent_step_);
...
void SGDSolver<Dtype>::RestoreSolverStateFromBinaryProto(const string& state_file) {
  ...
  this->lwrrent_step_ = state.lwrrent_step();
...
void SGDSolver<Dtype>::RestoreSolverStateFromHDF5(const string& state_file) {
  ...
  this->lwrrent_step_ = hdf5_load_int(file_hid, "lwrrent_step");
