...
// If LWCA is available and in GPU mode, host memory will be allocated pinned,
// using lwdaMallocHost. It avoids dynamic pinning for transfers (DMA).
// The improvement in performance seems negligible in the single GPU case,
// but might be more significant for parallel training. Most importantly,
// it improved stability for large models on many GPUs.
void SyncedMemory::MallocHost(void** ptr, size_t size, bool* use_lwda) {
  if (Caffe::mode() == Caffe::GPU) {
    LWDA_CHECK(lwdaMallocHost(ptr, size));
    *use_lwda = true;
  } else {
    *ptr = malloc(size);
    *use_lwda = false;
...
void SyncedMemory::FreeHost(void* ptr, bool use_lwda) {
  if (use_lwda) {
    LWDA_CHECK(lwdaFreeHost(ptr));
  } else {
    free(ptr);
  }
}
...
SyncedMemory::~SyncedMemory() {
  if (cpu_ptr_ && own_cpu_data_) {
    FreeHost(cpu_ptr_, cpu_malloc_use_lwda_);
  }
  if (gpu_ptr_ && own_gpu_data_) {
//#ifdef DEBUG
//    lwdaPointerAttributes attr;
//    lwdaError_t status = lwdaPointerGetAttributes(&attr, gpu_ptr_);
//    if (status == lwdaSuccess) {
//      CHECK_EQ(attr.memoryType, lwdaMemoryTypeDevice);
...
void SyncedMemory::to_cpu(bool copy_from_gpu, int group) {
  switch (head_) {
    case UNINITIALIZED:
      MallocHost(&cpu_ptr_, size_, &cpu_malloc_use_lwda_);
...
void SyncedMemory::to_gpu(bool copy_from_cpu, int group) {
  switch (head_) {
    case UNINITIALIZED:
      LWDA_CHECK(lwdaGetDevice(&device_));
...
std::string SyncedMemory::to_string(int indent, Type type) {  // debug helper
  ...
  os << idt << "own_cpu_data_, own_gpu_data_: " << own_cpu_data_ << " " << own_gpu_data_
     << std::endl;
  os << idt << "cpu_malloc_use_lwda_, gpu_device_: " << cpu_malloc_use_lwda_ << " " << device_
     << std::endl;
