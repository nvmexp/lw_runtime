...
void Solver::Step(int iters) {
  ...
  for (auto b : net_->learnable_params_mapped()) {
    b->lwrrent_mutable_data_memory(true);
  }

  if (Caffe::solver_count() > 1) {
    // we need to sync all threads before starting, otherwise some lwca init,
    // malloc or other lwca stuff could interlock with in-loop lwca GPU sync
    // called in on_start.
    callback_soft_barrier();
    callback_->on_start(net_->learnable_params_mapped());
    LOG(INFO) << net_->print_lwrrent_device() << " Starting Optimization on GPU "
              << Caffe::device();
  }
  ...
  while (iter_ < stop_iter) {
    ...
    if (this->param_display() && (display || rel_iter <= 2 || iter_ + 1 == stop_iter)) {
      ...
      if (rel_iter > 2) {  // we skip 0,1,2 for correct benchmarking
        ...
        LOG_IF(INFO, rank_ == 0) << "    " << net_->print_lwrrent_device() << " Iteration "
      ...
      for (int j = 0; j < result.size(); ++j) {
        ...
        for (int k = 0; k < result[j]->count(); ++k) {
          ...
          LOG_IF(INFO, rank_ == 0)
          << "    " << net_->print_lwrrent_device() << "     Train net output #" << score_index++
...
void Solver::Reduce(Callback* callback, int device, Caffe::Brew mode, uint64_t random_seed,
    ...
    if (mode == Caffe::GPU) {
      LWDA_CHECK(lwdaSetDevice(device));
#ifndef NO_LWML
      lwml::setCpuAffinity(device);
#endif
    }
    Caffe::set_mode(mode);
...
bool Solver::Solve(const char* resume_file) {
  LOG(INFO) << net_->print_lwrrent_device() << " Solving " << net_->name()
            << " Learning Rate Policy: " << param_.lr_policy();
  // Initialize to false every time we start solving.
  requested_early_exit_ = false;

  if (resume_file != nullptr) {
    LOG(INFO) << net_->print_lwrrent_device()
  ...
  iterations_last_ = 0;
  if (requested_early_exit_) {
    LOG(INFO) << net_->print_lwrrent_device() << " Optimization stopped early.";
    return true;
  }
...
vector<float> Solver::Test(const int test_net_id, const int iters, bool use_multi_gpu) {
  ...
  for (int i = 0; i < test_score.size(); ++i) {
    ...
    LOG_IF(INFO, rank_ == 0) << "    " << test_net->print_lwrrent_device()
