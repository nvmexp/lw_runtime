...
#ifdef USE_NCCL
#include "caffe/util/lwcl.hpp"
#ifndef NCCL_MAJOR
#error "NCCL_MAJOR is not defined. Please check LWCL installation"
#endif
...
#else
  LOG_IF(FATAL, CAFFE_NCCL_VER < 20200) << "LWCL 2.2 or higher is required";
  if (global_rank_ == 0) {
...
#else
  LOG(FATAL) << "Multi-GPU exelwtion not available - rebuild with USE_NCCL";
#endif  // USE_NCCL
...
void P2PSync::InternalThreadEntry() {
  ...
  // https://docs.lwpu.com/deeplearning/sdk/lwcl-developer-guide/index.html
  //initializing LWCL, group API is required around ncclCommInitRank as it is
  //called across multiple GPUs in each thread/process
  NCCL_CHECK(ncclGroupStart());
  LWDA_CHECK(lwdaSetDevice(target_device_));
  ...
  comm_stream_ = LwdaStream::create(true);
...
void P2PSync::on_start(const vector<shared_ptr<Blob>>& net) {
  ...
  for (int i = 0; i < net.size(); ++i) {
    ...
    NCCL_CHECK(ncclGroupStart());
    NCCL_CHECK(ncclBcast(param->lwrrent_mutable_data_memory(true),
        even(param->count()),
        lwcl::nccl_type(param_type),
        0,
        nccl_comm_,
        comm_stream()));
    NCCL_CHECK(ncclGroupEnd());
    LWDA_CHECK(lwdaStreamSynchronize(comm_stream()));
  }
#endif  // USE_NCCL
}
...
void P2PSync::allreduce(int param_id) {
  ...
  allreduce_bucket(param->count(), param->lwrrent_mutable_diff_memory(true),
                   param->diff_type());
#endif  // USE_NCCL
}
