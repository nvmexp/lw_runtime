...
void LRNLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
  ...
  if (this->layer_param_.lrn_param().norm_region() ==
    ...
    // ilwerse demoninator computed by the power layer.
...
void LRNLayer<Ftype, Btype>::CrossChannelBackward_cpu(
  ...
  TBlob<Btype> aclwm_ratio(1, 1, height_, width_);
  Btype* padded_ratio_data = padded_ratio.mutable_cpu_data();
  Btype* aclwm_ratio_data = aclwm_ratio.mutable_cpu_data();
  // We hack a little bit by using the diff() to store an additional result
  Btype* aclwm_ratio_times_bottom = aclwm_ratio.mutable_cpu_diff();
  ...
  int ilwerse_pre_pad = size_ - (size_ + 1) / 2;
  for (int n = 0; n < num_; ++n) {
    int block_offset = scale_.offset(n);
    // first, compute diff_i * y_i / s_i
    caffe_mul(channels_ * height_ * width_,
        top_diff + block_offset, top_data + block_offset,
        padded_ratio_data + padded_ratio.offset(0, ilwerse_pre_pad));
    caffe_div(channels_ * height_ * width_,
        padded_ratio_data + padded_ratio.offset(0, ilwerse_pre_pad),
        scale_data + block_offset,
        padded_ratio_data + padded_ratio.offset(0, ilwerse_pre_pad));
    // Now, compute the aclwmulated ratios and the bottom diff
    caffe_set(aclwm_ratio.count(), Btype(0), aclwm_ratio_data);
    for (int c = 0; c < size_ - 1; ++c) {
      caffe_axpy(height_ * width_, Btype(1),
          padded_ratio_data + padded_ratio.offset(0, c), aclwm_ratio_data);
    }
    for (int c = 0; c < channels_; ++c) {
      ...
      caffe_mul(height_ * width_,
          ...
          aclwm_ratio_data, aclwm_ratio_times_bottom);
      caffe_axpy(height_ * width_, Btype(-cache_ratio_value),
          aclwm_ratio_times_bottom, bottom_diff + top[0]->offset(n, c));
      caffe_axpy(height_ * width_, Btype(-1),
          padded_ratio_data + padded_ratio.offset(0, c), aclwm_ratio_data);
    }
  }
