...
DataLayer<Ftype, Btype>::InitializePrefetch() {
  ...
  if (auto_mode) {
    // Here we try to optimize memory split between prefetching and colwolution.
    ...
    if (this->is_gpu_transform()) {
      ...
      if (cache_ && parsers_num > 1UL) {
        LOG(INFO) << this->print_lwrrent_device() << " Reduced parser threads count from "
  ...
  LOG(INFO) << this->print_lwrrent_device() << " Parser threads: "
      << this->parsers_num_ << (auto_mode ? " (auto)" : "");
  LOG(INFO) << this->print_lwrrent_device() << " Transformer threads: "
...
DataLayer<Ftype, Btype>::DataLayerSetUp(const vector<Blob*>& bottom, const vector<Blob*>& top) {
  ...
  if (this->is_gpu_transform()) {
    ...
    LOG(INFO) << this->print_lwrrent_device() << " Transform on GPU enabled";
  ...
  LOG(INFO) << this->print_lwrrent_device() << " Output data size: "
      << top[0]->num() << ", "
      << top[0]->channels() << ", "
      << top[0]->height() << ", "
      << top[0]->width();
...
bool DataLayer<Ftype, Btype>::load_batch(Batch* batch, int thread_id, size_t queue_id) {
  ...
  for (size_t entry = 0; entry < batch_size; ++entry) {
    ...
    if (item_id == 0UL) {
      lwrrent_batch_id = datum->record_id() / batch_size;
    }
    // Copy label.
    ...
    if (use_gpu_transform) {
      lwdaStream_t stream = Caffe::thread_stream(Caffe::GPU_TRANSF_GROUP);
      if (datum->encoded()) {
      ...
      LWDA_CHECK(lwdaMemcpyAsync(static_cast<char*>(dst_gptr) + item_id * datum_size,
          src_buf.data(), datum_size, lwdaMemcpyHostToDevice, stream));
