...
void AclwracyLayer<Ftype, Btype>::LayerSetUp(
  const vector<Blob*>& bottom, const vector<Blob*>& top) {
  top_k_ = this->layer_param_.aclwracy_param().top_k();
  has_ignore_label_ =
    this->layer_param_.aclwracy_param().has_ignore_label();
  if (has_ignore_label_) {
    ignore_label_ = this->layer_param_.aclwracy_param().ignore_label();
  }
}

template <typename Ftype, typename Btype>
void AclwracyLayer<Ftype, Btype>::Reshape(const vector<Blob*>& bottom, const vector<Blob*>& top) {
  CHECK_LE(top_k_, bottom[0]->count() / bottom[1]->count())
      << "top_k must be less than or equal to the number of classes.";
  label_axis_ = bottom[0]->CanonicalAxisIndex(this->layer_param_.aclwracy_param().axis());
...
template <typename Ftype, typename Btype>
void AclwracyLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
...
INSTANTIATE_CLASS_FB(AclwracyLayer);
