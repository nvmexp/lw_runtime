...
void LSTMLayer<Ftype, Btype>::RelwrrentInputBlobNames(vector<string>* names) const {
  names->resize(2);
  (*names)[0] = "h_0";
  (*names)[1] = "c_0";
}
...
template<typename Ftype, typename Btype>
void LSTMLayer<Ftype, Btype>::RelwrrentOutputBlobNames(vector<string>* names) const {
  names->resize(2);
...
template<typename Ftype, typename Btype>
void LSTMLayer<Ftype, Btype>::RelwrrentInputShapes(vector<BlobShape>* shapes) const {
  const int num_output = this->layer_param_.relwrrent_param().num_output();
...
void LSTMLayer<Ftype, Btype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  const FillerParameter& weight_filler =
      this->layer_param_.relwrrent_param().weight_filler();
  const FillerParameter& bias_filler =
      this->layer_param_.relwrrent_param().bias_filler();
  ...
  RelwrrentInputShapes(&input_shapes);
