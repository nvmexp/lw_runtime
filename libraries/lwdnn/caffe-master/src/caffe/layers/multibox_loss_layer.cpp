...
void MultiBoxLossLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
  ...
  use_diffilwlt_gt_ = multibox_loss_param.use_diffilwlt_gt();
  ...
  if (do_neg_mining_) {
    CHECK(share_location_)
        << "Lwrrently only support negative mining if share_location is true.";
  }
...
void MultiBoxLossLayer<Ftype, Btype>::Forward_cpu(const vector<Blob*>& bottom,
  ...
  GetGroundTruth(gt_data, num_gt_, background_label_id_, use_diffilwlt_gt_,
                 &all_gt_bboxes);
