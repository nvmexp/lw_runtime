...
ImageDataLayer<Ftype, Btype>::ImageDataLayer(const LayerParameter& param, size_t solver_rank)
  ...
  LOG_IF(INFO, P2PManager::global_rank() == 0)
             << this->print_lwrrent_device() << " ImageDataLayer: " << this
             << " name: " << this->name()
             << " id: " << id_
...
void ImageDataLayer<Ftype, Btype>::DataLayerSetUp(const vector<Blob*>& bottom,
  ...
  LOG_IF(INFO, P2PManager::global_rank() == 0)
  << this->print_lwrrent_device() << " A total of " << lines_[id_].size() << " images.";
    ...
    if (cache_on && !cached_[id_] && !from_cache) {
      ...
      if (cv_img.data != nullptr) {
        ...
        if (em.second) {
          ...
          DLOG(WARNING) << this->print_lwrrent_device()
      ...
      if (cached_num_[id_] + failed_num_[id_] >= lines_size) {
        ...
        LOG_IF(INFO, P2PManager::global_rank() == 0)     << this->print_lwrrent_device() << " "
    ...
    if (line_ids_[thread_id] >= lines_size) {
      ...
      if (thread_id == 0 &&
        ...
        if (this->phase_ == TRAIN) {
          ...
          LOG_IF(INFO, P2PManager::global_rank() == 0)
          << this->print_lwrrent_device() << " Restarting data prefetching (" << lines_size << ")";
          if (epoch_count_ == 0UL) {
