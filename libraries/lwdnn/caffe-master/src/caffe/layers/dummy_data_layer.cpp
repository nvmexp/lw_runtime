...
void DummyDataLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
  ...
  // We first fill refill_ in with the ILWERSE of its final values.
  ...
  if (num_data_filler <= 1) {
    ...
    // but use the ilwerse of this rule for the first run.
    refill_.resize(1);
  ...
  // Run Forward once, with refill_ ilwerted, to fill the constant Blobs.
  this->Forward(bottom, top);
  // Ilwert the ilwerted refill_ values to refill the desired (non-constant)
