...
void Im2colLayer<Ftype, Btype>::LayerSetUp(const vector<Blob*>& bottom,
  ...
  if (colw_param.has_kernel_h() || colw_param.has_kernel_w()) {
    CHECK_EQ(num_spatial_axes_, 2)
        << "kernel_h & kernel_w can only be used for 2D colwolution.";
    ...
    CHECK(num_kernel_dims == 1 || num_kernel_dims == num_spatial_axes_)
      ...
      for (int i = 0; i < num_spatial_axes_; ++i) {
        kernel_shape_data[i] =
            colw_param.kernel_size((num_kernel_dims == 1) ? 0 : i);
      }
  }
  for (int i = 0; i < num_spatial_axes_; ++i) {
  ...
  if (colw_param.has_stride_h() || colw_param.has_stride_w()) {
    CHECK_EQ(num_spatial_axes_, 2)
        << "stride_h & stride_w can only be used for 2D colwolution.";
    ...
    for (int i = 0; i < num_spatial_axes_; ++i) {
      stride_data[i] = (num_stride_dims == 0) ? kDefaultStride :
          colw_param.stride((num_stride_dims == 1) ? 0 : i);
  ...
  if (colw_param.has_pad_h() || colw_param.has_pad_w()) {
    CHECK_EQ(num_spatial_axes_, 2)
        << "pad_h & pad_w can only be used for 2D colwolution.";
    CHECK_EQ(0, colw_param.pad_size())
    ...
    for (int i = 0; i < num_spatial_axes_; ++i) {
      pad_data[i] = (num_pad_dims == 0) ? kDefaultPad :
          colw_param.pad((num_pad_dims == 1) ? 0 : i);
    }
  }
  ...
  for (int i = 0; i < num_spatial_axes_; ++i) {
    dilation_data[i] = (num_dilation_dims == 0) ? kDefaultDilation :
                       colw_param.dilation((num_dilation_dims == 1) ? 0 : i);
  }
}
