...
static mxArray* blob_to_mx_mat(const TBlob<float>* blob,
  ...
  // matlab array needs to have at least one dimension, colwert scalar to 1-dim
  if (num_axes == 0) {
    dims.push_back(1);
  }
  mxArray* mx_mat =
...
// Colwert vector<int> to matlab row vector
...
// Colwert vector<string> to matlab cell vector of strings
...
/** -----------------------------------------------------------------
 ** handle and pointer colwersion functions
 ...
 **/
// Colwert a handle in matlab to a pointer in C++. Check if init_key matches
template <typename T>
static T* handle_to_ptr(const mxArray* mx_handle) {
  ...
  mxCHECK(mxGetScalar(mx_init_key) == init_key,
      "Could not colwert handle to pointer due to invalid init_key. "
...
// Colwert a pointer in C++ to a handle in matlab
template <typename T>
...
// Colwert a vector of shared_ptr in C++ to handle struct vector
template <typename T>
