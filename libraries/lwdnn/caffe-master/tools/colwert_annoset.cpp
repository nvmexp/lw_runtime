// This program colwerts a set of images and annotations to a lmdb/leveldb by
// storing them as AnnotatedDatum proto buffers.
// Usage:
//   colwert_annoset [FLAGS] ROOTFOLDER/ LISTFILE DB_NAME
//
...
int main(int argc, char** argv) {
  ...
  gflags::SetUsageMessage("Colwert a set of images and annotations to the "
  ...
  if (argc < 4) {
    gflags::ShowUsageWithFlagsRestrict(argv[0], "tools/colwert_annoset");
    return 1;
  }
  ...
  if (anno_type == "classification") {
    ...
    CHECK(MapNameToLabel(label_map, check_label, &name_to_label))
        << "Failed to colwert name to label.";
