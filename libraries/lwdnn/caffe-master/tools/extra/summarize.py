...
def summarize_net(net):
    ...
    for lr in net.layer:
        ...
        # Summarize colw/pool parameters.
        # TODO support rectangular/ND parameters
        colw_param = lr.colwolution_param
        if (lr.type in ['Colwolution', 'Decolwolution']
                and len(colw_param.kernel_size) == 1):
            arg_str = str(colw_param.kernel_size[0])
            if len(colw_param.stride) > 0 and colw_param.stride[0] != 1:
                arg_str += '/' + str(colw_param.stride[0])
            if len(colw_param.pad) > 0 and colw_param.pad[0] != 0:
                arg_str += '+' + str(colw_param.pad[0])
            arg_str += ' ' + str(colw_param.num_output)
            if colw_param.group != 1:
                arg_str += '/' + str(colw_param.group)
