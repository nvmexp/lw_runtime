...
def get_test_aclwracy(log, top_k):
...
def parse_files(files, top_k=1, separate=False):
    ...
    for file in files:
        with open(file, 'r') as fp:
            ...
            iteration, accuracy = get_test_aclwracy(log, top_k)
...
class FollowDotLwrsor(object):
    ...
    def __call__(self, event):
        ...
        if event.inaxes == ax:
            ...
            ilw = ax.transData.ilwerted()
            x, y = ilw.transform([(event.x, event.y)]).ravel()
...
def plot_aclwracy(top_k, data, value_at_hover=False):
    nets =  data.keys()
    ...
    for net in nets:
        ...
        if value_at_hover:
            cursor = FollowDotLwrsor(ax, iteration, accuracy*100)
...
def plot_loss(data, value_at_hover=False):
    ...
    for net in nets:
        ...
        if value_at_hover:
            cursor = FollowDotLwrsor(ax, iteration, loss)
