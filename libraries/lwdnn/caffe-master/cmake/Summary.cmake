...
function(caffe_status text)
  ...
  set(status_lwrrent_name "cond")
  foreach(arg ${ARGN})
    if(arg STREQUAL "THEN")
      set(status_lwrrent_name "then")
    elseif(arg STREQUAL "ELSE")
      set(status_lwrrent_name "else")
    else()
      list(APPEND status_${status_lwrrent_name} ${arg})
    endif()
  endforeach()
...
function(caffe_extract_caffe_version)
  ...
  find_package(Git)
  if(GIT_FOUND)
    exelwte_process(COMMAND ${GIT_EXELWTABLE} describe --tags --always --dirty
...
################################################################################################
# Prints aclwmulated caffe configuration summary
# Usage:
...
function(caffe_print_configuration_summary)
  ...
  caffe_status("  LWCA              : " HAVE_LWDA THEN "Yes (ver. ${LWDA_VERSION})" ELSE "No" )
  caffe_status("")
  if(HAVE_LWDA)
    caffe_status("LWPU LWCA:")
    caffe_status("  Target GPU(s)     :   ${LWDA_ARCH_NAME}" )
    caffe_status("  GPU arch(s)       :   ${LWCC_FLAGS_EXTRA_readable}")
    if(USE_LWDNN)
      caffe_status("  lwDNN             : " HAVE_LWDNN THEN "Yes (ver. ${LWDNN_VERSION})" ELSE "Not found")
    else()
      caffe_status("  lwDNN             :   Disabled")
    endif()
    if(USE_NCCL_SET)
      if(USE_NCCL)
        caffe_status("  LWCL              : " NCCL_FOUND THEN "Yes (ver. ${NCCL_VERSION})" ELSE "Not found")
      else()
        caffe_status("  LWCL              :   Disabled")
      endif()
    else()
      caffe_status("  LWCL              : " NCCL_FOUND THEN "Yes (ver. ${NCCL_VERSION})" ELSE "Not found (not requested)")
    endif()
    caffe_status("  USE_MPI           :   ${USE_MPI}")

    if(LWML_FOUND)
      caffe_status("  LWML              :   ${LWML_LIBRARY} ")
    else()
      caffe_status("  LWML              :   NOT FOUND" )
    endif()
  ...
  if(HAVE_PYTHON)
    caffe_status("Python:")
    caffe_status("  Interpreter       :" PYTHON_EXELWTABLE THEN "${PYTHON_EXELWTABLE} (ver. ${PYTHON_VERSION_STRING})" ELSE "No")
  ...
  if(BUILD_docs)
    caffe_status("Dolwmentaion:")
    caffe_status("  Doxygen           :" DOXYGEN_FOUND THEN "${DOXYGEN_EXELWTABLE} (${DOXYGEN_VERSION})" ELSE "No")
