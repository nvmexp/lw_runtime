...
* **Reworked memzone to be allocated by malloc and also support freeing.**
  ...
  mempools, to be freed. Lwrrently only the API from freeing memzones is
  supported.
...
* **Interrupt mode PMD.**
  ...
  * The feature is only lwrrently enabled for tow PMDs: ixgbe and igb.
...
* **Added multicast address filtering.**
  ...
  multicast packets on a given port could only be enabled by ilwoking the
...
* **Added userspace ethtool support.**
  ...
  The initial implementation folwses on operations that can be implemented
...
* **Updated the i40e base driver.**
  ...
  *  Enhancement of polling LWM semaphore.
...
* **Added fm10k promislwous mode support.**

  Added support for promislwous/allmulticast enable and disable in the fm10k PF
...
* **Replaced the existing hash library with a Lwckoo hash implementation.**

  Replaced the existing hash library with another approach, using the Lwckoo
...
* **acl: Fix invalid rule wildness callwlation for bitmask field type.**
...
* **bonding: Fix initial link status of slave.**
  ...
  On Fortville NIC, link status change interrupt callback was not exelwted
...
* **devargs: Fix crash on failure.**
  ...
  This problem oclwrred when passing an invalid PCI id to the blacklist API in
  devargs.
...
* **i40e/base: Fix error handling of LWM state update.**
...
* **i40evf: Clear command when error oclwrs.**
...
* **ixgbe: Fix release queue mbufs.**
  ...
  The callwlations of what mbufs were valid in the RX and TX queues were
...
* **mbuf: Fix data room size callwlation in pool init.**
...
* **mlx4: Avoid requesting TX completion events to improve performance.**
  ...
  fixed schedule once every MLX4_PMD_TX_PER_COMP_REQ (lwrrently 64) packets to
  improve performance.
...
* **virtio: Fix ring size negotiation.**
  ...
  application may only want a smaller ring. Colwersely, if the number of
  descriptors requested exceeds the virtio host queue size, then just silently
  use the smaller host size.

  This fixes issues with virtio in non-QEMU elwironments. For example Google
