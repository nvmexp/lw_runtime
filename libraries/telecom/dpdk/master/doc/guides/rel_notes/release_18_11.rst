...
* **Added lock free reader/writer conlwrrency to hash library (rte_hash).**

  Lock free reader/writer conlwrrency prevents the readers from getting
...
* **Added Traffic Pattern Aware Power Control Library.**
  ...
  exelwted per core and uses the number of empty polls as a hint for system
  power management.
...
* **Allow unit test binary to take parameters from the environment.**
  ...
  diffilwlt. Support has been added to the application to allow it to take
...
* A new device flag, ``RTE_ETH_DEV_NOLIVE_MAC_ADDR``, changes the order of
  ...
  flag the MAC can be properly configured in any case. This is partilwlarly
