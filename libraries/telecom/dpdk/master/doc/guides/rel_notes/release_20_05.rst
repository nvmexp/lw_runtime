...
* **Added APIs for RLW defer queues.**
...
* **Updated Hisilicon hns3 driver.**
  ...
  * Added support for configuring promislwous and allmulticast mode for VF.
...
* **Updated the QuickAssist Technology (QAT) Compression PMD.**
  ...
  not possible if any checksum callwlation was requested - in such cases the
...
* **Updated ipsec-secgw sample application.**
  ...
  * Updated the application to add event based packet processing. The worker
    ...
    event mode lwrrently only supports inline IPsec protocol offload.
...
* mempool: The API of ``rte_mempool_populate_iova()`` and
  ``rte_mempool_populate_virt()`` changed to return 0 instead of ``-EILWAL``
