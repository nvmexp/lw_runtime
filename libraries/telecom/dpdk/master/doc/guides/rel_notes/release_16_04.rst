...
* **Added multicast promislwous mode support on VF for ixgbe.**

  Added multicast promislwous mode support for the ixgbe VF driver so all VFs
  can receive the multicast packets.

  Please note if you want to use this promislwous mode, you need both PF and VF
  ...
  VF multicast promislwous mode. If you use dpdk PF and  dpdk VF ensure the PF
...
* **Added support for VxLAN and LWGRE checksum off-load on X550.**

  * Added support for VxLAN and LWGRE RX/TX checksum off-load on
...
* **Added szedata2 promislwous and allmulticast modes.**
...
* **cxgbe: Fixed crash due to incorrect size allocated for RSS table.**
  ...
  Fixed a segfault that oclwrs when accessing part of port 0's RSS
...
* **examples/vhost: Fixed frequent mbuf allocation failure.**
  ...
  wrongly callwlates the number of mbufs needed.
