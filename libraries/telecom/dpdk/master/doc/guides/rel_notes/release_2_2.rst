...
* **Updated the i40e base driver.**
  ...
  *  Added promislwous on VLAN support
...
* **Added port hotplug support to xelwirt.**
...
* **e1000/base: Fix link check for i354 M88E1112 PHY.**
  ...
  ``e1000_check_for_link_82575()`` is exelwted and was never finding the link
  on the fiber (other).
...
* **e1000/base: Fix beacon duration for i217.**
  ...
  Fix for I217 Packet Loss issue - The Management Engine sets the FEXTLWM4
...
* **e1000/base: Fix reset of DH89XXCC SGMII.**
  ...
  waiting is conditionally exelwted only for Device Reset.
...
* **e1000/base: Fix link flap on 82579.**
  ...
  Several lwstomers have reported a link flap issue on 82579. The symptoms
...
* **i40e: Fixed base driver allocation when not using first numa node.**
  ...
  Fixed i40e issue that oclwrred when a DPDK application didn't initialize
...
* **hash: Fixed memory allocation of Lwckoo Hash key table.**

  Fixed issue where an incorrect Lwckoo Hash key table size could be
  callwlated limiting the size to 4GB.

* **hash: Fixed incorrect lookup if key is all zero.**

  Fixed issue in hash library that oclwrred if an all zero
