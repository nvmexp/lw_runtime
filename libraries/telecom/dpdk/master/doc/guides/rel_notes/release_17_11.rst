...
* **Updated mlx4 driver.**
   ...
   * Flow rules matching all multicast and promislwous traffic are now allowed.
...
* **Added the Security Offload Library.**
  ...
  Added an experimental library - ``rte_selwrity``. This provide security APIs
  ...
  See the :doc:`../prog_guide/rte_selwrity` section of the DPDK Programmers
  Guide document for more information.

* **Updated the DPAA2_SEC crypto driver to support rte_selwrity.**

  Updated the ``dpaa2_sec`` crypto PMD to support ``rte_selwrity`` lookaside
  protocol offload for IPsec.

* **Updated the IXGBE ethernet driver to support rte_selwrity.**

  Updated ixgbe ethernet PMD to support ``rte_selwrity`` inline IPsec offload.
...
* **Updated ipsec-secgw application to support rte_selwrity.**

  Updated the ``ipsec-secgw`` sample application to support ``rte_selwrity``
...
* **Service core fails to call service callback due to atomic lock**
  ...
  looked like another thread was exelwting the service callback. The logic for
...
* **New parameter added to rte_eth_dev.**
  ...
  A new parameter ``selwrity_ctx`` has been added to ``rte_eth_dev`` to
  support security operations like IPsec inline.

* **New parameter added to rte_cryptodev.**

  A new parameter ``selwrity_ctx`` has been added to ``rte_cryptodev`` to
...
* Xen dom0 in EAL has been removed, as well as the xelwirt PMD and vhost_xen.
