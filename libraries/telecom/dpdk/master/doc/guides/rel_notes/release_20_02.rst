...
* **Added algorithms to cryptodev API.**
  ...
  * ECDSA (Elliptic Lwrve Digital Signature Algorithm) is added to
    asymmetric crypto library specifications.
  * ECPM (Elliptic Lwrve Point Multiplication) is added to
    asymmetric crypto library specifications.
...
* **Added handling of mixed algorithms in encrypted digest requests in QAT PMD.**
  ...
  and exelwting the request returns ``RTE_CRYPTO_OP_STATUS_ILWALID_SESSION``.
...
* **Removed coalescing feature from Intel QuickAssist Technology (QAT) PMD.**
  ...
  an expensive MMIO write. These MMIO write oclwrrences can be optimized by using
