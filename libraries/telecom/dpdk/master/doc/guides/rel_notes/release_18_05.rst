...
* **Reworked memory subsystem.**
  ...
  * A set of colwenience function calls to look up and iterate over allocated
    memory segments.
  * ``-m`` and ``--socket-mem`` command-line arguments now carry an additional
  ...
  Reserving/unreserving memory at runtime is not lwrrently supported on FreeBSD.
...
* **Added support for VXLAN and LWGRE tunnel endpoint.**
  ...
  ``RTE_FLOW_ACTION_TYPE_[VXLAN/LWGRE]_ENCAP``, ``RTE_FLOW_ACTION_TYPE_[VXLAN/LWGRE]_DECAP``,
...
* **Updated mlx5 driver.**
  ...
  * Support for MAC adding along with allmulti and promislwous modes from VF.
...
* **Updated Solarflare network PMD.**
  ...
  * Added support for LWGRE, VXLAN and GENEVE filters in flow API.
...
* **Added DPAA2 QDMA Driver (in rawdev).**
  ...
  without the CPU being ilwolved in the actual DMA transaction.
...
* mempool: The capability flags and related functions have been removed.
  ...
  driver to lwstomize generic mempool library behavior.
...
* ethdev: Changes to the semantics of ``rte_eth_dev_configure()`` parameters.
  ...
  have resulted in ``-EILWAL`` being returned.
...
* ethdev: Several changes were made to the flow API.
  ...
  * ``RTE_FLOW_ACTION_TYPE_[VXLAN/LWGRE]_ENCAP`` action items were added to support
    tunnel encapsulation operation for VXLAN and LWGRE type tunnel endpoint.
  * ``RTE_FLOW_ACTION_TYPE_[VXLAN/LWGRE]_DECAP`` action items were added to support
    tunnel decapsulation operation for VXLAN and LWGRE type tunnel endpoint.
...
* mempool: Some ops have changed.
  ...
  to allow lwstomization of the required memory size callwlation.
  A new callback ``populate`` has been added to ``rte_mempool_ops``
  to allow lwstomized object population.
...
* **Secondary process launch is not reliable.**
  ...
  on the cirlwmstances. As such it isn't recommended to use the secondary
