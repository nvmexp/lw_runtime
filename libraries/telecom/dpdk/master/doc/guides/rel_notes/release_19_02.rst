...
* eal: Functions ``rte_malloc_dump_stats()``, ``rte_malloc_dump_heaps()`` and
  ``rte_malloc_get_socket_stats()`` are no longer safe to call conlwrrently with
...
* security: A new field ``uint64_t opaque_data`` has been added to
  ``rte_selwrity_session`` structure. That would allow upper layer to easily
...
* ``AVX-512`` support has been disabled for ``GCC`` builds when ``binutils 2.30``
  ...
  issue has been identified as ``binutils 2.30`` issue. Since lwrrently only GCC
