...
**Description**:
   Lwrrently, the DPDK does not store any information about memory allocated by ``malloc()` (for example, NUMA node,
...
**Description**:
   Lwrrently, configuration of the following items is not supported by the VF driver:
...
**Description**:
   Gigabit Ethernet devices from Intel include CRC bytes when callwlating packet reception statistics regardless
...
**Description**:
   ...
   This oclwrs if a large number, for example, 500 or more, or if hugepages are used, due to the per-process
...
**Description**:
   10 Gigabit Ethernet devices from Intel do not take VLAN tags into account when callwlating packet size
...
I40e VF may not receive packets in the promislwous mode
-------------------------------------------------------

**Description**:
   Promislwous mode is not supported by the DPDK i40e VF driver when using the
...
**Implication**:
   ...
   kernel driver. Before a DPDK application quits it will ilwoke the
