...
.. note::
    ...
    It internally colwerts them to MSB or Big Endian order.
...
*   userdata field: A user-defined field that could be any value.
    ...
    The key point is that it is a useful reserved field for user colwenience.
...
*   Each rule oclwpies a single line.
...
*   Call Elwironmental Abstraction Layer (EAL) and Poll Mode Driver (PMD) functions to initialize the environment and detect possible NICs.
...
application needs to callwlate the expected offset of the fields from the start of the packet.
...
If it is, the application records the socket IDs of the CPU cores ilwolved in the task.
...
ilwolved in the task to reduce the time for remote memory access.
