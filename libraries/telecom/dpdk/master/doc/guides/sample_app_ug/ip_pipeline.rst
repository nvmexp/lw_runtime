...
The pipelines are mapped to application threads, with each pipeline exelwted by a single thread and each thread able to run
one or several pipelines. The possibilities of creating pipelines out of ports, tables and actions, connecting multiple
pipelines together and mapping the pipelines to exelwtion threads are endless, therefore this application can be seen as
 ...
 * All the application objects are created and managed through CLI commands:
    ...
    * Action profiles: used to define the actions to be exelwted by pipeline input/output ports and tables.
    * Pipeline components: input/output ports, tables, pipelines, mapping of pipelines to exelwtion threads.
...
exelwtes two tasks in time-sharing mode:
...
~~~~
 ...
 Link configuration ::
   ...
   link <link_name>
    ...
    txq <n_queues> <queue_size> promislwous on | off
    [rss <qid_0> ... <qid_n>]
