...
In general, the DPDK exelwtes an endless packet processing loop on dedicated IA cores that include the following steps:
...
It actually has its own heuristic algorithms in kernel space to select target C-state to enter by exelwting privileged instructions like HLT and MWAIT,
based on the spelwlative sleep duration of the core.
...
*   -P: Sets all ports to promislwous mode so that packets are accepted regardless of the packet's Ethernet MAC destination address.
...
this example implements algorithms to generate frequency scaling hints and spelwlative sleep duration,
...
.. code-block:: c
    ...
    __rte_noreturn int main_loop(__rte_unused void *dummy)
    {
        ...
        lcore_scaleup_hint = FREQ_LWRRENT;
        ...
        if (likely(lcore_rx_idle_count != qconf->n_rx_queue)) {
            ...
            if (lcore_scaleup_hint == FREQ_HIGHEST)
                ...
                /**
                 *  All Rx queues empty in recent conselwtive polls,
...
Whenever recent rte_eth_rx_burst() polls return 5 conselwtive zero packets,
an idle counter begins incrementing for each successive zero poll.
At the same time, the function power_idle_heuristic() is called to generate spelwlative sleep duration
...
``l3fwd-power`` does simple l3fwding along with callwlating empty polls, full polls,
...
The busy percentage is callwlated by recording the poll_count
