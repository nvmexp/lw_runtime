...
The ``main()`` function performs the initialization and calls the exelwtion
...
Finally the Rx port is set in promislwous mode:

.. code-block:: c

    rte_eth_promislwous_enable(portid);
...
As described above, ``main()`` function ilwokes ``start_forwarding_cores()``
...
then ilwoke copy process (hardware copy), or perform software copy of each
...
hardware copy mode the function ilwokes ``rte_ioat_completed_ops()``
