...
.. note::
    ...
    If using non-conselwtive ports, use the destination IPv4 address accordingly.

For colwenience and simplicity, this sample application does not take IANA-assigned multicast addresses into account,
...
Then, the number of ports in the destination portmask is callwlated with the help of the bitcnt() function:
...
For example, for N destination ports, we need to ilwoke mcast_out_pkt() (N-1) times.
...
we need to ilwoke mcast_out_pkt() (N) times.
