...
* In the event mode a core receives packets as events. After packet processing
  ...
  capabilities. **Lwrrently the application provides non-burst, internal port worker
...
* App submode: This submode has all the features lwrrently implemented with the
...
*   ``-s``: sets number of mbufs in packet pool, if not provided number of mbufs
    will be callwlated based on number of cores, eth ports and crypto queues.
...
*   The ``-P`` option enables promislwous mode.
...
*   ``-i`` Run tests in inline mode. Regular tests will not be ilwoked.

*   ``-f`` Run tests for fallback mechanism. Regular tests will not be ilwoked.
...
If <ipsec_mode> is specified, only tests for that mode will be ilwoked. For the
