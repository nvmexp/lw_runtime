...
module loading using::
    ...
    kelw hw.contigmem.num_buffers=n
    kelw hw.contigmem.buffer_size=m
...
.. note::
    ...
    If the ports to be used are lwrrently bound to a existing kernel driver
...
Lwrrently loaded modules can be seen by using the ``kldstat`` command and a module
...
kernel environment variable prior to loading ``nic_uio``, as follows::
    ...
    kelw hw.nic_uio.bdfs="b:d:f,b:d:f,..."
...
upon loading, use the following command::
    ...
    kelw hw.nic_uio.bdfs="2:0:0,2:0:1"
...
``nic_uio`` module, modify or clear the ``hw.nic_uio.bdfs`` kernel environment (kelw)
