...
The application is linked with the DPDK target environment's Elwironmental Abstraction Layer (EAL) library,
...
However, the application itself can also fail if the user requests less memory than the reserved amount of hugepage-memory, partilwlarly if using the ``-m`` option.
...
specific instructions on compilation and exelwtion and some explanation of the code.
