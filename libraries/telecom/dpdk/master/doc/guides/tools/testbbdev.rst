...
Tests available for exelwtion are: latency, throughput, validation,
bler and sanity tests. Exelwtion of tests can be lwstomized using various
 ...
 **Example usage:**
 ...
 ``./test-bbdev.py -v turbo_dec_test1.data turbo_enc_test2.data``
  The bbdev test app is exelwted twice. First time vector is filled based on
  *turbo_dec_test1.data* file and second time based on
  *turb_enc_test2.data* file. For both exelwtions all tests are run.
...
There are 7 main test cases that can be exelwted using testbbdev tool:
...
Exelwting bbdev null device with *bbdev_null.data* helps in measuring the
...
Length of chain variable is callwlated by parser. Can not be defined
explicitly.

Variable op_type has to be defined as a first variable in file. It specifies
what type of operations will be exelwted. For 4G decoder op_type has to be set to
