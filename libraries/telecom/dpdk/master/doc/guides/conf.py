...
release = elwiron.setdefault('DPDK_VERSION', "None")
version = release
...
lwstom_latex_preamble = r"""
\usepackage{textalpha}
\RelwstomVerbatimElwironment{Verbatim}{Verbatim}{xleftmargin=5mm}
\usepackage{etoolbox}
\robustify\(
\robustify\)
"""
...
latex_elements = {
    ...
    # lwstomize Latex formatting
    'preamble': lwstom_latex_preamble
}


# Override the default Latex formatter in order to modify the
# code/verbatim blocks.
class LwstomLatexFormatter(LatexFormatter):
    def __init__(self, **options):
        super(LwstomLatexFormatter, self).__init__(**options)
        # Use the second smallest font size for code/verbatim blocks.
        self.verboptions = r'formatcom=\footnotesize'

# Replace the default latex formatter.
PygmentsBridge.latex_formatter = LwstomLatexFormatter
...
def numref_role(reftype, rawtext, text, lineno, inliner):
    """
    Add a Sphinx role to handle numref references. Note, we can't colwert
...
def process_numref(app, doctree, from_docname):
    ...
    writing the files. The processing ilwolves replacing the numref with a
    ...
    for node in doctree.traverse(nodes.reference):
        ...
        if target.startswith('_local_numref_#'):
            ...
            # Get the target label and link information from the Sphinx elw.
            data = app.builder.elw.domains['std'].data
