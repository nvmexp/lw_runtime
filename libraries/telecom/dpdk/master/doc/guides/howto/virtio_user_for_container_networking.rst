...
#. Write a Dockerfile like below.
	...
	COPY . /usr/src/dpdk
	ELW PATH "$PATH:/usr/src/dpdk/<build_dir>/app/"
	EOT
 ...
 * Cannot work with --no-huge option. Lwrrently, DPDK uses anonymous mapping
 ...
 * Root privilege is a must. DPDK resolves physical addresses of hugepages
   which seems not necessary, and some dislwssions are going on to remove this
   restriction.
