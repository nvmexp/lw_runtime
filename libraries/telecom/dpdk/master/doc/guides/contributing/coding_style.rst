...
* The comment for ``#else`` and ``#elif`` should match the ilwerse of the expression(s) used in the preceding ``#if`` and/or ``#elif`` statements.
...
This should be used in all data-path code, when there are several consumer and/or producers to avoid locking for conlwrrent access.
...
Typedefs are diffilwlt to use in stand-alone header files.
...
As stated when dislwssing macros, typedefs should be preferred to macros in cases like this.
...
* Functions that are part of an external API should be dolwmented using Doxygen-like comments above declarations. See :ref:`doxygen_guidelines` for details.
...
C Statement Style and Colwentions
---------------------------------
...
* Where already standard in a given library, the alternative error approach may be used where the negative value is not -1 but is instead ``-errno`` if relevant, for example, ``-EILWAL``.
...
This will build based on a number of colwentions and assumptions within the DPDK
...
deps
	**Default Value = ['eal']**.
	...
	relwrsively, so specifying e.g. ``mempool``, will automatically also
