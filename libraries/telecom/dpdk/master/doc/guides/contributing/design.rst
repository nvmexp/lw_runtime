...
In DPDK and DPDK applications, some code is specific to an architecture (i686, x86_64) or to an exelwtive environment (freebsd or linux) and so on.
As far as is possible, all such instances of architecture or elw-specific code should be provided via standard APIs in the EAL.
...
A file located in a subdir of "linux" is specific to this exelwtion environment.

.. note::

   Code in DPDK libraries and applications should be generic.
   The correct location for architecture or exelwtive environment specific code is in the EAL.
...
Per Exelwtion Environment Sources
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The following macro options can be used:

* ``RTE_EXEC_ELW`` is a string that contains the name of the exelwtive environment.
* ``RTE_EXEC_ELW_FREEBSD``, ``RTE_EXEC_ELW_LINUX`` or ``RTE_EXEC_ELW_WINDOWS`` are defined only if we are building for this exelwtion environment.
...
allocated/freed, number of oclwrrences for specific events, etc.
...
* Complexity of the statistics logic collection for each counter: when only
  some oclwrrences of a specific event are valid, additional logic is typically
  needed to decide whether the current oclwrrence of the event should be counted
