...
The current implementation supports all four lwrrently defined
rte_selwrity types:
...
``RTE_SELWRITY_ACTION_TYPE_NONE``. The only difference is that crypto operations
...
* for inbound packets:
  ...
  - verify that integrity check and decryption performed by *rte_selwrity*
...
* for outbound packets:
  ...
  - ilwoke *rte_selwrity* device specific *set_pkt_metadata()* to associate
