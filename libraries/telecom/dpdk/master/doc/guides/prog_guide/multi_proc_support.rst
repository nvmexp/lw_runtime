...
These are more fully dolwmented in the "Multi- process Sample Application" chapter
...
In addition to the above scenarios ilwolving multiple DPDK processes working together,
...
Some of these are dolwmented below:
...
To work around this issue, it is recommended that multi-process applications perform the hash callwlations by directly calling
...
performance-critical, but rather is intended to be a colwenient, general
...
Sending requests ilwolves waiting for the other side to reply, so they can block
...
Due to the underlying IPC implementation being single-threaded, relwrsive
