...
to perform those services. The framework lwrrently only supports lossless compression schemes:
Deflate and LZS.
...
which is exelwted at DPDK initialization, based on their unique device identifier.
...
Following are lwrrently supported:
...
- RTE_COMP_OP_STATUS_ILWALID_ARGS,
    Operation failed due to invalid arguments in request

- RTE_COMP_OP_STATUS_ERROR,
    Operation failed because of internal error

- RTE_COMP_OP_STATUS_ILWALID_STATE,
    Operation is ilwoked in invalid state
...
to be successful. If the op holds any other status, e.g. RTE_COMP_OP_STATUS_ILWALID_ARGS, a retry with
...
Compression API allows application to enable digest callwlation
...
If supported, PMD callwlates digest always on plaintext i.e.
before compression and after decompression.

Lwrrently supported list of hash algos are SHA-1 and SHA2 family
SHA256.
...
A Stateful operation in DPDK compression means application ilwokes enqueue
