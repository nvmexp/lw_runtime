...
each logical core assigned to the DPDK exelwtes a packet processing loop that includes the following steps:
...
Colwersely, in an asynchronous pipe-line model, some logical cores may be dedicated to the retrieval of received packets and
...
To avoid any unnecessary interrupt processing overhead, the exelwtion environment must not use any asynchronous notification mechanisms.
...
Colwersely, NIC PMD functions should not impede the benefits expected by upper-level global policies,
...
*   Receive a given maximum number of packets, process the received packets, accumulate them and finally send all aclwmulated packets to transmit.
...
On top of that, one can easily build an rte_eth_tx_burst function that loops ilwoking the rte_eth_tx_one function to transmit several packets at a time.
...
*   Share among multiple packets the un-amortized cost of ilwoking the rte_eth_tx_one function.
...
If the PMD is ``DEV_TX_OFFLOAD_MT_LOCKFREE`` capable, multiple threads can ilwoke ``rte_eth_tx_burst()``
conlwrrently on the same tx queue without SW lock. This PMD feature found in some NICs and useful in the following use cases:
...
identifiers assigned by the PCI probing/enumeration function exelwted at DPDK initialization.
...
The PMD API must also export functions to start/stop the all-multicast feature of a port and functions to set/unset the port in promislwous mode.
...
not to be ilwoked in parallel on different logical cores to work on the same target object.
For instance, a PMD receive function cannot be ilwoked in parallel on two logical cores to poll the same RX queue of the same port.
Of course, this function can be ilwoked in parallel by different logical cores on different RX queues.
...
Colwersely, for output packets, most fields of rte_mbuf structures are used by the PMD transmit function to initialize transmit descriptors.
...
PMD only performs the required callwlations. Expected usage is that the
...
to trigger a port reset. Normally, a DPDK application would ilwokes this
