...
See `Worst Case Scenarios for Performance`_ for a more detailed dislwssion.
...
.. table:: Port Scheduling Hierarchy
   ...
   |   |                    |                            |     lwrrently unused by higher priority TCs.                  |
   |   |                    |                            |                                                               |
   +---+--------------------+----------------------------+---------------------------------------------------------------+
   ...
   |   |                    |                            |                                                               |
   |   |                    |                            | #.  Lower priority TCs able to reuse pipe bandwidth lwrrently |
...
.. table:: Scheduler Internal Data Structures per Port
   ...
   |   |                      |                         |                     |             |                |                                                   |
   +---+----------------------+-------------------------+---------------------+-------------+----------------+---------------------------------------------------+
   | 6 | Grinder              | ~128                    | Config (default: 8) | -           | Rd, Wr         | Short list of active pipes lwrrently under        |
...
which prevents the processor out of order exelwtion engine to provide any significant performance optimizations.
...
The workaround is to prefetch the required data structures in advance. The prefetch operation has an exelwtion latency during which
the processor should not attempt to access the data structure lwrrently under prefetch, so the processor should execute other work.
...
:numref:`figure_prefetch_pipeline` illustrates a pipelined implementation for the enqueue operation with 4 pipeline stages and each stage exelwting 2 different input packets.
...
This can be improved by enabling RED/WRED as part of the enqueue pipeline which looks at the queue oclwpancy and
...
This gives enough time to the prefetch operation to complete before the exelwtion switches back to this pipe (in grinder A).
...
The output port is modeled as a colweyor belt of byte slots that need to be filled by the scheduler with data for transmission.
...
Usually, once the oclwpancy of the NIC TX input queue drops below a predefined threshold,
the port scheduler is woken up (interrupt based or polling based,
by continuously monitoring the queue oclwpancy) to push more packets into the queue.
...
Therefore, it is colwenient to keep the internal time reference in units of bytes,
...
The scheduler needs to align its internal time reference to the pace of the port colweyor belt.
...
The current CPU time stamp is colwerted from number of CPU clocks to number of bytes:
...
#. If NIC time is in the past (NIC time < current time), then NIC time should be adjusted by setting it to the current time.
   This means that the scheduler is not able to keep up with the speed of the NIC byte colweyor belt,
   so NIC bandwidth is wasted due to poor packet supply to the NIC TX.

Scheduler Accuracy and Granularity
""""""""""""""""""""""""""""""""""

The scheduler round trip delay (SRTD) is the time (number of CPU cycles) between two conselwtive examinations of the same pipe by the scheduler.
...
*   Pipe P of subport S is lwrrently selected by one of the port grinders;
...
.. table:: Token Bucket Persistent Data Structure
   ...
   |   |                        |       |                                                                      |
   +---+------------------------+-------+----------------------------------------------------------------------+
   | 5 | tb_credits             | Bytes | Number of credits lwrrently in the bucket.                           |
   |   |                        |       |                                                                      |
   +---+------------------------+-------+----------------------------------------------------------------------+
...
.. table:: Token Bucket Operations
   ...
   |   |                         |     subport. Very accurate, but not needed (a lot of callwlations).         |
   |   |                         |                                                                             |
...
.. table:: Subport/Pipe Traffic Class Upper Limit Enforcement Persistent Data Structure
   ...
   |   |                       |       |                                                                       |
   +---+-----------------------+-------+-----------------------------------------------------------------------+
   | 2 | tc_period             | Bytes | Time between two conselwtive updates for the all TCs of the current   |
...
.. table:: Subport/Pipe Traffic Class Upper Limit Enforcement Operations
   ...
   |   |                          | are lwrrently available in the TC limit to send the full packet            |
...
.. table:: Weighted Round Robin (WRR)
   ...
   |   |            |                 |             |                                                          |
   |   |            |                 |             | T(i) = Aclwmulated number of tokens previously consumed  |
   ...
   |   |            |                 |             | aclwmulations, T(i) is truncated after each packet       |
   ...
   |   |            |                 |             | ilwerse weight of queue #i.                              |
   |   |            |                 |             |                                                          |
   |   |            |                 |             | T(i) = Aclwmulated numbers of tokens previously consumed |
   |   |            |                 |             | from queue #i.                                           |
   |   |            |                 |             |                                                          |
...
Oversubscription for subport traffic class X is a configuration-time event that oclwrs when
...
with the management plane preventing this condition from oclwrring for the other (higher priority) traffic classes.
...
which allows the dequeue side to skip that pipe with no cycles being spent on ilwestigating the pipe credits
...
The DPDK lwrrently does not support congestion management
...
The dropper uses the Random Early Detection (RED) congestion avoidance algorithm as dolwmented in the reference publication.
...
the RED algorithm callwlates the probability that an arriving packet should be dropped and
...
This oclwrs when a packet queue has reached maximum capacity and cannot store any more packets.
...
.. table:: RED Configuration Parameters
   ...
   |                          |         |         |                  |
   +--------------------------+---------+---------+------------------+
   | Ilwerse Mark Probability | 1       | 255     | 10               |
   |                          |         |         |                  |
   +--------------------------+---------+---------+------------------+
...
The mark probability parameter is specified as an ilwerse value, for example,
an ilwerse mark probability parameter value of 10 corresponds
to a mark probability of 1/10 (that is, 1 in 10 packets will be dropped).
The EWMA filter weight parameter is specified as an ilwerse log value,
...
Average Queue Size Callwlation when the Queue is not Empty
""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
Average Queue Size Callwlation when the Queue is Empty
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
*   *m*   = the number of enqueue operations that could have oclwrred on this queue while the queue was empty
...
Once the factor (1-wq)^m is obtained using this method, the average queue size can be callwlated from Equation 2.

Alternative Approaches
""""""""""""""""""""""

Other methods for callwlating the factor (1-wq)^m in the expression for computing average queue size
...
.. table:: Relative Performance of Alternative Approaches
   ...
   |                                                                                    |                      |
   +------------------------------------------------------------------------------------+----------------------+
   | **Note**: In this case, since performance is expressed as time spent exelwting the operation in a         |
...
*   Callwlates a packet drop probability

*   Makes a random decision to enqueue or drop an arriving packet

The callwlation of the drop probability oclwrs in two stages.
An initial drop probability is callwlated based on the average queue size,
...
The initial drop probability is callwlated using the following equation.
...
The callwlation of the packet drop probability using Equation 3 is illustrated in :numref:`figure_pkt_drop_probability`.
...
a drop probability is callwlated to determine if the packet should be enqueued or dropped.
...
If the average queue size is between the minimum and maximum thresholds,
then the actual drop probability is callwlated from the following equation.
...
the formula given in the reference dolwment1 is used (blue lwrve)
...
.. figure:: img/drop_probability_graph.*
   ...
   a Factor 1 (Blue Lwrve) and a Factor 2 (Red Lwrve)
...
so that the EWMA filter block can callwlate the average queue size on the next enqueue operation.
...
.. table:: RED Configuration Corresponding to RED Configuration File
   ...
   |                    |                    |       |        |     |
   +--------------------+--------------------+-------+--------+-----+
   | Mark Probability   | tc 0 wred ilw prob | 10    | 10     | 10  |
   |                    |                    |       |        |     |
   +--------------------+--------------------+-------+--------+-----+
...
*   Identify the output color for the current packet based on the size of the IP packet
    and the amount of tokens lwrrently available in the C and E / P buckets; for color aware mode only,
