...
in the underlying hardware); that is, applications must explicitly ilwoke the
...
#. In addition, the GSO library doesn't re-callwlate checksums for segmented
...
#. Lwrrently, the GSO library supports the following IPv4 packet types:
...
#. First create a GSO context ``(struct rte_gso_ctx)``; this contains:
   ...
   - the bit mask of required GSO types. The GSO library uses the same macros as
     ...
     ``DEV_TX_OFFLOAD_TCP_TSO``. The only other supported values lwrrently
...
2. Set the appropriate ol_flags in the mbuf.
   ...
   - If checksum callwlation in hardware is required, the application should
...
#. Ilwoke the GSO segmentation API, ``rte_gso_segment()``.
...
#. If required, update the L3 and L4 checksums of the newly-created segments.
   ...
   updated. Alternatively, the application may offload checksum callwlation
   to HW.
