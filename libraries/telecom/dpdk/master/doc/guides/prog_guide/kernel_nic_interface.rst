...
There are lwrrently four callbacks for which the user can register
...
``config_promislwsity``:
    ...
    Called when the user changes the promislwity state of the KNI
    ...
    callback handler in the rte_kni library ``kni_config_promislwsity()``
    will be called which calls ``rte_eth_promislwous_<enable|disable>()``
    on the specified ``port_id``.
...
and the next pointers in mbuf-chain will colwert to physical address.
The KNI thread will poll all KNI active devices for the rx_q.
If an mbuf is dequeued, it will be colwerted to a sk_buff and sent to the net stack via netif_rx().
The dequeued mbuf must be freed, so the same pointer is sent back in the free_q FIFO,
and next pointers must colwert back to virtual address if exists before put in the free_q FIFO.

The RX thread, in the same main loop, polls this FIFO and frees the mbuf after dequeuing it.
The address colwersion of the next pointer is to prevent the chained mbuf
