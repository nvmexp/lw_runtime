...
partilwlarity allows them to be specified anywhere in the stack without
...
- Its numeric value is 0 for colwenience.
...
Used as a placeholder for colwenience. It is ignored and simply discarded by
PMDs.
...
Ilwerted matching, i.e. process packets that do not match the pattern.

- ``spec``, ``last`` and ``mask`` are ignored.

.. _table_rte_flow_item_ilwert:
...
.. _table_rte_flow_item_ilwert_example:
...
Matches a LWGRE header (RFC 7637).
...
Matches a VXLAN-GPE header (draft-ietf-lwo3-vxlan-gpe-05).
...
.. _table_rte_flow_action_selwrity:
...
.. table:: SECURITY
   ...
   | Field                | Value                                |
   +======================+======================================+
   | ``selwrity_session`` | security session to apply            |
   +----------------------+--------------------------------------+
...
``rte_selwrity_session``. The SA is identified according to the IP and ESP
...
Performs a LWGRE encapsulation action by encapsulating the matched flow in the
LWGRE tunnel as defined in the``rte_flow_action_tunnel_encap`` flow item
definition.

This action modifies the payload of matched flows. The flow definition specified
in the ``rte_flow_action_tunnel_encap`` action structure must defined a valid
LWGRE network overlay which conforms with RFC 7637 (LWGRE: Network
...
.. _table_rte_flow_action_lwgre_encap:
...
.. table:: LWGRE_ENCAP
   ...
   | Field          | Value                               |
   +================+=====================================+
   | ``definition`` | LWGRE end-point overlay definition  |
   +----------------+-------------------------------------+

.. _table_rte_flow_action_lwgre_encap_example:

.. table:: IPv4 LWGRE flow pattern example.
...
Performs a decapsulation action by stripping all headers of the LWGRE tunnel
network overlay from the matched flow.

The flow items pattern defined for the flow rule with which a ``LWGRE_DECAP``
action is specified, must define a valid LWGRE tunnel as per RFC7637. If the
flow pattern does not specify a valid LWGRE tunnel then a
RTE_FLOW_ERROR_TYPE_ACTION error should be returned.
...
Colwenience function to destroy all flow rule handles associated with a
...
underlying device or unloading the PMD ilwalidates it.
...
Colwert ``src`` to ``dst`` according to operation ``op``. Possible
operations include:
...
It is lwrrently implemented on top of the legacy filtering framework through
...
- PMD callbacks implement exactly the interface described in `Rules
  ...
  colwerted to a pointer to the underlying ``struct rte_eth_dev``.
...
This pattern item stands for anything, which can be diffilwlt to translate
to something hardware would understand, partilwlarly if followed by more
specific types.
...
For overlapping rules (partilwlarly in order to use `Action: PASSTHRU`_)
