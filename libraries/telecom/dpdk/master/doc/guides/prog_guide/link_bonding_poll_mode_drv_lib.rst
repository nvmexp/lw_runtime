...
Lwrrently the Link Bonding PMD library supports following modes of operation:
...
.. figure:: img/bond-mode-2.*
   ...
   Balance XOR (Mode 2)
    ...
    a simple callwlation based on the packet flow source and destination MAC
    ...
    destination addresses into the callwlation of the transmit slave port and
...
.. note::
    ...
    classification callwlated by the selected transmit policy
...
that are assumed not to be ilwoked in parallel on different logical cores to
work on the same target object.

It should also be noted that the PMD receive function should not be ilwoked
...
*   **Layer 2:**   Ethernet MAC address based balancing is the default
    ...
    callwlation on the source MAC address and destination MAC address of the
    packet and then callwlate the modulus of this value to callwlate the slave
...
*   mode: Integer value defining the bonding mode of the device.
    Lwrrently supports modes 0,1,2,3,4,5 (round-robin, active backup, balance,
