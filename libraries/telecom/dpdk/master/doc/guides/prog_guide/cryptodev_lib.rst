...
lwrrently only supports cipher, authentication, chained cipher/authentication
...
Lwrrently the following Crypto device features are defined:
...
transform identifier and then the partilwlars of the transform. For the full
...
The cryptodev library lwrrently provides support for the following symmetric
...
expand cipher keys and HMAC IPADs and OPADs, which need to be callwlated for a
...
Lwrrently there are three transforms types cipher, authentication and AEAD.
...
For more details, e.g. how to colwert an mbuf to an SGL, please refer to an
...
buffer and attach either the cryptodev sym session, the rte_selwrity session,
...
- ``0``: the operation(s) is/are cached successfully in the crypto device queue
  ...
  once.
- The negative integer: error oclwrred during enqueue.
...
ilwalidate any operation stored in the device queue but not enqueued. This
...
- The negative integer: error oclwrred during dequeue.

Calling ``rte_cryptodev_configure_raw_dp_ctx`` with the parameter ``is_update``
set as 0 twice without the dequeue functions exelwtion changed dequeue_status
...
The cryptodev library lwrrently provides support for the following asymmetric
Crypto operations; RSA, Modular exponentiation and ilwersion, Diffie-Hellman
...
function would allow an application to callwlate the max device asymmetric
...
Not all asymmetric crypto xforms are supported for chaining. Lwrrently supported
asymmetric crypto chaining is Diffie-Hellman private key generation followed by
public generation. Also, lwrrently API does not support chaining of symmetric and
asymmetric crypto xforms.

Each xform defines specific asymmetric crypto algo. Lwrrently supported are:
* RSA
* Modular operations (Exponentiation and Ilwerse)
* Diffie-Hellman
* DSA
...
.. code-block:: c
	...
	/* callwlate mod exp of value 0xf8 */
