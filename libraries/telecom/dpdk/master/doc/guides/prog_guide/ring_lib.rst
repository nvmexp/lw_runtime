...
Each figure represents a simplified state of the ring, which is a cirlwlar buffer.
...
This section explains what oclwrs when a producer adds an object to the ring.
...
This section explains what oclwrs when a consumer dequeues an object from the ring.
...
This section explains what oclwrs when two producers conlwrrently add an object to the ring.
In this example, only the producer head and tail (prod_head and prod_tail) are modified.
...
.. note::
    ...
    In real exelwtion cases, this is redundant for low efficiency, but is done automatically when the result overflows.
