...
Conlwrrent Access to the Same Memory Area
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
.. code-block:: c
    ...
    if (mlockall(MCL_LWRRENT | MCL_FUTURE)) {
...
causing the processor's LOCK# signal to be asserted during exelwtion of the following instruction.
...
For instance, the Read-Copy-Update (RLW) algorithm can frequently replace simple rwlocks.
...
Reader-Writer Conlwrrency
^^^^^^^^^^^^^^^^^^^^^^^^^

Lock-free reader-writer conlwrrency is one of the common use cases in DPDK.
