...
function. The callback function is ilwoked if the adapter needs to use a
...
ilwokes the ``rte_eth_rx_burst()`` to receive packets on the queue and
colwerts the received packets to events in the same manner as packets
