...
.. table:: Port Types
   ...
   |   |                  |                                                                                       |
   +===+==================+=======================================================================================+
   | 1 | SW ring          | SW cirlwlar buffer used for message passing between the application threads. Uses     |
...
#.  *add (key, value)*: When no value is lwrrently associated with *key*, then the (key, *value* ) association is created.
...
#.  *delete key*: When no value is lwrrently associated with *key*, this operation has no effect.
    When *key* is already associated  *value*, then association  *(key, value)* is removed;

#.  *lookup key*: When no value is lwrrently associated with  *key*, then this operation returns void value (lookup miss).
...
so the key search can be narrowed down from the full set of keys lwrrently in the table
to just the set of keys lwrrently in the identified table bucket.
...
In packet processing context, the sequence of operations ilwolved in hash table operations is described in :numref:`figure_figure33`:
...
*Description:* The flow classification is exelwted at least once for each input packet.
...
cache memory space, cache memory bandwidth, external memory bandwidth, multiple exelwtion units working in parallel,
out of order instruction exelwtion, special CPU instructions, etc.
...
two packets (that just completed stage 2) are now exelwting stage 3, two packets (that just completed stage 1) are now exelwting stage 2,
two packets (that just completed stage 0) are now exelwting stage 1 and two packets (next two packets to read from the burst of input packets)
...
when the next pipeline stage eventually gets exelwted for the same packets,
it will read the data structures it needs from L1 or L2 cache memory and thus avoid the significant penalty inlwrred by L2 or L3 cache memory miss.

By prefetching the data structures required by the next pipeline stage in advance (before they are used)
and switching to exelwting another pipeline stage for different packets,
...
the CPU exelwtion units have to stall until the read operation is completed from L3 cache memory or external DRAM memory.
...
By splitting the processing into several stages that are exelwted on different packets (the packets from the input burst are interlaced),
...
For example, for the 4-stage pipeline, stage 0 is exelwted on packets 0 and 1 and then,
before same packets 0 and 1 are used (i.e. before stage 1 is exelwted on packets 0 and 1),
different packets are used: packets 2 and 3 (exelwting stage 1), packets 4 and 5 (exelwting stage 2) and packets 6 and 7 (exelwting stage 3).
By exelwting useful work while the data structures are brought into the L1 or L2 cache memory, the latency of the read memory accesses is hidden.
By increasing the gap between two conselwtive accesses to the same data structure, the data dependency between instructions is loosened;
this allows making the best use of the super-scalar and out-of-order exelwtion CPU architecture,
as the number of CPU core exelwtion units that are active (rather than idle or stalled due to data dependency constraints between instructions) is maximized.

The bucket search logic is also implemented without using any branch instructions.
This avoids the important cost associated with flushing the CPU core exelwtion pipeline on every instance of branch misprediction.
...
.. table:: Field Description for Bucket Array Entry (Configurable Key Size Hash Tables)
   ...
   |   |                  |                    | is lwrrently extended or NULL otherwise.                         |
...
#.  The pipelined version of the bucket search algorithm is exelwted only if there are at least 7 packets in the burst of input packets.
    If there are less than 7 packets in the burst of input packets,
    a non-optimized implementation of the bucket search algorithm is exelwted.

#.  Once the pipelined version of the bucket search algorithm has been exelwted for all the packets in the burst of input packets,
    the non-optimized implementation of the bucket search algorithm is also exelwted for any packets that did not produce a lookup hit,
    but have the *match_many* flag set.
    As result of exelwting the non-optimized version, some of these packets may produce a lookup hit or lookup miss.
...
.. table:: Field Description for Bucket Array Entry (8-byte and 16-byte Key Hash Tables)
   ...
   |   |               |                    | next pointer is not NULL if the bucket is lwrrently extended or NULL          |
   |   |               |                    | otherwise.                                                                    |
   |   |               |                    |                                                                               |
   +---+---------------+--------------------+-------------------------------------------------------------------------------+
...
#.  The pipelined version of the bucket search algorithm is exelwted only if there are at least 5 packets in the burst of input packets.
    If there are less than 5 packets in the burst of input packets, a non-optimized implementation of the bucket search algorithm is exelwted.
...
The table entries contain the actions defining the operations to be exelwted on the input packets and the packet flow within the pipeline.
...
#.  The table connectivity to other tables or to output ports is regulated by the next hop actions of each table entry and the default table entry.
    ...
    *   A table can have multiple entries (including the default entry) connected to another table,
        ...
        with the purpose of simplifying the implementation of the pipeline run-time exelwtion engine.
...
An action handler can be assigned to each input/output port to define actions to be exelwted on each input packet that is received by the port.
Defining the action handler for a specific input/output port is optional (i.e. the action handler can be disabled).

For input ports, the action handler is exelwted after RX function. For output ports, the action handler is exelwted before the TX function.
...
An action handler to be exelwted on each input packet can be assigned to each table.
Defining the action handler for a specific table is optional (i.e. the action handler can be disabled).

The action handler is exelwted after the table lookup operation is performed and the table entry associated with each input packet is identified.
...
*   Packet headers: Each packet typically has one or multiple headers. The headers are extracted from the input packet as part of the packet parsing operation,
    which is likely exelwted immediately after the packet reception. As result of the extract operation, each header is logically removed from the packet, so
...
*   Actions: The pipeline actions are dynamically defined through instructions as opposed to predefined. Essentially, the actions are subroutines of the
    pipeline program and their exelwtion is triggered by the table lookup. The input arguments of each action are read from the table entry (in case of table
...
*   Pipeline: The pipeline represents the main program that defines the life of the packet, with subroutines (actions) exelwted on table lookup. As packets
