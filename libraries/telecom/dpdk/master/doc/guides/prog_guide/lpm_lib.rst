...
*   If RLW is not used, tbl8s are cleared and reclaimed immediately.

*   If RLW is used, tbl8s are reclaimed when readers are in quiescent state.

When the LPM is not using RLW, tbl8 group can be freed immediately even though the readers might be using
the tbl8 group entries. This might result in incorrect lookup results.

RLW QSBR process is integrated for safe tbl8 group reclamation. Application has certain responsibilities
while using this feature. Please refer to resource reclamation framework of :ref:`RLW library <RLW_Library>`
for more details.
