...
* ``rte_vhost_driver_register(path, flags)``
  ...
  - ``RTE_VHOST_USER_ASYNC_COPY``
    ...
    Lwrrently this feature is only implemented on split ring enqueue data
    path.
...
* ``rte_vhost_driver_callback_register(path, vhost_device_ops)``
  ...
  * ``new_device(int vid)``
    ...
    This callback is ilwoked when a virtio device becomes ready. ``vid``
    is the vhost device ID.

  * ``destroy_device(int vid)``

    This callback is ilwoked when a virtio device is paused or shut down.

  * ``vring_state_changed(int vid, uint16_t queue_id, int enable)``

    This callback is ilwoked when a specific queue's state is changed, for
    example to enabled or disabled.

  * ``features_changed(int vid, uint64_t features)``

    This callback is ilwoked when the features is changed. For example,
    ``VHOST_F_LOG_ALL`` will be set/cleared at the start/end of live
    migration, respectively.

  * ``new_connection(int vid)``

    This callback is ilwoked on new vhost-user socket connection. If DPDK
    acts as the server the device should not be deleted before
    ``destroy_connection`` callback is received.

  * ``destroy_connection(int vid)``

    This callback is ilwoked when vhost-user socket connection is closed.
...
* ``rte_vhost_driver_start(path)``
  ...
  This function triggers the vhost-user negotiation. It should be ilwoked at
...
* ``rte_vhost_async_channel_register(vid, queue_id, features, ops)``
  ...
  * ``async_inorder``
    ...
    Lwrrently, only ``async_inorder`` capable device is supported by vhost.
  ...
  * ``transfer_data(vid, queue_id, descs, opaque_data, count)``
    ...
    vhost ilwokes this function to submit copy data to the async devices.
  ...
  * ``check_completed_copies(vid, queue_id, opaque_data, max_packets)``
    ...
    vhost ilwokes this function to get the copy data completed by async
    devices.
