...
*   Entering into different deeper C-States according to the adaptive algorithms to spelwlate
...
Core state can be altered by spelwlative sleeps whenever the specified lcore has nothing to do.
In the DPDK, if no packet is received after polling,
spelwlative sleeps can be triggered according the strategies defined by the user space application.
...
those cores are doing. It is critical to aclwrately determine how busy
...
Proposed Solution
~~~~~~~~~~~~~~~~~
The proposed solution folwses on how many times empty polls are exelwted.
...
* LOW:  Not lwrrently used, reserved for future use.
