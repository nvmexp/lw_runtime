...
thereby consumes much less space than colwentional flow-based tables.
...
Folwsing on one group that has four keys, :numref:`figure_efd8` depicts the search
...
slow, because of perfect hash search as previously dislwssed).
...
When dislwssing EFD inserts and lookups, the dislwssion is simplified by
