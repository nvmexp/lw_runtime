...
The Timer library provides a timer service to DPDK exelwtion units to enable exelwtion of callback functions asynchronously.
Features of the library are:

*   Timers can be periodic (multi-shot) or single (one-shot).

*   Timers can be loaded from one core and exelwted on another. It has to be specified in the call to rte_timer_reset().
