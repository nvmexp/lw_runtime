...
Like for standard Ethernet switches, this ilwolves a combination of
...
logic applies to any number of ports without necessarily ilwolving SR-IOV.
...
- The device or group relationship of ports can be discovered using the
  ...
  ``RTE_ETH_DEV_SWITCH_DOMAIN_ID_ILWALID`` to indicate that the port doesn't
...
The following diagram pictures a setup ilwolving a device with one PF, two
...
them (e.g. assigning a MAC address or setting up promislwous mode) affects
...
concept. Interconnection oclwrs when hardware configuration enables traffic
flows from one place to another (e.g. physical port 0 to VF 1) according to
some criteria.

This is dislwssed in more detail in `traffic steering`_.
...
a `basic SR-IOV`_ configuration ilwolving physical port 0 could be broken
down as:
...
Flow rules are lwrrently restricted to at most a single action of each
...
rules of varying priority levels is prohibitively diffilwlt to implement
