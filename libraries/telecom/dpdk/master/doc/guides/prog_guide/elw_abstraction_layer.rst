...
.. _Elwironment_Abstraction_Layer:
...
*   Core Affinity/Assignment Procedures:
    The EAL provides mechanisms for assigning exelwtion units to specific cores as well as creating exelwtion instances.
...
EAL in a Linux-userland Exelwtion Environment
---------------------------------------------
...
each exelwtion unit will be assigned to a specific logical core to run as a user-level thread.
...
Lwrrently, this mode is only supported on Linux.
...
cirlwmstances. Requesting an allocation to take place from a specified
...
.. note::
    ...
    lcore refers to a logical exelwtion unit of the processor, sometimes called a hardware *thread*.
...
unmapped). The PMD must make sure that on such oclwrrence, the application can
still safely use its callbacks.

This event can be subscribed to in the same way one would subscribe to a link
status change event. The exelwtion context is thus the same, i.e. it is the
...
It is possible to use the DPDK exelwtion context with any user pthread (aka. non-EAL pthreads).
...
+ rte_ring
  ...
  This means, use cases ilwolving preemptible pthreads should consider using rte_ring carefully.
  ...
  - It is lwrrently limited to the aarch64 and x86_64 platforms, because it uses
...
malloc heap. This setup ilwolves placing an :ref:`element header<malloc_elem>`
...
to be valid at all times in all lwrrently running processes.
