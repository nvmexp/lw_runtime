...
The callback function will be ilwoked by worker thread using
...
The callback function will be ilwoked by ``rte_graph_create()`` on when
a node gets attached to a graph.

fini():
^^^^^^^

The callback function will be ilwoked by ``rte_graph_destroy()`` on when a
...
objects till ``rte_graph_create()`` ilwoked.
...
* The library registers the node by ilwoking RTE_NODE_REGISTER on library load
...
When ``rte_node_clone()`` ilwoked, The library, would clone all the attributes
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
1. Start with spelwlation that next_node = node->ctx.
...
4. if all the next nodes of the current pkt_set match spelwlated next node,
just count them as successfully spelwlated(``last_spec``) till now and
...
Also, one of the next_node can be updated as spelwlated next_node if it is more
probable. Finally, reset ``last_spec`` to zero.

5. if n_left_from != 0 then goto 3) to process remaining packets.

6. if last_spec == nb_objs, All the objects passed were successfully spelwlated
...
Graph object consists of a header, cirlwlar buffer to store the pending
