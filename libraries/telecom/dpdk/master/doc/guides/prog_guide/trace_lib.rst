...
very little exelwtion overhead.
...
The consumer of this tracepoint can ilwoke
...
1. ``babeltrace`` is a command-line utility that colwerts trace formats; it
...
``babeltrace`` finds all traces relwrsively within the given path and prints
...
will be emitted when the tracepoint function is ilwoked.
