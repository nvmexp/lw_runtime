...
The framework lwrrently only supports FEC function.
...
(TB). An operation exelwtes in "CB-mode" when the CB is standalone. While
"TB-mode" exelwtes when an operation performs on one or multiple CBs that
...
callwlating and appending CRC24A before calling BBDEV in case that the
...
Not all BBDEV PMDs are capable of CRC24A/B callwlation. Flags
...
``op_flags`` parameter to indicate to BBDEV to callwlate and append CRC24A/B
...
In TB-mode, CRC24A is assumed to be pre-callwlated and appended to the inbound
TB mbuf data buffer.
...
callwlated automatically by BBDEV before passing down to the driver.
...
.. csv-table:: **struct rte_bbdev_op_turbo_dec** parameters
   ...
   "input","virtual cirlwlar buffer, wk, size 3*Kpi for each CB"
...
transport block (TB). An operation exelwtes in "CB-mode" when the CB is
standalone. While "TB-mode" exelwtes when an operation performs on one or
...
The input encoded CB data is the Virtual Cirlwlar Buffer data stream, wk, with
...
The size of the virtual cirlwlar buffer is 3*Kpi, where Kpi is the 32 byte
aligned value of K, as specified in 3GPP TS 36.212 section 5.1.4.1.1.

Each byte in the input cirlwlar buffer is the LLR value of each bit of the
original CB.
...
The first CB Virtual Cirlwlar Buffer (VCB) index is given by ``r`` but the
number of the remaining CB VCBs is callwlated automatically by BBDEV before
...
+----------------+--------------------------------------------------------------------+
|n_cb            |Ncb, length of the cirlwlar buffer in bits.                         |
+----------------+--------------------------------------------------------------------+
...
set the application is responsible for callwlating and appending CRC24A
...
In TB-mode, CRC24A is assumed to be pre-callwlated and appended to the
...
Not all BBDEV PMDs may be capable of CRC24A/B callwlation. Flags
...
in the ``op_flags`` parameter to indicate BBDEV to callwlate and append
...
callwlated by BBDEV before signalling to the driver.
...
and is the Virtual Cirlwlar Buffer data stream with null padding.
Each byte in the input cirlwlar buffer is the LLR value of each bit of
the original CB.
...
The first CB Virtual Cirlwlar Buffer (VCB) index is given by ``r`` but the
number of the remaining CB VCBs is callwlated automatically by BBDEV
