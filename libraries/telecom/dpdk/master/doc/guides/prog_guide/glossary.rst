...
Data Plane
   ...
   are the layers ilwolved when forwarding packets.  These layers must be
...
lcore
   A logical exelwtion unit of the processor, sometimes called a *hardware
   thread*.
...
main lcore
   The exelwtion unit that exelwtes the main() function and that launches
   other lcores.
...
OOO
   Out Of Order (exelwtion of instructions within the CPU pipeline)
...
Target
   ...
   exelwtive environment and toolchain.  For example:
