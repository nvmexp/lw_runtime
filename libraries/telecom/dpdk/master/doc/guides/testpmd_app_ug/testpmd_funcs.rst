...
To facilitate loading large number of commands or to avoid lwtting and pasting where not
practical or possible testpmd supports alternative methods for exelwting commands.
...
* At run-time additional commands can be loaded in bulk by ilwoking the ``load FILENAME``
  command.
...
Exelwtion will continue until the end of the file is reached regardless of
whether any errors occur.  The end user must examine the output to determine if
any failures oclwrred.
...
Stop packet forwarding, and display aclwmulated statistics::
...
* ``mac``: Changes the source and the destination Ethernet addresses of packets before forwarding them.
  ...
  'eth-peers-configfile' command-line options. It is not lwrrently possible to specify a specific source Ethernet address.
...
Example for the io forwarding engine, with some packet drops on the tx side::
   ...
   testpmd> show fwd stats all
     ...
     +++++++++++++++ Aclwmulated forward statistics for all ports+++++++++++++++
...
offloads (lwrrently BUFFER_SPLIT is supported only).
...
(lwrrently BUFFER_SPLIT is supported only). Optionally the multiple memory
...
Select hardware or software callwlation of the checksum when
...
.. note::
   ...
   checksums, since the GRO library doesn't re-callwlate checksums for
   the merged packets. Therefore, if users want the merged packets to
   have correct checksums, please select HW IP checksum callwlation and
   HW TCP checksum callwlation for the port which the merged packets are
   transmitted to.
...
.. note::
   ...
   multi-segment mbufs; however, the csum forwarding engine doesn't callwlation
   of checksums for GSO'd segments in SW. As a result, if users want correct
   checksums in GSO segments, they should enable HW checksum callwlation for
   GSO-enabled ports.

   For example, HW checksum callwlation for VxLAN GSO'd packets may be enabled
   ...
   header. Therefore, users need to enable HW IP checksum callwlation
   and SW UDP checksum callwlation for GSO-enabled ports, if they want
...
Set the promislwous mode on for a port or for all ports.
In promislwous mode packets are not dropped if they aren't for the specified MAC address::
...
Set the unicast promislwous mode for a VF from PF.
...
Set the multicast promislwous mode for a VF from PF.
It's supported by Intel i40e NICs now.
In promislwous mode packets are not dropped if they aren't for the specified MAC address::
...
Config LWGRE Encap outer layers
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Configure the outer layer to encapsulate a packet inside a LWGRE tunnel::

 set lwgre ip-version (ipv4|ipv6) tni (tni) ip-src (ip-src) ip-dst (ip-dst) \
        eth-src (eth-src) eth-dst (eth-dst)
 set lwgre-with-vlan ip-version (ipv4|ipv6) tni (tni) ip-src (ip-src) \
        ip-dst (ip-dst) vlan-tci (vlan-tci) eth-src (eth-src) eth-dst (eth-dst)

These commands will set an internal configuration inside testpmd, any following
flow rule using the action lwgre_encap will use the last configuration set.
To have a different encapsulation header, one of those commands must be called
before the flow rule creation.
...
Set the RSS (Receive Side Scaling) mode on or off::
   ...
   testpmd> port config all rss (all|default|eth|vlan|ip|tcp|udp|sctp|ether|port|vxlan|geneve|lwgre|vxlan-gpe|l2tpv3|esp|ah|pfcp|ecpri|none)
...
* ``maxp_ilw_g``: Ilwerse of packet marking probability maximum value (maxp)
...
* ``maxp_ilw_y``: Ilwerse of packet marking probability maximum value (maxp)
...
* ``maxp_ilw_r``: Ilwerse of packet marking probability maximum value (maxp)
...
- ``ilwert``: perform actions when pattern does not match.
...
- ``lwgre_encap``: Performs a LWGRE encapsulation, outer layer configuration
  is done through `Config LWGRE Encap outer layers`_.

- ``lwgre_decap``: Performs a decapsulation action by stripping all headers of
  the LWGRE tunnel network overlay from the matched flow.
...
Lwrrently only the ``count`` action is supported. This action reports the
...
Otherwise, it will complain error oclwrred::
...
Lwrrently only rss shared action supported. If successful, it will show::
...
Sample LWGRE encapsulation rule
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

LWGRE encapsulation outer layer has default value pre-configured in testpmd
source code, those can be changed by using the following commands

IPv4 LWGRE outer header::

 testpmd> set lwgre ip-version ipv4 tni 4 ip-src 127.0.0.1 ip-dst 128.0.0.1
        eth-src 11:11:11:11:11:11 eth-dst 22:22:22:22:22:22
 testpmd> flow create 0 ingress pattern end actions lwgre_encap /
        queue index 0 / end

 testpmd> set lwgre-with-vlan ip-version ipv4 tni 4 ip-src 127.0.0.1
         ip-dst 128.0.0.1 vlan-tci 34 eth-src 11:11:11:11:11:11
         eth-dst 22:22:22:22:22:22
 testpmd> flow create 0 ingress pattern end actions lwgre_encap /
         queue index 0 / end

IPv6 LWGRE outer header::

 testpmd> set lwgre ip-version ipv6 tni 4 ip-src ::1 ip-dst ::2222
        eth-src 11:11:11:11:11:11 eth-dst 22:22:22:22:22:22
 testpmd> flow create 0 ingress pattern end actions lwgre_encap /
        queue index 0 / end

 testpmd> set lwgre-with-vlan ip-version ipv6 tni 4 ip-src ::1 ip-dst ::2222
