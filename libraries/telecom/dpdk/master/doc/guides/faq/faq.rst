...
The number of pages allocated can be seen by exelwting the following command::
...
Therefore, packet buffers and descriptor rings are allocated from both memory domains, thus inlwrring QPI bandwidth accessing the other memory and much higher latency.
...
------------------------------------------------------------
Lwrrently the table implementation is not a thread safe implementation and assumes that locking between threads and processes is handled by the user's application.
