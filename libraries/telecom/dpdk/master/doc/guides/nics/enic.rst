...
boot. Unbinding it lwrrently hangs due to a known issue with the driver. To
...
the following operations for VXLAN, LWGRE, and GENEVE packets. In all cases,
inner and outer packets can be IPv4 or IPv6.

- TSO for VXLAN and GENEVE packets.

  Hardware supports LWGRE TSO, but DPDK lwrrently has no LWGRE offload flags.
...
- Inner RSS
  ...
  RSS hash callwlation, therefore queue selection, is done on inner packets.
...
lwrrently mutually exclusive. When this feature is successfully
...
- **SR-IOV**
  ...
  - Lwrrently, unbind of the ENIC kernel mode driver 'enic.ko' on the VM
...
- **Flow API**
  ...
  - The number of filters that can be specified with the Generic Flow API is
    ...
    were added. Since there lwrrently is no grouping or priority support,
...
- Exelwtion of application by unprivileged system users
...
- Overlay offload
  ...
  - Rx/Tx checksum offloads for VXLAN, LWGRE, GENEVE
  - TSO for VXLAN and GENEVE packets
  - Inner RSS
