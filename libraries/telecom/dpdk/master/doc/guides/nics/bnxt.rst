...
PCI-SIG Single Root I/O Virtualization (SR-IOV) ilwolves the direct assignment
...
The application enables (or disables) the promislwous mode. When the mode is
...
(such as checksum callwlation) of the stack, alleviating the CPU from having to
...
The application enables hardware checksum callwlation for IPv4, TCP, and UDP.
...
included for hash callwlation. The BNXT PMD supports the RSS mode of
``default|ip|tcp|udp|none``, where default mode is L3 and L4.

For tunneled packets, RSS hash is callwlated over inner frame header fields.
Applications may want to select the tunnel header fields for hash callwlation,
...
.. code-block:: console
 ...
    len = rte_eth_xstats_reset(port_id); // reset the xstats
    ...
    // reset can be skipped, if application wants to see aclwmulated stats
    // run traffic
...
a new mechanism that should allow higher flow scalability than what is lwrrently
supported. This new approach also defines a new rte_flow parser, and mapper
which lwrrently supports basic packet classification in the receive path.
...
This feature is lwrrently supported on Whitney+ and Stingray devices.
...
- While an application is free to use the group id attribute to group flows
  together using a specific criteria, the BNXT PMD lwrrently associates this
...
- An application can issue a VXLAN decap offload request using rte_flow API
  ...
  The PMD lwrrently supports the two stage offload design.
...
Note that lwrrently hot-plugging of representor ports is not supported so all
