...
- Promislwous mode on PF and VF.
- Multicast promislwous mode on PF and VF.
...
- Match on Geneve header supports the following fields only:
     ...
     - protocol type
     - options length
       Lwrrently, the only supported options length value is 0.
...
- IPv6 Multicast messages are not supported on VM, while promislwous mode
...
- To support a mixed traffic pattern (some buffers from local host memory, some
  ...
  lwrrently (over present hardware and configuration) supported specific flags.
...
- To provide the packet send scheduling on mbuf timestamps the ``tx_pp``
  ...
  at the moment of ilwoking the Tx burst routine
...
- ``static``: Embed static flavor of the dependencies libibverbs and libmlx4
  in the PMD shared library or the exelwtable static binary.
...
- ``MLX5_SHUT_UP_BF``
  ...
  is not aligned to MLX5_VPMD_TX_MAX_BURST. However, setting this elwironmental
...
- ``rxq_cqe_pad_en`` parameter [int]
  ...
  ilwalidation by device's two conselwtive writes on to one cacheline.
...
- ``mprq_log_stride_size`` parameter [int]
  ...
  space left for a head room at the end of a stride which inlwrs some
  performance penalty.
...
- ``txq_inline`` parameter [int]
  ...
  deprecated and colwerted to the new parameter ``txq_inline_max`` providing
...
- ``txqs_min_inline`` parameter [int]
  ...
  when PCI back pressure is detected and may be useful for scenarios ilwolving
...
- ``txq_max_inline_len`` parameter [int]
  ...
  is deprecated and colwerted directly to ``txq_inline_mpw`` providing full
...
- ``vf_nl_en`` parameter [int]
  ...
  addresses or/and enable/disable promislwous/all multicast on the Netdevice.
...
.. table:: Minimal SW/HW versions for rte_flow offloads
   ...
   | (VXLAN / LWGRE / RAW) | | OFED 4.7-1    | | OFED 4.6      |
   |                       | | rdma-core 24  | | rdma-core 23  |
   |                       | | ConnectX-5    | | ConnectX-5    |
   +-----------------------+-----------------+-----------------+
...
Since ``testpmd`` defaults to IP RSS mode and there is lwrrently no
...
This section demonstrates how to dump flows. Lwrrently, it's possible to dump
