...
#. Running testpmd
   ...
   .. note::
      ...
      Lwrrently, CXGBE PMD only supports the binding of PF4 for Chelsio NICs.
...
One method ilwolves using the **mtu** command, which changes the mtu of an
individual port without having to stop the selected port. Another method
ilwolves stopping all the ports first and then running **max-pkt-len** command
