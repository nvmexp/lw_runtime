...
/**
 * Init the cirlwlar buffer
 */
int cirbuf_init(struct cirbuf *cbuf, char *buf, unsigned int start, unsigned int maxlen);


/**
 * Return 1 if the cirlwlar buffer is full
 */
#define CIRBUF_IS_FULL(cirbuf) ((cirbuf)->maxlen == (cirbuf)->len)

/**
 * Return 1 if the cirlwlar buffer is empty
 */
#define CIRBUF_IS_EMPTY(cirbuf) ((cirbuf)->len == 0)

/**
 * return current size of the cirlwlar buffer (number of used elements)
 */
#define CIRBUF_GET_LEN(cirbuf) ((cirbuf)->len)

/**
 * return size of the cirlwlar buffer (used + free elements)
 */
...
/**
 * Iterator for a cirlwlar buffer
...
/**
 * Add a character at head of the cirlwlar buffer. Return 0 on success, or
 * a negative value on error.
 */
int cirbuf_add_head_safe(struct cirbuf *cbuf, char c);

/**
 * Add a character at head of the cirlwlar buffer. You _must_ check that you
 * have enough free space in the buffer before calling this func.
 */
void cirbuf_add_head(struct cirbuf *cbuf, char c);

/**
 * Add a character at tail of the cirlwlar buffer. Return 0 on success, or
 * a negative value on error.
 */
int cirbuf_add_tail_safe(struct cirbuf *cbuf, char c);

/**
 * Add a character at tail of the cirlwlar buffer. You _must_ check that you
 * have enough free space in the buffer before calling this func.
 */
void cirbuf_add_tail(struct cirbuf *cbuf, char c);

/**
 * Remove a char at the head of the cirlwlar buffer. Return 0 on
 * success, or a negative value on error.
 */
int cirbuf_del_head_safe(struct cirbuf *cbuf);

/**
 * Remove a char at the head of the cirlwlar buffer. You _must_ check
 * that buffer is not empty before calling the function.
 */
void cirbuf_del_head(struct cirbuf *cbuf);

/**
 * Remove a char at the tail of the cirlwlar buffer. Return 0 on
 * success, or a negative value on error.
 */
int cirbuf_del_tail_safe(struct cirbuf *cbuf);

/**
 * Remove a char at the tail of the cirlwlar buffer. You _must_ check
 * that buffer is not empty before calling the function.
 */
void cirbuf_del_tail(struct cirbuf *cbuf);

/**
 * Return the head of the cirlwlar buffer. You _must_ check that
 * buffer is not empty before calling the function.
 */
char cirbuf_get_head(struct cirbuf *cbuf);

/**
 * Return the tail of the cirlwlar buffer. You _must_ check that
 * buffer is not empty before calling the function.
 */
char cirbuf_get_tail(struct cirbuf *cbuf);

/**
 * Add a buffer at head of the cirlwlar buffer. 'c' is a pointer to a
...
/**
 * Add a buffer at tail of the cirlwlar buffer. 'c' is a pointer to a
 * buffer, and n is the number of char to add. Return the number of
 * copied bytes on success, or a negative value on error.
 */
...
/**
 * Remove chars at the head of the cirlwlar buffer. Return 0 on
 * success, or a negative value on error.
 */
int cirbuf_del_buf_head(struct cirbuf *cbuf, unsigned int size);

/**
 * Remove chars at the tail of the cirlwlar buffer. Return 0 on
 * success, or a negative value on error.
 */
int cirbuf_del_buf_tail(struct cirbuf *cbuf, unsigned int size);

/**
 * Copy a maximum of 'size' characters from the head of the cirlwlar
...
/**
 * Copy a maximum of 'size' characters from the tail of the cirlwlar
 * buffer to a flat one pointed by 'c'. Return the number of copied
 * chars.
 */
