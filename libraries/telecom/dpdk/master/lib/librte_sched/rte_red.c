 ...
 */
uint16_t rte_red_pow2_frac_ilw[16];
...
__rte_red_init_tables(void)
{
	...
	table_size = (double)(RTE_DIM(rte_red_pow2_frac_ilw));

	for (i = 0; i < RTE_DIM(rte_red_pow2_frac_ilw); i++) {
		double m = (double)i;

		rte_red_pow2_frac_ilw[i] = (uint16_t) round(scale / pow(2, m / table_size));
	}
	...
	const uint16_t maxp_ilw)
{
	...
	if (maxp_ilw < RTE_RED_MAXP_ILW_MIN) {
		return -6;
	}
	if (maxp_ilw > RTE_RED_MAXP_ILW_MAX) {
		return -7;
	}
	...
	red_cfg->pa_const = (2 * (max_th - min_th) * maxp_ilw) << RTE_RED_SCALING;
	red_cfg->maxp_ilw = maxp_ilw;
