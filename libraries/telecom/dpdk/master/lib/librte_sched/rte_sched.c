...
#define RTE_SCHED_PIPE_ILWALID                UINT32_MAX
#define RTE_SCHED_BMP_POS_ILWALID             UINT32_MAX

/* Scaling for cycles_per_byte callwlation
...
struct rte_sched_port {
	...
	struct rte_reciprocal ilw_cycles_per_byte; /* CPU cycles per byte */
...
static void
rte_sched_pipe_profile_colwert(struct rte_sched_subport *subport,
...
static void
rte_sched_subport_profile_colwert(struct rte_sched_subport_profile_params *src,
	...
	struct rte_sched_subport_params *params, uint64_t rate)
{
	...
	for (i = 0; i < subport->n_pipe_profiles; i++) {
		...
		rte_sched_pipe_profile_colwert(subport, src, dst, rate);
	...
	uint64_t rate)
{
	...
	for (i = 0; i < port->n_subport_profiles; i++) {
		...
		rte_sched_subport_profile_colwert(src, dst, rate);
...
rte_sched_port_config(struct rte_sched_port_params *params)
{
	...
	cycles_per_byte = (rte_get_tsc_hz() << RTE_SCHED_TIME_SHIFT)
		/ params->rate;
	port->ilw_cycles_per_byte = rte_reciprocal_value(cycles_per_byte);
	...
	uint32_t subport_profile_id)
{
	...
	if (port->subports[subport_id] == NULL) {
		...
		for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++) {
			...
			/* if min/max are both zero, then RED is disabled */
				...
				if (rte_red_config_init(&s->red_config[i][j],
				    ...
				    params->red_params[i][j].maxp_ilw) != 0) {
					rte_sched_free_memory(port, n_subports);
		...
		/* Scheduling loop detection */
		s->pipe_loop = RTE_SCHED_PIPE_ILWALID;
		s->pipe_exhaustion = 0;
		...
		for (i = 0; i < RTE_SCHED_PORT_N_GRINDERS; i++)
			s->grinder_base_bmp_pos[i] = RTE_SCHED_PIPE_ILWALID;
	...
	uint32_t *pipe_profile_id)
{
	...
	rte_sched_pipe_profile_colwert(s, params, pp, port->rate);
	...
	uint32_t *subport_profile_id)
{
	...
	rte_sched_subport_profile_colwert(params, dst, port->rate);
	...
	struct rte_sched_subport *subport, uint32_t pos)
{
	...
	subport->pipe_loop = RTE_SCHED_PIPE_ILWALID;
...
grinder_pipe_exists(struct rte_sched_subport *subport, uint32_t base_pipe)
{
	...
	pipes = vld1q_u32(pos);
	if (!vmilwq_u32(veorq_u32(pipes, index)))
		return 1;
	...
	struct rte_sched_subport *subport, uint32_t pos)
{
	...
	if (grinder->pcache_r < grinder->pcache_w) {
		...
		subport->grinder_base_bmp_pos[pos] = RTE_SCHED_BMP_POS_ILWALID;
...
rte_sched_port_time_resync(struct rte_sched_port *port)
{
	...
	/* Reset pipe loop detection */
	for (i = 0; i < port->n_subports_per_port; i++)
		port->subports[i]->pipe_loop = RTE_SCHED_PIPE_ILWALID;
}
