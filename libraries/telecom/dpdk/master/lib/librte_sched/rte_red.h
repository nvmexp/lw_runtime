...
#define RTE_RED_WQ_LOG2_MIN                 1          /**< Min ilwerse filter weight value */
#define RTE_RED_WQ_LOG2_MAX                 12         /**< Max ilwerse filter weight value */
#define RTE_RED_MAXP_ILW_MIN                1          /**< Min ilwerse mark probability value */
#define RTE_RED_MAXP_ILW_MAX                255        /**< Max ilwerse mark probability value */
...
extern uint16_t rte_red_pow2_frac_ilw[16];
...
struct rte_red_params {
	...
	uint16_t maxp_ilw; /**< Ilwerse of packet marking probability maximum value (maxp = 1 / maxp_ilw) */
...
struct rte_red_config {
	...
	uint32_t pa_const; /**< Precomputed constant value used for pa callwlation (scaled in fixed-point format) */
	uint8_t maxp_ilw;  /**< maxp_ilw */
 ...
 * @param maxp_ilw [in] ilwerse maximum mark probability
 *
 * @return Operation status
 * @retval 0 success
 * @retval !0 error
...
/**
 * @brief callwlate factor to scale average queue size when queue
 *        becomes empty
 *
...
__rte_red_calc_qempty_factor(uint8_t wq_log2, uint16_t m)
{
	...
	if (n < RTE_RED_SCALING)
		return (uint16_t) ((rte_red_pow2_frac_ilw[f] + (1 << (n - 1))) >> n);
	...
	const uint64_t time)
{
	 ...
	 *  min_th or max_th, nor callwlate drop probability
	 */
 ...
 *
 *     pb = (1 / maxp_ilw) * (avg - min_th) / (max_th - min_th)
 *     pa = pb / (2 - count * pb)
 *
 *
 *                 (1 / maxp_ilw) * (avg - min_th)
 *                ---------------------------------
 *                         max_th - min_th
 *     pa = -----------------------------------------------
 *                count * (1 / maxp_ilw) * (avg - min_th)
 *           2 - -----------------------------------------
 *                          max_th - min_th
 *
 *
 *                                  avg - min_th
 *     pa = -----------------------------------------------------------
 *           2 * (max_th - min_th) * maxp_ilw - count * (avg - min_th)
 *
 *
 *  We define pa_const as: pa_const =  2 * (max_th - min_th) * maxp_ilw. Then:
 *
 *
 *                     avg - min_th
 *     pa = -----------------------------------
