		...
		bool grow)
{
	...
	do {
		if (fallocate_supported == 0) {
			...
			uint64_t lwr_size = get_file_size(fd);

			/* fallocate isn't supported, fall back to ftruncate */
			if (new_size > lwr_size &&
		...
		unsigned int seg_idx)
{
	...
	*(volatile int *)addr = *(volatile int *)addr;
	 ...
	 */
	if (check_numa()) {
		ret = get_mempolicy(&lwr_socket_id, NULL, 0, addr,
					MPOL_F_NODE | MPOL_F_ADDR);
		if (ret < 0) {
			RTE_LOG(DEBUG, EAL, "%s(): get_mempolicy: %s\n",
				__func__, strerror(errno));
			goto mapped;
		} else if (lwr_socket_id != socket_id) {
			RTE_LOG(DEBUG, EAL,
					"%s(): allocation happened on wrong socket (wanted %d, got %d)\n",
				__func__, socket_id, lwr_socket_id);
			goto mapped;
		}
	}
#else
...
alloc_seg_walk(const struct rte_memseg_list *msl, void *arg)
{
	...
	struct rte_memseg_list *lwr_msl;
	size_t page_sz;
	int lwr_idx, start_idx, j, dir_fd = -1;
	...
	if (wa->exact) {
		...
		lwr_idx = rte_fbarray_find_next_n_free(&lwr_msl->memseg_arr, 0,
				need);
		if (lwr_idx < 0)
			return 0;
		start_idx = lwr_idx;
		 ...
		 */
		lwr_idx = rte_fbarray_find_biggest_free(&lwr_msl->memseg_arr,
				0);
		if (lwr_idx < 0)
			return 0;
		start_idx = lwr_idx;
		 ...
		 */
		lwr_len = rte_fbarray_find_contig_free(&lwr_msl->memseg_arr,
				lwr_idx);
		need = RTE_MIN(need, (unsigned int)lwr_len);
	}
	...
	for (i = 0; i < need; i++, lwr_idx++) {
		struct rte_memseg *lwr;
		void *map_addr;

		lwr = rte_fbarray_get(&lwr_msl->memseg_arr, lwr_idx);
		map_addr = RTE_PTR_ADD(lwr_msl->base_va,
				lwr_idx * page_sz);

		if (alloc_seg(lwr, map_addr, wa->socket, wa->hi,
				msl_idx, lwr_idx)) {
		...
		rte_fbarray_set_used(&lwr_msl->memseg_arr, lwr_idx);
	}
out:
	wa->segs_allocated = i;
	if (i > 0)
...
eal_memalloc_free_seg_bulk(struct rte_memseg **ms, int n_segs)
{
	...
	for (seg = 0; seg < n_segs; seg++) {
		struct rte_memseg *lwr = ms[seg];
		struct hugepage_info *hi = NULL;
		struct free_walk_param wa;
		int i, walk_res;

		/* if this page is marked as unfreeable, fail */
		if (lwr->flags & RTE_MEMSEG_FLAG_DO_NOT_FREE) {
		...
		for (i = 0; i < (int)RTE_DIM(internal_conf->hugepage_info);
				i++) {
			hi = &internal_conf->hugepage_info[i];
			if (lwr->hugepage_sz == hi->hugepage_sz)
				break;
		}
		...
		unsigned int msl_idx, bool used, int start, int end)
{
	...
	/* callwlate how much we can advance until next chunk */
	diff_len = used ?
			rte_fbarray_find_contig_used(l_arr, start) :
			rte_fbarray_find_contig_free(l_arr, start);
