...
aslr_enabled(void)
{
	...
	switch (c) {
		...
		case '1' : return 1;
		case '2' : return 2;
		default: return -EILWAL;
	}
}

static sigjmp_buf huge_jmpelw;

static void huge_sigbus_handler(int signo __rte_unused)
{
	siglongjmp(huge_jmpelw, 1);
}
...
{
	return sigsetjmp(huge_jmpelw, 1);
}
...
remap_segment(struct hugepage_file *hugepages, int seg_start, int seg_end)
{
	...
	for (lwr_page = seg_end - 1; lwr_page >= seg_start;
			lwr_page--, ms_idx++) {
#else
	for (lwr_page = seg_start; lwr_page < seg_end; lwr_page++, ms_idx++) {
#endif
		struct hugepage_file *hfile = &hugepages[lwr_page];
...
{
	struct rte_mem_config *mcfg = rte_eal_get_configuration()->mem_config;
	int lwr_page, seg_start_page, end_seg, new_memseg;
	...
	for (lwr_page = 0; lwr_page < n_pages; lwr_page++) {
		struct hugepage_file *hpi = &hugepages[lwr_page];
	...
	for (hpi_idx = 0; hpi_idx < internal_conf->num_hugepage_sizes;
		...
		for (i = 0; i < rte_socket_count(); i++) {
			...
			for (lwr_page = 0; lwr_page < n_pages; lwr_page++) {
				struct hugepage_file *prev, *lwr;
				int prev_seg_start_page = -1;

				lwr = &hugepages[lwr_page];
				prev = lwr_page == 0 ? NULL :
						&hugepages[lwr_page - 1];
				...
				if (lwr->size == 0)
					end_seg = 1;
				else if (lwr->socket_id != (int) socket)
					end_seg = 1;
				else if (lwr->size != page_sz)
					end_seg = 1;
				else if (lwr_page == 0)
					new_memseg = 1;
#ifdef RTE_ARCH_PPC_64
				 ...
				 */
				else if ((prev->physaddr - lwr->physaddr) !=
						lwr->size)
					new_memseg = 1;
#else
				else if ((lwr->physaddr - prev->physaddr) !=
						lwr->size)
					new_memseg = 1;
#endif
				if (new_memseg) {
			...
			if (seg_start_page != -1) {
				n_contig_segs++;
				n_segs += lwr_page - seg_start_page;
			}
...
{
	int lwr_page, seg_start_page, new_memseg, ret;
	...
	for (lwr_page = 0; lwr_page < n_pages; lwr_page++) {
		...
		lwr = &hugepages[lwr_page];
		prev = lwr_page == 0 ? NULL : &hugepages[lwr_page - 1];
		...
		if (lwr_page == 0)
			new_memseg = 1;
		else if (lwr->socket_id != prev->socket_id)
			new_memseg = 1;
		else if (lwr->size != prev->size)
			new_memseg = 1;
#ifdef RTE_ARCH_PPC_64
		 ...
		 * descending order.
		 */
		else if ((prev->physaddr - lwr->physaddr) != lwr->size)
			new_memseg = 1;
#else
		else if ((lwr->physaddr - prev->physaddr) != lwr->size)
			new_memseg = 1;
#endif
...
eal_legacy_hugepage_init(void)
{
	...
	/* callwlate total number of hugepages available. at this point we haven't
	...
	/* make a copy of socket_mem, needed for number of pages callwlation */
	for (i = 0; i < RTE_MAX_NUMA_NODES; i++)
		memory[i] = internal_conf->socket_mem[i];

	/* callwlate final number of pages */
...
error:
	...
	for (lwr_seg = 0; lwr_seg < i; lwr_seg++)
		munmap(hp[lwr_seg].final_va, hp[lwr_seg].size);
...
memseg_primary_init_32(void)
{
	...
	/* the allocation logic is a little bit colwoluted, but here's how it
	 * works, in a nutshell:
	...
	for (i = 0; i < rte_socket_count(); i++) {
		...
		uint64_t max_socket_mem, lwr_socket_mem;
		...
		for (hpi_idx = 0; hpi_idx < hp_sizes; hpi_idx++) {
			uint64_t max_pagesz_mem, lwr_pagesz_mem = 0;
			...
			max_pagesz_mem = max_socket_mem - lwr_socket_mem;
			...
			type_msl_idx = 0;
			while (lwr_pagesz_mem < max_pagesz_mem &&
					total_segs < max_segs) {
				uint64_t lwr_mem;
				unsigned int n_segs;
				...
				lwr_mem = get_mem_amount(hugepage_sz,
						max_pagesz_mem);
				n_segs = lwr_mem / hugepage_sz;
				...
				lwr_pagesz_mem = total_segs * hugepage_sz;
				type_msl_idx++;
				msl_idx++;
			}
			lwr_socket_mem += lwr_pagesz_mem;
		}
		if (lwr_socket_mem == 0) {
