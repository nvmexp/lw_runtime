	...
	do {
		...
		while ((ap = LIST_FIRST(&alarm_list)) != NULL &&
			...
			if (ap->exelwting == 0) {
				...
				/* If calling from other context, mark that alarm is exelwting
				 * so loop can spin till it finish. Otherwise we are trying to
				 * cancel our self - mark it by EINPROGRESS */
				if (pthread_equal(ap->exelwting_id, pthread_self()) == 0)
					exelwting++;
				else
					err = EINPROGRESS;
		...
		LIST_FOREACH(ap, &alarm_list, next) {
			...
			if (cb_fn == ap->cb_fn &&
				...
				if (ap->exelwting == 0) {
					...
					count++;
					ap = ap_prev;
				} else if (pthread_equal(ap->exelwting_id, pthread_self()) == 0)
					exelwting++;
				else
					err = EINPROGRESS;
			}
