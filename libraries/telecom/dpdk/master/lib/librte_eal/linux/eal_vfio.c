	...
	if (n_merged > 0) {
		...
		for (i = 0; i < user_mem_maps->n_maps; i++) {
			if (!is_null_map(&user_mem_maps->maps[i])) {
				...
				dst = &user_mem_maps->maps[lwr_idx++];
		...
		int iommu_group_num)
{
	...
	for (i = 0; i < VFIO_MAX_GROUPS; i++)
		if (vfio_cfg->vfio_groups[i].group_num == -1) {
			lwr_grp = &vfio_cfg->vfio_groups[i];
			break;
		}
	...
	lwr_grp->group_num = iommu_group_num;
	lwr_grp->fd = vfio_group_fd;
		...
		void *arg __rte_unused)
{
	...
	while (lwr_len < len) {
		...
		bool last_seg = (len - lwr_len) == ms->len;
		...
		int *vfio_dev_fd, struct vfio_device_info *device_info)
{
	...
	if (!(group_status.flags & VFIO_GROUP_FLAGS_CONTAINER_SET)) {
		...
		if (internal_conf->process_type == RTE_PROC_PRIMARY &&
			...
			rte_spinlock_relwrsive_lock(&user_mem_maps->lock);
			...
			for (i = 0; i < user_mem_maps->n_maps; i++) {
				...
				if (ret) {
					RTE_LOG(ERR, EAL, "Couldn't map user memory for DMA: "
							...
							map->addr, map->iova,
							map->len);
					rte_spinlock_relwrsive_unlock(
							&user_mem_maps->lock);
					rte_mcfg_mem_read_unlock();
					return -1;
				}
			}
			rte_spinlock_relwrsive_unlock(&user_mem_maps->lock);
...
rte_vfio_enable(const char *modname)
{
	...
	rte_spinlock_relwrsive_t lock = RTE_SPINLOCK_RELWRSIVE_INITIALIZER;
		...
		uint64_t len)
{
	...
	if (vfio_dma_mem_map(vfio_cfg, vaddr, iova, len, 1)) {
		/* technically, this will fail if there are lwrrently no devices
...
{
	struct vfio_config *vfio_cfg;
	struct vfio_group *lwr_grp = NULL;
	int i;
	...
	/* This should not happen */
	if (i == VFIO_MAX_GROUPS || lwr_grp == NULL) {
		RTE_LOG(ERR, EAL, "Specified group number not found\n");
		return -1;
	}

	if (lwr_grp->fd >= 0 && close(lwr_grp->fd) < 0) {
