 ...
 *     - EILWAL  - ``ms`` pointer was NULL or did not point to a valid memseg
 ...
 *   -1 in case of error, with ``rte_errno`` set to the following values:
 *     - EILWAL  - ``ms`` pointer was NULL or did not point to a valid memseg
 *     - EILWAL  - ``offset`` pointer was NULL
 *     - ENODEV  - ``ms`` fd is not available
 *     - ENOENT  - ``ms`` is an unused segment
 *     - ENOTSUP - segment fd's are not supported
 */
 ...
 *     EILWAL - one of the parameters was invalid
 ...
 * @b EXPERIMENTAL: this API may change without prior notice
 *
 * Check if all lwrrently allocated memory segments are compliant with
...
typedef int (*rte_mem_alloc_validator_t)(int socket_id,
		size_t lwr_limit, size_t new_len);
