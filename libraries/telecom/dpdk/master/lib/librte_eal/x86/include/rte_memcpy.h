...
rte_memcpy_generic(void *dst, const void *src, size_t n)
{
	...
	if (n < 16) {
		if (n & 0x01) {
			*(uint8_t *)dstu = *(const uint8_t *)srlw;
			srlw = (uintptr_t)((const uint8_t *)srlw + 1);
			dstu = (uintptr_t)((uint8_t *)dstu + 1);
		}
		if (n & 0x02) {
			*(uint16_t *)dstu = *(const uint16_t *)srlw;
			srlw = (uintptr_t)((const uint16_t *)srlw + 1);
			dstu = (uintptr_t)((uint16_t *)dstu + 1);
		}
		if (n & 0x04) {
			*(uint32_t *)dstu = *(const uint32_t *)srlw;
			srlw = (uintptr_t)((const uint32_t *)srlw + 1);
			dstu = (uintptr_t)((uint32_t *)dstu + 1);
		}
		if (n & 0x08)
			*(uint64_t *)dstu = *(const uint64_t *)srlw;
		return ret;
	}
