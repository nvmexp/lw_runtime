	...
	for (idx = 0; idx < RTE_HEAP_NUM_FREELISTS; idx++) {
		for (elem = LIST_FIRST(&heap->free_head[idx]);
			...
			if (contig) {
				...
				if (aligned >= data_end)
					continue;
				lwr_size = RTE_PTR_DIFF(data_end, aligned);
			}
			if (lwr_size > max_size) {
				max_size = lwr_size;
				max_elem = elem;
			}
		}
		...
		bool contig, struct rte_memseg **ms, int n_segs)
{
	 ...
	 * exelwted. For 2) implies the new memory can not be added.
	 */
	if (mcfg->dma_maskbits &&
	    rte_mem_check_dma_mask_thread_unsafe(mcfg->dma_maskbits)) {
		/*
		 * Lwrrently this can only happen if IOMMU is enabled
...
{
	int socket, i, lwr_socket, heap_id;
	void *ret;
	...
	/* try other heaps */
	for (i = 0; i < (int) rte_socket_count(); i++) {
		lwr_socket = rte_socket_id_by_idx(i);
		if (lwr_socket == socket)
			continue;
...
malloc_heap_free(struct malloc_elem *elem)
{
	...
	/* recallwlate length and number of segments */
	aligned_len = RTE_PTR_DIFF(aligned_end, aligned_start);
	n_segs = aligned_len / page_sz;
