 ...
 * that is lwrrently being processed by one lcore
 */
struct log_lwr_msg {
 ...
 /* per core log */
static RTE_DEFINE_PER_LCORE(struct log_lwr_msg, log_lwr_msg);
...
int rte_log_lwr_msg_loglevel(void)
{
	return RTE_PER_LCORE(log_lwr_msg).loglevel;
}

/* get the current logtype for the message being processed */
int rte_log_lwr_msg_logtype(void)
{
	return RTE_PER_LCORE(log_lwr_msg).logtype;
}
...
rte_vlog(uint32_t level, uint32_t logtype, const char *format, va_list ap)
{
	...
	RTE_PER_LCORE(log_lwr_msg).loglevel = level;
	RTE_PER_LCORE(log_lwr_msg).logtype = logtype;
