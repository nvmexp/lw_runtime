...
{
	void *lwr_page, *contig_seg_start, *page_end, *lwr_seg_end;
	void *data_start, *data_end;
	rte_iova_t expected_iova;
	struct rte_memseg *ms;
	size_t page_sz, lwr, max;
	...
	lwr_page = RTE_PTR_ALIGN_FLOOR(contig_seg_start, page_sz);
	ms = rte_mem_virt2memseg(lwr_page, elem->msl);

	/* do first iteration outside the loop */
	page_end = RTE_PTR_ADD(lwr_page, page_sz);
	lwr_seg_end = RTE_MIN(page_end, data_end);
	lwr = RTE_PTR_DIFF(lwr_seg_end, contig_seg_start) -
	...
	lwr_page = RTE_PTR_ADD(lwr_page, page_sz);

	while (lwr_page < data_end) {
		page_end = RTE_PTR_ADD(lwr_page, page_sz);
		lwr_seg_end = RTE_MIN(page_end, data_end);
		...
		if (ms->iova != expected_iova) {
			 ...
			 * alignment.
			 */
			contig_seg_start = RTE_PTR_ALIGN(lwr_page, align);
...
/*
 * callwlate the starting point of where data of the requested size
