		...
		size_t len)
{
	...
	if (!rte_eal_has_hugepages()) {
		...
		lwr = rte_mem_virt2iova(aligned_start);
		expected = lwr + pgsz;
		...
		for (lwr_seg = start_seg + 1; lwr_seg < end_seg;
				lwr_seg++, expected += pgsz) {
			ms = rte_fbarray_get(&msl->memseg_arr, lwr_seg);
