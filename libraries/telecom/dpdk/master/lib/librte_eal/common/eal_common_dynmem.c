...
eal_dynmem_memseg_lists_init(void)
{
	...
	unsigned int n_memtypes, lwr_type;
	 ...
	 * ilwolved process. the basic element we're operating with is a memory
	...
	for (hpi_idx = 0; hpi_idx < (int) internal_conf->num_hugepage_sizes;
		...
		for (i = 0; i < (int) rte_socket_count(); i++, lwr_type++) {
			...
			if (!internal_conf->legacy_mem && socket_id > 0)
				break;
#endif
			memtypes[lwr_type].page_sz = hugepage_sz;
			memtypes[lwr_type].socket_id = socket_id;
	...
	msl_idx = 0;
	for (lwr_type = 0; lwr_type < n_memtypes; lwr_type++) {
		unsigned int lwr_seglist, n_seglists, n_segs;
		unsigned int max_segs_per_type, max_segs_per_list;
		struct memtype *type = &memtypes[lwr_type];
		...
		/* callwlate how much segments we will need in total */
		...
		/* callwlate how much memory we can have per segment list */
		max_mem_per_list = RTE_MIN(max_segs_per_list * pagesz,
				(uint64_t)RTE_MAX_MEM_MB_PER_LIST << 20);

		/* callwlate how many segments each segment list will have */
		n_segs = RTE_MIN(max_segs_per_list, max_mem_per_list / pagesz);

		/* callwlate how many segment lists we can have */
		...
		for (lwr_seglist = 0; lwr_seglist < n_seglists; lwr_seglist++) {
...
static int
limits_callback(int socket_id, size_t lwr_limit, size_t new_len)
{
	RTE_SET_USED(socket_id);
	RTE_SET_USED(lwr_limit);
	...
	for (hp_sz_idx = 0;
		...
		for (socket_id = 0; socket_id < RTE_MAX_NUMA_NODES;
			...
			do {
				...
				lwr_pages = eal_memalloc_alloc_seg_bulk(pages,
	...
	struct hugepage_info *hp_used, unsigned int num_hp_info)
{
	...
	uint64_t remaining_mem, lwr_mem;
	...
	for (socket = 0; socket < RTE_MAX_NUMA_NODES && total_mem != 0;
		...
		for (i = 0; i < num_hp_info && memory[socket] != 0; i++) {
			...
			lwr_mem = hp_used[i].num_pages[socket] *
					hp_used[i].hugepage_sz;

			memory[socket] -= lwr_mem;
			total_mem -= lwr_mem;
