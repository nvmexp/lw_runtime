		...
		unsigned int bound)
{
	...
	if (len > SIZE_MAX - RTE_CACHE_LINE_MASK) {
		rte_errno = EILWAL; /* requested size too big */
		return NULL;
	}
...
dump_memzone(const struct rte_memzone *mz, void *arg)
{
	...
	/* go through each page oclwpied by this memzone */
	...
	lwr_addr = RTE_PTR_ALIGN_FLOOR(mz->addr, page_sz);
	mz_end = RTE_PTR_ADD(lwr_addr, mz->len);
	...
	do {
		fprintf(f, "  addr: %p iova: 0x%" PRIx64 " "
				"len: 0x%zx "
				"pagesz: 0x%zx\n",
			lwr_addr, ms->iova, ms->len, page_sz);

		/* advance VA to next page */
		lwr_addr = RTE_PTR_ADD(lwr_addr, page_sz);

		/* memzones occupy contiguous segments */
		++ms;
	} while (lwr_addr < mz_end);
}
