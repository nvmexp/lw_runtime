	    ...
	    bool used)
{
	...
	/* array length may not be aligned, so callwlate ignore mask for last
	 * mask index.
	 */
	...
	for (msk_idx = first; msk_idx < msk->n_masks; msk_idx++) {
		uint64_t lwr_msk, lookahead_msk;
		unsigned int run_start, clz, left;
		bool found = false;
		/*
		 * The process of getting n conselwtive bits for arbitrary n is
		 * a bit ilwolved, but here it is in a nutshell:
		 *
		 *  1. let n be the number of conselwtive bits we're looking for
		 ...
		 *  3. ilwert the mask and count leading zeroes (that is, count
		 *     how many conselwtive set bits we had starting from the
		 ...
		 *     conselwtive set bits right at the start, so we will do
		 ...
		 * were looking for.
		 */
		lwr_msk = msk->data[msk_idx];
		left = n;

		/* if we're looking for free spaces, ilwert the mask */
		if (!used)
			lwr_msk = ~lwr_msk;
		...
		/* count leading zeroes on ilwerted mask */
		if (~lwr_msk == 0)
			clz = sizeof(lwr_msk) * 8;
		else
			clz = __builtin_clzll(~lwr_msk);
		...
		for (lookahead_idx = msk_idx + 1; lookahead_idx < msk->n_masks;
			...
			/* if we're looking for free space, ilwert the mask */
			if (!used)
				lookahead_msk = ~lookahead_msk;

			/* figure out how many conselwtive bits we need here */
...
find_next(const struct rte_fbarray *arr, unsigned int start, bool used)
{
	...
	for (idx = first; idx < msk->n_masks; idx++) {
		uint64_t lwr = msk->data[idx];
		int found;

		/* if we're looking for free entries, ilwert mask */
		if (!used)
			lwr = ~lwr;
...
find_contig(const struct rte_fbarray *arr, unsigned int start, bool used)
{
	...
	for (idx = first; idx < msk->n_masks; idx++, result += need_len) {
		...
		/* we will be looking for zeroes, so ilwert the mask */
		lwr = ~lwr;
		...
		bool used)
{
	...
	msk_idx = first;
	do {
		uint64_t lwr_msk, lookbehind_msk;
		unsigned int run_start, run_end, ctz, left;
		bool found = false;
		/*
		 * The process of getting n conselwtive bits from the top for
		 * arbitrary n is a bit ilwolved, but here it is in a nutshell:
		 *
		 *  1. let n be the number of conselwtive bits we're looking for
		 *  2. check if n can fit in one mask, and if so, do n-1
		 ...
		 *    2a. if we found a run, bail out early
		 *    2b. if we didn't find a run, proceed
		 *  3. ilwert the mask and count trailing zeroes (that is, count
		 *     how many conselwtive set bits we had starting from the
		 *     start of current mask) as k
		 *    3a. if k is 0, continue to next mask
		 *    3b. if k is not 0, we have a potential run
		 *  4. to satisfy our requirements, next mask must have n-k
		 *     conselwtive set bits at the end, so we will do (n-k-1)
		...
		/* count trailing zeroes on ilwerted mask */
		if (~lwr_msk == 0)
			ctz = sizeof(lwr_msk) * 8;
		else
			ctz = __builtin_ctzll(~lwr_msk);
		 ...
		 * the run ended, so callwlate start position.
		 */
		return run_end - n;
...
find_prev(const struct rte_fbarray *arr, unsigned int start, bool used)
{
	...
	do {
		 ...
		 * the value we get is counted from end of mask, so callwlate
		 * position from start of mask.
		 */
		found = MASK_ALIGN - __builtin_clzll(lwr) - 1;
...
fbarray_find_biggest(struct rte_fbarray *arr, unsigned int start, bool used,
		bool rev)
{
	int lwr_idx, next_idx, lwr_len, biggest_idx, biggest_len;
	...
	biggest_len = 0;
	for (;;) {
		lwr_idx = find_func(arr, lwr_idx);

		/* block found, check its length */
		if (lwr_idx >= 0) {
			lwr_len = find_contig_func(arr, lwr_idx);
			/* decide where we go next */
			next_idx = rev ? lwr_idx - lwr_len : lwr_idx + lwr_len;
			/* move current index to start of chunk */
			lwr_idx = rev ? next_idx + 1 : lwr_idx;

			if (lwr_len > biggest_len) {
				biggest_idx = lwr_idx;
				biggest_len = lwr_len;
			}
			lwr_idx = next_idx;
...
rte_fbarray_dump_metadata(struct rte_fbarray *arr, FILE *f)
{
	...
	fprintf(f, "size: %i oclwpied: %i elt_sz: %i\n",
