...
verify_perms(const char *dirpath)
{
	...
	if (st.st_mode & S_IWOTH) {
		RTE_LOG(ERR, EAL,
				"Error, directory path %s is world-writable and inselwre\n",
				dirpath);
		return -1;
	}
...
eal_check_common_options(struct internal_config *internal_cfg)
{
	...
	if (internal_cfg->process_type == RTE_PROC_ILWALID) {
