...
struct core_state {
	...
	uint8_t is_service_core; /* set if core is lwrrently a service core */
...
{
	struct rte_service_spec_impl *s;
	SERVICE_VALID_GET_OR_ERR_RET(id, s, -EILWAL);
	    ...
	    struct rte_service_spec_impl *s, uint32_t serialize_mt_unsafe)
{
	...
	if ((service_mt_safe(s) == 0) && (serialize_mt_unsafe == 1)) {
		if (!rte_spinlock_trylock(&s->exelwte_lock))
			return -EBUSY;

		service_runner_do_callback(s, cs, i);
		rte_spinlock_unlock(&s->exelwte_lock);
	} else
		service_runner_do_callback(s, cs, i);
