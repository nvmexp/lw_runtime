...
struct alarm_entry {
	...
	void *cb_arg;
	volatile uint8_t exelwting;
	volatile pthread_t exelwting_id;
};
...
static int
unregister_lwrrent_callback(void)
{
	struct alarm_entry *ap;
	int ret = 0;
...
rte_eal_alarm_set(uint64_t us, rte_eal_alarm_callback cb_fn, void *cb_arg)
{
	...
	/* use current time to callwlate absolute time of alarm */
	...
	do {
		...
		while (1) {
			...
			if (ap->exelwting == 0) {
				 ...
				 * alarm is exelwting so loop can spin till it
				 * finish. Otherwise we are trying to cancel
				 * ourselves - mark it by EINPROGRESS.
				 */
				if (pthread_equal(ap->exelwting_id,
						pthread_self()) == 0)
					exelwting++;
				else
					err = EINPROGRESS;
		...
		LIST_FOREACH(ap, &alarm_list, next) {
			...
			if (cb_fn == ap->cb_fn &&
				...
				if (ap->exelwting == 0) {
					...
					count++;
					ap = ap_prev;
				} else if (pthread_equal(ap->exelwting_id,
							 pthread_self()) == 0) {
					exelwting++;
				} else {
					err = EINPROGRESS;
