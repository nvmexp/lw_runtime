		...
		rte_intr_callback_fn cb, void *cb_arg)
{
	...
	if (add_event || src->intr_handle.type == RTE_INTR_HANDLE_ALARM) {
		...
		if (intr_source_to_kevent(intr_handle, &ke) < 0) {
			RTE_LOG(ERR, EAL, "Cannot colwert interrupt handle to kevent\n");
			ret = -ENODEV;
			goto fail;
		}
		...
		if (kevent(kq, &ke, 1, NULL, 0, NULL) < 0) {
			/* lwrrently, nic_uio does not support interrupts, so
		...
		rte_intr_callback_fn cb_fn, void *cb_arg)
{
	...
	/* ok to remove. */
		...
		if (intr_source_to_kevent(intr_handle, &ke) < 0) {
			RTE_LOG(ERR, EAL, "Cannot colwert to kevent\n");
			ret = -ENODEV;
			goto out;
		}
		...
		if (kevent(kq, &ke, 1, NULL, 0, NULL) < 0) {
			 ...
			 * in some cirlwmstances (e.g. oneshot events).
			 */
		}
		...
		for (cb = TAILQ_FIRST(&src->callbacks); cb != NULL; cb = next) {
			...
			if (cb->pending_delete) {
				...
				if (kevent(kq, &ke, 1, NULL, 0, NULL) < 0) {
					 ...
					 * condition in some cirlwmstances
					 * (e.g. oneshot events).
					 */
				}
