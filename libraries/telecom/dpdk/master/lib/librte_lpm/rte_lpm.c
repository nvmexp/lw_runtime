...
struct __rte_lpm {
	...
	enum rte_lpm_qsbr_mode rlw_mode;/* Blocking, defer queue. */
	struct rte_rlw_qsbr_dq *dq;	/* RLW QSBR defer queue. */
};
...
/*
 * Colwerts a given depth value to its corresponding mask value.
 *
 * depth  (IN)		: range = 1 - 32
 * mask   (OUT)		: 32bit mask
 */
...
depth_to_mask(uint8_t depth)
{
	...
	/* To callwlate a mask start with a 1 on the left hand side and right
...
/*
 * Colwerts given depth value to its corresponding range value.
 */
static uint32_t __attribute__((pure))
depth_to_range(uint8_t depth)
{
	...
	/*
	 * Callwlate tbl24 range. (Note: 2^depth = 1 << depth)
	 */
...
rte_lpm_free(struct rte_lpm *lpm)
{
	...
	if (i_lpm->dq != NULL)
		rte_rlw_qsbr_dq_delete(i_lpm->dq);
...
static void
__lpm_rlw_qsbr_free_resource(void *p, void *data, unsigned int n)
{
 ...
 */
int
rte_lpm_rlw_qsbr_add(struct rte_lpm *lpm, struct rte_lpm_rlw_config *cfg)
{
	...
	if (cfg->mode == RTE_LPM_QSBR_MODE_SYNC) {
		...
		if (params.max_reclaim_size == 0)
			params.max_reclaim_size = RTE_LPM_RLW_DQ_RECLAIM_MAX;
		params.esize = sizeof(uint32_t);	/* tbl8 group index */
		params.free_fn = __lpm_rlw_qsbr_free_resource;
		params.p = i_lpm;
		params.v = cfg->v;
		i_lpm->dq = rte_rlw_qsbr_dq_create(&params);
		if (i_lpm->dq == NULL) {
	...
	uint32_t next_hop)
{
	...
	if (i_lpm->rule_info[depth - 1].used_rules > 0) {
		...
		/* Callwlate the position in which the rule will be stored. */
		rule_index = 0;
...
rule_find(struct __rte_lpm *i_lpm, uint32_t ip_masked, uint8_t depth)
{
	...
	/* If rule is not found return -EILWAL. */
	return -EILWAL;
}
...
tbl8_alloc(struct __rte_lpm *i_lpm)
{
	...
	if (group_idx == -ENOSPC && i_lpm->dq != NULL) {
		...
		if (rte_rlw_qsbr_dq_reclaim(i_lpm->dq, 1,
				NULL, NULL, NULL) == 0)
			group_idx = _tbl8_alloc(i_lpm);
	}
...
tbl8_free(struct __rte_lpm *i_lpm, uint32_t tbl8_group_start)
{
	...
	if (i_lpm->v == NULL) {
		...
		__atomic_store(&i_lpm->lpm.tbl8[tbl8_group_start], &zero_tbl8_entry,
				__ATOMIC_RELAXED);
	} else if (i_lpm->rlw_mode == RTE_LPM_QSBR_MODE_SYNC) {
		/* Wait for quiescent state change. */
		rte_rlw_qsbr_synchronize(i_lpm->v,
			RTE_QSBR_THRID_ILWALID);
		/* Set tbl8 group invalid*/
		__atomic_store(&i_lpm->lpm.tbl8[tbl8_group_start], &zero_tbl8_entry,
				__ATOMIC_RELAXED);
	} else if (i_lpm->rlw_mode == RTE_LPM_QSBR_MODE_DQ) {
		/* Push into QSBR defer queue. */
		status = rte_rlw_qsbr_dq_enqueue(i_lpm->dq,
...
#define group_idx next_hop
	...
	/* Callwlate the index into Table24. */
	tbl24_index = ip >> 8;
	...
	for (i = tbl24_index; i < (tbl24_index + tbl24_range); i++) {
		...
		if (i_lpm->lpm.tbl24[i].valid_group == 1) {
			/* If tbl24 entry is valid and extended callwlate the
			 *  index into tbl8.
			 */
...
#define group_idx next_hop
	...
	if (!i_lpm->lpm.tbl24[tbl24_index].valid) {
	...
	} /* If valid entry but not extended callwlate the index into Table8. */
		...
		* If it is valid, extended entry callwlate the index into tbl8.
		*/
...
#define group_idx next_hop
	...
	/* Callwlate the range and index into Table24. */
	tbl24_range = depth_to_range(depth);
	tbl24_index = (ip_masked >> 8);
	...
	if (sub_rule_index < 0) {
		/*
		 * If no replacement rule exists then ilwalidate entries
 ...
 * Return of -EILWAL means tbl8 is empty and thus can be recycled
		...
		uint32_t tbl8_group_start)
{
	...
	/* If no valid entries are found then return -EILWAL. */
	return -EILWAL;
}
...
#define group_idx next_hop
	...
	/*
	 * Callwlate the index into tbl24 and range. Note: All depths larger
	 * than MAX_DEPTH_TBL24 are associated with only one tbl24 entry.
	 */
	tbl24_index = ip_masked >> 8;

	/* Callwlate the index into tbl8 and range. */
	tbl8_group_index = i_lpm->lpm.tbl24[tbl24_index].group_idx;
	 ...
	 * tbl8 entries are invalid we can free the tbl8 and ilwalidate the
	 * associated tbl24 entry.
	 */

	tbl8_recycle_index = tbl8_recycle_check(i_lpm->lpm.tbl8, tbl8_group_start);

	if (tbl8_recycle_index == -EILWAL) {
...
rte_lpm_delete(struct rte_lpm *lpm, uint32_t ip, uint8_t depth)
{
	 ...
	 * function rule_find returns -EILWAL.
	 */
	 ...
	 * replace the rule_to_delete we return -1 and ilwalidate the table
