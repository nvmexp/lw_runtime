...
/*
 * Callwlate index to the table based on the number and position
		...
		uint8_t is_new_rule)
{
	...
	if (depth <= bits_covered) {
		...
		for (i = entry_ind; i < (entry_ind + tbl_range); i++) {
			if (!tbl[i].valid || (tbl[i].ext_entry == 0 &&
				...
				/*
				 * If tbl entry is valid and extended callwlate the index
	 ...
	 * and callwlate the index to the next table.
	 */
	else {
		...
		if (!tbl[entry_ind].valid) {
			...
			/* ilwalidate all new tbl8 entries */
		...
		uint8_t first_byte, uint32_t *next_hop)
{
	...
	/* If it is valid and extended we callwlate the new pointer to return. */
		...
		uint32_t *next_hop)
{
	...
	/* Callwlate pointer to the first entry to be inspected */
...
/*
 * Colwert a depth to a one byte long mask
 *   Example: 4 will be colwerted to 0xF0
 */
...
/*
 * Find range of tbl8 cells oclwpied by a rule
 */
static void
...
rte_lpm6_delete(struct rte_lpm6 *lpm, const uint8_t *ip, uint8_t depth)
{
	...
	for (; from <= to; from++)
		if (from->ext_entry == 1) {
			 ...
			 * a more specific prefix must be oclwpied
			 * by the prefix
			 */
			if (lsp_rule != NULL)
				expand_rule(lpm,
				 ...
				 * its more specific space must be ilwalidated
				 */
				expand_rule(lpm,
					from->lpm6_tbl8_gindex *
					RTE_LPM6_TBL8_GROUP_NUM_ENTRIES,
