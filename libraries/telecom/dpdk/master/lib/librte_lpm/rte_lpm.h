...
#define RTE_LPM_RLW_DQ_RECLAIM_MAX	16
...
struct rte_lpm_config {
	...
	int flags;               /**< This field is lwrrently unused. */
};
...
/** LPM RLW QSBR configuration structure. */
struct rte_lpm_rlw_config {
	struct rte_rlw_qsbr *v;	/* RLW QSBR variable. */
	/* Mode of RLW QSBR. RTE_LPM_QSBR_MODE_xxx
	 * '0' for default: create defer queue for reclaim.
	 */
	enum rte_lpm_qsbr_mode mode;
	uint32_t dq_size;	/* RLW defer queue size.
	...
	uint32_t reclaim_max;	/* Max entries to reclaim in one go.
				 * default: RTE_LPM_RLW_DQ_RECLAIM_MAX.
				 */
};
 ...
 *
 * Associate RLW QSBR variable with an LPM object.
 *
 * @param lpm
 *   the lpm object to add RLW QSBR
 * @param cfg
 ...
 */
__rte_experimental
int rte_lpm_rlw_qsbr_add(struct rte_lpm *lpm, struct rte_lpm_rlw_config *cfg);
 ...
 * @return
 *   -EILWAL for incorrect arguments, -ENOENT on lookup miss, 0 on lookup hit
 */
static inline int
rte_lpm_lookup(struct rte_lpm *lpm, uint32_t ip, uint32_t *next_hop)
{
	...
	RTE_LPM_RETURN_IF_TRUE(((lpm == NULL) || (next_hop == NULL)), -EILWAL);
 ...
 *  @return
 *   -EILWAL for incorrect arguments, otherwise 0
 */
