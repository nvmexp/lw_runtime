...
*  callwlate the number of ranges in a node to see if it fits in a quad range
*  node.
*/
static int
	...
	struct rte_acl_node *node, uint64_t no_match, int force_dfa)
{
	...
	/*
	 * relwrsively count the types of all children
	 */
	for (n = 0; n < node->num_ptrs; n++) {
		if (node->ptrs[n].ptr != NULL)
...
/*
 * Routine that allocates space for this node and relwrsively calls
	...
	uint64_t no_match, struct rte_acl_indices *index, int num_categories)
{
	...
	/* relwrsively allocate space for all children */
	for (n = 0; n < node->num_ptrs; n++) {
		if (node->ptrs[n].ptr != NULL)
