...
/*
 * Callwlate the address of the next transition for
 * all types of nodes. Note that only DFA nodes and range
 * nodes actually transition to another node. Match
 * nodes not supposed to be encountered here.
 * For quad range nodes:
 * Callwlate number of range boundaries that are less than the
	...
	_T_simd four_32, _T_simd range_base, _T_simd tr_lo, _T_simd tr_hi)
{
	...
	/* callwlate address for next transitions. */
	...
	_T_simd *tr_hi)
{
	...
	/* Callwlate the address (array index) for all _N_ transitions. */
	...
	_T_simd pdata[2], _T_simd *idx, _T_simd *di)
{
	...
	/* callwlate match indexes of new flows */
