	...
	struct rte_acl_bitset *intersect)
{
	...
	/*
	 * callwlate and store intersection and check if A and/or B have
	...
	struct rte_acl_build_rule **last, uint32_t *count)
{
	...
	*last = prev;
	...
	while (rule != NULL) {
		...
		if (node_count > context->lwr_node_max) {
			*last = prev;
			return trie;
		}
 ...
 * Use relwrsive mergesort algorithm.
 */
static struct rte_acl_build_rule *
