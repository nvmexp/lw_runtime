 ...
 * represented by an array of transitions. The next position is callwlated
 ...
 * In attempt to conserve space all transitions are divided into 4 conselwtive
 ...
 * So to callwlate transition index within the node for given input byte value:
...
struct rte_acl_node {
	...
	uint32_t                node_type;
	int32_t                 fanout;
	/* number of ranges (transitions w/ conselwtive bits) */
	int32_t                 id;
