...
/*
 * Every time rte_vhost_driver_register() is ilwoked, an associated
...
rte_vhost_driver_register(const char *path, uint64_t flags)
{
	 ...
	 * to ilwoke rte_vhost_driver_set_features(). To workaround it, here
	...
	for (i = 0; i < vhost_user.vsocket_cnt; i++) {
		...
		if (!strcmp(vsocket->path, path)) {
			...
			for (conn = TAILQ_FIRST(&vsocket->conn_list);
				...
				/*
				 * If r/wcb is exelwting, release vsocket's
			...
			if (vsocket->is_server) {
				/*
				 * If r/wcb is exelwting, release vhost_user's
