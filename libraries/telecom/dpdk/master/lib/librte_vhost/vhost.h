...
#define VIRTIO_ILWALID_EVENTFD		(-1)
...
#define VIRTIO_UNINITIALIZED_NOTIF	(-1)
	...
	int			callfd;
	/* Lwrrently unused as polling mode is enabled */
	int			kickfd;
...
#define VHOST_IOTLB_ILWALIDATE     3
...
/* Colwert guest physical address to host physical address */
static __rte_always_inline rte_iova_t
...
uint64_t translate_log_addr(struct virtio_net *dev, struct vhost_virtqueue *vq,
		uint64_t log_addr);
void vring_ilwalidate(struct virtio_net *dev, struct vhost_virtqueue *vq);
