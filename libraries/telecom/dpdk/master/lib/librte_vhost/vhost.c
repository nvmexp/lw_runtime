...
/*
 * Colwerts vring log address to GPA
...
void
vring_ilwalidate(struct virtio_net *dev, struct vhost_virtqueue *vq)
{
	if (dev->features & (1ULL << VIRTIO_F_IOMMU_PLATFORM))
...
/*
 * Ilwoked when there is a new vhost-user connection established (when
...
/*
 * Ilwoked when there is the vhost-user connection is broken (when
