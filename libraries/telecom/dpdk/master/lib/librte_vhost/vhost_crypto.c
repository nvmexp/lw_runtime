...
move_desc(struct vhost_crypto_desc *head,
		struct vhost_crypto_desc **lwr_desc,
		uint32_t size, uint32_t max_n_descs)
{
	struct vhost_crypto_desc *desc = *lwr_desc;
...
{
	void *data;
	uint64_t dlen = lwr_desc->len;

	data = IOVA_TO_VVA(void *, vc_req, lwr_desc->addr, &dlen, perm);
	if (unlikely(!data || dlen != lwr_desc->len)) {
 ...
 *   lwr_desc is expected to be updated after the function exelwtion.
 * @param end_wb_data
 ...
 *   destination data the offset is the number of bytes from *lwr_desc->addr
