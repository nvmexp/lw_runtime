...
/* Colwerts QEMU virtual address to Vhost virtual address. */
static uint64_t
...
/*
 * Colwerts ring address to Vhost virtual address.
...
translate_ring_addresses(struct virtio_net *dev, int vq_index)
{
	...
	/* The addresses are colwerted from QEMU virtual to Vhost virtual. */
 ...
 * This function then colwerts these to our address space.
 */
static int
			...
			int main_fd __rte_unused)
{
	...
	if (msg->payload.u64 & VHOST_USER_VRING_NOFD_MASK)
		file.fd = VIRTIO_ILWALID_EVENTFD;
	else
		file.fd = msg->fds[0];
	VHOST_LOG_CONFIG(INFO,
			...
			int main_fd __rte_unused)
{
	...
	if (!dev) {
		if (file.fd != VIRTIO_ILWALID_EVENTFD)
			close(file.fd);
...
skip_to_post_handle:
	...
	for (i = 0; i < dev->nr_vring; i++) {
		struct vhost_virtqueue *vq = dev->virtqueue[i];
		bool lwr_ready = vq_is_ready(dev, vq);

		if (lwr_ready != (vq && vq->ready)) {
			vq->ready = lwr_ready;
			vhost_user_notify_queue_state(dev, i, lwr_ready);
		}
	}
