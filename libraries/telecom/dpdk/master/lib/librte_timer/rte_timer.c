...
rte_timer_data_dealloc(uint32_t id)
{
	...
	TIMER_DATA_VALID_GET_OR_ERR_RET(id, timer_data, -EILWAL);
 ...
 */
static uint32_t
timer_get_skiplist_level(unsigned lwrr_depth)
{
#ifdef RTE_LIBRTE_TIMER_DEBUG
	 ...
	 * 1 in 16 at level 2, 1 in 64 at level 3, etc. Callwlated using lowest
...
{
	unsigned lvl = priv_timer[tim_lcore].lwrr_skiplist_depth;
				...
				struct priv_timer *priv_timer)
{
	...
	for (i = priv_timer[tim_lcore].lwrr_skiplist_depth - 1; i >= 0; i--) {
	  ...
	  struct priv_timer *priv_timer)
{
	...
	const unsigned tim_level = timer_get_skiplist_level(
			priv_timer[tim_lcore].lwrr_skiplist_depth);
	if (tim_level == priv_timer[tim_lcore].lwrr_skiplist_depth)
		priv_timer[tim_lcore].lwrr_skiplist_depth++;
	  ...
	  int local_is_locked, struct priv_timer *priv_timer)
{
	...
	for (i = priv_timer[prev_owner].lwrr_skiplist_depth - 1; i >= 0; i--) {
	...
	for (i = priv_timer[prev_owner].lwrr_skiplist_depth - 1; i >= 0; i--)
		if (priv_timer[prev_owner].pending_head.sl_next[i] == NULL)
			priv_timer[prev_owner].lwrr_skiplist_depth --;
		else
			break;
...
{
	uint64_t lwr_time = rte_get_timer_cycles();
	uint64_t period;
	struct rte_timer_data *timer_data;

	TIMER_DATA_VALID_GET_OR_ERR_RET(timer_data_id, timer_data, -EILWAL);
	...
	return __rte_timer_reset(tim,  lwr_time + ticks, period, tim_lcore,
...
__rte_timer_manage(struct rte_timer_data *timer_data)
{
	 ...
	 * outside the lock */
	if (likely(priv_timer[lcore_id].pending_head.expire > lwr_time))
		return;
#endif
	...
	if (priv_timer[lcore_id].pending_head.sl_next[0] == NULL ||
	    priv_timer[lcore_id].pending_head.sl_next[0]->expire > lwr_time) {
	...
	timer_get_prev_entries(lwr_time, lcore_id, prev, priv_timer);
	for (i = priv_timer[lcore_id].lwrr_skiplist_depth -1; i >= 0; i--) {
		if (prev[i] == &priv_timer[lcore_id].pending_head)
			continue;
		priv_timer[lcore_id].pending_head.sl_next[i] =
		    prev[i]->sl_next[i];
		if (prev[i]->sl_next[i] == NULL)
			priv_timer[lcore_id].lwrr_skiplist_depth--;
...
rte_timer_manage(void)
{
	...
	TIMER_DATA_VALID_GET_OR_ERR_RET(default_data_id, timer_data, -EILWAL);
		     ...
		     rte_timer_alt_manage_cb_t f)
{
	...
	TIMER_DATA_VALID_GET_OR_ERR_RET(timer_data_id, data, -EILWAL);
	...
	for (i = 0; i < nb_poll_lcores; i++) {
		 ...
		 */
		if (likely(privp->pending_head.expire > lwr_time))
			continue;
#endif
		...
		/* if nothing to do just unlock and return */
		if (privp->pending_head.sl_next[0] == NULL ||
		    privp->pending_head.sl_next[0]->expire > lwr_time) {
		...
		/* break the existing list at current time point */
		timer_get_prev_entries(lwr_time, poll_lcore, prev,
				       data->priv_timer);
		for (j = privp->lwrr_skiplist_depth - 1; j >= 0; j--) {
			if (prev[j] == &privp->pending_head)
				continue;
			privp->pending_head.sl_next[j] =
				prev[j]->sl_next[j];
			if (prev[j]->sl_next[j] == NULL)
				privp->lwrr_skiplist_depth--;
