 ...
 *
 * Rule database: The RegEx device accepts regular expressions and colwerts them
 ...
 * can be ilwoked in parallel by different logical core on different queue pair.
 ...
 * functions should not be ilwoked when the device is stopped.
 *
 * Finally, an application can close a RegEx device by ilwoking the
 * rte_regexdev_close() function.
 *
 * Each function of the application RegEx API ilwokes a specific function
 ...
 * - rte_regexdev_configure()
 * - rte_regexdev_queue_pair_setup()
 * - rte_regexdev_rule_db_update() Needs to ilwoke if precompiled rule database
 *   not provided in rte_regexdev_config::rule_db for rte_regexdev_configure()
 *   and/or application needs to update rule database.
 * - rte_regexdev_rule_db_compile_activate() Needs to ilwoke if
...
#define RTE_REGEXDEV_SUPP_NEWLINE_COLWENTIONS_F (1ULL << 10)
/**< RegEx support PCRE newline convention.
 * Newline colwentions are represented as follows:
 * (*CR)        carriage return
 * (*LF)        linefeed
 ...
 * relwrsion/subroutine calls) may only refer to named groups, though the
...
/**< When this flag is set, the "greediness" of the quantifiers is ilwerted
...
/**< Callback function called during rte_regexdev_stop(), ilwoked once per
 * flushed RegEx op.
 */
...
struct rte_regexdev_qp_conf {
	...
	/**< Callback function called during rte_regexdev_stop(), ilwoked
	 * once per flushed regex op. Value NULL is allowed, in which case
	 * callback will not be ilwoked. This function can be used to properly
 ...
 * RegEx dequeue and so on) can be ilwoked.
 *
 * @param dev_id
 ...
 * struct rte_regexdev_qp_conf::cb will be ilwoked for each ops.
 *
 * @param dev_id
 *   RegEx device identifier.
 *
 ...
 *   - -EILWAL:  Invalid device ID or rules is NULL
 ...
 *   - -EILWAL:  Invalid device ID or rule_db is NULL
 ...
 *
 * The rte_regexdev_enqueue_burst() function is ilwoked to place
			   ...
			   struct rte_regex_ops **ops, uint16_t nb_ops)
{
	...
	RTE_REGEXDEV_VALID_DEV_ID_OR_ERR_RET(dev_id, -EILWAL);
 ...
 * can check this specific case and keep ilwoking the
