...
/* Mark the area oclwpied by a mbuf field as available in the shm. */
...
void rte_mbuf_dyn_dump(FILE *out)
{
	...
	fprintf(out, "Free space in mbuf (0 = oclwpied, value = free zone alignment):\n");
	for (i = 0; i < sizeof(struct rte_mbuf); i++) {
		if ((i % 8) == 0)
			fprintf(out, "  %4.4zx: ", i);
	...
	fprintf(out, "Free bit in mbuf->ol_flags (0 = oclwpied, 1 = free):\n");
