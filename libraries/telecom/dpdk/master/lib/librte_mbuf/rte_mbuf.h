...
rte_mbuf_refcnt_update(struct rte_mbuf *m, int16_t value)
{
	 ...
	 * operation has to be used because conlwrrent accesses on the
 ...
 *    - EILWAL - cache size provided is too large, or priv_size is not aligned.
 ...
 * ilwokes rte_mbuf_refcnt_update().
 *
 * @param m
...
static inline int rte_pktmbuf_chain(struct rte_mbuf *head, struct rte_mbuf *tail)
{
	...
	lwr_tail = rte_pktmbuf_lastseg(head);
	lwr_tail->next = tail;
