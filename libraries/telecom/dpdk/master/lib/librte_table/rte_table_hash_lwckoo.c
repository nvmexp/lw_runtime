...
#include "rte_table_hash_lwckoo.h"

#ifdef RTE_TABLE_STATS_COLLECT

#define RTE_TABLE_HASH_LWCKOO_STATS_PKTS_IN_ADD(table, val) \
	(table->stats.n_pkts_in += val)
#define RTE_TABLE_HASH_LWCKOO_STATS_PKTS_LOOKUP_MISS(table, val) \
...
static int
check_params_create_hash_lwckoo(struct rte_table_hash_lwckoo_params *params)
{
	if (params == NULL) {
...
static void *
rte_table_hash_lwckoo_create(void *params,
			int socket_id,
			uint32_t entry_size)
{
	struct rte_table_hash_lwckoo_params *p = params;
	struct rte_hash *h_table;
	struct rte_table_hash *t;
	uint32_t total_size;

	/* Check input parameters */
	if (check_params_create_hash_lwckoo(params))
		return NULL;
	...
	if (t == NULL) {
		RTE_LOG(ERR, TABLE,
			"%s: Cannot allocate %u bytes for lwckoo hash table %s\n",
			__func__, total_size, p->name);
		return NULL;
	}

	/* Create lwckoo hash table */
	struct rte_hash_parameters hash_lwckoo_params = {
		.entries = p->n_keys,
		.key_len = p->key_size,
		.hash_func = p->f_hash,
	...
	if (h_table == NULL) {
		h_table = rte_hash_create(&hash_lwckoo_params);
		if (h_table == NULL) {
			RTE_LOG(ERR, TABLE,
				"%s: failed to create lwckoo hash table %s\n",
				__func__, p->name);
			rte_free(t);
			return NULL;
		}
	...
	/* initialize the lwckoo hash parameters */
	...
	RTE_LOG(INFO, TABLE,
		"%s: Lwckoo hash table %s memory footprint is %u bytes\n",
		__func__, p->name, total_size);
	return t;
}

static int
rte_table_hash_lwckoo_free(void *table) {
...
static int
rte_table_hash_lwckoo_entry_add(void *table, void *key, void *entry,
...
static int
rte_table_hash_lwckoo_entry_delete(void *table, void *key,
...
static int
rte_table_hash_lwckoo_lookup(void *table,
	...
	void **entries)
{
	...
	RTE_TABLE_HASH_LWCKOO_STATS_PKTS_IN_ADD(t, n_pkts_in);
	...
	*lookup_hit_mask = pkts_mask_out;
	RTE_TABLE_HASH_LWCKOO_STATS_PKTS_LOOKUP_MISS(t,
...
static int
rte_table_hash_lwckoo_stats_read(void *table, struct rte_table_stats *stats,
	int clear)
{
	struct rte_table_hash *t = table;
...
struct rte_table_ops rte_table_hash_lwckoo_ops = {
	.f_create = rte_table_hash_lwckoo_create,
	.f_free = rte_table_hash_lwckoo_free,
	.f_add = rte_table_hash_lwckoo_entry_add,
	.f_delete = rte_table_hash_lwckoo_entry_delete,
	.f_add_bulk = NULL,
	.f_delete_bulk = NULL,
	.f_lookup = rte_table_hash_lwckoo_lookup,
	.f_stats = rte_table_hash_lwckoo_stats_read,
};
