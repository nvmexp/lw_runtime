...
static void *
elw_malloc(size_t size, size_t alignment, int numa_node)
{
	return rte_zmalloc_socket(NULL, size, alignment, numa_node);
}

static void
elw_free(void *start, size_t size __rte_unused)
{
	rte_free(start);
}
...
static void *
elw_malloc(size_t size, size_t alignment __rte_unused, int numa_node)
{
	return numa_alloc_onnode(size, numa_node);
}

static void
elw_free(void *start, size_t size)
{
	       ...
	       int numa_node)
{
	...
	CHECK(params, EILWAL);
	CHECK(params->match_type == RTE_SWX_TABLE_MATCH_EXACT, EILWAL);
	CHECK(params->key_size, EILWAL);
	CHECK(params->key_size <= 64, EILWAL);
	CHECK(params->n_keys_max, EILWAL);
	...
	memory = elw_malloc(total_size, RTE_CACHE_LINE_SIZE, numa_node);
	CHECK(memory,  ENOMEM);
...
table_add(void *table, struct rte_swx_table_entry *entry)
{
	...
	CHECK(t, EILWAL);
	CHECK(entry, EILWAL);
	CHECK(entry->key, EILWAL);
	CHECK((!t->params.action_data_size && !entry->action_data) ||
	      (t->params.action_data_size && entry->action_data), EILWAL);
