 ...
 * for multiple conlwrrent lookup operations into the same table.
 *
 * @param[in] params
 *   Table creation parameters.
 * @param[in] entries
 ...
 *   -EILWAL: Invalid table handle, entry or entry field;
 *   -ENOSPC: Table full.
 */
typedef int
 ...
 *
 * Multiple ilwocations of this function may be required in order to complete a
 ...
 * in case of a return value of 0, the function must be ilwoked again with
 * exactly the same arguments.
 *
 * The mailbox argument is used to store the context of an on-going table lookup
 * operation. The mailbox mechanism allows for multiple conlwrrent table lookup
 ...
 * exelwtes in parallel with the source data being fetched into the CPU cache;
