		...
		uint32_t *prim_bkt, uint32_t *sec_bkt, member_sig_t *sig)
{
	 ...
	 * lwckoo hashing" proposed by B. Fan, et al's paper
	 * "Lwckoo Filter: Practically Better Than Bloom". The benefit to use
...
/*
 * This function is similar to the lwckoo hash make_space function in hash
 * library
 */
static inline int
			...
			unsigned int *nr_pushes)
{
	...
	/* Alternative location has spare room (end of relwrsive function) */
		...
		const void *key, member_set_t set_id)
{
	...
	/* Random pick prim or sec for relwrsive displacement */
