 ...
 * filter (for example Bloom Filter and lwckoo filter) structure that has
 ...
 * @b EXPERIMENTAL: this API may change without prior notice
 *
 * Parameters used when create the set summary table. Lwrrently user can
...
struct rte_member_parameters {
	 ...
	 * be evicted to other buckets like lwckoo hash. The table will also
	 ...
	 * evenly distributed to each BF in vBF. This is used to callwlate the
	...
	/**
	 * The length of key is used for hash callwlation. Since key is not
	 ...
	 * callwlate the total number of bits for each BF, and the number of
	...
	/**
	 * We use two seeds to callwlate two independent hashes for each key.
	 *
 ...
 *   valid range. In such case -EILWAL is returned.
 ...
 *   -ENOSPC for full, and 1 if lwckoo eviction happens.
 ...
 * in current implementation. For vBF, error code of -EILWAL will be returned.
 *
 * @param setsum
 *   Pointer to the set-summary.
 * @param key
