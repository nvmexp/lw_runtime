...
#define MIN_HASH_ENTRIES	8U /* From rte_lwckoo_hash.h */
...
rte_ipsec_sad_create(const char *name, const struct rte_ipsec_sad_conf *conf)
{
	...
	if (conf->flags & RTE_IPSEC_SAD_FLAG_RW_CONLWRRENCY)
		hash_params.extra_flag = RTE_HASH_EXTRA_FLAGS_RW_CONLWRRENCY;
