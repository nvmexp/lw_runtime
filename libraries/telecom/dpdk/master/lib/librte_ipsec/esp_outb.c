	...
	union sym_op_data *icv, uint8_t sqh_len)
{
	...
	/* callwlate extra header space required */
		...
		esp_outb_prepare_t prepare, uint32_t cofs_mask)
{
	...
	/* colwert mbufs to iovecs and do actual crypto/auth processing */
	if (k != 0)
	...
	struct rte_mbuf *mb[], uint16_t num)
{
	...
	ol_flags = ss->security.ol_flags & RTE_SELWRITY_TX_OLOAD_NEED_MDATA;
	for (i = 0; i != num; i++) {

		mb[i]->ol_flags |= PKT_TX_SEC_OFFLOAD;
		if (ol_flags != 0)
			rte_selwrity_set_pkt_metadata(ss->security.ctx,
...
/*
 * outbound for RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL:
