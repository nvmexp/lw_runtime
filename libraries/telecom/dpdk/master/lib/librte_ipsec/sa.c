...
/**
 * Based on number of buckets callwlated required size for the
...
/*
 * for given size, callwlate required number of buckets.
 */
static uint32_t
...
fill_sa_type(const struct rte_ipsec_sa_prm *prm, uint64_t *type)
{
	...
	if (prm->ipsec_xform.proto == RTE_SELWRITY_IPSEC_SA_PROTO_AH)
		tp |= RTE_IPSEC_SATP_PROTO_AH;
	else if (prm->ipsec_xform.proto == RTE_SELWRITY_IPSEC_SA_PROTO_ESP)
		tp |= RTE_IPSEC_SATP_PROTO_ESP;
	else
		return -EILWAL;

	if (prm->ipsec_xform.direction == RTE_SELWRITY_IPSEC_SA_DIR_EGRESS)
	...
	if (prm->ipsec_xform.mode == RTE_SELWRITY_IPSEC_SA_MODE_TUNNEL) {
	...
	uint32_t size)
{
	...
	if (prm->ipsec_xform.proto != RTE_SELWRITY_IPSEC_SA_PROTO_ESP)
		return -EILWAL;
	...
	struct rte_mbuf *mb[], struct rte_crypto_op *cop[], uint16_t num)
{
	...
	for (i = 0; i != num; i++) {
		...
		cop[i]->sess_type = RTE_CRYPTO_OP_SELWRITY_SESSION;
		sop->m_src = mb[i];
		__rte_selwrity_attach_session(sop, ss->security.ses);
	}
}
 ...
 * used for:
 * - inbound for RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL
 * - inbound/outbound for RTE_SELWRITY_ACTION_TYPE_LOOKASIDE_PROTOCOL
 * - outbound for RTE_SELWRITY_ACTION_TYPE_NONE when ESN is disabled
 */
uint16_t
