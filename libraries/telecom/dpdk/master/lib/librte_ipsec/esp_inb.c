	...
	struct rte_mbuf *mb, uint32_t hlen, rte_be64_t *sqc)
{
	 ...
	 * colwert it back into network byte order.
	 */
	...
	uint32_t hlen, union sym_op_data *icv)
{
	 ...
	 * Would probably be more colwenient to reserve space for AAD
	 ...
	 * callwlation but are not transmitted, update packet length
	...
	uint32_t adj, uint32_t tofs, uint32_t tlen, uint32_t *sqn)
{
	...
	/* cut of ICV, ESP tail and padding bytes */
	mbuf_lwt_seg_ofs(mb, ml, tofs, tlen);
