...
#define RTE_IPSEC_SAD_FLAG_RW_CONLWRRENCY	0x2
...
/**
 * Add a rule into the SAD. Could be safely called with conlwrrent lookups
 *  if RTE_IPSEC_SAD_FLAG_RW_CONLWRRENCY flag was configured on creation time.
...
/**
 * Delete a rule from the SAD. Could be safely called with conlwrrent lookups
 *  if RTE_IPSEC_SAD_FLAG_RW_CONLWRRENCY flag was configured on creation time.
 *  While with this flag multi-reader - one-writer model Is MT safe,
 *  multi-writer model is not and required extra synchronisation.
 *
 ...
 *  @return
 *   -EILWAL for incorrect arguments, otherwise number of successful lookups.
 */
int
