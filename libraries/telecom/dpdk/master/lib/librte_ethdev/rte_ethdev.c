...
struct rte_eth_dev_callback {
	...
	uint32_t active;                        /**< Callback is exelwting */
};
...
#define iter_anybus_str "class=eth,"
	...
	/* Colwert bus args to new syntax for use with new API dev_iterate. */
...
rte_eth_dev_get_sec_ctx(uint16_t port_id)
{
	...
	return rte_eth_devices[port_id].selwrity_ctx;
}
 ...
 *   - (-EILWAL) if requested offload has been silently disabled.
 *
 */
static int
		...
		struct rte_eth_dev_info *dev_info, uint16_t port_id)
{
	...
	/* replay promislwous configuration */
	/*
	 * use callbacks directly since we don't need port_id check and
	 * would like to bypass the same value set
	 */
	if (rte_eth_promislwous_get(port_id) == 1 &&
	    *dev->dev_ops->promislwous_enable != NULL) {
		ret = eth_err(port_id,
			      (*dev->dev_ops->promislwous_enable)(dev));
		if (ret != 0 && ret != -ENOTSUP) {
			RTE_ETHDEV_LOG(ERR,
				"Failed to enable promislwous mode for device (port %u): %s\n",
				port_id, rte_strerror(-ret));
			return ret;
		}
	} else if (rte_eth_promislwous_get(port_id) == 0 &&
		   *dev->dev_ops->promislwous_disable != NULL) {
		ret = eth_err(port_id,
			      (*dev->dev_ops->promislwous_disable)(dev));
		if (ret != 0 && ret != -ENOTSUP) {
			RTE_ETHDEV_LOG(ERR,
				"Failed to disable promislwous mode for device (port %u): %s\n",
				port_id, rte_strerror(-ret));
			return ret;
		}
	}
...
int
rte_eth_promislwous_enable(uint16_t port_id)
{
	struct rte_eth_dev *dev;
	int diag = 0;
	...
	RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->promislwous_enable, -ENOTSUP);

	diag = (*dev->dev_ops->promislwous_enable)(dev);
	dev->data->promislwous = (diag == 0) ? 1 : 0;

	return eth_err(port_id, diag);
}

int
rte_eth_promislwous_disable(uint16_t port_id)
{
	struct rte_eth_dev *dev;
	int diag = 0;
	...
	if (dev->data->promislwous == 0)
		return 0;

	RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->promislwous_disable, -ENOTSUP);

	dev->data->promislwous = 0;
	diag = (*dev->dev_ops->promislwous_disable)(dev);
	if (diag != 0)
...
int
rte_eth_promislwous_get(uint16_t port_id)
{
	struct rte_eth_dev *dev;
	...
	uint64_t *ids)
{
	...
	if (ids && dev->dev_ops->xstats_get_names_by_id != NULL && size > 0) {
		...
		for (i = 0; i < size; i++) {
			...
			/*
			 * Colwert ids to xstats ids that PMD knows.
...
rte_eth_dev_info_get(uint16_t port_id, struct rte_eth_dev_info *dev_info)
{
	...
	dev_info->switch_info.domain_id = RTE_ETH_DEV_SWITCH_DOMAIN_ID_ILWALID;
...
rte_eth_dev_set_vlan_offload(uint16_t port_id, int offload_mask)
{
	...
	lwr = !!(offload_mask & ETH_VLAN_STRIP_OFFLOAD);
	...
	lwr = !!(offload_mask & ETH_VLAN_FILTER_OFFLOAD);
	...
	lwr = !!(offload_mask & ETH_VLAN_EXTEND_OFFLOAD);
	...
	lwr = !!(offload_mask & ETH_QINQ_STRIP_OFFLOAD);
			...
			rte_eth_dev_cb_fn cb_fn, void *cb_arg)
{
	...
	do {
		...
		for (cb = TAILQ_FIRST(&dev->link_intr_cbs); cb != NULL;
			...
			/*
			 * if this callback is not exelwting right now,
			 * then remove it.
			 */
			if (cb->active == 0) {
	...
	ethdev_init_t ethdev_init, void *init_params)
{
	...
	RTE_FUNC_PTR_OR_ERR_RET(*ethdev_init, -EILWAL);
	...
	ethdev_uninit_t ethdev_uninit)
{
	...
	RTE_FUNC_PTR_OR_ERR_RET(*ethdev_uninit, -EILWAL);
...
rte_eth_switch_domain_alloc(uint16_t *domain_id)
{
	...
	*domain_id = RTE_ETH_DEV_SWITCH_DOMAIN_ID_ILWALID;
...
{
	if (domain_id == RTE_ETH_DEV_SWITCH_DOMAIN_ID_ILWALID ||
...
rte_eth_hairpin_queue_peer_update(uint16_t peer_port, uint16_t peer_queue,
				  struct rte_hairpin_peer_info *lwr_info,
				  ...
				  uint32_t direction)
{
	...
	return (*dev->dev_ops->hairpin_queue_peer_update)(dev, peer_queue,
					lwr_info, peer_info, direction);
}

int
rte_eth_hairpin_queue_peer_bind(uint16_t lwr_port, uint16_t lwr_queue,
				struct rte_hairpin_peer_info *peer_info,
				uint32_t direction)
{
	struct rte_eth_dev *dev;
	...
	/* No need to check the validity again. */
	dev = &rte_eth_devices[lwr_port];
	RTE_FUNC_PTR_OR_ERR_RET(*dev->dev_ops->hairpin_queue_peer_bind,
				-ENOTSUP);

	return (*dev->dev_ops->hairpin_queue_peer_bind)(dev, lwr_queue,
							peer_info, direction);
}

int
rte_eth_hairpin_queue_peer_unbind(uint16_t lwr_port, uint16_t lwr_queue,
				  uint32_t direction)
{
	struct rte_eth_dev *dev;
	...
	return (*dev->dev_ops->hairpin_queue_peer_unbind)(dev, lwr_queue,
							  direction);
}
