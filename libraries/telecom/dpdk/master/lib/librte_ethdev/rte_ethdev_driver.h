...
/**
 * @internal
 * Function used to enable the Rx promislwous mode of an Ethernet device.
 *
 * @param dev
 ...
 *
 * @retval 0
 *   Success, promislwous mode is enabled.
 * @retval -ENOTSUP
 *   Promislwous mode is not supported.
 * @retval -ENODEV
 *   Device is gone.
 * @retval -E_RTE_SECONDARY
 *   Function was called from a secondary process instance and not supported.
 * @retval -ETIMEDOUT
 *   Attempt to enable promislwos mode failed because of timeout.
 * @retval -EAGAIN
 *   Failed to enable promislwous mode.
 */
typedef int (*eth_promislwous_enable_t)(struct rte_eth_dev *dev);

/**
 * @internal
 * Function used to disable the Rx promislwous mode of an Ethernet device.
 *
 * @param dev
 *   ethdev handle of port.
 *
 ...
 *
 * @retval 0
 *   Success, promislwous mode is disabled.
 * @retval -ENOTSUP
 *   Promislwous mode disabling is not supported.
 * @retval -ENODEV
 *   Device is gone.
 * @retval -E_RTE_SECONDARY
 *   Function was called from a secondary process instance and not supported.
 * @retval -ETIMEDOUT
 *   Attempt to disable promislwos mode failed because of timeout.
 * @retval -EAGAIN
 *   Failed to disable promislwous mode.
 */
typedef int (*eth_promislwous_disable_t)(struct rte_eth_dev *dev);
...
typedef int (*eth_hairpin_queue_peer_update_t)
	(struct rte_eth_dev *dev, uint16_t peer_queue,
	 struct rte_hairpin_peer_info *lwrrent_info,
...
struct eth_dev_ops {
	...
	eth_promislwous_enable_t   promislwous_enable; /**< Promislwous ON. */
	eth_promislwous_disable_t  promislwous_disable;/**< Promislwous OFF. */
...
/**
 * @internal Exelwtes all the user application registered callbacks for
