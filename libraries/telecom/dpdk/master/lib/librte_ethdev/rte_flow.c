...
static const struct rte_flow_desc_data rte_flow_desc_item[] = {
	...
	MK_FLOW_ITEM(LWGRE, sizeof(struct rte_flow_item_lwgre)),
...
static const struct rte_flow_desc_data rte_flow_desc_action[] = {
	...
	MK_FLOW_ACTION(SECURITY, sizeof(struct rte_flow_action_selwrity)),
	...
	MK_FLOW_ACTION(LWGRE_ENCAP, sizeof(struct rte_flow_action_vxlan_encap)),
	MK_FLOW_ACTION(LWGRE_DECAP, 0),
...
enum rte_flow_colw_item_spec_type {
	RTE_FLOW_COLW_ITEM_SPEC,
	RTE_FLOW_COLW_ITEM_LAST,
	RTE_FLOW_COLW_ITEM_MASK,
};
 ...
 */
static size_t
rte_flow_colw_item_spec(void *buf, const size_t size,
			const struct rte_flow_item *item,
			enum rte_flow_colw_item_spec_type type)
{
	size_t off;
	const void *data =
		type == RTE_FLOW_COLW_ITEM_SPEC ? item->spec :
		type == RTE_FLOW_COLW_ITEM_LAST ? item->last :
		type == RTE_FLOW_COLW_ITEM_MASK ? item->mask :
		NULL;
	...
	case RTE_FLOW_ITEM_TYPE_RAW:
		...
		off = sizeof(*dst.raw);
		if (type == RTE_FLOW_COLW_ITEM_SPEC ||
		    (type == RTE_FLOW_COLW_ITEM_MASK &&
 ...
 */
static size_t
rte_flow_colw_action_conf(void *buf, const size_t size,
			  const struct rte_flow_action *action)
{
	size_t off;
	...
	switch (action->type) {
		union {
			...
			const struct rte_flow_action_lwgre_encap *lwgre_encap;
		} src;
		union {
			struct rte_flow_action_rss *rss;
			struct rte_flow_action_vxlan_encap *vxlan_encap;
			struct rte_flow_action_lwgre_encap *lwgre_encap;
		} dst;
		size_t tmp;
		int ret;
	...
	case RTE_FLOW_ACTION_TYPE_LWGRE_ENCAP:
		...
		RTE_BUILD_BUG_ON(sizeof(*src.vxlan_encap) !=
				 sizeof(*src.lwgre_encap) ||
				 offsetof(struct rte_flow_action_vxlan_encap,
					  definition) !=
				 offsetof(struct rte_flow_action_lwgre_encap,
					  definition));
 ...
 */
static int
rte_flow_colw_pattern(struct rte_flow_item *dst,
		      const size_t size,
		...
		if (((int)src->type >= 0) &&
			...
			return rte_flow_error_set
				(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ITEM, src,
				 "cannot colwert unknown item type");
 ...
 */
static int
rte_flow_colw_actions(struct rte_flow_action *dst,
		      const size_t size,
		...
		if (((int)src->type >= 0) &&
			...
			return rte_flow_error_set
				(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_ACTION,
				 src, "cannot colwert unknown action type");
		if (size >= off + sizeof(*dst))
 ...
 */
static int
rte_flow_colw_rule(struct rte_flow_colw_rule *dst,
		   const size_t size,
		   const struct rte_flow_colw_rule *src,
		   struct rte_flow_error *error)
{
	size_t off;
	int ret;
	...
	if (src->pattern_ro) {
		off = RTE_ALIGN_CEIL(off, sizeof(double));
		ret = rte_flow_colw_pattern((void *)((uintptr_t)dst + off),
					    size > off ? size - off : 0,
	...
	if (src->actions_ro) {
		off = RTE_ALIGN_CEIL(off, sizeof(double));
		ret = rte_flow_colw_actions((void *)((uintptr_t)dst + off),
					    size > off ? size - off : 0,
 ...
 */
static int
rte_flow_colw_name(int is_action,
		   int is_ptr,
		   char *dst,
		   const size_t size,
		   const void *src,
		   struct rte_flow_error *error)
{
	...
	if (type >= info->num)
		return rte_flow_error_set
			(error, EILWAL, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
...
/** Helper function to colwert flow API objects. */
int
	      ...
	      struct rte_flow_error *error)
{
	...
	case RTE_FLOW_COLW_OP_NONE:
		return 0;
	case RTE_FLOW_COLW_OP_ATTR:
		attr = src;
		if (size > sizeof(*attr))
			size = sizeof(*attr);
		rte_memcpy(dst, attr, size);
		return sizeof(*attr);
	case RTE_FLOW_COLW_OP_ITEM:
		return rte_flow_colw_pattern(dst, size, src, 1, error);
	case RTE_FLOW_COLW_OP_ACTION:
		return rte_flow_colw_actions(dst, size, src, 1, error);
	case RTE_FLOW_COLW_OP_PATTERN:
		return rte_flow_colw_pattern(dst, size, src, 0, error);
	case RTE_FLOW_COLW_OP_ACTIONS:
		return rte_flow_colw_actions(dst, size, src, 0, error);
	case RTE_FLOW_COLW_OP_RULE:
		return rte_flow_colw_rule(dst, size, src, error);
	case RTE_FLOW_COLW_OP_ITEM_NAME:
		return rte_flow_colw_name(0, 0, dst, size, src, error);
	case RTE_FLOW_COLW_OP_ACTION_NAME:
		return rte_flow_colw_name(1, 0, dst, size, src, error);
	case RTE_FLOW_COLW_OP_ITEM_NAME_PTR:
		return rte_flow_colw_name(0, 1, dst, size, src, error);
	case RTE_FLOW_COLW_OP_ACTION_NAME_PTR:
		return rte_flow_colw_name(1, 1, dst, size, src, error);
	}
	return rte_flow_error_set
		(error, ENOTSUP, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
		 "unknown object colwersion operation");
}
...
{
	/*
	 * Overlap struct rte_flow_colw with struct rte_flow_desc in order
	 * to colwert the former to the latter without wasting space.
	 */
	struct rte_flow_colw_rule *dst =
		len ?
		(void *)((uintptr_t)desc +
			 (offsetof(struct rte_flow_desc, actions) -
			  offsetof(struct rte_flow_colw_rule, actions))) :
		NULL;
	size_t dst_size =
		len > sizeof(*desc) - sizeof(*dst) ?
		len - (sizeof(*desc) - sizeof(*dst)) :
		0;
	struct rte_flow_colw_rule src = {
		.attr_ro = NULL,
		.pattern_ro = items,
		.actions_ro = actions,
	};
	...
	RTE_BUILD_BUG_ON(sizeof(struct rte_flow_desc) <
			 sizeof(struct rte_flow_colw_rule));
	if (dst_size &&
	...
	ret = rte_flow_colw(RTE_FLOW_COLW_OP_RULE, dst, dst_size, &src, NULL);
	if (ret < 0)
		return 0;
