...
enum rte_tm_stats_type {
	...
	/** Number of packets lwrrently waiting in the packet queue of current
	 * leaf node.
	 */
	RTE_TM_STATS_N_PKTS_QUEUED = 1 << 8,

	/** Number of bytes lwrrently waiting in the packet queue of current
	 * leaf node.
	 */
...
struct rte_tm_node_stats {
	...
	struct {
		...
		/** Number of packets lwrrently waiting in the packet queue of
		 * current leaf node.
		 */
		uint64_t n_pkts_queued;

		/** Number of bytes lwrrently waiting in the packet queue of
		 * current leaf node.
		 */
...
struct rte_tm_capabilities {
	 ...
	 * nodes that can conlwrrently have their private shaper enabled. The
	 ...
	 * Indicates the maximum number of nodes that can conlwrrently have
	 ...
	 * number of leaf nodes that can conlwrrently have their private WRED
 ...
 * more and more input packets as the queue oclwpancy builds up. When the queue
...
struct rte_tm_red_params {
	...
	/** Ilwerse of packet marking probability maximum value (maxp), i.e.
	 * maxp_ilw = 1 / maxp
	 */
	uint16_t maxp_ilw;
	...
	union {
		...
		struct {
			...
			struct {
				 ...
				 * current leaf node should not lwrrently be
 ...
 * lwrrently at least one user (i.e. WRED context) of this WRED profile.
 *
 * @param[in] port_id
 *   The port identifier of the Ethernet device.
 * @param[in] wred_profile_id
 ...
 * lwrrently at least one user (i.e. hierarchy leaf node) of this shared WRED
 * context.
 *
 * @param[in] port_id
 *   The port identifier of the Ethernet device.
 ...
 * lwrrently at least one user (i.e. shaper) of this shaper profile.
 *
 * @param[in] port_id
 *   The port identifier of the Ethernet device.
 * @param[in] shaper_profile_id
 ...
 * lwrrently at least one user (i.e. hierarchy node) of this shared shaper.
 *
 * @param[in] port_id
 *   The port identifier of the Ethernet device.
 * @param[in] shared_shaper_id
 ...
 *   successive ilwocations of this function. When not desired, this check can
 ...
 *
 * Delete an existing node. This operation fails when this node lwrrently has
 ...
 *
 * Resume an existing node that is lwrrently in suspended state. The node
 ...
 *       current port through successive rte_tm_node_add() ilwocations;
 ...
 *
 * This function fails when the lwrrently configured hierarchy is not supported
 ...
 *   lwrrently enabled for this node, indicating which of the counters
