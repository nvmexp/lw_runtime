...
enum rte_flow_item_type {
	 ...
	 * [META]
	 *
	 * Used as a placeholder for colwenience. It is ignored and simply
	 * discarded by PMDs.
	 *
	 * No associated specification structure.
	 */
	 ...
	 * [META]
	 *
	 * Ilwerted matching, i.e. process packets that do not match the
	 * pattern.
	 *
	 * No associated specification structure.
	 */
	RTE_FLOW_ITEM_TYPE_ILWERT,
	...
	/**
	 * Matches a LWGRE header.
	 *
	 * See struct rte_flow_item_lwgre.
	 */
...
/** Default mask for RTE_FLOW_ITEM_TYPE_LWGRE. */
#ifndef __cplusplus
static const struct rte_flow_item_lwgre rte_flow_item_lwgre_mask = {
	.tni = "\xff\xff\xff",
};
#endif
...
/**
 * RTE_FLOW_ITEM_TYPE_VXLAN_GPE (draft-ietf-lwo3-vxlan-gpe-05).
 *
 * Matches a VXLAN-GPE header.
 */
...
enum rte_flow_action_type {
	 ...
	 *
	 * See struct rte_flow_action_selwrity.
	 */
	RTE_FLOW_ACTION_TYPE_SELWRITY,
	...
	/**
	 * Encapsulate flow in LWGRE tunnel defined in the
	 * rte_flow_action_lwgre_encap action structure.
	 *
	 * See struct rte_flow_action_lwgre_encap.
	 */
	RTE_FLOW_ACTION_TYPE_LWGRE_ENCAP,

	/**
	 * Decapsulate outer most LWGRE tunnel from matched flow.
	 *
	 * If flow pattern does not define a valid LWGRE tunnel (as specified by
	 * RFC7637) then the PMD should return a RTE_FLOW_ERROR_TYPE_ACTION
	 * error.
	 */
	RTE_FLOW_ACTION_TYPE_LWGRE_DECAP,
 ...
 */
struct rte_flow_action_selwrity {
	void *selwrity_session; /**< Pointer to security session structure. */
};
 ...
 * RTE_FLOW_ACTION_TYPE_LWGRE_ENCAP
 *
 * LWGRE tunnel end-point encapsulation data definition
 *
 * The tunnel definition is provided through the flow item pattern  the
 ...
 * ignored.
 *
 * Valid flow definition for RTE_FLOW_ACTION_TYPE_LWGRE_ENCAP include:
 *
 * - ETH / IPV4 / LWGRE / END
 * - ETH / VLAN / IPV6 / LWGRE / END
 *
 */
struct rte_flow_action_lwgre_encap {
	/**
	 * Encapsulating vxlan tunnel definition
	 * (terminated by the END pattern item).
	 */
 ...
 * device or unloading the PMD ilwalidates it.
 *
 ...
 *
 * This object type is used when colwerting a flow rule description.
 *
 * @see RTE_FLOW_COLW_OP_RULE
 * @see rte_flow_colw()
 */
RTE_STD_C11
struct rte_flow_colw_rule {
	union {
...
/**
 * Colwersion operations for flow API objects.
 *
 * @see rte_flow_colw()
 */
enum rte_flow_colw_op {
	/**
	 * No operation to perform.
	 *
	 * rte_flow_colw() simply returns 0.
	 */
	RTE_FLOW_COLW_OP_NONE,

	/**
	 * Colwert attributes structure.
	 *
	 ...
	 */
	RTE_FLOW_COLW_OP_ATTR,

	/**
	 * Colwert a single item.
	 *
	 ...
	 */
	RTE_FLOW_COLW_OP_ITEM,

	/**
	 * Colwert a single action.
	 *
	 ...
	 */
	RTE_FLOW_COLW_OP_ACTION,

	/**
	 * Colwert an entire pattern.
	 *
	 ...
	 *   @code struct rte_flow_item * @endcode
	 */
	RTE_FLOW_COLW_OP_PATTERN,

	/**
	 * Colwert a list of actions.
	 *
	 * Duplicates the entire list of actions at once with the same
	 * constraints as RTE_FLOW_COLW_OP_ACTION.
	 *
	 * - @p src type:
	 *   @code const struct rte_flow_action * @endcode
	 * - @p dst type:
	 *   @code struct rte_flow_action * @endcode
	 */
	RTE_FLOW_COLW_OP_ACTIONS,

	/**
	 * Colwert a complete flow rule description.
	 *
	 * Comprises attributes, pattern and actions together at once with
	 * the usual constraints.
	 *
	 * - @p src type:
	 *   @code const struct rte_flow_colw_rule * @endcode
	 * - @p dst type:
	 *   @code struct rte_flow_colw_rule * @endcode
	 */
	RTE_FLOW_COLW_OP_RULE,

	/**
	 * Colwert item type to its name string.
	 *
	 ...
	 *   @code char * @endcode
	 **/
	RTE_FLOW_COLW_OP_ITEM_NAME,

	/**
	 * Colwert action type to its name string.
	 *
	 * Writes a NUL-terminated string to @p dst. Like snprintf(), the
	 * returned value excludes the terminator which is always written
	 * nonetheless.
	 ...
	 *   @code char * @endcode
	 **/
	RTE_FLOW_COLW_OP_ACTION_NAME,

	/**
	 * Colwert item type to pointer to item name.
	 *
	 ...
	 */
	RTE_FLOW_COLW_OP_ITEM_NAME_PTR,

	/**
	 * Colwert action type to pointer to action name.
	 *
	 ...
	 *   @code const char ** @endcode
	 */
	RTE_FLOW_COLW_OP_ACTION_NAME_PTR,
};
 ...
 * such cases EILWAL should not be returned).
 *
 * @param port_id
 ...
 *
 *   -EILWAL: unknown or invalid rule specification.
 *
 ...
 * implemented as a wrapper to rte_flow_colw(). It is deprecated due to its
...
/**
 * Flow object colwersion helper.
 *
 * This function performs colwersion of various flow API objects to a
 * pre-allocated destination buffer. See enum rte_flow_colw_op for possible
 ...
 *   to store the colwersion of @p src to @p dst according to @p op
 ...
 *   than that.
 * - In case of colwersion error, a negative error code is returned and
 ...
 *
 * @return
 *   The number of bytes required to colwert @p src to @p dst on success, a
 *   negative errno value otherwise and rte_errno is set.
 *
 * @see rte_flow_colw_op
 */
__rte_experimental
int
rte_flow_colw(enum rte_flow_colw_op op,
	      void *dst,
	      size_t size,
	      const void *src,
	      struct rte_flow_error *error);
