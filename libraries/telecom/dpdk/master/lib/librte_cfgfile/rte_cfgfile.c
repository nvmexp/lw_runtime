		...
		const char *entryvalue)
{
	...
	struct rte_cfgfile_entry *lwrr_entry =
					&section->entries[section->num_entries];

	strlcpy(lwrr_entry->name, entryname, sizeof(lwrr_entry->name));
	strlcpy(lwrr_entry->value, entryvalue, sizeof(lwrr_entry->value));
	section->num_entries++;
		...
		const char *entryvalue)
{
	...
	struct rte_cfgfile_section *lwrr_section = _get_section(cfg,
								sectionname);
	if (lwrr_section == NULL)
		return -EILWAL;

	ret = _add_entry(lwrr_section, entryname, entryvalue);
		...
		const char *entryname, const char *entryvalue)
{
	...
	for (i = 0; i < lwrr_section->num_entries; i++)
		if (!strcmp(lwrr_section->entries[i].name, entryname)) {
			strlcpy(lwrr_section->entries[i].value, entryvalue,
				sizeof(lwrr_section->entries[i].value));
			return 0;
		}
