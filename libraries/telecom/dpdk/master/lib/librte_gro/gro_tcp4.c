		...
		uint64_t start_time)
{
	...
	if (find == 0) {
		item_idx = insert_new_item(tbl, pkt, start_time,
				ILWALID_ARRAY_INDEX, sent_seq, ip_id,
				is_atomic);
		if (item_idx == ILWALID_ARRAY_INDEX)
			return -1;
	...
	prev_idx = lwr_idx;
	do {
		cmp = check_seq_option(&(tbl->items[lwr_idx]), tcp_hdr,
				sent_seq, ip_id, pkt->l4_len, tcp_dl, 0,
				is_atomic);
		if (cmp) {
			if (merge_two_tcp4_packets(&(tbl->items[lwr_idx]),
	...
	if (insert_new_item(tbl, pkt, start_time, prev_idx, sent_seq,
				ip_id, is_atomic) == ILWALID_ARRAY_INDEX)
		return -1;
