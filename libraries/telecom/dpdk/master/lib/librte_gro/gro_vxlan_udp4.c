		...
		uint16_t max_item_per_flow)
{
	...
	for (i = 0; i < entries_num; i++)
		tbl->flows[i].start_index = ILWALID_ARRAY_INDEX;
...
find_an_empty_flow(struct gro_vxlan_udp4_tbl *tbl)
{
	...
	for (i = 0; i < max_flow_num; i++)
		if (tbl->flows[i].start_index == ILWALID_ARRAY_INDEX)
			return i;
	return ILWALID_ARRAY_INDEX;
}
		...
		uint8_t is_last_frag)
{
	...
	if (unlikely(item_idx == ILWALID_ARRAY_INDEX))
		return ILWALID_ARRAY_INDEX;
	...
	tbl->items[item_idx].inner_item.next_pkt_idx = ILWALID_ARRAY_INDEX;
	...
	if (prev_idx != ILWALID_ARRAY_INDEX) {
		...
		uint32_t prev_item_idx)
{
	...
	tbl->item_num--;
	if (prev_item_idx != ILWALID_ARRAY_INDEX)
		...
		uint32_t item_idx)
{
	...
	if (unlikely(flow_idx == ILWALID_ARRAY_INDEX))
		return ILWALID_ARRAY_INDEX;
...
{
	/* For VxLAN packet, outer udp src port is callwlated from
		...
		uint64_t start_time)
{
	...
	uint32_t lwr_idx, prev_idx, item_idx;
	...
	if (find == 0) {
		item_idx = insert_new_item(tbl, pkt, start_time,
				ILWALID_ARRAY_INDEX, frag_offset,
				is_last_frag);
		if (unlikely(item_idx == ILWALID_ARRAY_INDEX))
			return -1;
		if (insert_new_flow(tbl, &key, item_idx) ==
			 ...
			 */
			delete_item(tbl, item_idx, ILWALID_ARRAY_INDEX);
			return -1;
		}
		return 0;
	}
	...
	lwr_idx = tbl->flows[i].start_index;
	prev_idx = lwr_idx;
	do {
		cmp = udp4_check_vxlan_neighbor(&(tbl->items[lwr_idx]),
				frag_offset, ip_dl);
		if (cmp) {
		...
		if (frag_offset
			< tbl->items[lwr_idx].inner_item.frag_offset) {
			break;
		}

		prev_idx = lwr_idx;
		lwr_idx = tbl->items[lwr_idx].inner_item.next_pkt_idx;
	} while (lwr_idx != ILWALID_ARRAY_INDEX);

	/* Can't find neighbor. Insert the packet into the flow. */
	if (lwr_idx == tbl->flows[i].start_index) {
		...
		if (unlikely(item_idx == ILWALID_ARRAY_INDEX))
			return -1;
		tbl->items[item_idx].inner_item.next_pkt_idx = lwr_idx;
			   ...
			   uint32_t start_idx)
{
	...
	while (item_idx != ILWALID_ARRAY_INDEX) {
	...
	for (i = 0; i < max_flow_num; i++) {
		...
		while (j != ILWALID_ARRAY_INDEX) {
			if (tbl->items[j].inner_item.start_time <=
				 ...
				 * index.
				 */
				j = delete_item(tbl, j, ILWALID_ARRAY_INDEX);
				tbl->flows[i].start_index = j;
				if (j == ILWALID_ARRAY_INDEX)
					tbl->flow_num--;
