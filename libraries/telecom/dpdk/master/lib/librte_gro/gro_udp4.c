		...
		uint16_t max_item_per_flow)
{
	...
	/* ILWALID_ARRAY_INDEX indicates an empty flow */
		...
		uint8_t is_last_frag)
{
	...
	tbl->items[item_idx].next_pkt_idx = ILWALID_ARRAY_INDEX;
		...
		uint64_t start_time)
{
	...
	prev_idx = lwr_idx;
	do {
		cmp = udp4_check_neighbor(&(tbl->items[lwr_idx]),
				frag_offset, ip_dl, 0);
		if (cmp) {
			if (merge_two_udp4_packets(&(tbl->items[lwr_idx]),
						pkt, cmp, frag_offset,
						is_last_frag, 0))
				return 1;
			/*
		...
		if (frag_offset
			< tbl->items[lwr_idx].frag_offset) {
			break;
		}

		prev_idx = lwr_idx;
		lwr_idx = tbl->items[lwr_idx].next_pkt_idx;
	...
	if (lwr_idx == tbl->flows[i].start_index) {
		...
		if (unlikely(item_idx == ILWALID_ARRAY_INDEX))
			return -1;
		tbl->items[item_idx].next_pkt_idx = lwr_idx;
