		...
		uint64_t start_time)
{
	 ...
	 */
	if (unlikely(ILWALID_TCP_HDRLEN(pkt->l4_len)))
		return -1;
	...
	if (find == 0) {
		item_idx = insert_new_item(tbl, pkt, start_time,
				ILWALID_ARRAY_INDEX, sent_seq, outer_ip_id,
				ip_id, outer_is_atomic, is_atomic);
		if (item_idx == ILWALID_ARRAY_INDEX)
			return -1;
	...
	prev_idx = lwr_idx;
	do {
		cmp = check_vxlan_seq_option(&(tbl->items[lwr_idx]), tcp_hdr,
				sent_seq, outer_ip_id, ip_id, pkt->l4_len,
				tcp_dl, outer_is_atomic, is_atomic);
		if (cmp) {
			if (merge_two_vxlan_tcp4_packets(&(tbl->items[lwr_idx]),
						pkt, cmp, sent_seq,
						outer_ip_id, ip_id))
				return 1;
			/*
