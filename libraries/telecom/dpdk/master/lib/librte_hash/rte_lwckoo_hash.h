...
/* rte_lwckoo_hash.h
 * This file hold Lwckoo Hash private data structures to allows include from
 * platform specific files like rte_lwckoo_hash_x86.h
 */

#ifndef _RTE_LWCKOO_HASH_H_
#define _RTE_LWCKOO_HASH_H_
...
#define RTE_XABORT_LWCKOO_PATH_ILWALIDED 0x4
...
struct rte_hash_bucket {
	uint16_t sig_lwrrent[RTE_HASH_BUCKET_ENTRIES];
...
struct rte_hash {
	...
	/* RLW config */
	struct rte_hash_rlw_config *hash_rlw_cfg;
	 ...
	 */
	uint8_t readwrite_conlwr_support;
	/**< If read-write conlwrrency support is enabled */
	 ...
	 */
	uint8_t readwrite_conlwr_lf_support;
	/**< If read-write conlwrrency lock free support is enabled */
	uint8_t writer_takes_lock;
	/**< Indicates if the writer threads need to take lock */
	rte_hash_function hash_func;    /**< Function used to callwlate hash. */
	uint32_t hash_func_init_val;    /**< Init value used by hash_func. */
	rte_hash_cmp_eq_t rte_hash_lwstom_cmp_eq;
	 ...
	 * rte_hash_del_xxx APIs. When lock free read-write conlwrrency is
...
struct queue_node {
	...
	uint32_t lwr_bkt_idx;

	struct queue_node *prev;     /* Parent(bucket) in search path */
	int prev_slot;               /* Parent(slot) in search path */
};

/** @internal Default RLW defer queue entries to reclaim in one go. */
#define RTE_HASH_RLW_DQ_RECLAIM_MAX	16
