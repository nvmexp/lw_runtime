...
/** Initialising value used when callwlating hash. */
...
struct rte_fbk_hash_table {
	...
	uint32_t bucket_shift;		/**< Colwert bucket to table offset. */
	rte_fbk_hash_fn hash_func;	/**< The hash function. */
	uint32_t init_val;		/**< For initialising hash function. */
...
rte_fbk_hash_lookup_with_bucket(const struct rte_fbk_hash_table *ht,
				uint32_t key, uint32_t bucket)
{
	union rte_fbk_hash_entry lwrrent_entry;
	uint32_t i;

	for (i = 0; i < ht->entries_per_bucket; i++) {
		/* Single read of entry, which should be atomic. */
		lwrrent_entry.whole_entry = ht->t[bucket + i].whole_entry;
		if (! lwrrent_entry.entry.is_entry) {
			return -ENOENT; /* Error once we hit an empty field. */
		}
		if (lwrrent_entry.entry.key == key) {
			return lwrrent_entry.entry.value;
		}
	}
	return -ENOENT; /* Key didn't exist. */
}
 ...
 *    - EILWAL - invalid parameter value passed to function
