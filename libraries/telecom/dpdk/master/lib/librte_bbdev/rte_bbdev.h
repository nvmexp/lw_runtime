 ...
 *   - -EILWAL if num_queues is invalid, 0 or greater than maximum
 ...
 * @return
 *   - 0 on success
 *   - EILWAL if the identified queue size or priority are invalid
 ...
 * @return
 *   - 0 on success
 *   - EILWAL if invalid parameter pointer is provided
 */
__rte_experimental
int
...
struct rte_bbdev_info {
	...
	uint16_t num_queues;  /**< Number of queues lwrrently configured */
	bool started;  /**< Set if device is lwrrently started */
...
struct rte_bbdev_queue_info {
	...
	/** Set if queue is lwrrently started */
	bool started;
};
...
struct rte_bbdev_data {
	...
	uint16_t num_queues;  /**< Number of lwrrently configured queues */
...
struct __rte_cache_aligned rte_bbdev {
	...
	enum rte_bbdev_state state;  /**< If device is lwrrently used or not */
 ...
 * This functions only enqueues as many operations as lwrrently possible and
