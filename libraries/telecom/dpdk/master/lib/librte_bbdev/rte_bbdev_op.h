...
/* Maximum size of cirlwlar buffer */
#define RTE_BBDEV_TURBO_MAX_KW (18528)
/*
 * Turbo: Maximum number of Code Blocks in Transport Block. It is callwlated
...
enum rte_bbdev_op_ldpcdec_flag_bitmasks {
	...
	/** Set if a device includes LLR filler bits in the cirlwlar buffer
...
struct rte_bbdev_op_dec_turbo_cb_params {
	 ...
	 * pre-callwlated by the application or not.
	 */
	uint16_t k;
 ...
 *
 * The input encoded CB data is the Virtual Cirlwlar Buffer data stream, wk,
...
struct rte_bbdev_op_turbo_dec {
	/** The Virtual Cirlwlar Buffer, wk, size 3*Kpi for each CB */
 ...
 * belong to a TB "TB-mode" (Lwrrently not supported).
 *
 * The input encoded CB data is the Virtual Cirlwlar Buffer data stream.
 *
 * Each byte in the input cirlwlar buffer is the LLR value of each bit of the
 * original CB.
 *
...
struct rte_bbdev_op_ldpc_dec {
	/** The Virtual Cirlwlar Buffer for this code block, one LLR
	 ...
	 */
	uint16_t z_c;
	/** Ncb, length of the cirlwlar buffer in bits.
	 *  [3GPP TS38.212, section 5.4.2.1]
	 */
	uint16_t n_cb;
...
struct rte_bbdev_op_enc_turbo_tb_params {
	 ...
	 * pre-callwlated and appended by the application or not.
	 */
	uint16_t k_neg;
	/** The K+ size of the input CB, in bits [40:6144], that is in the
	 * Turbo operation when r >= C-, as in 3GPP TS 36.212.
 ...
 * length. Not all BBDEV PMDs are capable of CRC24A/B callwlation. Flags
 ...
 * op_flags parameter to indicate BBDEV to callwlate and append CRC24A to CB
...
/**
 * Colwerts queue operation type from enum to string
 *
 * @param op_type
 ...
 * @returns
 *   - 0 on success
 *   - EILWAL if invalid mempool is provided
 */
__rte_experimental
static inline int
