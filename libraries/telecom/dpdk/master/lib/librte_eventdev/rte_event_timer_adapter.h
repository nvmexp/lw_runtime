 ...
 * to get the timer adapter pointer from its id and use it to ilwoke fastpath
 ...
 *
 * This function must be ilwoked first before any other function in the API.
 *
 * @param conf
 ...
 *   - EILWAL: invalid event device identifier specified in config
 ...
 *   - -EILWAL: adapter identifier invalid
 *
 ...
 * timer cancel and so on) can be ilwoked.
 *
 * @param adapter
 *   A pointer to the event timer adapter structure.
 *
 ...
 *   - -EILWAL if adapter identifier invalid
 ...
 *
 * Before ilwoking this function, the application must wait for all the
 ...
 *   - -EILWAL: adapter id invalid, or adapter invalid
 */
int
 ...
 *   - EILWAL Invalid timer adapter, expiry event queue ID is invalid, or an
			  ...
			  uint16_t nb_evtims)
{
	...
	ADAPTER_VALID_OR_ERR_RET(adapter, -EILWAL);
	FUNC_PTR_OR_ERR_RET(adapter->arm_burst, -EILWAL);
#endif
...
#ifdef RTE_LIBRTE_EVENTDEV_DEBUG
	ADAPTER_VALID_OR_ERR_RET(adapter, -EILWAL);
	FUNC_PTR_OR_ERR_RET(adapter->arm_tmo_tick_burst, -EILWAL);
#endif
 ...
 *   are not consumed, and the caller has to take care of them, and rte_errno
 *   is set accordingly. Possible errno values include:
 *   - EILWAL Invalid timer adapter identifier
 *   - EAGAIN Specified timer adapter is not running
...
#ifdef RTE_LIBRTE_EVENTDEV_DEBUG
	ADAPTER_VALID_OR_ERR_RET(adapter, -EILWAL);
	FUNC_PTR_OR_ERR_RET(adapter->cancel_burst, -EILWAL);
#endif
