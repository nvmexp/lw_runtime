 ...
 * callback is ilwoked when creating a SW service for packet transfer from
...
/**
 *
 * Callback function ilwoked by the SW adapter before it continues
 * to process events. The callback is passed the size of the enqueue
 * buffer in the SW adapter and the oclwpancy of the buffer. The
