 ...
 * the dequeue function of a PMD cannot be ilwoked in parallel on two logical
 * cores to operates on same  event port. Of course, this function
 * can be ilwoked in parallel by different logical cores on different ports.
 ...
 * designated by its device identifier must be ilwoked in the following order:
 ...
 *     - rte_event_dev_start()
 *
 * Then, the application can ilwoke, in any order, the functions
 ...
 * again. The schedule, enqueue and dequeue functions should not be ilwoked
 * when the device is stopped.
 *
 * Finally, an application can close an Event device by ilwoking the
 * rte_event_dev_close() function.
 *
 * Each function of the application Event API ilwokes a specific function
 ...
 * ilwoked during the PCI/SoC device probing phase, as explained earlier.
 *
 ...
 * For hardware devices, scheduling oclwrs asynchronously without any software
 ...
 *   -(-EILWAL)  dev_id value is out of range.
 */
int
 ...
 *   - -EILWAL: Invalid device or  *attr_id* provided, or *attr_value* is NULL
 */
int
 ...
 * API. This function can also be re-ilwoked when a device is in the
 * stopped state.
 *
 ...
 *   - -EILWAL: invalid device, queue or attr_id provided, or attr_value was
 *		NULL
 ...
 * @return
 *   - 0: Successfully returned value
 *   - (-EILWAL) Invalid device, port or attr_id, or attr_value was NULL
 */
int
 ...
 * event dequeue and so on) can be ilwoked.
 *
 * @param dev_id
...
typedef void (*eventdev_stop_flush_t)(uint8_t dev_id, struct rte_event event,
		void *arg);
/**< Callback function called during rte_event_dev_stop(), ilwoked once per
 * flushed event.
 */

/**
 * Registers a callback function to be ilwoked during rte_event_dev_stop() for
 ...
 *   The identifier of the device.
 * @param callback
 *   Callback function ilwoked once per flushed event.
 * @param userdata
 *   Argument supplied to callback.
 *
 * @return
 *  - 0 on success.
 *  - -EILWAL if *dev_id* is invalid
 *
 ...
 * ports for conlwrrent processing while maintaining the original event order.
 ...
 * by ilwoking rte_event_enqueue_burst() with RTE_EVENT_OP_RELEASE operation.
 *
 ...
 * release the context earlier than that by ilwoking rte_event_enqueue_burst()
 ...
 * to multiple events ports for conlwrrent processing.
 ...
 *   - EILWAL   The port ID is invalid, device ID is invalid, an event's queue
...
/**
 * Colwerts nanoseconds to *timeout_ticks* value for rte_event_dequeue_burst()
 *
 * If the device is configured with RTE_EVENT_DEV_CFG_PER_DEQUEUE_TIMEOUT flag
 * then application can use this function to colwert timeout value in
 ...
 *  - -EILWAL if *dev_id* is invalid or *timeout_ticks* is NULL
 ...
 * releases, or ilwoking rte_event_enqueue_burst() with RTE_EVENT_OP_RELEASE
 ...
 * A negative return value indicates an error, -EILWAL indicates an invalid
 ...
 *        -EILWAL for invalid mode, queue port or id parameters
 ...
 *   - negative value: -EILWAL if stat not found, -ENOTSUP if not supported.
 */
uint64_t
 ...
 *   - negative value: -EILWAL invalid parameters, -ENOTSUP if not supported.
 */
int
