...
struct eth_device_info {
	 ...
	 * rx_adapter_stop callback needs to be ilwoked
	 */
	 ...
	 * be ilwoked if not already ilwoked
	 */
	uint16_t nb_rx_poll;
...
/* Callwlate change in number of vectors after Rx queue ID is add/deleted */
static int
...
/* Callwlate nb_rx_intr after deleting interrupt mode rx queues
 */
static void
...
/* Callwlate nb_rx_* after adding interrupt mode rx queues, newly added
 * interrupt queues could lwrrently be poll mode Rx queues
 */
static void
...
/* Callwlate size of the eth_rx_poll and wrr_sched arrays
...
/* Callwlate nb_rx_* after adding poll mode rx queues
 */
static void
...
/* Callwlate nb_rx_* after adding rx_queue_id */
static void
...
/* Callwlate nb_rx_* after deleting rx_queue_id */
static void
...
/* Precallwlate WRR polling sequence for all queues in rx_adapter */
static void
		...
		uint32_t *rx_wrr)
{
	...
	/* Initialize variables for callwlation of wrr schedule */
 ...
 * These events are picked up by rxa_intr_ring_dequeue() which is ilwoked from
 ...
 * eth device, in virtual device elwironments this back pressure is relayed to
	...
	if (wt == 0) {
		...
		if (rx_adapter->num_rx_intr > nb_rx_intr) {
			num_intr_vec = rxa_nb_intr_vect(dev_info,
						rx_queue_id, 0);
			/* interrupt based queues are being colwerted to
...
rxa_ctrl(uint8_t id, int start)
{
	...
	RTE_EVENT_ETH_RX_ADAPTER_ID_VALID_OR_ERR_RET(id, -EILWAL);
				...
				void *conf_arg)
{
	...
	RTE_EVENT_ETH_RX_ADAPTER_ID_VALID_OR_ERR_RET(id, -EILWAL);
	RTE_EVENTDEV_VALID_DEVID_OR_ERR_RET(dev_id, -EILWAL);
	if (conf_cb == NULL)
		return -EILWAL;
	...
	rx_adapter->eth_devices = rte_zmalloc_socket(rx_adapter->mem_name,
					...
					socket_id);
	rte_colwert_rss_key((const uint32_t *)default_rss_key,
		...
		const struct rte_event_eth_rx_adapter_queue_conf *queue_conf)
{
	...
	RTE_EVENT_ETH_RX_ADAPTER_ID_VALID_OR_ERR_RET(id, -EILWAL);
	RTE_ETH_VALID_PORTID_OR_ERR_RET(eth_dev_id, -EILWAL);
