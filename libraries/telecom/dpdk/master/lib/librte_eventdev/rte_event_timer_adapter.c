		...
		void *conf_arg)
{
	...
	if (!(adapter->data->caps &
		...
		FUNC_PTR_OR_NULL_RET_WITH_ERRNO(conf_cb, EILWAL);
...
rte_event_timer_adapter_start(const struct rte_event_timer_adapter *adapter)
{
	...
	ADAPTER_VALID_OR_ERR_RET(adapter, -EILWAL);
	FUNC_PTR_OR_ERR_RET(adapter->ops->start, -EILWAL);
...
rte_event_timer_adapter_stop(const struct rte_event_timer_adapter *adapter)
{
	...
	ADAPTER_VALID_OR_ERR_RET(adapter, -EILWAL);
	FUNC_PTR_OR_ERR_RET(adapter->ops->stop, -EILWAL);
...
rte_event_timer_adapter_free(struct rte_event_timer_adapter *adapter)
{
	...
	ADAPTER_VALID_OR_ERR_RET(adapter, -EILWAL);
	FUNC_PTR_OR_ERR_RET(adapter->ops->uninit, -EILWAL);
...
{
	ADAPTER_VALID_OR_ERR_RET(adapter, -EILWAL);
	FUNC_PTR_OR_ERR_RET(adapter->ops->stats_get, -EILWAL);
	if (stats == NULL)
		return -EILWAL;
...
{
	ADAPTER_VALID_OR_ERR_RET(adapter, -EILWAL);
	FUNC_PTR_OR_ERR_RET(adapter->ops->stats_reset, -EILWAL);
...
event_buffer_add(struct event_buffer *bufp, struct rte_event *eventp)
{
	...
	/* Wrap automatically when overflow oclwrs. */
	bufp->head++;
		   ...
		   uint16_t *nb_events_ilw)
{
	...
	*nb_events_flushed = rte_event_enqueue_burst(dev_id, port_id,
	...
	bufp->tail = bufp->tail + *nb_events_flushed + *nb_events_ilw;
}
 ...
 */
struct swtim {
	/* Identifier of service exelwting timer management logic. */
	uint32_t service_id;
...
swtim_callback(struct rte_timer *tim)
{
	...
	if (event_buffer_batch_ready(&sw->buffer)) {
		...
		sw->stats.ev_ilw_count += nb_evs_ilwalid;
	}
}
...
/* This function returns true if one or more (adapter) ticks have oclwrred since
