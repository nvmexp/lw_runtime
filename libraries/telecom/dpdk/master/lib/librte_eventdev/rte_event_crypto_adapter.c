				...
				void *conf_arg)
{
	...
	EVENT_CRYPTO_ADAPTER_ID_VALID_OR_ERR_RET(id, -EILWAL);
...
{
	struct rte_event_crypto_adapter_stats *stats = &adapter->crypto_stats;
	struct crypto_device_info *lwrr_dev;
	struct crypto_queue_pair_info *lwrr_queue;
	struct rte_crypto_op **op_buffer;
	struct rte_cryptodev *dev;
	uint8_t cdev_id;
	uint16_t qp;
	...
	for (cdev_id = 0; cdev_id < num_cdev; cdev_id++) {
		lwrr_dev = &adapter->cdevs[cdev_id];
		dev = lwrr_dev->dev;
		if (dev == NULL)
			continue;
		for (qp = 0; qp < dev->data->nb_queue_pairs; qp++) {

			lwrr_queue = &lwrr_dev->qpairs[qp];
			if (!lwrr_queue->qp_enabled)
				continue;

			op_buffer = lwrr_queue->op_buffer;
			ret = rte_cryptodev_enqueue_burst(cdev_id,
							  qp,
							  op_buffer,
							  lwrr_queue->len);
			...
			unsigned int max_deq)
{
	...
	do {
		...
		for (cdev_id = adapter->next_cdev_id;
			...
			for (qp = lwrr_dev->next_queue_pair_id;
				...
				if (nb_deq > max_deq) {
					...
					lwrr_dev->next_queue_pair_id = (qp + 1)
