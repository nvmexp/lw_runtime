...
#define TXA_ILWALID_DEV_ID	INT32_C(-1)
#define TXA_ILWALID_SERVICE_ID	INT64_C(-1)
...
do {\
	int ret; \
	RTE_EVENT_ETH_TX_ADAPTER_ID_VALID_OR_ERR_RET((id), -EILWAL); \
	ret = txa_init(); \
	if (ret != 0) \
		return ret; \
...
{
	return txa_dev_id_array[id] != TXA_ILWALID_DEV_ID;
}
...
txa_dev_id_array_init(void)
{
	if (txa_dev_id_array == NULL) {
		...
		for (i = 0; i < RTE_EVENT_ETH_TX_ADAPTER_MAX_INSTANCE; i++)
			txa_dev_id_array[i] = TXA_ILWALID_DEV_ID;
	}
...
{
	if (txa->service_id != TXA_ILWALID_SERVICE_ID) {
	...
	txa->service_id = TXA_ILWALID_SERVICE_ID;
}
				...
				struct rte_event_port_conf *port_conf)
{
	...
	if (ret != 0) {
		txa_dev_id_array[id] = TXA_ILWALID_DEV_ID;
		return ret;
	}
