...
struct rte_latency_stats {
	...
	rte_spinlock_t lock; /** Latency callwlation lock */
};
		...
		void *_ __rte_unused)
{
	...
	for (i = 0; i < cnt; i++) {
		/*
		 * The jitter is callwlated as statistical mean of interpacket
		 ...
		 * D(i-1,i) is difference in latency of two conselwtive packets
		 * i-1 and i.
		 * Reference: Callwlated as per RFC 5481, sec 4.1,
