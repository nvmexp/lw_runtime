...
enum power_management_elw global_default_elw = PM_ELW_NOT_SET;

static rte_spinlock_t global_elw_cfg_lock = RTE_SPINLOCK_INITIALIZER;
...
int
rte_power_check_elw_supported(enum power_management_elw elw)
{
	switch (elw) {
	case PM_ELW_ACPI_CPUFREQ:
		return power_acpi_cpufreq_check_supported();
	case PM_ELW_PSTATE_CPUFREQ:
...
int
rte_power_set_elw(enum power_management_elw elw)
{
	rte_spinlock_lock(&global_elw_cfg_lock);

	if (global_default_elw != PM_ELW_NOT_SET) {
		RTE_LOG(ERR, POWER, "Power Management Environment already set.\n");
		rte_spinlock_unlock(&global_elw_cfg_lock);
		return -1;
	}
	...
	} else if (elw == PM_ELW_PSTATE_CPUFREQ) {
	...
	if (ret == 0)
		global_default_elw = elw;
	else {
		global_default_elw = PM_ELW_NOT_SET;
...
enum power_management_elw
rte_power_get_elw(void) {
	return global_default_elw;
}
...
rte_power_init(unsigned int lcore_id)
{
	...
	default:
		RTE_LOG(INFO, POWER, "Elw isn't set yet!\n");
	}
	...
	if (ret == 0) {
		rte_power_set_elw(PM_ELW_ACPI_CPUFREQ);
		goto out;
	}
	...
	if (ret == 0) {
		rte_power_set_elw(PM_ELW_PSTATE_CPUFREQ);
		goto out;
	}
	...
	if (ret == 0) {
		rte_power_set_elw(PM_ELW_KVM_VM);
		goto out;
	}
