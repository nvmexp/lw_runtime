...
enum power_management_elw {PM_ELW_NOT_SET, PM_ELW_ACPI_CPUFREQ, PM_ELW_KVM_VM,
 ...
 */
__rte_experimental
int rte_power_check_elw_supported(enum power_management_elw elw);

/**
 * Set the default power management implementation. If this is not called prior
 * to rte_power_init(), then auto-detect of the environment will take place.
 * It is thread safe. New elw can be set only in uninitialized state
 * (thus rte_power_unset_elw must be called if different elw was already set).
 *
 * @param elw
 *  elw. The environment in which to initialise Power Management for.
 *
 * @return
 *  - 0 on success.
 *  - Negative on error.
 */
int rte_power_set_elw(enum power_management_elw elw);

/**
 * Unset the global environment configuration.
 * This can only be called after all threads have completed.
 */
void rte_power_unset_elw(void);

/**
 * Get the default power management implementation.
 *
 * @return
 *  power_management_elw The configured environment.
 */
enum power_management_elw rte_power_get_elw(void);

/**
 * Initialize power management for a specific lcore. If rte_power_set_elw() has
 ...
 * Function pointer definition. Review each elwironments
 ...
 * each elwironments specific documentation for usage.
 *
 * @param lcore_id
 *  lcore id.
 *
 ...
 * frequencies.
 * Review each elwironments specific documentation for usage.
 *
 * @param lcore_id
 *  lcore id.
 *
