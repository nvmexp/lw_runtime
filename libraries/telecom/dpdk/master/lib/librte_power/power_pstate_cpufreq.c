...
#define POWER_COLWERT_TO_DECIMAL 10
...
#define POWER_SYSFILE_LWR_FREQ  \
		"/sys/devices/system/cpu/cpu%u/cpufreq/scaling_lwr_freq"
...
struct pstate_power_info {
	...
	FILE *f_lwr_min;                     /**< FD of scaling_min */
	FILE *f_lwr_max;                     /**< FD of scaling_max */
	char governor_ori[32];               /**< Original governor name */
	uint32_t lwrr_idx;                   /**< Freq index in freqs array */
...
power_init_for_setting_freq(struct pstate_power_info *pi)
{
	...
	if (f_base == NULL) {
		...
		base_ratio = strtoul(buf_base, NULL, POWER_COLWERT_TO_DECIMAL)
				/ BUS_FREQ;
	}
...
set_freq_internal(struct pstate_power_info *pi, uint32_t idx)
{
	 ...
	 */
	if (fseek(pi->f_lwr_min, 0, SEEK_SET) < 0) {
	...
	if (fseek(pi->f_lwr_max, 0, SEEK_SET) < 0) {
		RTE_LOG(ERR, POWER, "Fail to set file position indicator to 0 "
				"for setting frequency for lcore %u\n",
				pi->lcore_id);
		return -1;
	...
	if (idx  >  pi->lwrr_idx) {
		...
		if (fprintf(pi->f_lwr_min, "%u", target_freq) < 0) {
			RTE_LOG(ERR, POWER, "Fail to write new frequency for "
					"lcore %u\n", pi->lcore_id);
			return -1;
		}

		if (fprintf(pi->f_lwr_max, "%u", target_freq) < 0) {
			RTE_LOG(ERR, POWER, "Fail to write new frequency for "
					"lcore %u\n", pi->lcore_id);
			return -1;
		}
...
power_get_available_freqs(struct pstate_power_info *pi)
{
	...
	sys_min_freq = strtoul(buf_min, &p_min, POWER_COLWERT_TO_DECIMAL);
	sys_max_freq = strtoul(buf_max, &p_max, POWER_COLWERT_TO_DECIMAL);
...
static int
power_get_lwr_idx(struct pstate_power_info *pi)
{
	FILE *f_lwr;
	int ret = -1;
	char *p_lwr;
	char buf_lwr[BUFSIZ];
	char fullpath_lwr[PATH_MAX];
	char *s_lwr;
	uint32_t sys_lwr_freq = 0;
	unsigned int i;

	snprintf(fullpath_lwr, sizeof(fullpath_lwr),
			POWER_SYSFILE_LWR_FREQ,
			pi->lcore_id);
	f_lwr = fopen(fullpath_lwr, "r");
	FOPEN_OR_ERR_RET(f_lwr, ret);

	/* initialize the lwr_idx to matching current frequency freq index */
	s_lwr = fgets(buf_lwr, sizeof(buf_lwr), f_lwr);
	FOPS_OR_NULL_GOTO(s_lwr, fail);

	p_lwr = strchr(buf_lwr, '\n');
	if (p_lwr != NULL)
		*p_lwr = 0;
	sys_lwr_freq = strtoul(buf_lwr, &p_lwr, POWER_COLWERT_TO_DECIMAL);

	/* colwert the frequency to nearest 100000 value
	 * Ex: if sys_lwr_freq=1396789 then freq_colw=1400000
	 * Ex: if sys_lwr_freq=800030 then freq_colw=800000
	 * Ex: if sys_lwr_freq=800030 then freq_colw=800000
	 */
	unsigned int freq_colw = 0;
	freq_colw = (sys_lwr_freq + FREQ_ROUNDING_DELTA)
				/ ROUND_FREQ_TO_N_100000;
	freq_colw = freq_colw * ROUND_FREQ_TO_N_100000;

	for (i = 0; i < pi->nb_freqs; i++) {
		if (freq_colw == pi->freqs[i]) {
			pi->lwrr_idx = i;
			break;
		}
	}
...
power_pstate_cpufreq_get_freq(unsigned int lcore_id)
{
	if (lcore_id >= RTE_MAX_LCORE) {
		RTE_LOG(ERR, POWER, "Invalid lcore ID\n");
		return RTE_POWER_ILWALID_FREQ_INDEX;
	}

	return lcore_power_info[lcore_id].lwrr_idx;
}
...
power_pstate_cpufreq_freq_up(unsigned int lcore_id)
{
	...
	pi = &lcore_power_info[lcore_id];
	if (pi->lwrr_idx == 0 ||
	    (pi->lwrr_idx == 1 && pi->turbo_available && !pi->turbo_enable))
		return 0;

	/* Frequencies in the array are from high to low. */
	return set_freq_internal(pi, pi->lwrr_idx - 1);
}
...
power_pstate_disable_turbo(unsigned int lcore_id)
{
	...
	if (pi->turbo_available && pi->lwrr_idx <= 1) {
