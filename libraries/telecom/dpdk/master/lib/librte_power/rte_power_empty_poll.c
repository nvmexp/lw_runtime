		...
		uint32_t max_train_iter)
{
	...
	if (poll_stats->lwr_freq == freq &&
			poll_stats->thresh[freq].trained == false) {
		if (poll_stats->thresh[freq].lwr_train_iter == 0) {
			...
			poll_stats->thresh[freq].lwr_train_iter++;

			return;
		} else if (poll_stats->thresh[freq].lwr_train_iter
				<= max_train_iter) {
...
update_stats(struct priority_worker *poll_stats)
{
	...
	uint64_t lwr_edpi = s->empty_dequeues - s->empty_dequeues_prev;
	...
	if (s->thresh[s->lwr_freq].base_edpi < lwr_edpi) {

		/* edpi mean empty poll counter difference per interval */
		RTE_LOG(DEBUG, POWER, "lwr_edpi is too large "
				"lwr edpi %"PRId64" "
				"base edpi %"PRId64"\n",
				lwr_edpi,
				s->thresh[s->lwr_freq].base_edpi);
		/* Value to make us fail need debug log*/
		return 1000UL;
	}

	s->edpi_av[s->ec++ % BINS_AV] = lwr_edpi;
	...
	percent = 100 - (uint32_t)(((float)tot_edpi /
			(float)s->thresh[s->lwr_freq].base_edpi) * 100);
...
update_stats_normal(struct priority_worker *poll_stats)
{
	...
	if (poll_stats->thresh[poll_stats->lwr_freq].base_edpi == 0) {

		enum freq_val lwr_freq = poll_stats->lwr_freq;

		/* edpi mean empty poll counter difference per interval */
		RTE_LOG(DEBUG, POWER, "lwre freq is %d, edpi is %"PRIu64"\n",
				lwr_freq,
				poll_stats->thresh[lwr_freq].base_edpi);
		return;
	}
	...
	if (poll_stats->lwr_freq == LOW)
		RTE_LOG(INFO, POWER, "Purge Mode is not lwrrently supported\n");
	else if (poll_stats->lwr_freq == MED) {
	...
	} else if (poll_stats->lwr_freq == HGH) {
