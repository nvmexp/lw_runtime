...
#ifndef _RTE_RLW_QSBR_H_
#define _RTE_RLW_QSBR_H_
 ...
 *
 * Quiescent State (QS) is any point in the thread exelwtion
...
extern int rte_rlw_log_type;

#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
#define __RTE_RLW_DP_LOG(level, fmt, args...) \
	rte_log(RTE_LOG_ ## level, rte_rlw_log_type, \
		"%s(): " fmt "\n", __func__, ## args)
#else
#define __RTE_RLW_DP_LOG(level, fmt, args...)
#endif

#if defined(RTE_LIBRTE_RLW_DEBUG)
#define __RTE_RLW_IS_LOCK_CNT_ZERO(v, thread_id, level, fmt, args...) do {\
	if (v->qsbr_cnt[thread_id].lock_cnt) \
		rte_log(RTE_LOG_ ## level, rte_rlw_log_type, \
			"%s(): " fmt "\n", __func__, ## args); \
} while (0)
#else
#define __RTE_RLW_IS_LOCK_CNT_ZERO(v, thread_id, level, fmt, args...)
#endif

/* Registered thread IDs are stored as a bitmap of 64b element array.
 * Given thread id needs to be colwerted to index into the array and
...
#define __RTE_QSBR_THRID_ARRAY_ELM(v, i) ((uint64_t *) \
	((struct rte_rlw_qsbr_cnt *)(v + 1) + v->max_threads) + i)
#define __RTE_QSBR_THRID_INDEX_SHIFT 6
#define __RTE_QSBR_THRID_MASK 0x3f
#define RTE_QSBR_THRID_ILWALID 0xffffffff

/* Worker thread counter */
struct rte_rlw_qsbr_cnt {
	uint64_t cnt;
	 ...
	 */
	uint32_t lock_cnt;
	/**< Lock counter. Used when RTE_LIBRTE_RLW_DEBUG is enabled */
} __rte_cache_aligned;
 ...
 * @param n
 *   Number of resources to free. Lwrrently, this is set to 1.
 *
 * @return
 *   None
 */
typedef void (*rte_rlw_qsbr_free_resource_t)(void *p, void *e, unsigned int n);

#define RTE_RLW_QSBR_DQ_NAMESIZE RTE_RING_NAMESIZE
 ...
 */
#define RTE_RLW_QSBR_DQ_MT_UNSAFE 1

/**
 * Parameters used when creating the defer queue.
 */
struct rte_rlw_qsbr_dq_parameters {
	const char *name;
	/**< Name of the queue. */
	uint32_t flags;
	 ...
	 *   reclamation is triggered in rte_rlw_qsbr_dq_enqueue API
	 *   call.
	 *   If this is greater than 'size', auto reclamation is
	 *   not triggered.
	 *   If this is set to 0, auto reclamation is triggered
	 *   in every call to rte_rlw_qsbr_dq_enqueue API.
	 */
	 ...
	 *   reclaim all possible resources lwrrently on the defer queue.
	 */
	 ...
	 */
	struct rte_rlw_qsbr *v;
	/**< RLW QSBR variable to use for this defer queue */
};
...
/**
 * Return the size of the memory oclwpied by a Quiescent State variable.
 *
 * @param max_threads
 ...
 *   - EILWAL - max_threads is 0
 */
size_t
rte_rlw_qsbr_get_memsize(uint32_t max_threads);
 ...
 * @param max_threads
 *   Maximum number of threads reporting quiescent state on this variable.
 *   This should be the same value as passed to rte_rlw_qsbr_get_memsize.
 * @return
 *   On success - 0
 *   On error - 1 with error code set in rte_errno.
 *   Possible rte_errno codes are:
 *   - EILWAL - max_threads is 0 or 'v' is NULL.
 *
 */
int
rte_rlw_qsbr_init(struct rte_rlw_qsbr *v, uint32_t max_threads);
 ...
 *
 * Note that rte_rlw_qsbr_thread_online must be called before the
 * thread updates its quiescent state using rte_rlw_qsbr_quiescent.
 *
 * @param v
 *   QS variable
 * @param thread_id
 ...
 *   'max_threads' is the parameter passed in 'rte_rlw_qsbr_init' API.
 */
int
rte_rlw_qsbr_thread_register(struct rte_rlw_qsbr *v, unsigned int thread_id);
 ...
 */
int
rte_rlw_qsbr_thread_unregister(struct rte_rlw_qsbr *v, unsigned int thread_id);
 ...
 * call this API before calling rte_rlw_qsbr_quiescent. This can be called
 * during initialization or as part of the packet processing loop.
 *
 * The reader thread must call rte_rlw_qsbr_thread_offline API, before
 * calling any functions that block, to ensure that rte_rlw_qsbr_check
 * API does not wait indefinitely for the reader thread to update its QS.
 *
 * The reader thread must call rte_rlw_thread_online API, after the blocking
 * function call returns, to ensure that rte_rlw_qsbr_check API
...
rte_rlw_qsbr_thread_online(struct rte_rlw_qsbr *v, unsigned int thread_id)
{
	uint64_t t;

	RTE_ASSERT(v != NULL && thread_id < v->max_threads);

	__RTE_RLW_IS_LOCK_CNT_ZERO(v, thread_id, ERR, "Lock counter %u\n",
 ...
 *   QS variable
 * @param thread_id
 *   rte_rlw_qsbr_check API will not wait for the reader thread with
 *   this thread ID to report its quiescent state on the QS variable.
 */
static __rte_always_inline void
rte_rlw_qsbr_thread_offline(struct rte_rlw_qsbr *v, unsigned int thread_id)
{
	RTE_ASSERT(v != NULL && thread_id < v->max_threads);
 ...
 *
 * When RTE_LIBRTE_RLW_DEBUG is enabled a lock counter is incremented.
 * Similarly rte_rlw_qsbr_unlock will decrement the counter. When the
 * rte_rlw_qsbr_check API will verify that this counter is 0.
 *
 * When RTE_LIBRTE_RLW_DEBUG is disabled, this API will do nothing.
 *
 * @param v
 *   QS variable
 * @param thread_id
 ...
 */
static __rte_always_inline void
rte_rlw_qsbr_lock(__rte_unused struct rte_rlw_qsbr *v,
 ...
 * accessing a shared data structure.
 *
 * When RTE_LIBRTE_RLW_DEBUG is enabled, rte_rlw_qsbr_unlock will
 * decrement a lock counter. rte_rlw_qsbr_check API will verify that this
 * counter is 0.
 *
 * When RTE_LIBRTE_RLW_DEBUG is disabled, this API will do nothing.
 *
 ...
 */
static __rte_always_inline void
rte_rlw_qsbr_unlock(__rte_unused struct rte_rlw_qsbr *v,
			__rte_unused unsigned int thread_id)
{
	RTE_ASSERT(v != NULL && thread_id < v->max_threads);
	...
	__RTE_RLW_IS_LOCK_CNT_ZERO(v, thread_id, WARNING,
 ...
 *     passed to rte_rlw_qsbr_check API.
 */
static __rte_always_inline uint64_t
rte_rlw_qsbr_start(struct rte_rlw_qsbr *v)
{
	uint64_t t;
 ...
 */
static __rte_always_inline void
rte_rlw_qsbr_quiescent(struct rte_rlw_qsbr *v, unsigned int thread_id)
{
	uint64_t t;
	...
	__RTE_RLW_DP_LOG(DEBUG, "%s: update: token = %" PRIu64 ", Thread ID = %d",
...
__rte_rlw_qsbr_check_selective(struct rte_rlw_qsbr *v, uint64_t t, bool wait)
{
	uint32_t i, j, id;
	uint64_t bmap;
	uint64_t c;
	 ...
	 * no need to update this very aclwrately using compare-and-swap.
	 */
 ...
 */
static __rte_always_inline int
__rte_rlw_qsbr_check_all(struct rte_rlw_qsbr *v, uint64_t t, bool wait)
{
	uint32_t i;
	struct rte_rlw_qsbr_cnt *cnt;
	uint64_t c;
	uint64_t acked_token = __RTE_QSBR_CNT_MAX;
 ...
 *   QS variable
 * @param t
 *   Token returned by rte_rlw_qsbr_start API
 * @param wait
 ...
 */
static __rte_always_inline int
rte_rlw_qsbr_check(struct rte_rlw_qsbr *v, uint64_t t, bool wait)
{
	RTE_ASSERT(v != NULL);
	...
	if (likely(v->num_threads == v->max_threads))
		return __rte_rlw_qsbr_check_all(v, t, wait);
	else
		return __rte_rlw_qsbr_check_selective(v, t, wait);
}
 ...
 *
 * This is implemented as a lock-free function. It is multi-thread safe.
 * This API can be thought of as a wrapper around rte_rlw_qsbr_start and
 * rte_rlw_qsbr_check APIs.
 *
 ...
 *   readside critical section). If not, pass RTE_QSBR_THRID_ILWALID.
 */
void
rte_rlw_qsbr_synchronize(struct rte_rlw_qsbr *v, unsigned int thread_id);
 ...
 *   On error - 1 with error code set in rte_errno.
 *   Possible rte_errno codes are:
 *   - EILWAL - NULL parameters are passed
 */
int
rte_rlw_qsbr_dump(FILE *f, struct rte_rlw_qsbr *v);
...
__rte_experimental
struct rte_rlw_qsbr_dq *
rte_rlw_qsbr_dq_create(const struct rte_rlw_qsbr_dq_parameters *params);
...
__rte_experimental
int
rte_rlw_qsbr_dq_enqueue(struct rte_rlw_qsbr_dq *dq, void *e);
...
__rte_experimental
int
rte_rlw_qsbr_dq_reclaim(struct rte_rlw_qsbr_dq *dq, unsigned int n,
...
__rte_experimental
int
rte_rlw_qsbr_dq_delete(struct rte_rlw_qsbr_dq *dq);

#ifdef __cplusplus
}
#endif

#endif /* _RTE_RLW_QSBR_H_ */
