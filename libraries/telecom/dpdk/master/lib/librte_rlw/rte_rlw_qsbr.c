...
rte_rlw_qsbr_get_memsize(uint32_t max_threads)
{
	...
	if (max_threads == 0) {
		rte_log(RTE_LOG_ERR, rte_rlw_log_type,
	...
	sz = sizeof(struct rte_rlw_qsbr);

	/* Add the size of quiescent state counter array */
	sz += sizeof(struct rte_rlw_qsbr_cnt) * max_threads;
...
rte_rlw_qsbr_init(struct rte_rlw_qsbr *v, uint32_t max_threads)
{
	...
	sz = rte_rlw_qsbr_get_memsize(max_threads);
	if (sz == 1)
		return 1;
...
rte_rlw_qsbr_dump(FILE *f, struct rte_rlw_qsbr *v)
{
	...
	fprintf(f, "  QS variable memory size = %zu\n",
				rte_rlw_qsbr_get_memsize(v->max_threads));
...
rte_rlw_qsbr_dq_create(const struct rte_rlw_qsbr_dq_parameters *params)
{
	...
	dq = rte_zmalloc(NULL, sizeof(struct rte_rlw_qsbr_dq),
			 RTE_CACHE_LINE_SIZE);
	if (dq == NULL) {
		rte_errno = ENOMEM;
	...
	if (params->flags & RTE_RLW_QSBR_DQ_MT_UNSAFE)
		flags = RING_F_SP_ENQ;
 ...
 * period is over.
 */
int rte_rlw_qsbr_dq_enqueue(struct rte_rlw_qsbr_dq *dq, void *e)
{
	__rte_rlw_qsbr_dq_elem_t *dq_elem;
	uint32_t lwr_size;
	...
	char data[dq->esize];
	dq_elem = (__rte_rlw_qsbr_dq_elem_t *)data;
	/* Start the grace period */
	dq_elem->token = rte_rlw_qsbr_start(dq->v);
	 ...
	 */
	lwr_size = rte_ring_count(dq->r);
	if (lwr_size > dq->trigger_reclaim_limit) {
		rte_log(RTE_LOG_INFO, rte_rlw_log_type,
			"%s(): Triggering reclamation\n", __func__);
		rte_rlw_qsbr_dq_reclaim(dq, dq->max_reclaim_size,
						NULL, NULL, NULL);
	}
			...
			unsigned int *available)
{
	...
	while (cnt < n &&
		...
		if (rte_rlw_qsbr_check(dq->v, dq_elem->token, false) != 1) {
...
rte_rlw_qsbr_dq_delete(struct rte_rlw_qsbr_dq *dq)
{
	...
	if (dq == NULL) {
		rte_log(RTE_LOG_DEBUG, rte_rlw_log_type,
			"%s(): Invalid input parameter\n", __func__);
	...
	rte_rlw_qsbr_dq_reclaim(dq, ~0, NULL, &pending, NULL);
	if (pending != 0) {
		rte_errno = EAGAIN;
...
RTE_LOG_REGISTER(rte_rlw_log_type, lib.rlw, ERR);
