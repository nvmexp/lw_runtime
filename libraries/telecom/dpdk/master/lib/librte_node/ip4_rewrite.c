			 ...
			 void **objs, uint16_t nb_objs)
{
	...
	/* Get stream for the spelwlated next node */
	...
	while (n_left_from >= 4) {
		...
		if (unlikely(fix_spec == 0)) {
			/* Copy things successfully spelwlated till now */
			...
			/* Change spelwlation if last two are same */
			if ((next_index != next3) && (next2 == next3)) {
				/* Put the current spelwlated node */
				rte_node_next_stream_put(graph, node,
							 next_index, held);
				held = 0;

				/* Get next spelwlated stream */
				next_index = next3;
