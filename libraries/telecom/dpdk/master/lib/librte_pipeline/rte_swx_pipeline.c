				      ...
				      uint32_t n_fields)
{
	...
	CHECK(p, EILWAL);
	CHECK_NAME(name, EILWAL);
	CHECK(fields, EILWAL);
	CHECK(n_fields, EILWAL);

	for (i = 0; i < n_fields; i++) {
		struct rte_swx_field_params *f = &fields[i];
		uint32_t j;

		CHECK_NAME(f->name, EILWAL);
		CHECK(f->n_bits, EILWAL);
		CHECK(f->n_bits <= 64, EILWAL);
		CHECK((f->n_bits & 7) == 0, EILWAL);

		for (j = 0; j < i; j++) {
			struct rte_swx_field_params *f_prev = &fields[j];

			CHECK(strcmp(f->name, f_prev->name), EILWAL);
		}
	}
				       ...
				       struct rte_swx_port_in_ops *ops)
{
	...
	CHECK_NAME(name, EILWAL);
	CHECK(ops, EILWAL);
	CHECK(ops->create, EILWAL);
	CHECK(ops->free, EILWAL);
	CHECK(ops->pkt_rx, EILWAL);
	CHECK(ops->stats_read, EILWAL);
				...
				void *args)
{
	...
	CHECK(!port_in_find(p, port_id), EILWAL);

	CHECK_NAME(port_type_name, EILWAL);
...
port_in_build(struct rte_swx_pipeline *p)
{
	...
	CHECK(p->n_ports_in, EILWAL);
	CHECK(rte_is_power_of_2(p->n_ports_in), EILWAL);

	for (i = 0; i < p->n_ports_in; i++)
		CHECK(port_in_find(p, i), EILWAL);
				 ...
				 void *args)
{
	...
	CHECK(!port_out_find(p, port_id), EILWAL);
...
port_out_build(struct rte_swx_pipeline *p)
{
	...
	CHECK(p->n_ports_out, EILWAL);

	for (i = 0; i < p->n_ports_out; i++)
		CHECK(port_out_find(p, i), EILWAL);
	...
	rte_swx_extern_type_destructor_t destructor)
{
	...
	CHECK_NAME(mailbox_struct_type_name, EILWAL);
	mailbox_struct_type = struct_type_find(p, mailbox_struct_type_name);
	CHECK(mailbox_struct_type, EILWAL);

	CHECK(constructor, EILWAL);
	CHECK(destructor, EILWAL);
	...
	rte_swx_extern_type_member_func_t member_func)
{
	...
	CHECK_NAME(extern_type_name, EILWAL);
					...
					const char *struct_type_name)
{
	...
	CHECK(p, EILWAL);
	CHECK_NAME(name, EILWAL);
	CHECK_NAME(struct_type_name, EILWAL);
 ...
 */
static int
instr_hdr_ilwalidate_translate(struct rte_swx_pipeline *p,
			       struct action *action __rte_unused,
			       char **tokens,
			       int n_tokens,
			       struct instruction *instr,
			       struct instruction_data *data __rte_unused)
{
	...
	instr->type = INSTR_HDR_ILWALIDATE;
	instr->valid.header_id = h->id;
	return 0;
}

static inline void
instr_hdr_ilwalidate_exec(struct rte_swx_pipeline *p)
{
	struct thread *t = &p->threads[p->thread_id];
	struct instruction *ip = t->ip;
	uint32_t header_id = ip->valid.header_id;

	TRACE("[Thread %2u] ilwalidate header %u\n", p->thread_id, header_id);
			  ...
			  struct instruction_data *data __rte_unused)
{
	...
	CHECK(fdst && (fdst->n_bits == 16), EILWAL);
...
static int
instr_jmp_ilwalid_translate(struct rte_swx_pipeline *p,
			    struct action *action __rte_unused,
			    char **tokens,
			    int n_tokens,
			    struct instruction *instr,
...
static inline void
instr_jmp_ilwalid_exec(struct rte_swx_pipeline *p)
{
	struct thread *t = &p->threads[p->thread_id];
	struct instruction *ip = t->ip;
	uint32_t header_id = ip->jmp.header_id;

	TRACE("[Thread %2u] jmplw\n", p->thread_id);
		...
		struct instruction_data *data)
{
	...
	for ( ; ; ) {
		...
		CHECK(n_tokens < RTE_SWX_INSTRUCTION_TOKENS_MAX, EILWAL);
		CHECK_NAME(token, EILWAL);

		tokens[n_tokens] = token;
		n_tokens++;
	}

	CHECK(n_tokens, EILWAL);
	...
	if (!strcmp(tokens[tpos], "ilwalidate"))
		return instr_hdr_ilwalidate_translate(p,
						      action,
						      &tokens[tpos],
						      n_tokens - tpos,
						      instr,
	...
	if (!strcmp(tokens[tpos], "jmplw"))
		return instr_jmp_ilwalid_translate(p,
						   action,
						   &tokens[tpos],
						   n_tokens - tpos,
						   instr,
		  ...
		  uint32_t n_instructions)
{
	...
	for (i = 0; i < n_instructions; i++) {
		...
		for (j = i + 1; j < n_instructions; j++)
			CHECK(strcmp(label, data[j].label), EILWAL);
	}
	     ...
	     uint32_t n_instructions)
{
	if (!a) {
		...
		CHECK(instr[0].type == INSTR_RX, EILWAL);
		   ...
		   uint32_t n_instructions)
{
	...
	CHECK(n_instructions, EILWAL);
	CHECK(instructions, EILWAL);
	for (i = 0; i < n_instructions; i++)
		CHECK_INSTRUCTION(instructions[i], EILWAL);
...
static instr_exec_t instruction_table[] = {
	...
	[INSTR_HDR_ILWALIDATE] = instr_hdr_ilwalidate_exec,
	...
	[INSTR_JMP_ILWALID] = instr_jmp_ilwalid_exec,
			       ...
			       uint32_t n_instructions)
{
	...
	if (args_struct_type_name) {
		CHECK_NAME(args_struct_type_name, EILWAL);
		args_struct_type = struct_type_find(p, args_struct_type_name);
		CHECK(args_struct_type, EILWAL);
	} else {
			      ...
			      uint32_t size)
{
	...
	/* Match checks. */
	CHECK(!params->n_fields || params->fields, EILWAL);
	for (i = 0; i < params->n_fields; i++) {
		...
		CHECK(offset > offset_prev, EILWAL);
		offset_prev = offset;
	}

	/* Action checks. */
	CHECK(params->n_actions, EILWAL);
	CHECK(params->action_names, EILWAL);
	for (i = 0; i < params->n_actions; i++) {
		const char *action_name = params->action_names[i];
		struct action *a;
		uint32_t action_data_size;

		CHECK_NAME(action_name, EILWAL);
