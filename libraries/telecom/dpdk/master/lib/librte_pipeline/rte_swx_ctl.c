		  ...
		  int data_check)
{
	...
	if (data_check) {
		...
		CHECK(i < table->info.n_actions, EILWAL);

		/* action_data. */
		a = &ctl->actions[entry->action_id];
		CHECK((a->data_size && entry->action_data) ||
		      (!a->data_size && !entry->action_data), EILWAL);
	}
				     ...
				     struct rte_swx_table_entry *entry)
{
	...
	CHECK(ctl, EILWAL);
	CHECK(table_name && table_name[0], EILWAL);
					...
					struct rte_swx_table_entry *entry)
{
	...
	CHECK(entry, EILWAL);
	CHECK(!table_entry_check(ctl, table_id, entry, 1, 0), EILWAL);
	...
	if (existing_entry) {
		...
		CHECK(real_existing_entry, EILWAL); /* Coverity. */
					     ...
					     struct rte_swx_table_entry *entry)
{
	...
	CHECK(table, EILWAL);
	table_id = table - ctl->tables;
	CHECK(!table->info.default_action_is_const, EILWAL);
				   ...
				   const char *table_name)
{
	...
	fprintf(f, "# Table %s lwrrently has %u entries.\n",
		table_name,
		n_entries);
	return 0;
}
