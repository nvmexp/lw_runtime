 ...
 * during the pipeline exelwtion. The pipeline library allows the user to define
 * his own table actions by providing lwstomized table action handlers (table
 ...
 *    (ilwoked by the pipeline table lookup operation). API functions:
...
struct rte_table_action_mtr_counters_tc {
	 ...
	 * and before the policer actions are exelwted. Only valid when
	 * *n_packets_valid* is non-zero.
	 */
	uint64_t n_packets[RTE_COLORS];

	/** Number of packet bytes per color at the output of the traffic
	 * metering and before the policer actions are exelwted. Only valid when
 ...
 *   lwrrently registered for the *action* object).
 * @param[in] profile
