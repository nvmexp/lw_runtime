 ...
 * actions to be exelwted on the current packet and the associated action
 ...
 * being exelwted, resulting in action meta-data (stored in the table entry)
 ...
 * associated meta-data. As all the lwrrently defined reserved actions are
...
struct rte_pipeline_port_in_params {
	...
	/** Opaque parameter to be passed to create operation when ilwoked */
	void *arg_create;
	...
	/** Opaque parameter to be passed to the action handler when ilwoked */
	void *arg_ah;
...
struct rte_pipeline_port_out_params {
	...
	/** Callback function exelwting the user actions on bust of input
	packets */
 ...
 * packets read from one of the pipeline input ports and lwrrently processed by
 ...
 * pipeline input ports and lwrrently processed by the pipeline. The hijacked
 ...
 * pipeline input ports and lwrrently processed by the pipeline. The dropped
 ...
 * and then dropping them ilwisibly to the pipeline (by using the
