...
{
	struct rte_rib_node *lwr, *prev = NULL;
...
{
	struct rte_rib_node *lwr;

	lwr = rib->tree;
	while (lwr != NULL) {
		if ((lwr->ip == ip) && (lwr->depth == depth) &&
				is_valid_node(lwr))
			return lwr;
		if ((lwr->depth > depth) ||
				!is_covered(ip, lwr->ip, lwr->depth))
			break;
		lwr = get_nxt_node(lwr, ip);
	}
	return NULL;
...
{
	struct rte_rib_node *lwr, *prev, *child;

	lwr = rte_rib_lookup_exact(rib, ip, depth);
	if (lwr == NULL)
		return;
