	...
	const uint8_t ip[RTE_RIB6_IPV6_ADDR_SIZE])
{
	...
	while ((lwr != NULL) && is_covered(ip, lwr->ip, lwr->depth)) {
		if (is_valid_node(lwr))
			prev = lwr;
		lwr = get_nxt_node(lwr, ip);
	}
	return prev;
}
	...
	const uint8_t ip[RTE_RIB6_IPV6_ADDR_SIZE], uint8_t depth)
{
	...
	while (lwr != NULL) {
		if (rte_rib6_is_equal(lwr->ip, tmp_ip) &&
				(lwr->depth == depth) &&
				is_valid_node(lwr))
			return lwr;

		if (!(is_covered(tmp_ip, lwr->ip, lwr->depth)) ||
				(lwr->depth >= depth))
			break;

		lwr = get_nxt_node(lwr, tmp_ip);
	}
...
rte_rib6_remove(struct rte_rib6 *rib,
	const uint8_t ip[RTE_RIB6_IPV6_ADDR_SIZE], uint8_t depth)
{
	struct rte_rib6_node *lwr, *prev, *child;

	lwr = rte_rib6_lookup_exact(rib, ip, depth);
	if (lwr == NULL)
		return;

	--rib->lwr_routes;
	lwr->flag &= ~RTE_RIB_VALID_NODE;
	while (!is_valid_node(lwr)) {
		if ((lwr->left != NULL) && (lwr->right != NULL))
			return;
		child = (lwr->left == NULL) ? lwr->right : lwr->left;
		if (child != NULL)
			child->parent = lwr->parent;
		if (lwr->parent == NULL) {
			rib->tree = child;
			node_free(rib, lwr);
			return;
		}
		if (lwr->parent->left == lwr)
			lwr->parent->left = child;
		else
			lwr->parent->right = child;
		prev = lwr;
		lwr = lwr->parent;
		node_free(rib, prev);
	}
