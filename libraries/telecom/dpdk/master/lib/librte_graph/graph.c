...
graph_mem_fixup_node_ctx(struct rte_graph *graph)
{
	...
	rte_graph_foreach_node(count, off, graph, node) {
		if (node->parent_id == RTE_NODE_ID_ILWALID) /* Static node */
			name = node->name;
		else /* Cloned node */
			name = node->parent;
...
rte_graph_create(const char *name, struct rte_graph_param *prm)
{
	...
	if (prm == NULL)
		SET_ERR_JMP(EILWAL, fail, "Param should not be NULL");

	if (name == NULL)
		SET_ERR_JMP(EILWAL, fail, "Graph name should not be NULL");
