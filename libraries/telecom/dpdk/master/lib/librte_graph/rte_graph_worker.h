 ...
 */
struct rte_graph {
	uint32_t tail;		     /**< Tail of cirlwlar buffer. */
	uint32_t head;		     /**< Head of cirlwlar buffer. */
	uint32_t cir_mask;	     /**< Cirlwlar buffer wrap around mask. */
	rte_node_t nb_nodes;	     /**< Number of nodes in the graph. */
	rte_graph_off_t *cir_start;  /**< Pointer to cirlwlar buffer. */
...
/**
 * Perform graph walk on the cirlwlar buffer and ilwoke the process function
 ...
 * the next node to pending state in the cirlwlar buffer.
 *
 * @param graph
 ...
 * set the next node to pending state in the cirlwlar buffer.
 *
 * @param graph
 *   Graph pointer returned from rte_graph_lookup().
 * @param node
 ...
 * set the next nodes to pending state in the cirlwlar buffer.
...
/**
 * Put the next stream to pending state in the cirlwlar buffer
 * for further processing. Should be ilwoked after rte_node_next_stream_get().
 *
 * @param graph
 *   Graph pointer returned from rte_graph_lookup().
 * @param node
