...
struct rte_kni_ops {
	...
	/* Pointer to function of configuring promislwous mode */
	int (*config_promislwsity)(uint16_t port_id, uint8_t to_on);
 ...
 *
 * This function is to be exelwted on the main lcore only, after EAL
 * initialization and before any KNI interface is attempted to be
 * allocated
 *
 * @param max_kni_ifaces
 *  The maximum number of KNI interfaces that can coexist conlwrrently
 *
 * @return
 *  - 0 indicates success.
