	      ...
	      struct rte_kni_ops *ops)
{
	...
	if (kni_fd < 0) {
		RTE_LOG(ERR, KNI, "KNI subsystem has not been initialized. Ilwoke rte_kni_init() first\n");
		return NULL;
	}
...
/* default callback for request of configuring promislwous mode */
static int
kni_config_promislwsity(uint16_t port_id, uint8_t to_on)
{
	int ret;
	...
	RTE_LOG(INFO, KNI, "Configure promislwous mode of %d to %d\n",
		port_id, to_on);
	...
	if (ret != 0)
		RTE_LOG(ERR, KNI,
			"Failed to %s promislwous mode for port %u: %s\n",
	...
	case RTE_KNI_REQ_CHANGE_MAC_ADDR: /* Change MAC Address */
		if (kni->ops.config_mac_address)
			...
			req->result = kni_config_mac_address(
					kni->ops.port_id, req->mac_addr);
		break;
	case RTE_KNI_REQ_CHANGE_PROMISC: /* Change PROMISLWOUS MODE */
		if (kni->ops.config_promislwsity)
			req->result = kni->ops.config_promislwsity(
					kni->ops.port_id, req->promislwsity);
		else if (kni->ops.port_id != UINT16_MAX)
			req->result = kni_config_promislwsity(
					kni->ops.port_id, req->promislwsity);
		break;
