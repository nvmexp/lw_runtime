...
#define A64_ILWALID_OP_CODE	(0xffffffff)
...
static int
check_ilwalid_args(struct a64_jit_ctx *ctx, uint32_t limit)
{
	uint32_t idx;
	...
	for (idx = 0; idx < limit; idx++) {
		if (rte_le_to_cpu_32(ctx->ins[idx]) == A64_ILWALID_OP_CODE) {
			RTE_BPF_LOG(ERR,
...
emit_mov_imm(struct a64_jit_ctx *ctx, bool is64, uint8_t rd, uint64_t val)
{
	...
	sr = movn ? rte_fls_u64(lwal) - 1 : rte_fls_u64(val) - 1;
	sr = RTE_ALIGN_FLOOR(sr, 16);
	sr = RTE_MAX(sr, 0);

	if (movn)
		mov_imm(ctx, 1, rd, A64_MOVN, (lwal >> sr) & 0xffff, sr);
	else
...
emit(struct a64_jit_ctx *ctx, struct rte_bpf *bpf)
{
	...
	rc = check_ilwalid_args(ctx, ctx->idx);
...
bpf_jit_arm64(struct rte_bpf *bpf)
{
	...
	/* First pass to callwlate total code size and valid jump offsets */
	rc = emit(&ctx, bpf);
	if (rc)
		goto finish;
