 ...
 * callwlate load offset and check is it inside first packet segment.
 */
static void
 ...
 * perform data load and endianness colwersion.
...
emit_ld_mbuf(struct bpf_jit_state *st, uint32_t op, uint32_t sreg, uint32_t imm)
{
	...
	/* dry run first to callwlate jump offsets */
...
bpf_jit_x86(struct rte_bpf *bpf)
{
	...
	/*
	 * dry runs, used to callwlate total code size and valid jump offsets.
