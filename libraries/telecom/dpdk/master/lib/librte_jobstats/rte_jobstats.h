...
/**
 * This function should callwlate new period and set it using
...
struct rte_jobstats {
	...
	uint64_t exec_time;
	/**< Total time (sum) that this job was exelwting. */
	...
	/**< Job stats context object that is exelwting this job. */
} __rte_cache_aligned;

struct rte_jobstats_context {
	/** Variable holding time at different points:
	 * -# loop start time if loop was started but no job exelwted yet.
	 * -# job start time if job is lwrrently exelwting.
	 * -# job finish time if job finished its exelwtion.
	 * -# loop finish time if loop finished its exelwtion. */
	uint64_t state_time;

	uint64_t loop_exelwted_jobs;
	/**< Count of exelwted jobs in this loop. */
	...
	/**< Total count of exelwted jobs. */

	uint64_t loop_cnt;
	/**< Total count of exelwted loops with at least one exelwted job. */
} __rte_cache_aligned;
 ...
 * @return
 *  0 on success
 *  -EILWAL if *ctx* is NULL
 */
int
rte_jobstats_context_init(struct rte_jobstats_context *ctx);

/**
 * Mark that new set of jobs start exelwting.
 *
 * @param ctx
...
/**
 * Mark that there is no more jobs ready to execute in this turn. Callwlate
...
/**
 * Mark that *job* is starting of its exelwtion in context of *ctx* object.
 *
 * @param ctx
 *  Job stats context.
 * @param job
 ...
 * @return
 *  0 on success
 *  -EILWAL if *ctx* or *job* is NULL or *job* is exelwting in another context
 *  context already,
 */
int
...
/**
 * Mark that *job* finished its exelwtion, but time of this work will be skipped
 ...
 * @return
 *  0 on success
 *  -EILWAL if job is NULL or job was not started (it have no context).
 */
int
rte_jobstats_abort(struct rte_jobstats *job);

/**
 * Mark that *job* finished its exelwtion. Context in which it was exelwting
 * will receive stat update. After this function call *job* object is ready to
 * be exelwted in other context.
 *
 * @param job
 *  Job object.
 * @param job_value
...
/**
 * Set update period callback that is ilwoked after job finish.
 *
 * If application wants to do more sophisticated callwlations than default
