	...
	int common_bytes, void **tbl)
{
	...
	for (i = 3, j = 0; i <= common_bytes; i++) {
		...
		val = get_tbl_val_by_idx(lwr_tbl, idx, dp->nh_sz);
		tbl_ptr = get_tbl_p_by_idx(lwr_tbl, idx, dp->nh_sz);
	...
	*tbl = get_tbl_p_by_idx(lwr_tbl, prev_idx * TRIE_TBL8_GRP_NUM_ENT,
		dp->nh_sz);
	return 0;
}
