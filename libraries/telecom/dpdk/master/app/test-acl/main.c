...
get_ulong_opt(const char *opt, const char *name, size_t min, size_t max)
{
	...
	if (errno != 0 || end[0] != 0 || val > max || val < min)
		rte_exit(-EILWAL, "invalid value: \"%s\" for option: %s\n",
			opt, name);
	return val;
}
...
print_usage(const char *prgname)
{
	...
	fprintf(stdout,
		...
		"[--" OPT_MAX_SIZE
			"=<size limit (in bytes) for runtime ACL strulwtures> "
...
check_config(void)
{
	if (config.rule_file == NULL) {
		...
		rte_exit(-EILWAL, "mandatory option %s is not specified\n",
			OPT_RULE_FILE);
	}
}
...
get_input_opts(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argv, "", lgopts,  &opt_idx)) != EOF) {
		...
		if (opt != 0) {
			print_usage(config.prgname);
			rte_exit(-EILWAL, "unknown option: %c", opt);
		}
