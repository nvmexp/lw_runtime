		  ...
		  struct ipsec_sa *sa)
{
	...
	sa->ipsec_xform.direction = RTE_SELWRITY_IPSEC_SA_DIR_EGRESS;
	sa->ipsec_xform.proto = RTE_SELWRITY_IPSEC_SA_PROTO_ESP;
	sa->ipsec_xform.mode = RTE_SELWRITY_IPSEC_SA_MODE_TUNNEL;
	sa->ipsec_xform.tunnel.type = RTE_SELWRITY_IPSEC_TUNNEL_IPV4;
...
{
	sa->ipsec_xform.spi = DEFAULT_SPI;
	sa->ipsec_xform.direction = RTE_SELWRITY_IPSEC_SA_DIR_INGRESS;
	sa->ipsec_xform.proto = RTE_SELWRITY_IPSEC_SA_PROTO_ESP;
	sa->ipsec_xform.mode = RTE_SELWRITY_IPSEC_SA_MODE_TUNNEL;
	sa->ipsec_xform.tunnel.type = RTE_SELWRITY_IPSEC_TUNNEL_IPV4;
	sa->ipsec_xform.options.esn = test_cfg->esn;
		...
		struct ipsec_sa *sa_out, struct ipsec_sa *sa_in)
{
	...
	rc = create_sa(RTE_SELWRITY_ACTION_TYPE_NONE, sa_out);
	if (rc != 0) {
		RTE_LOG(ERR, USER1, "out bound create_sa failed, cfg\n");
		return TEST_FAILED;
	}

	rc = create_sa(RTE_SELWRITY_ACTION_TYPE_NONE, sa_in);
	if (rc != 0) {
		RTE_LOG(ERR, USER1, "out bound create_sa failed, cfg\n");
		return TEST_FAILED;
	}
