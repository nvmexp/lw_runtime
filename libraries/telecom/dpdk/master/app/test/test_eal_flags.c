...
test_allow_flag(void)
{
	...
	if (get_lwrrent_prefix(tmp, sizeof(tmp)) == NULL) {
	...
	for (i = 0; i < RTE_DIM(wlilwal); i++) {
		if (launch_proc(wlilwal[i]) == 0) {
...
test_ilwalid_b_flag(void)
{
	...
	for (i = 0; i != RTE_DIM(blilwal); i++) {
		if (launch_proc(blilwal[i]) == 0) {
			printf("Error - process did run ok with invalid "
			    "blocklist parameter\n");
			return -1;
		}
...
test_ilwalid_vdev_flag(void)
{
	...
	const char *vdevilwal[] = {prgname, prefix, no_huge,
				vdev, "eth_dummy"};
...
test_ilwalid_r_flag(void)
{
	...
	for (i = 0; i != RTE_DIM(rilwal); i++) {
		if (launch_proc(rilwal[i]) == 0) {
			printf("Error - process did run ok with invalid "
			    "-r (rank) parameter\n");
			return -1;
		}
...
test_file_prefix(void)
{
	...
	return 0;
#else
	if (get_lwrrent_prefix(prefix, sizeof(prefix)) == NULL) {
		printf("Error - unable to get current prefix!\n");
		return -1;
	}
#endif
...
test_memory_flags(void)
{
	...
	char ilwalid_socket_mem[SOCKET_MEM_STRLEN];
	const char *argv9[] = {prgname,
			"--file-prefix=" memtest, ilwalid_socket_mem};
	...
	populate_socket_mem_param(num_sockets + 1, "2", "",
		ilwalid_socket_mem, sizeof(ilwalid_socket_mem));
	if (launch_proc(argv9) == 0) {
...
test_eal_flags(void)
{
	...
	ret = test_ilwalid_n_flag();
	if (ret < 0) {
		printf("Error in test_ilwalid_n_flag()\n");
		return ret;
	}
	...
	ret = test_ilwalid_b_flag();
	if (ret < 0) {
		printf("Error in test_ilwalid_b_flag()\n");
		return ret;
	}

#ifdef RTE_NET_RING
	ret = test_ilwalid_vdev_flag();
	if (ret < 0) {
		printf("Error in test_ilwalid_vdev_flag()\n");
		return ret;
	}
#endif
	ret = test_ilwalid_r_flag();
	if (ret < 0) {
		printf("Error in test_ilwalid_r_flag()\n");
		return ret;
	}
...
REGISTER_TEST_COMMAND(eal_flags_n_opt_autotest, test_ilwalid_n_flag);
...
REGISTER_TEST_COMMAND(eal_flags_b_opt_autotest, test_ilwalid_b_flag);
REGISTER_TEST_COMMAND(eal_flags_vdev_opt_autotest, test_ilwalid_vdev_flag);
REGISTER_TEST_COMMAND(eal_flags_r_opt_autotest, test_ilwalid_r_flag);
