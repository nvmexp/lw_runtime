...
testsuite_setup(void)
{
	...
	if (rte_event_dev_service_id_get(evdev, &service_id) == 0) {
		 ...
		 * adapter as well. 2 more cores required to colwert to
		 * service cores.
		 */
...
adapter_create(void)
{
	...
	TEST_ASSERT_NULL(adapter, "Created adapter with invalid "
			"event device id");
	TEST_ASSERT_EQUAL(rte_errno, EILWAL, "Incorrect errno value for "
...
static int
event_timer_arm_ilwalid_sched_type(void)
{
	int ret;
	struct rte_event_timer *evtim = NULL;
	const struct rte_event_timer init_tim = {
	...
	*evtim = init_tim;
	...
	TEST_ASSERT_EQUAL(rte_errno, EILWAL, "Unexpected rte_errno value after"
 ...
 */
static int
event_timer_arm_ilwalid_timeout(void)
{
	int ret;
	struct rte_event_timer *evtim = NULL;
	const struct rte_event_timer init_tim = {
...
static struct unit_test_suite event_timer_adptr_functional_testsuite  = {
	...
	.unit_test_cases = {
		...
		TEST_CASE_ST(timdev_setup_msec, timdev_teardown,
				event_timer_arm_max),
		TEST_CASE_ST(timdev_setup_msec, timdev_teardown,
				event_timer_arm_ilwalid_sched_type),
		TEST_CASE_ST(timdev_setup_msec, timdev_teardown,
				event_timer_arm_ilwalid_timeout),
		TEST_CASE_ST(timdev_setup_msec, timdev_teardown,
				event_timer_cancel),
		TEST_CASE_ST(timdev_setup_msec, timdev_teardown,
