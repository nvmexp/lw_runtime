 ...
 *    - Again we check that the expected number of callbacks has oclwrred when
...
timer_basic_cb(struct rte_timer *tim, void *arg)
{
	...
	RTE_LOG(INFO, TESTTIMER,
		...
		lwr_time, timinfo->id, timinfo->count, lcore_id);
...
test_timer(void)
{
	...
	/* callwlate the "end of test" time */
	lwr_time = rte_get_timer_cycles();
	hz = rte_get_timer_hz();
	end_time = lwr_time + (hz * TEST_DURATION_S);
