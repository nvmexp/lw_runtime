...
static const char *relwrsive_call; /* used in linux for MP and other tests */

static int
no_action(void){ return 0; }

static int
do_relwrsive_call(void)
{
	unsigned i;
	struct {
		const char *elw_var;
		int (*action_fn)(void);
			...
			{ "test_ilwalid_n_flag", no_action },
			{ "test_no_hpet_flag", no_action },
			{ "test_allow_flag", no_action },
			{ "test_ilwalid_b_flag", no_action },
			{ "test_ilwalid_vdev_flag", no_action },
			{ "test_ilwalid_r_flag", no_action },
	...
	if (relwrsive_call == NULL)
		return -1;
	for (i = 0; i < RTE_DIM(actions); i++) {
		if (strcmp(actions[i].elw_var, relwrsive_call) == 0)
...
main(int argc, char **argv)
{
	...
	extra_args = getelw("DPDK_TEST_PARAMS");
	...
	relwrsive_call = getelw(RELWRSIVE_ELW_VAR);
	if (relwrsive_call != NULL) {
		ret = do_relwrsive_call();
		goto out;
	}
	...
	char *dpdk_test = getelw("DPDK_TEST");
	...
	/* if no DPDK_TEST elw variable, go interactive */
	cmdline_interact(cl);
	cmdline_stdin_exit(cl);
#endif
	ret = 0;
...
{
	int test_success;
	unsigned int total = 0, exelwted = 0, skipped = 0;
