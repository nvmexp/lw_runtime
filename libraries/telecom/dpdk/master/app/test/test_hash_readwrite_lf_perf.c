...
#ifndef RTE_HASH_EXTRA_FLAGS_RW_CONLWRRENCY_LF
#define RTE_HASH_EXTRA_FLAGS_RW_CONLWRRENCY_LF 0
#endif
			...
			uint32_t lwr_bkt_idx, uint16_t sig)
{
	...
	num_buckets  = rte_align32pow2(TOTAL_ENTRY) / 8;
	bucket_bitmask = num_buckets - 1;
	return (lwr_bkt_idx ^ sig) & bucket_bitmask;
}
...
check_bucket(uint32_t bkt_idx, uint32_t key)
{
	...
	while (rte_hash_iterate(tbl_rwc_test_param.h,
		...
		/* All entries in the bucket are oclwpied */
		if (count == 8) {
			...
			if (scanned_bkts[bkt_idx] == 0) {
				 ...
				 * collision oclwrs. Thus, add it to
				 * keys_shift_path.
				 */
...
generate_keys(void)
{
	 ...
	 * path (i.e. being moved to alternate location), when collision oclwrs
			   ...
			   int htm, int ext_bkt)
{
	...
	for (m = 1; m < NUM_TEST; m++) {
		/* Callwlate keys added by each writer */
...
test_hash_rlw_qsbr_reader(void *arg)
{
	...
	(void)rte_rlw_qsbr_thread_register(rv, lcore_id);
	rte_rlw_qsbr_thread_online(rv, lcore_id);
	do {
		for (i = 0; i < num_keys; i += j) {
			...
			rte_rlw_qsbr_quiescent(rv, lcore_id);
		}
	} while (!writer_done);
	rte_rlw_qsbr_thread_offline(rv, lcore_id);
	(void)rte_rlw_qsbr_thread_unregister(rv, lcore_id);

	return 0;
}

/*
 * Writer thread using rte_hash data structure with RLW
 */
static int
test_hash_rlw_qsbr_writer(void *arg)
{
	uint32_t i, offset;
	uint64_t begin, cycles;
...
/*
 * Writer perf test with RLW QSBR in DQ mode:
 * Writer(s) delete and add keys in the table.
 * Readers lookup keys in the hash table
 */
static int
test_hash_rlw_qsbr_writer_perf(struct rwc_perf *rwc_perf_results, int rwc_lf,
				int htm, int ext_bkt)
{
	unsigned int n;
	uint64_t i;
	uint8_t write_type;
	int use_jhash = 0;
	struct rte_hash_rlw_config rlw_config = {0};
	uint32_t sz;
	uint8_t pos_core;

	printf("\nTest: Writer perf with integrated RLW\n");

	if (init_params(rwc_lf, use_jhash, htm, ext_bkt) != 0)
		goto err;

	sz = rte_rlw_qsbr_get_memsize(RTE_MAX_LCORE);
	rv = (struct rte_rlw_qsbr *)rte_zmalloc(NULL, sz, RTE_CACHE_LINE_SIZE);
	rlw_config.v = rv;

	if (rte_hash_rlw_qsbr_add(tbl_rwc_test_param.h, &rlw_config) < 0) {
		printf("RLW init in hash failed\n");
		goto err;
	}
	...
	for (n = 0; n < NUM_TEST; n++) {
		...
		rte_hash_reset(tbl_rwc_test_param.h);
		rte_rlw_qsbr_init(rv, RTE_MAX_LCORE);
		...
		for (; i <= rwc_core_cnt[n] + 2; i++) {
			rte_eal_remote_launch(test_hash_rlw_qsbr_writer,
				(void *)(uintptr_t)pos_core,
				enabled_core_ids[i]);
			pos_core++;
		}
...
test_hash_readwrite_lf_perf_main(void)
{
	 ...
	 * rwc_lf indicates if read-write conlwrrency lock-free support is
	 * enabled.
	...
	if (RTE_HASH_EXTRA_FLAGS_RW_CONLWRRENCY_LF) {
		rwc_lf = 1;
		ext_bkt = 1;
		printf("Test lookup with read-write conlwrrency lock free support"
		       " enabled\n");
		...
		if (test_hash_add_ks_lookup_hit_extbkt(&rwc_lf_results, rwc_lf,
							htm, ext_bkt) < 0)
			return -1;
		if (test_hash_rlw_qsbr_writer_perf(&rwc_lf_results, rwc_lf,
						   htm, ext_bkt) < 0)
			return -1;
	}
	printf("\nTest lookup with read-write conlwrrency lock free support"
	       " disabled\n");
	rwc_lf = 0;
	if (!htm) {
