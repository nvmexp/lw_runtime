 ...
 * subprocess the source of the call, the elw_value parameter is set in the
 * environment as $RTE_TEST
 */
static inline int
process_dup(const char *const argv[], int numargs, const char *elw_value)
{
	int num;
	...
	if (pid < 0)
...
#ifdef RTE_EXEC_ELW_LINUX
		{
			...
			while ((dirent = readdir(dir)) != NULL) {
				...
				if (errno != 0 || endptr[0] != '\0') {
					printf("Error colwerting name fd %d %s:\n",
						fd, dirent->d_name);
					continue;
				}
		...
		if (setelw(RELWRSIVE_ELW_VAR, elw_value, 1) != 0)
...
#ifdef RTE_LIB_PDUMP
#ifdef RTE_NET_RING
	if ((strcmp(elw_value, "run_pdump_server_tests") == 0))
...
#ifdef RTE_EXEC_ELW_LINUX
static char *
get_lwrrent_prefix(char *prefix, int size)
{
