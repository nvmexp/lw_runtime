...
static int
test_cfgfile_ilwalid_section_header(void)
{
	struct rte_cfgfile *cfgfile;

	cfgfile = rte_cfgfile_load(CFG_FILES_ETC "/ilwalid_section.ini", 0);
...
static int
test_cfgfile_ilwalid_comment(void)
{
	struct rte_cfgfile_parameters params;
	struct rte_cfgfile *cfgfile;
...
static int
test_cfgfile_ilwalid_key_value_pair(void)
{
	struct rte_cfgfile *cfgfile;
...
test_cfgfile(void)
{
	...
	if (test_cfgfile_ilwalid_section_header())
		return -1;

	if (test_cfgfile_ilwalid_comment())
		return -1;

	if (test_cfgfile_ilwalid_key_value_pair())
		return -1;
