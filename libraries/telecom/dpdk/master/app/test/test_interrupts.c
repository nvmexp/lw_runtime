...
test_interrupt_init(void)
{
	...
	intr_handles[TEST_INTERRUPT_HANDLE_ILWALID].fd = -1;
	intr_handles[TEST_INTERRUPT_HANDLE_ILWALID].type =
...
test_interrupt_enable(void)
{
	...
	test_intr_handle = intr_handles[TEST_INTERRUPT_HANDLE_ILWALID];
...
test_interrupt(void)
{
	...
	if (test_interrupt_full_path_check(TEST_INTERRUPT_HANDLE_VALID) < 0) {
		printf("failure oclwrred during checking unknown valid "
						"interrupt full path\n");
		goto out;
	}
	...
	if (test_interrupt_full_path_check(TEST_INTERRUPT_HANDLE_VALID_UIO)
									< 0) {
		printf("failure oclwrred during checking valid UIO interrupt "
								"full path\n");
		goto out;
	}
	...
	if (test_interrupt_full_path_check(
		...
		printf("failure oclwrred during checking valid device event "
						"interrupt full path\n");
		goto out;
	}
	...
	if (test_interrupt_full_path_check(
		...
		printf("failure oclwrred during checking valid alarm "
						"interrupt full path\n");
		goto out;
	}
