...
/*
 * Hash values callwlated for key sizes from array "hashtest_key_lens"
 ...
 * (using precallwlated values stored above)
 */
static int
verify_precallwlated_hash_func_tests(void)
{
	unsigned i, j;
	uint8_t key[64];
	uint32_t hash;
...
{
	if (verify_precallwlated_hash_func_tests() != 0)
		return -1;
