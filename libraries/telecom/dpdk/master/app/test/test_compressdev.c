...
static int
test_compressdev_ilwalid_configuration(void)
{
	struct rte_compressdev_config ilwalid_config;
	...
	memcpy(&ilwalid_config, &valid_config,
			sizeof(struct rte_compressdev_config));
	ilwalid_config.nb_queue_pairs = 0;

	TEST_ASSERT_FAIL(rte_compressdev_configure(0, &ilwalid_config),
	...
	if (dev_info.max_nb_queue_pairs != 0) {
		memcpy(&ilwalid_config, &valid_config,
			sizeof(struct rte_compressdev_config));
		ilwalid_config.nb_queue_pairs = dev_info.max_nb_queue_pairs + 1;
...
/**
 * Data size callwlation (for both compression and decompression).
 *
 * Callwlate size of anticipated output buffer required for both
...
test_mbufs_callwlate_data_size(
...
test_setup_output_bufs(
		...
		const struct interim_data_params *int_data,
		const struct test_data_params *test_data,
		struct rte_mbuf_ext_shared_info *lwrrent_extbuf_info)
{
	/* local variables: */
	unsigned int i;
	uint32_t data_size;
	...
	unsigned int buff_type = test_data->buff_type;
	unsigned int big_data = test_data->big_data;
	const struct rte_memzone *lwrrent_memzone;
	...
	if (op_type == OPERATION_COMPRESSION)
		lwrrent_bufs = test_priv_data->comp_bufs;
	else
		lwrrent_bufs = test_priv_data->uncomp_bufs;

	/* the mbufs allocation*/
	ret = rte_pktmbuf_alloc_bulk(buf_pool, lwrrent_bufs, num_bufs);
	if (ret < 0) {
		RTE_LOG(ERR, USER1,
	...
	if (test_data->use_external_mbufs) {
		lwrrent_extbuf_info->free_cb = extbuf_free_callback;
		lwrrent_extbuf_info->fcb_opaque = NULL;
		rte_mbuf_ext_refcnt_set(lwrrent_extbuf_info, 1);
		if (op_type == OPERATION_COMPRESSION)
			lwrrent_memzone = test_data->compbuf_memzone;
		else
			lwrrent_memzone = test_data->uncompbuf_memzone;

		for (i = 0; i < num_bufs; i++) {
			rte_pktmbuf_attach_extbuf(lwrrent_bufs[i],
					lwrrent_memzone->addr,
					lwrrent_memzone->iova,
					lwrrent_memzone->len,
					lwrrent_extbuf_info);
			rte_pktmbuf_append(lwrrent_bufs[i],
					lwrrent_memzone->len);
		}
	} else {
		for (i = 0; i < num_bufs; i++) {
			...
			/* data size callwlation */
			data_size = test_mbufs_callwlate_data_size(
					op_type,
			...
			if (buff_type == SGL_BOTH || buff_type == LB_TO_SGL) {
				ret = prepare_sgl_bufs(NULL, lwrrent_bufs[i],
				      data_size,
				      big_data ? buf_pool :
				...
				buf_ptr = rte_pktmbuf_append(lwrrent_bufs[i],
						data_size);
				if (buf_ptr == NULL) {
					RTE_LOG(ERR, USER1,
 ...
 *   A container used for aggregation all the private test arrays.
 * @return
 *   - 2: Next step must be exelwted by the caller (stateful decompression only)
...
static struct unit_test_suite compressdev_testsuite  = {
	...
	.unit_test_cases = {
		TEST_CASE_ST(NULL, NULL,
			test_compressdev_ilwalid_configuration),
