...
#define REORDER_BUFFER_SIZE_ILWALID 2049
...
test_reorder_create(void)
{
	...
	TEST_ASSERT((b == NULL) && (rte_errno == EILWAL),
			"No error on create() with NULL name");

	b = rte_reorder_create("PKT", rte_socket_id(), REORDER_BUFFER_SIZE_ILWALID);
	TEST_ASSERT((b == NULL) && (rte_errno == EILWAL),
...
test_reorder_init(void)
{
	...
	size = 262336;
	b = rte_malloc(NULL, size, 0);
	b = rte_reorder_init(b, size, "PKT1", REORDER_BUFFER_SIZE_ILWALID);
	TEST_ASSERT((b == NULL) && (rte_errno == EILWAL),
