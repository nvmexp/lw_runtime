...
{
	char coremask[10];
#ifdef RTE_EXEC_ELW_LINUXAPP
	char tmp[PATH_MAX] = {0};
	char prefix[PATH_MAX] = {0};

	get_lwrrent_prefix(tmp, sizeof(tmp));
...
#define TICK_MSECS 1
	...
	uint64_t prev_tsc = 0, lwr_tsc, diff_tsc;
...
test_timer_secondary(void)
{
	...
	if (proc_type == RTE_PROC_PRIMARY) {
		...
		*sec_lcorep = rte_get_next_lcore(*mgr_lcorep, 1, 1);
		...
		TEST_ASSERT_SUCCESS(ret, "Secondary process exelwtion failed");
