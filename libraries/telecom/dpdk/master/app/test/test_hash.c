...
#define RETURN_IF_ERROR_RLW_QSBR(cond, str, ...) do {			\
	if (cond) {							\
		printf("ERROR line %d: " str "\n", __LINE__, ##__VA_ARGS__); \
		if (rlw_cfg.mode == RTE_HASH_QSBR_MODE_SYNC) {		\
			writer_done = 1;				\
...
/*
 * Sequence of operations for a single key with 'rw conlwrrency lock free' set:
 *	- add
 *	- delete: hit
 *	- free: hit
...
{
/* Should match the #define LCORE_CACHE_SIZE value in rte_lwckoo_hash.h */
#define LCORE_CACHE_SIZE	64
	...
	ut_params.extra_flag = RTE_HASH_EXTRA_FLAGS_RW_CONLWRRENCY_LF;
	handle = rte_hash_create(&ut_params);
	RETURN_IF_ERROR(handle == NULL, "hash creation failed");
...
fbk_hash_unit_test(void)
{
	...
	struct rte_fbk_hash_params ilwalid_params_1 = {
		.name = "ilwalid_1",
	...
	struct rte_fbk_hash_params ilwalid_params_2 = {
		.name = "ilwalid_2",
		.entries = 4,
	...
	struct rte_fbk_hash_params ilwalid_params_3 = {
		.name = "ilwalid_3",
	...
	struct rte_fbk_hash_params ilwalid_params_4 = {
		.name = "ilwalid_4",
		.entries = LOCAL_FBK_HASH_ENTRIES_MAX,
	...
	struct rte_fbk_hash_params ilwalid_params_5 = {
		.name = "ilwalid_5",
		.entries = 4,
	...
	struct rte_fbk_hash_params ilwalid_params_6 = {
		.name = "ilwalid_6",
	...
	struct rte_fbk_hash_params ilwalid_params_7 = {
		.name = "ilwalid_7",
	...
	struct rte_fbk_hash_params ilwalid_params_8 = {
		.name = "ilwalid_7",
		.entries = RTE_FBK_HASH_ENTRIES_MAX,
		.entries_per_bucket = 4,
	 ...
	 */
	struct rte_fbk_hash_params ilwalid_params_same_name_1 = {
	...
	struct rte_fbk_hash_params ilwalid_params_same_name_2 = {
		.name = "same_name",				/* hash with identical name */
		.entries = RTE_FBK_HASH_ENTRIES_MAX,
		.entries_per_bucket = 4,
		.socket_id = 0,
	...
	printf("# Testing hash creation with invalid parameters "
			"- expect error msgs\n");
	handle = rte_fbk_hash_create(&ilwalid_params_1);
	RETURN_IF_ERROR_FBK(handle != NULL, "fbk hash creation should have failed");

	handle = rte_fbk_hash_create(&ilwalid_params_2);
	RETURN_IF_ERROR_FBK(handle != NULL, "fbk hash creation should have failed");

	handle = rte_fbk_hash_create(&ilwalid_params_3);
	RETURN_IF_ERROR_FBK(handle != NULL, "fbk hash creation should have failed");

	handle = rte_fbk_hash_create(&ilwalid_params_4);
	RETURN_IF_ERROR_FBK(handle != NULL, "fbk hash creation should have failed");

	handle = rte_fbk_hash_create(&ilwalid_params_5);
	RETURN_IF_ERROR_FBK(handle != NULL, "fbk hash creation should have failed");

	handle = rte_fbk_hash_create(&ilwalid_params_6);
	RETURN_IF_ERROR_FBK(handle != NULL, "fbk hash creation should have failed");

	handle = rte_fbk_hash_create(&ilwalid_params_7);
	RETURN_IF_ERROR_FBK(handle != NULL, "fbk hash creation should have failed");

	if (rte_eal_has_hugepages()) {
		handle = rte_fbk_hash_create(&ilwalid_params_8);
		RETURN_IF_ERROR_FBK(handle != NULL,
					"fbk hash creation should have failed");
	}

	handle = rte_fbk_hash_create(&ilwalid_params_same_name_1);
	RETURN_IF_ERROR_FBK(handle == NULL, "fbk hash creation should have succeeded");

	tmp = rte_fbk_hash_create(&ilwalid_params_same_name_2);
	if (tmp != NULL)
...
static struct rte_rlw_qsbr *g_qsv;
static volatile uint8_t writer_done;
struct flow_key g_rand_keys[9];

/*
 * rte_hash_rlw_qsbr_add positive and negative tests.
 *  - Add RLW QSBR variable to Hash
 *  - Add another RLW QSBR variable to Hash
 *  - Check returns
 */
static int
test_hash_rlw_qsbr_add(void)
{
	size_t sz;
	struct rte_rlw_qsbr *qsv2 = NULL;
	int32_t status;
	struct rte_hash_rlw_config rlw_cfg = {0};
	struct rte_hash_parameters params;

	printf("\n# Running RLW QSBR add tests\n");
	memcpy(&params, &ut_params, sizeof(params));
	params.name = "test_hash_rlw_qsbr_add";
	params.extra_flag = RTE_HASH_EXTRA_FLAGS_RW_CONLWRRENCY_LF |
				RTE_HASH_EXTRA_FLAGS_MULTI_WRITER_ADD;
	g_handle = rte_hash_create(&params);
	RETURN_IF_ERROR_RLW_QSBR(g_handle == NULL, "Hash creation failed");

	/* Create RLW QSBR variable */
	sz = rte_rlw_qsbr_get_memsize(RTE_MAX_LCORE);
	g_qsv = (struct rte_rlw_qsbr *)rte_zmalloc_socket(NULL, sz,
					RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
	RETURN_IF_ERROR_RLW_QSBR(g_qsv == NULL,
				 "RLW QSBR variable creation failed");

	status = rte_rlw_qsbr_init(g_qsv, RTE_MAX_LCORE);
	RETURN_IF_ERROR_RLW_QSBR(status != 0,
				 "RLW QSBR variable initialization failed");

	rlw_cfg.v = g_qsv;
	/* Invalid QSBR mode */
	rlw_cfg.mode = 0xff;
	status = rte_hash_rlw_qsbr_add(g_handle, &rlw_cfg);
	RETURN_IF_ERROR_RLW_QSBR(status == 0, "Invalid QSBR mode test failed");

	rlw_cfg.mode = RTE_HASH_QSBR_MODE_DQ;
	/* Attach RLW QSBR to hash table */
	status = rte_hash_rlw_qsbr_add(g_handle, &rlw_cfg);
	RETURN_IF_ERROR_RLW_QSBR(status != 0,
				 "Attach RLW QSBR to hash table failed");

	/* Create and attach another RLW QSBR to hash table */
	qsv2 = (struct rte_rlw_qsbr *)rte_zmalloc_socket(NULL, sz,
					RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
	RETURN_IF_ERROR_RLW_QSBR(qsv2 == NULL,
				 "RLW QSBR variable creation failed");

	rlw_cfg.v = qsv2;
	rlw_cfg.mode = RTE_HASH_QSBR_MODE_SYNC;
	status = rte_hash_rlw_qsbr_add(g_handle, &rlw_cfg);
	rte_free(qsv2);
	RETURN_IF_ERROR_RLW_QSBR(status == 0,
			"Attach RLW QSBR to hash table succeeded where failure"
			" is expected");
...
/*
 * rte_hash_rlw_qsbr_add DQ mode functional test.
 ...
 */
static int
test_hash_rlw_qsbr_dq_mode(uint8_t ext_bkt)
{
	uint32_t total_entries = (ext_bkt == 0) ? 8 : 9;

	uint8_t hash_extra_flag = RTE_HASH_EXTRA_FLAGS_RW_CONLWRRENCY_LF;

	if (ext_bkt)
		hash_extra_flag |= RTE_HASH_EXTRA_FLAGS_EXT_TABLE;

	struct rte_hash_parameters params_pseudo_hash = {
		.name = "test_hash_rlw_qsbr_dq_mode",
	...
	if (ext_bkt)
		printf("\n# Running RLW QSBR DQ mode functional test with"
		       " ext bkt\n");
	else
		printf("\n# Running RLW QSBR DQ mode functional test\n");
	...
	for (i = 0; i < total_entries; i++) {
		...
		RETURN_IF_ERROR_RLW_QSBR(pos[i] < 0,
	...
	status = rte_rlw_qsbr_thread_register(g_qsv, 0);
	RETURN_IF_ERROR_RLW_QSBR(status != 0,
				 "RLW QSBR thread registration failed");
	rte_rlw_qsbr_thread_online(g_qsv, 0);

	/* Lookup */
	pos[0] = rte_hash_lookup(g_handle, &g_rand_keys[0]);
	print_key_info("Lkp", &g_rand_keys[0], pos[0]);
	RETURN_IF_ERROR_RLW_QSBR(pos[0] != expected_pos[0],
	...
	pos[0] = rte_hash_lookup(g_handle, &g_rand_keys[0]);
	print_key_info("Lkp", &g_rand_keys[0], pos[0]);
	RETURN_IF_ERROR_RLW_QSBR(pos[0] != -ENOENT,
				 "found deleted key (pos[%u]=%d)", 0, pos[0]);

	/* Fill bucket */
	pos[0] = rte_hash_add_key(g_handle, &g_rand_keys[0]);
	print_key_info("Add", &g_rand_keys[0], pos[0]);
	RETURN_IF_ERROR_RLW_QSBR(pos[0] != -ENOSPC,
				 "Added key successfully (pos[%u]=%d)", 0, pos[0]);

	/* Reader quiescent */
	rte_rlw_qsbr_quiescent(g_qsv, 0);

	/* Fill bucket */
	pos[0] = rte_hash_add_key(g_handle, &g_rand_keys[0]);
	print_key_info("Add", &g_rand_keys[0], pos[0]);
	RETURN_IF_ERROR_RLW_QSBR(pos[0] < 0,
				 "failed to add key (pos[%u]=%d)", 0, pos[0]);
	expected_pos[0] = pos[0];

	rte_rlw_qsbr_thread_offline(g_qsv, 0);
	(void)rte_rlw_qsbr_thread_unregister(g_qsv, 0);
...
/*
 * Reader thread using rte_hash data structure with RLW.
 */
static int
test_hash_rlw_qsbr_reader(void *arg)
{
	int i;

	RTE_SET_USED(arg);
	/* Register this thread to report quiescent state */
	(void)rte_rlw_qsbr_thread_register(g_qsv, 0);
	rte_rlw_qsbr_thread_online(g_qsv, 0);
...
/*
 * rte_hash_rlw_qsbr_add sync mode functional test.
 ...
 */
static int
test_hash_rlw_qsbr_sync_mode(uint8_t ext_bkt)
{
	uint32_t total_entries = (ext_bkt == 0) ? 8 : 9;
	...
	struct rte_hash_parameters params_pseudo_hash = {
		.name = "test_hash_rlw_qsbr_sync_mode",
		.entries = total_entries,
		.key_len = sizeof(struct flow_key), /* 13 */
		.hash_func = pseudo_hash,
		.hash_func_init_val = 0,
	...
	if (ext_bkt)
		printf("\n# Running RLW QSBR sync mode functional test with"
		       " ext bkt\n");
	else
		printf("\n# Running RLW QSBR sync mode functional test\n");
	...
	rte_eal_remote_launch(test_hash_rlw_qsbr_reader, NULL,
...
test_hash(void)
{
	...
	if (test_hash_rlw_qsbr_add() < 0)
		return -1;

	if (test_hash_rlw_qsbr_dq_mode(0) < 0)
		return -1;

	if (test_hash_rlw_qsbr_dq_mode(1) < 0)
		return -1;

	if (test_hash_rlw_qsbr_sync_mode(0) < 0)
		return -1;

	if (test_hash_rlw_qsbr_sync_mode(1) < 0)
		return -1;
