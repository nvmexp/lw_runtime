...
check_mem(void *addr, rte_iova_t *iova, size_t pgsz, int n_pages)
{
	...
	for (i = 0; i < n_pages; i++) {
		...
		void *lwr = RTE_PTR_ADD(addr, pgsz * i);
		rte_iova_t expected_iova;

		msl = rte_mem_virt2memseg_list(lwr);
		if (!msl->external) {
		...
		ms = rte_mem_virt2memseg(lwr, msl);
		if (ms == NULL) {
...
static int
test_malloc_ilwalid_param(void *addr, size_t len, size_t pgsz, rte_iova_t *iova,
		int n_pages)
{
	...
	for (i = 0; i < RTE_DIM(names); i++) {
		...
		if (rte_malloc_heap_create(name) >= 0 || rte_errno != EILWAL) {
			printf("%s():%i: Created heap with invalid name\n",
					__func__, __LINE__);
			goto fail;
		}

		if (rte_malloc_heap_destroy(name) >= 0 || rte_errno != EILWAL) {
		...
		if (rte_malloc_heap_memory_add(name, addr, len,
				NULL, 0, pgsz) >= 0 || rte_errno != EILWAL) {
	...
	if (iova != NULL) {
		/* wrong page count */
		if (rte_malloc_heap_memory_add(valid_name, addr, len,
				iova, 0, pgsz) >= 0 || rte_errno != EILWAL) {
			printf("%s():%i: Added memory with invalid parameters\n",
				__func__, __LINE__);
			goto fail;
		}
...
static int
test_extmem_ilwalid_param(void *addr, size_t len, size_t pgsz, rte_iova_t *iova,
		int n_pages)
{
	/* these calls may fail for other reasons, so check errno */
...
test_external_mem(void)
{
	...
	ret = test_malloc_ilwalid_param(addr, len, pgsz, iova, n_pages);
	ret |= test_malloc_basic(addr, len, pgsz, iova, n_pages);
	/* when iova table is NULL, everything should still work */
	ret |= test_malloc_ilwalid_param(addr, len, pgsz, NULL, n_pages);
	ret |= test_malloc_basic(addr, len, pgsz, NULL, n_pages);

	/* test non-heap memory */
	ret |= test_extmem_ilwalid_param(addr, len, pgsz, iova, n_pages);
	ret |= test_extmem_basic(addr, len, pgsz, iova, n_pages);
	/* when iova table is NULL, everything should still work */
	ret |= test_extmem_ilwalid_param(addr, len, pgsz, NULL, n_pages);
