...
combined_table_test table_tests_combined[] = {
	...
	test_table_hash_lwckoo_combined,
};
...
int
test_table_hash_lwckoo_combined(void)
{
	int status, i;
	...
	memset(key_lwckoo, 0, sizeof(key_lwckoo));
	klwckoo[0] = 0xadadadad;
	...
	status = test_table_type(&rte_table_hash_lwckoo_ops,
		(void *)&lwckoo_params, (void *)key_lwckoo, &table_packets,
		NULL, 0);
	VERIFY(status, CHECK_TABLE_OK);
