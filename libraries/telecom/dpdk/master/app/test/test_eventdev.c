...
test_eventdev_socket_id(void)
{
	...
	TEST_ASSERT(socket_id != -EILWAL, "Failed to get socket_id %d",
				socket_id);
	socket_id = rte_event_dev_socket_id(RTE_EVENT_MAX_DEVS);
	TEST_ASSERT(socket_id == -EILWAL, "Expected -EILWAL %d", socket_id);
...
test_eventdev_info_get(void)
{
	...
	TEST_ASSERT(ret == -EILWAL, "Expected -EILWAL, %d", ret);
