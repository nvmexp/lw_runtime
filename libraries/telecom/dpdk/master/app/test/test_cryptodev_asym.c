...
union test_case_structure {
	...
	struct modilw_test_data modilw;
				...
				struct rte_crypto_op *result_op)
{
	...
	case RTE_CRYPTO_ASYM_XFORM_MODEX:
		...
		break;
	case RTE_CRYPTO_ASYM_XFORM_MODILW:
		data_expected = data_tc->modilw.ilwerse.data;
		data_received = result_op->asym->modilw.result.data;
		data_size = result_op->asym->modilw.result.length;
		break;
	...
	enum rte_crypto_rsa_priv_key_type key_type)
{
	...
	case RTE_CRYPTO_ASYM_XFORM_MODEX:
		...
		break;
	case RTE_CRYPTO_ASYM_XFORM_MODILW:
		result = rte_zmalloc(NULL, data_tc->modilw.result_len, 0);
		xform_tc.modilw.modulus.data = data_tc->modilw.modulus.data;
		xform_tc.modilw.modulus.length = data_tc->modilw.modulus.len;
		memcpy(input, data_tc->modilw.base.data,
			data_tc->modilw.base.len);
		asym_op->modilw.base.data = input;
		asym_op->modilw.base.length = data_tc->modilw.base.len;
		asym_op->modilw.result.data = result;
		asym_op->modilw.result.length = data_tc->modilw.result_len;
		if (rte_cryptodev_asym_xform_capability_check_modlen(capability,
				xform_tc.modilw.modulus.length)) {
			snprintf(test_msg, ASYM_TEST_MSG_LEN,
				"line %u "
				"FAILED: %s", __LINE__,
				"Invalid MODULUS length specified");
	...
	case RTE_CRYPTO_ASYM_XFORM_RSA:
		...
		if (key_type == RTE_RSA_KEY_TYPE_EXP) {
			...
			xform_tc.rsa.qt.qIlw.data = data_tc->rsa_data.qIlw.data;
			xform_tc.rsa.qt.qIlw.length = data_tc->rsa_data.qIlw.len;
		}
...
test_one_case(const void *test_case, int sessionless)
{
	...
	if (tc.modex.xform_type == RTE_CRYPTO_ASYM_XFORM_MODEX
			|| tc.modex.xform_type == RTE_CRYPTO_ASYM_XFORM_MODILW) {
...
load_test_vectors(void)
{
	...
	/* Load MODILW vector*/
	v_size = RTE_DIM(modilw_test_case);
	for (i = 0; i < v_size; i++) {
		if (test_vector.size >= (TEST_VECTOR_SIZE)) {
			RTE_LOG(DEBUG, USER1,
				"TEST_VECTOR_SIZE too small\n");
		...
		test_vector.address[test_vector.size] = &modilw_test_case[i];
		test_vector.size++;
	}
	/* Load RSA vector*/
...
test_mod_ilw(void)
{
	...
	if (rte_cryptodev_asym_get_xform_enum(
		&modilw_xform.xform_type, "modilw") < 0) {
		RTE_LOG(ERR, USER1,
				 "Invalid ASYM algorithm specified\n");
		return -1;
	}

	cap_idx.type = modilw_xform.xform_type;
	capability = rte_cryptodev_asym_capability_get(dev_id,
					&cap_idx);

	if (capability == NULL) {
		RTE_LOG(INFO, USER1,
			"Device doesn't support MOD ILW. Test Skipped\n");
		return -ENOTSUP;
	}

	if (rte_cryptodev_asym_xform_capability_check_modlen(
		capability,
		modilw_xform.modilw.modulus.length)) {
		RTE_LOG(ERR, USER1,
				 "Invalid MODULUS length specified\n");
				return -ENOTSUP;
		}
	...
	if (rte_cryptodev_asym_session_init(dev_id, sess, &modilw_xform,
			sess_mpool) < 0) {
		RTE_LOG(ERR, USER1,
				"line %u FAILED: %s",
				__LINE__, "unabled to config sym session");
	...
	asym_op->modilw.base.data = input;
	asym_op->modilw.base.length = sizeof(base);
	asym_op->modilw.result.data = result;
	asym_op->modilw.result.length = sizeof(result);
	...
	ret = verify_modilw(mod_ilw, result_op);
	if (ret) {
		RTE_LOG(ERR, USER1,
...
static int
test_ecdsa_sign_verify(enum lwrve lwrve_id)
{
	struct crypto_testsuite_params *ts_params = &testsuite_params;
	struct rte_mempool *sess_mpool = ts_params->session_mpool;
	struct rte_mempool *op_mpool = ts_params->op_mpool;
	...
	xform.ec.lwrve_id = input_params.lwrve;
...
static int
test_ecdsa_sign_verify_all_lwrve(void)
{
	...
	for (lwrve_id = SECP192R1; lwrve_id < END_OF_LWRVE_LIST; lwrve_id++) {
		status = test_ecdsa_sign_verify(lwrve_id);
		...
		printf("  %u) TestCase Sign/Veriy Lwrve %s  %s\n",
		       test_index ++, lwrve[lwrve_id], msg);
	}
	return overall_status;
}

static int
test_ecpm(enum lwrve lwrve_id)
{
	struct crypto_testsuite_params *ts_params = &testsuite_params;
	struct rte_mempool *sess_mpool = ts_params->session_mpool;
	struct rte_mempool *op_mpool = ts_params->op_mpool;
...
test_ecpm_all_lwrve(void)
{
	...
	for (lwrve_id = SECP192R1; lwrve_id < END_OF_LWRVE_LIST; lwrve_id++) {
		...
		printf("  %u) TestCase EC Point Mul Lwrve %s  %s\n",
		       test_index ++, lwrve[lwrve_id], msg);
	}
	return overall_status;
}
...
static struct unit_test_suite cryptodev_openssl_asym_testsuite  = {
	...
	.unit_test_cases = {
		...
		TEST_CASE_ST(ut_setup, ut_teardown, test_mod_ilw),
...
static struct unit_test_suite cryptodev_octeontx_asym_testsuite  = {
	...
	.unit_test_cases = {
		...
		TEST_CASE_ST(ut_setup, ut_teardown,
			     test_ecdsa_sign_verify_all_lwrve),
		TEST_CASE_ST(ut_setup, ut_teardown, test_ecpm_all_lwrve),
		TEST_CASES_END() /**< NULL terminate unit test array */
	}
};
