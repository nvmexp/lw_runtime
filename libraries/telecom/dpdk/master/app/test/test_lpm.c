...
/*
 * rte_lpm_rlw_qsbr_add positive and negative tests.
 *  - Add RLW QSBR variable to LPM
 *  - Add another RLW QSBR variable to LPM
 *  - Check returns
 */
int32_t
test19(void)
{
	...
	struct rte_lpm_config config;
	size_t sz;
	struct rte_rlw_qsbr *qsv;
	struct rte_rlw_qsbr *qsv2;
	int32_t status;
	struct rte_lpm_rlw_config rlw_cfg = {0};
	...
	qsv = (struct rte_rlw_qsbr *)rte_zmalloc_socket(NULL, sz,
					RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
	TEST_LPM_ASSERT(qsv != NULL);

	status = rte_rlw_qsbr_init(qsv, RTE_MAX_LCORE);
	TEST_LPM_ASSERT(status == 0);

	rlw_cfg.v = qsv;
	/* Invalid QSBR mode */
	rlw_cfg.mode = 2;
	status = rte_lpm_rlw_qsbr_add(lpm, &rlw_cfg);
	TEST_LPM_ASSERT(status != 0);

	rlw_cfg.mode = RTE_LPM_QSBR_MODE_DQ;
	/* Attach RLW QSBR to LPM table */
	status = rte_lpm_rlw_qsbr_add(lpm, &rlw_cfg);
	TEST_LPM_ASSERT(status == 0);

	/* Create and attach another RLW QSBR to LPM table */
	qsv2 = (struct rte_rlw_qsbr *)rte_zmalloc_socket(NULL, sz,
					RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
	TEST_LPM_ASSERT(qsv2 != NULL);

	rlw_cfg.v = qsv2;
	rlw_cfg.mode = RTE_LPM_QSBR_MODE_SYNC;
	status = rte_lpm_rlw_qsbr_add(lpm, &rlw_cfg);
	TEST_LPM_ASSERT(status != 0);
...
/*
 * rte_lpm_rlw_qsbr_add DQ mode functional test.
...
test20(void)
{
	...
	/* Create RLW QSBR variable */
	sz = rte_rlw_qsbr_get_memsize(1);
	qsv = (struct rte_rlw_qsbr *)rte_zmalloc_socket(NULL, sz,
				RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
	TEST_LPM_ASSERT(qsv != NULL);

	status = rte_rlw_qsbr_init(qsv, 1);
	TEST_LPM_ASSERT(status == 0);
	...
	status = rte_rlw_qsbr_thread_register(qsv, 0);
	TEST_LPM_ASSERT(status == 0);
	rte_rlw_qsbr_thread_online(qsv, 0);
	...
	/* Reader quiescent */
	rte_rlw_qsbr_quiescent(qsv, 0);

	status = rte_lpm_add(lpm, ip, depth, next_hop);
	TEST_LPM_ASSERT(status == 0);

	rte_rlw_qsbr_thread_offline(qsv, 0);
	status = rte_rlw_qsbr_thread_unregister(qsv, 0);
	TEST_LPM_ASSERT(status == 0);
...
static struct rte_rlw_qsbr *g_v;
...
/*
 * Reader thread using rte_lpm data structure with RLW.
 */
static int
test_lpm_rlw_qsbr_reader(void *arg)
{
	int i;
	uint32_t next_hop_return = 0;
	...
	rte_rlw_qsbr_thread_register(g_v, 0);
	rte_rlw_qsbr_thread_online(g_v, 0);
	...
	do {
		...
		rte_rlw_qsbr_quiescent(g_v, 0);
	} while (!writer_done);

	rte_rlw_qsbr_thread_offline(g_v, 0);
	rte_rlw_qsbr_thread_unregister(g_v, 0);

	return 0;
}

/*
 * rte_lpm_rlw_qsbr_add sync mode functional test.
 * 1 Reader and 1 writer. They cannot be in the same thread in this test.
 *  - Create LPM which supports 1 tbl8 group at max
 *  - Add RLW QSBR variable with sync mode to LPM
...
test21(void)
{
	...
	/* Create RLW QSBR variable */
	sz = rte_rlw_qsbr_get_memsize(1);
	g_v = (struct rte_rlw_qsbr *)rte_zmalloc_socket(NULL, sz,
				RTE_CACHE_LINE_SIZE, SOCKET_ID_ANY);
	TEST_LPM_ASSERT(g_v != NULL);

	status = rte_rlw_qsbr_init(g_v, 1);
	TEST_LPM_ASSERT(status == 0);

	rlw_cfg.v = g_v;
	rlw_cfg.mode = RTE_LPM_QSBR_MODE_SYNC;
	/* Attach RLW QSBR to LPM table */
	status = rte_lpm_rlw_qsbr_add(g_lpm, &rlw_cfg);
	TEST_LPM_ASSERT(status == 0);

	writer_done = 0;
	/* Launch reader thread */
	rte_eal_remote_launch(test_lpm_rlw_qsbr_reader, NULL,
