...
static struct link_bonding_unittest_params test_params  = {
	.bonded_port_id = ILWALID_PORT_ID,
	.slave_ports = { [0 ... SLAVE_COUNT - 1] = { .port_id = ILWALID_PORT_ID} },
...
/* Macro for iterating over every port that is lwrrently a slave of a bonded
 * device.
 * _i variable used as an index in test_params->slave_ports
 * _slave pointer to &test_params->slave_ports[_idx]
 * */
...
add_slave(struct slave_conf *slave, uint8_t start)
{
	...
	RTE_VERIFY(slave->port_id != ILWALID_PORT_ID);
...
initialize_bonded_device_with_slaves(uint16_t slave_count, uint8_t external_sm)
{
	...
	RTE_VERIFY(test_params.bonded_port_id != ILWALID_PORT_ID);
	...
	TEST_ASSERT_SUCCESS(ret,
		"Failed disable promislwous mode for port %d: %s",
...
test_setup(void)
{
	...
	FOR_EACH_PORT(i, port) {
		...
		if (port->port_id == ILWALID_PORT_ID) {
	...
	if (test_params.bonded_port_id == ILWALID_PORT_ID) {
...
test_mode4_rx(void)
{
	...
	/* Now, disable promislwous mode. When promislwous mode is disabled we
	 * expect to receive only packets that are directed to bonding port. */
	retval = rte_eth_promislwous_disable(test_params.bonded_port_id);
	TEST_ASSERT_SUCCESS(retval,
	...
	uint8_t slave_down_id = ILWALID_PORT_ID;
	...
	RTE_VERIFY(slave_down_id != ILWALID_PORT_ID);
...
check_elwironment(void)
{
	...
	TEST_ASSERT_EQUAL(elw_state, 0,
		"Environment not clean (port %u):%s%s%s%s%s",
		port->port_id,
		elw_state & 0x01 ? " slave rx queue not clean" : "",
		elw_state & 0x02 ? " slave tx queue not clean" : "",
		elw_state & 0x04 ? " port marked as enslaved" : "",
		elw_state & 0x80 ? " slave state is not reset" : "",
		elw_state & 0x10 ? " slave count not equal 0" : ".");
...
static int
test_mode4_exelwtor(int (*test_func)(void))
{
	struct slave_conf *port;
	int test_result;
	uint8_t i;
	...
	TEST_ASSERT_SUCCESS(check_elwironment(),
...
test_mode4_agg_mode_selection_wrapper(void){
	return test_mode4_exelwtor(&test_mode4_agg_mode_selection);
}

static int
test_mode4_lacp_wrapper(void)
{
	return test_mode4_exelwtor(&test_mode4_lacp);
}

static int
test_mode4_marker_wrapper(void)
{
	return test_mode4_exelwtor(&test_mode4_marker);
}

static int
test_mode4_rx_wrapper(void)
{
	return test_mode4_exelwtor(&test_mode4_rx);
}

static int
test_mode4_tx_burst_wrapper(void)
{
	return test_mode4_exelwtor(&test_mode4_tx_burst);
}

static int
test_mode4_expired_wrapper(void)
{
	return test_mode4_exelwtor(&test_mode4_expired);
}

static int
test_mode4_ext_ctrl_wrapper(void)
{
	return test_mode4_exelwtor(&test_mode4_ext_ctrl);
}

static int
test_mode4_ext_lacp_wrapper(void)
{
	return test_mode4_exelwtor(&test_mode4_ext_lacp);
}
