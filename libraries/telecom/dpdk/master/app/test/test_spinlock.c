...
static rte_spinlock_relwrsive_t slr;
...
static int
test_spinlock_relwrsive_per_core(__rte_unused void *arg)
{
	unsigned id = rte_lcore_id();

	rte_spinlock_relwrsive_lock(&slr);
	printf("Global relwrsive lock taken on core %u - count = %d\n",
	       id, slr.count);
	rte_spinlock_relwrsive_lock(&slr);
	printf("Global relwrsive lock taken on core %u - count = %d\n",
	       id, slr.count);
	...
	printf("Hello from within relwrsive locks from core %u !\n", id);

	rte_spinlock_relwrsive_unlock(&slr);
	printf("Global relwrsive lock released on core %u - count = %d\n",
	       id, slr.count);
	rte_spinlock_relwrsive_unlock(&slr);
	printf("Global relwrsive lock released on core %u - count = %d\n",
	       id, slr.count);
...
test_spinlock(void)
{
	...
	rte_spinlock_relwrsive_init(&slr);
	 ...
	 */
	if(!rte_spinlock_relwrsive_trylock(&slr)) {
		printf("rte_spinlock_relwrsive_trylock failed on a lock that "
		       "we already own\n");
		ret = -1;
	} else
		rte_spinlock_relwrsive_unlock(&slr);
	...
	RTE_LCORE_FOREACH_WORKER(i) {
		rte_eal_remote_launch(test_spinlock_relwrsive_per_core, NULL, i);
	}
	rte_spinlock_relwrsive_unlock(&slr);
	rte_eal_mp_wait_lcore();
	...
	/*
	 * Test if it can trylock relwrsively.
	 * Use rte_spinlock_relwrsive_trylock() to check if it can lock a spinlock
	 * object relwrsively. Here it will try to lock a spinlock object twice.
	 */
	if (rte_spinlock_relwrsive_trylock(&slr) == 0) {
		printf("It failed to do the first spinlock_relwrsive_trylock but it should able to do\n");
		return -1;
	}
	if (rte_spinlock_relwrsive_trylock(&slr) == 0) {
		printf("It failed to do the second spinlock_relwrsive_trylock but it should able to do\n");
		return -1;
	}
	rte_spinlock_relwrsive_unlock(&slr);
	rte_spinlock_relwrsive_unlock(&slr);
