...
#define TEST_POWER_VM_LCORE_ILWALID       1U
...
test_power_kvm_vm(void)
{
	...
	ret = rte_power_set_elw(PM_ELW_KVM_VM);
	if (ret != 0) {
		printf("Failed on setting environment to PM_ELW_KVM_VM\n");
		return -1;
	}

	/* Test environment configuration */
	elw = rte_power_get_elw();
	if (elw != PM_ELW_KVM_VM) {
	...
	ret = rte_power_freq_up(TEST_POWER_VM_LCORE_ILWALID);
	if (ret == 1) {
		printf("rte_power_freq_up unexpectedly succeeded on invalid lcore %u\n",
				TEST_POWER_VM_LCORE_ILWALID);
		goto fail_all;
	}

	/* Test frequency down of valid but uninitialised lcore */
	ret = rte_power_freq_down(TEST_POWER_VM_LCORE_ILWALID);
	if (ret == 1) {
		printf("rte_power_freq_down unexpectedly succeeded on invalid lcore "
				"%u\n", TEST_POWER_VM_LCORE_ILWALID);
		goto fail_all;
	...
	ret = rte_power_freq_min(TEST_POWER_VM_LCORE_ILWALID);
	if (ret == 1) {
		printf("rte_power_freq_min unexpectedly succeeded on invalid lcore "
				"%u\n", TEST_POWER_VM_LCORE_ILWALID);
		goto fail_all;
	...
	ret = rte_power_freq_max(TEST_POWER_VM_LCORE_ILWALID);
	if (ret == 1) {
		printf("rte_power_freq_max unexpectedly succeeded on invalid lcore "
				"%u\n", TEST_POWER_VM_LCORE_ILWALID);
		goto fail_all;
