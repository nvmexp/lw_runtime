...
#define TOTAL_WRITES (RLW_ITERATIONS * NUM_LDEPTH_ROUTE_ENTRIES)
...
/*
 * Reader thread using rte_lpm data structure without RLW.
 */
static int
...
test_lpm_rlw_qsbr_reader(void *arg)
{
	...
	rte_rlw_qsbr_thread_register(rv, thread_id);
	rte_rlw_qsbr_thread_online(rv, thread_id);
	...
	do {
		...
		rte_rlw_qsbr_quiescent(rv, thread_id);
	} while (!writer_done);

	rte_rlw_qsbr_thread_offline(rv, thread_id);
	rte_rlw_qsbr_thread_unregister(rv, thread_id);

	return 0;
}

/*
 * Writer thread using rte_lpm data structure with RLW.
 */
static int
test_lpm_rlw_qsbr_writer(void *arg)
{
 ...
 */
static int
test_lpm_rlw_perf_multi_writer(uint8_t use_rlw)
{
	...
	if (rte_lcore_count() < 3) {
		printf("Not enough cores for lpm_rlw_perf_autotest, expecting at least 3\n");
		return TEST_SKIPPED;
	}
	...
	for (j = 1; j < 3; j++) {
		if (use_rlw)
			printf("\nPerf test: %d writer(s), %d reader(s),"
			       " RLW integration enabled\n", j, num_cores - j);
		else
			printf("\nPerf test: %d writer(s), %d reader(s),"
			       " RLW integration disabled\n", j, num_cores - j);
		...
		/* Init RLW variable */
		if (use_rlw) {
			sz = rte_rlw_qsbr_get_memsize(num_cores);
			rv = (struct rte_rlw_qsbr *)rte_zmalloc("rlw0", sz,
							RTE_CACHE_LINE_SIZE);
			rte_rlw_qsbr_init(rv, num_cores);

			rlw_cfg.v = rv;
			/* Assign the RLW variable to LPM */
			if (rte_lpm_rlw_qsbr_add(lpm, &rlw_cfg) != 0) {
				printf("RLW variable assignment failed\n");
				goto error;
			}

			reader_f = test_lpm_rlw_qsbr_reader;
		} else
		...
		for (i = 0; i < j; i++)
			rte_eal_remote_launch(test_lpm_rlw_qsbr_writer,
						(void *)(uintptr_t)i,
						enabled_core_ids[i]);
...
test_lpm_perf(void)
{
	...
	if (test_lpm_rlw_perf_multi_writer(0) < 0)
		return -1;

	if (test_lpm_rlw_perf_multi_writer(1) < 0)
		return -1;
