...
const char * ether_addr_ilwalid_strs[] = {
...
int
test_parse_etheraddr_ilwalid_param(void)
{
...
int
test_parse_etheraddr_ilwalid_data(void)
{
	int ret = 0;
	unsigned i;
	struct rte_ether_addr result;
	...
	/* test full strings */
	for (i = 0; i < RTE_DIM(ether_addr_ilwalid_strs); i++) {

		memset(&result, 0, sizeof(struct rte_ether_addr));

		ret = cmdline_parse_etheraddr(NULL, ether_addr_ilwalid_strs[i],
			(void*)&result, sizeof(result));
		if (ret != -1) {
			printf("Error: parsing %s succeeded!\n",
					ether_addr_ilwalid_strs[i]);
			return -1;
		}
	}
