...
#include "test_cryptodev_selwrity_pdcp_test_vectors.h"
#include "test_cryptodev_selwrity_pdcp_sdap_test_vectors.h"
#include "test_cryptodev_selwrity_pdcp_test_func.h"
#include "test_cryptodev_selwrity_docsis_test_vectors.h"
...
static enum rte_selwrity_session_action_type gbl_action_type =
...
#ifdef RTE_LIB_SELWRITY
	struct rte_selwrity_docsis_xform docsis_xform;
#endif

	union {
		struct rte_cryptodev_sym_session *sess;
#ifdef RTE_LIB_SELWRITY
		struct rte_selwrity_session *sec_session;
#endif
	};
#ifdef RTE_LIB_SELWRITY
	enum rte_selwrity_session_action_type type;
#endif
...
process_crypto_request(uint8_t dev_id, struct rte_crypto_op *op)
{
	...
	RTE_VERIFY(gbl_action_type != RTE_SELWRITY_ACTION_TYPE_CPU_CRYPTO);
...
testsuite_setup(void)
{
	...
	ts_params->conf.ff_disable = RTE_CRYPTODEV_FF_SELWRITY;

	unsigned int session_size =
		rte_cryptodev_sym_get_private_session_size(dev_id);

#ifdef RTE_LIB_SELWRITY
	unsigned int selwrity_session_size = rte_selwrity_session_get_size(
			rte_cryptodev_get_sec_ctx(dev_id));

	if (session_size < selwrity_session_size)
		session_size = selwrity_session_size;
#endif
	/*
...
ut_setup(void)
{
	...
	return dev_configure_and_start(RTE_CRYPTODEV_FF_SELWRITY);
}

static int
ut_setup_selwrity(void)
{
...
#ifdef RTE_LIB_SELWRITY
	if (ut_params->type == RTE_SELWRITY_ACTION_TYPE_LOOKASIDE_PROTOCOL) {
		if (ut_params->sec_session) {
			rte_selwrity_session_destroy(rte_cryptodev_get_sec_ctx
						(ts_params->valid_devs[0]),
...
static int
test_device_configure_ilwalid_dev_id(void)
{
	struct crypto_testsuite_params *ts_params = &testsuite_params;
...
static int
test_device_configure_ilwalid_queue_pair_ids(void)
{
	struct crypto_testsuite_params *ts_params = &testsuite_params;
...
test_AES_CBC_HMAC_SHA1_encrypt_digest(void)
{
	...
	if (gbl_action_type == RTE_SELWRITY_ACTION_TYPE_CPU_CRYPTO)
...
#ifdef RTE_LIB_SELWRITY
static int
selwrity_proto_supported(enum rte_selwrity_session_action_type action,
	enum rte_selwrity_session_protocol proto)
{
	struct crypto_testsuite_params *ts_params = &testsuite_params;
	...
	capabilities = rte_selwrity_capabilities_get(ctx);
...
static int test_pdcp_proto(int i, int oop, enum rte_crypto_cipher_operation opc,
			   ...
			   uint8_t bearer, enum rte_selwrity_pdcp_domain domain,
			   ...
			   uint32_t hfn, uint32_t hfn_threshold, uint8_t sdap)
{
	...
	/* Verify the capabilities */
	struct rte_selwrity_capability_idx sec_cap_idx;

	sec_cap_idx.action = ut_params->type;
	sec_cap_idx.protocol = RTE_SELWRITY_PROTOCOL_PDCP;
	sec_cap_idx.pdcp.domain = domain;
	if (rte_selwrity_capability_get(ctx, &sec_cap_idx) == NULL)
		return -ENOTSUP;
	...
	ut_params->sec_session = rte_selwrity_session_create(ctx,
	...
	*per_pkt_hfn = packet_direction ? hfn : 0;
	...
	rte_selwrity_attach_session(ut_params->op, ut_params->sec_session);
...
on_err:
	...
	if (ut_params->sec_session)
		rte_selwrity_session_destroy(ctx, ut_params->sec_session);
	ut_params->sec_session = NULL;
...
test_PDCP_SDAP_PROTO_encap_all(void)
{
	...
	const struct pdcp_sdap_test *lwr_test;

	size = ARRAY_SIZE(list_pdcp_sdap_tests);

	for (i = 0; i < size; i++) {
		lwr_test = &list_pdcp_sdap_tests[i];
		err = test_pdcp_proto(
			i, 0, RTE_CRYPTO_CIPHER_OP_ENCRYPT,
			RTE_CRYPTO_AUTH_OP_GENERATE, lwr_test->data_in,
			lwr_test->in_len, lwr_test->data_out,
			lwr_test->in_len + ((lwr_test->auth_key) ? 4 : 0),
			lwr_test->param.cipher_alg, lwr_test->cipher_key,
			lwr_test->param.cipher_key_len,
			lwr_test->param.auth_alg,
			lwr_test->auth_key, lwr_test->param.auth_key_len,
			lwr_test->bearer, lwr_test->param.domain,
			lwr_test->packet_direction, lwr_test->sn_size,
			lwr_test->hfn,
			lwr_test->hfn_threshold, SDAP_ENABLED);
		if (err) {
...
test_PDCP_SDAP_PROTO_decap_all(void)
{
	...
	for (i = 0; i < size; i++) {
		...
		err = test_pdcp_proto(
			...
			lwr_test->data_out,
			lwr_test->in_len + ((lwr_test->auth_key) ? 4 : 0),
			lwr_test->data_in, lwr_test->in_len,
			lwr_test->param.cipher_alg,
			lwr_test->cipher_key, lwr_test->param.cipher_key_len,
			lwr_test->param.auth_alg, lwr_test->auth_key,
			lwr_test->param.auth_key_len, lwr_test->bearer,
			lwr_test->param.domain, lwr_test->packet_direction,
			lwr_test->sn_size, lwr_test->hfn,
			lwr_test->hfn_threshold, SDAP_ENABLED);
		if (err) {
...
test_PDCP_PROTO_all(void)
{
	...
	if (!(feat_flags & RTE_CRYPTODEV_FF_SELWRITY))
		return -ENOTSUP;

	/* Set action type */
	ut_params->type = gbl_action_type == RTE_SELWRITY_ACTION_TYPE_NONE ?
		RTE_SELWRITY_ACTION_TYPE_LOOKASIDE_PROTOCOL :
		gbl_action_type;

	if (selwrity_proto_supported(ut_params->type,
			RTE_SELWRITY_PROTOCOL_PDCP) < 0)
		return -ENOTSUP;
...
test_docsis_proto_uplink(int i, struct docsis_test_data *d_td)
{
	...
	/* Verify the capabilities */
	struct rte_selwrity_capability_idx sec_cap_idx;
	const struct rte_selwrity_capability *sec_cap;
	const struct rte_cryptodev_capabilities *crypto_cap;
	const struct rte_cryptodev_symmetric_capability *sym_cap;
	int j = 0;

	sec_cap_idx.action = ut_params->type;
	sec_cap_idx.protocol = RTE_SELWRITY_PROTOCOL_DOCSIS;
	sec_cap_idx.docsis.direction = RTE_SELWRITY_DOCSIS_UPLINK;

	sec_cap = rte_selwrity_capability_get(ctx, &sec_cap_idx);
	if (sec_cap == NULL)
		return -ENOTSUP;
	...
	ut_params->docsis_xform.direction = RTE_SELWRITY_DOCSIS_UPLINK;
	...
	/* Create security session */
	ut_params->sec_session = rte_selwrity_session_create(ctx, &sess_conf,
					ts_params->session_mpool,
					ts_params->session_priv_mpool);
...
test_docsis_proto_downlink(int i, struct docsis_test_data *d_td)
{
	...
	sec_cap_idx.action = ut_params->type;
	sec_cap_idx.protocol = RTE_SELWRITY_PROTOCOL_DOCSIS;
	sec_cap_idx.docsis.direction = RTE_SELWRITY_DOCSIS_DOWNLINK;
	...
	/* Setup DOCSIS session parameters */
	ut_params->docsis_xform.direction = RTE_SELWRITY_DOCSIS_DOWNLINK;
...
test_DOCSIS_PROTO_all(void)
{
	...
	if (selwrity_proto_supported(ut_params->type,
			RTE_SELWRITY_PROTOCOL_DOCSIS) < 0)
		return -ENOTSUP;
...
static int
test_null_ilwalid_operation(void)
{
	struct crypto_testsuite_params *ts_params = &testsuite_params;
	struct crypto_unittest_params *ut_params = &unittest_params;
	int ret;
		...
		const int oop, uint32_t fragsz, uint32_t fragsz_oop)
{
	...
	if (oop && gbl_action_type == RTE_SELWRITY_ACTION_TYPE_CPU_CRYPTO)
		return -ENOTSUP;
	...
	/* Process crypto operation */
	if (oop == IN_PLACE &&
			gbl_action_type == RTE_SELWRITY_ACTION_TYPE_CPU_CRYPTO)
		process_cpu_aead_op(ts_params->valid_devs[0], ut_params->op);
	else if (global_api_test_type == CRYPTODEV_RAW_API_TEST)
		process_sym_raw_dp_op(ts_params->valid_devs[0], 0,
				ut_params->op, 0, 0, 0, 0);
...
static struct unit_test_suite cryptodev_testsuite  = {
	...
	.unit_test_cases = {
		TEST_CASE_ST(ut_setup, ut_teardown,
				test_device_configure_ilwalid_dev_id),
		TEST_CASE_ST(ut_setup, ut_teardown,
				test_queue_pair_descriptor_setup),
		TEST_CASE_ST(ut_setup, ut_teardown,
				test_device_configure_ilwalid_queue_pair_ids),
...
#ifdef RTE_LIB_SELWRITY
		TEST_CASE_ST(ut_setup_selwrity, ut_teardown,
			test_PDCP_PROTO_all),
		TEST_CASE_ST(ut_setup_selwrity, ut_teardown,
...
{
	int32_t rc;
	enum rte_selwrity_session_action_type at;
