 ...
 *
 *   - Ilwoke ``test_atomic_tas()`` on each lcore: before doing anything
 ...
 *   - Initialize 16-bit, 32-bit and 64-bit atomic variables to zero.
 *
 *   - Ilwoke ``test_atomic_addsub_return()`` on each lcore. Before doing
 ...
 *
 *   - Ilwoke ``test_atomic128_cmp_exchange()`` on each lcore. Before doing
 *     anything else, the cores are waiting a synchro. Each lcore does
 ...
 *
 *   - Ilwoke ``test_atomic_exchange`` on each lcore.  Before doing
