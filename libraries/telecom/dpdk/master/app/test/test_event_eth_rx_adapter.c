...
testsuite_setup(void)
{
	 ...
	 * so rte_eth_dev_start ilwokes rte_event_dev_start internally, so
...
adapter_create_free(void)
{
	...
	err = rte_event_eth_rx_adapter_create(TEST_INST_ID, TEST_DEV_ID,
					NULL);
	TEST_ASSERT(err == -EILWAL, "Expected -EILWAL got %d", err);
	...
	err = rte_event_eth_rx_adapter_free(TEST_INST_ID);
	TEST_ASSERT(err == -EILWAL, "Expected -EILWAL %d got %d", -EILWAL, err);
