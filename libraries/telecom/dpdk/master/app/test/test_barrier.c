 ...
 */
  ...
  * for two exelwtion units to make sure that rte_smp_mb() prevents
  * store-load reordering to happen.
  * Also when exelwted on a single lcore could be used as a approxiamate
...
plock_test(uint64_t iter, enum plock_use_type utype)
{
	...
	for (i = 0; i != n; i++) {
		...
		/* race condition oclwrred, lock doesn't work properly */
