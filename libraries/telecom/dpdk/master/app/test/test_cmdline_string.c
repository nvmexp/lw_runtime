...
struct string_ilwalid_str {
	const char * str;	/* parsed string */
	const char * fixed_str;	/* parsing mode (any, fixed or multi) */
};

struct string_ilwalid_str string_ilwalid_strs[] = {
		{"invalid", "one"},	/* fixed string */
		{"invalid", "one#two#three"},	/* multiple choice string */
		{"invalid", "ilwalidone"},	/* string that starts the same */
		{"ilwalidone", "invalid"},	/* string that starts the same */
...
int
test_parse_string_ilwalid_param(void)
{
...
int
test_parse_string_ilwalid_data(void)
{
	cmdline_parse_token_string_t token;
	...
	for (i = 0; i < RTE_DIM(string_ilwalid_strs); i++) {
		memset(&token, 0, sizeof(token));
		memset(buf, 0, sizeof(buf));

		/* prepare test token data */
		token.string_data.str = string_ilwalid_strs[i].fixed_str;

		if (cmdline_parse_string((cmdline_parse_token_hdr_t*)&token,
				string_ilwalid_strs[i].str, (void*)buf,
				sizeof(buf)) != -1) {
			memset(help_str, 0, sizeof(help_str));
			memset(&help_token, 0, sizeof(help_token));

			help_token.string_data.str = string_ilwalid_strs[i].fixed_str;
			...
			printf("Error: parsing %s as %s succeeded!\n",
					string_ilwalid_strs[i].str, help_str);
			return -1;
		}
	}
