...
{
	struct rte_ring *r;
	void **src = NULL, **lwr_src = NULL, **dst = NULL, **lwr_dst = NULL;
	int ret;
	...
	for (i = 0; i < RTE_DIM(esize); i++) {
		...
		for (j = 0; j != TEST_RING_FULL_EMPTY_ITER; j++) {
			...
			printf("%s: iteration %u, random shift: %u;\n",
			    __func__, i, rand);
			ret = test_ring_enq_impl(r, lwr_src, esize[i], rand,
							test_idx);
			TEST_RING_VERIFY(ret != 0, r, goto fail);

			ret = test_ring_deq_impl(r, lwr_dst, esize[i], rand,
							test_idx);
			TEST_RING_VERIFY(ret == rand, r, goto fail);

			/* fill the ring */
			ret = test_ring_enq_impl(r, lwr_src, esize[i], rsz,
							test_idx);
			TEST_RING_VERIFY(ret != 0, r, goto fail);
			...
			/* empty the ring */
			ret = test_ring_deq_impl(r, lwr_dst, esize[i], rsz,
							test_idx);
...
test_ring_burst_bulk_tests2(unsigned int test_idx)
{
	...
	for (i = 0; i < RTE_DIM(esize); i++) {
		...
		ret = test_ring_enq_impl(r, lwr_src, esize[i], 1, test_idx);
		TEST_RING_VERIFY(ret == 1, r, goto fail);
		lwr_src = test_ring_inc_ptr(lwr_src, esize[i], 1);

		printf("enqueue 2 objs\n");
		ret = test_ring_enq_impl(r, lwr_src, esize[i], 2, test_idx);
		TEST_RING_VERIFY(ret == 2, r, goto fail);
		lwr_src = test_ring_inc_ptr(lwr_src, esize[i], 2);

		printf("enqueue MAX_BULK objs\n");
		ret = test_ring_enq_impl(r, lwr_src, esize[i], MAX_BULK,
						test_idx);
		TEST_RING_VERIFY(ret == MAX_BULK, r, goto fail);

		printf("dequeue 1 obj\n");
		ret = test_ring_deq_impl(r, lwr_dst, esize[i], 1, test_idx);
		TEST_RING_VERIFY(ret == 1, r, goto fail);
		lwr_dst = test_ring_inc_ptr(lwr_dst, esize[i], 1);

		printf("dequeue 2 objs\n");
		ret = test_ring_deq_impl(r, lwr_dst, esize[i], 2, test_idx);
		TEST_RING_VERIFY(ret == 2, r, goto fail);
		lwr_dst = test_ring_inc_ptr(lwr_dst, esize[i], 2);

		printf("dequeue MAX_BULK objs\n");
		ret = test_ring_deq_impl(r, lwr_dst, esize[i], MAX_BULK,
						test_idx);
		TEST_RING_VERIFY(ret == MAX_BULK, r, goto fail);
		lwr_dst = test_ring_inc_ptr(lwr_dst, esize[i], MAX_BULK);

		/* check data */
		TEST_RING_VERIFY(test_ring_mem_cmp(src, dst,
					RTE_PTR_DIFF(lwr_dst, dst)) == 0,
					r, goto fail);
	...
	for (i = 0; i < RTE_DIM(esize); i++) {
		...
		for (j = 0; j < RING_SIZE / MAX_BULK; j++) {
			ret = test_ring_enq_impl(r, lwr_src, esize[i], MAX_BULK,
							test_idx);
			TEST_RING_VERIFY(ret == MAX_BULK, r, goto fail);
			lwr_src = test_ring_inc_ptr(lwr_src, esize[i],
								MAX_BULK);

			ret = test_ring_deq_impl(r, lwr_dst, esize[i], MAX_BULK,
							test_idx);
			TEST_RING_VERIFY(ret == MAX_BULK, r, goto fail);
			lwr_dst = test_ring_inc_ptr(lwr_dst, esize[i],
								MAX_BULK);
		}
...
test_ring_burst_bulk_tests4(unsigned int test_idx)
{
	...
	for (i = 0; i < RTE_DIM(esize); i++) {
		...
		ret = test_ring_enq_impl(r, lwr_src, esize[i], num_elems,
						test_idx);
		TEST_RING_VERIFY(ret == MAX_BULK - 3, r, goto fail);
		lwr_src = test_ring_inc_ptr(lwr_src, esize[i], MAX_BULK - 3);
		...
		if ((api_type & TEST_RING_ELEM_BULK) == TEST_RING_ELEM_BULK)
			...
			num_elems = MAX_BULK;
		ret = test_ring_deq_impl(r, lwr_dst, esize[i], num_elems,
						test_idx);
		TEST_RING_VERIFY(ret == MAX_BULK - 3, r, goto fail);
		lwr_dst = test_ring_inc_ptr(lwr_dst, esize[i], MAX_BULK - 3);
...
test_ring_basic_ex(void)
{
	...
	for (i = 0; i < RTE_DIM(esize); i++) {
		...
		for (j = 0; j < RING_SIZE - 1; j++) {
			ret = test_ring_enqueue(rp, lwr_src, esize[i], 1,
		...
		for (j = 0; j < RING_SIZE - 1; j++) {
			ret = test_ring_dequeue(rp, lwr_dst, esize[i], 1,
				TEST_RING_THREAD_DEF | TEST_RING_ELEM_SINGLE);
			TEST_RING_VERIFY(ret == 0, rp, goto fail_test);
			lwr_dst = test_ring_inc_ptr(lwr_dst, esize[i], 1);
		}
		 ...
		 */
		/* reset memory of dst */
		memset(dst, 0, RTE_PTR_DIFF(lwr_dst, dst));

		/* reset lwr_src and lwr_dst */
		lwr_src = src;
		lwr_dst = dst;

		/* Covering the ring burst operation */
		ret = test_ring_enqueue(rp, lwr_src, esize[i], 2,
				TEST_RING_THREAD_DEF | TEST_RING_ELEM_BURST);
		TEST_RING_VERIFY(ret == 2, rp, goto fail_test);
		lwr_src = test_ring_inc_ptr(lwr_src, esize[i], 2);

		ret = test_ring_dequeue(rp, lwr_dst, esize[i], 2,
				TEST_RING_THREAD_DEF | TEST_RING_ELEM_BURST);
		TEST_RING_VERIFY(ret == 2, rp, goto fail_test);
		lwr_dst = test_ring_inc_ptr(lwr_dst, esize[i], 2);
...
test_ring_with_exact_size(void)
{
	...
	for (i = 0; i < RTE_DIM(esize); i++) {
		...
		for (j = 0; j < ring_sz - 1; j++) {
			ret = test_ring_enqueue(std_r, lwr_src, esize[i], 1,
				TEST_RING_THREAD_DEF | TEST_RING_ELEM_SINGLE);
			TEST_RING_VERIFY(ret == 0, std_r, goto test_fail);
			ret = test_ring_enqueue(exact_sz_r, lwr_src, esize[i], 1,
				TEST_RING_THREAD_DEF | TEST_RING_ELEM_SINGLE);
		...
		ret = test_ring_dequeue(exact_sz_r, lwr_dst, esize[i], ring_sz,
				TEST_RING_THREAD_DEF | TEST_RING_ELEM_BURST);
		TEST_RING_VERIFY(ret == (int)ring_sz, exact_sz_r, goto test_fail);
		lwr_dst = test_ring_inc_ptr(lwr_dst, esize[i], ring_sz);
