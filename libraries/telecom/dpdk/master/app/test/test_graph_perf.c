...
graph_node_get(const char *pname, char *nname)
{
	...
	if (node_id != RTE_NODE_ID_ILWALID) {
	   ...
	   uint8_t burst_one)
{
	...
	for (i = 0; i < stages; i++) {
		...
		for (j = 0; j < nodes_per_stage; j++) {
			...
			if (node_map[i][j] == RTE_NODE_ID_ILWALID) {
	...
	for (i = 0; i < nb_srcs; i++) {
		...
		src_nodes[i] =
			graph_node_get(burst_one ? TEST_GRAPH_SRC_BRST_ONE_NAME
						 : TEST_GRAPH_SRC_NAME,
				       nname);
		if (src_nodes[i] == RTE_NODE_ID_ILWALID) {
			printf("Failed to create node[%s]\n", nname);
			graph_data->nb_nodes++;
			goto pattern_name_free;
		}
	...
	for (i = 0; i < nb_sinks; i++) {
		...
		if (snk_nodes[i] == RTE_NODE_ID_ILWALID) {
			printf("Failed to create node[%s]\n", nname);
			graph_data->nb_nodes++;
			goto pattern_name_free;
		}
	...
	if (graph_id == RTE_GRAPH_ID_ILWALID) {
