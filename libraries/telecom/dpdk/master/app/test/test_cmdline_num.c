...
int
test_parse_num_ilwalid_param(void)
{
...
int
test_parse_num_ilwalid_data(void)
{
...
test_parse_num_valid(void)
{
	...
	for (type = RTE_UINT8; type <= RTE_INT64; type++) {
		...
		for (i = 0; i < RTE_DIM(num_valid_positive_strs); i++) {
			 ...
			 * since unsigned numbers don't care about number of bits, we can just colwert
