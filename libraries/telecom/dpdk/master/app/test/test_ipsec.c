...
static int
dummy_sec_create(void *device, struct rte_selwrity_session_conf *conf,
	struct rte_selwrity_session *sess, struct rte_mempool *mp)
{
	RTE_SET_USED(device);
	RTE_SET_USED(conf);
	RTE_SET_USED(mp);
...
static int
dummy_sec_destroy(void *device, struct rte_selwrity_session *sess)
{
	RTE_SET_USED(device);
	RTE_SET_USED(sess);
	return 0;
...
static const struct rte_selwrity_ops dummy_sec_ops = {
	.session_create = dummy_sec_create,
	.session_destroy = dummy_sec_destroy,
};

static struct rte_selwrity_ctx dummy_sec_ctx = {
	.ops = &dummy_sec_ops,
};
...
testsuite_setup(void)
{
	...
	sess_sz = RTE_MAX(sess_sz, sizeof(struct rte_selwrity_session));
...
{
	static struct rte_selwrity_session_conf conf;

	ut->ss[j].security.ses = rte_selwrity_session_create(&dummy_sec_ctx,
					&conf, qp->mp_session,
...
{
	if (ut->ss[j].type == RTE_SELWRITY_ACTION_TYPE_NONE)
...
static int
create_sa(enum rte_selwrity_session_action_type action_type,
		uint32_t replay_win_sz, uint64_t flags, uint32_t j)
{
	...
	rc = (rc > 0 && (uint32_t)rc <= sz) ? 0 : -EILWAL;
	if (rc == 0)
...
lksd_proto_ipsec(uint16_t num_pkts)
{
	...
	for (i = 0; i != num_pkts; i++) {
		...
		TEST_ASSERT_EQUAL(ut_params->cop[i]->sess_type,
			RTE_CRYPTO_OP_SELWRITY_SESSION,
...
destroy_dummy_sec_session(struct ipsec_unitest_params *ut,
	uint32_t j)
{
	rte_selwrity_session_destroy(&dummy_sec_ctx,
...
test_ipsec_crypto_inb_burst_null_null(int i)
{
	...
	/* create rte_ipsec_sa */
	rc = create_sa(RTE_SELWRITY_ACTION_TYPE_NONE,
...
test_ipsec_crypto_inb_burst_null_null_wrapper(void)
{
	...
	ut_params->ipsec_xform.direction = RTE_SELWRITY_IPSEC_SA_DIR_INGRESS;
	ut_params->ipsec_xform.proto = RTE_SELWRITY_IPSEC_SA_PROTO_ESP;
	ut_params->ipsec_xform.mode = RTE_SELWRITY_IPSEC_SA_MODE_TUNNEL;
	ut_params->ipsec_xform.tunnel.type = RTE_SELWRITY_IPSEC_TUNNEL_IPV4;
...
test_ipsec_crypto_outb_burst_null_null_wrapper(void)
{
	...
	ut_params->ipsec_xform.direction = RTE_SELWRITY_IPSEC_SA_DIR_EGRESS;
	ut_params->ipsec_xform.proto = RTE_SELWRITY_IPSEC_SA_PROTO_ESP;
	ut_params->ipsec_xform.mode = RTE_SELWRITY_IPSEC_SA_MODE_TUNNEL;
	ut_params->ipsec_xform.tunnel.type = RTE_SELWRITY_IPSEC_TUNNEL_IPV4;
...
test_ipsec_inline_crypto_inb_burst_null_null(int i)
{
	...
	/* create rte_ipsec_sa*/
	rc = create_sa(RTE_SELWRITY_ACTION_TYPE_INLINE_CRYPTO,
			test_cfg[i].replay_win_sz, test_cfg[i].flags, 0);
	if (rc != 0) {
		RTE_LOG(ERR, USER1, "create_sa failed, cfg %d\n", i);
		return TEST_FAILED;
...
test_ipsec_inline_proto_inb_burst_null_null(int i)
{
	...
	/* create rte_ipsec_sa*/
	rc = create_sa(RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL,
			test_cfg[i].replay_win_sz, test_cfg[i].flags, 0);
	if (rc != 0) {
		RTE_LOG(ERR, USER1, "create_sa failed, cfg %d\n", i);
		return TEST_FAILED;
...
test_ipsec_lksd_proto_inb_burst_null_null(int i)
{
	...
	/* create rte_ipsec_sa */
	rc = create_sa(RTE_SELWRITY_ACTION_TYPE_LOOKASIDE_PROTOCOL,
			test_cfg[i].replay_win_sz, test_cfg[i].flags, 0);
	if (rc != 0) {
		RTE_LOG(ERR, USER1, "create_sa failed, cfg %d\n", i);
		return TEST_FAILED;
