...
test_table_hash_lwckoo(void)
{
	...
	/* Initialize params and create tables */
	struct rte_table_hash_lwckoo_params lwckoo_params = {
		.name = "TABLE",
		.key_size = 32,
		.key_offset = APP_METADATA_OFFSET(32),
		.key_mask = NULL,
		.n_keys = 1 << 16,
		.n_buckets = 1 << 16,
		.f_hash = pipeline_test_hash_lwckoo,
		.seed = 0,
	};

	table = rte_table_hash_lwckoo_ops.f_create(NULL, 0, entry_size);
	if (table != NULL)
		return -1;

	lwckoo_params.key_size = 0;

	table = rte_table_hash_lwckoo_ops.f_create(&lwckoo_params,
		0, entry_size);
	if (table != NULL)
		return -2;

	lwckoo_params.key_size = 32;
	lwckoo_params.n_keys = 0;
	...
	lwckoo_params.n_keys = 1 << 24;
	lwckoo_params.f_hash = NULL;
	...
	lwckoo_params.f_hash = pipeline_test_hash_lwckoo;
	lwckoo_params.name = NULL;
	...
	/* Free */
	status = rte_table_hash_lwckoo_ops.f_free(table);
	if (status < 0)
		return -7;

	status = rte_table_hash_lwckoo_ops.f_free(NULL);
	if (status == 0)
		return -8;

	/* Add */
	uint8_t key_lwckoo[32];
	uint32_t *klwckoo = (uint32_t *) &key_lwckoo;

	memset(key_lwckoo, 0, 32);
	klwckoo[0] = rte_be_to_cpu_32(0xadadadad);

	table = rte_table_hash_lwckoo_ops.f_create(&lwckoo_params, 0, 1);
	if (table == NULL)
		return -9;

	entry = 'A';
	status = rte_table_hash_lwckoo_ops.f_add(NULL, &key_lwckoo,
		&entry, &key_found, &entry_ptr);
	if (status == 0)
		return -10;

	status = rte_table_hash_lwckoo_ops.f_add(table, NULL, &entry,
		&key_found, &entry_ptr);
	if (status == 0)
		return -11;

	status = rte_table_hash_lwckoo_ops.f_add(table, &key_lwckoo,
	...
	/* Delete */
	status = rte_table_hash_lwckoo_ops.f_delete(NULL, &key_lwckoo,
		&key_found, NULL);
	if (status == 0)
		return -15;

	status = rte_table_hash_lwckoo_ops.f_delete(table, NULL,
		&key_found, NULL);
	if (status == 0)
		return -16;

	status = rte_table_hash_lwckoo_ops.f_delete(table, &key_lwckoo,
		&key_found, NULL);
	if (status != 0)
		return -17;
	...
	rte_table_hash_lwckoo_ops.f_lookup(table, mbufs, -1,
		&result_mask, (void **)entries);
	if (result_mask != expected_mask)
		return -20;
