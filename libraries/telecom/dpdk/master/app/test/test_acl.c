...
static void
acl_ipv4vlan_colwert_rule(const struct rte_acl_ipv4vlan_rule *ri,
	...
	uint32_t num)
{
	...
	/* perform colwersion to the internal format and add to the context. */
	for (i = 0, rc = 0; i != num && rc == 0; i++) {
		acl_ipv4vlan_colwert_rule(rules + i, &rv);
...
static void
colwert_rule(const struct rte_acl_ipv4vlan_rule *ri,
	struct acl_ipv4vlan_rule *ro)
{
	ro->data = ri->data;
...
/*
 * Colwert IPV4 source and destination from RTE_ACL_FIELD_TYPE_MASK to
 * RTE_ACL_FIELD_TYPE_BITMASK.
 */
static void
colwert_rule_1(const struct rte_acl_ipv4vlan_rule *ri,
	struct acl_ipv4vlan_rule *ro)
{
	uint32_t v;
 ...
 */
static void
colwert_rule_2(const struct rte_acl_ipv4vlan_rule *ri,
	struct acl_ipv4vlan_rule *ro)
{
	uint32_t hi, lo, mask;
...
/*
 * Colwert rte_acl_ipv4vlan_rule: swap VLAN and PORTS rule fields.
 */
static void
colwert_rule_3(const struct rte_acl_ipv4vlan_rule *ri,
	struct acl_ipv4vlan_rule *ro)
{
...
/*
 * Colwert rte_acl_ipv4vlan_rule: swap SRC and DST IPv4 address rules.
 */
static void
colwert_rule_4(const struct rte_acl_ipv4vlan_rule *ri,
	struct acl_ipv4vlan_rule *ro)
{
...
static int
colwert_rules(struct rte_acl_ctx *acx,
	void (*colwert)(const struct rte_acl_ipv4vlan_rule *,
...
static void
colwert_config(struct rte_acl_config *cfg)
{
	ipv4vlan_config(cfg, ipv4_7tuple_layout, RTE_ACL_MAX_CATEGORIES);
}

/*
 * Colwert rte_acl_ipv4vlan_rule to use RTE_ACL_FIELD_TYPE_BITMASK.
 */
static void
colwert_config_1(struct rte_acl_config *cfg)
{
	ipv4vlan_config(cfg, ipv4_7tuple_layout, RTE_ACL_MAX_CATEGORIES);
...
/*
 * Colwert rte_acl_ipv4vlan_rule to use RTE_ACL_FIELD_TYPE_RANGE.
 */
static void
colwert_config_2(struct rte_acl_config *cfg)
{
	ipv4vlan_config(cfg, ipv4_7tuple_layout, RTE_ACL_MAX_CATEGORIES);
...
/*
 * Colwert rte_acl_ipv4vlan_rule: swap VLAN and PORTS rule definitions.
 */
static void
colwert_config_3(struct rte_acl_config *cfg)
{
...
/*
 * Colwert rte_acl_ipv4vlan_rule: swap SRC and DST ip address rule definitions.
 */
static void
colwert_config_4(struct rte_acl_config *cfg)
{
...
static int
build_colwert_rules(struct rte_acl_ctx *acx,
...
static int
test_colwert_rules(const char *desc,
	void (*config)(struct rte_acl_config *),
	void (*colwert)(const struct rte_acl_ipv4vlan_rule *,
	struct acl_ipv4vlan_rule *))
{
	...
	rc = colwert_rules(acx, colwert, acl_test_rules,
		RTE_DIM(acl_test_rules));
	if (rc != 0)
		printf("Line %i: Error colwerting ACL rules!\n", __LINE__);

	for (i = 0; rc == 0 && i != RTE_DIM(mem_sizes); i++) {

		rc = build_colwert_rules(acx, config, mem_sizes[i]);
		if (rc != 0) {
			printf("Line %i: Error @ build_colwert_rules(%zu)!\n",
...
test_colwert(void)
{
	...
	for (i = 0; i != RTE_DIM(colwert_param); i++) {
		rc = test_colwert_rules(colwert_param[i].desc,
			colwert_param[i].config,
			colwert_param[i].colwert);
		if (rc != 0) {
			printf("%s for test-case: %s failed, error code: %d;\n",
				__func__, colwert_param[i].desc, rc);
			return rc;
		}
	}
 ...
 */
static int
test_ilwalid_layout(void)
{
	struct rte_acl_ctx *acx;
	int ret, i;

	uint32_t results[RTE_DIM(ilwalid_layout_data)];
	const uint8_t *data[RTE_DIM(ilwalid_layout_data)];
	...
	for (i = 0; i < 1000; i++) {
		/* add rules to the context */
		ret = rte_acl_ipv4vlan_add_rules(acx, ilwalid_layout_rules,
				RTE_DIM(ilwalid_layout_rules));
		if (ret != 0) {
			printf("Line %i: Adding rules to ACL context failed!\n",
				__LINE__);
			rte_acl_free(acx);
	...
	/* swap all bytes in the data to network order */
	bswap_test_data(ilwalid_layout_data, RTE_DIM(ilwalid_layout_data), 1);

	/* prepare data */
	for (i = 0; i < (int) RTE_DIM(ilwalid_layout_data); i++) {
		data[i] = (uint8_t *)&ilwalid_layout_data[i];
	}
	...
	/* swap data back to cpu order so that next time tests don't fail */
	bswap_test_data(ilwalid_layout_data, RTE_DIM(ilwalid_layout_data), 0);
...
test_ilwalid_rules(void)
{
	...
	/* test ilwerted high/low source and destination ports.
...
test_u32_range(void)
{
	...
	for (i = 0; i != RTE_DIM(acl_u32_range_test_rules); i++) {
		colwert_rule(&acl_u32_range_test_rules[i], &r);
		rc = rte_acl_add_rules(acx, (struct rte_acl_rule *)&r, 1);
		if (rc != 0) {
	...
	rc = build_colwert_rules(acx, colwert_config_2, 0);
	if (rc != 0) {
		printf("%s#%i Error @ build_colwert_rules!\n",
			__func__, __LINE__);
		rte_acl_free(acx);
		return rc;
	}
...
test_acl(void)
{
	if (test_ilwalid_parameters() < 0)
		return -1;
	if (test_ilwalid_rules() < 0)
		return -1;
	if (test_create_find_add() < 0)
		return -1;
	if (test_ilwalid_layout() < 0)
		return -1;
	if (test_misc() < 0)
		return -1;
	if (test_classify() < 0)
