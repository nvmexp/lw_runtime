...
test_cirbuf_string_misc(void)
{
	...
	/*
	 * initialize cirlwlar buffer
	 */
	if (cirbuf_init(&cb, buf, 0, sizeof(buf)) < 0) {
		printf("Error: failed to initialize cirlwlar buffer!\n");
		return -1;
	}
	...
	/*
	 * reinitialize cirlwlar buffer with start at the end of cirbuf
	 */
	if (cirbuf_init(&cb, buf, CMDLINE_TEST_BUFSIZE - 2, sizeof(buf)) < 0) {
		printf("Error: failed to reinitialize cirlwlar buffer!\n");
		return -1;
	}
...
int
test_cirbuf_ilwalid_param(void)
{
	struct cirbuf cb;
	char buf[CMDLINE_TEST_BUFSIZE];
