...
static rte_ticketlock_relwrsive_t tlr;
...
static int
test_ticketlock_relwrsive_per_core(__rte_unused void *arg)
{
	unsigned int id = rte_lcore_id();

	rte_ticketlock_relwrsive_lock(&tlr);
	...
	rte_ticketlock_relwrsive_unlock(&tlr);
...
test_ticketlock(void)
{
	...
	rte_ticketlock_relwrsive_init(&tlr);
	RTE_LCORE_FOREACH_WORKER(i) {
	 ...
	 */
	if (!rte_ticketlock_relwrsive_trylock(&tlr)) {
		printf("rte_ticketlock_relwrsive_trylock failed on a lock that "
		       "we already own\n");
		ret = -1;
	} else
		rte_ticketlock_relwrsive_unlock(&tlr);
	...
	RTE_LCORE_FOREACH_WORKER(i) {
		rte_eal_remote_launch(test_ticketlock_relwrsive_per_core,
					NULL, i);
	}
	rte_ticketlock_relwrsive_unlock(&tlr);
	rte_eal_mp_wait_lcore();
	 ...
	 * Use rte_ticketlock_relwrsive_trylock() to check if it can lock
	 * a ticketlock object relwrsively. Here it will try to lock a
	 * ticketlock object twice.
	 */
	if (rte_ticketlock_relwrsive_trylock(&tlr) == 0) {
		printf("It failed to do the first ticketlock_relwrsive_trylock "
			   "but it should able to do\n");
		return -1;
	}
	if (rte_ticketlock_relwrsive_trylock(&tlr) == 0) {
		printf("It failed to do the second ticketlock_relwrsive_trylock "
			   "but it should able to do\n");
		return -1;
	}
	rte_ticketlock_relwrsive_unlock(&tlr);
