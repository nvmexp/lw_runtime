...
test_get_rx_ol_flag_name(void)
{
	...
	const struct flag_name rx_flags[] = {
		...
		VAL_NAME(PKT_RX_OUTER_L4_CKSUM_ILWALID),
	};
 ...
 *  - Ilwoke pktmbuf_ext_shinfo_init_helper to initialize shared data.
 *  - Ilwoke rte_pktmbuf_attach_extbuf to attach external buffer to the mbuf.
 *  - Clone another mbuf and attach the same external buffer to it.
 *  - Ilwoke rte_pktmbuf_detach_extbuf to detach the external buffer from mbuf.
 */
static int
