...
/* invalid strings */
const char * portlist_ilwalid_strs[] = {
...
int
test_parse_portlist_ilwalid_param(void)
{
...
int
test_parse_portlist_ilwalid_data(void)
{
	int ret = 0;
	unsigned i;
	cmdline_portlist_t result;
	...
	for (i = 0; i < RTE_DIM(portlist_ilwalid_strs); i++) {

		memset(&result, 0, sizeof(cmdline_portlist_t));

		ret = cmdline_parse_portlist(NULL, portlist_ilwalid_strs[i],
			(void*)&result, sizeof(result));
		if (ret != -1) {
			printf("Error: parsing %s succeeded!\n",
					portlist_ilwalid_strs[i]);
			return -1;
		}
	}
