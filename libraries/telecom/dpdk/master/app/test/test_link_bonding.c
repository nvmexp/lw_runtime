...
#define ILWALID_SOCKET_ID		(-1)
#define ILWALID_PORT_ID			(-1)
#define ILWALID_BONDING_MODE	(-1)
...
test_create_bonded_device(void)
{
	...
	lwrrent_slave_count = rte_eth_bond_slaves_get(test_params->bonded_port_id,
			slaves, RTE_MAX_ETHPORTS);

	TEST_ASSERT_EQUAL(lwrrent_slave_count, 0,
			"Number of slaves %d is great than expected %d.",
			lwrrent_slave_count, 0);

	lwrrent_slave_count = rte_eth_bond_active_slaves_get(
...
static int
test_create_bonded_device_with_ilwalid_params(void)
{
	int port_id;
	...
	test_params->bonding_mode = ILWALID_BONDING_MODE;
...
test_add_slave_to_bonded_device(void)
{
	...
	lwrrent_slave_count = rte_eth_bond_slaves_get(test_params->bonded_port_id,
			slaves, RTE_MAX_ETHPORTS);
	TEST_ASSERT_EQUAL(lwrrent_slave_count, test_params->bonded_slave_count + 1,
			"Number of slaves (%d) is greater than expected (%d).",
			lwrrent_slave_count, test_params->bonded_slave_count + 1);
...
static int
test_add_slave_to_ilwalid_bonded_device(void)
{
	/* Invalid port ID */
...
static int
test_remove_slave_from_ilwalid_bonded_device(void)
{
	/* Invalid port ID */
...
{
	int port_id, lwrrent_slave_count;
	uint16_t slaves[RTE_MAX_ETHPORTS];
	char pmd_name[RTE_ETH_NAME_MAX_LEN];
	...
	lwrrent_slave_count = rte_eth_bond_slaves_get(test_params->bonded_port_id,
			slaves, RTE_MAX_ETHPORTS);
	TEST_ASSERT_EQUAL(lwrrent_slave_count, 1,
			"Number of slaves (%d) is not that expected (%d).",
			lwrrent_slave_count, 1);
...
test_get_slaves_from_bonded_device(void)
{
	...
	/* Invalid port id */
	lwrrent_slave_count = rte_eth_bond_slaves_get(ILWALID_PORT_ID, slaves,
			RTE_MAX_ETHPORTS);
	TEST_ASSERT(lwrrent_slave_count < 0,
			"Invalid port id unexpectedly succeeded");

	lwrrent_slave_count = rte_eth_bond_active_slaves_get(ILWALID_PORT_ID,
			slaves, RTE_MAX_ETHPORTS);
	TEST_ASSERT(lwrrent_slave_count < 0,
			"Invalid port id unexpectedly succeeded");
	...
	/* non bonded device*/
	lwrrent_slave_count = rte_eth_bond_slaves_get(
...
test_start_bonded_device(void)
{
	...
	int lwrrent_slave_count, lwrrent_bonding_mode, primary_port;
	uint16_t slaves[RTE_MAX_ETHPORTS];
	int retval;
	...
	lwrrent_slave_count = rte_eth_bond_slaves_get(test_params->bonded_port_id,
			slaves, RTE_MAX_ETHPORTS);
	TEST_ASSERT_EQUAL(lwrrent_slave_count, test_params->bonded_slave_count,
			"Number of slaves (%d) is not expected value (%d).",
			lwrrent_slave_count, test_params->bonded_slave_count);
	...
	lwrrent_bonding_mode = rte_eth_bond_mode_get(test_params->bonded_port_id);
	TEST_ASSERT_EQUAL(lwrrent_bonding_mode, test_params->bonding_mode,
			"Bonded device mode (%d) is not expected value (%d).\n",
			lwrrent_bonding_mode, test_params->bonding_mode);
...
test_set_bonding_mode(void)
{
	...
	for (i = 0; i < (int)RTE_DIM(bonding_modes);	i++) {
		/* Invalid port ID */
		TEST_ASSERT_FAIL(rte_eth_bond_mode_set(ILWALID_PORT_ID,
				bonding_modes[i]),
		...
		/* Invalid port ID */
		bonding_mode = rte_eth_bond_mode_get(ILWALID_PORT_ID);
...
test_set_primary_slave(void)
{
	...
	/* Invalid port ID */
	TEST_ASSERT_FAIL(rte_eth_bond_primary_set(ILWALID_PORT_ID,
...
test_set_explicit_bonded_mac(void)
{
	...
	/* Invalid port ID */
	TEST_ASSERT_FAIL(rte_eth_bond_mac_address_set(ILWALID_PORT_ID, mac_addr),
			"Expected call to failed as invalid port specified.");
...
lsc_timeout(int wait_us)
{
	...
	/* Colwert from timeval to timespec */
	ts.tv_sec = tp.tv_sec;
...
static int
test_roundrobin_verify_promislwous_enable_disable(void)
{
	int i, promislwous_en;
	int ret;
	...
	ret = rte_eth_promislwous_enable(test_params->bonded_port_id);
	TEST_ASSERT_SUCCESS(ret,
		"Failed to enable promislwous mode for port %d: %s",
		test_params->bonded_port_id, rte_strerror(-ret));

	promislwous_en = rte_eth_promislwous_get(test_params->bonded_port_id);
	TEST_ASSERT_EQUAL(promislwous_en, 1,
			"Port (%d) promislwous mode not enabled",
			test_params->bonded_port_id);

	for (i = 0; i < test_params->bonded_slave_count; i++) {
		promislwous_en = rte_eth_promislwous_get(
				test_params->slave_port_ids[i]);
		TEST_ASSERT_EQUAL(promislwous_en, 1,
				"slave port (%d) promislwous mode not enabled",
				test_params->slave_port_ids[i]);
	}

	ret = rte_eth_promislwous_disable(test_params->bonded_port_id);
	TEST_ASSERT_SUCCESS(ret,
		"Failed to disable promislwous mode for port %d: %s",
		test_params->bonded_port_id, rte_strerror(-ret));

	promislwous_en = rte_eth_promislwous_get(test_params->bonded_port_id);
	TEST_ASSERT_EQUAL(promislwous_en, 0,
			"Port (%d) promislwous mode not disabled\n",
			test_params->bonded_port_id);
...
static int
test_activebackup_verify_promislwous_enable_disable(void)
{
	int i, primary_port, promislwous_en;
	int ret;
	...
	TEST_ASSERT_EQUAL(rte_eth_promislwous_get(test_params->bonded_port_id), 1,
			"Port (%d) promislwous mode not enabled",
			test_params->bonded_port_id);
	...
	for (i = 0; i < test_params->bonded_slave_count; i++) {
		...
		if (primary_port == test_params->slave_port_ids[i]) {
			...
			TEST_ASSERT_EQUAL(promislwous_en, 0,
					"slave port (%d) promislwous mode enabled",
					test_params->slave_port_ids[i]);
		}
	...
	TEST_ASSERT_EQUAL(rte_eth_promislwous_get(test_params->bonded_port_id), 0,
			"Port (%d) promislwous mode not disabled\n",
			test_params->bonded_port_id);
	...
	for (i = 0; i < test_params->bonded_slave_count; i++) {
		promislwous_en = rte_eth_promislwous_get(
				test_params->slave_port_ids[i]);
		TEST_ASSERT_EQUAL(promislwous_en, 0,
				"slave port (%d) promislwous mode not disabled\n",
				test_params->slave_port_ids[i]);
	}
...
test_balance_xmit_policy_configuration(void)
{
	...
	TEST_ASSERT_FAIL(rte_eth_bond_xmit_policy_set(
			ILWALID_PORT_ID, BALANCE_XMIT_POLICY_LAYER2),
			"Expected call to failed as invalid port specified.");
	...
	/* Invalid port id */
	TEST_ASSERT_FAIL(rte_eth_bond_xmit_policy_get(ILWALID_PORT_ID),
			"Expected call to failed as invalid port specified.");
...
static int
test_balance_verify_promislwous_enable_disable(void)
{
	int i;
	int ret;
	...
	for (i = 0; i < test_params->bonded_slave_count; i++) {
		TEST_ASSERT_EQUAL(rte_eth_promislwous_get(
				test_params->slave_port_ids[i]), 1,
				"Port (%d) promislwous mode not enabled",
				test_params->slave_port_ids[i]);
	}
...
static int
test_broadcast_verify_promislwous_enable_disable(void)
{
	int i;
	int ret;
...
static int
test_tlb_verify_promislwous_enable_disable(void)
{
	int i, primary_port, promislwous_en;
	int ret;
	...
	promislwous_en = rte_eth_promislwous_get(test_params->bonded_port_id);
	TEST_ASSERT_EQUAL(promislwous_en, (int)1,
			"Port (%d) promislwous mode not enabled\n",
			test_params->bonded_port_id);
	for (i = 0; i < test_params->bonded_slave_count; i++) {
		promislwous_en = rte_eth_promislwous_get(
		...
		if (primary_port == test_params->slave_port_ids[i]) {
			...
			TEST_ASSERT_EQUAL(promislwous_en, (int)0,
					"Port (%d) promislwous mode enabled\n",
					test_params->bonded_port_id);
		}
...
static struct unit_test_suite link_bonding_test_suite  = {
	...
	.unit_test_cases = {
		...
		TEST_CASE(test_create_bonded_device_with_ilwalid_params),
		TEST_CASE(test_add_slave_to_bonded_device),
		TEST_CASE(test_add_slave_to_ilwalid_bonded_device),
		TEST_CASE(test_remove_slave_from_bonded_device),
		TEST_CASE(test_remove_slave_from_ilwalid_bonded_device),
		...
		TEST_CASE(test_roundrobin_verify_promislwous_enable_disable),
		...
		TEST_CASE(test_activebackup_verify_promislwous_enable_disable),
		...
		TEST_CASE(test_balance_verify_promislwous_enable_disable),
		...
		TEST_CASE(test_tlb_verify_promislwous_enable_disable),
		...
		TEST_CASE(test_broadcast_verify_promislwous_enable_disable),
