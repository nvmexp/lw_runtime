...
test_cmdline(void)
{
	...
	if (test_parse_etheraddr_valid() < 0)
		return -1;
	if (test_parse_etheraddr_ilwalid_data() < 0)
		return -1;
	if (test_parse_etheraddr_ilwalid_param() < 0)
		return -1;
	printf("Testind parsing port lists...\n");
	if (test_parse_portlist_valid() < 0)
		return -1;
	if (test_parse_portlist_ilwalid_data() < 0)
		return -1;
	if (test_parse_portlist_ilwalid_param() < 0)
		return -1;
	printf("Testind parsing numbers...\n");
	if (test_parse_num_valid() < 0)
		return -1;
	if (test_parse_num_ilwalid_data() < 0)
		return -1;
	if (test_parse_num_ilwalid_param() < 0)
		return -1;
	printf("Testing parsing IP addresses...\n");
	if (test_parse_ipaddr_valid() < 0)
		return -1;
	if (test_parse_ipaddr_ilwalid_data() < 0)
		return -1;
	if (test_parse_ipaddr_ilwalid_param() < 0)
		return -1;
	printf("Testing parsing strings...\n");
	if (test_parse_string_valid() < 0)
		return -1;
	if (test_parse_string_ilwalid_data() < 0)
		return -1;
	if (test_parse_string_ilwalid_param() < 0)
		return -1;
	printf("Testing cirlwlar buffer...\n");
	...
	if (test_cirbuf_align() < 0)
		return -1;
	if (test_cirbuf_ilwalid_param() < 0)
		return -1;
