...
static int
test_memzone_ilwalid_alignment(void)
{
	const struct rte_memzone * mz;

	mz = rte_memzone_lookup(TEST_MEMZONE_NAME("ilwalid_alignment"));
	if (mz != NULL) {
		printf("Zone with invalid alignment has been reserved\n");
		return -1;
	}

	mz = rte_memzone_reserve_aligned(TEST_MEMZONE_NAME("ilwalid_alignment"),
...
test_memzone(void)
{
	...
	if (test_memzone_ilwalid_alignment() < 0)
		return -1;
