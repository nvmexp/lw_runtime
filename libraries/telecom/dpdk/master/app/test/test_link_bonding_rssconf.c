...
static struct link_bonding_rssconf_unittest_params test_params  = {
	.bond_port_id = ILWALID_PORT_ID,
	.slave_ports = {
		[0 ... SLAVE_COUNT - 1] = { .port_id = ILWALID_PORT_ID, .is_slave = 0}
	},
	.mbuf_pool = NULL,
};
...
test_setup(void)
{
	...
	if (test_params.bond_port_id == ILWALID_PORT_ID) {
...
static int
test_rssconf_exelwtor(int (*test_func)(void))
{
	int test_result;
...
{
	return test_rssconf_exelwtor(&test_setup);
}

static int
test_rss_wrapper(void)
{
	return test_rssconf_exelwtor(&test_rss);
}

static int
test_rss_lazy_wrapper(void)
{
	return test_rssconf_exelwtor(&test_rss_lazy);
}
