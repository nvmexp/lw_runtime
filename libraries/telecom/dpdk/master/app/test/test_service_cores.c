...
static int32_t dummy_mt_unsafe_cb(void *args)
{
	 ...
	 * test, because two threads are conlwrrently in a non-MT safe callback.
	 */
...
static int32_t dummy_mt_safe_cb(void *args)
{
	 ...
	 * conlwrrently run the callback. The concept is as follows;
...
unregister_all(void)
{
	...
	TEST_ASSERT_EQUAL(-EILWAL, rte_service_component_unregister(1000),
...
service_attr_get(void)
{
	...
	TEST_ASSERT_EQUAL(-EILWAL, rte_service_attr_get(id, attr_id,
							&attr_value),
			"Invalid attr_id didn't return -EILWAL");

	attr_id = RTE_SERVICE_ATTR_CYCLES;
	TEST_ASSERT_EQUAL(-EILWAL, rte_service_attr_get(UINT32_MAX, attr_id,
							&attr_value),
			"Invalid service id didn't return -EILWAL");

	TEST_ASSERT_EQUAL(-EILWAL, rte_service_attr_get(id, attr_id, NULL),
			"Invalid attr_value pointer id didn't return -EILWAL");
...
service_lcore_attr_get(void)
{
	...
	/* check error return values */
	TEST_ASSERT_EQUAL(-EILWAL, rte_service_lcore_attr_get(UINT32_MAX,
			lcore_attr_id, &lcore_attr_value),
			"Invalid lcore_id didn't return -EILWAL");
	...
	TEST_ASSERT_EQUAL(-EILWAL, rte_service_lcore_attr_get(slcore_id,
			lcore_attr_id, &lcore_attr_value),
			"Invalid lcore attr didn't return -EILWAL");
...
service_lcore_en_dis_able(void)
{
	...
	TEST_ASSERT_EQUAL(-EILWAL, rte_service_map_lcore_set(sid, 100000, 1),
			"Enable on invalid core did not fail");
	TEST_ASSERT_EQUAL(-EILWAL, rte_service_map_lcore_set(sid, 100000, 0),
...
service_threaded_test(int mt_safe)
{
	...
	TEST_ASSERT_EQUAL(1, test_params[1],
			"MT Safe service not run by two cores conlwrrently");
	TEST_ASSERT_EQUAL(0, rte_service_runstate_set(sid, 0),
 ...
 * two threads access the callback conlwrrently.
 */
static int
service_mt_safe_poll(void)
{
	...
	TEST_ASSERT_EQUAL(1, service_threaded_test(mt_safe),
			"Error: MT Safe service not run by two cores conlwrrently");
	return TEST_SUCCESS;
}
...
service_mt_unsafe_poll(void)
{
	...
	TEST_ASSERT_EQUAL(1, service_threaded_test(mt_safe),
			"Error: NON MT Safe service run by two cores conlwrrently");
	return TEST_SUCCESS;
}
...
service_lcore_start_stop(void)
{
	...
	TEST_ASSERT_EQUAL(0, rte_service_runstate_set(sid, 1),
			"Starting valid service failed");
	TEST_ASSERT_EQUAL(-EILWAL, rte_service_map_lcore_set(sid, slcore_id, 1),
			"Enabling valid service on non-service core must fail");

	/* core start */
	TEST_ASSERT_EQUAL(-EILWAL, rte_service_lcore_start(slcore_id),
			"Service core start without add should return EILWAL");
	TEST_ASSERT_EQUAL(0, rte_service_lcore_add(slcore_id),
			"Service core add did not return zero");
	TEST_ASSERT_EQUAL(0, rte_service_map_lcore_set(sid, slcore_id, 1),
	...
	TEST_ASSERT_EQUAL(-EILWAL, rte_service_lcore_stop(100000),
			"Invalid Service core stop should return -EILWAL");
	TEST_ASSERT_EQUAL(0, rte_service_lcore_stop(slcore_id),
...
service_may_be_active(void)
{
	...
	/* expected failure cases */
	TEST_ASSERT_EQUAL(-EILWAL, rte_service_may_be_active(10000),
