...
#define ACC100_QMGR_ILWALID_IDX -1
...
#define SYNC_WAIT 0
#define SYNC_START 1
#define ILWALID_OPAQUE -1

#define ILWALID_QUEUE_ID -1
...
/* LLR arithmetic representation for numerical colwersion */
...
/* callwlates optimal mempool size not smaller than the val */
static unsigned int
 ...
 */
	...
	if ((get_init_device() == true) &&
		...
		conf.q_ul_4g.first_qgroup_index = ACC100_QMGR_ILWALID_IDX;
		...
		conf.q_dl_4g.first_qgroup_index = ACC100_QMGR_ILWALID_IDX;
		...
		conf.q_ul_5g.first_qgroup_index = ACC100_QMGR_ILWALID_IDX;
		...
		conf.q_dl_5g.first_qgroup_index = ACC100_QMGR_ILWALID_IDX;
...
gen_qm8_llr(int8_t *llrs, uint32_t i, double N0, double llr_max)
{
	...
	Q += sqrt(N0 / 2) * randn(1);
	/*
	 * Callwlate the log of the probability that each of
	 * the constellation points was transmitted
	 */
	for (m = 0; m < qam; m++)
		log_syml_prob[m] = -(pow(I - symbols_I[m], 2.0)
				+ pow(Q - symbols_Q[m], 2.0)) / N0;
	/* Callwlate an LLR for each of the k_64QAM bits in the set */
	for (k = 0; k < qm; k++) {
		p0 = -999999;
		p1 = -999999;
		...
		unsigned int order_idx, const int expected_status)
{
	...
	if (op->opaque_data != (void *)(uintptr_t)ILWALID_OPAQUE)
		TEST_ASSERT((void *)(uintptr_t)order_idx == op->opaque_data,
				"Ordering error, expected %p, got %p",
				(void *)(uintptr_t)order_idx, op->opaque_data);
		...
		struct rte_bbdev_op_ldpc_dec *ops_ld)
{
	...
	uint32_t byte_error = 0, lwm_error = 0, error;
		...
		struct rte_bbdev_dec_op *ref_op, const int vector_mask)
{
	...
	for (i = 0; i < n; ++i) {
		 ...
		 * colwerge or for loop-back cases
		 */
