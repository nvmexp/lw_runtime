...
static int
test_bbdev_configure_ilwalid_dev_id(void)
{
	uint8_t dev_id;
	uint8_t num_queues;
...
static int
test_bbdev_configure_ilwalid_num_queues(void)
{
...
static int
test_bbdev_configure_ilwalid_queue_configure(void)
{
	struct bbdev_testsuite_params *ts_params = &testsuite_params;
	int return_value;
	struct rte_bbdev_info info;
...
test_bbdev_op_pool(void)
{
	...
	TEST_ASSERT((mp = rte_bbdev_op_pool_create("Test_ILW",
	...
	TEST_ASSERT((mp = rte_bbdev_op_pool_create("Test_IlwSize",
			RTE_BBDEV_OP_NONE, size, cache_size, 0)) == NULL,
			"Failed test for rte_bbdev_op_pool_create: "
...
test_bbdev_driver_init(void)
{
	...
	sprintf(name_tmp, "%s", "ilwalid_name");
	dev2 = rte_bbdev_get_named_dev(name_tmp);
...
test_bbdev_callback(void)
{
	...
	int event_status;
	uint8_t ilwalid_dev_id = RTE_BBDEV_MAX_DEVS;
	enum rte_bbdev_event_type ilwalid_event_type = RTE_BBDEV_EVENT_MAX;
	uint8_t dev_id;
	...
	TEST_ASSERT_FAIL(rte_bbdev_callback_register(ilwalid_dev_id,
			RTE_BBDEV_EVENT_ERROR, event_callback, &event_status),
			"Failed test for rte_bbdev_callback_register "
			"for ilwalid_dev_id ");

	TEST_ASSERT_FAIL(rte_bbdev_callback_register(dev1->data->dev_id,
			ilwalid_event_type, event_callback, &event_status),
	...
	event_status = -1;
	rte_bbdev_pmd_callback_process(dev1, ilwalid_event_type, NULL);
	TEST_ASSERT(event_status == -1,
			"Failed test for rte_bbdev_pmd_callback_process: "
			"for invalid event type ");

	/* rte_dev_callback_unregister with invalid inputs */
	TEST_ASSERT_FAIL(rte_bbdev_callback_unregister(ilwalid_dev_id,
			RTE_BBDEV_EVENT_UNKNOWN, event_callback, &event_status),
	...
	TEST_ASSERT_FAIL(rte_bbdev_callback_unregister(dev1->data->dev_id,
			ilwalid_event_type, NULL, &event_status),
			"Failed rte_dev_callback_unregister "
...
static int
test_bbdev_ilwalid_driver(void)
{
	struct rte_bbdev dev1, *dev2;
	uint8_t dev_id = null_dev_id;
	uint16_t queue_id = 0;
...
static struct unit_test_suite bbdev_null_testsuite = {
	...
	.unit_test_cases = {
		...
		TEST_CASE(test_bbdev_configure_ilwalid_dev_id),

		TEST_CASE_ST(ut_setup, ut_teardown,
				test_bbdev_configure_ilwalid_num_queues),
		...
		TEST_CASE_ST(ut_setup, ut_teardown,
				test_bbdev_configure_ilwalid_queue_configure),
