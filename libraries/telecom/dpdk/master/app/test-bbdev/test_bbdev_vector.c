...
/* colwert turbo decoder flag from string to unsigned long int*/
static int
...
/* colwert LDPC flag from string to unsigned long int*/
static int
...
/* colwert turbo encoder flag from string to unsigned long int*/
static int
...
/* colwert LDPC encoder flag from string to unsigned long int*/
static int
...
/* colwert turbo encoder/decoder op_type from string to enum*/
static int
		...
		struct test_bbdev_vector *vector)
{
	...
	if (ret != 0) {
		printf("Failed with colwert '%s\t%s'\n", key_token, token);
		return -1;
	}
		...
		struct test_bbdev_vector *vector)
{
	...
	while (getline(&line, &len, fp) != -1) {
		...
		if (ret != 0) {
			printf("An error oclwrred while parsing!\n");
			goto exit;
		}
	}
	ret = bbdev_check_vector(vector);
	if (ret != 0)
		printf("An error oclwrred while checking!\n");
