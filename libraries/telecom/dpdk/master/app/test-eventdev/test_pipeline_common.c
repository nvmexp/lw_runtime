		...
		int (*worker)(void *))
{
	...
	while (t->done == false) {
		...
		if ((new_cycles - perf_cycles) > perf_sample) {
			const uint64_t lwrr_pkts = processed_pkts(t);

			float mpps = (float)(lwrr_pkts - prev_pkts)/1000000;
...
pipeline_opt_check(struct evt_options *opt, uint64_t nb_queues)
{
	...
	if (evt_has_ilwalid_stage(opt))
		return -1;

	if (evt_has_ilwalid_sched_type(opt))
		return -1;
...
pipeline_ethdev_setup(struct evt_test *test, struct evt_options *opt)
{
	...
	RTE_ETH_FOREACH_DEV(i) {
		...
		ret = rte_eth_promislwous_enable(i);
		if (ret != 0) {
			evt_err("Failed to enable promislwous mode for eth port [%d]: %s",
				i, rte_strerror(-ret));
			return ret;
		}
	}
