                                 ...
                                 __rte_unused void *data)
{
	...
	if (show_all || !strcmp(res->section, "control")) {
		...
		cmdline_printf(
			...
			"stop\n"
			"    Stop packet forwarding, and display aclwmulated"
			" statistics.\n\n"
	...
	if (show_all || !strcmp(res->section, "config")) {
		cmdline_printf(
			...
			"    Select hardware or software callwlation of the"
			...
			"    Set the promislwous mode on port_id, or all.\n\n"

			"set allmulti (port_id|all) (on|off)\n"
			"    Set the allmulti mode on port_id, or all.\n\n"

			"set vf promisc (port_id) (vf_id) (on|off)\n"
			"    Set unicast promislwous mode for a VF from the PF.\n\n"

			"set vf allmulti (port_id) (vf_id) (on|off)\n"
			"    Set multicast promislwous mode for a VF from the PF.\n\n"
	...
	if (show_all || !strcmp(res->section, "ports")) {
		...
		cmdline_printf(
			...
			"ether|port|vxlan|geneve|lwgre|vxlan-gpe|ecpri|none|level-default|"
	...
	if (show_all || !strcmp(res->section, "filters")) {
		...
		cmdline_printf(
			...
			"set lwgre ip-version (ipv4|ipv6) tni (tni) ip-src"
			" (ip-src) ip-dst (ip-dst) eth-src (eth-src) eth-dst"
			" (eth-dst)\n"
			"       Configure the LWGRE encapsulation for flows.\n\n"

			"set lwgre-with-vlan ip-version (ipv4|ipv6) tni (tni)"
	...
	if (show_all || !strcmp(res->section, "traffic_management")) {
		cmdline_printf(
			...
			" (color_g) (min_th_g) (max_th_g) (maxp_ilw_g) (wq_log2_g)"
			" (color_y) (min_th_y) (max_th_y) (maxp_ilw_y) (wq_log2_y)"
			" (color_r) (min_th_r) (max_th_r) (maxp_ilw_r) (wq_log2_r)\n"
				...
				__rte_unused void *data)
{
	...
	if (port_id_is_ilwalid(res->id, ENABLED_WARN))
		return;
				...
				__rte_unused void *data)
{
	...
	if (port_id_is_ilwalid(res->port_id, ENABLED_WARN))
		return;
			...
			__rte_unused void *data)
{
	...
	else if (!strcmp(res->value, "lwgre"))
		rss_conf.rss_hf = ETH_RSS_LWGRE;
...
cmdline_parse_inst_t cmd_config_rss = {
	...
	.help_str = "port config all rss "
		...
		"lwgre|vxlan-gpe|l2tpv3|esp|ah|pfcp|ecpri|none|level-default|"
		"level-outer|level-inner|<flowtype_id>",
	.tokens = {
				 ...
				 __rte_unused void *data)
{
	...
	if (port_id_is_ilwalid(res->portid, ENABLED_WARN))
		return;
	...
	if (isrx && rx_queue_id_is_ilwalid(res->qid))
		return;
	else if (!isrx && tx_queue_id_is_ilwalid(res->qid))
		return;
	...
	__rte_unused void *data)
{
	...
	if (isrx) {
		...
		mp = mbuf_pool_find(socket_id, 0, mbuf_mem_types[0]);
		if (mp == NULL) {
			...
			__rte_unused void *data)
{
	...
	if (dev_info.reta_size > ETH_RSS_RETA_SIZE_512) {
		printf("Lwrrently do not support more than %u entries of "
...
unsigned int
parse_buffer_list(char* str, const char* item_name, unsigned int max_items,
		unsigned int *parsed_items, int check_unique_values)
{
	unsigned int nb_item;
	unsigned int value;
...
cmdline_parse_inst_t cmd_csum_set = {
	...
	.help_str = "csum set ip|tcp|udp|sctp|outer-ip|outer-udp hw|sw <port_id>: "
		"Enable/Disable hardware callwlation of L3/L4 checksum when "
			  ...
			  __rte_unused void *data)
{
	...
	if (ports[res->port_id].tunnel_tso_segsz == 0) {
		 ...
		 * of outer IP header callwlated by sw should be wrong; that
		...
		__rte_unused void *data)
{
	...
	if (port_id < 0) {
		...
		if (ret != 0)
			printf("Failed to enable promislwous mode for port %u: %s - ignore\n",
		...
		__rte_unused void *data)
{
	...
	if (port_id_is_ilwalid(res->port_num, ENABLED_WARN))
		return;
...
/* VF unicast promislwous mode configuration */

/* Common result structure for VF unicast promislwous mode */
...
/* Common CLI fields for VF unicast promislwous mode enable disable */
...
/* VF multicast promislwous mode configuration */

/* Common result structure for VF multicast promislwous mode */
...
/* Common CLI fields for VF multicast promislwous mode enable disable */
...
/** Set LWGRE encapsulation details */
struct cmd_set_lwgre_result {
	cmdline_fixed_string_t set;
	cmdline_fixed_string_t lwgre;
	cmdline_fixed_string_t pos_token;
	cmdline_fixed_string_t ip_version;
	uint32_t tni;
	cmdline_ipaddr_t ip_src;
...
cmdline_parse_token_string_t cmd_set_lwgre_set =
	TOKEN_STRING_INITIALIZER(struct cmd_set_lwgre_result, set, "set");
cmdline_parse_token_string_t cmd_set_lwgre_lwgre =
	TOKEN_STRING_INITIALIZER(struct cmd_set_lwgre_result, lwgre, "lwgre");
cmdline_parse_token_string_t cmd_set_lwgre_lwgre_with_vlan =
	TOKEN_STRING_INITIALIZER(struct cmd_set_lwgre_result, lwgre,
				 "lwgre-with-vlan");
cmdline_parse_token_string_t cmd_set_lwgre_ip_version =
	TOKEN_STRING_INITIALIZER(struct cmd_set_lwgre_result, pos_token,
				 "ip-version");
cmdline_parse_token_string_t cmd_set_lwgre_ip_version_value =
	TOKEN_STRING_INITIALIZER(struct cmd_set_lwgre_result, ip_version,
				 "ipv4#ipv6");
cmdline_parse_token_string_t cmd_set_lwgre_tni =
	TOKEN_STRING_INITIALIZER(struct cmd_set_lwgre_result, pos_token,
				 "tni");
cmdline_parse_token_num_t cmd_set_lwgre_tni_value =
	TOKEN_NUM_INITIALIZER(struct cmd_set_lwgre_result, tni, RTE_UINT32);
cmdline_parse_token_string_t cmd_set_lwgre_ip_src =
	TOKEN_STRING_INITIALIZER(struct cmd_set_lwgre_result, pos_token,
				 "ip-src");
cmdline_parse_token_num_t cmd_set_lwgre_ip_src_value =
	TOKEN_IPADDR_INITIALIZER(struct cmd_set_lwgre_result, ip_src);
cmdline_parse_token_string_t cmd_set_lwgre_ip_dst =
	TOKEN_STRING_INITIALIZER(struct cmd_set_lwgre_result, pos_token,
				 "ip-dst");
cmdline_parse_token_ipaddr_t cmd_set_lwgre_ip_dst_value =
	TOKEN_IPADDR_INITIALIZER(struct cmd_set_lwgre_result, ip_dst);
cmdline_parse_token_string_t cmd_set_lwgre_vlan =
	TOKEN_STRING_INITIALIZER(struct cmd_set_lwgre_result, pos_token,
				 "vlan-tci");
cmdline_parse_token_num_t cmd_set_lwgre_vlan_value =
	TOKEN_NUM_INITIALIZER(struct cmd_set_lwgre_result, tci, RTE_UINT16);
cmdline_parse_token_string_t cmd_set_lwgre_eth_src =
	TOKEN_STRING_INITIALIZER(struct cmd_set_lwgre_result, pos_token,
				 "eth-src");
cmdline_parse_token_etheraddr_t cmd_set_lwgre_eth_src_value =
	TOKEN_ETHERADDR_INITIALIZER(struct cmd_set_lwgre_result, eth_src);
cmdline_parse_token_string_t cmd_set_lwgre_eth_dst =
	TOKEN_STRING_INITIALIZER(struct cmd_set_lwgre_result, pos_token,
				 "eth-dst");
cmdline_parse_token_etheraddr_t cmd_set_lwgre_eth_dst_value =
	TOKEN_ETHERADDR_INITIALIZER(struct cmd_set_lwgre_result, eth_dst);

static void cmd_set_lwgre_parsed(void *parsed_result,
	__rte_unused struct cmdline *cl,
	__rte_unused void *data)
{
	struct cmd_set_lwgre_result *res = parsed_result;
	union {
		uint32_t lwgre_tni;
		uint8_t tni[4];
	} id = {
		.lwgre_tni = rte_cpu_to_be_32(res->tni) & RTE_BE32(0x00ffffff),
	};

	if (strcmp(res->lwgre, "lwgre") == 0)
		lwgre_encap_conf.select_vlan = 0;
	else if (strcmp(res->lwgre, "lwgre-with-vlan") == 0)
		lwgre_encap_conf.select_vlan = 1;
	if (strcmp(res->ip_version, "ipv4") == 0)
		lwgre_encap_conf.select_ipv4 = 1;
	else if (strcmp(res->ip_version, "ipv6") == 0)
		lwgre_encap_conf.select_ipv4 = 0;
	else
		return;
	rte_memcpy(lwgre_encap_conf.tni, &id.tni[1], 3);
	if (lwgre_encap_conf.select_ipv4) {
		IPV4_ADDR_TO_UINT(res->ip_src, lwgre_encap_conf.ipv4_src);
		IPV4_ADDR_TO_UINT(res->ip_dst, lwgre_encap_conf.ipv4_dst);
	} else {
		IPV6_ADDR_TO_ARRAY(res->ip_src, lwgre_encap_conf.ipv6_src);
		IPV6_ADDR_TO_ARRAY(res->ip_dst, lwgre_encap_conf.ipv6_dst);
	}
	if (lwgre_encap_conf.select_vlan)
		lwgre_encap_conf.vlan_tci = rte_cpu_to_be_16(res->tci);
	rte_memcpy(lwgre_encap_conf.eth_src, res->eth_src.addr_bytes,
		   RTE_ETHER_ADDR_LEN);
	rte_memcpy(lwgre_encap_conf.eth_dst, res->eth_dst.addr_bytes,
		   RTE_ETHER_ADDR_LEN);
}

cmdline_parse_inst_t cmd_set_lwgre = {
	.f = cmd_set_lwgre_parsed,
	.data = NULL,
	.help_str = "set lwgre ip-version <ipv4|ipv6> tni <tni> ip-src"
		" <ip-src> ip-dst <ip-dst> eth-src <eth-src>"
		" eth-dst <eth-dst>",
	.tokens = {
		(void *)&cmd_set_lwgre_set,
		(void *)&cmd_set_lwgre_lwgre,
		(void *)&cmd_set_lwgre_ip_version,
		(void *)&cmd_set_lwgre_ip_version_value,
		(void *)&cmd_set_lwgre_tni,
		(void *)&cmd_set_lwgre_tni_value,
		(void *)&cmd_set_lwgre_ip_src,
		(void *)&cmd_set_lwgre_ip_src_value,
		(void *)&cmd_set_lwgre_ip_dst,
		(void *)&cmd_set_lwgre_ip_dst_value,
		(void *)&cmd_set_lwgre_eth_src,
		(void *)&cmd_set_lwgre_eth_src_value,
		(void *)&cmd_set_lwgre_eth_dst,
		(void *)&cmd_set_lwgre_eth_dst_value,
		NULL,
	},
};

cmdline_parse_inst_t cmd_set_lwgre_with_vlan = {
	.f = cmd_set_lwgre_parsed,
	.data = NULL,
	.help_str = "set lwgre-with-vlan ip-version <ipv4|ipv6> tni <tni>"
		" ip-src <ip-src> ip-dst <ip-dst> vlan-tci <vlan-tci>"
		" eth-src <eth-src> eth-dst <eth-dst>",
	.tokens = {
		(void *)&cmd_set_lwgre_set,
		(void *)&cmd_set_lwgre_lwgre_with_vlan,
		(void *)&cmd_set_lwgre_ip_version,
		(void *)&cmd_set_lwgre_ip_version_value,
		(void *)&cmd_set_lwgre_tni,
		(void *)&cmd_set_lwgre_tni_value,
		...
		(void *)&cmd_set_lwgre_ip_dst,
		(void *)&cmd_set_lwgre_ip_dst_value,
		(void *)&cmd_set_lwgre_vlan,
		(void *)&cmd_set_lwgre_vlan_value,
		(void *)&cmd_set_lwgre_eth_src,
		(void *)&cmd_set_lwgre_eth_src_value,
		(void *)&cmd_set_lwgre_eth_dst,
		(void *)&cmd_set_lwgre_eth_dst_value,
		...
		__rte_unused void *data)
{
	...
	if (port_id_is_ilwalid(res->cmd_pid, ENABLED_WARN))
		return;
...
cmd_reconfig_device_queue(portid_t id, uint8_t dev, uint8_t queue)
{
	...
	} else if (!port_id_is_ilwalid(id, DISABLED_WARN)) {
		/* check if need_reconfig has been set to 1 */
