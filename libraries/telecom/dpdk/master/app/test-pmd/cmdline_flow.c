...
struct lwgre_encap_conf lwgre_encap_conf = {
	.select_ipv4 = 1,
	.select_vlan = 0,
	.tni = "\x00\x00\x00",
	.ipv4_src = RTE_IPV4(127, 0, 0, 1),
...
/** Maximum number of items in struct rte_flow_action_lwgre_encap. */
#define ACTION_LWGRE_ENCAP_ITEMS_NUM 5

/** Storage for struct rte_flow_action_lwgre_encap including external data. */
struct action_lwgre_encap_data {
	struct rte_flow_action_lwgre_encap conf;
	struct rte_flow_item items[ACTION_LWGRE_ENCAP_ITEMS_NUM];
	struct rte_flow_item_eth item_eth;
	struct rte_flow_item_vlan item_vlan;
	union {
		struct rte_flow_item_ipv4 item_ipv4;
	...
	struct rte_flow_item_lwgre item_lwgre;
};
...
struct context {
	...
	enum index lwrr; /**< Current token index. */
...
static const enum index item_lwgre[] = {
	ITEM_LWGRE_TNI,
	ITEM_NEXT,
	ZERO,
};
...
static int parse_vc_action_vxlan_encap(struct context *, const struct token *,
				       const char *, unsigned int, void *,
				       unsigned int);
static int parse_vc_action_lwgre_encap(struct context *, const struct token *,
				       const char *, unsigned int, void *,
				       unsigned int);
...
static const struct token token_list[] = {
	...
	[ITEM_LWGRE] = {
		.name = "lwgre",
		.help = "match LWGRE header",
		.priv = PRIV_ITEM(LWGRE, sizeof(struct rte_flow_item_lwgre)),
		.next = NEXT(item_lwgre),
		.call = parse_vc,
	},
	[ITEM_LWGRE_TNI] = {
		.name = "tni",
		.help = "virtual subnet ID",
		.next = NEXT(item_lwgre, NEXT_ENTRY(UNSIGNED), item_param),
		.args = ARGS(ARGS_ENTRY_HTON(struct rte_flow_item_lwgre, tni)),
	},
	[ITEM_MPLS] = {
		.name = "mpls",
	...
	[ACTION_LWGRE_ENCAP] = {
		.name = "lwgre_encap",
		.help = "LWGRE encapsulation, uses configuration set by \"set"
			" lwgre\"",
		.priv = PRIV_ACTION(LWGRE_ENCAP,
				    sizeof(struct action_lwgre_encap_data)),
		.next = NEXT(NEXT_ENTRY(ACTION_NEXT)),
		.call = parse_vc_action_lwgre_encap,
	},
	[ACTION_LWGRE_DECAP] = {
		.name = "lwgre_decap",
		.help = "Performs a decapsulation action by stripping all"
			" headers of the LWGRE tunnel network overlay from the"
			" matched flow.",
		.priv = PRIV_ACTION(LWGRE_DECAP, 0),
		.next = NEXT(NEXT_ENTRY(ACTION_NEXT)),
		.call = parse_vc,
	},
	[ACTION_L2_ENCAP] = {
	     ...
	     void *buf, unsigned int size)
{
	...
	static const uint8_t colw[] = "\x00\x80\xc0\xe0\xf0\xf8\xfc\xfe\xff";
	char *end;
	uintmax_t u;
	unsigned int bytes;
	unsigned int extra;
	...
	if (!arg->hton) {
		...
		if (extra)
			((uint8_t *)buf)[size - bytes - 1] = colw[extra];
	} else
#endif
	{
	 ...
	 void *buf, unsigned int size)
{
	...
	if (!out)
		return len;
	if (!out->command) {
		if (ctx->lwrr != SHARED_ACTION)
			return -1;
		if (sizeof(*out) > size)
			return -1;
		out->command = ctx->lwrr;
		ctx->objdata = 0;
		ctx->object = out;
		ctx->objmask = NULL;
		 ...
		 void *buf, unsigned int size)
{
	...
	if (!out->command || out->command == SHARED_ACTION) {
		if (ctx->lwrr != SHARED_ACTION_DESTROY)
			return -1;
		if (sizeof(*out) > size)
			return -1;
		out->command = ctx->lwrr;
	 ...
	 void *buf, unsigned int size)
{
	...
	if (!out)
		return len;
	if (!out->command) {
		if (ctx->lwrr != VALIDATE && ctx->lwrr != CREATE)
			return -1;
		if (sizeof(*out) > size)
			return -1;
		out->command = ctx->lwrr;
...
/** Parse LWGRE encap action. */
static int
parse_vc_action_lwgre_encap(struct context *ctx, const struct token *token,
			    const char *str, unsigned int len,
			    void *buf, unsigned int size)
{
	struct buffer *out = buf;
	struct rte_flow_action *action;
	struct action_lwgre_encap_data *action_lwgre_encap_data;
	int ret;
	...
	ctx->objmask = NULL;
	/* Set up default configuration. */
	action_lwgre_encap_data = ctx->object;
	*action_lwgre_encap_data = (struct action_lwgre_encap_data){
		.conf = (struct rte_flow_action_lwgre_encap){
			.definition = action_lwgre_encap_data->items,
		},
		.items = {
			{
				.type = RTE_FLOW_ITEM_TYPE_ETH,
				.spec = &action_lwgre_encap_data->item_eth,
				.mask = &rte_flow_item_eth_mask,
			},
			{
				.type = RTE_FLOW_ITEM_TYPE_VLAN,
				.spec = &action_lwgre_encap_data->item_vlan,
				.mask = &rte_flow_item_vlan_mask,
			},
			{
				.type = RTE_FLOW_ITEM_TYPE_IPV4,
				.spec = &action_lwgre_encap_data->item_ipv4,
				.mask = &rte_flow_item_ipv4_mask,
			},
			{
				.type = RTE_FLOW_ITEM_TYPE_LWGRE,
				.spec = &action_lwgre_encap_data->item_lwgre,
				.mask = &rte_flow_item_lwgre_mask,
			},
			{
				.type = RTE_FLOW_ITEM_TYPE_END,
			},
		...
		.item_eth.type = 0,
		.item_vlan = {
			.tci = lwgre_encap_conf.vlan_tci,
			.inner_type = 0,
		},
		.item_ipv4.hdr = {
		       .src_addr = lwgre_encap_conf.ipv4_src,
		       .dst_addr = lwgre_encap_conf.ipv4_dst,
		},
		.item_lwgre.flow_id = 0,
	};
	memcpy(action_lwgre_encap_data->item_eth.dst.addr_bytes,
	       lwgre_encap_conf.eth_dst, RTE_ETHER_ADDR_LEN);
	memcpy(action_lwgre_encap_data->item_eth.src.addr_bytes,
	       lwgre_encap_conf.eth_src, RTE_ETHER_ADDR_LEN);
	if (!lwgre_encap_conf.select_ipv4) {
		memcpy(&action_lwgre_encap_data->item_ipv6.hdr.src_addr,
		       &lwgre_encap_conf.ipv6_src,
		       sizeof(lwgre_encap_conf.ipv6_src));
		memcpy(&action_lwgre_encap_data->item_ipv6.hdr.dst_addr,
		       &lwgre_encap_conf.ipv6_dst,
		       sizeof(lwgre_encap_conf.ipv6_dst));
		action_lwgre_encap_data->items[2] = (struct rte_flow_item){
			.type = RTE_FLOW_ITEM_TYPE_IPV6,
			.spec = &action_lwgre_encap_data->item_ipv6,
			.mask = &rte_flow_item_ipv6_mask,
		};
	}
	if (!lwgre_encap_conf.select_vlan)
		action_lwgre_encap_data->items[1].type =
			RTE_FLOW_ITEM_TYPE_VOID;
	memcpy(action_lwgre_encap_data->item_lwgre.tni, lwgre_encap_conf.tni,
	       RTE_DIM(lwgre_encap_conf.tni));
	action->conf = &action_lwgre_encap_data->conf;
	return ret;
}
				...
				unsigned int size)
{
	...
	RTE_SET_USED(buf);
	RTE_SET_USED(size);
	if (ctx->lwrr != ACTION_RAW_ENCAP_INDEX_VALUE)
		return -1;
	arg = ARGS_ENTRY_ARB_BOUNDED
				...
				unsigned int size)
{
	...
	RTE_SET_USED(buf);
	RTE_SET_USED(size);
	if (ctx->lwrr != ACTION_SAMPLE_INDEX_VALUE)
		return -1;
	arg = ARGS_ENTRY_ARB_BOUNDED
	     ...
	     unsigned int ent, char *buf, unsigned int size)
{
	...
	(void)token;
	if (port_id_is_ilwalid(ctx->port, DISABLED_WARN) ||
...
{
	struct context *ctx = &cmd_flow_context;
	const struct token *token = &token_list[ctx->lwrr];
	const enum index *list;
	int i;
...
update_fields(uint8_t *buf, struct rte_flow_item *item, uint16_t next_proto)
{
	...
	struct rte_flow_item_lwgre *lwgre;
	...
	case RTE_FLOW_ITEM_TYPE_LWGRE:
		lwgre = (struct rte_flow_item_lwgre *)buf;
		lwgre->protocol = rte_cpu_to_be_16(0x6558);
		lwgre->c_k_s_rsvd0_ver = rte_cpu_to_be_16(0x2000);
		break;
	default:
		break;
	}
	...
	*total_size = 0;
	...
	for (i = n - 1 ; i >= 0; --i) {
		...
		case RTE_FLOW_ITEM_TYPE_MPLS:
			...
			break;
		case RTE_FLOW_ITEM_TYPE_LWGRE:
			size = sizeof(struct rte_flow_item_lwgre);
			proto = 0x2F;
			break;
		case RTE_FLOW_ITEM_TYPE_GENEVE:
	     ...
	     cmdline_parse_token_hdr_t **hdr_inst)
{
	...
	/* Return NULL when no more tokens are expected. */
	if (!ctx->next_num && (ctx->lwrr != START_SET)) {
		*hdr = NULL;
		return;
	}
	/* Determine if command should end here. */
