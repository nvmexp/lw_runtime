...
/* if possible, callwlate the checksum of a packet in hw or sw,
	...
	uint64_t tx_offloads)
{
	...
	if (info->l4_proto == IPPROTO_UDP) {
		...
		/* do not recallwlate udp cksum if it was 0 */
...
/* Callwlate the checksum of outer header */
static uint64_t
	...
	uint64_t tx_offloads, int tso_enabled)
{
	 ...
	 * set/callwlate a non-zero checksum, because it will be a wrong
...
pkt_copy_split(const struct rte_mbuf *pkt)
{
	...
	/* callwlate number of segments to use and their length. */
	len = 0;
 ...
 * wether a checksum must be callwlated in software or in hardware. The
...
tunnel_update:
		 ...
		 * mbuf to offload the callwlation to the NIC. If TSO
	...
	if (unlikely(gro_enable)) {
		if (gro_flush_cycles == GRO_DEFAULT_FLUSH_CYCLES) {
			...
			gro_ctx = lwrrent_fwd_lcore()->gro_ctx;
	...
	if (gso_ports[fs->tx_port].enable == 0)
		...
		gso_ctx = &(lwrrent_fwd_lcore()->gso_ctx);
