...
parse_pdcp_sn_sz(struct cperf_options *opts, const char *arg)
{
	...
	if (val != RTE_SELWRITY_PDCP_SN_SIZE_5 &&
			val != RTE_SELWRITY_PDCP_SN_SIZE_7 &&
			val != RTE_SELWRITY_PDCP_SN_SIZE_12 &&
			val != RTE_SELWRITY_PDCP_SN_SIZE_15 &&
			val != RTE_SELWRITY_PDCP_SN_SIZE_18) {
		printf("\nIlwalid pdcp SN size: %u\n", val);
		return -ERANGE;
	}
	opts->pdcp_sn_sz = val;
...
const char *cperf_pdcp_domain_strs[] = {
	[RTE_SELWRITY_PDCP_MODE_CONTROL] = "control",
	[RTE_SELWRITY_PDCP_MODE_DATA] = "data"
};
...
parse_pdcp_domain(struct cperf_options *opts, const char *arg)
{
	struct name_id_map pdcp_domain_namemap[] = {
		{
			...
			[RTE_SELWRITY_PDCP_MODE_CONTROL],
			RTE_SELWRITY_PDCP_MODE_CONTROL },
		{
			cperf_pdcp_domain_strs
			[RTE_SELWRITY_PDCP_MODE_DATA],
			RTE_SELWRITY_PDCP_MODE_DATA
		}
	};
	...
	opts->pdcp_domain = (enum rte_selwrity_pdcp_domain)id;
...
#ifdef RTE_LIB_SELWRITY
	opts->pdcp_sn_sz = 12;
	opts->pdcp_domain = RTE_SELWRITY_PDCP_MODE_CONTROL;
