...
cperf_pmd_cyclecount_test_free(struct cperf_pmd_cyclecount_ctx *ctx)
{
	...
	if (ctx->sess) {
		...
		if (ctx->options->op_type == CPERF_PDCP ||
			...
			rte_selwrity_session_destroy(sec_ctx,
				(struct rte_selwrity_session *)ctx->sess);
		} else
#endif
		{
...
static inline int
pmd_cyclecount_bench_ops(struct pmd_cyclecount_state *state, uint32_t lwr_op,
		uint16_t test_burst_size)
{
	uint32_t iter_ops_left = state->opts->total_ops - lwr_op;
	uint32_t iter_ops_needed =
			RTE_MIN(state->opts->nb_descriptors, iter_ops_left);
	uint32_t lwr_iter_op;
	uint32_t imix_idx = 0;

	for (lwr_iter_op = 0; lwr_iter_op < iter_ops_needed;
			lwr_iter_op += test_burst_size) {
		uint32_t burst_size = RTE_MIN(iter_ops_needed - lwr_iter_op,
				test_burst_size);
		struct rte_crypto_op **ops = &state->ctx->ops[lwr_iter_op];
		...
		uint32_t iter_ops_needed, uint16_t test_burst_size)
{
	...
	for (lwr_iter_op = 0; lwr_iter_op < iter_ops_needed;
		...
		uint32_t burst_size = RTE_MIN(
				iter_ops_needed - lwr_iter_op, test_burst_size);
		struct rte_crypto_op **ops = &state->ctx->ops[lwr_iter_op];
		...
		uint32_t iter_ops_needed, uint16_t test_burst_size)
{
	...
	uint32_t lwr_iter_op = 0;
	while (lwr_iter_op < iter_ops_needed) {
		uint32_t burst_size = RTE_MIN(iter_ops_needed - lwr_iter_op,
				test_burst_size);
		struct rte_crypto_op **ops = &state->ctx->ops[lwr_iter_op];
		uint32_t burst_enqd;
		...
		struct pmd_cyclecount_state *state, uint16_t test_burst_size)
{
	...
	for (lwr_op = 0; lwr_op < state->opts->total_ops;
			lwr_op += state->opts->nb_descriptors) {
		if (unlikely(pmd_cyclecount_bench_ops(
				state, lwr_op, test_burst_size)))
			return -1;
	}
	 ...
	 */
	lwr_op = 0;
	while (lwr_op < state->opts->total_ops) {
		uint32_t iter_ops_left = state->opts->total_ops - lwr_op;
