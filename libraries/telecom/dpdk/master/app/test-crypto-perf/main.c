...
cperf_initialize_cryptodev(struct cperf_options *opts, uint8_t *enabled_cdevs)
{
	...
	for (cdev_id = 0; cdev_id < rte_cryptodev_count(); cdev_id++) {
		sess_size = rte_selwrity_session_get_size(
...
#endif
	/*
	 * Callwlate number of needed queue pairs, based on the amount
	...
	for (i = 0; i < enabled_cdev_count &&
		...
		if (opts->op_type != CPERF_PDCP &&
			...
			conf.ff_disable |= RTE_CRYPTODEV_FF_SELWRITY;
...
main(int argc, char **argv)
{
	...
	if (opts.imix_distribution_count != 0) {
		...
		opts.imix_buffer_sizes = rte_malloc(NULL,
					...
					0);
		/*
		 * Callwlate aclwmulated distribution of
		...
		/* Callwlate a random sequence of packet sizes, based on distribution */
		...
		/* Callwlate average buffer size for the IMIX distribution */
		for (i = 0; i < buffer_size_count; i++)
