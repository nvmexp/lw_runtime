...
main_loop(struct cperf_verify_ctx *ctx, enum rte_comp_xform_type type)
{
	...
	if (ops == NULL) {
		RTE_LOG(ERR, USER1,
			"Can't allocate memory for ops strulwres\n");
		return -1;
	}
	...
	for (iter = 0; iter < num_iter; iter++) {
		...
		while (remaining_ops > 0) {
			...
			for (i = 0; i < ops_needed; i++) {
				/*
				 * Callwlate next buffer to attach to operation
				 */
			...
			for (i = 0; i < num_deq; i++) {
				...
				if (op->status ==
				  ...
				  RTE_COMP_OP_STATUS_OUT_OF_SPACE_RECOVERABLE) {
					RTE_LOG(ERR, USER1,
"Out of space error oclwrred due to uncompressible input data expanding to larger than destination buffer. Increase the EXPANSE_RATIO constant to use this data.\n");
					res = -1;
					goto end;
				} else if (op->status !=
