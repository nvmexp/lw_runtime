...
static void
show_selwrity_context(uint16_t portid)
{
	void *p_ctx = rte_eth_dev_get_sec_ctx(portid);
	const struct rte_selwrity_capability *s_cap;
	...
	printf("\t  -- size %u\n",
	       rte_selwrity_session_get_size(p_ctx));

	s_cap = rte_selwrity_capabilities_get(p_ctx);
	if (s_cap) {
...
display_crypto_feature_info(uint64_t x)
{
	...
	printf("\t\t  + SECURITY OFFLOAD (%c)\n",
		(x & RTE_CRYPTODEV_FF_SELWRITY) ? 'y' : 'n');
