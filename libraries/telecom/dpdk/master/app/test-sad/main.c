...
print_usage(void)
{
	fprintf(stdout,
		...
		"[-c <init sad supporting read/write conlwrrency>]\n",
		config.prgname);
	...
	const char *file_name, int rule_tbl)
{
	...
	if (file_name != NULL) {
		f = fopen(file_name, "r");
		if (f == NULL)
			rte_exit(-EILWAL, "failed to open file: %s\n",
				file_name);
	}
	...
	*tbl = rte_zmalloc(NULL, sizeof(struct rule) * *n_entries,
	...
	if (f != NULL) {
		...
		if (ret != 0)
			rte_exit(-EILWAL, "failed to parse file %s\n"
				"rules file must be: "
				"<uint32_t: spi> <space> "
				"<ip_addr: dip> <space> "
				"<ip_addr: sip> <space> "
	...
	while ((opt = getopt(argc, argv, "f:t:n:d:l:r:6b:vpc")) != -1) {
		...
		case 'n':
			...
			if ((errno != 0) || (config.nb_rules == 0) ||
					(endptr[0] != 0)) {
				print_usage();
				rte_exit(-EILWAL, "Invalid option -n\n");
			}
			break;
		case 'd':
			...
			if (ret != 0) {
				print_usage();
				rte_exit(-EILWAL, "Invalid option -d\n");
			}
			break;
		case 'b':
			errno = 0;
			...
			if ((errno != 0) || (config.burst_sz == 0) ||
					...
					(endptr[0] != 0)) {
				print_usage();
				rte_exit(-EILWAL, "Invalid option -b\n");
			}
			break;
		case 'l':
			errno = 0;
			...
			if ((errno != 0) || (config.nb_tuples == 0) ||
					(endptr[0] != 0)) {
				print_usage();
				rte_exit(-EILWAL, "Invalid option -l\n");
			}
			break;
		case 'r':
			errno = 0;
			...
			if ((errno != 0) || (config.fract_rnd_tuples == 0) ||
					...
					(endptr[0] != 0)) {
				print_usage();
				rte_exit(-EILWAL, "Invalid option -r\n");
			}
			break;
		case '6':
			config.ipv6 = 1;
		...
		case 'p':
			...
			break;
		case 'c':
			config.conlwrrent_rw = 1;
			break;
		default:
			print_usage();
			rte_exit(-EILWAL, "Invalid options\n");
		}
	}
}
...
main(int argc, char **argv)
{
	...
	if (config.conlwrrent_rw)
		conf.flags |= RTE_IPSEC_SAD_FLAG_RW_CONLWRRENCY;
