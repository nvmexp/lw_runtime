...
port_init(uint16_t port)
{
	...
	if (promislwous) {
		retval = rte_eth_promislwous_enable(port);
		if (retval != 0) {
			RTE_LOG(ERR, VHOST_PORT,
				"Failed to enable promislwous mode on port %u: %s\n",
...
link_vmdq(struct vhost_dev *vdev, struct rte_mbuf *m)
{
	...
	/* vlan_tag lwrrently uses the device_id. */
...
drain_mbuf_table(struct mbuf_table *tx_q)
{
	...
	lwr_tsc = rte_rdtsc();
	if (unlikely(lwr_tsc - prev_tsc > MBUF_TABLE_DRAIN_TSC)) {
		prev_tsc = lwr_tsc;
 ...
 * main linked list. Synchonization  oclwrs through the use of the
 ...
 *   a simple callwlation like following:
 *
