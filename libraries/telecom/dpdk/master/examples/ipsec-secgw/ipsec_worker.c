...
prepare_out_sessions_tbl(struct sa_ctx *sa_out,
		struct rte_selwrity_session **sess_tbl, uint16_t size)
{
		...
		struct rte_event *ev)
{
	...
	case PKT_TYPE_PLAIN_IPV4:
		if (pkt->ol_flags & PKT_RX_SEC_OFFLOAD) {
			...
			sa = *(struct ipsec_sa **)rte_selwrity_dynfield(pkt);
		}
		...
		struct rte_event *ev)
{
	...
	if (sess->type != RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL) {
		RTE_LOG(ERR, IPSEC, "SA type not supported\n");
		goto drop_pkt_and_exit;
	}

	if (sess->security.ol_flags & RTE_SELWRITY_TX_OLOAD_NEED_MDATA)
		*(struct rte_selwrity_session **)rte_selwrity_dynfield(pkt) =
				sess->security.ses;
...
ipsec_wrkr_non_burst_int_port_drv_mode(struct eh_event_link_info *links,
		uint8_t nb_links)
{
	struct rte_selwrity_session *sess_tbl[RTE_MAX_ETHPORTS] = { NULL };
	...
	while (!force_quit) {
		...
		if (!is_unprotected_port(port_id)) {
			...
			if (rte_selwrity_dynfield_is_registered())
