...
{
	struct eh_app_worker_params lwrr_conf = { {{0} }, NULL};
	...
	/* Populate the lwrr_conf with the capabilities */

	/* Check for Tx internal port */
	if (eh_dev_has_tx_internal_port(eventdev_id))
		lwrr_conf.cap.tx_internal_port = EH_TX_TYPE_INTERNAL_PORT;
	else
		lwrr_conf.cap.tx_internal_port = EH_TX_TYPE_NO_INTERNAL_PORT;

	/* Check for burst mode */
	if (eh_dev_has_burst_mode(eventdev_id))
		lwrr_conf.cap.burst = EH_RX_TYPE_BURST;
	else
		lwrr_conf.cap.burst = EH_RX_TYPE_NON_BURST;

	lwrr_conf.cap.ipsec_mode = conf->ipsec_mode;
	...
	for (i = 0; i < nb_wrkr_param; i++, tmp_wrkr++) {
		...
		if (tmp_wrkr->cap.u64 != lwrr_conf.cap.u64)
			continue;
...
eh_conf_init(void)
{
	...
	eth_core_id = rte_get_next_lcore(0,	/* lwrr core */
					 1,	/* skip main core */
					 0	/* wrap */);
	...
	eth_core_id = rte_get_next_lcore(eth_core_id,	/* lwrr core */
					 1,		/* skip main core */
					 0		/* wrap */);
