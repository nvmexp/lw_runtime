...
static int
extend_sa_arr(struct ipsec_sa **sa_tbl, uint32_t lwr_cnt, uint32_t *lwr_sz)
{
	if (*sa_tbl == NULL) {
	...
	if (lwr_cnt >= *lwr_sz) {
		*sa_tbl = realloc(*sa_tbl,
			*lwr_sz * sizeof(struct ipsec_sa) * 2);
		if (*sa_tbl == NULL)
			return -1;
		/* clean reallocated extra space */
		memset(&(*sa_tbl)[*lwr_sz], 0,
			*lwr_sz * sizeof(struct ipsec_sa));
		*lwr_sz *= 2;
	}
	...
	struct parse_status *status)
{
	...
	if (strcmp(tokens[0], "in") == 0) {
		...
		if (extend_sa_arr(&sa_in, nb_sa_in, &sa_in_sz) < 0)
			return;
		rule = &sa_in[*ri];
		rule->direction = RTE_SELWRITY_IPSEC_SA_DIR_INGRESS;
	} else {
		ri = &nb_sa_out;
		sa_cnt = &sa_out_cnt;
		if (extend_sa_arr(&sa_out, nb_sa_out, &sa_out_sz) < 0)
			return;
		rule = &sa_out[*ri];
		rule->direction = RTE_SELWRITY_IPSEC_SA_DIR_EGRESS;
	}
	...
	for (ti = 2; ti < n_tokens; ti++) {
		...
		if (strcmp(tokens[ti], "type") == 0) {
			...
			if (strcmp(tokens[ti], "inline-crypto-offload") == 0)
				...
				ips->type = RTE_SELWRITY_ACTION_TYPE_NONE;
			else if (strcmp(tokens[ti], "cpu-crypto") == 0)
				ips->type = RTE_SELWRITY_ACTION_TYPE_CPU_CRYPTO;
			else {
		...
		if (strcmp(tokens[ti], "fallback") == 0) {
			...
			if (strcmp(tokens[ti], "lookaside-none") == 0)
				fb->type = RTE_SELWRITY_ACTION_TYPE_NONE;
			else if (strcmp(tokens[ti], "cpu-crypto") == 0)
				fb->type = RTE_SELWRITY_ACTION_TYPE_CPU_CRYPTO;
			else {
		...
		if (strcmp(tokens[ti], "flow-direction") == 0) {
			switch (ips->type) {
			case RTE_SELWRITY_ACTION_TYPE_NONE:
			case RTE_SELWRITY_ACTION_TYPE_CPU_CRYPTO:
				rule->fdir_flag = 1;
				INCREMENT_TOKEN_INDEX(ti, n_tokens, status);
				if (status->status < 0)
					return;
				...
				break;
			case RTE_SELWRITY_ACTION_TYPE_INLINE_CRYPTO:
			case RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL:
			case RTE_SELWRITY_ACTION_TYPE_LOOKASIDE_PROTOCOL:
			default:
				APP_CHECK(0, status,
	...
	if ((ips->type != RTE_SELWRITY_ACTION_TYPE_NONE && ips->type !=
			RTE_SELWRITY_ACTION_TYPE_CPU_CRYPTO) && (portid_p == 0))
 ...
 */
static int
get_spi_proto(uint32_t spi, enum rte_selwrity_ipsec_sa_direction dir,
		struct ip_addr ip_addr[2], uint32_t mask[2])
{
	int32_t rc4, rc6;

	rc4 = sp4_spi_present(spi, dir == RTE_SELWRITY_IPSEC_SA_DIR_INGRESS,
				ip_addr, mask);
	rc6 = sp6_spi_present(spi, dir == RTE_SELWRITY_IPSEC_SA_DIR_INGRESS,
				ip_addr, mask);
		...
		struct socket_ctx *skt_ctx)
{
	...
	for (i = 0; i < nb_entries; i++) {
		...
		*sa = entries[i];
		...
		if (ips->type == RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL ||
			ips->type == RTE_SELWRITY_ACTION_TYPE_INLINE_CRYPTO) {
	...
	const struct rte_ipv4_hdr *v4, struct rte_ipv6_hdr *v6)
{
	...
	prm->ipsec_xform.proto = RTE_SELWRITY_IPSEC_SA_PROTO_ESP;
	prm->ipsec_xform.mode = (IS_TRANSPORT(ss->flags)) ?
		RTE_SELWRITY_IPSEC_SA_MODE_TRANSPORT :
	...
	if (IS_IP4_TUNNEL(ss->flags)) {
		prm->ipsec_xform.tunnel.type = RTE_SELWRITY_IPSEC_TUNNEL_IPV4;
		prm->tun.hdr_len = sizeof(*v4);
		prm->tun.next_proto = rc;
		prm->tun.hdr = v4;
	} else if (IS_IP6_TUNNEL(ss->flags)) {
		prm->ipsec_xform.tunnel.type = RTE_SELWRITY_IPSEC_TUNNEL_IPV6;
...
fill_ipsec_session(struct rte_ipsec_session *ss, struct rte_ipsec_sa *sa)
{
	...
	if (ss->type == RTE_SELWRITY_ACTION_TYPE_INLINE_CRYPTO ||
		ss->type == RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL) {
...
/*
 * Allocate space and init rte_ipsec_sa strlwtures,
 * one per session.
 */
static int
		...
		uint64_t *tx_offloads)
{
	...
	uint32_t idx_sa;
	enum rte_selwrity_session_action_type rule_type;
	...
	*tx_offloads = 0;
	...
	for (idx_sa = 0; idx_sa < nb_sa_in; idx_sa++) {
		...
		if ((rule_type == RTE_SELWRITY_ACTION_TYPE_INLINE_CRYPTO ||
				rule_type ==
				RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL)
				&& rule->portid == port_id)
			*rx_offloads |= DEV_RX_OFFLOAD_SELWRITY;
	}
	...
	for (idx_sa = 0; idx_sa < nb_sa_out; idx_sa++) {
		...
		if ((rule_type == RTE_SELWRITY_ACTION_TYPE_INLINE_CRYPTO ||
				...
				RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL)
				&& rule->portid == port_id)
			*tx_offloads |= DEV_TX_OFFLOAD_SELWRITY;
	}
	return 0;
}
