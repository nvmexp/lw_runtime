...
static int
extend_sp_arr(struct acl6_rules **sp_tbl, uint32_t lwr_cnt, uint32_t *lwr_sz)
{
	if (*sp_tbl == NULL) {
		*sp_tbl = calloc(INIT_ACL_RULE_NUM, sizeof(struct acl6_rules));
		if (*sp_tbl == NULL)
			return -1;
		*lwr_sz = INIT_ACL_RULE_NUM;
		return 0;
	}

	if (lwr_cnt >= *lwr_sz) {
		*sp_tbl = realloc(*sp_tbl,
			*lwr_sz * sizeof(struct acl6_rules) * 2);
		if (*sp_tbl == NULL)
			return -1;
		/* clean reallocated extra space */
		memset(&(*sp_tbl)[*lwr_sz], 0,
			*lwr_sz * sizeof(struct acl6_rules));
		*lwr_sz *= 2;
	}
