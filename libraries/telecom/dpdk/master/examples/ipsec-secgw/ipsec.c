...
static inline void
set_ipsec_conf(struct ipsec_sa *sa, struct rte_selwrity_ipsec_xform *ipsec)
{
	if (ipsec->mode == RTE_SELWRITY_IPSEC_SA_MODE_TUNNEL) {
		...
		struct rte_ipsec_session *ips)
{
	...
	if (ips->type != RTE_SELWRITY_ACTION_TYPE_NONE &&
		...
		if (ips->type == RTE_SELWRITY_ACTION_TYPE_LOOKASIDE_PROTOCOL) {
			...
			ips->security.ses = rte_selwrity_session_create(ctx,
		...
		if (ips->type == RTE_SELWRITY_ACTION_TYPE_CPU_CRYPTO) {
...
{
	int32_t ret = 0;
	struct rte_selwrity_ctx *sec_ctx;
	struct rte_selwrity_session_conf sess_conf = {
		.action_type = ips->type,
		.protocol = RTE_SELWRITY_PROTOCOL_IPSEC,
		{.ipsec = {
	...
	if (ips->type == RTE_SELWRITY_ACTION_TYPE_INLINE_CRYPTO) {
		struct rte_flow_error err;
		const struct rte_selwrity_capability *sec_cap;
		int ret = 0;

		sec_ctx = (struct rte_selwrity_ctx *)
		...
		ips->security.ses = rte_selwrity_session_create(sec_ctx,
		...
		sec_cap = rte_selwrity_capabilities_get(sec_ctx);

		/* iterate until ESP tunnel*/
		while (sec_cap->action != RTE_SELWRITY_ACTION_TYPE_NONE) {
			if (sec_cap->action == ips->type &&
			    sec_cap->protocol ==
				RTE_SELWRITY_PROTOCOL_IPSEC &&
			    sec_cap->ipsec.mode ==
				RTE_SELWRITY_IPSEC_SA_MODE_TUNNEL &&
		...
		if (sec_cap->action == RTE_SELWRITY_ACTION_TYPE_NONE) {
			RTE_LOG(ERR, IPSEC,
		...
		sa->action[0].type = RTE_FLOW_ACTION_TYPE_SELWRITY;
		...
		if (sa->attr.ingress) {
			...
			if (ret)
				...
				(ips->security.ol_flags &
					RTE_SELWRITY_TX_HW_TRAILER_OFFLOAD)) {
			sa->action[1].type =
...
flow_create_failure:
	...
	} else if (ips->type ==	RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL) {
		const struct rte_selwrity_capability *sec_cap;
...
create_ipsec_esp_flow(struct ipsec_sa *sa)
{
	int ret = 0;
	struct rte_flow_error err;
	if (sa->direction == RTE_SELWRITY_IPSEC_SA_DIR_EGRESS) {
		RTE_LOG(ERR, IPSEC,
		...
		uint16_t nb_pkts)
{
	...
	for (i = 0; i < nb_pkts; i++) {
		...
		case RTE_SELWRITY_ACTION_TYPE_LOOKASIDE_PROTOCOL:
			...
			rte_selwrity_attach_session(&priv->cop,
				ips->security.ses);
			break;
		...
		case RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL:
			...
			if (ips->security.ol_flags &
				RTE_SELWRITY_TX_OLOAD_NEED_MDATA)
				rte_selwrity_set_pkt_metadata(
