...
static int32_t promislwous_on = 1;
...
prepare_one_packet(struct rte_mbuf *pkt, struct ipsec_traffic *t)
{
	...
	if (pkt->ol_flags & PKT_RX_SEC_OFFLOAD &&
			rte_selwrity_dynfield_is_registered()) {
		struct ipsec_sa *sa;
		 ...
		 */
		sa = (struct ipsec_sa *)rte_selwrity_get_userdata(ctx,
				*rte_selwrity_dynfield(pkt));
		if (sa == NULL) {
...
print_usage(const char *prgname)
{
	fprintf(stderr, "%s [EAL options] --"
		...
		"     of mbufs will be callwlated based on number of cores,\n"
...
max_session_size(void)
{
	...
	for (cdev_id = 0; cdev_id != n; cdev_id++) {
		...
		sz = rte_selwrity_session_get_size(sec_ctx);
		if (sz > max_sz)
			max_sz = sz;
	}
...
static inline int
inline_ipsec_event_esn_overflow(struct rte_selwrity_ctx *ctx, uint64_t md)
{
	struct ipsec_sa *sa;
	...
	sa = (struct ipsec_sa *)rte_selwrity_get_userdata(ctx, md);
...
create_default_ipsec_flow(uint16_t port_id, uint64_t rx_offloads)
{
	...
	if (!(rx_offloads & DEV_RX_OFFLOAD_SELWRITY))
		return;
	...
	action[0].type = RTE_FLOW_ACTION_TYPE_SELWRITY;
	action[0].conf = NULL;
...
check_event_mode_params(struct eh_conf *eh_conf)
{
	...
	/*
	 * Event mode lwrrently supports only inline protocol sessions.
...
inline_sessions_free(struct sa_ctx *sa_ctx)
{
	...
	for (i = 0; i < sa_ctx->nb_sa; i++) {
		...
		ret = rte_selwrity_session_destroy(
...
static uint32_t
callwlate_nb_mbufs(uint16_t nb_ports, uint16_t nb_crypto_qp, uint32_t nb_rxq,
		uint32_t nb_txq)
{
...
main(int32_t argc, char **argv)
{
	...
	if (nb_bufs_in_pool == 0) {
		...
		nb_bufs_in_pool = callwlate_nb_mbufs(nb_ports, nb_crypto_qp,
						nb_rxq, nb_txq);
	}
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		if (promislwous_on) {
			...
			if (ret != 0)
				rte_exit(EXIT_FAILURE,
					"rte_eth_promislwous_enable: err=%s, port=%d\n",
