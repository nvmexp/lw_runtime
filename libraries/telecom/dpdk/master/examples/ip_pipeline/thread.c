...
static int
match_colwert(struct table_rule_match *mh,
	...
	uint32_t *n_rules)
{
	...
	/* Rule (match, action) colwersion */
	i = 0;
	TAILQ_FOREACH(rule, list, node) {
		status = match_colwert(&rule->match, match_ll_ptr[i], 1);
		if (status)
			goto table_rule_add_bulk_ll_free;

		status = action_colwert(table->a, &rule->action, action_ll_ptr[i]);
		if (status)
			goto table_rule_add_bulk_ll_free;
	...
	struct table_rule_action *action)
{
	...
	if (!pipeline_is_running(p)) {
		...
		status = action_colwert(table->a, action, data_in);
		if (status) {
			free(buffer);
			free(rule);
			return -1;
