	...
	size_t out_size)
{
	...
	if (parser_read_uint32(&p.cpu_id, tokens[9]) != 0) {
		snprintf(out, out_size, MSG_ARG_ILWALID, "cpu_id");
		return;
	}
	...
	size_t out_size)
{
	...
	if (parser_read_uint32(&p.rx.n_queues, tokens[5]) != 0) {
		snprintf(out, out_size, MSG_ARG_ILWALID, "n_queues");
		return;
	}
	if (parser_read_uint32(&p.rx.queue_size, tokens[6]) != 0) {
		snprintf(out, out_size, MSG_ARG_ILWALID, "queue_size");
		return;
	}
	...
	if (strcmp(tokens[11], "promislwous") != 0) {
		snprintf(out, out_size, MSG_ARG_NOT_FOUND, "promislwous");
		return;
	}
	...
	size_t out_size)
{
	...
	for (i = 0; i < RTE_SCHED_TRAFFIC_CLASSES_PER_PIPE; i++)
		if (parser_read_uint64(&subport_profile.tc_rate[i],
				tokens[5 + i]) != 0) {
			snprintf(out, out_size, MSG_ARG_ILWALID, "tc_rate");
			return;
		}

	if (parser_read_uint64(&subport_profile.tc_period, tokens[18]) != 0) {
		snprintf(out, out_size, MSG_ARG_ILWALID, "tc_period");
		return;
	}
	...
	size_t out_size)
{
	...
	if (parser_read_uint8(&p.tc_ov_weight, tokens[19]) != 0) {
		snprintf(out, out_size, MSG_ARG_ILWALID, "tc_ov_weight");
		return;
	}

	for (i = 0; i < RTE_SCHED_BE_QUEUES_PER_PIPE; i++)
		if (parser_read_uint8(&p.wrr_weights[i], tokens[20 + i]) != 0) {
			snprintf(out, out_size, MSG_ARG_ILWALID, "wrr_weights");
			return;
		}
	...
	size_t out_size)
{
	...
	if (parser_read_uint64(&p.rate, tokens[3]) != 0) {
		snprintf(out, out_size, MSG_ARG_ILWALID, "rate");
		return;
	}
	...
	if (parser_read_uint32(&p.frame_overhead, tokens[9]) != 0) {
		snprintf(out, out_size, MSG_ARG_ILWALID, "frame_overhead");
		return;
	}
	...
	size_t out_size)
{
	...
	if (parser_read_uint32(&subport_id, tokens[3]) != 0) {
		snprintf(out, out_size, MSG_ARG_ILWALID, "subport_id");
		return;
	}

	if (parser_read_uint32(&subport_profile_id, tokens[5]) != 0) {
		snprintf(out, out_size, MSG_ARG_ILWALID, "subport_profile_id");
		return;
	}
	...
	size_t out_size)
{
	...
	if (parser_read_uint32(&pipe_id_first, tokens[6]) != 0) {
		snprintf(out, out_size, MSG_ARG_ILWALID, "pipe_id_first");
		return;
	}
	...
	if (parser_read_uint32(&pipe_id_last, tokens[8]) != 0) {
		snprintf(out, out_size, MSG_ARG_ILWALID, "pipe_id_last");
		return;
	}
