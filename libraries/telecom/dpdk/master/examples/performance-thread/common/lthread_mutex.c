...
int lthread_mutex_destroy(struct lthread_mutex *m)
{
	if ((m == NULL) || (m->blocked == NULL)) {
		DIAG_EVENT(m, LT_DIAG_MUTEX_DESTROY, m, POSIX_ERRNO(EILWAL));
		return POSIX_ERRNO(EILWAL);
	}
...
int lthread_mutex_lock(struct lthread_mutex *m)
{
	...
	if ((m == NULL) || (m->blocked == NULL)) {
		DIAG_EVENT(m, LT_DIAG_MUTEX_LOCK, m, POSIX_ERRNO(EILWAL));
		return POSIX_ERRNO(EILWAL);
	}
...
int lthread_mutex_trylock(struct lthread_mutex *m)
{
	...
	if ((m == NULL) || (m->blocked == NULL)) {
		DIAG_EVENT(m, LT_DIAG_MUTEX_TRYLOCK, m, POSIX_ERRNO(EILWAL));
		return POSIX_ERRNO(EILWAL);
	}
...
int lthread_mutex_unlock(struct lthread_mutex *m)
{
	...
	if ((m == NULL) || (m->blocked == NULL)) {
		DIAG_EVENT(m, LT_DIAG_MUTEX_UNLOCKED, m, POSIX_ERRNO(EILWAL));
		return POSIX_ERRNO(EILWAL);
	}
