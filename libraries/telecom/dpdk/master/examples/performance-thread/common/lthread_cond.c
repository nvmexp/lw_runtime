...
{
	if (c == NULL) {
		DIAG_EVENT(c, LT_DIAG_COND_DESTROY, c, POSIX_ERRNO(EILWAL));
		return POSIX_ERRNO(EILWAL);
	}
...
int lthread_cond_wait(struct lthread_cond *c, __rte_unused uint64_t reserved)
{
	...
	if (c == NULL) {
		DIAG_EVENT(c, LT_DIAG_COND_WAIT, c, POSIX_ERRNO(EILWAL));
		return POSIX_ERRNO(EILWAL);
	}
...
int lthread_cond_signal(struct lthread_cond *c)
{
	...
	if (c == NULL) {
		DIAG_EVENT(c, LT_DIAG_COND_SIGNAL, c, POSIX_ERRNO(EILWAL));
		return POSIX_ERRNO(EILWAL);
	}
...
int lthread_cond_broadcast(struct lthread_cond *c)
{
	...
	if (c == NULL) {
		DIAG_EVENT(c, LT_DIAG_COND_BROADCAST, c, POSIX_ERRNO(EILWAL));
		return POSIX_ERRNO(EILWAL);
	}
