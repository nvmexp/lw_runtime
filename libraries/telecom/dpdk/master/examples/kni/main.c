...
/* Colwert string to unsigned number. 0 is returned if error oclwrs */
static uint32_t
...
init_kni(void)
{
	...
	/* Callwlate the maximum number of KNI interfaces that will be used */
	for (i = 0; i < RTE_MAX_ETHPORTS; i++) {
		if (kni_port_params_array[i]) {
	...
	/* Ilwoke rte KNI init to preallocate the ports */
...
init_port(uint16_t port)
{
	...
	if (promislwous_on) {
		ret = rte_eth_promislwous_enable(port);
		if (ret != 0)
			rte_exit(EXIT_FAILURE,
				"Could not enable promislwous mode for port%u: %s\n",
