...
/*
 * This expression is used to callwlate the number of mbufs needed depending on
			  ...
			  __rte_unused void *arg)
{
	...
	/* accumulate total exelwtion time in us when callback is ilwoked */
				...
				struct lcore_conf *qconf)
{
	...
	if (RTE_ETH_IS_IPV4_HDR(m->packet_type)) {
		...
		*((uint64_t *)d_addr_bytes) =
		...
		send_single_packet(m, dst_port);
#else
		/* We don't lwrrently handle IPv6 packets in LPM mode. */
		rte_pktmbuf_free(m);
#endif
	} else
		rte_pktmbuf_free(m);
...
static int main_intr_loop(__rte_unused void *dummy)
{
	...
	uint64_t prev_tsc, diff_tsc, lwr_tsc;
	int i, j, nb_rx;
	uint8_t queueid;
	uint16_t portid;
	struct lcore_conf *qconf;
...
main_telemetry_loop(__rte_unused void *dummy)
{
	struct rte_mbuf *pkts_burst[MAX_PKT_BURST];
	unsigned int lcore_id;
	uint64_t prev_tsc, diff_tsc, lwr_tsc, prev_tel_tsc;
	int i, j, nb_rx;
	uint8_t queueid;
	uint16_t portid;
	struct lcore_conf *qconf;
...
main_legacy_loop(__rte_unused void *dummy)
{
	struct rte_mbuf *pkts_burst[MAX_PKT_BURST];
	unsigned lcore_id;
	uint64_t prev_tsc, diff_tsc, lwr_tsc, tim_res_tsc, hz;
	uint64_t prev_tsc_power = 0, lwr_tsc_power, diff_tsc_power;
	int i, j, nb_rx;
	uint8_t queueid;
	uint16_t portid;
	struct lcore_conf *qconf;
	...
	while (!is_done()) {
		...
		diff_tsc_power = lwr_tsc_power - prev_tsc_power;
		if (diff_tsc_power > tim_res_tsc) {
			rte_timer_manage();
			prev_tsc_power = lwr_tsc_power;
		}
...
parse_args(int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argvopt, "p:l:m:h:P",
		...
		case 'p':
			...
			break;
		case 'P':
			printf("Promislwous mode selected\n");
			promislwous_on = 1;
			break;
		case 'l':
...
init_power_library(void)
{
	...
	RTE_LCORE_FOREACH(lcore_id) {
		...
		if (elw != PM_ELW_ACPI_CPUFREQ &&
				elw != PM_ELW_PSTATE_CPUFREQ) {
			RTE_LOG(ERR, POWER,
...
static void
get_lwrrent_stat_values(uint64_t *values)
{
		...
		__rte_unused void *arg)
{
	...
	get_lwrrent_stat_values(values);
...
{
	int64_t prev_tsc = 0, lwr_tsc, diff_tsc, cycles_10ms;
...
autodetect_mode(void)
{
	 ...
	 */
	if (rte_power_check_elw_supported(PM_ELW_ACPI_CPUFREQ))
		return APP_MODE_LEGACY;
	if (rte_power_check_elw_supported(PM_ELW_PSTATE_CPUFREQ))
		return APP_MODE_LEGACY;
