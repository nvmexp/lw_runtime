...
{
	uint64_t lwrrent_time, last_time = rte_rdtsc();
	...
	while (1) {
		...
		for (i = 0; i < nb_rx; i ++) {
			...
			if (app_pkt_handle(pkt, lwrrent_time) == DROP)
...
main(int argc, char **argv)
{
	...
	ret = rte_eth_promislwous_enable(port_rx);
	if (ret != 0)
		rte_exit(EXIT_FAILURE,
			"Port %d promislwous mode enable error (%s)\n",
			port_rx, rte_strerror(-ret));

	ret = rte_eth_promislwous_enable(port_tx);
	if (ret != 0)
		rte_exit(EXIT_FAILURE,
			"Port %d promislwous mode enable error (%s)\n",
			port_rx, rte_strerror(-ret));
