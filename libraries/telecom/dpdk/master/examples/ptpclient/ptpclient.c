...
port_init(uint16_t port, struct rte_mempool *mbuf_pool)
{
	...
	/* Enable RX in promislwous mode for the Ethernet device. */
	retval = rte_eth_promislwous_enable(port);
	if (retval != 0) {
		printf("Promislwous mode enable failed: %s\n",
			rte_strerror(-retval));
		return retval;
	}
...
print_clock_info(struct ptpv2_data_slave_ordinary *ptp_data)
{
	...
	rte_eth_timesync_read_time(ptp_data->lwrrent_ptp_port, &net_time);

	time_t ts = net_time.tv_sec;

	printf("\n\nComparison between Linux kernel Time and PTP:");

	printf("\nLwrrent PTP Time: %.24s %.9ld ns",
...
parse_drsp(struct ptpv2_data_slave_ordinary *ptp_data)
{
	...
	if (memcmp(&ptp_data->client_clock_id,
		...
		if (seq_id == ptp_data->seqID_FOLLOWUP) {
			...
			ptp_data->lwrrent_ptp_port = ptp_data->portid;
