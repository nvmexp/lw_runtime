...
vhost_blk_ctrlr_find(const char *ctrlr_name)
{
	...
	/* lwrrently we only support 1 socket file fd */
...
static bool
descriptor_has_next_packed(struct vring_packed_desc *lwr_desc)
{
	return !!(lwr_desc->flags & VRING_DESC_F_NEXT);
}

static bool
descriptor_has_next_split(struct vring_desc *lwr_desc)
{
	return !!(lwr_desc->flags & VRING_DESC_F_NEXT);
}
			    ...
			    uint32_t *payload)
{
	...
	do {
		...
		if (desc_payload_to_iovs(ctrlr, iovs, iovs_idx,
			...
			fprintf(stderr, "Failed to colwert desc payload to iovs\n");
			return -1;
		}
