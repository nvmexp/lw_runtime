					...
					const char *vm_name)
{
	...
	json_object_foreach(element, key, value) {
		if (!strcmp(key, "policy")) {
			/* Relwrse in to get the contents of profile */
			ret = parse_json_to_pkt(value, pkt, vm_name);
			if (ret)
				return ret;
		} else if (!strcmp(key, "instruction")) {
			/* Relwrse in to get the contents of instruction */
			ret = parse_json_to_pkt(value, pkt, vm_name);
			if (ret)
				return ret;
...
get_pcpu_to_control(struct policy *pol)
{
	 ...
	 * Virtual cores need to be colwerted to physical cores.
	 */
...
get_pfid(struct policy *pol)
{
	...
	for (i = 0; i < pol->pkt.nb_mac_to_monitor; i++) {
		...
		if (ret == -EILWAL || ret == -ENOTSUP || ret == ENODEV) {
			RTE_LOG(INFO, CHANNEL_MONITOR,
...
get_pkt_diff(struct policy *pol)
{
	...
	double rdtsc_lwrr, rdtsc_diff, diff;
	int x;
#ifdef RTE_NET_I40E
	...
	rdtsc_lwrr = rte_rdtsc_precise();
	rdtsc_diff = rdtsc_lwrr - rdtsc_prev[pol->pfid[x-1]];
	rdtsc_prev[pol->pfid[x-1]] = rdtsc_lwrr;
...
apply_time_profile(struct policy *pol)
{
	...
	/* Obtain the time of day, and colwert it to a tm struct. */
	...
	if (freq_list) {
		...
		for (i = 0; i < info.num_vcpus; i++)
			channel_pkt_freq_list.freq_list[i] =
			  power_manager_get_lwrrent_frequency(info.pcpu_map[i]);
	} else {
		channel_pkt_freq_list.freq_list[vcore_id] =
		  power_manager_get_lwrrent_frequency(info.pcpu_map[vcore_id]);
	}
