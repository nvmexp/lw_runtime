 ...
 * e.g: for N destination ports we need to ilwoke mcast_out_pkt (N-1) times.
 ...
 * need to ilwoke mcast_out_pkt N times.
...
mcast_forward(struct rte_mbuf *m, struct lcore_queue_conf *qconf)
{
	...
	/* Callwlate number of destination ports. */
...
send_timeout_burst(struct lcore_queue_conf *qconf)
{
	...
	lwr_tsc = rte_rdtsc();
	if (likely (lwr_tsc < qconf->tx_tsc + drain_tsc))
		return;
