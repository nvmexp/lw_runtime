...
struct cryptodev_fips_validate_elw {
	const char *req_path;
	const char *rsp_path;
...
cryptodev_fips_validate_app_int(void)
{
	...
	uint32_t sess_sz = rte_cryptodev_sym_get_private_session_size(
			elw.dev_id);
	uint32_t nb_mbufs = UINT16_MAX / elw.mbuf_data_room + 1;
	int ret;

	if (elw.self_test) {
		ret = fips_dev_self_test(elw.dev_id, elw.broken_test_config);
		if (ret < 0) {
			struct rte_cryptodev *cryptodev =
					rte_cryptodev_pmd_get_dev(elw.dev_id);
	...
	ret = rte_cryptodev_configure(elw.dev_id, &conf);
	if (ret < 0)
		return ret;

	rte_cryptodev_info_get(elw.dev_id, &dev_info);
	if (dev_info.feature_flags & RTE_CRYPTODEV_FF_IN_PLACE_SGL)
		elw.dev_support_sgl = 1;
	else
		elw.dev_support_sgl = 0;

	elw.mpool = rte_pktmbuf_pool_create("FIPS_MEMPOOL", nb_mbufs,
			0, 0, sizeof(struct rte_mbuf) + RTE_PKTMBUF_HEADROOM +
			elw.mbuf_data_room, rte_socket_id());
	if (!elw.mpool)
		return ret;

	ret = rte_cryptodev_queue_pair_setup(elw.dev_id, 0, &qp_conf,
			rte_socket_id());
	if (ret < 0)
		return ret;
	...
	elw.sess_mpool = rte_cryptodev_sym_session_pool_create(
			"FIPS_SESS_MEMPOOL", 16, 0, 0, 0, rte_socket_id());
	if (!elw.sess_mpool)
		goto error_exit;

	elw.sess_priv_mpool = rte_mempool_create("FIPS_SESS_PRIV_MEMPOOL",
			16, sess_sz, 0, 0, NULL, NULL, NULL,
			NULL, rte_socket_id(), 0);
	if (!elw.sess_priv_mpool)
		goto error_exit;

	elw.op_pool = rte_crypto_op_pool_create(
			"FIPS_OP_POOL",
	...
	elw.op = rte_crypto_op_alloc(elw.op_pool, RTE_CRYPTO_OP_TYPE_SYMMETRIC);
	if (!elw.op)
		goto error_exit;

	qp_conf.mp_session = elw.sess_mpool;
	qp_conf.mp_session_private = elw.sess_priv_mpool;
...
error_exit:
	...
	rte_mempool_free(elw.mpool);
	if (elw.sess_mpool)
		rte_mempool_free(elw.sess_mpool);
	if (elw.sess_priv_mpool)
		rte_mempool_free(elw.sess_priv_mpool);
	if (elw.op_pool)
		rte_mempool_free(elw.op_pool);
...
{
	rte_pktmbuf_free(elw.mbuf);
	rte_crypto_op_free(elw.op);
	rte_cryptodev_sym_session_clear(elw.dev_id, elw.sess);
	rte_cryptodev_sym_session_free(elw.sess);
	rte_mempool_free(elw.mpool);
	rte_mempool_free(elw.sess_mpool);
	rte_mempool_free(elw.sess_priv_mpool);
	rte_mempool_free(elw.op_pool);
...
parse_cryptodev_id_arg(char *arg)
{
	...
	elw.dev_id = (uint8_t)cryptodev_id;
...
cryptodev_fips_validate_parse_args(int argc, char **argv)
{
	...
	elw.mbuf_data_room = DEF_MBUF_SEG_SIZE;
	...
	while ((opt = getopt_long(argc, argvopt, "s:",
		...
		case 0:
			if (strcmp(lgopts[option_index].name,
				...
				if (!elw.broken_test_config) {
					elw.broken_test_config = rte_malloc(
						NULL,
						sizeof(*elw.broken_test_config),
						0);
					if (!elw.broken_test_config)
						return -ENOMEM;

					elw.broken_test_config->expect_fail_dir =
						self_test_dir_enc_auth_gen;
				}

				if (parser_read_uint32(
					&elw.broken_test_config->expect_fail_test_idx,
						optarg) < 0) {
					rte_free(elw.broken_test_config);
					cryptodev_fips_validate_usage(prgname);
					return -EILWAL;
				}
			} else if (strcmp(lgopts[option_index].name,
				...
				elw.mbuf_data_room = data_room_size;
			} else {
				cryptodev_fips_validate_usage(prgname);
				return -EILWAL;
			}
	...
	if ((elw.req_path == NULL && elw.rsp_path != NULL) ||
			(elw.req_path != NULL && elw.rsp_path == NULL)) {
		RTE_LOG(ERR, USER1, "Missing req path or rsp path\n");
		cryptodev_fips_validate_usage(prgname);
		return -EILWAL;
	}

	if (elw.req_path == NULL && elw.self_test == 0) {
...
main(int argc, char *argv[])
{
	...
	if (!elw.is_path_folder) {
		printf("Processing file %s... ", elw.req_path);

		ret = fips_test_init(elw.req_path, elw.rsp_path,
			rte_cryptodev_name_get(elw.dev_id));
		if (ret < 0) {
		...
		d_rsp = opendir(elw.rsp_path);
		if (!d_rsp) {
			ret = mkdir(elw.rsp_path, 0700);
			if (ret == 0)
				d_rsp = opendir(elw.rsp_path);
			else {
		...
		while ((dir = readdir(d_req)) != NULL) {
			...
			snprintf(req_path, 1023, "%s/%s", elw.req_path,
					dir->d_name);
			snprintf(rsp_path, 1023, "%s/%s", elw.rsp_path,
					dir->d_name);
...
prepare_data_mbufs(struct fips_val *val)
{
	...
	nb_seg = total_len / elw.mbuf_data_room;
	if (total_len % elw.mbuf_data_room)
		nb_seg++;

	m = rte_pktmbuf_alloc(elw.mpool);
	if (!m) {
	...
	while (nb_seg) {
		uint16_t len = RTE_MIN(total_len, elw.mbuf_data_room);
...
{
	struct rte_crypto_sym_op *sym = elw.op->sym;
	uint8_t *iv = rte_crypto_op_ctod_offset(elw.op, uint8_t *, IV_OFF);
	int ret;

	__rte_crypto_op_reset(elw.op, RTE_CRYPTO_OP_TYPE_SYMMETRIC);
	...
	rte_crypto_op_attach_sym_session(elw.op, elw.sess);
...
prepare_auth_op(void)
{
	...
	if (vec.iv.len) {
		uint8_t *iv = rte_crypto_op_ctod_offset(elw.op, uint8_t *,
				IV_OFF);
	...
	elw.digest = rte_zmalloc(NULL, vec.cipher_auth.digest.len,
			RTE_CACHE_LINE_SIZE);
	if (!elw.digest) {
		RTE_LOG(ERR, USER1, "Not enough memory\n");
		return -ENOMEM;
	}
	elw.digest_len = vec.cipher_auth.digest.len;

	sym->m_src = elw.mbuf;
	sym->auth.data.offset = 0;
	sym->auth.data.length = vec.pt.len;
	sym->auth.digest.data = elw.digest;
	sym->auth.digest.phys_addr = rte_malloc_virt2iova(elw.digest);

	if (info.op == FIPS_TEST_DEC_AUTH_VERIF)
		memcpy(elw.digest, vec.cipher_auth.digest.val,
...
prepare_aead_op(void)
{
	...
	if (info.op == FIPS_TEST_ENC_AUTH_GEN) {
		...
		if (elw.digest)
			rte_free(elw.digest);
		elw.digest = rte_zmalloc(NULL, vec.aead.digest.len,
				RTE_CACHE_LINE_SIZE);
		if (!elw.digest) {
			RTE_LOG(ERR, USER1, "Not enough memory\n");
			return -ENOMEM;
		...
		sym->aead.digest.data = elw.digest;
		sym->aead.digest.phys_addr = rte_malloc_virt2iova(elw.digest);
	} else {
		ret = prepare_data_mbufs(&vec.ct);
		if (ret < 0)
			return ret;
...
prepare_aes_xform(struct rte_crypto_sym_xform *xform)
{
	...
	cap = rte_cryptodev_sym_capability_get(elw.dev_id, &cap_idx);
	if (!cap) {
...
{
	struct rte_mbuf *m = elw.mbuf;
	uint16_t data_len = rte_pktmbuf_pkt_len(m);
	uint16_t total_len = data_len + elw.digest_len;
	...
	if (elw.digest)
		memcpy(dst, elw.digest, elw.digest_len);
...
fips_run_test(void)
{
	...
	elw.sess = rte_cryptodev_sym_session_create(elw.sess_mpool);
	if (!elw.sess)
		return -ENOMEM;

	ret = rte_cryptodev_sym_session_init(elw.dev_id,
			elw.sess, &xform, elw.sess_priv_mpool);
	if (ret < 0) {
	...
	if (rte_cryptodev_enqueue_burst(elw.dev_id, 0, &elw.op, 1) < 1) {
	...
	do {
		...
		n_deqd = rte_cryptodev_dequeue_burst(elw.dev_id, 0, &deqd_op,
				1);
	} while (n_deqd == 0);

	vec.status = elw.op->status;
