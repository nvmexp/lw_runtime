...
run_single_test(uint8_t dev_id,
		...
		struct fips_dev_auto_test_elw *elw,
		uint32_t dir,
		uint32_t negative_test)
{
	...
	rte_pktmbuf_reset(elw->mbuf);
	elw->op->sym->m_src = elw->mbuf;
	...
	sess = rte_cryptodev_sym_session_create(elw->sess_pool);
	if (!sess)
		return -ENOMEM;

	ret = rte_cryptodev_sym_session_init(dev_id,
			sess, &xform, elw->sess_priv_pool);
	if (ret < 0) {
		RTE_LOG(ERR, PMD, "Error %i: Init session\n", ret);
		return ret;
	}

	ret = test_ops->prepare_op(elw->op, elw->mbuf, sess, dir, vec);
	if (ret < 0) {
		RTE_LOG(ERR, PMD, "Error %i: Prepare op\n", ret);
		return ret;
	}

	if (rte_cryptodev_enqueue_burst(dev_id, 0, &elw->op, 1) < 1) {
	...
	if (elw->op->status != RTE_CRYPTO_OP_STATUS_SUCCESS)
		return -1;

	return test_ops->check_result(elw->op, vec, dir);
}
		...
		struct fips_dev_auto_test_elw *elw)
{
	...
	if (elw->sess_pool)
		rte_mempool_free(elw->sess_pool);
	if (elw->sess_priv_pool)
		rte_mempool_free(elw->sess_priv_pool);
...
static int
fips_dev_auto_test_init(uint8_t dev_id, struct fips_dev_auto_test_elw *elw)
{
	...
	memset(elw, 0, sizeof(*elw));

	elw->mpool = rte_pktmbuf_pool_create(name, 128, 0, 0,
	...
	elw->sess_pool = rte_cryptodev_sym_session_pool_create(name,
	...
	elw->sess_priv_pool = rte_mempool_create(name,
	...
	qp_conf.mp_session = elw->sess_pool;
	qp_conf.mp_session_private = elw->sess_priv_pool;
	...
	elw->mbuf = rte_pktmbuf_alloc(elw->mpool);
	if (!elw->mbuf) {
		ret = -ENOMEM;
		goto error_exit;
	}

	elw->op = rte_crypto_op_alloc(elw->op_pool,
			RTE_CRYPTO_OP_TYPE_SYMMETRIC);
	if (!elw->op) {
		ret = -ENOMEM;
		goto error_exit;
...
error_exit:
	...
	fips_dev_auto_test_uninit(dev_id, elw);
		...
		struct fips_dev_broken_test_config *config)
{
	...
	ret = fips_dev_auto_test_init(dev_id, &elw);
	if (ret < 0) {
