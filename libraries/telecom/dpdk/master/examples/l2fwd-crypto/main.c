...
parse_bytes(uint8_t *data, char *input_arg, uint16_t max_size)
{
	...
	for (byte_count = 0, token = strtok(input_arg, ":");
		...
		if (errno == EILWAL || errno == ERANGE || number > 0xFF)
			return -1;
		...
		uint8_t *enabled_cdevs)
{
	...
	for (cdev_id = 0; cdev_id < cdev_count; cdev_id++) {
		...
		struct rte_cryptodev_config conf = {
			.nb_queue_pairs = 1,
			.socket_id = socket_id,
			.ff_disable = RTE_CRYPTODEV_FF_SELWRITY,
		};
...
initialize_ports(struct l2fwd_crypto_options *options)
{
	...
	RTE_ETH_FOREACH_DEV(portid) {
		...
		retval = rte_eth_promislwous_enable(portid);
		if (retval != 0) {
			printf("rte_eth_promislwous_enable:err=%s, port=%u\n",
