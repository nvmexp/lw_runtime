...
static uint32_t
colwert_depth_to_bitmask(uint32_t depth_val)
{
	uint32_t bitmask = 0;
	int i, j;
		...
		struct flow_classifier *cls_app)
{
	...
	ipv4_mask.hdr.src_addr =
		colwert_depth_to_bitmask(ipv4_mask.hdr.src_addr);
	ipv4_mask.hdr.dst_addr = ntuple_filter->dst_ip_mask;
	ipv4_mask.hdr.dst_addr =
		colwert_depth_to_bitmask(ipv4_mask.hdr.dst_addr);
...
print_usage(const char *prgname)
{
	...
	printf("Each rule oclwpies one line in the file.\n");
}
