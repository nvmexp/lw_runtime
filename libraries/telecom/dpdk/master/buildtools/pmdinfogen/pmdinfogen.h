...
/*
 * Note, it seems odd that we have both a COLWERT_NATIVE and a TO_NATIVE macro
 ...
 */
#define COLWERT_NATIVE(fend, width, x) ({ \
typeof(x) ___x; \
if ((fend) == ELFDATA2LSB) \
	___x = le##width##toh(x); \
else \
...
#define TO_NATIVE(fend, width, x) COLWERT_NATIVE(fend, width, x)
