 ...
 */
static int
mvneta_promislwous_enable(struct rte_eth_dev *dev)
{
	struct mvneta_priv *priv = dev->data->dev_private;
	int ret, en;
	...
	if (en) {
		MVNETA_LOG(INFO, "Promislwous already enabled");
		return 0;
	}

	ret = neta_ppio_set_promisc(priv->ppio, 1);
	if (ret)
		MVNETA_LOG(ERR, "Failed to enable promislwous mode");
 ...
 */
static int
mvneta_promislwous_disable(struct rte_eth_dev *dev)
{
	struct mvneta_priv *priv = dev->data->dev_private;
	int ret, en;
	...
	neta_ppio_get_promisc(priv->ppio, &en);
	if (!en) {
		MVNETA_LOG(INFO, "Promislwous already disabled");
		return 0;
	}

	ret = neta_ppio_set_promisc(priv->ppio, 0);
	if (ret)
		MVNETA_LOG(ERR, "Failed to disable promislwous mode");
...
static const struct eth_dev_ops mvneta_ops = {
	...
	.promislwous_enable = mvneta_promislwous_enable,
	.promislwous_disable = mvneta_promislwous_disable,
