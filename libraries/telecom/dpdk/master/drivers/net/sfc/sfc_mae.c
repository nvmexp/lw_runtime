		       ...
		       struct sfc_mae_outer_rule **rulep)
{
	...
	rule->fw_rsrc.rule_id.id = EFX_MAE_RSRC_ID_ILWALID;
		       ...
		       struct sfc_mae_outer_rule *rule)
{
	...
	SFC_ASSERT(rule->fw_rsrc.rule_id.id == EFX_MAE_RSRC_ID_ILWALID);
			  ...
			  efx_mae_match_spec_t *match_spec_action)
{
	...
	if (fw_rsrc->refcnt == 0) {
		SFC_ASSERT(fw_rsrc->rule_id.id == EFX_MAE_RSRC_ID_ILWALID);
			   ...
			   struct sfc_mae_outer_rule *rule)
{
	...
	if (fw_rsrc->refcnt == 1) {
		...
		fw_rsrc->rule_id.id = EFX_MAE_RSRC_ID_ILWALID;
	}
		       ...
		       struct sfc_mae_action_set **action_setp)
{
	...
	action_set->fw_rsrc.aset_id.id = EFX_MAE_RSRC_ID_ILWALID;
		       ...
		       struct sfc_mae_action_set *action_set)
{
	...
	SFC_ASSERT(action_set->fw_rsrc.aset_id.id == EFX_MAE_RSRC_ID_ILWALID);
			  ...
			  struct sfc_mae_action_set *action_set)
{
	...
	if (fw_rsrc->refcnt == 0) {
		SFC_ASSERT(fw_rsrc->aset_id.id == EFX_MAE_RSRC_ID_ILWALID);
			   ...
			   struct sfc_mae_action_set *action_set)
{
	...
	if (fw_rsrc->refcnt == 1) {
		...
		fw_rsrc->aset_id.id = EFX_MAE_RSRC_ID_ILWALID;
	}
		     ...
		     struct rte_flow *flow)
{
	...
	SFC_ASSERT(spec_mae->rule_id.id == EFX_MAE_RSRC_ID_ILWALID);
				 ...
				 struct rte_flow_error *error)
{
	...
	if (rc != 0) {
		return rte_flow_error_set(error, rc,
				RTE_FLOW_ERROR_TYPE_ITEM, item,
				"Failed to colwert the PHY_PORT index");
	}
			   ...
			   struct rte_flow_error *error)
{
	...
	rc = efx_mae_mport_by_pcie_function(encp->enc_pf, EFX_PCI_VF_ILWALID,
					    &mport_v);
	if (rc != 0) {
		return rte_flow_error_set(error, rc,
				RTE_FLOW_ERROR_TYPE_ITEM, item,
				"Failed to colwert the PF ID");
	}
			   ...
			   struct rte_flow_error *error)
{
	...
	if (rc != 0) {
		return rte_flow_error_set(error, rc,
				RTE_FLOW_ERROR_TYPE_ITEM, item,
				"Failed to colwert the PF + VF IDs");
	}
			   ...
			   struct rte_flow_error *error)
{
	...
	*rulep = sfc_mae_outer_rule_attach(sa, ctx->match_spec_outer,
	 ...
	 * EFX_MAE_RSRC_ID_ILWALID. Set it in the action rule match
			      ...
			      struct rte_flow_error *error)
{
	...
	for (;;) {
		...
		case RTE_FLOW_ITEM_TYPE_LWGRE:
			ctx->encap_type = EFX_TUNNEL_PROTOCOL_LWGRE;
			ctx->tunnel_def_mask = &rte_flow_item_lwgre_mask;
			RTE_BUILD_BUG_ON(sizeof(ctx->tunnel_def_mask) !=
					 sizeof(rte_flow_item_lwgre_mask));
			break;
 ...
 *
 * A lwrrently tracked bundle will be submitted if a repeating
				...
				struct sfc_mae_outer_rule *rule)
{
	...
	if (fw_rsrc->rule_id.id != EFX_MAE_RSRC_ID_ILWALID) {
		    ...
		    struct rte_flow *flow)
{
	...
	spec_mae->rule_id.id = EFX_MAE_RSRC_ID_ILWALID;
