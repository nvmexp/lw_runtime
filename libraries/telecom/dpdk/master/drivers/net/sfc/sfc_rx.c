...
sfc_rx_default_rxq_set_filter(struct sfc_adapter *sa, struct sfc_rxq *rxq)
{
	...
	/*
	 * If promislwous or all-multicast mode has been requested, setting
	 ...
	 * privilege groups; if this oclwrs, few iterations will be made to
	 * repeat this step without promislwous and all-multicast flags set
	 */
retry:
	...
	if (port->promisc) {
		sfc_warn(sa, "promislwous mode has been requested, "
			     "but the HW rejects it");
		sfc_warn(sa, "promislwous mode will be disabled");

		port->promisc = B_FALSE;
		sa->eth_dev->data->promislwous = 0;
...
sfc_rx_mb_pool_buf_size(struct sfc_adapter *sa, struct rte_mempool *mb_pool)
{
	...
	/* Callwlate guaranteed data start alignment */
	     ...
	     struct rte_mempool *mb_pool)
{
	...
	if (!sfc_rx_check_scatter(sa->port.pdu, buf_size,
		...
		sfc_err(sa, "RxQ %u callwlated Rx buffer size is %u vs "
