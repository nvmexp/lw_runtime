...
/*
 * Lwrrently, filter-based (VNIC) flow API is implemented in such a manner
 * that each flow rule is colwerted to one or more hardware filters.
...
static sfc_flow_item_parse sfc_flow_parse_lwgre;
...
/**
 * Colwert Ethernet item to EFX filter specification.
 *
 * @param item[in]
...
/**
 * Colwert VLAN item to EFX filter specification.
 *
 * @param item[in]
...
/**
 * Colwert IPv4 item to EFX filter specification.
 *
 * @param item[in]
...
/**
 * Colwert IPv6 item to EFX filter specification.
 *
 * @param item[in]
 *   Item specification. Only source and destination addresses and
...
/**
 * Colwert TCP item to EFX filter specification.
 *
 * @param item[in]
...
/**
 * Colwert UDP item to EFX filter specification.
 *
 * @param item[in]
 *   Item specification. Only source and destination ports fields
 *   are supported. If the mask is NULL, default mask will be used.
					...
					struct rte_flow_error *error)
{
	if (!(efx_spec->efs_match_flags & EFX_FILTER_MATCH_IP_PROTO)) {
		...
		default:
			rte_flow_error_set(error, EILWAL,
				RTE_FLOW_ERROR_TYPE_ITEM, item,
				"Only VxLAN/GENEVE/LWGRE tunneling patterns "
				"are supported");
			return -rte_errno;
		}
	}
...
/**
 * Colwert VXLAN item to EFX filter specification.
 *
 * @param item[in]
...
/**
 * Colwert GENEVE item to EFX filter specification.
 *
 * @param item[in]
...
/**
 * Colwert LWGRE item to EFX filter specification.
 *
 * @param item[in]
 ...
 */
static int
sfc_flow_parse_lwgre(const struct rte_flow_item *item,
		     struct sfc_flow_parse_ctx *parse_ctx,
		     struct rte_flow_error *error)
{
	int rc;
	efx_filter_spec_t *efx_spec = parse_ctx->filter;
	const struct rte_flow_item_lwgre *spec = NULL;
	const struct rte_flow_item_lwgre *mask = NULL;
	const struct rte_flow_item_lwgre supp_mask = {
		.tni = { 0xff, 0xff, 0xff }
	};
	...
	efx_spec->efs_encap_type = EFX_TUNNEL_PROTOCOL_LWGRE;
	efx_spec->efs_match_flags |= EFX_FILTER_MATCH_ENCAP_TYPE;
