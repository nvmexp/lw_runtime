...
{
	const unsigned int lwrr_done = txq->completed - 1;
	unsigned int anew_done = lwrr_done;
	efx_qword_t tx_ev;
	...
	return (anew_done - lwrr_done) & txq->ptr_mask;
}
...
#define SFC_MBUF_SEG_LEN_MAX		UINT16_MAX
	...
	/*
	 * Each segment is already counted once below.  So, callwlate
		  ...
		  bool *reap_done)
{
	...
	*reap_done = true;
	...
	/*
	 * Recallwlate DMA descriptor space since Tx reap may change
