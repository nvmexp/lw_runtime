...
sfc_set_fw_subvariant(struct sfc_adapter *sa)
{
	...
	efx_nic_fw_subvariant_t lwr_fw_subvariant;
	int rc;
	...
	rc = efx_nic_get_fw_subvariant(sa->nic, &lwr_fw_subvariant);
	if (rc != 0) {
		sfc_err(sa, "failed to get FW subvariant: %d", rc);
		return rc;
	}
	sfc_info(sa, "FW subvariant is %u vs required %u",
		 lwr_fw_subvariant, req_fw_subvariant);

	if (lwr_fw_subvariant == req_fw_subvariant)
		return 0;
...
sfc_start(struct sfc_adapter *sa)
{
	...
	do {
		...
		rc = sfc_try_start(sa);
		 ...
		 (rc == EIO || rc == EAGAIN || rc == ENOENT || rc == EILWAL));
