...
enum ecore_lwm_images {
	ECORE_LWM_IMAGE_ISCSI_CFG,
	ECORE_LWM_IMAGE_FCOE_CFG,
	ECORE_LWM_IMAGE_MDUMP,
	ECORE_LWM_IMAGE_LWM_CFG1,
	ECORE_LWM_IMAGE_DEFAULT_CFG,
	ECORE_LWM_IMAGE_LWM_META,
};
#endif
 ...
 * @param p_running_bundle_id	- image id in lwram; Optional.
 *
 ...
 *  @param p_dev
 *  @param addr - lwm offset
 *  @param cmd - lwm command
 *  @param p_buf - lwm write buffer
 ...
 * @return enum _ecore_status_t - ECORE_SUCCESS - operation was successful.
 */
enum _ecore_status_t ecore_mcp_lwm_del_file(struct ecore_dev *p_dev,
					    u32 addr);
...
struct ecore_lwm_image_att {
	u32 start_addr;
	u32 length;
};

/**
 * @brief Allows reading a whole lwram image
 *
 * @param p_hwfn
 ...
 *
 * @return ECORE_SUCCESS if p_buffer now contains the lwram image.
 */
...
/**
 * @brief - Sends an LWM write command request to the MFW with
 *          payload.
 *
 * @param p_hwfn
 * @param p_ptt
 * @param cmd - Command: Either DRV_MSG_CODE_LWM_WRITE_LWRAM or
 *            DRV_MSG_CODE_LWM_PUT_FILE_DATA
...
/**
 * @brief - Sends an LWM read command request to the MFW to get
 *        a buffer.
 *
 * @param p_hwfn
 * @param p_ptt
 * @param cmd - Command: DRV_MSG_CODE_LWM_GET_FILE_DATA or
 *            DRV_MSG_CODE_LWM_READ_LWRAM commands
 * @param param - [0:23] - Offset [24:31] - Size
 * @param o_mcp_resp - MCP response
 * @param o_mcp_param - MCP response param
...
/**
 * @brief Bist lwm test - get number of images
 *
 *  @param p_hwfn       - hw function
 *  @param p_ptt        - PTT required for register access
...
/**
 * @brief Bist lwm test - get image attributes by index
 *
 *  @param p_hwfn      - hw function
 *  @param p_ptt       - PTT required for register access
