			...
			struct ecore_queue_cid_vf_params *p_vf_params)
{
	...
	/* Don't try callwlating the absolute indices for VFs */
	...
	/* Callwlate the engine-absolute indices of the resources.
...
static u32 ecore_calc_crc32c(u8 *crc32_packet, u32 crc32_length, u32 crc32_seed)
{
	...
	for (byte = 0; byte < crc32_length; byte++) {
		lwrrent_byte = crc32_packet[byte];
		for (bit = 0; bit < 8; bit++) {
			msb = (u8)(crc32_result >> 31);
			crc32_result = crc32_result << 1;
			if (msb != (0x1 & (lwrrent_byte >> bit))) {
