...
struct ecore_chain {
	...
	/* Elements information for fast callwlations */
	u16 elem_per_page;
	 ...
	 * but isn't ilwolved in regular functionality.
	 */
 ...
 *
 * @param p_chain
 * @param p_virt_lwrr	virtual address of a chain page of which the next
 ...
 */
static OSAL_INLINE void
ecore_chain_init_next_ptr_elem(struct ecore_chain *p_chain, void *p_virt_lwrr,
			       void *p_virt_next, dma_addr_t p_phys_next)
{
	...
	p_next = (struct ecore_chain_next *)((u8 *)p_virt_lwrr + size);
...
{
	if (p_chain->mode == ECORE_CHAIN_MODE_PBL) {
		u32 lwr_prod, page_mask, page_cnt, page_diff;

		lwr_prod = is_chain_u16(p_chain) ? p_chain->u.chain16.prod_idx
						 : p_chain->u.chain32.prod_idx;

		/* Assume that number of elements in a page is power of 2 */
		page_mask = ~p_chain->elem_per_page_mask;

		/* Use "lwr_prod - 1" and "prod_idx - 1" since producer index
		 ...
		 */
		page_diff = (((lwr_prod - 1) & page_mask) -
...
{
	if (p_chain->mode == ECORE_CHAIN_MODE_PBL) {
		u32 lwr_cons, page_mask, page_cnt, page_diff;

		lwr_cons = is_chain_u16(p_chain) ? p_chain->u.chain16.cons_idx
						 : p_chain->u.chain32.cons_idx;

		/* Assume that number of elements in a page is power of 2 */
		page_mask = ~p_chain->elem_per_page_mask;

		/* Use "lwr_cons - 1" and "cons_idx - 1" since consumer index
		 * reaches the first element of next page before the page index
		 ...
		 * positive and lower than the chain's capacity.
		 */
		page_diff = (((lwr_cons - 1) & page_mask) -
