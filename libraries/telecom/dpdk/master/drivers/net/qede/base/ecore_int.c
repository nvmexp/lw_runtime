...
struct aeu_ilwert_reg {
	struct aeu_ilwert_reg_bit bits[32];
};
					  ...
					  struct ecore_ptt *p_ptt)
{
	...
	if (!overflow) {
		ecore_db_recovery_exelwte(p_hwfn, DB_REC_ONCE);
		return ECORE_SUCCESS;
	}
	...
	ecore_db_recovery_exelwte(p_hwfn, DB_REC_REAL_DEAL);
 ...
 */
enum aeu_ilwert_reg_special_type {
	AEU_ILWERT_REG_SPECIAL_CNIG_0,
	AEU_ILWERT_REG_SPECIAL_CNIG_1,
	AEU_ILWERT_REG_SPECIAL_CNIG_2,
	AEU_ILWERT_REG_SPECIAL_CNIG_3,
	AEU_ILWERT_REG_SPECIAL_MCP_UMP_TX,
	AEU_ILWERT_REG_SPECIAL_MCP_SCPAD,
	AEU_ILWERT_REG_SPECIAL_MAX,
};

static struct aeu_ilwert_reg_bit
aeu_descs_special[AEU_ILWERT_REG_SPECIAL_MAX] = {
...
/* Notice aeu_ilwert_reg must be defined in the same order of bits as HW; */
static struct aeu_ilwert_reg aeu_descs[NUM_ATTN_REGS] = {
	{
	 {			/* After Ilwert 1 */
...

	{
	 {			/* After Ilwert 4 */
	  ...
	  {"NWS Parity", ATTENTION_PAR | ATTENTION_BB_DIFFERENT |
			 ATTENTION_BB(AEU_ILWERT_REG_SPECIAL_CNIG_0),
			 OSAL_NULL, BLOCK_NWS},
	  {"NWS Interrupt", ATTENTION_SINGLE | ATTENTION_BB_DIFFERENT |
			    ATTENTION_BB(AEU_ILWERT_REG_SPECIAL_CNIG_1),
			    OSAL_NULL, BLOCK_NWS},
	  {"NWM Parity", ATTENTION_PAR | ATTENTION_BB_DIFFERENT |
			 ATTENTION_BB(AEU_ILWERT_REG_SPECIAL_CNIG_2),
			 OSAL_NULL, BLOCK_NWM},
	  {"NWM Interrupt", ATTENTION_SINGLE | ATTENTION_BB_DIFFERENT |
			    ATTENTION_BB(AEU_ILWERT_REG_SPECIAL_CNIG_3),
			    OSAL_NULL, BLOCK_NWM},
...

	{
	 {			/* After Ilwert 9 */
	  ...
	  {"AVS", ATTENTION_PAR | ATTENTION_BB_DIFFERENT |
	   ATTENTION_BB(AEU_ILWERT_REG_SPECIAL_MCP_UMP_TX), OSAL_NULL,
	   BLOCK_AVS_WRAP},
	  {"AVS", ATTENTION_SINGLE | ATTENTION_BB_DIFFERENT |
	   ATTENTION_BB(AEU_ILWERT_REG_SPECIAL_MCP_SCPAD), OSAL_NULL,
	   BLOCK_AVS_WRAP},
...
ecore_int_aeu_translate(struct ecore_hwfn *p_hwfn,
			struct aeu_ilwert_reg_bit *p_bit)
{
...
struct ecore_sb_attn_info {
	...
	struct aeu_ilwert_reg *p_aeu_desc;
...
ecore_int_deassertion_aeu_bit(struct ecore_hwfn *p_hwfn,
			      struct aeu_ilwert_reg_bit *p_aeu,
			      u32 aeu_en_reg,
...
#define MISC_REG_AEU_AFTER_ILWERT_IGU(n) \
	(MISC_REG_AEU_AFTER_ILWERT_1_IGU + (n) * 0x4)
...
{
	struct ecore_sb_attn_info *sb_attn_sw = p_hwfn->p_sb_attn;
	u32 aeu_ilw_arr[NUM_ATTN_REGS], aeu_mask, aeu_en, en;
	u8 i, j, k, bit_idx;
	enum _ecore_status_t rc = ECORE_SUCCESS;

	/* Read the attention registers in the AEU */
	for (i = 0; i < NUM_ATTN_REGS; i++) {
		aeu_ilw_arr[i] = ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt,
					  MISC_REG_AEU_AFTER_ILWERT_IGU(i));
		DP_VERBOSE(p_hwfn, ECORE_MSG_INTR,
	...
	for (i = 0; i < NUM_ATTN_REGS; i++) {
		struct aeu_ilwert_reg *p_aeu = &sb_attn_sw->p_aeu_desc[i];
		u32 parities;

		aeu_en = MISC_REG_AEU_ENABLE_IGU_OUT(i, 0);
		en = ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, aeu_en);
		parities = sb_attn_sw->parity_mask[i] & aeu_ilw_arr[i] & en;

		/* Skip register in which no parity bit is lwrrently set */
		if (!parities)
			continue;

		for (j = 0, bit_idx = 0; bit_idx < 32; j++) {
			struct aeu_ilwert_reg_bit *p_bit = &p_aeu->bits[j];
	...
	for (k = 0; k < MAX_ATTN_GRPS; k++) {
		...
		/* Handle only groups whose attention is lwrrently deasserted */
		...
		for (i = 0; i < NUM_ATTN_REGS; i++) {
			...
			en = ecore_rd(p_hwfn, p_hwfn->p_dpc_ptt, aeu_en);
			bits = aeu_ilw_arr[i] & en;

			/* Skip if no bit from this group is lwrrently set */
			if (!bits)
				continue;
...
u16 ecore_get_igu_sb_id(struct ecore_hwfn *p_hwfn, u16 sb_id)
{
	...
	if (igu_sb_id == ECORE_SB_ILWALID_IDX)
				       ...
				       dma_addr_t sb_phy_addr, u16 sb_id)
{
	...
	if (sb_info->igu_sb_id == ECORE_SB_ILWALID_IDX)
		return ECORE_ILWAL;
				     ...
				     u16 opaque_fid)
{
	...
	/* callwlate where to read the status bit from */
				    ...
				    struct ecore_ptt *p_ptt)
{
	 ...
	 * SBs, and would re-callwlate the originals based on those.
	 */
					    ...
					    struct ecore_ptt *p_ptt)
{
	...
	p_igu_info->igu_dsb_id = ECORE_SB_ILWALID_IDX;
	...
	for (igu_sb_id = 0;
		...
		if ((p_block->is_pf) &&
			...
			if (p_igu_info->igu_dsb_id != ECORE_SB_ILWALID_IDX)
