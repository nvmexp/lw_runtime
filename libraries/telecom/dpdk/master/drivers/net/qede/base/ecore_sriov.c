...
static u8 ecore_vf_callwlate_legacy(struct ecore_vf_info *p_vf)
{
	u8 legacy = 0;
				...
				u8 num_sbs)
{
	 ...
	 * the lwrrently enabled child VFs, and set the number to be that.
	 */
	if (!ECORE_IS_BB(p_hwfn->p_dev)) {
		ecore_for_each_vf(p_hwfn, i) {
			...
			lwrrent_max = OSAL_MAX_T(u8, lwrrent_max,
						 p_vf->num_sbs);
		}
	}

	if (num_sbs > lwrrent_max)
...
/**
 *
 * @brief The function ilwalidates all the VF entries,
				      ...
				      struct ecore_vf_info *vf)
{
	...
	/* Ilwalidate igu CAM lines and mark them as free */
			 ...
			 struct ecore_iov_vf_init_params *p_params)
{
	...
	/* Since it's possible to relocate SBs, it's a bit diffilwlt to check
					   ...
					   u16 expected_tlv)
{
	...
	/* record the locking op */
	/* vf->op_lwrrent = CHANNEL_TLV_NONE; */
}
				 ...
				 u64 events)
{
	...
	if ((events & (1 << MAC_ADDR_FORCED)) ||
		...
		/* Since there's no way [lwrrently] of removing the MAC,
				       ...
				       struct ecore_vf_info *vf)
{
	...
	if (qid_usage_idx == ECORE_IOV_QID_ILWALID)
		goto out;
	...
	vf_legacy = ecore_vf_callwlate_legacy(vf);
	 ...
	 * callwlate the location by their own and clean them prior to this.
	 */
...
static enum _ecore_status_t
ecore_iov_exelwte_vf_flr_cleanup(struct ecore_hwfn *p_hwfn,
				 struct ecore_ptt *p_ptt,
					      ...
					      struct ecore_ptt *p_ptt)
{
	...
	for (i = 0; i < p_hwfn->p_dev->p_iov_info->total_vfs; i++)
		ecore_iov_exelwte_vf_flr_cleanup(p_hwfn, p_ptt, i, ack_vfs);
				...
				struct ecore_ptt *p_ptt, u16 rel_vf_id)
{
	...
	ecore_iov_exelwte_vf_flr_cleanup(p_hwfn, p_ptt, rel_vf_id, ack_vfs);
