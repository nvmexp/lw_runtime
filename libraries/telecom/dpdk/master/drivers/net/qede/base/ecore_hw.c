...
/* Invalid values */
#define ECORE_BAR_ILWALID_OFFSET	(OSAL_CPU_TO_LE32(-1))
...
enum _ecore_status_t ecore_ptt_pool_alloc(struct ecore_hwfn *p_hwfn)
{
	...
	for (i = 0; i < PXP_EXTERNAL_BAR_PF_WINDOW_NUM; i++) {
		p_pool->ptts[i].idx = i;
		p_pool->ptts[i].pxp.offset = ECORE_BAR_ILWALID_OFFSET;
...
void ecore_ptt_ilwalidate(struct ecore_hwfn *p_hwfn)
{
	...
	for (i = 0; i < PXP_EXTERNAL_BAR_PF_WINDOW_NUM; i++) {
		...
		p_ptt->pxp.offset = ECORE_BAR_ILWALID_OFFSET;
	}
}
			      ...
			      struct dmae_params *p_params)
{
	 ...
	 * the HSI somewhere. Since it is lwrrently
	 */
...
ecore_dmae_exelwte_sub_operation(struct ecore_hwfn *p_hwfn,
				 struct ecore_ptt *p_ptt,
				 u64 src_addr,
				 u64 dst_addr,
...
static enum _ecore_status_t
ecore_dmae_exelwte_command(struct ecore_hwfn *p_hwfn,
			   struct ecore_ptt *p_ptt,
			   u64 src_addr,
			   u64 dst_addr,
			   u8 src_type,
			   ...
			   struct dmae_params *p_params)
{
	...
	u16 length_lwr = 0, i = 0, cnt_split = 0, length_mod = 0;
	struct dmae_cmd *cmd = p_hwfn->dmae_info.p_dmae_cmd;
	...
	if (!cmd) {
		DP_NOTICE(p_hwfn, true,
			  "ecore_dmae_exelwte_sub_operation failed. Invalid state. source_addr 0x%lx, destination addr 0x%lx, size_in_dwords 0x%x\n",
	...
	for (i = 0; i <= cnt_split; i++) {
		...
		length_lwr = (cnt_split == i) ? length_mod : length_limit;

		/* might be zero on last iteration */
		if (!length_lwr)
			continue;

		ecore_status = ecore_dmae_exelwte_sub_operation(p_hwfn,
								p_ptt,
								src_addr_split,
								dst_addr_split,
								src_type,
					 ...
					 struct dmae_params *p_params)
{
	...
	rc = ecore_dmae_exelwte_command(p_hwfn, p_ptt, source_addr,
					grc_addr_in_dw,
					ECORE_DMAE_ADDRESS_HOST_VIRT,
					ECORE_DMAE_ADDRESS_GRC,
					 ...
					 struct dmae_params *p_params)
{
	...
	rc = ecore_dmae_exelwte_command(p_hwfn, p_ptt, grc_addr_in_dw,
			 ...
			 enum ecore_hw_err_type err_type)
{
	...
	OSAL_HW_ERROR_OCLWRRED(p_hwfn, err_type);
}
