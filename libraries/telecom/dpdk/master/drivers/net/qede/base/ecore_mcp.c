...
void ecore_mcp_read_mb(struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt)
{
	...
	for (i = 0; i < length; i++) {
		...
		((u32 *)p_hwfn->mcp_info->mfw_mb_lwr)[i] =
...
enum _ecore_status_t ecore_mcp_free(struct ecore_hwfn *p_hwfn)
{
	if (p_hwfn->mcp_info) {
		...
		OSAL_FREE(p_hwfn->p_dev, p_hwfn->mcp_info->mfw_mb_lwr);
						   ...
						   struct ecore_ptt *p_ptt)
{
	...
	/* Callwlate the driver and MFW mailbox address */
					...
					struct ecore_ptt *p_ptt)
{
	...
	p_info->mfw_mb_lwr = OSAL_ZALLOC(p_hwfn->p_dev, GFP_KERNEL, size);
				     ...
				     struct ecore_ptt *p_ptt)
{
	...
	/* Use MCP history register to check if MCP reset oclwrred between init
	 * time and now.
	 */
			 ...
			 u32 max_retries, u32 delay)
{
	...
	/* Wait until the mailbox is non-oclwpied */
	do {
	...
	if (cnt >= max_retries) {
		DP_NOTICE(p_hwfn, false,
			  "The MFW mailbox is oclwpied by an uncompleted command. Failed to send command 0x%08x [param 0x%08x].\n",
...
enum _ecore_status_t ecore_mcp_lwm_wr_cmd(struct ecore_hwfn *p_hwfn,
					  struct ecore_ptt *p_ptt,
					  u32 cmd,
					  u32 param,
					  u32 *o_mcp_resp,
...
enum _ecore_status_t ecore_mcp_lwm_rd_cmd(struct ecore_hwfn *p_hwfn,
					  struct ecore_ptt *p_ptt,
					  u32 cmd,
					  u32 param,
					  u32 *o_mcp_resp,
					  ...
					  u32 *o_txn_size, u32 *o_buf)
{
	...
	mb_params.data_dst_size = MCP_DRV_LWM_BUF_LEN;
	...
	*o_txn_size = *o_mcp_param;
	/* @DPDK */
	OSAL_MEMCPY(o_buf, raw_data, RTE_MIN(*o_txn_size, MCP_DRV_LWM_BUF_LEN));
					 ...
					 struct ecore_ptt *p_ptt)
{
	...
	/* Check if there is a DID mismatch between lwm-cfg/efuse */
		    ...
		    struct ecore_mdump_cmd_params *p_mdump_cmd_params)
{
	...
	if (p_mdump_cmd_params->mcp_resp == FW_MSG_CODE_MDUMP_ILWALID_CMD) {
		DP_INFO(p_hwfn,
					    ...
					    struct ecore_ptt *p_ptt)
{
	...
	if (rc == ECORE_SUCCESS && mdump_retain.valid) {
		DP_NOTICE(p_hwfn, false,
			  "The MFW notified that a critical error oclwrred in the device [epoch 0x%08x, pf 0x%x, status 0x%08x]\n",
			  mdump_retain.epoch, mdump_retain.pf,
			  mdump_retain.status);
	} else {
		DP_NOTICE(p_hwfn, false,
			  "The MFW notified that a critical error oclwrred in the device\n");
	}
					     ...
					     struct ecore_ptt *p_ptt)
{
	...
	for (i = 0; i < info->mfw_mb_length; i++) {
		if (info->mfw_mb_lwr[i] == info->mfw_mb_shadow[i])
			continue;
		...
		DP_VERBOSE(p_hwfn, ECORE_MSG_LINK,
			   ...
			   i, info->mfw_mb_shadow[i], info->mfw_mb_lwr[i]);
		...
		case MFW_DRV_MSG_FAILURE_DETECTED:
			...
			break;
		case MFW_DRV_MSG_CRITICAL_ERROR_OCLWRRED:
	...
	for (i = 0; i < MFW_DRV_MSG_MAX_DWORDS(info->mfw_mb_length); i++) {
		OSAL_BE32 val = OSAL_CPU_TO_BE32(((u32 *)info->mfw_mb_lwr)[i]);
	...
	OSAL_MEMCPY(info->mfw_mb_shadow, info->mfw_mb_lwr, info->mfw_mb_length);
...
{
	u32 lwm_cfg_addr, lwm_cfg1_offset, mbi_ver_addr;
	...
	/* Read the address of the lwm_cfg */
	lwm_cfg_addr = ecore_rd(p_hwfn, p_ptt, MISC_REG_GEN_PURP_CR0);
	if (!lwm_cfg_addr) {
		DP_NOTICE(p_hwfn, false, "Shared memory not initialized\n");
		return -EILWAL;
	}

	/* Read the offset of lwm_cfg1 */
	lwm_cfg1_offset = ecore_rd(p_hwfn, p_ptt, lwm_cfg_addr + 4);

	mbi_ver_addr = MCP_REG_SCRATCH + lwm_cfg1_offset +
	    offsetof(struct lwm_cfg1, glob) + offsetof(struct lwm_cfg1_glob,
						       mbi_version);
	*p_mbi_ver =
	    ecore_rd(p_hwfn, p_ptt,
		     mbi_ver_addr) & (LWM_CFG1_GLOB_MBI_VERSION_0_MASK |
				      LWM_CFG1_GLOB_MBI_VERSION_1_MASK |
				      LWM_CFG1_GLOB_MBI_VERSION_2_MASK);
						...
						u32 *p_speed_mask)
{
	...
	case ETH_TRANSCEIVER_TYPE_1000BASET:
		*p_speed_mask = LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_1G;
		break;
	...
	case ETH_TRANSCEIVER_TYPE_4x10G:
		*p_speed_mask = LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_10G;
		break;
	...
	case ETH_TRANSCEIVER_TYPE_MULTI_RATE_10G_40G_LR:
		*p_speed_mask = LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_40G |
		 LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_10G;
		break;
	...
	case ETH_TRANSCEIVER_TYPE_100G_ACC:
		*p_speed_mask =
			LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_BB_100G |
			LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_25G;
		break;
	...
	case ETH_TRANSCEIVER_TYPE_25G_ACC_L:
		*p_speed_mask = LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_25G;
		break;
	...
	case ETH_TRANSCEIVER_TYPE_4x25G_CR:
		*p_speed_mask = LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_25G |
			LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_10G |
			LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_1G;
		break;
	...
	case ETH_TRANSCEIVER_TYPE_MULTI_RATE_40G_100G_CR:
		*p_speed_mask =
			LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_BB_100G |
			LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_50G |
			LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_40G |
			LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_25G |
			LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_20G |
			LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_10G |
			LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_1G;
		break;
...
enum _ecore_status_t ecore_mcp_get_board_config(struct ecore_hwfn *p_hwfn,
						...
						u32 *p_board_config)
{
	u32 lwm_cfg_addr, lwm_cfg1_offset, port_cfg_addr;
	enum _ecore_status_t rc = ECORE_SUCCESS;
	...
	if (!p_ptt) {
		*p_board_config = LWM_CFG1_PORT_PORT_TYPE_UNDEFINED;
		rc = ECORE_ILWAL;
	} else {
		lwm_cfg_addr = ecore_rd(p_hwfn, p_ptt,
					MISC_REG_GEN_PURP_CR0);
		lwm_cfg1_offset = ecore_rd(p_hwfn, p_ptt,
					   lwm_cfg_addr + 4);
		port_cfg_addr = MCP_REG_SCRATCH + lwm_cfg1_offset +
			offsetof(struct lwm_cfg1, port[MFW_PORT(p_hwfn)]);
		*p_board_config  =  ecore_rd(p_hwfn, p_ptt,
					     port_cfg_addr +
					     offsetof(struct lwm_cfg1_port,
					     board_cfg));
	}
						    ...
						    struct ecore_ptt *p_ptt)
{
	...
	/* TODO - are these callwlations true for BE machine? */
					      ...
					      u32 *p_flash_size)
{
	...
	flash_size = ecore_rd(p_hwfn, p_ptt, MCP_REG_LWM_CFG4);
	flash_size = (flash_size & MCP_REG_LWM_CFG4_FLASH_SIZE) >>
		     MCP_REG_LWM_CFG4_FLASH_SIZE_SHIFT;
...
enum _ecore_status_t
ecore_mcp_ov_update_lwrrent_config(struct ecore_hwfn *p_hwfn,
				   struct ecore_ptt *p_ptt,
				   enum ecore_ov_client client)
{
	...
	case ECORE_OV_CLIENT_DRV:
		drv_mb_param = DRV_MB_PARAM_OV_LWRR_CFG_OS;
		break;
	case ECORE_OV_CLIENT_USER:
		drv_mb_param = DRV_MB_PARAM_OV_LWRR_CFG_OTHER;
		break;
	case ECORE_OV_CLIENT_VENDOR_SPEC:
		drv_mb_param = DRV_MB_PARAM_OV_LWRR_CFG_VENDOR_SPEC;
		break;
	default:
	...
	rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_OV_UPDATE_LWRR_CFG,
...
enum _ecore_status_t ecore_mcp_lwm_read(struct ecore_dev *p_dev, u32 addr,
					u8 *p_buf, u32 len)
{
	struct ecore_hwfn *p_hwfn = ECORE_LEADING_HWFN(p_dev);
	u32 bytes_left, offset, bytes_to_copy, buf_size;
	u32 lwm_offset, resp, param;
	struct ecore_ptt *p_ptt;
	enum _ecore_status_t rc = ECORE_SUCCESS;
	...
	while (bytes_left > 0) {
		bytes_to_copy = OSAL_MIN_T(u32, bytes_left,
					   MCP_DRV_LWM_BUF_LEN);
		lwm_offset = (addr + offset) | (bytes_to_copy <<
						DRV_MB_PARAM_LWM_LEN_OFFSET);
		rc = ecore_mcp_lwm_rd_cmd(p_hwfn, p_ptt,
					  DRV_MSG_CODE_LWM_READ_LWRAM,
					  lwm_offset, &resp, &param, &buf_size,
					  (u32 *)(p_buf + offset));
		if (rc != ECORE_SUCCESS) {
			DP_NOTICE(p_dev, false,
				  "ecore_mcp_lwm_rd_cmd() failed, rc = %d\n",
				  rc);
			resp = FW_MSG_CODE_ERROR;
			break;
		}

		if (resp != FW_MSG_CODE_LWM_OK) {
			DP_NOTICE(p_dev, false,
				  "lwm read failed, resp = 0x%08x\n", resp);
			rc = ECORE_UNKNOWN_ERROR;
			break;
		}
...
enum _ecore_status_t ecore_mcp_lwm_resp(struct ecore_dev *p_dev, u8 *p_buf)
{
	struct ecore_hwfn *p_hwfn = ECORE_LEADING_HWFN(p_dev);
	struct ecore_ptt *p_ptt;
	...
	OSAL_MEMCPY(p_buf, &p_dev->mcp_lwm_resp, sizeof(p_dev->mcp_lwm_resp));
	ecore_ptt_release(p_hwfn, p_ptt);

	return ECORE_SUCCESS;
}

enum _ecore_status_t ecore_mcp_lwm_del_file(struct ecore_dev *p_dev, u32 addr)
{
	struct ecore_hwfn *p_hwfn = ECORE_LEADING_HWFN(p_dev);
	struct ecore_ptt *p_ptt;
	u32 resp = 0, param;
	...
	if (!p_ptt)
		return ECORE_BUSY;
	rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_LWM_DEL_FILE, addr,
			   &resp, &param);
	p_dev->mcp_lwm_resp = resp;
	ecore_ptt_release(p_hwfn, p_ptt);
...
enum _ecore_status_t ecore_mcp_lwm_put_file_begin(struct ecore_dev *p_dev,
						  u32 addr)
{
	struct ecore_hwfn *p_hwfn = ECORE_LEADING_HWFN(p_dev);
	struct ecore_ptt *p_ptt;
	...
	if (!p_ptt)
		return ECORE_BUSY;
	rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_LWM_PUT_FILE_BEGIN, addr,
			   &resp, &param);
	p_dev->mcp_lwm_resp = resp;
	ecore_ptt_release(p_hwfn, p_ptt);
...
/* rc receives ECORE_ILWAL as default parameter because
 * it might not enter the while loop if the len is 0
 */
enum _ecore_status_t ecore_mcp_lwm_write(struct ecore_dev *p_dev, u32 cmd,
					 u32 addr, u8 *p_buf, u32 len)
{
	u32 buf_idx, buf_size, lwm_cmd, lwm_offset;
	u32 resp = FW_MSG_CODE_ERROR, param;
	struct ecore_hwfn *p_hwfn = ECORE_LEADING_HWFN(p_dev);
	enum _ecore_status_t rc = ECORE_ILWAL;
	struct ecore_ptt *p_ptt;
	...
	case ECORE_PUT_FILE_DATA:
		lwm_cmd = DRV_MSG_CODE_LWM_PUT_FILE_DATA;
		break;
	case ECORE_LWM_WRITE_LWRAM:
		lwm_cmd = DRV_MSG_CODE_LWM_WRITE_LWRAM;
		break;
	case ECORE_EXT_PHY_FW_UPGRADE:
		lwm_cmd = DRV_MSG_CODE_EXT_PHY_FW_UPGRADE;
		break;
	default:
		DP_NOTICE(p_hwfn, true, "Invalid lwm write command 0x%x\n",
			  cmd);
		rc = ECORE_ILWAL;
		goto out;
	}
	...
	while (buf_idx < len) {
		buf_size = OSAL_MIN_T(u32, (len - buf_idx),
				      MCP_DRV_LWM_BUF_LEN);
		lwm_offset = ((buf_size << DRV_MB_PARAM_LWM_LEN_OFFSET) |
			      addr) +
			     buf_idx;
		rc = ecore_mcp_lwm_wr_cmd(p_hwfn, p_ptt, lwm_cmd, lwm_offset,
					  &resp, &param, buf_size,
					  (u32 *)&p_buf[buf_idx]);
		if (rc != ECORE_SUCCESS) {
			DP_NOTICE(p_dev, false,
				  "ecore_mcp_lwm_write() failed, rc = %d\n",
				  rc);
			resp = FW_MSG_CODE_ERROR;
			break;
		}
		...
		if (resp != FW_MSG_CODE_OK &&
		    resp != FW_MSG_CODE_LWM_OK &&
		    resp != FW_MSG_CODE_LWM_PUT_FILE_FINISH_OK) {
			DP_NOTICE(p_dev, false,
				  "lwm write failed, resp = 0x%08x\n", resp);
			rc = ECORE_UNKNOWN_ERROR;
			break;
		}
					 ...
					 u32 addr, u8 *p_buf, u32 len)
{
	...
	lwm_cmd = (cmd == ECORE_PHY_CORE_WRITE) ?  DRV_MSG_CODE_PHY_CORE_WRITE :
			DRV_MSG_CODE_PHY_RAW_WRITE;
	rc = ecore_mcp_lwm_wr_cmd(p_hwfn, p_ptt, lwm_cmd, addr,
...
enum _ecore_status_t ecore_mcp_lwm_set_selwre_mode(struct ecore_dev *p_dev,
						   u32 addr)
{
	struct ecore_hwfn *p_hwfn = ECORE_LEADING_HWFN(p_dev);
	struct ecore_ptt *p_ptt;
	...
	rc = ecore_mcp_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_SET_SELWRE_MODE, addr,
			   &resp, &param);
	p_dev->mcp_lwm_resp = resp;
	ecore_ptt_release(p_hwfn, p_ptt);
...
enum _ecore_status_t ecore_mcp_phy_sfp_read(struct ecore_hwfn *p_hwfn,
					    ...
					    u32 len, u8 *p_buf)
{
	u32 bytes_left, bytes_to_copy, buf_size, lwm_offset;
	u32 resp, param;
	enum _ecore_status_t rc;

	lwm_offset = (port << DRV_MB_PARAM_TRANSCEIVER_PORT_OFFSET) |
	...
	while (bytes_left > 0) {
		...
		lwm_offset &= (DRV_MB_PARAM_TRANSCEIVER_I2C_ADDRESS_MASK |
			       DRV_MB_PARAM_TRANSCEIVER_PORT_MASK);
		lwm_offset |= ((addr + offset) <<
				DRV_MB_PARAM_TRANSCEIVER_OFFSET_OFFSET);
		lwm_offset |= (bytes_to_copy <<
...
enum _ecore_status_t ecore_mcp_phy_sfp_write(struct ecore_hwfn *p_hwfn,
					     ...
					     u32 len, u8 *p_buf)
{
	u32 buf_idx, buf_size, lwm_offset, resp, param;
	enum _ecore_status_t rc;
	...
	while (buf_idx < len) {
		...
		lwm_offset &= (DRV_MB_PARAM_TRANSCEIVER_I2C_ADDRESS_MASK |
				 DRV_MB_PARAM_TRANSCEIVER_PORT_MASK);
		lwm_offset |= ((offset + buf_idx) <<
				 DRV_MB_PARAM_TRANSCEIVER_OFFSET_OFFSET);
		lwm_offset |= (buf_size <<
			       DRV_MB_PARAM_TRANSCEIVER_SIZE_OFFSET);
		rc = ecore_mcp_lwm_wr_cmd(p_hwfn, p_ptt,
...
enum _ecore_status_t ecore_mcp_bist_lwm_test_get_num_images(
	struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt, u32 *num_images)
{
	u32 drv_mb_param = 0, rsp = 0;
	enum _ecore_status_t rc = ECORE_SUCCESS;

	drv_mb_param = (DRV_MB_PARAM_BIST_LWM_TEST_NUM_IMAGES <<
			DRV_MB_PARAM_BIST_TEST_INDEX_OFFSET);
...
enum _ecore_status_t ecore_mcp_bist_lwm_test_get_image_att(
	struct ecore_hwfn *p_hwfn, struct ecore_ptt *p_ptt,
	struct bist_lwm_image_att *p_image_att, u32 image_index)
{
	u32 buf_size, lwm_offset, resp, param;
	enum _ecore_status_t rc;

	lwm_offset = (DRV_MB_PARAM_BIST_LWM_TEST_IMAGE_BY_INDEX <<
				    DRV_MB_PARAM_BIST_TEST_INDEX_OFFSET);
	lwm_offset |= (image_index <<
		       DRV_MB_PARAM_BIST_TEST_IMAGE_INDEX_OFFSET);
	rc = ecore_mcp_lwm_rd_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_BIST_TEST,
				  lwm_offset, &resp, &param, &buf_size,
				  (u32 *)p_image_att);
	if (rc != ECORE_SUCCESS)
		return rc;
...
enum _ecore_status_t
ecore_mcp_bist_lwm_get_num_images(struct ecore_hwfn *p_hwfn,
				  struct ecore_ptt *p_ptt, u32 *num_images)
{
	...
	SET_MFW_FIELD(drv_mb_param, DRV_MB_PARAM_BIST_TEST_INDEX,
		      DRV_MB_PARAM_BIST_LWM_TEST_NUM_IMAGES);
...
enum _ecore_status_t
ecore_mcp_bist_lwm_get_image_att(struct ecore_hwfn *p_hwfn,
				 struct ecore_ptt *p_ptt,
				 struct bist_lwm_image_att *p_image_att,
				 u32 image_index)
{
	u32 buf_size, lwm_offset = 0, resp, param;
	enum _ecore_status_t rc;

	SET_MFW_FIELD(lwm_offset, DRV_MB_PARAM_BIST_TEST_INDEX,
		      DRV_MB_PARAM_BIST_LWM_TEST_IMAGE_BY_INDEX);
	SET_MFW_FIELD(lwm_offset, DRV_MB_PARAM_BIST_TEST_IMAGE_INDEX,
		      image_index);
	rc = ecore_mcp_lwm_rd_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_BIST_TEST,
				  lwm_offset, &resp, &param, &buf_size,
				  (u32 *)p_image_att);
...
enum _ecore_status_t ecore_mcp_lwm_info_populate(struct ecore_hwfn *p_hwfn)
{
	struct ecore_lwm_image_info lwm_info;
	struct ecore_ptt *p_ptt;
	enum _ecore_status_t rc;
	u32 i;

	if (p_hwfn->lwm_info.valid)
		return ECORE_SUCCESS;
	...
	OSAL_MEM_ZERO(&lwm_info, sizeof(lwm_info));
	rc = ecore_mcp_bist_lwm_get_num_images(p_hwfn, p_ptt,
					       &lwm_info.num_images);
	if (rc == ECORE_NOTIMPL) {
		DP_INFO(p_hwfn, "DRV_MSG_CODE_BIST_TEST is not supported\n");
		goto out;
	} else if ((rc != ECORE_SUCCESS) || (lwm_info.num_images == 0)) {
		DP_ERR(p_hwfn, "Failed getting number of images\n");
		goto err0;
	}

	lwm_info.image_att = OSAL_ALLOC(p_hwfn->p_dev, GFP_KERNEL,
					 lwm_info.num_images *
					 sizeof(struct bist_lwm_image_att));
	if (!lwm_info.image_att) {
		rc = ECORE_NOMEM;
		goto err0;
	}

	/* Iterate over images and get their attributes */
	for (i = 0; i < lwm_info.num_images; i++) {
		rc = ecore_mcp_bist_lwm_get_image_att(p_hwfn, p_ptt,
						      &lwm_info.image_att[i],
						      i);
		if (rc != ECORE_SUCCESS) {
			DP_ERR(p_hwfn,
		...
		DP_VERBOSE(p_hwfn, ECORE_MSG_SP, "image index %d, size %x\n", i,
			   lwm_info.image_att[i].len);
	}
out:
	/* Update hwfn's lwm_info */
	if (lwm_info.num_images) {
		p_hwfn->lwm_info.num_images = lwm_info.num_images;
		if (p_hwfn->lwm_info.image_att)
			OSAL_FREE(p_hwfn->p_dev, p_hwfn->lwm_info.image_att);
		p_hwfn->lwm_info.image_att = lwm_info.image_att;
		p_hwfn->lwm_info.valid = true;
	}
...
err1:
	OSAL_FREE(p_hwfn->p_dev, lwm_info.image_att);
err0:
	ecore_ptt_release(p_hwfn, p_ptt);
	return rc;
}
...
enum _ecore_status_t
ecore_mcp_get_lwm_image_att(struct ecore_hwfn *p_hwfn,
			    enum ecore_lwm_images image_id,
			    struct ecore_lwm_image_att *p_image_att)
{
	enum lwm_image_type type;
	u32 i;

	/* Translate image_id into MFW definitions */
	switch (image_id) {
	case ECORE_LWM_IMAGE_ISCSI_CFG:
		type = LWM_TYPE_ISCSI_CFG;
		break;
	case ECORE_LWM_IMAGE_FCOE_CFG:
		type = LWM_TYPE_FCOE_CFG;
		break;
	case ECORE_LWM_IMAGE_MDUMP:
		type = LWM_TYPE_MDUMP;
		break;
	case ECORE_LWM_IMAGE_LWM_CFG1:
		type = LWM_TYPE_LWM_CFG1;
		break;
	case ECORE_LWM_IMAGE_DEFAULT_CFG:
		type = LWM_TYPE_DEFAULT_CFG;
		break;
	case ECORE_LWM_IMAGE_LWM_META:
		type = LWM_TYPE_META;
		break;
	default:
	...
	ecore_mcp_lwm_info_populate(p_hwfn);
	for (i = 0; i < p_hwfn->lwm_info.num_images; i++) {
		if (type == p_hwfn->lwm_info.image_att[i].image_type)
			break;
	}
	if (i == p_hwfn->lwm_info.num_images) {
		DP_VERBOSE(p_hwfn, ECORE_MSG_STORAGE,
			   "Failed to find lwram image of type %08x\n",
			   image_id);
		return ECORE_NOENT;
	}

	p_image_att->start_addr = p_hwfn->lwm_info.image_att[i].lwm_start_addr;
	p_image_att->length = p_hwfn->lwm_info.image_att[i].len;

	return ECORE_SUCCESS;
}

enum _ecore_status_t ecore_mcp_get_lwm_image(struct ecore_hwfn *p_hwfn,
					     enum ecore_lwm_images image_id,
					     u8 *p_buffer, u32 buffer_len)
{
	struct ecore_lwm_image_att image_att;
	enum _ecore_status_t rc;

	OSAL_MEM_ZERO(p_buffer, buffer_len);

	rc = ecore_mcp_get_lwm_image_att(p_hwfn, image_id, &image_att);
	if (rc != ECORE_SUCCESS)
		return rc;
	...
	return ecore_mcp_lwm_read(p_hwfn->p_dev, image_att.start_addr,
			       ...
			       struct ecore_temperature_info *p_temp_info)
{
	...
	for (i = 0; i < p_temp_info->num_sensors; i++) {
		...
		p_temp_sensor->lwrrent_temp = (val & LWRRENT_TEMP_MASK) >>
					      LWRRENT_TEMP_OFFSET;
	}
	...
	struct ecore_mba_vers *p_mba_vers)
{
	...
	rc = ecore_mcp_lwm_rd_cmd(p_hwfn, p_ptt, DRV_MSG_CODE_GET_MBA_VERSION,
	...
	if ((resp & FW_MSG_CODE_MASK) != FW_MSG_CODE_LWM_OK)
		rc = ECORE_UNKNOWN_ERROR;

	if (buf_size != MCP_DRV_LWM_BUF_LEN)
		rc = ECORE_UNKNOWN_ERROR;
...
{
	enum resource_id_enum mfw_res_id = RESOURCE_NUM_ILWALID;
			      ...
			      struct ecore_resc_alloc_out_params *p_out_params)
{
	...
	if (mfw_resc_info.res_id == RESOURCE_NUM_ILWALID) {
		DP_ERR(p_hwfn,
