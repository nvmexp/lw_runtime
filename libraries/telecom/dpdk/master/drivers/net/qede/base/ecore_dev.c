 ...
 * registers ilwolved are not split and thus configuration is a race where
...
void ecore_db_recovery_exelwte(struct ecore_hwfn *p_hwfn,
			       enum ecore_db_rec_exec db_exec)
{
	struct ecore_db_recovery_entry *db_entry = OSAL_NULL;

	if (db_exec != DB_REC_ONCE) {
		DP_NOTICE(p_hwfn, false, "Exelwting doorbell recovery. Counter was %d\n",
			  p_hwfn->db_recovery_info.db_recovery_counter);

		/* track amount of times recovery was exelwted */
		p_hwfn->db_recovery_info.db_recovery_counter++;
	}
...
#define ECORE_LLH_ILWALID_FILTER_IDX	0xff
			       ...
			       u8 *p_filter_idx)
{
	...
	*p_filter_idx = ECORE_LLH_ILWALID_FILTER_IDX;
			    ...
			    u8 *p_filter_idx, u32 *p_ref_cnt)
{
	...
	if (*p_filter_idx == ECORE_LLH_ILWALID_FILTER_IDX) {
 ...
 * params may be affected by the init but would simply recallwlate to the same
						 ...
						 int hw_mode)
{
	...
	if (ECORE_IS_BB(p_dev)) {
		 ...
		 * ilwolving non initialized function in processing ROCE packet.
		 */
	...
	/* Workaround for avoiding CCFC exelwtion error when getting packets
	 * with CRC errors, and allowing instead the ilwoking of the FW error
	 * handler.
	 * This is not done inside the init tool since it lwrrently can't
...
static u32 ecore_hw_norm_region_conn(struct ecore_hwfn *p_hwfn)
{
	 ...
	 * region is callwlated based on the CORE CIDs, in case of non-ETH
			      ...
			      struct ecore_ptt *p_ptt)
{
	 ...
	 * non-PWM connection. The callwlation below computes the total non-PWM
		 ...
		 int hw_mode, struct ecore_hw_init_params *p_params)
{
	...
	/* QM_PF Init sequence (may be ilwoked separately e.g. for DCB) */
						      ...
						      struct ecore_ptt *p_ptt)
{
	...
	if (!(p_dev->ppfid_bitmap & (0x1 << native_ppfid_idx))) {
		...
		DP_INFO(p_hwfn,
			"Fix the PPFID bitmap to inlwlde the native PPFID: %hhd -> 0x%hhx\n",
			p_dev->ppfid_bitmap, new_bitmap);
		p_dev->ppfid_bitmap = new_bitmap;
	}
					      ...
					      bool drv_resc_alloc)
{
	 ...
	 * resource allocation scheme, but isn't lwrrently present for
...
#ifndef ASIC_ONLY
static enum _ecore_status_t
ecore_emul_hw_get_lwm_info(struct ecore_hwfn *p_hwfn)
{
	if (IS_LEAD_HWFN(p_hwfn)) {
		struct ecore_dev *p_dev = p_hwfn->p_dev;
...
static enum _ecore_status_t
ecore_hw_get_lwm_info(struct ecore_hwfn *p_hwfn,
		      struct ecore_ptt *p_ptt,
		      struct ecore_hw_prepare_params *p_params)
{
	u32 lwm_cfg1_offset, mf_mode, addr, generic_cont0, core_cfg, dcbx_mode;
	u32 port_cfg_addr, link_temp, lwm_cfg_addr, device_capabilities;
...
#ifndef ASIC_ONLY
	if (CHIP_REV_IS_SLOW(p_hwfn->p_dev))
		return ecore_emul_hw_get_lwm_info(p_hwfn);
#endif

	/* Read global lwm_cfg address */
	...
	if (!lwm_cfg_addr) {
		...
		if (p_params->b_relaxed_probe)
			p_params->p_relaxed_res = ECORE_HW_PREPARE_FAILED_LWM;
		return ECORE_ILWAL;
	}

/* Read lwm_cfg1  (Notice this is just offset, and not offsize (TBD) */

	lwm_cfg1_offset = ecore_rd(p_hwfn, p_ptt, lwm_cfg_addr + 4);

	addr = MCP_REG_SCRATCH + lwm_cfg1_offset +
		   OFFSETOF(struct lwm_cfg1, glob) +
		   OFFSETOF(struct lwm_cfg1_glob, core_cfg);

	core_cfg = ecore_rd(p_hwfn, p_ptt, addr);

	switch ((core_cfg & LWM_CFG1_GLOB_NETWORK_PORT_MODE_MASK) >>
		LWM_CFG1_GLOB_NETWORK_PORT_MODE_OFFSET) {
	case LWM_CFG1_GLOB_NETWORK_PORT_MODE_BB_2X40G:
		p_hwfn->hw_info.port_mode = ECORE_PORT_MODE_DE_2X40G;
		break;
	case LWM_CFG1_GLOB_NETWORK_PORT_MODE_2X50G:
		p_hwfn->hw_info.port_mode = ECORE_PORT_MODE_DE_2X50G;
		break;
	case LWM_CFG1_GLOB_NETWORK_PORT_MODE_BB_1X100G:
		p_hwfn->hw_info.port_mode = ECORE_PORT_MODE_DE_1X100G;
		break;
	case LWM_CFG1_GLOB_NETWORK_PORT_MODE_4X10G_F:
		p_hwfn->hw_info.port_mode = ECORE_PORT_MODE_DE_4X10G_F;
		break;
	case LWM_CFG1_GLOB_NETWORK_PORT_MODE_BB_4X10G_E:
		p_hwfn->hw_info.port_mode = ECORE_PORT_MODE_DE_4X10G_E;
		break;
	case LWM_CFG1_GLOB_NETWORK_PORT_MODE_BB_4X20G:
		p_hwfn->hw_info.port_mode = ECORE_PORT_MODE_DE_4X20G;
		break;
	case LWM_CFG1_GLOB_NETWORK_PORT_MODE_1X40G:
		p_hwfn->hw_info.port_mode = ECORE_PORT_MODE_DE_1X40G;
		break;
	case LWM_CFG1_GLOB_NETWORK_PORT_MODE_2X25G:
		p_hwfn->hw_info.port_mode = ECORE_PORT_MODE_DE_2X25G;
		break;
	case LWM_CFG1_GLOB_NETWORK_PORT_MODE_2X10G:
		p_hwfn->hw_info.port_mode = ECORE_PORT_MODE_DE_2X10G;
		break;
	case LWM_CFG1_GLOB_NETWORK_PORT_MODE_1X25G:
		p_hwfn->hw_info.port_mode = ECORE_PORT_MODE_DE_1X25G;
		break;
	case LWM_CFG1_GLOB_NETWORK_PORT_MODE_4X25G:
	...
	dcbx_mode = ecore_rd(p_hwfn, p_ptt,
			     port_cfg_addr +
			     OFFSETOF(struct lwm_cfg1_port, generic_cont0));
	dcbx_mode = (dcbx_mode & LWM_CFG1_PORT_DCBX_MODE_MASK)
		>> LWM_CFG1_PORT_DCBX_MODE_OFFSET;
	switch (dcbx_mode) {
	case LWM_CFG1_PORT_DCBX_MODE_DYNAMIC:
		p_hwfn->hw_info.dcbx_mode = ECORE_DCBX_VERSION_DYNAMIC;
		break;
	case LWM_CFG1_PORT_DCBX_MODE_CEE:
		p_hwfn->hw_info.dcbx_mode = ECORE_DCBX_VERSION_CEE;
		break;
	case LWM_CFG1_PORT_DCBX_MODE_IEEE:
	...
	link_temp = ecore_rd(p_hwfn, p_ptt,
			     port_cfg_addr +
			     OFFSETOF(struct lwm_cfg1_port, speed_cap_mask));
	link_temp &= LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_MASK;
	link->speed.advertised_speeds = link_temp;
	p_caps->speed_capabilities = link->speed.advertised_speeds;

	link_temp = ecore_rd(p_hwfn, p_ptt,
				 port_cfg_addr +
				 OFFSETOF(struct lwm_cfg1_port, link_settings));
	switch ((link_temp & LWM_CFG1_PORT_DRV_LINK_SPEED_MASK) >>
		LWM_CFG1_PORT_DRV_LINK_SPEED_OFFSET) {
	case LWM_CFG1_PORT_DRV_LINK_SPEED_AUTONEG:
		link->speed.autoneg = true;
		break;
	case LWM_CFG1_PORT_DRV_LINK_SPEED_1G:
		link->speed.forced_speed = 1000;
		break;
	case LWM_CFG1_PORT_DRV_LINK_SPEED_10G:
		link->speed.forced_speed = 10000;
		break;
	case LWM_CFG1_PORT_DRV_LINK_SPEED_25G:
		link->speed.forced_speed = 25000;
		break;
	case LWM_CFG1_PORT_DRV_LINK_SPEED_40G:
		link->speed.forced_speed = 40000;
		break;
	case LWM_CFG1_PORT_DRV_LINK_SPEED_50G:
		link->speed.forced_speed = 50000;
		break;
	case LWM_CFG1_PORT_DRV_LINK_SPEED_BB_100G:
	...
	link_temp &= LWM_CFG1_PORT_DRV_FLOW_CONTROL_MASK;
	link_temp >>= LWM_CFG1_PORT_DRV_FLOW_CONTROL_OFFSET;
	link->pause.autoneg = !!(link_temp &
				  LWM_CFG1_PORT_DRV_FLOW_CONTROL_AUTONEG);
	link->pause.forced_rx = !!(link_temp &
				    LWM_CFG1_PORT_DRV_FLOW_CONTROL_RX);
	link->pause.forced_tx = !!(link_temp &
				    LWM_CFG1_PORT_DRV_FLOW_CONTROL_TX);
	link->loopback_mode = 0;

	if (p_hwfn->mcp_info->capabilities & FW_MB_PARAM_FEATURE_SUPPORT_EEE) {
		link_temp = ecore_rd(p_hwfn, p_ptt, port_cfg_addr +
				     OFFSETOF(struct lwm_cfg1_port, ext_phy));
		link_temp &= LWM_CFG1_PORT_EEE_POWER_SAVING_MODE_MASK;
		link_temp >>= LWM_CFG1_PORT_EEE_POWER_SAVING_MODE_OFFSET;
		p_caps->default_eee = ECORE_MCP_EEE_ENABLED;
		link->eee.enable = true;
		switch (link_temp) {
		case LWM_CFG1_PORT_EEE_POWER_SAVING_MODE_DISABLED:
			p_caps->default_eee = ECORE_MCP_EEE_DISABLED;
			link->eee.enable = false;
			break;
		case LWM_CFG1_PORT_EEE_POWER_SAVING_MODE_BALANCED:
			p_caps->eee_lpi_timer = EEE_TX_TIMER_USEC_BALANCED_TIME;
			break;
		case LWM_CFG1_PORT_EEE_POWER_SAVING_MODE_AGGRESSIVE:
			p_caps->eee_lpi_timer =
				EEE_TX_TIMER_USEC_AGGRESSIVE_TIME;
			break;
		case LWM_CFG1_PORT_EEE_POWER_SAVING_MODE_LOW_LATENCY:
	...
	addr = MCP_REG_SCRATCH + lwm_cfg1_offset +
		   OFFSETOF(struct lwm_cfg1, glob) +
		   OFFSETOF(struct lwm_cfg1_glob, generic_cont0);

	generic_cont0 = ecore_rd(p_hwfn, p_ptt, addr);

	mf_mode = (generic_cont0 & LWM_CFG1_GLOB_MF_MODE_MASK) >>
	    LWM_CFG1_GLOB_MF_MODE_OFFSET;

	switch (mf_mode) {
	case LWM_CFG1_GLOB_MF_MODE_MF_ALLOWED:
		p_hwfn->p_dev->mf_bits = 1 << ECORE_MF_OVLAN_CLSS;
		break;
	case LWM_CFG1_GLOB_MF_MODE_UFP:
		p_hwfn->p_dev->mf_bits = 1 << ECORE_MF_OVLAN_CLSS |
					 1 << ECORE_MF_UFP_SPECIFIC |
					 1 << ECORE_MF_8021Q_TAGGING;
		break;
	case LWM_CFG1_GLOB_MF_MODE_BD:
		p_hwfn->p_dev->mf_bits = 1 << ECORE_MF_OVLAN_CLSS |
					 1 << ECORE_MF_LLH_PROTO_CLSS |
					 1 << ECORE_MF_8021AD_TAGGING |
					 1 << ECORE_MF_FIP_SPECIAL;
		break;
	case LWM_CFG1_GLOB_MF_MODE_NPAR1_0:
		p_hwfn->p_dev->mf_bits = 1 << ECORE_MF_LLH_MAC_CLSS |
					 ...
					 1 << ECORE_MF_DISABLE_ARFS;
		break;
	case LWM_CFG1_GLOB_MF_MODE_DEFAULT:
		p_hwfn->p_dev->mf_bits = 1 << ECORE_MF_LLH_MAC_CLSS |
					 1 << ECORE_MF_LLH_PROTO_CLSS |
					 1 << ECORE_MF_LL2_NON_UNICAST;
		if (ECORE_IS_BB(p_hwfn->p_dev))
	 ...
	 * colwert client code and eliminate this.
	 */
	switch (mf_mode) {
	case LWM_CFG1_GLOB_MF_MODE_MF_ALLOWED:
	case LWM_CFG1_GLOB_MF_MODE_BD:
	...
	addr = MCP_REG_SCRATCH + lwm_cfg1_offset +
		   OFFSETOF(struct lwm_cfg1, glob) +
		   OFFSETOF(struct lwm_cfg1_glob, device_capabilities);

	device_capabilities = ecore_rd(p_hwfn, p_ptt, addr);
	if (device_capabilities & LWM_CFG1_GLOB_DEVICE_CAPABILITIES_ETHERNET)
		OSAL_SET_BIT(ECORE_DEV_CAP_ETH,
				&p_hwfn->hw_info.device_capabilities);
	if (device_capabilities & LWM_CFG1_GLOB_DEVICE_CAPABILITIES_FCOE)
		OSAL_SET_BIT(ECORE_DEV_CAP_FCOE,
				&p_hwfn->hw_info.device_capabilities);
	if (device_capabilities & LWM_CFG1_GLOB_DEVICE_CAPABILITIES_ISCSI)
		OSAL_SET_BIT(ECORE_DEV_CAP_ISCSI,
				&p_hwfn->hw_info.device_capabilities);
	if (device_capabilities & LWM_CFG1_GLOB_DEVICE_CAPABILITIES_ROCE)
		OSAL_SET_BIT(ECORE_DEV_CAP_ROCE,
				&p_hwfn->hw_info.device_capabilities);
	if (device_capabilities & LWM_CFG1_GLOB_DEVICE_CAPABILITIES_IWARP)
				...
				struct ecore_ptt *p_ptt)
{
	...
	DP_VERBOSE(p_hwfn, ECORE_MSG_PROBE,
		   "PF [rel_id %d, abs_id %d] oclwpies index %d within the %d enabled functions on the engine\n",
		  ...
		  struct ecore_hw_prepare_params *p_params)
{
	...
	rc = ecore_hw_get_lwm_info(p_hwfn, p_ptt, p_params);
	if (rc != ECORE_SUCCESS)
		return rc;
	 ...
	 * and after getting the number of functions, since the callwlation of
...
/* Callwlate final WFQ values for all vports and configure it.
