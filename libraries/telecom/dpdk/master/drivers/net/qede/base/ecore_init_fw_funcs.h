 ...
 * @param pf_id			- PF ID
 * @param max_phys_tcs_per_port	- max number of physical TCs per port in HW
 * @param is_pf_loading -	  indicates if the PF is lwrrently loading,
 ...
 *
 * @return bool, true if successful, false if timeout oclwrred while waiting
 *  for QM command done.
 */
...
void ecore_enable_context_validation(struct ecore_hwfn *p_hwfn,
				     struct ecore_ptt *p_ptt);
/**
 * @brief ecore_calc_session_ctx_validation - Callwalte validation byte for
 * session context.
 *
 * @param p_hwfn -		HW device data
...
/**
 * @brief ecore_calc_task_ctx_validation - Callwalte validation byte for task
 * context.
 *
 * @param p_hwfn -		HW device data
 * @param p_ctx_mem -	pointer to context memory.
