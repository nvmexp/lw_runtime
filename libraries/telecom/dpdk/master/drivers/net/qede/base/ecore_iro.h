...
/* Tstorm LWMf per port per producer consumer data */
#define TSTORM_LWMF_PORT_TASKPOOL_PRODUCER_CONSUMER_OFFSET(port_num_id, \
	taskpool_index) (IRO[67].base + ((port_num_id) * IRO[67].m1) + \
	((taskpool_index) * IRO[67].m2))
#define TSTORM_LWMF_PORT_TASKPOOL_PRODUCER_CONSUMER_SIZE (IRO[67].size)
/* Ustorm LWMf per port counters */
#define USTORM_LWMF_PORT_COUNTERS_OFFSET(port_num_id) (IRO[68].base + \
	((port_num_id) * IRO[68].m1))
#define USTORM_LWMF_PORT_COUNTERS_SIZE (IRO[68].size)
