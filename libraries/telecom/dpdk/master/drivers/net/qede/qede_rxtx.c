...
/* Criterias for callwlating Rx buffer size -
		    ...
		    struct rte_mempool *mp)
{
	...
	/* cache align the mbuf size to simplfy rx_buf_size callwlation */
...
{
	if ((PARSING_AND_ERR_FLAGS_TUNNELL4CHKSMWASCALLWLATED_MASK <<
	     PARSING_AND_ERR_FLAGS_TUNNELL4CHKSMWASCALLWLATED_SHIFT) & flag)
...
{
	if ((PARSING_AND_ERR_FLAGS_L4CHKSMWASCALLWLATED_MASK <<
	     PARSING_AND_ERR_FLAGS_L4CHKSMWASCALLWLATED_SHIFT) & flag)
...
qede_reuse_page(__rte_unused struct qede_dev *qdev,
		struct qede_rx_queue *rxq, struct qede_rx_entry *lwrr_cons)
{
	struct eth_rx_bd *rx_bd_prod = ecore_chain_produce(&rxq->rx_bd_ring);
	uint16_t idx = rxq->sw_rx_prod & NUM_RX_BDS(rxq);
	struct qede_rx_entry *lwrr_prod;
	dma_addr_t new_mapping;

	lwrr_prod = &rxq->sw_rx_ring[idx];
	*lwrr_prod = *lwrr_cons;

	new_mapping = rte_mbuf_data_iova_default(lwrr_prod->mbuf) +
		      lwrr_prod->page_offset;
...
{
	struct qede_rx_entry *lwrr_cons;

	for (; count > 0; count--) {
		lwrr_cons = &rxq->sw_rx_ring[rxq->sw_rx_cons & NUM_RX_BDS(rxq)];
		qede_reuse_page(qdev, rxq, lwrr_cons);
				     ...
				     uint8_t agg_index, uint16_t len)
{
	...
	struct rte_mbuf *lwrr_frag; /* Pointer to lwrrently filled TPA seg */
	uint16_t cons_idx;
	...
	if (rte_le_to_cpu_16(len)) {
		...
		lwrr_frag = rxq->sw_rx_ring[cons_idx].mbuf;
		assert(lwrr_frag);
		lwrr_frag->nb_segs = 1;
		lwrr_frag->pkt_len = rte_le_to_cpu_16(len);
		lwrr_frag->data_len = lwrr_frag->pkt_len;
		tpa_info->tpa_tail->next = lwrr_frag;
		tpa_info->tpa_tail = lwrr_frag;
		qede_rx_bd_ring_consume(rxq);
		     ...
		     uint8_t num_segs, uint16_t pkt_len)
{
	...
	seg1 = rx_mb;
	while (num_segs) {
		lwr_size = pkt_len > rxq->rx_buf_size ? rxq->rx_buf_size :
							pkt_len;
		if (unlikely(!lwr_size)) {
...
qede_xmit_pkts(void *p_txq, struct rte_mbuf **tx_pkts, uint16_t nb_pkts)
{
	...
	while (nb_tx_pkts--) {
		...
		if (tunn_flg) {
			...
			/**
			 * Lwrrently, only inner checksum offload in MPLS-in-UDP
		...
		if (tx_ol_flags & PKT_TX_IP_CKSUM) {
			 ...
			 * recallwlation of L4 tunnel header csum also.
			 */
 ...
 * to callwlate number of BDs used by HW.
