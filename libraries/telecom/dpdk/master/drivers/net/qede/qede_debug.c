...
static struct storm_defs s_storm_defs[] = {
	...
	/* Ustorm */
	{'U', BLOCK_USEM,
		{DBG_BUS_CLIENT_RBLW, DBG_BUS_CLIENT_RBLW},
		false,
...
static u32 qed_dump_last_section(u32 *dump_buf, u32 offset, bool dump)
{
	...
	/* Callwlate CRC32 and add it to the dword after the "last" section */
	if (dump)
...
/* is_mode_match relwrsive function */
				  ...
				  u16 *modes_buf_offset, u8 rec_depth)
{
	...
	if (rec_depth > MAX_RELWRSION_DEPTH) {
		DP_NOTICE(p_hwfn, false,
			  "Unexpected error: is_mode_match_rec exceeded the max relwrsion depth. This is probably due to a corrupt init/debug buffer.\n");
		return false;
	}
 ...
 * - read_len:	no. conselwtive dwords to read
				       ...
				       u32 read_len, u32 skip_len)
{
	...
	while (reg_offset < total_len) {
		u32 lwrr_len = OSAL_MIN_T(u32, read_len,
					  total_len - reg_offset);

		offset += qed_grc_dump_addr_range(p_hwfn,
						  p_ptt,
						  dump_buf + offset,
						  dump,  addr, lwrr_len, false,
						  SPLIT_TYPE_NONE, 0);
		reg_offset += lwrr_len;
		addr += lwrr_len;

		if (reg_offset < total_len) {
			lwrr_len = OSAL_MIN_T(u32, skip_len,
					      total_len - skip_len);
			memset(dump_buf + offset, 0, DWORDS_TO_BYTES(lwrr_len));
			offset += lwrr_len;
			reg_offset += lwrr_len;
			addr += lwrr_len;
		}
				   ...
				   u8 split_id, const char *reg_type_name)
{
	...
	/* Callwlate register dump header size (and skip it for now) */
				  ...
				  const char *reg_type_name)
{
	...
	while (input_offset < BYTES_TO_DWORDS(dbg_buf->size)) {
		...
		struct virt_mem_desc lwrr_input_regs_arr;
		enum init_split_types split_type;
		u16 split_count = 0;
		u32 split_data_size;
		u8 split_id;
		...
		lwrr_input_regs_arr.ptr =
		    (u32 *)p_hwfn->dbg_arrays[BIN_BUF_DBG_DUMP_REG].ptr +
		    input_offset;
		lwrr_input_regs_arr.size = DWORDS_TO_BYTES(split_data_size);
				 ...
				 u32 *dump_buf, bool dump)
{
	...
	while (input_offset < BYTES_TO_DWORDS(dbg_buf->size)) {
		const struct dbg_dump_split_hdr *split_hdr;
		struct virt_mem_desc lwrr_input_mems_arr;
		enum init_split_types split_type;
		u32 split_data_size;
		...
		split_data_size = GET_FIELD(split_hdr->hdr,
					    DBG_DUMP_SPLIT_HDR_DATA_SIZE);
		lwrr_input_mems_arr.ptr = (u32 *)dbg_buf->ptr + input_offset;
		lwrr_input_mems_arr.size = DWORDS_TO_BYTES(split_data_size);
		...
		if (split_type == SPLIT_TYPE_NONE)
			...
			DP_NOTICE(p_hwfn, false,
				  "Dumping split memories is lwrrently not supported\n");
...
static enum dbg_status qed_find_lwram_image(struct ecore_hwfn *p_hwfn,
					    struct ecore_ptt *p_ptt,
					    u32 image_type,
					    u32 *lwram_offset_bytes,
					    u32 *lwram_size_bytes);

static enum dbg_status qed_lwram_read(struct ecore_hwfn *p_hwfn,
				      struct ecore_ptt *p_ptt,
				      u32 lwram_offset_bytes,
				      u32 lwram_size_bytes, u32 *ret_buf);

/* Dumps the MCP HW dump from LWRAM. Returns the dumped size in dwords. */
				    ...
				    u32 *dump_buf, bool dump)
{
	...
	/* Read HW dump image from LWRAM */
	status = qed_find_lwram_image(p_hwfn,
				      p_ptt,
				      LWM_TYPE_HW_DUMP_OUT,
	...
	if (dump && hw_dump_size_dwords) {
		status = qed_lwram_read(p_hwfn,
					p_ptt,
					hw_dump_offset_bytes,
					hw_dump_size_bytes, dump_buf + offset);
		if (status != DBG_STATUS_OK) {
			DP_NOTICE(p_hwfn, false,
				  "Failed to read MCP HW Dump image from LWRAM\n");
			return 0;
		}
	}
	...
	for (i = 0; i < num_input_rules; i++) {
		...
		for (entry_id = 0; entry_id < num_reg_entries; entry_id++) {
			...
			for (reg_id = 0; reg_id < rule->num_cond_regs;
				 ...
				 * address of the specific entry is callwlated).
				 */
				addr = GET_FIELD(reg->data,
		...
		if (mode_match) {
			...
			offset +=
			    qed_idle_chk_dump_rule_entries(p_hwfn,
							   ...
							   num_input_rules,
							   &lwrr_failing_rules);
			num_failing_rules += lwrr_failing_rules;
		}
...
/* Finds the meta data image in LWRAM */
static enum dbg_status qed_find_lwram_image(struct ecore_hwfn *p_hwfn,
					    struct ecore_ptt *p_ptt,
					    u32 image_type,
					    u32 *lwram_offset_bytes,
					    u32 *lwram_size_bytes)
{
	...
	/* Call LWRAM get file command */
	lwm_result = ecore_mcp_lwm_rd_cmd(p_hwfn,
					p_ptt,
					DRV_MSG_CODE_LWM_GET_FILE_ATT,
					image_type,
					&ret_mcp_resp,
					&ret_mcp_param,
	...
	/* Check response */
	if (lwm_result ||
	    (ret_mcp_resp & FW_MSG_CODE_MASK) != FW_MSG_CODE_LWM_OK)
		return DBG_STATUS_LWRAM_GET_IMAGE_FAILED;

	/* Update return values */
	*lwram_offset_bytes = file_att.lwm_start_addr;
	*lwram_size_bytes = file_att.len;

	DP_VERBOSE(p_hwfn->p_dev,
		   ECORE_MSG_DEBUG,
		   "find_lwram_image: found LWRAM image of type %d in LWRAM offset %d bytes with size %d bytes\n",
		   image_type, *lwram_offset_bytes, *lwram_size_bytes);

	/* Check alignment */
	if (*lwram_size_bytes & 0x3)
		return DBG_STATUS_NON_ALIGNED_LWRAM_IMAGE;

	return DBG_STATUS_OK;
}

/* Reads data from LWRAM */
static enum dbg_status qed_lwram_read(struct ecore_hwfn *p_hwfn,
				      struct ecore_ptt *p_ptt,
				      u32 lwram_offset_bytes,
				      u32 lwram_size_bytes, u32 *ret_buf)
{
	...
	s32 bytes_left = lwram_size_bytes;
	u32 read_offset = 0, param = 0;

	DP_NOTICE(p_hwfn->p_dev, false,
		   "lwram_read: reading image of size %d bytes from LWRAM\n",
		   lwram_size_bytes);

	do {
		bytes_to_copy =
		    (bytes_left >
		     MCP_DRV_LWM_BUF_LEN) ? MCP_DRV_LWM_BUF_LEN : bytes_left;

		/* Call LWRAM read command */
		SET_MFW_FIELD(param,
			      DRV_MB_PARAM_LWM_OFFSET,
			      lwram_offset_bytes + read_offset);
		SET_MFW_FIELD(param, DRV_MB_PARAM_LWM_LEN, bytes_to_copy);
		if (ecore_mcp_lwm_rd_cmd(p_hwfn, p_ptt,
					 DRV_MSG_CODE_LWM_READ_LWRAM, param,
					 &ret_mcp_resp,
					 &ret_mcp_param, &ret_read_size,
					 (u32 *)((u8 *)ret_buf +
						 read_offset))) {
			DP_NOTICE(p_hwfn->p_dev, false, "rc = DBG_STATUS_LWRAM_READ_FAILED\n");
			return DBG_STATUS_LWRAM_READ_FAILED;
		}

		/* Check response */
		if ((ret_mcp_resp & FW_MSG_CODE_MASK) != FW_MSG_CODE_LWM_OK) {
			DP_NOTICE(p_hwfn->p_dev, false, "rc = DBG_STATUS_LWRAM_READ_FAILED\n");
			return DBG_STATUS_LWRAM_READ_FAILED;
		}
						   ...
						   u32 *trace_data_size)
{
	...
	*trace_data_grc_addr =
	...
	if (signature != MFW_TRACE_SIGNATURE)
		return DBG_STATUS_ILWALID_TRACE_SIGNATURE;
...
/* Reads MCP trace meta data image from LWRAM
 * - running_bundle_id (OUT): running bundle ID (invalid when loaded from file)
 * - trace_meta_offset (OUT): trace meta offset in LWRAM in bytes (invalid when
 *			      loaded from file).
...
static enum dbg_status qed_mcp_trace_get_meta_info(struct ecore_hwfn *p_hwfn,
						   ...
						   u32 *trace_meta_size)
{
	u32 spad_trace_offsize, lwram_image_type, running_mfw_addr;
	...
	*running_bundle_id = ecore_rd(p_hwfn, p_ptt, running_mfw_addr);
	if (*running_bundle_id > 1)
		return DBG_STATUS_ILWALID_LWRAM_BUNDLE;

	/* Find image in LWRAM */
	lwram_image_type =
	    (*running_bundle_id ==
	     DIR_ID_1) ? LWM_TYPE_MFW_TRACE1 : LWM_TYPE_MFW_TRACE2;
	return qed_find_lwram_image(p_hwfn,
				    p_ptt,
				    lwram_image_type,
				    trace_meta_offset, trace_meta_size);
}

/* Reads the MCP Trace meta data from LWRAM into the specified buffer */
static enum dbg_status qed_mcp_trace_read_meta(struct ecore_hwfn *p_hwfn,
					       struct ecore_ptt *p_ptt,
					       u32 lwram_offset_in_bytes,
					       u32 size_in_bytes, u32 *buf)
{
	...
	/* Read meta data from LWRAM */
	status = qed_lwram_read(p_hwfn,
				p_ptt,
				lwram_offset_in_bytes, size_in_bytes, buf);
	if (status != DBG_STATUS_OK)
		return status;
					  ...
					  bool dump, u32 *num_dumped_dwords)
{
	...
	*num_dumped_dwords = offset;
	 ...
	 * data from LWRAM.
	 */
	return use_mfw ? status : DBG_STATUS_LWRAM_GET_IMAGE_FAILED;
}
				    ...
				    const u8 * const bin_ptr)
{
	...
	/* Colwert binary data to debug arrays */
				   ...
				   enum dbg_grc_params grc_param, u32 val)
{
	...
	if (grc_param >= MAX_DBG_GRC_PARAMS)
		return DBG_STATUS_ILWALID_ARGS;
				       ...
				       u32 *num_dumped_dwords)
{
	...
	if (status != DBG_STATUS_OK && status !=
	    DBG_STATUS_LWRAM_GET_IMAGE_FAILED)
		return status;
	if (buf_size_in_dwords < needed_buf_size_in_dwords)
		return DBG_STATUS_DUMP_BUF_TOO_SMALL;
...
static const char * const s_status_str[] = {
	...
	/* DBG_STATUS_ILWALID_ARGS */
	"Invalid arguments",

	/* DBG_STATUS_OUTPUT_ALREADY_SET */
	"The debug output was already set",

	/* DBG_STATUS_ILWALID_PCI_BUF_SIZE */
	...
	/* DBG_STATUS_ILWALID_FILTER_TRIGGER_DWORDS */
	...
	/* DBG_STATUS_ILWALID_TRACE_SIGNATURE */
	"Invalid MCP trace signature found in LWRAM",

	/* DBG_STATUS_ILWALID_LWRAM_BUNDLE */
	"Invalid bundle ID found in LWRAM",

	/* DBG_STATUS_LWRAM_GET_IMAGE_FAILED */
	"Failed getting LWRAM image",

	/* DBG_STATUS_NON_ALIGNED_LWRAM_IMAGE */
	"LWRAM image is not dword-aligned",

	/* DBG_STATUS_LWRAM_READ_FAILED */
	"Failed reading from LWRAM",
	...
	/* DBG_STATUS_ILWALID_STORM_DBG_MODE */
...
/* Temporary buffer, used for print size callwlations */
						  ...
						  u32 *results_offset)
{
	...
	for (i = 0; i < OSAL_ARRAY_SIZE(s_igu_fifo_addr_data) && !found_addr;
	     i++) {
		const struct igu_fifo_addr_data *lwrr_addr =
			&s_igu_fifo_addr_data[i];

		if (cmd_addr >= lwrr_addr->start_addr && cmd_addr <=
		    lwrr_addr->end_addr)
			found_addr = lwrr_addr;
	}
				    ...
				    enum ecore_dbg_features feature_idx)
{
	...
	rc = qed_features_lookup[feature_idx].get_size(p_hwfn, p_ptt,
						       &buf_size_dwords);
	if (rc != DBG_STATUS_OK && rc != DBG_STATUS_LWRAM_GET_IMAGE_FAILED)
		return rc;
...
static int qed_dbg_lwm_image_length(struct ecore_hwfn *p_hwfn,
				    enum ecore_lwm_images image_id, u32 *length)
{
...
{
	u8 lwr_engine, omit_engine = 0, org_engine;
	struct ecore_hwfn *p_hwfn =
	...
	for (lwr_engine = 0; lwr_engine < edev->num_hwfns; lwr_engine++) {
		...
		qed_set_debug_engine(edev, lwr_engine);
		...
		if (!rc) {
			*(u32 *)((u8 *)buffer + offset) =
			    qed_calc_regdump_header(edev, IDLE_CHK, lwr_engine,
		...
		if (!rc) {
			*(u32 *)((u8 *)buffer + offset) =
			    qed_calc_regdump_header(edev, REG_FIFO, lwr_engine,
						    feature_size, omit_engine);
			offset += (feature_size + REGDUMP_HEADER_SIZE);
		} else {
		...
		if (!rc) {
			*(u32 *)((u8 *)buffer + offset) =
			    qed_calc_regdump_header(edev, IGU_FIFO, lwr_engine,
						    feature_size, omit_engine);
			offset += (feature_size + REGDUMP_HEADER_SIZE);
		} else {
	...
	if (!rc) {
		*(u32 *)((u8 *)buffer + offset) =
		    qed_calc_regdump_header(edev, MCP_TRACE, lwr_engine,
					    feature_size, omit_engine);
		offset += (feature_size + REGDUMP_HEADER_SIZE);
	} else {
...
int qed_dbg_all_data_size(struct ecore_dev *edev)
{
	...
	for (lwr_engine = 0; lwr_engine < edev->num_hwfns; lwr_engine++) {
		/* Engine specific */
		DP_VERBOSE(edev, ECORE_MSG_DEBUG,
			   "callwlating idle_chk and grcdump register length for current engine\n");
		qed_set_debug_engine(edev, lwr_engine);
	...
	qed_dbg_lwm_image_length(p_hwfn, ECORE_LWM_IMAGE_LWM_CFG1, &image_len);
	if (image_len)
		regs_len += REGDUMP_HEADER_SIZE + image_len;
	qed_dbg_lwm_image_length(p_hwfn, ECORE_LWM_IMAGE_DEFAULT_CFG,
				 &image_len);
	if (image_len)
		regs_len += REGDUMP_HEADER_SIZE + image_len;
	qed_dbg_lwm_image_length(p_hwfn, ECORE_LWM_IMAGE_LWM_META, &image_len);
	if (image_len)
		regs_len += REGDUMP_HEADER_SIZE + image_len;
	qed_dbg_lwm_image_length(p_hwfn, ECORE_LWM_IMAGE_MDUMP, &image_len);
	if (image_len)
		regs_len += REGDUMP_HEADER_SIZE + image_len;
