		  ...
		  struct rte_eth_dev_info *dev_info)
{
	...
	if (link.adv_speed & LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_1G)
		speed_cap |= ETH_LINK_SPEED_1G;
	if (link.adv_speed & LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_10G)
		speed_cap |= ETH_LINK_SPEED_10G;
	if (link.adv_speed & LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_25G)
		speed_cap |= ETH_LINK_SPEED_25G;
	if (link.adv_speed & LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_40G)
		speed_cap |= ETH_LINK_SPEED_40G;
	if (link.adv_speed & LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_50G)
		speed_cap |= ETH_LINK_SPEED_50G;
	if (link.adv_speed & LWM_CFG1_PORT_DRV_SPEED_CAPABILITY_MASK_BB_100G)
...
static int qede_promislwous_enable(struct rte_eth_dev *eth_dev)
{
...
static int qede_promislwous_disable(struct rte_eth_dev *eth_dev)
{
	struct qede_dev *qdev = eth_dev->data->dev_private;
	struct ecore_dev *edev = &qdev->edev;
	enum _ecore_status_t ecore_status;
...
static int qede_allmulticast_disable(struct rte_eth_dev *eth_dev)
{
	...
	if (rte_eth_promislwous_get(eth_dev->data->port_id) == 1)
		ecore_status = qed_configure_filter_rx_mode(eth_dev,
	...
	if (IS_PF(edev)) {
		...
		for_each_hwfn(edev, i) {
			...
			if (rc == ECORE_ILWAL) {
				...
				if (eth_dev->data->promislwous)
					qede_promislwous_enable(eth_dev);
				else
					qede_promislwous_disable(eth_dev);
			      ...
			      struct rte_eth_fc_conf *fc_conf)
{
	struct qede_dev *qdev = QEDE_INIT_QDEV(eth_dev);
	struct ecore_dev *edev = QEDE_INIT_EDEV(qdev);
	struct qed_link_output lwrrent_link;
	struct qed_link_params params;

	memset(&lwrrent_link, 0, sizeof(lwrrent_link));
	qdev->ops->common->get_link(edev, &lwrrent_link);

	memset(&params, 0, sizeof(params));
	params.override_flags |= QED_LINK_OVERRIDE_PAUSE_CONFIG;
	if (fc_conf->autoneg) {
		if (!(lwrrent_link.supported_caps & QEDE_SUPPORTED_AUTONEG)) {
			      ...
			      struct rte_eth_fc_conf *fc_conf)
{
	...
	if (lwrrent_link.pause_config & QED_LINK_PAUSE_AUTONEG_ENABLE)
		fc_conf->autoneg = true;

	if (lwrrent_link.pause_config & (QED_LINK_PAUSE_RX_ENABLE |
					 QED_LINK_PAUSE_TX_ENABLE))
		fc_conf->mode = RTE_FC_FULL;
	else if (lwrrent_link.pause_config & QED_LINK_PAUSE_RX_ENABLE)
		fc_conf->mode = RTE_FC_RX_PAUSE;
	else if (lwrrent_link.pause_config & QED_LINK_PAUSE_TX_ENABLE)
...
static const struct eth_dev_ops qede_eth_dev_ops = {
	...
	.promislwous_enable = qede_promislwous_enable,
	.promislwous_disable = qede_promislwous_disable,
