...
{
	uint32_t lwrr_prog_id = 0;

	if (bpf_get_link_xdp_id(internals->if_index, &lwrr_prog_id,
...
static int
load_lwstom_xdp_prog(const char *prog_path, int if_index)
{
	int ret, prog_fd = -1;
	      ...
	      int ring_size)
{
	...
	if (strnlen(internals->prog_path, PATH_MAX) &&
				!internals->lwstom_prog_configured) {
		ret = load_lwstom_xdp_prog(internals->prog_path,
					   internals->if_index);
		if (ret) {
		...
		internals->lwstom_prog_configured = 1;
	}
...
static int
eth_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct pmd_internals *internals = dev->data->dev_private;
...
static int
eth_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct pmd_internals *internals = dev->data->dev_private;
...
static const struct eth_dev_ops ops = {
	...
	.promislwous_enable = eth_dev_promislwous_enable,
	.promislwous_disable = eth_dev_promislwous_disable,
		...
		const char *prog_path)
{
	...
	internals->lwstom_prog_configured = 0;
