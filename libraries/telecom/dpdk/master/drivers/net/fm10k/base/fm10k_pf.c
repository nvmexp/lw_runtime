...
STATIC s32 fm10k_read_mac_addr_pf(struct fm10k_hw *hw)
{
	...
	if ((~serial_num) << 24)
		return  FM10K_ERR_ILWALID_MAC_ADDR;
 ...
 *  so that it can enable either multicast, multicast promislwous, or
 *  promislwous mode of operation.
 **/
					 ...
					 u16 num_pools)
{
	...
	/* callwlate starting index for queues */
				 ...
				 struct fm10k_mbx_info *mbx)
{
	...
	if (!!results[FM10K_LPORT_STATE_MSG_XCAST_MODE]) {
		...
		/* if mode is not lwrrently enabled, enable it */
...
{
	/* colwert from le32 struct to CPU byte ordered values */
...
/* lwrrently there is no shared 1588 timestamp handler */
...
STATIC s32 fm10k_adjust_systime_pf(struct fm10k_hw *hw, s32 ppb)
{
	...
	/* we must colwert the value from parts per billion to parts per
