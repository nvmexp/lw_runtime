...
static int fm10k_dev_promislwous_enable(struct rte_eth_dev *dev);
static int fm10k_dev_promislwous_disable(struct rte_eth_dev *dev);
 ...
 * return -EILWAL if buffers do not satisfy alignment condition
 */
static inline int
...
static int
fm10k_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	int status;
	...
	if (status != FM10K_SUCCESS) {
		PMD_INIT_LOG(ERR, "Failed to enable promislwous mode");
		return -EAGAIN;
	}
...
static int
fm10k_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct fm10k_hw *hw = FM10K_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint8_t mode;
	int status;
	...
	if (status != FM10K_SUCCESS) {
		PMD_INIT_LOG(ERR, "Failed to disable promislwous mode");
		return -EAGAIN;
	}
...
fm10k_dev_allmulticast_enable(struct rte_eth_dev *dev)
{
	...
	/* If promislwous mode is enabled, it doesn't make sense to enable
	 * allmulticast and disable promislwous since fm10k only can select
	 * one of the modes.
	 */
	if (dev->data->promislwous) {
		PMD_INIT_LOG(INFO, "Promislwous mode is enabled, "\
...
fm10k_dev_dglort_map_configure(struct rte_eth_dev *dev)
{
	...
	/* Ilwalidate all other GLORT entries */
...
fm10k_dev_handle_fault(struct fm10k_hw *hw, uint32_t eicr)
{
	...
	if (eicr & FM10K_EICR_FUM_FAULT) {
		...
		case FUM_ILWALID_TYPE:
			estr = "FUM_ILWALID_TYPE"; break;
		case FUM_ILWALID_LENGTH:
			estr = "FUM_ILWALID_LENGTH"; break;
		case FUM_ILWALID_BE:
			estr = "FUM_ILWALID_BE"; break;
		case FUM_ILWALID_ALIGN:
			estr = "FUM_ILWALID_ALIGN"; break;
		default:
			goto error;
		}
		PMD_INIT_LOG(ERR, "%s: %s(%d) Addr:0x%"PRIx64" Spec: 0x%x",
...
static const struct eth_dev_ops fm10k_eth_dev_ops = {
	...
	.promislwous_enable     = fm10k_dev_promislwous_enable,
	.promislwous_disable    = fm10k_dev_promislwous_disable,
