...
#define HINIC_NONTSO_SEG_NUM_ILWALID(num)	\
			((num) > HINIC_NONTSO_PKT_MAX_SGE)

#define HINIC_TSO_PKT_MAX_SGE			127	/* tso max sge 127 */
#define HINIC_TSO_SEG_NUM_ILWALID(num)		((num) > HINIC_TSO_PKT_MAX_SGE)
...
static inline void hinic_sq_wqe_cpu_to_be32(void *data, int nr_wqebb)
{
	...
	for (i = 0; i < nr_wqebb; i++) {
		/* colwert 64B wqebb using 4 SSE instructions */
...
static inline void hinic_sge_cpu_to_be32(void *data, int nr_sge)
{
	...
	for (i = 0; i < nr_sge; i++) {
		/* colwert 16B sge using 1 SSE instructions */
		 ...
		 struct hinic_wqe_info *wqe_info)
{
	...
	/* record current pi */
	lwr_pi = MASKED_WQE_IDX(wq, wq->prod_idx);
	end_pi = lwr_pi + wqebb_cnt;
	...
	if (unlikely(end_pi >= txq->q_depth)) {
		...
		if (unlikely(end_pi > txq->q_depth)) {
			wqe_info->around = 1;
			remain_wqebbs = txq->q_depth - lwr_pi;
			wqe_info->seq_wqebbs = remain_wqebbs;
		}
	}

	return (struct hinic_sq_wqe *)WQ_WQE_ADDR(wq, lwr_pi);
}
...
static inline void hinic_callwlate_tcp_checksum(struct rte_mbuf *mbuf,
					struct hinic_tx_offload_info *off_info,
...
static inline void hinic_callwlate_udp_checksum(struct rte_mbuf *mbuf,
					struct hinic_tx_offload_info *off_info,
					uint64_t inner_l3_offset)
{
	struct rte_ipv4_hdr *ipv4_hdr;
...
static inline void
hinic_callwlate_sctp_checksum(struct hinic_tx_offload_info *off_info)
{
...
static inline void hinic_callwlate_checksum(struct rte_mbuf *mbuf,
					struct hinic_tx_offload_info *off_info,
					uint64_t inner_l3_offset)
{
	uint64_t ol_flags = mbuf->ol_flags;
	...
	case PKT_TX_UDP_CKSUM:
		hinic_callwlate_udp_checksum(mbuf, off_info, inner_l3_offset);
		break;

	case PKT_TX_TCP_CKSUM:
		hinic_callwlate_tcp_checksum(mbuf, off_info, inner_l3_offset);
		break;

	case PKT_TX_SCTP_CKSUM:
		hinic_callwlate_sctp_checksum(off_info);
		break;

	default:
		if (ol_flags & PKT_TX_TCP_SEG)
			hinic_callwlate_tcp_checksum(mbuf, off_info,
						     inner_l3_offset);
		break;
	}
}
					...
					struct hinic_tx_offload_info *off_info)
{
	...
	if (ol_flags & PKT_TX_TUNNEL_VXLAN) {
		...
		/* inner_l4_tcp_udp csum should be set to callwlate outer
	...
	hinic_callwlate_checksum(m, off_info, inner_l3_offset);
					    ...
					    *off_info)
{
	...
	if (likely(!(mbuf_pkt->ol_flags & PKT_TX_TCP_SEG))) {
		if (unlikely(mbuf_pkt->pkt_len > MAX_SINGLE_SGE_SIZE)) {
			...
			return false;
		} else if (unlikely(HINIC_NONTSO_SEG_NUM_ILWALID(sge_cnt))) {
		...
		/* tso mbuf */
		if (unlikely(HINIC_TSO_SEG_NUM_ILWALID(sge_cnt)))
			/* too many mbuf segs */
			return false;
...
u16 hinic_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, u16 nb_pkts)
{
	...
	for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
		...
		/* 8.colwert continue or bottom wqe byteorder to big endian */
