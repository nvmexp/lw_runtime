				...
				struct rte_flow_error *error)
{
	...
	if (!attr) {
		rte_flow_error_set(error, EILWAL, RTE_FLOW_ERROR_TYPE_ATTR,
				   NULL, "NULL attribute.");
		return -rte_errno;
	}
...
static int hinic_set_lacp_tcam(struct hinic_nic_dev *nic_dev)
{
	...
	/* LACP TCAM action */
	lacp_action.err_type = 0x3f; /* err from ipsu, not colwert */
	lacp_action.fwd_action = 0x7; /* 0x3:drop; 0x7: not colwert */
	lacp_action.pkt_type = PKT_LACP_TYPE;
	lacp_action.pri = 0x0;
	lacp_action.push_len = 0xf; /* push_len:0xf, not colwert */
...
static int hinic_set_bgp_dport_tcam(struct hinic_nic_dev *nic_dev)
{
	...
	/* BGP TCAM action */
	bgp_action.err_type = 0x3f; /* err from ipsu, not colwert */
	bgp_action.fwd_action = 0x7; /* 0x3:drop; 0x7: not colwert */
	bgp_action.pkt_type = PKT_BGPD_DPORT_TYPE; /* bgp_dport: 83 */
	bgp_action.pri = 0xf; /* pri of BGP is 0xf, result from ipsu parse
			       * results, not need to colwert
			       */
	bgp_action.push_len = 0xf; /* push_len:0xf, not colwert */
...
static void tcam_key_callwlate(struct tag_tcam_key *tcam_key,
				     ...
				     struct tag_tcam_cfg_rule *fdir_tcam_rule)
{
	...
	tcam_key_callwlate(tcam_key, fdir_tcam_rule);
