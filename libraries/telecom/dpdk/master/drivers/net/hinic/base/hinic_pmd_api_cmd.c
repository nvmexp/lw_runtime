...
static void prepare_cell(struct hinic_api_cmd_chain *chain,
			 ...
			 void *cmd, u16 cmd_size)
{
	struct hinic_api_cmd_cell *lwrr_node;
	u16 cell_size;

	lwrr_node = chain->lwrr_node;

	cell_size = get_cell_data_size(chain->chain_type, cmd_size);

	prepare_cell_ctrl(&lwrr_node->ctrl, cell_size);
	prepare_api_cmd(chain, lwrr_node, dest, cmd, cmd_size);
}
		   ...
		   void *cmd, u16 cmd_size, void *ack, u16 ack_size)
{
	...
	chain->lwrr_node = chain->cell_ctxt[chain->prod_idx].cell_vaddr;
...
static int api_cmd_create_cells(struct hinic_api_cmd_chain *chain)
{
	...
	chain->lwrr_node = chain->head_node;
	return 0;
