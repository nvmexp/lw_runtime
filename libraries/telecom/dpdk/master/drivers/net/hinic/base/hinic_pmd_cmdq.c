...
#define CMDQ_CTXT_LWRR_WQE_PAGE_PFN_SHIFT		0
...
#define CMDQ_CTXT_LWRR_WQE_PAGE_PFN_MASK		0xFFFFFFFFFFFFF
				 ...
				 struct hinic_cmdq_ctxt *cmdq_ctxt)
{
	...
	ctxt_info->lwrr_wqe_page_pfn =
		...
		CMDQ_CTXT_PAGE_INFO_SET(pfn, LWRR_WQE_PAGE_PFN);
...
{
	struct hinic_wq *wq = cmdq->wq;
	struct hinic_cmdq_wqe *lwrr_wqe, wqe;
	struct hinic_cmdq_wqe_lcmd *wqe_lcmd;
	u16 lwrr_prod_idx, next_prod_idx, num_wqebbs;
	int wrapped;
	u32 timeo, wqe_size;
	int err;
	...
	lwrr_wqe = hinic_get_wqe(cmdq->wq, num_wqebbs, &lwrr_prod_idx);
	if (!lwrr_wqe) {
		err = -EBUSY;
		goto cmdq_unlock;
	}
	...
	next_prod_idx = lwrr_prod_idx + num_wqebbs;
	...
	cmdq_set_lcmd_wqe(&wqe, SYNC_CMD_DIRECT_RESP, buf_in, NULL,
			  wrapped, ack_type, mod, cmd, lwrr_prod_idx);

	/* The data that is written to HW should be in Big Endian Format */
	hinic_cpu_to_be32(&wqe, wqe_size);

	/* CMDQ WQE is not shadow, therefore wqe will be written to wq */
	cmdq_wqe_fill(lwrr_wqe, &wqe);

	cmdq->cmd_infos[lwrr_prod_idx].cmd_type = HINIC_CMD_TYPE_NORMAL;
	...
	if (out_param) {
		wqe_lcmd = &lwrr_wqe->wqe_lcmd;
		*out_param = cpu_to_be64(wqe_lcmd->completion.direct_resp);
	}

	if (cmdq->errcode[lwrr_prod_idx] > 1) {
		err = cmdq->errcode[lwrr_prod_idx];
		goto cmdq_unlock;
	}
