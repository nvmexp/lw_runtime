...
static int init_sq_ctxts(struct hinic_nic_io *nic_io)
{
	...
	struct hinic_qp *qp;
	u64 out_param = EIO;
	u16 q_id, lwrr_id, global_qpn, max_ctxts, i;
	int err = 0;
	...
	while (q_id < nic_io->num_sqs) {
		...
		for (i = 0; i < max_ctxts; i++) {
			lwrr_id = q_id + i;
			qp = &nic_io->qps[lwrr_id];
			global_qpn = nic_io->global_qpn + lwrr_id;
...
/**
 * hinic_colwert_rx_buf_size - colwert rx buffer size to hw size
 ...
 *   negative error value otherwise.
 */
int hinic_colwert_rx_buf_size(u32 rx_buf_sz, u32 *match_sz)
{
