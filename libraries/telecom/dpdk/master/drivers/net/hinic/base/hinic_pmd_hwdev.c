...
/**
 * hinic_cpu_to_be32 - colwert data to big endian 32 bit format
 * @data: the data to colwert
 * @len: length of data to colwert, must be Multiple of 4B
 */
...
/**
 * hinic_be32_to_cpu - colwert data from big endian 32 bit format
 * @data: the data to colwert
 * @len: length of data to colwert, must be Multiple of 4B
 */
				    ...
				    void *buf_out, u16 *out_size)
{
	...
	PMD_DRV_LOG(ERR, "Mgmt deadloop time: 0x%x 0x%x, task id: 0x%x, sp: 0x%x",
		watchdog_info->lwrr_time_h, watchdog_info->lwrr_time_l,
	...
	PMD_DRV_LOG(ERR, "Stack current used: 0x%x, peak used: 0x%x, overflow flag: 0x%x, top: 0x%x, bottom: 0x%x",
		watchdog_info->lwrr_used, watchdog_info->peak_used,
