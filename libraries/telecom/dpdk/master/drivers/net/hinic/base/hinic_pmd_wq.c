...
void *hinic_read_wqe(struct hinic_wq *wq, int num_wqebbs, u16 *cons_idx)
{
	...
	lwrr_cons_idx = (u16)(wq->cons_idx);

	lwrr_cons_idx = MASKED_WQE_IDX(wq, lwrr_cons_idx);

	*cons_idx = lwrr_cons_idx;
...
void *hinic_get_wqe(struct hinic_wq *wq, int num_wqebbs, u16 *prod_idx)
{
	...
	wq->delta -= num_wqebbs;
	lwrr_prod_idx = wq->prod_idx;
	wq->prod_idx += num_wqebbs;
	*prod_idx = MASKED_WQE_IDX(wq, lwrr_prod_idx);
