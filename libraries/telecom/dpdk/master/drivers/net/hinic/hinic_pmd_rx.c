...
{
	struct hinic_nic_dev *nic_dev = rxq->nic_dev;
	struct rte_mbuf *lwr_mbuf, *rxm = NULL;
	...
	while (remain_pkt_len > 0) {
		...
		lwr_mbuf = rx_info->mbuf;
		lwr_mbuf->data_len = (u16)pkt_len;
		lwr_mbuf->next = NULL;

		head_mbuf->pkt_len += lwr_mbuf->data_len;
		head_mbuf->nb_segs++;

		if (!rxm)
			head_mbuf->next = lwr_mbuf;
		else
			rxm->next = lwr_mbuf;
...
u16 hinic_recv_pkts(void *rx_queue, struct rte_mbuf **rx_pkts, u16 nb_pkts)
{
	...
	while (pkts < nb_pkts) {
		...
		/* colwert cqe and get packet length */
