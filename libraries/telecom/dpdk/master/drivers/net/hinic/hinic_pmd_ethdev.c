			 ...
			 struct rte_mempool *mp)
{
	...
	rc =
	hinic_colwert_rx_buf_size(rte_pktmbuf_data_room_size(rxq->mb_pool) -
...
static int hinic_set_dev_promislwous(struct hinic_nic_dev *nic_dev, bool enable)
{
	u32 rx_mode_ctrl;
	int err;
 ...
 *   negative error value otherwise.
 */
static int hinic_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	int rc = HINIC_OK;
	struct hinic_nic_dev *nic_dev = HINIC_ETH_DEV_TO_PRIVATE_NIC_DEV(dev);

	PMD_DRV_LOG(INFO, "Enable promislwous, nic_dev: %s, port_id: %d, promisc: %d",
		    nic_dev->proc_dev_name, dev->data->port_id,
		    dev->data->promislwous);

	rc = hinic_set_dev_promislwous(nic_dev, true);
	if (rc)
		PMD_DRV_LOG(ERR, "Enable promislwous failed");
 ...
 *   negative error value otherwise.
 */
static int hinic_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	int rc = HINIC_OK;
	struct hinic_nic_dev *nic_dev = HINIC_ETH_DEV_TO_PRIVATE_NIC_DEV(dev);

	PMD_DRV_LOG(INFO, "Disable promislwous, nic_dev: %s, port_id: %d, promisc: %d",
		    nic_dev->proc_dev_name, dev->data->port_id,
		    dev->data->promislwous);

	rc = hinic_set_dev_promislwous(nic_dev, false);
	if (rc)
		PMD_DRV_LOG(ERR, "Disable promislwous failed");
...
static const struct eth_dev_ops hinic_pmd_ops = {
	...
	.promislwous_enable            = hinic_dev_promislwous_enable,
	.promislwous_disable           = hinic_dev_promislwous_disable,
