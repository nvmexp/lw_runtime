...
static int  atl_dev_promislwous_enable(struct rte_eth_dev *dev);
static int  atl_dev_promislwous_disable(struct rte_eth_dev *dev);
...
static struct atl_xstats_tbl_s atl_xstats_tbl[] = {
	...
	ATL_MACSEC_XSTATS_FIELD(in_ilwalid_pkts),
...
static const struct eth_dev_ops atl_eth_dev_ops = {
	...
	/* PROMISC */
	.promislwous_enable   = atl_dev_promislwous_enable,
	.promislwous_disable  = atl_dev_promislwous_disable,
...
atl_dev_configure_macsec(struct rte_eth_dev *dev)
{
	...
	msg_macsec.txsc.lwrr_an = 0; /* SA index which lwrrently used */
...
atl_dev_stats_get(struct rte_eth_dev *dev, struct rte_eth_stats *stats)
{
	...
	stats->ipackets = hw->lwrr_stats.dma_pkt_rc;
	stats->ibytes = hw->lwrr_stats.dma_oct_rc;
	stats->imissed = hw->lwrr_stats.dpc;
	stats->ierrors = hw->lwrr_stats.erpt;

	stats->opackets = hw->lwrr_stats.dma_pkt_tc;
	stats->obytes = hw->lwrr_stats.dma_oct_tc;
	stats->oerrors = 0;
...
atl_dev_stats_reset(struct rte_eth_dev *dev)
{
	...
	memset(&hw->lwrr_stats, 0, sizeof(hw->lwrr_stats));
		   ...
		   unsigned int n)
{
	...
	for (i = 0; i < n && i < count; i++) {
		...
		case XSTATS_TYPE_MSM:
			stats[i].value = *(u64 *)((uint8_t *)&hw->lwrr_stats +
...
static int
atl_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct aq_hw_s *hw = ATL_DEV_PRIVATE_TO_HW(dev->data->dev_private);

	hw_atl_rpfl2promislwous_mode_en_set(hw, true);

	return 0;
}

static int
atl_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct aq_hw_s *hw = ATL_DEV_PRIVATE_TO_HW(dev->data->dev_private);

	hw_atl_rpfl2promislwous_mode_en_set(hw, false);
