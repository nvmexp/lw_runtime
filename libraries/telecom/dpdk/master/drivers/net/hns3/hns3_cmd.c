...
static int
hns3_cmd_colwert_err_code(uint16_t desc_ret)
{
	switch (desc_ret) {
			    ...
			    struct hns3_cmd_desc *desc, int num, int ntc)
{
	...
	for (handle = 0; handle < num; handle++) {
		...
		desc[handle] = hw->cmq.csq.desc[lwrrent_ntc];

		lwrrent_ntc++;
		if (lwrrent_ntc == hw->cmq.csq.desc_num)
			lwrrent_ntc = 0;
	}
	...
	return hns3_cmd_colwert_err_code(desc_ret);
}
