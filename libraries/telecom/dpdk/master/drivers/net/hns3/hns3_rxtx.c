...
hns3_init_tunnel_ptype_tbl(struct hns3_ptype_table *tbl)
{
	...
	tbl->ol4table[2] = RTE_PTYPE_TUNNEL_LWGRE;
}
...
static inline void
recallwlate_data_len(struct rte_mbuf *first_seg, struct rte_mbuf *last_seg,
			 ...
			 uint16_t nb_pkts)
{
	...
	while (nb_rx < nb_pkts) {
		...
		if (unlikely(rxq->crc_len > 0)) {
			...
			recallwlate_data_len(first_seg, last_seg, rxm, rxq,
				rxm->data_len);
		}
		     ...
		     struct rte_mbuf *rxm)
{
	...
	/*
	 * Lwrrently, hardware doesn't support more than two layers VLAN offload
...
#define HNS3_LWGRE_HLEN 8
	...
	case PKT_TX_TUNNEL_VXLAN:
		 ...
		 * Therefore, driver need to callwlate the outer L4 length and
		 * inner L2 length.
		 */
	...
	case PKT_TX_TUNNEL_GRE:
		tmp_outer |= hns3_gen_field_val(HNS3_TXD_TUNTYPE_M,
					HNS3_TXD_TUNTYPE_S, HNS3_TUN_LWGRE);
		/*
		 * For LWGRE tunnel packect, the outer L4 is empty. So only
		 * fill the LWGRE header length to the outer L4 field.
		 */
		tmp_outer |= hns3_gen_field_val(HNS3_TXD_L4LEN_M,
				HNS3_TXD_L4LEN_S,
				(uint8_t)HNS3_LWGRE_HLEN >> HNS3_L4_LEN_UNIT);

		inner_l2_len = m->l2_len - HNS3_LWGRE_HLEN;
		break;
	default:
			    ...
			    uint16_t tx_desc_id)
{
	 ...
	 * callwlations, the length of the L2 header include the outer and
				 ...
				 uint32_t max_non_tso_bd_num)
{
	 ...
	 * conselwtive buffers is greater than MSS in hns3 network engine.
	 * We simplify it by ensuring pkt_headlen + the first 8 conselwtive
	 * frags greater than gso header len + mss, and the remaining 7
	 * conselwtive frags greater than MSS except the last 7 frags.
	 */
	...
	/*
	 * ensure the sum of the data length of every 7 conselwtive buffer
...
hns3_prep_pkt_proc(struct hns3_tx_queue *tx_queue, struct rte_mbuf *m)
{
	...
	if (hns3_pkt_is_tso(m)) {
		...
		if (tx_queue->tso_mode != HNS3_TSO_SW_CAL_PSEUDO_H_CSUM) {
			 ...
			 * hardware support recallwlate the TCP pseudo header
			 * checksum of packets that need TSO, so network driver
			 * software not need to recallwlate it.
			 */
