...
/*
 * hns3_shaper_para_calc: callwlate ir parameter for the shaper
...
hns3_dcb_port_shaper_cfg(struct hns3_hw *hw)
{
	...
	if (ret) {
		hns3_err(hw, "callwlate shaper parameter failed: %d", ret);
		return ret;
	}
	 ...
	 * field in hns3_port_shapping_cmd to require firmware to recallwlate
	 * shapping parameters. And whether the parameters are recallwlated
	 * depends on the firmware version. But driver still needs to
	 * callwlate it and configure to firmware for better compatibility.
	 */
			 ...
			 uint8_t pg_id, uint32_t shapping_para, uint32_t rate)
{
	 ...
	 * hns3_pg_shapping_cmd to require firmware to recallwlate shapping
	 * parameters. And whether parameters are recallwlated depends on
	 * the firmware version. But driver still needs to callwlate it and
...
hns3_dcb_pg_shaper_cfg(struct hns3_hw *hw)
{
	...
	for (i = 0; i < hw->dcb_info.num_pg; i++) {
		...
		ret = hns3_shaper_para_calc(hw, rate, HNS3_SHAPER_LVL_PG,
					    &shaper_parameter);
		if (ret) {
			hns3_err(hw, "callwlate shaper parameter failed: %d",
				 ret);
			return ret;
		}
			  ...
			  uint8_t pri_id, uint32_t shapping_para, uint32_t rate)
{
	 ...
	 * field in hns3_pri_shapping_cmd to require firmware to recallwlate
	 * shapping parameters. And whether the parameters are recallwlated
	 * depends on the firmware version. But driver still needs to
	 * callwlate it and configure to firmware for better compatibility.
	 */
...
hns3_q_to_qs_map_cfg(struct hns3_hw *hw, uint16_t q_id, uint16_t qs_id)
{
	 ...
	 * configure qset_id. So we need to colwert qs_id to the follow
...
hns3_pfc_setup_hw(struct hns3_hw *hw)
{
	...
	if (hw->lwrrent_fc_status == HNS3_FC_STATUS_PFC)
		hns3_get_rx_tx_en_status(hw, &tx_en, &rx_en);
	else {
		tx_en = false;
		rx_en = false;
...
hns3_dcb_cfg_validate(struct hns3_adapter *hns, uint8_t *tc, bool *changed)
{
	...
	*tc = max_tc + 1;
	 ...
	 */
	if (hw->lwrrent_mode != HNS3_FC_FULL)
		*changed = true;
...
hns3_dcb_info_update(struct hns3_adapter *hns, uint8_t num_tc)
{
	...
	/* Lwrrently not support uncontinuous tc */
...
hns3_dcb_hw_configure(struct hns3_adapter *hns)
{
	...
	struct hns3_pf *pf = &hns->pf;
	struct hns3_hw *hw = &hns->hw;
	enum hns3_fc_status fc_status = hw->lwrrent_fc_status;
	enum hns3_fc_mode lwrrent_mode = hw->lwrrent_mode;
...
pfc_setup_fail:
	hw->lwrrent_mode = lwrrent_mode;
	hw->lwrrent_fc_status = fc_status;
...
hns3_dcb_init(struct hns3_hw *hw)
{
	 ...
	 * is only exelwted to initialize default configurations of dcb during
	...
	if (hw->adapter_state == HNS3_NIC_UNINITIALIZED) {
		...
		hw->lwrrent_mode = hw->requested_mode;
		pf->pause_time = HNS3_DEFAULT_PAUSE_TRANS_TIME;
		hw->lwrrent_fc_status = HNS3_FC_STATUS_NONE;
...
hns3_fc_enable(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf)
{
	...
	/*
	 * In fact, lwrrent_fc_status is HNS3_FC_STATUS_NONE when mode
	 * of flow control is configured to be HNS3_FC_NONE.
	 */
	if (hw->lwrrent_mode == HNS3_FC_NONE)
		hw->lwrrent_fc_status = HNS3_FC_STATUS_NONE;
	else
