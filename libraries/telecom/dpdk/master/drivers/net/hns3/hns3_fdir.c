 ...
 * |   1   |   1   |   invalid      |
 * ----------------------------------
 * Then for input key(k) and mask(v), we can callwlate the value by
 * the formulae:
 *	x = (~k) & v
 *	y = k & v
 */
...
static inline void hns3_fd_colwert_mac(uint8_t *key, uint8_t *mask,
...
static void hns3_fd_colwert_int16(uint32_t tuple, struct hns3_fdir_rule *rule,
...
static inline void hns3_fd_colwert_int32(uint32_t key, uint32_t mask,
					 uint8_t *val_x, uint8_t *val_y)
{
	uint32_t tmp_x_l;
	uint32_t tmp_y_l;
...
static bool hns3_fd_colwert_tuple(struct hns3_hw *hw,
				  ...
				  uint8_t *key_y, struct hns3_fdir_rule *rule)
{
	...
	switch (tuple) {
	case INNER_DST_MAC:
		hns3_fd_colwert_mac(key_conf->spec.dst_mac,
				    key_conf->mask.dst_mac, key_x, key_y);
		break;
	case INNER_SRC_MAC:
		hns3_fd_colwert_mac(key_conf->spec.src_mac,
	...
	case INNER_VLAN_TAG2:
	case INNER_ETH_TYPE:
		hns3_fd_colwert_int16(tuple, rule, key_x, key_y);
		break;
	case INNER_SRC_IP:
		hns3_fd_colwert_int32(key_conf->spec.src_ip[IP_ADDR_KEY_ID],
				      key_conf->mask.src_ip[IP_ADDR_KEY_ID],
				      key_x, key_y);
		break;
	case INNER_DST_IP:
		hns3_fd_colwert_int32(key_conf->spec.dst_ip[IP_ADDR_KEY_ID],
				      key_conf->mask.dst_ip[IP_ADDR_KEY_ID],
				      key_x, key_y);
		break;
	case INNER_SCTP_TAG:
		hns3_fd_colwert_int32(key_conf->spec.sctp_tag,
...
static void hns3_fd_colwert_meta_data(struct hns3_fd_key_cfg *cfg,
				      uint8_t vf_id,
				      ...
				      uint8_t *key_x, uint8_t *key_y)
{
	...
	for (i = 0; i < MAX_META_DATA; i++) {
		...
		if (i == TUNNEL_PACKET) {
			hns3_set_bit(meta_data, lwr_pos,
			...
			hns3_set_field(meta_data,
				       GENMASK(lwr_pos + tuple_size,
					       lwr_pos), lwr_pos, vlan_tag);
			lwr_pos += tuple_size;
		} else if (i == DST_VPORT) {
			port_number = hns3_get_port_number(0, vf_id);
			hns3_set_field(meta_data,
				       GENMASK(lwr_pos + tuple_size, lwr_pos),
				       lwr_pos, port_number);
			lwr_pos += tuple_size;
		}
	}
	...
	shift_bits = sizeof(meta_data) * HNS3_BITS_PER_BYTE - lwr_pos;
			   ...
			   struct hns3_fdir_rule *rule)
{
	...
	for (i = 0; i < MAX_TUPLE; i++) {
		...
		if (key_cfg->tuple_active & BIT(i)) {
			tuple_valid = hns3_fd_colwert_tuple(hw, i, lwr_key_x,
							    lwr_key_y, rule);
			if (tuple_valid) {
				lwr_key_x += tuple_size;
				lwr_key_y += tuple_size;
			}
		}
	}
	...
	hns3_fd_colwert_meta_data(key_cfg, vf_id, rule,
