 ...
 * engine, the algorithm callwlating shapping parameter is moved to firmware to
 ...
 * firmware to recallwlate shapping parameters. However, whether the parameters
 * are recallwlated depends on the firmware version. If firmware doesn't support
 * the callwlation of shapping parameters, such as on network engine with
 * revision id 0x21, the value driver callwlated will be used to configure to
 * hardware. On the contrary, firmware ignores configuration of driver
 * and recallwlates the parameter.
 */
