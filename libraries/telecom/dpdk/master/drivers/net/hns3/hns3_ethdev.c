...
#define HNS3_ILWALID_PVID		0xFFFF
...
hns3_vlan_offload_set(struct rte_eth_dev *dev, int mask)
{
	...
	if (tmp_mask & ETH_VLAN_FILTER_MASK) {
		/* ignore vlan filter configuration during promislwous mode */
...
hns3_remove_all_vlan_table(struct hns3_adapter *hns)
{
	...
	if (hw->port_base_vlan_cfg.pvid != HNS3_ILWALID_PVID) {
...
out:
	...
	hw->port_base_vlan_cfg.pvid = on ? pvid : HNS3_ILWALID_PVID;
	return ret;
}
...
hns3_init_vlan_config(struct hns3_adapter *hns)
{
	...
	if (rte_atomic16_read(&hw->reset.resetting) == 0) {
		...
		hw->port_base_vlan_cfg.pvid = HNS3_ILWALID_PVID;
	}
	 ...
	 */
	if (rte_atomic16_read(&hw->reset.resetting) == 0) {
		ret = hns3_vlan_pvid_configure(hns, HNS3_ILWALID_PVID, 0);
		if (ret) {
			   ...
			   int *rm_addr_num)
{
	...
	/* Callwlate the mc mac address list that should be removed */
	lwrrent_addr_num = hw->mc_addrs_num;
	for (i = 0; i < lwrrent_addr_num; i++) {
		addr = &hw->mc_addrs[i];
		same_addr = false;
	...
	/* Callwlate the mc mac address list that should be added */
	for (i = 0; i < mc_addr_num; i++) {
		addr = &mc_addr_set[i];
		same_addr = false;
		for (j = 0; j < lwrrent_addr_num; j++) {
			  ...
			  uint32_t nb_mc_addr)
{
	...
	/*
	 * Callwlate the mc mac address lists those should be removed and be
...
hns3_check_dcb_cfg(struct rte_eth_dev *dev)
{
	...
	if (hw->lwrrent_fc_status == HNS3_FC_STATUS_MAC_PAUSE) {
...
/*
 * hns3_rx_buffer_calc: callwlate the rx private buffer size for all TCs
 * @hw: pointer to struct hns3_hw
 * @buf_alloc: pointer to buffer callwlation data
 * @return: 0: callwlate sucessful, negative: fail
 */
static int
...
static int
hns3_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	...
	/*
	 * When promislwous mode was enabled, disable the vlan filter to let
	...
	if (offloads & DEV_RX_OFFLOAD_VLAN_FILTER) {
		...
		if (ret) {
			hns3_err(hw, "failed to enable promislwous mode due to "
				     "failure to disable vlan filter, ret = %d",
				 ret);
			err = hns3_set_promisc_mode(hw, false, allmulti);
			if (err)
				hns3_err(hw, "failed to restore promislwous "
					 "status after disable vlan filter "
					 "failed during enabling promislwous "
					 "mode, ret = %d", ret);
		}
	}
...
static int
hns3_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	bool allmulti = dev->data->all_multicast ? true : false;
	struct hns3_adapter *hns = dev->data->dev_private;
	struct hns3_hw *hw = &hns->hw;
	...
	/* when promislwous mode was disabled, restore the vlan filter status */
	offloads = dev->data->dev_conf.rxmode.offloads;
	if (offloads & DEV_RX_OFFLOAD_VLAN_FILTER) {
		ret = hns3_enable_vlan_filter(hns, true);
		if (ret) {
			hns3_err(hw, "failed to disable promislwous mode due to"
			...
			if (err)
				hns3_err(hw, "failed to restore promislwous "
					 ...
					 "failed during disabling promislwous "
					 "mode, ret = %d", ret);
		}
	}
	rte_spinlock_unlock(&hw->lock);
...
hns3_dev_allmulticast_disable(struct rte_eth_dev *dev)
{
	...
	/* If now in promislwous mode, must remain in all_multicast mode. */
	if (dev->data->promislwous)
		return 0;
...
hns3_dev_promisc_restore(struct hns3_adapter *hns)
{
	...
	if (hw->data->promislwous) {
		ret = hns3_set_promisc_mode(hw, true, true);
		if (ret)
			hns3_err(hw, "failed to restore promislwous mode, "
				 "ret = %d", ret);
		return ret;
	}
...
{
	struct hns3_mac *mac = &hw->mac;
	uint32_t lwr_speed = mac->link_speed;
	int ret;
...
hns3_flow_ctrl_set(struct rte_eth_dev *dev, struct rte_eth_fc_conf *fc_conf)
{
	...
	if (!(hw->lwrrent_fc_status == HNS3_FC_STATUS_NONE ||
	    hw->lwrrent_fc_status == HNS3_FC_STATUS_MAC_PAUSE)) {
		hns3_err(hw, "PFC is enabled. Cannot set MAC pause. "
			 "lwrrent_fc_status = %d", hw->lwrrent_fc_status);
		return -EOPNOTSUPP;
	}

	hns3_get_fc_mode(hw, fc_conf->mode);
	if (hw->requested_mode == hw->lwrrent_mode &&
			    ...
			    struct rte_eth_pfc_conf *pfc_conf)
{
	...
	if (!(hw->lwrrent_fc_status == HNS3_FC_STATUS_NONE ||
	    hw->lwrrent_fc_status == HNS3_FC_STATUS_PFC)) {
	...
	if (hw->dcb_info.pfc_en & BIT(priority) &&
	    hw->requested_mode == hw->lwrrent_mode &&
...
static int
get_lwrrent_fec_auto_state(struct hns3_hw *hw, uint8_t *state)
{
...
#define QUERY_ACTIVE_SPEED	1
	...
	if (hw->mac.link_status == ETH_LINK_DOWN) {
		ret = get_lwrrent_fec_auto_state(hw, &auto_state);
		if (ret)
			return ret;
...
static uint32_t
get_lwrrent_speed_fec_cap(struct hns3_hw *hw, struct rte_eth_fec_capa *fec_capa)
{
	struct hns3_mac *mac = &hw->mac;
	uint32_t lwr_capa;
	...
	switch (mac->link_speed) {
	case ETH_SPEED_NUM_10G:
		lwr_capa = fec_capa[1].capa;
		break;
	case ETH_SPEED_NUM_25G:
	case ETH_SPEED_NUM_100G:
	case ETH_SPEED_NUM_200G:
		lwr_capa = fec_capa[0].capa;
		break;
	default:
		lwr_capa = 0;
		break;
...
#define FEC_CAPA_NUM 2
	 ...
	 */
	lwr_capa = get_lwrrent_speed_fec_cap(hw, fec_capa);
	if (!(lwr_capa & mode)) {
...
static const struct eth_dev_ops hns3_eth_dev_ops = {
	...
	.promislwous_enable = hns3_dev_promislwous_enable,
	.promislwous_disable = hns3_dev_promislwous_disable,
