						 ...
						 u16 header_len)
{
	...
	struct ena_com_llq_info *llq_info = &io_sq->llq_info;
	u8 *bounce_buffer = pkt_ctrl->lwrr_bounce_buf;
	u16 header_offset;
...
static void *get_sq_desc_llq(struct ena_com_io_sq *io_sq)
{
	...
	bounce_buffer = pkt_ctrl->lwrr_bounce_buf;
					   ...
					   u16 *first_cdesc_idx)
{
	...
	if (last) {
		*first_cdesc_idx = io_cq->lwr_rx_pkt_cdesc_start_idx;
		count += io_cq->lwr_rx_pkt_cdesc_count;

		head_masked = io_cq->head & (io_cq->q_depth - 1);

		io_cq->lwr_rx_pkt_cdesc_count = 0;
		io_cq->lwr_rx_pkt_cdesc_start_idx = head_masked;

		ena_trc_dbg("ena q_id: %d packets were completed. first desc idx %u descs# %d\n",
			    io_cq->qid, *first_cdesc_idx, count);
	} else {
		io_cq->lwr_rx_pkt_cdesc_count += count;
		count = 0;
	}
