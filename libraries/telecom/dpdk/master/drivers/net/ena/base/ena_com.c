					  ...
					  u16 command_id, bool capture)
{
	...
	if (unlikely(queue->comp_ctx[command_id].oclwpied && capture)) {
		ena_trc_err("Completion context is oclwpied\n");
		return NULL;
	}

	if (capture) {
		ATOMIC32_INC(&queue->outstanding_cmds);
		queue->comp_ctx[command_id].oclwpied = true;
	}
						       ...
						       size_t comp_size_in_bytes)
{
	...
	cmd_id = admin_queue->lwrr_cmd_id;
	...
	admin_queue->lwrr_cmd_id = (admin_queue->lwrr_cmd_id + 1) &
		queue_size_mask;
			      ...
			      struct ena_com_io_sq *io_sq)
{
	...
	if (io_sq->mem_queue_type == ENA_ADMIN_PLACEMENT_POLICY_DEV) {
		...
		io_sq->llq_buf_ctrl.lwrr_bounce_buf =
			ena_com_get_next_bounce_buffer(&io_sq->bounce_buf_ctrl);
		memset(io_sq->llq_buf_ctrl.lwrr_bounce_buf,
...
static int ena_com_set_llq(struct ena_com_dev *ena_dev)
{
	...
	ret = ena_com_exelwte_admin_command(admin_queue,
					    (struct ena_admin_aq_entry *)&cmd,
					    sizeof(cmd),
							...
							struct ena_com_admin_queue *admin_queue)
{
	...
	if (unlikely(comp_ctx->status == ENA_CMD_SUBMITTED)) {
		 ...
		 * and autopolling mode is enabled. Continuing normal exelwtion in such case
		 */
		if (!admin_queue->polling) {
			admin_queue->running_state = false;
			ret = ENA_COM_TIMER_EXPIRED;
				...
				u16 exp_state)
{
	...
	/* Colwert timeout from resolution of 100ms to us resolution. */
	...
	while (1) {
		...
		if (unlikely(val == ENA_MMIO_READ_TIMEOUT)) {
			ena_trc_err("Reg read timeout oclwrred\n");
...
int ena_com_get_lwrrent_hash_function(struct ena_com_dev *ena_dev)
{
...
static int ena_com_ind_tbl_colwert_to_device(struct ena_com_dev *ena_dev)
{
	struct ena_rss *rss = &ena_dev->rss;
	struct ena_com_io_sq *io_sq;
	u16 qid;
...
int ena_com_exelwte_admin_command(struct ena_com_admin_queue *admin_queue,
				  struct ena_admin_aq_entry *cmd,
				  size_t cmd_size,
				  struct ena_admin_acq_entry *comp,
				  size_t comp_size)
		      ...
		      enum ena_regs_reset_reason_types reset_reason)
{
	...
	if (unlikely((stat == ENA_MMIO_READ_TIMEOUT) ||
		...
		ena_trc_err("Reg read32 timeout oclwrred\n");
		return ENA_COM_TIMER_EXPIRED;
	}
...
int ena_com_indirect_table_set(struct ena_com_dev *ena_dev)
{
	...
	ret = ena_com_ind_tbl_colwert_to_device(ena_dev);
	if (ret) {
		ena_trc_err("Failed to colwert host indirection table to device table\n");
		return ret;
	}
