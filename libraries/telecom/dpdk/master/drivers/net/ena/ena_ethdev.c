...
static inline int validate_rx_req_id(struct ena_ring *rx_ring, uint16_t req_id)
{
	...
	rx_ring->adapter->reset_reason = ENA_REGS_RESET_ILW_RX_REQ_ID;
...
static int validate_tx_req_id(struct ena_ring *tx_ring, u16 req_id)
{
	...
	tx_ring->adapter->reset_reason = ENA_REGS_RESET_ILW_TX_REQ_ID;
		...
		uint16_t nb_pkts)
{
	...
	for (i = 0; i != nb_pkts; i++) {
		...
		if ((frag_field & RTE_IPV4_HDR_DF_FLAG) != 0) {
			 ...
			 * disabled, partial chcecksum should not be callwlated.
			 */
		 ...
		 * it will take care of necessary callwlations.
		 */
