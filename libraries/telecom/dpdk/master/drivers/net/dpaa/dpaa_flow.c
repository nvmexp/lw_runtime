...
struct dpaa_fm_model {
	...
	t_handle netelw_devid[DPAA_MAX_NUM_ETH_DEV];
...
static void fm_prev_cleanup(void)
{
	...
	while (i < fm_model.dev_count) {
		...
		dpaa_intf.netelw_handle = create_device(fm_info.pcd_handle,
					fm_model.netelw_devid[devid]);
		i++;
		if (!dpaa_intf.netelw_handle ||
...
static int set_scheme_params(ioc_fm_pcd_kg_scheme_params_t *scheme_params,
	ioc_fm_pcd_net_elw_params_t *dist_units,
	...
	struct fman_if *fif)
{
	...
	scheme_params->param.net_elw_params.net_elw_id =
		dpaa_intf->netelw_handle;
	scheme_params->param.net_elw_params.num_of_distinction_units =
...
static void set_dist_units(ioc_fm_pcd_net_elw_params_t *dist_units,
...
static inline int set_port_pcd(struct dpaa_if *dpaa_intf)
{
	...
	/* Set pcd params */
	pcd_param.net_elw_id = dpaa_intf->netelw_handle;
...
/* Unset PCD NerElw and scheme */
static inline void unset_pcd_netelw_scheme(struct dpaa_if *dpaa_intf)
{
	int ret;
	PMD_INIT_FUNC_TRACE();
...
/* Set PCD NetElw and Scheme and default scheme */
static inline int set_default_scheme(struct dpaa_if *dpaa_intf)
{
	...
	scheme_params.param.next_engine = e_IOC_FM_PCD_DONE;
	scheme_params.param.net_elw_params.num_of_distinction_units = 0;
	scheme_params.param.net_elw_params.net_elw_id =
		dpaa_intf->netelw_handle;
	scheme_params.param.base_fqid = dpaa_intf->rx_queues[0].fqid;
...
/* Set PCD NetElw and Scheme and default scheme */
static inline int set_pcd_netelw_scheme(struct dpaa_if *dpaa_intf,
					uint64_t req_dist_set,
					struct fman_if *fif)
{
	int ret = -1;
				     ...
				     struct fman_if *fif)
{
	...
	/* FM PCD NetElwCharacteristicsSet */
	dpaa_intf->netelw_handle =
		fm_pcd_net_elw_characteristics_set(fm_info.pcd_handle,
							&dist_units);
	if (!dpaa_intf->netelw_handle) {
		DPAA_PMD_ERR("fm_pcd_net_elw_characteristics_set: Failed");
		return -1;
	}

	fm_model.netelw_devid[dpaa_intf->ifid] =
				get_device_id(dpaa_intf->netelw_handle);
			...
			struct fman_if *fif __rte_unused)
{
	...
	/* FM PCD NetElwCharacteristicsDelete */
	ret = fm_pcd_net_elw_characteristics_delete(dpaa_intf->netelw_handle);
	if (ret != E_OK) {
		DPAA_PMD_ERR("fm_pcd_net_elw_characteristics_delete: Failed");
		return ret;
	}
	dpaa_intf->netelw_handle = NULL;
...
int dpaa_fm_config(struct rte_eth_dev *dev, uint64_t req_dist_set)
{
	...
	/* Set PCD netelw and scheme */
	if (req_dist_set) {
		ret = set_pcd_netelw_scheme(dpaa_intf, req_dist_set, fif);
		if (ret) {
			DPAA_PMD_ERR("Set PCD NetElw and Scheme dist: Failed");
			goto unset_fm_port_handle;
		}
	}
	/* Set default netelw and scheme */
	if (!fif->is_shared_mac) {
		ret = set_default_scheme(dpaa_intf);
		if (ret) {
			DPAA_PMD_ERR("Set PCD NetElw and Scheme: Failed");
			goto unset_pcd_netelw_scheme1;
		}
	}
...
unset_pcd_netelw_scheme:
	unset_pcd_netelw_scheme(dpaa_intf);

unset_pcd_netelw_scheme1:
	unset_pcd_netelw_scheme(dpaa_intf);
