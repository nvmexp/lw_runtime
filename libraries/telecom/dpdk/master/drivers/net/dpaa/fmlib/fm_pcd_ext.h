...
typedef enum ioc_fm_pcd_counters {
	...
	/**< Parser counter - counts the number of cycles spent exelwting soft
	 ...
	 * parser internal memory reads while exelwting soft parser instruction.
	 */
	e_IOC_FM_PCD_PRS_COUNTERS_HARD_PRS_CYCLE_INCL_STALL_CYCLES,
	/**< Parser counter - counts the number of cycles spent exelwting hard
 ...
 *		  elwironments. Later, when needed, the application will have to
...
typedef struct ioc_fm_pcd_prs_label_params_t {
	...
	ioc_net_header_type	hdr;
		/**< The existence of this header will ilwoke the SW parser
		 * code.
		 */
...
typedef struct ioc_fm_pcd_prs_sw_params_t {
	bool		override;
			/**< FALSE to ilwoke a check that nothing else was
...
typedef enum ioc_fm_pcd_engine {
	e_IOC_FM_PCD_ILWALID = 0,   /**< Invalid PCD engine */
...
typedef enum ioc_fm_pcd_extract_from {
	...
	e_IOC_FM_PCD_EXTRACT_FROM_LWRR_END_OF_PARSE,
 ...
 */
typedef enum ioc_fm_pcd_manip_hdr_lwstom_type {
	e_IOC_FM_PCD_MANIP_HDR_LWSTOM_IP_REPLACE,
			/**< Replace IPv4/IPv6 */
	e_IOC_FM_PCD_MANIP_HDR_LWSTOM_GEN_FIELD_REPLACE,
} ioc_fm_pcd_manip_hdr_lwstom_type;

/*
 * @Description   Enumeration type for selecting specific custom command
 */
typedef enum ioc_fm_pcd_manip_hdr_lwstom_ip_replace {
	e_IOC_FM_PCD_MANIP_HDR_LWSTOM_REPLACE_IPV4_BY_IPV6,
					/**< Replace IPv4 by IPv6 */
	e_IOC_FM_PCD_MANIP_HDR_LWSTOM_REPLACE_IPV6_BY_IPV4
					/**< Replace IPv6 by IPv4 */
} ioc_fm_pcd_manip_hdr_lwstom_ip_replace;
 ...
 *		  between flows.
 *		  (Must match struct t_FmPcdNetElwParams defined in
 *		  fm_pcd_ext.h)
 */
struct fm_pcd_net_elw_params_t {
...
typedef struct ioc_fm_pcd_net_elw_params_t {
	struct fm_pcd_net_elw_params_t param;
	void				*id;
		/**< Output parameter; Returns the net-elw Id to be used */
} ioc_fm_pcd_net_elw_params_t;
...
struct fm_pcd_kg_scheme_params_t {
	...
	struct {
		 ...
		 * by fm_pcd_net_elw_characteristics_set()
		 */
		uint8_t num_of_distinction_units;
		/**< Number of NetElw units listed in unit_ids array */
		uint8_t unit_ids[IOC_FM_PCD_MAX_NUM_OF_DISTINCTION_UNITS];
		/**< Indexes as passed to SetNetElwCharacteristics (?) array */
	} net_elw_params;
	bool use_hash;
 ...
 *		  The driver callwlates the maximal memory size that may be used
...
struct fm_pcd_hash_table_params_t {
	...
	uint16_t hash_res_mask;
		 ...
		 * number-of-sets for this hash will be callwlated as (2^(number
 ...
 *
 *		  This structure defines a CC group in terms of NetElw units and
...
typedef struct ioc_fm_pcd_cc_grp_params_t {
	...
	uint8_t		unit_ids[IOC_FM_PCD_MAX_NUM_OF_CC_UNITS];
		 ...
		 * fm_pcd_net_elw_characteristics_set()
		 */
	ioc_fm_pcd_cc_next_engine_params_t
...
typedef struct ioc_fm_pcd_manip_frag_ip_params_t {
	uint16_t			size_for_fragmentation;
		 ...
		 * fragmentation will be exelwted.
		 */
	bool			sg_bpid_en;
...
typedef struct ioc_fm_pcd_manip_reassem_ip_params_t {
	...
	uint32_t			non_consistent_sp_fqid;
			 ...
			 * of two possible scenarios oclwrs: if
...
typedef struct ioc_fm_pcd_manip_frag_capwap_params_t {
	uint16_t	size_for_fragmentation;
			 ...
			 * CAPWAP fragmentation will be exelwted.
			 */
	bool		sg_bpid_en;
 ...
 *		  replacement
 */
typedef struct ioc_fm_pcd_manip_hdr_lwstom_ip_hdr_replace_t {
	ioc_fm_pcd_manip_hdr_lwstom_ip_replace  replace_type;
	...
	uint8_t	hdr[IOC_FM_PCD_MANIP_MAX_HDR_SIZE];
			/**< The new IP header */
} ioc_fm_pcd_manip_hdr_lwstom_ip_hdr_replace_t;

/*
 * @Description   Parameters for defining custom header manipulation
 */
typedef struct ioc_fm_pcd_manip_hdr_lwstom_params_t {
	ioc_fm_pcd_manip_hdr_lwstom_type		type;
			/**< Type of header field update manipulation */
	union {
	ioc_fm_pcd_manip_hdr_lwstom_ip_hdr_replace_t	ip_hdr_replace;
			/**< Parameters IP header replacement */
	} u;
} ioc_fm_pcd_manip_hdr_lwstom_params_t;
...
typedef struct ioc_fm_pcd_manip_hdr_insrt_ip_params_t {
	...
	uint8_t	last_dst_offset;
			 ...
			 * to callwlate UDP checksum pseudo header; Otherwise
			 * set it to '0'.
			 */
...
typedef struct ioc_fm_pcd_manip_hdr_params_t {
	...
	ioc_fm_pcd_manip_hdr_lwstom_params_t	lwstom_params;
...
typedef struct ioc_fm_pcd_manip_reassem_ip_stats_t {
	...
	uint32_t	timeout;
		/**< Counts the number of TimeOut oclwrrences */
	...
	uint32_t	internal_buffer_busy;
		/**< Counts the number of times an internal buffer busy oclwrred
		 */
	uint32_t	external_buffer_busy;
		/**< Counts the number of times external buffer busy oclwrred */
...
struct {
	...
	uint32_t	auto_learn_busy;
		/**< Counts the number of times a busy condition oclwrs when
		 * attempting to access an IP-Reassembly Automatic Learning Hash
		 * set
		 */
	uint32_t	more_than16fragments;
		/**< Counts the fragment oclwrrences in which the number of
...
typedef struct ioc_fm_pcd_manip_reassem_capwap_stats_t {
	...
	uint32_t	malformed_fragments;
			 ...
			 */
	uint32_t	auto_learn_busy;
			/**< Counts the number of times a busy condition oclwrs
	...
	uint32_t	discarded_fragments;
			 ...
			 */
	uint32_t	more_than16fragments;
			/**< Counts the fragment oclwrrences in which the number
...
/*
 * @Function	  fm_pcd_net_elw_characteristics_set
 *
 ...
 *		  elwironments. A port using the PCD functionality is bound to
 ...
 *		  initialized elwironments must be stated and related to. When a
 ...
 *		  indexing.
 *
 * @Param[in,out] ioc_fm_pcd_net_elw_params_t	A structure defining the
 *						distinction units for this
 *						configuration.
 *
 * @Return	  0 on success; Error code otherwise.
 */
#define FM_PCD_IOC_NET_ELW_CHARACTERISTICS_SET \
		_IOWR(FM_IOC_TYPE_BASE, FM_PCD_IOC_NUM(20), \
		      ioc_fm_pcd_net_elw_params_t)

/*
 * @Function	  fm_pcd_net_elw_characteristics_delete
 *
 ...
 * @Return	  0 on success; Error code otherwise.
 */
#define FM_PCD_IOC_NET_ELW_CHARACTERISTICS_DELETE \
 ...
 *		  Number of hash table ways is then callwlated by dividing
...
typedef struct t_fm_pcd_hc_params {
	 ...
	 * NOTE: When configuring Host Command port for FMALw3 devices
 ...
 *		  Calling this routine ilwalidates the descriptor.
 *
 * @Param[in]	  h_fm_pcd	FM PCD module descriptor.
 *
 * @Return	  E_OK on success; Error code otherwise.
 ...
 *		  field replacement
 */
typedef struct ioc_fm_pcd_manip_hdr_lwstom_gen_field_replace {
	uint8_t		src_offset;
	...
	uint8_t		mask_offset;
			 ...
			 * replaces "size"
			 */
} ioc_fm_pcd_manip_hdr_lwstom_gen_field_replace;
 ...
 *
 * @Param[in]	  h_fm_pcd		FM PCD module descriptor.
 * @Param[in]	  p_netelw_params	A structure of parameters for the
 ...
 * @Cautions	  Allowed only following fm_pcd_init().
 */
t_handle fm_pcd_net_elw_characteristics_set(t_handle h_fm_pcd,
				 ioc_fm_pcd_net_elw_params_t *p_netelw_params);
 ...
 *
 * @Param[in]	  h_net_elw	A handle to the Network environment.
 *
 * @Return	  E_OK on success; Error code otherwise.
 */
uint32_t fm_pcd_net_elw_characteristics_delete(t_handle h_net_elw);
 ...
 *		  different from the lwrrently changed table.
 *
 */
 ...
 *		  and callwlates to which hash bucket it will be mapped.
 *
 * @Param[in]	  h_cc_node			A handle to the node.
