...
fm_get_api_version(t_handle h_fm, ioc_fm_api_version_t *p_version)
{
	...
	if (ret) {
		...
		RETURN_ERROR(MINOR, E_ILWALID_OPERATION, NO_MSG);
	}
	_fml_dbg("Finishing.\n");
...
t_handle
fm_pcd_net_elw_characteristics_set(t_handle h_fm_pcd,
		ioc_fm_pcd_net_elw_params_t *params)
{
	...
	if (ioctl(p_pcd_dev->fd, FM_PCD_IOC_NET_ELW_CHARACTERISTICS_SET,
		  params))
		return NULL;
...
uint32_t
fm_pcd_net_elw_characteristics_delete(t_handle h_net_elw)
{
	t_device *p_dev = (t_device *)h_net_elw;
	...
	if (ioctl(p_pcd_dev->fd, FM_PCD_IOC_NET_ELW_CHARACTERISTICS_DELETE,
		  &id))
		RETURN_ERROR(MINOR, E_ILWALID_OPERATION, NO_MSG);
		     ...
		     ioc_fm_pcd_kg_scheme_params_t *params)
{
	...
	/* correct h_net_elw param from scheme */
	if (params->param.net_elw_params.net_elw_id)
		DEV_TO_ID(params->param.net_elw_params.net_elw_id);
		...
		ioc_fm_port_pcd_params_t *p)
{
	...
	/* correct h_net_elw param from t_fm_portPcdParams */
	DEV_TO_ID(p->net_elw_id);
