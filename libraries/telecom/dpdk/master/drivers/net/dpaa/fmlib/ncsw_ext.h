...
typedef enum e_rx_store_response {
	e_RX_STORE_RESPONSE_PAUSE
		/**< Pause ilwoking callback with received data; in polling
		 * mode, start again ilwoking callback only next time user
		 * ilwokes the receive routine; in interrupt mode, start again
		 * ilwoking callback only next time a receive event triggers an
	...
	, e_RX_STORE_RESPONSE_CONTINUE
		/**< Continue ilwoking callback with received data. */
} e_rx_store_response;
