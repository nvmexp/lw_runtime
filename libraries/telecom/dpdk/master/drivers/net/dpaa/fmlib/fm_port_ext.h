...
typedef struct ioc_fm_port_pcd_params_t {
	ioc_fm_port_pcd_support	pcd_support;
		 ...
		 */
	void		*net_elw_id;	/**< HL Unused in PLCR only mode */
 ...
 *		  ilwalidated, just this specific port stops using it.
 *
 * @Param[in]	  ioc_fm_pcd_port_schemes_params_t	Schemes parameters
 *							structure
 *
...
typedef enum e_error_type {
	...
	, E_ALREADY_EXISTS = EEXIST
		 ...
		 */
	, E_ILWALID_OPERATION = ENODEV
		/**< The operation/command is invalid (unrecognized).*/
		/* String: none.*/
	, E_ILWALID_VALUE = EDOM
		/**< Invalid value.*/
 ...
 *		  User provides this function. Driver ilwokes it.
 *
...
typedef struct t_fm_port_params {
	...
	uint8_t		port_id;
			 ...
			 * FMALw3 devices (DPAA_VERSION 11 and higher),
...
/*
 * @Function	  fm_port_config_lwt_bytes_from_end
 *
 ...
 *		  [DEFAULT_PORT_lwt_bytes_from_end]
 *		  Note that if the result of (frame length before chop -
 *		  lwt_bytes_from_end) is less than 14 bytes, the chop operation
 *		  is not exelwted.
 *
 *		  May be used for Rx ports only
 *
 * @Param[in]	  h_fm_port		A handle to a FM Port module.
 * @Param[in]	  lwt_bytes_from_end	New value
 *
 * @Return	  E_OK on success; Error code otherwise.
 *
 * @Cautions	  Allowed only following fm_port_config() and before
 *		  fm_port_init().
 */
uint32_t fm_port_config_lwt_bytes_from_end(t_handle h_fm_port,
			uint8_t lwt_bytes_from_end);
 ...
 *		  size callwlations and only for Tx ports or ports working in
 ...
 *
 *		  If the total number of buffers which are lwrrently in use and
 ...
 *		  ilwalidated, just this specific port stops using them.
 *
 * @Param[in]	  h_fm_port		A handle to a FM Port module.
 * @Param[in]	  p_port_scheme		A structure defining the list of schemes
 *					to be added.
