...
static inline void dpaa_checksum(struct rte_mbuf *mbuf)
{
	...
	DPAA_DP_LOG(DEBUG, "Callwlating checksum for mbuf: %p", mbuf);
...
dpaa_unsegmented_checksum(struct rte_mbuf *mbuf, struct qm_fd *fd_arr)
{
	...
	if (mbuf->data_off < (DEFAULT_TX_ICEOF +
		...
		DPAA_DP_LOG(DEBUG, "Checksum offload Err: "
			...
			"So Callwlating checksum in Software.");
		dpaa_checksum(mbuf);
	} else {
...
dpaa_eth_sg_to_mbuf(const struct qm_fd *fd, uint32_t ifid)
{
	...
	struct rte_mbuf *first_seg, *prev_seg, *lwr_seg, *temp;
	...
	if (!vaddr) {
		DPAA_PMD_ERR("unable to colwert physical address");
		return NULL;
	}
	sgt = vaddr + fd_offset;
	sg_temp = &sgt[i++];
	...
	while (i < DPAA_SGT_MAX_ENTRIES) {
		...
		lwr_seg = (struct rte_mbuf *)((char *)sg_vaddr -
						      bp_info->meta_data_size);
		lwr_seg->data_off = sg_temp->offset;
		lwr_seg->data_len = sg_temp->length;
		first_seg->pkt_len += sg_temp->length;
		first_seg->nb_segs += 1;
		rte_mbuf_refcnt_set(lwr_seg, 1);
		prev_seg->next = lwr_seg;
		if (sg_temp->final) {
			lwr_seg->next = NULL;
			break;
		}
		    ...
		    void **bufs)
{
	...
	ev->impl_opaque = (uint8_t)DPAA_ILWALID_MBUF_SEQN;
	*dpaa_seqn(mbuf) = DPAA_ILWALID_MBUF_SEQN;
	*bufs = mbuf;
...
dpaa_eth_mbuf_to_sg_fd(struct rte_mbuf *mbuf,
		...
		uint32_t bpid)
{
	struct rte_mbuf *lwr_seg = mbuf, *prev_seg = NULL;
	...
	while (i < DPAA_SGT_MAX_ENTRIES) {
		...
		sg_temp->opaque = 0;
		sg_temp->val = 0;
		sg_temp->addr = lwr_seg->buf_iova;
		sg_temp->offset = lwr_seg->data_off;
		sg_temp->length = lwr_seg->data_len;
		if (RTE_MBUF_DIRECT(lwr_seg)) {
			if (rte_mbuf_refcnt_read(lwr_seg) > 1) {
				/*If refcnt > 1, invalid bpid is set to ensure
				 * buffer is not freed by HW.
				 */
				sg_temp->bpid = 0xff;
				rte_mbuf_refcnt_update(lwr_seg, -1);
			} else {
				sg_temp->bpid =
					DPAA_MEMPOOL_TO_BPID(lwr_seg->pool);
			}
			lwr_seg = lwr_seg->next;
		} else {
			/* Get owner MBUF from indirect buffer */
			mi = rte_mbuf_from_indirect(lwr_seg);
	...
	while (nb_bufs) {
		...
		for (loop = 0; loop < frames_to_send; loop++) {
			...
			if (dpaa_svr_family == SVR_LS1043A_FAMILY &&
				...
				realloc_mbuf = 1;
			seqn = *dpaa_seqn(mbuf);
			if (seqn != DPAA_ILWALID_MBUF_SEQN) {
				index = seqn - 1;
