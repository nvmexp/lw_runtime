...
typedef struct fmc_port_t {
	e_fm_port_type type;
	unsigned int number;
	struct fm_pcd_net_elw_params_t distinction_units;
	...
	t_handle handle;
	t_handle elw_id_handle;
	t_handle elw_id_dev_id;
...
static int dpaa_port_fmc_port_parse(struct fman_if *fif,
				    ...
				    int apply_idx)
{
	int lwrrent_port = fmc_model->apply_order[apply_idx].index;
	const fmc_port *pport = &fmc_model->port[lwrrent_port];
		       ...
		       uint32_t *fqids, int8_t *vspids, int max_nb_rxq)
{
	...
	for (i = 0; i < fmc_model->apply_order_count; i++) {
		...
		case fmcengine_end:
			break;
		case fmcport_start:
			lwrrent_port = dpaa_port_fmc_port_parse(fif,
								fmc_model, i);
			break;
		case fmcport_end:
			break;
