...
static int default_q;	/* use default queue - FMC is not exelwted*/
...
static int dpaa_eth_promislwous_enable(struct rte_eth_dev *dev)
{
	PMD_INIT_FUNC_TRACE();

	fman_if_promislwous_enable(dev->process_private);

	return 0;
}

static int dpaa_eth_promislwous_disable(struct rte_eth_dev *dev)
{
	PMD_INIT_FUNC_TRACE();

	fman_if_promislwous_disable(dev->process_private);
...
static struct eth_dev_ops dpaa_devops = {
	...
	.promislwous_enable	  = dpaa_eth_promislwous_enable,
	.promislwous_disable	  = dpaa_eth_promislwous_disable,
...
dpaa_dev_init(struct rte_eth_dev *eth_dev)
{
	 ...
	 */
	if (getelw("DPAA_TX_TAILDROP_THRESHOLD")) {
		td_tx_threshold = atoi(getelw("DPAA_TX_TAILDROP_THRESHOLD"));
		DPAA_PMD_DEBUG("Tail drop threshold elw configured: %u",
			       td_tx_threshold);
	...
	if (!fman_intf->is_shared_mac) {
		...
		/* Disable promislwous mode */
		fman_if_promislwous_disable(fman_intf);
		/* Disable multicast */
	       ...
	       struct rte_dpaa_device *dpaa_dev)
{
	...
	if (!is_global_init && (rte_eal_process_type() == RTE_PROC_PRIMARY)) {
		...
		/* if push mode queues to be enabled. Lwrrenly we are allowing
		 * only one queue per thread.
		 */
		if (getelw("DPAA_PUSH_QUEUES_NUMBER")) {
			dpaa_push_mode_max_queue =
					atoi(getelw("DPAA_PUSH_QUEUES_NUMBER"));
