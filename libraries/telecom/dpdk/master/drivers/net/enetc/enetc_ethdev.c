...
enetc_hardware_init(struct enetc_eth_hw *hw)
{
	...
	PMD_INIT_FUNC_TRACE();
	/* Callwlating and storing the base HW addresses */
...
static int
enetc_promislwous_enable(struct rte_eth_dev *dev)
{
	struct enetc_eth_hw *hw =
		ENETC_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct enetc_hw *enetc_hw = &hw->hw;
	...
	/* Setting to enable promislwous mode*/
...
static int
enetc_promislwous_disable(struct rte_eth_dev *dev)
{
	struct enetc_eth_hw *hw =
		ENETC_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct enetc_hw *enetc_hw = &hw->hw;
	...
	/* Setting to disable promislwous mode for SI0*/
	psipmr = enetc_port_rd(enetc_hw, ENETC_PSIPMR);
...
static const struct eth_dev_ops enetc_ops = {
	...
	.promislwous_enable   = enetc_promislwous_enable,
	.promislwous_disable  = enetc_promislwous_disable,
