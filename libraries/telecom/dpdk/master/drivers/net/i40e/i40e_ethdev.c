...
/* UDP Tunneling ID, LWGRE/GRE key */
...
static int i40e_dev_promislwous_enable(struct rte_eth_dev *dev);
static int i40e_dev_promislwous_disable(struct rte_eth_dev *dev);
...
static int i40e_ethertype_filter_colwert(
	const struct rte_eth_ethertype_filter *input,
	struct i40e_ethertype_filter *filter);
static int i40e_sw_ethertype_filter_insert(struct i40e_pf *pf,
				   struct i40e_ethertype_filter *filter);

static int i40e_tunnel_filter_colwert(
...
static const struct eth_dev_ops i40e_eth_dev_ops = {
	...
	.promislwous_enable           = i40e_dev_promislwous_enable,
	.promislwous_disable          = i40e_dev_promislwous_disable,
...
static void
i40e_init_lwstomized_info(struct i40e_pf *pf)
{
	int i;

	/* Initialize lwstomized pctype */
	for (i = I40E_LWSTOMIZED_GTPC; i < I40E_LWSTOMIZED_MAX; i++) {
		pf->lwstomized_pctype[i].index = i;
		pf->lwstomized_pctype[i].pctype = I40E_FILTER_PCTYPE_ILWALID;
		pf->lwstomized_pctype[i].valid = false;
	}
...
static void
i40e_init_filter_ilwalidation(struct i40e_pf *pf)
{
	struct i40e_hw *hw = I40E_PF_TO_HW(pf);
	struct i40e_fdir_info *fdir_info = &pf->fdir;
	...
	if (!pf->support_multi_driver) {
		fdir_info->fdir_ilwalprio = 1;
		glqf_ctl_reg |= I40E_GLQF_CTL_ILWALPRIO_MASK;
		PMD_DRV_LOG(INFO, "FDIR ILWALPRIO set to guaranteed first");
		i40e_write_rx_ctl(hw, I40E_GLQF_CTL, glqf_ctl_reg);
	} else {
		if (glqf_ctl_reg & I40E_GLQF_CTL_ILWALPRIO_MASK) {
			fdir_info->fdir_ilwalprio = 1;
			PMD_DRV_LOG(INFO, "FDIR ILWALPRIO is: guaranteed first");
		} else {
			fdir_info->fdir_ilwalprio = 0;
			PMD_DRV_LOG(INFO, "FDIR ILWALPRIO is: shared first");
		}
	}
}
...
eth_i40e_dev_init(struct rte_eth_dev *dev, void *init_params __rte_unused)
{
	...
	PMD_INIT_LOG(INFO, "FW %d.%d API %d.%d LWM %02d.%02d.%02d eetrack %04x",
		     hw->aq.fw_maj_ver, hw->aq.fw_min_ver,
		     hw->aq.api_maj_ver, hw->aq.api_min_ver,
		     ((hw->lwm.version >> 12) & 0xf),
		     ((hw->lwm.version >> 4) & 0xff),
		     (hw->lwm.version & 0xf), hw->lwm.eetrack);
	...
	/*
	 * To work around the LWM issue, initialize registers
	 * for packet type of QinQ by software.
	 * It should be removed once issues are fixed in LWM.
	 */
	if (!pf->support_multi_driver)
	...
	/* Initialize lwstomized information */
	i40e_init_lwstomized_info(pf);

	/* Initialize the filter ilwalidation configuration */
	i40e_init_filter_ilwalidation(pf);
...
i40e_dev_configure(struct rte_eth_dev *dev)
{
	 ...
	 *  For RSS setting, it will try to callwlate actual configured RX queue
		   ...
		   bool is_up)
{
	...
	phy_conf.fec_config = phy_ab.fec_cfg_lwrr_mod_ext_info;
	phy_conf.eee_capability = phy_ab.eee_capability;
	phy_conf.eeer = phy_ab.eeer_val;
	phy_conf.low_power_ctrl = phy_ab.d3_lpan;

	PMD_DRV_LOG(DEBUG, "\tLwrrent: abilities %x, link_speed %x",
...
i40e_dev_start(struct rte_eth_dev *dev)
{
	...
	/* Enable the VLAN promislwous mode. */
	if (pf->vfs) {
...
i40e_dev_reset(struct rte_eth_dev *dev)
{
	 ...
	 * To avoid unexpected behavior in VF, lwrrently reset of PF with
...
static int
i40e_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
	struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	...
	status = i40e_aq_set_vsi_unicast_promislwous(hw, vsi->seid,
						     true, NULL, true);
	if (status != I40E_SUCCESS) {
		PMD_DRV_LOG(ERR, "Failed to enable unicast promislwous");
		return -EAGAIN;
	}

	status = i40e_aq_set_vsi_multicast_promislwous(hw, vsi->seid,
							TRUE, NULL);
	if (status != I40E_SUCCESS) {
		PMD_DRV_LOG(ERR, "Failed to enable multicast promislwous");
		/* Rollback unicast promislwous mode */
		i40e_aq_set_vsi_unicast_promislwous(hw, vsi->seid,
						    false, NULL, true);
		return -EAGAIN;
	}
...
static int
i40e_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct i40e_pf *pf = I40E_DEV_PRIVATE_TO_PF(dev->data->dev_private);
	struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct i40e_vsi *vsi = pf->main_vsi;
	...
	status = i40e_aq_set_vsi_unicast_promislwous(hw, vsi->seid,
						     false, NULL, true);
	if (status != I40E_SUCCESS) {
		PMD_DRV_LOG(ERR, "Failed to disable unicast promislwous");
		return -EAGAIN;
	}
	...
	status = i40e_aq_set_vsi_multicast_promislwous(hw, vsi->seid,
							false, NULL);
	if (status != I40E_SUCCESS) {
		PMD_DRV_LOG(ERR, "Failed to disable multicast promislwous");
		/* Rollback unicast promislwous mode */
		i40e_aq_set_vsi_unicast_promislwous(hw, vsi->seid,
						    true, NULL, true);
		return -EAGAIN;
...
i40e_dev_allmulticast_enable(struct rte_eth_dev *dev)
{
	...
	ret = i40e_aq_set_vsi_multicast_promislwous(hw, vsi->seid, TRUE, NULL);
	if (ret != I40E_SUCCESS) {
		PMD_DRV_LOG(ERR, "Failed to enable multicast promislwous");
		return -EAGAIN;
	}
...
i40e_dev_allmulticast_disable(struct rte_eth_dev *dev)
{
	...
	if (dev->data->promislwous == 1)
		return 0; /* must remain in all_multicast mode */

	ret = i40e_aq_set_vsi_multicast_promislwous(hw,
				vsi->seid, FALSE, NULL);
	if (ret != I40E_SUCCESS) {
		PMD_DRV_LOG(ERR, "Failed to disable multicast promislwous");
		return -EAGAIN;
...
/*
 * When using LWM 6.01(for X710 XL710 XXV710)/3.33(for X722) or later,
...
i40e_need_stop_lldp(struct rte_eth_dev *dev)
{
	...
	lwm_ver = atof(ver_str);
	if ((hw->mac.type == I40E_MAC_X722 ||
	     hw->mac.type == I40E_MAC_X722_VF) &&
	     ((uint32_t)(lwm_ver * 1000) >= (uint32_t)(3.33 * 1000)))
		return true;
	else if ((uint32_t)(lwm_ver * 1000) >= (uint32_t)(6.01 * 1000))
		return true;
		   ...
		   uint16_t tpid)
{
	...
	/* 802.1ad frames ability is added in LWM API 1.7*/
	if (hw->flags & I40E_HW_FLAG_802_1AD_CAPABLE) {
		...
		/* If LWM API < 1.7, keep the register setting */
...
i40e_get_cap(struct i40e_hw *hw)
{
	...
	/* Callwlate a huge enough buff for saving response data temporarily */
	       ...
	       uint16_t user_param)
{
	...
	if (type == I40E_VSI_MAIN) {
		 ...
		 * For LWM 4.2.2 or after, the updating is not needed anymore.
	...
	for (i = 0; i < pf->vf_num; i++) {
		...
		if (val & (0x1 << offset)) {
			...
			I40E_WRITE_REG(hw, I40E_GLGEN_VFLRSTAT(index),
							(0x1 << offset));
			PMD_DRV_LOG(INFO, "VF %u reset oclwrred", abs_vf_id);
			/**
			 * Only notify a VF reset event oclwrred,
...
/* Colwert tunnel filter structure */
static int
i40e_tunnel_filter_colwert(
	struct i40e_aqc_cloud_filters_element_bb *cld_filter,
	struct i40e_tunnel_filter *tunnel_filter)
{
		      ...
		      uint8_t add)
{
	...
	/* internal variable to colwert ipv6 byte order */
	uint32_t colwert_ipv6[4];
	int val, ret = 0;
	...
	case I40E_TUNNEL_TYPE_VXLAN:
		...
		break;
	case I40E_TUNNEL_TYPE_LWGRE:
		tun_type = I40E_AQC_ADD_CLOUD_TNL_TYPE_LWGRE_OMAC;
		break;
	...
	i40e_tunnel_filter_colwert(cld_filter, &check_filter);
...
/* Callwlate the maximum number of contiguous PF queues that are configured */
static int
...
i40e_pf_config_rss(struct i40e_pf *pf)
{
	 ...
	 * It's necessary to callwlate the actual PF queues that are configured.
	 */
		     ...
		     uint16_t size)
{
	...
	for (i = 0, *inset = 0; i < size; i++) {
		for (j = 0; j < RTE_DIM(inset_colwert_table); j++) {
			if (field[i] == inset_colwert_table[j].field) {
				*inset |= inset_colwert_table[j].inset;
				break;
			}
		}

		/* It contains unsupported input set, return immediately */
		if (j == RTE_DIM(inset_colwert_table))
			return ret;
	}
...
/* Colwert ethertype filter structure */
static int
i40e_ethertype_filter_colwert(const struct rte_eth_ethertype_filter *input,
			      struct i40e_ethertype_filter *filter)
{
			...
			bool add)
{
	...
	/* Check if there is the filter in SW list */
	memset(&check_filter, 0, sizeof(check_filter));
	i40e_ethertype_filter_colwert(filter, &check_filter);
 ...
 * recent firmware versions, if promislwous mode is disabled, or promislwous
...
i40e_vsi_config_tc(struct i40e_vsi *vsi, uint8_t tc_map)
{
	...
	/* Update Queue Pairs Mapping for lwrrently enabled UPs */
	ctxt.seid = vsi->seid;
	ctxt.pf_num = hw->pf_id;
	ctxt.vf_num = 0;
	ctxt.uplink_seid = vsi->uplink_seid;
...
static int i40e_get_eeprom_length(struct rte_eth_dev *dev)
{
	...
	/* Colwert word count to byte count */
	return hw->lwm.sr_size << 1;
}
			   ...
			   struct rte_dev_eeprom_info *eeprom)
{
	...
	ret_code = i40e_read_lwm_buffer(hw, offset, &cnt_words, data);
...
struct i40e_lwstomized_pctype*
i40e_find_lwstomized_pctype(struct i40e_pf *pf, uint8_t index)
{
	int i;

	for (i = 0; i < I40E_LWSTOMIZED_MAX; i++) {
		if (pf->lwstomized_pctype[i].index == index)
			return &pf->lwstomized_pctype[i];
	}
	return NULL;
}

static int
i40e_update_lwstomized_pctype(struct rte_eth_dev *dev, uint8_t *pkg,
			      ...
			      enum rte_pmd_i40e_package_op op)
{
	...
	uint32_t buff_size;
	struct i40e_lwstomized_pctype *new_pctype = NULL;
	uint8_t proto_id;
	...
	for (i = 0; i < pctype_num; i++) {
		...
		if (!strcmp(name, "GTPC"))
			new_pctype =
				i40e_find_lwstomized_pctype(pf,
			...
			new_pctype =
				i40e_find_lwstomized_pctype(pf,
						I40E_LWSTOMIZED_AH_IPV4);
		else if (!strcmp(name, "IPV6_AH"))
			new_pctype =
				i40e_find_lwstomized_pctype(pf,
						I40E_LWSTOMIZED_AH_IPV6);
		if (new_pctype) {
			if (op == RTE_PMD_I40E_PKG_OP_WR_ADD) {
				new_pctype->pctype = pctype_value;
				new_pctype->valid = true;
			} else {
				new_pctype->pctype = I40E_FILTER_PCTYPE_ILWALID;
...
static int
i40e_update_lwstomized_ptype(struct rte_eth_dev *dev, uint8_t *pkg,
			     uint32_t pkg_size, uint32_t proto_num,
			     struct rte_pmd_i40e_proto_info *proto,
			     enum rte_pmd_i40e_package_op op)
{
...
void
i40e_update_lwstomized_info(struct rte_eth_dev *dev, uint8_t *pkg,
			    uint32_t pkg_size, enum rte_pmd_i40e_package_op op)
{
	...
	/* Update lwstomized pctype info */
	ret = i40e_update_lwstomized_pctype(dev, pkg, pkg_size,
					    proto_num, proto, op);
	if (ret)
		PMD_DRV_LOG(INFO, "No pctype is updated.");

	/* Update lwstomized ptype info */
	ret = i40e_update_lwstomized_ptype(dev, pkg, pkg_size,
					   proto_num, proto, op);
	if (ret)
...
static void
i40e_rss_mark_ilwalid_rule(struct i40e_pf *pf,
		...
		struct i40e_rte_flow_rss_conf *conf)
{
	...
	if (conf->conf.func == RTE_ETH_HASH_FUNCTION_SIMPLE_XOR) {
		...
		if (!(reg & I40E_GLQF_CTL_HTOEP_MASK)) {
			...
			I40E_WRITE_FLUSH(hw);
			i40e_rss_mark_ilwalid_rule(pf, conf);
		...
		for (j = I40E_FILTER_PCTYPE_ILWALID + 1;
