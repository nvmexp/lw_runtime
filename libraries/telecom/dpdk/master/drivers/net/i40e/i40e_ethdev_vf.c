...
static int i40evf_dev_promislwous_enable(struct rte_eth_dev *dev);
static int i40evf_dev_promislwous_disable(struct rte_eth_dev *dev);
...
static const struct eth_dev_ops i40evf_eth_dev_ops = {
	...
	.promislwous_enable   = i40evf_dev_promislwous_enable,
	.promislwous_disable  = i40evf_dev_promislwous_disable,
...
/*
 * Check there is pending cmd in exelwtion. If none, set new command.
 */
static inline int
...
static int
_i40evf_exelwte_vf_cmd(struct rte_eth_dev *dev, struct vf_cmd_info *args)
{
	struct i40e_hw *hw = I40E_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
...
static int
i40evf_exelwte_vf_cmd(struct rte_eth_dev *dev, struct vf_cmd_info *args)
{
	struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
	int err;
	...
	err = _i40evf_exelwte_vf_cmd(dev, args);
...
i40evf_check_api_version(struct rte_eth_dev *dev)
{
	...
	err = i40evf_exelwte_vf_cmd(dev, &args);
	if (err) {
...
i40evf_enable_vlan_strip(struct rte_eth_dev *dev)
{
	...
	args.out_buffer = vf->aq_resp;
	args.out_size = I40E_AQ_BUF_SZ;
	ret = i40evf_exelwte_vf_cmd(dev, &args);
	if (ret)
...
static int
i40evf_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
...
static int
i40evf_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct i40e_vf *vf = I40EVF_DEV_PRIVATE_TO_VF(dev->data->dev_private);
