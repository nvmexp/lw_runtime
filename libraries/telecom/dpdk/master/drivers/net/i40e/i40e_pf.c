					...
					bool b_op)
{
	...
	if (promisc->flags & FLAG_VF_UNICAST_PROMISC)
		unicast = TRUE;
	ret = i40e_aq_set_vsi_unicast_promislwous(hw,
	...
	if (promisc->flags & FLAG_VF_MULTICAST_PROMISC)
		multicast = TRUE;
	ret = i40e_aq_set_vsi_multicast_promislwous(hw, vf->vsi->seid,
						multicast, NULL);

send_msg:
	i40e_pf_host_send_msg_to_vf(vf,
		VIRTCHNL_OP_CONFIG_PROMISLWOUS_MODE, ret, NULL, 0);
...
i40e_notify_vf_link_status(struct rte_eth_dev *dev, struct i40e_pf_vf *vf)
{
	...
	/* need to colwert the ETH_SPEED_xxx into VIRTCHNL_LINK_SPEED_xxx */
...
i40e_pf_host_process_cmd_request_queues(struct i40e_pf_vf *vf, uint8_t *msg)
{
	...
	uint32_t lwr_pairs = vf->vsi->nb_used_qps;
	...
	} else if (req_pairs > lwr_pairs + pf->qp_pool.num_free) {
		PMD_DRV_LOG(ERR, "VF %d requested %d queues (rounded to %d) "
			"but only %d available\n",
			vf->vf_idx,
			vfres->num_queue_pairs,
			req_pairs,
			lwr_pairs + pf->qp_pool.num_free);
			   ...
			   uint16_t msglen)
{
	...
	uint64_t first_cycle, lwr_cycle;
	bool b_op = TRUE;
	int ret;
	...
	lwr_cycle = rte_get_timer_cycles();

	/* if the VF being blocked, ignore the message and return */
	if (lwr_cycle < vf->ignore_end_cycle)
		return;
	...
	case VIRTCHNL_OP_CONFIG_PROMISLWOUS_MODE:
		PMD_DRV_LOG(INFO, "OP_CONFIG_PROMISLWOUS_MODE received");
...
check:
	...
	/* store current cycle */
	vf->msg_timestamps[vf->msg_index++] = lwr_cycle;
	 ...
	 */
	if (first_cycle && lwr_cycle < first_cycle +
		...
		PMD_DRV_LOG(WARNING, "VF %u too much messages(%u in %u"
				...
				(uint32_t)((lwr_cycle - first_cycle +
...
i40e_pf_host_init(struct rte_eth_dev *dev)
{
	...
	/* callwlate the memory size for storing timestamp of messages */
