...
STATIC void i40e_set_hw_flags(struct i40e_hw *hw)
{
	...
	/* Newer versions of firmware require lock when reading the LWM */
	if (aq->api_maj_ver > 1 ||
	    (aq->api_maj_ver == 1 &&
	     aq->api_min_ver >= 5))
		hw->flags |= I40E_HW_FLAG_LWM_READ_REQUIRES_LOCK;
...
enum i40e_status_code i40e_init_adminq(struct i40e_hw *hw)
{
	...
	/* get the LWM version info */
	i40e_read_lwm_word(hw, I40E_SR_LWM_DEV_STARTER_VERSION,
			   &hw->lwm.version);
	i40e_read_lwm_word(hw, I40E_SR_LWM_EETRACK_LO, &eetrack_lo);
	i40e_read_lwm_word(hw, I40E_SR_LWM_EETRACK_HI, &eetrack_hi);
	hw->lwm.eetrack = (eetrack_hi << 16) | eetrack_lo;
	i40e_read_lwm_word(hw, I40E_SR_BOOT_CONFIG_PTR, &cfg_ptr);
	i40e_read_lwm_word(hw, (cfg_ptr + I40E_LWM_OEM_VER_OFF),
			   &oem_hi);
	i40e_read_lwm_word(hw, (cfg_ptr + (I40E_LWM_OEM_VER_OFF + 1)),
			   &oem_lo);
	hw->lwm.oem_ver = ((u32)oem_hi << 16) | oem_lo;
	...
	i40e_aq_release_resource(hw, I40E_LWM_RESOURCE_ID, 0, NULL);
...
enum i40e_status_code i40e_shutdown_adminq(struct i40e_hw *hw)
{
	...
	if (hw->lwm_buff.va)
		i40e_free_virt_mem(hw, &hw->lwm_buff);
...
#ifdef PF_DRIVER
	i40e_lwmupd_check_wait_event(hw, LE16_TO_CPU(e->desc.opcode), &e->desc);
#endif /* PF_DRIVER */
