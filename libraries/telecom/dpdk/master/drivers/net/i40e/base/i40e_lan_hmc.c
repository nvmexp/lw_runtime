...
/**
 * i40e_callwlate_l2fpm_size - callwlates layer 2 FPM memory size
 ...
 *
 * Callwlates the maximum amount of memory for the function required, based
 * on the number of resources it must provide context for.
 **/
u64 i40e_callwlate_l2fpm_size(u32 txq_num, u32 rxq_num,
					...
					u32 fcoe_filt_num)
{
	...
	if (txq_num > obj->max_cnt) {
		ret_code = I40E_ERR_ILWALID_HMC_OBJ_COUNT;
	...
	l2fpm_size = i40e_callwlate_l2fpm_size(txq_num, rxq_num, fcoe_cntx_num,
					       fcoe_filt_num);
 ...
 *	2. write to register PMPDILW to ilwalidate the backing page in FV cache
 ...
 *	   to 0) and PMSDDATAHIGH to ilwalidate the sd page
				...
				struct i40e_hmc_lan_create_obj_info *info)
{
	...
	if (info->start_idx >= info->hmc_info->hmc_obj[info->rsrc_type].cnt) {
		ret_code = I40E_ERR_ILWALID_HMC_OBJ_INDEX;
	...
	if (sd_idx >= info->hmc_info->sd_table.sd_cnt ||
	    sd_lmt > info->hmc_info->sd_table.sd_cnt) {
			ret_code = I40E_ERR_ILWALID_SD_INDEX;
			goto exit;
	}
	/* find pd index */
	...
	for (j = sd_idx; j < sd_lmt; j++) {
		...
		if (!sd_entry->valid) {
			...
			case I40E_SD_TYPE_DIRECT:
				...
				break;
			default:
				ret_code = I40E_ERR_ILWALID_SD_TYPE;
				goto exit;
			}
		}
	}
