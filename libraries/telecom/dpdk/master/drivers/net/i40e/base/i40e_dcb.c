				   ...
				   struct i40e_dcbx_config *dcbcfg)
{
	...
	for (i = 0; i < dcbcfg->numapps; i++) {
		...
		/* Get Selector from lower 2 bits, and colwert to IEEE */
 ...
 * @dcbcfg: Local store to update ETS REC data
 *
 * Lwrrently only IEEE 802.1Qaz TLV is supported, all others
 * will be returned
 **/
 ...
 *
 * Colwert CEE v1 configuration from firmware to DCB configuration
 **/
 ...
 * @dcbcfg: DCB configuration struct
 *
 * Colwert CEE configuration from firmware to DCB configuration
 **/
...
enum i40e_status_code i40e_init_dcb(struct i40e_hw *hw, bool enable_mib_change)
{
	...
	if (hw->flags & I40E_HW_FLAG_FW_LLDP_PERSISTENT) {
		...
		if (hw->mac.type == I40E_MAC_XL710)
			offset = I40E_LLDP_LWRRENT_STATUS_XL710_OFFSET;
		else if (hw->mac.type == I40E_MAC_X722)
			offset = I40E_LLDP_LWRRENT_STATUS_X722_OFFSET;
		else
			return I40E_NOT_SUPPORTED;

		ret = i40e_read_lwm_module_data(hw,
						I40E_SR_EMP_SR_SETTINGS_PTR,
						offset,
						I40E_LLDP_LWRRENT_STATUS_OFFSET,
						I40E_LLDP_LWRRENT_STATUS_SIZE,
...
/**
 * i40e_dcb_config_to_lldp - Colwert Dcbconfig to MIB format
...
/**
 * _i40e_read_lldp_cfg - generic read of LLDP Configuration data from LWM
 * @hw: pointer to the HW structure
 * @lldp_cfg: pointer to hold lldp configuration variables
 * @module: address of the module pointer
 * @word_offset: offset of LLDP configuration
 *
 * Reads the LLDP configuration data from LWM using passed addresses
 **/
					  ...
					  u8 module, u32 word_offset)
{
	...
	ret = i40e_acquire_lwm(hw, I40E_RESOURCE_READ);
	if (ret != I40E_SUCCESS)
		return ret;

	ret = i40e_aq_read_lwm(hw, 0x0, module * 2, sizeof(raw_mem), &raw_mem,
			       true, NULL);
	i40e_release_lwm(hw);
	if (ret != I40E_SUCCESS)
		return ret;
	...
	ret = i40e_aq_read_lwm(hw, module, offset, sizeof(raw_mem), &raw_mem,
			       true, NULL);
	i40e_release_lwm(hw);
	if (ret != I40E_SUCCESS)
		return ret;
	...
	ret = i40e_aq_read_lwm(hw, 0, address + offset,
...
/**
 * i40e_read_lldp_cfg - read LLDP Configuration data from LWM
 * @hw: pointer to the HW structure
 * @lldp_cfg: pointer to hold lldp configuration variables
 *
 * Reads the LLDP configuration data from LWM
 **/
					 ...
					 struct i40e_lldp_variables *lldp_cfg)
{
	...
	ret = i40e_aq_read_lwm(hw, I40E_SR_LWM_CONTROL_WORD, 0, sizeof(mem),
			       &mem, true, NULL);
	i40e_release_lwm(hw);
	if (ret != I40E_SUCCESS)
		return ret;
	 ...
	 * structured LWM image. Flat image has LLDP configuration in shadow
	 * ram, so there is a need to pass different addresses for both cases.
	 */
	if (mem & I40E_SR_LWM_MAP_STRUCTURE_TYPE) {
		/* Flat LWM case */
		ret = _i40e_read_lldp_cfg(hw, lldp_cfg, I40E_SR_EMP_MODULE_PTR,
					  I40E_SR_LLDP_CFG_PTR);
	} else {
		/* Good old structured LWM image */
		ret = _i40e_read_lldp_cfg(hw, lldp_cfg, I40E_EMP_MODULE_PTR,
					  I40E_LWM_LLDP_CFG_PTR);
	}
