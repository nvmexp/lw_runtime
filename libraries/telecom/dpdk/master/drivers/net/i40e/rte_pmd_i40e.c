...
rte_pmd_i40e_set_vf_mac_anti_spoof(uint16_t port, uint16_t vf_id, uint8_t on)
{
	...
	if (vsi->info.valid_sections &
		rte_cpu_to_le_16(I40E_AQ_VSI_PROP_SELWRITY_VALID)) {
		if (on) {
	...
	vsi->info.valid_sections = cpu_to_le16(I40E_AQ_VSI_PROP_SELWRITY_VALID);
	if (on)
...
rte_pmd_i40e_set_vf_unicast_promisc(uint16_t port, uint16_t vf_id, uint8_t on)
{
	...
	ret = i40e_aq_set_vsi_unicast_promislwous(hw, vsi->seid,
						  on, NULL, true);
	if (ret != I40E_SUCCESS) {
		ret = -ENOTSUP;
		PMD_DRV_LOG(ERR, "Failed to set unicast promislwous mode");
	}
...
rte_pmd_i40e_set_vf_multicast_promisc(uint16_t port, uint16_t vf_id, uint8_t on)
{
	...
	ret = i40e_aq_set_vsi_multicast_promislwous(hw, vsi->seid,
						    on, NULL);
	if (ret != I40E_SUCCESS) {
		ret = -ENOTSUP;
		PMD_DRV_LOG(ERR, "Failed to set multicast promislwous mode");
	}
				 ...
				 enum rte_pmd_i40e_package_op op)
{
	...
	if (op == RTE_PMD_I40E_PKG_OP_WR_ADD) {
		if (is_exist) {
			...
			i40e_update_lwstomized_info(dev, buff, size, op);
			rte_free(profile_info_sec);
			return -EEXIST;
		}
...
static int check_ilwalid_pkt_type(uint32_t pkt_type)
{
...
static int check_ilwalid_ptype_mapping(
		...
		uint16_t count)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (check_ilwalid_pkt_type(pkt_type))
			return -1;
	}
			...
			uint8_t exclusive)
{
	...
	if (check_ilwalid_ptype_mapping(mapping_items, count))
		return -EILWAL;
				       ...
				       uint32_t pkt_type)
{
	...
	if (!mask && check_ilwalid_pkt_type(target))
		return -EILWAL;
			...
			uint8_t exclusive)
{
	...
	for (i = 0; i < count; i++)
		if (mapping_items[i].flow_type >= I40E_FLOW_TYPE_MAX ||
		    ...
		    (1ULL << I40E_FILTER_PCTYPE_ILWALID)))
			return -EILWAL;
