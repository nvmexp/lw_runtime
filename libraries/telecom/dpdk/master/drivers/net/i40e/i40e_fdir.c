...
static int i40e_fdir_filter_colwert(const struct i40e_fdir_filter_conf *input,
...
static struct i40e_lwstomized_pctype *
i40e_flow_fdir_find_lwstomized_pctype(struct i40e_pf *pf, uint8_t pctype)
{
	struct i40e_lwstomized_pctype *lws_pctype;
	enum i40e_new_pctype i = I40E_LWSTOMIZED_GTPC;

	for (; i < I40E_LWSTOMIZED_MAX; i++) {
		lws_pctype = &pf->lwstomized_pctype[i];
		if (pctype == lws_pctype->pctype)
			return lws_pctype;
	}
	return NULL;
}
				...
				bool vlan)
{
	...
	bool is_lwstomized_pctype = fdir_input->flow_ext.lwstomized_pctype;
	...
	if (is_lwstomized_pctype) {
		lws_pctype = i40e_flow_fdir_find_lwstomized_pctype(pf, pctype);
		if (!lws_pctype) {
		...
		*ether_type = fdir_input->flow.l2_flow.ether_type;
		...
		if (pctype == I40E_FILTER_PCTYPE_NONF_IPV4_TCP ||
			...
			len = fill_ip6_head(fdir_input, raw_pkt,
					...
					ether_type);
		} else if (lws_pctype->index == I40E_LWSTOMIZED_GTPC ||
			 lws_pctype->index == I40E_LWSTOMIZED_GTPU_IPV4 ||
			 lws_pctype->index == I40E_LWSTOMIZED_GTPU_IPV6 ||
			 lws_pctype->index == I40E_LWSTOMIZED_GTPU) {
			len = fill_ip4_head(fdir_input, raw_pkt, IPPROTO_UDP,
					len, ether_type);
		} else if (lws_pctype->index == I40E_LWSTOMIZED_IPV4_L2TPV3) {
			len = fill_ip4_head(fdir_input, raw_pkt, IPPROTO_L2TP,
					len, ether_type);
		} else if (lws_pctype->index == I40E_LWSTOMIZED_ESP_IPV4) {
			len = fill_ip4_head(fdir_input, raw_pkt, IPPROTO_ESP,
					len, ether_type);
		} else if (lws_pctype->index == I40E_LWSTOMIZED_ESP_IPV4_UDP) {
			len = fill_ip4_head(fdir_input, raw_pkt, IPPROTO_UDP,
					len, ether_type);
		} else if (lws_pctype->index == I40E_LWSTOMIZED_ESP_IPV4_UDP) {
			len = fill_ip4_head(fdir_input, raw_pkt, IPPROTO_UDP,
					len, ether_type);
		} else if (lws_pctype->index == I40E_LWSTOMIZED_ESP_IPV6)
			len = fill_ip6_head(fdir_input, raw_pkt, IPPROTO_ESP,
					len, ether_type);
		else if (lws_pctype->index == I40E_LWSTOMIZED_ESP_IPV6_UDP)
			len = fill_ip6_head(fdir_input, raw_pkt, IPPROTO_UDP,
					len, ether_type);
		else if (lws_pctype->index == I40E_LWSTOMIZED_IPV6_L2TPV3)
			     ...
			     unsigned char *raw_pkt)
{
	...
	} else if (fdir_input->flow_ext.lwstomized_pctype) {
		/* If lwstomized pctype is used */
		lws_pctype = i40e_flow_fdir_find_lwstomized_pctype(pf, pctype);
		if (lws_pctype->index == I40E_LWSTOMIZED_GTPC ||
		    lws_pctype->index == I40E_LWSTOMIZED_GTPU_IPV4 ||
		    lws_pctype->index == I40E_LWSTOMIZED_GTPU_IPV6 ||
		    lws_pctype->index == I40E_LWSTOMIZED_GTPU) {
			udp = (struct rte_udp_hdr *)(raw_pkt + len);
			...
			if (lws_pctype->index == I40E_LWSTOMIZED_GTPU_IPV4) {
			   ...
			   lws_pctype->index == I40E_LWSTOMIZED_IPV6_L2TPV3) {
			l2tpv3oip = (struct rte_flow_item_l2tpv3oip *)(raw_pkt
								       + len);

			if (lws_pctype->index == I40E_LWSTOMIZED_IPV4_L2TPV3)
			...
			lws_pctype->index == I40E_LWSTOMIZED_ESP_IPV6 ||
			lws_pctype->index == I40E_LWSTOMIZED_ESP_IPV4_UDP ||
			lws_pctype->index == I40E_LWSTOMIZED_ESP_IPV6_UDP) {
			if (lws_pctype->index == I40E_LWSTOMIZED_ESP_IPV4) {
...
static int
i40e_fdir_filter_colwert(const struct i40e_fdir_filter_conf *input,
			 struct i40e_fdir_filter *filter)
{
			      ...
			      bool add)
{
	...
	i40e_fdir_filter_colwert(filter, &check_filter);
