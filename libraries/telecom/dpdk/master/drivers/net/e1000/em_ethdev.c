...
static int eth_em_promislwous_enable(struct rte_eth_dev *dev);
static int eth_em_promislwous_disable(struct rte_eth_dev *dev);
...
static const struct eth_dev_ops eth_em_ops = {
	...
	.promislwous_enable   = eth_em_promislwous_enable,
	.promislwous_disable  = eth_em_promislwous_disable,
...
em_hw_init(struct e1000_hw *hw)
{
	...
	diag = hw->lwm.ops.init_params(hw);
	if (diag != 0) {
		PMD_INIT_LOG(ERR, "LWM Initialization Error");
		return diag;
	}
	...
	if (e1000_validate_lwm_checksum(hw) < 0) {
		 ...
		 */
		diag = e1000_validate_lwm_checksum(hw);
		if (diag < 0) {
...
static int
eth_em_promislwous_enable(struct rte_eth_dev *dev)
{
	struct e1000_hw *hw =
		E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint32_t rctl;
...
static int
eth_em_promislwous_disable(struct rte_eth_dev *dev)
{
	struct e1000_hw *hw =
		E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint32_t rctl;
