...
STATIC s32 e1000_init_lwm_params_vf(struct e1000_hw *hw);
...
/**
 *  e1000_init_lwm_params_vf - Inits LWM params
 *  @hw: pointer to the HW structure
 *
 *  Doesn't do much - there's no LWM available to the VF.
 **/
STATIC s32 e1000_init_lwm_params_vf(struct e1000_hw *hw)
{
	DEBUGFUNC("e1000_init_lwm_params_vf");
	hw->lwm.type = e1000_lwm_none;
	hw->lwm.ops.acquire = e1000_acquire_vf;
	hw->lwm.ops.release = e1000_release_vf;
...
void e1000_init_function_pointers_vf(struct e1000_hw *hw)
{
	...
	hw->lwm.ops.init_params = e1000_init_lwm_params_vf;
	hw->phy.ops.init_params = e1000_init_phy_params_vf;
	hw->mbx.ops.init_params = e1000_init_mbx_params_vf;
}

/**
 *  e1000_acquire_vf - Acquire rights to access PHY or LWM.
 *  @hw: pointer to the HW structure
 *
 *  There is no PHY or LWM so we want all attempts to acquire these to fail.
 *  In addition, the MAC registers to access PHY/LWM don't exist so we don't
...
/**
 *  e1000_release_vf - Release PHY or LWM
 *  @hw: pointer to the HW structure
 *
 *  There is no PHY or LWM so we want all attempts to acquire these to fail.
 *  In addition, the MAC registers to access PHY/LWM don't exist so we don't
 ...
 *  the status register's data which is often stale and inaclwrate.
 **/
STATIC s32 e1000_get_link_up_info_vf(struct e1000_hw *hw, u16 *speed,
				     u16 *duplex)
{
				  ...
				  u8 *mc_addr_list, u32 mc_addr_count)
{
	 ...
	 * addresses except for in large enterprise network elwironments.
	 */
