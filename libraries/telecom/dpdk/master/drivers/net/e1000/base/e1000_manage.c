...
/**
 *  e1000_callwlate_checksum - Callwlate checksum for buffer
 ...
 **/
u8 e1000_callwlate_checksum(u8 *buffer, u32 length)
{
	u32 i;
	u8 sum = 0;

	DEBUGFUNC("e1000_callwlate_checksum");
...
bool e1000_enable_tx_pkt_filtering_generic(struct e1000_hw *hw)
{
	...
	hdr->checksum = 0;
	csum = e1000_callwlate_checksum((u8 *)hdr,
				      ...
				      struct e1000_host_mng_command_header *hdr)
{
	...
	hdr->checksum = e1000_callwlate_checksum((u8 *)hdr, length);
...
bool e1000_enable_mng_pass_thru(struct e1000_hw *hw)
{
	...
	if (hw->mac.has_fwsm) {
		...
		factps = E1000_READ_REG(hw, E1000_FACTPS);
		ret_val = e1000_read_lwm(hw, LWM_INIT_CONTROL2_REG, 1, &data);
		if (ret_val)
			return false;

		if (!(factps & E1000_FACTPS_MNGCG) &&
		    ((data & E1000_LWM_INIT_CTRL2_MNGM) ==
