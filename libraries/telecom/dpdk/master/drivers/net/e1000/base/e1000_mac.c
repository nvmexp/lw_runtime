 ...
 *  @hw: pointer to the HW structure
 *
 *  Checks the lwm for an alternate MAC address.  An alternate MAC address
...
s32 e1000_check_alt_mac_addr_generic(struct e1000_hw *hw)
{
	u32 i;
	s32 ret_val;
	u16 offset, lwm_alt_mac_addr_offset, lwm_data;
	u8 alt_mac_addr[ETH_ADDR_LEN];

	DEBUGFUNC("e1000_check_alt_mac_addr_generic");

	ret_val = hw->lwm.ops.read(hw, LWM_COMPAT, 1, &lwm_data);
	if (ret_val)
		return ret_val;
	...
	ret_val = hw->lwm.ops.read(hw, LWM_ALT_MAC_ADDR_PTR, 1,
				   &lwm_alt_mac_addr_offset);
	if (ret_val) {
		DEBUGOUT("LWM Read Error\n");
		return ret_val;
	}

	if ((lwm_alt_mac_addr_offset == 0xFFFF) ||
	    (lwm_alt_mac_addr_offset == 0x0000))
		/* There is no Alternate MAC Address */
		return E1000_SUCCESS;

	if (hw->bus.func == E1000_FUNC_1)
		lwm_alt_mac_addr_offset += E1000_ALT_MAC_ADDRESS_OFFSET_LAN1;
	if (hw->bus.func == E1000_FUNC_2)
		lwm_alt_mac_addr_offset += E1000_ALT_MAC_ADDRESS_OFFSET_LAN2;

	if (hw->bus.func == E1000_FUNC_3)
		lwm_alt_mac_addr_offset += E1000_ALT_MAC_ADDRESS_OFFSET_LAN3;
	for (i = 0; i < ETH_ADDR_LEN; i += 2) {
		offset = lwm_alt_mac_addr_offset + (i >> 1);
		...
		alt_mac_addr[i] = (u8)(lwm_data & 0xFF);
		alt_mac_addr[i + 1] = (u8)(lwm_data >> 8);
	}
...
int e1000_rar_set_generic(struct e1000_hw *hw, u8 *addr, u32 index)
{
	...
	/* Some bridges will combine conselwtive 32-bit writes into
...
s32 e1000_set_default_fc_generic(struct e1000_hw *hw)
{
	...
	if (hw->mac.type == e1000_i350) {
		lwm_offset = LWM_82580_LAN_FUNC_OFFSET(hw->bus.func);
		ret_val = hw->lwm.ops.read(hw,
					   LWM_INIT_CONTROL2_REG +
					   lwm_offset,
					   1, &lwm_data);
	} else {
		ret_val = hw->lwm.ops.read(hw,
	...
	if (!(lwm_data & LWM_WORD0F_PAUSE_MASK))
		hw->fc.requested_mode = e1000_fc_none;
	else if ((lwm_data & LWM_WORD0F_PAUSE_MASK) ==
		 LWM_WORD0F_ASM_DIR)
...
s32 e1000_set_fc_watermarks_generic(struct e1000_hw *hw)
{
	 ...
	 */
	if (hw->fc.lwrrent_mode & e1000_fc_tx_pause) {
...
s32 e1000_force_mac_fc_generic(struct e1000_hw *hw)
{
	 ...
	 * according to the "hw->fc.lwrrent_mode" parameter.
	 *
	 * The possible values of the "fc" parameter are:
	 *      0:  Flow control is completely disabled
	 ...
	 */
	DEBUGOUT1("hw->fc.lwrrent_mode = %u\n", hw->fc.lwrrent_mode);
	...
	if ((hw->phy.media_type == e1000_media_type_copper) && mac->autoneg) {
		...
		if ((mii_nway_adv_reg & NWAY_AR_PAUSE) &&
			...
			if (hw->fc.requested_mode == e1000_fc_full) {
				hw->fc.lwrrent_mode = e1000_fc_full;
				DEBUGOUT("Flow Control = FULL.\n");
			} else {
				hw->fc.lwrrent_mode = e1000_fc_rx_pause;
		 ...
		 */
			...
			hw->fc.lwrrent_mode = e1000_fc_tx_pause;
		 ...
		 */
			 ...
			 * should be disabled.
			 */
			hw->fc.lwrrent_mode = e1000_fc_none;
...
s32 e1000_get_auto_rd_done_generic(struct e1000_hw *hw)
{
	...
	if (i == AUTO_READ_DONE_TIMEOUT) {
		DEBUGOUT("Auto read by HW from LWM has not completed.\n");
...
s32 e1000_blink_led_generic(struct e1000_hw *hw)
{
	...
	if (hw->phy.media_type == e1000_media_type_fiber) {
		 ...
		 * (or "off" if ilwerted) in ledctl_mode2.  The blink
...
void e1000_update_adaptive_generic(struct e1000_hw *hw)
{
	...
	if ((mac->collision_delta * mac->ifs_ratio) > mac->tx_packet_delta) {
		if (mac->tx_packet_delta > MIN_NUM_XMITS) {
			...
			if (mac->lwrrent_ifs_val < mac->ifs_max_val) {
				if (!mac->lwrrent_ifs_val)
					mac->lwrrent_ifs_val = mac->ifs_min_val;
				else
					mac->lwrrent_ifs_val +=
						mac->ifs_step_size;
