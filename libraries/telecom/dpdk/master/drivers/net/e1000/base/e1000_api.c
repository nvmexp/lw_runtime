...
/**
 *  e1000_init_lwm_params - Initialize LWM function pointers
 *  @hw: pointer to the HW structure
 *
 *  This function initializes the function pointers for the LWM
 *  set of functions.  Called by drivers or by e1000_setup_init_funcs.
 **/
s32 e1000_init_lwm_params(struct e1000_hw *hw)
{
	s32 ret_val = E1000_SUCCESS;

	if (hw->lwm.ops.init_params) {
		ret_val = hw->lwm.ops.init_params(hw);
		if (ret_val) {
			DEBUGOUT("LWM Initialization Error\n");
			goto out;
		}
	} else {
		DEBUGOUT("lwm.init_lwm_params was NULL\n");
		ret_val = -E1000_ERR_CONFIG;
	}
...
s32 e1000_setup_init_funcs(struct e1000_hw *hw, bool init_device)
{
	...
	if (!(ret_val) && init_device) {
		...
		ret_val = e1000_init_lwm_params(hw);
		if (ret_val)
			goto out;
 ...
 *  @hw: pointer to the HW structure
 *
 *  Force the MAC's flow control settings. Lwrrently no func pointer exists
 ...
 *  @hw: pointer to the HW structure
 *
 *  Resets the adaptive IFS. Lwrrently no func pointer exists and all
 ...
 *  @hw: pointer to the HW structure
 *
 *  Updates adapter IFS. Lwrrently no func pointer exists and all
 *  implementations are handled in the generic version of this function.
 **/
 ...
 *  requests. Lwrrently no func pointer exists and all implementations are
 ...
 *  table. Lwrrently no func pointer exists and all implementations
 ...
 *  Lwrrently no func pointer exists and all implementations are handled in the
 ...
 *
 *  Writes the command header after does the checksum callwlation.
 **/
 ...
 *
 *  Reads a register out of the Kumeran interface. Lwrrently no func pointer
 ...
 *  @data: the value to write.
 *
 *  Writes a register to the Kumeran interface. Lwrrently no func pointer
 *  exists and all implementations are handled in the generic version of
 *  this function.
 **/
...
/**
 *  e1000_validate_lwm_checksum - Verifies LWM (EEPROM) checksum
 *  @hw: pointer to the HW structure
 *
 *  Validates the LWM checksum is correct. This is a function pointer entry
 *  point called by drivers.
 **/
s32 e1000_validate_lwm_checksum(struct e1000_hw *hw)
{
	if (hw->lwm.ops.validate)
		return hw->lwm.ops.validate(hw);

	return -E1000_ERR_CONFIG;
}

/**
 *  e1000_update_lwm_checksum - Updates LWM (EEPROM) checksum
 *  @hw: pointer to the HW structure
 *
 *  Updates the LWM checksum. Lwrrently no func pointer exists and all
 *  implementations are handled in the generic version of this function.
 **/
s32 e1000_update_lwm_checksum(struct e1000_hw *hw)
{
	if (hw->lwm.ops.update)
		return hw->lwm.ops.update(hw);

	return -E1000_ERR_CONFIG;
}

/**
 *  e1000_reload_lwm - Reloads EEPROM
 *  @hw: pointer to the HW structure
 *
 *  Reloads the EEPROM by setting the "Reinitialize from EEPROM" bit in the
 *  extended control register.
 **/
void e1000_reload_lwm(struct e1000_hw *hw)
{
	if (hw->lwm.ops.reload)
		hw->lwm.ops.reload(hw);
}

/**
 *  e1000_read_lwm - Reads LWM (EEPROM)
 *  @hw: pointer to the HW structure
 *  @offset: the word offset to read
 *  @words: number of 16-bit words to read
 *  @data: pointer to the properly sized buffer for the data.
 *
 *  Reads 16-bit chunks of data from the LWM (EEPROM). This is a function
 *  pointer entry point called by drivers.
 **/
s32 e1000_read_lwm(struct e1000_hw *hw, u16 offset, u16 words, u16 *data)
{
	if (hw->lwm.ops.read)
		return hw->lwm.ops.read(hw, offset, words, data);

	return -E1000_ERR_CONFIG;
}

/**
 *  e1000_write_lwm - Writes to LWM (EEPROM)
 *  @hw: pointer to the HW structure
 *  @offset: the word offset to read
 *  @words: number of 16-bit words to write
 *  @data: pointer to the properly sized buffer for the data.
 *
 *  Writes 16-bit chunks of data to the LWM (EEPROM). This is a function
 *  pointer entry point called by drivers.
 **/
s32 e1000_write_lwm(struct e1000_hw *hw, u16 offset, u16 words, u16 *data)
{
	if (hw->lwm.ops.write)
		return hw->lwm.ops.write(hw, offset, words, data);
