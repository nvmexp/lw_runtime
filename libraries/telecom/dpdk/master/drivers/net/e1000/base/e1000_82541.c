...
STATIC s32  e1000_init_lwm_params_82541(struct e1000_hw *hw);
...
/**
 *  e1000_init_lwm_params_82541 - Init LWM func ptrs.
 *  @hw: pointer to the HW structure
 **/
STATIC s32 e1000_init_lwm_params_82541(struct e1000_hw *hw)
{
	...
	DEBUGFUNC("e1000_init_lwm_params_82541");
	...
	case e1000_lwm_override_microwire_small:
		...
		break;
	default:
		lwm->type = eecd & E1000_EECD_TYPE ? e1000_lwm_eeprom_spi
			    : e1000_lwm_eeprom_microwire;
		break;
	}
	...
	if (lwm->type == e1000_lwm_eeprom_spi) {
		...
		/*
		 * lwm->word_size must be discovered after the pointers
		 * are set so we can verify the size from the lwm image
		 * itself.  Temporarily set it to a dummy value so the
		 * read will work.
		 */
		lwm->word_size = 64;
		ret_val = lwm->ops.read(hw, LWM_CFG, 1, &size);
		if (ret_val)
			goto out;
		size = (size & LWM_SIZE_MASK) >> LWM_SIZE_SHIFT;
		/*
		 ...
		 */
		if (size) {
			size += LWM_WORD_SIZE_BASE_SHIFT_82541;
			lwm->word_size = 1 << size;
		}
	} else {
		lwm->address_bits = (eecd & E1000_EECD_ADDR_BITS) ? 8 : 6;
		lwm->delay_usec = 50;
		lwm->opcode_bits = 3;
		lwm->word_size = (eecd & E1000_EECD_ADDR_BITS) ? 256 : 64;
...
void e1000_init_function_pointers_82541(struct e1000_hw *hw)
{
	...
	hw->lwm.ops.init_params = e1000_init_lwm_params_82541;
...
STATIC s32 e1000_get_cable_length_igp_82541(struct e1000_hw *hw)
{
	...
	for (i = 0; i < IGP01E1000_PHY_CHANNEL_NUM; i++) {
		...
		lwr_agc_value = data >> IGP01E1000_AGC_LENGTH_SHIFT;

		/* Bounds checking */
		if ((lwr_agc_value >= IGP01E1000_AGC_LENGTH_TABLE_SIZE - 1) ||
		    (lwr_agc_value == 0)) {
			ret_val = -E1000_ERR_PHY;
			goto out;
		}

		agc_value += lwr_agc_value;

		if (min_agc_value > lwr_agc_value)
			min_agc_value = lwr_agc_value;
	}
...
STATIC s32 e1000_phy_init_script_82541(struct e1000_hw *hw)
{
	...
	/* Delay after phy reset to enable LWM configuration to load */
	msec_delay(20);
