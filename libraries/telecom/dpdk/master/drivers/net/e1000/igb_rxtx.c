...
/**
 * Strulwtre to check if new context need be built
 */
		...
		uint64_t ol_flags, union igb_tx_offload tx_offload)
{
	...
	uint32_t ctx_idx, ctx_lwrr;
	uint32_t vlan_macip_lens;
	union igb_tx_offload tx_offload_mask;

	ctx_lwrr = txq->ctx_lwrr;
	ctx_idx = ctx_lwrr + txq->ctx_start;
		...
		union igb_tx_offload tx_offload)
{
	...
	if (likely((txq->ctx_cache[txq->ctx_lwrr].flags == flags) &&
		(txq->ctx_cache[txq->ctx_lwrr].tx_offload.data ==
		(txq->ctx_cache[txq->ctx_lwrr].tx_offload_mask.data & tx_offload.data)))) {
			return txq->ctx_lwrr;
	}
	       ...
	       uint16_t nb_pkts)
{
	...
	for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
		...
		if (tx_ol_req) {
			...
			ctx = txq->ctx_lwrr + txq->ctx_start;
