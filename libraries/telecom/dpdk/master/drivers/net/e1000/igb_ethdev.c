...
static int  eth_igb_promislwous_enable(struct rte_eth_dev *dev);
static int  eth_igb_promislwous_disable(struct rte_eth_dev *dev);
...
static int igbvf_promislwous_enable(struct rte_eth_dev *dev);
static int igbvf_promislwous_disable(struct rte_eth_dev *dev);
...
static const struct eth_dev_ops eth_igb_ops = {
	...
	.promislwous_enable   = eth_igb_promislwous_enable,
	.promislwous_disable  = eth_igb_promislwous_disable,
...
static const struct eth_dev_ops igbvf_eth_dev_ops = {
	...
	.promislwous_enable   = igbvf_promislwous_enable,
	.promislwous_disable  = igbvf_promislwous_disable,
...
eth_igb_dev_init(struct rte_eth_dev *eth_dev)
{
	...
	/*
	 * Start from a known state, this is important in reading the lwm
	 * and mac from that.
	 */
	igb_pf_reset_hw(hw);

	/* Make sure we have a good EEPROM before we read from it */
	if (e1000_validate_lwm_checksum(hw) < 0) {
		/*
...
eth_igb_reset(struct rte_eth_dev *dev)
{
	 ...
	 * mechanism is PMD specific and is lwrrently not implemented.
...
static int
eth_igb_promislwous_enable(struct rte_eth_dev *dev)
{
	struct e1000_hw *hw =
		E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint32_t rctl;
...
static int
eth_igb_promislwous_disable(struct rte_eth_dev *dev)
{
	struct e1000_hw *hw =
		E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint32_t rctl;
...
/*
 * It exelwtes link_update after knowing an interrupt is prsent.
 *
 * @param dev
 *  Pointer to struct rte_eth_dev.
 *
...
static int
igbvf_promislwous_enable(struct rte_eth_dev *dev)
{
	struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
static int
igbvf_promislwous_disable(struct rte_eth_dev *dev)
{
	struct e1000_hw *hw = E1000_DEV_PRIVATE_TO_HW(dev->data->dev_private);
...
igbvf_allmulticast_enable(struct rte_eth_dev *dev)
{
	...
	/* In promislwous mode multicast promisc already set */
...
igbvf_allmulticast_disable(struct rte_eth_dev *dev)
{
	...
	/* In promislwous mode leave multicast promisc enabled */
	if (dev->data->promislwous == 0)
		e1000_promisc_set_vf(hw, e1000_promisc_disabled);
			...
			struct e1000_2tuple_filter_info *filter_info)
{
	...
	if (filter->priority > E1000_2TUPLE_MAX_PRI)
		return -EILWAL;  /* filter index is out of range. */
	if (filter->tcp_flags > RTE_NTUPLE_TCP_FLAGS_MASK)
		return -EILWAL;  /* flags is invalid. */
	...
	struct rte_dev_eeprom_info *in_eeprom)
{
	...
	if ((first >= hw->lwm.word_size) ||
	    ((first + length) >= hw->lwm.word_size))
		return -EILWAL;

	in_eeprom->magic = hw->vendor_id |
		((uint32_t)hw->device_id << 16);

	if ((lwm->ops.read) == NULL)
		return -ENOTSUP;

	return lwm->ops.read(hw, first, length, data);
}
	...
	struct rte_dev_eeprom_info *in_eeprom)
{
	...
	if ((lwm->ops.write) == NULL)
		return -ENOTSUP;
	return lwm->ops.write(hw,  first, length, data);
}
			  ...
			  struct rte_dev_eeprom_info *info)
{
	...
	for (i = 0; i < last_word - first_word + 1; i++) {
		status = e1000_read_phy_reg_i2c(hw, (first_word + i) * 2,
						&dataword[i]);
		if (status) {
			/* Error oclwrred while reading module */
			return -EIO;
		}
