...
static int virtio_dev_promislwous_enable(struct rte_eth_dev *dev);
static int virtio_dev_promislwous_disable(struct rte_eth_dev *dev);
			  ...
			  int *dlen, int pkt_num)
{
	...
	PMD_INIT_LOG(DEBUG, "vq->vq_free_cnt=%d\lwq->vq_desc_head_idx=%d",
...
static int
virtio_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct virtio_hw *hw = dev->data->dev_private;
	struct virtio_pmd_ctrl ctrl;
	int dlen[1];
...
static int
virtio_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct virtio_hw *hw = dev->data->dev_private;
	struct virtio_pmd_ctrl ctrl;
	int dlen[1];
...
static const struct eth_dev_ops virtio_eth_dev_ops = {
	...
	.promislwous_enable      = virtio_dev_promislwous_enable,
	.promislwous_disable     = virtio_dev_promislwous_disable,
 ...
 * some limited supports lwrrently.
 */
