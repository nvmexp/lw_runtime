 ...
 */
static int
ipn3ke_pattern_ip_lwgre(const struct rte_flow_item patterns[],
	struct rte_flow_error *error, struct ipn3ke_flow_parse *parser)
{
	const struct rte_flow_item_lwgre *lwgre = NULL;
	const struct rte_flow_item_ipv4 *ipv4 = NULL;
	const struct rte_flow_item_udp *udp = NULL;
	const struct rte_flow_item *item;
	...
	for (item = patterns; item->type != RTE_FLOW_ITEM_TYPE_END; item++) {
		...
		case RTE_FLOW_ITEM_TYPE_LWGRE:
			...
			rte_memcpy(&parser->key[6], lwgre->tni, 3);
			break;
	...
	if (ipv4 != NULL && udp != NULL && lwgre != NULL) {
...
static const struct ipn3ke_flow_pattern ipn3ke_supported_patterns[] = {
	...
	[IPN3KE_HASH_KEY_IP_LWGRE] = {
		.items = FLOW_PATTERNS(RTE_FLOW_ITEM_TYPE_IPV4,
					RTE_FLOW_ITEM_TYPE_UDP,
					RTE_FLOW_ITEM_TYPE_LWGRE),
		.filter = ipn3ke_pattern_ip_lwgre,
	},
...
static int
ipn3ke_flow_colwert_attributes(const struct rte_flow_attr *attr,
...
static int
ipn3ke_flow_colwert_actions(const struct rte_flow_action actions[],
	struct rte_flow_error *error, struct ipn3ke_flow_parse *parser)
{
...
static int
ipn3ke_flow_colwert_items(const struct rte_flow_item items[],
	struct rte_flow_error *error, struct ipn3ke_flow_parse *parser)
{
	pattern_filter_t filter = NULL;
	uint32_t idx;
...
static void
ipn3ke_flow_colwert_finalise(struct ipn3ke_hw *hw,
...
static int
ipn3ke_flow_colwert(const struct rte_flow_attr *attr,
	...
	struct ipn3ke_flow_parse *parser)
{
	...
	ret = ipn3ke_flow_colwert_attributes(attr, error);
	if (ret)
		return ret;

	ret = ipn3ke_flow_colwert_actions(actions, error, parser);
	if (ret)
		return ret;

	ret = ipn3ke_flow_colwert_items(items, error, parser);
	if (ret)
		return ret;
	...
	const struct rte_flow_action actions[], struct rte_flow_error *error)
{
	...
	return ipn3ke_flow_colwert(attr, pattern, actions, error, &parser);
}
	...
	const struct rte_flow_action actions[], struct rte_flow_error *error)
{
	...
	ret = ipn3ke_flow_colwert(attr, pattern, actions, error, &parser);
	if (ret < 0) {
		rte_flow_error_set(error,
				-ret,
				RTE_FLOW_ERROR_TYPE_HANDLE,
	...
	ipn3ke_flow_colwert_finalise(hw, &parser, flow);
