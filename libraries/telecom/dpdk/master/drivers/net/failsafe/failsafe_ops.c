...
static int
fs_promislwous_enable(struct rte_eth_dev *dev)
{
	struct sub_device *sdev;
	uint8_t i;
	int ret = 0;
	...
	fs_lock(dev, 0);
	FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
		ret = rte_eth_promislwous_enable(PORT_ID(sdev));
		ret = fs_err(sdev, ret);
		if (ret != 0) {
			ERROR("Promislwous mode enable failed for subdevice %d",
				PORT_ID(sdev));
			break;
		}
	}
	if (ret != 0) {
		...
		FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
			ret = rte_eth_promislwous_disable(PORT_ID(sdev));
			ret = fs_err(sdev, ret);
			if (ret != 0)
				ERROR("Promislwous mode disable during rollback failed for subdevice %d",
					PORT_ID(sdev));
		}
	}
	fs_unlock(dev, 0);
...
static int
fs_promislwous_disable(struct rte_eth_dev *dev)
{
	struct sub_device *sdev;
	uint8_t i;
	int ret = 0;
	...
	FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
		...
		ret = fs_err(sdev, ret);
		if (ret != 0) {
			ERROR("Promislwous mode disable failed for subdevice %d",
				PORT_ID(sdev));
			break;
		}
	}
	if (ret != 0) {
		...
		FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
			...
			ret = fs_err(sdev, ret);
			if (ret != 0)
				ERROR("Promislwous mode enable during rollback failed for subdevice %d",
					PORT_ID(sdev));
		}
	}
	fs_unlock(dev, 0);
	     ...
	     struct rte_eth_stats *stats)
{
	...
	fs_lock(dev, 0);
	rte_memcpy(stats, &PRIV(dev)->stats_aclwmulator, sizeof(*stats));
	FOREACH_SUBDEV_STATE(sdev, i, dev, DEV_ACTIVE) {
...
fs_stats_reset(struct rte_eth_dev *dev)
{
	...
	memset(&PRIV(dev)->stats_aclwmulator, 0, sizeof(struct rte_eth_stats));
	fs_unlock(dev, 0);
...
const struct eth_dev_ops failsafe_ops = {
	...
	.promislwous_enable = fs_promislwous_enable,
	.promislwous_disable = fs_promislwous_disable,
