		...
		struct sub_device *sdev)
{
	...
	/* promislwous */
	if (dev->data->promislwous != edev->data->promislwous) {
		DEBUG("Configuring promislwous");
		if (dev->data->promislwous)
			ret = rte_eth_promislwous_enable(PORT_ID(sdev));
		else
			ret = rte_eth_promislwous_disable(PORT_ID(sdev));
		if (ret != 0) {
			ERROR("Failed to apply promislwous mode");
			return ret;
		}
	} else {
		DEBUG("promislwous already set");
	}
	/* all_multicast */
...
fs_dev_stats_save(struct sub_device *sdev)
{
	...
	failsafe_stats_increment
		(&PRIV(fs_dev(sdev))->stats_aclwmulator,
