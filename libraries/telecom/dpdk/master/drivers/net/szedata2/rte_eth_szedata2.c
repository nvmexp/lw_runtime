		...
		uint16_t nb_pkts)
{
	...
	for (i = 0; i < nb_pkts; i++) {
		...
		if (!sze->ct_rx_rem_bytes && sze->ct_rx_lck_orig == NULL) {
			...
			sze->ct_rx_lwr_ptr = sze->ct_rx_lck->start;
		...
		if (sze->ct_rx_rem_bytes < RTE_SZE2_PACKET_HEADER_SIZE) {
			...
			rte_memcpy(sze->ct_rx_buffer, sze->ct_rx_lwr_ptr,
					sze->ct_rx_rem_bytes);
			...
			sze->ct_rx_lwr_ptr += RTE_SZE2_PACKET_HEADER_SIZE -
				sze->ct_rx_rem_bytes;
			sze->ct_rx_rem_bytes = sze->ct_rx_lck->len -
				RTE_SZE2_PACKET_HEADER_SIZE +
				sze->ct_rx_rem_bytes;
			...
			/* not cut */
			header_ptr = (uint8_t *)sze->ct_rx_lwr_ptr;
			sze->ct_rx_lwr_ptr += RTE_SZE2_PACKET_HEADER_SIZE;
		...
		if (sze->ct_rx_rem_bytes >=
			...
			packet_ptr1 = sze->ct_rx_lwr_ptr + hw_data_align;
			packet_len1 = packet_size;
			packet_ptr2 = NULL;
			packet_len2 = 0;

			sze->ct_rx_lwr_ptr += RTE_SZE2_ALIGN8(sg_size) -
				RTE_SZE2_PACKET_HEADER_SIZE;
		...
		uint16_t nb_pkts)
{
	...
	for (i = 0; i < nb_pkts; i++) {
		...
		unsigned char *ct_rx_lwr_ptr_backup;
		...
		ct_rx_lwr_ptr_backup = sze->ct_rx_lwr_ptr;
		...
		if (unlikely(mbuf == NULL)) {
			...
			sze->ct_rx_lwr_ptr = ct_rx_lwr_ptr_backup;
			sze_q->priv->dev->data->rx_mbuf_alloc_failed++;
			break;
		}
		...
		if (packet_size <= buf_size) {
			...
			if (packet_ptr2 != NULL) {
				...
				/* fill the rest of lwrrently last mbuf */
...
static int
eth_promislwous_enable(struct rte_eth_dev *dev __rte_unused)
{
	PMD_DRV_LOG(WARNING, "Enabling promislwous mode is not supported. "
			"The card is always in promislwous mode.");
	return 0;
}

static int
eth_promislwous_disable(struct rte_eth_dev *dev __rte_unused)
{
	PMD_DRV_LOG(WARNING, "Disabling promislwous mode is not supported. "
			"The card is always in promislwous mode.");
	return -ENOTSUP;
}
...
static const struct eth_dev_ops ops = {
	...
	.promislwous_enable   = eth_promislwous_enable,
	.promislwous_disable  = eth_promislwous_disable,
