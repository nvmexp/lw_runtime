...
show_warnings(uint16_t slave_id)
{
	...
	if (warnings & WRN_RX_QUEUE_FULL) {
		RTE_BOND_LOG(DEBUG,
			     ...
			     "Receive and transmit functions must be ilwoked on bonded"
		...
		struct lacpdu *lacp)
{
	...
	if (SM_FLAG(port, BEGIN)) {
		...
		timer_cancel(&port->lwrrent_while_timer);
	...
	if (!SM_FLAG(port, LACP_ENABLED)) {
		...
		if (!timer_is_stopped(&port->lwrrent_while_timer)) {
			port->selected = UNSELECTED;
			record_default(port);
			PARTNER_STATE_CLR(port, AGGREGATION);
			ACTOR_STATE_CLR(port, EXPIRED);
	...
	if (lacp) {
		...
		timer_set(&port->lwrrent_while_timer, timeout);
		ACTOR_STATE_CLR(port, EXPIRED);
	...
	if (!timer_is_running(&port->lwrrent_while_timer)) {
		if (SM_FLAG(port, EXPIRED)) {
			...
			timer_set(&port->lwrrent_while_timer,
...
{
	struct port *port = &bond_mode_8023ad_ports[slave_id];
	/* Callwlate if either site is LACP enabled */
	uint64_t timeout;
...
bond_mode_8023ad_register_lacp_mac(uint16_t slave_id)
{
	...
	ret = rte_eth_promislwous_enable(slave_id);
	if (ret != 0) {
		RTE_BOND_LOG(ERR,
			"failed to enable promislwous mode for port %u: %s",
			slave_id, rte_strerror(-ret));
	}
	if (rte_eth_promislwous_get(slave_id)) {
		RTE_BOND_LOG(DEBUG, "forced promislwous for port %u",
			     slave_id);
		bond_mode_8023ad_ports[slave_id].forced_rx_flags =
...
bond_mode_8023ad_unregister_lacp_mac(uint16_t slave_id)
{
	...
	case BOND_8023AD_FORCED_PROMISC:
		...
		ret = rte_eth_promislwous_disable(slave_id);
		if (ret != 0)
			RTE_BOND_LOG(ERR,
				  ...
				  uint16_t slave_id, struct rte_mbuf *pkt)
{
	...
	if (subtype == SLOW_SUBTYPE_MARKER) {
		...
		/* Setup marker timer. Do it in loop in case conlwrrent access. */
		do {
