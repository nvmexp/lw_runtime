		...
		uint16_t nb_pkts)
{
	...
	return rte_eth_rx_burst(internals->lwrrent_primary_port,
		...
		bool dedicated_rxq)
{
	...
	const uint8_t promisc = rte_eth_promislwous_get(internals->port_id);
	...
	for (i = 0; i < slave_count && num_rx_total < nb_pkts; i++) {
		...
		while (j < num_rx_total) {
			 ...
			 * - bonding interface is not in promislwous mode:
...
arp_op_name(uint16_t arp_op, char *buf, size_t buf_len)
{
	...
	case RTE_ARP_OP_REVREPLY:
		...
		return;
	case RTE_ARP_OP_ILWREQUEST:
		strlcpy(buf, "Peer Identify Request", buf_len);
		return;
	case RTE_ARP_OP_ILWREPLY:
		...
		struct rte_mbuf **bufs, uint16_t nb_pkts)
{
	...
	return rte_eth_tx_burst(internals->lwrrent_primary_port, bd_tx_q->queue_id,
			bufs, nb_pkts);
}
	...
	default:
		...
		for (i = 0; i < internals->slave_count; i++) {
			if (internals->slaves[i].port_id ==
					internals->lwrrent_primary_port) {
				if (rte_eth_dev_default_mac_addr_set(
						internals->lwrrent_primary_port,
						bonded_eth_dev->data->mac_addrs)) {
					RTE_BOND_LOG(ERR, "Failed to update port Id %d MAC address",
		...
		uint16_t slave_port_id)
{
	...
	if (internals->active_slave_count < 1)
		internals->lwrrent_primary_port = slave_port_id;
	else
...
static int
bond_ethdev_promislwous_enable(struct rte_eth_dev *eth_dev);
...
bond_ethdev_slave_link_status_change_monitor(void *cb_arg)
{
	...
	/* If device is lwrrently being configured then don't check slaves link
...
bond_ethdev_link_update(struct rte_eth_dev *ethdev, int wait_to_complete)
{
	...
	case BONDING_MODE_ACTIVE_BACKUP:
		...
		ret = link_update(bond_ctx->lwrrent_primary_port, &slave_link);
		if (ret < 0) {
			RTE_BOND_LOG(ERR, "Slave (port %u) link get failed: %s",
				bond_ctx->lwrrent_primary_port,
				rte_strerror(-ret));
			return 0;
		}
...
bond_ethdev_promislwous_enable(struct rte_eth_dev *eth_dev)
{
	...
	switch (internals->mode) {
	/* Promislwous mode is propagated to all slaves */
	case BONDING_MODE_ROUND_ROBIN:
	case BONDING_MODE_BALANCE:
	case BONDING_MODE_BROADCAST:
	case BONDING_MODE_8023AD: {
		...
		for (i = 0; i < internals->slave_count; i++) {
			...
			if (ret != 0)
				RTE_BOND_LOG(ERR,
					"Failed to enable promislwous mode for port %u: %s",
					port_id, rte_strerror(-ret));
			else
				slave_ok++;
		}
	...
	/* Promislwous mode is propagated only to primary slave */
	case BONDING_MODE_ACTIVE_BACKUP:
	case BONDING_MODE_TLB:
	case BONDING_MODE_ALB:
	default:
		...
		if (internals->slave_count == 0)
			break;
		port_id = internals->lwrrent_primary_port;
		ret = rte_eth_promislwous_enable(port_id);
		if (ret != 0)
			RTE_BOND_LOG(ERR,
				"Failed to enable promislwous mode for port %u: %s",
...
static int
bond_ethdev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct bond_dev_private *internals = dev->data->dev_private;
	int i;
	int ret = 0;
	...
	case BONDING_MODE_8023AD: {
		...
		for (i = 0; i < internals->slave_count; i++) {
			...
			ret = rte_eth_promislwous_disable(port_id);
			if (ret != 0)
				RTE_BOND_LOG(ERR,
					"Failed to disable promislwous mode for port %u: %s",
					port_id, rte_strerror(-ret));
			else
				slave_ok++;
		}
	...
	if (ret == 0 && link.link_status) {
		...
		if (internals->active_slave_count < 1) {
			...
			bonded_eth_dev->data->dev_link.link_status =
								ETH_LINK_UP;
			internals->lwrrent_primary_port = port_id;
			lsc_flag = 1;
		 ...
		 * available set to -1 */
		if (port_id == internals->lwrrent_primary_port) {
			if (internals->active_slave_count > 0)
				bond_ethdev_primary_set(internals,
						internals->active_slaves[0]);
			else
				internals->lwrrent_primary_port = internals->primary_port;
			mac_address_slaves_update(bonded_eth_dev);
		}
	}
...
const struct eth_dev_ops default_dev_ops = {
	...
	.promislwous_enable   = bond_ethdev_promislwous_enable,
	.promislwous_disable  = bond_ethdev_promislwous_disable,
...
bond_alloc(struct rte_vdev_device *dev, uint8_t mode)
{
	...
	internals->mode = BONDING_MODE_ILWALID;
	internals->lwrrent_primary_port = RTE_MAX_ETHPORTS + 1;
