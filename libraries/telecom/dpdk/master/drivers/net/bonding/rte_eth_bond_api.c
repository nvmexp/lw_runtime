				   ...
				   uint16_t slave_port_id)
{
	...
	if (internals->lwrrent_primary_port == slave_port_id) {
		if (internals->active_slave_count > 0)
			internals->lwrrent_primary_port = internals->active_slaves[0];
		else if (internals->slave_count > 0)
			internals->lwrrent_primary_port = internals->slaves[0].port_id;
		else
...
rte_eth_bond_primary_get(uint16_t bonded_port_id)
{
	...
	return internals->lwrrent_primary_port;
}
