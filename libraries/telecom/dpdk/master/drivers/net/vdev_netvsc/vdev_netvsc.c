		      ...
		      const unsigned char family)
{
	...
	if (!iface || (family != AF_INET && family != AF_INET6)) {
		DRV_LOG(ERR, "%s", rte_strerror(EILWAL));
		return -EILWAL;
	}
...
vdev_netvsc_vdev_probe(struct rte_vdev_device *dev)
{
	...
	DRV_LOG(DEBUG, "ilwoked as \"%s\", using arguments \"%s\"", name, args);
...
RTE_INIT(vdev_netvsc_lwstom_scan_add)
{
	if (rte_hypervisor_get() == RTE_HYPERVISOR_HYPERV)
		rte_vdev_add_lwstom_scan(vdev_netvsc_scan_callback, NULL);
}
