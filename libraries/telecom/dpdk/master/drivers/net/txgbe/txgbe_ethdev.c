...
static const struct reg_info txgbe_regs_lwm[] = {
	{0, 0, 0, ""}
};
...
static const struct rte_txgbe_xstats_name_off rte_txgbe_stats_strings[] = {
	...
	HW_XSTAT(rx_macsec_sa_pkts_ilwalid),
...
eth_txgbe_dev_init(struct rte_eth_dev *eth_dev, void *init_params __rte_unused)
{
	...
	hw->fc.lwrrent_mode = txgbe_fc_full;
	 ...
	 * The failure oclwrs in txgbe_identify_phy() for all devices,
...
#define UPDATE_QP_COUNTER_32bit(reg, last_counter, counter)     \
	{                                                       \
		uint32_t lwrrent_counter = rd32(hw, reg);       \
		if (lwrrent_counter < last_counter)             \
			lwrrent_counter += 0x100000000LL;       \
		if (!hw->offset_loaded)                         \
			last_counter = lwrrent_counter;         \
		counter = lwrrent_counter - last_counter;       \
		counter &= 0xFFFFFFFFLL;                        \
	}

#define UPDATE_QP_COUNTER_36bit(reg_lsb, reg_msb, last_counter, counter) \
	{                                                                \
		uint64_t lwrrent_counter_lsb = rd32(hw, reg_lsb);        \
		uint64_t lwrrent_counter_msb = rd32(hw, reg_msb);        \
		uint64_t lwrrent_counter = (lwrrent_counter_msb << 32) | \
			lwrrent_counter_lsb;                             \
		if (lwrrent_counter < last_counter)                      \
			lwrrent_counter += 0x1000000000LL;               \
		if (!hw->offset_loaded)                                  \
			last_counter = lwrrent_counter;                  \
		counter = lwrrent_counter - last_counter;                \
...
static int
txgbe_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
	uint32_t fctrl;
...
static int
txgbe_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
	uint32_t fctrl;
...
uint32_t
txgbe_colwert_vm_rx_mask_to_val(uint16_t rx_mask, uint32_t orig_val)
{
...
{
	struct txgbe_hw *hw = TXGBE_DEV_HW(dev);
	uint32_t status = TXGBE_ERR_PHY_ADDR_ILWALID;
...
static const struct eth_dev_ops txgbe_eth_dev_ops = {
	...
	.promislwous_enable         = txgbe_dev_promislwous_enable,
	.promislwous_disable        = txgbe_dev_promislwous_disable,
