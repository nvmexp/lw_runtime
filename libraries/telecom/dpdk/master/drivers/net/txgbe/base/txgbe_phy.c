...
static bool txgbe_identify_extphy(struct txgbe_hw *hw)
{
	...
	if (hw->phy.type == txgbe_phy_unknown) {
		...
		if (ext_ability & (TXGBE_MD_PHY_10GBASET_ABILITY |
			...
			hw->phy.type = txgbe_phy_lw_unknown;
		else
...
{
	s32 err = TXGBE_ERR_PHY_ADDR_ILWALID;
...
ERR_I2C:
	...
	if (err != 0)
	  ...
	  * 9   SFP_1g_lw_CORE0 - chip-specific
	  * 10  SFP_1g_lw_CORE1 - chip-specific
	  * 11  SFP_1g_sx_CORE0 - chip-specific
	  * 12  SFP_1g_sx_CORE1 - chip-specific
	  */
	if (cable_tech & TXGBE_SFF_CABLE_DA_PASSIVE) {
		if (hw->bus.lan_id == 0)
			hw->phy.sfp_type = txgbe_sfp_type_da_lw_core0;
		else
			hw->phy.sfp_type = txgbe_sfp_type_da_lw_core1;
		...
		hw->phy.sfp_type = (hw->bus.lan_id == 0
				? txgbe_sfp_type_1g_lw_core0
				: txgbe_sfp_type_1g_lw_core1);
	...
	if (comp_codes_10g == 0 &&
	    !(hw->phy.sfp_type == txgbe_sfp_type_1g_lw_core1 ||
	      hw->phy.sfp_type == txgbe_sfp_type_1g_lw_core0 ||
			       ...
			       u32 speed)
{
	...
	if (speed == TXGBE_LINK_SPEED_10GB_FULL) {
		...
		wr32_epcs(hw, TXGBE_PHY_TX_EQ_CTL1, value);
		if (hw->phy.sfp_type == txgbe_sfp_type_da_lw_core0 ||
			hw->phy.sfp_type == txgbe_sfp_type_da_lw_core1) {
