...
/**
 *  txgbe_callwlate_checksum - Callwlate checksum for buffer
 ...
 **/
static u8
txgbe_callwlate_checksum(u8 *buffer, u32 length)
{
	u32 i;
	u8 sum = 0;
...
s32 txgbe_hic_sr_read(struct txgbe_hw *hw, u32 addr, u8 *buf, int len)
{
	...
	while (i < (len >> 2)) {
		value = rd32a(hw, TXGBE_MNGMBX, FW_LWM_DATA_OFFSET + i);
		((u32 *)buf)[i] = value;
		i++;
	}
...
s32 txgbe_hic_sr_write(struct txgbe_hw *hw, u32 addr, u8 *buf, int len)
{
	...
	while (i < (len >> 2)) {
		value = ((u32 *)buf)[i];
		wr32a(hw, TXGBE_MNGMBX, FW_LWM_DATA_OFFSET + i, value);
		i++;
	}
	...
	wr32a(hw, TXGBE_MNGMBX, FW_LWM_DATA_OFFSET + (i >> 2), value);
				 ...
				 const char *driver_ver)
{
	...
	fw_cmd.pad = 0;
	fw_cmd.pad2 = 0;
	fw_cmd.hdr.checksum = txgbe_callwlate_checksum((u8 *)&fw_cmd,
...
txgbe_hic_reset(struct txgbe_hw *hw)
{
	...
	reset_cmd.hdr.checksum = txgbe_callwlate_checksum((u8 *)&reset_cmd,
