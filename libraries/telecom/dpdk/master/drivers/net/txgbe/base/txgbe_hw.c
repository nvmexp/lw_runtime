...
s32 txgbe_setup_fc(struct txgbe_hw *hw)
{
	...
	if (hw->fc.strict_ieee && hw->fc.requested_mode == txgbe_fc_rx_pause) {
		...
		err = TXGBE_ERR_ILWALID_LINK_SETTINGS;
		goto out;
	}
	...
	case txgbe_fc_none:
		...
		if (hw->phy.media_type == txgbe_media_type_backplane)
			...
			reg_lw &= ~(TXGBE_TAF_SYM_PAUSE | TXGBE_TAF_ASM_PAUSE);
		break;
	case txgbe_fc_tx_pause:
		...
		if (hw->phy.media_type == txgbe_media_type_backplane) {
			...
			reg_lw |= TXGBE_TAF_ASM_PAUSE;
			reg_lw &= ~TXGBE_TAF_SYM_PAUSE;
		}
		reg |= SR_MII_MMD_AN_ADV_PAUSE_ASM;
...
s32 txgbe_validate_mac_addr(u8 *mac_addr)
{
	...
	if (TXGBE_IS_MULTICAST(mac_addr)) {
		status = TXGBE_ERR_ILWALID_MAC_ADDR;
			  ...
			  u32 enable_addr)
{
	...
	if (index >= rar_entries) {
		...
		return TXGBE_ERR_ILWALID_ARGUMENT;
	}
...
s32 txgbe_fc_enable(struct txgbe_hw *hw)
{
	...
	for (i = 0; i < TXGBE_DCB_TC_MAX; i++) {
		if ((hw->fc.lwrrent_mode & txgbe_fc_tx_pause) &&
		    hw->fc.high_water[i]) {
		       ...
		       u32 adv_sym, u32 adv_asm, u32 lp_sym, u32 lp_asm)
{
	...
	if ((adv_reg & adv_sym) && (lp_reg & lp_sym)) {
		...
		if (hw->fc.requested_mode == txgbe_fc_full) {
			...
			hw->fc.lwrrent_mode = txgbe_fc_rx_pause;
			DEBUGOUT("Flow Control=RX PAUSE frames only\n");
		}
	} else if (!(adv_reg & adv_sym) && (adv_reg & adv_asm) &&
		   (lp_reg & lp_sym) && (lp_reg & lp_asm)) {
		hw->fc.lwrrent_mode = txgbe_fc_tx_pause;
		...
		hw->fc.lwrrent_mode = txgbe_fc_none;
				      ...
				      bool *autoneg)
{
	...
	if (hw->phy.sfp_type == txgbe_sfp_type_1g_lw_core0 ||
	    hw->phy.sfp_type == txgbe_sfp_type_1g_lw_core1 ||
...
s32 txgbe_reset_pipeline_raptor(struct txgbe_hw *hw)
{
	...
	/* Enable link if disabled in LWM */
