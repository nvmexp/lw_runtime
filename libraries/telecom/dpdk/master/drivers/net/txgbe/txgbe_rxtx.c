...
txgbe_get_tun_len(struct rte_mbuf *mbuf)
{
	...
	case PKT_TX_TUNNEL_VXLAN_GPE:
		...
		break;
	case PKT_TX_TUNNEL_GRE:
		tun_len = sizeof(struct txgbe_lwgrehdr);
		break;
	case PKT_TX_TUNNEL_GENEVE:
	     ...
	     uint16_t nb_pkts)
{
	...
	if (rxq->rx_tail > rxq->rx_free_trigger) {
		...
		/* update tail pointer */
		rte_wmb();
		txgbe_set32_relaxed(rxq->rdt_reg_addr, lwr_free_trigger);
	}
			...
			struct txgbe_dcb_config *dcb_config)
{
	...
	/* colwert rte_eth_conf.rx_adv_conf to struct txgbe_dcb_config */
			...
			struct txgbe_dcb_config *dcb_config)
{
	...
	txgbe_dcb_callwlate_tc_credits_cee(hw, dcb_config, max_frame,
				TXGBE_DCB_TX_CONFIG);
	txgbe_dcb_callwlate_tc_credits_cee(hw, dcb_config, max_frame,
				TXGBE_DCB_RX_CONFIG);
...
txgbe_vmdq_rx_hw_configure(struct rte_eth_dev *dev)
{
	...
	for (i = 0; i < (int)num_pools; i++) {
		vmolr = txgbe_colwert_vm_rx_mask_to_val(cfg->rx_mode, vmolr);
