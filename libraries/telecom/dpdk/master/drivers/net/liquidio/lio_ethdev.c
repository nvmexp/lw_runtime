...
static int
lio_dev_promislwous_enable(struct rte_eth_dev *eth_dev)
{
	struct lio_device *lio_dev = LIO_DEV(eth_dev);
	...
	if (!lio_dev->intf_open) {
		lio_dev_err(lio_dev, "Port %d down, can't enable promislwous\n",
			    lio_dev->port_id);
		return -EAGAIN;
	}
...
static int
lio_dev_promislwous_disable(struct rte_eth_dev *eth_dev)
{
	struct lio_device *lio_dev = LIO_DEV(eth_dev);
	...
	if (!lio_dev->intf_open) {
		lio_dev_err(lio_dev, "Port %d down, can't disable promislwous\n",
			    lio_dev->port_id);
		return -EAGAIN;
	}
...
/* This function will be ilwoked every LSC_TIMEOUT ns (100ms)
...
/**
 * Reset and stop the device. This oclwrs on the first
...
/**
 * Enable checksum callwlation for inner packet in a tunnel.
 */
static void
...
static const struct eth_dev_ops liovf_eth_dev_ops = {
	...
	.promislwous_enable	= lio_dev_promislwous_enable,
	.promislwous_disable	= lio_dev_promislwous_disable,
