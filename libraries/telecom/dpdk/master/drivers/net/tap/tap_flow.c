...
#define REMOTE_PROMISLWOUS_HANDLE 2
...
struct tap_flow_items {
	...
	/**
	 * Colwersion function from rte_flow to netlink attributes.
	 *
	 * @param item
	 *   rte_flow item to colwert.
	 * @param data
	 *   Internal structure to store the colwersion.
	 *
	 * @return
	 *   0 on success, negative value otherwise.
	 */
	int (*colwert)(const struct rte_flow_item *item, void *data);
...
{
	struct colwert_data *info = (struct colwert_data *)data;
...
priv_flow_process(struct pmd_internals *pmd,
		  ...
		  int mirred)
{
	const struct tap_flow_items *lwr_item = tap_flow_items;
	struct colwert_data data = {
		.eth_type = 0,
		.ip_proto = 0,
		.flow = flow,
	};
	...
	if (attr->group > MAX_GROUP) {
		rte_flow_error_set(
			error, EILWAL, RTE_FLOW_ERROR_TYPE_ATTR_GROUP,
			NULL, "group value too big: cannot exceed 15");
		return -rte_errno;
	}
	if (attr->priority > MAX_PRIORITY) {
		rte_flow_error_set(
			error, EILWAL, RTE_FLOW_ERROR_TYPE_ATTR_PRIORITY,
	...
	for (; items->type != RTE_FLOW_ITEM_TYPE_END; ++items) {
		...
		for (i = 0;
		     lwr_item->items &&
		     lwr_item->items[i] != RTE_FLOW_ITEM_TYPE_END;
		     ++i) {
			if (lwr_item->items[i] == items->type) {
		...
		err = tap_flow_item_validate(
			items, lwr_item->mask_sz,
			(const uint8_t *)lwr_item->mask,
			(const uint8_t *)lwr_item->default_mask);
		if (err)
			goto exit_item_not_supported;
		if (flow && lwr_item->colwert) {
			err = lwr_item->colwert(items, &data);
			if (err)
				goto exit_item_not_supported;
		}
	}
			     ...
			     enum implicit_rule_index idx)
{
	 ...
	 * disabled (PASSTHRU).
	 * There is just one REMOTE_PROMISLWOUS rule in all cases. It should
	 ...
	 * same REMOTE_PROMISLWOUS rules if they had different handles.
	 */
	if (idx == TAP_ISOLATE)
		remote_flow->msg.t.tcm_handle = ISOLATE_HANDLE;
	else if (idx == TAP_REMOTE_PROMISC)
		remote_flow->msg.t.tcm_handle = REMOTE_PROMISLWOUS_HANDLE;
	else
		tap_flow_set_handle(remote_flow);
			      ...
			      enum implicit_rule_index idx)
{
	...
	for (remote_flow = LIST_FIRST(&pmd->implicit_flows);
		...
		lwr_prio = (remote_flow->msg.t.tcm_info >> 16) & PRIORITY_MASK;
		if (lwr_prio != idx_prio)
			continue;
			...
			struct rte_flow_error *error)
{
	...
	if (err < 0) {
		rte_flow_error_set(
			error, EILWAL, RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
...
static int bpf_rss_key(enum bpf_rss_key_e cmd, __u32 *key_idx)
{
	...
	case KEY_CMD_RELEASE:
		 ...
		 * entry 0 - the offset subtraction will callwlate the real
...
/**
 * Add RSS hash callwlations and queue selection
 *
 * @param[in, out] pmd
			   ...
			   struct rte_flow_error *error)
{
	...
	/*
	 * Load bpf rules to callwlate hash for this key_idx
	 */
