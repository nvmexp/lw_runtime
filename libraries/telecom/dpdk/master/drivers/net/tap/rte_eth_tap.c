...
/* Finalize l4 checksum callwlation */
static void
...
pmd_tx_burst(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts)
{
	...
	for (i = 0; i < nb_pkts; i++) {
		...
		if (tso) {
			...
			/* gso size is callwlated without RTE_ETHER_CRC_LEN */
		...
		struct rte_eth_rss_conf *rss_conf)
{
	...
	if (rss_conf->rss_key && rss_conf->rss_key_len) {
		/*
		 * Lwrrently TAP RSS key is hard coded
...
static const struct eth_dev_ops ops = {
	...
	.promislwous_enable     = tap_promisc_enable,
	.promislwous_disable    = tap_promisc_disable,
