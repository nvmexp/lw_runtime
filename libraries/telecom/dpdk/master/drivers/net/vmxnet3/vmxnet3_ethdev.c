...
static int vmxnet3_dev_promislwous_enable(struct rte_eth_dev *dev);
static int vmxnet3_dev_promislwous_disable(struct rte_eth_dev *dev);
...
static const struct eth_dev_ops vmxnet3_eth_dev_ops = {
	...
	.promislwous_enable   = vmxnet3_dev_promislwous_enable,
	.promislwous_disable  = vmxnet3_dev_promislwous_disable,
...
/* Promislwous supported only if Vmxnet3_DriverShared is initialized in adapter */
static int
vmxnet3_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct vmxnet3_hw *hw = dev->data->dev_private;
...
/* Promislwous supported only if Vmxnet3_DriverShared is initialized in adapter */
static int
vmxnet3_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct vmxnet3_hw *hw = dev->data->dev_private;
	uint32_t *vf_table = hw->shared->devRead.rxFilterConf.vfTable;
	uint64_t rx_offloads = dev->data->dev_conf.rxmode.offloads;
...
vmxnet3_dev_vlan_filter_set(struct rte_eth_dev *dev, uint16_t vid, int on)
{
	...
	/* don't change active filter if in promislwous mode */
