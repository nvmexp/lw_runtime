...
enum {
	...
	RAMROD_EXEC,
	/* Don't add a new command and continue exelwtion of posponed
...
typedef int (*exe_q_optimize)(struct bnx2x_softc *sc,
			      union ecore_qable_obj *o,
			      struct ecore_exeq_elem *elem);
typedef int (*exe_q_exelwte)(struct bnx2x_softc *sc,
			     union ecore_qable_obj *o,
...
struct ecore_exe_queue_obj {
	/* Commands pending for an exelwtion. */
	...
	/* Maximum length of commands' list for one exelwtion */
	int			exe_chunk_len;
	...
	/****** Virtual functions ******/
	/**
	 * Called before commands exelwtion for commands that are really
	 * going to be exelwted (after 'optimize').
	 *
...
struct ecore_vlan_mac_obj {
	...
	bool		head_exe_request; /* Pending exelwtion request. */
	uint32_t	saved_ramrod_flags; /* Ramrods of pending exelwtion */

	/* Exelwtion queue interface instance */
	...
	*  exelwtion commands. Will schedule all all lwrrently
	...
	*  ramrod_flags for the last DEL operation.
	 ...
	 *		       pending commands will be exelwted.
	 ...
	 *         pending for exelwtion or for completion commands.
...
struct ecore_mcast_obj {
	...
	/* Total number of lwrrently pending MACs to configure: both
...
struct ecore_credit_pool_obj {
	 ...
	 *
	 * Lwrrently the maximum allowed size is MAX_MAC_CREDIT_E2(272)
	 *
...
struct ecore_queue_sp_obj {
	 ...
	 * more colwenient to have different bits for different
	 * commands.
	 */
	uint32_t	pending;
