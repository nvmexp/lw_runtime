...
bnx2x_stats_pmf_update(struct bnx2x_softc *sc)
{
	...
	if (sc->devinfo.bc_ver <= 0x06001400) {
		/*
		 * Bootcode v6.0.21 fixed a GRC timeout that oclwrs when accessing
	...
	dmae = BNX2X_SP(sc, dmae[sc->exelwter_idx++]);
...
bnx2x_storm_stats_validate_counters(struct bnx2x_softc *sc)
{
    ...
    uint16_t lwr_stats_counter;

    /*
     * Make sure we use the value of the counter
     * used for sending the last stats ramrod.
     */
    lwr_stats_counter = (sc->stats_counter - 1);

    /* are storm stats valid? */
    if (le16toh(counters->xstats_counter) != lwr_stats_counter) {
	PMD_DRV_LOG(DEBUG, sc,
    ...
    if (le16toh(counters->ustats_counter) != lwr_stats_counter) {
	PMD_DRV_LOG(DEBUG, sc,
    ...
    if (le16toh(counters->cstats_counter) != lwr_stats_counter) {
	PMD_DRV_LOG(DEBUG, sc,
    ...
    if (le16toh(counters->tstats_counter) != lwr_stats_counter) {
	PMD_DRV_LOG(DEBUG, sc,
...
bnx2x_prep_fw_stats_req(struct bnx2x_softc *sc)
{
    ...
    rte_iova_t lwr_data_offset;
    struct stats_query_entry *lwr_query_entry;
     ...
     */
    lwr_data_offset = (sc->fw_stats_data_mapping +
		       offsetof(struct bnx2x_fw_stats_data, storm_counters));

    stats_hdr->stats_counters_addrs.hi = htole32(U64_HI(lwr_data_offset));
    stats_hdr->stats_counters_addrs.lo = htole32(U64_LO(lwr_data_offset));
    ...
    lwr_query_entry = &sc->fw_stats_req->query[BNX2X_PORT_QUERY_IDX];

    lwr_query_entry->kind = STATS_TYPE_PORT;
    /* For port query index is a DON'T CARE */
    lwr_query_entry->index = SC_PORT(sc);
    /* For port query funcID is a DON'T CARE */
    lwr_query_entry->funcID = htole16(SC_FUNC(sc));
    lwr_query_entry->address.hi = htole32(U64_HI(lwr_data_offset));
    lwr_query_entry->address.lo = htole32(U64_LO(lwr_data_offset));

    /**** PF FW statistics data ****/
    lwr_data_offset = (sc->fw_stats_data_mapping +
		       offsetof(struct bnx2x_fw_stats_data, pf));

    lwr_query_entry = &sc->fw_stats_req->query[BNX2X_PF_QUERY_IDX];

    lwr_query_entry->kind = STATS_TYPE_PF;
	...
	lwr_query_entry->kind = STATS_TYPE_QUEUE;
	lwr_query_entry->index = bnx2x_stats_id(&sc->fp[i]);
	lwr_query_entry->funcID = htole16(SC_FUNC(sc));
	lwr_query_entry->address.hi = htole32(U64_HI(lwr_data_offset));
	lwr_query_entry->address.lo = htole32(U64_LO(lwr_data_offset));

	lwr_data_offset += sizeof(struct per_queue_stats);
    }
}
