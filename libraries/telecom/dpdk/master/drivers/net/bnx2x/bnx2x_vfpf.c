...
/* callwlate the crc in the bulletin board */
...
bnx2x_check_bull(struct bnx2x_softc *sc)
{
	...
	if (old_version == bull->version) {
		...
		if (tries == BNX2X_VF_BULLETIN_TRIES) {
			PMD_DRV_LOG(ERR, sc, "pf to vf bulletin board crc was wrong %d conselwtive times. Aborting",
					tries);
			return FALSE;
		}
	}
...
int bnx2x_vf_get_resources(struct bnx2x_softc *sc, uint8_t tx_count, uint8_t rx_count)
{
	...
	if (rte_is_valid_assigned_ether_addr(&sc_resp.resc.lwrrent_mac_addr))
		rte_ether_addr_copy(&sc_resp.resc.lwrrent_mac_addr,
