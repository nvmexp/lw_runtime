...
static inline void ecore_map_q_cos(struct bnx2x_softc *sc, uint32_t q_num, uint32_t new_cos)
{
	...
	uint32_t lwrr_cos = REG_RD(sc, QM_REG_QVOQIDX_0 + q_num * 4);
	...
	if (lwrr_cos != new_cos) {
		...
		if (INIT_MODE_FLAGS(sc) & MODE_PORT4) {
			...
			if (SC_PORT(sc)) {
				lwrr_cos += ECORE_E3B0_PORT1_COS_OFFSET;
		...
		for (vnic = 0; vnic < num_vnics; vnic++) {
			...
			reg_addr = ECORE_VOQ_Q_REG_ADDR(lwrr_cos, pf_q_num);
...
static inline void ecore_clear_blocks_parity(struct bnx2x_softc *sc)
{
	...
	reg_val = REG_RD(sc, MISC_REG_AEU_AFTER_ILWERT_4_MCP);
