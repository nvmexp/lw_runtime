...
static int elink_check_lfa(struct elink_params *params)
{
	...
	uint32_t lwr_speed_cap_mask, lwr_req_fc_auto_adv, additional_config;
	...
	*/
	...
	for (cfg_idx = 0; cfg_idx < cfg_size; cfg_idx++) {
		lwr_speed_cap_mask = REG_RD(sc, params->lfa_base +
					    offsetof(struct shmem_lfa,
						     speed_cap_mask[cfg_idx]));

		if (lwr_speed_cap_mask != params->speed_cap_mask[cfg_idx]) {
	...
	if ((uint16_t)lwr_req_fc_auto_adv != params->req_fc_auto_adv) {
		ELINK_DEBUG_P2(sc, "Flow Ctrl AN mismatch %x vs. %x",
			       lwr_req_fc_auto_adv, params->req_fc_auto_adv);
		return LFA_FLOW_CTRL_MISMATCH;
	}
...
static void elink_ets_e2e3a0_disabled(struct elink_params *params)
{
	...
	/* For strict priority entries defines the number of conselwtive
	...
	/* Defines the number of conselwtive slots for the strict priority */
				     ...
				     const uint8_t port)
{
	...
	/* Callwlate and set BW for this COS - use 1 instead of 0 for BW */
...
/******************************************************************************
 * Description:
 *	Callwlate the total BW.A value of 0 isn't legal.
 *
 ******************************************************************************/
...
/******************************************************************************
 * Description:
 *	Ilwalidate all the sp_pri_to_cos.
 *
 ******************************************************************************/
static void elink_ets_e3b0_sp_pri_to_cos_init(uint8_t *sp_pri_to_cos)
{
	...
	for (pri = 0; pri < ELINK_DCBX_MAX_NUM_COS; pri++)
		sp_pri_to_cos[pri] = DCBX_ILWALID_COS;
}
/******************************************************************************
 * Description:
 *	Callwlate and set the SP (ARB_PRIORITY_CLIENT) NIG and PBF registers
					    ...
					    const uint8_t cos_entry)
{
	...
	if (sp_pri_to_cos[pri] != DCBX_ILWALID_COS) {
		ELINK_DEBUG_P0(sc, "elink_ets_e3b0_sp_pri_to_cos_set invalid "
					     ...
					     uint8_t *sp_pri_to_cos)
{
	...
	for (i = 0; i < max_num_of_cos; i++) {
		if (sp_pri_to_cos[i] != DCBX_ILWALID_COS) {
				...
				const uint8_t strict_cos)
{
	...
	/* For strict priority entries defines the number of conselwtive slots
...
static void elink_set_mdio_clk(struct bnx2x_softc *sc,
			       ...
			       uint32_t emac_base)
{
	uint32_t new_mode, lwr_mode;
	uint32_t clc_cnt;
	/* Set clause 45 mode, slow down the MDIO clock to 2.5MHz
	 * (a value of 49==0x31) and make sure that the AUTO poll is off
	 */
	lwr_mode = REG_RD(sc, emac_base + EMAC_REG_EMAC_MDIO_MODE);
	...
	if (((lwr_mode & EMAC_MDIO_MODE_CLOCK_CNT) == clc_cnt) &&
	    (lwr_mode & (EMAC_MDIO_MODE_CLAUSE_45)))
		return;
			     ...
			     struct elink_vars *vars, uint8_t lb)
{
	...
	/* KEEP_VLAN_TAG, promislwous */
	val = REG_RD(sc, emac_base + EMAC_REG_EMAC_RX_MODE);
	val |= EMAC_RX_MODE_KEEP_VLAN_TAG | EMAC_RX_MODE_PROMISLWOUS;
 ...
 * by lwram configuration. This parameter, whether this is the
 * default phy configuration, or the lwram overrun
...
static elink_status_t elink_eee_lwram_to_time(uint32_t lwram_mode,
					      uint32_t *idle_timer)
{
	switch (lwram_mode) {
	case PORT_FEAT_CFG_EEE_POWER_MODE_BALANCED:
		*idle_timer = ELINK_EEE_MODE_LWRAM_BALANCED_TIME;
		break;
	case PORT_FEAT_CFG_EEE_POWER_MODE_AGGRESSIVE:
		*idle_timer = ELINK_EEE_MODE_LWRAM_AGGRESSIVE_TIME;
		break;
	case PORT_FEAT_CFG_EEE_POWER_MODE_LOW_LATENCY:
		*idle_timer = ELINK_EEE_MODE_LWRAM_LATENCY_TIME;
		break;
	default:
		*idle_timer = 0;
		break;
...
static elink_status_t elink_eee_time_to_lwram(uint32_t idle_timer,
					      uint32_t *lwram_mode)
{
	switch (idle_timer) {
	case ELINK_EEE_MODE_LWRAM_BALANCED_TIME:
		*lwram_mode = PORT_FEAT_CFG_EEE_POWER_MODE_BALANCED;
		break;
	case ELINK_EEE_MODE_LWRAM_AGGRESSIVE_TIME:
		*lwram_mode = PORT_FEAT_CFG_EEE_POWER_MODE_AGGRESSIVE;
		break;
	case ELINK_EEE_MODE_LWRAM_LATENCY_TIME:
		*lwram_mode = PORT_FEAT_CFG_EEE_POWER_MODE_LOW_LATENCY;
		break;
	default:
		*lwram_mode = PORT_FEAT_CFG_EEE_POWER_MODE_DISABLED;
		break;
	}
...
static uint32_t elink_eee_calc_timer(struct elink_params *params)
{
	...
	if (params->eee_mode & ELINK_EEE_MODE_OVERRIDE_LWRAM) {
		if (params->eee_mode & ELINK_EEE_MODE_OUTPUT_TIME) {
			/* time value in eee_mode --> used directly*/
			eee_idle = params->eee_mode & ELINK_EEE_MODE_TIMER_MASK;
		} else {
			/* hsi value in eee_mode --> time */
			if (elink_eee_lwram_to_time(params->eee_mode &
						    ELINK_EEE_MODE_LWRAM_MASK,
						    &eee_idle))
				return 0;
		}
	} else {
		/* hsi values in lwram --> time*/
		...
		if (elink_eee_lwram_to_time(eee_mode, &eee_idle))
			return 0;
	}
				   ...
				   struct elink_vars *vars)
{
	...
	if (eee_idle) {
		REG_WR(sc, MISC_REG_CPMU_LP_IDLE_THR_P0 + (params->port << 2),
		   ...
		   (params->eee_mode & ELINK_EEE_MODE_OVERRIDE_LWRAM) &&
	...
	if (params->eee_mode & ELINK_EEE_MODE_OUTPUT_TIME) {
		...
		if (elink_eee_time_to_lwram(eee_idle, &eee_mode))
			return ELINK_STATUS_ERROR;
				       ...
				       uint8_t is_xfi)
{
	...
	if (is_xfi) {
		...
		/* TAP values are controlled by lwram, if value there isn't 0 */
		if (tx_equal)
	...
	/* Enable fiber mode, enable and ilwert sig_det */
	elink_cl45_read_or_write(sc, phy, MDIO_WC_DEVAD,
			      ...
			      uint8_t enable_cl73)
{
	...
	reg_val &= ~(MDIO_SERDES_DIGITAL_A_1000X_CONTROL1_SIGNAL_DETECT_EN |
		    MDIO_SERDES_DIGITAL_A_1000X_CONTROL1_ILWERT_SIGNAL_DETECT);
					   ...
					   struct elink_vars *vars)
{
	...
	CL22_RD_OVER_CL45(sc, phy,
			  ...
			  &control1);
	control1 |= MDIO_SERDES_DIGITAL_A_1000X_CONTROL1_ILWERT_SIGNAL_DETECT;
				 ...
				 struct elink_vars *vars)
{
	...
	uint8_t ext_phy_link_up = 0, lwr_link_up;
	elink_status_t rc = ELINK_STATUS_OK;
	...
	for (phy_index = ELINK_EXT_PHY1; phy_index < params->num_phys;
		...
		lwr_link_up = phy->read_status(phy, params,
					       &phy_vars[phy_index]);
		if (lwr_link_up) {
				  ...
				  struct elink_vars *vars)
{
	 ...
	 * tap. Rest commands are exelwted after link is up
				 ...
				 struct elink_vars *vars)
{
	...
	if (link_up) {
		...
		if (params->lane_config &
			...
			/* Set bit 3 to ilwert Rx in 1G mode and clear this bit
					...
					uint16_t edc_mode)
{
	...
	if (edc_mode == ELINK_EDC_MODE_LIMITING) {
		 ...
		 */
		if (lwr_limiting_mode != ELINK_EDC_MODE_LIMITING)
			return ELINK_STATUS_OK;
	...
	if (!(phy->flags & ELINK_FLAGS_NOC) && !(rx_alarm_status & (1 << 5))) {
		...
		if ((val1 & (1 << 8)) == 0) {
			...
			ELINK_DEBUG_P1(sc,
			   ...
			   oc_port);
			elink_cb_event_log(sc, ELINK_LOG_ID_OVER_LWRRENT,
					   oc_port);
					   ...
					   uint16_t cmd_args[], int argc)
{
	 ...
	 * check again until the previous command finishes exelwtion and the
					       ...
					       struct elink_vars *vars)
{
	...
	elink_cl22_read(sc, phy,
			MDIO_REG_GPHY_SHADOW,
			&temp);
	temp |= MDIO_REG_GPHY_SHADOW_ILWERT_FIB_SD;
	elink_cl22_write(sc, phy,
			MDIO_REG_GPHY_SHADOW,
			MDIO_REG_GPHY_SHADOW_WR_ENA | temp);
				      ...
				      __rte_unused uint32_t chip_id)
{
	...
	/* Callwlate the port based on port swap */
	port ^= (swap_val && swap_override);
...
static void elink_check_over_lwrr(struct elink_params *params,
				  struct elink_vars *vars)
{
	struct bnx2x_softc *sc = params->sc;
	uint32_t cfg_pin;
	...
	cfg_pin = (REG_RD(sc, params->shmem_base +
		   ...
		   PORT_HW_CFG_E3_OVER_LWRRENT_MASK) >>
		PORT_HW_CFG_E3_OVER_LWRRENT_SHIFT;
	...
	if (!pin_val) {
		if ((vars->phy_flags & PHY_OVER_LWRRENT_FLAG) == 0) {
			elink_cb_event_log(sc, ELINK_LOG_ID_OVER_LWRRENT,
					   params->port);
					/* "Error:  Power fault on Port %d has"
					 *  " been detected and the power to "
					 ...
					 *  " error.",
					 */
			vars->phy_flags |= PHY_OVER_LWRRENT_FLAG;
			elink_warpcore_power_module(params, 0);
		}
	} else
		vars->phy_flags &= ~PHY_OVER_LWRRENT_FLAG;
...
/* Returns 0 if no change oclwrred since last check; 1 otherwise. */
 ...
 *	When such change oclwrs, it calls the elink_analyze_link_error
...
void elink_period_func(struct elink_params *params, struct elink_vars *vars)
{
	...
	if (CHIP_IS_E3(sc)) {
		...
		elink_check_over_lwrr(params, vars);
		if (vars->rx_tx_asic_rst)
