...
static int bnx2x_release_hw_lock(struct bnx2x_softc *sc, uint32_t resource)
{
	...
	/* validate the resource is lwrrently taken */
	lock_status = REG_RD(sc, hw_lock_control_reg);
	if (!(lock_status & resource_bit)) {
		PMD_DRV_LOG(NOTICE, sc,
 ...
 *   connection. (Not lwrrently used under FreeBSD until OS support becomes
 *   available.)
 *
 ...
 *   RSS connection that is being offloaded.  (Not lwrrently used under
 *   FreeBSD.)
 *
...
static uint8_t bnx2x_rxeof(struct bnx2x_softc *sc, struct bnx2x_fastpath *fp)
{
	...
	/*
	 * Memory barrier necessary as spelwlative reads of the rx
 ...
 * elwironments a pf from anoth VM may have already initialized the device
...
bnx2x_chk_parity_attn(struct bnx2x_softc *sc, uint8_t * global, uint8_t print)
{
	...
	attn.sig[0] = REG_RD(sc, MISC_REG_AEU_AFTER_ILWERT_1_FUNC_0 + port * 4);
	attn.sig[1] = REG_RD(sc, MISC_REG_AEU_AFTER_ILWERT_2_FUNC_0 + port * 4);
	attn.sig[2] = REG_RD(sc, MISC_REG_AEU_AFTER_ILWERT_3_FUNC_0 + port * 4);
	attn.sig[3] = REG_RD(sc, MISC_REG_AEU_AFTER_ILWERT_4_FUNC_0 + port * 4);

	if (!CHIP_IS_E1x(sc))
		attn.sig[4] =
		    REG_RD(sc, MISC_REG_AEU_AFTER_ILWERT_5_FUNC_0 + port * 4);
...
static void bnx2x_drv_info_ether_stat(struct bnx2x_softc *sc)
{
	...
	ether_stat->promislwous_mode = 0;	// (flags & PROMISC) ? 1 : 0;
...
static void bnx2x_handle_drv_info_req(struct bnx2x_softc *sc)
{
	...
	if ((drv_info_ctl & DRV_INFO_CONTROL_VER_MASK) != DRV_INFO_LWR_VER) {
...
static int bnx2x_mc_assert(struct bnx2x_softc *sc)
{
	...
	for (i = 0; i < STORM_ASSERT_ARRAY_SIZE; i++) {
		...
		if (row0 != COMMON_ASM_ILWALID_ASSERT_OPCODE) {
			PMD_DRV_LOG(ERR, sc,
...
/* want client ID same as IGU SB ID for non-E1 */
	...
	bnx2x_init_sb(sc, fp->sb_dma.paddr, BNX2X_VF_ID_ILWALID, FALSE,
...
static void bnx2x_init_internal_common(struct bnx2x_softc *sc)
{
	...
	/*
	 * Zero this manually as its initialization is lwrrently missing
	 * in the initTool.
	 */
...
/*
 * Callwlates the sum of vn_min_rates.
...
/* ATTN */
	 ...
	 * re-callwlated according to the actual link rate.
	 */
...
static void bnx2x_nic_init(struct bnx2x_softc *sc, int load_code)
{
	...
	bnx2x_attn_int_deasserted0(sc,
				 REG_RD(sc,
					(MISC_REG_AEU_AFTER_ILWERT_1_FUNC_0 +
					 SC_PORT(sc) * 4)) &
 ...
 * belongs to. Lwrrently only only 2 engines is supported.
 */
...
static int bnx2x_leader_reset(struct bnx2x_softc *sc)
{
	...
	if (bnx2x_process_kill(sc, global)) {
		PMD_DRV_LOG(NOTICE, sc, "Something bad oclwrred on engine %d!",
			    SC_PATH(sc));
		rc = -1;
		goto exit_leader_reset2;
	}
...
static int bnx2x_get_lwr_phy_idx(struct bnx2x_softc *sc)
{
...
{
	uint32_t sel_phy_idx = bnx2x_get_lwr_phy_idx(sc);
...
/* callwlate the current MAX line speed limit for the MF devices */
		if (IS_MF_SI(sc)) {
...
static void bnx2x_link_report_locked(struct bnx2x_softc *sc)
{
	struct bnx2x_link_report_data lwr_data;
	...
	bnx2x_fill_report_data(sc, &lwr_data);

	/* Don't report link down or exactly the same link status twice */
	if (!memcmp(&lwr_data, &sc->last_reported_link, sizeof(lwr_data)) ||
	    (rte_bit_relaxed_get32(BNX2X_LINK_REPORT_LINK_DOWN,
			  &sc->last_reported_link.link_report_flags) &&
	     rte_bit_relaxed_get32(BNX2X_LINK_REPORT_LINK_DOWN,
			  &lwr_data.link_report_flags))) {
		return;
	}

	ELINK_DEBUG_P2(sc, "Change in link status : lwr_data = %x, last_reported_link = %x",
		       lwr_data.link_report_flags,
		       sc->last_reported_link.link_report_flags);
	...
	rte_memcpy(&sc->last_reported_link, &lwr_data, sizeof(lwr_data));
 ...
 */
		...
		PMD_DRV_LOG(INFO, sc,
			    ...
			    lwr_data.line_speed, duplex, flow);
	}
}
...
int bnx2x_init(struct bnx2x_softc *sc)
{
	...
	/*
	 * If parity oclwrred during the unload, then attentions and/or
...
{
	uint32_t phy_idx = bnx2x_get_lwr_phy_idx(sc);
...
/* this should ONLY prevent upcoming shmem reads */
	...
	/* get the lwram size */
	val = REG_RD(sc, MCP_REG_MCPR_LWM_CFG4);
	sc->devinfo.flash_size =
	    (LWRAM_1MB_SIZE << (val & MCPR_LWM_CFG4_FLASH_SIZE));
...
/*
 * 57712+ - We lwrrently use one FW SB per IGU SB (Rx and Tx of
...
bnx2x_link_settings_supported(struct bnx2x_softc *sc, uint32_t switch_cfg)
{
	...
	if (!(sc->port.supported[0] || sc->port.supported[1])) {
		PMD_DRV_LOG(ERR, sc,
			    "Invalid phy config in LWRAM (PHY1=0x%08x PHY2=0x%08x)",
			    SHMEM_RD(sc,
	...
	for (idx = 0; idx < cfg_size; idx++) {
		...
		case PORT_FEATURE_LINK_SPEED_10M_FULL:
			    ...
			    port.supported[idx] & ELINK_SUPPORTED_10baseT_Full)
			{
				...
				PMD_DRV_LOG(ERR, sc,
					    "Invalid LWRAM config link_config=0x%08x "
...
static void bnx2x_get_phy_info(struct bnx2x_softc *sc)
{
	...
	/* configure link feature according to lwram value */
	eee_mode =
...
/*
* Previous driver DMAE transaction may have oclwrred when pre-boot stage
* ended and boot began. This would ilwalidate the addresses of the
...
static int bnx2x_prev_unload(struct bnx2x_softc *sc)
{
	...
	if (hw_lock_val) {
		if (hw_lock_val & HW_LOCK_RESOURCE_LWRAM) {
			PMD_DRV_LOG(DEBUG, sc, "Releasing previously held LWRAM lock\n");
			REG_WR(sc, MCP_REG_MCPR_LWM_SW_ARB,
			       (MCPR_LWM_SW_ARB_ARB_REQ_CLR1 << SC_PORT(sc)));
		}
...
bnx2x_dcbx_set_state(struct bnx2x_softc *sc, uint8_t dcb_on, uint32_t dcbx_enabled)
{
	if (!CHIP_IS_E1x(sc)) {
		...
		sc->dcb_state = FALSE;
		sc->dcbx_enabled = BNX2X_DCBX_ENABLED_ILWALID;
	}
	PMD_DRV_LOG(DEBUG, sc,
		    "DCB state [%s:%s]",
		    dcb_on ? "ON" : "OFF",
...
static void bnx2x_setup_fan_failure_detection(struct bnx2x_softc *sc)
{
	 ...
	 * the power consumption of the board is affected by the PHY. Lwrrently,
 ...
 * This oclwrs when a different function (func2,3) is being marked
 ...
 *          oclwrred while driver was down)
 ...
 *
 *      Notes:
 *      Lwrrently the PF error in the PGLC is non recoverable.
 *      In the future the there will be a recovery routine for this error.
 *      Lwrrently attention is masked.
 *      Having an MCP lock on the load/unload process does not guarantee that
 *      there is no Timer disable during Func6/7 enable. This is because the
 *      Timers scan is lwrrently being cleared by the MCP on FLR.
...
/* we lwrrently assume that a context is 1024 bytes */
		PMD_DRV_LOG(NOTICE, sc,
...
{
	uint32_t lwr_cnt = poll_count;
	uint32_t val;

	while ((val = REG_RD(sc, reg)) != expected && lwr_cnt--) {
...
bnx2x_pbf_pN_cmd_flushed(struct bnx2x_softc *sc, struct pbf_pN_cmd_regs *regs,
		       uint32_t poll_count)
{
	uint32_t oclwp, to_free, freed, freed_start;
	uint32_t lwr_cnt = poll_count;

	oclwp = to_free = REG_RD(sc, regs->lines_oclwp);
	...
	while (oclwp &&
		...
		if (lwr_cnt--) {
			DELAY(FLR_WAIT_INTERVAL);
			oclwp = REG_RD(sc, regs->lines_oclwp);
...
static void bnx2x_tx_hw_flushed(struct bnx2x_softc *sc, uint32_t poll_count)
{
	struct pbf_pN_cmd_regs cmd_regs[] = {
		{0, (CHIP_IS_E3B0(sc)) ?
		 PBF_REG_TQ_OCLWPANCY_Q0 : PBF_REG_P0_TQ_OCLWPANCY,
		 (CHIP_IS_E3B0(sc)) ?
		 PBF_REG_TQ_LINES_FREED_CNT_Q0 : PBF_REG_P0_TQ_LINES_FREED_CNT},
		{1, (CHIP_IS_E3B0(sc)) ?
		 PBF_REG_TQ_OCLWPANCY_Q1 : PBF_REG_P1_TQ_OCLWPANCY,
		 (CHIP_IS_E3B0(sc)) ?
		 PBF_REG_TQ_LINES_FREED_CNT_Q1 : PBF_REG_P1_TQ_LINES_FREED_CNT},
		{4, (CHIP_IS_E3B0(sc)) ?
		 PBF_REG_TQ_OCLWPANCY_LB_Q : PBF_REG_P4_TQ_OCLWPANCY,
		 (CHIP_IS_E3B0(sc)) ?
...
static void bnx2x_reset_port(struct bnx2x_softc *sc)
{
	...
	/* Check for BRB port oclwpancy */
	val = REG_RD(sc, BRB1_REG_PORT_NUM_OCC_BLOCKS_0 + port * 4);
	if (val) {
		PMD_DRV_LOG(DEBUG, sc,
			    "BRB1 is not empty, %d blocks are oclwpied", val);
	}
}
...
static int lwt_gzip_prefix(const uint8_t * zbuf, int len)
{
...
{
	int ret;
	int data_begin = lwt_gzip_prefix(zbuf, len);
