...
extern void elink_cb_download_progress(struct bnx2x_softc *sc, uint32_t lwr, uint32_t total);
...
typedef enum elink_log_id {
	...
	ELINK_LOG_ID_OVER_LWRRENT	= 1, /* uint8_t port */
...
#define ELINK_FEATURE_CONFIG_DISABLE_PD				(1 << 15)
	 ...
	 * 2'b00: Timer will be configured by lwram, output will be the value
	 *        from lwram.
	 * 2'b01: Timer will be configured by lwram, output will be in
	 *        microseconds.
	 * 2'b10: bits 1:0 contain an lwram value which will be used instead
	 *        of the one located in the lwram. Output will be that value.
	 * 2'b11: bits 19:0 contain the idle timer in microseconds; output
	 *        will be in microseconds.
	 * Bits 31:30 should be 2'b11 in order for EEE to be enabled.
	 */
	uint32_t eee_mode;
#define ELINK_EEE_MODE_LWRAM_BALANCED_TIME		(0xa00)
#define ELINK_EEE_MODE_LWRAM_AGGRESSIVE_TIME		(0x100)
#define ELINK_EEE_MODE_LWRAM_LATENCY_TIME		(0x6000)
#define ELINK_EEE_MODE_LWRAM_MASK		(0x3)
#define ELINK_EEE_MODE_TIMER_MASK		(0xfffff)
#define ELINK_EEE_MODE_OUTPUT_TIME		(1 << 28)
#define ELINK_EEE_MODE_OVERRIDE_LWRAM		(1 << 29)
...
#define PHY_OVER_LWRRENT_FLAG		(1 << 4)
