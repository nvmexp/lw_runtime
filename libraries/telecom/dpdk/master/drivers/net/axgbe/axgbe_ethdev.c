...
static int axgbe_dev_promislwous_enable(struct rte_eth_dev *dev);
static int axgbe_dev_promislwous_disable(struct rte_eth_dev *dev);
...
static const struct eth_dev_ops axgbe_eth_dev_ops = {
	...
	.promislwous_enable   = axgbe_dev_promislwous_enable,
	.promislwous_disable  = axgbe_dev_promislwous_disable,
...
static int
axgbe_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct axgbe_port *pdata = dev->data->dev_private;
...
static int
axgbe_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct axgbe_port *pdata = dev->data->dev_private;
				...
				struct timespec *timestamp, uint32_t flags)
{
	...
	idx = AXGBE_GET_DESC_IDX(rxq, rxq->lwr);
	desc = &rxq->desc[idx];
...
static void axgbe_set_counts(struct axgbe_port *pdata)
{
	...
	/* Callwlate the number of Tx and Rx rings to be created
