...
static enum axgbe_mode axgbe_lwr_mode(struct axgbe_port *pdata)
{
	return pdata->phy_if.phy_impl.lwr_mode(pdata);
}

static bool axgbe_in_kr_mode(struct axgbe_port *pdata)
{
	return axgbe_lwr_mode(pdata) == AXGBE_MODE_KR;
}
...
{
	enum axgbe_an lwr_state = pdata->an_state;
...
static void axgbe_phy_status(struct axgbe_port *pdata)
{
	...
	if (pdata->phy.link) {
		if (link_aneg && !axgbe_phy_aneg_done(pdata)) {
			if (axgbe_lwr_mode(pdata) == AXGBE_MODE_SGMII_1000) {
