				  ...
				  unsigned int queue)
{
	...
	/* Callwlate the status register to read and the position within */
			 ...
			 struct rte_mbuf *mbuf)
{
	...
	idx = AXGBE_GET_DESC_IDX(txq, txq->lwr);
	desc = &txq->desc[idx];
		...
		uint16_t nb_pkts)
{
	...
	nb_desc_free = txq->nb_desc - (txq->lwr - txq->dirty);
