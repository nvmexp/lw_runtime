...
/*
 * Lwrrently, raw pattern match is very limited. It is intended for matching
...
stacking_error:
	return rte_flow_error_set(error, EILWAL, RTE_FLOW_ERROR_TYPE_ITEM,
			 ...
			 struct rte_flow_error *error)
{
	...
	default:
		return rte_flow_error_set(error,
			EILWAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
...
{
	if (fm_flow->entry_handle != FM_ILWALID_HANDLE) {
		enic_fm_entry_free(fm, fm_flow->entry_handle);
		fm_flow->entry_handle = FM_ILWALID_HANDLE;
	}
...
static void
colwert_jump_flows(struct enic_flowman *fm, struct enic_fm_fet *fet,
		   struct rte_flow_error *error)
{
	struct enic_fm_flow *fm_flow;
	struct enic_fm_jump_flow *j;
	...
	while (j) {
		ENICPMD_LOG(DEBUG, "colwert jump flow: flow=%p group=%u",
			    j->flow, group);
		/* Delete old entry */
		fm_flow = j->flow->fm;
		...
		if (__enic_fm_flow_add_entry(fm, fm_flow, &j->match, fma,
			...
			ENICPMD_LOG(ERR, "cannot colwert jump flow: flow=%p",
				    j->flow);
		} else {
			fm_flow->fet = fet;
			fet->ref++;
			ENICPMD_LOG(DEBUG, "colwert ok: group=%u ref=%u",
				    fet->group, fet->ref);
		}
		    ...
		    struct rte_flow_error *error)
{
	...
	if (flow) {
		...
		if (fet && fet->default_key) {
			 ...
			 * so we can colwert this flow when that group
			 * materializes.
			 */
			...
			/*
			 * A new table is created. Colwert the saved flows
			 * that should jump to this group.
			 */
			colwert_jump_flows(fm, fet, error);
		}
	}
		   ...
		   __rte_unused struct rte_flow_error *error)
{
	...
	while (!LIST_EMPTY(&enic->flows)) {
		 ...
		 */
		if (fm->ig_tcam_hndl == FM_ILWALID_HANDLE) {
			fm_flow->entry_handle = FM_ILWALID_HANDLE;
			fm_flow->action = NULL;
			fm_flow->fet = NULL;
		}
...
{
	const struct rte_pci_addr *lwr_a, *prev_a;
	struct rte_eth_dev *dev;
	struct enic *lwr, *prev;
	uint16_t domain_id;
	uint64_t vnic_h;
	uint16_t pid;
	int ret;
	...
	if (enic_is_vf_rep(pf))
		return -EILWAL;
	lwr = pf;
	lwr_a = &RTE_ETH_DEV_TO_PCI(lwr->rte_dev)->addr;
	...
	lwr->switch_domain_id = domain_id;
