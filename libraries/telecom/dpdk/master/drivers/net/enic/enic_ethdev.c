		     ...
		     void *arg)
{
	...
	/*
	 * Lwrrently, when Geneve with options offload is enabled, host
...
static int enicpmd_dev_promislwous_enable(struct rte_eth_dev *eth_dev)
{
	struct enic *enic = pmd_priv(eth_dev);
	int ret;
...
static int enicpmd_dev_promislwous_disable(struct rte_eth_dev *eth_dev)
{
	struct enic *enic = pmd_priv(eth_dev);
	int ret;
...
static const struct eth_dev_ops enicpmd_eth_dev_ops = {
	...
	.promislwous_enable   = enicpmd_dev_promislwous_enable,
	.promislwous_disable  = enicpmd_dev_promislwous_disable,
	...
	struct rte_pci_device *pci_dev)
{
	...
	/* Callwlate max VF ID before initializing representor*/
	pf_enic->max_vf_id = 0;
