...
int enic_dev_stats_get(struct enic *enic, struct rte_eth_stats *r_stats)
{
	...
	/* The number of truncated packets can only be callwlated by
	 * subtracting a hardware counter from error packets received by
	 * the driver. Note: this causes transient inaclwracies in the
...
int enic_enable(struct enic *enic)
{
	...
	if (enic->enable_avx2_rx) {
		 ...
		 * It is lwrrently used by the vectorized handler.
		 */
		mb_def.nb_segs = 1;
...
static int enic_dev_init(struct enic *enic)
{
	...
	*/
	...
	if (!enic->disable_overlay && enic->vxlan &&
	    /* 'VXLAN feature' enables VXLAN, LWGRE, and GENEVE. */
	    vnic_dev_overlay_offload_ctrl(enic->vdev,
					  OVERLAY_FEATURE_VXLAN,
