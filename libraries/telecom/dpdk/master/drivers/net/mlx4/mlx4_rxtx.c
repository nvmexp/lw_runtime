...
/**
 * Obtain and callwlate TSO information needed for assembling a TSO WQE.
 *
 * @param buf
			     ...
			     struct tso_info *tinfo)
{
	...
	if (unlikely(buf->tso_segsz == 0 ||
		...
		return -EILWAL;
	/*
	 * Callwlate the WQE TSO segment size
	 * Note:
			   ...
			   volatile struct mlx4_wqe_ctrl_seg *ctrl)
{
	...
	tseg->mss_hdr_size = rte_cpu_to_be_32((buf->tso_segsz << 16) |
					      tinfo->tso_header_size);
	/* Callwlate data segment location */
...
mlx4_tx_burst(void *dpdk_txq, struct rte_mbuf **pkts, uint16_t pkts_n)
{
	...
	for (i = 0; (i != max); ++i) {
		 ...
		 * that no ICRC should be callwlated.
		 */
	...
	/* Take a shortlwt if nothing must be sent. */
	if (unlikely(i == 0))
		return 0;
...
mlx4_rx_burst(void *dpdk_rxq, struct rte_mbuf **pkts, uint16_t pkts_n)
{
	...
	while (pkts_n) {
		...
		if (!pkt) {
			...
			pkt->hash.rss = cqe->immed_rss_ilwalid;
			if (rxq->crc_present)
