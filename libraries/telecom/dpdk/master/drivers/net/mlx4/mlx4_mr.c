...
mlx4_mr_mem_event_free_cb(struct rte_eth_dev *dev, const void *addr, size_t len)
{
	...
	if (rebuild) {
		...
		/*
		 * Flush local caches by propagating ilwalidation across cores.
...
mlx4_mr_flush_local_cache(struct mlx4_mr_ctrl *mr_ctrl)
{
	...
	mr_ctrl->lwr_gen = *mr_ctrl->dev_gen_ptr;
	DEBUG("mr_ctrl(%p): flushed, lwr_gen=%d",
	      (void *)mr_ctrl, mr_ctrl->lwr_gen);
}
