...
/**
 * Colwert supported RSS hash field types between DPDK and Verbs formats.
 *
 ...
 * @param verbs_to_dpdk
 *   A zero value colwerts @p types from DPDK to Verbs, a nonzero value
 *   performs the reverse operation.
 *
 * @return
 *   Colwerted RSS hash fields on success, (uint64_t)-1 otherwise and
 *   rte_errno is set.
 */
uint64_t
mlx4_colw_rss_types(struct mlx4_priv *priv, uint64_t types, int verbs_to_dpdk)
{
	enum {
		INNER,
	...
	if ((verbs_to_dpdk || (colw & priv->hw_rss_sup) == colw) &&
	    !(types & ~seen))
		return colw;
	rte_errno = ENOTSUP;
	return (uint64_t)-1;
 ...
 *   criteria (i.e. rule is promislwous-like).
 *
 * @param[in, out] flow
		     ...
		     struct rte_flow_error *error)
{
	...
	if (!item->spec && (item->mask || item->last))
		return rte_flow_error_set
			(error, EILWAL, RTE_FLOW_ERROR_TYPE_ITEM, item,
		...
		case RTE_FLOW_ACTION_TYPE_RSS:
			...
			if (i != rss->queue_num) {
				msg = "mlx4 requires RSS contexts to use"
					" conselwtive queue indices only";
				goto exit_action_not_supported;
			}
			...
			rte_errno = 0;
			fields = mlx4_colw_rss_types(priv, rss->types, 0);
		 ...
		 struct rte_flow_error *error)
{
	...
	if (!err) {
		...
		if (!lwrr || !lwrr->internal) {
			LIST_INSERT_HEAD(&priv->flows, flow, next);
		} else {
			while (LIST_NEXT(lwrr, next) &&
			       LIST_NEXT(lwrr, next)->internal)
...
next_vlan:
	...
	/* Take care of promislwous and all multicast flow rules. */
	if (ETH_DEV(priv)->data->promislwous ||
