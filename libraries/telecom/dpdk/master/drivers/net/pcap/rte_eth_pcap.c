...
#if defined(RTE_EXEC_ELW_FREEBSD)
...
static inline void
callwlate_timestamp(struct timeval *ts) {
	uint64_t cycles;
	struct timeval lwr_time;

	cycles = rte_get_timer_cycles() - start_cycles;
	lwr_time.tv_sec = cycles / hz;
	lwr_time.tv_usec = (cycles % hz) * NSEC_PER_SEC / hz;

	ts->tv_sec = start_time.tv_sec + lwr_time.tv_sec;
	ts->tv_usec = start_time.tv_usec + lwr_time.tv_usec;
		...
		struct rte_eth_dev **eth_dev)
{
	...
	*internals = (*eth_dev)->data->dev_private;
	 ...
	 * where the middle 4 characters are colwerted to hex.
	 */
...
#elif defined(RTE_EXEC_ELW_FREEBSD)
	void *mac_addrs;
