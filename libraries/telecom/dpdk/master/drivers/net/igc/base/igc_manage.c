...
/**
 *  igc_callwlate_checksum - Callwlate checksum for buffer
 ...
 **/
u8 igc_callwlate_checksum(u8 *buffer, u32 length)
{
	u32 i;
	u8 sum = 0;

	DEBUGFUNC("igc_callwlate_checksum");
...
bool igc_enable_tx_pkt_filtering_generic(struct igc_hw *hw)
{
	...
	hdr->checksum = 0;
	csum = igc_callwlate_checksum((u8 *)hdr,
				      ...
				      struct igc_host_mng_command_header *hdr)
{
	...
	hdr->checksum = igc_callwlate_checksum((u8 *)hdr, length);
...
bool igc_enable_mng_pass_thru(struct igc_hw *hw)
{
	...
	if (hw->mac.has_fwsm) {
		...
		factps = IGC_READ_REG(hw, IGC_FACTPS);
		ret_val = igc_read_lwm(hw, LWM_INIT_CONTROL2_REG, 1, &data);
		if (ret_val)
			return false;

		if (!(factps & IGC_FACTPS_MNGCG) &&
		    ((data & IGC_LWM_INIT_CTRL2_MNGM) ==
		     (igc_mng_mode_pt << 13)))
			return true;
