...
/**
 *  igc_init_lwm_params - Initialize LWM function pointers
 *  @hw: pointer to the HW structure
 *
 *  This function initializes the function pointers for the LWM
 *  set of functions.  Called by drivers or by igc_setup_init_funcs.
 **/
s32 igc_init_lwm_params(struct igc_hw *hw)
{
	s32 ret_val = IGC_SUCCESS;
...
s32 igc_set_mac_type(struct igc_hw *hw)
{
	...
	case IGC_DEV_ID_I225_BLANK_LWM:
		mac->type = igc_i225;
		break;
...
s32 igc_setup_init_funcs(struct igc_hw *hw, bool init_device)
{
	...
	if (!(ret_val) && init_device) {
		...
		ret_val = igc_init_lwm_params(hw);
		if (ret_val)
			goto out;
...
/**
 *  igc_validate_lwm_checksum - Verifies LWM (EEPROM) checksum
 *  @hw: pointer to the HW structure
 *
 *  Validates the LWM checksum is correct. This is a function pointer entry
 *  point called by drivers.
 **/
s32 igc_validate_lwm_checksum(struct igc_hw *hw)
{
...
/**
 *  igc_update_lwm_checksum - Updates LWM (EEPROM) checksum
 *  @hw: pointer to the HW structure
 *
 *  Updates the LWM checksum. Lwrrently no func pointer exists and all
 *  implementations are handled in the generic version of this function.
 **/
s32 igc_update_lwm_checksum(struct igc_hw *hw)
{
...
/**
 *  igc_reload_lwm - Reloads EEPROM
 *  @hw: pointer to the HW structure
 *
 *  Reloads the EEPROM by setting the "Reinitialize from EEPROM" bit in the
 *  extended control register.
 **/
void igc_reload_lwm(struct igc_hw *hw)
{
 ...
 *  pointer entry point called by drivers.
 **/
s32 igc_read_lwm(struct igc_hw *hw, u16 offset, u16 words, u16 *data)
{
	if (hw->lwm.ops.read)
...
/**
 *  igc_write_lwm - Writes to LWM (EEPROM)
 *  @hw: pointer to the HW structure
 *  @offset: the word offset to read
 ...
 *  pointer entry point called by drivers.
 **/
s32 igc_write_lwm(struct igc_hw *hw, u16 offset, u16 words, u16 *data)
{
	if (hw->lwm.ops.write)
