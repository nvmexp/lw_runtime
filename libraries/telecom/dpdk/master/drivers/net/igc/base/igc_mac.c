...
s32 igc_check_alt_mac_addr_generic(struct igc_hw *hw)
{
	...
	if (hw->bus.func == IGC_FUNC_1)
		lwm_alt_mac_addr_offset += IGC_ALT_MAC_ADDRESS_OFFSET_LAN1;
	if (hw->bus.func == IGC_FUNC_2)
		lwm_alt_mac_addr_offset += IGC_ALT_MAC_ADDRESS_OFFSET_LAN2;

	if (hw->bus.func == IGC_FUNC_3)
		lwm_alt_mac_addr_offset += IGC_ALT_MAC_ADDRESS_OFFSET_LAN3;
...
s32 igc_set_fc_watermarks_generic(struct igc_hw *hw)
{
	 ...
	 */
	if (hw->fc.lwrrent_mode & igc_fc_tx_pause) {
	...
	if (hw->phy.media_type == igc_media_type_copper && mac->autoneg) {
		...
		if ((mii_nway_adv_reg & NWAY_AR_PAUSE) &&
			...
			if (hw->fc.requested_mode == igc_fc_full) {
				hw->fc.lwrrent_mode = igc_fc_full;
				DEBUGOUT("Flow Control = FULL.\n");
			} else {
				hw->fc.lwrrent_mode = igc_fc_rx_pause;
		 ...
		 */
			...
			hw->fc.lwrrent_mode = igc_fc_tx_pause;
		 ...
		 */
			 ...
			 * should be disabled.
			 */
			hw->fc.lwrrent_mode = igc_fc_none;
