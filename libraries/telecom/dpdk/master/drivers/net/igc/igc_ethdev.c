...
static int eth_igc_promislwous_enable(struct rte_eth_dev *dev);
static int eth_igc_promislwous_disable(struct rte_eth_dev *dev);
...
static const struct eth_dev_ops eth_igc_ops = {
	...
	.promislwous_enable	= eth_igc_promislwous_enable,
	.promislwous_disable	= eth_igc_promislwous_disable,
...
/*
 * It exelwtes link_update after knowing an interrupt is present.
 */
static void
...
eth_igc_close(struct rte_eth_dev *dev)
{
	...
	do {
		...
		if (ret >= 0 || ret == -ENOENT || ret == -EILWAL)
			break;
...
eth_igc_dev_init(struct rte_eth_dev *dev)
{
	...
	if (igc_validate_lwm_checksum(hw) < 0) {
		/*
...
static int
eth_igc_promislwous_enable(struct rte_eth_dev *dev)
{
	struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
	uint32_t rctl;
...
static int
eth_igc_promislwous_disable(struct rte_eth_dev *dev)
{
	struct igc_hw *hw = IGC_DEV_PRIVATE_HW(dev);
	uint32_t rctl;
