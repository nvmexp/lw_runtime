			 ...
			 struct rte_mempool *mp)
{
	...
	if (nb_desc % IGC_RX_DESCRIPTOR_MULTIPLE != 0 ||
		...
		PMD_DRV_LOG(ERR,
			"RX descriptor must be multiple of %u(lwr: %u) and between %u and %u",
...
what_advctx_update(struct igc_tx_queue *txq, uint64_t flags,
		union igc_tx_offload tx_offload)
{
	uint32_t lwrr = txq->ctx_lwrr;

	/* If match with the current context */
	if (likely(txq->ctx_cache[lwrr].flags == flags &&
		txq->ctx_cache[lwrr].tx_offload.data ==
		(txq->ctx_cache[lwrr].tx_offload_mask.data &
		tx_offload.data))) {
		return lwrr;
	}
		...
		uint64_t ol_flags, union igc_tx_offload tx_offload)
{
	...
	txq->ctx_lwrr ^= 1;
	ctx_lwrr = txq->ctx_lwrr;

	tx_offload_mask.data = 0;
	type_tucmd_mlhl = 0;

	/* Specify which HW CTX to upload. */
	mss_l4len_idx = (ctx_lwrr << IGC_ADVTXD_IDX_SHIFT);
	...
	txq->ctx_cache[ctx_lwrr].flags = ol_flags;
	txq->ctx_cache[ctx_lwrr].tx_offload.data =
		tx_offload_mask.data & tx_offload.data;
	txq->ctx_cache[ctx_lwrr].tx_offload_mask = tx_offload_mask;
...
igc_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts)
{
	...
	for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
		...
		if (tx_ol_req) {
			...
			olinfo_status |= (uint32_t)txq->ctx_lwrr <<
		...
		const struct rte_eth_txconf *tx_conf)
{
	...
	if (nb_desc % IGC_TX_DESCRIPTOR_MULTIPLE != 0 ||
		...
		PMD_DRV_LOG(ERR,
			"TX-descriptor must be a multiple of %u and between %u and %u, lwr: %u",
