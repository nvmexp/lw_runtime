			   ...
			   size_t dq_num, octeontx_pko_dq_getter_t getter)
{
	...
	memset(&lwrr, 0, sizeof(octeontx_dq_t));
	...
	lwrr.lmtline_va = ctl->pko[dq_vf].bar2;
	lwrr.ioreg_va = (void *)((uintptr_t)ctl->pko[dq_vf].bar0
		+ PKO_VF_DQ_OP_SEND((dq), 0));
	lwrr.fc_status_va = ctl->fc_ctl + dq_num;

	octeontx_log_dbg("lmtline=%p ioreg_va=%p fc_status_va=%p",
			 lwrr.lmtline_va, lwrr.ioreg_va,
			 lwrr.fc_status_va);

	getter(&lwrr, (void *)iter);
	return 0;
}
