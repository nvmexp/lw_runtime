...
octeontx_port_promisc_set(struct octeontx_nic *nic, int en)
{
	...
	if (res < 0) {
		octeontx_log_err("failed to set promislwous mode %d",
				nic->port_id);
		return res;
	}

	/* Set proper flag for the mode */
	dev->data->promislwous = (en != 0) ? 1 : 0;

	octeontx_log_dbg("port %d : promislwous mode %s",
...
static const struct eth_dev_ops octeontx_dev_ops = {
	...
	.promislwous_enable	 = octeontx_dev_promisc_enable,
	.promislwous_disable	 = octeontx_dev_promisc_disable,
