 ...
 * - A 16bit hash is callwlated and masked based on the number of entries
...
/**
 * The idx provided by the caller is within a region, so lwrrently the base is
 * either added or subtracted from the idx to ensure it can be used as a
 * compressed index
 */

/* Colwert the tcam index to a shadow index */
#define TF_SHADOW_TCAM_IDX_TO_SHIDX(ctxt, idx) ((idx) - \
						(ctxt)->shadow_ctxt.base_addr)

/* Colwert the shadow index to a tcam index */
