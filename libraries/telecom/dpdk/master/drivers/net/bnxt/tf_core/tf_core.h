 ...
 *
 * Colwert absolute offset to action record pointer in EEM record entry
 * Colwert action record pointer in EEM record entry to absolute offset
 */
...
#define TF_FW_SESSION_ID_ILWALID  0xFF  /**< Invalid FW Session ID define */
...
struct tf_session_info {
	 ...
	 * would come from the same version of an exelwtable.
...
struct tf_open_session_parms {
	 ...
	 * 'false'.
	 *
	 * Size of memory depends on the LWM Resource settings for the
	 * control channel.
	 */
	bool shadow_copy;
	/**
 ...
 * pool reserved for the session during tf_open().  No firmware is ilwolved.
 *
 ...
 * reserved for the session.  No firmware is ilwolved.  During tf_close, the
 ...
 * device constraints based upon callwlations using either the number of flows
 ...
 * session.  TCAM entry will be ilwalidated.  All-ones mask.
 * writes to hw.
 *
 * WCTCAM profile id of 0 must be used to ilwalidate an entry.
 *
 * Returns success or failure code.
 */
 ...
 * given table type.  Lwrrently, this is only used for collecting statistics.
 *
 * Returns success or failure code. Failure will be returned if the
 * provided data buffer is too small for the data type requested.
 */
