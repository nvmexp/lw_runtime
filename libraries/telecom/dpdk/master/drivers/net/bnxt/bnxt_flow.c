...
bnxt_flow_non_void_item(const struct rte_flow_item *lwr)
{
	while (1) {
		if (lwr->type != RTE_FLOW_ITEM_TYPE_VOID)
			return lwr;
		lwr++;
	}
}
...
bnxt_flow_non_void_action(const struct rte_flow_action *lwr)
{
	while (1) {
		if (lwr->type != RTE_FLOW_ACTION_TYPE_VOID)
			return lwr;
		lwr++;
	}
}
				  ...
				  struct bnxt_filter_info *filter)
{
	...
	while (item->type != RTE_FLOW_ITEM_TYPE_END) {
		...
		case RTE_FLOW_ITEM_TYPE_VLAN:
			...
			break;
		case RTE_FLOW_ITEM_TYPE_IPV4:
			/* If mask is not ilwolved, we could use EM filters. */
			ipv4_spec = item->spec;
			ipv4_mask = item->mask;
		...
		case RTE_FLOW_ITEM_TYPE_LWGRE:
			...
			if (!lwgre_spec && !lwgre_mask) {
				filter->tunnel_type =
				CFA_NTUPLE_FILTER_ALLOC_REQ_TUNNEL_TYPE_LWGRE;
				break;
			}

			if (lwgre_spec->c_k_s_rsvd0_ver != 0x2000 ||
			    lwgre_spec->protocol != 0x6558) {
				rte_flow_error_set(error,
						   EILWAL,
						   RTE_FLOW_ERROR_TYPE_ITEM,
						   item,
...
bnxt_find_matching_l2_filter(struct bnxt *bp, struct bnxt_filter_info *nf)
{
	...
	for (i = bp->max_vnics - 1; i >= 0; i--) {
		...
		if (vnic->fw_vnic_id == ILWALID_VNIC_ID)
			continue;
...
bnxt_flow_flush(struct rte_eth_dev *dev, struct rte_flow_error *error)
{
	...
	for (i = 0; i < bp->max_vnics; i++) {
		vnic = &bp->vnic_info[i];
		if (vnic && vnic->fw_vnic_id == ILWALID_VNIC_ID)
			continue;
