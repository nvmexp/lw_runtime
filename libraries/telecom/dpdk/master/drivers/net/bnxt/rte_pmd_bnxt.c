				...
				uint16_t rx_mask, uint8_t on)
{
	...
	if (rx_mask & ETH_VMDQ_ACCEPT_UNTAG) {
		PMD_DRV_LOG(ERR, "Lwrrently cannot toggle this setting\n");
		return -ENOTSUP;
	}
				...
				uint32_t vf_id)
{
	...
	/* If the VF lwrrently uses a random MAC, update default to this one */
		...
		uint16_t vlan_id)
{
	...
	if (bnxt_hwrm_func_qcfg_lwrrent_vf_vlan(bp, vf) ==
