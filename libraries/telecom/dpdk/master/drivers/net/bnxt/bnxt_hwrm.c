...
#define HWRM_CHECK_RESULT() do {\
	if (rc) { \
		...
		if (rc == HWRM_ERR_CODE_RESOURCE_ACCESS_DENIED) \
			...
			rc = -ENOSPC; \
		else if (rc == HWRM_ERR_CODE_ILWALID_PARAMS) \
			rc = -EILWAL; \
				 ...
				 struct bnxt_vlan_table_entry *vlan_table)
{
	...
	if (vnic->fw_vnic_id == ILWALID_HW_RING_ID)
		return rc;
	...
	if (vnic->flags & BNXT_VNIC_INFO_UNTAGGED)
		mask |= HWRM_CFA_L2_SET_RX_MASK_INPUT_MASK_VLAN_NOLWLAN;

	if (vnic->flags & BNXT_VNIC_INFO_PROMISC)
		mask |= HWRM_CFA_L2_SET_RX_MASK_INPUT_MASK_PROMISLWOUS;
	...
	if (vlan_table) {
		if (!(mask & HWRM_CFA_L2_SET_RX_MASK_INPUT_MASK_VLAN_NOLWLAN))
			 ...
			 uint16_t tx_cosq_id)
{
	...
	case HWRM_RING_ALLOC_INPUT_RING_TYPE_TX:
		...
		if (stats_ctx_id != ILWALID_STATS_CTX_ID)
			enables |=
...
int bnxt_hwrm_ring_grp_free(struct bnxt *bp, unsigned int idx)
{
	...
	bp->grp_info[idx].fw_grp_id = ILWALID_HW_RING_ID;
	return rc;
}
	...
	if (BNXT_CHIP_THOR(bp)) {
		...
		for (j = 0; j < vnic->num_lb_ctxts; j++) {
			rc = _bnxt_hwrm_vnic_ctx_free(bp,
						      vnic,
						      vnic->fw_grp_ids[j]);
			vnic->fw_grp_ids[j] = ILWALID_HW_RING_ID;
		}
		vnic->num_lb_ctxts = 0;
	} else {
		rc = _bnxt_hwrm_vnic_ctx_free(bp, vnic, vnic->rss_rule);
		vnic->rss_rule = ILWALID_HW_RING_ID;
	}
...
int bnxt_hwrm_vnic_free(struct bnxt *bp, struct bnxt_vnic_info *vnic)
{
	...
	vnic->fw_vnic_id = ILWALID_HW_RING_ID;
...
bnxt_free_all_hwrm_ring_grps(struct bnxt *bp)
{
	...
	for (idx = 0; idx < bp->rx_cp_nr_rings; idx++) {
		...
		if (bp->grp_info[idx].fw_grp_id == ILWALID_HW_RING_ID)
			continue;
...
void bnxt_free_nq_ring(struct bnxt *bp, struct bnxt_cp_ring_info *cpr)
{
	...
	cp_ring->fw_ring_id = ILWALID_HW_RING_ID;
...
void bnxt_free_hwrm_rx_ring(struct bnxt *bp, int queue_index)
{
	...
	if (ring->fw_ring_id != ILWALID_HW_RING_ID) {
	...
	if (cpr->cp_ring_struct->fw_ring_id != ILWALID_HW_RING_ID)
		bnxt_free_cp_ring(bp, cpr);

	if (BNXT_HAS_RING_GRPS(bp))
		bp->grp_info[queue_index].cp_fw_ring_id = ILWALID_HW_RING_ID;
}
...
bnxt_free_all_hwrm_rings(struct bnxt *bp)
{
	...
	for (i = 0; i < bp->tx_cp_nr_rings; i++) {
		...
		if (cpr->cp_ring_struct->fw_ring_id != ILWALID_HW_RING_ID) {
			bnxt_free_cp_ring(bp, cpr);
			cpr->cp_ring_struct->fw_ring_id = ILWALID_HW_RING_ID;
		}
	}
...
int bnxt_hwrm_func_qcfg(struct bnxt *bp, uint16_t *mtu)
{
	...
	uint16_t flags;
	int rc = 0;
	bp->func_svif = BNXT_SVIF_ILWALID;
	uint16_t svif_info;
...
int bnxt_hwrm_parent_pf_qcfg(struct bnxt *bp)
{
	...
	bp->parent->fid = BNXT_PF_FID_ILWALID;
...
int bnxt_hwrm_port_mac_qcfg(struct bnxt *bp)
{
	...
	bp->port_svif = BNXT_SVIF_ILWALID;
...
int bnxt_hwrm_func_qcfg_lwrrent_vf_vlan(struct bnxt *bp, int vf)
{
	struct hwrm_func_qcfg_input req = {0};
	struct hwrm_func_qcfg_output *resp = bp->hwrm_cmd_resp_addr;
	int rc;
...
static void
bnxt_callwlate_pf_resources(struct bnxt *bp,
			    struct bnxt_pf_resource_info *pf_resc,
			    int num_vfs)
{
	if (!num_vfs) {
...
int bnxt_hwrm_allocate_pf_only(struct bnxt *bp)
{
	...
	bnxt_callwlate_pf_resources(bp, &pf_resc, 0);
...
int bnxt_hwrm_allocate_vfs(struct bnxt *bp, int num_vfs)
{
	...
	bnxt_callwlate_pf_resources(bp, &pf_resc, num_vfs);
...
int bnxt_hwrm_lwm_get_dir_info(struct bnxt *bp, uint32_t *entries,
			       uint32_t *length)
{
	int rc;
	struct hwrm_lwm_get_dir_info_input req = {0};
	struct hwrm_lwm_get_dir_info_output *resp = bp->hwrm_cmd_resp_addr;

	HWRM_PREP(&req, HWRM_LWM_GET_DIR_INFO, BNXT_USE_CHIMP_MB);
...
int bnxt_get_lwram_directory(struct bnxt *bp, uint32_t len, uint8_t *data)
{
	int rc;
	...
	struct hwrm_lwm_get_dir_entries_input req = {0};
	struct hwrm_lwm_get_dir_entries_output *resp = bp->hwrm_cmd_resp_addr;
	...
	*data++ = entry_length;
	...
	buf = rte_malloc("lwm_dir", buflen, 0);
	if (buf == NULL)
		return -ENOMEM;
	...
	HWRM_PREP(&req, HWRM_LWM_GET_DIR_ENTRIES, BNXT_USE_CHIMP_MB);
...
int bnxt_hwrm_get_lwram_item(struct bnxt *bp, uint32_t index,
			     ...
			     uint8_t *data)
{
	...
	uint8_t *buf;
	rte_iova_t dma_handle;
	struct hwrm_lwm_read_input req = {0};
	struct hwrm_lwm_read_output *resp = bp->hwrm_cmd_resp_addr;

	buf = rte_malloc("lwm_item", length, 0);
	if (!buf)
		return -ENOMEM;
	...
	HWRM_PREP(&req, HWRM_LWM_READ, BNXT_USE_CHIMP_MB);
	req.host_dest_addr = rte_cpu_to_le_64(dma_handle);
...
int bnxt_hwrm_erase_lwram_directory(struct bnxt *bp, uint8_t index)
{
	int rc;
	struct hwrm_lwm_erase_dir_entry_input req = {0};
	struct hwrm_lwm_erase_dir_entry_output *resp = bp->hwrm_cmd_resp_addr;

	HWRM_PREP(&req, HWRM_LWM_ERASE_DIR_ENTRY, BNXT_USE_CHIMP_MB);
	req.dir_idx = rte_cpu_to_le_16(index);
	rc = bnxt_hwrm_send_message(bp, &req, sizeof(req), BNXT_USE_CHIMP_MB);
	HWRM_CHECK_RESULT();
	HWRM_UNLOCK();
...
int bnxt_hwrm_flash_lwram(struct bnxt *bp, uint16_t dir_type,
			  uint16_t dir_ordinal, uint16_t dir_ext,
			  uint16_t dir_attr, const uint8_t *data,
			  size_t data_len)
{
	int rc;
	struct hwrm_lwm_write_input req = {0};
	struct hwrm_lwm_write_output *resp = bp->hwrm_cmd_resp_addr;
	rte_iova_t dma_handle;
	uint8_t *buf;

	buf = rte_malloc("lwm_write", data_len, 0);
	if (!buf)
		return -ENOMEM;
	...
	HWRM_PREP(&req, HWRM_LWM_WRITE, BNXT_USE_CHIMP_MB);
...
int bnxt_hwrm_port_ts_query(struct bnxt *bp, uint8_t path, uint64_t *timestamp)
{
	...
	case BNXT_PTP_FLAGS_PATH_RX:
		...
		break;
	case BNXT_PTP_FLAGS_LWRRENT_TIME:
		flags |= HWRM_PORT_TS_QUERY_INPUT_FLAGS_LWRRENT_TIME;
		break;
	}
