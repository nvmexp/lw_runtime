...
ulp_post_process_normal_flow(struct ulp_rte_parser_params *params)
{
	...
	if (ULP_BITMAP_ISSET(params->act_bitmap.bits,
		 ...
		 * Similarly add GRE and LWGRE in future.
		 */
			...
			uint16_t mask)
{
	...
	if (port_type == BNXT_ULP_INTF_TYPE_ILWALID) {
		       ...
		       struct ulp_rte_parser_params *params)
{
	...
	/* perform the colwersion from dpdk port to bnxt ifindex */
	if (ulp_port_db_dev_port_to_ulp_index(params->ulp_ctx,
					      port_id,
					      &ifindex)) {
		BNXT_TF_DBG(ERR, "ParseErr:Portid is not valid\n");
		       ...
		       struct ulp_rte_parser_params *params)
{
	...
	/* perform the colwersion from VF Func id to bnxt ifindex */
			 ...
			 struct ulp_rte_parser_params *params)
{
	 ...
	 * the mask in the proto value callwlation.
	 */
	if (ipv4_mask)
			 ...
			 struct ulp_rte_parser_params *params)
{
	 ...
	 * the mask in proto value callwlation.
	 */
	if (ipv6_mask)
