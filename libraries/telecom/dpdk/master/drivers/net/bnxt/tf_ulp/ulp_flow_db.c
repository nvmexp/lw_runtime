 ...
 *
 * returns 0 if counter aclwm is set else -1.
 */
static int32_t
			 ...
			 struct ulp_flow_db_res_params *params)
{
	 ...
	 * corresponding slot in the SW aclwmulation table in the Flow Counter
	 * manager
	 */
	if (params->resource_type == TF_TBL_TYPE_ACT_STATS_64 &&
	    params->resource_sub_type ==
...
/*
 * Set the counter aclwmulation in the parent flow
 *
 * ulp_ctxt [in] Ptr to ulp_context
 * parent_idx [in] The parent index of the parent flow entry
 *
 ...
 */
static int32_t
ulp_flow_db_parent_flow_count_aclwm_set(struct bnxt_ulp_context *ulp_ctxt,
					uint32_t parent_idx)
{
	struct bnxt_ulp_flow_db *flow_db;
	struct ulp_fdb_parent_child_db *p_pdb;
...
/*
 * Get the counter aclwmulation in the parent flow
 *
 * ulp_ctxt [in] Ptr to ulp_context
 * parent_fid [in] The flow id of the parent flow entry
 *
 ...
 */
static int32_t
ulp_flow_db_parent_flow_count_aclwm_get(struct bnxt_ulp_context *ulp_ctxt,
					uint32_t parent_fid)
{
	struct bnxt_ulp_flow_db *flow_db;
	struct ulp_fdb_parent_child_db *p_pdb;
			     ...
			     uint32_t fid)
{
	...
	while (res_id) {
		...
		if (ulp_flow_db_resource_func_get(fid_res) ==
			...
			/* ilwalidate the resource details */
...
ulp_flow_db_parent_flow_create(struct bnxt_ulp_mapper_parms *parms)
{
	...
	/* check of the flow has internal counter aclwmulation enabled */
	if (!ulp_flow_db_resource_params_get(parms->ulp_ctx,
					     ...
					     sub_type,
					     &res_params)) {
		/* Enable the counter aclwmulation in parent entry */
		if (ulp_flow_db_parent_flow_count_aclwm_set(parms->ulp_ctx,
							    fid_idx)) {
...
ulp_flow_db_child_flow_create(struct bnxt_ulp_mapper_parms *parms)
{
	...
	/* check if aclwmulation count is set for parent flow */
	rc = ulp_flow_db_parent_flow_count_aclwm_get(parms->ulp_ctx,
						     parms->parent_fid);
	if (!rc) {
...
/*
 * Get the parent aclwmulation counters
 *
 * ulp_ctxt [in] Ptr to ulp_context
 * parent_fid [in] The flow id of the parent flow entry
...
/*
 * reset the parent aclwmulation counters
 *
 * ulp_ctxt [in] Ptr to ulp_context
 *
 * returns none
