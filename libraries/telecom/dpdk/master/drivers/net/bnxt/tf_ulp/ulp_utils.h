...
/* Macro to colwert bytes to bits */
#define ULP_BYTE_2_BITS(byte_x)		((byte_x) * 8)
/* Macro to colwert bits to bytes */
#define ULP_BITS_2_BYTE(bits_x)		(((bits_x) + 7) / 8)
/* Macro to colwert bits to bytes with no round off*/
 ...
 *
 * order [in] The byte order for the blob.  Lwrrently only supporting
