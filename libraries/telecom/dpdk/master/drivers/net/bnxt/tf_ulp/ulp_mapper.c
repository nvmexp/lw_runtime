			    ...
			    struct ulp_flow_db_res_params *res)
{
	...
	for (i = 0; i < BNXT_ULP_CACHE_TBL_IDENT_MAX_NUM; i++) {
		if (cache_entry->idents[i] == ULP_IDENTS_ILWALID)
			continue;
			 ...
			 struct ulp_blob *res_blob)
{
	...
	/* colwert the idx into cpu format */
			     ...
			     struct bnxt_ulp_mapper_tbl_info *tbl)
{
	...
	/*
	 * callwlate the idx for the result record, for external EM the offset
			     ...
			     struct bnxt_ulp_mapper_tbl_info *tbl)
{
	 ...
	 * cache_key is a byte array of tmplen, it needs to be colwerted to a
	...
	if (!cache_entry->ref_count) {
		...
		for (i = 0; i < BNXT_ULP_CACHE_TBL_IDENT_MAX_NUM; i++)
			cache_entry->idents[i] = ULP_IDENTS_ILWALID;
				      ...
				      struct bnxt_ulp_mapper_tbl_info *tbl)
{
	...
	case BNXT_ULP_MEM_TYPE_OPCODE_EXELWTE_IF_INT:
		if (mtype == BNXT_ULP_FLOW_MEM_TYPE_INT)
			rc = 0;
		break;
	case BNXT_ULP_MEM_TYPE_OPCODE_EXELWTE_IF_EXT:
		if (mtype == BNXT_ULP_FLOW_MEM_TYPE_EXT)
			rc = 0;
		break;
