...
#define ULP_MARK_DB_ENTRY_SET_VALID(mark_info) ((mark_info)->flags |=\
						BNXT_ULP_MARK_VALID)
#define ULP_MARK_DB_ENTRY_IS_ILWALID(mark_info) (!((mark_info)->flags &\
						   BNXT_ULP_MARK_VALID))
		     ...
		     uint32_t *mark)
{
	...
	if (is_gfid) {
		if (idx >= mtbl->gfid_num_entries ||
		    ULP_MARK_DB_ENTRY_IS_ILWALID(&mtbl->gfid_tbl[idx]))
			return -EILWAL;
		...
		*mark = mtbl->gfid_tbl[idx].mark_id;
		...
		if (idx >= mtbl->lfid_num_entries ||
		    ULP_MARK_DB_ENTRY_IS_ILWALID(&mtbl->lfid_tbl[idx]))
			return -EILWAL;
