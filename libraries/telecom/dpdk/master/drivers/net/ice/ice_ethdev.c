...
static const struct eth_dev_ops ice_eth_dev_ops = {
	...
	.promislwous_enable           = ice_promisc_enable,
	.promislwous_disable          = ice_promisc_disable,
...
#define FLX_REG(val, fld, idx) \
	...
	static struct {
		...
		[PROTO_XTR_TCP] = { ICE_RXDID_COMMS_AUX_TCP,
				    ICE_RX_OPC_EXTRACT,
				    ICE_PROT_TCP_IL, ICE_PROT_ID_ILWAL },
	...
	for (i = 0; i < RTE_DIM(xtr_sets); i++) {
		...
		if (xtr_sets[i].protid_0 != ICE_PROT_ID_ILWAL) {
		...
		if (xtr_sets[i].protid_1 != ICE_PROT_ID_ILWAL) {
...
ice_setup_vsi(struct ice_pf *pf, enum ice_vsi_type type)
{
	...
	case ICE_VSI_PF:
		...
		/* FDIR */
		cfg = ICE_AQ_VSI_PROP_SELWRITY_VALID |
...
ice_vsi_config_vlan_filter(struct ice_vsi *vsi, bool on)
{
	...
	ctxt.info.valid_sections =
		rte_cpu_to_le_16(ICE_AQ_VSI_PROP_SW_VALID |
				 ICE_AQ_VSI_PROP_SELWRITY_VALID);
	ctxt.vsi_num = vsi->vsi_id;
	       ...
	       struct rte_dev_eeprom_info *eeprom)
{
	...
	if (status) {
		PMD_DRV_LOG(ERR, "acquire lwm failed.");
		return -EIO;
	}

	status = ice_read_flat_lwm(hw, eeprom->offset, &eeprom->length,
				   data, false);
