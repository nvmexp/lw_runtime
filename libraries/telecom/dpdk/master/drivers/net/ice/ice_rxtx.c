...
ice_program_hw_rx_queue(struct ice_rx_queue *rxq)
{
	...
	if (!(pf->supported_rxdid & BIT(rxdid))) {
		PMD_DRV_LOG(ERR, "lwrrently package doesn't support RXDID (%u)",
			    rxdid);
		return -EILWAL;
	}
	      ...
	      uint16_t nb_pkts)
{
	...
	while (nb_rx < nb_pkts) {
		...
		/* callwlate rx_packet_len of the received pkt */
		rx_packet_len = (rte_le_to_cpu_16(rxd.wb.pkt_len) &
				 ICE_RX_FLX_DESC_PKT_LEN_M) - rxq->crc_len;
...
/* Callwlate the number of TX descriptors needed for each pkt */
static inline uint16_t
...
ice_xmit_pkts(void *tx_queue, struct rte_mbuf **tx_pkts, uint16_t nb_pkts)
{
	...
	for (nb_tx = 0; nb_tx < nb_pkts; nb_tx++) {
		 ...
		 * Recallwlate the needed tx descs when TSO enabled in case
...
#else
			PMD_DRV_LOG(NOTICE,
				"AVX512 is not supported in build elw");
#endif
			if (!use_avx512 &&
