...
#define ICE_ACL_SCEN_ENTRY_ILWAL 0xFFFF
 ...
 *
 * Returns ICE_ACL_SCEN_ENTRY_ILWAL if fails
			      ...
			      enum ice_acl_entry_prio prio)
{
	...
	if (prio >= ICE_ACL_MAX_PRIO)
		return ICE_ACL_SCEN_ENTRY_ILWAL;
 ...
 *
 * Callwlate the end entry index for a partition with starting entry index
 ...
 *
 * Initialize the ACL table by ilwalidating TCAM entries and action pairs.
 */
static enum ice_status ice_acl_init_tbl(struct ice_hw *hw)
{
	...
	while (tcam_idx < tbl->last_tcam ||
		...
		/* Use the same value for entry_key and entry_key_ilw since
	...
	for (i = 0; i < ICE_AQC_MAX_ACTION_MEMORIES; i++) {
		...
		if (tbl->act_mems[i].act_mem == ICE_ACL_ACT_PAIR_MEM_ILWAL)
			continue;
		...
		for (act_entry_idx = start; act_entry_idx <= end;
		     act_entry_idx++) {
			/* Ilwalidate all allocated action pairs */
 ...
 * @lwr_tcam: Index of current TCAM. Value = 0 to (ICE_AQC_ACL_SLICES - 1)
 * @lwr_mem_idx: Index of current action memory bank. Value = 0 to
 *		 (ICE_AQC_MAX_ACTION_MEMORIES - 1)
 * @num_mem: Number of action memory banks for this TCAM
 *
 * Assign "num_mem" valid action memory banks from "lwrr_mem_idx" to
 * "lwrr_tcam" TCAM.
 */
static void
ice_acl_assign_act_mems_to_tcam(struct ice_acl_tbl *tbl, u8 lwr_tcam,
				u8 *lwr_mem_idx, u8 num_mem)
{
	u8 mem_cnt;

	for (mem_cnt = 0;
	     *lwr_mem_idx < ICE_AQC_MAX_ACTION_MEMORIES && mem_cnt < num_mem;
	     (*lwr_mem_idx)++) {
		struct ice_acl_act_mem *p_mem = &tbl->act_mems[*lwr_mem_idx];

		if (p_mem->act_mem == ICE_ACL_ACT_PAIR_MEM_ILWAL)
			continue;

		p_mem->member_of_tcam = lwr_tcam;
 ...
 *
 * Create a LEM table for ACL usage. We are lwrrently starting with some fixed
...
ice_acl_create_tbl(struct ice_hw *hw, struct ice_acl_tbl_params *params)
{
	...
	tbl_alloc.conlwrr = params->conlwrr;
	/* Set dependent_alloc_id only for conlwrrent table type */
	if (params->conlwrr) {
		tbl_alloc.num_dependent_alloc_ids =
			ICE_AQC_MAX_CONLWRRENT_ACL_TBL;

		for (i = 0; i < ICE_AQC_MAX_CONLWRRENT_ACL_TBL; i++)
	...
	/* Initialize the resources allocated by ilwalidating all TCAM entries
...
ice_acl_alloc_partition(struct ice_hw *hw, struct ice_acl_scen *req)
{
	 ...
	 * aclwmulates contiguous available 64-entry chunks until there are
 ...
 * @lwrrent_tcam_idx: theoretical index of the TCAM that we associated those
...
ice_acl_assign_act_mem_for_scen(struct ice_acl_tbl *tbl,
				...
				u8 lwrrent_tcam_idx, u8 target_tcam_idx)
{
	u8 i;
	...
	for (i = 0; i < ICE_AQC_MAX_ACTION_MEMORIES; i++) {
		...
		if (p_mem->act_mem == ICE_ACL_ACT_PAIR_MEM_ILWAL ||
		    p_mem->member_of_tcam != lwrrent_tcam_idx)
			continue;
 ...
 * @ilwerts: buffer of the value of the key ilwerts to be programmed
 ...
 *
 * Given an ACL table and a scenario, to add the specified key and key ilwert
 * to an available entry in the specified scenario.
 * The "keys" and "ilwerts" buffers must be of the size which is the same as
 * the scenario's width
 */
enum ice_status
ice_acl_add_entry(struct ice_hw *hw, struct ice_acl_scen *scen,
		  enum ice_acl_entry_prio prio, u8 *keys, u8 *ilwerts,
		  struct ice_acl_act_entry *acts, u8 acts_cnt, u16 *entry_idx)
{
	...
	*entry_idx = ice_acl_scen_assign_entry_idx(scen, prio);
	...
	for (i = 0; i < num_cscd; i++) {
		 ...
		 * TCAMs, the key and key ilwerts need to be properly split
		...
		ice_memcpy(&buf.entry_key_ilwert.val,
			   &ilwerts[offset * sizeof(buf.entry_key_ilwert.val)],
			   sizeof(buf.entry_key_ilwert.val),
...
ice_acl_rem_entry(struct ice_hw *hw, struct ice_acl_scen *scen, u16 entry_idx)
{
	...
	ice_for_each_set_bit(i, scen->act_mem_bitmap,
		...
		if (mem->member_of_tcam >= entry_tcam &&
		    mem->member_of_tcam < entry_tcam + num_cscd) {
			/* Ilwalidate allocated action pairs */
			status = ice_aq_program_actpair(hw, i, idx, &act_buf,
							NULL);
			if (status)
				ice_debug(hw, ICE_DBG_ACL, "program actpair failed status: %d\n",
