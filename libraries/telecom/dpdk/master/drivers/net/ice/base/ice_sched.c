 ...
 * from the SW DB. The search is relwrsive and is restricted by the number of
	...
	*num_nodes_added = 0;
	...
	if ((parent->num_children + num_nodes) > max_child_nodes) {
		...
		if (parent->num_children < max_child_nodes) {
			...
			/* this relwrsion is intentional, and wouldn't
		 ...
		 * memory for all other relwrsive calls.
		 */
		if (num_added)
			first_teid_ptr = &temp;
		...
		/* this relwrsion is intentional, for 1024 queues
...
static u8 ice_sched_get_vsi_layer(struct ice_hw *hw)
{
	 ...
	 */
	/* callwlate the VSI layer based on number of layers. */
...
static u8 ice_sched_get_agg_layer(struct ice_hw *hw)
{
	 ...
	 */
	/* callwlate the aggregator layer based on number of layers. */
...
/**
 * ice_sched_calc_vsi_child_nodes - callwlate number of VSI child nodes
 * @hw: pointer to the HW struct
 * @num_qs: number of queues
 * @num_nodes: num nodes array
 *
 * This function callwlates the number of VSI child nodes based on the
 * number of queues.
 */
static void
ice_sched_calc_vsi_child_nodes(struct ice_hw *hw, u16 num_qs, u16 *num_nodes)
{
	...
	/* callwlate num nodes from queue group to VSI layer */
	for (i = qgl; i > vsil; i--) {
...
/**
 * ice_sched_calc_vsi_support_nodes - callwlate number of VSI support nodes
 * @pi: pointer to the port info structure
 * @tc_node: pointer to TC node
 * @num_nodes: pointer to num nodes array
 *
 * This function callwlates the number of supported nodes needed to add this
				 ...
				 struct ice_sched_node *tc_node, u16 *num_nodes)
{
	...
	for (i = vsil; i >= pi->hw->sw_entry_point_layer; i--)
		...
		if (!tc_node->num_children || i == vsil) {
			 ...
			 * So no need to callwlate supported nodes for below
			 * layers.
			 */
			if (node)
				break;
...
ice_sched_add_vsi_to_topo(struct ice_port_info *pi, u16 vsi_handle, u8 tc)
{
	...
	/* callwlate number of supported nodes needed for this VSI */
		  ...
		  u8 owner, bool enable)
{
	...
	if (!vsi_node) {
		...
		/* ilwalidate the max queues whenever VSI gets added first time
			      ...
			      u16 *num_nodes)
{
	...
	if (node->num_children < hw->max_children[l])
		num_nodes[l] = 0;
	/* The below relwrsive call is intentional and wouldn't go more than
	 * 2 or 3 iterations.
	 */
...
/**
 * ice_sched_calc_wakeup - callwlate RL profile wakeup parameter
 * @hw: pointer to the HW struct
 * @bw: bandwidth in Kbps
 *
 * This function callwlates the wakeup parameter of RL profile.
 */
static u16 ice_sched_calc_wakeup(struct ice_hw *hw, s32 bw)
{
	...
	if (wakeup_int > 63) {
		...
		/* Callwlate fraction value up to 4 decimals
		 * Colwert Integer value to a constant multiplier
		 */
...
/**
 * ice_sched_bw_to_rl_profile - colwert BW to profile parameters
 * @hw: pointer to the HW struct
 * @bw: bandwidth in Kbps
 * @profile: profile parameters to return
 *
 * This function colwerts the BW to profile structure format.
 */
static enum ice_status
			   ...
			   struct ice_aqc_rl_profile_elem *profile)
{
	...
	for (i = 0; i < 64; i++) {
		 ...
		 * accuracy bytes
		 */
		if (mv > ICE_RL_PROF_ACLWRACY_BYTES) {
			encode = i;
			found = true;
			break;
		}
...
ice_sched_get_node_rl_prof_id(struct ice_sched_node *node,
			      enum ice_rl_type rl_type)
{
	u16 rl_prof_id = ICE_SCHED_ILWAL_PROF_ID;
	struct ice_aqc_txsched_elem *data;
			    ...
			    u8 layer_index)
{
	...
	if (layer_index >= hw->num_tx_sched_layers)
		return ICE_SCHED_ILWAL_LAYER_NUM;
	switch (rl_type) {
	case ICE_MIN_BW:
			   ...
			   enum ice_rl_type rl_type, u8 layer_num)
{
	...
	if (old_id == ICE_SCHED_DFLT_RL_PROF_ID ||
	    old_id == ICE_SCHED_ILWAL_PROF_ID)
		return ICE_SUCCESS;
		      ...
		      enum ice_rl_type rl_type, u32 bw, u8 layer_num)
{
	...
	if ((old_id == ICE_SCHED_DFLT_RL_PROF_ID && rl_type != ICE_SHARED_BW) ||
	    old_id == ICE_SCHED_ILWAL_PROF_ID || old_id == rl_prof_id)
		return ICE_SUCCESS;
...
{
	u8 sel_layer = ICE_SCHED_ILWAL_LAYER_NUM;
	u8 tc;
	...
	ice_for_each_traffic_class(tc) {
		...
		/* SRL bandwidth layer selection */
		if (sel_layer == ICE_SCHED_ILWAL_LAYER_NUM) {
