		     ...
		     struct ice_sq_cd *cd)
{
	...
	/* If this is conlwrrent table, then buffer shall be valid and
		       ...
		       struct ice_sq_cd *cd)
{
	...
	if (!status) {
		...
		if (first_cntr == ICE_AQC_ACL_ALLOC_CNT_ILWAL ||
		    last_cntr == ICE_AQC_ACL_ALLOC_CNT_ILWAL)
