...
#define ICE_IPV4_LWGRE_PROTO_ID		0x002F
...
static const u8 dummy_gre_tcp_packet[] = {
	...
	0x80, 0x00, 0x65, 0x58,	/* ICE_LWGRE 34 */
	0x00, 0x00, 0x00, 0x00,
		    ...
		    bool *refresh_required)
{
	...
	for (sub_recps = 0; sub_recps < num_recps; sub_recps++) {
		...
		if (root_bufs.content.result_indx & ICE_AQ_RECIPE_RESULT_EN) {
			...
			recps[idx].chain_idx = ICE_ILWAL_CHAIN_IND;
		}
			   ...
			   struct ice_sq_cd *cd, u16 *rule_id)
{
	...
	if ((*rule_id) != ICE_ILWAL_MIRROR_RULE_ID)
		   ...
		   u16 swid, u16 pf_vf_num, bool is_vf)
{
	...
	case ICE_AQC_GET_SW_CONF_RESP_PHYS_PORT:
		...
		pi->is_vf = is_vf;
		pi->dflt_tx_vsi_num = ICE_DFLT_VSI_ILWAL;
		pi->dflt_rx_vsi_num = ICE_DFLT_VSI_ILWAL;
		break;
	default:
...
static void ice_fill_sw_info(struct ice_hw *hw, struct ice_fltr_info *fi)
{
	...
	if ((fi->flag & ICE_FLTR_TX) &&
		 ...
		 * promislwous, ethertype-MAC, promislwous-VLAN
		 * and default-port OR
			...
			struct ice_fltr_list_entry *f_entry)
{
	...
	fm_entry->lg_act_idx = ICE_ILWAL_LG_ACT_INDEX;
	fm_entry->sw_marker_id = ICE_ILWAL_SW_MARKER_ID;
	fm_entry->counter_index = ICE_ILWAL_COUNTER_ID;
 ...
 * @lwr_fltr: filter information from the book keeping entry
...
ice_add_update_vsi_list(struct ice_hw *hw,
			...
			struct ice_fltr_info *lwr_fltr,
			struct ice_fltr_info *new_fltr)
{
	enum ice_status status = ICE_SUCCESS;
	u16 vsi_list_id = 0;

	if ((lwr_fltr->fltr_act == ICE_FWD_TO_Q ||
	     lwr_fltr->fltr_act == ICE_FWD_TO_QGRP))
		return ICE_ERR_NOT_IMPL;

	if ((new_fltr->fltr_act == ICE_FWD_TO_Q ||
	     new_fltr->fltr_act == ICE_FWD_TO_QGRP) &&
	    (lwr_fltr->fltr_act == ICE_FWD_TO_VSI ||
	     lwr_fltr->fltr_act == ICE_FWD_TO_VSI_LIST))
		return ICE_ERR_NOT_IMPL;
	...
	if (m_entry->vsi_count < 2 && !m_entry->vsi_list_info) {
		...
		if (lwr_fltr->fwd_id.hw_vsi_id == new_fltr->fwd_id.hw_vsi_id)
			return ICE_ERR_ALREADY_EXISTS;

		vsi_handle_arr[0] = lwr_fltr->vsi_handle;
		...
		tmp_fltr = *new_fltr;
		tmp_fltr.fltr_rule_id = lwr_fltr->fltr_rule_id;
		...
		lwr_fltr->fwd_id.vsi_list_id = vsi_list_id;
		lwr_fltr->fltr_act = ICE_FWD_TO_VSI_LIST;
		 ...
		 */
		if (m_entry->sw_marker_id != ICE_ILWAL_SW_MARKER_ID)
			status =
		 ...
		 */
		vsi_list_id = lwr_fltr->fwd_id.vsi_list_id;
...
{
	struct ice_fltr_info *new_fltr, *lwr_fltr;
	struct ice_fltr_mgmt_list_entry *m_entry;
	struct ice_lock *rule_lock; /* Lock to protect filter rule list */
	enum ice_status status = ICE_SUCCESS;
	...
	lwr_fltr = &m_entry->fltr_info;
	status = ice_add_update_vsi_list(hw, m_entry, lwr_fltr, new_fltr);
		      ...
		      struct ice_fltr_list_entry *f_entry)
{
	...
	if (!v_list_itr) {
		...
		if (new_fltr->fltr_act == ICE_FWD_TO_VSI) {
			...
			/* Colwert the action to forwarding to a VSI list. */
		 ...
		 * by one VLAN rule.
		 */
		lwr_fltr = &v_list_itr->fltr_info;
		status = ice_add_update_vsi_list(hw, v_list_itr, lwr_fltr,
						 new_fltr);
	} else {
		...
		/* A rule already exists with the new VSI being added */
		if (lwr_handle == vsi_handle) {
			status = ICE_ERR_ALREADY_EXISTS;
			goto exit;
		}

		vsi_handle_arr[0] = lwr_handle;
		 ...
		 u8 direction)
{
	...
	if (set) {
		...
		if (f_info.flag & ICE_FLTR_TX) {
			pi->dflt_tx_vsi_num = ICE_DFLT_VSI_ILWAL;
			pi->dflt_tx_vsi_rule_id = ICE_ILWAL_ACT;
		} else if (f_info.flag & ICE_FLTR_RX) {
			pi->dflt_rx_vsi_num = ICE_DFLT_VSI_ILWAL;
			pi->dflt_rx_vsi_rule_id = ICE_ILWAL_ACT;
		}
	}
...
/**
 * _ice_get_vsi_promisc - get promislwous mode of given VSI
 * @hw: pointer to the hardware structure
...
/**
 * ice_get_vsi_promisc - get promislwous mode of given VSI
 * @hw: pointer to the hardware structure
 * @vsi_handle: VSI handle to retrieve info from
 * @promisc_mask: pointer to mask to be filled in
 * @vid: VLAN ID of promisc VLAN VSI
...
/**
 * ice_get_vsi_vlan_promisc - get VLAN promislwous mode of given VSI
 * @hw: pointer to the hardware structure
 * @vsi_handle: VSI handle to retrieve info from
 * @promisc_mask: pointer to mask to be filled in
 * @vid: VLAN ID of promisc VLAN VSI
...
/**
 * _ice_clear_vsi_promisc - clear specified promislwous mode(s)
 * @hw: pointer to the hardware structure
 * @vsi_handle: VSI handle to clear mode
 * @promisc_mask: mask of promislwous config bits to clear
 * @vid: VLAN ID to clear VLAN promislwous
 * @sw: pointer to switch info struct for which function add rule
 */
static enum ice_status
...
/**
 * ice_clear_vsi_promisc - clear specified promislwous mode(s) for given VSI
 * @hw: pointer to the hardware structure
 * @vsi_handle: VSI handle to clear mode
 * @promisc_mask: mask of promislwous config bits to clear
 * @vid: VLAN ID to clear VLAN promislwous
...
/**
 * _ice_set_vsi_promisc - set given VSI to given promislwous mode(s)
 * @hw: pointer to the hardware structure
 * @vsi_handle: VSI handle to configure
 * @promisc_mask: mask of promislwous config bits
 * @vid: VLAN ID to set VLAN promislwous
		     ...
		     u16 vid, u8 lport, struct ice_switch_info *sw)
{
	...
	while (promisc_mask) {
		...
		/* Check for VLAN promislwous flag */
...
/**
 * ice_set_vsi_promisc - set given VSI to given promislwous mode(s)
 * @hw: pointer to the hardware structure
 * @vsi_handle: VSI handle to configure
 * @promisc_mask: mask of promislwous config bits
 * @vid: VLAN ID to set VLAN promislwous
 ...
 * @sw: pointer to switch info struct for which function add rule
 *
 * Configure VSI with all associated VLANs to given promislwous mode(s)
 */
static enum ice_status
			   ...
			   u16 sw_marker)
{
	...
	if (sw_marker == ICE_ILWAL_SW_MARKER_ID)
		return ICE_ERR_PARAM;
	 ...
	 */
	if (m_entry->counter_index != ICE_ILWAL_COUNTER_ID) {
		ret = ICE_ERR_PARAM;
		goto exit_error;
	}
	...
	if (lg_act_id == ICE_ILWAL_LG_ACT_INDEX)
		goto exit_error;
			 ...
			 enum ice_sw_tunnel_type tun_type)
{
	...
	for (i = 0; i < ICE_MAX_NUM_RECIPES; i++) {
		...
		/* Skip ilwerse action recipes */
		if (recp[i].root_buf && recp[i].root_buf->content.act_ctrl &
		    ICE_AQ_RECIPE_ACT_ILW_ACT)
			continue;
 ...
 *
 * callwlate valid words in a lookup rule using mask value
 */
static u8
		  ...
		  ice_bitmap_t *profiles)
{
	...
	if (rm->n_grp_count == 1) {
		 ...
		 */
		last_chain_entry->chain_idx = ICE_ILWAL_CHAIN_IND;
		      ...
		      const struct ice_dummy_pkt_offsets **offsets)
{
	...
	if (tun_type == ICE_SW_TUN_LWGRE || gre) {
		if (tcp) {
...
ice_adv_add_update_vsi_list(struct ice_hw *hw,
			    ...
			    struct ice_adv_rule_info *lwr_fltr,
			    struct ice_adv_rule_info *new_fltr)
{
	enum ice_status status;
	u16 vsi_list_id = 0;

	if (lwr_fltr->sw_act.fltr_act == ICE_FWD_TO_Q ||
	    lwr_fltr->sw_act.fltr_act == ICE_FWD_TO_QGRP ||
	    lwr_fltr->sw_act.fltr_act == ICE_DROP_PACKET)
		return ICE_ERR_NOT_IMPL;

	if ((new_fltr->sw_act.fltr_act == ICE_FWD_TO_Q ||
	     new_fltr->sw_act.fltr_act == ICE_FWD_TO_QGRP) &&
	    (lwr_fltr->sw_act.fltr_act == ICE_FWD_TO_VSI ||
	     lwr_fltr->sw_act.fltr_act == ICE_FWD_TO_VSI_LIST))
		return ICE_ERR_NOT_IMPL;
	...
	if (m_entry->vsi_count < 2 && !m_entry->vsi_list_info) {
		...
		/* A rule already exists with the new VSI being added */
		if (lwr_fltr->sw_act.fwd_id.hw_vsi_id ==
		    new_fltr->sw_act.fwd_id.hw_vsi_id)
			return ICE_ERR_ALREADY_EXISTS;

		vsi_handle_arr[0] = lwr_fltr->sw_act.vsi_handle;
		...
		lwr_fltr->sw_act.fwd_id.vsi_list_id = vsi_list_id;
		lwr_fltr->sw_act.fltr_act = ICE_FWD_TO_VSI_LIST;
		m_entry->vsi_list_info =
			ice_create_vsi_list_map(hw, &vsi_handle_arr[0], 2,
						vsi_list_id);
		 ...
		 * the new VSI ID passed in
		 */
		vsi_list_id = lwr_fltr->sw_act.fwd_id.vsi_list_id;
