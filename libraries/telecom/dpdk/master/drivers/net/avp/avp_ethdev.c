...
static int avp_dev_promislwous_enable(struct rte_eth_dev *dev);
static int avp_dev_promislwous_disable(struct rte_eth_dev *dev);
...
#define AVP_DPDK_DRIVER_VERSION RTE_AVP_LWRRENT_GUEST_VERSION
...
static const struct eth_dev_ops avp_eth_dev_ops = {
	...
	.promislwous_enable  = avp_dev_promislwous_enable,
	.promislwous_disable = avp_dev_promislwous_disable,
	       ...
	       struct rte_eth_dev *eth_dev)
{
	...
	/*
	 * store the host mbuf virtual address so that we can callwlate
...
_avp_mac_filter(struct avp_dev *avp, struct rte_mbuf *m)
{
	...
	if (avp->flags & AVP_F_PROMISC) {
		/* allow all packets when in promislwous mode */
		return 0;
	}
...
static int
avp_dev_promislwous_enable(struct rte_eth_dev *eth_dev)
{
	struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
	...
	if ((avp->flags & AVP_F_PROMISC) == 0) {
		...
		PMD_DRV_LOG(DEBUG, "Promislwous mode enabled on %u\n",
			    eth_dev->data->port_id);
	}
	rte_spinlock_unlock(&avp->lock);
...
static int
avp_dev_promislwous_disable(struct rte_eth_dev *eth_dev)
{
	struct avp_dev *avp = AVP_DEV_PRIVATE_TO_HW(eth_dev->data->dev_private);
	...
	if ((avp->flags & AVP_F_PROMISC) != 0) {
		avp->flags &= ~AVP_F_PROMISC;
		PMD_DRV_LOG(DEBUG, "Promislwous mode disabled on %u\n",
			    eth_dev->data->port_id);
	}
	rte_spinlock_unlock(&avp->lock);
