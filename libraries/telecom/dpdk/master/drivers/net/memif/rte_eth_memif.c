...
memif_free_stored_mbufs(struct pmd_process_private *proc_private, struct memif_queue *mq)
{
	 ...
	 */
	lwr_tail = __atomic_load_n(&ring->tail, __ATOMIC_ACQUIRE);
	while (mq->last_tail != lwr_tail) {
...
static int
memif_pktmbuf_chain(struct rte_mbuf *head, struct rte_mbuf *lwr_tail,
...
eth_memif_rx(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts)
{
	...
	memif_ring_t *ring = memif_get_ring_from_queue(proc_private, mq);
	uint16_t lwr_slot, last_slot, n_slots, ring_size, mask, s0;
...
no_free_bufs:
	if (type == MEMIF_RING_C2S) {
		__atomic_store_n(&ring->tail, lwr_slot, __ATOMIC_RELEASE);
		mq->last_head = lwr_slot;
	} else {
		mq->last_tail = lwr_slot;
	}
...
eth_memif_rx_zc(void *queue, struct rte_mbuf **bufs, uint16_t nb_pkts)
{
	...
	struct pmd_process_private *proc_private =
		rte_eth_devices[mq->in_port].process_private;
	memif_ring_t *ring = memif_get_ring_from_queue(proc_private, mq);
	uint16_t lwr_slot, last_slot, n_slots, ring_size, mask, s0, head;
	uint16_t n_rx_pkts = 0;
	memif_desc_t *d0;
...
next_slot:
		...
		if (n_rx_pkts + 1 < nb_pkts)
			rte_prefetch0(&ring->desc[(lwr_slot + 1) & mask]);
