...
/* Parsing tokens. Defined colweniently, so that any correction is easy. */
...
/**
 * Colwert string to uint32_t with extra checks for result correctness.
 *
 * @param string String to colwert.
 * @param val Colwersion result.
 * @returns 0 in case of success, negative value otherwise.
 */
static int
get_val_selwrely(const char *string, uint32_t *val)
{
	char *endptr;
		...
		struct mrvl_qos_cfg *cfg)
{
	...
	if (cfg->port[port].outq[outq].sched_mode == PP2_PPIO_SCHED_M_WRR) {
		entry = rte_cfgfile_get_entry(file, sec_name,
				MRVL_TOK_WRR_WEIGHT);
		if (entry) {
			if (get_val_selwrely(entry, &val) < 0)
				return -1;
 ...
 *
 * Entry can be anything, e.g. "1 2-3 5 6 7-9". This needs to be colwerted to
	...
	size_t elem_sz, uint8_t max_elems, uint8_t max_val)
{
	...
	for (i = 0; i < nb_tokens; ++i) {
		if (strchr(tokens[i], '-') != NULL) {
			...
			if (get_val_selwrely(rng_tokens[0], &token_val) < 0)
				return -4;
			beg = (char)token_val;
			if (get_val_selwrely(rng_tokens[1], &token_val) < 0)
				return -4;
			end = (char)token_val;
			...
			/* Single values. */
			if (get_val_selwrely(tokens[i], &token_val) < 0)
				return -5;
			val = (char)token_val;
			if (val > max_val)
				return -5;
		...
		void *extra_args)
{
	...
	*cfg = rte_zmalloc("mrvl_qos_cfg", sizeof(struct mrvl_qos_cfg), 0);
	...
	for (n = 0; n < (PP2_NUM_ETH_PPIO * PP2_NUM_PKT_PROC); ++n) {
		...
		/* Use global defaults, unless an override oclwrs */
