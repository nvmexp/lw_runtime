		   ...
		   struct rte_tm_error *error)
{
	...
	if (!is_leaf)
		return -rte_tm_error_set(error, EILWAL,
		       ...
		       struct rte_tm_error *error)
{
	if (node_id == RTE_TM_NODE_ID_NULL)
		return -rte_tm_error_set(error, EILWAL, RTE_TM_NODE_ID_NULL,
		      ...
		      struct rte_tm_error *error)
{
	...
	LIST_FOREACH(node, &priv->nodes, next) {
		...
		if (node->id >= dev->data->nb_tx_queues) {
			ret = -rte_tm_error_set(error, EILWAL,
		...
		if (node->profile) {
			...
			/* colwert Bytes/s to kilo bits/s */
			p->rate_limit_params.cir =
				node->profile->params.peak.rate * 8 / 1000;
			/* colwert bits to kilo bits */
