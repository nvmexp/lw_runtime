...
#define MRVL_COOKIE_ADDR_ILWALID ~0ULL
...
static uint64_t cookie_addr_high = MRVL_COOKIE_ADDR_ILWALID;
...
mrvl_dev_start(struct rte_eth_dev *dev)
{
	...
	/*
	 * Callwlate the minimum bpool size for refill feature as follows:
	...
	/*
	 * Callwlate the maximum bpool size for refill feature as follows:
 ...
 */
static int
mrvl_promislwous_enable(struct rte_eth_dev *dev)
{
	struct mrvl_priv *priv = dev->data->dev_private;
	int ret;
	...
	if (ret) {
		MRVL_LOG(ERR, "Failed to enable promislwous mode");
		return -EAGAIN;
	}
 ...
 */
static int
mrvl_promislwous_disable(struct rte_eth_dev *dev)
{
	struct mrvl_priv *priv = dev->data->dev_private;
	int ret;
	...
	if (ret) {
		MRVL_LOG(ERR, "Failed to disable promislwous mode");
		return -EAGAIN;
	}
		  ...
		  uint32_t index, uint32_t vmdq __rte_unused)
{
	 ...
	 * MRVL_MAC_ADDRS_MAX - uc_filter_max. Lwrrently it defaults to 4 and
	 * 21 respectively.
	 *
	 * If more than uc_filter_max uc addresses were added to filter list
	 * then NIC will switch to promislwous mode automatically.
	 *
...
mrvl_fill_bpool(struct mrvl_rxq *rxq, int num)
{
	...
	if (cookie_addr_high == MRVL_COOKIE_ADDR_ILWALID)
		cookie_addr_high =
...
static const struct eth_dev_ops mrvl_ops = {
	...
	.promislwous_enable = mrvl_promislwous_enable,
	.allmulticast_enable = mrvl_allmulticast_enable,
	.promislwous_disable = mrvl_promislwous_disable,
				    ...
				    uint8_t *l3_offset, uint8_t *l4_offset)
{
	...
	case PP2_INQ_L3_TYPE_ARP:
		 ...
		 * callwlated subtracting l4_offset and l3_offset.
		 */
