...
/*
 * ICMP, ICMP6, UDP, TCP, SCTP, VXLAN, GRE, LWGRE,
 * GTP, GTPC, GTPU, ESP
 *
...
otx2_flow_parse_ld(struct otx2_parse_state *pst)
{
	...
	case RTE_FLOW_ITEM_TYPE_LWGRE:
		lt = NPC_LT_LD_LWGRE;
		lflags = NPC_F_GRE_LWGRE;
		info.def_mask = &rte_flow_item_lwgre_mask;
		info.len = sizeof(struct rte_flow_item_lwgre);
			...
			struct rte_flow *flow)
{
	...
	for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
		...
		case RTE_FLOW_ACTION_TYPE_SELWRITY:
