...
static const struct rte_selwrity_capability otx2_eth_sec_capabilities[] = {
	{	/* IPsec Inline Protocol ESP Tunnel Ingress */
		...
		.ol_flags = RTE_SELWRITY_TX_OLOAD_NEED_MDATA
	},
...
static int
ipsec_sa_const_set(struct rte_selwrity_ipsec_xform *ipsec,
		   ...
		   struct otx2_sec_session_ipsec_ip *sess)
{
	...
	if (ipsec->proto == RTE_SELWRITY_IPSEC_SA_PROTO_ESP) {
		sess->partial_len += sizeof(struct rte_esp_hdr);
		sess->roundup_len = sizeof(struct rte_esp_tail);
	} else if (ipsec->proto == RTE_SELWRITY_IPSEC_SA_PROTO_AH) {
	...
	if (ipsec->direction == RTE_SELWRITY_IPSEC_SA_DIR_EGRESS) {
		cipher_xform = xform;
		auth_xform = xform->next;
	} else if (ipsec->direction == RTE_SELWRITY_IPSEC_SA_DIR_INGRESS) {
		auth_xform = xform;
			      ...
			      struct rte_selwrity_session *sec_sess)
{
	...
	priv->ipsec.dir = RTE_SELWRITY_IPSEC_SA_DIR_EGRESS;
	sess = &priv->ipsec.ip;
	...
	if (ipsec->mode == RTE_SELWRITY_IPSEC_SA_MODE_TUNNEL) {
		...
		if (ipsec->tunnel.type == RTE_SELWRITY_IPSEC_TUNNEL_IPV4) {
			     ...
			     struct rte_selwrity_session *sec_sess)
{
	...
	priv = get_sec_session_private_data(sec_sess);
	priv->ipsec.dir = RTE_SELWRITY_IPSEC_SA_DIR_INGRESS;
	sess = &priv->ipsec.ip;
			  ...
			  struct rte_selwrity_session *sess)
{
	...
	if (ipsec->direction == RTE_SELWRITY_IPSEC_SA_DIR_INGRESS)
			    ...
			    struct rte_mempool *mempool)
{
	...
	if (conf->action_type != RTE_SELWRITY_ACTION_TYPE_INLINE_PROTOCOL)
		return -ENOTSUP;
	...
	if (conf->protocol == RTE_SELWRITY_PROTOCOL_IPSEC)
			     ...
			     struct rte_selwrity_session *sess)
{
	...
	if (priv->ipsec.dir == RTE_SELWRITY_IPSEC_SA_DIR_INGRESS)
...
otx2_eth_sec_set_pkt_mdata(void *device __rte_unused,
			    struct rte_selwrity_session *session,
			    struct rte_mbuf *m, void *params __rte_unused)
{
	/* Set security session as the pkt metadata */
	*rte_selwrity_dynfield(m) = (rte_selwrity_dynfield_t)session;
...
static struct rte_selwrity_ops otx2_eth_sec_ops = {
...
otx2_eth_sec_init(struct rte_eth_dev *eth_dev)
{
	...
	if (!(dev->tx_offloads & DEV_TX_OFFLOAD_SELWRITY) &&
	    !(dev->rx_offloads & DEV_RX_OFFLOAD_SELWRITY))
		return 0;

	if (rte_selwrity_dynfield_register() < 0)
		return -rte_errno;
