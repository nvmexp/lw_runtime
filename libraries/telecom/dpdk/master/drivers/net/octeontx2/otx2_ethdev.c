	       ...
	       uint16_t qid, struct otx2_eth_rxq *rxq, struct rte_mempool *mp)
{
	...
	if (rxq->offloads & DEV_RX_OFFLOAD_SELWRITY)
		aq->rq.ipsech_ena = 1;
			...
			struct rte_mempool *mp)
{
	...
	/* Callwlating delta and freq mult between PTP HI clock and tsc.
	...
	if ((dev->rx_offloads & DEV_RX_OFFLOAD_TIMESTAMP) ||
		...
		rc = otx2_nix_raw_clock_tsc_colw(dev);
		if (rc) {
			otx2_err("Failed to callwlate delta and freq mult");
			goto fail;
		}
	}
...
nix_rx_offload_flags(struct rte_eth_dev *eth_dev)
{
	...
	if (dev->rx_offloads & DEV_RX_OFFLOAD_SELWRITY)
		flags |= NIX_RX_OFFLOAD_SELWRITY_F;
...
nix_tx_offload_flags(struct rte_eth_dev *eth_dev)
{
	...
	if (conf & DEV_TX_OFFLOAD_SELWRITY)
		flags |= NIX_TX_OFFLOAD_SELWRITY_F;
...
otx2_nix_form_default_desc(struct otx2_eth_txq *txq)
{
	...
	if (txq->dev->tx_offload_flags & NIX_TX_NEED_EXT_HDR) {
		...
		if (txq->dev->tx_offload_flags & NIX_TX_OFFLOAD_TSTAMP_F) {
			...
			/* To callwlate the offset for send_mem,
			 * send_hdr->w0.sizem1 * 2
			 */
...
nix_restore_queue_cfg(struct rte_eth_dev *eth_dev)
{
	 ...
	 * NOT be ilwoked for TXQs/RXQs in port_configure().
...
otx2_nix_configure(struct rte_eth_dev *eth_dev)
{
	 ...
	 * reconfigure and no queue configure ilwoked from application case.
	 */
	if (dev->configured == 1) {
...
otx2_nix_dev_start(struct rte_eth_dev *eth_dev)
{
	...
	/* MTU recallwlate should be avoided here if PTP is enabled by PF, as
	 * otx2_nix_recalc_mtu would be ilwoked during otx2_nix_ptp_enable_vf
	 * call below.
	 */
...
static const struct eth_dev_ops otx2_eth_dev_ops = {
	...
	.promislwous_enable       = otx2_nix_promisc_enable,
	.promislwous_disable      = otx2_nix_promisc_disable,
...
otx2_eth_dev_init(struct rte_eth_dev *eth_dev)
{
	...
	if (rc)
		goto free_mac_addrs;
	dev->tx_offload_capa |= DEV_TX_OFFLOAD_SELWRITY;
	dev->rx_offload_capa |= DEV_RX_OFFLOAD_SELWRITY;
