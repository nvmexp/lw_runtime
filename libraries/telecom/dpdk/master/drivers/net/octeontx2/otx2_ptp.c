...
/* This function callwlates two parameters "clk_freq_mult" and
 * "clk_delta" which is useful in deriving PTP HI clock from
 * timestamp counter (tsc) value.
 */
int
otx2_nix_raw_clock_tsc_colw(struct otx2_eth_dev *dev)
{
	uint64_t ticks_base = 0, ticks = 0, tsc = 0, t_freq;
	int rc, val;

	/* Callwlating the frequency at which PTP HI clock is running */
	...
	/* Callwlating the freq multiplier viz the ratio between the
	...
	/* Callwlating delta between PTP HI clock and tsc */
...
otx2_nix_timesync_adjust_time(struct rte_eth_dev *eth_dev, int64_t delta)
{
	...
	if (delta < PTP_FREQ_ADJUST && delta > -PTP_FREQ_ADJUST) {
		 ...
		 * freq mult callwlation for deriving PTP_HI from timestamp
...
otx2_nix_read_clock(struct rte_eth_dev *eth_dev, uint64_t *clock)
{
	 ...
	 * packet (which ilwoles mbox call) becomes very expensive, hence
	 * we should be able to derive PTP HI clock value from tsc by
	 * using freq_mult and clk_delta callwlated during configure stage.
	 */
