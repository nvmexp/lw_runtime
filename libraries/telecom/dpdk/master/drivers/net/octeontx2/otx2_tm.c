		   ...
		   uint64_t *mantissa_p, uint64_t *div_exp_p)
{
	...
	if (value <= SHAPER_RATE(0, 0, 0)) {
		/* Callwlate rate div_exp and mantissa using
		...
		/* Callwlate rate exponent and mantissa using
		 * the following formula:
		 *
		    ...
		    uint64_t *mantissa_p)
{
	...
	/* Callwlate burst exponent and mantissa using
	 * the following formula:
	 *
		     ...
		     struct shaper_params *pir)
{
	...
	/* Callwlate CIR exponent and mantissa */
	...
	/* Callwlate PIR exponent and mantissa */
	if (param->peak.rate)
	...
	/* Callwlate CIR burst exponent and mantissa */
	if (param->committed.size)
		cir->burst = shaper_burst_to_nix(param->committed.size,
						 &cir->burst_exponent,
						 &cir->burst_mantissa);

	/* Callwlate PIR burst exponent and mantissa */
	if (param->peak.size)
...
nix_tm_alloc_resources(struct rte_eth_dev *eth_dev, bool xmit_enable)
{
	...
	/* Trigger MTU recallwlate as SMQ needs MTU conf */
