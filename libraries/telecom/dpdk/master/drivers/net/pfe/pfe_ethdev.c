...
pfe_eth_open_cdev(struct pfe_eth_priv_s *priv)
{
	...
	if (pfe_cdev_fd < 0) {
		...
		priv->link_fd = PFE_CDEV_ILWALID_FD;
		return -1;
	}
...
pfe_eth_close_cdev(struct pfe_eth_priv_s *priv)
{
	...
	if (priv->link_fd != PFE_CDEV_ILWALID_FD) {
		close(priv->link_fd);
		priv->link_fd = PFE_CDEV_ILWALID_FD;
	}
}
...
static int
pfe_promislwous_enable(struct rte_eth_dev *dev)
{
	struct pfe_eth_priv_s *priv = dev->data->dev_private;
...
static int
pfe_promislwous_disable(struct rte_eth_dev *dev)
{
	struct pfe_eth_priv_s *priv = dev->data->dev_private;
...
static const struct eth_dev_ops ops = {
	...
	.promislwous_enable   = pfe_promislwous_enable,
	.promislwous_disable  = pfe_promislwous_disable,
