			    ...
			    int data1, __rte_unused int data2)
{
	 ...
	 * Lwrrently we don't have checking for tx pending
	 */
}
...
#define STATUS_LWMULATIVE_ARC_HIT       BIT(23)
	...
	if (getelw("PFE_INTR_SUPPORT")) {
		...
		if (pfe_cdev_fd < 0) {
			PFE_PMD_WARN("Unable to open PFE device file (%s).\n",
				     PFE_CDEV_PATH);
			pfe->cdev_fd = PFE_CDEV_ILWALID_FD;
			return -1;
		}
