...
do { \
	if ((fs)->mask.elem && ((fs)->val.elem != (__v))) \
		return rte_flow_error_set(e, EILWAL, RTE_FLOW_ERROR_TYPE_ITEM, \
		      ...
		      struct rte_flow_error *e)
{
	...
	if (fs->mask.ethtype) {
		if (fs->val.ethtype != RTE_ETHER_TYPE_VLAN &&
			...
			return rte_flow_error_set(e, EILWAL,
		 ...
		 struct rte_flow_error *e)
{
	 ...
	 */
	if (!fs.hitcnts)
		return rte_flow_error_set(e, EILWAL, RTE_FLOW_ERROR_TYPE_ACTION,
		    ...
		    struct rte_flow_error *e)
{
	...
	if (cxgbe_validate_filter(adap, &flow->fs)) {
		t4_os_free(flow);
		return rte_flow_error_set(e, EILWAL, RTE_FLOW_ERROR_TYPE_HANDLE,
				NULL,
	...
	if (cxgbe_verify_fidx(flow, fidx, 0)) {
		ret = rte_flow_error_set(e, EILWAL, RTE_FLOW_ERROR_TYPE_HANDLE,
