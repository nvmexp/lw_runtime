 ...
 * command or FW exelwtes it but signals an error.  In the latter case
			    ...
			    int size, void *rpl, bool sleep_ok, int timeout)
{
	 ...
	 * responsiveness for simple, fast exelwting commands but then back
 ...
 * this in the Firmware Version Format since it's colwenient.  Return
...
#define ADVERT_MASK (V_FW_PORT_CAP32_SPEED(M_FW_PORT_CAP32_SPEED) | \
		     FW_PORT_CAP32_ANEG)
/**
 *     fwcaps16_to_caps32 - colwert 16-bit Port Capabilities to 32-bits
...
/**
 *     fwcaps32_to_caps16 - colwert 32-bit Port Capabilities to 16-bits
		  ...
		  struct link_config *lc)
{
	...
	/* Colwert Common Code Forward Error Control settings into the
...
void t4_intr_enable(struct adapter *adapter)
{
	...
	t4_write_reg(adapter, A_SGE_INT_ENABLE3, F_ERR_CPL_EXCEED_IQE_SIZE |
		     F_ERR_ILWALID_CIDX_INC | F_ERR_CPL_OPCODE_0 |
 ...
 * stored conselwtively so the space needed is @nmac * 6 bytes.
 ...
 * @rss_size: size of RSS table slice associated with this VI
 *
 * Backwards compatible and colwieniance routine to allocate a Virtual
 ...
 * @promisc: 1 to enable promislwous mode, 0 to disable it, -1 no change
		      ...
		      fw_port_cap32_t acaps)
{
	 ...
	 * tells us the Link is lwrrently advertising.
	 */
		      ...
		      unsigned int *pbar2_qid)
{
	...
	/*
	 * Callwlate the basics of the BAR2 SGE Queue register area:
...
int t4_init_tp_params(struct adapter *adap)
{
	...
	/*
	 * Now that we have TP_VLAN_PRI_MAP cached, we can callwlate the field
...
/**
 * t4_filter_field_shift - callwlate filter field shift
 * @adap: the adapter
 ...
 *     perform appropriate byte order colwersions.
 *
		      ...
		      u32 len, void *hbuf, int dir)
{
	...
	/* It's colwenient to be able to handle lengths which aren't a
	...
	/* Callwlate our initial PCI-E Memory Window Position and Offset into
	 * that Window.
	 */
	 ...
	 * Window ilwoke the standard adapter Big-Endian to PCI-E Link
