 ...
 * per mbuf buffer).  We lwrrently only support two sizes for 1500- and
...
/**
 * sgl_len - callwlates the size of an SGL of the given capacity
 * @n: the number of SGL entries
 *
 * Callwlates the number of flits needed for a scatter/gather list that
...
static inline unsigned int sgl_len(unsigned int n)
{
	 ...
	 *
	 * The following callwlation incorporates all of the above.  It's
...
/**
 * calc_tx_flits - callwlate the number of flits for a packet Tx WR
 * @m: the packet
					    ...
					    struct adapter *adap)
{
	...
	/* callwlate the number of flits required for coalescing this packet
...
{
	q->lwr_desc = (const __be64 *)((const char *)q->lwr_desc + q->iqe_len);
			     ...
			     struct rte_mbuf **rx_pkts)
{
	...
	while (likely(budget_left)) {
		...
		rc = (const struct rsp_ctrl *)
		     ((const char *)q->lwr_desc + (q->iqe_len - sizeof(*rc)));
		...
		if (likely(rsp_type == X_RSPD_TYPE_FLBUF)) {
			...
			while (stat_pidx_diff && budget_left) {
				...
				const struct rss_header *rss_hdr =
					(const void *)q->lwr_desc;
				const struct cpl_rx_pkt *cpl =
					(const void *)&q->lwr_desc[1];
				struct rte_mbuf *pkt, *npkt;
				u32 len, bufsz;

				rc = (const struct rsp_ctrl *)
				     ((const char *)q->lwr_desc +
				      (q->iqe_len - sizeof(*rc)));
			...
			ret = q->handler(q, q->lwr_desc, NULL);
		} else {
		     ...
		     struct rte_mempool *mp, int queue_id, int socket_id)
{
	 ...
	 * get ilwolved with and it's different per chip and this is almost
