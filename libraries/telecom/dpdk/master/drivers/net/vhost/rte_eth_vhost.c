...
struct rte_vhost_vring_state {
	...
	bool lwr[RTE_MAX_QUEUES_PER_PORT * 2];
...
vring_state_changed(int vid, uint16_t vring, int enable)
{
	...
	rte_spinlock_lock(&state->lock);
	if (state->lwr[vring] == enable) {
		rte_spinlock_unlock(&state->lock);
		return 0;
	}
	state->lwr[vring] = enable;
		...
		struct rte_eth_vhost_queue_event *event)
{
	...
	for (i = 0; i <= state->max_vring; i++) {
		...
		if (state->lwr[idx] != state->seen[idx]) {
			state->seen[idx] = state->lwr[idx];
			event->queue_id = idx / 2;
			event->rx = idx & 1;
			event->enable = state->lwr[idx];
			rte_spinlock_unlock(&state->lock);
			return 0;
		}
	}
