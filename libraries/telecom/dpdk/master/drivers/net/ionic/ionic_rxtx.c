		...
		const struct rte_eth_txconf *tx_conf)
{
	...
	if (!rte_is_power_of_2(nb_desc) || nb_desc < IONIC_MIN_RING_DESC)
		return -EILWAL; /* or use IONIC_DEFAULT_RING_DESC */
		...
		uint64_t offloads __rte_unused, bool not_xmit_more)
{
	 ...
	 * callwlated with IP length set to zero.  HW will later
...
ionic_rx_empty(struct ionic_queue *q)
{
	...
	struct ionic_desc_info *lwr;
	struct rte_mbuf *mbuf;

	while (q->tail_idx != q->head_idx) {
		lwr = &q->info[q->tail_idx];
		mbuf = lwr->cb_arg;
		...
		struct rte_mempool *mp)
{
	...
	/*
	 * Note: the interface does not lwrrently support
		...
		void *service_cb_arg)
{
	...
	bool more;
	uint32_t lwrr_q_tail_idx, lwrr_cq_tail_idx;
	...
	while (color_match(cq_desc->pkt_type_color, cq->done_color)) {
		...
		do {
			...
			ionic_rx_clean(q, lwrr_q_tail_idx, lwrr_cq_tail_idx,
