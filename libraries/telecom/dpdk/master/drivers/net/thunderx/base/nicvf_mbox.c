...
static const char *mbox_message[NIC_MBOX_MSG_MAX] =  {
	[NIC_MBOX_MSG_ILWALID]            = "NIC_MBOX_MSG_ILWALID",
...
nicvf_mbox_config_rss(struct nicvf *nic)
{
	...
	while (lwr_idx < tot_len) {
		lwr_len = nicvf_min(tot_len - lwr_idx,
				(size_t)RSS_IND_TBL_LEN_PER_MBX_MSG);
		mbx.msg.msg = (lwr_idx > 0) ?
			NIC_MBOX_MSG_RSS_CFG_CONT : NIC_MBOX_MSG_RSS_CFG;
		mbx.rss_cfg.tbl_offset = lwr_idx;
		mbx.rss_cfg.tbl_len = lwr_len;
		for (i = 0; i < lwr_len; i++)
			mbx.rss_cfg.ind_tbl[i] = rss->ind_tbl[lwr_idx++];
