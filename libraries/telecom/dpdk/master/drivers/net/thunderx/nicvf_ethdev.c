...
/* Promislwous mode enabled by default in LMAC to VF 1:1 map configuration */
static int
...
nicvf_rss_ethdev_to_nic(struct nicvf *nic, uint64_t ethdev_rss)
{
	...
	if (nicvf_hw_cap(nic) & NICVF_CAP_TUNNEL_PARSING) {
		...
		if (ethdev_rss & ETH_RSS_LWGRE)
			nic_rss |= RSS_TUN_LWGRE_ENA;
	}
...
nicvf_rss_nic_to_ethdev(struct nicvf *nic,  uint64_t nic_rss)
{
	...
	if (nicvf_hw_cap(nic) & NICVF_CAP_TUNNEL_PARSING) {
		...
		if (nic_rss & RSS_TUN_LWGRE_ENA)
			ethdev_rss |= ETH_RSS_LWGRE;
	}
	return ethdev_rss;
}
...
static const struct eth_dev_ops nicvf_eth_dev_ops = {
	...
	.promislwous_enable       = nicvf_dev_promisc_enable,
