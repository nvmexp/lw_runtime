 ...
 * Performs any endian colwersion necessary.
 */
...
/*
 * Configure an Ethernet device. This function must be ilwoked first
 * before any other function in the Ethernet API. This function can
 * also be re-ilwoked when a device is in the stopped state.
 */
static int
...
nfp_net_promisc_enable(struct rte_eth_dev *dev)
{
	...
	PMD_DRV_LOG(DEBUG, "Promislwous mode enable");

	hw = NFP_NET_DEV_PRIVATE_TO_HW(dev->data->dev_private);

	if (!(hw->cap & NFP_NET_CFG_CTRL_PROMISC)) {
		PMD_INIT_LOG(INFO, "Promislwous mode not supported");
		return -ENOTSUP;
	}

	if (hw->ctrl & NFP_NET_CFG_CTRL_PROMISC) {
		PMD_DRV_LOG(INFO, "Promislwous mode already enabled");
		return 0;
	}
	...
	/*
	 * DPDK sets promislwous mode on just after this call assuming
	 * it can not fail ...
	 */
...
nfp_net_promisc_disable(struct rte_eth_dev *dev)
{
	...
	if ((hw->ctrl & NFP_NET_CFG_CTRL_PROMISC) == 0) {
		PMD_DRV_LOG(INFO, "Promislwous mode already disabled");
		return 0;
	}
	...
	/*
	 * DPDK sets promislwous mode off just before this call
 ...
 * performance improvement but it requires further ilwestigation. For example,
	...
	while (avail < nb_pkts) {
		...
		if (unlikely((mb->data_len + hw->rx_offset) >
			...
			RTE_LOG_DP(ERR, PMD,
				...
				" RX offset=%u bytes.\n"
				"\t\tLwrrently you just have %u bytes available"
...
static const struct eth_dev_ops nfp_net_eth_dev_ops = {
	...
	.promislwous_enable	= nfp_net_promisc_enable,
	.promislwous_disable	= nfp_net_promisc_disable,
...
nfp_cpp_bridge_serve_write(int sockfd, struct nfp_cpp *cpp)
{
	...
	while (count > 0) {
		...
		count -= pos;
		lwrlen = (count > NFP_CPP_MEMIO_BOUNDARY) ?
 ...
 * bound to the PMD. Lwrrently just one ioctl command is served and it
...
/*
 * This is the code to be exelwted by a service core. The CPP bridge interface
 ...
 * can be exelwted with a wrapper library and LD_LIBRARY being completely
