...
nfp_acquire_secondary_process_lock(struct nfp_pcie_user *desc)
{
	 ...
	 * being exelwted as non-root. This is not the case for the previous
	 * function nfp_acquire_process_lock which is ilwoked only when UIO
	 * driver is used because that implies root user.
	 */
	home_path = getelw("HOME");
