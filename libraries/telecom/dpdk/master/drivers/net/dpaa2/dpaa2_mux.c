			      ...
			      struct rte_flow_action *actions[])
{
	...
	/* Lwrrently taking only IP protocol as an extract type.
	...
	ret = dpdmux_set_lwstom_key(&dpdmux_dev->dpdmux, CMD_PRI_LOW,
				    dpdmux_dev->token,
			(uint64_t)(DPAA2_VADDR_TO_IOVA(key_cfg_iova)));
	if (ret) {
		DPAA2_PMD_ERR("dpdmux_set_lwstom_key failed: err(%d)", ret);
		goto creation_error;
	}
	...
	ret = dpdmux_add_lwstom_cls_entry(&dpdmux_dev->dpdmux, CMD_PRI_LOW,
					  dpdmux_dev->token, &flow->rule,
					  &dpdmux_action);
	if (ret) {
		DPAA2_PMD_ERR("dpdmux_add_lwstom_cls_entry failed: err(%d)",
			      ret);
		goto creation_error;
	}
