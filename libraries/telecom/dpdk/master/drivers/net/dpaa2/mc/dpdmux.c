 ...
 * created the object (not the one that lwrrently owns the object). The object
...
/**
 * dpdmux_set_lwstom_key - Set a custom classification key.
 *
 * This API is only available for DPDMUX instance created with
 * DPDMUX_METHOD_LWSTOM.  This API must be called before populating the
 * classification table using dpdmux_add_lwstom_cls_entry.
 *
 * Calls to dpdmux_set_lwstom_key remove all existing classification entries
 * that may have been added previously using dpdmux_add_lwstom_cls_entry.
 *
 * @mc_io:		Pointer to MC portal's I/O object
 * @cmd_flags:		Command flags; one or more of 'MC_CMD_FLAG_'
 * @token:		Token of DPSW object
 ...
 * @returns	'0' on Success; Error code otherwise.
 */
int dpdmux_set_lwstom_key(struct fsl_mc_io *mc_io,
			uint32_t cmd_flags,
			uint16_t token,
			uint64_t key_cfg_iova)
{
	struct dpdmux_set_lwstom_key *cmd_params;
	struct mc_command cmd = { 0 };

	/* prepare command */
	cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_SET_LWSTOM_KEY,
					  cmd_flags,
					  token);
	cmd_params = (struct dpdmux_set_lwstom_key *)cmd.params;
...
/**
 * dpdmux_add_lwstom_cls_entry - Adds a custom classification entry.
 *
 * This API is only available for DPDMUX instances created with
 * DPDMUX_METHOD_LWSTOM.  Before calling this function a classification key
 * composition rule must be set up using dpdmux_set_lwstom_key.
 *
 * @mc_io:	Pointer to MC portal's I/O object
 * @cmd_flags:	Command flags; one or more of 'MC_CMD_FLAG_'
 * @token: Token of DPSW object
 ...
 * @returns	'0' on Success; Error code otherwise.
 */
int dpdmux_add_lwstom_cls_entry(struct fsl_mc_io *mc_io,
		uint32_t cmd_flags,
		uint16_t token,
		struct dpdmux_rule_cfg *rule,
		struct dpdmux_cls_action *action)
{
	struct dpdmux_cmd_add_lwstom_cls_entry *cmd_params;
	struct mc_command cmd = { 0 };

	/* prepare command */
	cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_ADD_LWSTOM_CLS_ENTRY,
					  cmd_flags,
					  token);

	cmd_params = (struct dpdmux_cmd_add_lwstom_cls_entry *)cmd.params;
...
/**
 * dpdmux_remove_lwstom_cls_entry - Removes a custom classification entry.
 *
 * This API is only available for DPDMUX instances created with
 * DPDMUX_METHOD_LWSTOM.  The API can be used to remove classification
 * entries previously inserted using dpdmux_add_lwstom_cls_entry.
 *
 * @mc_io:	Pointer to MC portal's I/O object
 * @cmd_flags:	Command flags; one or more of 'MC_CMD_FLAG_'
 * @token: Token of DPSW object
 ...
 * @returns	'0' on Success; Error code otherwise.
 */
int dpdmux_remove_lwstom_cls_entry(struct fsl_mc_io *mc_io,
		uint32_t cmd_flags,
		uint16_t token,
		struct dpdmux_rule_cfg *rule)
{
	struct dpdmux_cmd_remove_lwstom_cls_entry *cmd_params;
	struct mc_command cmd = { 0 };

	/* prepare command */
	cmd.header = mc_encode_cmd_header(DPDMUX_CMDID_REMOVE_LWSTOM_CLS_ENTRY,
					  cmd_flags,
					  token);
	cmd_params = (struct dpdmux_cmd_remove_lwstom_cls_entry *)cmd.params;
	cmd_params->key_size = rule->key_size;
	cmd_params->key_iova = cpu_to_le64(rule->key_iova);
	cmd_params->mask_iova = cpu_to_le64(rule->mask_iova);
