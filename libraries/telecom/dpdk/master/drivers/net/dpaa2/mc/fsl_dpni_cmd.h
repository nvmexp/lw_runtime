...
#define DPNI_CMDID_ADD_LWSTOM_TPID		DPNI_CMD(0x275)
#define DPNI_CMDID_REMOVE_LWSTOM_TPID		DPNI_CMD(0x276)
#define DPNI_CMDID_GET_LWSTOM_TPID		DPNI_CMD(0x277)
...
struct dpni_cmd_add_lwstom_tpid {
	uint16_t	pad;
	uint16_t	tpid;
};

struct dpni_cmd_remove_lwstom_tpid {
	uint16_t	pad;
	uint16_t	tpid;
};

struct dpni_rsp_get_lwstom_tpid {
	uint16_t	tpid1;
	uint16_t	tpid2;
};
