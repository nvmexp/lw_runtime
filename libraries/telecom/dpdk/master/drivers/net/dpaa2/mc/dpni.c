...
/**
 * dpni_set_multicast_promisc() - Enable/disable multicast promislwous mode
 * @mc_io:	Pointer to MC portal's I/O object
 * @cmd_flags:	Command flags; one or more of 'MC_CMD_FLAG_'
 * @token:	Token of DPNI object
...
/**
 * dpni_get_multicast_promisc() - Get multicast promislwous mode
 * @mc_io:	Pointer to MC portal's I/O object
 * @cmd_flags:	Command flags; one or more of 'MC_CMD_FLAG_'
 * @token:	Token of DPNI object
...
/**
 * dpni_set_unicast_promisc() - Enable/disable unicast promislwous mode
 * @mc_io:	Pointer to MC portal's I/O object
 * @cmd_flags:	Command flags; one or more of 'MC_CMD_FLAG_'
 * @token:	Token of DPNI object
 * @en:		Set to '1' to enable; '0' to disable
...
/**
 * dpni_get_unicast_promisc() - Get unicast promislwous mode
 * @mc_io:	Pointer to MC portal's I/O object
 * @cmd_flags:	Command flags; one or more of 'MC_CMD_FLAG_'
 * @token:	Token of DPNI object
 * @en:		Returns '1' if enabled; '0' otherwise
...
/**
 * dpni_add_lwstom_tpid() - Configures a distinct Ethertype value
 ...
 * To replace an existing value use dpni_remove_lwstom_tpid() to remove
 * a previous TPID and after that use again the function.
 *
 * Return:	'0' on Success; Error code otherwise.
 */
int dpni_add_lwstom_tpid(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
		uint16_t token, uint16_t tpid)
{
	struct dpni_cmd_add_lwstom_tpid *cmd_params;
	struct mc_command cmd = { 0 };

	/* prepare command */
	cmd.header = mc_encode_cmd_header(DPNI_CMDID_ADD_LWSTOM_TPID,
					  cmd_flags,
					  token);
	cmd_params = (struct dpni_cmd_add_lwstom_tpid *)cmd.params;
...
/**
 * dpni_remove_lwstom_tpid() - Removes a distinct Ethertype value added
 *		previously with dpni_add_lwstom_tpid()
 * @mc_io:	Pointer to MC portal's I/O object
 * @cmd_flags:	Command flags; one or more of 'MC_CMD_FLAG_'
 * @token:	Token of DPNI object
 * @tpid:	New value for TPID
 *
 * Use this function when a TPID value added with dpni_add_lwstom_tpid() needs
 * to be replaced.
 *
 * Return:	'0' on Success; Error code otherwise.
 */
int dpni_remove_lwstom_tpid(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
		uint16_t token, uint16_t tpid)
{
	struct dpni_cmd_remove_lwstom_tpid *cmd_params;
	struct mc_command cmd = { 0 };

	/* prepare command */
	cmd.header = mc_encode_cmd_header(DPNI_CMDID_REMOVE_LWSTOM_TPID,
					  cmd_flags,
					  token);
	cmd_params = (struct dpni_cmd_remove_lwstom_tpid *)cmd.params;
	cmd_params->tpid = cpu_to_le16(tpid);
...
/**
 * dpni_get_lwstom_tpid() - Returns custom TPID (vlan tags) values configured
 ...
 * Return:	'0' on Success; Error code otherwise.
 */
int dpni_get_lwstom_tpid(struct fsl_mc_io *mc_io, uint32_t cmd_flags,
		uint16_t token, struct dpni_lwstom_tpid_cfg *tpid)
{
	struct dpni_rsp_get_lwstom_tpid *rsp_params;
	struct mc_command cmd = { 0 };
	int err;

	/* prepare command */
	cmd.header = mc_encode_cmd_header(DPNI_CMDID_GET_LWSTOM_TPID,
					  cmd_flags,
					  token);
	...
	rsp_params = (struct dpni_rsp_get_lwstom_tpid *)cmd.params;
