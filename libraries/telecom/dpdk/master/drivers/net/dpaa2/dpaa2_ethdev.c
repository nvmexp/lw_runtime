		      ...
		      uint16_t tpid)
{
	...
	ret = dpni_add_lwstom_tpid(dpni, CMD_PRI_LOW,
				   priv->token, tpid);
	if (ret < 0)
		DPAA2_PMD_INFO("Unable to set vlan tpid = %d", ret);
	/* if already configured tpids, remove them first */
	if (ret == -EBUSY) {
		struct dpni_lwstom_tpid_cfg tpid_list = {0};

		ret = dpni_get_lwstom_tpid(dpni, CMD_PRI_LOW,
				   priv->token, &tpid_list);
		if (ret < 0)
			goto fail;
		ret = dpni_remove_lwstom_tpid(dpni, CMD_PRI_LOW,
...
dpaa2_dev_allmulticast_disable(struct rte_eth_dev *dev)
{
	...
	/* must remain on for all promislwous */
...
dpaa2_dev_set_link_up(struct rte_eth_dev *dev)
{
	...
	/* Check if DPNI is lwrrently enabled */
		...
		const struct rte_event_eth_rx_adapter_queue_conf *queue_conf)
{
	...
	if (queue_conf->ev.sched_type == RTE_SCHED_TYPE_ORDERED &&
		...
		if (getelw("DPAA2_STRICT_ORDERING_ENABLE")) {
			ocfg.oloe = 0;
...
static struct eth_dev_ops dpaa2_ethdev_ops = {
	...
	.promislwous_enable   = dpaa2_dev_promislwous_enable,
	.promislwous_disable  = dpaa2_dev_promislwous_disable,
...
dpaa2_dev_init(struct rte_eth_dev *eth_dev)
{
	...
	if (attr.options & DPNI_OPT_LWSTOM_CG)
	 ...
	 * Lwrrently it is assumed that DPAA2 driver has byte code by some
	 * mean and same if offloaded to H/W.
	 */
	if (getelw("DPAA2_ENABLE_SOFT_PARSER")) {
