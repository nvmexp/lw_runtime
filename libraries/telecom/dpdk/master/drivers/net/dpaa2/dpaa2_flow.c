			 ...
			 int *device_configured)
{
	...
	/* TODO: Lwrrently upper bound of range parameter is not implemented */
...
{
	struct rte_flow *lwrr = LIST_FIRST(&priv->flows);
	...
	while (lwrr) {
		if (lwrr->ipaddr_rule.ipaddr_type ==
			FLOW_NONE_IPADDR) {
			lwrr = LIST_NEXT(lwrr, next);
			continue;
		}
		...
		qos_index = lwrr->tc_id * priv->fs_entries +
			lwrr->tc_index;

		dpaa2_flow_qos_entry_log("Before update", lwrr, qos_index);

		if (priv->num_rx_tc > 1) {
			ret = dpni_remove_qos_entry(dpni, CMD_PRI_LOW,
					priv->token, &lwrr->qos_rule);
			if (ret) {
		...
		if (lwrr->ipaddr_rule.qos_ipsrc_offset >= 0) {
			RTE_ASSERT(qos_ipsrc_offset >=
				lwrr->ipaddr_rule.qos_ipsrc_offset);
			...
			memcpy(ipsrc_key,
				(char *)(size_t)lwrr->qos_rule.key_iova +
				lwrr->ipaddr_rule.qos_ipsrc_offset,
				size);
			memset((char *)(size_t)lwrr->qos_rule.key_iova +
				lwrr->ipaddr_rule.qos_ipsrc_offset,
				0, size);

			memcpy(ipsrc_mask,
				(char *)(size_t)lwrr->qos_rule.mask_iova +
				lwrr->ipaddr_rule.qos_ipsrc_offset,
				size);
			memset((char *)(size_t)lwrr->qos_rule.mask_iova +
				lwrr->ipaddr_rule.qos_ipsrc_offset,
				0, size);

			lwrr->ipaddr_rule.qos_ipsrc_offset = qos_ipsrc_offset;
		}

		if (lwrr->ipaddr_rule.qos_ipdst_offset >= 0) {
			RTE_ASSERT(qos_ipdst_offset >=
				lwrr->ipaddr_rule.qos_ipdst_offset);
			...
			lwrr->ipaddr_rule.qos_ipdst_offset = qos_ipdst_offset;
		}

		if (lwrr->ipaddr_rule.qos_ipsrc_offset >= 0) {
			RTE_ASSERT((size == NH_FLD_IPV4_ADDR_SIZE) ||
				(size == NH_FLD_IPV6_ADDR_SIZE));
			memcpy((char *)(size_t)lwrr->qos_rule.key_iova +
				lwrr->ipaddr_rule.qos_ipsrc_offset,
				ipsrc_key,
				size);
			memcpy((char *)(size_t)lwrr->qos_rule.mask_iova +
				lwrr->ipaddr_rule.qos_ipsrc_offset,
				ipsrc_mask,
				size);
		}
		...
		if (extend >= 0)
			lwrr->qos_real_key_size += extend;

		lwrr->qos_rule.key_size = FIXED_ENTRY_SIZE;

		dpaa2_flow_qos_entry_log("Start update", lwrr, qos_index);

		if (priv->num_rx_tc > 1) {
			ret = dpni_add_qos_entry(dpni, CMD_PRI_LOW,
					priv->token, &lwrr->qos_rule,
					lwrr->tc_id, qos_index,
					0, 0);
			if (ret) {
		...
		if (lwrr->action != RTE_FLOW_ACTION_TYPE_QUEUE) {
			lwrr = LIST_NEXT(lwrr, next);
			continue;
		}

		dpaa2_flow_fs_entry_log("Before update", lwrr);
		extend = -1;

		ret = dpni_remove_fs_entry(dpni, CMD_PRI_LOW,
				priv->token, lwrr->tc_id, &lwrr->fs_rule);
		if (ret) {
			DPAA2_PMD_ERR("FS entry remove failed.");
			return -1;
		}

		if (lwrr->ipaddr_rule.fs_ipsrc_offset >= 0 &&
			tc_id == lwrr->tc_id) {
			RTE_ASSERT(fs_ipsrc_offset >=
				lwrr->ipaddr_rule.fs_ipsrc_offset);
			...
			memcpy(ipsrc_key,
				(char *)(size_t)lwrr->fs_rule.key_iova +
				lwrr->ipaddr_rule.fs_ipsrc_offset,
				size);
			memset((char *)(size_t)lwrr->fs_rule.key_iova +
				lwrr->ipaddr_rule.fs_ipsrc_offset,
				0, size);

			memcpy(ipsrc_mask,
				(char *)(size_t)lwrr->fs_rule.mask_iova +
				lwrr->ipaddr_rule.fs_ipsrc_offset,
				size);
			memset((char *)(size_t)lwrr->fs_rule.mask_iova +
				lwrr->ipaddr_rule.fs_ipsrc_offset,
				0, size);

			lwrr->ipaddr_rule.fs_ipsrc_offset = fs_ipsrc_offset;
		}

		if (lwrr->ipaddr_rule.fs_ipdst_offset >= 0 &&
			tc_id == lwrr->tc_id) {
			RTE_ASSERT(fs_ipdst_offset >=
				lwrr->ipaddr_rule.fs_ipdst_offset);
			...
			lwrr->ipaddr_rule.fs_ipdst_offset = fs_ipdst_offset;
		}

		if (lwrr->ipaddr_rule.fs_ipsrc_offset >= 0) {
			memcpy((char *)(size_t)lwrr->fs_rule.key_iova +
				lwrr->ipaddr_rule.fs_ipsrc_offset,
				ipsrc_key,
				size);
			memcpy((char *)(size_t)lwrr->fs_rule.mask_iova +
				lwrr->ipaddr_rule.fs_ipsrc_offset,
				ipsrc_mask,
				size);
		}
		...
		if (extend >= 0)
			lwrr->fs_real_key_size += extend;
		lwrr->fs_rule.key_size = FIXED_ENTRY_SIZE;

		dpaa2_flow_fs_entry_log("Start update", lwrr);

		ret = dpni_add_fs_entry(dpni, CMD_PRI_LOW,
				priv->token, lwrr->tc_id, lwrr->tc_index,
				&lwrr->fs_rule, &lwrr->action_cfg);
		if (ret) {
	...
	const struct rte_flow_attr *attr)
{
	...
	while (lwrr) {
		if (lwrr->tc_id == attr->group &&
			lwrr->tc_index == attr->priority) {
			DPAA2_PMD_ERR(
		       ...
		       struct rte_flow_error *error)
{
	...
	if (!ret) {
		...
		if (!lwrr) {
			...
			while (LIST_NEXT(lwrr, next))
				lwrr = LIST_NEXT(lwrr, next);
			LIST_INSERT_AFTER(lwrr, flow, next);
		}
	}
	return ret;
}
				   ...
				   struct rte_flow_error *error)
{
	...
	dpaa2_flow_control_log =
		getelw("DPAA2_FLOW_CONTROL_LOG");

	if (getelw("DPAA2_FLOW_CONTROL_MISS_FLOW")) {
		struct dpaa2_dev_priv *priv = dev->data->dev_private;

		dpaa2_flow_miss_flow_id =
			atoi(getelw("DPAA2_FLOW_CONTROL_MISS_FLOW"));
