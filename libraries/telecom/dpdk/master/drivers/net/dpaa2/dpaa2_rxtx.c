		  ...
		  int port_id)
{
	...
	int i = 0;
	void *hw_annot_addr;
	struct rte_mbuf *first_seg, *next_seg, *lwr_seg, *temp;
...
{
	struct rte_mbuf *lwr_seg = mbuf, *prev_seg, *mi, *temp;
	...
	for (i = 0; i < mbuf->nb_segs; i++) {
		...
		DPAA2_SET_FLE_ADDR(sge, DPAA2_MBUF_VADDR_TO_IOVA(lwr_seg));
		DPAA2_SET_FLE_OFFSET(sge, lwr_seg->data_off);
		sge->length = lwr_seg->data_len;
	...
	while (nb_pkts) {
		...
		for (loop = 0; loop < frames_to_send; loop++) {
			if (*dpaa2_seqn(*bufs)) {
				...
				DPAA2_PER_LCORE_DQRR_SIZE--;
				DPAA2_PER_LCORE_DQRR_HELD &= ~(1 << dqrr_index);
				*dpaa2_seqn(*bufs) = DPAA2_ILWALID_MBUF_SEQN;
			}
			     ...
			     struct qbman_eq_desc *eqdesc)
{
	...
	*dpaa2_seqn(m) = DPAA2_ILWALID_MBUF_SEQN;
}
		      ...
		      uint16_t nb_pkts)
{
	...
	/* todo - lwrrently we are using 1st TX queue only for loopback*/
