		    ...
		    struct rte_mbuf **elts)
{
	...
	for (pos = 0; pos < mcqe_n; ) {
		...
		_mm_storeu_si128((__m128i *)
				 ...
				 rxdf2);
#ifdef MLX5_PMD_SOFT_COUNTERS
		ilwalid_mask = _mm_set_epi64x(0,
					      (mcqe_n - pos) *
					      sizeof(uint16_t) * 8);
		ilwalid_mask = _mm_sll_epi64(ones, ilwalid_mask);
		byte_cnt = _mm_blend_epi16(_mm_srli_si128(mcqe1, 4),
					   mcqe2, 0xcc);
		byte_cnt = _mm_shuffle_epi8(byte_cnt, len_shuf_mask);
		byte_cnt = _mm_andnot_si128(ilwalid_mask, byte_cnt);
		...
		if (rxq->mark) {
			if (rxq->mcqe_format !=
				...
				ol_flags = _mm_or_si128(ol_flags,
					_mm_andnot_si128(ilwalid_mask,
							 fdir_flags));
				/* Mask out invalid entries. */
				fdir_id_flags = _mm_andnot_si128(ilwalid_mask,
								 fdir_id_flags);
		...
		/* Move to next CQE and ilwalidate consumed CQEs. */
		if (!(pos & 0x7) && pos < mcqe_n) {
			if (pos + 8 < mcqe_n)
				rte_prefetch0((void *)(cq + pos + 8));
			mcq = (void *)(cq + pos);
			for (i = 0; i < 8; ++i)
				cq[ilw++].op_own = MLX5_CQE_ILWALIDATE;
		}
	}
	/* Ilwalidate the rest of CQEs. */
	for (; ilw < mcqe_n; ++ilw)
		cq[ilw].op_own = MLX5_CQE_ILWALIDATE;
#ifdef MLX5_PMD_SOFT_COUNTERS
...
/**
 * Callwlate packet type and offload flag for mbuf and store it.
 *
 * @param rxq
 *   Pointer to RX queue structure.
 * @param cqes[4]
			 ...
			 __m128i op_err, struct rte_mbuf **pkts)
{
	...
	if (rxq->mark) {
		...
		__m128i flow_tag, ilwalid_mask;

		flow_tag = _mm_and_si128(pinfo, pinfo_ft_mask);
		/* Check if flow tag is non-zero then set PKT_RX_FDIR. */
		ilwalid_mask = _mm_cmpeq_epi32(flow_tag, zero);
		ol_flags = _mm_or_si128(ol_flags,
					_mm_andnot_si128(ilwalid_mask,
							 fdir_flags));
		/* Mask out invalid entries. */
		fdir_id_flags = _mm_andnot_si128(ilwalid_mask, fdir_id_flags);
		/* Check if flow tag MLX5_FLOW_MARK_DEFAULT. */
		ol_flags = _mm_or_si128(ol_flags,
					_mm_andnot_si128(
	...
	/* Make pinfo has merged fields for ol_flags callwlation. */
		 ...
		 uint16_t pkts_n, uint64_t *err, uint64_t *comp)
{
	...
	for (pos = 0;
		...
		__m128i opcode, owner_mask, ilwalid_mask;
		__m128i comp_mask;
		__m128i mask;
#ifdef MLX5_PMD_SOFT_COUNTERS
		__m128i byte_cnt;
		...
		/* E.3 get mask for ilwalidated CQEs. */
		opcode = _mm_and_si128(op_own, opcode_check);
		ilwalid_mask = _mm_cmpeq_epi32(opcode_check, opcode);
		ilwalid_mask = _mm_packs_epi32(ilwalid_mask, zero);
		/* E.4 mask out beyond boundary. */
		ilwalid_mask = _mm_or_si128(ilwalid_mask, mask);
		/* E.5 merge ilwalid_mask with invalid owner. */
		ilwalid_mask = _mm_or_si128(ilwalid_mask, owner_mask);
		...
		comp_mask = _mm_andnot_si128(ilwalid_mask, comp_mask);
		...
		n = _mm_cvtsi128_si64(ilwalid_mask);
		...
		opcode = _mm_andnot_si128(ilwalid_mask, opcode);
