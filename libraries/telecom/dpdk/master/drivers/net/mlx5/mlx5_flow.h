...
#define MLX5_FLOW_LAYER_LWGRE (1u << 25)
...
#define MLX5_FLOW_LAYER_TUNNEL \
	(MLX5_FLOW_LAYER_VXLAN | MLX5_FLOW_LAYER_VXLAN_GPE | \
	 MLX5_FLOW_LAYER_GRE | MLX5_FLOW_LAYER_LWGRE | MLX5_FLOW_LAYER_MPLS | \
...
/*
 * When tunnel offload is active, all JUMP group ids are colwerted
 * using the same method. That colwersion is applied both to tunnel and
 * regular rule types.
...
/* colwert jump group to flow table ID in tunnel rules */
...
int mlx5_flow_validate_item_icmp6(const struct rte_flow_item *item,
				   ...
				   uint8_t target_protocol,
				   struct rte_flow_error *error);
int mlx5_flow_validate_item_lwgre(const struct rte_flow_item *item,
				  uint64_t item_flags,
				  uint8_t target_protocol,
				  struct rte_flow_error *error);
