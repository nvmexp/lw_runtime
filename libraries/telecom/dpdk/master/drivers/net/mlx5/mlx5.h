 ...
 * INT16_MAX.
 */
#define POOL_IDX_ILWALID UINT16_MAX
...
#define MLX5_LWRR_TIME_SEC	(rte_rdtsc() / rte_get_tsc_hz())
...
struct mlx5_flow_id_pool {
	...
	uint32_t *lwrr; /**< Pointer to the index to pop. */
...
struct mlx5_flow_rss_desc {
	...
	union {
		...
		const uint16_t *const_q; /**< Const pointer colwert. */
	};
};
...
int mlx5_getelw_int(const char *);
...
int mlx5_promislwous_enable(struct rte_eth_dev *dev);
int mlx5_promislwous_disable(struct rte_eth_dev *dev);
...
int mlx5_hairpin_queue_peer_update(struct rte_eth_dev *dev, uint16_t peer_queue,
				   ...
				   uint32_t direction);
int mlx5_hairpin_queue_peer_bind(struct rte_eth_dev *dev, uint16_t lwr_queue,
				 struct rte_hairpin_peer_info *peer_info,
				 uint32_t direction);
int mlx5_hairpin_queue_peer_unbind(struct rte_eth_dev *dev, uint16_t lwr_queue,
				   uint32_t direction);
