...
mlx5_ipool_grow(struct mlx5_indexed_pool *pool)
{
	...
	trunk->prev = TRUNK_ILWALID;
	trunk->next = TRUNK_ILWALID;
	MLX5_ASSERT(pool->free_list == TRUNK_ILWALID);
	pool->free_list = idx;
...
mlx5_ipool_malloc(struct mlx5_indexed_pool *pool, uint32_t *idx)
{
	...
	if (pool->free_list == TRUNK_ILWALID) {
	...
	if (!trunk->free) {
		...
		if (trunk->next != TRUNK_ILWALID)
			pool->trunks[trunk->next]->prev = TRUNK_ILWALID;
		trunk->prev = TRUNK_ILWALID;
		trunk->next = TRUNK_ILWALID;
#ifdef POOL_DEBUG
		pool->trunk_empty++;
