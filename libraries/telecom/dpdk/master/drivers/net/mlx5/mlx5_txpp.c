...
mlx5_txpp_alloc_pp_index(struct mlx5_dev_ctx_shared *sh)
{
	...
	if (sh->txpp.test) {
		...
		/* Colwert packets per second into kilobits. */
...
mlx5_txpp_fill_cqe_rearm_queue(struct mlx5_dev_ctx_shared *sh)
{
	...
	for (i = 0; i < MLX5_TXPP_REARM_CQ_SIZE; i++) {
		cqe->op_own = (MLX5_CQE_ILWALID << 4) | MLX5_CQE_OWNER_MASK;
		++cqe;
	}
}
...
mlx5_txpp_update_timestamp(struct mlx5_dev_ctx_shared *sh)
{
	...
	ts = mlx5_txpp_colwert_rx_ts(sh, ts);
...
/* Ilwoked periodically on Rearm Queue completions. */
void
...
mlx5_txpp_xstats_jitter(struct mlx5_dev_txpp *txpp)
{
	...
	/* We have two neighbor reports, callwlate the jitter. */
	dts = tsa1.ts - tsa0.ts;
