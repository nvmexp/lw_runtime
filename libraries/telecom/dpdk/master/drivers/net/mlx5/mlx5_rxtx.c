 ...
 * out due to if conditions can be explicitly callwlated at compile time.
		...
		unsigned int olx)
{
	 ...
	 * Only lwstomized tunnels may ask for.
	 */
...
/**
 * Colwert the Checksum offloads to Verbs.
 *
 * @param buf
 *   Pointer to the mbuf.
 *
 * @return
 *   Colwerted checksum flags.
 */
 ...
 *
 * @return
 *   0 as success, -EILWAL as failure.
 */
 ...
 * @return
 *   The number of used rx descriptor.
 *   -EILWAL if the queue is invalid
 */
uint32_t
	...
	do {
		 ...
		 */
			...
			if (MLX5_CQE_FORMAT(op_own) == MLX5_COMPRESSED) {
				...
				*mcqe = &(*mc)[0];
				zip->ai = 1;
				/* Prefetch all to be ilwalidated */
				idx = zip->ca;
				end = zip->cq_ci;
				while (idx != end) {
	       ...
	       volatile struct mlx5_mini_cqe8 *mcqe)
{
	...
	if (rxq->hw_timestamp) {
		...
		if (rxq->rt_timestamp)
			ts = mlx5_txpp_colwert_rx_ts(rxq->sh, ts);
...
{
	/*
	 * The HW callwlates only the TCP payload checksum, need to complete
			  ...
			  unsigned int olx __rte_unused)
{
	...
	do {
		...
		if (unlikely(ret != MLX5_CQE_STATUS_SW_OWN)) {
			...
			/*
			 * Some error oclwrred, try to restart.
			...
			if (unlikely(ret < 0)) {
				/*
				 * Some error oclwrred on queue error
		  ...
		  unsigned int olx)
{
	...
	/*
	 * Callwlate and set check sum flags first, dword field
	...
	/*
	 * Callwlate and set Software Parser offsets and flags.
		  ...
		  unsigned int olx)
{
	...
	/*
	 * Callwlate and set check sum flags first, uint32_t field
	 * in segment may be shared with Software Parser flags.
	 */
	csum = MLX5_TXOFF_CONFIG(CSUM) ? txq_ol_cksum_to_cs(loc->mbuf) : 0;
	if (tso) {
...
dseg_done:
	/* Callwlate actual segments used from the dseg pointer. */
		      ...
		      unsigned int olx)
{
	if (MLX5_TXOFF_CONFIG(TXPP) &&
		...
		/* Colwert the timestamp into completion to wait. */
		ts = *RTE_MBUF_DYNFIELD(loc->mbuf, txq->ts_offset, uint64_t *);
		wci = mlx5_txpp_colwert_tx_ts(txq->sh, ts);
		if (unlikely(wci < 0))
 ...
 * @return
 *   MLX5_TXCMP_CODE_EXIT - sending is done or impossible.
 *   MLX5_TXCMP_CODE_ERROR - some unrecoverable error oclwrred.
 * Local context variables partially updated.
 */
static __rte_always_inline enum mlx5_txcmp_code
			...
			unsigned int olx)
{
	...
	/*
	 * Callwlate data length to be inlined to estimate
			    ...
			    unsigned int olx)
{
	...
	/*
	 * First callwlate data length to be inlined
		  ...
		  unsigned int olx)
{
	...
	for (;;) {
		...
		/*
		 * First callwlate the WQE size to check
 ...
 * @param slen
 *   Aclwmulated statistics, bytes sent
 * @param olx
 *   Configured Tx offloads mask. It is fully defined at
 *   compile time and may be used for optimization.
 *
 ...
 * @param slen
 *   Aclwmulated statistics, data bytes sent.
 * @param wqem
...
send_loop:
	...
	/*
	 * Callwlate the number of available resources - elts and WQEs.
...
enter_send_single:
	...
	/* Take a shortlwt if nothing is sent. */
