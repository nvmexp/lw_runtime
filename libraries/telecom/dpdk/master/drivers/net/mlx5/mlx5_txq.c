 ...
 * all ilwolved mbufs are freed from elts/WQ.
 *
 * @param dev
...
/**
 * Callwlate the total number of WQEBB for Tx queue.
 *
...
/**
 * Callwlate the maximal inline data size for Tx queue.
 *
 * @param txq_ctrl
 *   Pointer to Tx queue control structure.
 *
...
txq_calc_inline_max(struct mlx5_txq_ctrl *txq_ctrl)
{
	...
	if (!wqe_size)
		return 0;
	/*
	 * This callwlation is derived from tthe source of
