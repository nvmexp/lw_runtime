...
mlx5_flow_counters_mng_init(struct mlx5_dev_ctx_shared *sh)
{
	...
	sh->cmng.max_id = -1;
	sh->cmng.last_pool_idx = POOL_IDX_ILWALID;
		     ...
		     const struct mlx5_dev_config *config)
{
	...
	for (retry = 0; retry < MLX5_ALLOC_UAR_RETRY; ++retry) {
		...
		if (!sh->tx_uar &&
			...
			/*
			 * In some elwironments like virtual machine
 ...
 */
int
mlx5_getelw_int(const char *name)
{
	const char *val = getelw(name);
...
mlx5_args_check(const char *key, const char *val, void *opaque)
{
	...
	if (strcmp(MLX5_RXQ_CQE_COMP_EN, key) == 0) {
		...
		DRV_LOG(WARNING, "%s: deprecated parameter,"
				 " colwerted to txq_inline_max", key);
		...
		DRV_LOG(WARNING, "%s: deprecated parameter,"
				 " colwerted to txq_inline_mpw", key);
		config->txq_inline_mpw = tmp;
...
{
	mlx5_common_init();
	/* Build the static tables for Verbs colwersion. */
