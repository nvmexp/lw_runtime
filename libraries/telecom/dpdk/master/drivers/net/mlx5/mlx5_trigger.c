 ...
 * @param lwrrent_info
 *   Pointer to the input peer information, not used lwrrently.
 * @param peer_info
 ...
 */
int
mlx5_hairpin_queue_peer_bind(struct rte_eth_dev *dev, uint16_t lwr_queue,
			     struct rte_hairpin_peer_info *peer_info,
			     uint32_t direction)
{
	int ret = 0;
	 ...
	 */
	if (peer_info->peer_q != lwr_queue) {
		rte_errno = EILWAL;
		DRV_LOG(ERR, "port %u queue %d and peer queue %d mismatch",
			dev->data->port_id, lwr_queue, peer_info->peer_q);
		return -rte_errno;
	}
	if (direction != 0) {
		struct mlx5_txq_ctrl *txq_ctrl;
		...
		txq_ctrl = mlx5_txq_get(dev, lwr_queue);
		if (txq_ctrl == NULL) {
			rte_errno = EILWAL;
			DRV_LOG(ERR, "Failed to get port %u Tx queue %d",
				dev->data->port_id, lwr_queue);
			return -rte_errno;
		}
		if (txq_ctrl->type != MLX5_TXQ_TYPE_HAIRPIN) {
			rte_errno = EILWAL;
			DRV_LOG(ERR, "port %u queue %d not a hairpin Txq",
				dev->data->port_id, lwr_queue);
			mlx5_txq_release(dev, lwr_queue);
			return -rte_errno;
		}
		if (txq_ctrl->obj == NULL || txq_ctrl->obj->sq == NULL) {
			rte_errno = ENOMEM;
		...
		if (peer_info->tx_explicit !=
			...
			DRV_LOG(ERR, "port %u Tx queue %d and peer Tx rule mode"
				" mismatch", dev->data->port_id, lwr_queue);
			mlx5_txq_release(dev, lwr_queue);
			return -rte_errno;
		}
		...
		rxq_ctrl = mlx5_rxq_get(dev, lwr_queue);
		if (rxq_ctrl == NULL) {
			rte_errno = EILWAL;
			DRV_LOG(ERR, "Failed to get port %u Rx queue %d",
				dev->data->port_id, lwr_queue);
		...
		if (rxq_ctrl->type != MLX5_RXQ_TYPE_HAIRPIN) {
			...
			DRV_LOG(ERR, "port %u queue %d not a hairpin Rxq",
				dev->data->port_id, lwr_queue);
			mlx5_rxq_release(dev, lwr_queue);
			return -rte_errno;
		}
		if (rxq_ctrl->obj == NULL || rxq_ctrl->obj->rq == NULL) {
			rte_errno = ENOMEM;
 ...
 */
int
mlx5_hairpin_queue_peer_unbind(struct rte_eth_dev *dev, uint16_t lwr_queue,
			       uint32_t direction)
{
	int ret = 0;
...
mlx5_hairpin_bind_single_port(struct rte_eth_dev *dev, uint16_t rx_port)
{
	...
	struct rte_hairpin_peer_info lwr;
	...
	for (i = 0; i != priv->txqs_n; i++) {
		...
		lwr.peer_q = rx_queue;
		lwr.qp_id = txq_ctrl->obj->sq->id;
		lwr.vhca_id = priv->config.hca_attr.vhca_id;
		lwr.tx_explicit = txq_ctrl->hairpin_conf.tx_explicit;
		lwr.manual_bind = txq_ctrl->hairpin_conf.manual_bind;
		/*
...
mlx5_hairpin_unbind_single_port(struct rte_eth_dev *dev, uint16_t rx_port)
{
	...
	uint32_t i;
	int ret;
	uint16_t lwr_port = priv->dev_data->port_id;
		...
		if (txq_ctrl->hairpin_conf.manual_bind == 0) {
			if (lwr_port != rx_port) {
				...
				DRV_LOG(ERR, "port %u and port %u are in"
					" auto-bind mode", lwr_port, rx_port);
				mlx5_txq_release(dev, i);
				return -rte_errno;
			} else {
				return 0;
...
mlx5_dev_start(struct rte_eth_dev *dev)
{
	...
	/* Set flags and context to colwert Rx timestamps. */
