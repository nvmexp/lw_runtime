 ...
 * (i.e. lwrrently rte_mem_page_size()).
 *
 * @param[in] size
...
err_secondary:
	...
	/*
	 * Some parameters ("tx_db_nc" in partilwlarly) are needed in
	...
	priv->domain_id = RTE_ETH_DEV_SWITCH_DOMAIN_ID_ILWALID;
	...
	if (priv->domain_id == RTE_ETH_DEV_SWITCH_DOMAIN_ID_ILWALID) {
		  ...
		  struct rte_pci_device *pci_dev)
{
	...
	MLX5_ASSERT(ns);
	/*
	 * Sort list to probe devices in natural order for users colwenience
...
static int
mlx5_config_doorbell_mapping_elw(const struct mlx5_dev_config *config)
{
	char *elw;
	int value;
	...
	elw = getelw(MLX5_SHUT_UP_BF);
	value = elw ? !!strcmp(elw, "0") : MLX5_ARG_UNSET;
	if (config->dbnc == MLX5_ARG_UNSET)
		setelw(MLX5_SHUT_UP_BF, MLX5_SHUT_UP_BF_DEFAULT, 1);
	else
		setelw(MLX5_SHUT_UP_BF,
		       config->dbnc == MLX5_TXDB_NCACHED ? "1" : "0", 1);
	return value;
}

static void
mlx5_restore_doorbell_mapping_elw(int value)
{
	MLX5_ASSERT(rte_eal_process_type() == RTE_PROC_PRIMARY);
	/* Restore the original environment variable state. */
	if (value == MLX5_ARG_UNSET)
		unsetelw(MLX5_SHUT_UP_BF);
	else
		setelw(MLX5_SHUT_UP_BF, value ? "1" : "0", 1);
}
		     ...
		     struct mlx5_dev_ctx_shared *sh)
{
	 ...
	 */
	dbmap_elw = mlx5_config_doorbell_mapping_elw(config);
	...
	if (sh->ctx) {
		...
		mlx5_restore_doorbell_mapping_elw(dbmap_elw);
	} else {
...
/**
 * Set device promislwous mode
 *
 * @param dev
 *   Pointer to Ethernet device structure.
 * @param enable
 *   0 - promislwous is disabled, otherwise - enabled
 *
 * @return
...
const struct eth_dev_ops mlx5_os_dev_ops = {
	...
	.promislwous_enable = mlx5_promislwous_enable,
	.promislwous_disable = mlx5_promislwous_disable,
