...
static struct mlx5_flow_tunnel_info tunnels_info[] = {
	...
	{
		.tunnel = MLX5_FLOW_LAYER_LWGRE,
		.ptype = RTE_PTYPE_TUNNEL_LWGRE,
	},
	{
			 ...
			 struct rte_flow_error *error)
{
	...
	for (i = 0; i != rss->queue_num; ++i) {
		...
		if (!(*priv->rxqs)[rss->queue[i]])
			return rte_flow_error_set
				(error, EILWAL, RTE_FLOW_ERROR_TYPE_ACTION_CONF,
			     ...
			     struct rte_flow_error *error)
{
	...
	else if ((item_flags & MLX5_FLOW_LAYER_LWGRE) &&
		  !(item_flags & MLX5_FLOW_LAYER_INNER_L2))
		return rte_flow_error_set(error, EILWAL,
					  RTE_FLOW_ERROR_TYPE_ITEM, item,
					  "L3 cannot follow an LWGRE layer.");
	if (!mask)
 ...
 */
int
mlx5_flow_validate_item_lwgre(const struct rte_flow_item *item,
			      uint64_t item_flags,
			      uint8_t target_protocol,
			      struct rte_flow_error *error)
{
	const struct rte_flow_item_lwgre *mask = item->mask;
	int ret;
	...
	ret = mlx5_flow_item_acceptable
		(item, (const uint8_t *)mask,
		 (const uint8_t *)&rte_flow_item_lwgre_mask,
		 sizeof(struct rte_flow_item_lwgre),
		 MLX5_ITEM_RANGE_NOT_ACCEPTED, error);
	if (ret < 0)
		return ret;
	return 0;
 ...
 * callwlates the size of memory required for device flow, allocates the memory,
 ...
 * This functionality used to run same exelwtion path for both shared & non
			      ...
			      struct rte_flow_error *error)
{
	...
	for (n = 0; actions[n].type != RTE_FLOW_ACTION_TYPE_END; n++) {
		...
		if (copied_n == *shared_n) {
			return rte_flow_error_set
				(error, EILWAL, RTE_FLOW_ERROR_TYPE_ACTION_NUM,
...
flow_get_prefix_layer_flags(struct mlx5_flow *dev_flow)
{
	...
	/* Colwert L3 layers with decap action. */
	...
	/* Colwert L4 layers with decap action.  */
		 ...
		 struct rte_flow_action actions_pre[])
{
	...
	for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++) {
		struct rte_flow_action **action_lwr = NULL;
		...
		if (!action_lwr)
			action_lwr = &actions_sfx;
		memcpy(*action_lwr, actions, sizeof(struct rte_flow_action));
		(*action_lwr)++;
	}
	...
	for (; items->type != RTE_FLOW_ITEM_TYPE_END; items++) {
		...
		case RTE_FLOW_ITEM_TYPE_VLAN:
			if (copy_vlan) {
				memcpy(sfx_items, items, sizeof(*sfx_items));
				/*
				 * Colwert to internal match item, it is used
 ...
 *   no list insertion oclwrred, flow is just created,
...
{
	struct mlx5_flow_workspace *lwrr;
	struct mlx5_flow_workspace *data;
	...
	lwrr = pthread_getspecific(key_workspace);
	if (!lwrr) {
		   ...
		   struct mlx5_flow_counter_pool *pool)
{
	...
	struct mlx5_counter_stats_raw *lwr = pool->raw_hw;
	struct mlx5_counter_stats_raw *prev = pool->raw;
	const uint64_t lwrr_time = MLX5_LWRR_TIME_SEC;
	const uint32_t time_delta = lwrr_time - pool->time_of_last_age_check;
	uint16_t expected = AGE_CANDIDATE;
	uint32_t i;

	pool->time_of_last_age_check = lwrr_time;
	for (i = 0; i < MLX5_COUNTERS_PER_POOL; ++i) {
		...
		if (__atomic_load_n(&age_param->state,
			...
			continue;
		if (lwr->data[i].hits != prev->data[i].hits) {
 ...
 *
 * If tunnel offload is disabled, all group ids colwerted to flow table
 * id using the standard method.
 * If tunnel offload is enabled, group id can be colwerted using the
 * standard or tunnel colwersion method. Group colwersion method
 * selection depends on flags in `grp_info` parameter:
 * - Internal (grp_info.external == 0) groups colwersion uses the
 *   standard method.
 * - Group ids in JUMP action colwerted with the tunnel colwersion.
 * - Group id in rule attribute colwersion depends on a rule type and
 *   group id value:
 *   ** non zero group attributes colwerted with the tunnel method
 *   ** zero group attribute in non-tunnel rule is colwerted using the
 *      standard method - there's only one root table
 *   ** zero group attribute in steer tunnel rule is colwerted with the
 ...
 *      id is colwerted with the tunnel colwersion method.
 *
 * @param[in] dev
 *   Port device
 * @param[in] tunnel
...
tunnel_element_release_miss(struct rte_eth_dev *dev, void *x)
{
	struct tunnel_db_element_release_ctx *ctx = x;
	RTE_SET_USED(dev);
	ctx->ret = rte_flow_error_set(ctx->error, EILWAL,
				      RTE_FLOW_ERROR_TYPE_HANDLE, NULL,
				      "invalid argument");
}
...
err:
	return rte_flow_error_set(err, EILWAL,
