...
#ifdef MLX5_PMD_SOFT_COUNTERS
		byte_cnt = vbic_u16(byte_cnt, ilwalid_mask);
			 ...
			 uint16x4_t op_err, struct rte_mbuf **pkts)
{
	...
	if (rxq->mark) {
		...
		uint32x4_t ilwalid_mask;

		/* Check if flow tag is non-zero then set PKT_RX_FDIR. */
		ilwalid_mask = vceqzq_u32(flow_tag);
		ol_flags = vorrq_u32(ol_flags,
				     vbicq_u32(fdir_flags, ilwalid_mask));
		/* Mask out invalid entries. */
		fdir_id_flags = vbicq_u32(fdir_id_flags, ilwalid_mask);
		/* Check if flow tag MLX5_FLOW_MARK_DEFAULT. */
		ol_flags = vorrq_u32(ol_flags,
				     vbicq_u32(fdir_id_flags,
		 ...
		 uint16_t pkts_n, uint64_t *err, uint64_t *comp)
{
	...
	for (pos = 0;
	     ...
	     pos += MLX5_VPMD_DESCS_PER_LOOP) {
		uint16x4_t op_own;
		uint16x4_t opcode, owner_mask, ilwalid_mask;
		...
		/* D.3 get mask for ilwalidated CQEs. */
		opcode = vand_u16(op_own, opcode_check);
		ilwalid_mask = vceq_u16(opcode_check, opcode);
		...
		ilwalid_mask = vorr_u16(ilwalid_mask, mask);
		/* D.5 merge ilwalid_mask with invalid owner. */
		ilwalid_mask = vorr_u16(ilwalid_mask, owner_mask);
		/* E.2 mask out invalid entries. */
		comp_mask = vbic_u16(comp_mask, ilwalid_mask);
		...
		n = __builtin_clzl(vget_lane_u64(vreinterpret_u64_u16(
				   ilwalid_mask), 0)) / (sizeof(uint16_t) * 8);
		nocmp_n += n;
		...
		opcode = vbic_u16(opcode, ilwalid_mask);
