...
{
	/* Colwert from DPDK RSS hash flags to NDIS hash flags */
...
static int
hn_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct hn_data *hv = dev->data->dev_private;

	hn_rndis_set_rxfilter(hv, NDIS_PACKET_TYPE_PROMISLWOUS);
	return hn_vf_promislwous_enable(dev);
}

static int
hn_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct hn_data *hv = dev->data->dev_private;
	uint32_t filter;
	...
	return hn_vf_promislwous_disable(dev);
}
				...
				uint32_t subchan)
{
	...
	err = hn_lws_alloc_subchans(hv, &subchan);
	if (err)
		return  err;
...
static const struct eth_dev_ops hn_eth_dev_ops = {
	...
	.promislwous_enable     = hn_dev_promislwous_enable,
	.promislwous_disable    = hn_dev_promislwous_disable,
