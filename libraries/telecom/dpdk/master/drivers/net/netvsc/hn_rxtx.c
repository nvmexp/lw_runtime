...
#define HN_LWS_SEND_MSG_SIZE \
	(sizeof(struct vmbus_chanpkt_hdr) + sizeof(struct hn_lws_rndis))
...
#define HN_NDIS_VLAN_INFO_ILWALID	0xffffffff
#define HN_NDIS_RXCSUM_INFO_ILWALID	0
#define HN_NDIS_HASH_INFO_ILWALID	0
			...
			void *opaque, void *obj, unsigned int idx)
{
	...
	txd->chim_index = LWS_CHIM_IDX_ILWALID;
...
{
	uint32_t index = LWS_CHIM_IDX_ILWALID;
	uint64_t slab = 0;
 ...
 *
 * returns:
 *  - -EILWAL              - offset outside of tx_descriptor pool.
...
static void
hn_lws_send_completed(struct rte_eth_dev *dev, uint16_t queue_id,
		      unsigned long xactid, const struct hn_lws_rndis_ack *ack)
{
	struct hn_data *hv = dev->data->dev_private;
	...
	if (likely(ack->status == LWS_STATUS_OK)) {
	...
	if (txd->chim_index != LWS_CHIM_IDX_ILWALID) {
...
static void
hn_lws_handle_comp(struct rte_eth_dev *dev, uint16_t queue_id,
		   const struct vmbus_chanpkt_hdr *pkt,
		   const void *data)
{
	const struct hn_lws_hdr *hdr = data;

	switch (hdr->type) {
	case LWS_TYPE_RNDIS_ACK:
		hn_lws_send_completed(dev, queue_id, pkt->xactid, data);
		break;
...
next:
	...
	/*
	 * Final fixup.
	 * - If there is no hash value, ilwalidate the hash info.
	 */
	if (!(mask & HN_RXINFO_HASHVAL))
		info->hash_info = HN_NDIS_HASH_INFO_ILWALID;
	return 0;
}
...
static void hn_rx_buf_free_cb(void *buf __rte_unused, void *opaque)
{
	...
	hn_lws_ack_rxbuf(rxb->chan, rxb->xactid);
}
		     ...
		     const struct hn_rxinfo *info)
{
	...
	if (info->vlan_info != HN_NDIS_VLAN_INFO_ILWALID) {
	...
	if (info->csum_info != HN_NDIS_RXCSUM_INFO_ILWALID) {
	...
	if (info->hash_info != HN_NDIS_HASH_INFO_ILWALID) {
			     ...
			     void *data, uint32_t dlen)
{
	...
	struct hn_rxinfo info = {
		.vlan_info = HN_NDIS_VLAN_INFO_ILWALID,
		.csum_info = HN_NDIS_RXCSUM_INFO_ILWALID,
		.hash_info = HN_NDIS_HASH_INFO_ILWALID,
	};
	int err;
...
static void
hn_lws_handle_rxbuf(struct rte_eth_dev *dev,
		    struct hn_data *hv,
...
{
	const struct vmbus_chanpkt_rxbuf *pkt;
	const struct hn_lws_hdr *lws_hdr = buf;
	...
	if (unlikely(vmbus_chanpkt_datalen(hdr) < sizeof(*lws_hdr))) {
		PMD_RX_LOG(ERR, "invalid receive lws RNDIS");
		return;
	}

	/* Make sure that this is a RNDIS message. */
	if (unlikely(lws_hdr->type != LWS_TYPE_RNDIS)) {
		PMD_RX_LOG(ERR, "lws type %u, not RNDIS",
			   lws_hdr->type);
		return;
	}
	...
	if (unlikely(pkt->rxbuf_id != LWS_RXBUF_SIG)) {
...
/*
 * Called when LWS inband events are received.
 * Send up a two part message with port_id and the LWS message
 * to the pipe to the netvsc-vf-event control thread.
 */
static void hn_lws_handle_notify(struct rte_eth_dev *dev,
				 const struct vmbus_chanpkt_hdr *pkt,
				 const void *data)
{
	const struct hn_lws_hdr *hdr = data;
	...
	case LWS_TYPE_VFASSOC_NOTE:
		hn_lws_handle_vfassoc(dev, pkt, data);
		break;

	default:
		PMD_DRV_LOG(INFO,
			    "got notify, lws type %u", hdr->type);
	}
}
...
retry:
		...
		case VMBUS_CHANPKT_TYPE_COMP:
			++tx_done;
			hn_lws_handle_comp(dev, queue_id, pkt, data);
			break;

		case VMBUS_CHANPKT_TYPE_RXBUF:
			hn_lws_handle_rxbuf(dev, hv, rxq, pkt, data);
			break;

		case VMBUS_CHANPKT_TYPE_INBAND:
			hn_lws_handle_notify(dev, pkt, data);
			break;
...
{
	struct hn_txdesc *txd = txq->agg_txd;
	struct hn_lws_rndis rndis;
	int ret;

	if (!txd)
		return 0;

	rndis = (struct hn_lws_rndis) {
		.type = LWS_TYPE_RNDIS,
		.rndis_mtype = LWS_RNDIS_MTYPE_DATA,
	...
	ret = hn_lws_send(txq->chan, VMBUS_CHANPKT_FLAG_RC,
		     ...
		     const struct rte_mbuf *m)
{
	...
	*pi_data = queue_id;
	...
	/* Colwert RNDIS packet message offsets */
		      ...
		      bool *need_sig)
{
	...
	if (txd->chim_size > 0)
		lws_rndis.chim_idx = txd->chim_index;
	else
		lws_rndis.chim_idx = LWS_CHIM_IDX_ILWALID;
	...
	PMD_TX_LOG(DEBUG, "port %u:%u tx %u segs %u size %u",
		   txq->port_id, txq->queue_id, txd->chim_index,
		   segs, lws_rndis.chim_sz);

	return hn_lws_send_sglist(txq->chan, sg, segs,
				  &lws_rndis, sizeof(lws_rndis),
