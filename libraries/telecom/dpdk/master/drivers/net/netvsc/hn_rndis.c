...
static int hn_lws_send_rndis_ctrl(struct vmbus_channel *chan,
				  const void *req, uint32_t reqlen)

{
	struct hn_lws_rndis lws_rndis = {
		.type = LWS_TYPE_RNDIS,
		.rndis_mtype = LWS_RNDIS_MTYPE_CTRL,
		.chim_idx = LWS_CHIM_IDX_ILWALID,
		.chim_sz = 0
	};
	struct vmbus_gpa sg;
	rte_iova_t addr;
	...
	return hn_lws_send_sglist(chan, &sg, 1,
				  &lws_rndis, sizeof(lws_rndis), 0U, NULL);
}
...
void hn_rndis_link_status(struct rte_eth_dev *dev, const void *msg)
{
	...
	case RNDIS_STATUS_TASK_OFFLOAD_LWRRENT_CONFIG:
			  ...
			  void *comp, uint32_t comp_len)
{
	...
	error = hn_lws_send_rndis_ctrl(chan, req, reqlen);
	if (error) {
...
static int hn_rndis_exelwte(struct hn_data *hv, uint32_t rid,
			    const void *req, uint32_t reqlen,
	       ...
	       void *odata, uint32_t odlen)
{
	...
	error = hn_rndis_exelwte(hv, rid, req, reqlen,
...
hn_rndis_halt(struct hn_data *hv)
{
	...
	/* No RNDIS completion; rely on LWS message send completion */
...
hn_rndis_set_rxfilter(struct hn_data *hv, uint32_t filter)
{
	...
	error = hn_rndis_set(hv, OID_GEN_LWRRENT_PACKET_FILTER,
...
static int hn_rndis_init(struct hn_data *hv)
{
	...
	error = hn_rndis_exelwte(hv, rid, req, sizeof(*req),
				 &comp, comp_len,
