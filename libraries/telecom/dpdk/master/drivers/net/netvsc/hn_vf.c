...
int hn_vf_add(struct rte_eth_dev *dev, struct hn_data *hv)
{
	...
	if (err == 0)
		hn_lws_set_datapath(hv, LWS_DATAPATH_VF);
...
static void hn_vf_remove(struct hn_data *hv)
{
	...
	if (!hn_vf_attached(hv)) {
		...
		hn_lws_set_datapath(hv, LWS_DATAPATH_SYNTHETIC);

		/* Give back ownership */
		rte_eth_dev_owner_unset(hv->vf_port, hv->owner.id);

		/* Stop transmission over VF */
		hv->vf_port = HN_ILWALID_PORT;
	}
}

/* Handle VF association message from host */
void
hn_lws_handle_vfassoc(struct rte_eth_dev *dev,
		      const struct vmbus_chanpkt_hdr *hdr,
		      const void *data)
{
	struct hn_data *hv = dev->data->dev_private;
	const struct hn_lws_vf_association *vf_assoc = data;

	if (unlikely(vmbus_chanpkt_datalen(hdr) < sizeof(*vf_assoc))) {
		PMD_DRV_LOG(ERR, "invalid vf association LWS");
		return;
	}
...
int hn_vf_promislwous_enable(struct rte_eth_dev *dev)
{
	VF_ETHDEV_FUNC_RET_STATUS(dev, rte_eth_promislwous_enable);
}

int hn_vf_promislwous_disable(struct rte_eth_dev *dev)
{
	VF_ETHDEV_FUNC_RET_STATUS(dev, rte_eth_promislwous_disable);
}
