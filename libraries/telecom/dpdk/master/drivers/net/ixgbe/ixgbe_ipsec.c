		...
		struct rte_mempool *mempool)
{
	...
	if (conf->ipsec.direction == RTE_SELWRITY_IPSEC_SA_DIR_INGRESS) {
		if (dev_conf->rxmode.offloads & DEV_RX_OFFLOAD_SELWRITY) {
		...
		if (dev_conf->txmode.offloads & DEV_TX_OFFLOAD_SELWRITY) {
...
ixgbe_crypto_capabilities_get(void *device __rte_unused)
{
	...
	return ixgbe_selwrity_capabilities;
}
...
ixgbe_crypto_enable_ipsec(struct rte_eth_dev *dev)
{
	...
	if (tx_offloads & DEV_TX_OFFLOAD_SELWRITY) {
...
static struct rte_selwrity_ops ixgbe_selwrity_ops = {
...
{
	struct rte_selwrity_ctx *ctx = NULL;

	if (ixgbe_crypto_capable(dev)) {
		ctx = rte_malloc("rte_selwrity_instances_ops",
				 sizeof(struct rte_selwrity_ctx), 0);
		if (ctx) {
			ctx->device = (void *)dev;
			ctx->ops = &ixgbe_selwrity_ops;
			ctx->sess_cnt = 0;
			dev->selwrity_ctx = ctx;
		} else {
			return -ENOMEM;
		}
	}
