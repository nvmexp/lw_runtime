...
static int ixgbe_dev_promislwous_enable(struct rte_eth_dev *dev);
static int ixgbe_dev_promislwous_disable(struct rte_eth_dev *dev);
...
static int ixgbevf_dev_promislwous_enable(struct rte_eth_dev *dev);
static int ixgbevf_dev_promislwous_disable(struct rte_eth_dev *dev);
 ...
 */
#define UPDATE_VF_STAT(reg, last, lwr)                          \
{                                                               \
	uint32_t latest = IXGBE_READ_REG(hw, reg);              \
	lwr += (latest - last) & UINT_MAX;                      \
	last = latest;                                          \
}

#define UPDATE_VF_STAT_36BIT(lsb, msb, last, lwr)                \
{                                                                \
	u64 new_lsb = IXGBE_READ_REG(hw, lsb);                   \
	u64 new_msb = IXGBE_READ_REG(hw, msb);                   \
	u64 latest = ((new_msb << 32) | new_lsb);                \
	lwr += (0x1000000000LL + latest - last) & 0xFFFFFFFFFLL; \
	last = latest;                                           \
}
...
static const struct eth_dev_ops ixgbe_eth_dev_ops = {
	...
	.promislwous_enable   = ixgbe_dev_promislwous_enable,
	.promislwous_disable  = ixgbe_dev_promislwous_disable,
...
static const struct eth_dev_ops ixgbevf_eth_dev_ops = {
	...
	.promislwous_enable   = ixgbevf_dev_promislwous_enable,
	.promislwous_disable  = ixgbevf_dev_promislwous_disable,
...
static const struct rte_ixgbe_xstats_name_off rte_ixgbe_macsec_strings[] = {
	...
	{"in_pkts_ok", offsetof(struct ixgbe_macsec_stats,
		in_pkts_ok)},
	{"in_pkts_ilwalid", offsetof(struct ixgbe_macsec_stats,
		in_pkts_ilwalid)},
...
/*
 * Ensure that all locks are released before first LWM or PHY access
 */
static void
...
eth_ixgbe_dev_init(struct rte_eth_dev *eth_dev, void *init_params __rte_unused)
{
...
#ifdef RTE_LIB_SELWRITY
	/* Initialize selwrity_ctx only for primary process*/
	 ...
	 * is called too soon after the kernel driver unbinding/binding oclwrs.
	 * The failure oclwrs in ixgbe_identify_phy_generic() for all devices,
	 ...
	 * failure is not known, and only oclwts when virtualisation features
...
eth_ixgbevf_dev_init(struct rte_eth_dev *eth_dev)
{
	...
	/*
	 * The VF reset operation returns the IXGBE_ERR_ILWALID_MAC_ADDR when
	 * the underlying PF driver has not assigned a MAC address to the VF.
	 * In this case, assign a random MAC address.
	 */
	if ((diag != IXGBE_SUCCESS) && (diag != IXGBE_ERR_ILWALID_MAC_ADDR)) {
			   ...
			   uint64_t *total_qprc, uint64_t *total_qprdc)
{
	...
	for (i = 0; i < 2; i++) {
		...
		macsec_stats->in_pkts_ilwalid +=
			IXGBE_READ_REG(hw, IXGBE_LSECRXILW(i));
		macsec_stats->in_pkts_notvalid +=
			IXGBE_READ_REG(hw, IXGBE_LSECRXLW(i));
	}
...
/* This function callwlates the number of xstats based on the current config */
static unsigned
...
static int
ixgbe_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint32_t fctrl;
...
static int
ixgbe_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint32_t fctrl;
...
/*
 * It exelwtes link_update after knowing an interrupt oclwrred.
 *
 * @param dev
 *  Pointer to struct rte_eth_dev.
 *
...
ixgbevf_dev_start(struct rte_eth_dev *dev)
{
	 ...
	 * initialization.
	 */
	if (err != IXGBE_SUCCESS && err != IXGBE_ERR_ILWALID_MAC_ADDR) {
...
uint32_t
ixgbe_colwert_vm_rx_mask_to_val(uint16_t rx_mask, uint32_t orig_val)
{
...
#define IXGBE_ILWALID_MIRROR_TYPE(mirror_type) \
		      ...
		      uint8_t rule_id, uint8_t on)
{
	...
	if (IXGBE_ILWALID_MIRROR_TYPE(mirror_conf->rule_type)) {
	...
	/* read  mirror control register and recallwlate it */
			   ...
			   uint16_t queue_idx, uint16_t tx_rate)
{
	...
	if (tx_rate != 0) {
		/* Callwlate the rate factor values to set */
...
colwert_protocol_type(uint8_t protocol_value)
{
			...
			struct ixgbe_5tuple_filter_info *filter_info)
{
	...
	case UINT8_MAX:
		...
		filter_info->proto =
			colwert_protocol_type(filter->proto);
		break;
	case 0:
...
{
	struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	uint32_t status = IXGBE_ERR_PHY_ADDR_ILWALID;
		     ...
		     uint16_t port)
{
	...
	lwr_port = (uint16_t)IXGBE_READ_REG(hw, IXGBE_VXLANCTRL);
...
static int
ixgbevf_dev_promislwous_enable(struct rte_eth_dev *dev)
{
	struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	int ret;
...
static int
ixgbevf_dev_promislwous_disable(struct rte_eth_dev *dev)
{
	struct ixgbe_hw *hw = IXGBE_DEV_PRIVATE_TO_HW(dev->data->dev_private);
	int ret;
