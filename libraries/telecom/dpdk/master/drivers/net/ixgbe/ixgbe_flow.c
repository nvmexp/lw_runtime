...
const struct rte_flow_item *next_no_void_pattern(
		...
		const struct rte_flow_item *lwr)
{
...
const struct rte_flow_action *next_no_void_action(
		...
		const struct rte_flow_action *lwr)
{
 ...
 *
 * Special case for flow action type RTE_FLOW_ACTION_TYPE_SELWRITY.
 *
 */
static int
			 ...
			 struct rte_flow_error *error)
{
	...
	if (!pattern) {
		rte_flow_error_set(error,
			EILWAL, RTE_FLOW_ERROR_TYPE_ITEM_NUM,
			NULL, "NULL pattern.");
		return -rte_errno;
	}
	...
	if (act->type == RTE_FLOW_ACTION_TYPE_SELWRITY) {
	...
	if (item->type == RTE_FLOW_ITEM_TYPE_ETH) {
		...
		if (item->type != RTE_FLOW_ITEM_TYPE_IPV4 &&
			...
			rte_flow_error_set(error,
			  EILWAL, RTE_FLOW_ERROR_TYPE_ITEM,
			  item, "Not supported by ntuple filter");
			  return -rte_errno;
		}
	}
			...
			struct rte_flow_error *error)
{
	...
	if (!item->spec || !item->mask) {
		memset(filter, 0, sizeof(struct ixgbe_l2_tunnel_conf));
		rte_flow_error_set(error, EILWAL, RTE_FLOW_ERROR_TYPE_ITEM,
			item, "Not supported by L2 tunnel filter");
		return -rte_errno;
	}
...
{
	const struct rte_flow_item *next =
		next_no_void_pattern(pattern, lwr);
	while (1) {
...
#define LWGRE_PROTOCOL 0x6558

/**
 * Parse the rule to see if it is a VxLAN or LWGRE flow director rule.
 * And get the flow director filter info BTW.
 * VxLAN PATTERN:
 * The first not void item must be ETH.
 * The second not void item must be IPV4/ IPV6.
 * The third not void item must be LWGRE.
 * The next not void item must be END.
 * LWGRE PATTERN:
 * The first not void item must be ETH.
 * The second not void item must be IPV4/ IPV6.
			       ...
			       struct rte_flow_error *error)
{
	...
	if (item->type != RTE_FLOW_ITEM_TYPE_ETH &&
	    ...
	    item->type != RTE_FLOW_ITEM_TYPE_LWGRE) {
		memset(rule, 0, sizeof(struct ixgbe_fdir_rule));
		rte_flow_error_set(error, EILWAL,
			RTE_FLOW_ERROR_TYPE_ITEM,
			item, "Not supported by fdir filter");
	...
	if (item->type == RTE_FLOW_ITEM_TYPE_IPV4 ||
		...
		/* Check if the next not void item is UDP or LWGRE. */
		item = next_no_void_pattern(pattern, item);
		if (item->type != RTE_FLOW_ITEM_TYPE_UDP &&
		    item->type != RTE_FLOW_ITEM_TYPE_LWGRE) {
			memset(rule, 0, sizeof(struct ixgbe_fdir_rule));
	...
	/* Get the LWGRE info */
	if (item->type == RTE_FLOW_ITEM_TYPE_LWGRE) {
		rule->ixgbe_fdir.formatted.tunnel_type =
				IXGBE_FDIR_LWGRE_TUNNEL_TYPE;
		...
		if (lwgre_mask->tni[0] &&
		    ((lwgre_mask->tni[0] != 0xFF) ||
		    (lwgre_mask->tni[1] != 0xFF) ||
		    (lwgre_mask->tni[2] != 0xFF))) {
			memset(rule, 0, sizeof(struct ixgbe_fdir_rule));
			rte_flow_error_set(error, EILWAL,
				RTE_FLOW_ERROR_TYPE_ITEM,
				item, "Not supported by fdir filter");
		...
		rte_memcpy(&rule->mask.tunnel_id_mask, lwgre_mask->tni,
			RTE_DIM(lwgre_mask->tni));
		...
		if (item->spec) {
			...
			if (lwgre_mask->protocol &&
			    lwgre_spec->protocol !=
			    rte_cpu_to_be_16(LWGRE_PROTOCOL)) {
				memset(rule, 0, sizeof(struct ixgbe_fdir_rule));
				rte_flow_error_set(error, EILWAL,
					RTE_FLOW_ERROR_TYPE_ITEM,
					item, "Not supported by fdir filter");
			...
			lwgre_spec->tni, RTE_DIM(lwgre_spec->tni));
		}
	}
		...
		struct rte_flow_error *error)
{
	...
	if (ret < 0) {
		rte_flow_error_set(error, EILWAL, RTE_FLOW_ERROR_TYPE_HANDLE,
