...
/* For callwlating memory required for FDIR filters */
...
fdir_set_input_mask_82599(struct rte_eth_dev *dev)
{
	...
	/*
	 * mask VM pool and DIPv6 since there are lwrrently not supported
				...
				uint16_t offset)
{
	...
	fdirctrl |= ((offset >> 1) /* colwert to word offset */
		<< IXGBE_FDIRCTRL_FLEX_SHIFT);
 ...
 *
 * The newer driver has optimised functions for callwlating bucket and
...
/*
 * Callwlate the hash value needed for signature-match filters. In the FreeBSD
 * driver, this is done by the optimised function
 * ixgbe_atr_compute_sig_hash_82599(). However that can't be used here as it
 * doesn't support callwlating a hash for an IPv6 filter.
 */
static uint32_t
 ...
 * added, and IPv6 support also added. The hash value is also pre-callwlated
 ...
 * base/ixgbe_82599.c, but uses a pre-callwlated hash value. It also supports
 ...
 *  @fdirhash: pre-callwlated hash value for the filter
 **/
static int
