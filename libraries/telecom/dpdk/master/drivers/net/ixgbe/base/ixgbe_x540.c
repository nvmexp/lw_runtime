...
/**
 * ixgbe_calc_eeprom_checksum_X540 - Callwlates and returns the checksum
 *
 ...
 * the specified function (CSR, PHY0, PHY1, LWM, Flash)
 **/
s32 ixgbe_acquire_swfw_sync_X540(struct ixgbe_hw *hw, u32 mask)
{
	u32 swmask = mask & IXGBE_GSSR_LWM_PHY_MASK;
	u32 fwmask = swmask << 5;
	...
	for (i = 0; i < timeout; i++) {
		...
		if (ixgbe_get_swfw_sync_semaphore(hw)) {
			DEBUGOUT("Failed to get LWM access and register semaphore, returning IXGBE_ERR_SWFW_SYNC\n");
			return IXGBE_ERR_SWFW_SYNC;
		}
		...
		/* Firmware lwrrently using resource (fwmask), hardware
		 * lwrrently using resource (hwmask), or other software
		 * thread lwrrently using resource (swmask)
		 */
		ixgbe_release_swfw_sync_semaphore(hw);
		msec_delay(5);
	}
	 ...
	 */
	if (ixgbe_get_swfw_sync_semaphore(hw)) {
		DEBUGOUT("Failed to get LWM semaphore and register semaphore while forcefully ignoring FW semaphore bit(s) and setting SW semaphore bit(s), returning IXGBE_ERR_SWFW_SYNC\n");
		return IXGBE_ERR_SWFW_SYNC;
	}
	swfw_sync = IXGBE_READ_REG(hw, IXGBE_SWFW_SYNC_BY_MAC(hw));
...
{
	u32 swmask = mask & (IXGBE_GSSR_LWM_PHY_MASK | IXGBE_GSSR_SW_MNG_SM);
	u32 swfw_sync;
...
STATIC s32 ixgbe_get_swfw_sync_semaphore(struct ixgbe_hw *hw)
{
	...
	if (status == IXGBE_SUCCESS) {
		...
		/*
		 * Release semaphores and return error if SW LWM semaphore
		 * was not granted because we don't have access to the EEPROM
		 */
		if (i >= timeout) {
			ERROR_REPORT1(IXGBE_ERROR_POLLING,
				"REGSMP Software LWM semaphore not granted.\n");
			ixgbe_release_swfw_sync_semaphore(hw);
