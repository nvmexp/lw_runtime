...
static bool ixgbe_probe_phy(struct ixgbe_hw *hw, u16 phy_addr)
{
	...
	if (hw->phy.type == ixgbe_phy_unknown) {
		...
		if (ext_ability &
			...
			hw->phy.type = ixgbe_phy_lw_unknown;
		else
...
{
	s32 status = IXGBE_ERR_PHY_ADDR_ILWALID;
	u16 phy_addr;
...
s32 ixgbe_identify_sfp_module_generic(struct ixgbe_hw *hw)
{
	...
	if (identifier != IXGBE_SFF_IDENTIFIER_SFP) {
		...
		if (status != IXGBE_SUCCESS)
		  ...
		  * 9   SFP_1g_lw_CORE0 - 82599-specific
		  * 10  SFP_1g_lw_CORE1 - 82599-specific
		  * 11  SFP_1g_sx_CORE0 - 82599-specific
		  * 12  SFP_1g_sx_CORE1 - 82599-specific
		  */
		if (hw->mac.type == ixgbe_mac_82598EB) {
			if (cable_tech & IXGBE_SFF_DA_PASSIVE_CABLE)
				hw->phy.sfp_type = ixgbe_sfp_type_da_lw;
			...
			if (cable_tech & IXGBE_SFF_DA_PASSIVE_CABLE) {
				if (hw->bus.lan_id == 0)
					hw->phy.sfp_type =
						     ixgbe_sfp_type_da_lw_core0;
				else
					hw->phy.sfp_type =
						     ixgbe_sfp_type_da_lw_core1;
				...
				if (hw->bus.lan_id == 0)
					hw->phy.sfp_type =
						ixgbe_sfp_type_1g_lw_core0;
				else
					hw->phy.sfp_type =
						ixgbe_sfp_type_1g_lw_core1;
		...
		if (comp_codes_10g == 0 &&
		    !(hw->phy.sfp_type == ixgbe_sfp_type_1g_lw_core1 ||
		      hw->phy.sfp_type == ixgbe_sfp_type_1g_lw_core0 ||
...
u64 ixgbe_get_supported_phy_sfp_layer_generic(struct ixgbe_hw *hw)
{
	...
	case ixgbe_phy_qsfp_passive_unknown:
		physical_layer = IXGBE_PHYSICAL_LAYER_SFP_PLUS_LW;
		break;
...
s32 ixgbe_identify_qsfp_module_generic(struct ixgbe_hw *hw)
{
	...
	if (comp_codes_10g & IXGBE_SFF_QSFP_DA_PASSIVE_CABLE) {
		...
		if (hw->bus.lan_id == 0)
			hw->phy.sfp_type = ixgbe_sfp_type_da_lw_core0;
		else
			hw->phy.sfp_type = ixgbe_sfp_type_da_lw_core1;
	} else if (comp_codes_10g & (IXGBE_SFF_10GBASESR_CAPABLE |
				     IXGBE_SFF_10GBASELR_CAPABLE)) {
		if (hw->bus.lan_id == 0)
					...
					u16 *data_offset)
{
	...
	if (sfp_type == ixgbe_sfp_type_da_act_lmt_core0 ||
	    ...
	    sfp_type == ixgbe_sfp_type_1g_lw_core0 ||
	    sfp_type == ixgbe_sfp_type_1g_sx_core0)
		sfp_type = ixgbe_sfp_type_srlr_core0;
	else if (sfp_type == ixgbe_sfp_type_da_act_lmt_core1 ||
		 sfp_type == ixgbe_sfp_type_1g_lx_core1 ||
		 sfp_type == ixgbe_sfp_type_1g_lha_core1 ||
		 sfp_type == ixgbe_sfp_type_1g_lw_core1 ||
...
/**
 * ixgbe_tn_check_overtemp - Checks if an overtemp oclwrred.
 * @hw: pointer to hardware structure
 *
