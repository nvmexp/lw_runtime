...
STATIC s32 ixgbe_reset_cs4227(struct ixgbe_hw *hw)
{
	...
	if (retry == IXGBE_CS4227_RETRIES) {
		ERROR_REPORT1(IXGBE_ERROR_ILWALID_STATE,
...
static s32 ixgbe_get_phy_id_fw(struct ixgbe_hw *hw)
{
	...
	if (!hw->phy.id || hw->phy.id == IXGBE_PHY_REVISION_MASK)
		return IXGBE_ERR_PHY_ADDR_ILWALID;
	return IXGBE_SUCCESS;
}
...
static s32 ixgbe_setup_fw_link(struct ixgbe_hw *hw)
{
	...
	if (hw->fc.strict_ieee && hw->fc.requested_mode == ixgbe_fc_rx_pause) {
		...
		return IXGBE_ERR_ILWALID_LINK_SETTINGS;
	}
...
s32 ixgbe_dmac_config_tcs_X550(struct ixgbe_hw *hw)
{
	...
	for (tc = 0; tc < IXGBE_DCB_MAX_TRAFFIC_CLASS; tc++) {
		...
		if (tc < hw->mac.dmac_config.num_tcs) {
			...
			/* Callwlate receive buffer threshold in kilobytes */
...
s32 ixgbe_get_phy_token(struct ixgbe_hw *hw)
{
	...
	if (token_cmd.hdr.cmd_or_resp.ret_status != FW_PHY_TOKEN_RETRY) {
		DEBUGOUT1("Host interface command returned 0x%08x , returning IXGBE_ERR_FW_RESP_ILWALID\n",
			  token_cmd.hdr.cmd_or_resp.ret_status);
		return IXGBE_ERR_FW_RESP_ILWALID;
	}
...
STATIC s32 ixgbe_supported_sfp_modules_X550em(struct ixgbe_hw *hw, bool *linear)
{
	...
	case ixgbe_sfp_type_da_lw_core0:
	case ixgbe_sfp_type_da_lw_core1:
		*linear = true;
		break;
	...
	case ixgbe_sfp_type_1g_lw_core0:
	case ixgbe_sfp_type_1g_lw_core1:
	default:
...
STATIC s32 ixgbe_get_lasi_ext_t_x550em(struct ixgbe_hw *hw, bool *lsc)
{
	...
	*lsc = false;
	...
	/* link connect/disconnect event oclwrred */
...
s32 ixgbe_init_phy_ops_X550em(struct ixgbe_hw *hw)
{
	...
	if (ret_val == IXGBE_ERR_SFP_NOT_SUPPORTED ||
	    ret_val == IXGBE_ERR_PHY_ADDR_ILWALID)
		return ret_val;
...
s32 ixgbe_reset_hw_X550em(struct ixgbe_hw *hw)
{
	...
	if (status == IXGBE_ERR_SFP_NOT_SUPPORTED ||
	    status == IXGBE_ERR_PHY_ADDR_ILWALID) {
...
STATIC s32 ixgbe_setup_ixfi_x550em_x(struct ixgbe_hw *hw)
{
	...
	if (status != IXGBE_SUCCESS)
		return status;
	reg_val |= IXGBE_KRM_RX_TRN_LINKUP_CTRL_COLW_WO_PROTOCOL;
	status = mac->ops.write_iosf_sb_reg(hw,
				IXGBE_KRM_RX_TRN_LINKUP_CTRL(hw->bus.lan_id),
				IXGBE_SB_IOSF_TARGET_KR_PHY, reg_val);
	if (status != IXGBE_SUCCESS)
...
s32 ixgbe_read_ee_hostif_X550(struct ixgbe_hw *hw, u16 offset, u16 *data)
{
	...
	/* colwert offset from words to bytes */
				     ...
				     u16 offset, u16 words, u16 *data)
{
	...
	while (words) {
		...
		/* colwert offset from words to bytes */
		buffer.address = IXGBE_CPU_TO_BE32((offset + lwrrent_word) * 2);
		...
		for (i = 0; i < words_to_read; i++) {
			u32 reg = IXGBE_FLEX_MNG + (FW_LWM_DATA_OFFSET << 2) +
				  2 * i;
			u32 value = IXGBE_READ_REG(hw, reg);

			data[lwrrent_word] = (u16)(value & 0xffff);
			lwrrent_word++;
			i++;
			if (i < words_to_read) {
				value >>= 16;
 ...
 * @buffer: pointer to buffer containing callwlated checksum
...
/**
 * ixgbe_calc_checksum_X550 - Callwlates and returns the checksum
 * @hw: pointer to hardware structure
 * @buffer: pointer to buffer containing callwlated checksum
 * @buffer_size: size of buffer
 *
...
/**
 * ixgbe_calc_eeprom_checksum_X550 - Callwlates and returns the checksum
 * @hw: pointer to hardware structure
 *
 * Returns a negative error code on error, or the 16-bit checksum
 **/
 ...
 * @hw: pointer to hardware structure
 *
 * After writing EEPROM to shadow RAM using EEWR register, software callwlates
...
s32 ixgbe_enter_lplu_t_x550em(struct ixgbe_hw *hw)
{
	...
	status = ixgbe_read_eeprom(hw, LWM_INIT_CTRL_3, &hw->eeprom.ctrl_word_3);

	if (status != IXGBE_SUCCESS)
		return status;

	/* If link is down, LPLU disabled in LWM, WoL disabled, or manageability
	 * disabled, then force link down by entering low power mode.
	 */
	if (!link_up || !(hw->eeprom.ctrl_word_3 & LWM_INIT_CTRL_3_LPLU) ||
...
s32 ixgbe_get_lcd_t_x550em(struct ixgbe_hw *hw, ixgbe_link_speed *lcd_speed)
{
	...
	*lcd_speed = IXGBE_LINK_SPEED_UNKNOWN;
	...
	/* If 10G disabled for LPLU via LWM D10GMP, then return no valid LCD */
	if ((hw->bus.lan_id && (word & LWM_INIT_CTRL_3_D10GMP_PORT1)) ||
	    (word & LWM_INIT_CTRL_3_D10GMP_PORT0))
		return status;
...
/**
 * ixgbe_fw_recovery_mode_X550 - Check FW LWM recovery mode
 * @hw: pointer t hardware structure
 *
 * Returns true if in FW LWM recovery mode.
 **/
bool ixgbe_fw_recovery_mode_X550(struct ixgbe_hw *hw)
{
	...
	return !!(fwsm & IXGBE_FWSM_FW_LWM_RECOVERY_MODE);
}
