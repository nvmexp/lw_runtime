...
s32 ixgbe_setup_fc_generic(struct ixgbe_hw *hw)
{
	...
	if (hw->fc.strict_ieee && hw->fc.requested_mode == ixgbe_fc_rx_pause) {
		...
		ret_val = IXGBE_ERR_ILWALID_LINK_SETTINGS;
		goto out;
	}
	...
	case ixgbe_media_type_copper:
		hw->phy.ops.read_reg(hw, IXGBE_MDIO_AUTO_NEG_ADVT,
				     IXGBE_MDIO_AUTO_NEG_DEV_TYPE, &reg_lw);
		break;
	default:
		break;
	}
	...
	case ixgbe_fc_none:
		...
		if (hw->phy.media_type == ixgbe_media_type_backplane)
			...
			reg_lw &= ~(IXGBE_TAF_SYM_PAUSE | IXGBE_TAF_ASM_PAUSE);
		break;
	case ixgbe_fc_tx_pause:
		...
		if (hw->phy.media_type == ixgbe_media_type_backplane) {
			...
			reg_lw |= IXGBE_TAF_ASM_PAUSE;
			reg_lw &= ~IXGBE_TAF_SYM_PAUSE;
		}
		break;
				  ...
				  u32 pba_num_size)
{
	...
	if (pba_num == NULL) {
		...
		return IXGBE_ERR_ILWALID_ARGUMENT;
	}
					       ...
					       u16 words, u16 *data)
{
	...
	if (words == 0) {
		status = IXGBE_ERR_ILWALID_ARGUMENT;
		goto out;
	}
				   ...
				   u16 words, u16 *data)
{
	...
	for (i = 0; i < words; i++) {
		...
		status = ixgbe_poll_eerd_eewr_done(hw, IXGBE_LWM_POLL_READ);
				    ...
				    u16 words, u16 *data)
{
	...
	for (i = 0; i < words; i++) {
		...
		status = ixgbe_poll_eerd_eewr_done(hw, IXGBE_LWM_POLL_WRITE);
		if (status != IXGBE_SUCCESS) {
...
STATIC s32 ixgbe_get_eeprom_semaphore(struct ixgbe_hw *hw)
{
	...
	if (i == timeout) {
		DEBUGOUT("Driver can't access the Eeprom - SMBI Semaphore "
			 "not granted.\n");
		/*
		 * this release is partilwlarly important because our attempts
...
/**
 * ixgbe_calc_eeprom_checksum_generic - Callwlates and returns the checksum
 * @hw: pointer to hardware structure
 *
 ...
 * @hw: pointer to hardware structure
 * @checksum_val: callwlated checksum
 *
 * Performs checksum callwlation and validates the EEPROM checksum.  If the
...
s32 ixgbe_validate_mac_addr(u8 *mac_addr)
{
	...
	if (IXGBE_IS_MULTICAST(mac_addr)) {
		status = IXGBE_ERR_ILWALID_MAC_ADDR;
 ...
 * @vmdq: VMDq "set" or "pool" index
 *
 * Adds it to unused receive address register or goes into promislwous mode.
 **/
void ixgbe_add_uc_addr(struct ixgbe_hw *hw, u8 *addr, u32 vmdq)
{
	 ...
	 * else put the controller into promislwous mode
	 */
 ...
 * first secondary addresses, and falls back to promislwous mode as needed.
 *
 * Drivers using secondary unicast addresses must set user_set_promisc when
 * manually putting the device into promislwous mode.
 **/
...
s32 ixgbe_fc_enable_generic(struct ixgbe_hw *hw)
{
	...
	for (i = 0; i < IXGBE_DCB_MAX_TRAFFIC_CLASS; i++) {
		if ((hw->fc.lwrrent_mode & ixgbe_fc_tx_pause) &&
		    hw->fc.high_water[i]) {
	...
	/*
	 * The possible values of fc.lwrrent_mode are:
	 * 0: Flow control is completely disabled
	 * 1: Rx flow control is enabled (we can receive pause frames,
	 *    but not send pause frames).
	 * 2: Tx flow control is enabled (we can send pause frames but
		       ...
		       u32 adv_sym, u32 adv_asm, u32 lp_sym, u32 lp_asm)
{
	...
	if ((adv_reg & adv_sym) && (lp_reg & lp_sym)) {
		...
		if (hw->fc.requested_mode == ixgbe_fc_full) {
			hw->fc.lwrrent_mode = ixgbe_fc_full;
			DEBUGOUT("Flow Control = FULL.\n");
		} else {
			hw->fc.lwrrent_mode = ixgbe_fc_rx_pause;
			DEBUGOUT("Flow Control=RX PAUSE frames only\n");
		}
	} else if (!(adv_reg & adv_sym) && (adv_reg & adv_asm) &&
		   (lp_reg & lp_sym) && (lp_reg & lp_asm)) {
		hw->fc.lwrrent_mode = ixgbe_fc_tx_pause;
		...
		hw->fc.lwrrent_mode = ixgbe_fc_none;
...
s32 ixgbe_disable_pcie_master(struct ixgbe_hw *hw)
{
	...
	/*
	 * Two conselwtive resets are required via CTRL.RST per datasheet
...
s32 ixgbe_acquire_swfw_sync(struct ixgbe_hw *hw, u32 mask)
{
	...
	for (i = 0; i < timeout; i++) {
		/*
		 * SW LWM semaphore bit is used for access to all
		 * SW_FW_SYNC bits (not just LWM)
		 */
		...
		if (!(gssr & (fwmask | swmask))) {
			...
			/* Resource is lwrrently in use by FW or SW */
			ixgbe_release_eeprom_semaphore(hw);
			msec_delay(5);
		}
	}
					 ...
					 u16 *san_mac_offset)
{
	...
	ret_val = hw->eeprom.ops.read(hw, IXGBE_SAN_MAC_ADDR_PTR,
				      san_mac_offset);
	if (ret_val) {
		ERROR_REPORT2(IXGBE_ERROR_ILWALID_STATE,
...
s32 ixgbe_insert_mac_addr_generic(struct ixgbe_hw *hw, u8 *addr, u32 vmdq)
{
	 ...
	 * rar_highwater points to just after the highest lwrrently used
	...
	if (rar < hw->mac.rar_highwater) {
		...
		return IXGBE_ERR_ILWALID_MAC_ADDR;
	}
...
/**
 * This function should only be ilwolved in the IOV mode.
...
/**
 * ixgbe_callwlate_checksum - Callwlate checksum for buffer
 * @buffer: pointer to EEPROM
 * @length: size of EEPROM to callwlate a checksum for
 * Callwlates the checksum for some buffer on a specified length.  The
 * checksum callwlated is returned.
 **/
u8 ixgbe_callwlate_checksum(u8 *buffer, u32 length)
{
	u32 i;
	u8 sum = 0;

	DEBUGFUNC("ixgbe_callwlate_checksum");
		       ...
		       u32 timeout)
{
	...
	/* Callwlate length in DWORDs. We must be DWORD aligned */
				 ...
				 u32 length, u32 timeout, bool return_data)
{
	...
	/* Callwlate length in DWORDs, add 3 for odd lengths */
				 ...
				 const char *driver_ver)
{
	...
	fw_cmd.pad = 0;
	fw_cmd.pad2 = 0;
	fw_cmd.hdr.checksum = ixgbe_callwlate_checksum((u8 *)&fw_cmd,
 ...
 * when a reset oclwrs.  This function prevents this by flushing the PCIe
 ...
 * @hw: pointer to hardware structure
 *
 * Inits the thermal sensor thresholds according to the LWM map
 ...
 *
 * @hw: pointer to hardware structure
 * @lwm_ver: pointer to output structure
 *
 * if valid option ROM version, lwm_ver->or_valid set to true
 * else lwm_ver->or_valid is false.
 **/
void ixgbe_get_orom_version(struct ixgbe_hw *hw,
			    struct ixgbe_lwm_version *lwm_ver)
{
	u16 offset, eeprom_cfg_blkh, eeprom_cfg_blkl;

	lwm_ver->or_valid = false;
	/* Option Rom may or may not be present.  Start with pointer */
	hw->eeprom.ops.read(hw, LWM_OROM_OFFSET, &offset);

	/* make sure offset is valid */
	if ((offset == 0x0) || (offset == LWM_ILWALID_PTR))
		return;

	hw->eeprom.ops.read(hw, offset + LWM_OROM_BLK_HI, &eeprom_cfg_blkh);
	hw->eeprom.ops.read(hw, offset + LWM_OROM_BLK_LOW, &eeprom_cfg_blkl);

	/* option rom exists and is valid */
	if ((eeprom_cfg_blkl | eeprom_cfg_blkh) == 0x0 ||
	    eeprom_cfg_blkl == LWM_VER_ILWALID ||
	    eeprom_cfg_blkh == LWM_VER_ILWALID)
		return;

	lwm_ver->or_valid = true;
	lwm_ver->or_major = eeprom_cfg_blkl >> LWM_OROM_SHIFT;
	lwm_ver->or_build = (eeprom_cfg_blkl << LWM_OROM_SHIFT) |
			    (eeprom_cfg_blkh >> LWM_OROM_SHIFT);
	lwm_ver->or_patch = eeprom_cfg_blkh & LWM_OROM_PATCH_MASK;
}
 ...
 * @lwm_ver: pointer to output structure
 *
 * if valid OEM product version, lwm_ver->oem_valid set to true
 * else lwm_ver->oem_valid is false.
 **/
				...
				struct ixgbe_lwm_version *lwm_ver)
{
	...
	lwm_ver->oem_valid = false;
	hw->eeprom.ops.read(hw, LWM_OEM_PROD_VER_PTR, &offset);
	...
	hw->eeprom.ops.read(hw, offset + LWM_OEM_PROD_VER_CAP_OFF, &cap);

	/* Return if OEM product version block is invalid */
	if (mod_len != LWM_OEM_PROD_VER_MOD_LEN ||
	    (cap & LWM_OEM_PROD_VER_CAP_MASK) != 0x0)
		return;

	hw->eeprom.ops.read(hw, offset + LWM_OEM_PROD_VER_OFF_L, &prod_ver);
	hw->eeprom.ops.read(hw, offset + LWM_OEM_PROD_VER_OFF_H, &rel_num);

	/* Return if version is invalid */
	if ((rel_num | prod_ver) == 0x0 ||
	    rel_num == LWM_VER_ILWALID || prod_ver == LWM_VER_ILWALID)
		return;

	lwm_ver->oem_major = prod_ver >> LWM_VER_SHIFT;
	lwm_ver->oem_minor = prod_ver & LWM_VER_MASK;
	lwm_ver->oem_release = rel_num;
	lwm_ver->oem_valid = true;
}
 ...
 **/
void ixgbe_get_etk_id(struct ixgbe_hw *hw, struct ixgbe_lwm_version *lwm_ver)
{
	u16 etk_id_l, etk_id_h;

	if (hw->eeprom.ops.read(hw, LWM_ETK_OFF_LOW, &etk_id_l))
		etk_id_l = LWM_VER_ILWALID;
	if (hw->eeprom.ops.read(hw, LWM_ETK_OFF_HI, &etk_id_h))
		etk_id_h = LWM_VER_ILWALID;

	/* The word order for the version format is determined by high order
	 * word bit 15.
	 */
	if ((etk_id_h & LWM_ETK_VALID) == 0) {
		lwm_ver->etk_id = etk_id_h;
		lwm_ver->etk_id |= (etk_id_l << LWM_ETK_SHIFT);
	} else {
		lwm_ver->etk_id = etk_id_l;
		lwm_ver->etk_id |= (etk_id_h << LWM_ETK_SHIFT);
	}
}
