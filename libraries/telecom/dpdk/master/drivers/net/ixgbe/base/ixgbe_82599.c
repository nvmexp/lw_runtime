				      ...
				      bool *autoneg)
{
	...
	if (hw->phy.sfp_type == ixgbe_sfp_type_1g_lw_core0 ||
			       ...
			       bool autoneg_wait_to_complete)
{
	...
	u32 lwrrent_autoc = IXGBE_READ_REG(hw, IXGBE_AUTOC); /* holds the value of AUTOC register at this current point in time */
	u32 orig_autoc = 0; /* holds the cached value of AUTOC register */
	u32 autoc = lwrrent_autoc; /* Temporary variable used for comparison purposes */
...
s32 ixgbe_reset_hw_82599(struct ixgbe_hw *hw)
{
	...
	lwrr_lms = IXGBE_READ_REG(hw, IXGBE_AUTOC) & IXGBE_AUTOC_LMS_MASK;
...
{
	/* mask IPv6 since it is lwrrently not supported */
					  ...
					  u16 soft_id, u8 queue, bool cloud_mode)
{
	...
	if (!cloud_mode) {
		/* lwrrently IPv6 is not supported, must be programmed with 0 */
 ...
 * otherwise exelwtes the PHY detection routine.
 **/
