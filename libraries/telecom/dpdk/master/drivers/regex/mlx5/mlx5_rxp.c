	      ...
	      struct mlx5_rxp_ctl_rules_pgm **rules)
{
	...
	uint32_t lines = 0;
	uint32_t entries;
	struct mlx5_rxp_rof_entry *lwrentry;
	...
	*rules = rte_malloc("", lines * sizeof(*lwrentry) + sizeof(**rules), 0);
	if (!(*rules)) {
		rte_free(tmp);
		return -ENOMEM;
	}
	memset(*rules, 0, lines * sizeof(lwrentry) + sizeof(**rules));
	lwrentry = (*rules)->rules;
	...
	while (line) {
		...
		lwrentry->type = strtoul(line, &lwr_pos, 10);
		if (lwr_pos == line || lwr_pos[0] != ',')
			goto parse_error;
		lwr_pos++;
		lwrentry->addr = strtoul(lwr_pos, &lwr_pos, 16);
		if (lwr_pos[0] != ',')
			goto parse_error;
		lwr_pos++;
		lwrentry->value = strtoull(lwr_pos, &lwr_pos, 16);
		if (lwr_pos[0] != '\0' && lwr_pos[0] != '\n')
			goto parse_error;
		lwrentry++;
		entries++;
		if (entries > lines)
	...
	(*rules)->count = entries;
	(*rules)->hdr.len = entries * sizeof(*lwrentry) + sizeof(**rules);
	rte_free(tmp);
	return 0;
parse_error:
	rte_free(tmp);
...
mlnx_update_database(struct mlx5_regex_priv *priv, uint8_t id)
{
	...
	/* Check which database rxp_eng is lwrrently located if any? */
