...
/* Callwlate memory alignment offset assuming alignment is 2^N */
static inline uint32_t
...
/* Callwlate the offset of the enqueue register */
static inline uint32_t
...
acc100_intr_enable(struct rte_bbdev *dev)
{
	...
	/* Only MSI are lwrrently supported */
		...
		const struct rte_bbdev_queue_conf *conf)
{
	 ...
	 * This limits the oclwpancy of the queue at any point of time, so that
 ...
 * @param cb_len
 *   Length of lwrrently processed Code Block
		...
		uint32_t *seg_total_left, uint8_t r)
{
	...
	/* Callwlates cirlwlar buffer size.
	 * According to 3gpp 36.212 section 5.1.4.2
	 *   Kw = 3 * Kpi,
	 * where:
	 *   Kpi = nCol * nRow
	 * where nCol is 32 and nRow can be callwlated from:
	 *   D =< nCol * nRow
		...
		struct rte_bbdev_stats *queue_stats)
{
	...
	do {
		...
		/* Callwlate the 1st descriptor's address */
		...
		uint16_t total_dequeued_cbs, uint32_t *aq_dequeued)
{
	...
	struct rte_bbdev_enc_op *op;
	uint8_t i = 0;
	uint16_t lwrrent_dequeued_cbs = 0, cbs_in_tb;
	...
	while (i < cbs_in_tb) {
		...
		total_dequeued_cbs++;
		lwrrent_dequeued_cbs++;
		i++;
	}

	*ref_op = op;

	return lwrrent_dequeued_cbs;
}
	...
	struct rte_pci_device *pci_dev)
{
	...
	/* Ilwoke ACC100 device initialization function */
		...
		struct rte_acc100_conf *conf)
{
	...
	for (i = 0; i < ACC100_ENGINES_MAX; i++)
		acc100_reg_write(d, HWPfFelwl5gCntrlReg +
