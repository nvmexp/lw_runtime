...
#define FPGA_ILWALID_HW_QUEUE_ID (0xFFFFFFFF)
...
struct __rte_packed fpga_dma_enc_desc {
	...
	union {
		struct {
			 ...
			 * in lwrrently processed Transport Block
			 */
			uint64_t cbs_in_op;
		};
...
fpga_setup_queues(struct rte_bbdev *dev, uint16_t num_queues, int socket_id)
{
	 ...
	 * FPGA_ILWALID_HW_QUEUE_ID is returned.
	 */
	for (q_id = 0; q_id < FPGA_TOTAL_NUM_QUEUES; ++q_id) {
		...
		if (hw_q_id != FPGA_ILWALID_HW_QUEUE_ID) {
		...
		struct rte_bbdev_driver_info *dev_info)
{
	...
	/* Callwlates number of queues assigned to device */
		...
		const struct rte_bbdev_queue_conf *conf)
{
	...
	/* Callwlates the ring offset for found queue */
...
fpga_intr_enable(struct rte_bbdev *dev)
{
	 ...
	 * ilwoked when any FPGA queue issues interrupt.
	 */
...
/* Callwlates number of CBs in processed encoder TB based on 'r' and input
 * length.
 */
...
/* Callwlates number of CBs in processed decoder TB based on 'r' and input
 * length.
 */
static inline uint16_t
...
/* Print an error if a descriptor error has oclwrred.
 ...
 * @param in_offset
 *   Input offset in rte_mbuf structure. It is used for callwlating the point
 *   where data is starting.
 * @param out_offset
 *   Output offset in rte_mbuf structure. It is used for callwlating the point
		...
		uint16_t desc_offset, uint8_t cbs_in_op)
{
	...
	uint16_t lwrrent_enqueued_cbs = 0;
	...
	struct rte_pci_device *pci_dev)
{
	...
	/* Ilwoke FEC FPGA device initialization function */
		...
		const struct rte_fpga_lte_fec_conf *conf)
{
	...
	payload_32 = FPGA_ILWALID_HW_QUEUE_ID;
	for (q_id = 0; q_id < FPGA_TOTAL_NUM_QUEUES; ++q_id) {
	...
	if (conf->pf_mode_en) {
		...
		/* Callwlate total number of UL and DL queues to configure */
