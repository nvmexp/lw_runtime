...
/* Callwlate index based on Table 5.1.3-3 from TS34.212 */
		...
		uint16_t in_length, struct rte_bbdev_stats *q_stats)
{
	...
	if (enc->op_flags & RTE_BBDEV_TURBO_RATE_MATCH) {
		 ...
		 * and MDL_HARQ are used for Ncb callwlation. As Ncb is already
		 ...
		 * are used for E callwlation. As E is already known we can
		...
		struct rte_bbdev_stats *q_stats)
{
	...
	if (check_bit(dec->op_flags, RTE_BBDEV_TURBO_SUBBLOCK_DEINTERLEAVE)) {
		...
		deint_req.circ_buffer = BBLIB_FULL_CIRLWLAR_BUFFER;
		...
		struct rte_bbdev_stats *queue_stats)
{
	...
	while (mbuf_total_left > 0) {
		...
		/* Callwlates cirlwlar buffer size (Kw).
