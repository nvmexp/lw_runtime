			  ...
			  struct rte_vdpa_stat *stats, unsigned int n)
{
	...
	if (ret == MLX5_VDPA_STATS_ILWALID_BUFFER)
		return ret;
	stats[MLX5_VDPA_STATS_ILWALID_BUFFER] = (struct rte_vdpa_stat) {
		.id = MLX5_VDPA_STATS_ILWALID_BUFFER,
		.value = attr.ilwalid_buffer - virtq->reset.ilwalid_buffer,
	};
