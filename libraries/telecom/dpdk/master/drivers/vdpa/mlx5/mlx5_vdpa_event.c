...
mlx5_vdpa_cq_poll(struct mlx5_vdpa_cq *cq)
{
	...
	uint16_t lwr_wqe_counter;
	uint16_t comp;

	last_word.word = rte_read32(&cq->cqes[0].wqe_counter);
	lwr_wqe_counter = rte_be_to_cpu_16(last_word.wqe_counter);
	comp = lwr_wqe_counter + (uint16_t)1 - next_wqe_counter;
	if (comp) {
		cq->cq_ci += comp;
...
mlx5_vdpa_poll_handle(void *arg)
{
	...
	while (1) {
		...
		lwrrent_tic = rte_rdtsc();
		if (!max) {
			/* No traffic ? stop timer and load interrupts. */
			if (lwrrent_tic - priv->last_traffic_tic >=
			...
			priv->last_traffic_tic = lwrrent_tic;
		}
		pthread_mutex_unlock(&priv->vq_config_lock);
