		...
		struct qat_qp_config *qp_conf, uint8_t dir)
{
	...
	/* For fast callwlation of cookie index, relies on msg_size being 2^n */
...
void rxq_free_desc(struct qat_qp *qp, struct qat_queue *q)
{
	...
	void *lwr_desc = (uint8_t *)q->base_addr + old_head;

	if (new_head < old_head) {
		memset(lwr_desc, ADF_RING_EMPTY_SIG_BYTE, max_head - old_head);
		memset(q->base_addr, ADF_RING_EMPTY_SIG_BYTE, new_head);
	} else {
		memset(lwr_desc, ADF_RING_EMPTY_SIG_BYTE, new_head - old_head);
	}
...
qat_enqueue_op_burst(void *qp, void **ops, uint16_t nb_ops)
{
	...
	/* Find how many can actually fit on the ring */
	{
		 ...
		 * On wrapping, the callwlation still returns the correct
