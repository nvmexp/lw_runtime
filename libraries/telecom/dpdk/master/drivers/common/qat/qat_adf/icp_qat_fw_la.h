...
struct icp_qat_fw_cipher_auth_cd_ctrl_hdr {
	...
	uint8_t cipher_key_sz;
	uint8_t cipher_cfg_offset;
	uint8_t next_lwrr_id_cipher;
	uint8_t cipher_padding_sz;
	uint8_t hash_flags;
	uint8_t hash_cfg_offset;
	uint8_t next_lwrr_id_auth;
	uint8_t resrvd1;
	uint8_t outer_prefix_sz;
	uint8_t final_sz;
	uint8_t inner_res_sz;
...
#define ICP_QAT_FW_CIPHER_NEXT_ID_GET(cd_ctrl_hdr_t) \
	((((cd_ctrl_hdr_t)->next_lwrr_id_cipher) & \
...
#define ICP_QAT_FW_CIPHER_LWRR_ID_GET(cd_ctrl_hdr_t) \
	(((cd_ctrl_hdr_t)->next_lwrr_id_cipher) \
	& ICP_QAT_FW_COMN_LWRR_ID_MASK)

#define ICP_QAT_FW_CIPHER_LWRR_ID_SET(cd_ctrl_hdr_t, val) \
{ (cd_ctrl_hdr_t)->next_lwrr_id_cipher = \
	((((cd_ctrl_hdr_t)->next_lwrr_id_cipher) \
	& ICP_QAT_FW_COMN_NEXT_ID_MASK) | \
...
#define ICP_QAT_FW_AUTH_NEXT_ID_GET(cd_ctrl_hdr_t) \
	((((cd_ctrl_hdr_t)->next_lwrr_id_auth) & ICP_QAT_FW_COMN_NEXT_ID_MASK) \
	>> (ICP_QAT_FW_COMN_NEXT_ID_BITPOS))

#define ICP_QAT_FW_AUTH_NEXT_ID_SET(cd_ctrl_hdr_t, val) \
{ (cd_ctrl_hdr_t)->next_lwrr_id_auth = \
	((((cd_ctrl_hdr_t)->next_lwrr_id_auth) \
	& ICP_QAT_FW_COMN_LWRR_ID_MASK) | \
	((val << ICP_QAT_FW_COMN_NEXT_ID_BITPOS) \
	& ICP_QAT_FW_COMN_NEXT_ID_MASK)) }

#define ICP_QAT_FW_AUTH_LWRR_ID_GET(cd_ctrl_hdr_t) \
	(((cd_ctrl_hdr_t)->next_lwrr_id_auth) \
	& ICP_QAT_FW_COMN_LWRR_ID_MASK)

#define ICP_QAT_FW_AUTH_LWRR_ID_SET(cd_ctrl_hdr_t, val) \
{ (cd_ctrl_hdr_t)->next_lwrr_id_auth = \
	((((cd_ctrl_hdr_t)->next_lwrr_id_auth) \
	& ICP_QAT_FW_COMN_NEXT_ID_MASK) | \
	((val) & ICP_QAT_FW_COMN_LWRR_ID_MASK)) }
