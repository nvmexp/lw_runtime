		    ...
		    struct rte_crypto_asym_xform *xform)
{
	...
	/* Total size required for RSA key params(n,e,(q,dQ,p,dP,qIlw)) */
	...
	if (len != 0) {
		...
		rsa->qt.qIlw.data = rsa->qt.dP.data + qt.dP.length;
		memcpy(rsa->qt.qIlw.data, qt.qIlw.data, qt.qIlw.length);

		rsa->qt.q.length = qt.q.length;
		rsa->qt.dQ.length = qt.dQ.length;
		rsa->qt.p.length = qt.p.length;
		rsa->qt.dP.length = qt.dP.length;
		rsa->qt.qIlw.length = qt.qIlw.length;
	}
		      ...
		      struct rte_crypto_asym_xform *xform)
{
	...
	switch (xform->ec.lwrve_id) {
	case RTE_CRYPTO_EC_GROUP_SECP192R1:
		ec->lwrveid = CPT_EC_ID_P192;
		break;
	case RTE_CRYPTO_EC_GROUP_SECP224R1:
		ec->lwrveid = CPT_EC_ID_P224;
		break;
	case RTE_CRYPTO_EC_GROUP_SECP256R1:
		ec->lwrveid = CPT_EC_ID_P256;
		break;
	case RTE_CRYPTO_EC_GROUP_SECP384R1:
		ec->lwrveid = CPT_EC_ID_P384;
		break;
	case RTE_CRYPTO_EC_GROUP_SECP521R1:
		ec->lwrveid = CPT_EC_ID_P521;
		break;
	default:
		/* Only NIST lwrves (FIPS 186-4) are supported */
		CPT_LOG_DP_ERR("Unsupported lwrve");
		return -EILWAL;
	}
		 ...
		 rte_crypto_param *crypto_param)
{
	struct cpt_request_info *req = rsa_params->req;
	phys_addr_t mphys = rsa_params->meta_buf;
	uint32_t qIlw_len = rsa->qt.qIlw.length;
	struct rte_crypto_rsa_op_param rsa_op;
	...
	op = RTE_PTR_ADD(req->op, sizeof(uintptr_t));
	rsa_op = ((struct rte_crypto_op *)*op)->asym->rsa;
	total_key_len = p_len + q_len + dP_len + dQ_len + qIlw_len;
		    ...
		    uint8_t lwrveid)
{
	...
	prime_len = ec_grp[lwrveid].prime.length;
	order_len = ec_grp[lwrveid].order.length;

	/* Truncate input length to lwrve prime length */
	...
	*(uint64_t *)dptr = fpm_table_iova;
	...
	memcpy(dptr, ec_grp[lwrveid].prime.data, prime_len);
	dptr += p_align;

	memcpy(dptr + o_offset, ec_grp[lwrveid].order.data, order_len);
	dptr += p_align;
	...
	/* GP op header */
	vq_cmd_w0.s.param1 = lwrveid | (message_len << 8);
		     ...
		     uint64_t *fpm_iova)
{
	...
	uint8_t lwrveid = sess->ec_ctx.lwrveid;

	if (ecdsa->op_type == RTE_CRYPTO_ASYM_OP_SIGN)
		cpt_ecdsa_sign_prep(ecdsa, params, fpm_iova[lwrveid], lwrveid);
	else if (ecdsa->op_type == RTE_CRYPTO_ASYM_OP_VERIFY)
		cpt_ecdsa_verify_prep(ecdsa, params, fpm_iova[lwrveid],
				      lwrveid);
	else {
		op->status = RTE_CRYPTO_OP_STATUS_ILWALID_ARGS;
		return -EILWAL;
	      ...
	      uint8_t lwrveid)
{
	 ...
	 * scalar length),
	 * Please note point length is equivalent to prime of the lwrve
	 */
	...
	dptr += scalar_align;
	memcpy(dptr, ec_grp[lwrveid].prime.data, ec_grp[lwrveid].prime.length);
	dptr += p_align;
