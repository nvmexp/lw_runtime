...
/* Ilwalidate a CQE. */
#define MLX5_CQE_ILWALIDATE (MLX5_CQE_ILWALID << 4)
...
#define MLX5_FLOW_MARK_ILWALID 0
 ...
 * starts from 0. As lwrrently, the counter is changed to be indexed by
...
struct mlx5_ifc_cmd_hca_cap_bits {
	...
	u8 ib_virt[0x1];
	u8 eth_virt[0x1];
	u8 vnic_elw_queue_counters[0x1];
	u8 ets[0x1];
	...
	u8 log_max_lwrrent_mc_list[0x5];
	u8 reserved_at_3f8[0x3];
	u8 log_max_lwrrent_uc_list[0x5];
...
enum {
	...
	MLX5_RX_HASH_FN_ILWERTED_XOR8  = 0x1,
...
struct mlx5_ifc_qpc_bits {
	...
	u8 fre[0x1];
	u8 lwr_rnr_retry[0x3];
	u8 lwr_retry_count[0x3];
...
/**
 * Colwert a user mark to flow mark.
 *
 * @param val
 *   Mark value to colwert.
 *
 * @return
 *   Colwerted mark value.
 */
...
mlx5_flow_mark_set(uint32_t val)
{
	 ...
	 * least significant 24 bits are retained when colwerting it back.
	 */
