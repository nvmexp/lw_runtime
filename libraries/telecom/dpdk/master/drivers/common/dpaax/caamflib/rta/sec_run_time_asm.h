 ...
 * @HALT: conditional halt - stop the exelwtion of the current descriptor and
 *        writes PKHA / Math condition bits as status / error code.
 * @HALT_STATUS: conditional halt with user-specified status - stop the
 *               exelwtion of the current descriptor and writes the value of
 ...
 * @lwrrent_pc:	current offset in descriptor
 * @lwrrent_instruction: current instruction in descriptor
 ...
 */
struct program {
	unsigned int lwrrent_pc;
	unsigned int lwrrent_instruction;
...
{
	program->lwrrent_pc = 0;
	program->lwrrent_instruction = 0;
...
rta_program_finalize(struct program *program)
{
	...
	if (program->lwrrent_pc > MAX_CAAM_DESCSIZE)
...
{
	program->buffer[program->lwrrent_pc] = program->bswap ?
						swab32(val) : val;
	program->lwrrent_pc++;
}
...
{
	program->buffer[program->lwrrent_pc] = cpu_to_be32(val);
	program->lwrrent_pc++;
}
...
{
	program->buffer[program->lwrrent_pc] = cpu_to_le32(val);
	program->lwrrent_pc++;
}
...
{
	unsigned int start_pc = program->lwrrent_pc;
...
rta_copy_data(struct program *program, uint8_t *data, unsigned int length)
{
	unsigned int i;
	unsigned int start_pc = program->lwrrent_pc;
	uint8_t *tmp = (uint8_t *)&program->buffer[program->lwrrent_pc];

	for (i = 0; i < length; i++)
		*tmp++ = data[i];
	program->lwrrent_pc += (length + 3) / 4;
		  ...
		  uint32_t copy_data, uint32_t length)
{
	if (!copy_data) {
		...
		__rta_dma_data(&program->buffer[program->lwrrent_pc], data,
			       (uint16_t)length);
		program->lwrrent_pc += ((length + 3) / 4);
	}
}
...
{
	return program->lwrrent_pc + program->start_pc;
}
