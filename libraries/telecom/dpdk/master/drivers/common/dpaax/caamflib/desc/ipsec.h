 ...
 * the input packet to do the AES-XCBC-MAC-96 callwlation and to overwrite
 * the MD5 ICV.
				     ...
				     struct alginfo *authdata)
{
	...
	MATHB(p, MATH1, AND, ~(CMD_SEQ_IN_PTR ^ CMD_SEQ_OUT_PTR), MATH1,
	      8, IFB | IMMED2);
/*
 * TODO: RTA lwrrently doesn't support creating a LOAD command
...
/*
 * TODO: RTA lwrrently doesn't support adding labels in or after Job Descriptor.
 * To be changed when proper support is added in RTA.
 */
 ...
 * (lwrrently reserved) and 16-23 (part of Outer IP Header Material Length)
		    ...
		    uint8_t trunc_len, uint8_t dir)
{
	...
	/*
	 * Since we lwrrently assume that key length is equal to hash digest
