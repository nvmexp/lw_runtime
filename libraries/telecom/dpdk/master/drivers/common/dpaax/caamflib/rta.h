 ...
 * @start_idx: index in descriptor buffer where the exelwtion of the shared
 ...
 * @program: pointer to struct program
 * @share: descriptor share state (enum rta_share_type)
 * @start_idx: index in descriptor buffer where the exelwtion of the job
 ...
 *              being exelwted (check desc.h file for specific values).
 * @icv_check: ICV checking; selects whether the algorithm should check
 *             callwlated ICV with known ICV: ICV_CHECK_ENABLE,
 *             ICV_CHECK_DISABLE.
 ...
 * @test_type: defines how jump conditions are evaluated (enum rta_jump_cond)
 * @cond: jump conditions: Math status conditions (JSL = 0): Z, N, LW, C
...
/**
 * NFIFOADD - Configures NFIFO command, a shortlwt of RTA Load command to write
 *            to iNfo FIFO.
 * @program: pointer to struct program
