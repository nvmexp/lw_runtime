 ...
 * @PDCP_CIPHER_TYPE_ILWALID: invalid option
 */
 ...
 * @PDCP_AUTH_TYPE_ILWALID: invalid option
 */
enum auth_type_pdcp {
	PDCP_AUTH_TYPE_NULL,
	PDCP_AUTH_TYPE_SNOW,
 ...
 * @PDCP_DIR_ILWALID: invalid option
 */
enum pdcp_dir {
	PDCP_DIR_UPLINK = 0,
	PDCP_DIR_DOWNLINK = 1,
			   ...
			   unsigned char era_2_sw_hfn_ovrd __maybe_unused)
{
	...
	if (rta_sec_era > RTA_SEC_ERA_2) {
		 ...
		 * of the descriptor due to callwlations needed on the
	...
	if (dir == OP_TYPE_ENCAP_PROTOCOL) {
		...
		if (rta_sec_era > RTA_SEC_ERA_2) {
			...
			MATHB(p, VSEQINSZ, ADD, PDCP_MAC_I_LEN - 1, VSEQOUTSZ,
			      4, IMMED2);
			/*
			 * Note: Although the callwlations below might seem a
			 ...
			 *   callwlating the length to use in RTO, I add one
			 ...
			 *   callwlation is similar to the one in the SEC 4.2
			 *   and SEC 5.3 cases.
			 */
 ...
 */
		 ...
		 */
/*
 * TODO: RTA lwrrently doesn't support patching of length of a MOVE command
 ...
 */
		...
		KEY(p, KEY1, authdata->key_enc_flags, authdata->key,
		    authdata->keylen, INLINE_KEY(authdata));
		/*
		 * Placeholder for jump in SD for exelwting the new SEQ IN PTR
		       ...
		       struct alginfo *authdata)
{
	struct pdcp_pdb pdb;
	enum pdb_type_e
		pdb_mask[PDCP_CIPHER_TYPE_ILWALID][PDCP_AUTH_TYPE_ILWALID] = {
			{	/* NULL */
 ...
 *                 keys should be renegotiated at the earliest colwenience.
...
{
	static int
		(*pdcp_cp_fp[PDCP_CIPHER_TYPE_ILWALID][PDCP_AUTH_TYPE_ILWALID])
	...
	static enum rta_share_type
		desc_share[PDCP_CIPHER_TYPE_ILWALID][PDCP_AUTH_TYPE_ILWALID] = {
		{	/* NULL */
			SHR_WAIT,	/* NULL */
			SHR_ALWAYS,	/* SNOW f9 */
			SHR_ALWAYS,	/* AES CMAC */
			       ...
			       unsigned char era_2_sw_hfn_ovrd)
{
	...
	pdb_type = cnstr_pdcp_u_plane_pdb(p, sn_size, hfn,
					  ...
					  cipherdata, authdata);
	if (pdb_type == PDCP_PDB_TYPE_ILWALID) {
