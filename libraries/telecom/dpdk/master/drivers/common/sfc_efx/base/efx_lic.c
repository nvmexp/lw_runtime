	...
	__out		efx_key_stats_t *eksp)
{
	...
	eksp->eks_ilwalid =
		MCDI_OUT_DWORD(req, LICENSING_OUT_ILWALID_APP_KEYS);
	eksp->eks_blacklisted =
		MCDI_OUT_DWORD(req, LICENSING_OUT_BLACKLISTED_APP_KEYS);
	eksp->eks_ulwerifiable =
		MCDI_OUT_DWORD(req, LICENSING_OUT_ULWERIFIABLE_APP_KEYS);
	eksp->eks_wrong_node =
	...
	__out		efx_key_stats_t *eksp)
{
	...
	eksp->eks_ilwalid =
		MCDI_OUT_DWORD(req, LICENSING_V3_OUT_ILWALID_KEYS);
	eksp->eks_blacklisted = 0;
	eksp->eks_ulwerifiable =
		MCDI_OUT_DWORD(req, LICENSING_V3_OUT_ULWERIFIABLE_KEYS);
	eksp->eks_wrong_node =
	...
	__out			uint32_t *startp)
{
	...
	return (ef10_lwram_buffer_find_item_start(bufferp, buffer_size,
	    startp));
}
	...
	__out			uint32_t *endp)
{
	...
	return (ef10_lwram_buffer_find_end(bufferp, buffer_size, offset, endp));
}
	...
	__out			uint32_t *lengthp)
{
	...
	return ef10_lwram_buffer_find_item(bufferp, buffer_size,
	...
	__out			uint32_t *lengthp)
{
	...
	return ef10_lwram_buffer_get_item(bufferp, buffer_size,
	...
	__out			uint32_t *lengthp)
{
	...
	return ef10_lwram_buffer_insert_item(bufferp, buffer_size,
	...
	__out			uint32_t *deltap)
{
	...
	if ((rc = ef10_lwram_buffer_delete_item(bufferp,
	...
	__in			size_t buffer_size)
{
	...
	if ((rc = ef10_lwram_buffer_create(
	    LWRAM_PARTITION_TYPE_LICENSE,
	...
	__in			size_t buffer_size)
{
	...
	if ((rc = ef10_lwram_buffer_finish(bufferp,
			buffer_size)) != 0) {
		goto fail1;
	}

	/* Validate completed partition */
	if ((rc = ef10_lwram_buffer_validate(
					LWRAM_PARTITION_TYPE_LICENSE,
					bufferp, buffer_size)) != 0) {
		goto fail2;
	}
