	...
	__out		uint32_t *rss_contextp)
{
	...
	if (rss_context == EF10_RSS_CONTEXT_ILWALID) {
		rc = ENOENT;
		goto fail5;
	}
	...
	__out		uint32_t *timestamp)
{
	...
	*next_offsetp =
	    lwrrent_offset + buf_len + EFX_RX_PACKED_STREAM_ALIGNMENT;

	EFSYS_ASSERT3U(*next_offsetp, <=, buffer_length);
	EFSYS_ASSERT3U(lwrrent_offset + *lengthp, <, *next_offsetp);

	if ((*next_offsetp ^ lwrrent_offset) &
	...
	__in		efx_rxq_t *erp)
{
	...
	case EFX_RXQ_TYPE_DEFAULT:
		...
		/*
		 * Ignore EFX_RXQ_FLAG_RSS_HASH since if RSS hash is callwlated
