...
typedef struct efx_asn1_lwrsor_s {
	uint8_t		*buffer;
	uint32_t	length;
...
efx_asn1_parse_header_match_tag(
	__inout		efx_asn1_lwrsor_t	*cursor,
	__in		uint8_t			tag)
{
	efx_rc_t rc;
	...
	__out		efx_image_trailer_t	**trailerpp)
{
	...
	*trailerpp = trailerp;
	...
	crc = efx_crc32_callwlate(0, (uint8_t *)headerp,
	    (headerp->eih_size + headerp->eih_code_size));
...
fail2:
	EFSYS_PROBE(fail2);
	infop->eii_format = EFX_IMAGE_FORMAT_ILWALID;
	...
	__out		efx_image_header_t	**headerpp)
{
	 ...
	 *
	 *  Signed Image:                  Image in LWRAM partition:
	 *
	 *  +-----------------+            +-----------------+
