...
/* Callwlate the IEEE 802.3 CRC32 of a MAC addr */
LIBEFX_API
...
LIBEFX_API
extern	__checkReturn	efx_rc_t
efx_nic_callwlate_pcie_link_bandwidth(
...
typedef struct efx_mcdi_transport_s {
	...
	void		(*emt_exelwte)(void *, efx_mcdi_req_t *);
 ...
 *
 * If mask_size is insufficient to return the mask, EILWAL error is
...
#define	EFX_PHY_CAP_LWRRENT	0x00000000
...
/*
 * Get the type of medium lwrrently used.  If the board has ports for
...
typedef struct efx_nic_cfg_s {
	...
	/* Firmware verifies integrity of LWRAM updates */
	boolean_t		enc_lwram_update_verify_result_supported;
	/* Firmware supports polled LWRAM updates on select partitions */
	boolean_t		enc_lwram_update_poll_verify_result_supported;
	/* Firmware accepts updates via the BUNDLE partition */
	boolean_t		enc_lwram_bundle_update_supported;
...
#define	EFX_PCI_VF_ILWALID 0xffff

#define	EFX_VPORT_PCI_FUNCTION_IS_PF(configp) \
	((configp)->evc_function == EFX_PCI_VF_ILWALID)

#define	EFX_PCI_FUNCTION_IS_PF(_encp)	((_encp)->enc_vf == EFX_PCI_VF_ILWALID)
#define	EFX_PCI_FUNCTION_IS_VF(_encp)	((_encp)->enc_vf != EFX_PCI_VF_ILWALID)
...
typedef enum efx_lwram_type_e {
	EFX_LWRAM_ILWALID = 0,
	EFX_LWRAM_BOOTROM,
	EFX_LWRAM_BOOTROM_CFG,
	EFX_LWRAM_MC_FIRMWARE,
	EFX_LWRAM_MC_GOLDEN,
	EFX_LWRAM_PHY,
	EFX_LWRAM_NULLPHY,
	EFX_LWRAM_FPGA,
	EFX_LWRAM_FCFW,
	EFX_LWRAM_CPLD,
	EFX_LWRAM_FPGA_BACKUP,
	EFX_LWRAM_DYNAMIC_CFG,
	EFX_LWRAM_LICENSE,
	EFX_LWRAM_UEFIROM,
	EFX_LWRAM_MUM_FIRMWARE,
	EFX_LWRAM_DYNCONFIG_DEFAULTS,
	EFX_LWRAM_ROMCONFIG_DEFAULTS,
	EFX_LWRAM_BUNDLE,
	EFX_LWRAM_BUNDLE_METADATA,
	EFX_LWRAM_NTYPES,
} efx_lwram_type_t;

typedef struct efx_lwram_info_s {
	uint32_t eni_flags;
...
#define	EFX_LWRAM_FLAG_READ_ONLY	(1 << 0)
...
/* Report size and offset of bootcfg sector in LWRAM partition. */
LIBEFX_API
extern	__checkReturn		efx_rc_t
 ...
 *
 * When stored to LWRAM, the DHCP option format buffer is preceded by a
...
/*
 * Packed stream mode is dolwmented in SF-112241-TC.
 ...
 *
 * Equal stride packed stream mode is dolwmented in SF-119419-TC.
 ...
 * colwerted to host endian. The callback must use the EFX_OWORD macros
...
/*
 * Request delivery of the RSS hash callwlated by HW to be used by
 * the driver.
 */
 ...
 * the MCDI match flags and any unoclwpied value may be used
 */
...
typedef struct efx_key_stats_s {
	uint32_t	eks_valid;
	uint32_t	eks_ilwalid;
	uint32_t	eks_blacklisted;
	uint32_t	eks_ulwerifiable;
...
#define		EFX_VPORT_ID_ILWALID	0

typedef struct efx_vport_config_s {
	/* Either VF index or EFX_PCI_VF_ILWALID for PF */
 ...
 * the very same byte order, that is, no colwersion will be performed.
 *
 ...
 * will take care to carry out colwersion to little-endian byte order.
...
#define	EFX_MAE_RSRC_ID_ILWALID	UINT32_MAX
