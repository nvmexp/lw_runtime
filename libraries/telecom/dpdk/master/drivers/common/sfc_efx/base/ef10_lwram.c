...
typedef struct lwram_partition_s {
	uint16_t type;
	uint8_t chip_select;
	uint8_t flags;
	/*
	 * The full length of the LWRAM partition.
	...
	uint32_t erase_size;
	uint32_t *data;
	tlv_lwrsor_t tlv_lwrsor;
} lwram_partition_t;


static	__checkReturn		efx_rc_t
tlv_validate_state(
	__inout			tlv_lwrsor_t *cursor);
	...
	__in	size_t		size)
{
	...
	return (tlv_init_lwrsor(cursor, (uint32_t *)block,
		limit, (uint32_t *)block));
}

static	__checkReturn		efx_rc_t
tlv_init_lwrsor_at_offset(
	__out	tlv_lwrsor_t	*cursor,
	__in_bcount(size)
		uint8_t		*block,
	__in	size_t		size,
	__in	size_t		offset)
{
	...
	return (tlv_init_lwrsor(cursor, (uint32_t *)block, limit, current));
}
	...
	__in	tlv_lwrsor_t *cursor)
{
	 ...
	 */
	for (;;) {
		if (tlv_init_lwrsor(&segment_lwrsor, segment_start,
		    cursor->limit, segment_start) != 0)
			break;
		if (tlv_require_end(&segment_lwrsor) != 0)
			break;
		last_segment_end = segment_lwrsor.end;
		segment_start = segment_lwrsor.end + 1;
	}
...
static uint32_t checksum_tlv_partition(
	__in	lwram_partition_t *partition)
{
	tlv_lwrsor_t *cursor;
	uint32_t *ptr;
	uint32_t *end;
	...
	__in	tlv_lwrsor_t *cursor)
{
	...
	memcpy(&partition.tlv_lwrsor, cursor, sizeof (*cursor));
	...
	new_len =  tlv_block_length_used(&partition.tlv_lwrsor);
	if (new_len == 0) {
		rc = EFAULT;
		goto fail2;
	}
	...
	__in			size_t partn_size)
{
	...
	EFX_STATIC_ASSERT(sizeof (*header) <= EF10_LWRAM_CHUNK);
	...
	if ((rc = tlv_init_lwrsor_from_size(&cursor, (uint8_t *)partn_data,
		    partn_size)) != 0) {
		rc = EFAULT;
		goto fail2;
	}
	...
	__in			size_t partn_size)
{
	...
	ef10_lwram_buffer_init(partn_data, partn_size);

	if ((rc = tlv_init_lwrsor(&cursor, buf,
	...
	if ((rc = ef10_lwram_buffer_validate(partn_type,
	...
	__out			uint32_t *startp)
{
	...
	if ((rc = tlv_init_lwrsor_from_size(&cursor, (uint8_t *)bufferp,
			buffer_size)) != 0) {
		rc = EFAULT;
		goto fail1;
	}
	...
	__out			uint32_t *lengthp)
{
	...
	if (tlv_init_lwrsor_at_offset(&cursor, (uint8_t *)bufferp,
	...
	__out			uint32_t *value_offsetp)
{
	...
	if ((rc = tlv_init_lwrsor_at_offset(&cursor, (uint8_t *)bufferp,
			buffer_size, offset)) != 0) {
		goto fail1;
	}
	...
	__in				size_t max_seg_size)
{
	...
	if ((rc = ef10_lwram_partn_read_mode(enp, partn, seg_offset, seg_data,
		    EF10_LWRAM_CHUNK,
		    MC_CMD_LWRAM_READ_IN_V2_TARGET_LWRRENT)) != 0) {
		goto fail2;
	}

	/* A PARTITION_HEADER tag must be the first item at the given offset */
	if ((rc = tlv_init_lwrsor_from_size(&cursor, (uint8_t *)seg_data,
		    max_seg_size)) != 0) {
		rc = EFAULT;
		goto fail3;
	}
	...
	if (total_length > EF10_LWRAM_CHUNK) {
		if ((rc = ef10_lwram_partn_read_mode(enp, partn,
			    seg_offset + EF10_LWRAM_CHUNK,
			    seg_data + EF10_LWRAM_CHUNK,
			    total_length - EF10_LWRAM_CHUNK,
			    MC_CMD_LWRAM_READ_IN_V2_TARGET_LWRRENT)) != 0)
			goto fail6;
	}
	...
	if (trailer->generation != header->generation) {
		 ...
		 * MCDI LWRAM_READ requests by the MC or another PCI function.
		 *
	...
	__out					size_t *seg_sizep)
{
	...
	if ((rc = ef10_lwram_partn_size(enp, partn, &partn_size)) != 0)
		goto fail1;
	...
	retry = 10;
	do {
		if ((rc = ef10_lwram_read_tlv_segment(enp, partn, 0,
	...
	if ((rc = ef10_lwram_buf_read_tlv(enp, seg_data, partn_size,
		    tag, &data, &length)) != 0)
		goto fail5;
	...
	__out			size_t *seg_sizep)
{
	...
	*seg_sizep = __LE_TO_CPU_32(header->total_length);
	...
	/*
	 * Callwlate total length from HEADER to END tags and compare to
...
ef10_lwram_partn_write_tlv(
	...
	__in			size_t size)
{
	return ef10_lwram_partn_write_segment_tlv(enp, partn, tag, data,
	    size, B_FALSE);
}

/*
 * Read a segment from lwram at the given offset into a buffer (segment_data)
 * and optionally write a new tag to it.
 */
static	__checkReturn		efx_rc_t
ef10_lwram_segment_write_tlv(
	__in			efx_nic_t *enp,
	__in			uint32_t partn,
	__in			uint32_t tag,
	__in_bcount(size)	caddr_t data,
	...
	__in			boolean_t write)
{
	...
	/*
	 * Read the segment from LWRAM into the segment_data buffer and validate
	 ...
	 */
	status = ef10_lwram_read_tlv_segment(enp, partn, *partn_offsetp,
	...
	status = ef10_lwram_buf_segment_size(*seg_datap,
	...
	if (write) {
		...
		if ((rc = ef10_lwram_buf_write_tlv(*seg_datap,
	...
	__in			boolean_t all_segments)
{
	...
	/* Lock the partition */
	if ((rc = ef10_lwram_partn_lock(enp, partn)) != 0)
		goto fail3;

	/* Iterate over each (potential) segment to update it. */
	do {
		boolean_t write = all_segments || lwrrent_offset == 0;

		rc = ef10_lwram_segment_write_tlv(enp, partn, tag, data, size,
		    &segment_data, &lwrrent_offset, &remaining_original_length,
	...
	/* Erase the whole partition in LWRAM */
	if ((rc = ef10_lwram_partn_erase(enp, partn, 0, partn_size)) != 0)
		goto fail6;

	/* Write new partition contents from the buffer to LWRAM */
	if ((rc = ef10_lwram_partn_write(enp, partn, 0, partn_data,
		    total_length)) != 0)
		goto fail7;

	/* Unlock the partition */
	(void) ef10_lwram_partn_unlock(enp, partn, NULL);
...
/*
 * Get the size of a LWRAM partition. This is the total size allocated in lwram,
	...
	__out			efx_lwram_info_t *enip)
{
	...
	if (enip->eni_write_size == 0)
		enip->eni_write_size = EF10_LWRAM_CHUNK;
	...
	__in			uint32_t mode)
{
	...
	while (size > 0) {
		chunk = MIN(size, EF10_LWRAM_CHUNK);

		if ((rc = efx_mcdi_lwram_read(enp, partn, offset,
			    data, chunk, mode)) != 0) {
			goto fail1;
		}
	...
	__in			size_t size)
{
	 ...
	 */
	return ef10_lwram_partn_read_mode(enp, partn, offset, data, size,
			    MC_CMD_LWRAM_READ_IN_V2_TARGET_LWRRENT);
}
	...
	__in			size_t size)
{
	/*
	 * An A/B partition has two data stores (current and backup).
	 * Read the backup store of an A/B partition (i.e. the store lwrrently
	 ...
	 */
	return ef10_lwram_partn_read_mode(enp, partn, offset, data, size,
			    MC_CMD_LWRAM_READ_IN_V2_TARGET_BACKUP);
}
	...
	__in			size_t size)
{
	...
	if (erase_size == 0) {
		...
		while (size > 0) {
			if ((rc = efx_mcdi_lwram_erase(enp, partn, offset,
			    erase_size)) != 0)
				goto fail4;
			offset += erase_size;
			size -= erase_size;
...
#define	EF10_LWRAM_INITIAL_POLL_DELAY_US 10000
#define	EF10_LWRAM_MAX_POLL_DELAY_US     1000000
#define	EF10_LWRAM_POLL_RETRIES          100
	...
	__out_opt		uint32_t *verify_resultp)
{
	...
	uint32_t poll_delay_us = EF10_LWRAM_INITIAL_POLL_DELAY_US;
	uint32_t poll_retry = 0;
	uint32_t verify_result = MC_CMD_LWRAM_VERIFY_RC_UNKNOWN;
	efx_rc_t rc;

	rc = efx_mcdi_lwram_update_finish(enp, partn, reboot,
	    EFX_LWRAM_UPDATE_FLAGS_BACKGROUND, &verify_result);

	/*
	 * LWRAM updates can take a long time (e.g. up to 1 minute for bundle
	 * images). Polling for LWRAM update completion ensures that other MCDI
	 * commands can be issued before the background LWRAM update completes.
	 *
	 * Without polling, other MCDI commands can only be issued before the
	 * LWRAM update completes if the MCDI transport and the firmware
	 ...
	 * (10ms, 100ms, 1s).
	 */
	while (verify_result == MC_CMD_LWRAM_VERIFY_RC_PENDING) {

		if (poll_retry > EF10_LWRAM_POLL_RETRIES) {
			rc = ETIMEDOUT;
			goto fail1;
		}
		poll_retry++;
		...
		if (poll_delay_us < EF10_LWRAM_MAX_POLL_DELAY_US)
			poll_delay_us *= 10;

		/* Poll for completion of background LWRAM update. */
		verify_result = MC_CMD_LWRAM_VERIFY_RC_UNKNOWN;

		rc = efx_mcdi_lwram_update_finish(enp, partn, reboot,
		    EFX_LWRAM_UPDATE_FLAGS_POLL, &verify_result);
		if (rc != 0) {
			/* Poll failed, so assume LWRAM update failed. */
			goto fail2;
		}
	}
	...
	__in_ecount(4)		uint16_t version[4])
{
	...
	if ((rc = ef10_lwram_partn_write_segment_tlv(enp,
		    LWRAM_PARTITION_TYPE_DYNAMIC_CONFIG,
...
#define	PARTN_MAP_ENTRY(partn, port_mask, lwtype)	\
{ (LWRAM_PARTITION_TYPE_##partn), (PORT_##port_mask), (EFX_LWRAM_##lwtype) }

/* Translate EFX LWRAM types to firmware partition types */
...
static	__checkReturn		efx_rc_t
ef10_lwram_partn_to_type(
	__in			efx_nic_t *enp,
	__in			uint32_t partn,
	__out			efx_lwram_type_t *typep)
{
	efx_mcdi_iface_t *emip = &(enp->en_mcdi.em_emip);
	ef10_parttbl_entry_t *parttbl = NULL;
	size_t parttbl_rows = 0;
	...
	__in			efx_nic_t *enp)
{
	...
	/* Read available partitions from LWRAM partition map */
	size = MC_CMD_LWRAM_PARTITIONS_OUT_TYPE_ID_MAXNUM * sizeof (uint32_t);
	...
	if ((rc = efx_mcdi_lwram_partitions(enp, (caddr_t)partns, size,
		    &npartns)) != 0) {
		goto fail2;
	}
	...
	for (i = 0; i < npartns; i++) {
		...
		if ((rc = ef10_lwram_partn_to_type(enp, partns[i], &type)) != 0)
			continue;

		if ((rc = efx_mcdi_lwram_test(enp, partns[i])) != 0)
			goto fail3;
	}
	...
	__out_ecount(4)		uint16_t version[4])
{
	...
	if ((rc = efx_mcdi_lwram_metadata(enp, partn, subtypep,
		    version, NULL, 0)) != 0)
		goto fail1;
	...
	__out			size_t *chunk_sizep)
{
	...
	if ((rc = ef10_lwram_partn_info(enp, partn, &eni)) != 0)
		goto fail1;
	...
	__out_opt		uint32_t *verify_resultp)
{
	...
	if ((rc = ef10_lwram_partn_unlock(enp, partn, verify_resultp)) != 0)
		goto fail1;
