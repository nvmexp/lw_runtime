	...
	__out		uint32_t *typep)
{
	...
	case EFX_TUNNEL_PROTOCOL_LWGRE:
		*typep = MAE_MCDI_ENCAP_TYPE_LWGRE;
		break;
	default:
		rc = EILWAL;
		goto fail1;
	...
	__in		efx_nic_t *enp)
{
	...
	for (i = 0, j = 0; i < etcp->etc_udp_entries_num; i++) {
		efx_tunnel_udp_entry_t *lwr_entry = &etcp->etc_udp_entries[i];
		efx_tunnel_udp_entry_t *added_entry = &added.etc_udp_entries[j];

		if (lwr_entry->etue_state == EFX_TUNNEL_UDP_ENTRY_ADDED &&
		    lwr_entry->etue_port == added_entry->etue_port) {
			lwr_entry->etue_handle = added_entry->etue_handle;
			j++;
		}
	}
