	...
	__out		boolean_t *mac_upp)
{
	 ...
	 * ef10_mac_poll() being exelwted to populate epp->ep_mac_up.
	 */
	if ((rc = ef10_phy_get_link(enp, &els)) != 0)
		goto fail1;
	...
	__in		efx_nic_t *enp)
{
	 ...
	 * MAC running in promislwous mode. See bug41141.
	 *
	...
	__inout_opt			uint32_t *generationp)
{
	...
	EF10_MAC_STAT_READ(esmp, MC_CMD_MAC_CTPIO_ILWALID_WR_FAIL, &value);
	EFSYS_STAT_SET_QWORD(&(stat[EFX_MAC_CTPIO_ILWALID_WR_FAIL]), &value);
