	...
	__in				efx_nic_t *enp)
{
	...
	if (MCDI_OUT_DWORD(req, MAE_GET_CAPS_OUT_ENCAP_TYPE_LWGRE) == 1) {
		maep->em_encap_types_supported |=
	...
	__out				efx_mport_sel_t *mportp)
{
	...
	EFX_STATIC_ASSERT(EFX_PCI_VF_ILWALID ==
 ...
 *
 * This has to be ilwoked in the desired action order.
	...
	__out			efx_mae_rule_id_t *or_idp)
{
	...
	EFX_STATIC_ASSERT(EFX_MAE_RSRC_ID_ILWALID ==
	...
	case EFX_TUNNEL_PROTOCOL_GENEVE:
		...
		break;
	case EFX_TUNNEL_PROTOCOL_LWGRE:
		encap_type_mcdi = MAE_MCDI_ENCAP_TYPE_LWGRE;
		break;
	default:
		rc = ENOTSUP;
		goto fail3;
	...
	if (or_id.id == EFX_MAE_RSRC_ID_ILWALID) {
		rc = ENOENT;
		goto fail6;
	}
	...
	__out				efx_mae_aset_id_t *aset_idp)
{
	...
	/*
	 * TODO: Remove these EFX_MAE_RSRC_ID_ILWALID assignments once the
	 * corresponding resource types are supported by the implementation.
	 * Use proper resource ID assignments instead.
	 */
	MCDI_IN_SET_DWORD(req,
	    MAE_ACTION_SET_ALLOC_IN_COUNTER_LIST_ID, EFX_MAE_RSRC_ID_ILWALID);
	MCDI_IN_SET_DWORD(req,
	    MAE_ACTION_SET_ALLOC_IN_COUNTER_ID, EFX_MAE_RSRC_ID_ILWALID);
	MCDI_IN_SET_DWORD(req,
	    MAE_ACTION_SET_ALLOC_IN_ENCAP_HEADER_ID, EFX_MAE_RSRC_ID_ILWALID);
	...
	if (aset_id.id == EFX_MAE_RSRC_ID_ILWALID) {
		rc = ENOENT;
		goto fail4;
	}
	...
	__out				efx_mae_rule_id_t *ar_idp)
{
	...
	EFX_POPULATE_OWORD_3(*rule_response,
	    ...
	    (asl_idp != NULL) ? asl_idp->id : EFX_MAE_RSRC_ID_ILWALID,
	    MAE_ACTION_RULE_RESPONSE_AS_ID,
	    (as_idp != NULL) ? as_idp->id : EFX_MAE_RSRC_ID_ILWALID,
	    MAE_ACTION_RULE_RESPONSE_COUNTER_ID, EFX_MAE_RSRC_ID_ILWALID);
	...
	if (ar_id.id == EFX_MAE_RSRC_ID_ILWALID) {
		rc = ENOENT;
		goto fail5;
	}
