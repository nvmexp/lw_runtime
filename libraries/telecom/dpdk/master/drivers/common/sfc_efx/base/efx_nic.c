	...
	__deref_out	efx_nic_t **enpp)
{
	...
	EFSYS_ASSERT3U(family, >, EFX_FAMILY_ILWALID);
	...
	__in	efx_nic_t *enp)
{
	...
	enp->en_family = EFX_FAMILY_ILWALID;
	enp->en_esip = NULL;
	enp->en_esbp = NULL;
	enp->en_eslp = NULL;
	...
	__in		efx_nic_t *enp)
{
	...
	EFSYS_ASSERT(enp->en_mod_flags & EFX_MOD_PROBE);
	/*
	 * All modules except the MCDI, PROBE, LWRAM, VPD, MON, TUNNEL
	...
	mod_flags &= ~(EFX_MOD_MCDI | EFX_MOD_PROBE | EFX_MOD_LWRAM |
	...
	__out		efx_pcie_link_performance_t *resultp)
{
	...
	/* Callwlate the available bandwidth in megabits per second */
	rc = efx_nic_callwlate_pcie_link_bandwidth(pcie_link_width,
