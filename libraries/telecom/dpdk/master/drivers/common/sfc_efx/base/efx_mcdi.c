...
#if EFSYS_OPT_MCDI_PROXY_AUTH_SERVER
		/*
		 * A proxy MCDI command is exelwted by PF on behalf of
	...
	__inout		efx_mcdi_req_t *emrp)
{
	...
	emtp->emt_exelwte(emtp->emt_context, emrp);
}

			void
efx_mcdi_exelwte_quiet(
	__in		efx_nic_t *enp,
	__inout		efx_mcdi_req_t *emrp)
{
	const efx_mcdi_transport_t *emtp = enp->en_mcdi.em_emtp;
	...
	__in		efx_nic_t *enp)
{
	...
	if (epp->ep_fixed_port_type >= EFX_PHY_MEDIA_NTYPES)
		epp->ep_fixed_port_type = EFX_PHY_MEDIA_ILWALID;
	...
	__in		efsys_mem_t *esmp)
{
	...
	/*
	 * The MC DMAs aggregate statistics for our colwenience, so we can
	...
	__in		boolean_t low_latency)
{
	...
	if (encp->enc_init_evq_v2_supported) {
		...
		switch (flags & EFX_EVQ_FLAGS_TYPE_MASK) {
		case EFX_EVQ_FLAGS_TYPE_AUTO:
			ev_lwt_through = low_latency ? 1 : 0;
			break;
		case EFX_EVQ_FLAGS_TYPE_THROUGHPUT:
			ev_lwt_through = 0;
			break;
	...
	MCDI_IN_POPULATE_DWORD_8(req, INIT_EVQ_V2_IN_FLAGS,
	    ...
	    INIT_EVQ_V2_IN_FLAG_LWT_THRU, ev_lwt_through,
