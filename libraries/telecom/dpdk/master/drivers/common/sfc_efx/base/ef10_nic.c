	...
	__out_opt	uint32_t *default_modep)
{
	 ...
	 * was requested (LwrrentMode field was added for Medford).
	 */
	if (req.emr_out_length_used <
	    MC_CMD_GET_PORT_MODES_OUT_LWRRENT_MODE_OFST) {
		rc = EMSGSIZE;
		goto fail2;
	}
	if ((lwrrent_modep != NULL) && (req.emr_out_length_used <
	    MC_CMD_GET_PORT_MODES_OUT_LWRRENT_MODE_OFST + 4)) {
		rc = EMSGSIZE;
		goto fail3;
	}
	...
	*modesp = MCDI_OUT_DWORD(req, GET_PORT_MODES_OUT_MODES);
	...
	if (lwrrent_modep != NULL) {
		*lwrrent_modep = MCDI_OUT_DWORD(req,
					    GET_PORT_MODES_OUT_LWRRENT_MODE);
	}
	...
	__out_opt	uint32_t *vf_lwecp)
{
	...
	if (pf_lwecp != NULL)
		*pf_lwecp = MCDI_OUT_DWORD(req, GET_VECTOR_CFG_OUT_VECS_PER_PF);
	if (vf_lwecp != NULL)
		*vf_lwecp = MCDI_OUT_DWORD(req, GET_VECTOR_CFG_OUT_VECS_PER_VF);
...
fail1:
	for (i = 0; i < enp->en_arch.ef10.ena_piobuf_count; i++) {
		...
		*handlep = EFX_PIOBUF_HANDLE_ILWALID;
	}
	enp->en_arch.ef10.ena_piobuf_count = 0;
}
...
#define	CAP_FLAGS3(_req, _flag)						\
	...
	/*
	 * Check if firmware-verified LWRAM updates must be used.
	 *
	 * The firmware trusted installer requires all LWRAM updates to use
	 * version 2 of MC_CMD_LWRAM_UPDATE_START (to enable verified update)
	 * and version 2 of MC_CMD_LWRAM_UPDATE_FINISH (to verify the updated
	 * partition and report the result).
	 */
	if (CAP_FLAGS2(req, LWRAM_UPDATE_REPORT_VERIFY_RESULT))
		encp->enc_lwram_update_verify_result_supported = B_TRUE;
	else
		encp->enc_lwram_update_verify_result_supported = B_FALSE;

	if (CAP_FLAGS2(req, LWRAM_UPDATE_POLL_VERIFY_RESULT))
		encp->enc_lwram_update_poll_verify_result_supported = B_TRUE;
	else
		encp->enc_lwram_update_poll_verify_result_supported = B_FALSE;

	/*
	 * Check if firmware update via the BUNDLE partition is supported
	 */
	if (CAP_FLAGS2(req, BUNDLE_UPDATE))
		encp->enc_lwram_bundle_update_supported = B_TRUE;
	else
		encp->enc_lwram_bundle_update_supported = B_FALSE;
	...
	/*
	 * Check if firmware supports VXLAN and LWGRE tunnels.
	 * The capability indicates Geneve protocol support as well.
	 */
	if (CAP_FLAGS1(req, VXLAN_LWGRE)) {
		encp->enc_tunnel_encapsulations_supported =
		    (1u << EFX_TUNNEL_PROTOCOL_VXLAN) |
		    (1u << EFX_TUNNEL_PROTOCOL_GENEVE) |
		    (1u << EFX_TUNNEL_PROTOCOL_LWGRE);
	...
	if (req.emr_out_length_used >= MC_CMD_GET_CAPABILITIES_V3_OUT_LEN) {
		...
		case MC_CMD_GET_CAPABILITIES_V3_OUT_VI_WINDOW_MODE_64K:
			...
			break;
		default:
			encp->enc_vi_window_shift = EFX_VI_WINDOW_SHIFT_ILWALID;
			break;
		}
...
#define	EF10_LEGACY_PF_PRIVILEGE_MASK					\
	...
	MC_CMD_PRIVILEGE_MASK_IN_GRP_INSELWRE_FILTERS		|	\
	...
	MC_CMD_PRIVILEGE_MASK_IN_GRP_PROMISLWOUS)
 ...
 *
 * The count of conselwtive port numbers that map to each external number,
...
out:
	if (mapp != NULL) {
		/*
		 * External ports are assigned a sequence of conselwtive
	...
	__in		efx_nic_t *enp)
{
	...
	if ((rc = efx_mcdi_get_vector_cfg(enp, &base, &lwec, NULL)) != 0) {
		if (EFX_PCI_FUNCTION_IS_PF(encp))
			goto fail10;
