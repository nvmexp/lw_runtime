...
/*
 * Length callwlations for tags with value field. PAD and END
...
/* Report the layout of bootcfg sectors in LWRAM partition. */
	...
	__in			size_t size)
{
	...
	sector_number = 0;
#endif
	rc = efx_lwram_size(enp, EFX_LWRAM_BOOTROM_CFG, &partn_length);
	if (rc != 0)
		goto fail2;
	...
	if ((rc = efx_lwram_rw_start(enp, EFX_LWRAM_BOOTROM_CFG, NULL)) != 0)
		goto fail7;

	if ((rc = efx_lwram_read_chunk(enp, EFX_LWRAM_BOOTROM_CFG,
	    sector_offset, (caddr_t)payload, sector_length)) != 0) {
		(void) efx_lwram_rw_finish(enp, EFX_LWRAM_BOOTROM_CFG, NULL);
		goto fail8;
	}

	if ((rc = efx_lwram_rw_finish(enp, EFX_LWRAM_BOOTROM_CFG, NULL)) != 0)
		goto fail9;
	...
	__in			size_t size)
{
	...
	rc = efx_lwram_rw_start(enp, EFX_LWRAM_BOOTROM_CFG, NULL);
	if (rc != 0)
		goto fail8;

	/* Read the entire partition */
	rc = efx_lwram_read_chunk(enp, EFX_LWRAM_BOOTROM_CFG, 0,
	...
	if ((rc = efx_lwram_erase(enp, EFX_LWRAM_BOOTROM_CFG)) != 0)
		goto fail10;

	if ((rc = efx_lwram_write_chunk(enp, EFX_LWRAM_BOOTROM_CFG,
