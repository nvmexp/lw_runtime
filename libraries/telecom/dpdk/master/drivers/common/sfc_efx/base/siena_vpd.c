	...
	__out				size_t *sizep)
{
	...
	EFSYS_ASSERT(partn == MC_CMD_LWRAM_TYPE_STATIC_CFG_PORT0 ||
		    partn == MC_CMD_LWRAM_TYPE_STATIC_CFG_PORT1);
	...
	if ((rc = siena_lwram_partn_read(enp, partn, 0,
	    (caddr_t)scfg, SIENA_LWRAM_CHUNK)) != 0)
		goto fail4;
	...
	if (region > SIENA_LWRAM_CHUNK) {
		if ((rc = siena_lwram_partn_read(enp, partn, SIENA_LWRAM_CHUNK,
		    (caddr_t)scfg + SIENA_LWRAM_CHUNK,
	...
	__in			efx_nic_t *enp)
{
	...
	partn = (emip->emi_port == 1)
		? MC_CMD_LWRAM_TYPE_STATIC_CFG_PORT0
		: MC_CMD_LWRAM_TYPE_STATIC_CFG_PORT1;
	...
	__out			size_t *sizep)
{
	...
	if ((rc = siena_lwram_partn_size(enp, partn, sizep)) != 0)
		goto fail1;
	...
	__in			size_t size)
{
	...
	if ((rc = siena_lwram_partn_size(enp, dcfg_partn, &partn_size)) != 0)
		goto fail2;
	...
	if ((rc = siena_lwram_partn_erase(enp, dcfg_partn, 0, partn_size)) != 0)
		goto fail6;

	/* Write out the new structure to lwram */
	if ((rc = siena_lwram_partn_write(enp, dcfg_partn, 0, (caddr_t)dcfg,
