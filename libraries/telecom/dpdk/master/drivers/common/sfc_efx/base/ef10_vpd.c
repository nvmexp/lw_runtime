	...
	__in			efx_nic_t *enp)
{
	...
	svpd = NULL;
	svpd_size = 0;
	rc = ef10_lwram_partn_read_tlv(enp,
	    LWRAM_PARTITION_TYPE_STATIC_CONFIG,
	    tag, &svpd, &svpd_size);
	if (rc != 0) {
		if (rc == EACCES) {
	...
	__out			size_t *sizep)
{
	 ...
	 */
	if ((rc = efx_mcdi_lwram_info(enp,
		    LWRAM_PARTITION_TYPE_DYNAMIC_CONFIG, &eni)) != 0)
		goto fail1;
	...
	__in			size_t size)
{
	...
	if ((rc = ef10_lwram_partn_read_tlv(enp,
