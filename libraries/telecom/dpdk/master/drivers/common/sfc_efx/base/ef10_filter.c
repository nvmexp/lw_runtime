	...
	if (filter_op != MC_CMD_FILTER_OP_IN_OP_REPLACE) {
		...
		case EFX_TUNNEL_PROTOCOL_LWGRE:
			MCDI_IN_POPULATE_DWORD_1(req,
			    ...
			    MC_CMD_FILTER_OP_EXT_IN_VSID_TYPE_LWGRE);
			break;
		default:
			EFSYS_ASSERT(0);
			rc = EILWAL;
 ...
 * delivery to additional recipients.  Lwrrently we decide that
	...
	__out		efx_filter_spec_t **overridden_spec)
{
	...
	if (saved_spec == NULL) {
		*action = EF10_FILTER_ADD_NEW;
		 ...
		 * in the table. The only case ilwolving a saved spec is
		...
		*action = EF10_FILTER_ADD_REFRESH;
		...
		if (spec->efs_priority > saved_spec->efs_priority) {
			...
			*action = EF10_FILTER_ADD_REPLACE;
			 ...
			 *
			 * Lwrrently there are only two priority levels, so this
...
static	__checkReturn	efx_rc_t
ef10_filter_add_exelwte_action(
	__in		efx_nic_t *enp,
	__in		efx_filter_spec_t *saved_spec,
	__in		efx_filter_spec_t *spec,
...
retry:
	...
	rc = ef10_filter_add_exelwte_action(enp, saved_spec, spec,
	...
	__out				size_t *list_lengthp)
{
	...
	*list_lengthp = matches_count;
	 ...
	 * we expect, so we can just copy them directly. Any colwersion of the
...
static ef10_filter_encap_entry_t ef10_filter_encap_list[] = {
	...
	EF10_ENCAP_FILTER_ENTRY(IPV4, LWGRE, UCAST_DST),
	EF10_ENCAP_FILTER_ENTRY(IPV4, LWGRE, MCAST_DST),
	EF10_ENCAP_FILTER_ENTRY(IPV6, LWGRE, UCAST_DST),
	EF10_ENCAP_FILTER_ENTRY(IPV6, LWGRE, MCAST_DST),
};
	...
	__in				uint32_t count)
{
	...
	if (table->eft_default_rxq == NULL) {
		 ...
		 * inserted until it is available. Any lwrrently configured
	...
	__in		boolean_t using_rss)
{
	...
	EFSYS_ASSERT((using_rss == B_FALSE) ||
	    (enp->en_rss_context != EF10_RSS_CONTEXT_ILWALID));
