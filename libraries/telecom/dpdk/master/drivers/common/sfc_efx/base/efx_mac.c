...
{
	efx_port_t *epp = &(enp->en_port);
	efx_mac_type_t type = EFX_MAC_ILWALID;
	...
	EFSYS_ASSERT(type != EFX_MAC_ILWALID);
	...
	__out				efx_oword_t *hash_high)
{
	...
	for (i = 0; i < count; i++) {
		/* Callwlate hash bucket (IEEE 802.3 CRC32 of the MAC addr) */
		crc = efx_crc32_callwlate(0xffffffff, addrs, EFX_MAC_ADDR_LEN);
