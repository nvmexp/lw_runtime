...
{
	efx_mcdi_req_t req;
	EFX_MCDI_DECLARE_BUF(payload, MC_CMD_LWRAM_TYPES_IN_LEN,
		MC_CMD_LWRAM_TYPES_OUT_LEN);
	efx_rc_t rc;

	req.emr_cmd = MC_CMD_LWRAM_TYPES;
	req.emr_in_buf = payload;
	req.emr_in_length = MC_CMD_LWRAM_TYPES_IN_LEN;
	req.emr_out_buf = payload;
	req.emr_out_length = MC_CMD_LWRAM_TYPES_OUT_LEN;
	...
	if (req.emr_out_length_used < MC_CMD_LWRAM_TYPES_OUT_LEN) {
		rc = EMSGSIZE;
		goto fail2;
	}

	*maskp = MCDI_OUT_DWORD(req, LWRAM_TYPES_OUT_TYPES);
...
#if EFSYS_OPT_PHY_STATS
	/* Colwert the MCDI statistic mask into the EFX_PHY_STAT mask */
