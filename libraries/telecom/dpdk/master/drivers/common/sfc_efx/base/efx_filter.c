...
/*
 * Specify inner and outer Ethernet address and vsid in LWGRE filter
 * specification.
 */
__checkReturn		efx_rc_t
efx_filter_spec_set_lwgre(
	...
	__in		const uint8_t *outer_addr)
{
	return efx_filter_spec_set_tunnel(spec, EFX_TUNNEL_PROTOCOL_LWGRE,
	...
	__in		efx_filter_spec_t *gen_spec)
{
	...
	case EFX_FILTER_MATCH_ETHER_TYPE | EFX_FILTER_MATCH_IP_PROTO |
		 ...
		 * wildcard filter. We need to colwert from local and remote
