		      ...
		      u32 domain_id)
{
	...
	/* To reduce the odds of conselwtive load-balanced ports mapping to the
...
{
	enum dlb_qid_map_state lwrr_state = port->qid_map[slot].state;
	struct dlb_domain *domain;
	...
	DLB_HW_INFO(hw,
		    ...
		    __func__, queue->id, port->id, lwrr_state,
		    new_state);
	return 0;
 ...
 * at a later point, and <0 if an error oclwrred.
 */
				...
				struct dlb_cmd_response *resp)
{
	...
	if (domain == NULL) {
		resp->status = DLB_ST_ILWALID_DOMAIN_ID;
		return -1;
	}
 ...
 * before ilwoking this function, otherwise undefined behavior will result.
 *
				 ...
				 struct dlb_cmd_response *resp)
{
	...
	if (args->num_qid_inflights > 4096) {
		resp->status = DLB_ST_ILWALID_QID_INFLIGHT_ALLOCATION;
		return -1;
	}
				 ...
				 struct dlb_cmd_response *resp)
{
	...
	if (args->port_id != -1) {
		...
		if (port  == NULL || port->domain_id != domain->id ||
			...
			resp->status = DLB_ST_ILWALID_PORT_ID;
			return -1;
		}
	}
				...
				struct dlb_cmd_response *resp)
{
	...
	if (!dlb_list_empty(&domain->used_ldb_queues) ||
		...
		if (pool  == NULL || !pool->configured ||
		    pool->domain_id != domain->id) {
			resp->status = DLB_ST_ILWALID_LDB_CREDIT_POOL_ID;
			return -1;
		}
		...
		if (args->ldb_credit_low_watermark >=
		    args->ldb_credit_high_watermark) {
			resp->status = DLB_ST_ILWALID_LDB_CREDIT_LOW_WATERMARK;
			return -1;
		}

		if (args->ldb_credit_quantum >=
		    args->ldb_credit_high_watermark) {
			resp->status = DLB_ST_ILWALID_LDB_CREDIT_QUANTUM;
			return -1;
		}
	...
	if (!dlb_list_empty(&domain->used_dir_pq_pairs) ||
		...
		if (pool  == NULL || !pool->configured ||
		    pool->domain_id != domain->id) {
			resp->status = DLB_ST_ILWALID_DIR_CREDIT_POOL_ID;
			return -1;
		}
		...
		if (args->dir_credit_low_watermark >=
		    args->dir_credit_high_watermark) {
			resp->status = DLB_ST_ILWALID_DIR_CREDIT_LOW_WATERMARK;
			return -1;
		}

		if (args->dir_credit_quantum >=
		    args->dir_credit_high_watermark) {
			resp->status = DLB_ST_ILWALID_DIR_CREDIT_QUANTUM;
			return -1;
		}
	...
	if ((pop_count_dma_base & 0x3F) != 0) {
		resp->status = DLB_ST_ILWALID_POP_COUNT_VIRT_ADDR;
		return -1;
	}

	if ((cq_dma_base & 0x3F) != 0) {
		resp->status = DLB_ST_ILWALID_CQ_VIRT_ADDR;
		return -1;
	}
	...
	if (args->cq_depth != 1 &&
	    ...
	    args->cq_depth != 512 &&
	    args->cq_depth != 1024) {
		resp->status = DLB_ST_ILWALID_CQ_DEPTH;
		return -1;
	}

	/* The history list size must be >= 1 */
	if (!args->cq_history_list_size) {
		resp->status = DLB_ST_ILWALID_HIST_LIST_DEPTH;
		return -1;
	}
				...
				struct dlb_cmd_response *resp)
{
	...
	if (args->queue_id != -1) {
		...
		if (queue  == NULL || queue->domain_id != domain->id ||
			...
			resp->status = DLB_ST_ILWALID_DIR_QUEUE_ID;
			return -1;
		}
	}
				     ...
				     struct dlb_cmd_response *resp)
{
	...
	if (queue == NULL || !queue->configured) {
		DLB_HW_ERR(hw, "[%s()] Can't unmap unconfigured queue %d\n",
			   __func__, args->qid);
		resp->status = DLB_ST_ILWALID_QID;
		return -1;
	}
				   ...
				   struct dlb_cmd_response *resp)
{
	...
	if (args->priority >= DLB_QID_PRIORITIES) {
		resp->status = DLB_ST_ILWALID_PRIORITY;
		return -1;
	}
		   ...
		   struct dlb_cmd_response *resp)
{
	 ...
	 * the slot's current oclwpant, this mapping will be performed.
	 */
