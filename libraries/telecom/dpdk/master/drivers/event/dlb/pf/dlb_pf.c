		       ...
		       enum dlb_cq_poll_modes poll_mode)
{
	...
	/* Callwlate the port memory required, including two cache lines for
...
static int
dlb_pf_get_sn_oclwpancy(struct dlb_hw_dev *handle,
			struct dlb_get_sn_oclwpancy_args *args)
{
	struct dlb_dev *dlb_dev = (struct dlb_dev *)handle->pf_dev;
	struct dlb_cmd_response response = {0};
	int ret;

	ret = dlb_get_group_sequence_number_oclwpancy(&dlb_dev->hw,
						      args->group);
...
dlb_pf_iface_fn_ptrs_init(void)
{
	...
	dlb_iface_get_sn_oclwpancy = dlb_pf_get_sn_oclwpancy;
