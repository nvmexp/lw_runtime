...
static int
dlb2_pf_get_sn_oclwpancy(struct dlb2_hw_dev *handle,
			 struct dlb2_get_sn_oclwpancy_args *args)
{
	struct dlb2_dev *dlb2_dev = (struct dlb2_dev *)handle->pf_dev;
	struct dlb2_cmd_response response = {0};
	int ret;

	ret = dlb2_get_group_sequence_number_oclwpancy(&dlb2_dev->hw,
						       args->group);
			...
			enum dlb2_cq_poll_modes poll_mode)
{
	...
	/* Callwlate the port memory required, and round up to the nearest
	 * cache line.
	 */
...
dlb2_pf_iface_fn_ptrs_init(void)
{
	...
	dlb2_iface_get_sn_oclwpancy = dlb2_pf_get_sn_oclwpancy;
}
...
dlb2_eventdev_pci_init(struct rte_eventdev *eventdev)
{
	...
	if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
		...
		/* Were we ilwoked with runtime parameters? */
