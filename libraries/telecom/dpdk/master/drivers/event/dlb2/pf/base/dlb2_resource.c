...
{
	enum dlb2_qid_map_state lwrr_state = port->qid_map[slot].state;
	struct dlb2_hw_domain *domain;
	int domain_id;
				  ...
				  unsigned int vdev_id)
{
	...
	if (domain == NULL) {
		resp->status = DLB2_ST_ILWALID_DOMAIN_ID;
		return -EILWAL;
	}
	...
	if (args->num_qid_inflights > 4096) {
		resp->status = DLB2_ST_ILWALID_QID_INFLIGHT_ALLOCATION;
		return -EILWAL;
	}
	...
	if (args->num_atomic_inflights &&
		...
		resp->status = DLB2_ST_ILWALID_LOCK_ID_COMP_LEVEL;
		return -EILWAL;
	}
				 ...
				 unsigned int vdev_id)
{
	...
	if (args->cos_id >= DLB2_NUM_COS_DOMAINS) {
		resp->status = DLB2_ST_ILWALID_COS_ID;
		return -EILWAL;
	}
	...
	if ((cq_dma_base & 0x3F) != 0) {
		resp->status = DLB2_ST_ILWALID_CQ_VIRT_ADDR;
		return -EILWAL;
	}
	...
	if (args->cq_depth != 1 &&
	    ...
	    args->cq_depth != 512 &&
	    args->cq_depth != 1024) {
		resp->status = DLB2_ST_ILWALID_CQ_DEPTH;
		return -EILWAL;
	}

	/* The history list size must be >= 1 */
	if (!args->cq_history_list_size) {
		resp->status = DLB2_ST_ILWALID_HIST_LIST_DEPTH;
		return -EILWAL;
	}
				 ...
				 unsigned int vdev_id)
{
	...
	if (args->queue_id != -1) {
		...
		if (queue == NULL || queue->domain_id.phys_id !=
			...
			resp->status = DLB2_ST_ILWALID_DIR_QUEUE_ID;
			return -EILWAL;
		}
	}
				  ...
				  unsigned int vdev_id)
{
	...
	if (args->port_id != -1) {
		...
		if (port == NULL || port->domain_id.phys_id !=
			...
			resp->status = DLB2_ST_ILWALID_PORT_ID;
			return -EILWAL;
		}
	}
				    ...
				    unsigned int vdev_id)
{
	...
	if (args->priority >= DLB2_QID_PRIORITIES) {
		resp->status = DLB2_ST_ILWALID_PRIORITY;
		return -EILWAL;
	}
	...
	if (queue == NULL || !queue->configured) {
		resp->status = DLB2_ST_ILWALID_QID;
		return -EILWAL;
	}
