...
enum dlb2_xstats_type {
	...
	tx_ilwalid,                     /**< Send an event with an invalid op */
	...
	rx_sched_ilwalid,               /**< Dequeue event sched type invalid */
	...
	lwrrent_depth,			/**< Current queue depth */
	      ...
	      enum dlb2_xstats_type type, int extra_arg __rte_unused)
{
	...
	case tx_ilwalid: return ev_port->stats.tx_ilwalid;
	...
	case rx_sched_ilwalid: return ev_port->stats.rx_sched_ilwalid;
	       ...
	       enum dlb2_xstats_type type, int extra_arg __rte_unused)
{
	...
	case lwrrent_depth: return dlb2_get_queue_depth(dlb2, ev_queue);
			...
			const uint32_t ids[], uint32_t nb_ids)
{
	...
	if (ids) {
		uint32_t nb_reset = dlb2_xstats_update(dlb2,
					...
					reset);
		return nb_reset == nb_ids ? 0 : -EILWAL;
	}
...
dlb2_eventdev_dump(struct rte_eventdev *dev, FILE *f)
{
	...
	for (i = 0; i < dlb2->num_ports; i++) {
		...
		fprintf(f, "\t\trx_sched_ilwalid %" PRIu64 "\n",
			p->stats.rx_sched_ilwalid);
	}
	...
	for (i = 0; i < dlb2->num_queues; i++) {
		...
		fprintf(f, "\tlwrrent depth: %u events\n",
