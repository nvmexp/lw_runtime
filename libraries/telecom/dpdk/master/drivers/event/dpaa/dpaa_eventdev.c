...
dpaa_eventq_portal_add(u16 ch_id)
{
	...
	sdqcr = QM_SDQCR_CHANNELS_POOL_COLW(ch_id);
			 ...
			 uint16_t nb_events, uint64_t timeout_ticks)
{
	...
	void *buffers[8];
	u32 num_frames, i;
	uint64_t lwr_ticks = 0, wait_time_ticks = 0;
	...
	do {
		...
		if (num_frames)
			break;
		lwr_ticks = rte_get_timer_cycles();
	} while (lwr_ticks < wait_time_ticks);
...
dpaa_event_dev_configure(const struct rte_eventdev *dev)
{
	...
	if (priv->event_dev_cfg & RTE_EVENT_DEV_CFG_PER_DEQUEUE_TIMEOUT) {
		 ...
		 * So ilwalidating this timeout value.
		 */
	...
	/*
	 * TODO: Lwrrently portals are affined with threads. Maximum threads
