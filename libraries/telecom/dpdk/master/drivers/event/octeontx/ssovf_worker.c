...
{
	const uint8_t lwr_tt = ws->lwr_tt;
	const uint8_t new_tt = ev->sched_type;
	const uint32_t tag = (uint32_t)ev->event;
	/*
	 * lwr_tt/new_tt     SSO_SYNC_ORDERED SSO_SYNC_ATOMIC SSO_SYNC_UNTAGGED
	 *
	 * SSO_SYNC_ORDERED        norm           norm             untag
	 * SSO_SYNC_ATOMIC         norm           norm		   untag
	 * SSO_SYNC_UNTAGGED       full           full             NOOP
	 */
	if (unlikely(lwr_tt == SSO_SYNC_UNTAGGED)) {
...
{
	if (likely(ws->lwr_tt != SSO_SYNC_UNTAGGED))
		ssows_swtag_untag(ws);
}
				...
				ssows_handle_event_t fn, void *arg)
{
	...
	while (aq_cnt || cq_ds_cnt) {
		...
		ws->lwr_tt = sched_type_queue & 0x3;
		ws->lwr_grp = sched_type_queue >> 2;
