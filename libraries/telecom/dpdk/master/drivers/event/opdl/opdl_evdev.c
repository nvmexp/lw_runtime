	       ...
	       uint16_t num)
{
	...
	if (p->external_qid != OPDL_ILWALID_QID) {
		PMD_DRV_LOG(ERR, "DEV_ID:[%02d] : "
		 ...
		 uint16_t nb_unlinks)
{
	...
	/* Port Stuff */
	p->queue_id = OPDL_ILWALID_QID;
	p->p_type = OPDL_ILWALID_PORT;
	p->external_qid = OPDL_ILWALID_QID;
...
opdl_close(struct rte_eventdev *dev)
{
	...
	memset(device->q_map_ex_to_in,
			0,
			sizeof(uint8_t)*OPDL_ILWALID_QID);
