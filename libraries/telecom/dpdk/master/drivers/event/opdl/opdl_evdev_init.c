...
check_queues_linked(struct rte_eventdev *dev)
{
	...
	for (i = 0; i < RTE_EVENT_MAX_QUEUES_PER_DEV; i++) {
		...
		if (queue->external_qid == OPDL_ILWALID_QID)
			nb_iq++;
		...
		int32_t i)
{
	...
	if (i == -1) {
		...
		queue->external_qid = OPDL_ILWALID_QID;
	} else {
...
initialise_all_other_ports(struct rte_eventdev *dev)
{
	...
	if (!err) {
		for (i = 0; i < device->nb_ports; i++) {
			...
			if (port->configured &&
					(port->queue_id != OPDL_ILWALID_QID)) {
				if (queue->nb_ports == 0) {
					PMD_DRV_LOG(ERR, "DEV_ID:[%02d] : "
...
initialise_queue_zero_ports(struct rte_eventdev *dev)
{
	...
	for (i = 0; i < device->nb_ports; i++) {
		struct opdl_port *port = &device->ports[i];
		if (port->queue_id == OPDL_ILWALID_QID) {
			port->queue_id = 0;
			port->external_qid = OPDL_ILWALID_QID;
...
assign_internal_queue_ids(struct rte_eventdev *dev)
{
	...
	for (i = 0; i < device->nb_ports; i++) {
		struct opdl_port *port = &device->ports[i];
		if (port->external_qid != OPDL_ILWALID_QID) {
			port->queue_id =
