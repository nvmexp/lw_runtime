			 ...
			 int32_t credits)
{
	...
	/* This is a race, no locks are ilwolved, and thus some other
...
{
	uint16_t i;
	struct dsw_queue_flow_burst *lwrrent_burst = NULL;
	...
	/* arrange the (now-conselwtive) events into bursts */
	for (i = 0; i < qfs_len; i++) {
		if (i == 0 ||
		    dsw_cmp_qf(&qfs[i], &lwrrent_burst->queue_flow) != 0) {
			lwrrent_burst = &bursts[num_bursts];
			lwrrent_burst->queue_flow = qfs[i];
			lwrrent_burst->count = 0;
			num_bursts++;
		}
		lwrrent_burst->count++;
	}
