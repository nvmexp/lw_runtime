...
#undef T
	...
	if (dev->rx_offloads & NIX_RX_MULTI_SEG_F) {
		event_dev->dequeue		= ssogws_deq_seg
			[!!(dev->rx_offloads & NIX_RX_OFFLOAD_SELWRITY_F)]
	...
	if (dev->tx_offloads & NIX_TX_MULTI_SEG_F) {
		...
		event_dev->txa_enqueue = ssogws_tx_adptr_enq_seg
			[!!(dev->tx_offloads & NIX_TX_OFFLOAD_SELWRITY_F)]
...
sso_xaq_allocate(struct otx2_sso_evdev *dev)
{
	 ...
	 * comparing fc_addr above against the xaq_lmt callwlated below.
...
sso_cleanup(struct rte_eventdev *event_dev, uint8_t enable)
{
	...
	for (i = 0; i < dev->nb_event_ports; i++) {
		if (dev->dual_ws) {
			...
			ws->swtag_req = 0;
			ws->vws = 0;
			ws->ws_state[0].lwr_grp = 0;
			ws->ws_state[0].lwr_tt = SSO_SYNC_EMPTY;
			ws->ws_state[1].lwr_grp = 0;
			ws->ws_state[1].lwr_tt = SSO_SYNC_EMPTY;
		} else {
			struct otx2_ssogws *ws;
	...
	/* reset SSO GWS cache */
	otx2_mbox_alloc_msg_sso_ws_cache_ilw(dev->mbox);
	otx2_mbox_process(dev->mbox);
}
