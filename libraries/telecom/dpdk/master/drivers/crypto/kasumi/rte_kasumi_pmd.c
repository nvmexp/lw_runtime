...
process_op_bit(struct rte_crypto_op *op, struct kasumi_session *session,
		struct kasumi_qp *qp, uint16_t *aclwmulated_enqueued_ops)
{
	...
	*aclwmulated_enqueued_ops += enqueued_op;
		...
		uint16_t nb_ops)
{
	...
	struct kasumi_session *prev_sess = NULL, *lwrr_sess = NULL;
	...
	for (i = 0; i < nb_ops; i++) {
		...
		if (!rte_pktmbuf_is_contiguous(lwrr_c_op->sym->m_src) ||
				(lwrr_c_op->sym->m_dst != NULL &&
				!rte_pktmbuf_is_contiguous(
						lwrr_c_op->sym->m_dst))) {
			KASUMI_LOG(ERR, "PMD supports only contiguous mbufs, "
				"op (%p) provides noncontiguous mbuf as "
				"source/destination buffer.", lwrr_c_op);
			lwrr_c_op->status = RTE_CRYPTO_OP_STATUS_ILWALID_ARGS;
			break;
		}
#endif

		/* Set status as enqueued (not processed yet) by default. */
		lwrr_c_op->status = RTE_CRYPTO_OP_STATUS_NOT_PROCESSED;

		lwrr_sess = kasumi_get_session(qp, lwrr_c_op);
		if (unlikely(lwrr_sess == NULL ||
				lwrr_sess->op == KASUMI_OP_NOT_SUPPORTED)) {
			lwrr_c_op->status =
		...
		if (((lwrr_c_op->sym->cipher.data.length % BYTE_LEN) != 0)
			...
			processed_ops = process_op_bit(lwrr_c_op, lwrr_sess,
						qp, &enqueued_ops);
			if (processed_ops != 1)
				break;
