...
/* process modilw operation */
static int
process_openssl_modilw_op(struct rte_crypto_op *cop,
		struct openssl_asym_session *sess)
{
	...
	base = BN_bin2bn((const unsigned char *)op->modilw.base.data,
			op->modilw.base.length, base);

	if (BN_mod_ilwerse(res, base, sess->u.m.modulus, sess->u.m.ctx)) {
		cop->status = RTE_CRYPTO_OP_STATUS_SUCCESS;
		op->modilw.result.length = BN_bn2bin(res, op->modilw.result.data);
	} else {
		cop->status = RTE_CRYPTO_OP_STATUS_ERROR;
	}
		...
		struct openssl_asym_session *sess)
{
	...
	case RTE_CRYPTO_RSA_PADDING_NONE:
		...
		break;
	default:
		cop->status = RTE_CRYPTO_OP_STATUS_ILWALID_ARGS;
		OPENSSL_LOG(ERR,
		...
		struct openssl_asym_session *sess)
{
	...
	case RTE_CRYPTO_ASYM_XFORM_MODILW:
		retval = process_openssl_modilw_op(op, sess);
		break;
