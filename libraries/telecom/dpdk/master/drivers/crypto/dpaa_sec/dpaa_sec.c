		  ...
		  const struct qm_dqrr_entry *dqrr)
{
	...
	if (ctx->op->sess_type == RTE_CRYPTO_OP_SELWRITY_SESSION) {
...
dpaa_sec_prep_pdcp_cdb(dpaa_sec_session *ses)
{
	...
	if (ses->pdcp.domain == RTE_SELWRITY_PDCP_MODE_CONTROL) {
		if (ses->dir == DIR_ENC)
...
dpaa_sec_deq(struct dpaa_sec_qp *qp, struct rte_crypto_op **ops, int nb_ops)
{
	...
	do {
		...
		ctx->fd_status = fd->status;
		op = ctx->op;
		if (op->sess_type == RTE_CRYPTO_OP_SELWRITY_SESSION) {
			struct qm_sg_entry *sg_out;
			uint32_t len;
...
build_auth_only_sg(struct rte_crypto_op *op, dpaa_sec_session *ses)
{
	...
	if (ses->iv.length) {
		...
		if (ses->auth_alg == RTE_CRYPTO_AUTH_SNOW3G_UIA2) {
			iv_ptr = colw_to_snow_f9_iv(iv_ptr);
			sg->length = 12;
		} else if (ses->auth_alg == RTE_CRYPTO_AUTH_ZUC_EIA3) {
			iv_ptr = colw_to_zuc_eia_iv(iv_ptr);
			sg->length = 8;
		} else {
	...
	while (nb_ops) {
		...
		for (loop = 0; loop < frames_to_send; loop++) {
			...
			case RTE_CRYPTO_OP_WITH_SESSION:
				...
				break;
#ifdef RTE_LIB_SELWRITY
			case RTE_CRYPTO_OP_SELWRITY_SESSION:
				ses = (dpaa_sec_session *)
...
dpaa_sec_ipsec_aead_init(struct rte_crypto_aead_xform *aead_xform,
			struct rte_selwrity_ipsec_xform *ipsec_xform,
			dpaa_sec_session *session)
{
	PMD_INIT_FUNC_TRACE();
...
{
	struct dpaa_sec_dev_private *internals = dev->data->dev_private;
	struct rte_selwrity_ipsec_xform *ipsec_xform = &conf->ipsec;
	...
	if (ipsec_xform->direction == RTE_SELWRITY_IPSEC_SA_DIR_EGRESS)
		session->dir = DIR_ENC;
	else
		session->dir = DIR_DEC;
	...
	if (ipsec_xform->direction == RTE_SELWRITY_IPSEC_SA_DIR_EGRESS) {
		if (ipsec_xform->tunnel.type ==
				RTE_SELWRITY_IPSEC_TUNNEL_IPV4) {
		...
		if (ipsec_xform->tunnel.type == RTE_SELWRITY_IPSEC_TUNNEL_IPV4)
...
dpaa_sec_set_pdcp_session(struct rte_cryptodev *dev,
			  ...
			  void *sess)
{
	struct rte_selwrity_pdcp_xform *pdcp_xform = &conf->pdcp;
	...
	if (pdcp_xform->domain == RTE_SELWRITY_PDCP_MODE_CONTROL) {
		if (pdcp_xform->sn_size != RTE_SELWRITY_PDCP_SN_SIZE_5 &&
		    pdcp_xform->sn_size != RTE_SELWRITY_PDCP_SN_SIZE_12) {
			DPAA_SEC_ERR(
...
static int
dpaa_sec_selwrity_session_create(void *dev,
				 struct rte_selwrity_session_conf *conf,
				 ...
				 struct rte_mempool *mempool)
{
	...
	case RTE_SELWRITY_PROTOCOL_IPSEC:
		ret = dpaa_sec_set_ipsec_session(cdev, conf,
				sess_private_data);
		break;
	case RTE_SELWRITY_PROTOCOL_PDCP:
		ret = dpaa_sec_set_pdcp_session(cdev, conf,
				sess_private_data);
		break;
	case RTE_SELWRITY_PROTOCOL_MACSEC:
		return -ENOTSUP;
	default:
		return -EILWAL;
	}
...
/** Clear the memory of session so it doesn't leave key material behind */
static int
dpaa_sec_selwrity_session_destroy(void *dev __rte_unused,
		struct rte_selwrity_session *sess)
{
	PMD_INIT_FUNC_TRACE();
...
{
	return dpaa_sec_selwrity_cap;
}

static const struct rte_selwrity_ops dpaa_sec_selwrity_ops = {
	.session_create = dpaa_sec_selwrity_session_create,
	.session_update = NULL,
	.session_stats_get = NULL,
	.session_destroy = dpaa_sec_selwrity_session_destroy,
...
dpaa_sec_dev_init(struct rte_cryptodev *cryptodev)
{
	struct dpaa_sec_dev_private *internals;
#ifdef RTE_LIB_SELWRITY
	struct rte_selwrity_ctx *selwrity_instance;
#endif
	...
	selwrity_instance = rte_malloc("rte_selwrity_instances_ops",
				sizeof(struct rte_selwrity_ctx), 0);
	if (selwrity_instance == NULL)
		return -ENOMEM;
	selwrity_instance->device = (void *)cryptodev;
	selwrity_instance->ops = &dpaa_sec_selwrity_ops;
	selwrity_instance->sess_cnt = 0;
	cryptodev->selwrity_ctx = selwrity_instance;
#endif
...
init_error:
	...
	rte_free(cryptodev->selwrity_ctx);
	return -EFAULT;
}
				...
				struct rte_dpaa_device *dpaa_dev)
{
	...
	/* Ilwoke PMD device initialization function */
