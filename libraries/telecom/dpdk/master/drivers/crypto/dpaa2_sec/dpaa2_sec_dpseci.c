		     ...
		     struct qbman_fd *fd, uint16_t bpid)
{
	 ...
	 * Lwrrently we donot know which FLE has the mbuf stored.
...
#ifdef RTE_LIB_SELWRITY
	else if (op->sess_type == RTE_CRYPTO_OP_SELWRITY_SESSION)
			   ...
			   void *sess)
{
	...
	if (session->auth_alg) {
		...
		p_authdata = &authdata;
	} else if (pdcp_xform->domain == RTE_SELWRITY_PDCP_MODE_CONTROL) {
...
static int
dpaa2_sec_selwrity_session_create(void *dev,
				  struct rte_selwrity_session_conf *conf,
...
static int
dpaa2_sec_selwrity_session_destroy(void *dev __rte_unused,
		struct rte_selwrity_session *sess)
{
	PMD_INIT_FUNC_TRACE();
...
{
	return dpaa2_sec_selwrity_cap;
}

static const struct rte_selwrity_ops dpaa2_sec_selwrity_ops = {
	.session_create = dpaa2_sec_selwrity_session_create,
	.session_update = NULL,
	.session_stats_get = NULL,
	.session_destroy = dpaa2_sec_selwrity_session_destroy,
...
dpaa2_sec_dev_init(struct rte_cryptodev *cryptodev)
{
	...
	selwrity_instance->ops = &dpaa2_sec_selwrity_ops;
