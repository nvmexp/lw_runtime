		...
		enum rte_crypto_asym_xform_type alg, int in_size, int out_size)
{
	if (alg == RTE_CRYPTO_ASYM_XFORM_MODEX)
		qat_clear_arrays(cookie, QAT_ASYM_MODEXP_NUM_IN_PARAMS,
				...
				out_size);
	else if (alg == RTE_CRYPTO_ASYM_XFORM_MODILW)
		qat_clear_arrays(cookie, QAT_ASYM_MODILW_NUM_IN_PARAMS,
				QAT_ASYM_MODILW_NUM_OUT_PARAMS, in_size,
				out_size);
}
		...
		struct rte_crypto_asym_xform *xform)
{
	...
	if (xform->xform_type == RTE_CRYPTO_ASYM_XFORM_MODEX) {
		...
		QAT_DP_HEXDUMP_LOG(DEBUG, " ModExpmodulus",
				...
				alg_size_in_bytes);
#endif
	} else if (xform->xform_type == RTE_CRYPTO_ASYM_XFORM_MODILW) {
		err = qat_asym_check_nonzero(xform->modilw.modulus);
		if (err) {
			QAT_LOG(ERR, "Empty modulus in modular multiplicative"
					" ilwerse, aborting this operation");
			return err;
		}

		alg_size_in_bytes = max_of(2, asym_op->modilw.base.length,
				xform->modilw.modulus.length);
		alg_size = alg_size_in_bytes << 3;

		if (xform->modilw.modulus.data[
				xform->modilw.modulus.length - 1] & 0x01) {
			if (qat_asym_get_sz_and_func_id(MOD_ILW_IDS_ODD,
					sizeof(MOD_ILW_IDS_ODD)/
					sizeof(*MOD_ILW_IDS_ODD),
					&alg_size, &func_id)) {
				return -(EILWAL);
			}
		} else {
			if (qat_asym_get_sz_and_func_id(MOD_ILW_IDS_EVEN,
					sizeof(MOD_ILW_IDS_EVEN)/
					sizeof(*MOD_ILW_IDS_EVEN),
					&alg_size, &func_id)) {
				return -(EILWAL);
			}
		...
		alg_size_in_bytes = alg_size >> 3;
		rte_memcpy(cookie->input_array[0] + alg_size_in_bytes -
			asym_op->modilw.base.length
				, asym_op->modilw.base.data,
				asym_op->modilw.base.length);
		rte_memcpy(cookie->input_array[1] + alg_size_in_bytes -
				xform->modilw.modulus.length
				, xform->modilw.modulus.data,
				xform->modilw.modulus.length);
		cookie->alg_size = alg_size;
		qat_req->pke_hdr.cd_pars.func_id = func_id;
		qat_req->input_param_count =
				QAT_ASYM_MODILW_NUM_IN_PARAMS;
		qat_req->output_param_count =
				QAT_ASYM_MODILW_NUM_OUT_PARAMS;
#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
		QAT_DP_HEXDUMP_LOG(DEBUG, "ModIlw base",
				cookie->input_array[0],
				alg_size_in_bytes);
		QAT_DP_HEXDUMP_LOG(DEBUG, "ModIlw modulus",
				cookie->input_array[1],
				alg_size_in_bytes);
#endif
		...
		if (asym_op->rsa.op_type == RTE_CRYPTO_ASYM_OP_ENCRYPT ||
			...
			if (xform->rsa.key_type == RTE_RSA_KET_TYPE_QT) {
				...
				rte_memcpy(cookie->input_array[5] +
						(alg_size_in_bytes >> 1) -
						xform->rsa.qt.qIlw.length
						, xform->rsa.qt.qIlw.data,
						xform->rsa.qt.qIlw.length);
				cookie->alg_size = alg_size;
				qat_req->pke_hdr.cd_pars.func_id = func_id;
			...
			__rte_unused enum qat_device_gen qat_dev_gen)
{
	...
	if (op->sess_type == RTE_CRYPTO_OP_WITH_SESSION) {
		...
		if (err) {
			op->status = RTE_CRYPTO_OP_STATUS_ILWALID_ARGS;
			goto error;
		}
		...
		struct rte_crypto_asym_xform *xform)
{
	...
	if (xform->xform_type == RTE_CRYPTO_ASYM_XFORM_MODEX) {
		...
		rte_crypto_param n = xform->modilw.modulus;

		alg_size = cookie->alg_size;
		alg_size_in_bytes = alg_size >> 3;
		uint8_t *modilw_result = asym_op->modilw.result.data;

		if (rx_op->status == RTE_CRYPTO_OP_STATUS_NOT_PROCESSED) {
			rte_memcpy(modilw_result + (asym_op->modilw.result.length
				- n.length),
				cookie->output_array[0] + alg_size_in_bytes
				- n.length, n.length);
			rx_op->status = RTE_CRYPTO_OP_STATUS_SUCCESS;
#if RTE_LOG_DP_LEVEL >= RTE_LOG_DEBUG
			QAT_DP_HEXDUMP_LOG(DEBUG, "ModIlw result",
					cookie->output_array[0],
					alg_size_in_bytes);
#endif
		}
		...
		struct rte_mempool *mempool)
{
	...
	if (xform->xform_type == RTE_CRYPTO_ASYM_XFORM_MODEX) {
		...
		if (xform->modilw.modulus.length == 0) {
			QAT_LOG(ERR, "Invalid mod ilw input parameter");
			err = -EILWAL;
			goto error;
		}
	} else if (xform->xform_type == RTE_CRYPTO_ASYM_XFORM_RSA) {
