...
#include <openssl/sha.h>	/* Needed to callwlate pre-compute values */
#include <openssl/aes.h>	/* Needed to callwlate pre-compute values */
#include <openssl/md5.h>	/* Needed to callwlate pre-compute values */
						...
						uint32_t cipherkeylen)
{
	...
	enum icp_qat_hw_cipher_colwert key_colwert;
	enum qat_sym_proto_flag qat_proto_flag =
		QAT_CRYPTO_PROTO_FLAG_NONE;
	...
	if (cdesc->qat_cmd == ICP_QAT_FW_LA_CMD_CIPHER) {
		...
		ICP_QAT_FW_COMN_LWRR_ID_SET(cipher_cd_ctrl,
		...
		cdesc->cd_lwr_ptr = (uint8_t *)&cdesc->cd;
		...
		ICP_QAT_FW_COMN_LWRR_ID_SET(hash_cd_ctrl,
					ICP_QAT_FW_SLICE_AUTH);
	...
	cipher_offset = cdesc->cd_lwr_ptr-((uint8_t *)&cdesc->cd);
	cipher_cd_ctrl->cipher_cfg_offset = cipher_offset >> 3;

	header->service_cmd_id = cdesc->qat_cmd;
	qat_sym_session_init_common_hdr(header, qat_proto_flag);

	cipher = (struct icp_qat_hw_cipher_algo_blk *)cdesc->cd_lwr_ptr;
	cipher->cipher_config.val =
	    ICP_QAT_HW_CIPHER_CONFIG_BUILD(cdesc->qat_mode,
					cdesc->qat_cipher_alg, key_colwert,
					cdesc->qat_dir);

	if (cdesc->qat_cipher_alg == ICP_QAT_HW_CIPHER_ALGO_KASUMI) {
		temp_key = (uint32_t *)(cdesc->cd_lwr_ptr +
		...
		cdesc->cd_lwr_ptr += sizeof(struct icp_qat_hw_cipher_config) +
	...
	if (total_key_size > cipherkeylen) {
		...
		if ((cdesc->qat_cipher_alg == ICP_QAT_HW_CIPHER_ALGO_3DES)
			...
			memcpy(cdesc->cd_lwr_ptr, cipherkey, padding_size);
		} else if ((cdesc->qat_cipher_alg == ICP_QAT_HW_CIPHER_ALGO_3DES)
			&& (cipherkeylen == QAT_3DES_KEY_SZ_OPT3)) {
			/* K2 and K3 not provided so use K1 = K2 = K3*/
			memcpy(cdesc->cd_lwr_ptr, cipherkey,
				cipherkeylen);
			memcpy(cdesc->cd_lwr_ptr+cipherkeylen,
				cipherkey, cipherkeylen);
		} else
			memset(cdesc->cd_lwr_ptr, 0, padding_size);

		cdesc->cd_lwr_ptr += padding_size;
	}
	cd_size = cdesc->cd_lwr_ptr-(uint8_t *)&cdesc->cd;
						...
						unsigned int operation)
{
	 ...
	 */
	hash_offset = cdesc->cd_lwr_ptr-((uint8_t *)&cdesc->cd);
	hash = (struct icp_qat_hw_auth_setup *)cdesc->cd_lwr_ptr;
	...
	cdesc->cd_lwr_ptr += sizeof(struct icp_qat_hw_auth_setup);

	/*
	 * cd_lwr_ptr now points at the state1 information.
	 */
	switch (cdesc->qat_hash_alg) {
	case ICP_QAT_HW_AUTH_ALGO_SHA1:
		if (cdesc->auth_mode == ICP_QAT_HW_AUTH_MODE0) {
			/* Plain SHA-1 */
			rte_memcpy(cdesc->cd_lwr_ptr, sha1InitialState,
		...
		if (qat_sym_do_precomputes(ICP_QAT_HW_AUTH_ALGO_SHA1, authkey,
			authkeylen, cdesc->cd_lwr_ptr, &state1_size,
			cdesc->aes_cmac)) {
	...
	case ICP_QAT_HW_AUTH_ALGO_SHA224:
		if (cdesc->auth_mode == ICP_QAT_HW_AUTH_MODE0) {
			/* Plain SHA-224 */
			rte_memcpy(cdesc->cd_lwr_ptr, sha224InitialState,
	...
	case ICP_QAT_HW_AUTH_ALGO_SHA256:
		if (cdesc->auth_mode == ICP_QAT_HW_AUTH_MODE0) {
			/* Plain SHA-256 */
			rte_memcpy(cdesc->cd_lwr_ptr, sha256InitialState,
	...
	case ICP_QAT_HW_AUTH_ALGO_SHA384:
		if (cdesc->auth_mode == ICP_QAT_HW_AUTH_MODE0) {
			/* Plain SHA-384 */
			rte_memcpy(cdesc->cd_lwr_ptr, sha384InitialState,
	...
	case ICP_QAT_HW_AUTH_ALGO_SHA512:
		if (cdesc->auth_mode == ICP_QAT_HW_AUTH_MODE0) {
			/* Plain SHA-512 */
			rte_memcpy(cdesc->cd_lwr_ptr, sha512InitialState,
	...
	case ICP_QAT_HW_AUTH_ALGO_AES_XCBC_MAC:
		...
		if (cdesc->aes_cmac)
			memset(cdesc->cd_lwr_ptr, 0, state1_size);
		if (qat_sym_do_precomputes(ICP_QAT_HW_AUTH_ALGO_AES_XCBC_MAC,
			authkey, authkeylen, cdesc->cd_lwr_ptr + state1_size,
	...
	case ICP_QAT_HW_AUTH_ALGO_GALOIS_64:
		...
		aad_len = (uint32_t *)(cdesc->cd_lwr_ptr +
	...
	case ICP_QAT_HW_AUTH_ALGO_ZUC_3G_128_EIA3:
		...
		memcpy(cdesc->cd_lwr_ptr + state1_size, authkey, authkeylen);
	...
	case ICP_QAT_HW_AUTH_ALGO_KASUMI_F9:
		...
		memset(cdesc->cd_lwr_ptr, 0, state1_size + state2_size);
		pTempKey = (uint32_t *)(cdesc->cd_lwr_ptr + state1_size
							+ authkeylen);
		/*
	...
	cdesc->cd_lwr_ptr += state1_size + state2_size + cd_extra_size;
	cd_size = cdesc->cd_lwr_ptr-(uint8_t *)&cdesc->cd;
...
#ifdef RTE_LIB_SELWRITY
static int
qat_sec_session_check_docsis(struct rte_selwrity_session_conf *conf)
{
	struct rte_crypto_sym_xform *crypto_sym = conf->crypto_xform;
	struct rte_selwrity_docsis_xform *docsis = &conf->docsis;

	/* CRC generate -> Cipher encrypt */
	if (docsis->direction == RTE_SELWRITY_DOCSIS_DOWNLINK) {
	...
	} else if (docsis->direction == RTE_SELWRITY_DOCSIS_UPLINK) {
...
qat_sec_session_set_docsis_parameters(struct rte_cryptodev *dev,
		struct rte_selwrity_session_conf *conf, void *session_private)
{
	int ret;
	int qat_cmd_id;
...
int
qat_selwrity_session_create(void *dev,
				...
				struct rte_mempool *mempool)
{
	...
	if (conf->action_type != RTE_SELWRITY_ACTION_TYPE_LOOKASIDE_PROTOCOL ||
			conf->protocol != RTE_SELWRITY_PROTOCOL_DOCSIS) {
...
int
qat_selwrity_session_destroy(void *dev __rte_unused,
				 struct rte_selwrity_session *sess)
{
