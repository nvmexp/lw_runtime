...
static const struct rte_cryptodev_capabilities
					qat_selwrity_sym_capabilities[] = {
	QAT_SELWRITY_SYM_CAPABILITIES,
	RTE_CRYPTODEV_END_OF_CAPABILITIES_LIST()
};

static const struct rte_selwrity_capability qat_selwrity_capabilities[] = {
	QAT_SELWRITY_CAPABILITIES(qat_selwrity_sym_capabilities),
	{
...
qat_selwrity_cap_get(void *device __rte_unused)
{
	return qat_selwrity_capabilities;
}

static struct rte_selwrity_ops selwrity_qat_ops = {

		.session_create = qat_selwrity_session_create,
		.session_update = NULL,
		.session_stats_get = NULL,
		.session_destroy = qat_selwrity_session_destroy,
		.set_pkt_metadata = NULL,
		.capabilities_get = qat_selwrity_cap_get
};
#endif
		...
		struct qat_dev_cmd_param *qat_dev_cmd_param __rte_unused)
{
	...
	selwrity_instance = rte_malloc("qat_sec",
				sizeof(struct rte_selwrity_ctx),
				RTE_CACHE_LINE_SIZE);
	if (selwrity_instance == NULL) {
		QAT_LOG(ERR, "rte_selwrity_ctx memory alloc failed");
		ret = -ENOMEM;
		goto error;
	}

	selwrity_instance->device = (void *)cryptodev;
	selwrity_instance->ops = &selwrity_qat_ops;
	selwrity_instance->sess_cnt = 0;
	cryptodev->selwrity_ctx = selwrity_instance;
	cryptodev->feature_flags |= RTE_CRYPTODEV_FF_SELWRITY;
#endif
