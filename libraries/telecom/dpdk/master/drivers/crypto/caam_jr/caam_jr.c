...
/* virtual address colwersin when mempool support is available for ctx */
...
is_auth_cipher(struct caam_jr_session *ses)
{
	return ((ses->cipher_alg != RTE_CRYPTO_CIPHER_NULL) &&
		(ses->auth_alg != RTE_CRYPTO_AUTH_NULL) &&
		(ses->proto_alg != RTE_SELWRITY_PROTOCOL_IPSEC));
}

static inline int
is_proto_ipsec(struct caam_jr_session *ses)
{
	return (ses->proto_alg == RTE_SELWRITY_PROTOCOL_IPSEC);
}
		  ...
		  uint32_t *notified_descs)
{
	...
	while (jobs_no_to_discard > discarded_descs_no) {
		 ...
		 */
		job_ring->cidx = SEC_CIRLWLAR_COUNTER(job_ring->cidx,
					 SEC_JOB_RING_SIZE);
		 ...
		 struct caam_jr_qp *jr_qp)
{
	...
	struct job_descriptor *lwrrent_desc;
	phys_addr_t lwrrent_desc_addr;
	phys_addr_t *temp_addr;
	struct caam_jr_op_ctx *ctx;

	/* TODO check for ops have memory*/
	/* check here if any JR error that cannot be written
	 * in the output status word has oclwrred
	 */
	...
	while (jobs_no_to_notify > notified_descs_no) {
		...
		lwrrent_desc_addr = (phys_addr_t)sec_read_addr(temp_addr);

		real_poll++;
		/* todo check if it is false alarm no desc present */
		if (!lwrrent_desc_addr) {
			false_alarm++;
		...
		lwrrent_desc = (struct job_descriptor *)
				caam_jr_dma_ptov(lwrrent_desc_addr);
		/* now increment the consumer index for the current job ring,
		 * AFTER saving job in temporary location!
		 */
		job_ring->cidx = SEC_CIRLWLAR_COUNTER(job_ring->cidx,
		...
		ctx = container_of(lwrrent_desc, struct caam_jr_op_ctx, jobdes);
		if (unlikely(sec_error_code)) {
...
err1:
	...
	ring->pidx = SEC_CIRLWLAR_COUNTER(ring->pidx, SEC_JOB_RING_SIZE);
			  ...
			  void *sess)
{
	...
	if (ipsec_xform->direction == RTE_SELWRITY_IPSEC_SA_DIR_EGRESS) {
		...
		session->ip4_hdr.ip_p = (ipsec_xform->proto ==
				RTE_SELWRITY_IPSEC_SA_PROTO_ESP) ? IPPROTO_ESP
				: IPPROTO_AH;
...
static int
caam_jr_selwrity_session_create(void *dev,
				struct rte_selwrity_session_conf *conf,
...
/* Clear the memory of session so it doesn't leave key material behind */
static int
caam_jr_selwrity_session_destroy(void *dev __rte_unused,
				 struct rte_selwrity_session *sess)
{
	PMD_INIT_FUNC_TRACE();
...
static struct rte_selwrity_ops caam_jr_selwrity_ops = {
	.session_create = caam_jr_selwrity_session_create,
	.session_update = NULL,
	.session_stats_get = NULL,
	.session_destroy = caam_jr_selwrity_session_destroy,
	.set_pkt_metadata = NULL,
	.capabilities_get = caam_jr_get_selwrity_capabilities
};
 ...
 *
 * @retval ::0			is returned for a successful exelwtion
		 ...
		 struct rte_cryptodev_pmd_init_params *init_params)
{
	...
	/*TODO free it during teardown*/
	selwrity_instance = rte_malloc("caam_jr",
	...
	selwrity_instance->device = (void *)dev;
	selwrity_instance->ops = &caam_jr_selwrity_ops;
	selwrity_instance->sess_cnt = 0;
	dev->selwrity_ctx = selwrity_instance;
