...
cpt_caps_add(const struct rte_cryptodev_capabilities *caps, int nb_caps)
{
	...
	if (lwr_pos + nb_caps > OTX2_CPT_MAX_CAPS)
		return;

	memcpy(&otx2_cpt_caps[lwr_pos], caps, nb_caps * sizeof(caps[0]));
	lwr_pos += nb_caps;
}
...
sec_caps_add(const struct rte_cryptodev_capabilities *caps, int nb_caps)
{
	...
	if (lwr_pos + nb_caps > OTX2_SEC_MAX_CAPS)
		return;

	memcpy(&otx2_cpt_sec_caps[lwr_pos], caps, nb_caps * sizeof(caps[0]));
	lwr_pos += nb_caps;
}
