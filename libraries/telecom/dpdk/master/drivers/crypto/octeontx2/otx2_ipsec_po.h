...
static inline int
ipsec_po_xform_aead_verify(struct rte_selwrity_ipsec_xform *ipsec,
...
static inline int
ipsec_po_xform_verify(struct rte_selwrity_ipsec_xform *ipsec,
		      struct rte_crypto_sym_xform *xform)
{
...
static inline int
ipsec_po_sa_ctl_set(struct rte_selwrity_ipsec_xform *ipsec,
		    struct rte_crypto_sym_xform *xform,
		    struct otx2_ipsec_po_sa_ctl *ctl)
{
	...
	if (ipsec->mode == RTE_SELWRITY_IPSEC_SA_MODE_TRANSPORT)
		ctl->ipsec_mode = OTX2_IPSEC_PO_SA_MODE_TRANSPORT;
	else if (ipsec->mode == RTE_SELWRITY_IPSEC_SA_MODE_TUNNEL)
		ctl->ipsec_mode = OTX2_IPSEC_PO_SA_MODE_TUNNEL;
	else
		return -EILWAL;

	if (ipsec->proto == RTE_SELWRITY_IPSEC_SA_PROTO_AH)
		ctl->ipsec_proto = OTX2_IPSEC_PO_SA_PROTOCOL_AH;
	else if (ipsec->proto == RTE_SELWRITY_IPSEC_SA_PROTO_ESP)
