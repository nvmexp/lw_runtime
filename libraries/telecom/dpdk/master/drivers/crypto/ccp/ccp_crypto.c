...
/* callwlate CCP descriptors requirement */
static inline int
		       ...
		       uint16_t *total_nb_ops)
{
	...
	*total_nb_ops = b_info->total_nb_ops;
	lwr_head_offset = CCP_READ_REG(b_info->cmd_q->reg_base,
				       CMD_Q_HEAD_LO_BASE);

	if (b_info->head_offset < b_info->tail_offset) {
		if ((lwr_head_offset >= b_info->head_offset) &&
		    (lwr_head_offset < b_info->tail_offset)) {
			qp->b_info = b_info;
			return 0;
		}
