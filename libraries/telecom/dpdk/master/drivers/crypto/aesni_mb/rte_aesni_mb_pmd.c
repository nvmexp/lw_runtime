...
/**
 * Callwlate the authentication pre-computes
 *
 * @param one_block_hash	Function pointer to callwlate digest on ipad/opad
 ...
 */
static void
callwlate_auth_precomputes(hash_one_block_t one_block_hash,
		...
		const struct rte_crypto_sym_xform *xform)
{
	...
	/* Callwlate Authentication precomputes */
	if (key_larger_block_size) {
		callwlate_auth_precomputes(hash_oneblock_fn,
...
static int
check_docsis_sec_session(struct rte_selwrity_session_conf *conf)
{
...
aesni_mb_set_docsis_sec_session_auth_parameters(struct aesni_mb_session *sess,
		struct rte_selwrity_docsis_xform *xform)
{
	if (xform == NULL) {
	...
	if (xform->direction == RTE_SELWRITY_DOCSIS_UPLINK) {
		sess->auth.algo = IMB_AUTH_DOCSIS_CRC32;
		sess->auth.operation = RTE_CRYPTO_AUTH_OP_VERIFY;
	} else if (xform->direction == RTE_SELWRITY_DOCSIS_DOWNLINK) {
		sess->auth.algo = IMB_AUTH_DOCSIS_CRC32;
		sess->auth.operation = RTE_CRYPTO_AUTH_OP_GENERATE;
	} else {
		...
		uint32_t oop)
{
	...
	/* Only cipher then hash needs special callwlation. */
	...
	struct rte_crypto_sym_vec *vec)
{
	...
	if (s == NULL) {
		aesni_mb_fill_error_code(vec, EILWAL);
		return 0;
	}
			...
			struct rte_cryptodev_pmd_init_params *init_params)
{
	...
	selwrity_instance = rte_malloc("aesni_mb_sec",
				sizeof(struct rte_selwrity_ctx),
				RTE_CACHE_LINE_SIZE);
	if (selwrity_instance == NULL) {
		AESNI_MB_LOG(ERR, "rte_selwrity_ctx memory alloc failed");
		rte_cryptodev_pmd_destroy(dev);
		return -ENOMEM;
	}

	selwrity_instance->device = (void *)dev;
	selwrity_instance->ops = rte_aesni_mb_pmd_sec_ops;
	selwrity_instance->sess_cnt = 0;
	dev->selwrity_ctx = selwrity_instance;
	dev->feature_flags |= RTE_CRYPTODEV_FF_SELWRITY;
#endif
