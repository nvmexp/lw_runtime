...
process_op_bit(struct rte_crypto_op *op, struct snow3g_session *session,
		struct snow3g_qp *qp, uint16_t *aclwmulated_enqueued_ops)
{
		...
		uint16_t nb_ops)
{
	...
	struct snow3g_session *prev_sess = NULL, *lwrr_sess = NULL;
	...
	for (i = 0; i < nb_ops; i++) {
		...
		lwrr_sess = snow3g_get_session(qp, lwrr_c_op);
		if (unlikely(lwrr_sess == NULL ||
				lwrr_sess->op == SNOW3G_OP_NOT_SUPPORTED)) {
			lwrr_c_op->status =
		...
		if (((lwrr_c_op->sym->cipher.data.length % BYTE_LEN) != 0)
				|| ((lwrr_c_op->sym->cipher.data.offset
					% BYTE_LEN) != 0)) {
