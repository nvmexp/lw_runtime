...
{
	int ret;
	ret = rte_rawdev_get_dev_id("ilwalid_rawdev_device");
...
test_rawdev_info_get(void)
{
	...
	RTE_TEST_ASSERT(ret == -EILWAL, "Expected -EILWAL, %d", ret);
...
test_rawdev_configure(void)
{
	...
	RTE_TEST_ASSERT(ret == -EILWAL,
			"Null configure; Expected -EILWAL, got %d", ret);
...
/* After exelwting test_rawdev_queue_setup, queue_id=0 would have depth as 15.
