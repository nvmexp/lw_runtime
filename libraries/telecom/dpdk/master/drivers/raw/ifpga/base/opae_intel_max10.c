...
static int
max10_selwre_hw_init(struct intel_max10_device *dev)
{
...
static int
max10_non_selwre_hw_init(struct intel_max10_device *dev)
{
	max10_check_capability(dev);
		...
		int chipselect)
{
	...
	if (dev->flags & MAX10_FLAGS_SELWRE)
		ret = max10_selwre_hw_init(dev);
	else
		ret = max10_non_selwre_hw_init(dev);
	if (ret) {
