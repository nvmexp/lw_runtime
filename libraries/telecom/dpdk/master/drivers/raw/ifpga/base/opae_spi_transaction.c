...
static int byte_to_core_colwert(struct spi_transaction_dev *dev,
		...
		unsigned int *valid_resp_len)
{
	...
	for (i = 0; i < send_len; i++) {
		lwrrent_byte = send_data[i];
		switch (lwrrent_byte) {
		case SPI_BYTE_IDLE:
			*p++ = SPI_BYTE_ESC;
			*p++ = xor_20(lwrrent_byte);
			break;
		case SPI_BYTE_ESC:
			*p++ = SPI_BYTE_ESC;
			*p++ = xor_20(lwrrent_byte);
...
static int packet_to_byte_colwer(struct spi_transaction_dev *dev,
		...
		unsigned int *valid)
{
	...
	*p++ = 0;
	...
	ret = byte_to_core_colwert(dev, p - send_packet,
			send_packet, resp_max_len, resp_packet,
			&valid_resp_len);
	if (ret)
		return -EBUSY;

	print_buffer("after byte colwer:", resp_packet, valid_resp_len);
	...
	while (i < valid_resp_len) {
		...
		case SPI_PACKET_EOP:
			i++;
			lwrrent_byte = resp_packet[i];
			if (lwrrent_byte == SPI_PACKET_ESC ||
					lwrrent_byte == SPI_PACKET_CHANNEL ||
					lwrrent_byte == SPI_PACKET_SOP) {
				i++;
				lwrrent_byte = resp_packet[i];
				*p++ = xor_20(lwrrent_byte);
			} else
		...
		unsigned int trans_type)
{
	...
	case SPI_TRAN_NON_SEQ_WRITE:
		...
		ret = packet_to_byte_colwer(dev, size + HEADER_LEN,
	...
	case SPI_TRAN_NON_SEQ_READ:
		ret = packet_to_byte_colwer(dev, HEADER_LEN,
