		...
		size_t config_size __rte_unused)
{
	...
	RTE_FUNC_PTR_OR_ERR_RET(dev, -EILWAL);
...
#define MAX_PORT_PER_RETIMER    4
	...
	/* lwrrently, eth_group_num is always 2 */
	eth_group_num = opae_manager_get_eth_group_nums(mgr);
	if (eth_group_num < 0)
		return -1;

	if (!strcmp(attr_name, "LineSideBaseMAC")) {
		/* Lwrrently FPGA not implement, so just set all zeros*/
...
fme_err_handle_nonfaterror(struct opae_manager *mgr)
{
	...
	if (nonfaterr.temp_thresh_ap1)
		...
		IFPGA_RAWDEV_PMD_INFO("an error has oclwrred in pcie\n");
	else if (nonfaterr.portfatal_error)
		IFPGA_RAWDEV_PMD_INFO("fatal error oclwrred in AFU port.\n");
...
fme_interrupt_handler(void *param)
{
	...
	IFPGA_RAWDEV_PMD_INFO("%s interrupt oclwrred\n", __func__);
