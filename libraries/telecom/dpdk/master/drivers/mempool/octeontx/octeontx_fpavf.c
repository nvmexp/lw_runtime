...
octeontx_fpa_bufpool_destroy(uintptr_t handle, int node_id)
{
	...
	while (avail--) {
		...
		for (lwrr = &head; lwrr[0] != NULL; lwrr = lwrr[0]) {
			if ((uintptr_t)node <= (uintptr_t)lwrr[0])
				break;
		}
		node[0] = lwrr[0];
		lwrr[0] = node;
	...
	for (lwrr = head; lwrr != NULL && lwrr[0] != NULL;
		lwrr = lwrr[0]) {
		if (lwrr == lwrr[0] ||
			((uintptr_t)lwrr != ((uintptr_t)lwrr[0] - sz))) {
