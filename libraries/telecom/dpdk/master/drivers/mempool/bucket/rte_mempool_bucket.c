...
{
	struct bucket_stack *lwr_stack = bd->buckets[rte_lcore_id()];
	unsigned int n_buckets_from_stack = RTE_MIN(n_buckets, lwr_stack->top);
	void **obj_table_base = obj_table;

	n_buckets -= n_buckets_from_stack;
	while (n_buckets_from_stack-- > 0) {
		void *obj = bucket_stack_pop_unsafe(lwr_stack);
			     ...
			     unsigned int n)
{
	...
	struct bucket_stack *lwr_stack = bd->buckets[rte_lcore_id()];
	unsigned int n_buckets_from_stack = RTE_MIN(n, lwr_stack->top);
	struct bucket_header *hdr;
	...
	while (n_buckets_from_stack-- > 0) {
		hdr = bucket_stack_pop_unsafe(lwr_stack);
...
bucket_alloc(struct rte_mempool *mp)
{
	...
	if (rc < 0 || rc >= (int)sizeof(rg_name)) {
		rc = -ENAMETOOLONG;
		goto ilwalid_shared_orphan_ring;
	}
	...
	if (rc < 0 || rc >= (int)sizeof(rg_name)) {
		rc = -ENAMETOOLONG;
		goto ilwalid_shared_bucket_ring;
	}
		     ...
		     size_t *align)
{
	...
	*min_total_elt_size = bucket_page_sz;
	/*
	 * Each bucket oclwpies its own block aligned to
