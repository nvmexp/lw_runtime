 ...
 * the "irq en/disable" machinery isn't relwrsive...).
 */
#define FQLOCK(fq) \
	do { \
...
struct qman_portal {
	 ...
	 * to 2 ** 10 to ensure DQRR index callwlations based shadow copy
...
static inline int qm_eqcr_init(struct qm_portal *portal,
			       ...
			       int eq_stash_prio)
{
	/* This use of 'register', as well as all other oclwrrences, is because
			...
			enum qm_dqrr_cmode cmode, u8 max_fill)
{
	...
	dqrr->cmode = cmode;
#endif
	/* Ilwalidate every ring entry before beginning */
		   ...
		   const struct qman_cgrs *cgrs)
{
	...
	qm_isr_disable_write(p, isdr);
	if (qm_dqrr_lwrrent(p)) {
		pr_err("Qman DQRR unclean\n");
		qm_dqrr_cdc_consume_n(p, 0xffff);
	}
	if (qm_mr_lwrrent(p) && drain_mr_fqrni(p)) {
...
void qman_destroy_portal(struct qman_portal *qm)
{
	 ...
	 * path, this update will only ilwalidate the CI cacheline *after*
 ...
 *
 * Note also that the callbacks are ilwoked at points which are safe against the
	...
	case QM_MCR_NP_STATE_PARKED:
		...
		if (result == QM_MCR_RESULT_PENDING) {
			...
			if (channel < qm_channel_pool1)
				...
				qm_dqrr_sdqcr_set(low_p,
						  QM_SDQCR_TYPE_ACTIVE |
						  QM_SDQCR_CHANNELS_POOL_COLW
						  (channel));
			while (!found_fqrn) {
				/* Keep draining DQRR while checking the MR*/
				qm_dqrr_pvb_update(low_p);
				dqrr = qm_dqrr_lwrrent(low_p);
				while (dqrr) {
				...
				while (msg) {
					if ((msg->ern.verb &
						...
						found_fqrn = 1;
					qm_mr_next(low_p);
					qm_mr_cci_consume_to_lwrrent(low_p);
					qm_mr_pvb_update(low_p);
					msg = qm_mr_lwrrent(low_p);
				}
				cpu_relax();
