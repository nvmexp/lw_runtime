...
#define __bm_cl_ilwalidate(bm, o) dccivac((bm)->ce + (o))
...
#define bm_cl_ilwalidate(reg)\
	__bm_cl_ilwalidate(&portal->addr, BM_CL_##reg##_CENA)
...
static inline void bm_rcr_pvb_commit(struct bm_portal *portal, u8 myverb)
{
	...
	lwsync();
	rlwrsor = rcr->cursor;
	rlwrsor->__dont_write_directly__verb = myverb | rcr->vbit;
	dcbf_64(rlwrsor);
	RCR_INC(rcr);
	rcr->available--;
#ifdef RTE_LIBRTE_DPAA_HWDEBUG
