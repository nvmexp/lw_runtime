...
#define __qm_cl_ilwalidate(qm, o) dccivac((qm)->ce + (o))
...
#define qm_cl_ilwalidate(reg)\
	__qm_cl_ilwalidate(&portal->addr, QM_CL_##reg##_CENA)
...
/* Bit-wise logic to colwert a ring pointer to a ring index */
...
#endif
	lwsync();
	eqlwrsor = eqcr->cursor;
	eqlwrsor->__dont_write_directly__verb = myverb | eqcr->vbit;
	dcbf(eqlwrsor);
	EQCR_INC(eqcr);
	eqcr->available--;
#ifdef RTE_LIBRTE_DPAA_HWDEBUG
...
static inline const struct qm_dqrr_entry *qm_dqrr_lwrrent(
...
static inline u8 qm_dqrr_lwrsor(struct qm_portal *portal)
{
	register struct qm_dqrr *dqrr = &portal->dqrr;
...
static inline void qm_dqrr_cci_consume_to_lwrrent(struct qm_portal *portal)
{
	register struct qm_dqrr *dqrr = &portal->dqrr;
...
static inline void qm_dqrr_cce_consume_to_lwrrent(struct qm_portal *portal)
{
	register struct qm_dqrr *dqrr = &portal->dqrr;
...
static inline void qm_dqrr_park_lwrrent(struct qm_portal *portal)
{
	register struct qm_dqrr *dqrr = &portal->dqrr;
...
static inline const struct qm_mr_entry *qm_mr_lwrrent(struct qm_portal *portal)
{
	register struct qm_mr *mr = &portal->mr;
...
static inline void qm_mr_cci_consume_to_lwrrent(struct qm_portal *portal)
{
	register struct qm_mr *mr = &portal->mr;
