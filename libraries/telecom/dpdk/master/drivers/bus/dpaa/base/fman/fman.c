...
fman_if_init(const struct device_node *dpa_node)
{
	...
	if (!regs_addr) {
		FMAN_ERR(-EILWAL, "of_get_address(%s)\n", mname);
		goto err;
	}
	phys_addr = of_translate_address(mac_node, regs_addr);
	if (!phys_addr) {
		FMAN_ERR(-EILWAL, "of_translate_address(%s, %p)\n",
			 mname, regs_addr);
		goto err;
	}
	...
	/* Get rid of endianness (issues). Colwert to host byte order */
	...
	if (of_device_is_compatible(mac_node, "fsl,fman-1g-mac"))
		...
		FMAN_ERR(-EILWAL, "%s: unknown MAC type\n", mname);
		goto err;
	}
	...
	if (_errno) {
		FMAN_ERR(-EILWAL, "Invalid register address: %" PRIx64,
			 regs_addr_host);
		goto err;
	}
	...
	mac_addr = of_get_property(mac_node, "local-mac-address",
				   &lenp);
	if (!mac_addr) {
		FMAN_ERR(-EILWAL, "%s: no local-mac-address\n",
			 mname);
		goto err;
	}
	...
	if (!ports_phandle)
		ports_phandle = of_get_property(mac_node, "fsl,fman-ports",
						&lenp);
	if (!ports_phandle) {
		FMAN_ERR(-EILWAL, "%s: no fsl,port-handles\n",
			 mname);
		goto err;
	}
	...
	tx_channel_id = of_get_property(tx_node, "fsl,qman-channel-id",
					&lenp);
	if (!tx_channel_id) {
		FMAN_ERR(-EILWAL, "%s: no fsl-qman-channel-id\n",
			 tx_node->full_name);
		goto err;
	}
	...
	if (!rx_phandle) {
		FMAN_ERR(-EILWAL, "%s: no fsl,qman-frame-queues-rx\n", dname);
		goto err;
	}
	...
	if (!tx_phandle) {
		FMAN_ERR(-EILWAL, "%s: no fsl,qman-frame-queues-tx\n", dname);
		goto err;
	}
	...
	pools_phandle = of_get_property(dpa_node, "fsl,bman-buffer-pools",
					&lenp);
	if (!pools_phandle) {
		FMAN_ERR(-EILWAL, "%s: no fsl,bman-buffer-pools\n", dname);
		goto err;
	}
