...
/* Enable/disable Rx promislwous mode on specified interface */
__rte_internal
void fman_if_promislwous_enable(struct fman_if *p);
__rte_internal
void fman_if_promislwous_disable(struct fman_if *p);
...
/* Set interface next ilwoked action for dequeue operation */
