...
/* for colwersion from n of qm_channel */
static inline u32 QM_SDQCR_CHANNELS_POOL_COLW(u16 channel)
{
...
enum qm_fd_format {
	 ...
	 * interpreted purely for congestion callwlations, ie. a "congestion
	 * weight".
	 */
	qm_fd_contig = 0,
...
struct qm_fd {
	 ...
	 * other union elements. Note, union'd structs are diffilwlt to use with
 ...
 * latter has two inlines to assist with colwerting to/from the mant+exp
 * representation.
 */
...
/* 64-bit colwerters for context_hi/lo */
...
/* colwert a threshold value into mant+exp representation */
 ...
 * and associated commands/responses. The WRED parameters are callwlated from
 ...
 * how we represent it here. The congestion state threshold is callwlated from
 * these fields as follows;
...
/* Colwert CGR thresholds to/from "cs_thres" format */
...
struct qm_mcc_alterfq {
	...
	u32 fqid;	/* 24-bit */
	u8 __reserved2;
	u8 count;	/* number of conselwtive FQID */
	u8 __reserved3[10];
 ...
 * -EILWAL if the current CPU is sharing a portal hosted on another CPU.
 */
__rte_internal
 ...
 * or -EILWAL if the current CPU is sharing a portal hosted on another CPU.
 */
__rte_internal
 ...
 * this function will return -EILWAL, otherwise the return value is >=0 and
 ...
 * fast-path (which ilwolves demuxing dequeue ring (DQRR) entries and tracking
 * enqueue ring (EQCR) consumption), and slow-path (which ilwolves EQCR
 ...
 * entire register is returned, so if only the lwrrently-enabled pool channels
 ...
 * cpu that is lwrrently exelwting the function. For reasons of locking, this
 ...
 * the VDQCR command has finished exelwting (ie. once the callback for the last
 ...
 * function returns -EAGAIN if the congestion group is lwrrently congested.
 ...
 * portal affine to the cpu portal on which this API is exelwted. If opts is
 ...
 * is exelwted. This must be exlwted on the same affine portal on which it was
 * created.
 */
__rte_internal
 ...
 * the OOS state - this function polls for that condition. It is partilwlarly
