...
/* The lock needs to be relwrsive because a vdev can manage another vdev. */
static rte_spinlock_relwrsive_t vdev_device_list_lock =
	RTE_SPINLOCK_RELWRSIVE_INITIALIZER;

static struct vdev_driver_list vdev_driver_list =
	TAILQ_HEAD_INITIALIZER(vdev_driver_list);

struct vdev_lwstom_scan {
	TAILQ_ENTRY(vdev_lwstom_scan) next;
	rte_vdev_scan_callback callback;
	void *user_arg;
};
TAILQ_HEAD(vdev_lwstom_scans, vdev_lwstom_scan);
static struct vdev_lwstom_scans vdev_lwstom_scans =
	TAILQ_HEAD_INITIALIZER(vdev_lwstom_scans);
static rte_spinlock_t vdev_lwstom_scan_lock = RTE_SPINLOCK_INITIALIZER;
...
int
rte_vdev_add_lwstom_scan(rte_vdev_scan_callback callback, void *user_arg)
{
	struct vdev_lwstom_scan *lwstom_scan;

	rte_spinlock_lock(&vdev_lwstom_scan_lock);

	/* check if already registered */
	TAILQ_FOREACH(lwstom_scan, &vdev_lwstom_scans, next) {
		if (lwstom_scan->callback == callback &&
				lwstom_scan->user_arg == user_arg)
			break;
	}

	if (lwstom_scan == NULL) {
		lwstom_scan = malloc(sizeof(struct vdev_lwstom_scan));
		if (lwstom_scan != NULL) {
			lwstom_scan->callback = callback;
			lwstom_scan->user_arg = user_arg;
			TAILQ_INSERT_TAIL(&vdev_lwstom_scans, lwstom_scan, next);
		}
	}

	rte_spinlock_unlock(&vdev_lwstom_scan_lock);

	return (lwstom_scan == NULL) ? -1 : 0;
}

int
rte_vdev_remove_lwstom_scan(rte_vdev_scan_callback callback, void *user_arg)
{
	struct vdev_lwstom_scan *lwstom_scan, *tmp_scan;

	rte_spinlock_lock(&vdev_lwstom_scan_lock);
	TAILQ_FOREACH_SAFE(lwstom_scan, &vdev_lwstom_scans, next, tmp_scan) {
		if (lwstom_scan->callback != callback ||
				(lwstom_scan->user_arg != (void *)-1 &&
				lwstom_scan->user_arg != user_arg))
			continue;
		TAILQ_REMOVE(&vdev_lwstom_scans, lwstom_scan, next);
		free(lwstom_scan);
	}
	rte_spinlock_unlock(&vdev_lwstom_scan_lock);
...
rte_vdev_init(const char *name, const char *args)
{
	...
	rte_spinlock_relwrsive_lock(&vdev_device_list_lock);
	...
	rte_spinlock_relwrsive_unlock(&vdev_device_list_lock);
	return ret;
}
...
scan:
	...
	TAILQ_FOREACH(lwstom_scan, &vdev_lwstom_scans, next) {
		if (lwstom_scan->callback != NULL)
			 ...
			 */
			lwstom_scan->callback(lwstom_scan->user_arg);
	}
	rte_spinlock_unlock(&vdev_lwstom_scan_lock);
