...
struct qbman_fle {
	...
	uint32_t frc;
	uint32_t reserved[3]; /* Not used lwrrently */
};
 ...
 * Need to call colwersion routines dpaa2_mem_vtop & dpaa2_mem_ptov
 * wherever required.
...
/**
 * macro to colwert Virtual address to IOVA
 */
#define DPAA2_VADDR_TO_IOVA(_vaddr) dpaa2_mem_vtop((size_t)(_vaddr))

/**
 * macro to colwert IOVA to Virtual address
 */
#define DPAA2_IOVA_TO_VADDR(_iova) dpaa2_mem_ptov((size_t)(_iova))

/**
 * macro to colwert modify the memory containing IOVA to Virtual address
 */
