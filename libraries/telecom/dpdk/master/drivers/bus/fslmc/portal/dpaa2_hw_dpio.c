...
#define COMMAND_LEN	50
	...
	if (ret < 0)
		...
		DPAA2_BUS_DEBUG(" %s command is exelwted", command);
...
static int dpaa2_dpio_intr_init(struct dpaa2_dpio_dev *dpio_dev, int cpu_id)
{
	...
	if (getelw("DPAA2_PORTAL_INTR_THRESHOLD"))
		threshold = atoi(getelw("DPAA2_PORTAL_INTR_THRESHOLD"));

	if (getelw("DPAA2_PORTAL_INTR_TIMEOUT"))
		sscanf(getelw("DPAA2_PORTAL_INTR_TIMEOUT"), "%x", &timeout);
			 ...
			 int object_id)
{
	...
	if (!dpaa2_portal_key) {
		/* create the key, supplying a function that'll be ilwoked
