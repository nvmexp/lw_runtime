...
/**
 * qbman_swp_ilwalidate() - Ilwalidate the cache enabled area of the QBMan
 * portal. This is required to be called if a portal moved to another core
 * because the QBMan portal area is non coherent
 * @p: the qbman_swp object to be ilwalidated
 *
 */
void qbman_swp_ilwalidate(struct qbman_swp *p);
 ...
 * colwersion to ensure that the user's dequeue result storage is in host-endian
 ...
 * if another dequeue command has been exelwted to produce a new result to that
 * location).
 *
...
/**
 * qbman_result_is_CGLW() - Check for Congestion Group Count Update.
 * @dq: the qbman_result object to check.
 *
 * Return 1 if this is CGLW.
 */
int qbman_result_is_CGLW(const struct qbman_result *dq);
...
/* Parsing CGLW */
/**
 * qbman_result_cglw_cgid() - Check CGLW resouce id, i.e. cgid
 * @scn: the state change notification.
 *
 * Return the CGLW resource id.
 */
uint16_t qbman_result_cglw_cgid(const struct qbman_result *scn);

/**
 * qbman_result_cglw_icnt() - Get the I_CNT from CGLW
 * @scn: the state change notification.
 *
 * Return instantaneous count in the CGLW notification.
 */
uint64_t qbman_result_cglw_icnt(const struct qbman_result *scn);
 ...
 * a pull dequeue of that channel oclwrs that selects that FQ for dequeuing,
