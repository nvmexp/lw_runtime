...
struct qbman_swp *qbman_swp_init(const struct qbman_swp_desc *d)
{
	 ...
	 * error.  The values that were callwlated above will be
...
void *qbman_swp_mc_result(struct qbman_swp *p)
{
	...
	if ((p->desc.qman_version & QMAN_REV_MASK) >= QMAN_REV_5000
		...
		qbman_cena_ilwalidate_prefetch(&p->sys,
...
const struct qbman_result *qbman_swp_dqrr_next_direct(struct qbman_swp *s)
{
	...
	if (s->dqrr.reset_bug) {
		 ...
		 * ilwalidate and read *only* once that PI has indicated that
		...
		qbman_cena_ilwalidate_prefetch(&s->sys,
					QBMAN_CENA_SWP_DQRR(s->dqrr.next_idx));
	}
	p = qbman_cena_read_wo_shadow(&s->sys,
			QBMAN_CENA_SWP_DQRR(s->dqrr.next_idx));
	 ...
	 * and we've ilwalidated the cacheline before reading it, so the
...
{
	return __qbman_result_is_x(dq, QBMAN_RESULT_CGLW);
}
