...
/* Lwrrently, the CENA support code expects each 32-bit word to be written in
 * host order, and these are colwerted to hardware (little-endian) order on
...
static inline void qbman_cena_ilwalidate(struct qbman_swp_sys *s,
					 uint32_t offset)
{
	dccivac(s->addr_cena + offset);
}

static inline void qbman_cena_ilwalidate_prefetch(struct qbman_swp_sys *s,
						  uint32_t offset)
{
	dccivac(s->addr_cena + offset);
