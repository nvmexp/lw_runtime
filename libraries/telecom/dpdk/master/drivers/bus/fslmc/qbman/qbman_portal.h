...
struct qbman_swp {
	...
	struct {
		 ...
		 * the last-submitted command is already exelwting in the
		 * hardware (as evidenced by at least 1 valid dequeue result),
		 * you can write another dequeue command to the register, the
		 * hardware will start exelwting it as soon as the
		 * already-exelwting command terminates. (This minimises latency
		 * and stalls.) With that in mind, this "busy" variable refers
		 * to whether or not a command can be submitted, not whether or
		 * not a previously-submitted command is still exelwting. In
		 * other words, once proof is seen that the previously-submitted
		 * command is exelwting, "vdq" is no longer "busy".
		 */
		atomic_t busy;
		uint32_t valid_bit; /* 0x00 or 0x80 */
 ...
 * exposed. The following macro is used within the driver for colwerting *any*
