	...
	uint64_t *end_addr, uint64_t *flags)
{
	union pci_resource_info {
		...
		char *ptrs[PCI_RESOURCE_FMT_LWAL];
	} res_info;
...
parse_pci_addr_format(const char *buf, int bufsize, struct rte_pci_addr *addr)
{
	...
	union splitaddr {
		...
		char *str[PCI_FMT_LWAL]; /* last element-separator is "." not ":" */
	} splitaddr;
	...
	if (rte_strsplit(buf_copy, bufsize, splitaddr.str, PCI_FMT_LWAL, ':')
			!= PCI_FMT_LWAL - 1)
		goto error;
...
{
	/*
	 * IOMMU is always present on a PowerLW host (IOMMUv2).
	 * IOMMU is also present in a KVM/QEMU VM (IOMMUv1) but is not
	 * lwrrently supported by DPDK. Test for our current environment
	...
	/* Check for a PowerLW platform */
	while (getline(&line, &len, fp) != -1) {
		if (strstr(line, "platform") != NULL)
			continue;

		if (strstr(line, "PowerLW") != NULL) {
			RTE_LOG(DEBUG, EAL, "Running on a PowerLW system\n");
			ret = true;
			break;
		}
	}
