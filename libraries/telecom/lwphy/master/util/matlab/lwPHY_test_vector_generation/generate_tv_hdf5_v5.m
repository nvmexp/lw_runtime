...
function generate_tv_hdf5_v5(test_case_name, L_BS, L_UE, N0_data, snr_db, time_data_mask, TF_received_signal, s_grid, sd, s, gnb_pars, tb_pars, chEst_pars, qam, j_slot,sp)
...
N0_data = N0_data + 10^(-4); %add noise floor
...
%ChEst filter:
if chEst_pars.filterArray
   %filter idx:
   lwrrentSnr = sp.sim.channel.lwrrentSnr;
   snrGrid = chEst_pars.snrGrid;
   [~,filter_idx] = min(abs(snrGrid - lwrrentSnr));
   
   %load filter:
    W_middle = chEst_pars.W_middle_array{filter_idx,1};
    W_upper = chEst_pars.W_upper_array{filter_idx,1};
    W_lower = chEst_pars.W_lower_array{filter_idx,1};
else
    W_middle = chEst_pars.W_middle;
    W_upper = chEst_pars.W_upper;
    W_lower = chEst_pars.W_lower;
end
...
if newEq
    ...
    if sp.gnb.pusch.reciever.equal.ChEstError
        Rnn_ilw = compute_total_error_ilw(lwrrentSnr,sp);
        N0_est = complex(Rnn_ilw,zeros(size(Rnn_ilw)));
    else  
...
tvName = sprintf('TV_lwphy_%s_snrdb%2.2f_MIMO%dx%d_PRB%d_DataSyms%d_qam%d.h5',test_case_name, snr_db, nLayers, nBSAnts, nDataPRB, nDataSyms, 2^qam);
...
%"cast" recieved signal to fp16:
for i = 1 : size(TF_received_signal,3)
    
    R = real(TF_received_signal(:,:,i));
    R = double(vfp16(R));
    
    I = imag(TF_received_signal(:,:,i));
    I = double(vfp16(I));
    
    TF_received_signal(:,:,i) = R + 1i*I;
end
    
    

% channel estimation inputs
hdf5_write_lw(h5File, 'DataRx', single(TF_received_signal(1:(12*Nprb),:,:)),'fp16'); 
hdf5_write_lw(h5File, 'WFreq', single(WFreq));
hdf5_write_lw(h5File, 'DescrShiftSeq', single(sd));
hdf5_write_lw(h5File, 'ShiftSeq', single(s_grid));
