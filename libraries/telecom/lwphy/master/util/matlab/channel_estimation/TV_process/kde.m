...
% now apply the ilwerse discrete cosine transform
...
if nargout>3
    ...
    % now get values of cdf on grid points using IDCT and lwmsum function

    a_cdf=a.*exp(-[0:n-1]'.^2*pi^2*t_cdf/2);

    cdf=lwmsum(idct1d(a_cdf))*(dx/R);
...
% computes the ilwerse discrete cosine transform
